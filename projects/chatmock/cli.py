...
def cmd_login(no_browser: bool, verbose: bool) -> int:
    ...
    if not client_id:
        logger.error("No OAuth client id configured. Set CHATGPT_LOCAL_CLIENT_ID.")
        return 1
    ...
    with httpd:
        logger.info(f"Starting local login server on {URL_BASE}")
        if not no_browser:
            try:
                webbrowser.open(auth_url, new=1, autoraise=True)
            except Exception as e:
                logger.warning(f"Failed to open browser: {e}")
        logger.info(f"If your browser did not open, navigate to:\n{auth_url}")
        ...
        def _stdin_paste_worker() -> None:
            try:
                ...
                try:
                    ...
                    if not code:
                        logger.error("Input did not contain an auth code. Ignoring.")
                        return
                    if state and state != httpd.state:
                        logger.error("State mismatch. Ignoring pasted URL for safety.")
                        return
                    logger.info("Received redirect URL. Completing login without callback…")
                    bundle, _ = httpd.exchange_code(code)
                    if httpd.persist_auth(bundle):
                        httpd.exit_code = 0
                        logger.info("Login successful. Tokens saved.")
                    else:
                        logger.error("Unable to persist auth file.")
                    httpd.shutdown()
                except Exception as exc:
                    logger.error(f"Failed to process pasted redirect URL: {exc}")
            except Exception:
                pass
...
def cmd_serve(
    ...
    logger.info(f"Starting ChatMock server on {host}:{port}")
    app.run(host=host, debug=False, use_reloader=False, port=port, threaded=True)
    return 0


def cmd_chat(model: str, verbose: bool = False) -> int:
    from .interactive import InteractiveChat
    chat = InteractiveChat(model=model, verbose=verbose)
    return chat.run()
...
def main() -> None:
    ...
    p_chat.add_argument("--model", default="gpt-5", help="Model to use for chat (default: gpt-5)")
    p_chat.add_argument("--verbose", action="store_true", help="Enable verbose logging")
    ...
    # Setup logging globally based on verbosity of command
    verbose_arg = getattr(args, 'verbose', False)
    setup_logging(verbose=verbose_arg)
    ...
    if args.command == "login":
        ...
        sys.exit(cmd_chat(model=args.model, verbose=args.verbose))
