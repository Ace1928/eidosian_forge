...
def ollama_generate() -> Response:
    ...
    if not isinstance(model, str) or not isinstance(prompt, str):
        ...
        err = {"error": "Invalid request format: model and prompt are required"}
        if verbose:
            _log_json("OUT POST /api/generate", err)
        return jsonify(err), 400
    ...
    # Convert to input items
    input_items = convert_chat_messages_to_responses_input(messages)
    
    # Handle tools if passed in options or extension fields (less common in generate but possible)
    tools_responses = []
    ...
    if upstream.status_code >= 400:
        ...
        if verbose:
            print("/api/generate upstream error status=", upstream.status_code, " body:", json.dumps(err_body)[:2000])
        err = {"error": (err_body.get("error", {}) or {}).get("message", "Upstream error")}
        if verbose:
            _log_json("OUT POST /api/generate", err)
        return jsonify(err), upstream.status_code
    ...
    if stream_req:
        def _gen():
            compat = (current_app.config.get("REASONING_COMPAT", "think-tags") or "think-tags").strip().lower()
            # ... streaming logic (simplified from chat, generate usually returns 'response' field) ...
            full_response = ""
            think_open = False
            think_closed = False
            ...
            try:
                for raw_line in upstream.iter_lines(decode_unicode=False):
                    ...
                    if kind == "response.output_text.delta":
                        delta = evt.get("delta") or ""
                    elif kind == "response.reasoning_summary_text.delta" or kind == "response.reasoning_text.delta":
                         # Reuse reasoning handling logic if needed, or just append if compat=think-tags
                         delta = evt.get("delta") or ""
                    ...
                    if delta:
                        # For generate, we stream 'response' field
                        yield json.dumps({
                            "model": model_out, 
                            "created_at": created_at, 
                            "response": delta, 
                ...
                done_obj = {
                    ...
                    "total_duration": _OLLAMA_FAKE_EVAL["total_duration"],
                    "load_duration": _OLLAMA_FAKE_EVAL["load_duration"],
                    "prompt_eval_count": 0,
        ...
        resp = current_app.response_class(stream_iter, status=200, mimetype="application/x-ndjson")
        for k, v in build_cors_headers().items():
            resp.headers.setdefault(k, v)
        return resp
