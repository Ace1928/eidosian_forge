...
__kernel void calculate_time(
    ...
    // Read alpha values
    float alpha_t = alpha_values[0];
    float alpha_Temporal_Scale = alpha_values[1];

    // Calculate partial time
    float partial_time = alpha_t * temporal_scales[global_id];
    local_time[local_id] = partial_time;

    // Perform local reduction
    barrier(CLK_LOCAL_MEM_FENCE);
    for (int stride = group_size / 2; stride > 0; stride /= 2) {
        if (local_id < stride) {
            local_time[local_id] += local_time[local_id + stride];
        }
        barrier(CLK_LOCAL_MEM_FENCE);
    }
    ...
    if (local_id == 0) {
        atomic_add_f(result, local_time[0]);
    }
}
