...
class KernelManager:
    def __init__(self, opencl_manager: OpenCLManager):
        """
        Initializes the KernelManager with the provided OpenCLManager.
        Logs detailed information about each step and handles errors comprehensively.

        Args:
            opencl_manager (OpenCLManager): The OpenCLManager instance to use for managing OpenCL resources.
        """
        logging.debug("Initializing KernelManager.")
        self.opencl_manager = opencl_manager
        self.context = opencl_manager.context
        self.queue = opencl_manager.queue
        self.programs = opencl_manager.programs
        logging.debug(f"KernelManager initialized with context: {self.context}, queue: {self.queue}, programs: {self.programs}")
    ...
    def calculate_entropy(self, probabilities: np.ndarray, alpha_parameters: Dict[str, float] = alpha_parameters) -> float:
        ...
        try:
            # Convert probabilities to numpy array with float32 type
            ...
            # Copy result from OpenCL buffer to host
    ...
    def calculate_mutual_information(self, H_X: np.ndarray, H_Y: np.ndarray, H_XY: np.ndarray) -> float:
        ...
        try:
            ...
            # Enqueue kernel execution
            self.programs['mutual_information'].calculate_mutual_information(
                self.queue, 
                H_X.shape, 
                None, 
                H_X_buf, 
    ...
    def calculate_operational_efficiency(self, P: np.ndarray, E: np.ndarray) -> float:
        ...
        try:
            ...
            # Create alpha values buffer
            alpha_values = np.array([1.0, 1.0, 1.0], dtype=np.float32)  # Example alpha values
            alpha_values_buf = cl.Buffer(self.context, cl.mem_flags.READ_ONLY | cl.mem_flags.COPY_HOST_PTR, hostbuf=alpha_values)
            logging.debug(f"Created OpenCL buffer for alpha values: {alpha_values_buf}")

            # Enqueue kernel execution
            self.programs['operational_efficiency'].calculate_operational_efficiency(
                self.queue, 
                P.shape, 
                None, 
                P_buf, 
    ...
    def calculate_error_management(self, error_detection_rate: np.ndarray, correction_capability: np.ndarray) -> float:
        ...
        try:
            ...
            # Enqueue kernel execution
            self.programs['error_management'].calculate_error_management(
                self.queue, 
                error_detection_rate.shape, 
                None, 
                error_detection_rate_buf, 
                correction_capability_buf, 
                alpha_values_buf, 
                result_buf
            )
            ...
            # Calculate error management effectiveness value
    ...
    def calculate_adaptability(self, adaptation_rate: np.ndarray) -> float:
        ...
        try:
            # Convert adaptation_rate to numpy array with float32 type
            ...
            # Create alpha values buffer
            alpha_values = np.array([1.0, 1.0], dtype=np.float32)  # Example alpha values
            alpha_values_buf = cl.Buffer(self.context, cl.mem_flags.READ_ONLY | cl.mem_flags.COPY_HOST_PTR, hostbuf=alpha_values)
            logging.debug(f"Created OpenCL buffer for alpha values: {alpha_values_buf}")

            # Enqueue kernel execution
            self.programs['adaptability'].calculate_adaptability(
                self.queue, 
                adaptation_rate.shape, 
                None, 
                adaptation_rate_buf, 
                alpha_values_buf, 
                result_buf
    ...
    def calculate_volume(self, spatial_scale: np.ndarray) -> float:
        ...
        try:
            # Convert spatial_scale to numpy array with float32 type
            ...
            # Enqueue kernel execution
            self.programs['volume'].calculate_volume(
                self.queue, 
                spatial_scale.shape, 
                None, 
                spatial_scale_buf, 
    ...
    def calculate_time(self, temporal_scale: np.ndarray) -> float:
        ...
        try:
            # Convert temporal_scale to numpy array with float32 type
            ...
            # Enqueue kernel execution
            self.programs['time'].calculate_time(
                self.queue, 
                temporal_scale.shape, 
                None, 
                temporal_scale_buf, 
                alpha_values_buf, 
                result_buf
...
def main():
    ...
    if os.path.exists('parameter_ranges.npz'):
        ...
        logging.debug(f"Generated {len(probabilities_set)} probability distributions: {probabilities_set}")
    ...
    logging.debug(f"Generated {len(all_combinations)} combinations of input parameters.")
