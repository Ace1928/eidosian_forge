...
int main() {
    ...
    std::vector<float> biases1(hidden_size);
    std::vector<float> hidden(hidden_size);
    std::vector<float> weights2(hidden_size * output_size);
    std::vector<float> biases2(output_size);
    ...
    cl_mem bufWeights1 = clCreateBuffer(context, CL_MEM_READ_ONLY, input_size * hidden_size * sizeof(float), NULL, &err);
    check_error(err, "clCreateBuffer bufWeights1");
    cl_mem bufBiases1 = clCreateBuffer(context, CL_MEM_READ_ONLY, hidden_size * sizeof(float), NULL, &err);
    check_error(err, "clCreateBuffer bufBiases1");
    cl_mem bufHidden = clCreateBuffer(context, CL_MEM_READ_WRITE, hidden_size * sizeof(float), NULL, &err);
    check_error(err, "clCreateBuffer bufHidden");
    cl_mem bufWeights2 = clCreateBuffer(context, CL_MEM_READ_ONLY, hidden_size * output_size * sizeof(float), NULL, &err);
    check_error(err, "clCreateBuffer bufWeights2");
    cl_mem bufBiases2 = clCreateBuffer(context, CL_MEM_READ_ONLY, output_size * sizeof(float), NULL, &err);
    ...
    err = clEnqueueWriteBuffer(queue, bufWeights1, CL_TRUE, 0, input_size * hidden_size * sizeof(float), weights1.data(), 0, NULL, NULL);
    check_error(err, "clEnqueueWriteBuffer bufWeights1");
    err = clEnqueueWriteBuffer(queue, bufBiases1, CL_TRUE, 0, hidden_size * sizeof(float), biases1.data(), 0, NULL, NULL);
    check_error(err, "clEnqueueWriteBuffer bufBiases1");
    err = clEnqueueWriteBuffer(queue, bufWeights2, CL_TRUE, 0, hidden_size * output_size * sizeof(float), weights2.data(), 0, NULL, NULL);
    check_error(err, "clEnqueueWriteBuffer bufWeights2");
    err = clEnqueueWriteBuffer(queue, bufBiases2, CL_TRUE, 0, output_size * sizeof(float), biases2.data(), 0, NULL, NULL);
    ...
    err = clSetKernelArg(kernel, 3, sizeof(cl_mem), &bufHidden);
    ...
    err = clSetKernelArg(kernel, 6, sizeof(cl_mem), &bufOutput);
    check_error(err, "clSetKernelArg 6");
    err = clSetKernelArg(kernel, 7, sizeof(int), &input_size);
    check_error(err, "clSetKernelArg 7");
    err = clSetKernelArg(kernel, 8, sizeof(int), &hidden_size);
    check_error(err, "clSetKernelArg 8");
    err = clSetKernelArg(kernel, 9, sizeof(int), &output_size);
    check_error(err, "clSetKernelArg 9");

    // Execute the kernel
    size_t global_work_size[1] = { std::max(hidden_size, output_size) };
