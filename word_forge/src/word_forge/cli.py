...
class WordForgeCLI(StandardCLI):
    ...
    def _cmd_lookup(self, args) -> None:
        ...
        try:
            ...
            if not data:
                result = CommandResult(True, f"No data found for '{args.word}'", {"word": args.word, "found": False})
            else:
    ...
    def _cmd_define(self, args) -> None:
        ...
        try:
            ...
            if not synsets:
                result = CommandResult(
                    True, f"No definitions found for '{args.word}'", {"word": args.word, "definitions": []}
                )
            else:
                definitions = []
    ...
    def _cmd_related(self, args) -> None:
        ...
        try:
            ...
            if not synsets:
                result = CommandResult(True, f"No synsets found for '{args.word}'", {"word": args.word, "related": []})
            else:
                related = set()
    ...
    def _cmd_graph(self, args) -> None:
        ...
        try:
            ...
            result = CommandResult(True, f"Graph stats: {data.get('word_count', 'N/A')} words in database", data)
        except Exception as e:
    ...
    def _cmd_build(self, args) -> None:
        ...
        try:
            ...
            if not data:
                result = CommandResult(
                    True, f"No lexical data generated for '{args.word}'", {"word": args.word, "built": False}
                )
            else:
                result = CommandResult(
                    True,
