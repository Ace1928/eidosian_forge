...
def test_emotional_subgraph_context_filtering(tmp_path):
    ...
    manager.add_relationship(a, c, "sadness_associated", dimension="emotional", weight=1.0)

    manager.integrate_emotional_context("clinical", {"sadness_associated": 0.2, "joy_associated": 1.0})

    sub = manager.get_emotional_subgraph("happy", depth=1, context="clinical", min_intensity=0.5)
