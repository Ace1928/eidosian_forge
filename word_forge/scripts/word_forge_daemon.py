...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(sys.stdout), logging.FileHandler("word_forge_daemon.log")],
)
...
def main():
    ...
    word_processor = WordProcessor(db_manager=db_manager, parser_refiner=parser_refiner, logger=LOGGER)
    ...
    vector_worker = VectorWorker(
        db=db_manager, vector_store=vector_store, embedder=vector_store.model_name, poll_interval=5.0
    )

    # Emotion Manager & Worker
    emotion_manager = EmotionManager(db_manager)
    emotion_worker = EmotionWorker(db=db_manager, emotion_manager=emotion_manager, poll_interval=10.0)
