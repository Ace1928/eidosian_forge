...
class DocumentationDiscovery:
    ...
    def _identify_orphans(self) -> None:
        ...
        structure_dirs = {
            str(path) for path in self.doc_structure.values() if hasattr(path, "exists") and path.exists()
        }
        ignored_dirs = [
        ...
        for _, docs_list in self.documents.items():
            for doc in docs_list:
                ...
                if isinstance(items_list, list):
                    items_list.append(
                        {"title": doc.title, "url": doc.url, "priority": doc.priority, "category": doc.category}
                    )
    ...
    def _analyze_orphan_content(self, orphan: Path, section_patterns: Dict[str, List[str]]) -> Tuple[str, str]:
        ...
        if best_score < 2:
            try:
                ...
                for sect, patterns in section_patterns.items():
                    section_score = sum(content.count(pat) * (3 if pat in content[:500] else 1) for pat in patterns)
    ...
    def find_all_sources(self) -> Dict[str, List[Path]]:
        all_sources: Dict[str, List[Path]] = {"user": [], "auto": [], "ai": [], "orphaned": []}
        user_docs_dir = self.doc_structure.get("user_docs", self.docs_dir / "user_docs")
        if user_docs_dir.exists():
            for ext in self.doc_extensions:
                all_sources["user"].extend(
                    p for p in user_docs_dir.glob(f"**/*{ext}") if not any(part.startswith("_") for part in p.parts)
                )
        auto_docs_dir = self.doc_structure.get("auto_docs", self.docs_dir / "auto_docs")
        autoapi_dirs = [auto_docs_dir, self.docs_dir / "autoapi", self.docs_dir / "apidoc", self.docs_dir / "reference"]
        for directory in autoapi_dirs:
            if directory.exists():
                for ext in self.doc_extensions:
                    all_sources["auto"].extend(
                        p for p in directory.glob(f"**/*{ext}") if not any(part.startswith("_") for part in p.parts)
                    )
        ai_docs_dir = self.doc_structure.get("ai_docs", self.docs_dir / "ai_docs")
        if ai_docs_dir.exists():
            for ext in self.doc_extensions:
                all_sources["ai"].extend(
                    p for p in ai_docs_dir.glob(f"**/*{ext}") if not any(part.startswith("_") for part in p.parts)
                )
...
def discover_code_structures(src_dir: Optional[Path] = None) -> List[Dict[str, Any]]:
    ...
    for file_path in python_files:
        try:
            ...
            discovered_items.append({"name": module_name, "type": "module", "file": str(file_path), "doc_ready": True})
...
if __name__ == "__main__":
    ...
    print("📚 Documentation Discovery Report:")
    ...
    if args.output:
        if args.format == "json":
            ...
            with open(args.output, "w") as f:
                json.dump(
                    {cat: [{"title": d.title, "path": str(d.path)} for d in dl] for cat, dl in discovered.items()},
                    f,
                    indent=2,
                )
