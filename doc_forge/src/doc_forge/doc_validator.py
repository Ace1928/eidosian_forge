...
from .source_discovery import discover_code_structures, discover_documentation
...
class DocumentationValidator:
    ...
    def _validate_required_docs(self) -> None:
        ...
        if getting_started_dir.exists():
            ...
            if user_docs_getting_started.exists():
                ...
                self.discrepancies["missing_docs"].append(
                    "Missing 'Getting Started' section (should be at docs/getting_started/ or docs/user_docs/getting_started/)"
                )
        ...
        for path, description in required_docs:
            if not path.exists():
                self.discrepancies["missing_docs"].append(
                    f"Missing {description} at {path.relative_to(self.repo_root)}"
                )
    ...
    def _validate_structure(self) -> None:
            ...
            for dir_path, doc_list in dir_docs.items():
                ...
                if not index_found:
                    ...
                    self.discrepancies["structural_issues"].append(
                        f"Directory {rel_path} has multiple docs but no index file"
                    )
        ...
        for dir_path in all_dirs:
            ...
            if dir_path.name.lower() in standard_dirs or dir_path.name in [
                "user_docs",
                "auto_docs",
                "ai_docs",
                "assets",
            ...
            try:
                ...
                for section, alternatives in required_sections.items():
                    if not any(alt in content for alt in alternatives):
                        self.discrepancies["quality_issues"].append(f"README.md missing '{section}' section")
            except Exception:
                ...
                try:
                    ...
                    if doc.path.suffix == ".md":
                        if not re.search(r"^#\s+", content, re.MULTILINE):
                            rel_path = doc.path.relative_to(self.repo_root)
                            self.discrepancies["quality_issues"].append(f"Document missing headings: {rel_path}")
                            quality_issues += 1
