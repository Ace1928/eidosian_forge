...
def fix_ambiguous_references(repo_root: Path) -> None:
    ...
    for rst_file in autoapi_dir.glob("**/*.rst"):
        ...
        if (
            "more than one target" in content
            or ":exc:" in content
            or ":class:" in content
            and any(err in content for err in ["Error", "Exception"])
        ):
...
def fix_file_references(file_path: Path) -> None:
    ...
    for exception in exception_classes:
        ...
        if exception not in ["ValueError", "TypeError"]:  # These are Python builtins - leave them be
            updated_content = re.sub(pattern, r":class:`ollama_forge.exceptions.\1`", updated_content)
        ...
        if exception not in ["ValueError", "TypeError"]:
            updated_content = re.sub(pattern, r":raises ollama_forge.exceptions.\1:", updated_content)
    ...
    for py_file in source_dir.glob("**/*.py"):
        if py_file.name == "__init__.py":
            ...
            if "from .exceptions import" in content:
                ...
                updated_content = re.sub(r"(from \.exceptions import .*?)(?:\n|$)", r"\1  # noqa: F401\n", content)
