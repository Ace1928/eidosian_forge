...
def add_test_subparsers(subparsers: Any) -> None:
    ...
    analyze_parser.add_argument("--repo-dir", type=Path, help="Repository directory (default: auto-detect)")
    analyze_parser.add_argument("--output", "-o", type=Path, help="Output directory (default: tests directory)")
    analyze_parser.set_defaults(func=cmd_analyze)

    # Generate test TODO
    todo_parser = subparsers.add_parser("todo", help="Generate test TODO document")
    todo_parser.add_argument("--repo-dir", type=Path, help="Repository directory (default: auto-detect)")
    todo_parser.add_argument("--output", "-o", type=Path, help="Output file (default: tests/TODO.md)")
    todo_parser.set_defaults(func=cmd_todo)

    # Generate test stubs
    stubs_parser = subparsers.add_parser("stubs", help="Generate test stubs")
    stubs_parser.add_argument("--repo-dir", type=Path, help="Repository directory (default: auto-detect)")
    stubs_parser.add_argument("--output", "-o", type=Path, help="Output directory (default: tests directory)")
    stubs_parser.set_defaults(func=cmd_stubs)

    # Generate test suite
    suite_parser = subparsers.add_parser("suite", help="Generate test suite")
    suite_parser.add_argument("--repo-dir", type=Path, help="Repository directory (default: auto-detect)")
    suite_parser.add_argument("--output", "-o", type=Path, help="Output directory (default: tests/generated_suite)")
    suite_parser.set_defaults(func=cmd_suite)

    # Run tests
    run_parser = subparsers.add_parser("run", help="Run tests")
    run_parser.add_argument("--repo-dir", type=Path, help="Repository directory (default: auto-detect)")
    run_parser.add_argument(
        "--pattern", type=str, default="test_*.py", help="Pattern for test files (default: test_*.py)"
    )
    run_parser.add_argument("--pytest", action="store_true", help="Use pytest instead of unittest")
    ...
    validate_parser.add_argument("--level", "-l", choices=["basic", "full"], default="basic", help="Validation level")
...
def cmd_analyze(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        logger.info("✨ Generated Eidosian test analysis suite:")
...
def cmd_stubs(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        logger.info(
            f"✅ Generated {stubs_info['stub_count']} test stub files covering {stubs_info['covered_items']} code items"
        )
        return 0
    except ImportError:
        logger.error("❌ Could not import ast_scanner module.")
