...
class TestFindAllSources:
    ...
    def mock_discovery(self):
        ...
        with mock.patch("doc_forge.source_discovery.get_repo_root") as mock_repo_root, mock.patch(
            "doc_forge.source_discovery.get_docs_dir"
        ) as mock_docs_dir, mock.patch("doc_forge.source_discovery.get_doc_structure") as mock_structure:
            ...
            discovery.orphaned_documents = [Path("/mock/repo/docs/orphaned1.md"), Path("/mock/repo/docs/orphaned2.rst")]
    ...
    def test_find_all_sources_empty_directories(self, mock_discovery):
        ...
        with mock.patch.object(Path, "exists") as mock_exists, mock.patch.object(Path, "glob") as mock_glob:
        ...
        with mock.patch.object(Path, "exists") as mock_exists, mock.patch.object(Path, "glob") as mock_glob:
            ...
            def mock_glob_side_effect(pattern):
                if "user_docs" in str(pattern):
                    ...
                    "auto_docs" in str(pattern)
                    or "autoapi" in str(pattern)
                    or "apidoc" in str(pattern)
                    or "reference" in str(pattern)
                ):
                    return [
    ...
    def test_find_all_sources_only_user_docs(self, mock_discovery):
        ...
        with mock.patch.object(Path, "exists") as mock_exists, mock.patch.object(Path, "glob") as mock_glob:
            ...
            def mock_glob_side_effect(pattern):
                if "user_docs" in str(pattern):
                    return [Path("/mock/repo/docs/user_docs/guide1.md"), Path("/mock/repo/docs/user_docs/guide2.rst")]
                return []
    ...
    def test_find_all_sources_integration(self, tmp_path):
        ...
        with mock.patch("doc_forge.source_discovery.get_doc_structure") as mock_structure:
            mock_structure.return_value = {"user_docs": user_docs, "auto_docs": auto_docs, "ai_docs": ai_docs}
