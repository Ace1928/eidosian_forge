...
while true; do
  ...
  connected="$(curl -fsS -H "X-API-Key: $API_KEY" "http://127.0.0.1:8384/rest/system/connections" | jq -r --arg rid "$REMOTE_ID" '.connections[$rid].connected // false')"
  echo "[$now] connected=$connected"

  all_done=1
  for folder in "${folders[@]}"; do
    row="$(curl -fsS -H "X-API-Key: $API_KEY" "http://127.0.0.1:8384/rest/db/completion?folder=${folder}&device=${REMOTE_ID}")"
    ...
    printf '  %-16s completion=%9s needBytes=%12s needItems=%8s remoteState=%s\n' "$folder" "$completion" "$need_bytes" "$need_items" "$remote_state"
  ...
  if [[ "$all_done" -eq 1 ]]; then
    echo "All monitored folders reached 100% completion."
    exit 0
  fi
