...
MEMORY_TREND_PATH = SCHEDULER_STATUS_PATH.parent / "memory_health_trends.json"
...
@eidosian()
def _append_memory_trend(payload: dict[str, Any]) -> dict[str, Any]:
    MEMORY_TREND_PATH.parent.mkdir(parents=True, exist_ok=True)
    existing: dict[str, Any] = {"contract": "eidos.memory_health_trends.v1", "entries": []}
    if MEMORY_TREND_PATH.exists():
        try:
            loaded = json.loads(MEMORY_TREND_PATH.read_text(encoding="utf-8"))
            if isinstance(loaded, dict):
                existing = loaded
        except Exception:
            existing = {"contract": "eidos.memory_health_trends.v1", "entries": []}
    entries = [row for row in (existing.get("entries") or []) if isinstance(row, dict)]
    entries.append(dict(payload))
    existing["contract"] = "eidos.memory_health_trends.v1"
    existing["entries"] = entries[-240:]
    MEMORY_TREND_PATH.write_text(json.dumps(existing, indent=2) + "\n", encoding="utf-8")
    return payload
...
def main() -> int:
    ...
    while True:
        ...
        try:
            ...
            _append_memory_trend(
                {
                    "updated_at": _now_utc(),
                    "cycle": cycle,
                    "run_id": manifest.get("run_id"),
                    "vector_count": int((((memory_report.get("reindex_report") or {}).get("vector_count")) or 0)),
                    "reindexed": int((((memory_report.get("reindex_report") or {}).get("reindexed")) or 0)),
                    "memory_enriched": int((((memory_report.get("enrich_report") or {}).get("updated")) or 0)),
                    "community_count": int(((memory_report.get("community_summary") or {}).get("count")) or 0),
                    "llm_used": bool(effective_memory_llm),
            ...
            _write_scheduler_status(
                {
                    ...
                    "summary": {
                        ...
                        "doc_forge": manifest.get("doc_forge"),
                        "graphrag": {
            ...
            coordinator.heartbeat(
                ...
                metadata={
                    ...
                    "consecutive_failures": consecutive_failures,
                    "last_error": str(exc),
                    "summary": {"budget_saturated": False},
                },
            )
            if args.once:
                return 1
