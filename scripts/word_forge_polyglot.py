...
def _setup_imports(repo_root: Path) -> None:
    for candidate in (repo_root / "word_forge" / "src", repo_root / "lib"):
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run Word Forge lexeme-level polyglot decomposition.")
    ...
    result = run_polyglot_decomposition(
        repo_root=repo_root,
        db_path=db_path,
        lang=args.lang,
        limit=args.limit,
