...
class KnowledgeForgeCLI(StandardCLI):
    ...
    def _cmd_list(self, args) -> None:
        ...
        try:
            ...
            result = CommandResult(True, f"Found {len(items)} nodes", {"nodes": items, "total": len(items)})
        except Exception as e:
    ...
    def _cmd_search(self, args) -> None:
        ...
        try:
            ...
            result = CommandResult(
                True, f"Found {len(results)} matching nodes", {"results": results, "count": len(results)}
            )
        except Exception as e:
    ...
    def _cmd_add(self, args) -> None:
        ...
        try:
            ...
            if not content:
                ...
                result = CommandResult(True, f"Added node {node.id[:8]}", {"id": node.id, "tags": list(node.tags)})
        except Exception as e:
    ...
    def _cmd_link(self, args) -> None:
        ...
        try:
            ...
            if not node_a:
                ...
                result = CommandResult(
                    True, f"Linked {node_a[:8]} ↔ {node_b[:8]}", {"linked": [node_a[:8], node_b[:8]]}
                )
        except Exception as e:
    ...
    def _cmd_concepts(self, args) -> None:
        ...
        try:
            ...
            result = CommandResult(True, f"Found {len(concepts)} concepts", {"concepts": concept_data})
        except Exception as e:
    ...
    def _cmd_unified(self, args) -> None:
        ...
        if not self.bridge:
            ...
            try:
                ...
                result = CommandResult(
                    True, f"Found {len(items)} results across knowledge and memory", {"results": items}
                )
            except Exception as e:
                result = CommandResult(False, f"Search error: {e}")
        self._output(result, args)
    ...
    def _cmd_stats(self, args) -> None:
        ...
        try:
            ...
            result = CommandResult(
                True, f"Graph: {data['nodes']} nodes, {data['concepts']} concepts, {data['total_links']} links", data
            )
        except Exception as e:
            result = CommandResult(False, f"Error: {e}")
        self._output(result, args)
    ...
    def _cmd_delete(self, args) -> None:
        ...
        try:
            ...
            if not node_id:
                ...
                if success:
                    result = CommandResult(True, f"Deleted node {node_id[:8]}", {"deleted": node_id[:8]})
                else:
