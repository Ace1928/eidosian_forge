"""Spatial utility functions for profiling and data layout experiments."""
...
@dataclass
class GridConfig:
    """Configuration for a uniform 2D spatial grid."""
    ...
    def from_world(
        ...
        n_particles: int,
        density_factor: float = 20.0,
    ) -> "GridConfig":
        """Build a grid config from world parameters."""
        ...
        avg_density = n_particles / float(grid_width * grid_height)
...
if HAS_NUMBA:
    ...
    def morton_encode_2d(x: int, y: int) -> int:
        """Encode 2D grid coordinates into Morton/Z-order code."""
    ...
    def morton_decode_2d(z: int) -> Tuple[int, int]:
        """Decode Morton/Z-order code into 2D grid coordinates."""
    ...
    def compute_morton_order(
        ...
        """Compute Morton ordering for particle indices."""
    ...
    def adaptive_cell_size(
        ...
        target_avg: float = 10.0,
        min_cell_size: float = 0.1,
        max_cell_size: float = 5.0,
    ) -> float:
        """Adapt cell size based on density stats."""

        if max_density > target_avg * 5:
            return min(current_cell_size * 1.2, max_cell_size)
        if avg_density < target_avg * 0.2:
    ...
    def pack_positions_soa(pos: np.ndarray, n_active: int) -> Tuple[np.ndarray, np.ndarray]:
        """Pack positions into Structure-of-Arrays layout."""
    ...
    @njit(fastmath=True, cache=True)
    def unpack_positions_aos(x_arr: np.ndarray, y_arr: np.ndarray, pos: np.ndarray, n_active: int) -> None:
        """Unpack positions from SoA back to AoS."""
    ...
    def compute_batch_ranges(n_active: int, batch_size: int) -> np.ndarray:
        """Return ranges for batch processing."""
    ...
    def prefetch_neighbor_data(
        ...
        """Prefetch neighbor data into contiguous arrays."""
    ...
    def _no_numba(*_args, **_kwargs):
        raise ImportError("numba is required for spatial utilities")
    ...
    def compute_cell_densities(grid_counts: np.ndarray) -> Tuple[float, float, int]:
        h, w = grid_counts.shape
        total = int(grid_counts.sum())
        max_count = int(grid_counts.max()) if total else 0
        empty_count = int(np.count_nonzero(grid_counts == 0))
        n_cells = h * w
        avg = float(total) / float(n_cells) if n_cells > 0 else 0.0
        return avg, float(max_count), empty_count
