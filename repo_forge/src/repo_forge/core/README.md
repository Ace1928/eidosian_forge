# `repo_forge/src/repo_forge/core`

<!-- EIDOS:DOCSYS:START -->
- Contract: `eidos.directory_doc.v1`
- Generated: `2026-03-13T00:19:25Z`
- Path: `repo_forge/src/repo_forge/core`

## What It Is

Managed directory documentation for `repo_forge/src/repo_forge/core`, generated from tracked files and directory structure.

## Why It Exists

This directory exists to hold the `core` surface for `repo_forge` and keep that responsibility separate from adjacent forge concerns.

## How It Works

- Tracked files in scope: `7`
- Child directories: `0`
- Tests detected: `False`
- Python modules: `diagnostics, directory, files, registry, templates, utils`

## Contents

- No managed child directories detected.

## Prominent Files

- [`__init__.py`](./__init__.py)
- [`diagnostics.py`](./diagnostics.py)
- [`directory.py`](./directory.py)
- [`files.py`](./files.py)
- [`registry.py`](./registry.py)
- [`templates.py`](./templates.py)
- [`utils.py`](./utils.py)

## Strengths

- The directory exposes importable Python modules rather than only opaque assets.

## Weaknesses / Risks

- No directly associated test coverage was detected under the tracked file set for this directory.

## Next Steps

- Add focused tests or point this directory explicitly at its validating test surface.
- Keep this README synchronized with code and test changes through the managed documentation toolchain.

## References

- Parent README: [`repo_forge/src/repo_forge/README.md`](../README.md)
- Forge README: [`repo_forge/README.md`](../../../README.md)

## Accuracy Contract

- This README is generated from tracked repository structure and conservative code scanning.
- Behavior claims should be backed by tests, routes, or directly linked source files.
<!-- EIDOS:DOCSYS:END -->
