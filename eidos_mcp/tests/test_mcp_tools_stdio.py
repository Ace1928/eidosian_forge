...
class TestMcpToolsStdio(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_tools_end_to_end(self) -> None:
        if SANDBOX.exists():
            shutil.rmtree(SANDBOX)
        ...
        async with streamable_http_client("http://127.0.0.1:8765/mcp") as (read, write, _):
            async with ClientSession(read, write) as session:
                ...
                self.assertIn("Committed", create_result)
                ...
                self.assertIn(type_register, {"Updated", "No-op: Schema unchanged"})
                ...
                self.assertIn("Stored memory", sem_add)
                ...
                self.assertIn(audit_add, {"Added", "No-op: Task already exists"})
