...
def test_sync_agent_tools(mock_list_tools, forge_manager):
    # Setup mock tools
    mock_list_tools.return_value = [{"name": "test_tool", "func": lambda: "ok", "description": "A test tool"}]
    ...
    mock_agent.register_tool.assert_called_once_with(
        "test_tool", mock_list_tools.return_value[0]["func"], "A test tool"
    )
...
def test_initialize_all(forge_manager):
    ...
    with patch.object(forge_manager, "register_routers") as mock_reg, patch.object(
        forge_manager, "sync_agent_tools"
    ) as mock_sync, patch.object(forge_manager, "load_plugins") as mock_load:
