...
def _get_live_controller():
    ...
    if _live_controller is None:
        from computer_control_forge.live_controller import ControllerConfig, LiveController

        config = ControllerConfig(
            enable_stdout=False, enable_file=True, file_path="/tmp/eidos_live.jsonl"  # Don't spam stdout in MCP context
        )
...
def control_get_cursor_position() -> Dict[str, Any]:
    ...
    try:
        ...
        return {"success": True, "x": x, "y": y, "timestamp": datetime.now().isoformat()}
...
def control_move_mouse_relative(dx: int, dy: int) -> Dict[str, Any]:
    ...
    """
    try:
        result = subprocess.run(["ydotool", "mousemove", "-x", str(dx), "-y", str(dy)], capture_output=True, timeout=5)
    ...
    try:
        if x is not None and y is not None:
            ...
            if not move_result.success:
                return {"success": False, "error": f"Failed to move to position: error={move_result.error}"}
        ...
        result = subprocess.run(["ydotool", "click", code], capture_output=True, timeout=5)
...
def control_type_text(text: str, delay_ms: int = 0) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"success": result.returncode == 0, "text_length": len(text), "timestamp": datetime.now().isoformat()}
    except Exception as e:
        return {"success": False, "error": str(e)}
...
def control_press_key(key: str) -> Dict[str, Any]:
    ...
    try:
        ...
        result = subprocess.run(["ydotool", "key", key_code], capture_output=True, timeout=5)

        return {"success": result.returncode == 0, "key": key, "timestamp": datetime.now().isoformat()}
    except Exception as e:
        return {"success": False, "error": str(e)}
...
def control_screenshot(output_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        result = subprocess.run(["spectacle", "-b", "-n", "-o", output_path], capture_output=True, timeout=10)
...
def control_system_status() -> Dict[str, Any]:
    ...
    try:
        x, y = _get_cursor_position()
        status["components"]["cursor_tracking"] = {"working": True, "position": {"x": x, "y": y}}
    except Exception as e:
...
def control_start_monitoring(duration_seconds: float = 10.0) -> Dict[str, Any]:
    ...
    try:
        ...
        events = ctrl.get_mcp_events(clear=True)
        return {"success": True, "duration": duration, "events": events["events"], "event_count": events["event_count"]}
    except Exception as e:
        return {"success": False, "error": str(e)}
