    ...
    def acquire(self, url: str, timeout: float = 30.0) -> bool:
        ...
        while time.time() < deadline:
            with self._lock:
                ...
                self._tokens[domain] = min(self.burst_size, self._tokens[domain] + elapsed * rate)
...
class WebCache:
    ...
    def __init__(self, cache_dir: Optional[Path] = None, default_ttl: int = 3600, max_size_mb: int = 100):  # 1 hour
        ...
        with self._lock:
            try:
                with sqlite3.connect(self.db_path) as conn:
                    ...
                    cur = conn.execute("SELECT * FROM cache WHERE url = ? AND expires_at > ?", (url, time.time()))
                    row = cur.fetchone()
            ...
            try:
                with sqlite3.connect(self.db_path) as conn:
                    if older_than:
                        ...
                        cur = conn.execute("DELETE FROM cache WHERE cached_at < ?", (threshold,))
                    else:
        ...
        with self._lock:
            try:
                with sqlite3.connect(self.db_path) as conn:
                    ...
                    # Valid entries
                    cur = conn.execute("SELECT COUNT(*) as count FROM cache WHERE expires_at > ?", (time.time(),))
...
_cache = WebCache(
    cache_dir=Path.home() / ".cache" / "eidosian_web", default_ttl=3600, max_size_mb=100  # 1 hour default
)
...
def web_fetch(
    ...
    try:
                ...
                return json.dumps(
                    {
                        ...
                        "content": (
                            text_content[:10000]
                            if text_content
                            else f"[Binary content: {len(cached['content'])} bytes]"
                        ),
        ...
        with urllib.request.urlopen(req, timeout=timeout) as response:
            ...
            # Store in cache
            if use_cache:
                _cache.put(url, content, content_type, response_headers, status_code, ttl=cache_ttl)
        ...
        return json.dumps(
            {"status": "error", "url": url, "error": str(e), "timestamp": datetime.now(timezone.utc).isoformat()}
        )


@eidosian()
def web_parse_document(source: str, source_type: str = "auto") -> str:
    """
    ...
    if not tika_parser:
        return json.dumps({"status": "error", "error": "Tika not available. Install with: pip install tika"})
    ...
    try:
        ...
        return json.dumps(
            {"status": "error", "source": source, "error": str(e), "timestamp": datetime.now(timezone.utc).isoformat()}
        )
...
@eidosian()
def web_download(url: str, save_path: str, timeout: int = 60) -> str:
    """
...
def web_hash_content(url: str, algorithm: str = "sha256") -> str:
    ...
    if algorithm not in algorithms:
        return json.dumps(
            {"status": "error", "error": f"Unsupported algorithm. Choose from: {list(algorithms.keys())}"}
        )
...
def web_cache_stats() -> str:
    ...
    """
    stats = _cache.stats()
    return json.dumps({"status": "success", **stats, "timestamp": datetime.now(timezone.utc).isoformat()}, indent=2)
...
def web_rate_limit_status() -> str:
    ...
    return json.dumps({"status": "success", **status, "timestamp": datetime.now(timezone.utc).isoformat()}, indent=2)
