...
from localllm import LLMConfig, LocalLLM, configure_logging
...
def initialize_llm():
    ...
    def init_llm_background():
        try:
            logger.info(f"⚙️ Initializing LocalLLM with model: {model_name}...")
            llm_config = LLMConfig()
            llm_instance = LocalLLM(config=llm_config)
            chat_interface.llm_instance = llm_instance
            chat_interface.eidos_config['llm_operational'] = True
            logger.info(f"✅ LocalLLM initialized successfully with {model_name}.")
...
if __name__ == "__main__":
    ...
    try:
        ...
        error_emoji = app.config.get('chat_interface').eidos_config.get('error_emoji', '⚠️') if app.config.get('chat_interface') and app.config.get('chat_interface').eidos_config else '⚠️'
        logger.critical(f"{error_emoji} Critical error in main thread: {main_thread_error}", exc_info=True)