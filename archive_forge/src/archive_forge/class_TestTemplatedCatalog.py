...
class TestTemplatedCatalog(unit.TestCase, catalog_tests.CatalogTests):
    DEFAULT_FIXTURE = {'RegionOne': {'compute': {'adminURL': 'http://localhost:8774/v1.1/bar', 'publicURL': 'http://localhost:8774/v1.1/bar', 'internalURL': 'http://localhost:8774/v1.1/bar', 'name': "'Compute Service'", 'id': '2'}, 'identity': {'adminURL': 'http://localhost:35357/v3', 'publicURL': 'http://localhost:5000/v3', 'internalURL': 'http://localhost:35357/v3', 'name': "'Identity Service'", 'id': '1'}}}
    ...
    def config_overrides(self):
        ...
        self.config_fixture.config(group='catalog', driver='templated', template_file=unit.dirs.tests('default_catalog.templates'))
    ...
    def test_get_v3_catalog_endpoint_disabled(self):
        self.skip_test_overrides("Templated backend doesn't have disabled endpoints")
    ...
    def test_get_catalog_ignores_endpoints_with_invalid_urls(self):
        ...
        project_id = None
        catalog_ref = PROVIDERS.catalog_api.get_v3_catalog(user_id, project_id)
        exp_catalog = [{'endpoints': [], 'type': 'compute', 'name': "'Compute Service'", 'id': '2'}, {'endpoints': [{'interface': 'admin', 'region': 'RegionOne', 'url': 'http://localhost:35357/v3'}, {'interface': 'public', 'region': 'RegionOne', 'url': 'http://localhost:5000/v3'}, {'interface': 'internal', 'region': 'RegionOne', 'url': 'http://localhost:35357/v3'}], 'type': 'identity', 'name': "'Identity Service'", 'id': '1'}]
        self.assert_catalogs_equal(exp_catalog, catalog_ref)
    ...
    @mock.patch.object(catalog_base.CatalogDriverBase, '_ensure_no_circle_in_hierarchical_regions')
    ...
    def test_get_endpoint_returns_not_found(self):
        self.skip_test_overrides("Templated backend doesn't use IDs for endpoints.")
    ...
    def test_list_endpoints(self):
        expected_urls = set(['http://localhost:5000/v3', 'http://localhost:35357/v3', 'http://localhost:8774/v1.1/$(tenant_id)s'])
    ...
    def test_delete_endpoint_group_association_by_project(self):
        PROVIDERS.catalog_api.delete_endpoint_group_association_by_project(uuid.uuid4().hex)
