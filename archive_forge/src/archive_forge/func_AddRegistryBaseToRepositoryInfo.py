...
def AddRegistryBaseToRepositoryInfo(response, unused_args):
    ...
    repo_name = resources.REGISTRY.ParseRelativeName(response.name, collection='artifactregistry.projects.locations.repositories')
    log.status.Print('Registry URL: {}-{}.pkg.dev/{}/{}'.format(repo_name.locationsId, str(response.format).lower(), repo_name.projectsId.replace(':', '/'), repo_name.repositoriesId))
    return response