...
class OrthoSlicer3D:
    ...
    def __init__(self, data, affine=None, axes=None, title=None):
        ...
        self._flips = list(self._flips) + [False]
        ...
        r = [self._scalers[self._order[2]] / self._scalers[self._order[1]], self._scalers[self._order[2]] / self._scalers[self._order[0]], self._scalers[self._order[1]] / self._scalers[self._order[0]]]
        ...
        for ii, xax, yax, ratio, label in zip([0, 1, 2], [1, 0, 0], [2, 2, 1], r, ('SAIP', 'SRIL', 'ARPL')):
            ...
            im = self._axes[ii].imshow(d, vmin=self._clim[0], vmax=self._clim[1], aspect=1, cmap='gray', interpolation='nearest', origin='lower')
            self._ims.append(im)
            vert = ax.plot([0] * 2, [-0.5, self._sizes[yax] - 0.5], color=(0, 1, 0), linestyle='-')[0]
            horiz = ax.plot([-0.5, self._sizes[xax] - 0.5], [0] * 2, color=(0, 1, 0), linestyle='-')[0]
            self._crosshairs[ii] = dict(vert=vert, horiz=horiz)
            lims = [0, self._sizes[xax], 0, self._sizes[yax]]
            bump = 0.01
            poss = [[lims[1] / 2.0, lims[3]], [(1 + bump) * lims[1], lims[3] / 2.0], [lims[1] / 2.0, 0], [lims[0] - bump * lims[1], lims[3] / 2.0]]
            anchors = [['center', 'bottom'], ['left', 'center'], ['center', 'top'], ['right', 'center']]
            for pos, anchor, lab in zip(poss, anchors, label):
                ax.text(pos[0], pos[1], lab, horizontalalignment=anchor[0], verticalalignment=anchor[1])
            ax.axis(lims)
            ax.set_aspect(ratio)
        ...
        if self.n_volumes > 1 and len(self._axes) > 3:
            ...
            patch = mpl_patch.Rectangle([-0.5, yl[0]], 1.0, np.diff(yl)[0], fill=True, facecolor=(0, 1, 0), edgecolor=(0, 1, 0), alpha=0.25)
            ax.add_patch(patch)
            ax.set_ylim(yl)
    ...
    def __repr__(self):
        ...
        r = f'<{self.__class__.__name__}: {title}({self._sizes[0]}, {self._sizes[1]}, {self._sizes[2]}{vol})>'
        return r
    ...
    def link_to(self, other):
        ...
        if not isinstance(other, self.__class__):
            raise TypeError(f'other must be an instance of {self.__class__.__name__}, not {type(other)}')
