...
class NetAppESeriesSnapshot(NetAppESeriesModule):
    ...
    def __init__(self):
        ...
        if args['pit_timestamp']:
            if args['pit_timestamp'] in ['newest', 'oldest']:
                ...
                self.module.fail_json(msg='Invalid argument! pit_timestamp must be in the form YYYY-MM-DD HH:MM:SS (AM|PM) (time portion is optional). Array [%s].' % self.ssid)
        if self.state == 'present':
            if self.type == 'group':
                if not self.volumes:
                    self.module.fail_json(msg='Missing argument! Volumes must be defined to create a snapshot consistency group. Group [%s]. Array [%s]' % (self.group_name, self.ssid))
            elif self.type == 'pit':
                if self.pit_timestamp and self.pit_name:
                    self.module.fail_json(msg='Invalid arguments! Either define pit_name with or without pit_description or pit_timestamp. Group [%s]. Array [%s]' % (self.group_name, self.ssid))
            elif self.type == 'view':
                if not self.view_name:
                    self.module.fail_json(msg='Missing argument! view_name must be defined to create a snapshot consistency group view. Group [%s]. Array [%s]' % (self.group_name, self.ssid))
                if not (self.pit_name or self.pit_timestamp):
                    self.module.fail_json(msg='Missing argument! Either pit_name or pit_timestamp must be defined to create a consistency group point-in-time snapshot. Group [%s]. Array [%s]' % (self.group_name, self.ssid))
        elif self.state == 'rollback':
            if not (self.pit_name or self.pit_timestamp):
                self.module.fail_json(msg='Missing argument! Either pit_name or pit_timestamp must be defined to create a consistency group point-in-time snapshot. Group [%s]. Array [%s]' % (self.group_name, self.ssid))
        elif self.type == 'pit':
            if self.pit_name and self.pit_timestamp:
                self.module.fail_json(msg='Invalid arguments! Either define pit_name or pit_timestamp. Group [%s]. Array [%s]' % (self.group_name, self.ssid))
            if not (self.pit_name or self.pit_timestamp):
                self.module.fail_json(msg='Missing argument! Either pit_name or pit_timestamp must be defined to create a consistency group point-in-time snapshot. Group [%s]. Array [%s]' % (self.group_name, self.ssid))
        elif self.type == 'view':
            if not self.view_name:
    ...
    def get_candidate(self, volume_name, volume_info):
        ...
        if volume_name in existing_volumes_by_name:
            ...
            volume_info.update({'name': volume_name, 'id': existing_volumes_by_name[volume_name]['id'], 'storage_pool_name': base_volume_storage_pool_name, 'storage_pool_id': base_volume_storage_pool_id, 'preferred_reserve_storage_pool': preferred_reserve_storage_pool, 'candidate': None})
        else:
            self.module.fail_json(msg='Volume does not exist! Volume [%s]. Group [%s]. Array [%s].' % (volume_name, self.group_name, self.ssid))
        candidate_request = {'candidateRequest': {'baseVolumeRef': volume_info['id'], 'percentCapacity': volume_info['reserve_capacity_pct'], 'concatVolumeType': 'snapshot'}}
        try:
            ...
            for candidate in candidates:
                ...
                self.module.fail_json(msg='Failed to retrieve capacity volume candidate in preferred storage pool or volume group! Volume [%s]. Group [%s]. Array [%s].' % (volume_info['name'], self.group_name, self.ssid))
        except Exception as error:
            self.module.fail_json(msg='Failed to get reserve capacity candidates! Volumes %s. Group [%s]. Array [%s]. Error [%s]' % (volume_info['name'], self.group_name, self.ssid, error))
        return volume_info
    ...
    def create_view(self, view_info):
        ...
        group_id = self.get_consistency_group()['consistency_group_id']
        view_request = {'name': view_info['name'], 'pitSequenceNumber': view_info['sequence_number'], 'requests': []}
        for volume_name, volume_info in view_info['volumes'].items():
            ...
            for image in view_info['images']:
                if volume_name == image['base_volume_name']:
                    view_request['requests'].append({'pitId': image['id'], 'candidate': candidate['candidate']['candidate'] if candidate else None, 'accessMode': 'readWrite' if volume_info['snapshot_volume_writable'] else 'readOnly', 'scanMedia': volume_info['snapshot_volume_validate'], 'validateParity': volume_info['snapshot_volume_validate']})
                    break
            else:
                ...
                if volume_info['snapshot_volume_host']:
                    for snapshot_volume in view['snapshot_volumes']:
                        if volume_name == existing_volumes_by_id[snapshot_volume['baseVol']]['name']:
                            snapshot_volume_map_request = {'mappableObjectId': snapshot_volume['id'], 'lun': volume_info['snapshot_volume_lun'], 'targetId': existing_hosts_by_name[volume_info['snapshot_volume_host']]['id']}
                            try:
                                rc, mapping = self.request('storage-systems/%s/volume-mappings' % self.ssid, method='POST', data=snapshot_volume_map_request)
                            except Exception as error:
                                self.module.fail_json(msg='Failed to map snapshot volume! Snapshot volume [%s]. View [%s]. Group [%s]. Array [%s]. Error [%s]' % (snapshot_volume['name'], self.view_name, self.group_name, self.ssid, error))
                            break
        except Exception as error:
            self.module.fail_json(msg='Failed to create consistency group snapshot volumes! Group [%s]. Array [%s]. Error [%s].' % (self.group_name, self.ssid, error))
    ...
    def move_view_mapping(self, map_information_list):
        ...
        for map_request in map_information_list:
            try:
                ...
                self.module.fail_json(msg='Failed to move snapshot volume mapping! Snapshot volume [%s]. Target [%s]. Lun [%s]. Group [%s]. Array [%s]. Error [%s].' % (existing_volumes[map_request['mappableObjectId']], existing_host_or_hostgroups[map_request['targetId']], map_request['lun'], self.group_name, self.ssid, error))
    ...
    def rollback(self, rollback_info):
        ...
        if self.rollback_priority:
            try:
                rc, resp = self.request('storage-systems/%s/consistency-groups/%s' % (self.ssid, group_id), method='POST', data={'rollbackPriority': self.rollback_priority})
            except Exception as error:
                self.module.fail_json(msg='Failed to updated consistency group rollback priority! Group [%s]. Array [%s]. Error [%s].' % (self.group_name, self.ssid, error))
        try:
            rc, resp = self.request('storage-systems/%s/symbol/startPITRollback' % self.ssid, method='POST', data={'pitRef': [image['id'] for image in rollback_info['images']]})
        except Exception as error:
        ...
        if group:
            if self.state == 'absent':
                ...
                if self.type == 'group':
                    ...
                    if group_changes['update_group'] or group_changes['add_volumes'] or group_changes['remove_volumes'] or group_changes['expand_reserve_capacity'] or group_changes['trim_reserve_capacity']:
                        changes_required = True
                elif self.type == 'pit':
                    changes_required = True
                elif self.type == 'view':
                    if self.get_consistency_group_view():
                        ...
                        if group_changes['expand_reserve_capacity'] or group_changes['trim_reserve_capacity'] or group_changes['map_snapshot_volumes_mapping'] or group_changes['unmap_snapshot_volumes_mapping'] or group_changes['move_snapshot_volumes_mapping'] or group_changes['update_snapshot_volumes_writable']:
                            changes_required = True
                    else:
            ...
            if self.type == 'group':
                ...
                self.module.fail_json(msg='Snapshot point-in-time images cannot be taken when the snapshot consistency group does not exist! Group [%s]. Array [%s].' % (self.group_name, self.ssid))
            elif self.type == 'view':
                self.module.fail_json(msg='Snapshot view cannot be created when the snapshot consistency group does not exist! Group [%s]. Array [%s].' % (self.group_name, self.ssid))
        elif self.state == 'rollback':
            self.module.fail_json(msg='Rollback operation is not available when the snapshot consistency group does not exist! Group [%s]. Array [%s].' % (self.group_name, self.ssid))
