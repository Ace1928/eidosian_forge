...
class IBMSVCRestApi(object):
    ...
    def __init__(self, module, clustername, domain, username, password, validate_certs, log_path, token):
        ...
        if self.token is None:
            if not self.username or not self.password:
                self.module.fail_json(msg='You must pass in either pre-acquired token or username/password to generate new token')
    ...
    def _svc_authorize(self):
        ...
        """
        headers = {'Content-Type': 'application/json', 'X-Auth-Username': self.username, 'X-Auth-Password': self.password}
