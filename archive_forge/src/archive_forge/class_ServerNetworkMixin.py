...
class ServerNetworkMixin(object):
    ...
    def _floating_ip_neutron_associate(self, floating_ip, floating_ip_data):
        self.client('neutron').update_floatingip(floating_ip, {'floatingip': floating_ip_data})

    def _floating_ip_disassociate(self, floating_ip):
        with self.client_plugin('neutron').ignore_not_found:
            self.client('neutron').update_floatingip(floating_ip, {'floatingip': {'port_id': None}})
    ...
    def _exclude_not_updated_networks(self, old_nets, new_nets, interfaces):
        ...
        for new_net in list(new_nets):
            new_net_reduced = {k: v for k, v in new_net.items() if k not in self._IFACE_MANAGED_KEYS or v is not None}
    ...
    def detach_ports(self, server):
        ...
        for port in self.get_all_ports(server):
            detach_called = self.client_plugin().interface_detach(existing_server_id, port['id'])
            if not detach_called:
                return
            try:
                if self.client_plugin().check_interface_detach(existing_server_id, port['id']):
                    LOG.info('Detach interface %(port)s successful from server %(server)s.', {'port': port['id'], 'server': existing_server_id})
