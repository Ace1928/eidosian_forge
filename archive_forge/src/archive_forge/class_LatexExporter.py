...
class LatexExporter(TemplateExporter):
    ...
    def default_config(self):
        c = Config({'NbConvertBase': {'display_data_priority': ['text/latex', 'application/pdf', 'image/png', 'image/jpeg', 'image/svg+xml', 'text/markdown', 'text/plain']}, 'ExtractAttachmentsPreprocessor': {'enabled': True}, 'ExtractOutputPreprocessor': {'enabled': True}, 'SVG2PDFPreprocessor': {'enabled': True}, 'LatexPreprocessor': {'enabled': True}, 'SphinxPreprocessor': {'enabled': True}, 'HighlightMagicsPreprocessor': {'enabled': True}})
    ...
    def from_notebook_node(self, nb, resources=None, **kw):
        ...
        convert_explicitly_relative_paths = self.filters.get('convert_explicitly_relative_paths', ConvertExplicitlyRelativePaths(texinputs=texinputs, parent=self))
