...
class PyDevJsonCommandProcessor(object):
    ...
    def _verify_launch_or_attach_done(self, request):
        if not self._launch_or_attach_request_done:
            ...
            response = pydevd_base_schema.build_response(request, kwargs={'body': body, 'success': False, 'message': 'Breakpoints may only be set after the launch request is received.'})
            return NetCommand(CMD_RETURN, 0, response, is_json=True)
    ...
    def _on_changed_breakpoint_state(self, py_db, source, breakpoint_id, result):
        ...
        body = BreakpointEventBody(reason='changed', breakpoint=bp)
    ...
    def on_scopes_request(self, py_db, request):
        ...
        scopes = [Scope('Locals', ScopeRequest(int(variables_reference), 'locals'), False, presentationHint='locals'), Scope('Globals', ScopeRequest(int(variables_reference), 'globals'), False)]
    ...
    def on_evaluate_request(self, py_db, request):
        ...
        if arguments.frameId is None:
            ...
            if thread_id is not None:
                ...
                response = pydevd_base_schema.build_response(request, kwargs={'body': body, 'success': False, 'message': 'Unable to find thread for evaluation.'})
                return NetCommand(CMD_RETURN, 0, response, is_json=True)
    ...
    def on_setexpression_request(self, py_db, request):
        ...
        if thread_id is not None:
            ...
            response = pydevd_base_schema.build_response(request, kwargs={'body': body, 'success': False, 'message': 'Unable to find thread to set expression.'})
            return NetCommand(CMD_RETURN, 0, response, is_json=True)
    ...
    def on_setvariable_request(self, py_db, request):
        ...
        if arguments.name.startswith('(return) '):
            response = pydevd_base_schema.build_response(request, kwargs={'body': SetVariableResponseBody(''), 'success': False, 'message': 'Cannot change return value'})
            return NetCommand(CMD_RETURN, 0, response, is_json=True)
        thread_id = py_db.suspended_frames_manager.get_thread_id_for_variable_reference(variables_reference)
        if thread_id is not None:
            self.api.request_change_variable_json(py_db, request, thread_id)
        else:
            response = pydevd_base_schema.build_response(request, kwargs={'body': SetVariableResponseBody(''), 'success': False, 'message': 'Unable to find thread to evaluate variable reference.'})
            return NetCommand(CMD_RETURN, 0, response, is_json=True)

    def on_modules_request(self, py_db, request):
        modules_manager = py_db.cmd_factory.modules_manager
    ...
    def on_setpydevdsourcemap_request(self, py_db, request):
        ...
        new_mappings = [SourceMappingEntry(source_map['line'], source_map['endLine'], source_map['runtimeLine'], self.api.filename_to_str(source_map['runtimeSource']['path'])) for source_map in source_maps]
        error_msg = self.api.set_source_mapping(py_db, path, new_mappings)
        if error_msg:
            response = pydevd_base_schema.build_response(request, kwargs={'body': {}, 'success': False, 'message': error_msg})
            return NetCommand(CMD_RETURN, 0, response, is_json=True)
        response = pydevd_base_schema.build_response(request)
        return NetCommand(CMD_RETURN, 0, response, is_json=True)