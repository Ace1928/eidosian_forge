...
class MessengerAgent(ChatServiceAgent):
    ...
    def observe(self, act):
        ...
        """
        if 'payload' in act:
            resp = self.manager.observe_payload(self.id, act['payload'], act.get('quick_replies', None), act.get('persona_id', None))
        else:
            if act['id'] != '':
                msg = '{}: {}'.format(act['id'], act['text'])
            else:
                msg = act['text']
            resp = self.manager.observe_message(self.id, msg, act.get('quick_replies', None), act.get('persona_id', None))
        try:
    ...
    def put_data(self, messenger_data):
        ...
        if mid not in self.acted_packets:
            ...
            action = {'episode_done': False, 'text': text, 'id': self.disp_id, 'report_sender': message.get('report_sender', None), 'sticker_sender': messenger_data.get('sticker_sender', None), 'img_attempt': img_attempt}
    ...
    def act(self, timeout=None):
        ...
        if msg is not None:
            if msg.get('img_attempt') and (not self.data.get('allow_images', False)):
                msg = None
                act = {'id': 'SYSTEM', 'text': 'Only text messages are supported at this time. Please try with a text-only message.', 'episode_done': True}
                self.observe(act)
            elif not msg.get('text') and (not (msg.get('image_url') or msg.get('attachment_url'))):
                msg = None
