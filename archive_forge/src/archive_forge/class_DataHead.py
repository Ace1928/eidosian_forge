            ...
            try:
                for metric, queries in DATASET_METRICS.items():
                    for query in queries:
                        ...
                        dataset_result = await self._query_prometheus(prom_query.format(metric, self._session_name, 'dataset'))
                        ...
                        operator_result = await self._query_prometheus(prom_query.format(metric, self._session_name, 'dataset, operator'))
                ...
                logging.exception('Exception occurred while querying Prometheus. The Prometheus server may not be running.')
            for dataset in datasets:
