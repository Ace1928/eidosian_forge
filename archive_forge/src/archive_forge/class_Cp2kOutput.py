...
class Cp2kOutput:
    ...
    def convergence(self):
        ...
        unconverged_inner_loop = re.compile('(Leaving inner SCF loop)')
        scf_converged = re.compile('(SCF run converged)|(SCF run NOT converged)')
        self.read_pattern(patterns={'unconverged_inner_loop': unconverged_inner_loop, 'scf_converged': scf_converged}, reverse=True, terminate_on_match=False, postprocess=bool)
        for idx, val in enumerate(self.data['scf_converged']):
            self.data['scf_converged'][idx] = bool(val[0])
        geo_opt_not_converged = re.compile('(MAXIMUM NUMBER OF OPTIMIZATION STEPS REACHED)')
        geo_opt_converged = re.compile('(GEOMETRY OPTIMIZATION COMPLETED)')
        self.read_pattern(patterns={'geo_opt_converged': geo_opt_converged, 'geo_opt_not_converged': geo_opt_not_converged}, reverse=True, terminate_on_match=True, postprocess=bool)
        if not all(self.data['scf_converged']):
            warnings.warn('There is at least one unconverged SCF cycle in the provided cp2k calculation', UserWarning)
    ...
    def parse_forces(self):
        ...
        if len(self.filenames['forces']) == 1:
            self.data['forces'] = [[list(atom.coords) for atom in step] for step in XYZ.from_file(self.filenames['forces'][0]).all_molecules]
        else:
            header_pattern = 'ATOMIC FORCES.+Z'
            row_pattern = '\\s+\\d+\\s+\\d+\\s+\\w+\\s+(-?\\d+\\.\\d+)\\s+(-?\\d+\\.\\d+)\\s+(-?\\d+\\.\\d+)'
            footer_pattern = 'SUM OF ATOMIC FORCES'
            self.data['forces'] = self.read_table_pattern(header_pattern=header_pattern, row_pattern=row_pattern, footer_pattern=footer_pattern, postprocess=postprocessor, last_one_only=False)
    ...
    def parse_plus_u_params(self):
        ...
        method = re.compile('\\s+DFT\\+U\\|\\s+Method\\s+()$')
        self.read_pattern({'dft_plus_u_method': method}, terminate_on_match=True, reverse=False, postprocess=postprocessor)
    ...
    def parse_scf_opt(self):
        ...
        header = 'Step\\s+Update method\\s+Time\\s+Convergence\\s+Total energy\\s+Change\\s+\\-+'
        row = '(\\d+)\\s+([A-Za-z\\./_]+\\s?[A-Za-z\\./]+)\\s+(-?\\d+\\.\\d+(?:[eE][+\\-]?\\d+)?)\\s+(-?\\d+\\.\\d+(?:[eE][+\\-]?\\d+)?)(\\s+-?\\d+\\.\\d+(?:[eE][+\\-]?\\d+)?)?\\s+(-?\\d+\\.\\d+(?:[eE][+\\-]?\\d+)?)(\\s+-?\\d+\\.\\d+(?:[eE][+\\-]?\\d+)?)?'
        footer = '^$'
        scfs = self.read_table_pattern(header_pattern=header, row_pattern=row, footer_pattern=footer, last_one_only=False, strip=['HFX_MEM_INFO', '*'])
    ...
    def parse_mulliken(self):
        ...
        """
        header = 'Mulliken Population Analysis.+Net charge'
        pattern = '\\s+(\\d)\\s+(\\w+)\\s+(\\d+)\\s+(-?\\d+\\.\\d+)\\s+(-?\\d+\\.\\d+)'
        footer = '.+Total charge'
        d = self.read_table_pattern(header_pattern=header, row_pattern=pattern, footer_pattern=footer, last_one_only=False)
        if d:
    ...
    def parse_hirshfeld(self):
        ...
        header = 'Hirshfeld Charges.+Net charge'
        footer = '^$'
        if not uks:
            pattern = '\\s+(\\d)\\s+(\\w+)\\s+(\\d+)\\s+(-?\\d+\\.\\d+)\\s+(-?\\d+\\.\\d+)\\s+(-?\\d+\\.\\d+)'
            d = self.read_table_pattern(header_pattern=header, row_pattern=pattern, footer_pattern=footer, last_one_only=False)
            ...
            pattern = '\\s+(\\d)\\s+(\\w+)\\s+(\\d+)\\s+(-?\\d+\\.\\d+)\\s+(-?\\d+\\.\\d+)\\s+(-?\\d+\\.\\d+)\\s+(-?\\d+\\.\\d+)\\s+(-?\\d+\\.\\d+)'
            d = self.read_table_pattern(header_pattern=header, row_pattern=pattern, footer_pattern=footer, last_one_only=False)
            for i, ionic_step in enumerate(d):
                population = []
                net_charge = []
                ...
                hirshfeld = [{'population': population[j], 'net_charge': net_charge[j], 'spin_moment': spin_moment[j]} for j in range(len(population))]
                self.structures[i].add_site_property('hirshfield', hirshfeld)
    ...
    def parse_homo_lumo(self):
        ...
        """
        pattern = re.compile('HOMO.*-.*LUMO.*gap.*\\s(-?\\d+.\\d+)')
        self.read_pattern(patterns={'band_gap': pattern}, reverse=True, terminate_on_match=False, postprocess=float)
    ...
    def as_dict(self):
        ...
        dct['output']['ionic_steps'] = [{k: v.as_dict() if isinstance(v, MSONable) else v for k, v in step.items()} for step in self.ionic_steps]
