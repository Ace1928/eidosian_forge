...
class BaseOpenAI(BaseLLM):
    ...
    def _default_params(self) -> Dict[str, Any]:
        ...
        normal_params: Dict[str, Any] = {'temperature': self.temperature, 'top_p': self.top_p, 'frequency_penalty': self.frequency_penalty, 'presence_penalty': self.presence_penalty, 'n': self.n, 'logit_bias': self.logit_bias}
    ...
    def _stream(self, prompt: str, stop: Optional[List[str]]=None, run_manager: Optional[CallbackManagerForLLMRun]=None, **kwargs: Any) -> Iterator[GenerationChunk]:
        ...
        for stream_resp in completion_with_retry(self, prompt=prompt, run_manager=run_manager, **params):
            ...
            if run_manager:
                run_manager.on_llm_new_token(chunk.text, chunk=chunk, verbose=self.verbose, logprobs=chunk.generation_info['logprobs'] if chunk.generation_info else None)
            yield chunk
    ...
    async def _astream(self, prompt: str, stop: Optional[List[str]]=None, run_manager: Optional[AsyncCallbackManagerForLLMRun]=None, **kwargs: Any) -> AsyncIterator[GenerationChunk]:
        ...
        async for stream_resp in await acompletion_with_retry(self, prompt=prompt, run_manager=run_manager, **params):
            ...
            chunk = _stream_response_to_generation_chunk(stream_resp)
            if run_manager:
                await run_manager.on_llm_new_token(chunk.text, chunk=chunk, verbose=self.verbose, logprobs=chunk.generation_info['logprobs'] if chunk.generation_info else None)
            yield chunk
    ...
    def _generate(self, prompts: List[str], stop: Optional[List[str]]=None, run_manager: Optional[CallbackManagerForLLMRun]=None, **kwargs: Any) -> LLMResult:
        ...
        for _prompts in sub_prompts:
            if self.streaming:
                ...
                choices.append({'text': generation.text, 'finish_reason': generation.generation_info.get('finish_reason') if generation.generation_info else None, 'logprobs': generation.generation_info.get('logprobs') if generation.generation_info else None})
            else:
    ...
    def _invocation_params(self) -> Dict[str, Any]:
        ...
        if not is_openai_v1():
            openai_creds.update({'api_key': self.openai_api_key, 'api_base': self.openai_api_base, 'organization': self.openai_organization})
        if self.openai_proxy:
            import openai
