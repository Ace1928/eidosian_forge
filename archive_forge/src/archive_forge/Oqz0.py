...
def setup() -> Tuple[pg.Surface, apple.Apple, Search, pg.time.Clock]:
    ...
    apple_obj: apple.Apple = apple_instance
    search_algorithm: Search = Search()
    search_algorithm.get_path()
    clock: pg.time.Clock = clock
    return screen, apple_obj, search_algorithm, clock
    ...
    def update(self, apple: apple.Apple) -> None:
        ...
        if self.get_head_position().x == 39 and self.x_direction == 1:
            ...
            for i in range(len(self.body) - 1):
                ...
                if not apple_instance.generate(self.body):
                    self.show()
                else:
                    self.body.pop(0)
...
def main() -> None:
    ...
    apple_obj: apple.Apple = apple_instance
    snake: Snake = Snake()
    search_algorithm: Search = Search(snake, apple_obj)
    search_algorithm.get_path()
    clock: pg.time.Clock = clock
    ...
    while True:
        ...
        snake.update(apple_obj)
        snake.show(screen)
        apple_obj.show(screen)
        pg.display.flip()
        clock.tick(FPS)
