...
class LaunchpadGitMergeProposalBuilder(MergeProposalBuilder):
    ...
    def get_initial_body(self):
        ...
        with self.target_branch.lock_read(), self.source_branch.lock_read():
            ...
            for hook in self.hooks['merge_proposal_body']:
                body = hook({'target_branch': self.target_branch, 'modified_files_callback': list_modified_files, 'old_body': body})
            return body
    ...
    def create_proposal(self, description, reviewers=None, labels=None, prerequisite_branch=None, commit_message=None, work_in_progress=False, allow_collaboration=False):
        ...
        if reviewers is None:
            reviewers = []
        try:
            mp = _call_webservice(self.source_branch_lp.createMergeProposal, merge_target=self.target_branch_lp, merge_prerequisite=prereq_branch_lp, initial_comment=description.strip(), commit_message=commit_message, needs_review=not work_in_progress, reviewers=[self.launchpad.people[reviewer].self_link for reviewer in reviewers], review_types=[None for reviewer in reviewers])
        except WebserviceFailure as e:
            if 'There is already a branch merge proposal registered for branch ' in e.message:
