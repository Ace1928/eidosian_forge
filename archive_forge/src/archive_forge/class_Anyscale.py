...
class Anyscale(BaseOpenAI):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        values['anyscale_api_base'] = get_from_dict_or_env(values, 'anyscale_api_base', 'ANYSCALE_API_BASE', default=DEFAULT_BASE_URL)
        values['anyscale_api_key'] = convert_to_secret_str(get_from_dict_or_env(values, 'anyscale_api_key', 'ANYSCALE_API_KEY'))
        values['model_name'] = get_from_dict_or_env(values, 'model_name', 'MODEL_NAME', default=DEFAULT_MODEL)
        try:
            import openai
            if is_openai_v1():
                client_params = {'api_key': values['anyscale_api_key'].get_secret_value(), 'base_url': values['anyscale_api_base']}
    ...
    def _invocation_params(self) -> Dict[str, Any]:
        ...
        if not is_openai_v1():
            openai_creds.update({'api_key': self.anyscale_api_key.get_secret_value(), 'api_base': self.anyscale_api_base})
    ...
    def _generate(self, prompts: List[str], stop: Optional[List[str]]=None, run_manager: Optional[CallbackManagerForLLMRun]=None, **kwargs: Any) -> LLMResult:
        ...
        for _prompts in sub_prompts:
            if self.streaming:
                ...
                response = completion_with_retry(self, prompt=_prompts[0], run_manager=run_manager, **params)
    ...
    async def _agenerate(self, prompts: List[str], stop: Optional[List[str]]=None, run_manager: Optional[AsyncCallbackManagerForLLMRun]=None, **kwargs: Any) -> LLMResult:
        ...
        for _prompts in sub_prompts:
            if self.streaming:
                ...
                response = await acompletion_with_retry(self, prompt=_prompts[0], run_manager=run_manager, **params)
                if not isinstance(response, dict):
                    response = response.dict()
                choices.extend(response['choices'])
                update_token_usage(_keys, response, token_usage)
