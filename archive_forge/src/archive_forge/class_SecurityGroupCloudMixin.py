...
class SecurityGroupCloudMixin:
    ...
    def create_security_group(self, name, description, project_id=None, stateful=None):
        ...
        if self._use_neutron_secgroups():
            ...
            data = proxy._json_response(self.compute.post('/os-security-groups', json={'security_group': security_group_json}))
        return self._normalize_secgroup(self._get_and_munchify('security_group', data))
    ...
    def delete_security_group(self, name_or_id):
        ...
        if self._use_neutron_secgroups():
            ...
            proxy._json_response(self.compute.delete('/os-security-groups/{id}'.format(id=secgroup['id'])))
            return True
    ...
    def update_security_group(self, name_or_id, **kwargs):
        ...
        if group is None:
            raise exceptions.SDKException('Security group %s not found.' % name_or_id)
        if self._use_neutron_secgroups():
            return self.network.update_security_group(group['id'], **kwargs)
        else:
            for key in ('name', 'description'):
                kwargs.setdefault(key, group[key])
            data = proxy._json_response(self.compute.put('/os-security-groups/{id}'.format(id=group['id']), json={'security_group': kwargs}))
        return self._normalize_secgroup(self._get_and_munchify('security_group', data))

    def create_security_group_rule(self, secgroup_name_or_id, port_range_min=None, port_range_max=None, protocol=None, remote_ip_prefix=None, remote_group_id=None, remote_address_group_id=None, direction='ingress', ethertype='IPv4', project_id=None, description=None):
        ...
        if not secgroup:
            raise exceptions.SDKException('Security group %s not found.' % secgroup_name_or_id)
        if self._use_neutron_secgroups():
            rule_def = {'security_group_id': secgroup['id'], 'port_range_min': None if port_range_min == -1 else port_range_min, 'port_range_max': None if port_range_max == -1 else port_range_max, 'protocol': protocol, 'remote_ip_prefix': remote_ip_prefix, 'remote_group_id': remote_group_id, 'remote_address_group_id': remote_address_group_id, 'direction': direction, 'ethertype': ethertype}
            if project_id is not None:
            ...
            security_group_rule_dict = dict(security_group_rule=dict(parent_group_id=secgroup['id'], ip_protocol=protocol, from_port=port_range_min, to_port=port_range_max, cidr=remote_ip_prefix, group_id=remote_group_id))
            if project_id is not None:
                security_group_rule_dict['security_group_rule']['tenant_id'] = project_id
            data = proxy._json_response(self.compute.post('/os-security-group-rules', json=security_group_rule_dict))
        return self._normalize_secgroup_rule(self._get_and_munchify('security_group_rule', data))
    ...
    def delete_security_group_rule(self, rule_id):
        ...
        if self._use_neutron_secgroups():
            ...
            try:
                exceptions.raise_from_response(self.compute.delete('/os-security-group-rules/{id}'.format(id=rule_id)))
