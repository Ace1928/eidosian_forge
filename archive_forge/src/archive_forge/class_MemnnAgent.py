...
class MemnnAgent(TorchRankerAgent):
    ...
    def add_cmdline_args(argparser):
        ...
        arg_group.add_argument('-esz', '--embedding-size', type=int, default=128, help='size of token embeddings')
        arg_group.add_argument('-hops', '--hops', type=int, default=3, help='number of memory hops')
        arg_group.add_argument('--memsize', type=int, default=32, help='size of memory, set to 0 for "nomemnn" model which just embeds query and candidates and picks most similar candidate')
        arg_group.add_argument('-tf', '--time-features', type='bool', default=True, help='use time features for memory embeddings')
        arg_group.add_argument('-pe', '--position-encoding', type='bool', default=False, help='use position encoding instead of bag of words embedding')
        argparser.set_defaults(split_lines=True, add_p1_after_newln=True, encode_candidate_vecs=True)
    ...
    def build_model(self):
        ...
        return MemNN(len(self.dict), self.opt['embedding_size'], padding_idx=self.NULL_IDX, **kwargs)
    ...
    def _set_text_vec(self, obs, history, truncate):
        ...
        if 'memory_vecs' in obs:
            obs.force_set('memory_vecs', [torch.LongTensor(self._check_truncate(m, truncate, True)) for m in obs['memory_vecs']])
        return obs
