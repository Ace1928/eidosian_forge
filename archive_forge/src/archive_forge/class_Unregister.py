...
class Unregister(calliope_base.DeleteCommand):
  ...
  """
    ...
    @classmethod
    def Args(cls, parser):
        resources.AddMembershipResourceArg(parser, membership_help=textwrap.dedent('          The membership name that you choose to uniquely represent the cluster\n          being registered in the fleet.\n        '), location_help=textwrap.dedent('          The location of the membership resource, e.g. `us-central1`.\n          If not specified, defaults to `global`.\n        '), membership_required=True, positional=True)
        parser.add_argument('--uninstall-connect-agent', action='store_true', help=textwrap.dedent('          If set to True for a GKE cluster, Connect agent will be uninstalled\n          from the cluster. No-op for third-party clusters, where Connect agent\n          will always be uninstalled.\n          '), default=False)
        hub_util.AddClusterConnectionCommonArgs(parser)

    def Run(self, args):
        project = arg_utils.GetFromNamespace(args, '--project', use_defaults=True)
        gke_cluster_resource_link, gke_cluster_uri = gke_util.GetGKEClusterResoureLinkAndURI(gke_uri=args.GetValue('gke_uri'), gke_cluster=args.GetValue('gke_cluster'))
        location = 'global'
        ...
        if resources.MembershipLocationSpecified(args):
            ...
            regional_name = 'projects/{}/locations/{}/memberships/{}'.format(project, cluster_location, membership_id)
            try:
                regional_obj = api_util.GetMembership(regional_name, self.ReleaseTrack())
                if regional_obj.endpoint and regional_obj.endpoint.gkeCluster.resourceLink == gke_cluster_resource_link:
                    location = cluster_location
            except apitools_exceptions.HttpError:
                pass
        if gke_cluster_resource_link and (not args.GetValue('uninstall_connect_agent')):
            return self._UnregisterGKE(gke_cluster_resource_link, gke_cluster_uri, project, location, membership_id, args)
        kube_client = kube_util.KubernetesClient(gke_uri=getattr(args, 'gke_uri', None), gke_cluster=getattr(args, 'gke_cluster', None), kubeconfig=getattr(args, 'kubeconfig', None), context=getattr(args, 'context', None), public_issuer_url=getattr(args, 'public_issuer_url', None), enable_workload_identity=getattr(args, 'enable_workload_identity', False))
        ...
        try:
            ...
            if not obj.externalId:
                console_io.PromptContinue('invalid membership {0} does not have external_id field set. We cannot determine if registration is requested against a valid existing Membership. Consult the documentation on container fleet memberships update for more information or run gcloud container fleet memberships delete {0} if you are sure that this is an invalid or otherwise stale Membership'.format(membership_id), cancel_on_no=True)
            uuid = kube_util.GetClusterUUID(kube_client)
            if obj.externalId != uuid:
                raise exceptions.Error('Membership [{}] is not associated with the cluster you are trying to unregister. Please double check the cluster identifier that you have supplied.'.format(name))
        ...
        if not namespaces:
            log.status.Print("There's no namespace for the label [{}]. If [gke-connect] is labeled with another project, You'll have to manually delete the namespace. You can find all namespaces by running:\n  `kubectl get ns -l {}`".format(agent_util.CONNECT_RESOURCE_LABEL, agent_util.CONNECT_RESOURCE_LABEL))
        try:
