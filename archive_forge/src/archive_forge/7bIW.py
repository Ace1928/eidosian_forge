...
class Pathfinder:
    ...
    def heuristic(
        ...
        Parameters:
            ...
            space_around_boundaries (int): The space around the boundaries to consider for path planning. If the space has to be occupied, so long as a collision won't occur then the agent will. Default is 0.
            obstacles (Set[Tuple[int, int]]): The positions of obstacles in the grid. Default is an empty set.
            path_count (int): The number of alternative paths to generate and evaluate. Default is 1.
            path_granularity (int): The level of detail in the generated paths. Default is 1.
            update_frequency (int): The frequency at which the pathfinding algorithm updates paths. Default is 100. Which is 100ms.
            escape_route_availability (bool): Whether escape routes should be considered. Default is False.
            enhancements (List[str]): The enhancements to apply to the path. Default is applying zigzagging.
            dense_packing (bool): Whether dense packing scenarios should be considered. Default is True. Dense packing means that the agent will keep its path changes in such a way that it increases the overall area it covers between 2 points but keeps the paths as close to each other as possible with out overlapping.
            body_size_adaptations (bool): Whether the agent's body size should be considered. Default is True.
            last_direction_moved (Optional[Tuple[int, int]]): The last direction moved by the agent. Default is None.
