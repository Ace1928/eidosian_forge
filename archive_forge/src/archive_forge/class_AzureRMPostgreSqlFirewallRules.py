...
class AzureRMPostgreSqlFirewallRules(AzureRMModuleBase):
    ...
    def __init__(self):
        ...
        super(AzureRMPostgreSqlFirewallRules, self).__init__(derived_arg_spec=self.module_arg_spec, supports_check_mode=True, supports_tags=False)
    ...
    def create_update_firewallrule(self):
        ...
        try:
            response = self.postgresql_client.firewall_rules.begin_create_or_update(resource_group_name=self.resource_group, server_name=self.server_name, firewall_rule_name=self.name, parameters=self.parameters)
    ...
    def delete_firewallrule(self):
        ...
        try:
            response = self.postgresql_client.firewall_rules.begin_delete(resource_group_name=self.resource_group, server_name=self.server_name, firewall_rule_name=self.name)
    ...
    def get_firewallrule(self):
        ...
        found = False
        try:
            response = self.postgresql_client.firewall_rules.get(resource_group_name=self.resource_group, server_name=self.server_name, firewall_rule_name=self.name)
            found = True
