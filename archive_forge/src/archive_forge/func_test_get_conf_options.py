...
def test_get_conf_options(self):
    ...
    for opt in opts:
        if opt.name.endswith('-retries'):
            ...
            self.assertIsInstance(opt, cfg.ListOpt)
    self.assertEqual({'service-type', 'service-name', 'interface', 'valid-interfaces', 'region-name', 'endpoint-override', 'version', 'min-version', 'max-version', 'connect-retries', 'status-code-retries', 'connect-retry-delay', 'status-code-retry-delay', 'retriable-status-codes'}, {opt.name for opt in opts})