...
class IntraCloudCopyTask(copy_util.ObjectCopyTaskWithExitHandler):
    ...
    def __init__(self, source_resource, destination_resource, delete_source=False, fetch_source_fields_scope=None, posix_to_set=None, print_created_message=False, print_source_version=False, user_request_args=None, verbose=False):
        """Initializes task.
    ...
    """
        super(IntraCloudCopyTask, self).__init__(source_resource, destination_resource, posix_to_set=posix_to_set, print_created_message=print_created_message, print_source_version=print_source_version, user_request_args=user_request_args, verbose=verbose)
        if source_resource.storage_url.scheme != destination_resource.storage_url.scheme or not isinstance(source_resource.storage_url, storage_url.CloudUrl):
            raise errors.InvalidUrlError('IntraCloudCopyTask takes two URLs from the same cloud provider.')
    ...
    def execute(self, task_status_queue=None):
        ...
        if copy_util.check_for_cloud_clobber(self._user_request_args, api_client, self._destination_resource):
            ...
            return
        progress_callback = progress_callbacks.FilesAndBytesProgressCallback(status_queue=task_status_queue, offset=0, length=self._source_resource.size, source_url=self._source_resource.storage_url, destination_url=self._destination_resource.storage_url, operation_name=task_status.OperationName.INTRA_CLOUD_COPYING, process_id=os.getpid(), thread_id=threading.get_ident())
        if self._fetch_source_fields_scope:
            copy_source = api_client.get_object_metadata(self._source_resource.bucket, self._source_resource.name, generation=self._source_resource.generation, fields_scope=self._fetch_source_fields_scope)
        else:
            copy_source = self._source_resource
        request_config = request_config_factory.get_request_config(self._destination_resource.storage_url, decryption_key_hash_sha256=self._source_resource.decryption_key_hash_sha256, user_request_args=self._user_request_args)
        result_resource = api_client.copy_object(copy_source, self._destination_resource, request_config, posix_to_set=self._posix_to_set, progress_callback=progress_callback)
    ...
    def __eq__(self, other):
        ...
        return self._source_resource == other._source_resource and self._destination_resource == other._destination_resource and (self._delete_source == other._delete_source) and (self._fetch_source_fields_scope == other._fetch_source_fields_scope) and (self._posix_to_set == other._posix_to_set) and (self._print_created_message == other._print_created_message) and (self._print_source_version == other._print_source_version) and (self._user_request_args == other._user_request_args) and (self._verbose == other._verbose)