...
class Api:
    ...
    def runs(self, path: Optional[str]=None, filters: Optional[Dict[str, Any]]=None, order: str='-created_at', per_page: int=50, include_sweeps: bool=True):
        ...
        if not self._runs.get(key):
            self._runs[key] = public.Runs(self.client, entity, project, filters=filters, order=order, per_page=per_page, include_sweeps=include_sweeps)
    ...
    def run_queue(self, entity, name):
        ...
        """
        return public.RunQueue(self.client, name, entity)
    ...
    def artifact_collections(self, project_name: str, type_name: str, per_page=50):
        ...
        return public.ArtifactCollections(self.client, entity, project, type_name, per_page=per_page)
    ...
    def artifact_collection(self, type_name: str, name: str):
        ...
        return public.ArtifactCollection(self.client, entity, project, collection_name, type_name)
    ...
    def artifacts(self, type_name, name, per_page=50):
        entity, project, collection_name = self._parse_artifact_path(name)
        return public.Artifacts(self.client, entity, project, collection_name, type_name, per_page=per_page)
    ...
    def artifact(self, name, type=None):
        ...
        artifact = wandb.Artifact._from_name(entity, project, artifact_name, self.client)
        if type is not None and artifact.type != type:
            raise ValueError(f'type {type} specified but this artifact is of type {artifact.type}')
        return artifact
    ...
    def list_jobs(self, entity, project):
        ...
        try:
            artifact_query = self._client.execute(query, {'projectName': project, 'entityName': entity, 'artifactTypeName': 'job'})
            if not artifact_query or not artifact_query['project']:
                wandb.termerror(f"Project: '{project}' not found in entity: '{entity}' or access denied.")
                return []
