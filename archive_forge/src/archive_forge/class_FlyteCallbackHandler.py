...
class FlyteCallbackHandler(BaseMetadataCallbackHandler, BaseCallbackHandler):
    ...
    def __init__(self) -> None:
        ...
        try:
            ...
            logger.warning('Textstat library is not installed.                 It may result in the inability to log                 certain metrics that can be captured with Textstat.')
        spacy = None
        try:
            spacy = import_spacy()
        except ImportError:
            logger.warning('Spacy library is not installed.                 It may result in the inability to log                 certain metrics that can be captured with Spacy.')
        super().__init__()
        self.nlp = None
        if spacy:
            try:
                ...
                logger.warning("FlyteCallbackHandler uses spacy's en_core_web_sm model for certain metrics. To download, run the following command in your terminal: `python -m spacy download en_core_web_sm`")
        self.table_renderer = renderer.TableRenderer
        self.markdown_renderer = renderer.MarkdownRenderer
        self.deck = flytekit.Deck('LangChain Metrics', self.markdown_renderer().to_html('## LangChain Metrics'))
