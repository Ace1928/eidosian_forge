...
class TrioStream(AsyncNetworkStream):
    ...
    async def read(self, max_bytes: int, timeout: typing.Optional[float]=None) -> bytes:
        ...
        exc_map: ExceptionMapping = {trio.TooSlowError: ReadTimeout, trio.BrokenResourceError: ReadError, trio.ClosedResourceError: ReadError}
    ...
    async def write(self, buffer: bytes, timeout: typing.Optional[float]=None) -> None:
        if not buffer:
            return
        timeout_or_inf = float('inf') if timeout is None else timeout
        exc_map: ExceptionMapping = {trio.TooSlowError: WriteTimeout, trio.BrokenResourceError: WriteError, trio.ClosedResourceError: WriteError}
        with map_exceptions(exc_map):
            with trio.fail_after(timeout_or_inf):
    ...
    async def start_tls(self, ssl_context: ssl.SSLContext, server_hostname: typing.Optional[str]=None, timeout: typing.Optional[float]=None) -> AsyncNetworkStream:
        timeout_or_inf = float('inf') if timeout is None else timeout
        exc_map: ExceptionMapping = {trio.TooSlowError: ConnectTimeout, trio.BrokenResourceError: ConnectError}
        ssl_stream = trio.SSLStream(self._stream, ssl_context=ssl_context, server_hostname=server_hostname, https_compatible=True, server_side=False)
        with map_exceptions(exc_map):
            try:
                with trio.fail_after(timeout_or_inf):
