...
@SolverFactory.register('gdpopt.lbb', doc='The LBB (logic-based branch and bound) Generalized Disjunctive Programming (GDP) solver')
class GDP_LBB_Solver(_GDPoptAlgorithm):
    ...
    _add_nlp_solve_configs(CONFIG, default_nlp_init_method=restore_vars_to_original_values)
    ...
    def _solve_gdp(self, model, config):
        ...
        root_node = TransformationFactory('contrib.logical_to_disjunctive').create_using(model)
        ...
        for disjunction in root_util_blk.disjunction_list:
            ...
            for disjunct in unfixed_disjuncts:
                nonlinear_constraints_in_disjunct = [constr for constr in disjunct.component_data_objects(Constraint, active=True) if constr.body.polynomial_degree() not in _linear_degrees]
        ...
        unbranched_disjunction_indices = [i for i, disjunction in enumerate(root_util_blk.disjunction_list) if disjunction in root_util_blk.disjunction_to_unfixed_disjuncts]
        sort_tuple = BBNodeData(obj_lb=float('-inf'), obj_ub=float('inf'), is_screened=False, is_evaluated=False, num_unbranched_disjunctions=len(unbranched_disjunction_indices), node_count=0, unbranched_disjunction_indices=unbranched_disjunction_indices)
        ...
        while len(queue) > 0:
            ...
            if self.reached_time_limit(config):
                ...
                self.LB = node_data.obj_lb if self.objective_sense == minimize else -no_feasible_soln
                self.UB = no_feasible_soln if self.objective_sense == minimize else -node_data.obj_lb
                config.logger.info('Final bound values: LB: {}  UB: {}'.format(self.LB, self.UB))
            ...
            if not node_data.is_screened:
                ...
                self.update_incumbent(node_model.component(self.original_util_block.name))
                self._transfer_incumbent_to_original_model(config.logger)
                self.LB = node_data.obj_lb if self.objective_sense == minimize else -node_data.obj_ub
                self.UB = node_data.obj_ub if self.objective_sense == minimize else -node_data.obj_lb
