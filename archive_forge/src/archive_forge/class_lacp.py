...
class lacp(packet_base.PacketBase):
    ...
    _ALL_PACK_LEN = _HLEN_PACK_LEN + _ACTPRT_INFO_PACK_LEN * 2 + _COL_INFO_PACK_LEN + _TRM_PACK_LEN
    ...
    def serialize(self, payload, prev):
        header = struct.pack(self._HLEN_PACK_STR, self._subtype, self.version)
        actor = struct.pack(self._ACTPRT_INFO_PACK_STR, self._actor_tag, self._actor_length, self.actor_system_priority, addrconv.mac.text_to_bin(self.actor_system), self.actor_key, self.actor_port_priority, self.actor_port, self._actor_state)
        partner = struct.pack(self._ACTPRT_INFO_PACK_STR, self._partner_tag, self._partner_length, self.partner_system_priority, addrconv.mac.text_to_bin(self.partner_system), self.partner_key, self.partner_port_priority, self.partner_port, self._partner_state)
        collector = struct.pack(self._COL_INFO_PACK_STR, self._collector_tag, self._collector_length, self.collector_max_delay)
        terminator = struct.pack(self._TRM_PACK_STR, self._terminator_tag, self._terminator_length)
