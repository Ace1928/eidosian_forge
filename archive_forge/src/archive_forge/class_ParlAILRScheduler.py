...
class ParlAILRScheduler(object):
    ...
    def add_cmdline_args(cls, argparser):
        ...
        lr_group.add_argument('--lr-scheduler', type=str, default='reduceonplateau', choices=['reduceonplateau', 'none', 'fixed', 'invsqrt', 'cosine', 'linear'], help='Learning rate scheduler.')
        lr_group.add_argument('--lr-scheduler-patience', type=int, default=3, help='LR scheduler patience. In number of validation runs. If using fixed scheduler, LR is decayed every <patience> validations.')
        lr_group.add_argument('--lr-scheduler-decay', type=float, default=0.5, help='Decay factor for LR scheduler, or how much LR is multiplied by when it is lowered.')
        lr_group.add_argument('--max-lr-steps', type=int, default=-1, help='Number of train steps the scheduler should take after warmup. Training is terminated after this many steps. This should only be set for --lr-scheduler cosine or linear')
        lr_group.add_argument('--invsqrt-lr-decay-gamma', type=int, default=-1, help='Constant used only to find the lr multiplier for the invsqrt scheduler. Must be set for --lr-scheduler invsqrt')
        lr_group.add_argument('--warmup-updates', type=int, default=-1, hidden=True, help='Learning rate warmup period, in number of SGD updates. Linearly scales up LR over period. Only enabled if > 0.')
        lr_group.add_argument('--warmup-rate', type=float, default=0.0001, hidden=True, help='Warmup learning rate *multiplier*. Initial LR is multiplied by this value. Linearly adjusted up to 1.0 across --warmup-updates steps.')
        lr_group.add_argument('--update-freq', type=int, default=1, hidden=True, help='Accumulate gradients N times before performing an optimizer.step().')
    ...
    def lr_scheduler_factory(cls, opt, optimizer, states, hard_reset=False):
        ...
        warmup_rate = opt.get('warmup_rate', 0.0001)
        max_lr_steps = opt.get('max_lr_steps', -1)
        invsqrt_lr_decay_gamma = opt.get('invsqrt_lr_decay_gamma', -1)
        if opt.get('lr_scheduler') == 'none':
            return None
        elif decay == 1.0:
            warn_once('Your LR decay is set to 1.0. Assuming you meant you wanted to disable learning rate scheduling. Adjust --lr-scheduler-decay if this is not correct.')
            return None
        elif opt.get('lr_scheduler') == 'reduceonplateau':
            scheduler = ReduceOnPlateauLRScheduler(optimizer, hard_reset, patience, decay, warmup_updates, warmup_rate)
        elif opt.get('lr_scheduler') == 'fixed':
            scheduler = FixedLRScheduler(optimizer, hard_reset, patience, decay, warmup_updates, warmup_rate)
        elif opt.get('lr_scheduler') == 'invsqrt':
            scheduler = InvSqrtLRScheduler(optimizer, hard_reset, patience, decay, warmup_updates, warmup_rate, invsqrt_lr_decay_gamma)
        elif opt.get('lr_scheduler') == 'cosine':
            scheduler = CosineLRScheduler(optimizer, hard_reset, patience, decay, warmup_updates, warmup_rate, max_lr_steps)
        elif opt.get('lr_scheduler') == 'linear':
            scheduler = LinearLRScheduler(optimizer, hard_reset, patience, decay, warmup_updates, warmup_rate, max_lr_steps)
        else:
            raise ValueError("Don't know what to do with --lr-scheduler '{}'".format(opt.get('lr_scheduler')))
        if states and 'lr_scheduler_type' in states and (states.get('lr_scheduler_type') != opt['lr_scheduler']) and (not hard_reset):
            warn_once(f'LR scheduler ({opt['lr_scheduler']}) is different from saved ({states.get('lr_scheduler_type')}). Starting fresh!')
            hard_reset = True
        if not hard_reset:
