...
class ClientSettings(BaseSettings):
    ...
    api_dev_mode: Optional[bool] = False
    ...
    def get_headers(self):
        ...
        if self.api_key and self.api_header:
            _base_headers[self.api_header] = self.api_key
        return _base_headers

    @property
    def endpoints(self) -> Dict[str, str]:
        return {'local': self.local_url, 'cluster': self.cluster_url, 'external': self.external_url}