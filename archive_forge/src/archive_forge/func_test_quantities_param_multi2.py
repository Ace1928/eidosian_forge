...
def test_quantities_param_multi2():
    ...
    units = [1 / pq.s]
    odesys = ODESys(sine, sine_jac, param_names=['k'], par_by_name=True, to_arrays_callbacks=(None, None, lambda p: np.array([[elem.rescale(u).magnitude for elem in parvals] for parvals, u in zip(p.T, units)]).T))
    A = 2.0
    kvals = (7452.0, 13853.0, 22123.0)
    results = odesys.integrate(np.linspace(0, 1), [[0, A * kval / 3600] for kval in kvals], {'k': kvals / pq.hour})
