...
class MistralExternalResource(resource.Resource):
    ...
    def _handle_action(self, action, inputs=None):
        ...
        if action_data:
            ...
            execution = self.client().executions.create(action_data[self.WORKFLOW], workflow_input=jsonutils.dumps(inputs), description=self.properties[self.DESCRIPTION], **action_data[self.PARAMS])
            LOG.debug('Mistral execution %(id)s params set to %(params)s' % {'id': execution.id, 'params': action_data[self.PARAMS]})
            return execution.id
    ...
    def _check_action(self, action, execution_id):
        ...
        if execution_id:
            ...
            if output.get('resource_id'):
                ...
                LOG.debug('ExternalResource id set to %(rid)s from Mistral execution %(eid)s output' % {'eid': execution_id, 'rid': rsrc_id})
    ...
    def _needs_update(self, after, before, after_props, before_props, prev_resource, check_init_complete=True):
        ...
        for i in after_props[self.REPLACE_ON_CHANGE]:
            if old_inputs.get(i) != new_inputs.get(i):
                LOG.debug('Replacing ExternalResource %(id)s instead of updating due to change to input "%(i)s"' % {'id': self.resource_id, 'i': i})
                raise resource.UpdateReplace(self)
        if self.properties[self.ALWAYS_UPDATE]:
            return True
        else:
            return super(MistralExternalResource, self)._needs_update(after, before, after_props, before_props, prev_resource, check_init_complete)
