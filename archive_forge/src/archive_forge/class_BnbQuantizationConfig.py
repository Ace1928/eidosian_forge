...
class BnbQuantizationConfig:
    ...
    bnb_4bit_quant_type: str = field(default='fp4', metadata={'help': "set the quantization data type in the `bnb.nn.Linear4Bit` layers. Options are {'fp4','np4'}."})
    bnb_4bit_use_double_quant: bool = field(default=False, metadata={'help': 'enable nested quantization where the quantization constants from the first quantization are quantized again.'})
    bnb_4bit_compute_dtype: bool = field(default='fp16', metadata={'help': "This sets the computational type which might be different than the input time. For example, inputs might be fp32, but computation can be set to bf16 for speedups. Options are {'fp32','fp16','bf16'}."})
    torch_dtype: torch.dtype = field(default=None, metadata={'help': 'this sets the dtype of the remaining non quantized layers. `bitsandbytes` library suggests to set the valueto `torch.float16` for 8 bit model and use the same dtype as the compute dtype for 4 bit model '})
    ...
    def __post_init__(self):
        ...
        if isinstance(self.bnb_4bit_compute_dtype, str):
            if self.bnb_4bit_compute_dtype == 'fp32':
                ...
                raise ValueError(f"bnb_4bit_compute_dtype must be in ['fp32','fp16','bf16'] but found {self.bnb_4bit_compute_dtype}")
