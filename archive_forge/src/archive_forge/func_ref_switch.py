...
def ref_switch(data: _atypes.TensorFuzzingAnnotation[TV_RefSwitch_T], pred: _atypes.TensorFuzzingAnnotation[_atypes.Bool], name=None):
  ...
  """
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('RefSwitch', data=data, pred=pred, name=name)
    _result = _outputs[:]
    if _execute.must_record_gradient():
        _attrs = ('T', _op._get_attr_type('T'))
        _inputs_flat = _op.inputs
        _execute.record_gradient('RefSwitch', _inputs_flat, _attrs, _result)
