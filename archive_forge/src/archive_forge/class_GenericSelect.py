...
class GenericSelect(select_and_prepare.SelectOracle, unary_iteration_gate.UnaryIterationGate):
    ...
    def __attrs_post_init__(self):
        if any((len(dps) != self.target_bitsize for dps in self.select_unitaries)):
            raise ValueError(f'Each dense pauli string in {self.select_unitaries} should contain {self.target_bitsize} terms.')
        min_bitsize = (len(self.select_unitaries) - 1).bit_length()
        if self.selection_bitsize < min_bitsize:
            raise ValueError(f'selection_bitsize={self.selection_bitsize} should be at-least {min_bitsize}')
    ...
    def selection_registers(self) -> Tuple[infra.SelectionRegister, ...]:
        return (infra.SelectionRegister('selection', self.selection_bitsize, len(self.select_unitaries)),)

    @cached_property
    def target_registers(self) -> Tuple[infra.Register, ...]:
        return (infra.Register('target', self.target_bitsize),)

    def decompose_from_registers(self, context, **quregs: NDArray[cirq.Qid]) -> cirq.OP_TREE:
    ...
    def nth_operation(self, context: cirq.DecompositionContext, selection: int, control: cirq.Qid, target: Sequence[cirq.Qid]) -> cirq.OP_TREE:
    ...
    def controlled(self, num_controls: Optional[int]=None, control_values: Optional[Union[cirq.ops.AbstractControlValues, Sequence[Union[int, Collection[int]]]]]=None, control_qid_shape: Optional[Tuple[int, ...]]=None) -> 'GenericSelect':
        ...
        if isinstance(control_values, Sequence) and isinstance(control_values[0], int) and (len(control_values) == 1) and (self.control_val is None):
            return GenericSelect(self.selection_bitsize, self.target_bitsize, self.select_unitaries, control_val=control_values[0])
        raise NotImplementedError(f'Cannot create a controlled version of {self} with control_values={control_values}.')

    def __repr__(self) -> str:
        return f'cirq_ft.GenericSelect({self.selection_bitsize},{self.target_bitsize}, {self.select_unitaries}, {self.control_val})'