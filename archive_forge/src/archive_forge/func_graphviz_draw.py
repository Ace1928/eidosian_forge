...
def graphviz_draw(graph: 'PyDiGraph[_S, _T] | PyGraph[_S, _T]', node_attr_fn: Callable[[_S], dict[str, str]] | None=None, edge_attr_fn: Callable[[_T], dict[str, str]] | None=None, graph_attr: dict[str, str] | None=None, filename: str | None=None, image_type: str | None=None, method: str | None=None) -> Image | None:
    ...
    """
    if not HAS_PILLOW:
        raise ImportError("Pillow is necessary to use graphviz_draw() it can be installed with 'pip install pydot pillow'")
    try:
        subprocess.run(['dot', '-V'], cwd=tempfile.gettempdir(), check=True, capture_output=True)
    except Exception:
        raise RuntimeError('Graphviz could not be found or run. This function requires that Graphviz is installed. If you need to install Graphviz you can refer to: https://graphviz.org/download/#executable-packages for instructions.')
    dot_str = cast(str, graph.to_dot(node_attr_fn, edge_attr_fn, graph_attr))
    if image_type is None:
        output_format = 'png'
    else:
        if image_type not in IMAGE_TYPES:
            raise ValueError(f"The specified value for the image_type argument, '{image_type}' is not a valid choice. It must be one of: {IMAGE_TYPES}")
        output_format = image_type
    if method is None:
        prog = 'dot'
    else:
        if method not in METHODS:
            raise ValueError(f"The specified value for the method argument, '{method}' is not a valid choice. It must be one of: {METHODS}")
        prog = method
    if not filename:
        dot_result = subprocess.run([prog, '-T', output_format], input=dot_str.encode('utf-8'), capture_output=True, encoding=None, check=True, text=False)
        dot_bytes_image = io.BytesIO(dot_result.stdout)
        image = Image.open(dot_bytes_image)
        return image
    else:
        subprocess.run([prog, '-T', output_format, '-o', filename], input=dot_str, check=True, encoding='utf8', text=True)
        return None