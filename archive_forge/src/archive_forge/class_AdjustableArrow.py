    ...
    def draw(self):
        ...
        if self.right:
            ...
            if boxAnchor in ('w', 'c', 'e'):
                ...
                dx = 0
            points = [dx, dy + headProjection + stemThickness, dx + stemLength, dy + headProjection + stemThickness, dx + stemLength + headSweep, dy + 2 * headProjection + stemThickness, dx + stemLength + headLength, dy + 0.5 * stemThickness + headProjection, dx + stemLength + headSweep, dy, dx + stemLength, dy + headProjection, dx, dy + headProjection]
        else:
            w, h = (h, w)
            ...
            if boxAnchor in ('ne', 'e', 'se'):
                ...
                dx = 0
            points = [dx + headProjection, dy, dx + headProjection + stemThickness, dy, dx + headProjection + stemThickness, dy + stemLength, dx + w, dy + stemLength + headSweep, dx + headProjection + 0.5 * stemThickness, dy + h, dx, dy + stemLength + headSweep, dx + headProjection, dy + stemLength]
        g.add(Polygon(points=points, fillColor=self.fillColor, strokeColor=self.strokeColor, strokeWidth=self.strokeWidth))
        g.translate(x, y)
        g.rotate(self.angle)
        return g