...
def test_ui_seek_ahead_message(self):
    ...
    num_objects = 10
    total_size = 1024 ** 3
    PutToQueueWithTimeout(status_queue, SeekAheadMessage(num_objects, total_size, start_time))
    fpath = self.CreateTempFile(file_name='sample-file.txt', contents=b'foo')
    PutToQueueWithTimeout(status_queue, FileMessage(StorageUrlFromString(suri(fpath)), None, start_time + 10, size=UPLOAD_SIZE, message_type=FileMessage.FILE_UPLOAD, finished=False))
    PutToQueueWithTimeout(status_queue, FileMessage(StorageUrlFromString(suri(fpath)), None, start_time + 20, size=UPLOAD_SIZE, message_type=FileMessage.FILE_UPLOAD, finished=True))
    PutToQueueWithTimeout(status_queue, ZERO_TASKS_TO_DO_ARGUMENT)
    JoinThreadAndRaiseOnTimeout(ui_thread)
    content = stream.getvalue()
    expected_message = 'Estimated work for this command: objects: %s, total size: %s\n' % (num_objects, MakeHumanReadable(total_size))
