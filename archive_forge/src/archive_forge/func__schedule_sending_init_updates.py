...
def _schedule_sending_init_updates(self):
    ...
    self._rtc_eor_timer = self._create_timer(Peer.RTC_EOR_TIMER_NAME, _enqueue_non_rtc_init_updates)
    self._rtc_eor_timer.start(const.RTC_EOR_DEFAULT_TIME, now=False)
    LOG.debug('Scheduled sending of initial Non-RTC UPDATEs after: %s sec', const.RTC_EOR_DEFAULT_TIME)