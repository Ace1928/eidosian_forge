...
class AzureRMStorageAccount(AzureRMModuleBase):
    ...
    def update_account(self):
        ...
        if self.encryption is not None:
            ...
            if self.encryption.get('require_infrastructure_encryption') and bool(self.encryption.get('require_infrastructure_encryption')) != bool(self.account_dict['encryption']['require_infrastructure_encryption']):
    ...
    def create_account(self):
        ...
        self.results['changed'] = True
        if self.check_mode:
            account_dict = dict(location=self.location, account_type=self.account_type, name=self.name, resource_group=self.resource_group, enable_https_traffic_only=self.https_only, minimum_tls_version=self.minimum_tls_version, public_network_access=self.public_network_access, allow_blob_public_access=self.allow_blob_public_access, encryption=self.encryption, is_hns_enabled=self.is_hns_enabled, tags=dict())
            if self.tags:
