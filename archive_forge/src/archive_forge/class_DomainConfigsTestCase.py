...
class DomainConfigsTestCase(base.V3ClientTestCase):
    ...
    def test_create_invalid_domain_config(self):
        ...
        self.assertRaises(http.Forbidden, self.client.domain_configs.create, self.test_domain.id, invalid_groups_ref)
        invalid_options_ref = {'identity': {uuid.uuid4().hex: uuid.uuid4().hex}, 'ldap': {uuid.uuid4().hex: uuid.uuid4().hex}}
        self.assertRaises(http.Forbidden, self.client.domain_configs.create, self.test_domain.id, invalid_options_ref)
    ...
    def test_domain_config_delete(self):
        ...
        self.assertRaises(http.NotFound, self.client.domain_configs.get, self.project_domain_id)