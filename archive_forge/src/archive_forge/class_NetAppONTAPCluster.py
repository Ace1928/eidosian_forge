...
class NetAppONTAPCluster:
    ...
    def get_cluster_identity_rest(self):
        ...
        if record:
            cluster_info = {'cluster_contact': record.get('contact'), 'cluster_location': record.get('location'), 'cluster_name': record.get('name'), 'timezone': self.na_helper.safe_get(record, ['timezone'])}
    ...
    def create_node_body(self):
        node = {}
        for param_key, rest_key in {'cluster_ip_address': 'cluster_interface.ip.address', 'cluster_location': 'location', 'node_name': 'name'}.items():
    ...
    def create_cluster(self, older_api=False):
        ...
        cluster_create = netapp_utils.zapi.NaElement.create_node_with_children('cluster-create', **options)
        try:
            self.server.invoke_successfully(cluster_create, enable_tunneling=True)
        except netapp_utils.zapi.NaApiError as error:
    ...
    def get_uuid(self):
        if self.parameters.get('cluster_ip_address') is not None:
            ...
            uuid = None
        if uuid is None:
            self.module.fail_json(msg='Internal error, cannot find UUID in %s: for %s or %s' % (self.get_cluster_nodes_rest(), self.parameters['cluster_ip_address'], self.parameters.get('node_name') is not None), exception=traceback.format_exc())
    ...
    def modify_web_services(self):
        body = {'certificate': {'uuid': self.parameters['certificate']['uuid']}}
        dummy, error = rest_generic.patch_async(self.rest_api, 'cluster/web', None, body)
        if error:
            self.module.fail_json(msg='Error modifying cluster web service config for %s: %s' % (self.parameters['cluster_name'], to_native(error)), exception=traceback.format_exc())
