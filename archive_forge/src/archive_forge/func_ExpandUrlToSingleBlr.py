...
def ExpandUrlToSingleBlr(url_str, gsutil_api, project_id, treat_nonexistent_object_as_subdir=False, logger=None):
  ...
  """
    ...
    if ContainsWildcard(url_str):
        blr_expansion = list(CreateWildcardIterator(url_str, gsutil_api, project_id=project_id, logger=logger))
        if len(blr_expansion) != 1:
            raise CommandException('Destination (%s) must match exactly 1 URL' % url_str)
        blr = blr_expansion[0]
    ...
    list_iterator = gsutil_api.ListObjects(storage_url.bucket_name, prefix=prefix, delimiter='/', provider=storage_url.scheme, fields=['prefixes', 'items/name'])
    for obj_or_prefix in list_iterator:
        if obj_or_prefix.datatype == CloudApi.CsObjectOrPrefixType.PREFIX and obj_or_prefix.data == prefix + '/':
            return (storage_url, True)
        elif obj_or_prefix.datatype == CloudApi.CsObjectOrPrefixType.OBJECT and obj_or_prefix.data.name == storage_url.object_name + '_$folder$':
            return (storage_url, True)
        elif obj_or_prefix.datatype == CloudApi.CsObjectOrPrefixType.OBJECT and obj_or_prefix.data.name == storage_url.object_name:
