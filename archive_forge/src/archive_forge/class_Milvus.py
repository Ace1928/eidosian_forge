...
class Milvus(VectorStore):
    ...
    def __init__(self, embedding_function: Embeddings, collection_name: str='LangChainCollection', collection_description: str='', collection_properties: Optional[dict[str, Any]]=None, connection_args: Optional[dict[str, Any]]=None, consistency_level: str='Session', index_params: Optional[dict]=None, search_params: Optional[dict]=None, drop_old: Optional[bool]=False, auto_id: bool=False, *, primary_field: str='pk', text_field: str='text', vector_field: str='vector', metadata_field: Optional[str]=None, partition_key_field: Optional[str]=None, partition_names: Optional[list]=None, replica_number: int=1, timeout: Optional[float]=None, num_shards: Optional[int]=None):
        ...
        if drop_old and isinstance(self.col, Collection):
            self.col.drop()
            self.col = None
        self._init(partition_names=partition_names, replica_number=replica_number, timeout=timeout)
    ...
    def _init(self, embeddings: Optional[list]=None, metadatas: Optional[list[dict]]=None, partition_names: Optional[list]=None, replica_number: int=1, timeout: Optional[float]=None) -> None:
        if embeddings is not None:
            self._create_collection(embeddings, metadatas)
        self._extract_fields()
        self._create_index()
        self._create_search_params()
        self._load(partition_names=partition_names, replica_number=replica_number, timeout=timeout)
    ...
    def _create_collection(self, embeddings: list, metadatas: Optional[list[dict]]=None) -> None:
        ...
        if self._metadata_field is not None:
            ...
            for key, value in metadatas[0].items():
                ...
                if dtype == DataType.UNKNOWN or dtype == DataType.NONE:
                    logger.error('Failure to create collection, unrecognized dtype for key: %s', key)
        ...
        if self.auto_id:
            ...
            fields.append(FieldSchema(self._primary_field, DataType.VARCHAR, is_primary=True, auto_id=False, max_length=65535))
        fields.append(FieldSchema(self._vector_field, infer_dtype_bydata(embeddings[0]), dim=dim))
        schema = CollectionSchema(fields, description=self.collection_description, partition_key_field=self._partition_key_field)
        try:
            if self.num_shards is not None:
                self.col = Collection(name=self.collection_name, schema=schema, consistency_level=self.consistency_level, using=self.alias, num_shards=self.num_shards)
            else:
        ...
        if isinstance(self.col, Collection) and self._get_index() is None:
            try:
                if self.index_params is None:
                    self.index_params = {'metric_type': 'L2', 'index_type': 'HNSW', 'params': {'M': 8, 'efConstruction': 64}}
                try:
    ...
    def _load(self, partition_names: Optional[list]=None, replica_number: int=1, timeout: Optional[float]=None) -> None:
        """Load the collection if available."""
        from pymilvus import Collection, utility
        from pymilvus.client.types import LoadState
        timeout = self.timeout or timeout
        if isinstance(self.col, Collection) and self._get_index() is not None and (utility.load_state(self.collection_name, using=self.alias) == LoadState.NotLoad):
            self.col.load(partition_names=partition_names, replica_number=replica_number, timeout=timeout)

    def add_texts(self, texts: Iterable[str], metadatas: Optional[List[dict]]=None, timeout: Optional[float]=None, batch_size: int=1000, *, ids: Optional[List[str]]=None, **kwargs: Any) -> List[str]:
        ...
        if not self.auto_id:
            ...
            assert all((len(x.encode()) <= 65535 for x in ids)), 'Each id should be a string less than 65535 bytes.'
        try:
        ...
        if self._metadata_field is not None:
            ...
            for d in metadatas:
                for key, value in d.items():
                    keys = [x for x in self.fields if x != self._primary_field] if self.auto_id else [x for x in self.fields]
                    if key in keys:
    ...
    def similarity_search(self, query: str, k: int=4, param: Optional[dict]=None, expr: Optional[str]=None, timeout: Optional[float]=None, **kwargs: Any) -> List[Document]:
    ...
    def similarity_search_by_vector(self, embedding: List[float], k: int=4, param: Optional[dict]=None, expr: Optional[str]=None, timeout: Optional[float]=None, **kwargs: Any) -> List[Document]:
        """Perform a similarity search against the query string.
    ...
    def similarity_search_with_score(self, query: str, k: int=4, param: Optional[dict]=None, expr: Optional[str]=None, timeout: Optional[float]=None, **kwargs: Any) -> List[Tuple[Document, float]]:
    ...
    def similarity_search_with_score_by_vector(self, embedding: List[float], k: int=4, param: Optional[dict]=None, expr: Optional[str]=None, timeout: Optional[float]=None, **kwargs: Any) -> List[Tuple[Document, float]]:
        """Perform a search on a query string and return results with score.
    ...
    def max_marginal_relevance_search(self, query: str, k: int=4, fetch_k: int=20, lambda_mult: float=0.5, param: Optional[dict]=None, expr: Optional[str]=None, timeout: Optional[float]=None, **kwargs: Any) -> List[Document]:
    ...
    def max_marginal_relevance_search_by_vector(self, embedding: list[float], k: int=4, fetch_k: int=20, lambda_mult: float=0.5, param: Optional[dict]=None, expr: Optional[str]=None, timeout: Optional[float]=None, **kwargs: Any) -> List[Document]:
        """Perform a search and return results that are reordered by MMR.
    ...
    def delete(self, ids: Optional[List[str]]=None, expr: Optional[str]=None, **kwargs: str):
    ...
    def from_texts(cls, texts: List[str], embedding: Embeddings, metadatas: Optional[List[dict]]=None, collection_name: str='LangChainCollection', connection_args: dict[str, Any]=DEFAULT_MILVUS_CONNECTION, consistency_level: str='Session', index_params: Optional[dict]=None, search_params: Optional[dict]=None, drop_old: bool=False, *, ids: Optional[List[str]]=None, **kwargs: Any) -> Milvus:
        ...
        if isinstance(ids, list) and len(ids) > 0:
            ...
            auto_id = True
        vector_db = cls(embedding_function=embedding, collection_name=collection_name, connection_args=connection_args, consistency_level=consistency_level, index_params=index_params, search_params=search_params, drop_old=drop_old, auto_id=auto_id, **kwargs)
    ...
    def upsert(self, ids: Optional[List[str]]=None, documents: List[Document] | None=None, **kwargs: Any) -> List[str] | None:
