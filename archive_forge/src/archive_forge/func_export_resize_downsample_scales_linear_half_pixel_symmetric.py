...
def export_resize_downsample_scales_linear_half_pixel_symmetric() -> None:
    node = onnx.helper.make_node('Resize', inputs=['X', '', 'scales'], outputs=['Y'], mode='linear', coordinate_transformation_mode='half_pixel_symmetric')
    data = np.array([[[[1, 2, 3, 4]]]], dtype=np.float32)
    scales = np.array([1.0, 1.0, 1.0, 0.6], dtype=np.float32)
    output = interpolate_nd(data, lambda x, _: linear_coeffs(x), scale_factors=scales, coordinate_transformation_mode='half_pixel_symmetric').astype(np.float32)
    expect(node, inputs=[data, scales], outputs=[output], name='test_resize_downsample_scales_linear_half_pixel_symmetric')