...
class NetworkFlavorTests(common.NetworkTests):
    ...
    def test_network_flavor_delete(self):
        ...
        name1 = uuid.uuid4().hex
        cmd_output = self.openstack('network flavor create --description testdescription --enable  --service-type L3_ROUTER_NAT ' + name1, parse_output=True)
        ...
        cmd_output = self.openstack('network flavor create --description testdescription1 --disable --service-type  L3_ROUTER_NAT ' + name2, parse_output=True)
        self.assertEqual(name2, cmd_output['name'])
        self.assertFalse(cmd_output['enabled'])
        self.assertEqual('testdescription1', cmd_output['description'])
        raw_output = self.openstack('network flavor delete ' + name1 + ' ' + name2)
    ...
    def test_network_flavor_list(self):
        ...
        self.assertEqual(name1, cmd_output['name'])
        self.assertEqual(True, cmd_output['enabled'])
        self.assertEqual('testdescription', cmd_output['description'])
        name2 = uuid.uuid4().hex
        cmd_output = self.openstack('network flavor create --description testdescription1 --disable --service-type  L3_ROUTER_NAT ' + name2, parse_output=True)
        self.assertEqual(name2, cmd_output['name'])
        ...
        cmd_output = self.openstack('network flavor list ', parse_output=True)
