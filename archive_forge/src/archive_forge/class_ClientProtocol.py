...
class ClientProtocol(Protocol):
    ...
    def __init__(self, wsuri: WebSocketURI, *, origin: Optional[Origin]=None, extensions: Optional[Sequence[ClientExtensionFactory]]=None, subprotocols: Optional[Sequence[Subprotocol]]=None, state: State=CONNECTING, max_size: Optional[int]=2 ** 20, logger: Optional[LoggerLike]=None):
        super().__init__(side=CLIENT, state=state, max_size=max_size, logger=logger)
        self.wsuri = wsuri
        self.origin = origin
    ...
    def connect(self) -> Request:
        ...
        """
        headers = Headers()
        headers['Host'] = build_host(self.wsuri.host, self.wsuri.port, self.wsuri.secure)
        ...
        if self.available_extensions is not None:
            extensions_header = build_extension([(extension_factory.name, extension_factory.get_request_params()) for extension_factory in self.available_extensions])
    ...
    def process_subprotocol(self, headers: Headers) -> Optional[Subprotocol]:
        ...
        if subprotocols:
            ...
            parsed_subprotocols: Sequence[Subprotocol] = sum([parse_subprotocol(header_value) for header_value in subprotocols], [])
    ...
    def parse(self) -> Generator[None, None, None]:
        if self.state is CONNECTING:
            try:
                response = (yield from Response.parse(self.reader.read_line, self.reader.read_exact, self.reader.read_to_eof))
