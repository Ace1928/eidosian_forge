...
class Airfoil(Graph):
    ...
    def __init__(self, **kwargs):
        ...
        W = (A + A.T) / 2.0
        plotting = {'vertex_size': 30, 'limits': np.array([-0.0001, 1.01 * data['x'].max(), -0.0001, 1.01 * data['y'].max()])}
        super(Airfoil, self).__init__(W=W, coords=coords, plotting=plotting, gtype='Airfoil', **kwargs)