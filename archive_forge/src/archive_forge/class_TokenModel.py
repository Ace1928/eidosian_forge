...
class TokenModel(object):
    ...
    def __repr__(self):
        ...
        desc = '<%(type)s (audit_id=%(audit_id)s, audit_chain_id=%(audit_ids)s) at %(loc)s>'
        self_cls_name = reflection.get_class_name(self, fully_qualified=False)
        return desc % {'type': self_cls_name, 'audit_id': self.audit_id, 'audit_ids': self.audit_ids, 'loc': hex(id(self))}
    ...
    def domain(self):
        if not self.__domain:
            if self.domain_id:
                self.__domain = PROVIDERS.resource_api.get_domain(self.domain_id)
    ...
    def project(self):
        if not self.__project:
            if self.project_id:
                self.__project = PROVIDERS.resource_api.get_project(self.project_id)
    ...
    def application_credential(self):
        if not self.__application_credential:
            if self.application_credential_id:
                ...
                self.__application_credential = app_cred_api.get_application_credential(self.application_credential_id)
    ...
    def access_token(self):
        if not self.__access_token:
            if self.access_token_id:
                self.__access_token = PROVIDERS.oauth_api.get_access_token(self.access_token_id)
    ...
    def _get_application_credential_roles(self):
        ...
        assignment_list = PROVIDERS.assignment_api.list_role_assignments(user_id=self.user_id, project_id=self.project_id, domain_id=self.domain_id, effective=True)
