...
class MultimodalTextbox(FormComponent):
    ...
    def __init__(self, value: dict[str, str | list] | Callable | None=None, *, file_types: list[str] | None=None, lines: int=1, max_lines: int=20, placeholder: str | None=None, label: str | None=None, info: str | None=None, every: float | None=None, show_label: bool | None=None, container: bool=True, scale: int | None=None, min_width: int=160, interactive: bool | None=None, visible: bool=True, elem_id: str | None=None, autofocus: bool=False, autoscroll: bool=True, elem_classes: list[str] | str | None=None, render: bool=True, text_align: Literal['left', 'right'] | None=None, rtl: bool=False, submit_btn: str | Literal[False] | None=None):
        """
        Parameters:
    ...
    def preprocess(self, payload: MultimodalData | None) -> MultimodalValue | None:
        ...
        if payload is None:
            return None
        return {'text': payload.text, 'files': [f.path for f in payload.files]}
    ...
    def postprocess(self, value: MultimodalValue | None) -> MultimodalData:
        ...
        if not isinstance(value, dict):
            raise ValueError(f"MultimodalTextbox expects a dictionary with optional keys 'text' and 'files'. Received {value.__class__.__name__}")
        text = value.get('text', '')
        if 'files' in value and isinstance(value['files'], list):
            files = [file if isinstance(file, FileData) else FileData(path=file, orig_name=Path(file).name, mime_type=client_utils.get_mimetype(file)) for file in value['files']]
        else:
            files = []
        if not isinstance(text, str):
            raise TypeError(f"Expected 'text' to be a string, but got {type(text).__name__}")
        if not isinstance(files, list):
            raise TypeError(f"Expected 'files' to be a list, but got {type(files).__name__}")
