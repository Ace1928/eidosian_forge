...
class GenerateMappingFile:
    ...
    def run(self, conf_file, log_file, log_level):
        ...
        log.info('Start generate variable mapping file for oVirt ansible disaster recovery')
        dr_tag = 'generate_mapping'
        site, username, password, ca_file, var_file, ansible_play_file = self._init_vars(conf_file, log)
        log.info('Site address: %s \nusername: %s \npassword: *******\nca file location: %s \noutput file location: %s \nansible play location: %s ', site, username, ca_file, var_file, ansible_play_file)
        if not self._validate_connection(log, site, username, password, ca_file):
            self._print_error(log)
            sys.exit()
        extra_vars = 'site={0} username={1} password={2} ca={3} var_file={4}'.format(site, username, password, ca_file, var_file)
        command = ['ansible-playbook', ansible_play_file, '-t', dr_tag, '-e', extra_vars, '-vvvvv']
    ...
    def _validate_output_file_exists(self, output_file, log):
        ...
        if os.path.isfile(output_file):
            ...
            ans = input("%s%sThe output file '%s' already exists. Would you like to override it (y,n)? %s" % (WARN, PREFIX, output_file, END))
            while True:
                ans = ans.lower()
                if ans in valid:
                    if valid[ans]:
                        break
                    msg = 'Failed to create output file. File could not be overridden.'
                    log.error(msg)
                    print('%s%s%s%s' % (FAIL, PREFIX, msg, END))
                    sys.exit(0)
                ans = input("%s%sPlease respond with 'yes' or 'no': %s" % (INPUT, PREFIX, END))
            try:
                os.remove(output_file)
            except OSError:
                log.error('File %s could not be replaced.', output_file)
                print('%s%sFile %s could not be replaced.%s' % (FAIL, PREFIX, output_file, END))
                sys.exit(0)
    ...
    def _init_vars(self, conf_file, log):
        ...
        site = settings.get(_SECTION, _SITE, vars=DefaultOption(settings, _SECTION, site=None))
        username = settings.get(_SECTION, _USERNAME, vars=DefaultOption(settings, _SECTION, username=None))
        password = settings.get(_SECTION, _PASSWORD, vars=DefaultOption(settings, _SECTION, password=None))
        ca_file = settings.get(_SECTION, _CA_FILE, vars=DefaultOption(settings, _SECTION, ca_file=None))
        ca_file = os.path.expanduser(ca_file)
        output_file = settings.get(_SECTION, _OUTPUT_FILE, vars=DefaultOption(settings, _SECTION, output_file=None))
        ...
        if not site:
            site = input('%s%sSite address is not initialized. Please provide the site URL (%s): %s' % (INPUT, PREFIX, SITE_DEF, END)) or SITE_DEF
        if not username:
            username = input('%s%sUsername is not initialized. Please provide the username (%s): %s' % (INPUT, PREFIX, USERNAME_DEF, END)) or USERNAME_DEF
        while not password:
            password = input('%s%sPassword is not initialized. Please provide the password for username %s: %s' % (INPUT, PREFIX, username, END))
        while not os.path.isfile(ca_file):
            ca_file = input("%s%sCA file '%s' does not exist. Please provide the CA file location (%s):%s " % (INPUT, PREFIX, ca_file, CA_DEF, END)) or CA_DEF
            ca_file = os.path.expanduser(ca_file)
        while not output_file:
            output_file = input('%s%sOutput file location is not initialized. Please provide the output file location for the mapping var file (%s): %s' % (INPUT, PREFIX, _OUTPUT_FILE, END)) or _OUTPUT_FILE
            output_file = os.path.expanduser(output_file)
        self._validate_output_file_exists(output_file, log)
        while not os.path.isfile(ansible_play_file):
            ansible_play_file = input("%s%sAnsible play file '%s' does not exist. Please provide the ansible play file to generate the mapping var file (%s): %s" % (INPUT, PREFIX, ansible_play_file, PLAY_DEF, END)) or PLAY_DEF
            ansible_play_file = os.path.expanduser(ansible_play_file)
