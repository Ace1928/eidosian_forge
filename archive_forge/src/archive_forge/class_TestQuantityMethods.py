...
class TestQuantityMethods(TestCase):
    ...
    def test_squeeze(self):
        self.assertQuantityEqual(self.q.reshape([1, 4]).squeeze(), [1, 2, 3, 4] * pq.m)
    ...
    def test_compress(self):
        self.assertQuantityEqual(self.q.compress([False, True], axis=0), [[3, 4]] * pq.m)
        self.assertQuantityEqual(self.q.compress([False, True], axis=1), [[2], [4]] * pq.m)

    def test_searchsorted(self):
        self.assertQuantityEqual(self.q.flatten().searchsorted([1.5, 2.5] * pq.m), [1, 2])
    ...
    def test_clip(self):
        self.methodWithOut('clip', [[1, 2], [2, 2]] * pq.m, max=2 * pq.m)
        self.methodWithOut('clip', [[3, 3], [3, 4]] * pq.m, min=3 * pq.m)
        self.methodWithOut('clip', [[2, 2], [3, 3]] * pq.m, min=2 * pq.m, max=3 * pq.m)
        self.assertRaises(ValueError, self.q.clip, pq.J)
        self.assertRaises(ValueError, self.q.clip, 1)

    def test_round(self):
        q = [1, 1.33, 5.67, 22] * pq.m
        self.methodWithOut('round', [1, 1, 6, 22] * pq.m, q=q, decimals=0)
        self.methodWithOut('round', [0, 0, 10, 20] * pq.m, q=q, decimals=-1)
        self.methodWithOut('round', [1, 1.3, 5.7, 22] * pq.m, q=q, decimals=1)
    ...
    def test_std(self):
        self.methodWithOut('std', 1.118033988749895 * pq.m)
    ...
    def test_cumprod(self):
        ...
        q = self.q / pq.m
        self.methodWithOut('cumprod', [1, 2, 6, 24], q=q)
        self.methodWithOut('cumprod', [[1, 2], [3, 8]], q=q, axis=0)
        self.methodWithOut('cumprod', [[1, 2], [3, 12]], q=q, axis=1)
    ...
    def test_real(self):
        ...
        self.assertQuantityEqual(test_q.real, [[1.0, 1.0], [1.0, 1.0]] * pq.m)

    def test_imag(self):
        test_q = self.q * (1 + 1j)
        test_q.imag = [[39.3701, 39.3701], [39.3701, 39.3701]] * pq.inch
        self.assertQuantityEqual(test_q.imag, [[1.0, 1.0], [1.0, 1.0]] * pq.m)
