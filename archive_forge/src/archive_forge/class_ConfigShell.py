...
class ConfigShell(object):
    ...
    """
    default_prefs = {'color_path': 'magenta', 'color_command': 'cyan', 'color_parameter': 'magenta', 'color_keyword': 'cyan', 'logfile': None, 'loglevel_console': 'info', 'loglevel_file': 'debug9', 'color_mode': True, 'prompt_length': 30, 'tree_max_depth': 0, 'tree_status_mode': True, 'tree_round_nodes': True, 'tree_show_root': True}
    ...
    def __init__(self, preferences_dir=None):
        ...
        if preferences_dir is not None:
            ...
            if not os.path.isfile(self._cmd_history):
                try:
                    ...
                    self.log.warning('Cannot create history file %s, ' % self._cmd_history + 'command history will not be saved.')
                    self._save_history = False
            if os.path.isfile(self._cmd_history) and tty:
                try:
                    readline.read_history_file(self._cmd_history)
                except IOError:
                    self.log.warning('Cannot read command history file %s.' % self._cmd_history)
        ...
        try:
            ...
            self.log.warning('Could not load preferences file %s.' % self._prefs_file)
    ...
    def _complete(self, text, state):
        ...
        if state == 0:
            ...
            if beg == end:
                ...
                current_token = 'kparam'
            self._current_completions = self._dispatch_completion(path, command, pparams, kparams, text, current_token)
            self.log.debug('Returning completions %s to readline.' % str(self._current_completions))
    ...
    def _get_prompt(self):
        ...
        if prompt_length and prompt_length < len(prompt_path):
            ...
            prompt_path = '%s...%s' % (prompt_path[:half], prompt_path[-half:])
        if 'prompt_msg' in dir(self._current_node):
            return '%s%s> ' % (self._current_node.prompt_msg(), prompt_path)
        else:
