...
def test_apply_mixture_apply_unitary_not_implemented():
    ...
    with pytest.raises(TypeError, match='has no'):
        cirq.apply_mixture(ApplyUnitaryNotImplemented(), args=cirq.ApplyMixtureArgs(target_tensor=rho, left_axes=[1], right_axes=[3], out_buffer=out_buf, auxiliary_buffer0=aux_buf0, auxiliary_buffer1=aux_buf1))