...
class KerasLayer(Layer):
    ...
    **Example**
    ...
    def __init__(self, qnode, weight_shapes: dict, output_dim, weight_specs: Optional[dict]=None, **kwargs):
        if not CORRECT_TF_VERSION:
            raise ImportError('KerasLayer requires TensorFlow version 2 or above. The latest version of TensorFlow can be installed using:\npip install tensorflow --upgrade\nAlternatively, visit https://www.tensorflow.org/install for detailed instructions.')
        self.weight_shapes = {weight: tuple(size) if isinstance(size, Iterable) else (size,) if size > 1 else () for weight, size in weight_shapes.items()}
    ...
    def get_config(self) -> dict:
        ...
        config.update({'output_dim': self.output_dim, 'weight_specs': self.weight_specs, 'weight_shapes': self.weight_shapes})
        return config