...
class GandiLiveDNSAPI(object):
    ...
    changed = False
    error_strings = {400: 'Bad request', 401: 'Permission denied', 404: 'Resource not found'}
    ...
    def create_record(self, record, type, values, ttl, domain):
        url = '/domains/%s/records' % domain
        new_record = {'rrset_name': record, 'rrset_type': type, 'rrset_values': values, 'rrset_ttl': ttl}
    ...
    def update_record(self, record, type, values, ttl, domain):
        ...
        new_record = {'rrset_values': values, 'rrset_ttl': ttl}
