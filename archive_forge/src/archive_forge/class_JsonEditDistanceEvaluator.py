...
class JsonEditDistanceEvaluator(StringEvaluator):
    ...
    def __init__(self, string_distance: Optional[Callable[[str, str], float]]=None, canonicalize: Optional[Callable[[Any], Any]]=None, **kwargs: Any) -> None:
        super().__init__()
        if string_distance is not None:
            ...
            try:
                ...
                raise ImportError('The default string_distance operator for the  JsonEditDistanceEvaluator requires installation of the rapidfuzz package. Please install it with `pip install rapidfuzz`.')
            self._string_distance = rfd.DamerauLevenshtein.normalized_distance
        if canonicalize is not None:
            self._canonicalize = canonicalize
        else:
            self._canonicalize = lambda x: json.dumps(x, separators=(',', ':'), sort_keys=True)
