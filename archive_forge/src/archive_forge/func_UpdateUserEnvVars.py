...
def UpdateUserEnvVars(env_vars, workflow, updated_fields):
  ...
  """
    if env_vars is None:
        return
    env_vars_cls = apis.GetClientInstance('workflows', 'v1').MESSAGES_MODULE.Workflow.UserEnvVarsValue
    workflow.userEnvVars = env_vars_cls(additionalProperties=[env_vars_cls.AdditionalProperty(key=key, value=value) for key, value in sorted(env_vars.items())])
