...
def test_rebuild_password(self):
    output, _err = self.run_command('rebuild sample-server %s --rebuild-password asdf' % FAKE_UUID_1)
    ...
    self.assert_called('POST', '/servers/1234/action', {'rebuild': {'imageRef': FAKE_UUID_1, 'adminPass': 'asdf'}}, pos=3)
