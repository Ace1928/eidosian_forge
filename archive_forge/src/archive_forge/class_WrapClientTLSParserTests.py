...
class WrapClientTLSParserTests(unittest.TestCase):
    ...
    def test_hostnameEndpointConstruction(self):
        ...
        """
        reactor = object()
        endpoint = endpoints.clientFromString(reactor, nativeString('tls:example.com:443:timeout=10:bindAddress=127.0.0.1'))
    ...
    def test_utf8Encoding(self):
        ...
        """
        reactor = object()
        endpoint = endpoints.clientFromString(reactor, b'tls:\xc3\xa9xample.example.com:443')
        self.assertEqual(endpoint._wrappedEndpoint._hostBytes, b'xn--xample-9ua.example.com')
        connectionCreator = connectionCreatorFromEndpoint(reactor, endpoint)
        self.assertEqual(connectionCreator._hostname, 'éxample.example.com')
