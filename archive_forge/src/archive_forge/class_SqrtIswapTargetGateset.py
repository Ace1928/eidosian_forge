...
class SqrtIswapTargetGateset(compilation_target_gateset.TwoQubitCompilationTargetGateset):
    ...
    def __init__(self, *, atol: float=1e-08, required_sqrt_iswap_count: Optional[int]=None, use_sqrt_iswap_inv: bool=False, additional_gates: Sequence[Union[Type['cirq.Gate'], 'cirq.Gate', 'cirq.GateFamily']]=()):
        ...
        super().__init__(ops.SQRT_ISWAP_INV if use_sqrt_iswap_inv else ops.SQRT_ISWAP, ops.MeasurementGate, ops.PhasedXZGate, ops.GlobalPhaseGate, *additional_gates, name='SqrtIswapInvTargetGateset' if use_sqrt_iswap_inv else 'SqrtIswapTargetGateset')
        self.additional_gates = tuple((g if isinstance(g, ops.GateFamily) else ops.GateFamily(gate=g) for g in additional_gates))
        self._additional_gates_repr_str = ', '.join([ops.gateset._gate_str(g, repr) for g in additional_gates])
        self.atol = atol
    ...
    def _decompose_two_qubit_operation(self, op: 'cirq.Operation', _) -> DecomposeResult:
        if protocols.has_unitary(op):
            return two_qubit_to_sqrt_iswap.two_qubit_matrix_to_sqrt_iswap_operations(op.qubits[0], op.qubits[1], protocols.unitary(op), required_sqrt_iswap_count=self.required_sqrt_iswap_count, use_sqrt_iswap_inv=self.use_sqrt_iswap_inv, atol=self.atol, check_preconditions=False, clean_operations=True)
        if protocols.is_parameterized(op):
            return two_qubit_to_sqrt_iswap.parameterized_2q_op_to_sqrt_iswap_operations(op, use_sqrt_iswap_inv=self.use_sqrt_iswap_inv)
        return NotImplemented

    def __repr__(self) -> str:
        return f'cirq.SqrtIswapTargetGateset(atol={self.atol}, required_sqrt_iswap_count={self.required_sqrt_iswap_count}, use_sqrt_iswap_inv={self.use_sqrt_iswap_inv}, additional_gates=[{self._additional_gates_repr_str}])'

    def _value_equality_values_(self) -> Any:
        return (self.atol, self.required_sqrt_iswap_count, self.use_sqrt_iswap_inv, frozenset(self.additional_gates))

    def _json_dict_(self) -> Dict[str, Any]:
        d: Dict[str, Any] = {'atol': self.atol, 'required_sqrt_iswap_count': self.required_sqrt_iswap_count, 'use_sqrt_iswap_inv': self.use_sqrt_iswap_inv}
        if self.additional_gates:
            d['additional_gates'] = list(self.additional_gates)
        return d

    @classmethod
    def _from_json_dict_(cls, atol, required_sqrt_iswap_count, use_sqrt_iswap_inv, additional_gates=(), **kwargs):
        return cls(atol=atol, required_sqrt_iswap_count=required_sqrt_iswap_count, use_sqrt_iswap_inv=use_sqrt_iswap_inv, additional_gates=additional_gates)