...
def test_solve_with_store3(self):
    ...
    with open(join(currdir, 'solve_with_store5.out'), 'r') as out, open(join(currdir, 'solve_with_store4.txt'), 'r') as txt:
    ...
    with open(join(currdir, 'solve_with_store6.out'), 'r') as out, open(join(currdir, 'solve_with_store4.txt'), 'r') as txt:
        self.assertStructuredAlmostEqual(yaml.full_load(txt), yaml.full_load(out), allow_second_superset=True)
    ...
    with open(join(currdir, 'solve_with_store7.out'), 'r') as out, open(join(currdir, 'solve_with_store4.txt'), 'r') as txt:
        self.assertStructuredAlmostEqual(yaml.full_load(txt), yaml.full_load(out), allow_second_superset=True)