...
class BlazarHostTestCase(common.HeatTestCase):
    ...
    def setUp(self):
        ...
        self.host = {'id': uuids.id, 'name': 'test-host', 'gpu': True, 'hypervisor_hostname': 'compute-1', 'hypervisor_type': 'QEMU', 'hypervisor_version': 2010001, 'cpu_info': "{'arch': 'x86_64', 'model': 'cpu64-rhel6', 'vendor': 'Intel', 'features': ['pge', 'clflush', 'sep', 'syscall', 'msr', 'vmx', 'cmov', 'nx', 'pat', 'lm', 'tsc', 'fpu', 'fxsr', 'pae', 'mmx', 'cx8', 'mce', 'de', 'mca', 'pse', 'pni', 'apic', 'sse', 'lahf_lm', 'sse2', 'hypervisor', 'cx16', 'pse36', 'mttr', 'x2apic'], 'topology': {'cores': 1, 'cells': 1, 'threads': 1, 'sockets': 4}}", 'memory_mb': 8192, 'local_gb': 100, 'vcpus': 2, 'service_name': 'compute-1', 'reservable': True, 'trust_id': uuids.trust_id, 'created_at': '2020-01-01 08:00', 'updated_at': '2020-01-01 12:00', 'extra_capability': 'foo'}
    ...
    def test_host_create(self):
        ...
        self.assertEqual(self.host['name'], host_resource.properties.get(host.Host.NAME))
        scheduler.TaskRunner(host_resource.create)()
        self.assertEqual(uuids.id, host_resource.resource_id)
        self.assertEqual((host_resource.CREATE, host_resource.COMPLETE), host_resource.state)
        self.assertEqual('host', host_resource.entity)
        self.client.host.create.assert_called_once_with(name=self.host['name'], gpu=self.host['gpu'])

    def test_host_delete(self):
        host_resource = self._create_resource('host', self.rsrc_defn, self.stack)
        scheduler.TaskRunner(host_resource.create)()
        self.client.host.delete.return_value = None
        self.client.host.get.side_effect = ['host_obj', client_exception.BlazarClientException(code=404)]
    ...
    def test_resolve_attributes(self):
        ...
        self.assertEqual(self.host['vcpus'], host_resource._resolve_attribute(host.Host.VCPUS))
