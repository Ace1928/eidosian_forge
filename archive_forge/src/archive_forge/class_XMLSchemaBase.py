...
class XMLSchemaBase(XsdValidator, ElementPathMixin[Union[SchemaType, XsdElement]], metaclass=XMLSchemaMeta):
    """
    ...
    def __init__(self, source: Union[SchemaSourceType, List[SchemaSourceType]], namespace: Optional[str]=None, validation: str='strict', global_maps: Optional[XsdGlobals]=None, converter: Optional[ConverterType]=None, locations: Optional[LocationsType]=None, base_url: Optional[str]=None, allow: str='all', defuse: str='remote', timeout: int=300, build: bool=True, use_meta: bool=True, use_fallback: bool=True, loglevel: Optional[Union[str, int]]=None) -> None:
        ...
        if loglevel is not None:
            if isinstance(loglevel, str):
                ...
                if level not in {'DEBUG', 'INFO', 'WARN', 'WARNING', 'ERROR', 'CRITICAL'}:
                    raise XMLSchemaValueError(_('{!r} is not a valid loglevel').format(loglevel))
        ...
        if 'blockDefault' in root.attrib:
            if self.meta_schema is None:
                ...
                try:
                    self.block_default = get_xsd_derivation_attribute(root, 'blockDefault', {'extension', 'restriction', 'substitution'})
    ...
    def __repr__(self) -> str:
        if self.url:
            return '%s(name=%r, namespace=%r)' % (self.__class__.__name__, self.name, self.target_namespace)
    ...
    def xpath_node(self) -> SchemaElementNode:
        if self._xpath_node is None:
            self._xpath_node = build_schema_node_tree(cast(Union[XsdSchemaProtocol], self))
    ...
    def builtin_types(cls) -> NamespaceView[BaseXsdType]:
        ...
        try:
            ...
            raise XMLSchemaNotBuiltError(cls.meta_schema, _('missing XSD namespace in meta-schema'))
        else:
    ...
    def annotations(self) -> List[XsdAnnotation]:
        if self._annotations is None:
            self._annotations = [XsdAnnotation(child, self) for child in self.source.root if child.tag == XSD_ANNOTATION]
    ...
    def create_empty_content_group(self, parent: Union[XsdComplexType, XsdGroup], model: str='sequence', **attrib: Any) -> XsdGroup:
        if model == 'sequence':
    ...
    def create_any_attribute_group(self, parent: Union[XsdComplexType, XsdElement]) -> XsdAttributeGroup:
        """
        ...
        """
        attribute_group = self.xsd_attribute_group_class(ATTRIBUTE_GROUP_ELEMENT, self, parent)
        attribute_group[None] = self.xsd_any_attribute_class(ANY_ATTRIBUTE_ELEMENT, self, attribute_group)
        return attribute_group

    def create_empty_attribute_group(self, parent: Union[XsdComplexType, XsdElement]) -> XsdAttributeGroup:
        """
    ...
    def create_element(self, name: str, parent: Optional[XsdComponent]=None, text: Optional[str]=None, **attrib: Any) -> XsdElement:
        """
    ...
    @classmethod
    def check_schema(cls, schema: SchemaType, namespaces: Optional[NamespacesType]=None) -> None:
        """
        ...
        if cls.meta_schema is None:
            raise XMLSchemaRuntimeError(_('meta-schema unavailable for %r') % cls)
        msg = f'check_schema() class method will be removed in v3.0, use {cls.__name__}.meta_schema instead for validating XSD data.'
    ...
    def check_validator(self, validation: str='strict') -> None:
        ...
        if self.built:
            ...
            self.build()
        elif validation == 'skip' and self.validation == 'skip' and any((isinstance(comp, tuple) or comp.validation_attempted == 'partial' for comp in self.iter_globals())):
            pass
        else:
    ...
    def iter_globals(self, schema: Optional[SchemaType]=None) -> Iterator[Union[SchemaGlobalType, Tuple[Any, ...]]]:
        """
    ...
    def iter_components(self, xsd_classes: ComponentClassType=None) -> Iterator[Union[XsdComponent, SchemaType]]:
        """
    ...
    def get_converter(self, converter: Optional[ConverterType]=None, **kwargs: Any) -> XMLSchemaConverter:
        """
    ...
    def get_element(self, tag: str, path: Optional[str]=None, namespaces: Optional[NamespacesType]=None) -> Optional[XsdElement]:
        if not path:
    ...
    def include_schema(self, location: str, base_url: Optional[str]=None, build: bool=False) -> SchemaType:
        """
        ...
        for schema in self.maps.namespaces[self.target_namespace]:
            ...
            schema = type(self)(source=schema_url, namespace=self.target_namespace, validation=self.validation, global_maps=self.maps, converter=self.converter, locations=self._locations, base_url=self.base_url, allow=self.allow, defuse=self.defuse, timeout=self.timeout, build=build)
        if schema is self:
            return self
    ...
    def _parse_imports(self) -> None:
        ...
        """
        namespace_imports = NamespaceResourcesMap(map(lambda x: (x.get('namespace'), x.get('schemaLocation')), filter(lambda x: x.tag == XSD_IMPORT, self.source.root)))
        for namespace, locations in namespace_imports.items():
            if namespace is None:
                namespace = ''
                if namespace == self.target_namespace:
                    msg = _("if the 'namespace' attribute is not present on the import statement then the imported schema must have a 'targetNamespace'")
                    self.parse_error(msg)
                    continue
            elif namespace == self.target_namespace:
                msg = _("the attribute 'namespace' must be different from schema's 'targetNamespace'")
                self.parse_error(msg)
                continue
    ...
    def export(self, target: str, save_remote: bool=False, remove_residuals: bool=True) -> None:
        """
        ...
        :param remove_residuals: for default removes residual schema locations         from redundant import statements.
        """
    ...
    def validate(self, source: Union[XMLSourceType, XMLResource], path: Optional[str]=None, schema_path: Optional[str]=None, use_defaults: bool=True, namespaces: Optional[NamespacesType]=None, max_depth: Optional[int]=None, extra_validator: Optional[ExtraValidatorType]=None, allow_empty: bool=True) -> None:
        """
        ...
        """
        for error in self.iter_errors(source, path, schema_path, use_defaults, namespaces, max_depth, extra_validator, allow_empty):
            raise error

    def is_valid(self, source: Union[XMLSourceType, XMLResource], path: Optional[str]=None, schema_path: Optional[str]=None, use_defaults: bool=True, namespaces: Optional[NamespacesType]=None, max_depth: Optional[int]=None, extra_validator: Optional[ExtraValidatorType]=None, allow_empty: bool=True) -> bool:
        """
    ...
    def iter_errors(self, source: Union[XMLSourceType, XMLResource], path: Optional[str]=None, schema_path: Optional[str]=None, use_defaults: bool=True, namespaces: Optional[NamespacesType]=None, max_depth: Optional[int]=None, extra_validator: Optional[ExtraValidatorType]=None, allow_empty: bool=True) -> Iterator[XMLSchemaValidationError]:
        ...
        kwargs: Dict[Any, Any] = {'level': resource.lazy_depth or bool(path), 'source': resource, 'namespaces': namespaces, 'converter': None, 'use_defaults': use_defaults, 'id_map': Counter[str](), 'identities': identities, 'inherited': {}, 'locations': locations}
    ...
    def iter_decode(self, source: Union[XMLSourceType, XMLResource], path: Optional[str]=None, schema_path: Optional[str]=None, validation: str='lax', process_namespaces: bool=True, namespaces: Optional[NamespacesType]=None, use_defaults: bool=True, decimal_type: Optional[Type[Any]]=None, datetime_types: bool=False, binary_types: bool=False, converter: Optional[ConverterType]=None, filler: Optional[FillerType]=None, fill_missing: bool=False, keep_empty: bool=False, keep_unknown: bool=False, process_skipped: bool=False, max_depth: Optional[int]=None, depth_filler: Optional[DepthFillerType]=None, value_hook: Optional[ValueHookType]=None, element_hook: Optional[ElementHookType]=None, **kwargs: Any) -> Iterator[Union[Any, XMLSchemaValidationError]]:
        """
        ...
        :param process_namespaces: whether to use namespace information in the         decoding process, using the map provided with the argument *namespaces*         and the map extracted from the XML document.
        :param namespaces: is an optional mapping from namespace prefix to URI.
        ...
        schema = self.get_schema(namespace)
        kwargs.update(converter=converter, namespaces=namespaces, source=resource, use_defaults=use_defaults, id_map=Counter[str](), identities={}, inherited={})
        ...
        if path:
            ...
            decoder = self.raw_decoder(schema_path=resource.get_absolute_path(), validation=validation, **kwargs)
    ...
    def decode(self, source: Union[XMLSourceType, XMLResource], path: Optional[str]=None, schema_path: Optional[str]=None, validation: str='strict', *args: Any, **kwargs: Any) -> DecodeType[Any]:
        """
    ...
    def iter_encode(self, obj: Any, path: Optional[str]=None, validation: str='lax', namespaces: Optional[NamespacesType]=None, use_defaults: bool=True, converter: Optional[ConverterType]=None, unordered: bool=False, **kwargs: Any) -> Iterator[Union[ElementType, XMLSchemaValidationError]]:
        """
        ...
        :param kwargs: keyword arguments with other options for encoding and for         building the converter instance.
        ...
        if not isinstance(xsd_element, XsdElement):
            if path is not None:
                ...
                reason = _("unable to select an element for decoding data, provide a valid 'path' argument.")
            raise XMLSchemaEncodeError(self, obj, self.elements, reason, namespaces=namespaces)
        else:
            yield from xsd_element.iter_encode(obj, validation, use_defaults=use_defaults, converter=converter, unordered=unordered, **kwargs)

    def encode(self, obj: Any, path: Optional[str]=None, validation: str='strict', *args: Any, **kwargs: Any) -> EncodeType[Any]:
        """
