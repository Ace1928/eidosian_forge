...
class NetAppONTAPFirmwareUpgrade:
    ...
    def __init__(self):
        ...
        self.module = AnsibleModule(argument_spec=self.argument_spec, required_if=[('firmware_type', 'acp', ['node']), ('firmware_type', 'disk', ['node']), ('firmware_type', 'service-processor', ['node']), ('force_disruptive_update', True, ['firmware_type']), ('reboot_sp', True, ['node']), ('reboot_sp_after_download', True, ['node'])], required_together=[['server_username', 'server_password']], supports_check_mode=True)
        ...
        unsupported_rest_properties.extend((option for option in ('install_baseline_image', 'force_disruptive_update', 'fail_on_502_error') if self.parameters[option]))
        ...
        if not self.use_rest and self.parameters.get('firmware_type') == 'service-processor':
            ...
            if self.parameters.get('force_disruptive_update') and self.parameters.get('install_baseline_image') is False and (self.parameters.get('package') is None):
    ...
    def acp_firmware_update_required(self):
        ...
        try:
            ...
            self.module.fail_json(msg='Error fetching acp firmware details details: %s' % to_native(error), exception=traceback.format_exc())
    ...
    def disk_firmware_upgrade(self):
        ...
        try:
            ...
            self.module.fail_json(msg='Error updating disk firmware image : %s' % to_native(error), exception=traceback.format_exc())
        return True
    ...
    def download_sp_image(self):
        ...
        try:
            ...
            self.module.fail_json(msg='Error fetching system image package from %s: %s' % (self.parameters['package_url'], to_native(error)), exception=traceback.format_exc())
