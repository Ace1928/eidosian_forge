...
def wireless_controller_vap_group(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('wireless-controller', 'vap-group', filtered_data, vdom=vdom)
    ...
    if state == 'present' or state is True:
        return fos.set('wireless-controller', 'vap-group', data=filtered_data, vdom=vdom)
    elif state == 'absent':
        return fos.delete('wireless-controller', 'vap-group', mkey=filtered_data['name'], vdom=vdom)
    else:
