...
class NumPyProcessor:
    ...
    def preprocess_text(self, texts: Union[str, List[str]]) -> Dict[str, np.ndarray]:
        ...
        if isinstance(texts, str):
            texts = [texts]
        input_ids = np.full((len(texts), self._max_seq_len), fill_value=self._pad_token_idx, dtype=np.int64)
        attention_mask = np.zeros((len(texts), self._max_seq_len), dtype=np.int32)
    ...
    def preprocess_image(self, images: Union[Image, List[Image]]) -> np.ndarray:
        ...
        if isinstance(images, list):
            batch_images = np.empty((len(images), 3, self._image_size, self._image_size), dtype=np.float32)
