...
class TencentVectorDB(VectorStore):
    ...
    def __init__(self, embedding: Embeddings, connection_params: ConnectionParams, index_params: IndexParams=IndexParams(768), database_name: str='LangChainDatabase', collection_name: str='LangChainCollection', drop_old: Optional[bool]=False, collection_description: Optional[str]='Collection for LangChain', meta_fields: Optional[List[MetaField]]=None, t_vdb_embedding: Optional[str]='bge-base-zh'):
        ...
        if t_vdb_embedding:
            ...
            if not any(embedding_model):
                raise ValueError(f'embedding model `{t_vdb_embedding}` is invalid. choices: {[member.model_name for member in enum.EmbeddingModel]}')
        ...
        self.vdb_client = tcvectordb.VectorDBClient(url=connection_params.url, username=connection_params.username, key=connection_params.key, timeout=connection_params.timeout)
    ...
    def _create_collection(self, collection_name: str) -> None:
        ...
        params = vdb_index.HNSWParams(m=(self.index_params.params or {}).get('M', 16), efconstruction=(self.index_params.params or {}).get('efConstruction', 200))
        index = vdb_index.Index(vdb_index.FilterIndex(self.field_id, enum.FieldType.String, enum.IndexType.PRIMARY_KEY), vdb_index.VectorIndex(self.field_vector, self.index_params.dimension, index_type, metric_type, params), vdb_index.FilterIndex(self.field_text, enum.FieldType.String, enum.IndexType.FILTER))
        ...
        self.collection = self.database.create_collection(name=collection_name, shard=self.index_params.shard, replicas=self.index_params.replicas, description=self.collection_description, index=index, embedding=self.embedding_model)

    @property
    def embeddings(self) -> Embeddings:
        return self.embedding_func

    def delete(self, ids: Optional[List[str]]=None, filter_expr: Optional[str]=None, **kwargs: Any) -> Optional[bool]:
    ...
    def from_texts(cls, texts: List[str], embedding: Embeddings, metadatas: Optional[List[dict]]=None, connection_params: Optional[ConnectionParams]=None, index_params: Optional[IndexParams]=None, database_name: str='LangChainDatabase', collection_name: str='LangChainCollection', drop_old: Optional[bool]=False, collection_description: Optional[str]='Collection for LangChain', meta_fields: Optional[List[MetaField]]=None, t_vdb_embedding: Optional[str]='bge-base-zh', **kwargs: Any) -> TencentVectorDB:
        ...
        vector_db = cls(embedding=embedding, connection_params=connection_params, index_params=index_params, database_name=database_name, collection_name=collection_name, drop_old=drop_old, collection_description=collection_description, meta_fields=meta_fields, t_vdb_embedding=t_vdb_embedding)
        vector_db.add_texts(texts=texts, metadatas=metadatas)
        return vector_db

    def add_texts(self, texts: Iterable[str], metadatas: Optional[List[dict]]=None, timeout: Optional[int]=None, batch_size: int=1000, ids: Optional[List[str]]=None, **kwargs: Any) -> List[str]:
    ...
    def similarity_search_with_score(self, query: str, k: int=4, param: Optional[dict]=None, expr: Optional[str]=None, timeout: Optional[int]=None, **kwargs: Any) -> List[Tuple[Document, float]]:
        ...
        if self.embedding_func:
            ...
            return self.similarity_search_with_score_by_vector(embedding=embedding, k=k, param=param, expr=expr, timeout=timeout, **kwargs)
        return self.similarity_search_with_score_by_vector(embedding=[], k=k, param=param, expr=expr, timeout=timeout, query=query, **kwargs)
    ...
    def similarity_search_by_vector(self, embedding: List[float], k: int=4, param: Optional[dict]=None, expr: Optional[str]=None, timeout: Optional[int]=None, **kwargs: Any) -> List[Document]:
        """Perform a similarity search against the query string."""
        docs = self.similarity_search_with_score_by_vector(embedding=embedding, k=k, param=param, expr=expr, timeout=timeout, **kwargs)
        return [doc for doc, _ in docs]

    def similarity_search_with_score_by_vector(self, embedding: List[float], k: int=4, param: Optional[dict]=None, expr: Optional[str]=None, filter: Optional[str]=None, timeout: Optional[int]=None, query: Optional[str]=None, **kwargs: Any) -> List[Tuple[Document, float]]:
        """Perform a search on a query string and return results with score."""
        if filter and (not expr):
            expr = translate_filter(filter, [f.name for f in self.meta_fields or [] if f.index])
        search_args = {'filter': self.document.Filter(expr) if expr else None, 'params': self.document.HNSWSearchParams(ef=(param or {}).get('ef', 10)), 'retrieve_vector': False, 'limit': k, 'timeout': timeout}
        if query:
    ...
    def max_marginal_relevance_search_by_vector(self, embedding: list[float], k: int=4, fetch_k: int=20, lambda_mult: float=0.5, param: Optional[dict]=None, expr: Optional[str]=None, filter: Optional[str]=None, timeout: Optional[int]=None, **kwargs: Any) -> List[Document]:
        """Perform a search and return results that are reordered by MMR."""
        if filter and (not expr):
            expr = translate_filter(filter, [f.name for f in self.meta_fields or [] if f.index])
        res: List[List[Dict]] = self.collection.search(vectors=[embedding], filter=self.document.Filter(expr) if expr else None, params=self.document.HNSWSearchParams(ef=(param or {}).get('ef', 10)), retrieve_vector=True, limit=fetch_k, timeout=timeout)
        documents = []
