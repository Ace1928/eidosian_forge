...
class RedisJobboardTest(test.TestCase, base.BoardTestMixin):
    ...
    def create_board(self, persistence=None):
        ...
        config = {'namespace': ('taskflow-%s' % namespace).encode('latin-1')}
        kwargs = {'client': client, 'persistence': persistence}
    ...
    def test__make_client_sentinel(self):
        conf = {'host': '127.0.0.1', 'port': 26379, 'username': 'default', 'password': 'secret', 'namespace': 'test', 'sentinel': 'mymaster', 'sentinel_kwargs': {'username': 'default', 'password': 'senitelsecret'}}
        with mock.patch('redis.sentinel.Sentinel') as mock_sentinel:
            impl_redis.RedisJobBoard('test-board', conf)
            test_conf = {'username': 'default', 'password': 'secret'}
            mock_sentinel.assert_called_once_with([('127.0.0.1', 26379)], sentinel_kwargs={'username': 'default', 'password': 'senitelsecret'}, **test_conf)
