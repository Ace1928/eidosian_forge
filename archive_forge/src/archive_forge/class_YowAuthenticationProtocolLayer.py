...
class YowAuthenticationProtocolLayer(YowProtocolLayer):
    ...
    def __init__(self):
        handleMap = {'stream:features': (self.handleStreamFeatures, None), 'failure': (self.handleFailure, None), 'success': (self.handleSuccess, None), 'stream:error': (self.handleStreamError, None)}
    ...
    def on_connected(self, event):
        self.broadcastEvent(YowLayerEvent(self.EVENT_AUTH, passive=self.getProp(self.PROP_PASSIVE, False)))
