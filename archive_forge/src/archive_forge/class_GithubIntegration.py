...
class GithubIntegration:
    ...
    def __init__(self, integration_id: int | str | None=None, private_key: str | None=None, base_url: str=Consts.DEFAULT_BASE_URL, *, timeout: int=Consts.DEFAULT_TIMEOUT, user_agent: str=Consts.DEFAULT_USER_AGENT, per_page: int=Consts.DEFAULT_PER_PAGE, verify: bool | str=True, retry: int | Retry | None=None, pool_size: int | None=None, seconds_between_requests: float | None=Consts.DEFAULT_SECONDS_BETWEEN_REQUESTS, seconds_between_writes: float | None=Consts.DEFAULT_SECONDS_BETWEEN_WRITES, jwt_expiry: int=Consts.DEFAULT_JWT_EXPIRY, jwt_issued_at: int=Consts.DEFAULT_JWT_ISSUED_AT, jwt_algorithm: str=Consts.DEFAULT_JWT_ALGORITHM, auth: AppAuth | None=None) -> None:
        ...
        if integration_id is not None or private_key is not None or jwt_expiry != Consts.DEFAULT_JWT_EXPIRY or (jwt_issued_at != Consts.DEFAULT_JWT_ISSUED_AT) or (jwt_algorithm != Consts.DEFAULT_JWT_ALGORITHM):
            warnings.warn('Arguments integration_id, private_key, jwt_expiry, jwt_issued_at and jwt_algorithm are deprecated, please use auth=github.Auth.AppAuth(...) instead', category=DeprecationWarning)
            auth = AppAuth(integration_id, private_key, jwt_expiry=jwt_expiry, jwt_issued_at=jwt_issued_at, jwt_algorithm=jwt_algorithm)
        assert isinstance(auth, AppAuth), f'GithubIntegration requires github.Auth.AppAuth authentication, not {type(auth)}'
        self.auth = auth
        self.__requester = Requester(auth=auth, base_url=self.base_url, timeout=timeout, user_agent=user_agent, per_page=per_page, verify=verify, retry=retry, pool_size=pool_size, seconds_between_requests=seconds_between_requests, seconds_between_writes=seconds_between_writes)
    ...
    def _get_headers(self) -> dict[str, str]:
        ...
        """
        return {'Accept': Consts.mediaTypeIntegrationPreview}
    ...
    @deprecated.deprecated('Use github.Github(auth=github.Auth.AppAuth), github.Auth.AppAuth.token or github.Auth.AppAuth.create_jwt(expiration) instead')
    ...
    def get_access_token(self, installation_id: int, permissions: dict[str, str] | None=None) -> InstallationAuthorization:
        ...
        headers, response = self.__requester.requestJsonAndCheck('POST', f'/app/installations/{installation_id}/access_tokens', headers=self._get_headers(), input=body)
        return InstallationAuthorization(requester=self.__requester, headers=headers, attributes=response, completed=True)
