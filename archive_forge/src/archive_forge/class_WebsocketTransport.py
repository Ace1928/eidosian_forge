...
class WebsocketTransport(base.Transport):
    ...
    *NOTE:* Zaqar's websocket interface does not yet appear to work
    ...
    def __init__(self, options):
        ...
        self._project_id = option.get('os_project_id', option.get('project_id'))
    ...
    def send(self, request):
        ...
        headers.update({'Client-ID': self._websocket_client_id, 'X-Project-ID': self._project_id})
        ...
        ret = self.recv()
        resp = response.Response(request, json.dumps(ret.get('body', '')), headers=ret['headers'], status_code=int(ret['headers']['status']))
