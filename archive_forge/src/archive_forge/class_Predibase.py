...
class Predibase(LLM):
    ...
    default_options_for_generation: dict = Field({'max_new_tokens': 256, 'temperature': 0.1}, const=True)
    ...
    def _call(self, prompt: str, stop: Optional[List[str]]=None, run_manager: Optional[CallbackManagerForLLMRun]=None, **kwargs: Any) -> str:
        try:
            ...
            raise ImportError('Could not import Predibase Python package. Please install it with `pip install predibase`.') from e
        except ValueError as e:
            raise ValueError('Your API key is not correct. Please try again') from e
        options: Dict[str, Union[str, float]] = self.model_kwargs or self.default_options_for_generation
        ...
        if self.adapter_id:
            ...
            try:
                adapter_model = pc.get_model(name=self.adapter_id, version=self.adapter_version, model_id=None)
            except ServerResponseError:
                adapter_model = pc.LLM(uri=f'hf://{self.adapter_id}')
            result = base_llm_deployment.with_adapter(model=adapter_model).generate(prompt=prompt, options=options)
        else:
