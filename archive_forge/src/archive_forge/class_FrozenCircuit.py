...
class FrozenCircuit(AbstractCircuit, protocols.SerializableByKey):
    ...
    def _is_parameterized_(self) -> bool:
        return super()._is_parameterized_() or any((protocols.is_parameterized(tag) for tag in self.tags))
    ...
    def _resolve_parameters_(self, resolver: 'cirq.ParamResolver', recursive: bool) -> 'cirq.FrozenCircuit':
        resolved_circuit = super()._resolve_parameters_(resolver, recursive)
        resolved_tags = [protocols.resolve_parameters(tag, resolver, recursive) for tag in self.tags]
    ...
    def zip(*circuits: 'cirq.AbstractCircuit', align: Union['cirq.Alignment', str]=Alignment.LEFT) -> 'cirq.FrozenCircuit':
