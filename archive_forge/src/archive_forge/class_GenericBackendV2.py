...
class GenericBackendV2(BackendV2):
    ...
    def __init__(self, num_qubits: int, basis_gates: list[str] | None=None, *, coupling_map: list[list[int]] | CouplingMap | None=None, control_flow: bool=False, calibrate_instructions: bool | InstructionScheduleMap | None=None, dtm: float | None=None, seed: int | None=None):
        """
        Args:
        ...
        """
        super().__init__(provider=None, name=f'generic_backend_{num_qubits}q', description=f'This is a device with {num_qubits} qubits and generic settings.', backend_version='')
        self._sim = None
        ...
        if coupling_map is None:
            ...
            if num_qubits != self._coupling_map.size():
                raise QiskitError(f'The number of qubits (got {num_qubits}) must match the size of the provided coupling map (got {self._coupling_map.size()}).')
        self._basis_gates = basis_gates if basis_gates is not None else ['cx', 'id', 'rz', 'sx', 'x']
