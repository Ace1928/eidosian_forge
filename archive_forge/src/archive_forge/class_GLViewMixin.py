...
class GLViewMixin:
    ...
    def __init__(self, *args, rotationMethod='euler', **kwargs):
        ...
        **Arguments:**
        ...
        self.opts = {'center': Vector(0, 0, 0), 'rotation': QtGui.QQuaternion(1, 0, 0, 0), 'distance': 10.0, 'fov': 60, 'elevation': 30, 'azimuth': 45, 'viewport': None, 'rotationMethod': rotationMethod}
        self.reset()
        self.items = []
    ...
    def initializeGL(self):
        ...
        if ctx.isOpenGLES() or fmt.version() < (2, 0):
            ...
            raise RuntimeError('pyqtgraph.opengl: Requires >= OpenGL 2.0 (not ES); Found %s' % verString)
        for item in self.items:
    ...
    def cameraPosition(self):
        ...
        if self.opts['rotationMethod'] == 'quaternion':
            ...
            pos = Vector(center.x() + dist * cos(elev) * cos(azim), center.y() + dist * cos(elev) * sin(azim), center.z() + dist * sin(elev))
        return pos
    ...
    def orbit(self, azim, elev):
        ...
        if self.opts['rotationMethod'] == 'quaternion':
            q = QtGui.QQuaternion.fromEulerAngles(elev, -azim, 0)
            q *= self.opts['rotation']
            self.opts['rotation'] = q
        else:
            self.opts['azimuth'] += azim
            self.opts['elevation'] = fn.clip_scalar(self.opts['elevation'] + elev, -90.0, 90.0)
        self.update()
    ...
    def pan(self, dx, dy, dz, relative='global'):
        ...
        *relative*      String that determines the direction of dx,dy,dz. 
        ...
        if relative == 'global':
            ...
            dist = cVec.length()
            xDist = dist * 2.0 * tan(0.5 * radians(self.opts['fov']))
