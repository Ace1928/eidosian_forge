...
class NIBittensorLLM(LLM):
    ...
    def _call(self, prompt: str, stop: Optional[List[str]]=None, run_manager: Optional[CallbackManagerForLLMRun]=None, **kwargs: Any) -> str:
        ...
        headers = {'Content-Type': 'application/json', 'Authorization': f'Bearer {api_key}', 'Endpoint-Version': '2023-05-19'}
        ...
        if isinstance(uids, list) and uids and (not top_n):
            for uid in uids:
                try:
                    payload = json.dumps({'uids': [uid], 'messages': [{'role': 'system', 'content': system_prompt}, {'role': 'user', 'content': prompt}]})
                    ...
                    continue
        try:
            payload = json.dumps({'top_n': top_n, 'messages': [{'role': 'system', 'content': system_prompt}, {'role': 'user', 'content': prompt}]})
            conn.request('POST', '/chat', payload, headers)
    ...
    def _identifying_params(self) -> Mapping[str, Any]:
        ...
        return {'system_prompt': self.system_prompt, 'top_responses': self.top_responses}