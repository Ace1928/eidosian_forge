...
class TimeDistributed(Wrapper):
    ...
    def __init__(self, layer, **kwargs):
        if not isinstance(layer, Layer):
            raise ValueError(f'Please initialize `TimeDistributed` layer with a `tf.keras.layers.Layer` instance. Received: {layer}')
        super().__init__(layer, **kwargs)
        self.supports_masking = True
        self._always_use_reshape = layer_utils.is_builtin_layer(layer) and (not getattr(layer, 'stateful', False))
    ...
    def build(self, input_shape):
        ...
        input_dims = tf.nest.flatten(tf.nest.map_structure(lambda x: x.ndims, input_shape))
        ...
        self.input_spec = tf.nest.map_structure(lambda x: InputSpec(shape=[None, None] + x.as_list()[2:]), input_shape)
    ...
    def compute_output_shape(self, input_shape):
        ...
        child_output_shape = tf_utils.convert_shapes(child_output_shape, to_tuples=False)
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        input_shape = tf.nest.map_structure(lambda x: tf.TensorShape(backend.int_shape(x)), inputs)
        ...
        if batch_size and (not self._always_use_reshape):
            ...
            def step(x, _):
                ...
                return (output, [])
            _, outputs, _ = backend.rnn(step, inputs, initial_states=[], input_length=row_lengths[0] if is_ragged_input else input_length, mask=mask, unroll=False)
            y = tf.nest.map_structure(lambda output: backend.maybe_convert_to_ragged(is_ragged_input, output, row_lengths), outputs)
        else:
            is_ragged_input = tf.nest.map_structure(lambda x: isinstance(x, tf.RaggedTensor), inputs)
            ...
            if all(is_ragged_input):
                ...
                input_row_lenghts = tf.nest.map_structure(lambda x: x.nested_row_lengths()[0], inputs)
                y = self.layer(input_values, **kwargs)
                y = tf.nest.map_structure(tf.RaggedTensor.from_row_lengths, y, input_row_lenghts)
            elif any(is_ragged_input):
                raise ValueError(f'All inputs has to be either ragged or not, but not mixed. Received: {inputs}')
            else:
                input_length = tf_utils.convert_shapes(input_shape)
                input_length = tf.nest.flatten(input_length)[1]
                if not input_length:
                ...
                inner_input_shape = tf.nest.map_structure(lambda x: self._get_shape_tuple((-1,), x, 2), inputs)
                inputs = tf.__internal__.nest.map_structure_up_to(inputs, tf.reshape, inputs, inner_input_shape)
                ...
                output_shape = tf.nest.map_structure(lambda tensor: self._get_shape_tuple((reshape_batch_size, input_length), tensor, 1), y)
    ...
    def compute_mask(self, inputs, mask=None):
        ...
        inner_input_shape = tf.nest.map_structure(lambda tensor: self._get_shape_tuple((-1,), tensor, 2), inputs)
        inner_inputs = tf.__internal__.nest.map_structure_up_to(inputs, tf.reshape, inputs, inner_input_shape)
        ...
        if output_mask is None:
            ...
            reshape_batch_size = batch_size if batch_size else -1
            output_mask_shape = self._get_shape_tuple((reshape_batch_size, input_length), output_mask, 1)
