...
class ZunClientPlugin(client_plugin.ClientPlugin):
    ...
    supported_versions = [V1_12, V1_18, V1_36] = ['1.12', '1.18', '1.36']

    def _create(self, version=None):
        if not version:
            version = self.default_version
        interface = self._get_client_option(CLIENT_NAME, 'endpoint_type')
        args = {'interface': interface, 'service_type': self.CONTAINER, 'session': self.context.keystone_session, 'region_name': self._get_region_name()}
        client = zun_client.Client(version, **args)
        return client

    def update_container(self, container_id, **prop_diff):
        if prop_diff:
            self.client(version=self.V1_18).containers.update(container_id, **prop_diff)

    def network_detach(self, container_id, port_id):
        with self.ignore_not_found:
            self.client(version=self.V1_18).containers.network_detach(container_id, port=port_id)
            return True

    def network_attach(self, container_id, port_id=None, net_id=None, fip=None, security_groups=None):
        with self.ignore_not_found:
            kwargs = {}
            if port_id:
                kwargs['port'] = port_id
            ...
            if fip:
                kwargs['fixed_ip'] = fip
            self.client(version=self.V1_18).containers.network_attach(container_id, **kwargs)
            return True
