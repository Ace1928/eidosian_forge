...
class AbstractAudioPlayer(metaclass=ABCMeta):
    ...
    def __init__(self, source, player):
        ...
        self._buffered_data_ideal_size = max(32768, afmt.timestamp_to_bytes_aligned(self.audio_buffer_length))
    ...
    def _get_and_compensate_audio_data(self, requested_size, audio_position=None):
        ...
        assert _debug(f'Audio desync, desync_bytes={desync_bytes!r}, extreme_desync={extreme_desync!r}')
        assert desync_bytes % afmt.bytes_per_frame == 0
        if desync_bytes > 0:
            compensated_bytes = min(requested_size - afmt.align_ceil(1024), desync_bytes, self.desync_correction_bytes_minor)
            ...
            compensated_bytes = -desync_bytes if extreme_desync else min(-desync_bytes, self.desync_correction_bytes_minor)
            audio_data = self.source.get_audio_data(requested_size + compensated_bytes)
            if audio_data is not None:
                if audio_data.length <= compensated_bytes:
                    ...
                    audio_data = AudioData(ctypes.string_at(audio_data.pointer + compensated_bytes, audio_data.length - compensated_bytes), audio_data.length - compensated_bytes, audio_data.events)
                    compensated_bytes *= -1
