...
class SurrealDBLoader(BaseLoader):
    ...
    def __init__(self, filter_criteria: Optional[Dict]=None, **kwargs: Any) -> None:
        try:
            ...
            raise ImportError('Cannot import from surrealdb.\n                please install with `pip install surrealdb`.') from e
    ...
    async def aload(self) -> List[Document]:
        ...
        return [Document(page_content=json.dumps(result), metadata={'id': result['id'], **result['metadata'], **metadata}) for result in results[0]['result']]