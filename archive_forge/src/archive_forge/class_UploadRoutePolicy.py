  ...
  *{command}* uploads a route policy into a Compute Engine router.
  """
    ...
    def Args(cls, parser):
        ...
        parser.add_argument('--policy-name', help='Name of the route policy to add/replace.')
        parser.add_argument('--file-name', required=True, help='Local path to the file defining the policy')
        parser.add_argument('--file-format', choices=['json', 'yaml'], help='Format of the file passed to --file-name')
    ...
    """
        ...
        router_ref = UploadRoutePolicy.ROUTER_ARG.ResolveAsResource(args, holder.resources, scope_lister=compute_flags.GetDefaultScopeLister(client))
        route_policy = self.ParseRoutePolicyFromFile(args.file_name, args.file_format, client.messages)
    ...
    def ParseRoutePolicyFromFile(self, input_file, file_format, messages):
        ...
        route_policy = messages_util.DictToMessageWithErrorCheck(resource, messages.RoutePolicy)
