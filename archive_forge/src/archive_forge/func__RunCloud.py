...
def _RunCloud(self, args):
    ...
    with _DeployTempFile(kubernetes_config) as kubernetes_file:
        skaffold_config = six.ensure_text(cloud_file_generator.SkaffoldConfig(kubernetes_file.name))
        skaffold_event_port = args.skaffold_events_port or portpicker.pick_unused_port()
        with _SkaffoldTempFile(skaffold_config) as skaffold_file, self._CloudSkaffoldProcess(skaffold_file, skaffold_event_port) as running_process, skaffold.PrintUrlThreadContext(settings.service_name, skaffold_event_port):
