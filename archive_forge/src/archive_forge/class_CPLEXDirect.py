...
class CPLEXDirect(DirectSolver):
    ...
    def _init(self):
        try:
            ...
            self._version = tuple((int(k) for k in self._cplex.Cplex().get_version().split('.')))
    ...
    def _get_expr_from_pyomo_repn(self, repn, max_degree=2):
        ...
        return (_CplexExpr(variables=[self._pyomo_var_to_ndx_map[var] for var in repn.linear_vars], coefficients=repn.linear_coefs, offset=repn.constant, q_variables1=q_variables1, q_variables2=q_variables2, q_coefficients=q_coefficients), referenced_vars)
    ...
    def _add_var(self, var, var_data=None):
        ...
        cplex_var_data = _VariableData(self._solver_model) if var_data is None else var_data
            ...
            if n_ref != 0:
                if var.fixed:
                    if not self._output_fixed_variable_bounds:
                        raise ValueError("Encountered a fixed variable (%s) inside an active objective or constraint expression on model %s, which is usually indicative of a preprocessing error. Use the IO-option 'output_fixed_variable_bounds=True' to suppress this error and fix the variable by overwriting its bounds in the CPLEX instance." % (var.name, self._pyomo_model.name))

    def _add_block(self, block):
        var_data = _VariableData(self._solver_model)
        for var in block.component_data_objects(ctype=Var, descend_into=True, active=True, sort=True):
    ...
    def _add_constraint(self, con, lin_con_data=None):
        ...
        if con._linear_canonical_form:
            cplex_expr, referenced_vars = self._get_expr_from_pyomo_repn(con.canonical_form(), self._max_constraint_degree)
        else:
        ...
        if len(cplex_expr.q_coefficients) == 0:
            cplex_lin_con_data = _LinearConstraintData(self._solver_model) if lin_con_data is None else lin_con_data
            cplex_lin_con_data.add(cplex_expr, sense, rhs, range_, conname)
            if lin_con_data is None:
                cplex_lin_con_data.store_in_cplex()
        else:
            if sense == 'R':
                raise ValueError('The CPLEXDirect interface does not support quadratic range constraints: {0}'.format(con))
            self._solver_model.quadratic_constraints.add(lin_expr=[cplex_expr.variables, cplex_expr.coefficients], quad_expr=[cplex_expr.q_variables1, cplex_expr.q_variables2, cplex_expr.q_coefficients], sense=sense, rhs=rhs, name=conname)
    ...
    def _set_objective(self, obj):
        ...
        if quadratic_objective_already_exists or contains_quadratic_terms:
            ...
            if contains_quadratic_terms:
                self._solver_model.objective.set_quadratic_coefficients(list(zip(cplex_expr.q_variables1, cplex_expr.q_variables2, cplex_expr.q_coefficients)))
        self._objective = obj
    ...
    def _postsolve(self):
        ...
        if cpxprob.get_problem_type() in [cpxprob.problem_type.MILP, cpxprob.problem_type.MIQP, cpxprob.problem_type.MIQCP]:
        ...
        if cpxprob.objective.get_sense() == cpxprob.objective.sense.minimize:
            ...
            raise RuntimeError('Unrecognized cplex objective sense: {0}'.format(cpxprob.objective.get_sense()))
        ...
        self.results.problem.number_of_constraints = cpxprob.linear_constraints.get_num() + cpxprob.quadratic_constraints.get_num() + cpxprob.SOS.get_num()
        ...
        self.results.problem.number_of_continuous_variables = cpxprob.variables.get_num() - cpxprob.variables.get_num_binary() - cpxprob.variables.get_num_integer()
