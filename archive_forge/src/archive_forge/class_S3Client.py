...
class S3Client(Client):
    ...
    def __init__(self, aws_access_key_id: Optional[str]=None, aws_secret_access_key: Optional[str]=None, aws_session_token: Optional[str]=None, no_sign_request: Optional[bool]=False, botocore_session: Optional['botocore.session.Session']=None, profile_name: Optional[str]=None, boto3_session: Optional['Session']=None, file_cache_mode: Optional[Union[str, FileCacheMode]]=None, local_cache_dir: Optional[Union[str, os.PathLike]]=None, endpoint_url: Optional[str]=None, boto3_transfer_config: Optional['TransferConfig']=None, content_type_method: Optional[Callable]=mimetypes.guess_type, extra_args: Optional[dict]=None):
        ...
        if boto3_session is not None:
            ...
            self.sess = Session(aws_access_key_id=aws_access_key_id, aws_secret_access_key=aws_secret_access_key, aws_session_token=aws_session_token, botocore_session=botocore_session, profile_name=profile_name)
        if no_sign_request:
            self.s3 = self.sess.resource('s3', endpoint_url=endpoint_url, config=Config(signature_version=botocore.session.UNSIGNED))
            self.client = self.sess.client('s3', endpoint_url=endpoint_url, config=Config(signature_version=botocore.session.UNSIGNED))
        else:
        ...
        self.boto3_dl_extra_args = {k: v for k, v in extra_args.items() if k in S3Transfer.ALLOWED_DOWNLOAD_ARGS}
        self.boto3_ul_extra_args = {k: v for k, v in extra_args.items() if k in S3Transfer.ALLOWED_UPLOAD_ARGS}
        self.boto3_list_extra_args = {k: self._extra_args[k] for k in ['RequestPayer', 'ExpectedBucketOwner'] if k in self._extra_args}
        super().__init__(local_cache_dir=local_cache_dir, content_type_method=content_type_method, file_cache_mode=file_cache_mode)

    def _get_metadata(self, cloud_path: S3Path) -> Dict[str, Any]:
        data = self.s3.ObjectSummary(cloud_path.bucket, cloud_path.key).get(**self.boto3_dl_extra_args)
        return {'last_modified': data['LastModified'], 'size': data['ContentLength'], 'etag': data['ETag'], 'content_type': data.get('ContentType', None), 'extra': data['Metadata']}

    def _download_file(self, cloud_path: S3Path, local_path: Union[str, os.PathLike]) -> Path:
        local_path = Path(local_path)
        obj = self.s3.Object(cloud_path.bucket, cloud_path.key)
        obj.download_file(str(local_path), Config=self.boto3_transfer_config, ExtraArgs=self.boto3_dl_extra_args)
        return local_path
    ...
    def _exists(self, cloud_path: S3Path) -> bool:
        if not cloud_path.key:
            extra = {k: self._extra_args[k] for k in ['ExpectedBucketOwner'] if k in self._extra_args}
            try:
    ...
    def _list_dir(self, cloud_path: S3Path, recursive=False) -> Iterable[Tuple[S3Path, bool]]:
        if not cloud_path.bucket:
            ...
            yield from ((self.CloudPath(f's3://{b['Name']}'), True) for b in self.client.list_buckets().get('Buckets', []))
            return
        ...
        for result in paginator.paginate(Bucket=cloud_path.bucket, Prefix=prefix, Delimiter='' if recursive else '/', **self.boto3_list_extra_args):
            for result_prefix in result.get('CommonPrefixes', []):
                canonical = result_prefix.get('Prefix').rstrip('/')
                if canonical not in yielded_dirs:
                    yield (self.CloudPath(f's3://{cloud_path.bucket}/{canonical}'), True)
            ...
            for result_key in result.get('Contents', []):
                ...
                for parent in PurePosixPath(o_relative_path).parents:
                    ...
                    if parent_canonical not in yielded_dirs and str(parent) != '.':
                        yield (self.CloudPath(f's3://{cloud_path.bucket}/{parent_canonical}'), True)
                ...
                if result_key.get('Key').endswith('/') and result_key.get('Size') == 0:
                    ...
                    yield (self.CloudPath(f's3://{cloud_path.bucket}/{result_key.get('Key')}'), False)

    def _move_file(self, src: S3Path, dst: S3Path, remove_src: bool=True) -> S3Path:
        if src == dst:
            o = self.s3.Object(src.bucket, src.key)
            o.copy_from(CopySource={'Bucket': src.bucket, 'Key': src.key}, Metadata=self._get_metadata(src).get('extra', {}), MetadataDirective='REPLACE', **self.boto3_ul_extra_args)
        else:
            target = self.s3.Object(dst.bucket, dst.key)
            target.copy({'Bucket': src.bucket, 'Key': src.key}, ExtraArgs=self.boto3_dl_extra_args, Config=self.boto3_transfer_config)
            if remove_src:
                self._remove(src)
        return dst
    ...
    def _remove(self, cloud_path: S3Path, missing_ok: bool=True) -> None:
        ...
        if file_or_dir == 'file':
            resp = self.s3.Object(cloud_path.bucket, cloud_path.key).delete(**self.boto3_list_extra_args)
            if resp.get('ResponseMetadata').get('HTTPStatusCode') not in (204, 200):
                raise CloudPathException(f'Delete operation failed for {cloud_path} with response: {resp}')
        elif file_or_dir == 'dir':
            bucket = self.s3.Bucket(cloud_path.bucket)
            prefix = cloud_path.key
            if prefix and (not prefix.endswith('/')):
                prefix += '/'
            resp = bucket.objects.filter(Prefix=prefix, **self.boto3_list_extra_args).delete(**self.boto3_list_extra_args)
            if resp[0].get('ResponseMetadata').get('HTTPStatusCode') not in (204, 200):
                raise CloudPathException(f'Delete operation failed for {cloud_path} with response: {resp}')
        elif not missing_ok:
            raise FileNotFoundError(f'Cannot delete file that does not exist: {cloud_path} (consider passing missing_ok=True)')
