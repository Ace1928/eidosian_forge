...
def switch_controller_location(data, fos, check_mode=False):
    ...
    filtered_data = underscore_to_hyphen(filter_switch_controller_location_data(switch_controller_location_data))
    if check_mode:
    ...
    if state == 'present' or state is True:
        ...
        return fos.delete('switch-controller', 'location', mkey=filtered_data['name'], vdom=vdom)
    else:
