...
import six
def SSHIntoPreppedNodes(prepped_nodes, args, total_ssh_batch_size):
  ...
  """
    ...
    log.status.Print('Using ssh batch size of {}. Attempting to SSH into {} nodes with a total of {} workers.'.format(total_ssh_batch_size, len(prepped_nodes), num_ips))
    for prepped_node in prepped_nodes:
        for worker, ips in prepped_node.worker_ips.items():
            ...
            if args.output_directory:
                output_file_writer = FileWriter('{}/{}.log'.format(output_directory_path, six.text_type(worker)))
            if len(prepped_node.worker_ips) > 1 or len(prepped_nodes) > 1:
                ssh_threads.append(threading.Thread(target=AttemptRunWithRetries, args=('SSH', worker, exit_statuses, cmd, prepped_node.ssh_helper.env, output_file_writer, True, SSHRunCmd)))
                ssh_threads[-1].start()
                current_batch_size += 1
                if prepped_node.enable_batching and current_batch_size == total_ssh_batch_size:
                    WaitForBatchCompletion(ssh_threads, exit_statuses)
                    current_batch_size = 0
                    ssh_threads = []
            else:
                AttemptRunWithRetries('SSH', worker, exit_statuses, cmd, prepped_node.ssh_helper.env, output_file_writer, False, SSHRunCmd)
