...
def _legacy_convert_issue(self, attributes: dict[str, Any]) -> dict[str, Any]:
    convertedAttributes = {'number': attributes['number'], 'url': f'/repos{urllib.parse.urlparse(attributes['html_url']).path}', 'user': {'login': attributes['user'], 'url': f'/users/{attributes['user']}'}}
