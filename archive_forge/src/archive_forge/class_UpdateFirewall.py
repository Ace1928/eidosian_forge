...
class UpdateFirewall(base.UpdateCommand):
    ...
    @classmethod
    def Args(cls, parser):
        messages = apis.GetMessagesModule('compute', compute_api.COMPUTE_GA_API_VERSION)
    ...
    def Run(self, args):
        ...
        resource_reference = self.FIREWALL_RULE_ARG.ResolveAsResource(args, holder.resources)
        ...
        with client.apitools_client.IncludeFields(cleared_fields):
            resource_list = client.MakeRequests([self._GetSetRequest(client, resource_reference, new_object)])
        return resource_list

    def _GetGetRequest(self, client, resource_reference):
        """Returns the request for the existing Firewall resource."""
        return (client.apitools_client.firewalls, 'Get', client.messages.ComputeFirewallsGetRequest(firewall=resource_reference.Name(), project=resource_reference.project))

    def _GetSetRequest(self, client, resource_reference, replacement):
        request = client.messages.ComputeFirewallsPatchRequest(firewall=replacement.name, firewallResource=replacement, project=resource_reference.project)
    ...
    def Modify(self, client, args, existing, cleared_fields):
        ...
        if args.IsSpecified('enable_logging'):
            ...
            log_config = encoding.CopyProtoMessage(existing.logConfig) if existing.logConfig is not None else None
            self._AddLoggingMetadata(client.messages, args, log_config)
        new_firewall = client.messages.Firewall(name=existing.name, direction=direction, priority=priority, allowed=allowed, denied=denied, description=description, network=existing.network, sourceRanges=source_ranges, sourceTags=source_tags, destinationRanges=destination_ranges, targetTags=target_tags, sourceServiceAccounts=source_service_accounts, targetServiceAccounts=target_service_accounts, logConfig=log_config)
