...
class AnsibleCloudStack:
    ...
    def __init__(self, module):
        ...
        self.common_returns = {'id': 'id', 'name': 'name', 'created': 'created', 'zonename': 'zone', 'state': 'state', 'project': 'project', 'account': 'account', 'domain': 'domain', 'displaytext': 'display_text', 'displayname': 'display_name', 'description': 'description', 'tags': 'tags'}
        self.returns = {}
        self.returns_to_int = {}
    ...
    def get_network(self, key=None):
        ...
        if not network:
            ...
            return None
        args = {'account': self.get_account(key='name'), 'domainid': self.get_domain(key='id'), 'projectid': self.get_project(key='id'), 'zoneid': self.get_zone(key='id'), 'vpcid': self.get_vpc(key='id')}
    ...
    def get_pod(self, key=None):
        ...
        if not pod_name:
            return None
        args = {'name': pod_name, 'zoneid': self.get_zone(key='id')}
    ...
    def get_vm(self, key=None, filter_zone=True):
        ...
        args = {'account': self.get_account(key='name'), 'domainid': self.get_domain(key='id'), 'projectid': self.get_project(key='id'), 'zoneid': self.get_zone(key='id') if filter_zone else None, 'fetch_list': True}
