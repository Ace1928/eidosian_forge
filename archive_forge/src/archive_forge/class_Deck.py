...
class Deck(JSONMixin):
    ...
    def __init__(self, layers=None, views=[View(type='MapView', controller=True)], map_style=DARK, api_keys=None, initial_view_state=ViewState(latitude=0, longitude=0, zoom=1), width='100%', height=500, tooltip=True, description=None, effects=None, map_provider=BaseMapProvider.CARTO.value, parameters=None):
    ...
    def to_html(self, filename=None, open_browser=False, notebook_display=None, iframe_width='100%', iframe_height=500, as_string=False, offline=False, **kwargs):
        ...
        f = deck_to_html(deck_json, mapbox_key=self.mapbox_key, google_maps_key=self.google_maps_key, filename=filename, open_browser=open_browser, notebook_display=notebook_display, iframe_height=iframe_height, iframe_width=iframe_width, tooltip=self._tooltip, custom_libraries=pydeck_settings.custom_libraries, configuration=pydeck_settings.configuration, as_string=as_string, offline=offline, **kwargs)
        return f
