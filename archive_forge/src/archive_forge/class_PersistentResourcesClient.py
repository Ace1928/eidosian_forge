...
class PersistentResourcesClient(object):
    ...
    def __init__(self, version=constants.GA_VERSION):
        client = apis.GetClientInstance(constants.AI_PLATFORM_API_NAME, constants.AI_PLATFORM_API_VERSION[version])
    ...
    def Create(self, parent, resource_pools, persistent_resource_id, display_name=None, kms_key_name=None, labels=None, network=None, enable_custom_service_account=False, service_account=None):
    ...
    """
        persistent_resource = self.PersistentResourceMessage()(displayName=display_name, resourcePools=resource_pools)
        if kms_key_name is not None:
            persistent_resource.encryptionSpec = self.GetMessage('EncryptionSpec')(kmsKeyName=kms_key_name)
        if labels:
            persistent_resource.labels = labels
        if network:
            persistent_resource.network = network
        if enable_custom_service_account:
            persistent_resource.resourceRuntimeSpec = self.GetMessage('ResourceRuntimeSpec')(serviceAccountSpec=self.GetMessage('ServiceAccountSpec')(enableCustomServiceAccount=True, serviceAccount=service_account))
        if self._version == constants.GA_VERSION:
            return self._service.Create(self._messages.AiplatformProjectsLocationsPersistentResourcesCreateRequest(parent=parent, googleCloudAiplatformV1PersistentResource=persistent_resource, persistentResourceId=persistent_resource_id))
        return self._service.Create(self._messages.AiplatformProjectsLocationsPersistentResourcesCreateRequest(parent=parent, googleCloudAiplatformV1beta1PersistentResource=persistent_resource, persistentResourceId=persistent_resource_id))
    ...
    """
        return list_pager.YieldFromList(self._service, self._messages.AiplatformProjectsLocationsPersistentResourcesListRequest(parent=region), field='persistentResources', batch_size_attribute='pageSize', limit=limit)

    def Get(self, name):
        request = self._messages.AiplatformProjectsLocationsPersistentResourcesGetRequest(name=name)
        return self._service.Get(request)

    def Delete(self, name):
        request = self._messages.AiplatformProjectsLocationsPersistentResourcesDeleteRequest(name=name)
        return self._service.Delete(request)

    def Reboot(self, name):
        request = self._messages.AiplatformProjectsLocationsPersistentResourcesRebootRequest(name=name)
