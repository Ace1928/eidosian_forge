...
class AdaptiveActivationNetwork(nn.Module):
    ...
    def forward(self, x: torch.Tensor, batch_id: Optional[int] = None) -> torch.Tensor:
        ...
        try:
            ...
            logger.error(
        f"An error occurred during the forward pass at layer {layer_index}: {e}",
        exc_info=True,
        )
            raise RuntimeError(f"Forward pass failed due to: {e}") from e        logger.debug(f"Final output after dynamic activation: {x}")
        return x
    ...
    def dynamic_activation(
        ...
        selected_activations: List[Callable[[torch.Tensor], torch.Tensor]] = [
            self.activations[self.activation_keys[int(idx.long())]]
