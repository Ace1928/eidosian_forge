...
def infer_storage_type_entry(num_tensor, tensor_stypes, _):
    ...
    try:
        ...
        assert len(ostype) == n_out, 'InferStorageType Error: expecting %d entries in returned output stypes, got %d.' % (n_out, len(ostype))
        assert len(istype) == n_in, 'InferStorageType Error: expecting %d entries in returned input stypes, got %d.' % (n_in, len(istype))
        assert len(astype) == n_aux, 'InferStorageType Error: expecting %d entries in returned aux state stypes, got %d.' % (n_aux, len(astype))
