...
def compare_opts(opt_path_1: str, opt_path_2: str, load_raw: bool=False) -> str:
    ...
    keys_with_conflicting_values = sorted([k for k, v in opt1.items() if k in opt2.keys() and v != opt2[k]])
    for key in keys_with_conflicting_values:
        if isinstance(opt1[key], dict) and isinstance(opt2[key], dict):
            ...
            all_inner_keys = sorted(list(set(opt1[key].keys()).union(set(opt2[key].keys()))))
            for inner_key in all_inner_keys:
                if inner_key not in opt1[key] or inner_key not in opt2[key] or opt1[key][inner_key] != opt2[key][inner_key]:
                    outputs.append(f'\t{inner_key}:')
                    outputs.append(f'\t\tIn opt 1: {opt1[key].get(inner_key, '<MISSING>')}')
                    outputs.append(f'\t\tIn opt 2: {opt2[key].get(inner_key, '<MISSING>')}')
        else:
