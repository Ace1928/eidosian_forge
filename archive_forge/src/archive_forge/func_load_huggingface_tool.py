...
def load_huggingface_tool(task_or_repo_id: str, model_repo_id: Optional[str]=None, token: Optional[str]=None, remote: bool=False, **kwargs: Any) -> BaseTool:
    ...
    try:
        ...
        raise ImportError('HuggingFace tools require the libraries `transformers>=4.29.0` and `huggingface_hub>=0.14.1` to be installed. Please install it with `pip install --upgrade transformers huggingface_hub`.')
    hf_tool = load_tool(task_or_repo_id, model_repo_id=model_repo_id, token=token, remote=remote, **kwargs)
