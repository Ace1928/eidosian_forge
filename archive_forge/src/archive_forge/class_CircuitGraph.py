...
class CircuitGraph:
    ...
    def get_depth(self):
        ...
        if self._depth is None and self.operations:
            if self._operation_graph is None:
                self._operation_graph = self._graph.subgraph(list((self._indices[id(node)] for node in self.operations)))
                self._extend_graph(self._operation_graph)
                self._depth = rx.dag_longest_path_length(self._operation_graph, weight_fn=lambda _, __, w: self._weight_func(w)) + 1
        return self._depth
    ...
    def _extend_graph(self, graph: rx.PyDiGraph) -> rx.PyDiGraph:
        ...
        for node_index, depth in custom_depth_node_dict.items():
            ...
            sub_graph.add_edge(sub_graph.nodes().index(source_node), sub_graph.nodes().index(target_node), depth - 1)
            graph.substitute_node_with_subgraph(node_index, sub_graph, lambda _, t, __: _link_graph(t, sub_graph, node_index))
    ...
    def has_path(self, a, b):
        ...
        if a is b:
            return True
        return len(rx.digraph_dijkstra_shortest_paths(self._graph, self._indices[id(a)], self._indices[id(b)], weight_fn=None, default_weight=1.0, as_undirected=False)) != 0
