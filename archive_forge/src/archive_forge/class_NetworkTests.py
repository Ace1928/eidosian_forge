...
class NetworkTests(common.NetworkTagTests):
    ...
    def test_network_create_network(self):
        ...
        name1 = uuid.uuid4().hex
        cmd_output = self.openstack('network create ' + name1, parse_output=True)
        self.addCleanup(self.openstack, 'network delete ' + name1)
        self.assertIsNotNone(cmd_output['id'])
        self.assertEqual(admin_project_id, cmd_output['project_id'])
        self.assertEqual('', cmd_output['description'])
        ...
        name2 = uuid.uuid4().hex
        cmd_output = self.openstack('network create ' + '--project demo ' + name2, parse_output=True)
        self.addCleanup(self.openstack, 'network delete ' + name2)
        self.assertIsNotNone(cmd_output['id'])
    ...
    def test_network_list(self):
        ...
        cmd_output = self.openstack('network create ' + network_options + name1, parse_output=True)
        ...
        if self.haz_network:
            ...
            self.assertEqual('3.4.5.0/28', cmd_output['cidr'])
        name2 = uuid.uuid4().hex
        if self.haz_network:
            network_options = '--description bbbb --disable '
        else:
            network_options = '--subnet 4.5.6.7/28 '
        cmd_output = self.openstack('network create --share %s%s' % (network_options, name2), parse_output=True)
        self.addCleanup(self.openstack, 'network delete ' + name2)
        self.assertIsNotNone(cmd_output['id'])
        if self.haz_network:
            self.assertEqual('bbbb', cmd_output['description'])
            ...
            self.assertEqual('4.5.6.0/28', cmd_output['cidr'])
            self.assertTrue(cmd_output['share_address'])
        cmd_output = self.openstack('network list ', parse_output=True)
        col_name = [x['Name'] for x in cmd_output]
        self.assertIn(name1, col_name)
        self.assertIn(name2, col_name)
        if self.haz_network:
            cmd_output = self.openstack('network list --long', parse_output=True)
            col_name = [x['Name'] for x in cmd_output]
            self.assertIn(name1, col_name)
            self.assertIn(name2, col_name)
        if self.haz_network:
            cmd_output = self.openstack('network list --enable --long', parse_output=True)
            col_name = [x['Name'] for x in cmd_output]
            self.assertIn(name1, col_name)
            self.assertNotIn(name2, col_name)
        if self.haz_network:
            cmd_output = self.openstack('network list --disable --long', parse_output=True)
            col_name = [x['Name'] for x in cmd_output]
            self.assertNotIn(name1, col_name)
            self.assertIn(name2, col_name)
        if self.haz_network:
            cmd_output = self.openstack('network list --share ', parse_output=True)
            col_name = [x['Name'] for x in cmd_output]
            self.assertNotIn(name1, col_name)
            self.assertIn(name2, col_name)
        if self.haz_network:
            cmd_output = self.openstack('network list --no-share ', parse_output=True)
            col_name = [x['Name'] for x in cmd_output]
            self.assertIn(name1, col_name)
            self.assertNotIn(name2, col_name)
