...
def test_rename_snapshot(self):
    ...
    self.assert_called('PUT', '/snapshots/1234', body=expected)
    self.run_command('snapshot-rename 1234 new-name --description=new-description')
    expected = {'snapshot': {'display_name': 'new-name', 'display_description': 'new-description'}}
    self.assert_called('PUT', '/snapshots/1234', body=expected)
    self.assertRaises(exceptions.CommandError, self.run_command, 'snapshot-rename 1234')