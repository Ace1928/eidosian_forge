...
class DoubleFactorization(Operation):
    ...
    **Example**
    ...
    def __init__(self, one_electron, two_electron, error=0.0016, rank_r=None, rank_m=None, rank_max=None, tol_factor=1e-05, tol_eigval=1e-05, br=7, alpha=10, beta=20, chemist_notation=False):
        ...
        self.factors, self.eigvals, self.eigvecs = factorize(self.two_electron, self.tol_factor, self.tol_eigval)
        ...
        self.gates = self.gate_cost(self.n, self.lamb, self.error, self.rank_r, self.rank_m, self.rank_max, self.br, self.alpha, self.beta)
        self.qubits = self.qubit_cost(self.n, self.lamb, self.error, self.rank_r, self.rank_m, self.rank_max, self.br, self.alpha, self.beta)
        super().__init__(wires=range(self.qubits))

    def _flatten(self):
        return ((self.one_electron, self.two_electron), (('error', self.error), ('rank_r', self.rank_r), ('rank_m', self.rank_m), ('rank_max', self.rank_max), ('tol_factor', self.tol_factor), ('tol_eigval', self.tol_eigval), ('br', self.br), ('alpha', self.alpha), ('beta', self.beta), ('chemist_notation', True)))
    ...
    def unitary_cost(n, rank_r, rank_m, rank_max, br=7, alpha=10, beta=20):
        ...
        **Example**
        ...
        if rank_max <= 0 or not isinstance(rank_max, int):
            raise ValueError('The maximum rank of the second factorization step must be a positive integer.')
        ...
        nxi = np.ceil(np.log2(rank_max))
        nl = np.ceil(np.log2(rank_r + 1))
        nlxi = np.ceil(np.log2(rank_rm + n / 2))
        bp1 = nl + alpha
        bo = nxi + nlxi + br + 1
        bp2 = nxi + alpha + 2
