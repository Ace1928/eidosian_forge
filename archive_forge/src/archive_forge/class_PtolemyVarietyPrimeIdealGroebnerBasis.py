...
class PtolemyVarietyPrimeIdealGroebnerBasis:
    ...
    def __init__(self, polys, term_order, size, dimension, is_prime, free_variables, py_eval, manifold_thunk=lambda: None, witnesses=[], genus=None):
        self.polys = polys
    ...
    def solutions(self, numerical=False):
        if self.dimension > 0:
            return NonZeroDimensionalComponent([witness.solutions(numerical=numerical) for witness in self.witnesses], dimension=self.dimension, free_variables=self.free_variables, genus=self.genus)
        if numerical:
