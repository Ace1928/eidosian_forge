...
class VPSNetNodeDriver(NodeDriver):
    ...
    def _to_node(self, vm):
        ...
        n = Node(id=vm['id'], name=vm['label'], state=state, public_ips=[vm.get('primary_ip_address', None)], private_ips=[], extra={'slices_count': vm['slices_count']}, driver=self.connection.driver)
        return n

    def _to_image(self, image, cloud):
        image = NodeImage(id=image['id'], name='{}: {}'.format(cloud, image['label']), driver=self.connection.driver)
        return image

    def _to_size(self, num):
        size = NodeSize(id=num, name='%d Node' % (num,), ram=RAM_PER_NODE * num, disk=DISK_PER_NODE, bandwidth=BANDWIDTH_PER_NODE * num, price=self._get_price_per_node(num) * num, driver=self.connection.driver)
        return size
    ...
    def create_node(self, name, image, size, ex_backups_enabled=False, ex_fqdn=None):
        ...
        request = {'virtual_machine': {'label': name, 'fqdn': ex_fqdn or '', 'system_template_id': image.id, 'backups_enabled': ex_backups_enabled, 'slices_required': size.id}}
        res = self.connection.request('/virtual_machines.{}'.format(API_VERSION), data=json.dumps(request), headers=headers, method='POST')
        node = self._to_node(res.object['virtual_machine'])
        return node

    def reboot_node(self, node):
        res = self.connection.request('/virtual_machines/{}/{}.{}'.format(node.id, 'reboot', API_VERSION), method='POST')
        node = self._to_node(res.object['virtual_machine'])
        return True
    ...
    def destroy_node(self, node):
        res = self.connection.request('/virtual_machines/{}.{}'.format(node.id, API_VERSION), method='DELETE')
