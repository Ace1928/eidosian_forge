    ...
    def _read(cls, path_or_buf, **kwargs):
        ...
        **kwargs : dict
        ...
        if isinstance(path_or_buf, str):
            if not cls.file_exists(path_or_buf, storage_options=kwargs.get('storage_options')):
        ...
        with OpenFile(path_or_buf, 'rb', **kwargs.get('storage_options', None) or {}) as f:
            columns = pandas.read_json(BytesIO(b'' + f.readline()), lines=True).columns
        kwargs['columns'] = columns
        empty_pd_df = pandas.DataFrame(columns=columns)
        with OpenFile(path_or_buf, 'rb', kwargs.get('compression', 'infer'), **kwargs.get('storage_options', None) or {}) as f:
