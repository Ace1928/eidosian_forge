...
class ClientSecretPostAuthClient(AbstractAuthClient):
    ...
    def __init__(self, session, introspect_endpoint, audience, client_id, func_get_config_option, logger):
        super(ClientSecretPostAuthClient, self).__init__(session, introspect_endpoint, audience, client_id, func_get_config_option, logger)
    ...
    def introspect(self, access_token):
        ...
        """
        req_data = {'client_id': self.client_id, 'client_secret': self.client_secret, 'token': access_token, 'token_type_hint': 'access_token'}
