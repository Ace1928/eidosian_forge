...
class DevicesCameraSense(object):
    ...
    def __init__(self, params, meraki):
        self.meraki = meraki
        self.new_object = dict(senseEnabled=params.get('senseEnabled'), mqttBrokerId=params.get('mqttBrokerId'), audioDetection=params.get('audioDetection'), detectionModelId=params.get('detectionModelId'), serial=params.get('serial'))
    ...
    def update_all_params(self):
        ...
        if self.new_object.get('audioDetection') is not None or self.new_object.get('audio_detection') is not None:
            new_object_params['audioDetection'] = self.new_object.get('audioDetection') or self.new_object.get('audio_detection')
        if self.new_object.get('detectionModelId') is not None or self.new_object.get('detection_model_id') is not None:
            new_object_params['detectionModelId'] = self.new_object.get('detectionModelId') or self.new_object.get('detection_model_id')
        if self.new_object.get('serial') is not None or self.new_object.get('serial') is not None:
    ...
    def get_object_by_name(self, name):
        result = None
        try:
            items = self.meraki.exec_meraki(family='camera', function='getDeviceCameraSense', params=self.get_all_params(name=name))
    ...
    def requires_update(self, current_obj):
        ...
        obj_params = [('senseEnabled', 'senseEnabled'), ('mqttBrokerId', 'mqttBrokerId'), ('audioDetection', 'audioDetection'), ('detectionModelId', 'detectionModelId'), ('serial', 'serial')]
    ...
    def update(self):
        ...
        name = self.new_object.get('name')
        result = None
        result = self.meraki.exec_meraki(family='camera', function='updateDeviceCameraSense', params=self.update_all_params(), op_modifies=True)
        return result