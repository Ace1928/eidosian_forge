...
class WulffShape:
    ...
    def __init__(self, lattice: Lattice, miller_list, e_surf_list, symprec=1e-05):
        """
        Args:
    ...
    def get_plot(self, color_set='PuBu', grid_off=True, axis_off=True, show_area=False, alpha=1, off_color='red', direction=None, bar_pos=(0.75, 0.15, 0.05, 0.65), bar_on=False, units_in_JPERM2=True, legend_on=True, aspect_ratio=(8, 8), custom_colors=None):
        """
        ...
        if legend_on:
            if show_area:
                ax_3d.legend(color_proxy, self.miller_area, loc='upper left', bbox_to_anchor=(0, 1), fancybox=True, shadow=False)
            else:
                ax_3d.legend(color_proxy_on_wulff, miller_on_wulff, loc='upper center', bbox_to_anchor=(0.5, 1), ncol=3, fancybox=True, shadow=False)
    ...
    def get_plotly(self, color_set='PuBu', off_color='red', alpha=1, custom_colors=None, units_in_JPERM2=True):
        """
        ...
        color_list, _color_proxy, _color_proxy_on_wulff, _miller_on_wulff, e_surf_on_wulff = self._get_colors(color_set, alpha, off_color, custom_colors=custom_colors or {})
        ...
        for plane in self.facets:
            ...
            planes_data.append(go.Mesh3d(x=x_pts, y=y_pts, z=z_pts, i=tri_indices[0], j=tri_indices[1], k=tri_indices[2], hovertemplate=f'<br>%{{text}}<br>y={plane.e_surf:.3f} {units}<br>', color=color, text=[f'Miller index: {hkl}'] * len(x_pts), hoverinfo='name', name=''))
        ...
        colorbar = go.Mesh3d(x=[0], y=[0], z=[0], colorbar=go.ColorBar(title={'text': f'Surface energy {units}', 'side': 'right', 'font': {'size': 25}}, ticktext=ticktext, tickvals=tickvals), colorscale=[[0, 'rgb(255,255,255, 255)'], *color_scale], intensity=[0, 0.33, 0.66, 1], i=[0], j=[0], k=[0], name='y', showscale=True)
        planes_data.append(colorbar)
        axis_dict = {'title': '', 'autorange': True, 'showgrid': False, 'zeroline': False, 'ticks': '', 'showline': False, 'showticklabels': False, 'showbackground': False}
