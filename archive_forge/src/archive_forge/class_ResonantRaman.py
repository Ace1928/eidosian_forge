...
class ResonantRaman(Raman):
    ...
    def __init__(self, atoms, Excitations, *args, observation=None, form='v', exkwargs=None, exext='.ex.gz', overlap=False, minoverlap=0.02, minrep=0.8, comm=world, **kwargs):
        """
        Parameters
        ----------
    ...
    def get_spectrum(self, omega, gamma=0.1, start=None, end=None, npts=None, width=20, type='Gaussian', intensity_unit='????', normalize=False):
    ...
    def summary(self, omega, gamma=0.1, method='standard', direction='central', log=sys.stdout):
        ...
        for n, e in enumerate(hnu):
            if e.imag != 0:
                ...
                c = ' '
                e = e.real
            parprint('%3d %6.1f%s  %7.1f%s  %9.2f' % (n, 1000 * e, c, e / u.invcm, c, intensities[n] * scale), file=log)
        parprint('-------------------------------------', file=log)
