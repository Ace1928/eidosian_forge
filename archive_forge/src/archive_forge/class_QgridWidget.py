...
class QgridWidget(widgets.DOMWidget):
    ...
    def _update_table(self, update_columns=False, triggered_by=None, scroll_to_row=None, fire_data_change_event=True):
        ...
        if update_columns:
            self._string_columns = list(df.select_dtypes(include=[np.dtype('O'), 'category']).columns.values)
        ...
        for col_name in self._string_columns:
            ...
            if sort_column_name:
                ...
                series_to_set = self._get_col_series_from_df(col_name, df, level_vals=True).map(stringify)
        ...
        if type(df.index) == pd.MultiIndex and (not self._disable_grouping):
            ...
            self._row_styles = {}
        df_json = pd_json.to_json(None, df, orient='table', date_format='iso', double_precision=self.precision)
        if update_columns:
            ...
            for i, cur_column in enumerate(df_schema['fields']):
                ...
                if col_name in self._primary_key:
                    ...
                    cur_column['index_display_text'] = self._primary_key_display[col_name]
        ...
        if len(self._period_columns) > 0:
            for col_name in self._period_columns:
                ...
                if sort_column_name:
                    ...
                    series_to_set = self._get_col_series_from_df(col_name, df, level_vals=True).to_timestamp()
                self._set_col_series_on_df(col_name, df, series_to_set)
        ...
        if fire_data_change_event:
            self._notify_listeners({'name': 'json_updated', 'triggered_by': triggered_by, 'range': self._df_range})
            data_to_send = {'type': 'update_data_view', 'columns': self._columns, 'triggered_by': triggered_by}
            if scroll_to_row:
    ...
    def _handle_show_filter_dropdown(self, content):
        ...
        if col_info['type'] in ['integer', 'number']:
            if 'filter_info' not in col_info or (col_info['filter_info']['min'] is None and col_info['filter_info']['max'] is None):
                col_info['slider_max'] = max(col_series)
                col_info['slider_min'] = min(col_series)
                self._columns[col_name] = col_info
            self.send({'type': 'column_min_max_updated', 'field': col_name, 'col_info': col_info})
            return
            ...
            elif col_name in self._sorted_column_cache:
            ...
            if content['search_val'] is not None:
                unique_list = [k for k in unique_list if content['search_val'].lower() in str(k).lower()]
            if 'filter_info' in col_info and 'selected' in col_info['filter_info']:
            ...
            try:
                self.send({'type': message_type, 'field': col_name, 'col_info': col_info})
            except ValueError:
    ...
    def _handle_qgrid_msg_helper(self, content):
        ...
        if content['type'] == 'edit_cell':
            ...
            try:
                ...
                self._notify_listeners({'name': 'cell_edited', 'index': location[0], 'column': location[1], 'old': old_value, 'new': val_to_set, 'source': 'gui'})
            except (ValueError, TypeError):
                msg = 'Error occurred while attempting to edit the DataFrame. Check the notebook server logs for more information.'
            ...
            self._notify_listeners({'name': 'viewport_changed', 'old': old_viewport_range, 'new': self._viewport_range})
        elif content['type'] == 'add_row':
            row_index = self._duplicate_last_row()
            self._notify_listeners({'name': 'row_added', 'index': row_index, 'source': 'gui'})
        elif content['type'] == 'remove_row':
            removed_indices = self._remove_rows()
            self._notify_listeners({'name': 'row_removed', 'indices': removed_indices, 'source': 'gui'})
            ...
            self._columns[col_name] = col_info
            self.send({'type': 'update_data_view_filter', 'field': col_name, 'col_info': col_info})
            self._notify_listeners({'name': 'text_filter_viewport_changed', 'column': col_name, 'old': old_viewport_range, 'new': col_info['viewport_range']})
            ...
            self._notify_listeners({'name': 'sort_changed', 'old': {'column': old_column, 'ascending': old_ascending}, 'new': {'column': self._sort_field, 'ascending': self._sort_ascending}})
        elif content['type'] == 'show_filter_dropdown':
            self._handle_show_filter_dropdown(content)
            self._notify_listeners({'name': 'filter_dropdown_shown', 'column': content['field']})
        elif content['type'] == 'change_filter':
            self._handle_change_filter(content)
            self._notify_listeners({'name': 'filter_changed', 'column': content['field']})
    ...
    def add_row(self, row=None):
        ...
        self._notify_listeners({'name': 'row_added', 'index': added_index, 'source': 'api'})
    ...
    def remove_rows(self, rows=None):
        ...
        self._notify_listeners({'name': 'row_removed', 'indices': row_indices, 'source': 'api'})
        return row_indices
