...
class TestDu(testcase.GsUtilIntegrationTestCase):
    ...
    def test_multi_args(self):
        ...
        @Retry(AssertionError, tries=3, timeout_secs=1)
        def _Check():
            stdout = self.RunGsUtil(['du', suri(obj_uri1), suri(obj_uri2)], return_stdout=True)
            self.assertSetEqual(set(stdout.splitlines()), set(['%-11s  %s' % (3, suri(obj_uri1)), '%-11s  %s' % (4, suri(obj_uri2))]))
        _Check()
    ...
    def test_summary(self):
        ...
        @Retry(AssertionError, tries=3, timeout_secs=1)
        def _Check():
            stdout = self.RunGsUtil(['du', '-s', suri(bucket_uri1), suri(bucket_uri2)], return_stdout=True)
            self.assertSetEqual(set(stdout.splitlines()), set(['%-11s  %s' % (18, suri(bucket_uri1)), '%-11s  %s' % (18, suri(bucket_uri2))]))
        _Check()
    ...
    def test_null_endings(self):
        ...
        def _Check():
            ...
            self.assertSetEqual(set(stdout.split('\x00')), set(['%-11s  %s' % (3, suri(obj_uri1)), '%-11s  %s' % (5, suri(obj_uri2)), '%-11s  total' % 8, '']))
        _Check()
