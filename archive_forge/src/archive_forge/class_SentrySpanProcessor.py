...
class SentrySpanProcessor(SpanProcessor):
    ...
    def on_start(self, otel_span, parent_context=None):
        ...
        sentry_parent_span = self.otel_span_map.get(parent_span_id, None) if parent_span_id else None
        sentry_span = None
        if sentry_parent_span:
            sentry_span = sentry_parent_span.start_child(span_id=trace_data['span_id'], description=otel_span.name, start_timestamp=utc_from_timestamp(otel_span.start_time / 1000000000.0), instrumenter=INSTRUMENTER.OTEL)
        else:
            sentry_span = hub.start_transaction(name=otel_span.name, span_id=trace_data['span_id'], parent_span_id=parent_span_id, trace_id=trace_data['trace_id'], baggage=trace_data['baggage'], start_timestamp=utc_from_timestamp(otel_span.start_time / 1000000000.0), instrumenter=INSTRUMENTER.OTEL)
        self.otel_span_map[trace_data['span_id']] = sentry_span
        span_start_in_minutes = int(otel_span.start_time / 1000000000.0 / 60)
        self.open_spans.setdefault(span_start_in_minutes, set()).add(trace_data['span_id'])
