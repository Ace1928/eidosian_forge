...
class BaseOutputParser(BaseLLMOutputParser, RunnableSerializable[LanguageModelOutput, T]):
    ...
    def OutputType(self) -> Type[T]:
        for cls in self.__class__.__orig_bases__:
            ...
            if type_args and len(type_args) == 1:
                return type_args[0]
        raise TypeError(f"Runnable {self.__class__.__name__} doesn't have an inferable OutputType. Override the OutputType property to specify the output type.")
    ...
    def _type(self) -> str:
        ...
        raise NotImplementedError(f'_type property is not implemented in class {self.__class__.__name__}. This is required for serialization.')
