...
def _ConstructCloudSqlMachineTypePatch(cloud_sql_machine_type, release_track):
  ...
  """
    ...
    config = messages.EnvironmentConfig(databaseConfig=messages.DatabaseConfig(machineType=cloud_sql_machine_type))
    return ('config.database_config.machine_type', messages.Environment(config=config))