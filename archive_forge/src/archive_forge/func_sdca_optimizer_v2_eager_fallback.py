...
def sdca_optimizer_v2_eager_fallback(sparse_example_indices: List[_atypes.TensorFuzzingAnnotation[_atypes.Int64]], sparse_feature_indices: List[_atypes.TensorFuzzingAnnotation[_atypes.Int64]], sparse_feature_values: List[_atypes.TensorFuzzingAnnotation[_atypes.Float32]], dense_features: List[_atypes.TensorFuzzingAnnotation[_atypes.Float32]], example_weights: _atypes.TensorFuzzingAnnotation[_atypes.Float32], example_labels: _atypes.TensorFuzzingAnnotation[_atypes.Float32], sparse_indices: List[_atypes.TensorFuzzingAnnotation[_atypes.Int64]], sparse_weights: List[_atypes.TensorFuzzingAnnotation[_atypes.Float32]], dense_weights: List[_atypes.TensorFuzzingAnnotation[_atypes.Float32]], example_state_data: _atypes.TensorFuzzingAnnotation[_atypes.Float32], loss_type: str, l1: float, l2: float, num_loss_partitions: int, num_inner_iterations: int, adaptive: bool, name, ctx):
    if not isinstance(sparse_example_indices, (list, tuple)):
        raise TypeError("Expected list for 'sparse_example_indices' argument to 'sdca_optimizer_v2' Op, not %r." % sparse_example_indices)
    _attr_num_sparse_features = len(sparse_example_indices)
    if not isinstance(sparse_feature_indices, (list, tuple)):
        raise TypeError("Expected list for 'sparse_feature_indices' argument to 'sdca_optimizer_v2' Op, not %r." % sparse_feature_indices)
    if len(sparse_feature_indices) != _attr_num_sparse_features:
        raise ValueError("List argument 'sparse_feature_indices' to 'sdca_optimizer_v2' Op with length %d must match length %d of argument 'sparse_example_indices'." % (len(sparse_feature_indices), _attr_num_sparse_features))
    if not isinstance(sparse_indices, (list, tuple)):
        raise TypeError("Expected list for 'sparse_indices' argument to 'sdca_optimizer_v2' Op, not %r." % sparse_indices)
    if len(sparse_indices) != _attr_num_sparse_features:
        raise ValueError("List argument 'sparse_indices' to 'sdca_optimizer_v2' Op with length %d must match length %d of argument 'sparse_example_indices'." % (len(sparse_indices), _attr_num_sparse_features))
    if not isinstance(sparse_weights, (list, tuple)):
        raise TypeError("Expected list for 'sparse_weights' argument to 'sdca_optimizer_v2' Op, not %r." % sparse_weights)
    if len(sparse_weights) != _attr_num_sparse_features:
        raise ValueError("List argument 'sparse_weights' to 'sdca_optimizer_v2' Op with length %d must match length %d of argument 'sparse_example_indices'." % (len(sparse_weights), _attr_num_sparse_features))
    if not isinstance(sparse_feature_values, (list, tuple)):
        raise TypeError("Expected list for 'sparse_feature_values' argument to 'sdca_optimizer_v2' Op, not %r." % sparse_feature_values)
    _attr_num_sparse_features_with_values = len(sparse_feature_values)
    if not isinstance(dense_features, (list, tuple)):
        raise TypeError("Expected list for 'dense_features' argument to 'sdca_optimizer_v2' Op, not %r." % dense_features)
    _attr_num_dense_features = len(dense_features)
    if not isinstance(dense_weights, (list, tuple)):
        raise TypeError("Expected list for 'dense_weights' argument to 'sdca_optimizer_v2' Op, not %r." % dense_weights)
    if len(dense_weights) != _attr_num_dense_features:
        raise ValueError("List argument 'dense_weights' to 'sdca_optimizer_v2' Op with length %d must match length %d of argument 'dense_features'." % (len(dense_weights), _attr_num_dense_features))
    ...
    _attrs = ('loss_type', loss_type, 'adaptive', adaptive, 'num_sparse_features', _attr_num_sparse_features, 'num_sparse_features_with_values', _attr_num_sparse_features_with_values, 'num_dense_features', _attr_num_dense_features, 'l1', l1, 'l2', l2, 'num_loss_partitions', num_loss_partitions, 'num_inner_iterations', num_inner_iterations)
    _result = _execute.execute(b'SdcaOptimizerV2', _attr_num_sparse_features + _attr_num_dense_features + 1, inputs=_inputs_flat, attrs=_attrs, ctx=ctx, name=name)
    if _execute.must_record_gradient():
        _execute.record_gradient('SdcaOptimizerV2', _inputs_flat, _attrs, _result)
