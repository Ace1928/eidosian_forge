...
class BaseDatabaseWrapper:
    ...
    def queries(self):
        if len(self.queries_log) == self.queries_log.maxlen:
            warnings.warn('Limit for query logging exceeded, only the last {} queries will be returned.'.format(self.queries_log.maxlen))
    ...
    def validate_no_broken_transaction(self):
        if self.needs_rollback:
            raise TransactionManagementError("An error occurred in the current transaction. You can't execute queries until the end of the 'atomic' block.") from self.rollback_exc
