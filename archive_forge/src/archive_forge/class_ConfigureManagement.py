...
class ConfigureManagement(base.UpdateCommand):
  ...
  """
    ...
    @staticmethod
    def Args(parser):
        resource_args.AddRegistrationResourceArg(parser, 'to configure management settings for')
    ...
    def Run(self, args):
        ...
        transfer_lock_state = util.ParseTransferLockState(api_version, args.transfer_lock_state)
        renewal_method = util.ParseRenewalMethod(api_version, args.preferred_renewal_method)
        if transfer_lock_state is None and renewal_method is None:
            transfer_lock_state = util.PromptForTransferLockState(api_version, registration.managementSettings.transferLockState)
            renewal_method = util.PromptForRenewalMethod(api_version, registration.managementSettings.preferredRenewalMethod)
        if transfer_lock_state is None and renewal_method is None:
            return None
        response = client.ConfigureManagement(registration_ref, transfer_lock_state, renewal_method)
