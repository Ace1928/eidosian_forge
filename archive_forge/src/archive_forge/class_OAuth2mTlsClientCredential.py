...
class OAuth2mTlsClientCredential(base.BaseAuth, metaclass=abc.ABCMeta):
    ...
    def __init__(self, auth_url, oauth2_endpoint, oauth2_client_id, *args, **kwargs):
        super(OAuth2mTlsClientCredential, self).__init__(auth_url, *args, **kwargs)
    ...
    def get_auth_ref(self, session, **kwargs):
        ...
        """
        data = {'grant_type': 'client_credentials', 'client_id': self.oauth2_client_id}
        resp = session.post(url=self.oauth2_endpoint, authenticated=False, raise_exc=False, data=data)
        ...
        headers = {'Accept': 'application/json', 'X-Auth-Token': self.oauth2_access_token, 'X-Subject-Token': self.oauth2_access_token}
    ...
    def get_headers(self, session, **kwargs):
        ...
        :rtype: dict
        """
        headers = super(OAuth2mTlsClientCredential, self).get_headers(session, **kwargs)
        if headers:
