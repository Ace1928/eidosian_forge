...
def _log_tensorflow_dataset(tensorflow_dataset, source, context, name=None, targets=None):
    ...
    if isinstance(tensorflow_dataset, np.ndarray):
        ...
        dataset = from_tensorflow(features=tensorflow_dataset, targets=targets, source=source, name=name)
        ...
        _logger.warning('Unrecognized dataset type %s. Dataset logging skipped.', type(tensorflow_dataset))
        return
