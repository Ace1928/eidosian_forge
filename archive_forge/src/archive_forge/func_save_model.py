...
def save_model(model, filepath, overwrite=True, include_optimizer=True, save_format=None, signatures=None, options=None, save_traces=True):
  ...
  """
    ...
    if save_format == 'h5' or (h5py is not None and isinstance(filepath, h5py.File)) or saving_utils.is_hdf5_filepath(filepath):
        if not model._is_graph_network and (not isinstance(model, sequential.Sequential)):
            raise NotImplementedError('Saving the model to HDF5 format requires the model to be a Functional model or a Sequential model. It does not work for subclassed models, because such models are defined via the body of a Python method, which isn\'t safely serializable. Consider saving to the Tensorflow SavedModel format (by setting save_format="tf") or using `save_weights`.')
        hdf5_format.save_model_to_hdf5(model, filepath, overwrite, include_optimizer)
    else:
        with generic_utils.SharedObjectSavingScope():
            saved_model_save.save(model, filepath, overwrite, include_optimizer, signatures, options, save_traces)