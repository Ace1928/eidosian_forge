...
class _OllamaCommon(BaseLanguageModel):
    ...
    async def _acreate_generate_stream(self, prompt: str, stop: Optional[List[str]]=None, images: Optional[List[str]]=None, **kwargs: Any) -> AsyncIterator[str]:
        payload = {'prompt': prompt, 'images': images}
        async for item in self._acreate_stream(payload=payload, stop=stop, api_url=f'{self.base_url}/api/generate', **kwargs):
            yield item
    ...
    async def _acreate_stream(self, api_url: str, payload: Any, stop: Optional[List[str]]=None, **kwargs: Any) -> AsyncIterator[str]:
        if self.stop is not None and stop is not None:
            raise ValueError('`stop` found in both the input and default params.')
        elif self.stop is not None:
            stop = self.stop
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url=api_url, headers={'Content-Type': 'application/json', **(self.headers if isinstance(self.headers, dict) else {})}, json=request_payload, timeout=self.timeout) as response:
                if response.status != 200:
                    if response.status == 404:
                        raise OllamaEndpointNotFoundError('Ollama call failed with status code 404.')
                    else:
                        optional_detail = response.text
                        raise ValueError(f'Ollama call failed with status code {response.status}. Details: {optional_detail}')
                async for line in response.content:
                    yield line.decode('utf-8')

    def _stream_with_aggregation(self, prompt: str, stop: Optional[List[str]]=None, run_manager: Optional[CallbackManagerForLLMRun]=None, verbose: bool=False, **kwargs: Any) -> GenerationChunk:
    ...
    async def _astream_with_aggregation(self, prompt: str, stop: Optional[List[str]]=None, run_manager: Optional[AsyncCallbackManagerForLLMRun]=None, verbose: bool=False, **kwargs: Any) -> GenerationChunk:
        final_chunk: Optional[GenerationChunk] = None
