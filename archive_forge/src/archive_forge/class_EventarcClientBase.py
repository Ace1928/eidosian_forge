...
class EventarcClientBase(object):
    ...
    """
        ...
        operation_ref = resources.REGISTRY.Parse(operation.name, collection='eventarc.projects.locations.operations')
        resource_name = resource_ref.Name()
        project_name = resource_ref.Parent().Parent().Name()
        location_name = resource_ref.Parent().Name()
        message = '{} {} [{}] in project [{}], location [{}]'.format(operation_type, self._resource_label, resource_name, project_name, location_name)
        if loading_msg:
            message = '{}, {}'.format(message, loading_msg)
        try:
            return waiter.WaitFor(poller, operation_ref, message, wait_ceiling_ms=20000)
        except apitools_exceptions.HttpForbiddenError as e:
            desc_cmd = 'gcloud eventarc {}s describe {} --location={}'.format(self._resource_label, resource_name, location_name)
            error_message = 'Failed to poll status of the operation due to {status_message}, but the operation may have succeeded. Please fix the permission issue, then either check the %s by running `%s`, or rerun the original command.' % (self._resource_label, desc_cmd)
