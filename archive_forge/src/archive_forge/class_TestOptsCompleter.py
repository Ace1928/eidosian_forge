...
class TestOptsCompleter(IPTestCase):
    ...
    def setUp(self):
        self.completions = {'AnElement': (['plotoptA1', 'plotoptA2'], ['styleoptA1', 'styleoptA2']), 'AnotherElement': (['plotoptB1', 'plotoptB2'], ['styleoptB1', 'styleoptB2']), 'BarElement': (['plotoptC1', 'plotoptC2'], ['styleoptC1', 'styleoptC2'])}
    ...
    def test_completions_long_norm1(self):
        """Suggest corresponding plot options"""
        suggestions = OptsCompleter.line_completer('%%opts AnElement norm{', self.completions, self.compositor_defs)
        self.assertEqual(suggestions, ['+axiswise', '+framewise'])

    def test_completions_short_plot_closed1(self):
        """Suggest corresponding plot options"""
        suggestions = OptsCompleter.line_completer('%%opts AnElement [test=1]', self.completions, self.compositor_defs)
        self.assertEqual(suggestions, self.all_keys)
