...
class AbderaConverter(XMLSchemaConverter):
    ...
    def __init__(self, namespaces: Optional[NamespacesType]=None, dict_class: Optional[Type[Dict[str, Any]]]=None, list_class: Optional[Type[List[Any]]]=None, **kwargs: Any) -> None:
        ...
        super(AbderaConverter, self).__init__(namespaces, dict_class, list_class, **kwargs)
    ...
    def element_decode(self, data: ElementData, xsd_element: 'XsdElement', xsd_type: Optional[BaseXsdType]=None, level: int=0) -> Any:
        ...
        if data.attributes:
            if children != []:
                return self.dict([('attributes', self.dict(((k, v) for k, v in self.map_attributes(data.attributes)))), ('children', self.list([children]) if children is not None else self.list())])
            else:
                return self.dict([('attributes', self.dict(((k, v) for k, v in self.map_attributes(data.attributes))))])
        else:
