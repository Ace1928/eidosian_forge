...
class AsyncPipe(Module):
    ...
    def __init__(self, module: Union[nn.Sequential, List[LazyModule]], balance: Iterable[int], *, group: Optional[torch.distributed.ProcessGroup]=None, worker_map: Optional[Dict[int, str]]=None, input_device: Union[None, int, str, torch.device]=None, chunks: int=chunks, checkpoint: str=checkpoint, deferred_batch_norm: bool=False) -> None:
        super().__init__()
        if chunks <= 0:
        ...
        if self.group.size() < len(self.balance):
            raise IndexError(f'too few ranks to hold given partitions (ranks: {self.group.size()}, partitions: {len(self.balance)})')
    ...
    def create_pipeline(self) -> None:
        ...
        self.pipeline = AsyncPipeline(self.partitions, self._skip_layout, checkpoint_stop, group=self.group, worker_map=self.worker_map, input_device=self.input_device, final_stage=self.final_stage)
