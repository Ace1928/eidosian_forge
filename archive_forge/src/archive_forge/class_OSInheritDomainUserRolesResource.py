...
class OSInheritDomainUserRolesResource(flask_restful.Resource):
    ...
    def put(self, domain_id, user_id, role_id):
        ...
        """
        ENFORCER.enforce_call(action='identity:create_grant', build_target=functools.partial(_build_enforcement_target_attr, domain_id=domain_id, user_id=user_id, role_id=role_id))
        PROVIDERS.assignment_api.create_grant(domain_id=domain_id, user_id=user_id, role_id=role_id, inherited_to_projects=True)
        return (None, http.client.NO_CONTENT)
    ...
    def delete(self, domain_id, user_id, role_id):
        ...
        DELETE /OS-INHERIT/domains/{domain_id}/users/{user_id}/roles
               /{role_id}/inherited_to_projects
        """
        ENFORCER.enforce_call(action='identity:revoke_grant', build_target=functools.partial(_build_enforcement_target_attr, domain_id=domain_id, user_id=user_id, role_id=role_id))
        PROVIDERS.assignment_api.delete_grant(domain_id=domain_id, user_id=user_id, role_id=role_id, inherited_to_projects=True)
        return (None, http.client.NO_CONTENT)