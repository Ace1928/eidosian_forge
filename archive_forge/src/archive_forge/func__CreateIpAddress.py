...
def _CreateIpAddress(messages, ip_address):
    if re.search('[A-Za-z]', ip_address):
        return messages.PreservedStatePreservedNetworkIpIpAddress(address=ip_address)
    else:
        return messages.PreservedStatePreservedNetworkIpIpAddress(literal=ip_address)