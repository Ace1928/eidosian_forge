...
def wireless_controller_ap_status(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('wireless-controller', 'ap-status', filtered_data, vdom=vdom)
    ...
    if state == 'present' or state is True:
        return fos.set('wireless-controller', 'ap-status', data=filtered_data, vdom=vdom)
    elif state == 'absent':
        return fos.delete('wireless-controller', 'ap-status', mkey=filtered_data['id'], vdom=vdom)
    else:
