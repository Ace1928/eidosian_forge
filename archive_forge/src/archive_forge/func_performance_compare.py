...
def performance_compare(arguments):
    parser = argparse.ArgumentParser(description='Performance CI/CD check for Charset-Normalizer')
    parser.add_argument('-s', '--size-increase', action='store', default=1, type=int, dest='size_coeff', help='Apply artificial size increase to challenge the detection mechanism further')
    ...
    for tbt_path in sorted(glob('./char-dataset/**/*.*')):
        ...
        before = time_ns()
        detect(content)
        charset_normalizer_results.append(round((time_ns() - before) / 1000000000, 5))
