...
class EnhancedPolicyNetwork(nn.Module):
    def __init__(
        ...
        Detailed logging is used to trace the initialization process and the network configuration.
        """
        ...
        if (
            ...
            logger.error(
                "Invalid dimensions or layers information provided: input_dim={}, output_dim={}, layers_info={}".format(
                    input_dim, output_dim, layers_info
                )
            )
        ...
        # Constructing the sequential model layer by layer
        layers = []
        current_dim = input_dim
        for index, layer_dim in enumerate(layers_info):
            linear_layer = nn.Linear(current_dim, layer_dim)
            relu_layer = nn.ReLU()
            batch_norm_layer = nn.BatchNorm1d(layer_dim)
            layers.extend([linear_layer, relu_layer, batch_norm_layer])
            current_dim = layer_dim

        # Adding the final output layer
        final_output_layer = nn.Linear(current_dim, output_dim)
        layers.append(final_output_layer)

        # Compiling all layers into a sequential model
