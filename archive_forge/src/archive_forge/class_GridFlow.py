...
class GridFlow(WidgetWrap[Pile], WidgetContainerMixin, WidgetContainerListContentsMixin):
    ...
    def __init__(self, cells: Iterable[Widget], cell_width: int, h_sep: int, v_sep: int, align: Literal['left', 'center', 'right'] | Align | tuple[Literal['relative', WHSettings.RELATIVE], int], focus: int | Widget | None=None) -> None:
        """
        ...
        self._contents: MonitoredFocusList[tuple[Widget, tuple[Literal[WHSettings.GIVEN], int]]] = MonitoredFocusList(prepared_contents, focus=focus_position)
    ...
    def _repr_attrs(self) -> dict[str, typing.Any]:
        attrs = {**super()._repr_attrs(), 'cell_width': self.cell_width, 'h_sep': self.h_sep, 'v_sep': self.v_sep, 'align': self.align, 'focus': self.focus_position if len(self._contents) > 1 else None}
    ...
    def _contents_modified(self, _slc: tuple[int, int, int], new_items: Iterable[tuple[Widget, tuple[Literal['given', WHSettings.GIVEN], int]]]) -> None:
    ...
    def cells(self):
        ...
        .. note:: only for backwards compatibility. You should use the new
            ...
            contents.
        """
        warnings.warn('only for backwards compatibility.You should use the new standard container property `contents` to modify GridFlow', PendingDeprecationWarning, stacklevel=2)
    ...
    def options(self, width_type: Literal['given', WHSettings.GIVEN]=WHSettings.GIVEN, width_amount: int | None=None) -> tuple[Literal[WHSettings.GIVEN], int]:
        """
    ...
    def set_focus(self, cell: Widget | int) -> None:
        ...
        """
        warnings.warn('only for backwards compatibility.You may also use the new standard container property `focus_position` to set the focus.', PendingDeprecationWarning, stacklevel=2)
        if isinstance(cell, int):
            try:
                if cell < 0 or cell >= len(self.contents):
                    raise IndexError(f'No GridFlow child widget at position {cell}')
            except TypeError as exc:
                raise IndexError(f'No GridFlow child widget at position {cell}').with_traceback(exc.__traceback__) from exc
    ...
    def get_focus(self):
        ...
        """
        warnings.warn('only for backwards compatibility.You may also use the new standard container property `focus` to get the focus.', PendingDeprecationWarning, stacklevel=2)
        if not self.contents:
            return None
        return self.contents[self.focus_position][0]
    ...
    def _set_focus_cell(self, cell: Widget) -> None:
        warnings.warn('only for backwards compatibility.You may also use the new standard container property`focus` to get the focus and `focus_position` to get/set the cell in focus by index', DeprecationWarning, stacklevel=3)
        for i, (w, _options) in enumerate(self.contents):
            if cell == w:
                self.focus_position = i
                return
    ...
    def focus_position(self, position: int) -> None:
        ...
        try:
            ...
            raise IndexError(f'No GridFlow child widget at position {position}').with_traceback(exc.__traceback__) from exc
    ...
    def mouse_event(self, size: tuple[int] | tuple[()], event: str, button: int, col: int, row: int, focus: bool) -> Literal[True]:
        self.get_display_widget(size)
