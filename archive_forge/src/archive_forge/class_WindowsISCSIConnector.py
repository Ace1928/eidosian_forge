...
class WindowsISCSIConnector(win_conn_base.BaseWindowsConnector, base_iscsi.BaseISCSIConnector):
    ...
    def validate_initiators(self):
        ...
        if not self.initiator_list:
            LOG.info('No iSCSI initiator was explicitly requested. The Microsoft iSCSI initiator will choose the initiator when establishing sessions.')
        else:
            ...
            for initiator in self.initiator_list:
                if initiator not in available_initiators:
                    LOG.warning('The requested initiator %(req_initiator)s is not in the list of available initiators: %(avail_initiators)s.', dict(req_initiator=initiator, avail_initiators=available_initiators))
    ...
    def _get_all_paths(self, connection_properties):
        ...
        paths = [(initiator_name, target_portal, target_iqn, target_lun) for target_portal, target_iqn, target_lun in all_targets for initiator_name in initiator_list]
        return paths
    ...
    def connect_volume(self, connection_properties):
        ...
        for initiator_name, target_portal, target_iqn, target_lun in self._get_all_paths(connection_properties):
            try:
                LOG.info('Attempting to establish an iSCSI session to target %(target_iqn)s on portal %(target_portal)s accessing LUN %(target_lun)s using initiator %(initiator_name)s.', dict(target_portal=target_portal, target_iqn=target_iqn, target_lun=target_lun, initiator_name=initiator_name))
                self._iscsi_utils.login_storage_target(target_lun=target_lun, target_iqn=target_iqn, target_portal=target_portal, auth_username=connection_properties.get('auth_username'), auth_password=connection_properties.get('auth_password'), mpio_enabled=self.use_multipath, initiator_name=initiator_name, ensure_lun_available=False)
        ...
        for target_iqn, target_lun in connected_target_mappings:
            try:
                device_number, device_path = self._iscsi_utils.get_device_number_and_path(target_iqn, target_lun, retry_attempts=self.device_scan_attempts, retry_interval=self.device_scan_interval, rescan_disks=True, ensure_mpio_claimed=self.use_multipath)
                volume_connected = True
            except os_win_exc.OSWinException:
                LOG.exception('Could not retrieve device path for target %(target_iqn)s and lun %(target_lun)s.', dict(target_iqn=target_iqn, target_lun=target_lun))
        if not volume_connected:
            raise exception.BrickException(_('Could not connect volume %s.') % connection_properties)
        scsi_wwn = self._get_scsi_wwn(device_number)
        device_info = {'type': 'block', 'path': device_path, 'number': device_number, 'scsi_wwn': scsi_wwn}
        return device_info
    ...
    def disconnect_volume(self, connection_properties, device_info=None, force=False, ignore_errors=False):
        ...
        for target_portal, target_iqn, target_lun in self._get_all_targets(connection_properties):
    ...
    def get_volume_paths(self, connection_properties):
        device_paths = set()
        for target_portal, target_iqn, target_lun in self._get_all_targets(connection_properties):
            device_number, device_path = self._iscsi_utils.get_device_number_and_path(target_iqn, target_lun, ensure_mpio_claimed=self.use_multipath)
            if device_path:
