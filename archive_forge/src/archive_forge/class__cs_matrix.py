...
class _cs_matrix(_data_matrix, _minmax_mixin, IndexMixin):
    ...
    def __init__(self, arg1, shape=None, dtype=None, copy=False):
        ...
        if issparse(arg1):
            ...
            if isshape(arg1):
                ...
                raise ValueError(f'unrecognized {self.format}_matrix constructor usage')
        else:
            try:
    ...
    def __eq__(self, other):
        if isscalarlike(other):
            ...
            if other == 0:
                warn('Comparing a sparse matrix with 0 using == is inefficient, try using != instead.', SparseEfficiencyWarning, stacklevel=3)
            ...
            warn('Comparing sparse matrices using == is inefficient, try using != instead.', SparseEfficiencyWarning, stacklevel=3)
    ...
    def __ne__(self, other):
        if isscalarlike(other):
            if np.isnan(other):
                warn('Comparing a sparse matrix with nan using != is inefficient', SparseEfficiencyWarning, stacklevel=3)
                all_true = self.__class__(np.ones(self.shape, dtype=np.bool_))
                return all_true
            elif other != 0:
                warn('Comparing a sparse matrix with a nonzero scalar using != is inefficient, try using == instead.', SparseEfficiencyWarning, stacklevel=3)
                all_true = self.__class__(np.ones(self.shape), dtype=np.bool_)
    ...
    def multiply(self, other):
        ...
        if self.shape == other.shape:
            ...
            return self._coo_container((data.view(np.ndarray).ravel(), (row, col)), shape=(other.shape[0], self.shape[1]), copy=False)
        elif self.shape[1] == 1:
            if other.shape[0] == 1:
            ...
            return self._coo_container((data.view(np.ndarray).ravel(), (row, col)), shape=(self.shape[0], other.shape[1]), copy=False)
        elif other.shape[0] == 1 and self.shape[1] == other.shape[1]:
    ...
    def _mul_sparse_matrix(self, other):
        ...
        nnz = fn(M, N, np.asarray(self.indptr, dtype=idx_dtype), np.asarray(self.indices, dtype=idx_dtype), np.asarray(other.indptr, dtype=idx_dtype), np.asarray(other.indices, dtype=idx_dtype))
        idx_dtype = self._get_index_dtype((self.indptr, self.indices, other.indptr, other.indices), maxval=nnz)
    ...
    def _maximum_minimum(self, other, npop, op_name, dense_check):
        if isscalarlike(other):
            if dense_check(other):
                warn('Taking maximum (minimum) with > 0 (< 0) number results to a dense matrix.', SparseEfficiencyWarning, stacklevel=3)
    ...
    def _get_intXint(self, row, col):
        ...
        indptr, indices, data = get_csr_submatrix(M, N, self.indptr, self.indices, self.data, major, major + 1, minor, minor + 1)
    ...
    def _set_many(self, i, j, x):
        ...
        if -1 not in offsets:
            ...
            warn('Changing the sparsity structure of a {}_matrix is expensive. lil_matrix is more efficient.'.format(self.format), SparseEfficiencyWarning, stacklevel=3)
            mask = offsets > -1
    ...
    def has_canonical_format(self) -> bool:
        ...
        if not getattr(self, '_has_sorted_indices', True):
            ...
            self.has_canonical_format = bool(_sparsetools.csr_has_canonical_format(len(self.indptr) - 1, self.indptr, self.indices))
    ...
    def has_sorted_indices(self) -> bool:
        ...
        if not hasattr(self, '_has_sorted_indices'):
            self._has_sorted_indices = bool(_sparsetools.csr_has_sorted_indices(len(self.indptr) - 1, self.indptr, self.indices))
    ...
    def resize(self, *shape):
        ...
        if hasattr(self, 'blocksize'):
            ...
            if rm or rn:
                raise ValueError('shape must be divisible into {} blocks. Got {}'.format(self.blocksize, shape))
