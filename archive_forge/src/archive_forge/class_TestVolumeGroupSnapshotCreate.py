...
class TestVolumeGroupSnapshotCreate(volume_fakes.TestVolume):
    ...
    columns = ('ID', 'Status', 'Name', 'Description', 'Group', 'Group Type')
    data = (fake_volume_group_snapshot.id, fake_volume_group_snapshot.status, fake_volume_group_snapshot.name, fake_volume_group_snapshot.description, fake_volume_group_snapshot.group_id, fake_volume_group_snapshot.group_type_id)

    def setUp(self):
        super().setUp()
        self.volume_sdk_client.find_group.return_value = self.fake_volume_group
        self.volume_sdk_client.create_group_snapshot.return_value = self.fake_volume_group_snapshot
        self.volume_sdk_client.find_group_snapshot.return_value = self.fake_volume_group_snapshot
        self.cmd = volume_group_snapshot.CreateVolumeGroupSnapshot(self.app, None)
    ...
    def test_volume_group_snapshot_create_with_options(self, mock_mv):
        ...
        verifylist = [('volume_group', self.fake_volume_group.id), ('name', 'foo'), ('description', 'hello, world')]
        parsed_args = self.check_parser(self.cmd, arglist, verifylist)
        columns, data = self.cmd.take_action(parsed_args)
        self.volume_sdk_client.find_group.assert_called_once_with(self.fake_volume_group.id, ignore_missing=False, details=False)
        self.volume_sdk_client.create_group_snapshot.assert_called_once_with(group_id=self.fake_volume_group.id, name='foo', description='hello, world')
        self.assertEqual(self.columns, columns)
        self.assertCountEqual(self.data, data)
