...
def conv_transpose(input, filters, output_shape, strides, padding='SAME', data_format=None, dilations=None, name=None):
  ...
  """
    with ops.name_scope(name, 'conv_transpose', [input, filter, output_shape]) as name:
        if tensor_util.is_tf_type(output_shape):
            ...
            raise ValueError(f'`output_shape` must be a tensor or sized collection. Received: output_shape={output_shape}')
        if not 1 <= n <= 3:
            raise ValueError(f'`output_shape` must be of length 3, 4 or 5. Received: output_shape={output_shape} of length {n + 2}.')
        op = CONV_TRANSPOSE_OPS[n - 1]
        return op(input, filters, output_shape, strides, padding=padding, data_format=data_format, dilations=dilations, name=name)