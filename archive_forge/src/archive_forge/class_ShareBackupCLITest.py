...
class ShareBackupCLITest(base.OSCClientTestBase):
    ...
    def test_share_backup_create(self):
        ...
        backup = self.create_backup(share_id=share['id'], name='test_backup_create', description='Description', backup_options={'dummy': True})
        backup = json.loads(self.openstack(f'share backup show -f json {backup['id']}'))
    ...
    def test_share_backup_delete(self):
        share = self.create_share()
        backup = self.create_backup(share_id=share['id'], backup_options={'dummy': True}, add_cleanup=False)
    ...
    def test_share_backup_show(self):
        share = self.create_share()
        backup = self.create_backup(share_id=share['id'], name='test_backup_show', description='Description', backup_options={'dummy': True})
    ...
    def test_share_backup_list(self):
        ...
        backup_1 = self.create_backup(share_id=share_1['id'], backup_options={'dummy': True})
        backup_2 = self.create_backup(share_id=share_2['id'], backup_options={'dummy': True})
        backups_list = self.listing_result('share backup', f'list --name {backup_2['name']} ')
        self.assertTableStruct(backups_list, ['ID', 'Name', 'Share ID', 'Status'])
        self.assertEqual(1, len(backups_list))
        self.assertIn(backup_2['id'], [item['ID'] for item in backups_list])
        backups_list = self.listing_result('share backup', f'list --share {share_1['id']} --detail')
        self.assertTableStruct(backups_list, ['ID', 'Name', 'Share ID', 'Status', 'Description', 'Availability Zone', 'Created At', 'Updated At', 'Size', 'Progress', 'Restore Progress', 'Host', 'Topic'])
        self.assertEqual(1, len(backups_list))
