...
class GradientTape:
  ...
  """
    ...
    """
        if self._tape is None:
            raise RuntimeError('A non-persistent GradientTape can only be used to compute one set of gradients (or jacobians)')
        if self._recording:
        ...
        if target is None:
            raise TypeError('Argument `target` should be a list or nested structure of Tensors, Variables or CompositeTensors to be differentiated, but received None.')
        flat_targets = composite_tensor_gradient.get_flat_tensors_for_gradients(nest.flatten(target))
        flat_targets = nest.map_structure(_handle_or_self, flat_targets)
        for t in flat_targets:
            if not backprop_util.IsTrainable(t):
                logging.vlog(1, 'The dtype of the target tensor must be floating (e.g. tf.float32) when calling GradientTape.gradient, got %r', t.dtype)
        flat_sources_raw = nest.flatten(sources)
        flat_sources = []
        for t in flat_sources_raw:
            flat_sources.append(_handle_or_self(t))
        flat_sources = composite_tensor_gradient.get_flat_tensors_for_gradients(flat_sources)
        for t in flat_sources:
            if not backprop_util.IsTrainable(t):
                logging.vlog(1, 'The dtype of the source tensor must be floating (e.g. tf.float32) when calling GradientTape.gradient, got %r', t.dtype)
            if getattr(t, 'is_packed', False):
                raise ValueError('GradientTape.gradient is not supported on packed EagerTensors yet.')
        if output_gradients is not None:
            output_gradients = nest.flatten(variable_utils.convert_variables_to_tensors(output_gradients))
            output_gradients = composite_tensor_gradient.get_flat_tensors_for_gradients(output_gradients)
            output_gradients = [None if x is None else ops.convert_to_tensor(x) for x in output_gradients]
        flat_grad = imperative_grad.imperative_grad(self._tape, flat_targets, flat_sources, output_gradients=output_gradients, sources_raw=flat_sources_raw, unconnected_gradients=unconnected_gradients)
        if not self._persistent:
            self._watched_variables = self._tape.watched_variables()
            self._tape = None
        flat_sources_raw = nest.map_structure(_handle_or_self, flat_sources_raw)
        flat_grad = composite_tensor_gradient.replace_flat_tensors_for_gradients(flat_sources_raw, flat_grad)
        grad = nest.pack_sequence_as(sources, flat_grad)
        return grad

    def jacobian(self, target, sources, unconnected_gradients=UnconnectedGradients.NONE, parallel_iterations=None, experimental_use_pfor=True):
    ...
    """
        ...
        def loop_fn(i):
            ...
            return self.gradient(y, flat_sources, unconnected_gradients=unconnected_gradients)
        try:
        ...
        if experimental_use_pfor:
            try:
                output = pfor_ops.pfor(loop_fn, target_size, parallel_iterations=parallel_iterations)
            except ValueError as err:
                raise ValueError('Encountered an exception while vectorizing the jacobian computation. Vectorization can be disabled by setting experimental_use_pfor to False.') from err
        else:
            if context.executing_eagerly() and (not self._persistent):
                raise RuntimeError('GradientTape must be created with persistent=True to compute the jacobian with eager execution enabled and with  experimental_use_pfor set to False.')
            output = pfor_ops.for_loop(loop_fn, [target.dtype] * len(flat_sources), target_size, parallel_iterations=parallel_iterations)
        for i, out in enumerate(output):
            if out is not None:
                new_shape = array_ops.concat([target_shape, array_ops.shape(out)[1:]], axis=0)
    ...
    """
        if self._tape is None:
            raise RuntimeError('A non-persistent GradientTape can only be used tocompute one set of gradients (or jacobians)')
        ...
        def loop_fn(i):
            ...
            if run_once and (not self._persistent):
                if parallel_iterations is not None:
                    raise RuntimeError('GradientTape must be created with persistent=True to compute the batch_jacobian with parallel_iterations.')
                else:
                    raise RuntimeError('GradientTape must be created with persistent=True to compute the batch_jacobian.')
            run_once = True
            with self._ensure_recording():
        ...
        if experimental_use_pfor:
            try:
                ...
                raise ValueError('Encountered an exception while vectorizing the batch_jacobian computation. Vectorization can be disabled by setting experimental_use_pfor to False.') from err
        else:
            if context.executing_eagerly() and (not self._persistent):
