...
class PromptTemplate(StringPromptTemplate):
    ...
    *Security warning*: Prefer using `template_format="f-string"` instead of
    ...
    @classmethod
    def from_file(cls, template_file: Union[str, Path], input_variables: Optional[List[str]]=None, **kwargs: Any) -> PromptTemplate:
        ...
        with open(str(template_file), 'r') as f:
            template = f.read()
        if input_variables:
            warnings.warn("`input_variables' is deprecated and ignored.", DeprecationWarning)
    ...
    def from_template(cls, template: str, *, template_format: str='f-string', partial_variables: Optional[Dict[str, Any]]=None, **kwargs: Any) -> PromptTemplate:
        ...
        *Security warning*: Prefer using `template_format="f-string"` instead of
        ...
        if _partial_variables:
            input_variables = [var for var in input_variables if var not in _partial_variables]
        return cls(input_variables=input_variables, template=template, template_format=template_format, partial_variables=_partial_variables, **kwargs)