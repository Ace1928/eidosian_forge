...
class OntotextGraphDBQAChain(Chain):
    ...
    *Security note*: Make sure that the database connection uses credentials
    ...
    @classmethod
    def from_llm(cls, llm: BaseLanguageModel, *, sparql_generation_prompt: BasePromptTemplate=GRAPHDB_SPARQL_GENERATION_PROMPT, sparql_fix_prompt: BasePromptTemplate=GRAPHDB_SPARQL_FIX_PROMPT, max_fix_retries: int=5, qa_prompt: BasePromptTemplate=GRAPHDB_QA_PROMPT, **kwargs: Any) -> OntotextGraphDBQAChain:
        """Initialize from LLM."""
        ...
        return cls(qa_chain=qa_chain, sparql_generation_chain=sparql_generation_chain, sparql_fix_chain=sparql_fix_chain, max_fix_retries=max_fix_retries, **kwargs)
    ...
    def _call(self, inputs: Dict[str, Any], run_manager: Optional[CallbackManagerForChainRun]=None) -> Dict[str, str]:
        ...
        sparql_generation_chain_result = self.sparql_generation_chain.invoke({'prompt': prompt, 'schema': ontology_schema}, callbacks=callbacks)
