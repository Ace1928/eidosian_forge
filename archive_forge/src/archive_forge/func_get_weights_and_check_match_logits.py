...
def get_weights_and_check_match_logits(features, weight_column, logits, allow_per_logit_weights=False):
  ...
  * [D0, D1, ... DN]: In this case, weights is reshaped into
  ...
  """
    ...
    with ops.name_scope('weights', values=tuple(six.itervalues(features)) + (logits,)) as scope:
        if weight_column is None:
            return 1.0
        if isinstance(weight_column, six.string_types):
            weight_column = tf.feature_column.numeric_column(key=weight_column, shape=(1,))
        if not isinstance(weight_column, (feature_column_lib.NumericColumn, _NumericColumn)):
            raise TypeError('Weight column must be either a string or NumericColumn. Given type: {}.'.format(type(weight_column)))
        weights = weight_column._get_dense_tensor(_LazyBuilder(features))
        ...
        if tf.executing_eagerly():
            ...
            if weights_rank is not None and logits_rank is not None and (weights_rank == logits_rank - 1):
                if logits_shape[:-1] != weights_shape:
                    raise ValueError('{}, logits_shape: {}. weights_shape: {}.'.format(err_msg, logits_shape, weights_shape))
                return tf.compat.v1.expand_dims(weights, -1, name=scope)
            supported_weights_shape = logits_shape[:-1] + (1,)
            if allow_per_logit_weights:
                if logits_shape != weights_shape and supported_weights_shape != weights_shape:
                    raise ValueError('{}, logits_shape: {}. weights_shape: {}.'.format(err_msg, logits_shape, weights_shape))
            elif supported_weights_shape != weights_shape:
                raise ValueError('{}, logits_shape: {}. weights_shape: {}.'.format(err_msg, logits_shape, weights_shape))
            return weights
