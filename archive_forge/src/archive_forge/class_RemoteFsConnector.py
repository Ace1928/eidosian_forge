...
class RemoteFsConnector(base.BaseLinuxConnector):
    ...
    def __init__(self, mount_type, root_helper, driver=None, execute=None, device_scan_attempts=initiator.DEVICE_SCAN_ATTEMPTS_DEFAULT, *args, **kwargs):
        kwargs = kwargs or {}
        conn = kwargs.get('conn')
        mount_type_lower = mount_type.lower()
        if conn:
            mount_point_base = conn.get('mount_point_base')
            if mount_type_lower in ('nfs', 'glusterfs', 'scality', 'quobyte', 'vzstorage'):
                kwargs[mount_type_lower + '_mount_point_base'] = kwargs.get(mount_type_lower + '_mount_point_base') or mount_point_base
        else:
            LOG.warning('Connection details not present. RemoteFsClient may not initialize properly.')
        cls: Any
        ...
        self._remotefsclient = cls(mount_type, root_helper, *args, execute=execute, **kwargs)
        super(RemoteFsConnector, self).__init__(root_helper, *args, driver=driver, execute=execute, device_scan_attempts=device_scan_attempts, **kwargs)
    ...
    @utils.trace
    def connect_volume(self, connection_properties: dict[str, Any]) -> dict[str, Any]:
