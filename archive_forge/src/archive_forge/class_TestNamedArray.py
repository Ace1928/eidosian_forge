...
class TestNamedArray(NamedArraySubclassobjects):
    ...
    @pytest.mark.parametrize('expected', [np.array([1, 2], dtype=np.dtype(np.int8)), pytest.param([1, 2], marks=pytest.mark.xfail(reason='NamedArray only supports array-like objects'))])
    ...
    def test_from_array(self, dims: _DimsLike, data: ArrayLike, expected: np.ndarray[Any, Any], raise_error: bool) -> None:
    ...
    @pytest.mark.parametrize('dim,expected_ndim,expected_shape,expected_dims', [(None, 3, (1, 2, 5), (None, 'x', 'y')), (_default, 3, (1, 2, 5), ('dim_2', 'x', 'y')), ('z', 3, (1, 2, 5), ('z', 'x', 'y'))])
    def test_expand_dims(self, target: NamedArray[Any, np.dtype[np.float32]], dim: _Dim | Default, expected_ndim: int, expected_shape: _ShapeLike, expected_dims: _DimsLike) -> None:
    ...
    @pytest.mark.parametrize('broadcast_dims,expected_ndim', [({'x': 2, 'y': 5}, 2), ({'x': 2, 'y': 5, 'z': 2}, 3), ({'w': 1, 'x': 2, 'y': 5}, 3)])
    def test_broadcast_to(self, target: NamedArray[Any, np.dtype[np.float32]], broadcast_dims: Mapping[_Dim, int], expected_ndim: int) -> None:
