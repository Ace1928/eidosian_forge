...
class MultiQueryPastKeyValuesGenerator(DummyPastKeyValuesGenerator):
    ...
    def generate(self, input_name: str, framework: str='pt', int_dtype: str='int64', float_dtype: str='fp32'):
        past_shape = (self.batch_size * self.num_kv_heads, self.sequence_length, self.hidden_size // self.num_attention_heads)
        return [(self.random_float_tensor(past_shape, framework=framework, dtype=float_dtype), self.random_float_tensor(past_shape, framework=framework, dtype=float_dtype)) for _ in range(self.num_layers)]