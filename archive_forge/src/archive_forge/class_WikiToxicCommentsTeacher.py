...
class WikiToxicCommentsTeacher(FixedDialogTeacher):
    ...
    def add_cmdline_args(parser):
        ...
        parser.add_argument('--use-test-set', type='bool', default=False, help='Defaults to 80/10/10 train/test/valid split of train set. Else, will partition train data into 90/10 train/valid and use the test set as is.')
        parser.add_argument('--balance-data', type='bool', default=False, help='Balances the data so there are equal numbers of OK and NOT OK training data')
        return parser
    ...
    def __init__(self, opt, shared=None):
        ...
        self.data_path = os.path.join(opt['datapath'], 'dialogue_safety', 'wiki-toxic-comments')
    ...
    def build(self, opt):
        ...
        if not build_data.built(self.data_path, version):
            ...
            test_labels = test_labels[(test_labels.toxic != -1) & (test_labels.severe_toxic != -1) & (test_labels.obscene != -1) & (test_labels.threat != -1) & (test_labels.insult != -1) & (test_labels.identity_hate != -1)]
            ...
            total_data['sensitive'] = total_data['severe_toxic'] + total_data['toxic'] + total_data['obscene'] + total_data['threat'] + total_data['insult'] + total_data['identity_hate']
            ...
            original_train = total_data[(total_data['data_type'] == 'valid') | (total_data['data_type'] == 'train')].copy()
