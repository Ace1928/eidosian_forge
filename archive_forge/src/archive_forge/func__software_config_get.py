...
def _software_config_get(context, config_id):
    ...
    if result is not None and context is not None and (not context.is_admin) and (result.tenant != context.tenant_id):
        result = None
    if not result:
        raise exception.NotFound(_('Software config with id %s not found') % config_id)
    return result