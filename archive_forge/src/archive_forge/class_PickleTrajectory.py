...
class PickleTrajectory:
    ...
    def __init__(self, filename, mode='r', atoms=None, master=None, backup=True, _warn=True):
    ...
    def _write_atoms(self, atoms):
        ...
        if atoms.has('momenta'):
            ...
            momenta = None
        d = {'positions': atoms.get_positions(), 'cell': atoms.get_cell(), 'momenta': momenta}
    ...
    def write_header(self, atoms):
        ...
        if atoms.has('masses'):
            ...
            masses = None
        d = {'version': 3, 'pbc': atoms.get_pbc(), 'numbers': atoms.get_atomic_numbers(), 'tags': tags, 'masses': masses, 'constraints': [], 'constraints_string': pickle.dumps(atoms.constraints, protocol=0)}
        pickle.dump(d, self.fd, protocol=2)
    ...
    def __getitem__(self, i=-1):
        ...
        if 0 <= i < N:
            ...
            atoms = Atoms(positions=d['positions'], numbers=self.numbers, cell=d['cell'], momenta=d['momenta'], magmoms=magmoms, charges=charges, tags=self.tags, masses=self.masses, pbc=self.pbc, info=unstringnify_info(d.get('info', {})), constraint=constraints)
            if 'energy' in d:
                calc = SinglePointCalculator(atoms, energy=d.get('energy', None), forces=d.get('forces', None), stress=d.get('stress', None), magmoms=magmoms)
                atoms.calc = calc
            return atoms
        if i >= N:
