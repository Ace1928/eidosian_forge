...
class SyncVectorEnv(VectorEnv):
    ...
    def __init__(self, env_fns: Iterator[Callable[[], Env]], observation_space: Space=None, action_space: Space=None, copy: bool=True):
        """Vectorized environment that serially runs multiple environments.
        ...
        super().__init__(num_envs=len(self.envs), observation_space=observation_space, action_space=action_space)
        self._check_spaces()
    ...
    def step_wait(self):
        ...
        for i, (env, action) in enumerate(zip(self.envs, self._actions)):
            observation, self._rewards[i], self._terminateds[i], self._truncateds[i], info = env.step(action)
            ...
            infos = self._add_info(infos, info, i)
        self.observations = concatenate(self.single_observation_space, observations, self.observations)
        return (deepcopy(self.observations) if self.copy else self.observations, np.copy(self._rewards), np.copy(self._terminateds), np.copy(self._truncateds), infos)
