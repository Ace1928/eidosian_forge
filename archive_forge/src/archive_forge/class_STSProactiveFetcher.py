...
class STSProactiveFetcher:
    ...
    def __init__(self, cfg, loop, cache):
        ...
        self._resolver = STSResolver(loop=loop, timeout=cfg['default_zone']['timeout'])
    ...
    async def iterate_domains(self):
        self._logger.info('Proactive policy fetching for all domains in cache started...')
        domain_processors = []
        ...
        self._logger.info('Proactive policy fetching for all domains in cache finished.')

    async def fetch_periodically(self):
        while True:
            next_fetch_ts = await self._cache.get_proactive_fetch_ts() + self._pf_interval
            sleep_duration = max(constants.MIN_PROACTIVE_FETCH_INTERVAL, next_fetch_ts - time.time() + 1)
