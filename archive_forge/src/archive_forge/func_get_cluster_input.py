...
def get_cluster_input():
    distributed_type = _ask_options('Which type of machine are you using?', ['No distributed training', 'multi-CPU', 'multi-XPU', 'multi-GPU', 'multi-NPU', 'TPU'], _convert_distributed_mode)
    machine_rank = 0
    num_machines = 1
    num_processes = 1
    gpu_ids = None
    ...
    same_network = True
    debug = False
    if distributed_type in [DistributedType.MULTI_GPU, DistributedType.MULTI_NPU, DistributedType.MULTI_XPU, DistributedType.MULTI_CPU]:
    ...
    if not use_cpu and is_xpu_available() and (distributed_type not in [DistributedType.MULTI_GPU, DistributedType.MULTI_NPU, DistributedType.XLA]):
    ...
    if distributed_type in [DistributedType.MULTI_CPU, DistributedType.MULTI_XPU, DistributedType.MULTI_GPU, DistributedType.MULTI_NPU, DistributedType.XLA]:
    ...
    if distributed_type in [DistributedType.MULTI_GPU, DistributedType.MULTI_NPU, DistributedType.MULTI_XPU, DistributedType.NO] and (not use_cpu) and (not use_mps):
        if is_npu_available():
            machine_type = 'NPU(s)'
        else:
            machine_type = 'GPU(s)'
