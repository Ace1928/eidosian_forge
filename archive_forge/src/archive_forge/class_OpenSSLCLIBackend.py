...
class OpenSSLCLIBackend(CryptoBackend):
    ...
    def parse_key(self, key_file=None, key_content=None, passphrase=None):
        ...
        dummy, out, dummy = self.module.run_command(openssl_keydump_cmd, check_rc=True, environ_update=_OPENSSL_ENVIRONMENT_UPDATE)
        if account_key_type == 'rsa':
            ...
            return {'key_file': key_file, 'type': 'rsa', 'alg': 'RS256', 'jwk': {'kty': 'RSA', 'e': nopad_b64(binascii.unhexlify(pub_exp.encode('utf-8'))), 'n': nopad_b64(binascii.unhexlify(re.sub('(\\s|:)', '', pub_hex).encode('utf-8')))}, 'hash': 'sha256'}
            ...
            return {'key_file': key_file, 'type': 'ec', 'alg': alg, 'jwk': {'kty': 'EC', 'crv': curve, 'x': nopad_b64(pub_hex[:num_bytes]), 'y': nopad_b64(pub_hex[num_bytes:])}, 'hash': hashalg, 'point_size': point_size}
    ...
    def sign(self, payload64, protected64, key_data):
        ...
        dummy, out, dummy = self.module.run_command(openssl_sign_cmd, data=sign_payload, check_rc=True, binary_data=True, environ_update=_OPENSSL_ENVIRONMENT_UPDATE)
        if key_data['type'] == 'ec':
            dummy, der_out, dummy = self.module.run_command([self.openssl_binary, 'asn1parse', '-inform', 'DER'], data=out, binary_data=True, environ_update=_OPENSSL_ENVIRONMENT_UPDATE)
        ...
        return {'protected': protected64, 'payload': payload64, 'signature': nopad_b64(to_bytes(out))}
