...
class GlpsolMIPConverter(object):
    ...
    def apply(self, *args, **kwargs):
        ...
        cmd = [_exec.path(), '--math']
        modfile = ''
        if args[1] == ProblemFormat.mps:
            ofile = TempfileManager.create_tempfile(suffix='.glpsol.mps')
            cmd.extend(['--check', '--name', 'MPS model derived from ' + os.path.basename(args[2]), '--wfreemps', ofile])
        elif args[1] == ProblemFormat.cpxlp:
            ofile = TempfileManager.create_tempfile(suffix='.glpsol.lp')
            cmd.extend(['--check', '--name', 'MPS model derived from ' + os.path.basename(args[2]), '--wcpxlp', ofile])
        if len(args[2:]) == 1:
            cmd.append(args[2])
        else:
