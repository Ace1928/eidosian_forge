...
class ChatMlflow(BaseChatModel):
    ...
    def __init__(self, **kwargs: Any):
        ...
        try:
            ...
            raise ImportError(f'Failed to create the client. Please run `pip install mlflow{self._mlflow_extras}` to install required dependencies.') from e
    ...
    def _default_params(self) -> Dict[str, Any]:
        params: Dict[str, Any] = {'target_uri': self.target_uri, 'endpoint': self.endpoint, 'temperature': self.temperature, 'n': self.n, 'stop': self.stop, 'max_tokens': self.max_tokens, 'extra_params': self.extra_params}
        return params

    def _prepare_inputs(self, messages: List[BaseMessage], stop: Optional[List[str]]=None, **kwargs: Any) -> Dict[str, Any]:
        message_dicts = [ChatMlflow._convert_message_to_dict(message) for message in messages]
        data: Dict[str, Any] = {'messages': message_dicts, 'temperature': self.temperature, 'n': self.n, **self.extra_params, **kwargs}
    ...
    def _get_invocation_params(self, stop: Optional[List[str]]=None, **kwargs: Any) -> Dict[str, Any]:
        ...
        return {**self._default_params, **super()._get_invocation_params(stop=stop, **kwargs)}
    ...
    def _raise_functions_not_supported() -> None:
        raise ValueError('Function messages are not supported by Databricks. Please create a feature request at https://github.com/mlflow/mlflow/issues.')
    ...
    def _convert_message_to_dict(message: BaseMessage) -> dict:
        ...
        if message.additional_kwargs:
            logger.warning('Additional message arguments are unsupported by Databricks and will be ignored: %s', message.additional_kwargs)
        return message_dict
