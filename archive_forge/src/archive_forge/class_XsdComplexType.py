...
class XsdComplexType(XsdType, ValidationMixin[Union[ElementType, str, bytes], Any]):
    ...
    content: Union[XsdGroup, XsdSimpleType] = None
    abstract: bool = False
    mixed: bool = False
    ...
    def __init__(self, elem: ElementType, schema: SchemaType, parent: Optional[XsdComponent]=None, name: Optional[str]=None, **kwargs: Any) -> None:
        if kwargs:
    ...
    def __repr__(self) -> str:
        if self.name is not None:
            ...
            return '%s(content=%r, attributes=%r)' % (self.__class__.__name__, self.content_type_label, [a if a.name is None else a.prefixed_name for a in self.attributes.values()])
            ...
            if 'mixed' in content_elem.attrib:
                ...
                if mixed is not self.mixed:
                    ...
                    if 'mixed' in self.elem.attrib and self.xsd_version == '1.1':
                        msg = _("value of 'mixed' attribute in complexType and complexContent must be the same")
                        self.parse_error(msg)
            derivation_elem = self._parse_derivation_elem(content_elem)
            if derivation_elem is None:
                return
            ...
            if content_elem is self.elem[-1]:
                ...
                for index, child in enumerate(self.elem):
                    if content_elem is not child:
                        ...
                        self.content = self.schema.xsd_group_class(self.elem[index + 1], self.schema, self)
                    else:
                        self.content = self.schema.create_empty_content_group(self)
                    break
            self._parse_content_tail(self.elem)
        ...
        if base_type.is_empty():
            if not base_type.mixed:
                if group_elem is not None and group_elem.tag in XSD_MODEL_GROUP_TAGS:
                    ...
                    self.content = self.schema.create_empty_content_group(parent=self, elem=base_type.content.elem)
            else:
                self.content = self.schema.create_empty_content_group(self)
    ...
    def validate(self, obj: Union[ElementType, str, bytes], use_defaults: bool=True, namespaces: Optional[NamespacesType]=None, max_depth: Optional[int]=None, extra_validator: Optional[ExtraValidatorType]=None) -> None:
        kwargs: Any = {'use_defaults': use_defaults, 'namespaces': namespaces, 'max_depth': max_depth, 'extra_validator': extra_validator}
    ...
    def is_valid(self, obj: Union[ElementType, str, bytes], use_defaults: bool=True, namespaces: Optional[NamespacesType]=None, max_depth: Optional[int]=None, extra_validator: Optional[ExtraValidatorType]=None) -> bool:
        kwargs: Any = {'use_defaults': use_defaults, 'namespaces': namespaces, 'max_depth': max_depth, 'extra_validator': extra_validator}
        if not isinstance(obj, (str, bytes)):
            ...
            return self.mixed or (self.base_type is not None and self.base_type.is_valid(obj, **kwargs))
    ...
    def is_derived(self, other: Union[BaseXsdType, Tuple[ElementType, SchemaType]], derivation: Optional[str]=None) -> bool:
        ...
        if self is other:
            ...
            if not self.has_simple_content():
                return False
            return isinstance(self.content, XsdSimpleType) and self.content.is_derived(other, derivation)
        elif self.has_simple_content():
            return isinstance(self.content, XsdSimpleType) and self.content.is_derived(other, derivation) or self.base_type.is_derived(other, derivation)
        else:
    ...
    def decode(self, obj: Union[ElementType, str, bytes], *args: Any, **kwargs: Any) -> DecodeType[Any]:
        if not isinstance(obj, (str, bytes)):
            ...
            raise XMLSchemaDecodeError(self, obj, str, msg % {'obj': obj, 'decoder': self})

    def iter_decode(self, obj: Union[ElementType, str, bytes], validation: str='lax', **kwargs: Any) -> IterDecodeType[Any]:
        """
