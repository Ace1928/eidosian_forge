...
# Utility function to adaptively run asynchronous coroutines
async def run_async_coroutine(coroutine):
    """
    Runs a coroutine, adapting based on the current event loop state.

    Args:
        coroutine: The coroutine to be run.

    Returns:
        The result of the coroutine if the event loop is not running; otherwise, schedules the coroutine as a task.
    """
    try:
        loop = asyncio.get_running_loop()
        if loop.is_running():
            # Schedule the coroutine as a task if the loop is already running
            return asyncio.create_task(coroutine)
        else:
            # Run the coroutine directly if no event loop is running
            return await coroutine
    except RuntimeError as e:
        logging.error(f"Failed to run coroutine: {e}")
        raise
...
class AsyncFileHandler(logging.Handler):
    ...
    def __init__(self, filename: str, mode: str = "a", loop=None):
        ...
        Args:
            filename: The name of the file to which logs are written.
            mode: The mode in which the file is opened.
            loop: The asyncio event loop. If not provided, the current running loop is used.
        """
        super().__init__()
        self.filename = filename
        self.mode = mode
    ...
    def emit(self, record):
        ...
        """
        msg = self.format(record)
        asyncio.create_task(self.aio_write(msg + "\n"), loop=self.loop)
...
class ResourceProfiler:
    ...
    def __init__(self, interval: int = 60, output: str = "resource_usage.log"):
        ...
        Args:
            interval: The interval (in seconds) at which to log resource usage.
            output: The file path for resource profiling logs.
        """
        self.interval = interval
        self.output = output
        self.running = False
    ...
    async def _log_resource_usage(self):
        ...
        memory_usage = process.memory_percent()
        current, peak = tracemalloc.get_traced_memory()
        async with AIOFile(self.output, "a") as af:
            writer = Writer(af)
            await writer(
                f"Resource Usage - CPU: {cpu_usage}%, Memory: {memory_usage}%, Traced Memory: Current {current} bytes, Peak {peak} bytes\n"
            )
...
class StandardDecorator:
    ...
    def _initialize_resource_profiler(self):
        ...
        This method checks if resource profiling is enabled and, if so, initializes the ResourceProfiler
        with the specified interval and output settings. It then starts the resource profiler asynchronously,
        adapting based on the current event loop state to avoid RuntimeError when an event loop is already running.
        """
        if self.enable_resource_profiling:
            ...
            # Adaptively run the resource profiler start coroutine based on the event loop state
            self._run_async_coroutine(self.resource_profiler.start())
    ...
    def _run_async_coroutine(self, coroutine):
        """
        Runs an asynchronous coroutine, adapting based on the current event loop state.
        This method checks if the current event loop is running and schedules the coroutine accordingly
        to avoid RuntimeError. If no event loop is running, it creates a new event loop to run the coroutine.

        Args:
            coroutine (Coroutine): The asynchronous coroutine to be run.
        """
        try:
            loop = asyncio.get_running_loop()
            if loop.is_running():
            ...
            # Create a new event loop and run the coroutine if no event loop is running
    ...
    async def _graceful_shutdown(self, signum, frame):
        ...
        This method logs the received signal and initiates a graceful shutdown process. It saves the cache to a file
        if caching is enabled and performs additional cleanup actions. Finally, it cancels all outstanding tasks and
        stops the asyncio event loop, ensuring a clean shutdown.
    ...
    async def _cancel_outstanding_tasks(self):
        """
        Cancels all outstanding tasks in the current event loop and stops the loop.
        This method retrieves all tasks in the current event loop, cancels them, and then gathers
        them to ensure they are properly handled. It logs the cancellation of tasks and the successful
