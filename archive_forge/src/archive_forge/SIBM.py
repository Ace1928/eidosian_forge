...
c_analysis.execute(
    """CREATE TABLE IF NOT EXISTS analysis (hash TEXT PRIMARY KEY, word_count INTEGER, most_common_words TEXT)"""
)
...
def analyze_text(text: str, file_hash: str) -> None:
    ...
    c_analysis.execute(
        "INSERT OR IGNORE INTO analysis (hash, word_count, most_common_words) VALUES (?, ?, ?)",
        (file_hash, word_count, most_common_words),
    )
    conn_analysis.commit()
