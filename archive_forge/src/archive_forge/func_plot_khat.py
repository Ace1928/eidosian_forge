...
def plot_khat(hover_label, hover_format, ax, figsize, xdata, khats, kwargs, threshold, coord_labels, show_hlines, show_bins, hlines_kwargs, xlabels, legend, color, dims, textsize, markersize, n_data_points, bin_format, backend_kwargs, show):
    """Bokeh khat plot."""
    ...
    if not isinstance(rgba_c, str) and isinstance(rgba_c, Iterable):
        for idx, (alpha, rgba_c_) in enumerate(zip(alphas, rgba_c)):
            ax.cross(xdata[idx], khats[idx], line_color=rgba_c_, fill_color=rgba_c_, line_alpha=alpha, fill_alpha=alpha, size=10)
    else:
        ax.cross(xdata, khats, line_color=rgba_c, fill_color=rgba_c, size=10, line_alpha=alphas, fill_alpha=alphas)
    ...
    if show_hlines:
        for hline in hlines_kwargs.pop('hlines'):
            _hline = Span(location=hline, dimension='width', line_color='grey', line_width=line_width, line_dash='dashed')
    ...
    if show_bins:
        ...
        for idx, count in enumerate(hist):
            ax.text(x=[(n_data_points - 1 + xmax) / 2], y=[np.mean(bin_edges[idx:idx + 2])], text=[bin_format.format(count, count / n_data_points * 100)])
        ax.x_range._property_values['end'] = xmax + 1
    ax.xaxis.axis_label = 'Data Point'
    ax.yaxis.axis_label = 'Shape parameter k'
    if ymin > 0:
        ax.y_range._property_values['start'] = -0.02
    if ymax < 1:
        ax.y_range._property_values['end'] = 1.02
    elif ymax > 1 & threshold:
        ax.y_range._property_values['end'] = 1.1 * ymax
    show_layout(ax, show)
    return ax