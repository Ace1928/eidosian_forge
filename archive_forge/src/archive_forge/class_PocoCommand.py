...
class PocoCommand:
    ...
    def _membership_paths(self, args: parser_extensions.Namespace) -> Dict[str, str]:
        """Filtered map of short membership names to full membership paths."""
        return {fleet_util.MembershipPartialName(path): path for path in base.ParseMembershipsPlural(args, prompt=True, prompt_cancel=False, autoselect=True)}
