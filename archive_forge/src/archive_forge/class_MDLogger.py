...
class MDLogger(IOContext):
    ...
    def __init__(self, dyn, atoms, logfile, header=True, stress=False, peratom=False, mode='a'):
        ...
        if self.dyn is not None:
            ...
            self.fmt = ''
        if self.peratom:
            self.hdr += '%12s %12s %12s  %6s' % ('Etot/N[eV]', 'Epot/N[eV]', 'Ekin/N[eV]', 'T[K]')
            self.fmt += '%12.4f %12.4f %12.4f  %6.1f'
        else:
            self.hdr += '%12s %12s %12s  %6s' % ('Etot[eV]', 'Epot[eV]', 'Ekin[eV]', 'T[K]')
