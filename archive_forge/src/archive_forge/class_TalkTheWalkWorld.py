...
class TalkTheWalkWorld(MultiAgentDialogWorld):
    ...
    def load_data(self):
        ...
        """
        data_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), self.logs_file)
    ...
    def send_map(self, agent):
        ...
        """
        msg = {'id': 'WORLD_MAP', 'message_id': 'WORLD_MAP', 'text': {'landmarks': self.landmarks, 'target': self.target_location, 'boundaries': [self.min_x, self.min_y, self.max_x, self.max_y]}}
        agent.observe(msg)
    ...
    def timeout(self, agent):
        ...
        for other_agent in self.agents:
            if other_agent.id != agent.id:
                msg = {'id': 'WORLD_PARTNER_TIMEOUT', 'text': '', 'message_id': 'WORLD_PARTNER_TIMEOUT'}
    ...
    def evaluate_location(self):
        ...
        success = self.location[0] == self.target_location[0] and self.location[1] == self.target_location[1]
        if success:
            print('SUCCESS!!')
            self.status = 'success'
            ...
            if self.num_evaluations < 3:
                msg = {'id': 'Noah', 'message_id': 'Noah', 'text': "Unfortunately, the Tourist is not at the target location. You have {} attempt(s) left, and you'll now receive a bonus of {}c upon completion.".format(str(3 - self.num_evaluations), str(40 - self.num_evaluations * 15))}
                for agent in self.agents:
                    agent.observe(msg)
                return False
            else:
    ...
    def save(self):
        ...
        data = {'neighborhood': self.neighborhood, 'start_location': self.start_location, 'target_location': self.target_location, 'location': self.location, 'status': self.status, 'dialog': self.acts, 'landmarks': self.landmarks, 'tourist_worker_id': self.agents[0].worker_id, 'tourist_assignment_id': self.agents[0].assignment_id, 'guide_worker_id': self.agents[1].worker_id, 'guide_assignment_id': self.agents[1].assignment_id, 'boundaries': [self.min_x, self.min_y, self.max_x, self.max_y], 'total_time': self.total_time, 'version': 1}
