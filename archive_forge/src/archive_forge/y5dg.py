...
class HarmonicGenerator(SoundModule):
    ...
    def __init__(self, fundamental_frequency: float) -> None:
        ...
        self.overtones: dict = {1: 1.0}  # Initialize with the fundamental frequency only, represented as the first harmonic.
    ...
    def process_sound(self, sound: np.ndarray) -> np.ndarray:
        ...
        if self.modulation_type == "AM":
            ...
            carrier = np.sin(
                ...
                + self.modulation_depth * np.sin(2 * np.pi * self.modulation_frequency * t)
            )
...
class ReverbEffect(SoundModule):
    ...
    def process_sound(self, sound: np.ndarray) -> np.ndarray:
        ...
        reverberated_sound = fftconvolve(sound, impulse_response, mode="full")[:len(sound)]
    ...
    def set_parameter(self, parameter: str, value: float) -> None:
        ...
        if parameter in ["decay", "pre_delay", "mix"]:
            if not (0.0 <= value <= 10.0):
                raise ValueError(f"{parameter.capitalize()} must be between 0.0 and 10.0.")
            setattr(self, parameter, value)
        else:
            raise ValueError(f"Unsupported parameter: {parameter}")
class EchoEffect(SoundModule):
    ...
    def set_parameter(self, parameter: str, value: float) -> None:
        ...
        Raises:
            ValueError: If the specified parameter is not supported or if the value is out of the expected range.
        """
        supported_parameters = {'delay_time': (0.0, 10.0), 'feedback': (0.0, 1.0), 'mix': (0.0, 1.0)}
        if parameter in supported_parameters:
            min_val, max_val = supported_parameters[parameter]
            if not (min_val <= value <= max_val):
                raise ValueError(f"{parameter.capitalize()} must be between {min_val} and {max_val}.")
            setattr(self, parameter, value)
        else:
            raise ValueError(f"Unsupported parameter: {parameter}")
class ChorusEffect(SoundModule):
    ...
    def set_parameter(self, parameter: str, value: float) -> None:
        ...
        """
        supported_parameters = {'rate': (0.01, 10.0), 'depth': (0.0, 1.0), 'mix': (0.0, 1.0)}
        if parameter in supported_parameters:
            min_val, max_val = supported_parameters[parameter]
            if not (min_val <= value <= max_val):
                raise ValueError(f"{parameter.capitalize()} must be between {min_val} and {max_val}.")
...
class StereoPanning(SoundModule):
    """
    Manages the distribution of a sound's signal across a stereo field, transforming a mono sound input into a stereo output by adjusting the balance between the left and right channels based on the pan parameter.

    Methods:
        process_sound(sound: np.ndarray, pan: float) -> np.ndarray:
            Applies stereo panning to the input sound based on the pan parameter, producing a stereo output.
    """
    ...
    def process_sound(self, sound: np.ndarray, pan: float) -> np.ndarray:
        ...
        Parameters:
            ...
            pan (float): The panning position (-1.0 for fully left, 0.0 for center, 1.0 for fully right).

        Returns:
            np.ndarray: The processed sound data with stereo panning applied, in stereo format (2D array).
        ...
        stereo_sound = np.zeros((2, len(sound)))
...
class SampleRateAdjustment(SoundModule):
    """
    Adjusts the sample rate of a digital sound signal. This class inherits from the SoundModule base class and provides
    specific functionality to resample the input sound to a new sample rate, ensuring high fidelity and precision in the
    resampling process.
