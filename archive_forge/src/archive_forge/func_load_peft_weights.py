...
def load_peft_weights(model_id: str, device: Optional[str]=None, **hf_hub_download_kwargs) -> dict:
    ...
    if os.path.exists(os.path.join(path, SAFETENSORS_WEIGHTS_NAME)):
        ...
        hub_filename = os.path.join(hf_hub_download_kwargs['subfolder'], SAFETENSORS_WEIGHTS_NAME) if hf_hub_download_kwargs.get('subfolder', None) is not None else SAFETENSORS_WEIGHTS_NAME
        has_remote_safetensors_file = file_exists(repo_id=model_id, filename=hub_filename, revision=hf_hub_download_kwargs.get('revision', None), repo_type=hf_hub_download_kwargs.get('repo_type', None), token=token)
