...
class SparkNodeProvider(NodeProvider):
    ...
    def __init__(self, provider_config, cluster_name):
        ...
        self.lock = RLock()
        self._nodes = {str(HEAD_NODE_ID): {'tags': {TAG_RAY_NODE_KIND: NODE_KIND_HEAD, TAG_RAY_USER_NODE_TYPE: HEAD_NODE_TYPE, TAG_RAY_NODE_NAME: HEAD_NODE_ID, TAG_RAY_NODE_STATUS: STATUS_UP_TO_DATE}}}
            ...
            for node_id in self._nodes:
                ...
                if status == 'running':
                    if self._nodes[node_id]['tags'][TAG_RAY_NODE_STATUS] == STATUS_SETTING_UP:
                        ...
                        logger.info(f'Spark node provider node {node_id} starts running.')
    ...
    def is_running(self, node_id):
        with self.lock:
            return node_id in self._nodes and self._nodes[node_id]['tags'][TAG_RAY_NODE_STATUS] == STATUS_UP_TO_DATE
    ...
    def create_node_with_resources_and_labels(self, node_config, tags, count, resources, labels):
        for _ in range(count):
            self._create_node_with_resources_and_labels(node_config, tags, resources, labels)
    ...
    def terminate_node(self, node_id):
        if node_id in self._nodes:
            response = requests.post(url=self.spark_job_server_url + '/terminate_node', json={'spark_job_group_id': self._gen_spark_job_group_id(node_id)})
            response.raise_for_status()
        with self.lock:
            if node_id in self._nodes:
