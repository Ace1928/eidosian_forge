...
class CubeInterface(GridInterface):
    ...
    def init(cls, eltype, data, kdims, vdims):
        ...
        if not isinstance(data, iris.cube.Cube):
            ...
            coords = [(iris.coords.DimCoord(data[kd.name], long_name=kd.name, units=kd.unit), ndims - n - 1) for n, kd in enumerate(kdims)]
            try:
    ...
    def assign(cls, dataset, new_data):
        import iris
        value_dims = [d for d, v in new_data.items() if d in dataset.vdims and isinstance(v, iris.cube.Cube)]
        if value_dims:
