...
class NetworkCloudMixin:
    ...
    def create_network(self, name, shared=False, admin_state_up=True, external=False, provider=None, project_id=None, availability_zone_hints=None, port_security_enabled=None, mtu_size=None, dns_domain=None):
        ...
        if availability_zone_hints is not None:
            ...
            if not self._has_neutron_extension('network_availability_zone'):
                raise exc.OpenStackCloudUnavailableExtension('network_availability_zone extension is not available on target cloud')
        ...
        if provider:
            ...
            for attr in ('physical_network', 'network_type', 'segmentation_id'):
                if attr in provider:
                    arg = 'provider:' + attr
    ...
    @_utils.valid_kwargs('name', 'shared', 'admin_state_up', 'external', 'provider', 'mtu_size', 'port_security_enabled', 'dns_domain')
    ...
    @_utils.valid_kwargs('action', 'description', 'destination_firewall_group_id', 'destination_ip_address', 'destination_port', 'enabled', 'ip_version', 'name', 'project_id', 'protocol', 'shared', 'source_firewall_group_id', 'source_ip_address', 'source_port')
    ...
    @_utils.valid_kwargs('audited', 'description', 'firewall_rules', 'name', 'project_id', 'shared')
    ...
    def delete_firewall_policy(self, name_or_id, filters=None):
        ...
        if not filters:
            filters = {}
        try:
            firewall_policy = self.network.find_firewall_policy(name_or_id, ignore_missing=False, **filters)
    ...
    def insert_rule_into_policy(self, name_or_id, rule_name_or_id, insert_after=None, insert_before=None, filters=None):
        ...
        if firewall_rule['id'] in firewall_policy['firewall_rules']:
            self.log.debug('Firewall rule %s already associated with firewall policy %s', rule_name_or_id, name_or_id)
            return firewall_policy
        pos_params = {}
        if insert_after is not None:
            pos_params['insert_after'] = self.network.find_firewall_rule(insert_after, ignore_missing=False)['id']
        if insert_before is not None:
            pos_params['insert_before'] = self.network.find_firewall_rule(insert_before, ignore_missing=False)['id']
        return self.network.insert_rule_into_policy(firewall_policy['id'], firewall_rule['id'], **pos_params)
    ...
    def remove_rule_from_policy(self, name_or_id, rule_name_or_id, filters=None):
        ...
        if firewall_rule['id'] not in firewall_policy['firewall_rules']:
            self.log.debug('Firewall rule %s not associated with firewall policy %s', rule_name_or_id, name_or_id)
            return firewall_policy
        return self.network.remove_rule_from_policy(firewall_policy['id'], firewall_rule['id'])

    @_utils.valid_kwargs('admin_state_up', 'description', 'egress_firewall_policy', 'ingress_firewall_policy', 'name', 'ports', 'project_id', 'shared')
    ...
    def delete_firewall_group(self, name_or_id, filters=None):
        ...
        if not filters:
            filters = {}
        try:
            firewall_group = self.network.find_firewall_group(name_or_id, ignore_missing=False, **filters)
    ...
    def update_qos_policy(self, name_or_id, **kwargs):
        ...
        if not curr_policy:
            raise exceptions.SDKException('QoS policy %s not found.' % name_or_id)
    ...
    def search_qos_bandwidth_limit_rules(self, policy_name_or_id, rule_id=None, filters=None):
    ...
    def create_qos_bandwidth_limit_rule(self, policy_name_or_id, max_kbps, **kwargs):
        ...
        if kwargs.get('direction') is not None:
            if not self._has_neutron_extension('qos-bw-limit-direction'):
                ...
                self.log.debug("'qos-bw-limit-direction' extension is not available on target cloud")
    ...
    def update_qos_bandwidth_limit_rule(self, policy_name_or_id, rule_id, **kwargs):
        ...
        if not curr_rule:
            raise exceptions.SDKException('QoS bandwidth_limit_rule {rule_id} not found in policy {policy_id}'.format(rule_id=rule_id, policy_id=policy['id']))
        return self.network.update_qos_bandwidth_limit_rule(qos_rule=curr_rule, qos_policy=policy, **kwargs)
    ...
    def delete_qos_bandwidth_limit_rule(self, policy_name_or_id, rule_id):
        ...
        try:
            ...
            self.log.debug('QoS bandwidth limit rule {rule_id} not found in policy {policy_id}. Ignoring.'.format(rule_id=rule_id, policy_id=policy['id']))
            return False
        return True
    ...
    def update_qos_dscp_marking_rule(self, policy_name_or_id, rule_id, **kwargs):
        ...
        if not curr_rule:
            raise exceptions.SDKException('QoS dscp_marking_rule {rule_id} not found in policy {policy_id}'.format(rule_id=rule_id, policy_id=policy['id']))
    ...
    def delete_qos_dscp_marking_rule(self, policy_name_or_id, rule_id):
        ...
        try:
            ...
            self.log.debug('QoS DSCP marking rule {rule_id} not found in policy {policy_id}. Ignoring.'.format(rule_id=rule_id, policy_id=policy['id']))
            return False
        return True
    ...
    def create_qos_minimum_bandwidth_rule(self, policy_name_or_id, min_kbps, **kwargs):
    ...
    def update_qos_minimum_bandwidth_rule(self, policy_name_or_id, rule_id, **kwargs):
        ...
        if not curr_rule:
            raise exceptions.SDKException('QoS minimum_bandwidth_rule {rule_id} not found in policy {policy_id}'.format(rule_id=rule_id, policy_id=policy['id']))
    ...
    def delete_qos_minimum_bandwidth_rule(self, policy_name_or_id, rule_id):
        ...
        try:
            ...
            self.log.debug('QoS minimum bandwidth rule {rule_id} not found in policy {policy_id}. Ignoring.'.format(rule_id=rule_id, policy_id=policy['id']))
            return False
        return True
    ...
    def create_router(self, name=None, admin_state_up=True, ext_gateway_net_id=None, enable_snat=None, ext_fixed_ips=None, project_id=None, availability_zone_hints=None):
        ...
        if name:
            router['name'] = name
        ext_gw_info = self._build_external_gateway_info(ext_gateway_net_id, enable_snat, ext_fixed_ips)
        if ext_gw_info:
        ...
        if availability_zone_hints is not None:
            ...
            if not self._has_neutron_extension('router_availability_zone'):
                raise exc.OpenStackCloudUnavailableExtension('router_availability_zone extension is not available on target cloud')
            router['availability_zone_hints'] = availability_zone_hints
        return self.network.create_router(**router)

    def update_router(self, name_or_id, name=None, admin_state_up=None, ext_gateway_net_id=None, enable_snat=None, ext_fixed_ips=None, routes=None):
    ...
    def create_subnet(self, network_name_or_id, cidr=None, ip_version=4, enable_dhcp=False, subnet_name=None, tenant_id=None, allocation_pools=None, gateway_ip=None, disable_gateway_ip=False, dns_nameservers=None, host_routes=None, ipv6_ra_mode=None, ipv6_address_mode=None, prefixlen=None, use_default_subnetpool=False, subnetpool_name_or_id=None, **kwargs):
        ...
        if not network:
            raise exceptions.SDKException('Network %s not found.' % network_name_or_id)
        if disable_gateway_ip and gateway_ip:
            raise exceptions.SDKException('arg:disable_gateway_ip is not allowed with arg:gateway_ip')
        ...
        if cidr and uses_subnetpool:
            raise exceptions.SDKException('arg:cidr and subnetpool may not be used at the same time')
        if use_default_subnetpool and subnetpool_name_or_id:
            raise exceptions.SDKException('arg:use_default_subnetpool and arg:subnetpool_id may not be used at the same time')
        subnetpool = None
        if subnetpool_name_or_id:
            subnetpool = self.get_subnetpool(subnetpool_name_or_id)
            if not subnetpool:
                raise exceptions.SDKException('Subnetpool %s not found.' % subnetpool_name_or_id)
        if isinstance(ip_version, str):
            try:
                ip_version = int(ip_version)
            except ValueError:
                raise exceptions.SDKException('ip_version must be an integer')
        subnet = dict({'network_id': network['id'], 'ip_version': ip_version, 'enable_dhcp': enable_dhcp}, **kwargs)
        if cidr:
            subnet['cidr'] = cidr
        if subnet_name:
    ...
    def update_subnet(self, name_or_id, subnet_name=None, enable_dhcp=None, gateway_ip=None, disable_gateway_ip=None, allocation_pools=None, dns_nameservers=None, host_routes=None):
    ...
    @_utils.valid_kwargs('name', 'admin_state_up', 'fixed_ips', 'security_groups', 'allowed_address_pairs', 'extra_dhcp_opts', 'device_owner', 'device_id', 'binding:vnic_type', 'binding:profile', 'port_security_enabled', 'qos_policy_id', 'binding:host_id')
    def update_port(self, name_or_id, **kwargs):
        ...
        if port is None:
            raise exceptions.SDKException("failed to find port '{port}'".format(port=name_or_id))
