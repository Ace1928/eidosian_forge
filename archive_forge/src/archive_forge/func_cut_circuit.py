...
def cut_circuit(tape: QuantumTape, auto_cutter: Union[bool, Callable]=False, use_opt_einsum: bool=False, device_wires: Optional[Wires]=None, max_depth: int=1, **kwargs) -> (Sequence[QuantumTape], Callable):
    """
    ...
    **Example**
    ...
    if use_opt_einsum:
        try:
            ...
            raise ImportError('The opt_einsum package is required when use_opt_einsum is set to True in the cut_circuit function. This package can be installed using:\npip install opt_einsum') from e
    ...
    for tapes, p, m in expanded:
        ...
        measure_nodes.append(m)
    tapes = tuple((tape for c in configurations for tape in c))
    return (tapes, partial(qcut_processing_fn, communication_graph=communication_graph, prepare_nodes=prepare_nodes, measure_nodes=measure_nodes, use_opt_einsum=use_opt_einsum))