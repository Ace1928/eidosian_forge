...
class OpenShiftImportImage(AnsibleOpenshiftModule):
    ...
    def create_image_stream_import_all(self, stream, source):
        ...
        isi['spec']['repository'] = {'from': {'kind': 'DockerImage', 'name': source}, 'importPolicy': {'insecure': insecure, 'scheduled': self.params.get('scheduled')}, 'referencePolicy': self.ref_policy}
        return isi
    ...
    def import_all(self, istream):
        ...
        if not source:
            if docker_image_repo:
                ...
                if tags == {}:
                    msg = 'image stream %s/%s does not have tags pointing to external container images' % (stream['metadata']['namespace'], stream['metadata']['name'])
        ...
        if err:
            if err == err_stream_not_found_ref:
                ...
                if not source:
                    ...
                    self.fail_json(msg=msg)
                existing = {'from': {'kind': 'DockerImage', 'name': source}}
            else:
                self.fail_json(msg=err)
        else:
            if existing.get('from', {}) and existing['from'].get('kind') != 'DockerImage':
                msg = 'tag {tag} points to existing {kind}/={name}, it cannot be re-imported.'.format(tag=tag, kind=existing['from']['kind'], name=existing['from']['name'])
            ...
            if source and source != existing['from']['name']:
                if multiple:
                    msg = 'the tag {0} points to the tag {1} which points to {2} you cannot change the source using this module'.format(tag, final_tag, existing['from']['name'])
                else:
    ...
    def execute_module(self):
        ...
        try:
            ...
            msg = 'Failed to create object {kind}/{namespace}/{name} due to: {error}'.format(kind=kind, namespace=namespace, name=isi['metadata']['name'], error=exc)
