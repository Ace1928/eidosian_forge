...
def csv_dataset(filenames: _atypes.TensorFuzzingAnnotation[_atypes.String], compression_type: _atypes.TensorFuzzingAnnotation[_atypes.String], buffer_size: _atypes.TensorFuzzingAnnotation[_atypes.Int64], header: _atypes.TensorFuzzingAnnotation[_atypes.Bool], field_delim: _atypes.TensorFuzzingAnnotation[_atypes.String], use_quote_delim: _atypes.TensorFuzzingAnnotation[_atypes.Bool], na_value: _atypes.TensorFuzzingAnnotation[_atypes.String], select_cols: _atypes.TensorFuzzingAnnotation[_atypes.Int64], record_defaults, output_shapes, name=None) -> _atypes.TensorFuzzingAnnotation[_atypes.Variant]:
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'CSVDataset', name, filenames, compression_type, buffer_size, header, field_delim, use_quote_delim, na_value, select_cols, record_defaults, 'output_shapes', output_shapes)
            return _result
            ...
            pass
        try:
            return csv_dataset_eager_fallback(filenames, compression_type, buffer_size, header, field_delim, use_quote_delim, na_value, select_cols, record_defaults, output_shapes=output_shapes, name=name, ctx=_ctx)
