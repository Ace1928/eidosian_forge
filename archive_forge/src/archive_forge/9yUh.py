...
def heuristic(
    ...
    with ThreadPoolExecutor(max_workers=multiprocessing.cpu_count()) as executor:
        ...
        center_of_mass_future = executor.submit(
            lambda: tuple(map(lambda x: sum(x) / len(x), zip(*snake_body)))
        )

        # Collecting results from futures
        base_distance = base_distance_future.result()
        body_proximity_penalty = sum(
            f.result() for f in as_completed(body_proximity_futures)
        )
        ...
        density_bonus_future = executor.submit(
            lambda: -math.sqrt(
                (node1[0] - center_of_mass[0]) ** 2
                + (node1[1] - center_of_mass[1]) ** 2
            )
        )
...
def get_neighbors(
    node: Tuple[int, int], block_size: int, screen_width: int, screen_height: int
) -> List[Tuple[int, int]]:
    """
...
def a_star(
    ...
    def process_node(current, current_f_score):
        if current == goal:
