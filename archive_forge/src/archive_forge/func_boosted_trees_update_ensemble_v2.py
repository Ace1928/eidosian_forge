...
def boosted_trees_update_ensemble_v2(tree_ensemble_handle: _atypes.TensorFuzzingAnnotation[_atypes.Resource], feature_ids: List[_atypes.TensorFuzzingAnnotation[_atypes.Int32]], dimension_ids: List[_atypes.TensorFuzzingAnnotation[_atypes.Int32]], node_ids: List[_atypes.TensorFuzzingAnnotation[_atypes.Int32]], gains: List[_atypes.TensorFuzzingAnnotation[_atypes.Float32]], thresholds: List[_atypes.TensorFuzzingAnnotation[_atypes.Int32]], left_node_contribs: List[_atypes.TensorFuzzingAnnotation[_atypes.Float32]], right_node_contribs: List[_atypes.TensorFuzzingAnnotation[_atypes.Float32]], split_types: List[_atypes.TensorFuzzingAnnotation[_atypes.String]], max_depth: _atypes.TensorFuzzingAnnotation[_atypes.Int32], learning_rate: _atypes.TensorFuzzingAnnotation[_atypes.Float32], pruning_mode: _atypes.TensorFuzzingAnnotation[_atypes.Int32], logits_dimension: int=1, name=None):
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'BoostedTreesUpdateEnsembleV2', name, tree_ensemble_handle, feature_ids, dimension_ids, node_ids, gains, thresholds, left_node_contribs, right_node_contribs, split_types, max_depth, learning_rate, pruning_mode, 'logits_dimension', logits_dimension)
            return _result
            ...
            pass
        try:
            return boosted_trees_update_ensemble_v2_eager_fallback(tree_ensemble_handle, feature_ids, dimension_ids, node_ids, gains, thresholds, left_node_contribs, right_node_contribs, split_types, max_depth, learning_rate, pruning_mode, logits_dimension=logits_dimension, name=name, ctx=_ctx)
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('BoostedTreesUpdateEnsembleV2', tree_ensemble_handle=tree_ensemble_handle, feature_ids=feature_ids, dimension_ids=dimension_ids, node_ids=node_ids, gains=gains, thresholds=thresholds, left_node_contribs=left_node_contribs, right_node_contribs=right_node_contribs, split_types=split_types, max_depth=max_depth, learning_rate=learning_rate, pruning_mode=pruning_mode, logits_dimension=logits_dimension, name=name)
    return _op