...
class Aphrodite(BaseLLM):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        try:
            ...
            raise ImportError('Could not import aphrodite-engine python package. Please install it with `pip install aphrodite-engine`.')
        values['client'] = AphroditeModel(model=values['model'], tensor_parallel_size=values['tensor_parallel_size'], trust_remote_code=values['trust_remote_code'], dtype=values['dtype'], download_dir=values['download_dir'], **values['aphrodite_kwargs'])
        return values

    @property
    def _default_params(self) -> Dict[str, Any]:
        """Get the default parameters for calling aphrodite."""
        return {'n': self.n, 'best_of': self.best_of, 'max_tokens': self.max_tokens, 'top_k': self.top_k, 'top_p': self.top_p, 'top_a': self.top_a, 'min_p': self.min_p, 'temperature': self.temperature, 'presence_penalty': self.presence_penalty, 'frequency_penalty': self.frequency_penalty, 'repetition_penalty': self.repetition_penalty, 'tfs': self.tfs, 'eta_cutoff': self.eta_cutoff, 'epsilon_cutoff': self.epsilon_cutoff, 'typical_p': self.typical_p, 'mirostat_mode': self.mirostat_mode, 'mirostat_tau': self.mirostat_tau, 'length_penalty': self.length_penalty, 'early_stopping': self.early_stopping, 'use_beam_search': self.use_beam_search, 'stop': self.stop, 'ignore_eos': self.ignore_eos, 'logprobs': self.logprobs, 'prompt_logprobs': self.prompt_logprobs, 'custom_token_bans': self.custom_token_bans, 'skip_special_tokens': self.skip_special_tokens, 'spaces_between_special_tokens': self.spaces_between_special_tokens, 'logit_bias': self.logit_bias}
