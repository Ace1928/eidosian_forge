...
class DataIterator(abc.ABC):
    ...
    def iter_batches(self, *, prefetch_batches: int=1, batch_size: int=256, batch_format: Optional[str]='default', drop_last: bool=False, local_shuffle_buffer_size: Optional[int]=None, local_shuffle_seed: Optional[int]=None, _collate_fn: Optional[Callable[[DataBatch], 'CollatedData']]=None, _finalize_fn: Optional[Callable[[Any], Any]]=None, prefetch_blocks: int=0) -> Iterable[DataBatch]:
        ...
        def _create_iterator() -> Iterator[DataBatch]:
            ...
            iterator = iter(iter_batches(block_iterator, stats=stats, clear_block_after_read=blocks_owned_by_consumer, batch_size=batch_size, batch_format=batch_format, drop_last=drop_last, collate_fn=_collate_fn, finalize_fn=_finalize_fn, shuffle_buffer_min_size=local_shuffle_buffer_size, shuffle_seed=local_shuffle_seed, prefetch_batches=prefetch_batches))
    ...
    def iter_torch_batches(self, *, prefetch_batches: int=1, batch_size: Optional[int]=256, dtypes: Optional[Union['torch.dtype', Dict[str, 'torch.dtype']]]=None, device: str='auto', collate_fn: Optional[Callable[[Dict[str, np.ndarray]], 'CollatedData']]=None, drop_last: bool=False, local_shuffle_buffer_size: Optional[int]=None, local_shuffle_seed: Optional[int]=None, prefetch_blocks: int=0) -> Iterable['TorchBatchType']:
        ...
        """
        from ray.air._internal.torch_utils import convert_ndarray_batch_to_torch_tensor_batch, get_device
        if collate_fn is not None and (dtypes is not None or device != 'auto'):
            raise ValueError('collate_fn cannot be used with dtypes and device.You should manually move the output Torch tensors to thedesired dtype and device outside of collate_fn.')
        if device == 'auto':
            device = get_device()
        if collate_fn is None:
            ...
            finalize_fn = None
        return self.iter_batches(prefetch_batches=prefetch_batches, prefetch_blocks=prefetch_blocks, batch_size=batch_size, drop_last=drop_last, local_shuffle_buffer_size=local_shuffle_buffer_size, local_shuffle_seed=local_shuffle_seed, _collate_fn=collate_fn, _finalize_fn=finalize_fn)
    ...
    def iter_tf_batches(self, *, prefetch_batches: int=1, batch_size: Optional[int]=256, dtypes: Optional[Union['tf.dtypes.DType', Dict[str, 'tf.dtypes.DType']]]=None, drop_last: bool=False, local_shuffle_buffer_size: Optional[int]=None, local_shuffle_seed: Optional[int]=None, prefetch_blocks: int=0) -> Iterable['TensorFlowTensorBatchType']:
        ...
        batch_iterable = self.iter_batches(prefetch_batches=prefetch_batches, prefetch_blocks=prefetch_blocks, batch_size=batch_size, drop_last=drop_last, local_shuffle_buffer_size=local_shuffle_buffer_size, local_shuffle_seed=local_shuffle_seed)
        mapped_iterable = map(lambda batch: convert_ndarray_batch_to_tf_tensor_batch(batch, dtypes=dtypes), batch_iterable)
        ...
        if feature_column_dtypes and (not isinstance(feature_column_dtypes, torch.dtype)):
            if isinstance(feature_columns, dict):
                if not isinstance(feature_column_dtypes, dict):
                    raise TypeError(f'If `feature_columns` is a dict, `feature_column_dtypes` must be None, `torch.dtype`, or dict, got {type(feature_column_dtypes)}.')
                if set(feature_columns) != set(feature_column_dtypes):
                    raise ValueError('`feature_columns` and `feature_column_dtypes` must have the same keys.')
                ...
                if not isinstance(feature_column_dtypes, (list, tuple)):
                    raise TypeError(f'If `feature_columns` is a list of lists, `feature_column_dtypes` must be None, `torch.dtype`, or a sequence, got {type(feature_column_dtypes)}.')
                if len(feature_columns) != len(feature_column_dtypes):
                    raise ValueError('`feature_columns` and `feature_column_dtypes` must have the same length.')
    ...
    def to_tf(self, feature_columns: Union[str, List[str]], label_columns: Union[str, List[str]], *, prefetch_batches: int=1, batch_size: int=1, drop_last: bool=False, local_shuffle_buffer_size: Optional[int]=None, local_shuffle_seed: Optional[int]=None, feature_type_spec: Union['tf.TypeSpec', Dict[str, 'tf.TypeSpec']]=None, label_type_spec: Union['tf.TypeSpec', Dict[str, 'tf.TypeSpec']]=None, prefetch_blocks: int=0) -> 'tf.data.Dataset':
        ...
        """
        from ray.air._internal.tensorflow_utils import convert_ndarray_to_tf_tensor, get_type_spec
        try:
        ...
        def generator():
            for batch in self.iter_batches(prefetch_batches=prefetch_batches, prefetch_blocks=prefetch_blocks, batch_size=batch_size, drop_last=drop_last, local_shuffle_buffer_size=local_shuffle_buffer_size, local_shuffle_seed=local_shuffle_seed):
                assert isinstance(batch, dict)
                features = convert_batch_to_tensors(batch, columns=feature_columns, type_spec=feature_type_spec)
                labels = convert_batch_to_tensors(batch, columns=label_columns, type_spec=label_type_spec)
        ...
        dataset = tf.data.Dataset.from_generator(generator, output_signature=(feature_type_spec, label_type_spec))
