...
class CsvDatasetV2(dataset_ops.DatasetSource):
  ...
  """
    ...
    def __init__(self, filenames, record_defaults, compression_type=None, buffer_size=None, header=False, field_delim=',', use_quote_delim=True, na_value='', select_cols=None, exclude_cols=None):
    ...
    """
        self._filenames = ops.convert_to_tensor(filenames, dtype=dtypes.string, name='filenames')
        ...
        record_defaults = [constant_op.constant([], dtype=x) if not tensor_util.is_tf_type(x) and x in _ACCEPTABLE_CSV_TYPES else x for x in record_defaults]
        self._record_defaults = ops.convert_n_to_tensor(record_defaults, name='record_defaults')
        ...
        self._field_delim = ops.convert_to_tensor(field_delim, dtype=dtypes.string, name='field_delim')
        self._use_quote_delim = ops.convert_to_tensor(use_quote_delim, dtype=dtypes.bool, name='use_quote_delim')
        self._na_value = ops.convert_to_tensor(na_value, dtype=dtypes.string, name='na_value')
        self._select_cols = convert.optional_param_to_tensor('select_cols', select_cols, argument_default=[], argument_dtype=dtypes.int64)
        self._exclude_cols = convert.optional_param_to_tensor('exclude_cols', exclude_cols, argument_default=[], argument_dtype=dtypes.int64)
        self._element_spec = tuple((tensor_spec.TensorSpec([], d.dtype) for d in self._record_defaults))
        variant_tensor = gen_experimental_dataset_ops.csv_dataset_v2(filenames=self._filenames, record_defaults=self._record_defaults, buffer_size=self._buffer_size, header=self._header, output_shapes=self._flat_shapes, field_delim=self._field_delim, use_quote_delim=self._use_quote_delim, na_value=self._na_value, select_cols=self._select_cols, exclude_cols=self._exclude_cols, compression_type=self._compression_type)
