...
@log_exception
@log_function_call
def calculate_position_size(equity: float, risk_percent: float, entry_price: float, stop_loss_price: float) -> float:
...
def send_alert(message: str) -> None:
    ...
    if config['notification_channel'] == 'email':
        ...
        logging.warning(f"Unsupported notification channel: {config['notification_channel']}")
...
def main() -> None:
   ...
   # Calculating Donchian Channels
    ...
    logging.warning(f"Failed to fetch order book data for {symbol} on {exchange_id}")
    ...
    lstm_predictions = train_and_predict_lstm(X_train, y_train, X_test, epochs=50, batch_size=32)
