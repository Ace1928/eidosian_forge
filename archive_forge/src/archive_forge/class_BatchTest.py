...
class BatchTest(unittest.TestCase):
    ...
    def _DoTestConvertIdToHeader(self, test_id, expected_result):
        ...
        self.assertEqual(expected_result % batch_request._BatchHttpRequest__base_id, batch_request._ConvertIdToHeader(test_id))
    ...
    def testSerializeRequest(self):
        request = http_wrapper.Request(body='Hello World', headers={'content-type': 'protocol/version'})
        expected_serialized_request = '\n'.join(['GET  HTTP/1.1', 'Content-Type: protocol/version', 'MIME-Version: 1.0', 'content-length: 11', 'Host: ', '', 'Hello World'])
        batch_request = batch.BatchHttpRequest('https://www.example.com')
    ...
    def testInternalExecuteWithFailedRequest(self):
        with mock.patch.object(http_wrapper, 'MakeRequest', autospec=True) as mock_request:
            self.__ConfigureMock(mock_request, http_wrapper.Request('https://www.example.com', 'POST', {'content-type': 'multipart/mixed; boundary="None"', 'content-length': 80}, 'x' * 80), http_wrapper.Response({'status': '300'}, None, None))
            batch_request = batch.BatchHttpRequest('https://www.example.com')
    ...
    def testInternalExecuteWithEncodedResponse(self):
        with mock.patch.object(http_wrapper, 'MakeRequest', autospec=True) as mock_request:
            self.__ConfigureMock(mock_request, http_wrapper.Request('https://www.example.com', 'POST', {'content-type': 'multipart/mixed; boundary="None"', 'content-length': 274}, 'x' * 274), http_wrapper.Response({'status': '200', 'content-type': 'multipart/mixed; boundary="boundary"'}, textwrap.dedent('                --boundary\n                content-type: text/plain\n                content-id: <id+1>\n\n                HTTP/1.1 200 OK\n                response\n\n                --boundary--').encode('utf-8'), None))
            test_request = {'1': batch.RequestResponseAndHandler(http_wrapper.Request(body='first'), None, None)}
            batch_request = batch.BatchHttpRequest('https://www.example.com', response_encoding='utf-8')
