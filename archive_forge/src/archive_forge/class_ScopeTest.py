...
class ScopeTest(test_utils.TestCase):
    ...
    def test_multilevel_import_reads(self):
        source = textwrap.dedent('        import aaa.bbb.ccc\n        aaa.bbb.ccc.foo()\n        ')
        tree = ast.parse(source)
        nodes = tree.body
        node_ref = nodes[1].value.func.value
        s = scope.analyze(tree)
        self.assertItemsEqual(s.names.keys(), {'aaa'})
        self.assertItemsEqual(s.external_references.keys(), {'aaa', 'aaa.bbb', 'aaa.bbb.ccc'})
        ...
        self.assertItemsEqual(s.names['aaa'].attrs['bbb'].attrs['ccc'].reads, [node_ref])
