...
class GeneratorBasedBuilder(DatasetBuilder):
    ...
    def _prepare_split(self, split_generator: SplitGenerator, check_duplicate_keys: bool, file_format='arrow', num_proc: Optional[int]=None, max_shard_size: Optional[Union[int, str]]=None):
        ...
        if total_shards > 1:
            ...
            shards_and_jobs = [(shard_id, job_id) for job_id, num_shards in enumerate(shards_per_job) for shard_id in range(num_shards)]
    ...
    def _prepare_split_single(self, gen_kwargs: dict, fpath: str, file_format: str, max_shard_size: int, split_info: SplitInfo, check_duplicate_keys: bool, job_id: int) -> Iterable[Tuple[int, bool, Union[int, tuple]]]:
        ...
        try:
            writer = writer_class(features=self.info.features, path=fpath.replace('SSSSS', f'{shard_id:05d}').replace('JJJJJ', f'{job_id:05d}'), writer_batch_size=self._writer_batch_size, hash_salt=split_info.name, check_duplicates=check_duplicate_keys, storage_options=self._fs.storage_options, embed_local_files=embed_local_files)
            try:
                _time = time.time()
                for key, record in generator:
                    if max_shard_size is not None and writer._num_bytes > max_shard_size:
                        ...
                        shard_id += 1
                        writer = writer_class(features=writer._features, path=fpath.replace('SSSSS', f'{shard_id:05d}').replace('JJJJJ', f'{job_id:05d}'), writer_batch_size=self._writer_batch_size, hash_salt=split_info.name, check_duplicates=check_duplicate_keys, storage_options=self._fs.storage_options, embed_local_files=embed_local_files)
    ...
    def _download_and_prepare(self, dl_manager, verification_mode, **prepare_splits_kwargs):
        super()._download_and_prepare(dl_manager, verification_mode, check_duplicate_keys=verification_mode == VerificationMode.BASIC_CHECKS or verification_mode == VerificationMode.ALL_CHECKS, **prepare_splits_kwargs)
