...
class TestCanonicalLayout(unittest.TestCase):
  ...
  """
    ...
    def setUp(self):
        self.config.parse.fn_spec.add('foo', flags=['BAR', 'BAZ'], kwargs={'HEADERS': '*', 'SOURCES': '*', 'DEPENDS': '*'})
        self.parse_ctx.parse_db.update(parse.funs.get_funtree(self.config.parse.fn_spec))
