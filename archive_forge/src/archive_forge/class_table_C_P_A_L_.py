...
class table_C_P_A_L_(DefaultTable.DefaultTable):
    ...
    def decompile(self, data, ttFont):
        ...
        if self.version == 0:
            ...
            offsetToPaletteTypeArray, offsetToPaletteLabelArray, offsetToPaletteEntryLabelArray = struct.unpack('>LLL', data[pos:pos + 12])
        self.paletteTypes = self._decompileUInt32Array(data, offsetToPaletteTypeArray, numPalettes, default=self.DEFAULT_PALETTE_TYPE)
        self.paletteLabels = self._decompileUInt16Array(data, offsetToPaletteLabelArray, numPalettes, default=self.NO_NAME_ID)
        self.paletteEntryLabels = self._decompileUInt16Array(data, offsetToPaletteEntryLabelArray, self.numPaletteEntries, default=self.NO_NAME_ID)
    ...
    def compile(self, ttFont):
        ...
        header = struct.pack('>HHHHL', self.version, self.numPaletteEntries, len(self.palettes), numColorRecords, offsetToFirstColorRecord)
        if self.version == 0:
            ...
            if len(paletteEntryLabels) == 0:
                ...
                pos += len(paletteLabels)
            header1 = struct.pack('>LLL', offsetToPaletteTypeArray, offsetToPaletteLabelArray, offsetToPaletteEntryLabelArray)
            dataList = [header, colorRecordIndices, header1, colorRecords, paletteTypes, paletteLabels, paletteEntryLabels]
