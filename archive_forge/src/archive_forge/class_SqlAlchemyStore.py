...
class SqlAlchemyStore(AbstractStore):
    ...
    def __init__(self, db_uri):
        ...
        self.ManagedSessionMaker = mlflow.store.db.utils._get_managed_session_maker(SessionMaker, self.db_type)
    ...
    def create_registered_model(self, name, tags=None, description=None):
        ...
        with self.ManagedSessionMaker() as session:
            try:
                ...
                registered_model = SqlRegisteredModel(name=name, creation_time=creation_time, last_updated_time=creation_time, description=description)
                tags_dict = {}
                for tag in tags or []:
                    tags_dict[tag.key] = tag.value
                registered_model.registered_model_tags = [SqlRegisteredModelTag(key=key, value=value) for key, value in tags_dict.items()]
                session.add(registered_model)
                session.flush()
                return registered_model.to_mlflow_entity()
            except sqlalchemy.exc.IntegrityError as e:
                raise MlflowException(f'Registered Model (name={name}) already exists. Error: {e}', RESOURCE_ALREADY_EXISTS)
    ...
    def rename_registered_model(self, name, new_name):
        ...
        with self.ManagedSessionMaker() as session:
            ...
            try:
                ...
                raise MlflowException(f'Registered Model (name={new_name}) already exists. Error: {e}', RESOURCE_ALREADY_EXISTS)
    ...
    def search_registered_models(self, filter_string=None, max_results=SEARCH_REGISTERED_MODEL_MAX_RESULTS_DEFAULT, order_by=None, page_token=None):
        ...
        if max_results > SEARCH_REGISTERED_MODEL_MAX_RESULTS_THRESHOLD:
            raise MlflowException(f'Invalid value for request parameter max_results. It must be at most {SEARCH_REGISTERED_MODEL_MAX_RESULTS_THRESHOLD}, but got value {max_results}', INVALID_PARAMETER_VALUE)
        parsed_filters = SearchModelUtils.parse_search_filter(filter_string)
        filter_query = self._get_search_registered_model_filter_query(parsed_filters, self.engine.dialect.name)
        parsed_orderby = self._parse_search_registered_models_order_by(order_by)
        offset = SearchUtils.parse_start_offset_from_page_token(page_token)
        max_results_for_query = max_results + 1
        with self.ManagedSessionMaker() as session:
            query = filter_query.options(*self._get_eager_registered_model_query_options()).order_by(*parsed_orderby).limit(max_results_for_query)
            if page_token:
                query = query.offset(offset)
            sql_registered_models = session.execute(query).scalars(SqlRegisteredModel).all()
            next_page_token = self._compute_next_token(max_results_for_query, len(sql_registered_models), offset, max_results)
    ...
    def _parse_search_registered_models_order_by(cls, order_by_list):
        ...
        if order_by_list:
            for order_by_clause in order_by_list:
                attribute_token, ascending = SearchUtils.parse_order_by_for_search_registered_models(order_by_clause)
                if attribute_token == SqlRegisteredModel.name.key:
                    ...
                    raise MlflowException(f"Invalid order by key '{attribute_token}' specified." + 'Valid keys are ' + f"'{SearchUtils.RECOMMENDED_ORDER_BY_KEYS_REGISTERED_MODELS}'", error_code=INVALID_PARAMETER_VALUE)
    ...
    def create_model_version(self, name, source, run_id=None, tags=None, run_link=None, description=None, local_model_path=None):
        ...
        if urllib.parse.urlparse(source).scheme == 'models':
            ...
            try:
                storage_location = self.get_model_version_download_uri(parsed_model_uri.name, parsed_model_uri.version)
            except Exception as e:
                raise MlflowException(f"Unable to fetch model from model URI source artifact location '{source}'.Error: {e}") from e
        with self.ManagedSessionMaker() as session:
            creation_time = get_current_time_millis()
            for attempt in range(self.CREATE_MODEL_VERSION_RETRIES):
                try:
                    ...
                    model_version = SqlModelVersion(name=name, version=version, creation_time=creation_time, last_updated_time=creation_time, source=source, storage_location=storage_location, run_id=run_id, run_link=run_link, description=description)
                    tags_dict = {}
                    for tag in tags or []:
                        tags_dict[tag.key] = tag.value
                    model_version.model_version_tags = [SqlModelVersionTag(key=key, value=value) for key, value in tags_dict.items()]
                    session.add_all([sql_registered_model, model_version])
                    session.flush()
                    return self._populate_model_version_aliases(session, name, model_version.to_mlflow_entity())
                except sqlalchemy.exc.IntegrityError:
                    more_retries = self.CREATE_MODEL_VERSION_RETRIES - attempt - 1
                    _logger.info('Model Version creation error (name=%s) Retrying %s more time%s.', name, str(more_retries), 's' if more_retries > 1 else '')
        raise MlflowException(f'Model Version creation error (name={name}). Giving up after {self.CREATE_MODEL_VERSION_RETRIES} attempts.')
    ...
    def _populate_model_version_aliases(cls, session, name, version):
        ...
        version.aliases = [alias.alias for alias in model_aliases if alias.version == version.version]
        return version
    ...
    def transition_model_version_stage(self, name, version, stage, archive_existing_versions):
        ...
        if archive_existing_versions and (not is_active_stage):
            msg_tpl = "Model version transition cannot archive existing model versions because '{}' is not an Active stage. Valid stages are {}"
        ...
        with self.ManagedSessionMaker() as session:
            ...
            if archive_existing_versions:
                conditions = [SqlModelVersion.name == name, SqlModelVersion.version != version, SqlModelVersion.current_stage == get_canonical_stage(stage)]
            ...
            sql_model_version = self._get_sql_model_version(session=session, name=name, version=version)
    ...
    def search_model_versions(self, filter_string=None, max_results=SEARCH_MODEL_VERSION_MAX_RESULTS_DEFAULT, order_by=None, page_token=None):
        ...
        if max_results > SEARCH_MODEL_VERSION_MAX_RESULTS_THRESHOLD:
            raise MlflowException(f'Invalid value for request parameter max_results. It must be at most {SEARCH_MODEL_VERSION_MAX_RESULTS_THRESHOLD}, but got value {max_results}', INVALID_PARAMETER_VALUE)
        parsed_filters = SearchModelVersionUtils.parse_search_filter(filter_string)
        filter_query = self._get_search_model_versions_filter_clauses(parsed_filters, self.engine.dialect.name)
        parsed_orderby = self._parse_search_model_versions_order_by(order_by or ['last_updated_timestamp DESC', 'name ASC', 'version_number DESC'])
        offset = SearchUtils.parse_start_offset_from_page_token(page_token)
        max_results_for_query = max_results + 1
        with self.ManagedSessionMaker() as session:
            query = filter_query.options(*self._get_eager_model_version_query_options()).filter(SqlModelVersion.current_stage != STAGE_DELETED_INTERNAL).order_by(*parsed_orderby).limit(max_results_for_query)
            if page_token:
                query = query.offset(offset)
            sql_model_versions = session.execute(query).scalars(SqlModelVersion).all()
            next_page_token = self._compute_next_token(max_results_for_query, len(sql_model_versions), offset, max_results)
    ...
    def _get_model_version_tag(cls, session, name, version, key):
        tags = session.query(SqlModelVersionTag).filter(SqlModelVersionTag.name == name, SqlModelVersionTag.version == version, SqlModelVersionTag.key == key).all()
        if len(tags) == 0:
            return None
        if len(tags) > 1:
            raise MlflowException(f'Expected only 1 model version tag with name={name}, version={version}, key={key}. Found {len(tags)}.', INVALID_STATE)
        return tags[0]
    ...
    def set_model_version_tag(self, name, version, tag):
        ...
        with self.ManagedSessionMaker() as session:
            ...
            session.merge(SqlModelVersionTag(name=name, version=version, key=tag.key, value=tag.value))
    ...
    def get_model_version_by_alias(self, name, alias):
        ...
        with self.ManagedSessionMaker() as session:
            existing_alias = self._get_registered_model_alias(session, name, alias)
            if existing_alias is not None:
                sql_model_version = self._get_sql_model_version(session, existing_alias.name, existing_alias.version)
                return self._populate_model_version_aliases(session, name, sql_model_version.to_mlflow_entity())
            else:
                raise MlflowException(f'Registered model alias {alias} not found.', INVALID_PARAMETER_VALUE)
