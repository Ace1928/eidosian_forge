...
class CmdStanGQ(Generic[Fit]):
    ...
    def __init__(self, runset: RunSet, previous_fit: Fit) -> None:
        """Initialize object."""
        if not runset.method == Method.GENERATE_QUANTITIES:
            raise ValueError('Wrong runset method, expecting generate_quantities runset, found method {}'.format(runset.method))
        self.runset = runset
    ...
    def __repr__(self) -> str:
        repr = 'CmdStanGQ: model={} chains={}{}'.format(self.runset.model, self.chains, self.runset._args.method_args.compose(0, cmd=[]))
    ...
    @overload
    def draws_xr(self: Union['CmdStanGQ[CmdStanMLE]', 'CmdStanGQ[CmdStanVB]'], vars: Union[str, List[str], None]=None, inc_warmup: bool=False, inc_sample: bool=False) -> NoReturn:
        ...

    @overload
    def draws_xr(self: 'CmdStanGQ[CmdStanMCMC]', vars: Union[str, List[str], None]=None, inc_warmup: bool=False, inc_sample: bool=False) -> 'xr.Dataset':
        ...
