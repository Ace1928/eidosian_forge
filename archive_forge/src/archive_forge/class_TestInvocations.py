...
class TestInvocations(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.env = {'PYTHONPATH': os.path.dirname(parentdir)}
    ...
    """
        ...
        infile_path = os.path.join(thisdir, 'testdata', 'test_in.cmake')
        expectfile_path = os.path.join(thisdir, 'testdata', 'test_out.cmake')
        subprocess.check_call([sys.executable, '-Bm', 'cmakelang.format', '-c', os.path.join(configdir, 'cmake-format-split-1.py'), '-c', os.path.join(configdir, 'cmake-format-split-2.py'), '-o', os.path.join(self.tempdir, 'test_out.cmake'), infile_path], cwd=self.tempdir, env=self.env)
        with io.open(os.path.join(self.tempdir, 'test_out.cmake'), 'r', encoding='utf8') as infile:
            actual_text = infile.read()
        with io.open(expectfile_path, 'r', encoding='utf8') as infile:
            expected_text = infile.read()
