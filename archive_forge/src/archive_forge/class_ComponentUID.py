...
class ComponentUID(object):
    ...
    __slots__ = ('_cids',)
    _lex = None
    _repr_v1_map = {slice: lambda x: '*', Ellipsis.__class__: lambda x: '**', int: lambda x: '#' + str(x), float: lambda x: '#' + str(x), str: lambda x: '$' + str(x)}

    def __init__(self, component, cuid_buffer=None, context=None):
        if isinstance(component, str):
            if context is not None:
                raise ValueError('Context is not allowed when initializing a ComponentUID object from a string type')
            try:
            ...
            self._cids = tuple(self._generate_cuid_from_slice(component, context=context))
        else:
            self._cids = tuple(self._generate_cuid(component, cuid_buffer=cuid_buffer, context=context))
    ...
    def get_repr(self, version=2):
        if version == 1:
            ...
            for name, args in self._cids:
                ...
                if len(args) == 0:
                    continue
                a += ':' + ','.join((self._repr_v1_map.get(x.__class__, _unknown)(x) for x in args))
            return a[1:]
        elif version == 2:
            return repr(self)
        else:
    ...
    def __hash__(self):
        ...
        try:
            ...
            return hash(tuple(((name, tuple(((slice, x.start, x.stop, x.step) if x.__class__ is slice else x for x in idx))) for name, idx in self._cids)))
    ...
    def __lt__(self, other):
        ...
        try:
            ...
            raise TypeError("'<' not supported between instances of 'ComponentUID' and '%s'" % type(other).__name__)
        for (self_name, self_idx), (other_name, other_idx) in zip(self._cids, other_cids):
    ...
    def _generate_cuid(self, component, cuid_buffer=None, context=None):
        ...
        while component is not context:
            if component is model:
                raise ValueError("Context '%s' does not apply to component '%s'" % (context.name, orig_component.name))
