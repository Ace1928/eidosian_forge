...
class Series(base.IndexOpsMixin, NDFrame):
    ...
    def __init__(self, data=None, index=None, dtype: Dtype | None=None, name=None, copy: bool | None=None, fastpath: bool | lib.NoDefault=lib.no_default) -> None:
        if fastpath is not lib.no_default:
            warnings.warn("The 'fastpath' keyword in pd.Series is deprecated and will be removed in a future version.", DeprecationWarning, stacklevel=find_stack_level())
        else:
            fastpath = False
        allow_mgr = False
        if isinstance(data, (SingleBlockManager, SingleArrayManager)) and index is None and (dtype is None) and (copy is False or copy is None):
            if not allow_mgr:
        ...
        if original_dtype is None and is_pandas_object and (data_dtype == np.object_):
            if self.dtype != data_dtype:
                warnings.warn('Dtype inference on a pandas object (Series, Index, ExtensionArray) is deprecated. The Series constructor will keep the original dtype in the future. Call `infer_objects` on the result to get the old behavior.', FutureWarning, stacklevel=find_stack_level())
    ...
    def ravel(self, order: str='C') -> ArrayLike:
        ...
        array([1, 2, 3])
        """
        warnings.warn('Series.ravel is deprecated. The underlying array is already 1D, so ravel is not necessary.  Use `to_numpy()` for conversion to a numpy array instead.', FutureWarning, stacklevel=2)
    ...
    def view(self, dtype: Dtype | None=None) -> Series:
        ...
        """
        warnings.warn('Series.view is deprecated and will be removed in a future version. Use ``astype`` as an alternative to change the dtype.', FutureWarning, stacklevel=2)
    ...
    def __setitem__(self, key, value) -> None:
        ...
        if not PYPY and using_copy_on_write():
            ...
            if not warn_copy_on_write() and _check_cacher(self):
                ref_count += 1
            if ctr <= ref_count and (warn_copy_on_write() or (not warn_copy_on_write() and self._mgr.blocks[0].refs.has_reference())):
                warn = False
    ...
    def groupby(self, by=None, axis: Axis=0, level: IndexLabel | None=None, as_index: bool=True, sort: bool=True, group_keys: bool=True, observed: bool | lib.NoDefault=lib.no_default, dropna: bool=True) -> SeriesGroupBy:
        ...
        return SeriesGroupBy(obj=self, keys=by, axis=axis, level=level, as_index=as_index, sort=sort, group_keys=group_keys, observed=observed, dropna=dropna)
    ...
    def argsort(self, axis: Axis=0, kind: SortKind='quicksort', order: None=None, stable: None=None) -> Series:
        ...
        mask = isna(values)
        if mask.any():
            warnings.warn('The behavior of Series.argsort in the presence of NA values is deprecated. In a future version, NA values will be ordered last instead of set to -1.', FutureWarning, stacklevel=find_stack_level())
    ...
    def apply(self, func: AggFuncType, convert_dtype: bool | lib.NoDefault=lib.no_default, args: tuple[Any, ...]=(), *, by_row: Literal[False, 'compat']='compat', **kwargs) -> DataFrame | Series:
        """
        ...
        **kwargs
        ...
        dtype: float64
        """
        return SeriesApply(self, func, convert_dtype=convert_dtype, by_row=by_row, args=args, kwargs=kwargs).apply()
    ...
    def rename(self, index: Renamer | Hashable | None=None, *, axis: Axis | None=None, copy: bool | None=None, inplace: bool=False, level: Level | None=None, errors: IgnoreRaise='ignore') -> Series | None:
        ...
        if callable(index) or is_dict_like(index):
            return super()._rename(index, copy=copy, inplace=inplace, level=level, errors=errors)
        else:
    ...
    def reindex(self, index=None, *, axis: Axis | None=None, method: ReindexMethod | None=None, copy: bool | None=None, level: Level | None=None, fill_value: Scalar | None=None, limit: int | None=None, tolerance=None) -> Series:
        return super().reindex(index=index, method=method, copy=copy, level=level, fill_value=fill_value, limit=limit, tolerance=tolerance)
    ...
    def rename_axis(self, mapper: IndexLabel | lib.NoDefault=lib.no_default, *, index=lib.no_default, axis: Axis=0, copy: bool=True, inplace: bool=False) -> Self | None:
        return super().rename_axis(mapper=mapper, index=index, axis=axis, copy=copy, inplace=inplace)
            ...
            if not left.index.equals(right.index):
                if align_asobject:
                    if left.dtype not in (object, np.bool_) or right.dtype not in (object, np.bool_):
                        warnings.warn('Operation between non boolean Series with different indexes will no longer return a boolean result in a future version. Cast both Series to object type to maintain the prior behavior.', FutureWarning, stacklevel=find_stack_level())
