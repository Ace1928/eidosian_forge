...
class FileManager:
    ...
    def generate_segmented_file_hash(file_path: str, segment_number: int) -> str:
        with open(file_path, "rb") as file:
            content = file.read()
        file_stat = os.stat(file_path)
        unique_identifier = (
            f"{file_stat.st_size}-{file_stat.st_mtime}-{segment_number}-{content}"
        )
    ...
    def process_file(
        ...
        """
        Processes a single Python file by segmenting its content, extracting functions, imports, and classes from each segment,
        and storing embeddings in a database. This method includes a robust hashing process that incorporates file size and segment index
        to ensure uniqueness of each segment.
        ...
        try:
            ...
            # Define a function to split the content into manageable segments
            def split_into_segments(
                content: bytes, segment_size: int = 1024
            ) -> List[bytes]:
                return [
                    content[i : i + segment_size]
            ...
            for i, segment in enumerate(segments):
                unique_identifier = (
                    f"{file_stat.st_size}-{file_stat.st_mtime}-{i}-{segment}"
                )
                file_hash = hashlib.sha256(unique_identifier.encode()).hexdigest()

                if db_manager.is_file_processed(file_hash):
                    logging.info(f"Segment {i} of file {file_path} already processed")
                    continue

                # Decode the segment to process with the code parser
                decoded_segment = segment.decode("utf-8")
                functions, imports, classes = (
                    CodeParser.extract_functions_imports_classes_from_content(
                        decoded_segment
                    )
                )
                embeddings = [
                ...
                logging.info(
                    f"Segment {i} of file processed and embeddings stored: {file_path}"
                )
...
class CodeParser:
    """
    A class dedicated to parsing Python code to extract functions, imports, classes, and their associated documentation comprehensively.
    This parser is designed to be robust, flexible, and universal, ensuring that it captures every aspect of the code in its entirety.
    """
    ...
    def extract_functions_from_ast(tree: ast.Module) -> List[Dict[str, str]]:
        """
        Extracts functions from the AST of a Python file, including their docstrings and code.
    ...
    @staticmethod
    def extract_class_definitions_from_ast(tree: ast.Module) -> List[Dict[str, str]]:
        """
        Extracts all class definitions from the AST of a Python file, including their docstrings and code.
        :param tree: ast.Module - The AST of the Python file.
        :return: List[Dict[str, str]] - A list of dictionaries, each representing a class definition with its code and docstring.
        """
        classes = []
        for node in ast.walk(tree):
            if isinstance(node, ast.ClassDef):
                ...
                classes.append({"class": class_code, "docstring": class_docstring})
        return classes
    ...
    def extract_functions_imports_classes_from_content(
        ...
        """
        Extracts functions, imports, and classes from the content of a Python file, including all associated documentation.
        This method ensures that all elements are extracted comprehensively and formatted consistently for seamless integration.
        :param content: str - The content of the Python file.
        :return: Tuple[List[Dict[str, str]], str, List[Dict[str, str]]] - A tuple containing a list of functions, a string of imports, and a list of classes, all with their respective docstrings.
        """
