...
class LocalDataFrameIterableDataFrame(LocalUnboundedDataFrame):
    ...
    def __init__(self, df: Any=None, schema: Any=None):
        if isinstance(df, Iterable):
            ...
            raise ValueError(f'{df} is incompatible with LocalDataFrameIterableDataFrame')
        if orig_schema is None and schema is None:
            raise FugueDataFrameInitError('schema is not provided and the input is empty')
            ...
            assert_or_throw(orig_schema == schema, lambda: f'iterable schema {orig_schema} is different from {schema}')
    ...
    def as_array(self, columns: Optional[List[str]]=None, type_safe: bool=False) -> List[Any]:
        return sum((df.as_array(columns=columns, type_safe=type_safe) for df in self.native), [])
    ...
    def head(self, n: int, columns: Optional[List[str]]=None) -> LocalBoundedDataFrame:
        ...
        for row in self.as_array_iterable(columns, type_safe=True):
            ...
            res.append(list(row))
            n -= 1
        return ArrayDataFrame(res, self.schema if columns is None else self.schema.extract(columns))
