...
class BaseVolume(resource.Resource):
    ...
    def check_create_complete(self, vol_id):
        ...
        if vol.status == 'error':
            ...
            raise exception.ResourceUnknownStatus(resource_status=vol.status, result=_('Volume create failed'))
    ...
    def handle_check(self):
        ...
        checks = [{'attr': 'status', 'expected': statuses, 'current': vol.status}]
        self._verify_check_conditions(checks)

    def handle_snapshot_delete(self, state):
        backup = state not in ((self.CREATE, self.FAILED), (self.UPDATE, self.FAILED))
    ...
    def check_delete_complete(self, prg):
        ...
        if not prg.delete['complete']:
            ...
            if vol.status.lower() == 'error_deleting':
                raise exception.ResourceInError(status_reason='delete', resource_status=vol.status)
            else:
                return False
        return True
