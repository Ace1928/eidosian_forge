...
class LanceDB(VectorStore):
    ...
    def __init__(self, connection: Optional[Any]=None, embedding: Optional[Embeddings]=None, vector_key: Optional[str]='vector', id_key: Optional[str]='id', text_key: Optional[str]='text', table_name: Optional[str]='vectorstore'):
        """Initialize with Lance DB vectorstore"""
        try:
            import lancedb
        except ImportError:
            raise ImportError('Could not import lancedb python package. Please install it with `pip install lancedb`.')
        ...
        if connection is not None:
            if not isinstance(connection, lancedb.db.LanceTable):
                raise ValueError('connection should be an instance of lancedb.db.LanceTable, ', f'got {type(connection)}')
    ...
    def add_texts(self, texts: Iterable[str], metadatas: Optional[List[dict]]=None, ids: Optional[List[str]]=None, **kwargs: Any) -> List[str]:
        ...
        for idx, text in enumerate(texts):
            ...
            docs.append({self._vector_key: embedding, self._id_key: ids[idx], self._text_key: text, **metadata})
    ...
    def similarity_search(self, query: str, k: int=4, **kwargs: Any) -> List[Document]:
        ...
        return [Document(page_content=docs[self._text_key][idx].as_py(), metadata={col: docs[col][idx].as_py() for col in columns if col != self._text_key}) for idx in range(len(docs))]

    @classmethod
    def from_texts(cls, texts: List[str], embedding: Embeddings, metadatas: Optional[List[dict]]=None, connection: Any=None, vector_key: Optional[str]='vector', id_key: Optional[str]='id', text_key: Optional[str]='text', **kwargs: Any) -> LanceDB:
