...
class AnsibleVultrCommonInstance(AnsibleVultr):
    VPC_CONFIGS = {'v1': {'param': 'vpcs', 'path': '/vpcs', 'suffix': ''}, 'v2': {'param': 'vpc2s', 'path': '/vpc2', 'suffix': '2'}}
    ...
    def get_firewall_group(self):
        return self.query_filter_list_by_name(key_name='description', param_key='firewall_group', path='/firewalls', result_key='firewall_groups', fail_not_found=True)

    def get_snapshot(self):
        return self.query_filter_list_by_name(key_name='description', param_key='snapshot', path='/snapshots', result_key='snapshots', fail_not_found=True)

    def get_startup_script(self):
        return self.query_filter_list_by_name(key_name='name', param_key='startup_script', path='/startup-scripts', result_key='startup_scripts', fail_not_found=True)

    def get_os(self):
        return self.query_filter_list_by_name(key_name='name', param_key='os', path='/os', result_key='os', fail_not_found=True)

    def get_app(self):
        return self.query_filter_list_by_name(key_name='deploy_name', param_key='app', path='/applications', result_key='applications', fail_not_found=True, query_params={'type': 'one-click'})

    def get_image(self):
        return self.query_filter_list_by_name(key_name='deploy_name', param_key='image', path='/applications', result_key='applications', fail_not_found=True, query_params={'type': 'marketplace'})
