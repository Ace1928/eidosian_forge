...
class NetAppOntapSecurityKeyManager:
    ...
    def __init__(self):
        ...
        if self.use_rest:
            ...
            if rest_only:
                self.module.fail_json(msg='Error: REST is required for %s option%s.' % (', '.join(rest_only), 's' if len(rest_only) > 1 else ''))
    ...
    def set_scope(vserver):
        ...
        return ('cluster' if vserver is None else 'svm', 'cluster' if vserver is None else 'vserver: %s' % vserver)
    ...
    def create_key_manager_rest(self, retrying=None):
        ...
        if error:
            if not retrying and ('already has external key management configured' in error or 'External key management already configured' in error):
