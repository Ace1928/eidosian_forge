...
class AddressGroupTests(common.NetworkTests):
    ...
    def test_address_group_list(self):
        ...
        name2 = uuid.uuid4().hex
        cmd_output = self.openstack('address group create ' + '--project ' + demo_project_id + ' ' + name2, parse_output=True)
        self.addCleanup(self.openstack, 'address group delete ' + name2)
        self.assertEqual(demo_project_id, cmd_output['project_id'])
        cmd_output = self.openstack('address group list ', parse_output=True)
        names = [x['Name'] for x in cmd_output]
        self.assertIn(name1, names)
        self.assertIn(name2, names)
        cmd_output = self.openstack('address group list ' + '--project ' + demo_project_id, parse_output=True)
        names = [x['Name'] for x in cmd_output]
        self.assertNotIn(name1, names)
        self.assertIn(name2, names)
        cmd_output = self.openstack('address group list ' + '--name ' + name1, parse_output=True)
        names = [x['Name'] for x in cmd_output]
        self.assertIn(name1, names)
