...
class JupyterServerListApp(JupyterApp):
    ...
    flags = {'jsonlist': ({'JupyterServerListApp': {'jsonlist': True}}, _i18n('Produce machine-readable JSON list output.')), 'json': ({'JupyterServerListApp': {'json': True}}, _i18n('Produce machine-readable JSON object on each line of output.'))}
    jsonlist = Bool(False, config=True, help=_i18n('If True, the output will be a JSON list of objects, one per active Jupyer server, each with the details from the relevant server info file.'))
    json = Bool(False, config=True, help=_i18n('If True, each line of output will be a JSON object with the details from the server info file. For a JSON list output, see the JupyterServerListApp.jsonlist configuration value'))
