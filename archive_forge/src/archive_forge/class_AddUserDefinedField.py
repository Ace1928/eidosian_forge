  ...
  *{command}* is used to add user defined fields to security policies.
  ...
  """
    ...
    def Args(cls, parser):
        ...
        parser.add_argument('--user-defined-field-name', required=True, help='The name for the user defined field.')
        parser.add_argument('--base', choices=['ipv4', 'ipv6', 'tcp', 'udp'], required=True, help='The base relative to which offset is measured.')
        parser.add_argument('--offset', type=int, required=True, help='Offset of the first byte of the field (in network byte order) relative to base.')
        parser.add_argument('--size', type=int, required=True, help='Size of the field in bytes. Valid values: 1-4.')
        parser.add_argument('--mask', help='If specified, apply this mask (bitwise AND) to the field to ignore bits before matching. Encoded as a hexadecimal number (starting with "0x").')
    ...
    def Run(self, args):
        ...
        user_defined_field = security_policies_utils.CreateUserDefinedField(holder.client, args)
        user_defined_fields = existing_security_policy.userDefinedFields
        user_defined_fields.append(user_defined_field)
        updated_security_policy = holder.client.messages.SecurityPolicy(userDefinedFields=user_defined_fields, fingerprint=existing_security_policy.fingerprint)
