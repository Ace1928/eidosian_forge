...
class LayerSet:
    ...
    """
    _layers: Dict[str, Layer] = field(validator=_must_have_at_least_one_item)
    ...
    def __attrs_post_init__(self) -> None:
        if self._defaultLayer == _LAYER_NOT_LOADED:
            ...
            if not any((layer is self._defaultLayer for layer in self._layers.values())):
                raise ValueError(f'default layer {repr(self._defaultLayer)} must be in layer set.')
    ...
    @classmethod
    def from_iterable(cls, value: Iterable[Layer], defaultLayerName: str=DEFAULT_LAYER_NAME) -> LayerSet:
        ...
        if defaultLayerName != DEFAULT_LAYER_NAME:
            import warnings
            warnings.warn("'defaultLayerName' parameter is deprecated; use Layer.default attribute instead", DeprecationWarning)
    ...
    @staticmethod
    def _loadLayer(reader: UFOReader, layerName: str, lazy: bool=True, default: bool=False) -> Layer:
    ...
    def __repr__(self) -> str:
        n = len(self._layers)
        return '<{}.{} ({} layer{}) at {}>'.format(self.__class__.__module__, self.__class__.__name__, n, 's' if n > 1 else '', hex(id(self)))
    ...
    @staticmethod
    def _structure(data: list[dict[str, Any]], cls: Type[LayerSet], converter: Converter) -> LayerSet:
