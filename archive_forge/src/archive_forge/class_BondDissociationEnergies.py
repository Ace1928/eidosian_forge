...
class BondDissociationEnergies(MSONable):
    ...
    def __init__(self, molecule_entry: dict[str, str | dict[str, str | int]], fragment_entries: list[dict[str, str | dict[str, str | int]]], allow_additional_charge_separation: bool=False, multibreak: bool=False) -> None:
        """
        ...
        for key in required_keys:
            if key not in self.molecule_entry:
                raise RuntimeError(f'key={key!r} must be present in molecule entry! Exiting...')
            for entry in self.filtered_entries:
                if key not in entry:
                    raise RuntimeError(f'key={key!r} must be present in all fragment entries! Exiting...')
        final_charge = int(molecule_entry['final_molecule']['charge'])
        ...
        self.mol_graph = MoleculeGraph.from_local_env_strategy(Molecule.from_dict(molecule_entry['final_molecule']), OpenBabelNN())
        for bond in self.mol_graph.graph.edges:
            bonds = [(bond[0], bond[1])]
            self.fragment_and_process(bonds)
        if multibreak:
            warnings.warn('Breaking pairs of ring bonds. WARNING: Structure changes much more likely, meaning dissociation values are less reliable! This is a bad idea!')
            self.bond_pairs = []
