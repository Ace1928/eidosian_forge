...
class TemplateExporter(Exporter):
    ...
    def default_config(self):
        c = Config({'RegexRemovePreprocessor': {'enabled': True}, 'TagRemovePreprocessor': {'enabled': True}})
    ...
    template_file = Unicode(None, allow_none=True, help='Name of the template file to use').tag(config=True, affects_template=True)
    ...
    template_data_paths = List(jupyter_path('nbconvert', 'templates'), help='Path where templates can be installed too.').tag(affects_environment=True)
    ...
    exclude_input = Bool(False, help='This allows you to exclude code cell inputs from all templates if set to True.').tag(config=True)
    exclude_input_prompt = Bool(False, help='This allows you to exclude input prompts from all templates if set to True.').tag(config=True)
    exclude_output = Bool(False, help='This allows you to exclude code cell outputs from all templates if set to True.').tag(config=True)
    exclude_output_prompt = Bool(False, help='This allows you to exclude output prompts from all templates if set to True.').tag(config=True)
    exclude_output_stdin = Bool(True, help='This allows you to exclude output of stdin stream from lab template if set to True.').tag(config=True)
    exclude_code_cell = Bool(False, help='This allows you to exclude code cells from all templates if set to True.').tag(config=True)
    exclude_markdown = Bool(False, help='This allows you to exclude markdown cells from all templates if set to True.').tag(config=True)
    exclude_raw = Bool(False, help='This allows you to exclude raw cells from all templates if set to True.').tag(config=True)
    exclude_unknown = Bool(False, help='This allows you to exclude unknown cells from all templates if set to True.').tag(config=True)
    extra_loaders: List[t.Any] = List(help='Jinja loaders to find templates. Will be tried in order before the default FileSystem ones.').tag(affects_environment=True)
    filters = Dict(help='Dictionary of filters, by name and namespace, to add to the Jinja\n        environment.').tag(config=True, affects_environment=True)
    raw_mimetypes = List(Unicode(), help="formats of raw cells to be included in this Exporter's output.").tag(config=True)
            ...
            if not found_at_least_one:
                for root_dir in root_dirs:
                    ...
                    if os.path.exists(compatibility_path):
                        found_at_least_one = True
                        warnings.warn(f"5.x template name passed '{self.template_name}'. Use 'lab' or 'classic' for new template usage.", DeprecationWarning, stacklevel=2)
