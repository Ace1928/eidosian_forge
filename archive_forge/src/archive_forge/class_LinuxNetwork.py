...
class LinuxNetwork(Network):
    ...
    """
    platform = 'Linux'
    INTERFACE_TYPE = {'1': 'ether', '32': 'infiniband', '512': 'ppp', '772': 'loopback', '65534': 'tunnel'}
    ...
    def populate(self, collected_facts=None):
        ...
        default_ipv4, default_ipv6 = self.get_default_interfaces(ip_path, collected_facts=collected_facts)
    ...
    def get_default_interfaces(self, ip_path, collected_facts=None):
        ...
        command = dict(v4=[ip_path, '-4', 'route', 'get', '8.8.8.8'], v6=[ip_path, '-6', 'route', 'get', '2404:6800:400a:800::1012'])
        interface = dict(v4={}, v6={})
        for v in ('v4', 'v6'):
            if v == 'v6' and collected_facts.get('ansible_os_family') == 'RedHat' and collected_facts.get('ansible_distribution_version', '').startswith('4.'):
                continue
                ...
                for line in output.splitlines():
                    ...
                    if words[0] == 'inet':
                        ...
                        if not secondary and 'ipv4' not in interfaces[iface]:
                            interfaces[iface]['ipv4'] = {'address': address, 'broadcast': broadcast, 'netmask': netmask, 'network': network, 'prefix': netmask_length}
                        else:
                            if 'ipv4_secondaries' not in interfaces[iface]:
                                interfaces[iface]['ipv4_secondaries'] = []
                            interfaces[iface]['ipv4_secondaries'].append({'address': address, 'broadcast': broadcast, 'netmask': netmask, 'network': network, 'prefix': netmask_length})
                        if secondary:
                            if 'ipv4_secondaries' not in interfaces[device]:
                                interfaces[device]['ipv4_secondaries'] = []
                            if device != iface:
                                interfaces[device]['ipv4_secondaries'].append({'address': address, 'broadcast': broadcast, 'netmask': netmask, 'network': network, 'prefix': netmask_length})
                        ...
                        interfaces[device]['ipv6'].append({'address': address, 'prefix': prefix, 'scope': scope})
