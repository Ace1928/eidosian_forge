...
class EC2Tests(LibcloudTestCase, TestCaseMixin):
    ...
    def test_create_node_with_ex_mincount(self):
        image = NodeImage(id='ami-be3adfd7', name=self.image_name, driver=self.driver)
        size = NodeSize('m1.small', 'Small Instance', None, None, None, None, driver=self.driver)
        node = self.driver.create_node(name='foo', image=image, size=size, ex_mincount=1, ex_maxcount=10)
        self.assertEqual(node.id, 'i-2ba64342')
        self.assertEqual(node.name, 'foo')
        self.assertEqual(node.extra['tags']['Name'], 'foo')
        self.assertEqual(len(node.extra['tags']), 1)
    ...
    def test_create_node_with_metadata(self):
        image = NodeImage(id='ami-be3adfd7', name=self.image_name, driver=self.driver)
        size = NodeSize('m1.small', 'Small Instance', None, None, None, None, driver=self.driver)
        node = self.driver.create_node(name='foo', image=image, size=size, ex_metadata={'Bar': 'baz', 'Num': '42'})
        self.assertEqual(node.name, 'foo')
        self.assertEqual(node.extra['tags']['Name'], 'foo')
    ...
    def test_list_nodes(self):
        ...
        self.assertEqual(node.extra['block_device_mapping'][0]['ebs']['attach_time'], datetime(2013, 4, 9, 18, 1, 1, tzinfo=UTC))
    ...
    def test_ex_create_node_with_ex_blockdevicemappings_attribute_error(self):
        ...
        self.assertRaises(AttributeError, self.driver.create_node, name='foo', image=image, size=size, ex_blockdevicemappings=mappings)
    ...
    def test_list_sizes(self):
        ...
        names = [('ec2_us_east', 'us-east-1'), ('ec2_us_west', 'us-west-1'), ('ec2_us_west', 'us-west-2'), ('ec2_eu_west', 'eu-west-1'), ('ec2_ap_southeast', 'ap-southeast-1'), ('ec2_ap_northeast', 'ap-northeast-1'), ('ec2_ap_southeast_2', 'ap-southeast-2'), ('ec2_ap_south_1', 'ap-south-1')]
    ...
    def test_ex_create_node_with_ex_iam_profile(self):
        iamProfile = {'id': 'AIDGPMS9RO4H3FEXAMPLE', 'name': 'Foo', 'arn': 'arn:aws:iam:...'}
        image = NodeImage(id='ami-be3adfd7', name=self.image_name, driver=self.driver)
        size = NodeSize('m1.small', 'Small Instance', None, None, None, None, driver=self.driver)
        EC2MockHttp.type = None
        node1 = self.driver.create_node(name='foo', image=image, size=size)
        EC2MockHttp.type = 'ex_iam_profile'
        node2 = self.driver.create_node(name='bar', image=image, size=size, ex_iamprofile=iamProfile['name'])
        node3 = self.driver.create_node(name='bar', image=image, size=size, ex_iamprofile=iamProfile['arn'])
    ...
    def test_ex_create_node_with_ex_spot(self):
        ...
        node = self.driver.create_node(name='foo', image=image, size=size, ex_spot=True, ex_spot_max_price=1.5)
        self.assertEqual(node.extra['instance_lifecycle'], 'spot')
    ...
    def test_copy_image(self):
        ...
        resp = self.driver.copy_image(image, 'us-east-1', name='Faux Image', description='Test Image Copy')
        self.assertEqual(resp.id, 'ami-4db38224')

    def test_create_image(self):
        node = self.driver.list_nodes()[0]
        mapping = [{'VirtualName': None, 'Ebs': {'VolumeSize': 10, 'VolumeType': 'standard', 'DeleteOnTermination': 'true'}, 'DeviceName': '/dev/sda1'}]
        resp = self.driver.create_image(node, 'New Image', description='New EBS Image', block_device_mapping=mapping)
    ...
    def test_wait_for_import_snapshot_completion(self):
        snap = self.driver._wait_for_import_snapshot_completion(import_task_id='import-snap-fhdysyq6')
        self.assertEqual(snap.id, 'snap-0ea83e8a87e138f39')
    ...
    def test_ex_associate_address_with_node(self):
        ...
        self.assertRaises(AttributeError, self.driver.ex_associate_address_with_node, node, elastic_ips[1], domain='bogus')
    ...
    def test_ex_disassociate_address(self):
        ...
        self.assertTrue(ret)
        self.assertRaises(AttributeError, self.driver.ex_disassociate_address, elastic_ips[1], domain='bogus')
    ...
    def test_create_volume(self):
        ...
        self.assertRaisesRegex(ValueError, expected_msg, self.driver.create_volume, 10, 'invalid-vol', location=location, ex_volume_type='invalid-type')
    ...
    def test_detach(self):
        vol = StorageVolume(id='vol-4282672b', name='test', state=StorageVolumeState.INUSE, size=10, driver=self.driver)
    ...
    def test_create_node_ex_security_groups(self):
        ...
        self.driver.create_node(name='foo', image=image, size=size, ex_securitygroup=security_groups)
        self.driver.create_node(name='foo', image=image, size=size, ex_security_groups=security_groups)
        self.assertRaises(ValueError, self.driver.create_node, name='foo', image=image, size=size, ex_securitygroup=security_groups, ex_security_groups=security_groups)
    ...
    def test_ex_create_security_group(self):
        group = self.driver.ex_create_security_group('WebServers', 'Rules to protect web nodes', 'vpc-143cab4')
    ...
    def test_ex_list_networks_filters(self):
        ...
        filters = OrderedDict([('dhcp-options-id', 'dopt-7eded312'), ('cidr', '192.168.51.0/24')])
        self.driver.ex_list_networks(filters=filters)

    def test_ex_create_network(self):
        vpc = self.driver.ex_create_network('192.168.55.0/24', name='Test VPC', instance_tenancy='default')
    ...
    def test_ex_create_subnet(self):
        subnet = self.driver.ex_create_subnet('vpc-532135d1', '192.168.51.128/26', 'us-east-1b', name='Test Subnet')
    ...
    def test_ex_create_network_interface(self):
        subnet = self.driver.ex_list_subnets()[0]
        interface = self.driver.ex_create_network_interface(subnet, name='Test Interface', description='My Test')
    ...
    def test_params_is_not_simple_type_exception_is_thrown(self):
        ...
        self.assertRaisesRegex(ValueError, expected_msg, self.driver.connection.request, '/', params=params)
