...
class SymmetryEquivalenceCheck:
    ...
    def __init__(self, angle_tol=1.0, ltol=0.05, stol=0.05, vol_tol=0.1, scale_volume=False, to_primitive=False):
    ...
    def _expand(self, ref_atoms, tol=0.0001):
        ...
        normal_vectors = np.array([np.cross(cell[1, :], cell[2, :]), np.cross(cell[0, :], cell[2, :]), np.cross(cell[0, :], cell[1, :])])
        normalize(normal_vectors)
        pos2faces = np.abs(positions.dot(normal_vectors.T))
        pos2oppofaces = np.abs(np.dot(positions - np.sum(cell, axis=0), normal_vectors.T))
