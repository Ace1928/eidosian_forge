...
def _sync_filters_if_needed(self, *, central_worker: EnvRunner, workers: WorkerSet, config: AlgorithmConfig) -> None:
    ...
    if central_worker and config.observation_filter != 'NoFilter':
        FilterManager.synchronize(central_worker.filters, workers, update_remote=config.update_worker_filter_stats, timeout_seconds=config.sync_filters_on_rollout_workers_timeout_s, use_remote_data_for_update=config.use_worker_filter_stats)