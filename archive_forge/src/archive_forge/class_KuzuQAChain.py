...
class KuzuQAChain(Chain):
    ...
    *Security note*: Make sure that the database connection uses credentials
    ...
    @classmethod
    def from_llm(cls, llm: BaseLanguageModel, *, qa_prompt: BasePromptTemplate=CYPHER_QA_PROMPT, cypher_prompt: BasePromptTemplate=KUZU_GENERATION_PROMPT, **kwargs: Any) -> KuzuQAChain:
        """Initialize from LLM."""
    ...
    def _call(self, inputs: Dict[str, Any], run_manager: Optional[CallbackManagerForChainRun]=None) -> Dict[str, str]:
        ...
        generated_cypher = self.cypher_generation_chain.run({'question': question, 'schema': self.graph.get_schema}, callbacks=callbacks)
