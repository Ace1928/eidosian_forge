...
class BaseWindowsConnectorTestCase(test_base.WindowsConnectorTestBase):
    ...
    @mock.patch.object(base_win_conn.BaseWindowsConnector, 'check_multipath_support')
    @ddt.unpack
    def test_get_connector_properties(self, mock_check_mpio, mpio_requested=True, mpio_available=True):
        mock_check_mpio.return_value = mpio_available
        enforce_multipath = False
        props = base_win_conn.BaseWindowsConnector.get_connector_properties(multipath=mpio_requested, enforce_multipath=enforce_multipath)
        self.assertEqual(mpio_requested and mpio_available, props['multipath'])
        if mpio_requested:
    ...
    @mock.patch('os_brick.initiator.windows.base.open', new_callable=mock.mock_open)
    def test_check_valid_device(self, exc, mock_open):
        mock_open.side_effect = exc
        valid_device = self._connector.check_valid_device(mock.sentinel.dev_path)
    ...
    def test_extend_volume(self, mock_get_vol_paths):
        ...
        self._diskutils.refresh_disk.assert_called_once_with(mock_get_dev_num.return_value)
