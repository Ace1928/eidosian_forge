    ...
    def run(self, dag: DAGCircuit):
        ...
        for idx1 in range(0, circ_size):
            ...
            for idx2 in range(idx1 - 1, -1, -1):
                if removed[idx2]:
                    continue
                if not self._skip_node(topo_sorted_nodes[idx2]) and topo_sorted_nodes[idx2].qargs == topo_sorted_nodes[idx1].qargs and (topo_sorted_nodes[idx2].cargs == topo_sorted_nodes[idx1].cargs):
                    is_inverse, phase = self._check_inverse(topo_sorted_nodes[idx1], topo_sorted_nodes[idx2])
                    if is_inverse:
                        phase_update += phase
                        matched_idx2 = idx2
                        break
                if not cc.commute(topo_sorted_nodes[idx1].op, topo_sorted_nodes[idx1].qargs, topo_sorted_nodes[idx1].cargs, topo_sorted_nodes[idx2].op, topo_sorted_nodes[idx2].qargs, topo_sorted_nodes[idx2].cargs, max_num_qubits=self._max_qubits):
                    break
            if matched_idx2 != -1:
                removed[idx1] = True
