...
class HuggingFaceHubEmbeddings(BaseModel, Embeddings):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        try:
            ...
            client = InferenceClient(model=values['model'], token=huggingfacehub_api_token)
            async_client = AsyncInferenceClient(model=values['model'], token=huggingfacehub_api_token)
            if values['task'] not in VALID_TASKS:
                raise ValueError(f'Got invalid task {values['task']}, currently only {VALID_TASKS} are supported')
