...
def stateless_case(branch_index: _atypes.TensorFuzzingAnnotation[_atypes.Int32], input, Tout, branches, output_shapes=[], name=None):
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'StatelessCase', name, branch_index, input, 'Tout', Tout, 'branches', branches, 'output_shapes', output_shapes)
            return _result
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('StatelessCase', branch_index=branch_index, input=input, Tout=Tout, branches=branches, output_shapes=output_shapes, name=name)
    _result = _outputs[:]
