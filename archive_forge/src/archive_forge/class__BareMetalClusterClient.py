...
class _BareMetalClusterClient(client.ClientBase):
    ...
    def _island_mode_cidr_config(self, args: parser_extensions.Namespace):
        ...
        kwargs = {'serviceAddressCidrBlocks': getattr(args, 'island_mode_service_address_cidr_blocks', []), 'podAddressCidrBlocks': getattr(args, 'island_mode_pod_address_cidr_blocks', [])}
        if any(kwargs.values()):
            return messages.BareMetalIslandModeCidrConfig(**kwargs)
        return None

    def _sr_iov_config(self, args: parser_extensions.Namespace):
        kwargs = {'enabled': self.sr_iov_config_enabled(args)}
    ...
    def _multiple_network_interfaces_config(self, args: parser_extensions.Namespace):
        kwargs = {'enabled': self.GetFlag(args, 'enable_multi_nic_config')}
        if self.IsSet(kwargs):
    ...
    def _network_config(self, args: parser_extensions.Namespace):
        ...
        kwargs = {'islandModeCidr': self._island_mode_cidr_config(args), 'advancedNetworking': self.GetFlag(args, 'enable_advanced_networking'), 'multipleNetworkInterfacesConfig': self._multiple_network_interfaces_config(args), 'srIovConfig': self._sr_iov_config(args)}
        if self.IsSet(kwargs):
    ...
    def _vip_config(self, args: parser_extensions.Namespace):
        ...
        kwargs = {'controlPlaneVip': getattr(args, 'control_plane_vip', None), 'ingressVip': getattr(args, 'ingress_vip', None)}
        if any(kwargs.values()):
    ...
    def _port_config(self, args: parser_extensions.Namespace):
        ...
        kwargs = {'controlPlaneLoadBalancerPort': getattr(args, 'control_plane_load_balancer_port', None)}
        if any(kwargs.values()):
    ...
    def _address_pools_from_file(self, args: parser_extensions.Namespace):
        ...
        if not address_pools:
            raise exceptions.BadArgumentException('--metal_lb_address_pools_from_file', 'Missing field [addressPools] in Metal LB address pools file.')
    ...
    def _address_pool(self, address_pool):
        ...
        if not addresses:
            raise exceptions.BadArgumentException('--metal_lb_address_pools_from_file', 'Missing field [addresses] in Metal LB address pools file.')
        pool = address_pool.get('pool', None)
        if not pool:
            raise exceptions.BadArgumentException('--metal_lb_address_pools_from_file', 'Missing field [pool] in Metal LB address pools file.')
    ...
    def _metal_lb_serialize_image_pulls_disabled(self, args: parser_extensions.Namespace):
        if 'enable_metal_lb_load_balancer_serialize_image_pulls' in args.GetSpecifiedArgsDict():
            return False
        elif 'disable_metal_lb_load_balancer_serialize_image_pulls' in args.GetSpecifiedArgsDict():
            return True
        else:
            return None
    ...
    def _manual_lb_config(self, args: parser_extensions.Namespace):
        ...
        kwargs = {'enabled': getattr(args, 'enable_manual_lb', False)}
        if any(kwargs.values()):
    ...
    def _load_balancer_config(self, args: parser_extensions.Namespace):
        ...
        kwargs = {'vipConfig': self._vip_config(args), 'portConfig': self._port_config(args), 'metalLbConfig': self._metal_lb_config(args), 'manualLbConfig': self._manual_lb_config(args), 'bgpLbConfig': self._bgp_lb_config(args)}
        if any(kwargs.values()):
    ...
    def _bgp_load_balancer_node_pool_config(self, args: parser_extensions.Namespace):
        """Constructs proto message BareMetalBgpLbConfig.BareMetalLoadBalancerNodePoolConfig."""
        kwargs = {'nodePoolConfig': self._bgp_node_pool_config(args)}
        if any(kwargs.values()):
            return messages.BareMetalLoadBalancerNodePoolConfig(**kwargs)
        return None
    ...
    def _bgp_serialize_image_pulls_disabled(self, args: parser_extensions.Namespace):
        if 'disable_bgp_load_balancer_serialize_image_pulls' in args.GetSpecifiedArgsDict():
            return True
        elif 'enable_bgp_load_balancer_serialize_image_pulls' in args.GetSpecifiedArgsDict():
            return False
        else:
            return None

    def _bgp_load_balancer_kubelet_config(self, args: parser_extensions.Namespace):
        ...
        kwargs = {'registryBurst': self.GetFlag(args, 'bgp_load_balancer_registry_burst'), 'registryPullQps': self.GetFlag(args, 'bgp_load_balancer_registry_pull_qps'), 'serializeImagePullsDisabled': self._bgp_serialize_image_pulls_disabled(args)}
        if self.IsSet(kwargs):
            return messages.BareMetalKubeletConfig(**kwargs)
        return None
    ...
    def _lvp_share_config(self, args: parser_extensions.Namespace):
        ...
        kwargs = {'lvpConfig': self._lvp_config(args), 'sharedPathPvCount': getattr(args, 'lvp_share_path_pv_count', None)}
        if any(kwargs.values()):
    ...
    def _lvp_node_mounts_config(self, args: parser_extensions.Namespace):
        """Constructs proto message BareMetalLvpConfig."""
        kwargs = {'path': getattr(args, 'lvp_node_mounts_config_path', None), 'storageClass': getattr(args, 'lvp_node_mounts_config_storage_class', None)}
        if any(kwargs.values()):
            return messages.BareMetalLvpConfig(**kwargs)
        return None
    ...
    def _node_pool_config(self, args: parser_extensions.Namespace):
        ...
        kwargs = {'nodeConfigs': node_configs, 'labels': self._control_plane_node_labels(args), 'taints': self._control_plane_node_taints(args), 'kubeletConfig': self._control_plane_kubelet_config(args)}
        if any(kwargs.values()):
            return messages.BareMetalNodePoolConfig(**kwargs)
        return None
    ...
    def _api_server_args(self, args: parser_extensions.Namespace):
        ...
        if api_server_args_flag_value:
            for key, val in api_server_args_flag_value.items():
                api_server_args.append(messages.BareMetalApiServerArgument(argument=key, value=val))
    ...
    def _proxy_config(self, args: parser_extensions.Namespace):
        ...
        kwargs = {'uri': getattr(args, 'uri', None), 'noProxy': getattr(args, 'no_proxy', [])}
        if any(kwargs.values()):
    ...
    def _workload_node_config(self, args: parser_extensions.Namespace):
        ...
        kwargs = {'containerRuntime': self._container_runtime(container_runtime), 'maxPodsPerNode': getattr(args, 'max_pods_per_node', None)}
        if any(kwargs.values()):
