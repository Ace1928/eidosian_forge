...
class TensorFlowTestCase(googletest.TestCase):
    ...
    def tearDown(self):
        if self._test_start_time is not None:
            logging.info('time(%s): %ss', self.id(), round(time.time() - self._test_start_time, 2))
    ...
    def assertProtoEquals(self, expected_message_maybe_ascii, message, msg=None, relative_tolerance=None):
    ...
    """
        if isinstance(expected_message_maybe_ascii, type(message)):
            ...
            self._AssertProtoEquals(expected_message, message, msg=msg, relative_tolerance=relative_tolerance)
            ...
            assert False, "Can't compare protos of type %s and %s." % (type(expected_message_maybe_ascii), type(message))

    def assertProtoEqualsVersion(self, expected, actual, producer=versions.GRAPH_DEF_VERSION, min_consumer=versions.GRAPH_DEF_VERSION_MIN_CONSUMER, msg=None):
        expected = 'versions { producer: %d min_consumer: %d };\n%s' % (producer, min_consumer, expected)
        ...
        if tensor is None:
            ...
            try:
                if sparse_tensor.is_sparse(tensor):
                    return sparse_tensor.SparseTensorValue(tensor.indices.numpy(), tensor.values.numpy(), tensor.dense_shape.numpy())
                elif ragged_tensor.is_ragged(tensor):
                    return ragged_tensor_value.RaggedTensorValue(self._eval_tensor(tensor.values), self._eval_tensor(tensor.row_splits))
                elif isinstance(tensor, indexed_slices.IndexedSlices):
                    return indexed_slices.IndexedSlicesValue(values=tensor.values.numpy(), indices=tensor.indices.numpy(), dense_shape=None if tensor.dense_shape is None else tensor.dense_shape.numpy())
                elif hasattr(tensor, 'numpy') and callable(tensor.numpy):
                    return tensor.numpy()
                else:
                    return nest.map_structure(lambda t: t.numpy(), tensor, expand_composites=True)
    ...
    def test_session(self, graph=None, config=None, use_gpu=True, force_gpu=False):
        ...
        if self.id().endswith('.test_session'):
            self.skipTest('Tests that have the name "test_session" are automatically skipped by TensorFlow test fixture, as the name is reserved for creating sessions within tests. Please rename your test if you have a test with this name.')
        if context.executing_eagerly():
            yield None
        elif graph is None:
            sess = self._get_cached_session(graph, config, force_gpu, crash_if_inconsistent_args=False)
            with self._constrain_devices_and_set_default(sess, use_gpu, force_gpu) as cached:
                yield cached
        else:
    ...
    def evaluate_if_both_tensors(self, a, b):
        if tensor_util.is_tf_type(a) and tensor_util.is_tf_type(b) and (not isinstance(a, ops._EagerTensorBase)) and (not isinstance(b, ops._EagerTensorBase)):
            return self.evaluate((a, b))
        else:
            return (a, b)

    def _assertArrayLikeAllClose(self, a, b, rtol=1e-06, atol=1e-06, msg=None):
        a, b = self.evaluate_if_both_tensors(a, b)
        a = self._GetNdArray(a)
        b = self._GetNdArray(b)
        if a.shape != b.shape and (b.ndim <= 3 or b.size < 500):
            shape_mismatch_msg = 'Shape mismatch: expected %s, got %s with contents %s.' % (a.shape, b.shape, b)
        else:
            shape_mismatch_msg = 'Shape mismatch: expected %s, got %s.' % (a.shape, b.shape)
        self.assertEqual(a.shape, b.shape, shape_mismatch_msg)
        msgs = [msg]
        a_dtype = a.dtype
        custom_dtypes = (dtypes.bfloat16.as_numpy_dtype, dtypes.float8_e5m2.as_numpy_dtype, dtypes.float8_e4m3fn.as_numpy_dtype)
    ...
    @py_func_if_in_function
    def assertAllClose(self, a, b, rtol=1e-06, atol=1e-06, msg=None):
    ...
    """
        ...
        if np.size(diff):
            raise AssertionError('%d unique element(s) are not in the set %s: %s' % (np.size(diff), expected_set, diff))
    ...
    """
        ...
        try:
            ...
            if not isinstance(e, exception_type) or not predicate(e):
                raise AssertionError('Exception of type %s: %s' % (str(type(e)), str(e)))

    def assertRaisesOpError(self, expected_err_re_or_predicate):
        return self.assertRaisesWithPredicateMatch(errors.OpError, expected_err_re_or_predicate)

    def assertRaisesIncompatibleShapesError(self, exception_type=errors.InvalidArgumentError):
        return self.assertRaisesWithPredicateMatch(exception_type, 'Incompatible shapes|Dimensions must be equal|required broadcastable shapes')
    ...
    """
        ...
        self.assertEqual(device1, device2, 'Devices %s and %s are not equal. %s' % (device1, device2, msg))
    ...
    def _assertListCloseRecursive(self, a, b, rtol, atol, msg, path='value'):
        ...
        if isinstance(a, (list, tuple)):
            ...
            for i in range(len(a)):
                self._assertListCloseRecursive(a[i], b[i], rtol, atol, msg, '%s[%s]' % (path, i))
        else:
    ...
    def _get_cached_session(self, graph=None, config=None, force_gpu=False, crash_if_inconsistent_args=True):
        ...
        if self._cached_session is None:
            ...
            if crash_if_inconsistent_args and self._cached_graph is not graph:
                raise ValueError('The graph used to get the cached session is different than the one that was used to create the session. Maybe create a new session with self.session()')
            if crash_if_inconsistent_args and self._cached_config is not config:
                raise ValueError('The config used to get the cached session is different than the one that was used to create the session. Maybe create a new session with self.session()')
            if crash_if_inconsistent_args and self._cached_force_gpu is not force_gpu:
                raise ValueError('The force_gpu value used to get the cached session is different than the one that was used to create the session. Maybe create a new session with self.session()')
