...
class FSDPPrecision(Precision):
    ...
    def __init__(self, precision: _PRECISION_INPUT, scaler: Optional['ShardedGradScaler']=None) -> None:
        ...
        if precision not in supported_precision:
            raise ValueError(f'`precision={precision!r})` is not supported in FSDP. `precision` must be one of: {supported_precision}.')
        ...
        precision_to_type = {'bf16-mixed': torch.float32, '16-mixed': torch.float32, 'bf16-true': torch.bfloat16, '16-true': torch.float16, '32-true': torch.float32}
