...
def _get_metric_from_line(metric_name, metric_line):
    ...
    if len(metric_parts) != 2 and len(metric_parts) != 3:
        raise MlflowException(f"Metric '{metric_name}' is malformed; persisted metric data contained {len(metric_parts)} fields. Expected 2 or 3 fields.", databricks_pb2.INTERNAL_ERROR)
