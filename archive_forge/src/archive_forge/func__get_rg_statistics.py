...
def _get_rg_statistics(row_group, col_names):
    ...
    """
    row_group_schema = {col_name: i for i, col_name in enumerate(row_group.schema.names)}
    ...
    return {name: stats for name, stats in map(name_stats, col_names) if stats is not None}