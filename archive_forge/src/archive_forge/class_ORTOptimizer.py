...
class ORTOptimizer:
    ...
    def __init__(self, onnx_model_path: List[os.PathLike], config: 'PretrainedConfig', from_ortmodel: bool=False):
        ...
        try:
            ...
            raise NotImplementedError(f'Tried to use ORTOptimizer for the model type {self.model_type}, but it is not available yet. Please open an issue or submit a PR at https://github.com/huggingface/optimum.')
    ...
    def from_pretrained(cls, model_or_path: Union[str, os.PathLike, ORTModel], file_names: Optional[List[str]]=None) -> 'ORTOptimizer':
        ...
        if isinstance(model_or_path, ORTModel):
            ...
            if isinstance(model_or_path, ORTModelForConditionalGeneration):
                onnx_model_path += [model_or_path.encoder_model_path, model_or_path.decoder_model_path]
                ...
                raise NotImplementedError('ORTOptimizer does not support ORTModelForCausalLM models when without/with past models are merged. Please re-export your model. This can be done by using the optimum-cli ONNX export tool or `ORTModelForCausalLM.from_pretrained(..., export=True, use_merged=False)`.')
            else:
    ...
    def optimize(self, optimization_config: OptimizationConfig, save_dir: Union[str, os.PathLike], file_suffix: Optional[str]='optimized', use_external_data_format: Optional[bool]=None, one_external_file: bool=True):
        """
        ...
        if use_external_data_format is not None:
            logger.warning('The argument use_external_data_format in the ORTOptimizer.optimize() method is deprecated and will be removed in optimum 2.0.')
        ...
        del onnx_model
        gc.collect()
        ort_config = ORTConfig(optimization=optimization_config, use_external_data_format=model_uses_external_data, one_external_file=one_external_file)
        for model_path in self.onnx_model_path:
            suffix = f'_{file_suffix}' if file_suffix else ''
            output_path = save_dir.joinpath(f'{model_path.stem}{suffix}').with_suffix(model_path.suffix)
            try:
                optimizer = optimize_model(model_path.as_posix(), model_type, self.normalized_config.num_attention_heads, self.normalized_config.hidden_size, opt_level=optimization_config.optimization_level, optimization_options=optimization_options, use_gpu=optimization_config.optimize_for_gpu, only_onnxruntime=not optimization_config.enable_transformers_specific_optimizations)
                if optimization_config.fp16:
                    if model_uses_external_data:
                        optimizer.save_model_to_file(output_path.as_posix(), use_external_data_format=model_uses_external_data, all_tensors_to_one_file=one_external_file)
                        optimizer.model = output_path.as_posix()
                    optimizer.convert_float_to_float16(use_symbolic_shape_infer=not optimization_config.disable_shape_inference, keep_io_types=True)
            except Exception as e:
                if 'Incomplete symbolic shape inference' in str(e):
                    err = RuntimeError(f'{str(e)}. Try to set `disable_shape_inference=True` in your optimization configuration.')
                    raise err from e
                raise
            optimizer.save_model_to_file(output_path.as_posix(), use_external_data_format=model_uses_external_data, all_tensors_to_one_file=one_external_file)
        ...
        logger.info(f'Optimized model saved at: {save_dir} (external data format: {model_uses_external_data}; saved all tensor to one file: {one_external_file})')
