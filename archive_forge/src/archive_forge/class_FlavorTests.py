...
class FlavorTests(base.TestCase):
    ...
    def setUpClass(cls):
        ...
        cmd_output = cls.openstack('project create --enable ' + cls.PROJECT_NAME, parse_output=True)
    ...
    def test_flavor_delete(self):
        ...
        cmd_output = self.openstack('flavor create ' + '--project ' + self.PROJECT_NAME + ' ' + '--private ' + name1, parse_output=True)
        self.assertIsNotNone(cmd_output['id'])
        name2 = uuid.uuid4().hex
        cmd_output = self.openstack('flavor create ' + '--id qaz ' + '--project ' + self.PROJECT_NAME + ' ' + '--private ' + name2, parse_output=True)
        self.assertIsNotNone(cmd_output['id'])
        self.assertEqual('qaz', cmd_output['id'])
        raw_output = self.openstack('flavor delete ' + name1 + ' ' + name2)
        self.assertOutput('', raw_output)

    def test_flavor_list(self):
        """Test create defaults, list filters, delete"""
        name1 = uuid.uuid4().hex
        cmd_output = self.openstack('flavor create ' + '--property a=b ' + '--property c=d ' + name1, parse_output=True)
        self.addCleanup(self.openstack, 'flavor delete ' + name1)
        self.assertIsNotNone(cmd_output['id'])
        self.assertEqual(name1, cmd_output['name'])
        name2 = uuid.uuid4().hex
        cmd_output = self.openstack('flavor create ' + '--id qaz ' + '--ram 123 ' + '--private ' + '--property a=b2 ' + '--property b=d2 ' + name2, parse_output=True)
        ...
        self.assertEqual(0, cmd_output['disk'])
        self.assertFalse(cmd_output['os-flavor-access:is_public'])
        self.assertDictEqual({'a': 'b2', 'b': 'd2'}, cmd_output['properties'])
        cmd_output = self.openstack('flavor list', parse_output=True)
        col_name = [x['Name'] for x in cmd_output]
        self.assertIn(name1, col_name)
        self.assertNotIn(name2, col_name)
        cmd_output = self.openstack('flavor list ' + '--long', parse_output=True)
        ...
        cmd_output = self.openstack('flavor list ' + '--public', parse_output=True)
        col_name = [x['Name'] for x in cmd_output]
        self.assertIn(name1, col_name)
        self.assertNotIn(name2, col_name)
        cmd_output = self.openstack('flavor list ' + '--private', parse_output=True)
        col_name = [x['Name'] for x in cmd_output]
        self.assertNotIn(name1, col_name)
        self.assertIn(name2, col_name)
        cmd_output = self.openstack('flavor list ' + '--all', parse_output=True)
        col_name = [x['Name'] for x in cmd_output]
        self.assertIn(name1, col_name)
        self.assertIn(name2, col_name)
