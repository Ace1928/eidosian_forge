...
class LanguageModel(metaclass=ABCMeta):
    ...
    def __init__(self, order, vocabulary=None, counter=None):
        ...
        if vocabulary and (not isinstance(vocabulary, Vocabulary)):
            warnings.warn(f'The `vocabulary` argument passed to {self.__class__.__name__!r} must be an instance of `nltk.lm.Vocabulary`.', stacklevel=3)
    ...
    def score(self, word, context=None):
        ...
        method.
        """
        return self.unmasked_score(self.vocab.lookup(word), self.vocab.lookup(context) if context else None)
    ...
    def generate(self, num_words=1, text_seed=None, random_seed=None):
        ...
        if num_words == 1:
            ...
            return _weighted_choice(samples, tuple((self.score(w, context) for w in samples)), random_generator)
        generated = []
        for _ in range(num_words):
            generated.append(self.generate(num_words=1, text_seed=text_seed + generated, random_seed=random_generator))
        return generated