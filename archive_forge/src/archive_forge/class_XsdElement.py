...
class XsdElement(XsdComponent, ParticleMixin, ElementPathMixin[SchemaElementType], ValidationMixin[ElementType, Any]):
    """
    ...
    def __init__(self, elem: Element, schema: SchemaType, parent: Optional[XsdComponent]=None, build: bool=True) -> None:
        if not build:
            self._build = False
    ...
    def __repr__(self) -> str:
        return '%s(%s=%r, occurs=%r)' % (self.__class__.__name__, 'name' if self.ref is None else 'ref', self.prefixed_name, list(self.occurs))
    ...
    def _parse_attributes(self) -> None:
        ...
        if 'block' in attrib:
            try:
                self._block = get_xsd_derivation_attribute(self.elem, 'block', XSD_ELEMENT_DERIVATIONS)
        ...
        if type_name is not None:
            try:
                ...
                if child is not None and child.tag in (XSD_COMPLEX_TYPE, XSD_SIMPLE_TYPE):
                    msg = _("the attribute 'type' and a xs:{} local declaration are mutually exclusive")
    ...
    @property
    def built(self) -> bool:
        return hasattr(self, 'type') and (self.type.parent is None or self.type.built) and all((c.built for c in self.identities))
    ...
    def get_binding(self, *bases: Type[Any], replace_existing: bool=False, **attrs: Any) -> DataBindingType:
        """
        ...
        if self.binding is None or replace_existing:
            ...
            self.binding = cast(DataBindingType, dataobjects.DataBindingMeta(class_name, bases, attrs))
        return self.binding
    ...
    def get_path(self, ancestor: Optional[XsdComponent]=None, reverse: bool=False) -> Optional[str]:
        """
    ...
    def iter_decode(self, obj: ElementType, validation: str='lax', **kwargs: Any) -> IterDecodeType[Any]:
        ...
        if XSI_TYPE in obj.attrib and self.schema.meta_schema is not None:
            ...
            try:
                ...
                if self.identities:
                    ...
                    for identity in self.identities:
                        if isinstance(identity.elements, tuple) or identity.selector is None:
                            continue
                        elif identity.selector.token is None:
                            raise XMLSchemaNotBuiltError(identity, 'identity selector is not built')
                        context = XPathContext(root=self.schema.xpath_node, item=cast(XsdElementProtocol, xpath_element))
        ...
        if nilled:
            ...
            if self.fixed is not None and (len(obj) > 0 or (value is not None and self.fixed != value)):
    ...
    def to_objects(self, obj: ElementType, with_bindings: bool=False, **kwargs: Any) -> DecodeType['dataobjects.DataElement']:
        """
    ...
    def iter_encode(self, obj: Any, validation: str='lax', **kwargs: Any) -> IterEncodeType[ElementType]:
        """
        ...
        if isinstance(xsd_type, XsdSimpleType):
            ...
            if element_data.text is not None:
                for result in xsd_type.content.iter_encode(element_data.text, validation, **kwargs):
                    if isinstance(result, XMLSchemaValidationError):
                        errors.append(result)
                    else:
                        text = result
    ...
    def is_matching(self, name: Optional[str], default_namespace: Optional[str]=None, group: Optional['XsdGroup']=None, **kwargs: Any) -> bool:
        if not name:
            return False
            ...
            if not self.qualified and default_namespace == self.target_namespace:
                return name == self.qualified_name or any((name == e.qualified_name for e in self.iter_substitutes()))
        return name == self.name or any((name == e.name for e in self.iter_substitutes()))

    def match(self, name: Optional[str], default_namespace: Optional[str]=None, **kwargs: Any) -> Optional['XsdElement']:
        if not name:
            return None
        elif default_namespace and name[0] != '{':
            name = f'{{{default_namespace}}}{name}'
    ...
    def is_restriction(self, other: ModelParticleType, check_occurs: bool=True) -> bool:
        ...
        if isinstance(other, XsdAnyElement):
            ...
            if self.name != other.name:
                if other.name == self.substitution_group and other.min_occurs != other.max_occurs and (self.max_occurs != 0) and (not other.abstract) and (self.xsd_version == '1.0'):
                    return False
            ...
            if check_occurs and (not self.has_occurs_restriction(other)):
                ...
                return True
            elif not self.is_consistent(other) and self.type.elem is not other.type.elem and (not self.type.is_derived(other.type, 'restriction')) and (not other.type.abstract):
                return False
            elif other.fixed is not None and (self.fixed is None or self.type.normalize(self.fixed) != other.type.normalize(other.fixed)):
                return False
