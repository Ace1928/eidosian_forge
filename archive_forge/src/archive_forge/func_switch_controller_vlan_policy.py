...
def switch_controller_vlan_policy(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('switch-controller', 'vlan-policy', filtered_data, vdom=vdom)
    ...
    if state == 'present' or state is True:
        return fos.set('switch-controller', 'vlan-policy', data=filtered_data, vdom=vdom)
    elif state == 'absent':
        return fos.delete('switch-controller', 'vlan-policy', mkey=filtered_data['name'], vdom=vdom)
    else:
