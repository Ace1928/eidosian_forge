...
class GraphQLTypeMap(OrderedDict):
    ...
    def is_possible_type(self, abstract_type, possible_type):
        ...
        assert isinstance(possible_types, Sequence), ('Could not find possible implementing types for ${} in ' + 'schema. Check that schema.types is defined and is an array of' + 'all possible types in the schema.').format(abstract_type)
    ...
    def reducer(cls, map, type):
        ...
        if type.name in map:
            assert map[type.name] == type, 'Schema must contain unique named types but contains multiple types named "{}".'.format(type.name)
            return map
        map[type.name] = type
        reduced_map = map
        ...
        if isinstance(type, (GraphQLObjectType, GraphQLInterfaceType, GraphQLInputObjectType)):
            ...
            for field_name, field in field_map.items():
                if type_is_input:
                    assert isinstance(field, GraphQLInputObjectField), '{}.{} must be an instance of GraphQLInputObjectField.'.format(type, field_name)
                    assert is_input_type(field.type), '{}.{} field type must be Input Type but got: {}.'.format(type, field_name, field.type)
                else:
                    assert isinstance(field, (GraphQLField, GraphQLField)), '{}.{} must be an instance of GraphQLField.'.format(type, field_name)
                    assert is_output_type(field.type), '{}.{} field type must be Output Type but got: {}.'.format(type, field_name, field.type)
                    for arg_name, arg in field.args.items():
                        assert isinstance(arg, (GraphQLArgument, GraphQLArgument)), '{}.{}({}:) argument must be an instance of GraphQLArgument.'.format(type, field_name, arg_name)
                        assert is_input_type(arg.type), '{}.{}({}:) argument type must be Input Type but got: {}.'.format(type, field_name, arg_name, arg.type)
