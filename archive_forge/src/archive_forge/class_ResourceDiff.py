...
class ResourceDiff(object):
    ...
    """
        ...
        printer = resource_printer.Printer(print_format, out=buff_original, defaults=defaults)
        printer.PrintSingleRecord(self.original)
        buff_changed = io.StringIO()
        printer = resource_printer.Printer(print_format, out=buff_changed, defaults=defaults)
