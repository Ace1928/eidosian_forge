...
class NetAppESeriesMgmtInterface(NetAppESeriesModule):
    ...
    def __init__(self):
        ansible_options = dict(state=dict(type='str', choices=['enabled', 'disabled'], required=False), controller=dict(type='str', required=True, choices=['A', 'B']), port=dict(type='int'), address=dict(type='str', required=False), subnet_mask=dict(type='str', required=False), gateway=dict(type='str', required=False), config_method=dict(type='str', required=False, choices=['dhcp', 'static']), dns_config_method=dict(type='str', required=False, choices=['dhcp', 'static']), dns_address=dict(type='str', required=False), dns_address_backup=dict(type='str', required=False), ntp_config_method=dict(type='str', required=False, choices=['disabled', 'dhcp', 'static']), ntp_address=dict(type='str', required=False), ntp_address_backup=dict(type='str', required=False), ssh=dict(type='bool', required=False))
        required_if = [['config_method', 'static', ['port', 'address', 'subnet_mask']], ['dns_config_method', 'static', ['dns_address']], ['ntp_config_method', 'static', ['ntp_address']]]
        super(NetAppESeriesMgmtInterface, self).__init__(ansible_options=ansible_options, web_services_version='02.00.0000.0000', required_if=required_if, supports_check_mode=True)
    ...
    def update_body_ntp_server_settings(self):
        ...
        if self.ntp_config_method == 'disabled':
            ...
            ntp_servers = []
            if self.ntp_address and self.ntp_address_backup and (not self.interface_info['ntp_servers'] or len(self.interface_info['ntp_servers']) != 2) or (self.ntp_address and (not self.ntp_address_backup) and (not self.interface_info['ntp_servers'] or len(self.interface_info['ntp_servers']) != 1)):
                change_required = True
            if self.ntp_address:
                if is_ipv4(self.ntp_address):
                    ntp_servers.append({'addrType': 'ipvx', 'ipvxAddress': {'addressType': 'ipv4', 'ipv4Address': self.ntp_address}})
                    if not self.interface_info['ntp_servers'] or len(self.interface_info['ntp_servers']) < 1 or self.interface_info['ntp_servers'][0]['addrType'] != 'ipvx' or (self.interface_info['ntp_servers'][0]['ipvxAddress']['addressType'] != 'ipv4') or (self.interface_info['ntp_servers'][0]['ipvxAddress']['ipv4Address'] != self.ntp_address):
                        change_required = True
                elif is_ipv6(self.ntp_address):
                    ntp_servers.append({'addrType': 'ipvx', 'ipvxAddress': {'addressType': 'ipv6', 'ipv6Address': self.ntp_address}})
                    if not self.interface_info['ntp_servers'] or len(self.interface_info['ntp_servers']) < 1 or self.interface_info['ntp_servers'][0]['addrType'] != 'ipvx' or (self.interface_info['ntp_servers'][0]['ipvxAddress']['addressType'] != 'ipv6') or (self.interface_info['ntp_servers'][0]['ipvxAddress']['ipv6Address'].replace(':', '').lower() != self.ntp_address.replace(':', '').lower()):
                        change_required = True
                else:
                    ntp_servers.append({'addrType': 'domainName', 'domainName': self.ntp_address})
                    if not self.interface_info['ntp_servers'] or len(self.interface_info['ntp_servers']) < 1 or self.interface_info['ntp_servers'][0]['addrType'] != 'domainName' or (self.interface_info['ntp_servers'][0]['domainName'] != self.ntp_address):
                        change_required = True
            if self.ntp_address_backup:
                if is_ipv4(self.ntp_address_backup):
                    ...
                    if not self.interface_info['ntp_servers'] or len(self.interface_info['ntp_servers']) < 2 or self.interface_info['ntp_servers'][1]['addrType'] != 'ipvx' or (self.interface_info['ntp_servers'][1]['ipvxAddress']['addressType'] != 'ipv4') or (self.interface_info['ntp_servers'][1]['ipvxAddress']['ipv4Address'] != self.ntp_address_backup):
                        change_required = True
                    ...
                    if not self.interface_info['ntp_servers'] or len(self.interface_info['ntp_servers']) < 2 or self.interface_info['ntp_servers'][1]['addrType'] != 'ipvx' or (self.interface_info['ntp_servers'][1]['ipvxAddress']['addressType'] != 'ipv6') or (self.interface_info['ntp_servers'][1]['ipvxAddress']['ipv6Address'].replace(':', '').lower() != self.ntp_address_backup.replace(':', '').lower()):
                        change_required = True
                else:
                    ntp_servers.append({'addrType': 'domainName', 'domainName': self.ntp_address_backup})
                    if not self.interface_info['ntp_servers'] or len(self.interface_info['ntp_servers']) < 2 or self.interface_info['ntp_servers'][1]['addrType'] != 'domainName' or (self.interface_info['ntp_servers'][1]['domainName'].lower() != self.ntp_address_backup.lower()):
                        change_required = True
    ...
    def update(self):
        ...
        if change_required and (not self.module.check_mode):
            ...
            self.module.exit_json(msg='The interface settings have been updated.', changed=change_required, available_embedded_api_urls=self.available_embedded_api_urls)
        self.module.exit_json(msg='No changes are required.', changed=change_required, available_embedded_api_urls=self.available_embedded_api_urls if self.is_embedded() else [])