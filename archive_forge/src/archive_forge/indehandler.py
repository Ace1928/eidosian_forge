...
# Define a decorator to log function entry and exit points with detailed information
...
class AsyncFileHandler(logging.Handler):
    ...
    def __init__(
        ...
        self.aiofile: Optional[aiofiles.AsyncFile] = (
            None  # Deferred initialization to the aio_open method.
        )
    ...
    async def aio_open(self):
        ...
        if self.aiofile is None:
            async with self.lock:
                if self.aiofile is None:  # Double-check to avoid race condition
                    self.aiofile = await aiofiles.open(self.filename, mode=self.mode)
                    self.logger.debug(
                        f"Log file {self.filename} opened in mode {self.mode}."
                    )
...
# TODO: Explore options for optimizing asynchronous file access for high throughput logging.
# TODO: Consider adding support for configurable log rotation to manage log file sizes.
