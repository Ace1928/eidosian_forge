...
class ProjectionPlot(param.Parameterized):
    ...
    def _get_projection(self, obj):
        ...
        opts = self._traverse_options(obj, 'plot', ['projection', 'infer_projection'], [CompositeOverlay, Element], keyfn=isoverlay, defaults=False)
        ...
        projections = sorted([p for p in obj.traverse(_get_projection, [Element]) if p is not None and p[1] is not None])
        if projections:
