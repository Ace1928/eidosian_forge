...
def vpn_ipsec_manualkey_interface(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('vpn.ipsec', 'manualkey-interface', filtered_data, vdom=vdom)
    ...
    if state == 'present' or state is True:
        return fos.set('vpn.ipsec', 'manualkey-interface', data=filtered_data, vdom=vdom)
    elif state == 'absent':
        return fos.delete('vpn.ipsec', 'manualkey-interface', mkey=filtered_data['name'], vdom=vdom)
    else:
