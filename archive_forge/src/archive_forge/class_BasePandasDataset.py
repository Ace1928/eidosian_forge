...
class BasePandasDataset(ClassLogger):
    ...
    def _binary_op(self, op, other, **kwargs):
        ...
        **kwargs : dict
        ...
        exclude_list = ['__add__', '__radd__', '__and__', '__rand__', '__or__', '__ror__', '__xor__', '__rxor__']
    ...
    def _default_to_pandas(self, op, *args, reason: str=None, **kwargs):
        ...
        **kwargs : dict
        ...
        ErrorMessage.default_to_pandas('`{}.{}`{}'.format(type(self).__name__, op if isinstance(op, str) else op.__name__, empty_self_str), reason=reason)
        ...
        with warnings.catch_warnings():
            ...
            if callable(op):
                ...
                ErrorMessage.catch_bugs_and_request_email(failure_condition=True, extra_log='{} is an unsupported operation'.format(op))
        if isinstance(result, pandas.DataFrame):
            ...
            return self._create_or_update_from_compiler(getattr(pd, type(pandas_obj).__name__)(pandas_obj)._query_compiler, inplace=True)
        else:
            try:
                if isinstance(result, (list, tuple)) and len(result) == 2 and isinstance(result[0], pandas.DataFrame):
    ...
    def add(self, other, axis='columns', level=None, fill_value=None):
        ...
        """
        return self._binary_op('add', other, axis=axis, level=level, fill_value=fill_value)
    ...
    def aggregate(self, func=None, axis=0, *args, **kwargs):
        ...
        result = None
        if axis == 0:
            result = self._aggregate(func, *args, _axis=axis, **kwargs)
        if result is None:
    ...
    def align(self, other, join='outer', axis=None, level=None, copy=None, fill_value=None, method=lib.no_default, limit=lib.no_default, fill_axis=lib.no_default, broadcast_axis=lib.no_default):
        """
        Align two objects on their axes with the specified join method.
        """
        if method is not lib.no_default or limit is not lib.no_default or fill_axis is not lib.no_default:
            warnings.warn("The 'method', 'limit', and 'fill_axis' keywords in " + f'{type(self).__name__}.align are deprecated and will be removed ' + 'in a future version. Call fillna directly on the returned objects ' + 'instead.', FutureWarning)
        ...
        if broadcast_axis is not lib.no_default:
            ...
            broadcast_axis = None
        left, right = self._query_compiler.align(other._query_compiler, join=join, axis=axis, level=level, copy=copy, fill_value=fill_value, method=method, limit=limit, fill_axis=fill_axis, broadcast_axis=broadcast_axis)
        return (self.__constructor__(query_compiler=left), self.__constructor__(query_compiler=right))
    ...
    def all(self, axis=0, bool_only=False, skipna=True, **kwargs):
        ...
        if axis is not None:
            ...
            if bool_only and axis == 0:
                if hasattr(self, 'dtype'):
                    raise NotImplementedError('{}.{} does not implement numeric_only.'.format(type(self).__name__, 'all'))
                data_for_compute = self[self.columns[self.dtypes == np.bool_]]
                return data_for_compute.all(axis=axis, bool_only=False, skipna=skipna, **kwargs)
            return self._reduce_dimension(self._query_compiler.all(axis=axis, bool_only=bool_only, skipna=skipna, **kwargs))
        else:
            if bool_only:
                raise ValueError('Axis must be 0 or 1 (got {})'.format(axis))
            result = self._reduce_dimension(self._query_compiler.all(axis=0, bool_only=bool_only, skipna=skipna, **kwargs))
        ...
        if axis is not None:
            ...
            if bool_only and axis == 0:
                if hasattr(self, 'dtype'):
                    raise NotImplementedError('{}.{} does not implement numeric_only.'.format(type(self).__name__, 'all'))
                data_for_compute = self[self.columns[self.dtypes == np.bool_]]
                return data_for_compute.any(axis=axis, bool_only=False, skipna=skipna, **kwargs)
            return self._reduce_dimension(self._query_compiler.any(axis=axis, bool_only=bool_only, skipna=skipna, **kwargs))
        else:
            if bool_only:
                raise ValueError('Axis must be 0 or 1 (got {})'.format(axis))
            result = self._reduce_dimension(self._query_compiler.any(axis=0, bool_only=bool_only, skipna=skipna, **kwargs))
            if isinstance(result, BasePandasDataset):
                return result.any(axis=axis, bool_only=bool_only, skipna=skipna, **kwargs)
            return result

    def apply(self, func, axis, raw, result_type, args, **kwds):
        """
        ...
        if isinstance(func, str):
            ...
            if len(self.columns) != len(set(self.columns)):
                warnings.warn('duplicate column names not supported with apply().', FutureWarning, stacklevel=2)
        query_compiler = self._query_compiler.apply(func, axis, args=args, raw=raw, result_type=result_type, **kwds)
    ...
    def asfreq(self, freq, method=None, how=None, normalize=False, fill_value=None):
        ...
        """
        return self.__constructor__(query_compiler=self._query_compiler.asfreq(freq=freq, method=method, how=how, normalize=normalize, fill_value=fill_value))
    ...
    def astype(self, dtype, copy=None, errors='raise'):
        ...
        if isinstance(dtype, dict):
            if not self._query_compiler.lazy_execution and (not set(dtype.keys()).issubset(set(self._query_compiler.columns))) and (errors == 'raise'):
                ...
                if isinstance(dtype, dict):
                    for col in dtype:
                        if dtype[col] != frame_dtypes[col]:
                            copy = True
                            break
                elif not (frame_dtypes == dtype).all():
                    copy = True
            else:
                copy = True
        if copy:
    ...
    def at_time(self, time, asof=False, axis=None):
        ...
        return self.between_time(start_time=time, end_time=time, inclusive='both', axis=axis)

    @_inherit_docstrings(pandas.DataFrame.between_time, apilink='pandas.DataFrame.between_time')
    def between_time(self: 'BasePandasDataset', start_time, end_time, inclusive='both', axis=None):
        return self._create_or_update_from_compiler(self._query_compiler.between_time(start_time=pandas.core.tools.times.to_time(start_time), end_time=pandas.core.tools.times.to_time(end_time), inclusive=inclusive, axis=self._get_axis_number(axis)))

    def _deprecate_downcast(self, downcast, method_name: str):
        if downcast is not lib.no_default:
            warnings.warn(f"The 'downcast' keyword in {method_name} is deprecated and " + 'will be removed in a future version. Use ' + 'res.infer_objects(copy=False) to infer non-object dtype, or ' + "pd.to_numeric with the 'downcast' keyword to downcast numeric " + 'results.', FutureWarning)
        else:
            downcast = None
        return downcast

    def bfill(self, *, axis=None, inplace=False, limit=None, limit_area=None, downcast=lib.no_default):
        """
        Synonym for `DataFrame.fillna` with ``method='bfill'``.
        """
        if limit_area is not None:
            return self._default_to_pandas('bfill', reason="'limit_area' parameter isn't supported", axis=axis, inplace=inplace, limit=limit, limit_area=limit_area, downcast=downcast)
        downcast = self._deprecate_downcast(downcast, 'bfill')
        with warnings.catch_warnings():
            warnings.filterwarnings('ignore', ".*fillna with 'method' is deprecated", category=FutureWarning)
            return self.fillna(method='bfill', axis=axis, limit=limit, downcast=downcast, inplace=inplace)

    def backfill(self, *, axis=None, inplace=False, limit=None, downcast=lib.no_default):
        """
    ...
    def clip(self, lower=None, upper=None, *, axis=None, inplace=False, **kwargs):
        ...
        new_query_compiler = self._query_compiler.clip(lower=lower, upper=upper, axis=axis, **kwargs)
    ...
    def combine(self, other, func, fill_value=None, **kwargs):
        ...
        """
        return self._binary_op('combine', other, _axis=0, func=func, fill_value=fill_value, **kwargs)
    ...
    def count(self, axis=0, numeric_only=False):
        ...
        return frame._reduce_dimension(frame._query_compiler.count(axis=axis, numeric_only=numeric_only))
    ...
    def cummax(self, axis=None, skipna=True, *args, **kwargs):
        ...
        if axis == 1:
            self._validate_dtypes(numeric_only=True)
        return self.__constructor__(query_compiler=self._query_compiler.cummax(fold_axis=axis, axis=axis, skipna=skipna, **kwargs))
    ...
    def cummin(self, axis=None, skipna=True, *args, **kwargs):
        ...
        if axis == 1:
            self._validate_dtypes(numeric_only=True)
        return self.__constructor__(query_compiler=self._query_compiler.cummin(fold_axis=axis, axis=axis, skipna=skipna, **kwargs))
    ...
    def cumprod(self, axis=None, skipna=True, *args, **kwargs):
        ...
        axis = self._get_axis_number(axis)
        self._validate_dtypes(numeric_only=True)
        return self.__constructor__(query_compiler=self._query_compiler.cumprod(fold_axis=axis, axis=axis, skipna=skipna, **kwargs))
    ...
    def cumsum(self, axis=None, skipna=True, *args, **kwargs):
        ...
        axis = self._get_axis_number(axis)
        self._validate_dtypes(numeric_only=True)
        return self.__constructor__(query_compiler=self._query_compiler.cumsum(fold_axis=axis, axis=axis, skipna=skipna, **kwargs))
    ...
    def describe(self, percentiles=None, include=None, exclude=None):
        ...
        return self.__constructor__(query_compiler=data._query_compiler.describe(percentiles=percentiles))
    ...
    def diff(self, periods=1, axis=0):
        ...
        axis = self._get_axis_number(axis)
        return self.__constructor__(query_compiler=self._query_compiler.diff(axis=axis, periods=periods))
    ...
    def drop(self, labels=None, *, axis=0, index=None, columns=None, level=None, inplace=False, errors='raise'):
        ...
        if level is not None:
            return self._default_to_pandas('drop', labels=labels, axis=axis, index=index, columns=columns, level=level, inplace=inplace, errors=errors)
        inplace = validate_bool_kwarg(inplace, 'inplace')
        ...
        for axis in ['index', 'columns']:
            if axis not in axes:
                ...
                if self._query_compiler.lazy_execution:
                    continue
                if errors == 'raise':
                    non_existent = pandas.Index(axes[axis]).difference(getattr(self, axis))
                    if len(non_existent):
                        raise KeyError(f'labels {non_existent} not contained in axis')
                else:
                    axes[axis] = [obj for obj in axes[axis] if obj in getattr(self, axis)]
                    if not len(axes[axis]):
                        axes[axis] = None
        new_query_compiler = self._query_compiler.drop(index=axes['index'], columns=axes['columns'], errors=errors)
        return self._create_or_update_from_compiler(new_query_compiler, inplace)

    def dropna(self, *, axis: Axis=0, how: str | lib.NoDefault=lib.no_default, thresh: int | lib.NoDefault=lib.no_default, subset: IndexLabel=None, inplace: bool=False, ignore_index: bool=False):
        """
        ...
        if subset is not None:
            if axis == 1:
                ...
                if check.any():
                    raise KeyError(list(np.compress(check, subset)))
        new_query_compiler = self._query_compiler.dropna(axis=axis, how=how, thresh=thresh, subset=subset)
        if ignore_index:
            new_query_compiler.index = pandas.RangeIndex(stop=len(new_query_compiler.index))
        return self._create_or_update_from_compiler(new_query_compiler, inplace)
    ...
    def droplevel(self, level, axis=0):
        ...
        if axis == 0:
            ...
            if is_integer(level):
                ...
                level = [index_columns[lev] if is_integer(lev) else lev for lev in level]
    ...
    def drop_duplicates(self, keep='first', inplace=False, **kwargs):
        ...
        if subset is not None:
            ...
            if len((diff := pandas.Index(subset).difference(self.columns))) > 0:
                raise KeyError(diff)
        result_qc = self._query_compiler.unique(keep=keep, ignore_index=ignore_index, subset=subset)
    ...
    def explode(self, column, ignore_index: bool=False):
        ...
        """
        exploded = self.__constructor__(query_compiler=self._query_compiler.explode(column))
        if ignore_index:
            exploded = exploded.reset_index(drop=True)
        return exploded

    def ewm(self, com: 'float | None'=None, span: 'float | None'=None, halflife: 'float | TimedeltaConvertibleTypes | None'=None, alpha: 'float | None'=None, min_periods: 'int | None'=0, adjust: bool=True, ignore_na: bool=False, axis: 'Axis'=lib.no_default, times: 'str | np.ndarray | BasePandasDataset | None'=None, method: 'str'='single') -> pandas.core.window.ewm.ExponentialMovingWindow:
        """
        Provide exponentially weighted (EW) calculations.
        """
        return self._default_to_pandas('ewm', com=com, span=span, halflife=halflife, alpha=alpha, min_periods=min_periods, adjust=adjust, ignore_na=ignore_na, axis=axis, times=times, method=method)

    def expanding(self, min_periods=1, axis=lib.no_default, method='single'):
        """
        ...
        if axis is not lib.no_default:
            ...
            name = 'expanding'
            if axis == 1:
                warnings.warn(f'Support for axis=1 in {type(self).__name__}.{name} is ' + 'deprecated and will be removed in a future version. ' + f'Use obj.T.{name}(...) instead', FutureWarning)
            else:
                warnings.warn(f"The 'axis' keyword in {type(self).__name__}.{name} is " + 'deprecated and will be removed in a future version. ' + 'Call the method without the axis keyword instead.', FutureWarning)
        else:
            axis = 0
        return Expanding(self, min_periods=min_periods, axis=axis, method=method)

    def ffill(self, *, axis=None, inplace=False, limit=None, limit_area=None, downcast=lib.no_default):
        """
        Synonym for `DataFrame.fillna` with ``method='ffill'``.
        """
        if limit_area is not None:
            return self._default_to_pandas('ffill', reason="'limit_area' parameter isn't supported", axis=axis, inplace=inplace, limit=limit, limit_area=limit_area, downcast=downcast)
        downcast = self._deprecate_downcast(downcast, 'ffill')
        with warnings.catch_warnings():
            warnings.filterwarnings('ignore', ".*fillna with 'method' is deprecated", category=FutureWarning)
            return self.fillna(method='ffill', axis=axis, limit=limit, downcast=downcast, inplace=inplace)

    def pad(self, *, axis=None, inplace=False, limit=None, downcast=lib.no_default):
        """
    ...
    def fillna(self, squeeze_self, squeeze_value, value=None, method=None, axis=None, inplace=False, limit=None, downcast=lib.no_default):
        """
        ...
        if method is not None:
            warnings.warn(f"{type(self).__name__}.fillna with 'method' is deprecated and " + 'will raise in a future version. Use obj.ffill() or obj.bfill() ' + 'instead.', FutureWarning)
        ...
        if method is not None and method not in ['backfill', 'bfill', 'pad', 'ffill']:
            ...
            msg = 'Invalid fill method. Expecting {expecting}. Got {method}'.format(expecting=expecting, method=method)
            raise ValueError(msg)
        ...
        new_query_compiler = self._query_compiler.fillna(squeeze_self=squeeze_self, squeeze_value=squeeze_value, value=value, method=method, axis=axis, inplace=False, limit=limit, downcast=downcast)
        return self._create_or_update_from_compiler(new_query_compiler, inplace)
    ...
    def first(self, offset):
        ...
        """
        warnings.warn('first is deprecated and will be removed in a future version. ' + 'Please create a mask and filter using `.loc` instead', FutureWarning)
        return self._create_or_update_from_compiler(self._query_compiler.first(offset=to_offset(offset)))
    ...
    def idxmax(self, axis=0, skipna=True, numeric_only=False):
        ...
        axis = self._get_axis_number(axis)
        return self._reduce_dimension(self._query_compiler.idxmax(axis=axis, skipna=skipna, numeric_only=numeric_only))
    ...
    def idxmin(self, axis=0, skipna=True, numeric_only=False):
        ...
        axis = self._get_axis_number(axis)
        return self._reduce_dimension(self._query_compiler.idxmin(axis=axis, skipna=skipna, numeric_only=numeric_only))
    ...
    def infer_objects(self, copy=None):
        ...
        return self._create_or_update_from_compiler(new_query_compiler, inplace=False if copy is None else not copy)

    def convert_dtypes(self, infer_objects: bool=True, convert_string: bool=True, convert_integer: bool=True, convert_boolean: bool=True, convert_floating: bool=True, dtype_backend: DtypeBackend='numpy_nullable'):
        """
        Convert columns to best possible dtypes using dtypes supporting ``pd.NA``.
        """
        return self.__constructor__(query_compiler=self._query_compiler.convert_dtypes(infer_objects=infer_objects, convert_string=convert_string, convert_integer=convert_integer, convert_boolean=convert_boolean, convert_floating=convert_floating, dtype_backend=dtype_backend))
    ...
    def isin(self, values):
        ...
        return self.__constructor__(query_compiler=self._query_compiler.isin(values=values, ignore_indices=ignore_indices))
    ...
    def last(self, offset):
        ...
        """
        warnings.warn('last is deprecated and will be removed in a future version. ' + 'Please create a mask and filter using `.loc` instead', FutureWarning)
        return self._create_or_update_from_compiler(self._query_compiler.last(offset=to_offset(offset)))
    ...
    def mask(self, cond, other=lib.no_default, *, inplace: bool=False, axis: Optional[Axis]=None, level: Optional[Level]=None):
        """
        Replace values where the condition is True.
        """
        return self._create_or_update_from_compiler(self._query_compiler.mask(cond, other=other, inplace=False, axis=axis, level=level), inplace=inplace)

    def max(self, axis: Axis=0, skipna=True, numeric_only=False, **kwargs):
        """
        ...
        res = data._reduce_dimension(data._query_compiler.max(axis=axis, skipna=skipna, numeric_only=numeric_only, **kwargs))
        if orig_axis is None:
            res = res._reduce_dimension(res._query_compiler.max(axis=0, skipna=skipna, numeric_only=False, **kwargs))
        return res

    def min(self, axis: Axis=0, skipna: bool=True, numeric_only=False, **kwargs):
        """
        ...
        axis = self._get_axis_number(axis)
        data = self._validate_dtypes_min_max(axis, numeric_only)
        res = data._reduce_dimension(data._query_compiler.min(axis=axis, skipna=skipna, numeric_only=numeric_only, **kwargs))
        if orig_axis is None:
            res = res._reduce_dimension(res._query_compiler.min(axis=0, skipna=skipna, numeric_only=False, **kwargs))
        return res
    ...
    def mod(self, other, axis='columns', level=None, fill_value=None):
        ...
        """
        return self._binary_op('mod', other, axis=axis, level=level, fill_value=fill_value)
    ...
    def mode(self, axis=0, numeric_only=False, dropna=True):
        ...
        """
        axis = self._get_axis_number(axis)
        return self.__constructor__(query_compiler=self._query_compiler.mode(axis=axis, numeric_only=numeric_only, dropna=dropna))

    def mul(self, other, axis='columns', level=None, fill_value=None):
        """
        Get multiplication of `BasePandasDataset` and `other`, element-wise (binary operator `mul`).
        """
        return self._binary_op('mul', other, axis=axis, level=level, fill_value=fill_value)
    multiply = mul
    ...
    def nunique(self, axis=0, dropna=True):
        ...
        """
        axis = self._get_axis_number(axis)
        return self._reduce_dimension(self._query_compiler.nunique(axis=axis, dropna=dropna))

    def pct_change(self, periods=1, fill_method=lib.no_default, limit=lib.no_default, freq=None, **kwargs):
        """
        Percentage change between the current and a prior element.
        """
        if fill_method not in (lib.no_default, None) or limit is not lib.no_default:
            warnings.warn("The 'fill_method' keyword being not None and the 'limit' keyword in " + f'{type(self).__name__}.pct_change are deprecated and will be removed ' + 'in a future version. Either fill in any non-leading NA values prior ' + "to calling pct_change or specify 'fill_method=None' to not fill NA " + 'values.', FutureWarning)
        if fill_method is lib.no_default:
            if self.isna().values.any():
                warnings.warn("The default fill_method='pad' in " + f'{type(self).__name__}.pct_change is deprecated and will be ' + 'removed in a future version. Call ffill before calling ' + 'pct_change to retain current behavior and silence this warning.', FutureWarning)
            fill_method = 'pad'
        if limit is lib.no_default:
            limit = None
        if not isinstance(periods, int):
        ...
        for dtype in self._get_dtypes():
            if not is_numeric_dtype(dtype):
                raise TypeError(f'unsupported operand type for /: got {dtype}')
        return self.__constructor__(query_compiler=self._query_compiler.pct_change(periods=periods, fill_method=fill_method, limit=limit, freq=freq, **kwargs))
    ...
    def pow(self, other, axis='columns', level=None, fill_value=None):
        ...
        """
        return self._binary_op('pow', other, axis=axis, level=level, fill_value=fill_value)
    ...
    def quantile(self, q, axis, numeric_only, interpolation, method):
        ...
        if not numeric_only:
            ...
            numeric_only_df = self.drop(columns=[i for i in self.dtypes.index if not is_numeric_dtype(self.dtypes[i])])
        validate_percentile(q)
        axis = numeric_only_df._get_axis_number(axis)
        if isinstance(q, (pandas.Series, np.ndarray, pandas.Index, list, tuple)):
            return numeric_only_df.__constructor__(query_compiler=numeric_only_df._query_compiler.quantile_for_list_of_values(q=q, axis=axis, numeric_only=False, interpolation=interpolation, method=method))
        else:
            result = numeric_only_df._reduce_dimension(numeric_only_df._query_compiler.quantile_for_single_value(q=q, axis=axis, numeric_only=False, interpolation=interpolation, method=method))
            if isinstance(result, BasePandasDataset):
                result.name = q
            return result

    @_inherit_docstrings(pandas.DataFrame.rank, apilink='pandas.DataFrame.rank')
    def rank(self, axis=0, method: str='average', numeric_only=False, na_option: str='keep', ascending: bool=True, pct: bool=False):
        if axis is None:
            raise ValueError(f'No axis named None for object type {type(self).__name__}')
        axis = self._get_axis_number(axis)
        return self.__constructor__(query_compiler=self._query_compiler.rank(axis=axis, method=method, numeric_only=numeric_only, na_option=na_option, ascending=ascending, pct=pct))
    ...
    def reindex(self, index=None, columns=None, copy=True, **kwargs):
        ...
        if index is not None:
            if not isinstance(index, pandas.Index) or not index.equals(self.index):
                new_query_compiler = self._query_compiler.reindex(axis=0, labels=index, **kwargs)
        ...
        if columns is not None:
            if not isinstance(index, pandas.Index) or not columns.equals(self.columns):
                final_query_compiler = new_query_compiler.reindex(axis=1, labels=columns, **kwargs)
        if final_query_compiler is None:
            final_query_compiler = new_query_compiler
        return self._create_or_update_from_compiler(final_query_compiler, inplace=False if copy is None else not copy)

    def rename_axis(self, mapper=lib.no_default, *, index=lib.no_default, columns=lib.no_default, axis=0, copy=None, inplace=False):
        """
    ...
    def resample(self, rule, axis: Axis=lib.no_default, closed: Optional[str]=None, label: Optional[str]=None, convention: str=lib.no_default, kind: Optional[str]=lib.no_default, on: Level=None, level: Level=None, origin: Union[str, TimestampConvertibleTypes]='start_day', offset: Optional[TimedeltaConvertibleTypes]=None, group_keys=False):
        """
        ...
        if axis is not lib.no_default:
            axis = self._get_axis_number(axis)
            if axis == 1:
                warnings.warn('DataFrame.resample with axis=1 is deprecated. Do ' + '`frame.T.resample(...)` without axis instead.', FutureWarning)
            else:
                warnings.warn(f"The 'axis' keyword in {type(self).__name__}.resample is " + 'deprecated and will be removed in a future version.', FutureWarning)
        else:
            axis = 0
        return Resampler(dataframe=self, rule=rule, axis=axis, closed=closed, label=label, convention=convention, kind=kind, on=on, level=level, origin=origin, offset=offset, group_keys=group_keys)

    def reset_index(self, level: IndexLabel=None, *, drop: bool=False, inplace: bool=False, col_level: Hashable=0, col_fill: Hashable='', allow_duplicates=lib.no_default, names: Hashable | Sequence[Hashable]=None):
        """
        Reset the index, or a level of it.
        """
        inplace = validate_bool_kwarg(inplace, 'inplace')
        if not drop and (not self._query_compiler.lazy_execution) and (not self._query_compiler.has_multiindex()) and all((n in self.columns for n in ['level_0', 'index'])):
            raise ValueError('cannot insert level_0, already exists')
        new_query_compiler = self._query_compiler.reset_index(drop=drop, level=level, col_level=col_level, col_fill=col_fill, allow_duplicates=allow_duplicates, names=names)
        return self._create_or_update_from_compiler(new_query_compiler, inplace)
    ...
    def radd(self, other, axis='columns', level=None, fill_value=None):
        ...
        """
        return self._binary_op('radd', other, axis=axis, level=level, fill_value=fill_value)

    def rfloordiv(self, other, axis='columns', level=None, fill_value=None):
        """
        Get integer division of `BasePandasDataset` and `other`, element-wise (binary operator `rfloordiv`).
        """
        return self._binary_op('rfloordiv', other, axis=axis, level=level, fill_value=fill_value)
    ...
    def rmul(self, other, axis='columns', level=None, fill_value=None):
        ...
        """
        return self._binary_op('rmul', other, axis=axis, level=level, fill_value=fill_value)

    def rolling(self, window, min_periods: int | None=None, center: bool=False, win_type: str | None=None, on: str | None=None, axis: Axis=lib.no_default, closed: str | None=None, step: int | None=None, method: str='single'):
        """
    ...
    def round(self, decimals=0, *args, **kwargs):
        ...
        """
        return self.__constructor__(query_compiler=self._query_compiler.round(decimals=decimals, **kwargs))

    def rpow(self, other, axis='columns', level=None, fill_value=None):
        """
        Get exponential power of `BasePandasDataset` and `other`, element-wise (binary operator `rpow`).
        """
        return self._binary_op('rpow', other, axis=axis, level=level, fill_value=fill_value)
    ...
    def rtruediv(self, other, axis='columns', level=None, fill_value=None):
        ...
        """
        return self._binary_op('rtruediv', other, axis=axis, level=level, fill_value=fill_value)
    rdiv = rtruediv

    def sample(self, n: int | None=None, frac: float | None=None, replace: bool=False, weights=None, random_state: RandomState | None=None, axis: Axis | None=None, ignore_index: bool=False):
        """
        ...
        if n == 0:
            return self._default_to_pandas('sample', n=n, frac=None, replace=replace, weights=weights, random_state=random_state, axis=axis, ignore_index=ignore_index)
        if random_state is not None:
            if isinstance(random_state, int):
                ...
                raise ValueError('Please enter an `int` OR a ' + 'np.random.RandomState for random_state')
            sample_indices = random_num_gen.choice(np.arange(0, axis_length), size=n, replace=replace, p=weights)
    ...
    def sem(self, axis: Axis=0, skipna: bool=True, ddof: int=1, numeric_only=False, **kwargs):
        """
        Return unbiased standard error of the mean over requested axis.
        """
        return self._stat_operation('sem', axis, skipna, numeric_only, ddof=ddof, **kwargs)

    def mean(self, axis: Axis=0, skipna=True, numeric_only=False, **kwargs):
        """
        Return the mean of the values over the requested axis.
        """
        return self._stat_operation('mean', axis, skipna, numeric_only, **kwargs)

    def median(self, axis: Axis=0, skipna=True, numeric_only=False, **kwargs):
        """
        Return the mean of the values over the requested axis.
        """
        return self._stat_operation('median', axis, skipna, numeric_only, **kwargs)

    def set_axis(self, labels, *, axis: Axis=0, copy=None):
        """
    ...
    def set_flags(self, *, copy: bool=False, allows_duplicate_labels: Optional[bool]=None):
        """
        Return a new `BasePandasDataset` with updated flags.
        """
        return self._default_to_pandas(pandas.DataFrame.set_flags, copy=copy, allows_duplicate_labels=allows_duplicate_labels)

    @property
    def flags(self):
        return self._default_to_pandas(lambda df: df.flags)

    def shift(self, periods: int=1, freq=None, axis: Axis=0, fill_value: Hashable=lib.no_default, suffix=None):
        """
        Shift index by desired number of periods with an optional time `freq`.
        """
        if suffix:
            return self._default_to_pandas(lambda df: df.shift(periods=periods, freq=freq, axis=axis, fill_value=fill_value, suffix=suffix))
        if freq is not None and fill_value is not lib.no_default:
            raise ValueError("Cannot pass both 'freq' and 'fill_value' to " + f'{type(self).__name__}.shift')
        if periods == 0:
            return self.copy()
        return self._create_or_update_from_compiler(new_query_compiler=self._query_compiler.shift(periods, freq, axis, fill_value), inplace=False)

    def skew(self, axis: Axis=0, skipna: bool=True, numeric_only=False, **kwargs):
        """
        Return unbiased skew over requested axis.
        """
        return self._stat_operation('skew', axis, skipna, numeric_only, **kwargs)

    def sort_index(self, *, axis=0, level=None, ascending=True, inplace=False, kind='quicksort', na_position='last', sort_remaining=True, ignore_index: bool=False, key: Optional[IndexKeyFunc]=None):
        """
        Sort object by labels (along an axis).
        """
        if ascending is None:
            raise ValueError('the `axis` parameter is not supported in the pandas implementation of argsort()')
        axis = self._get_axis_number(axis)
        inplace = validate_bool_kwarg(inplace, 'inplace')
        new_query_compiler = self._query_compiler.sort_index(axis=axis, level=level, ascending=ascending, inplace=inplace, kind=kind, na_position=na_position, sort_remaining=sort_remaining, ignore_index=ignore_index, key=key)
        return self._create_or_update_from_compiler(new_query_compiler, inplace)

    def sort_values(self, by, *, axis=0, ascending=True, inplace: bool=False, kind='quicksort', na_position='last', ignore_index: bool=False, key: Optional[IndexKeyFunc]=None):
        """
        ...
        if axis == 0:
            result = self._query_compiler.sort_rows_by_column_values(by, ascending=ascending, kind=kind, na_position=na_position, ignore_index=ignore_index, key=key)
        else:
            result = self._query_compiler.sort_columns_by_row_values(by, ascending=ascending, kind=kind, na_position=na_position, ignore_index=ignore_index, key=key)
        return self._create_or_update_from_compiler(result, inplace)

    def std(self, axis: Axis=0, skipna: bool=True, ddof: int=1, numeric_only=False, **kwargs):
        """
    ...
    def sub(self, other, axis='columns', level=None, fill_value=None):
        ...
        """
        return self._binary_op('sub', other, axis=axis, level=level, fill_value=fill_value)
    subtract = sub
    ...
    def to_hdf(self, path_or_buf, key: str, mode: Literal['a', 'w', 'r+']='a', complevel: int | None=None, complib: Literal['zlib', 'lzo', 'bzip2', 'blosc'] | None=None, append: bool=False, format: Literal['fixed', 'table'] | None=None, index: bool=True, min_itemsize: int | dict[str, int] | None=None, nan_rep=None, dropna: bool | None=None, data_columns: Literal[True] | list[str] | None=None, errors: str='strict', encoding: str='UTF-8') -> None:
        ...
        """
        return self._default_to_pandas('to_hdf', path_or_buf, key=key, mode=mode, complevel=complevel, complib=complib, append=append, format=format, index=index, min_itemsize=min_itemsize, nan_rep=nan_rep, dropna=dropna, data_columns=data_columns, errors=errors, encoding=encoding)

    @expanduser_path_arg('path_or_buf')
    def to_json(self, path_or_buf=None, orient=None, date_format=None, double_precision=10, force_ascii=True, date_unit='ms', default_handler=None, lines=False, compression='infer', index=None, indent=None, storage_options: StorageOptions=None, mode='w'):
        """
        Convert the object to a JSON string.
        """
        from modin.core.execution.dispatching.factories.dispatcher import FactoryDispatcher
        return FactoryDispatcher.to_json(self._query_compiler, path_or_buf, orient=orient, date_format=date_format, double_precision=double_precision, force_ascii=force_ascii, date_unit=date_unit, default_handler=default_handler, lines=lines, compression=compression, index=index, indent=indent, storage_options=storage_options, mode=mode)
    ...
    def to_latex(self, buf=None, columns=None, header=True, index=True, na_rep='NaN', formatters=None, float_format=None, sparsify=None, index_names=True, bold_rows=False, column_format=None, longtable=None, escape=None, encoding=None, decimal='.', multicolumn=None, multicolumn_format=None, multirow=None, caption=None, label=None, position=None):
        ...
        """
        return self._default_to_pandas('to_latex', buf=buf, columns=columns, header=header, index=index, na_rep=na_rep, formatters=formatters, float_format=float_format, sparsify=sparsify, index_names=index_names, bold_rows=bold_rows, column_format=column_format, longtable=longtable, escape=escape, encoding=encoding, decimal=decimal, multicolumn=multicolumn, multicolumn_format=multicolumn_format, multirow=multirow, caption=caption, label=label, position=position)

    @expanduser_path_arg('buf')
    def to_markdown(self, buf=None, mode: str='wt', index: bool=True, storage_options: StorageOptions=None, **kwargs):
        """
        Print `BasePandasDataset` in Markdown-friendly format.
        """
        return self._default_to_pandas('to_markdown', buf=buf, mode=mode, index=index, storage_options=storage_options, **kwargs)

    @expanduser_path_arg('path')
    def to_pickle(self, path, compression: CompressionOptions='infer', protocol: int=pkl.HIGHEST_PROTOCOL, storage_options: StorageOptions=None):
        """
    ...
    def to_numpy(self, dtype=None, copy=False, na_value=lib.no_default):
        ...
        return self._to_bare_numpy(dtype=dtype, copy=copy, na_value=na_value)

    def to_period(self, freq=None, axis=0, copy=None):
        """
    ...
    def to_string(self, buf=None, columns=None, col_space=None, header=True, index=True, na_rep='NaN', formatters=None, float_format=None, sparsify=None, index_names=True, justify=None, max_rows=None, min_rows=None, max_cols=None, show_dimensions=False, decimal='.', line_width=None, max_colwidth=None, encoding=None):
        ...
        """
        return self._default_to_pandas('to_string', buf=buf, columns=columns, col_space=col_space, header=header, index=index, na_rep=na_rep, formatters=formatters, float_format=float_format, sparsify=sparsify, index_names=index_names, justify=justify, max_rows=max_rows, max_cols=max_cols, show_dimensions=show_dimensions, decimal=decimal, line_width=line_width, max_colwidth=max_colwidth, encoding=encoding)
    ...
    def to_sql(self, name, con, schema=None, if_exists='fail', index=True, index_label=None, chunksize=None, dtype=None, method=None):
        ...
        if index:
            ...
            if index_label is not None:
                ...
                new_query_compiler.columns = list(index_label) + list(new_query_compiler.columns[len(index_label):])
            index = False
        from modin.core.execution.dispatching.factories.dispatcher import FactoryDispatcher
        FactoryDispatcher.to_sql(new_query_compiler, name=name, con=con, schema=schema, if_exists=if_exists, index=index, index_label=index_label, chunksize=chunksize, dtype=dtype, method=method)

    def to_timestamp(self, freq=None, how='start', axis=0, copy=None):
        """
        Cast to DatetimeIndex of timestamps, at *beginning* of period.
        """
        return self._default_to_pandas('to_timestamp', freq=freq, how=how, axis=axis, copy=copy)
    ...
    def truediv(self, other, axis='columns', level=None, fill_value=None):
        ...
        """
        return self._binary_op('truediv', other, axis=axis, level=level, fill_value=fill_value)
    div = divide = truediv
    ...
    def truncate(self, before=None, after=None, axis=None, copy=None):
        ...
        """
        axis = self._get_axis_number(axis)
        if not self._get_axis(axis).is_monotonic_increasing and (not self._get_axis(axis).is_monotonic_decreasing):
    ...
    def transform(self, func, axis=0, *args, **kwargs):
        ...
        try:
            result = self.agg(func, *args, axis=axis, **kwargs)
        except (TypeError, pandas.errors.SpecificationError):
            raise
        except Exception as err:
            raise ValueError('Transform function failed') from err
        if getattr(result, '_pandas_class', None) not in (pandas.Series, pandas.DataFrame) or not result.index.equals(self.index):
    ...
    def tz_convert(self, tz, axis=0, level=None, copy=None):
        ...
        if copy is None:
            copy = True
        return self._create_or_update_from_compiler(self._query_compiler.tz_convert(tz, axis=self._get_axis_number(axis), level=level, copy=copy), inplace=not copy)
    ...
    def tz_localize(self, tz, axis=0, level=None, copy=None, ambiguous='raise', nonexistent='raise'):
        ...
        if copy is None:
            copy = True
        return self._create_or_update_from_compiler(self._query_compiler.tz_localize(tz, axis=self._get_axis_number(axis), level=level, copy=copy, ambiguous=ambiguous, nonexistent=nonexistent), inplace=not copy)

    def interpolate(self, method='linear', *, axis=0, limit=None, inplace=False, limit_direction: Optional[str]=None, limit_area=None, downcast=lib.no_default, **kwargs):
        if downcast is not lib.no_default:
            warnings.warn(f"The 'downcast' keyword in {type(self).__name__}.interpolate " + 'is deprecated and will be removed in a future version. ' + 'Call result.infer_objects(copy=False) on the result instead.', FutureWarning)
        else:
            downcast = None
        return self._create_or_update_from_compiler(self._query_compiler.interpolate(method=method, axis=axis, limit=limit, inplace=False, limit_direction=limit_direction, limit_area=limit_area, downcast=downcast, **kwargs), inplace=inplace)

    @_inherit_docstrings(pandas.DataFrame.value_counts, apilink='pandas.DataFrame.value_counts')
    def value_counts(self, subset: Sequence[Hashable] | None=None, normalize: bool=False, sort: bool=True, ascending: bool=False, dropna: bool=True):
        if subset is None:
            subset = self._query_compiler.columns
        with warnings.catch_warnings():
            warnings.filterwarnings('ignore', message='.*groupby keys will be sorted anyway.*', category=UserWarning)
            counted_values = self.groupby(by=subset, dropna=dropna, observed=True, sort=False).size()
        if sort:
    ...
    def var(self, axis: Axis=0, skipna: bool=True, ddof: int=1, numeric_only=False, **kwargs):
        """
    ...
    def xs(self, key, axis=0, level=None, drop_level: bool=True):
        """
        ...
        if isinstance(key, list):
            warnings.warn('Passing lists as key for xs is deprecated and will be removed in a ' + 'future version. Pass key as a tuple instead.', FutureWarning)
        if level is not None:
        ...
        if is_scalar(loc) and axis == 0:
            ...
            if new_index is None:
                raise RuntimeError("`new_index` variable shouldn't be equal to None here, something went wrong.")
    ...
    def _getitem_slice(self, key: slice):
        ...
        modin.pandas.BasePandasDataset
            Selected rows.
        """
        if is_full_grab_slice(key, sequence_len=None if self._query_compiler.lazy_execution else len(self)):
            return self.copy()
    ...
    def __invert__(self):
        ...
        if not all((is_bool_dtype(d) or is_integer_dtype(d) for d in self._get_dtypes())):
            raise TypeError("bad operand type for unary ~: '{}'".format(next((d for d in self._get_dtypes() if not (is_bool_dtype(d) or is_integer_dtype(d))))))
    ...
    def __nonzero__(self):
        ...
        """
        raise ValueError(f'The truth value of a {self.__class__.__name__} is ambiguous. ' + 'Use a.empty, a.bool(), a.item(), a.any() or a.all().')
    ...
    def _repartition(self, axis: Optional[int]=None):
        ...
        if axis not in allowed_axis_values:
            raise ValueError(f'Passed `axis` parameter: {axis}, but should be one of {allowed_axis_values}')
        return self.__constructor__(query_compiler=self._query_compiler.repartition(axis=axis))
    ...
    def __array_ufunc__(self, ufunc: np.ufunc, method: str, *inputs: Any, **kwargs: Any):
        ...
        **kwargs : dict
        ...
        pandas_result = pandas_self.__array_ufunc__(ufunc, method, *(pandas_self if each_input is self else try_cast_to_pandas(each_input) for each_input in inputs), **try_cast_to_pandas(kwargs))
