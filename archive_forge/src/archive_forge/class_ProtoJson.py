...
class ProtoJson(object):
    ...
    ALTERNATIVE_CONTENT_TYPES = ['application/x-javascript', 'text/javascript', 'text/x-javascript', 'text/x-json', 'text/json']
    ...
    def decode_field(self, field, value):
        ...
        elif isinstance(field, messages.MessageField) and issubclass(field.type, messages.Message):
            return self.__decode_dictionary(field.type, value)
        elif isinstance(field, messages.FloatField) and isinstance(value, (six.integer_types, six.string_types)):
            try:
                return float(value)
            except:
                pass
        elif isinstance(field, messages.IntegerField) and isinstance(value, six.string_types):
            try:
                return int(value)
            except:
                pass
