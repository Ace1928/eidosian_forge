...
def save_visualization(
    ...
    if viz_type == "original_signal":
        ...
        if mel_spectrogram.shape[1] > 1000:  # Arbitrary threshold for large tensors
            chunk_size = 1000
            for i in range(0, mel_spectrogram.shape[1], chunk_size):
                fig, ax = plt.subplots(figsize=(10, 4))
                ax.imshow(
                    mel_spectrogram[:, i : i + chunk_size],
                    aspect="auto",
                    origin="lower",
                    cmap="viridis",
                )
                ...
                ax.set_title(f"{title} - Mel Spectrogram (Chunk {i//chunk_size + 1})")
                save_plot(
                    fig, f"mel_spectrogram_{timestamp}_chunk_{i//chunk_size + 1}.png"
                )
        else:
            fig, ax = plt.subplots(figsize=(10, 4))
