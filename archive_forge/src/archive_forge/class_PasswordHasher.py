...
class PasswordHasher:
    ...
    def __init__(self, time_cost: int=DEFAULT_TIME_COST, memory_cost: int=DEFAULT_MEMORY_COST, parallelism: int=DEFAULT_PARALLELISM, hash_len: int=DEFAULT_HASH_LENGTH, salt_len: int=DEFAULT_RANDOM_SALT_LENGTH, encoding: str='utf-8', type: Type=Type.ID):
        e = _check_types(time_cost=(time_cost, int), memory_cost=(memory_cost, int), parallelism=(parallelism, int), hash_len=(hash_len, int), salt_len=(salt_len, int), encoding=(encoding, str), type=(type, Type))
        if e:
            raise TypeError(e)
        self._parameters = Parameters(type=type, version=19, salt_len=salt_len, hash_len=hash_len, time_cost=time_cost, memory_cost=memory_cost, parallelism=parallelism)
    ...
    def hash(self, password: str | bytes, *, salt: bytes | None=None) -> str:
        ...
        """
        return hash_secret(secret=_ensure_bytes(password, self.encoding), salt=salt or os.urandom(self.salt_len), time_cost=self.time_cost, memory_cost=self.memory_cost, parallelism=self.parallelism, hash_len=self.hash_len, type=self.type).decode('ascii')
    _header_to_type: ClassVar[dict[bytes, Type]] = {b'$argon2i$': Type.I, b'$argon2d$': Type.D, b'$argon2id': Type.ID}
