...
class DistanceIntersectionOverUnion(IntersectionOverUnion):
    ...
    def __init__(self, box_format: str='xyxy', iou_threshold: Optional[float]=None, class_metrics: bool=False, respect_labels: bool=True, **kwargs: Any) -> None:
        if not _TORCHVISION_GREATER_EQUAL_0_13:
            raise ModuleNotFoundError(f'Metric `{self._iou_type.upper()}` requires that `torchvision` version 0.13.0 or newer is installed. Please install with `pip install torchvision>=0.13` or `pip install torchmetrics[detection]`.')
