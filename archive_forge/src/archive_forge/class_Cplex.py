...
class Cplex(PersistentSolver):
    ...
    def _postsolve(self, timer: HierarchicalTimer, solve_time):
        ...
        if config.load_solution:
            if cpxprob.solution.get_solution_type() == cpxprob.solution.type.none:
                raise RuntimeError('A feasible solution was not found, so no solution can be loaded. If using the appsi.solvers.Cplex interface, you can set opt.config.load_solution=False. If using the environ.SolverFactory interface, you can set opt.solve(model, load_solutions = False). Then you can check results.termination_condition and results.best_feasible_objective before loading a solution.')
            else:
                if results.termination_condition != TerminationCondition.optimal:
                    logger.warning('Loading a feasible but suboptimal solution. Please set load_solution=False and check results.termination_condition before loading a solution.')
    ...
    def get_duals(self, cons_to_load: Optional[Sequence[_GeneralConstraintData]]=None) -> Dict[_GeneralConstraintData, float]:
        ...
        if self._cplex_model.get_problem_type() in [self._cplex_model.problem_type.MILP, self._cplex_model.problem_type.MIQP, self._cplex_model.problem_type.MIQCP]:
