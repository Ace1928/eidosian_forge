...
class MTurkWizardOfWikipediaWorld(MultiAgentDialogWorld):
    ...
    def __init__(self, opt, agents=None, shared=None, world_tag='NONE', ir_agent=None, task='', wiki_title_to_passage=None):
        ...
        self.max_resp_time = opt['max_resp_time']
    ...
    def parley(self):
        ...
        if self.turn_idx == 1:
            ...
            self.agents[0].observe(validate({'id': 'SYSTEM', 'text': PICK_TOPIC_MSG, 'relevant_topics': self.relevant_topics}))
            ...
            for idx, agent in enumerate(self.agents):
                if agent.id == WIZARD:
                    ...
                    agent.observe(validate(control_msg))
                    self.chosen_topic_passage = {'topic': self.chosen_topic, 'full_passage': passage, 'shown_passage': sentences}
        ...
        acts = self.acts
        for idx, agent in enumerate(self.agents):
            max_response_time = self.max_resp_time * (1 if agent.id == APPRENTICE else 1.5)
            ...
            msg_info = {'speaker': '{}_{}'.format(idx, agent.id), 'text': acts[idx]['text'], 'turn': self.turn_idx, 'time': time.time(), 'offensive': self.OLD.contains_offensive_language(acts[idx]['text'])}
            ...
            if 'clicked_passages' in acts[idx]:
                ...
                for k, v in acts[idx]['checked_sentences'].items():
                    if k == 'no_passages_used':
                        ...
                        sent_idx = split[2]
                        if person == 'partner':
                            sub_passages = [p.split('\n')[0] for p in self.dialog[-1]['full_passages']]
                        ...
                        cs_key = '_'.join([person, '_'.join(topic.split(' ')), sent_idx])
                        checked_sents[cs_key] = v
                msg_info['checked_sentence'] = checked_sents
                msg_info['checked_passage'] = acts[idx]['checked_passages']
                msg_info['good_message'] = self.sufficient_overlap(msg_info['text'], msg_info['checked_sentence'])
            'Retrieve Passages'
    ...
    def save_data(self):
        ...
        if convo_finished:
            ...
            self.good_wiz = True
        pickle.dump({'persona': self.persona_to_topics, 'relevant_topics': self.relevant_topics, 'chosen_topic_passage': self.chosen_topic_passage, 'dialog': self.dialog, 'speaker_with_persona': self.agents[0].worker_id, 'workers': [ag.worker_id for ag in self.agents], 'n_turn': self.num_turns, 'hit_ids': [ag.hit_id for ag in self.agents], 'assignment_ids': [ag.assignment_id for ag in self.agents], 'wizard_eval': self.wizard_eval, 'chosen_topic': self.chosen_topic, 'wizard_good': convo_finished and self.good_wiz, 'good_wizard_worker': self.wizard_worker if self.good_wiz else '', 'bad_wizard_worker': self.wizard_worker if not self.good_wiz else ''}, open(filename, 'wb'))
        ...
        def review_agent(ag):
            ...
            for d in self.dialog:
                if role in d['speaker']:
                    if d['offensive']:
                        ag.reject_work(reason='Your HIT has been rejected because we detected offensive language in your submission.')
