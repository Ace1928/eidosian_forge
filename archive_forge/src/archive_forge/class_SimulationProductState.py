...
class SimulationProductState(Generic[TSimulationState], SimulationStateBase[TSimulationState], abc.Mapping):
    ...
    def __init__(self, sim_states: Dict[Optional['cirq.Qid'], TSimulationState], qubits: Sequence['cirq.Qid'], split_untangled_states: bool, classical_data: Optional['cirq.ClassicalDataStore']=None):
    ...
    def _act_on_fallback_(self, action: Any, qubits: Sequence['cirq.Qid'], allow_decompose: bool=True) -> bool:
        gate_opt = action if isinstance(action, ops.Gate) else action.gate if isinstance(action, ops.Operation) else None
        if isinstance(gate_opt, ops.IdentityGate):
            return True
        if isinstance(gate_opt, ops.SwapPowGate) and gate_opt.exponent % 2 == 1 and (gate_opt.global_shift == 0):
            q0, q1 = qubits
        ...
        if self.split_untangled_states and isinstance(gate_opt, (ops.ResetChannel, ops.MeasurementGate)):
            for q in qubits:
    ...
    def copy(self, deep_copy_buffers: bool=True) -> 'cirq.SimulationProductState[TSimulationState]':
        ...
        return SimulationProductState(args, self.qubits, self.split_untangled_states, classical_data=classical_data)
