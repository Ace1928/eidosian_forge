...
class Unk:
    ...
    def data(self, value: np.ndarray) -> None:
        ...
        if len(temp_val.shape) not in [4, 5]:
            raise ValueError(f'invalid data shape, must be (nbnd, ngx, ngy, ngz) or (nbnd, 2, ngx, ngy, ngz) for noncollinear data, given {temp_val.shape}')
        if len(temp_val.shape) == 5 and temp_val.shape[1] != 2:
            raise ValueError(f'invalid noncollinear data, shape should be (nbnd, 2, ngx, ngy, ngz), given {temp_val.shape}')
        self._data = temp_val
    ...
    def __repr__(self) -> str:
        ...
        return f'{type(self).__name__}(ik={ik!r}, nbnd={nbnd!r}, ncl={ncl!r}, ngx={ngx!r}, ngy={ngy!r}, ngz={ngz!r})'
    ...
    def __eq__(self, other: object) -> bool:
        ...
        for ib in range(self.nbnd):
            if self.is_noncollinear:
                if not (np.allclose(self.data[ib, 0], other.data[ib, 0], atol=0.0001) and np.allclose(self.data[ib, 1], other.data[ib, 1], atol=0.0001)):
                    return False
            elif not np.allclose(self.data[ib], other.data[ib], atol=0.0001):
                return False
        return True