...
def _create_default_callbacks(callbacks: Optional[List[Callback]], *, air_verbosity: Optional['AirVerbosity']=None, entrypoint: Optional[str]=None, metric: Optional[str]=None, mode: Optional[str]=None, config: Optional[dict]=None, progress_metrics: Optional[Collection[str]]=None) -> List[Callback]:
    ...
    has_trial_progress_callback = any((isinstance(c, TrialProgressCallback) for c in callbacks))
    if has_trial_progress_callback and air_verbosity is not None:
        logger.warning('AIR_VERBOSITY is set, ignoring passed-in TrialProgressCallback.')
        new_callbacks = [c for c in callbacks if not isinstance(c, TrialProgressCallback)]
        callbacks = new_callbacks
    if air_verbosity is not None:
        from ray.tune.experimental.output import _detect_reporter as _detect_air_reporter
        air_progress_reporter = _detect_air_reporter(air_verbosity, num_samples=1, entrypoint=entrypoint, metric=metric, mode=mode, config=config, progress_metrics=progress_metrics)
        callbacks.append(air_progress_reporter)
    elif not has_trial_progress_callback:
        trial_progress_callback = TrialProgressCallback(metric=metric, progress_metrics=progress_metrics)
    ...
    if os.environ.get('TUNE_DISABLE_AUTO_CALLBACK_LOGGERS', '0') != '1':
        ...
        if not has_tbx_logger:
            try:
                ...
                logger.warning("The TensorboardX logger cannot be instantiated because either TensorboardX or one of it's dependencies is not installed. Please make sure you have the latest version of TensorboardX installed: `pip install -U tensorboardx`")
    return callbacks