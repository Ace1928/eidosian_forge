...
class OVSBridge(object):
    ...
    def __init__(self, CONF, datapath_id, ovsdb_addr, timeout=None, exception=None, br_name=None):
    ...
    def delete_port(self, port_name):
        ...
        """
        command = ovs_vsctl.VSCtlCommand('del-port', (self.br_name, port_name), '--if-exists')
        self.run_command([command])
    ...
    def add_bond(self, name, ifaces, bond_mode=None, lacp=None):
        ...
        command_add = ovs_vsctl.VSCtlCommand('add-bond', (self.br_name, name, ifaces), options)
        self.run_command([command_add])

    def add_tunnel_port(self, name, tunnel_type, remote_ip, local_ip=None, key=None, ofport=None):
        """
        ...
        args = ['Interface', name, 'type=%s' % tunnel_type, 'options:%s' % options]
        if ofport:
    ...
    def add_gre_port(self, name, remote_ip, local_ip=None, key=None, ofport=None):
        """
        Creates a GRE tunnel port.

        See the description of ``add_tunnel_port()``.
        """
        self.add_tunnel_port(name, 'gre', remote_ip, local_ip=local_ip, key=key, ofport=ofport)
