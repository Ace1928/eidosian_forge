...
class LocalFileHandler(StorageHandler):
    ...
    def load_path(self, manifest_entry: ArtifactManifestEntry, local: bool=False) -> Union[URIStr, FilePathStr]:
        ...
        if md5 != manifest_entry.digest:
            raise ValueError(f'Local file reference: Digest mismatch for path {local_path}: expected {manifest_entry.digest} but found {md5}')
    ...
    def store_path(self, artifact: 'Artifact', path: Union[URIStr, FilePathStr], name: Optional[StrPath]=None, checksum: bool=True, max_objects: Optional[int]=None) -> Sequence[ArtifactManifestEntry]:
        ...
        if os.path.isdir(local_path):
            ...
            if checksum:
                termlog('Generating checksum for up to %i files in "%s"...\n' % (max_objects, local_path), newline=False)
            for root, _, files in os.walk(local_path):
                for sub_path in files:
                    ...
                    entry = ArtifactManifestEntry(path=logical_path, ref=FilePathStr(os.path.join(path, logical_path)), size=os.path.getsize(physical_path), digest=md5(physical_path))
                    entries.append(entry)
            if checksum:
                termlog('Done. %.1fs' % (time.time() - start_time), prefix=False)
        elif os.path.isfile(local_path):
            name = name or os.path.basename(local_path)
            entry = ArtifactManifestEntry(path=name, ref=path, size=os.path.getsize(local_path), digest=md5(local_path))
            entries.append(entry)
        else:
