...
def ensure_key_present(module, session, name, pubkey, force, check_mode):
    ...
    for key in all_keys:
        ...
        if new_signature == existing_signature and key['title'] != name:
            module.fail_json(msg='another key with the same content is already registered under the name |{0}|'.format(key['title']))
    ...
    if not matching_keys:
        ...
        key = matching_keys[0]
    return {'changed': bool(deleted_keys or not matching_keys), 'deleted_keys': deleted_keys, 'matching_keys': matching_keys, 'key': key}