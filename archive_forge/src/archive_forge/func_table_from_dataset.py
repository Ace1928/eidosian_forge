...
def table_from_dataset(dataset=None, num_oov_buckets=0, vocab_size=None, default_value=None, hasher_spec=lookup_ops.FastHashSpec, key_dtype=dtypes.string, name=None):
  ...
  """
    ...
    if default_value is None:
        ...
        if not (elem_spec[1].dtype.is_integer or elem_spec[1].dtype.is_floating):
            raise ValueError(f'`default_value` must be specified when creating a table from a dataset that produces values of type {elem_spec[1].dtype}.')
    if num_oov_buckets < 0:
        raise ValueError(f'`num_oov_buckets` must be greater than or equal to 0, got {num_oov_buckets}.')
    if not isinstance(vocab_size, tensor.Tensor) and vocab_size is not None and (vocab_size < 1):
    ...
    if not key_dtype.is_integer and dtypes.string != key_dtype.base_dtype:
        raise TypeError(f'`key_dtype` must be either an integer or string type, but got {key_dtype}')
    ...
    with ops.name_scope(name, 'string_to_index'):
        ...
        with ops.name_scope(None, 'hash_table'):
            ...
            if num_oov_buckets:
                table = lookup_ops.IdTableWithHashBuckets(table, num_oov_buckets=num_oov_buckets, hasher_spec=hasher_spec, key_dtype=key_dtype)
            return table