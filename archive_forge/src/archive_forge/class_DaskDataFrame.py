...
class DaskDataFrame(DataFrame):
    ...
    def __init__(self, df: Any=None, schema: Any=None, num_partitions: int=0, type_safe=True):
        if num_partitions <= 0:
    ...
    def as_pandas(self) -> pd.DataFrame:
        ...
        return PD_UTILS.cast_df(pdf, self.schema.pa_schema, use_extension_types=True, use_arrow_dtype=False)

    def as_arrow(self, type_safe: bool=False) -> pa.Table:
        schema = self.schema.pa_schema
        return pa.concat_tables(collect(self.native, lambda df: cast_pa_table(pa.Table.from_pandas(df.reset_index(drop=True), schema=schema), schema=schema)))
    ...
    def as_dicts(self, columns: Optional[List[str]]=None, type_safe: bool=False) -> List[Dict[str, Any]]:
        return _dd_as_dicts(self.native, columns)

    def as_dict_iterable(self, columns: Optional[List[str]]=None, type_safe: bool=False) -> Iterable[Dict[str, Any]]:
    ...
    def _apply_schema(self, pdf: dd.DataFrame, schema: Optional[Schema], type_safe: bool=True) -> Tuple[dd.DataFrame, Schema]:
        if not type_safe:
        ...
        return (DASK_UTILS.cast_df(pdf, schema.pa_schema, use_extension_types=True, use_arrow_dtype=FUGUE_DASK_USE_ARROW), schema)