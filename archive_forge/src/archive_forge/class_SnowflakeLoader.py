...
class SnowflakeLoader(BaseLoader):
    ...
    def __init__(self, query: str, user: str, password: str, account: str, warehouse: str, role: str, database: str, schema: str, parameters: Optional[Dict[str, Any]]=None, page_content_columns: Optional[List[str]]=None, metadata_columns: Optional[List[str]]=None):
    ...
    def _execute_query(self) -> List[Dict[str, Any]]:
        try:
            ...
            raise ImportError('Could not import snowflake-connector-python package. Please install it with `pip install snowflake-connector-python`.') from ex
        conn = snowflake.connector.connect(user=self.user, password=self.password, account=self.account, warehouse=self.warehouse, role=self.role, database=self.database, schema=self.schema, parameters=self.parameters)
        try:
            cur = conn.cursor()
