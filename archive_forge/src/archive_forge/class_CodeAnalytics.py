...
class CodeAnalytics:
    ...
    def __init__(self):
        ...
        self.data_types: np.ndarray = np.array([], dtype='str')
        self.data_structures: np.ndarray = np.array([], dtype='str')
        self.arguments: pd.Series = pd.Series(dtype=object)
        self.return_values: pd.Series = pd.Series(dtype=object)
        self.exceptions: set = set()
        self.docstrings: pd.Series = pd.Series(dtype=str)
        self.comments: pd.Series = pd.Series(dtype=str)
        self.code_comments: pd.Series = pd.Series(dtype=str)
