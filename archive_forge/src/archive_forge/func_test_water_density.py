...
def test_water_density():
    ...
    assert abs(water_density(273.15 + 4) - 999.972) < 0.003
    ...
    try:
        ...
        import numpy as np
        unit = pq.kg / pq.m ** 3
        assert allclose(water_density(298.15 * pq.K, units=pq), 997.047021671824 * unit, atol=1e-08 * unit)
        assert allclose(water_density(np.linspace(297, 299) * pq.K, units=pq), 997 * unit, rtol=0.001, atol=0.001 * unit)
    except ImportError:
        pass