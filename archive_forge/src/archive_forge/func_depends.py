...
def depends(func, *dependencies, watch=False, on_init=False, **kw):
    ...
    on_init : bool, optional
        ...
        by default False
    """
    dependencies, kw = (tuple((transform_reference(arg) for arg in dependencies)), {key: transform_reference(arg) for key, arg in kw.items()})
    ...
    for dep in deps:
        if isinstance(dep, str):
            ...
            raise ValueError('The depends decorator only accepts string types referencing a parameter or parameter instances, found %s type instead.' % type(dep).__name__)
        elif not (isinstance(dep.owner, Parameterized) or isinstance(dep.owner, ParameterizedMetaclass)):
            owner = 'None' if dep.owner is None else '%s class' % type(dep.owner).__name__
            raise ValueError('Parameters supplied to the depends decorator, must be bound to a Parameterized class or instance, not %s.' % owner)
    if any((isinstance(dep, Parameter) for dep in deps)) and any((isinstance(dep, str) for dep in deps)):
        raise ValueError('Dependencies must either be defined as strings referencing parameters on the class defining the decorated method or as parameter instances. Mixing of string specs and parameter instances is not supported.')
    elif string_specs and kw:
        raise AssertionError('Supplying keywords to the decorated method or function is not supported when referencing parameters by name.')
    ...
    _dinfo.update({'dependencies': dependencies, 'kw': kw, 'watch': watch, 'on_init': on_init})
