...
class Composition(collections.abc.Hashable, collections.abc.Mapping, MSONable, Stringify):
    ...
    charge_balanced_tolerance = 1e-08
    special_formulas = dict(LiO='Li2O2', NaO='Na2O2', KO='K2O2', HO='H2O2', CsO='Cs2O2', RbO='Rb2O2', O='O2', N='N2', F='F2', Cl='Cl2', H='H2')
    oxi_prob = None
    ...
    def __init__(self, *args, strict: bool=False, **kwargs) -> None:
        ...
        if len(args) == 1 and isinstance(args[0], Composition):
            ...
            elem_map = self._parse_formula(args[0])
    ...
    def __getitem__(self, key: SpeciesLike) -> float:
        try:
            ...
            return sum((val for key, val in self._data.items() if getattr(key, 'symbol', key) == getattr(sp, 'symbol', sp)))
        except ValueError as exc:
            raise KeyError(f'Invalid key={key!r}') from exc
    ...
    def __contains__(self, key) -> bool:
        try:
            ...
            raise TypeError(f'Invalid key={key!r} for Composition') from exc
    ...
    def get_integer_formula_and_factor(self, max_denominator: int=10000, iupac_ordering: bool=False) -> tuple[str, float]:
    ...
    def charge(self) -> float | None:
        ...
        """
        warnings.warn('Composition.charge is experimental and may produce incorrect results. Use with caution and open a GitHub issue pinging @janosh to report bad behavior.')
    ...
    def replace(self, elem_map: dict[str, str | dict[str, float]]) -> Composition:
        ...
        if invalid_elems:
            warnings.warn(f'Some elements to be substituted are not present in composition. Please check your input. Problematic element = {invalid_elems}; {self}')
        ...
        for old_elem, new_elem in elem_map.items():
            ...
            for el, amt in subs.items():
                if el in new_comp:
                    ...
                    new_comp[el] = amt
                if el in self:
                    warnings.warn(f'Same element ({el}) in both the keys and values of the substitution!This can be ambiguous, so be sure to check your result.')
