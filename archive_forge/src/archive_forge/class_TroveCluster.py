...
class TroveCluster(resource.Resource):
    ...
    def handle_create(self):
        ...
        args = {'name': self._cluster_name(), 'datastore': datastore_type, 'datastore_version': datastore_version, 'instances': instances}
    ...
    def check_create_complete(self, cluster_id):
        ...
        for instance in cluster.instances:
            if instance['status'] in self.BAD_STATUSES:
                raise exception.ResourceInError(resource_status=instance['status'], status_reason=self.TROVE_STATUS_REASON.get(instance['status'], _('Unknown')))
    ...
    def validate(self):
        ...
        datastore_type = self.properties[self.DATASTORE_TYPE]
        datastore_version = self.properties[self.DATASTORE_VERSION]
        self.client_plugin().validate_datastore(datastore_type, datastore_version, self.DATASTORE_TYPE, self.DATASTORE_VERSION)
        ...
        for instance in self.properties[self.INSTANCES]:
            for nic in instance[self.NETWORKS]:
                ...
                if bool(nic.get(self.NET) is not None) == bool(nic.get(self.PORT) is not None):
                    msg = _('Either %(net)s or %(port)s must be provided.') % {'net': self.NET, 'port': self.PORT}
                    raise exception.StackValidationFailed(message=msg)
