...
class PointDNSDriver(DNSDriver):
    ...
    RECORD_TYPE_MAP = {RecordType.A: 'A', RecordType.AAAA: 'AAAA', RecordType.ALIAS: 'ALIAS', RecordType.CNAME: 'CNAME', RecordType.MX: 'MX', RecordType.NS: 'NS', RecordType.PTR: 'PTR', RecordType.SRV: 'SRV', RecordType.SSHFP: 'SSHFP', RecordType.TXT: 'TXT'}
    ...
    def get_zone(self, zone_id):
        ...
        try:
            ...
            if e.body == 'Not found':
                raise ZoneDoesNotExistError(driver=self, value="The zone doesn't exists", zone_id=zone_id)
            raise e
    ...
    def get_record(self, zone_id, record_id):
        ...
        try:
            ...
            if e.body == 'Not found':
                raise RecordDoesNotExistError(value="Record doesn't exists", driver=self, record_id=record_id)
            raise e
    ...
    def update_zone(self, zone, domain, type='master', ttl=None, extra=None):
        ...
        try:
            ...
            if isinstance(e, MalformedResponseError) and e.body == 'Not found':
                raise ZoneDoesNotExistError(value="Zone doesn't exists", driver=self, zone_id=zone.id)
            raise PointDNSException(value=e.message, http_code=e.code, driver=self)
        zone = self._to_zone(response.object)
        return zone
    ...
    def update_record(self, record, name, type, data, extra=None):
        ...
        r_data = json.dumps({'zone_record': r_json})
        try:
            response = self.connection.request('/zones/{}/records/{}'.format(zone.id, record.id), method='PUT', data=r_data)
        except (BaseHTTPError, MalformedResponseError) as e:
            if isinstance(e, MalformedResponseError) and e.body == 'Not found':
                raise RecordDoesNotExistError(value="Record doesn't exists", driver=self, record_id=record.id)
            raise PointDNSException(value=e.message, http_code=e.code, driver=self)
        record = self._to_record(response.object, zone=zone)
        return record
    ...
    def delete_zone(self, zone):
        ...
        try:
            ...
            if e.body == 'Not found':
                raise ZoneDoesNotExistError(driver=self, value="The zone doesn't exists", zone_id=zone.id)
            raise e
        return True
    ...
    def delete_record(self, record):
        ...
        record_id = record.id
        try:
            self.connection.request('/zones/{}/records/{}'.format(zone_id, record_id), method='DELETE')
        except MalformedResponseError as e:
            if e.body == 'Not found':
                raise RecordDoesNotExistError(value="Record doesn't exists", driver=self, record_id=record_id)
            raise e
    ...
    def ex_delete_mail_redirect(self, mail_r):
        ...
        mail_r_id = mail_r.id
        try:
            self.connection.request('/zones/{}/mail_redirects/{}'.format(zone_id, mail_r_id), method='DELETE')
        except (BaseHTTPError, MalformedResponseError) as e:
            if isinstance(e, MalformedResponseError) and e.body == 'Not found':
                raise PointDNSException(value="Couldn't found mail redirect", http_code=httplib.NOT_FOUND, driver=self)
            raise PointDNSException(value=e.message, http_code=e.code, driver=self)
