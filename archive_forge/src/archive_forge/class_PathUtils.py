...
class PathUtils(object):
    ...
    def copy(self, src, dest, fail_if_exists=True):
        ...
        try:
            self._win32_utils.run_and_check_output(kernel32.CopyFileW, ctypes.c_wchar_p(src), ctypes.c_wchar_p(dest), wintypes.BOOL(fail_if_exists), kernel32_lib_func=True)
        except exceptions.Win32Exception as exc:
            err_msg = _('The file copy from %(src)s to %(dest)s failed.Exception: %(exc)s')
    ...
    def create_sym_link(self, link, target, target_is_dir=True):
        ...
        """
        self._win32_utils.run_and_check_output(kernel32.CreateSymbolicLinkW, link, target, target_is_dir, kernel32_lib_func=True)

    def create_temporary_file(self, suffix=None, *args, **kwargs):
        fd, tmp_file_path = tempfile.mkstemp(*args, suffix=suffix, **kwargs)
        os.close(fd)
