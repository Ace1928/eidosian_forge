...
class PanelExecutor(WSHandler):
    ...
    def __init__(self, path, token, root_url):
        ...
        self._context = None
        self.resources = Resources(mode=os.environ.get('BOKEH_RESOURCES', 'server'), root_url=self.root_url, path_versioner=StaticHandler.append_version, absolute=True)
        self._set_state()
        try:
        ...
        with set_curdoc(self.session.document):
            if not self.session.document.roots:
                if self._error:
                    ...
                    return Mimebundle({'text/error': f"<span>Application errored while starting up:</span><br><br><div style='overflow: auto'>  <code style='font-size: 0.75em; white-space: pre;'>{error}</code></div>"})
                return Mimebundle({'text/error': "The Panel application being served did not serve any contents. Ensure you mark one or more Panel components in your notebook or script with .servable(), e.g. pn.Row('Hello World!').servable()."})
            html_page = server_html_page_for_session(self.session, resources=self.resources, title=self.session.document.title, template=self.session.document.template, template_variables=self.session.document.template_variables)
        return Mimebundle({'text/html': html_page, 'application/bokeh-extensions': {name: str(ext) for name, ext in extension_dirs.items()}})