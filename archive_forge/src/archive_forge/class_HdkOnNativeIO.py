...
class HdkOnNativeIO(BaseIO, TextFileDispatcher):
    ...
    unsupported_args = ['decimal', 'thousands', 'index_col', 'prefix', 'converters', 'skipfooter', 'nrows', 'skipinitialspace', 'na_values', 'keep_default_na', 'na_filter', 'verbose', 'infer_datetime_format', 'keep_date_col', 'date_parser', 'date_format', 'dayfirst', 'cache_dates', 'iterator', 'chunksize', 'encoding', 'encoding_errors', 'lineterminator', 'dialect', 'quoting', 'comment', 'on_bad_lines', 'low_memory', 'memory_map', 'float_precision', 'storage_options', 'dtype_backend']
    ...
    def read_csv(cls, **kwargs):
        ...
        try:
            ...
            use_modin_impl, error_message = cls._read_csv_check_support(kwargs)
            ...
            if dtype:
                ...
                column_types = {}
            if (parse_dates := (None if entire_dataframe_dtype else kwargs['parse_dates'])):
                if isinstance(parse_dates, list) and (all((isinstance(col, str) for col in parse_dates)) or all((isinstance(col, int) for col in parse_dates))):
                    ...
                    raise NotImplementedError(f'Argument parse_dates={parse_dates} is not supported')
            sep = kwargs['sep']
            delimiter = kwargs['delimiter']
            if delimiter is None and sep is not lib.no_default:
                delimiter = sep
            usecols_md = cls._prepare_pyarrow_usecols(kwargs)
            po = ParseOptions(delimiter='\\s+' if kwargs['delim_whitespace'] is True else delimiter, quote_char=kwargs['quotechar'], double_quote=kwargs['doublequote'], escape_char=kwargs['escapechar'], newlines_in_values=False, ignore_empty_lines=kwargs['skip_blank_lines'])
            true_values = kwargs['true_values']
            false_values = kwargs['false_values']
            co = ConvertOptions(check_utf8=None, column_types=column_types, null_values=None, true_values=true_values + ['TRUE', 'True', 'true'] if true_values is not None else true_values, false_values=false_values + ['False', 'FALSE', 'false'] if false_values is not None else false_values, timestamp_parsers=[''] if parse_dates is None or isinstance(parse_dates, bool) else None, strings_can_be_null=None, include_columns=usecols_md, include_missing_columns=None, auto_dict_encode=None, auto_dict_max_cardinality=None)
            ro = ReadOptions(use_threads=True, block_size=None, skip_rows=skiprows, column_names=names if names is not lib.no_default else None, autogenerate_column_names=None)
            at = read_csv(kwargs['filepath_or_buffer'], read_options=ro, parse_options=po, convert_options=co)
            if names:
        ...
        except (pa.ArrowNotImplementedError, pa.ArrowInvalid, NotImplementedError, ArrowEngineException) as err:
            if eng in ['arrow']:
                raise
            ErrorMessage.warn(f'Failed to read csv {kwargs['filepath_or_buffer']} ' + f'due to error: {err}. Defaulting to pandas.')
            return super().read_csv(**kwargs)
    ...
    @classmethod
    def _validate_read_csv_kwargs(cls, read_csv_kwargs: ReadCsvKwargsType):
        """
    ...
    def to_csv(cls, qc, **kwargs):
        ...
        for key, value in kwargs.items():
            ...
            elif not (key == 'na_rep' and len(value) == 0 or (key == 'decimal' and value == '.') or (key == 'quotechar' and value == '"') or (key == 'doublequote' and value is True) or (key == 'encoding' and value == 'utf-8') or (key == 'lineterminator' and value == os.linesep) or (key in ('path_or_buf', 'columns', 'header', 'index', 'index_label', 'mode', 'compression', 'errors', 'storage_options'))):
        ...
        if 'b' not in mode:
            mode += 'b'
        with get_handle(path_or_buf=path_or_buf, mode=mode, errors=kwargs.get('errors', 'strict'), compression=kwargs.get('compression', 'infer'), storage_options=kwargs.get('storage_options', None), is_text=False) as handles:
            out = handles.handle
            write_header(out)
            pa.csv.write_csv(at, out, write_opts)
    ...
    def read_sql(cls, **kwargs):
        ...
        if len(filtered) != len(kwargs):
            if (unsupported := {k: v for k, v in kwargs.items() if k not in filtered and v is not None}):
