...
class DatabricksArtifactRepository(CloudArtifactRepository):
    ...
    def __init__(self, artifact_uri):
        ...
        if not is_databricks_acled_artifacts_uri(artifact_uri):
            raise MlflowException(message='Artifact URI incorrect. Expected path prefix to be databricks/mlflow-tracking/path/to/artifact/..', error_code=INVALID_PARAMETER_VALUE)
        super().__init__(remove_databricks_profile_info_from_artifact_uri(artifact_uri))
        self.databricks_profile_uri = get_databricks_profile_uri_from_artifact_uri(artifact_uri) or mlflow.tracking.get_tracking_uri()
        ...
        run_relative_root_path = posixpath.relpath(path=artifact_repo_root_path, start=run_artifact_root_path)
        self.run_relative_artifact_repo_root_path = '' if run_artifact_root_path == artifact_repo_root_path else run_relative_root_path
    ...
    def _get_write_credential_infos(self, remote_file_paths):
        ...
        run-relative artifact `paths` within the MLflow Run specified by `run_id`.
        """
        run_relative_remote_paths = [posixpath.join(self.run_relative_artifact_repo_root_path, p or '') for p in remote_file_paths]
        return self._get_credential_infos(GetCredentialsForWrite, self.run_id, run_relative_remote_paths)
    ...
    def _get_read_credential_infos(self, remote_file_paths):
        ...
        if type(remote_file_paths) != list:
            raise MlflowException(f'Expected `paths` to be a list of strings. Got {type(remote_file_paths)}')
        run_relative_remote_paths = [posixpath.join(self.run_relative_artifact_repo_root_path, p) for p in remote_file_paths]
        return self._get_credential_infos(GetCredentialsForRead, self.run_id, run_relative_remote_paths)
    ...
    def _retryable_adls_function(self, func, artifact_file_path, **kwargs):
        try:
            ...
            if e.response.status_code in [403]:
                _logger.info('Failed to authorize ADLS operation, possibly due to credential expiration. Refreshing credentials and trying again...')
    ...
    def _upload_to_cloud(self, cloud_credential_info, src_file_path, artifact_file_path):
        ...
        if cloud_credential_info.type == ArtifactCredentialType.AZURE_SAS_URI:
            ...
            self._azure_adls_gen2_upload_file(cloud_credential_info, src_file_path, artifact_file_path)
            ...
            raise MlflowException(message='Cloud provider not supported.', error_code=INTERNAL_ERROR)
    ...
    def _download_from_cloud(self, remote_file_path, local_path):
        ...
        if cloud_credential_info.type not in [ArtifactCredentialType.AZURE_SAS_URI, ArtifactCredentialType.AZURE_ADLS_GEN2_SAS_URI, ArtifactCredentialType.AWS_PRESIGNED_URL, ArtifactCredentialType.GCP_SIGNED_URL]:
            raise MlflowException(message='Cloud provider not supported.', error_code=INTERNAL_ERROR)
        try:
            download_file_using_http_uri(cloud_credential_info.signed_uri, local_path, MLFLOW_MULTIPART_DOWNLOAD_CHUNK_SIZE.get(), self._extract_headers_from_credentials(cloud_credential_info.headers))
        except Exception as err:
            raise MlflowException(err)

    def _create_multipart_upload(self, run_id, path, num_parts):
        return self._call_endpoint(DatabricksMlflowArtifactsService, CreateMultipartUpload, message_to_json(CreateMultipartUpload(run_id=run_id, path=path, num_parts=num_parts)))

    def _get_presigned_upload_part_url(self, run_id, path, upload_id, part_number):
        return self._call_endpoint(DatabricksMlflowArtifactsService, GetPresignedUploadPartUrl, message_to_json(GetPresignedUploadPartUrl(run_id=run_id, path=path, upload_id=upload_id, part_number=part_number)))

    def _upload_part(self, cred_info, data):
        headers = self._extract_headers_from_credentials(cred_info.headers)
        with cloud_storage_http_request('put', cred_info.signed_uri, data=data, headers=headers) as response:
            augmented_raise_for_status(response)
    ...
    def _upload_parts(self, local_file, create_mpu_resp):
        ...
        for index, cred_info in enumerate(create_mpu_resp.upload_credential_infos):
            part_number = index + 1
            start_byte = index * MLFLOW_MULTIPART_UPLOAD_CHUNK_SIZE.get()
            future = self.chunk_thread_pool.submit(self._upload_part_retry, cred_info=cred_info, upload_id=create_mpu_resp.upload_id, part_number=part_number, local_file=local_file, start_byte=start_byte, size=MLFLOW_MULTIPART_UPLOAD_CHUNK_SIZE.get())
            futures[future] = part_number
        results, errors = _complete_futures(futures, local_file)
        if errors:
            raise MlflowException(f'Failed to upload at least one part of {local_file}. Errors: {errors}')
        return [PartEtag(part_number=part_number, etag=results[part_number]) for part_number in sorted(results)]
    ...
    def _multipart_upload(self, local_file, artifact_file_path):
        run_relative_artifact_path = posixpath.join(self.run_relative_artifact_repo_root_path, artifact_file_path or '')
        num_parts = _compute_num_chunks(local_file, MLFLOW_MULTIPART_UPLOAD_CHUNK_SIZE.get())
        create_mpu_resp = self._create_multipart_upload(self.run_id, run_relative_artifact_path, num_parts)
        try:
            part_etags = self._upload_parts(local_file, create_mpu_resp)
            self._complete_multipart_upload(self.run_id, run_relative_artifact_path, create_mpu_resp.upload_id, part_etags)
        except Exception as e:
            _logger.warning('Encountered an unexpected error during multipart upload: %s, aborting', e)
    ...
    def log_artifact(self, local_file, artifact_path=None):
        ...
        self._upload_to_cloud(cloud_credential_info=write_credential_info, src_file_path=local_file, artifact_file_path=artifact_file_path)
    ...
    def list_artifacts(self, path=None):
        ...
        page_token = None
        while True:
            json_body = message_to_json(ListArtifacts(run_id=self.run_id, path=run_relative_path, page_token=page_token))
            response = self._call_endpoint(MlflowService, ListArtifacts, json_body)
            artifact_list = response.files
            if len(artifact_list) == 1 and artifact_list[0].path == run_relative_path and (not artifact_list[0].is_dir):
                return []
            for output_file in artifact_list:
                file_rel_path = posixpath.relpath(path=output_file.path, start=self.run_relative_artifact_repo_root_path)
