...
class Dcm2nii(CommandLine):
    ...
    def _format_arg(self, opt, spec, val):
        if opt in ['anonymize', 'collapse_folders', 'date_in_filename', 'events_in_filename', 'source_in_filename', 'gzip_output', 'id_in_filename', 'nii_output', 'protocol_in_filename', 'reorient', 'spm_analyze', 'convert_all_pars', 'reorient_and_crop']:
            spec = deepcopy(spec)
            if val:
                spec.argstr += ' y'
            else:
    ...
    def _run_interface(self, runtime):
        ...
        self.output_files, self.reoriented_files, self.reoriented_and_cropped_files, self.bvecs, self.bvals = self._parse_stdout(new_runtime.stdout)
            ...
            if not skip:
                ...
                if line.startswith('Saving '):
                    ...
                    for l in (bvecs, bvals):
                        l[-1] = os.path.join(os.path.dirname(l[-1]), 'x%s' % (os.path.basename(l[-1]),))
                elif re.search('.*->(.*)', line):
                    val = re.search('.*->(.*)', line)
                    val = val.groups()[0]
                    if isdefined(self.inputs.output_dir):
