...
class ChatServiceManager(ABC):
    ...
    def _parse_config(self, opt: Opt):
        ...
        self.world_runner = ChatServiceWorldRunner(self.runner_opt, self.world_path, self.max_workers, self, opt['is_debug'])
        self.max_agents_for = {task: cfg.agents_required for task, cfg in self.task_configs.items()}
    ...
    def _get_unique_pool(self):
        ...
        for world_type, agent_pool in self.agent_pool.items():
            eligibility_function = utils.get_eligibility_fn(self.world_module, world_type)
            if eligibility_function is not None:
                valid_pools[world_type] = [w for w in agent_pool if eligibility_function(w)]
            else:
    ...
    def _on_new_message(self, message: Dict[str, Any]):
        ...
        if not self.world_runner.is_initialized():
            self.observe_message(agent_id, 'Please wait while the worlds are initializing...')
        ...
        if agent_state.get_active_agent() is None:
            if message.get('text', '') and message['text'].upper() == 'EXIT':
                ...
                self.observe_message(agent_id, 'Please wait while we pair you with another person. If you wish to exit, type *EXIT*.')
