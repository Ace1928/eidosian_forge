...
def _get_latest_versions():
    request_message = _get_request_message(GetLatestVersions(), schema={'name': [_assert_string, _assert_required], 'stages': [_assert_array, _assert_item_type_string]})
    latest_versions = _get_model_registry_store().get_latest_versions(name=request_message.name, stages=request_message.stages)
