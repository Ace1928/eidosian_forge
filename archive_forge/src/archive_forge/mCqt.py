...
def download_missing_package(detected_language_code: str) -> bool:
    print("Checking available packages for:", detected_language_code)
    for package in available_packages:
        print(f"Found package from {package.from_code} to {package.to_code}")
        if package.from_code == detected_language_code and package.to_code == "en":
            ...
            if not os.path.exists(model_path):
                ...
                return True
    print("No suitable package found for download.")
    return False
