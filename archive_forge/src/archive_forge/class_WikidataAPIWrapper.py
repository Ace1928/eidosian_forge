...
class WikidataAPIWrapper(BaseModel):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        try:
            ...
            raise ImportError('Could not import mediawikiapi python package. Please install it with `pip install mediawikiapi`.')
        try:
            from wikibase_rest_api_client import Client
            client = Client(timeout=60, base_url=WIKIDATA_REST_API_URL, headers={'User-Agent': WIKIDATA_USER_AGENT}, follow_redirects=True)
            values['wikidata_rest'] = client
        except ImportError:
            raise ImportError('Could not import wikibase_rest_api_client python package. Please install it with `pip install wikibase-rest-api-client`.')
        return values
