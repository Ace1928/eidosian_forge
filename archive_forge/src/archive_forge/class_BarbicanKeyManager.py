...
class BarbicanKeyManager(key_manager.KeyManager):
    ...
    def __init__(self, configuration):
        ...
        loading.register_session_conf_options(self.conf, _BARBICAN_SERVICE_USER_OPT_GROUP)
        loading.register_auth_conf_options(self.conf, _BARBICAN_SERVICE_USER_OPT_GROUP)
    ...
    def create_key(self, context, algorithm, length, expiration=None, name=None):
        ...
        try:
            key_order = barbican_client.orders.create_key(name=name, algorithm=algorithm, bit_length=length, expiration=expiration)
    ...
    def create_key_pair(self, context, algorithm, length, expiration=None, name=None):
        ...
        barbican_client = self._get_barbican_client(context)
        try:
            key_pair_order = barbican_client.orders.create_asymmetric(algorithm=algorithm, bit_length=length, name=name, expiration=expiration)
    ...
    def add_consumer(self, context, managed_object_id, consumer_data):
        ...
        try:
            secret_ref = self._create_secret_ref(managed_object_id)
            barbican_client.secrets.register_consumer(secret_ref, **consumer_data)
        except (barbican_exceptions.HTTPAuthError, barbican_exceptions.HTTPClientError, barbican_exceptions.HTTPServerError) as e:
    ...
    def remove_consumer(self, context, managed_object_id, consumer_data):
        ...
        try:
            secret_ref = self._create_secret_ref(managed_object_id)
            barbican_client.secrets.remove_consumer(secret_ref, **consumer_data)
        except (barbican_exceptions.HTTPAuthError, barbican_exceptions.HTTPClientError, barbican_exceptions.HTTPServerError) as e:
