...
class MTurkManager:
    ...
    def _assert_opts(self):
        ...
        if self.opt.get('allow_reviews') and len(self.mturk_agent_ids) != 2:
            shared_utils.print_and_log(logging.WARN, '[OPT CONFIGURATION ISSUE] allow_reviews is currently only supported on 2 person tasks, overriding this value to false.', should_print=True)
        ...
        if self.opt.get('frontend_version', 0) < 1:
            ...
            for feat in features:
                if self.opt.get(feat) is not None:
                    shared_utils.print_and_log(logging.WARN, '[OPT CONFIGURATION ISSUE] {} only works when using the react frontend (frontend_version >= 1), so this option will be ignored'.format(feat), should_print=True)
    ...
    def _maintain_hit_status(self):
        ...
        self.hit_status_thread = threading.Thread(target=update_status, name='Hit-Status-Thread', daemon=True)
    ...
    def _move_agents_to_waiting(self, agents):
        ...
        for agent in agents:
            ...
            if self.accepting_workers:
                agent.set_status(AssignState.STATUS_WAITING, conversation_id=conversation_id, agent_id='waiting')
    ...
    def _onboard_new_agent(self, mturk_agent):
        ...
        if assignment_id in self.assignment_to_onboard_thread:
            ...
            if agent.get_status() != AssignState.STATUS_NONE:
                return False
        onboard_thread = threading.Thread(target=_onboard_function, args=(mturk_agent,), name='onboard-{}-{}'.format(worker_id, assignment_id))
    ...
    def setup_server(self, task_directory_path=None):
        ...
        if self.opt['count_complete']:
            fin_word = 'finish'
        shared_utils.print_and_log(logging.INFO, "\nYou are going to allow workers from Amazon Mechanical Turk to be an agent in ParlAI.\nDuring this process, Internet connection is required, and you should turn off your computer's auto-sleep feature.", should_print=True)
        if self.opt['max_connections'] == 0:
            shared_utils.print_and_log(logging.INFO, 'Enough HITs will be created to fulfill {} times the number of conversations requested, extra HITs will be expired once the desired conversations {}.'.format(self.hit_mult, fin_word), should_print=True)
        else:
            shared_utils.print_and_log(logging.INFO, 'Enough HITs will be launched over time up to a max of {} times the amount requested until the desired number of conversations {}.'.format(self.hit_mult, fin_word), should_print=True)
        input('Please press Enter to continue... ')
        shared_utils.print_and_log(logging.NOTSET, '', True)
        if self.opt['local'] is True:
            shared_utils.print_and_log(logging.INFO, 'In order to run the server locally, you will need to have a public HTTPS endpoint (SSL signed) running on the server you are currently excecuting ParlAI on. Enter that public URL hostname when prompted and ensure that the port being used by ParlAI (usually 3000) has external traffic routed to it.', should_print=True)
            input('Please press Enter to continue... ')
        mturk_utils.setup_aws_credentials()
        num_assignments = self.required_hits
        payment_opt = {'type': 'reward', 'num_total_assignments': num_assignments, 'reward': self.opt['reward']}
        total_cost = mturk_utils.calculate_mturk_cost(payment_opt=payment_opt)
        if not mturk_utils.check_mturk_balance(balance_needed=total_cost, is_sandbox=self.opt['is_sandbox']):
            raise SystemExit('Insufficient funds')
        if not self.opt['is_sandbox'] and (total_cost > 100 or self.opt['reward'] > 1):
            confirm_string = '$%.2f' % total_cost
            expected_cost = total_cost / self.hit_mult
            expected_string = '$%.2f' % expected_cost
            shared_utils.print_and_log(logging.INFO, 'You are going to create {} HITs at {} per assignment, for a total cost up to {} after MTurk fees. Please enter "{}" to confirm and continue, and anything else to cancel.\nNote that of the {}, the target amount to spend is {}.'.format(self.required_hits, '$%.2f' % self.opt['reward'], confirm_string, confirm_string, confirm_string, expected_string), should_print=True)
            check = input('Enter here: ')
            if check != confirm_string and '$' + check != confirm_string:
                raise SystemExit('Cancelling')
        if not self.is_test:
            shared_utils.print_and_log(logging.INFO, 'Querying the parlai website for possible notices...', should_print=True)
        ...
        shared_utils.print_and_log(logging.INFO, 'Setting up MTurk server...', should_print=True)
        self.is_unique = self.opt['unique_worker']
        self.max_hits_per_worker = self.opt.get('max_hits_per_worker', 0)
        mturk_utils.create_hit_config(opt=self.opt, task_description=self.opt['task_description'], unique_worker=self.is_unique, is_sandbox=self.opt['is_sandbox'])
        task_name = '{}-{}'.format(str(uuid.uuid4())[:8], self.opt['task'])
        self.server_task_name = ''.join((e for e in task_name.lower() if e.isalnum() or e == '-'))
        if 'heroku_team' in self.opt:
            heroku_team = self.opt['heroku_team']
        else:
            heroku_team = None
        assert self.opt.get('frontend_version', 0) > 0, 'Tasks requiring the legacy frontend have to use the legacy infrastructure. This can be done by importing from parlai.mturk.core.legacy_2018.mturk_manager in your run code.'
        self.populate_task_files(task_directory_path)
        self.server_url = server_utils.setup_server(self.server_task_name, self.task_files_to_copy, self.opt['local'], heroku_team, self.opt['hobby'], tmp_dir=self.opt['tmp_dir'])
    ...
    def move_agent_to_task(self, agent, new_conversation_id):
        agent.set_status(AssignState.STATUS_IN_TASK, conversation_id=new_conversation_id, agent_id=agent.id)
        self._remove_from_agent_pool(agent)
    ...
    def start_task(self, eligibility_function, assign_role_function, get_task_world):
        ...
        """
        assert self.task_state >= self.STATE_HITS_MADE, 'Must have launched HITs with `mturk_manager.create_hits` to start the task'
        if callable(eligibility_function):
            ...
            if 'func' not in eligibility_function:
                shared_utils.print_and_log(logging.CRITICAL, "eligibility_function has no 'func'. Cancelling.")
                raise Exception('eligibility_function dict must contain a `func` field containing the actual function.')
            elif not callable(eligibility_function['func']):
                shared_utils.print_and_log(logging.CRITICAL, "eligibility_function['func'] not a function. Cancelling.")
                raise Exception("eligibility_function['func'] must contain a function. If eligibility_function['multiple'] is set, it should filter through the list of workers and only return those that are currently eligible to participate. If it is not set, it should take in a single worker and return whether or not they are eligible.")
        ...
        while not self.is_shutdown.is_set():
            ...
            with self.agent_pool_change_condition:
                ...
                if len(valid_agents) >= needed_agents:
                    ...
                    task_thread = threading.Thread(target=_task_function, args=(self.opt, agents, new_conversation_id), name='task-{}'.format(new_conversation_id))
    ...
    def shutdown(self, force=False):
        ...
        try:
            ...
            if self.server_task_name is not None:
                server_utils.delete_server(self.server_task_name, self.opt['local'], tmp_dir=self.opt['tmp_dir'])
    ...
    def send_message(self, receiver_id, assignment_id, data, blocking=True, ack_func=None):
        """
        ...
        packet = Packet(event_id, data_model.WORLD_MESSAGE, self.socket_manager.get_my_sender_id(), receiver_id, assignment_id, data, conversation_id=conversation_id, ack_func=ack_func)
        shared_utils.print_and_log(logging.INFO, 'Manager sending: {}'.format(packet), should_print=self.opt['verbose'])
        self.socket_manager.queue_packet(packet)
        return data['message_id']

    def send_command(self, receiver_id, assignment_id, data, blocking=True, ack_func=None):
        """
    ...
    def block_worker(self, worker_id, reason):
        ...
        shared_utils.print_and_log(logging.INFO, 'Worker {} blocked for reason {}.'.format(worker_id, reason))
