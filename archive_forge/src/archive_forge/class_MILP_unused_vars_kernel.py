...
class MILP_unused_vars_kernel(MILP_unused_vars):
    ...
    def _generate_model(self):
        ...
        model.X_unused = pmo.variable_dict(((i, pmo.variable(domain=pmo.IntegerSet)) for i in model.s))
        model.X_unused_initially_stale = pmo.variable_dict(((i, pmo.variable(domain=pmo.IntegerSet)) for i in model.s))
        for i in model.s:
        ...
        model.X = pmo.variable_dict(((i, pmo.variable(domain=pmo.IntegerSet)) for i in model.s))
        model.X_initially_stale = pmo.variable_dict(((i, pmo.variable(domain=pmo.IntegerSet)) for i in model.s))
        for i in model.s:
