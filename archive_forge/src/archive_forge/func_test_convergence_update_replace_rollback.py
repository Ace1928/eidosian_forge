...
def test_convergence_update_replace_rollback(self):
    ...
    rsrc_def = rsrc_defn.ResourceDefinition('test_res', 'ResourceWithPropsType')
    ...
    self.stack.state_set(self.stack.ROLLBACK, self.stack.IN_PROGRESS, 'Simulate rollback')
    res.restore_prev_rsrc = mock.Mock()
    tr = scheduler.TaskRunner(res.update_convergence, 'new_tmpl_id', set(), 'engine-007', self.dummy_timeout, new_stack, self.dummy_event)
