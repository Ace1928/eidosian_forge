...
class TableCoreManager(object):
    ...
    def remove_vrf_by_vrf_conf(self, vrf_conf):
        ...
        for destination in vrf_table.values():
            ...
            if best_path and best_path.source is None:
                vpn_clone = best_path.clone_to_vpn(vrf_conf.route_dist, for_withdrawal=True)
    ...
    def import_all_vpn_paths_to_vrf(self, vrf_table, import_rts=None):
        ...
        if vrf_table.route_family == Vrf4Table.ROUTE_FAMILY:
            ...
            raise ValueError('Invalid VRF table route family: %s' % vrf_table.route_family)
    ...
    def on_interesting_rts_change(self, new_global_rts, removed_global_rts):
        ...
        """
        if new_global_rts:
            LOG.debug('Sending route_refresh to all neighbors that did not negotiate RTC capability.')
            pm = self._core_service.peer_manager
            pm.schedule_rr_to_non_rtc_peers()
        if removed_global_rts:
            LOG.debug('Cleaning up global tables as some interested RTs were removed')
    ...
    def get_ipv4fs_table(self):
        ...
        if not ipv4fs_table:
            ipv4fs_table = IPv4FlowSpecTable(self._core_service, self._signal_bus)
    ...
    def get_ipv6fs_table(self):
        ...
        if not ipv6fs_table:
            ipv6fs_table = IPv6FlowSpecTable(self._core_service, self._signal_bus)
    ...
    def get_l2vpnfs_table(self):
        ...
        if not l2vpnfs_table:
            l2vpnfs_table = L2VPNFlowSpecTable(self._core_service, self._signal_bus)
    ...
    def update_vrf_table_links(self, vrf_table, new_imp_rts, removed_imp_rts):
        """Update mapping from RT to VRF table."""
        assert vrf_table
        if new_imp_rts:
            self._link_vrf_table(vrf_table, new_imp_rts)
        if removed_imp_rts:
            self._remove_links_to_vrf_table_for_rts(vrf_table, removed_imp_rts)
        ...
        for dest in vrf_table.values():
            for path in dest.known_path_list:
                if path.source is None:
                    vrf_table.insert_vrf_path(nlri=path.nlri, next_hop=path.nexthop, gen_lbl=True)
    ...
    def _remove_links_to_vrf_table(self, vrf_table):
        ...
        self._remove_links_to_vrf_table_for_rts(vrf_table, vrf_conf.import_rts)
        ...
        for rt in rts:
            ...
            if rt_specific_tables:
                try:
                    ...
                    LOG.debug('Did not find table listed as interested for its import RT: %s', rt)
    ...
    def update_global_table(self, prefix, next_hop=None, is_withdraw=False):
        ...
        if ip.valid_ipv4(addr):
            ...
            if next_hop is None:
                next_hop = '::'
            p = Ipv6Path
        new_path = p(peer, _nlri, src_ver_num, pattrs=pathattrs, nexthop=next_hop, is_withdraw=is_withdraw)
