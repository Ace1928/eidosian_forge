...
class DeviceAdministrationNetworkConditions(object):
    ...
    def get_object_by_name(self, name):
        result = None
        items = self.ise.exec(family='device_administration_network_conditions', function='get_device_admin_network_conditions').response.get('response', []) or []
        result = get_dict_result(items, 'name', name)
        return result

    def get_object_by_id(self, id):
        try:
            result = self.ise.exec(family='device_administration_network_conditions', function='get_device_admin_network_condition_by_id', params={'id': id}, handle_func_exception=False).response['response']
    ...
    def create(self):
        result = self.ise.exec(family='device_administration_network_conditions', function='create_device_admin_network_condition', params=self.new_object).response
        return result
    ...
    def update(self):
        ...
        result = self.ise.exec(family='device_administration_network_conditions', function='update_device_admin_network_condition_by_id', params=self.new_object).response
        return result
    ...
    def delete(self):
        ...
        if not id:
            id_ = self.get_object_by_name(name).get('id')
            self.new_object.update(dict(id=id_))
        result = self.ise.exec(family='device_administration_network_conditions', function='delete_device_admin_network_condition_by_id', params=self.new_object).response
        return result