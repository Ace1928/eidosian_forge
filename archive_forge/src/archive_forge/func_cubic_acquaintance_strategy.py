...
def cubic_acquaintance_strategy(qubits: Iterable['cirq.Qid'], swap_gate: 'cirq.Gate'=ops.SWAP) -> 'cirq.Circuit':
    ...
    for separation in range(1, max_separation):
        stepped_indices_concatenated = tuple(itertools.chain(*(range(offset, n_qubits, separation) for offset in range(separation))))
        ...
        for i in range(n_qubits + 1):
            for offset in range(3):
                moment = circuits.Moment((acquaint(*qubits[j:j + 3]) for j in range(offset, n_qubits - 2, 3)))
                moments.append(moment)
            if i < n_qubits:
                moment = circuits.Moment((swap_gate(*qubits[j:j + 2]) for j in range(i % 2, n_qubits - 1, 2)))
                moments.append(moment)
