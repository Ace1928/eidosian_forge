...
class LoggingCalculator(Calculator):
    ...
    def write_json(self, filename):
        with open(filename, 'w') as fd:
            json.dump({'fmax': self.fmax, 'walltime': self.walltime, 'energy_evals': self.energy_evals, 'energy_count': self.energy_count}, fd)
    ...
    def tabulate(self):
        ...
        for label in sorted(self.fmax.keys()):
            print(fmt2 % (label, len(self.fmax[label]), len(self.energy_count[label]), self.walltime[label][-1] - self.walltime[label][0]))

    def plot(self, fmaxlim=(0.01, 100.0), forces=True, energy=True, walltime=True, markers=None, labels=None, **kwargs):
        import matplotlib.pyplot as plt
        if markers is None:
            markers = [c + s for c in ['r', 'g', 'b', 'c', 'm', 'y', 'k'] for s in ['.-', '.--']]
