...
class IntervalTestBase(object):
    ...
    def test_div(self):
        ...
        y_bounds = [(np.random.uniform(-5, -2), np.random.uniform(2, 5)), (0, np.random.uniform(2, 5)), (np.random.uniform(0, 2), np.random.uniform(2, 5)), (np.random.uniform(-5, -2), 0), (np.random.uniform(-5, -2), np.random.uniform(-2, 0))]
        for xl, xu in x_bounds:
            for yl, yu in y_bounds:
                zl, zu = self.div(xl, xu, yl, yu, 1e-08)
                x = np.linspace(xl, xu, 100)
                y = np.linspace(yl, yu, 100)
                for _x in x:
                    _z = _x / y
    ...
    def test_pow(self):
        if not numpy_available:
            raise unittest.SkipTest('Numpy is not available.')
        x_bounds = [(np.random.uniform(0, 2), np.random.uniform(2, 5)), (0, np.random.uniform(2, 5)), (0, 0)]
        y_bounds = [(np.random.uniform(-5, -2), np.random.uniform(2, 5)), (0, np.random.uniform(2, 5)), (np.random.uniform(0, 2), np.random.uniform(2, 5)), (np.random.uniform(-5, -2), 0), (np.random.uniform(-5, -2), np.random.uniform(-2, 0))]
        for xl, xu in x_bounds:
            for yl, yu in y_bounds:
                zl, zu = self.power(xl, xu, yl, yu, 1e-08)
        ...
        x_bounds = [(np.random.uniform(-5, -2), np.random.uniform(2, 5)), (np.random.uniform(-5, -2), np.random.uniform(-2, 0)), (np.random.uniform(-5, -2), 0)]
                ...
                for _yl in yl:
                    for _yu in yu:
                        ...
                        if _xl == 0 and _xu == 0 and (_yu < 0):
                            lb, ub = self.power(_xl, _xu, _yl, _yu, 1e-08)
                            self.assertEqual(lb, -interval.inf)
                            self.assertEqual(ub, interval.inf)
                            ...
                            self.assertTrue(lb - 1e-08 <= estimated_lb)
                            self.assertTrue(ub + 1e-08 >= estimated_ub)
    ...
    def test_asin(self):
        if not numpy_available:
            raise unittest.SkipTest('Numpy is not available.')
        yl, yu = self.asin(-0.5, 0.5, -interval.inf, interval.inf, 1e-08)
        self.assertEqual(yl, -interval.inf)
        self.assertEqual(yu, interval.inf)
        yl, yu = self.asin(-0.5, 0.5, -math.pi, math.pi, 1e-08)
        self.assertAlmostEqual(yl, -math.pi, 12)
        self.assertAlmostEqual(yu, math.pi, 12)
        yl, yu = self.asin(-0.5, 0.5, -math.pi / 2, math.pi / 2, 1e-08)
        self.assertAlmostEqual(yl, math.asin(-0.5))
        self.assertAlmostEqual(yu, math.asin(0.5))
        yl, yu = self.asin(-0.5, 0.5, -math.pi / 2 - 0.1, math.pi / 2 + 0.1, 1e-08)
        self.assertAlmostEqual(yl, math.asin(-0.5))
        self.assertAlmostEqual(yu, math.asin(0.5))
        yl, yu = self.asin(-0.5, 0.5, -math.pi / 2 + 0.1, math.pi / 2 - 0.1, 1e-08)
        self.assertAlmostEqual(yl, math.asin(-0.5))
        self.assertAlmostEqual(yu, math.asin(0.5))
        yl, yu = self.asin(-0.5, 0.5, -1.5 * math.pi, 1.5 * math.pi, 1e-08)
        self.assertAlmostEqual(yl, -3.665191429188092, 12)
        self.assertAlmostEqual(yu, 3.665191429188092, 12)
        yl, yu = self.asin(-0.5, 0.5, -1.5 * math.pi - 0.1, 1.5 * math.pi + 0.1, 1e-08)
        self.assertAlmostEqual(yl, -3.665191429188092, 12)
        self.assertAlmostEqual(yu, 3.665191429188092, 12)
        yl, yu = self.asin(-0.5, 0.5, -1.5 * math.pi + 0.1, 1.5 * math.pi - 0.1, 1e-08)
        self.assertAlmostEqual(yl, -3.665191429188092, 12)
    ...
    def test_acos(self):
        if not numpy_available:
            raise unittest.SkipTest('Numpy is not available.')
        yl, yu = self.acos(-0.5, 0.5, -interval.inf, interval.inf, 1e-08)
        self.assertEqual(yl, -interval.inf)
        self.assertEqual(yu, interval.inf)
        yl, yu = self.acos(-0.5, 0.5, -0.5 * math.pi, 0.5 * math.pi, 1e-08)
        self.assertAlmostEqual(yl, -0.5 * math.pi, 12)
        self.assertAlmostEqual(yu, 0.5 * math.pi, 12)
        yl, yu = self.acos(-0.5, 0.5, 0, math.pi, 1e-08)
        self.assertAlmostEqual(yl, math.acos(0.5))
        self.assertAlmostEqual(yu, math.acos(-0.5))
        yl, yu = self.acos(-0.5, 0.5, 0 - 0.1, math.pi + 0.1, 1e-08)
        self.assertAlmostEqual(yl, math.acos(0.5))
        self.assertAlmostEqual(yu, math.acos(-0.5))
        yl, yu = self.acos(-0.5, 0.5, 0 + 0.1, math.pi - 0.1, 1e-08)
        self.assertAlmostEqual(yl, math.acos(0.5))
        self.assertAlmostEqual(yu, math.acos(-0.5))
        yl, yu = self.acos(-0.5, 0.5, -math.pi, 0, 1e-08)
        self.assertAlmostEqual(yl, -math.acos(-0.5), 12)
        self.assertAlmostEqual(yu, -math.acos(0.5), 12)
        yl, yu = self.acos(-0.5, 0.5, -math.pi - 0.1, 0 + 0.1, 1e-08)
        self.assertAlmostEqual(yl, -math.acos(-0.5), 12)
        self.assertAlmostEqual(yu, -math.acos(0.5), 12)
        yl, yu = self.acos(-0.5, 0.5, -math.pi + 0.1, 0 - 0.1, 1e-08)
        self.assertAlmostEqual(yl, -math.acos(-0.5), 12)
