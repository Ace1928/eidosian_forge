  ...
  """
    ...
    """
        for job_ref in job_utils.ExtractJobRefs(args):
            try:
                apis.Jobs.Drain(job_ref.jobId, project_id=job_ref.projectId, region_id=job_ref.location)
                log.status.Print('Started draining job [{0}]'.format(job_ref.jobId))
            except exceptions.HttpException as error:
                log.status.Print("Failed to drain job [{0}]: {1} Please ensure you have permission to access the job and the `--region` flag, {2}, matches the job's region.".format(job_ref.jobId, error.payload.status_message, job_ref.location))