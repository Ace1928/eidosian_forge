...
class TestVersionConverter(unittest.TestCase):
    ...
    def test_backwards_compatible(self) -> None:
        ...
        graph = helper.make_graph(nodes, 'test', [helper.make_tensor_value_info('X1', TensorProto.FLOAT, (5,)), helper.make_tensor_value_info('X2', TensorProto.FLOAT, (5,))], [helper.make_tensor_value_info('Y', TensorProto.FLOAT, (5,))])
    ...
    def test_add_8_5(self) -> None:
        nodes = [helper.make_node('Add', ['X1', 'X2'], ['Y'])]
        graph = helper.make_graph(nodes, 'test', [helper.make_tensor_value_info('X1', TensorProto.FLOAT, (5,)), helper.make_tensor_value_info('X2', TensorProto.FLOAT, (1,))], [helper.make_tensor_value_info('Y', TensorProto.FLOAT, (5,))])
    ...
    def test_max_7_8(self) -> None:
        ...
        graph = helper.make_graph(nodes, 'test_max', [onnx.helper.make_tensor_value_info('X', data_type, data_shape)], [onnx.helper.make_tensor_value_info('Y', data_type, data_shape)])
    ...
    def test_min_7_8(self) -> None:
        ...
        graph = helper.make_graph(nodes, 'test_min', [onnx.helper.make_tensor_value_info('X', data_type, data_shape)], [onnx.helper.make_tensor_value_info('Y', data_type, data_shape)])
        converted_model = self._converted(graph, helper.make_operatorsetid('', from_opset), to_opset)
    ...
    def test_less_8_9(self) -> None:
        ...
        input_shape = [2, 3, 4]
        graph = helper.make_graph(nodes, 'test_less', [onnx.helper.make_tensor_value_info('X1', data_type, input_shape), onnx.helper.make_tensor_value_info('X2', data_type, input_shape)], [onnx.helper.make_tensor_value_info('Y', TensorProto.BOOL, input_shape)])
        converted_model = self._converted(graph, helper.make_operatorsetid('', from_opset), to_opset)
    ...
    def test_matmul_8_9(self) -> None:
        ...
        graph = helper.make_graph(nodes, 'test_matmul', [onnx.helper.make_tensor_value_info('X1', data_type, [3, 4]), onnx.helper.make_tensor_value_info('X2', data_type, [4, 3])], [onnx.helper.make_tensor_value_info('Y', data_type, [3, 3])])
        converted_model = self._converted(graph, helper.make_operatorsetid('', from_opset), to_opset)
    ...
    def helper_upsample_with_initializer(self, raw_scale: bool=False) -> None:
        ...
        to_opset = 8
        data_type = TensorProto.FLOAT
        nodes = [onnx.helper.make_node('Upsample', inputs=['X', 'Scales'], outputs=['Y'], mode='nearest')]
        ...
        graph = helper.make_graph(nodes, 'test_upsample', [onnx.helper.make_tensor_value_info('X', data_type, [1, 1, 2, 2]), onnx.helper.make_tensor_value_info('Scales', data_type, [4])], [onnx.helper.make_tensor_value_info('Y', data_type, [1, 1, 4, 6])], [scale_tensor])
        converted_model = self._converted(graph, helper.make_operatorsetid('', from_opset), to_opset)
        assert converted_model.graph.node[0].op_type == 'Upsample'
    ...
    def test_split_13_12(self) -> None:
        nodes = [helper.make_node('Constant', [], ['split'], value=helper.make_tensor('', TensorProto.INT64, [2], [2, 3])), helper.make_node('Split', ['X', 'split'], ['Y1', 'Y2'])]
    ...
    def test_gru_13_14(self) -> None:
        ...
        num_directions = 1
        hidden_size = 5
        nodes = [onnx.helper.make_node('GRU', inputs=['X', 'W', 'R'], outputs=['', 'Y_h'], hidden_size=hidden_size)]
    ...
    def test_lstm_13_14(self) -> None:
        ...
        num_directions = 1
        hidden_size = 5
        nodes = [onnx.helper.make_node('LSTM', inputs=['X', 'W', 'R'], outputs=['', 'Y_h'], hidden_size=hidden_size)]
    ...
    def test_rnn_14_13(self) -> None:
        ...
        num_directions = 1
        hidden_size = 5
        nodes = [onnx.helper.make_node('RNN', inputs=['X', 'W', 'R'], outputs=['', 'Y_h'], hidden_size=hidden_size, layout=0)]
        graph = helper.make_graph(nodes, 'test_rnn', [onnx.helper.make_tensor_value_info('X', data_type, [seq_length, batch_size, input_size]), onnx.helper.make_tensor_value_info('W', data_type, [num_directions, hidden_size, input_size]), onnx.helper.make_tensor_value_info('R', data_type, [num_directions, hidden_size, hidden_size]), onnx.helper.make_tensor_value_info('B', data_type, [num_directions, 2 * hidden_size])], [onnx.helper.make_tensor_value_info('Y_h', data_type, [num_directions, batch_size, hidden_size])])
        converted_model = self._converted(graph, helper.make_operatorsetid('', from_opset), to_opset)
        assert converted_model.graph.node[0].op_type == 'RNN'
        assert converted_model.opset_import[0].version == to_opset
    ...
    def test_lstm_14_13(self) -> None:
        ...
        num_directions = 1
        hidden_size = 5
        nodes = [onnx.helper.make_node('LSTM', inputs=['X', 'W', 'R'], outputs=['', 'Y_h'], hidden_size=hidden_size, layout=0)]
        graph = helper.make_graph(nodes, 'test_lstm', [onnx.helper.make_tensor_value_info('X', data_type, [seq_length, batch_size, input_size]), onnx.helper.make_tensor_value_info('W', data_type, [num_directions, 4 * hidden_size, input_size]), onnx.helper.make_tensor_value_info('R', data_type, [num_directions, 4 * hidden_size, hidden_size]), onnx.helper.make_tensor_value_info('B', data_type, [num_directions, 8 * hidden_size])], [onnx.helper.make_tensor_value_info('Y_h', data_type, [num_directions, batch_size, hidden_size])])
        converted_model = self._converted(graph, helper.make_operatorsetid('', from_opset), to_opset)
        assert converted_model.graph.node[0].op_type == 'LSTM'
        assert converted_model.opset_import[0].version == to_opset
