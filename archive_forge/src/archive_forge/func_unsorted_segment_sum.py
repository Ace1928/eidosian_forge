...
def unsorted_segment_sum(data: _atypes.TensorFuzzingAnnotation[TV_UnsortedSegmentSum_T], segment_ids: _atypes.TensorFuzzingAnnotation[TV_UnsortedSegmentSum_Tindices], num_segments: _atypes.TensorFuzzingAnnotation[TV_UnsortedSegmentSum_Tnumsegments], name=None) -> _atypes.TensorFuzzingAnnotation[TV_UnsortedSegmentSum_T]:
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'UnsortedSegmentSum', name, data, segment_ids, num_segments)
            return _result
            ...
            pass
        try:
            _result = _dispatcher_for_unsorted_segment_sum((data, segment_ids, num_segments, name), None)
            ...
            _result = _dispatch.dispatch(unsorted_segment_sum, (), dict(data=data, segment_ids=segment_ids, num_segments=num_segments, name=name))
        ...
        if _result is not NotImplemented:
            return _result
    try:
        _, _, _op, _outputs = _op_def_library._apply_op_helper('UnsortedSegmentSum', data=data, segment_ids=segment_ids, num_segments=num_segments, name=name)
    except (TypeError, ValueError):
        _result = _dispatch.dispatch(unsorted_segment_sum, (), dict(data=data, segment_ids=segment_ids, num_segments=num_segments, name=name))
        if _result is not _dispatch.OpDispatcher.NOT_SUPPORTED:
            return _result
