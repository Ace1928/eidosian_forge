...
class AzureAiServicesImageAnalysisTool(BaseTool):
    ...
    image_analysis_client: Any
    visual_features: Any
    name: str = 'azure_ai_services_image_analysis'
    description: str = 'A wrapper around Azure AI Services Image Analysis. Useful for when you need to analyze images. Input should be a url to an image.'
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        try:
            ...
            raise ImportError('azure-ai-vision-imageanalysis is not installed. Run `pip install azure-ai-vision-imageanalysis` to install. ')
        'Validate Azure AI Vision Image Analysis client can be initialized.'
        try:
            values['image_analysis_client'] = ImageAnalysisClient(endpoint=azure_ai_services_endpoint, credential=AzureKeyCredential(azure_ai_services_key))
        except Exception as e:
            raise RuntimeError(f'Initialization of Azure AI Vision Image Analysis client failed: {e}')
        values['visual_features'] = [VisualFeatures.TAGS, VisualFeatures.OBJECTS, VisualFeatures.CAPTION, VisualFeatures.READ]
        return values
    ...
    def _image_analysis(self, image_path: str) -> Dict:
        ...
        if image_src_type == 'local':
            ...
            result = self.image_analysis_client.analyze(image_data=image_data, visual_features=self.visual_features)
        elif image_src_type == 'remote':
            result = self.image_analysis_client.analyze_from_url(image_url=image_path, visual_features=self.visual_features)
        else:
