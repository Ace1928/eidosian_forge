...
class RemovedEnableRules(base.SilentCommand):
  ...
  """
    ...
    """
        common_flags.consumer_service_flag(suffix='to remove enable rule for').AddToParser(parser)
        ...
        parser.add_argument('--force', action='store_true', help='If specified, the remove-enable-rules call will proceed even if there are enabled services which depend on the service to be removed from enable rule. Forcing the call means that the services which depend on the service to be removed from enable rule will also be removed  .')
    ...
    """
        ...
        for service_name in args.service:
            ...
            if protected_msg:
                ...
                if not do_disable:
                    continue
            op = serviceusage.RemoveEnableRule(project, service_name, args.policy_name, args.force, folder, organization, args.validate_only)
