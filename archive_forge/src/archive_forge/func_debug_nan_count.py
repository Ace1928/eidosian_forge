...
def debug_nan_count(input: _atypes.TensorFuzzingAnnotation[TV_DebugNanCount_T], device_name: str='', tensor_name: str='', debug_urls=[], gated_grpc: bool=False, name=None) -> _atypes.TensorFuzzingAnnotation[_atypes.Int64]:
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'DebugNanCount', name, input, 'device_name', device_name, 'tensor_name', tensor_name, 'debug_urls', debug_urls, 'gated_grpc', gated_grpc)
            return _result
            ...
            pass
        try:
            return debug_nan_count_eager_fallback(input, device_name=device_name, tensor_name=tensor_name, debug_urls=debug_urls, gated_grpc=gated_grpc, name=name, ctx=_ctx)
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('DebugNanCount', input=input, device_name=device_name, tensor_name=tensor_name, debug_urls=debug_urls, gated_grpc=gated_grpc, name=name)
    _result = _outputs[:]
