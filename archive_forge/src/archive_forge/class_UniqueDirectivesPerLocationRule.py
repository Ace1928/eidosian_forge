...
class UniqueDirectivesPerLocationRule(ASTValidationRule):
    ...
    def __init__(self, context: Union[ValidationContext, SDLValidationContext]):
        ...
        self.type_directives_map: Dict[str, Dict[str, DirectiveNode]] = defaultdict(dict)
        ...
        for directive in directives:
            ...
            if self.unique_directive_map.get(directive_name):
                if directive_name in seen_directives:
                    self.report_error(GraphQLError(f"The directive '@{directive_name}' can only be used once at this location.", [seen_directives[directive_name], directive]))
                else:
