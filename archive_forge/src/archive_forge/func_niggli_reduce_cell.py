...
def niggli_reduce_cell(cell, epsfactor=None):
    ...
    for _ in range(10000):
        ...
        D[5] *= ijk[0] * ijk[1]
        g = np.dot(D, g0)
        if gt(abs(g[3]), g[1]) or (eq(g[3], g[1]) and lt(2 * g[4], g[5])) or (eq(g[3], -g[1]) and lt(g[5], 0)):
            s = int(np.sign(g[3]))
            A = I3.copy()
            A[1, 2] = -s
            C = np.dot(C, A)
            ...
            D = np.dot(B, D)
            g = np.dot(D, g0)
        elif gt(abs(g[4]), g[0]) or (eq(g[4], g[0]) and lt(2 * g[3], g[5])) or (eq(g[4], -g[0]) and lt(g[5], 0)):
            s = int(np.sign(g[4]))
            A = I3.copy()
            A[0, 2] = -s
            C = np.dot(C, A)
            ...
            D = np.dot(B, D)
            g = np.dot(D, g0)
        elif gt(abs(g[5]), g[0]) or (eq(g[5], g[0]) and lt(2 * g[3], g[4])) or (eq(g[5], -g[0]) and lt(g[4], 0)):
            s = int(np.sign(g[5]))
            A = I3.copy()
            A[0, 1] = -s
            C = np.dot(C, A)
            ...
            D = np.dot(B, D)
            g = np.dot(D, g0)
        elif lt(g[[0, 1, 3, 4, 5]].sum(), 0) or (eq(g[[0, 1, 3, 4, 5]].sum(), 0) and gt(2 * (g[0] + g[4]) + g[5], 0)):
            A = I3.copy()
            A[:, 2] = 1
            C = np.dot(C, A)
            B = I6.copy()
