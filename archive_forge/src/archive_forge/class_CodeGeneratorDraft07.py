...
class CodeGeneratorDraft07(CodeGeneratorDraft06):
    FORMAT_REGEXS = dict(CodeGeneratorDraft06.FORMAT_REGEXS, **{'date': '^(?P<year>\\d{4})-(?P<month>\\d{2})-(?P<day>\\d{2})\\Z', 'iri': '^\\w+:(\\/?\\/?)[^\\s]+\\Z', 'iri-reference': '^(\\w+:(\\/?\\/?))?[^#\\\\\\s]*(#[^\\\\\\s]*)?\\Z', 'idn-email': '^[^@]+@[^@]+\\.[^@]+\\Z', 'relative-json-pointer': '^(?:0|[1-9][0-9]*)(?:#|(?:\\/(?:[^~/]|~0|~1)*)*)\\Z', 'time': '^(?P<hour>\\d{1,2}):(?P<minute>\\d{1,2})(?::(?P<second>\\d{1,2})(?:\\.(?P<microsecond>\\d{1,6}))?([zZ]|[+-]\\d\\d:\\d\\d)?)?\\Z'})
    ...
    def __init__(self, definition, resolver=None, formats={}, use_default=True, use_formats=True):
        ...
        self._json_keywords_to_function.update((('if', self.generate_if_then_else), ('contentEncoding', self.generate_content_encoding), ('contentMediaType', self.generate_content_media_type)))
