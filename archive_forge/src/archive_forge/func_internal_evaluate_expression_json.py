...
def internal_evaluate_expression_json(py_db, request, thread_id):
    ...
    with ctx:
        ...
        if frame_id is None:
            ...
            if is_error:
                if context == 'hover':
                    ...
                    msg = '%s: %s' % (eval_result.result.__class__.__name__, eval_result.result)
        ...
        if context == 'clipboard':
            safe_repr_custom_attrs = dict(maxstring_outer=2 ** 64, maxstring_inner=2 ** 64, maxother_outer=2 ** 64, maxother_inner=2 ** 64)
        if context == 'repl' and eval_result is None:
            ...
            body = pydevd_schema.EvaluateResponseBody(result=var_data['value'], variablesReference=var_data.get('variablesReference', 0), type=var_data.get('type'), presentationHint=var_data.get('presentationHint'), namedVariables=var_data.get('namedVariables'), indexedVariables=var_data.get('indexedVariables'))
