...
class NetworksSettings(object):
    ...
    def __init__(self, params, meraki):
        self.meraki = meraki
        self.new_object = dict(localStatusPageEnabled=params.get('localStatusPageEnabled'), remoteStatusPageEnabled=params.get('remoteStatusPageEnabled'), localStatusPage=params.get('localStatusPage'), securePort=params.get('securePort'), network_id=params.get('networkId'))
    ...
    def update_all_params(self):
        ...
        if self.new_object.get('localStatusPage') is not None or self.new_object.get('local_status_page') is not None:
            new_object_params['localStatusPage'] = self.new_object.get('localStatusPage') or self.new_object.get('local_status_page')
        if self.new_object.get('securePort') is not None or self.new_object.get('secure_port') is not None:
            new_object_params['securePort'] = self.new_object.get('securePort') or self.new_object.get('secure_port')
        if self.new_object.get('networkId') is not None or self.new_object.get('network_id') is not None:
            new_object_params['networkId'] = self.new_object.get('networkId') or self.new_object.get('network_id')
        return new_object_params
    ...
    def get_object_by_name(self, name):
        result = None
        try:
            items = self.meraki.exec_meraki(family='networks', function='getNetworkSettings', params=self.get_all_params(name=name))
    ...
    def requires_update(self, current_obj):
        ...
        obj_params = [('localStatusPageEnabled', 'localStatusPageEnabled'), ('remoteStatusPageEnabled', 'remoteStatusPageEnabled'), ('localStatusPage', 'localStatusPage'), ('securePort', 'securePort'), ('networkId', 'networkId')]
    ...
    def update(self):
        ...
        name = self.new_object.get('name')
        result = None
        result = self.meraki.exec_meraki(family='networks', function='updateNetworkSettings', params=self.update_all_params(), op_modifies=True)
        return result