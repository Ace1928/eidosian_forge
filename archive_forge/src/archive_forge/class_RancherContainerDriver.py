...
class RancherContainerDriver(ContainerDriver):
    ...
    def __init__(self, key, secret, secure=True, host='localhost', port=443):
        ...
        super().__init__(key=key, secret=secret, secure=False if parsed.scheme == 'http' else secure, host=parsed.hostname, port=parsed.port if parsed.port else port)
    ...
    def ex_deactivate_service(self, service_id):
        ...
        :rtype: ``bool``
        """
        result = self.connection.request('{}/services/{}?action=deactivate'.format(self.baseuri, service_id), method='POST')
        return result.status in VALID_RESPONSE_CODES
    ...
    def stop_container(self, container):
        ...
        :rtype: :class:`libcloud.container.base.Container`
        """
        result = self.connection.request('{}/containers/{}?action=stop'.format(self.baseuri, container.id), method='POST').object
        return self._to_container(result)
