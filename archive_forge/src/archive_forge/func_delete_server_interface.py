...
def delete_server_interface(self, server_interface, server=None, ignore_missing=True):
    """Delete a server interface
        ...
        :returns: ``None``
        """
    server_id = self._get_uri_attribute(server_interface, server, 'server_id')
    server_interface = resource.Resource._get_id(server_interface)
    self._delete(_server_interface.ServerInterface, server_interface, server_id=server_id, ignore_missing=ignore_missing)