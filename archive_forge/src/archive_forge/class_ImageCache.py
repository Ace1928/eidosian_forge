...
class ImageCache(object):
    ...
    def init_driver(self):
        ...
        try:
            ...
            LOG.warning(_LW("Image cache driver '%(driver_name)s' failed to load. Got error: '%(import_err)s."), {'driver_name': driver_name, 'import_err': import_err})
    ...
    def prune(self):
        ...
        LOG.debug('Image cache currently %(overage)d bytes over max size. Starting prune to max size of %(max_size)d ', {'overage': overage, 'max_size': max_size})
        ...
        while entry and current_size > max_size:
            image_id, size = entry
            LOG.debug("Pruning '%(image_id)s' to free %(size)d bytes", {'image_id': image_id, 'size': size})
            self.driver.delete_cached_image(image_id)
            total_bytes_pruned = total_bytes_pruned + size
            total_files_pruned = total_files_pruned + 1
            current_size = current_size - size
            entry = self.driver.get_least_recently_accessed()
        LOG.debug('Pruning finished pruning. Pruned %(total_files_pruned)d and %(total_bytes_pruned)d.', {'total_files_pruned': total_files_pruned, 'total_bytes_pruned': total_bytes_pruned})
