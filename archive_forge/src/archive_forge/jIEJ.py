...
async def graceful_shutdown(signals: List[int], loop: asyncio.AbstractEventLoop):
    """
    A coroutine that sets up signal handlers and ensures a graceful shutdown of the program.
...
def setup_signal_handlers(loop: asyncio.AbstractEventLoop):
    ...
    for sig in [signal.SIGTERM, signal.SIGINT]:
        loop.add_signal_handler(
            sig, lambda s=sig: asyncio.create_task(graceful_shutdown([s], loop))
        )

    logging.info("Signal handlers registered")


async def import_from_path(name: str, path: str) -> types.ModuleType:
    """
    Asynchronously imports a module from a given file path, ensuring compatibility with
    asynchronous programming paradigms and preventing blocking of the event loop.

    This function adapts its behavior based on the state of the event loop, ensuring that
    it can be called in both synchronous and asynchronous contexts without causing runtime errors.
    ...
    # Define a synchronous inner function to perform the actual import operation
    def _sync_import_module(name: str, path: str) -> types.ModuleType:
        ...
        if spec and spec.loader:
            ...
            raise ImportError(f"Cannot import module {name} from path {path}")

    # Attempt to get the running event loop; if successful, schedule the synchronous import operation
    # to run asynchronously, ensuring it does not block the event loop.
    try:
        loop = asyncio.get_running_loop()
        if loop.is_running():
            return await loop.run_in_executor(None, _sync_import_module, name, path)
        else:
            # If the loop is not running, perform the import operation directly.
            return _sync_import_module(name, path)
    except RuntimeError:
        # In case there's no running event loop, fall back to synchronous execution.
        return _sync_import_module(name, path)
...
class ResourceProfiler:
    ...
    def main_or_imported():
        if (
            __name__ == "__main__"
        ):  # If this is the root program then start the resource profiler
            profiler = ResourceProfiler(interval=60)
            asyncio.run(profiler.start())
        elif (
            __name__ == "standard_decorator"
        ):  # If this is imported and used in another program then start the resource profiler
            profiler = ResourceProfiler(interval=60)
            asyncio.run(profiler.start())
        else:
            # If this is imported and used in another program then start the resource profiler
    ...
    async def _log_resource_usage(self):
        ...
        logging.info(f"Resource Usage - CPU: {cpu_usage}%, Memory: {memory_usage}%")


signal.signal(
    signal.SIGTERM, setup_signal_handlers
)  # Register signal handlers for graceful shutdown
signal.signal(
    signal.SIGINT, setup_signal_handlers
)  # Register signal handlers for graceful shutdown
...
async def setup_logging(log_level=logging.DEBUG):
    """
    Sets up logging with both terminal and asynchronous file handlers.
    Ensures maximum verbosity and detail for logging, with non-blocking file I/O.
    Adapts to system resources and ensures fail-safe logging.
    """
    logger = logging.getLogger()
    logger.setLevel(log_level)

    # Resource Profiler
    resource_profiler = ResourceProfiler(interval=60)
    asyncio.run(resource_profiler.start())

    # Terminal handler for immediate console output
    ...
    # Asynchronous file handler for non-blocking file logging
    async_file_handler = AsyncFileHandler("application_async.log", mode="a")
    ...
    # Rotating file handler for archiving logs
    ...
    logging.info(
        "Logging setup complete with terminal, asynchronous, and rotating file handlers and profiling."
    )
...
class StandardDecorator:
    ...
    and dynamic input validation and sanitization. Designed for use across various projects,
    providing a robust framework for enhancing function execution with features like
    automatic retries on specified exceptions, performance logging, input validation, and result caching
    with a thread-safe strategy. It allows for granular control over logging levels and
    ...
    cache (Optional[collections.OrderedDict]): The cache storage.
    call_threshold (int): The threshold of calls to trigger specific actions.
    """
    ...
    def __init__(
        ...
        call_counter: int = 0,
        start_time: float = 0,  # Start time for performance logging
        end_time: float = 0,  # End time for performance logging
    ):
        ...
        self.call_counter: int = call_counter
        ...
        # Sets the start time when self.start_time is called. Checks if in an event loop or not and gets the time value accordingly using asyncio.time or time.time
        self.start_time = (
            asyncio.get_event_loop().time()
            if asyncio.get_event_loop().is_running()
            else time.time()  # Start time for performance logging
        )
        # Sets the end time when self.end_time is called. Checks if in an event loop or not and gets the time value accordingly using asyncio.time or time.time
        self.end_time = (
            asyncio.get_event_loop().time()
            if asyncio.get_event_loop().is_running()
            else time.time()  # End time for performance logging format/type timedelta/float
        )
        ...
        logging.debug(
            ...
            f"cache={cache}, call_threshold={call_threshold}, call_counter={call_counter}"
        )
...
class StandardDecorator:
    def __init__(
        ...
        asyncio.run(self._initialize_async_components())
        self.logger.info("StandardDecorator fully initialized.")

        # Initialize resource profiling if enabled
        if enable_resource_profiling:
            self._initialize_resource_profiler()

    def _initialize_resource_profiler(self):
        """
        Initializes and starts the resource profiler if enabled.
        """
        if self.enable_resource_profiling:
            ...
            asyncio.run(self.resource_profiler.start())
            self.logger.info("Resource profiling started.")
        else:
            self.logger.info("Resource profiling is disabled.")

    async def _graceful_shutdown(self, signals: List[int]):
        """
        Handles graceful shutdown for the decorator, including stopping the resource profiler.
        """
        if self.enable_resource_profiling:
            await self.resource_profiler.stop()
        # Additional shutdown procedures here
    ...
    async def _graceful_shutdown(self, signum, frame):
        """
    ...
    async def _setup_signal_handling(self, loop: asyncio.AbstractEventLoop):
        """
        Registers signal handlers for graceful shutdown.
        :param loop: The asyncio event loop.
        """
        ...
        for sig in [signal.SIGTERM, signal.SIGINT]:
            await loop.add_signal_handler(
                sig,
                lambda s=sig: asyncio.create_task(self._graceful_shutdown([s], loop)),
            )

        logging.info("Signal handlers registered")

    async def setup_logging(log_level: int = logging.DEBUG) -> None:
        """
        Sets up logging with terminal, asynchronous file handlers, and rotating file handlers.
        This function integrates the functionality of the AsyncFileHandler class for non-blocking file I/O,
        ensuring maximum verbosity and detail for logging. It adapts to system resources and ensures fail-safe logging.

        Args:
            log_level (int): The logging level to be set for the logger. Defaults to logging.DEBUG.
        """
        logger = logging.getLogger()
        logger.setLevel(log_level)

        # Resource Profiler initialization
        resource_profiler = ResourceProfiler(interval=60)
        await resource_profiler.start()  # Start the resource profiler asynchronously
    ...
    def _initialize_cache(self):
        """
        Initializes caching mechanisms based on configuration.
        """
        if self.enable_caching:
            ...
            self.logger.debug("Caching mechanisms initialized.")
        ...
        try:
            if os.path.exists(self.file_cache_path):
                async with aiofiles.open(self.file_cache_path, mode="rb") as file:
                    # Utilizing asyncio.to_thread to offload the blocking operation, pickle.load, to a separate thread.
                    self.file_cache = await asyncio.to_thread(pickle.load, file)
                    logging.debug(f"File cache successfully loaded from {self.file_cache_path}.")
            else:
    ...
    async def _save_file_cache(self) -> None:
        ...
        try:
            async with aiofiles.open(self.file_cache_path, mode="wb") as file:
                ...
                await asyncio.to_thread(pickle.dump, self.file_cache, file, pickle.HIGHEST_PROTOCOL)
                logging.debug("File cache has been successfully saved to disk.")
        except Exception as error:
