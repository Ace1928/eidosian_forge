...
class Typesense(VectorStore):
    ...
    def __init__(self, typesense_client: Client, embedding: Embeddings, *, typesense_collection_name: Optional[str]=None, text_key: str='text'):
        ...
        if not isinstance(typesense_client, Client):
            raise ValueError(f'typesense_client should be an instance of typesense.Client, got {type(typesense_client)}')
    ...
    def _create_collection(self, num_dim: int) -> None:
        fields = [{'name': 'vec', 'type': 'float[]', 'num_dim': num_dim}, {'name': f'{self._text_key}', 'type': 'string'}, {'name': '.*', 'type': 'auto'}]
        self._typesense_client.collections.create({'name': self._typesense_collection_name, 'fields': fields})
    ...
    def similarity_search_with_score(self, query: str, k: int=10, filter: Optional[str]='') -> List[Tuple[Document, float]]:
        ...
        query_obj = {'q': '*', 'vector_query': f'vec:([{','.join(embedded_query)}], k:{k})', 'filter_by': filter, 'collection': self._typesense_collection_name}
        docs = []
    ...
    def similarity_search(self, query: str, k: int=10, filter: Optional[str]='', **kwargs: Any) -> List[Document]:
    ...
    @classmethod
    def from_client_params(cls, embedding: Embeddings, *, host: str='localhost', port: Union[str, int]='8108', protocol: str='http', typesense_api_key: Optional[str]=None, connection_timeout_seconds: int=2, **kwargs: Any) -> Typesense:
        ...
        try:
            ...
            raise ValueError('Could not import typesense python package. Please install it with `pip install typesense`.')
        ...
        client_config = {'nodes': [node], 'api_key': typesense_api_key, 'connection_timeout_seconds': connection_timeout_seconds}
        return cls(Client(client_config), embedding, **kwargs)

    @classmethod
    def from_texts(cls, texts: List[str], embedding: Embeddings, metadatas: Optional[List[dict]]=None, ids: Optional[List[str]]=None, typesense_client: Optional[Client]=None, typesense_client_params: Optional[dict]=None, typesense_collection_name: Optional[str]=None, text_key: str='text', **kwargs: Any) -> Typesense:
