...
class YowMessagesProtocolLayer(YowProtocolLayer):
    ...
    def __init__(self):
        handleMap = {'message': (self.recvMessageStanza, self.sendMessageEntity)}
        ...
        if protoNode:
            if protoNode and protoNode['mediatype'] is None:
                ...
                if message.conversation:
                    self.toUpper(TextMessageProtocolEntity(message.conversation, MessageMetaAttributes.from_message_protocoltreenode(node)))
                elif message.extended_text:
                    self.toUpper(ExtendedTextMessageProtocolEntity(message.extended_text, MessageMetaAttributes.from_message_protocoltreenode(node)))
                elif not message.sender_key_distribution_message:
                    logger.warning('Unsupported message type: %s, will send receipts to prevent stream errors' % message)
                    self.toLower(OutgoingReceiptProtocolEntity(messageIds=[node['id']], to=node['from'], participant=node['participant']).toProtocolTreeNode())