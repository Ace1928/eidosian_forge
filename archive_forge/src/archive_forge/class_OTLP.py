...
class OTLP(base.Driver):
    ...
    def __init__(self, connection_str, project=None, service=None, host=None, conf=cfg.CONF, **kwargs):
        """OTLP driver using OTLP exporters."""
        super(OTLP, self).__init__(connection_str, project=project, service=service, host=host, conf=conf, **kwargs)
        try:
            ...
            raise exc.CommandError('To use OSProfiler with OTLP exporters, please install `opentelemetry-sdk` and opentelemetry-exporter-otlp libraries. To install with pip:\n `pip install opentelemetry-sdk opentelemetry-exporter-otlp`.')
        ...
        exporter = OTLPSpanExporter('{}/v1/traces'.format(parsed_url.geturl()))
        self.trace_api.get_tracer_provider().add_span_processor(BatchSpanProcessor(exporter))
    ...
    def _name(self, payload):
        info = payload['info']
        if info.get('request'):
            return 'WSGI_{}_{}'.format(info['request']['method'], info['request']['path'])
        elif info.get('db'):
            return 'SQL_{}'.format(info['db']['statement'].split(' ', 1)[0].upper())
        elif info.get('requests'):
            return 'REQUESTS_{}_{}'.format(info['requests']['method'], info['requests']['hostname'])
        return payload['name'].rstrip('-start')

    def notify(self, payload):
        if payload['name'].endswith('start'):
            parent = self.trace_api.SpanContext(trace_id=utils.uuid_to_int128(payload['base_id']), span_id=utils.shorten_id(payload['parent_id']), is_remote=False, trace_flags=self.trace_api.TraceFlags(self.trace_api.TraceFlags.SAMPLED))
            ctx = self.trace_api.set_span_in_context(self.trace_api.NonRecordingSpan(parent))
            span = self.tracer.start_span(name=self._name(payload), kind=self._kind(payload['name']), attributes=self.create_span_tags(payload), context=ctx)
            span._context = self.trace_api.SpanContext(trace_id=span.context.trace_id, span_id=utils.shorten_id(payload['trace_id']), is_remote=span.context.is_remote, trace_flags=span.context.trace_flags, trace_state=span.context.trace_state)
            ...
            if payload.get('info', {}).get('requests'):
                span.set_attribute('status_code', payload['info']['requests']['status_code'])
            if payload['info'].get('etype'):
                span.set_attribute('error', True)
                span.add_event('log', {'error.kind': payload['info']['etype'], 'message': payload['info']['message']})
            span.end()
