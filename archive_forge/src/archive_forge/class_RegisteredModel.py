...
class RegisteredModel(_ModelRegistryEntity):
    ...
    def __init__(self, name, creation_timestamp=None, last_updated_timestamp=None, description=None, latest_versions=None, tags=None, aliases=None):
        super().__init__()
        self._name = name
    ...
    def from_proto(cls, proto):
        registered_model = cls(proto.name, proto.creation_timestamp, proto.last_updated_timestamp, proto.description, [ModelVersion.from_proto(mvd) for mvd in proto.latest_versions])
    ...
    def to_proto(self):
        ...
        if self.latest_versions is not None:
            rmd.latest_versions.extend([model_version.to_proto() for model_version in self.latest_versions])
        rmd.tags.extend([ProtoRegisteredModelTag(key=key, value=value) for key, value in self._tags.items()])
        rmd.aliases.extend([ProtoRegisteredModelAlias(alias=alias, version=str(version)) for alias, version in self._aliases.items()])
        return rmd