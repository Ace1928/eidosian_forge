...
def debug_identity_v3_eager_fallback(input: _atypes.TensorFuzzingAnnotation[TV_DebugIdentityV3_T], device_name: str, tensor_name: str, io_of_node: str, is_input: bool, io_index: int, debug_urls, gated_grpc: bool, name, ctx) -> _atypes.TensorFuzzingAnnotation[TV_DebugIdentityV3_T]:
    ...
    _inputs_flat = [input]
    _attrs = ('T', _attr_T, 'device_name', device_name, 'tensor_name', tensor_name, 'io_of_node', io_of_node, 'is_input', is_input, 'io_index', io_index, 'debug_urls', debug_urls, 'gated_grpc', gated_grpc)
    _result = _execute.execute(b'DebugIdentityV3', 1, inputs=_inputs_flat, attrs=_attrs, ctx=ctx, name=name)
