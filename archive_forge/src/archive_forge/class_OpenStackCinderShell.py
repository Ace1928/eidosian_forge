...
class OpenStackCinderShell(object):
    ...
    def get_base_parser(self):
        parser = CinderClientArgumentParser(prog='cinder', description=__doc__.strip(), epilog=_('Run "cinder help SUBCOMMAND" for help on a subcommand.'), add_help=False, formatter_class=OpenStackHelpFormatter)
        parser.add_argument('-h', '--help', action='store_true', help=argparse.SUPPRESS)
        parser.add_argument('--version', action='version', version=cinderclient.__version__)
        parser.add_argument('-d', '--debug', action='store_true', default=utils.env('CINDERCLIENT_DEBUG', default=False), help=_('Shows debugging output.'))
        parser.add_argument('--service-type', metavar='<service-type>', help=_('Service type. For most actions, default is volume.'))
        parser.add_argument('--service_type', help=argparse.SUPPRESS)
        parser.add_argument('--service-name', metavar='<service-name>', default=utils.env('CINDER_SERVICE_NAME'), help=_('Service name. Default=env[CINDER_SERVICE_NAME].'))
        parser.add_argument('--service_name', help=argparse.SUPPRESS)
        parser.add_argument('--volume-service-name', metavar='<volume-service-name>', default=utils.env('CINDER_VOLUME_SERVICE_NAME'), help=_('Volume service name. Default=env[CINDER_VOLUME_SERVICE_NAME].'))
        parser.add_argument('--volume_service_name', help=argparse.SUPPRESS)
        parser.add_argument('--os-endpoint-type', metavar='<os-endpoint-type>', default=utils.env('CINDER_ENDPOINT_TYPE', default=utils.env('OS_ENDPOINT_TYPE', default=DEFAULT_CINDER_ENDPOINT_TYPE)), help=_('Endpoint type, which is publicURL or internalURL. Default=env[OS_ENDPOINT_TYPE] or nova env[CINDER_ENDPOINT_TYPE] or %s.') % DEFAULT_CINDER_ENDPOINT_TYPE)
        parser.add_argument('--os_endpoint_type', help=argparse.SUPPRESS)
        parser.add_argument('--os-volume-api-version', metavar='<volume-api-ver>', default=utils.env('OS_VOLUME_API_VERSION', default=None), help=_("Block Storage API version. Accepts X, X.Y (where X is major and Y is minor part).  NOTE: this client accepts only '3' for the major version. Default=env[OS_VOLUME_API_VERSION]."))
        parser.add_argument('--os_volume_api_version', help=argparse.SUPPRESS)
        parser.add_argument('--os-endpoint', metavar='<os-endpoint>', dest='os_endpoint', default=utils.env('CINDER_ENDPOINT'), help=_('Use this API endpoint instead of the Service Catalog. Defaults to env[CINDER_ENDPOINT].'))
    ...
    def get_subcommand_parser(self, version, do_help=False, input_args=None):
        ...
        self._find_actions(subparsers, actions_module, version, do_help, input_args)
        self._find_actions(subparsers, self, version, do_help, input_args)
        for extension in self.extensions:
            self._find_actions(subparsers, extension.module, version, do_help, input_args)
    ...
    def _find_actions(self, subparsers, actions_module, version, do_help, input_args):
        for attr in (a for a in dir(actions_module) if a.startswith('do_')):
            ...
            if hasattr(callback, 'versioned'):
                ...
                if do_help:
                    additional_msg = self._build_versioned_help_message(subs[0].start_version, subs[-1].end_version)
            ...
            self._add_subparser_args(subparser, arguments, version, do_help, input_args, command)
            self._add_subparser_exclusive_args(subparser, exclusive_args, version, do_help, input_args, command)
            subparser.set_defaults(func=callback)

    def _add_subparser_args(self, subparser, arguments, version, do_help, input_args, command):
        ...
        for args, kwargs in arguments:
            ...
            if do_help and (start_version or end_version):
                kwargs['help'] = kwargs.get('help', '') + self._build_versioned_help_message(start_version, end_version)
            if not version.matches(start_version, end_version):
                if args[0] in input_args and command == input_args[0]:
                    if arg_counter[args[0]] == 1:
                        raise exc.UnsupportedAttribute(args[0], start_version, end_version)
                    arg_counter[args[0]] -= 1
                continue
            kw = kwargs.copy()
    ...
    def _add_subparser_exclusive_args(self, subparser, exclusive_args, version, do_help, input_args, command):
        for group_name, arguments in exclusive_args.items():
            ...
            self._add_subparser_args(exclusive_group, arguments, version, do_help, input_args, command)
    ...
    def main(self, argv):
        ...
        subcommand_parser = self.get_subcommand_parser(api_version, do_help, args)
        ...
        if not utils.isunauthenticated(args.func) and (not auth_session):
            if not os_password:
                if hasattr(sys.stdin, 'isatty') and sys.stdin.isatty():
                    try:
                        ...
                        pass
                if not os_password:
                    raise exc.CommandError('You must provide a password through --os-password, env[OS_PASSWORD] or, prompted response.')
            ...
            if not os_auth_url:
                raise exc.CommandError('You must provide an authentication URL through --os-auth-url or env[OS_AUTH_URL].')
        if not project_info_provided:
            raise exc.CommandError(_('You must provide a project_id or project_name (with project_domain_name or project_domain_id) via   --os-project-id (env[OS_PROJECT_ID])  --os-project-name (env[OS_PROJECT_NAME]),  --os-project-domain-id (env[OS_PROJECT_DOMAIN_ID])  --os-project-domain-name (env[OS_PROJECT_DOMAIN_NAME])'))
        ...
        if not isinstance(auth_session, session.Session) and getattr(args, 'collect_timing', False) is True:
            raise exc.AuthorizationFailure("Provided auth plugin doesn't support collect_timing option")
        insecure = self.options.insecure
        client_args = dict(region_name=os_region_name, tenant_id=os_project_id, endpoint_type=endpoint_type, extensions=self.extensions, service_type=service_type, service_name=service_name, volume_service_name=volume_service_name, os_endpoint=os_endpoint, retries=options.retries, http_log_debug=args.debug, insecure=insecure, cacert=cacert, auth_system=os_auth_type, auth_plugin=auth_plugin, session=auth_session, logger=self.ks_logger if auth_session else self.client_logger)
        self.cs = client.Client(api_version, os_username, os_password, os_project_name, os_auth_url, **client_args)
        try:
        ...
        try:
            ...
            if api_version_input and endpoint_api_version:
                logger.warning('Cannot determine the API version from the endpoint URL. Falling back to the user-specified version: %s', endpoint_api_version)
            elif endpoint_api_version:
                logger.warning('Cannot determine the API version from the endpoint URL or user input. Falling back to the default API version: %s', endpoint_api_version)
            else:
                msg = _('Cannot determine API version.  Please specify by using --os-volume-api-version option.')
        ...
        if endpoint_api_version[0] == '3':
            disc_client = client.Client(API_MAX_VERSION, os_username, os_password, os_project_name, os_auth_url, **client_args)
            self.cs, discovered_version = self._discover_client(disc_client, api_version, args.os_endpoint_type, args.service_type, os_username, os_password, os_project_name, os_auth_url, client_args)
    ...
    @utils.arg('command', metavar='<subcommand>', nargs='?', help='Shows help for <subcommand>.')
    ...
    def _discover_auth_versions(self, session, auth_url):
        ...
        try:
            ...
            if path.startswith('/v3'):
                ...
                raise exc.CommandError('Unable to determine the Keystone version to authenticate with using the given auth_url.')
