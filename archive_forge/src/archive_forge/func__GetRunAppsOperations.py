...
def _GetRunAppsOperations(region, service_account, release_track):
    ...
    if not region:
        raise exceptions.ArgumentError('You must specify a region. Either use the `--region` flag or set the run/region property.')
    return RunAppsOperations(client, _RUN_APPS_API_VERSION, region, release_track, service_account)