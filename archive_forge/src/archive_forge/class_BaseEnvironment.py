...
class BaseEnvironment(ABC):
    ...
    def is_empty(self, user_id):
        return len(self.histories[user_id]['user_message']) == 0 and len(self.histories[user_id]['bot_message']) == 0 and (len(self.histories[user_id]['model_input']) == 0) and (len(self.histories[user_id]['prefix']) == 0) and (len(self.histories[user_id]['chosen_topic']) == 0)
