...
class L7RuleTest(common.HeatTestCase):
    ...
    def test_resource_mapping(self):
        ...
        self.assertEqual(mapping['OS::Octavia::L7Rule'], l7rule.L7Rule)
    ...
    def _create_stack(self, tmpl=inline_templates.L7RULE_TEMPLATE):
        ...
        self.l7rule.client = mock.MagicMock(return_value=self.octavia_client)
        self.l7rule.client_plugin().client = mock.MagicMock(return_value=self.octavia_client)
    ...
    def test_create(self):
        self._create_stack()
        self.octavia_client.l7rule_show.side_effect = [{'provisioning_status': 'PENDING_CREATE'}, {'provisioning_status': 'PENDING_CREATE'}, {'provisioning_status': 'ACTIVE'}]
        self.octavia_client.l7rule_create.side_effect = [exceptions.Conflict(409), {'rule': {'id': '1234'}}]
        expected = {'rule': {'admin_state_up': True, 'invert': False, 'type': u'HEADER', 'compare_type': u'ENDS_WITH', 'key': u'test_key', 'value': u'test_value', 'invert': False}}
        props = self.l7rule.handle_create()
        self.assertFalse(self.l7rule.check_create_complete(props))
        self.octavia_client.l7rule_create.assert_called_with('123', json=expected)
        self.assertFalse(self.l7rule.check_create_complete(props))
        self.octavia_client.l7rule_create.assert_called_with('123', json=expected)
        self.assertFalse(self.l7rule.check_create_complete(props))
    ...
    def test_create_missing_properties(self):
        for prop in ('l7policy', 'type', 'compare_type', 'value'):
            ...
            self._create_stack(tmpl=yaml.safe_dump(tmpl))
            self.assertRaises(exception.StackValidationFailed, self.l7rule.validate)
    ...
    def test_update(self):
        self._create_stack()
        self.l7rule.resource_id_set('1234')
        self.octavia_client.l7rule_show.side_effect = [{'provisioning_status': 'PENDING_UPDATE'}, {'provisioning_status': 'PENDING_UPDATE'}, {'provisioning_status': 'ACTIVE'}]
        self.octavia_client.l7rule_set.side_effect = [exceptions.Conflict(409), None]
        prop_diff = {'admin_state_up': False, 'name': 'your_l7policy', 'redirect_url': 'http://www.google.com'}
        ...
        self.octavia_client.l7rule_set.assert_called_with('1234', '123', json={'rule': prop_diff})
        self.assertFalse(self.l7rule.check_update_complete(prop_diff))
    ...
    def test_delete(self):
        self._create_stack()
        self.l7rule.resource_id_set('1234')
        self.octavia_client.l7rule_show.side_effect = [{'provisioning_status': 'PENDING_DELETE'}, {'provisioning_status': 'PENDING_DELETE'}, {'provisioning_status': 'DELETED'}]
        self.octavia_client.l7rule_delete.side_effect = [exceptions.Conflict(409), None]
        ...
        self.octavia_client.l7rule_delete.assert_called_with('1234', '123')
    ...
    def test_delete_failed(self):
        self._create_stack()
        self.l7rule.resource_id_set('1234')
        self.octavia_client.l7rule_delete.side_effect = exceptions.Unauthorized(401)
        self.l7rule.handle_delete()
        self.assertRaises(exceptions.Unauthorized, self.l7rule.check_delete_complete, None)