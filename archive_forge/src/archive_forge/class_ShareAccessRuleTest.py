...
class ShareAccessRuleTest(base.BaseSharedFileSystemTest):
    ...
    def setUp(self):
        ...
        mys = self.create_share(name=self.SHARE_NAME, size=2, share_type='dhss_false', share_protocol='NFS', description=None)
        self.user_cloud.shared_file_system.wait_for_status(mys, status='available', failures=['error'], interval=5, wait=self._wait_for_timeout)
    ...
    def test_list_access_rules(self):
        ...
        for rule in rules:
            for attribute in ('id', 'created_at', 'updated_at', 'access_level', 'access_type', 'access_to', 'share_id', 'access_key', 'metadata'):
