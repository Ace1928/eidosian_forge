...
def handle_boto_error(exc, msg, *args, **kwargs):
    ...
    generic_message_args = ['{}\nError code: {}', msg.format(*args, **kwargs), cf.bold(error_code)]
    credentials_expiration_codes = ['ExpiredTokenException', 'ExpiredToken', 'RequestExpired']
    if error_code in credentials_expiration_codes:
        token_command = 'aws sts get-session-token --serial-number arn:aws:iam::' + cf.underlined('ROOT_ACCOUNT_ID') + ':mfa/' + cf.underlined('AWS_USERNAME') + ' --token-code ' + cf.underlined('TWO_FACTOR_AUTH_CODE')
        secret_key_var = 'export AWS_SECRET_ACCESS_KEY = ' + cf.underlined('REPLACE_ME') + ' # found at Credentials.SecretAccessKey'
        session_token_var = 'export AWS_SESSION_TOKEN = ' + cf.underlined('REPLACE_ME') + ' # found at Credentials.SessionToken'
        access_key_id_var = 'export AWS_ACCESS_KEY_ID = ' + cf.underlined('REPLACE_ME') + ' # found at Credentials.AccessKeyId'
        aws_session_script_url = 'https://gist.github.com/maximsmol/a0284e1d97b25d417bd9ae02e5f450cf'
