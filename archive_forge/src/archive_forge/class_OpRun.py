...
class OpRun(abc.ABC):
    ...
    def __init__(self, onnx_node: NodeProto, run_params: dict[str, Any], schema: Any=None):
        ...
        for att in ['opsets', 'new_ops']:
            if att not in run_params:
                raise RuntimeError(f'Attribute {att!r} must be in run_params, only {sorted(run_params)} was found.')
    ...
    def _load_attributes(self) -> None:
        ...
        for att in self.onnx_node.attribute:
            ...
            if att.type == AttributeProto.GRAPH:
                ...
                setattr(self, f'_run_{att.name}', lambda context, value=value, attributes=None: OpRun._evaluate_subgraph(context, value, attributes))
            ...
            for k, v in self._schema.attributes.items():
                if not hasattr(self, k):
                    if getattr(v, 'required', True):
                        raise RuntimeError(f'Attribute {k!r} is expected based on ONNX specifications for node {self.onnx_node.op_type!r}.')
                    if hasattr(v, 'default_value'):
                        if v.default_value.type == 0 or (v.default_value.type == 4 and v.default_value.t.data_type == 0):
                            value = None
                        else:
    ...
    def _run(self, *args, **kwargs):
        ...
        Returns:
            outputs
        """
        raise NotImplementedError(f"Method '_run' must be overwritten for operator {self.__class__.__name__!r}.")
    ...
    def run(self, *args, linked_attributes=None, context=None):
        ...
        if self.need_context():
            if context is None:
                raise RuntimeError(f'This node if type {type(self)} needs context to be filled.')
        elif context is not None:
            raise RuntimeError(f'This node if type {type(self)} does not need any contextbut one is given.')
        if self.has_linked_attribute and linked_attributes is None:
            raise ValueError(f"This node {type(self)} has linked attributes but None are given in parameter 'linked_attributes'.")
        if not self.has_linked_attribute and linked_attributes is not None:
            raise ValueError(f"This node {type(self)} has no linked attribute but some are given in parameter 'linked_attributes' {set(linked_attributes)}.")
        overridden_attributes = {}
        if self.has_linked_attribute:
            if linked_attributes is None:
                raise AttributeError(f'One attribute is linked but no linked value is provided, in class {type(self)}.')
            for att in self.attributes_names_:
                v = getattr(self, att)
                if isinstance(v, RefAttrName):
                    if v.name not in linked_attributes:
                        raise ValueError(f'Unable to find a value for linked attribute {att!r} in {linked_attributes!r} in node {type(self)}.')
        ...
        for att in self.attributes_names_:
            ...
            if not hasattr(self, att):
                raise NameError(f'Attribute {att!r} is missing in operator {self.__class__.__name__!r}.')
            kwargs[att] = getattr(self, att)
        if self.has_subgraph:
            if self.has_linked_attribute and (not linked_attributes):
                raise RuntimeError(f'A subgraph has linked attribute but none was given to {type(self)}.')
        ...
        try:
            ...
            raise TypeError(f'Issues with types {[type(_) for _ in args]} and attributes {sorted(kwargs)} and linked attributes={sorted(overridden_attributes)} (operator {self.__class__.__name__!r}).') from e
        self._log('-- done %s.run -> %d outputs', self.__class__.__name__, len(res) if res is not None else 0)
    ...
    @classmethod
    def make_node(cls, n_inputs: int | None=None, n_outputs: int | None=None, **kwargs: Any) -> NodeProto:
    ...
    def create(cls, n_inputs: int | None=None, n_outputs: int | None=None, verbose: int=0, **kwargs: Any) -> Any:
        ...
        run_params = {'verbose': verbose, 'log': log_function, 'new_ops': None, 'opsets': {'': onnx_opset_version()}}
