...
class APIKeyDataFlow(StatefulProperty[APIKeyData]):
    ...
    async def aset(self, user_data: 'AZUserData', claims: 'UserJWTClaims') -> 'APIKeyData':
        """
        Saves the Data / Claims for the User ID
        """
        data = self.schema_type(user_data=user_data, claims=claims.to_api_key_claims() if hasattr(claims, 'to_api_key_claims') else claims)
        await self.pdict.aset(self.cache_key, data)
        return data
    if TYPE_CHECKING:
