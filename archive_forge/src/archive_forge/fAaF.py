...
class HybridModel:
    ...
    async def train(model, train_data, optimizer, num_epochs, batch_size):
        for epoch in range(num_epochs):
            ...
            await self.log_metrics(epoch, avg_train_loss, val_loss, val_accuracy)
            await self.log_gradients(model, epoch)
            await self.log_weights(model, epoch)
    ...
    def objective(trial):
        ...
        val_loss, val_accuracy = await self.evaluate(model, val_data)
