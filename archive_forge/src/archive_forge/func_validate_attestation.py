...
def validate_attestation(occurrence, attestor_ref, api_version, action='create'):
  ...
  """
    ...
    if validation_response.result == client.messages.ValidateAttestationOccurrenceResponse.ResultValueValuesEnum.VERIFIED:
        ...
        log.warning('Attestation could not be validated! ({})'.format(validation_response.denialReason))
    else:
        log.error('Attestation could not be validated! (Reason not provided)')
    console_io.PromptContinue(prompt_string='{} Attestation anyway?'.format(action.capitalize()), cancel_on_no=True)