...
class NetworkRBACTests(common.NetworkTests):
    ...
    def setUp(self):
        ...
        cmd_output = self.openstack('network create ' + self.NET_NAME, parse_output=True)
        self.addCleanup(self.openstack, 'network delete ' + cmd_output['id'])
        self.OBJECT_ID = cmd_output['id']
        cmd_output = self.openstack('network rbac create ' + self.OBJECT_ID + ' --action access_as_shared' + ' --target-project admin' + ' --type network', parse_output=True)
        self.addCleanup(self.openstack, 'network rbac delete ' + cmd_output['id'])
    ...
    def test_network_rbac_set(self):
        ...
        self.openstack('network rbac set ' + self.ID + ' --target-project ' + self.PROJECT_NAME)
        cmd_output_rbac = self.openstack('network rbac show ' + self.ID, parse_output=True)
