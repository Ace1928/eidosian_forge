...
class NodesManager:
    __slots__ = ('_moved_exception', 'connection_kwargs', 'default_node', 'nodes_cache', 'read_load_balancer', 'require_full_coverage', 'slots_cache', 'startup_nodes', 'address_remap')

    def __init__(self, startup_nodes: List['ClusterNode'], require_full_coverage: bool, connection_kwargs: Dict[str, Any], address_remap: Optional[Callable[[str, int], Tuple[str, int]]]=None) -> None:
    ...
    async def initialize(self) -> None:
                    ...
                    if i not in tmp_slots:
                        ...
                        if tmp_slot.name != target_node.name:
                            ...
                            if len(disagreements) > 5:
                                raise RedisClusterException(f'startup_nodes could not agree on a valid slots cache: {', '.join(disagreements)}')
            fully_covered = True
        ...
        if not startup_nodes_reachable:
            raise RedisClusterException(f'Redis Cluster cannot be connected. Please provide at least one reachable node: {str(exception)}') from exception
        if not fully_covered and self.require_full_coverage:
            raise RedisClusterException(f'All slots are not covered after query all startup_nodes. {len(tmp_slots)} of {REDIS_CLUSTER_HASH_SLOTS} covered...')
