...
class Bgp_neighbor_address_family(ResourceModule):
    ...
    def __init__(self, module):
        super(Bgp_neighbor_address_family, self).__init__(empty_fact_val={}, facts_module=Facts(module), module=module, resource='bgp_neighbor_address_family', tmplt=Bgp_neighbor_address_familyTemplate())
        self.parsers = ['router', 'aigp', 'allowas_in', 'as_override', 'bestpath_origin_as_allow_invalid', 'capability_orf_prefix', 'default_originate', 'long_lived_graceful_restart_capable', 'long_lived_graceful_restart_stale_time', 'maximum_prefix', 'multipath', 'next_hop_self', 'next_hop_unchanged', 'optimal_route_reflection_group_name', 'origin_as', 'remove_private_AS', 'route_reflector_client', 'send_community_ebgp', 'send_community_gshut_ebgp', 'send_extended_community_ebgp', 'send_multicast_attributes', 'soft_reconfiguration', 'weight', 'site_of_origin', 'validation', 'route_policy.inbound', 'route_policy.outbound']
    ...
    def generate_commands(self):
        ...
        if self.state == 'deleted':
            if wantd:
                to_del = {'neighbors': self._set_to_delete(haved, wantd), 'vrfs': {}}
                for k, hvrf in iteritems(haved.get('vrfs', {})):
                    wvrf = wantd.get('vrfs', {}).get(k, {})
                    to_del['vrfs'][k] = {'neighbors': self._set_to_delete(hvrf, wvrf), 'vrf': k}
                haved.update(to_del)
    ...
    def _compare(self, want, have):
        ...
        if self.commands and 'router bgp' not in self.commands[0]:
            self.commands.insert(0, self._tmplt.render({'as_number': want['as_number']}, 'router', False))
    ...
    def _compare_af(self, want, have):
        ...
        for name, entry in iteritems(wafs):
            ...
            if len(self.commands) != begin or (not af_have and entry):
                self.commands.insert(begin, self._tmplt.render({'afi': entry.get('afi'), 'safi': entry.get('safi')}, 'address_family', False))
