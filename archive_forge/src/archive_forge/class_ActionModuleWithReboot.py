...
class ActionModuleWithReboot(ActionBase):
    ...
    def run(self, tmp: t.Optional[str]=None, task_vars: t.Optional[t.Dict[str, t.Any]]=None) -> t.Dict[str, t.Any]:
        ...
        if self._task.async_val > 0 and reboot:
            return {'failed': True, 'msg': 'async is not supported for this task when reboot=true', 'changed': False}
        invocation = None
        while True:
            module_res = self._execute_module(task_vars=task_vars, wrap_async=wrap_async)
            ...
            if reboot and self._ad_should_reboot(module_res):
                ...
                if self._task.check_mode:
                    ...
                    reboot_res = reboot_host(self._task.action, self._connection, previous_boot_time=previous_boot_time)
                if reboot_res.get('failed', False):
                    module_res = {'changed': module_res.get('changed', False), 'failed': True, 'msg': 'Failed to reboot after module returned reboot_required, see reboot_result and module_result for more details', 'reboot_result': reboot_res, 'module_result': module_res}
                    break
                ...
                if self._ad_should_rerun(module_res) and (not self._task.check_mode):
                    display.vv('Module result has indicated it should rerun after a reboot has occured, rerunning')
                    continue
            break
