import logging  # Utilized for creating and managing log messages (https://docs.python.org/3/library/logging.html)
import re  # Provides regular expression matching operations (https://docs.python.org/3/library/re.html)
import matplotlib.pyplot as plt  # For creating static, interactive, and animated visualizations (https://matplotlib.org/stable/contents.html)
import networkx as nx  # Facilitates the creation, manipulation, and study of complex networks of nodes and edges (https://networkx.org/documentation/stable/)
from sklearn.feature_extraction.text import TfidfVectorizer  # Converts a collection of raw documents to a matrix of TF-IDF features (https://scikit-learn.org/stable/modules/generated/sklearn.feature_extraction.text.TfidfVectorizer.html)
from sklearn.cluster import KMeans  # K-Means clustering algorithm (https://scikit-learn.org/stable/modules/generated/sklearn.cluster.KMeans.html)
from sklearn.decomposition import LatentDirichletAllocation  # Latent Dirichlet Allocation is a topic model that is used for discovering abstract topics from a collection of documents (https://scikit-learn.org/stable/modules/generated/sklearn.decomposition.LatentDirichletAllocation.html)
from sklearn.metrics.pairwise import cosine_similarity  # Computes the Cosine similarity between samples in X and Y (https://scikit-learn.org/stable/modules/generated/sklearn.metrics.pairwise.cosine_similarity.html)
from nltk.corpus import stopwords  # Provides a list of stopwords for natural language processing (https://www.nltk.org/nltk_data/)
from nltk.stem import WordNetLemmatizer  # Lemmatizes words based on the WordNet Database (https://www.nltk.org/api/nltk.stem.html#module-nltk.stem.wordnet)
from elasticsearch import Elasticsearch  # Elasticsearch client for Python (https://elasticsearch-py.readthedocs.io/en/latest/)
from elasticsearch.exceptions import ElasticsearchException  # Exception handling for Elasticsearch operations (https://elasticsearch-py.readthedocs.io/en/latest/exceptions.html)
import json  # For parsing and generating JSON data (https://docs.python.org/3/library/json.html)
from typing import List, Tuple, Dict, Any  # Provides type hinting capabilities (https://docs.python.org/3/library/typing.html)
