...
@click.option('--src-crs', '--src_crs', 'src_crs', default=None, help='Source coordinate reference system.  Limited to EPSG codes for now.  Used as output coordinate system if output does not exist or --like option is not used. Default: EPSG:4326')
@options.all_touched_opt
@click.option('--default-value', '--default_value', 'default_value', type=float, default=1, help='Default value for rasterized pixels')
@click.option('--fill', type=float, default=0, help='Fill value for all pixels not overlapping features.  Will be evaluated as NoData pixels for output.  Default: 0')
@click.option('--property', 'prop', type=str, default=None, help='Property in GeoJSON features to use for rasterized values.  Any features that lack this property will be given --default_value instead.')
...
def rasterize(ctx, files, output, driver, like, bounds, dimensions, res, src_crs, all_touched, default_value, fill, prop, overwrite, nodata, creation_options):
    ...
    bad_param = click.BadParameter('invalid CRS.  Must be an EPSG code.', ctx, param=src_crs, param_hint='--src_crs')
        ...
        if rasterio.shutil.exists(output):
            with rasterio.open(output, 'r+') as out:
                if has_src_crs and src_crs != out.crs:
                    raise click.BadParameter('GeoJSON does not match crs of existing output raster', param='input', param_hint='input')
                if disjoint_bounds(geojson_bounds, out.bounds):
                    click.echo('GeoJSON outside bounds of existing output raster. Are they in different coordinate reference systems?', err=True)
                meta = out.meta
                result = rasterize(geometries, out_shape=(meta['height'], meta['width']), transform=meta.get('affine', meta['transform']), all_touched=all_touched, dtype=meta.get('dtype', None), default_value=default_value, fill=fill)
            ...
            if like is not None:
                ...
                if has_src_crs and src_crs != template_ds.crs:
                    raise click.BadParameter('GeoJSON does not match crs of --like raster', param='input', param_hint='input')
                if disjoint_bounds(geojson_bounds, template_ds.bounds):
                    click.echo('GeoJSON outside bounds of --like raster. Are they in different coordinate reference systems?', err=True)
                ...
                if src_crs.is_geographic:
                    if bounds[0] < -180 or bounds[2] > 180 or bounds[1] < -80 or (bounds[3] > 80):
                        raise click.BadParameter('Bounds are beyond the valid extent for EPSG:4326.', ctx, param=bounds, param_hint='--bounds')
                if dimensions:
                    width, height = dimensions
                    res = ((bounds[2] - bounds[0]) / float(width), (bounds[3] - bounds[1]) / float(height))
                else:
                    if not res:
                        raise click.BadParameter('pixel dimensions are required', ctx, param=res, param_hint='--res')
                    elif len(res) == 1:
                        res = (res[0], res[0])
                    width = max(int(ceil((bounds[2] - bounds[0]) / float(res[0]))), 1)
                    height = max(int(ceil((bounds[3] - bounds[1]) / float(res[1]))), 1)
                kwargs = {'count': 1, 'crs': src_crs, 'width': width, 'height': height, 'transform': Affine(res[0], 0, bounds[0], 0, -res[1], bounds[3]), 'driver': driver}
                if driver:
                    kwargs['driver'] = driver
            kwargs.update(**creation_options)
            if nodata is not None:
                kwargs['nodata'] = nodata
            result = rasterize(geometries, out_shape=(kwargs['height'], kwargs['width']), transform=kwargs['transform'], all_touched=all_touched, dtype=kwargs.get('dtype', None), default_value=default_value, fill=fill)
