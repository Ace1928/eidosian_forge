...
@TransformationFactory.register('core.tighten_constraints_from_vars', doc='[DEPRECATED] Tightens upper and lower bound on linear constraints.')
@deprecated('Use of the constraint tightener transformation is deprecated. Its functionality may be partially replicated using `pyomo.contrib.fbbt.compute_bounds_on_expr(constraint.body)`.', version='5.7')
class TightenConstraintFromVars(IsomorphicTransformation):
    ...
    def _apply_to(self, model):
        ...
        for constr in model.component_data_objects(ctype=Constraint, active=True, descend_into=True):
            ...
            if UB < LB:
                logger.error('Infeasible variable bounds: Constraint %s has inferred LB %s > UB %s' % (constr.name, new_lb, new_ub))