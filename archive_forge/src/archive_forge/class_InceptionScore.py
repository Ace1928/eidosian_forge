...
class InceptionScore(Metric):
    ...
    def __init__(self, feature: Union[str, int, Module]='logits_unbiased', splits: int=10, normalize: bool=False, **kwargs: Any) -> None:
        super().__init__(**kwargs)
        rank_zero_warn('Metric `InceptionScore` will save all extracted features in buffer. For large datasets this may lead to large memory footprint.', UserWarning)
        if isinstance(feature, (str, int)):
            if not _TORCH_FIDELITY_AVAILABLE:
                raise ModuleNotFoundError('InceptionScore metric requires that `Torch-fidelity` is installed. Either install as `pip install torchmetrics[image]` or `pip install torch-fidelity`.')
