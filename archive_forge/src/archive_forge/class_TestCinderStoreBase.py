...
class TestCinderStoreBase(object):
    ...
    def _test_get_cinderclient_with_api_insecure(self, group='glance_store'):
        self.config()
        with mock.patch.object(cinder.ksa_session, 'Session') as fake_session, mock.patch.object(cinder.ksa_identity, 'V3Password') as fake_auth_method:
            ...
            self._get_cinderclient_with_user_overriden(group=group, **{'cinder_api_insecure': True})
    ...
    def test__check_context(self):
        with mock.patch.object(cinder.Store, 'is_user_overriden', return_value=True) as fake_overriden:
            self.store._check_context(self.context)
            fake_overriden.assert_called_once()

    def test__check_context_no_context(self):
        with mock.patch.object(cinder.Store, 'is_user_overriden', return_value=False):
    ...
    def test__check_context_no_service_catalog(self):
        with mock.patch.object(cinder.Store, 'is_user_overriden', return_value=False):
            ...
            self.assertRaises(exceptions.BadStoreConfiguration, self.store._check_context, fake_context)
    ...
    def test_wait_volume_status_unexpected(self, mock_sleep):
        fake_manager = mock.MagicMock(get=mock.Mock())
        volume_available = mock.MagicMock(manager=fake_manager, id='fake-id', status='error')
    ...
    def test_open_cinder_volume_update_attachment_error(self):
        ...
        self._test_open_cinder_volume('rb', 'ro', None, update_attachment_error=err)
    ...
    def _test_configure_add_valid_type(self):
        with mock.patch.object(self.store, 'get_cinderclient') as mocked_cc:
            mocked_cc.return_value = mock.MagicMock(volume_types=mock.MagicMock(find=self._fake_volume_type_check))
    ...
    def _test_configure_add_invalid_type(self):
        with mock.patch.object(self.store, 'get_cinderclient') as mocked_cc:
            ...
            with mock.patch.object(cinder, 'LOG') as mock_log:
                self.store.configure_add()
                mock_log.warning.assert_called_with('Invalid `cinder_volume_type some_random_type`')
    ...
    def test_cinder_get_client_exception(self):
        ...
        with mock.patch.object(cinder.Store, 'get_cinderclient') as mock_cc:
            mock_cc.side_effect = cinder.cinder_exception.ClientException(code=500)
            self.assertRaises(exceptions.BackendException, self.store.get, loc, context=self.context)
    ...
    def test_cinder_get_size_generic_exception(self):
        fake_volume_uuid = str(uuid.uuid4())
        loc = mock.MagicMock(volume_id=fake_volume_uuid)
        fake_volumes = mock.MagicMock(get=mock.MagicMock(side_effect=Exception()))
        with mock.patch.object(cinder.Store, 'get_cinderclient') as mocked_cc:
            mocked_cc.return_value = mock.MagicMock(volumes=fake_volumes)
            image_size = self.store.get_size(loc, context=self.context)
    ...
    def test_cinder_add_extend_volume_delete_exception(self):
        ...
        expected_size = 0
        image_file = mock.MagicMock(read=mock.MagicMock(side_effect=[fakebuffer, fakebuffer, None]))
        fake_volume = mock.MagicMock(id=expected_volume_id, status='available', size=1, delete=mock.MagicMock(side_effect=Exception()))
        fake_client = mock.MagicMock()
        fake_volume.manager.get.return_value = fake_volume
        fake_volumes = mock.MagicMock(create=mock.Mock(return_value=fake_volume))
        verifier = None
    ...
    def test__wait_resize_device_resized(self, mock_sleep):
        ...
        with mock.patch.object(scaleio.ScaleIOBrickConnector, '_get_device_size') as mock_get_dev_size:
            mock_get_dev_size.side_effect = [1, 2]
            scaleio.ScaleIOBrickConnector._wait_resize_device(fake_vol, fake_file)
    ...
    def _test_parse_uri_invalid(self, uri):
        self.assertRaises(exceptions.BadStoreUri, self.location.parse_uri, uri)
