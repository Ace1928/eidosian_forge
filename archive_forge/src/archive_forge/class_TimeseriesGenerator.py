...
class TimeseriesGenerator(data_utils.Sequence):
    ...
    def __init__(self, data, targets, length, sampling_rate=1, stride=1, start_index=0, end_index=None, shuffle=False, reverse=False, batch_size=128):
        if len(data) != len(targets):
            raise ValueError('Data and targets have to be' + f' of same length. Data length is {len(data)}' + f' while target length is {len(targets)}')
        self.data = data
    ...
    def get_config(self):
        ...
        return {'data': json_data, 'targets': json_targets, 'length': self.length, 'sampling_rate': self.sampling_rate, 'stride': self.stride, 'start_index': self.start_index, 'end_index': self.end_index, 'shuffle': self.shuffle, 'reverse': self.reverse, 'batch_size': self.batch_size}
