...
def add_to_collaborators(self, collaborator: str | NamedUser, permission: Opt[str]=NotSet) -> Invitation | None:
    ...
    if is_defined(permission):
        ...
        put_parameters = None
    headers, data = self._requester.requestJsonAndCheck('PUT', f'{self.url}/collaborators/{collaborator}', input=put_parameters)
    return github.Invitation.Invitation(self._requester, headers, data, completed=True) if data is not None else None