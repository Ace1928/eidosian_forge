...
def _validate_allowed_address_pairs(address_pairs, valid_values=None):
    ...
    if len(address_pairs) > cfg.CONF.max_allowed_address_pair:
        raise exceptions.AllowedAddressPairExhausted(quota=cfg.CONF.max_allowed_address_pair)
    for address_pair in address_pairs:
        ...
        if (mac, ip_address) not in unique_check:
            ...
            raise exceptions.DuplicateAddressPairInRequest(mac_address=mac, ip_address=ip_address)
        invalid_attrs = set(address_pair.keys()) - set(['mac_address', 'ip_address'])
        if invalid_attrs:
            msg = _("Unrecognized attribute(s) '%s'") % ', '.join(set(address_pair.keys()) - set(['mac_address', 'ip_address']))
            raise exc.HTTPBadRequest(msg)
        if '/' in ip_address:
