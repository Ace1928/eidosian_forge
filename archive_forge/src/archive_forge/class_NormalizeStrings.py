...
class NormalizeStrings(Base):
    ...
    def export() -> None:
        ...
        def make_graph(node: onnx.helper.NodeProto, input_shape: Sequence[int], output_shape: Sequence[int]) -> onnx.helper.GraphProto:
            graph = onnx.helper.make_graph(nodes=[node], name='StringNormalizer', inputs=[onnx.helper.make_tensor_value_info('x', onnx.TensorProto.STRING, input_shape)], outputs=[onnx.helper.make_tensor_value_info('y', onnx.TensorProto.STRING, output_shape)])
            return graph
        stopwords = ['monday']
        node = onnx.helper.make_node('StringNormalizer', inputs=['x'], outputs=['y'], is_case_sensitive=1, stopwords=stopwords)
        ...
        stopwords = ['monday']
        node = onnx.helper.make_node('StringNormalizer', inputs=['x'], outputs=['y'], case_change_action='UPPER', is_case_sensitive=0, stopwords=stopwords)
