...
class SparkDatasetSource(DatasetSource):
    ...
    def __init__(self, path: Optional[str]=None, table_name: Optional[str]=None, sql: Optional[str]=None):
        if (path, table_name, sql).count(None) != 2:
            raise MlflowException('Must specify exactly one of "path", "table_name", or "sql"', INVALID_PARAMETER_VALUE)
        self._path = path
    ...
    def from_dict(cls, source_dict: Dict[Any, Any]) -> 'SparkDatasetSource':
        return cls(path=source_dict.get('path'), table_name=source_dict.get('table_name'), sql=source_dict.get('sql'))