...
class BaremetalDeployTemplateTests(base.TestCase):
    ...
    def setUp(self):
        ...
        self.steps = json.dumps([{'interface': 'bios', 'step': 'apply_configuration', 'args': {}, 'priority': 10}])
        name = self._get_random_trait()
        self.template = self.deploy_template_create(name, params="--steps '%s'" % self.steps)
    ...
    def test_list(self):
        ...
        self.assertIn(self.template['name'], [template['Name'] for template in template_list])
        self.assertIn(self.template['uuid'], [template['UUID'] for template in template_list])
    ...
    def test_delete(self):
        ...
        self.assertIn('Deleted deploy template {0}'.format(self.template['uuid']), output)
        template_list = self.deploy_template_list()
        self.assertNotIn(self.template['name'], [template['Name'] for template in template_list])
        self.assertNotIn(self.template['uuid'], [template['UUID'] for template in template_list])
        self.template = None
