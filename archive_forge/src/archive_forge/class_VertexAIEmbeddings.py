...
@deprecated(since='0.0.12', removal='0.2.0', alternative_import='langchain_google_vertexai.VertexAIEmbeddings')
class VertexAIEmbeddings(_VertexAICommon, Embeddings):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        if values['model_name'] == 'textembedding-gecko-default':
            logger.warning('Model_name will become a required arg for VertexAIEmbeddings starting from Feb-01-2024. Currently the default is set to textembedding-gecko@001')
    ...
    def __init__(self, model_name: str='textembedding-gecko-default', project: Optional[str]=None, location: str='us-central1', request_parallelism: int=5, max_retries: int=6, credentials: Optional[Any]=None, **kwargs: Any):
        """Initialize the sentence_transformer."""
        super().__init__(project=project, location=location, credentials=credentials, request_parallelism=request_parallelism, max_retries=max_retries, model_name=model_name, **kwargs)
    ...
    def embed(self, texts: List[str], batch_size: int=0, embeddings_task_type: Optional[Literal['RETRIEVAL_QUERY', 'RETRIEVAL_DOCUMENT', 'SEMANTIC_SIMILARITY', 'CLASSIFICATION', 'CLUSTERING']]=None) -> List[List[float]]:
        ...
        if self.show_progress_bar:
            ...
            iter_ = batches
        for batch in iter_:
            tasks.append(self.instance['task_executor'].submit(self._get_embeddings_with_retry, texts=batch, embeddings_type=embeddings_task_type))
        if len(tasks) > 0:
            wait(tasks)
        for t in tasks:
