...
class KeystoneClientPlugin(client_plugin.ClientPlugin):
    ...
    def is_not_found(self, ex):
        return isinstance(ex, (ks_exceptions.NotFound, exception.EntityNotFound))
    ...
    def get_project_id(self, project, domain=None):
        if project is None:
            return None
        if not domain:
            project, domain = self.parse_entity_with_domain(project, 'KeystoneProject')
        try:
            project_obj = self.client().client.projects.get(project)
            return project_obj.id
        except ks_exceptions.NotFound:
            project_list = self.client().client.projects.list(name=project, domain=domain)
    ...
    def get_user_id(self, user, domain=None):
        ...
        try:
            ...
            try:
                user_obj = self.client().client.users.find(name=user, domain_id=domain)
                return user_obj.id
            except ks_exceptions.NotFound:
                pass
