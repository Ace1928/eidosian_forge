...
class DynamicModelInterface(object):
    ...
    def __init__(self, model, time, context=NOTSET):
        ...
        if context is NOTSET:
            context = model
        cuid_buffer = {}
        self._scalar_var_cuids = [ComponentUID(var, cuid_buffer=cuid_buffer, context=context) for var in self._scalar_vars]
        self._dae_var_cuids = [ComponentUID(var.referent, cuid_buffer=cuid_buffer, context=context) for var in self._dae_vars]
        self._dae_expr_cuids = [ComponentUID(expr.referent, cuid_buffer=cuid_buffer, context=context) for expr in self._dae_expr]
    ...
    def get_scalar_variable_data(self):
        ...
        """
        return {cuid: var.value for cuid, var in zip(self._scalar_var_cuids, self._scalar_vars)}
    ...
    def get_data_at_time(self, time=None, include_expr=False):
        ...
        if _is_iterable(time):
            time_list = list(time)
            data = {cuid: [var[t].value for t in time] for cuid, var in zip(self._dae_var_cuids, self._dae_vars)}
            if include_expr:
                data.update({cuid: [pyo_value(expr[t]) for t in time] for cuid, expr in zip(self._dae_expr_cuids, self._dae_expr)})
            return TimeSeriesData(data, time_list, time_set=self.time)
        else:
            data = {cuid: var[time].value for cuid, var in zip(self._dae_var_cuids, self._dae_vars)}
            if include_expr:
                data.update({cuid: pyo_value(expr[time]) for cuid, expr in zip(self._dae_expr_cuids, self._dae_expr)})
            return ScalarData(data)

    def load_data(self, data, time_points=None, tolerance=0.0, prefer_left=None, exclude_left_endpoint=None, exclude_right_endpoint=None):
        ...
        def _error_if_used(prefer_left, excl_left, excl_right, dtype):
            if any((a is not None for a in (prefer_left, excl_left, excl_right))):
                raise RuntimeError('prefer_left, exclude_left_endpoint, and exclude_right_endpoint can only be set if data is IntervalData-compatible. Got prefer_left=%s, exclude_left_endpoint=%s, and exclude_right_endpoint=%s while loading data of type %s' % (prefer_left, excl_left, excl_right, dtype))
        ...
        if isinstance(data, ScalarData):
            ...
            load_data_from_interval(data, self.model, time_points, tolerance=tolerance, prefer_left=prefer_left, exclude_left_endpoint=excl_left, exclude_right_endpoint=excl_right)
    ...
    def get_penalty_from_target(self, target_data, time=None, variables=None, weight_data=None, variable_set=None, tolerance=None, prefer_left=None):
        ...
        if variables is None:
            variables = [self.model.find_component(key) for key in target_data.get_data().keys()]
        else:
            variables = [self.model.find_component(get_indexed_cuid(var, (self.time,))) for var in variables]
        return get_penalty_from_target(variables, time, target_data, weight_data=weight_data, variable_set=variable_set, tolerance=tolerance, prefer_left=prefer_left)

    def get_piecewise_constant_constraints(self, variables, sample_points, use_next=True, tolerance=0.0):
        ...
        time_list = list(self.time)
        sample_point_indices = [find_nearest_index(time_list, t, tolerance=tolerance) for t in sample_points]
        sample_points = [time_list[i] for i in sample_point_indices]
        return get_piecewise_constant_constraints(variables, self.time, sample_points, use_next=use_next)