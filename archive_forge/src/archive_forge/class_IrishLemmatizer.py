...
class IrishLemmatizer(Lemmatizer):
    ...
    def get_lookups_config(cls, mode: str) -> Tuple[List[str], List[str]]:
        if mode == 'pos_lookup':
            required = ['lemma_lookup_adj', 'lemma_lookup_adp', 'lemma_lookup_noun', 'lemma_lookup_verb']
            return (required, [])
        else:
    ...
    def pos_lookup_lemmatize(self, token: Token) -> List[str]:
        ...
        if univ_pos == 'PROPN':
            lookup_pos = 'noun'
        if token.has_morph():
            if univ_pos == 'NOUN' and ('VerbForm=Vnoun' in token.morph or 'VerbForm=Inf' in token.morph):
