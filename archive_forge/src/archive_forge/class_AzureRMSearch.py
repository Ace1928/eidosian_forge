...
class AzureRMSearch(AzureRMModuleBase):
    ...
    def __init__(self):
        ...
        super(AzureRMSearch, self).__init__(derived_arg_spec=self.module_arg_spec, supports_check_mode=False, supports_tags=True)
    ...
    def check_values(self, hosting_mode, sku, partition_count, replica_count):
        if hosting_mode == 'highDensity' and sku != 'standard3':
            self.fail("Hosting mode could not be 'highDensity' if sku is not 'standard3'.")
        if sku == 'standard3' and hosting_mode == 'highDensity' and (partition_count not in [1, 2, 3]):
        ...
        elif replica_count < 1 or replica_count > 12:
