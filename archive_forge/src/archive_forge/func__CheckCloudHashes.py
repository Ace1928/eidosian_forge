...
def _CheckCloudHashes(logger, src_url, dst_url, src_obj_metadata, dst_obj_metadata):
  ...
  """
    ...
    for alg, upload_b64_digest in six.iteritems(upload_hashes):
        ...
        logger.debug('Comparing source vs destination %s-checksum for %s. (%s/%s)', alg, dst_url, download_b64_digest, upload_b64_digest)
        if download_b64_digest != upload_b64_digest:
            raise HashMismatchException("%s signature for source object (%s) doesn't match destination object digest (%s). Object (%s) will be deleted." % (alg, download_b64_digest, upload_b64_digest, dst_url))
        checked_one = True
    if not checked_one:
        logger.warn('WARNING: Found no hashes to validate object downloaded from %s and uploaded to %s. Integrity cannot be assured without hashes.', src_url, dst_url)