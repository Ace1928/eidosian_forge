...
class ConfigureDNS(base.UpdateCommand):
  ...
  """
    ...
    @staticmethod
    def Args(parser):
        resource_args.AddRegistrationResourceArg(parser, 'to configure DNS settings for')
    ...
    def Run(self, args):
        ...
        if args.disable_dnssec and args.dns_settings_from_file:
            raise exceptions.Error('argument --disable-dnssec: At most one of --dns-settings-from-file | --disable-dnssec can be specified.')
        registration = client.Get(registration_ref)
        util.AssertRegistrationOperational(api_version, registration)
        dns_settings, update_mask = dns_util.ParseDNSSettings(api_version, args.name_servers, args.cloud_dns_zone, args.use_google_domains_dns, args.dns_settings_from_file, registration_ref.registrationsId, enable_dnssec=not args.disable_dnssec, dns_settings=registration.dnsSettings)
        if dns_settings is None:
            dns_settings, update_mask = dns_util.PromptForNameServers(api_version, registration_ref.registrationsId, enable_dnssec=not args.disable_dnssec, dns_settings=registration.dnsSettings)
            if dns_settings is None:
                return None
        if registration.dnsSettings.glueRecords and (not update_mask.glue_records):
            log.status.Print('Glue records will not be cleared. If you want to clear them, use --dns-settings-from-file flag.')
        ds_records_present = dns_util.DnssecEnabled(registration.dnsSettings)
        name_servers_changed = update_mask.name_servers and (not dns_util.NameServersEquivalent(registration.dnsSettings, dns_settings))
        if ds_records_present and name_servers_changed:
            log.warning('Name servers should not be changed if DS records are present. Disable DNSSEC first and wait 24 hours before you change name servers. Otherwise your domain may stop serving.')
            if not args.unsafe_dns_update:
                dns_util.PromptForUnsafeDnsUpdate()
        response = client.ConfigureDNS(registration_ref, dns_settings, update_mask, validate_only=args.validate_only)
