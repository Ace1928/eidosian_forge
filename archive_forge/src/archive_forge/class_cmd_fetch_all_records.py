...
class cmd_fetch_all_records(Command):
    ...
    takes_options = ['directory', Option('dry-run', help="Show what would be done, but don't actually do anything.")]
    ...
    def run(self, source_repo, directory='.', dry_run=False):
        ...
        resume_tokens, missing_keys = target._get_sink().insert_stream(source_stream(), source._format, [])
