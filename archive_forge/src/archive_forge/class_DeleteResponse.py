...
class DeleteResponse(ProtocolBuffer.ProtocolMessage):
    ...
    def MergeFrom(self, x):
        ...
        for i in range(x.version_size()):
            self.add_version(x.version(i))
    ...
    def Equals(self, x):
        ...
        if self.has_cost_ and self.cost_ != x.cost_:
            return 0
        if len(self.version_) != len(x.version_):
            return 0
    ...
    def ByteSize(self):
        ...
        for i in range(len(self.version_)):
            n += self.lengthVarInt64(self.version_[i])
        return n
    ...
    kcost = 1
    kversion = 3
    _TEXT = _BuildTagLookupTable({0: 'ErrorCode', 1: 'cost', 3: 'version'}, 3)
    _TYPES = _BuildTagLookupTable({0: ProtocolBuffer.Encoder.NUMERIC, 1: ProtocolBuffer.Encoder.STRING, 3: ProtocolBuffer.Encoder.NUMERIC}, 3, ProtocolBuffer.Encoder.MAX_TYPE)
    _STYLE = ''
    _STYLE_CONTENT_TYPE = ''
