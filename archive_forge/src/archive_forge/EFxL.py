...
class ImageCompressorApp(tk.Tk):
    def __init__(self):
        ...
        self.img_hash = None
        self.cipher_suite = Fernet(EncryptionManager.get_valid_encryption_key())
        self.current_page = 0
    ...
    def initialize_background_tasks(self) -> None:
        ...
        """
        Thread(target=asyncio.run, args=(self.async_init_db(),), daemon=True).start()
    ...
    def async_compress_image(self):
        try:
            image_format = await Image.open(io.BytesIO(self.original_image_data)).format
            compressed_data = await compress(self.original_image_data, image_format)
            encrypted_data = await encrypt(compressed_data, self.cipher_suite)
            asyncio.run(
                insert_compressed_image(self.img_hash, image_format, encrypted_data)
            )
            self.status_label.config(
                text="Image compressed, encrypted and stored successfully."
            )
        except Exception as e:
            logging.error(f"Failed to compress, encrypt and store image: {e}")
            self.status_label.config(
                text="Failed to compress, encrypt and store image. Please try again."
            )
        finally:
