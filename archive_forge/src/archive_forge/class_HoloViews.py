...
class HoloViews(PaneBase):
    ...
    widget_location = param.ObjectSelector(default='right_top', objects=['left', 'bottom', 'right', 'top', 'top_left', 'top_right', 'bottom_left', 'bottom_right', 'left_top', 'left_bottom', 'right_top', 'right_bottom'], doc='\n        The layout of the plot and the widgets. The value refers to the\n        position of the widgets relative to the plot.')
    widget_layout = param.ObjectSelector(objects=[WidgetBox, Row, Column], constant=True, default=WidgetBox, doc='\n        The layout object to display the widgets in.')
    widget_type = param.ObjectSelector(default='individual', objects=['individual', 'scrubber'], doc=')\n        Whether to generate individual widgets for each dimension or\n        on global scrubber.')
    ...
    def __init__(self, object=None, **params):
        ...
        self._overrides = [p for p, v in params.items() if p in Layoutable.param and v != self.param[p].default]
    ...
    def _get_model(self, doc: Document, root: Optional[Model]=None, parent: Optional[Model]=None, comm: Optional[Comm]=None) -> Model:
        ...
        kwargs = {p: v for p, v in self.param.values().items() if p in Layoutable.param and p != 'name'}
    ...
    def widgets_from_dimensions(cls, object, widget_types=None, widgets_type='individual', direction='vertical'):
        ...
        from holoviews.core.util import datetime_types, isnumeric, unique_iterator
        from holoviews.plotting.plot import GenericCompositePlot, Plot
        from holoviews.plotting.util import get_dynamic_mode
        from ..widgets import DatetimeInput, DiscreteSlider, FloatSlider, IntSlider, Select, Widget
        ...
        if dims == [Dimension('Frame')] and keys == [(0,)] or (not dynamic and len(keys) == 1):
            return ([], {})
        nframes = 1
        values = {} if dynamic else dict(zip(dims, zip(*keys)))
        dim_values = {}
        widgets = []
        dims = [d for d in dims if values.get(d) is not None or d.values or d.range != (None, None)]
        for i, dim in enumerate(dims):
            ...
            if widgets_type == 'scrubber':
                ...
                if isinstance(widget, Widget):
                    ...
                    raise ValueError('Explicit widget definitions expected to be a widget instance or type, %s dimension widget declared as %s.' % (dim, widget))
            ...
            if vals:
                ...
                widget_kwargs = dict(dict(step=step, name=dim.label, start=dim.range[0], end=dim.range[1], value=default), **widget_kwargs)
                widget = widget_type(**widget_kwargs)
