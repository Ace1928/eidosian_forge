...
class NullQubit(QubitDevice):
    ...
    version = __version__
    author = 'Xanadu Inc.'
    operations = {'Identity', 'Snapshot', 'BasisState', 'StatePrep', 'QubitStateVector', 'QubitUnitary', 'ControlledQubitUnitary', 'BlockEncode', 'MultiControlledX', 'DiagonalQubitUnitary', 'SpecialUnitary', 'PauliX', 'PauliY', 'PauliZ', 'MultiRZ', 'Hadamard', 'S', 'Adjoint(S)', 'T', 'Adjoint(T)', 'SX', 'Adjoint(SX)', 'CNOT', 'SWAP', 'ISWAP', 'PSWAP', 'Adjoint(ISWAP)', 'SISWAP', 'Adjoint(SISWAP)', 'SQISW', 'CSWAP', 'Toffoli', 'CY', 'CZ', 'CCZ', 'CH', 'PhaseShift', 'PCPhase', 'ControlledPhaseShift', 'CPhase', 'RX', 'RY', 'RZ', 'Rot', 'CRX', 'CRY', 'CRZ', 'CRot', 'IsingXX', 'IsingYY', 'IsingZZ', 'IsingXY', 'SingleExcitation', 'SingleExcitationPlus', 'SingleExcitationMinus', 'DoubleExcitation', 'DoubleExcitationPlus', 'DoubleExcitationMinus', 'QubitCarry', 'QubitSum', 'OrbitalRotation', 'QFT', 'ECR'}
    observables = {'PauliX', 'PauliY', 'PauliZ', 'Hadamard', 'Hermitian', 'Identity', 'Projector', 'SparseHamiltonian', 'Hamiltonian', 'Sum', 'SProd', 'Prod', 'Exp'}

    def __init__(self, wires, *args, **kwargs):
        defaultKwargs = {'shots': None, 'analytic': None, 'r_dtype': np.float64, 'c_dtype': np.complex128}
        kwargs = {**defaultKwargs, **kwargs}
        self._operation_calls = defaultdict(int)
        super().__init__(wires, shots=kwargs['shots'], r_dtype=kwargs['r_dtype'], c_dtype=kwargs['c_dtype'], analytic=kwargs['analytic'])
        self._debugger = None
    ...
    def capabilities(cls):
        ...
        capabilities.update(model='qubit', supports_inverse_operations=True, supports_analytic_computation=True, supports_broadcasting=False, returns_state=True, passthru_devices={'tf': 'null.qubit', 'torch': 'null.qubit', 'autograd': 'null.qubit', 'jax': 'null.qubit'})
        return capabilities
