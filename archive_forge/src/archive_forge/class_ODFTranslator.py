...
class ODFTranslator(nodes.GenericNodeVisitor):
    ...
    def __init__(self, document):
        ...
        if WhichElementTree == 'lxml':
            root = Element('office:document-content', nsmap=CONTENT_NAMESPACE_DICT)
        else:
    ...
    def add_doc_title(self):
        ...
        if text:
            ...
            if not self.found_doc_title:
                el = Element('text:p', attrib={'text:style-name': self.rststyle('title')})
                el.text = text
        ...
        if self.in_table_of_contents:
            if self.settings.generate_oowriter_toc:
                ...
                if 'classes' in node and 'auto-toc' in node.attributes['classes']:
                    el = SubElement(self.current_element, 'text:list', attrib={'text:style-name': self.rststyle('tocenumlist')})
                    self.list_style_stack.append(self.rststyle('enumitem'))
                else:
                    el = SubElement(self.current_element, 'text:list', attrib={'text:style-name': self.rststyle('tocbulletlist')})
                    self.list_style_stack.append(self.rststyle('bulletitem'))
                self.set_current_element(el)
        else:
            if self.blockstyle == self.rststyle('blockquote'):
                el = SubElement(self.current_element, 'text:list', attrib={'text:style-name': self.rststyle('blockquote-bulletlist')})
                self.list_style_stack.append(self.rststyle('blockquote-bulletitem'))
            elif self.blockstyle == self.rststyle('highlights'):
                el = SubElement(self.current_element, 'text:list', attrib={'text:style-name': self.rststyle('highlights-bulletlist')})
                self.list_style_stack.append(self.rststyle('highlights-bulletitem'))
            elif self.blockstyle == self.rststyle('epigraph'):
                el = SubElement(self.current_element, 'text:list', attrib={'text:style-name': self.rststyle('epigraph-bulletlist')})
                self.list_style_stack.append(self.rststyle('epigraph-bulletitem'))
            else:
                el = SubElement(self.current_element, 'text:list', attrib={'text:style-name': self.rststyle('bulletlist')})
                self.list_style_stack.append(self.rststyle('bulletitem'))
            self.set_current_element(el)
    ...
    def visit_enumerated_list(self, node):
        ...
        if self.blockstyle == self.rststyle('blockquote'):
            el2 = SubElement(el1, 'text:list', attrib={'text:style-name': self.rststyle('blockquote-enumlist')})
            self.list_style_stack.append(self.rststyle('blockquote-enumitem'))
        elif self.blockstyle == self.rststyle('highlights'):
            el2 = SubElement(el1, 'text:list', attrib={'text:style-name': self.rststyle('highlights-enumlist')})
            self.list_style_stack.append(self.rststyle('highlights-enumitem'))
        elif self.blockstyle == self.rststyle('epigraph'):
            el2 = SubElement(el1, 'text:list', attrib={'text:style-name': self.rststyle('epigraph-enumlist')})
            self.list_style_stack.append(self.rststyle('epigraph-enumitem'))
        else:
            liststylename = 'enumlist-%s' % (node.get('enumtype', 'arabic'),)
            el2 = SubElement(el1, 'text:list', attrib={'text:style-name': self.rststyle(liststylename)})
            self.list_style_stack.append(self.rststyle('enumitem'))
    ...
    def visit_description(self, node):
        el = self.append_child('table:table-cell', attrib={'table:style-name': 'Table%d.B2' % self.table_count, 'office:value-type': 'string'})
    ...
    def visit_reference(self, node):
        if self.settings.create_links:
            if 'refuri' in node:
                ...
                el = self.append_child('text:a', attrib={'xlink:href': '%s' % href, 'xlink:type': 'simple'})
                self.set_current_element(el)
            elif 'refid' in node:
                if self.settings.create_links:
                    href = node['refid']
                    el = self.append_child('text:reference-ref', attrib={'text:ref-name': '%s' % href, 'text:reference-format': 'text'})
            else:
                self.document.reporter.warning('References must have "refuri" or "refid" attribute.')
        if self.in_table_of_contents and len(node.children) >= 1 and isinstance(node.children[0], docutils.nodes.generated):
    ...
    def visit_rubric(self, node):
        ...
        if classes:
            ...
            if class1:
                style_name = class1
        el = SubElement(self.current_element, 'text:h', attrib={'text:style-name': style_name})
        text = node.astext()
    ...
    def visit_subscript(self, node):
        el = self.append_child('text:span', attrib={'text:style-name': 'rststyle-subscript'})
        self.set_current_element(el)
