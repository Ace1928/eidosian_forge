...
def make_asgi_app(registry: CollectorRegistry=REGISTRY, disable_compression: bool=False) -> Callable:
    ...
    async def prometheus_app(scope, receive, send):
        ...
        accept_header = ','.join([value.decode('utf8') for name, value in scope.get('headers') if name.decode('utf8').lower() == 'accept'])
        accept_encoding_header = ','.join([value.decode('utf8') for name, value in scope.get('headers') if name.decode('utf8').lower() == 'accept-encoding'])
        ...
        if payload.get('type') == 'http.request':
            await send({'type': 'http.response.start', 'status': int(status.split(' ')[0]), 'headers': formatted_headers})
