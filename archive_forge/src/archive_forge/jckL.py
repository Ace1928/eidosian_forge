...
class Snake:
    ...
    def update(self, apple: apple.Apple) -> None:
        ...
        for i in range(len(self.path)):
            ...
            if head.x == self.path[i].position.x and head.y == self.path[i].position.y:
                next_head: Vector2 = self.path[i + 1]
                if next_head.position.x - head.x == 1:
                    self.move_right()
                elif next_head.position.x - head.x == -1:
                    self.move_left()
                elif next_head.position.y - head.y == 1:
                    self.move_down()
                elif next_head.position.y - head.y == -1:
                    self.move_up()
                else:
