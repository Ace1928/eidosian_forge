...
def ragged_range(starts: _atypes.TensorFuzzingAnnotation[TV_RaggedRange_T], limits: _atypes.TensorFuzzingAnnotation[TV_RaggedRange_T], deltas: _atypes.TensorFuzzingAnnotation[TV_RaggedRange_T], Tsplits: TV_RaggedRange_Tsplits=_dtypes.int64, name=None):
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'RaggedRange', name, starts, limits, deltas, 'Tsplits', Tsplits)
            ...
            pass
        try:
            return ragged_range_eager_fallback(starts, limits, deltas, Tsplits=Tsplits, name=name, ctx=_ctx)
        except _core._SymbolicException:
            pass
    if Tsplits is None:
        Tsplits = _dtypes.int64
    Tsplits = _execute.make_type(Tsplits, 'Tsplits')
    _, _, _op, _outputs = _op_def_library._apply_op_helper('RaggedRange', starts=starts, limits=limits, deltas=deltas, Tsplits=Tsplits, name=name)
    _result = _outputs[:]
    if _execute.must_record_gradient():
        _attrs = ('T', _op._get_attr_type('T'), 'Tsplits', _op._get_attr_type('Tsplits'))
