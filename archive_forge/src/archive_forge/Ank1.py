...
class AdvancedASCIIFormatter:
    ...
    def __call__(self, log: LoggerConfig.LogEntryDictType) -> LoggerConfig.LogEntryDictType:
        ...
        dict["header"] = self.ENTRY_SEPARATOR

        # Format the timestamp component of the log entry
        dict["timestamp"] = f"{self.TOP_LEFT_CORNER} {log['timestamp']} {self.TOP_RIGHT_CORNER}"
        # Format the level component of the log entry
        dict["level"] = f"{self.LEVEL_SYMBOLS[log['level']]} {log['level'].name.upper()}"
        # Format the message component of the log entry
        dict["message"] = f"{self.VERTICAL_LINE} {log['message']}"
        # Format the module component of the log entry
        dict["module"] = f"{self.HORIZONAL_DIVIDER_LEFT} Module: {log['module']} {self.HORIZONAL_DIVIDER_RIGHT}"
        # Format the function component of the log entry
        dict["function"] = f"{self.HORIZONTAL_DIVIDER_MIDDLE} Function: {log['function']} {self.HORIZONTAL_DIVIDER_MIDDLE}"
        # Format the line component of the log entry
        dict["line"] = f"{self.VERTICAL_LINE} Line: {log['line']}"
        # Format the exception component of the log entry
        dict["exception"] = f"{self.ERROR_INDICATOR_BOX_TOP_LEFT} Exception: {log['exc_info']} {self.ERROR_INDICATOR_BOX_TOP_RIGHT}"
        # Add In the Authorship Details and a Footer Section to act as a clear seperator and identifier for the log entry end
        dict["authorship"] = f"{self.BOTTOM_LEFT_CORNER} Author: Lloyd Russell {self.BOTTOM_RIGHT_CORNER}"
