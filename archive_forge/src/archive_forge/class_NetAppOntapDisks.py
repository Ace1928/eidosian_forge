...
class NetAppOntapDisks:
    ...
    def __init__(self):
        ...
        self.argument_spec.update(dict(node=dict(required=True, type='str'), disk_count=dict(required=False, type='int'), disk_type=dict(required=False, type='str', choices=['ATA', 'BSAS', 'FCAL', 'FSAS', 'LUN', 'MSATA', 'SAS', 'SSD', 'SSD_NVM', 'VMDISK', 'unknown']), min_spares=dict(required=False, type='int')))
    ...
    def apply(self):
        ...
        needed_disks = None
        unassign = {'spare_disks': None, 'unassign_disks': None}
        if 'disk_count' in self.parameters:
            if self.parameters['disk_count'] < owned_disks_count:
                ...
                unassign = {'spare_disks': owned_spare_disks, 'unassign_disks': unassign_disks_count}
                ...
                if required_disks_count > len(partner_spare_disks) - self.parameters['min_spares']:
                    ...
                    unassign = {'spare_disks': partner_spare_disks, 'unassign_disks': required_disks_count}
            if self.parameters['disk_count'] > owned_disks_count:
                needed_disks = self.parameters['disk_count'] - owned_disks_count
        elif len(unassigned_disks) >= 1:
            needed_disks = 0
