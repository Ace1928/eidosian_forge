...
class CutAndSplicePairing(OffspringCreator):
    ...
    * `Lonie, Zurek, Comp. Phys. Comm. 182 (2011) 372-387`__
    ...
    def __init__(self, slab, n_top, blmin, number_of_variable_cell_vectors=0, p1=1, p2=0.05, minfrac=None, cellbounds=None, test_dist_to_slab=True, use_tags=False, rng=np.random, verbose=False):
    ...
    def cross(self, a1, a2):
        ...
        while invalid and counter < maxcount:
            ...
            if self.number_of_variable_cell_vectors == 0:
                ...
                phi = 2.0 * np.pi * self.rng.rand()
                cut_n = np.array([np.cos(phi) * np.sin(theta), np.sin(phi) * np.sin(theta), np.cos(theta)])
            else:
