...
class Highs(PersistentBase, PersistentSolver):
    ...
    def version(self):
        version = (highspy.HIGHS_VERSION_MAJOR, highspy.HIGHS_VERSION_MINOR, highspy.HIGHS_VERSION_PATCH)
        return version
            ...
            if _lb is not None or _ub is not None:
                if not is_constant(_lb) or not is_constant(_ub):
                    ...
                    if _ub is None:
                        ...
                        tmp_ub = _ub
                    mutable_bound = _MutableVarBounds(lower_expr=NPV_MaxExpression((tmp_lb, lb)), upper_expr=NPV_MinExpression((tmp_ub, ub)), pyomo_var_id=var_id, var_map=self._pyomo_var_to_solver_var_map, highs=self._solver_model)
    ...
    def _add_variables(self, variables: List[_GeneralVarData]):
        ...
        for v in variables:
            ...
            current_num_vars += 1
        self._solver_model.addVars(len(lbs), np.array(lbs, dtype=np.double), np.array(ubs, dtype=np.double))
        self._solver_model.changeColsIntegrality(len(vtypes), np.array(indices), np.array(vtypes))
        ...
        for con in cons:
            ...
            for ndx, coef in enumerate(repn.linear_coefs):
                ...
                if not is_constant(coef):
                    mutable_linear_coefficient = _MutableLinearCoefficient(pyomo_con=con, pyomo_var_id=v_id, con_map=self._pyomo_con_to_solver_con_map, var_map=self._pyomo_var_to_solver_var_map, expr=coef, highs=self._solver_model)
            ...
            if not is_constant(lb) or not is_constant(ub):
                mutable_con_bounds = _MutableConstraintBounds(lower_expr=lb, upper_expr=ub, pyomo_con=con, con_map=self._pyomo_con_to_solver_con_map, highs=self._solver_model)
                if con not in self._mutable_helpers:
            ...
            current_num_cons += 1
        self._solver_model.addRows(len(lbs), np.array(lbs, dtype=np.double), np.array(ubs, dtype=np.double), len(coef_values), np.array(starts), np.array(var_indices), np.array(coef_values, dtype=np.double))
    ...
    def _remove_constraints(self, cons: List[_GeneralConstraintData]):
        ...
        self._solver_con_to_pyomo_con_map.update({v: k for k, v in self._pyomo_con_to_solver_con_map.items()})
    ...
    def _update_variables(self, variables: List[_GeneralVarData]):
        ...
        self._solver_model.changeColsBounds(len(indices), np.array(indices), np.array(lbs, dtype=np.double), np.array(ubs, dtype=np.double))
        self._solver_model.changeColsIntegrality(len(indices), np.array(indices), np.array(vtypes))
    ...
    def _set_objective(self, obj):
        ...
        if obj is None:
            ...
            for coef, v in zip(repn.linear_coefs, repn.linear_vars):
                ...
                if not is_constant(coef):
                    mutable_objective_coef = _MutableObjectiveCoefficient(pyomo_var_id=v_id, var_map=self._pyomo_var_to_solver_var_map, expr=coef, highs=self._solver_model)
    ...
    def _postsolve(self, timer: HierarchicalTimer):
        ...
        elif results.termination_condition in {TerminationCondition.objectiveLimit, TerminationCondition.maxIterations, TerminationCondition.maxTimeLimit}:
            if self._sol.value_valid:
                has_feasible_solution = True
        if config.load_solution:
            if has_feasible_solution:
                if results.termination_condition != TerminationCondition.optimal:
                    logger.warning('Loading a feasible but suboptimal solution. Please set load_solution=False and check results.termination_condition and results.found_feasible_solution() before loading a solution.')
                self.load_vars()
            else:
                raise RuntimeError('A feasible solution was not found, so no solution can be loaded. If using the appsi.solvers.Highs interface, you can set opt.config.load_solution=False. If using the environ.SolverFactory interface, you can set opt.solve(model, load_solutions = False). Then you can check results.termination_condition and results.best_feasible_objective before loading a solution.')
    ...
    def get_primals(self, vars_to_load=None, solution_number=0):
        if self._sol is None or not self._sol.value_valid:
            raise RuntimeError('Solver does not currently have a valid solution. Please check the termination condition.')
        res = ComponentMap()
