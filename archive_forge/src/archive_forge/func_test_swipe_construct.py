...
def test_swipe_construct(document, comm):
    ...
    swipe = Swipe(before, after, value=20, height=800, slider_width=15, slider_color='lightgray')
    model = swipe.get_root(document, comm)
    assert model.children == {'before-inner': [before._models[model.ref['id']][0]], 'after-inner': [after._models[model.ref['id']][0]]}