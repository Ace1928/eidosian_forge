...
class KVStore(KVStoreBase):
    ...
    def broadcast(self, key, value, out, priority=0):
        ...
        if use_str_keys:
            check_call(_LIB.MXKVStoreBroadcastEx(self.handle, mx_uint(len(cvkeys)), cvkeys, mx_uint(len(cokeys)), cokeys, cvals, couts, ctypes.c_int(priority)))
        else:
            check_call(_LIB.MXKVStoreBroadcast(self.handle, mx_uint(len(cvkeys)), cvkeys, mx_uint(len(cokeys)), cokeys, cvals, couts, ctypes.c_int(priority)))
    ...
    def push(self, key, value, priority=0):
        ...
        ckeys, cvals, use_str_keys = _ctype_key_value(key, value)
        if use_str_keys:
            check_call(_LIB.MXKVStorePushEx(self.handle, mx_uint(len(ckeys)), ckeys, cvals, ctypes.c_int(priority)))
        else:
    ...
    def pull(self, key, out=None, priority=0, ignore_sparse=True):
        ...
        if use_str_keys:
            check_call(_LIB.MXKVStorePullWithSparseEx(self.handle, mx_uint(len(ckeys)), ckeys, cvals, ctypes.c_int(priority), ctypes.c_bool(ignore_sparse)))
        else:
            check_call(_LIB.MXKVStorePullWithSparse(self.handle, mx_uint(len(ckeys)), ckeys, cvals, ctypes.c_int(priority), ctypes.c_bool(ignore_sparse)))
