...
class RouteCQC:
    ...
    def __call__(self, circuit: 'cirq.AbstractCircuit', *, lookahead_radius: int=8, tag_inserted_swaps: bool=False, initial_mapper: Optional['cirq.AbstractInitialMapper']=None, context: Optional['cirq.TransformerContext']=None) -> 'cirq.AbstractCircuit':
        ...
        """
        routed_circuit, _, _ = self.route_circuit(circuit=circuit, lookahead_radius=lookahead_radius, tag_inserted_swaps=tag_inserted_swaps, initial_mapper=initial_mapper, context=context)
        return routed_circuit

    def route_circuit(self, circuit: 'cirq.AbstractCircuit', *, lookahead_radius: int=8, tag_inserted_swaps: bool=False, initial_mapper: Optional['cirq.AbstractInitialMapper']=None, context: Optional['cirq.TransformerContext']=None) -> Tuple['cirq.AbstractCircuit', Dict['cirq.Qid', 'cirq.Qid'], Dict['cirq.Qid', 'cirq.Qid']]:
        """Transforms the given circuit to make it executable on the device.
        ...
        if any((protocols.num_qubits(op) > 2 and (not protocols.is_measurement(op)) for op in circuit.all_operations())):
        ...
        routed_ops = self._route(mm, two_qubit_ops, single_qubit_ops, lookahead_radius, tag_inserted_swaps=tag_inserted_swaps)
        return (circuits.Circuit((circuits.Circuit(m) for m in routed_ops)), initial_mapping, {initial_mapping[mm.int_to_logical_qid[k]]: mm.int_to_physical_qid[v] for k, v in enumerate(mm.logical_to_physical)})
    ...
    @classmethod
    def _route(cls, mm: mapping_manager.MappingManager, two_qubit_ops: List[List['cirq.Operation']], single_qubit_ops: List[List['cirq.Operation']], lookahead_radius: int, tag_inserted_swaps: bool=False) -> List[List['cirq.Operation']]:
        ...
        """
        two_qubit_ops_ints: List[List[QidIntPair]] = [[(mm.logical_qid_to_int[op.qubits[0]], mm.logical_qid_to_int[op.qubits[1]]) for op in timestep_ops] for timestep_ops in two_qubit_ops]
        ...
        for timestep in range(len(two_qubit_ops)):
            ...
            while process_executable_two_qubit_ops(timestep):
                ...
                for swap in chosen_swaps:
                    inserted_swap = mm.mapped_op(ops.SWAP(mm.int_to_logical_qid[swap[0]], mm.int_to_logical_qid[swap[1]]))
    ...
    @classmethod
    def _choose_single_swap(cls, mm: mapping_manager.MappingManager, two_qubit_ops_ints: Sequence[Sequence[QidIntPair]], timestep: int, lookahead_radius: int) -> Optional[Tuple[QidIntPair, ...]]:
        """Computes cost function with list of single candidate swaps."""
        sigma: List[Tuple[QidIntPair, ...]] = [(swap,) for swap in cls._initial_candidate_swaps(mm, two_qubit_ops_ints[timestep])]
    ...
    @classmethod
    def _cost(cls, mm: mapping_manager.MappingManager, swaps: Tuple[QidIntPair, ...], two_qubit_ops: Sequence[QidIntPair]) -> Any:
