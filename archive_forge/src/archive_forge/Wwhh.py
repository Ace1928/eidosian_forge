...
class UniversalDataReader:
    ...
    def read_file(
        ...
        try:
            ...
            # Utilize a streaming approach for large files to manage memory usage effectively
            if (
                os.path.getsize(self.file_path) > 100 * 1024 * 1024
            ):  # Threshold for large files set at 100 MB
                logging.info(
                    f"Large file detected. Utilizing streaming read for {self.file_path}"
                )
                return self.stream_read_file(
                    file_handlers.get(self.file_type, self.attempt_read_unknown)
                )
            else:
    ...
    def stream_read_file(self, read_method):
        ...
        try:
            if read_method.__name__ in ["read_csv", "read_text", "read_json"]:
                ...
                )  # Process 10 MB chunks at a time
            else:
                logging.warning(
                    f"Streaming not implemented for {read_method.__name__}, using default read method."
                )
                return read_method()
        except Exception as e:
