...
def web_proxy_forward_server_group(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('web-proxy', 'forward-server-group', filtered_data, vdom=vdom)
        current_data = fos.get('web-proxy', 'forward-server-group', vdom=vdom, mkey=mkey)
    ...
    if state == 'present' or state is True:
        return fos.set('web-proxy', 'forward-server-group', data=filtered_data, vdom=vdom)
    elif state == 'absent':
        return fos.delete('web-proxy', 'forward-server-group', mkey=filtered_data['name'], vdom=vdom)
    else:
