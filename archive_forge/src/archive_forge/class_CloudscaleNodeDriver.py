...
class CloudscaleNodeDriver(NodeDriver):
    ...
    NODE_STATE_MAP = dict(changing=NodeState.PENDING, running=NodeState.RUNNING, stopped=NodeState.STOPPED, paused=NodeState.PAUSED)
    ...
    def create_node(self, name, size, image, location=None, ex_create_attr=None):
        ...
        * `user_data`: ``str`` for optional cloud-config data
        ...
        attr.update(name=name, image=image.id, flavor=size.id)
    ...
    def _action(self, node, action_name):
        response = self.connection.request(self._get_server_url(node.id) + '/' + action_name, method='POST')
    ...
    def _to_node(self, data):
        ...
        return Node(id=data['uuid'], name=data['name'], state=state, public_ips=public_ips, private_ips=private_ips, extra=extra, driver=self, image=self._to_image(data['image']), size=self._to_size(data['flavor']))

    def _to_size(self, data):
        extra = {'vcpu_count': data['vcpu_count']}
        ram = data['memory_gb'] * 1024
        return NodeSize(id=data['slug'], name=data['name'], ram=ram, disk=10, bandwidth=0, price=0, extra=extra, driver=self)
