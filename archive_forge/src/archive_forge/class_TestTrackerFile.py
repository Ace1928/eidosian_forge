...
class TestTrackerFile(GsUtilUnitTestCase):
    ...
    def testParallelUploadTrackerFileNoEncryption(self):
        ...
        enc_key, actual_prefix, actual_objects = ReadParallelUploadTrackerFile(fpath, self.logger)
        self.assertEqual(random_prefix, actual_prefix)
    ...
    def testWriteComponentToParallelUploadTrackerFile(self):
        ...
        try:
            WriteComponentToParallelUploadTrackerFile(fpath, tracker_file_lock, new_object, self.logger, encryption_key_sha256=None)
        ...
        WriteComponentToParallelUploadTrackerFile(fpath, tracker_file_lock, new_object, self.logger, encryption_key_sha256='456')
        actual_key, actual_prefix, actual_objects = ReadParallelUploadTrackerFile(fpath, self.logger)
        self.assertEqual(enc_key, actual_key)
        self.assertEqual(random_prefix, actual_prefix)
