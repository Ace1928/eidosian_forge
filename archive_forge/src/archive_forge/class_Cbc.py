...
class Cbc(PersistentSolver):
    ...
    def version(self):
        results = subprocess.run([str(self.config.executable), '-stop'], timeout=5, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, universal_newlines=True)
    ...
    def _apply_solver(self, timer: HierarchicalTimer):
        ...
        with TeeStream(*ostreams) as t:
            ...
            cp = subprocess.run(cmd, timeout=timeout, stdout=t.STDOUT, stderr=t.STDERR, universal_newlines=True)
            timer.stop('subprocess')
        if cp.returncode != 0:
            if self.config.load_solution:
                raise RuntimeError('A feasible solution was not found, so no solution can be loaded. If using the appsi.solvers.Cbc interface, you can set opt.config.load_solution=False. If using the environ.SolverFactory interface, you can set opt.solve(model, load_solutions = False). Then you can check results.termination_condition and results.best_feasible_objective before loading a solution.')
            results = Results()
        ...
        elif self._writer.get_active_objective().sense == minimize:
