...
class MA57(DirectLinearSolverInterface):
    ...
    def __init__(self, cntl_options=None, icntl_options=None, work_factor=1.2, fact_factor=2, ifact_factor=2):
        self._ma57 = MA57Interface(work_factor=work_factor, fact_factor=fact_factor, ifact_factor=ifact_factor)
    ...
    def do_symbolic_factorization(self, matrix: Union[spmatrix, BlockMatrix], raise_on_error: bool=True) -> LinearSolverResults:
        ...
        stat = self._ma57.do_symbolic_factorization(dim=self._dim, irn=matrix.row, jcn=matrix.col)
    ...
    def do_numeric_factorization(self, matrix: Union[spmatrix, BlockMatrix], raise_on_error: bool=True) -> LinearSolverResults:
        ...
        if nrows != ncols:
            raise ValueError('Matrix must be square')
        if nrows != self._dim:
            raise ValueError('Matrix dimensions do not match the dimensions of the matrix used for symbolic factorization')
