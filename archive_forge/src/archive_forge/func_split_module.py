...
def split_module(m: GraphModule, root_m: torch.nn.Module, split_callback: Callable[[Node], int], qualname_map: Optional[Dict[str, str]]=None, keep_original_order: Optional[bool]=False):
    ...
    for node in m.graph.nodes:
        if node.op in ['placeholder', 'get_attr', 'output']:
            if node.op == 'placeholder' and (val := node.meta.get('example_value')) is not None and isinstance(val, torch.SymInt) and isinstance(val.node.expr, sympy.Symbol):
    ...
    for node in m.graph.nodes:
        ...
        if assert_monotonically_increasing:
            ...
            assert highest_partition <= pid, f"autocast or set_grad_enabled require monotonically increasing partitions:highest: {highest_partition}, this node's: {pid}"
    ...
    for regions_mapping in [autocast_regions, grad_regions]:
        for node, regions in regions_mapping.items():
            ...
            for r in regions[1:]:
                ...
                new_node = partition.graph.create_node(op=node.op, target=node.target, args=tuple((arg for arg in node.args)), kwargs={}, type_expr=node.type)
    ...
    for regions_mapping in [autocast_regions]:
        for node in reversed(regions_mapping):
            ...
            for r in regions[:-1]:
                ...
                new_node = partition.graph.create_node(op=exit_node.op, target=exit_node.target, args=(partition.environment[node],), kwargs={}, type_expr=exit_node.type)
