...
def wanopt_content_delivery_network_rule(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('wanopt', 'content-delivery-network-rule', filtered_data, vdom=vdom)
        current_data = fos.get('wanopt', 'content-delivery-network-rule', vdom=vdom, mkey=mkey)
    ...
    if state == 'present' or state is True:
        return fos.set('wanopt', 'content-delivery-network-rule', data=filtered_data, vdom=vdom)
    elif state == 'absent':
        return fos.delete('wanopt', 'content-delivery-network-rule', mkey=filtered_data['name'], vdom=vdom)
    else:
