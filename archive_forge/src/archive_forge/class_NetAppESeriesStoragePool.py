...
class NetAppESeriesStoragePool(NetAppESeriesModule):
    ...
    def __init__(self):
        ...
        super(NetAppESeriesStoragePool, self).__init__(ansible_options=ansible_options, web_services_version=version, supports_check_mode=True, required_if=required_if)
    ...
    def apply(self):
        ...
        if self.state == 'present':
            if self.criteria_drive_count is None and self.criteria_min_usable_capacity is None:
                self.module.fail_json(msg='One of criteria_min_usable_capacity or criteria_drive_count must be specified.')
        ...
        if self.pool_detail:
            if self.state == 'absent':
                ...
                if self.criteria_drive_count and self.criteria_drive_count < len(self.storage_pool_drives):
                    self.module.fail_json(msg='Failed to reduce the size of the storage pool. Array [%s]. Pool [%s].' % (self.ssid, self.pool_detail['id']))
                if self.criteria_drive_type and self.criteria_drive_type != self.pool_detail['driveMediaType']:
                    self.module.fail_json(msg='Failed! It is not possible to modify storage pool media type. Array [%s]. Pool [%s].' % (self.ssid, self.pool_detail['id']))
                if self.criteria_drive_require_da is not None and self.criteria_drive_require_da != self.pool_detail['protectionInformationCapabilities']['protectionInformationCapable']:
                    self.module.fail_json(msg='Failed! It is not possible to modify DA-capability. Array [%s]. Pool [%s].' % (self.ssid, self.pool_detail['id']))
