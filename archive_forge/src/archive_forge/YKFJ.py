...
def setup() -> Tuple[pg.Surface, apple.Apple, search.Search, pg.time.Clock]:
    """
...
class Snake:
    ...
    def __init__(self):  # Python class constructor
        ...
        self.tail_position: Vector2 = self.get_tail_position()  # Method call in Python with type annotation

    # // Updating the snake position
    # update(apple) {
    def update(self, apple: apple.Apple) -> None:  # Python method definition with type annotations
        ...
        for i in range(len(self.path)):  # Python for loop
            ...
            head: Vector2 = self.get_head_position()  # Local variable assignment with type annotation
            ...
            if (
                ...
                next_head: Vector2 = self.path[i + 1]  # Accessing list element by index with type annotation
                ...
                # else {
                    ...
                    # noLoop();
                    pg.quit()  # Quit the game loop in Python
    ...
    # getHeadPosition() {
    def get_head_position(self) -> Vector2:  # Python method definition with return type annotation
        # return this.body[this.body.length - 1];
        return self.body[-1]  # Python negative indexing

    # getTailPosition() {
    def get_tail_position(self) -> Vector2:  # Python method definition with return type annotation
    def get_tail_position(self):  # Python method definition
        # return this.body[0];
    ...
    def change_direction(
        self, x_dir: int, y_dir: int
    ):  # Python method definition with type annotations
        # if (!(abs(this.x_dir - x_dir) == 2 || abs(this.y_dir - y_dir) == 2)) {
        if not (
            abs(self.x_dir - x_dir) == 2 or abs(self.y_dir - y_dir) == 2
        ):  # Python logical and comparison operators
            # this.x_dir = x_dir;
            self.x_dir = x_dir  # Assigning new direction
            # this.y_dir = y_dir;
            self.y_dir = y_dir  # Assigning new direction

    # // Direction functions
    # up() {
    def up(self):  # Python method definition
        # this.changeDirection(0, -1);
        self.change_direction(0, -1)  # Method call with arguments

    # down() {
    def down(self):  # Python method definition
        # this.changeDirection(0, 1);
        self.change_direction(0, 1)  # Method call with arguments

    # left() {
    def left(self):  # Python method definition
        # this.changeDirection(-1, 0);
        self.change_direction(-1, 0)  # Method call with arguments

    # right() {
    def right(self):  # Python method definition
        # this.changeDirection(1, 0);
        self.change_direction(1, 0)  # Method call with arguments

    # show() {
    def show(self):  # Python method definition
        # fill(0, 164, 239);

        pg.draw.rect(
            screen, (0, 164, 239), (self.body[0].x * 30, self.body[0].y * 30, 30, 30)
        )  # Drawing rectangle with pygame
        # strokeWeight(0);
        ...
        # strokeWeight(2);
        # let backToggle = -1;
        back_toggle: int = -1  # Python integer type annotation
        # let frontToggle = 1;
        front_toggle: int = 1  # Python integer type annotation
        ...
        for i in range(len(self.body)):  # Python for loop
            # strokeWeight(0);
            ...
            # strokeWeight(2);
            # if (i == 0) {
            if i == 0:  # Python comparison operator
                # backToggle = 1;
                back_toggle = 1  # Assigning new value
            # } else {
            else:  # Python else statement
                # backToggle = -1;
                back_toggle = -1  # Assigning new value
            ...
            if i == len(self.body) - 1:  # Python comparison operator
                # frontToggle = -1;
                front_toggle = -1  # Assigning new value
            # } else {
            else:  # Python else statement
                # frontToggle = 1;
                front_toggle = 1  # Assigning new value
