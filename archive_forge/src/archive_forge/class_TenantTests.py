...
class TenantTests(utils.ClientTestCase):
    ...
    def setUp(self):
        ...
        self.TEST_TENANTS = {'tenants': {'values': [{'enabled': True, 'description': 'A description change!', 'name': 'invisible_to_admin', 'id': self.INVIS_ID}, {'enabled': True, 'description': 'None', 'name': 'demo', 'id': self.DEMO_ID}, {'enabled': True, 'description': 'None', 'name': 'admin', 'id': self.ADMIN_ID}, {'extravalue01': 'metadata01', 'enabled': True, 'description': 'For testing extras', 'name': 'test_extras', 'id': self.EXTRAS_ID}], 'links': []}}

    def test_create(self):
        req_body = {'tenant': {'name': 'tenantX', 'description': 'Like tenant 9, but better.', 'enabled': True, 'extravalue01': 'metadata01'}}
        id_ = uuid.uuid4().hex
        resp_body = {'tenant': {'name': 'tenantX', 'enabled': True, 'id': id_, 'description': 'Like tenant 9, but better.', 'extravalue01': 'metadata01'}}
        self.stub_url('POST', ['tenants'], json=resp_body)
        tenant = self.client.tenants.create(req_body['tenant']['name'], req_body['tenant']['description'], req_body['tenant']['enabled'], extravalue01=req_body['tenant']['extravalue01'], name="don't overwrite priors")
    ...
    def test_update(self):
        req_body = {'tenant': {'id': self.EXTRAS_ID, 'name': 'tenantX', 'description': 'I changed you!', 'enabled': False, 'extravalue01': 'metadataChanged'}}
        resp_body = {'tenant': {'name': 'tenantX', 'enabled': False, 'id': self.EXTRAS_ID, 'description': 'I changed you!', 'extravalue01': 'metadataChanged'}}
        self.stub_url('POST', ['tenants', self.EXTRAS_ID], json=resp_body)
        tenant = self.client.tenants.update(req_body['tenant']['id'], req_body['tenant']['name'], req_body['tenant']['description'], req_body['tenant']['enabled'], extravalue01=req_body['tenant']['extravalue01'], name="don't overwrite priors")
        self.assertIsInstance(tenant, tenants.Tenant)
        self.assertRequestBodyIs(json=req_body)
    ...
    def test_remove_user(self):
        self.stub_url('DELETE', ['tenants', self.EXTRAS_ID, 'users', 'foo', 'roles', 'OS-KSADM', 'barrr'], status_code=204)
