...
class LightningBase(QubitDevice):
    ...
    def capabilities(cls):
        ...
        capabilities.update(model='qubit', supports_analytic_computation=True, supports_broadcasting=False, returns_state=True)
        return capabilities
    ...
    def probability(self, wires=None, shot_range=None, bin_size=None):
        ...
        if device_wires and len(device_wires) > 1 and (not np.all(np.array(device_wires)[:-1] <= np.array(device_wires)[1:])):
            raise RuntimeError('Lightning does not currently support out-of-order indices for probabilities')
    ...
    def _process_jacobian_tape(self, tape, starting_state, use_device_state, use_mpi: bool=False, split_obs: bool=False):
        state_vector = self._init_process_jacobian_tape(tape, starting_state, use_device_state)
        obs_serialized, obs_idx_offsets = QuantumScriptSerializer(self.short_name, self.use_csingle, use_mpi, split_obs).serialize_observables(tape, self.wire_map)
        ops_serialized, use_sp = QuantumScriptSerializer(self.short_name, self.use_csingle, use_mpi, split_obs).serialize_ops(tape, self.wire_map)
        ...
        for op_idx, trainable_param in enumerate(trainable_params):
            ...
            if isinstance(operation, Operation) and (not isinstance(operation, (BasisState, StatePrep))):
                tp_shift.append(trainable_param)
                record_tp_rows.append(all_params)
            all_params += 1
        if use_sp:
            tp_shift = [i - 1 for i in tp_shift]
        return {'state_vector': state_vector, 'obs_serialized': obs_serialized, 'ops_serialized': ops_serialized, 'tp_shift': tp_shift, 'record_tp_rows': record_tp_rows, 'all_params': all_params, 'obs_idx_offsets': obs_idx_offsets}
