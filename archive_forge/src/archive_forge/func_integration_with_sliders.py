...
def integration_with_sliders(rsys, tend, c0, parameters, fig_kwargs=None, slider_kwargs=None, conc_bounds=None, x_axis_type='linear', y_axis_type='linear', integrate_kwargs=None, odesys_extra=None, get_odesys_kw=None, integrate=None):
    """
    Parameters
    ----------
    rsys : ReactionSystem
    ...
    if x_axis_type == 'linear':
        ...
        tout = logspace_from_lin(tend * 1e-09, tend)
    else:
        raise NotImplementedError('Unknown x_axis_type: %s' % x_axis_type)
    result = integrate(tout, c0, parameters, **integrate_kwargs or {})
    sources = [ColumnDataSource(data={'tout': to_unitless(result.xout, output_time_unit), k: to_unitless(result.yout[:, idx], output_conc_unit)}) for idx, k in enumerate(rsys.substances)]
    if fig_kwargs is None:
        ...
        fig_kwargs = dict(plot_height=400, plot_width=400, title='C vs t', tools='crosshair,pan,reset,save,wheel_zoom', x_range=x_range, y_range=y_range, x_axis_type=x_axis_type, y_axis_type=y_axis_type)
    plot = Figure(**fig_kwargs)
    colors = 'red green blue black cyan magenta'.split()
    for idx, k in enumerate(rsys.substances):
        plot.line('tout', k, source=sources[idx], line_width=3, line_alpha=0.6, color=colors[idx % len(colors)])
    ...
    for k in rsys.substances:
        ...
        c0_widgets[k] = Slider(title=k + ' / ' + output_conc_unit.dimensionality.unicode if hasattr(output_conc_unit, 'dimensionality') else k, value=_C(k), **slider_kwargs.get(k, slider_defaults))
    param_widgets = OrderedDict([(k, Slider(title=k if u is None else k + ' / ' + u.dimensionality.unicode, value=v, **_dict_to_unitless(slider_kwargs.get(k, dict(start=v / 10, end=v * 10, step=v / 10)), u))) for k, v, u in zip(param_keys, p_ul, p_units)])
    ...
    def update_data(attrname, old, new):
        ...
        for idx, k in enumerate(rsys.substances):
            sources[idx].data = {'tout': to_unitless(_result.xout, output_time_unit), k: to_unitless(_result.yout[:, idx], output_conc_unit)}
