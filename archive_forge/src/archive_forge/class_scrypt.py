...
class scrypt(uh.ParallelismMixin, uh.HasRounds, uh.HasRawSalt, uh.HasRawChecksum, uh.HasManyIdents, uh.GenericHandler):
    ...
    def to_string(self):
        ...
        if ident == IDENT_SCRYPT:
            return '$scrypt$ln=%d,r=%d,p=%d$%s$%s' % (self.rounds, self.block_size, self.parallelism, bascii_to_str(b64s_encode(self.salt)), bascii_to_str(b64s_encode(self.checksum)))
        else:
            ...
            return bascii_to_str(b''.join([b'$7$', h64.encode_int6(self.rounds), h64.encode_int30(self.block_size), h64.encode_int30(self.parallelism), self.salt, b'$', h64.encode_bytes(self.checksum)]))
