...
class HollETeacher(FixedDialogTeacher):
    ...
    def add_cmdline_args(argparser):
        ...
        group.add_argument('--knowledge-types', '-kt', type=str, default='full', help='Either "full" (all of the following), "none" (knowledge not used) or comma separated list of knowledge types to include. Possible types: plot, review, comments, fact_table (contains awards, taglines, and similar movies). e.g. -kt plot,review,fact_table or -kt full')
    ...
    def get(self, episode_idx, entry_idx=0):
        ...
        if self.opt['knowledge_types'] != 'none' and entry_idx == 0:
            ...
            text = entry['query']
        action = {'id': self.id, 'text': text, 'episode_done': episode_done, 'labels': [entry['response']]}
        return action