...
class s3Test(common.HeatTestCase):
    ...
    def setUp(self):
        ...
        self.patchobject(s3.S3Bucket, 'client', return_value=self.mock_con)

    def create_resource(self, t, stack, resource_name):
        resource_defns = stack.t.resource_definitions(stack)
        rsrc = s3.S3Bucket('test_resource', resource_defns[resource_name], stack)
    ...
    def test_attributes(self):
        ...
        self.mock_con.get_auth.return_value = ('http://server.test:8080/v_2', None)
    ...
    def test_public_read(self):
        ...
        rsrc = self.create_resource(t, stack, 'S3Bucket')
        scheduler.TaskRunner(rsrc.delete)()
        self.mock_con.put_container.assert_called_once_with(utils.PhysName(stack.name, 'test_resource'), {'X-Container-Write': 'test_tenant:test_username', 'X-Container-Read': '.r:*'})
        self.mock_con.delete_container.assert_called_once_with(container_name)
    ...
    def test_tags(self):
        ...
        scheduler.TaskRunner(rsrc.delete)()
        self.mock_con.put_container.assert_called_once_with(utils.PhysName(stack.name, 'test_resource'), {'X-Container-Write': 'test_tenant:test_username', 'X-Container-Read': 'test_tenant:test_username', 'X-Container-Meta-S3-Tag-greeting': 'hello', 'X-Container-Meta-S3-Tag-location': 'here'})
        self.mock_con.delete_container.assert_called_once_with(container_name)
    ...
    def test_public_read_write(self):
        ...
        rsrc = self.create_resource(t, stack, 'S3Bucket')
        scheduler.TaskRunner(rsrc.delete)()
        self.mock_con.put_container.assert_called_once_with(container_name, {'X-Container-Write': '.r:*', 'X-Container-Read': '.r:*'})
        self.mock_con.delete_container.assert_called_once_with(container_name)
    ...
    def test_delete_exception(self):
        ...
        container_name = utils.PhysName(stack.name, 'test_resource')
        self.mock_con.put_container.return_value = None
        self.mock_con.delete_container.side_effect = sc.ClientException('Test Delete Failure')
        rsrc = self.create_resource(t, stack, 'S3Bucket')
        self.assertRaises(exception.ResourceFailure, scheduler.TaskRunner(rsrc.delete))
        self.mock_con.put_container.assert_called_once_with(container_name, {'X-Container-Write': 'test_tenant:test_username', 'X-Container-Read': 'test_tenant:test_username'})
        self.mock_con.delete_container.assert_called_once_with(container_name)
    ...
    def test_delete_not_found(self):
        ...
        container_name = utils.PhysName(stack.name, 'test_resource')
        self.mock_con.put_container.return_value = None
        self.mock_con.delete_container.side_effect = sc.ClientException('Gone', http_status=404)
        rsrc = self.create_resource(t, stack, 'S3Bucket')
        scheduler.TaskRunner(rsrc.delete)()
        self.mock_con.put_container.assert_called_once_with(container_name, {'X-Container-Write': 'test_tenant:test_username', 'X-Container-Read': 'test_tenant:test_username'})
        self.mock_con.delete_container.assert_called_once_with(container_name)
    ...
    def test_delete_retain(self):
        ...
        self.mock_con.put_container.assert_called_once_with(utils.PhysName(stack.name, 'test_resource'), {'X-Container-Write': 'test_tenant:test_username', 'X-Container-Read': 'test_tenant:test_username'})