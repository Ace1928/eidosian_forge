...
def test_remove_devices_force(self, mock_deconfigure_scsi_device, mock_get_fc_hbas_info, mock_get_possible_devices):
    exc = exception.ExceptionChainer()
    mock_deconfigure_scsi_device.side_effect = Exception
    connection_properties = {'targets': [5, 2]}
    self.connector._remove_devices(connection_properties, devices=None, device_info=None, force=True, exc=exc)
