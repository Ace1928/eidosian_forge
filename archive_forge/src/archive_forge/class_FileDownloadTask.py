...
class FileDownloadTask(copy_util.ObjectCopyTaskWithExitHandler):
    ...
    def __init__(self, source_resource, destination_resource, delete_source=False, do_not_decompress=False, posix_to_set=None, print_created_message=False, print_source_version=False, system_posix_data=None, user_request_args=None, verbose=False):
        """Initializes task.
    ...
    """
        super(FileDownloadTask, self).__init__(source_resource, destination_resource, posix_to_set=posix_to_set, print_created_message=print_created_message, print_source_version=print_source_version, user_request_args=user_request_args, verbose=verbose)
        self._delete_source = delete_source
        self._do_not_decompress = do_not_decompress
        self._system_posix_data = system_posix_data
        self._temporary_destination_resource = self._get_temporary_destination_resource()
        if self._source_resource.size and self._source_resource.size >= scaled_integer.ParseInteger(properties.VALUES.storage.resumable_threshold.Get()):
    ...
    def _restart_download(self):
        log.status.Print('Temporary download file corrupt. Restarting download {}'.format(self._source_resource))
    ...
    def execute(self, task_status_queue=None):
        ...
        posix_util.run_if_setting_posix(self._posix_to_set, self._user_request_args, posix_util.raise_if_invalid_file_permissions, self._system_posix_data, self._source_resource, known_posix=self._posix_to_set)
        ...
        if is_destination_symlink or destination_url.exists():
            if self._user_request_args and self._user_request_args.no_clobber:
                ...
                if self._send_manifest_messages:
                    manifest_util.send_skip_message(task_status_queue, self._source_resource, self._destination_resource, copy_util.get_no_clobber_message(destination_url))
                return
            os.remove(destination_url.object_name)
        temporary_download_file_exists = self._temporary_destination_resource.storage_url.exists()
        if temporary_download_file_exists and os.path.getsize(self._temporary_destination_resource.storage_url.object_name) > self._source_resource.size:
            self._restart_download()
        if _should_perform_sliced_download(self._source_resource, self._destination_resource):
            ...
            _, found_tracker_file = tracker_file_util.read_or_create_download_tracker_file(self._source_resource, self._temporary_destination_resource.storage_url, total_components=len(download_component_task_list))
            if found_tracker_file:
                log.debug('Resuming sliced download with {} components.'.format(len(download_component_task_list)))
            else:
                if temporary_download_file_exists:
                    self._restart_download()
                log.debug('Launching sliced download with {} components.'.format(len(download_component_task_list)))
            copy_component_util.create_file_if_needed(self._source_resource, self._temporary_destination_resource)
            return task.Output(additional_task_iterators=[download_component_task_list, finalize_sliced_download_task_list], messages=None)
        part_download_task_output = file_part_download_task.FilePartDownloadTask(self._source_resource, self._temporary_destination_resource, offset=0, length=self._source_resource.size, do_not_decompress=self._do_not_decompress, strategy=self._strategy, user_request_args=self._user_request_args).execute(task_status_queue=task_status_queue)
        temporary_file_url = self._temporary_destination_resource.storage_url
        server_encoding = task_util.get_first_matching_message_payload(part_download_task_output.messages, task.Topic.API_DOWNLOAD_RESULT)
        preserve_symlinks = symlink_util.get_preserve_symlink_from_user_request(self._user_request_args)
        download_util.finalize_download(self._source_resource, temporary_file_url.object_name, destination_url.object_name, convert_symlinks=preserve_symlinks, do_not_decompress_flag=self._do_not_decompress, server_encoding=server_encoding)
        tracker_file_util.delete_download_tracker_files(temporary_file_url)
        posix_util.run_if_setting_posix(self._posix_to_set, self._user_request_args, posix_util.set_posix_attributes_on_file_if_valid, self._system_posix_data, self._source_resource, self._destination_resource, known_source_posix=self._posix_to_set, preserve_symlinks=preserve_symlinks)
        self._print_created_message_if_requested(self._destination_resource)
        if self._send_manifest_messages:
            manifest_util.send_success_message(task_status_queue, self._source_resource, self._destination_resource, md5_hash=task_util.get_first_matching_message_payload(part_download_task_output.messages, task.Topic.MD5))
