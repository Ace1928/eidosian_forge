...
class EventHandler(AssistantEventHandler):
    ...
    def on_text_delta(self, delta, snapshot):
        # Append the text delta to the generated name
        self.generated_name += delta.value

    @override
    def on_run_created(self, data):
        logging.info("Run created: {}".format(data))

    @override
    def on_run_completed(self, data):
        logging.info("Run completed: {}".format(data))

    @override
    def on_run_failed(self, data):
        logging.error("Run failed: {}".format(data))

    @override
    def on_message_created(self, data):
        logging.info("Message created: {}".format(data))

    @override
    def on_message_completed(self, data):
        logging.info("Message completed: {}".format(data))

    @override
    def on_error(self, data):
        logging.error("Error occurred: {}".format(data))
...
class FileChecker:
    ...
    def browse_input_folder(self):
        ...
        if folder_selected:
            ...
            self.create_sorted_folder(folder_selected)
    ...
    def create_sorted_folder(self, folder_path: str):
        date_suffix = datetime.datetime.now().strftime("%d%m%Y")
        sorted_folder_name = f"sorted_py_{date_suffix}"
        parent_folder = os.path.dirname(folder_path)
        sorted_folder_path = os.path.join(parent_folder, sorted_folder_name)
        if not os.path.exists(sorted_folder_path):
            os.makedirs(sorted_folder_path)
        self.output_folder_path.set(sorted_folder_path)
    ...
    def generate_file_name(self, file_content: str) -> str:
        ...
        if not hasattr(self, "naming_thread"):
            self.naming_thread = self.openai_client.beta.threads.create()

        # Split file content into chunks of 4096 characters or less
        content_chunks = textwrap.wrap(file_content, width=4096)

        for chunk in content_chunks:
            # Add each chunk as a message to the existing or new thread
            self.openai_client.beta.threads.messages.create(
                thread_id=self.naming_thread.id,
                role="user",
