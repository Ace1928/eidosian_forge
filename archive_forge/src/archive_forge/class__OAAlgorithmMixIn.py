...
class _OAAlgorithmMixIn(object):
    ...
    def _fix_discrete_soln_solve_subproblem_and_add_cuts(self, discrete_prob_util_block, subprob_util_block, config):
        with fix_discrete_problem_solution_in_subproblem(discrete_prob_util_block, subprob_util_block, self, config):
            nlp_termination = solve_subproblem(subprob_util_block, self, config)
            if nlp_termination in {tc.optimal, tc.feasible}:
                primal_improved = self._update_bounds_after_solve('subproblem', primal=value(subprob_util_block.obj.expr), logger=config.logger)
                if primal_improved:
                    self.update_incumbent(subprob_util_block)
                self._add_cuts_to_discrete_problem(subprob_util_block, discrete_prob_util_block, self.objective_sense, config, self.timing)
