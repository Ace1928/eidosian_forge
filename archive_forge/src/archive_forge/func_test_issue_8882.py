...
def test_issue_8882():
    F = -a ** (-s + 1) * (4 + 1 / a ** 2) ** (-s / 2) * sqrt(1 / a ** 2) * exp(-s * I * pi) * sin(s * atan(sqrt(1 / a ** 2) / 2)) * gamma(s)
    raises(IntegralTransformError, lambda: inverse_mellin_transform(F, s, x, (-1, oo), **{'as_meijerg': True, 'needeval': True}))