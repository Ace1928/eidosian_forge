...
class LspStdIoReader(LspStdIoBase):
    ...
    async def _read_content(self, length: int, max_parts=1000, max_empties=200) -> Optional[bytes]:
        ...
        if raw_parts:
            ...
            if len(raw) != length:
                self.log.warning(f'Readout and content-length mismatch: {len(raw)} vs {length};remaining empties: {max_empties}; remaining parts: {max_parts}')
        return raw
        ...
        if line:
            ...
            if content_length:
                ...
                if raw is not None:
                    ...
                    self.log.warning('%s failed to read message of length %s', self, content_length)
        return message
