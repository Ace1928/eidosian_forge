...
class IBMSVCManageReplication(object):
    ...
    def __init__(self):
        ...
        argument_spec.update(dict(name=dict(type='str'), state=dict(type='str', required=True, choices=['present', 'absent']), remotecluster=dict(type='str'), copytype=dict(type='str', choices=['metro', 'global', 'GMCV']), master=dict(type='str'), aux=dict(type='str'), force=dict(type='bool', required=False), consistgrp=dict(type='str'), noconsistgrp=dict(type='bool', default=False), sync=dict(type='bool', default=False), cyclingperiod=dict(type='int')))
    ...
    def existing_rc(self):
        ...
        data = self.restapi.svc_obj_info(cmd='lsrcrelationship', cmdopts=None, cmdargs=[self.name])
        return data
    ...
    def delete(self):
        ...
        if result == '':
            ...
            self.changed = True
            self.log('delete the remote copy %s with result message %s', self.name, result['message'])
        else:
            self.module.fail_json(msg='Failed to delete the remote copy [%s]' % self.name)
    ...
    def apply(self):
        ...
        if rcrelationship_data:
            if self.state == 'absent':
                self.log("CHANGED: RemoteCopy relationship exists, requested state is 'absent'")
                changed = True
            ...
            changed = True
            self.log("CHANGED: Remotecopy relationship does not exist, but requested state is '%s'", self.state)
        if changed:
