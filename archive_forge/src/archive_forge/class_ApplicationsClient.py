...
class ApplicationsClient(object):
    ...
    def __init__(self, release_track=base.ReleaseTrack.ALPHA):
        ...
        self._poller = waiter.CloudOperationPoller(self._app_client, self.client.projects_locations_operations)
        self._delete_poller = waiter.CloudOperationPollerNoResources(self.client.projects_locations_operations)
        self._lookup_poller = waiter.CloudOperationPoller(self._lookup_client, self.client.projects_locations_operations)
    ...
    """
        describe_req = self.messages.ApphubProjectsLocationsApplicationsGetRequest(name=app_id)
    ...
    """
        list_req = self.messages.ApphubProjectsLocationsApplicationsListRequest(parent=parent)
        return list_pager.YieldFromList(self._app_client, list_req, field='applications', batch_size=page_size, limit=limit, batch_size_attribute='pageSize')

    def Create(self, app_id, scope_type, display_name, description, attributes, async_flag, parent):
    ...
    """
        application = self.messages.Application(description=description, displayName=display_name, scope=self.messages.Scope(type=self.messages.Scope.TypeValueValuesEnum(scope_type)), attributes=attributes)
        create_req = self.messages.ApphubProjectsLocationsApplicationsCreateRequest(application=application, applicationId=app_id, parent=parent)
        operation = self._app_client.Create(create_req)
        if async_flag:
            return operation
        create_response = api_lib_utils.WaitForOperation(self._poller, operation, api_lib_consts.CreateApplication.WAIT_FOR_ADD_MESSAGE, api_lib_consts.CreateApplication.ADD_TIMELIMIT_SEC)
    ...
    """
        ...
        if args.environment_type is not None:
            attributes.environment = api_lib_utils.GetMessagesModule(release_track).Environment(type=api_lib_utils.GetMessagesModule(release_track).Environment.TypeValueValuesEnum(args.environment_type))
            update_mask = api_lib_utils.AddToUpdateMask(update_mask, api_lib_consts.UpdateApplication.UPDATE_MASK_ENVIRONMENT_FIELD_NAME)
        if args.criticality_type:
            attributes.criticality = api_lib_utils.GetMessagesModule(release_track).Criticality(type=api_lib_utils.GetMessagesModule(release_track).Criticality.TypeValueValuesEnum(args.criticality_type))
            update_mask = api_lib_utils.AddToUpdateMask(update_mask, api_lib_consts.UpdateApplication.UPDATE_MASK_CRITICALITY_FIELD_NAME)
        for b_owner in args.business_owners or []:
            business_owner = api_lib_utils.GetMessagesModule(release_track).ContactInfo()
            ...
            if release_track == base.ReleaseTrack.ALPHA:
                if b_owner.get('channel-uri', None):
                    business_owner.channel = api_lib_utils.GetMessagesModule(release_track).Channel(uri=b_owner.get('channel-uri'))
            attributes.businessOwners.append(business_owner)
            update_mask = api_lib_utils.AddToUpdateMask(update_mask, api_lib_consts.UpdateApplication.UPDATE_MASK_BUSINESS_OWNERS_FIELD_NAME)
        for d_owner in args.developer_owners or []:
            developer_owner = api_lib_utils.GetMessagesModule(release_track).ContactInfo()
            ...
            if release_track == base.ReleaseTrack.ALPHA:
                if d_owner.get('channel-uri', None):
                    developer_owner.channel = api_lib_utils.GetMessagesModule(release_track).Channel(uri=d_owner.get('channel-uri'))
            attributes.developerOwners.append(developer_owner)
            update_mask = api_lib_utils.AddToUpdateMask(update_mask, api_lib_consts.UpdateApplication.UPDATE_MASK_DEVELOPER_OWNERS_FIELD_NAME)
        for o_owner in args.operator_owners or []:
            operator_owner = api_lib_utils.GetMessagesModule(release_track).ContactInfo()
            ...
            if release_track == base.ReleaseTrack.ALPHA:
                if o_owner.get('channel-uri'):
                    operator_owner.channel = api_lib_utils.GetMessagesModule(release_track).Channel(uri=o_owner.get('channel-uri'))
            attributes.operatorOwners.append(operator_owner)
            update_mask = api_lib_utils.AddToUpdateMask(update_mask, api_lib_consts.UpdateApplication.UPDATE_MASK_OPERATOR_OWNERS_FIELD_NAME)
        if args.display_name is not None:
            application.displayName = args.display_name
            update_mask = api_lib_utils.AddToUpdateMask(update_mask, api_lib_consts.UpdateApplication.UPDATE_MASK_DISPLAY_NAME_FIELD_NAME)
        if args.description is not None:
            application.description = args.description
            update_mask = api_lib_utils.AddToUpdateMask(update_mask, api_lib_consts.UpdateApplication.UPDATE_MASK_DESCRIPTION_FIELD_NAME)
        patch_req = self.messages.ApphubProjectsLocationsApplicationsPatchRequest(application=application, name=app_ref.RelativeName(), updateMask=update_mask)
        operation = self._app_client.Patch(patch_req)
        if args.async_:
            return operation
        patch_response = api_lib_utils.WaitForOperation(self._poller, operation, api_lib_consts.UpdateApplication.WAIT_FOR_UPDATE_MESSAGE, api_lib_consts.UpdateApplication.ADD_TIMELIMIT_SEC)
    ...
    """
        remove_req = self.messages.ApphubProjectsLocationsApplicationsDeleteRequest(name=app_id)
        operation = self._app_client.Delete(remove_req)
        if async_flag:
            return operation
        delete_response = api_lib_utils.WaitForOperation(self._delete_poller, operation, api_lib_consts.DeleteApplication.WAIT_FOR_DELETE_MESSAGE, api_lib_consts.DeleteApplication.REMOVE_TIMELIMIT_SEC)
    ...
    """
        get_req = self.messages.ApphubProjectsLocationsApplicationsGetIamPolicyRequest(resource=app_id)
