...
class scale(ABC, metaclass=Register):
    ...
    def __init__(self, **kwargs):
        ...
        if np.iterable(self.breaks) and np.iterable(self.labels) and (len(self.breaks) != len(self.labels)):
            raise PlotnineError('Breaks and labels have unequal lengths')
        if self.breaks is None and (not is_position_aes(self.aesthetics)) and (self.guide is not None):
            self.guide = None
    ...
    def expand_limits(self, limits, expand, coord_limits, trans) -> range_view:
        """
    ...
    def view(self, limits=None, range=None) -> scale_view:
        """
        Information about the trained scale
        """
        raise NotImplementedError

    def default_expansion(self, mult: float | TupleFloat2=0, add: float | TupleFloat2=0, expand=True) -> TupleFloat4:
        """
        ...
        if isinstance(add, (float, int)):
            add = (add, add)
        if len(mult) != 2:
            raise ValueError('The scale expansion multiplication factor should either be a single float, or a tuple of two floats. ')
        if len(add) != 2:
            raise ValueError('The scale expansion addition constant should either be a single float, or a tuple of two floats. ')
    ...
    def get_labels(self, breaks=None) -> ScaleLabels:
        """
        Get labels, calculating them if required
        """
        raise NotImplementedError

    def get_breaks(self, limits=None) -> ScaleBreaks:
        """
        Get Breaks
        """
        raise NotImplementedError

    def get_bounded_breaks(self, limits=None) -> ScaleBreaks:
        """
