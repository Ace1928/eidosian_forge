...
class V3IdentityPlugin(utils.TestCase):
    ...
    TEST_SERVICE_CATALOG = [{'endpoints': [{'url': 'http://cdn.admin-nets.local:8774/v1.0/', 'region': 'RegionOne', 'interface': 'public'}, {'url': 'http://127.0.0.1:8774/v1.0', 'region': 'RegionOne', 'interface': 'internal'}, {'url': 'http://cdn.admin-nets.local:8774/v1.0', 'region': 'RegionOne', 'interface': 'admin'}], 'type': 'nova_compat'}, {'endpoints': [{'url': 'http://nova/novapi/public', 'region': 'RegionOne', 'interface': 'public'}, {'url': 'http://nova/novapi/internal', 'region': 'RegionOne', 'interface': 'internal'}, {'url': 'http://nova/novapi/admin', 'region': 'RegionOne', 'interface': 'admin'}], 'type': 'compute', 'name': 'nova'}, {'endpoints': [{'url': 'http://glance/glanceapi/public', 'region': 'RegionOne', 'interface': 'public'}, {'url': 'http://glance/glanceapi/internal', 'region': 'RegionOne', 'interface': 'internal'}, {'url': 'http://glance/glanceapi/admin', 'region': 'RegionOne', 'interface': 'admin'}], 'type': 'image', 'name': 'glance'}, {'endpoints': [{'url': 'http://127.0.0.1:5000/v3', 'region': 'RegionOne', 'interface': 'public'}, {'url': 'http://127.0.0.1:5000/v3', 'region': 'RegionOne', 'interface': 'internal'}, {'url': TEST_ADMIN_URL, 'region': 'RegionOne', 'interface': 'admin'}], 'type': 'identity'}, {'endpoints': [{'url': 'http://swift/swiftapi/public', 'region': 'RegionOne', 'interface': 'public'}, {'url': 'http://swift/swiftapi/internal', 'region': 'RegionOne', 'interface': 'internal'}, {'url': 'http://swift/swiftapi/admin', 'region': 'RegionOne', 'interface': 'admin'}], 'type': 'object-store'}]
    TEST_SERVICE_PROVIDERS = [{'auth_url': 'https://sp1.com/v3/OS-FEDERATION/identity_providers/acme/protocols/saml2/auth', 'id': 'sp1', 'sp_url': 'https://sp1.com/Shibboleth.sso/SAML2/ECP'}, {'auth_url': 'https://sp2.com/v3/OS-FEDERATION/identity_providers/acme/protocols/saml2/auth', 'id': 'sp2', 'sp_url': 'https://sp2.com/Shibboleth.sso/SAML2/ECP'}]

    def setUp(self):
        super(V3IdentityPlugin, self).setUp()
        self.TEST_DISCOVERY_RESPONSE = {'versions': {'values': [fixture.V3Discovery(self.TEST_URL)]}}
        ...
        self.TEST_PROJECTS_RESPONSE = {'projects': [{'domain_id': '1789d1', 'enabled': 'True', 'id': '263fd9', 'links': {'self': 'https://identity:5000/v3/projects/263fd9'}, 'name': 'Dev Group A'}, {'domain_id': '1789d1', 'enabled': 'True', 'id': 'e56ad3', 'links': {'self': 'https://identity:5000/v3/projects/e56ad3'}, 'name': 'Dev Group B'}], 'links': {'self': 'https://identity:5000/v3/projects'}}
        ...
        self.TEST_OAUTH2_MTLS_TOKEN_RESPONSE = {'token': {'methods': ['oauth2_credential'], 'expires_at': '%i-02-01T00:00:10.000123Z' % nextyear, 'project': {'domain': {'id': self.TEST_DOMAIN_ID, 'name': self.TEST_DOMAIN_NAME}, 'id': self.TEST_TENANT_ID, 'name': self.TEST_TENANT_NAME}, 'user': {'domain': {'id': self.TEST_DOMAIN_ID, 'name': self.TEST_DOMAIN_NAME}, 'id': self.TEST_USER, 'name': self.TEST_USER}, 'issued_at': '2013-05-29T16:55:21.468960Z', 'catalog': self.TEST_SERVICE_CATALOG, 'service_providers': self.TEST_SERVICE_PROVIDERS, 'oauth2_credential': {'x5t#S256': '7UN-z4yFIm9s4jakecGoKa4rc353pDCuFUo9fsDD_1s='}}}
        self.TEST_RECEIPT_RESPONSE = {'receipt': {'methods': ['password'], 'expires_at': '%i-02-01T00:00:10.000123Z' % nextyear, 'user': {'domain': {'id': self.TEST_DOMAIN_ID, 'name': self.TEST_DOMAIN_NAME}, 'id': self.TEST_USER, 'name': self.TEST_USER}, 'issued_at': '2013-05-29T16:55:21.468960Z'}, 'required_auth_methods': [['password', 'totp']]}
    ...
    def test_with_trust_id(self):
        self.stub_auth(json=self.TEST_RESPONSE_DICT)
        a = v3.Password(self.TEST_URL, username=self.TEST_USER, password=self.TEST_PASS, trust_id='trust')
        self.assertTrue(a.has_scope_parameters)
        s = session.Session(a)
        self.assertEqual({'X-Auth-Token': self.TEST_TOKEN}, s.get_auth_headers())
    ...
    def test_full_url_overrides_endpoint_filter(self):
        self.stub_auth(json=self.TEST_RESPONSE_DICT)
        self.stub_url('GET', [], base_url='http://testurl/', text='SUCCESS', status_code=200)
        a = v3.Password(self.TEST_URL, username=self.TEST_USER, password=self.TEST_PASS)
        s = session.Session(auth=a)
    ...
    def test_unscoped_with_scope_data(self):
        a = v3.Password(self.TEST_URL, user_id=uuid.uuid4().hex, password=uuid.uuid4().hex, unscoped=True, project_id=uuid.uuid4().hex)
        s = session.Session()
        self.assertRaises(exceptions.AuthorizationFailure, a.get_auth_ref, s)
    ...
    def test_authenticate_with_unversioned_endpoint(self):
        self.stub_auth(json=self.TEST_RESPONSE_DICT)
        a = v3.Password(self.TEST_ROOT_URL, username=self.TEST_USER, password=self.TEST_PASS)
        self.assertFalse(a.has_scope_parameters)
        s = session.Session(auth=a)
        self.assertEqual({'X-Auth-Token': self.TEST_TOKEN}, s.get_auth_headers())
        req = {'auth': {'identity': {'methods': ['password'], 'password': {'user': {'name': self.TEST_USER, 'password': self.TEST_PASS}}}}}
    ...
    def test_oauth2_client_credential_method_http(self):
        ...
        self.stub_auth(json=self.TEST_APP_CRED_TOKEN_RESPONSE)
        client_cre = v3.OAuth2ClientCredential(base_http, oauth2_endpoint=oauth2_endpoint, oauth2_client_id=self.TEST_CLIENT_CRED_ID, oauth2_client_secret=self.TEST_CLIENT_CRED_SECRET)
    ...
    def test_oauth2_mtls_client_credential_method(self):
        ...
        self.assertEqual(auth_ref.project_id, self.TEST_OAUTH2_MTLS_TOKEN_RESPONSE.get('token', {}).get('project', {}).get('id'))
