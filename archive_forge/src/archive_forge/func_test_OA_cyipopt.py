...
@unittest.skipUnless(SolverFactory('cyipopt').available(exception_flag=False), 'APPSI_IPOPT not available.')
def test_OA_cyipopt(self):
    """Test the outer approximation decomposition algorithm."""
    with SolverFactory('mindtpy') as opt:
        for model in nonconvex_model_list:
            model = model.clone()
            results = opt.solve(model, strategy='OA', mip_solver=required_solvers[1], nlp_solver='cyipopt', heuristic_nonconvex=True)
