...
@SolverFactory.register('mindtpy.oa', doc='MindtPy: Mixed-Integer Nonlinear Decomposition Toolbox in Pyomo')
class MindtPy_OA_Solver(_MindtPyAlgorithm):
    ...
    def check_config(self):
        ...
        if config.single_tree:
            ...
            if config.mip_solver not in {'cplex_persistent', 'gurobi_persistent'}:
                raise ValueError('Only cplex_persistent and gurobi_persistent are supported for LP/NLP based Branch and Bound method.Please refer to https://pyomo.readthedocs.io/en/stable/contributed_packages/mindtpy.html#lp-nlp-based-branch-and-bound.')
            if config.threads > 1:
                config.threads = 1
                config.logger.info('The threads parameter is corrected to 1 since lazy constraint callback conflicts with multi-threads mode.')
    ...
    def initialize_mip_problem(self):
        ...
        self.jacobians = calc_jacobians(self.mip.MindtPy_utils.nonlinear_constraint_list, self.config.differentiate_mode)
    ...
    def add_cuts(self, dual_values, linearize_active=True, linearize_violated=True, cb_opt=None, nlp=None):
        ...
        if len(self.mip.MindtPy_utils.grey_box_list) > 0:
            add_oa_cuts_for_grey_box(self.mip, nlp, self.config, self.objective_sense, self.mip_iter, cb_opt)
