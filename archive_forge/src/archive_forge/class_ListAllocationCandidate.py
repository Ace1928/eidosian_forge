...
class ListAllocationCandidate(command.Lister, version.CheckerMixin):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument('--resource', metavar='<resource_class>=<value>', dest='resources', action=AppendToGroup, help='String indicating an amount of resource of a specified class that providers in each allocation request must collectively have the capacity and availability to serve. Can be specified multiple times per resource class. For example: ``--resource VCPU=4 --resource DISK_GB=64 --resource MEMORY_MB=2048``')
        parser.add_argument('--limit', metavar='<limit>', help='A positive integer to limit the maximum number of allocation candidates. This option requires at least ``--os-placement-api-version 1.16``.')
        parser.add_argument('--required', metavar='<required>', action=AppendToGroup, help='A required trait. May be repeated. Allocation candidates must collectively contain all of the required traits. This option requires at least ``--os-placement-api-version 1.17``. Since ``--os-placement-api-version 1.39`` the value of this parameter can be a comma separated list of trait names to express OR relationship between those traits.')
        parser.add_argument('--forbidden', metavar='<forbidden>', action=AppendToGroup, help='A forbidden trait. May be repeated. Returned allocation candidates must not contain any of the specified traits. This option requires at least ``--os-placement-api-version 1.22``.')
    ...
    def take_action(self, parsed_args):
        ...
        if 'groups' not in parsed_args:
            raise exceptions.CommandError('At least one --resource must be specified.')
        ...
        for suffix, group in parsed_args.groups.items():
            ...
            if 'resources' not in group:
                raise exceptions.CommandError('--resources should be provided in group %s', suffix)
            for resource in group['resources']:
                if not len(resource.split('=')) == 2:
                    raise exceptions.CommandError('Arguments to --resource must be of form <resource_class>=<value>')
            params[_get_key('resources')] = ','.join((resource.replace('=', ':') for resource in group['resources']))
            required_traits = []
            ...
            params[_get_key('required')] = common.get_required_query_param_from_args(required_traits, forbidden_traits)
            if 'aggregate_uuid' in group and group['aggregate_uuid']:
                ...
                params[_get_key('member_of')] = 'in:' + ','.join(group['aggregate_uuid'])
            if 'member_of' in group and group['member_of']:
                self.check_version(version.ge('1.21'))
                params[_get_key('member_of')] = ['in:' + aggs for aggs in group['member_of']]
        ...
        for rp_uuid, resources in resp['provider_summaries'].items():
            rp_resources[rp_uuid] = ','.join(('%s=%s/%s' % (rc, value['used'], value['capacity']) for rc, value in resources['resources'].items()))
            if include_traits:
        ...
        if self.compare_version(version.ge('1.12')):
            for i, allocation_req in enumerate(resp['allocation_requests']):
                for rp, resources in allocation_req['allocations'].items():
                    req = ','.join(('%s=%s' % (rc, value) for rc, value in resources['resources'].items()))
                    if include_traits:
            ...
            for i, allocation_req in enumerate(resp['allocation_requests']):
                for allocation in allocation_req['allocations']:
                    ...
                    req = ','.join(('%s=%s' % (rc, value) for rc, value in allocation['resources'].items()))
