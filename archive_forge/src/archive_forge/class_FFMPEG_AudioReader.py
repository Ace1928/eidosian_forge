...
class FFMPEG_AudioReader:
    ...
    def read_chunk(self, chunksize):
        ...
        result = (1.0 * result / 2 ** (8 * self.nbytes - 1)).reshape((int(len(result) / self.nchannels), self.nchannels))
        self.pos = self.pos + chunksize
        return result
    ...
    def get_frame(self, tt):
        ...
        if isinstance(tt, np.ndarray):
            ...
            if not in_time.any():
                raise IOError('Error in file %s, ' % self.filename + 'Accessing time t=%.02f-%.02f seconds, ' % (tt[0], tt[-1]) + 'with clip duration=%d seconds, ' % self.duration)
            ...
            try:
                ...
                warnings.warn('Error in file %s, ' % self.filename + 'At time t=%.02f-%.02f seconds, ' % (tt[0], tt[-1]) + 'indices wanted: %d-%d, ' % (indices.min(), indices.max()) + 'but len(buffer)=%d\n' % len(self.buffer) + str(error), UserWarning)
                indices[indices >= len(self.buffer)] = len(self.buffer) - 1
                result[in_time] = self.buffer[indices]
                return result
        else:
            ind = int(self.fps * tt)
            if ind < 0 or ind > self.nframes:
