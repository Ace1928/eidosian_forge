...
def queue_dequeue_v2(handle: _atypes.TensorFuzzingAnnotation[_atypes.Resource], component_types, timeout_ms: int=-1, name=None):
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'QueueDequeueV2', name, handle, 'component_types', component_types, 'timeout_ms', timeout_ms)
            return _result
            ...
            pass
        try:
            return queue_dequeue_v2_eager_fallback(handle, component_types=component_types, timeout_ms=timeout_ms, name=name, ctx=_ctx)
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('QueueDequeueV2', handle=handle, component_types=component_types, timeout_ms=timeout_ms, name=name)
    _result = _outputs[:]
    if not _result:
        return _op
