...
def pytorch_logger_v1(prefix: str='pytorch', device: int=0, cuda_mem_pool: str='all', cuda_mem_metric: str='all') -> LoggerT:
    try:
        import torch
    except ImportError:
        raise ImportError("The 'torch' library could not be found - did you install it? Alternatively, specify the 'ConsoleLogger' in the 'training.logger' config section, instead of the 'PyTorchLogger'.")
    ...
    def setup_logger(nlp: Language, stdout: IO=sys.stdout, stderr: IO=sys.stderr):
        ...
        if cuda_mem_pool not in expected_cuda_mem_pool:
            raise ValueError(f"Got CUDA memory pool '{cuda_mem_pool}', but expected one of: '{expected_cuda_mem_pool}'")
        elif cuda_mem_metric not in expected_cuda_mem_metric:
            raise ValueError(f"Got CUDA memory metric '{cuda_mem_metric}', but expected one of: '{expected_cuda_mem_metric}'")
