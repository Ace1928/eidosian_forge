...
class IRDielectricTensor(MSONable):
    ...
    def __init__(self, oscillator_strength: ArrayLike, ph_freqs_gamma: ArrayLike, epsilon_infinity: ArrayLike, structure: Structure) -> None:
        """
        Args:
    ...
    def as_dict(self) -> dict:
        ...
        return {'@module': type(self).__module__, '@class': type(self).__name__, 'oscillator_strength': self.oscillator_strength.tolist(), 'ph_freqs_gamma': self.ph_freqs_gamma.tolist(), 'structure': self.structure.as_dict(), 'epsilon_infinity': self.epsilon_infinity.tolist()}
    ...
    @add_fig_kwargs
    def plot(self, components: Sequence=('xx',), reim: str='reim', show_phonon_frequencies: bool=True, xlim: float | None=None, ylim: float | None=None, **kwargs) -> Axes:
    ...
    def get_spectrum(self, component: Sequence | str, reim: str, broad: list | float=5e-05, emin: float=0, emax: float | None=None, divs: int=500, label=None) -> Spectrum:
