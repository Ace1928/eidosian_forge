...
class NonIterativeStudy:
    ...
    def __init__(self, objective: NonIterativeObjectiveFunc, optimizer: NonIterativeObjectiveLocalOptimizer):
        self._objective = objective
        self._optimizer = optimizer

    def optimize(self, dataset: TuneDataset, distributed: Optional[bool]=None, monitor: Optional[Monitor]=None, stopper: Optional[NonIterativeStopper]=None, stop_check_interval: Any=None, logger: Any=None) -> StudyResult:
        ...
        if not _dist:
            ...
            res = dataset.data.process(preprocess).per_row().transform(self._compute_transformer, schema=f'*,{TUNE_REPORT_ADD_SCHEMA}', callback=entrypoint, params=dict(stop_check_interval=_interval, logger=logger))
    ...
    def _compute_transformer(self, df: Iterable[Dict[str, Any]], entrypoint: Optional[Callable[[str, Dict[str, Any]], Any]]=None, stop_check_interval: Any=None, logger: Any=None) -> Iterable[Dict[str, Any]]:
        j: Optional[RemoteTrialJudge] = None if entrypoint is None else RemoteTrialJudge(entrypoint)
        for row in df:
            for n, trial in enumerate(_get_trials_from_row(row, with_dfs=False)):
                if j is not None:
                    if stop_check_interval is None:
                        ...
                        try:
                            report = run_monitored_process(self._local_process_trial, [row, n], {'logger': logger}, lambda: not j.can_accept(trial), stop_check_interval)
