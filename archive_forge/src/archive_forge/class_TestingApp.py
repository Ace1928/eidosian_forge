...
class TestingApp(RequestHandler):
    ...
    def upload(self, request):
        ...
        if len(files_) != 1:
            return Response("Expected 1 file for '%s', not %d" % (param, len(files_)), status='400 Bad Request')
        file_ = files_[0]
        data = file_['body']
        if int(size) != len(data):
            return Response('Wrong size: %d != %d' % (size, len(data)), status='400 Bad Request')
        ...
        if filename != got_filename:
            return Response(u'Wrong filename: %s != %s' % (filename, file_.filename), status='400 Bad Request')
        return Response()
    ...
    def echo_params(self, request):
        params = sorted([(ensure_str(k), ensure_str(v)) for k, v in request.params.items()])
