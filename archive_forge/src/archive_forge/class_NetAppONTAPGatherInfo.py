...
class NetAppONTAPGatherInfo(object):
    ...
    def get_subset_info(self, gather_subset_info, default_fields=None):
        ...
        if not error:
            return gathered_ontap_info
        if int(error.get('code', 0)) == 3 or (int(error.get('code', 0)) == 19726344 and 'No recommendation can be made for this cluster' in error.get('message')):
