...
class FirestoreChatMessageHistory(BaseChatMessageHistory):
    ...
    def __init__(self, collection_name: str, session_id: str, user_id: str, firestore_client: Optional[Client]=None):
        """
    ...
    def upsert_messages(self, new_message: Optional[BaseMessage]=None) -> None:
        ...
        if not self._document:
            raise ValueError('Document not initialized')
        self._document.set({'id': self.session_id, 'user_id': self.user_id, 'messages': messages_to_dict(self.messages)})
