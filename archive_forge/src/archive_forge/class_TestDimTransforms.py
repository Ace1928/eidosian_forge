...
class TestDimTransforms(ComparisonTestCase):
    ...
    def setUp(self):
        ...
        self.dataset = Dataset((self.linear_ints, self.linear_floats, self.negative, self.repeating, self.booleans), ['int', 'float', 'negative', 'categories', 'booleans'])
        if dd is not None:
        ...
        if da is not None:
            ...
            dask_da = xr.DataArray(data=dask_array, coords=dict([('x', x), ('y', y)]), dims=['y', 'x'])
    ...
    def test_categorize_transform_dict_with_default(self):
        expr = dim('categories').categorize({'A': 'circle', 'B': 'square'}, default='triangle')
        expected = pd.Series(['circle', 'square', 'triangle'] * 3 + ['circle'])
        self.assert_apply(expr, expected)
    ...
    def test_multi_dim_expression_applies(self):
        self.assertEqual((dim('int') - dim('float')).applies(self.dataset), True)

    def test_multi_dim_expression_not_applies(self):
        self.assertEqual((dim('foo') - dim('bar')).applies(self.dataset), False)
