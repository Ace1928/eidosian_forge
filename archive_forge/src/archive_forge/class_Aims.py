...
class Aims(FileIOCalculator):
    ...
    def read_stress(self):
        ...
        stress = None
        for n, line in enumerate(lines):
            if line.rfind('|              Analytical stress tensor') > -1 or line.rfind('Numerical stress tensor') > -1:
                stress = []
                for i in [n + 5, n + 6, n + 7]:
                    data = lines[i].split()
                    stress += [float(data[2]), float(data[3]), float(data[4])]
        self.results['stress'] = np.array([stress[0], stress[4], stress[8], stress[5], stress[2], stress[1]])
    ...
    def get_stresses(self, voigt=False):
        ...
        if voigt:
            ...
            for ii, stress in enumerate(voigt_stresses):
                ...
                stresses[ii] = np.array([(xx, xy, xz), (xy, yy, yz), (xz, yz, zz)])
            return stresses
