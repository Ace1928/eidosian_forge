...
class ModuleValidator(Validator):
    ...
    REJECTLIST_FILES = frozenset(('.git', '.gitignore', '.travis.yml', '.gitattributes', '.gitmodules', 'COPYING', '__init__.py', 'VERSION', 'test-docs.sh'))
    REJECTLIST = REJECTLIST_FILES.union(REJECTLIST['module'])
    PS_ARG_VALIDATE_REJECTLIST = frozenset(('win_dsc.ps1',))

    def __init__(self, path, git_cache: GitCache, analyze_arg_spec=False, collection=None, collection_version=None, reporter=None, routing=None, plugin_type='module'):
    ...
    def _check_interpreter(self, powershell=False):
        if powershell:
            if not self.text.startswith('#!powershell\n'):
                self.reporter.error(path=self.object_path, code='missing-powershell-interpreter', msg='Interpreter line is not "#!powershell"')
            return
        missing_python_interpreter = False
        if not self.text.startswith('#!/usr/bin/python'):
            if NEW_STYLE_PYTHON_MODULE_RE.search(to_bytes(self.text)):
                missing_python_interpreter = self.text.startswith('#!')
            else:
                missing_python_interpreter = True
        if missing_python_interpreter:
            self.reporter.error(path=self.object_path, code='missing-python-interpreter', msg='Interpreter line is not "#!/usr/bin/python"')
    ...
    def _check_type_instead_of_isinstance(self, powershell=False):
        ...
        for line_no, line in enumerate(self.text.splitlines()):
            ...
            if typekeyword:
                self.reporter.error(path=self.object_path, code='unidiomatic-typecheck', msg='Type comparison using type() found. Use isinstance() instead', line=line_no + 1)
    ...
    def _check_for_sys_exit(self):
        ...
        for line_no, line in enumerate(self.text.splitlines()):
            ...
            if sys_exit_usage:
                self.reporter.error(path=self.object_path, code='use-fail-json-not-sys-exit', msg='sys.exit() call found. Should be exit_json/fail_json', line=line_no + 1)
            ...
            if isinstance(child, ast.Import):
                if child.names[0].name == 'subprocess':
                    for line_no, line in enumerate(self.text.splitlines()):
                        ...
                        if sp_match:
                            self.reporter.error(path=self.object_path, code='use-run-command-not-popen', msg='subprocess.Popen call found. Should be module.run_command', line=line_no + 1, column=sp_match.span()[0] + 1)
                ...
                for rejectlist_import, options in REJECTLIST_IMPORTS.items():
                    if re.search(rejectlist_import, name.name):
                        ...
                        if self._is_new_module() and new_only:
                            self.reporter.error(path=self.object_path, **options['error'])
                        elif not new_only:
                            self.reporter.error(path=self.object_path, **options['error'])
            ...
            if isinstance(child, (ast.Import, ast.ImportFrom)):
                ...
                if [n for n in names if n.startswith('ansible.module_utils')]:
                    ...
                    for name in child.names:
                        if 'module_utils' in getattr(child, 'module', '') and isinstance(name, ast.alias) and (name.name == '*'):
                            msg = ('module-utils-specific-import', 'module_utils imports should import specific components, not "*"')
                            if self._is_new_module():
                                self.reporter.error(path=self.object_path, code=msg[0], msg=msg[1], line=child.lineno)
                            else:
                                self.reporter.warning(path=self.object_path, code=msg[0], msg=msg[1], line=child.lineno)
                        if isinstance(name, ast.alias) and name.name == 'basic':
                            found_basic = True
        if not found_basic:
            self.reporter.warning(path=self.object_path, code='missing-module-utils-basic-import', msg='Did not find "ansible.module_utils.basic" import')
        return linenos
    ...
    def _find_has_import(self):
        for child in self.ast.body:
            ...
            if found_try_except_import and (not found_has):
                self.reporter.warning(path=self.object_path, code='try-except-missing-has', msg='Found Try/Except block without HAS_ assignment')
    ...
    def _ensure_imports_below_docs(self, doc_info, first_callable):
        ...
        for child in self.ast.body:
            if isinstance(child, (ast.Import, ast.ImportFrom)):
                ...
                if min_doc_line and child.lineno < min_doc_line:
                    self.reporter.error(path=self.object_path, code='import-before-documentation', msg='Import found before documentation variables. All imports must appear below DOCUMENTATION/EXAMPLES/RETURN.', line=child.lineno)
                    break
            elif isinstance(child, TRY_EXCEPT):
                bodies = child.body
                for handler in child.handlers:
        ...
        for import_line in import_lines:
            if not max_doc_line < import_line < first_callable:
                msg = ('import-placement', 'Imports should be directly below DOCUMENTATION/EXAMPLES/RETURN.')
                if self._is_new_module():
                    self.reporter.error(path=self.object_path, code=msg[0], msg=msg[1], line=import_line)
                else:
                    self.reporter.warning(path=self.object_path, code=msg[0], msg=msg[1], line=import_line)

    def _validate_ps_replacers(self):
        module_requires = '(?im)^#\\s*requires\\s+\\-module(?:s?)\\s*(Ansible\\.ModuleUtils\\..+)'
        csharp_requires = '(?im)^#\\s*ansiblerequires\\s+\\-csharputil\\s*(Ansible\\..+)'
        found_requires = False
        for req_stmt in re.finditer(module_requires, self.text):
            found_requires = True
            module_list = [x.strip() for x in req_stmt.group(1).split(',')]
            if len(module_list) > 1:
                self.reporter.error(path=self.object_path, code='multiple-utils-per-requires', msg='Ansible.ModuleUtils requirements do not support multiple modules per statement: "%s"' % req_stmt.group(0))
                continue
            module_name = module_list[0]
            if module_name.lower().endswith('.psm1'):
                self.reporter.error(path=self.object_path, code='invalid-requires-extension', msg='Module #Requires should not end in .psm1: "%s"' % module_name)
        for req_stmt in re.finditer(csharp_requires, self.text):
            found_requires = True
            module_list = [x.strip() for x in req_stmt.group(1).split(',')]
            if len(module_list) > 1:
                self.reporter.error(path=self.object_path, code='multiple-csharp-utils-per-requires', msg='Ansible C# util requirements do not support multiple utils per statement: "%s"' % req_stmt.group(0))
                continue
            module_name = module_list[0]
            if module_name.lower().endswith('.cs'):
                self.reporter.error(path=self.object_path, code='illegal-extension-cs', msg='Module #AnsibleRequires -CSharpUtil should not end in .cs: "%s"' % module_name)
        if not found_requires and REPLACER_WINDOWS not in self.text:
            self.reporter.error(path=self.object_path, code='missing-module-utils-import-csharp-requirements', msg='No Ansible.ModuleUtils or C# Ansible util requirements/imports found')
    ...
    def _find_ps_docs_file(self):
        ...
        if not os.path.isfile(py_path):
            self.reporter.error(path=self.object_path, code='missing-documentation', msg='No DOCUMENTATION provided')
        return py_path
    ...
    def _validate_docs(self):
        ...
        if sidecar_text:
            ...
            for error in errors:
                self.reporter.error(path=self.object_path, code='documentation-syntax-error', **error)
            for trace in traces:
            ...
            if bool(doc_info['DOCUMENTATION']['value']):
                doc, errors, traces = parse_yaml(doc_info['DOCUMENTATION']['value'], doc_info['DOCUMENTATION']['lineno'], self.name, 'DOCUMENTATION')
                for error in errors:
                    self.reporter.error(path=self.object_path, code='documentation-syntax-error', **error)
                for trace in traces:
                    self.reporter.trace(path=self.object_path, tracebk=trace)
            ...
            if bool(doc_info['RETURN']['value']):
                returns, errors, traces = parse_yaml(doc_info['RETURN']['value'], doc_info['RETURN']['lineno'], self.name, 'RETURN')
                for error in errors:
                    self.reporter.error(path=self.object_path, code='return-syntax-error', **error)
                for trace in traces:
                    self.reporter.trace(path=self.object_path, tracebk=trace)
        if doc:
            add_collection_to_versions_and_dates(doc, self.collection_name, is_module=self.plugin_type == 'module')
            missing_fragment = False
            with CaptureStd():
                try:
                    get_docstring(self.path, fragment_loader=fragment_loader, verbose=True, collection_name=self.collection_name, plugin_type=self.plugin_type)
                except AssertionError:
                    fragment = doc['extends_documentation_fragment']
                    self.reporter.error(path=self.object_path, code='missing-doc-fragment', msg='DOCUMENTATION fragment missing: %s' % fragment)
                    missing_fragment = True
                except Exception as e:
                    self.reporter.trace(path=self.object_path, tracebk=traceback.format_exc())
                    self.reporter.error(path=self.object_path, code='documentation-error', msg='Unknown DOCUMENTATION error, see TRACE: %s' % e)
            if not missing_fragment:
                add_fragments(doc, self.object_path, fragment_loader=fragment_loader, is_module=self.plugin_type == 'module')
            if 'options' in doc and doc['options'] is None:
                self.reporter.error(path=self.object_path, code='invalid-documentation-options', msg='DOCUMENTATION.options must be a dictionary/hash when used')
            if 'deprecated' in doc and doc.get('deprecated'):
                ...
                if documentation_collection != self.collection_name:
                    self.reporter.error(path=self.object_path, code='deprecation-wrong-collection', msg='"DOCUMENTATION.deprecation.removed_from_collection must be the current collection name: %r vs. %r' % (documentation_collection, self.collection_name))
            else:
                doc_deprecated = False
            if os.path.islink(self.object_path):
                self._validate_docs_schema(doc, doc_schema(os.readlink(self.object_path).split('.')[0], for_collection=bool(self.collection), deprecated_module=deprecated, plugin_type=self.plugin_type), 'DOCUMENTATION', 'invalid-documentation')
            else:
                self._validate_docs_schema(doc, doc_schema(self.object_name.split('.')[0], for_collection=bool(self.collection), deprecated_module=deprecated, plugin_type=self.plugin_type), 'DOCUMENTATION', 'invalid-documentation')
        ...
        if not examples_raw and self.plugin_type in PLUGINS_WITH_EXAMPLES:
            if self.plugin_type in PLUGINS_WITH_EXAMPLES:
                self.reporter.error(path=self.object_path, code='missing-examples', msg='No EXAMPLES provided')
        elif self.plugin_type in PLUGINS_WITH_YAML_EXAMPLES:
            dummy, errors, traces = parse_yaml(examples_raw, examples_lineno, self.name, 'EXAMPLES', load_all=True, ansible_loader=True)
            for error in errors:
                self.reporter.error(path=self.object_path, code='invalid-examples', **error)
            for trace in traces:
                self.reporter.trace(path=self.object_path, tracebk=trace)
        if returns:
            if returns:
                add_collection_to_versions_and_dates(returns, self.collection_name, is_module=self.plugin_type == 'module', return_docs=True)
            self._validate_docs_schema(returns, return_schema(for_collection=bool(self.collection), plugin_type=self.plugin_type), 'RETURN', 'return-syntax-error')
        elif self.plugin_type in PLUGINS_WITH_RETURN_VALUES:
            if self._is_new_module():
                self.reporter.error(path=self.object_path, code='missing-return', msg='No RETURN provided')
            else:
                self.reporter.warning(path=self.object_path, code='missing-return-legacy', msg='No RETURN provided')
        if not self.collection:
            ...
            if mismatched_deprecation:
                self.reporter.error(path=self.object_path, code='deprecation-mismatch', msg='Module deprecation/removed must agree in documentation, by prepending filename with "_", and setting DOCUMENTATION.deprecated for deprecation or by removing all documentation for removed')
        else:
            if self.object_name.startswith('_'):
                self.reporter.error(path=self.object_path, code='collections-no-underscore-on-deprecation', msg='Deprecated content in collections MUST NOT start with "_", update meta/runtime.yml instead')
            if not doc_deprecated == routing_says_deprecated:
                self.reporter.error(path=self.object_path, code='deprecation-mismatch', msg='"meta/runtime.yml" and DOCUMENTATION.deprecation do not agree.')
                ...
                if not compare_dates(routing_date, documentation_date):
                    self.reporter.error(path=self.object_path, code='deprecation-mismatch', msg='"meta/runtime.yml" and DOCUMENTATION.deprecation do not agree on removal date: %r vs. %r' % (routing_date, documentation_date))
                if routing_version != documentation_version:
                    self.reporter.error(path=self.object_path, code='deprecation-mismatch', msg='"meta/runtime.yml" and DOCUMENTATION.deprecation do not agree on removal version: %r vs. %r' % (routing_version, documentation_version))
        return (doc_info, doc)
    ...
    def _check_sem_option(self, part: dom.OptionNamePart, current_plugin: dom.PluginIdentifier) -> None:
        ...
        if tuple(part.link) not in self._all_options:
            self.reporter.error(path=self.object_path, code='invalid-documentation-markup', msg='Directive "%s" contains a non-existing option "%s"' % (part.source, part.name))
    ...
    def _validate_argument_spec(self, docs, spec, kwargs, context=None, last_context_spec=None):
        ...
        try:
            if not context:
                add_fragments(docs, self.object_path, fragment_loader=fragment_loader, is_module=self.plugin_type == 'module')
        except Exception:
            return
        ...
        if docs:
            ...
            for arg in docs_missing_from_args:
                ...
                self.reporter.error(path=self.object_path, code='nonexistent-parameter-documented', msg=msg)
    ...
    def validate(self):
        ...
        if not self._python_module() and (not self._powershell_module()) and (not self._sidecar_doc()):
            self.reporter.error(path=self.object_path, code='invalid-extension', msg='Official Ansible modules must have a .py extension for python modules or a .ps1 for powershell modules')
            self._python_module_override = True
        if self._python_module() and self.ast is None:
            self.reporter.error(path=self.object_path, code='python-syntax-error', msg='Python SyntaxError while parsing module')
            try:
            ...
            if docs and docs.get('deprecated', False):
                if 'removed_in' in docs['deprecated']:
                    ...
                    if collection_name != self.collection_name:
                        self.reporter.error(path=self.object_path, code='invalid-module-deprecation-source', msg='The deprecation version for a module must be added in this collection')
                    else:
                        try:
                            removed_in = self._create_strict_version(str(version), collection_name=collection_name)
                        except ValueError as e:
                            self.reporter.error(path=self.object_path, code='invalid-module-deprecation-version', msg='The deprecation version %r cannot be parsed: %s' % (version, e))
                    if removed_in:
                        if not self.collection:
                            strict_ansible_version = self._create_strict_version('.'.join(ansible_version.split('.')[:2]), self.collection_name)
                            end_of_deprecation_should_be_removed_only = strict_ansible_version >= removed_in
                            if end_of_deprecation_should_be_removed_only:
                                self.reporter.error(path=self.object_path, code='ansible-deprecated-module', msg='Module is marked for removal in version %s of Ansible when the current version is %s' % (version, ansible_version))
                        elif self.collection_version:
                            strict_ansible_version = self.collection_version
                            end_of_deprecation_should_be_removed_only = strict_ansible_version >= removed_in
                            if end_of_deprecation_should_be_removed_only:
                                self.reporter.error(path=self.object_path, code='collection-deprecated-module', msg='Module is marked for removal in version %s of this collection when the current version is %s' % (version, self.collection_version_str))
        ...
        if self._python_module() and (not self._just_docs()) and (not end_of_deprecation_should_be_removed_only):
            ...
            if doc_info:
                first_callable = self._get_first_callable() or 1000000
        ...
        if self._powershell_module():
            ...
            pattern = '(?im)^#\\s*ansiblerequires\\s+\\-csharputil\\s*Ansible\\.Basic'
