...
class NetworkAccessLocalExceptionRules(object):
    ...
    def __init__(self, params, ise):
        self.ise = ise
        self.new_object = dict(link=params.get('link'), profile=params.get('profile'), rule=params.get('rule'), security_group=params.get('securityGroup'), policy_id=params.get('policyId'), id=params.get('id'))

    def get_object_by_name(self, name, policy_id):
        result = None
        items = self.ise.exec(family='network_access_authorization_exception_rules', function='get_network_access_local_exception_rules', params={'policy_id': policy_id}).response.get('response', []) or []
        for item in items:
    ...
    def get_object_by_id(self, id, policy_id):
        try:
            result = self.ise.exec(family='network_access_authorization_exception_rules', function='get_network_access_local_exception_rule_by_id', handle_func_exception=False, params={'id': id, 'policy_id': policy_id}).response['response']
    ...
    def requires_update(self, current_obj):
        ...
        obj_params = [('link', 'link'), ('profile', 'profile'), ('rule', 'rule'), ('securityGroup', 'security_group'), ('policyId', 'policy_id'), ('id', 'id')]
    ...
    def create(self):
        result = self.ise.exec(family='network_access_authorization_exception_rules', function='create_network_access_local_exception_rule', params=self.new_object).response
        return result
    ...
    def update(self):
        ...
        result = self.ise.exec(family='network_access_authorization_exception_rules', function='update_network_access_local_exception_rule_by_id', params=self.new_object).response
        return result
    ...
    def delete(self):
        ...
        if not id:
            ...
            rule.update(dict(id=id_))
            self.new_object.update(dict(rule=rule, id=id_))
        result = self.ise.exec(family='network_access_authorization_exception_rules', function='delete_network_access_local_exception_rule_by_id', params=self.new_object).response
        return result