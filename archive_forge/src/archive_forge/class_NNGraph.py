...
class NNGraph(Graph):
    ...
    def __init__(self, Xin, NNtype='knn', use_flann=False, center=True, rescale=True, k=10, sigma=0.1, epsilon=0.01, gtype=None, plotting={}, symmetrize_type='average', dist_type='euclidean', order=0, **kwargs):
        self.Xin = Xin
        self.NNtype = NNtype
        ...
        if self.NNtype == 'knn':
            ...
            kdt = spatial.KDTree(Xout)
            D, NN = kdt.query(Xout, k=None, distance_upper_bound=epsilon, p=dist_translation[dist_type])
            count = 0
            for i in range(N):
        ...
        super(NNGraph, self).__init__(W=W, gtype=gtype, plotting=plotting, coords=Xout, **kwargs)