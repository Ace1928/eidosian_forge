...
def test_stack_by_name_badname(self):
    ...
    self.assertIsNone(stack_object.Stack.get_by_name(ctx, {'notallowed': self.stack.name}))
    self.assertIsNone(stack_object.Stack.get_by_name_and_owner_id(ctx, {'notallowed': self.stack.name}, 'owner'))