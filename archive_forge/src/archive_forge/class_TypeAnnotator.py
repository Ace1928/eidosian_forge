...
class TypeAnnotator(metaclass=_TypeAnnotator):
    TYPE_TO_EXPRESSIONS: t.Dict[exp.DataType.Type, t.Set[t.Type[exp.Expression]]] = {exp.DataType.Type.BIGINT: {exp.ApproxDistinct, exp.ArraySize, exp.Count, exp.Length}, exp.DataType.Type.BOOLEAN: {exp.Between, exp.Boolean, exp.In, exp.RegexpLike}, exp.DataType.Type.DATE: {exp.CurrentDate, exp.Date, exp.DateFromParts, exp.DateStrToDate, exp.DiToDate, exp.StrToDate, exp.TimeStrToDate, exp.TsOrDsToDate}, exp.DataType.Type.DATETIME: {exp.CurrentDatetime, exp.DatetimeAdd, exp.DatetimeSub}, exp.DataType.Type.DOUBLE: {exp.ApproxQuantile, exp.Avg, exp.Div, exp.Exp, exp.Ln, exp.Log, exp.Pow, exp.Quantile, exp.Round, exp.SafeDivide, exp.Sqrt, exp.Stddev, exp.StddevPop, exp.StddevSamp, exp.Variance, exp.VariancePop}, exp.DataType.Type.INT: {exp.Ceil, exp.DatetimeDiff, exp.DateDiff, exp.Extract, exp.TimestampDiff, exp.TimeDiff, exp.DateToDi, exp.Floor, exp.Levenshtein, exp.Sign, exp.StrPosition, exp.TsOrDiToDi}, exp.DataType.Type.JSON: {exp.ParseJSON}, exp.DataType.Type.TIMESTAMP: {exp.CurrentTime, exp.CurrentTimestamp, exp.StrToTime, exp.TimeAdd, exp.TimeStrToTime, exp.TimeSub, exp.TimestampAdd, exp.TimestampSub, exp.UnixToTime}, exp.DataType.Type.TINYINT: {exp.Day, exp.Month, exp.Week, exp.Year, exp.Quarter}, exp.DataType.Type.VARCHAR: {exp.ArrayConcat, exp.Concat, exp.ConcatWs, exp.DateToDateStr, exp.GroupConcat, exp.Initcap, exp.Lower, exp.Substring, exp.TimeToStr, exp.TimeToTimeStr, exp.Trim, exp.TsOrDsToDateStr, exp.UnixToStr, exp.UnixToTimeStr, exp.Upper}}
    ...
    BINARY_COERCIONS: BinaryCoercions = {**swap_all({(t, exp.DataType.Type.INTERVAL): lambda l, r: _coerce_date_literal(l, r.args.get('unit')) for t in exp.DataType.TEXT_TYPES}), **swap_all({(text, numeric): lambda l, r: t.cast(exp.DataType.Type, l.type if l.type in exp.DataType.NUMERIC_TYPES else r.type) for text in exp.DataType.TEXT_TYPES for numeric in exp.DataType.NUMERIC_TYPES}), **swap_all({(exp.DataType.Type.DATE, exp.DataType.Type.INTERVAL): lambda l, r: _coerce_date(l, r.args.get('unit'))})}
    ...
    def _set_type(self, expression: exp.Expression, target_type: t.Optional[exp.DataType | exp.DataType.Type]) -> None:
        expression.type = target_type or exp.DataType.Type.UNKNOWN
