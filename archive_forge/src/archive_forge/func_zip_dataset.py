...
def zip_dataset(input_datasets: List[_atypes.TensorFuzzingAnnotation[_atypes.Variant]], output_types, output_shapes, metadata: str='', name=None) -> _atypes.TensorFuzzingAnnotation[_atypes.Variant]:
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'ZipDataset', name, input_datasets, 'output_types', output_types, 'output_shapes', output_shapes, 'metadata', metadata)
            return _result
            ...
            pass
        try:
            return zip_dataset_eager_fallback(input_datasets, output_types=output_types, output_shapes=output_shapes, metadata=metadata, name=name, ctx=_ctx)
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('ZipDataset', input_datasets=input_datasets, output_types=output_types, output_shapes=output_shapes, metadata=metadata, name=name)
    _result = _outputs[:]
    if _execute.must_record_gradient():
        _attrs = ('output_types', _op.get_attr('output_types'), 'output_shapes', _op.get_attr('output_shapes'), 'N', _op._get_attr_int('N'), 'metadata', _op.get_attr('metadata'))
