...
from aiokeydb.v1.exceptions import LockError, LockNotOwnedError
class AsyncLock:
    """
    ...
    def __init__(self, keydb: 'AsyncKeyDB', name: Union[str, bytes, memoryview], timeout: Optional[float]=None, sleep: float=0.1, blocking: bool=True, blocking_timeout: Optional[float]=None, thread_local: bool=True):
        """
    ...
    async def acquire(self, blocking: Optional[bool]=None, blocking_timeout: Optional[float]=None, token: Optional[Union[str, bytes]]=None):
        """
    ...
    async def do_release(self, expected_token: bytes) -> None:
        if not bool(await self.lua_release(keys=[self.name], args=[expected_token], client=self.keydb)):
    ...
    async def do_extend(self, additional_time, replace_ttl) -> bool:
        ...
        if not bool(await self.lua_extend(keys=[self.name], args=[self.local.token, additional_time, replace_ttl and '1' or '0'], client=self.keydb)):
    ...
    async def do_reacquire(self) -> bool:
        timeout = int(self.timeout * 1000)
        if not bool(await self.lua_reacquire(keys=[self.name], args=[self.local.token, timeout], client=self.keydb)):
