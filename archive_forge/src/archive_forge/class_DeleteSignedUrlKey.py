  ...
  *{command}* deletes an existing Cloud CDN Signed URL key from a backend
  ...
  """
    ...
    def Args(parser):
        ...
        DeleteSignedUrlKey.BACKEND_BUCKET_ARG.AddArgument(parser, operation_type='delete CDN signed URL key from')
    ...
    def Run(self, args):
        ...
        request = messages.ComputeBackendBucketsDeleteSignedUrlKeyRequest(project=backend_bucket_ref.project, backendBucket=backend_bucket_ref.Name(), keyName=args.key_name)
        ...
        return waiter.WaitFor(operation_poller, operation_ref, 'Deleting Cloud CDN Signed URL key from [{0}]'.format(backend_bucket_ref.Name()))