...
class IdentityCloudMixin:
    ...
    def get_project(self, name_or_id, filters=None, domain_id=None):
        ...
        :raises: :class:`~openstack.exceptions.SDKException` if something goes
            wrong during the OpenStack API call.
        """
        return _utils._get_entity(self, 'project', name_or_id, filters, domain_id=domain_id)
    ...
    def update_project(self, name_or_id, enabled=None, domain_id=None, **kwargs):
        ...
        :returns: An identity ``Project`` object.
        """
        project = self.identity.find_project(name_or_id=name_or_id, domain_id=domain_id)
        if not project:
    ...
    def create_project(self, name, domain_id, description=None, enabled=True, **kwargs):
        """Create a project.
        ...
        :returns: An identity ``Project`` object.
        """
        attrs = dict(name=name, description=description, domain_id=domain_id, is_enabled=enabled)
        if kwargs:
            attrs.update(kwargs)
    ...
    def delete_project(self, name_or_id, domain_id=None):
        ...
        """
        try:
            project = self.identity.find_project(name_or_id=name_or_id, ignore_missing=True, domain_id=domain_id)
            if not project:
            ...
            self.log.exception('Error in deleting project {project}'.format(project=name_or_id))
            return False
    ...
    @_utils.valid_kwargs('name', 'email', 'enabled', 'domain_id', 'password', 'description', 'default_project')
    ...
    def delete_user(self, name_or_id, **kwargs):
        try:
            ...
            if not user:
                self.log.debug('User {0} not found for deleting'.format(name_or_id))
                return False
            self.identity.delete_user(user)
            return True
        except exceptions.SDKException:
            self.log.exception('Error in deleting user {user}'.format(user=name_or_id))
            return False
    ...
    def create_endpoint(self, service_name_or_id, url=None, interface=None, region=None, enabled=True, **kwargs):
        ...
        if (url or interface) and (public_url or internal_url or admin_url):
            raise exceptions.SDKException('create_endpoint takes either url and interface OR public_url, internal_url, admin_url')
        service = self.get_service(name_or_id=service_name_or_id)
        if service is None:
            raise exceptions.SDKException('service {service} not found'.format(service=service_name_or_id))
        endpoints_args = []
        if url:
            endpoints_args.append({'url': url, 'interface': interface, 'service_id': service['id'], 'enabled': enabled, 'region_id': region})
        else:
            endpoint_args = {'region_id': region, 'enabled': enabled, 'service_id': service['id']}
            if public_url:
    ...
    def update_domain(self, domain_id=None, name=None, description=None, enabled=None, name_or_id=None):
        ...
        if domain_id is None:
            ...
            if dom is None:
                raise exceptions.SDKException('Domain {0} not found for updating'.format(name_or_id))
            domain_id = dom['id']
        domain_ref = {}
    ...
    def create_group(self, name, description, domain=None):
        ...
        if domain:
            ...
            if not dom:
                raise exceptions.SDKException('Creating group {group} failed: Invalid domain {domain}'.format(group=name, domain=domain))
    ...
    def update_group(self, name_or_id, name=None, description=None, **kwargs):
        ...
        if group is None:
            raise exceptions.SDKException('Group {0} not found for updating'.format(name_or_id))
        group_ref = {}
        if name:
            group_ref['name'] = name
        if description:
    ...
    def delete_group(self, name_or_id):
        ...
        try:
            ...
            self.log.exception('Unable to delete group {name}'.format(name=name_or_id))
            return False
    ...
    def delete_role(self, name_or_id, **kwargs):
        ...
        try:
            ...
            self.log.exception('Unable to delete role {name}'.format(name=name_or_id))
            raise
