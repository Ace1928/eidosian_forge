...
class Space:
    ...
    def __init__(self, *args, **kwargs: Any):
        if len(args) > 0:
            assert_or_throw(len(args) == 1 and len(kwargs) == 0, ValueError('when the first argument is a template or dict, it must be the only argument of the constructor'))
            if isinstance(args[0], dict):
                ...
                assert_or_throw(all((isinstance(x, TuningParametersTemplate) for x in self._templates)), ValueError('not a list of templates'))
            else:
    ...
    def __mul__(self, other: Any) -> 'Space':
        ...
        def get_templates() -> Iterable[TuningParametersTemplate]:
            ...
            for lr in product([self, o], safe=True, remove_empty=True):
    ...
    def __radd__(self, other: Any) -> 'Space':
        ...
        assert_or_throw(other is None or (isinstance(other, int) and other == 0), ValueError(other))
        return self