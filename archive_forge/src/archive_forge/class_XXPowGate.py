...
class XXPowGate(gate_features.InterchangeableQubitsGate, eigen_gate.EigenGate):
    ...
    def _decompose_into_clifford_with_qubits_(self, qubits):
        ...
        if self.exponent % 2 == 0.5:
            return [pauli_interaction_gate.PauliInteractionGate(pauli_gates.X, False, pauli_gates.X, False).on(*qubits), clifford_gate.SingleQubitCliffordGate.X_sqrt.on_each(*qubits)]
        if self.exponent % 2 == 1:
            return [clifford_gate.SingleQubitCliffordGate.X.on_each(*qubits)]
        if self.exponent % 2 == 1.5:
            return [pauli_interaction_gate.PauliInteractionGate(pauli_gates.X, False, pauli_gates.X, False).on(*qubits), clifford_gate.SingleQubitCliffordGate.X_nsqrt.on_each(*qubits)]
    ...
    def _circuit_diagram_info_(self, args: 'cirq.CircuitDiagramInfoArgs') -> Union[str, 'protocols.CircuitDiagramInfo']:
        return protocols.CircuitDiagramInfo(wire_symbols=('XX', 'XX'), exponent=self._diagram_exponent(args))
    ...
    def __repr__(self) -> str:
        ...
        return f'cirq.XXPowGate(exponent={proper_repr(self._exponent)}, global_shift={self._global_shift!r})'