...
class TestAssertMicroversionFor(base.TestCase):
    ...
    def test_compatible(self, mock_get_ver):
        ...
        self.assertEqual('1.42', self.res._assert_microversion_for(self.session, 'fetch', '1.6'))
    ...
    def test_incompatible(self, mock_get_ver):
        ...
        self.assertRaisesRegex(exceptions.NotSupported, '1.6 is required, but 1.1 will be used', self.res._assert_microversion_for, self.session, 'fetch', '1.6')
        mock_get_ver.assert_called_once_with(self.session, action='fetch')
    ...
    def test_none(self, mock_get_ver):
        ...
        self.assertRaisesRegex(exceptions.NotSupported, '1.6 is required, but the default version', self.res._assert_microversion_for, self.session, 'fetch', '1.6')
        mock_get_ver.assert_called_once_with(self.session, action='fetch')