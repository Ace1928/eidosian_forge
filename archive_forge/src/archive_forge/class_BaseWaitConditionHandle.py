...
class BaseWaitConditionHandle(signal_responder.SignalResponder):
    ...
    def handle_signal(self, details=None):
        ...
        signal_reason = 'status:%s reason:%s' % (data_written[self.STATUS], data_written[self.REASON])
        return signal_reason

    def get_status(self):
        """Return a list of the Status values for the handle signals."""
        return [v[self.STATUS] for v in self.metadata_get(refresh=True).values()]

    def get_status_reason(self, status):
        """Return a list of reasons associated with a particular status."""
        return [v[self.REASON] for v in self.metadata_get(refresh=True).values() if v[self.STATUS] == status]