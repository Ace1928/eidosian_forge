  ...
  """
    ...
    if isinstance(filetype, str):
        if filetype not in _FILETYPE_MAP:
            raise ValueError(f'Unexpected filetype. Received: {filetype}. Expected one of {list(_FILETYPE_MAP.keys())}')
        reader_fn = _FILETYPE_MAP[filetype]
    elif callable(filetype):
        reader_fn = filetype
    else:
        raise ValueError(f'Argument `filetype` should be a string or a callable. Received: {filetype} of type {type(filetype)}.')
    ...
    with ops.device(file_reader_device):
        if isinstance(files, str):
            ...
            raise ValueError(f'Argument `files` should be a string or a `tf.data.Dataset` instance. Received: {files}')
        if filename_shuffle_buffer_size:
            source_dataset = source_dataset.shuffle(buffer_size=filename_shuffle_buffer_size)
        source_dataset = source_dataset.apply(interleave_ops.parallel_interleave(reader_fn, cycle_length=num_parallel_reads, sloppy=sloppy))
    ...
    def LoadingFunc(h):
        remote_iterator = iterator_ops.Iterator.from_string_handle(h, dataset_ops.get_legacy_output_types(source_dataset), dataset_ops.get_legacy_output_shapes(source_dataset))
    ...
    with ops.device('/job:%s' % worker_job):
        output_dataset = dataset_ops.Dataset.range(2).repeat().map(MapFn, num_parallel_calls=4 if sloppy else None)
