...
def SortOrgFirewallRules(client, rules):
    ...
    ingress_org_firewall_rule = [item for item in rules if item.direction == client.messages.SecurityPolicyRule.DirectionValueValuesEnum.INGRESS]
    ingress_org_firewall_rule.sort(key=lambda x: x.priority, reverse=False)
    egress_org_firewall_rule = [item for item in rules if item.direction == client.messages.SecurityPolicyRule.DirectionValueValuesEnum.EGRESS]
