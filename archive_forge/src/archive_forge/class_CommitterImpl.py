...
class CommitterImpl(Committer, ConnectionReinitializer[StreamingCommitCursorRequest, StreamingCommitCursorResponse]):
    ...
    def __init__(self, initial: InitialCommitCursorRequest, flush_seconds: float, factory: ConnectionFactory[StreamingCommitCursorRequest, StreamingCommitCursorResponse]):
    ...
    def _handle_response(self, response: StreamingCommitCursorResponse):
        ...
        if response.commit.acknowledged_commits > len(self._outstanding_commits):
            self._connection.fail(FailedPrecondition('Received a commit response on the stream with no outstanding commits.'))
