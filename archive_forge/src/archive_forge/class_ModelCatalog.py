...
class ModelCatalog:
    ...
    @staticmethod
    @DeveloperAPI
    def get_preprocessor(env: gym.Env, options: Optional[dict]=None, include_multi_binary: bool=False) -> Preprocessor:
        """Returns a suitable preprocessor for the given env.

        This is a wrapper for get_preprocessor_for_space().
        """
        return ModelCatalog.get_preprocessor_for_space(env.observation_space, options, include_multi_binary)
    ...
    @staticmethod
    def _validate_config(config: ModelConfigDict, action_space: gym.spaces.Space, framework: str) -> None:
        ...
        if config.get('custom_preprocessor') is not None:
            deprecation_warning(old='model.custom_preprocessor', new='gym.ObservationWrapper around your env or handle complex inputs inside your Model', error=True)
        ...
        if (config.get('lstm_use_prev_action') or config.get('attention_use_n_prev_actions', 0) > 0) and (not config.get('_disable_action_flattening')) and isinstance(action_space, (Tuple, Dict)):
            raise ValueError("For your complex action space (Tuple|Dict) and your model's `prev-actions` setup of your model, you must set `_disable_action_flattening=True` in your main config dict!")
        if framework == 'jax':
