...
def _check_sample_weight_warning(self, x, sample_weight):
    sample_weight_present = sample_weight is not None or (isinstance(x, tf.data.Dataset) and isinstance(x.element_spec, tuple) and (len(x.element_spec) == 3))
    if sample_weight_present and self.compiled_metrics._user_weighted_metrics is None:
        logging.warning('`evaluate()` received a value for `sample_weight`, but `weighted_metrics` were not provided.  Did you mean to pass metrics to `weighted_metrics` in `compile()`?  If this is intentional you can pass `weighted_metrics=[]` to `compile()` in order to silence this warning.')