...
class CinderClientPlugin(os_client.ExtensionMixin, client_plugin.ClientPlugin):
    ...
    def _create(self):
        ...
        args = {'session': self.context.keystone_session, 'extensions': extensions, 'interface': self.interface, 'service_type': self.service_type, 'region_name': self._get_region_name(), 'connect_retries': cfg.CONF.client_retry_limit, 'http_log_debug': self._get_client_option(CLIENT_NAME, 'http_log_debug')}
    ...
    def get_volume_snapshot(self, snapshot):
        try:
            ...
            raise exception.EntityNotFound(entity='VolumeSnapshot', name=snapshot)

    def get_volume_backup(self, backup):
        try:
            return self.client().backups.get(backup)
        except exceptions.NotFound:
            raise exception.EntityNotFound(entity='Volume backup', name=backup)
    ...
    def get_volume_type(self, volume_type):
        ...
        for vt in volume_type_list:
            if volume_type in [vt.name, vt.id]:
                ...
                break
        if vt_id is None:
            raise exception.EntityNotFound(entity='VolumeType', name=volume_type)
        return vt_id
    ...
    def is_conflict(self, ex):
        return isinstance(ex, exceptions.ClientException) and ex.code == 409
    ...
    def check_detach_volume_complete(self, vol_id, server_id=None):
        try:
            ...
            return True
        server_ids = [a['server_id'] for a in vol.attachments if 'server_id' in a]
        ...
        if vol.status in ('in-use', 'detaching', 'reserved'):
            ...
            return False
        LOG.debug('Volume %(id)s - status: %(status)s', {'id': vol.id, 'status': vol.status})
        if vol.status not in ('available', 'deleting'):
            LOG.debug('Detachment failed - volume %(vol)s is in %(status)s status', {'vol': vol.id, 'status': vol.status})
            raise exception.ResourceUnknownStatus(resource_status=vol.status, result=_('Volume detachment failed'))
        else:
            return True
