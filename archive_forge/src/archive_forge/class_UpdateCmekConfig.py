...
class UpdateCmekConfig(base.Command):
    ...
    def Run(self, args):
        ...
        if args.clear_kms_key:
            ...
            project_id, full_kms_key_name, location_id = parsers.ParseKmsUpdateArgs(args)
        cmek_config = cmek_config_client.UpdateCmekConfig(project_id, location_id, full_kms_key_name, args.clear_kms_key)
        log.status.Print('Updated Cmek config for KMS key: [{}].'.format(parsers.GetConsolePromptString(full_kms_key_name)))
        return cmek_config