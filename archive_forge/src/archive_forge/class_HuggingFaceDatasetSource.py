...
class HuggingFaceDatasetSource(DatasetSource):
    ...
    def __init__(self, path: str, config_name: Optional[str]=None, data_dir: Optional[str]=None, data_files: Optional[Union[str, Sequence[str], Mapping[str, Union[str, Sequence[str]]]]]=None, split: Optional[Union[str, 'datasets.Split']]=None, revision: Optional[Union[str, 'datasets.Version']]=None):
    ...
    def load(self, **kwargs):
        ...
        """
        import datasets
        load_kwargs = {'path': self.path, 'name': self.config_name, 'data_dir': self.data_dir, 'data_files': self.data_files, 'split': self.split, 'revision': self.revision}
        intersecting_keys = set(load_kwargs.keys()) & set(kwargs.keys())
        if intersecting_keys:
            raise KeyError(f'Found duplicated arguments in `HuggingFaceDatasetSource` and `kwargs`: {intersecting_keys}. Please remove them from `kwargs`.')
    ...
    def to_dict(self) -> Dict[Any, Any]:
        return {'path': self.path, 'config_name': self.config_name, 'data_dir': self.data_dir, 'data_files': self.data_files, 'split': str(self.split), 'revision': self.revision}

    @classmethod
    def from_dict(cls, source_dict: Dict[Any, Any]) -> 'HuggingFaceDatasetSource':
        return cls(path=source_dict.get('path'), config_name=source_dict.get('config_name'), data_dir=source_dict.get('data_dir'), data_files=source_dict.get('data_files'), split=source_dict.get('split'), revision=source_dict.get('revision'))