...
class DirectoryLoader(BaseLoader):
    ...
    def __init__(self, path: str, glob: str='**/[!.]*', silent_errors: bool=False, load_hidden: bool=False, loader_cls: FILE_LOADER_TYPE=UnstructuredFileLoader, loader_kwargs: Union[dict, None]=None, recursive: bool=False, show_progress: bool=False, use_multithreading: bool=False, max_concurrency: int=4, *, exclude: Union[Sequence[str], str]=(), sample_size: int=0, randomize_sample: bool=False, sample_seed: Union[int, None]=None):
    ...
    def lazy_load(self) -> Iterator[Document]:
        ...
        items = [path for path in paths if not (self.exclude and any((path.match(glob) for glob in self.exclude))) and path.is_file()]
        ...
        if self.show_progress:
            try:
                ...
                if self.silent_errors:
                    ...
                    raise ImportError('To log the progress of DirectoryLoader you need to install tqdm, `pip install tqdm`')
        if self.use_multithreading:
            futures = []
            with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_concurrency) as executor:
                for i in items:
                    futures.append(executor.submit(self._lazy_load_file_to_non_generator(self._lazy_load_file), i, p, pbar))
