...
class RpcVRRPManager(app_manager.OSKenApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.CONF.register_opts([cfg.IntOpt('vrrp-rpc-port', default=VRRP_RPC_PORT, help='port for vrrp rpc interface')])
        self._args = args
        self._kwargs = kwargs
        self._peers = []
    ...
    def _config(self, msgid, params):
        ...
        if_params = self._params_to_dict(param_dict, ('primary_ip_address', 'device_name'))
        ...
        config_params = self._params_to_dict(param_dict, ('vrid', 'ip_addresses', 'version', 'admin_state', 'priority', 'advertisement_interval', 'preempt_mode', 'preempt_delay', 'statistics_interval'))
        try:
        ...
        api_result = [config_result.config.vrid, config_result.config.priority, str(netaddr.IPAddress(config_result.config.ip_addresses[0]))]
        return api_result
    ...
    def _list(self, msgid, params):
        ...
        for instance in instance_list:
            c = instance.config
            info_dict = {'instance_name': instance.instance_name, 'vrid': c.vrid, 'version': c.version, 'advertisement_interval': c.advertisement_interval, 'priority': c.priority, 'virtual_ip_address': str(netaddr.IPAddress(c.ip_addresses[0]))}
