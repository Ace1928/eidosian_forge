...
class ConfigPrinter(resource_printer_base.ResourcePrinter):
  ...
  """
    ...
    def __init__(self, *args, **kwargs):
        super(ConfigPrinter, self).__init__(*args, retain_none_values=True, **kwargs)
    ...
    """
        for name, value in sorted(items):
            ...
            if isinstance(value, dict):
                self._PrintEnvExport(six.iteritems(value), prefix=self._Prefix(prefix, name))
            elif value is None:
                ...
                for i, v in enumerate(value):
                    ...
                    self._PrintEnvExport(six.iteritems(v), prefix=self._Prefix(prefix, name))
            else:
                value = shlex.quote(six.text_type(value))
                self._out.write(self._env_command_format.format(name=prefix + name, value=value))
    ...
    """
        for name, value in sorted(items):
            name = six.text_type(name)
            if isinstance(value, dict):
                self._PrintEnvUnset(six.iteritems(value), prefix=self._Prefix(prefix, name))
            elif isinstance(value, list):
                for i, v in enumerate(value):
                    if not isinstance(v, dict):
                        v = {'I' + six.text_type(i): v}
                    self._PrintEnvUnset(six.iteritems(v), prefix=self._Prefix(prefix, name))
            else:
