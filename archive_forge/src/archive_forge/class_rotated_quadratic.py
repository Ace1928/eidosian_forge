...
class rotated_quadratic(_ConicBase):
    ...
    def check_convexity_conditions(self, relax=False):
        ...
        return (relax or (self.r1.is_continuous() and self.r2.is_continuous() and all((xi.is_continuous() for xi in self.x)))) and (self.r1.has_lb() and value(self.r1.lb) >= 0) and (self.r2.has_lb() and value(self.r2.lb) >= 0)