...
class SearchAlgorithm:
    ...
    def astar(self, start, end):
        ...
        # Calculate grid dimensions based on screen size and block size
        ...
        while len(open_list) > 0:
            ...
            for i in range(len(adjacent_squares)):
                ...
                # Check if the node position is within the grid boundaries
                if (
                    0 <= node_position[0] < grid_width
                    and 0 <= node_position[1] < grid_height
                ):
