...
class TiffFormat(Format):
    ...
    class Reader(Format.Reader):
        ...
        def _get_meta_data(self, index):
            ...
            for key in ('datetime', 'DateTime'):
                try:
                    meta['datetime'] = datetime.datetime.strptime(page.tags[key].value, '%Y:%m:%d %H:%M:%S')
                    break
                except Exception:
                    pass
            if 296 in page.tags:
            ...
            if 282 in page.tags and 283 in page.tags and (296 in page.tags):
                ...
                if resolution_x[1] == 0 or resolution_y[1] == 0:
                    warnings.warn('Ignoring resolution metadata, because at least one direction has a 0 denominator.', RuntimeWarning)
                else:
                    meta['resolution'] = (resolution_x[0] / resolution_x[1], resolution_y[0] / resolution_y[1], page.tags[296].value.name)
            return meta
    ...
    class Writer(Format.Writer):
        ...
        def _open(self, bigtiff=None, byteorder=None, software=None):
            try:
                self._tf = _tifffile.TiffWriter(self.request.get_file(), bigtiff=bigtiff, byteorder=byteorder, software=software)
                self._software = None
            except TypeError:
                self._tf = _tifffile.TiffWriter(self.request.get_file(), bigtiff=bigtiff, byteorder=byteorder)
