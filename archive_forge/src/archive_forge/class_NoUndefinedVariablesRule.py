    ...
    def leave_operation_definition(self, operation: OperationDefinitionNode, *_args: Any) -> None:
        ...
        for usage in usages:
            ...
            if var_name not in defined_variables:
                self.report_error(GraphQLError(f"Variable '${var_name}' is not defined by operation '{operation.name.value}'." if operation.name else f"Variable '${var_name}' is not defined.", [node, operation]))
