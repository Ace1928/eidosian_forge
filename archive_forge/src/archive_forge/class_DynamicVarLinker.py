...
class DynamicVarLinker(object):
    ...
    def __init__(self, source_variables, target_variables, source_time=None, target_time=None):
        if len(source_variables) != len(target_variables):
            raise ValueError('%s must be provided two lists of time-indexed variables of equal length. Got lengths %s and %s' % (type(self), len(source_variables), len(target_variables)))
    ...
    def transfer(self, t_source=None, t_target=None):
        if t_source is None and self._source_time is None:
            raise RuntimeError('Source time points were not provided in the transfer method or in the constructor.')
        elif t_source is None:
            t_source = self._source_time
        if t_target is None and self._target_time is None:
            raise RuntimeError('Target time points were not provided in the transfer method or in the constructor.')
        elif t_target is None:
            t_target = self._target_time
        copy_values_at_time(self._source_variables, self._target_variables, t_source, t_target)