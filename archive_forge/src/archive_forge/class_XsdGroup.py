...
class XsdGroup(XsdComponent, MutableSequence[ModelParticleType], ParticleMixin, ValidationMixin[ElementType, GroupDecodeType]):
    """
    ...
    interleave: Optional[Xsd11AnyElement] = None
    suffix: Optional[Xsd11AnyElement] = None
    _ADMITTED_TAGS = {XSD_GROUP, XSD_SEQUENCE, XSD_ALL, XSD_CHOICE}

    def __init__(self, elem: ElementType, schema: SchemaType, parent: Optional[Union['XsdComplexType', 'XsdGroup']]=None) -> None:
    ...
    def __repr__(self) -> str:
        if self.name is None:
            return '%s(model=%r, occurs=%r)' % (self.__class__.__name__, self.model, list(self.occurs))
        elif self.ref is None:
            return '%s(name=%r, model=%r, occurs=%r)' % (self.__class__.__name__, self.prefixed_name, self.model, list(self.occurs))
        else:
            return '%s(ref=%r, model=%r, occurs=%r)' % (self.__class__.__name__, self.prefixed_name, self.model, list(self.occurs))
    ...
    def __getitem__(self, i: Union[int, slice]) -> Union[ModelParticleType, MutableSequence[ModelParticleType]]:
    ...
    def has_occurs_restriction(self, other: Union[ModelParticleType, ParticleMixin, 'OccursCalculator']) -> bool:
        ...
        if self.max_occurs is None or any((e.max_occurs is None for e in self)):
            ...
            if self.min_occurs * min((e.min_occurs for e in self)) < other.min_occurs:
                ...
                value: int
                try:
                    value = max((e.max_occurs for e in self))
                except TypeError:
                    return False
                else:
                    return self.max_occurs * value <= other.max_occurs
        elif self.min_occurs * sum((e.min_occurs for e in self)) < other.min_occurs:
            return False
        elif other.max_occurs is None:
            return True
        else:
            try:
                value = sum((e.max_occurs for e in self))
            except TypeError:
                return False
            else:
                return self.max_occurs * value <= other.max_occurs
    ...
    def iter_components(self, xsd_classes: Optional[ComponentClassType]=None) -> Iterator[XsdComponent]:
    ...
    def is_restriction(self, other: ModelParticleType, check_occurs: bool=True) -> bool:
        ...
        if self.model != other.model and self.model != 'sequence' and (len(self) > 1 or (self.ref is not None and len(self[0]) > 1)):
            return False
    ...
    def iter_decode(self, obj: ElementType, validation: str='lax', **kwargs: Any) -> IterDecodeType[GroupDecodeType]:
        """
        ...
        :return: yields a list of 3-tuples (key, decoded data, decoder),         eventually preceded by a sequence of validation or decoding errors.
        """
        ...
        if not self._group and self.model == 'choice' and self.min_occurs:
            ...
            return
        if not self.mixed:
            if obj.text and obj.text.strip() or any((child.tail and child.tail.strip() for child in obj)):
        ...
        for index, child in enumerate(obj):
            ...
            while model.element is not None:
                if model.element.max_occurs == 0:
                    ...
                    xsd_element = model.element.match(child.tag, group=self, occurs=model.occurs)
                if xsd_element is None:
                    if self.interleave is not None and self.interleave.is_matching(child.tag, default_namespace, self, model.occurs):
        ...
        if errors:
            ...
            for index, particle, occurs, expected in errors:
                error = XMLSchemaChildrenValidationError(self, obj, index, particle, occurs, expected, source, namespaces)
                if validation == 'strict':
                    raise error
                yield error
        yield result_list

    def iter_encode(self, obj: ElementData, validation: str='lax', **kwargs: Any) -> IterEncodeType[GroupEncodeType]:
        """
        ...
        :return: yields a couple with the text of the Element and a list of child         elements, eventually preceded by a sequence of validation errors.
        """
            ...
            if self.interleave and self.interleave.is_matching(name, default_namespace, group=self):
                ...
                while model.element is not None:
                    if model.element.max_occurs == 0:
                        ...
                        xsd_element = model.element.match(name, group=self, occurs=model.occurs)
                    if xsd_element is None:
                    ...
                    if self.suffix and self.suffix.is_matching(name, default_namespace, group=self):
                        ...
                        if isinstance(xsd_element, XsdAnyElement):
                            ...
                            if name.startswith('{') or ':' not in name:
                                reason = _('{!r} does not match any declared element of the model group').format(name)
                            else:
                                reason = _('{0} has an unknown prefix {1!r}').format(name, name.split(':')[0])
        ...
        cdata_not_allowed = not self.mixed and text and text.strip() and self and (len(self) > 1 or not isinstance(self[0], XsdAnyElement))
        if errors or cdata_not_allowed or wrong_content_type:
            ...
            if cdata_not_allowed:
                ...
                yield self.validation_error(validation, reason, elem, **kwargs)
            for index, particle, occurs, expected in errors:
                error = XMLSchemaChildrenValidationError(validator=self, elem=elem, index=index, particle=particle, occurs=occurs, expected=expected, namespaces=converter.namespaces)
                if validation == 'strict':
                    raise error
                error.elem = None
                yield error
