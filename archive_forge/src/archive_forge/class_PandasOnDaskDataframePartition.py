...
class PandasOnDaskDataframePartition(PandasDataframePartition):
    ...
    def apply(self, func, *args, **kwargs):
        ...
        **kwargs : dict
        ...
        if len(call_queue) > 1:
            ...
            futures = self.execution_wrapper.deploy(func=apply_list_of_funcs, f_args=(call_queue, self._data), num_returns=2, pure=False)
        else:
            func, f_args, f_kwargs = call_queue[0]
            futures = self.execution_wrapper.deploy(func=apply_func, f_args=(self._data, func, *f_args), f_kwargs=f_kwargs, num_returns=2, pure=False)
    ...
    def mask(self, row_labels, col_labels):
        ...
        if isinstance(row_labels, slice) and isinstance(self._length_cache, Future):
            if row_labels == slice(None):
                ...
                new_obj._length_cache = self.execution_wrapper.deploy(func=compute_sliced_len, f_args=(row_labels, self._length_cache))
        if isinstance(col_labels, slice) and isinstance(self._width_cache, Future):
            if col_labels == slice(None):
                ...
                new_obj._width_cache = self.execution_wrapper.deploy(func=compute_sliced_len, f_args=(col_labels, self._width_cache))
    ...
    def __copy__(self):
        ...
        """
        return self.__constructor__(self._data, length=self._length_cache, width=self._width_cache, ip=self._ip_cache, call_queue=self.call_queue)
    ...
    def put(cls, obj):
        ...
        PandasOnDaskDataframePartition
            A new ``PandasOnDaskDataframePartition`` object.
        """
        return cls(cls.execution_wrapper.put(obj, hash=False), len(obj.index), len(obj.columns))
