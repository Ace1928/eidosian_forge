...
class OnTraitChangeTest(unittest.TestCase):
    ...
    def test_list1(self):
        ...
        for i in range(3):
            ac = ArgCheckBase()
            l1.trait_set(exp_object=l1, exp_name='refs_items', type_old=None, exp_old=Undefined, type_new=TraitListEvent)
            l1.refs.append(ac)
        self.assertEqual(l1.calls, {0: 3, 3: 0, 4: 0}, 'Behavior of a bug (#538) is not reproduced.')
        for i in range(3):
            self.assertEqual(l1.refs[i].value, 0)
        l1.reset_traits(['calls'])
        refs = [ArgCheckBase(), ArgCheckBase(), ArgCheckBase()]
        l1.trait_set(exp_object=l1, exp_name='refs', type_old=None, exp_old=l1.refs, type_new=TraitListObject)
        l1.refs = refs
        ...
        for i in range(3):
            for j in range(3):
                l1.trait_set(exp_object=l1.refs[j], exp_name='value', type_old=None, exp_old=i, type_new=None, exp_new=i + 1)
                l1.refs[j].value = i + 1
    ...
    def test_pattern_list3(self):
        c = Complex(tc=self)
        self.check_complex(c, c.ref, 'ref.[int1, int2, int3]', ['int1', 'int2', 'int3'], ['tint1', 'tint2', 'tint3'])
    ...
    def test_pattern_list7(self):
        c = Complex(tc=self)
        self.check_complex(c, c, '+test', ['int1', 'int3', 'tint2'], ['int2', 'tint1', 'tint3'])
    ...
    def check_complex(self, c, r, pattern, names, other=[]):
        handlers = [c.arg_check0, c.arg_check1, c.arg_check2, c.arg_check3, c.arg_check4]
        nh = len(handlers)
        nn = len(names)
        self.multi_register(c, handlers, pattern)
        for i in range(3):
            for n in names:
                c.trait_set(exp_object=r, exp_name=n, exp_old=i, exp_new=i + 1)
                setattr(r, n, i + 1)
            for n in other:
                c.trait_set(exp_object=r, exp_name=n, exp_old=i, exp_new=i + 1)
                setattr(r, n, i + 1)
