    ...
    class Tests(TestsBase):
        ...
        def test_is_in(self):
            ...
            def assert_eq(df, col, positive, collection, expected):
                ...
                self.assert_column_eq(self.op.is_in(df[col], *comp, positive=positive), expected)
        ...
        def test_is_between(self):
            ...
            def assert_eq(df, col, lower, upper, positive, expected):
                self.assert_column_eq(self.op.is_between(df[col], df[lower], df[upper], positive=positive), expected)
            df = self.to_df([[0, 1, 1], [2, 3, 4], [None, 1, 1], [1, None, 1], [1, 1, None]], ['a', 'b', 'c'])
        ...
        def test_case_when(self):
            ...
            a = self.to_df([[False, False, 1], [False, True, 2], [True, False, 3], [True, True, 4]], ['c1', 'c2', 'v'])
        ...
        def test_union(self):
            ...
            assert_eq(a, b, False, [['x', 'a'], ['x', 'a'], [None, None], ['xx', 'aa'], [None, None], ['a', 'x']], ['a', 'b'])
            assert_eq(a, b, True, [['x', 'a'], ['xx', 'aa'], [None, None], ['a', 'x']], ['a', 'b'])
        ...
        def test_intersect(self):
            ...
            def assert_eq(df1, df2, unique, expected, expected_cols):
                ...
                assert_df_eq(res, expected, expected_cols)
            a = self.to_df([['x', 'a'], ['x', 'a'], [None, None]], ['a', 'b'])
            b = self.to_df([['xx', 'aa'], [None, None], [None, None], ['a', 'x']], ['b', 'a'])
        ...
        def test_joins(self):
            ...
            def assert_eq(df1, df2, join_type, on, expected, expected_cols):
                res = self.to_pandas_df(self.op.join(df1, df2, join_type=join_type, on=on))
                assert_df_eq(res, expected, expected_cols)
            ...
            assert_eq(df1, df2, 'left_outer', ['a'], [[0, 1, 10], [2, 3, None]], ['a', 'b', 'c'])
            assert_eq(df3, df4, 'left_outer', ['a'], [[0, 1, 10], [None, 3, None]], ['a', 'b', 'c'])
            assert_eq(df1, df2, 'right_outer', ['a'], [[0, 1, 10], [20, None, 30]], ['a', 'b', 'c'])
            assert_eq(df3, df4, 'right_outer', ['a'], [[0, 1, 10], [None, None, 30]], ['a', 'b', 'c'])
            assert_eq(df1, df2, 'full_outer', ['a'], [[0, 1, 10], [2, 3, None], [20, None, 30]], ['a', 'b', 'c'])
            assert_eq(df3, df4, 'full_outer', ['a'], [[0, 1, 10], [None, 3, None], [None, None, 30]], ['a', 'b', 'c'])
            df1 = self.to_df([[0, 1], [None, 3]], ['a', 'b'])
            df2 = self.to_df([[0, 10], [None, 30]], ['c', 'd'])
            assert_eq(df1, df2, 'cross', [], [[0, 1, 0, 10], [None, 3, 0, 10], [0, 1, None, 30], [None, 3, None, 30]], ['a', 'b', 'c', 'd'])
        ...
        def test_sort(self):
            ...
            def assert_eq(df, index, *order_by):
                ...
                self.w_eq(df, ob(*order_by), [], 'x', e.values.tolist(), list(e.columns))
            df = self.to_df([[0, 10, None], [1, None, 100], [None, 20, 300]], list('abc'))
            assert_eq(df, [1, 2, 3])
            ...
            df = self.to_df([[0, 10, None], [0, 5, None], [0, None, 100], [None, 20, 300]], list('abc'))
        ...
        def test_agg_first_last(self):
            ...
            assert_eq(pdf, ['a'], {'b1': ('b', AggFunctionSpec('first', unique=False, dropna=False)), 'b2': ('b', AggFunctionSpec('first', unique=False, dropna=True)), 'b3': ('b', AggFunctionSpec('last', unique=False, dropna=False)), 'b4': ('b', AggFunctionSpec('last', unique=False, dropna=True))}, [[1.0, 1.0, None, 1.0], [None, 5.0, 5.0, 5.0]], ['b1', 'b2', 'b3', 'b4'])
        ...
        def test_agg_min_max(self):
            ...
            assert_eq(pdf, ['a'], {'b1': ('b', AggFunctionSpec('min', unique=False, dropna=False)), 'b2': ('b', AggFunctionSpec('min', unique=True, dropna=False)), 'b3': ('b', AggFunctionSpec('min', unique=True, dropna=True)), 'b4': ('b', AggFunctionSpec('min', unique=False, dropna=True))}, [[1.0, 1.0, 1.0, 1.0], [None, None, None, None]], ['b1', 'b2', 'b3', 'b4'])
            assert_eq(pdf, ['a'], {'b1': ('b', AggFunctionSpec('max', unique=False, dropna=False)), 'b2': ('b', AggFunctionSpec('max', unique=True, dropna=False)), 'b3': ('b', AggFunctionSpec('max', unique=True, dropna=True)), 'b4': ('b', AggFunctionSpec('max', unique=False, dropna=True))}, [[2.0, 2.0, 2.0, 2.0], [None, None, None, None]], ['b1', 'b2', 'b3', 'b4'])
            pdf = self.to_df([[0, 'a'], [0, 'a'], [0, 'b'], [1, None], [0, None]], ['a', 'b'])
            assert_eq(pdf, ['a'], {'b1': ('b', AggFunctionSpec('min', unique=False, dropna=False)), 'b2': ('b', AggFunctionSpec('min', unique=True, dropna=False)), 'b3': ('b', AggFunctionSpec('min', unique=True, dropna=True)), 'b4': ('b', AggFunctionSpec('min', unique=False, dropna=True))}, [['a', 'a', 'a', 'a'], [None, None, None, None]], ['b1', 'b2', 'b3', 'b4'])
            assert_eq(pdf, ['a'], {'b1': ('b', AggFunctionSpec('max', unique=False, dropna=False)), 'b2': ('b', AggFunctionSpec('max', unique=True, dropna=False)), 'b3': ('b', AggFunctionSpec('max', unique=True, dropna=True)), 'b4': ('b', AggFunctionSpec('max', unique=False, dropna=True))}, [['b', 'b', 'b', 'b'], [None, None, None, None]], ['b1', 'b2', 'b3', 'b4'])
        ...
        def test_window_sum_partition_by(self):
            ...
            def make_func(partition_keys, order_by, start=None, end=None):
                ...
                return WindowFunctionSpec('sum', False, False, ws)
            a = self.to_df([[0, 1], [None, 1], [0, None], [0, 2]], ['a', 'b'])
            self.w_eq(a, make_func(['a'], []), [col('b')], 'x', [[0, 1, 3], [None, 1, 1], [0, None, 3], [0, 2, 3]], ['a', 'b', 'x'])
            self.w_eq(a, make_func(['a'], ['b', 'a'], end=0), [col('b')], 'x', [[0, 1, 1], [None, 1, 1], [0, None, None], [0, 2, 3]], ['a', 'b', 'x'])
            self.w_eq(a, make_func(['a'], ['b', 'a'], start=-1, end=0), [col('b')], 'x', [[0, 1, 1], [None, 1, 1], [0, None, None], [0, 2, 3]], ['a', 'b', 'x'])
            self.w_eq(a, make_func(['a'], ['b', 'a'], start=0, end=0), [col('b')], 'x', [[0, 1, 1], [None, 1, 1], [0, None, None], [0, 2, 2]], ['a', 'b', 'x'])
            if not pd.__version__.startswith('1.1'):
                self.w_eq(a, make_func(['a'], ['b'], end=-1), [col('b')], 'x', [[0, 1, None], [None, 1, None], [0, None, None], [0, 2, 1]], ['a', 'b', 'x'])
                self.w_eq(a, make_func(['a'], ['b'], start=-1, end=-1), [col('b')], 'x', [[0, 1, None], [None, 1, None], [0, None, None], [0, 2, 1]], ['a', 'b', 'x'])
                self.w_eq(a, make_func(['a'], ['b'], start=1, end=None), [col('b')], 'x', [[0, 1, 2], [None, 1, None], [0, None, 3], [0, 2, None]], ['a', 'b', 'x'])
