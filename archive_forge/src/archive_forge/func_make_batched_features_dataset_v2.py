...
def make_batched_features_dataset_v2(file_pattern, batch_size, features, reader=None, label_key=None, reader_args=None, num_epochs=None, shuffle=True, shuffle_buffer_size=10000, shuffle_seed=None, prefetch_buffer_size=None, reader_num_threads=None, parser_num_threads=None, sloppy_ordering=False, drop_final_batch=False):
  ...
  """
    ...
    dataset = dataset_ops.Dataset.list_files(file_pattern, shuffle=shuffle, seed=shuffle_seed)
    if isinstance(reader, type) and issubclass(reader, io_ops.ReaderBase):
        raise TypeError('The `reader` argument must return a `Dataset` object. `tf.ReaderBase` subclasses are not supported. For example, pass `tf.data.TFRecordDataset` instead of `tf.TFRecordReader`.')
    if reader_args is None:
        reader_args = []
    if reader_num_threads == dataset_ops.AUTOTUNE:
        dataset = dataset.interleave(lambda filename: reader(filename, *reader_args), num_parallel_calls=reader_num_threads)
    ...
    if dataset_ops.get_legacy_output_types(dataset) == (dtypes.string, dtypes.string):
        dataset = map_op._MapDataset(dataset, lambda _, v: v, use_inter_op_parallelism=False)
    ...
    if label_key:
        if label_key not in features:
            raise ValueError(f'The `label_key` provided ({label_key}) must be one of the `features` keys: {features.keys()}.')
