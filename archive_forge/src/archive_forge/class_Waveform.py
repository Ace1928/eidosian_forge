...
class Waveform(Pulse):
    ...
    def __init__(self, samples: np.ndarray | list[complex], name: str | None=None, epsilon: float=1e-07, limit_amplitude: bool | None=None):
    ...
    def _clip(self, samples: np.ndarray, epsilon: float=1e-07) -> np.ndarray:
        ...
        if np.any(to_clip):
            ...
            if np.any(to_clip_epsilon):
                ...
                clipped_samples_epsilon = (1 - epsilon) * np.exp(1j * clip_angle[clip_where_epsilon], dtype=np.complex128)
        ...
        if np.any(samples_norm > 1.0) and self._limit_amplitude:
            ...
            raise PulseError(f'Pulse contains sample with norm {amp} greater than 1+epsilon. This can be overruled by setting Pulse.limit_amplitude.')
        return samples
    ...
    def __eq__(self, other: object) -> bool:
        ...
        return super().__eq__(other) and self.samples.shape == other.samples.shape and np.allclose(self.samples, other.samples, rtol=0, atol=self.epsilon)
    ...
    def __repr__(self) -> str:
        ...
        return '{}({}{})'.format(self.__class__.__name__, repr(self.samples), f", name='{self.name}'" if self.name is not None else '')