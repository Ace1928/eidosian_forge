...
class AzureRMEventHubInfo(AzureRMModuleBase):
    ...
    def __init__(self):
        self.module_arg_spec = dict(resource_group=dict(type='str', required=True), namespace_name=dict(type='str', required=False), name=dict(type='str', required=False))
        self.results = dict(changed=False)
        self.resource_group = None
        self.namespace_name = None
        self.name = None
        self.tags = None
        super(AzureRMEventHubInfo, self).__init__(self.module_arg_spec, supports_check_mode=True, supports_tags=False)
    ...
    def exec_module(self, **kwargs):
        ...
        if self.name and self.namespace_name and self.resource_group:
            ...
            self.results['eventhub'] = [self.event_hub_to_dict(x) for x in results]
    ...
    def get_namespace(self):
        ...
        results = []
        try:
            response = self.event_hub_client.namespaces.get(self.resource_group, self.namespace_name)
    ...
    def list_all_namespace(self):
        ...
        try:
            response = self.event_hub_client.namespaces.list_by_resource_group(self.resource_group)
