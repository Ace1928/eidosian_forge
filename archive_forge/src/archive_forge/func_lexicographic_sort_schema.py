...
def lexicographic_sort_schema(schema: GraphQLSchema) -> GraphQLSchema:
    ...
    def replace_type(type_: Union[GraphQLList, GraphQLNonNull, GraphQLNamedType]) -> Union[GraphQLList, GraphQLNonNull, GraphQLNamedType]:
    ...
    def sort_args(args_map: Dict[str, GraphQLArgument]) -> Dict[str, GraphQLArgument]:
        ...
        for name, arg in sorted(args_map.items()):
            args[name] = GraphQLArgument(**merge_kwargs(arg.to_kwargs(), type_=replace_type(cast(GraphQLNamedType, arg.type))))
        return args

    def sort_fields(fields_map: Dict[str, GraphQLField]) -> Dict[str, GraphQLField]:
        fields = {}
        for name, field in sorted(fields_map.items()):
            fields[name] = GraphQLField(**merge_kwargs(field.to_kwargs(), type_=replace_type(cast(GraphQLNamedType, field.type)), args=sort_args(field.args)))
        return fields
    ...
    def sort_named_type(type_: GraphQLNamedType) -> GraphQLNamedType:
        ...
        if is_object_type(type_):
            ...
            return GraphQLObjectType(**merge_kwargs(type_.to_kwargs(), interfaces=lambda: sort_types(type_.interfaces), fields=lambda: sort_fields(type_.fields)))
        if is_interface_type(type_):
            type_ = cast(GraphQLInterfaceType, type_)
            return GraphQLInterfaceType(**merge_kwargs(type_.to_kwargs(), interfaces=lambda: sort_types(type_.interfaces), fields=lambda: sort_fields(type_.fields)))
        if is_union_type(type_):
            type_ = cast(GraphQLUnionType, type_)
            return GraphQLUnionType(**merge_kwargs(type_.to_kwargs(), types=lambda: sort_types(type_.types)))
        if is_enum_type(type_):
            type_ = cast(GraphQLEnumType, type_)
            return GraphQLEnumType(**merge_kwargs(type_.to_kwargs(), values={name: GraphQLEnumValue(val.value, description=val.description, deprecation_reason=val.deprecation_reason, ast_node=val.ast_node) for name, val in sorted(type_.values.items())}))
        if is_input_object_type(type_):
            type_ = cast(GraphQLInputObjectType, type_)
            return GraphQLInputObjectType(**merge_kwargs(type_.to_kwargs(), fields=lambda: sort_input_fields(type_.fields)))
        raise TypeError(f'Unexpected type: {inspect(type_)}.')
    type_map: Dict[str, GraphQLNamedType] = {type_.name: sort_named_type(type_) for type_ in sorted(schema.type_map.values(), key=sort_by_name_key)}
    return GraphQLSchema(types=type_map.values(), directives=[sort_directive(directive) for directive in sorted(schema.directives, key=sort_by_name_key)], query=cast(Optional[GraphQLObjectType], replace_maybe_type(schema.query_type)), mutation=cast(Optional[GraphQLObjectType], replace_maybe_type(schema.mutation_type)), subscription=cast(Optional[GraphQLObjectType], replace_maybe_type(schema.subscription_type)), ast_node=schema.ast_node)