...
class DataForSeoAPIWrapper(BaseModel):
    ...
    default_params: dict = Field(default={'location_name': 'United States', 'language_code': 'en', 'depth': 10, 'se_name': 'google', 'se_type': 'organic'})
    ...
    def _prepare_request(self, keyword: str) -> dict:
        ...
        data = [obj]
        _url = f'https://api.dataforseo.com/v3/serp/{obj['se_name']}/{obj['se_type']}/live/advanced'
    ...
    async def _aresponse_json(self, url: str) -> dict:
        ...
        if not self.aiosession:
            async with aiohttp.ClientSession() as session:
                async with session.post(request_details['url'], headers=request_details['headers'], json=request_details['data']) as response:
                    res = await response.json()
        else:
            async with self.aiosession.post(request_details['url'], headers=request_details['headers'], json=request_details['data']) as response:
                res = await response.json()
