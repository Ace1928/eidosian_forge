...
@ddt.ddt
class RBDConnectorTestCase(test_base_rbd.RBDConnectorTestMixin, test_connector.ConnectorTestCase):
    ...
    def test_get_connector_properties(self):
        props = rbd.RBDConnector.get_connector_properties('sudo', multipath=True, enforce_multipath=True)
    ...
    def test_connect_volume(self, mock_path, mock_conf, mock_rados, mock_rbd):
        ...
        mock_rados.Rados.assert_called_once_with(clustername=self.clustername, rados_id=utils.convert_str(self.user), conffile='/tmp/fake_dir/fake_ceph.conf')
        self.assertEqual(1, mock_rados.Rados.return_value.connect.call_count)
        mock_rados.Rados.return_value.open_ioctx.assert_called_once_with(utils.convert_str(self.pool))
        mock_rbd.Image.assert_called_once_with(mock_rados.Rados.return_value.open_ioctx.return_value, utils.convert_str(self.volume), read_only=False, snapshot=None)
        self.assertIsInstance(device_info['path'], linuxrbd.RBDVolumeIOWrapper)
    ...
    def test_create_non_openstack_config_in_openstack(self, mock_priv_create):
        ...
        res = rbd.RBDConnector.create_non_openstack_config(connection_properties)
    ...
    def test_connect_local_volume_without_mons(self, mock_execute):
        rbd_connector = rbd.RBDConnector(None, do_local_attach=True)
        conn = {'name': 'pool/image', 'auth_username': 'fake_user'}
        device_info = rbd_connector.connect_volume(conn)
        execute_call1 = mock.call('which', 'rbd')
    ...
    @mock.patch.object(rbd.RBDConnector, 'create_non_openstack_config')
    @mock.patch.object(rbd.RBDConnector, '_execute')
    def test__local_attach_volume_fail_non_openstack(self, mock_execute, mock_rbd_cfg, mock_args, mock_delete):
        mock_args.return_value = [mock.sentinel.rbd_args]
        mock_execute.side_effect = [None, ValueError]
        connector = rbd.RBDConnector(None, do_local_attach=True)
        self.assertRaises(ValueError, connector._local_attach_volume, self.connection_properties)
        mock_rbd_cfg.assert_called_once_with(self.connection_properties)
        mock_args.assert_called_once_with(self.connection_properties, mock_rbd_cfg.return_value)
        self.assertEqual(2, mock_execute.call_count)
        mock_execute.assert_has_calls([mock.call('which', 'rbd'), mock.call('rbd', 'map', 'fake_volume', '--pool', 'fake_pool', mock.sentinel.rbd_args, root_helper=connector._root_helper, run_as_root=True)])
    ...
    def test_disconnect_volume(self, volume_close, mock_rados, mock_rbd):
        ...
        rbd_connector = rbd.RBDConnector(None)
        device_info = rbd_connector.connect_volume(self.connection_properties)
        rbd_connector.disconnect_volume(self.connection_properties, device_info)
    ...
    @mock.patch.object(rbd, 'open')
    def test_check_valid_device_handle_no_path(self, mock_open, check_path, check_device):
        connector = rbd.RBDConnector(None)
    ...
    @mock.patch('os_brick.privileged.rbd.check_valid_path')
    @mock.patch.object(rbd, 'open')
    def test_check_valid_device_block_non_root(self, mock_open, check_path, check_device):
        connector = rbd.RBDConnector(None)
        path = '/dev/rbd0'
