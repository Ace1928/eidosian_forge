...
class PartitionCountWatcherImpl(PartitionCountWatcher, PermanentFailable):
    ...
    def __init__(self, admin: AdminClientInterface, topic_path: TopicPath, duration: float):
        super().__init__()
        self._admin = admin
    ...
    async def __aenter__(self):
        ...
        self._partition_loop_poller = asyncio.ensure_future(self.run_poller(self._poll_partition_loop))
    ...
    async def _poll_partition_loop(self):
        try:
            partition_count = await asyncio.get_event_loop().run_in_executor(self._thread, self._get_partition_count_sync)
