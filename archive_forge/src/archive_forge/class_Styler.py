...
class Styler(StylerRenderer):
    ...
    *{"row": "MY_ROW_CLASS", "col_trim": "", "row_trim": ""}*.
    ...
    def _copy(self, deepcopy: bool=False) -> Styler:
        ...
        deep = ['css', 'concatenated', '_display_funcs', '_display_funcs_index', '_display_funcs_columns', 'hidden_rows', 'hidden_columns', 'ctx', 'ctx_index', 'ctx_columns', 'cell_context', '_todo', 'table_styles', 'tooltips']
        for attr in shallow:
        ...
        if data.empty:
            ...
            if not isinstance(result, DataFrame):
                if not isinstance(result, np.ndarray):
                    raise TypeError(f'Function {repr(func)} must return a DataFrame or ndarray when passed to `Styler.apply` with axis=None')
                if data.shape != result.shape:
                    raise ValueError(f'Function {repr(func)} returned ndarray with wrong shape.\nResult has shape: {result.shape}\nExpected shape: {data.shape}')
    ...
    def applymap(self, func: Callable, subset: Subset | None=None, **kwargs) -> Styler:
        ...
        **kwargs : dict
        ...
        Styler
        """
        warnings.warn('Styler.applymap has been deprecated. Use Styler.map instead.', FutureWarning, stacklevel=find_stack_level())
