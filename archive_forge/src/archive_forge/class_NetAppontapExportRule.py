...
class NetAppontapExportRule:
    ...
    def __init__(self):
        ...
        partially_supported_rest_properties = [['ntfs_unix_security', (9, 9, 1)], ['allow_suid', (9, 9, 1)], ['allow_device_creation', (9, 9, 1)], ['chown_mode', (9, 9, 1)]]
    ...
    def set_playbook_zapi_key_map(self):
        self.na_helper.zapi_string_keys = {'anonymous_user_id': 'anonymous-user-id', 'client_match': 'client-match', 'name': 'policy-name', 'ntfs_unix_security': 'export-ntfs-unix-security-ops', 'chown_mode': 'export-chown-mode'}
        self.na_helper.zapi_list_keys = {'protocol': ('protocol', 'access-protocol'), 'ro_rule': ('ro-rule', 'security-flavor'), 'rw_rule': ('rw-rule', 'security-flavor'), 'super_user_security': ('super-user-security', 'security-flavor')}
        self.na_helper.zapi_bool_keys = {'allow_suid': 'is-allow-set-uid-enabled', 'allow_device_creation': 'is-allow-dev-is-enabled'}
    ...
    def apply(self):
        ...
        if cd_action == 'create' and self.parameters.get('rule_index'):
            ...
            if rename is None and self.parameters.get('from_rule_index') is not None:
                self.module.fail_json(msg='Error reindexing: export policy rule %s does not exist.' % self.parameters['from_rule_index'])
            if rename:
