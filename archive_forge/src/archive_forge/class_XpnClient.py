...
class XpnClient(object):
  ...
  """
    ...
    def _MakeRequest(self, request, errors):
        return self.compute_client.MakeRequests(requests=[request], errors_to_collect=errors)
    ...
    """
        ...
        self._EnableXpnAssociatedResource(host_project, associated_project, xpn_resource_type=xpn_types.PROJECT)

    def _DisableXpnAssociatedResource(self, host_project, associated_resource, xpn_resource_type):
    ...
    Args:
      ...
      xpn_resource_type: XpnResourceId.TypeValueValuesEnum, the type of the
         resource
    """
        projects_disable_request = self.messages.ProjectsDisableXpnResourceRequest(xpnResource=self.messages.XpnResourceId(id=associated_resource, type=xpn_resource_type))
        request = self.messages.ComputeProjectsDisableXpnResourceRequest(project=host_project, projectsDisableXpnResourceRequest=projects_disable_request)
        request_tuple = (self.client.projects, 'DisableXpnResource', request)
        msg = 'disable resource [{0}] as an associated resource for project [{1}]'.format(associated_resource, host_project)
        self._MakeRequestSync(request_tuple, msg)
