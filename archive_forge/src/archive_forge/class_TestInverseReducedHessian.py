...
class TestInverseReducedHessian(unittest.TestCase):
    ...
    def _simple_model(self, add_constraint=False):
        data = pd.DataFrame([[1, 1.1, 0.365759306], [2, 1.2, 4], [3, 1.3, 4.8876684], [4, 1.4, 5.173455561], [5, 1.5, 2.093799081], [6, 1.6, 9], [7, 1.7, 6.475045106], [8, 1.8, 8.127111268], [9, 1.9, 6], [10, 1.21, 10.20642714], [11, 1.22, 13.08211636], [12, 1.23, 10], [13, 1.24, 15.38766047], [14, 1.25, 14.6587746], [15, 1.26, 13.68608604], [16, 1.27, 14.70707893], [17, 1.28, 18.46192779], [18, 1.29, 15.60649164]], columns=['tofu', 'chard', 'y'])
        ...
        def SSE_rule(m):
            return sum(((data.y[i] - m.response_function[data.tofu[i], data.chard[i]]) ** 2 for i in data.index))
