...
class TPOTBase(BaseEstimator):
        ...
        if scoring:
            if isinstance(scoring, str):
                if scoring not in SCORERS:
                    raise ValueError('The scoring function {} is not available. Please choose a valid scoring function from the TPOT documentation.'.format(scoring))
                self.scoring_function = scoring
            elif callable(scoring):
                module = getattr(scoring, '__module__', None)
                args_list = inspect.getfullargspec(scoring)[0]
                if args_list == ['y_true', 'y_pred'] or (hasattr(module, 'startswith') and (module.startswith('sklearn.metrics.') or module.startswith('tpot.metrics')) and (not module.startswith('sklearn.metrics._scorer')) and (not module.startswith('sklearn.metrics.tests.'))):
                    raise ValueError('Scoring function {} looks like it is a metric function rather than a scikit-learn scorer. This scoring type was removed in version 0.11. Please update your custom scoring function.'.format(scoring))
                else:
                    self.scoring_function = scoring
    ...
    def _setup_config(self, config_dict):
        if config_dict:
            if isinstance(config_dict, dict):
                ...
                if not _has_cuml():
                    raise ValueError('The GPU machine learning library cuML is not available. To use cuML, please install cuML via conda.')
                ...
                if hasattr(config, 'tpot_config'):
                    ...
                    raise ValueError('Could not find "tpot_config" in configuration file {}. When using a custom config file for customizing operators dictionary, the file must have a python dictionary with the standardized name of "tpot_config"'.format(config_dict))
        else:
    ...
    def fit(self, features, target, sample_weight=None, groups=None):
        ...
        if self.subsample < 1.0:
            ...
            if features.shape[0] < 1500:
                print('Warning: Although subsample can accelerate pipeline optimization process, too small training sample size may cause unpredictable effect on maximizing score in pipeline optimization process. Increasing subsample ratio may get a more reasonable outcome from optimization process in TPOT.')
        if self.random_state is not None:
            random.seed(self.random_state)
            np.random.seed(self.random_state)
        ...
        self._pbar = tqdm(total=total_evals, unit='pipeline', leave=False, file=self.log_file_, disable=not self.verbosity >= 2, desc='Optimization Progress')
        try:
            with warnings.catch_warnings():
                self._setup_memory()
                warnings.simplefilter('ignore')
                self._pop, _ = eaMuPlusLambda(population=self._pop, toolbox=self._toolbox, mu=self.population_size, lambda_=self._lambda, cxpb=self.crossover_rate, mutpb=self.mutation_rate, ngen=self.generations, pbar=self._pbar, halloffame=self._pareto_front, verbose=self.verbosity, per_generation_function=self._check_periodic_pipeline, log_file=self.log_file_)
        except (KeyboardInterrupt, SystemExit, StopIteration) as e:
            if self.verbosity > 0:
                self._pbar.write('', file=self.log_file_)
                self._pbar.write('{}\nTPOT closed prematurely. Will use the current best pipeline.'.format(e), file=self.log_file_)
        finally:
    ...
    def score(self, testing_features, testing_target):
        ...
        score = scorer(self.fitted_pipeline_, testing_features.astype(np.float64), testing_target.astype(np.float64))
        return score

    def _check_proba(self):
        if not hasattr(self, 'fitted_pipeline_'):
            raise AttributeError('A pipeline has not yet been optimized. Please call fit() first.')
        elif not hasattr(self.fitted_pipeline_, 'predict_proba'):
            raise AttributeError('The fitted pipeline does not have the predict_proba() function.')
        return True
    ...
    def export(self, output_file_name='', data_file_path=''):
        ...
        if self._optimized_pipeline is None:
            raise RuntimeError('A pipeline has not yet been optimized. Please call fit() first.')
        to_write = export_pipeline(self._optimized_pipeline, self.operators, self._pset, self._imputed, self._optimized_pipeline_score, self.random_state, data_file_path=data_file_path)
        if output_file_name != '':
    ...
    def _check_dataset(self, features, target, sample_weight=None):
        ...
        if sparse.issparse(features):
            if self.config_dict in [None, 'TPOT light', 'TPOT MDR']:
                raise ValueError('Not all operators in {} supports sparse matrix. Please use "TPOT sparse" for sparse matrix.'.format(self.config_dict))
            elif self.config_dict != 'TPOT sparse':
                print('Warning: Since the input matrix is a sparse matrix, please makes sure all the operators in the customized config dictionary supports sparse matriies.')
        else:
        ...
        try:
            ...
            raise ValueError('Error: Input data is not in a valid format. Please confirm that the input data is scikit-learn compatible. For example, the features must be a 2-D array and target labels must be a 1-D array.')
    ...
    def _stop_by_max_time_mins(self):
        ...
        if self.max_time_mins:
            total_mins_elapsed = (datetime.now() - self._start_datetime).total_seconds() / 60.0
    ...
    def _combine_individual_stats(self, operator_count, cv_score, individual_stats):
        ...
        """
        stats = deepcopy(individual_stats)
    ...
    def _update_evaluated_individuals_(self, result_score_list, eval_individuals_str, operator_counts, stats_dicts):
        ...
        for result_score, individual_str in zip(result_score_list, eval_individuals_str):
            if type(result_score) in [float, np.float64, np.float32]:
                self.evaluated_individuals_[individual_str] = self._combine_individual_stats(operator_counts[individual_str], result_score, stats_dicts[individual_str])
            else:
    ...
    def _update_val(self, val, result_score_list):
        ...
        self._update_pbar()
        if val == 'Timeout':
            self._update_pbar(pbar_msg='Skipped pipeline #{0} due to time out. Continuing to the next pipeline.'.format(self._pbar.n))
        ...
        while len(stack) != 0:
            ...
            if condition(height, depth, type_):
                try:
                    ...
                    raise IndexError('The gp.generate function tried to add a terminal of type {}, but there isnone available. {}'.format(type_, traceback))
                ...
                try:
                    ...
                    _, _, traceback = sys.exc_info()
                    raise IndexError('The gp.generate function tried to add a primitive of type {}, but there isnone available. {}'.format(type_, traceback))
                expr.append(prim)
