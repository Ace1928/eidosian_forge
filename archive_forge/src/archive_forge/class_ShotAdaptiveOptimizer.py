...
class ShotAdaptiveOptimizer(GradientDescentOptimizer):
    ...
    **Example**
    ...
    def __init__(self, min_shots, term_sampling=None, mu=0.99, b=1e-06, stepsize=0.07):
    ...
    def check_device(dev):
        ...
        """
        if not dev.shots:
            raise ValueError('The Rosalin optimizer can only be used with devices that estimate expectation values with a finite number of shots.')
    ...
    def compute_grad(self, objective_fn, args, kwargs):
        """Compute gradient of the objective function, as well as the variance of the gradient,
        at the given point.
        ...
        if isinstance(objective_fn, qml.QNode) or hasattr(objective_fn, 'device'):
            ...
            raise ValueError('The objective function must be encoded as a single QNode object for the shot adaptive optimizer. ')
        gradients = []
        gradient_variances = []
    ...
    def step(self, objective_fn, *args, **kwargs):
        ...
        if self.s is None:
            self.s = [np.zeros_like(a, dtype=np.int64) + self.min_shots for i, a in enumerate(args) if i in self.trainable_args]
        ...
        for idx, (c, x) in enumerate(zip(self.chi, self.xi)):
            ...
            s = np.ceil(2 * self.lipschitz * self.stepsize * xi / ((2 - self.lipschitz * self.stepsize) * (chi ** 2 + self.b * self.mu ** self.k)))
            gamma = ((self.stepsize - self.lipschitz * self.stepsize ** 2 / 2) * chi ** 2 - xi * self.lipschitz * self.stepsize ** 2 / (2 * s)) / s
