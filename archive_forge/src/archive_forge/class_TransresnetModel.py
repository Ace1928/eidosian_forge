...
class TransresnetModel(nn.Module):
    ...
    def add_cmdline_args(argparser):
        ...
        agent.add_argument('--truncate', type=int, default=32, help='Max amount of tokens allowed in a text sequence')
        agent.add_argument('--image-features-dim', type=int, default=2048, help='dimensionality of image features')
        agent.add_argument('--embedding-type', type=str, default=None, choices=[None, 'fasttext_cc'], help='Specify if using pretrained embeddings')
        agent.add_argument('--load-encoder-from', type=str, default=None, help='Specify if using a pretrained transformer encoder')
        agent.add_argument('--hidden-dim', type=int, default=300, help='Hidden dimesionality of personality and image encoder')
        agent.add_argument('--num-layers-all', type=int, default=-1, help='If >= 1, number of layers for both the text and image encoders.')
        agent.add_argument('--num-layers-text-encoder', type=int, default=1, help='Number of layers for the text encoder')
        agent.add_argument('--num-layers-image-encoder', type=int, default=1, help='Number of layers for the image encoder')
        agent.add_argument('--no-cuda', dest='no_cuda', action='store_true', help='If True, perform ops on CPU only')
        agent.add_argument('--learningrate', type=float, default=0.0005, help='learning rate for optimizer')
        agent.add_argument('--additional-layer-dropout', type=float, default=0.2, help='dropout for additional linear layer')
        argparser.set_params(ffn_size=1200, attention_dropout=0.2, relu_dropout=0.2, n_positions=1000)
    ...
    def __init__(self, opt, personalities_list, dictionary):
        ...
        self.optimizer = optim.Adam(filter(lambda p: p.requires_grad, self.parameters()), self.opt['learningrate'])
    ...
    def _build_personality_encoder(self):
        personality_layers = [nn.BatchNorm1d(self.num_personalities), nn.Dropout(p=self.opt['dropout']), nn.Linear(self.num_personalities, self.opt['hidden_dim'])]
        self.personality_encoder = nn.Sequential(*personality_layers)

    def forward(self, image_features, personalities, captions, personalities_tensor=None):
        """
        Model forward pass.
