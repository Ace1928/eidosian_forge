...
class ACMServiceManager:
    ...
    def get_certificates(self, *args, domain_name=None, statuses=None, arn=None, only_tags=None, **kwargs):
        ...
        """
        all_certificates = self.list_certificates_with_backoff(statuses=statuses, module=self.module, error="Couldn't obtain certificates")
        ...
        for certificate in certificates:
            cert_data = self.describe_certificate_with_backoff(certificate['CertificateArn'], module=self.module, error=f"Couldn't obtain certificate metadata for domain {certificate['DomainName']}", ignore_error_codes=['ResourceNotFoundException'])
            if cert_data is None:
                continue
            if cert_data['Status'] not in ('PENDING_VALIDATION', 'VALIDATION_TIMED_OUT', 'FAILED'):
                cert_info = self.get_certificate_with_backoff(certificate['CertificateArn'], module=self.module, error=f"Couldn't obtain certificate data for domain {certificate['DomainName']}", ignore_error_codes=['ResourceNotFoundException'])
                if cert_info is None:
                    continue
                cert_data.update(cert_info)
            cert_data = camel_dict_to_snake_dict(cert_data)
            tags = self.list_certificate_tags_with_backoff(certificate['CertificateArn'], module=self.module, error=f"Couldn't obtain tags for domain {certificate['DomainName']}", ignore_error_codes=['ResourceNotFoundException'])
            if tags is None:
                continue
