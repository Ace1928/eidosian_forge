...
def parallel_interleave_dataset_v2(input_dataset: _atypes.TensorFuzzingAnnotation[_atypes.Variant], other_arguments, cycle_length: _atypes.TensorFuzzingAnnotation[_atypes.Int64], block_length: _atypes.TensorFuzzingAnnotation[_atypes.Int64], num_parallel_calls: _atypes.TensorFuzzingAnnotation[_atypes.Int64], f, output_types, output_shapes, sloppy: bool=False, metadata: str='', name=None) -> _atypes.TensorFuzzingAnnotation[_atypes.Variant]:
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'ParallelInterleaveDatasetV2', name, input_dataset, other_arguments, cycle_length, block_length, num_parallel_calls, 'f', f, 'output_types', output_types, 'output_shapes', output_shapes, 'sloppy', sloppy, 'metadata', metadata)
            return _result
            ...
            pass
        try:
            return parallel_interleave_dataset_v2_eager_fallback(input_dataset, other_arguments, cycle_length, block_length, num_parallel_calls, f=f, output_types=output_types, output_shapes=output_shapes, sloppy=sloppy, metadata=metadata, name=name, ctx=_ctx)
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('ParallelInterleaveDatasetV2', input_dataset=input_dataset, other_arguments=other_arguments, cycle_length=cycle_length, block_length=block_length, num_parallel_calls=num_parallel_calls, f=f, output_types=output_types, output_shapes=output_shapes, sloppy=sloppy, metadata=metadata, name=name)
    _result = _outputs[:]
    if _execute.must_record_gradient():
        _attrs = ('f', _op.get_attr('f'), 'Targuments', _op.get_attr('Targuments'), 'output_types', _op.get_attr('output_types'), 'output_shapes', _op.get_attr('output_shapes'), 'sloppy', _op._get_attr_bool('sloppy'), 'metadata', _op.get_attr('metadata'))
