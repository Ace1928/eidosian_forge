...
class Gemm(Base):
    ...
    def export_default_scalar_bias() -> None:
        ...
        y = gemm_reference_implementation(a, b, c)
        expect(node, inputs=[a, b, c], outputs=[y], name='test_gemm_default_scalar_bias')
    ...
    def export_default_vector_bias() -> None:
        ...
        y = gemm_reference_implementation(a, b, c)
        expect(node, inputs=[a, b, c], outputs=[y], name='test_gemm_default_vector_bias')
    ...
    def export_default_matrix_bias() -> None:
        ...
        y = gemm_reference_implementation(a, b, c)
        expect(node, inputs=[a, b, c], outputs=[y], name='test_gemm_default_matrix_bias')
    ...
    def export_transposeB() -> None:
        node = onnx.helper.make_node('Gemm', inputs=['a', 'b', 'c'], outputs=['y'], transB=1)
        a = np.random.ranf([3, 6]).astype(np.float32)
