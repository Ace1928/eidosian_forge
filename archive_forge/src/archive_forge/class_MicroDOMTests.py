...
class MicroDOMTests(TestCase):
    ...
    def test_search(self) -> None:
        ...
        self.assertEqual(d.getElementsByTagName('foo'), [root, root.lastChild().firstChild()])
        ...
        self.assertEqual(d2.getElementsByTagName('fOO'), [root.lastChild().firstChild()])
        self.assertEqual(d2.getElementsByTagName('foo'), [])
        root = d3.documentElement
        self.assertEqual(root.firstChild(), d3.getElementById('me'))
        self.assertEqual(d3.getElementsByTagName('FOO'), [root, root.lastChild().firstChild()])
        self.assertEqual(d3.getElementsByTagName('fOo'), [root, root.lastChild().firstChild()])
    ...
    samples = [('<img/>', '<img />'), ("<foo A='b'>x</foo>", '<foo A="b">x</foo>'), ('<foo><BAR /></foo>', '<foo><BAR></BAR></foo>'), ('<foo>hello there &amp; yoyoy</foo>', '<foo>hello there &amp; yoyoy</foo>')]
    ...
    def test_deprecation(self) -> None:
        ...
        self.assertEqual('twisted.web.microdom was deprecated at Twisted 23.10.0', warnings[0]['message'])