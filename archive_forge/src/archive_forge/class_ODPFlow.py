...
class ODPFlow(Flow):
    ...
    def __init__(self, odp_string, id=None):
        ...
        if ufid_pos >= 0:
            ...
            ufid_parser = KVParser(ufid_string, KVDecoders({'ufid': decode_default}))
            ufid_parser.parse()
        ...
        if action_pos < 0:
            raise ValueError('malformed odp flow: %s' % odp_string)
        rest = odp_string[ufid_pos + len(ufid_string) if ufid_pos >= 0 else 0:action_pos]
        action_pos += 8
        ...
        iparser.parse()
        isection = Section(name='info', pos=odp_string.find(info), string=info, data=iparser.kv())
        sections.append(isection)
        mparser = KVParser(match, ODPFlow.match_decoders())
        mparser.parse()
        msection = Section(name='match', pos=odp_string.find(match), string=match, data=mparser.kv())
        sections.append(msection)
        aparser = KVParser(actions, ODPFlow.action_decoders())
        aparser.parse()
        asection = Section(name='actions', pos=action_pos, string=actions, data=aparser.kv(), is_list=True)
