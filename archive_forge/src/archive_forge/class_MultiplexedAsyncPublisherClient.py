...
class MultiplexedAsyncPublisherClient(AsyncPublisherClientInterface):
    ...
    def __init__(self, publisher_factory: AsyncPublisherFactory):
        ...
        self._multiplexer = AsyncClientMultiplexer(lambda topic: self._create_and_open(topic))
    ...
    async def publish(self, topic: Union[TopicPath, str], data: bytes, ordering_key: str='', **attrs: Mapping[str, str]) -> str:
        ...
        try:
            return await publisher.publish(data=data, ordering_key=ordering_key, **attrs)
