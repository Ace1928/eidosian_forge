...
def _process_outgoing_msg_list(self):
    while True:
        ...
        if outgoing_msg is None:
            ...
            continue
        assert isinstance(outgoing_msg, (BGPRouteRefresh, BGPUpdate, OutgoingRoute)), 'Peer cannot process object: %s in its outgoing queue' % outgoing_msg
        if isinstance(outgoing_msg, BGPRouteRefresh):
            ...
            LOG.debug('Update %s>> %s', self._neigh_conf.ip_address, outgoing_msg)
