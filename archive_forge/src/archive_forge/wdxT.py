...
def launch_file_browser() -> Path:
    ...
    file_browser = FileBrowserWidget(FileBrowserWidget)
    file_browser.show()
    app.exec_()
...
def is_admin() -> bool:
    ...
    has superuser privileges, which is generally not common.
    ...
    try:
        if os.name == "nt":  # Checks if the operating system is Windows
            # Utilizes ctypes to call Windows API for admin status check
            return ctypes.windll.shell32.IsUserAnAdmin() == 1
        elif (
            os.name == "posix"
        ):  # Checks if the operating system is POSIX-compliant (Linux, Unix, macOS, etc.)
            # On POSIX systems, root's UID is 0, so checking the current process's UID
            return os.geteuid() == 0
            ...
            return os.system("su -c id") == 0
        else:
            # For any other operating systems not explicitly checked, assume no admin privileges
            return False
    except Exception as e:
