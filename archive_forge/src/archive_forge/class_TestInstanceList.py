...
class TestInstanceList(TestInstances):
    defaults = {'include_clustered': False, 'limit': None, 'marker': None}
    ...
    def test_instance_list_defaults(self):
        ...
        insts = [{'id': instance_id, 'name': name, 'status': 'ACTIVE', 'operating_status': 'HEALTHY', 'addresses': [{'type': 'private', 'address': '10.0.0.13'}], 'volume': {'size': 2}, 'flavor': {'id': '02'}, 'region': 'regionOne', 'datastore': {'version': '5.6', 'type': 'mysql', 'version_number': '5.7.29'}, 'tenant_id': tenant_id, 'replica_of': self.random_uuid(), 'access': {'is_public': False, 'allowed_cidrs': []}}]
        self.instance_client.list.return_value = common.Paginated([instances.Instance(mock.MagicMock(), inst) for inst in insts])
        parsed_args = self.check_parser(self.cmd, [], [])
        columns, data = self.cmd.take_action(parsed_args)
        self.instance_client.list.assert_called_once_with(**self.defaults)
        self.assertEqual(database_instances.ListDatabaseInstances.columns, columns)
        values = [(instance_id, name, 'mysql', '5.6', 'ACTIVE', 'HEALTHY', False, [{'type': 'private', 'address': '10.0.0.13'}], '02', 2, 'replica')]
    ...
    def test_instance_list_for_project(self):
        ...
        insts = [{'id': instance_id, 'name': name, 'status': 'ACTIVE', 'operating_status': 'HEALTHY', 'addresses': [{'type': 'private', 'address': '10.0.0.13'}], 'volume': {'size': 2}, 'flavor': {'id': '02'}, 'region': 'regionOne', 'datastore': {'version': '5.6', 'type': 'mysql', 'version_number': '5.7.29'}, 'tenant_id': tenant_id, 'access': {'is_public': False, 'allowed_cidrs': []}, 'server_id': server_id, 'server': {'id': server_id}}]
        self.mgmt_client.list.return_value = common.Paginated([instances.Instance(mock.MagicMock(), inst) for inst in insts])
        parsed_args = self.check_parser(self.cmd, ['--project-id', tenant_id], [('project_id', tenant_id)])
        columns, data = self.cmd.take_action(parsed_args)
        self.assertEqual(database_instances.ListDatabaseInstances.admin_columns, columns)
        expected_instances = [(instance_id, name, 'mysql', '5.6', 'ACTIVE', 'HEALTHY', False, [{'type': 'private', 'address': '10.0.0.13'}], '02', 2, '', server_id, tenant_id)]
        self.assertEqual(expected_instances, data)
        expected_params = {'include_clustered': False, 'limit': None, 'marker': None, 'project_id': tenant_id}
