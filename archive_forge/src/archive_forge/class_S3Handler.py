...
class S3Handler(StorageHandler):
    ...
    def init_boto(self) -> 'boto3.resources.base.ServiceResource':
        ...
        self._s3 = boto.session.Session().resource('s3', endpoint_url=os.getenv('AWS_S3_ENDPOINT_URL'), region_name=os.getenv('AWS_REGION'))
    ...
    def load_path(self, manifest_entry: ArtifactManifestEntry, local: bool=False) -> Union[URIStr, FilePathStr]:
        ...
        assert manifest_entry.ref is not None
        path, hit, cache_open = self._cache.check_etag_obj_path(URIStr(manifest_entry.ref), ETag(manifest_entry.digest), manifest_entry.size if manifest_entry.size is not None else 0)
        if hit:
            return path
        self.init_boto()
        ...
        if key != '':
            try:
                ...
                if e.response['Error']['Code'] == '404':
                    ...
                    raise CommError('Unable to connect to S3 ({}): {}'.format(e.response['Error']['Code'], e.response['Error']['Message']))
        else:
            multi = True
        if multi:
            start_time = time.time()
            termlog('Generating checksum for up to %i objects in "%s/%s"... ' % (max_objects, bucket, key), newline=False)
            if key != '':
    ...
    def _size_from_obj(self, obj: Union['boto3.s3.Object', 'boto3.s3.ObjectSummary']) -> int:
        size: int
        if hasattr(obj, 'size'):
            size = obj.size
        else:
    ...
    def _entry_from_obj(self, obj: Union['boto3.s3.Object', 'boto3.s3.ObjectSummary'], path: str, name: Optional[StrPath]=None, prefix: str='', multi: bool=False) -> ArtifactManifestEntry:
    ...
    def _extra_from_obj(self, obj: Union['boto3.s3.Object', 'boto3.s3.ObjectSummary']) -> Dict[str, str]:
