...
def vectorized_map(fn, elems, fallback_to_while_loop=True, warn=True):
  ...
  """
    ...
    def loop_fn(i):
        gathered_elems = nest.map_structure(lambda x: _gather_from_tensor_or_composite(x, i), elems)
        return fn(gathered_elems)
    flat_elems = nest.flatten(nest.map_structure(functools.partial(_composite_to_tensors, is_batched=True), elems))
    ...
    if any((s is None for s in static_first_dims)):
        batch_size = math_ops.reduce_max([array_ops.shape(elem)[0] for elem in flat_elems])
    else:
        batch_size = max(static_first_dims)
    return pfor(loop_fn, batch_size, fallback_to_while_loop=fallback_to_while_loop, warn=warn)