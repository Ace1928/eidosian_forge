...
class QuaggaBGPContainer(base.BGPContainer):
    ...
    def __init__(self, name, asn, router_id, ctn_image_name, zebra=False):
        super(QuaggaBGPContainer, self).__init__(name, asn, router_id, ctn_image_name)
        self.shared_volumes.append((self.config_dir, self.SHARED_VOLUME))
        self.zebra = zebra
        self._create_config_debian()

    def run(self, wait=False, w_time=WAIT_FOR_BOOT):
        w_time = super(QuaggaBGPContainer, self).run(wait=wait, w_time=self.WAIT_FOR_BOOT)
        return w_time
    ...
    def get_global_rib(self, prefix='', rf='ipv4'):
        ...
        for line in out.split('\n'):
            ...
            if len(elems) == 1:
                ...
                nexthop = elems[1]
            read_next = False
            rib.append({'prefix': prefix, 'nexthop': nexthop, 'ibgp': ibgp})
        return rib
