...
class KernelClient(ConnectionFileMixin):
    ...
    * control: for kernel management calls to the kernel.
    ...
    def __del__(self) -> None:
        ...
        if self._created_context and self.context is not None and (not self.context.closed):
    ...
    async def _async_recv_reply(self, msg_id: str, timeout: t.Optional[float]=None, channel: str='shell') -> t.Dict[str, t.Any]:
    ...
    async def _async_execute_interactive(self, code: str, silent: bool=False, store_history: bool=True, user_expressions: t.Optional[t.Dict[str, t.Any]]=None, allow_stdin: t.Optional[bool]=None, stop_on_error: bool=True, timeout: t.Optional[float]=None, output_hook: t.Optional[t.Callable]=None, stdin_hook: t.Optional[t.Callable]=None) -> t.Dict[str, t.Any]:
        ...
        if allow_stdin and (not self.stdin_channel.is_alive()):
            ...
            raise RuntimeError(emsg)
        msg_id = await ensure_async(self.execute(code, silent=silent, store_history=store_history, user_expressions=user_expressions, allow_stdin=allow_stdin, stop_on_error=stop_on_error))
        ...
        if output_hook is None and 'IPython' in sys.modules:
            ...
            if in_kernel:
                output_hook = partial(self._output_hook_kernel, ip.display_pub.session, ip.display_pub.pub_socket, ip.display_pub.parent_header)
    ...
    def execute(self, code: str, silent: bool=False, store_history: bool=True, user_expressions: t.Optional[t.Dict[str, t.Any]]=None, allow_stdin: t.Optional[bool]=None, stop_on_error: bool=True) -> str:
        ...
        content = {'code': code, 'silent': silent, 'store_history': store_history, 'user_expressions': user_expressions, 'allow_stdin': allow_stdin, 'stop_on_error': stop_on_error}
