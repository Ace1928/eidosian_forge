...
class ParticleConservingU2(Operation):
    ...
    def __init__(self, weights, wires, init_state=None, id=None):
        if len(wires) < 2:
            raise ValueError(f'This template requires the number of qubits to be greater than one;got a wire sequence with {len(wires)} elements')
    ...
    def shape(n_layers, n_wires):
        ...
        """
        if n_wires < 2:
            raise ValueError(f"The number of qubits must be greater than one; got 'n_wires' = {n_wires}")
