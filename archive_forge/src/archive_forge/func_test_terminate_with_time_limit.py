...
def test_terminate_with_time_limit(self):
    ...
    results = pyros_solver.solve(model=m, first_stage_variables=[m.x1, m.x2], second_stage_variables=[], uncertain_params=[m.u], uncertainty_set=interval, local_solver=local_subsolver, global_solver=global_subsolver, objective_focus=ObjectiveType.worst_case, solve_master_globally=True, time_limit=0.001)
    ...
    for slvr, desc in zip(subsolvers, ['Local', 'Global']):
        self.assertEqual(len(list(slvr.options.keys())), 0, msg=f'{desc} subsolver options were changed by PyROS')
        self.assertIs(getattr(slvr.options, 'MaxTime', None), None, msg=f'{desc} subsolver (BARON) MaxTime setting was added by PyROS, but not reverted')