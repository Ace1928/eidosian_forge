...
class PerformMaintenance(base.UpdateCommand):
    ...
    def Run(self, args):
        ...
        perform_maintenance = messages.NodeGroupsPerformMaintenanceRequest(nodes=args.nodes)
        if args.start_time:
            perform_maintenance.startTime = times.FormatDateTime(args.start_time)
        request = messages.ComputeNodeGroupsPerformMaintenanceRequest(nodeGroupsPerformMaintenanceRequest=perform_maintenance, nodeGroup=node_group_ref.Name(), project=node_group_ref.project, zone=node_group_ref.zone)
        ...
        log.status.Print('Perform maintenance call in progress for nodes [{}] in node group [{}]: {}'.format(args.nodes, node_group_ref.Name(), operation_ref.SelfLink()))
