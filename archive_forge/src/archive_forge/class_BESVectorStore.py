...
class BESVectorStore(VectorStore):
    ...
    def __init__(self, index_name: str, bes_url: str, user: Optional[str]=None, password: Optional[str]=None, embedding: Optional[Embeddings]=None, **kwargs: Optional[dict]) -> None:
    ...
    @staticmethod
    def bes_client(*, bes_url: Optional[str]=None, username: Optional[str]=None, password: Optional[str]=None) -> 'Elasticsearch':
        try:
    ...
    def _create_index_if_not_exists(self, dims_length: Optional[int]=None) -> None:
        ...
        if self.client.indices.exists(index=self.index_name):
            ...
            self.client.indices.create(index=self.index_name, body={'settings': {'index': {'knn': True}}, 'mappings': {'properties': indexMapping}})
    ...
    def _index_mapping(self, dims_length: Union[int, None]) -> Dict:
        ...
        if 'linear' == self.index_type:
            return {self.vector_query_field: {'type': 'bpack_vector', 'dims': dims_length, 'build_index': self.index_params.get('build_index', False)}}
        elif 'hnsw' == self.index_type:
            return {self.vector_query_field: {'type': 'bpack_vector', 'dims': dims_length, 'index_type': 'hnsw', 'space_type': self.space_type, 'parameters': {'ef_construction': self.index_params.get('hnsw_ef_construction', 200), 'm': self.index_params.get('hnsw_m', 4)}}}
        else:
            return {self.vector_query_field: {'type': 'bpack_vector', 'model_id': self.index_params.get('model_id', '')}}
    ...
    def _query_body(self, query_vector: Union[List[float], None], filter: Optional[dict]=None, search_params: Dict={}) -> Dict:
        ...
        return {'size': search_params.get('size', 4), 'query': {'knn': {self.vector_query_field: query_vector_body}}}

    def _search(self, query: Optional[str]=None, query_vector: Union[List[float], None]=None, filter: Optional[dict]=None, custom_query: Optional[Callable[[Dict, Union[str, None]], Dict]]=None, search_params: Dict={}) -> List[Tuple[Document, float]]:
        ...
        docs_and_scores = [(Document(page_content=hit['_source'][self.query_field], metadata=hit['_source']['metadata']), hit['_score']) for hit in hits]
    ...
    def similarity_search_with_score(self, query: str, k: int, filter: Optional[dict]=None, **kwargs: Any) -> List[Tuple[Document, float]]:
    ...
    @classmethod
    def from_documents(cls, documents: List[Document], embedding: Optional[Embeddings]=None, **kwargs: Any) -> 'BESVectorStore':
    ...
    @classmethod
    def from_texts(cls, texts: List[str], embedding: Optional[Embeddings]=None, metadatas: Optional[List[Dict[str, Any]]]=None, **kwargs: Any) -> 'BESVectorStore':
    ...
    def add_texts(self, texts: Iterable[str], metadatas: Optional[List[Dict[Any, Any]]]=None, **kwargs: Any) -> List[str]:
        ...
        if self.embedding is not None:
            ...
            for i, (text, vector) in enumerate(zip(texts, embeddings)):
                ...
                requests.append({'_op_type': 'index', '_index': self.index_name, self.query_field: text, self.vector_query_field: vector, 'metadata': metadata, '_id': ids[i]})
        else:
            if create_index_if_not_exists:
                self._create_index_if_not_exists()
            for i, text in enumerate(texts):
                metadata = metadatas[i] if metadatas else {}
                requests.append({'_op_type': 'index', '_index': self.index_name, self.query_field: text, 'metadata': metadata, '_id': ids[i]})
        if len(requests) > 0:
            try:
