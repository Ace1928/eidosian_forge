...
def _ShouldDoParallelCompositeUpload(logger, allow_splitting, src_url, dst_url, file_size, gsutil_api, canned_acl=None):
  ...
  """
    ...
    parallel_composite_upload_threshold = HumanReadableToBytes(config.get('GSUtil', 'parallel_composite_upload_threshold', DEFAULT_PARALLEL_COMPOSITE_UPLOAD_THRESHOLD))
    all_factors_but_size = allow_splitting and (not src_url.IsStream()) and (not src_url.IsFifo()) and (dst_url.scheme == 'gs') and (not canned_acl)
    if all_factors_but_size and parallel_composite_upload_threshold == 0 and (file_size >= PARALLEL_COMPOSITE_SUGGESTION_THRESHOLD):
    ...
    return all_factors_but_size and parallel_composite_upload_threshold > 0 and (file_size >= parallel_composite_upload_threshold)