...
class AnalysisAdam(torch.optim.Optimizer):
    ...
    def __init__(self, params, lr=0.001, betas=(0.9, 0.999), eps=1e-08, weight_decay=0, amsgrad=False, bnb_analysis='dynamic-blockwise', savedir=None):
        ...
        for group in self.param_groups:
            for p_id, p in enumerate(group['params']):
                ...
                if p_data_fp32.numel() <= 8192 or p_data_fp32.numel() > 50000 * 1000:
                    ...
                    relerr = abserr / torch.abs(update_fp32 + 1e-06)
                    C1, C2 = (C1.int(), C2.int())
