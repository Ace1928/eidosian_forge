...
class BokehRenderer(Renderer):
    ...
    def __init__(self, nrows: int=1, ncols: int=1, figsize: tuple[float, float]=(9, 9), show_frame: bool=True, want_svg: bool=False) -> None:
        ...
        self._layout = gridplot(self._figures, ncols=ncols, toolbar_location=None, width=total_size[0] // ncols, height=total_size[1] // nrows)
    ...
    def filled(self, filled: FillReturn, fill_type: FillType | str, ax: figure | int=0, color: str='C0', alpha: float=0.7) -> None:
    ...
    def grid(self, x: ArrayLike, y: ArrayLike, ax: figure | int=0, color: str='black', alpha: float=0.1, point_color: str | None=None, quad_as_tri_alpha: float=0) -> None:
        ...
        if quad_as_tri_alpha > 0:
            ...
            fig.multi_line([row for row in np.stack((x[:-1, :-1].ravel(), xmid, x[1:, 1:].ravel()), axis=1)], [row for row in np.stack((y[:-1, :-1].ravel(), ymid, y[1:, 1:].ravel()), axis=1)], **kwargs)
            fig.multi_line([row for row in np.stack((x[:-1, 1:].ravel(), xmid, x[1:, :-1].ravel()), axis=1)], [row for row in np.stack((y[:-1, 1:].ravel(), ymid, y[1:, :-1].ravel()), axis=1)], **kwargs)
        if point_color is not None:
            fig.circle(x=x.ravel(), y=y.ravel(), fill_color=color, line_color=None, alpha=alpha, size=8)

    def lines(self, lines: LineReturn, line_type: LineType | str, ax: figure | int=0, color: str='C0', alpha: float=1.0, linewidth: float=1) -> None:
