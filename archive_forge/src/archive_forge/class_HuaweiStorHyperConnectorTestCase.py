...
class HuaweiStorHyperConnectorTestCase(test_connector.ConnectorTestCase):
    ...
    def setUp(self):
        ...
        newefile.write('test')
        newefile.close()
        self.connector = huawei.HuaweiStorHyperConnector(None, execute=self.fake_execute)
        self.connector.cli_path = self.fake_sdscli_file
        self.connector.iscliexist = True
        self.connector_fail = huawei.HuaweiStorHyperConnector(None, execute=self.fake_execute_fail)
        self.connector_fail.cli_path = self.fake_sdscli_file
        self.connector_fail.iscliexist = True
        self.connector_nocli = huawei.HuaweiStorHyperConnector(None, execute=self.fake_execute_fail)
        self.connector_nocli.cli_path = self.fake_sdscli_file
        self.connector_nocli.iscliexist = False
        self.connection_properties = {'access_mode': 'rw', 'qos_specs': None, 'volume_id': 'volume-b2911673-863c-4380-a5f2-e1729eecfe3f'}
        self.device_info = {'type': 'block', 'path': '/dev/vdxxx'}
    ...
    def test_get_connector_properties(self):
        props = huawei.HuaweiStorHyperConnector.get_connector_properties('sudo', multipath=True, enforce_multipath=True)
        expected_props = {}
    ...
    @mock.patch.object(huawei.HuaweiStorHyperConnector, '_query_attached_volume')
    def test_get_volume_paths(self, mock_query_attached):
        path = self.device_info['path']
        mock_query_attached.return_value = {'ret_code': 0, 'dev_addr': path}
        expected = [path]
    ...
    def test_connect_volume(self):
        ...
        expected_commands = [self.fake_sdscli_file + ' -c attach -v volume-b2911673-863c-4380-a5f2-e1729eecfe3f', self.fake_sdscli_file + ' -c querydev -v volume-b2911673-863c-4380-a5f2-e1729eecfe3f']
    ...
    def test_disconnect_volume_nocli(self):
        ...
        self.connector.connect_volume(self.connection_properties)
        self.assertEqual(True, HuaweiStorHyperConnectorTestCase.attached)
        self.assertRaises(exception.BrickException, self.connector_nocli.disconnect_volume, self.connection_properties, self.device_info)
        expected_commands = [self.fake_sdscli_file + ' -c attach -v volume-b2911673-863c-4380-a5f2-e1729eecfe3f', self.fake_sdscli_file + ' -c querydev -v volume-b2911673-863c-4380-a5f2-e1729eecfe3f']
        self.assertEqual(expected_commands, self.cmds)
