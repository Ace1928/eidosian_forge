...
class FugueTask(TaskSpec, ABC):
    ...
    def __init__(self, input_n: int=0, output_n: int=0, configs: Any=None, params: Any=None, deterministic: bool=True, lazy: bool=False, input_names: Optional[List[str]]=None):
        assert_or_throw(output_n <= 1, NotImplementedError("Fugue doesn't support multiple output tasks"))
        if input_names is None:
            inputs = [InputSpec('_' + str(i), DataFrame, nullable=False) for i in range(input_n)]
        else:
            inputs = [InputSpec(input_names[i], DataFrame, nullable=False) for i in range(input_n)]
        outputs = [OutputSpec('_' + str(i), DataFrame, nullable=False) for i in range(output_n)]
        self._input_has_key = input_names is not None
        super().__init__(configs=configs, inputs=inputs, outputs=outputs, func=self.execute, metadata=params, deterministic=deterministic, lazy=lazy)
    ...
    def __uuid__(self) -> str:
        return to_uuid(self.configs, self.inputs, self.outputs, self.metadata, self.deterministic, self.lazy, self._get_dependency_uuid(), self._broadcast)
    ...
    def single_output_expression(self) -> str:
        assert_or_throw(len(self.outputs) == 1, lambda: FugueWorkflowError(f'{self.name} does not have single output'))
    ...
    def set_result(self, ctx: TaskContext, df: DataFrame) -> DataFrame:
        ...
        if self._yield_dataframe_handler is not None:
            out_df = self._get_execution_engine(ctx).convert_yield_dataframe(df, as_local=self._yield_local_dataframe)
    ...
    def _handle_checkpoint(self, df: DataFrame, ctx: TaskContext) -> DataFrame:
        ...
        return self._execute_with_modified_traceback(lambda: self._checkpoint.run(df, wfctx.checkpoint_path))

    def _handle_broadcast(self, df: DataFrame, ctx: TaskContext) -> DataFrame:
        if not self._broadcast:
            return df
        return self._execute_with_modified_traceback(lambda: self._get_execution_engine(ctx).broadcast(df))
    ...
    def _execute_with_modified_traceback(self, func: Callable) -> Any:
        try:
            ...
            if _MODIFIED_EXCEPTION_VAR_NAME in self._traceback.tb_frame.f_locals:
                raise ex from self._traceback.tb_frame.f_locals[_MODIFIED_EXCEPTION_VAR_NAME]
            ctb = modify_traceback(sys.exc_info()[2].tb_next, None, self._traceback)
            if ctb is None:
                raise
