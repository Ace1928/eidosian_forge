...
def test_normal_return(log_tracker, server):
    ...
    resp = requests.get('http://%s:%s/as_string' % (host, port), headers={'Referer': 'http://www.cherrypy.dev/', 'User-Agent': 'Mozilla/5.0'})
    ...
    if not any((k for k, v in resp.headers.items() if k.lower() == 'content-length')):
        content_length = '-'
    log_tracker.assertLog(-1, '] "GET /as_string HTTP/1.1" 200 %s "http://www.cherrypy.dev/" "Mozilla/5.0"' % content_length)