  ...
  """
    ...
    def _resource_apply_dense(self, grad, var, apply_state=None):
        ...
        if self._momentum:
            ...
            return gen_training_ops.ResourceApplyKerasMomentum(var=var.handle, accum=momentum_var.handle, lr=coefficients['lr_t'], grad=grad, momentum=coefficients['momentum'], use_locking=self._use_locking, use_nesterov=self.nesterov)
        else:
            return gen_training_ops.ResourceApplyGradientDescent(var=var.handle, alpha=coefficients['lr_t'], delta=grad, use_locking=self._use_locking)
    ...
    def _resource_apply_sparse(self, grad, var, indices, apply_state=None):
        ...
        coefficients = (apply_state or {}).get((var_device, var_dtype)) or self._fallback_apply_state(var_device, var_dtype)
        momentum_var = self.get_slot(var, 'momentum')
        return gen_training_ops.ResourceSparseApplyKerasMomentum(var=var.handle, accum=momentum_var.handle, lr=coefficients['lr_t'], grad=grad, indices=indices, momentum=coefficients['momentum'], use_locking=self._use_locking, use_nesterov=self.nesterov)

    def get_config(self):
        config = super(SGD, self).get_config()
        config.update({'learning_rate': self._serialize_hyperparameter('learning_rate'), 'decay': self._initial_decay, 'momentum': self._serialize_hyperparameter('momentum'), 'nesterov': self.nesterov})
        return config