...
class Frame:
        ...
        try:
            ...
            if y < p - _FUZZ:
                if not rl_config.allowTableBoundsErrors and ((h > self._aH or w > aW) and (not trySplit)):
                    ...
                    raise LayoutError('Flowable %s (%sx%s points) too large for frame (%sx%s points).' % (flowable.__class__, w, h, aW, self._aH))
                return 0
            else:
    ...
    def drawBoundary(self, canv, __boundary__=None):
        canv.drawBoundary(__boundary__ or self.showBoundary, self._x1, self._y1, self._x2 - self._x1, self._y2 - self._y1)
