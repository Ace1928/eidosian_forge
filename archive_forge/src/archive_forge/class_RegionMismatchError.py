...
class RegionMismatchError(core_exceptions.Error):
    ...
    """
        msg = "Builds that run in a worker pool can only run in that worker pool's region. You selected %s, but your worker pool is in %s. To fix this, simply omit the --region flag." % (build_region, wp_region)
