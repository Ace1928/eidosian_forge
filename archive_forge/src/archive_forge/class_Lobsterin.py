...
class Lobsterin(UserDict, MSONable):
    ...
    @staticmethod
    def get_basis(structure: Structure, potcar_symbols: list, address_basis_file: str | None=None):
        ...
        for idx, basis in enumerate(potcar_names):
            if basis not in BASIS:
                raise ValueError(f"You have to provide the basis for {basis} manually. We don't have any information on this POTCAR.")
    ...
    @staticmethod
    def write_POSCAR_with_standard_primitive(POSCAR_input='POSCAR', POSCAR_output='POSCAR.lobster', symprec: float=0.01):
        """
    ...
    @staticmethod
    def write_KPOINTS(POSCAR_input: str='POSCAR', KPOINTS_output='KPOINTS.lobster', reciprocal_density: int=100, isym: int=-1, from_grid: bool=False, input_grid: Sequence[int]=(5, 5, 5), line_mode: bool=True, kpoints_line_density: int=20, symprec: float=0.01):
        """
        ...
        if line_mode:
            ...
            if not np.allclose(kpath.prim.lattice.matrix, structure.lattice.matrix):
                raise ValueError('You are not using the standard primitive cell. The k-path is not correct. Please generate a standard primitive cell first.')
        ...
        ISYM = isym
        comment = f'ISYM={ISYM!r}, grid: {mesh} plus kpoint path' if line_mode else f'ISYM={ISYM!r}, grid: {mesh}'
        KpointObject = Kpoints(comment=comment, style=Kpoints.supported_modes.Reciprocal, num_kpts=len(kpts), kpts=kpts, kpts_weights=weights, labels=all_labels)
        ...
        for datum in data:
            if not datum.startswith(('!', '#', '//')):
                ...
                if (matched_pattern := re.findall(pattern, datum)):
                    ...
                    key_word = raw_datum.strip().split(' ')
                    if len(key_word) > 1:
