...
def parse_list(ctx, tokens, breakstack):
  ...
  """
    ...
    parsemap = {'LENGTH': StandardParser(3, flags=['LENGTH']), 'GET': StandardParser('4+', flags=['GET']), 'JOIN': StandardParser(4, flags=['JOIN']), 'SUBLIST': StandardParser(5, flags=['SUBLIST']), 'FIND': StandardParser(4, flags=['FIND']), 'APPEND': StandardParser('2+', flags=['APPEND']), 'FILTER': StandardParser(5, flags=['FILTER', 'INCLUDE', 'EXCLUDE', 'REGEX']), 'INSERT': StandardParser('4+', flags=['INSERT']), 'POP_BACK': StandardParser('2+', flags=['POP_BACK']), 'POP_FRONT': StandardParser('2+', flags=['POP_FRONT']), 'PREPEND': StandardParser('2+', flags=['PREPEND']), 'REMOVE_ITEM': StandardParser('3+', flags=['REMOVE_ITEM']), 'REMOVE_AT': StandardParser('3+', flags=['REMOVE_AT']), 'REMOVE_DUPLICATES': StandardParser(2, flags=['REMOVE_DUPLICATES']), 'TRANSFORM': StandardParser(2, flags=['TRANSFORM'], kwargs=TRANSFORM_KWARGS), 'REVERSE': StandardParser(2, flags=['REVERSE']), 'SORT': StandardParser(2, flags=['SORT'], kwargs=SORT_KWARGS)}
    if descriminator not in parsemap:
        logger.warning('Indeterminate form of file() command "%s" at %s', descriminator, tokens[0].location())
