...
class BlackboardLoader(WebBaseLoader):
    ...
    def __init__(self, blackboard_course_url: str, bbrouter: str, load_all_recursively: bool=True, basic_auth: Optional[Tuple[str, str]]=None, cookies: Optional[dict]=None, continue_on_failure: bool=False):
        ...
        """
        super().__init__(web_paths=blackboard_course_url, continue_on_failure=continue_on_failure)
        try:
            self.base_url = blackboard_course_url.split('/webapps/blackboard')[0]
        except IndexError:
            raise IndexError('Invalid blackboard course url. Please provide a url that starts with https://<blackboard_url>/webapps/blackboard')
