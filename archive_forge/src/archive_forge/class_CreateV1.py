...
class CreateV1(base.CreateCommand):
  ...
  """
    ...
    @staticmethod
    def Args(parser):
        flags.AddRegionResourceArg(parser, 'to create index', prompt_func=region_util.GetPromptForRegionFunc(constants.SUPPORTED_OP_REGIONS))
    ...
    def _Run(self, args, version):
        ...
        with endpoint_util.AiplatformEndpointOverrides(version, region=region):
            ...
            log.status.Print(constants.OPERATION_CREATION_DISPLAY_MESSAGE.format(name=operation.name, verb='create index', id=op_ref.Name(), sub_commands='--index={} --region={} [--project={}]'.format(index_id, region, project_id)))
            return operation
