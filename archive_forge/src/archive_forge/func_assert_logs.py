...
def assert_logs(*matches: str, count: Optional[int]=1, min_level: int=logging.WARNING, max_level: int=logging.CRITICAL, capture_warnings: bool=True) -> Iterator[List[logging.LogRecord]]:
    ...
    try:
        ...
        assert count is None or len(records) == count, f'Expected {count} log message but got {len(records)}. Log messages: {msgs}'
        for match in matches:
            assert match in ''.join(msgs), f'{match} expected to appear in log messages but it was not found. Log messages: {msgs}.'