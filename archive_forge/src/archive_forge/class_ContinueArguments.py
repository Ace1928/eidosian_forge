...
class ContinueArguments(BaseSchema):
    ...
    """
    __props__ = {'threadId': {'type': 'integer', 'description': "Specifies the active thread. If the debug adapter supports single thread execution (see 'supportsSingleThreadExecutionRequests') and the optional argument 'singleThread' is true, only the thread with this ID is resumed."}, 'singleThread': {'type': 'boolean', 'description': "If this optional flag is true, execution is resumed only for the thread with given 'threadId'."}}
    __refs__ = set()
