...
class NetAppOntapServiceProcessorNetwork:
    ...
    def __init__(self):
        ...
        self.module = AnsibleModule(argument_spec=self.argument_spec, supports_check_mode=True, mutually_exclusive=[('netmask', 'prefix_length')])
    ...
    def set_playbook_zapi_key_map(self):
        self.na_helper.zapi_string_keys = {'address_type': 'address-type', 'node': 'node', 'dhcp': 'dhcp', 'gateway_ip_address': 'gateway-ip-address', 'ip_address': 'ip-address', 'netmask': 'netmask'}
        self.na_helper.zapi_int_keys = {'prefix_length': 'prefix-length'}
        self.na_helper.zapi_bool_keys = {'is_enabled': 'is-enabled'}
        self.na_helper.zapi_required = {'address_type': 'address-type', 'node': 'node', 'is_enabled': 'is-enabled'}
    ...
    def get_service_processor_network_rest(self):
        ...
        if error:
            self.module.fail_json(msg='Error fetching service processor network info for %s: %s' % (self.parameters['node'], error))
        current = None
        if record:
            ...
            current = {'dhcp': 'v4' if self.na_helper.safe_get(record, ['service_processor', 'dhcp_enabled']) else 'none', 'gateway_ip_address': self.na_helper.safe_get(record, ['service_processor', self.ipv4_or_ipv6, 'gateway']), 'ip_address': self.na_helper.safe_get(record, ['service_processor', self.ipv4_or_ipv6, 'address']), 'is_enabled': True if self.na_helper.safe_get(record, ['service_processor', self.ipv4_or_ipv6]) else False, netmask_or_prefix: self.na_helper.safe_get(record, ['service_processor', self.ipv4_or_ipv6, 'netmask'])}
        return current
