...
class Contour(ContourMapping):
    ...
    def __setitem__(self, index: int | slice, point: Point | Iterable[Point]) -> None:
        if isinstance(index, int) and isinstance(point, Point):
            self.points[index] = point
        elif isinstance(index, slice) and isinstance(point, Iterable) and all((isinstance(p, Point) for p in point)):
            self.points[index] = point
        else:
            raise TypeError(f'Expected Point or Iterable[Point], found {type(point).__name__}.')
