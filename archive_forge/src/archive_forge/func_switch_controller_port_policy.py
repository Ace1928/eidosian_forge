...
def switch_controller_port_policy(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('switch-controller', 'port-policy', filtered_data, vdom=vdom)
    ...
    if state == 'present' or state is True:
        return fos.set('switch-controller', 'port-policy', data=converted_data, vdom=vdom)
    elif state == 'absent':
        return fos.delete('switch-controller', 'port-policy', mkey=filtered_data['name'], vdom=vdom)
    else:
