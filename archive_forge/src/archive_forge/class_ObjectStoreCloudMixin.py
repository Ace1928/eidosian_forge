...
class ObjectStoreCloudMixin:
    ...
    def delete_container(self, name):
        ...
        try:
            ...
            raise exceptions.SDKException('Attempt to delete container {container} failed. The container is not empty. Please delete the objects inside it before deleting the container'.format(container=name))
    ...
    def is_object_stale(self, container, name, filename, file_md5=None, file_sha256=None):
        ...
        """
        return self.object_store.is_object_stale(container, name, filename, file_md5=file_md5, file_sha256=file_sha256)
    ...
    def create_object(self, container, name, filename=None, md5=None, sha256=None, segment_size=None, use_slo=True, metadata=None, generate_checksums=None, data=None, **headers):
        ...
        :raises: :class:`~openstack.exceptions.SDKException` on operation
            error.
        """
        return self.object_store.create_object(container, name, filename=filename, data=data, md5=md5, sha256=sha256, use_slo=use_slo, generate_checksums=generate_checksums, metadata=metadata, **headers)
    ...
    def delete_object(self, container, name, meta=None):
        ...
        """
        try:
            self.object_store.delete_object(name, ignore_missing=False, container=container)
            return True
    ...
    def stream_object(self, container, obj, query_string=None, resp_chunk_size=1024):
        ...
        """
        try:
            for ret in self.object_store.stream_object(obj, container, chunk_size=resp_chunk_size):
                yield ret
        except exceptions.ResourceNotFound:
            return

    def get_object(self, container, obj, query_string=None, resp_chunk_size=1024, outfile=None, stream=False):
        ...
        """
        try:
            obj = self.object_store.get_object(obj, container=container, resp_chunk_size=resp_chunk_size, outfile=outfile, remember_content=outfile is None)
