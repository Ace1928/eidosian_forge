            ...
            if desired_state == 'present':
                if active_directory_info.enabled:
                    if active_directory_info.domainMembershipStatus == 'ok':
                        ...
                        results['result'][host.name]['msg'] = 'Host is joined to AD domain and there are no problems with the domain membership'
                    else:
                    ...
                    try:
                        try:
                            task = ad_authentication.JoinDomain(domainName=domain, userName=ad_user, password=ad_password)
                            wait_for_task(task)
                        except TaskError as task_err:
                            self.module.fail_json(msg='Failed to join domain : %s' % to_native(task_err))
                        changed = results['result'][host.name]['changed'] = True
                        results['result'][host.name]['ad_state_previous'] = 'absent'
                        results['result'][host.name]['ad_state_current'] = 'present'
                        ...
                        self.module.fail_json(msg='The host has already joined a domain : %s' % to_native(invalid_state.msg))
                    except vim.fault.HostConfigFault as host_fault:
                        self.module.fail_json(msg='The host configuration prevents the join operation from succeeding : %s' % to_native(host_fault.msg))
                    except vim.fault.InvalidLogin as invalid_login:
                        self.module.fail_json(msg="Credentials aren't valid : %s" % to_native(invalid_login.msg))
                    except vim.fault.TaskInProgress as task_in_progress:
                        self.module.fail_json(msg='The ActiveDirectoryAuthentication object is busy : %s' % to_native(task_in_progress.msg))
                    except vim.fault.BlockedByFirewall as blocked_by_firewall:
                        self.module.fail_json(msg='Ports needed by the join operation are blocked by the firewall : %s' % to_native(blocked_by_firewall.msg))
                    except vim.fault.DomainNotFound as not_found:
                        self.module.fail_json(msg="The domain controller can't be reached : %s" % to_native(not_found.msg))
                    except vim.fault.NoPermissionOnAD as no_permission:
                        self.module.fail_json(msg='The specified user has no right to add hosts to the domain : %s' % to_native(no_permission.msg))
                    except vim.fault.InvalidHostName as invalid_host:
                        self.module.fail_json(msg="The domain part of the host's FQDN doesn't match the domain being joined : %s" % to_native(invalid_host.msg))
                    except vim.fault.ClockSkew as clock_skew:
                        self.module.fail_json(msg='The clocks of the host and the domain controller differ by more than the allowed amount of time : %s' % to_native(clock_skew.msg))
                    except vim.fault.ActiveDirectoryFault as ad_fault:
                        self.module.fail_json(msg='An error occurred during AD join : %s' % to_native(ad_fault.msg))
                ...
                if not active_directory_info.enabled:
                    ...
                    results['result'][host.name]['ad_state_current'] = 'absent'
                    results['result'][host.name]['msg'] = "Host would leave the AD domain '%s'" % active_directory_info.joinedDomain
                else:
                    ad_authentication = self.get_ad_auth_object(host)
                    try:
                        try:
                        ...
                        results['result'][host.name]['ad_state_previous'] = 'present'
                        results['result'][host.name]['ad_state_current'] = 'absent'
                        results['result'][host.name]['msg'] = "Host left the AD domain '%s'" % active_directory_info.joinedDomain
                    except vim.fault.InvalidState as invalid_state:
                        self.module.fail_json(msg='The host is not in a domain or there are active permissions for Active Directory users : %s' % to_native(invalid_state.msg))
                    except vim.fault.AuthMinimumAdminPermission as admin_permission:
                        self.module.fail_json(msg='This change would leave the system with no Administrator permission on the root node : %s' % to_native(admin_permission.msg))
                    except vim.fault.TaskInProgress as task_in_progress:
                        self.module.fail_json(msg='The ActiveDirectoryAuthentication object is busy : %s' % to_native(task_in_progress.msg))
                    except vim.fault.NonADUserRequired as non_ad_user:
                        self.module.fail_json(msg='Only non Active Directory users can initiate the leave domain operation : %s' % to_native(non_ad_user.msg))
                    except vim.fault.ActiveDirectoryFault as ad_fault:
                        self.module.fail_json(msg='An error occurred during AD leave : %s' % to_native(ad_fault.msg))
