...
class TestQosBandwidthLimitRule(base.TestCase):
    ...
    rule_max_kbps = 1000
    rule_max_burst = 100
    mock_policy = {'id': policy_id, 'name': policy_name, 'description': '', 'rules': [], 'project_id': project_id, 'tenant_id': project_id, 'shared': False, 'is_default': False}
    mock_rule = {'id': rule_id, 'max_kbps': rule_max_kbps, 'max_burst_kbps': rule_max_burst, 'direction': 'egress'}
    qos_extension = {'updated': '2015-06-08T10:00:00-00:00', 'name': 'Quality of Service', 'links': [], 'alias': 'qos', 'description': 'The Quality of Service extension.'}
    qos_bw_limit_direction_extension = {'updated': '2017-04-10T10:00:00-00:00', 'name': 'Direction for QoS bandwidth limit rule', 'links': [], 'alias': 'qos-bw-limit-direction', 'description': 'Allow to configure QoS bandwidth limit rule with specific direction: ingress or egress'}
    enabled_neutron_extensions = [qos_extension, qos_bw_limit_direction_extension]

    def _compare_rules(self, exp, real):
        self.assertDictEqual(qos_bandwidth_limit_rule.QoSBandwidthLimitRule(**exp).to_dict(computed=False), real.to_dict(computed=False))

    def test_get_qos_bandwidth_limit_rule(self):
        self.register_uris([dict(method='GET', uri=self.get_mock_url('network', 'public', append=['v2.0', 'extensions']), json={'extensions': self.enabled_neutron_extensions}), dict(method='GET', uri=self.get_mock_url('network', 'public', append=['v2.0', 'qos', 'policies', self.policy_name]), status_code=404), dict(method='GET', uri=self.get_mock_url('network', 'public', append=['v2.0', 'qos', 'policies'], qs_elements=['name=%s' % self.policy_name]), json={'policies': [self.mock_policy]}), dict(method='GET', uri=self.get_mock_url('network', 'public', append=['v2.0', 'qos', 'policies', self.policy_id, 'bandwidth_limit_rules', self.rule_id]), json={'bandwidth_limit_rule': self.mock_rule})])
    ...
    def test_get_qos_bandwidth_limit_rule_no_qos_policy_found(self):
        ...
        self.assertRaises(exceptions.NotFoundException, self.cloud.get_qos_bandwidth_limit_rule, self.policy_name, self.rule_id)
        self.assert_calls()
    ...
    def test_get_qos_bandwidth_limit_rule_no_qos_extension(self):
        ...
        self.assertRaises(exceptions.SDKException, self.cloud.get_qos_bandwidth_limit_rule, self.policy_name, self.rule_id)
        self.assert_calls()

    def test_create_qos_bandwidth_limit_rule(self):
        self.register_uris([dict(method='GET', uri=self.get_mock_url('network', 'public', append=['v2.0', 'extensions']), json={'extensions': self.enabled_neutron_extensions}), dict(method='GET', uri=self.get_mock_url('network', 'public', append=['v2.0', 'qos', 'policies', self.policy_name]), status_code=404), dict(method='GET', uri=self.get_mock_url('network', 'public', append=['v2.0', 'qos', 'policies'], qs_elements=['name=%s' % self.policy_name]), json={'policies': [self.mock_policy]}), dict(method='POST', uri=self.get_mock_url('network', 'public', append=['v2.0', 'qos', 'policies', self.policy_id, 'bandwidth_limit_rules']), json={'bandwidth_limit_rule': self.mock_rule})])
        rule = self.cloud.create_qos_bandwidth_limit_rule(self.policy_name, max_kbps=self.rule_max_kbps)
    ...
    def test_create_qos_bandwidth_limit_rule_no_qos_direction_extension(self):
        ...
        rule = self.cloud.create_qos_bandwidth_limit_rule(self.policy_name, max_kbps=self.rule_max_kbps, direction='ingress')
        self._compare_rules(self.mock_rule, rule)
        self.assert_calls()
    ...
    def test_update_qos_bandwidth_limit_rule_no_qos_direction_extension(self):
        ...
        rule = self.cloud.update_qos_bandwidth_limit_rule(self.policy_id, self.rule_id, max_kbps=self.rule_max_kbps + 100, direction='ingress')
        self._compare_rules(expected_rule, rule)
        self.assert_calls()

    def test_delete_qos_bandwidth_limit_rule(self):
        self.register_uris([dict(method='GET', uri=self.get_mock_url('network', 'public', append=['v2.0', 'extensions']), json={'extensions': self.enabled_neutron_extensions}), dict(method='GET', uri=self.get_mock_url('network', 'public', append=['v2.0', 'qos', 'policies', self.policy_name]), status_code=404), dict(method='GET', uri=self.get_mock_url('network', 'public', append=['v2.0', 'qos', 'policies'], qs_elements=['name=%s' % self.policy_name]), json={'policies': [self.mock_policy]}), dict(method='DELETE', uri=self.get_mock_url('network', 'public', append=['v2.0', 'qos', 'policies', self.policy_id, 'bandwidth_limit_rules', self.rule_id]), json={})])
    ...
    def test_delete_qos_bandwidth_limit_rule_not_found(self):
        self.register_uris([dict(method='GET', uri=self.get_mock_url('network', 'public', append=['v2.0', 'extensions']), json={'extensions': self.enabled_neutron_extensions}), dict(method='GET', uri=self.get_mock_url('network', 'public', append=['v2.0', 'qos', 'policies', self.policy_name]), status_code=404), dict(method='GET', uri=self.get_mock_url('network', 'public', append=['v2.0', 'qos', 'policies'], qs_elements=['name=%s' % self.policy_name]), json={'policies': [self.mock_policy]}), dict(method='DELETE', uri=self.get_mock_url('network', 'public', append=['v2.0', 'qos', 'policies', self.policy_id, 'bandwidth_limit_rules', self.rule_id]), status_code=404)])
