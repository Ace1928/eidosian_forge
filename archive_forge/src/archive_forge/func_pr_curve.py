...
def pr_curve(y_true=None, y_probas=None, labels=None, classes_to_plot=None, interp_size=21, title=None, split_table: Optional[bool]=False):
    ...
    df = pd.DataFrame({'class': np.hstack([[k] * len(v) for k, v in precision.items()]), 'precision': np.hstack(list(precision.values())), 'recall': np.tile(interp_recall, len(precision))})
    df = df.round(3)
    if len(df) > wandb.Table.MAX_ROWS:
        wandb.termwarn('wandb uses only %d data points to create the plots.' % wandb.Table.MAX_ROWS)
        df = sklearn_utils.resample(df, replace=False, n_samples=wandb.Table.MAX_ROWS, random_state=42, stratify=df['class']).sort_values(['precision', 'recall', 'class'])
    table = wandb.Table(dataframe=df)
    title = title or 'Precision v. Recall'
    return wandb.plot_table('wandb/area-under-curve/v0', table, {'x': 'recall', 'y': 'precision', 'class': 'class'}, {'title': title}, split_table=split_table)