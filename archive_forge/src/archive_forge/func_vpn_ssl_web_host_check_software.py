...
def vpn_ssl_web_host_check_software(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('vpn.ssl.web', 'host-check-software', filtered_data, vdom=vdom)
        current_data = fos.get('vpn.ssl.web', 'host-check-software', vdom=vdom, mkey=mkey)
    ...
    if state == 'present' or state is True:
        return fos.set('vpn.ssl.web', 'host-check-software', data=filtered_data, vdom=vdom)
    elif state == 'absent':
        return fos.delete('vpn.ssl.web', 'host-check-software', mkey=filtered_data['name'], vdom=vdom)
    else:
