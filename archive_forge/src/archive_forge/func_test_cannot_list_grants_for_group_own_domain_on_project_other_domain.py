...
def test_cannot_list_grants_for_group_own_domain_on_project_other_domain(self):
    ...
    with self.test_client() as c:
        c.get('/v3/projects/%s/groups/%s/roles' % (project['id'], group['id']), headers=self.headers, expected_status_code=http.client.FORBIDDEN)