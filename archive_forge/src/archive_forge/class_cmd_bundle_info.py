...
class cmd_bundle_info(Command):
    ...
    def run(self, location, verbose=False):
        ...
        if isinstance(bundle_info, merge_directive.BaseMergeDirective):
            ...
            raise errors.CommandError(gettext('--verbose requires a merge directive'))
        reader_method = getattr(bundle_info, 'get_bundle_reader', None)
        if reader_method is None:
            raise errors.CommandError(gettext('Bundle format not supported'))
        by_kind = {}
        file_ids = set()
        for bytes, parents, repo_kind, revision_id, file_id in reader_method().iter_records():
            by_kind.setdefault(repo_kind, []).append((bytes, parents, repo_kind, revision_id, file_id))
            if file_id is not None:
                file_ids.add(file_id)
        self.outf.write(gettext('Records\n'))
        for kind, records in sorted(by_kind.items()):
            multiparent = sum((1 for b, m, k, r, f in records if len(m.get('parents', [])) > 1))
            self.outf.write(gettext('{0}: {1} ({2} multiparent)\n').format(kind, len(records), multiparent))
