...
def test_stack_show_output_error(self):
    ...
    self.patchobject(stack.outputs['test'], 'get_value', side_effect=[exception.EntityNotFound(entity='one', name='name')])
    output = self.eng.show_output(self.ctx, mock.ANY, 'test')
    self.assertEqual({'output_key': 'test', 'output_error': 'The one (name) could not be found.', 'description': 'sec', 'output_value': None}, output)