...
def rope_padded(xq: torch.Tensor, xk: torch.Tensor, xv: torch.Tensor, cache_k: torch.Tensor, cache_v: torch.Tensor, attn_bias: BlockDiagonalCausalWithOffsetPaddedKeysMask, *, theta: float=10000.0, out_q: Optional[torch.Tensor]=None, adjacents: bool=True, internal_dtype: str=''):
    """
    ...
    """
    if torch.is_grad_enabled() and (xq.requires_grad or xk.requires_grad or xv.requires_grad or cache_k.requires_grad or cache_v.requires_grad or (out_q is not None)):
    ...
    if ndim == 4:
        ...
        if xk_shape != (1, n_total_queries, expected_kv_heads, dim):
            raise ValueError(f'unexpected k shape {xk_shape}: expected {(1, n_total_queries, expected_kv_heads, dim)}')
        if xv.shape != (1, n_total_queries, expected_kv_heads, dim):
            raise ValueError(f'unexpected v shape {xv.shape}: expected {(1, n_total_queries, expected_kv_heads, dim)}')
        ...
        if xk_shape != (1, n_total_queries, n_groups, expected_kv_heads, dim):
            raise ValueError(f'unexpected k shape {xk_shape}: expected {(1, n_total_queries, n_groups, expected_kv_heads, dim)}')
        if xv.shape != (1, n_total_queries, n_groups, expected_kv_heads, dim):
            raise ValueError(f'unexpected v shape {xv.shape}: expected {(1, n_total_queries, n_groups, expected_kv_heads, dim)}')
        if cache_k_shape != (1, cache_length, n_groups, expected_cache_heads, dim):
            raise ValueError(f'unexpected cache_k shape {cache_k_shape}: expected {(1, cache_length, n_groups, expected_cache_heads, dim)}')
        if cache_v.shape != (1, cache_length, n_groups, expected_cache_heads, dim):
            raise ValueError(f'unexpected cache_v shape {cache_v.shape}: expected {(1, cache_length, n_groups, expected_cache_heads, dim)}')
        out_q_stride = (0, n_q_heads * dim * n_groups, n_q_heads * dim, dim, 1)
    if bsz != 1:
        raise ValueError('Expected batch size dimension to be 1 as batches should be concatenated.')
    if xq_stride[-1] != 1:
