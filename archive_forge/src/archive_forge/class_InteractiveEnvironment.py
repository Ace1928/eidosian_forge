...
class InteractiveEnvironment(BaseEnvironment):
    ...
    def __init__(self, user_color=Colors.GREEN, bot_color=Colors.YELLOW, special_color=Colors.BLUE, system_color=Colors.CYAN):
        super().__init__()
    ...
    def start(self, agent: BaseAgent):
        cprint(f"\n[SYSTEM]: Let's talk with [{agent.name.upper()}].\n[SYSTEM]: Enter '.exit', if you want to exit chatting.\n[SYSTEM]: Enter '.reset', if you want reset all histories.\n", color=self.system_color)
        ...
        while True:
            ...
            if isinstance(agent, PromptAgent):
                ...
                user_message = cinput(f'[{user_name.upper()}]: ', color=self.user_color)
            else:
                user_message = cinput('[USER]: ', color=self.user_color)
            if user_message == '.exit':
                cprint(f'[SYSTEM]: good bye.\n', color=self.system_color)
                exit(0)
                sys.exit(0)
            if user_message == '.reset':
                cprint(f'[SYSTEM]: reset all histories.\n', color=self.system_color)
                self.clear_histories(self.user_id)
                continue
            ...
            if isinstance(agent, SingleTurn):
                ...
                model_input = self.make_model_input(self.user_id, user_message, agent)
            self.add_user_message(self.user_id, user_message)
            if isinstance(agent, PromptAgent):
                bot_message = agent.predict(model_input, person_1=user_name, person_2=bot_name)['output']
            else:
                bot_message = agent.predict(model_input)['output']
            cprint(f'[{agent.name.upper()}]: {bot_message}', color=self.bot_color)
    ...
    def pre_dialog_for_convai2(self, agent):
        cprint(f"[SYSTEM]: Please input [{agent.name.upper()}]'s perosna.\n[SYSTEM]: Enter '.done' if you want to end input persona.\n", color=self.system_color)
        while True:
            _persona = cinput(f"[{agent.name.upper()}'s PERSONA]: ", color=self.special_color)
            if _persona == '.done':
                cprint(f"[{agent.name.upper()}'s PERSONA]: Persona setting complete.\n", color=self.special_color)
                break
            else:
                agent.add_persona(self.histories, user_id=self.user_id, text=_persona)
