...
class Normalizer(Preprocessor):
    ...
    * `"max"` (:math:`L^\\infty`): Maximum value.
    ...
    """
    _norm_fns = {'l1': lambda cols: np.abs(cols).sum(axis=1), 'l2': lambda cols: np.sqrt(np.power(cols, 2).sum(axis=1)), 'max': lambda cols: np.max(abs(cols), axis=1)}
    _is_fittable = False
    ...
    def __init__(self, columns: List[str], norm='l2'):
        ...
        if norm not in self._norm_fns:
            raise ValueError(f'Norm {norm} is not supported.Supported values are: {self._norm_fns.keys()}')
    ...
    def __repr__(self):
        return f'{self.__class__.__name__}(columns={self.columns!r}, norm={self.norm!r})'