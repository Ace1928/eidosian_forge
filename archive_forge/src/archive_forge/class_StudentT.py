  ...
  * `Gamma` is the [gamma function](
  ...
  """
    ...
    def _param_shapes(sample_shape):
        return dict(zip(('df', 'loc', 'scale'), [ops.convert_to_tensor(sample_shape, dtype=dtypes.int32)] * 3))
    ...
    def _batch_shape_tensor(self):
        return array_ops.broadcast_dynamic_shape(array_ops.shape(self.df), array_ops.broadcast_dynamic_shape(array_ops.shape(self.loc), array_ops.shape(self.scale)))

    def _batch_shape(self):
        return array_ops.broadcast_static_shape(array_ops.broadcast_static_shape(self.df.get_shape(), self.loc.get_shape()), self.scale.get_shape())
    ...
    def _log_normalization(self):
        return math_ops.log(math_ops.abs(self.scale)) + 0.5 * math_ops.log(self.df) + 0.5 * np.log(np.pi) + math_ops.lgamma(0.5 * self.df) - math_ops.lgamma(0.5 * (self.df + 1.0))
    ...
    def _entropy(self):
        v = array_ops.ones(self.batch_shape_tensor(), dtype=self.dtype)[..., array_ops.newaxis]
        u = v * self.df[..., array_ops.newaxis]
        beta_arg = array_ops.concat([u, v], -1) / 2.0
        return math_ops.log(math_ops.abs(self.scale)) + 0.5 * math_ops.log(self.df) + special_math_ops.lbeta(beta_arg) + 0.5 * (self.df + 1.0) * (math_ops.digamma(0.5 * (self.df + 1.0)) - math_ops.digamma(0.5 * self.df))

    @distribution_util.AppendDocstring("The mean of Student's T equals `loc` if `df > 1`, otherwise it is\n      `NaN`. If `self.allow_nan_stats=True`, then an exception will be raised\n      rather than returning `NaN`.")
    def _mean(self):
        mean = self.loc * array_ops.ones(self.batch_shape_tensor(), dtype=self.dtype)
        if self.allow_nan_stats:
            nan = np.array(np.nan, dtype=self.dtype.as_numpy_dtype())
            return array_ops.where_v2(math_ops.greater(self.df, array_ops.ones(self.batch_shape_tensor(), dtype=self.dtype)), mean, array_ops.fill(self.batch_shape_tensor(), nan, name='nan'))
        else:
            return control_flow_ops.with_dependencies([check_ops.assert_less(array_ops.ones([], dtype=self.dtype), self.df, message='mean not defined for components of df <= 1')], mean)

    @distribution_util.AppendDocstring("\n      The variance for Student's T equals\n\n      ```\n      df / (df - 2), when df > 2\n      infinity, when 1 < df <= 2\n      NaN, when df <= 1\n      ```\n      ")
    def _variance(self):
        denom = array_ops.where_v2(math_ops.greater(self.df, 2.0), self.df - 2.0, array_ops.ones_like(self.df))
        var = array_ops.ones(self.batch_shape_tensor(), dtype=self.dtype) * math_ops.square(self.scale) * self.df / denom
        inf = np.array(np.inf, dtype=self.dtype.as_numpy_dtype())
        result_where_defined = array_ops.where_v2(self.df > array_ops.fill(self.batch_shape_tensor(), 2.0), var, array_ops.fill(self.batch_shape_tensor(), inf, name='inf'))
        if self.allow_nan_stats:
            nan = np.array(np.nan, dtype=self.dtype.as_numpy_dtype())
            return array_ops.where_v2(math_ops.greater(self.df, array_ops.ones(self.batch_shape_tensor(), dtype=self.dtype)), result_where_defined, array_ops.fill(self.batch_shape_tensor(), nan, name='nan'))
        else:
            return control_flow_ops.with_dependencies([check_ops.assert_less(array_ops.ones([], dtype=self.dtype), self.df, message='variance not defined for components of df <= 1')], result_where_defined)
