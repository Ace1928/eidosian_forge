...
def matrix_diag_part_v3(input: _atypes.TensorFuzzingAnnotation[TV_MatrixDiagPartV3_T], k: _atypes.TensorFuzzingAnnotation[_atypes.Int32], padding_value: _atypes.TensorFuzzingAnnotation[TV_MatrixDiagPartV3_T], align: str='RIGHT_LEFT', name=None) -> _atypes.TensorFuzzingAnnotation[TV_MatrixDiagPartV3_T]:
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'MatrixDiagPartV3', name, input, k, padding_value, 'align', align)
            return _result
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('MatrixDiagPartV3', input=input, k=k, padding_value=padding_value, align=align, name=name)
    _result = _outputs[:]
