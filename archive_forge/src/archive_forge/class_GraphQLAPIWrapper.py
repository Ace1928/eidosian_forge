...
class GraphQLAPIWrapper(BaseModel):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        try:
            ...
            raise ImportError(f'Could not import gql python package. Try installing it with `pip install gql`. Received error: {e}')
        headers = values.get('custom_headers')
        transport = RequestsHTTPTransport(url=values['graphql_endpoint'], headers=headers)
