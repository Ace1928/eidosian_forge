...
class _TestComponentDictBase(object):
    ...
    def test_init2(self):
        ...
        with self.assertRaises(TypeError):
            model.d = self._ctype(*tuple(((i, self._cdatatype(self._arg())) for i in index)))
    ...
    def test_keys(self):
        ...
        self.assertEqual(sorted(list(raw_constraint_dict.keys()), key=str), sorted(list(model.c.keys()), key=str))
    ...
    def test_values(self):
        ...
        raw_constraint_dict = {i: self._cdatatype(self._arg()) for i in index}
        model.c = self._ctype(raw_constraint_dict)
        self.assertEqual(sorted(list((id(_v) for _v in raw_constraint_dict.values())), key=str), sorted(list((id(_v) for _v in model.c.values())), key=str))
    ...
    def test_items(self):
        ...
        raw_constraint_dict = {i: self._cdatatype(self._arg()) for i in index}
        model.c = self._ctype(raw_constraint_dict)
        self.assertEqual(sorted(list(((_i, id(_v)) for _i, _v in raw_constraint_dict.items())), key=str), sorted(list(((_i, id(_v)) for _i, _v in model.c.items())), key=str))
