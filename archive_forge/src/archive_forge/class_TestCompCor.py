...
class TestCompCor:
    ...
    filenames = {'functionalnii': 'compcorfunc.nii', 'masknii': 'compcormask.nii', 'masknii2': 'compcormask2.nii', 'components_file': None}

    @pytest.fixture(autouse=True)
    def setup_class(self, tmpdir):
        tmpdir.chdir()
        noise = np.fromfunction(self.fake_noise_fun, self.fake_data.shape)
        self.realigned_file = utils.save_toy_nii(self.fake_data + noise, self.filenames['functionalnii'])
    ...
    def test_tcompcor_merge_intersect_masks(self):
        for method in ['union', 'intersect']:
            TCompCor(realigned_file=self.realigned_file, mask_files=self.mask_files, merge_method=method).run()
            if method == 'union':
                assert np.array_equal(nb.load('mask_000.nii.gz').dataobj, [[[0, 0], [0, 0]], [[0, 0], [1, 0]]])
            if method == 'intersect':
    ...
    def fake_noise_fun(i, j, l, m):
        return m * i + l - j
    fake_data = np.array([[[[8, 5, 3, 8, 0], [6, 7, 4, 7, 1]], [[7, 9, 1, 6, 5], [0, 7, 4, 7, 7]]], [[[2, 4, 5, 7, 0], [1, 7, 0, 5, 4]], [[7, 3, 9, 0, 4], [9, 4, 1, 5, 0]]]])