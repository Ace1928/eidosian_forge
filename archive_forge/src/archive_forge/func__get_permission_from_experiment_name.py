...
def _get_permission_from_experiment_name() -> Permission:
    ...
    if store_exp is None:
        raise MlflowException(f'Could not find experiment with name {experiment_name}', error_code=RESOURCE_DOES_NOT_EXIST)
    username = authenticate_request().username
    return _get_permission_from_store_or_default(lambda: store.get_experiment_permission(store_exp.experiment_id, username).permission)