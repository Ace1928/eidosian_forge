...
class NovaKeyPairTest(common.HeatTestCase):
    kp_template = {'heat_template_version': '2013-05-23', 'resources': {'kp': {'type': 'OS::Nova::KeyPair', 'properties': {'name': 'key_pair'}}}}
    ...
    def setUp(self):
        ...
        self.cp_mock = self.patchobject(nova.NovaClientPlugin, 'client', return_value=self.fake_nova)
    ...
    def test_create_key(self):
        ...
        self.patchobject(created_key, 'to_dict', return_value=key_info)
        scheduler.TaskRunner(tp_test.create)()
        self.fake_keypairs.create.assert_called_once_with(name=key_name, public_key=None)
    ...
    def test_create_key_empty_name(self):
        ...
        self.assertIn('kp.properties.name: length (0) is out of range (min: 1, max: 255)', str(error))
    ...
    def _test_validate(self, key_type=None, user=None):
        ...
        kp_res = keypair.KeyPair('kp', definition, stack)
        error = self.assertRaises(exception.StackValidationFailed, kp_res.validate)
        msg = 'Cannot use "%s" properties - nova does not support required api microversion.' % validate_props
    ...
    def test_delete_key_not_found(self):
        ...
        self.patchobject(self.fake_keypairs, 'delete', side_effect=fakes_nova.fake_exception())
