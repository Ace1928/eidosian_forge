...
class MA57Interface(object):
    ...
    def __init__(self, work_factor=None, fact_factor=None, ifact_factor=None):
        ...
        self.lib.do_symbolic_factorization.argtypes = [ctypes.c_void_p, ctypes.c_int, ctypes.c_int, array_1d_int, array_1d_int]
        self.lib.do_numeric_factorization.argtypes = [ctypes.c_void_p, ctypes.c_int, ctypes.c_int, array_1d_double]
        self.lib.do_backsolve.argtypes = [ctypes.c_void_p, ctypes.c_int, array_2d_double]
        self.lib.do_iterative_refinement.argtypes = [ctypes.c_void_p, ctypes.c_int, ctypes.c_int, array_1d_double, array_1d_int, array_1d_int, array_1d_double, array_1d_double, array_1d_double]
        self.lib.do_reallocation.argtypes = [ctypes.c_void_p, ctypes.c_int, ctypes.c_double, ctypes.c_int]
        self.icntl_len = 20
        self.cntl_len = 5
        self.info_len = 40
        self.rinfo_len = 20
    ...
    def do_numeric_factorization(self, dim, entries):
        ...
        ne = entries.size
        assert ne == self.ne_cached, 'Wrong number of entries in matrix. Please re-run symbolicfactorization with correct nonzero coordinates.'
        assert dim == self.dim_cached, 'Dimension mismatch between symbolic and numeric factorization.Please re-run symbolic factorization with the correct dimension.'
