...
def GetLocalSsdFlagWithCount(custom_name=None):
    ...
    return base.Argument(custom_name if custom_name else '--local-ssd', type=arg_parsers.ArgDict(spec={'interface': lambda x: x.upper(), 'size': int, 'count': int}), action='append', help=help_text)