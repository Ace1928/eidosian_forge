...
@ConsumptionAPI
def write_numpy(self, path: str, *, column: str, filesystem: Optional['pyarrow.fs.FileSystem']=None, try_create_dir: bool=True, arrow_open_stream_args: Optional[Dict[str, Any]]=None, filename_provider: Optional[FilenameProvider]=None, block_path_provider: Optional[BlockWritePathProvider]=None, ray_remote_args: Dict[str, Any]=None) -> None:
    """Writes a column of the :class:`~ray.data.Dataset` to .npy files.
        ...
        """
    datasink = _NumpyDatasink(path, column, filesystem=filesystem, try_create_dir=try_create_dir, open_stream_args=arrow_open_stream_args, filename_provider=filename_provider, block_path_provider=block_path_provider, dataset_uuid=self._uuid)
