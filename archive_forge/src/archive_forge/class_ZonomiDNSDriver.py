...
class ZonomiDNSDriver(DNSDriver):
    ...
    def _to_zone(self, item):
        if item['type'] == 'NATIVE':
            ...
            type = 'slave'
        zone = Zone(id=item['name'], domain=item['name'], type=type, driver=self, extra={}, ttl=None)
        return zone
    ...
    def _to_record(self, item, zone):
        ...
        if len(item['name']) > len(zone.domain):
            ...
            record_name = zone.domain
        record = Record(id=record_name, name=record_name, data=item['content'], type=item['type'], zone=zone, driver=self, ttl=ttl, extra=extra)
        return record
