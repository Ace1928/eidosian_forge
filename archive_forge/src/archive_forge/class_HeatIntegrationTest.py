...
class HeatIntegrationTest(testscenarios.WithScenarios, testtools.TestCase):
    ...
    def _load_template(self, base_file, file_name, sub_dir=None):
        sub_dir = sub_dir or ''
        filepath = os.path.join(os.path.dirname(os.path.realpath(base_file)), sub_dir, file_name)
    ...
    def is_service_available(self, service_type):
        try:
            self.identity_client.get_endpoint_url(service_type, self.conf.region)
    ...
    def _wait_for_resource_status(self, stack_identifier, resource_name, status, failure_pattern='^.*_FAILED$', success_on_not_found=False):
        ...
        while timeutils.delta_seconds(start, timeutils.utcnow()) < build_timeout:
            try:
                ...
                if resource_action == wait_for_action and fail_regexp.search(res.resource_status):
                    raise exceptions.StackResourceBuildErrorException(resource_name=res.resource_name, stack_identifier=stack_identifier, resource_status=res.resource_status, resource_status_reason=res.resource_status_reason)
            time.sleep(build_interval)
        message = 'Resource %s from stack %s failed to reach %s status within the required time (%s s). Current resource status: %s.' % (resource_name, stack_identifier, status, build_timeout, res.resource_status)
    ...
    def update_stack(self, stack_identifier, template=None, environment=None, files=None, parameters=None, tags=None, expected_status='UPDATE_COMPLETE', disable_rollback=True, existing=False):
        env = environment or {}
        env_files = files or {}
        parameters = parameters or {}
        self._handle_in_progress(self.client.stacks.update, stack_id=stack_identifier, template=template, files=env_files, disable_rollback=disable_rollback, parameters=parameters, environment=env, tags=tags, existing=existing)
        kwargs = {'stack_identifier': stack_identifier, 'status': expected_status}
        if expected_status in ['ROLLBACK_COMPLETE']:
            kwargs['failure_pattern'] = '^ROLLBACK_FAILED$'
        self._wait_for_stack_status(**kwargs)

    def cancel_update_stack(self, stack_identifier, rollback=True, expected_status='ROLLBACK_COMPLETE'):
    ...
    def preview_update_stack(self, stack_identifier, template, environment=None, files=None, parameters=None, tags=None, disable_rollback=True, show_nested=False):
        env = environment or {}
        env_files = files or {}
        parameters = parameters or {}
        return self.client.stacks.preview_update(stack_id=stack_identifier, template=template, files=env_files, disable_rollback=disable_rollback, parameters=parameters, environment=env, tags=tags, show_nested=show_nested)
    ...
    def list_resources(self, stack_identifier, filter_func=None):
        ...
        return dict(((r.resource_name, r.resource_type) for r in resources if (filter_func(r) if callable(filter_func) else True)))
    ...
    def stack_create(self, stack_name=None, template=None, files=None, parameters=None, environment=None, tags=None, expected_status='CREATE_COMPLETE', disable_rollback=True, enable_cleanup=True, environment_files=None, timeout=None):
        ...
        self.client.stacks.create(stack_name=name, template=templ, files=templ_files, disable_rollback=disable_rollback, parameters=params, environment=env, tags=tags, environment_files=environment_files, timeout_mins=timeout_mins)
        if enable_cleanup:
    ...
    def stack_abandon(self, stack_id):
        if self.conf.skip_test_stack_action_list and 'ABANDON' in self.conf.skip_test_stack_action_list:
    ...
    def stack_snapshot(self, stack_id, wait_for_status='SNAPSHOT_COMPLETE'):
        snapshot = self.client.stacks.snapshot(stack_id=stack_id)
        self._wait_for_stack_status(stack_id, wait_for_status)
        return snapshot['id']

    def stack_restore(self, stack_id, snapshot_id, wait_for_status='RESTORE_COMPLETE'):
        self.client.stacks.restore(stack_id, snapshot_id)
        self._wait_for_stack_status(stack_id, wait_for_status)

    def stack_suspend(self, stack_identifier):
        if self.conf.skip_test_stack_action_list and 'SUSPEND' in self.conf.skip_test_stack_action_list:
        ...
        self._wait_for_all_resource_status(stack_identifier, 'SUSPEND_COMPLETE')
        self._wait_for_stack_status(stack_identifier, 'SUSPEND_COMPLETE')

    def stack_resume(self, stack_identifier):
        if self.conf.skip_test_stack_action_list and 'RESUME' in self.conf.skip_test_stack_action_list:
            self.addCleanup(self._stack_delete, stack_identifier)
        ...
        self._wait_for_all_resource_status(stack_identifier, 'RESUME_COMPLETE')
