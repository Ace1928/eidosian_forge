...
class TestMapboxTilesPlot(TestPlotlyPlot):
    ...
    def test_overlay(self):
        ...
        rgb = RGB(rgb_data, bounds=(self.x_range[0], self.y_range[0], self.x_range[1], self.y_range[1])).opts(opacity=0.5)
        points = Points([(0, 0), (self.x_range[1], self.y_range[1])]).opts(show_legend=True)
        bounds = Bounds((self.x_range[0], self.y_range[0], 0, 0))
        overlay = (tiles * stamen_raster * rgb * points * bounds).redim.range(x=self.x_range, y=self.y_range)
        ...
        self.assertEqual(bounds_trace['lon'], np.array([self.lon_range[0], self.lon_range[0], 0, 0, self.lon_range[0]]))
        self.assertEqual(bounds_trace['lat'], np.array([self.lat_range[0], 0, 0, self.lat_range[0], self.lat_range[0]]))
