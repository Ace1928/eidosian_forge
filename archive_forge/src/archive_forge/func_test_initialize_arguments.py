...
def test_initialize_arguments():
    ...
    class Component:
        ...
        def initialize(self, get_examples, nlp, custom1: str, custom2: StrictBool=False):
    ...
    init_cfg = {'tokenizer': {'custom': 1}, 'components': {name: {'custom1': 'x', 'custom2': 1}}}
    nlp.config['initialize'].update(init_cfg)
    with pytest.raises(ConfigValidationError) as e:
        nlp.initialize(get_examples)
    errors = e.value.errors
    ...
    init_cfg = {'tokenizer': {'custom': 1}, 'components': {name: {'custom1': 'x'}}}
    nlp.config['initialize'].update(init_cfg)
    nlp.initialize(get_examples)
