...
class DuckDB(VectorStore):
    ...
    *Security Notice*: The default DuckDB configuration is not secure.
    ...
    def __init__(self, *, connection: Optional[Any]=None, embedding: Embeddings, vector_key: str='embedding', id_key: str='id', text_key: str='text', table_name: str='vectorstore'):
    ...
    def similarity_search(self, query: str, k: int=4, **kwargs: Any) -> List[Document]:
        ...
        docs = self._table.select(*[self.duckdb.StarExpression(exclude=[]), list_cosine_similarity.alias('similarity')]).order('similarity desc').limit(k).select(self.duckdb.StarExpression(exclude=['similarity', self._vector_key])).fetchdf()
        return [Document(page_content=docs[self._text_key][idx], metadata=json.loads(docs['metadata'][idx]) if docs['metadata'][idx] else {}) for idx in range(len(docs))]

    @classmethod
    def from_texts(cls: Type[VST], texts: List[str], embedding: Embeddings, metadatas: Optional[List[dict]]=None, **kwargs: Any) -> DuckDB:
        ...
        instance = DuckDB(connection=connection, embedding=embedding, vector_key=vector_key, id_key=id_key, text_key=text_key, table_name=table_name)
