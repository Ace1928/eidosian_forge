...
class GPTNeoAgent(PromptAgent, DecoderLM):
    ...
    def __init__(self, model, device, maxlen):
        ...
        super().__init__(name=model, suffix=' ', device=device, maxlen=maxlen, model=AutoModelForCausalLM.from_pretrained(name).to(device).eval(), tokenizer=tokenizer)

    @staticmethod
    def available_models():
        return ['gptneo.small', 'gptneo.medium', 'gptneo.large', 'gptneo.xlarge']
