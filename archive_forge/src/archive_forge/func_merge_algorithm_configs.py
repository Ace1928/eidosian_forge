...
@classmethod
def merge_algorithm_configs(cls, config1: AlgorithmConfigDict, config2: PartialAlgorithmConfigDict, _allow_unknown_configs: Optional[bool]=None) -> AlgorithmConfigDict:
    ...
    if 'callbacks' in config2 and type(config2['callbacks']) is dict:
        deprecation_warning('callbacks dict interface', 'a class extending rllib.algorithms.callbacks.DefaultCallbacks; see `rllib/examples/custom_metrics_and_callbacks.py` for an example.', error=True)
    if _allow_unknown_configs is None:
        _allow_unknown_configs = cls._allow_unknown_configs
    return deep_update(config1, config2, _allow_unknown_configs, cls._allow_unknown_subkeys, cls._override_all_subkeys_if_type_changes, cls._override_all_key_list)