...
@mock.patch.object(constants, 'IMPERSONATE_SERVICE_ACCOUNT', new='fake_service_account')
def test_impersonate_service_account_translation(self):
    ...
    with _mock_boto_config({'GSUtil': {'use_gcloud_storage': 'always', 'hidden_shim_mode': 'no_fallback'}}):
        with util.SetEnvironmentForTest({'CLOUDSDK_CORE_PASS_CREDENTIALS_TO_GSUTIL': 'True', 'CLOUDSDK_ROOT_DIR': 'fake_dir'}):
            ...
            self.assertEqual(self._fake_command._translated_gcloud_storage_command, [shim_util._get_gcloud_binary_path('fake_dir'), 'objects', 'fake', '--zip', 'opt1', '-x', 'arg1', 'arg2', '--impersonate-service-account=fake_service_account'])