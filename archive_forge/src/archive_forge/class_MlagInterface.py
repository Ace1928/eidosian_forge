...
class MlagInterface(object):
    ...
    def get_mlag_error_down_info(self):
        ...
        if '<data/>' in xml_str:
            ...
            root = ElementTree.fromstring(xml_str)
            mlag_error_infos = root.findall('./mlag/errordowns/errordown')
            if mlag_error_infos:
    ...
    def check_params(self):
        ...
        if self.mlag_id:
            if not self.mlag_id.isdigit():
                self.module.fail_json(msg='Error: The value of mlag_id is an integer.')
            if int(self.mlag_id) < 1 or int(self.mlag_id) > 2048:
                self.module.fail_json(msg='Error: The value of mlag_id is not in the range from 1 to 2048.')
        if self.mlag_system_id:
            if not self.check_macaddr():
                self.module.fail_json(msg='Error: mlag_system_id has invalid value %s.' % self.mlag_system_id)
        if self.mlag_priority_id:
            if not self.mlag_priority_id.isdigit():
                self.module.fail_json(msg='Error: The value of mlag_priority_id is an integer.')
            if int(self.mlag_priority_id) < 0 or int(self.mlag_priority_id) > 254:
                self.module.fail_json(msg='Error: The value of mlag_priority_id is not in the range from 0 to 254.')
        if self.interface:
