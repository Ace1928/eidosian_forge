...
class JobSubmissionClient(SubmissionClient):
    ...
    def __init__(self, address: Optional[str]=None, create_cluster_if_needed: bool=False, cookies: Optional[Dict[str, Any]]=None, metadata: Optional[Dict[str, Any]]=None, headers: Optional[Dict[str, Any]]=None, verify: Optional[Union[str, bool]]=True):
        ...
        if requests is None:
            raise RuntimeError("The Ray jobs CLI & SDK require the ray[default] installation: `pip install 'ray[default]'`")
        if address is not None and (not isinstance(address, str)):
            raise TypeError(f'address must be a string, got {type(address)}')
        if not isinstance(create_cluster_if_needed, bool):
            raise TypeError(f'create_cluster_if_needed must be a bool, got {type(create_cluster_if_needed)}')
        ...
        super().__init__(address=api_server_url, create_cluster_if_needed=create_cluster_if_needed, cookies=cookies, metadata=metadata, headers=headers, verify=verify)
        self._check_connection_and_version(min_version='1.9', version_error_message='Jobs API is not supported on the Ray cluster. Please ensure the cluster is running Ray 1.9 or higher.')
        if packaging.version.parse(self._client_ray_version) > packaging.version.parse('2.0'):
            self._check_connection_and_version(min_version='2.0', version_error_message=f'Client Ray version {self._client_ray_version} is not compatible with the Ray cluster. Please ensure the cluster is running Ray 2.0 or higher or downgrade the client Ray version.')
    ...
    def list_jobs(self) -> List[JobDetails]:
        ...
        if r.status_code == 200:
            ...
            jobs_info = [JobDetails(**job_info_json) for job_info_json in jobs_info_json]
            return jobs_info
        else:
            self._raise_error(r)
    ...
    async def tail_job_logs(self, job_id: str) -> Iterator[str]:
        ...
        Raises:
            RuntimeError: If the job does not exist or if the request to the
                job server fails.
        """
        async with aiohttp.ClientSession(cookies=self._cookies, headers=self._headers) as session:
            ws = await session.ws_connect(f'{self._address}/api/jobs/{job_id}/logs/tail', ssl=self._ssl_context)
            while True:
