...
async def retrieve_compressed_image(hash: str) -> Optional[tuple]:
    ...
    query = "SELECT compressed_data FROM images WHERE hash = ?"
    try:
        async with get_db_connection() as db_connection:
...
class TestImageInsertion(BaseDatabaseTestCase):
    async def asyncSetUp(self):
        ...
        with open(
            "/home/lloyd/EVIE/scripts/image_interconversion_gui/test_image.png", "rb"
        ) as image_file:
...
class TestImageRetrieval(BaseDatabaseTestCase):
    ...
    async def test_retrieve_compressed_image_success(self):
        ...
        result = await retrieve_compressed_image("hash123")
...
# - Looking into ORM for more efficient database mappings
# - Identify if SQLAlchemy would be beneficial for the project
# - Implement a method to migrate between database types (ie mongo, sql, etc.) and allow for universal interoperability between, potential in a seperate database conversion module.
# - Implement a method to save the database to a file, allowing for easy backups and migrations.
# - Implement a method to load the database from a file, allowing for easy backups and migrations.
# - Implement a method to encrypt the database, allowing for secure storage of sensitive information.
# - Implement a method to decrypt the database, allowing for secure storage of sensitive information.
# - Implement a method to compress the database, allowing for efficient storage of large amounts of data.
# - Implement a method to decompress the database, allowing for efficient storage of large amounts of data.
# - Implement a method to validate the database, ensuring that it is in a consistent state.
# - Implement a method to repair the database, ensuring that it is in a consistent state.
# - Implement a method to optimize the database, ensuring that it is in a performant state.
