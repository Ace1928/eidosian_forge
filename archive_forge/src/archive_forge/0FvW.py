    ...
    """
        ...
        def _rank_all(self) -> None:
            ...
            for key in self.memory:
                self.memory[key] = self.memory[key][0], self.memory[key][1], self.memory[key][2], self._rank(key) # Update the ranking value
                ranked_memory = sorted(self.memory.items(), key=lambda x: x[1][3], reverse=True) # Sort the memory based on the ranking value
                self.memory = dict(ranked_memory) # Update the memory with the sorted values
            return None
