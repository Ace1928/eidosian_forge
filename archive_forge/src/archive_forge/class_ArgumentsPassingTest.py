...
class ArgumentsPassingTest(utils.EngineTestBase):
    ...
    def test_save_dict(self):
        flow = utils.TaskMultiDict(provides=set(['badger', 'mushroom', 'snake']))
        engine = self._make_engine(flow)
        engine.run()
        self.assertEqual({'badger': 0, 'mushroom': 1, 'snake': 2}, engine.storage.fetch_all())
    ...
    def test_argument_injection(self):
        flow = utils.TaskMultiArgOneReturn(provides='result', inject={'x': 1, 'y': 4, 'z': 9})
        engine = self._make_engine(flow)
        engine.run()
