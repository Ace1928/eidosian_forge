...
from .request import (
def imopen(uri, io_mode, *, plugin=None, extension=None, format_hint=None, legacy_mode=False, **kwargs):
    ...
    **kwargs : Any
    ...
    if isinstance(uri, Request) and legacy_mode:
        warnings.warn('`iio.core.Request` is a low-level object and using it directly as input to `imopen` is discouraged. This will raise an exception in ImageIO v3.', DeprecationWarning, stacklevel=2)
        request = uri
        uri = request.raw_uri
    ...
    if plugin is not None:
        ...
        try:
            ...
            err_msg = f'The `{config.name}` plugin is not installed. Use `pip install imageio[{config.install_name}]` to install it.'
    ...
    if request.format_hint is not None:
        for candidate_format in known_extensions[format_hint]:
            ...
            resource = '<bytes>' if isinstance(request.raw_uri, bytes) else request.raw_uri
    ...
    if request._uri_type == URI_FILENAME and Path(request.raw_uri).is_dir():
        request.finish()
        err_type = ValueError if legacy_mode else IOError
        err_msg = "ImageIO does not generally support reading folders. Limited support may be available via specific plugins. Specify the plugin explicitly using the `plugin` kwarg, e.g. `plugin='DICOM'`"
        raise err_type(err_msg)
    request.finish()
    ...
    if request.extension in known_extensions:
        ...
        plugin_names = [plugin for file_format in formats for plugin in file_format.priority]
        ...
        if len(missing_plugins) > 0:
            install_candidates = '\n'.join([f'  {config.name}:  pip install imageio[{config.install_name}]' for config in missing_plugins])
