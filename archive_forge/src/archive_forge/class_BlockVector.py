...
class BlockVector(np.ndarray, BaseBlockVector):
    ...
    def all(self, axis=None, out=None, keepdims=False):
        ...
        """
        assert_block_structure(self)
        results = np.array([self.get_block(i).all() for i in range(self.nblocks)], dtype=bool)
    ...
    def any(self, axis=None, out=None, keepdims=False):
        ...
        """
        assert_block_structure(self)
        results = np.array([self.get_block(i).any() for i in range(self.nblocks)], dtype=bool)
        ...
        if copy:
            ...
            for bid, vv in enumerate(self):
                if bid not in self._undefined_brows:
                    bv.set_block(bid, vv.astype(dtype, order=order, casting=casting, subok=subok, copy=copy))
            return bv
    ...
    def compress(self, condition, axis=None, out=None):
        ...
        if isinstance(condition, BlockVector):
            ...
            assert self.nblocks == condition.nblocks, 'Number of blocks mismatch {} != {}'.format(self.nblocks, condition.nblocks)
    ...
    def copyto(self, other):
        ...
        """
        if isinstance(other, BlockVector):
            msgj = 'Number of blocks mismatch {} != {}'.format(self.nblocks, other.nblocks)
    ...
    def set_block(self, key, value):
        ...
        assert isinstance(value, np.ndarray) or isinstance(value, BaseBlockVector), 'Blocks need to be numpy arrays or BlockVectors'
