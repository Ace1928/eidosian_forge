...
def firewall_security_policy(data, fos, check_mode=False):
    ...
    firewall_security_policy_data = flatten_multilists_attributes(firewall_security_policy_data)
    ...
    if state == 'present' or state is True:
        ...
        return fos.delete('firewall', 'security-policy', mkey=filtered_data['policyid'], vdom=vdom)
    else:
