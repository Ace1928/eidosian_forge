...
def quantized_avg_pool(input: _atypes.TensorFuzzingAnnotation[TV_QuantizedAvgPool_T], min_input: _atypes.TensorFuzzingAnnotation[_atypes.Float32], max_input: _atypes.TensorFuzzingAnnotation[_atypes.Float32], ksize, strides, padding: str, name=None):
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'QuantizedAvgPool', name, input, min_input, max_input, 'ksize', ksize, 'strides', strides, 'padding', padding)
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('QuantizedAvgPool', input=input, min_input=min_input, max_input=max_input, ksize=ksize, strides=strides, padding=padding, name=name)
    _result = _outputs[:]
