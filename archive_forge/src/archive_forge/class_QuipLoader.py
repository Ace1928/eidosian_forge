...
class QuipLoader(BaseLoader):
    ...
    def __init__(self, api_url: str, access_token: str, request_timeout: Optional[int]=60):
        """
        Args:
    ...
    def load(self, folder_ids: Optional[List[str]]=None, thread_ids: Optional[List[str]]=None, max_docs: Optional[int]=1000, include_all_folders: bool=False, include_comments: bool=False, include_images: bool=False) -> List[Document]:
        """
        Args:
        ...
        if not folder_ids and (not thread_ids) and (not include_all_folders):
            raise ValueError('Must specify at least one among `folder_ids`, `thread_ids` or set `include_all`_folders as True')
