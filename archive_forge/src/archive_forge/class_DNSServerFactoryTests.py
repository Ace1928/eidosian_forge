...
class DNSServerFactoryTests(unittest.TestCase):
    ...
    def test_authorities(self):
        ...
        self.assertEqual(server.DNSServerFactory(authorities=[dummyResolver]).resolver.resolvers, [dummyResolver])
    ...
    def test_handleQuery(self):
        ...
        e = self.assertRaises(RaisingResolver.QueryArguments, f.handleQuery, message=m, protocol=NoopProtocol(), address=None)
        (query,), kwargs = e.args
    ...
    def test_handleStatus(self):
        ...
        """
        f = server.DNSServerFactory()
        e = self.assertRaises(RaisingProtocol.WriteMessageArguments, f.handleStatus, message=dns.Message(), protocol=RaisingProtocol(), address=None)
        (message,), kwargs = e.args
        self.assertEqual(message.rCode, dns.ENOTIMP)
    ...
    def test_handleNotify(self):
        ...
        """
        f = server.DNSServerFactory()
        e = self.assertRaises(RaisingProtocol.WriteMessageArguments, f.handleNotify, message=dns.Message(), protocol=RaisingProtocol(), address=None)
        (message,), kwargs = e.args
        self.assertEqual(message.rCode, dns.ENOTIMP)
