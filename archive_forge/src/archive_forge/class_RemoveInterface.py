...
class RemoveInterface(base.UpdateCommand):
  ...
  *{command}* is used to remove an interface to a Distributed Cloud Edge
  ...
  """
    ...
    @staticmethod
    def Args(parser):
        resource_args.AddRouterResourceArg(parser, 'from which we remove an interface', True)
        interface_parser = parser.add_mutually_exclusive_group(required=True)
        interface_parser.add_argument('--interface-names', type=arg_parsers.ArgList(), metavar='INTERFACE_NAME', help='The list of names for interfaces being removed.')
        interface_parser.add_argument('--interface-name', help='The name of the interface being removed.')
