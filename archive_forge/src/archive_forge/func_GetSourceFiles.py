...
def GetSourceFiles(upload_dir, skip_files_regex, has_explicit_skip_files, runtime, environment, source_dir, ignore_file=None):
  ...
  """
    ...
    if registry_entry:
        if has_explicit_skip_files:
            raise SkipFilesError('skip_files cannot be used with the [{}] runtime. Ignore patterns are instead expressed in a .gcloudignore file. For information on the format and syntax of .gcloudignore files, see https://cloud.google.com/sdk/gcloud/reference/topic/gcloudignore.'.format(runtime))
        file_chooser = gcloudignore.GetFileChooserForDir(source_dir, default_ignore_file=registry_entry, write_on_disk=True, gcloud_ignore_creation_predicate=lambda unused_dir: True, ignore_file=ignore_file, include_gitignore=False)
        it = file_chooser.GetIncludedFiles(upload_dir, include_dirs=False)
    elif ignore_file:
        if os.path.exists(os.path.join(source_dir, ignore_file)):
            if has_explicit_skip_files:
                raise SkipFilesError('Cannot have both an ignore file {0} and skip_files defined in the same application. We recommend you translate your skip_files ignore patterns to your {0} file. '.format(ignore_file))
            it = gcloudignore.GetFileChooserForDir(source_dir, ignore_file=ignore_file).GetIncludedFiles(upload_dir, include_dirs=False)
        else:
            raise exceptions.FileNotFoundError('File {0} referenced by --ignore-file does not exist.'.format(ignore_file))
    elif os.path.exists(os.path.join(source_dir, gcloudignore.IGNORE_FILE_NAME)):
        if has_explicit_skip_files:
            raise SkipFilesError('Cannot have both a .gcloudignore file and skip_files defined in the same application. We recommend you translate your skip_files ignore patterns to your .gcloudignore file. See https://cloud.google.com/sdk/gcloud/reference/topic/gcloudignore for more information about gcloudignore.')
        it = gcloudignore.GetFileChooserForDir(source_dir).GetIncludedFiles(upload_dir, include_dirs=False)
    else:
