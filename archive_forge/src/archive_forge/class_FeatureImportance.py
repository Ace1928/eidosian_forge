...
class FeatureImportance(OfflineEvaluator):
    ...
    def estimate(self, batch: SampleBatchType) -> Dict[str, Any]:
        ...
        for r in range(self.repeat):
            for i in range(n_features):
                ...
                perturbed_actions, _, _ = self.policy.compute_actions(copy_obs_batch, explore=False)
    ...
    def estimate_on_dataset(self, dataset: Dataset, *, n_parallelism: int=...) -> Dict[str, Any]:
        ...
        actions_ds = ds.map_batches(_compute_actions, batch_size=bsize, fn_kwargs={'output_key': 'ref_actions', 'policy_state': policy_state})
        ...
        for r in range(self.repeat):
            ...
            remote_fns = [get_feature_importance_on_index.remote(dataset=shuffled_ds, index=i, perturb_fn=self.perturb_fn, bsize=bsize_per_task, policy_state=policy_state) for i in range(n_features)]
