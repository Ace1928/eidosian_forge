...
class ReauthTest(unittest.TestCase):
    ...
    class _U2FInterfaceMock(object):
        ...
        def Authenticate(self, unused_app_id, challenge, unused_registered_keys):
            client_data = model.ClientData(model.ClientData.TYP_AUTHENTICATION, challenge, 'some_origin')
    ...
    def setUp(self):
        self.u2f_local_interface_mock = self.StartPatch('pyu2f.u2f.GetLocalU2FInterface')
    ...
    def accessTokenRefreshError(self, response, content):
        ...
        with mock.patch('httplib2.Http.request', side_effect=side_effect) as request_mock:
            with self.assertRaises(errors.HttpAccessTokenRefreshError):
                reauth.refresh_access_token(request_mock, self.client_id, self.client_secret, 'some_refresh_token', self.oauth_api_url)
    ...
    def testReauthAccessTokenError(self):
        ...
        self.reauthAccessTokenError(_error_response, json.dumps({'error': 'some error'}))
    ...
    def testChallangeNotActivatedError(self):
        ...
        def side_effect(*args, **kwargs):
            ...
            if uri == self.oauth_api_url:
                return (_ok_response, json.dumps({'access_token': 'access_token_for_reauth'}))
            if uri == _reauth_client._REAUTH_API + ':start':
                return (None, json.dumps({'status': 'CHALLENGE_REQUIRED', 'sessionId': 'session_id_1', 'challenges': [{'status': 'NOT_READY', 'challengeId': 1, 'challengeType': 'PASSWORD'}]}))
        ...
        with self.assertRaises(errors.ReauthFailError) as e:
            reauth.get_rapt_token(request_mock, self.client_id, self.client_secret, 'some_refresh_token', self.oauth_api_url, scopes=None)
