...
def _search_model_versions():
    request_message = _get_request_message(SearchModelVersions(), schema={'filter': [_assert_string], 'max_results': [_assert_intlike, lambda x: _assert_less_than_or_equal(int(x), 200000)], 'order_by': [_assert_array, _assert_item_type_string], 'page_token': [_assert_string]})
