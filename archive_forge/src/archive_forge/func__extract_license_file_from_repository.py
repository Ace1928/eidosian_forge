...
def _extract_license_file_from_repository(model_name):
    ...
    try:
        ...
        _logger.debug(f'Unable to list repository contents for the model repo {model_name}. In order to enable repository listing functionality, please install the huggingface_hub package by running `pip install huggingface_hub>0.10.0')
        return
    try:
        files = hub.list_repo_files(model_name)
        return next((file for file in files if _LICENSE_FILE_PATTERN.search(file)))
    except Exception as e:
        _logger.debug(f'Failed to retrieve repository file listing data for {model_name} due to {e}')