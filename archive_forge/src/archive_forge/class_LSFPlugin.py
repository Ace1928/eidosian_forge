...
class LSFPlugin(SGELikeBatchManagerBase):
    ...
    def _submit_batchtask(self, scriptfile, node):
        cmd = CommandLine('bsub', environ=dict(os.environ), resource_monitor=False, terminal_output='allatonce')
        bsubargs = ''
        if self._bsub_args:
        ...
        while True:
            try:
                ...
                if tries < self._max_tries:
                    ...
                    iflogger.setLevel(oldlevel)
                    raise RuntimeError('\n'.join(('Could not submit lsf task for node %s' % node._id, str(e))))
            else:
                break
        iflogger.setLevel(oldlevel)
        ...
        if match:
            ...
            raise IOError("Can't parse submission job output id: %s" % result.runtime.stdout)
