...
class Result:
    ...
    @classmethod
    def from_path(cls, path: Union[str, os.PathLike], storage_filesystem: Optional[pyarrow.fs.FileSystem]=None) -> 'Result':
        ...
        from ray.train._internal.storage import get_fs_and_path, _exists_at_fs_path, _list_at_fs_path
        ...
        if _exists_at_fs_path(fs, result_json_file):
            ...
            metrics_df = pd.read_csv(io.StringIO(cls._read_file_as_str(fs, progress_csv_file)))
            latest_metrics = metrics_df.iloc[-1].to_dict() if not metrics_df.empty else {}
        else:
            raise RuntimeError(f'Failed to restore the Result object: Neither {EXPR_RESULT_FILE} nor {EXPR_PROGRESS_FILE} exists in the trial folder!')
        checkpoint_dir_names = sorted(_list_at_fs_path(fs, fs_path, file_filter=lambda file_info: file_info.type == pyarrow.fs.FileType.Directory and file_info.base_name.startswith('checkpoint_')))
        if checkpoint_dir_names:
            checkpoints = [Checkpoint(path=Path(fs_path, checkpoint_dir_name).as_posix(), filesystem=fs) for checkpoint_dir_name in checkpoint_dir_names]
            metrics = []
            for checkpoint_dir_name in checkpoint_dir_names:
                metrics_corresponding_to_checkpoint = metrics_df[metrics_df[CHECKPOINT_DIR_NAME] == checkpoint_dir_name]
                if metrics_corresponding_to_checkpoint.empty:
                    logger.warning(f'Could not find metrics corresponding to {checkpoint_dir_name}. These will default to an empty dict.')
                metrics.append({} if metrics_corresponding_to_checkpoint.empty else metrics_corresponding_to_checkpoint.iloc[-1].to_dict())
        ...
        return Result(metrics=latest_metrics, checkpoint=latest_checkpoint, path=fs_path, _storage_filesystem=fs, metrics_dataframe=metrics_df, best_checkpoints=best_checkpoints, error=error)
    ...
    def get_best_checkpoint(self, metric: str, mode: str) -> Optional['Checkpoint']:
        ...
        if mode not in ['max', 'min']:
            raise ValueError(f'Unsupported mode: {mode}. Please choose from ["min", "max"]!')
        op = max if mode == 'max' else min
        valid_checkpoints = [ckpt_info for ckpt_info in self.best_checkpoints if metric in ckpt_info[1]]
        if not valid_checkpoints:
            raise RuntimeError(f'Invalid metric name {metric}! You may choose from the following metrics: {self.metrics.keys()}.')
