...
def firewall_shaper_per_ip_shaper(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('firewall.shaper', 'per-ip-shaper', filtered_data, vdom=vdom)
    ...
    if state == 'present' or state is True:
        return fos.set('firewall.shaper', 'per-ip-shaper', data=filtered_data, vdom=vdom)
    elif state == 'absent':
        return fos.delete('firewall.shaper', 'per-ip-shaper', mkey=filtered_data['name'], vdom=vdom)
    else:
