...
class TypeBuilder(object):
    ...
    typename std::remove_cv<typename std::remove_reference<argument_type4>::type>::type
    ...
    typename std::remove_cv<typename std::remove_reference<decltype(toto)>::type>::type
    ...
    typename std::remove_cv<typename std::iterator_traits<typename std::remove_reference<str>::type::iterator>::value_type>::type

    >>> builder.GetAttr(builder.NamedType('complex'), 'real')
    decltype(pythonic::builtins::getattr(pythonic::types::attr::REAL{}, std::declval<complex>()))
    ...
    typename std::tuple_element<1,typename std::remove_reference<decltype(pythonic::types::make_tuple(std::declval<int>(), std::declval<str>()))>::type>::type
    ...
    decltype(pythonic::types::make_tuple(std::declval<int>(), std::declval<bool>()))
    ...
    def __init__(builder):
        ...
        class ReturnType(Type):
            ...
            def generate(self, ctx):
                ...
                args = ('std::declval<{0}>()'.format(ctx(arg)) for arg in self.args)
        ...
        class ListType(DependentType):
            ...
            def generate(self, ctx):
                return 'pythonic::types::list<{}>'.format('typename std::remove_reference<{0}>::type'.format(ctx(self.of)))
        ...
        class ContainerType(DependentType):
            ...
            def generate(self, ctx):
                return 'container<typename std::remove_reference<{0}>::type>'.format(ctx(self.of))
