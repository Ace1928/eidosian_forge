...
class TransresnetAgent(Agent):
    ...
    def __init__(self, opt, shared=None):
        self.metrics = {'hits@1/100': 0.0, 'loss': 0.0, 'num_samples': 0, 'med_rank': []}
    ...
    def train_step(self, valid_obs, image_feats, personalities):
        ...
        loss, num_correct, num_examples = self.model.train_batch(image_feats, personalities, comments)
    ...
    def eval_step(self, valid_obs, image_feats, personalities):
        ...
        if 'label_candidates' in valid_obs[0] or self.fixed_cands is not None:
            ...
            chosen_captions = self.model.choose_best_caption(image_feats, personalities, candidates, candidates_encoded=candidates_encoded, k=-1 if self.fixed_cands is None else 100)
            ...
            if self.fixed_cands is not None:
                ...
                num_correct = sum([1 if chosen_captions[i][0] in chosen_captions[i] else 0 for i in range(len(chosen_captions))])
        else:
            comments = [random.choice(v['eval_labels']) for v in valid_obs]
            loss, num_correct, num_examples = self.model.eval_batch(image_feats, personalities, comments)
    ...
    def report(self):
        ...
        if self.metrics['num_samples'] > 0:
            m['hits@1/100'] = round_sigfigs(self.metrics['hits@1/100'] / self.metrics['num_samples'], 4)
