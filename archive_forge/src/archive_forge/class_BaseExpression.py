...
class BaseExpression:
    ...
    def contains_subquery(self):
        return any((expr and (getattr(expr, 'subquery', False) or expr.contains_subquery) for expr in self.get_source_expressions()))
    ...
    def relabeled_clone(self, change_map):
        clone = self.copy()
        clone.set_source_expressions([e.relabeled_clone(change_map) if e is not None else None for e in self.get_source_expressions()])
        return clone
    ...
    def replace_expressions(self, replacements):
        ...
        clone.set_source_expressions([expr.replace_expressions(replacements) if expr else None for expr in source_expressions])
        return clone
