...
def _CopyFileToFile(src_url, dst_url, status_queue=None, src_obj_metadata=None):
  ...
  """
    ...
    if not src_url.IsStream():
        src_fp.close()
    end_time = time.time()
    PutToQueueWithTimeout(status_queue, FileMessage(src_url, dst_url, end_time, message_type=FileMessage.FILE_LOCAL_COPY, size=src_obj_metadata.size if src_obj_metadata else None, finished=True))
