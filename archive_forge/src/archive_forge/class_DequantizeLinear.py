...
class DequantizeLinear(Base):
    ...
    @staticmethod
    def export() -> None:
        node = onnx.helper.make_node('DequantizeLinear', inputs=['x', 'x_scale', 'x_zero_point'], outputs=['y'])
        x = np.array([0, 3, 128, 255]).astype(np.uint8)
        x_scale = np.float32(2)
        x_zero_point = np.uint8(128)
        y = np.array([-256, -250, 0, 254], dtype=np.float32)
        expect(node, inputs=[x, x_scale, x_zero_point], outputs=[y], name='test_dequantizelinear')

    @staticmethod
    def export_axis() -> None:
        node = onnx.helper.make_node('DequantizeLinear', inputs=['x', 'x_scale', 'x_zero_point'], outputs=['y'])
        x = np.array([[[[3, 89], [34, 200], [74, 59]], [[5, 24], [24, 87], [32, 13]], [[245, 99], [4, 142], [121, 102]]]], dtype=np.uint8)
        ...
        expect(node, inputs=[x, x_scale, x_zero_point], outputs=[y], name='test_dequantizelinear_axis')
    ...
    def export_e4m3fn() -> None:
        ...
        expect(node, inputs=[x, x_scale], outputs=[y], name='test_dequantizelinear_e4m3fn')
    ...
    def export_e5m2() -> None:
        ...
        expect(node, inputs=[x, x_scale], outputs=[y], name='test_dequantizelinear_e5m2')
    ...
    def export_uint16() -> None:
        ...
        expect(node, inputs=[x, x_scale, x_zero_point], outputs=[y], name='test_dequantizelinear_uint16')
    ...
    def export_int16() -> None:
        ...
        expect(node, inputs=[x, x_scale, x_zero_point], outputs=[y], name='test_dequantizelinear_int16')

    @staticmethod
    def export_uint4() -> None:
        node = onnx.helper.make_node('DequantizeLinear', inputs=['x', 'x_scale', 'x_zero_point'], outputs=['y'], axis=0)
        ...
        expect(node, inputs=[x, x_scale, x_zero_point], outputs=[y], name='test_dequantizelinear_uint4')
    ...
    def export_int4() -> None:
        ...
        expect(node, inputs=[x, x_scale, x_zero_point], outputs=[y], name='test_dequantizelinear_int4')
