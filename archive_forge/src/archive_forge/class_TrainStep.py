...
class TrainStep(BaseStep):
    ...
    def _validate_and_apply_step_config(self):
        if 'using' in self.step_config:
            if self.step_config['using'] not in ['custom', 'automl/flaml']:
                raise MlflowException(f"Invalid train step configuration value {self.step_config['using']} for key 'using'. Supported values are: ['custom', 'automl/flaml']", error_code=INVALID_PARAMETER_VALUE)
        else:
            self.step_config['using'] = 'custom'
        if 'tuning' in self.step_config:
            if 'enabled' in self.step_config['tuning'] and isinstance(self.step_config['tuning']['enabled'], bool):
                self.step_config['tuning_enabled'] = self.step_config['tuning']['enabled']
            else:
                raise MlflowException("The 'tuning' configuration in the train step must include an 'enabled' key whose value is either true or false.", error_code=INVALID_PARAMETER_VALUE)
            if self.step_config['tuning_enabled']:
                if 'sample_fraction' in self.step_config['tuning']:
                    ...
                    if 0 < sample_fraction <= 1.0:
                        ...
                        raise MlflowException("The tuning 'sample_fraction' configuration in the train step must be between 0 and 1.", error_code=INVALID_PARAMETER_VALUE)
                else:
                ...
                if 'max_trials' not in self.step_config['tuning']:
                    raise MlflowException("The 'max_trials' configuration in the train step must be provided.", error_code=INVALID_PARAMETER_VALUE)
                if 'parameters' not in self.step_config['tuning']:
                    raise MlflowException("The 'parameters' configuration in the train step must be provided  when tuning is enabled.", error_code=INVALID_PARAMETER_VALUE)
        else:
        ...
        if self.recipe is None:
            raise MlflowException('Missing recipe config in recipe config.', error_code=INVALID_PARAMETER_VALUE)
        ...
        if 'estimator_method' not in self.step_config and self.step_config['using'] == 'custom':
            raise MlflowException("Missing 'estimator_method' configuration in the train step, which is using 'custom'.", error_code=INVALID_PARAMETER_VALUE)
        self.predict_prefix = self.step_config.get('predict_prefix', 'predicted_')
        self.predict_scores_for_all_classes = self.step_config.get('predict_scores_for_all_classes', True)
        self.primary_metric = _get_primary_metric(self.step_config.get('primary_metric'), self.extended_task)
        ...
        self.evaluation_metrics_greater_is_better = {metric.name: metric.greater_is_better for metric in builtin_metrics}
        self.evaluation_metrics_greater_is_better.update({metric.name: metric.greater_is_better for metric in custom_metrics})
        if self.primary_metric is not None and self.primary_metric not in self.evaluation_metrics:
            raise MlflowException(f"The primary metric '{self.primary_metric}' is a custom metric, but its corresponding custom metric configuration is missing from `recipe.yaml`.", error_code=INVALID_PARAMETER_VALUE)
    ...
    def construct_search_space_from_yaml(cls, params):
        ...
        for param_name, param_details in params.items():
            if 'values' in param_details:
                ...
                raise MlflowException(f"Parameter {param_name} must contain either a list of 'values' or a 'distribution' following hyperopt parameter expressions", error_code=INVALID_PARAMETER_VALUE)
        return search_space
    ...
    def _calibrated_classifier_fitted_estimator(self, estimator, X_train, y_train):
        ...
        if 'classification' in self.recipe:
            ...
            estimator = CalibratedClassifierCV(estimator, method=self.step_config['calibrate_proba'])
    ...
    def _run(self, output_directory):
        ...
        try:
            ...
            transformed_training_data_path = get_step_output_path(recipe_root_path=self.recipe_root, step_name='transform', relative_path='transformed_training_data.parquet')
            ...
            if self.extended_task == 'classification/binary':
                ...
                if self.rebalance_training_data and len(classes) == 2:
                    if len(train_df) > _REBALANCING_CUTOFF:
                        ...
                        _logger.info(f'Training data has less than {_REBALANCING_CUTOFF} rows, skipping rebalancing.')
            X_train, y_train = (train_df.drop(columns=[self.target_col]), train_df[self.target_col])
            transformed_validation_data_path = get_step_output_path(recipe_root_path=self.recipe_root, step_name='transform', relative_path='transformed_validation_data.parquet')
            ...
            raw_validation_data_path = get_step_output_path(recipe_root_path=self.recipe_root, step_name='split', relative_path='validation.parquet')
            raw_validation_df = pd.read_parquet(raw_validation_data_path)
            transformer_path = get_step_output_path(recipe_root_path=self.recipe_root, step_name='transform', relative_path='transformer.pkl')
            tags = {MLFLOW_SOURCE_TYPE: SourceType.to_string(SourceType.RECIPE), MLFLOW_RECIPE_TEMPLATE_NAME: self.step_config['recipe'], MLFLOW_RECIPE_PROFILE_NAME: self.step_config['profile'], MLFLOW_RECIPE_STEP_NAME: MLFLOW_RECIPES_EXECUTION_TARGET_STEP_NAME.get()}
            run_name = self.tracking_config.run_name
            best_estimator_params = None
            mlflow.autolog(log_models=False, silent=True)
            with mlflow.start_run(run_name=run_name, tags=tags) as run:
                estimator = self._resolve_estimator(X_train, y_train, validation_df, run, output_directory)
                best_estimator_params = estimator.get_params()
                fitted_estimator, additional_fitted_args = self._fitted_estimator(estimator, X_train, y_train)
                logged_estimator = self._log_estimator_to_mlflow(fitted_estimator, X_train)
                with open(transformer_path, 'rb') as f:
                    transformer = cloudpickle.load(f)
                mlflow.sklearn.log_model(transformer, 'transform/transformer', code_paths=self.code_paths)
                trained_pipeline = make_pipeline(transformer, fitted_estimator)
                wrapped_model = WrappedRecipeModel(self.predict_scores_for_all_classes, self.predict_prefix, target_column_class_labels=additional_fitted_args.get('target_column_class_labels'))
                model_uri = get_step_output_path(recipe_root_path=self.recipe_root, step_name=self.name, relative_path=TrainStep.MODEL_ARTIFACT_RELATIVE_PATH)
                sklearn_model_uri = get_step_output_path(recipe_root_path=self.recipe_root, step_name=self.name, relative_path=TrainStep.SKLEARN_MODEL_ARTIFACT_RELATIVE_PATH)
                ...
                with TempDir() as tmp:
                    ...
                    mlflow.pyfunc.save_model(path=pyfunc_model_tmp_path, python_model=wrapped_model, artifacts=artifacts)
                    tempModel = mlflow.pyfunc.load_model(pyfunc_model_tmp_path)
                    model_schema = infer_signature(raw_X_train, tempModel.predict(raw_X_train.copy()))
                    mlflow.pyfunc.save_model(path=model_uri, python_model=wrapped_model, artifacts=artifacts, signature=model_schema, code_path=self.code_paths)
                model = mlflow.pyfunc.load_model(model_uri)
                tmp_model_info = Model.load(model_uri)
                model_data_subpath = os.path.join('artifacts', TrainStep.SKLEARN_MODEL_ARTIFACT_RELATIVE_PATH, 'model.pkl')
                model_info = Model(artifact_path='train/model', run_id=run.info.run_id, utc_time_created=tmp_model_info.utc_time_created, flavors=tmp_model_info.flavors, signature=tmp_model_info.signature, saved_input_example_info=tmp_model_info.saved_input_example_info, model_uuid=tmp_model_info.model_uuid, mlflow_version=tmp_model_info.mlflow_version, metadata=tmp_model_info.metadata)
                model_info.add_flavor(mlflow.sklearn.FLAVOR_NAME, pickled_model=model_data_subpath, sklearn_version=sklearn.__version__, serialization_format=mlflow.sklearn.SERIALIZATION_FORMAT_CLOUDPICKLE, code='code')
                ...
                eval_metrics = {}
                for dataset_name, (dataset, metric_prefix) in {'training': (train_df, 'training_'), 'validation': (validation_df, 'val_')}.items():
                    eval_config = {'log_model_explainability': False, 'metric_prefix': metric_prefix}
                    if self.positive_class is not None:
                        eval_config['pos_label'] = self.positive_class
                    eval_result = mlflow.evaluate(model=logged_estimator.model_uri, data=dataset, targets=self.target_col, model_type=_get_model_type_from_template(self.recipe), evaluators='default', extra_metrics=_load_custom_metrics(self.recipe_root, self.evaluation_metrics.values()), evaluator_config=eval_config)
                    eval_result.save(os.path.join(output_directory, f'eval_{dataset_name}'))
                    eval_metrics[dataset_name] = {strip_prefix(k, metric_prefix): v for k, v in eval_result.metrics.items()}
            ...
            if isinstance(prediction_result, pd.DataFrame) and {f'{self.predict_prefix}label', f'{self.predict_prefix}score'}.issubset(prediction_result.columns):
                if self.positive_class:
                    prediction_result_for_error = prediction_result[f'{self.predict_prefix}score_{self.positive_class}'].values
                    use_probability_for_error_rate = True
                else:
                    prediction_result_for_error = prediction_result[f'{self.predict_prefix}score'].values
                prediction_result = prediction_result[f'{self.predict_prefix}label'].values
            else:
                prediction_result_for_error = prediction_result
            error_fn = _get_error_fn(self.recipe, use_probability=use_probability_for_error_rate, positive_class=self.positive_class)
            pred_and_error_df = pd.DataFrame({'target': target_data, 'prediction': prediction_result, 'error': error_fn(prediction_result_for_error, target_data.to_numpy())})
            calibrated_plot = None
            train_predictions = model.predict(raw_train_df.drop(self.target_col, axis=1))
            if isinstance(train_predictions, pd.DataFrame) and {f'{self.predict_prefix}label', f'{self.predict_prefix}score'}.issubset(train_predictions.columns):
                predicted_training_data = raw_train_df.assign(predicted_data=train_predictions[f'{self.predict_prefix}label'], predicted_score=train_predictions[f'{self.predict_prefix}score'].values)
                if self.positive_class:
                    worst_examples_df = BaseStep._generate_worst_examples_dataframe(raw_train_df, train_predictions[f'{self.predict_prefix}label'].values, error_fn(train_predictions[f'{self.predict_prefix}score_{self.positive_class}'].values, raw_train_df[self.target_col].to_numpy()), self.target_col)
                    if 'calibrate_proba' in self.step_config and hasattr(additional_fitted_args.get('original_estimator'), 'predict_proba'):
                        from sklearn.calibration import CalibrationDisplay
                        calibrated_plot = CalibrationDisplay.from_estimator(additional_fitted_args.get('original_estimator'), raw_train_df.drop(self.target_col, axis=1), raw_train_df[self.target_col], pos_label=self.positive_class)
                else:
                    worst_examples_df = pd.DataFrame()
            else:
                predicted_training_data = raw_train_df.assign(predicted_data=train_predictions)
                worst_examples_df = BaseStep._generate_worst_examples_dataframe(raw_train_df, train_predictions, error_fn(train_predictions, raw_train_df[self.target_col].to_numpy()), self.target_col)
            predicted_training_data.to_parquet(os.path.join(output_directory, TrainStep.PREDICTED_TRAINING_DATA_RELATIVE_PATH))
            leaderboard_df = None
            try:
            ...
            if self.step_config['tuning_enabled']:
                try:
                    ...
                    _logger.warning('Failed to build tuning results table due to unexpected failure: %s', e)
            card = self._build_step_card(eval_metrics=eval_metrics, pred_and_error_df=pred_and_error_df, model_schema=model_schema, run_id=run.info.run_id, model_uri=model_uri, worst_examples_df=worst_examples_df, train_df=raw_train_df, output_directory=output_directory, leaderboard_df=leaderboard_df, tuning_df=tuning_df, calibrated_plot=calibrated_plot)
    ...
    def _resolve_estimator(self, X_train, y_train, validation_df, run, output_directory):
        ...
        if using_plugin == 'custom':
            return self._get_user_defined_estimator(X_train, y_train, validation_df, run, output_directory)
        else:
    ...
    def _build_step_card(self, eval_metrics, pred_and_error_df, model_schema, run_id, model_uri, worst_examples_df, train_df, output_directory, leaderboard_df=None, tuning_df=None, calibrated_plot=None):
        import pandas as pd
        from sklearn import set_config
        from sklearn.utils import estimator_html_repr
        card = BaseCard(self.recipe_name, self.name)
        metric_df = get_merged_eval_metrics(eval_metrics, ordered_metric_names=[self.primary_metric, *self.user_defined_custom_metrics.keys()]).reset_index().rename(columns={'index': 'Metric'})
        ...
        metric_table_html = BaseCard.render_table(metric_df.style.format({'training': '{:.6g}', 'validation': '{:.6g}'}).apply(row_style, axis=1))
        card.add_tab('Model Performance', "<h3 class='section-title'>Summary Metrics (Validation)</h3>{{ METRICS }} ").add_html('METRICS', metric_table_html)
        if not self.skip_data_profiling:
            pred_and_error_df_profile = get_pandas_data_profiles([['Predictions and Errors (Validation Dataset)', pred_and_error_df.reset_index(drop=True)]])
            card.add_tab('Data Profile (Predictions)', '{{PROFILE}}').add_pandas_profile('PROFILE', pred_and_error_df_profile)
        set_config(display='diagram')
        model_repr = estimator_html_repr(mlflow.sklearn.load_model(os.path.join(model_uri, 'artifacts', TrainStep.SKLEARN_MODEL_ARTIFACT_RELATIVE_PATH)))
        card.add_tab('Model Architecture', '{{MODEL_ARCH}}').add_html('MODEL_ARCH', model_repr)

        def render_schema(inputs, title):
            from mlflow.types import ColSpec
            table = BaseCard.render_table(({'Name': '  ' + (spec.name or '-'), 'Type': repr(spec.type) if isinstance(spec, ColSpec) else repr(spec)} for spec in inputs))
        ...
        card.add_tab('Model Schema', '{{MODEL_SCHEMA}}').add_html('MODEL_SCHEMA', '<div style="display: flex">{tables}</div>'.format(tables='\n'.join(schema_tables)))
        if not worst_examples_df.empty:
            card.add_tab('Worst Predictions', '{{ WORST_EXAMPLES_TABLE }}').add_html('WORST_EXAMPLES_TABLE', BaseCard.render_table(worst_examples_df))
        if calibrated_plot:
            ...
            card.add_tab('Prob. Calibration', '{{ CALIBRATED_PLOT }}').add_image('CALIBRATED_PLOT', calibrated_plot_location)
        if not self.skip_data_profiling:
            worst_prediction_profile = get_pandas_data_profiles([['Worst Predictions', worst_examples_df.reset_index(drop=True)], ['Train', train_df.reset_index(drop=True)]])
            card.add_tab('Data Profile (Worst vs Train)', '{{ WORST_EXAMPLES_COMP }}').add_pandas_profile('WORST_EXAMPLES_COMP', worst_prediction_profile)
        if leaderboard_df is not None:
            card.add_tab('Leaderboard', '{{ LEADERBOARD_TABLE }}').add_html('LEADERBOARD_TABLE', BaseCard.render_table(leaderboard_df, hide_index=False))
        ...
        if os.path.exists(best_parameters_yaml):
            best_parameters_card_tab = card.add_tab(f'Best Parameters {(' (AutoML)' if is_automl_run else '')}', '{{ BEST_PARAMETERS }} ')
            if is_automl_run:
                automl_estimator_str = f'<b>Best estimator:</b><br><pre>{self.best_estimator_name}</pre><br><b>Best estimator class:</b><br><pre>{self.best_estimator_class}</pre><br><br>'
            else:
                automl_estimator_str = ''
            with open(best_parameters_yaml) as f:
                best_parameters = f.read()
            best_parameters_card_tab.add_html('BEST_PARAMETERS', f'{automl_estimator_str}<b>Best parameters:</b><br><pre>{best_parameters}</pre><br><br>')
        if tuning_df is not None:
            tuning_trials_card_tab = card.add_tab('Tuning Trials', '{{ SEARCH_SPACE }}' + '{{ TUNING_TABLE_TITLE }}' + '{{ TUNING_TABLE }}')
            tuning_params = yaml.dump(self.step_config['tuning']['parameters'])
            tuning_trials_card_tab.add_html('SEARCH_SPACE', f'<b>Tuning search space:</b> <br><pre>{tuning_params}</pre><br><br>')
            tuning_trials_card_tab.add_html('TUNING_TABLE_TITLE', '<b>Tuning results:</b><br>')
            tuning_trials_card_tab.add_html('TUNING_TABLE', BaseCard.render_table(tuning_df.style.apply(lambda row: pd.Series('font-weight: bold', row.index), axis=1, subset=tuning_df.index[0]), hide_index=True))
        ...
        run_card_tab = card.add_tab('Run Summary', '{{ RUN_ID }} ' + '{{ MODEL_URI }}' + '{{ EXE_DURATION }}' + '{{ LAST_UPDATE_TIME }}')
        model_uri_path = f'runs:/{run_id}/train/model'
        run_url = get_databricks_run_url(tracking_uri=mlflow.get_tracking_uri(), run_id=run_id)
        model_url = get_databricks_run_url(tracking_uri=mlflow.get_tracking_uri(), run_id=run_id, artifact_path=re.sub(f'^.*?{run_id}', '', model_uri_path))
        if run_url is not None:
            run_card_tab.add_html('RUN_ID', f'<b>MLflow Run ID:</b> <a href={run_url}>{run_id}</a><br><br>')
        else:
            run_card_tab.add_markdown('RUN_ID', f'**MLflow Run ID:** `{run_id}`')
        if model_url is not None:
            run_card_tab.add_html('MODEL_URI', f'<b>MLflow Model URI:</b> <a href={model_url}>{model_uri_path}</a>')
        else:
    ...
    def from_recipe_config(cls, recipe_config, recipe_root):
        ...
        step_config.update(get_recipe_tracking_config(recipe_root_path=recipe_root, recipe_config=recipe_config).to_dict())
    ...
    def get_artifacts(self):
        return [ModelArtifact('model', self.recipe_root, self.name, self.tracking_config.tracking_uri), RunArtifact('run', self.recipe_root, self.name, self.tracking_config.tracking_uri), HyperParametersArtifact('best_parameters', self.recipe_root, self.name), DataframeArtifact('predicted_training_data', self.recipe_root, self.name, TrainStep.PREDICTED_TRAINING_DATA_RELATIVE_PATH)]
    ...
    def _write_best_parameters_outputs(self, output_directory, best_hp_params=None, best_hardcoded_params=None, automl_params=None, default_params=None):
        if best_hp_params or best_hardcoded_params or automl_params or default_params:
            ...
            with open(best_parameters_path, 'a') as file:
                ...
                self._write_one_param_output(best_hardcoded_params or {}, file, 'hardcoded parameters')
