...
class CLIPImageQualityAssessment(Metric):
    ...
    def __init__(self, model_name_or_path: Literal['clip_iqa', 'openai/clip-vit-base-patch16', 'openai/clip-vit-base-patch32', 'openai/clip-vit-large-patch14-336', 'openai/clip-vit-large-patch14']='clip_iqa', data_range: float=1.0, prompts: Tuple[Union[str, Tuple[str, str]]]=('quality',), **kwargs: Any) -> None:
        ...
        with torch.inference_mode():
            anchors = _clip_iqa_get_anchor_vectors(model_name_or_path, self.model, self.processor, self.prompts_list, self.device)
    ...
    def update(self, images: Tensor) -> None:
        ...
        with torch.inference_mode():
            img_features = _clip_iqa_update(self.model_name_or_path, images, self.model, self.processor, self.data_range, self.device)
