...
from Utility import Grid.grid as grid, Node
...
class DecisionTree(Algorithm):
    ...
    def run_algorithm(self) -> Optional[Node]:
        ...
        output: np.ndarray = self.network.feedforward(
    ...
    def get_input_data(self) -> List[float]:
        ...
        food_x, food_y = self.snake.get_food_position()

        # Calculate relative position of food to head
        ...
        # Check for obstacles around the head (assuming a method exists to check obstacles)
        obstacle_left = self.snake.is_obstacle_at(head_x - 1, head_y)
        obstacle_right = self.snake.is_obstacle_at(head_x + 1, head_y)
        obstacle_up = self.snake.is_obstacle_at(head_x, head_y - 1)
        obstacle_down = self.snake.is_obstacle_at(head_x, head_y + 1)

        # Compile all data into a list
        input_data = [
            head_x,
            head_y,  # Snake head position
            food_relative_x,
            food_relative_y,  # Relative food position
            direction,  # Current direction of the snake
            obstacle_left,
            obstacle_right,
            obstacle_up,
            obstacle_down,  # Obstacles
        ]
    ...
    def interpret_output(self, output: np.ndarray) -> Optional[Node]:
        ...
        best_move: Optional[Node] = (
            None  # Placeholder for actual move interpretation logic
        )
