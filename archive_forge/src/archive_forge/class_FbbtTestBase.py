    ...
    def test_mul(self):
        ...
        for xl, xu in x_bounds:
            for cl, cu in c_bounds:
                ...
                m.c = pyo.Constraint(expr=pyo.inequality(body=m.x * m.y, lower=cl, upper=cu))
                self.tightener(m)
                x = np.linspace(pyo.value(m.x.lb) + 1e-06, pyo.value(m.x.ub), 100, endpoint=False)
                z = np.linspace(pyo.value(m.c.lower), pyo.value(m.c.upper), 100)
                if m.y.lb is None:
                    yl = -np.inf
                else:
    ...
    def test_div1(self):
        ...
        for xl, xu in x_bounds:
            for cl, cu in c_bounds:
                ...
                self.tightener(m)
                x = np.linspace(pyo.value(m.x.lb), pyo.value(m.x.ub), 100)
                z = np.linspace(pyo.value(m.c.lower) + 1e-06, pyo.value(m.c.upper), 100, endpoint=False)
                if m.y.lb is None:
                    yl = -np.inf
                else:
                    yl = m.y.lb
    ...
    def test_pow4(self):
        ...
        for yl, yu in y_bounds:
            for _exp_val in exp_vals:
                ...
                self.tightener(m)
                y = np.linspace(pyo.value(m.y.lb) + 1e-06, pyo.value(m.y.ub), 100, endpoint=True)
                if m.x.lb is None:
                    xl = -np.inf
                else:
                    xl = m.x.lb
