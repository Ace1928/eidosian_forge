...
class EquinixMetalNodeDriver(NodeDriver):
    ...
    def ex_list_nodes_for_project(self, ex_project_id, include='plan', page=1, per_page=1000):
        ...
        data = self.connection.request('/metal/v1/projects/%s/devices' % ex_project_id, params=params).object['devices']
    ...
    def list_sizes(self, ex_project_id=None):
        project_id = ex_project_id or self.project_id or (len(self.projects) and self.projects[0].id)
        if project_id:
            data = self.connection.request('/metal/v1/projects/%s/plans' % project_id).object['plans']
        else:
    ...
    def create_node(self, name, size, image, location, ex_project_id=None, ip_addresses=[], cloud_init=None, disk=None, disk_size=0, **kwargs):
        """
        Create a node.
        ...
        elif not ex_project_id:
            raise Exception('ex_project_id needs to be specified')
        facility = location.extra['code']
        params = {'hostname': name, 'plan': size.id, 'operating_system': image.id, 'facility': facility, 'include': 'plan', 'billing_cycle': 'hourly'}
        if ip_addresses:
            params['ip_addresses'] = ip_addresses
        params.update(kwargs)
        if cloud_init:
            params['userdata'] = cloud_init
        data = self.connection.request('/metal/v1/projects/%s/devices' % ex_project_id, data=json.dumps(params), method='POST')
    ...
    def _to_node(self, data):
        extra = {}
        extra_keys = ['created_at', 'updated_at', 'userdata', 'billing_cycle', 'locked', 'iqn', 'locked', 'project', 'description']
        if 'state' in data:
        ...
        for key in extra_keys:
            if key in data:
                extra[key] = data[key]
        node = Node(id=data['id'], name=data['hostname'], state=state, public_ips=ips['public'], private_ips=ips['private'], size=size, image=image, extra=extra, driver=self)
        return node

    def _to_image(self, data):
        extra = {'distro': data['distro'], 'version': data['version'], 'supported_sizes': data.get('provisionable_on', [])}
        return NodeImage(id=data['slug'], name=data['name'], extra=extra, driver=self)

    def _to_location(self, data):
        extra = data
        return NodeLocation(id=data['id'], name=data['name'], country=None, driver=self, extra=extra)
    ...
    def _to_size(self, data):
        try:
            ...
            cpus = None
        regions = [region.get('href').replace('/metal/v1/facilities/', '') for region in data.get('available_in', [])]
        extra = {'description': data['description'], 'line': data['line'], 'cpus': cpus, 'regions': regions}
        try:
        ...
        return NodeSize(id=data['slug'], name=name, ram=ram, disk=disk, bandwidth=0, price=price, extra=extra, driver=self)

    def _to_key_pairs(self, data):
        extra = {'label': data['label'], 'created_at': data['created_at'], 'updated_at': data['updated_at']}
        return KeyPair(name=data['id'], fingerprint=data['fingerprint'], public_key=data['key'], private_key=None, driver=self, extra=extra)
    ...
    def ex_associate_address_with_node(self, node, address, manageable=False, customdata=''):
        path = '/metal/v1/devices/%s/ips' % node.id
        params = {'address': address, 'manageable': manageable, 'customdata': customdata}
        result = self.connection.request(path, params=params, method='POST').object
        return result
    ...
    def _to_volume(self, data):
        return StorageVolume(id=data['id'], name=data['name'], size=data['size'], driver=self, extra=data)

    def create_volume(self, size, location, plan='storage_1', description='', ex_project_id=None, locked=False, billing_cycle=None, customdata='', snapshot_policies=None, **kwargs):
        """
        Create a new volume.
    ...
    def detach_volume(self, volume, ex_node=None, ex_attachment_id=''):
        ...
        for attachment in attachments:
            if not ex_attachment_id or ex_attachment_id in attachment['href']:
                ...
                if ex_node:
                    node_id = self.ex_describe_attachment(attachment_id)['device']['href'].split('/')[-1]
    ...
    def _to_volume_snapshot(self, data):
        ...
        return VolumeSnapshot(id=data['id'], name=data['id'], created=created, state=data['status'], driver=self, extra=data)

    def ex_modify_volume(self, volume, description=None, size=None, locked=None, billing_cycle=None, customdata=None):
        path = '/metal/v1/storage/%s' % volume.id
        params = {}
        if description:
            params['description'] = description
