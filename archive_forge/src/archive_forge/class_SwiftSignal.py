...
class SwiftSignal(resource.Resource):
    ...
    properties_schema = {HANDLE: properties.Schema(properties.Schema.STRING, required=True, description=_('URL of TempURL where resource will signal completion and optionally upload data.')), TIMEOUT: properties.Schema(properties.Schema.NUMBER, description=_('The maximum number of seconds to wait for the resource to signal completion. Once the timeout is reached, creation of the signal resource will fail.'), required=True, constraints=[constraints.Range(1, 43200)]), COUNT: properties.Schema(properties.Schema.INTEGER, description=_('The number of success signals that must be received before the stack creation process continues.'), default=1, constraints=[constraints.Range(1, 1000)])}
    ATTRIBUTES = DATA = 'data'
    attributes_schema = {DATA: attributes.Schema(_('JSON data that was uploaded via the SwiftSignalHandle.'), type=attributes.Schema.STRING)}
    WAIT_STATUSES = STATUS_FAILURE, STATUS_SUCCESS = ('FAILURE', 'SUCCESS')
    METADATA_KEYS = DATA, REASON, STATUS, UNIQUE_ID = ('data', 'reason', 'status', 'id')
    ...
    def check_create_complete(self, create_data):
        ...
        for status in statuses:
            if status == self.STATUS_FAILURE:
                ...
                LOG.info('%(name)s Failed (%(failure)s)', {'name': str(self), 'failure': str(failure)})
                raise failure
