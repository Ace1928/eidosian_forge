...
class StatisticsElement(Dataset, Element2D):
    ...
    def dataset(self):
        ...
        if self._dataset is None:
            ...
            dataset = Dataset(self, dataset=None, pipeline=None, transforms=None, vdims=[], datatype=datatype)
            return dataset
    ...
    def dframe(self, dimensions=None, multi_index=False):
        ...
        if vdims:
            raise ValueError('{} element does not hold data for value dimensions. Could not return data for {} dimension(s).'.format(type(self).__name__, ', '.join([d.name for d in vdims])))
