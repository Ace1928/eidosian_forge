...
class RocksetChatMessageHistory(BaseChatMessageHistory):
    ...
    def __init__(self, session_id: str, client: Any, collection: str, workspace: str='commons', messages_key: str='messages', sync: bool=False, message_uuid_method: Callable[[], Union[str, int]]=lambda: str(uuid4())) -> None:
    ...
    def messages(self) -> List[BaseMessage]:
        ...
        return messages_from_dict(self._query(f'\n                    SELECT *\n                    FROM UNNEST ((\n                        SELECT "{self.messages_key}"\n                        FROM {self.location}\n                        WHERE _id = :session_id\n                    ))\n                ', session_id=self.session_id))
    ...
    def add_message(self, message: BaseMessage) -> None:
        ...
        self.client.Documents.patch_documents(collection=self.collection, workspace=self.workspace, data=[self.rockset.model.patch_document.PatchDocument(id=self.session_id, patch=[self.rockset.model.patch_operation.PatchOperation(op='ADD', path=f'/{self.messages_key}/-', value=message_to_dict(message))])])
        if self.sync:
    ...
    def clear(self) -> None:
        ...
        self._create_empty_doc()
        if self.sync:
            self._wait_until(lambda: not self.messages, RocksetChatMessageHistory.ADD_TIMEOUT_MS)