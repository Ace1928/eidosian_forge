...
def host_vmk_update(self):
    """
        ...
        """
    changed = changed_settings = changed_vds = changed_services = changed_service_vmotion = changed_service_mgmt = changed_service_ft = changed_service_vsan = changed_service_prov = changed_service_rep = changed_service_rep_nfc = changed_service_backup_nfc = False
    changed_list = []
    ...
    if self.vnic.spec.netStackInstanceKey == 'defaultTcpipStack':
        ...
        if self.enable_replication and self.vnic.device not in service_type_vmks['vSphereReplication'] or (not self.enable_replication and self.vnic.device in service_type_vmks['vSphereReplication']):
            changed_services = changed_service_rep = True
        if self.enable_replication_nfc and self.vnic.device not in service_type_vmks['vSphereReplicationNFC'] or (not self.enable_replication_nfc and self.vnic.device in service_type_vmks['vSphereReplicationNFC']):
            changed_services = changed_service_rep_nfc = True
        if self.enable_backup_nfc and self.vnic.device not in service_type_vmks['vSphereBackupNFC'] or (not self.enable_backup_nfc and self.vnic.device in service_type_vmks['vSphereBackupNFC']):
    ...
    if changed_settings or changed_vds or changed_services:
        ...
        if changed_services:
            ...
            if changed_service_backup_nfc:
                ...
                self.set_service_type(vnic_manager=vnic_manager, vmk=self.vnic, service_type='vSphereBackupNFC', operation=operation)
