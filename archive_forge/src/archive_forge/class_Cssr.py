...
class Cssr:
    ...
    def __str__(self):
        ...
        output = [f'{a:.4f} {b:.4f} {c:.4f}', f'{alpha:.2f} {beta:.2f} {gamma:.2f} SPGR =  1 P 1    OPT = 1', f'{len(self.structure)} 0', f'0 {self.structure.formula}']
    ...
    def from_str(cls, string: str) -> Self:
        ...
        sp, coords = ([], [])
        for line in lines[4:]:
            match = re.match('\\d+\\s+(\\w+)\\s+([0-9\\-\\.]+)\\s+([0-9\\-\\.]+)\\s+([0-9\\-\\.]+)', line.strip())
            if match:
