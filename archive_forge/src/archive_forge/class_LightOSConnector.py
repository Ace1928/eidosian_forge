...
class LightOSConnector(base.BaseLinuxConnector):
    ...
    def __init__(self, root_helper, driver=None, execute=None, device_scan_attempts=DEVICE_SCAN_ATTEMPTS_DEFAULT, message_queue=None, *args, **kwargs):
        super(LightOSConnector, self).__init__(root_helper, *args, driver=driver, execute=execute, device_scan_attempts=device_scan_attempts, **kwargs)
    ...
    def get_connector_properties(root_helper, *args, **kwargs):
        ...
        props = {}
        lightos_connector = LightOSConnector(root_helper=root_helper, message_queue=None, execute=kwargs.get('execute'))
        hostnqn = utils.get_host_nqn()
        found_dsc = lightos_connector.find_dsc()
        if not found_dsc:
            LOG.debug('LIGHTOS: did not find dsc, continuing anyway.')
        if hostnqn:
            LOG.debug('LIGHTOS: finally hostnqn: %s dsc: %s', hostnqn, found_dsc)
            props['nqn'] = hostnqn
    ...
    def _check_device_exists_reading_block_class(self, uuid):
        ...
        for match_path in glob.glob(file_path):
            try:
                ...
                LOG.warning('LIGHTOS: Failed to read file %s', match_path)
                continue
            if wwid != match_wwid.strip():
                continue
            if nvmec_match.match(match_path.split('/')[-2]):
                continue
            LOG.info('LIGHTOS: matching uuid %s was found for device path %s', uuid, match_path)
    ...
    def connect_volume(self, connection_properties):
        ...
        LOG.info('LIGHTOS: connect_volume called for volume %s, connection properties: %s', uuid, connection_properties)
