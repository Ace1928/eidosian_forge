...
class ScaleModel(Transformation):
    ...
    def _apply_to(self, model, rename=True):
        ...
        if self._scaling_method == 'user':
            ...
            raise ValueError("ScaleModel transformation: unknown scaling_method found-- supported values: 'user' ")
        if rename:
            component_list = list(model.component_objects(ctype=[Var, Constraint, Objective]))
            scaled_component_to_original_name_map = rename_components(model=model, component_list=component_list, prefix='scaled_')
        else:
            scaled_component_to_original_name_map = ComponentMap([(comp, comp.name) for comp in model.component_objects(ctype=[Var, Constraint, Objective])])
        ...
        variable_substitution_dict = {id(k): v for k, v in variable_substitution_map.items()}
        already_scaled = set()
        for component in model.component_objects(ctype=(Constraint, Objective), descend_into=True):
            ...
            for k in component:
                ...
                if isinstance(c, _ConstraintData):
                    body = scaling_factor * replace_expressions(expr=c.body, substitution_map=variable_substitution_dict, descend_into_named_expressions=True, remove_named_expressions=True)
                    lower = c.lower
                    upper = c.upper
                    ...
                    c.expr = scaling_factor * replace_expressions(expr=c.expr, substitution_map=variable_substitution_dict, descend_into_named_expressions=True, remove_named_expressions=True)
                else:
