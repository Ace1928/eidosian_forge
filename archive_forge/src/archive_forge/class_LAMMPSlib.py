...
"""
    ...
    started = False
    initialized = False
    default_parameters = dict(atom_types=None, atom_type_masses=None, log_file=None, lammps_name='', keep_alive=False, lammps_header=['units metal', 'atom_style atomic', 'atom_modify map array sort 0 0'], amendments=None, post_changebox_cmds=None, boundary=True, create_box=True, create_atoms=True, read_molecular_info=False, comm=None)
    ...
    def propagate(self, atoms, properties, system_changes, n_steps, dt=None, dt_not_real_time=False, velocity_field=None):
        ...
        do_rebuild = not np.array_equal(atoms.numbers, self.previous_atoms_numbers) or 'numbers' in system_changes
        if not do_rebuild:
        ...
        if n_diff > 0:
            if any(['reax/c' in cmd for cmd in self.parameters.lmpcmds]):
                ...
                for cmd in self.parameters.lmpcmds:
                    if 'pair_style' in cmd or 'pair_coeff' in cmd or 'qeq/reax' in cmd:
                        self.lmp.command(cmd)
    ...
    def start_lammps(self):
        ...
        if self.parameters.log_file is None:
            ...
            cmd_args = ['-echo', 'log', '-log', self.parameters.log_file, '-screen', 'none', '-nocite']
