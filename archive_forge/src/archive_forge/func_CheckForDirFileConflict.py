...
def CheckForDirFileConflict(exp_src_url, dst_url):
  ...
  """
    ...
    if os.path.isfile(final_dir):
        raise CommandException('Cannot retrieve %s because a file exists where a directory needs to be created (%s).' % (exp_src_url.url_string, final_dir))
    if os.path.isdir(dst_path):
        raise CommandException('Cannot retrieve %s because a directory exists (%s) where the file needs to be created.' % (exp_src_url.url_string, dst_path))