...
class BERTScore(Metric):
    ...
    def __init__(self, model_name_or_path: Optional[str]=None, num_layers: Optional[int]=None, all_layers: bool=False, model: Optional[Module]=None, user_tokenizer: Optional[Any]=None, user_forward_fn: Optional[Callable[[Module, Dict[str, Tensor]], Tensor]]=None, verbose: bool=False, idf: bool=False, device: Optional[Union[str, torch.device]]=None, max_length: int=512, batch_size: int=64, num_threads: int=0, return_hash: bool=False, lang: str='en', rescale_with_baseline: bool=False, baseline_path: Optional[str]=None, baseline_url: Optional[str]=None, **kwargs: Any) -> None:
        ...
        if user_tokenizer:
            ...
            if not _TRANSFORMERS_GREATER_EQUAL_4_4:
                raise ModuleNotFoundError('`BERTScore` metric with default tokenizers requires `transformers` package be installed. Either install with `pip install transformers>=4.4` or `pip install torchmetrics[text]`.')
            from transformers import AutoTokenizer
            if model_name_or_path is None:
                rank_zero_warn(f'The argument `model_name_or_path` was not specified while it is required when the default `transformers` model is used. It will use the default recommended model - {_DEFAULT_MODEL!r}.')
    ...
    def update(self, preds: Union[str, Sequence[str]], target: Union[str, Sequence[str]]) -> None:
        ...
        if not isinstance(target, list):
            target = list(target)
        preds_dict, _ = _preprocess_text(preds, self.tokenizer, self.max_length, truncation=False, sort_according_length=False, own_tokenizer=self.user_tokenizer)
        target_dict, _ = _preprocess_text(target, self.tokenizer, self.max_length, truncation=False, sort_according_length=False, own_tokenizer=self.user_tokenizer)
    ...
    def compute(self) -> Dict[str, Union[Tensor, List[float], str]]:
        ...
        preds = {'input_ids': dim_zero_cat(self.preds_input_ids), 'attention_mask': dim_zero_cat(self.preds_attention_mask)}
        target = {'input_ids': dim_zero_cat(self.target_input_ids), 'attention_mask': dim_zero_cat(self.target_attention_mask)}
        return bert_score(preds=preds, target=target, model_name_or_path=self.model_name_or_path, num_layers=self.num_layers, all_layers=self.all_layers, model=self.model, user_tokenizer=self.tokenizer if self.user_tokenizer else None, user_forward_fn=self.user_forward_fn, verbose=self.verbose, idf=self.idf, device=self.embedding_device, max_length=self.max_length, batch_size=self.batch_size, num_threads=self.num_threads, return_hash=self.return_hash, lang=self.lang, rescale_with_baseline=self.rescale_with_baseline, baseline_path=self.baseline_path, baseline_url=self.baseline_url)
