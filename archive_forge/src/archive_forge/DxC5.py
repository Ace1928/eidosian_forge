...
- Currently undergoing extensive refactoring and upgrade so not functional at all
    - Data handling logic:
        - Data Processing Steps(Input to storage):
            ...
            - Create a unique hash using all of the metadata to verify integrity and uniqueness and ensure no redundancy or duplication.
            - Split the data into its appropriate categories for the different caches.
            - Reversibly encode the data to convert it into a bytes format for storage.
            - Convert from bytes into bit_matrix for data format standardisation.
            - Compress the data using the most efficient compression algorithm available.
            - encrypt the data using the most secure encryption algorithm available.
            - Store the data in the appropriate cache.
        - Data Processing Steps (Retrieval of data and restoring to original):
            - Retrieve the data from the appropriate cache, if not in cache load from blob into cache and then retrieve.
            - Decrypt the data using the most secure decryption algorithm available.
            - Decompress the data using the most efficient decompression algorithm available.
            - Convert from bit_matrix into bytes for data format standardisation.
            - Reversibly decode the data to convert it back into its original form.
            - Validate the data against the expected schema to ensure integrity and adherence to the expected structure.
            - Return the validated data for further processing.
        - Bit Matrix Conversion Logic: From Bytes to Vector to Bit matrix
            - From bytes to vector:
                - Convert the bytes into a standardised vector format for data processing.
        ...
        - Bit Matrix Conversion Logic: From Bit Matrix to Vector To Bytes        
            - From vector to bytes:
                - Convert the vector back into bytes format for data processing.
                - Vector to byte conversion is essential for efficient data processing and manipulation.
                - The bytes format allows for easy indexing, slicing, and transformation of the data.
                - Function: vector_to_bytes(vector_data: Vector) -> bytes
                - Arguments: vector_data (Vector) - The input data in vector format.
                - Returns: bytes - The data converted into bytes format.
                - Logic: Convert the vector data back into bytes format for further processing.
                - Step 1: Convert the vector data into a binary string.
                - Step 2: Split the binary string into chunks of 8 bits (1 byte each).
                - Step 3: Convert each 8-bit chunk back into an integer.
                - Step 4: Convert the integers back into bytes format.
                - Step 5: Return the bytes data.
