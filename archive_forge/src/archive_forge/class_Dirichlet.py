  ...
  * `Gamma` is the [gamma function](
  ...
  """
    ...
    """
        ...
        super(Dirichlet, self).__init__(dtype=self._concentration.dtype, validate_args=validate_args, allow_nan_stats=allow_nan_stats, reparameterization_type=distribution.FULLY_REPARAMETERIZED, parameters=parameters, graph_parents=[self._concentration, self._total_concentration], name=name)
    ...
    def _sample_n(self, n, seed=None):
        gamma_sample = random_ops.random_gamma(shape=[n], alpha=self.concentration, dtype=self.dtype, seed=seed)
    ...
    def _entropy(self):
        ...
        return self._log_normalization() + (self.total_concentration - k) * math_ops.digamma(self.total_concentration) - math_ops.reduce_sum((self.concentration - 1.0) * math_ops.digamma(self.concentration), axis=-1)
    ...
    @distribution_util.AppendDocstring('Note: The mode is undefined when any `concentration <= 1`. If\n      `self.allow_nan_stats` is `True`, `NaN` is used for undefined modes. If\n      `self.allow_nan_stats` is `False` an exception is raised when one or more\n      modes are undefined.')
    def _mode(self):
        k = math_ops.cast(self.event_shape_tensor()[0], self.dtype)
        mode = (self.concentration - 1.0) / (self.total_concentration[..., array_ops.newaxis] - k)
        if self.allow_nan_stats:
            nan = array_ops.fill(array_ops.shape(mode), np.array(np.nan, dtype=self.dtype.as_numpy_dtype()), name='nan')
            return array_ops.where_v2(math_ops.reduce_all(self.concentration > 1.0, axis=-1), mode, nan)
        return control_flow_ops.with_dependencies([check_ops.assert_less(array_ops.ones([], self.dtype), self.concentration, message='Mode undefined when any concentration <= 1')], mode)
