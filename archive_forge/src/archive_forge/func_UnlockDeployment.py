...
import six
def UnlockDeployment(messages, async_, deployment_full_name, lock_id):
  ...
  """
    unlock_deployment_request = messages.UnlockDeploymentRequest(lockId=int(lock_id))
    ...
    op = configmanager_util.UnlockDeployment(unlock_deployment_request, deployment_full_name)
    log.debug('LRO: %s', op.name)
    if async_:
        log.status.Print('Unlock deployment request issued for: [{0}]'.format(deployment_id))
        log.status.Print('Check operation [{}] for status.'.format(op.name))
        return op
    progress_message = 'Unlocking the deployment'
    unlock_response = configmanager_util.WaitForApplyDeploymentOperation(op, progress_message)
    if unlock_response.lockState == messages.Deployment.LockStateValueValuesEnum.UNLOCK_FAILED:
