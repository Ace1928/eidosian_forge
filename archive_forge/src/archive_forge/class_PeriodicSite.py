...
class PeriodicSite(Site, MSONable):
    ...
    def __init__(self, species: SpeciesLike | CompositionLike, coords: ArrayLike, lattice: Lattice, to_unit_cell: bool=False, coords_are_cartesian: bool=False, properties: dict | None=None, label: str | None=None, skip_checks: bool=False) -> None:
    ...
    def __eq__(self, other: object) -> bool:
        ...
        return self.species == other.species and self.lattice == other.lattice and np.allclose(self.coords, other.coords, atol=Site.position_atol) and (self.properties == other.properties)
    ...
    def as_dict(self, verbosity: int=0) -> dict:
        ...
        dct = {'species': species_list, 'abc': [float(c) for c in self._frac_coords], 'lattice': self._lattice.as_dict(verbosity=verbosity), '@module': type(self).__module__, '@class': type(self).__name__}
        if verbosity > 0:
