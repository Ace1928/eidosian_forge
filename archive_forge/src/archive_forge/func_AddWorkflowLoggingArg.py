...
def AddWorkflowLoggingArg(parser):
    ...
    log_level = base.ChoiceArgument('--call-log-level', choices={'none': 'No logging level specified.', 'log-all-calls': 'Log all calls to subworkflows or library functions and their results.', 'log-errors-only': 'Log when a call is stopped due to an exception.', 'log-none': 'Perform no call logging.'}, help_str='Level of call logging to apply by default for the workflow.', default='none')
