...
class UpdateCSR(base.UpdateCommand):
    ...
    """
        ...
        repo_spec = presentation_specs.ResourcePresentationSpec('--repo', repo_resource.GetRepoResourceSpec(), 'Cloud Source Repository.', prefixes=False)
    ...
    """
        ...
        trigger = messages.BuildTrigger(description=args.description, serviceAccount=args.service_account, triggerTemplate=messages.RepoSource(repoName=repo, branchName=args.branch_pattern, tagName=args.tag_pattern))
        trigger_utils.ParseRequireApproval(trigger, args, messages)
        trigger_utils.ParseBuildConfigArgsForUpdate(trigger, old_trigger, args, messages, update_mask, '', has_build_config=True)
