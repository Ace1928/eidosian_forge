...
class AQTSampler(cirq.Sampler):
    ...
    def _generate_json(self, circuit: cirq.AbstractCircuit, param_resolver: cirq.ParamResolverOrSimilarType) -> str:
    ...
    def _send_json(self, *, json_str: str, id_str: Union[str, uuid.UUID], repetitions: int=1, num_qubits: int=1) -> np.ndarray:
        ...
        response = put(self.remote_host, data={'data': json_str, 'access_token': self.access_token, 'repetitions': repetitions, 'no_qubits': num_qubits}, headers=header)
        ...
        id_str = data['id']
        while True:
            response = put(self.remote_host, data={'id': id_str, 'access_token': self.access_token}, headers=header)
            response = response.json()
            data = cast(Dict, response)
            if 'status' not in data.keys():
                raise RuntimeError('Got unexpected return data from AQT server: \n' + str(data))
    ...
    def run_sweep(self, program: cirq.AbstractCircuit, params: cirq.Sweepable, repetitions: int=1) -> Sequence[cirq.Result]:
        ...
        for param_resolver in cirq.to_resolvers(params):
            ...
            results = self._send_json(json_str=json_str, id_str=id_str, repetitions=repetitions, num_qubits=num_qubits)
