...
class WheeledModel:
    ...
    def log_model(cls, model_uri, registered_model_name=None):
        ...
        return Model.log(artifact_path=None, flavor=WheeledModel(model_uri), registered_model_name=registered_model_name or parsed_uri.name)
    ...
    def save_model(self, path, mlflow_model=None):
        ...
        if conda_env is None and (not os.path.isfile(pip_requirements_path)):
            raise MlflowException('Cannot add libraries for model with no logged dependencies.', BAD_REQUEST)
        if not os.path.isfile(pip_requirements_path):
            self._create_pip_requirement(conda_env_path, pip_requirements_path)
        WheeledModel._download_wheels(pip_requirements_path=pip_requirements_path, dst_path=wheels_dir)
        shutil.copy2(pip_requirements_path, os.path.join(local_model_path, _ORIGINAL_REQ_FILE_NAME))
        pip_deps = self._overwrite_pip_requirements_with_wheels(pip_requirements_path=pip_requirements_path, wheels_dir=wheels_dir)
        self._update_conda_env(pip_deps, conda_env_path)
        mlflow_model = self._update_mlflow_model(original_model_metadata=model_metadata, mlflow_model=mlflow_model)
