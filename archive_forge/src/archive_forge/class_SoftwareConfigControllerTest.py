...
class SoftwareConfigControllerTest(tools.ControllerTest, common.HeatTestCase):
    ...
    def test_index(self, mock_enforce):
        ...
        with mock.patch.object(self.controller.rpc_client, 'list_software_configs', return_value=[]):
            resp = self.controller.index(req, tenant_id=self.tenant)
            self.assertEqual({'software_configs': []}, resp)

    @mock.patch.object(policy.Enforcer, 'enforce')
    def test_index_limit_negative(self, mock_enforce):
        self._mock_enforce_setup(mock_enforce, 'index')
        params = {'limit': -1}
        with mock.patch.object(self.controller.rpc_client, 'list_software_configs', return_value=[]) as mock_call:
    ...
    def test_show(self, mock_enforce):
        ...
        return_value = {'id': config_id, 'name': 'config_mysql', 'group': 'Heat::Shell', 'config': '#!/bin/bash', 'inputs': [], 'ouputs': [], 'options': []}
        expected = {'software_config': return_value}
        with mock.patch.object(self.controller.rpc_client, 'show_software_config', return_value=return_value):
            resp = self.controller.show(req, config_id=config_id, tenant_id=self.tenant)
    ...
    def test_show_not_found(self, mock_enforce):
        ...
        with mock.patch.object(self.controller.rpc_client, 'show_software_config', side_effect=tools.to_remote_error(error)):
            resp = tools.request_with_middleware(fault.FaultWrapper, self.controller.show, req, config_id=config_id, tenant_id=self.tenant)
    ...
    def test_create(self, mock_enforce):
        ...
        expected = {'software_config': return_value}
        with mock.patch.object(self.controller.rpc_client, 'create_software_config', return_value=return_value):
    ...
    def test_delete(self, mock_enforce):
        ...
        return_value = None
        with mock.patch.object(self.controller.rpc_client, 'delete_software_config', return_value=return_value):
            self.assertRaises(webob.exc.HTTPNoContent, self.controller.delete, req, config_id=config_id, tenant_id=self.tenant)
