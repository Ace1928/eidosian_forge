...
def batch_norm_with_global_normalization_grad_eager_fallback(t: _atypes.TensorFuzzingAnnotation[TV_BatchNormWithGlobalNormalizationGrad_T], m: _atypes.TensorFuzzingAnnotation[TV_BatchNormWithGlobalNormalizationGrad_T], v: _atypes.TensorFuzzingAnnotation[TV_BatchNormWithGlobalNormalizationGrad_T], gamma: _atypes.TensorFuzzingAnnotation[TV_BatchNormWithGlobalNormalizationGrad_T], backprop: _atypes.TensorFuzzingAnnotation[TV_BatchNormWithGlobalNormalizationGrad_T], variance_epsilon: float, scale_after_normalization: bool, name, ctx):
    ...
    _attrs = ('T', _attr_T, 'variance_epsilon', variance_epsilon, 'scale_after_normalization', scale_after_normalization)
    _result = _execute.execute(b'BatchNormWithGlobalNormalizationGrad', 5, inputs=_inputs_flat, attrs=_attrs, ctx=ctx, name=name)
