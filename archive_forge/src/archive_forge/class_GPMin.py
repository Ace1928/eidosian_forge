...
class GPMin(Optimizer, GaussianProcess):
    ...
    def __init__(self, atoms, restart=None, logfile='-', trajectory=None, prior=None, kernel=None, master=None, noise=None, weight=None, scale=None, force_consistent=None, batch_size=None, bounds=None, update_prior_strategy='maximum', update_hyperparams=False):
        ...
        """
        if len(atoms) > 100:
            warning = 'Possible Memory Issue. There are more than 100 atoms in the unit cell. The memory of the process will increase with the number of steps, potentially causing a memory issue. Consider using a different optimizer.'
        ...
        if update_hyperparams:
            ...
            if bounds is not None:
                warning = 'The parameter bounds is of no use if update_hyperparams is False. The value provided by the user is being ignored.'
                warnings.warn(warning, UserWarning)
            if batch_size is not None:
                warning = 'The parameter batch_size is of no use if update_hyperparams is False. The value provided by the user is being ignored.'
                warnings.warn(warning, UserWarning)
            self.eps = False
            self.nbatch = None
