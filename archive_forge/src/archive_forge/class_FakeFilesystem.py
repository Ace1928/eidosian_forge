...
class FakeFilesystem(object):
    ...
    **Not** thread-safe when a single filesystem is mutated at the same
    ...
    def normpath(cls, path):
        ...
        if not path:
            raise ValueError('This filesystem can only normalize paths that are not empty')
        if not path.startswith(cls.root_path):
            raise ValueError("This filesystem can only normalize paths that start with %s: '%s' is not valid" % (cls.root_path, path))
    ...
    def __init__(self, deep_copy=True):
        ...
        self._reverse_mapping = {self.root_path: self._root}
        if deep_copy:
    ...
    def _get_item(self, path, links=None):
        ...
        if 'target' in node.metadata:
            ...
            if links is None:
                links = []
            if path in links:
                raise ValueError('Recursive link following not allowed (loop %s detected)' % (links + [path]))
            else:
                links.append(path)
    ...
    def ls(self, path, absolute=False):
        ...
        return [selector_func(node, child_node) for child_node in child_node_it]
    ...
    def delete(self, path, recursive=False):
        ...
        if recursive:
            ...
            if node_child_count:
                raise ValueError("Can not delete '%s', it has %s children" % (path, node_child_count))
            child_paths = []
