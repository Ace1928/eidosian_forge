...
class ExecutionEngineTests(object):
    ...
    class Tests(TestCase):
        ...
        def test_filter(self):
            a = ArrayDataFrame([[1, 2], [None, 2], [None, 1], [3, 4], [None, 4]], 'a:double,b:int')
        ...
        def test_select(self):
            ...
            df_eq(b, [[2, '3'], [2, '3'], [1, '2'], [4, '5'], [4, '5']], 'b:int,c:str', throw=True)
            b = fa.select(a, col('b'), (col('b') + 1).alias('c').cast(str), distinct=True)
            df_eq(b, [[2, '3'], [1, '2'], [4, '5']], 'b:int,c:str', throw=True)
            ...
            col_b = ff.sum(col('b'))
            b = fa.select(a, col('a'), col_b.cast(float).alias('c'), having=(col_b >= 7) | (col('a') == 1))
            df_eq(b, [[1, 2], [None, 7]], 'a:double,c:double', throw=True)
            col_b = ff.sum(col('b'))
            b = fa.select(a, col('a'), lit(1, 'o').cast(str), col_b.cast(float).alias('c'), having=(col_b >= 7) | (col('a') == 1))
            df_eq(b, [[1, '1', 2], [None, '1', 7]], 'a:double,o:str,c:double', throw=True)

        @pytest.mark.skipif(not HAS_QPD, reason='qpd not working')
        def test_assign(self):
            a = ArrayDataFrame([[1, 2], [None, 2], [None, 1], [3, 4], [None, 4]], 'a:double,b:int')
            b = fa.assign(a, x=1, b=col('b').cast(str), c=(col('b') + 1).cast(int))
            df_eq(b, [[1, '2', 1, 3], [None, '2', 1, 3], [None, '1', 1, 2], [3, '4', 1, 5], [None, '4', 1, 5]], 'a:double,b:str,x:long,c:long', throw=True)

        @pytest.mark.skipif(not HAS_QPD, reason='qpd not working')
        def test_aggregate(self):
            a = ArrayDataFrame([[1, 2], [None, 2], [None, 1], [3, 4], [None, 4]], 'a:double,b:int')
            b = fa.aggregate(df=a, b=ff.max(col('b')), c=(ff.max(col('b')) * 2).cast('int32').alias('c'))
            df_eq(b, [[4, 8]], 'b:int,c:int', throw=True)
            b = fa.aggregate(a, 'a', b=ff.max(col('b')), c=(ff.max(col('b')) * 2).cast('int32').alias('c'))
            df_eq(b, [[None, 4, 8], [1, 2, 4], [3, 4, 8]], 'a:double,b:int,c:int', throw=True)
        ...
        def test_map(self):
            ...
            def on_init(partition_no, data):
                ...
                data.peek_array()
            e = self.engine
            o = ArrayDataFrame([[1, 2], [None, 2], [None, 1], [3, 4], [None, 4]], 'a:double,b:int')
            a = fa.as_fugue_engine_df(e, o)
            c = e.map_engine.map_dataframe(a, noop, a.schema, PartitionSpec())
            df_eq(c, o, throw=True)
            c = e.map_engine.map_dataframe(a, noop, a.schema, PartitionSpec(by=['a'], presort='b'))
            df_eq(c, o, throw=True)
            c = e.map_engine.map_dataframe(a, select_top, a.schema, PartitionSpec(by=['a'], presort='b'))
            df_eq(c, [[None, 1], [1, 2], [3, 4]], 'a:double,b:int', throw=True)
            c = e.map_engine.map_dataframe(a, select_top, a.schema, PartitionSpec(partition_by=['a'], presort='b DESC'))
            df_eq(c, [[None, 4], [1, 2], [3, 4]], 'a:double,b:int', throw=True)
            c = e.map_engine.map_dataframe(a, select_top, a.schema, PartitionSpec(partition_by=['a'], presort='b DESC', num_partitions=3), on_init=on_init)
            df_eq(c, [[None, 4], [1, 2], [3, 4]], 'a:double,b:int', throw=True)
        ...
        def test_join_multiple(self):
            ...
            df_eq(d, [[1, 2, 20, 200], [3, 4, 40, 400]], 'a:int,b:int,c:int,d:int', throw=True)
        ...
        def test_union(self):
            e = self.engine
            a = fa.as_fugue_engine_df(e, [[1, 2, 3], [4, None, 6]], 'a:double,b:double,c:int')
            b = fa.as_fugue_engine_df(e, [[1, 2, 33], [4, None, 6]], 'a:double,b:double,c:int')
            c = fa.union(a, b)
            df_eq(c, [[1, 2, 3], [4, None, 6], [1, 2, 33]], 'a:double,b:double,c:int', throw=True)
            c = fa.union(a, b, distinct=False)
            df_eq(c, [[1, 2, 3], [4, None, 6], [1, 2, 33], [4, None, 6]], 'a:double,b:double,c:int', throw=True)
            d = fa.union(a, b, c, distinct=False)
            df_eq(d, [[1, 2, 3], [4, None, 6], [1, 2, 33], [4, None, 6], [1, 2, 3], [4, None, 6], [1, 2, 33], [4, None, 6]], 'a:double,b:double,c:int', throw=True)

        def test_subtract(self):
            e = self.engine
            a = fa.as_fugue_engine_df(e, [[1, 2, 3], [1, 2, 3], [4, None, 6]], 'a:double,b:double,c:int')
            b = fa.as_fugue_engine_df(e, [[1, 2, 33], [4, None, 6]], 'a:double,b:double,c:int')
            c = fa.subtract(a, b)
            ...
            z = fa.subtract(a, x, y)
            df_eq(z, [[1, 2, 3]], 'a:double,b:double,c:int', throw=True)

        def test_intersect(self):
            e = self.engine
            a = fa.as_fugue_engine_df(e, [[1, 2, 3], [4, None, 6], [4, None, 6]], 'a:double,b:double,c:int')
            b = fa.as_fugue_engine_df(e, [[1, 2, 33], [4, None, 6], [4, None, 6], [4, None, 6]], 'a:double,b:double,c:int')
            c = fa.intersect(a, b)
            df_eq(c, [[4, None, 6]], 'a:double,b:double,c:int', throw=True)
            x = fa.as_fugue_engine_df(e, [[1, 2, 33]], 'a:double,b:double,c:int')
            y = fa.as_fugue_engine_df(e, [[4, None, 6], [4, None, 6], [4, None, 6]], 'a:double,b:double,c:int')
            z = fa.intersect(a, x, y)
            df_eq(z, [], 'a:double,b:double,c:int', throw=True)

        def test_distinct(self):
            e = self.engine
            a = fa.as_fugue_engine_df(e, [[4, None, 6], [1, 2, 3], [4, None, 6]], 'a:double,b:double,c:int')
            c = fa.distinct(a)
            df_eq(c, [[4, None, 6], [1, 2, 3]], 'a:double,b:double,c:int', throw=True)

        def test_dropna(self):
            e = self.engine
            a = fa.as_fugue_engine_df(e, [[4, None, 6], [1, 2, 3], [4, None, None]], 'a:double,b:double,c:double')
            c = fa.dropna(a)
            ...
            df_eq(c, [[1, 2, 3]], 'a:double,b:double,c:double', throw=True)
            df_eq(d, [[4, None, 6], [1, 2, 3], [4, None, None]], 'a:double,b:double,c:double', throw=True)
            df_eq(f, [[4, None, 6], [1, 2, 3]], 'a:double,b:double,c:double', throw=True)
            df_eq(g, [[4, None, 6], [1, 2, 3]], 'a:double,b:double,c:double', throw=True)
            df_eq(h, [[4, None, 6], [1, 2, 3], [4, None, None]], 'a:double,b:double,c:double', throw=True)
        ...
        def test_fillna(self):
            ...
            g = fa.fillna(a, {'b': 99, 'c': -99}, subset=['c'])
            df_eq(c, [[4, 1, 6], [1, 2, 3], [4, 1, 1]], 'a:double,b:double,c:double', throw=True)
            df_eq(d, [[4, 99, 6], [1, 2, 3], [4, 99, -99]], 'a:double,b:double,c:double', throw=True)
            df_eq(f, [[4, None, 6], [1, 2, 3], [4, None, -99]], 'a:double,b:double,c:double', throw=True)
        ...
        def test_take(self):
            ...
            a = fa.as_fugue_engine_df(e, [['a', 2, 3], ['a', 3, 4], ['b', 1, 2], ['b', 2, 2], [None, 4, 2], [None, 2, 1]], 'a:str,b:int,c:long')
            ...
            df_eq(c, [[None, 4, 2], [None, 2, 1]], 'a:str,b:int,c:long', throw=True)
            df_eq(d, [['a', 3, 4], ['b', 2, 2], [None, 4, 2]], 'a:str,b:int,c:long', throw=True)
            df_eq(f, [['a', 2, 3], ['a', 3, 4], ['b', 1, 2], [None, 2, 1]], 'a:str,b:int,c:long', throw=True)
            df_eq(g, [['b', 1, 2], ['b', 2, 2]], 'a:str,b:int,c:long', throw=True)
            df_eq(h, [[None, 4, 2], [None, 2, 1]], 'a:str,b:int,c:long', throw=True)
