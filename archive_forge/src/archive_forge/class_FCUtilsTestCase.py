...
class FCUtilsTestCase(test_base.BaseTestCase):
    ...
    _autospec_classes = [fc_utils.win32utils.Win32Utils, fc_utils.diskutils.DiskUtils]
    ...
    def setUp(self):
        ...
        self._run_mocker = mock.patch.object(self._fc_utils, '_run_and_check_output')
    ...
    def _setup_lib_mocks(self):
        ...
        self._ctypes_mocker = mock.patch.object(fc_utils, 'ctypes', self._ctypes)
        self._ctypes_mocker.start()

    def test_run_and_check_output(self):
        self._run_mocker.stop()
        self._fc_utils._run_and_check_output(adapter_name=self._FAKE_ADAPTER_NAME)
        mock_win32_run = self._fc_utils._win32_utils.run_and_check_output
        mock_win32_run.assert_called_once_with(adapter_name=self._FAKE_ADAPTER_NAME, failure_exc=exceptions.FCWin32Exception)
    ...
    def test_get_adapter_name(self):
        ...
        resulted_adapter_name = self._fc_utils._get_adapter_name(fake_adapter_index)
        args_list = self._mock_run.call_args_list[0][0]
        self.assertEqual(fc_utils.hbaapi.HBA_GetAdapterName, args_list[0])
        self.assertIsInstance(args_list[1], ctypes.c_uint32)
        self.assertEqual(fake_adapter_index, args_list[1].value)
        buff = ctypes.cast(args_list[2], ctypes.POINTER(ctypes.c_char * 256)).contents
