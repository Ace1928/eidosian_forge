...
class BuildersTest(test.TestCase):
    ...
    def test_run_iterations_suspended(self):
        flow = lf.Flow('root')
        tasks = test_utils.make_many(2, task_cls=test_utils.TaskNoRequiresNoReturns)
        flow.add(*tasks)
        runtime, machine, memory, machine_runner = self._make_machine(flow, initial_state=st.RUNNING)
        transitions = []
        ...
        self.assertEqual(st.PENDING, runtime.storage.get_atom_state(tasks[1].name))
    ...
    def test_builder_manual_process(self):
        ...
        self.assertEqual(st.PENDING, runtime.storage.get_atom_state(tasks[0].name))
        machine.initialize()
        ...
        next_event = cb(last_state, machine.current_state, builder.START, *args, **kwargs)
        ...
        last_state = machine.current_state
        cb, args, kwargs = reaction
        next_event = cb(last_state, machine.current_state, next_event, *args, **kwargs)
        reaction, terminal = machine.process_event(next_event)
        self.assertFalse(terminal)
        self.assertEqual(st.WAITING, machine.current_state)
        self.assertRaises(excp.NotFound, machine.process_event, 'poke')
        self.assertEqual(st.RUNNING, runtime.storage.get_atom_state(tasks[0].name))
        last_state = machine.current_state
        cb, args, kwargs = reaction
        next_event = cb(last_state, machine.current_state, next_event, *args, **kwargs)
        reaction, terminal = machine.process_event(next_event)
