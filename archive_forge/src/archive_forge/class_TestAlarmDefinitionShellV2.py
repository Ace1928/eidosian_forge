...
class TestAlarmDefinitionShellV2(base.BaseTestCase):
    ...
    def test_alarm_definitions_list(self, mc):
        mc.return_value = c = FakeV2Client()
        c.alarm_definitions.list.return_value = [{'name': 'ntp_sync_check', 'id': 'aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee', 'expression': '(max(ntp.offset{}, deterministic)>=1)', 'match_by': ['hostname'], 'description': 'NTP time sync check', 'actions_enabled': True, 'deterministic': True, 'alarm_actions': ['aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee'], 'ok_actions': [], 'undetermined_actions': [], 'severity': 'HIGH'}]
        name, cmd_class = migr.create_command_class('do_alarm_definition_list', shell)
    ...
    def _patch_test(mc, args, called=True, **kwargs):
        mc.return_value = c = FakeV2Client()
        name, cmd_clazz = migr.create_command_class('do_alarm_definition_patch', shell)
        cmd = cmd_clazz(mock.Mock(), mock.Mock())
        parser = cmd.get_parser(name)
