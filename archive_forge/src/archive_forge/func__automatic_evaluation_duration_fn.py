...
@staticmethod
def _automatic_evaluation_duration_fn(unit, num_eval_workers, eval_cfg, train_future, num_units_done):
    if num_units_done > 0 and train_future.done():
        return 0
    elif unit == 'episodes':
        return num_eval_workers
    else:
        return num_eval_workers * eval_cfg['rollout_fragment_length'] * eval_cfg['num_envs_per_worker']