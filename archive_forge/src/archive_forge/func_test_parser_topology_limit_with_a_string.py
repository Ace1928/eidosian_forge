...
def test_parser_topology_limit_with_a_string(self, mock_parser):
    ...
    with ExpectedException(ArgumentTypeError, 'argument --limit: spam must be an integer'):
        parser.parse_args(args=['--filter', 'bla', '--limit', 'spam', '--root', 'blabla', '--graph-type', 'tree'])