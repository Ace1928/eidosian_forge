...
def synth_circuit_from_stabilizers(stabilizers: Collection[str], allow_redundant: bool=False, allow_underconstrained: bool=False, invert: bool=False) -> QuantumCircuit:
    ...
    for i in range(len(stabilizer_list)):
        ...
        if pivot == num_qubits:
            if curr_stab.x.any():
                raise QiskitError(f'Stabilizer {i} ({stabilizer_list[i]}) anti-commutes with some of the previous stabilizers.')
            if curr_stab.phase == 2:
                raise QiskitError(f'Stabilizer {i} ({stabilizer_list[i]}) contradicts some of the previous stabilizers.')
            if curr_stab.z.any() and (not allow_redundant):
                raise QiskitError(f'Stabilizer {i} ({stabilizer_list[i]}) is a product of the others and allow_redundant is False. Add allow_redundant=True to the function call if you want to allow redundant stabilizers.')
            continue
    ...
    if used < num_qubits and (not allow_underconstrained):
        raise QiskitError('Stabilizers are underconstrained and allow_underconstrained is False. Add allow_underconstrained=True  to the function call if you want to allow underconstrained stabilizers.')
    if invert:
        return circuit
