...
class Particle:
    ...
    def apply_forces(self, particles):
        ...
        current_particle_id = self.id
        current_particle_pos = self.pos
        current_particle_mass = self.mass
        current_particle_vel = self.vel

        # Calculate forces from other particles
        for other in particles:
            if other.id != current_particle_id:
                force += self.calculate_force(other)

        # Dark matter force
        direction_to_center = center - current_particle_pos
        ...
        current_particle_vel += force / current_particle_mass * DT
        current_particle_pos += (
            current_particle_vel * DT + 0.5 * force / current_particle_mass * DT**2
        )

        # Ensure the particle stays within the simulation boundaries
        current_particle_pos = np.clip(current_particle_pos, 0, [width, height])

        # Update instance variables
        self.vel = current_particle_vel
        self.pos = current_particle_pos
        ...
        pygame.draw.circle(
            ...
            (
                max(
                    min(
                        30,
                        PARTICLE_SIZE * (ELEMENTS[self.element]["mass"] / 10),
                    ),
                    2,
                )
            ),
