...
def switch_controller_stp_instance(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('switch-controller', 'stp-instance', filtered_data, vdom=vdom)
        current_data = fos.get('switch-controller', 'stp-instance', vdom=vdom, mkey=mkey)
    ...
    if state == 'present' or state is True:
        return fos.set('switch-controller', 'stp-instance', data=filtered_data, vdom=vdom)
    elif state == 'absent':
        return fos.delete('switch-controller', 'stp-instance', mkey=filtered_data['id'], vdom=vdom)
    else:
