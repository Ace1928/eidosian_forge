...
class EngineProcessor(abstract_processor.AbstractProcessor):
    ...
    def __init__(self, project_id: str, processor_id: str, context: 'engine_base.EngineContext', _processor: Optional[quantum.QuantumProcessor]=None) -> None:
    ...
    def __repr__(self) -> str:
        return f'<EngineProcessor: processor_id={self.processor_id!r}, project_id={self.project_id!r}>'
    ...
    def get_sampler(self, run_name: str='', device_config_name: str='') -> 'cg.engine.ProcessorSampler':
        ...
        Returns:
            ...
            when sampled.1
        """
        return processor_sampler.ProcessorSampler(processor=self, run_name=run_name, device_config_name=device_config_name)

    async def run_batch_async(self, programs: Sequence[cirq.AbstractCircuit], program_id: Optional[str]=None, job_id: Optional[str]=None, params_list: Optional[Sequence[cirq.Sweepable]]=None, repetitions: int=1, program_description: Optional[str]=None, program_labels: Optional[Dict[str, str]]=None, job_description: Optional[str]=None, job_labels: Optional[Dict[str, str]]=None, run_name: str='', device_config_name: str='') -> 'abstract_job.AbstractJob':
        ...
        """
        return await self.engine().run_batch_async(programs=programs, program_id=program_id, params_list=list(params_list) if params_list is not None else None, repetitions=repetitions, program_description=program_description, program_labels=program_labels, job_description=job_description, job_labels=job_labels, processor_id=self.processor_id, run_name=run_name, device_config_name=device_config_name)

    async def run_calibration_async(self, layers: List[calibration_layer.CalibrationLayer], program_id: Optional[str]=None, job_id: Optional[str]=None, program_description: Optional[str]=None, program_labels: Optional[Dict[str, str]]=None, job_description: Optional[str]=None, job_labels: Optional[Dict[str, str]]=None) -> 'abstract_job.AbstractJob':
        ...
        Returns:
            ...
            calibration_results().
        """
        return await self.engine().run_calibration_async(layers=layers, processor_id=self.processor_id, program_id=program_id, job_id=job_id, program_description=program_description, program_labels=program_labels, job_description=job_description, job_labels=job_labels)

    async def run_sweep_async(self, program: cirq.AbstractCircuit, program_id: Optional[str]=None, job_id: Optional[str]=None, params: cirq.Sweepable=None, repetitions: int=1, program_description: Optional[str]=None, program_labels: Optional[Dict[str, str]]=None, job_description: Optional[str]=None, job_labels: Optional[Dict[str, str]]=None, run_name: str='', device_config_name: str='') -> 'abstract_job.AbstractJob':
        ...
        Raises:
            ...
            ValueError: If either `run_name` and `device_config_name` are set but
                `processor_id` is empty.
        """
        return await self.engine().run_sweep_async(program=program, program_id=program_id, job_id=job_id, params=params, repetitions=repetitions, program_description=program_description, program_labels=program_labels, job_description=job_description, job_labels=job_labels, processor_id=self.processor_id, run_name=run_name, device_config_name=device_config_name)
    ...
    @cirq._compat.deprecated_parameter(deadline='v1.0', fix='Change earliest_timestamp_seconds to earliest_timestamp.', parameter_desc='earliest_timestamp_seconds', match=lambda args, kwargs: 'earliest_timestamp_seconds' in kwargs, rewrite=lambda args, kwargs: (args, _fix_deprecated_seconds_kwargs(kwargs)))
    @cirq._compat.deprecated_parameter(deadline='v1.0', fix='Change latest_timestamp_seconds to latest_timestamp.', parameter_desc='latest_timestamp_seconds', match=lambda args, kwargs: 'latest_timestamp_seconds' in kwargs, rewrite=lambda args, kwargs: (args, _fix_deprecated_seconds_kwargs(kwargs)))
    def list_calibrations(self, earliest_timestamp: Optional[Union[datetime.datetime, datetime.date, int]]=None, latest_timestamp: Optional[Union[datetime.datetime, datetime.date, int]]=None) -> List[calibration.Calibration]:
        ...
        if earliest_timestamp_seconds and latest_timestamp_seconds:
            filter_str = f'timestamp >= {earliest_timestamp_seconds:d} AND timestamp <= {latest_timestamp_seconds:d}'
            ...
            filter_str = ''
        response = self.context.client.list_calibrations(self.project_id, self.processor_id, filter_str)
    ...
    def get_calibration(self, calibration_timestamp_seconds: int) -> calibration.Calibration:
        ...
        """
        response = self.context.client.get_calibration(self.project_id, self.processor_id, calibration_timestamp_seconds)
    ...
    def create_reservation(self, start_time: datetime.datetime, end_time: datetime.datetime, whitelisted_users: Optional[List[str]]=None):
        ...
        """
        response = self.context.client.create_reservation(self.project_id, self.processor_id, start_time, end_time, whitelisted_users)
        return response
    ...
    def _delete_reservation(self, reservation_id: str):
        ...
        """
        return self.context.client.delete_reservation(self.project_id, self.processor_id, reservation_id)
    ...
    def remove_reservation(self, reservation_id: str):
        ...
        if proc is not None:
            ...
            freeze = None
        if not freeze:
            raise ValueError('Cannot determine freeze_schedule from processor.Call _cancel_reservation or _delete_reservation.')
    ...
    def get_reservation(self, reservation_id: str) -> Optional[quantum.QuantumReservation]:
        ...
        return self.context.client.get_reservation(self.project_id, self.processor_id, reservation_id)

    def update_reservation(self, reservation_id: str, start_time: Optional[datetime.datetime]=None, end_time: Optional[datetime.datetime]=None, whitelisted_users: Optional[List[str]]=None):
        ...
        """
        return self.context.client.update_reservation(self.project_id, self.processor_id, reservation_id, start=start_time, end=end_time, whitelisted_users=whitelisted_users)

    def list_reservations(self, from_time: Union[None, datetime.datetime, datetime.timedelta]=datetime.timedelta(), to_time: Union[None, datetime.datetime, datetime.timedelta]=datetime.timedelta(weeks=2)) -> List[quantum.QuantumTimeSlot]:
