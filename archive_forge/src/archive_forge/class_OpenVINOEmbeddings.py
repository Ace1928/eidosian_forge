...
class OpenVINOEmbeddings(BaseModel, Embeddings):
    ...
    def __init__(self, **kwargs: Any):
        ...
        try:
            ...
            raise ValueError("Could not import optimum-intel python package. Please install it with: pip install -U 'optimum[openvino,nncf]'") from e
        try:
            from huggingface_hub import HfApi
        except ImportError as e:
            raise ValueError('Could not import huggingface_hub python package. Please install it with: `pip install -U huggingface_hub`.') from e
        ...
        if require_model_export(self.model_name_or_path):
            self.ov_model = OVModelForFeatureExtraction.from_pretrained(self.model_name_or_path, export=True, **self.model_kwargs)
        else:
    ...
    def encode(self, sentences: Any, batch_size: int=4, show_progress_bar: bool=False, convert_to_numpy: bool=True, convert_to_tensor: bool=False, mean_pooling: bool=False, normalize_embeddings: bool=True) -> Any:
        """
