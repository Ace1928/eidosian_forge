...
def _include_stylesheets_inplace(self, obj):
    if hasattr(obj, 'objects'):
        obj.objects[:] = [self._include_stylesheets_inplace(o) for o in obj.objects]
    else:
        obj = _panel(obj)
    obj.stylesheets = [stylesheet for stylesheet in self._stylesheets + self.stylesheets if stylesheet not in obj.stylesheets] + obj.stylesheets
    return obj