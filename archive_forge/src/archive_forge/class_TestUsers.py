...
class TestUsers(base.TestCase):
    ...
    def test_delete_user(self):
        ...
        user_resource_uri = self._get_keystone_mock_url(resource='users', append=[user_data.user_id])
        self.register_uris([dict(method='GET', uri=self._get_keystone_mock_url(resource='users', qs_elements=['name=%s' % user_data.name]), status_code=200, json=self._get_user_list(user_data)), dict(method='DELETE', uri=user_resource_uri, status_code=204)])
    ...
    def test_remove_user_from_group(self):
        user_data = self._get_user_data()
        group_data = self._get_group_data()
        self.register_uris([dict(method='GET', uri=self._get_keystone_mock_url(resource='users'), json=self._get_user_list(user_data)), dict(method='GET', uri=self._get_keystone_mock_url(resource='groups'), status_code=200, json={'groups': [group_data.json_response['group']]}), dict(method='DELETE', uri=self._get_keystone_mock_url(resource='groups', append=[group_data.group_id, 'users', user_data.user_id]), status_code=204)])
