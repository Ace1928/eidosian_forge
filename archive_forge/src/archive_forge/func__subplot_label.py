...
def _subplot_label(self, axis):
    ...
    if self.sublabel_format and (not self.adjoined) and (layout_num > 0):
        ...
        at = AnchoredText(self.sublabel_format.format(**labels), loc=3, bbox_to_anchor=self.sublabel_position, frameon=False, prop=dict(size=self.sublabel_size, weight='bold'), bbox_transform=axis.transAxes)
