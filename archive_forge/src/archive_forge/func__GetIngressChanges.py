...
def _GetIngressChanges(args):
    ...
    if platform == platforms.PLATFORM_MANAGED:
        return config_changes.SetAnnotationChange(service.INGRESS_ANNOTATION, args.ingress)
        ...
        raise serverless_exceptions.ConfigurationError('Ingress value `{}` is not supported on platform `{}`.'.format(args.ingress, platform))