...
class NetAppONTAPSnapmirror(object):
    ...
    def check_parameters(self):
        ...
        if self.parameters.get('source_path') or self.parameters.get('destination_path'):
            if (not self.parameters.get('destination_path') or not self.parameters.get('source_path')) and (self.parameters['state'] == 'present' or (self.parameters['state'] == 'absent' and (not self.parameters.get('destination_path')))):
            ...
            self.parameters['destination_path'] = self.parameters['destination_vserver'] + ':' + self.parameters['destination_volume']
    ...
    def snapmirror_get_rest(self, destination=None):
        ...
        if error:
            self.module.fail_json(msg='Error getting SnapMirror %s: %s' % (destination, to_native(error)), exception=traceback.format_exc())
    ...
    def get_actions(self):
        ...
        if cd_action is None and self.parameters['state'] == 'present' and (not restore):
            ...
            if modify and 'relationship_type' in modify:
                self.module.fail_json(msg='Error: cannot modify relationship_type from %s to %s.' % (current['relationship_type'], modify['relationship_type']))
        actions = []
    ...
    def apply(self):
        ...
        if self.parameters['state'] == 'present' and self.parameters.get('connection_type') == 'ontap_elementsw':
            ...
            if current_elementsw_ontap is None:
                self.module.fail_json(msg='Error: creating an ONTAP to ElementSW snapmirror relationship requires an established SnapMirror relation from ElementSW to ONTAP cluster')
