    ...
    def leave_operation_definition(self, operation: OperationDefinitionNode, *_args: Any) -> None:
        ...
        for variable_def in self.variable_defs:
            ...
            if variable_name not in variable_name_used:
                self.report_error(GraphQLError(f"Variable '${variable_name}' is never used in operation '{operation.name.value}'." if operation.name else f"Variable '${variable_name}' is never used.", variable_def))
