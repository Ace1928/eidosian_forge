...
class NormalizeMethodCalls(Transformation):
    ...
    def __init__(self):
        ...
        self.imports = {'builtins': 'builtins', mangle('__dispatch__'): '__dispatch__'}
        self.to_import = set()
    ...
    def visit_Module(self, node):
        ...
        imports = [ast.Import(names=[ast.alias(name=mod[17:], asname=mod)]) for mod in new_imports]
    ...
    def visit_Attribute(self, node):
        ...
        if node.attr in methods:
            ...
            if obj is not None:
                ...
                z = ast.Call(ast.Attribute(ast.Name(mangle('functools'), ast.Load(), None, None), 'partial', ast.Load()), [func, obj], [])
                return z
            else:
                return node
        elif isinstance(node.value, ast.Name) and node.value.id in self.imports:
            module_id = self.imports[node.value.id]
            if node.attr not in MODULES[self.renamer(module_id, MODULES)[1]]:
                msg = '`' + node.attr + "' is not a member of " + demangle(module_id) + ' or Pythran does not support it'
                raise PythranSyntaxError(msg, node)
            node.value.id = module_id
            self.update = True
            return node
            ...
            self.update = True
            call = ast.Call(ast.Attribute(ast.Name('builtins', ast.Load(), None, None), 'getattr', ast.Load()), [node.value, ast.Constant(node.attr, None)], [])
            if isinstance(node.ctx, ast.Store):
                assert node.attr in ('real', 'imag'), 'only store to imag/real'
                return ast.Subscript(call, ast.Slice(None, None, None), node.ctx)
            else:
                return call
    ...
    def visit_Call(self, node):
        ...
        if isinstance(node.func, ast.Attribute):
            ...
            if ismethod or isfunction:
                if not ismethod and self.baseobj(node.func.value):
                    raise PythranSyntaxError("'{}' called as a method while it's a function".format(node.func.attr), node)
    ...
    def visit_BinOp(self, node):
        ...
        if isinstance(node.op, ast.Mod) and isstr(node.left):
            self.update = True
            return ast.Call(ast.Attribute(ast.Attribute(ast.Name('builtins', ast.Load(), None, None), 'str', ast.Load()), '__mod__', ast.Load()), [node.left, node.right], [])
        return node