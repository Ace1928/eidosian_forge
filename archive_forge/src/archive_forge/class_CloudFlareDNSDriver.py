...
class CloudFlareDNSDriver(DNSDriver):
    ...
    RECORD_TYPE_MAP = {RecordType.A: 'A', RecordType.AAAA: 'AAAA', RecordType.CNAME: 'CNAME', RecordType.MX: 'MX', RecordType.TXT: 'TXT', RecordType.SPF: 'SPF', RecordType.NS: 'NS', RecordType.SRV: 'SRV', RecordType.URL: 'LOC'}
    ZONES_PAGE_SIZE = 50
    ...
    def create_zone(self, domain, type='master', ttl=None, extra=None):
        ...
        url = '{}/zones'.format(API_BASE)
        body = {'name': domain, 'account': {'id': account}, 'type': LIBCLOUD_TO_CLOUDFLARE_ZONE_TYPE[type]}
    ...
    def update_record(self, record, name=None, type=None, data=None, extra=None):
        ...
        content, data = self._normalize_record_data_for_api(type=type, data=data)
        body = {'type': record.type if type is None else type, 'name': record.name if name is None else name, 'content': content, 'extra': record.extra or {}}
        if data:
            body['data'] = data
    ...
    def _to_zone(self, item):
        return Zone(id=item['id'], domain=item['name'], type=CLOUDFLARE_TO_LIBCLOUD_ZONE_TYPE[item['type']], ttl=None, driver=self, extra={key: item.get(key) for key in ZONE_EXTRA_ATTRIBUTES})
    ...
    def _to_record(self, zone, item):
        ...
        return Record(id=item['id'], name=name, type=item['type'], data=data, zone=zone, driver=self, ttl=ttl, extra={key: item.get(key) for key in RECORD_EXTRA_ATTRIBUTES})
