...
def record_audio(duration: int = 5, sr: int = 44100) -> tuple[np.ndarray, int]:
    ...
    file_name = f"{get_timestamp()}_recorded_audio.wav"
    file_path = os.path.join(output_dir, file_name)
    wav_write(file_path, sr, signal.astype(np.int16))
...
def get_max_freq_components(sr: int) -> int:
    ...
    memory_info = psutil.virtual_memory()
    max_freq_components = int((sr / 2) * 0.8)
...
def save_visualization(
    ...
    try:
        if viz_type == "original_signal":
            ...
            chromagram = np.abs(fft(signal))[:12]
            ...
            harmonic_signal = np.abs(fft(signal))
...
def main():
    ...
    for viz_type in visualizations:
        if not running:
            ...
            print("Stopping visualization processing as running flag is set to False.")
            break
