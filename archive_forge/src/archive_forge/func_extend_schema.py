...
def extend_schema(schema, documentAST=None):
    ...
    def extend_field_map(type):
        ...
        for field_name, field in old_field_map.items():
            new_field_map[field_name] = GraphQLField(extend_field_type(field.type), description=field.description, deprecation_reason=field.deprecation_reason, args=field.args, resolver=cannot_execute_client_schema)
        extensions = type_extensions_map[type.name]
        for extension in extensions:
            for field in extension.definition.fields:
                ...
                new_field_map[field_name] = GraphQLField(build_field_type(field.type), args=build_input_values(field.arguments), resolver=cannot_execute_client_schema)
