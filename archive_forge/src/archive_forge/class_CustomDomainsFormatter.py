...
class CustomDomainsFormatter(base.BaseFormatter):
    ...
    """
        ...
        components = [('Console link', status.consoleLink if status.consoleLink else 'n/a'), ('Frontend', details.get('ip_address', 'n/a'))]
        ...
        return cp.Labeled([cp.Lines(['Google Cloud Load Balancer ({})'.format(self._GetGCLBName(resource_components)), cp.Labeled(components)])])
    ...
    """
        ...
        missing_domains = []
        max_domain_length = 0
        for domain, status in self._GetSSLStatuses(resource_components, record.resource):
        ...
        return '{0} To complete the process, please ensure the following DNS records are configured for the domains:\n    NAME{2}  TTL   TYPE  DATA\n{1}It can take up to an hour for the certificate to be provisioned.'.format(con.Colorize('!', 'yellow'), records, padding_string)
    ...
    def _FindAllComponentsByType(self, components: List[runapps.ResourceComponentStatus], rtype: str) -> List[runapps.ResourceComponentStatus]:
        found = []
        if not components:
            return found
        for comp in components:
