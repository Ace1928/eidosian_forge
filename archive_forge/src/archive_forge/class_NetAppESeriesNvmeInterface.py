...
class NetAppESeriesNvmeInterface(NetAppESeriesModule):
    ...
    def __init__(self):
        ansible_options = dict(address=dict(type='str', required=False), subnet_mask=dict(type='str', required=False), gateway=dict(type='str', required=False), config_method=dict(type='str', required=False, default='dhcp', choices=['dhcp', 'static']), mtu=dict(type='int', default=1500, required=False, aliases=['max_frame_size']), speed=dict(type='str', default='auto', required=False), state=dict(type='str', default='enabled', required=False, choices=['enabled', 'disabled']), channel=dict(type='int', required=True), controller=dict(type='str', required=True, choices=['A', 'B']))
        required_if = [['config_method', 'static', ['address', 'subnet_mask']]]
        super(NetAppESeriesNvmeInterface, self).__init__(ansible_options=ansible_options, web_services_version='02.00.0000.0000', required_if=required_if, supports_check_mode=True)
    ...
    def get_controllers(self):
        ...
        try:
            ...
            self.module.fail_json(msg='Failed to retrieve controller list! Array Id [%s]. Error [%s].' % (self.ssid, to_native(error)))
        controllers.sort()
        controllers_dict = {}
        i = ord('A')
    ...
    def get_target_interface(self):
        ...
        if self.get_target_interface_cache is None:
            ...
            if self.channel < 1 or self.channel > len(controller_ifaces):
                status_msg = ', '.join(['%s (link %s)' % (index + 1, iface['link_status']) for index, iface in enumerate(sorted_controller_ifaces)])
                self.module.fail_json(msg='Invalid controller %s NVMe channel. Available channels: %s, Array Id [%s].' % (self.controller, status_msg, self.ssid))
    ...
    def update(self):
        ...
        if iface['properties']['provider'] == 'providerInfiniband':
            if iface['properties']['ibProperties']['ipAddressData']['addressType'] != 'ipv4' or iface['properties']['ibProperties']['ipAddressData']['ipv4Data']['ipv4Address'] != self.address:
            ...
            interface_supported_speeds = [str(speed).lower().replace('speed', '').replace('gig', '') for speed in interface_data['supportedInterfaceSpeeds']]
            if self.speed not in interface_supported_speeds:
                self.module.fail_json(msg='Unsupported interface speed! Options %s. Array [%s].' % (interface_supported_speeds, self.ssid))
            ...
            if self.speed and self.speed != current_speed:
                update_required = True
            if self.mtu and iface['interface']['interfaceData']['ethernetData']['maximumFramePayloadSize'] != self.mtu:
                update_required = True
            if update_required:
                body = {'id': iface['reference'], 'settings': {'roceV2Settings': {'networkSettings': {'ipv4Enabled': self.enabled, 'ipv4Settings': {'configurationMethod': self.config_method}}}}}
                if self.config_method == 'configStatic':
                    if self.address:
                        body['settings']['roceV2Settings']['networkSettings']['ipv4Settings'].update({'address': self.address})
                    if self.subnet_mask:
                        body['settings']['roceV2Settings']['networkSettings']['ipv4Settings'].update({'subnetMask': self.subnet_mask})
                    if self.gateway:
                        body['settings']['roceV2Settings']['networkSettings']['ipv4Settings'].update({'gatewayAddress': self.gateway})
                if self.speed:
                    if self.speed == 'auto':
                        body['settings']['roceV2Settings']['networkSettings'].update({'interfaceSpeed': 'speedAuto'})
                    else:
                        body['settings']['roceV2Settings']['networkSettings'].update({'interfaceSpeed': 'speed%sgig' % self.speed})
                if self.mtu:
        ...
        if update_required and (not self.module.check_mode):
            try:
                rc, iface = self.request('storage-systems/%s/nvmeof/interfaces/%s' % (self.ssid, iface['reference']), method='POST', data=body)
            except Exception as error:
                self.module.fail_json(msg='Failed to configure interface. Array Id [%s]. Error [%s].' % (self.ssid, to_native(error)))
