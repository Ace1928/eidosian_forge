...
class AlarmManagerTest(testtools.TestCase):
    ...
    def setUp(self):
        ...
        self.alarms = {'event_alarm': {'gnocchi_aggregation_by_metrics_threshold_rule': {}, 'gnocchi_resources_threshold_rule': {}, 'name': 'event_alarm', 'gnocchi_aggregation_by_resources_threshold_rule': {}, 'event_rule': {}, 'type': 'event'}}
        self.results = {'result1': {'event_rule': {}}}
    ...
    def test_query(self, mock_am):
        ...
        am.query(query)
        url = 'v2/query/alarms'
        expected_value = '{"filter": "{\\"=\\": {\\"type\\": \\"event\\"}}"}'
        headers_value = {'Content-Type': 'application/json'}
        mock_am.assert_called_with(url, data=expected_value, headers=headers_value)
