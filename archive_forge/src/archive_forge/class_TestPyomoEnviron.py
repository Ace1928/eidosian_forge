...
class TestPyomoEnviron(unittest.TestCase):
    ...
    def test_not_auto_imported(self):
        rc = subprocess.call([sys.executable, '-c', 'import pyomo.core, sys; sys.exit( 1 if "pyomo.environ" in sys.modules else 0 )'])
        if rc:
            self.fail('Importing pyomo.core automatically imports pyomo.environ and it should not.')
