...
class SysMonitorClient(abc.ABC):
    ...
    def __init__(self, interval: Optional[float]=None, **kwargs):
        """
    ...
    def capture(self) -> SysMonitorData:
        ...
        """
        return SysMonitorData(cpu=self.get_cpu(), memory=self.get_memory(), disk=self.get_disk(), network=self.get_network(), process=self.get_process(), system=self.get_system())
