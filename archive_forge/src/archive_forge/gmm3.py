...
class SyncAsyncLauncher:
    ...
    async def async_sync_wrapper(func):
        ...
        kwargs = func.__defaults__
        validate = validate._validate_func_signature(func, args, kwargs)
        start_time = alogging.start_time()
        end_time = alogging.end_time()
        log_performance = alogging.log_performance(func, start_time, end_time)
        result = wrapper_logic.wrapper_logic(func, True, args, kwargs)
        exception = Exception
