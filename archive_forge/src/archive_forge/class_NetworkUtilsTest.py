...
class NetworkUtilsTest(test_base.BaseTestCase):
    ...
    def test_set_tcp_keepalive(self):
        ...
        calls = [mock.call.setsockopt(socket.SOL_SOCKET, socket.SO_KEEPALIVE, True)]
        if hasattr(socket, 'TCP_KEEPIDLE'):
            calls += [mock.call.setsockopt(socket.IPPROTO_TCP, socket.TCP_KEEPIDLE, 100)]
        if hasattr(socket, 'TCP_KEEPINTVL'):
            calls += [mock.call.setsockopt(socket.IPPROTO_TCP, socket.TCP_KEEPINTVL, 10)]
        if hasattr(socket, 'TCP_KEEPCNT'):
            calls += [mock.call.setsockopt(socket.IPPROTO_TCP, socket.TCP_KEEPCNT, 5)]
    ...
    def test_get_my_ipv6(self):
        sock_attrs = {'return_value.getsockname.return_value': ['2001:db8::2', '', '', '']}
        with mock.patch('socket.socket', **sock_attrs):
