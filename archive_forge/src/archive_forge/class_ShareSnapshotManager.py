...
class ShareSnapshotManager(base.MetadataCapableManager):
    ...
    def _do_create(self, share, force=False, name=None, description=None, metadata=None):
        ...
        body = {'snapshot': {'share_id': base.getid(share), 'force': force, 'name': name, 'description': description, 'metadata': metadata}}
    ...
    def create(self, share, force=False, name=None, description=None, metadata=None):
        return self._do_create(share, force, name, description, metadata)

    @api_versions.wraps('2.12')
    def manage(self, share, provider_location, driver_options=None, name=None, description=None):
        ...
        body = {'share_id': base.getid(share), 'provider_location': provider_location, 'driver_options': driver_options, 'name': name, 'description': description}
        return self._create('/snapshots/manage', {'snapshot': body}, 'snapshot')
    ...
    def list(self, detailed=True, search_opts=None, sort_key=None, sort_dir=None):
        ...
        if sort_key is not None:
            if sort_key in constants.SNAPSHOT_SORT_KEY_VALUES:
                ...
                raise ValueError('sort_key must be one of the following: %s.' % ', '.join(constants.SNAPSHOT_SORT_KEY_VALUES))
