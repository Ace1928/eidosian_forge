...
class _ClientAPI:
    ...
    def get_actor(self, name: str, namespace: Optional[str]=None) -> 'ClientActorHandle':
    ...
    def method(self, *args, **kwargs):
        ...
        error_string = f"The @ray.method decorator must be applied using at least one of the arguments in the list {valid_kwargs}, for example '@ray.method(num_returns=2)'."
        assert len(args) == 0 and len(kwargs) > 0, error_string
        for key in kwargs:
            key_error_string = f'Unexpected keyword argument to @ray.method: "{key}". The supported keyword arguments are {valid_kwargs}'
    ...
    def _internal_kv_get(self, key: Union[str, bytes], *, namespace: Optional[Union[str, bytes]]=None) -> bytes:
        """Hook for internal_kv._internal_kv_get."""
        return self.worker.internal_kv_get(_as_bytes(key), namespace=_as_bytes(namespace))
    ...
    def _internal_kv_del(self, key: Union[str, bytes], *, del_by_prefix: bool=False, namespace: Optional[Union[str, bytes]]=None) -> int:
        """Hook for internal_kv._internal_kv_del."""
        return self.worker.internal_kv_del(_as_bytes(key), del_by_prefix=del_by_prefix, namespace=_as_bytes(namespace))

    def _internal_kv_list(self, prefix: Union[str, bytes], *, namespace: Optional[Union[str, bytes]]=None) -> List[bytes]:
        """Hook for internal_kv._internal_kv_list."""
        return self.worker.internal_kv_list(_as_bytes(prefix), namespace=_as_bytes(namespace))
    ...
    def __getattr__(self, key: str):
        if not key.startswith('_'):
            raise NotImplementedError('Not available in Ray client: `ray.{}`. This method is only available within Ray remote functions and is not yet implemented in the client API.'.format(key))
