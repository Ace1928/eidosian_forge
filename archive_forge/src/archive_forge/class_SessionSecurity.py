...
class SessionSecurity(object):
    ...
    def __init__(self, negotiate_flags, exported_session_key, source='client'):
        ...
        self._source = source
        self._client_sealing_key = compkeys.get_seal_key(self.negotiate_flags, exported_session_key, SignSealConstants.CLIENT_SEALING)
        self._server_sealing_key = compkeys.get_seal_key(self.negotiate_flags, exported_session_key, SignSealConstants.SERVER_SEALING)
        ...
        if source == 'client':
            ...
            raise ValueError('Invalid source parameter %s, must be client or server' % source)
