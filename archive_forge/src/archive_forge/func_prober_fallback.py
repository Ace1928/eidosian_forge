...
def prober_fallback(self):
    ...
    if self.want.prober_fallback != 'pool' and self.have.prober_fallback == 'pool':
        if self.want.prober_preference != 'pool' and self.want.prober_pool != '':
            raise F5ModuleError('To change prober_fallback from {0} to {1}, set prober_pool to an empty string'.format(self.have.prober_fallback, self.want.prober_fallback))
    ...
    if self.want.prober_fallback == self.have.prober_preference:
        raise F5ModuleError('Cannot set prober_fallback to {0} if prober_preference on device is set to {1}.'.format(self.want.prober_fallback, self.have.prober_preference))
    if self.want.prober_fallback != self.have.prober_fallback:
