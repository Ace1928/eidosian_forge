...
class APIClient(requests.Session, BuildApiMixin, ConfigApiMixin, ContainerApiMixin, DaemonApiMixin, ExecApiMixin, ImageApiMixin, NetworkApiMixin, PluginApiMixin, SecretApiMixin, ServiceApiMixin, SwarmApiMixin, VolumeApiMixin):
    ...
    def __init__(self, base_url=None, version=None, timeout=DEFAULT_TIMEOUT_SECONDS, tls=False, user_agent=DEFAULT_USER_AGENT, num_pools=None, credstore_env=None, use_ssh_client=False, max_pool_size=DEFAULT_MAX_POOL_SIZE):
        super().__init__()
        ...
        self._auth_configs = auth.load_config(config_dict=self._general_configs, credstore_env=credstore_env)
        ...
        if base_url.startswith('http+unix://'):
            self._custom_adapter = UnixHTTPAdapter(base_url, timeout, pool_connections=num_pools, max_pool_size=max_pool_size)
            ...
            try:
                self._custom_adapter = NpipeHTTPAdapter(base_url, timeout, pool_connections=num_pools, max_pool_size=max_pool_size)
            except NameError:
                raise DockerException('Install pypiwin32 package to enable npipe:// support')
            self.mount('http+docker://', self._custom_adapter)
            ...
            try:
                ...
                raise DockerException('Install paramiko package to enable ssh:// support')
        ...
        if not isinstance(self._version, str):
            raise DockerException('Version parameter must be a string or None. Found {}'.format(type(version).__name__))
        if utils.version_lt(self._version, MINIMUM_DOCKER_API_VERSION):
            raise InvalidVersion('API versions below {} are no longer supported by this library.'.format(MINIMUM_DOCKER_API_VERSION))

    def _retrieve_server_version(self):
        try:
            return self.version(api_version=False)['ApiVersion']
        except KeyError:
            raise DockerException('Invalid response from docker daemon: key "ApiVersion" is missing.')
