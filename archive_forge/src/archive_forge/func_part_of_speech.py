...
def part_of_speech(docs):
    ...
    if test_missing(docs=docs):
        ...
        options = {'compact': True, 'color': '#1a1c1f', 'font': 'Source Sans Pro', 'collapse_punct': True, 'collapse_phrases': True}
        html = spacy.displacy.render(nlp(str(docs)), style='dep', minify=True, options=options, page=True)
