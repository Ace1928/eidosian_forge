...
class BRAINSDemonWarpInputSpec(CommandLineInputSpec):
    ...
    smoothDisplacementFieldSigma = traits.Float(desc='A gaussian smoothing value to be applied to the deformation field at each iteration.', argstr='--smoothDisplacementFieldSigma %f')
    ...
    maskProcessingMode = traits.Enum('NOMASK', 'ROIAUTO', 'ROI', 'BOBF', desc='What mode to use for using the masks: NOMASK|ROIAUTO|ROI|BOBF.  If ROIAUTO is chosen, then the mask is implicitly defined using a otsu foreground and hole filling algorithm. Where the Region Of Interest mode uses the masks to define what parts of the image should be used for computing the deformation field.  Brain Only Background Fill uses the masks to pre-process the input images by clipping and filling in the background with a predefined value.', argstr='--maskProcessingMode %s')
