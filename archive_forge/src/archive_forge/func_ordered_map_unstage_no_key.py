...
def ordered_map_unstage_no_key(indices: _atypes.TensorFuzzingAnnotation[_atypes.Int32], dtypes, capacity: int=0, memory_limit: int=0, container: str='', shared_name: str='', name=None):
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'OrderedMapUnstageNoKey', name, indices, 'capacity', capacity, 'memory_limit', memory_limit, 'dtypes', dtypes, 'container', container, 'shared_name', shared_name)
            ...
            pass
        try:
            return ordered_map_unstage_no_key_eager_fallback(indices, capacity=capacity, memory_limit=memory_limit, dtypes=dtypes, container=container, shared_name=shared_name, name=name, ctx=_ctx)
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('OrderedMapUnstageNoKey', indices=indices, dtypes=dtypes, capacity=capacity, memory_limit=memory_limit, container=container, shared_name=shared_name, name=name)
    _result = _outputs[:]
