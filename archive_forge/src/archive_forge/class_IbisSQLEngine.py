...
class IbisSQLEngine(SQLEngine):
    ...
    def select(self, dfs: DataFrames, statement: StructuredRawSQL) -> DataFrame:
        return self.to_df(self.query_to_table(statement.construct(dialect=self.dialect, log=self.log), dfs))
        ...
        if how.lower() == 'cross':
            ...
            for k in end_schema.names:
                if k not in key_schema:
                    ...
                    cols.append(ibis.coalesce(tb[k], tb[k + _JOIN_RIGHT_SUFFIX]).name(k))
            tb = tb[cols]
    ...
    def dropna(self, df: DataFrame, how: str='any', thresh: int=None, subset: Optional[List[str]]=None) -> DataFrame:
        ...
        if thresh is None:
            ...
            return self.to_df(tb, df.schema)
        assert_or_throw(how == 'any', ValueError("when thresh is set, how must be 'any'"))
        sm = None
    ...
    def fillna(self, df: DataFrame, value: Any, subset: List[str]=None) -> DataFrame:
        ...
        if isinstance(value, dict):
            ...
            st = set(names)
            assert_or_throw(st.issuperset(subset), ValueError(f'{names} is not a superset of {subset}'))
            names = subset
        vd = _build_value_dict(names)
        assert_or_throw(all((v is not None for v in vd.values())), ValueError('fillna value can not be None or contain None'))
        tb = self.to_df(df).native
        cols = [ibis.coalesce(tb[f], ibis.literal(vd[f])).name(f) if f in names else tb[f] for f in df.columns]
    ...
    def take(self, df: DataFrame, n: int, presort: str, na_position: str='last', partition_spec: Optional[PartitionSpec]=None) -> DataFrame:
        ...
        if len(_presort) == 0:
            ...
            pcols = ', '.join((self.encode_column_name(x) for x in partition_spec.partition_by))
            sql = f'SELECT * FROM (SELECT *, ROW_NUMBER() OVER (PARTITION BY {pcols}) AS __fugue_take_param FROM {tbn}) WHERE __fugue_take_param<={n}'
        ...
        if len(partition_spec.partition_by) == 0:
            ...
            return self.to_df(tb[df.columns], schema=df.schema)
        pcols = ', '.join((self.encode_column_name(x) for x in partition_spec.partition_by))
        sql = f'SELECT * FROM (SELECT *, ROW_NUMBER() OVER (PARTITION BY {pcols} {sort_expr}) AS __fugue_take_param FROM {tbn}) WHERE __fugue_take_param<={n}'
        tb = self.query_to_table(sql, {tbn: idf})
        return self.to_df(tb[df.columns], schema=df.schema)
    ...
    def save_table(self, df: DataFrame, table: str, mode: str='overwrite', partition_spec: Optional[PartitionSpec]=None, **kwargs: Any) -> None:
        ...
        if isinstance(df, IbisDataFrame):
            ...
            self.backend.create_table(table, df.as_pandas(), schema=to_ibis_schema(df.schema))
