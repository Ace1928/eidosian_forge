    ...
    **kwargs : extra arguments for `pygsp.graphs.Graph`
    ...
    if graphtype == 'knn':
        ...
        if precomputed is not None:
            raise ValueError("kNNGraph does not support precomputed values. Use `graphtype='exact'` or `precomputed=None`")
        if sample_idx is not None:
            raise ValueError("kNNGraph does not support batch correction. Use `graphtype='mnn'` or `sample_idx=None`")
    elif graphtype == 'mnn':
        basegraph = graphs.MNNGraph
        if precomputed is not None:
            raise ValueError("MNNGraph does not support precomputed values. Use `graphtype='exact'` and `sample_idx=None` or `precomputed=None`")
    elif graphtype == 'exact':
        basegraph = graphs.TraditionalGraph
        if sample_idx is not None:
            raise ValueError("TraditionalGraph does not support batch correction. Use `graphtype='mnn'` or `sample_idx=None`")
    else:
        raise ValueError("graphtype '{}' not recognized. Choose from ['knn', 'mnn', 'exact', 'auto']".format(graphtype))
    ...
    for parent_class in parent_classes:
        for param in parent_class._get_param_names():
            try:
                ...
                pass
    _logger.log_debug('Initializing {} with arguments {}'.format(parent_classes, ', '.join(["{}='{}'".format(key, value) for key, value in params.items() if key != 'data'])))
    return Graph(**params)