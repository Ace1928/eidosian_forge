...
class DDPStrategy(ParallelStrategy):
    ...
    def __init__(self, accelerator: Optional[Accelerator]=None, parallel_devices: Optional[List[torch.device]]=None, cluster_environment: Optional[ClusterEnvironment]=None, checkpoint_io: Optional[CheckpointIO]=None, precision: Optional[Precision]=None, process_group_backend: Optional[str]=None, timeout: Optional[timedelta]=default_pg_timeout, start_method: Literal['popen', 'spawn', 'fork', 'forkserver']='popen', **kwargs: Any) -> None:
        super().__init__(accelerator=accelerator, parallel_devices=parallel_devices, cluster_environment=cluster_environment, checkpoint_io=checkpoint_io, precision=precision)
        self._num_nodes = 1
    ...
    def register_strategies(cls, strategy_registry: _StrategyRegistry) -> None:
        entries = (('ddp', 'popen'), ('ddp_spawn', 'spawn'), ('ddp_fork', 'fork'), ('ddp_notebook', 'fork'))
        for name, start_method in entries:
            strategy_registry.register(name, cls, description=f'DDP strategy with `start_method={start_method!r}`', start_method=start_method)
