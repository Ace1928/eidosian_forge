...
class UntrustedAstTransformer(ast.NodeTransformer):
    ...
    def visit_Attribute(self, node):
        ...
        if isinstance(node.ctx, ast.Load):
            node = self.visit_children(node)
            new_node = ast.Call(func=ast.Name('__rl_getattr__', ast.Load()), args=[node.value, ast.Str(node.attr)], keywords=[])
            copy_locations(new_node, node)
            return new_node
        elif isinstance(node.ctx, (ast.Store, ast.Del)):
            node = self.visit_children(node)
            new_value = ast.Call(func=ast.Name('__rl_sd__', ast.Load()), args=[node.value], keywords=[])
    ...
    def visit_Subscript(self, node):
        ...
        node = self.visit_children(node)
        if isinstance(node.ctx, ast.Load):
            new_node = ast.Call(func=ast.Name('__rl_getitem__', ast.Load()), args=[node.value, self.transform_slice(node.slice)], keywords=[])
            copy_locations(new_node, node)
            return new_node
    ...
    def visit_Assign(self, node):
        ...
        for target in reversed(node.targets):
            if isinstance(target, ast.Tuple):
                wrapper = ast.Assign(targets=[target], value=self.protect_unpack_sequence(target, node.value))
    ...
    def visit_AugAssign(self, node):
        ...
        if isinstance(node.target, ast.Attribute):
            ...
            new_node = ast.Assign(targets=[node.target], value=ast.Call(func=ast.Name('__rl_augAssign__', ast.Load()), args=[ast.Str(augOps[type(node.op)]), ast.Name(node.target.id, ast.Load()), node.value], keywords=[]))
            copy_locations(new_node, node)
            return new_node
        else:
    ...
    def visit_ClassDef(self, node):
        ...
        CLASS_DEF = textwrap.dedent('\t\t\tclass %s(metaclass=__metaclass__):\n\t\t\t\tpass\n\t\t' % node.name)
    ...
    def visit_Import(self, node):
        ...
        node = self.visit_children(node)
        new_node = ast.Call(func=ast.Name('__rl_add__', ast.Load()), args=[node.left, node.right], keywords=[])
        copy_locations(new_node, node)
        return new_node
    ...
    def visit_BinOp(self, node):
        ...
        if isinstance(op, (ast.Mult, ast.Add, ast.Pow)):
            opf = '__rl_mult__' if isinstance(op, ast.Mult) else '__rl_add__' if isinstance(op, ast.Add) else '__rl_pow__'
            new_node = ast.Call(func=ast.Name(opf, ast.Load()), args=[node.left, node.right], keywords=[])
            copy_locations(new_node, node)
            return new_node
        return node
