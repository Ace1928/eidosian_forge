...
class CountsMP(SampleMeasurement):
    ...
    def __init__(self, obs: Optional[Operator]=None, wires=None, eigvals=None, id: Optional[str]=None, all_outcomes: bool=False):
    ...
    def hash(self):
        ...
        fingerprint = (self.__class__.__name__, getattr(self.obs, 'hash', 'None'), str(self._eigvals), tuple(self.wires.tolist()), self.all_outcomes)
    ...
    def process_samples(self, samples: Sequence[complex], wire_order: Wires, shot_range: Tuple[int]=None, bin_size: int=None):
        with qml.queuing.QueuingManager.stop_recording():
            samples = qml.sample(op=self.obs or self.mv, wires=self._wires).process_samples(samples, wire_order, shot_range, bin_size)
        if bin_size is None:
            return self._samples_to_counts(samples)
        num_wires = len(self.wires) if self.wires else len(wire_order)
        samples = samples.reshape((num_wires, -1)).T.reshape(-1, bin_size, num_wires) if self.obs is None and (not isinstance(self.mv, MeasurementValue)) else samples.reshape((-1, bin_size))
