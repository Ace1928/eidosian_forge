...
class DuckDBIO:
    ...
    def load_df(self, uri: Union[str, List[str]], format_hint: Optional[str]=None, columns: Any=None, **kwargs: Any) -> LocalBoundedDataFrame:
    ...
    def save_df(self, df: DuckDataFrame, uri: str, format_hint: Optional[str]=None, mode: str='overwrite', **kwargs: Any) -> None:
        for k in kwargs.keys():
            assert_or_throw(k.isidentifier(), ValueError(f'{k} is invalid'))
        assert_or_throw(mode in ['overwrite', 'error'], NotImplementedError(f'{mode} is not supported'))
    ...
    def _load_csv(self, p: FileParser, columns: Any=None, **kwargs: Any) -> DuckDataFrame:
        kw = ParamDict({k.lower(): v for k, v in kwargs.items()})
        infer_schema = kw.pop('infer_schema', False)
        header = kw.pop('header', False)
        assert_or_throw(not (columns is None and (not header)), ValueError('when csv has no header, columns must be specified'))
        ...
        if infer_schema:
            ...
            if header:
                if columns is None:
                    ...
                    raise ValueError("columns can't be schema when infer_schema is true")
            ...
            if isinstance(columns, list):
                ...
                res = res[Schema(columns).names].alter_columns(columns)
            return res
        else:
            if isinstance(columns, list):
    ...
    def _load_parquet(self, p: FileParser, columns: Any=None, **kwargs: Any) -> DuckDataFrame:
        kw = ParamDict({k.lower(): v for k, v in kwargs.items()})
        params: List[str] = [encode_value_to_expr(p.path)]
        if isinstance(columns, list):
            cols = ', '.join(encode_column_names(columns))
            ...
            cols = '*'
        assert_or_throw(len(kw) == 0, NotImplementedError("can't take extra parameters for loading parquet"))
        pm = ', '.join(params)
        query = f'SELECT {cols} FROM parquet_scan([{pm}])'
        res = DuckDataFrame(self._con.from_query(query))
        return res if isinstance(columns, list) or columns is None else res.alter_columns(columns)