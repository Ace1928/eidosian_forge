...
class TorchRankerAgent(TorchAgent):
    ...
    def add_cmdline_args(cls, argparser):
        ...
        agent.add_argument('-cands', '--candidates', type=str, default='inline', choices=['batch', 'inline', 'fixed', 'batch-all-cands'], help='The source of candidates during training (see TorchRankerAgent._build_candidates() for details).')
        agent.add_argument('-ecands', '--eval-candidates', type=str, default='inline', choices=['batch', 'inline', 'fixed', 'vocab', 'batch-all-cands'], help='The source of candidates during evaluation (defaults to the samevalue as --candidates if no flag is given)')
        agent.add_argument('-icands', '--interactive-candidates', type=str, default='fixed', choices=['fixed', 'inline', 'vocab'], help='The source of candidates during interactive mode. Since in interactive mode, batchsize == 1, we cannot use batch candidates.')
        agent.add_argument('--repeat-blocking-heuristic', type='bool', default=True, help='Block repeating previous utterances. Helpful for many models that score repeats highly, so switched on by default.')
        agent.add_argument('-fcp', '--fixed-candidates-path', type=str, help='A text file of fixed candidates to use for all examples, one candidate per line')
        agent.add_argument('--fixed-candidate-vecs', type=str, default='reuse', help='One of "reuse", "replace", or a path to a file with vectors corresponding to the candidates at --fixed-candidates-path. The default path is a /path/to/model-file.<cands_name>, where <cands_name> is the name of the file (not the full path) passed by the flag --fixed-candidates-path. By default, this file is created once and reused. To replace it, use the "replace" option.')
        agent.add_argument('--encode-candidate-vecs', type='bool', default=True, help='Cache and save the encoding of the candidate vecs. This might be used when interacting with the model in real time or evaluating on fixed candidate set when the encoding of the candidates is independent of the input.')
        agent.add_argument('--encode-candidate-vecs-batchsize', type=int, default=256, hidden=True, help='Batchsize when encoding candidate vecs')
        agent.add_argument('--init-model', type=str, default=None, help='Initialize model with weights from this file.')
        agent.add_argument('--train-predict', type='bool', default=False, help='Get predictions and calculate mean rank during the train step. Turning this on may slow down training.')
        agent.add_argument('--cap-num-predictions', type=int, default=100, help='Limit to the number of predictions in output.text_candidates')
        agent.add_argument('--ignore-bad-candidates', type='bool', default=False, help='Ignore examples for which the label is not present in the label candidates. Default behavior results in RuntimeError. ')
        agent.add_argument('--rank-top-k', type=int, default=-1, help='Ranking returns the top k results of k > 0, otherwise sorts every single candidate according to the ranking.')
        agent.add_argument('--inference', choices={'max', 'topk'}, default='max', help='Final response output algorithm')
        agent.add_argument('--topk', type=int, default=5, help='K used in Top K sampling inference, when selected')
        agent.add_argument('--return-cand-scores', type='bool', default=False, help='Return sorted candidate scores from eval_step')
    ...
    def __init__(self, opt: Opt, shared=None):
        ...
        if shared:
            ...
            self.init_optim(optim_params, states.get('optimizer'), states.get('optimizer_type'))
    ...
    def is_valid(self, obs):
        ...
        if 'labels_vec' in obs and 'label_candidates_vecs' in obs:
            ...
            if len(matches) == 0:
                warn_once('At least one example has a set of label candidates that does not contain the label.')
                return False
        return True
    ...
    def train_step(self, batch):
        ...
        self.model.train()
        self.zero_grad()
        cands, cand_vecs, label_inds = self._build_candidates(batch, source=self.candidates, mode='train')
        try:
        ...
        if not self.opt.get('train_predict', False):
            warn_once('Some training metrics are omitted for speed. Set the flag `--train-predict` to calculate train metrics.')
            return Output()
    ...
    def eval_step(self, batch):
        ...
        if batch.text_vec is None and batch.image is None:
            return
        batchsize = batch.text_vec.size(0) if batch.text_vec is not None else batch.image.size(0)
        self.model.eval()
        cands, cand_vecs, label_inds = self._build_candidates(batch, source=self.eval_candidates, mode='eval')
        cand_encs = None
        if self.encode_candidate_vecs and self.eval_candidates in ['fixed', 'vocab']:
            if self.fixed_candidate_encs is None:
                self.fixed_candidate_encs = self._make_candidate_encs(cand_vecs).detach()
            if self.eval_candidates == 'fixed':
        ...
        if self.rank_top_k > 0:
            sorted_scores, ranks = scores.topk(min(self.rank_top_k, scores.size(1)), 1, largest=True)
        else:
        ...
        for i, ordering in enumerate(ranks):
            if cand_vecs.dim() == 2:
                ...
                cand_list = cands[i]
            cand_preds_generator = (cand_list[rank] for rank in ordering if rank < len(cand_list))
            cand_preds.append(list(islice(cand_preds_generator, max_preds)))
        if self.opt.get('repeat_blocking_heuristic', True) and self.eval_candidates == 'fixed':
    ...
    def _build_candidates(self, batch, source, mode):
        ...
        if source == 'batch':
            warn_once('[ Executing {} mode with batch labels as set of candidates. ]'.format(mode))
            if batchsize == 1:
                warn_once("[ Warning: using candidate source 'batch' and observed a batch of size 1. This may be due to uneven batch sizes at the end of an epoch. ]")
            if label_vecs is None:
                raise ValueError("If using candidate source 'batch', then batch.label_vec cannot be None.")
            cands = batch.labels
            cand_vecs = label_vecs
            label_inds = label_vecs.new_tensor(range(batchsize))
        elif source == 'batch-all-cands':
            warn_once('[ Executing {} mode with all candidates provided in the batch ]'.format(mode))
            if batch.candidate_vecs is None:
                raise ValueError("If using candidate source 'batch-all-cands', then batch.candidate_vecs cannot be None. If your task does not have inline candidates, consider using one of --{m}={{'batch','fixed','vocab'}}.".format(m='candidates' if mode == 'train' else 'eval-candidates'))
            cands = []
            all_cands_vecs = []
            cands_to_id = {}
            ...
            label_inds = label_vecs.new_tensor([cands_to_id[label] for label in batch.labels])
        elif source == 'inline':
            warn_once('[ Executing {} mode with provided inline set of candidates ]'.format(mode))
            if batch.candidate_vecs is None:
                raise ValueError("If using candidate source 'inline', then batch.candidate_vecs cannot be None. If your task does not have inline candidates, consider using one of --{m}={{'batch','fixed','vocab'}}.".format(m='candidates' if mode == 'train' else 'eval-candidates'))
            cands = batch.candidates
            cand_vecs = padded_3d(batch.candidate_vecs, self.NULL_IDX, use_cuda=self.use_cuda, fp16friendly=self.fp16)
            if label_vecs is not None:
                label_inds = label_vecs.new_empty(batchsize)
                bad_batch = False
                for i, label_vec in enumerate(label_vecs):
                    label_vec_pad = label_vec.new_zeros(cand_vecs[i].size(1)).fill_(self.NULL_IDX)
                ...
                if bad_batch:
                    if self.ignore_bad_candidates and (not self.is_training):
                        ...
                        raise RuntimeError('At least one of your examples has a set of label candidates that does not contain the label. To ignore this error set `--ignore-bad-candidates True`.')
        elif source == 'fixed':
            if self.fixed_candidates is None:
                raise ValueError("If using candidate source 'fixed', then you must provide the path to a file of candidates with the flag --fixed-candidates-path or the name of a task with --fixed-candidates-task.")
            warn_once('[ Executing {} mode with a common set of fixed candidates (n = {}). ]'.format(mode, len(self.fixed_candidates)))
            ...
            warn_once('[ Executing {} mode with tokens from vocabulary as candidates. ]'.format(mode))
    ...
    def set_vocab_candidates(self, shared):
        ...
        if shared:
            ...
            logging.info('Loaded fixed candidate set (n = {}) from vocabulary'.format(len(self.vocab_candidates)))
            if self.use_cuda:
                self.vocab_candidate_vecs = self.vocab_candidate_vecs.cuda()
            if self.encode_candidate_vecs:
                self.vocab_candidate_encs = self._make_candidate_encs(self.vocab_candidate_vecs)
                if self.use_cuda:
    ...
    def encode_candidates(self, padded_cands):
        ...
        """
        raise NotImplementedError('Abstract method: user must implement encode_candidates(). If your agent encodes candidates independently from context, you can get performance gains with fixed cands by implementing this function and running with the flag --encode-candidate-vecs True.')
    ...
    def _make_candidate_encs(self, vecs):
        ...
        logging.info('Encoding fixed candidates set from ({} batch(es) of up to {}) ]'.format(len(vec_batches), bsz))
        self.model.eval()
        with torch.no_grad():
    ...
    def vectorize_fixed_candidates(self, cands_batch, add_start=False, add_end=False):
        ...
        """
        return [self._vectorize_text(cand, truncate=self.label_truncate, truncate_left=False, add_start=add_start, add_end=add_end) for cand in cands_batch]