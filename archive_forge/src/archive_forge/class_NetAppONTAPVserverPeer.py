...
class NetAppONTAPVserverPeer:
    ...
    def __init__(self):
        ...
        self.module = AnsibleModule(argument_spec=self.argument_spec, mutually_exclusive=[['peer_options', 'dest_hostname'], ['peer_options', 'dest_username'], ['peer_options', 'dest_password']], supports_check_mode=True)
        ...
        if self.parameters.get('dest_hostname') is not None:
            self.parameters['peer_options'] = dict(hostname=self.parameters.get('dest_hostname'), username=self.parameters.get('dest_username'), password=self.parameters.get('dest_password'))
        else:
    ...
    def is_remote_peer(self):
        return self.parameters.get('dest_hostname') is not None and self.parameters['dest_hostname'] != self.parameters['hostname']
