...
def quick_view(structure, bonds=True, conventional=False, transform=None, show_box=True, bond_tol=0.2, stick_radius=0.1):
    ...
    for site in struct:
        ...
        mv.add_representation('spheres', {'coordinates': coord.astype(float), 'colors': [get_atom_color(el)], 'radii': [rad * 0.5], 'opacity': 1})
    if show_box:
        o = np.array([0, 0, 0])
        ...
        colors = [16777215 for i in range(12)]
        mv.add_representation('lines', {'startCoords': np.array(starts), 'endCoords': np.array(ends), 'startColors': colors, 'endColors': colors})
    return mv