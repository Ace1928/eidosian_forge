...
class SpectralNormalization(Wrapper):
    ...
    def __init__(self, layer, power_iterations=1, **kwargs):
        ...
        if power_iterations <= 0:
            raise ValueError(f'`power_iterations` should be greater than zero. Received: `power_iterations={power_iterations}`')
    ...
    def build(self, input_shape):
        ...
        if hasattr(self.layer, 'kernel'):
            ...
            raise ValueError(f"{type(self.layer).__name__} object has no attribute 'kernel' nor 'embeddings'")
        self.kernel_shape = self.kernel.shape.as_list()
        self.vector_u = self.add_weight(shape=(1, self.kernel_shape[-1]), initializer=TruncatedNormal(stddev=0.02), trainable=False, name='vector_u', dtype=self.kernel.dtype)
