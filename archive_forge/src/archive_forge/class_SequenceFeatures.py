    ...
    def __init__(self, feature_columns, trainable=True, name=None, **kwargs):
        ...
        Raises:
          ValueError: If any of the `feature_columns` is not a
            `SequenceDenseColumn`.
        """
        super().__init__(feature_columns=feature_columns, trainable=trainable, name=name, expected_column_type=tf.__internal__.feature_column.SequenceDenseColumn, **kwargs)
    ...
    def call(self, features, training=None):
        ...
        for column in self._feature_columns:
            with backend.name_scope(column.name):
                try:
                    dense_tensor, sequence_length = column.get_sequence_dense_tensor(transformation_cache, self._state_manager, training=training)
                except TypeError:
                    dense_tensor, sequence_length = column.get_sequence_dense_tensor(transformation_cache, self._state_manager)
                output_tensors.append(self._process_dense_tensor(column, dense_tensor))
                sequence_lengths.append(sequence_length)
        kfc._verify_static_batch_size_equality(sequence_lengths, self._feature_columns)
