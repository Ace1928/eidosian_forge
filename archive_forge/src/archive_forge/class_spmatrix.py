...
class spmatrix(object):
    ...
    def mean(self, axis=None, dtype=None, out=None):
        ...
        def _is_integral(dtype):
            return cupy.issubdtype(dtype, cupy.integer) or cupy.issubdtype(dtype, cupy.bool_)
        ...
        if axis is None:
            return (inter_self / cupy.array(self.shape[0] * self.shape[1])).sum(dtype=res_dtype, out=out)
        if axis < 0:
            axis += 2
        if axis == 0:
