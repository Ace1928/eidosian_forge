...
class ArcGISLoader(BaseLoader):
    ...
    def __init__(self, layer: Union[str, arcgis.features.FeatureLayer], gis: Optional[arcgis.gis.GIS]=None, where: str='1=1', out_fields: Optional[Union[List[str], str]]=None, return_geometry: bool=False, result_record_count: Optional[int]=None, lyr_desc: Optional[str]=None, **kwargs: Any):
        try:
            import arcgis
        ...
        query_params = dict(where=self.where, out_fields=self.out_fields, return_geometry=self.return_geometry, return_all_records=self.return_all_records, result_record_count=self.result_record_count)
    ...
    def lazy_load(self) -> Iterator[Document]:
        ...
        for feature in features:
            ...
            metadata = {'accessed': f'{datetime.now(timezone.utc).isoformat()}Z', 'name': self.layer_properties['layer_properties']['name'], 'url': self.url, 'layer_description': self.layer_properties['layer_description'], 'item_description': self.layer_properties['item_description'], 'layer_properties': self.layer_properties['layer_properties']}
