...
class Learner:
    ...
    def __init__(self, *, module_spec: Optional[Union[SingleAgentRLModuleSpec, MultiAgentRLModuleSpec]]=None, module: Optional[RLModule]=None, learner_group_scaling_config: Optional[LearnerGroupScalingConfig]=None, learner_hyperparameters: Optional[LearnerHyperparameters]=None, framework_hyperparameters: Optional[FrameworkHyperparameters]=None):
        if learner_hyperparameters and learner_hyperparameters.seed is not None:
            update_global_seed_if_necessary(self.framework, learner_hyperparameters.seed)
        if (module_spec is None) is (module is None):
            raise ValueError('Exactly one of `module_spec` or `module` must be provided to Learner!')
        ...
        self._framework_hyperparameters = framework_hyperparameters or FrameworkHyperparameters()
    ...
    @OverrideToImplementCustomLogic_CallToSuperRecommended
    def compile_results(self, *, batch: MultiAgentBatch, fwd_out: Mapping[str, Any], loss_per_module: Mapping[str, TensorType], metrics_per_module: DefaultDict[ModuleID, Dict[str, Any]]) -> Mapping[str, Any]:
        ...
        if not isinstance(batch, MultiAgentBatch):
            raise ValueError(f'batch must be a MultiAgentBatch, but got {type(batch)} instead.')
        ...
        for module_id in list(batch.policy_batches.keys()) + [ALL_MODULES]:
            module_learner_stats[module_id].update({self.TOTAL_LOSS_KEY: loss_per_module_numpy[module_id], **convert_to_numpy(metrics_per_module[module_id])})
            module_learner_stats[module_id].update({f'{optim_name}_lr': convert_to_numpy(self._get_optimizer_lr(optimizer)) for optim_name, optimizer in self.get_optimizers_for_module(module_id=module_id)})
    ...
    def add_module(self, *, module_id: ModuleID, module_spec: SingleAgentRLModuleSpec) -> None:
        ...
        self.configure_optimizers_for_module(module_id=module_id, hps=self.hps.get_hps_for_module(module_id))
    ...
    @OverrideToImplementCustomLogic
    def compute_loss(self, *, fwd_out: Union[MultiAgentBatch, NestedDict], batch: Union[MultiAgentBatch, NestedDict]) -> Union[TensorType, Mapping[str, Any]]:
        ...
        for module_id in fwd_out:
            ...
            loss = self.compute_loss_for_module(module_id=module_id, hps=self.hps.get_hps_for_module(module_id), batch=module_batch, fwd_out=module_fwd_out)
    ...
    def update(self, batch: MultiAgentBatch, *, minibatch_size: Optional[int]=None, num_iters: int=1, reduce_fn: Callable[[List[Mapping[str, Any]]], ResultDict]=_reduce_mean_results) -> Union[Mapping[str, Any], List[Mapping[str, Any]]]:
        ...
        if len(missing_module_ids) > 0:
            raise ValueError(f'Batch contains module ids that are not in the learner: {missing_module_ids}')
        if num_iters < 1:
        ...
        for tensor_minibatch in batch_iter(batch, minibatch_size, num_iters):
            ...
            result = self.compile_results(batch=tensor_minibatch, fwd_out=fwd_out, loss_per_module=loss_per_module, metrics_per_module=defaultdict(dict, **metrics_per_module))
    ...
    @OverrideToImplementCustomLogic
    @abc.abstractmethod
    def _update(self, batch: NestedDict, **kwargs) -> Tuple[Any, Any, Any]:
    ...
    def set_state(self, state: Mapping[str, Any]) -> None:
        ...
        if 'module_state' not in state:
            raise ValueError("state must have a key 'module_state' for the module weights")
        if 'optimizer_state' not in state:
            raise ValueError("state must have a key 'optimizer_state' for the optimizer weights")
    ...
    def get_state(self) -> Mapping[str, Any]:
        ...
        """
        self._check_is_built()
        return {'module_state': self.get_module_state(), 'optimizer_state': self.get_optimizer_state()}
    ...
    def _check_registered_optimizer(self, optimizer: Optimizer, params: Sequence[Param]) -> None:
        ...
        if not isinstance(params, list):
            raise ValueError(f'`params` ({params}) must be a list of framework-specific parameters (variables)!')
    ...
    def _check_is_built(self):
        if self.module is None:
            raise ValueError('Learner.build() must be called after constructing a Learner and before calling any methods on it.')
