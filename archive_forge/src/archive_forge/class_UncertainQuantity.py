...
class UncertainQuantity(Quantity):
    ...
    def __array_finalize__(self, obj):
        ...
        self._uncertainty = getattr(obj, 'uncertainty', Quantity(np.zeros(self.shape, self.dtype), self._dimensionality, copy=False))
    ...
    def __rtruediv__(self, other):
        temp = UncertainQuantity(1 / self.magnitude, self.dimensionality ** (-1), self.relative_uncertainty / self.magnitude, copy=False)
        return other * temp
    ...
    def __getitem__(self, key):
        return UncertainQuantity(self.magnitude[key], self._dimensionality, self.uncertainty[key], copy=False)

    @with_doc(Quantity.__repr__, use_header=False)
    def __repr__(self):
        return '%s(%s, %s, %s)' % (self.__class__.__name__, repr(self.magnitude), self.dimensionality.string, repr(self.uncertainty.magnitude))
    ...
    def __str__(self):
        ...
        s = '%s %s\n+/-%s (1 sigma)' % (str(self.magnitude), dims, str(self.uncertainty))
        if markup.config.use_unicode:
    ...
    def sum(self, axis=None, dtype=None, out=None):
        return UncertainQuantity(self.magnitude.sum(axis, dtype, out), self.dimensionality, np.sum(self.uncertainty.magnitude ** 2, axis) ** 0.5, copy=False)

    @with_doc(np.nansum)
    def nansum(self, axis=None, dtype=None, out=None):
        return UncertainQuantity(np.nansum(self.magnitude, axis, dtype, out), self.dimensionality, np.nansum(self.uncertainty.magnitude ** 2, axis) ** 0.5, copy=False)

    @with_doc(np.ndarray.mean)
    def mean(self, axis=None, dtype=None, out=None):
        return UncertainQuantity(self.magnitude.mean(axis, dtype, out), self.dimensionality, ((1.0 / np.size(self, axis)) ** 2 * np.sum(self.uncertainty.magnitude ** 2, axis)) ** 0.5, copy=False)

    @with_doc(np.nanmean)
    def nanmean(self, axis=None, dtype=None, out=None):
        size = np.sum(~np.isnan(self), axis)
        return UncertainQuantity(np.nanmean(self.magnitude, axis, dtype, out), self.dimensionality, ((1.0 / size) ** 2 * np.nansum(np.nan_to_num(self.uncertainty.magnitude) ** 2, axis)) ** 0.5, copy=False)
