...
class Pathfinder:
    ...
    def evaluate_escape_routes(
        ...
        for dx, dy in directions:
            ...
            if neighbor not in obstacles and self.is_within_boundaries(neighbor, boundaries):
                score += 1.0
        return -score
    ...
    def heuristic(
        ...
        if secondary_goal_position:
            heuristic_value += 0.5 * self.calculate_distance(self_position, secondary_goal_position)
        if tertiary_goal_position:
            heuristic_value += 0.3 * self.calculate_distance(self_position, tertiary_goal_position)
        if quaternary_goal_position:
            heuristic_value += 0.1 * self.calculate_distance(self_position, quaternary_goal_position)

        # Adjust heuristic based on the proximity to obstacles and boundaries
        heuristic_value += self.calculate_obstacle_proximity(self_position, obstacles, space_around_obstacles)
        heuristic_value += self.calculate_boundary_proximity(self_position, environment_boundaries, space_around_boundaries)

        # Consider agent's body positions if body size adaptations are enabled
        if body_size_adaptations:
            heuristic_value += self.calculate_body_position_proximity(self_position, self_body_positions, space_around_agent)

        # Factor in escape routes availability
        if escape_route_availability:
            heuristic_value += self.evaluate_escape_routes(self_position, obstacles, environment_boundaries)
    ...
    def reconstruct_path(self, came_from: Dict[Tuple[int, int], Tuple[int, int]], start: Tuple[int, int], goal: Tuple[int, int]) -> List[Tuple[int, int]]:
        """
        Reconstruct the path from start to goal using the came_from map.
...
def run_simulation():
    ...
    pathfinder = Pathfinder(screen_width // 10, screen_height // 10, logging.getLogger())
    num_scenarios = 50

    for _ in range(num_scenarios):
        start = (random.randint(0, pathfinder.width - 1), random.randint(0, pathfinder.height - 1))
        goal = (random.randint(0, pathfinder.width - 1), random.randint(0, pathfinder.height - 1))
        pathfinder.obstacles = {(random.randint(0, pathfinder.width - 1), random.randint(0, pathfinder.height - 1)) for _ in range(100)}

        agent_body_size = random.randint(1, 5)
        agent_body_positions = set()
        current_pos = start
        for _ in range(agent_body_size):
            agent_body_positions.add(current_pos)
            current_pos = random.choice(pathfinder.neighbors(current_pos))
        ...
        for y in range(pathfinder.height):
            for x in range(pathfinder.width):   
                if (x, y) in pathfinder.obstacles:
        ...
        pygame.draw.rect(screen, (255, 0, 0), (start[0] * 10, start[1] * 10, 10, 10))
