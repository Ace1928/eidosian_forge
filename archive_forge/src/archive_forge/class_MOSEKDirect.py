...
class MOSEKDirect(DirectSolver):
    ...
    def _add_constraints(self, con_seq):
        ...
        lq_ex = tuple(filter(lambda x: not isinstance(x, _ConicBase) and (not x._linear_canonical_form), con_seq))
        lq_all = lq + lq_ex
        ...
        if num_lq > 0:
            ...
            lbs = tuple((-inf if value(lq_all[i].lower) is None else value(lq_all[i].lower) - constants[i] for i in range(num_lq)))
            ubs = tuple((inf if value(lq_all[i].upper) is None else value(lq_all[i].upper) - constants[i] for i in range(num_lq)))
            fxs = tuple((c.equality for c in lq_all))
            bound_types = tuple(map(self._mosek_bounds, lbs, ubs, fxs))
            sub = range(con_num, con_num + num_lq)
            sub_names = tuple((self._symbol_map.getSymbol(c, self._labeler) for c in lq_all))
    ...
    def _set_objective(self, obj):
        ...
        mosek_arow, mosek_qexp, referenced_vars = self._get_expr_from_pyomo_expr(obj.expr, self._max_obj_degree)
    ...
    def _add_block(self, block):
        ...
        """
        var_seq = tuple(block.component_data_objects(ctype=pyomo.core.base.var.Var, descend_into=True, active=True, sort=True))
        self._add_vars(var_seq)
        for sub_block in block.block_data_objects(descend_into=True, active=True):
            con_list = []
            for con in sub_block.component_data_objects(ctype=pyomo.core.base.constraint.Constraint, descend_into=False, active=True, sort=True):
                if not con.has_lb() and (not con.has_ub()):
                    assert not con.equality
                    continue
                con_list.append(con)
            self._add_constraints(con_list)
            for con in sub_block.component_data_objects(ctype=pyomo.core.base.sos.SOSConstraint, descend_into=False, active=True, sort=True):
                self._add_sos_constraint(con)
            obj_counter = 0
            for obj in sub_block.component_data_objects(ctype=pyomo.core.base.objective.Objective, descend_into=False, active=True):
                obj_counter += 1
                if obj_counter > 1:
    ...
    def _postsolve(self):
        ...
        for suffix in self._suffixes:
            ...
            if re.match(suffix, 'rc'):
                ...
                flag = True
            if not flag:
                raise RuntimeError('***MOSEK solver plugin cannot extract solution suffix = ' + suffix)
        ...
        self.results.solver.wallclock_time = msk_task.getdouinf(mosek.dinfitem.optimizer_time)
        SOLSTA_MAP = {mosek.solsta.unknown: 'unknown', mosek.solsta.optimal: 'optimal', mosek.solsta.prim_and_dual_feas: 'pd_feas', mosek.solsta.prim_feas: 'p_feas', mosek.solsta.dual_feas: 'd_feas', mosek.solsta.prim_infeas_cer: 'p_infeas', mosek.solsta.dual_infeas_cer: 'd_infeas', mosek.solsta.prim_illposed_cer: 'p_illposed', mosek.solsta.dual_illposed_cer: 'd_illposed', mosek.solsta.integer_optimal: 'optimal'}
        PROSTA_MAP = {mosek.prosta.unknown: 'unknown', mosek.prosta.prim_and_dual_feas: 'pd_feas', mosek.prosta.prim_feas: 'p_feas', mosek.prosta.dual_feas: 'd_feas', mosek.prosta.prim_infeas: 'p_infeas', mosek.prosta.dual_infeas: 'd_infeas', mosek.prosta.prim_and_dual_infeas: 'pd_infeas', mosek.prosta.ill_posed: 'illposed', mosek.prosta.prim_infeas_or_unbounded: 'p_inf_unb'}
        if self._version[0] < 9:
            SOLSTA_OLD = {mosek.solsta.near_optimal: 'optimal', mosek.solsta.near_integer_optimal: 'optimal', mosek.solsta.near_prim_feas: 'p_feas', mosek.solsta.near_dual_feas: 'd_feas', mosek.solsta.near_prim_and_dual_feas: 'pd_feas', mosek.solsta.near_prim_infeas_cer: 'p_infeas', mosek.solsta.near_dual_infeas_cer: 'd_infeas'}
            PROSTA_OLD = {mosek.prosta.near_prim_and_dual_feas: 'pd_feas', mosek.prosta.near_prim_feas: 'p_feas', mosek.prosta.near_dual_feas: 'd_feas'}
        ...
        if self._termcode == mosek.rescode.ok:
            ...
            self.results.solver.termination_message = 'Optimizer terminated due to the return of the user-defined callback function.'
            self.results.solver.termination_condition = TerminationCondition.userInterrupt
            soln.status = SolutionStatus.unknown
        elif self._termcode in [mosek.rescode.trm_mio_num_relaxs, mosek.rescode.trm_mio_num_branches, mosek.rescode.trm_num_max_num_int_solutions]:
            self.results.solver.status = SolverStatus.ok
            self.results.solver.termination_message = 'The mixed-integer optimizer terminated as the maximum number of relaxations/branches/feasible solutions was reached.'
            self.results.solver.termination_condition = TerminationCondition.maxEvaluations
            soln.status = SolutionStatus.stoppedByLimit
        else:
            self.results.solver.termination_message = ' Optimization terminated with {} response code.Check MOSEK response code documentation for more information.'.format(self._termcode)
        ...
        if PROSTA_MAP[pro_status] == 'd_infeas':
            ...
            self.results.solver.status = SolverStatus.warning
            self.results.solver.termination_message += ' Problem is either primal infeasible or unbounded. This may happen for MIPs.'
            self.results.solver.Message = self.results.solver.termination_message
        ...
        if msk_task.getobjsense() == mosek.objsense.minimize:
            ...
            raise RuntimeError('Unrecognized Mosek objective sense: {0}'.format(msk_task.getobjname()))
        ...
        if msk_task.getnumintvar() == 0:
            try:
                ...
                pass
        elif msk_task.getobjsense() == mosek.objsense.minimize:
            try:
                self.results.problem.upper_bound = msk_task.getprimalobj(whichsol)
            except (mosek.MosekException, AttributeError):
                pass
            try:
                self.results.problem.lower_bound = msk_task.getdouinf(mosek.dinfitem.mio_obj_bound)
            except (mosek.MosekException, AttributeError):
                pass
        elif msk_task.getobjsense() == mosek.objsense.maximize:
            try:
                self.results.problem.upper_bound = msk_task.getdouinf(mosek.dinfitem.mio_obj_bound)
            except (mosek.MosekException, AttributeError):
                pass
            try:
            ...
            raise RuntimeError('Unrecognized Mosek objective sense: {0}'.format(msk_task.getobjsense()))
        try:
        ...
        self.results.problem.number_of_continuous_variables = msk_task.getnumvar() - msk_task.getnumintvar()
        ...
        if self._save_results:
            ...
            if self.results.problem.number_of_solutions > 0:
                ...
                mosek_vars = list(set(mosek_vars).intersection(set(self._pyomo_var_to_solver_var_map.values())))
                    ...
                    if self._version[0] <= 9:
                        ...
                        if ncone > 0:
                            ...
                            for name, cone in zip(cone_names, mosek_cones):
                                ...
                                soln_constraints[name]['Dual'] = tuple((vals[i] for i in members))
                    else:
                        ...
                        if ncone > 0:
                            ...
                            dim = 0
                            for name, cone in zip(cone_names, mosek_cones):
                                soln_constraints[name]['Dual'] = tuple(vals[dim:dim + cone_dims[cone]])
                                dim += cone_dims[cone]
                if extract_slacks:
                    Ax = [0] * len(mosek_cons)
    ...
    def _warm_start(self):
        ...
        for pyomo_var, mosek_var in self._pyomo_var_to_solver_var_map.items():
            if pyomo_var.value is not None:
                self._solver_model.putxxslice(self._whichsol, mosek_var, mosek_var + 1, [pyomo_var.value])
        if (self._version[0] > 9) & (self._whichsol == mosek.soltype.itg):
            self._solver_model.putintparam(mosek.iparam.mio_construct_sol, mosek.onoffkey.on)
