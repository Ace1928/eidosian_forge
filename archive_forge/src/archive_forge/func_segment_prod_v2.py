...
def segment_prod_v2(data: _atypes.TensorFuzzingAnnotation[TV_SegmentProdV2_T], segment_ids: _atypes.TensorFuzzingAnnotation[TV_SegmentProdV2_Tindices], num_segments: _atypes.TensorFuzzingAnnotation[TV_SegmentProdV2_Tnumsegments], name=None) -> _atypes.TensorFuzzingAnnotation[TV_SegmentProdV2_T]:
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'SegmentProdV2', name, data, segment_ids, num_segments)
            return _result
            ...
            pass
        try:
            return segment_prod_v2_eager_fallback(data, segment_ids, num_segments, name=name, ctx=_ctx)
        except _core._SymbolicException:
            pass
    _, _, _op, _outputs = _op_def_library._apply_op_helper('SegmentProdV2', data=data, segment_ids=segment_ids, num_segments=num_segments, name=name)
    _result = _outputs[:]
    if _execute.must_record_gradient():
        ...
        _execute.record_gradient('SegmentProdV2', _inputs_flat, _attrs, _result)
    _result, = _result
    return _result