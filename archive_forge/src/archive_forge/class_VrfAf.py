...
class VrfAf(object):
    ...
    def get_vrf(self):
        ...
        re_find_1 = re.findall('.*<vrfname>(.*)</vrfname>.*', xml_str.lower())
    ...
    def check_params(self):
        ...
        if not self.get_vrf():
            self.module.fail_json(msg='Error: The vrf name do not exist.')
        if self.state == 'present':
            if self.route_distinguisher:
                if not is_valid_value(self.route_distinguisher):
                    self.module.fail_json(msg='Error:The vrf route distinguisher length must between 3 ~ 21,i.e. X.X.X.X:number<0-65535> or number<0-65535>:number<0-4294967295>or number<0-65535>.number<0-65535>:number<0-65535>or number<65536-4294967295>:number<0-65535> but not be 0:0 or 0.0:0.')
            if not self.vpn_target_state:
                if self.vpn_target_value or self.vpn_target_type:
                    self.module.fail_json(msg='Error: The vpn target state should be exist.')
            if self.vpn_target_state:
                if not self.vpn_target_value or not self.vpn_target_type:
                    self.module.fail_json(msg='Error: The vpn target value and type should be exist.')
            if self.vpn_target_value:
                if not is_valid_value(self.vpn_target_value):
                    self.module.fail_json(msg='Error:The vrf target value length must between 3 ~ 21,i.e. X.X.X.X:number<0-65535> or number<0-65535>:number<0-4294967295>or number<0-65535>.number<0-65535>:number<0-65535>or number<65536-4294967295>:number<0-65535> but not be 0:0 or 0.0:0.')
    ...
    def get_existing(self):
        ...
        for vrf_af_ele in self.vrf_af_info['vpnInstAF']:
            ...
            self.existing['route_distinguisher'].append(vrf_af_ele['vrfRD'])
            if vrf_af_ele.get('vpnTargets'):
                for vpn_target in vrf_af_ele.get('vpnTargets'):
                    self.existing['vpn_target_type'].append(vpn_target['vrfRTType'])
                    self.existing['vpn_target_value'].append(vpn_target['vrfRTValue'])
            if vrf_af_ele.get('evpnTargets'):
                for evpn_target in vrf_af_ele.get('evpnTargets'):
                    self.existing['evpn_target_type'].append(evpn_target['vrfRTType'])
                    self.existing['evpn_target_value'].append(evpn_target['vrfRTValue'])
    ...
    def get_end_state(self):
        ...
        for vrf_af_ele in self.vrf_af_info['vpnInstAF']:
            ...
            if vrf_af_ele.get('vpnTargets'):
                for vpn_target in vrf_af_ele.get('vpnTargets'):
                    self.end_state['vpn_target_type'].append(vpn_target['vrfRTType'])
                    self.end_state['vpn_target_value'].append(vpn_target['vrfRTValue'])
            if vrf_af_ele.get('evpnTargets'):
                for evpn_target in vrf_af_ele.get('evpnTargets'):
                    self.end_state['evpn_target_type'].append(evpn_target['vrfRTType'])
                    self.end_state['evpn_target_value'].append(evpn_target['vrfRTValue'])
