...
class AzureRMRegistrationAssignment(AzureRMModuleBaseExt):
    ...
    def __init__(self):
        self.module_arg_spec = dict(scope=dict(type='str', required=True), registration_assignment_id=dict(type='str'), properties=dict(type='dict', disposition='/properties', options=dict(registration_definition_id=dict(type='str', disposition='registration_definition_id', required=True))), state=dict(type='str', default='present', choices=['present', 'absent']))
        self.scope = None
        ...
        super(AzureRMRegistrationAssignment, self).__init__(derived_arg_spec=self.module_arg_spec, supports_check_mode=True, supports_tags=True)
    ...
    def create_update_resource(self):
        try:
            response = self.mgmt_client.registration_assignments.begin_create_or_update(scope=self.scope, registration_assignment_id=self.registration_assignment_id, request_body=self.body)
    ...
    def delete_resource(self):
        try:
            response = self.mgmt_client.registration_assignments.begin_delete(scope=self.scope, registration_assignment_id=self.registration_assignment_id)
