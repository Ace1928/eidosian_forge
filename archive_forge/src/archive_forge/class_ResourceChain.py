...
class ResourceChain(stack_resource.StackResource):
    ...
    PROPERTIES = RESOURCES, CONCURRENT, RESOURCE_PROPERTIES = ('resources', 'concurrent', 'resource_properties')
    ATTRIBUTES = REFS, ATTR_ATTRIBUTES = ('refs', 'attributes')
    properties_schema = {RESOURCES: properties.Schema(properties.Schema.LIST, description=_('The list of resource types to create. This list may contain type names or aliases defined in the resource registry. Specific template names are not supported.'), required=True, update_allowed=True), CONCURRENT: properties.Schema(properties.Schema.BOOLEAN, description=_('If true, the resources in the chain will be created concurrently. If false or omitted, each resource will be treated as having a dependency on the previous resource in the list.'), default=False), RESOURCE_PROPERTIES: properties.Schema(properties.Schema.MAP, description=_('Properties to pass to each resource being created in the chain.'))}
    attributes_schema = {REFS: attributes.Schema(description=_('A list of resource IDs for the resources in the chain.'), type=attributes.Schema.LIST), ATTR_ATTRIBUTES: attributes.Schema(description=_('A map of resource names to the specified attribute of each individual resource.'), type=attributes.Schema.MAP)}
    ...
    def child_template(self):
        ...
        name_def_tuples = []
        concurrency_limit = service_objects.Service.active_service_count(self.context) or 1
        for index, rt in enumerate(resource_types):
            ...
            t = (name, self._build_resource_definition(name, rt, depends_on=depends_on))
            name_def_tuples.append(t)
        nested_template = scl_template.make_template(name_def_tuples)
        att_func = 'get_attr'
        get_attr = functools.partial(nested_template.functions[att_func], None, att_func)
        res_func = 'get_resource'
        get_res = functools.partial(nested_template.functions[res_func], None, res_func)
        ...
        try:
            ...
            if key.startswith('resource.'):
                ...
                if target not in names:
                    raise exception.NotFound(_("Member '%(mem)s' not found in group resource '%(grp)s'.") % {'mem': target, 'grp': self.name})
    ...
    def _build_resource_definition(self, resource_name, resource_type, depends_on=None):
        ...
        return rsrc_defn.ResourceDefinition(resource_name, resource_type, properties, depends=depends_on)