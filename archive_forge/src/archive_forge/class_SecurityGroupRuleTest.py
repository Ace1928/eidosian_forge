...
class SecurityGroupRuleTest(common.HeatTestCase):
    ...
    def test_resource_mapping(self):
        ...
        self.assertEqual(mapping['OS::Neutron::SecurityGroupRule'], security_group_rule.SecurityGroupRule)

    @mock.patch('heat.engine.clients.os.neutron.NeutronClientPlugin.has_extension', return_value=True)
    def _create_stack(self, ext_func, tmpl=inline_templates.SECURITY_GROUP_RULE_TEMPLATE):
        ...
        self.sg_rule.client_plugin().find_resourceid_by_name_or_id = mock.MagicMock(return_value='123')

    def test_create(self):
        self._create_stack()
        self.neutron_client.create_security_group_rule.return_value = {'security_group_rule': {'id': '1234'}}
        expected = {'security_group_rule': {'security_group_id': u'123', 'description': u'test description', 'remote_group_id': u'123', 'protocol': u'tcp', 'port_range_min': '100', 'direction': 'ingress', 'ethertype': 'IPv4'}}
        self.sg_rule.handle_create()
        self.neutron_client.create_security_group_rule.assert_called_with(expected)
    ...
    def test_show_resource(self):
        ...
        self.neutron_client.show_security_group_rule.return_value = {'security_group_rule': {'id': '1234'}}
