...
class LogBeginnerTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        class NotWarnings:
            ...
            def __init__(self) -> None:
                self.warnings: List[Tuple[str, Type[Warning], str, int, Optional[IO[Any]], Optional[int]]] = []

            def showwarning(self, message: str, category: Type[Warning], filename: str, lineno: int, file: Optional[IO[Any]]=None, line: Optional[int]=None) -> None:
                """
        ...
        self.beginner = LogBeginner(self.publisher, self.errorStream, self.sysModule, self.warningsModule)
    ...
    def _bufferLimitTest(self, limit: int, beginner: LogBeginner) -> None:
        ...
        self.assertEqual(list(range(1, limit + 1)), list((event['count'] for event in events)))
