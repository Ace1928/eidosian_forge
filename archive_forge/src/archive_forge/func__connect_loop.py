...
def _connect_loop(self, client_factory):
    ...
    while True:
        ...
        self.pause(1.0)
        if self.state.bgp_state in (const.BGP_FSM_IDLE, const.BGP_FSM_ACTIVE):
            self.state.bgp_state = const.BGP_FSM_CONNECT
            if self._neigh_conf.host_bind_ip and self._neigh_conf.host_bind_port:
                bind_addr = (self._neigh_conf.host_bind_ip, self._neigh_conf.host_bind_port)
            else:
                bind_addr = None
            peer_address = (self._neigh_conf.ip_address, self._neigh_conf.port)
            if bind_addr:
                LOG.debug('%s trying to connect from%s to %s', self, bind_addr, peer_address)
            else:
            ...
            try:
                ...
                self._connect_tcp(peer_address, client_factory, time_out=tcp_conn_timeout, bind_address=bind_addr, password=password)
            except socket.error:
                self.state.bgp_state = const.BGP_FSM_ACTIVE
                if LOG.isEnabledFor(logging.DEBUG):
                    LOG.debug('Socket could not be created in time (%s secs), reason %s', tcp_conn_timeout, traceback.format_exc())
                LOG.info('Will try to reconnect to %s after %s secs: %s', self._neigh_conf.ip_address, self._common_conf.bgp_conn_retry_time, self._connect_retry_event.is_set())
