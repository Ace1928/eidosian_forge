...
class NPUAcceleratorManager(AcceleratorManager):
    ...
    def get_current_process_visible_accelerator_ids() -> Optional[List[str]]:
        ascend_visible_devices = os.environ.get(NPUAcceleratorManager.get_visible_accelerator_ids_env_var(), None)
    ...
    @staticmethod
    def set_current_process_visible_accelerator_ids(visible_npu_devices: List[str]) -> None:
        if os.environ.get(NOSET_ASCEND_VISIBLE_DEVICES_ENV_VAR):
            return
        os.environ[NPUAcceleratorManager.get_visible_accelerator_ids_env_var()] = ','.join([str(i) for i in visible_npu_devices])