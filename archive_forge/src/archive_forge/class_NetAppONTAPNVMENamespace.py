...
class NetAppONTAPNVMENamespace:
    ...
    def get_namespace(self):
        ...
        query = {'query': {'nvme-namespace-info': {'path': self.parameters['path'], 'vserver': self.parameters['vserver']}}}
        namespace_get.translate_struct(query)
        try:
            result = self.server.invoke_successfully(namespace_get, enable_tunneling=False)
        except netapp_utils.zapi.NaApiError as error:
            self.module.fail_json(msg='Error fetching namespace info: %s' % to_native(error), exception=traceback.format_exc())
    ...
    def create_namespace(self):
        ...
        options = {'path': self.parameters['path'], 'ostype': self.parameters['ostype'], 'size': self.parameters['size']}
        ...
        try:
            ...
            self.module.fail_json(msg='Error creating namespace for path %s: %s' % (self.parameters.get('path'), to_native(error)), exception=traceback.format_exc())
    ...
    def delete_namespace(self):
        ...
        try:
            ...
            self.module.fail_json(msg='Error deleting namespace for path %s: %s' % (self.parameters.get('path'), to_native(error)), exception=traceback.format_exc())

    def get_namespace_rest(self):
        api = 'storage/namespaces'
        params = {'svm.name': self.parameters['vserver'], 'name': self.parameters['path'], 'fields': 'enabled'}
    ...
    def create_namespace_rest(self):
        api = 'storage/namespaces'
        body = {'svm.name': self.parameters['vserver'], 'os_type': self.parameters['ostype'], 'name': self.parameters['path'], 'space.size': self.parameters['size']}
        if self.parameters.get('block_size') is not None:
            body['space.block_size'] = self.parameters['block_size']
        dummy, error = rest_generic.post_async(self.rest_api, api, body)
        if error:
            self.module.fail_json(msg='Error creating namespace for vserver %s: %s' % (self.parameters['vserver'], to_native(error)), exception=traceback.format_exc())

    def delete_namespace_rest(self):
        api = 'storage/namespaces'
        dummy, error = rest_generic.delete_async(self.rest_api, api, self.namespace_uuid)
        if error:
            self.module.fail_json(msg='Error deleting namespace for vserver %s: %s' % (self.parameters['vserver'], to_native(error)), exception=traceback.format_exc())
