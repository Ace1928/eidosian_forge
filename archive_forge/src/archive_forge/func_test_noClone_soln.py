...
def test_noClone_soln(self):
    ...
    m_sipopt = sensitivity_calculation('sipopt', m_orig, [m_orig.a, m_orig.H], [m_orig.perturbed_a, m_orig.perturbed_H], cloneModel=False)
    self.assertTrue(m_sipopt == m_orig)
    self.assertTrue(hasattr(m_orig, '_SENSITIVITY_TOOLBOX_DATA') and m_orig._SENSITIVITY_TOOLBOX_DATA.ctype is Block)
