...
def test_prepare_floquet_characterization_for_moments_merges_compatible_sets():
    ...
    assert requests == [cirq_google.calibration.FloquetPhasedFSimCalibrationRequest(pairs=((a, b), (c, d)), gate=SQRT_ISWAP_INV_GATE, options=options), cirq_google.calibration.FloquetPhasedFSimCalibrationRequest(pairs=((a, f), (b, c), (d, e)), gate=SQRT_ISWAP_INV_GATE, options=options)]
