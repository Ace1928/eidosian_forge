...
class MPIBlockMatrix(BaseBlockMatrix):
    ...
    def __init__(self, nbrows, nbcols, rank_ownership, mpi_comm, assert_correct_owners=False):
        shape = (nbrows, nbcols)
    ...
    def get_block(self, row, col):
        ...
        rank = self._mpiw.Get_rank()
        assert owner == rank or owner < 0, 'Block {} not owned by processor {}'.format((row, col), rank)
        return block
    ...
    def _comparison_helper(self, operation, other):
        ...
        if isinstance(other, MPIBlockMatrix):
            ...
            assert np.array_equal(self.rank_ownership, other.rank_ownership), 'MPIBlockMatrices must be distributed in the same processors'
