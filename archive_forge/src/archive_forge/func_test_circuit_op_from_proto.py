...
def test_circuit_op_from_proto(repetitions):
    ...
    serialized = v2.program_pb2.CircuitOperation(circuit_constant_index=1, repetition_specification=repetition_spec, qubit_map=qubit_map, measurement_key_map=measurement_key_map, arg_map=arg_map)
    ...
    actual = deserializer.from_proto(serialized, constants=constants, deserialized_constants=deserialized_constants)
    expected = cirq.CircuitOperation(circuit=default_circuit(), qubit_map={cirq.GridQubit(1, 1): cirq.GridQubit(1, 2)}, measurement_key_map={'m': 'results'}, param_resolver={'k': 1.0}, repetitions=repetitions, repetition_ids=repetition_ids)
