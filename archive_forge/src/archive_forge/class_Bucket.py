...
class Bucket(S3Bucket):
    ...
    EncryptionConfigBody = '<?xml version="1.0" encoding="UTF-8"?>\n<EncryptionConfiguration>%s</EncryptionConfiguration>'
    ...
    VersioningBody = '<?xml version="1.0" encoding="UTF-8"?>\n<VersioningConfiguration><Status>%s</Status></VersioningConfiguration>'
    ...
    def get_key(self, key_name, headers=None, version_id=None, response_headers=None, generation=None):
        ...
        try:
            key, resp = self._get_key_internal(key_name, headers, query_args_l=query_args_l)
    ...
    def set_acl(self, acl_or_str, key_name='', headers=None, version_id=None, generation=None, if_generation=None, if_metageneration=None):
        ...
        if isinstance(acl_or_str, Policy):
            ...
            self.set_xml_acl(acl_or_str.to_xml(), key_name, headers=headers, generation=generation, if_generation=if_generation, if_metageneration=if_metageneration)
        else:
            self.set_canned_acl(acl_or_str, key_name, headers=headers, generation=generation, if_generation=if_generation, if_metageneration=if_metageneration)
    ...
    def set_canned_acl(self, acl_str, key_name='', headers=None, version_id=None, generation=None, if_generation=None, if_metageneration=None):
        ...
        query_args = STANDARD_ACL
        return self._set_acl_helper(acl_str, key_name, headers, query_args, generation, if_generation, if_metageneration, canned=True)
    ...
    def get_cors(self, headers=None):
        ...
        """
        response = self.connection.make_request('GET', self.name, query_args=CORS_ARG, headers=headers)
        body = response.read()
        if response.status == 200:
            cors = Cors()
    ...
    def configure_website(self, main_page_suffix=None, error_key=None, headers=None):
        ...
        response = self.connection.make_request('PUT', get_utf8able_str(self.name), data=get_utf8able_str(body), query_args='websiteConfig', headers=headers)
        body = response.read()
        if response.status == 200:
            return True
        else:
    ...
    def get_billing_configuration_with_xml(self, headers=None):
        ...
        :returns: 2-tuple containing:
            ...
            2) Unparsed XML describing the bucket's website configuration.
        """
        response = self.connection.make_request('GET', self.name, query_args='billing', headers=headers)
        body = response.read()
        boto.log.debug(body)
        if response.status != 200:
            raise self.connection.provider.storage_response_error(response.status, response.reason, body)
