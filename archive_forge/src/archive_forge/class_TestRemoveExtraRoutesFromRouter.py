...
class TestRemoveExtraRoutesFromRouter(TestRouter):
    ...
    def setUp(self):
        ...
        self.network_client.remove_extra_routes_from_router = mock.Mock(return_value=self._router)
    ...
    def test_remove_one_extra_route(self):
        ...
        parsed_args = self.check_parser(self.cmd, arglist, verifylist)
        result = self.cmd.take_action(parsed_args)
        self.network_client.remove_extra_routes_from_router.assert_called_with(self._router, body={'router': {'routes': [{'destination': 'dst1', 'nexthop': 'gw1'}]}})
        self.assertEqual(2, len(result))
