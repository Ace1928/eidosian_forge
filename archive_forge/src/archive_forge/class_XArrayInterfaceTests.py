...
class XArrayInterfaceTests(BaseGridInterfaceTests):
    ...
    def get_multi_dim_irregular_dataset(self):
        ...
        return xr.Dataset({'temperature': (['x', 'y', 'z', 'time'], temp), 'precipitation': (['x', 'y', 'z', 'time'], precip)}, coords={'lon': (['x', 'y'], lon), 'lat': (['x', 'y'], lat), 'z': np.arange(4), 'time': pd.date_range('2014-09-06', periods=3), 'reference_time': pd.Timestamp('2014-09-05')})
    ...
    def test_datetime64_bins_range(self):
        xs = list(np.arange(dt.datetime(2018, 1, 1), dt.datetime(2018, 1, 11), dt.timedelta(days=1)).astype('datetime64[ns]'))
        ys = np.arange(10)
        array = np.random.rand(10, 10)
        ds = QuadMesh((xs, ys, array))
        self.assertEqual(ds.interface.datatype, 'xarray')
    ...
    def test_image_stack_xarray_dataset(self):
        ...
        ds = xr.Dataset({'a': (['x', 'y'], a), 'b': (['x', 'y'], b), 'c': (['x', 'y'], c)}, coords={'x': x, 'y': y})
