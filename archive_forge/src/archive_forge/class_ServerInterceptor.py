    ...
    async def intercept_service(self, continuation, handler_call_details):
        ...
        if not handler.request_streaming and (not handler.response_streaming):
            ...
            async def wrapped(request, context):
                ...
                hub = Hub.current
                transaction = Transaction.continue_from_headers(dict(context.invocation_metadata()), op=OP.GRPC_SERVER, name=name, source=TRANSACTION_SOURCE_CUSTOM)
                with hub.start_transaction(transaction=transaction):
                    try:
                        ...
                        event, hint = event_from_exception(exc, mechanism={'type': 'grpc', 'handled': False})
            ...
            async def wrapped(request, context):
                async for r in handler.stream_stream(request, context):
                    yield r
        return handler_factory(wrapped, request_deserializer=handler.request_deserializer, response_serializer=handler.response_serializer)
