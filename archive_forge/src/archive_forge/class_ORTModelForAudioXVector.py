...
class ORTModelForAudioXVector(ORTModel):
    ...
    @add_start_docstrings_to_model_forward(ONNX_AUDIO_INPUTS_DOCSTRING.format('batch_size, sequence_length') + AUDIO_XVECTOR_EXAMPLE.format(processor_class=_FEATURE_EXTRACTOR_FOR_DOC, model_class='ORTModelForAudioXVector', checkpoint='optimum/wav2vec2-base-superb-sv'))
    def forward(self, input_values: Optional[torch.Tensor]=None, **kwargs):
        ...
        if self.device.type == 'cuda' and self.use_io_binding:
            ...
            return XVectorOutput(logits=output_buffers['logits'].view(output_shapes['logits']), embeddings=output_buffers['embeddings'].view(output_shapes['embeddings']))
        else:
            if use_torch:
