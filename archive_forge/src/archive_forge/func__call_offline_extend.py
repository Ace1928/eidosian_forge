...
def _call_offline_extend(self, volume, size_gb):
    size_gb += 1
    LOG.debug('Extending (offline) volume %(volume_id)s to %(size)s GB.', {'volume_id': volume.id, 'size': size_gb})
    volume.extend(volume, size_gb)
    try:
        volume = self._wait_volume_status(volume, 'extending', 'available')
        size_gb = volume.size
        return size_gb
