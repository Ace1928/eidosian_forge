...
class CommonTemplate(template.Template):
    ...
    def __init__(self, template, template_id=None, files=None, env=None):
        super(CommonTemplate, self).__init__(template, template_id=template_id, files=files, env=env)
    ...
    def _rsrc_defn_args(self, stack, name, data):
        ...
        def no_parse(field, path):
            return field
        yield ('resource_type', self._parse_resource_field(self.RES_TYPE, str, 'string', name, data, parse))
        yield ('properties', self._parse_resource_field(self.RES_PROPERTIES, (collections.abc.Mapping, function.Function), 'object', name, data, parse))
        yield ('metadata', self._parse_resource_field(self.RES_METADATA, (collections.abc.Mapping, function.Function), 'object', name, data, parse))
        depends = self._parse_resource_field(self.RES_DEPENDS_ON, collections.abc.Sequence, 'list or string', name, data, no_parse)
        if isinstance(depends, str):
            depends = [depends]
        elif depends:
            for dep in depends:
                if not isinstance(dep, str):
                    msg = _('Resource %(name)s %(key)s must be a list of strings') % {'name': name, 'key': self.RES_DEPENDS_ON}
                    raise exception.StackValidationFailed(message=msg)
        yield ('depends', depends)
        del_policy = self._parse_resource_field(self.RES_DELETION_POLICY, (str, function.Function), 'string', name, data, parse)
        ...
        yield ('update_policy', self._parse_resource_field(self.RES_UPDATE_POLICY, (collections.abc.Mapping, function.Function), 'object', name, data, parse))
        yield ('description', self._parse_resource_field(self.RES_DESCRIPTION, str, 'string', name, data, no_parse))
    ...
    def conditions(self, stack):
        ...
        if cached_conds is not None and get_cache_stack is not None and (get_cache_stack() is stack):
            return cached_conds
        raw_defs = self._get_condition_definitions()
        if not isinstance(raw_defs, collections.abc.Mapping):
            message = _('Condition definitions must be a map. Found a %s instead') % type(raw_defs).__name__
            raise exception.StackValidationFailed(error='Conditions validation error', message=message)
        parsed = {n: self.parse_condition(stack, c, '.'.join([self.CONDITIONS, n])) for n, c in raw_defs.items()}
        ...
        def get_outputs():
            for key, val in outputs.items():
                if not isinstance(val, collections.abc.Mapping):
                    message = _('Output definitions must be a map. Found a %s instead') % type(val).__name__
                    raise exception.StackValidationFailed(error='Output validation error', path=[self.OUTPUTS, key], message=message)
                if self.OUTPUT_VALUE not in val:
                    message = _('Each output definition must contain a %s key.') % self.OUTPUT_VALUE
                    raise exception.StackValidationFailed(error='Output validation error', path=[self.OUTPUTS, key], message=message)
                description = val.get(self.OUTPUT_DESCRIPTION)
                if hasattr(self, 'OUTPUT_CONDITION'):
                    path = [self.OUTPUTS, key, self.OUTPUT_CONDITION]
                    cond = self.parse_condition(stack, val.get(self.OUTPUT_CONDITION), '.'.join(path))
                    try:
                    ...
                    if not enabled:
                        yield (key, output.OutputDefinition(key, None, description))
                        continue
                value_def = self.parse(stack, val[self.OUTPUT_VALUE], path='.'.join([self.OUTPUTS, key, self.OUTPUT_VALUE]))
