...
class InsertionNoiseModel(devices.NoiseModel):
    ...
    """
    ops_added: Dict[noise_utils.OpIdentifier, 'cirq.Operation'] = dataclasses.field(default_factory=dict)
    prepend: bool = False
    require_physical_tag: bool = True

    def noisy_moment(self, moment: 'cirq.Moment', system_qubits: Sequence['cirq.Qid']) -> 'cirq.OP_TREE':
        noise_ops: List['cirq.Operation'] = []
        candidate_ops = [op for op in moment if not self.require_physical_tag or noise_utils.PHYSICAL_GATE_TAG in op.tags]
    ...
    def __repr__(self) -> str:
        return f'cirq.devices.InsertionNoiseModel(ops_added={self.ops_added},' + f' prepend={self.prepend},' + f' require_physical_tag={self.require_physical_tag})'

    def _json_dict_(self) -> Dict[str, Any]:
        return {'ops_added': list(self.ops_added.items()), 'prepend': self.prepend, 'require_physical_tag': self.require_physical_tag}
