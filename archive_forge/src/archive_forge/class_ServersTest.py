...
class ServersTest(common.HeatTestCase):
    ...
    def setUp(self):
        ...
        self.patchobject(nova.NovaClientPlugin, 'find_flavor_by_name_or_id', side_effect=flavor_side_effect)
        self.patchobject(glance.GlanceClientPlugin, 'find_image_by_name_or_id', side_effect=image_side_effect)
        self.port_show = self.patchobject(neutronclient.Client, 'show_port')
        self.subnet_show = self.patchobject(neutronclient.Client, 'show_subnet')
        self.network_show = self.patchobject(neutronclient.Client, 'show_network')
    ...
    def _setup_test_stack(self, stack_name, test_templ=wp_template):
        ...
        if test_templ == ns_template:
            files = {'a_file': 'stub'}
        templ = template.Template(t, env=environment.Environment({'key_name': 'test'}), files=files)
        stack = parser.Stack(utils.dummy_context(region_name='RegionOne'), stack_name, templ, stack_id=uuidutils.generate_uuid(), stack_user_project_id='8888')
        return (templ, stack)
    ...
    def _get_test_template(self, stack_name, server_name=None, image_id=None):
        tmpl, stack = self._setup_test_stack(stack_name)
        tmpl.t['Resources']['WebServer']['Properties']['image'] = image_id or 'CentOS 5.2'
        tmpl.t['Resources']['WebServer']['Properties']['flavor'] = '256 MB Server'
    ...
    def _create_test_server(self, return_server, name, override_name=False, stub_create=True, networks=None):
        server = self._setup_test_server(return_server, name, stub_create=stub_create, networks=networks)
        scheduler.TaskRunner(server.create)()
        return server

    def test_subnet_dependency_by_network_id(self):
        templ, stack = self._setup_test_stack('subnet-test', subnet_template)
    ...
    def test_subnet_dependency_unknown_network_id(self):
        templ, stack = self._setup_test_stack('subnet-test', multi_subnet_template)
        server_rsrc = stack['server']
    ...
    def test_subnet_nodeps(self):
        templ, stack = self._setup_test_stack('subnet-test', no_subnet_template)
        server_rsrc = stack['server']
        subnet_rsrc = stack['subnet']
        deps = []
        server_rsrc.add_explicit_dependencies(deps)
    ...
    def test_server_create_with_str_network(self):
        ...
        return_server = self.fc.servers.list()[1]
        tmpl, stack = self._setup_test_stack(stack_name)
        mock_nc = self.patchobject(nova.NovaClientPlugin, 'client', return_value=self.fc)
        self.patchobject(glance.GlanceClientPlugin, 'get_image', return_value=self.mock_image)
        ...
        self.patchobject(server, 'store_external_ports')
        create_mock = self.patchobject(self.fc.servers, 'create', return_value=return_server)
        scheduler.TaskRunner(server.create)()
    ...
    def test_server_duplicate_image_name_err(self):
        ...
        mock_image = self.patchobject(glance.GlanceClientPlugin, 'find_image_by_name_or_id')
        self.stub_KeypairConstraint_validate()
        mock_image.side_effect = glance.client_exception.EntityUniqueMatchNotFound(entity='image', args='CentOS 5.2')
        tmpl['Resources']['WebServer']['Properties']['image'] = 'CentOS 5.2'
        resource_defns = tmpl.resource_definitions(stack)
        server = servers.Server('WebServer', resource_defns['WebServer'], stack)
        error = self.assertRaises(exception.ResourceFailure, scheduler.TaskRunner(server.create))
        self.assertIn('No image unique match found for CentOS 5.2.', str(error))

    def test_server_create_unexpected_status(self):
        return_server = self.fc.servers.list()[1]
        server = self._create_test_server(return_server, 'cr_unexp_sts')
        return_server.status = 'BOGUS'
        self.patchobject(self.fc.servers, 'get', return_value=return_server)
        e = self.assertRaises(exception.ResourceUnknownStatus, server.check_create_complete, server.resource_id)
        self.assertEqual('Server is not active - Unknown status BOGUS due to "Unknown"', str(e))

    def test_server_create_error_status(self):
        return_server = self.fc.servers.list()[1]
        server = self._create_test_server(return_server, 'cr_err_sts')
        return_server.status = 'ERROR'
        return_server.fault = {'message': 'NoValidHost', 'code': 500, 'created': '2013-08-14T03:12:10Z'}
        self.patchobject(self.fc.servers, 'get', return_value=return_server)
        e = self.assertRaises(exception.ResourceInError, server.check_create_complete, server.resource_id)
        self.assertEqual('Went to status ERROR due to "Message: NoValidHost, Code: 500"', str(e))
    ...
    def test_server_create_raw_config_userdata(self):
        ...
        tmpl, stack = self._setup_test_stack(stack_name)
        tmpl['Resources']['WebServer']['Properties']['user_data_format'] = 'RAW'
        tmpl['Resources']['WebServer']['Properties']['user_data'] = '8c813873-f6ee-4809-8eec-959ef39acb55'
        resource_defns = tmpl.resource_definitions(stack)
        server = servers.Server('WebServer', resource_defns['WebServer'], stack)
        self.patchobject(server, 'store_external_ports')
    ...
    def _server_create_software_config(self, md=None, stack_name='software_config_s', ret_tmpl=False):
        self.patchobject(nova.NovaClientPlugin, 'client', return_value=self.fc)
        return_server = self.fc.servers.list()[1]
        tmpl, stack = self._setup_test_stack(stack_name)
        self.stack = stack
    ...
    def test_server_create_software_config(self, fake_url):
        ...
        self.assertEqual({'os-collect-config': {'cfn': {'access_key_id': '4567', 'metadata_url': 'http://ip:8000/v1/', 'path': 'WebServer.Metadata', 'secret_access_key': '8901', 'stack_name': 'software_config_s'}, 'collectors': ['ec2', 'cfn', 'local']}, 'deployments': []}, server.metadata_get())

    @mock.patch.object(heat_plugin.HeatClientPlugin, 'url_for')
    def test_resolve_attribute_os_collect_config(self, fake_url):
        fake_url.return_value = 'http://ip/heat-api-cfn/v1'
        server = self._server_create_software_config()
        self.assertEqual({'cfn': {'access_key_id': '4567', 'metadata_url': 'http://ip/heat-api-cfn/v1/', 'path': 'WebServer.Metadata', 'secret_access_key': '8901', 'stack_name': 'software_config_s'}, 'collectors': ['ec2', 'cfn', 'local']}, server.FnGetAtt('os_collect_config'))
    ...
    def test_server_create_software_config_metadata(self, fake_url):
        ...
        self.assertEqual({'os-collect-config': {'cfn': {'access_key_id': '4567', 'metadata_url': 'http://ip/heat-api-cfn/v1/', 'path': 'WebServer.Metadata', 'secret_access_key': '8901', 'stack_name': 'software_config_s'}, 'collectors': ['ec2', 'cfn', 'local'], 'polling_interval': 10}, 'deployments': []}, server.metadata_get())
    ...
    def test_server_create_software_config_poll_heat(self):
        ...
        self.assertEqual({'os-collect-config': {'heat': {'auth_url': 'http://server.test:5000/v2.0', 'password': server.password, 'project_id': '8888', 'region_name': 'RegionOne', 'resource_name': 'WebServer', 'stack_id': 'software_config_s/%s' % stack.id, 'user_id': '1234'}, 'collectors': ['ec2', 'heat', 'local']}, 'deployments': []}, server.metadata_get())
    ...
    def test_server_create_software_config_poll_heat_metadata(self):
        ...
        self.assertEqual({'os-collect-config': {'heat': {'auth_url': 'http://server.test:5000/v2.0', 'password': server.password, 'project_id': '8888', 'region_name': 'RegionOne', 'resource_name': 'WebServer', 'stack_id': 'software_config_s/%s' % stack.id, 'user_id': '1234'}, 'collectors': ['ec2', 'heat', 'local'], 'polling_interval': 10}, 'deployments': []}, server.metadata_get())
    ...
    def _server_create_software_config_poll_temp_url(self, md=None):
        ...
        test_path = '/v1/AUTH_test_tenant_id/%s/%s' % (server.physical_resource_name(), object_name)
    ...
    def test_server_create_software_config_poll_temp_url(self):
        ...
        self.assertEqual({'os-collect-config': {'request': {'metadata_url': metadata_url}, 'collectors': ['ec2', 'request', 'local']}, 'deployments': []}, server.metadata_get())

    def test_server_create_software_config_poll_temp_url_metadata(self):
        md = {'os-collect-config': {'polling_interval': 10}}
        metadata_url, server = self._server_create_software_config_poll_temp_url(md=md)
        self.assertEqual({'os-collect-config': {'request': {'metadata_url': metadata_url}, 'collectors': ['ec2', 'request', 'local'], 'polling_interval': 10}, 'deployments': []}, server.metadata_get())
    ...
    def test_server_create_software_config_zaqar(self):
        ...
        self.assertEqual({'os-collect-config': {'zaqar': {'user_id': '1234', 'password': server.password, 'auth_url': 'http://server.test:5000/v2.0', 'project_id': '8888', 'queue_id': queue_id, 'region_name': 'RegionOne'}, 'collectors': ['ec2', 'zaqar', 'local']}, 'deployments': []}, server.metadata_get())
    ...
    def test_create_delete_no_zaqar_service(self):
        ...
        self.assertEqual({'os-collect-config': {'zaqar': {'user_id': '1234', 'password': server.password, 'auth_url': 'http://server.test:5000/v2.0', 'project_id': '8888', 'queue_id': mock.ANY, 'region_name': 'RegionOne'}, 'collectors': ['ec2', 'zaqar', 'local']}, 'deployments': []}, server.metadata_get())
        scheduler.TaskRunner(server.delete)()
        self.assertEqual((server.DELETE, server.COMPLETE), server.state)
    ...
    def test_server_create_software_config_zaqar_metadata(self):
        ...
        self.assertEqual({'os-collect-config': {'zaqar': {'user_id': '1234', 'password': server.password, 'auth_url': 'http://server.test:5000/v2.0', 'project_id': '8888', 'queue_id': queue_id, 'region_name': 'RegionOne'}, 'collectors': ['ec2', 'zaqar', 'local'], 'polling_interval': 10}, 'deployments': []}, server.metadata_get())
    ...
    def test_check_maximum(self):
        ...
        error = self.assertRaises(exception.StackValidationFailed, servers.Server._check_maximum, 2, 1, msg)
    ...
    def test_server_validate(self):
        ...
        self.patchobject(nova.NovaClientPlugin, 'client', return_value=self.fc)
        resource_defns = tmpl.resource_definitions(stack)
        server = servers.Server('server_create_image', resource_defns['WebServer'], stack)
        self.patchobject(glance.GlanceClientPlugin, 'get_image', return_value=self.mock_image)
        self.patchobject(nova.NovaClientPlugin, 'get_flavor', return_value=self.mock_flavor)
    ...
    def test_server_validate_with_invalid_ssh_key(self):
        ...
        self.assertEqual("Property error: Resources.WebServer.Properties.key_name: Error validating value 'test2': The Key (test2) could not be found.", str(error))
    ...
    def test_server_validate_software_config_invalid_meta(self):
        ...
        self.patchobject(nova.NovaClientPlugin, 'get_flavor', return_value=self.mock_flavor)
        error = self.assertRaises(exception.StackValidationFailed, server.validate)
        self.assertEqual('deployments key not allowed in resource metadata with user_data_format of SOFTWARE_CONFIG', str(error))
    ...
    def test_server_validate_with_network_empty_ref(self):
        ...
        tmpl, stack = self._setup_test_stack(stack_name)
        self.patchobject(nova.NovaClientPlugin, 'client', return_value=self.fc)
        tmpl['Resources']['WebServer']['Properties']['networks'] = [{'network': ''}]
        resource_defns = tmpl.resource_definitions(stack)
        server = servers.Server('server_validate_with_networks', resource_defns['WebServer'], stack)
        self.patchobject(glance.GlanceClientPlugin, 'get_image', return_value=self.mock_image)
        self.patchobject(nova.NovaClientPlugin, 'get_flavor', return_value=self.mock_flavor)
    ...
    def test_server_validate_with_only_fixed_ip(self):
        stack_name = 'srv_net'
        tmpl, stack = self._setup_test_stack(stack_name)
        tmpl['Resources']['WebServer']['Properties']['networks'] = [{'fixed_ip': '10.0.0.99'}]
        self.patchobject(nova.NovaClientPlugin, 'client', return_value=self.fc)
        resource_defns = tmpl.resource_definitions(stack)
        server = servers.Server('server_validate_with_networks', resource_defns['WebServer'], stack)
        self.patchobject(glance.GlanceClientPlugin, 'get_image', return_value=self.mock_image)
        ...
        self.patchobject(neutron.NeutronClientPlugin, 'find_resourceid_by_name_or_id')
        ex = self.assertRaises(exception.StackValidationFailed, server.validate)
        self.assertIn(_('One of the properties "network", "port", "allocate_network" or "subnet" should be set for the specified network of server "%s".') % server.name, str(ex))

    def test_server_validate_with_network_floating_ip(self):
        stack_name = 'srv_net_floating_ip'
        tmpl, stack = self._setup_test_stack(stack_name)
        tmpl['Resources']['WebServer']['Properties']['networks'] = [{'floating_ip': '172.24.4.14', 'network': '6b1688bb-18a0-4754-ab05-19daaedc5871'}]
        self.patchobject(nova.NovaClientPlugin, 'client', return_value=self.fc)
        resource_defns = tmpl.resource_definitions(stack)
        server = servers.Server('server_validate_net_floating_ip', resource_defns['WebServer'], stack)
        self.patchobject(glance.GlanceClientPlugin, 'get_image', return_value=self.mock_image)
        self.patchobject(nova.NovaClientPlugin, 'get_flavor', return_value=self.mock_flavor)
        self.patchobject(neutron.NeutronClientPlugin, 'find_resourceid_by_name_or_id')
        ex = self.assertRaises(exception.StackValidationFailed, server.validate)
        self.assertIn(_('Property "floating_ip" is not supported if only "network" is specified, because the corresponding port can not be retrieved.'), str(ex))
    ...
    def test_server_validate_with_uuid_fixed_ip(self):
        stack_name = 'srv_net'
        tmpl, stack = self._setup_test_stack(stack_name)
        tmpl['Resources']['WebServer']['Properties']['networks'] = [{'uuid': 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa', 'fixed_ip': '10.0.0.99'}]
        self.patchobject(nova.NovaClientPlugin, 'client', return_value=self.fc)
        resource_defns = tmpl.resource_definitions(stack)
        server = servers.Server('server_validate_with_networks', resource_defns['WebServer'], stack)
        self.patchobject(neutron.NeutronClientPlugin, 'find_resourceid_by_name_or_id')
    ...
    def test_server_validate_with_network_fixed_ip(self):
        stack_name = 'srv_net'
        tmpl, stack = self._setup_test_stack(stack_name)
        tmpl['Resources']['WebServer']['Properties']['networks'] = [{'network': 'public', 'fixed_ip': '10.0.0.99'}]
        self.patchobject(nova.NovaClientPlugin, 'client', return_value=self.fc)
        resource_defns = tmpl.resource_definitions(stack)
        server = servers.Server('server_validate_with_networks', resource_defns['WebServer'], stack)
        self.patchobject(glance.GlanceClientPlugin, 'get_image', return_value=self.mock_image)
    ...
    def test_server_validate_net_security_groups(self):
        ...
        tmpl, stack = self._setup_test_stack(stack_name)
        tmpl['Resources']['WebServer']['Properties']['networks'] = [{'port': ''}]
        tmpl['Resources']['WebServer']['Properties']['security_groups'] = ['my_security_group']
        self.patchobject(nova.NovaClientPlugin, 'client', return_value=self.fc)
        resource_defns = tmpl.resource_definitions(stack)
        server = servers.Server('server_validate_net_security_groups', resource_defns['WebServer'], stack)
        self.patchobject(glance.GlanceClientPlugin, 'get_image', return_value=self.mock_image)
        self.patchobject(nova.NovaClientPlugin, 'get_flavor', return_value=self.mock_flavor)
        self.patchobject(neutron.NeutronClientPlugin, 'find_resourceid_by_name_or_id')
        error = self.assertRaises(exception.ResourcePropertyConflict, server.validate)
        self.assertEqual('Cannot define the following properties at the same time: security_groups, networks/port.', str(error))

    def test_server_delete(self):
        return_server = self.fc.servers.list()[1]
        server = self._create_test_server(return_server, 'create_delete')
        server.resource_id = '1234'
        self.assertGreater(server.id, 0)
        side_effect = [server, fakes_nova.fake_exception()]
        self.patchobject(self.fc.servers, 'get', side_effect=side_effect)
    ...
    def test_server_delete_notfound(self):
        return_server = self.fc.servers.list()[1]
        server = self._create_test_server(return_server, 'create_delete2')
        server.resource_id = '1234'
        self.assertGreater(server.id, 0)
        self.patchobject(self.fc.client, 'delete_servers_1234', side_effect=fakes_nova.fake_exception())
        scheduler.TaskRunner(server.delete)()
        self.assertEqual((server.DELETE, server.COMPLETE), server.state)
    ...
    def test_server_delete_error(self):
        ...
        self.patchobject(self.fc.servers, 'get', side_effect=[return_server, return_server, make_error()])
        resf = self.assertRaises(exception.ResourceFailure, scheduler.TaskRunner(server.delete))
        self.assertIn('Server %s delete failed' % return_server.name, str(resf))
    ...
    def test_server_delete_error_task_in_progress(self):
        ...
        def make_error_done(*args):
            ...
            return return_server
        self.patchobject(self.fc.servers, 'get', side_effect=[make_error(), make_error_done()])
        resf = self.assertRaises(exception.ResourceFailure, scheduler.TaskRunner(server.delete))
        self.assertIn('Server %s delete failed' % return_server.name, str(resf))
    ...
    def test_server_soft_delete(self):
        ...
        server.resource_id = '1234'
        self.assertGreater(server.id, 0)
        self.patchobject(self.fc.servers, 'get', side_effect=ServerStatus(return_server, [return_server.status, return_server.status, 'SOFT_DELETED', 'DELETED']))
        scheduler.TaskRunner(server.delete)()
        self.assertEqual((server.DELETE, server.COMPLETE), server.state)

    def test_server_update_metadata(self):
        return_server = self.fc.servers.list()[1]
        server = self._create_test_server(return_server, 'md_update')
    ...
    def test_server_update_metadata_software_config(self, fake_url):
        fake_url.return_value = 'http://ip:8000/v1'
        server, ud_tmpl = self._server_create_software_config(stack_name='update_meta_sc', ret_tmpl=True)
        expected_md = {'os-collect-config': {'cfn': {'access_key_id': '4567', 'metadata_url': 'http://ip:8000/v1/', 'path': 'WebServer.Metadata', 'secret_access_key': '8901', 'stack_name': 'update_meta_sc'}, 'collectors': ['ec2', 'cfn', 'local']}, 'deployments': []}
    ...
    def test_update_transport_heat_to_zaqar(self):
        ...
        self.assertEqual({'os-collect-config': {'heat': {'auth_url': 'http://server.test:5000/v2.0', 'password': password, 'project_id': '8888', 'region_name': 'RegionOne', 'resource_name': 'WebServer', 'stack_id': 'software_config_s/%s' % stack.id, 'user_id': '1234'}, 'collectors': ['ec2', 'heat', 'local']}, 'deployments': []}, server.metadata_get())
        update_props = self.server_props.copy()
        ...
        self.assertEqual({'os-collect-config': {'zaqar': {'user_id': '1234', 'password': password_1, 'auth_url': 'http://server.test:5000/v2.0', 'project_id': '8888', 'queue_id': server.data().get('metadata_queue_id'), 'region_name': 'RegionOne'}, 'heat': {'auth_url': None, 'password': None, 'project_id': None, 'region_name': None, 'resource_name': None, 'stack_id': None, 'user_id': None}, 'collectors': ['ec2', 'zaqar', 'local']}, 'deployments': []}, server.metadata_get())
    ...
    def test_server_update_nova_metadata(self):
        ...
        scheduler.TaskRunner(server.update, update_template)()
        self.assertEqual((server.UPDATE, server.COMPLETE), server.state)
        set_meta_mock.assert_called_with(return_server, server.client_plugin().meta_serialize(new_meta))
    ...
    def test_server_update_nova_metadata_with_delete(self):
        ...
        self.patchobject(self.fc.servers, 'get', return_value=new_return_server)
        ...
        scheduler.TaskRunner(server.update, update_template)()
        self.assertEqual((server.UPDATE, server.COMPLETE), server.state)
        del_meta_mock.assert_called_with(new_return_server, ['test', 'this'])
        set_meta_mock.assert_called_with(new_return_server, server.client_plugin().meta_serialize(new_meta))

    def test_server_update_server_name(self):
        """Server.handle_update supports changing the name."""
        return_server = self.fc.servers.list()[1]
        return_server.id = '5678'
        server = self._create_test_server(return_server, 'srv_update')
        new_name = 'new_name'
        update_props = self.server_props.copy()
    ...
    def test_server_update_server_admin_password(self):
        ...
        return_server = self.fc.servers.list()[1]
        return_server.id = '5678'
        server = self._create_test_server(return_server, 'change_password')
    ...
    def test_server_update_server_flavor(self):
        ...
        update_template = server.t.freeze(properties=update_props)
        self.patchobject(self.fc.servers, 'get', side_effect=ServerStatus(return_server, ['ACTIVE', 'RESIZE', 'VERIFY_RESIZE', 'VERIFY_RESIZE', 'ACTIVE']))
    ...
    def test_server_update_server_flavor_replace(self, mock_replace):
        ...
        resource_defns = tmpl.resource_definitions(stack)
        server = servers.Server('server_server_update_flavor_replace', resource_defns['WebServer'], stack)
    ...
    def test_server_update_server_userdata_replace(self, mock_create, mock_replace):
        stack_name = 'update_udatrep'
        tmpl, stack = self._setup_test_stack(stack_name)
        resource_defns = tmpl.resource_definitions(stack)
        server = servers.Server('server_update_userdata_replace', resource_defns['WebServer'], stack)
        update_props = tmpl.t['Resources']['WebServer']['Properties'].copy()
    ...
    @mock.patch.object(servers.Server, 'prepare_for_replace')
    @mock.patch.object(nova.NovaClientPlugin, 'client')
    def test_server_update_server_userdata_rebuild(self, mock_create, mock_replace):
    ...
    def test_server_update_image_rebuild_status_rebuild_keep_ephemeral(self):
        self._test_server_update_image_rebuild(policy='REBUILD_PRESERVE_EPHEMERAL', status=('REBUILD', 'ACTIVE'))

    def test_server_update_image_rebuild_status_active_keep_ephemeral(self):
        self._test_server_update_image_rebuild(policy='REBUILD_PRESERVE_EPHEMERAL', status=('ACTIVE',))

    def test_server_update_image_rebuild_with_new_password(self):
        self._test_server_update_image_rebuild(password='new_admin_password', status=('REBUILD', 'ACTIVE'))

    def test_server_update_image_rebuild_failed(self):
        return_server = self.fc.servers.list()[1]
        return_server.id = '1234'
        server = self._create_test_server(return_server, 'srv_updrbldfail')
        new_image = 'F17-x86_64-gold'
        before_props = self.server_props.copy()
        ...
        server.reparse()
        mock_rebuild = self.patchobject(self.fc.servers, 'rebuild')
        self.patchobject(self.fc.servers, 'get', side_effect=ServerStatus(return_server, ['REBUILD', 'ERROR']))
        updater = scheduler.TaskRunner(server.update, update_template)
        error = self.assertRaises(exception.ResourceFailure, updater)
        self.assertEqual("Error: resources.srv_updrbldfail: Rebuilding server failed, status 'ERROR'", str(error))
        self.assertEqual((server.UPDATE, server.FAILED), server.state)
        mock_rebuild.assert_called_once()
        self.assertEqual((return_server, '2'), mock_rebuild.call_args.args)
        self.assertLessEqual({'password': None, 'preserve_ephemeral': False, 'meta': {}, 'files': {}}.items(), mock_rebuild.call_args.kwargs.items())

    def test_server_update_properties(self):
        return_server = self.fc.servers.list()[1]
        server = self._create_test_server(return_server, 'update_prop')
        update_props = self.server_props.copy()
    ...
    def test_server_status_build(self):
        ...
        server = self._setup_test_server(return_server, 'sts_build')
        server.resource_id = '1234'

        def status_active(*args):
            return_server.status = 'ACTIVE'
            return return_server
        self.patchobject(self.fc.servers, 'get', return_value=status_active())
        scheduler.TaskRunner(server.create)()
        self.assertEqual((server.CREATE, server.COMPLETE), server.state)

    def test_server_status_suspend_no_resource_id(self):
        return_server = self.fc.servers.list()[1]
        server = self._create_test_server(return_server, 'srv_sus1')
        server.resource_id = None
        ex = self.assertRaises(exception.ResourceFailure, scheduler.TaskRunner(server.suspend))
        self.assertEqual('Error: resources.srv_sus1: Cannot suspend srv_sus1, resource_id not set', str(ex))
        self.assertEqual((server.SUSPEND, server.FAILED), server.state)

    def test_server_status_suspend_not_found(self):
        return_server = self.fc.servers.list()[1]
        server = self._create_test_server(return_server, 'srv_sus2')
        server.resource_id = '1234'
        self.patchobject(self.fc.servers, 'get', side_effect=fakes_nova.fake_exception())
        ex = self.assertRaises(exception.ResourceFailure, scheduler.TaskRunner(server.suspend))
        self.assertEqual('NotFound: resources.srv_sus2: Failed to find server 1234', str(ex))
        self.assertEqual((server.SUSPEND, server.FAILED), server.state)
    ...
    def _test_server_status_suspend(self, name, state=('CREATE', 'COMPLETE')):
        ...
        self.patchobject(self.fc.servers, 'get', side_effect=ServerStatus(return_server, ['ACTIVE', 'ACTIVE', 'SUSPENDED']))
    ...
    def test_server_suspend_in_suspend_failed(self):
        self._test_server_status_suspend(name='test_suspend_in_suspend_failed', state=('SUSPEND', 'FAILED'))

    def test_server_suspend_in_suspend_complete(self):
        self._test_server_status_suspend(name='test_suspend_in_suspend_complete', state=('SUSPEND', 'COMPLETE'))

    def test_server_status_suspend_unknown_status(self):
        return_server = self.fc.servers.list()[1]
        server = self._create_test_server(return_server, 'srv_susp_uk')
        server.resource_id = '1234'
        self.patchobject(return_server, 'suspend')
        self.patchobject(self.fc.servers, 'get', side_effect=ServerStatus(return_server, ['ACTIVE', 'ACTIVE', 'TRANSMOGRIFIED']))
        ex = self.assertRaises(exception.ResourceFailure, scheduler.TaskRunner(server.suspend))
        self.assertIsInstance(ex.exc, exception.ResourceUnknownStatus)
        self.assertEqual('Suspend of server %s failed - Unknown status TRANSMOGRIFIED due to "Unknown"' % return_server.name, str(ex.exc.message))
        self.assertEqual((server.SUSPEND, server.FAILED), server.state)
    ...
    def test_server_resume_in_suspend_complete(self):
        self._test_server_status_resume(name='test_resume_in_suspend_complete')

    def test_server_resume_in_resume_failed(self):
        self._test_server_status_resume(name='test_resume_in_resume_failed', state=('RESUME', 'FAILED'))

    def test_server_resume_in_resume_complete(self):
        self._test_server_status_resume(name='test_resume_in_resume_complete', state=('RESUME', 'COMPLETE'))

    def test_server_status_resume_no_resource_id(self):
        return_server = self.fc.servers.list()[1]
        server = self._create_test_server(return_server, 'srv_susp_norid')
        server.resource_id = None
        server.state_set(server.SUSPEND, server.COMPLETE)
        ex = self.assertRaises(exception.ResourceFailure, scheduler.TaskRunner(server.resume))
        self.assertEqual('Error: resources.srv_susp_norid: Cannot resume srv_susp_norid, resource_id not set', str(ex))
        self.assertEqual((server.RESUME, server.FAILED), server.state)

    def test_server_status_resume_not_found(self):
        return_server = self.fc.servers.list()[1]
        server = self._create_test_server(return_server, 'srv_res_nf')
        server.resource_id = '1234'
        self.patchobject(self.fc.servers, 'get', side_effect=fakes_nova.fake_exception())
        server.state_set(server.SUSPEND, server.COMPLETE)
        ex = self.assertRaises(exception.ResourceFailure, scheduler.TaskRunner(server.resume))
        self.assertEqual('NotFound: resources.srv_res_nf: Failed to find server 1234', str(ex))
        self.assertEqual((server.RESUME, server.FAILED), server.state)
    ...
    def _test_server_status_not_build_active(self, uncommon_status):
        return_server = self.fc.servers.list()[0]
        server = self._setup_test_server(return_server, 'srv_sts_bld')
        server.resource_id = '1234'
        self.patchobject(self.fc.servers, 'get', side_effect=ServerStatus(return_server, [uncommon_status, 'ACTIVE']))
        scheduler.TaskRunner(server.create)()
        self.assertEqual((server.CREATE, server.COMPLETE), server.state)
    ...
    @mock.patch.object(nova.NovaClientPlugin, 'client')
    def test_validate_block_device_mapping_volume_size_valid_int(self, mock_create):
    ...
    @mock.patch.object(nova.NovaClientPlugin, 'client')
    def test_validate_block_device_mapping_volume_size_valid_str(self, mock_create):
        stack_name = 'val_vsize_valid'
        tmpl, stack = self._setup_test_stack(stack_name)
        bdm = [{'device_name': 'vda', 'volume_id': '1234', 'volume_size': '10'}]
        wsp = tmpl.t['Resources']['WebServer']['Properties']
    ...
    def test_validate_conflict_block_device_mapping_props(self, mock_create):
        ...
        tmpl, stack = self._setup_test_stack(stack_name)
        bdm = [{'device_name': 'vdb', 'snapshot_id': '1234', 'volume_id': '1234'}]
        wsp = tmpl.t['Resources']['WebServer']['Properties']
        wsp['block_device_mapping'] = bdm
        resource_defns = tmpl.resource_definitions(stack)
        server = servers.Server('server_create_image_err', resource_defns['WebServer'], stack)
    ...
    @mock.patch.object(nova.NovaClientPlugin, 'client')
    def test_validate_insufficient_block_device_mapping_props(self, mock_create):
        stack_name = 'val_blkdev2'
        tmpl, stack = self._setup_test_stack(stack_name)
        bdm = [{'device_name': 'vdb', 'volume_size': 1, 'delete_on_termination': True}]
        wsp = tmpl.t['Resources']['WebServer']['Properties']
        wsp['block_device_mapping'] = bdm
        resource_defns = tmpl.resource_definitions(stack)
        server = servers.Server('server_create_image_err', resource_defns['WebServer'], stack)
        ex = self.assertRaises(exception.StackValidationFailed, server.validate)
        msg = 'Either volume_id or snapshot_id must be specified for device mapping vdb'
    ...
    def test_validate_block_device_mapping_with_empty_ref(self, mock_create):
        stack_name = 'val_blkdev2'
        tmpl, stack = self._setup_test_stack(stack_name)
        bdm = [{'device_name': 'vda', 'volume_id': '', 'volume_size': '10'}]
        wsp = tmpl.t['Resources']['WebServer']['Properties']
        wsp['block_device_mapping'] = bdm
        resource_defns = tmpl.resource_definitions(stack)
        server = servers.Server('server_create_image_err', resource_defns['WebServer'], stack)
    ...
    def test_validate_without_image_or_bootable_volume(self, mock_create):
        ...
        self.stub_VolumeConstraint_validate()
        ex = self.assertRaises(exception.StackValidationFailed, server.validate)
        msg = 'Neither image nor bootable volume is specified for instance %s' % server.name
        self.assertEqual(msg, str(ex))
    ...
    def test_validate_invalid_image_status(self, mock_create):
        ...
        tmpl, stack = self._setup_test_stack(stack_name)
        resource_defns = tmpl.resource_definitions(stack)
        server = servers.Server('server_inactive_image', resource_defns['WebServer'], stack)
        mock_image = mock.Mock(min_ram=2, status='sdfsdf')
        self.patchobject(glance.GlanceClientPlugin, 'get_image', return_value=mock_image)
        error = self.assertRaises(exception.StackValidationFailed, server.validate)
        self.assertEqual('Image status is required to be active not sdfsdf.', str(error))
    ...
    def test_validate_insufficient_ram_flavor(self, mock_create):
        ...
        tmpl, stack = self._setup_test_stack(stack_name)
        resource_defns = tmpl.resource_definitions(stack)
        server = servers.Server('server_insufficient_ram_flavor', resource_defns['WebServer'], stack)
        mock_image = mock.Mock(min_ram=100, status='active')
        self.patchobject(glance.GlanceClientPlugin, 'get_image', return_value=mock_image)
        self.patchobject(nova.NovaClientPlugin, 'get_flavor', return_value=self.mock_flavor)
        error = self.assertRaises(exception.StackValidationFailed, server.validate)
        self.assertEqual('Image F18-x86_64-gold requires 100 minimum ram. Flavor m1.large has only 4.', str(error))
    ...
    def test_validate_image_flavor_not_found(self, mock_create):
        ...
        tmpl, stack = self._setup_test_stack(stack_name)
        resource_defns = tmpl.resource_definitions(stack)
        server = servers.Server('image_not_found', resource_defns['WebServer'], stack)
        self.patchobject(glance.GlanceClientPlugin, 'get_image', side_effect=[glance.client_exception.EntityMatchNotFound, self.mock_image])
        self.patchobject(nova.NovaClientPlugin, 'get_flavor', side_effect=nova.exceptions.NotFound(''))
        self.assertIsNone(server.validate())
        self.assertIsNone(server.validate())
    ...
    def test_block_device_mapping_v2_image_resolve(self):
        tmpl, stack = self._setup_test_stack('mapping', test_templ=bdm_v2_template)
        resource_defns = tmpl.resource_definitions(stack)
        self.server = servers.Server('server', resource_defns['server'], stack)
        self.server.translate_properties(self.server.properties)
        self.assertEqual('2', self.server.properties['block_device_mapping_v2'][0]['image'])

    def test_block_device_mapping_v2_image_prop_conflict(self):
        test_templ = bdm_v2_template + '\n          image: F17-x86_64-gold'
        tmpl, stack = self._setup_test_stack('mapping', test_templ=test_templ)
        resource_defns = tmpl.resource_definitions(stack)
        msg = 'Cannot define the following properties at the same time: block_device_mapping_v2.image, block_device_mapping_v2.image_id'
        server = servers.Server('server', resource_defns['server'], stack)
        exc = self.assertRaises(exception.StackValidationFailed, server.validate)
    ...
    @mock.patch.object(nova.NovaClientPlugin, 'client')
    def test_validate_conflict_block_device_mapping_v2_props(self, mock_create):
        stack_name = 'val_blkdev2'
        bdm_v2 = [{'volume_id': '1', 'snapshot_id': 2}]
        error_msg = 'Cannot define the following properties at the same time: volume_id, snapshot_id'
        self.stub_SnapshotConstraint_validate()
        self._test_validate_bdm_v2(stack_name, bdm_v2, raise_exc=exception.ResourcePropertyConflict, error_msg=error_msg)

    @mock.patch.object(nova.NovaClientPlugin, 'client')
    def test_validate_bdm_v2_with_empty_mapping(self, mock_create):
        stack_name = 'val_blkdev2'
        bdm_v2 = [{}]
        msg = 'Either volume_id, snapshot_id, image_id, swap_size, ephemeral_size or ephemeral_format must be specified.'
        self._test_validate_bdm_v2(stack_name, bdm_v2, raise_exc=exception.StackValidationFailed, error_msg=msg)
    ...
    def test_validate_bdm_v2_properties_no_bootable_vol(self, mock_create):
        ...
        bdm_v2 = [{'swap_size': 10}]
        msg = 'Neither image nor bootable volume is specified for instance server_create_image_err'
        self._test_validate_bdm_v2(stack_name, bdm_v2, raise_exc=exception.StackValidationFailed, error_msg=msg, with_image=False)
    ...
    def test_validate_metadata_okay(self):
        stack_name = 'srv_val_metadata'
        tmpl, stack = self._setup_test_stack(stack_name)
        tmpl.t['Resources']['WebServer']['Properties']['metadata'] = {'a': 1, 'b': 2, 'c': 3}
        self.patchobject(nova.NovaClientPlugin, 'client', return_value=self.fc)
        resource_defns = tmpl.resource_definitions(stack)
        server = servers.Server('server_create_image_err', resource_defns['WebServer'], stack)
        self.patchobject(glance.GlanceClientPlugin, 'get_image', return_value=self.mock_image)
    ...
    def test_server_unsupported_microversion_tags(self):
        ...
        self.patchobject(nova.NovaClientPlugin, 'get_flavor', return_value=self.mock_flavor)
        exc = self.assertRaises(exception.StackValidationFailed, server.validate)
        self.assertEqual('Cannot use "tags" property - nova does not support required api microversion.', str(exc))
    ...
    def test_server_validate_too_many_personality(self):
        ...
        self.patchobject(nova.NovaClientPlugin, 'client', return_value=self.fc)
        self.patchobject(nova.NovaClientPlugin, 'is_version_supported', return_value=False)
        tmpl.t['Resources']['WebServer']['Properties']['personality'] = {'/fake/path1': 'fake contents1', '/fake/path2': 'fake_contents2', '/fake/path3': 'fake_contents3', '/fake/path4': 'fake_contents4', '/fake/path5': 'fake_contents5', '/fake/path6': 'fake_contents6'}
        resource_defns = tmpl.resource_definitions(stack)
        server = servers.Server('server_create_image_err', resource_defns['WebServer'], stack)
        self.patchobject(self.fc.limits, 'get', return_value=self.limits)
        self.patchobject(glance.GlanceClientPlugin, 'get_image', return_value=self.mock_image)
        exc = self.assertRaises(exception.StackValidationFailed, server.validate)
        self.assertEqual('The personality property may not contain greater than 5 entries.', str(exc))
    ...
    def test_server_validate_personality_unsupported(self):
        ...
        self.patchobject(nova.NovaClientPlugin, 'client', return_value=self.fc)
        self.patchobject(nova.NovaClientPlugin, 'is_version_supported', return_value=True)
        tmpl.t['Resources']['WebServer']['Properties']['personality'] = {'/fake/path1': 'fake contents1', '/fake/path2': 'fake_contents2', '/fake/path3': 'fake_contents3', '/fake/path4': 'fake_contents4', '/fake/path5': 'fake_contents5'}
        resource_defns = tmpl.resource_definitions(stack)
        server = servers.Server('server_create_image_err', resource_defns['WebServer'], stack)
        self.patchobject(self.fc.limits, 'get', return_value=self.limits)
        self.patchobject(glance.GlanceClientPlugin, 'get_image', return_value=self.mock_image)
        exc = self.assertRaises(exception.StackValidationFailed, server.validate)
        self.assertEqual('Cannot use the personality parameter as nova no longer supports it. Use user_data instead.', str(exc))
    ...
    def test_server_validate_personality_file_size_okay(self):
        ...
        self.patchobject(nova.NovaClientPlugin, 'client', return_value=self.fc)
        self.patchobject(nova.NovaClientPlugin, 'is_version_supported', return_value=False)
        tmpl.t['Resources']['WebServer']['Properties']['personality'] = {'/fake/path1': 'a' * 10240}
        resource_defns = tmpl.resource_definitions(stack)
        server = servers.Server('server_create_image_err', resource_defns['WebServer'], stack)
        self.patchobject(self.fc.limits, 'get', return_value=self.limits)
        self.patchobject(glance.GlanceClientPlugin, 'get_image', return_value=self.mock_image)
    ...
    def test_server_validate_personality_file_size_too_big(self):
        ...
        self.patchobject(nova.NovaClientPlugin, 'client', return_value=self.fc)
        self.patchobject(nova.NovaClientPlugin, 'is_version_supported', return_value=False)
        tmpl.t['Resources']['WebServer']['Properties']['personality'] = {'/fake/path1': 'a' * 10241}
        resource_defns = tmpl.resource_definitions(stack)
        server = servers.Server('server_create_image_err', resource_defns['WebServer'], stack)
        self.patchobject(self.fc.limits, 'get', return_value=self.limits)
        self.patchobject(glance.GlanceClientPlugin, 'get_image', return_value=self.mock_image)
        exc = self.assertRaises(exception.StackValidationFailed, server.validate)
        self.assertEqual('The contents of personality file "/fake/path1" is larger than the maximum allowed personality file size (10240 bytes).', str(exc))

    def test_server_validate_personality_get_attr_return_none(self):
        stack_name = 'srv_val'
        tmpl, stack = self._setup_test_stack(stack_name, server_with_sw_config_personality)
        self.patchobject(nova.NovaClientPlugin, 'client', return_value=self.fc)
        self.patchobject(nova.NovaClientPlugin, 'is_version_supported', return_value=False)
        resource_defns = tmpl.resource_definitions(stack)
        server = servers.Server('server_create_image_err', resource_defns['server'], stack)
        self.patchobject(self.fc.limits, 'get', return_value=self.limits)
        self.patchobject(glance.GlanceClientPlugin, 'get_image', return_value=self.mock_image)
        self.assertIsNone(server.validate())
    ...
    def test_resolve_attribute_server_not_found(self):
        return_server = self.fc.servers.list()[1]
        server = self._create_test_server(return_server, 'srv_resolve_attr')
        server.resource_id = '1234'
        self.patchobject(self.fc.servers, 'get', side_effect=fakes_nova.fake_exception())
    ...
    def test_resolve_attribute_console_url(self):
        ...
        self.patchobject(nova.NovaClientPlugin, 'client', return_value=self.fc)
        ws = servers.Server('WebServer', tmpl.resource_definitions(stack)['WebServer'], stack)
        ...
        supported_consoles = ('novnc', 'xvpvnc', 'spice-html5', 'rdp-html5', 'serial', 'webmks')
        self.assertEqual(set(supported_consoles), set(console_urls))
    ...
    def test_resolve_attribute_networks(self):
        ...
        self.patchobject(self.fc.servers, 'get', return_value=server)
        self.patchobject(neutron.NeutronClientPlugin, 'find_resourceid_by_name_or_id', return_value='fake_uuid')
        expect_networks = {'fake_uuid': ['10.0.0.3'], 'fake_net': ['10.0.0.3']}
        self.assertEqual(expect_networks, server._resolve_any_attribute('networks'))
    ...
    def create_old_net(self, port=None, net=None, ip=None, uuid=None, subnet=None, port_extra_properties=None, floating_ip=None, str_network=None, tag=None):
        return {'port': port, 'network': net, 'fixed_ip': ip, 'uuid': uuid, 'subnet': subnet, 'floating_ip': floating_ip, 'port_extra_properties': port_extra_properties, 'allocate_network': str_network, 'tag': tag}
    ...
    def test_exclude_not_updated_networks_success(self):
        ...
        server = self._create_test_server(return_server, 'networks_update')
        old_nets = [self.create_old_net(port='2a60cbaa-3d33-4af6-a9ce-83594ac546fc'), self.create_old_net(net='f3ef5d2f-d7ba-4b27-af66-58ca0b81e032', ip='1.2.3.4'), self.create_old_net(net='0da8adbf-a7e2-4c59-a511-96b03d2da0d7')]
        new_nets = [{'port': '2a60cbaa-3d33-4af6-a9ce-83594ac546fc'}, {'network': 'f3ef5d2f-d7ba-4b27-af66-58ca0b81e032', 'fixed_ip': '1.2.3.4'}, {'port': '952fd4ae-53b9-4b39-9e5f-8929c553b5ae'}]
    ...
    def test_exclude_not_updated_networks_nothing_for_update(self):
        return_server = self.fc.servers.list()[3]
        server = self._create_test_server(return_server, 'networks_update')
        old_nets = [self.create_old_net(net='f3ef5d2f-d7ba-4b27-af66-58ca0b81e032', ip='', port='')]
        new_nets = [{'network': 'f3ef5d2f-d7ba-4b27-af66-58ca0b81e032', 'fixed_ip': None, 'port': None, 'subnet': None, 'uuid': None, 'port_extra_properties': None, 'floating_ip': None, 'allocate_network': None, 'tag': None}]
        interfaces = [create_fake_iface(port='', net='f3ef5d2f-d7ba-4b27-af66-58ca0b81e032', ip='')]
        server._exclude_not_updated_networks(old_nets, new_nets, interfaces)
    ...
    def test_server_update_subnet_with_security_group(self):
        ...
        self.assertEqual(1, mock_detach_check.call_count)
        self.assertEqual(1, mock_attach_check.call_count)
        kwargs = {'network_id': '05d8e681-4b37-4570-bc8d-810089f706b2', 'fixed_ips': [{'subnet_id': '2a60cbaa-3d33-4af6-a9ce-83594ac546fc'}], 'security_groups': sec_uuids, 'name': 'update_subnet-port-0'}
        mock_create_port.assert_called_with({'port': kwargs})
    ...
    def test_server_update_empty_networks_with_complex_parameters(self):
        ...
        return_server.id = '9102'
        server = self._create_test_server(return_server, 'networks_update')
        new_networks = [{'network': 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa', 'fixed_ip': '1.2.3.4', 'port': '2a60cbaa-3d33-4af6-a9ce-83594ac546fc'}]
        update_props = self.server_props.copy()
        update_props['networks'] = new_networks
        update_template = server.t.freeze(properties=update_props)
        self.patchobject(self.fc.servers, 'get', return_value=return_server)
    ...
    def test_server_update_empty_networks_to_None(self):
        return_server = self.fc.servers.list()[3]
        return_server.id = '9102'
        server = self._create_test_server(return_server, 'networks_update', networks=[])
    ...
    def test_server_update_networks_with_complex_parameters(self):
        return_server = self.fc.servers.list()[1]
        return_server.id = '5678'
        old_networks = [{'port': '95e25541-d26a-478d-8f36-ae1c8f6b74dc'}, {'network': 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa', 'fixed_ip': '1.2.3.4'}, {'port': '4121f61a-1b2e-4ab0-901e-eade9b1cb09d'}, {'network': 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa', 'fixed_ip': '31.32.33.34'}]
        server = self._create_test_server(return_server, 'networks_update', networks=old_networks)
        new_networks = [{'network': 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa', 'fixed_ip': '1.2.3.4'}, {'port': '2a60cbaa-3d33-4af6-a9ce-83594ac546fc'}]
        update_props = copy.deepcopy(self.server_props)
        update_props['networks'] = new_networks
    ...
    def test_server_update_remove_network_non_empty(self):
        ...
        new_networks = [{'port': '95e25541-d26a-478d-8f36-ae1c8f6b74dc'}]
        server = self._create_test_server(return_server, 'networks_update', networks=old_networks)
        update_props = self.server_props.copy()
        update_props['networks'] = new_networks
        update_template = server.t.freeze(properties=update_props)
        self.patchobject(self.fc.servers, 'get', return_value=return_server)
        poor_interfaces = [create_fake_iface(port='95e25541-d26a-478d-8f36-ae1c8f6b74dc', net='aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa', ip='11.12.13.14'), create_fake_iface(port='4121f61a-1b2e-4ab0-901e-eade9b1cb09d', net='aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa', ip='21.22.23.24')]
        self.patchobject(return_server, 'interface_list', return_value=poor_interfaces)
        mock_detach = self.patchobject(return_server, 'interface_detach')
        mock_attach = self.patchobject(return_server, 'interface_attach')
        mock_detach_check = self.patchobject(nova.NovaClientPlugin, 'check_interface_detach', return_value=True)
    ...
    def test_server_snapshot(self):
        ...
        self.assertEqual({'snapshot_image_id': '456'}, resource_data_object.ResourceData.get_all(server))
    ...
    def test_server_check_snapshot_complete_with_not_complete_task_state(self):
        for task_state in {'image_uploading', 'image_snapshot_pending', 'image_snapshot', 'image_pending_upload'}:
            self._test_check_snapshot_complete_with_task_state(task_state=task_state)
    ...
    def _test_server_check_snapshot_complete(self, image_status='ERROR'):
        ...
        self.patchobject(glance.GlanceClientPlugin, 'get_image', return_value=image_in_error)
        self.assertRaises(exception.ResourceFailure, scheduler.TaskRunner(server.snapshot))
    ...
    def test_server_dont_validate_personality_if_personality_isnt_set(self):
        ...
        self.patchobject(nova.NovaClientPlugin, 'get_flavor', return_value=self.mock_flavor)
        self.patchobject(glance.GlanceClientPlugin, 'get_image', return_value=self.mock_image)
        mock_limits = self.patchobject(nova.NovaClientPlugin, 'absolute_limits')
    ...
    def test_server_validate_connection_error_retry_failure(self):
        ...
        resource_defns = tmpl.resource_definitions(stack)
        server = servers.Server('server_create_image_err', resource_defns['WebServer'], stack)
        self.patchobject(self.fc.limits, 'get', side_effect=[requests.ConnectionError(), requests.ConnectionError(), requests.ConnectionError()])
        self.patchobject(glance.GlanceClientPlugin, 'get_image', return_value=self.mock_image)
    ...
    def test_server_restore(self):
        ...
        mock_create = self.patchobject(self.fc.servers, 'create', return_value=return_server)
        self.patchobject(self.fc.servers, 'get', return_value=return_server)
        self.patchobject(neutron.NeutronClientPlugin, 'find_resourceid_by_name_or_id', return_value='aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa')
    ...
    def test_handle_delete_without_resource_id(self):
        t = template_format.parse(wp_template)
        tmpl = template.Template(t)
        stack = parser.Stack(utils.dummy_context(), 'without_resource_id', tmpl)
        rsrc = stack['WebServer']
        delete_server = self.patchobject(self.fc.servers, 'delete')
        self.patchobject(servers.Server, 'user_data_software_config', return_value=True)
        delete_internal_ports = self.patchobject(servers.Server, '_delete_internal_ports')
