...
class OcapiUtils(object):
    ...
    def get_request(self, uri):
        ...
        try:
            ...
            return {'ret': False, 'msg': "HTTP Error %s on GET request to '%s'" % (e.code, uri), 'status': e.code}
    ...
    def delete_request(self, uri, etag=None):
        ...
        username, password, basic_auth = self._auth_params()
        try:
            resp = open_url(uri, method='DELETE', headers=req_headers, url_username=username, url_password=password, force_basic_auth=basic_auth, validate_certs=False, follow_redirects='all', use_proxy=True, timeout=self.timeout)
            if resp.status != 204:
                data = json.loads(to_native(resp.read()))
            else:
                data = ''
            ...
            return {'ret': False, 'msg': "HTTP Error %s on DELETE request to '%s'" % (e.code, uri), 'status': e.code}
        except URLError as e:
    ...
    def put_request(self, uri, payload, etag=None):
        ...
        username, password, basic_auth = self._auth_params()
        try:
            resp = open_url(uri, data=json.dumps(payload), headers=req_headers, method='PUT', url_username=username, url_password=password, force_basic_auth=basic_auth, validate_certs=False, follow_redirects='all', use_proxy=True, timeout=self.timeout)
            headers = dict(((k.lower(), v) for k, v in resp.info().items()))
        except HTTPError as e:
            return {'ret': False, 'msg': "HTTP Error %s on PUT request to '%s'" % (e.code, uri), 'status': e.code}
        except URLError as e:
    ...
    def post_request(self, uri, payload, content_type='application/json', timeout=None):
        ...
        try:
            resp = open_url(uri, data=request_data, headers=req_headers, method='POST', url_username=username, url_password=password, force_basic_auth=basic_auth, validate_certs=False, follow_redirects='all', use_proxy=True, timeout=self.timeout if timeout is None else timeout)
            headers = dict(((k.lower(), v) for k, v in resp.info().items()))
        except HTTPError as e:
            return {'ret': False, 'msg': "HTTP Error %s on POST request to '%s'" % (e.code, uri), 'status': e.code}
        except URLError as e:
    ...
    def get_job_status(self, job_uri):
        ...
        if response['ret'] is False:
            if response.get('status') == 404:
                return {'ret': True, 'percentComplete': 0, 'operationStatus': 'Not Available', 'operationStatusId': 1, 'operationHealth': None, 'operationHealthId': None, 'details': 'Job does not exist.', 'jobExists': False}
            else:
                return response
        ...
        return_value = {'ret': True, 'percentComplete': response['data']['PercentComplete'], 'operationStatus': response['data']['Status']['State']['Name'], 'operationStatusId': response['data']['Status']['State']['ID'], 'operationHealth': health_list[0]['Name'] if len(health_list) > 0 else None, 'operationHealthId': health_list[0]['ID'] if len(health_list) > 0 else None, 'details': details, 'jobExists': True}
        return return_value
