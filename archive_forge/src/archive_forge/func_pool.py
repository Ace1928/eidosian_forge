...
def pool(input, window_shape, pooling_type, padding, dilation_rate=None, strides=None, name=None, data_format=None, dilations=None):
  ...
  """
    dilation_rate = deprecated_argument_lookup('dilations', dilations, 'dilation_rate', dilation_rate)
    with ops.name_scope(name, '%s_pool' % pooling_type.lower(), [input]) as scope:
        ...
        if num_spatial_dims < 1 or num_spatial_dims > 3:
            raise ValueError(f'`len(window_shape)` must be 1, 2, or 3. Received: window_shape={window_shape} of length {len(window_shape)}')
        ...
        if padding == 'SAME' and np.any(dilation_rate > 1):
            raise ValueError(f"pooling with 'SAME' padding is not implemented for `dilation_rate` > 1. Received: padding={padding} and dilation_rate={dilation_rate}")
        if np.any(strides > window_shape):
            raise ValueError(f'`strides` > `window_shape` not supported due to inconsistency between CPU and GPU implementations. Received: strides={strides} and window_shape={window_shape}')
        pooling_ops = {('MAX', 1): max_pool, ('MAX', 2): max_pool, ('MAX', 3): max_pool3d, ('AVG', 1): avg_pool, ('AVG', 2): avg_pool, ('AVG', 3): avg_pool3d}
        op_key = (pooling_type, num_spatial_dims)
        if op_key not in pooling_ops:
            raise ValueError(f'{num_spatial_dims}-D {pooling_type} pooling is not supported.')
        ...
        if num_spatial_dims == 1:
            if data_format is None or data_format == 'NWC':
                ...
                raise ValueError(f"data_format must be either 'NWC' or 'NCW'. Received: data_format={data_format}")
