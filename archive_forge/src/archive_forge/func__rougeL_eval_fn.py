...
def _rougeL_eval_fn(predictions, targets=None, metrics=None):
    if not _validate_text_data(targets, 'rougeL', targets_col_specifier) or not _validate_text_data(predictions, 'rougeL', predictions_col_specifier):
        return
    try:
        ...
        return
    scores = rouge.compute(predictions=predictions, references=targets, rouge_types=['rougeL'], use_aggregator=False)['rougeL']
