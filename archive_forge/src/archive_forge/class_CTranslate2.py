...
class CTranslate2(BaseLLM):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        try:
            ...
            raise ImportError('Could not import ctranslate2 python package. Please install it with `pip install ctranslate2`.')
        try:
            import transformers
        except ImportError:
            raise ImportError('Could not import transformers python package. Please install it with `pip install transformers`.')
        values['client'] = ctranslate2.Generator(model_path=values['model_path'], device=values['device'], device_index=values['device_index'], compute_type=values['compute_type'], **values['ctranslate2_kwargs'])
    ...
    def _default_params(self) -> Dict[str, Any]:
        ...
        return {'max_length': self.max_length, 'sampling_topk': self.sampling_topk, 'sampling_topp': self.sampling_topp, 'sampling_temperature': self.sampling_temperature}
