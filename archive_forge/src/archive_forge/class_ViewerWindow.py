...
class ViewerWindow(Tk_.Toplevel):
    ...
    def save_image(self):
        ...
        """
        view = self.view
        savefile = filedialog.asksaveasfile(parent=self, mode='wb', title='Save Image As PNG Image File', defaultextension='.png', filetypes=[('PNG image files', '*.png *.PNG', ''), ('All files', '')])
        if savefile:
