...
class UnsetBaremetalDeployTemplate(command.Command):
    ...
    def get_parser(self, prog_name):
        parser = super(UnsetBaremetalDeployTemplate, self).get_parser(prog_name)
        parser.add_argument('template', metavar='<template>', help=_('Name or UUID of the deploy template'))
        parser.add_argument('--extra', metavar='<key>', action='append', help=_('Extra to unset on this baremetal deploy template (repeat option to unset multiple extras).'))
        return parser
