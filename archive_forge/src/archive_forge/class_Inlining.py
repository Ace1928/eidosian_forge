...
class Inlining(Transformation):
    ...
    def visit_Call(self, node):
        ...
        if len(func_aliases) == 1:
            ...
            if isinstance(function_def, ast.FunctionDef) and function_def.name in self.inlinable:
                self.update = True
                to_inline = copy.deepcopy(self.inlinable[function_def.name])
                arg_to_value = dict()
                values = node.args
                values += to_inline.args.defaults[len(node.args) - len(to_inline.args.args):]
                for arg_fun, arg_call in zip(to_inline.args.args, values):
                    v_name = '__pythran_inline{}{}{}'.format(function_def.name, arg_fun.id, self.call_count)
                    new_var = ast.Name(id=v_name, ctx=ast.Store(), annotation=None, type_comment=None)
                    self.defs.append(ast.Assign(targets=[new_var], value=arg_call, type_comment=None))
                    arg_to_value[arg_fun.id] = ast.Name(id=v_name, ctx=ast.Load(), annotation=None, type_comment=None)
                self.call_count += 1
