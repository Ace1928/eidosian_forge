...
def test_chosen_angle_to_half_turns():
    ...
    np.testing.assert_allclose(cirq.chosen_angle_to_half_turns(rads=np.pi / 2), 0.5, atol=1e-08)
    np.testing.assert_allclose(cirq.chosen_angle_to_half_turns(rads=-np.pi / 4), -0.25, atol=1e-08)
