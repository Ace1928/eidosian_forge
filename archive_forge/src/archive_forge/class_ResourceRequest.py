...
class ResourceRequest:
    ...
    def __init__(self, bundles: List[Dict[str, Union[int, float]]], strategy: str='PACK', *args, **kwargs):
        if not bundles:
            raise ValueError('Cannot initialize a ResourceRequest with zero bundles.')
        self._bundles = [{k: float(v) for k, v in bundle.items() if v != 0} for bundle in bundles]
        if not self._bundles[0]:
            self._head_bundle_is_empty = True
            self._bundles.pop(0)
            if not self._bundles:
                raise ValueError('Cannot initialize a ResourceRequest with an empty head and zero worker bundles.')
        else:
    ...
    def __eq__(self, other: 'ResourceRequest'):
        return isinstance(other, ResourceRequest) and self._bound == other._bound and (self.head_bundle_is_empty == other.head_bundle_is_empty)

    def __hash__(self):
        if not self._hash:
            self._hash = hash(json.dumps({'args': self._bound.args, 'kwargs': self._bound.kwargs}, sort_keys=True, indent=0, ensure_ascii=True))
        return self._hash
    ...
    def __repr__(self) -> str:
        return f'<ResourceRequest (_bound={self._bound}, head_bundle_is_empty={self.head_bundle_is_empty})>'