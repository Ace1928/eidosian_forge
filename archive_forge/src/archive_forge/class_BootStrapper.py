...
class BootStrapper(WrapperMetric):
    ...
    def __init__(self, base_metric: Metric, num_bootstraps: int=10, mean: bool=True, std: bool=True, quantile: Optional[Union[float, Tensor]]=None, raw: bool=False, sampling_strategy: str='poisson', **kwargs: Any) -> None:
        ...
        if sampling_strategy not in allowed_sampling:
            raise ValueError(f'Expected argument ``sampling_strategy`` to be one of {allowed_sampling} but received {sampling_strategy}')
