...
class DecisionTree(Algorithm):
    def __init__(self, snake: Snake, network: NeuralNetwork, grid: List[List[Node]]):
        """
    ...
    def train(self, reward: float) -> None:
        ...
        self.network.backpropagation(np.array(input_data).reshape(1, -1), target_output)
    ...
    def load_model(self, file_path: str) -> None:
        ...
        with open(file_path, "r") as file:
            data = json.load(file)
        self.network.weights = [np.array(w) for w in data["weights"]]
        self.network.biases = [np.array(b) for b in data["biases"]]
