...
class TestRestJson(wsme.tests.protocol.RestOnlyProtocolTestCase):
    ...
    def call(self, fpath, _rt=None, _accept=None, _no_result_decode=False, body=None, **kw):
        ...
        if res.status_int == 200:
            ...
            raise wsme.tests.protocol.CallException(r['faultcode'], r['faultstring'], r.get('debuginfo'))
    ...
    def test_body_and_params(self):
        r = self.app.post('/argtypes/setint.json?value=2', '{"value": 2}', headers={'Content-Type': 'application/json'}, expect_errors=True)
        print(r)
        assert r.status_int == 400
        assert json.loads(r.text)['faultstring'] == 'Parameter value was given several times'
    ...
    def test_None_tojson(self):
        for dt in (datetime.date, datetime.time, datetime.datetime, decimal.Decimal):
            assert tojson(dt, None) is None

    def test_None_fromjson(self):
        for dt in (str, int, datetime.date, datetime.time, datetime.datetime, decimal.Decimal, [int], {int: int}):
    ...
    def test_body_arg(self):
        headers = {'Content-Type': 'application/json'}
        res = self.app.post('/crud/update_with_body?msg=hello', json.dumps(dict(id=1, name='test')), headers=headers, expect_errors=False)
        print('Received:', res.body)
        result = json.loads(res.text)
        print(result)
        assert result['data']['id'] == 1
