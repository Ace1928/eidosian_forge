...
class DataFrameWriter:
    ...
    def __init__(self, df: DataFrame, spark: t.Optional[SparkSession]=None, mode: t.Optional[str]=None, by_name: bool=False):
        self._df = df
    ...
    def copy(self, **kwargs) -> DataFrameWriter:
        return DataFrameWriter(**{k[1:] if k.startswith('_') else k: v for k, v in object_to_dict(self, **kwargs).items()})
    ...
    def insertInto(self, tableName: str, overwrite: t.Optional[bool]=None) -> DataFrameWriter:
        ...
        output_expression_container = exp.Insert(**{'this': exp.to_table(tableName), 'overwrite': overwrite})
        df = self._df.copy(output_expression_container=output_expression_container)
        if self._by_name:
            columns = sqlglot.schema.column_names(tableName, only_visible=True, dialect=SparkSession().dialect)
