...
def apply_adagrad(var: _atypes.TensorFuzzingAnnotation[TV_ApplyAdagrad_T], accum: _atypes.TensorFuzzingAnnotation[TV_ApplyAdagrad_T], lr: _atypes.TensorFuzzingAnnotation[TV_ApplyAdagrad_T], grad: _atypes.TensorFuzzingAnnotation[TV_ApplyAdagrad_T], use_locking: bool=False, update_slots: bool=True, name=None) -> _atypes.TensorFuzzingAnnotation[TV_ApplyAdagrad_T]:
  ...
  """
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('ApplyAdagrad', var=var, accum=accum, lr=lr, grad=grad, use_locking=use_locking, update_slots=update_slots, name=name)
    _result = _outputs[:]
    if _execute.must_record_gradient():
        ...
        _execute.record_gradient('ApplyAdagrad', _inputs_flat, _attrs, _result)
    _result, = _result
    return _result