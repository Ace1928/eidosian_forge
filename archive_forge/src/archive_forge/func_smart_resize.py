...
def smart_resize(x, size, interpolation='bilinear'):
    ...
    """
    if len(size) != 2:
        raise ValueError(f'Expected `size` to be a tuple of 2 integers, but got: {size}.')
    img = tf.convert_to_tensor(x)
    if img.shape.rank is not None:
        if img.shape.rank < 3 or img.shape.rank > 4:
            raise ValueError(f'Expected an image array with shape `(height, width, channels)`, or `(batch_size, height, width, channels)`, but got input with incorrect rank, of shape {img.shape}.')
    shape = tf.shape(img)
    ...
    crop_height = tf.cast(tf.cast(width * target_height, 'float32') / target_width, 'int32')
    crop_width = tf.cast(tf.cast(height * target_width, 'float32') / target_height, 'int32')
    crop_height = tf.minimum(height, crop_height)
    crop_width = tf.minimum(width, crop_width)
    crop_box_hstart = tf.cast(tf.cast(height - crop_height, 'float32') / 2, 'int32')
