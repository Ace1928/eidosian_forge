...
class VaultKeyManager(key_manager.KeyManager):
    ...
    def _get_resource_url(self, key_id=None):
        return '{}v1/{}/{}{}{}'.format(self._get_url(), self._kv_mountpoint, '' if self._kv_version == 1 else 'data/' if key_id else 'metadata/', self._kv_path + '/' if self._kv_path else '', key_id if key_id else '?list=true')
    ...
    def create_key_pair(self, context, algorithm, length, expiration=None, name=None):
        ...
        if algorithm.lower() != 'rsa':
            raise NotImplementedError('VaultKeyManager only implements rsa keys')
        priv_key = rsa.generate_private_key(public_exponent=65537, key_size=length, backend=default_backend())
        private_key = pri_key.PrivateKey('RSA', length, priv_key.private_bytes(Encoding.PEM, PrivateFormat.PKCS8, NoEncryption()))
        private_key_id = uuid.uuid4().hex
        private_id = self._store_key_value(private_key_id, private_key)
        public_key = pub_key.PublicKey('RSA', length, priv_key.public_key().public_bytes(Encoding.PEM, PublicFormat.SubjectPublicKeyInfo))
    ...
    def create_key(self, context, algorithm, length, name=None, **kwargs):
        ...
        key = sym_key.SymmetricKey(algorithm, length or 256, key_value, key_id, name or int(time.time()))
    ...
    def get(self, context, key_id, metadata_only=False):
        ...
        resp = self._do_http_request(self._session.get, self._get_resource_url(key_id))
        ...
        for type_clazz, type_name in self._secret_type_dict.items():
            if type_name == record['type']:
                clazz = type_clazz
        if clazz is None:
            raise exception.KeyManagerError('Unknown type : %r' % record['type'])
        if hasattr(clazz, 'algorithm') and hasattr(clazz, 'bit_length'):
            return clazz(record['algorithm'], record['bit_length'], key, record['name'], record['created'], key_id)
        else:
    ...
    def delete(self, context, key_id, force=False):
        ...
        if not key_id:
            raise exception.KeyManagerError('key identifier not provided')
        resp = self._do_http_request(self._session.delete, self._get_resource_url(key_id))
        if resp.status_code == requests.codes['not_found']:
            raise exception.ManagedObjectNotFoundError(uuid=key_id)

    def add_consumer(self, context, managed_object_id, consumer_data):
        raise NotImplementedError('VaultKeyManager does not implement adding consumers')

    def remove_consumer(self, context, managed_object_id, consumer_data):
        raise NotImplementedError('VaultKeyManager does not implement deleting consumers')
    ...
    def list(self, context, object_type=None, metadata_only=False):
        ...
        resp = self._do_http_request(self._session.get, self._get_resource_url())
        if resp.status_code == requests.codes['not_found']:
            keys = []
        else:
        ...
        for obj_id in keys:
            try:
                ...
                LOG.warning('Error occurred while retrieving object metadata, not adding it to the list: %s', e)
                pass
        return objects
