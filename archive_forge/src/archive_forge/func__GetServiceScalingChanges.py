...
def _GetServiceScalingChanges(args):
    ...
    if 'service_min_instances' in args and args.service_min_instances is not None:
        ...
        if scale_value.restore_default or scale_value.instance_count == 0:
            result.append(config_changes.DeleteAnnotationChange(service.SERVICE_MIN_SCALE_ANNOTATION))
        else:
            result.append(config_changes.SetAnnotationChange(service.SERVICE_MIN_SCALE_ANNOTATION, str(scale_value.instance_count)))
    return result