...
class TestSubnetPool(base.BaseFunctionalTest):
    ...
    def setUp(self):
        ...
        subnet_pool = self.user_cloud.network.create_subnet_pool(name=self.SUBNET_POOL_NAME, min_prefixlen=self.MINIMUM_PREFIX_LENGTH, default_prefixlen=self.DEFAULT_PREFIX_LENGTH, max_prefixlen=self.MAXIMUM_PREFIX_LENGTH, default_quota=self.DEFAULT_QUOTA, shared=self.IS_SHARED, prefixes=self.PREFIXES)
    ...
    def test_update(self):
        sot = self.user_cloud.network.update_subnet_pool(self.SUBNET_POOL_ID, name=self.SUBNET_POOL_NAME_UPDATED)
