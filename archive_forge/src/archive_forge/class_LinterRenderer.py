...
class LinterRenderer(text_renderer.TextRenderer):
    ...
    _HEADINGS_TO_LINT = ('NAME', 'EXAMPLES', 'DESCRIPTION', 'POSITIONAL ARGUMENTS', 'REQUIRED FLAGS', 'OPTIONAL FLAGS', 'FLAGS', 'LIST COMMAND FLAGS')
    _NAME_WORD_LIMIT = 20
    ...
    def Finish(self):
        ...
        for element in self.findings:
            if self.findings[element]:
                self._file_out.write(six.text_type(element) + ': ' + six.text_type(self.findings[element]) + '\n')
            else:
    ...
    def Example(self, line):
        if self.command_metadata and (not self.command_metadata.is_group) and (self._heading == 'EXAMPLES'):
