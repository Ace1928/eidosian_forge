...
def firewall_internet_service_list(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('firewall', 'internet-service-list', filtered_data, vdom=vdom)
        current_data = fos.get('firewall', 'internet-service-list', vdom=vdom, mkey=mkey)
    ...
    if state == 'present' or state is True:
        return fos.set('firewall', 'internet-service-list', data=filtered_data, vdom=vdom)
    elif state == 'absent':
        return fos.delete('firewall', 'internet-service-list', mkey=filtered_data['id'], vdom=vdom)
    else:
