...
class UCCSD(Operation):
    ...
    def __init__(self, weights, wires, s_wires=None, d_wires=None, init_state=None, id=None):
        if not s_wires and (not d_wires):
            raise ValueError(f's_wires and d_wires lists can not be both empty; got ph={s_wires}, pphh={d_wires}')
        for d_wires_ in d_wires:
            if len(d_wires_) != 2:
                raise ValueError(f'expected entries of d_wires to be of size 2; got {d_wires_} of length {len(d_wires_)}')
        shape = qml.math.shape(weights)
        if shape != (len(s_wires) + len(d_wires),):
            raise ValueError(f'Weights tensor must be of shape {(len(s_wires) + len(d_wires),)}; got {shape}.')
    ...
    @staticmethod
    def compute_decomposition(weights, wires, s_wires, d_wires, init_state):
