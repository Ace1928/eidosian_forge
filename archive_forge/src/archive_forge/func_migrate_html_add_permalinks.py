...
def migrate_html_add_permalinks(app: Sphinx, config: Config) -> None:
    ...
    if html_add_permalinks is UNSET:
        return
    logger.warning(__('html_add_permalinks has been deprecated since v3.5.0. Please use html_permalinks and html_permalinks_icon instead.'))
    if not html_add_permalinks:
        config.html_permalinks = False
        return
    config.html_permalinks_icon = html.escape(html_add_permalinks)