...
class DictHolder(ObjectHolder[T.Dict[str, TYPE_var]], IterableObject):
    ...
    def __init__(self, obj: T.Dict[str, TYPE_var], interpreter: 'Interpreter') -> None:
        ...
        self.methods.update({'has_key': self.has_key_method, 'keys': self.keys_method, 'get': self.get_method})
        self.trivial_operators.update({MesonOperator.PLUS: (dict, lambda x: {**self.held_object, **x}), MesonOperator.EQUALS: (dict, lambda x: self.held_object == x), MesonOperator.NOT_EQUALS: (dict, lambda x: self.held_object != x), MesonOperator.IN: (str, lambda x: x in self.held_object), MesonOperator.NOT_IN: (str, lambda x: x not in self.held_object)})
