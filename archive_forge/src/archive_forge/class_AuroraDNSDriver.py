...
class AuroraDNSDriver(DNSDriver):
    ...
    RECORD_TYPE_MAP = {RecordType.A: 'A', RecordType.AAAA: 'AAAA', RecordType.CNAME: 'CNAME', RecordType.MX: 'MX', RecordType.NS: 'NS', RecordType.SOA: 'SOA', RecordType.SRV: 'SRV', RecordType.TXT: 'TXT', RecordType.DS: 'DS', RecordType.PTR: 'PTR', RecordType.SSHFP: 'SSHFP', RecordType.TLSA: 'TLSA'}
    HEALTHCHECK_TYPE_MAP = {AuroraDNSHealthCheckType.HTTP: 'HTTP', AuroraDNSHealthCheckType.HTTPS: 'HTTPS', AuroraDNSHealthCheckType.TCP: 'TCP'}
    ...
    def create_record(self, name, zone, type, data, extra=None):
        ...
        self.connection.set_context({'resource': 'zone', 'id': zone.id})
        res = self.connection.request('/zones/%s/records' % zone.id, method='POST', data=json.dumps(rdata))
        record = res.parse_body()
        return self.__res_to_record(zone, record)
    ...
    def delete_record(self, record):
        ...
        self.connection.request('/zones/{}/records/{}'.format(record.zone.id, record.id), method='DELETE')
        return True
    ...
    def update_record(self, record, name, type, data, extra=None):
        ...
        rdata = self.__merge_extra_data(rdata, extra)
        self.connection.set_context({'resource': 'record', 'id': record.id})
        self.connection.request('/zones/{}/records/{}'.format(record.zone.id, record.id), method='PUT', data=json.dumps(rdata))
    ...
    def ex_update_healthcheck(self, healthcheck, type=None, hostname=None, ipaddress=None, port=None, path=None, interval=None, threshold=None, enabled=None, extra=None):
        """
        ...
        self.connection.request('/zones/{}/health_checks/{}'.format(healthcheck.zone.id, healthcheck.id), method='PUT', data=json.dumps(cdata))
