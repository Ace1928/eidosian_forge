...
@keras_tuner_export(['keras_tuner.HyperParameters', 'keras_tuner.engine.hyperparameters.HyperParameters'])
class HyperParameters:
    ...
    def is_active(self, hyperparameter):
        ...
        hp_name = str(hp)
        return any((self._conditions_are_active(temp_hp.conditions) for temp_hp in self._hps[hp_name]))
    ...
    def Choice(self, name, values, ordered=None, default=None, parent_name=None, parent_values=None):
        ...
        with self._maybe_conditional_scope(parent_name, parent_values):
            hp = hp_types.Choice(name=self._get_name(name), values=values, ordered=ordered, default=default, conditions=self._conditions)
    ...
    def Float(self, name, min_value, max_value, step=None, sampling='linear', default=None, parent_name=None, parent_values=None):
        ...
        """
        with self._maybe_conditional_scope(parent_name, parent_values):
            hp = hp_types.Float(name=self._get_name(name), min_value=min_value, max_value=max_value, step=step, sampling=sampling, default=default, conditions=self._conditions)
            return self._retrieve(hp)
    ...
    def Boolean(self, name, default=False, parent_name=None, parent_values=None):
        ...
        """
        with self._maybe_conditional_scope(parent_name, parent_values):
            hp = hp_types.Boolean(name=self._get_name(name), default=default, conditions=self._conditions)
            return self._retrieve(hp)
    ...
    def get_config(self):
        return {'space': [{'class_name': p.__class__.__name__, 'config': p.get_config()} for p in self.space], 'values': dict(self.values.items())}
    ...
    def from_proto(cls, proto):
        ...
        if isinstance(proto, protos.get_proto().HyperParameters.Values):
            space.extend((hp_types.Fixed(name, getattr(value, value.WhichOneof('kind'))) for name, value in proto.values.items()))
        else:
            space.extend((hp_types.Fixed.from_proto(fixed_proto) for fixed_proto in proto.space.fixed_space))
            space.extend((hp_types.Float.from_proto(float_proto) for float_proto in proto.space.float_space))
            space.extend((hp_types.Int.from_proto(int_proto) for int_proto in proto.space.int_space))
            space.extend((hp_types.Choice.from_proto(choice_proto) for choice_proto in proto.space.choice_space))
            space.extend((hp_types.Boolean.from_proto(boolean_proto) for boolean_proto in proto.space.boolean_space))
        hps.merge(space)
        if isinstance(proto, protos.get_proto().HyperParameters.Values):
            values = proto.values
        else:
    ...
    def to_proto(self):
        ...
        for name, value in self.values.items():
            ...
            values[name] = val
        return protos.get_proto().HyperParameters(space=protos.get_proto().HyperParameters.Space(fixed_space=fixed_space, float_space=float_space, int_space=int_space, choice_space=choice_space, boolean_space=boolean_space), values=protos.get_proto().HyperParameters.Values(values=values))
    ...
    def _validate_name(self, name):
        for condition in self._conditions:
            if condition.name == name:
                raise ValueError('A conditional `HyperParameter` cannot have the same name as its parent. Found: ' + str(name) + ' and parent_name: ' + str(condition.name))