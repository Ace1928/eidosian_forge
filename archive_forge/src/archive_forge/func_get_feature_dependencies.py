...
def get_feature_dependencies(model_dir):
    ...
    if model_info.flavors.get('python_function', {}).get('loader_module') == mlflow.models.model._DATABRICKS_FS_LOADER_MODULE:
        raise MlflowException('This model was packaged by Databricks Feature Store and can only be registered on a Databricks cluster.')
    return ''