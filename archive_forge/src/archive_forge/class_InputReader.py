...
class InputReader(metaclass=ABCMeta):
    ...
    def tf_input_ops(self, queue_size: int=1) -> Dict[str, TensorType]:
        ...
        if hasattr(self, '_queue_runner'):
            raise ValueError('A queue runner already exists for this input reader. You can only call tf_input_ops() once per reader.')
        logger.info('Reading initial batch of data from input reader.')
        batch = self.next()
        if isinstance(batch, MultiAgentBatch):
            raise NotImplementedError('tf_input_ops() is not implemented for multi agent batches')
        keys = [k for k in sorted(batch.keys()) if np.issubdtype(np.array(batch[k]).dtype, np.number)]
