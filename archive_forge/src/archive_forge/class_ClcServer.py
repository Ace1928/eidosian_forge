...
class ClcServer:
    ...
    def process_request(self):
        ...
        if state == 'absent':
            ...
            if not isinstance(server_ids, list):
                return self.module.fail_json(msg='server_ids needs to be a list of instances to delete: %s' % server_ids)
            changed, server_dict_array, new_server_ids = self._delete_servers(module=self.module, clc=self.clc, server_ids=server_ids)
        elif state in ('started', 'stopped'):
            server_ids = p.get('server_ids')
            if not isinstance(server_ids, list):
                return self.module.fail_json(msg='server_ids needs to be a list of servers to run: %s' % server_ids)
            changed, server_dict_array, new_server_ids = self._start_stop_servers(self.module, self.clc, server_ids)
        elif state == 'present':
            if not p.get('template') and p.get('type') != 'bareMetal':
                return self.module.fail_json(msg='template parameter is required for new instance')
            if p.get('exact_count') is None:
                server_dict_array, new_server_ids, partial_servers_ids, changed = self._create_servers(self.module, self.clc)
            else:
                server_dict_array, new_server_ids, partial_servers_ids, changed = self._enforce_count(self.module, self.clc)
        self.module.exit_json(changed=changed, server_ids=new_server_ids, partially_created_server_ids=partial_servers_ids, servers=server_dict_array)
    ...
    def _validate_name(module):
        ...
        state = module.params.get('state')
        if state == 'present' and (len(server_name) < 1 or len(server_name) > 6):
            module.fail_json(msg=str("When state = 'present', name must be a string with a minimum length of 1 and a maximum length of 6"))
    ...
    def _create_servers(self, module, clc, override_count=None):
        ...
        for i in range(0, count):
            if not module.check_mode:
                req = self._create_clc_server(clc=clc, module=module, server_params=params)
                server = req.requests[0].Server()
                request_list.append(req)
                servers.append(server)
        self._wait_for_requests(module, request_list)
        self._refresh_servers(module, servers)
        ip_failed_servers = self._add_public_ip_to_servers(module=module, should_add_public_ip=add_public_ip, servers=servers, public_ip_protocol=public_ip_protocol, public_ip_ports=public_ip_ports)
        ap_failed_servers = self._add_alert_policy_to_servers(clc=clc, module=module, servers=servers)
    ...
    def _add_alert_policy_to_servers(clc, module, servers):
        ...
        if alert_policy_id and (not module.check_mode):
            for server in servers:
                try:
                    ClcServer._add_alert_policy_to_server(clc=clc, alias=alias, server_id=server.id, alert_policy_id=alert_policy_id)
                except CLCException:
                    failed_servers.append(server)
        return failed_servers
    ...
    def _add_alert_policy_to_server(clc, alias, server_id, alert_policy_id):
        ...
        """
        try:
            clc.v2.API.Call(method='POST', url='servers/%s/%s/alertPolicies' % (alias, server_id), payload=json.dumps({'id': alert_policy_id}))
        except APIFailedResponse as e:
            raise CLCException('Failed to associate alert policy to the server : {0} with Error {1}'.format(server_id, str(e.response_text)))
    ...
    def _get_anti_affinity_policy_id(clc, module, alias, aa_policy_name):
        ...
        try:
            ...
            return module.fail_json(msg='Unable to fetch anti affinity policies for account: {0}. {1}'.format(alias, ex.response_text))
