...
class APIVersion(object):
    ...
    def __init__(self, version_str=None):
        ...
        if version_str is not None:
            match = re.match('^([1-9]\\d*)\\.([1-9]\\d*|0|latest)$', version_str)
            if match:
                ...
                msg = _("Invalid format of client version '%s'. Expected format 'X.Y', where X is a major part and Y is a minor part of version.") % version_str
    ...
    def __str__(self):
        ...
        return 'API Version Major: %s, Minor: %s' % (self.ver_major, self.ver_minor)
    ...
    def __lt__(self, other):
        if not isinstance(other, APIVersion):
            raise TypeError(_type_error_msg % {'other': other, 'cls': self.__class__})
    ...
    def get_string(self):
        ...
        """
        if self.is_null():
            raise ValueError(_('Null APIVersion cannot be converted to string.'))
        elif self.is_latest():
