...
class UniqueOperationNames(ValidationRule):
    ...
    def enter_OperationDefinition(self, node, key, parent, path, ancestors):
        ...
        if operation_name.value in self.known_operation_names:
            self.context.report_error(GraphQLError(self.duplicate_operation_name_message(operation_name.value), [self.known_operation_names[operation_name.value], operation_name]))
        else:
