...
class CinderQuotaTest(common.HeatTestCase):
    ...
    def setUp(self):
        ...
        self.stack = parser.Stack(self.ctx, 'cinder_quota_test_stack', template.Template(tpl))
        ...
        self.err_msg = 'Invalid quota %(property)s value(s): %(value)s. Can not be less than the current usage value(s): %(total)s.'
    ...
    def test_miss_all_quotas(self):
        ...
        my_quota.t = my_quota.t.freeze(properties=props)
        my_quota.reparse()
        msg = 'At least one of the following properties must be specified: gigabytes, snapshots, volumes.'
    ...
    def test_quota_handle_create(self):
        ...
        self.quotas.update.assert_called_once_with('some_project_id', gigabytes=5, snapshots=2, volumes=3)
    ...
    def test_quota_handle_update(self):
        ...
        props = {'project': 'some_project_id', 'gigabytes': 6, 'volumes': 4}
        json_snippet = rsrc_defn.ResourceDefinition(self.my_quota.name, 'OS::Cinder::Quota', properties=props)
        self.my_quota.reparse()
        self.my_quota.handle_update(json_snippet, tmpl_diff, prop_diff)
        self.quotas.update.assert_called_once_with('some_project_id', gigabytes=6, volumes=4)
    ...
    def test_quota_with_invalid_volumes(self):
        ...
        self.my_quota.reparse()
        err = self.assertRaises(ValueError, self.my_quota.handle_create)
        self.assertEqual(self.err_msg % {'property': 'volumes', 'value': 3, 'total': 4}, str(err))

    def test_quota_with_invalid_snapshots(self):
        fake_v = self.fv(0)
        self.volume_snapshots.list.return_value = [fake_v, fake_v, fake_v, fake_v]
        self.my_quota.physical_resource_name = mock.MagicMock(return_value='some_resource_id')
        self.my_quota.reparse()
        err = self.assertRaises(ValueError, self.my_quota.handle_create)
        self.assertEqual(self.err_msg % {'property': 'snapshots', 'value': 2, 'total': 4}, str(err))
