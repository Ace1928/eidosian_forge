...
class Test_dhcp_offer(unittest.TestCase):
    ...
    sname = 'abc'
    boot_file = ''
    option_list = [dhcp.option(dhcp.DHCP_MESSAGE_TYPE_OPT, b'\x02', 1), dhcp.option(dhcp.DHCP_SUBNET_MASK_OPT, b'\xff\xff\xff\x00', 4), dhcp.option(dhcp.DHCP_GATEWAY_ADDR_OPT, b'\xc0\xa8\n\t', 4), dhcp.option(dhcp.DHCP_DNS_SERVER_ADDR_OPT, b'\xc0\xa8\n\t', 4), dhcp.option(dhcp.DHCP_IP_ADDR_LEASE_TIME_OPT, b'\x00\x03\xf4\x80', 4), dhcp.option(dhcp.DHCP_RENEWAL_TIME_OPT, b'\x00\x01\xfa@', 4), dhcp.option(dhcp.DHCP_REBINDING_TIME_OPT, b'\x00\x03u\xf0', 4), dhcp.option(dhcp.DHCP_SERVER_IDENTIFIER_OPT, b'\xc0\xa8\n\t', 4)]
    magic_cookie = '99.130.83.99'
    options = dhcp.options(option_list=option_list, options_len=50, magic_cookie=magic_cookie)
    dh = dhcp.dhcp(op, chaddr, options, htype=htype, hlen=hlen, hops=hops, xid=xid, secs=secs, flags=flags, ciaddr=ciaddr, yiaddr=yiaddr, siaddr=siaddr, giaddr=giaddr, sname=sname, boot_file=boot_file)
    buf = b'\x02\x01\x06\x00\x00\x00\x00\x01\x00\x00\x00\x01\xc0\xa8\n\n\xc0\xa8\x14\x14\xc0\xa8\x1e\x1e\xc0\xa8((\xaa\xaa\xaa\xaa\xaa\xaa\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00abcc\x82Sc5\x01\x02\x01\x04\xff\xff\xff\x00\x03\x04\xc0\xa8\n\t\x06\x04\xc0\xa8\n\t3\x04\x00\x03\xf4\x80:\x04\x00\x01\xfa@;\x04\x00\x03u\xf06\x04\xc0\xa8\n\t\xff'
    ...
    def test_serialize(self):
        ...
        res = struct.unpack_from(dhcp.dhcp._DHCP_PACK_STR, bytes(buf))
        ...
        options = dhcp.options.parser(buf[struct.calcsize(dhcp.dhcp._DHCP_PACK_STR):])
    ...
    def test_to_string(self):
        ...
        for option in self.option_list:
            _opt_str = ','.join(['%s=%s' % (k, repr(getattr(option, k))) for k, v in inspect.getmembers(option) if k in option_values])
            opt_str = '%s(%s)' % (dhcp.option.__name__, _opt_str)
            opt_str_list.append(opt_str)
        option_str = '[%s]' % ', '.join(opt_str_list)
        opts_vals = {'magic_cookie': repr(self.magic_cookie), 'option_list': option_str, 'options_len': repr(self.options.options_len)}
        _options_str = ','.join(['%s=%s' % (k, opts_vals[k]) for k, v in inspect.getmembers(self.options) if k in opts_vals])
        options_str = '%s(%s)' % (dhcp.options.__name__, _options_str)
        dhcp_values = {'op': repr(self.op), 'htype': repr(self.htype), 'hlen': repr(self.hlen), 'hops': repr(self.hops), 'xid': repr(self.xid), 'secs': repr(self.secs), 'flags': repr(self.flags), 'ciaddr': repr(self.ciaddr), 'yiaddr': repr(self.yiaddr), 'siaddr': repr(self.siaddr), 'giaddr': repr(self.giaddr), 'chaddr': repr(self.chaddr), 'sname': repr(self.sname), 'boot_file': repr(self.boot_file), 'options': options_str}
        _dh_str = ','.join(['%s=%s' % (k, dhcp_values[k]) for k, v in inspect.getmembers(self.dh) if k in dhcp_values])
