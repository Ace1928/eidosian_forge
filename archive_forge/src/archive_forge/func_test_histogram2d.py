...
def test_histogram2d(weights, density, bins):
    ...
    a3, b3x, b3y = np.histogram2d(x.compute(), y.compute(), bins=b, range=r, density=density, weights=w.compute() if weights else None)
    assert_eq(a1, a2)
    assert_eq(a1, a3)
    if not (weights or density):
        assert a1.sum() == n
        assert a2.sum() == n
    assert same_keys(da.histogram2d(x, y, bins=b, range=r, density=density, weights=w)[0], a1)
