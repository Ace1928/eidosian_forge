...
class SenlinNodeTest(common.HeatTestCase):
    ...
    def setUp(self):
        ...
        self.senlin_mock.get_cluster.return_value = mock.Mock(id='fake_cluster_id')
    ...
    def _create_node(self):
        ...
        self.assertEqual((self.node.CREATE, self.node.COMPLETE), self.node.state)
    ...
    def test_cluster_resolve_attribute(self):
        excepted_show = {'id': 'some_id', 'status': 'ACTIVE', 'status_reason': 'Unknown', 'name': 'SenlinNode', 'metadata': {'foo': 'bar'}, 'profile_id': 'fake_profile_id', 'cluster_id': 'fake_cluster_id'}
        node = self._create_node()
        self.assertEqual(excepted_show, node._show_resource())
        self.assertEqual(self.fake_node.details, node._resolve_attribute('details'))
        self.senlin_mock.get_node.assert_called_with(node.resource_id, details=True)
