...
class CreateBackup(base.CreateCommand):
    ...
    """
        ...
        parser.add_argument('--description', help='A friendly description of the backup.')
        parser.add_argument('--location', help='Choose where to store your backup. Backups are stored in the closest multi-region location to you by default. Only customize if needed.')
    ...
    """
        ...
        if args.project_level:
            result_operation = sql_client.backups.CreateBackup(sql_messages.SqlBackupsCreateBackupRequest(parent='projects/' + instance_ref.project, backup=sql_messages.Backup(description=args.description, instance=instance_ref.instance, location=args.location, kind='sql#backup')))
        else:
            result_operation = sql_client.backupRuns.Insert(sql_messages.SqlBackupRunsInsertRequest(project=instance_ref.project, instance=instance_ref.instance, backupRun=sql_messages.BackupRun(description=args.description, instance=instance_ref.instance, location=args.location, kind='sql#backupRun')))
        ...
        operations.OperationsV1Beta4.WaitForOperation(sql_client, operation_ref, 'Backing up Cloud SQL instance')
