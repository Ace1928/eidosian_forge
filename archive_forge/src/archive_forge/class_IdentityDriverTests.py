...
class IdentityDriverTests(object):
    ...
    def test_delete_user(self):
        ...
        self.assertRaises(exception.UserNotFound, self.driver.get_user, user['id'])

    def test_delete_user_no_user_exc(self):
        self.assertRaises(exception.UserNotFound, self.driver.delete_user, user_id=uuid.uuid4().hex)
    ...
    def test_delete_group(self):
        ...
        self.assertRaises(exception.GroupNotFound, self.driver.get_group, group['id'])

    def test_delete_group_doesnt_exist_exc(self):
        self.assertRaises(exception.GroupNotFound, self.driver.delete_group, group_id=uuid.uuid4().hex)
    ...
    def test_remove_user_from_group_not_in_group(self):
        user = self.create_user()
        group = self.create_group()
        self.assertRaises(exception.NotFound, self.driver.remove_user_from_group, user['id'], group['id'])

    def test_remove_user_from_group_no_user(self):
        group = self.create_group()
        user_id = uuid.uuid4().hex
        self.assertRaises(exception.UserNotFound, self.driver.remove_user_from_group, user_id, group['id'])

    def test_remove_user_from_group_no_group(self):
        user = self.create_user()
        group_id = uuid.uuid4().hex
        self.assertRaises(exception.GroupNotFound, self.driver.remove_user_from_group, user['id'], group_id)
