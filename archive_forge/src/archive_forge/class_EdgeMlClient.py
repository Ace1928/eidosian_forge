...
class EdgeMlClient(object):
  ...
  """
    ...
    """
        ...
        analyze_req = self.messages.EdgemlProjectsModelsAnalyzeRequest(analyzeModelRequest=self.messages.AnalyzeModelRequest(gcsSource=self.messages.GcsSource(inputUris=[source])), project=project)
        operation = self._service.Analyze(analyze_req)
        result = self._operations_client.WaitForOperation(operation)
        response = encoding_helper.JsonToMessage(self.messages.AnalyzeModelResponse, encoding_helper.MessageToJson(result))
        return response
    ...
    """
        ...
        input_config = self.messages.InputConfig(gcsSource=self.messages.GcsSource(inputUris=[source]))
        output_config = self.messages.OutputConfig(gcsDestination=self.messages.GcsDestination(outputUri=destination))
        compile_req = self.messages.EdgemlProjectsModelsCompileRequest(compileModelRequest=compile_req_type(chipType=chip_type, inputConfig=input_config, outputConfig=output_config), project=project)
        operation = self._service.Compile(compile_req)
        result = self._operations_client.WaitForOperation(operation)
        response = encoding_helper.JsonToMessage(self.messages.CompileModelResponse, encoding_helper.MessageToJson(result))
    ...
    """
        ...
        input_config = self.messages.InputConfig(gcsSource=self.messages.GcsSource(inputUris=[source]))
        output_config = self.messages.OutputConfig(gcsDestination=self.messages.GcsDestination(outputUri=destination))
        convert_req = self.messages.EdgemlProjectsModelsConvertRequest(convertModelRequest=self.messages.ConvertModelRequest(inputConfig=input_config, outputConfig=output_config), project=project)
        operation = self._service.Convert(convert_req)
        result = self._operations_client.WaitForOperation(operation)
        response = encoding_helper.JsonToMessage(self.messages.ConvertModelResponse, encoding_helper.MessageToJson(result))
        return (response, destination)