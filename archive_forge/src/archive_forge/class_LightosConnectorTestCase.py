...
class LightosConnectorTestCase(test_connector.ConnectorTestCase):
    ...
    def setUp(self):
        ...
        self.connector = lightos.LightOSConnector(None, execute=self.fake_execute)

    @staticmethod
    def _get_connection_info():
        lightos_nodes = {}
        for ip in FAKE_LIGHTOS_CLUSTER_INFO['nodes_ips']:
            lightos_nodes[ip] = dict(transport_type='tcp', target_portal=ip, target_port=8009)
        return dict(subsysnqn=FAKE_LIGHTOS_CLUSTER_INFO['subsystemNQN'], uuid=FAKE_LIGHTOS_CLUSTER_INFO['UUID'], lightos_nodes=lightos_nodes)

    @mock.patch.object(utils, 'get_host_nqn', return_value=FAKE_NQN)
    @mock.patch.object(lightos.LightOSConnector, 'find_dsc', return_value=True)
    ...
    @mock.patch.object(lightos.http.client.HTTPConnection, 'request', return_value=None)
    @mock.patch.object(lightos.http.client.HTTPConnection, 'getresponse', return_value=get_http_response_mock(http.client.NOT_FOUND))
    ...
    @mock.patch.object(lightos.LightOSConnector, '_check_device_exists_using_dev_lnk', side_effect=[None, False, '/dev/nvme0n1'])
    @mock.patch.object(lightos.LightOSConnector, '_check_device_exists_reading_block_class', side_effect=[None, False, '/dev/nvme0n1'])
    def test_get_device_by_uuid_many_attempts(self, execute_mock, glob_mock):
        self.assertEqual(self.connector._get_device_by_uuid(FAKE_VOLUME_UUID), '/dev/nvme0n1')

    @mock.patch.object(lightos.LightOSConnector, 'dsc_connect_volume', return_value=None)
    @mock.patch.object(lightos.LightOSConnector, '_get_device_by_uuid', return_value='/dev/nvme0n1')
    def test_connect_volume(self, dsc_connect, path):
        connection_properties = {'nqn': FAKE_NQN, 'found_dsc': True, 'uuid': '123'}
    ...
    def test_extend_volume(self, mock_execute, m_open):
        ...
        self.assertEqual(self.connector.extend_volume(connection_properties), NUM_BLOCKS_IN_GIB * BLOCK_SIZE)
