...
def parallel_interleave_dataset_v4(input_dataset: _atypes.TensorFuzzingAnnotation[_atypes.Variant], other_arguments, cycle_length: _atypes.TensorFuzzingAnnotation[_atypes.Int64], block_length: _atypes.TensorFuzzingAnnotation[_atypes.Int64], buffer_output_elements: _atypes.TensorFuzzingAnnotation[_atypes.Int64], prefetch_input_elements: _atypes.TensorFuzzingAnnotation[_atypes.Int64], num_parallel_calls: _atypes.TensorFuzzingAnnotation[_atypes.Int64], f, output_types, output_shapes, deterministic: str='default', metadata: str='', name=None) -> _atypes.TensorFuzzingAnnotation[_atypes.Variant]:
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'ParallelInterleaveDatasetV4', name, input_dataset, other_arguments, cycle_length, block_length, buffer_output_elements, prefetch_input_elements, num_parallel_calls, 'f', f, 'deterministic', deterministic, 'output_types', output_types, 'output_shapes', output_shapes, 'metadata', metadata)
            return _result
            ...
            pass
        try:
            return parallel_interleave_dataset_v4_eager_fallback(input_dataset, other_arguments, cycle_length, block_length, buffer_output_elements, prefetch_input_elements, num_parallel_calls, f=f, deterministic=deterministic, output_types=output_types, output_shapes=output_shapes, metadata=metadata, name=name, ctx=_ctx)
        except _core._SymbolicException:
            pass
    if not isinstance(output_types, (list, tuple)):
        raise TypeError("Expected list for 'output_types' argument to 'parallel_interleave_dataset_v4' Op, not %r." % output_types)
    output_types = [_execute.make_type(_t, 'output_types') for _t in output_types]
    if not isinstance(output_shapes, (list, tuple)):
        raise TypeError("Expected list for 'output_shapes' argument to 'parallel_interleave_dataset_v4' Op, not %r." % output_shapes)
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('ParallelInterleaveDatasetV4', input_dataset=input_dataset, other_arguments=other_arguments, cycle_length=cycle_length, block_length=block_length, buffer_output_elements=buffer_output_elements, prefetch_input_elements=prefetch_input_elements, num_parallel_calls=num_parallel_calls, f=f, output_types=output_types, output_shapes=output_shapes, deterministic=deterministic, metadata=metadata, name=name)
    _result = _outputs[:]
    if _execute.must_record_gradient():
        ...
        _execute.record_gradient('ParallelInterleaveDatasetV4', _inputs_flat, _attrs, _result)
    _result, = _result
    return _result