...
def experimental_parse_example_dataset_eager_fallback(input_dataset: _atypes.TensorFuzzingAnnotation[_atypes.Variant], num_parallel_calls: _atypes.TensorFuzzingAnnotation[_atypes.Int64], dense_defaults, sparse_keys, dense_keys, sparse_types, dense_shapes, output_types, output_shapes, sloppy: bool, name, ctx) -> _atypes.TensorFuzzingAnnotation[_atypes.Variant]:
    ...
    _attrs = ('sparse_keys', sparse_keys, 'dense_keys', dense_keys, 'sparse_types', sparse_types, 'Tdense', _attr_Tdense, 'dense_shapes', dense_shapes, 'output_types', output_types, 'output_shapes', output_shapes, 'sloppy', sloppy)
    _result = _execute.execute(b'ExperimentalParseExampleDataset', 1, inputs=_inputs_flat, attrs=_attrs, ctx=ctx, name=name)
