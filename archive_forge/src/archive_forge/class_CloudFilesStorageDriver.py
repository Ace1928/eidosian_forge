...
class CloudFilesStorageDriver(StorageDriver, OpenStackDriverMixin):
    ...
    def __init__(self, key, secret=None, secure=True, host=None, port=None, region='ord', use_internal_url=False, **kwargs):
        """
    ...
    def get_object(self, container_name, object_name):
        ...
        response = self.connection.request('/{}/{}'.format(container_name_encoded, object_name_encoded), method='HEAD')
    ...
    def get_container_cdn_url(self, container, ex_ssl_uri=False):
        ...
        response = self.connection.request('/%s' % container_name_encoded, method='HEAD', cdn_request=True)
        if response.status == httplib.NO_CONTENT:
            if ex_ssl_uri:
    ...
    def enable_container_cdn(self, container, ex_ttl=None):
        ...
        response = self.connection.request('/%s' % container_name, method='PUT', headers=headers, cdn_request=True)
    ...
    def download_object(self, obj, destination_path, overwrite_existing=False, delete_on_failure=True):
        ...
        response = self.connection.request('/{}/{}'.format(container_name, object_name), method='GET', raw=True)
    ...
    def download_object_range(self, obj, destination_path, start_bytes, end_bytes=None, overwrite_existing=False, delete_on_failure=True):
        ...
        response = self.connection.request('/{}/{}'.format(container_name, object_name), method='GET', headers=headers, raw=True)
    ...
    def download_object_range_as_stream(self, obj, start_bytes, end_bytes=None, chunk_size=None):
        ...
        object_name = obj.name
        headers = {'Range': self._get_standard_range_str(start_bytes, end_bytes)}
        response = self.connection.request('/{}/{}'.format(container_name, object_name), headers=headers, method='GET', raw=True)
    ...
    def upload_object(self, file_path, container, object_name, extra=None, verify_hash=True, headers=None):
        ...
        """
        return self._put_object(container=container, object_name=object_name, extra=extra, file_path=file_path, verify_hash=verify_hash, headers=headers)

    def upload_object_via_stream(self, iterator, container, object_name, extra=None, headers=None):
        if isinstance(iterator, file):
            iterator = iter(iterator)
        return self._put_object(container=container, object_name=object_name, extra=extra, stream=iterator, headers=headers)

    def delete_object(self, obj):
        container_name = self._encode_container_name(obj.container.name)
        object_name = self._encode_object_name(obj.name)
        response = self.connection.request('/{}/{}'.format(container_name, object_name), method='DELETE')
        if response.status == httplib.NO_CONTENT:
            return True
        elif response.status == httplib.NOT_FOUND:
    ...
    def ex_get_meta_data(self):
        ...
        if response.status == httplib.NO_CONTENT:
            ...
            return {'container_count': int(container_count), 'object_count': int(object_count), 'bytes_used': int(bytes_used), 'temp_url_key': temp_url_key}
        raise LibcloudError('Unexpected status code: %s' % response.status)
    ...
    def _put_object(self, container, object_name, extra=None, file_path=None, stream=None, verify_hash=True, headers=None):
        extra = extra or {}
        container_name_encoded = self._encode_container_name(container.name)
        object_name_encoded = self._encode_object_name(object_name)
        ...
        request_path = '/{}/{}'.format(container_name_encoded, object_name_encoded)
        result_dict = self._upload_object(object_name=object_name, content_type=content_type, request_path=request_path, request_method='PUT', headers=headers, file_path=file_path, stream=stream)
        ...
        if response.status == httplib.EXPECTATION_FAILED:
            ...
            raise ObjectHashMismatchError(value=('MD5 hash checksum does not match (expected=%s, ' + 'actual=%s)') % (result_dict['data_hash'], server_hash), object_name=object_name, driver=self)
        elif response.status == httplib.CREATED:
            obj = Object(name=object_name, size=bytes_transferred, hash=server_hash, extra=None, meta_data=meta_data, container=container, driver=self)
            return obj
        else:
    ...
    def _to_container_list(self, response):
        for container in response:
            extra = {'object_count': int(container['count']), 'size': int(container['bytes'])}
    ...
    def _to_object_list(self, response, container):
        ...
        for obj in response:
            ...
            size = int(obj['bytes'])
            hash = obj['hash']
            extra = {'content_type': obj['content_type'], 'last_modified': obj['last_modified']}
    ...
    def _headers_to_object(self, name, container, headers):
        ...
        obj = Object(name=name, size=size, hash=etag, extra=extra, meta_data=meta_data, container=container, driver=self)
        return obj
