...
class FFI(object):
        ...
        if backend is None:
            ...
            if backend.__version__ != __version__:
                if hasattr(backend, '__file__'):
                    raise Exception("Version mismatch: this is the 'cffi' package version %s, located in %r.  When we import the top-level '_cffi_backend' extension module, we get version %s, located in %r.  The two versions should be equal; check your installation." % (__version__, __file__, backend.__version__, backend.__file__))
                else:
                    raise Exception("Version mismatch: this is the 'cffi' package version %s, located in %r.  This interpreter comes with a built-in '_cffi_backend' module, which is version %s.  The two versions should be equal; check your installation." % (__version__, __file__, backend.__version__))
        from . import cparser
    ...
    def include(self, ffi_to_include):
        ...
        if not isinstance(ffi_to_include, FFI):
            raise TypeError('ffi.include() expects an argument that is also of type cffi.FFI, not %r' % (type(ffi_to_include).__name__,))
    ...
    def distutils_extension(self, tmpdir='build', verbose=True):
        ...
        mkpath(tmpdir)
        ext, updated = recompile(self, module_name, source, tmpdir=tmpdir, extradir=tmpdir, source_extension=source_extension, call_c_compiler=False, **kwds)
        if verbose:
            if updated:
    ...
    def compile(self, tmpdir='.', verbose=0, target=None, debug=None):
        ...
        module_name, source, source_extension, kwds = self._assigned_source
        return recompile(self, module_name, source, tmpdir=tmpdir, target=target, source_extension=source_extension, compiler_verbose=verbose, debug=debug, **kwds)
