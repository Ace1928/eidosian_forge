...
def export_gradient_scalar_add_and_mul() -> None:
    ...
    gradient_node = onnx.helper.make_node('Gradient', ['a', 'b'], ['dd_da', 'dd_db'], name='my_gradient', domain=AI_ONNX_PREVIEW_TRAINING_DOMAIN, xs=['a', 'b'], y='d')
    ...
    expect(model, inputs=[a, b], outputs=[d, dd_da, dd_db], name='test_gradient_of_add_and_mul')