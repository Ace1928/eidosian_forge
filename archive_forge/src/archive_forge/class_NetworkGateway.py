...
class NetworkGateway(neutron.NeutronResource):
    ...
    def validate(self):
        ...
        for connection in connections:
            ...
            if segmentation_type == 'flat' and segmentation_id:
                msg = _('segmentation_id cannot be specified except 0 for using flat')
                raise exception.StackValidationFailed(message=msg)

    def handle_create(self):
        props = self.prepare_properties(self.properties, self.physical_resource_name())
        connections = props.pop(self.CONNECTIONS)
        ret = self.client().create_network_gateway({'network_gateway': props})['network_gateway']
    ...
    def handle_delete(self):
        ...
        for connection in connections:
            with self.client_plugin().ignore_not_found:
                if self.NETWORK in connection:
                    connection['network_id'] = connection.pop(self.NETWORK)
                self.client().disconnect_network_gateway(self.resource_id, connection)
        try:
    ...
    def handle_update(self, json_snippet, tmpl_diff, prop_diff):
        ...
        if prop_diff:
            ...
            self.client().update_network_gateway(self.resource_id, {'network_gateway': prop_diff})
        if connections:
            ...
            for connection in connections:
                if self.NETWORK in connection:
                    connection['network_id'] = connection.pop(self.NETWORK)
                self.client().connect_network_gateway(self.resource_id, connection)