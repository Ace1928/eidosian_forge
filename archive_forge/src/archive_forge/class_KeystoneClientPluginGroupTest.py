...
class KeystoneClientPluginGroupTest(common.HeatTestCase):
    ...
    def test_get_group_id_with_name(self, client_keystone):
        ...
        self._client.client.groups.list.return_value = [self._get_mock_group()]
        client_keystone.return_value = self._client
        client_plugin = keystone.KeystoneClientPlugin(context=mock.MagicMock())
        self.assertEqual(self.sample_uuid, client_plugin.get_group_id(self.sample_name))
    ...
    def test_get_group_id_with_name_and_domain(self, client_keystone):
        ...
        client_keystone.return_value = self._client
        client_plugin = keystone.KeystoneClientPlugin(context=mock.MagicMock())
        self.assertEqual(self.sample_uuid, client_plugin.get_group_id(self.sample_name_and_domain))
        self.assertRaises(keystone_exceptions.NotFound, self._client.client.groups.get, self.sample_name)
        self._client.client.groups.list.assert_called_once_with(domain=client_plugin.get_domain_id(self.sample_domain_uuid), name=self.sample_name)
    ...
    def test_get_group_id_with_domain_not_found(self, client_keystone):
        ...
        client_keystone.return_value = self._client
        client_plugin = keystone.KeystoneClientPlugin(context=mock.MagicMock())
        ex = self.assertRaises(exception.EntityNotFound, client_plugin.get_group_id, self.sample_name_and_domain)
        msg = 'The KeystoneGroup (%(name)s) could not be found.' % {'name': self.sample_name}
        self.assertEqual(msg, str(ex))
        self.assertRaises(keystone_exceptions.NotFound, self._client.client.groups.get, self.sample_name)
        self._client.client.groups.list.assert_called_once_with(domain=client_plugin.get_domain_id(self.sample_domain_uuid), name=self.sample_name)
    ...
    @mock.patch.object(keystone.KeystoneClientPlugin, 'client')
    def test_get_group_id_with_name_and_domain_invalid_input(self, client_keystone):
        self._client.client.groups.get.side_effect = keystone_exceptions.NotFound
        self._client.client.groups.list.return_value = []
        client_keystone.return_value = self._client
        client_plugin = keystone.KeystoneClientPlugin(context=mock.MagicMock())
        self.assertRaises(exception.EntityNotFound, client_plugin.get_group_id, self.sample_name_and_domain_invalid_input)