...
def extract_glimpse_v2(input: _atypes.TensorFuzzingAnnotation[_atypes.Float32], size: _atypes.TensorFuzzingAnnotation[_atypes.Int32], offsets: _atypes.TensorFuzzingAnnotation[_atypes.Float32], centered: bool=True, normalized: bool=True, uniform_noise: bool=True, noise: str='uniform', name=None) -> _atypes.TensorFuzzingAnnotation[_atypes.Float32]:
  ...
  * If the coordinates are not normalized they are interpreted as
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'ExtractGlimpseV2', name, input, size, offsets, 'centered', centered, 'normalized', normalized, 'uniform_noise', uniform_noise, 'noise', noise)
            return _result
            ...
            pass
        try:
            return extract_glimpse_v2_eager_fallback(input, size, offsets, centered=centered, normalized=normalized, uniform_noise=uniform_noise, noise=noise, name=name, ctx=_ctx)
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('ExtractGlimpseV2', input=input, size=size, offsets=offsets, centered=centered, normalized=normalized, uniform_noise=uniform_noise, noise=noise, name=name)
    _result = _outputs[:]
