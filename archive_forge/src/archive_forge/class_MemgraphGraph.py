    ...
    *Security note*: Make sure that the database connection uses credentials
    ...
    def refresh_schema(self) -> None:
        ...
        for rel_name, properties in db_structured_schema['rel_props'].items():
            formatted_rel_props.append(f"Relationship name: '{rel_name}', Relationship properties: {properties}")
        formatted_rels = [f'(:{rel['start']})-[:{rel['type']}]->(:{rel['end']})' for rel in db_structured_schema['relationships']]
