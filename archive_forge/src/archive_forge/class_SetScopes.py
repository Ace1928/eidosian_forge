  ...
  """
    ...
    def _get_instance(self, instance_ref, client):
        ...
        if not self._instance:
            request = (client.apitools_client.instances, 'Get', client.messages.ComputeInstancesGetRequest(**instance_ref.AsDict()))
    ...
    def Run(self, args):
        ...
        instance_ref = flags.INSTANCE_ARG.ResolveAsResource(args, compute_holder.resources, default_scope=compute_scope.ScopeEnum.ZONE, scope_lister=flags.GetInstanceZoneScopeLister(client))
        email = self._email(args, instance_ref, client)
        scopes = self._scopes(args, instance_ref, client)
        if scopes and (not email):
            raise exceptions.ScopesWithoutServiceAccountException('Can not set scopes when there is no service acoount.')
        request = client.messages.ComputeInstancesSetServiceAccountRequest(instancesSetServiceAccountRequest=client.messages.InstancesSetServiceAccountRequest(email=email, scopes=scopes), project=instance_ref.project, zone=instance_ref.zone, instance=instance_ref.Name())
        return client.MakeRequests([(client.apitools_client.instances, 'SetServiceAccount', request)])