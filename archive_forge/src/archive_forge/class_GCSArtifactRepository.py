...
class GCSArtifactRepository(ArtifactRepository, MultipartUploadMixin):
    ...
    def __init__(self, artifact_uri, client=None):
        ...
        self._GCS_DEFAULT_TIMEOUT = MLFLOW_ARTIFACT_UPLOAD_DOWNLOAD_TIMEOUT.get() or MLFLOW_GCS_DEFAULT_TIMEOUT.get() or _DEFAULT_TIMEOUT
        self._GCS_DEFAULT_TIMEOUT = None if self._GCS_DEFAULT_TIMEOUT == -1 else self._GCS_DEFAULT_TIMEOUT
    ...
    def log_artifacts(self, local_dir, artifact_path=None):
        ...
        for root, _, filenames in os.walk(local_dir):
            ...
            for f in filenames:
                ...
                gcs_bucket.blob(path, chunk_size=self._GCS_UPLOAD_CHUNK_SIZE).upload_from_filename(os.path.join(root, f), timeout=self._GCS_DEFAULT_TIMEOUT)
    ...
    def _download_file(self, remote_file_path, local_path):
        ...
        gcs_bucket = self._get_bucket(bucket)
        gcs_bucket.blob(remote_full_path, chunk_size=self._GCS_DOWNLOAD_CHUNK_SIZE).download_to_filename(local_path, timeout=self._GCS_DEFAULT_TIMEOUT)
    ...
    def _validate_support_mpu():
        if Version(importlib.metadata.version('google-cloud-storage')) < Version('2.12.0') or Version(importlib.metadata.version('google-resumable-media')) < Version('2.6.0'):
