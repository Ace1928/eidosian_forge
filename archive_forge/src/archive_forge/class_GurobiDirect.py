...
class GurobiDirect(DirectSolver):
    ...
    def available(self, exception_flag=True):
        ...
        if not gurobipy_available:
            if exception_flag:
                ...
                raise ApplicationError('No Python bindings available for %s solver plugin' % (type(self),))
            return False
        ...
        if exception_flag:
            logger.warning(msg)
            raise ApplicationError('Could not create Model for %s solver plugin - gurobi message=%s' % (type(self), msg))
        else:
            return False
    ...
    def _apply_solver(self):
        ...
        if self._env_options:
            new_options = {key: option for key, option in self.options.items() if key not in self._env_options or self._env_options[key] != option}
        else:
    ...
    def _set_instance(self, model, kwds={}):
        ...
        try:
            ...
            e = sys.exc_info()[1]
            msg = 'Unable to create Gurobi model. Have you installed the Python bindings for Gurobi?\n\n\t' + 'Error message: {0}'.format(e)
            raise Exception(msg)
    ...
    def _add_constraint(self, con):
        ...
        if con._linear_canonical_form:
            gurobi_expr, referenced_vars = self._get_expr_from_pyomo_repn(con.canonical_form(), self._max_constraint_degree)
        else:
        ...
        if con.equality:
            gurobipy_con = self._solver_model.addConstr(lhs=gurobi_expr, sense=gurobipy.GRB.EQUAL, rhs=value(con.lower), name=conname)
        elif con.has_lb() and con.has_ub():
            gurobipy_con = self._solver_model.addRange(gurobi_expr, value(con.lower), value(con.upper), name=conname)
            self._range_constraints.add(con)
        elif con.has_lb():
            gurobipy_con = self._solver_model.addConstr(lhs=gurobi_expr, sense=gurobipy.GRB.GREATER_EQUAL, rhs=value(con.lower), name=conname)
        elif con.has_ub():
            gurobipy_con = self._solver_model.addConstr(lhs=gurobi_expr, sense=gurobipy.GRB.LESS_EQUAL, rhs=value(con.upper), name=conname)
        else:
    ...
    def _postsolve(self):
        ...
        if status == grb.LOADED:
            ...
            self.results.solver.termination_message = 'Model was solved to optimality (subject to tolerances), and an optimal solution is available.'
            ...
            self.results.solver.status = SolverStatus.aborted
            self.results.solver.termination_message = 'Optimal objective for model was proven to be worse than the value specified in the Cutoff parameter. No solution information is available.'
            self.results.solver.termination_condition = TerminationCondition.minFunctionValue
            soln.status = SolutionStatus.unknown
        elif status == grb.ITERATION_LIMIT:
            self.results.solver.status = SolverStatus.aborted
            self.results.solver.termination_message = 'Optimization terminated because the total number of simplex iterations performed exceeded the value specified in the IterationLimit parameter.'
            self.results.solver.termination_condition = TerminationCondition.maxIterations
            soln.status = SolutionStatus.stoppedByLimit
        elif status == grb.NODE_LIMIT:
            self.results.solver.status = SolverStatus.aborted
            self.results.solver.termination_message = 'Optimization terminated because the total number of branch-and-cut nodes explored exceeded the value specified in the NodeLimit parameter'
            self.results.solver.termination_condition = TerminationCondition.maxEvaluations
            soln.status = SolutionStatus.stoppedByLimit
        elif status == grb.TIME_LIMIT:
            self.results.solver.status = SolverStatus.aborted
            self.results.solver.termination_message = 'Optimization terminated because the time expended exceeded the value specified in the TimeLimit parameter.'
            self.results.solver.termination_condition = TerminationCondition.maxTimeLimit
            soln.status = SolutionStatus.stoppedByLimit
        elif status == grb.SOLUTION_LIMIT:
            self.results.solver.status = SolverStatus.aborted
            self.results.solver.termination_message = 'Optimization terminated because the number of solutions found reached the value specified in the SolutionLimit parameter.'
            ...
            self.results.solver.status = SolverStatus.warning
            self.results.solver.termination_message = 'Unable to satisfy optimality tolerances; a sub-optimal solution is available.'
            ...
            self.results.solver.status = SolverStatus.aborted
            self.results.solver.termination_message = 'User specified an objective limit (a bound on either the best objective or the best bound), and that limit has been reached. Solution is available.'
            self.results.solver.termination_condition = TerminationCondition.other
            soln.status = SolutionStatus.stoppedByLimit
        else:
            self.results.solver.status = SolverStatus.error
            self.results.solver.termination_message = 'Unhandled Gurobi solve status (' + str(status) + ')'
            self.results.solver.termination_condition = TerminationCondition.error
            soln.status = SolutionStatus.error
