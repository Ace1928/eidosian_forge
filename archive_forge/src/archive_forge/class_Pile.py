...
class Pile(Widget, WidgetContainerMixin, WidgetContainerListContentsMixin):
    ...
    def sizing(self) -> frozenset[Sizing]:
        ...
        * PACK FIXED -> FIXED (widget knows its size)
        ...
        box_flow_fixed = _ContainerElementSizingFlag.BOX | _ContainerElementSizingFlag.FLOW | _ContainerElementSizingFlag.FIXED
        ...
        for idx, (widget, (size_kind, _size_weight)) in enumerate(self.contents):
            ...
            if not flag & box_flow_fixed:
                warnings.warn(f'Sizing combination of widget {idx} not supported: {size_kind.name} {'|'.join(w_sizing).upper()}', PileWarning, stacklevel=3)
                return frozenset((Sizing.BOX, Sizing.FLOW))
    ...
    def __init__(self, widget_list: Iterable[Widget | tuple[Literal['pack', WHSettings.PACK] | int, Widget] | tuple[Literal['given', WHSettings.GIVEN], int, Widget] | tuple[Literal['weight', WHSettings.WEIGHT], int | float, Widget]], focus_item: Widget | int | None=None) -> None:
        """
        ...
        *widget_list* may also contain tuples such as:
        ...
        super().__init__()
        self._contents: MonitoredFocusList[Widget, tuple[Literal[WHSettings.PACK], None] | tuple[Literal[WHSettings.GIVEN], int] | tuple[Literal[WHSettings.WEIGHT], int | float]] = MonitoredFocusList()
        ...
        for i, original in enumerate(widget_list):
            ...
            elif len(w) == 2 or w[0] in {Sizing.FIXED, WHSettings.GIVEN}:
                height, w = w[-2:]
    ...
    def __rich_repr__(self) -> Iterator[tuple[str | None, typing.Any] | typing.Any]:
        widget_list: list[Widget | tuple[Literal[WHSettings.PACK] | int, Widget] | tuple[Literal[WHSettings.WEIGHT], int | float, Widget]] = []
    ...
    def _validate_contents_modified(self, slc, new_items) -> None:
        ...
        try:
            for item in new_items:
                _w, (t, n) = item
                if any((t not in {WHSettings.PACK, WHSettings.GIVEN, WHSettings.WEIGHT}, n is not None and (not isinstance(n, (int, float)) or n < 0))):
    ...
    def item_types(self):
        ...
        """
        warnings.warn('only for backwards compatibility. You should use the new standard container property `contents`', PendingDeprecationWarning, stacklevel=2)
        ml = MonitoredList((({WHSettings.GIVEN: Sizing.FIXED, WHSettings.PACK: Sizing.FLOW}.get(f, f), height) for w, (f, height) in self.contents))
    ...
    @property
    def contents(self) -> MonitoredFocusList[Widget, tuple[Literal[WHSettings.PACK], None] | tuple[Literal[WHSettings.GIVEN], int] | tuple[Literal[WHSettings.WEIGHT], int | float]]:
        """
    ...
    @contents.setter
    def contents(self, c: Sequence[Widget, tuple[Literal[WHSettings.PACK], None] | tuple[Literal[WHSettings.GIVEN], int] | tuple[Literal[WHSettings.WEIGHT], int | float]]) -> None:
        self._contents[:] = c

    @staticmethod
    def options(height_type: Literal['pack', 'given', 'weight'] | WHSettings=WHSettings.WEIGHT, height_amount: int | float | None=1) -> tuple[Literal[WHSettings.PACK], None] | tuple[Literal[WHSettings.GIVEN], int] | tuple[Literal[WHSettings.WEIGHT], int | float]:
        """
    ...
    def render(self, size: tuple[()] | tuple[int] | tuple[int, int], focus: bool=False) -> SolidCanvas | CompositeCanvas:
    ...
    def keypress(self, size: tuple[()] | tuple[int] | tuple[int, int], key: str) -> str | None:
        ...
        if self._command_map[key] == Command.UP:
            candidates = tuple(range(i - 1, -1, -1))
        else:
    ...
    def move_cursor_to_coords(self, size: tuple[()] | tuple[int] | tuple[int, int], col: int, row: int) -> bool:
        """Capture pref col and set new focus."""
        self.pref_col = col
        focus = True
        wrow = 0
        _widths, heights, size_args = self.get_rows_sizes(size, focus=focus)
        for i, (r, w_size, (w, _)) in enumerate(zip(heights, size_args, self.contents)):
            if wrow + r > row:
                break
            wrow += r
        else:
