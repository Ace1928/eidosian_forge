...
class FugueSQLParser:
    ...
    def __init__(self, code: str, rule: str, ignore_case: bool, parse_mode: str):
        a_version = pkg_resources.get_distribution('antlr4-python3-runtime').version
        if version.parse(a_version) < version.parse('4.11.1'):
            raise RuntimeError(f'antlr4-python3-runtime>=4.11.1,<4.12 is required, current version is {a_version}')
        self._rule = rule
        self._raw_code = code
        ...
        if ignore_case:
            ...
            try:
                ...
                if _detect_case_issue(code, _FUGUE_SQL_CASE_ISSUE_THREHOLD):
                    prefix = f'(FugueSQL requires uppercase characters by default. To ignore casing, turn on {FUGUE_CONF_SQL_IGNORE_CASE})'
                    msg = prefix + str(e)
    ...
    def _get_tokens(self, node: Optional[ParseTree], parent: Optional[ParseTree]=None) -> Iterable[CommonToken]:
        if node is None:
            return
        elif isinstance(node, Token):
            if isinstance(node, CommonToken) and node.type != Token.EOF:
                if not isinstance(parent, fugue_sqlParser.NonReservedContext) and hasattr(parent, node.text):
    ...
    def _to_tree(self, upper: bool) -> ParseTree:
        ...
        if self._parse_mode == 'cpp':
            if not FUGUE_SQL_CPP_PARSER_AVAILABLE:
                raise ValueError("CPP parser is not installed, use 'auto' or 'py' as parse_mode")
