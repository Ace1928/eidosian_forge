...
End EAM Interface Documentation
    ...
    default_parameters = dict(skin=1.0, potential=None, header=[b'EAM/ADP potential file\n', b'Generated from eam.py\n', b'blank\n'])
    ...
    def __init__(self, restart=None, ignore_bad_restart_file=Calculator._deprecated, label=os.curdir, atoms=None, form=None, **kwargs):
        ...
        valid_args = ('potential', 'elements', 'header', 'drho', 'dr', 'cutoff', 'atomic_number', 'mass', 'a', 'lattice', 'embedded_energy', 'electron_density', 'phi', 'd_embedded_energy', 'd_electron_density', 'd_phi', 'd', 'q', 'd_d', 'd_q', 'skin', 'Z', 'nr', 'nrho', 'mass')
    ...
    def _read_potential(self, fd):
        ...
        if self.form == 'eam':
            ...
            self.density_data = np.array([np.float_(data[n + self.nr:n + 2 * self.nr])])
            ...
            for elem in range(self.Nelements):
                ...
                self.embedded_data[elem] = np.float_(data[d:d + self.nrho])
                d += self.nrho
                self.density_data[elem, :, :] = np.float_(data[d:d + self.nr * self.Nelements]).reshape([self.Nelements, self.nr])
    ...
    def plot(self, name=''):
        ...
        self.elem_subplot(rho, self.embedded_energy, '$\\rho$', 'Embedding Energy $F(\\bar\\rho)$', name, plt)
        plt.subplot(nrow, 2, 2)
        if self.form == 'fs':
            self.multielem_subplot(r, self.electron_density, '$r$', 'Electron Density $\\rho(r)$', name, plt, half=False)
        else:
            self.elem_subplot(r, self.electron_density, '$r$', 'Electron Density $\\rho(r)$', name, plt)
        plt.subplot(nrow, 2, 3)
        self.multielem_subplot(r, self.phi, '$r$', 'Pair Potential $\\phi(r)$', name, plt)
        plt.ylim(-1.0, 1.0)
        if self.form == 'adp':
            plt.subplot(nrow, 2, 5)
            self.multielem_subplot(r, self.d, '$r$', 'Dipole Energy', name, plt)
            plt.subplot(nrow, 2, 6)
            self.multielem_subplot(r, self.q, '$r$', 'Quadrupole Energy', name, plt)
        plt.plot()
