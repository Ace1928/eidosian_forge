...
class table__m_e_t_a(DefaultTable.DefaultTable):
    ...
    def compile(self, ttFont):
        ...
        header = sstruct.pack(META_HEADER_FORMAT, {'version': 1, 'flags': 0, 'dataOffset': dataOffset, 'numDataMaps': len(keys)})
        dataMaps = []
        dataBlocks = []
        for tag in keys:
            if tag in ['dlng', 'slng']:
                ...
                data = self.data[tag]
            dataMaps.append(sstruct.pack(DATA_MAP_FORMAT, {'tag': tag, 'dataOffset': dataOffset, 'dataLength': len(data)}))
    ...
    def toXML(self, writer, ttFont):
        for tag in sorted(self.data.keys()):
            if tag in ['dlng', 'slng']:
                ...
                writer.newline()
                data = self.data[tag]
                if min(data) >= 32 and max(data) <= 126:
