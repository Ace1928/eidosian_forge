...
class ScaleIOConnector(base.BaseLinuxConnector):
    ...
    def __init__(self, root_helper, driver=None, device_scan_attempts=initiator.DEVICE_SCAN_ATTEMPTS_DEFAULT, *args, **kwargs):
        super(ScaleIOConnector, self).__init__(root_helper, *args, driver=driver, device_scan_attempts=device_scan_attempts, **kwargs)
    ...
    def _get_client_id(self):
        request = 'https://%(server_ip)s:%(server_port)s/api/types/Client/instances/getByIp::%(sdc_ip)s/' % {'server_ip': self.server_ip, 'server_port': self.server_port, 'sdc_ip': self.local_sdc_ip}
        LOG.info('ScaleIO get client id by ip request: %(request)s', {'request': request})
        r = requests.get(request, auth=(self.server_username, self.server_token), verify=self._verify_cert())
        r = self._check_response(r, request)
        sdc_id = r.json()
        if not sdc_id:
            msg = _('Client with ip %(sdc_ip)s was not found.') % {'sdc_ip': self.local_sdc_ip}
            raise exception.BrickException(message=msg)
        if r.status_code != 200 and 'errorCode' in sdc_id:
            msg = _('Error getting sdc id from ip %(sdc_ip)s: %(err)s') % {'sdc_ip': self.local_sdc_ip, 'err': sdc_id['message']}
            LOG.error(msg)
            raise exception.BrickException(message=msg)
    ...
    def _check_response(self, response, request, is_get_request=True, params=None):
        if response.status_code == 401 or response.status_code == 403:
            LOG.info('Token is invalid, going to re-login to get a new one')
            login_request = 'https://%(server_ip)s:%(server_port)s/api/login' % {'server_ip': self.server_ip, 'server_port': self.server_port}
            r = requests.get(login_request, auth=(self.server_username, self.server_password), verify=self._verify_cert())
            token = r.json()
            LOG.debug(_('Going to perform request %(request)s again with valid token'), {'request': request})
            if is_get_request:
                res = requests.get(request, auth=(self.server_username, token), verify=self._verify_cert())
            else:
                headers = {'content-type': 'application/json'}
                res = requests.post(request, data=json.dumps(params), headers=headers, auth=(self.server_username, token), verify=self._verify_cert())
    ...
    def get_config(self, connection_properties):
        ...
        self.server_password, server_token = self._get_password_token(connection_properties)
        if server_token:
            self.server_token = server_token
        self.iops_limit = connection_properties['iopsLimit']
        self.bandwidth_limit = connection_properties['bandwidthLimit']
        self.verify_certificate = connection_properties.get('verify_certificate')
        self.certificate_path = connection_properties.get('certificate_path')
        device_info = {'type': 'block', 'path': self.volume_path}
        return device_info
    ...
    def connect_volume(self, connection_properties):
        ...
        LOG.debug(_('scaleIO Volume name: %(volume_name)s, SDC IP: %(sdc_ip)s, REST Server IP: %(server_ip)s, REST Server username: %(username)s, iops limit: %(iops_limit)s, bandwidth limit: %(bandwidth_limit)s.'), {'volume_name': self.volume_name, 'volume_id': self.volume_id, 'sdc_ip': self.local_sdc_ip, 'server_ip': self.server_ip, 'username': self.server_username, 'iops_limit': self.iops_limit, 'bandwidth_limit': self.bandwidth_limit})
        guid = self._get_guid()
        params = {'guid': guid, 'allowMultipleMappings': 'TRUE'}
        self.volume_id = self.volume_id or self._get_volume_id()
        headers = {'content-type': 'application/json'}
        request = 'https://%(server_ip)s:%(server_port)s/api/instances/Volume::%(volume_id)s/action/addMappedSdc' % {'server_ip': self.server_ip, 'server_port': self.server_port, 'volume_id': self.volume_id}
        LOG.info('map volume request: %(request)s', {'request': request})
        r = requests.post(request, data=json.dumps(params), headers=headers, auth=(self.server_username, self.server_token), verify=self._verify_cert())
        r = self._check_response(r, request, False, params)
        if r.status_code != self.OK_STATUS_CODE:
            response = r.json()
            error_code = response['errorCode']
            if error_code == self.VOLUME_ALREADY_MAPPED_ERROR or error_code == self.VOLUME_ALREADY_MAPPED_ERROR_v4:
                LOG.warning('Ignoring error mapping volume %(volume_name)s: volume already mapped.', {'volume_name': self.volume_name})
            else:
                msg = _('Error mapping volume %(volume_name)s: %(err)s') % {'volume_name': self.volume_name, 'err': response['message']}
                LOG.error(msg)
                raise exception.BrickException(message=msg)
        ...
        if self.iops_limit is not None or self.bandwidth_limit is not None:
            ...
            request = 'https://%(server_ip)s:%(server_port)s/api/instances/Volume::%(volume_id)s/action/setMappedSdcLimits' % {'server_ip': self.server_ip, 'server_port': self.server_port, 'volume_id': self.volume_id}
            LOG.info('Set client limit request: %(request)s', {'request': request})
            r = requests.post(request, data=json.dumps(params), headers=headers, auth=(self.server_username, self.server_token), verify=self._verify_cert())
            r = self._check_response(r, request, False, params)
            if r.status_code != self.OK_STATUS_CODE:
                response = r.json()
                LOG.info('Set client limit response: %(response)s', {'response': response})
                msg = _('Error setting client limits for volume %(volume_name)s: %(err)s') % {'volume_name': self.volume_name, 'err': response['message']}
                LOG.error(msg)
        return device_info
    ...
    def disconnect_volume(self, connection_properties, device_info, force=False, ignore_errors=False):
        ...
        self.get_config(connection_properties)
        self.volume_id = self.volume_id or self._get_volume_id()
        LOG.info('ScaleIO disconnect volume in ScaleIO brick volume driver.')
        LOG.debug(_('ScaleIO Volume name: %(volume_name)s, SDC IP: %(sdc_ip)s, REST Server IP: %(server_ip)s'), {'volume_name': self.volume_name, 'sdc_ip': self.local_sdc_ip, 'server_ip': self.server_ip})
        guid = self._get_guid()
        params = {'guid': guid}
        headers = {'content-type': 'application/json'}
        request = 'https://%(server_ip)s:%(server_port)s/api/instances/Volume::%(volume_id)s/action/removeMappedSdc' % {'server_ip': self.server_ip, 'server_port': self.server_port, 'volume_id': self.volume_id}
        LOG.info('Unmap volume request: %(request)s', {'request': request})
        r = requests.post(request, data=json.dumps(params), headers=headers, auth=(self.server_username, self.server_token), verify=self._verify_cert())
        r = self._check_response(r, request, False, params)
        if r.status_code != self.OK_STATUS_CODE:
            response = r.json()
            error_code = response['errorCode']
            if error_code == self.VOLUME_NOT_MAPPED_ERROR or error_code == self.VOLUME_NOT_MAPPED_ERROR_v4:
                LOG.warning('Ignoring error unmapping volume %(volume_id)s: volume not mapped.', {'volume_id': self.volume_name})
            else:
                msg = _('Error unmapping volume %(volume_id)s: %(err)s') % {'volume_id': self.volume_name, 'err': response['message']}
                LOG.error(msg)
                raise exception.BrickException(message=msg)
