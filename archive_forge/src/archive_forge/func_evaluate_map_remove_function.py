...
@method(function('remove', prefix='map', nargs=2, sequence_types=('map(*)', 'xs:anyAtomicType*', 'map(*)')))
def evaluate_map_remove_function(self, context=None):
    ...
    if keys is None:
        ...
        items = ((k, v) for k, v in map_.items(context) if all((not_equal(k, x) for x in keys)))
    else:
