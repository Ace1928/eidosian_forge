...
class NotificationProtocolEntity(ProtocolEntity):
    ...
    def toProtocolTreeNode(self):
        attribs = {'t': str(self.timestamp), 'from': self._from, 'offline': '1' if self.offline else '0', 'type': self._type, 'id': self._id, 'notify': self.notify}
        return self._createProtocolTreeNode(attribs, children=None, data=None)

    @staticmethod
    def fromProtocolTreeNode(node):
        return NotificationProtocolEntity(node.getAttributeValue('type'), node.getAttributeValue('id'), node.getAttributeValue('from'), node.getAttributeValue('t'), node.getAttributeValue('notify'), node.getAttributeValue('offline'))