...
@TransformationFactory.register('contrib.logical_to_disjunctive', doc='Convert logical propositions with only Boolean arguments to MIP representation and convert logical expressions with mixed integer-Boolean arguments (such as atleast, atmost, and exactly) to disjunctive representation')
class LogicalToDisjunctive(Transformation):
    ...
    def _transform_block(self, target_block, model, new_varlists, transBlocks):
        ...
        for block in _blocks:
            for logical_constraint in block.component_objects(ctype=LogicalConstraint, active=True, descend_into=(Block, Disjunct)):
