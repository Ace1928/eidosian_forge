...
def test_cannot_check_grant_for_group_own_domain_on_project_other_domain(self):
    ...
    with self.test_client() as c:
        c.get('/v3/projects/%s/groups/%s/roles/%s' % (project['id'], group['id'], self.bootstrapper.reader_role_id), headers=self.headers, expected_status_code=http.client.FORBIDDEN)