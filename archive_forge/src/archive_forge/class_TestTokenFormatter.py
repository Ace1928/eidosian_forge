...
class TestTokenFormatter(unit.TestCase):
    ...
    def test_restore_padding(self):
        ...
        for binary in binary_to_test:
            ...
            encoded_str_with_padding_restored = token_formatters.TokenFormatter.restore_padding(encoded_str_without_padding)
    ...
    def test_create_validate_federated_scoped_token_non_uuid_user_id(self):
        ...
        token_formatter = token_formatters.TokenFormatter()
        token = token_formatter.create_token(user_id=exp_user_id, expires_at=exp_expires_at, audit_ids=exp_audit_ids, payload_class=token_formatters.FederatedProjectScopedPayload, methods=exp_methods, federated_group_ids=exp_federated_group_ids, identity_provider_id=exp_idp_id, protocol_id=exp_protocol_id, project_id=exp_project_id)
        user_id, methods, audit_ids, system, domain_id, project_id, trust_id, federated_group_ids, identity_provider_id, protocol_id, access_token_id, app_cred_id, thumbprint, issued_at, expires_at = token_formatter.validate_token(token)
        self.assertEqual(exp_user_id, user_id)
        self.assertTrue(isinstance(user_id, str))
        self.assertEqual(exp_methods, methods)
