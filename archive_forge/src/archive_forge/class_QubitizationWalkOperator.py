...
class QubitizationWalkOperator(infra.GateWithRegisters):
    ...
    def reflect(self) -> reflection_using_prepare.ReflectionUsingPrepare:
        return reflection_using_prepare.ReflectionUsingPrepare(self.prepare, control_val=self.control_val)
    ...
    def _circuit_diagram_info_(self, args: cirq.CircuitDiagramInfoArgs) -> cirq.CircuitDiagramInfo:
        wire_symbols = ['@' if self.control_val else '@(0)'] * infra.total_bits(self.control_registers)
        wire_symbols += ['W'] * (infra.total_bits(self.signature) - infra.total_bits(self.control_registers))
        wire_symbols[-1] = f'W^{self.power}' if self.power != 1 else 'W'
        return cirq.CircuitDiagramInfo(wire_symbols=wire_symbols)

    def controlled(self, num_controls: Optional[int]=None, control_values: Optional[Union[cirq.ops.AbstractControlValues, Sequence[Union[int, Collection[int]]]]]=None, control_qid_shape: Optional[Tuple[int, ...]]=None) -> 'QubitizationWalkOperator':
        ...
        if isinstance(control_values, Sequence) and isinstance(control_values[0], int) and (len(control_values) == 1) and (self.control_val is None):
            ...
            return QubitizationWalkOperator(c_select, self.prepare, control_val=control_values[0], power=self.power)
    ...
    def with_power(self, new_power: int) -> 'QubitizationWalkOperator':
        return QubitizationWalkOperator(self.select, self.prepare, control_val=self.control_val, power=new_power)

    def __repr__(self) -> str:
        return f'cirq_ft.QubitizationWalkOperator({self.select}, {self.prepare}, {self.control_val}, {self.power})'
