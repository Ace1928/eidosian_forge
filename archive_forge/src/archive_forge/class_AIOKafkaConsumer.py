...
class AIOKafkaConsumer:
    ...
    def __del__(self, _warnings=warnings):
        if self._closed is False:
            _warnings.warn(f'Unclosed AIOKafkaConsumer {self!r}', ResourceWarning, source=self)
    ...
    async def start(self):
        ...
        """
        assert self._loop is get_running_loop(), 'Please create objects with the same loop as running with'
        ...
        if self._isolation_level == 'read_committed' and self._client.api_version < (0, 11):
            raise UnsupportedVersionError('`read_committed` isolation_level available only for Brokers 0.11 and above')
        self._fetcher = Fetcher(self._client, self._subscription, key_deserializer=self._key_deserializer, value_deserializer=self._value_deserializer, fetch_min_bytes=self._fetch_min_bytes, fetch_max_bytes=self._fetch_max_bytes, fetch_max_wait_ms=self._fetch_max_wait_ms, max_partition_fetch_bytes=self._max_partition_fetch_bytes, check_crcs=self._check_crcs, fetcher_timeout=self._consumer_timeout, retry_backoff_ms=self._retry_backoff_ms, auto_offset_reset=self._auto_offset_reset, isolation_level=self._isolation_level)
        if self._group_id is not None:
            self._coordinator = GroupCoordinator(self._client, self._subscription, group_id=self._group_id, group_instance_id=self._group_instance_id, heartbeat_interval_ms=self._heartbeat_interval_ms, session_timeout_ms=self._session_timeout_ms, retry_backoff_ms=self._retry_backoff_ms, enable_auto_commit=self._enable_auto_commit, auto_commit_interval_ms=self._auto_commit_interval_ms, assignors=self._partition_assignment_strategy, exclude_internal_topics=self._exclude_internal_topics, rebalance_timeout_ms=self._rebalance_timeout_ms, max_poll_interval_ms=self._max_poll_interval_ms)
            if self._subscription.subscription is not None:
                if self._subscription.partitions_auto_assigned():
                    await self._subscription.wait_for_assignment()
                else:
                    self._coordinator.start_commit_offsets_refresh_task(self._subscription.subscription.assignment)
        else:
            self._coordinator = NoGroupCoordinator(self._client, self._subscription, exclude_internal_topics=self._exclude_internal_topics)
            if self._subscription.subscription is not None:
                if self._subscription.partitions_auto_assigned():
    ...
    async def position(self, partition):
        ...
        while True:
            ...
            if not tp_state.has_valid_position:
                ...
                await asyncio.wait([tp_state.wait_for_position(), assignment.unassign_future], timeout=self._request_timeout_ms / 1000, return_when=asyncio.FIRST_COMPLETED)
                if not tp_state.has_valid_position:
    ...
    async def seek_to_beginning(self, *partitions):
        ...
        fut = self._fetcher.request_offset_reset(partitions, OffsetResetStrategy.EARLIEST)
        assignment = self._subscription.subscription.assignment
        await asyncio.wait([fut, assignment.unassign_future], timeout=self._request_timeout_ms / 1000, return_when=asyncio.FIRST_COMPLETED)
        self._coordinator.check_errors()
        return fut.done()
    ...
    async def offsets_for_times(self, timestamps):
        ...
        if self._client.api_version <= (0, 10, 0):
            raise UnsupportedVersionError(f'offsets_for_times API not supported for cluster version {self._client.api_version}')
        for tp, ts in timestamps.items():
            timestamps[tp] = int(ts)
            if ts < 0:
                raise ValueError(f'The target time for partition {tp} is {ts}. The target time cannot be negative.')
        offsets = await self._fetcher.get_offsets_by_times(timestamps, self._request_timeout_ms)
        return offsets
    ...
    async def beginning_offsets(self, partitions):
        ...
        if self._client.api_version <= (0, 10, 0):
            raise UnsupportedVersionError(f'offsets_for_times API not supported for cluster version {self._client.api_version}')
        offsets = await self._fetcher.beginning_offsets(partitions, self._request_timeout_ms)
        return offsets
    ...
    def subscribe(self, topics=(), pattern=None, listener=None):
        ...
        if listener is not None and (not isinstance(listener, ConsumerRebalanceListener)):
            raise TypeError('listener should be an instance of ConsumerRebalanceListener')
    ...
    async def getmany(self, *partitions, timeout_ms=0, max_records=None) -> Dict[TopicPartition, List[ConsumerRecord]]:
        ...
        with self._subscription.fetch_context():
            records = await self._fetcher.fetched_records(partitions, timeout, max_records=max_records or self._max_poll_records)
        return records
    ...
    async def __anext__(self) -> ConsumerRecord:
        ...
        while True:
            ...
            except (TopicAuthorizationFailedError, OffsetOutOfRangeError, NoOffsetForPartitionError) as err:
                raise err
