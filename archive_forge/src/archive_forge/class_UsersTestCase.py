...
class UsersTestCase(base.V3ClientTestCase):
    ...
    def check_user(self, user, user_ref=None):
        ...
        if user_ref:
            ...
            if hasattr(user_ref, 'default_project'):
                self.assertEqual(user_ref['default_project'], user.default_project_id)
        else:
            self.assertIsNotNone(user.name)
            self.assertIsNotNone(user.domain_id)

    def test_create_user(self):
        user_ref = {'name': fixtures.RESOURCE_NAME_PREFIX + uuid.uuid4().hex, 'domain': self.project_domain_id, 'default_project': self.project_id, 'password': uuid.uuid4().hex, 'description': uuid.uuid4().hex}
    ...
    def test_update_user(self):
        ...
        user_ret = self.client.users.update(user.id, description=new_description)
    ...
    def test_delete_user(self):
        user = self.client.users.create(name=uuid.uuid4().hex, domain=self.project_domain_id)
        self.client.users.delete(user.id)
        self.assertRaises(http.NotFound, self.client.users.get, user.id)