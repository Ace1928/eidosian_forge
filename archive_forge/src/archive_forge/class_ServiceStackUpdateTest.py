...
class ServiceStackUpdateTest(common.HeatTestCase):
    ...
    def test_stack_update_existing_parameters_remove(self):
        ...
        update_params = {'encrypted_param_names': [], 'parameter_defaults': {}, 'event_sinks': [], 'parameters': {'newparam': 123}, 'resource_registry': {'resources': {}}}
        api_args = {rpc_api.PARAM_TIMEOUT: 60, rpc_api.PARAM_EXISTING: True, rpc_api.PARAM_CLEAR_PARAMETERS: ['removeme'], rpc_api.PARAM_CONVERGE: False}
        t = template_format.parse(tools.wp_template)
        t['parameters']['removeme'] = {'type': 'string'}
        stk = utils.parse_stack(t, stack_name=stack_name, params={'KeyName': 'test', 'removeme': 'foo'})
        stk.set_stack_user_project_id('1234')
        self.assertEqual({'KeyName': 'test', 'removeme': 'foo'}, stk.t.env.params)
        t['parameters']['newparam'] = {'type': 'number'}
        with mock.patch('heat.engine.stack.Stack') as mock_stack:
            stk.update = mock.Mock()
            self.patchobject(service, 'NotifyEvent')
    ...
    def test_stack_update_existing_parameter_defaults(self):
        ...
        intial_params = {'encrypted_param_names': [], 'parameter_defaults': {'mydefault': 123}, 'parameters': {}, 'resource_registry': {}}
        update_params = {'encrypted_param_names': [], 'parameter_defaults': {'default2': 456}, 'parameters': {}, 'resource_registry': {}}
        api_args = {rpc_api.PARAM_TIMEOUT: 60, rpc_api.PARAM_EXISTING: True, rpc_api.PARAM_CONVERGE: False}
        t = template_format.parse(tools.wp_template)
        stk = utils.parse_stack(t, stack_name=stack_name, params=intial_params)
        stk.set_stack_user_project_id('1234')
        expected_env = {'encrypted_param_names': [], 'parameter_defaults': {'mydefault': 123, 'default2': 456}, 'parameters': {}, 'event_sinks': [], 'resource_registry': {'resources': {}}}
        with mock.patch('heat.engine.stack.Stack') as mock_stack:
            stk.update = mock.Mock()
            self.patchobject(service, 'NotifyEvent')
            mock_stack.load.return_value = stk
    ...
    def test_stack_update_reuses_api_params(self):
        ...
        mock_env = self.patchobject(environment, 'Environment', return_value=stk.env)
        mock_validate = self.patchobject(stk, 'validate', return_value=None)
        result = self.man.update_stack(self.ctx, old_stack.identifier(), template, params, None, {rpc_api.PARAM_CONVERGE: False})
        self.assertEqual(old_stack.identifier(), result)
        self.assertIsInstance(result, dict)
        self.assertTrue(result['stack_id'])
        mock_validate.assert_called_once_with()
    ...
    def test_stack_cancel_update_same_engine(self):
        ...
        self.patchobject(stack_lock.StackLock, 'get_engine_id', return_value=self.man.engine_id)
        ...
        self.man.thread_group_mgr.send.assert_called_once_with(stk.id, 'cancel')
    ...
    def test_stack_update_verify_err(self):
        ...
        api_args = {'timeout_mins': 60, rpc_api.PARAM_CONVERGE: False}
        ex = self.assertRaises(dispatcher.ExpectedException, self.man.update_stack, self.ctx, old_stack.identifier(), template, params, None, api_args)
    ...
    def test_stack_update_no_credentials(self):
        ...
        mock_env = self.patchobject(environment, 'Environment', return_value=stk.env)
        api_args = {'timeout_mins': 60, rpc_api.PARAM_CONVERGE: False}
        ex = self.assertRaises(dispatcher.ExpectedException, self.man.update_stack, self.ctx, stk.identifier(), template, params, None, api_args)
    ...
    def test_stack_update_existing_template(self):
        ...
        with mock.patch('heat.engine.stack.Stack') as mock_stack:
            ...
            mock_stack.validate.return_value = None
            result = self.man.update_stack(self.ctx, stack.identifier(), None, params, None, api_args)
            tmpl = mock_stack.call_args[0][2]
            self.assertEqual(t, tmpl.t)
    ...
    def test_update_mutable_parameter_allowed(self):
        ...
        params = {'param1': 'bar'}
        result = self.man.update_stack(self.ctx, old_stack.identifier(), templatem.Template(template), params, None, {rpc_api.PARAM_CONVERGE: False})
