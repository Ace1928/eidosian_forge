...
class ConductorManager(base.Manager):
    ...
    def list(self, marker=None, limit=None, sort_key=None, sort_dir=None, fields=None, detail=False, os_ironic_api_version=None, global_request_id=None):
        ...
        filters = utils.common_filters(marker, limit, sort_key, sort_dir, fields, detail)
        path = ''
        if filters:
            path += '?' + '&'.join(filters)
        ...
        if limit is None:
            ...
            return self._list_pagination(self._path(path), 'conductors', limit=limit, **header_values)

    def get(self, hostname, fields=None, os_ironic_api_version=None, global_request_id=None):
        return self._get(resource_id=hostname, fields=fields, os_ironic_api_version=os_ironic_api_version, global_request_id=global_request_id)