...
class TestSeriesData(unittest.TestCase):
    ...
    def test_construct_and_get_data(self):
        ...
        processed_data_dict = {pyo.ComponentUID(key): val for key, val in data_dict.items()}
    ...
    def test_to_serializable(self):
        ...
        pred_json_obj = ({str(pyo.ComponentUID(key)): val for key, val in data_dict.items()}, list(m.time))
        self.assertEqual(data, pred_json_obj)
        self.assertEqual(data.time, list(m.time))
        self.assertEqual(data.data, {str(pyo.ComponentUID(key)): val for key, val in data_dict.items()})
    ...
    def test_concatenate(self):
        ...
        pred_data = {m.var[:, 'A']: [1, 2, 3, 1, 2, 3], m.var[:, 'B']: [2, 4, 6, 2, 4, 6]}
