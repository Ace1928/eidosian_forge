...
class TestNetworkPoolMember(TestNetworkProxy):
    ...
    def test_pool_member_delete(self):
        self.verify_delete(self.proxy.delete_pool_member, pool_member.PoolMember, ignore_missing=False, method_kwargs={'pool': 'test_id'}, expected_kwargs={'pool_id': 'test_id'})

    def test_pool_member_delete_ignore(self):
        self.verify_delete(self.proxy.delete_pool_member, pool_member.PoolMember, ignore_missing=True, method_kwargs={'pool': 'test_id'}, expected_kwargs={'pool_id': 'test_id'})

    def test_pool_member_find(self):
        self._verify('openstack.proxy.Proxy._find', self.proxy.find_pool_member, method_args=['MEMBER', 'POOL'], expected_args=[pool_member.PoolMember, 'MEMBER'], expected_kwargs={'pool_id': 'POOL', 'ignore_missing': True})

    def test_pool_member_get(self):
        self._verify('openstack.proxy.Proxy._get', self.proxy.get_pool_member, method_args=['MEMBER', 'POOL'], expected_args=[pool_member.PoolMember, 'MEMBER'], expected_kwargs={'pool_id': 'POOL'})

    def test_pool_members(self):
        self.verify_list(self.proxy.pool_members, pool_member.PoolMember, method_args=['test_id'], expected_args=[], expected_kwargs={'pool_id': 'test_id'})

    def test_pool_member_update(self):
        self._verify('openstack.network.v2._proxy.Proxy._update', self.proxy.update_pool_member, method_args=['MEMBER', 'POOL'], expected_args=[pool_member.PoolMember, 'MEMBER'], expected_kwargs={'pool_id': 'POOL'})