...
class RotosolveOptimizer:
    ...
    **Example:**
    ...
    def step_and_cost(self, objective_fn, *args, nums_frequency=None, spectra=None, shifts=None, full_output=False, **kwargs):
        ...
        requires_grad = {arg_name: qml.math.requires_grad(arg) for arg_name, arg in zip(arg_names, args)}
        ...
        for arg_idx, (arg, arg_name) in enumerate(zip(args, arg_names)):
            ...
            for par_idx in indices:
                ...
                if num_freq == 1 or (spectrum is not None and len(spectrum[spectrum > 0])) == 1:
                    ...
                    univariate = _restrict_to_univariate(objective_fn, arg_idx, par_idx, _args, kwargs)
                    ...
                    recon_fn = qml.fourier.reconstruct(objective_fn, ids, _nums_frequency, _spectra, shifts)
                    recon = recon_fn(*before_args, arg, *after_args, f0=_fun_at_zero, **kwargs)[arg_name][par_idx]
                    if spectrum is None:
    ...
    def step(self, objective_fn, *args, nums_frequency=None, spectra=None, shifts=None, full_output=False, **kwargs):
        ...
        """
        x_new, _, *y_output = self.step_and_cost(objective_fn, *args, nums_frequency=nums_frequency, spectra=spectra, shifts=shifts, full_output=full_output, **kwargs)
        if full_output:
