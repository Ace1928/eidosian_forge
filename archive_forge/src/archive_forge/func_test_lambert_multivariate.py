...
def test_lambert_multivariate():
    ...
    assert solve((x ** 2 - 2 * x + 1).subs(x, (log(x) + 3 * x) ** 2 - 1)) == [LambertW(3 * exp(-sqrt(2))) / 3, LambertW(3 * exp(sqrt(2))) / 3]
    assert solve((x ** 2 - 2 * x - 2).subs(x, log(x) + 3 * x)) == [LambertW(3 * exp(1 - sqrt(3))) / 3, LambertW(3 * exp(1 + sqrt(3))) / 3]
