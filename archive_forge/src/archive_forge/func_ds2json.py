def ds2json(ds, u_var, v_var, lat_dim='latitude', lon_dim='longitude', units=None):
    ...
    for var_name in (u_var, v_var):
        ...
        if set(var_dims) != set([lat_dim, lon_dim]):
            raise ValueError("Invalid dimensions for variable '{}' in Dataset: should include only {}, found {}.".format(var_name, (lat_dim, lon_dim), var_dims))
    ...
    if units is None:
        ...
        if u_var_units != v_var_units:
            raise ValueError("Different units found for U-component '{}' and V-component '{}' variables: '{}' and '{}'".format(u_var, v_var, u_var_units, v_var_units))
        units = u_var_units
    if units is None:
        units = ''
    ...
    for p_number, p_name, var_name in zip(*u_v_spec):
        velocity_data.append({'header': {'parameterUnit': units, 'parameterNumber': p_number, 'dx': dx, 'dy': dy, 'parameterNumberName': p_name, 'la1': lat_upper, 'la2': lat_lower, 'parameterCategory': 2, 'lo2': lon_right, 'nx': nx, 'ny': ny, 'refTime': '2017-02-01 23:00:00', 'lo1': lon_left}, 'data': ds[var_name].values.flatten().tolist()})
