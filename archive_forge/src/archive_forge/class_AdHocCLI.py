...
class AdHocCLI(CLI):
    ...
    def init_parser(self):
        ...
        super(AdHocCLI, self).init_parser(usage='%prog <host-pattern> [options]', desc="Define and run a single task 'playbook' against a set of hosts", epilog='Some actions do not make sense in Ad-Hoc (include, meta, etc)')
        ...
        self.parser.add_argument('-a', '--args', dest='module_args', help='The action\'s options in space separated k=v format: -a \'opt1=val1 opt2=val2\' or a json string: -a \'{"opt1": "val1", "opt2": "val2"}\'', default=C.DEFAULT_MODULE_ARGS)
        self.parser.add_argument('-m', '--module-name', dest='module_name', help='Name of the action to execute (default=%s)' % C.DEFAULT_MODULE_NAME, default=C.DEFAULT_MODULE_NAME)
    ...
    def run(self):
        ...
        if context.CLIARGS['module_name'] in C._ACTION_IMPORT_PLAYBOOK:
            raise AnsibleOptionsError("'%s' is not a valid action for ad-hoc commands" % context.CLIARGS['module_name'])
        ...
        self._tqm = None
        try:
            self._tqm = TaskQueueManager(inventory=inventory, variable_manager=variable_manager, loader=loader, passwords=passwords, stdout_callback=cb, run_additional_callbacks=C.DEFAULT_LOAD_CALLBACK_PLUGINS, run_tree=run_tree, forks=context.CLIARGS['forks'])
