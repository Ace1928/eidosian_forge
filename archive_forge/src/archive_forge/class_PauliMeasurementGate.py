...
class PauliMeasurementGate(raw_types.Gate):
    ...
    def __init__(self, observable: Union['cirq.BaseDensePauliString', Iterable['cirq.Pauli']], key: Union[str, 'cirq.MeasurementKey']='') -> None:
        ...
        if not all((isinstance(p, pauli_gates.Pauli) for p in cast(Iterable['cirq.Gate'], observable))):
            raise ValueError(f'Pauli observable {observable} must be Iterable[`cirq.Pauli`].')
        coefficient = observable.coefficient if isinstance(observable, dps.BaseDensePauliString) else 1
        if coefficient not in [+1, -1]:
            raise ValueError(f'`cirq.DensePauliString` observable {observable} must have coefficient +1/-1.')
    ...
    def _with_rescoped_keys_(self, path: Tuple[str, ...], bindable_keys: FrozenSet['cirq.MeasurementKey']) -> 'PauliMeasurementGate':
    ...
    def _decompose_(self, qubits: Tuple['cirq.Qid', ...]) -> 'protocols.decompose_protocol.DecomposeResult':
        any_qubit = qubits[0]
        ...
        yield to_z_ops
        yield xor_decomp
        yield measurement_gate.MeasurementGate(1, self.mkey, invert_mask=(self._observable.coefficient != 1,)).on(any_qubit)
        yield protocols.inverse(xor_decomp)
        yield protocols.inverse(to_z_ops)

    def _circuit_diagram_info_(self, args: 'cirq.CircuitDiagramInfoArgs') -> 'cirq.CircuitDiagramInfo':
        coefficient = '' if self._observable.coefficient == 1 else '-'
        symbols = [f'M({('' if i else coefficient)}{self._observable[i]})' for i in range(len(self._observable))]
