...
class GandiLiveDNSDriver(BaseGandiLiveDriver, DNSDriver):
    ...
    RECORD_TYPE_MAP = {RecordType.A: 'A', RecordType.AAAA: 'AAAA', RecordType.ALIAS: 'ALIAS', RecordType.CNAME: 'CNAME', RecordType.DNAME: 'DNAME', RecordType.DS: 'DS', RecordType.KEY: 'KEY', RecordType.LOC: 'LOC', RecordType.MX: 'MX', RecordType.NS: 'NS', RecordType.PTR: 'PTR', RecordType.SPF: 'SPF', RecordType.SRV: 'SRV', RecordType.SSHFP: 'SSHFP', RecordType.TLSA: 'TLSA', RecordType.TXT: 'TXT', RecordType.WKS: 'WKS', RecordType.CAA: 'CAA'}
    ...
    def create_zone(self, domain, type='master', ttl=None, extra=None):
        ...
        try:
            new_zone = self.connection.request(action='%s/zones' % API_BASE, method='POST', data=zone_data)
    ...
    def update_record(self, record, name, type, data, extra):
        ...
        action = '{}/domains/{}/records/{}/{}'.format(API_BASE, record.zone.id, record.name, self.RECORD_TYPE_MAP[record.type])
        ...
        if multiple_value_record and len(other_records) > 0:
            ...
            for other_record in other_records:
                if record.type == RecordType.MX:
                    rvalue.append('{} {}'.format(other_record['extra']['priority'], other_record['data']))
                else:
        ...
        try:
            ...
            raise RecordDoesNotExistError(value='', driver=self.connection.driver, record_id=record.id)
    ...
    def _to_zone(self, zone):
        ...
        return Zone(id=str(zone['fqdn']), domain=zone['fqdn'], type='master', ttl=0, driver=self, extra=extra)
