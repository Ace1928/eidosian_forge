...
class ShareServicesTestCase(base.OSCClientTestBase):
    ...
    def test_services_list(self):
        ...
        self.assertTableStruct(services, ['ID', 'Binary', 'Host', 'Zone', 'Status', 'State', 'Updated At'])
        self.assertTrue(len(services) > 0)
        first_service = services[0]
        services = self.list_services(host=first_service['Host'], status=first_service['Status'], state=first_service['State'])
