...
class MainMenu(Menu):
    ...
    async def update_cursor_position_down(self) -> None:
        """
        Updates the cursor position when the DOWN key is pressed, ensuring thread safety.
        """
        ...
        if self.state == "BFS":
            ...
            self.cursor_rect.midtop = (int(self.ASTARx + self.offset), int(self.ASTARy))
            self.state = "ASTAR"
    ...
    async def update_cursor_position_up(self) -> None:
        """
        Updates the cursor position when the UP key is pressed, ensuring thread safety.
        """
