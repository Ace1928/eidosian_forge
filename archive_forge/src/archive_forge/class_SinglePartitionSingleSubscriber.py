...
class SinglePartitionSingleSubscriber(PermanentFailable, AsyncSingleSubscriber, SubscriberResetHandler):
    ...
    def __init__(self, subscriber_factory: ResettableSubscriberFactory, flow_control_settings: FlowControlSettings, ack_set_tracker: AckSetTracker, nack_handler: NackHandler, transformer: MessageTransformer):
        super().__init__()
    ...
    async def __aenter__(self):
        ...
        self._underlying.allow_flow(FlowControlRequest(allowed_messages=self._flow_control_settings.messages_outstanding, allowed_bytes=self._flow_control_settings.bytes_outstanding))
        return self
