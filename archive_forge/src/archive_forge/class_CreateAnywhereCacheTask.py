...
class CreateAnywhereCacheTask(task.Task):
    ...
    def execute(self, task_status_queue=None):
        log.status.Print('Creating a cache instance for bucket {} in zone {}...'.format(self._bucket_url, self._zone))
        provider = self._bucket_url.scheme
        api_client = api_factory.get_api(provider)
        response = api_client.create_anywhere_cache(self._bucket_url.bucket_name, self._zone, admission_policy=self._admission_policy, ttl=self._ttl)
        log.status.Print('Initiated the operation id: {} for creating a cache instance for bucket {} in zone {}...'.format(response.name, self._bucket_url, self._zone))
    ...
    def __eq__(self, other):
        ...
        return self._bucket_url == other._bucket_url and self._zone == other._zone and (self._admission_policy == other._admission_policy) and (self._ttl == other._ttl)