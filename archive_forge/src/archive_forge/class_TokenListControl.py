        ...
        def get_cursor_position():
            ...
            for y, line in enumerate(token_lines):
                ...
                for token, text in line:
                    ...
                    x += len(text)
            return None
        key = (default_char.char, default_char.token, tuple(tokens_with_mouse_handlers), width, right, center)

        def get_content():
            return UIContent(get_line=lambda i: token_lines[i], line_count=len(token_lines), default_char=default_char, cursor_position=get_cursor_position())
