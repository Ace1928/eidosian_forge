...
class DatabricksDeploymentClient(BaseDeploymentClient):
    ...
    def _call_endpoint(self, *, method: str, prefix: str='/api/2.0', route: Optional[str]=None, json_body: Optional[Dict[str, Any]]=None, timeout: Optional[int]=None):
        call_kwargs = {}
        if method.lower() == 'get':
            call_kwargs['params'] = json_body
        else:
            call_kwargs['json'] = json_body
        response = http_request(host_creds=get_databricks_host_creds(self.target_uri), endpoint=posixpath.join(prefix, 'serving-endpoints', route or ''), method=method, timeout=MLFLOW_HTTP_REQUEST_TIMEOUT.get() if timeout is None else timeout, raise_on_status=False, retry_codes=MLFLOW_DEPLOYMENT_CLIENT_REQUEST_RETRY_CODES, extra_headers={'X-Databricks-Endpoints-API-Client': 'Databricks Deployment Client'}, **call_kwargs)
    ...
    def predict(self, deployment_name=None, inputs=None, endpoint=None):
        ...
        """
        return self._call_endpoint(method='POST', prefix='/', route=posixpath.join(endpoint, 'invocations'), json_body=inputs, timeout=MLFLOW_DEPLOYMENT_PREDICT_TIMEOUT.get())
    ...
    def update_endpoint(self, endpoint, config=None):
        ...
        if list(config) == ['rate_limits']:
            return self._call_endpoint(method='PUT', route=posixpath.join(endpoint, 'rate-limits'), json_body=config)
        else:
            return self._call_endpoint(method='PUT', route=posixpath.join(endpoint, 'config'), json_body=config)
