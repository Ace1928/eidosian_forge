...
class TestGridPlots(ComparisonTestCase):
    ...
    def setUp(self):
        ...
        self.da_rgb_by_time = xr.DataArray(np.arange(24).reshape((2, 3, 2, 2)), coords, ['time', 'band', 'y', 'x'])
        ...
        self.xarr_with_attrs = xr.DataArray(np.random.rand(10, 10), coords=[('x', range(10)), ('y', range(10))], dims=['y', 'x'], attrs={'long_name': 'luminosity', 'units': 'lm'})
        ...
        self.big_img = xr.DataArray(np.arange(-1000000.0, 1000000.0).reshape(1000, 2000))
        self.ds = xr.Dataset({'temp': (('lon', 'lat'), 15 + 8 * np.random.randn(2, 2)), 'precip': (('lon', 'lat'), 10 * np.random.rand(2, 2))}, coords={'lon': [-99.83, -99.32], 'lat': [42.25, 42.21]})
        xs = np.linspace(0, 10, 5)
        lon = xs * xs[np.newaxis, :].T
        lat = xs + xs[:, np.newaxis]
        coords = {'lon': (('ny', 'nx'), lon), 'lat': (('ny', 'nx'), lat), 'time': [1, 2, 3], 'samples': ('nsamples', [0, 1, 2, 3])}
        self.ds_unindexed = xr.DataArray(np.random.rand(5, 5, 3, 4), coords=coords, dims=('nx', 'ny', 'time', 'nsamples'))
