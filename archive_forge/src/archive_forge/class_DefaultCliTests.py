...
class DefaultCliTests(utils.TestCase):
    ...
    def test_no_auth_url(self):
        exc = self.assertRaises(exceptions.CommandError, self.new_plugin, ['--os-username', uuid.uuid4().hex])
    ...
    def test_prompt_password(self, mock_getpass, mock_stdin):
        ...
        p = self.new_plugin(['--os-auth-url', uuid.uuid4().hex, '--os-username', uuid.uuid4().hex])
        self.assertEqual(password, p._password)
