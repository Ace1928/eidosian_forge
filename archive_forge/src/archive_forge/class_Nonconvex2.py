...
class Nonconvex2(ConcreteModel):
    ...
    def __init__(self, *args, **kwargs):
        ...
        m = self
        m.x1 = Var(within=Reals, bounds=(0, 0.997))
        ...
        m.c1 = Constraint(expr=-log(1 - m.x1) + log(0.1) * m.y1 + log(0.2) * m.y2 + log(0.15) * m.y3 == 0)
        m.c2 = Constraint(expr=-log(1 - m.x2) + log(0.05) * m.y4 + log(0.2) * m.y5 + log(0.15) * m.y6 == 0)
        m.c3 = Constraint(expr=-log(1 - m.x3) + log(0.02) * m.y7 + log(0.06) * m.y8 == 0)
        m.c4 = Constraint(expr=-m.y1 - m.y2 - m.y3 <= -1)
        m.c5 = Constraint(expr=-m.y4 - m.y5 - m.y6 <= -1)
        m.c6 = Constraint(expr=-m.y7 - m.y8 <= -1)
        m.c7 = Constraint(expr=3 * m.y1 + m.y2 + 2 * m.y3 + 3 * m.y4 + 2 * m.y5 + m.y6 + 3 * m.y7 + 2 * m.y8 <= 10)
