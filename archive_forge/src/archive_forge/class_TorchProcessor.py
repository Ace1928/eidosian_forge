...
class TorchProcessor:
    ...
    def __init__(self, config: Dict, tokenizer_path: PathLike):
        ...
        self._image_transform = Compose([Resize(self._image_size, interpolation=InterpolationMode.BICUBIC), convert_to_rgb, CenterCrop(self._image_size), ToTensor(), Normalize(mean=(0.48145466, 0.4578275, 0.40821073), std=(0.26862954, 0.26130258, 0.27577711))])
    ...
    def preprocess_text(self, texts: Union[str, List[str]]) -> Dict[str, Tensor]:
        ...
        if isinstance(texts, str):
            texts = [texts]
        input_ids = torch.full((len(texts), self._max_seq_len), fill_value=self._pad_token_idx, dtype=torch.int64)
        attention_mask = torch.zeros(len(texts), self._max_seq_len, dtype=torch.int32)
        encoded = self._tokenizer.encode_batch(texts)
        for i, seq in enumerate(encoded):
            seq_len = min(len(seq), self._max_seq_len)
            input_ids[i, :seq_len] = torch.LongTensor(seq.ids[:seq_len])
