...
class KANWrapper:
    def __init__(
        ...
        self.optimizer = torch.optim.Adam(
            self.model.parameters(), lr=self.learnable_params.get("lr", 0.01)
        )
        logging.info(
    ...
    def _initialize_parameters(self) -> None:
        """
        Initialize all model parameters with log-normal distribution.
        """
...
class KANGUI:
    ...
    def create_widgets(self) -> None:
        ...
        self._create_additional_frame()
        self._create_default_frame()

    def _create_param_frame(self) -> None:
        """
        Create and pack the parameter frame widgets.
        """
        param_frame = tk.Frame(self.root)
        param_frame.pack(pady=10)

        self._add_label_and_entry(param_frame, "Width:", 0, 0)
        self.width_entry = self._add_entry(param_frame, 0, 1)

        self._add_label_and_entry(param_frame, "Grid:", 1, 0)
        self.grid_entry = self._add_entry(param_frame, 1, 1)

        self._add_label_and_entry(param_frame, "Neurons (k):", 2, 0)
        self.k_entry = self._add_entry(param_frame, 2, 1)

        tk.Button(
            param_frame, text="Initialize Model", command=self.initialize_model
        ).grid(row=3, columnspan=2, pady=10)

    def _create_train_frame(self) -> None:
        """
        Create and pack the training frame widgets.
        """
        train_frame = tk.Frame(self.root)
        train_frame.pack(pady=10)

        self._add_label_and_entry(train_frame, "Learning Rate:", 0, 0)
        self.lr_entry = self._add_entry(train_frame, 0, 1)

        self._add_label_and_entry(train_frame, "Epochs:", 1, 0)
        self.epochs_entry = self._add_entry(train_frame, 1, 1)

        tk.Button(train_frame, text="Train Model", command=self.train_model).grid(
            row=2, columnspan=2, pady=10
        )

    def _create_file_frame(self) -> None:
        """
        Create and pack the file frame widgets.
        """
    ...
    def _create_plot_frame(self) -> None:
        """
        Create and pack the plot frame widgets.
        """
    ...
    def _create_additional_frame(self) -> None:
        """
        Create and pack the additional functionalities frame widgets.
        """
    ...
    def _create_default_frame(self) -> None:
        """
        Create and pack the default model initialization frame widgets.
        """
    ...
    def _add_label_and_entry(
        self, frame: tk.Frame, text: str, row: int, column: int
    ) -> None:
        """
        Add a label and entry widget to the specified frame.

        Args:
            frame (tk.Frame): The frame to add the widgets to.
            text (str): The text for the label.
            row (int): The row position of the label.
            column (int): The column position of the label.
        """
        tk.Label(frame, text=text).grid(row=row, column=column)

    def _add_entry(self, frame: tk.Frame, row: int, column: int) -> tk.Entry:
        """
        Add an entry widget to the specified frame.

        Args:
            frame (tk.Frame): The frame to add the entry to.
            row (int): The row position of the entry.
            column (int): The column position of the entry.
