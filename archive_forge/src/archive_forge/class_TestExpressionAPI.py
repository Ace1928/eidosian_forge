...
class TestExpressionAPI(unittest.TestCase):
    ...
    def test_deprecated_functions(self):
        ...
        self.assertIs(e, f)
        self.assertIs(type(e), PowExpression)
        self.assertIn('DEPRECATED: The implicit recasting of a "not potentially variable" expression node to a potentially variable one is no longer supported', LOG.getvalue().replace('\n', ' '))
        ...
        with LoggingIntercept() as LOG:
            f = e.add(5)
        self.assertIn('DEPRECATED: SumExpression.add() is deprecated.  Please use regular Python operators', LOG.getvalue().replace('\n', ' '))
    ...
    def test_pow(self):
        ...
        self.assertEqual(e.polynomial_degree(), None)
        self.assertEqual(is_fixed(e), False)
        with self.assertRaisesRegex(ValueError, 'No value for uninitialized NumericValue object y'):
    ...
    def test_linear(self):
        ...
        self.assertEqual(e.to_string(verbose=True), 'sum(mon(0, x[0]), mon(1, x[1]), mon(2, x[2]), 5, mon(1, y), -3)')
        ...
        with self.assertRaisesRegex(ValueError, 'Cannot specify both args and any of {constant, linear_coefs, or linear_vars}'):
            LinearExpression(5, constant=5)
        with self.assertRaisesRegex(ValueError, 'linear_vars \\(\\[y\\]\\) is not compatible with linear_coefs \\(\\[3, 5\\]\\)'):
    ...
    def test_expr_if(self):
        ...
        self.assertEqual(is_fixed(e), False)
        self.assertEqual(value(e), 5)
        self.assertEqual(str(e), 'Expr_if( ( 5  <=  y ), then=( x[0] + 5 ), else=( x[1]**2 ) )')
        self.assertEqual(e.to_string(verbose=True), 'Expr_if( ( 5  <=  y ), then=( sum(mon(1, x[0]), 5) ), else=( pow(x[1], 2) ) )')
        m.y.fix()
        self.assertTrue(e.is_potentially_variable())
        self.assertEqual(e.polynomial_degree(), 1)
        self.assertEqual(is_fixed(e), False)
