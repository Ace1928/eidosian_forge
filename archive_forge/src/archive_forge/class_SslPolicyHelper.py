...
class SslPolicyHelper(object):
    ...
    """
        if ssl_policy_ref.Collection() == 'compute.regionSslPolicies':
            operation_poller = poller.Poller(self._client.regionSslPolicies, ssl_policy_ref)
    ...
    """
        if ref.Collection() == 'compute.regionSslPolicies':
            request = self._messages.ComputeRegionSslPoliciesInsertRequest(project=ref.project, region=ref.region, sslPolicy=ssl_policy)
            operation = self._client.regionSslPolicies.Insert(request)
            return self._resources.Parse(operation.selfLink, collection='compute.regionOperations')
        request = self._messages.ComputeSslPoliciesInsertRequest(project=ref.project, sslPolicy=ssl_policy)
        operation = self._client.sslPolicies.Insert(request)
        return self._resources.Parse(operation.selfLink, collection='compute.globalOperations')
    ...
    """
        if ref.Collection() == 'compute.regionSslPolicies':
            request = self._messages.ComputeRegionSslPoliciesGetRequest(project=ref.project, region=ref.region, sslPolicy=ref.Name())
            return self._client.regionSslPolicies.Get(request)
        request = self._messages.ComputeSslPoliciesGetRequest(project=ref.project, sslPolicy=ref.Name())
    ...
    """
        ...
        if ref.Collection() == 'compute.regionSslPolicies':
            request = self._messages.ComputeRegionSslPoliciesPatchRequest(project=ref.project, region=ref.region, sslPolicy=ref.Name(), sslPolicyResource=ssl_policy)
            with self._client.IncludeFields(cleared_fields):
                operation = self._client.regionSslPolicies.Patch(request)
            return self._resources.Parse(operation.selfLink, collection='compute.regionOperations')
        request = self._messages.ComputeSslPoliciesPatchRequest(project=ref.project, sslPolicy=ref.Name(), sslPolicyResource=ssl_policy)
        with self._client.IncludeFields(cleared_fields):
    ...
    """
        if ref.Collection() == 'compute.regionSslPolicies':
            request = self._messages.ComputeRegionSslPoliciesDeleteRequest(project=ref.project, region=ref.region, sslPolicy=ref.Name())
            operation = self._client.regionSslPolicies.Delete(request)
            return self._resources.Parse(operation.selfLink, collection='compute.regionOperations')
        request = self._messages.ComputeSslPoliciesDeleteRequest(project=ref.project, sslPolicy=ref.Name())
    ...
    """
        if region:
            request = self._messages.ComputeRegionSslPoliciesListAvailableFeaturesRequest(project=project, region=region)
            return self._client.regionSslPolicies.ListAvailableFeatures(request).features
        request = self._messages.ComputeSslPoliciesListAvailableFeaturesRequest(project=project)
