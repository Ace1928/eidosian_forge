...
def test_migrate_before_expand(self, mock_validate_engine, mock_get_alembic_branch_head, mock_get_current_alembic_heads, mock_has_pending_migrations):
    ...
    self.db.migrate()
    self.assertIn('Database migration is up to date. No migration needed.', self.output.getvalue())