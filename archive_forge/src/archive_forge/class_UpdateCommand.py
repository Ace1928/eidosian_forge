...
class UpdateCommand(Command):
    ...
    command_spec = Command.CreateCommandSpec('update', command_name_aliases=['refresh'], usage_synopsis=_SYNOPSIS, min_args=0, max_args=1, supported_sub_args='fn', file_url_ok=True, provider_url_ok=False, urls_start_arg=0, gs_api_support=[ApiSelector.XML, ApiSelector.JSON], gs_default_api=ApiSelector.JSON)
    help_spec = Command.HelpSpec(help_name='update', help_name_aliases=['refresh'], help_type='command_help', help_one_line_summary='Update to the latest gsutil release', help_text=_DETAILED_HELP_TEXT, subcommand_help_text={})
    ...
    unsafe_update_dirs = ['applications', 'auto', 'bin', 'boot', 'desktop', 'dev', 'documents and settings', 'etc', 'export', 'home', 'kernel', 'lib', 'lib32', 'library', 'lost+found', 'mach_kernel', 'media', 'mnt', 'net', 'null', 'network', 'opt', 'private', 'proc', 'program files', 'python', 'root', 'sbin', 'scripts', 'srv', 'sys', 'system', 'tmp', 'users', 'usr', 'var', 'volumes', 'win', 'win32', 'windows', 'winnt']
    ...
    def RunCommand(self):
        ...
        if gslib.IS_PACKAGE_INSTALL:
            raise CommandException('The update command is only available for gsutil installed from a tarball. If you installed gsutil via another method, use the same method to update it.')
        if system_util.InvokedViaCloudSdk():
            raise CommandException('The update command is disabled for Cloud SDK installs. Please run "gcloud components update" to update it. Note: the Cloud SDK incorporates updates to the underlying tools approximately every 2 weeks, so if you are attempting to update to a recently created release / pre-release of gsutil it may not yet be available via the Cloud SDK.')
        https_validate_certificates = CERTIFICATE_VALIDATION_ENABLED
        if not https_validate_certificates:
            raise CommandException('Your boto configuration has https_validate_certificates = False.\nThe update command cannot be run this way, for security reasons.')
        ...
        if self.args:
            ...
            for i, result in enumerate(self.WildcardIterator(update_from_url_str)):
                if i > 0:
                    raise CommandException('Invalid update URL. Must name a single .tar.gz file.')
                storage_url = result.storage_url
                if storage_url.IsFileUrl() and (not storage_url.IsDirectory()):
                    if not force_update:
                        raise CommandException('"update" command does not support "file://" URLs without the -f option.')
                elif not (storage_url.IsCloudUrl() and storage_url.IsObject()):
                    raise CommandException('Invalid update object URL. Must name a single .tar.gz file.')
        else:
        ...
        if not force_update and gslib.VERSION == tarball_version:
            self._CleanUpUpdateCommand(tf, dirs_to_remove, old_cwd)
            if self.args:
                raise CommandException('You already have %s installed.' % update_from_url_str, informational=True)
            else:
        ...
        if not no_prompt:
            ...
            if major:
                print('\n'.join(textwrap.wrap('This command will update to the "%s" version of gsutil at %s. NOTE: This a major new version, so it is strongly recommended that you review the release note details at %s before updating to this version, especially if you use gsutil in scripts.' % (tarball_version, gslib.GSUTIL_DIR, RELEASE_NOTES_URL))))
            else:
                print('This command will update to the "%s" version of\ngsutil at %s' % (tarball_version, gslib.GSUTIL_DIR))
        ...
        if not system_util.IS_WINDOWS:
            for dirname, subdirs, filenames in os.walk(new_dir):
                for filename in filenames:
                    ...
                    os.fchmod(fd, stat.S_IWRITE | stat.S_IRUSR | stat.S_IRGRP | stat.S_IROTH)
                    os.close(fd)
                for subdir in subdirs:
                    fd = os.open(os.path.join(dirname, subdir), os.O_RDONLY)
                    os.fchmod(fd, stat.S_IRWXU | stat.S_IXGRP | stat.S_IXOTH | stat.S_IRGRP | stat.S_IROTH)
                    os.close(fd)
