...
class AzureRMVirtualMachineScaleSet(AzureRMModuleBase):
    ...
    def __init__(self):
        ...
        self.results = dict(changed=False, actions=[], ansible_facts=dict(azure_vmss=None))
        super(AzureRMVirtualMachineScaleSet, self).__init__(derived_arg_spec=self.module_arg_spec, supports_check_mode=True, mutually_exclusive=mutually_exclusive)
    ...
    def exec_module(self, **kwargs):
        ...
        try:
            ...
            if self.state == 'present':
                ...
                if self.max_price and vmss_dict['virtual_machine_profile'].get('billing_profile', None) and (self.max_price != vmss_dict['virtual_machine_profile']['billing_profile'].get('max_price', None)):
                ...
                if self.ephemeral_os_disk and current_ephemeral is None:
                    ...
                    self.fail('Ephemeral OS disk not updatable: virtual machine scale set ephemeral OS disk is {0}'.format(self.ephemeral_os_disk))
                if self.os_disk_size_gb and self.os_disk_size_gb != vmss_dict['virtual_machine_profile']['storage_profile']['os_disk']['disk_size_gb']:
                    self.fail('VMSS OS disk size is not updatable: requested virtual machine OS disk size is {0}'.format(self.os_disk_size_gb))
                if self.os_disk_caching and self.os_disk_caching != vmss_dict['virtual_machine_profile']['storage_profile']['os_disk']['caching']:
                ...
                if self.data_disks and len(self.data_disks) != len(vmss_dict['virtual_machine_profile']['storage_profile'].get('data_disks', [])):
                    self.log('CHANGED: virtual machine scale set {0} - Data Disks'.format(self.name))
                    differences.append('Data Disks')
                    changed = True
                if self.upgrade_policy and self.upgrade_policy != vmss_dict['upgrade_policy']['mode']:
                ...
                if image_reference and image_reference.as_dict() != vmss_dict['virtual_machine_profile']['storage_profile']['image_reference']:
                ...
                if self.terminate_event_timeout_minutes:
                    ...
                    old = vmss_dict['virtual_machine_profile'].get('scheduled_events_profile', {}).get('terminate_notification_profile', {}).get('not_before_timeout', '')
                    if old != iso_8601_format:
                        differences.append('terminateNotification')
                        changed = True
                        vmss_dict['virtual_machine_profile'].setdefault('scheduled_events_profile', {})['terminate_notification_profile'] = {'not_before_timeout': iso_8601_format, 'enable': 'true'}
                ...
                if self.security_profile is not None:
                    ...
                    if 'securityProfile' not in vmss_dict['virtual_machine_profile'].keys():
                        ...
                        if self.security_profile.get('encryption_at_host') is not None:
                            if bool(self.security_profile.get('encryption_at_host')) != bool(vmss_dict['virtual_machine_profile']['security_profile']['encryption_at_host']):
                                update_security_profle = True
                            else:
                                self.security_profile['encryption_at_host'] = vmss_dict['virtual_machine_profile']['security_profile']['encryption_at_host']
                        if self.security_profile.get('security_type') is not None:
                            if self.security_profile.get('security_type') != vmss_dict['virtual_machine_profile']['security_profile']['security_type']:
                                update_security_profile = True
                        if self.security_profile.get('uefi_settings') is not None:
                            if self.security_profile['uefi_settings'].get('secure_boot_enabled') is not None:
                                if bool(self.security_profile['uefi_settings']['secure_boot_enabled']) != bool(vmss_dict['virtual_machine_profile']['security_profile']['uefi_settings']['secure_boot_nabled']):
                                    update_security_profile = True
                            else:
                                self.security_profile['uefi_settings']['secure_boot_enabled'] = vmss_dict['virtual_machine_profile']['security_profile']['uefi_settings']['secure_boot_enabled']
                            if self.security_profile['uefi_settings'].get('v_tpm_enabled') is not None:
                                if bool(self.security_profile['uefi_settings']['v_tpm_enabled']) != bool(vmss_dict['virtual_machine_profile']['security_profile']['uefi_settings']['v_tpm_enabled']):
                                    update_security_profile = True
                            else:
                                self.security_profile['uefi_settings']['v_tpm_enabled'] = vmss_dict['virtual_machine_profile']['security_profile']['uefi_settings']['v_tpm_enabled']
            ...
            if self.state == 'present':
                if not vmss:
                    ...
                    if self.priority == 'Spot':
                        ...
                        vmss_resource.virtual_machine_profile.billing_profile = self.compute_models.BillingProfile(max_price=self.max_price)
                    ...
                    if support_lb_change:
                        if self.load_balancer:
                            ...
                            vmss_resource.virtual_machine_profile.network_profile.network_interface_configurations[0].ip_configurations[0].application_gateway_backend_address_pools = None
                            ...
                            vmss_resource.virtual_machine_profile.network_profile.network_interface_configurations[0].ip_configurations[0].load_balancer_backend_address_pools = None
                            vmss_resource.virtual_machine_profile.network_profile.network_interface_configurations[0].ip_configurations[0].load_balancer_inbound_nat_pools = None
