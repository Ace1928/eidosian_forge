...
def collective_reduce_v2(input: _atypes.TensorFuzzingAnnotation[TV_CollectiveReduceV2_T], group_size: _atypes.TensorFuzzingAnnotation[_atypes.Int32], group_key: _atypes.TensorFuzzingAnnotation[_atypes.Int32], instance_key: _atypes.TensorFuzzingAnnotation[_atypes.Int32], ordering_token: List[_atypes.TensorFuzzingAnnotation[_atypes.Resource]], merge_op: str, final_op: str, communication_hint: str='auto', timeout_seconds: float=0, max_subdivs_per_device: int=-1, name=None) -> _atypes.TensorFuzzingAnnotation[TV_CollectiveReduceV2_T]:
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'CollectiveReduceV2', name, input, group_size, group_key, instance_key, ordering_token, 'merge_op', merge_op, 'final_op', final_op, 'communication_hint', communication_hint, 'timeout_seconds', timeout_seconds, 'max_subdivs_per_device', max_subdivs_per_device)
            return _result
            ...
            pass
        try:
            return collective_reduce_v2_eager_fallback(input, group_size, group_key, instance_key, ordering_token, merge_op=merge_op, final_op=final_op, communication_hint=communication_hint, timeout_seconds=timeout_seconds, max_subdivs_per_device=max_subdivs_per_device, name=name, ctx=_ctx)
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('CollectiveReduceV2', input=input, group_size=group_size, group_key=group_key, instance_key=instance_key, ordering_token=ordering_token, merge_op=merge_op, final_op=final_op, communication_hint=communication_hint, timeout_seconds=timeout_seconds, max_subdivs_per_device=max_subdivs_per_device, name=name)
    _result = _outputs[:]
