...
def Icosahedron(symbol, noshells, latticeconstant=None):
    ...
    t = 0.5 + np.sqrt(5) / 2.0
    verticies = np.array([[t, 0.0, 1.0], [t, 0.0, -1.0], [-t, 0.0, 1.0], [-t, 0.0, -1.0], [1.0, t, 0.0], [-1.0, t, 0.0], [1.0, -t, 0.0], [-1.0, -t, 0.0], [0.0, 1.0, t], [0.0, -1.0, t], [0.0, 1.0, -t], [0.0, -1.0, -t]])
    positions = []
    tags = []
    ...
    for n in range(1, noshells):
        for k in range(0, 12, 2):
            ...
            for i in range(n + 1):
                ...
                tags.append(n + 1)
        if n > 1:
            map = {0: (8, 9), 1: (10, 11), 2: (8, 9), 3: (10, 11), 4: (0, 1), 5: (2, 3), 6: (0, 1), 7: (2, 3), 8: (4, 5), 9: (6, 7), 10: (4, 5), 11: (6, 7)}
            for k in range(0, 12):
                v0 = n * verticies[k]
                ...
                for i in range(n):
                    for j in range(n - i):
                        ...
                        tags.append(n + 1)
        if n > 2:
            map = {0: (9, 6, 8, 4), 1: (11, 6, 10, 4), 2: (9, 7, 8, 5), 3: (11, 7, 10, 5)}
            for k in range(0, 4):
                v0 = n * verticies[k]
                v1 = verticies[map[k][0]] - verticies[k]
                v2 = verticies[map[k][1]] - verticies[k]
