...
class TPUAcceleratorManager(AcceleratorManager):
    ...
    def get_current_node_accelerator_type() -> Optional[str]:
        ...
        def tpu_pod_type_to_ray_accelerator_type(tpu_pod_type: str) -> Optional[str]:
            return 'TPU-' + str(tpu_pod_type.split('-')[0].upper())
        ray_accelerator_type = None
        tpu_pod_type = TPUAcceleratorManager._get_current_node_tpu_pod_type()
        if tpu_pod_type is not None:
            ray_accelerator_type = tpu_pod_type_to_ray_accelerator_type(tpu_pod_type=tpu_pod_type)
            if ray_accelerator_type is None:
                logger.info(f'While trying to autodetect a TPU type, received malformed accelerator_type: {tpu_pod_type}')
        if ray_accelerator_type is None:
    ...
    def get_current_node_additional_resources() -> Optional[Dict[str, float]]:
        ...
        if tpu_name and worker_id is not None and tpu_pod_type:
            ...
            logging.info('Failed to configure TPU pod. Got: tpu_name: %s, worker_id: %s, accelerator_type: %s', tpu_name, worker_id, tpu_pod_type)
        if resources:
            return resources
        return None