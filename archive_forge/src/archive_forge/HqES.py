...
def heuristic(
    ...
    """
    # Calculate the base Euclidean distance between the current node and the goal node
    ...
    # Encourage the snake to maintain a dense configuration by rewarding positions that are closer to the center of the snake's body mass
    ...
    # Prioritize reaching the fruit once the snake is in a dense configuration
    if node1 == center_of_mass:
        fruit_priority = -euclidean_distance(node1, fruit_position)
    else:
        fruit_priority = 0

    # Calculate a penalty for deviating from the snake's current path to maintain a zigzag movement pattern for density
    if len(snake_body) > 1:
        direction_vector = (
    ...
    # Combine all the heuristic components with appropriate weighting
    return (
        base_distance
...
def optimize_path_for_density(
    ...
    while path_queue:
        ...
        if is_position_optimal_for_density(next_position, center_of_mass, snake_body):
            ...
            current_position = alternative_path[-1]
...
def is_position_optimal_for_density(
    ...
    # Consider a position optimal if it is closer to the center of mass and does not overlap with the snake's body
    return (
...
class Snake:
    ...
    def calculate_path(self) -> List[Tuple[int, int]]:
        ...
        return a_star(
            ...
            set(self.body),
            list(self.body),
            fruit_position=self.fruit.positon,
        )
