...
class TestNetworkPool(TestNetworkProxy):
    ...
    def test_port_delete(self):
        self.verify_delete(self.proxy.delete_port, port.Port, False, expected_kwargs={'if_revision': None})

    def test_port_delete_ignore(self):
        self.verify_delete(self.proxy.delete_port, port.Port, True, expected_kwargs={'if_revision': None})

    def test_port_delete_if_revision(self):
        self.verify_delete(self.proxy.delete_port, port.Port, True, method_kwargs={'if_revision': 42}, expected_kwargs={'if_revision': 42})
    ...
    def test_port_update(self):
        self.verify_update(self.proxy.update_port, port.Port, expected_kwargs={'x': 1, 'y': 2, 'z': 3, 'if_revision': None})

    def test_port_update_if_revision(self):
        self.verify_update(self.proxy.update_port, port.Port, method_kwargs={'x': 1, 'y': 2, 'z': 3, 'if_revision': 42}, expected_kwargs={'x': 1, 'y': 2, 'z': 3, 'if_revision': 42})
