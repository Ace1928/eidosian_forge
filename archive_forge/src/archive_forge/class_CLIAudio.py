...
class CLIAudio:
    ...
    def translate(args: CLITranslationArgs) -> None:
        with open(args.file, 'rb') as file_reader:
            buffer_reader = BufferReader(file_reader.read(), desc='Upload progress')
        model = get_client().audio.translations.create(file=(args.file, buffer_reader), model=args.model, temperature=args.temperature or NOT_GIVEN, prompt=args.prompt or NOT_GIVEN, response_format=cast(Any, args.response_format))
        print_model(model)