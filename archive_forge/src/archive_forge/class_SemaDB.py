...
class SemaDB(VectorStore):
    ...
    def __init__(self, collection_name: str, vector_size: int, embedding: Embeddings, distance_strategy: DistanceStrategy=DistanceStrategy.EUCLIDEAN_DISTANCE, api_key: str=''):
    ...
    def headers(self) -> dict:
        ...
        return {'content-type': 'application/json', 'X-RapidAPI-Key': self.api_key, 'X-RapidAPI-Host': SemaDB.HOST}
    ...
    def create_collection(self) -> bool:
        ...
        payload = {'id': self.collection_name, 'vectorSize': self.vector_size, 'distanceMetric': self._get_internal_distance_strategy()}
        response = requests.post(SemaDB.BASE_URL + '/collections', json=payload, headers=self.headers)
        return response.status_code == 200

    def delete_collection(self) -> bool:
        """Deletes the corresponding collection in SemaDB."""
        response = requests.delete(SemaDB.BASE_URL + f'/collections/{self.collection_name}', headers=self.headers)
        return response.status_code == 200
    ...
    def add_texts(self, texts: Iterable[str], metadatas: Optional[List[dict]]=None, batch_size: int=1000, **kwargs: Any) -> List[str]:
        ...
        if metadatas is not None:
            for text, embedding, metadata in zip(texts, embeddings, metadatas):
                new_id = str(uuid4())
                ids.append(new_id)
                points.append({'id': new_id, 'vector': embedding, 'metadata': {**metadata, **{'text': text}}})
        else:
            for text, embedding in zip(texts, embeddings):
                new_id = str(uuid4())
                ids.append(new_id)
                points.append({'id': new_id, 'vector': embedding, 'metadata': {'text': text}})
        for i in range(0, len(points), batch_size):
            batch = points[i:i + batch_size]
            response = requests.post(SemaDB.BASE_URL + f'/collections/{self.collection_name}/points', json={'points': batch}, headers=self.headers)
    ...
    def delete(self, ids: Optional[List[str]]=None, **kwargs: Any) -> Optional[bool]:
        ...
        """
        payload = {'ids': ids}
        response = requests.delete(SemaDB.BASE_URL + f'/collections/{self.collection_name}/points', json=payload, headers=self.headers)
    ...
    def similarity_search_with_score(self, query: str, k: int=4, **kwargs: Any) -> List[Tuple[Document, float]]:
        ...
        return [(Document(page_content=p['metadata']['text'], metadata=p['metadata']), p['distance']) for p in points]
    ...
    @classmethod
    def from_texts(cls, texts: List[str], embedding: Embeddings, metadatas: Optional[List[dict]]=None, collection_name: str='', vector_size: int=0, api_key: str='', distance_strategy: DistanceStrategy=DistanceStrategy.EUCLIDEAN_DISTANCE, **kwargs: Any) -> 'SemaDB':
        ...
        semadb = cls(collection_name, vector_size, embedding, distance_strategy=distance_strategy, api_key=api_key)
