    ...
    async def get(self, kernel_name, path, include_body=True):
        ...
        if hasattr(ksm, 'get_kernel_spec_resource'):
            ...
            if kernel_spec_res is not None:
                ...
                self.log.warning(f"Kernelspec resource '{path}' for '{kernel_name}' not found.  Kernel spec manager may not support resource serving. Falling back to reading from disk")
        try:
