...
class FlavorsTest(utils.TestCase):
    ...
    def test_find(self):
        ...
        self.assertRaises(exceptions.NotFound, self.cs.flavors.find, disk=12345)

    @staticmethod
    def _create_body(name, ram, vcpus, disk, ephemeral, id, swap, rxtx_factor, is_public):
        return {'flavor': {'name': name, 'ram': ram, 'vcpus': vcpus, 'disk': disk, 'OS-FLV-EXT-DATA:ephemeral': ephemeral, 'id': id, 'swap': swap, 'rxtx_factor': rxtx_factor, 'os-flavor-access:is_public': is_public}}

    def test_create(self):
        f = self.cs.flavors.create('flavorcreate', 512, 1, 10, 1234, ephemeral=10, is_public=False)
        self.assert_request_id(f, fakes.FAKE_REQUEST_ID_LIST)
        body = self._create_body('flavorcreate', 512, 1, 10, 10, 1234, 0, 1.0, False)
    ...
    def test_set_keys(self):
        ...
        self.cs.assert_called('POST', '/flavors/1/os-extra_specs', {'extra_specs': {'k1': 'v1'}})
    ...
    def test_unset_keys(self, mock_delete):
        f = self.cs.flavors.get(1)
        keys = ['k1', 'k2']
        mock_delete.return_value = base.TupleWithMeta((), fakes.FAKE_REQUEST_ID_LIST)
        fu = f.unset_keys(keys)
        self.assert_request_id(fu, fakes.FAKE_REQUEST_ID_LIST)
        mock_delete.assert_has_calls([mock.call('/flavors/1/os-extra_specs/k1'), mock.call('/flavors/1/os-extra_specs/k2')])