...
class Commit(base.Object, TraversableIterableObj, Diffable, Serializable):
    ...
    @classmethod
    def iter_items(cls, repo: 'Repo', rev: Union[str, 'Commit', 'SymbolicReference'], paths: Union[PathLike, Sequence[PathLike]]='', **kwargs: Any) -> Iterator['Commit']:
    ...
    def _serialize(self, stream: BytesIO) -> 'Commit':
        ...
        c = self.committer
        fmt = '%s %s <%s> %s %s\n'
        write((fmt % ('author', aname, a.email, self.authored_date, altz_to_utctz_str(self.author_tz_offset))).encode(self.encoding))
        aname = c.name
        write((fmt % ('committer', aname, c.email, self.committed_date, altz_to_utctz_str(self.committer_tz_offset))).encode(self.encoding))
    ...
    def _deserialize(self, stream: BytesIO) -> 'Commit':
        ...
        while buf:
            ...
            buf = readline().strip()
        try:
            self.author, self.authored_date, self.author_tz_offset = parse_actor_and_date(author_line.decode(self.encoding, 'replace'))
        except UnicodeDecodeError:
            _logger.error("Failed to decode author line '%s' using encoding %s", author_line, self.encoding, exc_info=True)
        try:
            self.committer, self.committed_date, self.committer_tz_offset = parse_actor_and_date(committer_line.decode(self.encoding, 'replace'))
        except UnicodeDecodeError:
            _logger.error("Failed to decode committer line '%s' using encoding %s", committer_line, self.encoding, exc_info=True)
        self.message = stream.read()
        try:
            self.message = self.message.decode(self.encoding, 'replace')
        except UnicodeDecodeError:
            _logger.error("Failed to decode message '%s' using encoding %s", self.message, self.encoding, exc_info=True)
        return self
    ...
    def co_authors(self) -> List[Actor]:
        ...
        co_authors = []
        if self.message:
            results = re.findall('^Co-authored-by: (.*) <(.*?)>$', self.message, re.MULTILINE)
