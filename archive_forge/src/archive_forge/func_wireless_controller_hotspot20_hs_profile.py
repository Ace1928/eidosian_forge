...
def wireless_controller_hotspot20_hs_profile(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('wireless-controller.hotspot20', 'hs-profile', filtered_data, vdom=vdom)
        current_data = fos.get('wireless-controller.hotspot20', 'hs-profile', vdom=vdom, mkey=mkey)
    ...
    if state == 'present' or state is True:
        return fos.set('wireless-controller.hotspot20', 'hs-profile', data=converted_data, vdom=vdom)
    elif state == 'absent':
        return fos.delete('wireless-controller.hotspot20', 'hs-profile', mkey=filtered_data['name'], vdom=vdom)
    else:
