...
class ImageCloudMixin:
    ...
    def download_image(self, name_or_id, output_path=None, output_file=None, chunk_size=1024 * 1024):
        ...
        if output_path is None and output_file is None:
            raise exceptions.SDKException('No output specified, an output path or file object is necessary to write the image data to')
        elif output_path is not None and output_file is not None:
            raise exceptions.SDKException('Both an output path and file object were provided, however only one can be used at once')
        image = self.image.find_image(name_or_id)
        if not image:
            raise exceptions.NotFoundException('No images with name or ID %s were found' % name_or_id, None)
        return self.image.download_image(image, output=output_file or output_path, chunk_size=chunk_size)
    ...
    def delete_image(self, name_or_id, wait=False, timeout=3600, delete_objects=True):
        ...
        if self.image_api_use_tasks and (self.image._IMAGE_OBJECT_KEY in image.properties or self.image._SHADE_IMAGE_OBJECT_KEY in image.properties):
            container, objname = image.properties.get(self.image._IMAGE_OBJECT_KEY, image.properties.get(self.image._SHADE_IMAGE_OBJECT_KEY)).split('/', 1)
            self.delete_object(container=container, name=objname)
        if wait:
            for count in utils.iterate_timeout(timeout, 'Timeout waiting for the image to be deleted.'):
                if self.get_image(image.id) is None:
                    break
        return True

    def create_image(self, name, filename=None, container=None, md5=None, sha256=None, disk_format=None, container_format=None, disable_vendor_agent=True, wait=False, timeout=3600, tags=None, allow_duplicates=False, meta=None, volume=None, **kwargs):
        """Upload an image.
        ...
        """
        if volume:
            image = self.block_storage.create_image(name=name, volume=volume, allow_duplicates=allow_duplicates, container_format=container_format, disk_format=disk_format, wait=wait, timeout=timeout)
        else:
            image = self.image.create_image(name, filename=filename, container=container, md5=md5, sha256=sha256, disk_format=disk_format, container_format=container_format, disable_vendor_agent=disable_vendor_agent, wait=wait, timeout=timeout, tags=tags, allow_duplicates=allow_duplicates, meta=meta, **kwargs)
        if not wait:
            return image
        try:
