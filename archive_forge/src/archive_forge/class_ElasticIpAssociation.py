...
class ElasticIpAssociation(resource.Resource):
    PROPERTIES = INSTANCE_ID, EIP, ALLOCATION_ID, NETWORK_INTERFACE_ID = ('InstanceId', 'EIP', 'AllocationId', 'NetworkInterfaceId')
    ...
    def handle_create(self):
        ...
        eip = self.properties[self.EIP]
        if eip:
            self.client_plugin().associate_floatingip_address(self.properties[self.INSTANCE_ID], eip)
            self.resource_id_set(eip)
            LOG.debug('ElasticIpAssociation %(instance)s.add_floating_ip(%(eip)s)', {'instance': self.properties[self.INSTANCE_ID], 'eip': eip})
    ...
    def handle_delete(self):
        ...
        if self.properties[self.EIP]:
            eip = self.properties[self.EIP]
            self._remove_floating_ip_address(eip, ignore_not_found=True)
        elif self.properties[self.ALLOCATION_ID]:
            float_id = self.properties[self.ALLOCATION_ID]
            self._neutron_update_floating_ip(float_id, port_id=None, ignore_not_found=True)

    def needs_replace_with_prop_diff(self, changed_properties_set, after_props, before_props):
        if self.ALLOCATION_ID in changed_properties_set or self.EIP in changed_properties_set:
    ...
    def handle_update(self, json_snippet, tmpl_diff, prop_diff):
        if prop_diff:
            ...
            elif self.INSTANCE_ID in prop_diff or self.NETWORK_INTERFACE_ID in prop_diff:
