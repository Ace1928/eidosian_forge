...
class CryptsetupEncryptorTestCase(test_base.VolumeEncryptorTestCase):
    ...
    def _create(self, mock_exists):
        return cryptsetup.CryptsetupEncryptor(connection_info=self.connection_info, root_helper=self.root_helper, keymgr=self.keymgr)
    ...
    def test__open_volume(self, mock_execute):
        ...
        mock_execute.assert_has_calls([mock.call('cryptsetup', 'create', '--key-file=-', self.dev_name, self.dev_path, process_input='passphrase', run_as_root=True, root_helper=self.root_helper, check_exit_code=True)])
    ...
    def test_attach_volume(self, mock_execute):
        ...
        mock_execute.assert_has_calls([mock.call('cryptsetup', 'create', '--key-file=-', self.dev_name, self.dev_path, process_input=fake_key, root_helper=self.root_helper, run_as_root=True, check_exit_code=True), mock.call('ln', '--symbolic', '--force', '/dev/mapper/%s' % self.dev_name, self.symlink_path, root_helper=self.root_helper, run_as_root=True, check_exit_code=True)])

    @mock.patch('os_brick.executor.Executor._execute')
    def test__close_volume(self, mock_execute):
        self.encryptor.detach_volume()
        mock_execute.assert_has_calls([mock.call('cryptsetup', 'remove', self.dev_name, root_helper=self.root_helper, run_as_root=True, check_exit_code=[0, 4])])
    ...
    def test_extend_volume(self):
        self.assertRaises(NotImplementedError, self.encryptor.extend_volume, mock.sentinel.context)