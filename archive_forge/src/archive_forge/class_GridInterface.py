...
class GridInterface(DictInterface):
    ...
    def init(cls, eltype, data, kdims, vdims):
        ...
        if isinstance(data, tuple):
            if len(data) != len(dimensions) and len(data) == ndims + 1 and (len(data[-1].shape) == ndims + 1):
                value_array = data[-1]
        ...
        elif not any((isinstance(data, tuple((t for t in interface.types if t is not None))) for interface in cls.interfaces.values())):
        ...
        for vdim in vdim_names:
            ...
            if not expected and shape == (1,) or (len(set((shape,) + shapes)) == 1 and len(shape) > 1):
                ...
                raise error('The shape of the %s value array does not match the expected dimensionality indicated by the key dimensions. Expected %d-D array, found %d-D array.' % (vdim, len(expected), len(shape)))
            elif any((e not in (s, s + 1) for s, e in zip(shape, valid_shape))):
                raise error(f'Key dimension values and value array {vdim} shapes do not match. Expected shape {valid_shape}, actual shape: {shape}', cls)
    ...
    def concat(cls, datasets, dimensions, vdims):
        ...
        with sorted_context(False):
            ...
            datasets = datasets.clone([(k, v.data if isinstance(v, Dataset) else v) for k, v in datasets.data.items()])
        if len(datasets.kdims) > 1:
            items = datasets.groupby(datasets.kdims[:-1]).data.items()
            return cls.concat([(k, cls.concat(v, v.kdims, vdims=vdims)) for k, v in items], datasets.kdims[:-1], vdims)
    ...
    def concat_dim(cls, datasets, dim, vdims):
        ...
        for vdim in vdims:
            ...
            if len(shapes) > 1:
                raise DataError(f'When concatenating gridded data the shape of arrays must match. {cls.__name__} found that arrays along the {vdim.name} dimension do not match.')
    ...
    def coords(cls, dataset, dim, ordered=False, expanded=False, edges=False):
        ...
        if dim in dataset.kdims:
            ...
            isedges = dim in dataset.kdims and len(shape) == dataset.ndims and (len(data) == shape[dataset.ndims - idx - 1] + 1)
        else:
            isedges = False
    ...
    def canonicalize(cls, dataset, data, data_coords=None, virtual_coords=None):
        ...
        dims = [name for name in data_coords if isinstance(cls.coords(dataset, name), get_array_types())]
    ...
    def persist(cls, dataset):
        ...
        return {k: v.persist() if da and isinstance(v, da.Array) else v for k, v in dataset.data.items()}

    @classmethod
    def compute(cls, dataset):
        da = dask_array_module()
        return {k: v.compute() if da and isinstance(v, da.Array) else v for k, v in dataset.data.items()}

    @classmethod
    def values(cls, dataset, dim, expanded=True, flat=True, compute=True, keep_index=False, canonicalize=True):
        dim = dataset.get_dimension(dim, strict=True)
    ...
    def groupby(cls, dataset, dim_names, container_type, group_type, **kwargs):
        ...
        if invalid:
            if len(invalid) == 1:
                invalid = f"'{invalid[0]}'"
            raise ValueError('Cannot groupby irregularly sampled dimension(s) %s.' % invalid)
        group_kwargs = {}
    ...
    def select(cls, dataset, selection_mask=None, **selection):
        ...
        if val_dims:
            raise IndexError('Cannot slice value dimensions in compressed format, convert to expanded format before slicing.')
        ...
        for i, (dim, ind) in enumerate(full_selection):
            ...
            if irregular:
                if np.isscalar(ind) or isinstance(ind, (set, list)):
                    raise IndexError(f'Indexing not supported for irregularly sampled data. {ind} value along {dim} dimension.must be a slice or 2D boolean mask.')
                ...
                if np.isscalar(ind):
                    ...
                    if ind < emin:
                        raise IndexError(f'Index {ind} less than lower bound of {emin} for {dim} dimension.')
                    elif ind >= emax:
                        raise IndexError(f'Index {ind} more than or equal to upper bound of {emax} for {dim} dimension.')
        ...
        index = np.ix_(*[np.atleast_1d(np.squeeze(ind)) if ind.ndim > 1 else np.atleast_1d(ind) for ind in int_inds])
    ...
    def aggregate(cls, dataset, kdims, function, **kwargs):
        ...
        axes = tuple((dataset.ndims - dataset.get_dimension_index(kdim) - 1 for kdim in dataset.kdims if kdim not in kdims))
        da = dask_array_module()
        dropped = []
        vdim_tuple = cls.packed(dataset)
        if vdim_tuple:
    ...
    def reindex(cls, dataset, kdims, vdims):
        ...
        data = {k: values for k, values in dataset.data.items() if k not in dropped_kdims + dropped_vdims}
    ...
    def sort(cls, dataset, by=None, reverse=False):
        ...
        if not by or by in [dataset.kdims, dataset.dimensions()]:
            ...
            raise Exception('Compressed format cannot be sorted, either instantiate in the desired order or use the expanded format.')
