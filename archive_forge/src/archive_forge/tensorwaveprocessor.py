...
def ensure_system_resources():
    """
    Ensure the system has enough resources to proceed with the processing.
    Logs and prints detailed information about memory and CPU usage.
    Optimizes memory usage if necessary.
    """
    ...
    # Log and print memory and CPU usage
    logging.info(
...
def save_visualization(
    ...
    if viz_type == "original_signal":
    ...
    elif viz_type in tensor_wave_dict:
        fig, ax = plt.subplots(figsize=(10, 4))
        sns.heatmap(tensor_wave_dict[viz_type].T, cmap="viridis", cbar=True, ax=ax)
        ax.set_xlabel("Time [s]")
        ax.set_ylabel("Frequency Component")
        ax.set_title(f"{title} - {viz_type.replace('_', ' ').title()}")
        save_plot(fig, f"{viz_type}_{timestamp}.png")
        ...
        if viz_type in tensor_wave_dict:
            fig = go.Figure(
                data=go.Heatmap(
            ...
            logging.error(f"Key '{viz_type}' not found in tensor_wave_dict.")
            raise ValueError(f"Key '{viz_type}' not found in tensor_wave_dict.")

    else:
        logging.error(f"Unknown visualization type: {viz_type}")
