...
class ExceptionsArgsTest(test_base.BaseTestCase):
    ...
    def assert_exception(self, ex_cls, method, url, status_code, json_data, error_msg=None, error_details=None, check_description=True):
        ex = exceptions.from_response(FakeResponse(status_code=status_code, headers={'Content-Type': 'application/json'}, json_data=json_data), method, url)
    ...
    def test_from_response_non_openstack(self):
        ...
        status_code = http_client.BAD_REQUEST
        json_data = {'alien': 123}
        self.assert_exception(exceptions.BadRequest, method, url, status_code, json_data, check_description=False)
    ...
    def test_from_response_with_text_response_format_with_no_body(self):
        ...
        ex = exceptions.from_response(FakeResponse(status_code=status_code, headers={'Content-Type': 'text/html'}), method, url)
