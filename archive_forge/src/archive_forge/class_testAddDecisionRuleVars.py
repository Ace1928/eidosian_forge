...
class testAddDecisionRuleVars(unittest.TestCase):
    ...
    def test_correct_num_dr_vars_static(self):
        ...
        for indexed_dr_var in m.util.decision_rule_vars:
            self.assertEqual(len(indexed_dr_var), 1, msg=f'Number of decision rule coefficient variables in indexed Var object {indexed_dr_var.name!r}does not match correct value.')
    ...
    def test_correct_num_dr_vars_affine(self):
        ...
        add_decision_rule_variables(model_data=model_data, config=config)
        for indexed_dr_var in m.util.decision_rule_vars:
            self.assertEqual(len(indexed_dr_var), 1 + len(m.util.uncertain_params), msg=f'Number of decision rule coefficient variables in indexed Var object {indexed_dr_var.name!r}does not match correct value.')
        self.assertEqual(len(ComponentSet(m.util.decision_rule_vars)), len(m.util.second_stage_variables), msg='Number of unique indexed DR variable components should equal number of second-stage variables.')
