...
def prepare_data(data: pd.DataFrame, n_steps: int):
    """
    Prepares input and output data for LSTM model.

    Parameters:
    - data (pd.DataFrame): DataFrame containing stock prices.
    - n_steps (int): Number of past days to consider for predicting the future price.

    Returns:
    - np.array: Input features for the model.
    - np.array: Output target for the model.
    """
    X, y = [], []
...
def build_lstm_model(input_shape):
    ...
    Parameters:
    - input_shape (tuple): Shape of the input data (n_steps, n_features).
