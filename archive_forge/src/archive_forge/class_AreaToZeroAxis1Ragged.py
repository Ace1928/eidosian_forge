...
class AreaToZeroAxis1Ragged(_PointLike):
    ...
    def _build_extend(self, x_mapper, y_mapper, info, append, _antialias_stage_2, _antialias_stage_2_funcs):
        ...
        perform_extend_cpu = _build_extend_area_to_zero_axis1_ragged(draw_trapezoid_y, expand_aggs_and_cols)
        x_name = self.x
        y_name = self.y
        ...
        def extend(aggs, df, vt, bounds, plot_start=True):
            ...
            perform_extend_cpu(sx, tx, sy, ty, xmin, xmax, ymin, ymax, xs, ys, *aggs_and_cols)
        return extend