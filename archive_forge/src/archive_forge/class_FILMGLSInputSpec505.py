...
class FILMGLSInputSpec505(FSLCommandInputSpec):
    in_file = File(exists=True, mandatory=True, position=-3, argstr='--in=%s', desc='input data file')
    design_file = File(exists=True, position=-2, argstr='--pd=%s', desc='design matrix file')
    threshold = traits.Range(value=1000.0, low=0.0, argstr='--thr=%f', position=-1, usedefault=True, desc='threshold')
    ...
    autocorr_estimate_only = traits.Bool(argstr='--ac', xor=_estimate_xor, desc='perform autocorrelation estimation only')
    ...
    output_pwdata = traits.Bool(argstr='--outputPWdata', desc='output prewhitened data and average design matrix')
