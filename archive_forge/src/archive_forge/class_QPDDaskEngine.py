...
class QPDDaskEngine(QPDEngine):
    ...
    def to_df(self, obj: Any) -> DataFrame:
        ...
        if isinstance(obj, pandas.DataFrame):
            df = pd.from_pandas(obj, npartitions=2)
    ...
    def to_native(self, df: DataFrame) -> Any:
        if all((not self.is_series(x.native) for x in df.values())):
            return pd.from_pandas(pandas.DataFrame({k: [v.native] for k, v in df.items()}), npartitions=2)
    ...
    def window(self, df: DataFrame, func: WindowFunctionSpec, args: List[ArgumentSpec], dest_col_name: str) -> DataFrame:
        ...
        if func.name == 'sum':
            return self._window_simple_agg(lambda x: x.sum(), df, func, args, dest_col_name)
        if func.name == 'min':
    ...
    def _count_unique(self, df: Any, keys: List[str], cols: str, func: AggFunctionSpec) -> Any:
