...
class RedisBackend(BytesBackend):
    ...
    def __init__(self, arguments):
        ...
        if self.distributed_lock and self.thread_local_lock:
            warnings.warn('The Redis backend thread_local_lock parameter should be set to False when distributed_lock is True')
    ...
    def _create_client(self):
        if self.connection_pool is not None:
            ...
            if self.url is not None:
                ...
                args.update(host=self.host, username=self.username, password=self.password, port=self.port, db=self.db)
                self.writer_client = redis.StrictRedis(**args)
                self.reader_client = self.writer_client

    def get_mutex(self, key):
        if self.distributed_lock:
            return _RedisLockWrapper(self.writer_client.lock('_lock{0}'.format(key), timeout=self.lock_timeout, sleep=self.lock_sleep, thread_local=self.thread_local_lock))
        else:
            return None
