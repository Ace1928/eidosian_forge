...
@ConsumptionAPI
def write_csv(self, path: str, *, filesystem: Optional['pyarrow.fs.FileSystem']=None, try_create_dir: bool=True, arrow_open_stream_args: Optional[Dict[str, Any]]=None, filename_provider: Optional[FilenameProvider]=None, block_path_provider: Optional[BlockWritePathProvider]=None, arrow_csv_args_fn: Callable[[], Dict[str, Any]]=lambda: {}, ray_remote_args: Dict[str, Any]=None, **arrow_csv_args) -> None:
    """Writes the :class:`~ray.data.Dataset` to CSV files.
        ...
        """
    datasink = _CSVDatasink(path, arrow_csv_args_fn=arrow_csv_args_fn, arrow_csv_args=arrow_csv_args, filesystem=filesystem, try_create_dir=try_create_dir, open_stream_args=arrow_open_stream_args, filename_provider=filename_provider, block_path_provider=block_path_provider, dataset_uuid=self._uuid)
