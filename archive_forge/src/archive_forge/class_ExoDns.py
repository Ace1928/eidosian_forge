...
class ExoDns(object):
    ...
    def __init__(self, module):
        ...
        self.headers = {'X-DNS-Token': '%s:%s' % (self.api_key, self.api_secret), 'Content-Type': 'application/json', 'Accept': 'application/json'}
        self.result = {'changed': False, 'diff': {'before': {}, 'after': {}}}
    ...
    def read_config(self, ini_group=None):
        ...
        for key in keys:
            ...
            return env_conf
        paths = (os.path.join(os.path.expanduser('~'), '.cloudstack.ini'), os.path.join(os.getcwd(), 'cloudstack.ini'))
    ...
    def api_query(self, resource='/domains', method='GET', data=None):
        ...
        response, info = fetch_url(module=self.module, url=url, data=data, method=method, headers=self.headers, timeout=self.module.params.get('api_timeout'))
