...
class TestCommitBuilder(per_repository.TestCaseWithRepository):
    ...
    def _add_commit_renamed_check_changed(self, tree, name, expect_fs_hash=False):
        ...
        self._add_commit_change_check_changed(tree, (name, 'new_' + name), rename, expect_fs_hash=expect_fs_hash)
    ...
    def _commit_change_check_changed(self, tree, names, changer, expect_fs_hash=False):
        rev1 = tree.commit('rev1')
        changer()
        rev2 = self.mini_commit_record_iter_changes(tree, names[0], names[1], expect_fs_hash=expect_fs_hash)
        tree1, tree2 = self._get_revtrees(tree, [rev1, rev2])
    ...
    def test_last_modified_rev_after_content_link_changes(self):
        self._test_last_mod_rev_after_content_link_changes('link', 'target', 'newtarget')
    ...
    def test_get_commit_builder_with_surrogateescape(self):
        tree = self.make_branch_and_tree('.')
        with tree.lock_write():
            builder = tree.branch.get_commit_builder([], revprops={'invalid': 'property' + b'\xc0'.decode('utf-8', 'surrogateescape')})
            list(builder.record_iter_changes(tree, tree.last_revision(), tree.iter_changes(tree.basis_tree())))
            builder.finish_inventory()
            try:
                rev_id = builder.commit('foo bar blah')
            except NotImplementedError:
                raise tests.TestNotApplicable('Format does not support revision properties')
        rev = tree.branch.repository.get_revision(rev_id)
        self.assertEqual('foo bar blah', rev.message)
