...
class StronglyEntanglingLayers(Operation):
    ...
    def __init__(self, weights, wires, ranges=None, imprimitive=None, id=None):
        ...
        if ranges is None:
            ...
            for r in ranges:
                if r % len(wires) == 0:
                    raise ValueError(f'Ranges must not be zero nor divisible by the number of wires; got {r}')
        ...
        **Example**
        ...
        for l in range(n_layers):
            for i in range(len(wires)):
                op_list.append(qml.Rot(weights[..., l, i, 0], weights[..., l, i, 1], weights[..., l, i, 2], wires=wires[i]))
            if len(wires) > 1:
                for i in range(len(wires)):
