...
def _conv_implementation(X, W, B, auto_pad, dilations, group, kernel_shape, pads, strides):
    ...
    if group > 1:
        ...
        for b in range(X.shape[0]):
            for g in range(group):
                ...
                gw = W[g * mg:(g + 1) * mg]
                try:
                    cv = _conv_implementation(gx, gw, None, auto_pad, dilations, 1, kernel_shape, pads, strides)
                    ...
                    for io in range(bh, eh, sth):
                        ...
                        if img.shape != w.shape:
                            ...
                            if img.shape != w_.shape:
                                raise RuntimeError(f'Unexpected shape {img.shape} != {w_.shape}, oh={oh}, i={i}, kh={kh}, sH={sH}, sth={sth}.')
                            s = np.dot(img.reshape((1, -1)), w_.reshape((-1, 1)))[0, 0]
                        else:
                            s = np.dot(img.reshape((1, -1)), w.reshape((-1, 1)))[0, 0]
                        res[n, nw, hr] += s
        return res
    if len(X.shape) == 4:
        sN, sC, sH, sW = X.shape
                    ...
                    for io in range(bh, eh, sth):
                        ...
                        for jo in range(bw, ew, stw):
                            ...
                            if img.shape != w.shape:
                                ...
                                if img.shape != w_.shape:
                                    raise RuntimeError(f'Unexpected shape {img.shape} != {w_.shape}, oh={oh}, ow={ow}, i={i}, j={j}, kh={kh}, kw={kw}, sH={sH}, sW={sW}, sth={sth}, stw={stw}.')
                                s = np.dot(img.reshape((1, -1)), w_.reshape((-1, 1)))[0, 0]
                            else:
                                s = np.dot(img.reshape((1, -1)), w.reshape((-1, 1)))[0, 0]
                            res[n, nw, hr, wr] += s
                        ...
                        for jo in range(bw, ew, stw):
                            ...
                            for zo in range(bz, ez, stz):
                                ...
                                if img.shape != w.shape:
                                    jh1, jh2 = (max(-oh - i, 0), min(kh, kh + sH - (i + oh + kh)))
                                    jw1, jw2 = (max(-ow - j, 0), min(kw, kw + sW - (j + ow + kw)))
                                    jz1, jz2 = (max(-oz - z, 0), min(kz, kz + sZ - (z + oz + kz)))
                                    w_ = w[:1, :1, jh1:jh2, jw1:jw2, jz1:jz2]
                                    if img.shape != w_.shape:
                                        raise RuntimeError(f'Unexpected shape {img.shape} != {w_.shape}, oh={oh}, ow={ow}, oz={oz}, i={i}, j={j}, z={z}, kh={kh}, kw={kw}, kz={kz}, sH={sH}, sW={sW}, sZ={sZ}, sth={sth}, stw={stw}, stz={stz}.')
                                    s = np.dot(img.reshape((1, -1)), w_.reshape((-1, 1)))[0, 0]
                                else:
                                    s = np.dot(img.reshape((1, -1)), w.reshape((-1, 1)))[0, 0]
                                res[n, nw, hr, wr, zr] += s
