...
class ListenerTest(common.HeatTestCase):
    ...
    def test_resource_mapping(self):
        ...
        self.assertEqual(listener.Listener, mapping['OS::Octavia::Listener'])
    ...
    def _create_stack(self, tmpl=inline_templates.LISTENER_TEMPLATE):
        ...
        self.listener.client_plugin().client = mock.MagicMock(return_value=self.octavia_client)
    ...
    def test_create(self):
        self._create_stack()
        self.octavia_client.listener_show.side_effect = [{'provisioning_status': 'PENDING_CREATE'}, {'provisioning_status': 'PENDING_CREATE'}, {'provisioning_status': 'ACTIVE'}]
        self.octavia_client.listener_create.side_effect = [exceptions.Conflict(409), {'listener': {'id': '1234'}}]
        expected = {'listener': {'protocol_port': 80, 'protocol': 'TCP', 'loadbalancer_id': '123', 'default_pool_id': 'my_pool', 'name': 'my_listener', 'description': 'my listener', 'admin_state_up': True, 'default_tls_container_ref': 'ref', 'sni_container_refs': ['ref1', 'ref2'], 'connection_limit': -1, 'tenant_id': '1234', 'allowed_cidrs': ['10.10.0.0/16', '192.168.0.0/16']}}
    ...
    def test_update(self):
        self._create_stack()
        self.listener.resource_id_set('1234')
        self.octavia_client.listener_show.side_effect = [{'provisioning_status': 'PENDING_UPDATE'}, {'provisioning_status': 'PENDING_UPDATE'}, {'provisioning_status': 'ACTIVE'}]
        self.octavia_client.listener_set.side_effect = [exceptions.Conflict(409), None]
        prop_diff = {'admin_state_up': False, 'name': 'your_listener'}
        prop_diff = self.listener.handle_update(self.listener.t, None, prop_diff)
        self.assertFalse(self.listener.check_update_complete(prop_diff))
        self.assertFalse(self.listener._update_called)
        self.octavia_client.listener_set.assert_called_with('1234', json={'listener': prop_diff})
        self.assertFalse(self.listener.check_update_complete(prop_diff))
    ...
    def test_delete(self):
        self._create_stack()
        self.listener.resource_id_set('1234')
        self.octavia_client.listener_show.side_effect = [{'provisioning_status': 'PENDING_DELETE'}, {'provisioning_status': 'PENDING_DELETE'}, {'provisioning_status': 'DELETED'}]
        self.octavia_client.listener_delete.side_effect = [exceptions.Conflict(409), None]
    ...
    def test_delete_not_found(self):
        self._create_stack()
        self.listener.resource_id_set('1234')
        self.octavia_client.listener_show.side_effect = [{'provisioning_status': 'PENDING_DELETE'}]
        self.octavia_client.listener_delete.side_effect = [exceptions.Conflict(409), exceptions.NotFound(404)]
        self.listener.handle_delete()
        self.assertFalse(self.listener.check_delete_complete(None))
        self.assertFalse(self.listener._delete_called)
        self.octavia_client.listener_delete.assert_called_with('1234')
    ...
    def test_delete_failed(self):
        self._create_stack()
        self.listener.resource_id_set('1234')
        self.octavia_client.listener_delete.side_effect = exceptions.Unauthorized(401)
        self.listener.handle_delete()
