...
class ArgillaCallbackHandler(BaseCallbackHandler):
    ...
    def __init__(self, dataset_name: str, workspace_name: Optional[str]=None, api_url: Optional[str]=None, api_key: Optional[str]=None) -> None:
        ...
        try:
            ...
            raise ImportError('To use the Argilla callback manager you need to have the `argilla` Python package installed. Please install it with `pip install argilla`')
        if parse(self.ARGILLA_VERSION) < parse('1.8.0'):
            raise ImportError(f'The installed `argilla` version is {self.ARGILLA_VERSION} but `ArgillaCallbackHandler` requires at least version 1.8.0. Please upgrade `argilla` with `pip install --upgrade argilla`.')
        if api_url is None and os.getenv('ARGILLA_API_URL') is None:
            warnings.warn(f'Since `api_url` is None, and the env var `ARGILLA_API_URL` is not set, it will default to `{self.DEFAULT_API_URL}`, which is the default API URL in Argilla Quickstart.')
        ...
        if api_key is None and os.getenv('ARGILLA_API_KEY') is None:
            ...
            warnings.warn(f'Since `api_key` is None, and the env var `ARGILLA_API_KEY` is not set, it will default to `{self.DEFAULT_API_KEY}`, which is the default API key in Argilla Quickstart.')
            api_key = self.DEFAULT_API_KEY
        try:
            rg.init(api_key=api_key, api_url=api_url)
        except Exception as e:
            raise ConnectionError(f"Could not connect to Argilla with exception: '{e}'.\nPlease check your `api_key` and `api_url`, and make sure that the Argilla server is up and running. If the problem persists please report it to {self.ISSUES_URL} as an `integration` issue.") from e
        self.dataset_name = dataset_name
        self.workspace_name = workspace_name or rg.get_workspace()
        try:
            extra_args = {}
            if parse(self.ARGILLA_VERSION) < parse('1.14.0'):
                warnings.warn(f'You have Argilla {self.ARGILLA_VERSION}, but Argilla 1.14.0 or higher is recommended.', UserWarning)
                extra_args = {'with_records': False}
            self.dataset = rg.FeedbackDataset.from_argilla(name=self.dataset_name, workspace=self.workspace_name, **extra_args)
        except Exception as e:
            raise FileNotFoundError(f'`FeedbackDataset` retrieval from Argilla failed with exception `{e}`.\nPlease check that the dataset with name={self.dataset_name} in the workspace={self.workspace_name} exists in advance. If you need help on how to create a `langchain`-compatible `FeedbackDataset` in Argilla, please visit {self.BLOG_URL}. If the problem persists please report it to {self.ISSUES_URL} as an `integration` issue.') from e
        supported_fields = ['prompt', 'response']
        if supported_fields != [field.name for field in self.dataset.fields]:
            raise ValueError(f'`FeedbackDataset` with name={self.dataset_name} in the workspace={self.workspace_name} had fields that are not supported yet for the`langchain` integration. Supported fields are: {supported_fields}, and the current `FeedbackDataset` fields are {[field.name for field in self.dataset.fields]}. For more information on how to create a `langchain`-compatible `FeedbackDataset` in Argilla, please visit {self.BLOG_URL}.')
        self.prompts: Dict[str, List[str]] = {}
        warnings.warn(f'The `ArgillaCallbackHandler` is currently in beta and is subject to change based on updates to `langchain`. Please report any issues to {self.ISSUES_URL} as an `integration` issue.')
    ...
    def on_llm_end(self, response: LLMResult, **kwargs: Any) -> None:
        ...
        for prompt, generations in zip(prompts, response.generations):
            self.dataset.add_records(records=[{'fields': {'prompt': prompt, 'response': generation.text.strip()}} for generation in generations])
    ...
    def on_chain_end(self, outputs: Dict[str, Any], **kwargs: Any) -> None:
        ...
        if not any((key in self.prompts for key in [str(kwargs['parent_run_id']), str(kwargs['run_id'])])):
            return
        prompts: List = self.prompts.get(str(kwargs['parent_run_id'])) or cast(List, self.prompts.get(str(kwargs['run_id']), []))
        for chain_output_key, chain_output_val in outputs.items():
            if isinstance(chain_output_val, list):
                self.dataset.add_records(records=[{'fields': {'prompt': prompt, 'response': output['text'].strip()}} for prompt, output in zip(prompts, chain_output_val)])
            else:
                self.dataset.add_records(records=[{'fields': {'prompt': ' '.join(prompts), 'response': chain_output_val.strip()}}])
    ...
    def on_tool_end(self, output: Any, observation_prefix: Optional[str]=None, llm_prefix: Optional[str]=None, **kwargs: Any) -> None:
