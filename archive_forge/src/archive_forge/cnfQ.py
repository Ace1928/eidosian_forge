...
class AsyncCache:
    ...
    async def _initialize_cache(self):
        """
        Asynchronously initializes the cache based on the provided configuration. This method sets up both in-memory and file-based caches, ensuring they are ready for use without blocking the main event loop.

        This initialization process includes reading existing cache entries from the file cache (if present) and loading them into the in-memory cache, thereby maintaining cache persistence across application restarts.
        """
            ...
            if os.path.exists(self.file_cache_path):
                async with self.file_io_lock:
                    async with aiofiles.open(self.file_cache_path, "rb") as f:
                        self.file_cache = await f.read()
                        self.file_cache = pickle.loads(self.file_cache)
            self.logger.debug("Cache initialization completed asynchronously, ensuring optimal performance and responsiveness.")
