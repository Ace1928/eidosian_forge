...
def test_001_setup_docker_xnat():
    ...
    try:
        cmd = 'curl --cookie-jar /tmp/cookie --header "Content-Type: application/x-www-form-urlencoded" --request POST --data "username={user}&password={password}&login=&XNAT_CSRF=" http://localhost/login'.format(user=x._user, password=x._pwd)
        print(cmd)
        os.system(cmd)
        cmd = 'curl --cookie /tmp/cookie --header "Content-Type: application/json" --request POST --data \'%s\' http://localhost/xapi/siteConfig'
