...
class MonascaNotification(resource.Resource):
    ...
    def validate(self):
        ...
        if self.properties[self.PERIOD] is not None and self.properties[self.TYPE] != self.WEBHOOK:
            msg = _('The period property can only be specified against a Webhook Notification type.')
        ...
        if self.properties[self.TYPE] == self.WEBHOOK:
            try:
                ...
                msg = _('Address "%(addr)s" should have correct format required by "%(wh)s" type of "%(type)s" property') % {'addr': address, 'wh': self.WEBHOOK, 'type': self.TYPE}
                raise exception.StackValidationFailed(message=msg)
            if not parsed_address.scheme:
                msg = _('Address "%s" doesn\'t have required URL scheme') % address
                raise exception.StackValidationFailed(message=msg)
            if not parsed_address.netloc:
                msg = _('Address "%s" doesn\'t have required network location') % address
                raise exception.StackValidationFailed(message=msg)
            if parsed_address.scheme not in ['http', 'https']:
                msg = _('Address "%(addr)s" doesn\'t satisfies allowed schemes: %(schemes)s') % {'addr': address, 'schemes': ', '.join(['http', 'https'])}
                raise exception.StackValidationFailed(message=msg)
        elif self.properties[self.TYPE] == self.EMAIL and (not re.match('^\\S+@\\S+$', address)):
            msg = _('Address "%(addr)s" doesn\'t satisfies allowed format for "%(email)s" type of "%(type)s" property') % {'addr': address, 'email': self.EMAIL, 'type': self.TYPE}
            raise exception.StackValidationFailed(message=msg)

    def handle_create(self):
        args = dict(name=self.properties[self.NAME] or self.physical_resource_name(), type=self.properties[self.TYPE], address=self.properties[self.ADDRESS])
    ...
    def handle_update(self, json_snippet, tmpl_diff, prop_diff):
        ...
        args['name'] = prop_diff.get(self.NAME) or self.properties[self.NAME]
        args['type'] = prop_diff.get(self.TYPE) or self.properties[self.TYPE]
        args['address'] = prop_diff.get(self.ADDRESS) or self.properties[self.ADDRESS]
        if args['type'] == self.WEBHOOK:
            updated_period = prop_diff.get(self.PERIOD)
            args['period'] = updated_period if updated_period is not None else self._period_interval()
        self.client().notifications.update(**args)

    def handle_delete(self):
        if self.resource_id is not None:
            with self.client_plugin().ignore_not_found:
                self.client().notifications.delete(notification_id=self.resource_id)