...
class GCELBDriver(Driver):
    ...
    def create_balancer(self, name, port, protocol, algorithm, members, ex_region=None, ex_healthchecks=None, ex_address=None, ex_session_affinity=None):
        """
        ...
        tp_name = '%s-tp' % name
        targetpool = self.gce.ex_create_targetpool(tp_name, region=ex_region, healthchecks=ex_healthchecks, nodes=node_list, session_affinity=ex_session_affinity)
        try:
            forwarding_rule = self.gce.ex_create_forwarding_rule(name, targetpool, region=ex_region, protocol=protocol, port_range=port, address=ex_address)
        except Exception:
            targetpool.destroy()
            raise
    ...
    def _node_to_member(self, node, balancer):
        ...
        if hasattr(node, 'public_ips') and len(node.public_ips) > 0:
            ...
            member_ip = None
        extra = {'node': node}
        return Member(id=member_id, ip=member_ip, port=balancer.port, balancer=balancer, extra=extra)
