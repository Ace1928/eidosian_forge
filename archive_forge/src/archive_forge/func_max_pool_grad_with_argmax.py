...
def max_pool_grad_with_argmax(input: _atypes.TensorFuzzingAnnotation[TV_MaxPoolGradWithArgmax_T], grad: _atypes.TensorFuzzingAnnotation[TV_MaxPoolGradWithArgmax_T], argmax: _atypes.TensorFuzzingAnnotation[TV_MaxPoolGradWithArgmax_Targmax], ksize, strides, padding: str, include_batch_in_index: bool=False, name=None) -> _atypes.TensorFuzzingAnnotation[TV_MaxPoolGradWithArgmax_T]:
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'MaxPoolGradWithArgmax', name, input, grad, argmax, 'ksize', ksize, 'strides', strides, 'padding', padding, 'include_batch_in_index', include_batch_in_index)
            return _result
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('MaxPoolGradWithArgmax', input=input, grad=grad, argmax=argmax, ksize=ksize, strides=strides, padding=padding, include_batch_in_index=include_batch_in_index, name=name)
    _result = _outputs[:]
