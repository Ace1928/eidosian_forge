...
class ScopePrompter(six.with_metaclass(abc.ABCMeta, object)):
    ...
    def _PromptDidYouMeanScope(self, ambiguous_refs, attribute, resource_type, suggested_resource, raise_on_prompt_failure):
        ...
        message = 'Did you mean {0} [{1}] for {2}: [{3}]?'.format(attribute, suggested_resource, resource_name, ', '.join(names))
        try:
            if console_io.PromptContinue(message=message, default=True, throw_if_unattended=True):
    ...
    def _PromptForScopeList(self, ambiguous_refs, attributes, resource_type, choice_resources, raise_on_prompt_failure):
        ...
        title = utils.ConstructList('For the following {0}:'.format(resource_name), names)
        idx = console_io.PromptChoice(options=choice_names, message='{0}choose a {1}:'.format(title, ' or '.join(attributes)))
        if idx is None:
            raise_on_prompt_failure()
        else:
