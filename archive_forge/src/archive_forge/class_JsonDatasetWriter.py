...
class JsonDatasetWriter:
    ...
    def __init__(self, dataset: Dataset, path_or_buf: Union[PathLike, BinaryIO], batch_size: Optional[int]=None, num_proc: Optional[int]=None, **to_json_kwargs):
    ...
    def _write(self, file_obj: BinaryIO, orient, lines, **to_json_kwargs) -> int:
        ...
        if self.num_proc is None or self.num_proc == 1:
            ...
            with multiprocessing.Pool(self.num_proc) as pool:
                for json_str in hf_tqdm(pool.imap(self._batch_json, [(offset, orient, lines, to_json_kwargs) for offset in range(0, num_rows, batch_size)]), total=num_rows // batch_size + 1 if num_rows % batch_size else num_rows // batch_size, unit='ba', desc='Creating json from Arrow format'):
                    written += file_obj.write(json_str)
        return written