...
def system_replacemsg_traffic_quota(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('system.replacemsg', 'traffic-quota', filtered_data, vdom=vdom)
        current_data = fos.get('system.replacemsg', 'traffic-quota', vdom=vdom, mkey=mkey)
    ...
    if state == 'present' or state is True:
        return fos.set('system.replacemsg', 'traffic-quota', data=filtered_data, vdom=vdom)
    elif state == 'absent':
        return fos.delete('system.replacemsg', 'traffic-quota', mkey=filtered_data['msg-type'], vdom=vdom)
    else:
