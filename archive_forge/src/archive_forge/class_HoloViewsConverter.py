...
class HoloViewsConverter:
    ...
    _geo_types = sorted(_gridded_types + _geom_types + ['points', 'vectorfield', 'labels', 'hexbin', 'bivariate'])
    _stats_types = ['hist', 'kde', 'violin', 'box', 'density']
    _data_options = ['x', 'y', 'kind', 'by', 'use_index', 'use_dask', 'dynamic', 'crs', 'value_label', 'group_label', 'backlog', 'persist', 'sort_date']
    _geo_options = ['geo', 'crs', 'features', 'project', 'coastline', 'tiles', 'projection', 'global_extents']
    _axis_options = ['width', 'height', 'shared_axes', 'grid', 'legend', 'rot', 'xlim', 'ylim', 'xticks', 'yticks', 'colorbar', 'invert', 'title', 'logx', 'logy', 'loglog', 'xaxis', 'yaxis', 'xformatter', 'yformatter', 'xlabel', 'ylabel', 'clabel', 'padding', 'responsive', 'max_height', 'max_width', 'min_height', 'min_width', 'frame_height', 'frame_width', 'aspect', 'data_aspect', 'fontscale']
    _style_options = ['color', 'alpha', 'colormap', 'fontsize', 'c', 'cmap', 'color_key', 'cnorm', 'rescale_discrete_levels']
    _op_options = ['datashade', 'rasterize', 'x_sampling', 'y_sampling', 'downsample', 'aggregator', 'resample_when']
    _kind_options = {'area': ['y2'], 'errorbars': ['yerr1', 'yerr2'], 'bivariate': ['bandwidth', 'cut', 'filled', 'levels'], 'contour': ['z', 'levels', 'logz'], 'contourf': ['z', 'levels', 'logz'], 'dataset': ['columns'], 'heatmap': ['C', 'reduce_function', 'logz'], 'hexbin': ['C', 'reduce_function', 'gridsize', 'logz', 'min_count'], 'hist': ['bins', 'bin_range', 'normed', 'cumulative'], 'image': ['z', 'logz'], 'kde': ['bw_method', 'ind', 'bandwidth', 'cut', 'filled'], 'labels': ['text', 'c', 'xoffset', 'yoffset', 'text_font', 'text_font_size'], 'ohlc': ['bar_width', 'pos_color', 'neg_color', 'line_color'], 'points': ['s', 'marker', 'c', 'scale', 'logz'], 'polygons': ['logz', 'c'], 'rgb': ['z', 'bands'], 'scatter': ['s', 'c', 'scale', 'logz', 'marker'], 'step': ['where'], 'table': ['columns'], 'quadmesh': ['z', 'logz'], 'vectorfield': ['angle', 'mag']}
    _kind_mapping = {'area': Area, 'bar': Bars, 'barh': Bars, 'bivariate': Bivariate, 'box': BoxWhisker, 'contour': Contours, 'contourf': Polygons, 'dataset': Dataset, 'density': Distribution, 'errorbars': ErrorBars, 'hist': Histogram, 'image': Image, 'kde': Distribution, 'labels': Labels, 'line': Curve, 'scatter': Scatter, 'heatmap': HeatMap, 'hexbin': HexTiles, 'ohlc': Rectangles, 'paths': Path, 'points': Points, 'polygons': Polygons, 'quadmesh': QuadMesh, 'rgb': RGB, 'step': Curve, 'table': Table, 'vectorfield': VectorField, 'violin': Violin}
    _colorbar_types = ['bivariate', 'contour', 'contourf', 'heatmap', 'image', 'hexbin', 'quadmesh', 'polygons']
    _legend_positions = ('top_right', 'top_left', 'bottom_left', 'bottom_right', 'right', 'left', 'top', 'bottom')
    _default_plot_opts = {'logx': False, 'logy': False, 'show_legend': True, 'legend_position': 'right', 'show_grid': False, 'responsive': False, 'shared_axes': True}
    _default_cmaps = {'linear': 'kbc_r', 'categorical': cc.palette['glasbey_category10'], 'cyclic': 'colorwheel', 'diverging': 'coolwarm'}
    ...
    def __init__(self, data, x, y, kind=None, by=None, use_index=True, group_label=None, value_label='value', backlog=1000, persist=False, use_dask=False, crs=None, fields={}, groupby=None, dynamic=True, grid=None, legend=None, rot=None, title=None, xlim=None, ylim=None, clim=None, symmetric=None, logx=None, logy=None, loglog=None, hover=None, subplots=False, label=None, invert=False, stacked=False, colorbar=None, datashade=False, rasterize=False, downsample=None, resample_when=None, row=None, col=None, debug=False, framewise=True, aggregator=None, projection=None, global_extent=None, geo=False, precompute=False, flip_xaxis=None, flip_yaxis=None, dynspread=False, hover_cols=[], x_sampling=None, y_sampling=None, project=False, tools=[], attr_labels=None, coastline=False, tiles=False, sort_date=True, check_symmetric_max=1000000, transforms={}, stream=None, cnorm=None, features=None, rescale_discrete_levels=None, autorange=None, **kwds):
        ...
        self.label = label
        self._process_data(kind, data, x, y, by, groupby, row, col, use_dask, persist, backlog, label, group_label, value_label, hover_cols, attr_labels, transforms, stream, kwds)
        ...
        if self.geo or self.datatype == 'geopandas':
            try:
                ...
                raise ImportError('In order to use geo-related features the geoviews library must be available. It can be installed with:\n  conda install geoviews')
        if self.geo:
            if self.kind not in self._geo_types:
                param.main.param.warning(f'geo option cannot be used with kind={self.kind!r} plot type. Geographic plots are only supported for following plot types: {self._geo_types!r}')
            from cartopy import crs as ccrs
            from geoviews.util import project_extents
            if isinstance(projection, str):
                all_crs = [proj for proj in dir(ccrs) if callable(getattr(ccrs, proj)) and proj not in ['ABCMeta', 'CRS'] and proj[0].isupper() or proj == 'GOOGLE_MERCATOR']
                if projection in all_crs and projection != 'GOOGLE_MERCATOR':
                    projection = getattr(ccrs, projection)()
                elif projection == 'GOOGLE_MERCATOR':
                    projection = getattr(ccrs, projection)
                else:
                    raise ValueError(f'Projection must be defined as cartopy CRS or one of the following CRS string:\n {all_crs}')
            ...
            if tiles and self.output_projection != ccrs.GOOGLE_MERCATOR:
                raise ValueError('Tiles can only be used with output projection of `cartopy.crs.GOOGLE_MERCATOR`. To get rid of this error remove `projection=` or `tiles=`')
        ...
        if resample_when is not None and (not any([rasterize, datashade, downsample])):
            raise ValueError('At least one resampling operation (rasterize, datashader, downsample) must be enabled when resample_when is set.')
        ...
        if legend in self._legend_positions:
            ...
            raise ValueError('The legend option should be a boolean or a valid legend position (i.e. one of %s).' % list(self._legend_positions))
        plotwds = ['xticks', 'yticks', 'xlabel', 'ylabel', 'clabel', 'padding', 'xformatter', 'yformatter', 'height', 'width', 'frame_height', 'frame_width', 'min_width', 'min_height', 'max_width', 'max_height', 'fontsize', 'fontscale', 'responsive', 'shared_axes', 'aspect', 'data_aspect']
        ...
        if hover and (not any((t for t in tools if isinstance(t, HoverTool) or t in ['hover', 'vline', 'hline']))):
        ...
        if self.kind in self._colorbar_types or self.rasterize or self.datashade or self._color_dim:
            try:
                ...
                if self._style_opts.get('cmap') is None:
                    if (self.datashade or self.rasterize) and (self.aggregator is None or 'count_cat' in str(self.aggregator)) and (self.by and (not self.subplots) or (isinstance(self.y, list) and len(self.y) > 1 or (self.y is None and len(set(self.variables) - set(self.indexes)) > 1))):
        ...
        self._overlay_opts = {k: v for k, v in self._plot_opts.items() if k in OverlayPlot.param.objects()}
        ...
        if debug:
            kwds = dict(x=self.x, y=self.y, by=self.by, kind=self.kind, groupby=self.groupby, grid=self.grid)
            param.main.param.warning('Plotting {kind} plot with parameters x: {x}, y: {y}, by: {by}, groupby: {groupby}, row/col: {grid}'.format(**kwds))
    ...
    def _process_crs(self, data, crs):
        ...
        try:
            ...
            if crs:
                raise ValueError(f"'{crs}' must be either a valid crs or an reference to a `data.attr` containing a valid crs: {e}")

    def _process_data(self, kind, data, x, y, by, groupby, row, col, use_dask, persist, backlog, label, group_label, value_label, hover_cols, attr_labels, transforms, stream, kwds):
        ...
        if is_geodataframe(data):
            ...
            if kind is None:
                ...
                if len(geom_types) > 1:
                    raise ValueError('The GeopandasInterface can only read dataframes which share a common geometry type')
            ...
            if kind == 'rgb':
                ...
                other_dims = []
            da = data
            data, x, y, by_new, groupby_new = process_xarray(data, x, y, by, groupby, use_dask, persist, gridded, label, value_label, other_dims, kind=kind)
        ...
        if gridded_data:
            ...
            if groupby and not_found:
                raise ValueError(f'The supplied groupby dimension(s) {not_found} could not be found, expected one or more of: {list(data.coords)}')
        else:
            if gridded and kind not in ('points', 'dataset'):
                raise ValueError(f'{kind} plot type requires gridded data, e.g. a NumPy array or xarray Dataset, found {type(self.data).__name__} type')
            ...
            if isinstance(self.data, pd.DataFrame):
                ...
                indexes = [c for c in self.data.reset_index().columns if c not in self.data.columns]
            else:
            ...
            if groupby and not_found:
                raise ValueError(f'The supplied groupby dimension(s) {not_found} could not be found, expected one or more of: {list(self.data.columns)}')
        if transforms:
        ...
        if da is not None and attr_labels is True or attr_labels is None:
            try:
                ...
                if attr_labels is True:
                    param.main.param.warning(f'Unable to auto label using xarray attrs because {e}')
    ...
    def _process_plot(self):
        ...
        if kind == 'hist':
            if self.stacked:
                param.main.param.warning('Stacking for histograms is not yet implemented in holoviews. Use bar plots if stacking is required.')
        return plot_opts
    ...
    def _process_style(self, kwds, plot_opts):
        ...
        categories = ['accent', 'category', 'dark', 'colorblind', 'pastel', 'set1', 'set2', 'set3', 'paired', 'glasbey']
        ...
        if color is not None:
            ...
            if not isinstance(color, list) and color in self.variables and ('c' in self._kind_options.get(kind, [])):
        ...
        if size is not None:
            ...
            if self.datashade or self.rasterize:
                param.main.param.warning('There is no reasonable way to use size (or s) with rasterize or datashade. To aggregate along a third dimension, set color (or c) to the desired dimension.')
    ...
    def __call__(self, kind, x, y):
            ...
            if self.datatype in ('geopandas', 'spatialpandas'):
                ...
                if isinstance(data, xr.Dataset):
                    if kind == 'image':
                        ...
                        new_coords = {coord: np.arange(data[coord].shape[0]) for coord in missing_coords}
            ...
            if groups:
                ...
                if len(zs) > 1:
                    ...
                    if self.dynamic:
                        ...
                        obj = HoloMap({(z,) + k: method(x, y, z, dataset[k]) for k, v in datasets.data.items() for z in zs}, kdims=dimensions)
                else:
                    ...
                    def method_wrapper(ds, x, y):
                        ...
                        el._dataset = ds
                        return el
                    obj = datasets.apply(method_wrapper, x=x, y=y, per_element=True, link_inputs=False)
            elif len(zs) > 1:
                dimensions = [Dimension(self.group_label, values=zs)]
                if self.dynamic:
                    obj = DynamicMap(lambda z: method(x, y, z, data=dataset.data), kdims=dimensions)
                else:
                    obj = HoloMap({z: method(x, y, z, data=dataset.data) for z in zs}, kdims=dimensions)
            else:
        ...
        try:
            ...
            raise ImportError('In order to use datashading features the Datashader library must be available. It can be installed with:\n  conda install datashader')
        categorical = False
        if self.by and (not self.subplots):
            opts['aggregator'] = reductions.count_cat(self.by[0])
            categorical = True
        elif (isinstance(self.y, list) and len(self.y) > 1 or (self.y is None and len(set(self.variables) - set(self.indexes)) > 1)) and self.kind in ('scatter', 'line', 'area'):
        ...
        if self.dynspread:
            processed = dynspread(processed, max_px=self.kwds.get('max_px', 3), threshold=self.kwds.get('threshold', 0.5))
    ...
    def _resample_obj(self, operation, obj, opts):
        ...
        if self.resample_when is not None:
            processed = apply_when(obj, operation=partial(operation, **opts), predicate=exceeds_resample_when)
        else:
    ...
    def _get_tiles(self, source, sources, types):
        ...
        if tile_source is True:
            ...
            msg = f'{tile_source} tiles not recognized, must be one of: {sorted(sources)} or a tile object'
    ...
    def _get_dimensions(self, kdims, vdims):
        ...
        for dimension in self.hover_cols:
            if isinstance(dimension, str) and dimension in self.variables and (dimension not in dimensions):
                vdims.append(dimension)
        return (kdims, vdims)
    ...
    def _error_if_unavailable(self, kind, element=None):
        ...
        if element not in Store.registry[self._backend]:
            raise NotImplementedError(f'{kind!r} plot not supported by the {self._backend!r} backend.')
    ...
    def single_chart(self, element, x, y, data=None):
        ...
        cur_opts = {element.name: cur_el_opts, 'NdOverlay': filter_opts('NdOverlay', dict(self._overlay_opts, batched=False), backend='bokeh')}
        compat_opts = {element.name: compat_el_opts, 'NdOverlay': filter_opts('NdOverlay', dict(self._overlay_opts), backend=self._backend_compat)}
        ys = [y]
        ...
        if self.by:
            if element is Bars and (not self.subplots):
                if any((y in self.indexes for y in ys)):
                    data = data.reset_index()
                return element(data, ([x] if x else []) + self.by, ys).relabel(**self._relabel).redim(**self._redim).opts(cur_opts, backend='bokeh').opts(compat_opts, backend=self._backend_compat)
            chart = Dataset(data, self.by + kdims, vdims).to(element, kdims, vdims, self.by).relabel(**self._relabel)
            chart = chart.layout() if self.subplots else chart.overlay(sort=False)
        else:
            chart = element(data, kdims, vdims).relabel(**self._relabel)
        return chart.redim(**self._redim).opts(cur_opts, backend='bokeh').opts(compat_opts, backend=self._backend_compat)
    ...
    def chart(self, element, x, y, data=None):
        ...
        if 'ylabel' in self._plot_opts and 'y' not in labelled:
            labelled.append('y')
        cur_opts = {element.name: self._get_opts(element.name, labelled=labelled, backend='bokeh'), 'NdOverlay': filter_opts('NdOverlay', dict(self._overlay_opts, batched=False), backend='bokeh')}
        compat_opts = {element.name: self._get_opts(element.name, labelled=labelled, backend=self._backend_compat), 'NdOverlay': filter_opts('NdOverlay', dict(self._overlay_opts), backend=self._backend_compat)}
        charts = []
        for c in y:
        ...
        return self._by_type(charts, self.group_label, sort=False).opts(cur_opts, backend='bokeh').opts(compat_opts, backend=self._backend_compat)
    ...
    def hist(self, x=None, y=None, data=None):
        ...
        if 'ylabel' in self._plot_opts and 'y' not in labelled:
            labelled.append('y')
        cur_opts = {'Histogram': self._get_opts('Histogram', labelled=labelled, backend='bokeh'), 'NdOverlay': filter_opts('NdOverlay', self._overlay_opts, backend='bokeh')}
        compat_opts = {'Histogram': self._get_opts('Histogram', backend=self._backend_compat), 'NdOverlay': filter_opts('NdOverlay', self._overlay_opts, backend=self._backend_compat)}
        hist_opts = {'bin_range': self.kwds.get('bin_range', None), 'normed': self.kwds.get('normed', False), 'cumulative': self.kwds.get('cumulative', False)}
        ...
        if not isinstance(y, (list, tuple)):
            ...
            return hists.redim(**self._redim).opts(cur_opts, backend='bokeh').opts(compat_opts, backend=self._backend_compat)
        ranges = []
        for col in y:
            if not 'bin_range' in self.kwds:
                ys = data[col]
        ...
        return self._by_type(hists, sort=False).redim(**self._redim).opts(cur_opts, backend='bokeh').opts(compat_opts, backend=self._backend_compat)
    ...
    def kde(self, x=None, y=None, data=None):
        ...
        data, x, y = self._process_chart_args(data, x, y)
        cur_opts = {'Distribution': self._get_opts('Distribution', backend='bokeh', **dist_opts), 'Area': self._get_opts('Distribution', backend='bokeh'), 'NdOverlay': filter_opts('NdOverlay', dict(self._overlay_opts, legend_limit=0), backend='bokeh')}
        compat_opts = {'Distribution': self._get_opts('Distribution', backend=self._backend_compat, **dist_opts), 'Area': self._get_opts('Distribution', backend=self._backend_compat), 'NdOverlay': filter_opts('NdOverlay', self._overlay_opts, backend=self._backend_compat)}
        ...
        return dists.redim(**redim).relabel(**self._relabel).opts(cur_opts, backend='bokeh').opts(compat_opts, backend=self._backend_compat)
    ...
    def heatmap(self, x=None, y=None, data=None):
        ...
        return hmap.redim(**redim).apply(self._set_backends_opts, cur_opts=cur_opts, compat_opts=compat_opts)
    ...
    def hexbin(self, x=None, y=None, data=None):
        ...
        if self.geo:
            params['crs'] = self.crs
        return element(data, [x, y], z or [], **params).redim(**redim).apply(self._set_backends_opts, cur_opts=cur_opts, compat_opts=compat_opts)
    ...
    def bivariate(self, x=None, y=None, data=None):
        ...
        return element(data, [x, y]).redim(**self._redim).apply(self._set_backends_opts, cur_opts=cur_opts, compat_opts=compat_opts)
    ...
    def ohlc(self, x=None, y=None, data=None):
        ...
        if 'hover' in tools:
            tools[tools.index('hover')] = HoverTool(tooltips=[(x, f'@{x}'), ('Open', f'@{o}'), ('High', f'@{h}'), ('Low', f'@{l}'), ('Close', f'@{c}')] + [(hc, f'@{hc}') for hc in vdims[4:]])
        ...
        if 'ylabel' not in seg_cur_opts:
            seg_cur_opts['ylabel'] = ''
        segments = segments.redim(**self._redim).apply(self._set_backends_opts, cur_opts=seg_cur_opts, compat_opts=seg_compat_opts)
        rects = rects.redim(**self._redim).apply(self._set_backends_opts, cur_opts=rect_cur_opts, compat_opts=rect_compat_opts)
    ...
    def table(self, x=None, y=None, data=None):
        ...
        return element(data, self.kwds.get('columns'), []).redim(**self._redim).apply(self._set_backends_opts, cur_opts=cur_opts, compat_opts=compat_opts)
    ...
    def labels(self, x=None, y=None, data=None):
        ...
        return element(data, kdims, vdims).redim(**self._redim).apply(self._set_backends_opts, cur_opts=cur_opts, compat_opts=compat_opts)
    ...
    def image(self, x=None, y=None, z=None, data=None):
        ...
        if self.geo:
            params['crs'] = self.crs
        return element(data, [x, y], z, **params).redim(**redim).apply(self._set_backends_opts, cur_opts=cur_opts, compat_opts=compat_opts)
    ...
    def rgb(self, x=None, y=None, z=None, data=None):
        ...
        if nbands < 3:
            raise ValueError('Selected bands coordinate (%s) has only %d channels,expected at least three channels to convert to RGB.' % (bands, nbands))
        params = dict(self._relabel)
        ...
        return rgb.redim(**self._redim).apply(self._set_backends_opts, cur_opts=cur_opts, compat_opts=compat_opts)
    ...
    def contour(self, x=None, y=None, z=None, data=None, filled=False):
        ...
        return contours(qmesh, filled=filled, levels=levels).apply(self._set_backends_opts, cur_opts=cur_opts, compat_opts=compat_opts)
    ...
    def _geom_plot(self, x=None, y=None, data=None, kind='polygons'):
        ...
        return obj.redim(**redim).opts({element.name: cur_opts}, backend='bokeh').opts({element.name: compat_opts}, backend=self._backend_compat)
