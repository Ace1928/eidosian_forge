...
class XPathToken(Token[XPathTokenType]):
    ...
    def source(self) -> str:
        ...
        if self.label == 'axis':
            ...
            return 'for %s return %s' % (', '.join(('%s in %s' % (self[k].source, self[k + 1].source) for k in range(0, len(self) - 1, 2))), self[-1].source)
        elif symbol in ('every', 'some'):
            return '%s %s satisfies %s' % (symbol, ', '.join(('%s in %s' % (self[k].source, self[k + 1].source) for k in range(0, len(self) - 1, 2))), self[-1].source)
        elif symbol == 'let':
            return 'let %s return %s' % (', '.join(('%s := %s' % (self[k].source, self[k + 1].source) for k in range(0, len(self) - 1, 2))), self[-1].source)
    ...
    def get_argument(self, context: ContextArgType, index: int=0, required: bool=False, default_to_context: bool=False, default: Optional[AtomicValueType]=None, cls: Optional[Type[Any]]=None, promote: Optional[ClassCheckType]=None) -> Any:
        """
        ...
        """
        item: Union[None, ElementProtocol, DocumentProtocol, XPathNode, AnyAtomicType, XPathFunction]
        try:
    ...
    def get_results(self, context: ContextArgType) -> Union[List[XPathResultType], AtomicValueType]:
        """
        Returns results formatted according to XPath specifications.

        :param context: the XPath dynamic context.
        :return: a list or a simple datatype when the result is a single simple type         generated by a literal or function token.
        """
        item = None
        if context is None:
    ...
    def get_operands(self, context: ContextArgType, cls: Optional[Type[Any]]=None) -> OperandsType:
        """
        ...
        :param context: the XPath dynamic context.
        :param cls: if a type is provided performs a type checking on item.
        :return: a couple of values representing the operands. If any operand         is not available returns a `(None, None)` couple.
        """
    ...
    def adjust_datetime(self, context: ContextArgType, cls: Type[AbstractDateTime]) -> Union[List[Any], AbstractDateTime, DayTimeDuration]:
        """
    ...
    def boolean_value(self, obj: Any) -> bool:
        ...
        if isinstance(obj, list):
            if not obj:
                ...
                message = 'effective boolean value is not defined for a sequence of two or more items not starting with an XPath node.'
                raise self.error('FORG0006', message)
            else:
                obj = obj[0]
        if isinstance(obj, (int, str, UntypedAtomic, AnyURI)):
            return bool(obj)
    ...
    def error(self, code: Union[str, QName], message_or_error: Union[None, str, Exception]=None) -> ElementPathError:
        return xpath_error(code, message_or_error, self, self.parser.namespaces)

    def expected(self, *symbols: str, message: Optional[str]=None, code: str='XPST0003') -> None:
        if symbols and self.symbol not in symbols:
            raise self.wrong_syntax(message, code)

    def unexpected(self, *symbols: str, message: Optional[str]=None, code: str='XPST0003') -> None:
        if not symbols or self.symbol in symbols:
            raise self.wrong_syntax(message, code)

    def wrong_syntax(self, message: Optional[str]=None, code: str='XPST0003') -> ElementPathError:
        if self.label == 'function':
            code = 'XPST0017'
        if message:
