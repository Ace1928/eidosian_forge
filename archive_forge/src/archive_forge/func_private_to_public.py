...
def private_to_public():
    ...
    if cli.infilename:
        print('Reading private key from %s in %s format' % (cli.infilename, cli.inform), file=sys.stderr)
    ...
    if cli.outfilename:
        print('Writing public key to %s in %s format' % (cli.outfilename, cli.outform), file=sys.stderr)
