...
class NetworkAccessConditions(object):
    ...
    def get_object_by_name(self, name):
        try:
            result = self.ise.exec(family='network_access_conditions', function='get_network_access_condition_by_name', params={'name': name}, handle_func_exception=False).response['response']
    ...
    def get_object_by_id(self, id):
        try:
            result = self.ise.exec(family='network_access_conditions', function='get_network_access_condition_by_id', handle_func_exception=False, params={'id': id}).response['response']
        except (TypeError, AttributeError) as e:
            self.ise.fail_json(msg='An error occured when executing operation. Check the configuration of your API Settings and API Gateway settings on your ISE server. This collection assumes that the API Gateway, the ERS APIs and OpenAPIs are enabled. You may want to enable the (ise_debug: True) argument. The error was: {error}'.format(error=e))
        except Exception:
            result = None
    ...
    def create(self):
        result = self.ise.exec(family='network_access_conditions', function='create_network_access_condition', params=self.new_object).response
        return result
    ...
    def update(self):
        ...
        result = None
        if id:
            result = self.ise.exec(family='network_access_conditions', function='update_network_access_condition_by_id', params=self.new_object).response
        elif name:
            result = self.ise.exec(family='network_access_conditions', function='update_network_access_condition_by_name', params=self.new_object).response
        return result
    ...
    def delete(self):
        ...
        result = None
        if id:
            result = self.ise.exec(family='network_access_conditions', function='delete_network_access_condition_by_id', params=self.new_object).response
        elif name:
            result = self.ise.exec(family='network_access_conditions', function='delete_network_access_condition_by_name', params=self.new_object).response
        return result