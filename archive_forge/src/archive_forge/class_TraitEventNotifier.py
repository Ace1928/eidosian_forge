...
class TraitEventNotifier:
    ...
    def __init__(self, *, handler, target, event_factory, prevent_event, dispatcher):
        if not callable(handler):
            raise ValueError('handler must be a callable, got {!r}'.format(handler))
    ...
    def add_to(self, observable):
        ...
        for other in notifiers:
            ...
            if self._ref_count != 0:
                raise RuntimeError('Sharing notifiers across observables is unexpected.')
        ...
        for other in notifiers[:]:
            if self.equals(other):
                ...
                if other._ref_count < 0:
                    raise RuntimeError('Reference count is negative. Race condition?')
                break
        else:
    ...
    def equals(self, other):
        ...
        if type(other) is not type(self):
            return False
        return self.handler() == other.handler() and self.target() is other.target() and (self.dispatcher == other.dispatcher)