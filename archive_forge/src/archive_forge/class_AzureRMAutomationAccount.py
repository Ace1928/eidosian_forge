...
class AzureRMAutomationAccount(AzureRMModuleBase):
    ...
    def __init__(self):
        self.module_arg_spec = dict(resource_group=dict(type='str', required=True), name=dict(type='str', required=True), state=dict(type='str', default='present', choices=['present', 'absent']), location=dict(type='str'))
    ...
    def exec_module(self, **kwargs):
        ...
        if self.state == 'present':
            if not account:
                ...
                param = self.automation_models.AutomationAccountCreateOrUpdateParameters(location=self.location, sku=self.automation_models.Sku(name='Basic'), tags=self.tags)
                changed = True
                ...
                if update_tags:
                    changed = True
                    param = self.automation_models.AutomationAccountUpdateParameters(tags=tags)
                    changed = True
                    if not self.check_mode:
