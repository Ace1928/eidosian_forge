...
def load_from_pipeline(pipeline: transformers.Pipeline | diffusers.DiffusionPipeline) -> dict:
    ...
    def fn(*params):
        if pipeline_info:
            ...
            if str(type(pipeline).__module__).startswith('transformers.pipelines'):
                ...
                if isinstance(pipeline, (pipelines.text_classification.TextClassificationPipeline, pipelines.text2text_generation.Text2TextGenerationPipeline, pipelines.text2text_generation.TranslationPipeline)):
                    data = pipeline(*data)
                else:
                    data = pipeline(**data)
                if isinstance(pipeline, pipelines.object_detection.ObjectDetectionPipeline):
    ...
    interface_info['title'] = pipeline.model.__class__.__name__ if str(type(pipeline).__module__).startswith('transformers.pipelines') else pipeline.__class__.__name__
