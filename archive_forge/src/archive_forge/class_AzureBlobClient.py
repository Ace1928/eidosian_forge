...
class AzureBlobClient(Client):
    ...
    def __init__(self, account_url: Optional[str]=None, credential: Optional[Any]=None, connection_string: Optional[str]=None, blob_service_client: Optional['BlobServiceClient']=None, file_cache_mode: Optional[Union[str, FileCacheMode]]=None, local_cache_dir: Optional[Union[str, os.PathLike]]=None, content_type_method: Optional[Callable]=mimetypes.guess_type):
        ...
        if blob_service_client is not None:
            ...
            self.service_client = BlobServiceClient.from_connection_string(conn_str=connection_string, credential=credential)
    ...
    def _get_metadata(self, cloud_path: AzureBlobPath) -> Union['BlobProperties', Dict[str, Any]]:
        blob = self.service_client.get_blob_client(container=cloud_path.container, blob=cloud_path.blob)
        properties = blob.get_blob_properties()
        properties['content_type'] = properties.content_settings.content_type
        return properties

    def _download_file(self, cloud_path: AzureBlobPath, local_path: Union[str, os.PathLike]) -> Path:
        blob = self.service_client.get_blob_client(container=cloud_path.container, blob=cloud_path.blob)
    ...
    def _list_dir(self, cloud_path: AzureBlobPath, recursive: bool=False) -> Iterable[Tuple[AzureBlobPath, bool]]:
        if not cloud_path.container:
            ...
            yield from ((self.CloudPath(f'az://{c.name}'), True) for c in self.service_client.list_containers())
            return
        container_client = self.service_client.get_container_client(cloud_path.container)
        prefix = cloud_path.blob
        if prefix and (not prefix.endswith('/')):
            ...
            for parent in PurePosixPath(o.name[len(prefix):]).parents:
                if parent not in yielded_dirs and str(parent) != '.':
                    if not recursive and '/' in str(parent):
                        continue
                    yield (self.CloudPath(f'az://{cloud_path.container}/{prefix}{parent}'), True)
                    yielded_dirs.add(parent)
            if not recursive and '/' in o.name[len(prefix):]:
                continue
            yield (self.CloudPath(f'az://{cloud_path.container}/{o.name}'), False)

    def _move_file(self, src: AzureBlobPath, dst: AzureBlobPath, remove_src: bool=True) -> AzureBlobPath:
        if src == dst:
            blob_client = self.service_client.get_blob_client(container=src.container, blob=src.blob)
            blob_client.set_blob_metadata(metadata=dict(last_modified=str(datetime.utcnow().timestamp())))
        else:
    ...
    def _remove(self, cloud_path: AzureBlobPath, missing_ok: bool=True) -> None:
        ...
        if file_or_dir == 'dir':
            blobs = [b.blob for b, is_dir in self._list_dir(cloud_path, recursive=True) if not is_dir]
            container_client = self.service_client.get_container_client(cloud_path.container)
    ...
    def _upload_file(self, local_path: Union[str, os.PathLike], cloud_path: AzureBlobPath) -> AzureBlobPath:
        blob = self.service_client.get_blob_client(container=cloud_path.container, blob=cloud_path.blob)
        extra_args = {}
        ...
        blob.upload_blob(Path(local_path).read_bytes(), overwrite=True, content_settings=content_settings)
        return cloud_path