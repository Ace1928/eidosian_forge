...
class Llama:
    ...
    def generate(self, prompt_tokens: List[List[int]], max_gen_len: int, temperature: float=0.6, top_p: float=0.9, logprobs: bool=False, echo: bool=False) -> Tuple[List[List[int]], Optional[List[List[float]]]]:
        ...
        assert max_prompt_len <= params.max_seq_len, f"Maximum prompt length {max_prompt_len} exceeds the model's maximum sequence length {params.max_seq_len}"
        ...
        for cur_pos in range(min_prompt_len, total_len):
            ...
            if logprobs:
                token_logprobs[:, prev_pos + 1:cur_pos + 1] = -F.cross_entropy(input=logits.transpose(1, 2), target=tokens[:, prev_pos + 1:cur_pos + 1], reduction='none', ignore_index=pad_id)
