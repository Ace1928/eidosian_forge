...
class NetAppESeriesLunMapping(NetAppESeriesModule):
    ...
    def __init__(self):
        ansible_options = dict(state=dict(required=False, choices=['present', 'absent'], default='present'), target=dict(required=False, default=None), volume_name=dict(required=True, aliases=['volume']), lun=dict(type='int', required=False))
        super(NetAppESeriesLunMapping, self).__init__(ansible_options=ansible_options, web_services_version='02.00.0000.0000', supports_check_mode=True)
