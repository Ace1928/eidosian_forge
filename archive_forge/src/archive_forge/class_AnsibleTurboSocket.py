...
class AnsibleTurboSocket:
    ...
    def start_server(self):
        env = os.environ
        parameters = ['--fork', '--socket-path', self._socket_path]
        if self._ttl:
        ...
        if self._plugin == 'module':
            ...
            command += ['-m', 'ansible_collections.cloud.common.plugins.module_utils.turbo.server']
        else:
            parent_dir = os.path.dirname(__file__)
            server_path = os.path.join(parent_dir, 'server.py')
            command += [server_path]
        p = subprocess.Popen(command + parameters, env=env, close_fds=True)
        p.communicate()
        return p.pid
    ...
    def communicate(self, data, wait_sleep=0.01):
        ...
        try:
            ...
            raise EmbeddedModuleUnexpectedFailure('Cannot decode plugin answer: {0}'.format(raw_answer))
