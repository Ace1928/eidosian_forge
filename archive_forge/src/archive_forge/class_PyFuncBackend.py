...
class PyFuncBackend(FlavorBackend):
    ...
    def __init__(self, config, env_manager, workers=1, install_mlflow=False, create_env_root_dir=False, env_root_dir=None, **kwargs):
        """
        Args:
        ...
        if env_manager == _EnvManager.CONDA and ENV not in config:
            warnings.warn('Conda environment is not specified in config `env`. Using local environment.')
    ...
    def predict(self, model_uri, input_path, output_path, content_type, pip_requirements_override=None):
        ...
        if self._env_manager != _EnvManager.LOCAL:
            predict_cmd = ['python', _mlflow_pyfunc_backend_predict.__file__, '--model-uri', str(local_uri), '--content-type', shlex.quote(str(content_type))]
            if input_path:
            ...
            if pip_requirements_override and self._env_manager == _EnvManager.CONDA:
                pip_requirements_override = [l.replace('==', '=') for l in pip_requirements_override]
            environment = self.prepare_env(local_path, pip_requirements_override=pip_requirements_override)
            try:
                environment.execute(' '.join(predict_cmd))
            except ShellCommandException as e:
                raise MlflowException(f'{e}\n\nAn exception occurred while running model prediction within a {self._env_manager} environment. You can find the error message from the prediction subprocess by scrolling above.') from None
        else:
            if pip_requirements_override:
                raise MlflowException('`pip_requirements_override` is not supported for local env manager.Please use conda or virtualenv instead.')
            scoring_server._predict(local_uri, input_path, output_path, content_type)

    def serve(self, model_uri, port, host, timeout, enable_mlserver, synchronous=True, stdout=None, stderr=None):
        """
        ...
        if self._env_manager != _EnvManager.LOCAL:
            return self.prepare_env(local_path).execute(command, command_env, stdout=stdout, stderr=stderr, preexec_fn=setup_sigterm_on_parent_death, synchronous=synchronous)
        else:
            _logger.info("=== Running command '%s'", command)
            if not is_windows():
                command = ['bash', '-c', command]
            child_proc = subprocess.Popen(command, env=command_env, preexec_fn=setup_sigterm_on_parent_death, stdout=stdout, stderr=stderr)
            if synchronous:
                rc = child_proc.wait()
                if rc != 0:
                    raise Exception(f"Command '{command}' returned non zero return code. Return code = {rc}")
                return 0
            else:
                return child_proc
    ...
    def can_score_model(self):
        ...
        try:
            p = subprocess.Popen([conda_path, '--version'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            _, _ = p.communicate()
            return p.wait() == 0
        except FileNotFoundError:
            return False

    def build_image(self, model_uri, image_name, install_java=False, install_mlflow=False, mlflow_home=None, enable_mlserver=False):
        with TempDir() as tmp:
            cwd = tmp.path()
            self.generate_dockerfile(model_uri=model_uri, output_dir=cwd, install_java=install_java, install_mlflow=install_mlflow, mlflow_home=mlflow_home, enable_mlserver=enable_mlserver)
    ...
    def generate_dockerfile(self, model_uri, output_dir, install_java=False, install_mlflow=False, mlflow_home=None, enable_mlserver=False):
        ...
        if model_uri:
            ...
            if base_image.startswith('python'):
                ...
                env_manager = _EnvManager.VIRTUALENV if self._env_manager is None else self._env_manager
            ...
            model_install_steps = self._model_installation_steps(model_path, env_manager, install_mlflow, enable_mlserver)
            ...
            if env_manager == _EnvManager.LOCAL:
                raise MlflowException.invalid_parameter_value(LOCAL_ENV_MANAGER_ERROR_MESSAGE)
            model_install_steps = ''
            entrypoint = self._get_install_pyfunc_deps_cmd(env_manager, install_mlflow, enable_mlserver) + f" C._serve('{env_manager}')"
        dockerfile_text = docker_utils.generate_dockerfile(output_dir=output_dir, base_image=base_image, model_install_steps=model_install_steps, entrypoint=entrypoint, env_manager=env_manager, mlflow_home=mlflow_home, enable_mlserver=enable_mlserver, disable_env_creation_at_runtime=True)
