...
class ConfigManager:
    ...
    async def load_config(
        ...
        if file_type not in ["ini", "json", "yaml"]:
            error_message = f"Unsupported file type: {file_type}. Using default configuration template."
        ...
        # Check for file existence and ensure defaults are handled properly
        if not os.path.exists(file_path):
            if self.default_config_template:
                await self.ensure_config_file_exists(file_path)
                logging.info(f"Default configuration file created: {file_path}")
            else:
                error_message = f"Configuration file does not exist and no default template provided: {file_path}"
                logging.error(error_message)
...
class TestConfigManager(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncTearDown(self):
        await aiofiles.os.remove(self.test_config_path)
    ...
    async def test_save_config(self):
        ...
        self.assertTrue(
            await aiofiles.os.path.exists(new_config_path),
            "New config file should exist after saving.",
        )
        await aiofiles.os.remove(new_config_path)
