...
class CubeSemanticLoader(BaseLoader):
    ...
    def __init__(self, cube_api_url: str, cube_api_token: str, load_dimension_values: bool=True, dimension_values_limit: int=10000, dimension_values_max_retries: int=10, dimension_values_retry_delay: int=3):
    ...
    def lazy_load(self) -> Iterator[Document]:
        ...
        for cube_data_obj in cube_data_objects:
            ...
            for item in measures + dimensions:
                ...
                metadata = dict(table_name=str(cube_data_obj_name), column_name=item_name, column_data_type=item_type, column_title=str(item.get('title')), column_description=str(item.get('description')), column_member_type=column_member_type, column_values=dimension_values, cube_data_obj_type=cube_data_obj_type)
