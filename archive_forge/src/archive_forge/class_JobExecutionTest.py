...
class JobExecutionTest(base.BaseTestCase):
    body = {'job_id': 'job_id', 'cluster_id': 'cluster_id', 'configs': {}, 'interface': {}, 'input_id': None, 'output_id': None}
    response = {'cluster_id': 'cluster_id', 'interface': {}, 'job_configs': {}}
    update_json = {'is_public': True, 'is_protected': True}
    ...
    def test_create_job_execution_with_io(self):
        ...
        self.responses.post(url, status_code=202, json={'job_execution': response})
        resp = self.client.job_executions.create(**body)
        self.assertEqual(url, self.responses.last_request.url)
        self.assertEqual(response, json.loads(self.responses.last_request.body))
