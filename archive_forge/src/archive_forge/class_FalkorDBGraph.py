...
class FalkorDBGraph(GraphStore):
    ...
    *Security note*: Make sure that the database connection uses credentials
    ...
    def __init__(self, database: str, host: str='localhost', port: int=6379, username: Optional[str]=None, password: Optional[str]=None, ssl: bool=False) -> None:
    ...
    def refresh_schema(self) -> None:
        ...
        self.structured_schema = {'node_props': {el[0]['label']: el[0]['keys'] for el in node_properties}, 'rel_props': {el[0]['types']: el[0]['keys'] for el in rel_properties}, 'relationships': [el[0] for el in relationships]}
    ...
    def add_graph_documents(self, graph_documents: List[GraphDocument], include_source: bool=False) -> None:
        ...
        for document in graph_documents:
            for node in document.nodes:
                self.query(f"MERGE (n:{node.type} {{id:'{node.id}'}}) SET n += $properties RETURN distinct 'done' AS result", {'properties': node.properties})
            for rel in document.relationships:
                self.query(f"MATCH (a:{rel.source.type} {{id:'{rel.source.id}'}}), (b:{rel.target.type} {{id:'{rel.target.id}'}}) MERGE (a)-[r:{rel.type.replace(' ', '_').upper()}]->(b) SET r += $properties RETURN distinct 'done' AS result", {'properties': rel.properties})