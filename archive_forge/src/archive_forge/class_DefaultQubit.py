    ...
    **Example:**
    ...
    def preprocess(self, execution_config: ExecutionConfig=DefaultExecutionConfig) -> Tuple[TransformProgram, ExecutionConfig]:
        ...
        * Currently does not intrinsically support parameter broadcasting
        ...
        transform_program.add_transform(decompose, stopping_condition=stopping_condition, stopping_condition_shots=stopping_condition_shots, name=self.name)
        ...
        if config.gradient_method == 'adjoint':
            _add_adjoint_transforms(transform_program, device_vjp=config.use_device_jacobian_product)
    ...
    def _setup_execution_config(self, execution_config: ExecutionConfig) -> ExecutionConfig:
        ...
        if execution_config.use_device_jacobian_product is None:
            updated_values['use_device_jacobian_product'] = execution_config.gradient_method == 'adjoint'
    ...
    def execute(self, circuits: QuantumTape_or_Batch, execution_config: ExecutionConfig=DefaultExecutionConfig) -> Result_or_ResultBatch:
        ...
        interface = execution_config.interface if execution_config.gradient_method in {'backprop', None} else None
        if max_workers is None:
            return tuple((simulate(c, rng=self._rng, prng_key=self._prng_key, debugger=self._debugger, interface=interface, state_cache=self._state_cache) for c in circuits))
        vanilla_circuits = [convert_to_numpy_parameters(c) for c in circuits]
        seeds = self._rng.integers(2 ** 31 - 1, size=len(vanilla_circuits))
        _wrap_simulate = partial(simulate, debugger=None, interface=interface)
        with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
            exec_map = executor.map(_wrap_simulate, vanilla_circuits, seeds, [self._prng_key] * len(vanilla_circuits))
            results = tuple(exec_map)
        self._rng = np.random.default_rng(self._rng.integers(2 ** 31 - 1))
        return results

    def compute_derivatives(self, circuits: QuantumTape_or_Batch, execution_config: ExecutionConfig=DefaultExecutionConfig):
        max_workers = execution_config.device_options.get('max_workers', self._max_workers)
        if max_workers is None:
    ...
    def execute_and_compute_derivatives(self, circuits: QuantumTape_or_Batch, execution_config: ExecutionConfig=DefaultExecutionConfig):
        max_workers = execution_config.device_options.get('max_workers', self._max_workers)
        if max_workers is None:
            results = tuple((_adjoint_jac_wrapper(c, rng=self._rng, debugger=self._debugger, prng_key=self._prng_key) for c in circuits))
        else:
            vanilla_circuits = [convert_to_numpy_parameters(c) for c in circuits]
            seeds = self._rng.integers(2 ** 31 - 1, size=len(vanilla_circuits))
            with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
                results = tuple(executor.map(_adjoint_jac_wrapper, vanilla_circuits, seeds, [self._prng_key] * len(vanilla_circuits)))
            self._rng = np.random.default_rng(self._rng.integers(2 ** 31 - 1))
        return tuple(zip(*results))

    def supports_jvp(self, execution_config: Optional[ExecutionConfig]=None, circuit: Optional[QuantumTape]=None) -> bool:
    ...
    def compute_jvp(self, circuits: QuantumTape_or_Batch, tangents: Tuple[Number], execution_config: ExecutionConfig=DefaultExecutionConfig):
        max_workers = execution_config.device_options.get('max_workers', self._max_workers)
        if max_workers is None:
    ...
    def execute_and_compute_jvp(self, circuits: QuantumTape_or_Batch, tangents: Tuple[Number], execution_config: ExecutionConfig=DefaultExecutionConfig):
        max_workers = execution_config.device_options.get('max_workers', self._max_workers)
        if max_workers is None:
            results = tuple((_adjoint_jvp_wrapper(c, t, rng=self._rng, debugger=self._debugger, prng_key=self._prng_key) for c, t in zip(circuits, tangents)))
        else:
            vanilla_circuits = [convert_to_numpy_parameters(c) for c in circuits]
            seeds = self._rng.integers(2 ** 31 - 1, size=len(vanilla_circuits))
            with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
                results = tuple(executor.map(_adjoint_jvp_wrapper, vanilla_circuits, tangents, seeds, [self._prng_key] * len(vanilla_circuits)))
            self._rng = np.random.default_rng(self._rng.integers(2 ** 31 - 1))
        return tuple(zip(*results))
    ...
    def compute_vjp(self, circuits: QuantumTape_or_Batch, cotangents: Tuple[Number], execution_config: ExecutionConfig=DefaultExecutionConfig):
        ...
        * For ``n`` expectation values, the cotangents must have shape ``(n, batch_size)``. If ``n = 1``,
        ...
        if max_workers is None:
            ...
            def _state(circuit):
                return None if self._state_cache is None else self._state_cache.get(circuit.hash, None)
    ...
    def execute_and_compute_vjp(self, circuits: QuantumTape_or_Batch, cotangents: Tuple[Number], execution_config: ExecutionConfig=DefaultExecutionConfig):
        max_workers = execution_config.device_options.get('max_workers', self._max_workers)
        if max_workers is None:
            results = tuple((_adjoint_vjp_wrapper(c, t, rng=self._rng, prng_key=self._prng_key, debugger=self._debugger) for c, t in zip(circuits, cotangents)))
        else:
            vanilla_circuits = [convert_to_numpy_parameters(c) for c in circuits]
            seeds = self._rng.integers(2 ** 31 - 1, size=len(vanilla_circuits))
            with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
                results = tuple(executor.map(_adjoint_vjp_wrapper, vanilla_circuits, cotangents, seeds, [self._prng_key] * len(vanilla_circuits)))
            self._rng = np.random.default_rng(self._rng.integers(2 ** 31 - 1))
        return tuple(zip(*results))