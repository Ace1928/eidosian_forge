...
class RunSet:
    ...
    def __init__(self, args: CmdStanArgs, chains: int=1, *, chain_ids: Optional[List[int]]=None, time_fmt: str='%Y%m%d%H%M%S', one_process_per_chain: bool=True) -> None:
        ...
        self._diagnostic_files = [''] * chains
        self._profile_files = [''] * chains
        if chains == 1:
            ...
            for i in range(chains):
                ...
                if args.save_latent_dynamics:
                    self._diagnostic_files[i] = self.file_path('.csv', extra='-diagnostic', id=chain_ids[i])
                if args.save_profile:
                    self._profile_files[i] = self.file_path('.csv', extra='-profile', id=chain_ids[i])

    def __repr__(self) -> str:
        repr = 'RunSet: chains={}, chain_ids={}, num_processes={}'.format(self._chains, self._chain_ids, self._num_procs)
    ...
    def cmd(self, idx: int) -> List[str]:
        ...
        if self._one_process_per_chain:
            return self._args.compose_command(idx, csv_file=self.csv_files[idx], diagnostic_file=self.diagnostic_files[idx] if self._args.save_latent_dynamics else None, profile_file=self.profile_files[idx] if self._args.save_profile else None)
        else:
            return self._args.compose_command(idx, csv_file=self.file_path('.csv'), diagnostic_file=self.file_path('.csv', extra='-diagnostic') if self._args.save_latent_dynamics else None, profile_file=self.file_path('.csv', extra='-profile') if self._args.save_profile else None)
