...
def assertQuantityEqual(self, q1, q2, msg=None, delta=None):
    """
        ...
        precision.
        """
    delta = 1e-05 if delta is None else delta
    msg = '' if msg is None else ' (%s)' % msg
    q1 = Quantity(q1)
    q2 = Quantity(q2)
    if q1.shape != q2.shape:
        raise self.failureException(f'Shape mismatch ({q1.shape} vs {q2.shape}){msg}')
    if not np.all(np.abs(q1.magnitude - q2.magnitude) < delta):
        raise self.failureException('Magnitudes differ by more than %g (%s vs %s)%s' % (delta, q1.magnitude, q2.magnitude, msg))
    d1 = getattr(q1, '_dimensionality', None)
    d2 = getattr(q2, '_dimensionality', None)
    if (d1 or d2) and (not d1 == d2):
        raise self.failureException(f'Dimensionalities are not equal ({d1} vs {d2}){msg}')