...
class SnapshotManager(base.ManagerWithFind):
    ...
    def create(self, volume_id, force=False, name=None, description=None, metadata=None):
        ...
        if force is None:
            force = False
        body = {'snapshot': {'volume_id': volume_id, 'force': force, 'name': name, 'description': description, 'metadata': snapshot_metadata}}
        return self._create('/snapshots', body, 'snapshot')

    @api_versions.wraps('3.66')
    def create(self, volume_id, force=None, name=None, description=None, metadata=None):
        """Creates a snapshot of the given volume.
        ...
        if metadata is None:
            ...
            snapshot_metadata = metadata
        body = {'snapshot': {'volume_id': volume_id, 'name': name, 'description': description, 'metadata': snapshot_metadata}}
    ...
    def reset_state(self, snapshot, state):
        ...
        return self._action('os-reset_status', snapshot, {'status': state} if state else {})
    ...
    def set_metadata(self, snapshot, metadata):
        ...
        return self._create('/snapshots/%s/metadata' % base.getid(snapshot), body, 'metadata')
    ...
    def delete_metadata(self, snapshot, keys):
        ...
        for k in keys:
            resp, body = self._delete('/snapshots/%s/metadata/%s' % (snapshot_id, k))
    ...
    def update_all_metadata(self, snapshot, metadata):
        ...
        """
        body = {'metadata': metadata}
        return self._update('/snapshots/%s/metadata' % base.getid(snapshot), body)

    def manage(self, volume_id, ref, name=None, description=None, metadata=None):
        """Manage an existing snapshot."""
        body = {'snapshot': {'volume_id': volume_id, 'ref': ref, 'name': name, 'description': description, 'metadata': metadata}}
    ...
    def list_manageable(self, host, detailed=True, marker=None, limit=None, offset=None, sort=None):
        url = self._build_list_url('os-snapshot-manage', detailed=detailed, search_opts={'host': host}, marker=marker, limit=limit, offset=offset, sort=sort)
        return self._list(url, 'manageable-snapshots')

    @api_versions.wraps('3.8')
    def list_manageable(self, host, detailed=True, marker=None, limit=None, offset=None, sort=None, cluster=None):
        search_opts = {'cluster': cluster} if cluster else {'host': host}
        url = self._build_list_url('manageable_snapshots', detailed=detailed, search_opts=search_opts, marker=marker, limit=limit, offset=offset, sort=sort)
        return self._list(url, 'manageable-snapshots')
