...
def _get_encryption_headers(key):
    if key and key != user_request_args_factory.CLEAR and (key.type == encryption_util.KeyType.CSEK):
        return {'x-goog-encryption-algorithm': 'AES256', 'x-goog-encryption-key': key.key, 'x-goog-encryption-key-sha256': key.sha256}
    return {}