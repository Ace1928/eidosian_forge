...
class ComponentReflectionTest(ComparesTables, OneConnectionTablesTest):
    ...
    def define_temp_tables(cls, metadata):
        ...
        user_tmp = Table(table_name, metadata, Column('id', sa.INT, primary_key=True), Column('name', sa.VARCHAR(50)), Column('foo', sa.INT), sa.UniqueConstraint('name', name=f'user_tmp_uq_{config.ident}'), sa.Index('user_tmp_ix', 'foo'), **kw)
    ...
    def _required_fk_keys(self):
        return {'name', 'constrained_columns', 'referred_schema', 'referred_table', 'referred_columns'}

    def exp_indexes(self, schema=None, scope=ObjectScope.ANY, kind=ObjectKind.ANY, filter_names=None):

        def idx(*cols, name, unique=False, column_sorting=None, duplicates=False, fk=False):
            ...
            if fk and (not fk_req.enabled) or (duplicates and (not dup_req.enabled)):
                return ()
            res = {'unique': unique, 'column_names': list(cols), 'name': name, 'dialect_options': mock.ANY, 'include_columns': []}
            if column_sorting:
    ...
    def test_get_schema_names_w_translate_map(self, connection):
        """test #7300"""
        connection = connection.execution_options(schema_translate_map={'foo': 'bar', BLANK_SCHEMA: testing.config.test_schema})
        insp = inspect(connection)
        is_true(testing.config.test_schema in insp.get_schema_names())
    ...
    def test_multi_get_table_options_tables(self, get_multi_exp, schema, scope, kind, use_filter):
        insp, kws, exp = get_multi_exp(schema, scope, kind, use_filter, Inspector.get_table_options, self.exp_options)
        for kw in kws:
            insp.clear_cache()
    ...
    def test_get_multi_table_comment(self, get_multi_exp, schema, scope, kind, use_filter):
        insp, kws, exp = get_multi_exp(schema, scope, kind, use_filter, Inspector.get_table_comment, self.exp_comments)
        for kw in kws:
            insp.clear_cache()
    ...
    def test_get_multi_columns(self, get_multi_exp, schema, scope, kind, use_filter):
        insp, kws, exp = get_multi_exp(schema, scope, kind, use_filter, Inspector.get_columns, self.exp_columns)
        for kw in kws:
            insp.clear_cache()
    ...
    def test_get_multi_pk_constraint(self, get_multi_exp, schema, scope, kind, use_filter):
        insp, kws, exp = get_multi_exp(schema, scope, kind, use_filter, Inspector.get_pk_constraint, self.exp_pks)
        for kw in kws:
            insp.clear_cache()
    ...
    def test_get_multi_foreign_keys(self, get_multi_exp, schema, scope, kind, use_filter):
        insp, kws, exp = get_multi_exp(schema, scope, kind, use_filter, Inspector.get_foreign_keys, self.exp_fks)
        for kw in kws:
            insp.clear_cache()
    ...
    def test_get_multi_indexes(self, get_multi_exp, schema, scope, kind, use_filter):
        insp, kws, exp = get_multi_exp(schema, scope, kind, use_filter, Inspector.get_indexes, self.exp_indexes)
        for kw in kws:
            insp.clear_cache()
    ...
    def test_get_multi_unique_constraints(self, get_multi_exp, schema, scope, kind, use_filter):
        insp, kws, exp = get_multi_exp(schema, scope, kind, use_filter, Inspector.get_unique_constraints, self.exp_ucs)
        for kw in kws:
            insp.clear_cache()
    ...
    def test_comments_unicode(self, connection, metadata):
        Table('unicode_comments', metadata, Column('unicode', Integer, comment='é試蛇ẟΩ'), Column('emoji', Integer, comment='☁️✨'), comment='試蛇ẟΩ✨')
    ...
    def test_comments_unicode_full(self, connection, metadata):
        Table('unicode_comments', metadata, Column('emoji', Integer, comment='🐍🧙🝝🧙\u200d♂️🧙\u200d♀️'), comment='🎩🁰🝑🤷\u200d♀️🤷\u200d♂️')
        metadata.create_all(connection)
        insp = inspect(connection)
        tc = insp.get_table_comment('unicode_comments')
        eq_(tc, {'text': '🎩🁰🝑🤷\u200d♀️🤷\u200d♂️'})
