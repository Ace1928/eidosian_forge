...
class TrigSerializer(TurtleSerializer):
    ...
    def reset(self) -> None:
        ...
        self._contexts: Dict[_ContextType, Tuple[List[_SubjectType], Dict[_SubjectType, bool]]] = {}

    def serialize(self, stream: IO[bytes], base: Optional[str]=None, encoding: Optional[str]=None, spacious: Optional[bool]=None, **args):
        self.reset()
        self.stream = stream
        if base is not None:
            self.base = base
