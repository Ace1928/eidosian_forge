...
class ShadowUsers(base.ShadowUsersDriverBase):
    ...
    def create_federated_user(self, domain_id, federated_dict, email=None):
        local_entity = {'domain_id': domain_id, 'local_id': federated_dict['unique_id'], 'entity_type': 'user'}
    ...
    def get_federated_objects(self, user_id):
        with sql.session_for_read() as session:
            ...
            fed_ref = []
            for row in query:
                m = model.FederatedUser(idp_id=row.idp_id, protocol_id=row.protocol_id, unique_id=row.unique_id)
    ...
    def get_federated_users(self, hints):
        with sql.session_for_read() as session:
            query = session.query(model.User).outerjoin(model.LocalUser).outerjoin(model.FederatedUser)
            ...
            for filter_ in hints.filters:
                if filter_['name'] == 'name':
                    name_filter = filter_
                    query = query.filter(model.FederatedUser.display_name == name_filter['value'])
                    break
            if name_filter:
    ...
    def update_federated_user_display_name(self, idp_id, protocol_id, unique_id, display_name):
        with sql.session_for_write() as session:
            ...
            query = query.filter(model.FederatedUser.protocol_id == protocol_id)
            query = query.filter(model.FederatedUser.unique_id == unique_id)
            query = query.filter(model.FederatedUser.display_name != display_name)
    ...
    def create_nonlocal_user(self, user_dict):
        ...
        with sql.session_for_write() as session:
            new_nonlocal_user_ref = model.NonLocalUser.from_dict(new_nonlocal_user_dict)
    ...
    def list_federated_users_info(self, hints=None):
        with sql.session_for_read() as session:
            query = session.query(model.FederatedUser)
            fed_user_refs = sql.filter_limit_query(model.FederatedUser, query, hints)
