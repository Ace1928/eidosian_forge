...
class CompositeAlarm(alarm_base.BaseAlarm):
    ...
    PROPERTIES = COMPOSITE_RULE, COMPOSITE_OPERATOR, RULES = ('composite_rule', 'operator', 'rules')
    composite_rule_schema = {COMPOSITE_OPERATOR: properties.Schema(properties.Schema.STRING, _('The operator indicates how to combine the rules.'), required=True, update_allowed=True, constraints=[constraints.AllowedValues(['or', 'and'])]), RULES: properties.Schema(properties.Schema.LIST, _('Rules list. Basic threshold/gnocchi rules and nested dict which combine threshold/gnocchi rules by "and" or "or" are allowed. For example, the form is like: [RULE1, RULE2, {"and": [RULE3, RULE4]}], the basic threshold/gnocchi rules must include a "type" field.'), required=True, update_allowed=True, constraints=[constraints.Length(min=2)])}
    properties_schema = {COMPOSITE_RULE: properties.Schema(properties.Schema.MAP, _('Composite threshold rules in JSON format.'), required=True, update_allowed=True, schema=composite_rule_schema)}
