...
class OctoAIEndpoint(BaseOpenAI):
    ...
    def _invocation_params(self) -> Dict[str, Any]:
        ...
        if not is_openai_v1():
            params.update({'api_key': self.octoai_api_token.get_secret_value(), 'api_base': self.octoai_api_base})
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        values['octoai_api_base'] = get_from_dict_or_env(values, 'octoai_api_base', 'OCTOAI_API_BASE', default=DEFAULT_BASE_URL)
        values['octoai_api_token'] = convert_to_secret_str(get_from_dict_or_env(values, 'octoai_api_token', 'OCTOAI_API_TOKEN'))
        values['model_name'] = get_from_dict_or_env(values, 'model_name', 'MODEL_NAME', default=DEFAULT_MODEL)
        try:
            import openai
            if is_openai_v1():
                client_params = {'api_key': values['octoai_api_token'].get_secret_value(), 'base_url': values['octoai_api_base']}
