...
class SxpVpns(object):
    ...
    def __init__(self, params, ise):
        self.ise = ise
        self.new_object = dict(sxp_vpn_name=params.get('sxpVpnName'), id=params.get('id'))

    def get_object_by_name(self, name):
        result = None
        gen_items_responses = self.ise.exec(family='sxp_vpns', function='get_sxp_vpns_generator')
        try:
    ...
    def get_object_by_id(self, id):
        try:
            result = self.ise.exec(family='sxp_vpns', function='get_sxp_vpn_by_id', params={'id': id}, handle_func_exception=False).response['ERSSxpVpn']
        except (TypeError, AttributeError) as e:
            self.ise.fail_json(msg='An error occured when executing operation. Check the configuration of your API Settings and API Gateway settings on your ISE server. This collection assumes that the API Gateway, the ERS APIs and OpenAPIs are enabled. You may want to enable the (ise_debug: True) argument. The error was: {error}'.format(error=e))
        except Exception:
            result = None
    ...
    def requires_update(self, current_obj):
        ...
        obj_params = [('sxpVpnName', 'sxp_vpn_name'), ('id', 'id')]
    ...
    def create(self):
        result = self.ise.exec(family='sxp_vpns', function='create_sxp_vpn', params=self.new_object).response
        return result
    ...
    def delete(self):
        ...
        result = self.ise.exec(family='sxp_vpns', function='delete_sxp_vpn_by_id', params=self.new_object).response
        return result