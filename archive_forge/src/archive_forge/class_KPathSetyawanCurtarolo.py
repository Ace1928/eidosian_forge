...
class KPathSetyawanCurtarolo(KPathBase):
    ...
    def __init__(self, structure: Structure, symprec: float=0.01, angle_tolerance=5, atol=1e-05):
        """
        Args:
        ...
        if 'magmom' in structure.site_properties:
            warn("'magmom' entry found in site properties but will be ignored for the Setyawan and Curtarolo convention.")
        ...
        if not np.allclose(self._structure.lattice.matrix, self._prim.lattice.matrix, atol=atol):
            warn('The input structure does not match the expected standard primitive! The path may be incorrect. Use at your own risk.')
        ...
        if lattice_type == 'cubic':
            if 'P' in spg_symbol:
                ...
                warn(f'Unexpected value for spg_symbol={spg_symbol!r}')
    ...
    def cubic(self):
        """CUB Path."""
        self.name = 'CUB'
        kpoints = {'\\Gamma': np.array([0.0, 0.0, 0.0]), 'X': np.array([0.0, 0.5, 0.0]), 'R': np.array([0.5, 0.5, 0.5]), 'M': np.array([0.5, 0.5, 0.0])}
    ...
    def hex(self):
        """HEX Path."""
        self.name = 'HEX'
        kpoints = {'\\Gamma': np.array([0.0, 0.0, 0.0]), 'A': np.array([0.0, 0.0, 0.5]), 'H': np.array([1 / 3, 1 / 3, 0.5]), 'K': np.array([1 / 3, 1 / 3, 0.0]), 'L': np.array([0.5, 0.0, 0.5]), 'M': np.array([0.5, 0.0, 0.0])}
    ...
    def trib(self):
        """TRI1b Path."""
        self.name = 'TRI1b'
        kpoints = {'\\Gamma': np.array([0.0, 0.0, 0.0]), 'L': np.array([0.5, -0.5, 0.0]), 'M': np.array([0.0, 0.0, 0.5]), 'N': np.array([-0.5, -0.5, 0.5]), 'R': np.array([0.0, -0.5, 0.5]), 'X': np.array([0.0, -0.5, 0.0]), 'Y': np.array([0.5, 0.0, 0.0]), 'Z': np.array([-0.5, 0.0, 0.5])}
        path = [['X', '\\Gamma', 'Y'], ['L', '\\Gamma', 'Z'], ['N', '\\Gamma', 'M'], ['R', '\\Gamma']]
        return {'kpoints': kpoints, 'path': path}