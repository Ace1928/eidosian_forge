...
class RetryAction(base.Action):
    ...
    def change_state(self, retry, state, result=base.Action.NO_RESULT):
        ...
        details = {'retry_name': retry.name, 'retry_uuid': retry_uuid, 'old_state': old_state}
        if result is not self.NO_RESULT:
    ...
    def schedule_execution(self, retry):
        ...
        return self._retry_executor.execute_retry(retry, self._get_retry_args(retry))
    ...
    def schedule_reversion(self, retry):
        ...
        arg_addons = {retry_atom.REVERT_FLOW_FAILURES: self._storage.get_failures()}
        return self._retry_executor.revert_retry(retry, self._get_retry_args(retry, addons=arg_addons, revert=True))
