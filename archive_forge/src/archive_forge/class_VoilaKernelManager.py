...
class VoilaKernelManager(base_class):
    """This class adds pooling heated kernels and pre-rendered notebook
            ...
            """
    kernel_pools_config = Dict(config=True, help='Mapping from notebook name to the kernel configuration\n                like: number of started kernels to keep on standby, environment\n                variables used to start kernel')
    default_env_variables = Dict({}, config=True, help='Default environmental variables for kernels\n                ')
    preheat_blacklist = List([], config=True, help='List of notebooks which do not use pre-heated kernel.')
    fill_delay = Float(1, config=True, help='Wait time before re-filling the pool after a kernel is used')
    ...
    def __init__(self, **kwargs):
        ...
        if self.parent.notebook_path is not None:
            self.notebook_path = os.path.relpath(self.parent.notebook_path, self.root_dir)
            self.fill_if_needed(delay=0, notebook_name=self.notebook_path)
        else:
            self.notebook_path = None
            all_notebooks = [x.relative_to(self.root_dir) for x in list(Path(self.root_dir).rglob('*.ipynb')) if self._notebook_filter(x)]
            for nb in all_notebooks:
                self.fill_if_needed(delay=0, notebook_name=str(nb))

    async def get_rendered_notebook(self, notebook_name: str, **kwargs) -> Tuple[asyncio.Task, TypeList[str]]:
    ...
    async def _initialize(self, notebook_path: str, kernel_id: str=None, **kwargs) -> str:
        ...
        if renderer.notebook_path not in self.notebook_data:
            self.notebook_data[renderer.notebook_path] = {'notebook': renderer.notebook, 'template': renderer.template_name, 'theme': renderer.theme, 'kernel_name': kernel_name, 'kernel_ids': {kernel_id}}
        else:
