...
class Snmpv3Credential(object):
    ...
    def __init__(self, params, dnac):
        self.dnac = dnac
        self.new_object = dict(authPassword=params.get('authPassword'), authType=params.get('authType'), comments=params.get('comments'), credentialType=params.get('credentialType'), description=params.get('description'), id=params.get('id'), instanceTenantId=params.get('instanceTenantId'), instanceUuid=params.get('instanceUuid'), privacyPassword=params.get('privacyPassword'), privacyType=params.get('privacyType'), snmpMode=params.get('snmpMode'), username=params.get('username'))

    def create_params(self):
        new_object_params = {}
        payload = {}
        keys = ['authPassword', 'authType', 'comments', 'credentialType', 'description', 'id', 'instanceTenantId', 'instanceUuid', 'privacyPassword', 'privacyType', 'snmpMode', 'username']
        for key in keys:
    ...
    def get_object_by_name(self, name):
        result = None
        try:
            items = self.dnac.exec(family='discovery', function='get_global_credentials', params={'credential_sub_type': 'SNMPV3'})
    ...
    def requires_update(self, current_obj):
        ...
        obj_params = [('authType', 'authType'), ('comments', 'comments'), ('credentialType', 'credentialType'), ('description', 'description'), ('id', 'id'), ('instanceTenantId', 'instanceTenantId'), ('instanceUuid', 'instanceUuid'), ('privacyType', 'privacyType'), ('snmpMode', 'snmpMode'), ('username', 'username')]
    ...
    def create(self):
        result = self.dnac.exec(family='discovery', function='create_snmpv3_credentials', params=self.create_params(), op_modifies=True)
        return result
    ...
    def update(self):
        ...
        result = None
        result = self.dnac.exec(family='discovery', function='update_snmpv3_credentials', params=self.update_all_params(), op_modifies=True)
        return result