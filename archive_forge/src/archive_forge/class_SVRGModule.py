...
class SVRGModule(Module):
    ...
    def __init__(self, symbol, data_names=('data',), label_names=('softmax_label',), logger=logging, context=mx.cpu(), work_load_list=None, fixed_param_names=None, state_names=None, group2ctxs=None, compression_params=None, update_freq=None):
        super(SVRGModule, self).__init__(symbol, data_names=data_names, label_names=label_names, logger=logger, context=context, work_load_list=work_load_list, fixed_param_names=fixed_param_names, state_names=state_names, group2ctxs=group2ctxs, compression_params=compression_params)
        if isinstance(update_freq, int):
            if update_freq <= 0:
                raise ValueError('update_freq in SVRGModule must be a positive integer to represent the frequency for calculating full gradients')
            self.update_freq = update_freq
        else:
            raise TypeError('update_freq in SVRGModule must be an integer to represent the frequency for calculating full gradients')
        self._mod_aux = mx.mod.Module(symbol, data_names, label_names, logger, context, work_load_list, fixed_param_names, state_names, group2ctxs, compression_params)
    ...
    def bind(self, data_shapes, label_shapes=None, for_training=True, inputs_need_grad=False, force_rebind=False, shared_module=None, grad_req='write'):
        ...
        """
        super(SVRGModule, self).bind(data_shapes, label_shapes, for_training, inputs_need_grad, force_rebind, shared_module, grad_req)
        if for_training:
            self._mod_aux.bind(data_shapes, label_shapes, for_training, inputs_need_grad, force_rebind, shared_module, grad_req)
        ...
        for epoch in range(begin_epoch, num_epoch):
            ...
            while not end_of_batch:
                ...
                if batch_end_callback is not None:
                    batch_end_params = mx.model.BatchEndParam(epoch=epoch, nbatch=nbatch, eval_metric=eval_metric, locals=locals())
