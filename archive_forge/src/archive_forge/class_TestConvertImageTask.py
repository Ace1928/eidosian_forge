...
class TestConvertImageTask(test_utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.image = self.img_factory.new_image(image_id=self.image_id, disk_format='raw', container_format='bare')
        task_input = {'import_from': 'http://cloud.foo/image.raw', 'import_from_format': 'raw', 'image_properties': {'disk_format': 'raw', 'container_format': 'bare'}}
        task_ttl = CONF.task.task_time_to_live
        self.task_type = 'import'
        request_id = 'fake_request_id'
        user_id = 'fake_user'
        self.task = self.task_factory.new_task(self.task_type, TENANT1, self.image_id, user_id, request_id, task_time_to_live=task_ttl, task_input=task_input)
        self.image.extra_properties = {'os_glance_import_task': self.task.task_id}
        self.wrapper = import_flow.ImportActionWrapper(self.img_repo, self.image_id, self.task.task_id)
    ...
    def test_image_convert_fails(self):
        convert = self._setup_image_convert_info_fail()
        with mock.patch.object(processutils, 'execute') as exc_mock:
            exc_mock.side_effect = [('{"format":"raw"}', ''), OSError('convert_fail')]
            self.assertRaises(OSError, convert.execute, 'file:///test/path.raw')
    ...
    def test_image_convert_fails_source_format(self):
        ...
        with mock.patch.object(processutils, 'execute') as exc_mock:
            ...
            exc = self.assertRaises(RuntimeError, convert.execute, 'file:///test/path.raw')
