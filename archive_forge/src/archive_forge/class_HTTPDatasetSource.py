...
class HTTPDatasetSource(DatasetSource):
    ...
    def _extract_filename(self, response) -> str:
        ...
        if (content_disposition := response.headers.get('Content-Disposition')):
            for match in re.finditer('filename=(.+)', content_disposition):
                filename = match[1].strip('\'"')
                if _is_path(filename):
                    raise MlflowException.invalid_parameter_value(f'Invalid filename in Content-Disposition header: {filename}. It must be a file name, not a path.')
                return filename
    ...
    def load(self, dst_path=None) -> str:
        ...
        """
        resp = cloud_storage_http_request(method='GET', url=self.url, stream=True)
    ...
    def from_dict(cls, source_dict: Dict[Any, Any]) -> 'HTTPDatasetSource':
        ...
        if url is None:
            raise MlflowException('Failed to parse HTTPDatasetSource. Missing expected key: "url"', INVALID_PARAMETER_VALUE)
        return cls(url=url)