...
class MaxihostNodeDriver(NodeDriver):
    ...
    def create_node(self, name, size, image, location, ex_ssh_key_ids=None):
        ...
        :rtype: :class:`Node`
        """
        attr = {'hostname': name, 'plan': size.id, 'operating_system': image.id, 'facility': location.id.lower(), 'billing_cycle': 'monthly'}
        if ex_ssh_key_ids:
    ...
    def _to_node(self, data):
        ...
        for key in data:
            extra[key] = data[key]
        node = Node(id=data['id'], name=data['description'], state=state, private_ips=private_ips, public_ips=public_ips, driver=self, extra=extra)
        return node
    ...
    def _to_size(self, data):
        extra = {'specs': data['specs'], 'regions': data['regions'], 'pricing': data['pricing']}
        ram = data['specs']['memory']['total']
        ram = re.sub('[^0-9]', '', ram)
        return NodeSize(id=data['slug'], name=data['name'], ram=int(ram), disk=None, bandwidth=None, price=data['pricing']['usd_month'], driver=self, extra=extra)
    ...
    def _to_image(self, data):
        extra = {'operating_system': data['operating_system'], 'distro': data['distro'], 'version': data['version'], 'pricing': data['pricing']}
