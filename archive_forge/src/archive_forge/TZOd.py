...
class DataProcessor:
    ...
    def process_and_prepare_md_data(
        ...
        X_train_tensor = torch.tensor(X_train, dtype=dtype).flatten()
        X_val_tensor = torch.tensor(X_val, dtype=dtype).flatten()
        y_train_tensor = torch.tensor(y_train, dtype=dtype).flatten()
        y_val_tensor = torch.tensor(y_val, dtype=dtype).flatten()
