...
class DeepEvalCallbackHandler(BaseCallbackHandler):
    ...
    def __init__(self, metrics: List[Any], implementation_name: Optional[str]=None) -> None:
        ...
        try:
            ...
            raise ImportError('To use the deepeval callback manager you need to have the \n                `deepeval` Python package installed. Please install it with \n                `pip install deepeval`')
        if os.path.exists('.deepeval'):
            warnings.warn('You are currently not logging anything to the dashboard, we \n                recommend using `deepeval login`.')
        self.implementation_name = implementation_name
        self.metrics = metrics
        warnings.warn(f'The `DeepEvalCallbackHandler` is currently in beta and is subject to change based on updates to `langchain`. Please report any issues to {self.ISSUES_URL} as an `integration` issue.')
        ...
        for metric in self.metrics:
            for i, generation in enumerate(response.generations):
                ...
                if isinstance(metric, AnswerRelevancy):
                    ...
                    raise ValueError(f'Metric {metric.__name__} is not supported by deepeval \n                        callbacks.')
