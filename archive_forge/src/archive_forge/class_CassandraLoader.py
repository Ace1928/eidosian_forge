...
class CassandraLoader(BaseLoader):
    ...
    def __init__(self, table: Optional[str]=None, session: Optional[Session]=None, keyspace: Optional[str]=None, query: Union[str, Statement, None]=None, page_content_mapper: Callable[[Any], str]=str, metadata_mapper: Callable[[Any], dict]=lambda _: {}, *, query_parameters: Union[dict, Sequence, None]=None, query_timeout: Optional[float]=_NOT_SET, query_trace: bool=False, query_custom_payload: Optional[dict]=None, query_execution_profile: Any=_NOT_SET, query_paging_state: Any=None, query_host: Optional[Host]=None, query_execute_as: Optional[str]=None) -> None:
        """
        ...
        if not session or (table and (not keyspace)):
            try:
                ...
                raise ImportError('Could not import a recent cassio package.Please install it with `pip install --upgrade cassio`.')
        if table:
        ...
        self.query_kwargs = {'parameters': query_parameters, 'trace': query_trace, 'custom_payload': query_custom_payload, 'paging_state': query_paging_state, 'host': query_host, 'execute_as': query_execute_as}
    ...
    async def alazy_load(self) -> AsyncIterator[Document]:
        for row in await wrapped_response_future(self.session.execute_async, self.query, **self.query_kwargs):
            metadata = self.metadata.copy()
            metadata.update(self.metadata_mapper(row))
            yield Document(page_content=self.page_content_mapper(row), metadata=metadata)