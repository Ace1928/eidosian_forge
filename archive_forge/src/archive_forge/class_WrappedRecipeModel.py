...
class WrappedRecipeModel(PythonModel):
    ...
    def __init__(self, predict_scores_for_all_classes, predict_prefix, target_column_class_labels=None):
        super().__init__()
    ...
    def predict(self, context, model_input, params: Optional[Dict[str, Any]]=None):
        ...
        if not hasattr(self._classifier, 'classes_') or not hasattr(self._classifier, 'predict_proba') or (not self.predict_scores_for_all_classes):
            return predicted_label
        classes = self.target_column_class_labels if self.target_column_class_labels is not None else self._classifier.classes_
