...
class DwiTool(NiftyFitCommand):
    ...
    def _format_arg(self, name, trait_spec, value):
        if name == 'syn_file':
            if not isdefined(self.inputs.bvec_file) or not isdefined(self.inputs.b0_file):
                return ''
        if name in ['logdti_file', 'rgbmap_file']:
            if not isdefined(self.inputs.dti_flag) and (not isdefined(self.inputs.dti_flag2)):
                return ''
