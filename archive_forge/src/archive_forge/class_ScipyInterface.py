...
class ScipyInterface(ScipyLU, IPLinearSolverInterface):
    ...
    def do_numeric_factorization(self, matrix: Union[spmatrix, BlockMatrix], raise_on_error: bool=True) -> LinearSolverResults:
        res = super(ScipyInterface, self).do_numeric_factorization(matrix=matrix, raise_on_error=raise_on_error)
    ...
    def get_inertia(self):
        if self._inertia is None:
            raise RuntimeError('The inertia was not computed during do_numeric_factorization. Set compute_inertia to True.')
