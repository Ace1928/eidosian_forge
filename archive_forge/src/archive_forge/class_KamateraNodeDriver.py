...
class KamateraNodeDriver(NodeDriver):
    ...
    def list_locations(self):
        ...
        return [self.ex_get_location(datacenter['id'], datacenter['subCategory'], datacenter['name']) for datacenter in response.object]
    ...
    def list_sizes(self, location):
        ...
        return [self.ex_get_size(size['ramMB'], size['diskSizeGB'], size['cpuType'], size['cpuCores'], extraDiskSizesGB=[], monthlyTrafficPackage=size['monthlyTrafficPackage'], id=size['id']) for size in response.object]
    ...
    def create_node(self, name, size, image, location, auth=None, ex_networks=None, ex_dailybackup=False, ex_managed=False, ex_billingcycle=EX_BILLINGCYCLE_HOURLY, ex_poweronaftercreate=True, ex_wait=True):
        """
        ...
        request_data = {'name': name, 'password': password or '', 'passwordValidate': password or '', 'ssh-key': pubkey or '', 'datacenter': location.id, 'image': image.id, 'cpu': '{}{}'.format(size.extra['cpuCores'], size.extra['cpuType']), 'ram': size.ram, 'disk': ' '.join(['size=%d' % disksize for disksize in [size.disk] + size.extra['extraDiskSizesGB']]), 'dailybackup': 'yes' if ex_dailybackup else 'no', 'managed': 'yes' if ex_managed else 'no', 'network': ' '.join([','.join(['{}={}'.format(k, v) for k, v in network.items()]) for network in ex_networks]), 'quantity': 1, 'billingcycle': ex_billingcycle, 'monthlypackage': size.extra['monthlyTrafficPackage'] or '', 'poweronaftercreate': 'yes' if ex_poweronaftercreate else 'no'}
        ...
        node = self.ex_get_node(name=name, size=size, image=image, location=location, dailybackup=ex_dailybackup, managed=ex_managed, billingcycle=ex_billingcycle, generated_password=generated_password, create_command_id=command_ids[0], poweronaftercreate=ex_poweronaftercreate)
        if ex_wait:
            ...
            response = self.connection.request('/service/server/info', method='POST', data=json.dumps({'name': node.name}))
    ...
    def list_nodes(self, ex_name_regex=None, ex_full_details=False, ex_id=None):
        ...
        if ex_name_regex or ex_full_details or ex_id:
            ...
            return [self.ex_get_node(id=server['id'], name=server['name'], state=NodeState.RUNNING if server['power'] == 'on' else NodeState.STOPPED, location=self.ex_get_location(server['datacenter'])) for server in response.object]
    ...
    def ex_get_size(self, ramMB, diskSizeGB, cpuType, cpuCores, extraDiskSizesGB=None, monthlyTrafficPackage=None, id=None, name=None):
        """
        ...
        if not name:
            name = id
        return NodeSize(id=id, name=name, ram=ramMB, disk=diskSizeGB, bandwidth=0, price=0, driver=self.connection.driver, extra={'cpuType': cpuType, 'cpuCores': cpuCores, 'monthlyTrafficPackage': monthlyTrafficPackage, 'extraDiskSizesGB': extraDiskSizesGB or []})
    ...
    def ex_get_node(self, id=None, name=None, state=NodeState.UNKNOWN, public_ips=None, private_ips=None, size=None, image=None, created_at=None, location=None, dailybackup=None, managed=None, billingcycle=None, generated_password=None, create_command_id=None, poweronaftercreate=None):
        """
        ...
        return Node(id=id, name=name, state=state, public_ips=public_ips, private_ips=private_ips, driver=self, size=size, image=image, created_at=created_at, extra=extra)
