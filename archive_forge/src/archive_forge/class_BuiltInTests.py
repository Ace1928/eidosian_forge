    ...
    class Tests(TestCase):
        ...
        def test_zip(self):
            with FugueWorkflow() as dag:
                ...
                c2 = dag.zip(a, b, how='left_outer', partition=dict(presort='b DESC, c ASC'))
                c1.assert_eq(c2)
        ...
        def test_transform(self):
            with FugueWorkflow() as dag:
                ...
                dag.df([[1, 2, 10], [None, 1, 10], [3, 4, 10], [None, 4, 10]], 'a:double,b:int,p:int').assert_eq(c)
            dag.run(self.engine)
        ...
        def test_join(self):
            with FugueWorkflow() as dag:
                ...
                d.show()
                dag.df([[1, 10, None, None], [2, 20, 200, 2000], [3, 30, 300, None]], 'a:int,b:int,c:int,d:int').assert_eq(d)
                d = a.right_outer_join(b, c)
                dag.df([[2, 20, 200, 2000], [4, None, None, 4000]], 'a:int,b:int,c:int,d:int').assert_eq(d)
                d = a.full_outer_join(b, c)
                dag.df([[1, 10, None, None], [2, 20, 200, 2000], [3, 30, 300, None], [4, None, None, 4000]], 'a:int,b:int,c:int,d:int').assert_eq(d)
                a = dag.df([[1, 10], [2, 20]], 'a:int,b:int')
                b = dag.df([[2], [3]], 'c:int')
                c = dag.df([[4]], 'd:int')
                d = a.cross_join(b, c)
                dag.df([[1, 10, 2, 4], [1, 10, 3, 4], [2, 20, 2, 4], [2, 20, 3, 4]], 'a:int,b:int,c:int,d:int').assert_eq(d)
            dag.run(self.engine)
        ...
        def test_aggregate(self):
            with FugueWorkflow() as dag:
                ...
                a.aggregate(ff.min(-col('y')), zz=ff.max(col('y')), ww=((ff.min(col('y')) + ff.max(col('y'))) / 3).cast('int32')).assert_eq(c)
            dag.run(self.engine)
        ...
        def test_select(self):
            ...
            with FugueWorkflow() as dag:
                ...
                dag.select('  SELECT t1.*,z AS zb FROM ', a, 'AS t1 INNER JOIN', b, 'AS t2 ON t1.x=t2.x  ').assert_eq(c)
                dag.select('t1.*,z AS zb FROM ', a, 'AS t1 INNER JOIN', b, 'AS t2 ON t1.x=t2.x').assert_eq(c)
                dag.select('SELECT t1.*,z AS zb FROM ', a, 'AS t1 INNER JOIN', b, 'AS t2 ON t1.x=t2.x', sql_engine='qpdpandas').assert_eq(c)
                dag.select('SELECT t1.*,z AS zb FROM ', a, 'AS t1 INNER JOIN', b, 'AS t2 ON t1.x=t2.x', sql_engine=MockEngine, sql_engine_params={'p': 2}).assert_eq(c)
        ...
        def test_union(self):
            with FugueWorkflow() as dag:
                ...
                a.union().assert_eq(a)
                a.union(b, c).assert_eq(ArrayDataFrame([[1, 10], [2, None], [2, 20]], 'x:long,y:double'))
                a.union(b, c, distinct=False).assert_eq(ArrayDataFrame([[1, 10], [2, None], [2, None], [2, None], [2, 20], [1, 10], [2, 20]], 'x:long,y:double'))
            dag.run(self.engine)
        ...
        def test_intersect(self):
            with FugueWorkflow() as dag:
                ...
                b = dag.df([[2, None], [2, 20]], 'x:long,y:double')
                c = dag.df([[1, 10], [2, 20]], 'x:long,y:double')
                a.intersect(b).assert_eq(ArrayDataFrame([[2, None]], 'x:long,y:double'))
                a.intersect(b, c).assert_eq(ArrayDataFrame([], 'x:long,y:double'))
            dag.run(self.engine)
        ...
        def test_subtract(self):
            with FugueWorkflow() as dag:
                ...
                b = dag.df([[2, None], [2, 20]], 'x:long,y:double')
                c = dag.df([[1, 10], [2, 20]], 'x:long,y:double')
                a.subtract(b).assert_eq(ArrayDataFrame([[1, 10]], 'x:long,y:double'))
                a.subtract(c).assert_eq(ArrayDataFrame([[2, None]], 'x:long,y:double'))
                a.subtract(b, c).assert_eq(ArrayDataFrame([], 'x:long,y:double'))
            dag.run(self.engine)

        def test_distinct(self):
            with FugueWorkflow() as dag:
                a = dag.df([[1, 10], [2, None], [2, None]], 'x:long,y:double')
                a.distinct().assert_eq(ArrayDataFrame([[1, 10], [2, None]], 'x:long,y:double'))
            dag.run(self.engine)

        def test_dropna(self):
            with FugueWorkflow() as dag:
                a = dag.df([[1, 10, 10], [None, 2, None], [2, None, 4]], 'x:double,y:double,z:double')
                a.dropna().assert_eq(ArrayDataFrame([[1, 10, 10]], 'x:double,y:double,z:double'))
                a.dropna(how='all').assert_eq(a)
                a.dropna(thresh=2).assert_eq(ArrayDataFrame([[1, 10, 10], [2, None, 4]], 'x:double,y:double,z:double'))
                a.dropna(how='any', subset=['x', 'z']).assert_eq(ArrayDataFrame([[1, 10, 10], [2, None, 4]], 'x:double,y:double,z:double'))
        ...
        def test_fillna(self):
            with FugueWorkflow() as dag:
                a = dag.df([[1, 10, 10], [None, 2, None], [2, None, 4]], 'x:double,y:double,z:double')
                a.fillna(-99).assert_eq(ArrayDataFrame([[1, 10, 10], [-99, 2, -99], [2, -99, 4]], 'x:double,y:double,z:double'))
                a.fillna(-99, subset=['y']).assert_eq(ArrayDataFrame([[1, 10, 10], [None, 2, None], [2, -99, 4]], 'x:double,y:double,z:double'))
                a.fillna({'y': 0, 'z': -99}, subset=['y']).assert_eq(ArrayDataFrame([[1, 10, 10], [None, 2, -99], [2, 0, 4]], 'x:double,y:double,z:double'))
            dag.run(self.engine)
            with raises(FugueWorkflowError):
                with FugueWorkflow() as dag:
        ...
        def test_take(self):
            with FugueWorkflow() as dag:
                df = dag.df(pd.DataFrame({'aaa': [1, 1, 2, 2], 'bbb': ['a', 'b', 'c', 'd']}))
            ...
            dag.run(self.engine)
            with FugueWorkflow() as dag:
                a = dag.df([['0', 1, 1], ['0', 2, 1], ['1', 3, 1], ['1', 4, 1], [None, 2, 1], [None, 3, 1]], 'a:str,b:int,c:long')
                a.partition(by=['a'], presort='b desc').take(n=1).assert_eq(ArrayDataFrame([['0', 2, 1], ['1', 4, 1], [None, 3, 1]], 'a:str,b:int,c:long'))
            dag.run(self.engine)
            with FugueWorkflow() as dag:
                a = dag.df([[0, 1], [0, 2], [1, 3]], 'a:int,b:int')
                a.take(1, presort='a,b desc').assert_eq(ArrayDataFrame([[0, 2]], 'a:int,b:int'))
            dag.run(self.engine)
            with FugueWorkflow() as dag:
                a = dag.df([['0', 1], ['0', 2], ['1', 3], ['1', 4], [None, 2], [None, 3]], 'a:str,b:double')
                a.partition(by=['a'], presort='b desc').take(n=1, presort='b asc').assert_eq(ArrayDataFrame([['0', 1], ['1', 3], [None, 2]], 'a:str,b:double'))
            dag.run(self.engine)
            with FugueWorkflow() as dag:
                a = dag.df([['0', 1], ['0', 2], [None, 3]], 'a:str,b:double')
                a.take(1, presort='a desc', na_position='first').assert_eq(ArrayDataFrame([[None, 3]], 'a:str,b:double'))
            dag.run(self.engine)
            with FugueWorkflow() as dag:
                a = dag.df([['0', 1], ['1', 2], [None, 3]], 'a:str,b:double')
                a.take(1, presort='a desc', na_position='last').assert_eq(ArrayDataFrame([['1', 2]], 'a:str,b:double'))
            dag.run(self.engine)
            with FugueWorkflow() as dag:
                a = dag.df([[0, 1], [0, 2], [1, 3]], 'a:int,b:int')
                a.take(1).show()
            ...
            with FugueWorkflow() as dag:
                df = dag.df(pd.DataFrame({'aaa': [1, 1, 2, 2], 'bbb': ['a', 'b', 'c', 'd']}))
                df1 = df.partition(by=['aaa'], presort='bbb').take(1).take(1, presort='aaa')
                df1.show()
                df2 = df.partition(by=['aaa'], presort='bbb').take(1).partition(by=['aaa'], presort='bbb').take(1, presort='aaa')
                df2.show()
            dag.run(self.engine)
        ...
        def test_io(self):
            ...
            pd.testing.assert_frame_equal(pdf, pd.DataFrame({'c': [6, 2], 'a': [1, 7]}).reset_index(drop=True), check_like=True, check_dtype=False)
        ...
        def test_transformer_validation(self):
            ...
            for t in [t1, t2, T3]:
                ...
                with raises(FugueWorkflowRuntimeValidationError):
                    ...
                    dag.run(self.engine)
                with FugueWorkflow() as dag:
                    dag.df([[0, 1]], 'a:int,b:int').partition(by=['b']).transform(t).assert_eq(dag.df([[0, 1]], 'a:int,b:int'))
                dag.run(self.engine)
        ...
        def test_callback(self):
            ...
            with FugueWorkflow() as dag:
                ...
                res = df.partition(by=['a']).transform(CallbackTransformer, callback=cb.call, params=dict(has=True))
                df.assert_eq(res)
                res = df.partition(by=['a']).transform(CallbackTransformer, params=dict(has=False))
                df.assert_eq(res)
            dag.run(self.engine)
            assert 8 == cb.n
            cb2 = Callbacks()
            ...
            def t12(df: pd.DataFrame, c: Optional[Callable[[int], int]]=None) -> pd.DataFrame:
                if c is not None:
                    c(1)
                return df
            ...
            with FugueWorkflow() as dag:
                ...
                df.partition(by=['a']).out_transform(t12, callback=cb2.call)
                df.partition(by=['a']).out_transform(t12)
                raises(FugueInterfacelessError, lambda: df.partition(by=['a']).out_transform(t1))
                df.assert_eq(res)
            dag.run(self.engine)
            assert 9 == cb2.n
            cb2 = Callbacks()

            def t3(df1: pd.DataFrame, df2: pd.DataFrame, c: Callable[[int], int]) -> pd.DataFrame:
                c(1)
                return df1

            def t4(df1: pd.DataFrame, df2: pd.DataFrame, c: Callable[[int], int]) -> None:
                c(1)

            def t42(df1: pd.DataFrame, df2: pd.DataFrame, c: Optional[Callable[[int], int]]) -> None:
                if c is not None:
                    c(1)
            with FugueWorkflow() as dag:
                ...
                df1.zip(df2).out_transform(t42, callback=cb2.call)
                df1.zip(df2).out_transform(t42)
                raises(FugueInterfacelessError, lambda: df1.zip(df2).transform(t3).persist())
            dag.run(self.engine)
            assert 9 == cb2.n
