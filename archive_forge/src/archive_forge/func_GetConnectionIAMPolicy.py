...
def GetConnectionIAMPolicy(self, reference):
    ...
    """
    bq_id_utils.typecheck(reference, bq_id_utils.ApiClientHelper.ConnectionReference, method='GetConnectionIAMPolicy')
    client = self.GetConnectionV1ApiClient()
    return client.projects().locations().connections().getIamPolicy(resource=reference.path()).execute()