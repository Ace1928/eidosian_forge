...
class InterpolatablePlot:
    ...
    def draw_legend(self, *, show_tolerance=True, tolerance=None, kinkiness=None):
        ...
        self.draw_circle(cr, x=xx, y=y + self.font_size * 0.5, diameter=self.kink_circle_size, stroke_width=self.kink_circle_stroke_width, color=self.kink_circle_color)
        y -= self.pad + self.font_size
        self.draw_label('Point causing kink in the contour', x=xxx, y=y, width=width)
        self.draw_dot(cr, x=xx, y=y + self.font_size * 0.5, diameter=self.kink_point_size, color=self.kink_point_color)
        y -= self.pad + self.font_size
        self.draw_label('Suggested new contour start point', x=xxx, y=y, width=width)
        self.draw_dot(cr, x=xx, y=y + self.font_size * 0.5, diameter=self.corrected_start_point_size, color=self.corrected_start_point_color)
        y -= self.pad + self.font_size
        self.draw_label('Contour start point in contours with wrong direction', x=xxx, y=y, width=width)
        self.draw_arrow(cr, x=xx - self.start_arrow_length * 0.3, y=y + self.font_size * 0.5, color=self.wrong_start_point_color)
        y -= self.pad + self.font_size
        self.draw_label('Contour start point when the first two points overlap', x=xxx, y=y, width=width)
        self.draw_dot(cr, x=xx, y=y + self.font_size * 0.5, diameter=self.corrected_start_point_size, color=self.start_point_color)
        y -= self.pad + self.font_size
        self.draw_label('Contour start point and direction', x=xxx, y=y, width=width)
        self.draw_arrow(cr, x=xx - self.start_arrow_length * 0.3, y=y + self.font_size * 0.5, color=self.start_point_color)
        y -= self.pad + self.font_size
        self.draw_label('Legend:', x=x, y=y, width=width, bold=True)
        y -= self.pad + self.font_size
        if kinkiness is not None:
            self.draw_label('Kink-reporting aggressiveness: %g' % kinkiness, x=xxx, y=y, width=width)
            y -= self.pad + self.font_size
        if tolerance is not None:
            self.draw_label('Error tolerance: %g' % tolerance, x=xxx, y=y, width=width)
            y -= self.pad + self.font_size
    ...
    def add_summary(self, problems):
        ...
        height = self.height - 2 * self.pad
        x = y = pad
        self.draw_label('Summary of problems', x=x, y=y, bold=True, width=width, font_size=self.title_font_size)
        y += self.title_font_size
        ...
        for problem_type in sorted(glyphs_per_problem, key=lambda x: InterpolatableProblem.severity[x]):
            y += self.font_size
            self.draw_label('%s: %d' % (problem_type, len(glyphs_per_problem[problem_type])), x=x, y=y, width=width, bold=True)
            y += self.font_size
    ...
    def draw_arrow(self, cr, *, x=0, y=0, color=(0, 0, 0)):
        ...
        cr.move_to(0, 0)
        cr.line_to(-self.start_arrow_length, -self.start_arrow_length * 0.4)
        cr.line_to(-self.start_arrow_length, self.start_arrow_length * 0.4)
        cr.close_path()
        cr.fill()
        cr.restore()
