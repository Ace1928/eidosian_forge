...
class HostAggregate(resource.Resource):
    ...
    entity = 'aggregates'
    PROPERTIES = NAME, AVAILABILITY_ZONE, HOSTS, METADATA = ('name', 'availability_zone', 'hosts', 'metadata')
    properties_schema = {NAME: properties.Schema(properties.Schema.STRING, _('Name for the aggregate.'), required=True, update_allowed=True), AVAILABILITY_ZONE: properties.Schema(properties.Schema.STRING, _('Name for the availability zone.'), update_allowed=True), HOSTS: properties.Schema(properties.Schema.LIST, _('List of hosts to join aggregate.'), update_allowed=True, schema=properties.Schema(properties.Schema.STRING, constraints=[constraints.CustomConstraint('nova.host')])), METADATA: properties.Schema(properties.Schema.MAP, _('Arbitrary key/value metadata to store information for aggregate.'), update_allowed=True, default={})}
    ...
    def handle_create(self):
        ...
        aggregate = self.client().aggregates.create(name=name, availability_zone=availability_zone)
