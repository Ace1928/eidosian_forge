...
def _GetSecretsChanges(args, container_name=None):
    ...
    volume_kwargs = {}
    env_kwargs = {}
    updates = _StripKeys(getattr(args, 'update_secrets', None) or args.set_secrets or {})
    volume_kwargs['updates'] = {k: secrets_mapping.ReachableSecret(v, _ValidatedMountPoint(k)) for k, v in updates.items() if _IsVolumeMountKey(k)}
    env_kwargs['updates'] = {k: secrets_mapping.ReachableSecret(v, k) for k, v in updates.items() if not _IsVolumeMountKey(k)}
    ...
    if any(env_kwargs.values()):
        secret_changes.append(config_changes.SecretEnvVarChanges(container_name=container_name, **env_kwargs))
    if any(volume_kwargs.values()):
        secret_changes.append(config_changes.SecretVolumeChanges(container_name=container_name, **volume_kwargs))
