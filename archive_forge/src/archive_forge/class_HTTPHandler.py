...
class HTTPHandler(StorageHandler):
    ...
    def load_path(self, manifest_entry: ArtifactManifestEntry, local: bool=False) -> Union[URIStr, FilePathStr]:
        ...
        if hit:
            return path
        response = self._session.get(manifest_entry.ref, stream=True, cookies=_thread_local_api_settings.cookies, headers=_thread_local_api_settings.headers)
        ...
        if manifest_entry.digest != digest:
            raise ValueError(f'Digest mismatch for url {manifest_entry.ref}: expected {manifest_entry.digest} but found {digest}')
    ...
    def store_path(self, artifact: 'Artifact', path: Union[URIStr, FilePathStr], name: Optional[StrPath]=None, checksum: bool=True, max_objects: Optional[int]=None) -> Sequence[ArtifactManifestEntry]:
        ...
        with self._session.get(path, stream=True, cookies=_thread_local_api_settings.cookies, headers=_thread_local_api_settings.headers) as response:
            response.raise_for_status()
            digest: Optional[Union[ETag, FilePathStr, URIStr]]
            digest, size, extra = self._entry_from_headers(response.headers)
            digest = digest or path
        return [ArtifactManifestEntry(path=name, ref=path, digest=digest, size=size, extra=extra)]
