        ...
        for m in members:
            if hasattr(m[1], 'clidesc'):
                ...
                if not cmd in self.commands:
                    self.commands[cmd] = {}
                self.commands[cmd][subcommand] = {'args': inspect.getargspec(fn)[0][1:], 'optional': len(inspect.getargspec(fn)[3]) if inspect.getargspec(fn)[3] else 0, 'desc': fn.clidesc, 'fn': fn, 'order': fn.cliorder}
