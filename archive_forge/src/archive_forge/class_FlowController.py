...
class FlowController(object):
    ...
    def add(self, message: MessageType) -> None:
        ...
        with self._operational_lock:
            if not self._would_overflow(message):
                ...
                return
            if self._settings.limit_exceeded_behavior == types.LimitExceededBehavior.ERROR:
                load_info = self._load_info(message_count=self._message_count + 1, total_bytes=self._total_bytes + message._pb.ByteSize())
                error_msg = 'Flow control limits would be exceeded - {}.'.format(load_info)
                raise exceptions.FlowControlLimitError(error_msg)
            assert self._settings.limit_exceeded_behavior == types.LimitExceededBehavior.BLOCK
            if message._pb.ByteSize() > self._settings.byte_limit or self._settings.message_limit < 1:
                load_info = self._load_info(message_count=1, total_bytes=message._pb.ByteSize())
                error_msg = 'Total flow control limits too low for the message, would block forever - {}.'.format(load_info)
                raise exceptions.FlowControlLimitError(error_msg)
            current_thread = threading.current_thread()
            while self._would_overflow(message):
                if current_thread not in self._waiting:
                    reservation = _QuantityReservation(bytes_reserved=0, bytes_needed=message._pb.ByteSize(), has_slot=False)
                    self._waiting[current_thread] = reservation
                _LOGGER.debug('Blocking until there is enough free capacity in the flow - {}.'.format(self._load_info()))
                self._has_capacity.wait()
                _LOGGER.debug('Woke up from waiting on free capacity in the flow - {}.'.format(self._load_info()))
            self._message_count += 1
            self._total_bytes += message._pb.ByteSize()
    ...
    def release(self, message: MessageType) -> None:
        ...
        with self._operational_lock:
            ...
            if self._message_count < 0 or self._total_bytes < 0:
                warnings.warn('Releasing a message that was never added or already released.', category=RuntimeWarning, stacklevel=2)
    ...
    def _load_info(self, message_count: Optional[int]=None, total_bytes: Optional[int]=None) -> str:
        ...
        return f'messages: {message_count} / {self._settings.message_limit} (reserved: {self._reserved_slots}), bytes: {total_bytes} / {self._settings.byte_limit} (reserved: {self._reserved_bytes})'