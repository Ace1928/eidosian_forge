...
class NumPyPrinter(ArrayPrinter, PythonCodePrinter):
    ...
    def _print_MatPow(self, expr):
        ...
        return '{}({}, {})'.format(self._module_format(self._module + '.linalg.matrix_power'), self._print(expr.args[0]), self._print(expr.args[1]))
    ...
    def _print_MatrixSolve(self, expr):
        return '%s(%s, %s)' % (self._module_format(self._module + '.linalg.solve'), self._print(expr.matrix), self._print(expr.vector))
    ...
    def _print_FunctionMatrix(self, expr):
        ...
        return '{}(lambda {}: {}, {})'.format(self._module_format(self._module + '.fromfunction'), ', '.join((self._print(arg) for arg in lamda.args[0])), self._print(lamda.args[1]), self._print(expr.shape))

    def _print_HadamardProduct(self, expr):
        func = self._module_format(self._module + '.multiply')
        return ''.join(('{}({}, '.format(func, self._print(arg)) for arg in expr.args[:-1])) + '{}{}'.format(self._print(expr.args[-1]), ')' * (len(expr.args) - 1))
    ...
    def _print_Adjoint(self, expr):
        return '{}({}({}))'.format(self._module_format(self._module + '.conjugate'), self._module_format(self._module + '.transpose'), self._print(expr.args[0]))
    ...
    def _print_Piecewise(self, expr):
        ...
        return '{}({}, {}, default={})'.format(self._module_format(self._module + '.select'), conds, exprs, self._print(S.NaN))
    ...
    def _print_Relational(self, expr):
        ...
        if expr.rel_op in op:
            ...
            return '{op}({lhs}, {rhs})'.format(op=self._module_format(self._module + '.' + op[expr.rel_op]), lhs=lhs, rhs=rhs)
    ...
    def _print_Mod(self, expr):
        return '%s(%s)' % (self._module_format(self._module + '.mod'), ', '.join((self._print(arg) for arg in expr.args)))
