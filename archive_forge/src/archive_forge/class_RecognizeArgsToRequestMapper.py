...
class RecognizeArgsToRequestMapper:
    ...
    def UpdateAlphaArgsInRecognitionConfig(self, args, config):
        ...
        if args.interaction_type is not None or args.original_media_type is not None or args.naics_code is not None or (args.microphone_distance is not None) or (args.recording_device_type is not None) or (args.recording_device_name is not None) or (args.original_mime_type is not None) or (args.audio_topic is not None):
            if config.metadata is None:
                config.metadata = config.field_by_name('metadata').message_type()
            config.metadata.interactionType = self._interaction_type_mapper.GetEnumForChoice(args.interaction_type)
            config.metadata.originalMediaType = self._original_media_type_mapper.GetEnumForChoice(args.original_media_type)
            config.metadata.industryNaicsCodeOfAudio = args.naics_code
            config.metadata.microphoneDistance = self._microphone_distance_type_mapper.GetEnumForChoice(args.microphone_distance)
            config.metadata.recordingDeviceType = self._device_type_mapper.GetEnumForChoice(args.recording_device_type)
