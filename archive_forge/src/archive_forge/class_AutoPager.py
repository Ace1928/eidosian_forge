...
class AutoPager:
    ...
    def __init__(self, output_stream: Optional[TextIO]=None, *, pager_command: command.CommandType=command.DefaultPager(), allow_color: bool=True, line_buffering: Optional[bool]=None, reset_on_exit: bool=False, errors: Optional[ErrorStrategy]=None):
        ...
        self._config = command.PagerConfig(color=allow_color, line_buffering_requested=bool(line_buffering), reset_terminal=reset_on_exit)
        self._set_line_buffering = line_buffering
        self._set_errors = ErrorStrategy(errors) if errors is not None else None
    ...
    def __exit__(self, exc_type: Optional[Type[BaseException]], exc: Optional[BaseException], traceback: Optional[types.TracebackType]) -> bool:
