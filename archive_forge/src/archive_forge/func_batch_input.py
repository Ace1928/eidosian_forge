    ...
    **Example**
    ...
    if any((num in tape.trainable_params for num in argnum)):
        if qml.math.get_interface(*argnum_params) != 'jax':
            raise ValueError('Batched inputs must be non-trainable. Please make sure that the parameters indexed by ' + "'argnum' are not marked as trainable.")
    batch_dims = np.unique([qml.math.shape(x)[0] for x in argnum_params])
    if len(batch_dims) != 1:
        raise ValueError("Batch dimension for all gate arguments specified by 'argnum' must be the same.")
