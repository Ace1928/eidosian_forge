...
class IBMSVCSupportRemoteAssistance:
    ...
    def __init__(self):
        ...
        argument_spec.update(dict(state=dict(type='str', required=True, choices=['enabled', 'disabled']), support=dict(type='str', required=True, choices=['remote', 'onsite']), name=dict(type='list', elements='str'), sra_ip=dict(type='list', elements='str'), sra_port=dict(type='list', elements='str')))
    ...
    def basic_checks(self):
        self.filtered_params = dict(filter(lambda item: item[0] in ['name', 'sra_ip', 'sra_port'], self.module.params.items()))
        if self.support == 'remote' and self.state == 'enabled':
            if self.name and self.sra_ip and self.sra_port:
                if len(self.name) == len(self.sra_ip) == len(self.sra_port):
                    if not all([all(self.name), all(self.sra_ip), all(self.sra_port)]):
                        ...
                        self.module.fail_json(msg='{0} should not contain blank values'.format(missing_params))
                else:
                    self.module.fail_json(msg='Name, sra_ip and sra_port parameters should contain same number of arguments')
            else:
            ...
            if self.sra_ip or self.sra_port:
                ...
                self.module.fail_json(msg='{0} should not be passed when support=remote and state=disabled'.format(invalid_params))
            ...
            if self.name or self.sra_ip or self.sra_port:
                ...
                self.module.fail_json(msg='{0} should not be passed when support=onsite'.format(invalid_params))
        ...
        if self.is_sra_enabled():
            if self.state == 'enabled':
                if not self.sra_probe():
                    self.msg = 'Support remote assistance already enabled. No modifications done.'
            else:
                self.disable_sra()
            ...
            self.enable_sra()
            self.msg = 'Support remote assistance({0}) enabled.'.format(self.support)
        if self.module.check_mode:
