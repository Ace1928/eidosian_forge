...
class cuDFOnRayDataframePartition(PandasDataframePartition):
    ...
    def __copy__(self):
        ...
        """
        return self.__constructor__(self.gpu_manager, self.key, self._length_cache, self._width_cache)
    ...
    def apply(self, func, *args, **kwargs):
        ...
        **kwargs : dict
        ...
        """
        return self.gpu_manager.apply.remote(self.get_key(), None, func, *args, **kwargs)
    ...
    def add_to_apply_calls(self, func, length=None, width=None, *args, **kwargs):
        ...
        **kwargs : dict
        ...
        """
        return self.__constructor__(self.gpu_manager, self.apply(func, *args, **kwargs), length=length, width=width)
    ...
    def mask(self, row_labels, col_labels):
        ...
        ray.ObjectRef
            ...
            in internal dict-storage of `self.gpu_manager`.
        """
        if (isinstance(row_labels, slice) and row_labels == slice(None) or (not isinstance(row_labels, slice) and self._length_cache is not None and (len(row_labels) == self._length_cache))) and (isinstance(col_labels, slice) and col_labels == slice(None) or (not isinstance(col_labels, slice) and self._width_cache is not None and (len(col_labels) == self._width_cache))):
            return self.__copy__()

        def iloc(df, row_labels, col_labels):
            if isinstance(df.index, cudf.core.multiindex.MultiIndex) and is_list_like(row_labels):
                new_row_labels = cp.full((1, df.index.size), False, dtype=bool).squeeze()
        ...
        return self.gpu_manager.apply.remote(self.key, None, iloc, col_labels=col_labels, row_labels=row_labels)
    ...
    def get_key(self):
        ...
        int
        """
        return RayWrapper.materialize(self.key) if isinstance(self.key, ray.ObjectRef) else self.key
    ...
    def to_pandas(self):
        ...
        pandas.DataFrame
        """
        return RayWrapper.materialize(self.gpu_manager.apply_non_persistent.remote(self.get_key(), None, cudf.DataFrame.to_pandas))
    ...
    def to_numpy(self):
        ...
        return self.gpu_manager.apply_result_not_dataframe.remote(self.get_key(), convert)
