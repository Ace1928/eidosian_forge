...
class DictionaryAgent(Agent):
    ...
    def add_cmdline_args(argparser):
        ...
        dictionary.add_argument('-df', '--dict-file', help='path to dictionary file. defaults to [model_file].dict if not set and model_file is set.', hidden=True)
        dictionary.add_argument('--dict-initpath', hidden=True, help='path to a saved dictionary to load tokens / counts from to seed the dictionary with initial tokens and/or frequencies')
        dictionary.add_argument('--dict-language', default=DictionaryAgent.default_lang, hidden=True, help='sets language for the punkt sentence tokenizer')
        dictionary.add_argument('--dict-max-ngram-size', type=int, hidden=True, default=DictionaryAgent.default_maxngram, help='looks for ngrams of up to this size. this is ignored when building the dictionary. note: this takes approximate runtime of len(sentence)^max_ngram_size')
        dictionary.add_argument('--dict-minfreq', default=DictionaryAgent.default_minfreq, type=int, help='minimum frequency of words to include them in sorted dict or minimum frequency of bpe codecs', hidden=True)
        dictionary.add_argument('--dict-maxtokens', default=DictionaryAgent.default_maxtokens, type=int, help='max number of tokens to include in dictionary or bpe codecs', hidden=True)
        dictionary.add_argument('--dict-nulltoken', default=DictionaryAgent.default_null, hidden=True, help='empty token, can be used for padding or just empty values')
        dictionary.add_argument('--dict-starttoken', default=DictionaryAgent.default_start, hidden=True, help='token for starting sentence generation, if needed')
        dictionary.add_argument('--dict-endtoken', default=DictionaryAgent.default_end, hidden=True, help='token for end of sentence markers, if needed')
        dictionary.add_argument('--dict-unktoken', default=DictionaryAgent.default_unk, hidden=True, help='token to return for unavailable words')
        dictionary.add_argument('-tok', '--dict-tokenizer', default=DictionaryAgent.default_tok, help='Which tokenizer to use. Defaults to "split", which splits on whitespace as well as recognizing basic punctuation. Other options include nltk, gpt2 and bytelevelbpe.', hidden=True)
        dictionary.add_argument('--dict-lower', default=DictionaryAgent.default_lower, type='bool', help='Whether or not to lowercase all text seen.', hidden=True)
        dictionary.add_argument('--bpe-debug', action='store_true', hidden=True, help='Leave BPE tokens untouched in output. Useful for debugging.')
        dictionary.add_argument('--dict-textfields', default=DictionaryAgent.default_textfields, hidden=True, help='Observation fields which dictionary learns vocabulary from. Tasks with additional fields may add to this list to handle any extra vocabulary.')
    ...
    def __init__(self, opt: Opt, shared=None):
        ...
        try:
            ...
            raise AttributeError('tokenizer type {} not yet supported'.format(self.tokenizer))
        if shared:
            ...
            if not loaded and opt.get('dict_initpath'):
                opt['dict_initpath'] = modelzoo_path(opt.get('datapath'), opt['dict_initpath'])
    ...
    def save(self, filename=None, append=False, sort=True):
        ...
        if self.tokenizer in ['bpe', 'gpt2', 'bytelevelbpe', 'slow_bytelevel_bpe']:
            needs_removal = self.bpe.finalize(self.freq, num_symbols=self.maxtokens, minfreq=self.minfreq)
            if needs_removal:
    ...
    def sort(self, trim=True):
        ...
        if trim and self.tokenizer == 'bytelevelbpe':
            raise RuntimeError('You should not trim the dictionary when using bytelevelbpe.')
        if trim:
    ...
    def vec2txt(self, vector, delimiter=' '):
        ...
        if self.tokenizer in ['gpt2', 'bpe', 'slow_bytelevel_bpe']:
            ...
            extra_tokens = 4
            vector = [self.bpe.special_tok_map[idx] if idx in self.bpe.special_tok_map else idx - extra_tokens for idx in vector]
            tokens = [self[int(idx)] for idx in vector]
            text = self.bpe.decode(tokens, vector, delimiter)
        else:
