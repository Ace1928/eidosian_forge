...
class TestNumericRange(unittest.TestCase):
    ...
    def test_init(self):
        ...
        self.assertEqual(a.step, 0)
        with self.assertRaisesRegex(ValueError, '.*start must be <= end for continuous ranges'):
            NR(0, -1, 0)
        with self.assertRaisesRegex(ValueError, '.*start must not be None'):
            NR(None, None, 1)
        with self.assertRaisesRegex(ValueError, '.*step must be int'):
            NR(None, None, 1.5)
        with self.assertRaisesRegex(ValueError, '.*start, end ordering incompatible with step direction'):
            NR(0, 1, -1)
        with self.assertRaisesRegex(ValueError, '.*start, end ordering incompatible with step direction'):
            NR(1, 0, 1)
        with self.assertRaisesRegex(ValueError, '\\[0:1\\] is discrete, but passed closed=\\(False, True\\)'):
            NR(0, 1, 1, '(]')
        a = NR(0, None, 1)
        self.assertEqual(a.start, 0)
        self.assertEqual(a.end, _inf)
        ...
        self.assertEqual(a.step, 1)
        with self.assertRaisesRegex(ValueError, '.*start, end ordering incompatible with step'):
            NR(0, -1, 1)
        with self.assertRaisesRegex(ValueError, '.*start, end ordering incompatible with step'):
            NR(0, 1, -2)
    ...
    def test_lcm(self):
        ...
        self.assertEqual(NR(0, None, 3)._step_lcm((NR(0, None, 2), NR(0, None, 10))), 30)

    def test_range_difference(self):
        self.assertEqual(NR(0, None, 1).range_difference([NR(1, None, 0)]), [NR(0, 0, 0)])
        self.assertEqual(NR(0, None, 1).range_difference([NR(0, 0, 0)]), [NR(1, None, 1)])
        self.assertEqual(NR(0, None, 2).range_difference([NR(10, None, 3)]), [NR(0, None, 6), NR(2, None, 6), NR(4, 4, 0)])
        with self.assertRaisesRegex(ValueError, 'Unknown range type, list'):
            NR(0, None, 0).range_difference([[0]])
        self.assertEqual(NR(0, 7, 2).range_difference([NR(6, None, 10)]), [NR(0, 0, 0), NR(2, 2, 0), NR(4, 4, 0)])
        self.assertEqual(NR(10, 0, -1).range_difference([NR(7, 4, -2)]), [NR(10, 0, -2), NR(1, 3, 2), NR(9, 9, 0)])
        self.assertEqual(NR(0, None, -1).range_difference([NR(-10, 10, 0)]), [NR(-11, None, -1)])
        ...
        self.assertEqual(NR(0, None, 0).range_difference([NR(5, None, 0)]), [NR(0, 5, 0, '[)')])
        self.assertEqual(NR(0, None, 0).range_difference([NR(5, 10, 0)]), [NR(0, 5, 0, '[)'), NR(10, None, 0, '(]')])
        self.assertEqual(NR(None, 0, 0).range_difference([NR(-5, None, 0)]), [NR(None, -5, 0, '[)')])
        self.assertEqual(NR(None, 0, 0).range_difference([NR(-5, 0, 0, '[]')]), [NR(None, -5, 0, '[)')])
        self.assertEqual(NR(None, 0, 0).range_difference([NR(-5, 0, 0, '[)')]), [NR(None, -5, 0, '[)'), NR(0, 0, 0)])
        self.assertEqual(NR(0, 10, 0).range_difference([NR(None, 5, 0, '[)')]), [NR(5, 10, 0, '[]')])
        self.assertEqual(NR(0, None, 0).range_difference([NR(5, 10, 0, '()')]), [NR(0, 5, 0, '[]'), NR(10, None, 0, '[]')])
        self.assertEqual(NR(None, None, 0).range_difference([NR(5, 10, 5)]), [NR(None, 5, 0, '[)'), NR(5, 10, 0, '()'), NR(10, None, 0, '(]')])
        self.assertEqual(NR(-10, 20, 0).range_difference([NR(5, 10, 5)]), [NR(-10, 5, 0, '[)'), NR(5, 10, 0, '()'), NR(10, 20, 0, '(]')])
        self.assertEqual(NR(-10, 20, 0, '()').range_difference([NR(5, 10, 5)]), [NR(-10, 5, 0, '()'), NR(5, 10, 0, '()'), NR(10, 20, 0, '()')])
        self.assertEqual(NR(-3, 3, 0).range_difference([NR(0, None, 5), NR(0, None, -5)]), [NR(-3, 0, 0, '[)'), NR(0, 3, 0, '(]')])
        a = NR(0.25, 10, 1)
        self.assertEqual(a.range_difference([NR(0.5, 20, 1)]), [a])
        self.assertEqual(a.range_difference([NR(0.5, 20, 2)]), [NR(0.25, 8.25, 2), NR(1.25, 9.25, 2)])
        a = NR(0, 100, 2)
        self.assertEqual(a.range_difference([NR(1, 100, 4)]), [NR(0, 100, 4), NR(2, 98, 4)])
        a = NR(0, None, 2)
        self.assertEqual(a.range_difference([NR(1, None, 4)]), [NR(0, None, 4), NR(2, None, 4)])
        a = NR(0.25, None, 1)
        self.assertEqual(a.range_difference([NR(0.5, None, 1)]), [a])
        a = NR(None, None, 0)
        self.assertEqual(a.range_difference([NR(None, None, 0, '()')]), [NR(-_inf, -_inf, 0), NR(_inf, _inf, 0)])
        self.assertEqual(a.range_difference([NR(None, None, 0, '()'), NR(None, None, 0, '[)')]), [NR(_inf, _inf, 0)])
    ...
    def test_range_intersection(self):
        self.assertEqual(NR(0, None, 1).range_intersection([NR(1, None, 0)]), [NR(1, None, 1)])
        self.assertEqual(NR(0, None, 1).range_intersection([NR(0, 0, 0)]), [NR(0, 0, 0)])
        self.assertEqual(NR(0, None, 1).range_intersection([NR(0.5, 1.5, 0)]), [NR(1, 1, 0)])
        self.assertEqual(NR(0, None, 2).range_intersection([NR(1, None, 3)]), [NR(4, None, 6)])
        with self.assertRaisesRegex(ValueError, 'Unknown range type, list'):
        ...
        self.assertEqual(NR(10, 0, -1).range_intersection([NR(7, 4, -2)]), [NR(5, 7, 2)])
        self.assertEqual(NR(10, 0, -1).range_intersection([NR(7, None, -2)]), [NR(1, 7, 2)])
        self.assertEqual(NR(0, None, -1).range_intersection([NR(None, -10, 0)]), [NR(-10, None, -1)])
        self.assertEqual(NR(0, 5, 0).range_intersection([NR(5, 10, 0)]), [NR(5, 5, 0)])
        self.assertEqual(NR(0, None, 0).range_intersection([NR(5, None, 0)]), [NR(5, None, 0)])
        a = NR(0.25, 10, 1)
