...
class NetAppOntapNTPKey:
    ...
    def __init__(self):
        ...
        self.argument_spec.update(dict(state=dict(required=False, type='str', choices=['present', 'absent'], default='present'), id=dict(required=True, type='int'), digest_type=dict(required=True, type='str', choices=['sha1']), value=dict(required=True, type='str')))
    ...
    def modify_ntp_key(self, modify):
        ...
        if body:
            ...
            if error:
                self.module.fail_json(msg='Error modifying key with id %s: %s' % (self.parameters['id'], to_native(error)), exception=traceback.format_exc())
