...
def CheckArguments(self):
    ...
    """
    if not self.command_spec.file_url_ok and HaveFileUrls(self.args[self.command_spec.urls_start_arg:]):
        raise CommandException('"%s" command does not support "file://" URLs. Did you mean to use a gs:// URL?' % self.command_name)
    if not self.command_spec.provider_url_ok and HaveProviderUrls(self.args[self.command_spec.urls_start_arg:]):
        raise CommandException('"%s" command does not support provider-only URLs.' % self.command_name)