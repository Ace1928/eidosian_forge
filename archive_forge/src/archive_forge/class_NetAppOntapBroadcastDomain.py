...
class NetAppOntapBroadcastDomain(object):
    ...
    def modify_broadcast_domain(self):
        ...
        try:
            ...
            self.module.fail_json(msg='Error modifying broadcast domain %s: %s' % (self.parameters['name'], to_native(error)), exception=traceback.format_exc())
    ...
    def add_broadcast_domain_ports(self, ports):
        ...
        try:
            ...
            self.module.fail_json(msg='Error creating port for broadcast domain %s: %s' % (self.parameters['name'], to_native(error)), exception=traceback.format_exc())
    ...
    def delete_broadcast_domain_ports(self, ports):
        ...
        try:
            ...
            self.module.fail_json(msg='Error deleting port for broadcast domain %s: %s' % (self.parameters['name'], to_native(error)), exception=traceback.format_exc())
    ...
    def add_or_move_broadcast_domain_ports_rest(self, ports):
        ...
        body = {'broadcast_domain': {'name': self.parameters['name'], 'ipspace': {'name': self.parameters['ipspace']}}}
        for port in ports:
    ...
    def get_net_port_rest(self, port):
        ...
        node = port.split(':')[0]
        api = 'network/ethernet/ports'
        query = {'name': port_name, 'node.name': node}
        fields = 'name,uuid'
        record, error = rest_generic.get_one_record(self.rest_api, api, query, fields)
        if error:
            self.module.fail_json(msg=error)
    ...
    def apply(self):
        ...
        if cd_action == 'create' and any((self.parameters.get(attr) is not None for attr in ('from_name', 'from_ipspace'))):
            ...
            if split is None:
                self.module.fail_json(msg='A domain cannot be split if it does not exist.', exception=traceback.format_exc())
            if split:
                cd_action = None
