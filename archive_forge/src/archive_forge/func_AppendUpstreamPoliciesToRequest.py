...
def AppendUpstreamPoliciesToRequest(repo_ref, repo_args, request):
    ...
    if repo_args.upstream_policy_file:
        if isinstance(request, messages.ArtifactregistryProjectsLocationsRepositoriesPatchRequest):
            request.updateMask = None
        content = console_io.ReadFromFileOrStdin(repo_args.upstream_policy_file, binary=False)
        ...
        for policy in policies:
            if all((key in policy for key in ('id', 'priority', 'repository'))):
                p = messages.UpstreamPolicy(id=policy['id'], priority=policy['priority'], repository=policy['repository'])
