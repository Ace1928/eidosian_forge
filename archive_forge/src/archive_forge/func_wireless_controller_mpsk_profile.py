...
def wireless_controller_mpsk_profile(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('wireless-controller', 'mpsk-profile', filtered_data, vdom=vdom)
        current_data = fos.get('wireless-controller', 'mpsk-profile', vdom=vdom, mkey=mkey)
    ...
    if state == 'present' or state is True:
        return fos.set('wireless-controller', 'mpsk-profile', data=filtered_data, vdom=vdom)
    elif state == 'absent':
        return fos.delete('wireless-controller', 'mpsk-profile', mkey=filtered_data['name'], vdom=vdom)
    else:
