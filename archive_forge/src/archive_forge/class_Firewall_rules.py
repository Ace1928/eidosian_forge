...
class Firewall_rules(ConfigBase):
    ...
    def _state_replaced(self, want, have):
        ...
        if have:
            for h in have:
                ...
                for rs in r_sets:
                    wanted_rule_set = self.search_r_sets_in_have(want, rs['name'], 'r_list', h['afi'])
                    if wanted_rule_set is not None:
                        commands.extend(self._add_r_sets(h['afi'], want=rs, have=wanted_rule_set, opr=False))
    ...
    def _add_rules(self, afi, name, w_rules, h_rules, opr=True):
        ...
        """
        commands = []
        l_set = ('ipsec', 'action', 'number', 'protocol', 'fragment', 'disable', 'description', 'log')
        if w_rules:
            for w in w_rules:
    ...
    def _add_time(self, attr, w, h, cmd, opr):
        ...
        commands = []
        h_time = {}
        l_set = ('utc', 'stopdate', 'stoptime', 'weekdays', 'monthdays', 'startdate', 'starttime')
        if w[attr]:
            if h and attr in h.keys():
