...
class TestRetention(testcase.GsUtilIntegrationTestCase):
    ...
    def test_set_retention_years(self):
        ...
        self.VerifyRetentionPolicy(bucket_uri, expected_retention_period_in_seconds=_SECONDS_IN_YEAR)
    ...
    def test_set_retention_multiple_sequential(self):
        ...
        self.RunGsUtil(['retention', 'set', '1y', suri(bucket1_uri), suri(bucket2_uri)])
    ...
    def test_clear_unlocked_retention(self):
        ...
        self.VerifyRetentionPolicy(bucket_uri, expected_retention_period_in_seconds=None)
    ...
    def test_lock_retention_userConfirms(self):
        ...
        self.VerifyRetentionPolicy(bucket_uri, expected_retention_period_in_seconds=_SECONDS_IN_DAY, expected_is_locked=True)
    ...
    def test_lock_with_no_retention_policy_invalid(self):
        bucket_uri = self.CreateBucket()
        stderr = self.RunGsUtil(['retention', 'lock', suri(bucket_uri)], stdin='y', expected_status=1, return_stderr=True)
        self.assertRegex(stderr, 'does not have a(n Unlocked)? [Rr]etention [Pp]olicy')
        self.VerifyRetentionPolicy(bucket_uri, expected_retention_period_in_seconds=None)
    ...
    def test_get_retention_locked(self):
        bucket_uri = self.CreateBucketWithRetentionPolicy(retention_period_in_seconds=_SECONDS_IN_DAY, is_locked=True)
        stdout = self.RunGsUtil(['-DD', 'retention', 'get', suri(bucket_uri)], return_stdout=True)
        if self._use_gcloud_storage:
            self.assertRegex(stdout, 'isLocked\\: true')
            self.assertRegex(stdout, "retentionPeriod\\: \\'86400\\'")
            self.assertRegex(stdout, "effectiveTime\\: \\'.*\\'")
        else:
            self.assertRegex(stdout, 'Retention Policy \\(LOCKED\\):')
    ...
    def test_temporary_hold_bucket_with_no_retention(self):
        ...
        self._VerifyObjectHoldAndRetentionStatus(bucket_uri, object_uri, temporary_hold=None)
        self.RunGsUtil(['retention', 'temp', 'set', suri(object_uri)])
        self._VerifyObjectHoldAndRetentionStatus(bucket_uri, object_uri, temporary_hold=True)
        self.RunGsUtil(['retention', 'temp', 'release', suri(object_uri)])
        self._VerifyObjectHoldAndRetentionStatus(bucket_uri, object_uri, temporary_hold=False)
    ...
    def test_temporary_hold_bucket_with_retention(self):
        ...
        object_uri = self.CreateObject(bucket_uri=bucket_uri, contents='content')
        self._VerifyObjectHoldAndRetentionStatus(bucket_uri, object_uri, temporary_hold=None, retention_period=retention_period)
        self.RunGsUtil(['retention', 'temp', 'set', suri(object_uri)])
        self._VerifyObjectHoldAndRetentionStatus(bucket_uri, object_uri, temporary_hold=True, retention_period=retention_period)
        self.RunGsUtil(['retention', 'temp', 'release', suri(object_uri)])
        self._VerifyObjectHoldAndRetentionStatus(bucket_uri, object_uri, temporary_hold=False, retention_period=retention_period)
    ...
    def test_temporary_hold_multiple_parallel(self):
        ...
        object1_uri = self.CreateObject(bucket_uri=bucket_uri, contents='content')
        object2_uri = self.CreateObject(bucket_uri=bucket_uri, contents='content')
        self.RunGsUtil(['-m', 'retention', 'temp', 'set', suri(object1_uri), suri(object2_uri)])
        self._VerifyObjectHoldAndRetentionStatus(bucket_uri, object1_uri, temporary_hold=True)
        self._VerifyObjectHoldAndRetentionStatus(bucket_uri, object2_uri, temporary_hold=True)
        self.RunGsUtil(['-m', 'retention', 'temp', 'release', suri(object1_uri), suri(object2_uri)])
        self._VerifyObjectHoldAndRetentionStatus(bucket_uri, object1_uri, temporary_hold=False)
        self._VerifyObjectHoldAndRetentionStatus(bucket_uri, object2_uri, temporary_hold=False)
    ...
    def test_set_event_based_hold_invalid_arg(self):
        object_uri = self.CreateObject()
        stderr = self.RunGsUtil(['retention', 'event', 'rel', suri(object_uri)], expected_status=1, return_stderr=True)
        self.assertRegex(stderr, 'Invalid subcommand ".*" for the "retention event" command')
    ...
    def test_event_based_hold_bucket_with_retention(self):
        ...
        bucket_uri = self.CreateBucketWithRetentionPolicy(retention_period)
        object_uri = self.CreateObject(bucket_uri=bucket_uri, contents='content')
        self._VerifyObjectHoldAndRetentionStatus(bucket_uri, object_uri, event_based_hold=None, retention_period=retention_period)
        self.RunGsUtil(['retention', 'event', 'set', suri(object_uri)])
        self._VerifyObjectHoldAndRetentionStatus(bucket_uri, object_uri, event_based_hold=True, retention_period=None)
        self.RunGsUtil(['retention', 'event', 'release', suri(object_uri)])
        self._VerifyObjectHoldAndRetentionStatus(bucket_uri, object_uri, event_based_hold=False, retention_period=retention_period)
    ...
    def test_event_based_hold_multiple_sequential(self):
        ...
        object1_uri = self.CreateObject(bucket_uri=bucket_uri, contents='content')
        object2_uri = self.CreateObject(bucket_uri=bucket_uri, contents='content')
        self.RunGsUtil(['retention', 'event', 'set', suri(object1_uri), suri(object2_uri)])
        self._VerifyObjectHoldAndRetentionStatus(bucket_uri, object1_uri, event_based_hold=True)
        self._VerifyObjectHoldAndRetentionStatus(bucket_uri, object2_uri, event_based_hold=True)
        self.RunGsUtil(['retention', 'event', 'release', suri(object1_uri), suri(object2_uri)])
        self._VerifyObjectHoldAndRetentionStatus(bucket_uri, object1_uri, event_based_hold=False)
        self._VerifyObjectHoldAndRetentionStatus(bucket_uri, object2_uri, event_based_hold=False)
    ...
    def test_event_based_hold_multiple_parallel(self):
        ...
        object1_uri = self.CreateObject(bucket_uri=bucket_uri, contents='content')
        object2_uri = self.CreateObject(bucket_uri=bucket_uri, contents='content')
        self.RunGsUtil(['-m', 'retention', 'event', 'set', suri(object1_uri), suri(object2_uri)])
        self._VerifyObjectHoldAndRetentionStatus(bucket_uri, object1_uri, event_based_hold=True)
        self._VerifyObjectHoldAndRetentionStatus(bucket_uri, object2_uri, event_based_hold=True)
        self.RunGsUtil(['-m', 'retention', 'event', 'release', suri(object1_uri), suri(object2_uri)])
        self._VerifyObjectHoldAndRetentionStatus(bucket_uri, object1_uri, event_based_hold=False)
        self._VerifyObjectHoldAndRetentionStatus(bucket_uri, object2_uri, event_based_hold=False)
    ...
    def _VerifyDefaultEventBasedHold(self, bucket_uri, expected_default_event_based_hold=None):
        actual_default_event_based_hold = self.json_api.GetBucket(bucket_uri.bucket_name, fields=['defaultEventBasedHold']).defaultEventBasedHold
        if expected_default_event_based_hold is None:
            self.assertEqual(actual_default_event_based_hold, None)
        else:
            self.assertEqual(actual_default_event_based_hold, expected_default_event_based_hold)
