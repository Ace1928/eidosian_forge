...
def one_particle(matrix_elements, core=None, active=None, cutoff=1e-12):
    ...
    **Example**
    ...
    if matrix_elements.ndim != 2:
        raise ValueError(f"'matrix_elements' must be a 2D array; got matrix_elements.ndim = {matrix_elements.ndim}")
    if not core:
        t_core = 0
    else:
    ...
    pairs = [[indices[0][i], indices[1][i]] for i in range(num_indices) if all((indices[j][i] in active for j in range(len(indices))))]
