...
def verified_maximal_cusp_area_matrix(snappy_manifold, bits_prec=None):
    ...
    """
    hyperbolic, shapes = snappy_manifold.verify_hyperbolicity(bits_prec=bits_prec)
    if not hyperbolic:
        raise Exception('Could not compute shape intervals for: triangulation does not hyperbolic structure or precision is insufficient')
    C = CuspTilingEngine.from_manifold_and_shapes(snappy_manifold, shapes)
    rows = [C.compute_maximal_cusp_area_matrix_row(i) for i in range(C.num_cusps)]
