...
def test_quota_show__with_default(self):
    arglist = ['--default', self.projects[0].name]
    verifylist = [('default', True), ('project', self.projects[0].name)]
    parsed_args = self.check_parser(self.cmd, arglist, verifylist)
    self.cmd.take_action(parsed_args)
    self.compute_quotas_mock.defaults.assert_called_once_with(self.projects[0].id)
