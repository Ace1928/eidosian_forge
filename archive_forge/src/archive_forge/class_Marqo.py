...
class Marqo(VectorStore):
    ...
    def __init__(self, client: marqo.Client, index_name: str, add_documents_settings: Optional[Dict[str, Any]]=None, searchable_attributes: Optional[List[str]]=None, page_content_builder: Optional[Callable[[Dict[str, Any]], str]]=None):
    ...
    def add_texts(self, texts: Iterable[str], metadatas: Optional[List[dict]]=None, **kwargs: Any) -> List[str]:
        ...
        if self._client.index(self._index_name).get_settings()['index_defaults']['treat_urls_and_pointers_as_images']:
            raise ValueError('Marqo.add_texts is disabled for multimodal indexes. To add documents with a multimodal index use the Python client for Marqo directly.')
        ...
        for i in range(0, num_docs, self._document_batch_size):
            response = self._client.index(self._index_name).add_documents(documents[i:i + self._document_batch_size], tensor_fields=self.tensor_fields, **self._add_documents_settings)
            if response['errors']:
    ...
    def similarity_search(self, query: Union[str, Dict[str, float]], k: int=4, **kwargs: Any) -> List[Document]:
    ...
    def similarity_search_with_score(self, query: Union[str, Dict[str, float]], k: int=4) -> List[Tuple[Document, float]]:
    ...
    def _construct_documents_from_results_with_score(self, results: Dict[str, List[Dict[str, str]]]) -> List[Tuple[Document, Any]]:
        ...
        for res in results['hits']:
            ...
            documents.append((Document(page_content=text, metadata=metadata), res['_score']))
        return documents
    ...
    @classmethod
    def from_documents(cls: Type[Marqo], documents: List[Document], embedding: Union[Embeddings, None]=None, **kwargs: Any) -> Marqo:
    ...
    @classmethod
    def from_texts(cls, texts: List[str], embedding: Any=None, metadatas: Optional[List[dict]]=None, index_name: str='', url: str='http://localhost:8882', api_key: str='', add_documents_settings: Optional[Dict[str, Any]]=None, searchable_attributes: Optional[List[str]]=None, page_content_builder: Optional[Callable[[Dict[str, str]], str]]=None, index_settings: Optional[Dict[str, Any]]=None, verbose: bool=True, **kwargs: Any) -> Marqo:
        ...
        instance: Marqo = cls(client, index_name, searchable_attributes=searchable_attributes, add_documents_settings=add_documents_settings or {}, page_content_builder=page_content_builder)
