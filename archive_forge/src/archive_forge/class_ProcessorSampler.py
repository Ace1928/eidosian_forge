...
class ProcessorSampler(cirq.Sampler):
    ...
    def __init__(self, *, processor: 'cg.engine.AbstractProcessor', run_name: str='', device_config_name: str=''):
    ...
    async def run_sweep_async(self, program: 'cirq.AbstractCircuit', params: cirq.Sweepable, repetitions: int=1) -> Sequence['cg.EngineResult']:
        job = await self._processor.run_sweep_async(program=program, params=params, repetitions=repetitions, run_name=self._run_name, device_config_name=self._device_config_name)
        return await job.results_async()
    run_sweep = duet.sync(run_sweep_async)

    async def run_batch_async(self, programs: Sequence[cirq.AbstractCircuit], params_list: Optional[Sequence[cirq.Sweepable]]=None, repetitions: Union[int, Sequence[int]]=1) -> Sequence[Sequence['cg.EngineResult']]:
        return cast(Sequence[Sequence['cg.EngineResult']], await super().run_batch_async(programs, params_list, repetitions))
