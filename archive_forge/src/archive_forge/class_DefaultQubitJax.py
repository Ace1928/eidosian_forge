...
class DefaultQubitJax(DefaultQubitLegacy):
    ...
    * You must use the ``"jax"`` interface for classical backpropagation, as JAX is
    ...
    _tensordot = staticmethod(lambda a, b, axes: jnp.tensordot(a, b, axes if isinstance(axes, int) else list(map(tuple, axes))))
    ...
    def _evolve_state_vector_under_parametrized_evolution(self, state: TensorLike, operation: ParametrizedEvolution):
        ...
        with jax.ensure_compile_time_eval():
            H_jax = ParametrizedHamiltonianPytree.from_hamiltonian(operation.H, dense=operation.dense, wire_order=self.wires)
    ...
    def sample_basis_states(self, number_of_states, state_probability):
        ...
        if jnp.ndim(state_probability) == 2:
            ...
            for _ in state_probability:
                ...
                keys.append(subkey)
            return jnp.array([jax.random.choice(_key, number_of_states, shape=(shots,), p=prob) for _key, prob in zip(keys, state_probability)])
