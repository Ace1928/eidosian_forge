    ...
    def _CAST_IN(x):
        if isinstance(x, complex):
            ...
            try:
                ...
                raise TypeError('Unable to turn value into an R complex number.')
        return res

    def __getitem__(self, i: Union[int, slice]) -> Union[complex, 'ComplexSexpVector']:
        ...
        if isinstance(i, int):
            ...
            res = type(self).from_iterable([openrlib.COMPLEX_ELT(cdata, i_c) for i_c in range(*i.indices(len(self)))])
        else:
