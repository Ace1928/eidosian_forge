...
class GsUtilIntegrationTestCase(base.GsUtilTestCase):
    ...
    def setUp(self):
        ...
        self.json_api = GcsJsonApi(BucketStorageUri, logging.getLogger(), DiscardMessagesQueue(), 'gs')
        self.xml_api = BotoTranslation(BucketStorageUri, logging.getLogger(), DiscardMessagesQueue, self.default_provider)
        self.kms_api = KmsApi(logging.getLogger())
        self.multiregional_buckets = util.USE_MULTIREGIONAL_BUCKETS
        self._use_gcloud_storage = config.getbool('GSUtil', 'use_gcloud_storage', False)
                ...
                for k in bucket_list:
                    try:
                        if isinstance(k, DeleteMarker):
                            bucket_uri.get_bucket().delete_key(k.name, version_id=k.version_id)
                        else:
                            k.delete()
                    except StorageResponseError as e:
                        if e.status == 404:
                            error = e
                        elif e.status == 403 and (e.error_code == 'ObjectUnderActiveHold' or e.error_code == 'RetentionPolicyNotMet'):
                            self._ClearHoldsOnObjectAndWaitForRetentionDuration(bucket_uri, k.name)
                        else:
                            raise
                if error:
                    raise error
    ...
    """
        ...
        self.RunGsUtil(['setmeta', '-h', 'x-%s-meta-%s' % (provider_meta_string, ATIME_ATTR), '-h', 'x-%s-meta-%s' % (provider_meta_string, MTIME_ATTR), '-h', 'x-%s-meta-%s' % (provider_meta_string, UID_ATTR), '-h', 'x-%s-meta-%s' % (provider_meta_string, GID_ATTR), '-h', 'x-%s-meta-%s' % (provider_meta_string, MODE_ATTR), suri(obj)])

    def _ServiceAccountCredentialsPresent(self):
        return config.has_option('Credentials', 'gs_service_key_file') or config.has_option('GoogleCompute', 'service_account')
    ...
    """
        with SetBotoConfigForTest([('GSUtil', 'prefer_api', 'json')]):
            stdout = self.RunGsUtil(['stat', object_uri_str], return_stdout=True, force_gsutil=True)
        self.assertRegex(stdout, 'KMS key:\\s+%s' % encryption_key)
    ...
    def CreateBucket(self, bucket_name=None, test_objects=0, storage_class=None, retention_policy=None, provider=None, prefer_json_api=False, versioning_enabled=False, bucket_policy_only=False, bucket_name_prefix='', bucket_name_suffix='', location=None, public_access_prevention=None):
    ...
    """
        ...
        if location is None:
            if self.multiregional_buckets or provider == 's3':
                ...
                location = boto.config.get('GSUtil', 'test_cmd_regional_bucket_location', 'us-central1')
        ...
        if bucket_name:
            ...
            bucket_name = self.MakeTempName('bucket', prefix=bucket_name_prefix, suffix=bucket_name_suffix)
        if prefer_json_api and provider == 'gs':
            json_bucket = self.CreateBucketJson(bucket_name=bucket_name, test_objects=test_objects, storage_class=storage_class, location=location, versioning_enabled=versioning_enabled, retention_policy=retention_policy, bucket_policy_only=bucket_policy_only, public_access_prevention=public_access_prevention)
            bucket_uri = boto.storage_uri('gs://%s' % json_bucket.name.lower(), suppress_consec_slashes=False)
            return bucket_uri
        bucket_uri = boto.storage_uri('%s://%s' % (provider, bucket_name.lower()), suppress_consec_slashes=False)
        if provider == 'gs':
            headers = {'x-goog-api-version': self.api_version, GOOG_PROJ_ID_HDR: PopulateProjectId()}
        else:
            headers = {}
      ...
      """
        ...
        if provider != 'gs' and (not public_access_prevention):
            xml_body = '<?xml version="1.0" encoding="UTF-8"?><PublicAccessBlockConfiguration><BlockPublicAcls>False</BlockPublicAcls></PublicAccessBlockConfiguration>'
            bucket_uri.set_subresource('publicAccessBlock', xml_body)
        for i in range(test_objects):
            self.CreateObject(bucket_uri=bucket_uri, object_name=self.MakeTempName('obj'), contents='test {:d}'.format(i).encode('ascii'))
        return bucket_uri
    ...
    """
        bucket_uri = self.CreateBucket(bucket_name=bucket_name, test_objects=test_objects, prefer_json_api=True, versioning_enabled=True)
        return bucket_uri

    def CreateObject(self, bucket_uri=None, object_name=None, contents=None, prefer_json_api=False, encryption_key=None, mode=None, mtime=None, uid=None, gid=None, storage_class=None, gs_idempotent_generation=0, kms_key_name=None):
    ...
    """
        ...
        if contents is not None:
            if not isinstance(contents, (six.binary_type, six.text_type)):
                raise TypeError('contents must be either none or bytes, not {}'.format(type(contents)))
            contents = six.ensure_binary(contents)
        if contents and bucket_uri.scheme == 'gs' and (prefer_json_api or encryption_key or kms_key_name):
            object_name = object_name or self.MakeTempName('obj')
            json_object = self.CreateObjectJson(contents=contents, bucket_name=bucket_uri.bucket_name, object_name=object_name, encryption_key=encryption_key, mtime=mtime, storage_class=storage_class, gs_idempotent_generation=gs_idempotent_generation, kms_key_name=kms_key_name)
            object_uri = bucket_uri.clone_replace_name(object_name)
            md5 = (Base64ToHexHash(json_object.md5Hash), json_object.md5Hash.strip('\n"\''))
        ...
        if contents is not None:
            if bucket_uri.scheme == 'gs' and gs_idempotent_generation is not None:
                try:
                    key_uri.set_contents_from_string(contents, headers={'x-goog-if-generation-match': str(gs_idempotent_generation)})
                except StorageResponseError as e:
                    if e.status == 412:
                        pass
                    else:
        ...
        custom_metadata_present = mode is not None or mtime is not None or uid is not None or (gid is not None)
        if custom_metadata_present:
            self.SetPOSIXMetadata(bucket_uri.scheme, bucket_uri.bucket_name, object_name, atime=None, mtime=mtime, uid=uid, gid=gid, mode=mode)
        return key_uri
    ...
    """
        ...
        return gsutil_api.GetObjectMetadata(bucket_name, object_name, provider=self.default_provider, fields=fields)

    def VerifyObjectCustomAttribute(self, bucket_name, object_name, attr_name, expected_value, expected_present=True):
    ...
    Returns:
      None
    """
        metadata = self.GetObjectMetadataWithFields(bucket_name, object_name, fields=['metadata/%s' % attr_name])
        attr_present, value = GetValueFromObjectCustomMetadata(metadata, attr_name, default_value=expected_value)
    ...
    def VerifyPublicAccessPreventionValue(self, bucket_uri, value):
        ...
        public_access_prevention_re = re.compile(':\\s+(?P<pap_val>.+)$')
