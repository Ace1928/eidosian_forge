...
def _handle_err_sor_msg(self, afi, safi):
    if not self._protocol.is_enhanced_rr_cap_valid():
        LOG.error('Received Start-of-RIB (SOR) even though ERR is not enabled')
        return
    self.version_num += 1
    ...
    if rst != 0:
        ...
        if route_family in SUPPORTED_GLOBAL_RF:
            self._refresh_stalepath_timer = self._spawn_after('err-refresh-stale-path-timer', rst, self._core_service.table_manager.clean_stale_routes, self, route_family)
