...
class PaiEasEndpoint(LLM):
    ...
    def _default_params(self) -> Dict[str, Any]:
        ...
        return {'max_new_tokens': self.max_new_tokens, 'temperature': self.temperature, 'top_k': self.top_k, 'top_p': self.top_p, 'stop_sequences': []}

    @property
    def _identifying_params(self) -> Mapping[str, Any]:
        """Get the identifying parameters."""
        _model_kwargs = self.model_kwargs or {}
        return {'eas_service_url': self.eas_service_url, 'eas_service_token': self.eas_service_token, **_model_kwargs}
    ...
    def _stream(self, prompt: str, stop: Optional[List[str]]=None, run_manager: Optional[CallbackManagerForLLMRun]=None, **kwargs: Any) -> Iterator[GenerationChunk]:
        ...
        headers = {'User-Agent': 'Test Client', 'Authorization': f'{self.eas_service_token}'}
        if self.version == '1.0':
