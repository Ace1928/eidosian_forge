...
class StructuredPrompt(ChatPromptTemplate):
    ...
    def __or__(self, other: Union[Runnable[Any, Other], Callable[[Any], Other], Callable[[Iterator[Any]], Iterator[Other]], Mapping[str, Union[Runnable[Any, Other], Callable[[Any], Other], Any]]]) -> RunnableSerializable[Dict, Other]:
    ...
    def pipe(self, *others: Union[Runnable[Any, Other], Callable[[Any], Other]], name: Optional[str]=None) -> RunnableSerializable[Dict, Other]:
        if others and isinstance(others[0], BaseLanguageModel) or hasattr(others[0], 'with_structured_output'):
            return RunnableSequence(self, others[0].with_structured_output(self.schema_), *others[1:], name=name)
        else:
            raise NotImplementedError('Structured prompts need to be piped to a language model.')