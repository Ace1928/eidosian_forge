...
class NetworksWirelessSettings(object):
    ...
    def __init__(self, params, meraki):
        self.meraki = meraki
        self.new_object = dict(meshingEnabled=params.get('meshingEnabled'), ipv6BridgeEnabled=params.get('ipv6BridgeEnabled'), locationAnalyticsEnabled=params.get('locationAnalyticsEnabled'), upgradeStrategy=params.get('upgradeStrategy'), ledLightsOn=params.get('ledLightsOn'), network_id=params.get('networkId'))
    ...
    def update_all_params(self):
        ...
        if self.new_object.get('upgradeStrategy') is not None or self.new_object.get('upgrade_strategy') is not None:
            new_object_params['upgradeStrategy'] = self.new_object.get('upgradeStrategy') or self.new_object.get('upgrade_strategy')
    ...
    def get_object_by_name(self, name):
        result = None
        try:
            items = self.meraki.exec_meraki(family='wireless', function='getNetworkWirelessSettings', params=self.get_all_params(name=name))
    ...
    def requires_update(self, current_obj):
        ...
        obj_params = [('meshingEnabled', 'meshingEnabled'), ('ipv6BridgeEnabled', 'ipv6BridgeEnabled'), ('locationAnalyticsEnabled', 'locationAnalyticsEnabled'), ('upgradeStrategy', 'upgradeStrategy'), ('ledLightsOn', 'ledLightsOn'), ('networkId', 'networkId')]
    ...
    def update(self):
        ...
        name = self.new_object.get('name')
        result = None
        result = self.meraki.exec_meraki(family='wireless', function='updateNetworkWirelessSettings', params=self.update_all_params(), op_modifies=True)
        return result