...
class NetworkUtilsTestCase(test_base.OsWinBaseTestCase):
    ...
    _autospec_classes = [networkutils.jobutils.JobUtils]
    ...
    _FAKE_INSTANCE_ID = 'Microsoft:609CBAAD-BC13-4A65-AADE-AD95861FE394\\\\55349F56-72AB-4FA3-B5FE-6A30A511A419\\C\\776E0BA7-94A1-41C8-8F28-951F524251B5\\77A43184-5444-49BF-ABE0-2210B72ABA73'
    ...
    def test_get_vnic_mac_address(self, mock_get_vnic_settings):
        ...
        actual_mac_address = self.netutils.get_vnic_mac_address(mock.sentinel.switch_port_name)
    ...
    def test_get_vlan_setting_data_from_port_alloc(self, mock_get_sd):
        mock_port = mock.MagicMock()
        result = self.netutils._get_vlan_setting_data_from_port_alloc(mock_port)
        self.assertEqual(mock_get_sd.return_value, result)
        mock_get_sd.assert_called_once_with(mock_port, self.netutils._vsid_sds, self.netutils._PORT_VLAN_SET_DATA)
    ...
    def test_get_hw_offload_sd_from_port_alloc(self, mock_get_sd):
        ...
        self.assertEqual(mock_get_sd.return_value, result)
        mock_get_sd.assert_called_once_with(mock_port, self.netutils._hw_offload_sds, self.netutils._PORT_HW_OFFLOAD_SET_DATA)

    @mock.patch.object(networkutils.NetworkUtils, '_get_setting_data_from_port_alloc')
    def test_get_bandwidth_setting_data_from_port_alloc(self, mock_get_sd):
        mock_port = mock.MagicMock()
        result = self.netutils._get_bandwidth_setting_data_from_port_alloc(mock_port)
        self.assertEqual(mock_get_sd.return_value, result)
        mock_get_sd.assert_called_once_with(mock_port, self.netutils._bandwidth_sds, self.netutils._PORT_BANDWIDTH_SET_DATA)
    ...
    def _setup_security_rule_test(self, mock_get_elem_assoc_cls):
        ...
        self.netutils._filter_security_acls = mock.MagicMock(return_value=[mock_acl])
    ...
    def test_remove_port_qos_rule(self, mock_get_bandwidth_sd):
        ...
        mock_get_bandwidth_sd.assert_called_once_with(mock_port_alloc)
        mock_remove_feature = self.netutils._jobutils.remove_virt_feature
        mock_remove_feature.assert_called_once_with(mock_bandwidth_settings)
