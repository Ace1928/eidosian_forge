...
def test_db2(testdir, name):
    ...
    if 'db' in name:
        kvp = {'external_tables': {'blabla': {'a': 1, 'b': 2, 'c': 3}, 'lala': {'a': 0.01, 'b': 0.02, 'c': 0.0}}}
    else:
        kvp = {'a': 1}
    ...
    with pytest.raises(ValueError):
        c.write(Atoms(), fmax=0.0)
    with pytest.raises(ValueError):
        c.write(Atoms(), S=42)
    id = c.write(Atoms(), b=np.bool_(True), i=np.int64(42), n=np.nan, x=np.inf, s='NaN2', A=42)
    row = c[id]
