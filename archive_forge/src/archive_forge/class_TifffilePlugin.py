...
class TifffilePlugin(PluginV3):
    ...
    def write(self, ndimage: ArrayLike, *, is_batch: bool=False, **kwargs) -> Optional[bytes]:
    ...
    def metadata(self, *, index: int=Ellipsis, page: int=None, exclude_applied: bool=True) -> Dict[str, Any]:
        ...
        if target is None:
            ...
            metadata.update({'planar_configuration': target.planarconfig, 'compression': target.compression, 'predictor': target.predictor, 'orientation': None, 'description1': target.description1, 'description': target.description, 'software': target.software, **_get_resolution(target), 'datetime': target.datetime})
        return metadata
    ...
    def properties(self, *, index: int=None, page: int=None) -> ImageProperties:
        ...
        if index is Ellipsis and page is None:
            ...
            props = ImageProperties(shape=(n_series, *target_page.shape), dtype=target_page.dtype, n_images=n_series, is_batch=True, spacing=_get_resolution(target_page).get('resolution'))
        elif index is Ellipsis and page is Ellipsis:
            n_pages = len(self._fh.pages)
            props = ImageProperties(shape=(n_pages, *target_page.shape), dtype=target_page.dtype, n_images=n_pages, is_batch=True, spacing=_get_resolution(target_page).get('resolution'))
        else:
            props = ImageProperties(shape=target_page.shape, dtype=target_page.dtype, is_batch=False, spacing=_get_resolution(target_page).get('resolution'))
        return props
