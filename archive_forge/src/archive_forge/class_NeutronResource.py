...
class NeutronResource(resource.Resource):
    ...
    def _store_config_default_properties(self, attrs):
        ...
        if 'port_security_enabled' in attrs:
            self.data_set('port_security_enabled', attrs['port_security_enabled'])
    ...
    def check_delete_complete(self, check):
        ...
        if not self._not_found_in_call(self._show_resource):
            raise exception.PhysicalResourceExists(name=self.physical_resource_name_or_FnGetRefId())
        return True
    ...
    def set_tags(self, tags):
        ...
        if resource_plural:
            tags = tags or []
            body = {'tags': tags}
            self.client().replace_tag(resource_plural, self.resource_id, body)

    def parse_live_resource_data(self, resource_properties, resource_data):
        result = super(NeutronResource, self).parse_live_resource_data(resource_properties, resource_data)
