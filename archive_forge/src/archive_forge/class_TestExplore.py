...
class TestExplore:
    ...
    def test_categories(self):
        m = self.nybb[['BoroName', 'geometry']].explore(column='BoroName', categories=['Brooklyn', 'Staten Island', 'Queens', 'Bronx', 'Manhattan'])
        out_str = self._fetch_map_string(m)
    ...
    def test_highlight(self):
        ...
        m = self.nybb.explore(highlight=True, highlight_kwds={'fillOpacity': 1, 'color': 'red'})
        out_str = self._fetch_map_string(m)
    ...
    def test_map_kwds(self):
        ...
        m = self.world.explore(zoom_control=False, map_kwds={'dragging': False, 'scrollWheelZoom': False})
        check()
        with pytest.raises(ValueError, match="'zoom_control' cannot be specified in 'map_kwds'"):
            self.world.explore(map_kwds={'dragging': False, 'scrollWheelZoom': False, 'zoom_control': False})
