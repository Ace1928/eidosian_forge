...
class AbstractLocalProcessor(AbstractProcessor):
    ...
    def __init__(self, *, processor_id: str, engine: Optional['AbstractEngine']=None, expected_down_time: Optional[datetime.datetime]=None, expected_recovery_time: Optional[datetime.datetime]=None, schedule: Optional[List[quantum.QuantumTimeSlot]]=None, project_name: str='fake_project'):
        self._engine = engine
        ...
        if schedule is None:
            self._schedule = [quantum.QuantumTimeSlot(processor_name=self._processor_id, time_slot_type=quantum.QuantumTimeSlot.TimeSlotType.UNALLOCATED)]
        else:
            self._schedule = sorted(schedule, key=lambda t: t.start_time.timestamp() if t.start_time else -1)
    ...
    def create_reservation(self, start_time: datetime.datetime, end_time: datetime.datetime, whitelisted_users: Optional[List[str]]=None) -> quantum.QuantumReservation:
        ...
        new_reservation = quantum.QuantumReservation(name=reservation_id, start_time=Timestamp(seconds=int(start_time.timestamp())), end_time=Timestamp(seconds=int(end_time.timestamp())), whitelisted_users=whitelisted_users)
    ...
    def update_reservation(self, reservation_id: str, start_time: Optional[datetime.datetime]=None, end_time: Optional[datetime.datetime]=None, whitelisted_users: Optional[List[str]]=None) -> None:
        ...
        if start_time is not None:
            self._reservations[reservation_id].start_time = datetime.datetime.fromtimestamp(_to_timestamp(start_time))
        if end_time is not None:
            self._reservations[reservation_id].end_time = datetime.datetime.fromtimestamp(_to_timestamp(end_time))
    ...
    def list_reservations(self, from_time: Union[None, datetime.datetime, datetime.timedelta]=datetime.timedelta(), to_time: Union[None, datetime.datetime, datetime.timedelta]=datetime.timedelta(weeks=2)) -> List[quantum.QuantumReservation]:
