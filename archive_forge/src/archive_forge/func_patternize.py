...
def patternize(pattern, inventory, storage_pool=None):
    ...
    if inventory:
        inventory_tokens = re.findall('\\[[a-zA-Z0-9_]*\\]', pattern)
        for token in inventory_tokens:
            pattern = pattern.replace(token, str(inventory[token[1:-1]]))
    tokens = re.findall('\\[[0-9]-[0-9]\\]|\\[[a-z]-[a-z]\\]|\\[[A-Z]-[A-Z]\\]', pattern)
    segments = '%s'.join(re.split('\\[[0-9]-[0-9]\\]|\\[[a-z]-[a-z]\\]|\\[[A-Z]-[A-Z]\\]', pattern))
    if len(tokens) == 0:
        return [pattern]
    combinations = []
    for token in tokens:
