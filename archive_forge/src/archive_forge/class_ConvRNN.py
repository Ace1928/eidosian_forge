...
class ConvRNN(RNN):
    ...
    def __init__(self, rank, cell, return_sequences=False, return_state=False, go_backwards=False, stateful=False, unroll=False, **kwargs):
        if unroll:
            raise TypeError(f'Unrolling is not possible with convolutional RNNs. Received: unroll={unroll}')
        if isinstance(cell, (list, tuple)):
            raise TypeError(f'It is not possible at the moment tostack convolutional cells. Only pass a single cell instance as the `cell` argument. Received: cell={cell}')
    ...
    def compute_output_shape(self, input_shape):
        ...
        norm_img_dims = tuple([conv_utils.conv_output_length(img_dims[idx], cell.kernel_size[idx], padding=cell.padding, stride=cell.strides[idx], dilation=cell.dilation_rate[idx]) for idx in range(len(img_dims))])
        if cell.data_format == 'channels_first':
        ...
        if self.return_state:
            ...
            if cell.data_format == 'channels_first':
                output_shape += [(input_shape[0], cell.filters) + norm_img_dims for _ in range(2)]
            elif cell.data_format == 'channels_last':
                output_shape += [(input_shape[0],) + norm_img_dims + (cell.filters,) for _ in range(2)]
        return output_shape
    ...
    def build(self, input_shape):
        ...
        self.input_spec[0] = InputSpec(shape=(batch_size, None) + input_shape[2:self.rank + 3])
        ...
        if self.state_spec is not None:
            ...
            if [spec.shape[ch_dim] for spec in self.state_spec] != state_size:
                raise ValueError(f'An `initial_state` was passed that is not compatible with `cell.state_size`. Received state shapes {[spec.shape for spec in self.state_spec]}. However `cell.state_size` is {self.cell.state_size}')
        else:
            img_dims = tuple((None for _ in range(self.rank)))
            if self.cell.data_format == 'channels_first':
                self.state_spec = [InputSpec(shape=(None, dim) + img_dims) for dim in state_size]
            elif self.cell.data_format == 'channels_last':
                self.state_spec = [InputSpec(shape=(None,) + img_dims + (dim,)) for dim in state_size]
        if self.stateful:
            self.reset_states()
        self.built = True
    ...
    def get_initial_state(self, inputs):
        ...
        initial_state = self.cell.input_conv(initial_state, tf.zeros(tuple(shape), initial_state.dtype), padding=self.cell.padding)
        if hasattr(self.cell.state_size, '__len__'):
    ...
    def call(self, inputs, mask=None, training=None, initial_state=None, constants=None):
        ...
        if constants:
            if not generic_utils.has_arg(self.cell.call, 'constants'):
                raise ValueError(f'RNN cell {self.cell} does not support constants. Received: constants={constants}')
        ...
        last_output, outputs, states = backend.rnn(step, inputs, initial_state, constants=constants, go_backwards=self.go_backwards, mask=mask, input_length=timesteps, return_all_outputs=self.return_sequences)
        if self.stateful:
            updates = [backend.update(self_state, state) for self_state, state in zip(self.states, states)]
    ...
    def reset_states(self, states=None):
        ...
        def get_tuple_shape(nb_channels):
            ...
            if self.cell.data_format == 'channels_first':
                ...
                raise KeyError(f'Cell data format must be one of {{"channels_first", "channels_last"}}. Received: cell.data_format={self.cell.data_format}')
            return tuple(result)
        if self.states[0] is None:
            if hasattr(self.cell.state_size, '__len__'):
                self.states = [backend.zeros(get_tuple_shape(dim)) for dim in self.cell.state_size]
            else:
            ...
            if hasattr(self.cell.state_size, '__len__'):
                ...
                backend.set_value(self.states[0], np.zeros(get_tuple_shape(self.cell.state_size)))
        else:
            if not isinstance(states, (list, tuple)):
                states = [states]
            if len(states) != len(self.states):
                raise ValueError(f'Layer {self.name} expects {len(self.states)} states, but it received {len(states)} state values. States received: {states}')
            for index, (value, state) in enumerate(zip(states, self.states)):
                ...
                if value.shape != get_tuple_shape(dim):
                    raise ValueError(f'State {{index}} is incompatible with layer {self.name}: expected shape={get_tuple_shape(dim)}, found shape={value.shape}')
