...
class KmsApi(object):
    ...
    """
        ...
        self.host_base = config.get('Credentials', 'gs_kms_host', 'cloudkms.googleapis.com')
        ...
        self.api_client = apitools_client.CloudkmsV1(url=self.url_base, http=self.http, log_request=log_request, log_response=log_response, credentials=self.credentials)
        ...
        if isinstance(e, apitools_exceptions.HttpError):
            ...
            if e.status_code == 400:
                ...
                if 'The account for the specified project has been disabled' in str(e):
                    ...
                    return AccessDeniedException('Access Not Configured. Please go to the Google Cloud Platform Console (https://cloud.google.com/console#/project) for your project, select APIs & services, and enable the Google Cloud KMS API.', status=e.status_code)
                elif 'insufficient_scope' in str(e):
                    return AccessDeniedException(_INSUFFICIENT_OAUTH2_SCOPE_MESSAGE, status=e.status_code, body=self._GetAcceptableScopesFromHttpError(e))
                else:
                    return AccessDeniedException(message or e.message or key_name, status=e.status_code)
            elif e.status_code == 404:
                return NotFoundException(message or e.message, status=e.status_code)
            elif e.status_code == 409 and key_name:
                return ServiceException('The key %s already exists.' % key_name, status=e.status_code)
