...
class DataConfig:
    ...
    def configure(self, datasets: Dict[str, Dataset], world_size: int, worker_handles: Optional[List[ActorHandle]], worker_node_ids: Optional[List[NodeIdStr]], **kwargs) -> List[Dict[str, DataIterator]]:
        ...
        locality_hints = worker_node_ids if self._execution_options.locality_with_output else None
        for name, ds in datasets.items():
            ds = ds.copy(ds)
            ds.context.execution_options = copy.deepcopy(self._execution_options)
            ds.context.execution_options.exclude_resources = ds.context.execution_options.exclude_resources.add(ExecutionResources(cpu=self._num_train_cpus, gpu=self._num_train_gpus))
            if name in datasets_to_split:
                for i, split in enumerate(ds.streaming_split(world_size, equal=True, locality_hints=locality_hints)):
                    output[i][name] = split
            else:
