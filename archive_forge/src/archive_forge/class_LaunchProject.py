...
class LaunchProject:
    ...
    def __init__(self, uri: Optional[str], job: Optional[str], api: Api, launch_spec: Dict[str, Any], target_entity: str, target_project: str, name: Optional[str], docker_config: Dict[str, Any], git_info: Dict[str, str], overrides: Dict[str, Any], resource: str, resource_args: Dict[str, Any], run_id: Optional[str], sweep_id: Optional[str]=None):
        ...
        self.accelerator_base_image: Optional[str] = resource_args_build.get('accelerator', {}).get('base_image') or resource_args_build.get('cuda', {}).get('base_image')
        self._base_image: Optional[str] = launch_spec.get('base_image')
        self.docker_image: Optional[str] = docker_config.get('docker_image') or launch_spec.get('image_uri')
        ...
        self._entry_point: Optional[EntryPoint] = None
        override_entrypoint = overrides.get('entry_point')
        if override_entrypoint:
            _logger.info('Adding override entry point')
            self.override_entrypoint = EntryPoint(name=_get_entrypoint_file(override_entrypoint), command=override_entrypoint)
        ...
        if self.docker_image is not None:
            ...
            wandb.termlog(f'{LOG_PREFIX}Launch received placeholder URI, replacing with local path.')
            self.uri = os.getcwd()
            ...
            if self.uri is not None and (not os.path.exists(self.uri)):
                raise LaunchError('Assumed URI supplied is a local path but path is not valid')
            self.source = LaunchSource.LOCAL
            self.project_dir = self.uri
