import sys
def coneqp(P, q, G=None, h=None, dims=None, A=None, b=None, initvals=None, kktsolver=None, xnewcopy=None, xdot=None, xaxpy=None, xscal=None, ynewcopy=None, ydot=None, yaxpy=None, yscal=None, **kwargs):
    """
    ...
    if (not matrixP or (not matrixG and G is not None) or (not matrixA and A is not None)) and (not customkkt):
        raise ValueError('use of function valued P, G, A requires a user-provided kktsolver')
    ...
    if customx and (matrixP or matrixG or matrixA or (not customkkt)):
        raise ValueError('use of non-vector type for x requires function valued P, G, A and user-provided kktsolver')
    ...
    if matrixP:
        if P.typecode != 'd' or P.size != (q.size[0], q.size[0]):
            raise TypeError("'P' must be a 'd' matrix of size (%d, %d)" % (q.size[0], q.size[0]))
    ...
    if matrixG:
        if G.typecode != 'd' or G.size != (cdim, q.size[0]):
            raise TypeError("'G' must be a 'd' matrix of size (%d, %d)" % (cdim, q.size[0]))
    ...
    if matrixA:
        if A.typecode != 'd' or A.size[1] != q.size[0]:
            raise TypeError("'A' must be a 'd' matrix with %d columns" % q.size[0])
    ...
    if cdim == 0:
        ...
        if pcost == 0.0:
            ...
            relgap = 0.0
        return {'status': 'optimal', 'x': x, 'y': y, 'z': matrix(0.0, (0, 1)), 's': matrix(0.0, (0, 1)), 'gap': 0.0, 'relative gap': 0.0, 'primal objective': pcost, 'dual objective': pcost, 'primal slack': 0.0, 'dual slack': 0.0, 'primal infeasibility': pres, 'dual infeasibility': dres, 'iterations': 0}
    ...
    for iters in range(MAXITERS + 1):
        ...
        if pres <= FEASTOL and dres <= FEASTOL and (gap <= ABSTOL or (relgap is not None and relgap <= RELTOL)) or iters == MAXITERS:
            ...
            if iters == MAXITERS:
                ...
                status = 'optimal'
            return {'x': x, 'y': y, 's': s, 'z': z, 'status': status, 'gap': gap, 'relative gap': relgap, 'primal objective': pcost, 'dual objective': dcost, 'primal infeasibility': pres, 'dual infeasibility': dres, 'primal slack': -ts, 'dual slack': -tz, 'iterations': iters}
        if iters == 0:
        ...
        try:
            ...
            if iters == 0:
                ...
                return {'x': x, 'y': y, 's': s, 'z': z, 'status': 'unknown', 'gap': gap, 'relative gap': relgap, 'primal objective': pcost, 'dual objective': dcost, 'primal infeasibility': pres, 'dual infeasibility': dres, 'primal slack': -ts, 'dual slack': -tz, 'iterations': iters}
        ...
        for i in [0, 1]:
            ...
            blas.axpy(lmbdasq, ds, n=dims['l'] + sum(dims['q']), alpha=-1.0)
            ds[:dims['l']] += sigma * mu
            ind = dims['l']
            for m in dims['q']:
                ds[ind] += sigma * mu
            ...
            if t == 0:
                ...
                step = min(1.0, STEP / t)
            if i == 0:
                sigma = min(1.0, max(0.0, 1.0 - step + dsdz / gap * step ** 2)) ** EXPON
                eta = 0.0
