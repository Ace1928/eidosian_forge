...
class DomainConfigsTests(utils.ClientTestCase, utils.CrudTests):
    ...
    def new_ref(self, **kwargs):
        config_groups = {'identity': {uuid.uuid4().hex: uuid.uuid4().hex}, 'ldap': {uuid.uuid4().hex: uuid.uuid4().hex}}
    ...
    def test_create(self):
        ...
        config = self.new_ref()
        self.stub_url('PUT', parts=['domains', domain_id, 'config'], json=config, status_code=201)
    ...
    def test_update(self):
        domain_id = uuid.uuid4().hex
        config = self.new_ref()
        self.stub_url('PATCH', parts=['domains', domain_id, 'config'], json=config, status_code=200)
    ...
    def test_get(self):
        ...
        self.stub_entity('GET', parts=['domains', domain_id, 'config'], entity=config)
        res = self.manager.get(domain_id)
        self._assert_resource_attributes(res, config)

    def test_delete(self):
        domain_id = uuid.uuid4().hex
        self.stub_url('DELETE', parts=['domains', domain_id, 'config'], status_code=204)
