...
def test_reorder_diagonal_submatrix():
    ...
    given[:, :, :] = np.array([[11, 0, 0], [0, 22, 0], [0, 0, 33]])[:, :, np.newaxis]
    desired = given.copy()
    given[:, :, 0] = np.array([[22, 0, 0], [0, 33, 0], [0, 0, 0]])
    desired[0, :, 0] = 0
    desired[:, 0, 0] = 0
    ...
    desired[2, :, 2] = 0
    desired[:, 2, 2] = 0
    given[:, :, 3] = np.array([[11, 0, 0], [0, 33, 0], [0, 0, 0]])
    desired[1, :, 3] = 0
    desired[:, 1, 3] = 0
    given[:, :, 4] = np.array([[11, 0, 0], [0, 22, 0], [0, 0, 0]])
    desired[2, :, 4] = 0
    desired[:, 2, 4] = 0
