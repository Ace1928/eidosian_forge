...
def fill_placeholders(obj, placeholder_values: Dict[Union[FormalArgument, Parameter], Any]):
    ...
    try:
        if obj is None or isinstance(obj, (int, float, complex, Qubit, MemoryReference)):
            ...
            specs = {Gate: ['params', 'qubits'], Measurement: ['qubit', 'classical_reg'], ResetQubit: ['qubit'], Pulse: ['frame', 'waveform'], SetFrequency: ['frame', 'freq'], ShiftFrequency: ['frame', 'freq'], SetPhase: ['frame', 'phase'], ShiftPhase: ['frame', 'phase'], SwapPhases: ['frameA', 'frameB'], SetScale: ['frame', 'scale'], Capture: ['frame', 'kernel', 'memory_region'], RawCapture: ['frame', 'duration', 'memory_region'], DelayQubits: ['qubits', 'duration'], DelayFrames: ['frames', 'duration'], Fence: ['qubits'], FenceAll: [], Declare: [], Pragma: []}
