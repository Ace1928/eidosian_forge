...
class SingleStoreDBChatMessageHistory(BaseChatMessageHistory):
    ...
    def __init__(self, session_id: str, *, table_name: str='message_store', id_field: str='id', session_id_field: str='session_id', message_field: str='message', pool_size: int=5, max_overflow: int=10, timeout: float=30, **kwargs: Any):
        ...
        try:
            ...
            raise ImportError('Could not import sqlalchemy.pool python package. Please install it with `pip install singlestoredb`.')
    ...
    def _create_table_if_not_exists(self) -> None:
        ...
        try:
            cur = conn.cursor()
            try:
                cur.execute('CREATE TABLE IF NOT EXISTS {}\n                    ({} BIGINT PRIMARY KEY AUTO_INCREMENT,\n                    {} TEXT NOT NULL,\n                    {} JSON NOT NULL);'.format(self.table_name, self.id_field, self.session_id_field, self.message_field))
    ...
    def messages(self) -> List[BaseMessage]:
        ...
        try:
            cur = conn.cursor()
            try:
                cur.execute('SELECT {} FROM {} WHERE {} = %s'.format(self.message_field, self.table_name, self.session_id_field), self.session_id)
    ...
    def add_message(self, message: BaseMessage) -> None:
        ...
        try:
            cur = conn.cursor()
            try:
                cur.execute('INSERT INTO {} ({}, {}) VALUES (%s, %s)'.format(self.table_name, self.session_id_field, self.message_field), (self.session_id, json.dumps(message_to_dict(message))))
            finally:
                cur.close()
        finally:
            conn.close()
    ...
    def clear(self) -> None:
        ...
        try:
            cur = conn.cursor()
            try:
                cur.execute('DELETE FROM {} WHERE {} = %s'.format(self.table_name, self.session_id_field), self.session_id)
            finally:
                cur.close()
        finally:
            conn.close()