...
class WizardOfWikipediaAgent(ParlaiGenerationAgent):
    ...
    def __init__(self, name, suffix, device, maxlen, model):
        super().__init__(name=name, suffix=suffix, device=device, maxlen=maxlen, model=model)
    ...
    def predict(self, text, method='beam', num_beams=5, top_k=None, top_p=None, no_repeat_ngram_size=4, length_penalty=0.65) -> Dict[str, str]:
        if not self.chosen_topic:
            raise Exception("topic isn't selected. please call `set_topic(topic: str)` to select topic for wizard of wikipedia task")
        return super().predict(text=text, method=method, num_beams=num_beams, top_k=top_k, top_p=top_p, no_repeat_ngram_size=no_repeat_ngram_size, length_penalty=length_penalty)