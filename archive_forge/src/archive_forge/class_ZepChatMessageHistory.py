...
class ZepChatMessageHistory(BaseChatMessageHistory):
    ...
    def __init__(self, session_id: str, url: str='http://localhost:8000', api_key: Optional[str]=None) -> None:
        try:
    ...
    def messages(self) -> List[BaseMessage]:
        ...
        if zep_memory.messages:
            ...
            for msg in zep_memory.messages:
                metadata: Dict = {'uuid': msg.uuid, 'created_at': msg.created_at, 'token_count': msg.token_count, 'metadata': msg.metadata}
                if msg.role == 'ai':
    ...
    def search(self, query: str, metadata: Optional[Dict]=None, search_scope: SearchScope=SearchScope.messages, search_type: SearchType=SearchType.similarity, mmr_lambda: Optional[float]=None, limit: Optional[int]=None) -> List[MemorySearchResult]:
        """Search Zep memory for messages matching the query"""
        from zep_python import MemorySearchPayload
        payload = MemorySearchPayload(text=query, metadata=metadata, search_scope=search_scope, search_type=search_type, mmr_lambda=mmr_lambda)
