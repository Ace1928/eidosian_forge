...
class KamateraNodeDriverTests(LibcloudTestCase):
    ...
    def setUp(self):
        ...
        self.eu_node_location = NodeLocation(id='EU', name='Amsterdam', country='The Netherlands', driver=self.driver)
        self.il_node_location = NodeLocation(id='IL', name='Rosh Haayin', country='Israel', driver=self.driver)
        self.centos_8_EU_node_image = NodeImage(id='EU:6000C2987c9641fd2619a149ba2ca01a', name='CentOS 8.0 64-bit - Minimal Configuration', driver=self.driver, extra={'datacenter': 'EU', 'os': 'CentOS', 'code': '8.0 64bit_minimal', 'osDiskSizeGB': 5, 'ramMBMin': {'A': 256, 'B': 256, 'T': 256, 'D': 256}})
        self.small_node_size = self.driver.ex_get_size(ramMB=4096, diskSizeGB=30, cpuType='B', cpuCores=2, monthlyTrafficPackage='t5000', id='small', name='small')
    ...
    def test_ex_list_capabilities(self):
        ...
        self.assertEqual({'cpuTypes', 'defaultMonthlyTrafficPackage', 'diskSizeGB', 'monthlyTrafficPackage'}, set(capabilities.keys()))
        self.assertTrue(len(capabilities['cpuTypes']), 4)
        self.assertEqual({'id', 'description', 'name', 'ramMB', 'cpuCores'}, set(capabilities['cpuTypes'][0]))

    def test_create_node(self):
        node = self.driver.create_node(name='test_server', size=self.small_node_size, image=self.centos_8_EU_node_image, location=self.eu_node_location)
    ...
    def test_create_node_with_ssh_keys(self):
        node = self.driver.create_node(name='test_server_pubkey', size=self.small_node_size, image=self.centos_8_EU_node_image, location=self.eu_node_location, auth=NodeAuthSSHKey('publickey'))
        self.assertTrue(len(node.id) > 8)
        self.assertEqual(node.name, 'my-server')
        self.assertEqual(node.state, NodeState.RUNNING)
        self.assertTrue(len(node.public_ips) > 0)
    ...
    def assert_object(self, expected_object, objects):
        ...
        self.assertTrue(same_data, 'Objects does not match ({}, {})'.format(expected_object, objects[:2]))
