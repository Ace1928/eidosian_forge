...
class _MPResterLegacy:
    ...
    def __init__(self, api_key: str | None=None, endpoint: str | None=None, notify_db_version: bool=True, include_user_agent: bool=True) -> None:
        """
        Args:
        ...
        """
        warnings.warn('You are using the legacy MPRester. This version of the MPRester will no longer be updated. To access the latest data with the new MPRester, obtain a new API key from https://materialsproject.org/api and consult the docs at https://docs.materialsproject.org/ for more information.')
    ...
    def _make_request(self, sub_url: str, payload: Any=None, method: Literal['GET', 'POST', 'PUT', 'DELETE']='GET', mp_decode: bool=True) -> Any:
        response = None
    ...
    def get_entries(self, chemsys_formula_id_criteria: str | dict[str, Any], compatible_only: bool=True, inc_structure: bool | Literal['initial'] | None=None, property_data: list[str] | None=None, conventional_unit_cell: bool=False, sort_by_e_above_hull: bool=False) -> list[ComputedEntry]:
        ...
        """
        params = ['run_type', 'is_hubbard', 'pseudo_potential', 'hubbards', 'potcar_symbols', 'oxide_type']
        ...
        for d in data:
            d['potcar_symbols'] = [f'{d['pseudo_potential']['functional']} {label}' for label in d['pseudo_potential']['labels']]
            data = {'oxide_type': d['oxide_type']}
            if property_data:
                data.update({k: d[k] for k in property_data})
            if not inc_structure:
                e = ComputedEntry(d['unit_cell_formula'], d['energy'], parameters={k: d[k] for k in params}, data=data, entry_id=d['task_id'])
            else:
                ...
                if conventional_unit_cell:
                    ...
                    struct = prim.copy()
                    energy = d['energy']
                e = ComputedStructureEntry(struct, energy, parameters={k: d[k] for k in params}, data=data, entry_id=d['task_id'])
            entries.append(e)
        if compatible_only:
    ...
    def get_structure_by_material_id(self, material_id: str, final: bool=True, conventional_unit_cell: bool=False) -> Structure:
        ...
        if not data:
            try:
                ...
                if new_material_id:
                    warnings.warn(f'The calculation task {material_id} is mapped to canonical mp-id {new_material_id}, so structure for {new_material_id} returned. This is not an error, see documentation. If original task data for {material_id} is required, use get_task_data(). To find the canonical mp-id from a task id use get_materials_id_from_task_id().')
                return self.get_structure_by_material_id(new_material_id)
            except MPRestError:
                raise MPRestError(f'material_id={material_id!r} unknown, if this seems like an error please let us know at matsci.org/materials-project')
        structure = data[0][prop]
        if conventional_unit_cell:
            structure = SpacegroupAnalyzer(structure).get_conventional_standard_structure()
        return structure

    def get_entry_by_material_id(self, material_id: str, compatible_only: bool=True, inc_structure: bool | Literal['initial'] | None=None, property_data: list[str] | None=None, conventional_unit_cell: bool=False) -> ComputedEntry | ComputedStructureEntry:
        ...
        if len(data) == 0:
            raise MPRestError(f'material_id = {material_id!r} does not exist')
        return data[0]
    ...
    def get_entries_in_chemsys(self, elements, compatible_only=True, inc_structure=None, property_data=None, conventional_unit_cell=False, additional_criteria=None):
        ...
        return self.get_entries(criteria, compatible_only=compatible_only, inc_structure=inc_structure, property_data=property_data, conventional_unit_cell=conventional_unit_cell)
    ...
    def query(self, criteria, properties, chunk_size: int=500, max_tries_per_chunk: int=5, mp_decode: bool=True, show_progress_bar: bool=True):
        ...
        for chunk in chunks:
            ...
            while n_tries < max_tries_per_chunk:
                try:
                    data += self.query(chunk_criteria, properties, chunk_size=0, mp_decode=mp_decode)
                    break
                except MPRestError as exc:
                    match = re.search('error status code (\\d+)', str(exc))
                    if match:
                        if not match.group(1).startswith('5'):
                            raise exc
                        n_tries += 1
                        print(f'Unknown server error. Trying again in five seconds (will try at most {max_tries_per_chunk} times)...')
                        sleep(5)
            progress_bar.update(len(chunk))
        return data

    def submit_structures(self, structures, authors, projects=None, references='', remarks=None, data=None, histories=None, created_at=None):
        ...
        snl_list = StructureNL.from_structures(structures, authors, projects, references, remarks, data, histories, created_at)
    ...
    def get_stability(self, entries):
        ...
        response = self.session.post(f'{self.preamble}/phase_diagram/calculate_stability', data=payload)
        if response.status_code in [200, 400]:
            response = json.loads(response.text, cls=MontyDecoder)
            if response['valid_response']:
                if response.get('warning'):
    ...
    def get_reaction(self, reactants, products):
        ...
        Returns:
            rxn
        """
        return self._make_request('/reaction', payload={'reactants[]': reactants, 'products[]': products}, mp_decode=False)
