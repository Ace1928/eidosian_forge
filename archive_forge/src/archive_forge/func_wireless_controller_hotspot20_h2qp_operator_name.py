...
def wireless_controller_hotspot20_h2qp_operator_name(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('wireless-controller.hotspot20', 'h2qp-operator-name', filtered_data, vdom=vdom)
        current_data = fos.get('wireless-controller.hotspot20', 'h2qp-operator-name', vdom=vdom, mkey=mkey)
    ...
    if state == 'present' or state is True:
        return fos.set('wireless-controller.hotspot20', 'h2qp-operator-name', data=filtered_data, vdom=vdom)
    elif state == 'absent':
        return fos.delete('wireless-controller.hotspot20', 'h2qp-operator-name', mkey=filtered_data['name'], vdom=vdom)
    else:
