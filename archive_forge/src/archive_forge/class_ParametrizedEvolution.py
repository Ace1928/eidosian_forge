...
class ParametrizedEvolution(Operation):
    ...
    **Example**
    ...
    def __init__(self, H: ParametrizedHamiltonian, params: list=None, t: Union[float, List[float]]=None, return_intermediate: bool=False, complementary: bool=False, dense: bool=None, id=None, **odeint_kwargs):
        if not all((op.has_matrix or isinstance(op, qml.Hamiltonian) for op in H.ops)):
            raise ValueError('All operators inside the parametrized hamiltonian must have a matrix defined.')
        ...
        if complementary and (not return_intermediate):
            warnings.warn('The keyword argument complementary does not have any effect if return_intermediate is set to False.')
        if params is None:
            params = []
        elif not isinstance(H, HardwareHamiltonian) and len(params) != len(H.coeffs_parametrized):
            raise ValueError(f'The length of the params argument and the number of scalar-valued functions in the Hamiltonian must be the same. Received len(params)={len(params)!r} parameters but expected {len(H.coeffs_parametrized)} parameters.')
    ...
    def __call__(self, params, t, return_intermediate=None, complementary=None, dense=None, **odeint_kwargs):
        if not has_jax:
            raise ImportError('Module jax is required for the ``ParametrizedEvolution`` class. You can install jax via: pip install jax')
        ...
        return ParametrizedEvolution(H=self.H, params=params, t=t, return_intermediate=return_intermediate, complementary=complementary, dense=dense, id=self.id, **odeint_kwargs)
    ...
    def hash(self):
        ...
        return hash((str(self.name), tuple(self.wires.tolist()), str(self.hyperparameters.values()), str(self.t), str(self.data), self.H, str(self.odeint_kwargs.values())))
    ...
    def _flatten(self):
        ...
        metadata = (t, self.H, self.hyperparameters['return_intermediate'], self.hyperparameters['complementary'], self.dense, odeint_kwargs_tuples)
        return (data, metadata)

    @classmethod
    def _unflatten(cls, data, metadata):
        t, H, return_intermediate, complementary, dense, odeint_kwargs = metadata
        return cls(H, None if len(data) == 0 else data, t, return_intermediate=return_intermediate, complementary=complementary, dense=dense, **dict(odeint_kwargs))
    ...
    def matrix(self, wire_order=None):
        ...
        if not self.has_matrix:
            raise ValueError('The parameters and the time window are required to compute the matrix. You can update its values by calling the class: EV(params, t).')
        y0 = jnp.eye(2 ** len(self.wires), dtype=complex)
        with jax.ensure_compile_time_eval():
            H_jax = ParametrizedHamiltonianPytree.from_hamiltonian(self.H, dense=self.dense, wire_order=self.wires)
