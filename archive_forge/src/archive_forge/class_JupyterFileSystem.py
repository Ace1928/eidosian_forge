...
class JupyterFileSystem(fsspec.AbstractFileSystem):
    ...
    def pipe_file(self, path, value, **_):
        path = self._strip_protocol(path)
        json = {'name': path.rsplit('/', 1)[-1], 'path': path, 'size': len(value), 'content': base64.b64encode(value).decode(), 'format': 'base64', 'type': 'file'}
    ...
    def mkdir(self, path, create_parents=True, **kwargs):
        ...
        json = {'name': path.rsplit('/', 1)[-1], 'path': path, 'size': None, 'content': None, 'type': 'directory'}
        self.session.put(f'{self.url}/{path}', json=json)
