...
class GPUMonitor(BaseMetricsMonitor):
    ...
    def __init__(self):
        if 'pynvml' not in sys.modules:
            raise ImportError('`pynvml` is not installed, to log GPU metrics please run `pip install pynvml` to install it.')
        try:
            pynvml.nvmlInit()
    ...
    def collect_metrics(self):
        for i, handle in enumerate(self.gpu_handles):
            ...
            self._metrics[f'gpu_{i}_memory_usage_percentage'].append(round(memory.used / memory.total * 100, 1))
            self._metrics[f'gpu_{i}_memory_usage_megabytes'].append(memory.used / 1000000.0)
