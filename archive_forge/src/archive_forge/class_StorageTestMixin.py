...
class StorageTestMixin(object):
    ...
    def test_fetch_unknown_name(self):
        s = self._get_storage()
        self.assertRaisesRegex(exceptions.NotFound, "^Name 'xxx' is not mapped", s.fetch, 'xxx')
    ...
    def test_fetch_mapped_args(self):
        s = self._get_storage()
        s.inject({'foo': 'bar', 'spam': 'eggs'})
        self.assertEqual({'viking': 'eggs'}, s.fetch_mapped_args({'viking': 'spam'}))
    ...
    def test_fetch_optional_args_found(self):
        s = self._get_storage()
        s.inject({'foo': 'bar', 'spam': 'eggs'})
        self.assertEqual({'viking': 'eggs'}, s.fetch_mapped_args({'viking': 'spam'}, optional_args=set(['viking'])))
    ...
    def test_get_state_of_unknown_task(self):
        s = self._get_storage()
        self.assertRaisesRegex(exceptions.NotFound, '^Unknown', s.get_atom_state, 'my task')
    ...
    def test_unknown_task_by_name(self):
        s = self._get_storage()
        self.assertRaisesRegex(exceptions.NotFound, '^Unknown atom', s.get_atom_uuid, '42')
    ...
    def test_result_is_checked(self):
        ...
        s.save('my task', {})
        self.assertRaisesRegex(exceptions.NotFound, '^Unable to find result', s.fetch, 'result')

    def test_empty_result_is_checked(self):
        s = self._get_storage()
        s.ensure_atom(test_utils.NoopTask('my task', provides=['a']))
        s.save('my task', ())
        self.assertRaisesRegex(exceptions.NotFound, '^Unable to find result', s.fetch, 'a')
    ...
    def test_short_result_is_checked(self):
        ...
        self.assertRaisesRegex(exceptions.NotFound, '^Unable to find result', s.fetch, 'b')
    ...
    def test_ensure_retry_and_task_with_same_name(self):
        ...
        self.assertRaisesRegex(exceptions.Duplicate, '^Atom detail', s.ensure_atom, test_utils.NoopRetry('my retry'))
