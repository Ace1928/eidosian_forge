...
class Normalization(base_preprocessing_layer.PreprocessingLayer):
    ...
    def __init__(self, axis=-1, mean=None, variance=None, invert=False, **kwargs):
        ...
        if isinstance(mean, tf.Variable):
            raise ValueError('Normalization does not support passing a Variable for the `mean` init arg.')
        if isinstance(variance, tf.Variable):
            raise ValueError('Normalization does not support passing a Variable for the `variance` init arg.')
        if (mean is not None) != (variance is not None):
            raise ValueError('When setting values directly, both `mean` and `variance` must be set. Got mean: {} and variance: {}'.format(mean, variance))
    ...
    def build(self, input_shape):
        ...
        if isinstance(input_shape, (list, tuple)) and all((isinstance(shape, tf.TensorShape) for shape in input_shape)):
            raise ValueError('Normalization only accepts a single input. If you are passing a python list or tuple as a single input, please convert to a numpy array or `tf.Tensor`.')
        input_shape = tf.TensorShape(input_shape).as_list()
        ndim = len(input_shape)
        if any((a < -ndim or a >= ndim for a in self.axis)):
            raise ValueError('All `axis` values must be in the range [-ndim, ndim). Found ndim: `{}`, axis: {}'.format(ndim, self.axis))
        self._keep_axis = sorted([d if d >= 0 else d + ndim for d in self.axis])
        for d in self._keep_axis:
            if input_shape[d] is None:
                raise ValueError('All `axis` values to be kept must have known shape. Got axis: {}, input shape: {}, with unknown axis at index: {}'.format(self.axis, input_shape, d))
        ...
        if self.input_mean is None:
            self.adapt_mean = self.add_weight(name='mean', shape=mean_and_var_shape, dtype=self.compute_dtype, initializer='zeros', trainable=False)
            self.adapt_variance = self.add_weight(name='variance', shape=mean_and_var_shape, dtype=self.compute_dtype, initializer='ones', trainable=False)
            self.count = self.add_weight(name='count', shape=(), dtype=tf.int64, initializer='zeros', trainable=False)
            self.finalize_state()
        else:
    ...
    def update_state(self, data):
        if self.input_mean is not None:
            raise ValueError('Cannot `adapt` a Normalization layer that is initialized with static `mean` and `variance`, you passed mean {} and variance {}.'.format(self.input_mean, self.input_variance))
        if not self.built:
        ...
        batch_weight = tf.cast(batch_count, dtype=self.compute_dtype) / tf.cast(total_count, dtype=self.compute_dtype)
        existing_weight = 1.0 - batch_weight
        total_mean = self.adapt_mean * existing_weight + batch_mean * batch_weight
        total_variance = (self.adapt_variance + (self.adapt_mean - total_mean) ** 2) * existing_weight + (batch_variance + (batch_mean - total_mean) ** 2) * batch_weight
    ...
    def call(self, inputs):
        ...
        if self.invert:
            return self.mean + inputs * tf.maximum(tf.sqrt(self.variance), backend.epsilon())
        else:
            return (inputs - self.mean) / tf.maximum(tf.sqrt(self.variance), backend.epsilon())
    ...
    def get_config(self):
        ...
        config.update({'axis': self.axis, 'invert': self.invert, 'mean': utils.listify_tensors(self.input_mean), 'variance': utils.listify_tensors(self.input_variance)})
        return config
