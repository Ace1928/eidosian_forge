...
class ClickHouse(Dialect):
    ...
    class Parser(parser.Parser):
        ...
        FUNC_TOKENS = {*parser.Parser.FUNC_TOKENS, TokenType.SET}
        ...
        FUNCTION_PARSERS = {**parser.Parser.FUNCTION_PARSERS, 'ARRAYJOIN': lambda self: self.expression(exp.Explode, this=self._parse_expression()), 'QUANTILE': lambda self: self._parse_quantile()}
        FUNCTION_PARSERS.pop('MATCH')
        NO_PAREN_FUNCTION_PARSERS = parser.Parser.NO_PAREN_FUNCTION_PARSERS.copy()
        NO_PAREN_FUNCTION_PARSERS.pop('ANY')
        RANGE_PARSERS = {**parser.Parser.RANGE_PARSERS, TokenType.GLOBAL: lambda self, this: self._match(TokenType.IN) and self._parse_in(this, is_global=True)}
        COLUMN_OPERATORS = parser.Parser.COLUMN_OPERATORS.copy()
        COLUMN_OPERATORS.pop(TokenType.PLACEHOLDER)
        JOIN_KINDS = {*parser.Parser.JOIN_KINDS, TokenType.ANY, TokenType.ASOF, TokenType.ARRAY}
        TABLE_ALIAS_TOKENS = parser.Parser.TABLE_ALIAS_TOKENS - {TokenType.ANY, TokenType.ARRAY, TokenType.FINAL, TokenType.FORMAT, TokenType.SETTINGS}
        ALIAS_TOKENS = parser.Parser.ALIAS_TOKENS - {TokenType.FORMAT}
        LOG_DEFAULTS_TO_LN = True
        QUERY_MODIFIER_PARSERS = {**parser.Parser.QUERY_MODIFIER_PARSERS, TokenType.SETTINGS: lambda self: ('settings', self._advance() or self._parse_csv(self._parse_conjunction)), TokenType.FORMAT: lambda self: ('format', self._advance() or self._parse_id_var())}
        CONSTRAINT_PARSERS = {**parser.Parser.CONSTRAINT_PARSERS, 'INDEX': lambda self: self._parse_index_constraint(), 'CODEC': lambda self: self._parse_compress()}
        SCHEMA_UNNAMED_CONSTRAINTS = {*parser.Parser.SCHEMA_UNNAMED_CONSTRAINTS, 'INDEX'}
        ...
        def _parse_table(self, schema: bool=False, joins: bool=False, alias_tokens: t.Optional[t.Collection[TokenType]]=None, parse_bracket: bool=False, is_db_reference: bool=False, parse_partition: bool=False) -> t.Optional[exp.Expression]:
            this = super()._parse_table(schema=schema, joins=joins, alias_tokens=alias_tokens, parse_bracket=parse_bracket, is_db_reference=is_db_reference)
        ...
        def _parse_function(self, functions: t.Optional[t.Dict[str, t.Callable]]=None, anonymous: bool=False, optional_parens: bool=True, any_token: bool=False) -> t.Optional[exp.Expression]:
            func = super()._parse_function(functions=functions, anonymous=anonymous, optional_parens=optional_parens, any_token=any_token)
            if isinstance(func, exp.Anonymous):
                parts = self.AGG_FUNC_MAPPING.get(func.this)
                params = self._parse_func_params(func)
                if params:
                    if parts and parts[1]:
                        return self.expression(exp.CombinedParameterizedAgg, this=func.this, expressions=func.expressions, params=params, parts=parts)
                    return self.expression(exp.ParameterizedAgg, this=func.this, expressions=func.expressions, params=params)
                if parts:
                    if parts[1]:
                        return self.expression(exp.CombinedAggFunc, this=func.this, expressions=func.expressions, parts=parts)
                    return self.expression(exp.AnonymousAggFunc, this=func.this, expressions=func.expressions)
            return func
    ...
    class Generator(generator.Generator):
        ...
        STRING_TYPE_MAPPING = {exp.DataType.Type.CHAR: 'String', exp.DataType.Type.LONGBLOB: 'String', exp.DataType.Type.LONGTEXT: 'String', exp.DataType.Type.MEDIUMBLOB: 'String', exp.DataType.Type.MEDIUMTEXT: 'String', exp.DataType.Type.TINYBLOB: 'String', exp.DataType.Type.TINYTEXT: 'String', exp.DataType.Type.TEXT: 'String', exp.DataType.Type.VARBINARY: 'String', exp.DataType.Type.VARCHAR: 'String'}
        SUPPORTED_JSON_PATH_PARTS = {exp.JSONPathKey, exp.JSONPathRoot, exp.JSONPathSubscript}
        TYPE_MAPPING = {**generator.Generator.TYPE_MAPPING, **STRING_TYPE_MAPPING, exp.DataType.Type.ARRAY: 'Array', exp.DataType.Type.BIGINT: 'Int64', exp.DataType.Type.DATE32: 'Date32', exp.DataType.Type.DATETIME64: 'DateTime64', exp.DataType.Type.DOUBLE: 'Float64', exp.DataType.Type.ENUM: 'Enum', exp.DataType.Type.ENUM8: 'Enum8', exp.DataType.Type.ENUM16: 'Enum16', exp.DataType.Type.FIXEDSTRING: 'FixedString', exp.DataType.Type.FLOAT: 'Float32', exp.DataType.Type.INT: 'Int32', exp.DataType.Type.MEDIUMINT: 'Int32', exp.DataType.Type.INT128: 'Int128', exp.DataType.Type.INT256: 'Int256', exp.DataType.Type.LOWCARDINALITY: 'LowCardinality', exp.DataType.Type.MAP: 'Map', exp.DataType.Type.NESTED: 'Nested', exp.DataType.Type.NULLABLE: 'Nullable', exp.DataType.Type.SMALLINT: 'Int16', exp.DataType.Type.STRUCT: 'Tuple', exp.DataType.Type.TINYINT: 'Int8', exp.DataType.Type.UBIGINT: 'UInt64', exp.DataType.Type.UINT: 'UInt32', exp.DataType.Type.UINT128: 'UInt128', exp.DataType.Type.UINT256: 'UInt256', exp.DataType.Type.USMALLINT: 'UInt16', exp.DataType.Type.UTINYINT: 'UInt8', exp.DataType.Type.IPV4: 'IPv4', exp.DataType.Type.IPV6: 'IPv6', exp.DataType.Type.AGGREGATEFUNCTION: 'AggregateFunction', exp.DataType.Type.SIMPLEAGGREGATEFUNCTION: 'SimpleAggregateFunction'}
        TRANSFORMS = {**generator.Generator.TRANSFORMS, exp.AnyValue: rename_func('any'), exp.ApproxDistinct: rename_func('uniq'), exp.ArrayFilter: lambda self, e: self.func('arrayFilter', e.expression, e.this), exp.ArraySize: rename_func('LENGTH'), exp.ArraySum: rename_func('arraySum'), exp.ArgMax: arg_max_or_min_no_count('argMax'), exp.ArgMin: arg_max_or_min_no_count('argMin'), exp.Array: inline_array_sql, exp.CastToStrType: rename_func('CAST'), exp.CountIf: rename_func('countIf'), exp.CompressColumnConstraint: lambda self, e: f'CODEC({self.expressions(e, key='this', flat=True)})', exp.ComputedColumnConstraint: lambda self, e: f'{('MATERIALIZED' if e.args.get('persisted') else 'ALIAS')} {self.sql(e, 'this')}', exp.CurrentDate: lambda self, e: self.func('CURRENT_DATE'), exp.DateAdd: date_delta_sql('DATE_ADD'), exp.DateDiff: date_delta_sql('DATE_DIFF'), exp.Explode: rename_func('arrayJoin'), exp.Final: lambda self, e: f'{self.sql(e, 'this')} FINAL', exp.IsNan: rename_func('isNaN'), exp.JSONExtract: json_extract_segments('JSONExtractString', quoted_index=False), exp.JSONExtractScalar: json_extract_segments('JSONExtractString', quoted_index=False), exp.JSONPathKey: json_path_key_only_name, exp.JSONPathRoot: lambda *_: '', exp.Map: lambda self, e: _lower_func(var_map_sql(self, e)), exp.Nullif: rename_func('nullIf'), exp.PartitionedByProperty: lambda self, e: f'PARTITION BY {self.sql(e, 'this')}', exp.Pivot: no_pivot_sql, exp.Quantile: _quantile_sql, exp.RegexpLike: lambda self, e: self.func('match', e.this, e.expression), exp.Rand: rename_func('randCanonical'), exp.Select: transforms.preprocess([transforms.eliminate_qualify]), exp.StartsWith: rename_func('startsWith'), exp.StrPosition: lambda self, e: self.func('position', e.this, e.args.get('substr'), e.args.get('position')), exp.TimeToStr: lambda self, e: self.func('DATE_FORMAT', e.this, self.format_time(e), e.args.get('timezone')), exp.VarMap: lambda self, e: _lower_func(var_map_sql(self, e)), exp.Xor: lambda self, e: self.func('xor', e.this, e.expression, *e.expressions)}
        PROPERTIES_LOCATION = {**generator.Generator.PROPERTIES_LOCATION, exp.VolatileProperty: exp.Properties.Location.UNSUPPORTED, exp.PartitionedByProperty: exp.Properties.Location.POST_SCHEMA, exp.OnCluster: exp.Properties.Location.POST_NAME}
        JOIN_HINTS = False
        TABLE_HINTS = False
        EXPLICIT_UNION = True
        GROUPINGS_SEP = ''
        OUTER_UNION_MODIFIERS = False
        ON_CLUSTER_TARGETS = {'DATABASE', 'TABLE', 'VIEW', 'DICTIONARY', 'INDEX', 'FUNCTION', 'NAMED COLLECTION'}
