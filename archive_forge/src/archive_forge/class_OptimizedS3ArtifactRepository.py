...
class OptimizedS3ArtifactRepository(CloudArtifactRepository):
    ...
    def __init__(self, artifact_uri, access_key_id=None, secret_access_key=None, session_token=None, addressing_style='path', s3_endpoint_url=None):
    ...
    def _get_region_name(self):
        ...
        temp_client = _get_s3_client(addressing_style='path', access_key_id=self._access_key_id, secret_access_key=self._secret_access_key, session_token=self._session_token, s3_endpoint_url=self._s3_endpoint_url)
        try:
            ...
            if _RESPONSE_METADATA in head_bucket_resp and _HTTP_HEADERS in head_bucket_resp[_RESPONSE_METADATA] and (_HTTP_HEADER_BUCKET_REGION in head_bucket_resp[_RESPONSE_METADATA][_HTTP_HEADERS]):
                return head_bucket_resp[_RESPONSE_METADATA][_HTTP_HEADERS][_HTTP_HEADER_BUCKET_REGION]
            if _BUCKET_LOCATION_NAME in head_bucket_resp:
                _logger.warning(f'Directory bucket {self.bucket} found with BucketLocationName {head_bucket_resp[_BUCKET_LOCATION_NAME]}.')
    ...
    def log_artifact(self, local_file, artifact_path=None):
        ...
        self._upload_to_cloud(cloud_credential_info=self._get_s3_client(), src_file_path=local_file, artifact_file_path=artifact_file_path)
    ...
    def _multipart_upload(self, cloud_credential_info, local_file, bucket, key):
        ...
        num_parts = math.ceil(os.path.getsize(local_file) / MLFLOW_MULTIPART_UPLOAD_CHUNK_SIZE.get())

        def _upload_part(part_number, local_file, start_byte, size):
            presigned_url = s3_client.generate_presigned_url('upload_part', Params={'Bucket': bucket, 'Key': key, 'UploadId': upload_id, 'PartNumber': part_number})
        ...
        try:
            ...
            for index in range(num_parts):
                ...
                future = self.chunk_thread_pool.submit(_upload_part, part_number=part_number, local_file=local_file, start_byte=start_byte, size=MLFLOW_MULTIPART_UPLOAD_CHUNK_SIZE.get())
            ...
            parts = [{'PartNumber': part_number, 'ETag': results[part_number]} for part_number in sorted(results)]
    ...
    def _get_presigned_uri(self, remote_file_path):
        ...
        return s3_client.generate_presigned_url('get_object', Params={'Bucket': self.bucket, 'Key': s3_full_path})
    ...
    def delete_artifacts(self, artifact_path=None):
        ...
        s3_client = self._get_s3_client()
        list_objects = s3_client.list_objects(Bucket=self.bucket, Prefix=dest_path).get('Contents', [])
