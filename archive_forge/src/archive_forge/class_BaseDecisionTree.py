    ...
    def _fit(self, X, y, sample_weight=None, check_input=True, missing_values_in_feature_mask=None):
        ...
        if self.monotonic_cst is None:
            ...
            if monotonic_cst.shape[0] != X.shape[1]:
                raise ValueError('monotonic_cst has shape {} but the input data X has {} features.'.format(monotonic_cst.shape[0], X.shape[1]))
            ...
            if not np.all(valid_constraints):
                ...
                raise ValueError(f'monotonic_cst must be None or an array-like of -1, 0 or 1, but got {unique_constaints_value}')
        ...
        if not isinstance(self.splitter, Splitter):
            splitter = SPLITTERS[self.splitter](criterion, self.max_features_, min_samples_leaf, min_weight_leaf, random_state, monotonic_cst)
        if is_classifier(self):
