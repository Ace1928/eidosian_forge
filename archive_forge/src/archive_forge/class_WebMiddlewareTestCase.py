...
class WebMiddlewareTestCase(test.TestCase):
    ...
    def test_wsgi_middleware_disabled(self, mock_profiler_init):
        ...
        headers = {'a': '1', 'b': '2', 'X-Trace-Info': pack[0], 'X-Trace-HMAC': pack[1]}
        self._test_wsgi_middleware_with_invalid_trace(headers, hmac_key, mock_profiler_init, enabled=False)
    ...
    def test_wsgi_middleware_invalid_trace_headers(self, mock_profiler_init):
        headers = {'a': '1', 'b': '2', 'X-Trace-Info': 'abbababababa', 'X-Trace-HMAC': 'abbababababa'}
        self._test_wsgi_middleware_with_invalid_trace(headers, 'secret', mock_profiler_init)
    ...
    def test_wsgi_middleware_key_passthrough(self, mock_profiler_init):
        ...
        middleware = web.WsgiMiddleware('app', 'secret1,%s' % hmac_key, enabled=True)
        self.assertEqual('yeah!', middleware(request))
    ...
    def test_wsgi_middleware(self, mock_profiler_init, mock_profiler_trace):
        ...
        self.assertEqual('yeah!', middleware(request))
        mock_profiler_init.assert_called_once_with(hmac_key=hmac_key, base_id='1', parent_id='2')
        expected_info = {'request': {'path': request.path, 'query': request.query_string, 'method': request.method, 'scheme': request.scheme}}
