...
class TestDataPyStan:
    ...
    def get_inference_data(self, data, eight_schools_params):
        """vars as str."""
        return from_pystan(posterior=data.obj, posterior_predictive='y_hat', predictions='y_hat', prior=data.obj, prior_predictive='y_hat', observed_data='y', constant_data='sigma', predictions_constant_data='sigma', log_likelihood={'y': 'log_lik'}, coords={'school': np.arange(eight_schools_params['J'])}, dims={'theta': ['school'], 'y': ['school'], 'sigma': ['school'], 'y_hat': ['school'], 'eta': ['school']}, posterior_model=data.model, prior_model=data.model)
    ...
    def get_inference_data3(self, data, eight_schools_params):
        ...
        return from_pystan(posterior=data.obj, posterior_predictive=['y_hat', 'log_lik'], predictions=['y_hat', 'log_lik'], prior=data.obj, prior_predictive=['y_hat', 'log_lik'], constant_data=['sigma', 'y'], predictions_constant_data=['sigma', 'y'], coords={'school': np.arange(eight_schools_params['J'])}, dims={'theta': ['school'], 'y': ['school'], 'sigma': ['school'], 'y_hat': ['school'], 'eta': ['school']}, posterior_model=data.model, prior_model=data.model)

    def get_inference_data4(self, data):
        """minimal input."""
        return from_pystan(posterior=data.obj, posterior_predictive=None, prior=data.obj, prior_predictive=None, coords=None, dims=None, posterior_model=data.model, log_likelihood=[], prior_model=data.model, save_warmup=True)

    def get_inference_data5(self, data):
        """minimal input."""
        return from_pystan(posterior=data.obj, posterior_predictive=None, prior=data.obj, prior_predictive=None, coords=None, dims=None, posterior_model=data.model, log_likelihood=False, prior_model=data.model, save_warmup=True, dtypes={'eta': int})
    ...
    def test_inference_data(self, data, eight_schools_params):
        ...
        test_dict = {'posterior': ['theta', '~log_lik'], 'posterior_predictive': ['y_hat'], 'predictions': ['y_hat'], 'observed_data': ['y'], 'constant_data': ['sigma'], 'predictions_constant_data': ['sigma'], 'sample_stats': ['diverging', 'lp'], 'log_likelihood': ['y', '~log_lik'], 'prior': ['theta']}
        fails = check_multiple_attrs(test_dict, inference_data1)
        assert not fails
        test_dict = {'posterior_predictive': ['y_hat'], 'predictions': ['y_hat'], 'observed_data': ['y'], 'sample_stats_prior': ['diverging'], 'sample_stats': ['diverging', 'lp'], 'log_likelihood': ['log_lik'], 'prior_predictive': ['y_hat']}
        fails = check_multiple_attrs(test_dict, inference_data2)
        assert not fails
        assert any((item in inference_data2.posterior.attrs for item in ['stan_code', 'program_code']))
        assert any((item in inference_data2.sample_stats.attrs for item in ['stan_code', 'program_code']))
        test_dict = {'posterior_predictive': ['y_hat', 'log_lik'], 'predictions': ['y_hat', 'log_lik'], 'constant_data': ['sigma', 'y'], 'predictions_constant_data': ['sigma', 'y'], 'sample_stats_prior': ['diverging'], 'sample_stats': ['diverging', 'lp'], 'log_likelihood': ['log_lik'], 'prior_predictive': ['y_hat', 'log_lik']}
        fails = check_multiple_attrs(test_dict, inference_data3)
        assert not fails
        test_dict = {'posterior': ['theta'], 'prior': ['theta'], 'sample_stats': ['diverging', 'lp'], '~log_likelihood': [''], 'warmup_posterior': ['theta'], 'warmup_sample_stats': ['diverging', 'lp']}
