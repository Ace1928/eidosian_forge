...
def fifo_queue_v2(component_types, shapes=[], capacity: int=-1, container: str='', shared_name: str='', name=None) -> _atypes.TensorFuzzingAnnotation[_atypes.Resource]:
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'FIFOQueueV2', name, 'component_types', component_types, 'shapes', shapes, 'capacity', capacity, 'container', container, 'shared_name', shared_name)
            return _result
            ...
            pass
        try:
            return fifo_queue_v2_eager_fallback(component_types=component_types, shapes=shapes, capacity=capacity, container=container, shared_name=shared_name, name=name, ctx=_ctx)
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('FIFOQueueV2', component_types=component_types, shapes=shapes, capacity=capacity, container=container, shared_name=shared_name, name=name)
    _result = _outputs[:]
