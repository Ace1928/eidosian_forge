...
class TiDBChatMessageHistory(BaseChatMessageHistory):
    ...
    def __init__(self, session_id: str, connection_string: str, table_name: str='langchain_message_store', earliest_time: Optional[datetime]=None):
        """
    ...
    def add_message(self, message: BaseMessage) -> None:
        ...
        query = text(f'INSERT INTO {self.table_name} (session_id, message) VALUES (:session_id, :message);')
        try:
            self.session.execute(query, {'session_id': self.session_id, 'message': json.dumps(message_to_dict(message))})
            self.session.commit()
