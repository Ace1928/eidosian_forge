...
class ComputeCloudMixin:
    ...
    def get_flavor(self, name_or_id, filters=None, get_extra=True):
        ...
        if not filters:
            filters = {}
        flavor = self.compute.find_flavor(name_or_id, get_extra_specs=get_extra, ignore_missing=True, **filters)
        return flavor
    ...
    def get_server(self, name_or_id=None, filters=None, detailed=False, bare=False, all_projects=False):
        ...
        """
        searchfunc = functools.partial(self.search_servers, detailed=detailed, bare=True, all_projects=all_projects)
    ...
    @_utils.valid_kwargs('meta', 'files', 'userdata', 'description', 'reservation_id', 'return_raw', 'min_count', 'max_count', 'security_groups', 'key_name', 'availability_zone', 'block_device_mapping', 'block_device_mapping_v2', 'nics', 'scheduler_hints', 'config_drive', 'admin_pass', 'disk_config')
    def create_server(self, name, image=None, flavor=None, auto_ip=True, ips=None, ip_pool=None, root_volume=None, terminate_volume=False, wait=False, timeout=180, reuse_ips=True, network=None, boot_from_volume=False, volume_size='50', boot_volume=None, volumes=None, nat_destination=None, group=None, **kwargs):
        ...
        for desired, given in (('OS-DCF:diskConfig', 'disk_config'), ('config_drive', 'config_drive'), ('key_name', 'key_name'), ('metadata', 'meta'), ('adminPass', 'admin_pass')):
        ...
        if group:
            ...
            if not group_obj:
                raise exceptions.SDKException('Server Group {group} was requested but was not found on the cloud'.format(group=group))
        ...
        if 'nics' in kwargs and (not isinstance(kwargs['nics'], list)):
            if isinstance(kwargs['nics'], dict):
                ...
                raise exceptions.SDKException('nics parameter to create_server takes a list of dicts. Got: {nics}'.format(nics=kwargs['nics']))
        if network and ('nics' not in kwargs or not kwargs['nics']):
            ...
            for net_name in network:
                ...
                if not network_obj:
                    raise exceptions.SDKException('Network {network} is not a valid network in {cloud}:{region}'.format(network=network, cloud=self.name, region=self._compute_region))
        ...
        for nic in kwargs.pop('nics', []):
            ...
            if 'net-id' in nic:
                ...
                if not nic_net:
                    raise exceptions.SDKException('Requested network {net} could not be found.'.format(net=net_name))
                net['uuid'] = nic_net['id']
            for ip_key in ('v4-fixed-ip', 'v6-fixed-ip', 'fixed_ip'):
                fixed_ip = nic.pop(ip_key, None)
                if fixed_ip and net.get('fixed_ip'):
                    raise exceptions.SDKException('Only one of v4-fixed-ip, v6-fixed-ip or fixed_ip may be given')
                if fixed_ip:
            ...
            if 'tag' in nic:
                ...
                net['tag'] = nic.pop('tag')
            if nic:
                raise exceptions.SDKException('Additional unsupported keys given for server network creation: {keys}'.format(keys=nic.keys()))
            networks.append(net)
        if networks:
            kwargs['networks'] = networks
        elif utils.supports_microversion(self.compute, '2.37'):
        ...
        kwargs = self._get_boot_from_volume_kwargs(image=image, boot_from_volume=boot_from_volume, boot_volume=boot_volume, volume_size=str(volume_size), terminate_volume=terminate_volume, volumes=volumes, kwargs=kwargs)
        kwargs['name'] = name
        ...
        if not wait:
            ...
            if server.status == 'ERROR':
                raise exc.OpenStackCloudCreateException(resource='server', resource_id=server.id)
            server = meta.add_server_interfaces(self, server)
        else:
            server = self.wait_for_server(server, auto_ip=auto_ip, ips=ips, ip_pool=ip_pool, reuse=reuse_ips, timeout=timeout, nat_destination=nat_destination)
    ...
    def wait_for_server(self, server, auto_ip=True, ips=None, ip_pool=None, reuse=True, timeout=180, nat_destination=None):
        """
        ...
        for count in utils.iterate_timeout(timeout, timeout_message, wait=min(5, timeout)):
            try:
            ...
            server = self.get_active_server(server=server, reuse=reuse, auto_ip=auto_ip, ips=ips, ip_pool=ip_pool, wait=True, timeout=remaining_timeout, nat_destination=nat_destination)
    ...
    def rebuild_server(self, server_id, image_id, admin_pass=None, detailed=False, bare=False, wait=False, timeout=180):
        """Rebuild a server.
    ...
    def set_server_metadata(self, name_or_id, metadata):
        ...
        server = self.get_server(name_or_id, bare=True)
        if not server:
            raise exceptions.SDKException('Invalid Server {server}'.format(server=name_or_id))
    ...
    def delete_server(self, name_or_id, wait=False, timeout=180, delete_ips=False, delete_ip_retry=1):
        ...
        if not server:
            return False
        return self._delete_server(server, wait=wait, timeout=timeout, delete_ips=delete_ips, delete_ip_retry=delete_ip_retry)
    ...
    def create_flavor(self, name, ram, vcpus, disk, description=None, flavorid='auto', ephemeral=0, swap=0, rxtx_factor=1.0, is_public=True):
        """Create a new flavor.
        ...
        :raises: :class:`~openstack.exceptions.SDKException` on operation
            error.
        """
        attrs = {'disk': disk, 'ephemeral': ephemeral, 'id': flavorid, 'is_public': is_public, 'name': name, 'ram': ram, 'rxtx_factor': rxtx_factor, 'swap': swap, 'vcpus': vcpus, 'description': description}
        if flavorid == 'auto':
            attrs['id'] = None
    ...
    def delete_flavor(self, name_or_id):
        ...
        try:
            ...
            raise exceptions.SDKException('Unable to delete flavor {name}'.format(name=name_or_id))
    ...
    def set_aggregate_metadata(self, name_or_id, metadata):
        ...
        aggregate = self.get_aggregate(name_or_id)
        if not aggregate:
            raise exceptions.SDKException('Host aggregate %s not found.' % name_or_id)
