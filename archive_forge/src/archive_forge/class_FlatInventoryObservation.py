...
class FlatInventoryObservation(TranslationHandler):
    ...
    def __init__(self, item_list, _other='other'):
        ...
        super().__init__(spaces.Dict(spaces={k: spaces.Box(low=0, high=2304, shape=(), dtype=np.int32, normalizer_scale='log') for k in item_list}))
    ...
    def from_universal(self, obs):
        item_dict = self.space.no_op()
        try:
            if obs['slots']['gui']['type'] == 'class net.minecraft.inventory.ContainerPlayer' or obs['slots']['gui']['type'] == 'class net.minecraft.inventory.ContainerWorkbench':
