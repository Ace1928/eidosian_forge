...
class ElasticsearchTestCase(test.TestCase):
    ...
    def test_get_non_empty_report(self):
        base_id = '1'
        elasticsearch_first_response = {'_scroll_id': '1', 'hits': {'hits': [{'_source': {'timestamp': '2016-08-10T16:58:03.064438', 'base_id': base_id, 'project': 'project', 'service': 'service', 'parent_id': '0', 'name': 'test', 'info': {'host': 'host'}, 'trace_id': '1'}}]}}
        elasticsearch_second_response = {'_scroll_id': base_id, 'hits': {'hits': []}}
        self.elasticsearch.client = mock.MagicMock()
        self.elasticsearch.client.search = mock.MagicMock(return_value=elasticsearch_first_response)
        self.elasticsearch.client.scroll = mock.MagicMock(return_value=elasticsearch_second_response)
        self.elasticsearch.client.reset_mock()
        self.elasticsearch.get_report(base_id)
        self.elasticsearch.client.search.assert_called_once_with(index='osprofiler-notifications', doc_type='notification', size=10000, scroll='2m', body={'query': {'match': {'base_id': base_id}}})
        self.elasticsearch.client.scroll.assert_called_once_with(scroll_id=base_id, scroll='2m')