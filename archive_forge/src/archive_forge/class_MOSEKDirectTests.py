...
class MOSEKDirectTests(unittest.TestCase):
    ...
    def test_infeasible_lp(self):
        ...
        self.assertIn(results.solver.termination_condition, (TerminationCondition.infeasible, TerminationCondition.infeasibleOrUnbounded))
    ...
    def test_unbounded_lp(self):
        ...
        opt = pyo.SolverFactory('mosek_direct')
        results = opt.solve(model)
        self.assertIn(results.solver.termination_condition, (TerminationCondition.unbounded, TerminationCondition.infeasibleOrUnbounded))
    ...
    def test_infeasible_mip(self):
        ...
        opt = pyo.SolverFactory('mosek_direct')
        results = opt.solve(model)
        self.assertIn(results.solver.termination_condition, (TerminationCondition.infeasibleOrUnbounded, TerminationCondition.infeasible))
    ...
    def _test_model(self):
        ...
        model.quad = pmo.conic.quadratic.as_domain(r=0.03, x=[pmo.expression(1.5 * model.x0 + 0.1 * model.x1), pmo.expression(0.3 * model.x0 + 2.1 * model.x2 + 0.1)])
        return model
