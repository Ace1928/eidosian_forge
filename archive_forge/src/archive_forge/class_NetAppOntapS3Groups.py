...
class NetAppOntapS3Groups:
    ...
    def delete_s3_groups(self):
        ...
        if error:
            self.module.fail_json(msg='Error deleting S3 group %s: %s' % (self.parameters['name'], to_native(error)), exception=traceback.format_exc())
    ...
    def apply(self):
        ...
        if cd_action == 'create' and (self.na_helper.safe_get(self.parameters, ['users']) is None or self.na_helper.safe_get(self.parameters, ['policies']) is None):
            self.module.fail_json(msg='policies and users are required for a creating a group.')
        if modify and (self.na_helper.safe_get(self.parameters, ['users']) is None or self.na_helper.safe_get(self.parameters, ['policies']) is None):
