...
class SentRoutes(Command):
    ...
    fmtstr = ' {0:<2s} {1:<19s} {2:<32s} {3:<8s} {4:<20s} {5:<6s} {6:<6s} {7:<}\n'
    ...
    def _retrieve_paths(self, addr_family, route_family, ip_addr):
        ...
        adj_rib_out.apply_filter(lambda k, v: addr_family == 'all' or v.path.route_family == route_family)
        return adj_rib_out
    ...
    def _format_header(cls):
        ...
        ret += cls.fmtstr.format('', 'Timestamp', 'Network', 'Labels', 'Next Hop', 'Metric', 'LocPrf', 'Path')
        return ret
    ...
    def _format_value(cls, value):
        ...
        for v in value:
            ...
            ret += cls.fmtstr.format(path_status, time, prefix, labels, str(next_hop), str(med), str(localpref), ' '.join(map(str, aspath)))
        return ret