...
class PanelCallbackHandler(BaseCallbackHandler):
    ...
    def __init__(self, instance: ChatFeed | ChatInterface, user: str='LangChain', avatar: str=DEFAULT_AVATARS['langchain']):
    ...
    def _stream(self, message: str):
        if message:
            return self.instance.stream(message, user=self._active_user, avatar=self._active_avatar, message=self._message)
    ...
    def on_retriever_end(self, documents, **kwargs: Any) -> Any:
        ...
        self.instance.send(message, user='LangChain (retriever)', avatar=DEFAULT_AVATARS['retriever'], respond=False)
