...
class ListItemObserver:
    ...
    def iter_observables(self, object):
        ...
        if not isinstance(object, TraitList):
            if self.optional:
                return
            raise ValueError('Expected a TraitList to be observed, got {!r} (type: {!r})'.format(object, type(object)))
        yield object
    ...
    def get_notifier(self, handler, target, dispatcher):
        ...
        """
        return TraitEventNotifier(handler=handler, target=target, dispatcher=dispatcher, event_factory=list_event_factory, prevent_event=lambda event: False)
    ...
    def get_maintainer(self, graph, handler, target, dispatcher):
        ...
        """
        return ObserverChangeNotifier(observer_handler=_observer_change_handler, event_factory=list_event_factory, prevent_event=lambda event: False, graph=graph, handler=handler, target=target, dispatcher=dispatcher)
