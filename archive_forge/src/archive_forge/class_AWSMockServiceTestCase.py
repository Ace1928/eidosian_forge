...
class AWSMockServiceTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.https_connection_factory = (mock.Mock(return_value=self.https_connection), ())
        self.service_connection = self.create_service_connection(https_connection_factory=self.https_connection_factory, aws_access_key_id='aws_access_key_id', aws_secret_access_key='aws_secret_access_key')
    ...
    def create_service_connection(self, **kwargs):
        if self.connection_class is None:
            raise ValueError('The connection_class class attribute must be set to a non-None value.')
