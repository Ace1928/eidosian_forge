...
class SuperconductingQubitsNoiseProperties(devices.NoiseProperties, abc.ABC):
    ...
    def _depolarizing_error(self) -> Dict[noise_utils.OpIdentifier, float]:
        ...
        for op_id, p_error in self.gate_pauli_errors.items():
            ...
            if len(op_id.qubits) != expected_qubits:
                raise ValueError(f'Gate {gate_type} takes {expected_qubits} qubit(s), but {op_id.qubits} were given.')
    ...
    def build_noise_models(self) -> List['cirq.NoiseModel']:
        ...
        if self.t1_ns:
            noise_models.append(devices.ThermalNoiseModel(set(self.t1_ns.keys()), self.gate_times_ns, cool_rate_GHz={q: 1 / T1 for q, T1 in self.t1_ns.items()}, dephase_rate_GHz={q: 1 / Tp for q, Tp in self.tphi_ns.items()}))
        depolarizing_error = self._depolarizing_error
        added_pauli_errors = {op_id: ops.depolarize(p_error, len(op_id.qubits)).on(*op_id.qubits) for op_id, p_error in depolarizing_error.items() if p_error > 0}
        ...
        if self.readout_errors:
            ...
            for qubit in self.readout_errors:
                ...
                p = p_11 / (p_00 + p_11)
                gamma = p_11 / p
                added_measure_errors[noise_utils.OpIdentifier(ops.MeasurementGate, qubit)] = ops.generalized_amplitude_damp(p, gamma).on(qubit)
            noise_models.append(devices.InsertionNoiseModel(ops_added=added_measure_errors, prepend=True))
        return noise_models