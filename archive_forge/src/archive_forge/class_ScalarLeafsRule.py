        ...
        if type_:
            ...
            if is_leaf_type(get_named_type(type_)):
                if selection_set:
                    ...
                    self.report_error(GraphQLError(f"Field '{field_name}' must not have a selection since type '{type_}' has no subfields.", selection_set))
            elif not selection_set:
                field_name = node.name.value
                self.report_error(GraphQLError(f"Field '{field_name}' of type '{type_}' must have a selection of subfields. Did you mean '{field_name} {{ ... }}'?", node))