...
def check_numerics_v2(tensor: _atypes.TensorFuzzingAnnotation[TV_CheckNumericsV2_T], message: str, name=None) -> _atypes.TensorFuzzingAnnotation[TV_CheckNumericsV2_T]:
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'CheckNumericsV2', name, tensor, 'message', message)
            return _result
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('CheckNumericsV2', tensor=tensor, message=message, name=name)
    _result = _outputs[:]
