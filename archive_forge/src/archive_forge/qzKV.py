...
class GeneticCodeElement:
    ...
    def __init__(self, type, data):
        ...
        logging.debug(
            f"Initialized GeneticCodeElement with type {self.type} and data {self.data}"
        )
    ...
    def mutate(self):
        ...
        if random.random() < self.mutation_rate:
            ...
            self.data *= mutation_factor
            logging.debug(
                f"Mutation occurred in genetic element: original data = {original_data}, mutation factor = {mutation_factor}, new data = {self.data}"
            )
    ...
    def __eq__(self, other):
        return (
            isinstance(other, GeneticCodeElement)
    ...
    def __sub__(self, other):
        if isinstance(other, GeneticCodeElement):
            ...
            raise ValueError("Can only subtract with another GeneticCodeElement")

    def __mul__(self, other):
        return self.express() * other
    ...
    def __abs__(self):
        return self.express() if self.express() >= 0 else -self.express()

    def __int__(self):
        return int(self.express())

    def __float__(self):
        return float(self.express())

    def __complex__(self):
        return complex(self.express())

    def __bool__(self):
        return bool(self.express())
    ...
    def __copy__(self):
        return DigitalSequence(self.elements.copy())

    def __deepcopy__(self):
        return DigitalSequence([element.copy() for element in self.elements])
