...
class PyPIExtensionManager(ExtensionManager):
    ...
    rpc_request_throttling = CFloat(1.0, config=True, help='Throttling time in seconds between PyPI requests using the XML-RPC API.')

    def __init__(self, app_options: Optional[dict]=None, ext_options: Optional[dict]=None, parent: Optional[config.Configurable]=None) -> None:
        ...
        self.__last_all_packages_request_time = datetime.now(tz=timezone.utc) - timedelta(seconds=self.cache_timeout * 1.01)
    ...
    async def get_latest_version(self, pkg: str) -> Optional[str]:
        ...
        """
        try:
            response = await self._httpx_client.get(self.base_url + f'/{pkg}/json', headers={'Content-Type': 'application/json'})
    ...
    async def __throttleRequest(self, recursive: bool, fn: Callable, *args) -> Any:
        ...
        try:
            ...
            if err.faultCode == -32500 and err.faultString.startswith('HTTPTooManyRequests:'):
                delay = 1.01
                match = re.search('Limit may reset in (\\d+) seconds.', err.faultString)
    ...
    async def list_packages(self, query: str, page: int, per_page: int) -> Tuple[Dict[str, ExtensionPackage], Optional[int]]:
        ...
        for name, group in groupby(filter(lambda m: query in m[0], matches), lambda e: e[0]):
            ...
            best_guess_home_url = homepage_url or data.get('project_url') or data.get('package_url') or documentation_url or source_url or bug_tracker_url
            extensions[normalized_name] = ExtensionPackage(name=normalized_name, description=data.get('summary'), homepage_url=best_guess_home_url, author=data.get('author'), license=data.get('license'), latest_version=ExtensionManager.get_semver_version(latest_version), pkg_type='prebuilt', bug_tracker_url=bug_tracker_url, documentation_url=documentation_url, package_manager_url=data.get('package_url'), repository_url=source_url)
        return (extensions, math.ceil((counter + 1) / per_page))

    async def __get_all_extensions(self) -> List[Tuple[str, str]]:
        if self.__all_packages_cache is None or datetime.now(tz=timezone.utc) > self.__last_all_packages_request_time + timedelta(seconds=self.cache_timeout):
            self.log.debug('Requesting PyPI.org RPC API for prebuilt JupyterLab extensions.')
            self.__all_packages_cache = await self.__throttleRequest(True, self._rpc_client.browse, ['Framework :: Jupyter :: JupyterLab :: Extensions :: Prebuilt'])
    ...
    async def install(self, name: str, version: Optional[str]=None) -> ActionResult:
        ...
        """
        current_loop = tornado.ioloop.IOLoop.current()
        with tempfile.TemporaryDirectory() as ve_dir, tempfile.NamedTemporaryFile(mode='w+', dir=ve_dir, delete=False) as fconstraint:
            fconstraint.write(f'jupyterlab=={__version__}')
            fconstraint.flush()
            cmdline = [sys.executable, '-m', 'pip', 'install', '--no-input', '--quiet', '--progress-bar', 'off', '--constraint', fconstraint.name]
            ...
            try:
                ...
                tmp_cmd.insert(-1, '-')
                result = await current_loop.run_in_executor(None, partial(run, tmp_cmd, capture_output=True, check=True))
                action_info = json.loads(result.stdout.decode('utf-8'))
                pkg_action = next(filter(lambda p: p.get('metadata', {}).get('name') == name.replace('_', '-'), action_info.get('install', [])))
                        ...
                        if response.status_code < 400:
                            if download_url.endswith('.whl'):
                                with ZipFile(io.BytesIO(response.content)) as wheel:
                                    for name in filter(lambda f: Path(f).name == 'package.json', wheel.namelist()):
                                ...
                                with TarFile(io.BytesIO(response.content)) as sdist:
                                    for name in filter(lambda f: Path(f).name == 'package.json', sdist.getnames()):
    ...
    async def uninstall(self, extension: str) -> ActionResult:
        ...
        try:
            ...
            lines = filter(lambda line: line.endswith('package.json'), map(lambda line: line.strip(), result.stdout.decode('utf-8').splitlines()))
            for filepath in filter(lambda f: f.name == 'package.json', map(Path, lines)):
