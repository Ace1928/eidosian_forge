...
class DistributeKplTestUtils(tf.test.TestCase):
    ...
    def dataset_fn(self, feature_mapper, label_mapper):
        ...
        raw_dataset = tf.data.Dataset.from_generator(feature_and_label_gen, output_signature={'features': tf.TensorSpec([3], tf.string), 'label': tf.TensorSpec([1], tf.string)}).shuffle(100).batch(32)
        train_dataset = raw_dataset.map(lambda x: ({'features': feature_mapper(x['features'])}, label_mapper(x['label'])))
