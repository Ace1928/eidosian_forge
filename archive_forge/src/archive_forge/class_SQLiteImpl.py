...
class SQLiteImpl(DefaultImpl):
    ...
    def requires_recreate_in_batch(self, batch_op: BatchOperationsImpl) -> bool:
        ...
        for op in batch_op.batch:
            if op[0] == 'add_column':
                col = op[1][1]
                if isinstance(col.server_default, schema.DefaultClause) and isinstance(col.server_default.arg, sql.ClauseElement):
                    return True
                elif isinstance(col.server_default, util.sqla_compat.Computed) and col.server_default.persisted:
                    return True
    ...
    def add_constraint(self, const: Constraint):
        if const._create_rule is None:
            raise NotImplementedError('No support for ALTER of constraints in SQLite dialect. Please refer to the batch mode feature which allows for SQLite migrations using a copy-and-move strategy.')
        elif const._create_rule(self):
            util.warn('Skipping unsupported ALTER for creation of implicit constraint. Please refer to the batch mode feature which allows for SQLite migrations using a copy-and-move strategy.')
