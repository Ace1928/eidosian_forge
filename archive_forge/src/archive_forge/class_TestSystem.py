...
class TestSystem(yaql.tests.TestCase):
    ...
    def test_def(self):
        self.assertEqual([1, 4, 9], self.eval('def(sq, $*$) -> $.select(sq($))', data=[1, 2, 3]))
        self.assertEqual([1, 4, 9], self.eval('def(sq, $arg * $arg) -> $.select(sq(arg => $))', data=[1, 2, 3]))

    def test_def_recursion(self):
        self.assertEqual(24, self.eval('def(rec, switch($ = 1 => 1, true => $*rec($-1))) -> rec($)', data=4))
    ...
    def test_unpack(self):
        self.assertEqual(5, self.eval('[2, 3].unpack() -> $1 + $2'))
    ...
    def test_assert(self):
        self.assertEqual([3, 4], self.eval('[2, 3].assert(len($) > 1).select($ + 1)'))
        self.assertRaises(AssertionError, self.eval, '[2].assert(len($) > 1).select($ + 1)')
        self.assertEqual(3, self.eval('[2].select($ + 1).assert(len($) = 1).first()'))
    ...
    def test_function_passing(self):
        ...
        context['func'] = func
        self.assertEqual(8, self.eval('$func(5, z => 2, y => 1)', data=func))

    def test_lambda_expression(self):
        delegate = lambda x: x ** 2
        self.assertEqual(9, self.eval('$.x[0](3)', data={'x': [delegate]}))
        self.assertEqual(9, self.eval('($.x[0])(3)', data={'x': [delegate]}))
    ...
    def test_properties(self):
        ...
        self.assertRaises(exceptions.NoMatchingFunctionException, self.eval, '"123".negValue')
        self.assertRaises(exceptions.NoMatchingFunctionException, self.eval, 'null.negValue')
        self.assertRaises(exceptions.NoFunctionRegisteredException, self.eval, '123.neg_value')

    def test_call_function(self):
        self.assertEqual(2, self.eval('call(len, [[1,2]], {})'))
        self.assertEqual(2, self.eval('call(len, [], {"sequence" => [1,2]})'))
        self.assertRaises(exceptions.NoMatchingFunctionException, self.eval, 'call(len, [[1,2]], null)')
        self.assertRaises(exceptions.NoMatchingFunctionException, self.eval, 'call(len, null, {sequence => [1,2]})')
        self.assertRaises(exceptions.NoMatchingFunctionException, self.eval, 'call(len, [], {invalid => [1,2]})')
        self.assertRaises(exceptions.NoMatchingFunctionException, self.eval, 'call(len, [1, 2], {})')
