...
class AlibabaCloudOpenSearch(VectorStore):
    ...
    def __init__(self, embedding: Embeddings, config: AlibabaCloudOpenSearchSettings, **kwargs: Any) -> None:
        try:
            from alibabacloud_ha3engine_vector import client, models
            from alibabacloud_tea_util import models as util_models
        except ImportError:
            raise ImportError('Could not import alibaba cloud opensearch python package. Please install it with `pip install alibabacloud-ha3engine-vector`.')
        self.config = config
        self.embedding = embedding
        self.runtime = util_models.RuntimeOptions(connect_timeout=5000, read_timeout=10000, autoretry=False, ignore_ssl=False, max_idle_conns=50)
    ...
    def add_texts(self, texts: Iterable[str], metadatas: Optional[List[dict]]=None, **kwargs: Any) -> List[str]:
        ...
        for idx, doc_id in enumerate(id_list):
            ...
            if embedding is not None:
                add_doc_fields.__setitem__(field_name_map['embedding'], self.config.embedding_field_separator.join((str(unit) for unit in embedding)))
    ...
    def similarity_search(self, query: str, k: int=4, search_filter: Optional[Dict[str, Any]]=None, **kwargs: Any) -> List[Document]:
    ...
    def similarity_search_by_vector(self, embedding: List[float], k: int=4, search_filter: Optional[dict]=None, **kwargs: Any) -> List[Document]:
    ...
    @classmethod
    def from_texts(cls, texts: List[str], embedding: Embeddings, metadatas: Optional[List[dict]]=None, config: Optional[AlibabaCloudOpenSearchSettings]=None, **kwargs: Any) -> 'AlibabaCloudOpenSearch':
    ...
    @classmethod
    def from_documents(cls, documents: List[Document], embedding: Embeddings, config: Optional[AlibabaCloudOpenSearchSettings]=None, **kwargs: Any) -> 'AlibabaCloudOpenSearch':
        """Create alibaba cloud opensearch vector store instance.
