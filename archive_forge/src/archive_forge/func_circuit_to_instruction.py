...
def circuit_to_instruction(circuit, parameter_map=None, equivalence_library=None, label=None):
    ...
    qc._data = data
    qc._parameter_table = ParameterTable({param: ParameterReferences(((operation_map[id(operation)], param_index) for operation, param_index in target._parameter_table[param])) for param in target._parameter_table})
