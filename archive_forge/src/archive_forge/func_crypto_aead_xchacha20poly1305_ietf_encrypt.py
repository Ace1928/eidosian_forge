...
def crypto_aead_xchacha20poly1305_ietf_encrypt(message: bytes, aad: Optional[bytes], nonce: bytes, key: bytes) -> bytes:
    ...
    mlen = len(message)
    ensure(mlen <= crypto_aead_xchacha20poly1305_ietf_MESSAGEBYTES_MAX, 'Message must be at most {} bytes long'.format(crypto_aead_xchacha20poly1305_ietf_MESSAGEBYTES_MAX), raising=exc.ValueError)
    ...
    res = lib.crypto_aead_xchacha20poly1305_ietf_encrypt(ciphertext, clen, message, mlen, _aad, aalen, ffi.NULL, nonce, key)
