...
class S3Connection(AWSAuthConnection):
    ...
    def __init__(self, aws_access_key_id=None, aws_secret_access_key=None, is_secure=True, port=None, proxy=None, proxy_port=None, proxy_user=None, proxy_pass=None, host=NoHostProvided, debug=0, https_connection_factory=None, calling_format=None, path='/', provider='aws', bucket_class=Bucket, security_token=None, suppress_consec_slashes=True, anon=None, validate_certs=None, profile_name=None):
        self.bucket_class = bucket_class
        if calling_format is None:
            calling_format = boto.config.get('s3', 'calling_format', self.DefaultCallingFormat)
        ...
        super(S3Connection, self).__init__(host, aws_access_key_id, aws_secret_access_key, is_secure, port, proxy, proxy_port, proxy_user, proxy_pass, debug=debug, https_connection_factory=https_connection_factory, path=path, provider=provider, security_token=security_token, suppress_consec_slashes=suppress_consec_slashes, validate_certs=validate_certs, profile_name=profile_name)
    ...
    def generate_url(self, expires_in, method, bucket='', key='', headers=None, query_auth=True, force_http=False, response_headers=None, expires_in_absolute=False, version_id=None):
        if self._auth_handler.capability[0] == 'hmac-v4-s3' and query_auth:
            return self.generate_url_sigv4(expires_in, method, bucket=bucket, key=key, headers=headers, force_http=force_http, response_headers=response_headers, version_id=version_id)
        headers = headers or {}
        ...
        if query_auth and (not self.anon):
            c_string = boto.utils.canonical_string(method, auth_path, headers, expires, self.provider)
            b64_hmac = self._auth_handler.sign_string(c_string)
            encoded_canonical = urllib.parse.quote(b64_hmac, safe='')
            query_part = '?' + self.QueryString % (encoded_canonical, expires, self.aws_access_key_id)
        else:
            query_part = ''
        if headers:
        ...
        if force_http:
            ...
            port = self.port
        return self.calling_format.build_url_base(self, protocol, self.server_name(port), bucket, key) + query_part
    ...
    def make_request(self, method, bucket='', key='', headers=None, data='', query_args=None, sender=None, override_num_retries=None, retry_handler=None):
        ...
        if query_args:
            ...
            boto.log.debug('auth_path=%s' % auth_path)
        return super(S3Connection, self).make_request(method, path, headers, data, host, auth_path, sender, override_num_retries=override_num_retries, retry_handler=retry_handler)