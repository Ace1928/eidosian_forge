...
def test_rebuild_name_meta(self):
    self.run_command('rebuild sample-server %s --name asdf --meta foo=bar' % FAKE_UUID_1)
    ...
    self.assert_called('POST', '/servers/1234/action', {'rebuild': {'imageRef': FAKE_UUID_1, 'name': 'asdf', 'metadata': {'foo': 'bar'}}}, pos=3)
