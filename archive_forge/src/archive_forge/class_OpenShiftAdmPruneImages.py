...
class OpenShiftAdmPruneImages(AnsibleOpenshiftModule):
    ...
    def list_objects(self):
        ...
        for kind, version in iteritems(ApiConfiguration):
            ...
            try:
                result[kind] = self.kubernetes_facts(kind=kind, api_version=version, namespace=namespace).get('resources')
            except DynamicApiError as e:
                self.fail_json(msg='An error occurred while trying to list objects.', reason=e.reason, status=e.status)
            except Exception as e:
                self.fail_json(msg='An error occurred while trying to list objects.', error=to_native(e))
        return result
    ...
    def delete_image(self, image):
        ...
        if not self.check_mode:
            try:
                ...
                self.fail_json(msg='Failed to delete object %s/%s due to: %s' % (kind, name, exc.body), reason=exc.reason, status=exc.status)
        else:
    ...
    def execute_module(self):
        ...
        analyze_ref = OpenShiftAnalyzeImageStream(ignore_invalid_refs=self.params.get('ignore_invalid_refs'), max_creation_timestamp=self.max_creation_timestamp, module=self.module)
        ...
        result = {'changed': self.changed, 'deleted_images': images, 'updated_image_streams': updated_image_streams}
