  ...
  """
    ...
    try:
        if not IsCidrWithinValidRanges(cidr):
            raise exceptions.BadArgumentException('--subnet-ip-range', 'The subnet IP range is invalid, see https://cloud.google.com/vpc/docs/subnets.md#valid-ranges')
    except ValueError:
        raise exceptions.BadArgumentException('--subnet-ip-range', 'Invalid CIDR address block.')
    return cidr