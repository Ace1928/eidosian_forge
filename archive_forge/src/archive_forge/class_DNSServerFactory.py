...
class DNSServerFactory(protocol.ServerFactory):
    ...
    def sendReply(self, protocol, message, address):
        ...
        self._verboseLog('Processed query in %0.3f seconds' % (time.time() - message.timeReceived))

    def _responseFromMessage(self, message, rCode=dns.OK, answers=None, authority=None, additional=None):
        """
        ...
        for x in answers:
            if x.isAuthoritative():
                ...
                break
        response = dns._responseFromMessage(responseConstructor=self._messageFactory, message=message, recAv=self.canRecurse, rCode=rCode, auth=authoritativeAnswer)
