...
class BigqueryClient:
    ...
    def __init__(self, *, api: str, api_version: str, project_id: Optional[str]='', dataset_id: Optional[str]='', discovery_document=_DEFAULT, job_property: str='', trace=None, sync: bool=True, wait_printer_factory: Optional[Callable[[], bq_client_utils.TransitionWaitPrinter]]=bq_client_utils.TransitionWaitPrinter, job_id_generator=bq_client_utils.JobIdGeneratorIncrementing(bq_client_utils.JobIdGeneratorRandom()), max_rows_per_request=None, quota_project_id=None, use_google_auth: bool=False, **kwds):
    ...
    """
        ...
        default_flag_values = {'iam_policy_discovery_document': _DEFAULT, 'connection_service_path': None}
        for flagname, default in default_flag_values.items():
            if not hasattr(self, flagname):
                setattr(self, flagname, default)
    columns_to_include_for_transfer_run = ['updateTime', 'schedule', 'runTime', 'scheduleTime', 'params', 'endTime', 'dataSourceId', 'destinationDatasetId', 'state', 'startTime', 'name']
    ...
    def OverrideEndpoint(self, discovery_document: Union[str, bytes], service: Service) -> Optional[str]:
    ...
    """
        ...
        logging.info('Discovery doc loaded, considering overriding rootUrl: %s', discovery_document['rootUrl'])
        is_prod = True
        if is_prod:
            discovery_document['rootUrl'] = bq_api_utils.get_tpc_root_url_from_flags(service=service, inputted_flags=bq_flags, local_params=self)
        discovery_document['baseUrl'] = urllib.parse.urljoin(discovery_document['rootUrl'], discovery_document['servicePath'])
