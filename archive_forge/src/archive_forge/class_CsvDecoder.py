...
class CsvDecoder(object):
  ...
  """
    ...
    def __init__(self, column_names, numeric_column_names, delimiter, decode_to_dict, fail_on_error, skip_initial_space):
        """Initializer.
    ...
    """
        ...
        self._reader = self._ReaderWrapper(self._LineGenerator(), column_names, delimiter, decode_to_dict, skip_initial_space)
    ...
    """
        ...
        if len(record) != len(self._column_names):
            return self._handle_corrupt_row('Columns do not match specified csv headers: %s -> %s' % (self._column_names, record))
