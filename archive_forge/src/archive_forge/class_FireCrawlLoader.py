...
class FireCrawlLoader(BaseLoader):
    ...
    def __init__(self, url: str, *, api_key: Optional[str]=None, mode: Literal['crawl', 'scrape']='crawl', params: Optional[dict]=None):
        ...
        if mode not in ('crawl', 'scrape'):
            raise ValueError(f"Unrecognized mode '{mode}'. Expected one of 'crawl', 'scrape'.")
    ...
    def lazy_load(self) -> Iterator[Document]:
        if self.mode == 'scrape':
            ...
            raise ValueError(f"Unrecognized mode '{self.mode}'. Expected one of 'crawl', 'scrape'.")
        for doc in firecrawl_docs:
            yield Document(page_content=doc.get('markdown', ''), metadata=doc.get('metadata', {}))