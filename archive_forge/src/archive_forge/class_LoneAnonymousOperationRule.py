...
class LoneAnonymousOperationRule(ASTValidationRule):
    ...
    def enter_document(self, node: DocumentNode, *_args: Any) -> None:
        self.operation_count = sum((isinstance(definition, OperationDefinitionNode) for definition in node.definitions))
    ...
    def enter_operation_definition(self, node: OperationDefinitionNode, *_args: Any) -> None:
        if not node.name and self.operation_count > 1:
            self.report_error(GraphQLError('This anonymous operation must be the only defined operation.', node))