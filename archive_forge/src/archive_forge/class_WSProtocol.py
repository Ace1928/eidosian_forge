...
class WSProtocol(asyncio.Protocol):
    ...
    def __init__(self, config: Config, server_state: ServerState, app_state: dict[str, typing.Any], _loop: asyncio.AbstractEventLoop | None=None) -> None:
    ...
    async def send(self, message: ASGISendEvent) -> None:
        ...
        if not self.handshake_complete:
            if message_type == 'websocket.accept':
                ...
                self.logger.info('%s - "WebSocket %s" %d', self.scope['client'], get_path_with_query_string(self.scope), message['status'])
                self.handshake_complete = True
                event = events.RejectConnection(status_code=message['status'], headers=list(message['headers']), has_body=True)
                output = self.conn.send(event)
                self.transport.write(output)
                self.response_started = True
            else:
                msg = "Expected ASGI message 'websocket.accept', 'websocket.close' or 'websocket.http.response.start' but got '%s'."
