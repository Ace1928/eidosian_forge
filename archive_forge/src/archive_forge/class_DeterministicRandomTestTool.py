...
class DeterministicRandomTestTool(object):
    ...
    def __init__(self, seed: int=42, mode='constant'):
        if mode not in {'constant', 'num_random_ops'}:
            raise ValueError("Mode arg must be 'constant' or 'num_random_ops'. " + f'Got: {mode}')
    ...
    def scope(self):
        ...
        return tf.compat.v1.test.mock.patch.object(self.seed_implementation, 'get_seed', wraps=_get_seed)