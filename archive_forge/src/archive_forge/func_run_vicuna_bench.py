...
def run_vicuna_bench(mt_bench_path, model_name):
    ...
    if os.path.exists(os.path.join(working_dir, 'data', 'vicuna_bench', 'model_answer', f'{model_name}.jsonl')):
        return
    commands = [f'python gen_api_answer.py --model {model_name} --max-tokens {MAX_CONTEXT} --parallel 128 --openai-api-base http://localhost:18888/v1 --bench-name vicuna_bench', f'python gen_judgment.py --model-list {model_name} --parallel 8 --mode pairwise-baseline --bench-name vicuna_bench']
    env = os.environ.copy()
