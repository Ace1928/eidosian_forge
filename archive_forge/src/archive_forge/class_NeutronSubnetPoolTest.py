...
class NeutronSubnetPoolTest(common.HeatTestCase):
    ...
    def setUp(self):
        ...
        self.find_resource = self.patchobject(neutronV20, 'find_resourceid_by_name_or_id', return_value='new_test')
    ...
    def test_validate_prefixlen_default_gt_max(self):
        ...
        self.stack = utils.parse_stack(self.t)
        rsrc = self.stack['sub_pool']
        errMessage = 'Illegal prefix bounds: max_prefixlen=24, default_prefixlen=28.'
        error = self.assertRaises(exception.StackValidationFailed, rsrc.validate)
        self.assertEqual(errMessage, str(error))
    ...
    def test_validate_prefixlen_min_gt_default(self):
        ...
        self.stack = utils.parse_stack(self.t)
        rsrc = self.stack['sub_pool']
        errMessage = 'Illegal prefix bounds: min_prefixlen=28, default_prefixlen=24.'
        error = self.assertRaises(exception.StackValidationFailed, rsrc.validate)
        self.assertEqual(errMessage, str(error))
    ...
    def test_create_subnetpool_with_tags(self):
        tags = ['for_test']
        self.set_tag_mock = self.patchobject(neutronclient.Client, 'replace_tag')
    ...
    def test_delete_subnetpool_not_found(self):
        self.patchobject(neutronclient.Client, 'delete_subnetpool', side_effect=qe.NotFound(status_code=404))
        rsrc = self.create_subnetpool()
        ref_id = rsrc.FnGetRefId()
        self.assertEqual('fc68ea2c-b60b-4b4f-bd82-94ec81110766', ref_id)
        self.assertIsNone(scheduler.TaskRunner(rsrc.delete)())
    ...
    def test_update_subnetpool(self):
        ...
        self.patchobject(rsrc, 'physical_resource_name', return_value='the_new_sp')
        ref_id = rsrc.FnGetRefId()
        self.assertEqual('fc68ea2c-b60b-4b4f-bd82-94ec81110766', ref_id)
        new_tags = ['new_tag']
        props = {'name': 'the_new_sp', 'prefixes': ['10.1.0.0/16', '10.2.0.0/16'], 'address_scope': 'new_test', 'default_quota': '16', 'default_prefixlen': '24', 'min_prefixlen': '24', 'max_prefixlen': '28', 'is_default': False, 'tags': new_tags}
        ...
        self.set_tag_mock.assert_called_with('subnetpools', rsrc.resource_id, {'tags': new_tags})
        props['name'] = None
        self.assertIsNone(rsrc.handle_update(update_snippet, {}, props))
        self.assertEqual(2, update_subnetpool.call_count)
        update_dict.pop('tags')
        update_subnetpool.assert_called_with('fc68ea2c-b60b-4b4f-bd82-94ec81110766', {'subnetpool': update_dict})
