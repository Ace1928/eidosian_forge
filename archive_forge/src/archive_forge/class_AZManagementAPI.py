...
class AZManagementAPI(ABC):
    ...
    def __init__(self, client_id: Optional[str]=None, client_secret: Optional[str]=None, oauth_url: Optional[str]=None, audience: Optional[str]=None, management_api_url: Optional[str]=None, use_http_session: Optional[bool]=False, **kwargs):
        """
        ...
        self.token_flow = ClientCredentialsFlow(audience=audience or self.management_api_url, client_id=client_id, client_secret=client_secret, oauth_url=oauth_url, **kwargs)
    ...
    def session(self) -> 'Session':
        ...
        if self._session is None:
            self._session = niquests.Session(pool_connections=10, pool_maxsize=10, retries=5)
    ...
    def asession(self) -> 'AsyncSession':
        ...
        if self._asession is None:
            self._asession = niquests.AsyncSession(pool_connections=10, pool_maxsize=10, retries=5)
    ...
    async def aget_az_client(self, client_id: str, **kwargs) -> Optional['AuthZeroClientObject']:
        """
        Returns the AuthZero Client
        """
    ...
    @timed_cache(600)
    async def aget_service_client_name(self, client_id: str) -> Optional[str]:
        """
        Returns the Service Client Name
        """
