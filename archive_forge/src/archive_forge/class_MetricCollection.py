...
class MetricCollection(ModuleDict):
    ...
    def __init__(self, metrics: Union[Metric, Sequence[Metric], Dict[str, Metric]], *additional_metrics: Metric, prefix: Optional[str]=None, postfix: Optional[str]=None, compute_groups: Union[bool, List[List[str]]]=True) -> None:
        super().__init__()
    ...
    def add_metrics(self, metrics: Union[Metric, Sequence[Metric], Dict[str, Metric]], *additional_metrics: Metric) -> None:
        ...
        if isinstance(metrics, Sequence):
            ...
            for m in additional_metrics:
                ...
                sel.append(m)
            if remain:
                rank_zero_warn(f'You have passes extra arguments {remain} which are not `Metric` so they will be ignored.')
        elif additional_metrics:
            raise ValueError(f'You have passes extra arguments {additional_metrics} which are not compatible with first passed dictionary {metrics} so they will be ignored.')
        if isinstance(metrics, dict):
            for name in sorted(metrics.keys()):
                metric = metrics[name]
                if not isinstance(metric, (Metric, MetricCollection)):
                    raise ValueError(f'Value {metric} belonging to key {name} is not an instance of `torchmetrics.Metric` or `torchmetrics.MetricCollection`')
            ...
            for metric in metrics:
                if not isinstance(metric, (Metric, MetricCollection)):
                    raise ValueError(f'Input {metric} to `MetricCollection` is not a instance of `torchmetrics.Metric` or `torchmetrics.MetricCollection`')
                if isinstance(metric, Metric):
            ...
            raise ValueError(f'Unknown input to MetricCollection. Expected, `Metric`, `MetricCollection` or `dict`/`sequence` of the previous, but got {metrics}')
    ...
    def plot(self, val: Optional[Union[Dict, Sequence[Dict]]]=None, ax: Optional[Union[_AX_TYPE, Sequence[_AX_TYPE]]]=None, together: bool=False) -> Sequence[_PLOT_OUT_TYPE]:
        ...
        if ax is not None:
            ...
            if not together and (not (isinstance(ax, Sequence) and all((isinstance(a, _AX_TYPE) for a in ax)) and (len(ax) == len(self)))):
                raise ValueError(f'Expected argument `ax` to be a sequence of matplotlib axis objects with the same length as the number of metrics in the collection, but got {type(ax)} with len {len(ax)} when `together=False`')
