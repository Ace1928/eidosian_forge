...
class PyTorchGradScaler:
    ...
    def __init__(self, enabled: bool=False, init_scale: float=2.0 ** 16, backoff_factor: float=0.5, growth_factor: float=2.0, growth_interval: int=2000):
        """
    ...
    def scale(self, tensors: Union['torch.Tensor', Iterable['torch.Tensor']], inplace=False) -> Union['torch.Tensor', List['torch.Tensor']]:
        """Scale up the values in the given tensors."""
        if not self._enabled:
            return cast('torch.Tensor', tensors)
        incorrect_type = ValueError('Input to gradient scaling must be a Tensor or Iterable[Tensor]')
        ...
        if is_torch_array(tensors):
            ...
            for tensor in tensors:
                ...
                scaled_tensors.append(self._scale_tensor(tensor, scale_per_device, inplace))
    ...
    def update(self):
        ...
        if not self._enabled:
            return
        found_inf_device = torch.full((1,), 1.0 if self._found_inf else 0.0, device=self._scale.device)
        torch._amp_update_scale_(self._scale, self._growth_tracker, found_inf_device, self._growth_factor, self._backoff_factor, self._growth_interval)
        self._found_inf = False