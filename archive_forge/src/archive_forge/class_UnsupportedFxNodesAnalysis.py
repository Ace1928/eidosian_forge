    ...
    def analyze(self, diagnostic_level: diagnostics.infra.Level) -> UnsupportedFxNodesAnalysisResult:
        ...
        for node in self.module.graph.nodes:
            if node.op == 'call_function':
                internal_opname: registration.OpName = self.onnxfunction_dispatcher._get_aten_name(node=node, diagnostic_context=self.diagnostic_context)
                overload_registration = self.onnxfunction_dispatcher.onnx_registry.is_registered_op(namespace=internal_opname.namespace, op_name=internal_opname.op_name, overload=internal_opname.overload)
                default_registration = self.onnxfunction_dispatcher.onnx_registry.is_registered_op(namespace=internal_opname.namespace, op_name=internal_opname.op_name, overload=None)
