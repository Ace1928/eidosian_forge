...
def _build_computation_for_inference(model_fn, labels, config, params):
    ...
    def computation(computation_input):
        ...
        tpu_computation, tpu_capture = _build_tpu_computation_for_inference(model_fn, computation_input, labels, config, params)
        tensors_on_cpu = tf.compat.v1.tpu.rewrite(tpu_computation)
        tpu.prune_unconnected_ops_from_xla(tf.compat.v1.get_default_graph())
        estimator_spec, export_outputs_dict, export_outputs_list, predictions_dict = tpu_capture.get()
        ...
        for i, t in enumerate(export_outputs_list):
            if t is None:
                ...
                export_outputs_list_without_none.append(export_outputs_tpu_on_cpu_list.pop(0))
        capture.capture((estimator_spec, export_outputs_dict, predictions_dict, none_indices))
