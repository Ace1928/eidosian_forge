...
class EndpointV3Compatibility:
    ...
    def __init__(self, client: Client, fn_index: int, dependency: dict, *_args):
        ...
        self.api_name: str | Literal[False] | None = '/' + api_name if isinstance(api_name, str) else api_name
    ...
    def make_predict(self, helper: Communicator | None=None):
        ...
        def _predict(*data) -> tuple:
            data = json.dumps({'data': data, 'fn_index': self.fn_index, 'session_hash': self.client.session_hash})
            hash_data = json.dumps({'fn_index': self.fn_index, 'session_hash': self.client.session_hash})
            if self.use_ws:
                result = utils.synchronize_async(self._ws_fn, data, hash_data, helper)
                if 'error' in result:
                    raise ValueError(result['error'])
            else:
                response = httpx.post(self.client.api_url, headers=self.client.headers, json=data, verify=self.client.ssl_verify)
    ...
    def _upload(self, file_paths: list[str | list[str]]) -> list[str | list[str]] | list[dict[str, Any] | list[dict[str, Any]]]:
        if not file_paths:
            return []
        files = []
        indices = []
        for i, fs in enumerate(file_paths):
            ...
            for f in fs:
                ...
                indices.append(i)
        r = httpx.post(self.client.upload_url, headers=self.client.headers, files=files, verify=self.client.ssl_verify)
        if r.status_code != 200:
            ...
            for i, fs in enumerate(file_paths):
                if isinstance(fs, list):
                    ...
                    res = [{'is_file': True, 'name': o, 'orig_name': Path(f).name, 'data': None} for f, o in zip(fs, output)]
                else:
                    o = next((o for ix, o in enumerate(result) if indices[ix] == i))
                    res = {'is_file': True, 'name': o, 'orig_name': Path(fs).name, 'data': None}
                uploaded.append(res)
        return uploaded
    ...
    def remove_skipped_components(self, *data) -> tuple:
        data = [d for d, oct in zip(data, self.output_component_types) if oct not in utils.SKIP_COMPONENTS]
        return tuple(data)

    def reduce_singleton_output(self, *data) -> Any:
        if len([oct for oct in self.output_component_types if oct not in utils.SKIP_COMPONENTS]) == 1:
            return data[0]
        else:
            return data
    ...
    def serialize(self, *data) -> tuple:
        ...
        files = [f for f, t in zip(data, self.input_component_types) if t in ['file', 'uploadbutton']]
    ...
    def deserialize(self, *data) -> tuple:
        ...
        outputs = tuple([s.deserialize(d, save_dir=self.client.output_dir, hf_token=self.client.hf_token, root_url=self.root_url) for s, d in zip(self.deserializers, data)])
        return outputs
    ...
    async def _ws_fn(self, data, hash_data, helper: Communicator):
        async with websockets.connect(self.client.ws_url, open_timeout=10, extra_headers=self.client.headers, max_size=1024 * 1024 * 1024) as websocket:
