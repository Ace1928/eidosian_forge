...
class ModinIndex:
    ...
    def _get_default_callable(dataframe_obj, axis):
        ...
        """
        return functools.partial(type(dataframe_obj)._compute_axis_labels_and_lengths, dataframe_obj, axis)
    ...
    def maybe_specify_new_frame_ref(self, value, axis) -> 'ModinIndex':
        ...
        """
        if self._value is not None and (not callable(self._value) or not self._is_default_callable):
            return self
    ...
    def get(self, return_lengths=False) -> pandas.Index:
        ...
        if not self.is_materialized:
            if callable(self._value):
                ...
                raise RuntimeError("It's not allowed to call '.materialize()' before '._value' is specified.")
            else:
    ...
    def __reduce__(self):
        ...
        """
        return (self.__class__, (self._value, self._axis), {'_lengths_cache': self._lengths_cache, '_index_id': self._index_id, '_lengths_id': self._lengths_id, '_is_default_callable': self._is_default_callable})
