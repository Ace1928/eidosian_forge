...
def _persist_update_no_change(self, new_template_id):
    ...
    lock = self.LOCK_RESPECT if self.stack.convergence else self.LOCK_NONE
    if self.status == self.FAILED:
        status_reason = _('Update status to COMPLETE for FAILED resource neither update nor replace.')
        self.state_set(self.action, self.COMPLETE, status_reason, lock=lock)
