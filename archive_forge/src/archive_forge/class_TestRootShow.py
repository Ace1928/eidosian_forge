...
class TestRootShow(TestRoot):
    ...
    def setUp(self):
        ...
        self.data = {'instance': self.fake_root.get_instance_1234_root(), 'cluster': self.fake_root.get_cls_1234_root()}
    ...
    def test_show_cluster_1234_root(self, mock_find):
        ...
        self.root_client.is_cluster_root_enabled.return_value = self.data['cluster']
        args = ['1234']
        parsed_args = self.check_parser(self.cmd, args, [])
        columns, data = self.cmd.take_action(parsed_args)
        self.assertEqual(self.columns, columns)
