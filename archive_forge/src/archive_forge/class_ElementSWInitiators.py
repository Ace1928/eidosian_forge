...
class ElementSWInitiators(object):
    ...
    def create_initiator(self, initiator):
        ...
        try:
            ...
            self.module.fail_json(msg='Error creating initiator %s' % to_native(exception_object), exception=traceback.format_exc())
    ...
    def delete_initiator(self, initiator):
        ...
        try:
            ...
            self.module.fail_json(msg='Error deleting initiator %s' % to_native(exception_object), exception=traceback.format_exc())
    ...
    def apply(self):
        ...
        for in_initiator in input_initiators:
            if self.parameters.get('state') == 'present':
                if 'initiator_id' in in_initiator and in_initiator['initiator_id'] is not None and (self.find_initiator(id=in_initiator['initiator_id']) is not None):
                ...
                if 'name' in in_initiator and in_initiator['name'] is not None and (self.find_initiator(name=in_initiator['name']) is not None):
                    changed = True
                    result_message = 'deleting initiator(s)'
                    self.delete_initiator(self.find_initiator(name=in_initiator['name'])['initiator_id'])
                elif 'initiator_id' in in_initiator and in_initiator['initiator_id'] is not None and (self.find_initiator(id=in_initiator['initiator_id']) is not None):
                    changed = True
                    result_message = 'deleting initiator(s)'
