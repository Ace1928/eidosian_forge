...
class SecurityGatewayTunnel(object):
    ...
    def __init__(self, target, access_token_callback, send_local_data_callback, close_local_connection_callback, ignore_certs=False):
    ...
    def Send(self, data):
        ...
        if log.GetVerbosity() == logging.DEBUG:
            log.err.GetConsoleWriterStream().write('DEBUG: SEND data_len [%d] data[:20] %r\n' % (data_len, data[:20]))
        sent_data = 0
