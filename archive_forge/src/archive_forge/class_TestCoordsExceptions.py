...
class TestCoordsExceptions:
    ...
    def test_invalid_coord_name_list(self, sample_dataset):
        """Assert that nicer exception appears when user enters wrong coords name"""
        _, _, data = sample_dataset
        coords = {'NOT_A_COORD_NAME': [1]}
        with pytest.raises((KeyError, ValueError), match="data\\[1\\]:.+Coords ({'NOT_A_COORD_NAME'} are invalid coordinate keys|should follow mapping format {coord_name:\\[dim1, dim2\\]})"):
            get_coords((data, data), ({'draw': [0, 1]}, coords))

    def test_invalid_coord_value_list(self, sample_dataset):
        """Assert that nicer exception appears when user enters wrong coords value"""
        _, _, data = sample_dataset
        coords = {'draw': [1234567]}
        with pytest.raises(KeyError, match='data\\[0\\]:.+Coords should follow mapping format {coord_name:\\[dim1, dim2\\]}'):
