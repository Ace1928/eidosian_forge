...
class _layout_sankey(Operation):
    ...
    def layout(self, element, **params):
        ...
        for node in graph['nodes']:
            node_data.append((np.mean([node['x0'], node['x1']]), np.mean([node['y0'], node['y1']]), node['index']) + tuple(node['values']))
    ...
    def computeNodeLinks(cls, element, graph):
        ...
        if element.nodes.vdims:
            values = zip(*(element.nodes.dimension_values(d) for d in element.nodes.vdims))
        else:
            values = cycle([tuple()])
    ...
    def computeNodeColumns(self, graph):
        ...
        for node in graph['nodes']:
            node['column'] = max(0, min(depth_upper_bound - 1, math.floor(node['depth'] if node['sourceLinks'] else depth_upper_bound - 1)))
