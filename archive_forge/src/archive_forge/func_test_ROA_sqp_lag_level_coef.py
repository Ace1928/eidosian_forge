...
def test_ROA_sqp_lag_level_coef(self):
    ...
    with SolverFactory('mindtpy') as opt:
        for model in model_list:
            model = model.clone()
            results = opt.solve(model, strategy='OA', mip_solver=required_solvers[1], nlp_solver=required_solvers[0], add_regularization='sqp_lag', equality_relaxation=True, level_coef=0.4)
