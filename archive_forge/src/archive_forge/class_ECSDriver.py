...
class ECSDriver(NodeDriver):
    ...
    NODE_STATE_MAPPING = {'Starting': NodeState.PENDING, 'Running': NodeState.RUNNING, 'Stopping': NodeState.PENDING, 'Stopped': NodeState.STOPPED}
    VOLUME_STATE_MAPPING = {'In_use': StorageVolumeState.INUSE, 'Available': StorageVolumeState.AVAILABLE, 'Attaching': StorageVolumeState.ATTACHING, 'Detaching': StorageVolumeState.INUSE, 'Creating': StorageVolumeState.CREATING, 'ReIniting': StorageVolumeState.CREATING}
    SNAPSHOT_STATE_MAPPING = {'progressing': VolumeSnapshotState.CREATING, 'accomplished': VolumeSnapshotState.AVAILABLE, 'failed': VolumeSnapshotState.ERROR}
    ...
    def create_node(self, name, size, image, auth=None, ex_security_group_id=None, ex_description=None, ex_internet_charge_type=None, ex_internet_max_bandwidth_out=None, ex_internet_max_bandwidth_in=None, ex_hostname=None, ex_io_optimized=None, ex_system_disk=None, ex_data_disks=None, ex_vswitch_id=None, ex_private_ip_address=None, ex_client_token=None, **kwargs):
        """
        @inherits: :class:`NodeDriver.create_node`
        ...
        """
        params = {'Action': 'CreateInstance', 'RegionId': self.region, 'ImageId': image.id, 'InstanceType': size.id, 'InstanceName': name}
        ...
        inet_params = self._get_internet_related_params(ex_internet_charge_type, ex_internet_max_bandwidth_in, ex_internet_max_bandwidth_out)
        if inet_params:
        ...
        if len(nodes) != 1:
            raise LibcloudError('could not find the new created node with id %s. ' % node_id, driver=self)
        node = nodes[0]
    ...
    def reboot_node(self, node, ex_force_stop=False):
        ...
        """
        params = {'Action': 'RebootInstance', 'InstanceId': node.id, 'ForceStop': u(ex_force_stop).lower()}
        resp = self.connection.request(self.path, params=params)
    ...
    def stop_node(self, node, ex_force_stop=False):
        ...
        :rtype: ``bool``
        """
        params = {'Action': 'StopInstance', 'InstanceId': node.id, 'ForceStop': u(ex_force_stop).lower()}
        resp = self.connection.request(self.path, params)
    ...
    def ex_delete_security_group_by_id(self, group_id=None):
        ...
        :type group_id: ``str``
        """
        params = {'Action': 'DeleteSecurityGroup', 'RegionId': self.region, 'SecurityGroupId': group_id}
        resp = self.connection.request(self.path, params)
        return resp.success()
    ...
    def create_volume(self, size, name, location=None, snapshot=None, ex_zone_id=None, ex_description=None, ex_disk_category=None, ex_client_token=None):
        """
        Create a new volume.
        ...
        """
        params = {'Action': 'CreateDisk', 'RegionId': self.region, 'DiskName': name, 'Size': size}
        ...
        if len(volumes) != 1:
            raise LibcloudError('could not find the new create volume with id %s.' % volume_id, driver=self)
        return volumes[0]
    ...
    def create_volume_snapshot(self, volume, name=None, ex_description=None, ex_client_token=None):
        ...
        if len(snapshots) != 1:
            raise LibcloudError('could not find new created snapshot with id %s.' % snapshot_id, driver=self)
        return snapshots[0]
    ...
    def detach_volume(self, volume, ex_instance_id=None):
        ...
        if ex_instance_id:
            ...
            if len(volumes) != 1:
                raise AttributeError('could not find the instance id the volume %s attached to, ex_instance_id is required.' % volume.id)
    ...
    def destroy_volume(self, volume):
        ...
        if volumes[0].state != StorageVolumeState.AVAILABLE:
            raise LibcloudError('only volume in AVAILABLE state could be destroyed.', driver=self)
        resp = self.connection.request(self.path, params)
        return resp.success()
    ...
    def create_image(self, node, name, description=None, ex_snapshot_id=None, ex_image_version=None, ex_client_token=None):
        """
    ...
    def delete_image(self, node_image):
        params = {'Action': 'DeleteImage', 'RegionId': self.region, 'ImageId': node_image.id}
        resp = self.connection.request(self.path, params)
        return resp.success()
    ...
    def copy_image(self, source_region, node_image, name, description=None, ex_destination_region_id=None, ex_client_token=None):
        """
        ...
        :type ex_client_token: ``str``
        """
        params = {'Action': 'CopyImage', 'RegionId': source_region, 'ImageId': node_image.id}
    ...
    def _to_node(self, instance):
        ...
        public_ip_els = findall(element=instance, xpath='PublicIpAddress/IpAddress', namespace=self.namespace)
        public_ips = _get_ips(public_ip_els)
        private_ip_els = findall(element=instance, xpath='InnerIpAddress/IpAddress', namespace=self.namespace)
        ...
        node = Node(id=_id, name=name, state=state, public_ips=public_ips, private_ips=private_ips, driver=self.connection.driver, extra=extra)
        return node
    ...
    def _to_size(self, element):
        ...
        extra['instance_type_family'] = findtext(element, 'InstanceTypeFamily', namespace=self.namespace)
        return NodeSize(id=_id, name=_id, ram=ram, disk=None, bandwidth=None, price=None, driver=self, extra=extra)
    ...
    def _to_security_group(self, element):
        ...
        return ECSSecurityGroup(_id, name, description=description, driver=self, vpc_id=vpc_id, creation_time=creation_time)
    ...
    def _to_security_group_attribute(self, element):
        ...
        return ECSSecurityGroupAttribute(ip_protocol=ip_protocol, port_range=port_range, source_group_id=source_group_id, policy=policy, nic_type=nic_type)
    ...
    def _to_zone(self, element):
        ...
        resource_types = findall(element, 'AvailableResourceCreation/ResourceTypes', namespace=self.namespace)
        instance_types = findall(element, 'AvailableInstanceTypes/InstanceTypes', namespace=self.namespace)
        disk_categories = findall(element, 'AvailableDiskCategories/DiskCategories', namespace=self.namespace)

        def _text(element):
            return element.text
        return ECSZone(id=_id, name=local_name, driver=self, available_resource_types=list(map(_text, resource_types)), available_instance_types=list(map(_text, instance_types)), available_disk_categories=list(map(_text, disk_categories)))
