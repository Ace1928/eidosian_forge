...
def AddOverrideEnvVarsFlag(parser):
    ...
    parser.add_argument('--update-env-vars', metavar='KEY=VALUE', action=arg_parsers.UpdateAction, type=arg_parsers.ArgDict(key_type=env_vars_util.EnvVarKeyType, value_type=env_vars_util.EnvVarValueType), help='List of key-value pairs to set as environment variables overrides for an execution of a job. If provided, an execution will be created with the merge result of the input values and the existing environment variables. New value overrides existing value if they have the same key. If not provided, existing environment variables are used.')