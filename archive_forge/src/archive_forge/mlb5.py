...
async def import_module_from_path(name: str, path: str) -> Module:
    """
    Asynchronously imports a module from a given file path and returns it.
...
async def load_modules_from_config(config_path: pathlib.Path) -> ModulesDict:
    """
    Asynchronously loads modules based on a configuration file specifying module names and paths.
    ...
    load_tasks = []
    for module_name, module_path in config.items():
        task = import_module_from_path(module_name, module_path)
        load_tasks.append(task)

    loaded_modules = await asyncio.gather(*load_tasks)
...
validation_modules = asyncio.run(
    load_modules_from_config(pathlib.Path(VALIDATION_RULES_FILE))
)
