...
class WarpTimeSeriesImageMultiTransformInputSpec(ANTSCommandInputSpec):
    dimension = traits.Enum(4, 3, argstr='%d', usedefault=True, desc='image dimension (3 or 4)', position=1)
    input_image = File(argstr='%s', mandatory=True, copyfile=True, desc='image to apply transformation to (generally a coregistered functional)')
    out_postfix = traits.Str('_wtsimt', argstr='%s', usedefault=True, desc='Postfix that is prepended to all output files (default = _wtsimt)')
    ...
    transformation_series = InputMultiObject(File(exists=True), argstr='%s', desc='transformation file(s) to be applied', mandatory=True, copyfile=False)
