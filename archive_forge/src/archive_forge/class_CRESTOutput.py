...
class CRESTOutput(MSONable):
    ...
    def _parse_crest_output(self):
        ...
        if self.properly_terminated:
            conformer_pattern = re.compile('\\s+\\d+\\s+(?P<Erel>\\d*\\.\\d*)\\s+(?P<Etot>-*\\d+\\.\\d+)\\s+(?P<weight>-*\\d+\\.\\d+)\\s+(?P<conformer>-*\\d+\\.\\d+)\\s+(?P<set>\\d+)\\s+(?P<degen>\\d+)\\s+(?P<origin>\\w+)\\n')
            rotamer_pattern = re.compile('\\s+\\d+\\s+(?P<Erel>\\d*\\.\\d*)\\s+(?P<Etot>-*\\d+\\.\\d+)\\s+(?P<weight>-*\\d+\\.\\d+)\\s+(?P<origin>\\w+)\\n')
