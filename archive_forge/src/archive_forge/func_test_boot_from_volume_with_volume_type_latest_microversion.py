...
def test_boot_from_volume_with_volume_type_latest_microversion(self):
    self.run_command('boot --flavor 1 --block-device id=%s,source=image,dest=volume,size=1,bootindex=0,shutdown=remove,tag=foo,volume_type=lvm bfv-server' % FAKE_UUID_1, api_version='2.latest')
