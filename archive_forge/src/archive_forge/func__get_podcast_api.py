...
def _get_podcast_api(llm: BaseLanguageModel, **kwargs: Any) -> BaseTool:
    ...
    chain = APIChain.from_llm_and_api_docs(llm, podcast_docs.PODCAST_DOCS, headers={'X-ListenAPI-Key': listen_api_key}, limit_to_domains=['https://listen-api.listennotes.com/'])
    return Tool(name='Podcast-API', description='Use the Listen Notes Podcast API to search all podcasts or episodes. The input should be a question in natural language that this API can answer.', func=chain.run)