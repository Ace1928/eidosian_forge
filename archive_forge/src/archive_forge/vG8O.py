...
class DecisionMaker:
    ...
    def retrieve_historical_food_positions(
        self,
    ):
        """
...
class GameManager:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_UP:
                        self.snake.change_direction(np.array([0, -1]))
                    elif event.key == pygame.K_DOWN:
                        self.snake.change_direction(np.array([0, 1]))
                    elif event.key == pygame.K_LEFT:
                        self.snake.change_direction(np.array([-1, 0]))
                    elif event.key == pygame.K_RIGHT:
                        self.snake.change_direction(np.array([1, 0]))

            if pygame.time.get_ticks() - self.last_update_time > 1000 / FPS:
                if not self.snake.move():
                    self.running = False  # End game on collision
                if np.array_equal(self.snake.segments[-1], self.food.position):
                    self.snake.grow()
                    self.reset_food()
    ...
    async def update_game_state(self):
        if self.snake.alive and not self.snake.frozen:
            next_direction = await asyncio.run_coroutine_threadsafe(
                await self.decision_maker.decide_next_move()
            )
            if next_direction:
