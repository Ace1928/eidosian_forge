...
class ColorWindow:
    ...
    def reset(self, gui):
        ...
        values = ['jmol', 'tag', 'force', 'velocity', 'initial charge', 'magmom', 'neighbors']
        labels = [_('By atomic number, default "jmol" colors'), _('By tag'), _('By force'), _('By velocity'), _('By initial charge'), _('By magnetic moment'), _('By number of neighbors')]
        haveit = ['numbers', 'positions', 'forces', 'momenta', 'initial_charges', 'initial_magmoms']
    ...
    def toggle(self, value):
        ...
        if value == 'jmol' or value == 'neighbors':
            ...
            scalars = np.ma.array([self.gui.get_color_scalars(i) for i in range(len(self.gui.images))])
            mn = np.min(scalars)
            mx = np.max(scalars)
            ...
            try:
                ...
                pass
            self.cmaps = [_('cmap:'), ui.ComboBox(cmaps, cmaps, self.update_colormap), _('N:'), ui.SpinBox(26, 0, 100, 1, self.update_colormap)]
            self.update_colormap('default')
            try:
                unit = {'tag': '', 'force': 'eV/Ang', 'velocity': '(eV/amu)^(1/2)', 'charge': '|e|', 'initial charge': '|e|', u'magmom': 'μB'}[value]
            except KeyError:
                unit = ''
            text = ''
            rng = mx - mn
            self.mnmx = [_('min:'), ui.SpinBox(mn, mn - 10 * rng, mx + rng, rng / 10.0, self.change_mnmx, width=20), _('max:'), ui.SpinBox(mx, mn - 10 * rng, mx + rng, rng / 10.0, self.change_mnmx, width=20), _(unit)]
            self.win.close()
            self.reset(self.gui)
