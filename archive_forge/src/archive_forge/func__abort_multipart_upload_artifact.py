...
def _abort_multipart_upload_artifact(artifact_path):
    ...
    request_message = _get_request_message(AbortMultipartUpload(), schema={'path': [_assert_required, _assert_string], 'upload_id': [_assert_string]})
    path = request_message.path
    upload_id = request_message.upload_id
    artifact_repo = _get_artifact_repo_mlflow_artifacts()
    _validate_support_multipart_upload(artifact_repo)
    artifact_repo.abort_multipart_upload(path, upload_id, artifact_path)
