...
class NewsURLLoader(BaseLoader):
    ...
    def __init__(self, urls: List[str], text_mode: bool=True, nlp: bool=False, continue_on_failure: bool=True, show_progress_bar: bool=False, **newspaper_kwargs: Any) -> None:
    ...
    def load(self) -> List[Document]:
        ...
        if self.show_progress_bar:
            try:
                ...
                raise ImportError("Package tqdm must be installed if show_progress_bar=True. Please install with 'pip install tqdm' or set show_progress_bar=False.") from e
            iter = tqdm(iter)
        return list(iter)
        ...
        for url in self.urls:
            try:
                ...
                if self.continue_on_failure:
                    ...
                    raise e
            metadata = {'title': getattr(article, 'title', ''), 'link': getattr(article, 'url', getattr(article, 'canonical_link', '')), 'authors': getattr(article, 'authors', []), 'language': getattr(article, 'meta_lang', ''), 'description': getattr(article, 'meta_description', ''), 'publish_date': getattr(article, 'publish_date', '')}
            if self.text_mode:
