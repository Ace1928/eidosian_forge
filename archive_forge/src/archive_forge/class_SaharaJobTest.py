...
class SaharaJobTest(common.HeatTestCase):
    ...
    def setUp(self):
        ...
        fake_execution.to_dict.return_value = {'job_id': 'fake-resource-id', 'id': 'fake-execution-id'}
    ...
    def test_create(self):
        ...
        expected_args = {'name': 'test_name_job', 'type': 'MapReduce', 'libs': ['some res id'], 'description': 'test_description', 'is_public': True, 'is_protected': False, 'mains': []}
    ...
    def test_delete(self):
        ...
        self.client.job_executions.delete.assert_called_once_with('fake-execution-id')

    def test_delete_not_found(self):
        jb = self._create_resource('job', self.rsrc_defn, self.stack)
        self.client.jobs.delete.side_effect = sahara.sahara_base.APIException(error_code=404)
        scheduler.TaskRunner(jb.delete)()
        self.assertEqual((jb.DELETE, jb.COMPLETE), jb.state)
        self.client.jobs.delete.assert_called_once_with(jb.resource_id)
        self.client.job_executions.delete.assert_called_once_with('fake-execution-id')
    ...
    def test_delete_job_executions_raises_error(self):
        jb = self._create_resource('job', self.rsrc_defn, self.stack)
        self.client.job_executions.find.side_effect = [sahara.sahara_base.APIException(400)]
    ...
    def test_update(self):
        ...
        self.client.jobs.update.assert_called_once_with('fake-resource-id', name='test_name_job_new', description='test_description_new', is_public=False)
    ...
    def test_attributes(self):
        ...
        self.assertEqual('fake-execution-id', jb.FnGetAtt('executions')[0]['id'])
