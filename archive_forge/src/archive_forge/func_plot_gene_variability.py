...
def plot_gene_variability(data, kernel_size=0.005, smooth=5, cutoff=None, percentile=90, ax=None, figsize=None, xlabel='Gene mean', ylabel='Standardized variance', title=None, fontsize=None, filename=None, dpi=None, **kwargs):
    ...
    **kwargs : additional arguments for `matplotlib.pyplot.hist`
    ...
    """
    variability, means = measure.gene_variability(data, kernel_size=kernel_size, smooth=smooth, return_means=True)
    keep_cells_idx = utils._get_filter_idx(variability, cutoff, percentile, keep_cells='above')
    return scatter(means, variability, c=keep_cells_idx, cmap={True: 'red', False: 'black'}, xlabel=xlabel, ylabel=ylabel, title=title, fontsize=fontsize, filename=filename, dpi=dpi, **kwargs)