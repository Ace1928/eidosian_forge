...
class PhasedISwapPowGate(eigen_gate.EigenGate):
    ...
    def __init__(self, *, phase_exponent: Union[float, sympy.Expr]=0.25, exponent: Union[float, sympy.Expr]=1.0, global_shift: float=0.0):
    ...
    def _is_parameterized_(self) -> bool:
        return protocols.is_parameterized(self._iswap) or protocols.is_parameterized(self._phase_exponent)

    def _parameter_names_(self) -> AbstractSet[str]:
        return protocols.parameter_names(self._iswap) | protocols.parameter_names(self._phase_exponent)

    def _resolve_parameters_(self, resolver: 'cirq.ParamResolver', recursive: bool) -> 'PhasedISwapPowGate':
        return self.__class__(phase_exponent=protocols.resolve_parameters(self.phase_exponent, resolver, recursive), exponent=protocols.resolve_parameters(self.exponent, resolver, recursive))

    def _with_exponent(self, exponent: value.type_alias.TParamVal) -> 'PhasedISwapPowGate':
        return PhasedISwapPowGate(phase_exponent=self.phase_exponent, exponent=exponent, global_shift=self.global_shift)
    ...
    def _apply_unitary_(self, args: 'protocols.ApplyUnitaryArgs') -> Optional[np.ndarray]:
        ...
        linalg.apply_matrix_to_slices(args.target_tensor, matrix, [oz, zo], out=args.available_buffer)
    ...
    def _pauli_expansion_(self) -> value.LinearDict[str]:
        ...
        return value.LinearDict({'II': expansion['II'], 'XX': c * v, 'YY': c * v, 'XY': s * v, 'YX': -s * v, 'ZZ': expansion['ZZ']})

    def _circuit_diagram_info_(self, args: 'cirq.CircuitDiagramInfoArgs') -> 'cirq.CircuitDiagramInfo':
        s = f'PhISwap({args.format_real(self._phase_exponent)})'
        return protocols.CircuitDiagramInfo(wire_symbols=(s, s), exponent=self._diagram_exponent(args))
