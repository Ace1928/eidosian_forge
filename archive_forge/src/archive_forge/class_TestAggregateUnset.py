...
class TestAggregateUnset(TestAggregate):
    ...
    def test_aggregate_unset(self):
        arglist = ['--property', 'unset_key', 'ag1']
        verifylist = [('properties', ['unset_key']), ('aggregate', 'ag1')]
        parsed_args = self.check_parser(self.cmd, arglist, verifylist)
        result = self.cmd.take_action(parsed_args)
        self.compute_sdk_client.set_aggregate_metadata.assert_called_once_with(self.fake_ag.id, {'unset_key': None})
