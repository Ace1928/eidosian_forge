...
@staticmethod
def export_sequence_map_add_1_sequence_1_tensor():
    ...
    input_type_protos = [onnx.helper.make_sequence_type_proto(onnx.helper.make_tensor_type_proto(onnx.TensorProto.FLOAT, ['N'])), onnx.helper.make_tensor_type_proto(onnx.TensorProto.FLOAT, ['N'])]
    output_type_protos = [onnx.helper.make_sequence_type_proto(onnx.helper.make_tensor_type_proto(onnx.TensorProto.FLOAT, ['N']))]
    expect(node, inputs=[x0, x1], outputs=[y0], input_type_protos=input_type_protos, output_type_protos=output_type_protos, name='test_sequence_map_add_1_sequence_1_tensor')