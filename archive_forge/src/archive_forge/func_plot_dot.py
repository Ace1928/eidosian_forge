...
def plot_dot(values, binwidth, dotsize, stackratio, hdi_prob, quartiles, rotated, dotcolor, intervalcolor, markersize, markercolor, marker, figsize, linewidth, point_estimate, nquantiles, point_interval, ax, show, backend_kwargs, plot_kwargs):
    """Bokeh dotplot."""
    ...
    if point_interval:
        ax = plot_point_interval(ax, values, point_estimate, hdi_prob, quartiles, linewidth, markersize, markercolor, marker, rotated, intervalcolor, 'bokeh')
    if nquantiles > values.shape[0]:
        warnings.warn('nquantiles must be less than or equal to the number of data points', UserWarning)
