...
class EtherscanLoader(BaseLoader):
    ...
    def __init__(self, account_address: str, api_key: str='docs-demo', filter: str='normal_transaction', page: int=1, offset: int=10, start_block: int=0, end_block: int=99999999, sort: str='desc'):
        ...
        if not re.match('^0x[a-fA-F0-9]{40}$', self.account_address):
            raise ValueError(f'Invalid contract address {self.account_address}')
        if filter not in ['normal_transaction', 'internal_transaction', 'erc20_transaction', 'eth_balance', 'erc721_transaction', 'erc1155_transaction']:
