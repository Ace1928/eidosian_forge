...
def create_history_aware_retriever(llm: LanguageModelLike, retriever: RetrieverLike, prompt: BasePromptTemplate) -> RetrieverOutputLike:
    ...
    if 'input' not in prompt.input_variables:
        raise ValueError(f'Expected `input` to be a prompt variable, but got {prompt.input_variables}')
    retrieve_documents: RetrieverOutputLike = RunnableBranch((lambda x: not x.get('chat_history', False), (lambda x: x['input']) | retriever), prompt | llm | StrOutputParser() | retriever).with_config(run_name='chat_retriever_chain')
