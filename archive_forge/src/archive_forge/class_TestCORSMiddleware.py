...
class TestCORSMiddleware(functional.FunctionalTest):
    ...
    def _headers(self, extra=None):
        headers = {'X-Identity-Status': 'Confirmed', 'X-Auth-Token': '932c5c84-02ac-4fe5-a9ba-620af0e2bb96', 'X-User-Id': 'f9a41d13-0c13-47e9-bee2-ce4e8bfe958e', 'X-Tenant-Id': uuids.tenant, 'X-Roles': 'reader,member'}
        if extra:
            headers.update(extra)
        return headers

    def test_valid_cors_options_request(self):
        r_headers, content = self.http.request(self.api_path, 'OPTIONS', headers=self._headers({'Origin': 'http://valid.example.com', 'Access-Control-Request-Method': 'GET'}))
    ...
    def test_invalid_cors_options_request(self):
        r_headers, content = self.http.request(self.api_path, 'OPTIONS', headers=self._headers({'Origin': 'http://invalid.example.com', 'Access-Control-Request-Method': 'GET'}))
        self.assertEqual(http.client.OK, r_headers.status)
