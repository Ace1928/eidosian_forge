...
class GenericProcessor(processor.ImportProcessor):
    ...
    * export-marks - name of file to write to save mark information to
    """
    known_params = ['info', 'trees', 'count', 'checkpoint', 'autopack', 'inv-cache', 'mode', 'import-marks', 'export-marks']

    def __init__(self, bzrdir, params=None, verbose=False, outf=None, prune_empty_dirs=True):
    ...
    def pre_process(self):
        ...
        self.cache_mgr = cache_manager.CacheManager(self.info, self.verbose, self.inventory_cache_size)
        if self.params.get('import-marks') is not None:
            ...
            if self.skip_total:
                self.note('Found %d commits already loaded - skipping over these ...', self.skip_total)
    ...
    def post_process(self):
        ...
        if self.params.get('export-marks') is not None:
            marks_file.export_marks(self.params.get('export-marks'), self.cache_mgr.marks)
        if self.cache_mgr.reftracker.last_ref is None:
            'Nothing to refresh'
            return
        self.note('Updating branch information ...')
        updater = branch_updater.BranchUpdater(self.repo, self.branch, self.cache_mgr, invert_dictset(self.cache_mgr.reftracker.heads), self.cache_mgr.reftracker.last_ref, self.tags)
        ...
        if remind_about_update:
            note("To refresh the working tree for other branches, use 'bzr update' inside that branch.")
    ...
    def commit_handler(self, cmd):
        ...
        handler = self.commit_handler_factory(cmd, self.cache_mgr, self.rev_store, verbose=self.verbose, prune_empty_dirs=self.prune_empty_dirs)
        try:
            handler.process()
        except:
        ...
        if self.max_commits is not None and self._revision_count >= self.max_commits:
            ...
            self.note('%d commits - automatic checkpoint triggered', self._revision_count)
