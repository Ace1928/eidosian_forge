...
class Trainable:
    ...
    def __init__(self, config: Dict[str, Any]=None, logger_creator: Callable[[Dict[str, Any]], 'Logger']=None, storage: Optional[StorageContext]=None):
        ...
        if setup_time > SETUP_TIME_THRESHOLD:
            logger.info('Trainable.setup took {:.3f} seconds. If your trainable is slow to initialize, consider setting reuse_actors=True to reduce actor creation overheads.'.format(setup_time))
        log_sys_usage = self.config.get('log_sys_usage', False)
        self._monitor = UtilMonitor(start=log_sys_usage)
        self.sync_num_retries = int(os.getenv('TUNE_CHECKPOINT_CLOUD_RETRY_NUM', '2'))
        self.sync_sleep_time = float(os.getenv('TUNE_CHECKPOINT_CLOUD_RETRY_WAIT_TIME_S', '1'))
    ...
    def train(self):
        ...
        now = datetime.today()
        result.update(self.get_auto_filled_metrics(now=now, time_this_iter=time_this_iter, timestamp=result_timestamp))
    ...
    def get_state(self):
        return {'iteration': self._iteration, 'timesteps_total': self._timesteps_total, 'time_total': self._time_total, 'episodes_total': self._episodes_total, 'last_result': self._last_result, 'ray_version': ray.__version__}
    ...
    def save(self, checkpoint_dir: Optional[str]=None) -> _TrainingResult:
        ...
        if not isinstance(self, ray.tune.trainable.FunctionTrainable):
            if isinstance(checkpoint_dict_or_path, dict):
                with open(os.path.join(checkpoint_dir, _DICT_CHECKPOINT_FILE_NAME), 'wb') as f:
                ...
                if checkpoint_dict_or_path != checkpoint_dir:
                    raise ValueError(f'The returned checkpoint path from `save_checkpoint` must be None or the same as the provided path argument.Got {checkpoint_dict_or_path} != {checkpoint_dir}')
            ...
            if self._storage:
                ...
                persisted_checkpoint = self._storage.persist_current_checkpoint(local_checkpoint)
                checkpoint_result = _TrainingResult(checkpoint=persisted_checkpoint, metrics=metrics)
                self._storage.persist_artifacts(force=self._storage.sync_config.sync_artifacts_on_checkpoint)
            else:
                checkpoint_result = _TrainingResult(checkpoint=local_checkpoint, metrics=metrics)
        else:
    ...
    def restore(self, checkpoint_path: Union[str, Checkpoint, _TrainingResult]):
        ...
        if not _exists_at_fs_path(checkpoint.filesystem, checkpoint.path):
            raise ValueError(f'Could not recover from checkpoint as it does not exist on storage anymore. Got storage fs type `{checkpoint.filesystem.type_name}` and path: {checkpoint.path}')
        if not isinstance(self, ray.tune.trainable.FunctionTrainable):
    ...
    def reset(self, new_config, logger_creator=None, storage=None):
        ...
        if logger_creator:
            ...
            logger.debug(f'Did not reset logger. Got: trainable.reset(logger_creator={logger_creator}).')
