...
class RNNTBeamSearch(torch.nn.Module):
    ...
    def __init__(self, model: RNNT, blank: int, temperature: float=1.0, hypo_sort_key: Optional[Callable[[Hypothesis], float]]=None, step_max_tokens: int=100) -> None:
        super().__init__()
        self.model = model
        self.blank = blank
    ...
    def _init_b_hypos(self, device: torch.device) -> List[Hypothesis]:
        ...
        init_hypo = ([token], pred_out[0].detach(), pred_state, 0.0)
        return [init_hypo]
    ...
    def _gen_next_token_probs(self, enc_out: torch.Tensor, hypos: List[Hypothesis], device: torch.device) -> torch.Tensor:
        ...
        joined_out, _, _ = self.model.join(enc_out, one_tensor, predictor_out, torch.tensor([1] * len(hypos), device=device))
    ...
    def _gen_b_hypos(self, b_hypos: List[Hypothesis], a_hypos: List[Hypothesis], next_token_probs: torch.Tensor, key_to_b_hypo: Dict[str, Hypothesis]) -> List[Hypothesis]:
        for i in range(len(a_hypos)):
            ...
            h_b = (_get_hypo_tokens(h_a), _get_hypo_predictor_out(h_a), _get_hypo_state(h_a), score)
            b_hypos.append(h_b)
    ...
    def _search(self, enc_out: torch.Tensor, hypo: Optional[List[Hypothesis]], beam_width: int) -> List[Hypothesis]:
    ...
    @torch.jit.export
    def infer(self, input: torch.Tensor, length: torch.Tensor, beam_width: int, state: Optional[List[List[torch.Tensor]]]=None, hypothesis: Optional[List[Hypothesis]]=None) -> Tuple[List[Hypothesis], List[List[torch.Tensor]]]:
