...
class URIBuilder(object):
    ...
    def from_uri(cls, reference):
        ...
        return cls(scheme=reference.scheme, userinfo=reference.userinfo, host=reference.host, port=reference.port, path=reference.path, query=reference.query, fragment=reference.fragment)
    ...
    def add_scheme(self, scheme):
        ...
        return URIBuilder(scheme=scheme, userinfo=self.userinfo, host=self.host, port=self.port, path=self.path, query=self.query, fragment=self.fragment)
    ...
    def add_credentials(self, username, password):
        ...
        return URIBuilder(scheme=self.scheme, userinfo=userinfo, host=self.host, port=self.port, path=self.path, query=self.query, fragment=self.fragment)
    ...
    def add_host(self, host):
        ...
        """
        return URIBuilder(scheme=self.scheme, userinfo=self.userinfo, host=normalizers.normalize_host(host), port=self.port, path=self.path, query=self.query, fragment=self.fragment)
    ...
    def add_path(self, path):
        ...
        if not path.startswith('/'):
            path = '/{}'.format(path)
        return URIBuilder(scheme=self.scheme, userinfo=self.userinfo, host=self.host, port=self.port, path=normalizers.normalize_path(path), query=self.query, fragment=self.fragment)
    ...
    def add_query(self, query):
        ...
        """
        return URIBuilder(scheme=self.scheme, userinfo=self.userinfo, host=self.host, port=self.port, path=self.path, query=normalizers.normalize_query(query), fragment=self.fragment)
    ...
    def finalize(self):
        ...
        """
        return uri.URIReference(self.scheme, normalizers.normalize_authority((self.userinfo, self.host, self.port)), self.path, self.query, self.fragment)
