...
class SoapClient(object):
    ...
    def __init__(self, location=None, action=None, namespace=None, cert=None, exceptions=True, proxy=None, ns=None, soap_ns=None, wsdl=None, wsdl_basedir='', cache=False, cacert=None, sessions=False, soap_server=None, timeout=TIMEOUT, http_headers=None, trace=False, username=None, password=None, key_file=None, plugins=None, strict=True):
        """
    ...
    def call(self, method, *args, **kwargs):
        ...
        xml = self.__xml % dict(method=method, namespace=self.namespace, ns=self.__ns, soap_ns=self.__soap_ns, soap_uri=soap_uri)
        ...
        if 'wsse:Security' in self.__headers and (not self.plugins):
            warnings.warn('Replace wsse:Security with UsernameToken plugin', DeprecationWarning)
        ...
        if self.exceptions and response('Fault', ns=list(soap_namespaces.values()), error=False):
            ...
            raise SoapFault(unicode(response.faultcode), unicode(response.faultstring), detail)
        for plugin in self.plugins:
    ...
    def send(self, method, xml):
        ...
        response, content = self.http.request(location, http_method, body=xml, headers=headers)
    ...
    def get_operation(self, method):
        ...
        if not self.service_port:
            for service_name, service in self.services.items():
                for port_name, port in [port for port in service['ports'].items()]:
                    ...
                    raise RuntimeError('Cannot determine service in WSDL: SOAP version: %s' % soap_ver)
        else:
        ...
        if not operation:
            raise RuntimeError('Operation %s not found in WSDL: Service/Port Type: %s' % (method, self.service_port))
        return operation
    ...
    def help(self, method):
        ...
        return '%s(%s)\n -> %s:\n\n%s\nHeaders: %s' % (method, input or '', output and output or '', operation.get('documentation', ''), headers)
    soap_ns_uris = {'http://schemas.xmlsoap.org/wsdl/soap/': 'soap11', 'http://schemas.xmlsoap.org/wsdl/soap12/': 'soap12'}
