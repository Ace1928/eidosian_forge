...
class BooleanHolder(ObjectHolder[bool]):
    ...
    def __init__(self, obj: bool, interpreter: 'Interpreter') -> None:
        ...
        self.methods.update({'to_int': self.to_int_method, 'to_string': self.to_string_method})
        self.trivial_operators.update({MesonOperator.BOOL: (None, lambda x: self.held_object), MesonOperator.NOT: (None, lambda x: not self.held_object), MesonOperator.EQUALS: (bool, lambda x: self.held_object == x), MesonOperator.NOT_EQUALS: (bool, lambda x: self.held_object != x)})
