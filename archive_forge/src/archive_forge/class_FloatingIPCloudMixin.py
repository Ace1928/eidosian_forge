...
class FloatingIPCloudMixin:
    ...
    def create_floating_ip(self, network=None, server=None, fixed_address=None, nat_destination=None, port=None, wait=False, timeout=60):
        ...
        if self._use_neutron_floating():
            try:
                return self._neutron_create_floating_ip(network_name_or_id=network, server=server, fixed_address=fixed_address, nat_destination=nat_destination, port=port, wait=wait, timeout=timeout)
            except exceptions.NotFoundException as e:
                self.log.debug("Something went wrong talking to neutron API: '%(msg)s'. Trying with Nova.", {'msg': str(e)})
        if port:
            raise exceptions.SDKException('This cloud uses nova-network which does not support arbitrary floating-ip/port mappings. Please nudge your cloud provider to upgrade the networking stack to neutron, or alternately provide the server, fixed_address and nat_destination arguments as appropriate')
    ...
    def add_ips_to_server(self, server, auto_ip=True, ips=None, ip_pool=None, wait=False, timeout=60, reuse=True, fixed_address=None, nat_destination=None):
        if ip_pool:
            server = self._add_ip_from_pool(server, ip_pool, reuse=reuse, wait=wait, timeout=timeout, fixed_address=fixed_address, nat_destination=nat_destination)
        elif ips:
            server = self.add_ip_list(server, ips, wait=wait, timeout=timeout, fixed_address=fixed_address, nat_destination=nat_destination)
        elif auto_ip:
