...
class ZoomSlider(ttk.Frame):
    ...
    def __init__(self, container, left_end, right_end, label_text=None, on_change=None):
        ...
        style_args = {'style': 'Toolbutton'} if sys.platform == 'darwin' else {}
        self.compresser = ttk.Button(self, image=self.compress_icon, command=self.zoom_in, **style_args)
        self.expander = ttk.Button(self, image=self.expand_icon, command=self.zoom_out, **style_args)
