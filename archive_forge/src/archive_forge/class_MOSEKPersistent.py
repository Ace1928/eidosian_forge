    ...
    def remove_vars(self, *solver_vars):
        ...
        *solver_var: Var (scalar Var or single _VarData)
        ...
        try:
            ...
            raise ValueError('Variable {} needs to be added before removal.'.format(v_name))
        ...
        self._solver_var_to_pyomo_var_map = dict(zip(range(var_num), self._pyomo_var_to_solver_var_map.keys()))
    ...
    def update_vars(self, *solver_vars):
        ...
        *solver_var: Constraint (scalar Constraint or single _ConstraintData)
        ...
        try:
            ...
            lbs = tuple((value(v) if v.fixed else -float('inf') if value(v.lb) is None else value(v.lb) for v in solver_vars))
            ubs = tuple((value(v) if v.fixed else float('inf') if value(v.ub) is None else value(v.ub) for v in solver_vars))
            ...
            v_name = self._symbol_map.getSymbol(v, self._labeler)
            raise ValueError('Variable {} needs to be added before it can be modified.'.format(v_name))
