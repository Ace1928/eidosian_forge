...
try:
    ...
    for col in ["weight", "price", "age"]:
        mask = np.random.choice([True, False], num_data_points, p=[0.05, 0.95])
        data[col][mask] = np.nan  # Use np.nan instead of None
    ...
    logging.info(f"Data generated and saved to {file_path}")
