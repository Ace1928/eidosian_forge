...
@deprecated(since='0.0.26', removal='0.2', alternative_import='langchain_fireworks.Fireworks')
class Fireworks(BaseLLM):
    ...
    def _generate(self, prompts: List[str], stop: Optional[List[str]]=None, run_manager: Optional[CallbackManagerForLLMRun]=None, **kwargs: Any) -> LLMResult:
        ...
        for _prompts in sub_prompts:
            response = completion_with_retry_batching(self, self.use_retry, prompt=_prompts, run_manager=run_manager, stop=stop, **params)
    ...
    async def _agenerate(self, prompts: List[str], stop: Optional[List[str]]=None, run_manager: Optional[AsyncCallbackManagerForLLMRun]=None, **kwargs: Any) -> LLMResult:
        ...
        choices = []
        for _prompts in sub_prompts:
            response = await acompletion_with_retry_batching(self, self.use_retry, prompt=_prompts, run_manager=run_manager, stop=stop, **params)
            choices.extend(response)
        return self.create_llm_result(choices, prompts)
    ...
    def create_llm_result(self, choices: Any, prompts: List[str]) -> LLMResult:
        ...
        for i, _ in enumerate(prompts):
            ...
            generations.append([Generation(text=choice.__dict__['choices'][0].text) for choice in sub_choices])
    ...
    def _stream(self, prompt: str, stop: Optional[List[str]]=None, run_manager: Optional[CallbackManagerForLLMRun]=None, **kwargs: Any) -> Iterator[GenerationChunk]:
        params = {'model': self.model, 'prompt': prompt, 'stream': True, **self.model_kwargs}
    ...
    async def _astream(self, prompt: str, stop: Optional[List[str]]=None, run_manager: Optional[AsyncCallbackManagerForLLMRun]=None, **kwargs: Any) -> AsyncIterator[GenerationChunk]:
        params = {'model': self.model, 'prompt': prompt, 'stream': True, **self.model_kwargs}
        async for stream_resp in await acompletion_with_retry_streaming(self, self.use_retry, run_manager=run_manager, stop=stop, **params):
            chunk = _stream_response_to_generation_chunk(stream_resp)
            if run_manager:
