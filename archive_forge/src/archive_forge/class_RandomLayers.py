...
class RandomLayers(Operation):
    ...
    def __init__(self, weights, wires, ratio_imprim=0.3, imprimitive=None, rotations=None, seed=42, id=None):
        shape = qml.math.shape(weights)
        if len(shape) != 2:
            raise ValueError(f'Weights tensor must be 2-dimensional; got shape {shape}')
        self._hyperparameters = {'ratio_imprim': ratio_imprim, 'imprimitive': imprimitive or qml.CNOT, 'rotations': tuple(rotations) if rotations else (qml.RX, qml.RY, qml.RZ), 'seed': seed}
