...
class ELI5Teacher(FixedDialogTeacher):
    ...
    def add_cmdline_args(argparser):
        ...
        group.add_argument('--knowledge', type='bool', default=True, help='Whether to include supporting document knowledge')
    ...
    def load_eli5(self, opt):
        ...
        if not os.path.isfile(fname):
            raise FileNotFoundError(f'{fname} not found. Please follow the instructions found at https://github.com/facebookresearch/ParlAI/tree/master/parlai/tasks/eli5/README.md to construct the dataset.')
        opt['datafile'] = fname
        ...
        for d in data:
            if self.opt['knowledge']:
                ...
                text = d['question']
            act = {'id': 'eli5', 'text': text, 'labels': [d['answer']], 'episode_done': True}
            ds.append(act)
        return ds
