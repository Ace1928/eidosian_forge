...
def test_migrate_already_sync(self, mock_validate_engine, mock_get_alembic_branch_head, mock_get_current_alembic_heads):
    ...
    self.assertIn('Data migration did not run. Data migration cannot be run before database expansion. Run database expansion first using "glance-manage db expand"', exit.code)