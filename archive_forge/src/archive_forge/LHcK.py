...
def process_huggingface_datasets(urls, raw_data_dir, processed_data_dir):
        ...
        try:
            ...
            for config in configs:
                ...
                for split in splits:
                    ...
                    # Merge datasets with the same config
    ...
    for key, df in universal_datasets.items():
        universal_dataset_dir = os.path.join(processed_data_dir, key)
        os.makedirs(universal_dataset_dir, exist_ok=True)
        df.to_csv(os.path.join(universal_dataset_dir, "universal.csv"), index=False)
