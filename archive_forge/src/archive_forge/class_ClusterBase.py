...
class ClusterBase:
    ...
    def get_layer_distance(self, miller, layers=1, tol=1e-09, new=True):
        ...
        if new:
            ...
            for h in range(size[0]):
                for k in range(size[1]):
                    for l in range(size[2]):
                        ...
                        p[m * i:m * (i + 1)] = np.dot([h, k, l] + self.atomic_basis, self.lattice_basis)
            ...
            for i in range(len(d)):
                ...
                if pattern is not None:
                    break
            if pattern is None:
                raise RuntimeError('Could not find layer distance for the ' + '(%i,%i,%i) surface.' % miller)
            if reverse:
                pattern = pattern[::-1]
            if layers < 0:
                pattern = -1 * pattern[::-1]
