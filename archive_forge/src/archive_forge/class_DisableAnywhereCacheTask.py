...
class DisableAnywhereCacheTask(task.Task):
    ...
    def execute(self, task_status_queue=None):
        log.status.Print('Requesting to disable a cache instance of bucket gs://{} having anywhere_cache_id {}'.format(self._bucket_name, self._anywhere_cache_id))
        provider = storage_url.ProviderPrefix.GCS
        api_factory.get_api(provider).disable_anywhere_cache(self._bucket_name, self._anywhere_cache_id)
