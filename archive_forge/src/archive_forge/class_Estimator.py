...
class Estimator(object):
  ...
  """
    ...
    """
        ...
        self._warm_start_settings = _get_default_warm_start_settings(warm_start_from)
    ...
    """
        ...
        if self.config.task_type in (run_config.TaskType.EVALUATOR, run_config.TaskType.PS):
            raise ValueError('Train has been called wrong configuration. Please use tf.estimator.train_and_evaluate which calls proper API according to given configuration. Current configuration: {}.'.format(self.config))
        with context.graph_mode():
            ...
            if max_steps is not None:
                ...
                if max_steps <= start_step:
                    tf.compat.v1.logging.info('Skipping training since max_steps has already saved.')
                    return self
    ...
    """
        if steps is not None or max_steps is not None:
            if self._train_distribution:
                steps_per_run = getattr(self._train_distribution.extended, 'steps_per_run', 1)
                if steps_per_run > 1:
                    return [basic_session_run_hooks._MultiStepStopAtStepHook(steps, max_steps, steps_per_run)]
    ...
    """
        ...
        if self._eval_distribution and hasattr(self._config, '_distribute_coordinator_mode') and self._config._distribute_coordinator_mode:
            return distribute_coordinator_training.estimator_evaluate(self, lambda est, s, eval_hooks: est._actual_eval(input_fn, strategy=s, steps=steps, hooks=eval_hooks, checkpoint_path=checkpoint_path, name=name), hooks)
        else:
            return self._actual_eval(input_fn, strategy=self._eval_distribution, steps=steps, hooks=hooks, checkpoint_path=checkpoint_path, name=name)
    ...
    """
        ...
        if self._eval_distribution:
            steps_per_run = getattr(self._eval_distribution.extended, 'steps_per_run', 1)
            if steps_per_run > 1:
                return [evaluation._MultiStepStopAfterNEvalsHook(num_evals=steps, steps_per_run=steps_per_run)]
        return [evaluation._StopAfterNEvalsHook(num_evals=steps)]
    ...
    """
        ...
        with context.graph_mode():
            ...
            if not checkpoint_path:
                tf.compat.v1.logging.info('Could not find trained model in model_dir: {}, running initialization to predict.'.format(self._model_dir))
            with tf.Graph().as_default() as g:
                ...
                with tf.compat.v1.train.MonitoredSession(session_creator=tf.compat.v1.train.ChiefSessionCreator(checkpoint_filename_with_path=checkpoint_path, master=self._config.master, scaffold=estimator_spec.scaffold, config=self._session_config), hooks=all_hooks) as mon_sess:
    ...
    def export_saved_model(self, export_dir_base, serving_input_receiver_fn, assets_extra=None, as_text=False, checkpoint_path=None, experimental_mode=ModeKeys.PREDICT):
    ...
    """
        ...
        return self._export_all_saved_models(export_dir_base, input_receiver_fn_map, assets_extra=assets_extra, as_text=as_text, checkpoint_path=checkpoint_path, strip_default_attrs=True)

    def experimental_export_all_saved_models(self, export_dir_base, input_receiver_fn_map, assets_extra=None, as_text=False, checkpoint_path=None):
    ...
    def _add_meta_graph_for_mode(self, builder, input_receiver_fn_map, checkpoint_path, save_variables=True, mode=ModeKeys.PREDICT, export_tags=None, check_variables=True, strip_default_attrs=True):
    ...
    """
        ...
        with tf.Graph().as_default() as g:
            ...
            estimator_spec = self._call_model_fn(features=input_receiver.features, labels=getattr(input_receiver, 'labels', None), mode=mode, config=self.config)
            export_outputs = export_lib.export_outputs_for_mode(mode=estimator_spec.mode, serving_export_outputs=estimator_spec.export_outputs, predictions=estimator_spec.predictions, loss=estimator_spec.loss, metrics=estimator_spec.eval_metric_ops)
            signature_def_map = export_lib.build_all_signature_defs(input_receiver.receiver_tensors, export_outputs, getattr(input_receiver, 'receiver_tensors_alternatives', None), serving_only=mode == ModeKeys.PREDICT)
            with tf.compat.v1.Session(config=self._session_config) as session:
                ...
                if save_variables and (not check_variables):
                    raise ValueError('If `save_variables` is `True, `check_variables`must not be `False`.')
                if check_variables:
                    try:
                        graph_saver.restore(session, checkpoint_path)
                    except tf.errors.NotFoundError as e:
                        msg = 'Could not load all requested variables from checkpoint. Please make sure your model_fn does not expect variables that were not saved in the checkpoint.\n\nEncountered error with mode `{}` while restoring checkpoint from: `{}`. Full Traceback:\n\n{}'.format(mode, checkpoint_path, e)
                        raise ValueError(msg)
                builder._add_train_op(estimator_spec.train_op)
                meta_graph_kwargs = dict(tags=export_tags, signature_def_map=signature_def_map, assets_collection=tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.ASSET_FILEPATHS), main_op=local_init_op, saver=graph_saver, strip_default_attrs=strip_default_attrs)
                if save_variables:
    ...
    def _validate_features_in_predict_input(self, result):
        if not _has_dataset_or_queue_runner(result):
            tf.compat.v1.logging.warning('Input graph does not use tf.data.Dataset or contain a QueueRunner. That means predict yields forever. This is probably a mistake.')

    def _get_iterator_from_input_fn(self, input_fn, mode, distribution=None):
        """Calls `input_fn` and returns an iterator."""
        if distribution is not None:
            iterator = distribution.make_input_fn_iterator(lambda input_context: self._call_input_fn(input_fn, mode, input_context))
    ...
    def _get_features_and_labels_from_input_fn(self, input_fn, mode):
        ...
        return estimator_util.parse_input_fn_result(self._call_input_fn(input_fn, mode))
    ...
    def _extract_batch_length(self, preds_evaluated):
        ...
        for key, value in six.iteritems(preds_evaluated):
            ...
            if value.shape[0] != batch_length:
                raise ValueError('Batch length of predictions should be same. %s has different batch length than others.' % key)
        return batch_length
    ...
    """
        ...
        if input_context and 'input_context' in input_fn_args:
            tf.compat.v1.logging.info('The `input_fn` accepts an `input_context` which will be given by DistributionStrategy')
    ...
    def _call_model_fn_eval(self, input_fn, config):
        ...
        features, labels, input_hooks = self._get_features_and_labels_from_input_fn(input_fn, ModeKeys.EVAL)
        estimator_spec = self._call_model_fn(features, labels, ModeKeys.EVAL, config)
        eval_metric_ops = _verify_and_create_loss_metric(estimator_spec.eval_metric_ops, estimator_spec.loss)
        update_op, eval_dict = _extract_metric_update_ops(eval_metric_ops)
        return (estimator_spec.scaffold, estimator_spec.evaluation_hooks, input_hooks, update_op, eval_dict)

    def _call_model_fn_eval_distributed(self, input_fn, config):
        """Call model_fn in distribution mode and handle return values."""
        iterator, input_hooks = self._get_iterator_from_input_fn(input_fn, ModeKeys.EVAL, self._eval_distribution)
        is_tpu_strategy = self._eval_distribution.__class__.__name__.startswith('TPUStrategy')
        if is_tpu_strategy:
            steps_per_run_variable = training.get_or_create_steps_per_run_variable()
            ...
            def step_fn(ctx, inputs):
                ...
                if isinstance(inputs, tuple):
                    ...
                    features = inputs
                    labels = None
                estimator_spec = self._eval_distribution.extended.call_for_each_replica(self._call_model_fn, args=(features, labels, ModeKeys.EVAL, config))
                eval_metric_ops = _verify_and_create_loss_metric(estimator_spec.eval_metric_ops, estimator_spec.loss, self._eval_distribution)
                update_op, eval_dict = _extract_metric_update_ops(eval_metric_ops, self._eval_distribution)
                ctx.set_non_tensor_output(name='estimator_spec', output=estimator_spec)
                ctx.set_non_tensor_output(name='eval_dict', output=eval_dict)
                return update_op
            ctx = self._eval_distribution.extended.experimental_run_steps_on_iterator(step_fn, iterator, iterations=steps_per_run_variable)
            update_op = ctx.run_op
            eval_dict = ctx.non_tensor_outputs['eval_dict']
            grouped_estimator_spec = ctx.non_tensor_outputs['estimator_spec']
        else:
            features, labels = estimator_util.parse_iterator_result(iterator.get_next())
            grouped_estimator_spec = self._eval_distribution.extended.call_for_each_replica(self._call_model_fn, args=(features, labels, ModeKeys.EVAL, config))
            eval_metric_ops = _verify_and_create_loss_metric(grouped_estimator_spec.eval_metric_ops, grouped_estimator_spec.loss, self._eval_distribution)
            update_op, eval_dict = _extract_metric_update_ops(eval_metric_ops, self._eval_distribution)
        scaffold = _combine_distributed_scaffold(grouped_estimator_spec.scaffold, self._eval_distribution)

        def get_hooks_from_the_first_device(per_device_hooks):
            return [self._eval_distribution.experimental_local_results(per_device_hook)[0] for per_device_hook in per_device_hooks]
        evaluation_hooks = get_hooks_from_the_first_device(grouped_estimator_spec.evaluation_hooks)
    ...
    def _evaluate_run(self, checkpoint_path, scaffold, update_op, eval_dict, all_hooks, output_dir):
        """Run evaluation."""
        eval_results = evaluation._evaluate_once(checkpoint_path=checkpoint_path, master=self._config.evaluation_master, scaffold=scaffold, eval_ops=update_op, final_ops=eval_dict, hooks=all_hooks, config=self._session_config)
        current_global_step = eval_results[tf.compat.v1.GraphKeys.GLOBAL_STEP]
        _write_dict_to_summary(output_dir=output_dir, dictionary=eval_results, current_global_step=current_global_step)
        if checkpoint_path:
            _write_checkpoint_path_to_summary(output_dir=output_dir, checkpoint_path=checkpoint_path, current_global_step=current_global_step)
        return eval_results
