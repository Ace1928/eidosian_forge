...
class CoeCloudMixin:
    ...
    def get_cluster_template(self, name_or_id, filters=None, detail=False):
        ...
        """
        return _utils._get_entity(self, 'cluster_template', name_or_id, filters=filters, detail=detail)
    ...
    def create_cluster_template(self, name, image_id=None, keypair_id=None, coe=None, **kwargs):
        ...
        :raises: :class:`~openstack.exceptions.SDKException` if something goes
            wrong during the OpenStack API call
        """
        cluster_template = self.container_infrastructure_management.create_cluster_template(name=name, image_id=image_id, keypair_id=keypair_id, coe=coe, **kwargs)
    ...
    def delete_cluster_template(self, name_or_id):
        ...
        if not cluster_template:
            self.log.debug('Cluster template %(name_or_id)s does not exist', {'name_or_id': name_or_id}, exc_info=True)
            return False
    ...
    def update_cluster_template(self, name_or_id, **kwargs):
        ...
        cluster_template = self.get_cluster_template(name_or_id)
        if not cluster_template:
            raise exceptions.SDKException('Cluster template %s not found.' % name_or_id)
        cluster_template = self.container_infrastructure_management.update_cluster_template(cluster_template, **kwargs)
        return cluster_template
