...
def get_server_migration(self, server_migration, server, ignore_missing=True):
    """Get a single server migration
        ...
        """
    server_id = self._get_uri_attribute(server_migration, server, 'server_id')
    server_migration = resource.Resource._get_id(server_migration)
    return self._get(_server_migration.ServerMigration, server_migration, server_id=server_id, ignore_missing=ignore_missing)