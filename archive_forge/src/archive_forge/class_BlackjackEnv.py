...
class BlackjackEnv(gym.Env):
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def render(self):
        ...
        dealer_card_img = scale_card_img(get_image(os.path.join('img', f'{self.dealer_top_card_suit}{self.dealer_top_card_value_str}.png')))
        dealer_card_rect = self.screen.blit(dealer_card_img, (screen_width // 2 - card_img_width - spacing // 2, dealer_text_rect.bottom + spacing))
        hidden_card_img = scale_card_img(get_image(os.path.join('img', 'Card.png')))
        self.screen.blit(hidden_card_img, (screen_width // 2 + spacing // 2, dealer_text_rect.bottom + spacing))
        ...
        player_sum_text_rect = self.screen.blit(player_sum_text, (screen_width // 2 - player_sum_text.get_width() // 2, player_text_rect.bottom + spacing))
        if usable_ace:
            usable_ace_text = small_font.render('usable ace', True, white)
            self.screen.blit(usable_ace_text, (screen_width // 2 - usable_ace_text.get_width() // 2, player_sum_text_rect.bottom + spacing // 2))
        if self.render_mode == 'human':
            pygame.event.pump()
