...
class TestWorkerTaskExecutor(test.MockTestCase):
    ...
    def setUp(self):
        ...
        self.proxy_mock, self.proxy_inst_mock = self.patchClass(executor.proxy, 'Proxy')
        self.request_mock, self.request_inst_mock = self.patchClass(executor.pr, 'Request', autospec=False)
        ...
        self.message_mock.properties = {'correlation_id': self.task_uuid, 'type': pr.RESPONSE}
    ...
    def executor(self, reset_master_mock=True, **kwargs):
        executor_kwargs = dict(uuid=self.executor_uuid, exchange=self.executor_exchange, topics=[self.executor_topic], url=self.broker_url)
    ...
    def test_creation(self):
        ...
        master_mock_calls = [mock.call.Proxy(self.executor_uuid, self.executor_exchange, on_wait=ex._on_wait, url=self.broker_url, transport=mock.ANY, transport_options=mock.ANY, retry_options=mock.ANY), mock.call.proxy.dispatcher.type_handlers.update(mock.ANY)]
    ...
    def test_revert_task(self):
        ex = self.executor()
        ex._finder._add(self.executor_topic, [self.task.name])
        ex.revert_task(self.task, self.task_uuid, self.task_args, self.task_result, self.task_failures)
    ...
    def test_restart(self):
        ...
        self.assertTrue(self.proxy_started_event.wait(test_utils.WAIT_TIMEOUT))
        ex.stop()
        self.master_mock.assert_has_calls([mock.call.proxy.start(), mock.call.proxy.wait(), mock.call.proxy.stop(), mock.call.proxy.start(), mock.call.proxy.wait(), mock.call.proxy.stop()], any_order=True)