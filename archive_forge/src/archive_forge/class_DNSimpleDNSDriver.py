...
class DNSimpleDNSDriver(DNSDriver):
    ...
    RECORD_TYPE_MAP = {RecordType.A: 'A', RecordType.AAAA: 'AAAA', RecordType.ALIAS: 'ALIAS', RecordType.CNAME: 'CNAME', RecordType.HINFO: 'HINFO', RecordType.MX: 'MX', RecordType.NAPTR: 'NAPTR', RecordType.NS: 'NS', 'POOL': 'POOL', RecordType.SOA: 'SOA', RecordType.SPF: 'SPF', RecordType.SRV: 'SRV', RecordType.SSHFP: 'SSHFP', RecordType.TXT: 'TXT', RecordType.URL: 'URL'}
    ...
    def create_record(self, name, zone, type, data, extra=None):
        ...
        response = self.connection.request('/v1/domains/%s/records' % zone.id, method='POST', data=r_data)
    ...
    def update_record(self, record, name, type, data, extra=None):
        ...
        if extra is not None:
            r_json.update(extra)
        r_data = json.dumps({'record': r_json})
        response = self.connection.request('/v1/domains/{}/records/{}'.format(zone.id, record.id), method='PUT', data=r_data)
        record = self._to_record(response.object, zone=zone)
        return record
    ...
    def delete_record(self, record):
        ...
        self.connection.request('/v1/domains/{}/records/{}'.format(zone_id, record.id), method='DELETE')
        return True
    ...
    def _to_record(self, data, zone_id=None, zone=None):
        ...
        extra = {'ttl': record.get('ttl'), 'created_at': record.get('created_at'), 'updated_at': record.get('updated_at'), 'domain_id': record.get('domain_id'), 'priority': record.get('prio')}
