...
class CreateGroupsNotificationProtocolEntity(GroupsNotificationProtocolEntity):
    ...
    def __init__(self, _id, _from, timestamp, notify, participant, offline, createType, key, groupId, creationTimestamp, creatorJid, subject, subjectTime, subjectOwnerJid, participants):
        super(CreateGroupsNotificationProtocolEntity, self).__init__(_id, _from, timestamp, notify, participant, offline)
        self.setGroupProps(createType, key, groupId, creationTimestamp, creatorJid, subject, subjectTime, subjectOwnerJid, participants)

    def setGroupProps(self, createType, key, groupId, creationTimestamp, creatorJid, subject, subjectTime, subjectOwnerJid, participants):
    ...
    def toProtocolTreeNode(self):
        ...
        groupNode = ProtocolTreeNode('group', {'subject': self.getSubject(), 'creation': str(self.getCreationTimestamp()), 'creator': self.getCreatorJid(), 's_t': self.getSubjectTimestamp(), 's_o': self.getSubjectOwnerJid(), 'id': self.getGroupId()})
        participants = []
    ...
    def fromProtocolTreeNode(node):
        ...
        return CreateGroupsNotificationProtocolEntity(node['id'], node['from'], node['t'], node['notify'], node['participant'], node['offline'], createNode['type'], createNode['key'], groupNode['id'], groupNode['creation'], groupNode['creator'], groupNode['subject'], groupNode['s_t'], groupNode['s_o'], participants)