...
class MultiBinary(Space[np.ndarray]):
    ...
    def __init__(self, n: Union[np.ndarray, Sequence[int], int], seed: Optional[Union[int, np.random.Generator]]=None):
        ...
        if isinstance(n, (Sequence, np.ndarray)):
            ...
            assert (np.asarray(input_n) > 0).all()
        else:
            self.n = n = int(n)
            input_n = (n,)
            assert (np.asarray(input_n) > 0).all()
    ...
    def sample(self, mask: Optional[np.ndarray]=None) -> np.ndarray:
        ...
        if mask is not None:
            ...
            assert mask.dtype == np.int8, f'The expected dtype of the mask is np.int8, actual dtype: {mask.dtype}'
            assert mask.shape == self.shape, f'The expected shape of the mask is {self.shape}, actual shape: {mask.shape}'
            assert np.all((mask == 0) | (mask == 1) | (mask == 2)), f'All values of a mask should be 0, 1 or 2, actual values: {mask}'
