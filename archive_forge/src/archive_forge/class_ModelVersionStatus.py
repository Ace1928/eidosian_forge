...
class ModelVersionStatus:
    ...
    def from_string(status_str):
        if status_str not in ModelVersionStatus._STRING_TO_STATUS:
            raise Exception(f'Could not get model version status corresponding to string {status_str}. Valid status strings: {list(ModelVersionStatus._STRING_TO_STATUS.keys())}')
        return ModelVersionStatus._STRING_TO_STATUS[status_str]

    @staticmethod
    def to_string(status):
        if status not in ModelVersionStatus._STATUS_TO_STRING:
            raise Exception(f'Could not get string corresponding to model version status {status}. Valid statuses: {list(ModelVersionStatus._STATUS_TO_STRING.keys())}')
