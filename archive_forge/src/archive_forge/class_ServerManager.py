...
class ServerManager(base.BootingManagerWithFind):
    ...
    def list(self, detailed=True, search_opts=None, marker=None, limit=None, sort_keys=None, sort_dirs=None):
        ...
        if search_opts is None:
            search_opts = {}
        qparams = {}
        if 'locked' in search_opts and self.api_version < api_versions.APIVersion('2.73'):
    ...
    def get_console_url(self, server, console_type):
        ...
        body = {'remote_console': {'protocol': protocol, 'type': console_type}}
    ...
    def get_password(self, server, private_key=None):
        ...
        """
        resp, body = self.api.client.get('/servers/%s/os-server-password' % base.getid(server))
    ...
    def clear_password(self, server):
        ...
        """
        return self._delete('/servers/%s/os-server-password' % base.getid(server))
    ...
    def force_delete(self, server):
        ...
        :returns: An instance of novaclient.base.TupleWithMeta
        """
        resp, body = self._action_return_resp_and_body('forceDelete', server, None)
        return base.TupleWithMeta((resp, body), resp)
    ...
    def rescue(self, server, password=None, image=None):
        ...
        resp, body = self._action_return_resp_and_body('rescue', server, info or None)
        return base.TupleWithMeta((resp, body), resp)
    ...
    def ips(self, server):
        ...
        :returns: An instance of novaclient.base.DictWithMeta
        """
        resp, body = self.api.client.get('/servers/%s/ips' % base.getid(server))
    ...
    def diagnostics(self, server):
        ...
        :returns: An instance of novaclient.base.TupleWithMeta
        """
        resp, body = self.api.client.get('/servers/%s/diagnostics' % base.getid(server))
        return base.TupleWithMeta((resp, body), resp)
    ...
    def topology(self, server):
        ...
        :returns: An instance of novaclient.base.DictWithMeta
        """
        resp, body = self.api.client.get('/servers/%s/topology' % base.getid(server))
    ...
    def create(self, name, image, flavor, meta=None, files=None, reservation_id=False, min_count=None, max_count=None, security_groups=None, userdata=None, key_name=None, availability_zone=None, block_device_mapping=None, block_device_mapping_v2=None, nics=None, scheduler_hints=None, config_drive=None, disk_config=None, admin_pass=None, access_ip_v4=None, access_ip_v6=None, description=None, tags=None, trusted_image_certificates=None, host=None, hypervisor_hostname=None, hostname=None):
        ...
        if self.api_version < tags_microversion:
            if nics:
                for nic_info in nics:
                    if nic_info.get('tag'):
                        raise ValueError('Setting interface tags is unsupported before microversion 2.32')
            if block_device_mapping_v2:
                for bdm in block_device_mapping_v2:
                    if bdm.get('tag'):
                        raise ValueError('Setting block device tags is unsupported before microversion 2.32')
        ...
        if trusted_image_certificates and self.api_version < trusted_certs_microversion:
            raise exceptions.UnsupportedAttribute('trusted_image_certificates', '2.63')
        if block_device_mapping_v2 and self.api_version < api_versions.APIVersion('2.67'):
            for bdm in block_device_mapping_v2:
                if bdm.get('volume_type'):
                    raise ValueError('Block device volume_type is not supported before microversion 2.67')
        ...
        if hypervisor_hostname and self.api_version < hypervisor_hostname_microversion:
            raise exceptions.UnsupportedAttribute('hypervisor_hostname', '2.74')
    ...
    def rebuild(self, server, image, password=None, disk_config=None, preserve_ephemeral=False, name=None, meta=None, files=None, *, description=_SENTINEL, key_name=_SENTINEL, userdata=_SENTINEL, trusted_image_certificates=_SENTINEL, hostname=_SENTINEL):
        """
        ...
        :returns: :class:`Server`
        """
        if description is not _SENTINEL and self.api_version < api_versions.APIVersion('2.19'):
            raise exceptions.UnsupportedAttribute('description', '2.19')
        if key_name is not _SENTINEL and self.api_version < api_versions.APIVersion('2.54'):
            raise exceptions.UnsupportedAttribute('key_name', '2.54')
        if files and self.api_version >= api_versions.APIVersion('2.57'):
            raise exceptions.UnsupportedAttribute('files', '2.0', '2.56')
        if userdata is not _SENTINEL and self.api_version < api_versions.APIVersion('2.57'):
            raise exceptions.UnsupportedAttribute('userdata', '2.57')
        if trusted_image_certificates is not _SENTINEL and self.api_version < api_versions.APIVersion('2.63'):
            raise exceptions.UnsupportedAttribute('trusted_image_certificates', '2.63')
        if hostname is not _SENTINEL and self.api_version < api_versions.APIVersion('2.90'):
            raise exceptions.UnsupportedAttribute('hostname', '2.90')
        ...
        if userdata is not _SENTINEL:
            userdata = userdata
            body['user_data'] = userdata if userdata is None else self.transform_userdata(userdata)
        resp, body = self._action_return_resp_and_body('rebuild', server, body)
    ...
    def create_image(self, server, image_name, metadata=None):
        ...
        resp, body = self._action_return_resp_and_body('createImage', server, body)
    ...
    def backup(self, server, backup_name, backup_type, rotation):
        ...
        """
        body = {'name': backup_name, 'backup_type': backup_type, 'rotation': rotation}
    ...
    def get_console_output(self, server, length=None):
        ...
        :returns: An instance of novaclient.base.StrWithMeta or
                  novaclient.base.UnicodeWithMeta
        """
        resp, body = self._action_return_resp_and_body('os-getConsoleOutput', server, {'length': length})
    ...
    def _live_migrate(self, server, host, block_migration, disk_over_commit, force):
    ...
    def live_migrate(self, server, host, block_migration, disk_over_commit):
        ...
        :returns: An instance of novaclient.base.TupleWithMeta
        """
        return self._live_migrate(server, host, block_migration=block_migration, disk_over_commit=disk_over_commit, force=None)
    ...
    def live_migrate(self, server, host, block_migration):
        ...
        :returns: An instance of novaclient.base.TupleWithMeta
        """
        return self._live_migrate(server, host, block_migration=block_migration, disk_over_commit=None, force=None)
    ...
    def live_migrate(self, server, host, block_migration, force=None):
        ...
        :returns: An instance of novaclient.base.TupleWithMeta
        """
        return self._live_migrate(server, host, block_migration=block_migration, disk_over_commit=None, force=force)
    ...
    def add_security_group(self, server, security_group):
        ...
        :returns: An instance of novaclient.base.DictWithMeta
        """
        return self._action('addSecurityGroup', server, {'name': security_group})
    ...
    def remove_security_group(self, server, security_group):
        ...
        :returns: An instance of novaclient.base.TupleWithMeta
        """
        return self._action('removeSecurityGroup', server, {'name': security_group})
    ...
    def evacuate(self, server, host=None, on_shared_storage=True, password=None):
        """
        ...
        :returns: An instance of novaclient.base.TupleWithMeta
        """
        return self._evacuate(server, host, on_shared_storage=on_shared_storage, password=password, force=None)
    ...
    def evacuate(self, server, host=None, password=None):
        ...
        :returns: An instance of novaclient.base.TupleWithMeta
        """
        return self._evacuate(server, host, on_shared_storage=None, password=password, force=None)
    ...
    def evacuate(self, server, host=None, password=None, force=None):
        ...
        :returns: An instance of novaclient.base.TupleWithMeta
        """
        return self._evacuate(server, host, on_shared_storage=None, password=password, force=force)
    ...
    def interface_list(self, server):
        ...
        """
        return self._list('/servers/%s/os-interface' % base.getid(server), 'interfaceAttachments', obj_class=NetworkInterface)
    ...
    def interface_attach(self, server, port_id, net_id, fixed_ip):
        ...
        if fixed_ip:
            body['interfaceAttachment']['fixed_ips'] = [{'ip_address': fixed_ip}]
        return self._create('/servers/%s/os-interface' % base.getid(server), body, 'interfaceAttachment', obj_class=NetworkInterface)
    ...
    def interface_detach(self, server, port_id):
        ...
        :returns: An instance of novaclient.base.TupleWithMeta
        """
        return self._delete('/servers/%s/os-interface/%s' % (base.getid(server), port_id))
    ...
    def _action(self, action, server, info=None, **kwargs):
        ...
        """
        resp, body = self._action_return_resp_and_body(action, server, info=info, **kwargs)
        return self.convert_into_with_meta(body, resp)

    def _action_return_resp_and_body(self, action, server, info=None, **kwargs):
        """
    ...
    def tag_list(self, server):
        ...
        """
        resp, body = self.api.client.get('/servers/%s/tags' % base.getid(server))
    ...
    def add_tag(self, server, tag):
        ...
        """
        return self._update('/servers/%s/tags/%s' % (base.getid(server), tag), None)