...
class _TestListContainerBase(object):
    ...
    def test_pickle(self):
        ...
        pickled_clist = pickle.loads(pickle.dumps(clist, protocol=_pickle_test_protocol))
    ...
    def test_components(self):
        ...
        clistflattened.append(self._ctype_factory())
        clist.append(clistflattened[-1])
        self.assertEqual(list((id(_c) for _c in clist.components())), list((id(_c) for _c in clistflattened)))
        ...
        clistflattened.append(csublistflattened[-1])
        csublist.append(csublistflattened[-1])
        self.assertEqual(list((id(_c) for _c in csublist.components())), list((id(_c) for _c in csublistflattened)))
        self.assertEqual(len(set((id(_c) for _c in csublist.components()))), len(list((id(_c) for _c in csublist.components()))))
        self.assertEqual(len(set((id(_c) for _c in csublist.components()))), 3)
        clist.append(csublist)
        self.assertEqual(list((id(_c) for _c in clist.components())), list((id(_c) for _c in clistflattened)))
        self.assertEqual(len(set((id(_c) for _c in clist.components()))), len(list((id(_c) for _c in clist.components()))))
    ...
    def test_preorder_traversal(self):
        ...
        self.assertEqual([c.name for c in traversal], [c.name for c in pmo.preorder_traversal(clist, descend=descend)])
        self.assertEqual([id(c) for c in traversal], [id(c) for c in pmo.preorder_traversal(clist, descend=descend)])
