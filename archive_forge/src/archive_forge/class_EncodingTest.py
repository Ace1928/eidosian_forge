...
class EncodingTest(unittest.TestCase):
    ...
    def testBytesEncodingInAMap(self):
        data1 = b'\xf0\x11\x0f'
        data2 = b'\xff\xff\xff'
        msg = MapToBytesValue(additionalProperties=[MapToBytesValue.AdditionalProperty(key='1st', value=data1), MapToBytesValue.AdditionalProperty(key='2nd', value=data2)])
        self.assertEqual('{"1st": "%s", "2nd": "%s"}' % (base64.b64encode(data1, b'-_').decode('utf-8'), base64.b64encode(data2, b'-_').decode('utf-8')), encoding.MessageToJson(msg))
    ...
    def testNumericPropertyValue(self):
        ...
        self.assertEqual(AdditionalIntPropertiesMessage(additionalProperties=[AdditionalIntPropertiesMessage.AdditionalProperty(key='key_one', value=123)]), msg)
    ...
    def testRepeatedEnumRemapping(self):
        msg = MessageWithRemappings(repeated_enum=[MessageWithRemappings.SomeEnum.enum_value, MessageWithRemappings.SomeEnum.second_value])
        json_message = encoding.MessageToJson(msg)
        self.assertEqual('{"repeated_enum": ["wire_name", "second_value"]}', json_message)
        self.assertEqual(msg, encoding.JsonToMessage(MessageWithRemappings, json_message))
