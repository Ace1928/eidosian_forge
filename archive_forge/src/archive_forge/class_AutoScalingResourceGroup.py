...
class AutoScalingResourceGroup(aws_asg.AutoScalingGroup):
    ...
    def child_template_files(self, child_env):
        ...
        return grouputils.get_child_template_files(self.context, self.stack, is_update, self.old_template_id)

    def _try_rolling_update(self, prop_diff):
        if self.RESOURCE in prop_diff:
            policy = self.properties[self.ROLLING_UPDATES]
            self._replace(policy[self.MIN_IN_SERVICE], policy[self.MAX_BATCH_SIZE], policy[self.PAUSE_TIME])

    def _create_template(self, num_instances, num_replace=0, template_version=('heat_template_version', '2015-04-30')):
    ...
    def get_attribute(self, key, *path):
        ...
        try:
            output = self.get_output(self._attribute_output_name(op_key, *op_path))
            ...
            if keycomponents is not None:
                ...
                try:
                    ...
                    raise exception.NotFound(_("Member '%(mem)s' not found in group resource '%(grp)s'.") % {'mem': index, 'grp': self.name})
