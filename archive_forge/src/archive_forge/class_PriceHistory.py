...
class PriceHistory:
    ...
    def history(self, period='1mo', interval='1d', start=None, end=None, prepost=False, actions=True, auto_adjust=True, back_adjust=False, repair=False, keepna=False, proxy=None, rounding=False, timeout=10, raise_errors=False) -> pd.DataFrame:
        """
        :Parameters:
            period : str
        ...
        if data is None or not isinstance(data, dict):
            ...
            fail = True
        elif period is not None and 'timestamp' not in data['chart']['result'][0] and (period not in self._history_metadata['validRanges']):
        ...
        if interval.lower() == '30m':
            ...
            quotes = pd.DataFrame(index=quotes2.last().index, data={'Open': quotes2['Open'].first(), 'High': quotes2['High'].max(), 'Low': quotes2['Low'].min(), 'Close': quotes2['Close'].last(), 'Adj Close': quotes2['Adj Close'].last(), 'Volume': quotes2['Volume'].sum()})
            try:
