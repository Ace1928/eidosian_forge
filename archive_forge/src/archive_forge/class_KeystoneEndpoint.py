...
class KeystoneEndpoint(resource.Resource):
    ...
    entity = 'endpoints'
    PROPERTIES = NAME, REGION, SERVICE, INTERFACE, SERVICE_URL, ENABLED = ('name', 'region', 'service', 'interface', 'url', 'enabled')
    properties_schema = {NAME: properties.Schema(properties.Schema.STRING, _('Name of keystone endpoint.'), update_allowed=True), REGION: properties.Schema(properties.Schema.STRING, _('Name or Id of keystone region.'), update_allowed=True, constraints=[constraints.CustomConstraint('keystone.region')]), SERVICE: properties.Schema(properties.Schema.STRING, _('Name or Id of keystone service.'), update_allowed=True, required=True, constraints=[constraints.CustomConstraint('keystone.service')]), INTERFACE: properties.Schema(properties.Schema.STRING, _('Interface type of keystone service endpoint.'), update_allowed=True, required=True, constraints=[constraints.AllowedValues(['public', 'internal', 'admin'])]), SERVICE_URL: properties.Schema(properties.Schema.STRING, _('URL of keystone service endpoint.'), update_allowed=True, required=True), ENABLED: properties.Schema(properties.Schema.BOOLEAN, _('This endpoint is enabled or disabled.'), default=True, update_allowed=True, support_status=support.SupportStatus(version='6.0.0'))}

    def translation_rules(self, props):
        return [translation.TranslationRule(props, translation.TranslationRule.RESOLVE, [self.SERVICE], client_plugin=self.client_plugin(), finder='get_service_id'), translation.TranslationRule(props, translation.TranslationRule.RESOLVE, [self.REGION], client_plugin=self.client_plugin(), finder='get_region_id')]
    ...
    def handle_create(self):
        ...
        endpoint = self.client().endpoints.create(region=region, service=service, interface=interface, url=url, name=name, enabled=enabled)
    ...
    def handle_update(self, json_snippet, tmpl_diff, prop_diff):
        if prop_diff:
            ...
            self.client().endpoints.update(endpoint=self.resource_id, region=region, service=service, interface=interface, url=url, name=name, enabled=enabled)

    def parse_live_resource_data(self, resource_properties, resource_data):
        endpoint_reality = {}
        endpoint_reality.update({self.SERVICE: resource_data.get('service_id'), self.REGION: resource_data.get('region_id')})
