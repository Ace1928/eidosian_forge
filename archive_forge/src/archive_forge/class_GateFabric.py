...
class GateFabric(Operation):
    ...
    def __init__(self, weights, wires, init_state, include_pi=False, id=None):
        if len(wires) < 4:
            raise ValueError(f'This template requires the number of qubits to be greater than four; got wires {wires}')
        if len(wires) % 2:
            raise ValueError(f'This template requires an even number of qubits; got {len(wires)} wires')
        ...
        if shape[1] != len_wire_pattern:
            raise ValueError(f'Weights tensor must have second dimension of length {len_wire_pattern}; got {shape[1]}')
        if shape[2] != 2:
    ...
    def compute_decomposition(weights, wires, init_state, include_pi):
        ...
        **Example**
        ...
        if len(wires) > 4:
            wire_pattern += [wires[i:i + 4] for i in range(2, len(wires), 4) if len(wires[i:i + 4]) == 4]
