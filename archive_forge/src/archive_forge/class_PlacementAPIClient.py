...
class PlacementAPIClient(object):
    ...
    def __init__(self, conf, openstack_api_version=PLACEMENT_API_LATEST_SUPPORTED):
        self._openstack_api_version = openstack_api_version
        self._target_version = _get_version(openstack_api_version)
        self._conf = conf
        self._ks_filter = {'service_type': 'placement', 'region_name': self._conf.placement.region_name, 'interface': self._conf.placement.endpoint_type}
        self._api_version_header = {API_VERSION_REQUEST_HEADER: self._openstack_api_version}
        self._client = None
    ...
    def _create_client(self):
        ...
        if self._conf.placement.auth_type == 'noauth':
            ...
            auth_plugin = keystone.load_auth_from_conf_options(self._conf, 'placement')
            return keystone.load_session_from_conf_options(self._conf, 'placement', auth=auth_plugin, additional_headers={'accept': 'application/json'})
    ...
    def _get(self, url, **kwargs):
        ...
        return self._client.get(url, endpoint_filter=self._ks_filter, **kwargs)

    def _post(self, url, data, **kwargs):
        kwargs = self._extend_header_with_api_version(**kwargs)
        return self._client.post(url, json=data, endpoint_filter=self._ks_filter, **kwargs)

    def _put(self, url, data, **kwargs):
        kwargs = self._extend_header_with_api_version(**kwargs)
        return self._client.put(url, json=data, endpoint_filter=self._ks_filter, **kwargs)
    ...
    def _delete(self, url, **kwargs):
        kwargs = self._extend_header_with_api_version(**kwargs)
        return self._client.delete(url, endpoint_filter=self._ks_filter, **kwargs)
    ...
    def create_resource_provider(self, resource_provider):
        ...
        if self._target_version < _get_version(PLACEMENT_API_RETURN_PROVIDER_BODY):
            return
        else:
            return rsp.json()
    ...
    def update_resource_provider(self, resource_provider):
        ...
        try:
            ...
            raise n_exc.PlacementResourceProviderNotFound(resource_provider=resource_provider['uuid'])
        except ks_exc.Conflict:
            raise n_exc.PlacementResourceProviderNameNotUnique(name=resource_provider['name'])
    ...
    @_check_placement_api_available
    def update_resource_provider_inventories(self, resource_provider_uuid, inventories, resource_provider_generation=None):
        ...
        body = {'resource_provider_generation': resource_provider_generation, 'inventories': inventories}
        try:
    ...
    @_check_placement_api_available
    def delete_resource_provider_inventory(self, resource_provider_uuid, resource_class):
        ...
        :returns: None.
        """
        url = '/resource_providers/%s/inventories/%s' % (resource_provider_uuid, resource_class)
        try:
            self._delete(url)
        except ks_exc.NotFound as e:
            if 'No resource provider with uuid' in e.details:
            ...
            if 'No inventory of class' in e.details:
                raise n_exc.PlacementInventoryNotFound(resource_provider=resource_provider_uuid, resource_class=resource_class)
            raise
    ...
    @_check_placement_api_available
    def update_resource_provider_inventory(self, resource_provider_uuid, inventory, resource_class, resource_provider_generation=None):
    ...
    def list_aggregates(self, resource_provider_uuid):
        ...
        try:
            ...
            raise n_exc.PlacementAggregateNotFound(resource_provider=resource_provider_uuid)
