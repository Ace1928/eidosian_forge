...
class CallProtocolEntity(ProtocolEntity):
    ...
    def toProtocolTreeNode(self):
        children = []
        attribs = {'t': str(self.timestamp), 'offline': '1' if self.offline else '0', 'id': self._id}
    ...
    def fromProtocolTreeNode(node):
        ...
        return CallProtocolEntity(node.getAttributeValue('id'), _type, node.getAttributeValue('t'), node.getAttributeValue('notify'), node.getAttributeValue('offline'), node.getAttributeValue('retry'), node.getAttributeValue('e'), callId, node.getAttributeValue('from'), node.getAttributeValue('to'))