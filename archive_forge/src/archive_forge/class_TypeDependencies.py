...
class TypeDependencies(ModuleAnalysis):
    ...
    def visit_Return(self, node):
        ...
        for dep_set in self.visit(node.value):
            if dep_set:
                ...
                self.result.add_edge(TypeDependencies.NoDeps, self.current_function)
    ...
    def visit_For(self, node):
        ...
        if node.target.id in self.naming:
            body = [ast.Assign(targets=[node.target], value=node.iter, type_comment=None)] + body
    ...
    def visit_Name(self, node):
        ...
        if node.id in self.naming:
            ...
            raise PythranInternalError("Variable '{}' used before assignment".format(node.id))
