...
class CmdStanVB:
    ...
    def __init__(self, runset: RunSet) -> None:
        ...
        if not runset.method == Method.VARIATIONAL:
            raise ValueError('Wrong runset method, expecting variational inference, found method {}'.format(runset.method))
        self.runset = runset
        self._set_variational_attrs(runset.csv_files[0])

    def __repr__(self) -> str:
        repr = 'CmdStanVB: model={}{}'.format(self.runset.model, self.runset._args.method_args.compose(0, cmd=[]))
    ...
    def stan_variable(self, var: str, *, mean: Optional[bool]=None) -> Union[np.ndarray, float]:
        ...
        * If the variable is an array with N dimensions, the return array
        ...
        """
        if mean is None:
            get_logger().warning('The default behavior of CmdStanVB.stan_variable() will change in a future release to return the variational sample, rather than the mean.\nTo maintain the current behavior, pass the argument mean=True')
            mean = True
        if mean:
        ...
        try:
            ...
            if out.shape == () or out.shape == (1,):
                if mean:
                    get_logger().warning('The default behavior of CmdStanVB.stan_variable(mean=True) will change in a future release to always return a numpy.ndarray, even for scalar variables.')
                return out.item()
            return out
        except KeyError:
