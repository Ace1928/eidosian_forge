...
class AzureHandler(StorageHandler):
    ...
    def can_handle(self, parsed_url: 'ParseResult') -> bool:
        return parsed_url.scheme == 'https' and parsed_url.netloc.endswith('.blob.core.windows.net')

    def load_path(self, manifest_entry: 'ArtifactManifestEntry', local: bool=False) -> Union[URIStr, FilePathStr]:
        assert manifest_entry.ref is not None
        if not local:
            return manifest_entry.ref
        path, hit, cache_open = get_artifact_file_cache().check_etag_obj_path(URIStr(manifest_entry.ref), ETag(manifest_entry.digest), manifest_entry.size or 0)
        if hit:
            return path
    ...
    def store_path(self, artifact: 'Artifact', path: Union[URIStr, FilePathStr], name: Optional[StrPath]=None, checksum: bool=True, max_objects: Optional[int]=None) -> Sequence['ArtifactManifestEntry']:
        ...
        for i, blob_properties in enumerate(container_client.list_blobs(name_starts_with=f'{blob_name}/')):
            if i >= max_objects:
