...
class TestAutomaticConversion(unittest.TestCase):
    ...
    def _test_model_conversion(self, to_opset: int, model: str | onnx.ModelProto) -> None:
    ...
    def _test_op_conversion(self, op: str, from_opset: int, input_shapes: Sequence[Sequence[int | None] | str]=((3, 4, 5),), output_shapes: Sequence[Sequence[int | None]]=((3, 4, 5),), input_types: Sequence[Any] | None=None, output_types: Sequence[Any] | None=None, initializer: Sequence[Any]=(), attrs: dict[str, Any] | None=None, seq_inputs: Sequence[int]=(), seq_outputs: Sequence[int]=(), optional_inputs: Sequence[int]=(), optional_outputs: Sequence[int]=(), is_upgrade: bool=True) -> None:
        """Test conversion.
        ...
        input_names = [letter if shape != '' else '' for letter, shape in zip(letters, input_shapes)]
        ...
        input_shapes_cast = cast(List[List[int]], [[0] if isinstance(shape, str) else shape for shape in input_shapes])
        inputs: list[ValueInfoProto] = []
        for name, ttype, shape, is_seq, is_opt in zip(input_names, input_types, input_shapes_cast, is_sequence, is_optional):
            if name != '':
                if is_seq:
        ...
        output_shapes_cast = cast(List[List[int]], [[0] if isinstance(shape, str) else shape for shape in output_shapes])
        outputs: list[ValueInfoProto] = []
        for name, ttype, shape, is_seq, is_opt in zip(output_names, output_types, output_shapes_cast, is_sequence, is_optional):
            if is_seq:
        ...
        original = helper.make_model(graph, producer_name='test', opset_imports=[helper.make_opsetid('', start_opset)])
