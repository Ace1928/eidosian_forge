...
class VolumeTestCase(common.HeatTestCase):
    ...
    def get_volume(self, t, stack, resource_name):
        ...
        vol = Volume(resource_name, stack.defn.resource_definition(resource_name), stack)
        return vol

    def create_volume(self, t, stack, resource_name, az='nova', validate=True, no_create=False):
        ...
        if validate:
            if no_create:
                ...
                self.cinder_fc.volumes.create.assert_called_once_with(size=1, availability_zone=az, description=self.vol_name, name=self.vol_name, metadata={u'Usage': u'Wiki Data Volume'})
        return rsrc
    ...
    def create_attachment(self, t, stack, resource_name):
        ...
        rsrc = Attachment(resource_name, stack.defn.resource_definition(resource_name), stack)
        self.assertIsNone(rsrc.validate())
        scheduler.TaskRunner(rsrc.create)()
        self.assertEqual((rsrc.CREATE, rsrc.COMPLETE), rsrc.state)
        stk_defn.update_resource_data(stack.defn, resource_name, rsrc.node_data())
