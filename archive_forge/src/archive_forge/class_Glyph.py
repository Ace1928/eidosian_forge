...
class Glyph:
    ...
    def __repr__(self) -> str:
        return '<{}.{} {}at {}>'.format(self.__class__.__module__, self.__class__.__name__, f"'{self._name}' " if self._name is not None else '', hex(id(self)))
    ...
    def image(self, image: Image | Mapping[str, Any] | None) -> None:
        if image is None:
            ...
            self._image = Image(fileName=image['fileName'], transformation=Transform(image['xScale'], image['xyScale'], image['yxScale'], image['yScale'], image['xOffset'], image['yOffset']), color=image.get('color'))
