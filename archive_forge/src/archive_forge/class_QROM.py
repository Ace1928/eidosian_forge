...
class QROM(unary_iteration_gate.UnaryIterationGate):
    ...
    def build(cls, *data: ArrayLike, num_controls: int=0) -> 'QROM':
        ...
        return QROM(data=_data, selection_bitsizes=selection_bitsizes, target_bitsizes=target_bitsizes, num_controls=num_controls)
    ...
    def __attrs_post_init__(self):
        ...
        assert len(self.target_bitsizes) == len(self.data), f'len(self.target_bitsizes)={len(self.target_bitsizes)} should be same as len(self.data)={len(self.data)}'
        assert all((t >= int(np.max(d)).bit_length() for t, d in zip(self.target_bitsizes, self.data)))
    ...
    def selection_registers(self) -> Tuple[infra.SelectionRegister, ...]:
        if len(self.data[0].shape) == 1:
            return (infra.SelectionRegister('selection', self.selection_bitsizes[0], self.data[0].shape[0]),)
        else:
            return tuple((infra.SelectionRegister(f'selection{i}', sb, l) for i, (l, sb) in enumerate(zip(self.data[0].shape, self.selection_bitsizes))))

    @cached_property
    def target_registers(self) -> Tuple[infra.Register, ...]:
        return tuple((infra.Register(f'target{i}', l) for i, l in enumerate(self.target_bitsizes) if l))
    ...
    def __repr__(self) -> str:
        ...
        return f'cirq_ft.QROM({data_repr}, selection_bitsizes={selection_repr}, target_bitsizes={target_repr}, num_controls={self.num_controls})'
