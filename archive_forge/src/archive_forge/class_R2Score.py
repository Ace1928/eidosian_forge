...
class R2Score(base_metric.Metric):
    ...
    def build(self, y_true_shape, y_pred_shape):
        ...
        self.squared_sum = self.add_weight(name='squared_sum', shape=[num_classes], initializer='zeros')
        self.sum = self.add_weight(name='sum', shape=[num_classes], initializer='zeros')
        self.total_mse = self.add_weight(name='residual', shape=[num_classes], initializer='zeros')
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):
        ...
        sample_weight = tf.__internal__.ops.broadcast_weights(weights=sample_weight, values=y_true)
        ...
        self.total_mse.assign_add(tf.reduce_sum((y_true - y_pred) ** 2 * sample_weight, axis=0))
    ...
    def get_config(self):
        config = {'class_aggregation': self.class_aggregation, 'num_regressors': self.num_regressors}
