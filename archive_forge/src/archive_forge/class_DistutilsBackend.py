...
class DistutilsBackend(Backend):
    ...
    def __init__(sef, *args, **kwargs):
        warnings.warn('distutils has been deprecated since NumPy 1.26.xUse the Meson backend instead, or generate wrapperswithout -c and use a custom build script', VisibleDeprecationWarning, stacklevel=2)
    ...
    def compile(self):
        ...
        if self.sysinfo_flags:
            for n in self.sysinfo_flags:
                i = get_info(n)
                if not i:
                    print(f'No {repr(n)} resources foundin system (try `f2py --help-link`)')
