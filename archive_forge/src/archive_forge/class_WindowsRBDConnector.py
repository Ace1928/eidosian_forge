...
class WindowsRBDConnector(base_rbd.RBDConnectorMixin, win_conn_base.BaseWindowsConnector):
    ...
    def get_device_name(self, connection_properties, expect=True):
        ...
        if mapping:
            ...
            LOG.debug('Located RBD mapping: %(image)s. Disk number: %(disk_number)s.', dict(image=connection_properties['name'], disk_number=dev_num))
    ...
    def connect_volume(self, connection_properties):
        ...
        if not rbd_dev_path:
            ...
            LOG.debug('The RBD image %(image)s is already mapped to local device %(dev)s', {'image': connection_properties['name'], 'dev': rbd_dev_path})
        dev_num = self._diskutils.get_device_number_from_device_name(rbd_dev_path)
