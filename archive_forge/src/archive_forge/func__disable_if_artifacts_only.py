...
def _disable_if_artifacts_only(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        if os.environ.get(ARTIFACTS_ONLY_ENV_VAR):
            return Response(f'Endpoint: {request.url_rule} disabled due to the mlflow server running in `--artifacts-only` mode. To enable tracking server functionality, run `mlflow server` without `--artifacts-only`', 503)
