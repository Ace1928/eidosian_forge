...
@SolverFactory.register('gdpopt.enumerate', doc='Generalized Disjunctive Programming (GDP) solver that enumerates all discrete solutions')
class GDP_Enumeration_Solver(_GDPoptAlgorithm):
    ...
    _add_nlp_solve_configs(CONFIG, default_nlp_init_method=restore_vars_to_original_values_enumerate)
    ...
    def _solve_gdp(self, original_model, config):
        ...
        discrete_solns = list(self._discrete_solution_iterator(subproblem_util_block.disjunction_list, subproblem_util_block.non_indicator_boolean_variable_list, subproblem_util_block.discrete_variable_list, config))
        ...
        for soln in discrete_solns:
            ...
            with time_code(self.timing, 'nlp'):
                with fix_discrete_solution_in_subproblem(*soln, subproblem_util_block, config, self):
                    nlp_termination = solve_subproblem(subproblem_util_block, self, config)
                    if nlp_termination in {tc.optimal, tc.feasible}:
                        primal_improved = self._update_bounds_after_solve('subproblem', primal=value(subproblem_util_block.obj.expr), logger=config.logger)
                        if primal_improved:
            ...
            if self.iteration == self.num_discrete_solns:
                if self.incumbent_boolean_soln is None:
                    ...
                    config.logger.info('GDPopt exiting--all discrete solutions have been enumerated.')
