...
class XsdGlobals(XsdValidator):
    ...
    _lookup_function_resolver = {XSD_SIMPLE_TYPE: 'lookup_type', XSD_COMPLEX_TYPE: 'lookup_type', XSD_ELEMENT: 'lookup_element', XSD_GROUP: 'lookup_group', XSD_ATTRIBUTE: 'lookup_attribute', XSD_ATTRIBUTE_GROUP: 'lookup_attribute_group', XSD_NOTATION: 'lookup_notation'}
    ...
    def __init__(self, validator: SchemaType, validation: str='strict') -> None:
        ...
        self.identities = {}
        self.global_maps = (self.notations, self.types, self.attributes, self.attribute_groups, self.groups, self.elements)
        self._builders: Dict[str, Callable[[ElementType, SchemaType], Any]] = {XSD_NOTATION: validator.xsd_notation_class, XSD_SIMPLE_TYPE: validator.simple_type_factory, XSD_COMPLEX_TYPE: validator.xsd_complex_type_class, XSD_ATTRIBUTE: validator.xsd_attribute_class, XSD_ATTRIBUTE_GROUP: validator.xsd_attribute_group_class, XSD_GROUP: validator.xsd_group_class, XSD_ELEMENT: validator.xsd_element_class}
    ...
    def copy(self, validator: Optional[SchemaType]=None, validation: Optional[str]=None) -> 'XsdGlobals':
        """Makes a copy of the object."""
        obj = self.__class__(validator=self.validator if validator is None else validator, validation=validation or self.validation)
    ...
    def lookup(self, tag: str, qname: str) -> SchemaGlobalType:
        ...
        :param tag: the expanded QName of the XSD the global declaration/definition         (eg. '{http://www.w3.org/2001/XMLSchema}element'), that is used to select         the global map for lookup.
    ...
    def get_instance_type(self, type_name: str, base_type: BaseXsdType, namespaces: MutableMapping[str, str]) -> BaseXsdType:
        """
        ...
        if xsi_type.is_derived(base_type):
            return xsi_type
        elif isinstance(base_type, XsdSimpleType) and base_type.is_union() and (not base_type.facets):
            if isinstance(base_type, XsdAtomicRestriction) and isinstance(base_type.primitive_type, XsdUnion):
    ...
    def iter_components(self, xsd_classes: ComponentClassType=None) -> Iterator[Union['XsdGlobals', XsdComponent]]:
    ...
    def register(self, schema: SchemaType) -> None:
        ...
        try:
            ...
            elif not any((schema.url == obj.url and schema.__class__ is obj.__class__ for obj in ns_schemas)):
                ns_schemas.append(schema)
    ...
    def build(self) -> None:
        ...
        elif not self.types and meta_schema.maps is not self:
        ...
        for schema in not_built_schemas:
            ...
            try:
                ...
                schema.parse_error(error=msg.format(schema.root.get('defaultAttributes'), schema), elem=schema.root, validation=schema.validation)
            else:
    ...
    def check(self, schemas: Optional[Iterable[SchemaType]]=None, validation: str='strict') -> None:
        """
        ...
        if validation == 'strict' and (not self.built):
            raise XMLSchemaNotBuiltError(self, _('global map has unbuilt components: %r') % self.unbuilt)
        for group in self.groups.values():
            ...
            while group.redefine is not None:
                if not any((isinstance(e, XsdGroup) and e.name == group.name for e in group)) and (not group.is_restriction(group.redefine)):
        ...
        for xsd_global in filter(lambda x: x.schema in _schemas, self.iter_globals()):
            ...
            for xsd_type in xsd_global.iter_components(XsdComplexType):
                ...
                if xsd_type.derivation == 'restriction':
                    ...
                    if base_type.is_complex() and (not base_type.open_content) and xsd_type.open_content and (xsd_type.open_content.mode != 'none'):
                        _group = xsd_type.schema.create_any_content_group(parent=xsd_type, any_element=xsd_type.open_content.any_element)
                ...
                try:
                    ...
                    msg = _("can't verify the content model of {!r} due to exceeding of maximum recursion depth")
