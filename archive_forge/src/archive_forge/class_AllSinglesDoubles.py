...
class AllSinglesDoubles(Operation):
    ...
    def __init__(self, weights, wires, hf_state, singles=None, doubles=None, id=None):
        if len(wires) < 2:
            raise ValueError(f'The number of qubits (wires) can not be less than 2; got len(wires) = {len(wires)}')
        if doubles is not None:
            for d_wires in doubles:
                if len(d_wires) != 4:
                    raise ValueError(f"Expected entries of 'doubles' to be of size 4; got {d_wires} of length {len(d_wires)}")
        if singles is not None:
            for s_wires in singles:
                if len(s_wires) != 2:
                    raise ValueError(f"Expected entries of 'singles' to be of size 2; got {s_wires} of length {len(s_wires)}")
        ...
        self._hyperparameters = {'hf_state': tuple(hf_state), 'singles': singles, 'doubles': doubles}
