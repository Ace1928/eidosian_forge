...
class BarbicanKeyManagerTestCase(test_key_manager.KeyManagerTestCase):
    ...
    def setUp(self):
        ...
        self.hex = '0080f1429dbefae01b29a4d50cc5c5608bbc3c8ba0246aa42b424baa4534ae16'
    ...
    def test_barbican_endpoint_with_endpoint_type(self):
        ...
        endpoint = self.key_mgr._get_barbican_endpoint(auth, mock.Mock())
        self.assertEqual(endpoint, 'http://localhost:9311')
        auth.service_catalog.endpoint_data_for.assert_called_once_with(service_type='key-manager', interface='internal', region_name=None)
    ...
    def test_barbican_endpoint_with_region_name(self):
        ...
        endpoint = self.key_mgr._get_barbican_endpoint(auth, mock.Mock())
        self.assertEqual(endpoint, 'http://localhost:9311')
        auth.service_catalog.endpoint_data_for.assert_called_once_with(service_type='key-manager', interface='public', region_name='regionOne')
    ...
    def test_barbican_endpoint_by_get_endpoint_with_endpoint_type(self):
        ...
        endpoint = self.key_mgr._get_barbican_endpoint(auth, sess)
        self.assertEqual(endpoint, 'http://localhost:9311')
        auth.get_endpoint.assert_called_once_with(sess, service_type='key-manager', interface='internal', region_name=None)
    ...
    def test_barbican_endpoint_by_get_endpoint_with_region_name(self):
        ...
        endpoint = self.key_mgr._get_barbican_endpoint(auth, sess)
        self.assertEqual(endpoint, 'http://localhost:9311')
        auth.get_endpoint.assert_called_once_with(sess, service_type='key-manager', interface='public', region_name='regionOne')
    ...
    def test_create_key(self):
        ...
        returned_uuid = self.key_mgr.create_key(self.ctxt, algorithm='AES', length=256)
    ...
    def test_create_key_with_error(self):
        key_order = mock.Mock()
        self.mock_barbican.orders.create_key.return_value = key_order
        key_order.submit = mock.Mock(side_effect=barbican_exceptions.HTTPClientError('test error'))
        self.assertRaises(exception.KeyManagerError, self.key_mgr.create_key, self.ctxt, 'AES', 256)

    def test_create_key_pair(self):
        order_ref_url = 'http://localhost:9311/v1/orders/f45bf211-a917-4ead-9aec-1c91e52609df'
        ...
        container.secret_refs = {'public_key': pub_key_ref, 'private_key': priv_key_ref}
        self.mock_barbican.containers.get.return_value = container
        returned_private_uuid, returned_public_uuid = self.key_mgr.create_key_pair(self.ctxt, algorithm='RSA', length=2048)
        self.mock_barbican.orders.get.assert_called_once_with(order_ref_url)
        self.mock_barbican.containers.get.assert_called_once_with(container_ref)
        self.mock_barbican.orders.get.assert_called_once_with(order_ref_url)
    ...
    def test_create_key_pair_null_context(self):
        self.key_mgr._barbican_client = None
        self.assertRaises(exception.Forbidden, self.key_mgr.create_key_pair, None, 'RSA', 2048)
    ...
    def test_delete_null_context(self):
        self.key_mgr._barbican_client = None
        self.assertRaises(exception.Forbidden, self.key_mgr.delete, None, self.key_id)
    ...
    def test_delete_secret_with_consumers_no_force_parameter(self):
        self.mock_barbican.secrets.delete = mock.Mock(side_effect=exception.KeyManagerError("Secret has consumers! Use the 'force' parameter."))
        self.assertRaises(exception.KeyManagerError, self.key_mgr.delete, self.ctxt, self.key_id)
    ...
    def test_delete_unknown_key(self):
        self.assertRaises(exception.KeyManagerError, self.key_mgr.delete, self.ctxt, None)

    def test_delete_with_error(self):
        self.mock_barbican.secrets.delete = mock.Mock(side_effect=barbican_exceptions.HTTPClientError('test error'))
        self.assertRaises(exception.KeyManagerError, self.key_mgr.delete, self.ctxt, self.key_id)
    ...
    def test_get_null_context(self):
        self.key_mgr._barbican_client = None
        self.assertRaises(exception.Forbidden, self.key_mgr.get, None, self.key_id)

    def test_get_unknown_key(self):
        self.assertRaises(exception.KeyManagerError, self.key_mgr.get, self.ctxt, None)

    def test_get_with_error(self):
        self.mock_barbican.secrets.get = mock.Mock(side_effect=barbican_exceptions.HTTPClientError('test error'))
        self.assertRaises(exception.KeyManagerError, self.key_mgr.get, self.ctxt, self.key_id)
    ...
    def test_get_active_order(self):
        ...
        self.mock_barbican.orders.get.side_effect = [pending_order, active_order]
    ...
    def test_list(self):
        ...
        original_secret_data = b'test key'
        original_secret_metadata.payload = original_secret_data
        self.mock_barbican.secrets.list.return_value = [original_secret_metadata]
    ...
    def test_list_with_invalid_object_type(self):
        self.assertRaises(exception.KeyManagerError, self.key_mgr.list, self.ctxt, 'invalid_type')
    ...
    def _mock_list_versions_and_test_consumer_expects_error(self, Error, method, ctxt, obj_ref, service='storage', resource_type='volume', resource_id=uuidutils.generate_uuid()):
        self._mock_list_versions()
        self._test_consumer_expects_error(Error, method, ctxt, obj_ref, service=service, resource_type=resource_type, resource_id=resource_id)

    def _mock_list_versions_and_test_add_consumer_expects_error(self, Error, ctxt, obj_ref, side_effect=None, service='storage', resource_type='volume', resource_id=uuidutils.generate_uuid()):
        self.mock_barbican.secrets.register_consumer = mock.Mock(side_effect=side_effect)
        self._mock_list_versions_and_test_consumer_expects_error(Error, self.key_mgr.add_consumer, ctxt, obj_ref, service=service, resource_type=resource_type, resource_id=resource_id)
    ...
    def test_add_consumer_without_context_fails(self):
        self.key_mgr._barbican_client = None
        self._test_consumer_expects_error(exception.Forbidden, self.key_mgr.add_consumer, None, self.secret_ref)

    def test_add_consumer_with_different_project_fails(self):
        side_effect = barbican_exceptions.HTTPClientError('Forbidden: SecretConsumer creation attempt not allowed - please review your user/project privileges')
        self._mock_list_versions_and_test_add_consumer_expects_error(exception.KeyManagerError, self.ctxt, self.secret_ref, side_effect=side_effect)
    ...
    def test_add_consumer_with_empty_managed_object_id_fails(self):
        self._mock_list_versions_and_test_add_consumer_expects_error(exception.KeyManagerError, self.ctxt, '')
    ...
    def test_add_consumer_with_null_service_fails(self):
        side_effect = barbican_exceptions.HTTPClientError("Bad Request: Provided object does not match schema 'Secret Consumer': None is not of type 'string'. Invalid property: 'service'", status_code=400)
        self._mock_list_versions_and_test_add_consumer_expects_error(exception.KeyManagerError, self.ctxt, self.secret_ref, side_effect=side_effect, service=None)

    def test_add_consumer_with_empty_service_fails(self):
        side_effect = barbican_exceptions.HTTPClientError("Bad Request: Provided object does not match schema 'Secret Consumer': '' is too short. Invalid property: 'service'", status_code=400)
        self._mock_list_versions_and_test_add_consumer_expects_error(exception.KeyManagerError, self.ctxt, self.secret_ref, side_effect=side_effect, service='')

    def test_add_consumer_with_null_resource_type_fails(self):
        side_effect = barbican_exceptions.HTTPClientError("Bad Request: Provided object does not match schema 'Secret Consumer': None is not of type 'string'. Invalid property: 'resource_type'", status_code=400)
        self._mock_list_versions_and_test_add_consumer_expects_error(exception.KeyManagerError, self.ctxt, self.secret_ref, side_effect=side_effect, resource_type=None)
    ...
    def test_add_consumer_with_null_resource_id_fails(self):
        side_effect = barbican_exceptions.HTTPClientError("Bad Request: Provided object does not match schema 'Secret Consumer': None is not of type 'string'. Invalid property: 'resource_id'", status_code=400)
        self._mock_list_versions_and_test_add_consumer_expects_error(exception.KeyManagerError, self.ctxt, self.secret_ref, side_effect=side_effect, resource_id=None)

    def test_add_consumer_with_empty_resource_id_fails(self):
        side_effect = barbican_exceptions.HTTPClientError("Bad Request: Provided object does not match schema 'Secret Consumer': '' is too short. Invalid property: 'resource_id'", status_code=400)
        self._mock_list_versions_and_test_add_consumer_expects_error(exception.KeyManagerError, self.ctxt, self.secret_ref, side_effect=side_effect, resource_id='')
    ...
    def test_remove_consumer_with_different_project_fails(self):
        side_effect = barbican_exceptions.HTTPClientError('Forbidden: SecretConsumer creation attempt not allowed - please review your user/project privileges')
        self._mock_list_versions_and_test_remove_consumer_expects_error(exception.KeyManagerError, self.ctxt, self.secret_ref, side_effect=side_effect)

    def test_remove_consumer_with_null_managed_object_id_fails(self):
        side_effect = ValueError('secret incorrectly specified.')
        self._mock_list_versions_and_test_add_consumer_expects_error(exception.KeyManagerError, self.ctxt, None, side_effect=side_effect)

    def test_remove_consumer_with_empty_managed_object_id_fails(self):
        side_effect = ValueError('secret incorrectly specified.')
        self._mock_list_versions_and_test_add_consumer_expects_error(exception.KeyManagerError, self.ctxt, '', side_effect=side_effect)
    ...
    def test_remove_consumer_with_valid_parameters_doesnt_fail(self):
        self._mock_list_versions()
        self.key_mgr.remove_consumer(self.ctxt, self.secret_ref, self._get_custom_consumer_data())