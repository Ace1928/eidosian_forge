...
class LinuxFCTestCase(base.TestCase):
    ...
    @ddt.data(True, False)
    def test__get_hba_channel_scsi_target_lun_single_wwpn(self, remote_scan, mock_open):
        execute_results, expected_cmds = self._get_expected_info(remote_scan=remote_scan)
        if remote_scan:
    ...
    @mock.patch('builtins.open')
    @ddt.data(True, False)
    def test__get_hba_channel_scsi_target_lun_with_initiator_target_map_none(self, remote_scan, mock_open):
    ...
    @mock.patch('builtins.open')
    @ddt.data(True, False)
    def test__get_hba_channel_scsi_target_lun_multiple_wwpn_and_luns(self, remote_scan, mock_open):
        execute_results, expected_cmds = self._get_expected_info(targets=2)
        if remote_scan:
            mock_open = mock_open.return_value.__enter__.return_value
            mock_open.read.return_value = '1\n'
        ...
        con_props['targets'] = [('514f0c50023f6c00', 1), ('514f0c50023f6c01', 7)]
        with mock.patch.object(self.lfc, '_execute', side_effect=execute_results) as execute_mock:
            res = self.lfc._get_hba_channel_scsi_target_lun(hbas[0], con_props)
            execute_mock.assert_has_calls(expected_cmds)
    ...
    @mock.patch('builtins.open')
    @ddt.data(True, False)
    def test__get_hba_channel_scsi_target_lun_zone_manager(self, remote_scan, mock_open):
        execute_results, expected_cmds = self._get_expected_info()
        if remote_scan:
            mock_open = mock_open.return_value.__enter__.return_value
            mock_open.read.return_value = '1\n'
    ...
    @mock.patch('builtins.open')
    def test__get_hba_channel_scsi_target_lun_fc_remote_ports_exception(self, mock_open):
        execute_effects = [('', ''), ('/sys/class/fc_remote_ports/rport-6:0-1/port_name\n', ''), ('', ''), Exception()]
        mock_open = mock_open.return_value.__enter__.return_value
        mock_open.read.return_value = '1\n'
        _, expected_cmds = self._get_expected_info()
        hbas, con_props = self.__get_rescan_info()
