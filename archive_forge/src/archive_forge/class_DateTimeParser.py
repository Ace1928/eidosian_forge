...
class DateTimeParser:
    _FORMAT_RE: ClassVar[Pattern[str]] = re.compile('(YYY?Y?|MM?M?M?|Do|DD?D?D?|d?d?d?d|HH?|hh?|mm?|ss?|S+|ZZ?Z?|a|A|x|X|W)')
    _ESCAPE_RE: ClassVar[Pattern[str]] = re.compile('\\[[^\\[\\]]*\\]')
    _ONE_OR_TWO_DIGIT_RE: ClassVar[Pattern[str]] = re.compile('\\d{1,2}')
    _ONE_OR_TWO_OR_THREE_DIGIT_RE: ClassVar[Pattern[str]] = re.compile('\\d{1,3}')
    _ONE_OR_MORE_DIGIT_RE: ClassVar[Pattern[str]] = re.compile('\\d+')
    _TWO_DIGIT_RE: ClassVar[Pattern[str]] = re.compile('\\d{2}')
    _THREE_DIGIT_RE: ClassVar[Pattern[str]] = re.compile('\\d{3}')
    _FOUR_DIGIT_RE: ClassVar[Pattern[str]] = re.compile('\\d{4}')
    _TZ_Z_RE: ClassVar[Pattern[str]] = re.compile('([\\+\\-])(\\d{2})(?:(\\d{2}))?|Z')
    _TZ_ZZ_RE: ClassVar[Pattern[str]] = re.compile('([\\+\\-])(\\d{2})(?:\\:(\\d{2}))?|Z')
    _TZ_NAME_RE: ClassVar[Pattern[str]] = re.compile('\\w[\\w+\\-/]+')
    _TIMESTAMP_RE: ClassVar[Pattern[str]] = re.compile('^\\-?\\d+\\.?\\d+$')
    _TIMESTAMP_EXPANDED_RE: ClassVar[Pattern[str]] = re.compile('^\\-?\\d+$')
    _TIME_RE: ClassVar[Pattern[str]] = re.compile('^(\\d{2})(?:\\:?(\\d{2}))?(?:\\:?(\\d{2}))?(?:([\\.\\,])(\\d+))?$')
    _WEEK_DATE_RE: ClassVar[Pattern[str]] = re.compile('(?P<year>\\d{4})[\\-]?W(?P<week>\\d{2})[\\-]?(?P<day>\\d)?')
    _BASE_INPUT_RE_MAP: ClassVar[Dict[_FORMAT_TYPE, Pattern[str]]] = {'YYYY': _FOUR_DIGIT_RE, 'YY': _TWO_DIGIT_RE, 'MM': _TWO_DIGIT_RE, 'M': _ONE_OR_TWO_DIGIT_RE, 'DDDD': _THREE_DIGIT_RE, 'DDD': _ONE_OR_TWO_OR_THREE_DIGIT_RE, 'DD': _TWO_DIGIT_RE, 'D': _ONE_OR_TWO_DIGIT_RE, 'HH': _TWO_DIGIT_RE, 'H': _ONE_OR_TWO_DIGIT_RE, 'hh': _TWO_DIGIT_RE, 'h': _ONE_OR_TWO_DIGIT_RE, 'mm': _TWO_DIGIT_RE, 'm': _ONE_OR_TWO_DIGIT_RE, 'ss': _TWO_DIGIT_RE, 's': _ONE_OR_TWO_DIGIT_RE, 'X': _TIMESTAMP_RE, 'x': _TIMESTAMP_EXPANDED_RE, 'ZZZ': _TZ_NAME_RE, 'ZZ': _TZ_ZZ_RE, 'Z': _TZ_Z_RE, 'S': _ONE_OR_MORE_DIGIT_RE, 'W': _WEEK_DATE_RE}
    ...
    def __init__(self, locale: str=DEFAULT_LOCALE, cache_size: int=0) -> None:
        ...
        self._input_re_map.update({'MMMM': self._generate_choice_re(self.locale.month_names[1:], re.IGNORECASE), 'MMM': self._generate_choice_re(self.locale.month_abbreviations[1:], re.IGNORECASE), 'Do': re.compile(self.locale.ordinal_day_re), 'dddd': self._generate_choice_re(self.locale.day_names[1:], re.IGNORECASE), 'ddd': self._generate_choice_re(self.locale.day_abbreviations[1:], re.IGNORECASE), 'd': re.compile('[1-7]'), 'a': self._generate_choice_re((self.locale.meridians['am'], self.locale.meridians['pm'])), 'A': self._generate_choice_re(self.locale.meridians.values())})
        if cache_size > 0:
            self._generate_pattern_re = lru_cache(maxsize=cache_size)(self._generate_pattern_re)
    ...
    def parse(self, datetime_string: str, fmt: Union[List[str], str], normalize_whitespace: bool=False) -> datetime:
        if normalize_whitespace:
            datetime_string = re.sub('\\s+', ' ', datetime_string)
    ...
    @overload
    def _parse_token(self, token: Literal['YYYY', 'YY', 'MM', 'M', 'DDDD', 'DDD', 'DD', 'D', 'Do', 'HH', 'hh', 'h', 'H', 'mm', 'm', 'ss', 's', 'x'], value: Union[str, bytes, SupportsInt, bytearray], parts: _Parts) -> None:
        ...

    @overload
    def _parse_token(self, token: Literal['X'], value: Union[str, bytes, SupportsFloat, bytearray], parts: _Parts) -> None:
        ...

    @overload
    def _parse_token(self, token: Literal['MMMM', 'MMM', 'dddd', 'ddd', 'S'], value: Union[str, bytes, bytearray], parts: _Parts) -> None:
        ...

    @overload
    def _parse_token(self, token: Literal['a', 'A', 'ZZZ', 'ZZ', 'Z'], value: Union[str, bytes], parts: _Parts) -> None:
        ...

    @overload
    def _parse_token(self, token: Literal['W'], value: Tuple[_WEEKDATE_ELEMENT, _WEEKDATE_ELEMENT, Optional[_WEEKDATE_ELEMENT]], parts: _Parts) -> None:
        ...
    ...
    def _build_datetime(parts: _Parts) -> datetime:
        ...
        if expanded_timestamp is not None:
            return datetime.fromtimestamp(normalize_timestamp(expanded_timestamp), tz=tz.tzutc())
        ...
        return datetime(year=parts.get('year', 1), month=parts.get('month', 1), day=parts.get('day', 1), hour=hour, minute=parts.get('minute', 0), second=parts.get('second', 0), microsecond=microsecond, tzinfo=parts.get('tzinfo')) + increment
