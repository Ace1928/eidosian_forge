...
class NetAppOntapDns:
    ...
    def __init__(self):
        ...
        self.argument_spec.update(dict(state=dict(required=False, type='str', choices=['present', 'absent'], default='present'), vserver=dict(required=False, type='str'), domains=dict(required=False, type='list', elements='str'), nameservers=dict(required=False, type='list', elements='str'), skip_validation=dict(required=False, type='bool')))
        self.module = AnsibleModule(argument_spec=self.argument_spec, required_if=[('state', 'present', ['domains', 'nameservers'])], supports_check_mode=True)
    ...
    def create_dns(self):
        ...
        try:
            ...
            self.module.fail_json(msg='Error creating dns: %s' % to_native(error), exception=traceback.format_exc())
