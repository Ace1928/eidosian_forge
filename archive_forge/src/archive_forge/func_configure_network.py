    ...
    for key in range(0, len(network_devices)):
        ...
        if key < len(current_net_devices) and (vm_obj or self.params['template']):
            ...
            if 'start_connected' in network_devices[key] and nic.device.connectable.startConnected != network_devices[key].get('start_connected'):
                ...
                nic_change_detected = True
            if 'connected' in network_devices[key] and nic.device.connectable.connected != network_devices[key].get('connected'):
        ...
        if hasattr(net_obj, 'portKeys'):
            ...
            if host_system and host_system not in [host.config.host.name for host in pg_obj.config.distributedVirtualSwitch.config.host]:
                self.module.fail_json(msg="It seems that host system '%s' is not associated with distributed virtual portgroup '%s'. Please make sure host system is associated with given distributed virtual portgroup" % (host_system, pg_obj.name))
