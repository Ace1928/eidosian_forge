...
def _BindTagToInstance(tag_value, instance):
    ...
    binding_req = messages.CloudresourcemanagerTagBindingsCreateRequest(tagBinding=tag_binding)
    ...
    with endpoints.CrmEndpointOverrides(location):
        try:
            ...
            if not op.done:
                operations.WaitForReturnOperation(op, 'Waiting for TagBinding for parent [{}] and tag value [{}] to be created with [{}]'.format(resource_name, tag_value, op.name))
