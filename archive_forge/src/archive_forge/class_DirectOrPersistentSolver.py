...
class DirectOrPersistentSolver(OptSolver):
    ...
    def _presolve(self, **kwds):
        ...
        if self._save_results:
            ...
            if isinstance(self._pyomo_model, IBlock):
                ...
                getattr(self._pyomo_model, '._symbol_maps')[self._smap_id] = self._symbol_map
            else:
    ...
    def _set_instance(self, model, kwds={}):
        if not isinstance(model, (Model, IBlock, Block, _BlockData)):
            msg = "The problem instance supplied to the {0} plugin '_presolve' method must be a Model or a Block".format(type(self))
    ...
    def _add_block(self, block):
        for var in block.component_data_objects(ctype=pyomo.core.base.var.Var, descend_into=True, active=True, sort=True):
            self._add_var(var)
