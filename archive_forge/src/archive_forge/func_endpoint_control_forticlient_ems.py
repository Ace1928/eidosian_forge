...
def endpoint_control_forticlient_ems(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('endpoint-control', 'forticlient-ems', filtered_data, vdom=vdom)
        current_data = fos.get('endpoint-control', 'forticlient-ems', vdom=vdom, mkey=mkey)
    ...
    if state == 'present' or state is True:
        return fos.set('endpoint-control', 'forticlient-ems', data=filtered_data, vdom=vdom)
    elif state == 'absent':
        return fos.delete('endpoint-control', 'forticlient-ems', mkey=filtered_data['name'], vdom=vdom)
    else:
