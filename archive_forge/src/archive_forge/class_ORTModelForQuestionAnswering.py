...
class ORTModelForQuestionAnswering(ORTModel):
    ...
    @add_start_docstrings_to_model_forward(ONNX_TEXT_INPUTS_DOCSTRING.format('batch_size, sequence_length') + QUESTION_ANSWERING_EXAMPLE.format(processor_class=_TOKENIZER_FOR_DOC, model_class='ORTModelForQuestionAnswering', checkpoint='optimum/roberta-base-squad2'))
    def forward(self, input_ids: Optional[Union[torch.Tensor, np.ndarray]]=None, attention_mask: Optional[Union[torch.Tensor, np.ndarray]]=None, token_type_ids: Optional[Union[torch.Tensor, np.ndarray]]=None, **kwargs):
        ...
        if self.device.type == 'cuda' and self.use_io_binding:
            ...
            return QuestionAnsweringModelOutput(start_logits=output_buffers['start_logits'].view(output_shapes['start_logits']), end_logits=output_buffers['end_logits'].view(output_shapes['end_logits']))
        else:
            if use_torch:
