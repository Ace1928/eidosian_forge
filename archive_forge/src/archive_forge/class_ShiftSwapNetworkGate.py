...
class ShiftSwapNetworkGate(PermutationGate):
    ...
    def __init__(self, left_part_lens: Iterable[int], right_part_lens: Iterable[int], swap_gate: 'cirq.Gate'=ops.SWAP) -> None:
    ...
    def permutation(self) -> Dict[int, int]:
        return dict(zip(range(self.num_qubits()), itertools.chain(range(self.qubit_count('right'), self.num_qubits()), range(self.qubit_count('right')))))
    ...
    def __eq__(self, other) -> bool:
        return isinstance(other, self.__class__) and self.part_lens == other.part_lens and (self.swap_gate == other.swap_gate)