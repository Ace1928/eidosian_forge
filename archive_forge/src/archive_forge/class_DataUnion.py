...
class DataUnion(Union):
    ...
    def __init__(self, default_value=Undefined, dtype=None, shape_constraint=None, kw_array=None, kw_widget=None, **kwargs):
        self.dtype = dtype
        self.shape_constraint = shape_constraint
        kw_array = kw_array or {}
        kw_widget = kw_widget or {}
        traits = [NDArray(dtype=dtype, **kw_array), Instance(NDArrayBase)]
    ...
    def validate(self, obj, value):
        ...
        if isinstance(value, NDArrayBase) and self.dtype is not None:
            if value.dtype != self.dtype:
                raise TraitError('dtypes must match exactly when passing a NDArrayWidget to a dtype constrained DataUnion')
