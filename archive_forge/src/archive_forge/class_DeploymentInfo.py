...
class DeploymentInfo:
    ...
    def __init__(self, deployment_config: DeploymentConfig, replica_config: ReplicaConfig, start_time_ms: int, deployer_job_id: str, actor_name: Optional[str]=None, version: Optional[str]=None, end_time_ms: Optional[int]=None, route_prefix: str=None, docs_path: str=None, ingress: bool=False, target_capacity: Optional[float]=None, target_capacity_direction: Optional[TargetCapacityDirection]=None):
        ...
        if deployment_config.autoscaling_config is not None:
            self.autoscaling_policy = BasicAutoscalingPolicy(deployment_config.autoscaling_config)
        else:
    ...
    def update(self, deployment_config: DeploymentConfig=None, replica_config: ReplicaConfig=None, version: str=None, route_prefix: str=None) -> 'DeploymentInfo':
    ...
    def from_proto(cls, proto: DeploymentInfoProto):
        deployment_config = DeploymentConfig.from_proto(proto.deployment_config) if proto.deployment_config else None
        target_capacity = proto.target_capacity if proto.target_capacity != -1 else None
        target_capacity_direction = TargetCapacityDirectionProto.Name(proto.target_capacity_direction)
        if target_capacity_direction == 'UNSET':
            target_capacity_direction = None
        else:
            target_capacity_direction = TargetCapacityDirection(target_capacity_direction)
    ...
    def to_proto(self):
        data = {'start_time_ms': self.start_time_ms, 'actor_name': self.actor_name, 'version': self.version, 'end_time_ms': self.end_time_ms}
