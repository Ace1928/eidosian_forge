...
class FieldAttributeBase:
    ...
    def _resolve_group(self, fq_group_name, mandatory=True):
        ...
        action_group = action_groups.get(fq_group_name, action_groups.get(resource_name))
        ...
        for name, attribute in self.fattributes.items():
            if attribute.static:
                ...
                if name not in ('vars',) and templar.is_template(value):
                    display.warning('"%s" is not templatable, but we found: %s, it will not be templated and will be used "as is".' % (name, value))
                continue
            ...
            try:
                ...
                value = getattr(self, name)
                raise AnsibleParserError("the field '%s' has an invalid value (%s), and could not be converted to an %s.The error was: %s" % (name, value, attribute.isa, e), obj=self.get_ds(), orig_exc=e)
