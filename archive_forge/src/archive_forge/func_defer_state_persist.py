...
def defer_state_persist(self):
    ...
    if self.status == self.IN_PROGRESS:
        return False
    if self.convergence and self.action in {self.UPDATE, self.DELETE, self.CREATE, self.ADOPT, self.ROLLBACK, self.RESTORE}:
        return False
    return self.action not in {self.UPDATE, self.DELETE, self.ROLLBACK, self.RESTORE}