...
def vpn_ssl_web_user_group_bookmark(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('vpn.ssl.web', 'user-group-bookmark', filtered_data, vdom=vdom)
        current_data = fos.get('vpn.ssl.web', 'user-group-bookmark', vdom=vdom, mkey=mkey)
    ...
    if state == 'present' or state is True:
        return fos.set('vpn.ssl.web', 'user-group-bookmark', data=filtered_data, vdom=vdom)
    elif state == 'absent':
        return fos.delete('vpn.ssl.web', 'user-group-bookmark', mkey=filtered_data['name'], vdom=vdom)
    else:
