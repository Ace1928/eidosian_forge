...
class NetAppCloudmanagerNssAccount(object):
    ...
    def __init__(self):
        ...
        self.module = AnsibleModule(argument_spec=self.argument_spec, required_one_of=[['refresh_token', 'sa_client_id']], required_together=[['sa_client_id', 'sa_secret_key']], required_if=[('state', 'present', ['password'])], supports_check_mode=True)
    ...
    def delete_nss_account(self):
        response, err, second_dummy = self.rest_api.send_request('DELETE', '%s/accounts/%s' % (self.rest_api.api_root_path, self.parameters['public_id']), None, None, header=self.headers)
        if err is not None:
