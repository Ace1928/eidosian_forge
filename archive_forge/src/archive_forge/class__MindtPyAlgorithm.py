...
class _MindtPyAlgorithm(object):
    ...
    def __init__(self, **kwds):
        ...
        self.termination_condition_log_formatter = ' {:>9}   {:>15}   {:>15}   {:>12g}   {:>12g}   {:>7.2%}   {:>7.2f}'
        self.fixed_nlp_log_formatter = '{:1}{:>9}   {:>15}   {:>15g}   {:>12g}   {:>12g}   {:>7.2%}   {:>7.2f}'
        self.infeasible_fixed_nlp_log_formatter = '{:1}{:>9}   {:>15}   {:>15}   {:>12g}   {:>12g}   {:>7.2%}   {:>7.2f}'
    ...
    def init_rNLP(self, add_oa_cuts=True):
        ...
        with SuppressInfeasibleWarning():
            results = self.nlp_opt.solve(self.rnlp, tee=config.nlp_solver_tee, load_solutions=self.load_solutions, **nlp_args)
            if len(results.solution) > 0:
        ...
        if subprob_terminate_cond == tc.infeasible and config.partition_obj_nonlinear_terms and (self.rnlp.MindtPy_utils.objective_list[0].expr.polynomial_degree() not in self.mip_objective_polynomial_degree):
            config.logger.info('Initial relaxed NLP problem is infeasible. This might be related to partition_obj_nonlinear_terms. Trying to solve it again without partitioning nonlinear objective function.')
        ...
        if subprob_terminate_cond in {tc.optimal, tc.feasible, tc.locallyOptimal}:
            ...
            config.logger.info(self.log_formatter.format('-', 'Relaxed NLP', value(main_objective.expr), self.primal_bound, self.dual_bound, self.rel_gap, get_main_elapsed_time(self.timing)))
            if add_oa_cuts:
                if self.config.nlp_solver == 'cyipopt' and self.objective_sense == minimize:
                    dual_values = list((-1 * self.rnlp.dual[c] for c in MindtPy.constraint_list)) if config.calculate_dual_at_solution else None
                else:
                    dual_values = list((self.rnlp.dual[c] for c in MindtPy.constraint_list)) if config.calculate_dual_at_solution else None
                copy_var_list_values(self.rnlp.MindtPy_utils.variable_list, self.mip.MindtPy_utils.variable_list, config, ignore_integrality=True)
                if config.init_strategy == 'FP':
                    copy_var_list_values(self.rnlp.MindtPy_utils.variable_list, self.working_model.MindtPy_utils.variable_list, config, ignore_integrality=True)
                self.add_cuts(dual_values=dual_values, linearize_active=True, linearize_violated=True, cb_opt=None, nlp=self.rnlp)
            ...
            config.logger.info('Initial relaxed NLP problem is infeasible. Problem may be infeasible.')
            ...
            raise ValueError('MindtPy unable to handle relaxed NLP termination condition of %s. Solver message: %s' % (subprob_terminate_cond, results.solver.message))
    ...
    def init_max_binaries(self):
        ...
        binary_vars = (v for v in m.MindtPy_utils.discrete_variable_list if v.is_binary() and (not v.fixed))
        MindtPy.max_binary_obj = Objective(expr=sum((v for v in binary_vars)), sense=maximize)
        ...
        update_solver_timelimit(self.mip_opt, config.mip_solver, self.timing, config)
        results = self.mip_opt.solve(m, tee=config.mip_solver_tee, load_solutions=self.load_solutions, **mip_args)
        if len(results.solution) > 0:
            m.solutions.load_from(results)
        solve_terminate_cond = results.solver.termination_condition
        if solve_terminate_cond is tc.optimal:
            copy_var_list_values(MindtPy.variable_list, self.working_model.MindtPy_utils.variable_list, config)
            config.logger.info(self.log_formatter.format('-', 'Max binary MILP', value(MindtPy.max_binary_obj.expr), self.primal_bound, self.dual_bound, self.rel_gap, get_main_elapsed_time(self.timing)))
        elif solve_terminate_cond is tc.infeasible:
            raise ValueError('MIP main problem is infeasible. Problem may have no more feasible binary configurations.')
            ...
            raise ValueError('MindtPy unable to handle MILP main termination condition of %s. Solver message: %s' % (solve_terminate_cond, results.solver.message))
    ...
    def handle_subproblem_infeasible(self, fixed_nlp, cb_opt=None):
        ...
        """
        config = self.config
        config.logger.info(self.infeasible_fixed_nlp_log_formatter.format(' ', self.nlp_iter, 'Fixed NLP', 'Infeasible', self.primal_bound, self.dual_bound, self.rel_gap, get_main_elapsed_time(self.timing)))
        ...
        if self.should_terminate:
            return
        copy_var_list_values(feas_subproblem.MindtPy_utils.variable_list, self.mip.MindtPy_utils.variable_list, config)
        self.add_cuts(dual_values=dual_values, linearize_active=True, linearize_violated=True, cb_opt=cb_opt, nlp=feas_subproblem)
        var_values = list((v.value for v in fixed_nlp.MindtPy_utils.variable_list))
        if config.add_no_good_cuts:
            add_no_good_cuts(self.mip, var_values, config, self.timing, self.mip_iter, cb_opt)
    ...
    def algorithm_should_terminate(self, check_cycling):
        ...
        if self.should_terminate:
            ...
            return True
        return self.bounds_converged() or self.reached_iteration_limit() or self.reached_time_limit() or self.reached_stalling_limit() or (check_cycling and self.iteration_cycling())
    ...
    def setup_main(self):
        ...
        if config.add_slack:
            ...
            MindtPy.aug_penalty_expr = Expression(expr=sign_adjust * config.OA_penalty_factor * sum((v for v in MindtPy.cuts.slack_vars.values())))
        main_objective = MindtPy.objective_list[-1]
        MindtPy.mip_obj = Objective(expr=main_objective.expr + (MindtPy.aug_penalty_expr if config.add_slack else 0), sense=self.objective_sense)
        if config.use_dual_bound:
            MindtPy.cuts.del_component('dual_bound')
            if self.dual_bound not in {float('inf'), float('-inf')}:
                if self.objective_sense == minimize:
                    MindtPy.cuts.dual_bound = Constraint(expr=main_objective.expr + (MindtPy.aug_penalty_expr if config.add_slack else 0) >= self.dual_bound, doc='Objective function expression should improve on the best found dual bound')
                else:
                    MindtPy.cuts.dual_bound = Constraint(expr=main_objective.expr + (MindtPy.aug_penalty_expr if config.add_slack else 0) <= self.dual_bound, doc='Objective function expression should improve on the best found dual bound')
    ...
    def load_solution(self):
        ...
        copy_var_list_values(from_list=self.best_solution_found.MindtPy_utils.variable_list, to_list=MindtPy.variable_list, config=config)
        ...
        working_model_variable_list = list(get_vars_from_components(block=self.working_model, ctype=(Constraint, Objective), include_fixed=False, active=True, sort=True, descend_into=True, descent_order=None))
        original_model_variable_list = list(get_vars_from_components(block=self.original_model, ctype=(Constraint, Objective), include_fixed=False, active=True, sort=True, descend_into=True, descent_order=None))
        for v_from, v_to in zip(working_model_variable_list, original_model_variable_list):
            if v_from.name != v_to.name:
                raise DeveloperError('The name of the two variables is not the same. Loading final solution')
        copy_var_list_values(working_model_variable_list, original_model_variable_list, config=config)
    ...
    def check_config(self):
        ...
        if config.use_tabu_list:
            ...
            if config.threads > 1:
                config.threads = 1
                config.logger.info('The threads parameter is corrected to 1 since incumbent callback conflicts with multi-threads mode.')
        if config.solution_pool:
            if config.mip_solver not in {'cplex_persistent', 'gurobi_persistent'}:
                ...
                config.mip_solver = 'cplex_persistent'
        if 'appsi' in config.mip_solver or 'appsi' in config.nlp_solver or (config.mip_regularization_solver is not None and 'appsi' in config.mip_regularization_solver):
    ...
    def initialize_mip_problem(self):
        ...
        if len(MindtPy.grey_box_list) > 0:
            for grey_box in MindtPy.grey_box_list:
                grey_box.deactivate()
        if config.init_strategy == 'FP':
            MindtPy.cuts.fp_orthogonality_cuts = ConstraintList(doc='Orthogonality cuts in feasibility pump')
            if config.fp_projcuts:
                self.working_model.MindtPy_utils.cuts.fp_orthogonality_cuts = ConstraintList(doc='Orthogonality cuts in feasibility pump')
    ...
    def initialize_subsolvers(self):
        ...
        set_solver_constraint_violation_tolerance(self.nlp_opt, config.nlp_solver, config)
        set_solver_constraint_violation_tolerance(self.feasibility_nlp_opt, config.nlp_solver, config, warm_start=False)
        ...
        if config.mip_regularization_solver is not None:
            set_solver_mipgap(self.regularization_mip_opt, config.mip_regularization_solver, config)
            ...
            if config.mip_regularization_solver in {'cplex', 'appsi_cplex', 'cplex_persistent'}:
                if config.solution_limit is not None:
                    self.regularization_mip_opt.options['mip_limits_solutions'] = config.solution_limit
                ...
                if config.solution_limit is not None:
                    self.regularization_mip_opt.options['SolutionLimit'] = config.solution_limit
    ...
    def solve(self, model, **kwds):
        ...
        if config.integer_to_binary:
            TransformationFactory('contrib.integer_to_binary').apply_to(self.working_model)
        self.create_utility_block(self.working_model, 'MindtPy_utils')
        with time_code(self.timing, 'total', is_main_timer=True), lower_logger_level_to(config.logger, new_logging_level):
            ...
            config.logger.info(' {:<25}:   {:>7.4f} '.format('Primal-dual gap integral', self.primal_dual_gap_integral))
        self.update_result()
        if config.single_tree:
            self.results.solver.num_nodes = self.nlp_iter - (1 if config.init_strategy == 'rNLP' else 0)
        return self.results
        ...
        while self.mip_iter < config.iteration_limit:
            ...
            if config.add_regularization is not None:
                ...
                if config.single_tree:
                    ...
                    copy_var_list_values(main_mip.MindtPy_utils.variable_list, self.fixed_nlp.MindtPy_utils.variable_list, config, skip_fixed=False)
            ...
            if not config.single_tree:
                if not config.solution_pool:
                    ...
                    for index, (name, _) in enumerate(solution_name_obj):
                        if index > 0:
                            copy_var_list_values_from_solution_pool(self.mip.MindtPy_utils.variable_list, self.fixed_nlp.MindtPy_utils.variable_list, config, solver_model=main_mip_results._solver_model, var_map=main_mip_results._pyomo_var_to_solver_var_map, solution_name=name)
                            self.curr_int_sol = get_integer_solution(self.fixed_nlp)
                            if self.curr_int_sol in set(self.integer_list):
                                config.logger.info('The same combination has been explored and will be skipped here.')
                                continue
                            else:
                                self.integer_list.append(self.curr_int_sol)
                        fixed_nlp, fixed_nlp_result = self.solve_subproblem()
                        ...
                        if self.algorithm_should_terminate(check_cycling=False):
                            self.last_iter_cuts = True
                            break
        if (config.add_no_good_cuts or config.use_tabu_list) and (not self.should_terminate) and (config.add_regularization is None) and (self.best_solution_found is not None):
        ...
        if self.best_solution_found is not None:
            if self.dual_bound != self.dual_bound_progress[0]:
                with time_code(self.timing, 'regularization main'):
                    regularization_main_mip, regularization_main_mip_results = self.solve_regularization_main()
                self.handle_regularization_main_tc(regularization_main_mip, regularization_main_mip_results)

    def bounds_converged(self):
        if self.abs_gap <= self.config.absolute_bound_tolerance:
            self.config.logger.info('MindtPy exiting on bound convergence. Absolute gap: {} <= absolute tolerance: {} \n'.format(self.abs_gap, self.config.absolute_bound_tolerance))
            self.results.solver.termination_condition = tc.optimal
            return True
        if self.best_solution_found is not None:
            if self.rel_gap <= self.config.relative_bound_tolerance:
                self.config.logger.info('MindtPy exiting on bound convergence. Relative gap : {} <= relative tolerance: {} \n'.format(self.rel_gap, self.config.relative_bound_tolerance))
                self.results.solver.termination_condition = tc.optimal
                return True
        return False
    ...
    def reached_iteration_limit(self):
        if self.mip_iter >= self.config.iteration_limit:
            self.config.logger.info('MindtPy unable to converge bounds after {} main iterations.'.format(self.mip_iter))
    ...
    def reached_stalling_limit(self):
        ...
        if len(self.primal_bound_progress) >= config.stalling_limit:
            if abs(self.primal_bound_progress[-1] - self.primal_bound_progress[-config.stalling_limit]) <= config.zero_tolerance:
                config.logger.info('Algorithm is not making enough progress. Exiting iteration loop.')
                ...
                if self.best_solution_found is not None:
                    ...
                    config.logger.warning('Algorithm did not find a feasible solution. Returning best bound solution. Consider increasing stalling_limit or absolute_bound_tolerance.')
                    self.results.solver.termination_condition = tc.noSolution
                return True
        return False
