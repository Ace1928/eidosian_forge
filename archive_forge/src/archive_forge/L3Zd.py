...
def create_tensor_wave(signal: np.ndarray, sr: int) -> dict:
    """Create a tensor wave representation of the input signal."""
    ...
    # Construct tensor representation dictionary
    ...
    scaler = MinMaxScaler()
    for key, value in tensor_wave_dict.items():
        logging.info(f"Normalizing and scaling {key}")
        if value.ndim == 1:
            value = value.reshape(-1, 1)
        tensor_wave_dict[key] = scaler.fit_transform(value.T).T
        logging.info(f"{key} normalization and scaling complete")

    # Ensure tensor shapes are suitable for image data
    logging.info("Ensuring tensor shapes are suitable for image data")
    for key, value in tensor_wave_dict.items():
        if value.ndim == 1:
            tensor_wave_dict[key] = value.reshape(1, -1)
        elif value.ndim == 2:
            tensor_wave_dict[key] = value.reshape(1, *value.shape)
        logging.info(f"{key} reshaped to {tensor_wave_dict[key].shape}")
