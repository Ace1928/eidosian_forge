...
class TestXpressPersistent(unittest.TestCase):
    ...
    def test_basics(self):
        ...
        res = opt.solve()
        self.assertAlmostEqual(m.x.value, -0.4, delta=1e-06)
        self.assertAlmostEqual(m.y.value, 0.2, delta=1e-06)
        opt.load_duals()
        self.assertAlmostEqual(m.dual[m.c1], -0.4, delta=1e-06)
        del m.dual
        ...
        self.assertAlmostEqual(m.y.value, 0.2, delta=1e-06)
        opt.load_vars()
        self.assertAlmostEqual(m.x.value, 0, delta=1e-06)
        self.assertAlmostEqual(m.y.value, 1, delta=2e-06)
        opt.remove_constraint(m.c2)
        m.del_component(m.c2)
        self.assertEqual(opt.get_xpress_attribute('cols'), 2)
        self.assertEqual(opt.get_xpress_attribute('rows'), 1)
        self.assertEqual(opt.get_xpress_control('feastol'), 1e-06)
        res = opt.solve(options={'feastol': '1e-7'})
        self.assertEqual(opt.get_xpress_control('feastol'), 1e-07)
        self.assertAlmostEqual(m.x.value, -0.4, delta=1e-06)
        self.assertAlmostEqual(m.y.value, 0.2, delta=1e-06)
        m.x.setlb(-5)
        m.x.setub(5)
