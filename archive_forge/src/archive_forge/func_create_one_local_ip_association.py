...
def create_one_local_ip_association(attrs=None):
    ...
    """
    attrs = attrs or {}
    local_ip_association_attrs = {'local_ip_id': 'local-ip-id-' + uuid.uuid4().hex, 'local_ip_address': '172.24.4.228', 'fixed_port_id': 'fixed-port-id-' + uuid.uuid4().hex, 'fixed_ip': '10.0.0.5', 'host': 'host-' + uuid.uuid4().hex}
    local_ip_association_attrs.update(attrs)
    local_ip_association = _local_ip_association.LocalIPAssociation(**local_ip_association_attrs)
