...
def record_task_log_start(self, task_id: TaskID, attempt_number: int):
    ...
    if not hasattr(self, 'core_worker'):
        return
    self.core_worker.record_task_log_start(task_id, attempt_number, self.get_out_file_path(), self.get_err_file_path(), self.get_current_out_offset(), self.get_current_err_offset())