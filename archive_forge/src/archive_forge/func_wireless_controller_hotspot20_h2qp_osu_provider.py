...
def wireless_controller_hotspot20_h2qp_osu_provider(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('wireless-controller.hotspot20', 'h2qp-osu-provider', filtered_data, vdom=vdom)
        current_data = fos.get('wireless-controller.hotspot20', 'h2qp-osu-provider', vdom=vdom, mkey=mkey)
    ...
    if state == 'present' or state is True:
        return fos.set('wireless-controller.hotspot20', 'h2qp-osu-provider', data=filtered_data, vdom=vdom)
    elif state == 'absent':
        return fos.delete('wireless-controller.hotspot20', 'h2qp-osu-provider', mkey=filtered_data['name'], vdom=vdom)
    else:
