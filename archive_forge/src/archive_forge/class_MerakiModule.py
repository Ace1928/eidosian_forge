...
class MerakiModule(object):
    ...
    def __init__(self, module, function=None):
        ...
        self.retry = 0
        self.retry_time = 0
        self.get_urls = {'organizations': '/organizations', 'network': '/organizations/{org_id}/networks', 'admins': '/organizations/{org_id}/admins', 'configTemplates': '/organizations/{org_id}/configTemplates', 'samlymbols': '/organizations/{org_id}/samlRoles', 'ssids': '/networks/{net_id}/ssids', 'groupPolicies': '/networks/{net_id}/groupPolicies', 'staticRoutes': '/networks/{net_id}/staticRoutes', 'vlans': '/networks/{net_id}/vlans', 'devices': '/networks/{net_id}/devices'}
        self.get_one_urls = {'organizations': '/organizations/{org_id}', 'network': '/networks/{net_id}'}
        self.url_catalog = {'get_all': self.get_urls, 'get_one': self.get_one_urls, 'create': None, 'update': None, 'delete': None, 'misc': None}
        ...
        self.module.required_if = [('state', 'present', ['org_name']), ('state', 'absent', ['org_name'])]
        self.modifiable_methods = ['POST', 'PUT', 'DELETE']
        self.headers = {'Content-Type': 'application/json', 'Authorization': 'Bearer {key}'.format(key=module.params['auth_key'])}
        ...
        if isinstance(data, dict):
            ...
            for k, v in data.items():
                try:
                    ...
                    snake_k = re.sub('([a-z0-9])([A-Z])', '\\1_\\2', k).lower()
    ...
    def construct_path(self, action, function=None, org_id=None, net_id=None, org_name=None, custom=None, params=None):
    ...
    def request(self, path, method=None, payload=None, params=None, pagination_items=None):
        ...
        if pagination_items is not None:
            ...
            try:
                ...
                self.fail_json(msg='Failure occurred during pagination', response=self.response, status=self.status, body=self.body)
            ...
            try:
                ...
                self.fail_json(msg='Failure occurred', response=self.response, status=self.status, body=self.body)
    ...
    def exit_json(self, **kwargs):
        ...
        if self.params['output_format'] == 'camelcase':
            self.module.deprecate('Update your playbooks to support snake_case format instead of camelCase format.', date='2022-06-01', collection_name='cisco.meraki')
        elif 'data' in self.result:
            try:
