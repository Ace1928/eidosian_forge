...
class PBSQueueRun:
    ...
    def __init__(self, data_connection, tmp_folder, job_prefix, n_simul, job_template_generator, qsub_command='qsub', qstat_command='qstat', find_neighbors=None, perform_parametrization=None):
            ...
            if os.path.isfile(fdone) and os.path.getsize(fdone) > 0:
                try:
                    ...
                    while len(a) == 0 and niter < 5:
                        ...
                        niter += 1
                    if len(a) == 0:
                        txt = 'Could not read candidate ' + '{0} from the filesystem'.format(c)
                        raise IOError(txt)
                    a = a[-1]
                    a.info['confid'] = c
                    self.dc.add_relaxed_step(a, find_neighbors=self.find_neighbors, perform_parametrization=self.perform_parametrization)
                except IOError as e:
                    print(e)