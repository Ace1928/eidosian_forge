...
def xla_dot_eager_fallback(lhs: _atypes.TensorFuzzingAnnotation[TV_XlaDot_T], rhs: _atypes.TensorFuzzingAnnotation[TV_XlaDot_T], dimension_numbers: str, precision_config: str, name, ctx) -> _atypes.TensorFuzzingAnnotation[TV_XlaDot_T]:
    ...
    _attrs = ('T', _attr_T, 'dimension_numbers', dimension_numbers, 'precision_config', precision_config)
    _result = _execute.execute(b'XlaDot', 1, inputs=_inputs_flat, attrs=_attrs, ctx=ctx, name=name)
