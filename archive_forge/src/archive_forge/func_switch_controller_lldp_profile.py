...
def switch_controller_lldp_profile(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('switch-controller', 'lldp-profile', filtered_data, vdom=vdom)
        current_data = fos.get('switch-controller', 'lldp-profile', vdom=vdom, mkey=mkey)
    ...
    if state == 'present' or state is True:
        return fos.set('switch-controller', 'lldp-profile', data=converted_data, vdom=vdom)
    elif state == 'absent':
        return fos.delete('switch-controller', 'lldp-profile', mkey=filtered_data['name'], vdom=vdom)
    else:
