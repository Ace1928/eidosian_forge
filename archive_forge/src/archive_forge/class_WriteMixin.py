...
class WriteMixin:
    ...
    def write_stream(self, stream: Callable[..., Any] | Generator[Any, Any, Any] | Iterable[Any]) -> list[Any] | str:
        ...
        Example
        -------
        You can pass an OpenAI stream as shown in our tutorial, `Build a         basic LLM chat app <https://docs.streamlit.io/knowledge-base/tutorials        /build-conversational-apps#build-a-chatgpt-like-app>`_. Alternatively,
        ...
        if isinstance(stream, str) or type_util.is_dataframe_like(stream):
            raise StreamlitAPIException(f'`st.write_stream` expects a generator or stream-like object as input not {type(stream)}. Please use `st.write` instead for this data type.')
        ...
        try:
            ...
            raise StreamlitAPIException(f'The provided input (type: {type(stream)}) cannot be iterated. Please make sure that it is a generator, generator function or iterable.') from exc
        for chunk in stream:
            if type_util.is_openai_chunk(chunk):
                try:
                    ...
                    raise StreamlitAPIException('Failed to parse the OpenAI ChatCompletionChunk. The most likely cause is a change of the chunk object structure due to a recent OpenAI update. You might be able to fix this by downgrading the OpenAI library or upgrading Streamlit. Also, please report this issue to: https://github.com/streamlit/streamlit/issues.') from err
            ...
            if isinstance(chunk, str):
                ...
                stream_container.markdown(streamed_response + ('' if first_text else _TEXT_CURSOR))
            elif callable(chunk):
    ...
    def write(self, *args: Any, unsafe_allow_html: bool=False, **kwargs) -> None:
        ...
        **kwargs : any
        ...
        """
        if kwargs:
            _LOGGER.warning('Invalid arguments were passed to "st.write" function. Support for passing such unknown keywords arguments will be dropped in future. Invalid arguments were: %s', kwargs)
        string_buffer: list[str] = []
        if not self.dg._is_top_level and len(args) > 1:
            raise StreamlitAPIException('Cannot replace a single element with multiple elements.\n\nThe `write()` method only supports multiple elements when inserting elements rather than replacing. That is, only when called as `st.write()` or `st.sidebar.write()`.')
        ...
        for arg in args:
            ...
            elif isinstance(arg, (dict, list, SessionStateProxy, UserInfoProxy, QueryParamsProxy)):
                flush_buffer()
                self.dg.json(arg)
            ...
            elif inspect.isgenerator(arg) or inspect.isgeneratorfunction(arg) or type_util.is_type(arg, 'openai.Stream'):
                flush_buffer()
            ...
            elif hasattr(arg, '_repr_html_') and callable(arg._repr_html_) and (repr_html := arg._repr_html_()) and (unsafe_allow_html or not probably_contains_html_tags(repr_html)):
