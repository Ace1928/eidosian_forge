...
class _TestDictContainerBase(object):
    ...
    def test_keys(self):
        ...
        self.assertEqual(sorted(list(raw_constraint_dict.keys()), key=str), sorted(list(c.keys()), key=str))

    def test_values(self):
        index = ['a', 1, None, (1,), (1, 2)]
        raw_constraint_dict = {i: self._ctype_factory() for i in index}
        c = self._container_type(raw_constraint_dict)
        self.assertEqual(sorted(list((id(_v) for _v in raw_constraint_dict.values())), key=str), sorted(list((id(_v) for _v in c.values())), key=str))

    def test_items(self):
        index = ['a', 1, None, (1,), (1, 2)]
        raw_constraint_dict = {i: self._ctype_factory() for i in index}
        c = self._container_type(raw_constraint_dict)
        self.assertEqual(sorted(list(((_i, id(_v)) for _i, _v in raw_constraint_dict.items())), key=str), sorted(list(((_i, id(_v)) for _i, _v in c.items())), key=str))
    ...
    def test_name(self):
        ...
        for key, c in children.items():
            ...
            self.assertEqual(c.getname(fully_qualified=False, convert=str), '[%s]' % str(key))
            self.assertEqual(c.getname(fully_qualified=False, convert=repr), '[%s]' % repr(key))
            self.assertEqual(c.getname(fully_qualified=True, convert=str), '[%s]' % str(key))
            self.assertEqual(c.getname(fully_qualified=True, convert=repr), '[%s]' % repr(key))
        ...
        for key, c in children.items():
            ...
            self.assertEqual(c.getname(fully_qualified=False, convert=str), '[%s]' % str(key))
            self.assertEqual(c.getname(fully_qualified=False, convert=repr), '[%s]' % repr(key))
            self.assertEqual(c.getname(fully_qualified=True, convert=str), 'cdict[%s]' % str(key))
            self.assertEqual(c.getname(fully_qualified=True, convert=repr), 'cdict[%s]' % repr(key))
            self.assertEqual(c.name, names[c])
        for c in cdict.components():
            self.assertNotEqual(c.name, None)
            self.assertEqual(c.name, names[c])
        ...
        for key, c in children.items():
            ...
            self.assertEqual(c.getname(fully_qualified=False, convert=str), '[%s]' % str(key))
            self.assertEqual(c.getname(fully_qualified=False, convert=repr), '[%s]' % repr(key))
            self.assertEqual(c.getname(fully_qualified=True, convert=str), 'model.cdict[%s]' % str(key))
            self.assertEqual(c.getname(fully_qualified=True, convert=repr), 'model.cdict[%s]' % repr(key))
            self.assertEqual(c.name, names[c])
        for c in cdict.components():
            self.assertNotEqual(c.name, None)
            self.assertEqual(c.name, names[c])
        ...
        for key, c in children.items():
            ...
            self.assertEqual(c.getname(fully_qualified=False, convert=str), '[%s]' % str(key))
            self.assertEqual(c.getname(fully_qualified=False, convert=repr), '[%s]' % repr(key))
            self.assertEqual(c.getname(fully_qualified=True, convert=str), '[0].model.cdict[%s]' % str(key))
            self.assertEqual(c.getname(fully_qualified=True, convert=repr), '[0].model.cdict[%s]' % repr(key))
        m = block()
        m.bdict = bdict
        ...
        for key, c in children.items():
            ...
            self.assertEqual(c.getname(fully_qualified=False, convert=str), '[%s]' % str(key))
            self.assertEqual(c.getname(fully_qualified=False, convert=repr), '[%s]' % repr(key))
            self.assertEqual(c.getname(fully_qualified=True, convert=str), 'bdict[0].model.cdict[%s]' % str(key))
            self.assertEqual(c.getname(fully_qualified=True, convert=repr), 'bdict[0].model.cdict[%s]' % repr(key))
            self.assertEqual(c.name, names[c])
        for c in cdict.components():
            self.assertNotEqual(c.name, None)
            self.assertEqual(c.name, names[c])
    ...
    def test_preorder_traversal(self):
        ...
        self.assertEqual([c.name for c in traversal], [c.name for c in pmo.preorder_traversal(cdict, descend=descend)])
        self.assertEqual([id(c) for c in traversal], [id(c) for c in pmo.preorder_traversal(cdict, descend=descend)])
    ...
    def test_preorder_traversal_descend_check(self):
        ...
        self.assertEqual([c.name for c in traversal if c._is_container], [c.name for c in descend.seen])
        self.assertEqual([id(c) for c in traversal if c._is_container], [id(c) for c in descend.seen])
