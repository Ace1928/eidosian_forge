...
class RollingExp(Generic[T_DataWithCoords]):
    ...
    def mean(self, keep_attrs: bool | None=None) -> T_DataWithCoords:
        ...
        return apply_ufunc(numbagg.move_exp_nanmean, self.obj, input_core_dims=[[self.dim]], kwargs=self.kwargs, output_core_dims=[[self.dim]], keep_attrs=keep_attrs, on_missing_core_dim='copy', dask='parallelized').transpose(*dim_order)
    ...
    def sum(self, keep_attrs: bool | None=None) -> T_DataWithCoords:
        ...
        if keep_attrs is None:
            keep_attrs = _get_keep_attrs(default=True)
        dim_order = self.obj.dims
        return apply_ufunc(numbagg.move_exp_nansum, self.obj, input_core_dims=[[self.dim]], kwargs=self.kwargs, output_core_dims=[[self.dim]], keep_attrs=keep_attrs, on_missing_core_dim='copy', dask='parallelized').transpose(*dim_order)
    ...
    def std(self) -> T_DataWithCoords:
        ...
        if pycompat.mod_version('numbagg') < Version('0.4.0'):
            raise ImportError(f'numbagg >= 0.4.0 is required for rolling_exp().std(), currently {pycompat.mod_version('numbagg')} is installed')
        import numbagg
        dim_order = self.obj.dims
    ...
    def var(self) -> T_DataWithCoords:
        ...
        """
        if pycompat.mod_version('numbagg') < Version('0.4.0'):
            raise ImportError(f'numbagg >= 0.4.0 is required for rolling_exp().var(), currently {pycompat.mod_version('numbagg')} is installed')
        dim_order = self.obj.dims
        import numbagg
    ...
    def cov(self, other: T_DataWithCoords) -> T_DataWithCoords:
        ...
        """
        if pycompat.mod_version('numbagg') < Version('0.4.0'):
            raise ImportError(f'numbagg >= 0.4.0 is required for rolling_exp().cov(), currently {pycompat.mod_version('numbagg')} is installed')
        dim_order = self.obj.dims
        import numbagg
    ...
    def corr(self, other: T_DataWithCoords) -> T_DataWithCoords:
        ...
        """
        if pycompat.mod_version('numbagg') < Version('0.4.0'):
            raise ImportError(f'numbagg >= 0.4.0 is required for rolling_exp().corr(), currently {pycompat.mod_version('numbagg')} is installed')
        dim_order = self.obj.dims
        import numbagg
