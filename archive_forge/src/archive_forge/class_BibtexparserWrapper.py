...
class BibtexparserWrapper(BaseModel):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        try:
            ...
            raise ImportError('Could not import bibtexparser python package. Please install it with `pip install bibtexparser`.')
        return values
    ...
    def get_metadata(self, entry: Mapping[str, Any], load_extra: bool=False) -> Dict[str, Any]:
        ...
        if 'url' in entry:
            ...
            url = None
        meta = {'id': entry.get('ID'), 'published_year': entry.get('year'), 'title': entry.get('title'), 'publication': publication, 'authors': entry.get('author'), 'abstract': entry.get('abstract'), 'url': url}
        if load_extra:
