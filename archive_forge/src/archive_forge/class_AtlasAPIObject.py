...
class AtlasAPIObject:
    ...
    def __init__(self, module, object_name, group_id, path, data, data_is_array=False):
        self.module = module
        self.path = path
        self.data = data
    ...
    def call_url(self, path, data='', method='GET'):
        ...
        if self.data_is_array and data != '':
            data = '[' + data + ']'
        url = 'https://cloud.mongodb.com/api/atlas/v1.0/groups/' + self.group_id + path
    ...
    def exists(self):
        ...
        ret = self.call_url(path=self.path + additional_path + '/' + quote(self.data[self.object_name], ''))
        if ret['code'] == 200:
            return True
        return False

    def create(self):
        ret = self.call_url(path=self.path, data=self.module.jsonify(self.data), method='POST')
        return ret
    ...
    def delete(self):
        ...
        if self.path == '/databaseUsers':
            additional_path = '/admin'
        ret = self.call_url(path=self.path + additional_path + '/' + quote(self.data[self.object_name], ''), method='DELETE')
        return ret
    ...
    def modify(self):
        ...
        if self.path == '/databaseUsers':
            additional_path = '/admin'
        ret = self.call_url(path=self.path + additional_path + '/' + quote(self.data[self.object_name], ''), data=self.module.jsonify(self.data), method='PATCH')
        return ret
    ...
    def diff(self):
        ...
        if self.path == '/databaseUsers':
            additional_path = '/admin'
        ret = self.call_url(path=self.path + additional_path + '/' + quote(self.data[self.object_name], ''), method='GET')
        ...
        if self.exists():
            ...
            if state == 'absent':
                if self.module.check_mode:
                    ...
                    self.module.exit_json(changed=True, object_name=self.data[self.object_name], diff=diff_result)
                else:
                    try:
                        ret = self.delete()
                ...
                if self.module.check_mode:
                    if diff_result['after'] != '':
                        changed = True
                    self.module.exit_json(changed=changed, object_name=self.data[self.object_name], data=self.data, diff=diff_result)
                if diff_result['after'] != '':
