...
class Browser(QMainWindow):
    ...
    def init_ui(self):
        try:
            ...
            # Create a vertical box layout and set it for the central widget
            self.layout = QVBoxLayout(self.central_widget)

            # Create a QWebEngineView widget for displaying web pages
            self.browser = QWebEngineView()
            # Set the initial URL to Google
            self.browser.setUrl(QUrl("https://www.google.com"))  # Use HTTPS
            # Add the browser widget to the layout
            self.layout.addWidget(self.browser)

            # Create the navigation bar, menu, and status bar
            self.create_navbar()
            self.create_menu()
            ...
            # Connect the sslErrors signal from the QWebEnginePage
            self.browser.page().sslErrors.connect(self.handle_ssl_errors)
    ...
    def handle_ssl_errors(self, errors) -> None:
        """
        Handle SSL errors encountered during web page loading.

        Args:
            errors (List[QSslError]): A list of QSslError objects representing the SSL errors.
        """
        for error in errors:
            logging.error(f"SSL Error: {error.errorString()}")
        # Optionally, you can ignore SSL errors by calling ignoreSslErrors()
        # self.browser.page().ignoreSslErrors()
...
if __name__ == "__main__":
    try:
        # Create the application and set its name
        app = QApplication(sys.argv)
        QApplication.setApplicationName("Advanced Web Browser")
