...
class HextuplesSerializer(Serializer):
    ...
    def serialize(self, stream: IO[bytes], base: Optional[str]=None, encoding: Optional[str]='utf-8', **kwargs):
        if base is not None:
            warnings.warn('base has no meaning for Hextuples serialization. I will ignore this value')
        if encoding not in [None, 'utf-8']:
            warnings.warn(f"Hextuples files are always utf-8 encoded. I was passed: {encoding}, but I'm still going to use utf-8 anyway!")
        if self.store.formula_aware is True:
            raise Exception("Hextuple serialization can't (yet) handle formula-aware stores")
