...
class AbstractCopyTests:
    ...
    def test_copy_file_to_existing_directory(self, fs, fs_join, fs_bulk_operations_scenario_0, fs_target, supports_empty_directories):
    ...
    def test_copy_file_to_new_directory(self, fs, fs_join, fs_bulk_operations_scenario_0, fs_target):
        ...
        target = fs_target
        fs.mkdir(target)
        fs.cp(fs_join(source, 'subdir', 'subfile1'), fs_join(target, 'newdir/'))
        assert fs.isdir(target)
    ...
    def test_copy_directory_to_existing_directory(self, fs, fs_join, fs_bulk_operations_scenario_0, fs_target, supports_empty_directories):
        source = fs_bulk_operations_scenario_0
        target = fs_target
        fs.mkdir(target)
        if not supports_empty_directories:
