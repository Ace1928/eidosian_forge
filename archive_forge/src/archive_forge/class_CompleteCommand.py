...
class CompleteCommand(command.Command):
    ...
    def __init__(self, app, app_args, cmd_name=None):
        ...
        self._formatters = stevedore.ExtensionManager(namespace='cliff.formatter.completion')

    def get_parser(self, prog_name):
        parser = super(CompleteCommand, self).get_parser(prog_name)
        parser.add_argument('--name', default=None, metavar='<command_name>', help='Command name to support with command completion')
        parser.add_argument('--shell', default='bash', metavar='<shell>', choices=sorted(self._formatters.names()), help='Shell being used. Use none for data only (default: bash)')
        return parser
