...
class ServeControllerClient:
    ...
    def __init__(self, controller: ActorHandle, controller_name: str):
    ...
    def shutdown(self, timeout_s: float=30.0) -> None:
        ...
        if ray.is_initialized() and (not self._shutdown):
            try:
                ...
                logger.warning(f'Controller failed to shut down within {timeout_s}s. Check controller logs for more details.')
            self._shutdown = True
    ...
    @_ensure_connected
    def deploy(self, name: str, replica_config: ReplicaConfig, deployment_config: Union[None, DeploymentConfig, Dict[str, Any]]=None, version: Optional[str]=None, route_prefix: Optional[str]=None, url: Optional[str]=None, _blocking: Optional[bool]=True):
        controller_deploy_args = get_deploy_args(name=name, replica_config=replica_config, deployment_config=deployment_config, version=version, route_prefix=route_prefix)
        controller_deploy_args.pop('ingress')
        controller_deploy_args['name'] = controller_deploy_args.pop('deployment_name')
        updating = ray.get(self._controller.deploy.remote(is_deployed_from_python=True, **controller_deploy_args))
    ...
    def get_serve_status(self, name: str=SERVE_DEFAULT_APP_NAME) -> StatusOverview:
        proto = StatusOverviewProto.FromString(ray.get(self._controller.get_serve_status.remote(name)))
    ...
    def get_all_deployment_statuses(self) -> List[DeploymentStatusInfo]:
        ...
        return [DeploymentStatusInfo.from_proto(DeploymentStatusInfoProto.FromString(status_bytes)) for status_bytes in statuses_bytes]
    ...
    @_ensure_connected
    def get_handle(self, deployment_name: str, app_name: Optional[str]='default', missing_ok: Optional[bool]=False, sync: bool=True, use_new_handle_api: bool=RAY_SERVE_ENABLE_NEW_HANDLE_API) -> Union[DeploymentHandle, RayServeHandle, RayServeSyncHandle]:
        ...
        if not missing_ok and DeploymentID(deployment_name, app_name) not in all_deployments:
            raise KeyError(f"Deployment '{deployment_name}' in application '{app_name}' does not exist.")
        if use_new_handle_api:
            handle = DeploymentHandle(deployment_name, app_name, sync=sync)
        elif sync:
            handle = RayServeSyncHandle(deployment_name, app_name, sync=sync)
        else:
            handle = RayServeHandle(deployment_name, app_name, sync=sync)
        self.handle_cache[cache_key] = handle
        if cache_key in self._evicted_handle_keys:
            logger.warning("You just got a ServeHandle that was evicted from internal cache. This means you are getting too many ServeHandles in the same process, this will bring down Serve's performance. Please post a github issue at https://github.com/ray-project/ray/issues to let the Serve team to find workaround for your use case.")
