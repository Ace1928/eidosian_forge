...
class BayesianNetworkGate(raw_types.Gate):
    ...
    def __init__(self, init_probs: List[Tuple[str, Optional[float]]], arc_probs: List[Tuple[str, Tuple[str], List[float]]]):
    ...
    def _decompose_(self, qubits: Sequence['raw_types.Qid']) -> 'cirq.OP_TREE':
        ...
        for target, params, cond_probs in self._arc_probs:
            yield _generate_gate_set_for_arc_prob(qubit_map[target], [qubit_map[param] for param in params], cond_probs)
    ...
    @classmethod
    def _from_json_dict_(cls, init_probs: List[List[Union[str, Optional[float]]]], arc_probs: List[List[Union[str, List[str], List[float]]]], **kwargs) -> 'BayesianNetworkGate':
        converted_init_probs = cast(List[Tuple[str, Optional[float]]], [(param, init_prob) for param, init_prob in init_probs])
        converted_cond_probs = cast(List[Tuple[str, Tuple[str], List[float]]], [(target, tuple(params), cond_probs) for target, params, cond_probs in arc_probs])
        return cls(converted_init_probs, converted_cond_probs)

    def __repr__(self) -> str:
        return f'cirq.BayesianNetworkGate(init_probs={self._init_probs!r}, arc_probs={self._arc_probs!r})'