...
class TestListServerEvent(TestServerEvent):
    ...
    columns = ('Request ID', 'Server ID', 'Action', 'Start Time')
    data = ((fake_event.request_id, fake_event.server_id, fake_event.action, fake_event.start_time),)
    long_columns = ('Request ID', 'Server ID', 'Action', 'Start Time', 'Message', 'Project ID', 'User ID')
    long_data = ((fake_event.request_id, fake_event.server_id, fake_event.action, fake_event.start_time, fake_event.message, fake_event.project_id, fake_event.user_id),)

    def setUp(self):
        super().setUp()
        self.compute_sdk_client.find_server.return_value = self.fake_server
        self.compute_sdk_client.server_actions.return_value = [self.fake_event]
        self.cmd = server_event.ListServerEvent(self.app, None)

    def test_server_event_list(self):
        arglist = [self.fake_server.name]
        verifylist = [('server', self.fake_server.name), ('long', False)]
    ...
    def test_server_event_list_with_changes_since(self):
        ...
        columns, data = self.cmd.take_action(parsed_args)
        self.compute_sdk_client.find_server.assert_called_with(self.fake_server.name, ignore_missing=False)
        self.compute_sdk_client.server_actions.assert_called_with(self.fake_server.id, changes_since='2016-03-04T06:27:59Z')
        self.assertEqual(self.columns, columns)
    ...
    @mock.patch.object(iso8601, 'parse_date', side_effect=iso8601.ParseError)
    def test_server_event_list_with_changes_before_invalid(self, mock_parse_isotime):
        self._set_mock_microversion('2.66')
        arglist = ['--changes-before', 'Invalid time value', self.fake_server.name]
        verifylist = [('server', self.fake_server.name), ('changes_before', 'Invalid time value')]
        parsed_args = self.check_parser(self.cmd, arglist, verifylist)
        ex = self.assertRaises(exceptions.CommandError, self.cmd.take_action, parsed_args)
    ...
    def test_server_event_list_with_marker(self):
        self._set_mock_microversion('2.58')
        arglist = ['--marker', 'test_event', self.fake_server.name]
        verifylist = [('marker', 'test_event'), ('server', self.fake_server.name)]
        parsed_args = self.check_parser(self.cmd, arglist, verifylist)
        self.cmd.take_action(parsed_args)
        self.compute_sdk_client.server_actions.assert_called_with(self.fake_server.id, marker='test_event')
