...
class OpenAIInterface:
    ...
    def get_chat_response(self, messages, model_params) -> ChatCompletion:
        openai.api_key = self.api_key
    ...
    def transcribe_audio(self, audio_file) -> Transcription:
        openai.api_key = self.api_key
        return openai.Audio.transcribe(model="whisper-1", file=audio_file)

    def translate_audio(self, audio_file) -> Translation:
        openai.api_key = self.api_key
        return openai.Audio.translate(model="whisper-1", file=audio_file)

    def moderate_content(self, input) -> Moderation:
        openai.api_key = self.api_key
        return openai.Moderation.create(input=input)
