...
class TestGrid(unittest.TestCase):
    ...
    def setUp(self):
        self.grid = Grid(cells=20).get_grid()
        self.grid_size = len(self.grid)

    def test_init(self):
        self.assertIsInstance(self.grid, list)
        self.assertEqual(len(self.grid), self.grid_size)
        self.assertEqual(len(self.grid[0]), self.grid_size)
    ...
    def test_get_cell(self):
        cell = self.grid[0][0]
        self.assertIsInstance(cell, Node)
...
class TestA_STAR(unittest.TestCase):
    ...
    def test_calculate_heuristic(self):
        ...
        self.assertEqual(heuristic, 6)

    def test_select_optimal_path(self):
        path = [Node(1, 1), Node(1, 2), Node(1, 3), Node(1, 4)]
        optimal_path = self.a_star.select_optimal_path(path)
