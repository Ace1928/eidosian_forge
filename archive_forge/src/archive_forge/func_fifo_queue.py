...
def fifo_queue(component_types, shapes=[], capacity: int=-1, container: str='', shared_name: str='', name=None) -> _atypes.TensorFuzzingAnnotation[_atypes.String]:
  ...
  """
    ...
    if not isinstance(component_types, (list, tuple)):
        raise TypeError("Expected list for 'component_types' argument to 'fifo_queue' Op, not %r." % component_types)
    component_types = [_execute.make_type(_t, 'component_types') for _t in component_types]
    if shapes is None:
        shapes = []
    if not isinstance(shapes, (list, tuple)):
        raise TypeError("Expected list for 'shapes' argument to 'fifo_queue' Op, not %r." % shapes)
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('FIFOQueue', component_types=component_types, shapes=shapes, capacity=capacity, container=container, shared_name=shared_name, name=name)
    _result = _outputs[:]
    if _execute.must_record_gradient():
        ...
        _execute.record_gradient('FIFOQueue', _inputs_flat, _attrs, _result)
    _result, = _result
    return _result