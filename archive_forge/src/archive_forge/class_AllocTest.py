...
class AllocTest(common.HeatTestCase):
    ...
    def setUp(self):
        ...
        self.mock_list_ports = self.patchobject(neutronclient.Client, 'list_ports')
        self.mock_show_net = self.patchobject(neutronclient.Client, 'show_network')
        self.mock_list_routers = self.patchobject(neutronclient.Client, 'list_routers')
        self.patchobject(neutronclient.Client, 'remove_gateway_router')
    ...
    def mock_no_router_for_vpc(self):
        self.mock_list_routers.return_value = {'routers': []}
    ...
    def test_validate_properties_EIP_and_AllocationId(self):
        server = self.fc.servers.list()[0]
        self.patchobject(self.fc.servers, 'get', return_value=server)
        template, stack = self._setup_test_stack_validate(stack_name='validate_EIP_AllocationId')
    ...
    def test_update_association_needs_update_InstanceId(self):
        ...
        stack = utils.parse_stack(t)
        self.create_eip(t, stack, 'IPAddress')
        before_props = {'InstanceId': {'Ref': 'WebServer'}, 'EIP': '11.0.0.1'}
        after_props = {'InstanceId': {'Ref': 'WebServer2'}, 'EIP': '11.0.0.1'}
        before = self.create_association(t, stack, 'IPAssoc')
        after = rsrc_defn.ResourceDefinition(before.name, before.type(), after_props)
        self.assertTrue(before._needs_update(after, before, after_props, before_props, None))
