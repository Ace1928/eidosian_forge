...
class CommonGRU(OpRun):
    ...
    def _run(self, X, W, R, B=None, sequence_lens=None, initial_h=None, activation_alpha=None, activation_beta=None, activations=None, clip=None, direction=None, hidden_size=None, layout=None, linear_before_reset=None):
        ...
        if num_directions == 1:
            ...
            b = B if B is not None else np.zeros(2 * self.number_of_gates * hidden_size, dtype=X.dtype)
            h_0 = initial_h if initial_h is not None else np.zeros((batch_size, hidden_size), dtype=X.dtype)
            B = b
            H_0 = h_0
        else:
            raise NotImplementedError(f'Unsupported value {num_directions} for num_directions and operator {self.__class__.__name__!r}.')
