...
def plot_ppc(ax, length_plotters, rows, cols, figsize, animated, obs_plotters, pp_plotters, predictive_dataset, pp_sample_ix, kind, alpha, colors, textsize, mean, observed, observed_rug, jitter, total_pp_samples, legend, labeller, group, animation_kwargs, num_pp_samples, backend_kwargs, show):
    """Bokeh ppc plot."""
    ...
    if ax is None:
        axes = create_axes_grid(length_plotters, rows, cols, figsize=figsize, backend_kwargs=backend_kwargs)
    else:
        axes = np.atleast_2d(ax)
        if len([item for item in axes.ravel() if not None]) != length_plotters:
            raise ValueError(f'Found {length_plotters} variables to plot but {len(axes)} axes instances. They must be equal.')
    if alpha is None:
        if animated:
            alpha = 1
        elif kind.lower() == 'scatter':
            alpha = 0.7
        else:
            alpha = 0.2
    if jitter is None:
    ...
    for i, ax_i in enumerate((item for item in axes.flatten() if item is not None)):
        ...
        legend_it = []
        if dtype not in ['i', 'f']:
            raise ValueError(f"The data type of the predictive data must be one of 'i' or 'f', but is '{dtype}'")
        obs_vals = obs_vals.flatten()
        pp_vals = pp_vals.reshape(total_pp_samples, -1)
        pp_sampled_vals = pp_vals[pp_sample_ix]
        cds_rug = ColumnDataSource({'_': np.array(obs_vals)})
        if kind == 'kde':
            plot_kwargs = {'line_color': colors[0], 'line_alpha': alpha, 'line_width': 0.5 * linewidth}
            pp_densities = []
            pp_xs = []
            ...
            if observed:
                label = 'Observed'
                if dtype == 'f':
                    _, glyph = plot_kde(obs_vals, plot_kwargs={'line_color': colors[1], 'line_width': linewidth}, fill_kwargs={'alpha': 0}, ax=ax_i, backend='bokeh', backend_kwargs={}, show=False, return_glyph=True)
                    legend_it.append((label, glyph))
                    if observed_rug:
                        glyph = Scatter(x='_', y=0.0, marker='dash', angle=np.pi / 2, line_color=colors[1], line_width=linewidth)
                        ax_i.add_glyph(cds_rug, glyph)
                else:
                    bins = get_bins(obs_vals)
                    _, hist, bin_edges = histogram(obs_vals, bins=bins)
                    hist = np.concatenate((hist[:1], hist))
                    step = ax_i.step(bin_edges, hist, line_color=colors[1], line_width=linewidth, mode='center')
            ...
            if mean:
                ...
                if dtype == 'f':
                    ...
                    line = ax_i.line(new_x, new_d.mean(0), color=colors[2], line_dash='dashed', line_width=linewidth)
                    ...
                    _, hist, bin_edges = histogram(vals, bins=bins)
                    hist = np.concatenate((hist[:1], hist))
                    step = ax_i.step(bin_edges, hist, line_color=colors[2], line_width=linewidth, line_dash='dashed', mode='center')
                    legend_it.append((label, [step]))
            ...
            if observed:
                label = 'Observed'
                if dtype == 'f':
                    glyph = ax_i.line(*_empirical_cdf(obs_vals), line_color=colors[1], line_width=linewidth)
                    glyph.level = 'overlay'
                    legend_it.append((label, [glyph]))
                else:
                    step = ax_i.step(*_empirical_cdf(obs_vals), line_color=colors[1], line_width=linewidth, mode='center')
                    legend_it.append((label, [step]))
                if observed_rug:
                    glyph = Scatter(x='_', y=0.0, marker='dash', angle=np.pi / 2, line_color=colors[1], line_width=linewidth)
                    ax_i.add_glyph(cds_rug, glyph)
            ...
            multi_line = ax_i.multi_line(list(pp_densities[::2]), list(pp_densities[1::2]), line_alpha=alpha, line_color=colors[0], line_width=linewidth)
            legend_it.append((f'{group.capitalize()} predictive', [multi_line]))
            if mean:
                label = f'{group.capitalize()} predictive mean'
                line = ax_i.line(*_empirical_cdf(pp_vals.flatten()), color=colors[2], line_dash='dashed', line_width=linewidth)
                legend_it.append((label, [line]))
        elif kind == 'scatter':
            if mean:
                label = f'{group.capitalize()} predictive mean'
                if dtype == 'f':
                    _, glyph = plot_kde(pp_vals.flatten(), plot_kwargs={'line_color': colors[2], 'line_dash': 'dashed', 'line_width': linewidth}, ax=ax_i, backend='bokeh', backend_kwargs={}, show=False, return_glyph=True)
                    legend_it.append((label, glyph))
                else:
                    vals = pp_vals.flatten()
                    bins = get_bins(vals)
                    _, hist, bin_edges = histogram(vals, bins=bins)
                    hist = np.concatenate((hist[:1], hist))
                    step = ax_i.step(bin_edges, hist, color=colors[2], line_width=linewidth, line_dash='dashed', mode='center')
                    legend_it.append((label, [step]))
            jitter_scale = 0.1
            ...
            if observed:
                ...
                glyph = ax_i.circle(obs_vals, obs_yvals, line_color=colors[1], fill_color=colors[1], size=markersize, line_alpha=alpha)
                glyph.level = 'overlay'
                legend_it.append((label, [glyph]))
            all_scatter = []
            for vals, y in zip(pp_sampled_vals, y_rows[1:]):
                ...
                scatter = ax_i.scatter(vals, yvals, line_color=colors[0], fill_color=colors[0], size=markersize, fill_alpha=alpha)
