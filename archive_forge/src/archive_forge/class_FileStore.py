...
class FileStore(AbstractStore):
    ...
    def _validate_registered_model_does_not_exist(self, name):
        ...
        if exists(model_path):
            raise MlflowException(f'Registered Model (name={name}) already exists.', RESOURCE_ALREADY_EXISTS)
    ...
    def create_registered_model(self, name, tags=None, description=None):
        ...
        latest_versions = []
        registered_model = RegisteredModel(name=name, creation_timestamp=creation_time, last_updated_timestamp=creation_time, description=description, latest_versions=latest_versions, tags=tags)
        self._save_registered_model_as_meta_file(registered_model, meta_dir=meta_dir, overwrite=False)
        if tags is not None:
            for tag in tags:
    ...
    def rename_registered_model(self, name, new_name):
        ...
        if not exists(new_meta_dir):
            ...
            registered_model.last_updated_timestamp = updated_time
            self._save_registered_model_as_meta_file(registered_model, meta_dir=new_meta_dir, overwrite=False)
            ...
            for mv in model_versions:
                ...
                self._save_model_version_as_meta_file(mv, meta_dir=new_model_version_dir, overwrite=False)
                if mv.tags is not None:
                    for tag in mv.tags:
                        self.set_model_version_tag(new_name, mv.version, tag)
            shutil.rmtree(model_path)
        else:
            raise MlflowException(f'Registered Model (name={new_name}) already exists.', RESOURCE_ALREADY_EXISTS)
        return registered_model
    ...
    def get_latest_versions(self, name, stages=None) -> List[ModelVersion]:
        ...
        for mv in model_versions:
            if mv.current_stage in expected_stages:
                if mv.current_stage not in latest_versions or latest_versions[mv.current_stage].version < mv.version:
    ...
    def get_all_registered_model_aliases_from_path(self, model_path):
        parent_path, alias_files = self._get_resource_files(model_path, FileStore.REGISTERED_MODELS_ALIASES_FOLDER_NAME)
        aliases = []
    ...
    def _save_model_version_as_meta_file(self, model_version: FileModelVersion, meta_dir=None, overwrite=True):
        model_version_dict = dict(model_version)
        del model_version_dict['tags']
        meta_dir = meta_dir or self._get_model_version_dir(model_version.name, model_version.version)
        if overwrite:
            overwrite_yaml(meta_dir, FileStore.META_DATA_FILE_NAME, model_version_dict)
        else:
            write_yaml(meta_dir, FileStore.META_DATA_FILE_NAME, model_version_dict)

    def create_model_version(self, name, source, run_id=None, tags=None, run_link=None, description=None, local_model_path=None) -> ModelVersion:
        """
        ...
        for attempt in range(self.CREATE_MODEL_VERSION_RETRIES):
            try:
                ...
                model_version = FileModelVersion(name=name, version=version, creation_timestamp=creation_time, last_updated_timestamp=creation_time, description=description, current_stage=STAGE_NONE, source=source, run_id=run_id, run_link=run_link, tags=tags, aliases=[], storage_location=storage_location)
                model_version_dir = self._get_model_version_dir(name, version)
                mkdir(model_version_dir)
                self._save_model_version_as_meta_file(model_version, meta_dir=model_version_dir, overwrite=False)
                self._save_registered_model_as_meta_file(registered_model)
                if tags is not None:
                    for tag in tags:
                ...
                logging.warning('Model Version creation error (name=%s) Retrying %s more time%s.', name, str(more_retries), 's' if more_retries > 1 else '')
                if more_retries == 0:
                    raise MlflowException(f'Model Version creation error (name={name}). Error: {e}. Giving up after {self.CREATE_MODEL_VERSION_RETRIES} attempts.')
    ...
    def transition_model_version_stage(self, name, version, stage, archive_existing_versions) -> ModelVersion:
        """
    ...
    def search_model_versions(self, filter_string=None, max_results=None, order_by=None, page_token=None) -> List[ModelVersion]:
        """
        ...
        model_versions = []
        for path in registered_model_paths:
            model_versions.extend((file_mv.to_mlflow_entity() for file_mv in self._list_file_model_versions_under_path(path)))
        filtered_mvs = SearchModelVersionUtils.filter(model_versions, filter_string)
        sorted_mvs = SearchModelVersionUtils.sort(filtered_mvs, order_by or ['last_updated_timestamp DESC', 'name ASC', 'version_number DESC'])
        start_offset = SearchUtils.parse_start_offset_from_page_token(page_token)
        final_offset = start_offset + max_results
    ...
    def _get_registered_model_alias_path(self, name, alias):
        ...
        if not exists(registered_model_path):
            raise MlflowException(f'Registered Model with name={name} not found', RESOURCE_DOES_NOT_EXIST)
        return os.path.join(registered_model_path, FileStore.REGISTERED_MODELS_ALIASES_FOLDER_NAME, alias)
