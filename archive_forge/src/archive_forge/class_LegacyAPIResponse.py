    ...
    def _parse(self, *, to: type[_T] | None=None) -> R | _T:
        ...
        if self._stream:
            if to:
                ...
                return cast(_T, to(cast_to=extract_stream_chunk_type(to, failure_message='Expected custom stream type to be passed with a type argument, e.g. Stream[ChunkType]'), response=self.http_response, client=cast(Any, self._client)))
            if self._stream_cls:
                return cast(R, self._stream_cls(cast_to=extract_stream_chunk_type(self._stream_cls), response=self.http_response, client=cast(Any, self._client)))
