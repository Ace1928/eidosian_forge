...
class NetAppOntapSnapMirrorPolicy:
    ...
    def __init__(self):
        ...
        self.module = AnsibleModule(argument_spec=self.argument_spec, supports_check_mode=True, mutually_exclusive=[('copy_all_source_snapshots', 'copy_latest_source_snapshot', 'create_snapshot_on_source', 'sync_type')])
        self.na_helper = NetAppModule()
        self.parameters = self.na_helper.set_parameters(self.module.params)
        self.rest_api = netapp_utils.OntapRestAPI(self.module)
        unsupported_rest_properties = ['owner', 'restart', 'transfer_priority', 'tries', 'ignore_atime', 'common_snapshot_schedule']
        partially_supported_rest_properties = [['copy_all_source_snapshots', (9, 10, 1)], ['copy_latest_source_snapshot', (9, 11, 1)], ['create_snapshot_on_source', (9, 11, 1)]]
        self.unsupported_zapi_properties = ['identity_preservation', 'copy_all_source_snapshots', 'copy_latest_source_snapshot', 'sync_type', 'transfer_schedule']
    ...
    def format_record(self, record):
        return_value = {'uuid': record['uuid'], 'vserver': self.na_helper.safe_get(record, ['svm', 'name']), 'policy_name': record['name'], 'comment': '', 'is_network_compression_enabled': False, 'snapmirror_label': [], 'keep': [], 'prefix': [], 'schedule': [], 'identity_preservation': '', 'copy_all_source_snapshots': False, 'copy_latest_source_snapshot': False, 'transfer_schedule': ''}
        if 'type' in record:
    ...
    def validate_parameters(self):
        ...
        if 'snapmirror_label' in self.parameters:
            if 'policy_type' in self.parameters and self.parameters['policy_type'] == 'mirror_vault' and (len(self.parameters['snapmirror_label']) > 9) or len(self.parameters['snapmirror_label']) > 10:
                self.module.fail_json(msg="Error: A SnapMirror Policy can have up to a maximum of 10 rules (including builtin rules), with a 'keep' value representing the maximum number of Snapshot copies for each rule")
            if len(self.parameters['snapmirror_label']) > 0 and 'keep' not in self.parameters:
                self.module.fail_json(msg="Error: Missing 'keep' parameter. When specifying the 'snapmirror_label' parameter, the 'keep' parameter must also be supplied")
            for rule_parameter in ['keep', 'prefix', 'schedule']:
                if rule_parameter in self.parameters:
                    if len(self.parameters['snapmirror_label']) > len(self.parameters[rule_parameter]):
                        self.module.fail_json(msg="Error: Each 'snapmirror_label' value must have an accompanying '%s' value" % rule_parameter)
                    if len(self.parameters[rule_parameter]) > len(self.parameters['snapmirror_label']):
                        self.module.fail_json(msg="Error: Each '%s' value must have an accompanying 'snapmirror_label' value" % rule_parameter)
        else:
            for rule_parameter in ['keep', 'prefix', 'schedule']:
                if rule_parameter in self.parameters:
                    self.module.fail_json(msg="Error: Missing 'snapmirror_label' parameter. When specifying the '%s' parameter, the 'snapmirror_label' parameter must also be supplied" % rule_parameter)
        if 'prefix' in self.parameters and 'schedule' not in self.parameters:
            self.module.fail_json(msg="Error: Missing 'schedule' parameter. When specifying the 'prefix' parameter, the 'schedule' parameter must also be supplied")
        ...
        if self.use_rest:
            ...
            if snapmirror_policy_obj.get_children():
                ...
                try:
                    ...
                    self.module.fail_json(msg='Error modifying snapmirror policy %s: %s' % (self.parameters['policy_name'], to_native(error)), exception=traceback.format_exc())
        ...
        if current and cd_action is None and (self.parameters['state'] == 'present'):
            ...
            for property in ('policy_type', 'copy_all_source_snapshots', 'copy_latest_source_snapshot', 'sync_type', 'create_snapshot_on_source'):
                if property in modify:
                    self.module.fail_json(msg='Error: The policy property %s cannot be modified from %s to %s' % (property, current.get(property), modify[property]))
        body = None
        modify_body = any((key not in ('keep', 'prefix', 'schedule', 'snapmirror_label', 'copy_all_source_snapshots', 'copy_latest_source_snapshot', 'sync_type', 'create_snapshot_on_source') for key in modify)) if modify else False
