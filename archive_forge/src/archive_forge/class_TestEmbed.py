...
class TestEmbed:
    ...
    def test_escape(self):
        ...
        code = embed_snippet(w)
        assert code.find('<script A>') == -1
        assert code.find('\\u003cscript A> \\u003cScRipt> \\u003c/Script> \\u003c!-- --> <b>hi</b>') >= 0
        f = StringIO()
        embed_minimal_html(f, w)
        content = f.getvalue()
        assert content.find('<script A>') == -1
        assert content.find('\\u003cscript A> \\u003cScRipt> \\u003c/Script> \\u003c!-- --> <b>hi</b>') >= 0
