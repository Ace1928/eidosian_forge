...
def get_agent_connectors_from_config(ctx: ConnectorContext, config: 'AlgorithmConfig') -> AgentConnectorPipeline:
    connectors = []
    ...
    filter_connector = get_synced_filter_connector(ctx)
    if filter_connector:
        connectors.append(filter_connector)
    connectors.extend([StateBufferConnector(ctx), ViewRequirementAgentConnector(ctx)])
