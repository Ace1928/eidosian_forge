...
class KrausChannel(raw_types.Gate):
    ...
    def __init__(self, kraus_ops: Iterable[np.ndarray], key: Union[str, 'cirq.MeasurementKey', None]=None, validate: bool=False):
        ...
        if not num_qubits.is_integer() or kraus_ops[0].shape[1] != kraus_ops[0].shape[0]:
            raise ValueError(f'Input Kraus ops of shape {kraus_ops[0].shape} does not represent a square operator over qubits.')
        self._num_qubits = int(num_qubits)
        for i, op in enumerate(kraus_ops):
            if not op.shape == kraus_ops[0].shape:
                raise ValueError(f'Inconsistent Kraus operator shapes: op[0]: {kraus_ops[0].shape}, op[{i}]: {op.shape}')
    ...
    def _with_key_path_prefix_(self, prefix: Tuple[str, ...]):
        return KrausChannel(kraus_ops=self._kraus_ops, key=protocols.with_key_path_prefix(self._key, prefix))

    def _with_rescoped_keys_(self, path: Tuple[str, ...], bindable_keys: FrozenSet['cirq.MeasurementKey']):
        return KrausChannel(kraus_ops=self._kraus_ops, key=protocols.with_rescoped_keys(self._key, path, bindable_keys))
