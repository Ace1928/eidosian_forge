...
class OpenVINOReranker(BaseDocumentCompressor):
    ...
    def __init__(self, **kwargs: Any):
        ...
        if require_model_export(self.model_name_or_path):
            self.ov_model = OVModelForSequenceClassification.from_pretrained(self.model_name_or_path, export=True, **self.model_kwargs)
        else:
    ...
    def compress_documents(self, documents: Sequence[Document], query: str, callbacks: Optional[Callbacks]=None) -> Sequence[Document]:
        ...
        for r in rerank_response:
            doc = Document(page_content=r['text'], metadata={'id': r['id'], 'relevance_score': r['score']})
