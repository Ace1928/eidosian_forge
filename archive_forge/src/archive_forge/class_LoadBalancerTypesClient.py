...
class LoadBalancerTypesClient(ClientEntityBase):
    ...
    def get_by_id(self, id: int) -> BoundLoadBalancerType:
        ...
        """
        response = self._client.request(url=f'/load_balancer_types/{id}', method='GET')
        return BoundLoadBalancerType(self, response['load_balancer_type'])

    def get_list(self, name: str | None=None, page: int | None=None, per_page: int | None=None) -> LoadBalancerTypesPageResult:
        ...
        return LoadBalancerTypesPageResult(load_balancer_types, Meta.parse_meta(response))
