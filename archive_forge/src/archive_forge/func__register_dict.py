    ...
    if self.notify:
        if self.type == DST_LISTENER:
            ...
            if handler is not Undefined:
                ...
                if self.type == ANY_LISTENER:
                    object._on_trait_change(handler, name + '_items', remove=remove, dispatch=self.dispatch, priority=self.priority, target=self._get_target())
    object._on_trait_change(tl_handler, name, remove=remove, dispatch=self.dispatch, priority=self.priority, target=self._get_target())
    object._on_trait_change(tl_handler_items, name + '_items', remove=remove, dispatch=self.dispatch, priority=self.priority, target=self._get_target())
    if remove:
