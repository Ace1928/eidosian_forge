...
def test_role_create_with_immutable_option(self):
    ...
    arglist = ['--immutable', identity_fakes.ROLE_2['name']]
    verifylist = [('immutable', True), ('name', identity_fakes.ROLE_2['name'])]
    parsed_args = self.check_parser(self.cmd, arglist, verifylist)
    columns, data = self.cmd.take_action(parsed_args)
    kwargs = {'options': {'immutable': True}, 'description': None, 'name': identity_fakes.ROLE_2['name'], 'domain': None}
