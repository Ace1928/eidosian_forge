...
class StructuredRawSQL:
    ...
    def construct(self, name_map: Union[None, Callable[[str], str], Dict[str, str]]=None, dialect: Optional[str]=None, log: Optional[Logger]=None):
        ...
        """
        nm: Any = (lambda x: x) if name_map is None else name_map if not isinstance(name_map, dict) else lambda x: name_map.get(x, x)
        raw_sql = ' '.join((nm(tp[1]) if tp[0] else tp[1] for tp in self._statements))
        if self._dialect is not None and dialect is not None and (self._dialect != dialect):
            tsql = transpile_sql(raw_sql, self._dialect, dialect)
            if log is not None:
                log.debug('SQL transpiled from %s to %s\n\nOriginal:\n\n%s\n\nTranspiled:\n\n%s\n', self._dialect, dialect, raw_sql, tsql)
            return tsql
        return raw_sql

    @staticmethod
    def from_expr(sql: str, prefix: str=_TEMP_TABLE_EXPR_PREFIX, suffix: str=_TEMP_TABLE_EXPR_SUFFIX, dialect: Optional[str]=None) -> 'StructuredRawSQL':
