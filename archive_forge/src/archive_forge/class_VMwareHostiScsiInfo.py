...
class VMwareHostiScsiInfo(PyVmomi):
    ...
    def get_iscsi_config(self):
        ...
        self.existing_system_iscsi_config = {'iscsi_enabled': iscsi_enabled}
        ...
        if iscsi_enabled is True:
            ...
            for lun in self.host_obj.config.storageDevice.scsiLun:
                if isinstance(lun, vim.host.ScsiDisk):
                    detected_iscsi_drives_information.append({'key': lun.key, 'canonical_name': lun.canonicalName})
                        ...
                        for target in scsi_adapter.target:
                            ...
                            for scsi_info in detected_iscsi_drives_information:
                                if scsi_info['key'] == scsi_lun:
                                    self.detected_iscsi_drives.append({'iscsi_name': target.transport.iScsiName, 'canonical_name': scsi_info['canonical_name'], 'address': target.transport.address})
            vnic_devices = []
