...
def ensure_system_resources():
    ...
    logging.info(f"Swap memory free: {swap_info.free / (1024 ** 2):.2f} MB")
    print(f"Swap memory free: {swap_info.free / (1024 ** 2):.2f} MB")
    # Dynamic adjustment based on system load
    if cpu_percent > 80:
        logging.warning("High CPU usage detected, reducing processing load...")
        ...
        logging.info("Processing load reduced by downsampling audio.")
        print("Processing load reduced by downsampling audio.")
    ...
    if memory_info.available < memory_info.total * 0.15:
        logging.warning("Very low memory detected, further optimizing memory usage...")
        print("Very low memory detected, further optimizing memory usage...")
        # Further memory optimization logic
        logging.info("Further memory optimization complete.")
        print("Further memory optimization complete.")

    if cpu_percent > 85:
        logging.warning(
            "Very high CPU usage detected, further reducing processing load..."
        )
        ...
        logging.info("Further CPU load reduction complete.")
...
def create_tensor_wave(signal: np.ndarray, sr: int) -> dict:
    ...
    # Construct the dictionary of tensor representations
    tensor_wave_dict = {
    ...
    # Normalize and scale each tensor
    scaler = MinMaxScaler()
