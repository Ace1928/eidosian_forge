...
class Deploy(base.Command):
    ...
    detailed_help = {'DESCRIPTION': '          Creates or updates a Cloud Run job.\n          ', 'EXAMPLES': '          To deploy a new job `my-data-transformation` to Cloud Run:\n\n              $ {command} my-data-transformation --image=us-docker.pkg.dev/project/image\n\n          You may also omit the job name. Then a prompt will be displayed\n          with a suggested default value:\n\n              $ {command} --image=us-docker.pkg.dev/project/image\n          '}

    @staticmethod
    def CommonArgs(parser, add_container_args=True):
        job_presentation = presentation_specs.ResourcePresentationSpec('JOB', resource_args.GetJobResourceSpec(prompt=True), 'Job to deploy.', required=True, prefixes=False)
        ...
        if add_container_args:
            ...
            flags.AddSourceAndImageFlags(parser, image='us-docker.pkg.dev/cloudrun/container/job:latest')
        ...
        execute_group = polling_group.add_argument_group(help='--async cannot be used if executing the job after the update.')
    ...
    def Run(self, args):
        ...
        if len(containers) > 10:
            raise c_exceptions.InvalidArgumentException('--container', 'Jobs may include at most 10 containers')
        ...
        for name, container in build_from_source.items():
            if not flags.FlagIsExplicitlySet(container, 'source'):
                if console_io.CanPrompt():
                    ...
                    if name:
                        message = 'Container {} requires a container image to deploy (e.g. `us-docker.pkg.dev/cloudrun/container/job:latest`) if no  build source is provided.'.format(name)
                    else:
                        message = 'Requires a container image to deploy (e.g. `us-docker.pkg.dev/cloudrun/container/job:latest`) if no  build source is provided.'
        ...
        already_activated_services = api_enabler.check_and_enable_apis(properties.VALUES.core.project.Get(), required_apis)
        ...
        if build_from_source:
            ...
            container.image = '{repo}/{job}'.format(repo=ar_repo.GetDockerString(), job=job_ref.jobsId)
            ...
            if os.path.exists(docker_file):
                ...
                if self.ReleaseTrack() is base.ReleaseTrack.ALPHA:
                    ...
                    if command_arg is not None:
                        ...
                        pack[0].update({'env': 'GOOGLE_ENTRYPOINT="{command}"'.format(command=command)})
            ...
            pretty_print.Info(messages_util.GetBuildEquivalentForSourceRunMessage(job_ref.jobsId, pack, source, subgroup='jobs '))
        changes = flags.GetJobConfigurationChanges(args, release_track=self.ReleaseTrack())
        ...
        with serverless_operations.Connect(conn_context) as operations:
            ...
            pretty_print.Info(messages_util.GetStartDeployMessage(conn_context, job_ref, operation_message, 'job'))
            ...
            with progress_tracker.StagedProgressTracker(header_msg, stages.JobStages(execute_now=execute_now, include_completion=args.wait, include_build=bool(build_from_source), include_create_repo=repo_to_create is not None), failure_message='Job failed to deploy', suppress_output=args.async_) as tracker:
                job = operations.DeployJob(job_ref, changes, self.ReleaseTrack(), tracker, asyn=args.async_ and (not execute_now), build_image=image, build_pack=pack, build_source=source, repo_to_create=repo_to_create, prefetch=job_obj, already_activated_services=already_activated_services)
                if execute_now:
                    execution = operations.RunJob(job_ref, tracker, args.wait, args.async_, self.ReleaseTrack())
            if args.async_ and (not execute_now):
                pretty_print.Success('Job [{{bold}}{job}{{reset}}] is being deployed asynchronously.'.format(job=job.name))
            else:
                operation = 'been deployed'
                if args.wait:
                    operation += ' and completed execution [{}]'.format(execution.name)
                elif execute_now:
                    operation += ' and started running execution [{}]'.format(execution.name)
                pretty_print.Success('Job [{{bold}}{job}{{reset}}] has successfully {operation}.'.format(job=job.name, operation=operation))
            msg = ''
            if execute_now:
                msg += messages_util.GetExecutionCreatedMessage(self.ReleaseTrack(), execution)
                msg += '\n'
            msg += messages_util.GetRunJobMessage(self.ReleaseTrack(), job.name, repeat=execute_now)
