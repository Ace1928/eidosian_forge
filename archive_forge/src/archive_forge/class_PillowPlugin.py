...
class PillowPlugin(PluginV3):
        ...
        if request.mode.io_mode == IOMode.read:
            try:
                ...
                if request._uri_type == URI_BYTES:
                    raise InitializationError('Pillow can not read the provided bytes.') from None
                else:
                    raise InitializationError(f'Pillow can not read {request.raw_uri}.') from None
            self._image = Image.open(self._request.get_file())
        else:
            self.save_args = {}
            extension = self.request.extension or self.request.format_hint
            if extension is None:
                warnings.warn("Can't determine file format to write as. You _must_ set `format` during write or the call will fail. Use `extension` to supress this warning. ", UserWarning)
                return
            ...
            for format_loader in tirage:
                ...
                if extension in Image.registered_extensions().keys():
                    return
            raise InitializationError(f'Pillow can not write `{extension}` files.') from None
    ...
    def read(self, *, index: int=None, mode: str=None, rotate: bool=False, apply_gamma: bool=False, writeable_output: bool=True, pilmode: str=None, exifrotate: bool=None, as_gray: bool=None) -> np.ndarray:
        ...
        if pilmode is not None:
            warnings.warn('`pilmode` is deprecated. Use `mode` instead.', DeprecationWarning)
            mode = pilmode
        if exifrotate is not None:
            warnings.warn('`exifrotate` is deprecated. Use `rotate` instead.', DeprecationWarning)
            rotate = exifrotate
        if as_gray is not None:
            raise TypeError("The keyword `as_gray` is no longer supported.Use `mode='F'` for a backward-compatible result, or  `mode='L'` for an integer-valued result.")
        ...
        if isinstance(index, int):
            ...
            image = self._apply_transforms(self._image, mode, rotate, apply_gamma, writeable_output)
        else:
            iterator = self.iter(mode=mode, rotate=rotate, apply_gamma=apply_gamma, writeable_output=writeable_output)
    ...
    def iter(self, *, mode: str=None, rotate: bool=False, apply_gamma: bool=False, writeable_output: bool=True) -> Iterator[np.ndarray]:
        ...
        for im in ImageSequence.Iterator(self._image):
            yield self._apply_transforms(im, mode, rotate, apply_gamma, writeable_output)
    ...
    def write(self, ndimage: Union[ArrayLike, List[ArrayLike]], *, mode: str=None, format: str=None, is_batch: bool=None, **kwargs) -> Optional[bytes]:
        """
        ...
        """
        if 'fps' in kwargs:
            warnings.warn('The keyword `fps` is no longer supported. Use `duration`(in ms) instead, e.g. `fps=50` == `duration=20` (1000 * 1/50).', DeprecationWarning)
        ...
        if is_batch is not None:
            ...
            is_batch = ndimage.ndim > 3 if Image.getmodebands(mode) > 1 else ndimage.ndim > 2
        elif ndimage.ndim == 2:
            is_batch = False
        ...
        if format is not None and 'format' in self.save_args and (self.save_args['format'] != format):
            old_format = self.save_args['format']
            warnings.warn(f'Changing the output format during incremental writes is strongly discouraged. Was `{old_format}`, is now `{format}`.', UserWarning)
        extension = self.request.extension or self.request.format_hint
    ...
    def metadata(self, index: int=None, exclude_applied: bool=True) -> Dict[str, Any]:
        ...
        if self._image.getexif():
            exif_data = {ExifTags.TAGS.get(key, 'unknown'): value for key, value in dict(self._image.getexif()).items()}
    ...
    def properties(self, index: int=None) -> ImageProperties:
        ...
        return ImageProperties(shape=shape, dtype=dummy.dtype, n_images=n_frames, is_batch=index is Ellipsis)