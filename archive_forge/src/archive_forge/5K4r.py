...
# Configure logging with detailed settings for maximum verbosity and utility
logging.basicConfig(
...
class BFS(Algorithm):
    """
    Implements the Breadth-First Search (BFS) algorithm for pathfinding in a grid-based game environment.
    ...
    def __init__(self, grid: List[List[int]]) -> None:
        """
        Initialize the BFS algorithm instance with the grid.
    ...
    def run_algorithm(self, snake) -> Optional[Node]:
        """
        Execute the BFS algorithm to find the path for the snake to follow.

        Args:
            snake: The snake object for which the path is being calculated.

        Returns:
            Optional[Node]: The next node in the path for the snake to follow, if a path exists.
        """
