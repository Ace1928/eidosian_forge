...
class FSimGateFamily(cirq.GateFamily):
    ...
    def __init__(self, *, gates_to_accept: Sequence[Union[Type[POSSIBLE_FSIM_GATES], POSSIBLE_FSIM_GATES]]=(), gate_types_to_check: Sequence[Type[POSSIBLE_FSIM_GATES]]=(), allow_symbols: bool=False, atol=DEFAULT_ATOL):
        ...
        Raises:
            ...
            ValueError: If any gate type in `gate_types_to_check` is not one of
                `POSSIBLE_FSIM_GATES`.
        """
        self._supported_types: Dict[Type[POSSIBLE_FSIM_GATES], Callable[[POSSIBLE_FSIM_GATES], Optional[POSSIBLE_FSIM_GATES]]] = {cirq.FSimGate: self._convert_to_fsim, cirq.PhasedFSimGate: self._convert_to_phased_fsim, cirq.ISwapPowGate: self._convert_to_iswap, cirq.PhasedISwapPowGate: self._convert_to_phased_iswap, cirq.CZPowGate: self._convert_to_cz, cirq.IdentityGate: self._convert_to_identity}
        ...
        if any((g not in self._supported_types for g in gate_types_to_check)):
            raise ValueError(f'All gates in gate_types_to_check: {_gates_to_str(gate_types_to_check)} must be one of {_gates_to_str(self._supported_types.keys())}.')
        for g in gates_to_accept:
            if isinstance(g, tuple(self._supported_types.keys())):
                if cirq.is_parameterized(g):
                    raise ValueError(f'Parameterized gate {g} cannot be used in `gates_to_accept` initializer.')
            elif g not in self._supported_types:
                raise ValueError(f'Gate {g} in `gates_to_accept` must be either a type from or an instance of {_gates_to_str(self._supported_types.keys())}')
    ...
    def __repr__(self) -> str:
        ...
        return f'cirq_google.FSimGateFamily(gates_to_accept={_gates_to_str(self.gates_to_accept, _gate_repr)}, gate_types_to_check={_gates_to_str(self.gate_types_to_check, _gate_repr)}, allow_symbols={self.allow_symbols}, atol={self.atol})'

    def _value_equality_values_(self) -> Any:
        return (frozenset(self.gates_to_accept), frozenset(self.gate_types_to_check), self.allow_symbols, self.atol)

    def _json_dict_(self):
        accept_gates_json = [gate if not isinstance(gate, type) else cirq.json_cirq_type(gate) for gate in self.gates_to_accept]
        check_gates_json = [cirq.json_cirq_type(gate) for gate in self.gate_types_to_check]
        return {'gates_to_accept': accept_gates_json, 'gate_types_to_check': check_gates_json, 'allow_symbols': self.allow_symbols, 'atol': self.atol}

    @classmethod
    def _from_json_dict_(cls, gates_to_accept, gate_types_to_check, allow_symbols, atol, **kwargs):
        accept_gates = [gate if not isinstance(gate, str) else cirq.cirq_type_from_json(gate) for gate in gates_to_accept]
        check_gates = [cirq.cirq_type_from_json(gate) for gate in gate_types_to_check]
        return cls(gates_to_accept=accept_gates, gate_types_to_check=check_gates, allow_symbols=allow_symbols, atol=atol)
    ...
    def _predicate(self, gate: cirq.Gate) -> bool:
        ...
        for g in self.gates_to_accept:
            if isinstance(g, type):
                cg = self.convert(gate, cast(type, g))
                if cg is not None:
                    return True
    ...
    def _convert_to_phased_iswap(self, g: POSSIBLE_FSIM_GATES) -> Optional[cirq.PhasedISwapPowGate]:
        if isinstance(g, cirq.PhasedISwapPowGate):
            return g
        if isinstance(g, cirq.PhasedFSimGate) and self._approx_eq_or_symbol((g.zeta, g.gamma, g.phi), (0, 0, 0)):
            return cirq.PhasedISwapPowGate(exponent=-2 * g.theta / np.pi, phase_exponent=g.chi / (2 * np.pi))
        cg = self._convert_to_iswap(g)
        return None if cg is None else cirq.PhasedISwapPowGate(exponent=cg.exponent, phase_exponent=0)
