...
class TokenGenerator(object):
  ...
  """
    ...
    def scope(self, node, attr=None, trailing_comma=False):
        ...
        yield
        if attr:
            self.close_scope(node, prefix_attr=attr + '_prefix', suffix_attr=attr + '_suffix', trailing_comma=trailing_comma, single_paren=True)
        else:
    ...
    def str(self):
        ...
        def predicate(token):
            return token.type in (TOKENS.STRING, TOKENS.COMMENT) or (self.is_in_scope() and token.type in (TOKENS.NL, TOKENS.NEWLINE))
