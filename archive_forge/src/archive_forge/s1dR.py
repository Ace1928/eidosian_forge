...
class StandardDecorator:
    ...
    async def validate_arguments(self, func: F, *args, **kwargs) -> None:
        ...
        This method adjusts for whether the function is a bound method (instance or class method) or a regular function
        or static method, and applies argument validation accordingly.

        Args:
            func (F): The function whose arguments are to be validated.
            *args: Positional arguments passed to the function.
            **kwargs: Keyword arguments passed to the function.

        Raises:
            TypeError: If an argument does not match its expected type hint.
        ...
        # Determine if the function is a bound method and adjust arguments accordingly
        if inspect.ismethod(func) or (
            hasattr(func, "__self__") and func.__self__ is not None
        ):
            # Exclude 'self' or 'cls' from args for bound methods
            bound_args = inspect.signature(func).bind_partial(*args[1:], **kwargs)
        else:
            # Directly bind args and kwargs for functions and static methods
        ...
        # Iterate through each argument to perform validation
        for name, value in bound_args.arguments.items():
            # Retrieve the expected type for the current argument
            expected_type = type_hints.get(name)
            ...
            # Raise TypeError if value does not match expected type
            if expected_type and not isinstance(value, expected_type):
                raise TypeError(
                    f"Argument '{name}' must be of type '{expected_type.__name__}', got '{type(value).__name__}'"
                )

            # Retrieve and apply custom validation rule if present
            validation_rule = self.validation_rules.get(name)
            if validation_rule:
                # Support asynchronous validation rules
                valid = (
        ...
        # Record the end time and log the validation completion
        end_time = asyncio.get_event_loop().time()
        logging.debug(
