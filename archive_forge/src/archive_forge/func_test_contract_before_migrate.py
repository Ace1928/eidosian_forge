...
def test_contract_before_migrate(self, mock_validate_engine, mock_get_alembic_branch_head, mock_get_curr_alembic_heads, mock_has_pending_migrations):
    ...
    self.assertIn('Database contraction did not run. Database contraction cannot be run before data migration is complete. Run data migration using "glance-manage db migrate".', exit.code)