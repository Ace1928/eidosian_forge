...
class BaseVisitor(ast.NodeVisitor):
  ...
  """
    ...
    def suffix(self, node, max_lines=None, semicolon=False, comment=False, default=''):
    ...
    """
        if attr:
            self.attr(node, attr + '_prefix', [], default='(' if default_parens else '')
        yield
        if attr:
            self.attr(node, attr + '_suffix', [], default=')' if default_parens else '')
    ...
    def visit_If(self, node):
        ...
        self.attr(node, 'open_block', [self.ws, ':', self.ws_oneline], default=':\n')
        for stmt in self.indented(node, 'body'):
            self.visit(stmt)
        if node.orelse:
            if len(node.orelse) == 1 and isinstance(node.orelse[0], ast.If) and self.check_is_elif(node.orelse[0]):
                fmt.set(node.orelse[0], 'is_elif', True)
                self.visit(node.orelse[0])
            else:
                self.attr(node, 'elseprefix', [self.ws])
                self.token('else')
                self.attr(node, 'open_else', [self.ws, ':', self.ws_oneline], default=':\n')
    ...
    def visit_While(self, node):
        ...
        for stmt in self.indented(node, 'body'):
            self.visit(stmt)
        if node.orelse:
            self.attr(node, 'else', [self.ws, 'else', self.ws, ':', self.ws_oneline], default=self._indent + 'else:\n')
            for stmt in self.indented(node, 'orelse'):
                self.visit(stmt)
    ...
    def visit_For(self, node):
        if hasattr(ast, 'AsyncFor') and isinstance(node, ast.AsyncFor):
            self.attr(node, 'for_keyword', ['async', self.ws, 'for', self.ws], default='async for ')
        else:
    ...
    def visit_ClassDef(self, node):
        ...
        self.attr(node, 'class_def', ['class', self.ws, node.name, self.ws], default='class %s' % node.name, deps=('name',))
        class_args = getattr(node, 'bases', []) + getattr(node, 'keywords', [])
        with self.scope(node, 'bases', trailing_comma=bool(class_args), default_parens=True):
    ...
    def visit_FunctionDef(self, node):
        for i, decorator in enumerate(node.decorator_list):
            self.attr(node, 'decorator_symbol_%d' % i, [self.ws, '@', self.ws], default='@')
            self.visit(decorator)
            self.attr(node, 'decorator_suffix_%d' % i, [self.ws_oneline], default='\n' + self._indent)
        if hasattr(ast, 'AsyncFunctionDef') and isinstance(node, ast.AsyncFunctionDef):
            self.attr(node, 'function_def', [self.ws, 'async', self.ws, 'def', self.ws, node.name, self.ws], deps=('name',), default='async def %s' % node.name)
        else:
            self.attr(node, 'function_def', [self.ws, 'def', self.ws, node.name, self.ws], deps=('name',), default='def %s' % node.name)
        kwonlyargs = getattr(node.args, 'kwonlyargs', [])
        args_count = sum((len(node.args.args + kwonlyargs), 1 if node.args.vararg else 0, 1 if node.args.kwarg else 0))
        with self.scope(node, 'args', trailing_comma=args_count > 0, default_parens=True):
            self.visit(node.args)
        if getattr(node, 'returns', None):
            self.attr(node, 'returns_prefix', [self.ws, '->', self.ws], deps=('returns',), default=' -> ')
    ...
    def visit_TryFinally(self, node):
        self.attr(node, 'open_try', ['try', self.ws, ':', self.ws_oneline], default='try:\n')
        if len(node.body) == 1 and self.check_is_continued_try(node.body[0]):
            ...
            for stmt in self.indented(node, 'body'):
                self.visit(stmt)
        self.attr(node, 'open_finally', [self.ws, 'finally', self.ws, ':', self.ws_oneline], default='finally:\n')
    ...
    def visit_TryExcept(self, node):
        ...
        for handler in node.handlers:
            self.visit(handler)
        if node.orelse:
            self.attr(node, 'open_else', [self.ws, 'else', self.ws, ':', self.ws_oneline], default='else:\n')
            for stmt in self.indented(node, 'orelse'):
                self.visit(stmt)

    @block_statement
    def visit_Try(self, node):
        self.attr(node, 'open_try', [self.ws, 'try', self.ws, ':', self.ws_oneline], default='try:\n')
        for stmt in self.indented(node, 'body'):
            self.visit(stmt)
        for handler in node.handlers:
            self.visit(handler)
    ...
    def visit_ExceptHandler(self, node):
        ...
        if node.type and node.name:
            self.attr(node, 'as', [self.ws, self.one_of_symbols('as', ','), self.ws], default=' as ')
        if node.name:
    ...
    def visit_AugAssign(self, node):
        ...
        self.attr(node, 'operator', [self.ws, op_token, self.ws], default=' %s ' % op_token)
        self.visit(node.value)
    ...
    def visit_Exec(self, node):
        ...
        with self.scope(node, 'body', trailing_comma=False, default_parens=True):
            ...
            if node.globals:
                self.attr(node, 'in_globals', [self.ws, self.one_of_symbols('in', ','), self.ws], default=', ')
    ...
    def visit_ImportFrom(self, node):
        ...
        self.attr(node, 'module', module_pattern, deps=('level', 'module'), default='.' * node.level + (node.module or ''))
    ...
    def visit_YieldFrom(self, node):
        self.attr(node, 'yield_from', ['yield', self.ws, 'from', self.ws], default='yield from ')
        self.visit(node.value)
    ...
    def visit_BinOp(self, node):
        ...
        self.attr(node, 'op', [self.ws, op_symbol, self.ws], default=' %s ' % op_symbol, deps=('op',))
    ...
    def visit_BoolOp(self, node):
        ...
        for i, value in enumerate(node.values):
            self.visit(value)
            if value is not node.values[-1]:
                self.attr(node, 'op_%d' % i, [self.ws, op_symbol, self.ws], default=' %s ' % op_symbol, deps=('op',))
    ...
    def visit_Dict(self, node):
        ...
        for i, key, value in zip(range(len(node.keys)), node.keys, node.values):
            if key is None:
                ...
                self.visit(key)
                self.attr(node, 'key_val_sep_%d' % i, [self.ws, ':', self.ws], default=': ')
            self.visit(value)
            if value is not node.values[-1]:
                self.attr(node, 'comma_%d' % i, [self.ws, ',', self.ws], default=', ')
        ...
        with self.scope(node, 'elts', default_parens=True):
            for i, elt in enumerate(node.elts):
                ...
                if elt is not node.elts[-1]:
                    ...
                    self.optional_token(node, 'extracomma', ',', allow_whitespace_prefix=True, default=len(node.elts) == 1)
    ...
    def visit_arg(self, node):
        ...
        if node.annotation is not None:
            self.attr(node, 'annotation_prefix', [self.ws, ':', self.ws], default=': ')
            self.visit(node.annotation)
    ...
    def visit_arguments(self, node):
        ...
        total_args = sum((len(node.args + kwonlyargs), len(getattr(node, 'posonlyargs', [])), 1 if node.vararg else 0, 1 if node.kwarg else 0))
        arg_i = 0
        ...
        for arg in positional:
            ...
            arg_i += 1
            if arg_i < total_args:
                self.attr(node, 'comma_%d' % arg_i, [self.ws, ',', self.ws], default=', ')
            if arg_i == len(getattr(node, 'posonlyargs', [])):
                self.attr(node, 'posonly_sep', [self.ws, '/', self.ws, ',', self.ws], default='/, ')
        ...
        for i, (arg, default) in enumerate(zip(kwonlyargs, kw_defaults)):
            ...
            if default is not None:
                self.attr(node, 'kw_default_%d' % i, [self.ws, '=', self.ws], default='=')
                self.visit(default)
            arg_i += 1
            if arg_i < total_args:
                self.attr(node, 'comma_%d' % arg_i, [self.ws, ',', self.ws], default=', ')
    ...
    def visit_comprehension(self, node):
        if getattr(node, 'is_async', False):
            self.attr(node, 'for', [self.ws, 'async', self.ws, 'for', self.ws], default=' async for ')
        else:
    ...
    def visit_FormattedValue(self, node):
        ...
        if node.conversion != -1:
            self.attr(node, 'conversion', [self.ws, '!', chr(node.conversion)], deps=('conversion',), default='!%c' % node.conversion)
        if node.format_spec:
            self.attr(node, 'format_spec_prefix', [self.ws, ':', self.ws], default=':')
