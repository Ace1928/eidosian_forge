...
class Object3D(BatchableMedia):
    ...
    def __init__(self, data_or_path: Union['np.ndarray', str, 'TextIO', dict], **kwargs: Optional[Union[str, 'FileFormat3D']]) -> None:
        super().__init__()
        ...
        if hasattr(data_or_path, 'read'):
            ...
            with codecs.open(tmp_path, 'w', encoding='utf-8') as fp:
                json.dump(data, fp, separators=(',', ':'), sort_keys=True, indent=4)
            ...
            tmp_path = os.path.join(MEDIA_TMP.name, runid.generate_id() + '.pts.json')
            with codecs.open(tmp_path, 'w', encoding='utf-8') as fp:
                json.dump(list_data, fp, separators=(',', ':'), sort_keys=True, indent=4)
            self._set_file(tmp_path, is_tmp=True, extension='.pts.json')
        else:
            raise ValueError('data must be a numpy array, dict or a file object')

    @classmethod
    def from_file(cls, data_or_path: Union['TextIO', str], file_type: Optional['FileFormat3D']=None) -> 'Object3D':
    ...
    @classmethod
    def seq_to_json(cls: Type['Object3D'], seq: Sequence['BatchableMedia'], run: 'LocalRun', key: str, step: Union[int, str]) -> dict:
        seq = list(seq)
