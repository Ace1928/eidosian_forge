...
class OvhNodeDriver(NodeDriver):
    ...
    def create_node(self, name, image, size, location, ex_keyname=None):
        ...
        """
        action = self._get_project_action('instance')
        data = {'name': name, 'imageId': image.id, 'flavorId': size.id, 'region': location.id}
        if ex_keyname:
    ...
    def create_volume(self, size, name, location, snapshot=None, ex_volume_type='classic', ex_description=None):
        """
        Create a volume.
        ...
        data = {'name': name, 'region': location.id, 'size': size, 'type': ex_volume_type}
        if ex_description:
    ...
    def detach_volume(self, volume, ex_node=None):
        ...
        if ex_node is None:
            if len(volume.extra['attachedTo']) != 1:
                err_msg = "Volume '%s' has more or less than one attachednodes, you must specify one."
    ...
    def ex_get_pricing(self, size_id, subsidiary='US'):
        ...
        return {'hourly': pricing['price']['value'], 'monthly': pricing['monthlyPrice']['value']}
    ...
    def _to_volume(self, obj):
        ...
        return StorageVolume(id=obj['id'], name=obj['name'], size=obj['size'], state=state, extra=extra, driver=self)
    ...
    def _to_node(self, obj):
        ...
        del extra['id']
        del extra['name']
        return Node(id=obj['id'], name=obj['name'], state=self.NODE_STATE_MAP[obj['status']], public_ips=public_ips, private_ips=[], driver=self, extra=extra)
    ...
    def _to_size(self, obj):
        ...
        return NodeSize(id=obj['id'], name=obj['name'], ram=obj['ram'], disk=obj['disk'], bandwidth=obj['outboundBandwidth'], price=None, driver=self, extra=extra)
    ...
    def _to_key_pair(self, obj):
        ...
        return OpenStackKeyPair(name=obj['name'], public_key=obj['publicKey'], driver=self, fingerprint=None, extra=extra)

    def _to_key_pairs(self, objs):
        return [self._to_key_pair(obj) for obj in objs]

    def _to_snapshot(self, obj):
        extra = {'volume_id': obj['volumeId'], 'region': obj['region'], 'description': obj['description'], 'status': obj['status']}
        state = self.SNAPSHOT_STATE_MAP.get(obj['status'], VolumeSnapshotState.UNKNOWN)
        snapshot = VolumeSnapshot(id=obj['id'], driver=self, size=obj['size'], extra=extra, created=obj['creationDate'], state=state, name=obj['name'])
        return snapshot
