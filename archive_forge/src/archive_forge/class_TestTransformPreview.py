...
class TestTransformPreview(TestCaseWithTree):
    ...
    def test_iter_changes(self):
        ...
        self.assertThat(preview_tree.iter_changes(revision_tree), MatchesTreeChanges(revision_tree, preview_tree, [(('a', 'a'), True, (True, True), ('a', 'a'), ('file', 'file'), (False, False), False)]))
    ...
    def test_get_file_mtime(self):
        ...
        file_trans_id = preview.new_file('file', preview.root, [b'contents'], b'file-id')
        limbo_path = preview._limbo_name(file_trans_id)
        preview_tree = preview.get_preview_tree()
        self.assertEqual(os.stat(limbo_path).st_mtime, preview_tree.get_file_mtime('file'))
    ...
    def test_all_file_ids(self):
        ...
        preview_tree = preview.get_preview_tree()
        self.assertEqual({b'a-id', b'c-id', tree.path2id('')}, preview_tree.all_file_ids())
    ...
    def test_extras(self):
        work_tree = self.make_branch_and_tree('tree')
        self.build_tree(['tree/removed-file', 'tree/existing-file', 'tree/not-removed-file'])
        work_tree.add(['removed-file', 'not-removed-file'])
        preview = work_tree.preview_transform()
        self.addCleanup(preview.finalize)
        preview.new_file('new-file', preview.root, [b'contents'])
        preview.new_file('new-versioned-file', preview.root, [b'contents'], b'new-versioned-id')
        tree = preview.get_preview_tree()
        self.assertEqual({'existing-file'}, set(work_tree.extras()))
        preview.unversion_file(preview.trans_id_tree_path('removed-file'))
        self.assertEqual({'new-file', 'removed-file', 'existing-file'}, set(tree.extras()))
