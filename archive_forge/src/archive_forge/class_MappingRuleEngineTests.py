...
class MappingRuleEngineTests(unit.BaseTestCase):
    ...
    def test_rule_engine_whitelist_and_direct_groups_mapping(self):
        ...
        mapped_properties = rp.process(assertion)
        self.assertIsNotNone(mapped_properties)
        reference = {mapping_fixtures.DEVELOPER_GROUP_NAME: {'name': mapping_fixtures.DEVELOPER_GROUP_NAME, 'domain': {'id': mapping_fixtures.DEVELOPER_GROUP_DOMAIN_ID}}, mapping_fixtures.CONTRACTOR_GROUP_NAME: {'name': mapping_fixtures.CONTRACTOR_GROUP_NAME, 'domain': {'id': mapping_fixtures.DEVELOPER_GROUP_DOMAIN_ID}}}
        for rule in mapped_properties['group_names']:
            self.assertDictEqual(reference.get(rule.get('name')), rule)
    ...
    def test_rule_engine_groups_mapping_only_one_group(self):
        ...
        self.assertEqual('ALL USERS', mapped_properties['group_names'][0]['name'])
