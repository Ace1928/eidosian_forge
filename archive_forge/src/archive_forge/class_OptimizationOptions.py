...
class OptimizationOptions(options_lib.OptionsBase):
  ...
  ```
  """
    apply_default_optimizations = options_lib.create_option(name='apply_default_optimizations', ty=bool, docstring='Whether to apply default graph optimizations. If False, only graph optimizations that have been explicitly enabled will be applied.')
    filter_fusion = options_lib.create_option(name='filter_fusion', ty=bool, docstring='Whether to fuse filter transformations. If None, defaults to False.')
    filter_parallelization = options_lib.create_option(name='filter_parallelization', ty=bool, docstring='Whether to parallelize stateless filter transformations. If None, defaults to False.')
    inject_prefetch = options_lib.create_option(name='inject_prefetch', ty=bool, docstring='Whether to inject prefetch transformation as the last transformation when the last transformation is a synchronous transformation. If None, defaults to True.')
    map_and_batch_fusion = options_lib.create_option(name='map_and_batch_fusion', ty=bool, docstring='Whether to fuse map and batch transformations. If None, defaults to True.')
    map_and_filter_fusion = options_lib.create_option(name='map_and_filter_fusion', ty=bool, docstring='Whether to fuse map and filter transformations. If None, defaults to False.')
    map_fusion = options_lib.create_option(name='map_fusion', ty=bool, docstring='Whether to fuse map transformations. If None, defaults to False.')
    map_parallelization = options_lib.create_option(name='map_parallelization', ty=bool, docstring='Whether to parallelize stateless map transformations. If None, defaults to True.')
    noop_elimination = options_lib.create_option(name='noop_elimination', ty=bool, docstring='Whether to eliminate no-op transformations. If None, defaults to True.')
    parallel_batch = options_lib.create_option(name='parallel_batch', ty=bool, docstring='Whether to parallelize copying of batch elements. If None, defaults to True.')
    shuffle_and_repeat_fusion = options_lib.create_option(name='shuffle_and_repeat_fusion', ty=bool, docstring='Whether to fuse shuffle and repeat transformations. If None, defaults to True.')
    warm_start = options_lib.create_option(name='warm_start', ty=bool, docstring="Whether to start background threads of asynchronous transformations upon iterator creation, as opposed to during the first call to `next()`. Defaults to `False`. This improves the latency of the initial 'next()' calls at the expense of requiring more memory to hold prefetched elements between the time of iterator construction and usage.", default_factory=lambda: True if test_mode.TEST_MODE else False)
