...
def _wrap_sync_server(func: Callable[P, Server]) -> Callable[P, Server]:
    ...
    @wraps(func)
    def patched_server(*args: P.args, interceptors: Optional[Sequence[grpc.ServerInterceptor]]=None, **kwargs: P.kwargs) -> Server:
        interceptors = [interceptor for interceptor in interceptors or [] if not isinstance(interceptor, ServerInterceptor)]
