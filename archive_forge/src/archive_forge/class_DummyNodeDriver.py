...
class DummyNodeDriver(NodeDriver):
    ...
    def __init__(self, creds):
        ...
        if num:
            ...
            for i in range(num):
                ...
                self.nl.append(Node(id=i, name='dummy-%d' % i, state=NodeState.RUNNING, public_ips=[ip], private_ips=[], driver=self, extra={'foo': 'bar'}))
        else:
            self.nl = [Node(id=1, name='dummy-1', state=NodeState.RUNNING, public_ips=['127.0.0.1'], private_ips=[], driver=self, extra={'foo': 'bar'}), Node(id=2, name='dummy-2', state=NodeState.RUNNING, public_ips=['127.0.0.1'], private_ips=[], driver=self, extra={'foo': 'bar'})]
    ...
    def list_images(self, location=None):
        ...
        """
        return [NodeImage(id=1, name='Ubuntu 9.10', driver=self), NodeImage(id=2, name='Ubuntu 9.04', driver=self), NodeImage(id=3, name='Slackware 4', driver=self)]
    ...
    def list_sizes(self, location=None):
        ...
        @inherits: :class:`NodeDriver.list_images`
        """
        return [NodeSize(id=1, name='Small', ram=128, disk=4, bandwidth=500, price=4, driver=self), NodeSize(id=2, name='Medium', ram=512, disk=16, bandwidth=1500, price=8, driver=self), NodeSize(id=3, name='Big', ram=4096, disk=32, bandwidth=2500, price=32, driver=self), NodeSize(id=4, name='XXL Big', ram=4096 * 2, disk=32 * 4, bandwidth=2500 * 3, price=32 * 2, driver=self)]
    ...
    def list_locations(self):
        ...
        """
        return [NodeLocation(id=1, name="Paul's Room", country='US', driver=self), NodeLocation(id=2, name='London Loft', country='GB', driver=self), NodeLocation(id=3, name='Island Datacenter', country='FJ', driver=self)]
    ...
    def create_node(self, name, size, image):
        ...
        """
        num = len(self.nl) + 1
        n = Node(id=num, name='dummy-%d' % num, state=NodeState.RUNNING, public_ips=['127.0.0.%d' % num], private_ips=[], driver=self, size=NodeSize(id='s1', name='foo', ram=2048, disk=160, bandwidth=None, price=0.0, driver=self), image=NodeImage(id='i2', name='image', driver=self), extra={'foo': 'bar'})
        self.nl.append(n)
        return n

    def import_key_pair_from_string(self, name, key_material):
        key_pair = KeyPair(name=name, public_key=key_material, fingerprint='fingerprint', private_key='private_key', driver=self)
        return key_pair