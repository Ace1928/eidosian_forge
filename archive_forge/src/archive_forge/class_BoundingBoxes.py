...
class BoundingBoxes(TVTensor):
    ...
    @classmethod
    def _wrap(cls, tensor: torch.Tensor, *, format: Union[BoundingBoxFormat, str], canvas_size: Tuple[int, int], check_dims: bool=True) -> BoundingBoxes:
        if check_dims:
            if tensor.ndim == 1:
    ...
    def __new__(cls, data: Any, *, format: Union[BoundingBoxFormat, str], canvas_size: Tuple[int, int], dtype: Optional[torch.dtype]=None, device: Optional[Union[torch.device, str, int]]=None, requires_grad: Optional[bool]=None) -> BoundingBoxes:
        tensor = cls._to_tensor(data, dtype=dtype, device=device, requires_grad=requires_grad)
        return cls._wrap(tensor, format=format, canvas_size=canvas_size)

    @classmethod
    def _wrap_output(cls, output: torch.Tensor, args: Sequence[Any]=(), kwargs: Optional[Mapping[str, Any]]=None) -> BoundingBoxes:
        ...
        if isinstance(output, torch.Tensor) and (not isinstance(output, BoundingBoxes)):
            ...
            output = type(output)((BoundingBoxes._wrap(part, format=format, canvas_size=canvas_size, check_dims=False) for part in output))
        return output
