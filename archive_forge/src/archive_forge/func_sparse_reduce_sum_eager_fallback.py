...
def sparse_reduce_sum_eager_fallback(input_indices: _atypes.TensorFuzzingAnnotation[_atypes.Int64], input_values: _atypes.TensorFuzzingAnnotation[TV_SparseReduceSum_T], input_shape: _atypes.TensorFuzzingAnnotation[_atypes.Int64], reduction_axes: _atypes.TensorFuzzingAnnotation[_atypes.Int32], keep_dims: bool, name, ctx) -> _atypes.TensorFuzzingAnnotation[TV_SparseReduceSum_T]:
    ...
    _result = _execute.execute(b'SparseReduceSum', 1, inputs=_inputs_flat, attrs=_attrs, ctx=ctx, name=name)
    if _execute.must_record_gradient():
        _execute.record_gradient('SparseReduceSum', _inputs_flat, _attrs, _result)
    _result, = _result
    return _result