...
class Media(WBValue):
    ...
    @staticmethod
    def captions(media_items: Sequence['Media']) -> Union[bool, Sequence[Optional[str]]]:
    ...
    def to_json(self, run: Union['LocalRun', 'Artifact']) -> dict:
        ...
        if isinstance(run, wandb.wandb_sdk.wandb_run.Run):
            ...
            if artifact_entry_url is None or self.is_bound():
                assert self.is_bound(), 'Value of type {} must be bound to a run with bind_to_run() before being serialized to JSON.'.format(type(self).__name__)
            ...
            if self.file_is_set():
                ...
                if name is None:
                    ...
                    elif (isinstance(self, Audio) or isinstance(self, Image)) and self.path_is_reference(self._path):
