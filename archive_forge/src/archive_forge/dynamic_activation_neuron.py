...
class DataProcessor:
    ...
    def process_and_prepare_md_data(
        ...
        logger.info(f"Shape of X_train_tensor: {X_train_tensor.shape}")
        logger.info(f"Shape of y_train_tensor: {y_train_tensor.shape}")
        logger.info(f"Shape of X_val_tensor: {X_val_tensor.shape}")
        logger.info(f"Shape of y_val_tensor: {y_val_tensor.shape}")
...
class ModelTrainer:
    ...
    def _adjust_optimizer_params(self, dynamic: bool = False) -> None:
        for param_group in self.optimizer.param_groups:
            ...
            if dynamic:
                # Additional dynamic adjustments based on input characteristics
                param_group["lr"] *= self.dynamic_lr_factor
                param_group["weight_decay"] *= self.dynamic_weight_decay_factor
...
class Utility:
    @staticmethod
    def save_to_file(data: Dict[str, torch.Tensor], file_path: str) -> None:
        """
        Save data to a specified file.

        Args:
            data (Dict[str, torch.Tensor]): The data to save.
    ...
    def calculate_metrics(
        ...
        """
        Calculate loss, RMSE, and MAPE metrics.
        ...
        Returns:
            Tuple[torch.Tensor, float, float]: The loss, RMSE, and MAPE.
        """
        print(f"Shape of outputs: {outputs.shape}")
        print(f"Shape of labels: {labels.shape}")
        logger.info(f"Shape of outputs: {outputs.shape}")
...
class Analytics:
    ...
    def save_analysis(analysis: Dict[str, torch.Tensor], file_path: str) -> None:
        ...
        Args:
            ...
            file_path (str): The path to the file where analysis will be saved.
        """
        # Debugging: Print analysis keys and shapes
        for key, value in analysis.items():
            print(f"Shape of {key}: {value.shape}")
            logger.info(f"Shape of {key}: {value.shape}")

        Utility.save_to_file(analysis, file_path)
...
class Plotter:
    ...
    def plot_metrics(history: Dict[str, List[float]], output_dir: str) -> None:
        """
        Plot training and validation metrics over epochs and save the plots with timestamps.
        ...
        plot_path = os.path.join(
            output_dir, f"metrics_epoch_{len(epochs)}_{timestamp}.png"
        )
    ...
    @staticmethod
    def plot_epoch_metrics(history: Dict[str, List[float]], output_dir: str) -> None:
        """
        Plot metrics for each epoch and save the plots with timestamps.
        ...
        for epoch in range(len(history["train_loss"])):
            ...
            for i, (metric, (ylabel, title)) in enumerate(metrics.items(), start=1):
                ...
                plt.plot(
                    range(epoch + 1),
                    history[f"train_{metric}"][: epoch + 1],
                    label=f"Training {ylabel}",
                )
                plt.plot(
                    range(epoch + 1),
                    history[f"val_{metric}"][: epoch + 1],
                    label=f"Validation {ylabel}",
                )
                plt.xlabel("Epoch")
                plt.ylabel(ylabel)
                plt.title(
                    f"{title} (Best Val {ylabel}: {min(history[f'val_{metric}'][:epoch + 1]):.4f})"
                )
                plt.legend()
            ...
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            plot_path = os.path.join(
                output_dir, f"epoch_{epoch + 1}_metrics_{timestamp}.png"
            )
            plt.savefig(plot_path)
            logger.info(f"Epoch {epoch + 1} metrics plot saved to {plot_path}")
            plt.show(block=False)
            plt.pause(5)
            plt.close()


class DynamicActivationNeuronApp:
    def __init__(self):
        self.paths = [SYNTHETIC_DATA_DIR, PROCESSED_DATA_DIR, OUTPUT_DIR, ROOT_DIR]
        self.dan = None
        self.use_existing_data = False
        self.use_text_data = False

    def ensure_directories(self):
        DirectoryManager.ensure_directories_exist(self.paths)

    def select_model_file(self, initial_dir: str) -> Optional[str]:
    ...
    def load_or_create_model(self):
        model_path = self.select_model_file(os.path.dirname(ROOT_DIR))
        if model_path and os.path.isfile(model_path):
            self.dan = load_model(model_path, format="pkl")
            logger.info(f"Model loaded from {model_path}")
            print(f"Model loaded from {model_path}")
        else:
            self.dan = DynamicActivationNeuron(INPUT_SIZE)
            save_model(self.dan, UNTRAINED_MODEL_PATH, format="pkl")
            logger.info(f"New model created and saved to {UNTRAINED_MODEL_PATH}")
            print(f"New model created and saved to {UNTRAINED_MODEL_PATH}")

    def train_with_text_data(self):
        # Step 1: Load or create text data
        ...
        optimizer = torch.optim.Adam(
            self.dan.parameters(), lr=LR, weight_decay=WEIGHT_DECAY
        )
        ...
        # Step 4: Train model and save training history
        history = model_handler.train_and_save_model()

        # Debug: Check tensor shapes
        for inputs, labels in text_val_loader:
            outputs = self.dan(inputs)
            print(f"Shape of outputs: {outputs.shape}")
            print(f"Shape of labels: {labels.shape}")
            logger.info(f"Shape of outputs: {outputs.shape}")
            logger.info(f"Shape of labels: {labels.shape}")
        ...
        # Step 6: Clear memory to avoid any potential memory leaks
        ...
        # Step 9: Log the predictions
        Predictor.log_predictions(predictions)

        # Step 10: Analyze the predictions
        analysis = Analytics.analyze_predictions(predictions)

        # Step 11: Save the analysis results
        Analytics.save_analysis(
            analysis, os.path.join(OUTPUT_DIR, "text_data_analysis.pkl")
        )

    def train_with_synthetic_data(self):
        # Step 1: Load or create synthetic data
        if self.use_existing_data:
        ...
        # Debug: Check tensor shapes
        for inputs, labels in regression_val_loader:
            outputs = self.dan(inputs)
            print(f"Shape of outputs: {outputs.shape}")
            print(f"Shape of labels: {labels.shape}")
            logger.info(f"Shape of outputs: {outputs.shape}")
        ...
        # Step 11: Save the analysis results
        Analytics.save_analysis(
            analysis, os.path.join(OUTPUT_DIR, "regression_data_analysis.pkl")
        )
    ...
    def run(self):
        ...
        try:
            self.use_text_data = Utility.get_user_choice(
                "Do you want to train on text data?"
            )
            self.use_existing_data = Utility.get_user_choice(
                "Do you want to use existing dataset?"
            )

            if self.use_text_data:
                self.train_with_text_data()
            else:
...
def main() -> None:
    app = DynamicActivationNeuronApp()
    app.run()
