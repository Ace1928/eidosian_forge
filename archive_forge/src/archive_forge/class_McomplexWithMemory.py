...
class McomplexWithMemory(Mcomplex):
    ...
    def __init__(self, tetrahedron_list=None, two_zero_to_one_tet=True, invariant_tetrahedra=None):
    ...
    def _edge_permits_three_to_two(self, edge):
        ...
        if possible:
            if any((corner.Tetrahedron in self.invariant_tetrahedra for corner in edge.Corners)):
    ...
    def _arrow_permits_two_to_zero(self, arrow):
        ...
        if possible:
            ...
            if any((corner.Tetrahedron in self.invariant_tetrahedra for corner in arrow.axis().Corners)):
                return (False, 'Edge adjacent to fixed subcomplex')
        return (possible, reason)
    ...
    def zero_to_two(self, arrow, gap):
        ...
        if success and hasattr(self, 'move_memory'):
            T = arrow.Tetrahedron
            tet_index = self.Tetrahedra.index(T)
            self.move_memory.append(('zero_to_two', arrow.Edge, arrow.Face, tet_index, gap))
        return success
