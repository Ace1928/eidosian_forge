...
class InstanceConfigsGetterWithSimpleCache(object):
  ...
  """
    ...
    def check_if_instance_config_exists(self, igm_ref, instance_ref, should_exist=True):
        """Checks if instance config exists for given instance reference."""
        per_instance_config = self.get_instance_config(igm_ref=igm_ref, instance_ref=instance_ref)
        if should_exist:
            if per_instance_config is None:
                raise managed_instance_groups_utils.ResourceNotFoundException('Instance config for {instance} does not exist'.format(instance=instance_ref))
        elif per_instance_config is not None:
            raise managed_instance_groups_utils.ResourceAlreadyExistsException('Instance config for {instance} already exists'.format(instance=instance_ref))
