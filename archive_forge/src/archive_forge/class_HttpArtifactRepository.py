...
class HttpArtifactRepository(ArtifactRepository, MultipartUploadMixin):
    ...
    def log_artifact(self, local_file, artifact_path=None):
        ...
        if MLFLOW_ENABLE_PROXY_MULTIPART_UPLOAD.get() and os.path.getsize(local_file) >= MLFLOW_MULTIPART_UPLOAD_MINIMUM_FILE_SIZE.get():
            try:
        ...
        with open(local_file, 'rb') as f:
            resp = http_request(self._host_creds, endpoint, 'PUT', data=f, extra_headers=extra_headers)
    ...
    def list_artifacts(self, path=None):
        ...
        for f in resp.json().get('files', []):
            ...
            file_info = FileInfo(posixpath.join(path, validated_path) if path else validated_path, f['is_dir'], int(f['file_size']) if 'file_size' in f else None)
    ...
    def _construct_mpu_uri_and_path(self, base_endpoint, artifact_path):
        ...
        path = path.strip('/')
        endpoint = posixpath.join(base_endpoint, path, artifact_path) if artifact_path else posixpath.join(base_endpoint, path)
        return (uri, endpoint)

    def create_multipart_upload(self, local_file, num_parts=1, artifact_path=None):
        uri, endpoint = self._construct_mpu_uri_and_path('/mlflow-artifacts/mpu/create', artifact_path)
        host_creds = get_default_host_creds(uri)
        params = {'path': local_file, 'num_parts': num_parts}
    ...
    def complete_multipart_upload(self, local_file, upload_id, parts=None, artifact_path=None):
        uri, endpoint = self._construct_mpu_uri_and_path('/mlflow-artifacts/mpu/complete', artifact_path)
        host_creds = get_default_host_creds(uri)
        params = {'path': local_file, 'upload_id': upload_id, 'parts': [part.to_dict() for part in parts]}
        resp = http_request(host_creds, endpoint, 'POST', json=params)
        augmented_raise_for_status(resp)

    def abort_multipart_upload(self, local_file, upload_id, artifact_path=None):
        uri, endpoint = self._construct_mpu_uri_and_path('/mlflow-artifacts/mpu/abort', artifact_path)
        host_creds = get_default_host_creds(uri)
        params = {'path': local_file, 'upload_id': upload_id}
        resp = http_request(host_creds, endpoint, 'POST', json=params)
        augmented_raise_for_status(resp)
    ...
    def _upload_part(credential: MultipartUploadCredential, local_file, size, start_byte):
        ...
        return MultipartUploadPart(part_number=credential.part_number, etag=response.headers.get('ETag', ''), url=credential.url)
