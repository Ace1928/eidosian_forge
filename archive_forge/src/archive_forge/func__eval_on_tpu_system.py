...
def _eval_on_tpu_system(ctx, model_fn_wrapper, dequeue_fn):
    ...
    def multi_tpu_eval_steps_on_single_shard(replica_id):
        ...
        with tpu_context._TPUEstimatorReplicaContext(replica_id):
            return training_loop.repeat(iterations_per_loop_var, single_tpu_eval_step, [_ZERO_LOSS])
    replica_id_inputs = []
    replica_id_inputs.append([tf.constant(i) for i in range(ctx.num_replicas)])
    compile_op, loss = tpu.split_compile_and_shard(multi_tpu_eval_steps_on_single_shard, inputs=replica_id_inputs, num_shards=ctx.num_replicas, outputs_from_all_shards=False, device_assignment=ctx.device_assignment)
    loss = loss[0]
    return (compile_op, loss, host_calls, captured_scaffold_fn, captured_eval_hooks.get())