  ...
  *{command}* adds a backend to a Google Cloud load balancer or Traffic
  ...
  """
    ...
    def Args(cls, parser):
        ...
        flags.AddInstanceGroupAndNetworkEndpointGroupArgs(parser, 'add to', support_global_neg=cls.support_global_neg, support_region_neg=cls.support_region_neg)
        backend_flags.AddBalancingMode(parser, support_global_neg=cls.support_global_neg, support_region_neg=cls.support_region_neg)
        backend_flags.AddCapacityLimits(parser, support_global_neg=cls.support_global_neg, support_region_neg=cls.support_region_neg)
        backend_flags.AddCapacityScalar(parser, support_global_neg=cls.support_global_neg, support_region_neg=cls.support_region_neg)
    ...
    def _CreateBackendMessage(self, messages, group_uri, balancing_mode, preference, args):
    ...
    """
        ...
        if self.support_preference and preference is not None:
            return messages.Backend(balancingMode=balancing_mode, preference=preference, capacityScaler=args.capacity_scaler, description=args.description, group=group_uri, maxRate=args.max_rate, maxRatePerInstance=args.max_rate_per_instance, maxRatePerEndpoint=args.max_rate_per_endpoint, maxUtilization=args.max_utilization, maxConnections=args.max_connections, maxConnectionsPerInstance=args.max_connections_per_instance, maxConnectionsPerEndpoint=args.max_connections_per_endpoint, failover=args.failover)
        else:
    ...
    def _Modify(self, client, resources, backend_service_ref, args, existing):
        ...
        for backend in existing.backends:
            if group_uri == backend.group:
                if group_ref.Collection() == 'compute.instanceGroups' or group_ref.Collection() == 'compute.networkEndpointGroups':
                    scope = 'zone'
                elif group_ref.Collection() == 'compute.regionInstanceGroups':
                    scope = 'region'
                raise exceptions.ArgumentError('Backend [{}] in {} [{}] already exists in backend service [{}].'.format(group_ref.Name(), scope, getattr(group_ref, scope), backend_service_ref.Name()))
        if args.balancing_mode:
            balancing_mode = client.messages.Backend.BalancingModeValueValuesEnum(args.balancing_mode)
        else:
            balancing_mode = None
        preference = None
        if self.support_preference and args.preference:
            preference = client.messages.Backend.PreferenceValueValuesEnum(args.preference)
        backend = self._CreateBackendMessage(client.messages, group_uri, balancing_mode, preference, args)
    ...
    def Run(self, args):
        ...
        new_object = self._Modify(client, holder.resources, backend_service_ref, args, objects[0])
        return client.MakeRequests([self._GetSetRequest(client, backend_service_ref, new_object)])