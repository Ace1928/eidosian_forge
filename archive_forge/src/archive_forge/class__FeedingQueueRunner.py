...
class _FeedingQueueRunner(tf.compat.v1.train.queue_runner.QueueRunner):
    ...
    """
        ...
        if feed_fns is None:
            ...
            if len(feed_fns) != len(enqueue_ops):
                raise ValueError('If feed_fns is not None, it must have the same length as enqueue_ops.')
    ...
    def to_proto(self):
        raise NotImplementedError('{} does not support serialization to proto.'.format(type(self).__name__))