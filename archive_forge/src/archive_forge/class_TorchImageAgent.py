...
class TorchImageAgent(TorchAgent):
    ...
    def add_cmdline_args(cls, argparser):
        ...
        agent.add_argument('--image-encoder-num-layers', type=int, default=1, recommended=1, help='Number of linear layers to encode image features with')
        agent.add_argument('--n-image-tokens', type=int, default=1, help='Number of tokens that the image encoding will consist of. Specify to spread image encoding over multiple tokens')
        agent.add_argument('--n-image-channels', type=int, default=1, help='Number of channels that the image encoding will consist of. Specify if incoming image is multidimensional')
        return agent
    ...
    def batchify_image_features(self, batch: Batch) -> Batch:
        ...
        """
        raise NotImplementedError('Subclasses must implement method for batching images!')
