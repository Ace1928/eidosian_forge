...
def apply_effects_file(path: str, effects: List[List[str]], normalize: bool=True, channels_first: bool=True, format: Optional[str]=None) -> Tuple[torch.Tensor, int]:
    ...
    if not torch.jit.is_scripting():
        if hasattr(path, 'read'):
            raise RuntimeError('apply_effects_file function does not support file-like object. Please use torchaudio.io.AudioEffector.')
        path = os.fspath(path)
