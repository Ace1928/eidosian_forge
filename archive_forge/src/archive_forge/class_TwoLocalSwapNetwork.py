...
class TwoLocalSwapNetwork(Operation):
    ...
    **Example**
    ...
    def __init__(self, wires, acquaintances=None, weights=None, fermionic=True, shift=False, id=None, **kwargs):
        if len(wires) < 2:
        ...
        if weights is not None and acquaintances is not None and (qml.math.shape(weights)[0] != int(len(wires) * (len(wires) - 1) / 2)):
            raise ValueError(f'Weight tensor must be of length {int(len(wires) * (len(wires) - 1) / 2)},                     got {qml.math.shape(weights)[0]}')
        self._weights = weights
        self._hyperparameters = {'acquaintances': acquaintances, 'fermionic': fermionic, 'shift': shift, **kwargs}
    ...
    @property
    def num_params(self):
        return 1 if self._hyperparameters['acquaintances'] is not None and self._weights is not None else 0
        ...
        for layer in range(len(wires)):
            ...
            for i, j in qubit_pairs:
                ...
                if acquaintances is not None:
                    op_list.append(acquaintances(index=[wires[i], wires[j]], wires=[qb1, qb2], param=next(itrweights, None), **kwargs))
                op_list.append(SWAP(wires=[wires[i], wires[j]]) if not fermionic else FermionicSWAP(np.pi, wires=[wires[i], wires[j]]))
