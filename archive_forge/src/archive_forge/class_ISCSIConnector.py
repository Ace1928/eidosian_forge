...
class ISCSIConnector(base.BaseLinuxConnector, base_iscsi.BaseISCSIConnector):
    ...
    supported_transports = ['be2iscsi', 'bnx2i', 'cxgb3i', 'default', 'cxgb4i', 'qla4xxx', 'ocs', 'iser', 'tcp']
    VALID_SESSIONS_PREFIX = ('tcp:', 'iser:')

    def __init__(self, root_helper: str, driver=None, execute=None, use_multipath: bool=False, device_scan_attempts: int=initiator.DEVICE_SCAN_ATTEMPTS_DEFAULT, transport: str='default', *args, **kwargs):
        super(ISCSIConnector, self).__init__(root_helper, *args, driver=driver, execute=execute, device_scan_attempts=device_scan_attempts, transport=transport, **kwargs)
    ...
    def get_connector_properties(root_helper: str, *args, **kwargs) -> dict:
        ...
        props = {}
        iscsi = ISCSIConnector(root_helper=root_helper, execute=kwargs.get('execute'))
    ...
    def get_volume_paths(self, connection_properties: dict) -> list:
        ...
        """
        volume_paths: list = []
        if 'target_portal' not in connection_properties and 'target_portals' not in connection_properties:
            return volume_paths
    ...
    def extend_volume(self, connection_properties: dict) -> Optional[int]:
        ...
        for an iSCSI volume.
        """
        LOG.info('Extend volume for %s', strutils.mask_dict_password(connection_properties))
        volume_paths = self.get_volume_paths(connection_properties)
        LOG.info('Found paths for volume %s', volume_paths)
        if volume_paths:
            return self._linuxscsi.extend_volume(volume_paths, use_multipath=self.use_multipath)
        else:
            LOG.warning("Couldn't find any volume paths on the host to extend volume for %(props)s", {'props': strutils.mask_dict_password(connection_properties)})
            raise exception.VolumePathsNotFound()

    @utils.trace
    @utils.connect_volume_prepare_result
    @base.synchronized('connect_volume', external=True)
    def connect_volume(self, connection_properties: dict) -> Optional[dict[str, str]]:
    ...
    def disconnect_volume(self, connection_properties: dict, device_info: dict, force: bool=False, ignore_errors: bool=False) -> None:
        ...
        """
        return self._cleanup_connection(connection_properties, force=force, ignore_errors=ignore_errors, device_info=device_info, is_disconnect_call=True)
    ...
    def get_initiator(self) -> Optional[str]:
        ...
        try:
            lines, _err = self._execute('cat', file_path, run_as_root=True, root_helper=self._root_helper)
            for line in lines.split('\n'):
                if line.startswith('InitiatorName='):
                    return line[line.index('=') + 1:].strip()
        except putils.ProcessExecutionError:
            LOG.warning('Could not find the iSCSI Initiator File %s', file_path)
            return None
        return None
    ...
    def _run_multipath(self, multipath_command, **kwargs):
        check_exit_code = kwargs.pop('check_exit_code', 0)
        out, err = self._execute('multipath', *multipath_command, run_as_root=True, root_helper=self._root_helper, check_exit_code=check_exit_code)
        LOG.debug('multipath %(multipath_command)s: stdout=%(out)s stderr=%(err)s', {'multipath_command': multipath_command, 'out': out, 'err': err})
        return (out, err)
