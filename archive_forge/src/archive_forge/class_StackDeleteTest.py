...
class StackDeleteTest(common.HeatTestCase):
    ...
    @mock.patch.object(parser.Stack, 'load')
    @mock.patch.object(stack_lock.StackLock, 'try_acquire')
    def test_stack_delete_acquired_lock_stop_timers(self, mock_acquire, mock_load):
        mock_acquire.return_value = self.man.engine_id
    ...
    @mock.patch.object(stack_lock.StackLock, 'acquire')
    @mock.patch.object(timeutils.StopWatch, 'expired')
    def test_stack_delete_other_engine_active_lock_succeeded(self, mock_expired, mock_acquire, mock_alive, mock_try, mock_load):
        cfg.CONF.set_override('error_wait_time', 0)
        OTHER_ENGINE = 'other-engine-fake-uuid'
        self.man.engine_id = service_utils.generate_engine_id()
        self.man.listener = service.EngineListener(self.man.host, self.man.engine_id, self.man.thread_group_mgr)
        ...
        mock_call.assert_has_calls([mock.call(self.ctx, OTHER_ENGINE, mock.ANY, 'send', message='cancel', stack_identity=mock.ANY), mock.call(self.ctx, OTHER_ENGINE, mock.ANY, 'stop_stack', stack_identity=mock.ANY)])
        mock_acquire.assert_called_once_with(True)
    ...
    @mock.patch.object(stack_lock.StackLock, 'acquire')
    @mock.patch.object(timeutils.StopWatch, 'expired')
    def test_stack_delete_other_dead_engine_active_lock(self, mock_expired, mock_acquire, mock_alive, mock_try, mock_load):
        cfg.CONF.set_override('error_wait_time', 0)
        OTHER_ENGINE = 'other-engine-fake-uuid'
