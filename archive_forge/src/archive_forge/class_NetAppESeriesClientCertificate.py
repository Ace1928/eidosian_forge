...
class NetAppESeriesClientCertificate(NetAppESeriesModule):
    ...
    def __init__(self):
        ansible_options = dict(certificates=dict(type='list', required=False), remove_unspecified_user_certificates=dict(type='bool', default=False, required=False), reload_certificates=dict(type='bool', default=True, required=False))
        super(NetAppESeriesClientCertificate, self).__init__(ansible_options=ansible_options, web_services_version='02.00.0000.0000', supports_check_mode=True)
    ...
    def certificate_info(self, path):
        ...
        return dict(start_date=certificate.not_valid_before, expire_date=certificate.not_valid_after, subject_dn=[attr.value for attr in certificate.subject], issuer_dn=[attr.value for attr in certificate.issuer])
        ...
        if rc == 404:
            ...
            for path in self.certificates:
                for current_certificate in user_installed_certificates:
                    ...
                    tmp = dict(subject_dn=[re.sub('.*=', '', item) for item in current_certificate['subjectDN'].split(', ')], issuer_dn=[re.sub('.*=', '', item) for item in current_certificate['issuerDN'].split(', ')], start_date=datetime.strptime(current_certificate['start'].split('.')[0], '%Y-%m-%dT%H:%M:%S'), expire_date=datetime.strptime(current_certificate['expire'].split('.')[0], '%Y-%m-%dT%H:%M:%S'))
                    if all([attr in info['subject_dn'] for attr in tmp['subject_dn']]) and all([attr in info['issuer_dn'] for attr in tmp['issuer_dn']]) and (tmp['start_date'] == info['start_date']) and (tmp['expire_date'] == info['expire_date']):
