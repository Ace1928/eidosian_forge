...
class ZMQChannelsWebsocketConnection(BaseKernelWebsocketConnection):
    ...
    limit_rate = Bool(True, config=True, help=_i18n('Whether to limit the rate of IOPub messages (default: True). If True, use iopub_msg_rate_limit, iopub_data_rate_limit and/or rate_limit_window to tune the rate.'))
    ...
    def disconnect(self):
        ...
        if self.kernel_id in self.multi_kernel_manager:
            ...
            self.multi_kernel_manager.remove_restart_callback(self.kernel_id, self.on_kernel_restarted)
            self.multi_kernel_manager.remove_restart_callback(self.kernel_id, self.on_restart_failed, 'dead')
    ...
    def handle_incoming_message(self, incoming_msg: str) -> None:
        ...
        if am:
            ...
            if msg['header']['msg_type'] not in am:
                self.log.warning('Received message of type "%s", which is not allowed. Ignoring.' % msg['header']['msg_type'])
                ignore_msg = True
        if not ignore_msg:
            stream = self.channels[channel]
            if self.subprotocol == 'v1.kernel.websocket.jupyter.org':
    ...
    def get_part(self, field, value, msg_list):
        ...
        if value is None:
            field2idx = {'header': 0, 'parent_header': 1, 'content': 3}
