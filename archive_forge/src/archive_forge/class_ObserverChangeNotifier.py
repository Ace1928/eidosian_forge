...
class ObserverChangeNotifier:
    ...
    def __init__(self, *, observer_handler, event_factory, prevent_event, graph, handler, target, dispatcher):
        """
    ...
    def __call__(self, *args, **kwargs):
        ...
        if self.prevent_event(event):
            return
        self.observer_handler(event=event, graph=self.graph, target=target, handler=handler, dispatcher=self.dispatcher)
    ...
    def equals(self, other):
        ...
        boolean
        """
        return type(self) is type(other) and self.observer_handler is other.observer_handler and (self.graph == other.graph) and (self.handler() == other.handler()) and (self.target() is other.target()) and (self.dispatcher == other.dispatcher)