...
class PasqalGateset(cirq.CompilationTargetGateset):
    ...
    def __init__(self, include_additional_controlled_ops: bool=True):
        gate_families: List[Union[Type[cirq.Gate], cirq.Gate, cirq.GateFamily]] = [cirq.ParallelGateFamily(cirq.H), cirq.ParallelGateFamily(cirq.PhasedXPowGate), cirq.ParallelGateFamily(cirq.XPowGate), cirq.ParallelGateFamily(cirq.YPowGate), cirq.ParallelGateFamily(cirq.ZPowGate), cirq.AnyIntegerPowerGateFamily(cirq.CZPowGate), cirq.IdentityGate, cirq.MeasurementGate]
    ...
    def decompose_to_target_gateset(self, op: 'cirq.Operation', moment_idx: int) -> DecomposeResult:
        ...
        if mat is not None and len(op.qubits) == 2:
            return cirq.two_qubit_matrix_to_cz_operations(op.qubits[0], op.qubits[1], mat, allow_partial_czs=False, clean_operations=True)
    ...
    def __repr__(self):
        return f'cirq_pasqal.PasqalGateset(include_additional_controlled_ops={self.include_additional_controlled_ops})'
