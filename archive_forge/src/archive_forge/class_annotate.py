...
class annotate:
    ...
    def __init__(self, geom: str | type[geom_base_class], x: float | None=None, y: float | None=None, xmin: float | None=None, xmax: float | None=None, xend: float | None=None, xintercept: float | None=None, ymin: float | None=None, ymax: float | None=None, yend: float | None=None, yintercept: float | None=None, **kwargs: Any):
        variables = locals()
        pos_aesthetics = {loc: variables[loc] for loc in POSITION_AESTHETICS if variables[loc] is not None}
        ...
        if isinstance(geom, str):
            ...
            raise PlotnineError(f"geom must either be a plotnine.geom.geom() descendant (e.g. plotnine.geom_point), or a string naming a geom (e.g. 'point', 'text', ...). Got {repr(geom)}")
        mappings = aes(**{str(ae): ae for ae in data.columns})
        self._annotation_geom = geom_klass(mappings, data, stat='identity', inherit_aes=False, show_legend=False, **kwargs)
