...
def export_averagepool_2d_pads_count_include_pad() -> None:
    """input_shape: [1, 3, 28, 28]
        ...
        """
    node = onnx.helper.make_node('AveragePool', inputs=['x'], outputs=['y'], kernel_shape=[3, 3], pads=[2, 2, 2, 2], count_include_pad=1)
    ...
    padded = np.pad(x, ((0, 0), (0, 0), (pads[0], pads[2]), (pads[1], pads[3])), mode='constant', constant_values=0)
    y = pool(padded, x_shape, kernel_shape, strides, out_shape, 'AVG', pads, count_include_pad=1)
