...
def _execute(arr: '_array.DistributedArray', kernel, axis: int, dtype: DTypeLike) -> Any:
    overwrites = False
    mode_overrides = {'cupy_max': _modes.MAX, 'cupy_min': _modes.MIN, 'cupy_sum': _modes.SUM, 'cupy_prod': _modes.PROD}
        ...
        for chunk in chunks:
            with chunk.on_ready() as stream:
                ...
                if isinstance(chunk.array, _chunk._ArrayPlaceholder):
                    ...
                    out_chunk = _chunk._Chunk.create_placeholder(out_shape, chunk.array.device, out_index)
                else:
                    out_array = _manipulation_dims.atleast_1d(kernel(chunk.array, axis=axis, dtype=dtype))
                    out_dtype = out_array.dtype
                    out_chunk = _chunk._Chunk(out_array, stream.record(), out_index, prevent_gc=chunk.prevent_gc)
                ...
                for update, update_index in chunk.updates:
                    ...
                    out_update_array = _manipulation_dims.atleast_1d(kernel(update.array, axis=axis, dtype=dtype))
                    out_dtype = out_update_array.dtype
                    out_update = _data_transfer._AsyncData(out_update_array, stream.record(), prevent_gc=update.prevent_gc)
                    out_index = update_index[:axis] + update_index[axis + 1:]
                    out_chunk.add_update(out_update, out_index)
    return _array.DistributedArray(shape, out_dtype, out_chunks_map, mode, arr._comms)