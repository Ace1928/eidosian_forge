...
def ensure_only_allowed_dataclass_keys_updated(dataclass: dataclasses.dataclass, allowed_keys: Iterable[str]):
    """
    ...
    if keys_not_in_dict:
        raise ValueError(f'Key(s) {keys_not_in_dict} are not present in {dataclass.__class__.__name__}. Remove them from `allowed_keys`. Valid keys: {list(default_data.__dict__.keys())}')
    prohibited_keys = set(default_data.__dict__) - allowed_keys
    bad_keys = [key for key in prohibited_keys if dataclass.__dict__[key] != default_data.__dict__[key]]
    if bad_keys:
        raise ValueError(f'Key(s) {bad_keys} are not allowed to be updated in the current context. Remove them from the dataclass.')