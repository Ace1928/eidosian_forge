...
def _RemoteRepoHelpText(facade: str, hide_custom_remotes: bool) -> str:
    if hide_custom_remotes:
        return '({facade} only) Repo upstream for {facade_lower} remote repository.\nREMOTE_{command}_REPO must be one of: [{enums}].\n'.format(facade=facade, facade_lower=facade.lower(), command=_LanguagePackageCommandName(facade), enums=_EnumsStrForFacade(facade))
