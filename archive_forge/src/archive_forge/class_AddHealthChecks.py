...
class AddHealthChecks(base.SilentCommand):
  ...
  *{command}* is used to add a legacy HTTP health check
  ...
  """
    ...
    @classmethod
    def Args(cls, parser):
        cls.HEALTH_CHECK_ARG = http_health_check_flags.HttpHealthCheckArgumentForTargetPool('add to')
        cls.HEALTH_CHECK_ARG.AddArgument(parser)
        cls.TARGET_POOL_ARG = flags.TargetPoolArgument(help_suffix=' to which to add the health check.')
        cls.TARGET_POOL_ARG.AddArgument(parser, operation_type='add health checks to')

    def Run(self, args):
        holder = base_classes.ComputeApiHolder(self.ReleaseTrack())
        client = holder.client
        health_check_ref = self.HEALTH_CHECK_ARG.ResolveAsResource(args, holder.resources)
        ...
        request = client.messages.ComputeTargetPoolsAddHealthCheckRequest(region=target_pool_ref.region, project=target_pool_ref.project, targetPool=target_pool_ref.Name(), targetPoolsAddHealthCheckRequest=client.messages.TargetPoolsAddHealthCheckRequest(healthChecks=[client.messages.HealthCheckReference(healthCheck=health_check_ref.SelfLink())]))
        return client.MakeRequests([(client.apitools_client.targetPools, 'AddHealthCheck', request)])