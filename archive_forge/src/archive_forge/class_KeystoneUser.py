...
class KeystoneUser(resource.Resource, role_assignments.KeystoneRoleAssignmentMixin):
    ...
    def _update_user(self, user_id, domain, new_name=None, new_description=None, new_email=None, new_password=None, new_default_project=None, enabled=None):
        values = dict()
    ...
    def _find_diff(self, updated_prps, stored_prps):
        ...
        removed_group_ids = list(set(stored_prps or []) - set(updated_prps or []))
    ...
    def handle_create(self):
        user_name = self.properties[self.NAME] or self.physical_resource_name()
        ...
        user = self.client().users.create(name=user_name, domain=domain, description=description, enabled=enabled, email=email, password=password, default_project=default_project)
    ...
    def handle_update(self, json_snippet, tmpl_diff, prop_diff):
        if prop_diff:
            ...
            domain = prop_diff.get(self.DOMAIN) or self.properties[self.DOMAIN]
            default_project = prop_diff.get(self.DEFAULT_PROJECT)
            self._update_user(user_id=self.resource_id, domain=domain, new_name=name, new_description=description, enabled=enabled, new_default_project=default_project, new_email=email, new_password=password)
            if self.GROUPS in prop_diff:
                new_group_ids, removed_group_ids = self._find_diff(prop_diff[self.GROUPS], self.properties[self.GROUPS])
                if new_group_ids:
    ...
    def parse_live_resource_data(self, resource_properties, resource_data):
        user_reality = {self.ROLES: self.parse_list_assignments(user_id=self.resource_id), self.DEFAULT_PROJECT: resource_data.get('default_project_id'), self.DOMAIN: resource_data.get('domain_id'), self.GROUPS: [group.id for group in self.client().groups.list(user=self.resource_id)]}
        ...
        if self.resource_id is not None:
            with self.client_plugin().ignore_not_found:
                if self.properties[self.GROUPS] is not None:
                    self._remove_user_from_groups(self.resource_id, self.properties[self.GROUPS])
