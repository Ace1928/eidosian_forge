...
import os
def make_user_fn_markdown(user_fn_input_type, user_fn_input_description, user_fn_output_type, user_fn_output_description):
    ...
    md += f'- **As output:** Is passed, {format_description(user_fn_input_description)}\n' if user_fn_input_description else ''
    md += f'- **As input:** Should return, {format_description(user_fn_output_description)}' if user_fn_output_description else ''
