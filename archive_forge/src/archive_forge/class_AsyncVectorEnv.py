...
class AsyncVectorEnv(VectorEnv):
    ...
    def __init__(self, env_fns: Sequence[callable], observation_space: Optional[gym.Space]=None, action_space: Optional[gym.Space]=None, shared_memory: bool=True, copy: bool=True, context: Optional[str]=None, daemon: bool=True, worker: Optional[callable]=None):
        """Vectorized environment that runs multiple environments in parallel.
        ...
        dummy_env.close()
        del dummy_env
        super().__init__(num_envs=len(env_fns), observation_space=observation_space, action_space=action_space)
        if self.shared_memory:
            try:
                ...
                raise ValueError('Using `shared_memory=True` in `AsyncVectorEnv` is incompatible with non-standard Gym observation spaces (i.e. custom spaces inheriting from `gym.Space`), and is only compatible with default Gym spaces (e.g. `Box`, `Tuple`, `Dict`) for batching. Set `shared_memory=False` if you use custom observation spaces.')
        else:
            _obs_buffer = None
        ...
        with clear_mpi_env_vars():
            for idx, env_fn in enumerate(self.env_fns):
                ...
                process = ctx.Process(target=target, name=f'Worker<{type(self).__name__}>-{idx}', args=(idx, CloudpickleWrapper(env_fn), child_pipe, parent_pipe, _obs_buffer, self.error_queue))
    ...
    def reset_async(self, seed: Optional[Union[int, List[int]]]=None, options: Optional[dict]=None):
        ...
        if self._state != AsyncState.DEFAULT:
            raise AlreadyPendingCallError(f'Calling `reset_async` while waiting for a pending call to `{self._state.value}` to complete', self._state.value)
    ...
    def reset_wait(self, timeout: Optional[Union[int, float]]=None, seed: Optional[int]=None, options: Optional[dict]=None) -> Union[ObsType, Tuple[ObsType, List[dict]]]:
        ...
        if self._state != AsyncState.WAITING_RESET:
            raise NoAsyncCallError('Calling `reset_wait` without any prior call to `reset_async`.', AsyncState.WAITING_RESET.value)
    ...
    def step_wait(self, timeout: Optional[Union[int, float]]=None) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, List[dict]]:
        ...
        self._state = AsyncState.DEFAULT
        if not self.shared_memory:
            self.observations = concatenate(self.single_observation_space, observations_list, self.observations)
        return (deepcopy(self.observations) if self.copy else self.observations, np.array(rewards), np.array(terminateds, dtype=np.bool_), np.array(truncateds, dtype=np.bool_), infos)
    ...
    def call_async(self, name: str, *args, **kwargs):
        ...
        self._assert_is_running()
        if self._state != AsyncState.DEFAULT:
            raise AlreadyPendingCallError(f'Calling `call_async` while waiting for a pending call to `{self._state.value}` to complete.', self._state.value)
    ...
    def call_wait(self, timeout: Optional[Union[int, float]]=None) -> list:
        ...
        if self._state != AsyncState.WAITING_CALL:
            raise NoAsyncCallError('Calling `call_wait` without any prior call to `call_async`.', AsyncState.WAITING_CALL.value)
        if not self._poll(timeout):
            self._state = AsyncState.DEFAULT
    ...
    def set_attr(self, name: str, values: Union[list, tuple, object]):
        ...
        if len(values) != self.num_envs:
            raise ValueError(f'Values must be a list or tuple with length equal to the number of environments. Got `{len(values)}` values for {self.num_envs} environments.')
        if self._state != AsyncState.DEFAULT:
            raise AlreadyPendingCallError(f'Calling `set_attr` while waiting for a pending call to `{self._state.value}` to complete.', self._state.value)
