...
from inspect import (
    ...
    iscoroutinefunction,
    iscoroutine,
)  # For inspecting callable objects and binding arguments
from typing import (
    Any,
    Callable,
    Dict,
    ...
    get_origin,
    get_args,
)  # For type hinting and annotations, including handling generics
            ...
            if expected_type is not None:
                ...
                if origin is None:  # Not a generic type
                    if not isinstance(value, expected_type):
                        raise TypeError(
                            f"Argument {arg} must be of type {expected_type}"
                        )
                else:  # Handle generic types
                    args = get_args(expected_type)
                    if not all(isinstance(value, arg) for arg in args):
                        raise TypeError(f"Argument {arg} must match the types {args}")
