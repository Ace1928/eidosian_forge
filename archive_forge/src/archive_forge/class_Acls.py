                            ...
                            if ace.get(direction):
                                if ace[direction].get('address'):
                                    ...
                                    for address in addresses:
                                        build_child_xml_node(from_node, '{0}-address'.format(direction), address)
                                if ace[direction].get('prefix_list'):
                                    for prefix in ace[direction].get('prefix_list'):
                                        build_child_xml_node(from_node, '{0}-prefix-list'.format(direction), prefix['name'])
