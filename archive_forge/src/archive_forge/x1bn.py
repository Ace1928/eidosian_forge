...
def save_visualization(
    ...
    if viz_type == "original_signal":
        ...
        # Ensure the signal is robustly handled for any size tensor
        mel_spectrogram = np.log(np.abs(fft(signal)) + 1e-10)

        # Handle large tensors by splitting into manageable chunks if necessary
        chunk_size = 1000  # Define a manageable chunk size
        num_chunks = (
            mel_spectrogram.shape[1] + chunk_size - 1
        ) // chunk_size  # Calculate the number of chunks

        for i in range(num_chunks):
            start_idx = i * chunk_size
            end_idx = min((i + 1) * chunk_size, mel_spectrogram.shape[1])
            fig, ax = plt.subplots(figsize=(10, 4))
            ax.imshow(
                mel_spectrogram[:, start_idx:end_idx],
                aspect="auto",
                origin="lower",
                cmap="viridis",
            )
            ...
            ax.set_title(f"{title} - Mel Spectrogram (Chunk {i + 1})")
            save_plot(fig, f"mel_spectrogram_{timestamp}_chunk_{i + 1}.png")
