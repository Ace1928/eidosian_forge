...
@pytest.mark.parametrize('row_indexer', [slice(1, 2), np.array([False, True, True]), np.array([1, 2])], ids=['slice', 'mask', 'array'])
@pytest.mark.parametrize('column_indexer', [slice('b', 'c'), np.array([False, True, True]), ['b', 'c']], ids=['slice', 'mask', 'array'])
def test_subset_loc_rows_columns(backend, dtype, row_indexer, column_indexer, using_array_manager, using_copy_on_write, warn_copy_on_write):
