...
class GameController:
    ...
    def set_algorithm(self, algo_type: str) -> None:
        ...
        if algo_type == "BFS":
            ...
            if not self.model_loaded:
                self.algo.population.initialize_population()
    ...
    def update_path_finding_algo(self, pos: Union[Node, List[Node]]) -> None:
        """
        Updates the state of the game when using a path-finding algorithm for AI control.
        This method now handles only the next immediate move.

        Args:
            pos (Union[Node, List[Node]]): The next position or positions for the snake to move to, if available.
        """
        logging.debug("Updating path-finding algorithm")
        if isinstance(pos, list) and pos:
            # Only take the first position from the list for the current update
            next_pos = pos[0]
            x = next_pos.x
            y = next_pos.y
            logging.debug(f"Processing next immediate position ({x}, {y}) from the list")
        elif isinstance(pos, Node):
            # Handle a single position
            x = pos.x
            y = pos.y
            logging.debug(f"Processing single position ({x}, {y})")
        else:
            # If no valid position is provided, continue moving in the last known direction
            x, y = self.keep_moving()
            logging.debug(f"No new position provided, continuing movement to ({x}, {y})")
