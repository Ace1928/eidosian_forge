...
def generate_aggregate_fn(key: Optional[str], aggs: List[AggregateFn]) -> AllToAllTransformFn:
    ...
    def fn(refs: List[RefBundle], ctx: TaskContext) -> Tuple[List[RefBundle], StatsDict]:
        ...
        if key is None:
            ...
            boundaries = SortTaskSpec.sample_boundaries(blocks, SortKey(key), num_outputs)
        agg_spec = SortAggregateTaskSpec(boundaries=boundaries, key=key, aggs=aggs)
