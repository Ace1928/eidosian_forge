...
class cuDFInterfaceTests(HeterogeneousColumnTests, InterfaceTests):
    ...
    def test_dataset_groupby(self):
        ...
        grouped = HoloMap([('M', Dataset(group1, kdims=['Age'], vdims=self.vdims)), ('F', Dataset(group2, kdims=['Age'], vdims=self.vdims))], kdims=['Gender'])
        self.assertEqual(self.table.groupby(['Gender']).apply('sort'), grouped.apply('sort'))

    def test_dataset_groupby_alias(self):
        group1 = {'age': [10, 16], 'weight': [15, 18], 'height': [0.8, 0.6]}
        group2 = {'age': [12], 'weight': [10], 'height': [0.8]}
        grouped = HoloMap([('M', Dataset(group1, kdims=[('age', 'Age')], vdims=self.alias_vdims)), ('F', Dataset(group2, kdims=[('age', 'Age')], vdims=self.alias_vdims))], kdims=[('gender', 'Gender')])
    ...
    def test_dataset_2D_aggregate_partial_hm(self):
        ...
        self.assertEqual(dataset.aggregate(['x'], np.mean).sort(), Dataset({'x': self.xs, 'z': z_ints}, kdims=['x'], vdims=['z']).sort())
    ...
    def test_dataset_groupby_second_dim(self):
        ...
        grouped = HoloMap([(10, Dataset(group1, kdims=['Gender'], vdims=self.vdims)), (16, Dataset(group2, kdims=['Gender'], vdims=self.vdims)), (12, Dataset(group3, kdims=['Gender'], vdims=self.vdims))], kdims=['Age'])
