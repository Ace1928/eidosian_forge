...
class VitrageShell(app.App):
    ...
    def __init__(self):
        super(VitrageShell, self).__init__(description=__doc__, version=__version__, command_manager=VitrageCommandManager('vitrageclient'), deferred_help=True)
        self._client = None
    ...
    def build_option_parser(self, description, version, **argparse_kwargs):
        ...
        """
        parser = super(VitrageShell, self).build_option_parser(description, version, argparse_kwargs={'allow_abbrev': False})
        parser.add_argument('--vitrage-api-version', default=os.environ.get('VITRAGE_API_VERSION', '1'), help='Defaults to env[VITRAGE_API_VERSION] or 1.')
        if profiler:
            parser.add_argument('--profile', metavar='HMAC_KEY', default=os.environ.get('OS_PROFILE'), help='HMAC key to use for encrypting context data for performance profiling of request. This key should be the value of the HMAC key configured for the osprofiler middleware in Vitrage api; it is specified in the Vitrage configuration file at"/etc/vitrage/vitrage.conf". Without the key, profiling will not be triggered even if osprofiler is enabled on the server side.')
        plugin = self.register_keyauth_argparse_arguments(parser)
        if not isinstance(plugin, (auth.VitrageNoAuthLoader, auth.VitrageKeycloakLoader)):
            parser.add_argument('--endpoint', default=os.environ.get('VITRAGE_ENDPOINT'), help='Vitrage endpoint (Env: VITRAGE_ENDPOINT)')
        return parser
    ...
    def client(self):
        if self._client is None:
            ...
            self._client = client.Client(self.options.vitrage_api_version, session=session, interface=self.options.interface, region_name=self.options.region_name, endpoint_override=endpoint_override)
        return self._client
