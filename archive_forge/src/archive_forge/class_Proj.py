...
class Proj(Transformer):
    ...
    def __init__(self, projparams: Optional[Any]=None, preserve_units: bool=True, **kwargs) -> None:
        """
        ...
        **kwargs:
        ...
        if not preserve_units and 'foot' in self.crs.axis_info[0].unit_name:
            with warnings.catch_warnings():
                warnings.filterwarnings('ignore', 'You will likely lose important projection information', UserWarning)
                projstring = self.crs.to_proj4(4)
            projstring = re.sub('\\s\\+units=[\\w-]+', '', projstring)
            projstring += ' +units=m'
            self.crs = CRS(projstring)
        with warnings.catch_warnings():
            warnings.filterwarnings('ignore', 'You will likely lose important projection information', UserWarning)
            projstring = self.crs.to_proj4() or self.crs.srs
        self.srs = re.sub('\\s\\+?type=crs', '', projstring).strip()
        super().__init__(TransformerFromPipeline(cstrencode(self.srs)))

    def __call__(self, longitude: Any, latitude: Any, inverse: bool=False, errcheck: bool=False, radians: bool=False) -> tuple[Any, Any]:
        """
        ...
        return self.transform(xx=longitude, yy=latitude, direction=direction, errcheck=errcheck, radians=radians)

    def get_factors(self, longitude: Any, latitude: Any, radians: bool=False, errcheck: bool=False) -> Factors:
        """
        .. versionadded:: 2.6.0
        ...
        factors = self._transformer._get_factors(inx, iny, radians=radians, errcheck=errcheck)
