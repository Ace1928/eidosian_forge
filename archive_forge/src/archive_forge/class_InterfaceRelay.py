...
class InterfaceRelay(InterfaceQueue):
    ...
    def __init__(self, mailbox: Mailbox, record_q: Optional['Queue[pb.Record]']=None, result_q: Optional['Queue[pb.Result]']=None, relay_q: Optional['Queue[pb.Result]']=None, process: Optional[BaseProcess]=None, process_check: bool=True) -> None:
        self.relay_q = relay_q
        super().__init__(record_q=record_q, result_q=result_q, process=process, process_check=process_check, mailbox=mailbox)
