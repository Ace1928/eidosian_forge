...
class JobBase(six.with_metaclass(abc.ABCMeta, object)):
    ...
    def GetStagingDir(self, cluster, job_id, bucket=None):
        ...
        staging_dir = 'gs://{bucket}/{prefix}/{cluster_id}/jobs/{job_id}/staging/'.format(bucket=bucket, prefix=constants.GCS_METADATA_PREFIX, cluster_id=cluster_id, job_id=job_id)
        return staging_dir
