...
@pytest.mark.parametrize('filename', ['test_json_glob.json', 'test_json_glob*.json'])
def test_json_glob(tmp_path, filename):
    data = test_data['int_data']
    df = pd.DataFrame(data)
    filename_param = filename
    with warns_that_defaulting_to_pandas() if filename_param == 'test_json_glob.json' else contextlib.nullcontext():
