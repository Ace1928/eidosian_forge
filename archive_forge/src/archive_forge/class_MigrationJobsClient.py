...
class MigrationJobsClient(object):
    ...
    def _GetReverseSshConnectivity(self, args):
        return self.messages.ReverseSshConnectivity(vm=args.vm, vmIp=args.vm_ip, vmPort=args.vm_port, vpc=args.vpc)
    ...
    def _UpdateLabels(self, args, migration_job, update_fields):
        ...
        update_result = labels_util.Diff(additions=add_labels, subtractions=remove_labels, clear=args.clear_labels).Apply(value_type)
    ...
    def _GetUpdateMask(self, args):
        ...
        update_fields = [resource_property.ConvertToCamelCase(field) for field in sorted(self._FIELDS_MAP) if args.IsSpecified(field)]
        update_fields.extend(['reverseSshConnectivity.{0}'.format(resource_property.ConvertToCamelCase(field)) for field in sorted(self._REVERSE_MAP) if args.IsSpecified(field)])
        if args.IsSpecified('peer_vpc'):
        ...
        if args.IsKnownAndSpecified('commit_id') or args.IsKnownAndSpecified('filter'):
    ...
    def _GetExistingMigrationJob(self, name):
        get_req = self.messages.DatamigrationProjectsLocationsMigrationJobsGetRequest(name=name)
        return self._service.Get(get_req)

    def Create(self, parent_ref, migration_job_id, source_ref, destination_ref, conversion_workspace_ref=None, cmek_key_ref=None, args=None):
    ...
    """
        ...
        migration_job = self._GetMigrationJob(source_ref, destination_ref, conversion_workspace_ref, cmek_key_ref, args)
        ...
        create_req = create_req_type(migrationJob=migration_job, migrationJobId=migration_job_id, parent=parent_ref, requestId=request_id)
    ...
    """
        ...
        if hasattr(current_mj, 'conversionWorkspace') and current_mj.conversionWorkspace is not None:
            self._ValidateConversionWorkspaceMessageArgs(current_mj.conversionWorkspace, args)
            current_mj.conversionWorkspace = self._ComplementConversionWorkspaceInfo(current_mj.conversionWorkspace, args)
        migration_job, update_fields = self._GetUpdatedMigrationJob(current_mj, source_ref, destination_ref, args)
        request_id = api_util.GenerateRequestId()
        update_req_type = self.messages.DatamigrationProjectsLocationsMigrationJobsPatchRequest
        update_req = update_req_type(migrationJob=migration_job, name=name, requestId=request_id, updateMask=','.join(update_fields))
