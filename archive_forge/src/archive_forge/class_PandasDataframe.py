...
class PandasDataframe(ClassLogger, modin_layer='CORE-DATAFRAME'):
    ...
    def __init__(self, partitions, index=None, columns=None, row_lengths=None, column_widths=None, dtypes=None):
    ...
    def set_dtypes_cache(self, dtypes):
        ...
        if dtypes is None and self.has_materialized_columns:
            try:
                self._dtypes = ModinDtypes(DtypesDescriptor(cols_with_unknown_dtypes=self.columns.tolist(), parent_df=self))
    ...
    def _set_columns(self, new_columns):
        ...
        if self.has_materialized_columns:
            if isinstance(new_columns, pandas.Index) and self.columns.identical(new_columns) or (not isinstance(new_columns, pandas.Index) and np.array_equal(self.columns.values, new_columns)):
                return
    ...
    def take_2d_labels_or_positional(self, row_labels: Optional[List[Hashable]]=None, row_positions: Optional[List[int]]=None, col_labels: Optional[List[Hashable]]=None, col_positions: Optional[List[int]]=None) -> 'PandasDataframe':
        """
        ...
        if check_both_not_none(row_labels, row_positions):
            raise ValueError('Both row_labels and row_positions were provided - ' + 'please provide only one of row_labels and row_positions.')
        if check_both_not_none(col_labels, col_positions):
            raise ValueError('Both col_labels and col_positions were provided - ' + 'please provide only one of col_labels and col_positions.')
    ...
    def from_labels(self) -> 'PandasDataframe':
        ...
        if self.index.nlevels > 1:
            level_names = [self.index.names[i] if self.index.names[i] is not None else 'level_{}'.format(i) for i in range(self.index.nlevels)]
        else:
            level_names = [self.index.names[0] if self.index.names[0] is not None else 'index' if 'index' not in self.columns else 'level_{}'.format(0)]
        ...
        if self.columns.nlevels > 1:
            new_column_names = pandas.MultiIndex.from_tuples([tuple(list(level) + [''] * (self.columns.nlevels - len(level)) if isinstance(level, tuple) else [level] + [''] * (self.columns.nlevels - 1)) for level in level_names], names=self.columns.names)
        else:
        ...
        def from_labels_executor(df, **kwargs):
            ...
            return result
        new_parts = self._partition_mgr_cls.apply_func_to_select_indices(0, self._partitions, from_labels_executor, [0], keep_remaining=True)
        new_column_widths = [self.index.nlevels + self.column_widths[0]] + self.column_widths[1:]
        result = self.__constructor__(new_parts, new_row_labels, new_columns, row_lengths=self._row_lengths_cache, column_widths=new_column_widths, dtypes=new_dtypes)
    ...
    def to_labels(self, column_list: List[Hashable]) -> 'PandasDataframe':
        ...
        """
        extracted_columns = self.take_2d_labels_or_positional(col_labels=column_list).to_pandas()
        if len(column_list) == 1:
            new_labels = pandas.Index(extracted_columns.squeeze(axis=1), name=column_list[0])
        else:
            new_labels = pandas.MultiIndex.from_frame(extracted_columns, names=column_list)
        result = self.take_2d_labels_or_positional(col_labels=[i for i in self.columns if i not in extracted_columns.columns])
    ...
    def copy(self):
        ...
        """
        return self.__constructor__(self._partitions, self.copy_index_cache(copy_lengths=True), self.copy_columns_cache(copy_lengths=True), self._row_lengths_cache, self._column_widths_cache, self.copy_dtypes_cache())
    ...
    def astype(self, col_dtypes, errors: str='raise'):
        ...
        if isinstance(col_dtypes, dict):
            for column, dtype in col_dtypes.items():
                if not is_dtype_equal(dtype, self_dtypes[column]):
                    ...
                    if isinstance(new_dtype, pandas.CategoricalDtype):
                        new_dtypes[column] = LazyProxyCategoricalDtype._build_proxy(parent=None, column_name=column, materializer=lambda parent, column: parent._compute_dtypes(columns=[column])[column])
            ...
            def astype_builder(df):
                ...
                return df.astype({k: v for k, v in col_dtypes.items() if k in df}, errors=errors)
        else:
            if not (col_dtypes == self_dtypes).all():
                ...
                if isinstance(new_dtype, pandas.CategoricalDtype):
                    new_dtypes[:] = new_dtypes.to_frame().apply(lambda column: LazyProxyCategoricalDtype._build_proxy(parent=None, column_name=column.index[0], materializer=lambda parent, column: parent._compute_dtypes(columns=[column])[column]))[0]
                    use_full_axis_cast = True
                else:
        ...
        if use_full_axis_cast:
            new_frame = self._partition_mgr_cls.map_axis_partitions(0, self._partitions, astype_builder, keep_partitioning=True)
        else:
            new_frame = self._partition_mgr_cls.lazy_map_partitions(self._partitions, astype_builder)
        return self.__constructor__(new_frame, self.copy_index_cache(copy_lengths=True), self.copy_columns_cache(copy_lengths=True), self._row_lengths_cache, self._column_widths_cache, new_dtypes)
    ...
    @lazy_metadata_decorator(apply_axis='both')
    def reduce(self, axis: Union[int, Axis], function: Callable, dtypes: Optional[str]=None) -> 'PandasDataframe':
        """
        ...
        new_parts = self._partition_mgr_cls.map_axis_partitions(axis.value, self._partitions, function)
    ...
    def tree_reduce(self, axis: Union[int, Axis], map_func: Callable, reduce_func: Optional[Callable]=None, dtypes: Optional[str]=None) -> 'PandasDataframe':
        """
        ...
        reduce_parts = self._partition_mgr_cls.map_axis_partitions(axis.value, map_parts, reduce_func)
        return self._compute_tree_reduce_metadata(axis.value, reduce_parts, dtypes=dtypes)

    @lazy_metadata_decorator(apply_axis=None)
    def map(self, func: Callable, dtypes: Optional[str]=None, new_columns: Optional[pandas.Index]=None, func_args=None, func_kwargs=None, lazy=False) -> 'PandasDataframe':
        """
        ...
        PandasDataframe
            A new dataframe.
        """
        map_fn = self._partition_mgr_cls.lazy_map_partitions if lazy else self._partition_mgr_cls.map_partitions
        ...
        if new_columns is not None and self.has_materialized_columns:
            assert len(new_columns) == len(self.columns), "New column's length must be identical to the previous columns"
        ...
        if isinstance(dtypes, str) and dtypes == 'copy':
            ...
            dtypes = pandas.Series([pandas.api.types.pandas_dtype(dtypes)] * len(new_columns), index=new_columns)
        return self.__constructor__(new_partitions, self.copy_index_cache(copy_lengths=True), new_columns, self._row_lengths_cache, self._column_widths_cache, dtypes=dtypes)

    def window(self, axis: Union[int, Axis], reduce_fn: Callable, window_size: int, result_schema: Optional[Dict[Hashable, type]]=None) -> 'PandasDataframe':
        """
    ...
    def fold(self, axis, func, new_columns=None):
        ...
        if new_columns is not None:
            if self.has_materialized_columns:
                assert len(self.columns) == len(new_columns), "The length of `new_columns` doesn't match the columns' length of `self`"
            self.set_columns_cache(new_columns)
        new_partitions = self._partition_mgr_cls.map_axis_partitions(axis, self._partitions, func, keep_partitioning=True)
        return self.__constructor__(new_partitions, self.copy_index_cache(copy_lengths=True), self.copy_columns_cache(copy_lengths=True), self._row_lengths_cache, self._column_widths_cache)
    ...
    def infer_objects(self) -> 'PandasDataframe':
        ...
        """
        obj_cols = [col for col, dtype in enumerate(self.dtypes) if is_object_dtype(dtype)]
    ...
    def infer_types(self, col_labels: List[str]) -> 'PandasDataframe':
        ...
        return self.__constructor__(self._partitions, self.copy_index_cache(copy_lengths=True), self.copy_columns_cache(copy_lengths=True), self._row_lengths_cache, self._column_widths_cache, new_dtypes)
    ...
    def rename(self, new_row_labels: Optional[Union[Dict[Hashable, Hashable], Callable]]=None, new_col_labels: Optional[Union[Dict[Hashable, Hashable], Callable]]=None) -> 'PandasDataframe':
        """
    ...
    @lazy_metadata_decorator(apply_axis='both')
    def sort_by(self, axis: Union[int, Axis], columns: Union[str, List[str]], ascending: bool=True, **kwargs) -> 'PandasDataframe':
        """
        ...
        **kwargs : dict
        ...
        axis = Axis(axis)
        if axis != Axis.ROW_WISE:
            raise NotImplementedError(f'Algebra sort only implemented row-wise. {axis.name} sort not implemented yet!')
        result = self._apply_func_to_range_partitioning(key_columns=[columns[0]], func=sort_function, ascending=ascending, preserve_columns=True, **kwargs)
    ...
    def filter(self, axis: Union[Axis, int], condition: Callable) -> 'PandasDataframe':
        ...
        """
        axis = Axis(axis)
        assert axis in (Axis.ROW_WISE, Axis.COL_WISE), 'Axis argument to filter operator must be 0 (rows) or 1 (columns)'
        new_partitions = self._partition_mgr_cls.map_axis_partitions(axis.value, self._partitions, condition, keep_partitioning=True)
        new_axes, new_lengths = ([0, 0], [0, 0])
        new_axes[axis.value] = self.copy_axis_cache(axis.value, copy_lengths=True)
        new_lengths[axis.value] = self._row_lengths_cache if axis.value == 0 else self._column_widths_cache
        new_axes[axis.value ^ 1], new_lengths[axis.value ^ 1] = (None, None)
        return self.__constructor__(new_partitions, *new_axes, *new_lengths, self.copy_dtypes_cache() if axis == Axis.COL_WISE else None)
    ...
    def filter_by_types(self, types: List[Hashable]) -> 'PandasDataframe':
        ...
        """
        return self.take_2d_labels_or_positional(col_positions=[i for i, dtype in enumerate(self.dtypes) if dtype in types])
    ...
    def explode(self, axis: Union[int, Axis], func: Callable) -> 'PandasDataframe':
        ...
        """
        axis = Axis(axis)
        partitions = self._partition_mgr_cls.map_axis_partitions(axis.value, self._partitions, func, keep_partitioning=True)
        if axis == Axis.COL_WISE:
            new_index, row_lengths = self._compute_axis_labels_and_lengths(0, partitions)
            new_columns, column_widths = (self.columns, self._column_widths_cache)
        else:
            new_index, row_lengths = (self.index, self._row_lengths_cache)
            new_columns, column_widths = self._compute_axis_labels_and_lengths(1, partitions)
        return self.__constructor__(partitions, new_index, new_columns, row_lengths, column_widths)
    ...
    def _extract_partitions(self):
        ...
        if self._partitions.size > 0:
            ...
            return self._partition_mgr_cls.create_partition_from_metadata(index=self.index, columns=self.columns)
    ...
    def concat(self, axis: Union[int, Axis], others: Union['PandasDataframe', List['PandasDataframe']], how, sort) -> 'PandasDataframe':
        ...
        def _compute_new_widths():
            widths = None
            if self._column_widths_cache is not None and all((o._column_widths_cache is not None for o in others)):
                widths = self._column_widths_cache + [width for o in others for width in o._column_widths_cache]
            return widths
        if axis == Axis.ROW_WISE and all((o._check_if_axes_identical(self, axis=1) for o in others)):
            joined_index = self.copy_columns_cache(copy_lengths=True)
            left_parts = self._partitions
            right_parts = [o._partitions for o in others]
            new_widths = self._column_widths_cache
        elif axis == Axis.COL_WISE and all((o._check_if_axes_identical(self, axis=0) for o in others)):
            ...
            left_parts, right_parts, joined_index, partition_sizes_along_axis = self._copartition(axis.value ^ 1, others, how, sort, force_repartition=False)
            if axis == Axis.COL_WISE:
                new_lengths = partition_sizes_along_axis
                new_widths = _compute_new_widths()
            else:
                new_widths = partition_sizes_along_axis
        new_partitions, new_lengths2 = self._partition_mgr_cls.concat(axis.value, left_parts, right_parts)
        ...
        if axis == Axis.ROW_WISE:
            ...
            if not new_lengths:
                ...
                if new_partitions.size > 0:
                    if all((part._length_cache is not None for part in new_partitions.T[0])):
            ...
            try:
                new_dtypes = ModinDtypes.concat([self.copy_dtypes_cache()] + [o.copy_dtypes_cache() for o in others])
            except NotImplementedError:
                new_dtypes = None
            if not new_widths:
                new_widths = []
    ...
    @lazy_metadata_decorator(apply_axis='both')
    def groupby(self, axis: Union[int, Axis], internal_by: List[str], external_by: List['PandasDataframe'], by_positions: List[int], operator: Callable, result_schema: Optional[Dict[Hashable, type]]=None, align_result_columns: bool=False, series_groupby: bool=False, add_missing_cats: bool=False, **kwargs: dict) -> 'PandasDataframe':
        """
        ...
        **kwargs : dict
        ...
        axis = Axis(axis)
        if axis != Axis.ROW_WISE:
            raise NotImplementedError(f'Algebra groupby only implemented row-wise. {axis.name} axis groupby not implemented yet!')
        ...
        duplicated_pattern = '_[\\d]*__duplicated_suffix__'
        ...
        def apply_func(df):
            ...
            return result
        if has_external_grouper:
            grouper = external_by[0] if len(external_by) == 1 else external_by[0].concat(axis=1, others=external_by[1:], how='left', sort=False)
            new_grouper_cols = []
            ...
            for col in grouper.columns:
                ...
                if suffix is None:
                    ...
                    same_columns[col] += 1
                    col = (*col[:-1], f'{col[-1]}_{suffix}{duplicated_suffix}') if isinstance(col, tuple) else f'{col}_{suffix}{duplicated_suffix}'
        ...
        result = grouper._apply_func_to_range_partitioning(key_columns=grouper_key_columns, func=apply_func, data=data, data_key_columns=data_key_columns, level=level)
        if add_missing_cats or (align_result_columns and result._partitions.shape[0] > 1):
            if result._partitions.shape[1] > 1:
                raise NotImplementedError('Aligning columns is not yet implemented for multiple column partitions.')
            if add_missing_cats or not IsRayCluster.get():
                if self.has_materialized_dtypes:
                    original_dtypes = pandas.Series({name: 'category' if isinstance(dtype, LazyProxyCategoricalDtype) else dtype for name, dtype in self.dtypes.items()})
                else:
                ...
                external_by_cols = [None if col.startswith(MODIN_UNNAMED_SERIES_LABEL) else col for obj in external_by for col in obj.columns]
                by = []
                for idx in by_positions:
                    if idx >= 0:
                ...
                aligned_columns = parts[0].apply(compute_aligned_columns, *[part._data for part in parts[1:]], initial_columns=pandas.Index(external_by_cols).append(self.columns), by=by)
                ...
                def apply_aligned(df, args, partition_idx):
                    ...
                    return df
                new_partitions = self._partition_mgr_cls.lazy_map_partitions(result._partitions, apply_aligned, func_args=(aligned_columns._data,), enumerate_partitions=True)
            else:

                def join_cols(df, *cols):
                    """Join `cols` and apply the joined columns to `df`."""
                    valid_cols = [pandas.DataFrame(columns=col) for col in cols if col is not None]
                    if len(valid_cols) == 0:
                        return df
                    result_col = pandas.concat(valid_cols, axis=0, join='outer').columns
                    return df.reindex(columns=result_col)
                cols = [part.apply(lambda df: None if df.attrs.get(skip_on_aligning_flag, False) else df.columns)._data for part in result._partitions.flatten()]
                new_partitions = self._partition_mgr_cls.lazy_map_partitions(result._partitions, join_cols, func_args=cols)
            result = self.__constructor__(new_partitions, index=result.copy_index_cache(), row_lengths=result._row_lengths_cache)
        if not result.has_materialized_index and (not has_external_grouper) and (level is None):
    ...
    def from_pandas(cls, df):
        ...
        new_frame, new_lengths, new_widths = cls._partition_mgr_cls.from_pandas(df, True)
        return cls(new_frame, new_index, new_columns, new_lengths, new_widths, dtypes=new_dtypes)
    ...
    def from_arrow(cls, at):
        ...
        PandasDataframe
            New Modin DataFrame.
        """
        new_frame, new_lengths, new_widths = cls._partition_mgr_cls.from_arrow(at, return_dims=True)
        new_columns = Index.__new__(Index, data=at.column_names, dtype='O')
        new_index = Index.__new__(RangeIndex, data=range(at.num_rows))
        new_dtypes = pandas.Series([cls._arrow_type_to_dtype(col.type) for col in at.columns], index=at.column_names)
        return cls(partitions=new_frame, index=new_index, columns=new_columns, row_lengths=new_lengths, column_widths=new_widths, dtypes=new_dtypes)
    ...
    def to_pandas(self):
        ...
        if df.empty:
            ...
            for axis, has_external_index in enumerate(['has_materialized_index', 'has_materialized_columns']):
                if getattr(self, has_external_index):
                    external_index = self.columns if axis else self.index
                    ErrorMessage.catch_bugs_and_request_email(not df.axes[axis].equals(external_index), f'Internal and external indices on axis {axis} do not match.')
    ...
    def to_numpy(self, **kwargs):
        ...
        **kwargs : dict
        ...
        ErrorMessage.catch_bugs_and_request_email(self.has_materialized_index and len(arr) != len(self.index) or (self.has_materialized_columns and len(arr[0]) != len(self.columns)))
        return arr
    ...
    def transpose(self):
        ...
        PandasDataframe
            New Modin DataFrame.
        """
        new_partitions = self._partition_mgr_cls.lazy_map_partitions(self._partitions, lambda df: df.T).T
        if self.has_materialized_dtypes:
            new_dtypes = pandas.Series(np.full(len(self.index), find_common_type(self.dtypes.values)), index=self.index)
        else:
            new_dtypes = None
        return self.__constructor__(new_partitions, self.copy_columns_cache(copy_lengths=True), self.copy_index_cache(copy_lengths=True), self._column_widths_cache, self._row_lengths_cache, dtypes=new_dtypes)
    ...
    def __dataframe__(self, nan_as_null: bool=False, allow_copy: bool=True):
        ...
        """
        from modin.core.dataframe.pandas.interchange.dataframe_protocol.dataframe import PandasProtocolDataframe
        return PandasProtocolDataframe(self, nan_as_null=nan_as_null, allow_copy=allow_copy)
    ...
    def from_dataframe(cls, df: 'ProtocolDataframe') -> 'PandasDataframe':
        ...
        if not hasattr(df, '__dataframe__'):
            raise ValueError('`df` does not support DataFrame exchange protocol, i.e. `__dataframe__` method')
        from modin.core.dataframe.pandas.interchange.dataframe_protocol.from_dataframe import from_dataframe_to_pandas
    ...
    def case_when(self, caselist):
        ...
        @remote_function
        def remote_fn(df, name, caselist):
            caselist = [tuple((data.squeeze(axis=1) if isinstance(data, pandas.DataFrame) else data for data in case_tuple)) for case_tuple in caselist]
        ...
        for case_tuple in caselist:
            new_case = []
            for data, modin_list, fill_value in zip(case_tuple, modin_lists, fill_values):
                if isinstance(data, cls):
                    ...
                    if is_trivial_idx and is_trivial_index(data.index):
                        ...
                        if diff > 0:
                            data = pandas.concat([data, pandas.Series([fill_value] * diff)], ignore_index=True)
                    else:
        ...
        if modin_lists[0] or modin_lists[1]:
            ...
            for modin_list, fill_value in zip(modin_lists, fill_values):
                _, list_of_right_parts, joined_index, row_lengths = self._copartition(Axis.ROW_WISE.value, modin_list, how='left', sort=False, fill_value=fill_value)
                modin_list.clear()
                modin_list.extend((self.__constructor__(part, joined_index, columns, row_lengths, column_widths) for part in list_of_right_parts))
            caselist = new_caselist
            new_caselist = []
            for i in range(2):
                modin_lists[i] = iter(modin_lists[i])
            for case_tuple in caselist:
                new_case = tuple((next(modin_list) if isinstance(data, cls) else data for data, modin_list in zip(case_tuple, modin_lists)))
                new_caselist.append(new_case)
        if use_map:
            return self.map(func=remote_fn, func_args=[name, new_caselist], lazy=True)

        def map_data(part_idx, part_len, data, data_offset, fill_value):
            if isinstance(data, cls):
                return data._partitions[part_idx][0]._data
            if isinstance(data, pandas.Series):
                return data[data_offset:data_offset + part_len]
            return data[data_offset:data_offset + part_len] if is_list_like(data) else data
        parts = [p[0] for p in self._partitions]
        lengths = self.row_lengths
        new_parts = []
        data_offset = 0
        for i, part, part_len in zip(range(len(parts)), parts, lengths):
            cases = [tuple((map_data(i, part_len, data, data_offset, fill_value) for data, fill_value in zip(c, (True, None)))) for c in new_caselist]
            new_parts.append(part.add_to_apply_calls(remote_fn, name, cases, length=part_len, width=1))
            data_offset += part_len
        new_parts = np.array([[p] for p in new_parts])
        return self.__constructor__(new_parts, columns=self.columns, index=self.index, row_lengths=lengths, column_widths=[1])