...
def _on_background_task_done(self, task: asyncio.Task[None]) -> None:
    ...
    exc = task.exception()
    if exc is not None:
        get_running_loop().call_exception_handler({'message': f'prompt_toolkit.Application background task {task!r} raised an unexpected exception.', 'exception': exc, 'task': task})