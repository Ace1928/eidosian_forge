...
class MigrationKeyManagerTestCase(test_key_manager.KeyManagerTestCase):
    ...
    def _create_key_manager(self):
        self.fixed_key = '1' * 64
        try:
            self.conf.register_opt(cfg.StrOpt('fixed_key'), group='key_manager')
    ...
    def setUp(self):
        ...
        self.fixed_key_secret = key.SymmetricKey('AES', fixed_key_length, fixed_key_bytes)
    ...
    def test_delete_fixed_key_fail_bad_context(self):
        self.assertRaises(exception.Forbidden, self.key_mgr.delete, context=None, managed_object_id=self.fixed_key_id)

    def test_get_other_key(self):
        self.assertRaises(exception.KeyManagerError, self.key_mgr.get, context=self.ctxt, managed_object_id=self.other_key_id)
    ...
    def test_using_conf_key_manager(self):
        conf = self.conf
        ckm_backend = 'castellan.tests.unit.key_manager.test_migration_key_manager.ConfKeyManager'
        conf.set_override('backend', ckm_backend, group='key_manager')
        key_mgr = key_manager.API(conf)
        self.assertNotEqual('MigrationKeyManager', type(key_mgr).__name__)
        self.assertRaises(NotImplementedError, key_mgr.get, context=self.ctxt, managed_object_id=self.fixed_key_id)