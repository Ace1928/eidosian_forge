...
class DatabricksVectorSearch(VectorStore):
    ...
    def __init__(self, index: VectorSearchIndex, *, embedding: Optional[Embeddings]=None, text_column: Optional[str]=None, columns: Optional[List[str]]=None):
        try:
            from databricks.vector_search.client import VectorSearchIndex
        except ImportError as e:
            raise ImportError('Could not import databricks-vectorsearch python package. Please install it with `pip install databricks-vectorsearch`.') from e
        self.index = index
        ...
        if self._is_databricks_managed_embeddings():
            ...
            if text_column is not None and text_column != index_source_column:
                raise ValueError(f"text_column '{text_column}' does not match with the source column of the index: '{index_source_column}'.")
        ...
        if not self._is_databricks_managed_embeddings():
            ...
            if index_embedding_dimension is not None:
                ...
                if inferred_embedding_dimension != index_embedding_dimension:
                    raise ValueError(f"embedding model's dimension '{inferred_embedding_dimension}' does not match with the index's dimension '{index_embedding_dimension}'.")
        else:
    ...
    def add_texts(self, texts: Iterable[str], metadatas: Optional[List[dict]]=None, ids: Optional[List[Any]]=None, **kwargs: Any) -> List[str]:
        ...
        updates = [{self.primary_key: id_, self.text_column: text, self._embedding_vector_column_name(): vector, **metadata} for text, vector, id_, metadata in zip(texts, vectors, ids, metadatas)]
    ...
    def similarity_search(self, query: str, k: int=4, filters: Optional[Any]=None, **kwargs: Any) -> List[Document]:
        ...
        """
        docs_with_score = self.similarity_search_with_score(query=query, k=k, filters=filters, **kwargs)
        return [doc for doc, _ in docs_with_score]

    def similarity_search_with_score(self, query: str, k: int=4, filters: Optional[Any]=None, **kwargs: Any) -> List[Tuple[Document, float]]:
        ...
        search_resp = self.index.similarity_search(columns=self.columns, query_text=query_text, query_vector=query_vector, filters=filters, num_results=k)
    ...
    def max_marginal_relevance_search(self, query: str, k: int=4, fetch_k: int=20, lambda_mult: float=0.5, filters: Optional[Any]=None, **kwargs: Any) -> List[Document]:
        ...
        if not self._is_databricks_managed_embeddings():
            ...
            raise ValueError('`max_marginal_relevance_search` is not supported for index with Databricks-managed embeddings.')
        docs = self.max_marginal_relevance_search_by_vector(query_vector, k, fetch_k, lambda_mult=lambda_mult, filters=filters)
        return docs

    def max_marginal_relevance_search_by_vector(self, embedding: List[float], k: int=4, fetch_k: int=20, lambda_mult: float=0.5, filters: Optional[Any]=None, **kwargs: Any) -> List[Document]:
        """Return docs selected using the maximal marginal relevance.
        ...
        if not self._is_databricks_managed_embeddings():
            ...
            raise ValueError('`max_marginal_relevance_search` is not supported for index with Databricks-managed embeddings.')
        search_resp = self.index.similarity_search(columns=list(set(self.columns + [embedding_column])), query_text=None, query_vector=embedding, filters=filters, num_results=fetch_k)
    ...
    def similarity_search_by_vector(self, embedding: List[float], k: int=4, filters: Optional[Any]=None, **kwargs: Any) -> List[Document]:
        ...
        Returns:
            List of Documents most similar to the embedding.
        """
        docs_with_score = self.similarity_search_by_vector_with_score(embedding=embedding, k=k, filters=filters, **kwargs)
        return [doc for doc, _ in docs_with_score]

    def similarity_search_by_vector_with_score(self, embedding: List[float], k: int=4, filters: Optional[Any]=None, **kwargs: Any) -> List[Tuple[Document, float]]:
        ...
        search_resp = self.index.similarity_search(columns=self.columns, query_vector=embedding, filters=filters, num_results=k)
        return self._parse_search_response(search_resp)

    def _parse_search_response(self, search_resp: dict, ignore_cols: Optional[List[str]]=None) -> List[Tuple[Document, float]]:
        ...
        for result in search_resp.get('result', dict()).get('data_array', []):
            ...
            metadata = {col: value for col, value in zip(columns[:-1], result[:-1]) if col not in [self.primary_key, self.text_column] + ignore_cols}
