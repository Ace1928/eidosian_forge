...
def _send_file_internal(self, fp, headers=None, cb=None, num_cb=10, query_args=None, chunked_transfer=False, size=None, hash_algs=None):
    ...
    if content_type_headers:
        if len(content_type_headers) == 1 and headers[content_type_headers[0]] is None:
    ...
    resp = self.bucket.connection.make_request('PUT', self.bucket.name, self.name, headers, sender=sender, query_args=query_args)
