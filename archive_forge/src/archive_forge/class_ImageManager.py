...
class ImageManager(base.Manager):
    ...
    def list(self, marker=None, limit=None, sort_key=None, sort_dir=None):
        ...
        if filters:
            path += '?' + '&'.join(filters)
        if limit is None:
            return self._list(self._path(path), 'images')
        else:
            return self._list_pagination(self._path(path), 'images', limit=limit)
    ...
    def create(self, **kwargs):
        ...
        for key, value in kwargs.items():
            if key in PULL_ATTRIBUTES:
                ...
                raise exceptions.InvalidAttribute('Key must be in %s' % ','.join(PULL_ATTRIBUTES))
