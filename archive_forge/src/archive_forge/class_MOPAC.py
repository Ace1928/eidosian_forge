...
class MOPAC(FileIOCalculator):
    ...
    default_parameters = dict(method='PM7', task='1SCF GRADIENTS', charge=0, relscf=0.0001)
    methods = ['AM1', 'MNDO', 'MNDOD', 'PM3', 'PM6', 'PM6-D3', 'PM6-DH+', 'PM6-DH2', 'PM6-DH2X', 'PM6-D3H4', 'PM6-D3H4X', 'PMEP', 'PM7', 'PM7-TS', 'RM1']

    def __init__(self, restart=None, ignore_bad_restart_file=FileIOCalculator._deprecated, label='mopac', atoms=None, **kwargs):
    ...
    def read_results(self):
        ...
        for i, line in enumerate(lines):
            if line.find('TOTAL ENERGY') != -1:
                ...
                self.results['magmom'] = abs(self.no_alpha_electrons - self.no_beta_electrons)
            elif line.find('FINAL  POINT  AND  DERIVATIVES') != -1:
                forces = [-float(line.split()[6]) for line in lines[i + 3:i + 3 + 3 * len(self.atoms)]]
                ...
                self.results['dipole'] = np.array(lines[i + 3].split()[1:1 + 3], float) * Debye
