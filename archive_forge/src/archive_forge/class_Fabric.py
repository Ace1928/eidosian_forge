...
class Fabric:
    ...
    def __init__(self, *, accelerator: Union[str, Accelerator]='auto', strategy: Union[str, Strategy]='auto', devices: Union[List[int], str, int]='auto', num_nodes: int=1, precision: Optional[_PRECISION_INPUT]=None, plugins: Optional[Union[_PLUGIN_INPUT, List[_PLUGIN_INPUT]]]=None, callbacks: Optional[Union[List[Any], Any]]=None, loggers: Optional[Union[Logger, List[Logger]]]=None) -> None:
        self._connector = _Connector(accelerator=accelerator, strategy=strategy, devices=devices, num_nodes=num_nodes, precision=precision, plugins=plugins)
    ...
    def setup(self, module: nn.Module, *optimizers: Optimizer, move_to_device: bool=True, _reapply_compile: bool=True) -> Any:
        ...
        if optimizers:
            module, optimizers = self._strategy.setup_module_and_optimizers(module, list(optimizers))
        else:
        ...
        _update_properties(module, device=self.device if move_to_device else next(module.parameters(), torch.tensor(0)).device)
        ...
        if hasattr(original_module, '_fabric'):
    ...
    def setup_module(self, module: nn.Module, move_to_device: bool=True, _reapply_compile: bool=True) -> _FabricModule:
        ...
        if isinstance(self._strategy, DeepSpeedStrategy):
            if model is None:
                ...
                if self._models_setup > 1:
                    raise ValueError('When using multiple models + deepspeed, please provide the model used to perform the optimization: `self.backward(loss, model=model)`')
                module = self._strategy.model
            else:
                self._strategy._deepspeed_engine = module
        self._strategy.backward(tensor, module, *args, **kwargs)

    def clip_gradients(self, module: Union[torch.nn.Module, _FabricModule], optimizer: Union[Optimizer, _FabricOptimizer], clip_val: Optional[Union[float, int]]=None, max_norm: Optional[Union[float, int]]=None, norm_type: Union[float, int]=2.0, error_if_nonfinite: bool=True) -> Optional[torch.Tensor]:
        ...
        if clip_val is not None and max_norm is not None:
            raise ValueError('Only one of `clip_val` or `max_norm` can be set as this specifies the underlying clipping algorithm!')
        if clip_val is not None:
            self.strategy.clip_gradients_value(_unwrap_objects(module), _unwrap_objects(optimizer), clip_val=clip_val)
            return None
        if max_norm is not None:
            return self.strategy.clip_gradients_norm(_unwrap_objects(module), _unwrap_objects(optimizer), max_norm=max_norm, norm_type=norm_type, error_if_nonfinite=error_if_nonfinite)
    ...
    def all_reduce(self, data: Union[Tensor, Dict, List, Tuple], group: Optional[Any]=None, reduce_op: Optional[Union[ReduceOp, str]]='mean') -> Union[Tensor, Dict, List, Tuple]:
    ...
    def no_backward_sync(self, module: _FabricModule, enabled: bool=True) -> ContextManager:
        ...
        if not isinstance(module, _FabricModule):
            raise TypeError('You need to set up the model first before you can call `fabric.no_backward_sync()`: `model = fabric.setup(model, ...)`')
        ...
        if self._strategy._backward_sync_control is None:
            rank_zero_warn(f'The `{self._strategy.__class__.__name__}` does not support skipping the gradient synchronization. Remove `.no_backward_sync()` from your code or choose a different strategy.', category=PossibleUserWarning)
            return nullcontext()
    ...
    def init_module(self, empty_init: Optional[bool]=None) -> ContextManager:
        ...
        self._validate_launched()
        if not _TORCH_GREATER_EQUAL_2_0 and self.device.type != 'cpu':
            rank_zero_warn("`Fabric.init_module()` can't place the model parameters on the device directly with PyTorch < 2.0. Parameters will remain on CPU until `Fabric.setup()` is called. Upgrade to PyTorch >= 2.0 to fully utilize this feature.", category=PossibleUserWarning)
        return self._strategy.module_init_context(empty_init=empty_init)

    def save(self, path: Union[str, Path], state: Dict[str, Union[nn.Module, Optimizer, Any]], filter: Optional[Dict[str, Callable[[str, Any], bool]]]=None) -> None:
        ...
        if filter is not None:
            ...
            if not set(filter).issubset(state):
                raise ValueError(f'The filter keys {filter.keys() - state} are not present in the state keys {set(state)}.')
    ...
    def load(self, path: Union[str, Path], state: Optional[Dict[str, Union[nn.Module, Optimizer, Any]]]=None, strict: bool=True) -> Dict[str, Any]:
    ...
    def launch(self, function: Callable[['Fabric'], Any]=_do_nothing, *args: Any, **kwargs: Any) -> Any:
        ...
        """
        if _is_using_cli():
            raise RuntimeError('This script was launched through the CLI, and processes have already been created. Calling  `.launch()` again is not allowed.')
        if function is not _do_nothing:
            if not callable(function):
                raise TypeError(f'`Fabric.launch(...)` needs to be a callable, but got {function}. HINT: do `.launch(your_fn)` instead of `.launch(your_fn())`')
            if not inspect.signature(function).parameters:
                raise TypeError(f'`Fabric.launch(function={function})` needs to take at least one argument. The launcher will pass in the `Fabric` object so you can use it inside the function.')
        elif isinstance(self.strategy.launcher, (_MultiProcessingLauncher, _XLALauncher)):
            raise TypeError(f'To spawn processes with the `{type(self.strategy).__name__}` strategy, `.launch()` needs to be called with a function that contains the code to launch in processes.')
    ...
    def call(self, hook_name: str, *args: Any, **kwargs: Any) -> None:
        ...
        for callback in self._callbacks:
            ...
            if not callable(method):
                rank_zero_warn(f'Skipping the callback `{type(callback).__name__}.{hook_name}` because it is not callable.')
                continue
            method(*args, **kwargs)
    ...
    def _move_model_to_device(self, model: nn.Module, optimizers: List[Optimizer]) -> nn.Module:
        try:
            ...
            if count > 0:
                rank_zero_warn(f'The model passed to `Fabric.setup()` has {count} parameters on different devices (for example {first_name!r} on {first_device} and {initial_name!r} on {initial_device}). Since `move_to_device=True`, all parameters will be moved to the new device. If this is not desired, set `Fabric.setup(..., move_to_device=False)`.', category=PossibleUserWarning)
    ...
    def _requires_distributed_sampler(self, dataloader: DataLoader) -> bool:
        return getattr(self.strategy, 'distributed_sampler_kwargs', None) is not None and (not isinstance(dataloader.sampler, DistributedSampler)) and (not has_iterable_dataset(dataloader))
    ...
    def _prepare_run_method(self) -> None:
        if is_overridden('run', self, Fabric) and _is_using_cli():
            raise TypeError('Overriding `Fabric.run()` and launching from the CLI is not allowed. Run the script normally, or change your code to directly call `fabric = Fabric(...); fabric.setup(...)` etc.')
    ...
    def _validate_launched(self) -> None:
        if not self._launched and (not isinstance(self._strategy, (SingleDeviceStrategy, DataParallelStrategy))):
            raise RuntimeError('To use Fabric with more than one device, you must call `.launch()` or use the CLI: `lightning run model --help`.')
