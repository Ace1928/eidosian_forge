...
class InventoryCLI(CLI):
    ...
    ARGUMENTS = {'host': 'The name of a host to match in the inventory, relevant when using --list', 'group': 'The name of a group in the inventory, relevant when using --graph'}
    ...
    def init_parser(self):
        super(InventoryCLI, self).init_parser(usage='usage: %prog [options] [host|group]', desc='Show Ansible inventory information, by default it uses the inventory script JSON format')
        ...
        action_group.add_argument('--graph', action='store_true', default=False, dest='graph', help='create inventory graph, if supplying pattern it must be a valid group name. It will ignore limit')
        self.parser.add_argument_group(action_group)
        self.parser.add_argument('-y', '--yaml', action='store_true', default=False, dest='yaml', help='Use YAML format instead of default JSON, ignored for --graph')
        self.parser.add_argument('--toml', action='store_true', default=False, dest='toml', help='Use TOML format instead of default JSON, ignored for --graph')
        self.parser.add_argument('--vars', action='store_true', default=False, dest='show_vars', help='Add vars to graph display, ignored unless used with --graph')
        self.parser.add_argument('--export', action='store_true', default=C.INVENTORY_EXPORT, dest='export', help='When doing an --list, represent in a way that is optimized for export,not as an accurate representation of how Ansible has processed it')
        self.parser.add_argument('--output', default=None, dest='output_file', help='When doing --list, send the inventory to a file instead of to the screen')
    ...
    def dump(stuff):
        if context.CLIARGS['yaml']:
            ...
            try:
                ...
                raise AnsibleError('The source inventory contains a value that cannot be represented in TOML: %s' % e)
            except KeyError as e:
                raise AnsibleError('The source inventory contains a non-string key (%s) which cannot be represented in TOML. The specified key will need to be converted to a string. Be aware that if your playbooks expect this key to be non-string, your playbooks will need to be modified to support this change.' % e.args[0])
        else:
            import json
