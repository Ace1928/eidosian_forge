...
class Siesta(FileIOCalculator):
    ...
    """
    allowed_basis_names = ['SZ', 'SZP', 'DZ', 'DZP', 'DZP2', 'TZ', 'TZP', 'TZP2', 'TZP3']
    allowed_spins = ['non-polarized', 'collinear', 'non-collinear', 'spin-orbit']
    allowed_xc = {'LDA': ['PZ', 'CA', 'PW92'], 'GGA': ['PW91', 'PBE', 'revPBE', 'RPBE', 'WC', 'AM05', 'PBEsol', 'PBEJsJrLO', 'PBEGcGxLO', 'PBEGcGxHEG', 'BLYP'], 'VDW': ['DRSLL', 'LMKLL', 'KBM', 'C09', 'BH', 'VV']}
    name = 'siesta'
    command = 'siesta < PREFIX.fdf > PREFIX.out'
    implemented_properties = ['energy', 'forces', 'stress', 'dipole', 'eigenvalues', 'density', 'fermi_energy']
    ...
    def __init__(self, command=None, **kwargs):
        ...
        if commandvar is not None:
            warnings.warn('Please use $ASE_SIESTA_COMMAND and not $SIESTA_COMMAND, which will be ignored in the future.  The new command format will not work with the "<%s > %s" specification.  Use instead e.g. "ASE_SIESTA_COMMAND=siesta < PREFIX.fdf > PREFIX.out", where PREFIX will automatically be replaced by calculator label', np.VisibleDeprecationWarning)
            runfile = self.prefix + '.fdf'
            outfile = self.prefix + '.out'
            try:
                self.command = commandvar % (runfile, outfile)
            except TypeError:
                raise ValueError("The 'SIESTA_COMMAND' environment must " + 'be a format string' + ' with two string arguments.\n' + "Example : 'siesta < %s > %s'.\n" + "Got '%s'" % commandvar)
    ...
    def set(self, **kwargs):
        ...
        for arg in ['mesh_cutoff', 'energy_shift']:
            ...
            if not (isinstance(value, (float, int)) and value > 0):
                mess = "'%s' must be a positive number(in eV),                     got '%s'" % (arg, value)
        ...
        if 'spin' in kwargs:
            if kwargs['spin'] == 'UNPOLARIZED':
                warnings.warn("The keyword 'UNPOLARIZED' is deprecated,and replaced by 'non-polarized'", np.VisibleDeprecationWarning)
    ...
    def calculate(self, atoms=None, properties=['energy'], system_changes=all_changes):
        ...
        """
        FileIOCalculator.calculate(self, atoms=atoms, properties=properties, system_changes=system_changes)
    ...
    def _write_structure(self, fd, atoms):
        ...
        fd.write('\n')
        if cell.rank in [1, 2]:
            raise ValueError('Expected 3D unit cell or no unit cell.  You may wish to add vacuum along some directions.')
        if np.any(cell):
