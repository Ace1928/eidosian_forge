...
def endpoint_control_forticlient_registration_sync(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('endpoint-control', 'forticlient-registration-sync', filtered_data, vdom=vdom)
        current_data = fos.get('endpoint-control', 'forticlient-registration-sync', vdom=vdom, mkey=mkey)
    ...
    if state == 'present' or state is True:
        return fos.set('endpoint-control', 'forticlient-registration-sync', data=filtered_data, vdom=vdom)
    elif state == 'absent':
        return fos.delete('endpoint-control', 'forticlient-registration-sync', mkey=filtered_data['peer-name'], vdom=vdom)
    else:
