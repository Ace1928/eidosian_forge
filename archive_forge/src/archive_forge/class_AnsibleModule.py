    ...
    def tmpdir(self):
        if self._tmpdir is None:
            ...
            if basedir is not None and (not os.path.exists(basedir)):
                try:
                    os.makedirs(basedir, mode=448)
                except (OSError, IOError) as e:
                    self.warn('Unable to use %s as temporary directory, failing back to system: %s' % (basedir, to_native(e)))
                    basedir = None
                else:
                    self.warn('Module remote_tmp %s did not exist and was created with a mode of 0700, this may cause issues when running as another user. To avoid this, create the remote_tmp dir with the correct permissions manually' % basedir)
            ...
            if attrcmd:
                ...
                if not self.check_mode:
                    try:
                        ...
                        self.fail_json(path=to_text(b_path), msg='chattr failed', details=to_native(e), exception=traceback.format_exc())
        return changed
    ...
    def _log_to_syslog(self, msg):
        if HAS_SYSLOG:
            try:
                ...
                self.fail_json(msg='Failed to log to syslog (%s). To proceed anyway, disable syslog logging by setting no_target_syslog to True in your Ansible config.' % to_native(e), exception=traceback.format_exc(), msg_to_log=msg)
