...
class MsgPackSerializer(BinaryBaseSerializer):
    ...
    def __init__(self, compression: Optional[str]=None, compression_level: int | None=None, encoding: str | None=None, serialization_obj: Optional[Type[BaseModel]]=None, serialization_obj_kwargs: Optional[Dict[str, Any]]=None, disable_object_serialization: Optional[bool]=None, **kwargs):
    ...
    def encode_value(self, value: Union[Any, SchemaType], **kwargs) -> bytes:
        ...
        """
        if 'use_bin_type' not in kwargs:
            kwargs['use_bin_type'] = True
        if 'default' not in kwargs:
            kwargs['default'] = self.default_serialization_hook
        try:
    ...
    def decode_value(self, value: bytes, **kwargs) -> Union[SchemaType, Dict, Any]:
        ...
        try:
            if 'raw' not in kwargs:
                kwargs['raw'] = False
            if 'ext_hook' not in kwargs:
                kwargs['ext_hook'] = self.default_deserialization_hook
