    ...
    def containers(self, quiet=False, all=False, trunc=False, latest=False, since=None, before=None, limit=-1, size=False, filters=None):
        ...
        Raises:
            :py:class:`docker.errors.APIError`
                If the server returns an error.
        """
        params = {'limit': 1 if latest else limit, 'all': 1 if all else 0, 'size': 1 if size else 0, 'trunc_cmd': 1 if trunc else 0, 'since': since, 'before': before}
        if filters:
    ...
    def logs(self, container, stdout=True, stderr=True, stream=False, timestamps=False, tail='all', since=None, follow=None, until=None):
        ...
        if since is not None:
            if isinstance(since, datetime):
                ...
                raise errors.InvalidArgument('since value should be datetime or positive int/float, not {}'.format(type(since)))
        if until is not None:
            ...
            if isinstance(until, datetime):
                ...
                raise errors.InvalidArgument('until value should be datetime or positive int/float, not {}'.format(type(until)))
