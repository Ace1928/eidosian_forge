...
def test_create_credential_is_encrypted_when_stored(self):
    ...
    returned_credential = PROVIDERS.credential_api.create_credential(credential_id, credential)
    self.assertEqual(returned_credential['blob'], credential['blob'])
    credential_from_backend = PROVIDERS.credential_api.driver.get_credential(credential_id)
    self.assertNotEqual(credential_from_backend['encrypted_blob'], credential['blob'])