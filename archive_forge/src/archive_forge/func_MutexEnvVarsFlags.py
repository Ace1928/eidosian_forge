...
def MutexEnvVarsFlags():
    ...
    group = MapFlagsNoFile('env-vars', long_name='environment variables', key_type=env_vars_util.EnvVarKeyType, value_type=env_vars_util.EnvVarValueType)
    group.AddArgument(base.Argument('--env-vars-file', metavar='FILE_PATH', type=map_util.ArgDictFile(key_type=env_vars_util.EnvVarKeyType, value_type=env_vars_util.EnvVarValueType), help='Path to a local YAML file with definitions for all environment\n            variables. All existing environment variables will be removed before\n            the new environment variables are added. Example YAML content:\n\n              ```\n              KEY_1: "value1"\n              KEY_2: "value 2"\n              ```\n            '))
    return group