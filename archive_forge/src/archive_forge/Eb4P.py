        ...
        try:
            ...
            for layer_index, layer in enumerate(self.layers[:-1]):
                ...
                try:
                    ...
                    logger.error(
                        f"DimensionSafeLinear transformation failed at layer {layer_index}: {e}",
                        exc_info=True,
                    )
                    raise
            ...
            try:
                x = DimensionSafeLinear(
                    final_layer.in_features, final_layer.out_features
                )(x)
            except Exception as e:
                logger.error(
                    f"DimensionSafeLinear transformation failed at final layer {final_layer_index}: {e}",
                    exc_info=True,
                )
                raise
