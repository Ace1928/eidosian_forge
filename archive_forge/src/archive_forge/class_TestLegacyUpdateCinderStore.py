...
class TestLegacyUpdateCinderStore(functional.SynchronousAPIBase):
    ...
    def setUp(self):
        ...
        self.volume = mock.MagicMock(id=self.vol_id, status='available', size=1, multiattach=False, encrypted=False, delete=mock.MagicMock(), update_all_metadata=mock.MagicMock(), update_readonly_flag=mock.MagicMock())
        self.volume.manager = mock.MagicMock(get=lambda id: self.volume)
        self.cinder_store_mock = mock.MagicMock(attachments=mock.MagicMock(), client=mock.MagicMock(), volumes=mock.MagicMock(get=lambda v_id: mock.MagicMock(volume_type='fast'), create=lambda size_gb, name, metadata, volume_type: self.volume))
        fake_ip = '127.0.0.1'
    ...
    def test_create_image(self, mock_host_addr, mock_mask_pass, mock_wait, mock_open, mock_connector, mock_chown, mocked_cc, mock_base):
    ...
    @mock.patch.object(strutils, 'mask_dict_password')
    @mock.patch.object(socket, 'getaddrinfo')
    def test_migrate_image_after_upgrade(self, mock_host_addr, mock_mask_pass, mock_wait, mock_open, mock_connector, mock_chown, mocked_cc, mock_base):
