...
class LicensesManager(LoggingConfigurable):
    ...
    third_party_licenses_files = List(Unicode(), default_value=[DEFAULT_THIRD_PARTY_LICENSE_FILE, f'static/{DEFAULT_THIRD_PARTY_LICENSE_FILE}'], help='the license report data in built app and federated extensions')
    ...
    async def report_async(self, report_format: str='markdown', bundles_pattern: str='.*', full_text: bool=False) -> tuple[str, str]:
        """Asynchronous wrapper around the potentially slow job of locating
        and encoding all of the licenses
        """
        return await asyncio.wrap_future(self.executor.submit(self.report, report_format=report_format, bundles_pattern=bundles_pattern, full_text=full_text))
    ...
    def report(self, report_format: str, bundles_pattern: str, full_text: bool) -> tuple[str, str]:
        ...
        if report_format == 'markdown':
            return (self.report_markdown(bundles, full_text=full_text), 'text/markdown')
    ...
    def report_csv(self, bundles: dict[str, Any]) -> str:
        ...
        for bundle_name, bundle in bundles.items():
            for package in bundle['packages']:
                writer.writerow({'bundle': bundle_name, **{field: package.get(field, '') for field in fieldnames}})
    ...
    def bundles(self, bundles_pattern: str='.*') -> dict[str, Any]:
        ...
        TODO: schema
        """
        bundles = {name: self.license_bundle(Path(ext['ext_path']), name) for name, ext in self.federated_extensions.items() if re.match(bundles_pattern, name)}
