...
class KubeVirtNodeDriver(KubernetesDriverMixin, NodeDriver):
    ...
    def start_node(self, node):
        ...
        try:
            result = self.connection.request(req, method='PATCH', data=json.dumps(data), headers=headers)
    ...
    def reboot_node(self, node):
        ...
        method = 'DELETE'
        try:
            result = self.connection.request(KUBEVIRT_URL + 'namespaces/' + namespace + '/virtualmachineinstances/' + name, method=method)
            return result.status in VALID_RESPONSE_CODES
        except Exception:
            raise
        return
    ...
    def destroy_node(self, node):
        ...
        self.stop_node(node)
        try:
            result = self.connection.request(KUBEVIRT_URL + 'namespaces/' + namespace + '/virtualmachines/' + name, method='DELETE')
            return result.status in VALID_RESPONSE_CODES
        except Exception:
            raise

    def create_node(self, name, image, location=None, ex_memory=128, ex_cpu=1, ex_disks=None, ex_network=None, ex_termination_grace_period=0, ports=None):
        """
        ...
        """
        DISK_TYPES = {'containerDisk', 'ephemeral', 'configMap', 'dataVolume', 'cloudInitNoCloud', 'persistentVolumeClaim', 'emptyDisk', 'cloudInitConfigDrive', 'hostDisk'}
        if location is not None:
            namespace = location.name
        else:
            namespace = 'default'
        vm = {'apiVersion': 'kubevirt.io/v1alpha3', 'kind': 'VirtualMachine', 'metadata': {'labels': {'kubevirt.io/vm': name}, 'name': name}, 'spec': {'running': False, 'template': {'metadata': {'labels': {'kubevirt.io/vm': name}}, 'spec': {'domain': {'devices': {'disks': [], 'interfaces': [], 'networkInterfaceMultiqueue': False}, 'machine': {'type': ''}, 'resources': {'requests': {}, 'limits': {}}}, 'networks': [], 'terminationGracePeriodSeconds': ex_termination_grace_period, 'volumes': []}}}}
                ...
                if 'claim_name' in disk:
                    ...
                    if claimName not in self.ex_list_persistent_volume_claims(namespace=namespace):
                        if 'size' not in disk or 'storage_class_name' not in disk:
                            msg = "disk['size'] and disk['storage_class_name'] are both required to create a new claim."
                            raise KeyError(msg)
                        size = disk['size']
                        ...
                        self.create_volume(size=size, name=claimName, location=location, ex_storage_class_name=storage_class, ex_volume_mode=volume_mode, ex_access_mode=access_mode)
                else:
                    msg = 'You must provide either a claim_name of an existing claim or if you want one to be created you must additionally provide size and the storage_class_name of the cluster, which allows dynamic provisioning, so a Persistent Volume Claim can be created. In the latter case please provide the desired size as well.'
                    raise KeyError(msg)
                volumes_dict = {'persistentVolumeClaim': {'claimName': claimName}, 'name': disk_name}
    ...
    def list_locations(self):
        ...
        for item in result['items']:
            ...
            namespaces.append(NodeLocation(id=ID, name=name, country='', driver=self.connection.driver))
        return namespaces
    ...
    def create_volume(self, size, name, location=None, ex_storage_class_name='', ex_volume_mode='Filesystem', ex_access_mode='ReadWriteOnce', ex_dynamic=True, ex_reclaim_policy='Recycle', ex_volume_type=None, ex_volume_params=None):
        """
        ...
        if ex_dynamic:
            ...
            vol = self._create_volume_dynamic(size=size, name=name, storage_class_name=ex_storage_class_name, namespace=location.name, volume_mode=ex_volume_mode, access_mode=ex_access_mode)
            return vol
        elif ex_volume_type is None or ex_volume_params is None:
            msg = 'An ex_volume_type must be provided from the list of supported clouds, as well as the ex_volume_params necessary for your volume type choice.'
            raise ValueError(msg)
        pv = {'apiVersion': 'v1', 'kind': 'PersistentVolume', 'metadata': {'name': name}, 'spec': {'capacity': {'storage': str(size) + 'Gi'}, 'volumeMode': ex_volume_mode, 'accessModes': [ex_access_mode], 'persistentVolumeReclaimPolicy': ex_reclaim_policy, 'storageClassName': ex_storage_class_name, 'mountOptions': [], ex_volume_type: ex_volume_params}}
    ...
    def attach_volume(self, node, volume, device='disk', ex_bus='virtio', ex_name=None):
        ...
        if not volume.extra['is_bound']:
            ...
            if volume is None:
                raise LibcloudError('Selected Volume (PV) could not be bound (to a PVC), please select another volume', driver=self)
        ...
        headers = {'Content-Type': 'application/merge-patch+json'}
        data = {'spec': {'template': {'spec': {'volumes': volumes, 'domain': {'devices': {'disks': disks}}}}}}
        try:
            result = self.connection.request(req, method='PATCH', data=json.dumps(data), headers=headers)
    ...
    def list_volumes(self):
        ...
        for item in result['items']:
            ...
            if extra['is_bound']:
                ...
                name = item['metadata']['name']
            volume = StorageVolume(id=ID, name=name, size=size, driver=self.connection.driver, extra=extra)
    ...
    def _to_node(self, vm, is_stopped=False):
        ...
        if 'limits' in vm['spec']['template']['spec']['domain']['resources']:
            ...
            if vm['spec']['template']['spec']['domain']['resources']['requests'].get('memory', None):
        ...
        elif vm['spec']['template']['spec']['domain']['resources'].get('requests', None) and vm['spec']['template']['spec']['domain']['resources']['requests'].get('cpu', None):
        ...
        for service in services:
            ...
            for port_pair in service['spec']['ports']:
                ...
                port_forwards.append({'local_port': local_port, 'public_port': public_port, 'protocol': protocol, 'service_type': service_type})
        extra['port_forwards'] = port_forwards
        if is_stopped:
            state = NodeState.STOPPED
            public_ips = None
            private_ips = None
            return Node(id=ID, name=name, state=state, public_ips=public_ips, private_ips=private_ips, driver=driver, size=size, image=image, extra=extra)
        ...
        for cont_status in pod['status']['containerStatuses']:
            if cont_status['name'] != 'compute':
                ...
                state = NodeState.RUNNING if 'running' in cont_status['state'] else NodeState.PENDING
        public_ips = None
        ...
        if 'podIPs' in pod['status']:
            ...
            private_ips = []
        return Node(id=ID, name=name, state=state, public_ips=public_ips, private_ips=private_ips, driver=driver, size=size, image=image, extra=extra, created_at=created_at)
    ...
    def ex_create_service(self, node, ports, service_type='NodePort', cluster_ip=None, load_balancer_ip=None, override_existing_ports=False):
        """
        ...
        for port_group in ports:
            ...
            ports_to_expose.append({'protocol': port_group.get('protocol', 'TCP'), 'port': int(port_group['port']), 'targetPort': int(port_group['target_port']), 'name': port_group['name']})
        headers = None
        data = None
        if len(service_list) > 0:
            if not ports:
                ...
                for service in service_list:
                    service_name = service['metadata']['name']
                    result = result and self.ex_delete_service(namespace=namespace, service_name=service_name)
                return result
            else:
                method = 'PATCH'
            ...
            if not ports:
                raise ValueError('Argument ports is empty but there is no service of {} type to be deleted'.format(service_type))
            method = 'POST'
            service = {'kind': 'Service', 'apiVersion': 'v1', 'metadata': {'name': service_name, 'labels': {'service': 'kubevirt.io'}}, 'spec': {'type': '', 'selector': {'kubevirt.io/vm': node.name}, 'ports': []}}
