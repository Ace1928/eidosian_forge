...
def firewall_profile_protocol_options(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('firewall', 'profile-protocol-options', filtered_data, vdom=vdom)
        current_data = fos.get('firewall', 'profile-protocol-options', vdom=vdom, mkey=mkey)
    ...
    if state == 'present' or state is True:
        return fos.set('firewall', 'profile-protocol-options', data=filtered_data, vdom=vdom)
    elif state == 'absent':
        return fos.delete('firewall', 'profile-protocol-options', mkey=filtered_data['name'], vdom=vdom)
    else:
