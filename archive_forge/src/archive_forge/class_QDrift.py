...
class QDrift(Operation):
    ...
    **Example**
    ...
    def __init__(self, hamiltonian, time, n=1, seed=None, decomposition=None, id=None):
        ...
        if len(ops) < 2:
            raise ValueError('There should be atleast 2 terms in the Hamiltonian. Otherwise use `qml.exp`')
        if any((requires_grad(coeff) for coeff in coeffs)):
            raise qml.QuantumFunctionError("The QDrift template currently doesn't support differentiation through the coefficients of the input Hamiltonian.")
        if decomposition is None:
            unwrapped_coeffs = unwrap(coeffs)
            decomposition = _sample_decomposition(unwrapped_coeffs, ops, time, n=n, seed=seed)
        self._hyperparameters = {'n': n, 'seed': seed, 'base': hamiltonian, 'decomposition': decomposition}
    ...
    def error(hamiltonian, time, n=1):
        ...
        if isinstance(hamiltonian, Hamiltonian):
            ...
            lmbda = qml.math.sum((qml.math.abs(op.scalar) if isinstance(op, SProd) else 1.0 for op in hamiltonian))
        else:
            raise TypeError(f'The given operator must be a PennyLane ~.Hamiltonian or ~.Sum got {hamiltonian}')
