...
def plot_trace(data, var_names, divergences, kind, figsize, rug, lines, circ_var_names, circ_var_units, compact, compact_prop, combined, chain_prop, legend, labeller, plot_kwargs, fill_kwargs, rug_kwargs, hist_kwargs, trace_kwargs, rank_kwargs, plotters, divergence_data, axes, backend_kwargs, backend_config, show):
    """Bokeh traceplot."""
    if divergences is not False:
        assert divergence_data is not None
    if backend_config is None:
        backend_config = {}
    backend_config = {**backend_kwarg_defaults(('bounds_y_range', 'plot.bokeh.bounds_y_range')), **backend_config}
    ...
    if not compact:
        chain_prop = {'line_color': plt.rcParams['axes.prop_cycle'].by_key()['color']} if chain_prop is None else chain_prop
    else:
        chain_prop = {'line_dash': ('solid', 'dotted', 'dashed', 'dashdot')} if chain_prop is None else chain_prop
        compact_prop = {'line_color': plt.rcParams['axes.prop_cycle'].by_key()['color']} if compact_prop is None else compact_prop
    ...
    if isinstance(chain_prop, tuple):
        warnings.warn('chain_prop as a tuple will be deprecated in a future warning, use a dict instead', FutureWarning)
        chain_prop = {chain_prop[0]: chain_prop[1]}
    chain_prop = {prop_name: [prop for _, prop in zip(range(num_chain_props), cycle(props))] for prop_name, props in chain_prop.items()}
    ...
    if isinstance(compact_prop, tuple):
        warnings.warn('compact_prop as a tuple will be deprecated in a future warning, use a dict instead', FutureWarning)
    ...
    cds_var_groups = {}
    draw_name = 'draw'
    for var_name, selection, isel, value in list(xarray_var_iter(data, var_names=var_names, combined=True)):
        if selection:
            cds_name = '{}_ARVIZ_CDS_SELECTION_{}'.format(var_name, '_'.join((str(item) for key, value in selection.items() for item in ([key, value] if isinstance(value, str) or not isinstance(value, Iterable) else [key, *value]))))
        else:
            cds_name = var_name
    ...
    for idx, (var_name, selection, isel, value) in enumerate(plotters):
        ...
        if len(value.shape) == 2:
            y_name = var_name if not selection else '{}_ARVIZ_CDS_SELECTION_{}'.format(var_name, '_'.join((str(item) for key, value in selection.items() for item in ((key, value) if isinstance(value, str) or not isinstance(value, Iterable) else (key, *value)))))
            if rug:
                rug_kwargs['y'] = y_name
            _plot_chains_bokeh(ax_density=axes[idx, 0], ax_trace=axes[idx, 1], data=cds_data, x_name=draw_name, y_name=y_name, chain_prop=chain_prop, combined=combined, rug=rug, kind=kind, legend=legend, trace_kwargs=trace_kwargs, hist_kwargs=hist_kwargs, plot_kwargs=plot_kwargs, fill_kwargs=fill_kwargs, rug_kwargs=rug_kwargs, rank_kwargs=rank_kwargs)
        else:
        ...
        for col in (0, 1):
            ...
            axes[idx, col].y_range = DataRange1d(bounds=backend_config['bounds_y_range'], min_interval=0.1)
        for _, _, vlines in (j for j in lines if j[0] == var_name and j[1] == selection):
            ...
            for line_value in line_values:
                vline = Span(location=line_value, dimension='height', line_color='black', line_width=1.5, line_alpha=0.75)
                hline = Span(location=line_value, dimension='width', line_color='black', line_width=1.5, line_alpha=trace_kwargs['alpha'])
        ...
        if divergences:
            ...
            div_density_kwargs.setdefault('line_alpha', 0.5)
            ...
            div_trace_kwargs.setdefault('line_alpha', 0.5)
