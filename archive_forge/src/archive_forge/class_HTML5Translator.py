...
class HTML5Translator(SphinxTranslator, BaseTranslator):
    ...
    def depart_title(self, node: Element) -> None:
        ...
        if self.config.html_permalinks and self.builder.add_permalinks and node.parent.hasattr('ids') and node.parent['ids']:
            if close_tag.startswith('</h'):
                ...
                self.body.append('</a><a class="headerlink" href="#%s" ' % node.parent['ids'][0] + 'title="%s">%s' % (_('Permalink to this heading'), self.config.html_permalinks_icon))
    ...
    def _fieldlist_row_index(self) -> int:
        warnings.warn('_fieldlist_row_index is deprecated', RemovedInSphinx60Warning, stacklevel=2)
