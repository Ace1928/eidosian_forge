...
def test_structured_clf_fit_call_auto_model_fit(fit, tmp_path):
    ...
    auto_model.fit(x=pd.read_csv(test_utils.TRAIN_CSV_PATH).to_numpy().astype(np.unicode)[:100], y=test_utils.generate_one_hot_labels(num_instances=100, num_classes=3))
