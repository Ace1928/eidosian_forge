...
class HalfCosine(Filter):
    ...
    def __init__(self, G, Nf=6, **kwargs):
        ...
        dila_fact = G.lmax * (3.0 / (Nf - 2))
        main_window = lambda x: np.multiply(np.multiply(0.5 + 0.5 * np.cos(2.0 * np.pi * (x / dila_fact - 1.0 / 2)), x >= 0), x <= dila_fact)
        g = []
        for i in range(Nf):
            g.append(lambda x, ind=i: main_window(x - dila_fact / 3.0 * (ind - 2)))
