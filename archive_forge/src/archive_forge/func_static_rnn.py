    ...
    with vs.variable_scope(scope or 'rnn') as varscope:
        ...
        if first_input.get_shape().rank != 1:
            ...
            for flat_input in flat_inputs:
                ...
                batch_size, input_size = (tensor_shape.dimension_at_index(input_shape, 0), input_shape[1:])
                ...
                for i, size in enumerate(input_size.dims):
                    if tensor_shape.dimension_value(size) is None:
                        raise ValueError(f'Input size (dimension {i} of input {flat_input}) must be accessible via shape inference, but saw value None.')
        else:
        ...
        if initial_state is not None:
            ...
            if not dtype:
                raise ValueError('If no initial_state is provided, argument `dtype` must be specified')
            if getattr(cell, 'get_initial_state', None) is not None:
                state = cell.get_initial_state(inputs=None, batch_size=batch_size, dtype=dtype)
            else:
                state = cell.zero_state(batch_size, dtype)
        if sequence_length is not None:
            sequence_length = ops.convert_to_tensor(sequence_length, name='sequence_length')
            if sequence_length.get_shape().rank not in (None, 1):
                raise ValueError(f'Argument `sequence_length` must be a vector of length {batch_size}. Received sequence_length={sequence_length}.')

            def _create_zero_output(output_size):
                size = _concat(batch_size, output_size)
                output = array_ops.zeros(array_ops_stack.stack(size), _infer_state_dtype(dtype, state))
                shape = _concat(tensor_shape.dimension_value(fixed_batch_size), output_size, static=True)
                output.set_shape(tensor_shape.TensorShape(shape))
                return output
            output_size = cell.output_size
            flat_output_size = nest.flatten(output_size)
            flat_zero_output = tuple((_create_zero_output(size) for size in flat_output_size))
            zero_output = nest.pack_sequence_as(structure=output_size, flat_sequence=flat_zero_output)
