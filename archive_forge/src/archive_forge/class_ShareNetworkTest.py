...
class ShareNetworkTest(base.BaseSharedFileSystemTest):
    ...
    def setUp(self):
        ...
        subnet = self.user_cloud.network.create_subnet(name=self.SUBNET_NAME, network_id=self.NETWORK_ID, ip_version=4, cidr='10.0.0.0/24')
        self.SUBNET_ID = subnet.id
        self.SHARE_NETWORK_NAME = self.getUniqueString()
        snt = self.user_cloud.shared_file_system.create_share_network(name=self.SHARE_NETWORK_NAME, neutron_net_id=self.NETWORK_ID, neutron_subnet_id=self.SUBNET_ID)
    ...
    def test_get(self):
        ...
        self.assertEqual(self.NETWORK_ID, sot.share_network_subnets[0]['neutron_net_id'])
        self.assertEqual(self.SUBNET_ID, sot.share_network_subnets[0]['neutron_subnet_id'])
    ...
    def test_update(self):
        unt = self.user_cloud.shared_file_system.update_share_network(self.SHARE_NETWORK_ID, description='updated share network')
