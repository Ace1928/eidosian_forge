...
class TestClusterProxy(test_proxy_base.TestProxyBase):
    ...
    def test_build_info_get(self):
        self.verify_get(self.proxy.get_build_info, build_info.BuildInfo, method_args=[], expected_kwargs={'requires_id': False})
    ...
    def test_profiles(self):
        self.verify_list(self.proxy.profiles, profile.Profile, method_kwargs={'limit': 2}, expected_kwargs={'limit': 2})
    ...
    def test_cluster_force_delete(self):
        self._verify('openstack.clustering.v1.cluster.Cluster.force_delete', self.proxy.delete_cluster, method_args=['value', False, True], expected_args=[self.proxy])
    ...
    def test_clusters(self):
        self.verify_list(self.proxy.clusters, cluster.Cluster, method_kwargs={'limit': 2}, expected_kwargs={'limit': 2})
    ...
    def test_collect_cluster_attrs(self):
        self.verify_list(self.proxy.collect_cluster_attrs, cluster_attr.ClusterAttr, method_args=['FAKE_ID', 'path.to.attr'], expected_args=[], expected_kwargs={'cluster_id': 'FAKE_ID', 'path': 'path.to.attr'})

    @mock.patch.object(proxy_base.Proxy, '_get_resource')
    def test_cluster_check(self, mock_get):
        mock_cluster = cluster.Cluster.new(id='FAKE_CLUSTER')
        mock_get.return_value = mock_cluster
        self._verify('openstack.clustering.v1.cluster.Cluster.check', self.proxy.check_cluster, method_args=['FAKE_CLUSTER'], expected_args=[self.proxy])
    ...
    def test_node_force_delete(self):
        self._verify('openstack.clustering.v1.node.Node.force_delete', self.proxy.delete_node, method_args=['value', False, True], expected_args=[self.proxy])
    ...
    def test_node_get_with_details(self):
        self._verify('openstack.proxy.Proxy._get', self.proxy.get_node, method_args=['NODE_ID'], method_kwargs={'details': True}, expected_args=[node.NodeDetail], expected_kwargs={'node_id': 'NODE_ID', 'requires_id': False})

    def test_nodes(self):
        self.verify_list(self.proxy.nodes, node.Node, method_kwargs={'limit': 2}, expected_kwargs={'limit': 2})
    ...
    def test_node_check(self, mock_get):
        ...
        self._verify('openstack.clustering.v1.node.Node.check', self.proxy.check_node, method_args=['FAKE_NODE'], expected_args=[self.proxy])
    ...
    def test_node_adopt_preview(self, mock_get):
        mock_node = node.Node.new()
        mock_get.return_value = mock_node
        self._verify('openstack.clustering.v1.node.Node.adopt', self.proxy.adopt_node, method_kwargs={'preview': True, 'foo': 'bar'}, expected_args=[self.proxy], expected_kwargs={'preview': True, 'foo': 'bar'})
        mock_get.assert_called_once_with(node.Node, None)
    ...
    def test_policies(self):
        self.verify_list(self.proxy.policies, policy.Policy, method_kwargs={'limit': 2}, expected_kwargs={'limit': 2})

    def test_policy_update(self):
        self.verify_update(self.proxy.update_policy, policy.Policy)

    def test_cluster_policies(self):
        self.verify_list(self.proxy.cluster_policies, cluster_policy.ClusterPolicy, method_args=['FAKE_CLUSTER'], expected_args=[], expected_kwargs={'cluster_id': 'FAKE_CLUSTER'})
    ...
    def test_receivers(self):
        self.verify_list(self.proxy.receivers, receiver.Receiver, method_kwargs={'limit': 2}, expected_kwargs={'limit': 2})

    def test_action_get(self):
        self.verify_get(self.proxy.get_action, action.Action)

    def test_actions(self):
        self.verify_list(self.proxy.actions, action.Action, method_kwargs={'limit': 2}, expected_kwargs={'limit': 2})
    ...
    def test_events(self):
        self.verify_list(self.proxy.events, event.Event, method_kwargs={'limit': 2}, expected_kwargs={'limit': 2})
    ...
    def test_wait_for(self, mock_wait):
        ...
        mock_wait.assert_called_once_with(self.proxy, mock_resource, 'ACTIVE', [], 2, 120)
    ...
    def test_wait_for_params(self, mock_wait):
        ...
        mock_wait.assert_called_once_with(self.proxy, mock_resource, 'ACTIVE', ['ERROR'], 1, 2)
    ...
    def test_get_cluster_metadata(self):
        self._verify('openstack.clustering.v1.cluster.Cluster.fetch_metadata', self.proxy.get_cluster_metadata, method_args=['value'], expected_args=[self.proxy], expected_result=cluster.Cluster(id='value', metadata={}))
