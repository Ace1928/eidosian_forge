...
class RegexFullMatch(OpRun):
    ...
    def _run(self, x, pattern=None):
        try:
            ...
            raise ImportError('re2 must be installed to use the reference implementation of the RegexFullMatch operator') from e
        ...
        fullmatch_func = np.vectorize(lambda x: regex.fullmatch(x) is not None, otypes=[np.bool_])
