...
class block(IBlock):
    ...
    def __setattr__(self, name, obj):
        if name in self._block_reserved_words:
            raise ValueError('Attempting to modify a reserved block attribute: %s' % (name,))
        needs_del = False
        same_obj = False
        ...
        if name in self_order:
            ...
            if self_order[name] is not obj:
                logger.warning('Implicitly replacing attribute %s (type=%s) on block with new object (type=%s). This is usually indicative of a modeling error. To avoid this warning, delete the original object from the block before assigning a new object.' % (name, getattr(self, name).__class__.__name__, obj.__class__.__name__))
            else:
                same_obj = True
        ...
        try:
            ...
            if obj.parent is None or same_obj:
                ...
                elif ctype is not self_byctype:
                    if len(self_order) > self._lbs_count:
                        self._activate_large_storage_mode()
                    else:
                        self.__dict__['_block__byctype'] = hash(self_byctype) | hash(ctype)
            else:
                raise ValueError("Invalid assignment to %s type with name '%s' at entry %s. A parent container has already been assigned to the object being inserted: %s" % (self.__class__.__name__, self.name, name, obj.parent.name))
    ...
    def write(self, filename, format=None, _solver_capability=None, _called_by_solver=False, **kwds):
        """
        ...
        if problem_writer is None:
            raise ValueError("Cannot write model in format '%s': no model writer registered for that format" % str(format))
    ...
    def load_solution(self, solution, allow_consistent_values_for_fixed_vars=False, comparison_tolerance_for_fixed_vars=1e-05):
        """
        Load a solution.
        ...
        valid_import_suffixes = {obj.storage_key: obj for obj in import_suffix_generator(self)}
        ...
        for label, entry in solution.variable.items():
            ...
            if var is None or var is SymbolMap.UnknownSymbol:
                if 'ONE_VAR_CONST' in label:
                    ...
                    raise KeyError("Variable associated with symbol '%s' is not found on this block" % label)
            seen_var_ids.add(id(var))
            if not allow_consistent_values_for_fixed_vars and var.fixed:
                raise ValueError("Variable '%s' is currently fixed. A new value is not expected in solution" % var.name)
            for _attr_key, attr_value in entry.items():
                attr_key = _attr_key[0].lower() + _attr_key[1:]
                if attr_key == 'value':
                    if allow_consistent_values_for_fixed_vars and var.fixed and (math.fabs(attr_value - var.value) > comparison_tolerance_for_fixed_vars):
                        raise ValueError("Variable %s is currently fixed. A value of '%s' in solution is not within tolerance=%s of the current value of '%s'" % (var.name, attr_value, comparison_tolerance_for_fixed_vars, var.value))
        ...
        for label, entry in solution.objective.items():
            ...
            if obj is None or obj is SymbolMap.UnknownSymbol:
                raise KeyError("Objective associated with symbol '%s' is not found on this block" % label)
        ...
        for label, entry in solution.constraint.items():
            ...
            if con is SymbolMap.UnknownSymbol:
                if 'ONE_VAR_CONST' in label:
                    ...
                    raise KeyError("Constraint associated with symbol '%s' is not found on this block" % label)
        ...
        if default_variable_value is not None:
            for var_id in unseen_var_ids:
                ...
                if not allow_consistent_values_for_fixed_vars and var.fixed:
                    raise ValueError("Variable '%s' is currently fixed. A new value is not expected in solution" % var.name)
                if allow_consistent_values_for_fixed_vars and var.fixed and (math.fabs(default_variable_value - var.value) > comparison_tolerance_for_fixed_vars):
                    raise ValueError("Variable %s is currently fixed. A value of '%s' in solution is not within tolerance=%s of the current value of '%s'" % (var.name, default_variable_value, comparison_tolerance_for_fixed_vars, var.value))
