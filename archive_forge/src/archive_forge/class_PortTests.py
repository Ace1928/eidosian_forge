...
class PortTests(common.NetworkTagTests):
    ...
    def tearDownClass(cls):
        try:
            if cls.haz_network:
                raw_output = cls.openstack('network delete %s' % cls.NETWORK_NAME)
    ...
    def test_port_delete(self):
        ...
        json_output = self.openstack('port create --network %s %sx' % (self.NETWORK_NAME, self.NAME), parse_output=True)
    ...
    def test_port_list(self):
        ...
        self.assertEqual(self.NAME + 'x', json_output.get('name'))
        json_output = self.openstack('port list', parse_output=True)
        item_map = {item.get('ID'): item.get('MAC Address') for item in json_output}
        ...
        json_output = self.openstack('port list --long', parse_output=True)
        id_list = [item.get('ID') for item in json_output]
        self.assertIn(id1, id_list)
        self.assertIn(id2, id_list)
        json_output = self.openstack('port list --mac-address %s' % mac2, parse_output=True)
        item_map = {item.get('ID'): item.get('MAC Address') for item in json_output}
        self.assertNotIn(id1, item_map.keys())
        self.assertIn(id2, item_map.keys())
        self.assertNotIn(mac1, item_map.values())
        self.assertIn(mac2, item_map.values())
        json_output = self.openstack('port list -c ID -c Name -c device_id', parse_output=True)
    ...
    def _create_resource_for_tag_test(self, name, args):
        return self.openstack('{} create --network {} {} {}'.format(self.base_command, self.NETWORK_NAME, args, name), parse_output=True)