...
class TestMPSOrdering(unittest.TestCase):
    ...
    def _check_baseline(self, model, **kwds):
        ...
        model.write(test_fname, format='mps', io_options=io_options, int_marker=int_marker)
        self.assertTrue(cmp(test_fname, baseline_fname), msg='Files %s and %s differ' % (test_fname, baseline_fname))
        self._cleanup(test_fname)
