...
def test_create_config_exceeds_max_per_tenant(self):
    ...
    ex = self.assertRaises(dispatcher.ExpectedException, self._create_software_config)
    self.assertEqual(exception.RequestLimitExceeded, ex.exc_info[0])
    self.assertIn('You have reached the maximum software configs per tenant', str(ex.exc_info[1]))