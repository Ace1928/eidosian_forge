...
class PromoteReplica(base.Command):
    ...
    """
        ...
        parser.add_argument('replica', completer=flags.InstanceCompleter, help='Cloud SQL read replica ID.')
    ...
    """
        ...
        if instances.InstancesV1Beta4.IsMysqlDatabaseVersion(instance_resource.databaseVersion):
            database_type_fragment = 'mysql'
        elif instances.InstancesV1Beta4.IsPostgresDatabaseVersion(instance_resource.databaseVersion):
            database_type_fragment = 'postgres'
        else:
            database_type_fragment = None
        promote_replica_docs_link = None
        if database_type_fragment:
            promote_replica_docs_link = 'Learn more:\n' + 'https://cloud.google.com/sql/docs/{}/replication/manage-replicas#promote-replica\n\n'.format(database_type_fragment)
        sys.stderr.write(textwrap.TextWrapper().fill("Promoting a read replica stops replication and converts the instance to a standalone primary instance with read and write capabilities. This can't be undone. To avoid loss of data, before promoting the replica, you should verify that the replica has applied all transactions received from the primary.") + '\n\n')
        ...
        result = sql_client.instances.PromoteReplica(sql_messages.SqlInstancesPromoteReplicaRequest(project=instance_ref.project, instance=instance_ref.instance, failover=args.failover))
        ...
        operations.OperationsV1Beta4.WaitForOperation(sql_client, operation_ref, 'Promoting Cloud SQL replica')
