...
class NetAppOntapCifsLocalUserModify:
    ...
    def __init__(self):
        ...
        self.argument_spec.update(dict(name=dict(required=True, type='str'), vserver=dict(required=True, type='str'), is_account_disabled=dict(required=False, type='bool'), full_name=dict(required=False, type='str'), description=dict(required=False, type='str')))
    ...
    def get_cifs_local_user(self):
        ...
        if self.use_rest:
            ...
            query = {'fields': 'user-name,full-name,is-account-disabled,description', 'user-name': self.parameters['name'], 'vserver': self.parameters['vserver']}
            record, error = rest_generic.get_one_record(self.rest_api, api, query=query)
            if error:
                self.module.fail_json(msg=error)
            if record:
                return_value = {'name': record['user_name'], 'is_account_disabled': record['is_account_disabled'], 'vserver': record['vserver'], 'description': record.get('description', ''), 'full_name': record.get('full_name', '')}
        else:
            ...
            try:
                ...
                self.module.fail_json(msg='Error getting user %s on vserver %s: %s' % (self.parameters['name'], self.parameters['vserver'], to_native(error)), exception=traceback.format_exc())
            if result.get_child_by_name('attributes-list'):
                local_cifs_user_attributes = result['attributes-list']['cifs-local-user']
                return_value = {'name': local_cifs_user_attributes['user-name'], 'is_account_disabled': self.na_helper.get_value_for_bool(from_zapi=True, value=local_cifs_user_attributes['is-account-disabled']), 'vserver': local_cifs_user_attributes['vserver'], 'full_name': '', 'description': ''}
    ...
    def modify_cifs_local_user(self, modify):
        ...
        if self.use_rest:
            api = 'private/cli/vserver/cifs/users-and-groups/local-user'
            query = {'user-name': self.parameters['name'], 'vserver': self.parameters['vserver']}
            ...
            cifs_local_user_obj.add_new_child('is-account-disabled', self.na_helper.get_value_for_bool(from_zapi=False, value=self.parameters['is_account_disabled']))
            ...
            try:
                ...
                self.module.fail_json(msg='Error modifying local CIFS user %s on vserver %s: %s' % (self.parameters['name'], self.parameters['vserver'], to_native(error)), exception=traceback.format_exc())
