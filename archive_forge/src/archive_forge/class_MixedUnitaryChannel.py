...
class MixedUnitaryChannel(raw_types.Gate):
    ...
    def __init__(self, mixture: Iterable[Tuple[float, np.ndarray]], key: Union[str, 'cirq.MeasurementKey', None]=None, validate: bool=False):
        ...
        if not num_qubits.is_integer() or m0.shape[1] != m0.shape[0]:
            raise ValueError(f'Input mixture of shape {m0.shape} does not represent a square operator over qubits.')
        self._num_qubits = int(num_qubits)
        for i, op in enumerate((p[1] for p in mixture)):
            if not op.shape == m0.shape:
                raise ValueError(f'Inconsistent unitary shapes: op[0]: {m0.shape}, op[{i}]: {op.shape}')
    ...
    def __eq__(self, other) -> bool:
        ...
        if not np.allclose([m[0] for m in self._mixture], [m[0] for m in other._mixture]):
            return False
        return np.allclose(np.asarray([m[1] for m in self._mixture]), np.asarray([m[1] for m in other._mixture]))
    ...
    def _with_key_path_prefix_(self, prefix: Tuple[str, ...]):
        return MixedUnitaryChannel(mixture=self._mixture, key=protocols.with_key_path_prefix(self._key, prefix))

    def _with_rescoped_keys_(self, path: Tuple[str, ...], bindable_keys: FrozenSet['cirq.MeasurementKey']):
        return MixedUnitaryChannel(mixture=self._mixture, key=protocols.with_rescoped_keys(self._key, path, bindable_keys))
    ...
    def __repr__(self):
        unitary_tuples = ['(' + repr(op[0]) + ', ' + proper_repr(op[1]) + ')' for op in self._mixture]
