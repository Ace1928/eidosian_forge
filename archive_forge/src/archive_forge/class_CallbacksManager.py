...
class CallbacksManager(object):
    ...
    def subscribe(self, callback, resource, event, priority=priority_group.PRIORITY_DEFAULT, cancellable=False):
        ...
        """
        LOG.debug('Subscribe: %(callback)s %(resource)s %(event)s %(priority)d, %(cancellable)s', {'callback': callback, 'resource': resource, 'event': event, 'priority': priority, 'cancellable': cancellable})
        ...
        for pri_callbacks in pri_callbacks_list:
            ...
            pri_callbacks = {}
            pri_callbacks_list.append(PriorityCallbacks(priority, pri_callbacks, cancellable))
    ...
    def unsubscribe(self, callback, resource, event):
        ...
        """
        LOG.debug('Unsubscribe: %(callback)s %(resource)s %(event)s', {'callback': callback, 'resource': resource, 'event': event})
    ...
    def publish(self, resource, event, trigger, payload=None):
        ...
        if payload:
            if not isinstance(payload, events.EventPayload):
                raise exceptions.Invalid(element='event payload', value=type(payload))
        ...
        if errors:
            ...
            if event.startswith(events.PRECOMMIT) or any((error.is_cancellable for error in errors)):
                raise exceptions.CallbackFailure(errors=errors)
