...
class MLXPipeline(LLM):
    ...
    @classmethod
    def from_model_id(cls, model_id: str, tokenizer_config: Optional[dict]=None, adapter_file: Optional[str]=None, lazy: bool=False, pipeline_kwargs: Optional[dict]=None, **kwargs: Any) -> MLXPipeline:
        """Construct the pipeline object from model_id and task."""
        try:
            from mlx_lm import load
        except ImportError:
            raise ValueError('Could not import mlx_lm python package. Please install it with `pip install mlx_lm`.')
        ...
        return cls(model_id=model_id, model=model, tokenizer=tokenizer, tokenizer_config=tokenizer_config, adapter_file=adapter_file, lazy=lazy, pipeline_kwargs=_pipeline_kwargs, **kwargs)

    @property
    def _identifying_params(self) -> Mapping[str, Any]:
        """Get the identifying parameters."""
        return {'model_id': self.model_id, 'tokenizer_config': self.tokenizer_config, 'adapter_file': self.adapter_file, 'lazy': self.lazy, 'pipeline_kwargs': self.pipeline_kwargs}
    ...
    def _stream(self, prompt: str, stop: Optional[List[str]]=None, run_manager: Optional[CallbackManagerForLLMRun]=None, **kwargs: Any) -> Iterator[GenerationChunk]:
        ...
        for (token, prob), n in zip(generate_step(prompt_tokens, self.model, temp, repetition_penalty, repetition_context_size), range(max_new_tokens)):
