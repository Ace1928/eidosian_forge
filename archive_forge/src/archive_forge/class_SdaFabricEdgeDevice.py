...
class SdaFabricEdgeDevice(object):
    ...
    def get_object_by_name(self, name, is_absent=False):
        result = None
        try:
            items = self.dnac.exec(family='sda', function='get_edge_device', params=self.get_all_params(name=name))
    ...
    def create(self):
        result = self.dnac.exec(family='sda', function='add_edge_device', params=self.create_params(), op_modifies=True)
    ...
    def delete(self):
        ...
        name = self.new_object.get('name')
        result = None
        result = self.dnac.exec(family='sda', function='delete_edge_device', params=self.delete_all_params())
        return result