...
class DevicesSwitchRoutingInterfaces(object):
    ...
    def __init__(self, params, meraki):
        self.meraki = meraki
        self.new_object = dict(name=params.get('name'), subnet=params.get('subnet'), interfaceIp=params.get('interfaceIp'), multicastRouting=params.get('multicastRouting'), vlanId=params.get('vlanId'), defaultGateway=params.get('defaultGateway'), ospfSettings=params.get('ospfSettings'), ospfV3=params.get('ospfV3'), ipv6=params.get('ipv6'), serial=params.get('serial'), interfaceId=params.get('interfaceId'))
    ...
    def create_params(self):
        ...
        if self.new_object.get('ospfV3') is not None or self.new_object.get('ospf_v3') is not None:
            new_object_params['ospfV3'] = self.new_object.get('ospfV3') or self.new_object.get('ospf_v3')
    ...
    def get_object_by_name(self, name):
        result = None
        try:
            items = self.meraki.exec_meraki(family='switch', function='getDeviceSwitchRoutingInterfaces', params=self.get_all_params(name=name))
    ...
    def get_object_by_id(self, id):
        result = None
        try:
            items = self.meraki.exec_meraki(family='switch', function='getDeviceSwitchRoutingInterface', params=self.get_params_by_id())
            if isinstance(items, dict):
                if 'response' in items:
                    items = items.get('response')
    ...
    def requires_update(self, current_obj):
        ...
        obj_params = [('name', 'name'), ('subnet', 'subnet'), ('interfaceIp', 'interfaceIp'), ('multicastRouting', 'multicastRouting'), ('vlanId', 'vlanId'), ('defaultGateway', 'defaultGateway'), ('ospfSettings', 'ospfSettings'), ('ospfV3', 'ospfV3'), ('ipv6', 'ipv6'), ('serial', 'serial'), ('interfaceId', 'interfaceId')]
    ...
    def create(self):
        result = self.meraki.exec_meraki(family='switch', function='createDeviceSwitchRoutingInterface', params=self.create_params(), op_modifies=True)
        return result
    ...
    def update(self):
        ...
        result = self.meraki.exec_meraki(family='switch', function='updateDeviceSwitchRoutingInterface', params=self.update_by_id_params(), op_modifies=True)
        return result
    ...
    def delete(self):
        ...
        if not id:
            ...
            if id_:
                self.new_object.update(dict(interfaceId=id_))
        result = self.meraki.exec_meraki(family='switch', function='deleteDeviceSwitchRoutingInterface', params=self.delete_by_id_params())
        return result