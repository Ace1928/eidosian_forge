...
def wireless_controller_wtp_group(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('wireless-controller', 'wtp-group', filtered_data, vdom=vdom)
    ...
    if state == 'present' or state is True:
        return fos.set('wireless-controller', 'wtp-group', data=filtered_data, vdom=vdom)
    elif state == 'absent':
        return fos.delete('wireless-controller', 'wtp-group', mkey=filtered_data['name'], vdom=vdom)
    else:
