...
def _AddBGPLBAddressPools(bgp_lb_config_group, is_update=False):
  ...
  """
    ...
    bgp_lb_address_pools_mutex_group = bgp_lb_config_group.add_group(help='BGP LB address pools configuration.', mutex=True, required=required)
    ...
    bgp_lb_address_pools_mutex_group.add_argument('--bgp-lb-address-pools-from-file', help=bgp_lb_address_pools_from_file_help_text, type=arg_parsers.YAMLFileContents(), hidden=True)
    bgp_lb_address_pools_mutex_group.add_argument('--bgp-lb-address-pools', help='BGP LB address pools configuration.', action='append', type=arg_parsers.ArgDict(spec={'pool': str, 'avoid-buggy-ips': arg_parsers.ArgBoolean(), 'manual-assign': arg_parsers.ArgBoolean(), 'addresses': arg_parsers.ArgList(custom_delim_char=';')}, required_keys=['pool', 'addresses']))