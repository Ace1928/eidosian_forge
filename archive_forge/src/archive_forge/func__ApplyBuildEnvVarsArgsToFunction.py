...
def _ApplyBuildEnvVarsArgsToFunction(function, args):
  ...
  """
    updated_fields = []
    old_build_env_vars = env_vars_api_util.GetEnvVarsAsDict(function.buildEnvironmentVariables)
    build_env_var_flags = map_util.GetMapFlagsFromArgs('build-env-vars', args)
    new_build_env_vars = map_util.ApplyMapFlags(old_build_env_vars, **build_env_var_flags)
    if old_build_env_vars != new_build_env_vars:
        build_env_vars_type_class = api_util.GetApiMessagesModule().CloudFunction.BuildEnvironmentVariablesValue
        function.buildEnvironmentVariables = env_vars_api_util.DictToEnvVarsProperty(build_env_vars_type_class, new_build_env_vars)
