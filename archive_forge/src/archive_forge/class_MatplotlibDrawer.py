...
class MatplotlibDrawer:
    ...
    _mathmode_regex = re.compile('(?<!\\\\)\\$(.*)(?<!\\\\)\\$')

    def __init__(self, qubits, clbits, nodes, circuit, scale=None, style=None, reverse_bits=False, plot_barriers=True, fold=25, ax=None, initial_state=False, cregbundle=None, with_layout=False, expr_len=30):
    ...
    def draw(self, filename=None, verbose=False):
        ...
        max_x_index = self._get_coords(node_data, wire_map, self._circuit, layer_widths, qubits_dict, clbits_dict, glob_data)
        ...
        self._draw_ops(self._nodes, node_data, wire_map, self._circuit, layer_widths, qubits_dict, clbits_dict, glob_data, verbose)
        if filename:
            mpl_figure.savefig(filename, dpi=self._style['dpi'], bbox_inches='tight', facecolor=mpl_figure.get_facecolor())
        if not is_user_ax:
    ...
    def _get_coords(self, node_data, wire_map, outer_circuit, layer_widths, qubits_dict, clbits_dict, glob_data, flow_parent=None):
        ...
        for layer in self._nodes:
            ...
            for node in layer:
                ...
                node_data[node].q_xy = [self._plot_coord(x_index, qubits_dict[ii]['y'], layer_widths[node][0], glob_data, flow_op) for ii in q_indxs]
                node_data[node].c_xy = [self._plot_coord(x_index, clbits_dict[ii]['y'], layer_widths[node][0], glob_data, flow_op) for ii in c_indxs]
    ...
    def _get_colors(self, node, node_data):
        ...
        if color is not None:
            if isinstance(color, str):
                ...
                fc = color[0]
                gt = color[1]
        elif self._style['name'] in ['iqp', 'iqx', 'iqp-dark', 'iqx-dark'] and base_name in ['x', 'dcx', 'swap']:
    ...
    def _measure(self, node, node_data, outer_circuit, glob_data):
        ...
        self._gate(node, node_data, glob_data)
        arc = glob_data['patches_mod'].Arc(xy=(qx, qy - 0.15 * HIG), width=WID * 0.7, height=HIG * 0.7, theta1=0, theta2=180, fill=False, ec=node_data[node].gt, linewidth=self._lwidth2, zorder=PORDER_GATE)
        self._ax.add_patch(arc)
        self._ax.plot([qx, qx + 0.35 * WID], [qy - 0.15 * HIG, qy + 0.2 * HIG], color=node_data[node].gt, linewidth=self._lwidth2, zorder=PORDER_GATE)
        self._line(node_data[node].q_xy[0], [cx, cy + 0.35 * WID], lc=self._style['cc'], ls=self._style['cline'])
        arrowhead = glob_data['patches_mod'].Polygon(((cx - 0.2 * WID, cy + 0.35 * WID), (cx + 0.2 * WID, cy + 0.35 * WID), (cx, cy + 0.04)), fc=self._style['cc'], ec=None)
        self._ax.add_artist(arrowhead)
        if self._cregbundle and register is not None:
            self._ax.text(cx + 0.25, cy + 0.1, str(reg_index), ha='left', va='bottom', fontsize=0.8 * self._style['fs'], color=self._style['tc'], clip_on=True, zorder=PORDER_TEXT)
    ...
    def _barrier(self, node, node_data, glob_data):
        ...
        for i, xy in enumerate(node_data[node].q_xy):
            ...
            if i == 0 and node.op.label is not None:
                ...
                ypos_adj = 0.0
            self._ax.plot([xpos, xpos], [ypos + 0.5 + ypos_adj, ypos - 0.5], linewidth=self._lwidth1, linestyle='dashed', color=self._style['lc'], zorder=PORDER_TEXT)
            box = glob_data['patches_mod'].Rectangle(xy=(xpos - 0.3 * WID, ypos - 0.5), width=0.6 * WID, height=1.0 + ypos_adj, fc=self._style['bc'], ec=None, alpha=0.6, linewidth=self._lwidth15, zorder=PORDER_BARRIER)
            self._ax.add_patch(box)
            if i == 0 and node.op.label is not None:
                dir_ypos = ypos + 0.65 * HIG
                self._ax.text(xpos, dir_ypos, node.op.label, ha='center', va='top', fontsize=self._style['fs'], color=node_data[node].tc, clip_on=True, zorder=PORDER_TEXT)
    ...
    def _gate(self, node, node_data, glob_data, xy=None):
        ...
        box = glob_data['patches_mod'].Rectangle(xy=(xpos - 0.5 * wid, ypos - 0.5 * HIG), width=wid, height=HIG, fc=node_data[node].fc, ec=node_data[node].ec, linewidth=self._lwidth15, zorder=PORDER_GATE)
        self._ax.add_patch(box)
        if node_data[node].gate_text:
            gate_ypos = ypos
            if node_data[node].param_text:
                gate_ypos = ypos + 0.15 * HIG
                self._ax.text(xpos, ypos - 0.3 * HIG, node_data[node].param_text, ha='center', va='center', fontsize=self._style['sfs'], color=node_data[node].sc, clip_on=True, zorder=PORDER_TEXT)
            self._ax.text(xpos, gate_ypos, node_data[node].gate_text, ha='center', va='center', fontsize=self._style['fs'], color=node_data[node].gt, clip_on=True, zorder=PORDER_TEXT)
    ...
    def _swap(self, xy, node, node_data, color=None):
        ...
        if node_data[node].raw_gate_text in self._calibrations:
            xpos, ypos = xy[0]
            self._ax.text(xpos, ypos + 0.7 * HIG, gate_text, ha='center', va='top', fontsize=self._style['sfs'], color=self._style['tc'], clip_on=True, zorder=PORDER_TEXT)
    ...
    def _line(self, xy0, xy1, lc=None, ls=None, zorder=PORDER_LINE):
        ...
        if linestyle == 'doublet':
            ...
            self._ax.plot([x0 + dx, x1 + dx], [y0 + dy, y1 + dy], color=linecolor, linewidth=self._lwidth2, linestyle='solid', zorder=zorder)
            self._ax.plot([x0 - dx, x1 - dx], [y0 - dy, y1 - dy], color=linecolor, linewidth=self._lwidth2, linestyle='solid', zorder=zorder)
        else:
            self._ax.plot([x0, x1], [y0, y1], color=linecolor, linewidth=self._lwidth2, linestyle=linestyle, zorder=zorder)
