...
class LookupJoinView(Table):
    ...
    def __init__(self, left, right, lkey, rkey, presorted=False, missing=None, buffersize=None, tempdir=None, cache=True, lprefix=None, rprefix=None):
        if presorted:
            self.left = left
            self.right = right
        else:
    ...
    def __iter__(self):
        return iterlookupjoin(self.left, self.right, self.lkey, self.rkey, missing=self.missing, lprefix=self.lprefix, rprefix=self.rprefix)