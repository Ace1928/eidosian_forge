...
class GeometryArray(ExtensionArray):
    ...
    def __init__(self, data, crs=None):
        if isinstance(data, self.__class__):
            ...
            raise TypeError("'data' should be array of geometry objects. Use from_shapely, from_wkb, from_wkt functions to construct a GeometryArray.")
    ...
    @property
    def data(self):
        warnings.warn('Accessing the underlying geometries through the `.data` attribute is deprecated and will be removed in GeoPandas 1.0. You can use `np.asarray(..)` or the `to_numpy()` method instead.\nNote that if you are using PyGEOS and using this attribute to get an array of PyGEOS geometries, those other methods will always return an array of Shapely geometries. Accessing the underlying PyGEOS geometries directly is deprecated, and you should migrate to use Shapely >= 2.0 instead.', DeprecationWarning, stacklevel=2)
        return self._data
    ...
    def __setitem__(self, key, value):
        ...
        if isinstance(value, GeometryArray):
            ...
            raise TypeError('Value should be either a BaseGeometry or None, got %s' % str(value))
        self._sindex = None
    ...
    def delaunay_triangles(self, tolerance, only_edges):
        return GeometryArray(vectorized.delaunay_triangles(self._data, tolerance, only_edges), crs=self.crs)
    ...
    def minimum_rotated_rectangle(self):
        return GeometryArray(vectorized.minimum_rotated_rectangle(self._data), crs=self.crs)
    ...
    def offset_curve(self, distance, quad_segs=8, join_style='round', mitre_limit=5.0):
        return GeometryArray(vectorized.offset_curve(self._data, distance, quad_segs=quad_segs, join_style=join_style, mitre_limit=mitre_limit), crs=self.crs)
    ...
    def remove_repeated_points(self, tolerance=0.0):
        return GeometryArray(vectorized.remove_repeated_points(self._data, tolerance=tolerance), crs=self.crs)

    def representative_point(self):
        return GeometryArray(vectorized.representative_point(self._data), crs=self.crs)

    def minimum_bounding_circle(self):
        return GeometryArray(vectorized.minimum_bounding_circle(self._data), crs=self.crs)
    ...
    def segmentize(self, max_segment_length):
        return GeometryArray(vectorized.segmentize(self._data, max_segment_length), crs=self.crs)
    ...
    def geom_almost_equals(self, other, decimal):
        warnings.warn("The 'geom_almost_equals()' method is deprecated because the name is confusing. The 'geom_equals_exact()' method should be used instead.", FutureWarning, stacklevel=2)
        return self.geom_equals_exact(other, 0.5 * 10 ** (-decimal))

    def clip_by_rect(self, xmin, ymin, xmax, ymax):
        return GeometryArray(vectorized.clip_by_rect(self._data, xmin, ymin, xmax, ymax), crs=self.crs)

    def difference(self, other):
        return GeometryArray(self._binary_method('difference', self, other), crs=self.crs)

    def intersection(self, other):
        return GeometryArray(self._binary_method('intersection', self, other), crs=self.crs)

    def symmetric_difference(self, other):
        return GeometryArray(self._binary_method('symmetric_difference', self, other), crs=self.crs)
    ...
    def buffer(self, distance, resolution=16, **kwargs):
        if not (isinstance(distance, (int, float)) and distance == 0):
            self.check_geographic_crs(stacklevel=5)
        return GeometryArray(vectorized.buffer(self._data, distance, resolution=resolution, **kwargs), crs=self.crs)

    def interpolate(self, distance, normalized=False):
        self.check_geographic_crs(stacklevel=5)
        return GeometryArray(vectorized.interpolate(self._data, distance, normalized=normalized), crs=self.crs)

    def simplify(self, tolerance, preserve_topology=True):
        return GeometryArray(vectorized.simplify(self._data, tolerance, preserve_topology=preserve_topology), crs=self.crs)
    ...
    def affine_transform(self, matrix):
        return GeometryArray(vectorized._affinity_method('affine_transform', self._data, matrix), crs=self.crs)

    def translate(self, xoff=0.0, yoff=0.0, zoff=0.0):
        return GeometryArray(vectorized._affinity_method('translate', self._data, xoff, yoff, zoff), crs=self.crs)

    def rotate(self, angle, origin='center', use_radians=False):
        return GeometryArray(vectorized._affinity_method('rotate', self._data, angle, origin=origin, use_radians=use_radians), crs=self.crs)

    def scale(self, xfact=1.0, yfact=1.0, zfact=1.0, origin='center'):
        return GeometryArray(vectorized._affinity_method('scale', self._data, xfact, yfact, zfact, origin=origin), crs=self.crs)

    def skew(self, xs=0.0, ys=0.0, origin='center', use_radians=False):
        return GeometryArray(vectorized._affinity_method('skew', self._data, xs, ys, origin=origin, use_radians=use_radians), crs=self.crs)
    ...
    def to_crs(self, crs=None, epsg=None):
        ...
        """
        if self.crs is None:
            raise ValueError('Cannot transform naive geometries.  Please set a crs on the object first.')
        if crs is not None:
    ...
    def estimate_utm_crs(self, datum_name='WGS 84'):
        ...
        if self.crs.is_geographic:
            ...
            if minx > maxx:
                ...
                x_center = np.mean([minx, maxx])
        utm_crs_list = query_utm_crs_info(datum_name=datum_name, area_of_interest=AreaOfInterest(west_lon_degree=x_center, south_lat_degree=y_center, east_lon_degree=x_center, north_lat_degree=y_center))
        try:
    ...
    def _fill(self, idx, value):
        ...
        if vectorized.isna(value):
            ...
            raise TypeError(f"'value' parameter must be None, a scalar geometry, or a GeoSeries, but you passed a {type(value).__name__!r}")
    ...
    def value_counts(self, dropna: bool=True):
        ...
        result.index = Index(from_wkb(np.where(result.index.isna(), None, result.index)))
        return result
    ...
    def _binop(self, other, op):
        ...
        def convert_values(param):
            if not _is_scalar_geometry(param) and (isinstance(param, ExtensionArray) or pd.api.types.is_list_like(param)):
                ovalues = param
            else:
