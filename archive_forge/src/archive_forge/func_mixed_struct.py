...
def mixed_struct(n_a: int, name=None):
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'MixedStruct', name, 'n_a', n_a)
            ...
            pass
        try:
            _result = _dispatcher_for_mixed_struct((n_a, name), None)
            if _result is not NotImplemented:
                return _result
            return mixed_struct_eager_fallback(n_a=n_a, name=name, ctx=_ctx)
        except _core._SymbolicException:
            pass
        except (TypeError, ValueError):
            _result = _dispatch.dispatch(mixed_struct, (), dict(n_a=n_a, name=name))
    ...
    try:
        _, _, _op, _outputs = _op_def_library._apply_op_helper('MixedStruct', n_a=n_a, name=name)
    except (TypeError, ValueError):
        _result = _dispatch.dispatch(mixed_struct, (), dict(n_a=n_a, name=name))
        if _result is not _dispatch.OpDispatcher.NOT_SUPPORTED:
            return _result
