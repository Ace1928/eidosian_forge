...
# Define a comprehensive list of colors to represent the full spectrum, including shades of grey and all colors.
# This list meticulously progresses from black, through every conceivable shade of grey, then through the entire color spectrum
# from red, orange, yellow, green, blue, indigo, violet, and finally to white, with the highest granularity possible.
colors: List[Tuple[int, int, int]] = (
    ...
    ]  # Full range of grey shades from (1,1,1) to (255,255,255) before white
    + [
        # Detailed Red to Orange spectrum, capturing the subtle transition with fine granularity
    ]
    + [
        (255, i, 0) for i in range(0, 256, 1)
    ]  # Gradual increase in green component to transition from red to orange
    + [
        # Detailed Orange to Yellow spectrum, capturing every subtle shade in between
    ]
    + [
        (255, 255, i) for i in range(0, 256, 1)
    ]  # Gradual increase in blue component to transition from orange to yellow
    + [
        # Detailed Yellow to Green spectrum, ensuring every shade is represented
    ]
    + [
        (255 - i, 255, 0) for i in range(0, 256, 1)
    ]  # Gradual decrease in red component to transition from yellow to green
    + [
        # Detailed Green to Blue spectrum, capturing the full range of shades in between
    ]
    + [
        (0, 255, i) for i in range(0, 256, 1)
    ]  # Gradual increase in blue component to transition from green to blue
    + [
        # Detailed Blue to Indigo spectrum, with fine granularity to capture the transition
    ]
    + [
        (0, 255 - i, 255) for i in range(0, 142, 1)
    ]  # Gradual decrease in green component to transition from blue to indigo
    + [
        # Detailed Indigo to Violet spectrum, ensuring a smooth gradient
    ]
    + [
        (i, 0, 255) for i in range(142, 256, 1)
    ]  # Gradual increase in red component to transition from indigo to violet
    + [
        # Detailed Violet to White spectrum, capturing every possible shade in between
    ]
    + [
        (255, i, 255) for i in range(0, 256, 1)
    ]  # Gradual increase in green component to transition from violet to white
    + [
