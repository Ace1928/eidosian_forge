...
def test_cannot_check_grant_for_user_other_domain_on_project_own_domain(self):
    ...
    with self.test_client() as c:
        c.get('/v3/projects/%s/users/%s/roles/%s' % (project['id'], user['id'], self.bootstrapper.reader_role_id), headers=self.headers, expected_status_code=http.client.FORBIDDEN)