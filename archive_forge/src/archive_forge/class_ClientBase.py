...
class ClientBase(object):
    ...
    def _Labels(self, args, parent_type):
        ...
        return parent_type.LabelsValue(additionalProperties=[label_type(key=k, value=v) for k, v in labels.items()])
    ...
    """
        ...
        kwargs = {'field': self._list_result_field, 'batch_size_attribute': 'pageSize'}
        if limit:
            kwargs['limit'] = limit
        if page_size:
    ...
    def Delete(self, resource_ref, validate_only=None, allow_missing=None, ignore_errors=None):
        """Deletes a gkemulticloud API resource."""
        req = self._service.GetRequestType('Delete')(name=resource_ref.RelativeName())
        if validate_only:
