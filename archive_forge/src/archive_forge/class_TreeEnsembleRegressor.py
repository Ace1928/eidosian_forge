...
class TreeEnsembleRegressor(OpRunAiOnnxMl):
    ...
    def _run(self, X, aggregate_function=None, base_values=None, base_values_as_tensor=None, n_targets=None, nodes_falsenodeids=None, nodes_featureids=None, nodes_hitrates=None, nodes_hitrates_as_tensor=None, nodes_missing_value_tracks_true=None, nodes_modes=None, nodes_nodeids=None, nodes_treeids=None, nodes_truenodeids=None, nodes_values=None, nodes_values_as_tensor=None, post_transform=None, target_ids=None, target_nodeids=None, target_treeids=None, target_weights=None, target_weights_as_tensor=None):
        ...
        for i in range(res.shape[0]):
            ...
            t_index = [target_index[nodes_treeids[i], nodes_nodeids[i]] for i in indices]
            if aggregate_function in ('SUM', 'AVERAGE'):
                ...
                for its in t_index:
                    for it in its:
                        res[i, target_ids[it]] = min(res[i, target_ids[it]], tr.atts.target_weights[it])
            elif aggregate_function == 'MAX':
                res[i, :] = np.finfo(res.dtype).min
                for its in t_index:
                    for it in its:
                        res[i, target_ids[it]] = max(res[i, target_ids[it]], tr.atts.target_weights[it])
            else:
                raise NotImplementedError(f'aggregate_transform={aggregate_function!r} not supported yet.')
