...
class LanguageParser(BaseBlobParser):
    ...
    def lazy_parse(self, blob: Blob) -> Iterator[Document]:
        code = blob.as_string()
        language = self.language or (LANGUAGE_EXTENSIONS.get(blob.source.rsplit('.', 1)[-1]) if isinstance(blob.source, str) else None)
        if language is None:
            yield Document(page_content=code, metadata={'source': blob.source})
            return
        if self.parser_threshold >= len(code.splitlines()):
            yield Document(page_content=code, metadata={'source': blob.source, 'language': language})
            return
        ...
        for functions_classes in segmenter.extract_functions_classes():
            yield Document(page_content=functions_classes, metadata={'source': blob.source, 'content_type': 'functions_classes', 'language': language})
        yield Document(page_content=segmenter.simplify_code(), metadata={'source': blob.source, 'content_type': 'simplified_code', 'language': language})