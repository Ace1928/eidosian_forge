...
class _AwsClientBase(client.ClientBase):
    ...
    def _Cluster(self, cluster_ref, args):
        ...
        kwargs = {'annotations': self._Annotations(args, cluster_type), 'authorization': self._Authorization(args), 'awsRegion': aws_flags.GetAwsRegion(args), 'binaryAuthorization': self._BinaryAuthorization(args), 'controlPlane': self._ControlPlane(args), 'description': flags.GetDescription(args), 'fleet': self._Fleet(args), 'loggingConfig': flags.GetLogging(args), 'monitoringConfig': flags.GetMonitoringConfig(args), 'name': cluster_ref.awsClustersId, 'networking': self._ClusterNetworking(args)}
        return self._messages.GoogleCloudGkemulticloudV1AwsCluster(**kwargs) if any(kwargs.values()) else None
    ...
    def _SshConfig(self, args):
        ...
        return self._messages.GoogleCloudGkemulticloudV1AwsSshConfig(**kwargs) if any(kwargs.values()) else None
    ...
    def _NodePoolAutoscaling(self, args):
        ...
        if not any(kwargs.values()):
            return None
        return self._messages.GoogleCloudGkemulticloudV1AwsNodePoolAutoscaling(**kwargs)