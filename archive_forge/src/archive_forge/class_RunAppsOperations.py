...
class RunAppsOperations(object):
    ...
    def __init__(self, client, api_version, region, release_track, service_account):
    ...
    def _GetDeploymentName(self, appname):
        return '{}-{}'.format(appname, datetime.datetime.now().strftime('%Y%m%d%H%M%S'))
    ...
    def GetIntegrationGeneric(self, name: str, res_type: Optional[str]=None) -> Optional[runapps_v1alpha1_messages.Resource]:
        """Get an integration.
    ...
    def MaybeGetIntegrationGeneric(self, name: str, res_type: Optional[str]=None) -> Optional[runapps_v1alpha1_messages.Resource]:
    ...
    def _FindResource(self, appconfig: runapps_v1alpha1_messages.Config, name: str, res_type: Optional[str]=None) -> Optional[runapps_v1alpha1_messages.Resource]:
    ...
    def GetLatestDeployment(self, resource: runapps_v1alpha1_messages.Resource) -> Optional[runapps_v1alpha1_messages.Deployment]:
    ...
    def GetIntegrationTypeDefinition(self, type_name, include_workload: bool=False):
    ...
    """
        ...
        if include_workload or (type_metadata is not None and type_metadata.service_type != types_utils.ServiceType.WORKLOAD):
    ...
    def GetBindingData(self) -> List[base.BindingData]:
    ...
    """
        ...
        app_ref = resources.REGISTRY.Parse(name, params={'projectsId': project, 'locationsId': location}, collection='runapps.projects.locations.applications')
        return app_ref

    def ListAppsRequest(self) -> resources:
        """Creates request object for calling ListApplications for all regions."""
        project = properties.VALUES.core.project.Get(required=True)
        app_ref = resources.REGISTRY.Parse(ALL_REGIONS, params={'projectsId': project}, collection='runapps.projects.locations')
        return app_ref
    ...
    """
        project = properties.VALUES.core.project.Get(required=True)
        service_ref = resources.REGISTRY.Parse(name, params={'namespacesId': project, 'servicesId': name}, collection='run.namespaces.services')
        return service_ref

    def EnsureWorkloadResources(self, appconfig: runapps_v1alpha1_messages.Config, res_name: str, res_type: str):
    ...
    """
        if not self._FindResource(appconfig, res_name, res_type):
            appconfig.resourceList.append(runapps_v1alpha1_messages.Resource(id=runapps_v1alpha1_messages.ResourceID(name=res_name, type=res_type)))
    ...
    """
        conn_context = connection_context.RegionalConnectionContext(self._region, global_methods.SERVERLESS_API_NAME, global_methods.SERVERLESS_API_VERSION)
    ...
    """
        for name in service_names:
            ...
            if not service:
                raise exceptions.ServiceNotFoundError('Service [{}] could not be found.'.format(name))
    ...
    def _AppendTypeMatcher(self, type_matchers: MutableSequence[base.Selector], res_type: str, res_name: str, ignore_resource_config: Optional[bool]=None):
        for matcher in type_matchers:
            if matcher['type'] == res_type and matcher['name'] == res_name:
                return
        type_matchers.append({'type': res_type, 'name': res_name, 'ignoreResourceConfig': ignore_resource_config})
