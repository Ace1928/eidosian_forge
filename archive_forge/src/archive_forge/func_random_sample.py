...
def random_sample(self, fraction: float, *, seed: Optional[int]=None) -> 'Dataset':
    ...
    def process_batch(batch):
        ...
        if isinstance(batch, pa.Table):
            return batch.filter(pa.array((random.random() <= fraction for _ in range(len(batch)))))
        if isinstance(batch, pd.DataFrame):
            return batch.sample(frac=fraction)
        if isinstance(batch, np.ndarray):
            return _create_possibly_ragged_ndarray([row for row in batch if random.random() <= fraction])
