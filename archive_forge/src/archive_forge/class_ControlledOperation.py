...
class ControlledOperation(raw_types.Operation):
    ...
    def __init__(self, controls: Sequence['cirq.Qid'], sub_operation: 'cirq.Operation', control_values: Optional[Union[cv.AbstractControlValues, Sequence[Union[int, Collection[int]]]]]=None):
        ...
        if len(controls) != len(control_set):
            seen = set()
            dupes = [x for x in controls if x in seen or seen.add(x)]
    ...
    def gate(self) -> Optional['cirq.ControlledGate']:
        if self.sub_operation.gate is None:
            return None
        return controlled_gate.ControlledGate(self.sub_operation.gate, control_values=self.control_values, control_qid_shape=[q.dimension for q in self.controls])
    ...
    def with_qubits(self, *new_qubits):
        n = len(self.controls)
        return ControlledOperation(new_qubits[:n], self.sub_operation.with_qubits(*new_qubits[n:]), self.control_values)

    def _decompose_(self):
        return self._decompose_with_context_()

    def _decompose_with_context_(self, context: Optional['cirq.DecompositionContext']=None):
        result = protocols.decompose_once_with_qubits(self.gate, self.qubits, NotImplemented, flatten=False, context=context)
        if result is not NotImplemented:
            return result
        if isinstance(self.sub_operation.gate, matrix_gates.MatrixGate):
            return NotImplemented
        result = protocols.decompose_once(self.sub_operation, NotImplemented, flatten=False, context=context)
        if result is NotImplemented:
            return NotImplemented
        return op_tree.transform_op_tree(result, lambda op: op.controlled_by(*self.controls, control_values=self.control_values))

    def _value_equality_values_(self):
        sorted_controls, expanded_cvals = tuple(zip(*sorted(zip(self.controls, zip(*self.control_values.expand())))))
    ...
    def _apply_unitary_(self, args: 'protocols.ApplyUnitaryArgs') -> np.ndarray:
        ...
        for control_vals in self.control_values.expand():
            active: Tuple[Union['ellipsis', slice], ...] = (..., *(slice(v, v + 1) for v in control_vals), *(slice(None),) * sub_n)
            target_view = args.target_tensor[active]
            buffer_view = args.available_buffer[active]
            result = protocols.apply_unitary(self.sub_operation, protocols.ApplyUnitaryArgs(target_view, buffer_view, sub_axes), default=NotImplemented)
            if result is NotImplemented:
                return NotImplemented
    ...
    def _qasm_(self, args: 'cirq.QasmArgs') -> Optional[str]:
        if hasattr(self._sub_operation, 'gate') and len(self._controls) == 1 and (self.control_values == cv.ProductOfSums(((1,),))):
            gate = self.sub_operation.gate
            if isinstance(gate, eigen_gate.EigenGate) and gate.exponent == 1 and (gate.global_shift == 0):
                instr = None
    ...
    def __repr__(self):
        ...
        return f'cirq.ControlledOperation(sub_operation={self.sub_operation!r},control_values={self.control_values!r},controls={self.controls!r})'
    ...
    def _resolve_parameters_(self, resolver: 'cirq.ParamResolver', recursive: bool) -> 'ControlledOperation':
    ...
    def _circuit_diagram_info_(self, args: 'cirq.CircuitDiagramInfoArgs') -> Optional['protocols.CircuitDiagramInfo']:
        n = len(self.controls)
        sub_args = protocols.CircuitDiagramInfoArgs(known_qubit_count=args.known_qubit_count - n if args.known_qubit_count is not None else None, known_qubits=args.known_qubits[n:] if args.known_qubits is not None else None, use_unicode_characters=args.use_unicode_characters, precision=args.precision, label_map=args.label_map)
        ...
        return protocols.CircuitDiagramInfo(wire_symbols=wire_symbols, exponent=sub_info.exponent, exponent_qubit_index=exponent_qubit_index)

    def _json_dict_(self) -> Dict[str, Any]:
        return {'controls': self.controls, 'control_values': self.control_values, 'sub_operation': self.sub_operation}