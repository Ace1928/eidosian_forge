...
def get_html(node, fullpage=False):
  ...
  """
    ...
    try:
        ...
        logging.error('Cannot import jinja. Please install jinja in your python environment to use the fullpage html renderer')
        return None
    thisdir = os.path.realpath(os.path.dirname(__file__))
    tpldir = os.path.join(thisdir, 'templates')
    env = jinja2.Environment(loader=jinja2.FileSystemLoader(tpldir), autoescape=jinja2.select_autoescape(['html']))
