...
class TestBGPVPN(base.BaseFunctionalTest):
    ...
    def setUp(self):
        ...
        bgpvpn = self.operator_cloud.network.create_bgpvpn(name=self.BGPVPN_NAME, route_distinguishers=self.ROUTE_DISTINGUISHERS, route_targets=self.ROUTE_TARGETS, import_targets=self.IMPORT_TARGETS, export_targets=self.EXPORT_TARGETS)
        ...
        self.NETWORK = net
        subnet = self.operator_cloud.network.create_subnet(name=self.SUBNET_NAME, ip_version=4, network_id=self.NETWORK.id, cidr=self.CIDR)
        assert isinstance(subnet, _subnet.Subnet)
        self.SUBNET = subnet
        port = self.operator_cloud.network.create_port(name=self.PORT_NAME, network_id=self.NETWORK.id)
        ...
        self.ROUTER = router
        net_assoc = self.operator_cloud.network.create_bgpvpn_network_association(self.BGPVPN, network_id=self.NETWORK.id)
        assert isinstance(net_assoc, _bgpvpn_net_assoc.BgpVpnNetworkAssociation)
        self.NET_ASSOC = net_assoc
        port_assoc = self.operator_cloud.network.create_bgpvpn_port_association(self.BGPVPN, port_id=self.PORT.id)
        assert isinstance(port_assoc, _bgpvpn_port_assoc.BgpVpnPortAssociation)
        self.PORT_ASSOC = port_assoc
        router_assoc = self.operator_cloud.network.create_bgpvpn_router_association(self.BGPVPN, router_id=self.ROUTER.id)
    ...
    def tearDown(self):
        ...
        sot = self.operator_cloud.network.delete_bgpvpn_network_association(self.BGPVPN.id, self.NET_ASSOC.id)
        self.assertIsNone(sot)
        sot = self.operator_cloud.network.delete_bgpvpn_port_association(self.BGPVPN.id, self.PORT_ASSOC.id)
        self.assertIsNone(sot)
        sot = self.operator_cloud.network.delete_bgpvpn_router_association(self.BGPVPN.id, self.ROUTER_ASSOC.id)
        self.assertIsNone(sot)
