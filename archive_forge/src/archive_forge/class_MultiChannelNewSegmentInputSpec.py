...
class MultiChannelNewSegmentInputSpec(SPMCommandInputSpec):
    channels = traits.List(traits.Tuple(InputMultiPath(ImageFileSPM(exists=True), mandatory=True, desc='A list of files to be segmented', field='channel', copyfile=False), traits.Tuple(traits.Float(), traits.Float(), traits.Tuple(traits.Bool, traits.Bool), desc='A tuple with the following fields:\n                    - bias reguralisation (0-10)\n                    - FWHM of Gaussian smoothness of bias\n                    - which maps to save (Field, Corrected) - a tuple of two boolean values', field='channel')), desc='A list of tuples (one per each channel) with the following fields:\n            - a list of channel files (only 1rst channel files will be segmented)\n            - a tuple with the following channel-specific info fields:\n              - bias reguralisation (0-10)\n              - FWHM of Gaussian smoothness of bias\n              - which maps to save (Field, Corrected) - a tuple of two boolean values', field='channel')
