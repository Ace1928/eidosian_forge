...
class PauliInteractionGate(gate_features.InterchangeableQubitsGate, eigen_gate.EigenGate):
    ...
    def _with_exponent(self, exponent: value.TParamVal) -> 'PauliInteractionGate':
        return PauliInteractionGate(self.pauli0, self.invert0, self.pauli1, self.invert1, exponent=exponent)

    def _eigen_shifts(self) -> List[float]:
        return [0.0, 1.0]

    def _eigen_components(self) -> List[Tuple[float, np.ndarray]]:
        comp1 = np.kron(PAULI_EIGEN_MAP[self.pauli0][not self.invert0], PAULI_EIGEN_MAP[self.pauli1][not self.invert1])
    ...
    def _circuit_diagram_info_(self, args: 'cirq.CircuitDiagramInfoArgs') -> 'cirq.CircuitDiagramInfo':
        ...
        return protocols.CircuitDiagramInfo(wire_symbols=(l0, l1), exponent=self._diagram_exponent(args))

    def __repr__(self) -> str:
        base = f'cirq.PauliInteractionGate({self.pauli0!r}, {self.invert0!s}, {self.pauli1!r}, {self.invert1!s})'
        if self._exponent == 1:
            return base
