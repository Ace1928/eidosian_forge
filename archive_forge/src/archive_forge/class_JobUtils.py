...
class JobUtils(baseutils.BaseUtilsVirt):
    ...
    _completed_job_states = [constants.JOB_STATE_COMPLETED, constants.JOB_STATE_TERMINATED, constants.JOB_STATE_KILLED, constants.JOB_STATE_COMPLETED_WITH_WARNINGS, constants.JOB_STATE_EXCEPTION]
    _successful_job_states = [constants.JOB_STATE_COMPLETED, constants.JOB_STATE_COMPLETED_WITH_WARNINGS]
    ...
    def modify_virt_resource(self, virt_resource):
        job_path, out_set_data, ret_val = self._vs_man_svc.ModifyResourceSettings(ResourceSettings=[virt_resource.GetText_(1)])
        self.check_ret_val(ret_val, job_path)
    ...
    def modify_virt_feature(self, virt_feature):
        job_path, out_set_data, ret_val = self._vs_man_svc.ModifyFeatureSettings(FeatureSettings=[virt_feature.GetText_(1)])
        self.check_ret_val(ret_val, job_path)
