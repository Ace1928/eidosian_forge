...
class DetailedVoronoiContainer(MSONable):
    ...
    default_normalized_distance_tolerance = 1e-05
    default_normalized_angle_tolerance = 0.001

    def __init__(self, structure=None, voronoi_list2=None, voronoi_cutoff=default_voronoi_cutoff, isites=None, normalized_distance_tolerance=default_normalized_distance_tolerance, normalized_angle_tolerance=default_normalized_angle_tolerance, additional_conditions=None, valences=None, maximum_distance_factor=None, minimum_angle_factor=None):
        """
    ...
    def neighbors(self, isite, distfactor, angfactor, additional_condition=None):
        ...
        return [nb for nb in self.voronoi_list2[isite] if nb['normalized_distance'] <= dfact and nb['normalized_angle'] >= afact]
    ...
    def __eq__(self, other: object) -> bool:
        needed_attrs = ('normalized_angle_tolerance', 'normalized_distance_tolerance', 'additional_conditions', 'valences', 'voronoi_list2', 'structure')
    ...
    def as_dict(self):
        ...
        return {'@module': type(self).__module__, '@class': type(self).__name__, 'bson_nb_voro_list2': bson_nb_voro_list2, 'structure': self.structure.as_dict(), 'normalized_angle_tolerance': self.normalized_angle_tolerance, 'normalized_distance_tolerance': self.normalized_distance_tolerance, 'additional_conditions': self.additional_conditions, 'valences': self.valences, 'maximum_distance_factor': self.maximum_distance_factor, 'minimum_angle_factor': self.minimum_angle_factor}
    ...
    def from_dict(cls, dct: dict) -> Self:
        ...
        return cls(structure=structure, voronoi_list2=voronoi_list2, normalized_angle_tolerance=dct['normalized_angle_tolerance'], normalized_distance_tolerance=dct['normalized_distance_tolerance'], additional_conditions=dct['additional_conditions'], valences=dct['valences'], maximum_distance_factor=maximum_distance_factor, minimum_angle_factor=minimum_angle_factor)