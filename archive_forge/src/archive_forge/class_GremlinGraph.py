...
class GremlinGraph(GraphStore):
    ...
    *Implementation details*:
    ...
    def __init__(self, url: Optional[str]=None, username: Optional[str]=None, password: Optional[str]=None, traversal_source: str='g', message_serializer: Optional[Any]=None) -> None:
        ...
        try:
            ...
            raise ValueError('Please install gremlin-python first: `pip3 install gremlinpython')
    ...
    def refresh_schema(self) -> None:
        ...
        vertex_properties = self.client.submit('g.V().group().by(label).by(properties().label().dedup().fold())').all().result()[0]
        self.structured_schema = {'vertex_labels': vertex_schema, 'edge_labels': edge_schema, 'vertice_props': vertex_properties}
    ...
    def add_graph_documents(self, graph_documents: List[GraphDocument], include_source: bool=False) -> None:
        ...
        for document in graph_documents:
            ...
            for n in document.nodes:
                ...
                if include_source:
                    self.add_edge(Relationship(type='contains information about', source=doc_node, target=node, properties={}))
                    self.add_edge(Relationship(type='is extracted from', source=node, target=doc_node, properties={}))
