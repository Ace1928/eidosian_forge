...
class VaspInteractive(GenerateVaspInput, Calculator):
    ...
    mandatory_input = {'potim': 0.0, 'ibrion': -1, 'interactive': True}
    default_input = {'nsw': 2000}

    def __init__(self, txt='interactive.log', print_log=False, process=None, command=None, path='./', **kwargs):
        GenerateVaspInput.__init__(self)
        for kw, val in self.mandatory_input.items():
            if kw in kwargs and val != kwargs[kw]:
                raise ValueError('Keyword {} cannot be overridden! It must have have value {}, but {} was provided instead.'.format(kw, val, kwargs[kw]))
        ...
        if command is not None:
            ...
            raise RuntimeError('Please set either command in calculator or VASP_COMMAND environment variable')
    ...
    def calculate(self, atoms=None, properties=['energy'], system_changes=['positions', 'numbers', 'cell']):
        ...
        self.results = {'free_energy': new.get_potential_energy(force_consistent=True), 'energy': new.get_potential_energy(), 'forces': new.get_forces()[self.resort], 'stress': new.get_stress()}
