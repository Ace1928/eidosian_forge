...
class WideDeepModel(keras_training.Model):
        ...
        if getattr(self, 'train_function', None) is None or has_recompiled:
            ...
            with backend.get_graph().as_default():
                with backend.name_scope('training'):
                    updates = []
                    linear_updates = linear_optimizer.get_updates(params=self.linear_model.trainable_weights, loss=self.total_loss)
                    updates += linear_updates
                    dnn_updates = dnn_optimizer.get_updates(params=self.dnn_model.trainable_weights, loss=self.total_loss)
                    updates += dnn_updates
    ...
    def get_config(self):
        ...
        config = {'linear_model': linear_config, 'dnn_model': dnn_config, 'activation': activations.serialize(self.activation)}
    ...
    def from_config(cls, config, custom_objects=None):
        ...
        activation = activations.deserialize(config.pop('activation', None), custom_objects=custom_objects)
        return cls(linear_model=linear_model, dnn_model=dnn_model, activation=activation, **config)