...
class _MPResterBasic:
    ...
    def __getattr__(self, item):
        if item in ('summary', 'materials', 'thermo'):
            raise AttributeError(f'{item} is not an attribute of this implementation of MPRester, which only supports functionalityused by 80% of users. If you are looking for the full functionality MPRester, pls install the mp-api .')
    ...
    def get_entries(self, criteria, compatible_only=True, inc_structure=None, property_data=None, conventional_unit_cell=False, sort_by_e_above_hull=False):
        """
