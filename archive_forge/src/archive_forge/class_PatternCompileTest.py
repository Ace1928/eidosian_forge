...
class PatternCompileTest(test.TestCase):
    ...
    def test_linear(self):
        ...
        self.assertEqual(['test', 'a', 'b', 'c', 'sub-test', 'd', 'sub-test[$]', 'test[$]'], order)
    ...
    def test_graph_nested(self):
        ...
        self.assertCountEqual(g.edges(), [('test', 'a'), ('test', 'b'), ('test', 'c'), ('test', 'd'), ('a', 'test[$]'), ('b', 'test[$]'), ('c', 'test[$]'), ('d', 'test[$]'), ('test', 'test2'), ('test2', 'e'), ('e', 'f'), ('f', 'g'), ('g', 'test2[$]'), ('test2[$]', 'test[$]')])
    ...
    def test_checks_for_dups_globally(self):
        flo = gf.Flow('test').add(gf.Flow('int1').add(test_utils.DummyTask(name='a')), gf.Flow('int2').add(test_utils.DummyTask(name='a')))
        e = engines.load(flo)
        self.assertRaisesRegex(exc.Duplicate, '^Atoms with duplicate names', e.compile)
    ...
    def test_retry_in_unordered_flow_with_tasks(self):
        ...
        g = _replicate_graph_with_names(compiler.PatternCompiler(flo).compile())
        self.assertEqual(5, len(g))
        self.assertCountEqual(g.edges(data=True), [('test', 'c', {'invariant': True}), ('c', 'a', {'invariant': True, 'retry': True}), ('c', 'b', {'invariant': True, 'retry': True}), ('b', 'test[$]', {'invariant': True}), ('a', 'test[$]', {'invariant': True})])
        self.assertCountEqual(['test'], list(g.no_predecessors_iter()))
        self.assertCountEqual(['test[$]'], list(g.no_successors_iter()))
        self.assertIs(c, g.nodes['a']['retry'])
        self.assertIs(c, g.nodes['b']['retry'])
    ...
    def test_retries_hierarchy(self):
        ...
        self.assertCountEqual(g.edges(data=True), [('test', 'c1', {'invariant': True}), ('c1', 'a', {'invariant': True, 'retry': True}), ('a', 'test2', {'invariant': True}), ('test2', 'c2', {'invariant': True}), ('c2', 'b', {'invariant': True, 'retry': True}), ('b', 'c', {'invariant': True}), ('c', 'test2[$]', {'invariant': True}), ('test2[$]', 'd', {'invariant': True}), ('d', 'test[$]', {'invariant': True})])
