...
class ContentsManager(LoggingConfigurable):
    ...
    preferred_dir = Unicode('', config=True, help=_i18n('Preferred starting directory to use for notebooks. This is an API path (`/` separated, relative to root dir)'))
    ...
    hide_globs = List(Unicode(), ['__pycache__', '*.pyc', '*.pyo', '.DS_Store', '*.so', '*.dylib', '*~'], config=True, help='\n        Glob patterns to hide in file and directory listings.\n    ')
    untitled_notebook = Unicode(_i18n('Untitled'), config=True, help='The base name used when creating untitled notebooks.')
    ...
    pre_save_hook = Any(None, config=True, allow_none=True, help='Python callable or importstring thereof\n\n        To be called on a contents model prior to save.\n\n        This can be used to process the structure,\n        such as removing notebook outputs or other side effects that\n        should not be saved.\n\n        It will be called as (all arguments passed by keyword)::\n\n            hook(path=path, model=model, contents_manager=self)\n\n        - model: the model to be saved. Includes file contents.\n          Modifying this dict will affect the file that is stored.\n        - path: the API path of the save destination\n        - contents_manager: this ContentsManager instance\n        ')
    ...
    def _validate_pre_save_hook(self, proposal):
        ...
        return value
    post_save_hook = Any(None, config=True, allow_none=True, help='Python callable or importstring thereof\n\n        to be called on the path of a file just saved.\n\n        This can be used to process the file on disk,\n        such as converting the notebook to a script or HTML via nbconvert.\n\n        It will be called as (all arguments passed by keyword)::\n\n            hook(os_path=os_path, model=model, contents_manager=instance)\n\n        - path: the filesystem path to the file just written\n        - model: the model representing the file\n        - contents_manager: this ContentsManager instance\n        ')
    ...
    def _validate_post_save_hook(self, proposal):
        ...
        if callable(self.post_save_hook):
            warnings.warn(f'Overriding existing post_save_hook ({self.post_save_hook.__name__}) with a new one ({value.__name__}).', stacklevel=2)
        return value
    ...
    files_handler_class = Type(FilesHandler, klass=RequestHandler, allow_none=True, config=True, help='handler class to use when serving raw file requests.\n\n        Default is a fallback that talks to the ContentsManager API,\n        which may be inefficient, especially for large files.\n\n        Local files-based ContentsManagers can use a StaticFileHandler subclass,\n        which will be much more efficient.\n\n        Access to these files should be Authenticated.\n        ')
    files_handler_params = Dict(config=True, help='Extra parameters to pass to files_handler_class.\n\n        For example, StaticFileHandlers generally expect a `path` argument\n        specifying the root directory from which to serve files.\n        ')
    ...
    def get_extra_handlers(self):
        ...
        if self.files_handler_class:
            handlers.append(('/files/(.*)', self.files_handler_class, self.files_handler_params))
        return handlers
    ...
    def validate_notebook_model(self, model, validation_error=None):
        ...
        try:
            ...
            model['message'] = 'Notebook validation failed: {}:\n{}'.format(str(e), json.dumps(e.instance, indent=1, default=lambda obj: '<UNKNOWN>'))
        return model
