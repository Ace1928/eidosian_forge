...
class BartAgent(TransformerGeneratorAgent):
    ...
    def add_cmdline_args(argparser: ParlaiParser):
        ...
        group.add_argument('--init-fairseq-model', type=str, default=None, help='fairseq checkpoint for bart')
        group.add_argument('--output-conversion-path', type=str, default=None, help='where to save fairseq conversion')
    ...
    def _set_text_vec(self, obs: Message, history: History, truncate: Optional[int]) -> Message:
        """
        ...
        if truncate is not None:
            vec = torch.LongTensor(self._check_truncate(obs['text_vec'], truncate - 2, True))
        obs.force_set('text_vec', self._add_start_end_tokens(vec, add_start=True, add_end=True))
        return obs
    ...
    def _generate(self, batch: Batch, beam_size: int, max_ts: int, prefix_tokens: Optional[torch.LongTensor]=None):
        ...
        if text_vec is not None:
            prefix_tokens = text_vec.new_zeros((text_vec.size(0), 1)).fill_(self.START_IDX)
