...
def xla_all_reduce_eager_fallback(input: _atypes.TensorFuzzingAnnotation[TV_XlaAllReduce_T], group_assignment: _atypes.TensorFuzzingAnnotation[_atypes.Int32], reduce_op: str, mode: str, name, ctx) -> _atypes.TensorFuzzingAnnotation[TV_XlaAllReduce_T]:
    ...
    _result = _execute.execute(b'XlaAllReduce', 1, inputs=_inputs_flat, attrs=_attrs, ctx=ctx, name=name)
