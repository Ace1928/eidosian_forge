...
class RWKV(MyModule):
    ...
    def __init__(self, model: str, strategy: str, verbose: bool=True, convert_and_save_and_exit: Optional[bool]=None):
        """
        ...
        STRATEGY_REGEX = '^(?:(?:^|->) *(?:cuda(?::[\\d]+)?|cpu|mps|dml) (?:fp(?:16|32)|bf16)(?:i8|i4|i3)?(?: \\*[\\d]+\\+?)? *)+$'
        ...
        self.RESCALE_LAYER = int(os.getenv('RWKV_RESCALE_LAYER', '6' if 'fp16' in strategy else '0'))
        prxxx(f'RWKV_JIT_ON {os.getenv('RWKV_JIT_ON')} RWKV_CUDA_ON {os.getenv('RWKV_CUDA_ON')} RESCALE_LAYER {self.RESCALE_LAYER}\n')
        ...
        with torch.no_grad():
            ...
            if ALREADY_CONVERTED:
                assert convert_and_save_and_exit is None, 'You should only convert a raw model'
                prxxx(f'Converted model: strategy {w['_strategy']}, version {w['_version']}\n')
                assert w['_strategy'] == args.strategy_string, 'If using a new strategy, re-convert the model'
                assert float(w['_version']) >= 0.7, 'Re-convert using latest convert_model.py'
            ...
            args.n_att = w['blocks.0.att.key.weight'].shape[0]
            args.n_ffn = w['blocks.0.ffn.key.weight'].shape[0]
            args.n_layer = max((int(x.split('.')[1]) if 'blocks.' in x else 0 for x in w.keys()))
            self.version = max(4, *(5 if 'ln_x' in x else 5.1 if 'gate.weight' in x else 6 if 'time_maa' in x else 0 for x in w.keys()))
            ...
            for i, si in enumerate(s):
                si1 = si[1]
                dtype = {'fp32': torch.float, 'fp16': torch.float16, 'bf16': torch.bfloat16}.get(si1[:4], torch.float)
            ...
            for i, ss in enumerate(s):
                prxxx(f'* {ss[0]} {str(ss[1]).replace('torch.', '')}, store {(plan[i] - stream_count if i == stream_i else plan[i])} layers' + (f', stream {stream_count} layers' if i == stream_i else ''))
            ...
            for n in range(args.n_layer + 1):
                for i in range(len(s)):
                    if n < plan[i]:
                        strategy[n] = types.SimpleNamespace(device=s[i][0], atype=s[i][1][0], wtype=s[i][1][1], stream=False)
                        if s[i][0] == 'dml':
            ...
            for x in keys:
                ...
                if not ALREADY_CONVERTED:
                    ...
                    if 'key.weight' in x or 'value.weight' in x or 'receptance.weight' in x or ('gate.weight' in x) or ('output.weight' in x) or ('head.weight' in x):
                        w[x] = w[x].t()
                    if '.time_decay' in x and '_w' not in x:
                        if self.version == 4:
                        ...
                        w[x] = w[x].float()
                    elif len(w[x].shape) == 2 and 'emb' not in x and ('_w1' not in x) and ('_w2' not in x):
            ...
            if self.version == 5.2 and os.environ['RWKV_CUDA_ON'] == '1':
                ...
                rwkv5 = load(name='rwkv5', sources=[f'{current_path}/cuda/rwkv5_op.cpp', f'{current_path}/cuda/rwkv5.cu'], verbose=True, extra_cuda_cflags=['-res-usage', '--use_fast_math', '-O3', '-Xptxas -O3' if os.name != 'nt' else '', '--extra-device-vectorization', f'-D_N_={HEAD_SIZE}'])
            ...
            if self.version == 6.0 and os.environ['RWKV_CUDA_ON'] == '1':
                HEAD_SIZE = args.n_att // args.n_head
                rwkv6 = load(name='rwkv6', sources=[f'{current_path}/cuda/rwkv6_op.cpp', f'{current_path}/cuda/rwkv6.cu'], verbose=True, extra_cuda_cflags=['-res-usage', '--use_fast_math', '-O3', '-Xptxas -O3', '--extra-device-vectorization', f'-D_N_={HEAD_SIZE}', f'-D_T_={4096}'])
        ...
        with torch.no_grad():
            ...
            if state == None:
                if self.version == 4:
                    ...
                    for i in range(args.n_layer):
                        ...
                        state[i * 5 + 3] = torch.zeros(args.n_att, dtype=torch.float, requires_grad=False, device=dev).contiguous() - 1e+30
            ...
            for i in range(args.n_layer):
                ...
                if self.version == 4:
                    ...
                    x, state[i * 3 + 0], state[i * 3 + 1] = ATT(x, state[i * 3 + 0], state[i * 3 + 1], w[f'{bbb}ln1.weight'], w[f'{bbb}ln1.bias'], w[f'{att}ln_x.weight'], w[f'{att}ln_x.bias'], w[f'{att}time_mix_k'], w[f'{att}time_mix_v'], w[f'{att}time_mix_r'], w[f'{att}time_decay'], w[f'{att}time_first'], kw, vw, rw, ow, kmx, krx, kmy, kry, vmx, vrx, vmy, vry, rmx, rrx, rmy, rry, omx, orx, omy, ory)
                elif self.version in [5.1, 5.2]:
                    x, state[i * 3 + 0], state[i * 3 + 1] = ATT(x, state[i * 3 + 0], state[i * 3 + 1], w[f'{bbb}ln1.weight'], w[f'{bbb}ln1.bias'], w[f'{att}ln_x.weight'], w[f'{att}ln_x.bias'], w[f'{att}time_mix_k'], w[f'{att}time_mix_v'], w[f'{att}time_mix_r'], w[f'{att}time_mix_g'], w[f'{att}time_decay'], w[f'{att}time_first'], kw, vw, rw, gw, ow, kmx, krx, kmy, kry, vmx, vrx, vmy, vry, rmx, rrx, rmy, rry, gmx, grx, gmy, gry, omx, orx, omy, ory)
                elif self.version == 6.0:
                    x, state[i * 3 + 0], state[i * 3 + 1] = ATT(x, state[i * 3 + 0], state[i * 3 + 1], w[f'{bbb}ln1.weight'], w[f'{bbb}ln1.bias'], w[f'{att}ln_x.weight'], w[f'{att}ln_x.bias'], w[f'{att}time_maa_x'], w[f'{att}time_maa_w'], w[f'{att}time_maa_k'], w[f'{att}time_maa_v'], w[f'{att}time_maa_r'], w[f'{att}time_maa_g'], w[f'{att}time_maa_w1'], w[f'{att}time_maa_w2'], w[f'{att}time_decay_w1'], w[f'{att}time_decay_w2'], w[f'{att}time_decay'], w[f'{att}time_first'], kw, vw, rw, gw, ow, kmx, krx, kmy, kry, vmx, vrx, vmy, vry, rmx, rrx, rmy, rry, gmx, grx, gmy, gry, omx, orx, omy, ory)
                if dd.stream:
                    del kw, vw, rw, ow
                    if self.version in [5.1, 5.2, 6.0]:
                        del gw
                ...
                if self.version == 4:
                    ...
                    offset = i * 3 + 2
                if self.version < 6.0:
                    x, state[offset] = FFN(x, state[offset], w[f'{bbb}ln2.weight'], w[f'{bbb}ln2.bias'], w[f'{ffn}time_mix_k'], w[f'{ffn}time_mix_r'], kw, vw, rw, kmx, krx, kmy, kry, vmx, vrx, vmy, vry, rmx, rrx, rmy, rry)
                else:
                    x, state[offset] = FFN(x, state[offset], w[f'{bbb}ln2.weight'], w[f'{bbb}ln2.bias'], w[f'{ffn}time_maa_k'], w[f'{ffn}time_maa_r'], kw, vw, rw, kmx, krx, kmy, kry, vmx, vrx, vmy, vry, rmx, rrx, rmy, rry)
                if dd.stream:
                    del kw, vw, rw
                if self.RESCALE_LAYER > 0:
