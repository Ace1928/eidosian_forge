...
class Deactivate(base.Command):
  ...
  """
    ...
    def Run(self, args):
        client = networkconnectivity_api.SpokesClient(release_track=self.ReleaseTrack())
        spoke_ref = args.CONCEPTS.spoke.Parse()
        op_ref = client.Deactivate(spoke_ref)
        log.status.Print('Deactivate request issued for: [{}]'.format(spoke_ref.Name()))
        ...
        poller = waiter.CloudOperationPoller(client.spoke_service, client.operation_service)
        if op_ref.done:
