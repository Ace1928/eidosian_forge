...
def sample_gen(dataset, xy, indexes=None, masked=False):
    ...
    if masked:
        ...
        dataset_is_masked = any(({MaskFlags.alpha, MaskFlags.per_dataset, MaskFlags.nodata} & enums for enums in mask_flags))
        mask = [False if dataset_is_masked and enums == {MaskFlags.all_valid} else True for enums in mask_flags]
