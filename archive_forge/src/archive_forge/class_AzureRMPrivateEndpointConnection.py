...
class AzureRMPrivateEndpointConnection(AzureRMModuleBase):
    ...
    def __init__(self):
        self.module_arg_spec = dict(name=dict(type='str', required=True), service_name=dict(type='str', required=True), resource_group=dict(type='str', required=True), connection_state=dict(type='dict', options=connection_state_spec), state=dict(type='str', choices=['present', 'absent'], default='present'))
        self.name = None
        self.service_name = None
        self.resource_group = None
        self.connection_state = None
        self.results = dict(changed=False)
        super(AzureRMPrivateEndpointConnection, self).__init__(self.module_arg_spec, supports_check_mode=True, supports_tags=False, facts_module=False)
