        ...
        for arg_name, arg_type in config.input_dtypes.items():
            if arg_name in kwargs:
                ...
                if not isinstance(arg_value, arg_type):
                    raise TypeError(f"Expected argument '{arg_name}' to be of type {arg_type}, but got {type(arg_value)}")
            else:
                found_arg = False
                for arg in args:
                    if isinstance(arg, arg_type):
                        ...
                        break
                if not found_arg:
                    raise TypeError(f'Expected an argument of type {arg_type}, but none was found')
        result = await func(*args, **kwargs)
        if isinstance(result, tuple):
            for i, (output_name, output_type) in enumerate(config.output_dtypes.items()):
                if i < len(result):
                    output_value = result[i]
                    if not isinstance(output_value, output_type):
                        raise TypeError(f"Expected output at index {i} ('{output_name}') to be of type {output_type}, but got {type(output_value)}")
        elif isinstance(result, dict):
            for output_name, output_type in config.output_dtypes.items():
                if output_name in result:
                    output_value = result[output_name]
                    if not isinstance(output_value, output_type):
                        raise TypeError(f"Expected output '{output_name}' to be of type {output_type}, but got {type(output_value)}")
        else:
            output_name, output_type = next(iter(config.output_dtypes.items()))
            if not isinstance(result, output_type):
                raise TypeError(f"Expected output '{output_name}' to be of type {output_type}, but got {type(result)}")
        return result
    return wrapper