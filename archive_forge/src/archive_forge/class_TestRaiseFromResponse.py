...
class TestRaiseFromResponse(base.TestCase):
    ...
    def test_raise_bad_request_exception(self):
        ...
        response.headers = {'content-type': 'application/json', 'x-openstack-request-id': uuid.uuid4().hex}
        exc = self.assertRaises(exceptions.BadRequestException, self._do_raise, response, error_message=self.message)
        self.assertEqual(self.message, exc.message)
        self.assertEqual(response.status_code, exc.status_code)
        self.assertEqual(response.headers.get('x-openstack-request-id'), exc.request_id)
    ...
    def test_raise_http_exception(self):
        ...
        response.headers = {'content-type': 'application/json', 'x-openstack-request-id': uuid.uuid4().hex}
        exc = self.assertRaises(exceptions.HttpException, self._do_raise, response, error_message=self.message)
        self.assertEqual(self.message, exc.message)
        self.assertEqual(response.status_code, exc.status_code)
        self.assertEqual(response.headers.get('x-openstack-request-id'), exc.request_id)
    ...
    def test_raise_compute_format(self):
        ...
        response.json.return_value = {'itemNotFound': {'message': self.message, 'code': 404}}
        exc = self.assertRaises(exceptions.NotFoundException, self._do_raise, response, error_message=self.message)
        self.assertEqual(response.status_code, exc.status_code)
    ...
    def test_raise_baremetal_corrected_format(self):
        ...
        response.status_code = 404
        response.headers = {'content-type': 'application/json'}
        response.json.return_value = {'error_message': {'faultstring': self.message, 'faultcode': 'Client', 'debuginfo': None}}
        exc = self.assertRaises(exceptions.NotFoundException, self._do_raise, response, error_message=self.message)
        self.assertEqual(response.status_code, exc.status_code)
        self.assertEqual(self.message, exc.details)
        self.assertIn(self.message, str(exc))
    ...
    def test_raise_wsme_format(self):
        ...
        response.status_code = 404
        response.headers = {'content-type': 'application/json'}
        response.json.return_value = {'faultstring': self.message, 'faultcode': 'Client', 'debuginfo': None}
        exc = self.assertRaises(exceptions.NotFoundException, self._do_raise, response, error_message=self.message)
        self.assertEqual(response.status_code, exc.status_code)
        self.assertEqual(self.message, exc.details)
        self.assertIn(self.message, str(exc))