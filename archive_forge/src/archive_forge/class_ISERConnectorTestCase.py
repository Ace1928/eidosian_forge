...
class ISERConnectorTestCase(test_connector.ConnectorTestCase):
    ...
    def setUp(self):
        ...
        self.connection_data = {'volume_id': 'volume_id', 'target_portal': 'ip:port', 'target_iqn': 'target_1', 'target_lun': 1, 'target_portals': ['ip:port'], 'target_iqns': ['target_1'], 'target_luns': [1]}
    ...
    def test_get_connection_devices(self, nodes_mock, sessions_mock, glob_mock, iql_mock):
        ...
        iql_mock.return_value = self.connector._get_all_targets(self.connection_data)
        sessions_mock.return_value = [('iser:', '0', 'ip:port', '1', 'target_1')]
        nodes_mock.return_value = [('ip:port', 'target_1')]
        sys_cls = '/sys/class/scsi_host/host'
        glob_mock.side_effect = [[sys_cls + '1/device/session/target/1:1:1:1/block/sda']]
        res = self.connector._get_connection_devices(self.connection_data)
        expected = {('ip:port', 'target_1'): ({'sda'}, set())}
        self.assertDictEqual(expected, res)
        iql_mock.assert_called_once_with(self.connection_data, discover=False, is_disconnect_call=False)
