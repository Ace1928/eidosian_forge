...
class MlflowDeploymentClient(BaseDeploymentClient):
    ...
    def get_endpoint(self, endpoint) -> 'Endpoint':
        ...
        return Endpoint(**{**response, 'endpoint_url': resolve_endpoint_url(self.target_uri, response['endpoint_url'])})

    def _list_endpoints(self, page_token=None) -> 'PagedList[Endpoint]':
        params = None if page_token is None else {'page_token': page_token}
        response_json = self._call_endpoint('GET', MLFLOW_DEPLOYMENTS_CRUD_ENDPOINT_BASE, json_body=params)
        routes = [Endpoint(**{**resp, 'endpoint_url': resolve_endpoint_url(self.target_uri, resp['endpoint_url'])}) for resp in response_json.get('endpoints', [])]
    ...
    def predict(self, deployment_name=None, inputs=None, endpoint=None) -> Dict[str, Any]:
        ...
        """
        query_route = join_paths(MLFLOW_DEPLOYMENTS_ENDPOINTS_BASE, endpoint, MLFLOW_DEPLOYMENTS_QUERY_SUFFIX)
        try:
            return self._call_endpoint('POST', query_route, inputs, MLFLOW_DEPLOYMENT_PREDICT_TIMEOUT.get())
        except MlflowException as e:
            if isinstance(e.__cause__, requests.exceptions.Timeout):
                raise MlflowException(message=f'The provider has timed out while generating a response to your query. Please evaluate the available parameters for the query that you are submitting. Some parameter values and inputs can increase the computation time beyond the allowable route timeout of {MLFLOW_DEPLOYMENT_PREDICT_TIMEOUT} seconds.', error_code=BAD_REQUEST)
            raise e