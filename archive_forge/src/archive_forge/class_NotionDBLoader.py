...
class NotionDBLoader(BaseLoader):
    ...
    def __init__(self, integration_token: str, database_id: str, request_timeout_sec: Optional[int]=10, *, filter_object: Optional[Dict[str, Any]]=None) -> None:
        ...
        self.headers = {'Authorization': 'Bearer ' + self.token, 'Content-Type': 'application/json', 'Notion-Version': '2022-06-28'}
    ...
    def _request(self, url: str, method: str='GET', query_dict: Dict[str, Any]={}, *, filter_object: Optional[Dict[str, Any]]=None) -> Any:
        json_payload = query_dict.copy()
        if filter_object:
            json_payload['filter'] = filter_object
        res = requests.request(method, url, headers=self.headers, json=json_payload, timeout=self.request_timeout_sec)
