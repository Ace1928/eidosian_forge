...
class TestQosPolicy(base.TestCase):
    ...
    mock_policy = {'id': policy_id, 'name': policy_name, 'description': '', 'rules': [], 'project_id': project_id, 'tenant_id': project_id, 'shared': False, 'is_default': False, 'tags': []}
    ...
    qos_default_extension = {'updated': '2017-041-06T10:00:00-00:00', 'name': 'QoS default policy', 'links': [], 'alias': 'qos-default', 'description': 'Expose the QoS default policy per project'}
    enabled_neutron_extensions = [qos_extension, qos_default_extension]

    def _compare_policies(self, exp, real):
        self.assertDictEqual(_policy.QoSPolicy(**exp).to_dict(computed=False), real.to_dict(computed=False))
    ...
    def test_create_qos_policy_no_qos_extension(self):
        self.register_uris([dict(method='GET', uri=self.get_mock_url('network', 'public', append=['v2.0', 'extensions']), json={'extensions': []})])
        self.assertRaises(exceptions.SDKException, self.cloud.create_qos_policy, name=self.policy_name)
        self.assert_calls()
    ...
    def test_update_qos_policy_no_qos_extension(self):
        self.register_uris([dict(method='GET', uri=self.get_mock_url('network', 'public', append=['v2.0', 'extensions']), json={'extensions': []})])
        self.assertRaises(exceptions.SDKException, self.cloud.update_qos_policy, self.policy_id, name='goofy')
        self.assert_calls()
