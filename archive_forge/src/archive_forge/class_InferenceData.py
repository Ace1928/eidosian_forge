...
class InferenceData(Mapping[str, xr.Dataset]):
    ...
    def __init__(self, attrs: Union[None, Mapping[Any, Any]]=None, **kwargs: Union[xr.Dataset, List[xr.Dataset], Tuple[xr.Dataset, xr.Dataset]]) -> None:
        ...
        for key in kwargs:
            if key not in SUPPORTED_GROUPS_ALL:
                key_list.append(key)
                warnings.warn(f'{key} group is not defined in the InferenceData scheme', UserWarning)
        for key in key_list:
            dataset = kwargs[key]
            dataset_warmup = None
            if dataset is None:
                ...
                raise ValueError(f"Arguments to InferenceData must be xarray Datasets (argument '{key}' was type '{type(dataset)}')")
    ...
    def _repr_html_(self) -> str:
        ...
        try:
            ...
            if display_style == 'text':
                ...
                elements = ''.join([HtmlTemplate.element_template.format(group_id=group + str(uuid.uuid4()), group=group, xr_data=getattr(self, group)._repr_html_()) for group in self._groups_all])
                formatted_html_template = HtmlTemplate.html_template.format(elements)
    ...
    @staticmethod
    def from_netcdf(filename, *, engine='h5netcdf', group_kwargs=None, regex=False, base_group: str='/') -> 'InferenceData':
        ...
        try:
            with h5netcdf.File(filename, mode='r') if engine == 'h5netcdf' else nc.Dataset(filename, mode='r') as file_handle:
                if base_group == '/':
                    data = file_handle
                else:
            ...
            if err.errno == -101:
                raise type(err)(str(err) + ' while reading a NetCDF file. This is probably an error in HDF5, which happens because your OS does not support HDF5 file locking.  See https://stackoverflow.com/questions/49317927/errno-101-netcdf-hdf-error-when-opening-netcdf-file#49317928 for a possible solution.') from err
            raise err

    def to_netcdf(self, filename: str, compress: bool=True, groups: Optional[List[str]]=None, engine: str='h5netcdf', base_group: str='/', overwrite_existing: bool=True) -> str:
        ...
        if self._groups_all:
            ...
            for group in groups:
                ...
                if compress:
                    kwargs['encoding'] = {var_name: {'zlib': True} for var_name, values in data.variables.items() if _compressible_dtype(values.dtype)}
    ...
    def to_datatree(self):
        ...
        try:
            ...
            raise ModuleNotFoundError('datatree must be installed in order to use InferenceData.to_datatree') from err
            ...
            for group in groups:
                ...
                for var_name, dataarray in dataset.items():
                    ...
                    for coord_name, coord_values in dataarray.coords.items():
                        if coord_name not in ('chain', 'draw') and (not coord_name.startswith(f'{var_name}_dim_')):
    ...
    def to_json(self, filename, groups=None, filter_groups=None, **kwargs):
        ...
        """
        idata_dict = _make_json_serializable(self.to_dict(groups=groups, filter_groups=filter_groups))
        with open(filename, 'w', encoding='utf8') as file:
            json.dump(idata_dict, file, **kwargs)
        return filename

    def to_dataframe(self, groups=None, filter_groups=None, var_names=None, filter_vars=None, include_coords=True, include_index=True, index_origin=None):
        ...
        group_names = list(filter(lambda x: 'data' not in x, self._group_names(groups, filter_groups)))
        if not group_names:
        ...
        for group in group_names:
            ...
            df = None
            coords_to_idx = {name: dict(map(reversed, enumerate(dataset.coords[name].values, index_origin))) for name in list(filter(lambda x: x not in ('chain', 'draw'), dataset.coords))}
            for data_array in dataset.values():
                ...
                if list(filter(lambda x: x not in ('chain', 'draw'), data_array.dims)):
                    levels = [idx for idx, dim in enumerate(data_array.dims) if dim not in ('chain', 'draw')]
                    ...
                    for name, *coords in dataframe.columns:
                        if include_index:
                            ...
                            if include_coords:
                                tuple_columns.append((f'{name}[{','.join(map(str, idxs))}]', *coords))
                            else:
        ...
        if len(dfs) > 1:
            for group, df in dfs.items():
                df.columns = [col if col in ('draw', 'chain') else (group, *col) if isinstance(col, tuple) else (group, col) for col in df.columns]
    ...
    def sel(self: InferenceDataT, groups: Optional[Union[str, List[str]]]=None, filter_groups: Optional["Literal['like', 'regex']"]=None, inplace: bool=False, chain_prior: Optional[bool]=None, **kwargs: Any) -> Optional[InferenceDataT]:
        ...
        if chain_prior is not None:
            warnings.warn("chain_prior has been deprecated. Use groups argument and rcParams['data.metagroups'] instead.", DeprecationWarning)
        else:
            chain_prior = False
    ...
    def isel(self: InferenceDataT, groups: Optional[Union[str, List[str]]]=None, filter_groups: Optional["Literal['like', 'regex']"]=None, inplace: bool=False, **kwargs: Any) -> Optional[InferenceDataT]:
        """Perform an xarray selection on all groups.
    ...
    def stack(self, dimensions=None, groups=None, filter_groups=None, inplace=False, **kwargs):
    ...
    def extend(self, other, join='left'):
        ...
        for group in other._groups_all:
            if hasattr(self, group) and join == 'left':
                continue
            if group not in SUPPORTED_GROUPS_ALL:
                warnings.warn(f'{group} group is not defined in the InferenceData scheme', UserWarning)
