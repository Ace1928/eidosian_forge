...
class RadialHeatMapPlot(CompositeElementPlot, ColorbarPlot):
    ...
    _style_groups = {'annular_wedge': 'annular', 'text': 'ticks', 'multi_line': 'xmarks', 'arc': 'ymarks'}
    _draw_order = ['annular_wedge', 'multi_line', 'arc', 'text']
    style_opts = ['xmarks_' + p for p in base_properties + line_properties] + ['ymarks_' + p for p in base_properties + line_properties] + ['annular_' + p for p in base_properties + fill_properties + line_properties] + ['ticks_' + p for p in text_properties] + ['cmap']
    ...
    def get_default_mapping(self, z, cmapper):
        ...
        """
        map_annular = dict(x=self.max_radius, y=self.max_radius, inner_radius='inner_radius', outer_radius='outer_radius', start_angle='start_angle', end_angle='end_angle', fill_color={'field': z, 'transform': cmapper})
        map_seg_label = dict(x='x', y='y', text='text', angle='angle', text_align='center')
        map_ann_label = dict(x='x', y='y', text='text', angle='angle', text_align='center', text_baseline='bottom')
        map_xmarks = dict(xs='xs', ys='ys')
        map_ymarks = dict(x=self.max_radius, y=self.max_radius, start_angle=0, end_angle=2 * np.pi, radius='radius')
        return {'annular_wedge_1': map_annular, 'text_1': map_seg_label, 'text_2': map_ann_label, 'multi_line_1': map_xmarks, 'arc_1': map_ymarks}
    ...
    def get_data(self, element, ranges, style):
        ...
        data_annular = {'start_angle': start_angle, 'end_angle': end_angle, 'inner_radius': inner_radius, 'outer_radius': outer_radius, z: zvals, x: xvals, y: yvals}
        if 'hover' in self.handles:
            for vdim in element.vdims:
                sanitized = dimension_sanitizer(vdim.name)
                values = ['-' if is_nan(v) else vdim.pprint_value(v) for v in aggregate.dimension_values(vdim)]
        ...
        data = {'annular_wedge_1': data_annular, 'text_1': data_text_seg, 'text_2': data_text_ann, 'multi_line_1': data_xmarks, 'arc_1': data_ymarks}
