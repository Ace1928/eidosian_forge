...
class CallbackResult:
    ...
    generated_constraints: List[GeneratedConstraint] = dataclasses.field(default_factory=list)
    suggested_solutions: List[Mapping[model.Variable, float]] = dataclasses.field(default_factory=list)
    ...
    def add_lazy_constraint(self, bounded_expr: Optional[Union[bool, model.BoundedLinearTypes]]=None, *, lb: Optional[float]=None, ub: Optional[float]=None, expr: Optional[model.LinearTypes]=None) -> None:
        """Shortcut for add_generated_constraint(..., is_lazy=True).."""
        self.add_generated_constraint(bounded_expr, lb=lb, ub=ub, expr=expr, is_lazy=True)
    ...
    def to_proto(self) -> callback_pb2.CallbackResultProto:
        ...
        for suggested_solution in self.suggested_solutions:
            result.suggested_solutions.add().CopyFrom(sparse_containers.to_sparse_double_vector_proto(suggested_solution))
        return result