...
class Tigris(VectorStore):
    ...
    def similarity_search(self, query: str, k: int=4, filter: Optional[TigrisFilter]=None, **kwargs: Any) -> List[Document]:
        """Return docs most similar to query."""
        docs_with_scores = self.similarity_search_with_score(query, k, filter)
        return [doc for doc, _ in docs_with_scores]

    def similarity_search_with_score(self, query: str, k: int=4, filter: Optional[TigrisFilter]=None) -> List[Tuple[Document, float]]:
        ...
        for r in result:
            docs.append((Document(page_content=r.doc['text'], metadata=r.doc.get('metadata')), r.score))
        return docs

    @classmethod
    def from_texts(cls, texts: List[str], embedding: Embeddings, metadatas: Optional[List[dict]]=None, ids: Optional[List[str]]=None, client: Optional[TigrisClient]=None, index_name: Optional[str]=None, **kwargs: Any) -> Tigris:
