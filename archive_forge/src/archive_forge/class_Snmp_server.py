...
class Snmp_server(ResourceModule):
    ...
    def __init__(self, module):
        super(Snmp_server, self).__init__(empty_fact_val={}, facts_module=Facts(module), module=module, resource='snmp_server', tmplt=Snmp_serverTemplate())
        self.parsers = ['contact', 'description', 'location', 'smux_peer', 'trap_source', 'trap_target']
    ...
    def _compare_lists(self, want, have):
        parsers = ['communities', 'listen_addresses']
                ...
                if attrib == 'communities':
                    for k, v in iteritems(entry):
                        ...
                        if k != 'name':
                            ...
                            self.compare(parsers='communities', want={'communities': {k: v, 'name': key}}, have=h)
                else:
                    self.compare(parsers='listen_addresses', want={'listen_addresses': entry}, have={'listen_addresses': hdict.pop(key, {})})
                have.pop(attrib, {})
            for key, entry in iteritems(hdict):
                if attrib == 'communities':
                    for k, v in iteritems(entry):
                        if k != 'name':
                            self.compare(parsers='communities', have={'communities': {k: v, 'name': key}}, want={})
                else:
