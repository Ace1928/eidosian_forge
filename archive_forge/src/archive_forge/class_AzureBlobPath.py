...
class AzureBlobPath(CloudPath):
    ...
    def stat(self):
        try:
            ...
            raise NoStatError(f'No stats available for {self}; it may be a directory or not exist.')
        return os.stat_result((None, None, self.cloud_prefix, None, None, None, meta.get('size', 0), None, meta.get('last_modified', 0).timestamp(), None))
