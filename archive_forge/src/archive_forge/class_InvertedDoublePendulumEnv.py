...
class InvertedDoublePendulumEnv(MujocoEnv, utils.EzPickle):
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def __init__(self, **kwargs):
        ...
        MujocoEnv.__init__(self, 'inverted_double_pendulum.xml', 5, observation_space=observation_space, **kwargs)
    ...
    def _get_obs(self):
        return np.concatenate([self.data.qpos[:1], np.sin(self.data.qpos[1:]), np.cos(self.data.qpos[1:]), np.clip(self.data.qvel, -10, 10), np.clip(self.data.qfrc_constraint, -10, 10)]).ravel()

    def reset_model(self):
        self.set_state(self.init_qpos + self.np_random.uniform(low=-0.1, high=0.1, size=self.model.nq), self.init_qvel + self.np_random.standard_normal(self.model.nv) * 0.1)
