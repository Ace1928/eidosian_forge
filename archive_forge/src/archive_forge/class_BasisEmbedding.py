...
class BasisEmbedding(Operation):
    ...
    def __init__(self, features, wires, id=None):
        ...
        if qml.math.shape(features) == ():
            if not tracing and features >= 2 ** len(wires):
                raise ValueError(f'Features must be of length {len(wires)}, got features={features} which is >= {2 ** len(wires)}')
        ...
        if n_features != len(wires):
            raise ValueError(f'Features must be of length {len(wires)}; got length {n_features} (features={features}).')
        if not tracing:
    ...
    @staticmethod
    def compute_decomposition(wires, basis_state):
