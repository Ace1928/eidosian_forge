...
class SlackGetMessage(SlackBaseTool):
    ...
    def _run(self, channel_id: str, run_manager: Optional[CallbackManagerForToolRun]=None) -> str:
        logging.getLogger(__name__)
        try:
            result = self.client.conversations_history(channel=channel_id)
            messages = result['messages']
            filtered_messages = [{key: message[key] for key in ('user', 'text', 'ts')} for message in messages if 'user' in message and 'text' in message and ('ts' in message)]
