...
def optimize_dataset_v2(input_dataset: _atypes.TensorFuzzingAnnotation[_atypes.Variant], optimizations_enabled: _atypes.TensorFuzzingAnnotation[_atypes.String], optimizations_disabled: _atypes.TensorFuzzingAnnotation[_atypes.String], optimizations_default: _atypes.TensorFuzzingAnnotation[_atypes.String], output_types, output_shapes, optimization_configs=[], name=None) -> _atypes.TensorFuzzingAnnotation[_atypes.Variant]:
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'OptimizeDatasetV2', name, input_dataset, optimizations_enabled, optimizations_disabled, optimizations_default, 'output_types', output_types, 'output_shapes', output_shapes, 'optimization_configs', optimization_configs)
            return _result
            ...
            pass
        try:
            return optimize_dataset_v2_eager_fallback(input_dataset, optimizations_enabled, optimizations_disabled, optimizations_default, output_types=output_types, output_shapes=output_shapes, optimization_configs=optimization_configs, name=name, ctx=_ctx)
