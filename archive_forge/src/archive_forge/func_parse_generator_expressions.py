...
import typing as T
def parse_generator_expressions(raw: str, trace: 'CMakeTraceParser', *, context_tgt: T.Optional['CMakeTarget']=None) -> str:
    ...
    def target_file(arg: str) -> str:
        ...
        return ''
    supported: T.Dict[str, T.Callable[[str], str]] = {'BOOL': lambda x: '0' if x.upper() in {'', '0', 'FALSE', 'OFF', 'N', 'NO', 'IGNORE', 'NOTFOUND'} or x.endswith('-NOTFOUND') else '1', 'AND': lambda x: '1' if all((y == '1' for y in x.split(','))) else '0', 'OR': lambda x: '1' if any((y == '1' for y in x.split(','))) else '0', 'NOT': lambda x: '0' if x == '1' else '1', 'IF': lambda x: x.split(',')[1] if x.split(',')[0] == '1' else x.split(',')[2], '0': lambda x: '', '1': lambda x: x, 'STREQUAL': equal, 'EQUAL': equal, 'VERSION_LESS': lambda x: vers_comp('<', x), 'VERSION_GREATER': lambda x: vers_comp('>', x), 'VERSION_EQUAL': lambda x: vers_comp('=', x), 'VERSION_LESS_EQUAL': lambda x: vers_comp('<=', x), 'VERSION_GREATER_EQUAL': lambda x: vers_comp('>=', x), 'LOWER_CASE': lambda x: x.lower(), 'UPPER_CASE': lambda x: x.upper(), 'INSTALL_INTERFACE': lambda x: '', 'BUILD_INTERFACE': lambda x: x, 'ANGLE-R': lambda x: '>', 'COMMA': lambda x: ',', 'SEMICOLON': lambda x: ';', 'TARGET_EXISTS': lambda x: '1' if x in trace.targets else '0', 'TARGET_NAME_IF_EXISTS': lambda x: x if x in trace.targets else '', 'TARGET_PROPERTY': target_property, 'TARGET_FILE': target_file}
