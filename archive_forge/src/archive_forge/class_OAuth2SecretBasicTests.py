...
class OAuth2SecretBasicTests(test_v3.OAuth2RestfulTestCase):
    ...
    APP_CRED_DELETE_URL = '/users/%(user_id)s/application_credentials/%(app_cred_id)s'
    APP_CRED_SHOW_URL = '/users/%(user_id)s/application_credentials/%(app_cred_id)s'
    ACCESS_TOKEN_URL = '/OS-OAUTH2/token'

    def setUp(self):
        super(OAuth2SecretBasicTests, self).setUp()
        log.set_defaults(logging_context_format_string='%(asctime)s.%(msecs)03d %(color)s%(levelname)s %(name)s [^[[01;36m%(request_id)s ^[[00;36m%(project_name)s %(user_name)s%(color)s] ^[[01;35m%(instance)s%(color)s%(message)s^[[00m', default_log_levels=log.DEBUG)
        ...
        self.config_fixture.config(group='oauth2', oauth2_authn_methods=['client_secret_basic'])
    ...
    def _get_access_token(self, app_cred, b64str, headers, data, expected_status):
        if b64str is None:
        ...
        resp = self.post(self.ACCESS_TOKEN_URL, headers=headers, convert=False, body=data, expected_status=expected_status)
        return resp
    ...
    def test_get_access_token_failed_400(self):
        ...
        data = parse.urlencode(data).encode()
        with mock.patch('keystone.api._shared.authentication.authenticate_for_token') as co_mock:
            co_mock.side_effect = exception.ValidationError('Auth method is invalid')
            resp = self.post(self.ACCESS_TOKEN_URL, headers=headers, convert=False, body=data, noauth=True, expected_status=client.BAD_REQUEST)
            LOG.debug(f'response: {resp}')
            json_resp = jsonutils.loads(resp.body)
            self.assertEqual(error, json_resp.get('error'))
            LOG.debug(f'error: {json_resp.get('error')}')
    ...
    def test_get_access_token_method_patch_not_allowed(self):
        ...
        client_name = 'client_name_test'
        app_cred = self._create_app_cred(self.user_id, client_name)
        json_resp = self._get_access_token_method_not_allowed(app_cred, self.patch)
        self.assertEqual('other_error', json_resp.get('error'))
        self.assertEqual('The method is not allowed for the requested URL.', json_resp.get('error_description'))
    ...
    def test_get_access_token_method_put_not_allowed(self):
        ...
        client_name = 'client_name_test'
        app_cred = self._create_app_cred(self.user_id, client_name)
        json_resp = self._get_access_token_method_not_allowed(app_cred, self.put)
        self.assertEqual('other_error', json_resp.get('error'))
        self.assertEqual('The method is not allowed for the requested URL.', json_resp.get('error_description'))
    ...
    def test_get_access_token_method_delete_not_allowed(self):
        ...
        client_name = 'client_name_test'
        app_cred = self._create_app_cred(self.user_id, client_name)
        json_resp = self._get_access_token_method_not_allowed(app_cred, self.delete)
        self.assertEqual('other_error', json_resp.get('error'))
        self.assertEqual('The method is not allowed for the requested URL.', json_resp.get('error_description'))
