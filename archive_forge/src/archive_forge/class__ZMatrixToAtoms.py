...
class _ZMatrixToAtoms:
    known_units = dict(distance={'angstrom': Angstrom, 'bohr': Bohr, 'au': Bohr, 'nm': nm}, angle={'radians': 1.0, 'degrees': np.pi / 180})

    def __init__(self, dconv: Union[str, Real], aconv: Union[str, Real], defs: Optional[Union[Dict[str, float], str, List[str]]]=None) -> None:
    ...
    def get_var(self, val: str) -> float:
        try:
            ...
            if val_out is None:
                raise ValueError('Invalid value encountered in Z-matrix: {}'.format(val)) from e
    ...
    def get_index(self, name: str) -> int:
        ...
        try:
            ...
            if self.name_to_index is None or name not in self.name_to_index:
                raise ValueError('Failed to determine index for name "{}"'.format(name)) from e
    ...
    def validate_indices(self, *indices: int) -> None:
        ...
        if any(np.array(indices) >= self.nrows):
            raise ValueError("An invalid Z-matrix was provided! Row {} refers to atom indices {}, at least one of which hasn't been defined yet!".format(self.nrows, indices))
        if len(indices) != len(set(indices)):
            raise ValueError('An atom index has been used more than once a row of the Z-matrix! Row numbers {}, referred indices: {}'.format(self.nrows, indices))

    def parse_row(self, row: str) -> Tuple[str, Union[_ZMatrixRow, Tuple[float, float, float]]]:
        tokens = row.split()
        name = tokens[0]
        self.set_index(name)
        if len(tokens) == 1:
    ...
    def add_row(self, row: str) -> None:
        ...
        ax3 = ax2 * np.cos(zrow.a_dihedral) + np.cross(ax2, ax1) * np.sin(zrow.a_dihedral)
