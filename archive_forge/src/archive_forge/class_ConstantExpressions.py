...
class ConstantExpressions(NodeAnalysis):
    ...
    def __init__(self):
        self.result = set()
        super(ConstantExpressions, self).__init__(Globals, Locals, Aliases, PureFunctions)
    ...
    def visit_Name(self, node):
        if node in self.aliases:
            ...
            def is_function(x):
                return isinstance(x, (FunctionIntr, ast.FunctionDef, ast.alias))
            pure_fun = all((alias in self.pure_functions for alias in self.aliases[node]))
            return pure_fun
        else:
            return False
    ...
    def visit_Slice(self, node):
        return all([x is None or self.visit(x) for x in (node.lower, node.upper, node.step)]) and self.add(node)
