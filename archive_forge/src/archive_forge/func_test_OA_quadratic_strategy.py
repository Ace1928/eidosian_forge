...
@unittest.skipUnless(SolverFactory('cplex').available() or SolverFactory('gurobi').available(), 'CPLEX or Gurobi not available.')
def test_OA_quadratic_strategy(self):
    ...
    with SolverFactory('mindtpy') as opt:
        ...
        for quadratic_strategy in (0, 1, 2):
            results = opt.solve(model, strategy='OA', mip_solver=mip_solver, nlp_solver=required_solvers[0], quadratic_strategy=quadratic_strategy)
