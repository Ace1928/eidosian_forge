...
class EntityLinker(TrainablePipe):
    ...
    def __init__(self, vocab: Vocab, model: Model, name: str='entity_linker', *, labels_discard: Iterable[str], n_sents: int, incl_prior: bool, incl_context: bool, entity_vector_length: int, get_candidates: Callable[[KnowledgeBase, Span], Iterable[Candidate]], get_candidates_batch: Callable[[KnowledgeBase, Iterable[Span]], Iterable[Iterable[Candidate]]], generate_empty_kb: Callable[[Vocab, int], KnowledgeBase], overwrite: bool=BACKWARD_OVERWRITE, scorer: Optional[Callable]=entity_linker_score, use_gold_ents: bool, candidates_batch_size: int, threshold: Optional[float]=None) -> None:
        ...
        if threshold is not None and (not 0 <= threshold <= 1):
            raise ValueError(Errors.E1043.format(range_start=0, range_end=1, value=threshold))
        self.vocab = vocab
        self.model = model
        self.name = name
    ...
    def predict(self, docs: Iterable[Doc]) -> List[str]:
        ...
        for i, doc in enumerate(docs):
            ...
            for ent_idx in range(0, len(doc.ents), self.candidates_batch_size):
                ...
                valid_ent_idx = [idx for idx in range(len(ent_batch)) if ent_batch[idx].label_ not in self.labels_discard]
                batch_candidates = list(self.get_candidates_batch(self.kb, [ent_batch[idx] for idx in valid_ent_idx]) if self.candidates_batch_size > 1 else [self.get_candidates(self.kb, ent_batch[idx]) for idx in valid_ent_idx])
                for j, ent in enumerate(ent_batch):
                    ...
                    if ent.label_ in self.labels_discard:
                        ...
                        if not candidates:
                            ...
                            final_kb_ids.append(candidates[scores.argmax().item()].entity_ if self.threshold is None or scores.max() >= self.threshold else EntityLinker.NIL)
    ...
    def from_disk(self, path: Union[str, Path], *, exclude: Iterable[str]=SimpleFrozenList()) -> 'EntityLinker':
