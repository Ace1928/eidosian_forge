...
def quantized_relu6(features: _atypes.TensorFuzzingAnnotation[TV_QuantizedRelu6_Tinput], min_features: _atypes.TensorFuzzingAnnotation[_atypes.Float32], max_features: _atypes.TensorFuzzingAnnotation[_atypes.Float32], out_type: TV_QuantizedRelu6_out_type=_dtypes.quint8, name=None):
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'QuantizedRelu6', name, features, min_features, max_features, 'out_type', out_type)
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('QuantizedRelu6', features=features, min_features=min_features, max_features=max_features, out_type=out_type, name=name)
    _result = _outputs[:]
