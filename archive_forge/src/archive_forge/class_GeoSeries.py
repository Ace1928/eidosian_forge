...
class GeoSeries(GeoPandasBase, Series):
    ...
    def __init__(self, data=None, index=None, crs: Optional[Any]=None, **kwargs):
        if hasattr(data, 'crs') and crs:
            if not data.crs:
                ...
                raise ValueError("CRS mismatch between CRS of the passed geometries and 'crs'. Use 'GeoSeries.set_crs(crs, allow_override=True)' to overwrite CRS or 'GeoSeries.to_crs(crs)' to reproject geometries. ")
        if isinstance(data, SingleBlockManager):
            if not isinstance(data.blocks[0].dtype, GeometryDtype):
                raise TypeError(f"Non geometry data passed to GeoSeries constructor, received data of dtype '{data.blocks[0].dtype}'")
        ...
        if not is_geometry_type(data):
            ...
            if s.dtype != object:
                if s.empty and s.dtype == 'float64' or data is None:
                    ...
                    raise TypeError(f"Non geometry data passed to GeoSeries constructor, received data of dtype '{s.dtype}'")
            try:
    ...
    @classmethod
    def from_wkb(cls, data, index=None, crs: Optional[Any]=None, **kwargs) -> GeoSeries:
        """
    ...
    @classmethod
    def from_wkt(cls, data, index=None, crs: Optional[Any]=None, **kwargs) -> GeoSeries:
        """
        Alternate constructor to create a ``GeoSeries``
    ...
    def to_file(self, filename: os.PathLike | typing.IO, driver: Optional[str]=None, index: Optional[bool]=None, **kwargs):
    ...
    def notna(self) -> Series:
        ...
        if self.is_empty.any():
            warnings.warn("GeoSeries.notna() previously returned False for both missing (None) and empty geometries. Now, it only returns False for missing values. Since the calling GeoSeries contains empty geometries, the result has changed compared to previous versions of GeoPandas.\nGiven a GeoSeries 's', you can use '~s.is_empty & s.notna()' to get back the old behaviour.\n\nTo further ignore this warning, you can do: \nimport warnings; warnings.filterwarnings('ignore', 'GeoSeries.notna', UserWarning)", UserWarning, stacklevel=2)
        return super().notna()
    ...
    def explode(self, ignore_index=False, index_parts=None) -> GeoSeries:
        ...
        if compat.USE_SHAPELY_20 or (compat.USE_PYGEOS and compat.PYGEOS_GE_09):
            if compat.USE_SHAPELY_20:
                ...
                import pygeos
                geometries, outer_idx = pygeos.get_parts(self.values._data, return_index=True)
            index = _get_index_for_parts(self.index, outer_idx, ignore_index=ignore_index, index_parts=index_parts)
