...
class FSArray(Sequence):
    ...
    def __init__(self, num_rows: int, num_columns: int, *args: Any, **kwargs: Any) -> None:
    ...
    @overload
    def __getitem__(self, slicetuple: Tuple[Union[slice, int], Union[slice, int]]) -> List[FmtStr]:
        pass

    def __getitem__(self, slicetuple: Union[int, slice, Tuple[Union[int, slice], Union[int, slice]]]) -> Union[FmtStr, List[FmtStr]]:
    ...
    def __setitem__(self, slicetuple, value):
        ...
        if isinstance(slicetuple, slice):
            ...
            if isinstance(value, str):
                raise ValueError('if slice is 2D, value must be 2D as in of list type []')
        ...
        self.rows.extend([fmtstr('', *self.saved_args, **self.saved_kwargs) for _ in range(additional_rows)])
        ...
        if slicesize(colslice) > 1 and isinstance(value, str):
            raise ValueError('You cannot replace a multi column slice with a \n                string please use a list [] with strings for the \n                contents of each row')
        if slicesize(rowslice) != len(value):
            ...
            grid_fsarray = self.rows[:rowslice.start] + [fs.setslice_with_length(colslice.start, colslice.stop, v, self.num_columns) for fs, v in zip(self.rows[rowslice], grid_value)] + self.rows[rowslice.stop:]
            msg = 'You are trying to fit this value {} into the region {}: {}'.format(fmtstr(''.join(value), bg='cyan'), fmtstr('').join(grid_value), '\n '.join((grid_fsarray[x] for x in range(len(self.rows)))))
            raise ValueError('Error you are trying to replace a region of {} rows by {}\n                columns for and area of {} with a value of len {}. The value\n                used to replace the region must equal the area of the region\n                replace.\n                {}'.format(rowslice.stop - rowslice.start, colslice.stop - colslice.start, area, val_len, msg))
        self.rows = self.rows[:rowslice.start] + [fs.setslice_with_length(colslice.start, colslice.stop, v, self.num_columns) for fs, v in zip(self.rows[rowslice], value)] + self.rows[rowslice.stop:]
    ...
    def diff(cls, a: 'FSArray', b: 'FSArray', ignore_formatting: bool=False) -> str:
        ...
        return '\n'.join((f'{a_line} {a_len:3d} | {b_len:3d} {b_line}' for a_line, b_line, a_len, b_len in zip(a_rows, b_rows, a_lengths, b_lengths)))