...
@pytest.mark.skipif(Engine.get() != 'Ray', reason='Only Ray supports the Batch Pipeline API')
class TestPipelineRayEngine:
    ...
    def test_warnings(self):
        ...
        with pytest.warns(UserWarning, match='No outputs to compute. Returning an empty list. Please specify outputs by calling `add_query` with `is_output=True`.'):
    ...
    def test_postprocessing_with_partition_id(self):
        ...
        correct_modin_frame = correct_df._query_compiler._modin_frame
        partitions = correct_modin_frame._partition_mgr_cls.row_partitions(correct_modin_frame._partitions)
        partitions = [partition.add_to_apply_calls(new_col_adder, i) for i, partition in enumerate(partitions)]
        [partition.drain_call_queue() for partition in partitions]
        partitions = [partition.list_of_blocks for partition in partitions]
        correct_df = from_partitions(partitions, axis=None)
        df_equals(correct_df, new_dfs[20])
    ...
    def test_repartition_after(self):
        ...
        with pytest.raises(NotImplementedError, match='Dynamic repartitioning is currently only supported for DataFrames with 1 partition.'):
