...
@due.dcite(Doi('10.1021/acs.chemmater.5b04082'), description='Interface stability in solid-state batteries')
@due.dcite(Doi('10.1038/s41578-019-0157-5'), description='Understanding interface stability in solid-state batteries')
class InterfacialReactivity(MSONable):
    ...
    def __init__(self, c1: Composition, c2: Composition, pd: PhaseDiagram, norm: bool=True, use_hull_energy: bool=False, **kwargs):
        """
        Args:
        ...
        if isinstance(pd, GrandPotentialPhaseDiagram) and (not bypass_grand_warning):
            raise ValueError('Please use the GrandPotentialInterfacialReactivity class for interfacial reactions with open elements!')
        self.c1 = c1
        self.c2 = c2
        self.pd = pd
        self.norm = norm
    ...
    def get_dataframe(self) -> DataFrame:
        ...
        get_kinks() method.
        """
        rxns = [{'Atomic fraction': round(ratio, 3), 'Reaction': rxn, 'E$_{\textrm{rxn}}$ (kJ/mol)': round(rxn_energy, 1), 'E$_{\textrm{rxn}}$ (eV/atom)': round(reactivity, 3)} for _, ratio, reactivity, rxn, rxn_energy in self.get_kinks()]
    ...
    def labels(self):
        ...
        """
        return {j: f'x= {x:.4} energy in eV/atom = {energy:.4} {reaction}' for j, x, energy, reaction, _ in self.get_kinks()}
