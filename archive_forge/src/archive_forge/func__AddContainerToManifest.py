...
def _AddContainerToManifest(manifest, service_name, from_run_container):
    if len(manifest.get('spec', {}).get('template', {}).get('spec', {}).get('containers', [])) != 1:
        raise core_exceptions.Error('Number of containers in service {} is not 1.'.format(service_name))
