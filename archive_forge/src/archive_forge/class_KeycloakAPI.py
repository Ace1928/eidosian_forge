...
class KeycloakAPI(object):
    ...
    def get_realm_by_id(self, realm='master'):
        ...
        try:
            return json.loads(to_native(open_url(realm_url, method='GET', http_agent=self.http_agent, headers=self.restheaders, timeout=self.connection_timeout, validate_certs=self.validate_certs).read()))
        except HTTPError as e:
            if e.code == 404:
                return None
            else:
    ...
    def delete_realm(self, realm='master'):
        ...
        realm_url = URL_REALM.format(url=self.baseurl, realm=realm)
        try:
            return open_url(realm_url, method='DELETE', http_agent=self.http_agent, headers=self.restheaders, timeout=self.connection_timeout, validate_certs=self.validate_certs)
        except Exception as e:
            self.fail_open_url(e, msg='Could not delete realm %s: %s' % (realm, str(e)), exception=traceback.format_exc())
    ...
    def update_client(self, id, clientrep, realm='master'):
        ...
        client_url = URL_CLIENT.format(url=self.baseurl, realm=realm, id=id)
        try:
            return open_url(client_url, method='PUT', http_agent=self.http_agent, headers=self.restheaders, timeout=self.connection_timeout, data=json.dumps(clientrep), validate_certs=self.validate_certs)
        except Exception as e:
            self.fail_open_url(e, msg='Could not update client %s in realm %s: %s' % (id, realm, str(e)))
    ...
    def create_client(self, clientrep, realm='master'):
        ...
        try:
            return open_url(client_url, method='POST', http_agent=self.http_agent, headers=self.restheaders, timeout=self.connection_timeout, data=json.dumps(clientrep), validate_certs=self.validate_certs)
        except Exception as e:
            self.fail_open_url(e, msg='Could not create client %s in realm %s: %s' % (clientrep['clientId'], realm, str(e)))
    ...
    def delete_client(self, id, realm='master'):
        ...
        client_url = URL_CLIENT.format(url=self.baseurl, realm=realm, id=id)
        try:
            return open_url(client_url, method='DELETE', http_agent=self.http_agent, headers=self.restheaders, timeout=self.connection_timeout, validate_certs=self.validate_certs)
        except Exception as e:
            self.fail_open_url(e, msg='Could not delete client %s in realm %s: %s' % (id, realm, str(e)))
    ...
    def add_client_roles_by_id_composite_rolemapping(self, rid, roles_rep, realm='master'):
        ...
        try:
            open_url(available_rolemappings_url, method='POST', http_agent=self.http_agent, headers=self.restheaders, data=json.dumps(roles_rep), validate_certs=self.validate_certs, timeout=self.connection_timeout)
        except Exception as e:
            self.fail_open_url(e, msg='Could not assign roles to composite role %s and realm %s: %s' % (rid, realm, str(e)))
    ...
    def delete_group_realm_rolemapping(self, gid, role_rep, realm='master'):
        ...
        url = URL_REALM_GROUP_ROLEMAPPINGS.format(url=self.baseurl, realm=realm, group=gid)
        try:
            open_url(url, method='DELETE', http_agent=self.http_agent, headers=self.restheaders, data=json.dumps(role_rep), validate_certs=self.validate_certs, timeout=self.connection_timeout)
        except Exception as e:
            self.fail_open_url(e, msg='Could not delete realm role mappings for group %s, realm %s: %s' % (gid, realm, str(e)))
    ...
    def delete_group_rolemapping(self, gid, cid, role_rep, realm='master'):
        ...
        available_rolemappings_url = URL_CLIENT_GROUP_ROLEMAPPINGS.format(url=self.baseurl, realm=realm, id=gid, client=cid)
        try:
            open_url(available_rolemappings_url, method='DELETE', http_agent=self.http_agent, headers=self.restheaders, data=json.dumps(role_rep), validate_certs=self.validate_certs, timeout=self.connection_timeout)
        except Exception as e:
            self.fail_open_url(e, msg='Could not delete available rolemappings for client %s in group %s, realm %s: %s' % (cid, gid, realm, str(e)))
    ...
    def get_client_templates(self, realm='master'):
        ...
        try:
            return json.loads(to_native(open_url(url, method='GET', http_agent=self.http_agent, headers=self.restheaders, timeout=self.connection_timeout, validate_certs=self.validate_certs).read()))
        except ValueError as e:
            self.module.fail_json(msg='API returned incorrect JSON when trying to obtain list of client templates for realm %s: %s' % (realm, str(e)))
        except Exception as e:
            self.fail_open_url(e, msg='Could not obtain list of client templates for realm %s: %s' % (realm, str(e)))
    ...
    def get_client_template_by_id(self, id, realm='master'):
        ...
        try:
            ...
            self.module.fail_json(msg='API returned incorrect JSON when trying to obtain client templates %s for realm %s: %s' % (id, realm, str(e)))
        except Exception as e:
            self.fail_open_url(e, msg='Could not obtain client template %s for realm %s: %s' % (id, realm, str(e)))
    ...
    def get_clientscope_protocolmappers(self, cid, realm='master'):
        ...
        try:
            return json.loads(to_native(open_url(protocolmappers_url, method='GET', http_agent=self.http_agent, headers=self.restheaders, timeout=self.connection_timeout, validate_certs=self.validate_certs).read()))
        except Exception as e:
            self.fail_open_url(e, msg='Could not fetch list of protocolmappers in realm %s: %s' % (realm, str(e)))
    ...
    def get_clientscope_protocolmapper_by_protocolmapperid(self, pid, cid, realm='master'):
        ...
        try:
            return json.loads(to_native(open_url(protocolmapper_url, method='GET', http_agent=self.http_agent, headers=self.restheaders, timeout=self.connection_timeout, validate_certs=self.validate_certs).read()))
        except HTTPError as e:
            if e.code == 404:
                return None
            else:
                self.fail_open_url(e, msg='Could not fetch protocolmapper %s in realm %s: %s' % (pid, realm, str(e)))
        except Exception as e:
            self.module.fail_json(msg='Could not fetch protocolmapper %s in realm %s: %s' % (cid, realm, str(e)))
    ...
    def create_clientsecret(self, id, realm='master'):
        ...
        try:
            return json.loads(to_native(open_url(clientsecret_url, method='POST', http_agent=self.http_agent, headers=self.restheaders, timeout=self.connection_timeout, validate_certs=self.validate_certs).read()))
        except HTTPError as e:
            if e.code == 404:
                return None
            else:
    ...
    def get_groups(self, realm='master'):
        ...
        try:
            ...
            self.fail_open_url(e, msg='Could not fetch list of groups in realm %s: %s' % (realm, str(e)))
    ...
    def get_group_by_name(self, name, realm='master', parents=None):
        ...
        try:
            ...
            self.module.fail_json(msg='Could not fetch group %s in realm %s: %s' % (name, realm, str(e)))
    ...
    def create_group(self, grouprep, realm='master'):
        ...
        groups_url = URL_GROUPS.format(url=self.baseurl, realm=realm)
        try:
            return open_url(groups_url, method='POST', http_agent=self.http_agent, headers=self.restheaders, timeout=self.connection_timeout, data=json.dumps(grouprep), validate_certs=self.validate_certs)
        except Exception as e:
            self.fail_open_url(e, msg='Could not create group %s in realm %s: %s' % (grouprep['name'], realm, str(e)))
    ...
    def update_group(self, grouprep, realm='master'):
        ...
        try:
            return open_url(group_url, method='PUT', http_agent=self.http_agent, headers=self.restheaders, timeout=self.connection_timeout, data=json.dumps(grouprep), validate_certs=self.validate_certs)
        except Exception as e:
            self.fail_open_url(e, msg='Could not update group %s in realm %s: %s' % (grouprep['name'], realm, str(e)))
    ...
    def delete_group(self, name=None, groupid=None, realm='master'):
        ...
        try:
            return open_url(group_url, method='DELETE', http_agent=self.http_agent, headers=self.restheaders, timeout=self.connection_timeout, validate_certs=self.validate_certs)
        except Exception as e:
    ...
    def get_realm_role(self, name, realm='master'):
        ...
        try:
            ...
            if e.code == 404:
                ...
                self.fail_open_url(e, msg='Could not fetch role %s in realm %s: %s' % (name, realm, str(e)))
        except Exception as e:
            self.module.fail_json(msg='Could not fetch role %s in realm %s: %s' % (name, realm, str(e)))
    ...
    def get_role_composites(self, rolerep, clientid=None, realm='master'):
        ...
        try:
            ...
            return json.loads(to_native(open_url(composite_url, method='GET', http_agent=self.http_agent, headers=self.restheaders, timeout=self.connection_timeout, validate_certs=self.validate_certs).read()))
        except Exception as e:
            self.fail_open_url(e, msg='Could not get role %s composites in realm %s: %s' % (rolerep['name'], realm, str(e)))
    ...
    def create_role_composites(self, rolerep, composites, clientid=None, realm='master'):
        ...
        try:
            if clientid is not None:
                ...
                composite_url = URL_REALM_ROLE_COMPOSITES.format(url=self.baseurl, realm=realm, name=quote(rolerep['name'], safe=''))
            return open_url(composite_url, method='POST', http_agent=self.http_agent, headers=self.restheaders, timeout=self.connection_timeout, data=json.dumps(composites), validate_certs=self.validate_certs)
        except Exception as e:
    ...
    def delete_authentication_flow_by_id(self, id, realm='master'):
        ...
        try:
            return open_url(flow_url, method='DELETE', http_agent=self.http_agent, headers=self.restheaders, timeout=self.connection_timeout, validate_certs=self.validate_certs)
        except Exception as e:
            self.fail_open_url(e, msg='Could not delete authentication flow %s in realm %s: %s' % (id, realm, str(e)))
    ...
    def update_authentication_executions(self, flowAlias, updatedExec, realm='master'):
        ...
        """
        try:
            open_url(URL_AUTHENTICATION_FLOW_EXECUTIONS.format(url=self.baseurl, realm=realm, flowalias=quote(flowAlias, safe='')), method='PUT', http_agent=self.http_agent, headers=self.restheaders, data=json.dumps(updatedExec), timeout=self.connection_timeout, validate_certs=self.validate_certs)
        except HTTPError as e:
            self.fail_open_url(e, msg="Unable to update execution '%s': %s: %s %s" % (flowAlias, repr(e), ';'.join([e.url, e.msg, str(e.code), str(e.hdrs)]), str(updatedExec)))
        except Exception as e:
    ...
    def create_execution(self, execution, flowAlias, realm='master'):
        ...
        try:
            ...
            open_url(URL_AUTHENTICATION_FLOW_EXECUTIONS_EXECUTION.format(url=self.baseurl, realm=realm, flowalias=quote(flowAlias, safe='')), method='POST', http_agent=self.http_agent, headers=self.restheaders, data=json.dumps(newExec), timeout=self.connection_timeout, validate_certs=self.validate_certs)
        except HTTPError as e:
            self.fail_open_url(e, msg="Unable to create new execution '%s' %s: %s: %s %s" % (flowAlias, execution['providerId'], repr(e), ';'.join([e.url, e.msg, str(e.code), str(e.hdrs)]), str(newExec)))
        except Exception as e:
    ...
    def get_identity_provider(self, alias, realm='master'):
        ...
        try:
            return json.loads(to_native(open_url(idp_url, method='GET', http_agent=self.http_agent, headers=self.restheaders, timeout=self.connection_timeout, validate_certs=self.validate_certs).read()))
        except HTTPError as e:
            if e.code == 404:
                return None
            else:
                self.fail_open_url(e, msg='Could not fetch identity provider %s in realm %s: %s' % (alias, realm, str(e)))
        except Exception as e:
            self.module.fail_json(msg='Could not fetch identity provider %s in realm %s: %s' % (alias, realm, str(e)))
    ...
    def create_identity_provider(self, idprep, realm='master'):
        ...
        idps_url = URL_IDENTITY_PROVIDERS.format(url=self.baseurl, realm=realm)
        try:
            return open_url(idps_url, method='POST', http_agent=self.http_agent, headers=self.restheaders, timeout=self.connection_timeout, data=json.dumps(idprep), validate_certs=self.validate_certs)
        except Exception as e:
            self.fail_open_url(e, msg='Could not create identity provider %s in realm %s: %s' % (idprep['alias'], realm, str(e)))
    ...
    def update_identity_provider(self, idprep, realm='master'):
        ...
        try:
            return open_url(idp_url, method='PUT', http_agent=self.http_agent, headers=self.restheaders, timeout=self.connection_timeout, data=json.dumps(idprep), validate_certs=self.validate_certs)
        except Exception as e:
            self.fail_open_url(e, msg='Could not update identity provider %s in realm %s: %s' % (idprep['alias'], realm, str(e)))
    ...
    def delete_identity_provider(self, alias, realm='master'):
        ...
        idp_url = URL_IDENTITY_PROVIDER.format(url=self.baseurl, realm=realm, alias=alias)
        try:
            return open_url(idp_url, method='DELETE', http_agent=self.http_agent, headers=self.restheaders, timeout=self.connection_timeout, validate_certs=self.validate_certs)
        except Exception as e:
            self.fail_open_url(e, msg='Unable to delete identity provider %s in realm %s: %s' % (alias, realm, str(e)))
    ...
    def get_components(self, filter=None, realm='master'):
        ...
        if filter is not None:
            comps_url += '?%s' % filter
        try:
            return json.loads(to_native(open_url(comps_url, method='GET', http_agent=self.http_agent, headers=self.restheaders, timeout=self.connection_timeout, validate_certs=self.validate_certs).read()))
        except ValueError as e:
            self.module.fail_json(msg='API returned incorrect JSON when trying to obtain list of components for realm %s: %s' % (realm, str(e)))
        except Exception as e:
            self.fail_open_url(e, msg='Could not obtain list of components for realm %s: %s' % (realm, str(e)))
    ...
    def create_component(self, comprep, realm='master'):
        ...
        comps_url = URL_COMPONENTS.format(url=self.baseurl, realm=realm)
        try:
            resp = open_url(comps_url, method='POST', http_agent=self.http_agent, headers=self.restheaders, timeout=self.connection_timeout, data=json.dumps(comprep), validate_certs=self.validate_certs)
            comp_url = resp.getheader('Location')
            if comp_url is None:
                self.module.fail_json(msg='Could not create component in realm %s: %s' % (realm, 'unexpected response'))
            return json.loads(to_native(open_url(comp_url, method='GET', http_agent=self.http_agent, headers=self.restheaders, timeout=self.connection_timeout, validate_certs=self.validate_certs).read()))
        except Exception as e:
            self.fail_open_url(e, msg='Could not create component in realm %s: %s' % (realm, str(e)))
    ...
    def update_component(self, comprep, realm='master'):
        ...
        comp_url = URL_COMPONENT.format(url=self.baseurl, realm=realm, id=cid)
        try:
            return open_url(comp_url, method='PUT', http_agent=self.http_agent, headers=self.restheaders, timeout=self.connection_timeout, data=json.dumps(comprep), validate_certs=self.validate_certs)
        except Exception as e:
            self.fail_open_url(e, msg='Could not update component %s in realm %s: %s' % (cid, realm, str(e)))
    ...
    def get_user_by_id(self, user_id, realm='master'):
        ...
        try:
            ...
            userrep = json.load(open_url(user_url, method='GET', http_agent=self.http_agent, headers=self.restheaders, timeout=self.connection_timeout, validate_certs=self.validate_certs))
            return userrep
        except Exception as e:
            self.fail_open_url(e, msg='Could not get user %s in realm %s: %s' % (user_id, realm, str(e)))
    ...
    def create_user(self, userrep, realm='master'):
        ...
        try:
            ...
            users_url = URL_USERS.format(url=self.baseurl, realm=realm)
            open_url(users_url, method='POST', http_agent=self.http_agent, headers=self.restheaders, data=json.dumps(userrep), timeout=self.connection_timeout, validate_certs=self.validate_certs)
            created_user = self.get_user_by_username(username=userrep['username'], realm=realm)
            return created_user
        except Exception as e:
            self.fail_open_url(e, msg='Could not create user %s in realm %s: %s' % (userrep['username'], realm, str(e)))
    ...
    def update_user(self, userrep, realm='master'):
        ...
        try:
            ...
            open_url(user_url, method='PUT', http_agent=self.http_agent, headers=self.restheaders, data=json.dumps(userrep), timeout=self.connection_timeout, validate_certs=self.validate_certs)
            updated_user = self.get_user_by_id(user_id=userrep['id'], realm=realm)
            return updated_user
        except Exception as e:
            self.fail_open_url(e, msg='Could not update user %s in realm %s: %s' % (userrep['username'], realm, str(e)))
    ...
    def delete_user(self, user_id, realm='master'):
        ...
        try:
            user_url = URL_USER.format(url=self.baseurl, realm=realm, id=user_id)
            return open_url(user_url, method='DELETE', http_agent=self.http_agent, headers=self.restheaders, timeout=self.connection_timeout, validate_certs=self.validate_certs)
        except Exception as e:
            self.fail_open_url(e, msg='Could not delete user %s in realm %s: %s' % (user_id, realm, str(e)))
    ...
    def remove_user_from_group(self, user_id, group_id, realm='master'):
        ...
        try:
            user_group_url = URL_USER_GROUP.format(url=self.baseurl, realm=realm, id=user_id, group_id=group_id)
            return open_url(user_group_url, method='DELETE', http_agent=self.http_agent, headers=self.restheaders, timeout=self.connection_timeout, validate_certs=self.validate_certs)
        except Exception as e:
            self.fail_open_url(e, msg='Could not remove user %s from group %s in realm %s: %s' % (user_id, group_id, realm, str(e)))
