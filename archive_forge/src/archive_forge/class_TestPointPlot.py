...
class TestPointPlot(TestBokehPlot):
    ...
    def test_batched_points_line_color_and_color(self):
        opts = {'NdOverlay': dict(legend_limit=0), 'Points': dict(line_color=Cycle(values=['red', 'blue']))}
        overlay = NdOverlay({i: Points([(i, j) for j in range(2)]) for i in range(2)}).opts(opts)
        plot = bokeh_renderer.get_plot(overlay).subplots[()]
        line_color = np.array(['red', 'red', 'blue', 'blue'])
        fill_color = np.array(['#30a2da', '#30a2da', '#fc4f30', '#fc4f30'], dtype='<U7')
    ...
    def test_batched_points_alpha_and_color(self):
        opts = {'NdOverlay': dict(legend_limit=0), 'Points': dict(alpha=Cycle(values=[0.5, 1]))}
        overlay = NdOverlay({i: Points([(i, j) for j in range(2)]) for i in range(2)}).opts(opts)
        plot = bokeh_renderer.get_plot(overlay).subplots[()]
    ...
    def test_points_padding_logy(self):
        ...
        self.assertEqual(x_range.start, -0.2)
        self.assertEqual(x_range.end, 2.2)
        self.assertEqual(y_range.start, 0.8959584598407623)
        self.assertEqual(y_range.end, 3.348369522101713)

    def test_points_padding_datetime_square(self):
        points = Points([(np.datetime64('2016-04-0%d' % i), i) for i in range(1, 4)]).opts(padding=0.1)
        plot = bokeh_renderer.get_plot(points)
        x_range, y_range = (plot.handles['x_range'], plot.handles['y_range'])
    ...
    def test_point_color_op(self):
        points = Points([(0, 0, '#000'), (0, 1, '#F00'), (0, 2, '#0F0')], vdims='color').opts(color='color')
        plot = bokeh_renderer.get_plot(points)
        cds = plot.handles['cds']
    ...
    def test_point_fill_color_op(self):
        points = Points([(0, 0, '#000'), (0, 1, '#F00'), (0, 2, '#0F0')], vdims='color').opts(fill_color='color')
        plot = bokeh_renderer.get_plot(points)
        cds = plot.handles['cds']
        glyph = plot.handles['glyph']
    ...
    def test_point_alpha_op(self):
        points = Points([(0, 0, 0), (0, 1, 0.2), (0, 2, 0.7)], vdims='alpha').opts(alpha='alpha')
        plot = bokeh_renderer.get_plot(points)
        cds = plot.handles['cds']
        glyph = plot.handles['glyph']
    ...
    def test_point_fill_alpha_op(self):
        points = Points([(0, 0, 0), (0, 1, 0.2), (0, 2, 0.7)], vdims='alpha').opts(fill_alpha='alpha')
        plot = bokeh_renderer.get_plot(points)
        cds = plot.handles['cds']
        glyph = plot.handles['glyph']
    ...
    def test_point_size_op(self):
        points = Points([(0, 0, 1), (0, 1, 4), (0, 2, 8)], vdims='size').opts(size='size')
        plot = bokeh_renderer.get_plot(points)
        cds = plot.handles['cds']
        glyph = plot.handles['glyph']
    ...
    def test_point_line_width_op(self):
        points = Points([(0, 0, 1), (0, 1, 4), (0, 2, 8)], vdims='line_width').opts(line_width='line_width')
        plot = bokeh_renderer.get_plot(points)
        cds = plot.handles['cds']
        glyph = plot.handles['glyph']
    ...
    def test_point_color_index_color_no_clash(self):
        points = Points([(0, 0, 0), (0, 1, 1), (0, 2, 2)], vdims='color').opts(fill_color='color', color_index='color')
        plot = bokeh_renderer.get_plot(points)
        glyph = plot.handles['glyph']
