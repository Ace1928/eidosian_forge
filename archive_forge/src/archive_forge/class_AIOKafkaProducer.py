...
class AIOKafkaProducer:
    ...
    def __init__(self, *, loop=None, bootstrap_servers='localhost', client_id=None, metadata_max_age_ms=300000, request_timeout_ms=40000, api_version='auto', acks=_missing, key_serializer=None, value_serializer=None, compression_type=None, max_batch_size=16384, partitioner=DefaultPartitioner(), max_request_size=1048576, linger_ms=0, send_backoff_ms=100, retry_backoff_ms=100, security_protocol='PLAINTEXT', ssl_context=None, connections_max_idle_ms=540000, enable_idempotence=False, transactional_id=None, transaction_timeout_ms=60000, sasl_mechanism='PLAIN', sasl_plain_password=None, sasl_plain_username=None, sasl_kerberos_service_name='kafka', sasl_kerberos_domain_name=None, sasl_oauth_token_provider=None):
        ...
        if compression_type:
            ...
            if not checker():
                raise RuntimeError('Compression library for {} not found'.format(compression_type))
        else:
            compression_attrs = 0
        ...
        if enable_idempotence:
            if acks is _missing:
                ...
                raise ValueError('acks={} not supported if enable_idempotence=True'.format(acks))
            self._txn_manager = TransactionManager(transactional_id, transaction_timeout_ms)
        else:
        ...
        if client_id is None:
            client_id = 'aiokafka-producer-%s' % AIOKafkaProducer._PRODUCER_CLIENT_ID_SEQUENCE
        ...
        self._message_accumulator = MessageAccumulator(self._metadata, max_batch_size, compression_attrs, self._request_timeout_ms / 1000, txn_manager=self._txn_manager, loop=loop)
        self._sender = Sender(self.client, acks=acks, txn_manager=self._txn_manager, retry_backoff_ms=retry_backoff_ms, linger_ms=linger_ms, message_accumulator=self._message_accumulator, request_timeout_ms=request_timeout_ms)
        self._closed = False

    def __del__(self, _warnings=warnings):
        if self._closed is False:
            _warnings.warn(f'Unclosed AIOKafkaProducer {self!r}', ResourceWarning, source=self)
            context = {'producer': self, 'message': 'Unclosed AIOKafkaProducer'}
    ...
    async def start(self):
        ...
        if self._compression_type == 'lz4':
            assert self.client.api_version >= (0, 8, 2), 'LZ4 Requires >= Kafka 0.8.2 Brokers'
        elif self._compression_type == 'zstd':
            assert self.client.api_version >= (2, 1, 0), 'Zstd Requires >= Kafka 2.1.0 Brokers'
        if self._txn_manager is not None and self.client.api_version < (0, 11):
            raise UnsupportedVersionError('Idempotent producer available only for Broker version 0.11 and above')
    ...
    async def stop(self):
        ...
        if self._sender is not None and self._sender.sender_task is not None:
            await asyncio.wait([create_task(self._message_accumulator.close()), self._sender.sender_task], return_when=asyncio.FIRST_COMPLETED)
    ...
    def _serialize(self, topic, key, value):
        ...
        if message_size > self._max_request_size:
            raise MessageSizeTooLargeError('The message is %d bytes when serialized which is larger than the maximum request size you have configured with the max_request_size configuration' % message_size)
    ...
    async def send(self, topic, value=None, key=None, partition=None, timestamp_ms=None, headers=None):
        ...
        Note:
            ...
            **will**.
        """
        assert value is not None or self.client.api_version >= (0, 8, 1), 'Null messages require kafka >= 0.8.1'
        assert not (value is None and key is None), 'Need at least one: key or value'
        await self.client._wait_on_metadata(topic)
        if self._txn_manager is not None:
            txn_manager = self._txn_manager
            if txn_manager.transactional_id is not None and (not self._txn_manager.is_in_transaction()):
        ...
        fut = await self._message_accumulator.add_message(tp, key_bytes, value_bytes, self._request_timeout_ms / 1000, timestamp_ms=timestamp_ms, headers=headers)
        return fut
    ...
    async def send_batch(self, batch, topic, *, partition):
        ...
        future = await self._message_accumulator.add_batch(batch, tp, self._request_timeout_ms / 1000)
        return future
    ...
    async def begin_transaction(self):
        ...
        log.debug('Beginning a new transaction for id %s', self._txn_manager.transactional_id)
        await asyncio.shield(self._txn_manager.wait_for_pid())
        self._txn_manager.begin_transaction()

    async def commit_transaction(self):
        self._ensure_transactional()
        log.debug('Committing transaction for id %s', self._txn_manager.transactional_id)
        self._txn_manager.committing_transaction()
        await asyncio.shield(self._txn_manager.wait_for_transaction_end())
    ...
    async def send_offsets_to_transaction(self, offsets, group_id):
        ...
        log.debug('Begin adding offsets %s for consumer group %s to transaction', formatted_offsets, group_id)
