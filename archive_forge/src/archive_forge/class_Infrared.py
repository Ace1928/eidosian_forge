...
class Infrared(Vibrations):
    ...
    *get_frequencies()*, *get_spectrum()* and *write_spectra()*
    ...
    def __init__(self, atoms, indices=None, name='ir', delta=0.01, nfree=2, directions=None):
        Vibrations.__init__(self, atoms, indices=indices, name=name, delta=delta, nfree=nfree)
    ...
    def summary(self, method='standard', direction='central', intensity_unit='(D/A)2/amu', log=stdout):
        ...
        for n, e in enumerate(hnu):
            if e.imag != 0:
                ...
                c = ' '
                e = e.real
            parprint(('%3d %6.1f%s  %7.1f%s  ' + iu_format) % (n, 1000 * e, c, s * e, c, iu * self.intensities[n]), file=log)
        parprint('-------------------------------------', file=log)
