...
class MultipartMime(software_config.SoftwareConfig):
    ...
    PROPERTIES = PARTS, CONFIG, FILENAME, TYPE, SUBTYPE, GROUP = ('parts', 'config', 'filename', 'type', 'subtype', 'group')
    TYPES = TEXT, MULTIPART = ('text', 'multipart')
    properties_schema = {GROUP: properties.Schema(properties.Schema.STRING, _('Namespace to group this multi-part configs by when delivered to a server. This may imply what configuration tool is going to perform the configuration.'), support_status=support.SupportStatus(version='14.0.0'), default='Heat::Ungrouped'), PARTS: properties.Schema(properties.Schema.LIST, _('Parts belonging to this message.'), default=[], schema=properties.Schema(properties.Schema.MAP, schema={CONFIG: properties.Schema(properties.Schema.STRING, _('Content of part to attach, either inline or by referencing the ID of another software config resource.'), required=True), FILENAME: properties.Schema(properties.Schema.STRING, _('Optional filename to associate with part.')), TYPE: properties.Schema(properties.Schema.STRING, _('Whether the part content is text or multipart.'), default=TEXT, constraints=[constraints.AllowedValues(TYPES)]), SUBTYPE: properties.Schema(properties.Schema.STRING, _('Optional subtype to specify with the type.'))}))}
    message = None

    def handle_create(self):
        props = {rpc_api.SOFTWARE_CONFIG_NAME: self.physical_resource_name(), rpc_api.SOFTWARE_CONFIG_CONFIG: self.get_message(), rpc_api.SOFTWARE_CONFIG_GROUP: self.properties[self.GROUP]}
        ...
        for item in self.properties[self.PARTS]:
            ...
            if uuidutils.is_uuid_like(config):
                with self.rpc_client().ignore_error_by_name('NotFound'):
                    sc = self.rpc_client().show_software_config(self.context, config)
    ...
    def _create_message(part, subtype, filename):
        ...
        try:
            ...
            charset = 'utf-8'
        msg = text.MIMEText(part, _subtype=subtype, _charset=charset) if subtype else text.MIMEText(part, _charset=charset)
        if filename:
