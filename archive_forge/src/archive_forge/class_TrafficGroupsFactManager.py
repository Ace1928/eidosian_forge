...
class TrafficGroupsFactManager(BaseManager):
    ...
    def read_collection_from_device(self, skip=0):
        uri = 'https://{0}:{1}/mgmt/tm/cm/traffic-group'.format(self.client.provider['server'], self.client.provider['server_port'])
    ...
    def read_stats_from_device(self, full_path):
        uri = 'https://{0}:{1}/mgmt/tm/cm/traffic-group/{2}/stats'.format(self.client.provider['server'], self.client.provider['server_port'], transform_name(name=full_path))
