...
class AudioPlugin(base_plugin.TBPlugin):
    ...
    def __init__(self, context):
        ...
        self._version_checker = plugin_util._MetadataVersionChecker(data_kind='audio', latest_known_version=0)

    def get_plugin_apps(self):
        return {'/audio': self._serve_audio_metadata, '/individualAudio': self._serve_individual_audio, '/tags': self._serve_tags}
        ...
        for run, tag_to_time_series in mapping.items():
            for tag, time_series in tag_to_time_series.items():
                ...
                if not self._version_checker.ok(md.version, run, tag):
                    continue
                description = plugin_util.markdown_to_safe_html(time_series.description)
                result[run][tag] = {'displayName': time_series.display_name, 'description': description, 'samples': time_series.max_length}
        return result
    ...
    def _serve_individual_audio(self, request):
        ...
        if mime_type not in _ALLOWED_MIME_TYPES:
            raise errors.InvalidArgumentError('Illegal mime type %r' % mime_type)
