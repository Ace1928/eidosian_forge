...
class BackupsClient(object):
    ...
    def __init__(self, release_track=base.ReleaseTrack.BETA):
        if release_track == base.ReleaseTrack.BETA:
            ...
            raise ValueError('[{}] is not a valid API version.'.format(netapp_util.VERSION_MAP[release_track]))
    ...
    def CreateBackup(self, backup_ref, async_, backup):
        ...
        request = self.messages.NetappProjectsLocationsBackupVaultsBackupsCreateRequest(parent=backup_ref.Parent().RelativeName(), backupId=backup_ref.Name(), backup=backup)
        create_op = self.client.projects_locations_backupVaults_backups.Create(request)
        if async_:
            return create_op
    ...
    """
        request = self.messages.NetappProjectsLocationsBackupVaultsBackupsListRequest(parent=backupvault_ref)
        ...
        return list_pager.YieldFromList(self.client.projects_locations_backupVaults_backups, request, field=constants.BACKUP_RESOURCE, limit=limit, batch_size_attribute='pageSize')

    def GetBackup(self, backup_ref):
        """Get Cloud NetApp Backup information."""
        request = self.messages.NetappProjectsLocationsBackupVaultsBackupsGetRequest(name=backup_ref.RelativeName())
    ...
    def DeleteBackup(self, backup_ref, async_):
        ...
        request = self.messages.NetappProjectsLocationsBackupVaultsBackupsDeleteRequest(name=backup_ref.RelativeName())
        return self._DeleteBackup(async_, request)

    def _DeleteBackup(self, async_, request):
        delete_op = self.client.projects_locations_backupVaults_backups.Delete(request)
        if async_:
            return delete_op
        operation_ref = resources.REGISTRY.ParseRelativeName(delete_op.name, collection=constants.OPERATIONS_COLLECTION)
        return self.WaitForOperation(operation_ref)

    def UpdateBackup(self, backup_ref, backup, update_mask, async_):
        """Updates a Backup.
    ...
    """
        update_op = self._adapter.UpdateBackup(backup_ref, backup, update_mask)
        if async_:
            return update_op
    ...
    def ParseUpdatedBackup(self, backup, description=None, labels=None):
        ...
        return self._adapter.ParseUpdatedBackup(backup, description=description, labels=labels)