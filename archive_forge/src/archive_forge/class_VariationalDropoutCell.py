...
class VariationalDropoutCell(ModifierCell):
    ...
    def __init__(self, base_cell, drop_inputs=0.0, drop_states=0.0, drop_outputs=0.0):
        assert not drop_states or not isinstance(base_cell, BidirectionalCell), "BidirectionalCell doesn't support variational state dropout. Please add VariationalDropoutCell to the cells underneath instead."
        assert not drop_states or not isinstance(base_cell, SequentialRNNCell) or (not base_cell._bidirectional), "Bidirectional SequentialRNNCell doesn't support variational state dropout. Please add VariationalDropoutCell to the cells underneath instead."
    ...
    def unroll(self, length, inputs, begin_state=None, layout='NTC', merge_outputs=None, valid_length=None):
        ...
        outputs, states = self.base_cell.unroll(length, inputs, states, layout, merge_outputs=True, valid_length=valid_length)
        if self.drop_outputs:
