...
def _preprocess_input(image, footprint=None, out=None, mask=None, out_dtype=None, pixel_size=1, shift_x=None, shift_y=None):
    ...
    if input_dtype not in (np.uint8, np.uint16):
        message = f'Possible precision loss converting image of type {input_dtype} to uint8 as required by rank filters. Convert manually using skimage.util.img_as_ubyte to silence this warning.'
    ...
    for name, value in zip(('shift_x', 'shift_y'), (shift_x, shift_y)):
        if np.dtype(type(value)) == bool:
            warn(f'Paramter `{name}` is boolean and will be interpreted as int. This is not officially supported, use int instead.', category=UserWarning, stacklevel=4)
