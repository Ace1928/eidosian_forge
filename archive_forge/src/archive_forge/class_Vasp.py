...
class Vasp(GenerateVaspInput, Calculator):
    ...
    """
    name = 'vasp'
    ase_objtype = 'vasp_calculator'
    env_commands = ('ASE_VASP_COMMAND', 'VASP_COMMAND', 'VASP_SCRIPT')
    implemented_properties = ['energy', 'free_energy', 'forces', 'dipole', 'fermi', 'stress', 'magmom', 'magmoms']
    default_parameters: Dict[str, Any] = {}

    def __init__(self, atoms=None, restart=None, directory='.', label='vasp', ignore_bad_restart_file=Calculator._deprecated, command=None, txt='vasp.out', **kwargs):
        self._atoms = None
        self.results = {}
        ...
        if '/' in label:
            warn('Specifying directory in "label" is deprecated, use "directory" instead.', np.VisibleDeprecationWarning)
            if self.directory != '.':
                raise ValueError('Directory redundantly specified though directory="{}" and label="{}".  Please omit "/" in label.'.format(self.directory, label))
            self.label = label
        else:
            self.prefix = label
        if isinstance(restart, bool):
            if restart is True:
                ...
                restart = None
        Calculator.__init__(self, restart=restart, ignore_bad_restart_file=ignore_bad_restart_file, label=self.label, atoms=atoms, **kwargs)
    ...
    def calculate(self, atoms=None, properties=('energy',), system_changes=tuple(calculator.all_changes)):
        ...
        if errorcode:
            raise calculator.CalculationFailed('{} in {} returned an error: {:d}'.format(self.name, self.directory, errorcode))
    ...
    def check_state(self, atoms, tol=1e-15):
        ...
        for param_string, old_dict in self.param_state.items():
            param_dict = getattr(self, param_string)
    ...
    def asdict(self):
        ...
        self._store_param_state()
        inputs = {key: value for param_dct in self.param_state.values() for key, value in param_dct.items() if value is not None}
        dct = {'ase_version': asevers, 'vasp_version': vaspvers, 'inputs': inputs, 'results': self.results.copy()}
        if self.atoms:
    ...
    def read(self, label=None):
        ...
        for f in ['OUTCAR', 'CONTCAR', 'vasprun.xml']:
            file = self._indir(f)
            if not file.is_file():
                raise calculator.ReadError('VASP outputfile {} was not found'.format(file))
        self.read_sort()
    ...
    def read_results(self):
        ...
        self.results.update(dict(magmom=magmom, magmoms=magmoms, dipole=dipole, nbands=nbands))
    ...
    def _read_xml(self) -> SinglePointDFTCalculator:
        ...
        incomplete_msg = f'The file "{file}" is incomplete, and no DFT data was available. This is likely due to an incomplete calculation.'
        try:
    ...
    def _xml_calc(self) -> SinglePointDFTCalculator:
        if self.__xml_calc is None:
            raise RuntimeError('vasprun.xml data has not yet been loaded. Run read_results() first.')
        ...
        with self.load_file_iter('OUTCAR') as lines:
            for line in lines:
                if '- Iteration' in line:
                    niter = list(map(int, re.findall('\\d+', line)))[1]
        return niter
