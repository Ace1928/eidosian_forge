...
class LifespanOn:
    ...
    async def main(self) -> None:
        try:
            ...
            scope: LifespanScope = {'type': 'lifespan', 'asgi': {'version': self.config.asgi_version, 'spec_version': '2.0'}, 'state': self.state}
    ...
    async def send(self, message: LifespanSendMessage) -> None:
        assert message['type'] in ('lifespan.startup.complete', 'lifespan.startup.failed', 'lifespan.shutdown.complete', 'lifespan.shutdown.failed')
