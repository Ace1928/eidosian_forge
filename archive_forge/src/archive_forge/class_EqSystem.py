...
class EqSystem(ReactionSystem):
    ...
    def _solve(self, init_concs, x0=None, NumSys=(NumSysLog, NumSysLin), neqsys='chained_conditional', **kwargs):
        if isinstance(neqsys, str):
            neqsys = self.get_neqsys(neqsys, NumSys=NumSys, rref_equil=kwargs.pop('rref_equil', False), rref_preserv=kwargs.pop('rref_preserv', False), precipitates=kwargs.pop('precipitates', None))
        if x0 is None:
            x0 = init_concs
        params = np.concatenate((init_concs, [float(elem) for elem in self.eq_constants()]))
    ...
    def root(self, init_concs, x0=None, neqsys=None, NumSys=NumSysLog, neqsys_type='chained_conditional', **kwargs):
        init_concs = self.as_per_substance_array(init_concs)
        params = np.concatenate((init_concs, [float(elem) for elem in self.eq_constants()]))
        if neqsys is None:
            neqsys = self.get_neqsys(neqsys_type, NumSys=NumSys, rref_equil=kwargs.pop('rref_equil', False), rref_preserv=kwargs.pop('rref_preserv', False), precipitates=kwargs.pop('precipitates', None))
        if x0 is None:
            x0 = init_concs
        x, sol = neqsys.solve(x0, params, **kwargs)
        if not sol['success']:
    ...
    def roots(self, init_concs, varied_data, varied, x0=None, NumSys=NumSysLog, plot_kwargs=None, neqsys_type='chained_conditional', **kwargs):
        ...
        if _plot:
            ...
            if 'substances' in plot_kwargs:
                ...
                kwargs['plot_kwargs']['indices'] = map(self.as_substance_index, plot_kwargs.pop('substances'))
                print(kwargs['plot_kwargs']['indices'])
        else:
            cb = neqsys.solve_series
        params = np.concatenate((init_concs, self.eq_constants()))
        xvecs, info_dicts = cb(x0, params, varied_data, self.as_substance_index(varied), propagate=False, **kwargs)
        ...
        if _plot:
            ...
            xlbl = '$[' + varied_subst.latex_name + ']_0$' if latex_names else str(varied_subst)
