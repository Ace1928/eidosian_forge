...
def RaiseWrongNumberOfArgumentsException(self):
    ...
    if len(self.args) < self.command_spec.min_args:
        ...
        message = 'The %s command requires at least %d argument%s.' % (self.command_name, self.command_spec.min_args, tail_str)
    else:
        message = 'The %s command accepts at most %d arguments.' % (self.command_name, self.command_spec.max_args)
    message += ' Usage:\n%s\nFor additional help run:\n  gsutil help %s' % (self.command_spec.usage_synopsis, self.command_name)
