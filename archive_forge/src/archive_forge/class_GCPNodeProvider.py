    ...
    def create_node(self, base_config: dict, tags: dict, count: int) -> Dict[str, dict]:
        ...
        with self.lock:
            ...
            if self.cache_stopped_nodes:
                filters = {'ray-node-name': labels['ray-node-name'], 'ray-node-type': labels['ray-node-type'], 'ray-user-node-type': labels['ray-user-node-type']}
                reuse_nodes = resource.list_instances(filters, True)[:count]
                if reuse_nodes:
                    reused_nodes_dict = {n['name']: resource.start_instance(n['name']) for n in reuse_nodes}
                    all_nodes.update(reused_nodes_dict)
                    count -= len(reuse_nodes)
            if count > 0:
                results = resource.create_instances(base_config, labels, count)
                created_nodes_dict = {instance_id: result for result, instance_id in results}
