...
def derive_config_from_deleted_cmd(command, exist_conf, test_keys=None):
    ...
    if isinstance(command, list) and isinstance(exist_conf, list):
        nu, new_conf_dict = derive_config_from_deleted_cmd_dict({'config': command}, {'config': exist_conf}, test_keys)
        ...
        nu, new_conf = derive_config_from_deleted_cmd_dict(command, exist_conf, test_keys, None, None, delete_op_dft)
