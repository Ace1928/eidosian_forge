...
class VolumeEncryptionTypesTest(utils.TestCase):
    ...
    def test_create(self):
        ...
        """
        result = cs.volume_encryption_types.create(2, {'provider': 'Test', 'key_size': None, 'cipher': None, 'control_location': None})
    ...
    def test_update(self):
        ...
        """
        expected = {'id': 1, 'volume_type_id': 1, 'provider': 'test', 'cipher': 'test', 'key_size': 1, 'control_location': 'front-end'}
        result = cs.volume_encryption_types.update(1, {})
        cs.assert_called('PUT', '/types/1/encryption/provider')
        self.assertEqual(expected, result, 'empty update must yield original data')
        self._assert_request_id(result)
    ...
    def test___repr__(self):
        ...
        self.assertEqual('<VolumeEncryptionType: %s>' % FAKE_ENCRY_TYPE['encryption_id'], repr(encry_type))