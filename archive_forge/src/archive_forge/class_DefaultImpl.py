...
class DefaultImpl(metaclass=ImplMeta):
    ...
    def _exec(self, construct: Union[Executable, str], execution_options: Optional[dict[str, Any]]=None, multiparams: Sequence[dict]=(), params: Dict[str, Any]=util.immutabledict()) -> Optional[CursorResult]:
    ...
    def alter_column(self, table_name: str, column_name: str, nullable: Optional[bool]=None, server_default: Union[_ServerDefault, Literal[False]]=False, name: Optional[str]=None, type_: Optional[TypeEngine]=None, schema: Optional[str]=None, autoincrement: Optional[bool]=None, comment: Optional[Union[str, Literal[False]]]=False, existing_comment: Optional[str]=None, existing_type: Optional[TypeEngine]=None, existing_server_default: Optional[_ServerDefault]=None, existing_nullable: Optional[bool]=None, existing_autoincrement: Optional[bool]=None, **kw: Any) -> None:
        if autoincrement is not None or existing_autoincrement is not None:
            util.warn('autoincrement and existing_autoincrement only make sense for MySQL', stacklevel=3)
        if nullable is not None:
            self._exec(base.ColumnNullable(table_name, column_name, nullable, schema=schema, existing_type=existing_type, existing_server_default=existing_server_default, existing_nullable=existing_nullable, existing_comment=existing_comment))
        if server_default is not False:
            kw = {}
            cls_: Type[Union[base.ComputedColumnDefault, base.IdentityColumnDefault, base.ColumnDefault]]
            ...
            self._exec(cls_(table_name, column_name, server_default, schema=schema, existing_type=existing_type, existing_server_default=existing_server_default, existing_nullable=existing_nullable, existing_comment=existing_comment, **kw))
        if type_ is not None:
            self._exec(base.ColumnType(table_name, column_name, type_, schema=schema, existing_type=existing_type, existing_server_default=existing_server_default, existing_nullable=existing_nullable, existing_comment=existing_comment))
        if comment is not False:
            self._exec(base.ColumnComment(table_name, column_name, comment, schema=schema, existing_type=existing_type, existing_server_default=existing_server_default, existing_nullable=existing_nullable, existing_comment=existing_comment))
        if name is not None:
            self._exec(base.ColumnName(table_name, column_name, name, schema=schema, existing_type=existing_type, existing_server_default=existing_server_default, existing_nullable=existing_nullable))

    def add_column(self, table_name: str, column: Column[Any], schema: Optional[Union[str, quoted_name]]=None) -> None:
        self._exec(base.AddColumn(table_name, column, schema=schema))

    def drop_column(self, table_name: str, column: Column[Any], schema: Optional[str]=None, **kw) -> None:
    ...
    def rename_table(self, old_table_name: str, new_table_name: Union[str, quoted_name], schema: Optional[Union[str, quoted_name]]=None) -> None:
    ...
    def bulk_insert(self, table: Union[TableClause, Table], rows: List[dict], multiinsert: bool=True) -> None:
        ...
        if self.as_sql:
            for row in rows:
                self._exec(sqla_compat._insert_inline(table).values(**{k: sqla_compat._literal_bindparam(k, v, type_=table.c[k].type) if not isinstance(v, sqla_compat._literal_bindparam) else v for k, v in row.items()}))
        elif rows:
            if multiinsert:
    ...
    def _compare_identity_default(self, metadata_identity, inspector_identity):
        diff, ignored = _compare_identity_options(metadata_identity, inspector_identity, sqla_compat.Identity(), skip={'always'})
        ...
        return (diff, ignored, bool(diff) or bool(metadata_identity) != bool(inspector_identity))
    ...
    def compare_indexes(self, metadata_index: Index, reflected_index: Index) -> ComparisonResult:
        ...
        for sig in (m_sig, r_sig):
            if sig.has_expressions:
                log.warning('Generating approximate signature for index %s. The dialect implementation should either skip expression indexes or provide a custom implementation.', sig.const)
