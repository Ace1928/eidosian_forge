    ...
    def run(self, directory='.', revision=None):
        ...
        with ui.ui_factory.nested_progress_bar() as pb:
            for idx, inv in enumerate(repo.iter_inventories(inv_ids)):
                ...
                test_key = chk_map.CHKMap.from_dict(vf, d, maximum_size=inv.id_to_entry._root_node._maximum_size, key_width=inv.id_to_entry._root_node._key_width, search_key_func=inv.id_to_entry._search_key_func)
                ...
                test_key = chk_map.CHKMap.from_dict(vf, d, maximum_size=pid._root_node._maximum_size, key_width=pid._root_node._key_width, search_key_func=pid._search_key_func)
                if pid.key() != test_key:
                    trace.warning('Failed for parent_id_to_basename inv: %s' % (inv.revision_id,))