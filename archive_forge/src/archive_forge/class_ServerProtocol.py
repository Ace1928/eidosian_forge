...
class ServerProtocol(Protocol):
    ...
    def process_subprotocol(self, headers: Headers) -> Optional[Subprotocol]:
        ...
        """
        subprotocols: Sequence[Subprotocol] = sum([parse_subprotocol(header_value) for header_value in headers.get_all('Sec-WebSocket-Protocol')], [])
        return self.select_subprotocol(subprotocols)

    def select_subprotocol(self, subprotocols: Sequence[Subprotocol]) -> Optional[Subprotocol]:
        """
    ...
    def reject(self, status: StatusLike, text: str) -> Response:
        ...
        body = text.encode()
        headers = Headers([('Date', email.utils.formatdate(usegmt=True)), ('Connection', 'close'), ('Content-Length', str(len(body))), ('Content-Type', 'text/plain; charset=utf-8')])
