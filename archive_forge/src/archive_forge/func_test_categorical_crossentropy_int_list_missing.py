...
@pytest.mark.parametrize('guesses, labels', [(guesses1, [2, 1, 0, 2])])
def test_categorical_crossentropy_int_list_missing(guesses, labels):
    d_scores = CategoricalCrossentropy(normalize=True, missing_value=0).get_grad(guesses, labels)
    ...
    loss = CategoricalCrossentropy(normalize=True, missing_value=0).get_loss(guesses, labels)
