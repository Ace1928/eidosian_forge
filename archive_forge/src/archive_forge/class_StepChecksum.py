    ...
    def _thread_body(self) -> None:
        while True:
            ...
            if isinstance(req, RequestUpload):
                ...
                self._output_queue.put(step_upload.RequestUpload(path, req.save_name, None, None, req.copy, None, None, None))
                ...
                for entry in req.manifest.entries.values():
                    if entry.local_path:
                        ...
                        self._output_queue.put(step_upload.RequestUpload(entry.local_path, entry.path, req.artifact_id, entry.digest, False, functools.partial(req.save_fn, entry), functools.partial(req.save_fn_async, entry), entry.digest))
