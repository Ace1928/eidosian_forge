...
class WdcRedfishUtils(RedfishUtils):
    ...
    def __init__(self, creds, root_uris, timeout, module, resource_id, data_modification):
        super(WdcRedfishUtils, self).__init__(creds=creds, root_uri=root_uris[0], timeout=timeout, module=module, resource_id=resource_id, data_modification=data_modification)
    ...
    def update_and_activate(self, update_opts):
        ...
        if update_opts.get('update_creds') is not None and 'username' in update_opts['update_creds'] and ('password' in update_opts['update_creds']):
            ...
            if update_creds:
                ...
                parsed_url = parsed_url._replace(netloc='{0}:{1}@{2}'.format(update_creds.get('username'), update_creds.get('password'), original_netloc))
        ...
        if status_code not in [self.UPDATE_STATUS_CODE_READY_FOR_FW_UPDATE, self.UPDATE_STATUS_CODE_FW_UPDATE_FAILED]:
            return {'ret': False, 'msg': 'Target is not ready for FW update.  Current status: {0} ({1})'.format(status_code, status_description)}
        ...
        if bundle_firmware_version is None or is_bundle_multi_tenant is None:
            return {'ret': False, 'msg': 'Unable to extract bundle version or multi-tenant status from update image tarfile'}
        is_enclosure_multi_tenant = self._is_enclosure_multi_tenant()
        if is_enclosure_multi_tenant != is_bundle_multi_tenant:
            return {'ret': False, 'msg': 'Enclosure multi-tenant is {0} but bundle multi-tenant is {1}'.format(is_enclosure_multi_tenant, is_bundle_multi_tenant)}
        ...
        if is_enclosure_multi_tenant:
            firmware_already_installed = bundle_firmware_version == self._get_installed_firmware_version_of_multi_tenant_system(iom_a_firmware_version, iom_b_firmware_version)
        else:
        ...
        if firmware_already_installed:
            return {'ret': True, 'changed': False, 'msg': 'Version {0} already installed'.format(bundle_firmware_version)}
        ...
        status_codes_for_update_incomplete = [self.UPDATE_STATUS_CODE_FW_UPDATE_IN_PROGRESS, self.UPDATE_STATUS_CODE_READY_FOR_FW_UPDATE]
        iteration = 0
        while status_code in status_codes_for_update_incomplete and datetime.datetime.now() - start_time < datetime.timedelta(minutes=max_wait_minutes):
            if iteration != 0:
        ...
        if status_code != self.UPDATE_STATUS_CODE_FW_UPDATE_COMPLETED_WAITING_FOR_ACTIVATION:
            return {'ret': False, 'msg': 'Target is not ready for FW activation after update.  Current status: {0} ({1})'.format(status_code, status_description)}
    ...
    def manage_power_mode(self, command, resource_uri=None):
        ...
        if WdcRedfishUtils.OEM not in data or WdcRedfishUtils.WDC not in data[WdcRedfishUtils.OEM] or power_mode not in data[WdcRedfishUtils.OEM][WdcRedfishUtils.WDC]:
