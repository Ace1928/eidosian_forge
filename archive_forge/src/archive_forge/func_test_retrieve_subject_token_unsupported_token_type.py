...
def test_retrieve_subject_token_unsupported_token_type(self):
    EXECUTABLE_SUCCESSFUL_OIDC_RESPONSE = {'version': 1, 'success': True, 'token_type': 'unsupported_token_type', 'id_token': self.EXECUTABLE_OIDC_TOKEN, 'expiration_time': 9999999999}
    with mock.patch('subprocess.run', return_value=subprocess.CompletedProcess(args=[], stdout=json.dumps(EXECUTABLE_SUCCESSFUL_OIDC_RESPONSE).encode('UTF-8'), returncode=0)):
        ...
        assert excinfo.match('Executable returned unsupported token type.')