...
class cuDFOnRayDataframePartitionManager(GenericRayDataframePartitionManager):
    ...
    def from_pandas(cls, df, return_dims=False):
        ...
        pandas_dfs = split_result_of_axis_func_pandas(0, num_splits, df, min_block_size=MinPartitionSize.get())
        keys = [put_func(cls._get_gpu_managers()[i], pandas_dfs[i]) for i in range(num_splits)]
        keys = RayWrapper.materialize(keys)
        parts = cls._create_partitions(keys, cls._get_gpu_managers()).reshape((num_splits, 1))
        if not return_dims:
            return parts
        else:
    ...
    def lazy_map_partitions(cls, partitions, map_func):
        ...
        key_futures = [partition.apply(preprocessed_map_func) for partition in partitions_flat]
        gpu_managers = [partition.get_gpu_manager() for partition in partitions_flat]
        return cls._create_partitions(key_futures, gpu_managers).reshape(partitions.shape)
    ...
    def _apply_func_to_list_of_partitions(cls, func, partitions, **kwargs):
        ...
        **kwargs : dict
        ...
        key_futures = RayWrapper.materialize([partition.apply(preprocessed_map_func, **kwargs) for partition in partitions])
