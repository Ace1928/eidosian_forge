...
class OctoAIEmbeddings(BaseModel, Embeddings):
    ...
    model_kwargs: Optional[dict] = Field(None, description='Keyword arguments to pass to the model.')
    octoai_api_token: Optional[str] = Field(None, description='OCTOAI API Token')
    embed_instruction: str = Field(DEFAULT_EMBED_INSTRUCTION, description='Instruction to use for embedding documents.')
    query_instruction: str = Field(DEFAULT_QUERY_INSTRUCTION, description='Instruction to use for embedding query.')
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        values['octoai_api_token'] = get_from_dict_or_env(values, 'octoai_api_token', 'OCTOAI_API_TOKEN')
        values['endpoint_url'] = get_from_dict_or_env(values, 'endpoint_url', 'https://text.octoai.run/v1/embeddings')
        return values
