...
class QChemDictSet(QCInput):
    ...
    def __init__(self, molecule: Molecule, job_type: str, basis_set: str, scf_algorithm: str, qchem_version: int=5, dft_rung: int=4, pcm_dielectric: float | None=None, isosvp_dielectric: float | None=None, smd_solvent: str | None=None, cmirs_solvent: Literal['water', 'acetonitrile', 'dimethyl sulfoxide', 'cyclohexane', 'benzene'] | None=None, custom_smd: str | None=None, opt_variables: dict[str, list] | None=None, scan_variables: dict[str, list] | None=None, max_scf_cycles: int=100, geom_opt_max_cycles: int=200, plot_cubes: bool=False, nbo_params: dict | None=None, geom_opt: dict | None=None, cdft_constraints: list[list[dict]] | None=None, almo_coupling_states: list[list[tuple[int, int]]] | None=None, overwrite_inputs: dict | None=None, vdw_mode: Literal['atomic', 'sequential']='atomic', extra_scf_print: bool=False):
        """
        Args:
        ...
        if self.smd_solvent is not None:
            ...
            if self.smd_solvent in ('custom', 'other'):
                if self.custom_smd is None:
                    raise ValueError('A user-defined SMD requires passing custom_smd, a string of seven comma separated values in the following order: dielectric, refractive index, acidity, basicity, surface tension, aromaticity, electronegative halogenicity')
        ...
        if self.job_type.lower() in ['opt', 'optimization'] and self.qchem_version == 6 or (self.qchem_version == 5 and self.geom_opt is not None):
                    ...
                    for k, v in temp_svp.items():
                        if k == 'rhoiso' and self.cmirs_solvent is not None:
                            if v not in ['0.001', '0.0005']:
                                raise RuntimeError('CMIRS is only parameterized for RHOISO values of 0.001 or 0.0005! Exiting...')
                        ...
                        if k == 'idefesr':
                            if self.cmirs_solvent is not None and v == '0':
                                warnings.warn('Setting IDEFESR=0 will disable the CMIRS calculation you requested!', UserWarning)
                            if self.cmirs_solvent is None and v == '1':
                                warnings.warn('Setting IDEFESR=1 will have no effect unless you specify a cmirs_solvent!', UserWarning)
                        if k == 'dielst' and rem['solvent_method'] != 'isosvp':
                            warnings.warn('Setting DIELST will have no effect unless you specify a solvent_method=isosvp!', UserWarning)
                        svp[k] = v
                if sec == 'pcm_nonels':
        ...
        super().__init__(self.molecule, rem=rem, opt=opt, pcm=pcm, solvent=solvent, smx=smx, scan=scan, van_der_waals=vdw, vdw_mode=self.vdw_mode, plots=plots, nbo=nbo, geom_opt=geom_opt, cdft=self.cdft_constraints, almo_coupling=self.almo_coupling_states, svp=svp, pcm_nonels=pcm_nonels)
