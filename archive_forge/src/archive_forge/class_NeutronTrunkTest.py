...
class NeutronTrunkTest(common.HeatTestCase):
    ...
    def setUp(self):
        ...
        self.create_trunk_mock = self.patchobject(neutronclient.Client, 'create_trunk')
        self.delete_trunk_mock = self.patchobject(neutronclient.Client, 'delete_trunk')
        self.show_trunk_mock = self.patchobject(neutronclient.Client, 'show_trunk')
        self.update_trunk_mock = self.patchobject(neutronclient.Client, 'update_trunk')
        self.trunk_remove_subports_mock = self.patchobject(neutronclient.Client, 'trunk_remove_subports')
        self.trunk_add_subports_mock = self.patchobject(neutronclient.Client, 'trunk_add_subports')
        self.find_resource_mock = self.patchobject(neutronV20, 'find_resourceid_by_name_or_id')
        rv = {'trunk': {'id': 'trunk id', 'status': 'DOWN'}}
    ...
    def test_delete_already_gone(self):
        ...
        stack = utils.parse_stack(t)
        self._create_trunk(stack)
        self.delete_trunk_mock.side_effect = ncex.NeutronClientException(status_code=404)
        self._delete_trunk(stack)
        self.delete_trunk_mock.assert_called_once_with('trunk id')
    ...
    def test_update_basic_properties(self):
        ...
        self.update_trunk_mock.assert_called_once_with('trunk id', {'trunk': {'name': 'new trunk name'}})
