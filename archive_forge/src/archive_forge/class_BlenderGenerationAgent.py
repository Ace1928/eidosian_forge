...
class BlenderGenerationAgent(ConvAI2Agent, Seq2SeqLM):
    ...
    def __init__(self, model: str, device: str, maxlen: int) -> None:
        ...
        option = self.set_options(name=f'zoo:blender/blender_{size}/model', device=device)
        super().__init__(name=model, suffix='\n', device=device, maxlen=maxlen, model=create_agent_from_opt_file(option))

    @staticmethod
    def available_models():
        return ['blender.small', 'blender.medium', 'blender.large', 'blender.xlarge', 'blender.xxlarge']
