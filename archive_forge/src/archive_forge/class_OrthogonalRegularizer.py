...
@keras_export('keras.regularizers.OrthogonalRegularizer', 'keras.regularizers.orthogonal_regularizer', v1=[])
class OrthogonalRegularizer(Regularizer):
    ...
    def __init__(self, factor=0.01, mode='rows'):
        ...
        if mode not in {'rows', 'columns'}:
            raise ValueError(f'Invalid value for argument `mode`. Expected one of {{"rows", "columns"}}. Received: mode={mode}')
        self.mode = mode

    def __call__(self, inputs):
        if inputs.shape.rank != 2:
            raise ValueError(f'Inputs to OrthogonalRegularizer must have rank 2. Received: inputs.shape == {inputs.shape}')
        if self.mode == 'rows':
