...
class GUI:
    ...
    def setup_elements(self):
        ...
        self.restart_button = self._create_button(
            "Restart", (250, 50), self._restart_game
        )
        self.quit_button = self._create_button("Quit", (350, 50), self._quit_game)

        self.grid_size_slider = self._create_slider(
            "Grid Size", (50, 100), 10, 50, self._adjust_grid_size
        )
        self.game_speed_slider = self._create_slider(
            "Game Speed", (50, 150), 1, 10, self._adjust_game_speed
        )

        self.special_mode_toggle = self._create_toggle(
            "Special Mode", (50, 200), self._toggle_special_mode
        )
    ...
    def _create_button(self, text, position, callback):
        ...
        button = pygame_gui.elements.UIButton(
            relative_rect=button_rect, text=text, manager=self.manager
        )
    ...
    def _create_scoreboard(self, position, initial_score):
        ...
        scoreboard = pygame_gui.elements.UILabel(
            relative_rect=scoreboard_rect, text=str(initial_score), manager=self.manager
        )
        return scoreboard

    def _create_bubble(self, text, position):
        bubble_rect = pygame.Rect(position, (150, 100))
        bubble = pygame_gui.elements.UITooltip(
            relative_rect=bubble_rect, text=text, manager=self.manager
        )
        return bubble
    ...
    def _create_checkbox(self, text, position):
        ...
        checkbox = pygame_gui.elements.UICheckbox(
            relative_rect=checkbox_rect, text=text, manager=self.manager
        )
        return checkbox
    ...
    def _create_toggle(self, text, position, start_state):
        ...
        toggle = pygame_gui.elements.UIButton(
            relative_rect=toggle_rect, text=text, manager=self.manager
        )
    ...
    def _create_textbox(self, position, width):
        ...
        textbox = pygame_gui.elements.UITextEntryLine(
            relative_rect=textbox_rect, manager=self.manager
        )
        return textbox
...
class GameExecution:
    ...
    def main():
        ...
        try:
            ...
            GameLogic.start_game_loop(game_logic, gui, clock, fps)
        finally:
