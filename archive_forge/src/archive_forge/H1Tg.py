...
# Define the structure of your NumPy arrays based on the properties
masses = []
colors = []
fusion_products= []
...
for element, properties in ELEMENTS.items():
    ...
    # Fusion photon energy needs to handle multiple fusion products
    fusion_products.append([product["probability"] for product in ["fusion_products"])
    em_charges.append(properties["em_charge"])
    em_radii.append(properties["em_radius"])
    strong_couplings.append(properties["strong_coupling"])
    strong_radii.append(properties["strong_radius"])

# Convert lists to NumPy arrays
masses = np.array(masses)
colors = np.array(colors)
# Fusion photon energies might need special handling if arrays are of different lengths
em_charges = np.array(em_charges)
em_radii = np.array(em_radii)
strong_couplings = np.array(strong_couplings)
strong_radii = np.array(strong_radii)


# Now you have NumPy arrays for each property
print("Masses:", masses)
print("Colors:", colors)
print("Electromagnetic Charges:", em_charges)
# Assuming fusion_products is a list of dictionaries as shown in the ELEMENTS data structure
fusion_probabilities = np.array(
    [product["probability"] for product in fusion_products], dtype=float
)
...
class Particle:
    ...
    def apply_forces(self, particles: np.ndarray):
        ...
        other_particles = np.array([p for p in particles if p.id != self.id])
    ...
    @functools.lru_cache(maxsize=1024)
    def fusion(self, other: "Particle") -> float:
        """
        ...
        if collision_energy > FUSION_THRESHOLD:
            distance_squared = np.sum((self.pos - other.pos) ** 2)
            radius_sum_squared = (self.strong_radius + other.strong_radius) ** 2

            if distance_squared < radius_sum_squared:
                fusion_products = np.array(self.properties["fusion_products"])
                probabilities = fusion_products[:, 1].astype(float)
                max_index = np.argmax(probabilities)
                new_element, probability, photon_energy = fusion_products[max_index]

                if random.random() < probability:
                    self.mass += other.mass - BINDING_ENERGY_FACTOR
                    self.element = new_element
                    self.properties = ELEMENTS[new_element]
                    self.color = self.properties["color"]
                    self.em_charge = self.properties["em_charge"]
                    self.em_radius = self.properties["em_radius"]
                    self.strong_coupling = self.properties["strong_coupling"]
                    self.strong_radius = self.properties["strong_radius"]
                    other.mass = 0  # The other particle is consumed during fusion
