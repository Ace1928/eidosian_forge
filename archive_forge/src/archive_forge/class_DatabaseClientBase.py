...
class DatabaseClientBase(abc.ABC):
    ...
    def __init__(self, settings: Optional[BaseSettings]=None, config: Optional[PostgresConfig]=None, **kwargs):
        """
        ...
        if settings:
            self.config = PostgresConfig.from_settings(settings)
        elif config:
            self.config = config
        self.post_init(**kwargs)
        self._kwargs = kwargs

    def configure(self, settings: Optional['AppSettings']=None, config: Optional[PostgresConfig]=None, template_name: Optional[str]=None, template_base_path: Optional[Union[str, Path]]=None, template_file_suffix: Optional[str]=None, **kwargs):
        """
        ...
        if settings:
            ...
            self.config = config
        if template_name:
            self._sql_template_kwargs['name'] = template_name
        if template_base_path:
            self._sql_template_kwargs['base_path'] = template_base_path
        if template_file_suffix:
            self._sql_template_kwargs['file_suffix'] = template_file_suffix
        if kwargs:
            self._kwargs = update_dict(self._kwargs, kwargs)
    ...
    def engine(self) -> AsyncEngine:
        ...
        if self._engine_rw is None:
            ...
            self._engine_rw = create_async_engine(**self.config.get_engine_kwargs(readonly=False, verbose=self.is_verbose_for('postgres_url'), **self._kwargs))
    ...
    def get_database(self, readonly: Optional[bool]=None, auto_commit: Optional[bool]=None, raise_errors: Optional[bool]=None) -> Callable[..., AsyncSession]:
        """
    ...
    async def session(self, readonly: Optional[bool]=None, auto_commit: Optional[bool]=None, raise_errors: Optional[bool]=None, auto_rollback: Optional[bool]=True) -> AsyncGenerator[AsyncSession, None]:
        """
    ...
    @contextlib.asynccontextmanager
    async def session_readonly(self, raise_errors: Optional[bool]=None) -> AsyncGenerator[AsyncSession, None]:
        """
        Context manager for the database session
        """
        try:
    ...
    @contextlib.asynccontextmanager
    async def remote_session(self, name: str, readonly: Optional[bool]=None, auto_commit: Optional[bool]=None, raise_errors: Optional[bool]=None, auto_rollback: Optional[bool]=True) -> AsyncGenerator[AsyncSession, None]:
        """
        Context manager for the remote database session
        """
        if name not in self.remote_connections:
            raise ValueError(f'Remote Connection {name} does not exist')
    ...
    async def wait_for_ready(self, max_attempts: Optional[int]=10, interval: Optional[float]=5.0):
        """
    ...
    async def cast_result_to_object(self, result: 'Result', obj: 'DeclarativeBase', fields: Optional[List[str]], **kwargs) -> List['ModelType']:
        """
    ...
    async def bulk_commit_and_refresh(self, db: AsyncSession, orm_objects: List['ModelType'], **kwargs) -> List['ModelType']:
        """
    ...
    async def upgrade_extensions(self, extensions: Optional[List[str]]=None, **kwargs):
        """
    ...
    async def execute_sql(self, *ops: str, db: Optional[AsyncSession]=None, readonly: Optional[bool]=None, auto_commit: Optional[bool]=None, raise_errors: Optional[bool]=None, verbose: Optional[bool]=None, filters: Optional[Dict[str, Union[int, float, datetime.datetime, Dict, List, Any]]]=None, **kwargs: Any) -> 'Result':
        """
        ...
        if verbose:
            logger.info(sql_query, prefix='Executing SQL Query')
        if db is not None:
    ...
    async def database_exists(self, db_name: str) -> bool:
        ...
        """
        statement = self.text(f"SELECT 1 FROM pg_database WHERE datname = '{db_name}';")
        async with self.session() as session:
