...
class HyperImageAugment(hypermodel.HyperModel):
    ...
    def __init__(self, input_shape=None, input_tensor=None, rotate=0.5, translate_x=0.4, translate_y=0.4, contrast=0.3, augment_layers=3, **kwargs):
        ...
        if augment_layers:
            ...
            if not (isinstance(augment_layers_min, int) and isinstance(augment_layers_max, int)):
                raise ValueError(f'Keyword argument `augment_layers` must be int, but received {augment_layers}.')
