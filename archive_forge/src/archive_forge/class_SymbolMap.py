...
class SymbolMap(object):
    ...
    def addSymbols(self, obj_symbol_tuples):
        ...
        for obj, symbol in obj_symbol_tuples:
            ...
            nSymbols += 1
        if nSymbols != len(self.bySymbol):
            raise RuntimeError('SymbolMap.addSymbols(): duplicate symbol.  SymbolMap likely in an inconsistent state')
        if len(self.byObject) != len(self.bySymbol):
            raise RuntimeError('SymbolMap.addSymbols(): duplicate object.  SymbolMap likely in an inconsistent state')
    ...
    def getSymbol(self, obj, labeler=None, *args):
        ...
        if symbol in self.bySymbol:
            if obj is self.bySymbol[symbol]:
                return symbol
            raise RuntimeError("Duplicate symbol '%s' already associated with component '%s' (conflicting component: '%s')" % (symbol, self.bySymbol[symbol].name, obj.name))
        self.bySymbol[symbol] = obj
    ...
    def alias(self, obj, name):
        ...
        if name in self.aliases:
            ...
            if old_object is obj:
                ...
                raise RuntimeError("Duplicate alias '%s' already associated with component '%s' (conflicting component: '%s')" % (name, 'UNKNOWN' if old_object is None else old_object.name, obj.name))
        else:
