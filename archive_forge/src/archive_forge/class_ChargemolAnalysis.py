...
class ChargemolAnalysis:
    ...
    def __init__(self, path: str | Path | None=None, atomic_densities_path: str | Path | None=None, run_chargemol: bool=True) -> None:
        ...
        if run_chargemol and (not CHARGEMOL_EXE):
            raise OSError('ChargemolAnalysis requires the Chargemol executable to be in PATH. Please download the library at https://sourceforge.net/projects/ddec/filesand follow the instructions.')
        ...
        if run_chargemol and (not (self._chgcar_path and self._potcar_path and self._aeccar0_path and self._aeccar2_path)):
