...
def data_service_dataset_v3(dataset_id: _atypes.TensorFuzzingAnnotation[_atypes.Int64], processing_mode: _atypes.TensorFuzzingAnnotation[_atypes.String], address: _atypes.TensorFuzzingAnnotation[_atypes.String], protocol: _atypes.TensorFuzzingAnnotation[_atypes.String], job_name: _atypes.TensorFuzzingAnnotation[_atypes.String], consumer_index: _atypes.TensorFuzzingAnnotation[_atypes.Int64], num_consumers: _atypes.TensorFuzzingAnnotation[_atypes.Int64], max_outstanding_requests: _atypes.TensorFuzzingAnnotation[_atypes.Int64], iteration_counter: _atypes.TensorFuzzingAnnotation[_atypes.Resource], output_types, output_shapes, uncompress_fn, task_refresh_interval_hint_ms: int=-1, data_transfer_protocol: str='', target_workers: str='AUTO', uncompress: bool=False, cross_trainer_cache_options: str='', name=None) -> _atypes.TensorFuzzingAnnotation[_atypes.Variant]:
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'DataServiceDatasetV3', name, dataset_id, processing_mode, address, protocol, job_name, consumer_index, num_consumers, max_outstanding_requests, iteration_counter, 'task_refresh_interval_hint_ms', task_refresh_interval_hint_ms, 'output_types', output_types, 'output_shapes', output_shapes, 'data_transfer_protocol', data_transfer_protocol, 'target_workers', target_workers, 'uncompress', uncompress, 'uncompress_fn', uncompress_fn, 'cross_trainer_cache_options', cross_trainer_cache_options)
            return _result
            ...
            pass
        try:
            return data_service_dataset_v3_eager_fallback(dataset_id, processing_mode, address, protocol, job_name, consumer_index, num_consumers, max_outstanding_requests, iteration_counter, task_refresh_interval_hint_ms=task_refresh_interval_hint_ms, output_types=output_types, output_shapes=output_shapes, data_transfer_protocol=data_transfer_protocol, target_workers=target_workers, uncompress=uncompress, uncompress_fn=uncompress_fn, cross_trainer_cache_options=cross_trainer_cache_options, name=name, ctx=_ctx)
        except _core._SymbolicException:
            pass
    if not isinstance(output_types, (list, tuple)):
        raise TypeError("Expected list for 'output_types' argument to 'data_service_dataset_v3' Op, not %r." % output_types)
    output_types = [_execute.make_type(_t, 'output_types') for _t in output_types]
    if not isinstance(output_shapes, (list, tuple)):
        raise TypeError("Expected list for 'output_shapes' argument to 'data_service_dataset_v3' Op, not %r." % output_shapes)
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('DataServiceDatasetV3', dataset_id=dataset_id, processing_mode=processing_mode, address=address, protocol=protocol, job_name=job_name, consumer_index=consumer_index, num_consumers=num_consumers, max_outstanding_requests=max_outstanding_requests, iteration_counter=iteration_counter, output_types=output_types, output_shapes=output_shapes, uncompress_fn=uncompress_fn, task_refresh_interval_hint_ms=task_refresh_interval_hint_ms, data_transfer_protocol=data_transfer_protocol, target_workers=target_workers, uncompress=uncompress, cross_trainer_cache_options=cross_trainer_cache_options, name=name)
    _result = _outputs[:]
    if _execute.must_record_gradient():
        _attrs = ('task_refresh_interval_hint_ms', _op._get_attr_int('task_refresh_interval_hint_ms'), 'output_types', _op.get_attr('output_types'), 'output_shapes', _op.get_attr('output_shapes'), 'data_transfer_protocol', _op.get_attr('data_transfer_protocol'), 'target_workers', _op.get_attr('target_workers'), 'uncompress', _op._get_attr_bool('uncompress'), 'uncompress_fn', _op.get_attr('uncompress_fn'), 'cross_trainer_cache_options', _op.get_attr('cross_trainer_cache_options'))
        _inputs_flat = _op.inputs
        _execute.record_gradient('DataServiceDatasetV3', _inputs_flat, _attrs, _result)
    _result, = _result
    return _result