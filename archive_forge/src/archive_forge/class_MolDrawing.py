...
class MolDrawing(object):
    ...
    def __init__(self, canvas=None, drawingOptions=None):
        ...
        if self.drawingOptions.bgColor is not None:
            self.canvas.addCanvasPolygon(((0, 0), (canvas.size[0], 0), (canvas.size[0], canvas.size[1]), (0, canvas.size[1])), color=self.drawingOptions.bgColor, fill=True, stroke=False)
    ...
    def transformPoint(self, pos):
        ...
        res[1] = self.canvasSize[1] - ((pos[1] + self.molTrans[1]) * self.currDotsPerAngstrom * self.drawingOptions.useFraction + self.drawingTrans[1])
        return res
