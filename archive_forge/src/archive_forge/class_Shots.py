...
class Shots:
    ...
    **Examples**
    ...
    _SHOT_ERROR = ValueError('Shots must be a single positive integer, a tuple pair of the form (shots, copies), or a sequence of these types.')
    _frozen = False
    ...
    def __setattr__(self, name, value):
        if self._frozen:
            raise AttributeError('Shots is an immutable class. Consider creating a new instance if you need different shot values.')
    ...
    def __eq__(self, other):
        ...
        return isinstance(other, Shots) and self.total_shots == other.total_shots and (self.shot_vector == other.shot_vector)
    ...
    def __mul__(self, scalar):
        ...
        if self.total_shots is None:
            return self
        scaled_shot_vector = tuple((ShotCopies(int(i.shots * scalar), i.copies) for i in self.shot_vector))
