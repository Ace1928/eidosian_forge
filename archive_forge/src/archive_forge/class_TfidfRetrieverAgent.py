...
class TfidfRetrieverAgent(Agent):
    ...
    def add_cmdline_args(parser):
        ...
        parser.add_argument('--retriever-numworkers', type=int, default=None, help='Number of CPU processes (for tokenizing, etc)')
        parser.add_argument('--retriever-ngram', type=int, default=2, help='Use up to N-size n-grams (e.g. 2 = unigrams + bigrams)')
        parser.add_argument('--retriever-hashsize', type=int, default=int(math.pow(2, 24)), help='Number of buckets to use for hashing ngrams')
        parser.add_argument('--retriever-tokenizer', type=str, default='simple', help='String option specifying tokenizer type to use (e.g. "corenlp")')
        parser.add_argument('--retriever-num-retrieved', default=5, type=int, help='How many docs to retrieve.')
        parser.add_argument('--retriever-mode', choices=['keys', 'values'], default='values', help='Whether to retrieve the stored key or the stored value.')
        parser.add_argument('--remove-title', type='bool', default=False, help='Whether to remove the title from the retrieved passage')
        parser.add_argument('--retriever-mode', choices=['keys', 'values'], default='values', help='Whether to retrieve the stored key or the stored value. For example, if you want to return the text of an example, use keys here; if you want to return the label, use values here.')
        parser.add_argument('--index-by-int-id', type='bool', default=True, help='Whether to index into database by doc id as an integer. This                   defaults to true for DBs built using ParlAI; for the DrQA                   wiki dump, it is necessary to set this to False to                   index into the DB appropriately')
        parser.add_argument('--tfidf-context-length', default=-1, type=int, help='Number of past utterances to remember when building flattened batches of data in multi-example episodes.')
        parser.add_argument('--tfidf-include-labels', default=True, type='bool', help='Specifies whether or not to include labels as past utterances when building flattened batches of data in multi-example episodes.')
    ...
    def __init__(self, opt, shared=None):
        ...
        self.tfidf_args = AttrDict({'db_path': opt['retriever_dbpath'], 'out_dir': opt['retriever_tfidfpath'], 'ngram': opt['retriever_ngram'], 'hash_size': opt['retriever_hashsize'], 'tokenizer': opt['retriever_tokenizer'], 'num_workers': opt['retriever_numworkers']})
        ...
        if os.path.exists(self.tfidf_path + '.npz'):
            if shared is None:
                self.ranker = TfidfDocRanker(tfidf_path=opt['retriever_tfidfpath'], strict=False)
            else:
    ...
    def act(self):
        ...
        if 'text' in obs:
            self.rebuild()
            doc_ids, doc_scores = self.ranker.closest_docs(obs['text'], self.opt.get('retriever_num_retrieved', 5))
            if False and obs.get('label_candidates'):
                ...
                c_ids, c_scores = self.ranker.closest_docs(obs['text'], self.opt.get('retriever_num_retrieved', 5), matrix=self.cands_hash[cands_id][0])
                ...
                reply['text'] = choice(['Can you say something more interesting?', 'Why are you being so short with me?', 'What are you really thinking?', 'Can you expand on that?'])
        return reply