...
class PipeRPCWrapper(nn.Module):
    ...
    def __init__(self, *args: Any, **kwargs: Any):
        ...
        self.lock = Lock()
        if True:
            assert self.group == get_pipeline_parallel_group(), "Can't pickle groups, so group must be `get_pipeline_parallel_group()`"
            kwargs['group'] = None
        else:
    ...
    def foreach_worker(self, callback: Callable[[Any, AsyncPipe], None], ctx: Any=None, *, include_self: bool=False) -> None:
    ...
    def forward(self, tensor: TensorOrTensors) -> TensorOrTensors:
        ...
        futures = [rpc.rpc_async(self._get_rpc_name(rank), self._model_forward, args=(self.model.training, shape, dtype)) for rank in range(1, self.group.size())]
