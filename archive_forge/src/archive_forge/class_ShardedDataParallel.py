...
class ShardedDataParallel(nn.Module):
    ...
    def __init__(self, module: nn.Module, sharded_optimizer: Union[OSS, List[OSS]], process_group: Any=None, broadcast_buffers: bool=True, sync_models_at_startup: bool=True, reduce_buffer_size: int=2 ** 23, auto_refresh_trainable: bool=True, reduce_fp16: bool=False, warn_on_trainable_params_changed: bool=True):
        super().__init__()
        self.module = module
        ...
        if reduce_buffer_size > 0 and reduce_fp16:
            ...
            logging.warning('fp16 gradient reduction is not compatible with reduction buffers, which are requested. fp16 grad reduction is deactivated.')
        ...
        self._world_size_scaling = 1.0 / dist.get_world_size(self._process_group)
        self._reference_global_rank = get_global_rank(self._process_group, 0)
        self._rank = dist.get_rank(self._process_group)
        self._global_rank = get_global_rank(self._process_group, self._rank)
        self._local_to_global_rank = [get_global_rank(self._process_group, i) for i in range(dist.get_world_size(self._process_group))]
        ...
        assert len(distinct_device_types) == 1, "ShardedDataParallel's input module must be on the same type of devices, but input module parameters are located on {} different device types.".format(distinct_device_types)
        self.device_type = list(distinct_device_types)[0]
        self._all_params = list(chain(*[sum([sum(p, []) for p in optim._per_device_params.values()], []) for optim in self._sharded_optimizers]))
        ...
        logging.info('ShardedDDP bucket size: {:.2f}M parameters, model size {:.2f}M parameters'.format(self._buffer_max_size / 2 ** 20, model_size / 2 ** 20))
    ...
    def to(self, device: Optional[torch.device], dtype: Optional[torch.dtype]=None, non_blocking: bool=False) -> 'ShardedDataParallel':
        """
        ...
        assert device is None or len(self._buckets.keys()) == 0 or device.type in map(lambda x: x.type, self._buckets.keys()), 'Changing devices is not supported, because this would break OSSs state'
        assert len(self._buckets.keys()) < 2, 'Several devices specified to begin with, incompatible with setting a single device here'
    ...
    def reduce(self) -> None:
        ...
        """
        assert functools.reduce(lambda x, y: x or y, self._grad_to_be_reduced, False), 'No grads waiting to be reduced, maybe that this was called twice or there was no BW pass ?'
        ...
        if not self._use_buckets or not self._should_bucket_grad[index]:
            ...
            def reduce(*_: Any) -> None:
                if not self._should_accumulate_grads and self._grad_to_be_reduced[index]:
                    ...
                    self._work_handles.append(Workhandle(handle=dist.reduce(tensor=param.grad.data, dst=self._local_to_global_rank[dst_rank], group=self._process_group, async_op=True), callback=cleanup))
    ...
    def _sync_params_and_buffers(self) -> None:
        ...
        for t in self.module.state_dict().values():
            work_handles.append(dist.broadcast(t, src=self._reference_global_rank, group=self._process_group, async_op=True))
        if self._backend != dist.Backend.NCCL:
            _ = list(filter(lambda x: x.wait(), work_handles))
        elif work_handles:
            work_handles[-1].wait()
