...
class GroverOperator(Operation):
    ...
    **Example**
    ...
    def __init__(self, wires=None, work_wires=None, id=None):
        ...
        self._hyperparameters = {'n_wires': len(wires), 'work_wires': Wires(work_wires) if work_wires is not None else Wires([])}
    ...
    @staticmethod
    def compute_decomposition(wires, work_wires, **kwargs):
        ...
        op_list.append(MultiControlledX(control_values=ctrl_str, wires=wires, work_wires=work_wires))
        op_list.append(PauliZ(wires[-1]))
        for wire in wires[:-1]:
            op_list.append(Hadamard(wire))
