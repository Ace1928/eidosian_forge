...
class WriteManager:
    ...
    def __init__(self, settings: SettingsStatic, record_q: 'Queue[pb.Record]', result_q: 'Queue[pb.Result]', sender_q: 'Queue[pb.Record]', interface: InterfaceQueue, context_keeper: context.ContextKeeper):
        ...
        self._use_flow_control = not (self._settings._flow_control_disabled or self._settings._offline)

    def open(self) -> None:
        self._ds = datastore.DataStore()
        self._ds.open_for_write(self._settings.sync_file)
        self._flow_control = flow_control.FlowControl(settings=self._settings, write_record=self._write_record, forward_record=self._forward_record, pause_marker=self._pause_marker, recover_records=self._recover_records)
    ...
    def write(self, record: 'pb.Record') -> None:
        ...
        write_handler: Callable[[pb.Record], None] = getattr(self, writer_str, self._write)
