...
class PacketMirroring(object):
    ...
    def Create(self, packet_mirroring=None, only_generate_request=False, is_async=False):
        ...
        operation_ref = self._registry.Parse(result.name, collection='compute.regionOperations', params={'project': self.ref.project, 'region': self.ref.region}, api_version='v1')
        log.CreatedResource(operation_ref, kind='packet mirroring [{0}]'.format(self.ref.Name()), is_async=True, details='Run the [gcloud compute operations describe] command to check the status of this operation.')
        return result
    ...
    def Update(self, packet_mirroring=None, only_generate_request=False, is_async=False, cleared_fields=None):
        ...
        if errors_to_collect:
            raise exceptions.MultiError(errors_to_collect)
        operation_ref = self._registry.Parse(result.name, collection='compute.regionOperations', params={'project': self.ref.project, 'region': self.ref.region}, api_version='v1')
        log.UpdatedResource(operation_ref, kind='packet mirroring [{0}]'.format(self.ref.Name()), is_async=True, details='Run the [gcloud compute operations describe] command to check the status of this operation.')
        return result
