  ...
  *{command}* uploads a named set into a Compute Engine router.
  """
    ...
    def Args(cls, parser):
        ...
        parser.add_argument('--set-name', help='Name of the named set to add/replace.')
        parser.add_argument('--file-name', required=True, help='Local path to the file defining the named set')
    ...
    """
        ...
        router_ref = UploadNamedSet.ROUTER_ARG.ResolveAsResource(args, holder.resources, scope_lister=compute_flags.GetDefaultScopeLister(client))
        named_set = self.ParseNamedSetFromFile(args.file_name, args.file_format, client.messages)
        self.EnsureSameSetName(args.set_name, named_set)
        request = (client.apitools_client.routers, 'UpdateNamedSet', client.messages.ComputeRoutersUpdateNamedSetRequest(**router_ref.AsDict(), namedSet=named_set))
    ...
    def ParseFile(self, input_file, file_format):
        ...
        try:
            ...
            msg = 'Unable to read named set config from specified file [{0}] because {1}'.format(input_file, exp)
