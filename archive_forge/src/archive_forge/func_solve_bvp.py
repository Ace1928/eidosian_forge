...
def solve_bvp(fun, bc, x, y, p=None, S=None, fun_jac=None, bc_jac=None, tol=0.001, max_nodes=1000, verbose=0, bc_tol=None):
    ...
    if y.shape[1] != x.shape[0]:
        raise ValueError(f'`y` is expected to have {x.shape[0]} columns, but actually has {y.shape[1]}.')
    if p is None:
        p = np.array([])
    else:
    ...
    if S is not None:
        ...
        if S.shape != (n, n):
            raise ValueError(f'`S` is expected to have shape {(n, n)}, but actually has {S.shape}')
    ...
    f = fun_wrapped(x, y, p)
    if f.shape != y.shape:
        raise ValueError(f'`fun` return is expected to have shape {y.shape}, but actually has {f.shape}.')
    bc_res = bc_wrapped(y[:, 0], y[:, -1], p)
    if bc_res.shape != (n + k,):
        raise ValueError(f'`bc` return is expected to have shape {(n + k,)}, but actually has {bc_res.shape}.')
    status = 0
    iteration = 0
    if verbose == 2:
    ...
    if verbose > 0:
        if status == 0:
            print(f'Solved in {iteration} iterations, number of nodes {x.shape[0]}. \nMaximum relative residual: {max_rms_res:.2e} \nMaximum boundary residual: {max_bc_res:.2e}')
        elif status == 1:
            print(f'Number of nodes is exceeded after iteration {iteration}. \nMaximum relative residual: {max_rms_res:.2e} \nMaximum boundary residual: {max_bc_res:.2e}')
        elif status == 2:
            print(f'Singular Jacobian encountered when solving the collocation system on iteration {iteration}. \nMaximum relative residual: {max_rms_res:.2e} \nMaximum boundary residual: {max_bc_res:.2e}')
        elif status == 3:
            print(f'The solver was unable to satisfy boundary conditions tolerance on iteration {iteration}. \nMaximum relative residual: {max_rms_res:.2e} \nMaximum boundary residual: {max_bc_res:.2e}')
    if p.size == 0:
        p = None
