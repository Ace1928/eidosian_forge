...
@reader
def read_lammps_data(fileobj, Z_of_type=None, style='full', sort_by_id=False, units='metal'):
    ...
    angles_in = []
    dihedrals_in = []
    sections = ['Atoms', 'Velocities', 'Masses', 'Charges', 'Ellipsoids', 'Lines', 'Triangles', 'Bodies', 'Bonds', 'Angles', 'Dihedrals', 'Impropers', 'Impropers Pair Coeffs', 'PairIJ Coeffs', 'Pair Coeffs', 'Bond Coeffs', 'Angle Coeffs', 'Dihedral Coeffs', 'Improper Coeffs', 'BondBond Coeffs', 'BondAngle Coeffs', 'MiddleBondTorsion Coeffs', 'EndBondTorsion Coeffs', 'AngleTorsion Coeffs', 'AngleAngleTorsion Coeffs', 'BondBond13 Coeffs', 'AngleAngle Coeffs']
    header_fields = ['atoms', 'bonds', 'angles', 'dihedrals', 'impropers', 'atom types', 'bond types', 'angle types', 'dihedral types', 'improper types', 'extra bond per atom', 'extra angle per atom', 'extra dihedral per atom', 'extra improper per atom', 'extra special per atom', 'ellipsoids', 'lines', 'triangles', 'bodies', 'xlo xhi', 'ylo yhi', 'zlo zhi', 'xy xz yz']
        ...
        if section is not None:
            ...
            if section == 'Atoms':
                ...
                if style == 'full' and (len(fields) == 7 or len(fields) == 10):
                    ...
                    pos_in[id] = (int(fields[2]), float(fields[3]), float(fields[4]), float(fields[5]))
                    mol_id_in[id] = int(fields[1])
                    if len(fields) == 9:
                        travel_in[id] = (int(fields[6]), int(fields[7]), int(fields[8]))
                elif style == 'charge' and (len(fields) == 6 or len(fields) == 9):
                    pos_in[id] = (int(fields[1]), float(fields[3]), float(fields[4]), float(fields[5]))
                    charge_in[id] = float(fields[2])
                    if len(fields) == 9:
                        travel_in[id] = (int(fields[6]), int(fields[7]), int(fields[8]))
                else:
                    raise RuntimeError("Style '{}' not supported or invalid number of fields {}".format(style, len(fields)))
                ...
                dihedrals_in.append((int(fields[1]), int(fields[2]), int(fields[3]), int(fields[4]), int(fields[5])))
    cell = np.zeros((3, 3))
    cell[0, 0] = xhi - xlo
    cell[1, 1] = yhi - ylo
    cell[2, 2] = zhi - zlo
    ...
    at = Atoms(positions=positions, numbers=numbers, masses=masses, cell=cell, pbc=[True, True, True])
    if velocities is not None:
