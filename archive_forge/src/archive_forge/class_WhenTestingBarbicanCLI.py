...
class WhenTestingBarbicanCLI(testtools.TestCase):
    ...
    def setUp(self):
        ...
        self.responses.get('http://localhost:9311/v1/', json={'version': {'id': 'v1', 'status': 'CURRENT', 'min_version': '1.0', 'max_version': '1.1', 'links': [{'rel': 'self', 'href': 'http://192.168.122.110/key-manager/v1/'}, {'rel': 'describedby', 'type': 'text/html', 'href': 'https://docs.openstack.org/'}]}})
        self.responses.get('http://localhost:9311/', json={'versions': [{'id': 'v1', 'status': 'CURRENT', 'min_version': '1.0', 'max_version': '1.1', 'links': [{'rel': 'self', 'href': 'http://localhost:9311/v1/'}, {'rel': 'describedby', 'type': 'text/html', 'href': 'https://docs.openstack.org/'}]}]})
        self.captured_stdout = io.StringIO()
        self.captured_stderr = io.StringIO()
        self.barbican = Barbican(stdout=self.captured_stdout, stderr=self.captured_stderr)
    ...
    def test_endpoint_filter_kwargs_set_correctly(self):
        auth_args = '--no-auth --endpoint http://localhost:9311/ --os-project-id project1'
        endpoint_filter_args = '--interface private --service-type custom-type --service-name Burrbican --region-name RegionTwo --barbican-api-version v1'
    ...
    def test_should_prevent_mutual_exclusive_file_opt(self):
        args = '--no-auth --endpoint {0} --os-tenant-id {1}--file foo --payloadsecret get'.format(self.endpoint, self.project_id)
        list_secrets_url = '{0}/v1/secrets'.format(self.endpoint.rstrip('/'))
        self.responses.get(list_secrets_url, json={'secrets': [], 'total': 0})
        client = self.create_and_assert_client(args)
        secret_list = client.secrets.list()
