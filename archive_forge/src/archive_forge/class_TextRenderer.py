...
class TextRenderer(renderer.Renderer):
  ...
  """
    ...
    """
        ...
        self._out.write(self.Font(renderer.BOLD) + heading + self.Font(renderer.BOLD) + '\n')
        if level == 1:
            self._out.write('\n')
        self.Blank()
        self._level = 0
    ...
    """
        ...
        if not level:
            ...
            if definition:
                ...
                self._out.write(' ' * self._indent[level].second_line_indent + definition + '\n')
            else:
            ...
            self._out.write(' ' * self._indent[level].second_line_indent + self._bullet[(level - 1) % len(self._bullet)])
