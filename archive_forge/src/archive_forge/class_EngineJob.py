...
class EngineJob(abstract_job.AbstractJob):
    ...
    def __init__(self, project_id: str, program_id: str, job_id: str, context: 'engine_base.EngineContext', _job: Optional[quantum.QuantumJob]=None, result_type: ResultType=ResultType.Program, job_result_future: Optional[duet.AwaitableFuture[Union[quantum.QuantumResult, quantum.QuantumJob]]]=None) -> None:
    ...
    async def _get_job_async(self, return_run_context: bool=False) -> quantum.QuantumJob:
        return await self.context.client.get_job_async(self.project_id, self.program_id, self.job_id, return_run_context)
    ...
    def set_description(self, description: str) -> 'EngineJob':
        ...
        Returns:
             This EngineJob.
        """
        self._job = self.context.client.set_job_description(self.project_id, self.program_id, self.job_id, description)
        return self
    ...
    def set_labels(self, labels: Dict[str, str]) -> 'EngineJob':
        ...
        Returns:
             This EngineJob.
        """
        self._job = self.context.client.set_job_labels(self.project_id, self.program_id, self.job_id, labels)
        return self
    ...
    def add_labels(self, labels: Dict[str, str]) -> 'EngineJob':
        ...
        Returns:
             This EngineJob.
        """
        self._job = self.context.client.add_job_labels(self.project_id, self.program_id, self.job_id, labels)
        return self
    ...
    def remove_labels(self, keys: List[str]) -> 'EngineJob':
        ...
        Returns:
            This EngineJob.
        """
        self._job = self.context.client.remove_job_labels(self.project_id, self.program_id, self.job_id, keys)
        return self

    def processor_ids(self) -> List[str]:
        """Returns the processor ids provided when the job was created."""
        return [engine_client._ids_from_processor_name(p)[1] for p in self._inner_job().scheduling_config.processor_selector.processor_names]
    ...
    async def _await_result_async(self) -> quantum.QuantumResult:
        ...
        response = await self.context.client.get_job_results_async(self.project_id, self.program_id, self.job_id)
        return response
    ...
    def __str__(self) -> str:
        return f"EngineJob(project_id='{self.project_id}', program_id='{self.program_id}', job_id='{self.job_id}')"