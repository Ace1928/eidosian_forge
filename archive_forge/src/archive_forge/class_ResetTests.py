...
class ResetTests(PorcelainTestCase):
    ...
    def test_hard_commit(self):
        ...
        with open(fullpath, 'w') as f:
            f.write('BAR')
        porcelain.add(self.repo.path, paths=[fullpath])
        sha = porcelain.commit(self.repo.path, message=b'Some message', committer=b'Jane <jane@example.com>', author=b'John <john@example.com>')
        with open(fullpath, 'wb') as f:
            f.write(b'BAZ')
        porcelain.add(self.repo.path, paths=[fullpath])
        porcelain.commit(self.repo.path, message=b'Some other message', committer=b'Jane <jane@example.com>', author=b'John <john@example.com>')
