...
class MultipartMimeTest(common.HeatTestCase):
    ...
    def init_config(self, parts=None, group='Heat::Ungrouped'):
        parts = parts or []
        stack = parser.Stack(self.ctx, 'software_config_test_stack', template.Template({'HeatTemplateFormatVersion': '2012-12-12', 'Resources': {'config_mysql': {'Type': 'OS::Heat::MultipartMime', 'Properties': {'group': group, 'parts': parts}}}}))
    ...
    def _test_create(self, group='Heat::Ungrouped'):
        ...
        self.assertEqual({'name': self.config.physical_resource_name(), 'config': self.config.message, 'group': group}, kwargs)
    ...
    def test_get_message_text(self):
        parts = [{'config': '1e0e5a60-2843-4cfd-9137-d90bdf18eef5', 'type': 'text'}]
        self.init_config(parts=parts)
