...
class StandardDecorator:
    ...
    async def validate_arguments(self, func: F, *args, **kwargs) -> None:
        ...
        # Adjust for methods by checking if the function is bound to a class instance
        if inspect.ismethod(func) or (
            hasattr(func, "__self__") and func.__self__ is not None
        ):
            # For bound methods, the first argument is 'self' for instance methods or 'cls' for class methods
            # Remove the 'self' or 'cls' reference from args before binding
