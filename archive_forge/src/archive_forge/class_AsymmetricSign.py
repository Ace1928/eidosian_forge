...
class AsymmetricSign(base.Command):
  ...
  """
    ...
    def _VerifyResponseIntegrityFields(self, req, resp, use_digest=True):
        ...
        elif not resp.verifiedDataCrc32c:
            raise e2e_integrity.ClientSideIntegrityVerificationError(e2e_integrity.GetRequestToServerCorruptedErrorMessage())
    ...
    def Run(self, args):
        ...
        try:
            resp = client.projects_locations_keyRings_cryptoKeys_cryptoKeyVersions.AsymmetricSign(req)
        except apitools_exceptions.HttpBadRequestError as error:
            e2e_integrity.ProcessHttpBadRequestError(error)
        if self._PerformIntegrityVerification(args):
            self._VerifyResponseIntegrityFields(req, resp, use_digest=self._SignOnDigest(args))
        try:
            log.WriteToFileOrStdout(args.signature_file, resp.signature, overwrite=True, binary=True, private=True)
        except files.Error as e:
