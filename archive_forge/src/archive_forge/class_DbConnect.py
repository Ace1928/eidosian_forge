...
class DbConnect(object):
  ...
  """
    ...
    """
        return DbInfo.GetTableNames(self.dbName, self.user, self.password, includeViews=includeViews, cn=self.cn)
    ...
    """
        ...
        return DbInfo.GetColumnNames(self.dbName, table, self.user, self.password, join=join, what=what, cn=self.cn)
    ...
    """
        ...
        return DbUtils.GetData(self.dbName, table, fieldString=fields, whereString=where, user=self.user, password=self.password, removeDups=removeDups, join=join, cn=self.cn, transform=transform, randomAccess=randomAccess, **kwargs)
