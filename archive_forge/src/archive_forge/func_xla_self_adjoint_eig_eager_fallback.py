...
def xla_self_adjoint_eig_eager_fallback(a: _atypes.TensorFuzzingAnnotation[TV_XlaSelfAdjointEig_T], lower: bool, max_iter: int, epsilon: float, name, ctx):
    ...
    _inputs_flat = [a]
    _attrs = ('lower', lower, 'max_iter', max_iter, 'epsilon', epsilon, 'T', _attr_T)
    _result = _execute.execute(b'XlaSelfAdjointEig', 2, inputs=_inputs_flat, attrs=_attrs, ctx=ctx, name=name)
