...
class OrderEnforcing(gym.Wrapper):
    ...
    def render(self, *args, **kwargs):
        ...
        if not self._disable_render_order_enforcing and (not self._has_reset):
            raise ResetNeeded('Cannot call `env.render()` before calling `env.reset()`, if this is a intended action, set `disable_render_order_enforcing=True` on the OrderEnforcer wrapper.')
