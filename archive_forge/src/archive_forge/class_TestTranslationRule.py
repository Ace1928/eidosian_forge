...
class TestTranslationRule(common.HeatTestCase):
    ...
    def test_add_rule_exist(self):
        ...
        self.assertEqual([{'red': 'blue'}, {'red': 'dak'}], tran.resolved_translations['far'])
    ...
    def test_resolve_rule_list_with_function(self):
        ...
        data = {'far': [{'red': 'blue'}, {'red': join_func}]}
        props = properties.Properties(schema, data)
        rule = translation.TranslationRule(props, translation.TranslationRule.RESOLVE, ['far', 'red'], client_plugin=client_plugin, finder='find_name_id')
        tran = translation.Translation(props)
        tran.set_rules([rule])
    ...
    def test_resolve_rule_list_strings(self):
        ...
        data = {'far': ['one', 'rose']}
        schema = {'far': properties.Schema(properties.Schema.LIST, schema=properties.Schema(properties.Schema.STRING))}
        props = properties.Properties(schema, data)
    ...
    def test_property_json_param_correct_translation(self):
        ...
        schema = {'far': properties.Schema(properties.Schema.MAP, schema={'bar': properties.Schema(properties.Schema.STRING), 'dar': properties.Schema(properties.Schema.STRING)})}
        ...
        rule = translation.TranslationRule(props, translation.TranslationRule.REPLACE, ['far', 'bar'], value_path=['far', 'dar'])
        tran = translation.Translation(props)
        tran.set_rules([rule])
    ...
    def test_list_list_add_translation_rule(self):
        schema = {'far': properties.Schema(properties.Schema.LIST, schema=properties.Schema(properties.Schema.MAP, schema={'bar': properties.Schema(properties.Schema.LIST, schema=properties.Schema(properties.Schema.STRING)), 'car': properties.Schema(properties.Schema.STRING)}))}
        data = {'far': [{'bar': ['shar'], 'car': 'man'}, {'car': 'first'}]}
        props = properties.Properties(schema, data, resolver=function.resolve)
        rule = translation.TranslationRule(props, translation.TranslationRule.ADD, ['far', 'bar'], value_name='car')
        tran = translation.Translation(props)
        tran.set_rules([rule])
        self.assertTrue(tran.has_translation('far.0.bar'))
        result = tran.translate('far.0.bar', props['far'][0]['bar'], props['far'][0])
        self.assertEqual(['shar', 'man'], result)
        self.assertEqual(['shar', 'man'], tran.resolved_translations['far.0.bar'])
