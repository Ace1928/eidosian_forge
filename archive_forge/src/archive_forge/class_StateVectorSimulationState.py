...
class StateVectorSimulationState(SimulationState[_BufferedStateVector]):
    ...
    def __init__(self, *, available_buffer: Optional[np.ndarray]=None, prng: Optional[np.random.RandomState]=None, qubits: Optional[Sequence['cirq.Qid']]=None, initial_state: Union[np.ndarray, 'cirq.STATE_VECTOR_LIKE']=0, dtype: Type[np.complexfloating]=np.complex64, classical_data: Optional['cirq.ClassicalDataStore']=None):
        ...
        Args:
            ...
            classical_data: The shared classical data container for this
                simulation.
        """
        state = _BufferedStateVector.create(initial_state=initial_state, qid_shape=tuple((q.dimension for q in qubits)) if qubits is not None else None, dtype=dtype, buffer=available_buffer)
    ...
    def _act_on_fallback_(self, action: Any, qubits: Sequence['cirq.Qid'], allow_decompose: bool=True) -> bool:
        strats: List[Callable[[Any, Any, Sequence['cirq.Qid']], bool]] = [_strat_act_on_state_vector_from_apply_unitary, _strat_act_on_state_vector_from_mixture, _strat_act_on_state_vector_from_channel]
        if allow_decompose:
        ...
        raise TypeError(f"Can't simulate operations that don't implement SupportsUnitary, SupportsConsistentApplyUnitary, SupportsMixture or is a measurement: {action!r}")

    def __repr__(self) -> str:
        return f'cirq.StateVectorSimulationState(initial_state={proper_repr(self.target_tensor)}, qubits={self.qubits!r}, classical_data={self.classical_data!r})'
