...
class TestPoolManager(HTTPDummyServerTestCase):
    ...
    def test_redirect_twice(self):
        with PoolManager() as http:
            r = http.request('GET', '%s/redirect' % self.base_url, fields={'target': '%s/redirect' % self.base_url}, redirect=False)
            assert r.status == 303
            r = http.request('GET', '%s/redirect' % self.base_url, fields={'target': '%s/redirect?target=%s/' % (self.base_url, self.base_url)})
            assert r.status == 200
            assert r.data == b'Dummy server!'
    ...
    def test_cross_host_redirect(self):
        with PoolManager() as http:
            ...
            with pytest.raises(MaxRetryError):
                http.request('GET', '%s/redirect' % self.base_url, fields={'target': cross_host_location}, timeout=LONG_TIMEOUT, retries=0)
            r = http.request('GET', '%s/redirect' % self.base_url, fields={'target': '%s/echo?a=b' % self.base_url_alt}, timeout=LONG_TIMEOUT, retries=1)
