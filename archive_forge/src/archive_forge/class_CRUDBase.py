...
class CRUDBase(Generic[ModelTypeORM, SourceSchemaType]):
    ...
    def __init__(self, model: Union[Type[ModelTypeORM], str], defer_attrs: List[str]=None, auto_commit: Optional[bool]=True, primary_key: Optional[str]=None, model_dump_kwargs: Optional[Dict[str, Any]]=None, source_model: Optional[Type[SourceSchemaType]]=None, **kwargs):
        """
        ...
        * `defer_attrs`: A list of attributes to defer loading
        ...
        if source_model:
            if isinstance(source_model, str):
                source_model = lazy_import(source_model)
    ...
    async def all(self, db: AsyncSession, stmt: Select, **kwargs) -> List[ObjectResult]:
        """
    ...
    async def first(self, db: AsyncSession, stmt: Union[Select, Insert, Update], raise_for_not_found: bool=False, **kwargs) -> Optional[ObjectResult]:
        """
    ...
    async def get(self, db: AsyncSession, id: IDType, raise_for_not_found: bool=False, defer_attrs: List[str]=None, **kwargs) -> Optional[ObjectResult]:
        """
    ...
    async def get_first_filtered(self, db: AsyncSession, raise_for_not_found: bool=False, **filters) -> Optional[ObjectResult]:
        """
    ...
    async def get_multi(self, db: AsyncSession, *, skip: int=0, limit: int=100, sort_field: Optional[str]=None, sort_order: Optional[SortOrder]=None, **filters) -> List[ObjectResult]:
        """
        ...
        if and_stmt:
            stmt = stmt.where(and_stmt)
        if skip is not None:
            stmt = stmt.offset(skip)
        if limit is not None:
            stmt = stmt.limit(limit)
        if sort_field:
            stmt = self._construct_sort_statement(stmt, sort_field, sort_order)
        stmt = self._defer_attrs(stmt, None)
        return await self.all(db, stmt)

    async def get_multi_ids(self, db: AsyncSession, *, ids: List[IDType], skip: Optional[int]=None, limit: Optional[int]=None, sort_field: Optional[str]=None, sort_order: Optional[SortOrder]=None, load_children: List[str]=None, raise_for_not_found: bool=False, **kwargs) -> List[ObjectResult]:
        """
    ...
    async def get_random(self, db: AsyncSession, *, limit: int=100, sort_field: Optional[str]=None, sort_order: Optional[SortOrder]=None, **filters) -> List[ObjectResult]:
        """
    ...
    async def get_count(self, db: AsyncSession, **filters) -> int:
        ...
        """
        and_stmt = self._construct_and_statement(filters)
        stmt = select(sql_func.count(self.object_primary_key_field))
        if and_stmt:
            stmt = stmt.where(and_stmt)
        return await db.scalar(stmt)
    '\n    Create Methods\n    '

    async def create(self, db: AsyncSession, *, obj_in: SourceSchemaType, model_dump_kwargs: Optional[Dict[str, Any]]=None, no_commit: Optional[bool]=None, **kwargs) -> ModelTypeORM:
        """
        Creates an object
        """
    ...
    async def create_if_not_exists(self, db: AsyncSession, *, obj_in: SourceSchemaType, index_elements: Optional[List[str]]=None, model_dump_kwargs: Optional[Dict[str, Any]]=None, exclude_attrs: Optional[List[str]]=None, no_commit: Optional[bool]=None, **kwargs) -> None:
        """
        Creates an object if it does not exist
        """
        index_elements = index_elements or [self.primary_key]
        values = self._convert_source_to_dict(obj_in, model_dump_kwargs=model_dump_kwargs, exclude=exclude_attrs, **kwargs)
    ...
    async def upsert(self, db: AsyncSession, *, obj_in: SourceSchemaType, model_dump_kwargs: Optional[Dict[str, Any]]=None, no_commit: Optional[bool]=None, **kwargs) -> None:
        """
        Upserts an object
        """
        values = self._convert_source_to_dict(obj_in, model_dump_kwargs=model_dump_kwargs, **kwargs)
        stmt = insert(self.model).values(**values).on_conflict_do_update(index_elements=[self.primary_key], set_=values)
        await db.execute(stmt)
        do_commit = no_commit is False if no_commit is not None else self.auto_commit
        if do_commit:
            await db.commit()

    async def upsert_multi(self, db: AsyncSession, *, objs_in: List['SourceSchemaType'], model_dump_kwargs: Optional[Dict[str, Any]]=None, **kwargs):
        """
    ...
    async def upsert_bulk(self, db: AsyncSession, *, objs_in: List['SourceSchemaType'], model_dump_kwargs: Optional[Dict[str, Any]]=None, batch_size: Optional[int]=200, **kwargs) -> int:
        """
        Upserts multiple objects
        """
    ...
    async def upsert_multi_with_idx(self, db: AsyncSession, *, objs_in: List['SourceSchemaType'], start_idx: int, model_dump_kwargs: Optional[Dict[str, Any]]=None, raise_errors: Optional[bool]=None, **kwargs) -> Tuple[List[int], List[int]]:
        """
    ...
    async def upsert_bulk_with_index(self, db: AsyncSession, *, objs_in: List['SourceSchemaType'], batch_size: Optional[int]=200, model_dump_kwargs: Optional[Dict[str, Any]]=None, raise_errors: Optional[bool]=None, **kwargs) -> Tuple[List[int], List[int]]:
        """
        Upserts multiple objects
        """
        completed_idx: List[int] = []
        failed_idx: List[int] = []
        for i in range(0, len(objs_in), batch_size):
            idxs = await self.upsert_multi_with_idx(db, objs_in=objs_in[i:i + batch_size], start_idx=i, raise_errors=raise_errors, model_dump_kwargs=model_dump_kwargs, **kwargs)
    ...
    async def update(self, db: AsyncSession, *, db_obj: ModelTypeORM, obj_in: Union[SourceSchemaType, Dict[str, Any]], **kwargs) -> ModelTypeORM:
        """
        Updates an object
        """
    ...
    async def delete(self, db: AsyncSession, *, id: IDType) -> ModelTypeORM:
        """
        Removes an object
        """
    ...
    async def get_from_results(self, db: AsyncSession, *, results: List[Union[Tuple[str, Any], IDType]], cast_to_source: Optional[bool]=None, source_model: Optional[Type[SourceSchemaType]]=None, skip: Optional[int]=None, limit: Optional[int]=None, sort_field: Optional[str]=None, sort_order: Optional[SortOrder]=None, **kwargs) -> List[ObjectResult]:
        """
