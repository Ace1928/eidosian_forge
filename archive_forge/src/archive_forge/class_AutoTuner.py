...
class AutoTuner(keras_tuner.engine.tuner.Tuner):
    ...
    def _build_and_fit_model(self, trial, *args, **kwargs):
        ...
        pipeline, kwargs['x'], kwargs['validation_data'] = self._prepare_model_build(trial.hyperparameters, **kwargs)
    ...
    def search(self, epochs=None, callbacks=None, validation_split=0, verbose=1, **fit_kwargs):
        ...
        if epochs is None:
            ...
            if not utils.contain_instance(callbacks, tf_callbacks.EarlyStopping):
                callbacks.append(tf_callbacks.EarlyStopping(patience=10, min_delta=0.0001))
        early_stopping_inserted = False
        new_callbacks = self._deepcopy_callbacks(callbacks)
        if not utils.contain_instance(callbacks, tf_callbacks.EarlyStopping):
            early_stopping_inserted = True
            new_callbacks.append(tf_callbacks.EarlyStopping(patience=10, min_delta=0.0001))
        ...
        if validation_split > 0 or early_stopping_inserted:
            ...
            if validation_split > 0:
                copied_fit_kwargs['x'] = copied_fit_kwargs['x'].concatenate(fit_kwargs['validation_data'])
            ...
            history = None
            pipeline = pipeline_module.load_pipeline(self._pipeline_path(self.oracle.get_best_trials(1)[0].trial_id))
    ...
    def _remove_early_stopping(callbacks):
        return [copy.deepcopy(callbacks) for callback in callbacks if not isinstance(callback, tf_callbacks.EarlyStopping)]
