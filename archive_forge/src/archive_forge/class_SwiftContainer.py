...
class SwiftContainer(resource.Resource):
    ...
    """
    PROPERTIES = NAME, X_CONTAINER_READ, X_CONTAINER_WRITE, X_CONTAINER_META, X_ACCOUNT_META, PURGE_ON_DELETE = ('name', 'X-Container-Read', 'X-Container-Write', 'X-Container-Meta', 'X-Account-Meta', 'PurgeOnDelete')
    ATTRIBUTES = DOMAIN_NAME, WEBSITE_URL, ROOT_URL, OBJECT_COUNT, BYTES_USED, HEAD_CONTAINER = ('DomainName', 'WebsiteURL', 'RootURL', 'ObjectCount', 'BytesUsed', 'HeadContainer')
    properties_schema = {NAME: properties.Schema(properties.Schema.STRING, _('Name for the container. If not specified, a unique name will be generated.')), X_CONTAINER_READ: properties.Schema(properties.Schema.STRING, _('Specify the ACL permissions on who can read objects in the container.')), X_CONTAINER_WRITE: properties.Schema(properties.Schema.STRING, _('Specify the ACL permissions on who can write objects to the container.')), X_CONTAINER_META: properties.Schema(properties.Schema.MAP, _('A map of user-defined meta data to associate with the container. Each key in the map will set the header X-Container-Meta-{key} with the corresponding value.'), default={}), X_ACCOUNT_META: properties.Schema(properties.Schema.MAP, _('A map of user-defined meta data to associate with the account. Each key in the map will set the header X-Account-Meta-{key} with the corresponding value.'), default={}), PURGE_ON_DELETE: properties.Schema(properties.Schema.BOOLEAN, _('If True, delete any objects in the container when the container is deleted. Otherwise, deleting a non-empty container will result in an error.'), default=False, support_status=support.SupportStatus(version='2015.1'))}
    attributes_schema = {DOMAIN_NAME: attributes.Schema(_('The host from the container URL.'), type=attributes.Schema.STRING), WEBSITE_URL: attributes.Schema(_('The URL of the container.'), type=attributes.Schema.STRING), ROOT_URL: attributes.Schema(_('The parent URL of the container.'), type=attributes.Schema.STRING), OBJECT_COUNT: attributes.Schema(_('The number of objects stored in the container.'), type=attributes.Schema.INTEGER), BYTES_USED: attributes.Schema(_('The number of bytes stored in the container.'), type=attributes.Schema.INTEGER), HEAD_CONTAINER: attributes.Schema(_('A map containing all headers for the container.'), type=attributes.Schema.MAP)}
    ...
    def handle_create(self):
        ...
        container_headers = SwiftContainer._build_meta_headers('container', self.properties[self.X_CONTAINER_META])
        account_headers = SwiftContainer._build_meta_headers('account', self.properties[self.X_ACCOUNT_META])
        ...
        LOG.debug('SwiftContainer create container %(container)s with container headers %(container_headers)s and account headers %(account_headers)s', {'container': container, 'account_headers': account_headers, 'container_headers': container_headers})
    ...
    def handle_delete(self):
        ...
        if objects:
            if self.properties[self.PURGE_ON_DELETE]:
                self._deleter(objects.pop())
            else:
                msg = _('Deleting non-empty container (%(id)s) when %(prop)s is False') % {'id': self.resource_id, 'prop': self.PURGE_ON_DELETE}
    ...
    def _resolve_attribute(self, key):
        ...
        if key == self.DOMAIN_NAME:
            ...
            return '%s://%s%s/%s' % (parsed[0], parsed[1], parsed[2], self.resource_id)
        elif key == self.ROOT_URL:
            return '%s://%s%s' % (parsed[0], parsed[1], parsed[2])
        elif self.resource_id and key in (self.OBJECT_COUNT, self.BYTES_USED, self.HEAD_CONTAINER):
            try:
    ...
    def parse_live_resource_data(self, resource_properties, resource_data):
        ...
        swift_reality.update({self.PURGE_ON_DELETE: resource_properties.get(self.PURGE_ON_DELETE)})
        ...
        for key in resource_properties.get(self.X_CONTAINER_META):
            prefixed_key = '%(prefix)s-%(key)s' % {'prefix': self.X_CONTAINER_META.lower(), 'key': key.lower()}
            if prefixed_key in resource_data:
                swift_reality[self.X_CONTAINER_META].update({key: resource_data[prefixed_key]})
        for key in resource_properties.get(self.X_ACCOUNT_META):
            prefixed_key = '%(prefix)s-%(key)s' % {'prefix': self.X_ACCOUNT_META.lower(), 'key': key.lower()}
            if prefixed_key in resource_data['account_data']:
                swift_reality[self.X_ACCOUNT_META].update({key: resource_data['account_data'][prefixed_key]})
