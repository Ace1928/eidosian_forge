...
class EngineProgram(abstract_program.AbstractProgram):
    ...
    def __init__(self, project_id: str, program_id: str, context: 'engine_base.EngineContext', _program: Optional[quantum.QuantumProgram]=None, result_type: ResultType=ResultType.Program) -> None:
    ...
    async def run_sweep_async(self, job_id: Optional[str]=None, params: cirq.Sweepable=None, repetitions: int=1, processor_ids: Sequence[str]=('xmonsim',), description: Optional[str]=None, labels: Optional[Dict[str, str]]=None, *, processor_id: str='', run_name: str='', device_config_name: str='') -> engine_job.EngineJob:
        ...
        created_job_id, job = await self.context.client.create_job_async(project_id=self.project_id, program_id=self.program_id, job_id=job_id, processor_ids=processor_ids, run_context=run_context, description=description, labels=labels, processor_id=processor_id, run_name=run_name, device_config_name=device_config_name)
        return engine_job.EngineJob(self.project_id, self.program_id, created_job_id, self.context, job)
    run_sweep = duet.sync(run_sweep_async)

    async def run_batch_async(self, job_id: Optional[str]=None, params_list: Optional[List[cirq.Sweepable]]=None, repetitions: int=1, processor_ids: Sequence[str]=(), description: Optional[str]=None, labels: Optional[Dict[str, str]]=None, *, processor_id: str='', run_name: str='', device_config_name: str='') -> engine_job.EngineJob:
        ...
        batch_context = v2.batch_run_context_to_proto(((params, repetitions) for params in params_list))
        created_job_id, job = await self.context.client.create_job_async(project_id=self.project_id, program_id=self.program_id, job_id=job_id, processor_ids=processor_ids, run_context=util.pack_any(batch_context), description=description, labels=labels, processor_id=processor_id, run_name=run_name, device_config_name=device_config_name)
        return engine_job.EngineJob(self.project_id, self.program_id, created_job_id, self.context, job, result_type=ResultType.Batch)
    run_batch = duet.sync(run_batch_async)

    async def run_calibration_async(self, job_id: Optional[str]=None, processor_ids: Sequence[str]=(), description: Optional[str]=None, labels: Optional[Dict[str, str]]=None) -> engine_job.EngineJob:
        ...
        created_job_id, job = await self.context.client.create_job_async(project_id=self.project_id, program_id=self.program_id, job_id=job_id, processor_ids=processor_ids, run_context=util.pack_any(run_context), description=description, labels=labels)
        return engine_job.EngineJob(self.project_id, self.program_id, created_job_id, self.context, job, result_type=ResultType.Batch)
    run_calibration = duet.sync(run_calibration_async)

    async def run_async(self, job_id: Optional[str]=None, param_resolver: cirq.ParamResolver=cirq.ParamResolver({}), repetitions: int=1, processor_ids: Sequence[str]=('xmonsim',), description: Optional[str]=None, labels: Optional[Dict[str, str]]=None, *, processor_id: str='', run_name: str='', device_config_name: str='') -> cirq.Result:
        ...
        Raises:
            ...
            ValueError: If either `run_name` and `device_config_name` are set but
                `processor_id` is empty.
        """
        job = await self.run_sweep_async(job_id=job_id, params=[param_resolver], repetitions=repetitions, processor_ids=processor_ids, description=description, labels=labels, processor_id=processor_id, run_name=run_name, device_config_name=device_config_name)
    ...
    async def list_jobs_async(self, created_before: Optional[Union[datetime.datetime, datetime.date]]=None, created_after: Optional[Union[datetime.datetime, datetime.date]]=None, has_labels: Optional[Dict[str, str]]=None, execution_states: Optional[Set[quantum.ExecutionStatus.State]]=None) -> Sequence[engine_job.EngineJob]:
        ...
        response = await client.list_jobs_async(self.project_id, self.program_id, created_before=created_before, created_after=created_after, has_labels=has_labels, execution_states=execution_states)
        return [engine_job.EngineJob(project_id=engine_client._ids_from_job_name(j.name)[0], program_id=engine_client._ids_from_job_name(j.name)[1], job_id=engine_client._ids_from_job_name(j.name)[2], context=self.context, _job=j) for j in response]
    ...
    async def set_description_async(self, description: str) -> 'EngineProgram':
        ...
        Returns:
             This EngineProgram.
        """
        self._program = await self.context.client.set_program_description_async(self.project_id, self.program_id, description)
        return self
    ...
    async def set_labels_async(self, labels: Dict[str, str]) -> 'EngineProgram':
        ...
        Returns:
             This EngineProgram.
        """
        self._program = await self.context.client.set_program_labels_async(self.project_id, self.program_id, labels)
        return self
    ...
    async def add_labels_async(self, labels: Dict[str, str]) -> 'EngineProgram':
        ...
        Returns:
             This EngineProgram.
        """
        self._program = await self.context.client.add_program_labels_async(self.project_id, self.program_id, labels)
        return self
    ...
    async def remove_labels_async(self, keys: List[str]) -> 'EngineProgram':
        ...
        Returns:
             This EngineProgram.
        """
        self._program = await self.context.client.remove_program_labels_async(self.project_id, self.program_id, keys)
        return self
    ...
    async def batch_size_async(self) -> int:
        ...
        """
        if self.result_type != ResultType.Batch:
            raise ValueError(f'Program was not a batch program but instead was of type {self.result_type}.')
        import cirq_google.engine.engine as engine_base
        if self._program is None or self._program.code is None:
            self._program = await self.context.client.get_program_async(self.project_id, self.program_id, True)
    ...
    async def delete_async(self, delete_jobs: bool=False) -> None:
        ...
        """
        await self.context.client.delete_program_async(self.project_id, self.program_id, delete_jobs=delete_jobs)
