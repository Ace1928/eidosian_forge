...
# Function to generate high-quality sound array using sine wave generation with timbre and harmonics
def generate_sound_array(
    frequency: float,
    duration: float,
    volume: float = 0.75,
    harmonics_weights: List[float] = [1, 0.5, 0.25],
    timbre: str = "sine",
) -> np.ndarray:
    """
    Generate a high-quality sound array using sine wave generation with specified frequency, duration, volume, harmonics, and timbre.
    """
    ...
    if timbre == "sine":
        for i, weight in enumerate(harmonics_weights):
            tone += weight * np.sin(2 * np.pi * (i + 1) * frequency * t)
    elif timbre == "square":
        for i, weight in enumerate(harmonics_weights):
            tone += weight * np.sign(np.sin(2 * np.pi * (i + 1) * frequency * t))
    elif timbre == "sawtooth":
        for i, weight in enumerate(harmonics_weights):
            tone += weight * 2 * (t * frequency * (i + 1) % 1 - 0.5)
...
def save_sound_to_file(
    sound_array: np.ndarray, filename: str, timbre: str, harmonics_weights: List[float]
) -> None:
    """
    Save a sound array to a WAV file, including metadata for timbre and harmonics.
    """
    with wave.open(filename, "w") as wf:
        ...
        metadata = {"timbre": timbre, "harmonics_weights": harmonics_weights}
        with open(metadata_filename, "w") as metafile:
            json.dump(metadata, metafile, indent=4)
        logging.info(f"Saved {filename} with metadata {metadata_filename}")
...
def generate_and_save_notes() -> None:
    ...
    Each note is saved as a WAV file in the 'sounds' directory, including metadata for timbre and harmonics.
    """
    ...
    for octave in OCTAVE_RANGE:
        for note, base_freq in NOTE_FREQUENCIES.items():
            ...
            if not os.path.exists(filename):
                ...
                save_sound_to_file(sound_array, filename, "sine", [1, 0.5, 0.25])
...
def load_or_define_frequencies() -> Dict[int, float]:
    ...
    try:
        ...
        notes_sequence = "C C# Db D D# Eb E F F# Gb G G# Ab A A# Bb B C C# Db D D# Eb E F F# Gb G G# Ab A A# Bb B".split()
