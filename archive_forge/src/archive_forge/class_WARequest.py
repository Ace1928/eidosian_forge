...
class WARequest(object):
    OK = 200
    ENC_PUBKEY = Curve.decodePoint(bytearray([5, 142, 140, 15, 116, 195, 235, 197, 215, 166, 134, 92, 108, 60, 132, 56, 86, 176, 97, 33, 204, 232, 234, 119, 77, 34, 251, 111, 18, 37, 18, 48, 45]))
    ...
    def send(self, parser=None, encrypt=True, preview=False):
        logger.debug('send(parser=%s, encrypt=%s, preview=%s)' % (None if parser is None else '[omitted]', encrypt, preview))
        if self.type == 'POST':
