...
class NetAppONTAPCGSnapshot(object):
    ...
    def __init__(self):
        ...
        self.module = AnsibleModule(argument_spec=self.argument_spec, supports_check_mode=False, mutually_exclusive=[['consistency_group', 'volumes']])
    ...
    def cg_commit(self):
        ...
        """
        cgcommit = netapp_utils.zapi.NaElement.create_node_with_children('cg-commit', **{'cg-id': self.cgid})
        try:
            self.server.invoke_successfully(cgcommit, enable_tunneling=True)
        except netapp_utils.zapi.NaApiError as error:
            self.module.fail_json(msg='Error committing CG snapshot %s: %s' % (self.parameters['snapshot'], to_native(error)), exception=traceback.format_exc())
