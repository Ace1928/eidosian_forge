...
class CliffordTableau(StabilizerState):
    ...
    def __init__(self, num_qubits, initial_state: int=0, rs: Optional[np.ndarray]=None, xs: Optional[np.ndarray]=None, zs: Optional[np.ndarray]=None):
    ...
    def __eq__(self, other):
        ...
        return self.n == other.n and np.array_equal(self.rs, other.rs) and np.array_equal(self.xs, other.xs) and np.array_equal(self.zs, other.zs)
    ...
    def __repr__(self) -> str:
        return f'cirq.CliffordTableau({self.n},rs={proper_repr(np.delete(self._rs, len(self._rs) - 1))}, xs={proper_repr(np.delete(self._xs, len(self._xs) - 1, axis=0))},zs={proper_repr(np.delete(self._zs, len(self._zs) - 1, axis=0))}, initial_state={self.initial_state})'
    ...
    def then(self, second: 'CliffordTableau') -> 'CliffordTableau':
        ...
        if self.n != second.n:
            raise ValueError(f'Mismatched number of qubits of two tableaux: {self.n} vs {second.n}.')
        ...
        s_12 = s1 + m1 @ s2 + p1 * (m1 @ p2) + np.diag(m1 @ np.tril(np.outer(p2, p2.T) + m2 @ lmbda @ m2.T, -1) @ m1.T)
    ...
    def apply_cz(self, control_axis: int, target_axis: int, exponent: float=1, global_shift: float=0):
        ...
        self.xs[:, target_axis], self.zs[:, target_axis] = (self.zs[:, target_axis].copy(), self.xs[:, target_axis].copy())
        self.rs[:] ^= self.xs[:, target_axis] & self.zs[:, target_axis]
        self.rs[:] ^= self.xs[:, control_axis] & self.zs[:, target_axis] & ~(self.xs[:, target_axis] ^ self.zs[:, control_axis])
