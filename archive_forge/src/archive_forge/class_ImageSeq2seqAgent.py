...
class ImageSeq2seqAgent(TransformerGeneratorAgent, TorchImageAgent):
    ...
    def add_cmdline_args(cls, argparser):
        ...
        group.add_argument('--include-image-token', type='bool', default=True, recommended=True, help='if true, include image token (or no image token) for each example')
        group.add_argument('--image-fusion-type', type=str, default='late', choices=[f.value for f in FusionType], help='which fusion type to use')
        return group
    ...
    def _set_text_vec(self, *args, **kwargs) -> dict:
        ...
        if self.opt.get('include_image_token', False):
            ...
            vec = torch.LongTensor(self._check_truncate(obs['text_vec'], truncate, True))
            token = TOKEN_NO_IMAGE
            if obs.get('image', None) is not None:
                token = TOKEN_IMAGE
            obs.force_set('text_vec', torch.cat([vec, vec.new_tensor(self.dict[token]).unsqueeze(0)], 0))
        return obs
    ...
    def _dummy_batch(self, batchsize: int, maxlen: int) -> Batch:
        ...
        if self.fp16:
            image = image.half()
        return Batch(text_vec=b.text_vec, label_vec=b.label_vec, image=image, personalities=torch.ones(batchsize, self.opt['embedding_size']).cuda())
    ...
    def load_state_dict(self, state_dict: Dict[str, torch.Tensor]):
        ...
        if self.opt['init_model'] is not None:
            try:
                ...
                if embs.shape[0] + 2 != init_embs.shape[0] or embs.shape[1] != init_embs.shape[1]:
                    raise e
                state_dict.update({'embeddings.weight': torch.cat((embs.to(init_embs.device, dtype=init_embs.dtype), init_embs[-2:, :])), 'encoder.embeddings.weight': torch.cat((enc_embs.to(init_embs.device, dtype=init_embs.dtype), init_embs[-2:, :])), 'decoder.embeddings.weight': torch.cat((dec_embs.to(init_embs.device, dtype=init_embs.dtype), init_embs[-2:, :]))})
                pct_init = round(embs.shape[0] / len(self.dict) * 100, 1)
                print(f'Initialized embeddings for {embs.shape[0]} tokens ({pct_init}%)')
        self.model.load_state_dict(state_dict)