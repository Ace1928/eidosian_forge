...
class SlackClient:
    ...
    def __init__(self, token: Optional[str]=None, default_user: Optional[str]=None, default_channel: Optional[str]=None, username_mapping: Optional[Dict[str, Union[str, List[str]]]]=None, verbose: Optional[bool]=None, **kwargs):
        """
        Slack Client
        """
    ...
    def configure_defaults(self):
        ...
        if self.default_user:
            ...
            if self.verbose:
                self.logger.info(f'Default user id: {self.default_user} -> {self.default_user_id}')
        if self.default_channel:
            self.default_channel_id = self.ctx.lookup_id(self.default_channel)
            if self.verbose:
                self.logger.info(f'Default channel id: {self.default_channel} -> {self.default_channel_id}')
        if self.default_id and self.verbose:
            self.logger.info(f'Default id: {self.default_id}')
    ...
    def lookup(self, user_or_channel: str) -> Optional[str]:
        ...
        if self.disabled:
            return
        if user_or_channel in [self.default_id, self.default_channel_id]:
    ...
    def message(self, message: Optional[str]=None, user_or_channel: Optional[str]=None, channel_id: Optional[str]=None, **kwargs):
        """
        Send a message
        """
        if self.disabled:
        ...
        if not self.ctx.uids.get(channel_id):
    ...
    async def amessage(self, message: str, user_or_channel: Optional[str]=None, channel_id: Optional[str]=None, **kwargs):
        """
        Send a message
        """
        if self.disabled:
        ...
        if user_or_channel:
            ...
            channel_id = self.default_id
        if not self.ctx.uids.get(channel_id):
            await self.ajoin_channel(channel_id)
        if channel_id is None:
            raise ValueError('No channel id provided')
        return await self.api.chat_postMessage(channel=channel_id, text=message, **kwargs)

    def upload_file(self, file: Optional[Union[str, bytes, IOBase]]=None, content: Optional[str]=None, filename: Optional[str]=None, filetype: Optional[str]=None, initial_comment: Optional[str]=None, thread_ts: Optional[str]=None, title: Optional[str]=None, channels: Optional[Union[str, Sequence[str]]]=None, **kwargs):
        """
        Upload a file
        """
        if self.disabled:
        ...
        if not isinstance(channels, (list, tuple)):
            channels = [channels]
        if channels:
            channels = [self.lookup(channel) for channel in channels]
        return self.sapi.files_upload_v2(file=file, content=content, filename=filename, filetype=filetype, initial_comment=initial_comment, thread_ts=thread_ts, title=title, channels=channels, **kwargs)

    async def aupload_file(self, file: Optional[Union[str, bytes, IOBase]]=None, content: Optional[str]=None, filename: Optional[str]=None, filetype: Optional[str]=None, initial_comment: Optional[str]=None, thread_ts: Optional[str]=None, title: Optional[str]=None, channels: Optional[Union[str, Sequence[str]]]=None, **kwargs):
        """
        Upload a file
        """
        if self.disabled:
        ...
        if channels:
            channels = [self.lookup(channel) for channel in channels]
        return await self.api.files_upload_v2(file=file, content=content, filename=filename, filetype=filetype, initial_comment=initial_comment, thread_ts=thread_ts, title=title, channels=channels, **kwargs)

    def temp_message(self, text: str, user: str, channel: Optional[str]=None, attachments: Optional[Sequence[Union[Dict, 'Attachment']]]=None, blocks: Optional[Sequence[Union[Dict, 'Block']]]=None, **kwargs):
        """
        ...
        return self.sapi.chat_postEphemeral(text=text, channel=channel, user=user, attachments=attachments, blocks=blocks, **kwargs)

    async def atemp_message(self, text: str, user: str, channel: Optional[str]=None, attachments: Optional[Sequence[Union[Dict, 'Attachment']]]=None, blocks: Optional[Sequence[Union[Dict, 'Block']]]=None, **kwargs):
        """
        Send a temporary message
        """
        if self.disabled:
        ...
        user = self.lookup(user)
        channel = self.lookup(channel) if channel else self.default_id
        return await self.api.chat_postEphemeral(text=text, channel=channel, user=user, attachments=attachments, blocks=blocks, **kwargs)
