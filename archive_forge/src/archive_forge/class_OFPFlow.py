...
class OFPFlow(Flow):
    ...
    def __init__(self, ofp_string, id=None):
        ...
        iparser.parse()
        isection = Section(name='info', pos=ofp_string.find(info), string=info, data=iparser.kv())
        sections.append(isection)
        mparser = KVParser(match, OFPFlow.match_decoders())
        mparser.parse()
        msection = Section(name='match', pos=ofp_string.find(match), string=match, data=mparser.kv())
        sections.append(msection)
        aparser = KVParser(actions, OFPFlow.action_decoders())
        aparser.parse()
        asection = Section(name='actions', pos=ofp_string.find(actions), string=actions, data=aparser.kv(), is_list=True)
    ...
    def _gen_info_decoders():
        ...
        args = {'table': decode_int, 'duration': decode_time, 'n_packet': decode_int, 'n_bytes': decode_int, 'cookie': decode_int, 'idle_timeout': decode_time, 'hard_timeout': decode_time, 'hard_age': decode_time}
        return KVDecoders(args)

    @staticmethod
    def _gen_match_decoders():
        """Generate the match KVDecoders."""
        args = {**OFPFlow._field_decoder_args(), **OFPFlow._extra_match_decoder_args()}
        return KVDecoders(args)
    ...
    def _field_decoder_args():
        ...
        shorthands = ['eth', 'ip', 'ipv6', 'icmp', 'icmp6', 'tcp', 'tcp6', 'udp', 'udp6', 'sctp', 'arp', 'rarp', 'mpls', 'mplsm']
    ...
    def _gen_action_decoders():
        ...
        actions = {**OFPFlow._output_actions_decoders_args(), **OFPFlow._encap_actions_decoders_args(), **OFPFlow._field_action_decoders_args(), **OFPFlow._meta_action_decoders_args(), **OFPFlow._fw_action_decoders_args(), **OFPFlow._control_action_decoders_args(), **OFPFlow._other_action_decoders_args(), **OFPFlow._instruction_action_decoders_args()}
        clone_actions = OFPFlow._clone_actions_decoders_args(actions)
        actions.update(clone_actions)
        return KVDecoders(actions, default_free=decode_free_output, ignore_case=True)
