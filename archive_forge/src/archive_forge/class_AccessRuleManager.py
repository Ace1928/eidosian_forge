...
class AccessRuleManager(base.CrudManager):
    ...
    def delete(self, access_rule, user=None):
        ...
        user = user or self.client.user_id
        self.base_url = '/users/%(user)s' % {'user': user}
        return super(AccessRuleManager, self).delete(access_rule_id=base.getid(access_rule))

    def update(self):
        raise exceptions.MethodNotImplemented(_('Access rules are immutable, updating is not supported.'))

    def create(self):
        raise exceptions.MethodNotImplemented(_('Access rules can only be created as attributes of application credentials.'))