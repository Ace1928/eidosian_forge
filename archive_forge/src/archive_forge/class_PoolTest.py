...
class PoolTest(common.HeatTestCase):
    ...
    def test_resource_mapping(self):
        ...
        self.assertEqual(pool.Pool, mapping['OS::Octavia::Pool'])
    ...
    def _create_stack(self, tmpl=inline_templates.POOL_TEMPLATE):
        ...
        self.pool.client_plugin().client = mock.MagicMock(return_value=self.octavia_client)
    ...
    def test_create(self):
        self._create_stack()
        self.octavia_client.pool_show.side_effect = [{'provisioning_status': 'PENDING_CREATE'}, {'provisioning_status': 'PENDING_CREATE'}, {'provisioning_status': 'ACTIVE'}]
        self.octavia_client.pool_create.side_effect = [exceptions.Conflict(409), {'pool': {'id': '1234'}}]
        expected = {'pool': {'name': 'my_pool', 'description': 'my pool', 'session_persistence': {'type': 'HTTP_COOKIE'}, 'lb_algorithm': 'ROUND_ROBIN', 'listener_id': '123', 'loadbalancer_id': 'my_lb', 'protocol': 'HTTP', 'admin_state_up': True, 'tls_enabled': False}}
    ...
    def test_create_missing_properties(self):
        for prop in ('lb_algorithm', 'listener', 'protocol'):
            ...
            self._create_stack(tmpl=yaml.safe_dump(tmpl))
            if prop == 'listener':
                self.assertRaises(exception.PropertyUnspecifiedError, self.pool.validate)
            else:
                self.assertRaises(exception.StackValidationFailed, self.pool.validate)
    ...
    def test_update(self):
        self._create_stack()
        self.pool.resource_id_set('1234')
        self.octavia_client.pool_show.side_effect = [{'provisioning_status': 'PENDING_UPDATE'}, {'provisioning_status': 'PENDING_UPDATE'}, {'provisioning_status': 'ACTIVE'}]
        self.octavia_client.pool_set.side_effect = [exceptions.Conflict(409), None]
        prop_diff = {'admin_state_up': False, 'session_persistence': {'type': 'source_ip'}, 'name': 'your_pool', 'lb_algorithm': 'SOURCE_IP'}
        ...
        self.octavia_client.pool_set.assert_called_with('1234', json={'pool': prop_diff})
        self.assertFalse(self.pool.check_update_complete(prop_diff))
    ...
    def test_delete(self):
        self._create_stack()
        self.pool.resource_id_set('1234')
        self.octavia_client.pool_show.side_effect = [{'provisioning_status': 'PENDING_DELETE'}, {'provisioning_status': 'PENDING_DELETE'}, {'provisioning_status': 'DELETED'}]
        self.octavia_client.pool_delete.side_effect = [exceptions.Conflict(409), None]
    ...
    def test_delete_not_found(self):
        self._create_stack()
        self.pool.resource_id_set('1234')
        self.octavia_client.pool_show.side_effect = [{'provisioning_status': 'PENDING_DELETE'}]
        self.octavia_client.pool_delete.side_effect = [exceptions.Conflict(409), exceptions.NotFound(404)]
        self.pool.handle_delete()
        self.assertFalse(self.pool.check_delete_complete(None))
        self.assertFalse(self.pool._delete_called)
        self.octavia_client.pool_delete.assert_called_with('1234')
    ...
    def test_delete_failed(self):
        self._create_stack()
        self.pool.resource_id_set('1234')
        self.octavia_client.pool_delete.side_effect = exceptions.Unauthorized(401)
        self.pool.handle_delete()
        self.assertRaises(exceptions.Unauthorized, self.pool.check_delete_complete, None)