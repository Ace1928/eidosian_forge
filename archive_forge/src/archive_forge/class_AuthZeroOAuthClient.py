...
class AuthZeroOAuthClient(ABC):
    ...
    def __init__(self, app: 'FastAPI', client_id: Optional[str]=None, secret_key: Optional[str]=None, app_ingress: Optional[str]=None, callback_path: Optional[str]='/auth/callback', logout_path: Optional[str]=None, include_wildcard_origin: Optional[bool]=True, user_class: Optional[Type['CurrentUser']]=None, **kwargs):
        """
        ...
        if app_ingress:
            self.settings.configure(app_ingress=app_ingress)
    ...
    async def authorize_app_in_authzero(self, ingress_url: Optional[str]=None, include_wildcard_origin: Optional[bool]=None, verbose: Optional[bool]=True, **kwargs) -> 'AuthZeroClientObject':
        """
        ...
        if ingress_url is None:
            ingress_url = self.settings.app_ingress
        if include_wildcard_origin is None:
            include_wildcard_origin = self.include_wildcard_origin
        if include_wildcard_origin:
            include_wildcard_origin = 'localhost' not in ingress_url and '0.0.0' not in ingress_url
        if include_wildcard_origin:
        ...
        callback_url = self.callback_path if self.callback_path.startswith('http') else urljoin(ingress_url, self.callback_path)
        logout_url = (self.logout_path if self.logout_path.startswith('http') else urljoin(ingress_url, self.logout_path)) if self.logout_path else ingress_url
        ...
        if not az_client._needs_update:
            if verbose:
                logger.info(f'Client: `{az_client.name}` is already authorized with `{ingress_url}`', colored=True, prefix=f'|g|{az_client.client_id}|e|')
            return az_client
    ...
    async def authorize_app_user(self, request: 'Request', code: str, scope: Optional[str]=None, scopes: Optional[List[str]]=None, background_tasks: Optional['BackgroundTasks']=None, user_class: Optional[Type[CurrentUser]]=None, **kwargs) -> 'CurrentUser':
        """
        Authorizes the App User
        """
        data = {'grant_type': 'authorization_code', 'client_id': self.client_id, 'code_verifier': self.secret_key, 'code': code, 'redirect_uri': self.redirect_uri}
        ...
        try:
            await current_user.from_access_token(access_token=user_data['access_token'], request=request, scope=scope, scopes=scopes, background_tasks=background_tasks, **kwargs)
        except Exception as e:
            logger.error(f'Error Authorizing User: {e}')
            raise e
        return current_user
