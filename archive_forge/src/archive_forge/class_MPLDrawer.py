...
class MPLDrawer:
    ...
    **Positioning**
    ...
    def __init__(self, n_layers, n_wires, c_wires=0, wire_options=None, figsize=None, fig=None):
        if not has_mpl:
            raise ImportError('Module matplotlib is required for ``MPLDrawer`` class. You can install matplotlib via \n\n   pip install matplotlib')
        ...
        if fig is None:
            ...
            self._fig = fig
        self._ax = self._fig.add_axes([0, 0, 1, 1], xlim=(-2, self.n_layers + 1), ylim=(-1, self.n_wires + self._cwire_scaling * c_wires + 0.5 * (c_wires > 0)), xticks=[], yticks=[])
        self._ax.axis('off')
        self._ax.invert_yaxis()
        if wire_options is None:
            wire_options = {}
        self._wire_lines = [plt.Line2D((-1, self.n_layers), (wire, wire), zorder=1, **wire_options) for wire in range(self.n_wires)]
    ...
    def _add_notch(self, layer, wire, extra_width, box_options):
        ...
        box1 = patches.FancyBboxPatch((x1, y), self._notch_width, self._notch_height, boxstyle=self._notch_style, **box_options)
        self._ax.add_patch(box1)
        box2 = patches.FancyBboxPatch((x2, y), self._notch_width, self._notch_height, boxstyle=self._notch_style, **box_options)
        ...
        **Example**
        ...
        if len(wires_target) > 1:
            ...
            if any((min_target < w < max_target for w in wires_ctrl)):
                warnings.warn('Some control indicators are hidden behind an operator. Consider re-ordering your circuit wires to ensure all control indicators are visible.', UserWarning)
    ...
    def measure(self, layer, wires, text=None, box_options=None, lines_options=None):
        ...
        **Example**
        ...
        y_loc = box_min - self._box_length / 2.0 + self._pad
        box = patches.FancyBboxPatch((x_loc, y_loc), self._box_length - 2 * self._pad, box_max - box_min + self._box_length - 2 * self._pad, boxstyle=self._boxstyle, **box_options)
        self._ax.add_patch(box)
        arc = patches.Arc((layer, box_center + 0.15 * self._box_length), 0.6 * self._box_length, 0.55 * self._box_length, theta1=180, theta2=0, **lines_options)
        ...
        self.ax.arrow(arrow_start_x, arrow_start_y, arrow_width, arrow_height, head_width=self._box_length / 8.0, **lines_options)
        if text:
            self._ax.text(layer + 0.05 * self._box_length, box_center + 0.225, text, fontsize=self.fontsize - 2)
    ...
    def cwire_join(self, layer, wire, erase_right=False):
        ...
        line = plt.Line2D(xs, (self._y(wire), self._y(wire)), zorder=2, color=plt.rcParams['figure.facecolor'], linewidth=3 * plt.rcParams['lines.linewidth'])
        self.ax.add_line(line)
    ...
    def cond(self, layer, measured_layer, wires, wires_target, options=None):
        ...
        **Example**
        ...
        if wires_ctrl[-1] < wires_target[0]:
            lines.extend((plt.Line2D((start_x, layer + self._cond_shift), (wires_ctrl[0] - self._cond_shift,) * 2, **options), plt.Line2D((start_x, layer - self._cond_shift), (wires_ctrl[0] + self._cond_shift,) * 2, **options), plt.Line2D((layer + self._cond_shift,) * 2, (wires_ctrl[0] - self._cond_shift, wires_target[0]), **options), plt.Line2D((layer - self._cond_shift,) * 2, (wires_ctrl[-1] + self._cond_shift, wires_target[0]), **options)))
            for prev_idx, next_wire in enumerate(wires_ctrl[1:]):
                lines.extend((plt.Line2D((layer - self._cond_shift,) * 2, (wires_ctrl[prev_idx] + self._cond_shift, next_wire - self._cond_shift), **options), plt.Line2D((start_x, layer - self._cond_shift), (next_wire - self._cond_shift,) * 2, **options), plt.Line2D((start_x, layer - self._cond_shift), (next_wire + self._cond_shift,) * 2, **options)))
        elif wires_target[-1] < wires_ctrl[0]:
            lines.extend((plt.Line2D((start_x, layer + self._cond_shift), (wires_ctrl[-1] + self._cond_shift,) * 2, **options), plt.Line2D((start_x, layer - self._cond_shift), (wires_ctrl[-1] - self._cond_shift,) * 2, **options), plt.Line2D((layer + self._cond_shift,) * 2, (wires_ctrl[-1] + self._cond_shift, wires_target[-1]), **options), plt.Line2D((layer - self._cond_shift,) * 2, (wires_ctrl[0] - self._cond_shift, wires_target[-1]), **options)))
            for wire_idx, ctrl_wire in enumerate(wires_ctrl[:-1]):
                lines.extend((plt.Line2D((layer - self._cond_shift,) * 2, (ctrl_wire + self._cond_shift, wires_ctrl[wire_idx + 1] - self._cond_shift), **options), plt.Line2D((start_x, layer - self._cond_shift), (ctrl_wire - self._cond_shift,) * 2, **options), plt.Line2D((start_x, layer - self._cond_shift), (ctrl_wire + self._cond_shift,) * 2, **options)))
        else:
            raise ValueError('Cannot draw interspersed mid-circuit measurements and conditional operations. Consider providing a wire order such that all measurement wires precede all wires for the operator being controlled, or vice versa.')
        for line in lines:
            self._ax.add_line(line)