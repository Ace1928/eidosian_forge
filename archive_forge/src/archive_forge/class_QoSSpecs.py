...
class QoSSpecs(resource.Resource):
    ...
    PROPERTIES = NAME, SPECS = ('name', 'specs')
    properties_schema = {NAME: properties.Schema(properties.Schema.STRING, _('Name of the QoS.')), SPECS: properties.Schema(properties.Schema.MAP, _('The specs key and value pairs of the QoS.'), required=True, update_allowed=True)}

    def _find_diff(self, update_prps, stored_prps):
        remove_prps = list(set(stored_prps.keys() or []) - set(update_prps.keys() or []))
        add_prps = dict(set(update_prps.items() or []) - set(stored_prps.items() or []))
    ...
    def handle_update(self, json_snippet, tmpl_diff, prop_diff):
        ...
        if self.resource_id is not None:
            ...
            if remove_specs:
                self.client().qos_specs.unset_keys(self.resource_id, remove_specs)
