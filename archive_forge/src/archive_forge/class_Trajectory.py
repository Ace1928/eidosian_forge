...
class Trajectory(MSONable):
    ...
    def __init__(self, species: list[str | Element | Species | DummySpecies | Composition], coords: list[list[Vector3D]] | np.ndarray | list[np.ndarray], charge: float | None=None, spin_multiplicity: float | None=None, lattice: Lattice | Matrix3D | list[Lattice] | list[Matrix3D] | np.ndarray | None=None, *, site_properties: SitePropsType | None=None, frame_properties: list[dict] | None=None, constant_lattice: bool | None=True, time_step: float | None=None, coords_are_displacement: bool=False, base_positions: list[list[Vector3D]] | np.ndarray | None=None) -> None:
        ...
        if lattice is None:
            ...
            if not constant_lattice and lattice.shape == (3, 3):
                ...
                warnings.warn('Get `constant_lattice=False`, but only get a single `lattice`. Use this single `lattice` as the lattice for all frames.')
            else:
                self.lattice = lattice
            self.constant_lattice = constant_lattice
        if coords_are_displacement:
            if base_positions is None:
                warnings.warn('Without providing an array of starting positions, the positions for each time step will not be available.')
            self.base_positions = base_positions
        else:
            self.base_positions = coords[0]
    ...
    def extend(self, trajectory: Trajectory) -> None:
        ...
        """
        if self.lattice is None and trajectory.lattice is not None or (self.lattice is not None and trajectory.lattice is None):
            raise ValueError('Cannot combine `Molecule`- and `Structure`-based `Trajectory`. objects.')
        if self.time_step != trajectory.time_step:
            raise ValueError(f'Cannot extend trajectory. Time steps of the trajectories are incompatible: {self.time_step} and {trajectory.time_step}.')
        if self.species != trajectory.species:
            raise ValueError(f'Cannot extend trajectory. Species in the trajectories are incompatible: {self.species} and {trajectory.species}.')
        self.to_positions()
        trajectory.to_positions()
        self.site_properties = self._combine_site_props(self.site_properties, trajectory.site_properties, len(self), len(trajectory))
        self.frame_properties = self._combine_frame_props(self.frame_properties, trajectory.frame_properties, len(self), len(trajectory))
        if self.lattice is not None and trajectory.lattice is not None:
            self.lattice, self.constant_lattice = self._combine_lattice(self.lattice, trajectory.lattice, len(self), len(trajectory))
    ...
    def __getitem__(self, frames: int | slice | list[int]) -> Molecule | Structure | Trajectory:
        ...
        if isinstance(frames, int):
            ...
            if self.lattice is None:
                ...
                return Molecule(self.species, self.coords[frames], charge=charge, spin_multiplicity=spin, site_properties=self._get_site_props(frames))
            lattice = self.lattice if self.constant_lattice else self.lattice[frames]
            return Structure(Lattice(lattice), self.species, self.coords[frames], site_properties=self._get_site_props(frames), to_unit_cell=True)
        if isinstance(frames, (slice, list, np.ndarray)):
            ...
            if self.lattice is None:
                return Trajectory(species=self.species, coords=coords, charge=self.charge, spin_multiplicity=self.spin_multiplicity, site_properties=self._get_site_props(selected), frame_properties=frame_properties, time_step=self.time_step, coords_are_displacement=False, base_positions=self.base_positions)
            lattice = self.lattice if self.constant_lattice else self.lattice[selected]
            return Trajectory(species=self.species, coords=coords, lattice=lattice, site_properties=self._get_site_props(selected), frame_properties=frame_properties, constant_lattice=self.constant_lattice, time_step=self.time_step, coords_are_displacement=False, base_positions=self.base_positions)
        supported = [int, slice, list or np.ndarray]
        raise ValueError(f'Expect the type of frames be one of {supported}; {type(frames)}.')

    def write_Xdatcar(self, filename: str | Path='XDATCAR', system: str | None=None, significant_figures: int=6) -> None:
    ...
    def as_dict(self) -> dict:
        ...
        return {'@module': type(self).__module__, '@class': type(self).__name__, 'species': self.species, 'coords': self.coords.tolist(), 'charge': self.charge, 'spin_multiplicity': self.spin_multiplicity, 'lattice': lat, 'site_properties': self.site_properties, 'frame_properties': self.frame_properties, 'constant_lattice': self.constant_lattice, 'time_step': self.time_step, 'coords_are_displacement': self.coords_are_displacement, 'base_positions': self.base_positions}
    ...
    def from_structures(cls, structures: list[Structure], constant_lattice: bool=True, **kwargs) -> Self:
        ...
        return cls(species=species, coords=coords, lattice=lattice, site_properties=site_properties, constant_lattice=constant_lattice, **kwargs)
