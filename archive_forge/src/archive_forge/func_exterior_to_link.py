...
def exterior_to_link(manifold, verbose=False, check_input=True, check_answer=True, careful_perturbation=True, simplify_link=True, pachner_search_tries=10, seed=None):
    """
    ...
    **Warning:** The order of the link components and the cusps of the
    ...
    tris_with_moves = good_simplification(manifold, max_tries=pachner_search_tries)
    if tris_with_moves:
    ...
    M.rebuild()
    M.connect_arcs()
    print_status('    Moves resulted in', sum((len(tet.arcs) for tet in M)), f'PL segments; max denom of coor is {M._curr_max_denom()}.')
