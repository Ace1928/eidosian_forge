...
class Snake:
    ...
    def __init__(
        ...
        collision_detection_strategy: CollisionDetectionStrategy = DefaultCollisionDetectionStrategy(),
    ) -> None:
        """
...
class AIController:
    ...
    def __init__(self, snake: Snake, fruit: Fruit):
        ...
        self.pathfinding_algorithms: List[Callable] = [
            ThetaStar.adaptive_theta_star,  # Highest priority
            ThetaStar.hierarchical_theta_star,
            ThetaStar.theta_star,
    ...
    def _initialize_neural_network(self) -> keras.Model:
        """
        Initialize a more complex neural network for decision-making.
        ...
        model = keras.Sequential(
            [
                keras.Input(shape=(6,)),  # Increased input dimensions
            ...
            if (state, goal) in self.state_cache:
                ...
                for pathfinding_algorithm in self.pathfinding_algorithms:
                    path = pathfinding_algorithm(
                        state, goal, grid, self.snake, self.fruit
                    )
                    if path:
                        break
                if (
