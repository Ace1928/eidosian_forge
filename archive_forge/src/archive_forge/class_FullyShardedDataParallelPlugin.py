...
class FullyShardedDataParallelPlugin:
    ...
    """
    sharding_strategy: 'typing.Any' = field(default=None, metadata={'help': 'FSDP Sharding Strategy of type `torch.distributed.fsdp.fully_sharded_data_parallel.ShardingStrategy`'})
    backward_prefetch: 'typing.Any' = field(default=None, metadata={'help': 'FSDP Backward Prefetch of type `torch.distributed.fsdp.fully_sharded_data_parallel.BackwardPrefetch`'})
    mixed_precision_policy: 'typing.Any' = field(default=None, metadata={'help': 'A config to enable mixed precision training with FullyShardedDataParallel. The 3 flags that are set are `param_dtype`, `reduce_dtype`, `buffer_dtype`. Each flag expects `torch.dtype` as the value. It is of type `torch.distributed.fsdp.fully_sharded_data_parallel.MixedPrecision`.'})
    auto_wrap_policy: Optional[Callable] = field(default=None, metadata={'help': 'A callable specifying a policy to recursively wrap layers with FSDP'})
    cpu_offload: 'typing.Any' = field(default=None, metadata={'help': 'Decides Whether to offload parameters and gradients to CPU. It is of type `torch.distributed.fsdp.fully_sharded_data_parallel.CPUOffload`.'})
    ignored_modules: Optional[Iterable[torch.nn.Module]] = field(default=None, metadata={'help': 'A list of modules to ignore for FSDP.'})
    state_dict_type: 'typing.Any' = field(default=None, metadata={'help': 'FSDP State Dict Type of type `torch.distributed.fsdp.fully_sharded_data_parallel.StateDictType`'})
    state_dict_config: 'typing.Any' = field(default=None, metadata={'help': 'FSDP State Dict Config of type `torch.distributed.fsdp.fully_sharded_data_parallel.StateDictConfig`'})
    optim_state_dict_config: 'typing.Any' = field(default=None, metadata={'help': 'FSDP Optimizer State Dict Config of type `torch.distributed.fsdp.fully_sharded_data_parallel.OptimStateDictConfig`'})
    limit_all_gathers: bool = field(default=True, metadata={'help': 'If False, then FSDP allows the CPU thread to schedule all-gathers without any extra synchronization. If True, then FSDP explicitly synchronizes the CPU thread to prevent too many in-flight all-gathers. This bool only affects the sharded strategies that schedule all-gathers. Enabling this can help lower the number of CUDA malloc retries.'})
    use_orig_params: bool = field(default=True, metadata={'help': 'If `True`, allows non-uniform `requires_grad` during init, which means support for interspersed frozen and trainable parameters. Useful in cases such as parameter-efficient fine-tuning. Please refer this [blog](https://dev-discuss.pytorch.org/t/rethinking-pytorch-fully-sharded-data-parallel-fsdp-from-first-principles/1019). This also enables multiple optimizer param groups. This should be `True` when creating an optimizer object before preparing/wrapping the model with FSDP.'})
    param_init_fn: Optional[Callable[[torch.nn.Module], None]] = field(default=None, metadata={'help': 'A Callable[torch.nn.Module] -> None that specifies how modules that are currently on the meta device should be initialized onto an actual device.'})
    sync_module_states: bool = field(default=True, metadata={'help': 'If True, each individually wrapped FSDP unit will broadcast module parameters from rank 0 to ensure they are the same across all ranks after initialization'})
    forward_prefetch: bool = field(default=False, metadata={'help': 'If True, then FSDP explicitly prefetches the next upcoming all-gather while executing in the forward pass. only use with Static graphs.'})
    activation_checkpointing: bool = field(default=False, metadata={'help': 'If True, activation checkpointing is a technique to reduce memory usage by clearing activations of certain layers and recomputing them during a backward pass. Effectively, this trades extra computation time for reduced memory usage.'})
    ...
    def __post_init__(self):
        ...
        if self.sharding_strategy is None:
            ...
            sharding_strategy = FSDP_SHARDING_STRATEGY.index(sharding_strategy) + 1 if not sharding_strategy.isdigit() else int(sharding_strategy)
    ...
    def set_mixed_precision(self, mixed_precision, buffer_autocast=False, override=False):
        ...
        if self.mixed_precision_policy is None or override:
            self.mixed_precision_policy = MixedPrecision(param_dtype=dtype, reduce_dtype=dtype, buffer_dtype=buffer_dtype)
