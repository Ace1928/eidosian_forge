...
class TomlConfigParser:
    ...
    def read(self, filenames: Iterable[str]) -> list[str]:
        ...
        if has_tomllib:
            ...
            has_toml = re.search('^\\[tool\\.coverage(\\.|])', toml_text, flags=re.MULTILINE)
    ...
    def _check_type(self, section: str, option: str, value: Any, type_: type[TWant], converter: Callable[[Any], TWant] | None, type_desc: str) -> TWant:
        ...
        if isinstance(value, str) and converter is not None:
            try:
                ...
                raise ValueError(f"Option [{section}]{option} couldn't convert to {type_desc}: {value!r}") from e
