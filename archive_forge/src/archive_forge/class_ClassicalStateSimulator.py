...
class ClassicalStateSimulator(SimulatesSamples):
    ...
    def _run(self, circuit: AbstractCircuit, param_resolver: ParamResolver, repetitions: int) -> Dict[str, np.ndarray]:
        ...
        for moment in resolved_circuit:
            for op in moment:
                ...
                if op.gate == ops.X:
                    ...
                    measurement_values = np.array([[[values_dict[q] for q in op.qubits]]] * repetitions, dtype=np.uint8)
                    key = op.gate.key
                    if key in results_dict:
                        if op._num_qubits_() != results_dict[key].shape[-1]:
                            raise ValueError(f'Measurement shape {len(measurement_values)} does not match {results_dict[key].shape[-1]} in {key}.')
                        results_dict[key] = np.concatenate((results_dict[key], measurement_values), axis=1)
                    else:
                        results_dict[key] = measurement_values
                else:
                    raise ValueError(f'{op} is not one of cirq.X, cirq.CNOT, cirq.SWAP, cirq.CCNOT, or a measurement')
        return results_dict