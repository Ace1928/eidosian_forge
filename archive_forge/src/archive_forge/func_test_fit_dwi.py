...
def test_fit_dwi():
    ...
    cmd_tmp = '{cmd} -source {in_file} -bval {bval} -bvec {bvec} -dti -error {error} -famap {fa} -mcmap {mc} -mcout {mcout} -mdmap {md} -nodiff {nodiff} -res {res} -rgbmap {rgb} -syn {syn} -tenmap2 {ten2}  -v1map {v1}'
