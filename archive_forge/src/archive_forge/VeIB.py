...
class ConnectionWithDelay:
    ...
    def __init__(self, strength, repeat_factor, invert_signal):
        ...
        self.delayed_signals = deque(
            [0] * self.repeat_factor, maxlen=self.repeat_factor
        )
...
class MiniNetworkWithDelays:
    ...
    def __init__(self, neurons, connections):
        ...
        if not all(isinstance(neuron, SimplifiedNeuronWithDelay) for neuron in neurons):
            raise TypeError(
                "All neurons must be instances of SimplifiedNeuronWithDelay"
            )
    ...
    def simulate(self, external_input, current_time, simulation_params):
        ...
        for i in range(6):
            neuron_input = (
                ...
                else np.zeros((simulation_params.neuron_signal_size,))
            )
            neuron_outputs[i] = self.neurons[i].process_input(
                neuron_input.reshape(1, -1), current_time, simulation_params
            )
        ...
        for i in range(len(self.neurons)):
            ...
            for j in range(len(self.neurons)):
                if (j, i) in self.connections:
                    ...
                    transmitted_signal = connection.transmit(
                        neuron_outputs[j], simulation_params
                    )
                    transmitted_signal = np.array(
                        [transmitted_signal]
                    )  # Ensure transmitted_signal is a NumPy array
            ...
            neuron_outputs[i] = self.neurons[i].process_input(
                aggregated_input.reshape(1, -1), current_time, simulation_params
            )
...
def random_neuron_params(simulation_params):
    ...
    return {
        ...
        "over_threshold": np.clip(
            np.random.lognormal(mean=np.log(70), sigma=0.5), 0, 127
        ),
        "base_signal_strength": np.clip(
            np.random.lognormal(mean=np.log(64), sigma=0.5), 1, 128
        ),
        "refractory_period": np.clip(
            np.random.lognormal(mean=np.log(3), sigma=0.5), 1, 5
        ),
        "spike_threshold": np.clip(
            np.random.lognormal(mean=np.log(50), sigma=0.5), 1, 127
        ),
...
def dynamic_input_generator(num_time_steps, simulation_params, external_input=None):
    ...
    if external_input is not None:
        transformed_input = standard_input_transform(
            external_input, signal_size, num_neurons, simulation_params
        )
    else:
        transformed_input = np.random.randint(
            -128, 128, size=(num_neurons, signal_size)
        )
...
def create_mini_network(simulation_params, connection_mode="specified"):
    ...
    neurons = [
        SimplifiedNeuronWithDelay(**params, simulation_params=simulation_params)
    ...
    if connection_mode == "specified":
        ...
        for i, j in specified_connections:
            connections[(i, j)] = ConnectionWithDelay(
                **random_connection_params(simulation_params)
            )
...
def visualize_network_activity(initial_data):
    ...
    initial_data = (
        np.array(initial_data)
        if not isinstance(initial_data, np.ndarray)
        else initial_data
    )
    ...
    lines1 = [
        ax1.plot(
            initial_data[:, i], label=f"Neuron {i+1}", color=colors[i], linewidth=1
        )[0]
        for i in range(6)
    ]
    ...
    ax_slider_speed = plt.axes(
        [0.25, 0.01, 0.50, 0.03], facecolor="lightgoldenrodyellow"
    )
    ...
    reset_button = Button(
        ax_button, "Reset", color="lightgoldenrodyellow", hovercolor="0.975"
    )
...
def update_plot(
    ...
    try:
        new_data = generate_new_data(
            data_buffer[-min(len(data_buffer), buffer_size) :], num_new_points
        )
        data_buffer = np.vstack((data_buffer, new_data))[-buffer_size:]

        x_data = np.arange(
            start=max(0, len(data_buffer) - buffer_size), stop=len(data_buffer)
        )
...
def main(time_steps=100):
    ...
    try:
        ...
        for current_time in range(time_steps):
            output = mini_network.simulate(
                input_signals[current_time], current_time, simulation_params
            )
            network_output.append(output)

        ani = visualize_network_activity(
            network_output
        )  # Assign the result to ani once
    except Exception as e:
