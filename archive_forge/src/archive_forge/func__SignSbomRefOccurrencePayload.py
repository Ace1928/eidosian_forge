...
def _SignSbomRefOccurrencePayload(occ, kms_key_version):
  ...
  """
    payload_bytes = six.ensure_binary(encoding.MessageToJson(occ.sbomReference.payload))
    ...
    req = kms_messages.CloudkmsProjectsLocationsKeyRingsCryptoKeysCryptoKeyVersionsAsymmetricSignRequest(name=kms_key_version, asymmetricSignRequest=kms_messages.AsymmetricSignRequest(data=data))
    resp = kms_client.projects_locations_keyRings_cryptoKeys_cryptoKeyVersions.AsymmetricSign(req)
    messages = ca_requests.GetMessages()
    evelope_signature = messages.EnvelopeSignature(keyid=kms_key_version, sig=resp.signature)
    occ.envelope = messages.Envelope(payload=payload_bytes, payloadType=occ.sbomReference.payloadType, signatures=[evelope_signature])
