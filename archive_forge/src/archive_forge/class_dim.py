...
class dim:
    ...
    """
    _binary_funcs = {operator.add: '+', operator.and_: '&', operator.eq: '==', operator.floordiv: '//', operator.ge: '>=', operator.gt: '>', operator.le: '<=', operator.lshift: '<<', operator.lt: '<', operator.mod: '%', operator.mul: '*', operator.ne: '!=', operator.or_: '|', operator.pow: '**', operator.rshift: '>>', operator.sub: '-', operator.truediv: '/'}
    _builtin_funcs = {abs: 'abs', round_: 'round'}
    _custom_funcs = {norm: 'norm', lognorm: 'lognorm', bin: 'bin', categorize: 'categorize', digitize: 'digitize', isin: 'isin', python_isin: 'isin', astype: 'astype', round_: 'round', iloc: 'iloc', loc: 'loc'}
    _numpy_funcs = {np.any: 'any', np.all: 'all', np.cumprod: 'cumprod', np.cumsum: 'cumsum', np.max: 'max', np.mean: 'mean', np.min: 'min', np.sum: 'sum', np.std: 'std', np.var: 'var', np.log: 'log', np.log10: 'log10'}
    ...
    def __init__(self, obj, *args, **kwargs):
        ...
        if args:
            ...
            fn = None
        if fn is not None:
            if not (isinstance(fn, function_types + (str,)) or any((fn in funcs for funcs in self._all_funcs))):
                raise ValueError('Second argument must be a function, found %s type' % type(fn))
    ...
    def __call__(self, *args, **kwargs):
        if not self.ops or not isinstance(self.ops[-1]['fn'], str) or 'accessor' not in self.ops[-1]['kwargs']:
            raise ValueError('Cannot call method on %r expression. Only methods accessed via namespaces, e.g. dim(...).df or dim(...).xr), can be called. ' % self)
        op = self.ops[-1]
        if op['fn'] == 'str':
    ...
    def params(self):
        ...
        for op in self.ops:
            ...
            for op_arg in op_args:
                ...
                if isinstance(op_arg, param.Parameter) and isinstance(op_arg.owner, param.Parameterized):
    ...
    def _apply_fn(self, dataset, data, fn, fn_name, args, kwargs, accessor, drange):
        if (fn is norm or fn is lognorm) and drange != {} and (not ('min' in kwargs and 'max' in kwargs)):
            data = fn(data, *drange)
        elif isinstance(fn, str):
            method = getattr(data, fn, None)
            if method is None:
                mtype = 'attribute' if accessor else 'method'
                raise AttributeError(f"{self!r} could not be applied to '{dataset!r}', '{fn}' {mtype} does not exist on {type(data).__name__} type.")
            if accessor:
                data = method
            else:
                try:
    ...
    def apply(self, dataset, flat=False, expanded=None, ranges=None, all_values=False, keep_index=False, compute=True, strict=False):
        ...
        dimension = self.dimension
        if expanded is None:
            expanded = not (dataset.interface.gridded and dimension in dataset.kdims or (dataset.interface.multi and dataset.interface.isunique(dataset, dimension, True)))
        if not self.applies(dataset) and (not isinstance(dataset, Graph) or not self.applies(dataset.nodes)):
            raise KeyError(f"One or more dimensions in the expression {self!r} could not resolve on '{dataset}'. Ensure all dimensions referenced by the expression are present on the supplied object.")
        if not self.interface_applies(dataset, coerce=self.coerce):
            if self.coerce:
                raise ValueError(f'The expression {self!r} assumes a {self.namespace}-like API but the dataset contains {dataset.interface.datatype} data and cannot be coerced.')
            else:
                raise ValueError(f'The expression {self!r} assumes a {self.namespace}-like API but the dataset contains {dataset.interface.datatype} data and coercion is disabled.')
        ...
        if dimension.name == '*':
            ...
            data = dataset.interface.values(dataset, lookup, expanded=expanded, flat=flat, compute=compute_for_compute, keep_index=keep_index_for_compute)
        for op in self.ops:
            fn, fn_name, args, kwargs, accessor = self._resolve_op(op, dataset, data, flat, expanded, ranges, all_values, keep_index_for_compute, compute_for_compute, strict)
    ...
    def __repr__(self):
        ...
        for i, o in enumerate(self.ops):
            ...
            if i == 0 and self._accessor and (')' in format_string):
                ...
                format_string = ''.join([format_string[:idx], ').', self._accessor, format_string[idx + 1:]])
