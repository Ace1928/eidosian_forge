...
class SetDiskAutoDelete(base.UpdateCommand):
  ...
  """
    ...
    def GetSetRequest(self, client, instance_ref, attached_disk):
        return (client.apitools_client.instances, 'SetDiskAutoDelete', client.messages.ComputeInstancesSetDiskAutoDeleteRequest(deviceName=attached_disk.deviceName, instance=instance_ref.instance, project=instance_ref.project, zone=instance_ref.zone, autoDelete=attached_disk.autoDelete))
    ...
    def GetAttachedDiskByDeviceName(self, resources, device_name, instance_ref, instance):
    ...
    """
        for disk in instance.disks:
            if disk.deviceName == device_name:
                return disk
        raise compute_exceptions.ArgumentError('No disk with device name [{}] is attached to instance [{}] in zone [{}].'.format(device_name, instance_ref.instance, instance_ref.zone))
    ...
    def Run(self, args):
        ...
        if args.disk:
            disk = self.GetAttachedDiskByName(holder.resources, args.disk, instance_ref, objects[0])
        else:
            disk = self.GetAttachedDiskByDeviceName(holder.resources, args.device_name, instance_ref, objects[0])
        if disk.autoDelete == args.auto_delete:
            log.status.Print('No change requested; skipping update for [{}].'.format(objects[0].name))
            return objects
        disk.autoDelete = args.auto_delete
        return client.MakeRequests([self.GetSetRequest(client, instance_ref, disk)])