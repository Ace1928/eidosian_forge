...
class InfinityEmbeddingsLocal(BaseModel, Embeddings):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        try:
            ...
            raise ImportError("Please install the `pip install 'infinity_emb[optimum,torch]>=0.0.24'` package to use the InfinityEmbeddingsLocal.")
        logger.debug(f'Using InfinityEmbeddingsLocal with kwargs {values}')
        values['engine'] = AsyncEmbeddingEngine(model_name_or_path=values['model'], device=values['device'], revision=values['revision'], model_warmup=values['model_warmup'], batch_size=values['batch_size'], engine=values['backend'])
        return values
    ...
    async def aembed_documents(self, texts: List[str]) -> List[List[float]]:
        ...
        if not self.engine.running:
            logger.warning('Starting Infinity engine on the fly. This is not recommended.Please start the engine before using it.')
            async with self:
