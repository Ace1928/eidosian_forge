...
class HyperEfficientNet(hypermodel.HyperModel):
    ...
    def __init__(self, input_shape=None, input_tensor=None, classes=None, augmentation_model=None, **kwargs):
        if not isinstance(augmentation_model, (hypermodel.HyperModel, keras.Model, type(None))):
            raise ValueError(f'Keyword augmentation_model should be a `HyperModel`, a Keras `Model` or empty. Received {augmentation_model}.')
        if not classes:
    ...
    def build(self, hp):
        ...
        top_dropout_rate = hp.Float('top_dropout_rate', min_value=0.2, max_value=0.8, step=0.2, default=0.2)
    ...
    def _compile(self, model, hp):
        ...
        optimizer = keras.optimizers.SGD(momentum=0.1, learning_rate=learning_rate)
