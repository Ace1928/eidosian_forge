...
class TransformStep(BaseStep):
    ...
    def _validate_and_apply_step_config(self):
        ...
        if 'using' in self.step_config:
            if self.step_config['using'] not in ['custom']:
                raise MlflowException(f"Invalid transform step configuration value {self.step_config['using']} for key 'using'. Supported values are: ['custom']", error_code=INVALID_PARAMETER_VALUE)
        else:
    ...
    def _run(self, output_directory):
        ...
        train_data_path = get_step_output_path(recipe_root_path=self.recipe_root, step_name='split', relative_path='train.parquet')
        train_df = pd.read_parquet(train_data_path)
        validate_classification_config(self.task, self.positive_class, train_df, self.target_col)
        validation_data_path = get_step_output_path(recipe_root_path=self.recipe_root, step_name='split', relative_path='validation.parquet')
        ...
        if 'transformer_method' not in self.step_config and self.step_config['using'] == 'custom':
            raise MlflowException("Missing 'transformer_method' configuration in the transform step, which is using 'custom'.", error_code=INVALID_PARAMETER_VALUE)
        method_config = self.step_config.get('transformer_method')
        transformer = None
        if method_config and self.step_config['using'] == 'custom':
            transformer_fn = getattr(importlib.import_module(_USER_DEFINED_TRANSFORM_STEP_MODULE), method_config)
        ...
        validation_transformed.to_parquet(os.path.join(output_directory, 'transformed_validation_data.parquet'))
    ...
    def get_artifacts(self):
        return [DataframeArtifact('transformed_training_data', self.recipe_root, self.name, 'transformed_training_data.parquet'), DataframeArtifact('transformed_validation_data', self.recipe_root, self.name, 'transformed_validation_data.parquet'), TransformerArtifact('transformer', self.recipe_root, self.name, self.tracking_config.tracking_uri)]
