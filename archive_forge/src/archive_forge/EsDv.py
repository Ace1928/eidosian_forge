...
class FileOperationWorker(QObject):
    ...
    async def process_directory(self, directory: Path) -> None:
        # Directory processing logic (simplified for brevity)
        pass

    def run(self):
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        loop.run_until_complete(self.process_directory(self.directory))
        loop.close()
        self.finished.emit()
...
def main() -> None:
    ...
    app = QApplication(sys.argv)  # Create QApplication instance in the main thread

    loop: asyncio.AbstractEventLoop = asyncio.new_event_loop()
    t: threading.Thread = threading.Thread(
        target=run_asyncio_loop, args=(loop, async_main(app))
    )
    t.start()
    t.join()
