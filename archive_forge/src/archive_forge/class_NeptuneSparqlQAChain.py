...
class NeptuneSparqlQAChain(Chain):
    ...
    *Security note*: Make sure that the database connection uses credentials
    ...
    @classmethod
    def from_llm(cls, llm: BaseLanguageModel, *, qa_prompt: BasePromptTemplate=SPARQL_QA_PROMPT, sparql_prompt: BasePromptTemplate=SPARQL_GENERATION_PROMPT, examples: Optional[str]=None, **kwargs: Any) -> NeptuneSparqlQAChain:
        """Initialize from LLM."""
        ...
        return cls(qa_chain=qa_chain, sparql_generation_chain=sparql_generation_chain, examples=examples, **kwargs)
    ...
    def _call(self, inputs: Dict[str, Any], run_manager: Optional[CallbackManagerForChainRun]=None) -> Dict[str, str]:
        ...
        generated_sparql = self.sparql_generation_chain.run({'prompt': prompt, 'schema': self.graph.get_schema}, callbacks=callbacks)
