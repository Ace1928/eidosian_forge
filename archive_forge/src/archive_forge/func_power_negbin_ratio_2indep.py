...
def power_negbin_ratio_2indep(rate1, rate2, nobs1, nobs_ratio=1, exposure=1, value=1, alpha=0.05, dispersion=0.01, alternative='two-sided', method_var='alt', return_results=True):
    ...
    v1 = (1 / rate1 + 1 / (nobs_ratio * rate2)) / exposure + (1 + nobs_ratio) / nobs_ratio * dispersion
    if method_var == 'alt':
        v0 = v1
    elif method_var == 'ftotal':
        v0 = (1 + value * nobs_ratio) ** 2 / (exposure * nobs_ratio * value * (rate1 + nobs_ratio * rate2))
        v0 += (1 + nobs_ratio) / nobs_ratio * dispersion
    elif method_var == 'score':
        v0 = _var_cmle_negbin(rate1, rate2, nobs_ratio, exposure=exposure, value=value, dispersion=dispersion)[0]
    else:
    ...
    if return_results:
        res = HolderTuple(power=pow_, std_null=std_null, std_alt=std_alt, nobs1=nobs1, nobs2=nobs2, nobs_ratio=nobs_ratio, alpha=alpha, tuple_=('power',))
        return res
    return pow_