...
class GeodesicsWindow(tkinter.Toplevel):
    ...
    def __init__(self, inside_viewer, *args, **kwards):
        ...
        self.headings = (('Show', 0, 0, 1), ('Color', 1, 0, 1), ('Word(s)', 2, 0, 1), ('Complex length', 3, 0, 2), ('Radius', 5, 0, 1), ('    ', 6, 0, 1))
        ...
        self.length_box = ttk.Spinbox(left_top_frame, from_=0.2, to=20.0, increment=0.2, textvariable=self.length_var, width=4)
        self.length_box.grid(row=0, column=0)
        self.length_box.bind('<Return>', self.add_length_spectrum)
        self.length_box.focus_set()
        self.length_button = ttk.Button(left_top_frame, text='Add up to length', command=self.add_length_spectrum)
        ...
        self.word_button = ttk.Button(right_top_frame, text='Add word', command=self.add_word)
