...
def test_cannot_check_grant_for_user_own_domain_on_own_domain_with_role_other_domain(self):
    ...
    with self.test_client() as c:
        c.get('/v3/domains/%s/users/%s/roles/%s' % (domain_id, user['id'], role['id']), headers=self.headers, expected_status_code=http.client.FORBIDDEN)