...
class PullRequest(CompletableGithubObject):
    ...
    def create_comment(self, body: str, commit: github.Commit.Commit, path: str, position: int) -> github.PullRequestComment.PullRequestComment:
        """
    ...
    def create_review(self, commit: Opt[github.Commit.Commit]=NotSet, body: Opt[str]=NotSet, event: Opt[str]=NotSet, comments: Opt[list[ReviewComment]]=NotSet) -> github.PullRequestReview.PullRequestReview:
        """
    ...
    def create_review_request(self, reviewers: Opt[list[str] | str]=NotSet, team_reviewers: Opt[list[str] | str]=NotSet) -> None:
        """
        ...
        headers, data = self._requester.requestJsonAndCheck('POST', f'{self.url}/requested_reviewers', input=post_parameters)
    ...
    def edit(self, title: Opt[str]=NotSet, body: Opt[str]=NotSet, state: Opt[str]=NotSet, base: Opt[str]=NotSet, maintainer_can_modify: Opt[bool]=NotSet) -> None:
        """
        ...
        post_parameters = NotSet.remove_unset_items({'title': title, 'body': body, 'state': state, 'base': base, 'maintainer_can_modify': maintainer_can_modify})
    ...
    def get_comments(self, sort: Opt[str]=NotSet, direction: Opt[str]=NotSet, since: Opt[datetime]=NotSet) -> PaginatedList[github.PullRequestComment.PullRequestComment]:
        """
    ...
    def get_reviews(self) -> PaginatedList[github.PullRequestReview.PullRequestReview]:
        ...
        """
        return PaginatedList(github.PullRequestReview.PullRequestReview, self._requester, f'{self.url}/reviews', None)
    ...
    def disable_automerge(self, client_mutation_id: Opt[str]=NotSet) -> dict[str, Any]:
        """
        ...
        """
        assert is_optional(client_mutation_id, str), client_mutation_id
        variables = {'pullRequestId': self.node_id, 'clientMutationId': client_mutation_id}
        _, data = self._requester.graphql_named_mutation(mutation_name='disable_pull_request_auto_merge', variables=NotSet.remove_unset_items(variables), output='actor { avatarUrl login resourcePath url } clientMutationId')
        return data

    def merge(self, commit_message: Opt[str]=NotSet, commit_title: Opt[str]=NotSet, merge_method: Opt[str]=NotSet, sha: Opt[str]=NotSet) -> github.PullRequestMergeStatus.PullRequestMergeStatus:
        """
        ...
        post_parameters = NotSet.remove_unset_items({'commit_message': commit_message, 'commit_title': commit_title, 'merge_method': merge_method, 'sha': sha})
    ...
    def add_to_assignees(self, *assignees: github.NamedUser.NamedUser | str) -> None:
        ...
        headers, data = self._requester.requestJsonAndCheck('POST', f'{self.issue_url}/assignees', input=post_parameters)
    ...
    def remove_from_assignees(self, *assignees: github.NamedUser.NamedUser | str) -> None:
        ...
        assert all((isinstance(element, (github.NamedUser.NamedUser, str)) for element in assignees)), assignees
        post_parameters = {'assignees': [assignee.login if isinstance(assignee, github.NamedUser.NamedUser) else assignee for assignee in assignees]}
        headers, data = self._requester.requestJsonAndCheck('DELETE', f'{self.issue_url}/assignees', input=post_parameters)
        self._useAttributes({'assignees': data['assignees']})
    ...
    def _useAttributes(self, attributes: dict[str, Any]) -> None:
        ...
        if 'mergeable_state' in attributes:
        ...
        if 'rebaseable' in attributes:
            self._rebaseable = self._makeBoolAttribute(attributes['rebaseable'])
        if 'review_comment_url' in attributes:
        ...
        if 'review_comments_url' in attributes:
        ...
        if 'requested_reviewers' in attributes:
            self._requested_reviewers = self._makeListOfClassesAttribute(github.NamedUser.NamedUser, attributes['requested_reviewers'])
