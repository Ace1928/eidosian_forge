...
class EnvSpec(abc.ABC):
    ...
    def create_observation_space(self):
        return self._singlify(spaces.Dict({agent: spaces.Dict({o.to_string(): o.space for o in self.observables}) for agent in self.agent_names}))

    def create_action_space(self):
        return self._singlify(spaces.Dict({agent: spaces.Dict({a.to_string(): a.space for a in self.actionables}) for agent in self.agent_names}))
    ...
    def register(self, fake=False):
        reg_spec = dict(id=('Fake' if fake else '') + self.name, entry_point=self._entry_point(fake), kwargs=self._env_kwargs(), max_episode_steps=self.max_episode_steps)
    ...
    def _entry_point(self, fake: bool) -> str:
        if fake:
            return EnvSpec.U_FAKE_SINGLE_AGENT_ENTRYPOINT if self.is_single_agent else EnvSpec.U_FAKE_MULTI_AGENT_ENTRYPOINT
        else:
