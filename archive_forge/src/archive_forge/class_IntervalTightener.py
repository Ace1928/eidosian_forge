...
class IntervalTightener(PersistentBase):
    ...
    def _add_variables(self, variables: List[_GeneralVarData]):
        if self._symbolic_solver_labels:
            ...
            symbol_map = None
            labeler = None
        cmodel.process_pyomo_vars(self._pyomo_expr_types, variables, self._var_map, self._param_map, self._vars, self._rvar_map, set_name, symbol_map, labeler, False)
    ...
    def _add_constraints(self, cons: List[_GeneralConstraintData]):
        cmodel.process_fbbt_constraints(self._cmodel, self._pyomo_expr_types, cons, self._var_map, self._param_map, self._active_constraints, self._con_map, self._rcon_map)
        if self._symbolic_solver_labels:
    ...
    def _update_variables(self, variables: List[_GeneralVarData]):
        cmodel.process_pyomo_vars(self._pyomo_expr_types, variables, self._var_map, self._param_map, self._vars, self._rvar_map, False, None, None, True)
    ...
    def perform_fbbt(self, model: _BlockData, symbolic_solver_labels: Optional[bool]=None):
        if model is not self._model:
            ...
            if symbolic_solver_labels is not None and symbolic_solver_labels != self._symbolic_solver_labels:
                raise RuntimeError('symbolic_solver_labels can only be changed through the set_instance method. Please either use set_instance or create a new instance of IntervalTightener.')
            self.update()
        try:
