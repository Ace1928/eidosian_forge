...
class QutritDevice(QubitDevice):
    ...
    * :meth:`~.expval`, :meth:`~.var`, :meth:`~.sample`: return expectation values,
    ...
    """
    observables = {'Identity', 'THermitian'}
    ...
    def generate_basis_states(self, num_wires, dtype=np.uint32):
        ...
        return np.fromiter(itertools.chain(*basis_states_generator), dtype=dtype).reshape(-1, num_wires)
    ...
    def density_matrix(self, wires):
        ...
        """
        raise qml.QuantumFunctionError('Unsupported return type specified for observable density matrix')
    ...
    def vn_entropy(self, wires, log_base):
        ...
        """
        raise qml.QuantumFunctionError('Unsupported return type specified for observable Von Neumann entropy')
    ...
    def mutual_info(self, wires0, wires1, log_base):
        ...
        """
        raise qml.QuantumFunctionError('Unsupported return type specified for observable mutual information')
    ...
    def classical_shadow(self, obs, circuit):
        ...
        """
        raise qml.QuantumFunctionError("Qutrit devices don't support classical shadow measurements.")
    ...
    def shadow_expval(self, obs, circuit):
        ...
        """
        raise qml.QuantumFunctionError("Qutrit devices don't support shadow expectation value measurements.")
    ...
    def sample(self, observable, shot_range=None, bin_size=None, counts=False):
        ...
        if counts:
            ...
            return [_samples_to_counts(bin_sample, no_observable_provided) for bin_sample in samples.reshape(shape)]
        return samples.reshape((num_wires, bin_size, -1)) if no_observable_provided else samples.reshape((bin_size, -1))
