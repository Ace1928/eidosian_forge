...
class PersonalityCaptionsTeacher(FixedDialogTeacher):
    ...
    def add_cmdline_args(argparser):
        ...
        agent.add_argument('--num-test-labels', type=int, default=1, choices=[1, 5], help='Provide model with either 1 or 5 possible labels for each test example. The number of label candidates for each case is 100 and 500 respectively.')
    ...
    def get(self, episode_idx, entry_idx=0):
        ...
        action = {'text': data['personality'] if self.include_personality else '', 'image_id': data['image_hash'], 'episode_done': True, 'labels': [data['comment']]}
