...
def _SetCmekFields(args: parser_extensions.Namespace, function: api_types.Function, existing_function: Optional[api_types.Function], function_ref: resources.Resource) -> FrozenSet[str]:
  ...
  """
    updated_fields = set()
    function.kmsKeyName = existing_function.kmsKeyName if existing_function else None
    if args.IsSpecified('kms_key') or args.IsSpecified('clear_kms_key'):
        function.kmsKeyName = None if args.IsSpecified('clear_kms_key') else args.kms_key
    if existing_function is None or function.kmsKeyName != existing_function.kmsKeyName:
