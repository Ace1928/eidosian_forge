...
class ActionConnectorPipeline(ConnectorPipeline, ActionConnector):
    ...
    def to_state(self):
        ...
        for c in self.connectors:
            state = c.to_state()
            assert isinstance(state, tuple) and len(state) == 2, f'Serialized connector state must be in the format of Tuple[name: str, params: Any]. Instead we got {state}for connector {c.__name__}.'
