...
class TEMCalculator(AbstractDiffractionPatternCalculator):
    ...
    def __init__(self, symprec: float | None=None, voltage: float=200, beam_direction: tuple[int, int, int]=(0, 0, 1), camera_length: int=160, debye_waller_factors: dict[str, float] | None=None, cs: float=1) -> None:
        """
        Args:
    ...
    def get_pattern(self, structure: Structure, scaled: bool | None=None, two_theta_range: tuple[float, float] | None=None) -> pd.DataFrame:
        """
        ...
        rows_list = []
        for dot in tem_dots:
            dict1 = {'Position': dot.position, '(hkl)': dot.hkl, 'Intensity (norm)': dot.intensity, 'Film radius': dot.film_radius, 'Interplanar Spacing': dot.d_spacing}
    ...
    def is_parallel(self, structure: Structure, plane: tuple[int, int, int], other_plane: tuple[int, int, int]) -> bool:
        """
    ...
    @staticmethod
    def get_plot_coeffs(p1: tuple[int, int, int], p2: tuple[int, int, int], p3: tuple[int, int, int]) -> np.ndarray:
        """
    ...
    def get_positions(self, structure: Structure, points: list) -> dict[tuple[int, int, int], np.ndarray]:
        ...
        for plane in points:
            ...
            pos = np.array([coeffs[0] * positions[first_point][0] + coeffs[1] * positions[second_point][0], coeffs[0] * positions[first_point][1] + coeffs[1] * positions[second_point][1]])
            positions[plane] = pos
        points.append((0, 0, 0))
