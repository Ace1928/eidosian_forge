...
class CinderVolumeTest(vt_base.VolumeTestCase):
    ...
    def create_volume(self, t, stack, resource_name, mock_check_extend_ready=True):
        rsrc = super(CinderVolumeTest, self).create_volume(t, stack, resource_name, validate=False)
    ...
    def test_cinder_volume_size_constraint(self):
        ...
        error = self.assertRaises(exception.StackValidationFailed, self.create_volume, self.t, stack, 'volume')
        self.assertEqual('Property error: resources.volume.properties.size: 0 is out of range (min: 1, max: None)', str(error))
    ...
    def test_cinder_create(self):
        ...
        self.cinder_fc.volumes.create.assert_called_once_with(size=1, availability_zone='nova', description='test_description', name='test_name', metadata={'key': 'value'}, volume_type='lvm')
    ...
    def test_cinder_volume_shrink_fails(self):
        ...
        self._mock_create_volume(vt_base.FakeVolume('creating'), self.stack_name, size=2, extra_get_mocks=[fv])
        ...
        self.assertEqual('NotSupported: resources.volume: Shrinking volume is not supported.', str(ex))
    ...
    def test_cinder_volume_extend_fails_to_start(self):
        ...
        fv = vt_base.FakeVolume('available', size=1, attachments=[])
        self._mock_create_volume(vt_base.FakeVolume('creating'), self.stack_name, extra_get_mocks=[fv])
    ...
    def test_cinder_volume_extend_fails_to_complete(self):
        ...
        fv = vt_base.FakeVolume('available', size=1, attachments=[])
        self._mock_create_volume(vt_base.FakeVolume('creating'), self.stack_name, extra_get_mocks=[fv, vt_base.FakeVolume('extending'), vt_base.FakeVolume('extending'), vt_base.FakeVolume('error_extending')])
        stack = utils.parse_stack(self.t, stack_name=self.stack_name)
        rsrc = self.create_volume(self.t, stack, 'volume')
        props = copy.deepcopy(rsrc.properties.data)
        props['size'] = 2
        ...
        update_task = scheduler.TaskRunner(rsrc.update, after)
        ex = self.assertRaises(exception.ResourceFailure, update_task)
        self.assertIn('Volume resize failed - Unknown status error_extending', str(ex))
        self.assertEqual((rsrc.UPDATE, rsrc.FAILED), rsrc.state)
        self.cinder_fc.volumes.extend.assert_called_once_with(fv.id, 2)
    ...
    def test_cinder_volume_attachment_update_volume(self):
        ...
        self.stub_VolumeConstraint_validate()
        self._mock_create_volume(vt_base.FakeVolume('creating'), self.stack_name, extra_get_mocks=[fv2, fv3, fv_4ready, fv5, fv6, fv7ready], extra_create_mocks=[fv2])
        self.fc.volumes.create_server_volume.side_effect = [fv4, fv7]
        self.fc.volumes.get_server_volume.side_effect = [fv5, fv5, fakes_nova.fake_exception()]
        self.fc.volumes.delete_server_volume.return_value = None
        stack = utils.parse_stack(self.t, stack_name=self.stack_name)
        self.create_volume(self.t, stack, 'volume')
        ...
        self.cinder_fc.volumes.create.assert_called_with(size=2, availability_zone='nova', description=None, name=vol2_name, metadata={})
        self.fc.volumes.get_server_volume.assert_called_with(u'WikiDatabase', 'vol-123')
        self.fc.volumes.delete_server_volume.assert_called_with('WikiDatabase', 'vol-123')

    def test_cinder_volume_attachment_update_server(self):
        self.stack_name = 'test_cvolume_attach_usrv_stack'
        fv1 = self._mock_create_server_volume_script(vt_base.FakeVolume('attaching'))
        fva = vt_base.FakeVolume('in-use')
        fv2 = self._mock_create_server_volume_script(vt_base.FakeVolume('attaching'), update=True)
        self._mock_create_volume(vt_base.FakeVolume('creating'), self.stack_name, extra_get_mocks=[fv1, fva, vt_base.FakeVolume('available'), fv2])
        self.stub_VolumeConstraint_validate()
        self.fc.volumes.get_server_volume.side_effect = [fva, fva, fakes_nova.fake_exception()]
        self.fc.volumes.delete_server_volume.return_value = None
        stack = utils.parse_stack(self.t, stack_name=self.stack_name)
    ...
    def test_cinder_volume_attachment_with_serv_resize_task_state(self):
        ...
        self.fc.volumes.create_server_volume.side_effect = [nova_exp.Conflict('409')]
        self.assertEqual(False, rsrc.check_update_complete((prg_detach, prg_attach)))
    ...
    def test_cinder_create_with_size_snapshot_and_image(self):
        ...
        combinations = {'size': 1, 'image': 'image-123', 'snapshot_id': 'snapshot-123'}
        self.stub_ImageConstraint_validate()
        self.stub_SnapshotConstraint_validate()
        err_msg = 'If "size" is provided, only one of "image", "imageRef", "source_volid", "snapshot_id" can be specified, but currently specified options: [\'snapshot_id\', \'image\'].'
        self._test_cinder_create_invalid_property_combinations(self.stack_name, combinations, err_msg, exception.StackValidationFailed)

    def test_cinder_create_with_size_snapshot_and_imageRef(self):
        self.stack_name = 'test_create_with_size_snapshot_and_imageRef'
        combinations = {'size': 1, 'imageRef': 'image-123', 'snapshot_id': 'snapshot-123'}
        self.stub_ImageConstraint_validate()
        self.stub_SnapshotConstraint_validate()
        err_msg = 'If "size" is provided, only one of "image", "imageRef", "source_volid", "snapshot_id" can be specified, but currently specified options: [\'snapshot_id\', \'image\'].'
        self._test_cinder_create_invalid_property_combinations(self.stack_name, combinations, err_msg, exception.StackValidationFailed)
    ...
    def test_cinder_create_with_snapshot_and_source_volume(self):
        ...
        combinations = {'source_volid': 'source_volume-123', 'snapshot_id': 'snapshot-123'}
        err_msg = 'If neither "backup_id" nor "size" is provided, one and only one of "source_volid", "snapshot_id" must be specified, but currently specified options: [\'snapshot_id\', \'source_volid\'].'
        self.stub_VolumeConstraint_validate()
        self.stub_SnapshotConstraint_validate()
        self._test_cinder_create_invalid_property_combinations(self.stack_name, combinations, err_msg, exception.StackValidationFailed)
    ...
    def test_cinder_create_with_image_and_source_volume(self):
        ...
        combinations = {'source_volid': 'source_volume-123', 'image': 'image-123'}
        err_msg = 'If neither "backup_id" nor "size" is provided, one and only one of "source_volid", "snapshot_id" must be specified, but currently specified options: [\'source_volid\', \'image\'].'
        self.stub_VolumeConstraint_validate()
        self.stub_ImageConstraint_validate()
        self._test_cinder_create_invalid_property_combinations(self.stack_name, combinations, err_msg, exception.StackValidationFailed)
    ...
    def test_cinder_create_no_size_no_combinations(self):
        ...
        combinations = {}
        err_msg = 'If neither "backup_id" nor "size" is provided, one and only one of "source_volid", "snapshot_id" must be specified, but currently specified options: [].'
        self._test_cinder_create_invalid_property_combinations(self.stack_name, combinations, err_msg, exception.StackValidationFailed)
    ...
    def test_volume_restore_failed(self):
        self._test_volume_restore(stack_name='test_volume_restore_failed', final_status='error', stack_final_status=('RESTORE', 'FAILED'))
    ...
    def test_empty_string_az(self):
        ...
        fv_ready = vt_base.FakeVolume('available', id=fv.id)
        self.cinder_fc.volumes.get.side_effect = [fv, fv_ready]
        self.t['resources']['volume']['properties'] = {'size': '1', 'availability_zone': ''}
        stack = utils.parse_stack(self.t, stack_name=self.stack_name)
        self.create_volume(self.t, stack, 'volume')
        self.cinder_fc.volumes.create.assert_called_once_with(size=1, availability_zone=None, description=None, name=vol_name, metadata={})
    ...
    def test_try_detach_volume_if_server_was_temporarily_in_error(self):
        ...
        rsrc = self.create_attachment(self.t, stack, 'attachment')
        scheduler.TaskRunner(rsrc.delete)()
        self.assertEqual(self.fc.volumes.delete_server_volume.call_count, len(nova_responses))