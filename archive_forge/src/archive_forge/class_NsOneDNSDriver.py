...
class NsOneDNSDriver(DNSDriver):
    ...
    RECORD_TYPE_MAP = {RecordType.A: 'A', RecordType.AAAA: 'AAAA', RecordType.CNAME: 'CNAME', RecordType.MX: 'MX', RecordType.NS: 'NS', RecordType.PTR: 'PTR', RecordType.SOA: 'SOA', RecordType.SRV: 'SRV', RecordType.TXT: 'TXT'}
    ...
    def create_record(self, name, zone, type, data, extra=None):
        ...
        :return: :class:`Record`
        """
        record_name = '{}.{}'.format(name, zone.domain) if name != '' else zone.domain
        action = '/v1/zones/{}/{}/{}'.format(zone.domain, record_name, type)
        if type == RecordType.MX:
            answer = [extra.get('priority', 10), data]
        else:
            answer = [data]
        raw_data = {'answers': [{'answer': answer}], 'type': type, 'domain': record_name, 'zone': zone.domain}
    ...
    def update_record(self, record, name, type, data, extra=None):
        ...
        """
        zone = record.zone
        action = '/v1/zones/{}/{}/{}'.format(zone.domain, '{}.{}'.format(name, zone.domain), type)
    ...
    def _to_zone(self, item):
        ...
        zone = Zone(domain=item['zone'], id=item['id'], type=item.get('type'), extra=extra, ttl=extra.get('ttl'), driver=self)
        return zone
    ...
    def _to_record(self, item, zone):
        ...
        record = Record(id=item['id'], name=item['domain'], type=item['type'], data=data, zone=zone, driver=self, extra=extra)
        return record
