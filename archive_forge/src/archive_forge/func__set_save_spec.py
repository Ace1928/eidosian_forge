...
def _set_save_spec(self, inputs):
    ...
    for name, tensor in zip(input_names, flat_inputs):
        specs.append(tf_utils.get_tensor_spec(tensor, dynamic_batch=False, name=name))
    ...
    if self.__class__.__name__ == 'Sequential' and self._build_input_shape is None:
        self._build_input_shape = nest.map_structure(lambda x: None if x is None else x.shape, specs)