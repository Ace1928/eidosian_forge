...
class MessengerManager(ChatServiceManager):
    ...
    def parse_additional_args(self, opt):
        ...
        if self.service_reference_id == 1:
            raise RuntimeError('Please configure your own page in order to run this task. See the docs (https://parl.ai/docs/tutorial_messenger.html) for more information.')
    ...
    def _confirm_message_delivery(self, event):
        self._log_debug('Messages {} marked as received.'.format(event['delivery']['mids']))
    ...
    def _create_agent(self, task_id, agent_id):
        ...
        :param agent_id:
            int agent id
        """
        return MessengerAgent(self.opt, self, task_id, agent_id, self.service_reference_id)

    def _log_missing_agent(self, agent_id, assignment_id):
        """
        Log the occurence of a missing agent.
        """
        log_utils.print_and_log(logging.WARN, 'Expected to have an agent for {}_{}, yet none was found'.format(agent_id, assignment_id))
    ...
    def setup_server(self):
        ...
        if self.opt['local'] is True:
            log_utils.print_and_log(logging.INFO, 'In order to run the server locally, you will need to have a public HTTPS endpoint (SSL signed) running on the server you are currently excecuting ParlAI on. Enter that public URL hostname when prompted and ensure that the port being used by ParlAI (usually 3000) has external traffic routed to it.', should_print=True)
            input('Please press Enter to continue... ')
        log_utils.print_and_log(logging.INFO, 'Setting up Messenger webhook...', should_print=True)
        task_name = '{}-{}'.format('ParlAI-Messenger', self.opt['task'])
        self.server_task_name = ''.join((e for e in task_name.lower() if e.isalnum() or e == '-'))
        self.server_url = server_utils.setup_server(self.server_task_name, local=self.opt['local'])
        log_utils.print_and_log(logging.INFO, 'Webhook address: {}/webhook'.format(self.server_url), should_print=True)
    ...
    def setup_socket(self):
        ...
        if self.bypass_server_setup:
            return
        log_utils.print_and_log(logging.INFO, 'Local: Setting up WebSocket...', should_print=True)
        ...
        self.socket = ChatServiceMessageSocket(socket_use_url, self.port, self._handle_webhook_event)
    ...
    def observe_message(self, receiver_id, text, quick_replies=None, persona_id=None):
        ...
        """
        return self.sender.send_fb_message(receiver_id, text, True, quick_replies=quick_replies, persona_id=persona_id)
    ...
    def observe_payload(self, receiver_id, data, quick_replies=None, persona_id=None):
        ...
        :param persona_id:
            identifier of persona
        """
        return self.sender.send_fb_payload(receiver_id, data, quick_replies=quick_replies, persona_id=persona_id)
