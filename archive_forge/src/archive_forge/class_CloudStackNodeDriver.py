...
class CloudStackNodeDriver(CloudStackDriverMixIn, NodeDriver):
    ...
    NODE_STATE_MAP = {'Running': NodeState.RUNNING, 'Starting': NodeState.REBOOTING, 'Migrating': NodeState.MIGRATING, 'Stopped': NodeState.STOPPED, 'Stopping': NodeState.PENDING, 'Destroyed': NodeState.TERMINATED, 'Expunging': NodeState.PENDING, 'Error': NodeState.TERMINATED}
    VOLUME_STATE_MAP = {'Creating': StorageVolumeState.CREATING, 'Destroying': StorageVolumeState.DELETING, 'Expunging': StorageVolumeState.DELETING, 'Destroy': StorageVolumeState.DELETED, 'Expunged': StorageVolumeState.DELETED, 'Allocated': StorageVolumeState.AVAILABLE, 'Ready': StorageVolumeState.AVAILABLE, 'Snapshotting': StorageVolumeState.BACKUP, 'UploadError': StorageVolumeState.ERROR, 'Migrating': StorageVolumeState.MIGRATING}
    ...
    def list_images(self, location=None):
        ...
        for img in imgs.get('template', []):
            ...
            images.append(NodeImage(id=img['id'], name=img['name'], driver=self.connection.driver, extra=extra))
        return images
    ...
    def list_nodes(self, project=None, location=None):
        ...
        for vm in vms.get('virtualmachine', []):
            ...
            addresses = [CloudStackAddress(id=address_id, address=address, driver=node.driver) for address, address_id in addresses]
            node.extra['ip_addresses'] = addresses
            rules = []
            for addr in addresses:
                for r in ip_forwarding_rules.get('ipforwardingrule', []):
                    if str(r['virtualmachineid']) == node.id:
                        rule = CloudStackIPForwardingRule(node, r['id'], addr, r['protocol'].upper(), r['startport'], r['endport'])
                        rules.append(rule)
            node.extra['ip_forwarding_rules'] = rules
            rules = []
            for r in port_forwarding_rules.get('portforwardingrule', []):
                if str(r['virtualmachineid']) == node.id:
                    addr = [CloudStackAddress(id=a['id'], address=a['ipaddress'], driver=node.driver) for a in addrs.get('publicipaddress', []) if a['ipaddress'] == r['ipaddress']]
                    rule = CloudStackPortForwardingRule(node, r['id'], addr[0], r['protocol'].upper(), r['publicport'], r['privateport'], r['publicendport'], r['privateendport'])
    ...
    def ex_get_node(self, node_id, project=None):
        ...
        addresses = [CloudStackAddress(id=address_id, address=address, driver=node.driver) for address, address_id in public_ips.items()]
        node.extra['ip_addresses'] = addresses
        rules = []
    ...
    def create_node(self, name, size, image, location=None, networks=None, project=None, diskoffering=None, ex_keyname=None, ex_userdata=None, ex_security_groups=None, ex_displayname=None, ex_ip_address=None, ex_start_vm=False, ex_rootdisksize=None, ex_affinity_groups=None):
        """
        Create a new node
        ...
        :rtype:     :class:`.CloudStackNode`
        """
        server_params = self._create_args_to_params(node=None, name=name, size=size, image=image, location=location, networks=networks, diskoffering=diskoffering, ex_keyname=ex_keyname, ex_userdata=ex_userdata, ex_security_groups=ex_security_groups, ex_displayname=ex_displayname, ex_ip_address=ex_ip_address, ex_start_vm=ex_start_vm, ex_rootdisksize=ex_rootdisksize, ex_affinity_groups=ex_affinity_groups)
        data = self._async_request(command='deployVirtualMachine', params=server_params, method='GET')['virtualmachine']
    ...
    def ex_change_node_size(self, node, offering):
        ...
        :rtype ``str``
        """
        res = self._async_request(command='scaleVirtualMachine', params={'id': node.id, 'serviceofferingid': offering.id}, method='GET')
    ...
    def ex_start(self, node):
        ...
        :rtype ``str``
        """
        res = self._async_request(command='startVirtualMachine', params={'id': node.id}, method='GET')
    ...
    def ex_list_networks(self, project=None):
        ...
        for net in nets:
            ...
            networks.append(CloudStackNetwork(net['displaytext'], net['name'], net['networkofferingid'], net['id'], net['zoneid'], self, extra=extra))
        return networks
    ...
    def ex_create_network(self, display_text, name, network_offering, location, gateway=None, netmask=None, network_domain=None, vpc_id=None, project_id=None):
        """
        ...
        """
        extra_map = RESOURCE_EXTRA_ATTRIBUTES_MAP['network']
        args = {'displaytext': display_text, 'name': name, 'networkofferingid': network_offering.id, 'zoneid': location.id}
        ...
        network = CloudStackNetwork(display_text, name, network_offering.id, result['id'], location.id, self, extra=extra)
        return network
    ...
    def create_volume(self, size, name, location=None, snapshot=None, ex_volume_type=None):
        ...
        params = {'name': name, 'diskOfferingId': diskOffering.id, 'zoneId': location.id}
        ...
        return StorageVolume(id=volumeResponse['id'], name=name, size=size, state=state, driver=self, extra=dict(name=volumeResponse['name']))
    ...
    def attach_volume(self, node, volume, device=None):
        ...
        :rtype: ``bool``
        """
        self._async_request(command='attachVolume', params={'id': volume.id, 'virtualMachineId': node.id}, method='GET')
        return True
    ...
    def list_volumes(self, node=None):
        ...
        """
        if node:
            volumes = self._sync_request(command='listVolumes', params={'virtualmachineid': node.id}, method='GET')
        else:
        ...
        for vol in volumes.get('volume', []):
            ...
            list_volumes.append(StorageVolume(id=vol['id'], name=vol['name'], size=vol['size'], state=state, driver=self, extra=extra))
        return list_volumes
    ...
    def import_key_pair_from_string(self, name, key_material):
        ...
        :rtype: :class:`libcloud.compute.base.KeyPair`
        """
        res = self._sync_request(command='registerSSHKeyPair', params={'name': name, 'publickey': key_material}, method='GET')
        key_pair = self._to_key_pair(data=res['keypair'])
        return key_pair
    ...
    def ex_list_public_ips(self):
        ...
        for ip in res['publicipaddress']:
            ips.append(CloudStackAddress(ip['id'], ip['ipaddress'], self, ip.get('associatednetworkid', []), ip.get('vpcid'), ip.get('virtualmachineid')))
        return ips
    ...
    def ex_create_firewall_rule(self, address, cidr_list, protocol, icmp_code=None, icmp_type=None, start_port=None, end_port=None):
        """
        ...
        rule = CloudStackFirewallRule(result['firewallrule']['id'], address, cidr_list, protocol, icmp_code, icmp_type, start_port, end_port)
        return rule
    ...
    def ex_list_keypairs(self, **kwargs):
        ...
        for key_pair in key_pairs:
            item = {'name': key_pair.name, 'fingerprint': key_pair.fingerprint, 'privateKey': key_pair.private_key}
            result.append(item)
        return result
    ...
    def ex_create_keypair(self, name, **kwargs):
        ...
        result = {'name': key_pair.name, 'fingerprint': key_pair.fingerprint, 'privateKey': key_pair.private_key}
        return result
    ...
    def ex_create_security_group(self, name, **kwargs):
        ...
        params = {'name': name}
        params.update(extra_args)
        return self._sync_request(command='createSecurityGroup', params=params, method='GET')['securitygroup']
    ...
    def ex_delete_security_group(self, name):
        ...
        :rtype: ``bool``
        """
        return self._sync_request(command='deleteSecurityGroup', params={'name': name}, method='GET')['success']

    def ex_authorize_security_group_ingress(self, securitygroupname, protocol, cidrlist, startport=None, endport=None, icmptype=None, icmpcode=None, **kwargs):
        """
        ...
        args = kwargs.copy()
        protocol = protocol.upper()
        args.update({'securitygroupname': securitygroupname, 'protocol': protocol, 'cidrlist': cidrlist})
        if protocol not in ('TCP', 'UDP') and (startport is not None or endport is not None):
            raise LibcloudError('"startport" and "endport" are only valid with protocol TCP or UDP.')
        ...
        if protocol in ('TCP', 'UDP'):
            if startport is None:
                raise LibcloudError('Protocols TCP and UDP require at least "startport" to be set.')
        ...
        return self._async_request(command='authorizeSecurityGroupIngress', params=args, method='GET')['securitygroup']
    ...
    def ex_revoke_security_group_ingress(self, rule_id):
        ...
        :rtype: ``bool``
        """
        self._async_request(command='revokeSecurityGroupIngress', params={'id': rule_id}, method='GET')
        return True
    ...
    def create_volume_snapshot(self, volume, name=None):
        ...
        """
        snapshot = self._async_request(command='createSnapshot', params={'volumeid': volume.id}, method='GET')
    ...
    def ex_list_nics(self, node):
        ...
        """
        res = self._sync_request(command='listNics', params={'virtualmachineid': node.id}, method='GET')
        items = res.get('nic', [])
        nics = []
        extra_map = RESOURCE_EXTRA_ATTRIBUTES_MAP['nic']
        for item in items:
            extra = self._get_extra_dict(item, extra_map)
            nics.append(CloudStackNic(id=item['id'], network_id=item['networkid'], net_mask=item['netmask'], gateway=item['gateway'], ip_address=item['ipaddress'], is_default=item['isdefault'], mac_address=item['macaddress'], driver=self, extra=extra))
        return nics
    ...
    def ex_delete_vpn_gateway(self, vpn_gateway):
        ...
        :rtype: ``bool``
        """
        res = self._async_request(command='deleteVpnGateway', params={'id': vpn_gateway.id}, method='GET')
        return res['success']
    ...
    def ex_list_vpn_connections(self, account=None, domain_id=None, for_display=None, id=None, is_recursive=None, keyword=None, list_all=None, page=None, page_size=None, project_id=None, vpc_id=None):
        """
        ...
        for item in items:
            extra = self._get_extra_dict(item, extra_map)
            vpn_connections.append(CloudStackVpnConnection(id=item['id'], passive=item['passive'], vpn_customer_gateway_id=item['s2scustomergatewayid'], vpn_gateway_id=item['s2svpngatewayid'], state=item['state'], driver=self, extra=extra))
        return vpn_connections

    def ex_create_vpn_connection(self, vpn_customer_gateway, vpn_gateway, for_display=None, passive=None):
        """
        ...
        """
        args = {'s2scustomergatewayid': vpn_customer_gateway.id, 's2svpngatewayid': vpn_gateway.id}
        if for_display is not None:
            args['fordisplay'] = for_display
        ...
        extra_map = RESOURCE_EXTRA_ATTRIBUTES_MAP['vpnconnection']
        return CloudStackVpnConnection(id=item['id'], passive=item['passive'], vpn_customer_gateway_id=vpn_customer_gateway.id, vpn_gateway_id=vpn_gateway.id, state=item['state'], driver=self, extra=self._get_extra_dict(item, extra_map))
    ...
    def ex_delete_vpn_connection(self, vpn_connection):
        ...
        :rtype: ``bool``
        """
        res = self._async_request(command='deleteVpnConnection', params={'id': vpn_connection.id}, method='GET')
        return res['success']
    ...
    def _to_snapshot(self, data):
        ...
        :rtype: :class:`VolumeSnapshot`
        """
        extra = {'tags': data.get('tags', None), 'name': data.get('name', None), 'volume_id': data.get('volumeid', None)}
    ...
    def _to_node(self, data, public_ips=None):
        ...
        node = CloudStackNode(id=id, name=name, state=state, public_ips=list(set(public_ips)), private_ips=private_ips, driver=self, extra=extra)
        return node
    ...
    def _to_key_pair(self, data):
        key_pair = KeyPair(name=data['name'], fingerprint=data['fingerprint'], public_key=data.get('publickey', None), private_key=data.get('privatekey', None), driver=self)
        return key_pair
