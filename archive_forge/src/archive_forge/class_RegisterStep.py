...
class RegisterStep(BaseStep):
    ...
    def _validate_and_apply_step_config(self):
        ...
        if self.register_model_name is None:
            raise MlflowException("Missing 'model_name' config in register step config.", error_code=INVALID_PARAMETER_VALUE)
    ...
    def _run(self, output_directory):
        ...
        model_validation_path = get_step_output_path(recipe_root_path=self.recipe_root, step_name='evaluate', relative_path='model_validation_status')
        model_validation = Path(model_validation_path).read_text()
        artifact_path = 'train/model'
        tags = {MLFLOW_SOURCE_TYPE: SourceType.to_string(SourceType.RECIPE), MLFLOW_RECIPE_TEMPLATE_NAME: self.step_config['recipe']}
        self.model_uri = f'runs:/{run_id}/{artifact_path}'
        if model_validation == 'VALIDATED' or (model_validation == 'UNKNOWN' and self.allow_non_validated_model):
            if self.registry_uri:
                mlflow.set_registry_uri(self.registry_uri)
            self.model_details = mlflow.register_model(model_uri=self.model_uri, name=self.register_model_name, tags=tags, await_registration_for=DEFAULT_AWAIT_MAX_SLEEP_SECONDS)
            self.version = self.model_details.version
            registered_model_info = RegisteredModelVersionInfo(name=self.register_model_name, version=self.version)
            registered_model_info.to_json(path=str(Path(output_directory) / _REGISTERED_MV_INFO_FILE))
        else:
            raise MlflowException(f'Model registration on {self.model_uri} failed because it is not validated. Bypass by setting allow_non_validated_model to True. ')
    ...
    def get_artifacts(self):
        return [ModelVersionArtifact('registered_model_version', self.recipe_root, self.name, self.tracking_config.tracking_uri)]
