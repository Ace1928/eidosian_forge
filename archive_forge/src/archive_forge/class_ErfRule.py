...
class ErfRule(AtomicRule):
    ...
    def eval(self) -> Expr:
        ...
        if a.is_extended_real:
            return Piecewise((sqrt(S.Pi / -a) / 2 * exp(c - b ** 2 / (4 * a)) * erf((-2 * a * x - b) / (2 * sqrt(-a))), a < 0), (sqrt(S.Pi / a) / 2 * exp(c - b ** 2 / (4 * a)) * erfi((2 * a * x + b) / (2 * sqrt(a))), True))
        return sqrt(S.Pi / a) / 2 * exp(c - b ** 2 / (4 * a)) * erfi((2 * a * x + b) / (2 * sqrt(a)))