...
class ChatJavelinAIGateway(BaseChatModel):
    ...
    def __init__(self, **kwargs: Any):
        try:
            ...
            raise ImportError('Could not import javelin_sdk python package. Please install it with `pip install javelin_sdk`.')
        super().__init__(**kwargs)
        if self.gateway_uri:
            try:
                self.client = JavelinClient(base_url=self.gateway_uri, api_key=cast(SecretStr, self.javelin_api_key).get_secret_value())
            except UnauthorizedError as e:
                raise ValueError('Javelin: Incorrect API Key.') from e

    @property
    def _default_params(self) -> Dict[str, Any]:
        params: Dict[str, Any] = {'gateway_uri': self.gateway_uri, 'javelin_api_key': cast(SecretStr, self.javelin_api_key).get_secret_value(), 'route': self.route, **(self.params.dict() if self.params else {})}
        return params

    def _generate(self, messages: List[BaseMessage], stop: Optional[List[str]]=None, run_manager: Optional[CallbackManagerForLLMRun]=None, **kwargs: Any) -> ChatResult:
        message_dicts = [ChatJavelinAIGateway._convert_message_to_dict(message) for message in messages]
        data: Dict[str, Any] = {'messages': message_dicts, **(self.params.dict() if self.params else {})}
    ...
    def _convert_message_to_dict(message: BaseMessage) -> dict:
        ...
        if message.additional_kwargs:
            logger.warning('Additional message arguments are unsupported by Javelin AI Gateway  and will be ignored: %s', message.additional_kwargs)
        return message_dict
    ...
    def _create_chat_result(response: Mapping[str, Any]) -> ChatResult:
        ...
        for candidate in response['llm_response']['choices']:
            ...
            gen = ChatGeneration(message=message, generation_info=dict(message_metadata))
