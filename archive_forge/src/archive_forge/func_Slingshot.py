...
import pandas as pd
import warnings
def Slingshot(data, cluster_labels, start_cluster=None, end_cluster=None, distance=None, omega=None, shrink=True, extend='y', reweight=True, reassign=True, thresh=0.001, max_iter=15, stretch=2, smoother='smooth.spline', shrink_method='cosine', allow_breaks=True, seed=None, verbose=1, **kwargs):
    ...
    if data.shape[1] > 3:
        warnings.warn('Expected data to be low-dimensional. Got data.shape[1] = {}'.format(data.shape[1]), UserWarning)
    cluster_labels = utils.toarray(cluster_labels).flatten()
    if not cluster_labels.shape[0] == data.shape[0]:
        raise ValueError('Expected len(cluster_labels) ({}) to equal data.shape[0] ({})'.format(cluster_labels.shape[0], data.shape[0]))
    ...
    if omega is not None:
        kwargs['omega'] = omega
    slingshot = _Slingshot(data=data, cluster_labels=cluster_labels, shrink=shrink, extend=extend, reweight=reweight, reassign=reassign, thresh=thresh, max_iter=max_iter, stretch=stretch, smoother=smoother, shrink_method=shrink_method, allow_breaks=allow_breaks, **kwargs, seed=seed, rpy_verbose=verbose)
    ...
    branch_means = [np.nanmean(slingshot['pseudotime'][branch == id]) if not np.all(np.isnan(slingshot['pseudotime'][branch == id])) else np.nan for id in branch_ids]
