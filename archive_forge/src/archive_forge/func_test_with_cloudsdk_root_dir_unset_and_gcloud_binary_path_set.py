...
def test_with_cloudsdk_root_dir_unset_and_gcloud_binary_path_set(self):
    ...
    with _mock_boto_config({'GSUtil': {'use_gcloud_storage': 'always', 'hidden_shim_mode': 'no_fallback'}}):
        with util.SetEnvironmentForTest({'CLOUDSDK_CORE_PASS_CREDENTIALS_TO_GSUTIL': 'True', 'CLOUDSDK_ROOT_DIR': None, 'GCLOUD_BINARY_PATH': gcloud_path}):
            self.assertTrue(self._fake_command.translate_to_gcloud_storage_if_requested())
            self.assertEqual(self._fake_command._translated_gcloud_storage_command, [gcloud_path, 'objects', 'fake', '--zip', 'opt1', '-x', 'arg1', 'arg2'])