                ...
                if l2transport or l2protocol:
                    for each in l2protocol:
                        ...
                        if isinstance(each, dict) and each.get('cpsv'):
                            cmd = 'l2transport l2protocol {0} {1}'.format('cpsv', each.get('cpsv'))
                            add_command_to_config_list(interface, cmd, commands)
                            break
                if encapsulation:
                    ...
                    if encapsulation.get('dot1q'):
                        if encapsulation.get('second_dot1q'):
                            cmd = 'encapsulation dot1q {0} second-dot1q {1}'.format(encapsulation.get('dot1q'), encapsulation.get('second_dot1q'))
                        else:
                            cmd = 'encapsulation dot1q {0}'.format(encapsulation.get('dot1q'))
                        add_command_to_config_list(interface, cmd, commands)
            if l2transport or l2protocol:
                ...
                module.fail_json(msg='L2transport L2protocol or Propagate can only be configured when L2transport set to True!')
        return commands
