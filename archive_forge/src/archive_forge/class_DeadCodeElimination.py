...
class DeadCodeElimination(Transformation):
    ...
    def __init__(self):
        super(DeadCodeElimination, self).__init__(PureExpressions, DefUseChains, Ancestors)
        self.blacklist = set()
    ...
    def visit_Assign(self, node):
        targets = [target for target in node.targets if self.used_target(target)]
    ...
    def visit_Expr(self, node):
        if node.value in self.pure_expressions and (not isinstance(node.value, ast.Yield)):
            self.update = True
            return ast.Pass()
        ...
        try:
            if ast.literal_eval(node.test):
                if not metadata.get(node, OMPDirective):
                    self.update = True
                    return node.body
            elif not metadata.get(node, OMPDirective):
                self.update = True
                return node.orelse
        except ValueError:
            pass
