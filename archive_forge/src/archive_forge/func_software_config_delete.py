...
def software_config_delete(context, config_id):
    ...
    result = context.session.query(models.SoftwareDeployment).filter_by(config_id=config_id).first()
    if result:
        msg = _('Software config with id %s can not be deleted as it is referenced.') % config_id
