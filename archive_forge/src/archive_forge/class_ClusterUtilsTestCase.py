...
class ClusterUtilsTestCase(test_base.OsWinBaseTestCase):
    ...
    _autospec_classes = [clusterutils._clusapi_utils.ClusApiUtils, clusterutils._clusapi_utils.ClusterContextManager]
    ...
    def test_lookup_res_no_res(self):
        ...
        self.assertIsNone(self._clusterutils._lookup_res(resource_source, self._FAKE_RES_NAME))
    ...
    def test_lookup_res_duplicate_res(self):
        ...
        resource_source = mock.MagicMock()
        resource_source.return_value = res_list
        self.assertRaises(exceptions.HyperVClusterException, self._clusterutils._lookup_res, resource_source, self._FAKE_RES_NAME)
        resource_source.assert_called_once_with(Name=self._FAKE_RES_NAME)
    ...
    def test_list_instances(self, mock_get_vm_groups):
        mock_get_vm_groups.return_value = [dict(name='vm1'), dict(name='vm2')]
    ...
    def test_delete(self, mock_lookup_vm_group):
        ...
        vm.DestroyGroup.assert_called_once_with(self._clusterutils._DESTROY_GROUP)
    ...
    def test_monitor_vm_failover_no_vm(self, mock_patcher, mock_tpool):
        ...
        fake_wmi_object = mock.MagicMock(OwnerNode=self._FAKE_HOST, Name='Virtual Machine', previous=fake_prev)
        mock_tpool.execute.return_value = fake_wmi_object
        fake_callback = mock.MagicMock()
        self._clusterutils._monitor_vm_failover(mock_watcher, fake_callback, mock.sentinel.event_timeout_ms)
        mock_tpool.execute.assert_called_once_with(mock_watcher, mock.sentinel.event_timeout_ms)
