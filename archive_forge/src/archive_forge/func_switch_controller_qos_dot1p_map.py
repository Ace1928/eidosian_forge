...
def switch_controller_qos_dot1p_map(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('switch-controller.qos', 'dot1p-map', filtered_data, vdom=vdom)
        current_data = fos.get('switch-controller.qos', 'dot1p-map', vdom=vdom, mkey=mkey)
    ...
    if state == 'present' or state is True:
        return fos.set('switch-controller.qos', 'dot1p-map', data=filtered_data, vdom=vdom)
    elif state == 'absent':
        return fos.delete('switch-controller.qos', 'dot1p-map', mkey=filtered_data['name'], vdom=vdom)
    else:
