...
class BlazarLeaseTestCase(common.HeatTestCase):
    ...
    def setUp(self):
        ...
        self.lease = {'id': uuids.lease_id, 'name': 'test-lease', 'start_date': '2020-01-01 09:00', 'end_date': '2020-01-10 17:30', 'created_at': '2020-01-01 08:00', 'updated_at': '2020-01-01 12:00', 'degraded': False, 'user_id': uuids.user_id, 'project_id': uuids.project_id, 'trust_id': uuids.trust_id, 'reservations': [{'resource_type': 'physical:host', 'min': 1, 'max': 1, 'hypervisor_properties': '[">=", "$vcpus", "2"]', 'resource_properties': '', 'before_end': 'default'}], 'events': []}
    ...
    def test_lease_delete_not_found(self):
        lease_resource = self._create_resource('lease', self.rsrc_defn, self.stack)
        scheduler.TaskRunner(lease_resource.create)()
        self.client.lease.delete.side_effect = client_exception.BlazarClientException(code=404)
        self.client.lease.get.side_effect = client_exception.BlazarClientException(code=404)
        scheduler.TaskRunner(lease_resource.delete)()
        self.assertEqual((lease_resource.DELETE, lease_resource.COMPLETE), lease_resource.state)
    ...
    def test_resolve_attributes(self):
        ...
        self.assertEqual(self.lease['start_date'], lease_resource._resolve_attribute(lease.Lease.START_DATE))

    def test_resolve_attributes_not_found(self):
        lease_resource = self._create_resource('lease', self.rsrc_defn, self.stack)
        scheduler.TaskRunner(lease_resource.create)()
        self.client.lease.get.return_value = self.lease
        self.assertRaises(exception.InvalidTemplateAttribute, lease_resource._resolve_attribute, 'invalid_attribute')