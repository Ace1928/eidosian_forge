...
class Approve(base.CreateCommand):
    ...
    def Run(self, args):
        ...
        failed_activity_msg = 'Cannot approve rollout {}.'.format(rollout_ref.RelativeName())
        ...
        release_ref = resources.REGISTRY.ParseRelativeName(rollout_ref.Parent().RelativeName(), collection='clouddeploy.projects.locations.deliveryPipelines.releases')
        try:
            release_obj = release.ReleaseClient().Get(release_ref.RelativeName())
        except apitools_exceptions.HttpError as error:
            raise exceptions.HttpException(error)
        prompt = 'Approving rollout {} from {} to target {}.\n\n'.format(rollout_ref.Name(), release_ref.Name(), rollout_obj.targetId)
        ...
        return rollout.RolloutClient().Approve(rollout_ref.RelativeName(), True, policies)