...
class NetAppOntapSnapshot:
    ...
    def get_snapshot(self, snapshot_name=None, volume_id=None):
        ...
        if self.use_rest:
            ...
            params = {'svm.name': self.parameters['vserver'], 'fields': 'uuid,comment,expiry_time,volume,name'}
            ...
            if error:
                self.module.fail_json(msg='Error fetching snapshot %s: %s' % (params['name'], to_native(error)), exception=traceback.format_exc())
            if snapshot:
                return {'uuid': snapshot['uuid'], 'snapshot': snapshot['name'], 'snapmirror_label': snapshot.get('snapmirror_label'), 'expiry_time': snapshot.get('expiry_time'), 'comment': snapshot.get('comment')}
            return None
        else:
            ...
            try:
                ...
                self.module.fail_json(msg='Error fetching snapshot %s: %s' % (snapshot_name, to_native(error)), exception=traceback.format_exc())
            return_value = None
            if result.get_child_by_name('num-records') and int(result.get_child_content('num-records')) == 1:
                attributes_list = result.get_child_by_name('attributes-list')
                snap_info = attributes_list.get_child_by_name('snapshot-info')
                return_value = {'comment': snap_info.get_child_content('comment'), 'snapmirror_label': None}
    ...
    def create_snapshot(self, volume_id=None):
        ...
        if self.use_rest:
            api = 'storage/volumes/%s/snapshots' % volume_id
            body = {'name': self.parameters['snapshot'], 'svm': {'name': self.parameters['vserver']}}
    ...
    def rename_snapshot(self):
        ...
        try:
            ...
            self.module.fail_json(msg='Error renaming snapshot %s to %s: %s' % (self.parameters['from_name'], self.parameters['snapshot'], to_native(error)), exception=traceback.format_exc())
    ...
    def apply(self):
        ...
        if cd_action == 'create' and self.parameters.get('from_name'):
            ...
            if current is None:
                self.module.fail_json(msg='Error renaming snapshot: %s - no snapshot with from_name: %s.' % (self.parameters['snapshot'], self.parameters['from_name']))
            rename = True
            cd_action = None
