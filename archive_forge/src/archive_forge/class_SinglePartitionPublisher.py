...
class SinglePartitionPublisher(Publisher, ConnectionReinitializer[PublishRequest, PublishResponse], RequestSizer[_MessageWithSequence]):
    ...
    def __init__(self, initial: InitialPublishRequest, batching_settings: BatchSettings, factory: ConnectionFactory[PublishRequest, PublishResponse], initial_sequence: PublishSequenceNumber):
    ...
    def _handle_response(self, response: PublishResponse):
        ...
        if not self._outstanding_writes:
            self._connection.fail(FailedPrecondition('Received an publish response on the stream with no outstanding publishes.'))
        ...
        for msg_idx, item in enumerate(batch):
            if range_idx < len(ranges) and ranges[range_idx].end_index <= msg_idx:
                range_idx += 1
            offset = -1
            if range_idx < len(ranges) and msg_idx >= ranges[range_idx].start_index and (msg_idx < ranges[range_idx].end_index):
    ...
    async def _flush(self):
        ...
        aggregate.message_publish_request.messages = [item.request.message for item in batch]
        if self._initial.client_id:
            aggregate.message_publish_request.first_sequence_number = batch[0].request.sequence_number.value
        try:
    ...
    def get_size(self, request: _MessageWithSequence) -> BatchSize:
        return BatchSize(element_count=1, byte_count=PubSubMessage.pb(request.message).ByteSize())
