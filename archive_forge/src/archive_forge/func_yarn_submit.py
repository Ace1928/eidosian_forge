...
def yarn_submit(args, nworker, nserver, pass_env):
    ...
    if not os.path.exists(YARN_JAR_PATH):
        ...
        cmd = 'cd %s;./build.%s' % (os.path.join(os.path.dirname(__file__), os.pardir, 'yarn'), 'bat' if is_windows else 'sh')
        print(cmd)
        subprocess.check_call(cmd, shell=True, env=os.environ)
        assert os.path.exists(YARN_JAR_PATH), 'failed to build dmlc-yarn.jar, try it manually'
    out, _ = subprocess.Popen('%s version' % hadoop_binary, shell=True, stdout=subprocess.PIPE).communicate()
    out = py_str(out).split('\n')[0].split()
    assert out[0] == 'Hadoop', 'cannot parse hadoop version string'
    hadoop_version = int(out[1].split('.')[0])
    classpath, _ = subprocess.Popen('%s classpath' % hadoop_binary, shell=True, stdout=subprocess.PIPE).communicate()
    ...
    cmd = '%s -cp %s%s%s org.apache.hadoop.yarn.dmlc.Client ' % (JAVA, classpath, ';' if is_windows else ':', YARN_JAR_PATH)
    env = os.environ.copy()
