    ...
    * :math:`(L, 2n)` else.
    ...
    def __init__(self, features, weights, wires, local_field='Y', id=None):
        if local_field == 'Z':
            ...
            local_field = qml.RY
        elif not (isinstance(local_field, type) and issubclass(local_field, (qml.RX, qml.RY, qml.RZ))):
            raise ValueError(f'did not recognize local field {local_field}')
        shape = qml.math.shape(features)
        if len(shape) not in {1, 2}:
            raise ValueError(f'Features must be a one-dimensional tensor or two-dimensional with broadcasting; got shape {shape}.')
        n_features = shape[-1]
        if n_features > len(wires):
            raise ValueError(f'Features must be of length {len(wires)} or less; got length {n_features}.')
        shape = qml.math.shape(weights)
        if len(shape) not in {2, 3}:
            raise ValueError(f'Weights must be a two-dimensional tensor or three-dimensional with broadcasting; got shape {shape}.')
        repeat = shape[-2]
        if len(wires) == 1:
            exp_shape = (repeat, 1)
        elif len(wires) == 2:
        ...
        **Example**
        ...
        for l in range(repeat):
            ...
            if len(wires) == 1:
                ...
                multirz_gates = (qml.MultiRZ(weights[l][i], wires.subset([i, i + 1], periodic_boundary=True)) for i in range(len(wires)))
                op_list.extend(multirz_gates)
                local_field_gates = (local_field(weights[l][len(wires) + i], wires=wires[i]) for i in range(len(wires)))
