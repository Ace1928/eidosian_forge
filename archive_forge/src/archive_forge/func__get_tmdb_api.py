...
def _get_tmdb_api(llm: BaseLanguageModel, **kwargs: Any) -> BaseTool:
    ...
    chain = APIChain.from_llm_and_api_docs(llm, tmdb_docs.TMDB_DOCS, headers={'Authorization': f'Bearer {tmdb_bearer_token}'}, limit_to_domains=['https://api.themoviedb.org/'])
    return Tool(name='TMDB-API', description='Useful for when you want to get information from The Movie Database. The input should be a question in natural language that this API can answer.', func=chain.run)