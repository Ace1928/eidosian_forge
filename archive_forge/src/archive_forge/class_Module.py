...
class Module(autotrackable.AutoTrackable):
  ...
  <tf.Variable 'mlp/w:0' shape=(5, 5) dtype=float32, numpy=...,
     dtype=float32)>)
  """
    _TF_MODULE_IGNORED_PROPERTIES = frozenset(('_self_unconditional_checkpoint_dependencies', '_self_unconditional_dependency_names'))

    def __init__(self, name=None):
        if name is None:
            name = camel_to_snake(type(self).__name__)
        elif not valid_identifier(name):
            raise ValueError('%r is not a valid module name. Module names must be valid Python identifiers (e.g. a valid class name).' % name)
        self._name = name
        if tf2.enabled():
    ...
    Returns:
      ...
      first).
    """
        return tuple(self._flatten(predicate=_is_trainable_variable, expand_composites=True))
    ...
    def _flatten(self, recursive=True, predicate=None, attribute_traversal_key=None, with_path=False, expand_composites=False):
    ...
    """
        ...
        return _flatten_module(self, recursive=recursive, predicate=predicate, attributes_to_ignore=self._TF_MODULE_IGNORED_PROPERTIES, attribute_traversal_key=attribute_traversal_key, with_path=with_path, expand_composites=expand_composites)
