...
class MultiplexedSubscriberClient(SubscriberClientInterface):
    ...
    def __init__(self, executor: ThreadPoolExecutor, underlying_factory: AsyncSubscriberFactory):
    ...
    def subscribe(self, subscription: Union[SubscriptionPath, str], callback: MessageCallback, per_partition_flow_control_settings: FlowControlSettings, fixed_partitions: Optional[Set[Partition]]=None) -> StreamingPullFuture:
        ...
        underlying = self._underlying_factory(subscription, fixed_partitions, per_partition_flow_control_settings)
