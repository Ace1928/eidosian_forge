...
def run_scenario(
    ...
    Args:
        ...
        block_size (int): The size of each block in pixels.
    """
    start_node = Node(start.position.x, start.position.y)
    end_node = Node(goal.position.x, goal.position.y)
    search_algorithm = SearchAlgorithm(None, None)
    paths = search_algorithm.astar(maze=maze, start=start_node, end=end_node)
    ...
    if paths:
        for path in paths:
            ...
            pg.display.flip()
            time.sleep(1)  # Pause for a moment to view each path
