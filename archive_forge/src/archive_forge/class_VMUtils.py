...
class VMUtils(baseutils.BaseUtilsVirt):
    ...
    _SYNTHETIC_ETHERNET_PORT_SETTING_DATA_CLASS = 'Msvm_SyntheticEthernetPortSettingData'
    ...
    _SYNTH_DISP_CTRL_RES_SUB_TYPE = 'Microsoft:Hyper-V:Synthetic Display Controller'
    _REMOTEFX_DISP_CTRL_RES_SUB_TYPE = 'Microsoft:Hyper-V:Synthetic 3D Display Controller'
    _SYNTH_DISP_ALLOCATION_SETTING_DATA_CLASS = 'Msvm_SyntheticDisplayControllerSettingData'
    ...
    _remote_fx_res_map = {constants.REMOTEFX_MAX_RES_1024x768: 0, constants.REMOTEFX_MAX_RES_1280x1024: 1, constants.REMOTEFX_MAX_RES_1600x1200: 2, constants.REMOTEFX_MAX_RES_1920x1200: 3, constants.REMOTEFX_MAX_RES_2560x1600: 4}
    _remotefx_max_monitors_map = {constants.REMOTEFX_MAX_RES_1024x768: 4, constants.REMOTEFX_MAX_RES_1280x1024: 4, constants.REMOTEFX_MAX_RES_1600x1200: 3, constants.REMOTEFX_MAX_RES_1920x1200: 2, constants.REMOTEFX_MAX_RES_2560x1600: 1}
    _DISP_CTRL_ADDRESS_DX_11 = '02C1,00000000,01'
    _DISP_CTRL_ADDRESS = '5353,00000000,00'
    _vm_power_states_map = {constants.HYPERV_VM_STATE_ENABLED: 2, constants.HYPERV_VM_STATE_DISABLED: 3, constants.HYPERV_VM_STATE_REBOOT: 11, constants.HYPERV_VM_STATE_PAUSED: 9, constants.HYPERV_VM_STATE_SUSPENDED: 6}
    _disk_ctrl_type_mapping = {_SCSI_CTRL_RES_SUB_TYPE: constants.CTRL_TYPE_SCSI, _IDE_CTRL_RES_SUB_TYPE: constants.CTRL_TYPE_IDE}
    _DEFAULT_EVENT_CHECK_TIMEFRAME = 60

    def __init__(self, host='.'):
        super(VMUtils, self).__init__(host)
        self._jobutils = jobutils.JobUtils(host)
        self._pathutils = pathutils.PathUtils()
        self._enabled_states_map = {v: k for k, v in self._vm_power_states_map.items()}
    ...
    def list_instances(self):
        ...
        return [v.ElementName for v in self._conn.Msvm_VirtualSystemSettingData(['ElementName'], VirtualSystemType=self._VIRTUAL_SYSTEM_TYPE_REALIZED)]
    ...
    def _lookup_vm(self, vm_name, as_vssd=True, for_update=False):
        if as_vssd:
            ...
            vms = [v for v in vms if v.VirtualSystemType in [self._VIRTUAL_SYSTEM_TYPE_PLANNED, self._VIRTUAL_SYSTEM_TYPE_REALIZED]]
        else:
        ...
        if n == 0:
            ...
            raise exceptions.HyperVException(_('Duplicate VM name found: %s') % vm_name)
        else:
            return vms[0]
    ...
    def _get_vm_memory(self, vmsetting):
        mem_settings = _wqlutils.get_element_associated_class(self._compat_conn, self._MEMORY_SETTING_DATA_CLASS, element_instance_id=vmsetting.InstanceID)[0]
        return mem_settings
    ...
    def set_nested_virtualization(self, vm_name, state):
        ...
        """
        raise NotImplementedError(_('Nested virtualization is supported on Windows / Hyper-V Server 2016 or newer.'))
    ...
    def _get_vm_scsi_controller(self, vmsettings):
        res = self._get_vm_disk_controllers(vmsettings, self._SCSI_CTRL_RES_SUB_TYPE)
    ...
    def get_attached_disks(self, scsi_controller_path):
        volumes = self._conn.query(self._get_attached_disks_query_string(scsi_controller_path))
        return volumes
    ...
    def _get_new_setting_data(self, class_name):
        obj = self._compat_conn.query("SELECT * FROM %s WHERE InstanceID LIKE '%%\\Default'" % class_name)[0]
        return obj
    ...
    def _get_vm_nics(self, vm_name):
        vmsettings = self._lookup_vm_check(vm_name)
        nics = _wqlutils.get_element_associated_class(self._compat_conn, self._SYNTHETIC_ETHERNET_PORT_SETTING_DATA_CLASS, element_instance_id=vmsettings.InstanceId)
        return nics
    ...
    def soft_shutdown_vm(self, vm_name):
        try:
            vm = self._lookup_vm_check(vm_name, as_vssd=False)
            shutdown_component = self._conn.Msvm_ShutdownComponent(SystemName=vm.Name)
            if not shutdown_component:
                return
            reason = 'Soft shutdown requested by OpenStack Nova.'
            ret_val, = shutdown_component[0].InitiateShutdown(Force=False, Reason=reason)
    ...
    def is_disk_attached(self, disk_path, is_physical=True):
        disk_resource = self._get_mounted_disk_resource_from_path(disk_path, is_physical)
    ...
    def get_mounted_disk_by_drive_number(self, device_number):
        mounted_disks = self._conn.query('SELECT * FROM Msvm_DiskDrive WHERE DriveNumber=' + str(device_number))
    ...
    def get_vm_serial_port_connections(self, vm_name):
        ...
        conns = [serial_port.Connection[0] for serial_port in serial_ports if serial_port.Connection and serial_port.Connection[0]]
        return conns
    ...
    def _get_event_wql_query(self, cls, field, timeframe, filtered_states=None):
        """Return a WQL query used for polling WMI events.
            ...
            :param filtered_states: only catch events triggered when a WMI
                                    ...
                                    states.
        """
        query = "SELECT %(field)s, TargetInstance FROM __InstanceModificationEvent WITHIN %(timeframe)s WHERE TargetInstance ISA '%(class)s' AND TargetInstance.%(field)s != PreviousInstance.%(field)s" % {'class': cls, 'field': field, 'timeframe': timeframe}
        if filtered_states:
            checks = ["TargetInstance.%s = '%s'" % (field, state) for state in filtered_states]
    ...
    def get_vm_power_state(self, vm_enabled_state):
        return self._enabled_states_map.get(vm_enabled_state, constants.HYPERV_VM_STATE_OTHER)
    ...
    def _set_secure_boot(self, vs_data, msft_ca_required):
        ...
        if msft_ca_required:
            raise exceptions.HyperVException(_('UEFI SecureBoot is supported only on Windows instances for this Hyper-V version.'))
    ...
    def set_disk_qos_specs(self, disk_path, max_iops=None, min_iops=None):
        ...
        if min_iops or max_iops:
            raise exceptions.UnsupportedOperation(reason=_('Virtual disk QoS is not supported on this hypervisor version.'))
    ...
    def _set_boot_order_gen2(self, vm_name, device_boot_order):
        new_boot_order = [self._drive_to_boot_source(device) for device in device_boot_order if device]
        vssd = self._lookup_vm_check(vm_name)
    ...
    def _validate_remotefx_params(self, monitor_count, max_resolution, vram_bytes=None):
        ...
        if max_res_value is None:
            raise exceptions.HyperVRemoteFXException(_('Unsupported RemoteFX resolution: %s') % max_resolution)
        if monitor_count > self._remotefx_max_monitors_map[max_resolution]:
            raise exceptions.HyperVRemoteFXException(_('Unsuported RemoteFX monitor count: %(count)s for this resolution %(res)s. Hyper-V supports a maximum of %(max_monitors)s monitors for this resolution.') % {'count': monitor_count, 'res': max_resolution, 'max_monitors': self._remotefx_max_monitors_map[max_resolution]})
    ...
    def enable_remotefx_video_adapter(self, vm_name, monitor_count, max_resolution, vram_bytes=None):
        self._validate_remotefx_params(monitor_count, max_resolution, vram_bytes=vram_bytes)
        ...
        synth_disp_ctrl_res_list = [r for r in rasds if r.ResourceSubType == self._SYNTH_DISP_CTRL_RES_SUB_TYPE]
        ...
        remotefx_disp_ctrl_res = remotefx_disp_ctrl_res[0] if remotefx_disp_ctrl_res else None
        max_res_value = self._remote_fx_res_map.get(max_resolution)
        self._set_remotefx_display_controller(vm, remotefx_disp_ctrl_res, monitor_count, max_res_value, vram_bytes)
    ...
    def add_pci_device(self, vm_name, vendor_id, product_id):
        ...
        :raises NotImplemented: PCI passthrough is supported on
            Windows / Hyper-V Server 2016 or newer.
        """
        raise NotImplementedError(_('PCI passthrough is supported on Windows / Hyper-V Server 2016 or newer.'))
    ...
    def populate_fsk(self, fsk_filepath, fsk_pairs):
        ...
        """
        raise NotImplementedError(_('This method is supported on Windows / Hyper-V Server 2016 or newer'))
