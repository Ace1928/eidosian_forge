...
class ComplexPhaseOracle(infra.GateWithRegisters):
    ...
    def decompose_from_registers(self, *, context: cirq.DecompositionContext, **quregs: NDArray[cirq.Qid]) -> cirq.OP_TREE:
        ...
        target_reg = {reg.name: qm.qalloc(reg.total_bits()) for reg in self.encoder.target_registers}
        ...
        arctan_op = arctan.ArcTan(len(target_qubits), self.arctan_bitsize).on(*target_qubits, *arctan_sign, *arctan_target)
        yield encoder_op
        yield arctan_op
