...
class Jaeger(base.Driver):
    ...
    def __init__(self, connection_str, project=None, service=None, host=None, conf=cfg.CONF, **kwargs):
        ...
        super(Jaeger, self).__init__(connection_str, project=project, service=service, host=host, conf=conf, **kwargs)
        try:
            import jaeger_client
            self.jaeger_client = jaeger_client
        except ImportError:
            raise exc.CommandError('To use OSProfiler with Uber Jaeger tracer, please install `jaeger-client` library. To install with pip:\n `pip install jaeger-client`.')
        parsed_url = parser.urlparse(connection_str)
        cfg = {'local_agent': {'reporting_host': parsed_url.hostname, 'reporting_port': parsed_url.port}, 'tags': conf.profiler_jaeger.process_tags}
    ...
    def notify(self, payload):
        if payload['name'].endswith('start'):
            ...
            child_of = self.jaeger_client.SpanContext(trace_id=utils.shorten_id(payload['base_id']), span_id=utils.shorten_id(payload['parent_id']), parent_id=None, flags=self.jaeger_client.span.SAMPLED_FLAG)
            span = self.tracer.start_span(operation_name=payload['name'].rstrip('-start'), child_of=child_of, tags=self.create_span_tags(payload), start_time=start_time)
