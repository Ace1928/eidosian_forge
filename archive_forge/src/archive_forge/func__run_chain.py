...
def _run_chain(chain: Union[Chain, Runnable], inputs: Dict[str, Any], callbacks: Callbacks, *, tags: Optional[List[str]]=None, input_mapper: Optional[Callable[[Dict], Any]]=None, metadata: Optional[Dict[str, Any]]=None) -> Union[Dict, str]:
    ...
    if isinstance(chain, Chain) and isinstance(inputs_, dict) and (len(inputs_) == 1) and chain.input_keys:
        ...
        output = chain.invoke(val, config=RunnableConfig(callbacks=callbacks, tags=tags or [], metadata=metadata or {}))
    else:
