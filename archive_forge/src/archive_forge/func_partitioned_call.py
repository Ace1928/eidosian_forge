...
def partitioned_call(args, Tout, f, config: str='', config_proto: str='', executor_type: str='', name=None):
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'PartitionedCall', name, args, 'Tout', Tout, 'f', f, 'config', config, 'config_proto', config_proto, 'executor_type', executor_type)
            return _result
    ...
    if not isinstance(Tout, (list, tuple)):
        raise TypeError("Expected list for 'Tout' argument to 'partitioned_call' Op, not %r." % Tout)
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('PartitionedCall', args=args, Tout=Tout, f=f, config=config, config_proto=config_proto, executor_type=executor_type, name=name)
    _result = _outputs[:]
