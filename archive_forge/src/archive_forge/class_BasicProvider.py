...
class BasicProvider(ProviderV1):
    ...
    def backends(self, name: str | None=None, filters: Callable | None=None, **kwargs) -> list[Backend]:
        backends = self._backends.values()
        if name:
            available = [backend.name() if backend.version == 1 else backend.name for backend in backends]
            if name not in available:
                raise QiskitBackendNotFoundError(f"The '{name}' backend is not installed in your system.")
