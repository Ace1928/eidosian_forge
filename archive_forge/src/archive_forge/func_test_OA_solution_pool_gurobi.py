...
@unittest.skipIf(not (ipopt_available and gurobi_persistent_available), 'Required subsolvers are not available')
def test_OA_solution_pool_gurobi(self):
    """Test the outer approximation decomposition algorithm."""
    with SolverFactory('mindtpy') as opt:
        for model in model_list:
            model = model.clone()
            results = opt.solve(model, strategy='OA', init_strategy='rNLP', solution_pool=True, mip_solver=required_solvers[2], nlp_solver=required_solvers[0])
