...
def extract_volume_patches_eager_fallback(input: _atypes.TensorFuzzingAnnotation[TV_ExtractVolumePatches_T], ksizes, strides, padding: str, name, ctx) -> _atypes.TensorFuzzingAnnotation[TV_ExtractVolumePatches_T]:
    ...
    _inputs_flat = [input]
    _attrs = ('ksizes', ksizes, 'strides', strides, 'T', _attr_T, 'padding', padding)
    _result = _execute.execute(b'ExtractVolumePatches', 1, inputs=_inputs_flat, attrs=_attrs, ctx=ctx, name=name)
