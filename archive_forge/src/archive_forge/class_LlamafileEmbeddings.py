...
class LlamafileEmbeddings(BaseModel, Embeddings):
    ...
    def _embed(self, text: str) -> List[float]:
        try:
            response = requests.post(url=f'{self.base_url}/embedding', headers={'Content-Type': 'application/json'}, json={'content': text}, timeout=self.request_timeout)
        except requests.exceptions.ConnectionError:
            raise requests.exceptions.ConnectionError(f'Could not connect to Llamafile server. Please make sure that a server is running at {self.base_url}.')
        ...
        if sum(embedding) == 0.0:
            raise ValueError('Embedding sums to 0, did you start the llamafile server with the `--embedding` option enabled?')
        return embedding
