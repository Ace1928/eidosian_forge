...
class BasicEntanglerLayers(Operation):
    ...
    def __init__(self, weights, wires=None, rotation=None, id=None):
        ...
        if not (len(shape) == 3 or len(shape) == 2):
            raise ValueError(f'Weights tensor must be 2-dimensional or 3-dimensional if batching; got shape {shape}')
        if shape[-1] != len(wires):
            raise ValueError(f'Weights tensor must have last dimension of length {len(wires)}; got {shape[-1]}')
