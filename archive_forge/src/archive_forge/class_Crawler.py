    ...
    **Security Note**: This is an implementation of a crawler that uses a browser via
    ...
    def __init__(self) -> None:
        try:
            ...
            raise ImportError('Could not import playwright python package. Please install it with `pip install playwright`.')
    ...
    def crawl(self) -> List[str]:
        ...
        page_state_as_text.append({'x': 0, 'y': 0, 'text': '[scrollbar {:0.2f}-{:0.2f}%]'.format(round(percentage_progress_start, 2), round(percentage_progress_end))})
        tree = self.client.send('DOMSnapshot.captureSnapshot', {'computedStyles': [], 'includeDOMRects': True, 'includePaintOrder': True})
        ...
        def add_to_hash_tree(hash_tree: Dict[str, Tuple[bool, Optional[int]]], tag: str, node_id: int, node_name: Optional[str], parent_id: int) -> Tuple[bool, Optional[int]]:
        ...
        for index, node_name_index in enumerate(node_names):
            ...
            partially_is_in_viewport = elem_left_bound < win_right_bound and elem_right_bound >= win_left_bound and (elem_top_bound < win_lower_bound) and (elem_lower_bound >= win_upper_bound)
            ...
            if ancestor_exception and (node_name != 'a' and node_name != 'button'):
                continue
            elements_in_view_port.append({'node_index': str(index), 'backend_node_id': backend_node_id[index], 'node_name': node_name, 'node_value': element_node_value, 'node_meta': meta_data, 'is_clickable': index in is_clickable, 'origin_x': int(x), 'origin_y': int(y), 'center_x': int(x + width / 2), 'center_y': int(y + height / 2)})
        ...
        for element in elements_in_view_port:
            ...
            if ((converted_node_name != 'button' or meta == '') and converted_node_name != 'link' and (converted_node_name != 'input') and (converted_node_name != 'img') and (converted_node_name != 'textarea')) and inner_text.strip() == '':
                continue
            page_element_buffer[id_counter] = element
            if inner_text != '':
                elements_of_interest.append(f'<{converted_node_name} id={id_counter}{meta}>{inner_text}</{converted_node_name}>')
            else:
