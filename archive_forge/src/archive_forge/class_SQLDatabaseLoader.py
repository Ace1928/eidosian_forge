...
class SQLDatabaseLoader(BaseLoader):
    ...
    def __init__(self, query: Union[str, sa.Select], db: SQLDatabase, *, parameters: Optional[Dict[str, Any]]=None, page_content_mapper: Optional[Callable[..., str]]=None, metadata_mapper: Optional[Callable[..., Dict[str, Any]]]=None, source_columns: Optional[Sequence[str]]=None, include_rownum_into_metadata: bool=False, include_query_into_metadata: bool=False):
        """
        Args:
    ...
    def lazy_load(self) -> Iterator[Document]:
        try:
            ...
            raise ImportError('Could not import sqlalchemy python package. Please install it with `pip install sqlalchemy`.')
        result: sa.Result[Any]
        if isinstance(self.query, sa.SelectBase):
            result = self.db._execute(self.query, fetch='cursor', parameters=self.parameters)
            query_sql = str(self.query.compile(bind=self.db._engine))
        elif isinstance(self.query, str):
            result = self.db._execute(sa.text(self.query), fetch='cursor', parameters=self.parameters)
