...
class RandomHeight(base_layer.BaseRandomLayer):
    ...
    def __init__(self, factor, interpolation='bilinear', seed=None, **kwargs):
        ...
        if self.height_upper < self.height_lower:
            raise ValueError(f'`factor` argument cannot have an upper bound lesser than the lower bound. Received: factor={factor}')
        if self.height_lower < -1.0 or self.height_upper < -1.0:
            raise ValueError(f'`factor` argument must have values larger than -1. Received: factor={factor}')
    ...
    def get_config(self):
        config = {'factor': self.factor, 'interpolation': self.interpolation, 'seed': self.seed}
