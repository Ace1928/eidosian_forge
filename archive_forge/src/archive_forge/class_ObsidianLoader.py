...
class ObsidianLoader(BaseLoader):
    ...
    TEMPLATE_VARIABLE_REGEX = re.compile('{{(.*?)}}', re.DOTALL)
    TAG_REGEX = re.compile('[^\\S\\/]#([a-zA-Z_]+[-_/\\w]*)')
    DATAVIEW_LINE_REGEX = re.compile('^\\s*(\\w+)::\\s*(.*)$', re.MULTILINE)
    DATAVIEW_INLINE_BRACKET_REGEX = re.compile('\\[(\\w+)::\\s*(.*)\\]', re.MULTILINE)
    DATAVIEW_INLINE_PAREN_REGEX = re.compile('\\((\\w+)::\\s*(.*)\\)', re.MULTILINE)

    def __init__(self, path: Union[str, Path], encoding: str='UTF-8', collect_metadata: bool=True):
    ...
    def _parse_dataview_fields(self, content: str) -> dict:
        ...
        if not self.collect_metadata:
            return {}
        return {**{match[0]: match[1] for match in self.DATAVIEW_LINE_REGEX.findall(content)}, **{match[0]: match[1] for match in self.DATAVIEW_INLINE_PAREN_REGEX.findall(content)}, **{match[0]: match[1] for match in self.DATAVIEW_INLINE_BRACKET_REGEX.findall(content)}}
    ...
    def lazy_load(self) -> Iterator[Document]:
        ...
        for path in paths:
            ...
            metadata = {'source': str(path.name), 'path': str(path), 'created': path.stat().st_ctime, 'last_modified': path.stat().st_mtime, 'last_accessed': path.stat().st_atime, **self._to_langchain_compatible_metadata(front_matter), **dataview_fields}
