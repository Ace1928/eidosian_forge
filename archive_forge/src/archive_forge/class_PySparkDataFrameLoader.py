...
class PySparkDataFrameLoader(BaseLoader):
    ...
    def __init__(self, spark_session: Optional['SparkSession']=None, df: Optional[Any]=None, page_content_column: str='text', fraction_of_memory: float=0.1):
    ...
    def load(self) -> List[Document]:
        ...
        if self.df.count() > self.max_num_rows:
            logger.warning(f'The number of DataFrame rows is {self.df.count()}, but we will only include the amount of rows that can reasonably fit in memory: {self.num_rows}.')
