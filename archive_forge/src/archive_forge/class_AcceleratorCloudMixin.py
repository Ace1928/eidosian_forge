...
class AcceleratorCloudMixin:
    ...
    def delete_device_profile(self, name_or_id, filters):
        ...
        """
        device_profile = self.accelerator.get_device_profile(name_or_id, filters)
        if device_profile is None:
            self.log.debug('device_profile %s not found for deleting', name_or_id)
            return False
    ...
    def delete_accelerator_request(self, name_or_id, filters):
        ...
        :returns: True if delete succeeded, False otherwise.
        """
        accelerator_request = self.accelerator.get_accelerator_request(name_or_id, filters)
        if accelerator_request is None:
            self.log.debug('accelerator_request %s not found for deleting', name_or_id)
            return False
        self.accelerator.delete_accelerator_request(accelerator_request=accelerator_request)
        return True
