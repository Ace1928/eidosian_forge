...
class ActivationFunctionManager:
    ...
    def initialize_activation_types(self):
        ...
        def parallelized_computation(activation_dict):
            with ThreadPoolExecutor() as executor:
                future_to_activation = {
                    executor.submit(key, val): key
                    for key, val in activation_dict.items()
                }
                for future in concurrent.futures.as_completed(future_to_activation):
                    ...
                    try:
                        ...
                        logging.info(f"{activation_name}: {data}")
        ...
        # Utilize parallel computation to initialize and log activation functions
