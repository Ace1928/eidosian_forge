...
class PerceptualPathLength(Metric):
    ...
    def __init__(self, num_samples: int=10000, conditional: bool=False, batch_size: int=128, interpolation_method: Literal['lerp', 'slerp_any', 'slerp_unit']='lerp', epsilon: float=0.0001, resize: Optional[int]=64, lower_discard: Optional[float]=0.01, upper_discard: Optional[float]=0.99, sim_net: Union[nn.Module, Literal['alex', 'vgg', 'squeeze']]='vgg', **kwargs: Any) -> None:
        super().__init__(**kwargs)
        if not _TORCHVISION_AVAILABLE:
            raise ModuleNotFoundError('Metric `PerceptualPathLength` requires torchvision which is not installed.Install with `pip install torchvision` or `pip install torchmetrics[image]`')
