...
class SimulationState(SimulationStateBase, Generic[TState], metaclass=abc.ABCMeta):
    ...
    def __init__(self, *, state: TState, prng: Optional[np.random.RandomState]=None, qubits: Optional[Sequence['cirq.Qid']]=None, classical_data: Optional['cirq.ClassicalDataStore']=None):
    ...
    def measure(self, qubits: Sequence['cirq.Qid'], key: str, invert_mask: Sequence[bool], confusion_map: Dict[Tuple[int, ...], np.ndarray]):
    ...
    def sample(self, qubits: Sequence['cirq.Qid'], repetitions: int=1, seed: 'cirq.RANDOM_STATE_OR_SEED_LIKE'=None) -> np.ndarray:
        if self._state is not None:
