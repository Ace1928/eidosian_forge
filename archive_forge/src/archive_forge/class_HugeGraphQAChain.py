...
class HugeGraphQAChain(Chain):
    ...
    *Security note*: Make sure that the database connection uses credentials
    ...
    @classmethod
    def from_llm(cls, llm: BaseLanguageModel, *, qa_prompt: BasePromptTemplate=CYPHER_QA_PROMPT, gremlin_prompt: BasePromptTemplate=GREMLIN_GENERATION_PROMPT, **kwargs: Any) -> HugeGraphQAChain:
        """Initialize from LLM."""
        qa_chain = LLMChain(llm=llm, prompt=qa_prompt)
        gremlin_generation_chain = LLMChain(llm=llm, prompt=gremlin_prompt)
        return cls(qa_chain=qa_chain, gremlin_generation_chain=gremlin_generation_chain, **kwargs)
    ...
    def _call(self, inputs: Dict[str, Any], run_manager: Optional[CallbackManagerForChainRun]=None) -> Dict[str, str]:
        ...
        generated_gremlin = self.gremlin_generation_chain.run({'question': question, 'schema': self.graph.get_schema}, callbacks=callbacks)
