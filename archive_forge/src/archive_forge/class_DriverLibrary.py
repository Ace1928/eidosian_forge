...
class DriverLibrary:
    ...
    def __init__(self, status_socket=DEFAULT_STATUS_SOCKET, stats_socket=DEFAULT_STATS_SOCKET, get_socket=DEFAULT_GET_SOCKET, **kwargs):
    ...
    def _recv(self, sock):
        ...
        while True:
            ...
            if time.time() - begin > DRIVER_AGENT_TIMEOUT:
                raise driver_exceptions.DriverAgentTimeout(fault_string='The driver agent did not respond in {} seconds.'.format(DRIVER_AGENT_TIMEOUT))
            time.sleep(0.01)
        ...
        while payload_size - next_offset > 0:
            recv_size = sock.recv_into(mv_buffer[next_offset:], payload_size - next_offset)
    ...
    def _get_resource(self, resource, id):
        try:
            return self._send(self.get_socket, {constants.OBJECT: resource, constants.ID: id})
