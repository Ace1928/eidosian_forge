...
class TokenRenderer(renderer.Renderer):
  ...
  """
    CSI = '\x00'
    EMBELLISHMENTS = {'B': Token.Markdown.Bold, 'C': Token.Markdown.Code, 'I': Token.Markdown.Italic, 'N': Token.Markdown.Normal, 'Z': Token.Markdown.BoldItalic}
    INDENT = 4
    SPLIT_INDENT = 2
    TOKEN_TYPE_INDEX = 0
    TOKEN_TEXT_INDEX = 1
    ...
    """
        if attr is None:
            ...
            mask = 1 << attr
            self._font ^= mask
        font = self._font & (1 << renderer.BOLD | 1 << renderer.CODE | 1 << renderer.ITALIC)
    ...
    """
        ...
        if not level:
            ...
            self._AddToken(' ' * self._indent[level].second_line_indent + self._bullet[(level - 1) % len(self._bullet)])
    ...
    """
        ...
        for group in groups:
            ...
            if running_width + w >= self._width:
                ...
                self._AddToken(' ' * running_width)
                if running_width + w >= self._width:
                    running_width = self._SplitWideSynopsisGroup(group, indent, running_width)
                    continue
