...
class ModIndex(Transformation):
    ...
    def __init__(self):
        Transformation.__init__(self, UseDefChains, Ancestors, Aliases, RangeValues, Identifiers)
        self.loops_mod = dict()
    ...
    def visit_BinOp(self, node):
        ...
        header = ast.Assign([ast.Name(new_id, ast.Store(), None, None)], ast.BinOp(ast.BinOp(deepcopy(lower), ast.Sub(), ast.Constant(1, None)), ast.Mod(), deepcopy(node.right)), None)
        incr = ast.BinOp(ast.Name(new_id, ast.Load(), None, None), ast.Add(), ast.Constant(1, None))
        step = ast.Assign([ast.Name(new_id, ast.Store(), None, None)], ast.IfExp(ast.Compare(incr, [ast.Eq()], [deepcopy(node.right)]), ast.Constant(0, None), deepcopy(incr)), None)
