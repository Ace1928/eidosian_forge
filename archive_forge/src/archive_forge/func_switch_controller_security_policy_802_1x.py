...
def switch_controller_security_policy_802_1x(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('switch-controller.security-policy', '802-1X', filtered_data, vdom=vdom)
        current_data = fos.get('switch-controller.security-policy', '802-1X', vdom=vdom, mkey=mkey)
    ...
    if state == 'present' or state is True:
        return fos.set('switch-controller.security-policy', '802-1X', data=filtered_data, vdom=vdom)
    elif state == 'absent':
        return fos.delete('switch-controller.security-policy', '802-1X', mkey=filtered_data['name'], vdom=vdom)
    else:
