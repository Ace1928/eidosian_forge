...
class PhasedXPowGate(raw_types.Gate):
    ...
    def __init__(self, *, phase_exponent: Union[float, sympy.Expr], exponent: Union[float, sympy.Expr]=1.0, global_shift: float=0.0) -> None:
    ...
    def _qasm_(self, args: 'cirq.QasmArgs', qubits: Tuple['cirq.Qid', ...]) -> Optional[str]:
        ...
        if abs(e - 0.5) <= epsilon:
            return args.format('u2({0:half_turns}, {1:half_turns}) {2};\n', p - 0.5, -p + 0.5, qubits[0])
        return args.format('u3({0:half_turns}, {1:half_turns}, {2:half_turns}) {3};\n', -e, p + 0.5, -p - 0.5, qubits[0])
    ...
    def __pow__(self, exponent: Union[float, sympy.Expr]) -> 'PhasedXPowGate':
        ...
        return PhasedXPowGate(phase_exponent=self._phase_exponent, exponent=new_exponent, global_shift=self._global_shift)
    ...
    def _pauli_expansion_(self) -> value.LinearDict[str]:
        ...
        return value.LinearDict({'I': phase * np.cos(angle), 'X': -1j * phase * np.sin(angle) * np.cos(2 * phase_angle), 'Y': -1j * phase * np.sin(angle) * np.sin(2 * phase_angle)})

    def _is_parameterized_(self) -> bool:
        """See `cirq.SupportsParameterization`."""
        return protocols.is_parameterized(self._exponent) or protocols.is_parameterized(self._phase_exponent)

    def _parameter_names_(self) -> AbstractSet[str]:
        """See `cirq.SupportsParameterization`."""
        return protocols.parameter_names(self._exponent) | protocols.parameter_names(self._phase_exponent)
    ...
    def _resolve_parameters_(self, resolver: 'cirq.ParamResolver', recursive: bool) -> 'PhasedXPowGate':
        ...
        return PhasedXPowGate(phase_exponent=phase_exponent, exponent=exponent, global_shift=self._global_shift)

    def _phase_by_(self, phase_turns, qubit_index):
        """See `cirq.SupportsPhase`."""
        assert qubit_index == 0
        return PhasedXPowGate(exponent=self._exponent, phase_exponent=self._phase_exponent + phase_turns * 2, global_shift=self._global_shift)

    def _circuit_diagram_info_(self, args: 'cirq.CircuitDiagramInfoArgs') -> 'cirq.CircuitDiagramInfo':
        """See `cirq.SupportsCircuitDiagramInfo`."""
        return protocols.CircuitDiagramInfo(wire_symbols=(f'PhX({args.format_real(self.phase_exponent)})',), exponent=value.canonicalize_half_turns(self._exponent))
    ...
    def _value_equality_values_(self):
        if self.phase_exponent == 0:
            return common_gates.XPowGate(exponent=self._exponent, global_shift=self._global_shift)._value_equality_values_()
        if self.phase_exponent == 0.5:
            return common_gates.YPowGate(exponent=self._exponent, global_shift=self._global_shift)._value_equality_values_()
