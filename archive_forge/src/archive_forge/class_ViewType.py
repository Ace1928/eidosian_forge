...
class ViewType:
    ...
    _VIEW_TO_STRING = {ACTIVE_ONLY: 'active_only', DELETED_ONLY: 'deleted_only', ALL: 'all'}
    ...
    def from_string(cls, view_str):
        if view_str not in cls._STRING_TO_VIEW:
            raise Exception(f'Could not get valid view type corresponding to string {view_str}. Valid view types are {list(cls._STRING_TO_VIEW.keys())}')
        return cls._STRING_TO_VIEW[view_str]

    @classmethod
    def to_string(cls, view_type):
        if view_type not in cls._VIEW_TO_STRING:
            raise Exception(f'Could not get valid view type corresponding to string {view_type}. Valid view types are {list(cls._VIEW_TO_STRING.keys())}')
