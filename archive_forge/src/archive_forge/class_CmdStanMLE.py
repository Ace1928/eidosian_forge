...
class CmdStanMLE:
    ...
    def __init__(self, runset: RunSet) -> None:
        ...
        if not runset.method == Method.OPTIMIZE:
            raise ValueError('Wrong runset method, expecting optimize runset, found method {}'.format(runset.method))
        self.runset = runset
        self.converged = runset._check_retcodes()
        optimize_args = self.runset._args.method_args
        assert isinstance(optimize_args, OptimizeArgs)
        self._save_iterations: bool = optimize_args.save_iterations
        self._set_mle_attrs(runset.csv_files[0])

    def __repr__(self) -> str:
        repr = 'CmdStanMLE: model={}{}'.format(self.runset.model, self.runset._args.method_args.compose(0, cmd=[]))
    ...
    def stan_variable(self, var: str, *, inc_iterations: bool=False, warn: bool=True) -> Union[np.ndarray, float]:
        """
        ...
        try:
            ...
            if out.shape == () or out.shape == (1,):
                get_logger().warning('The default behavior of CmdStanMLE.stan_variable() will change in a future release to always return a numpy.ndarray, even for scalar variables.')
                return out.item()
            return out
        except KeyError:
