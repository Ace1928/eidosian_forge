...
def filter_continuation(continue_from, signatures, session, features):
  ...
  Returns:
    ...
    in `features`.
  """
    filter_signature = signatures.signature_def[_feature_keys.SavedModelLabels.FILTER]
    features = _canonicalize_numpy_data(data=features, require_single_batch=False)
    output_tensors_by_name, feed_dict = _colate_features_to_feeds_and_fetches(continue_from=continue_from, signature=filter_signature, features=features, graph=session.graph)
