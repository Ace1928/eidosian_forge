...
class QueryPowerBITool(BaseTool):
    ...
    def validate_llm_chain_input_variables(cls, llm_chain: Any) -> Any:
        ...
        for var in llm_chain.prompt.input_variables:
            if var not in ['tool_input', 'tables', 'schemas', 'examples']:
                raise ValueError("LLM chain for QueryPowerBITool must have input variables ['tool_input', 'tables', 'schemas', 'examples'], found %s", llm_chain.prompt.input_variables)
        return llm_chain
    ...
    def _run(self, tool_input: str, run_manager: Optional[CallbackManagerForToolRun]=None, **kwargs: Any) -> str:
        ...
        try:
            ...
            query = self.llm_chain.predict(tool_input=tool_input, tables=self.powerbi.get_table_names(), schemas=self.powerbi.get_schemas(), examples=self.examples, callbacks=run_manager.get_child() if run_manager else None)
        ...
        if error and iterations < self.max_iterations:
            return self._run(tool_input=RETRY_RESPONSE.format(tool_input=tool_input, query=query, error=error), run_manager=run_manager, iterations=iterations + 1)
        self.session_cache[tool_input] = result if result else BAD_REQUEST_RESPONSE.format(error=error)
        return self.session_cache[tool_input]

    async def _arun(self, tool_input: str, run_manager: Optional[AsyncCallbackManagerForToolRun]=None, **kwargs: Any) -> str:
        """Execute the query, return the results or an error message."""
        if (cache := self._check_cache(tool_input)):
            logger.debug('Found cached result for %s: %s', tool_input, cache)
        ...
        try:
            logger.info('Running PBI Query Tool with input: %s', tool_input)
            query = await self.llm_chain.apredict(tool_input=tool_input, tables=self.powerbi.get_table_names(), schemas=self.powerbi.get_schemas(), examples=self.examples, callbacks=run_manager.get_child() if run_manager else None)
        except Exception as exc:
            self.session_cache[tool_input] = f'Error on call to LLM: {exc}'
            return self.session_cache[tool_input]
        if query == 'I cannot answer this':
        ...
        if error is not None and ('TokenExpired' in error or 'TokenError' in error):
            self.session_cache[tool_input] = 'Authentication token expired or invalid, please try to reauthenticate or check the scope of the credential.'
            return self.session_cache[tool_input]
        iterations = kwargs.get('iterations', 0)
        if error and iterations < self.max_iterations:
            return await self._arun(tool_input=RETRY_RESPONSE.format(tool_input=tool_input, query=query, error=error), run_manager=run_manager, iterations=iterations + 1)
        self.session_cache[tool_input] = result if result else BAD_REQUEST_RESPONSE.format(error=error)
        return self.session_cache[tool_input]
    ...
    def _parse_output(self, pbi_result: Dict[str, Any]) -> Tuple[Optional[str], Optional[Any]]:
        ...
        if 'results' in pbi_result:
            ...
            if len(rows) == 0:
                ...
                return (None, '0 rows returned, this might be correct, but please validate if all filter values were correct?')
            result = json_to_md(rows)
            too_long, length = self._result_too_large(result)
            if too_long:
                return (f'Result too large, please try to be more specific or use the `TOPN` function. The result is {length} tokens long, the limit is {self.output_token_limit} tokens.', None)
            return (result, None)
