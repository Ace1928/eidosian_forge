...
class TextEncoder(nn.Module):
    ...
    def __post_init__(self):
        super().__init__()
        self.word_embeddings = nn.Embedding(self.vocab_size, self.dim, padding_idx=self.padding_idx)
        self.position_embeddings = nn.Embedding(self.max_position_embeddings, self.dim)
        if self.model_type == 'bert':
            self.register_buffer('position_ids', torch.arange(self.max_position_embeddings).unsqueeze(0), persistent=False)
        self.layer_norm = nn.LayerNorm(self.dim, eps=1e-12)
        self.dropout = nn.Dropout(self.dropout_prob)
        self.blocks = nn.ModuleList([TextEncoderBlock(self.dim, self.num_heads, self.dropout_prob, layer_id in self.multimodal_layers_ids) for layer_id in range(self.num_layers)])
    ...
    def forward_multimodal(self, x: Tensor, attn_mask: Tensor, context: Tensor) -> Tensor:
    ...
    def forward(self, x: Union[Tensor, dict], attention_mask: Optional[Tensor]=None, return_features: Optional[bool]=None) -> Tensor:
