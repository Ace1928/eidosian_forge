...
class WorkerTaskExecutor(executor.TaskExecutor):
    ...
    def __init__(self, uuid, exchange, topics, transition_timeout=pr.REQUEST_TIMEOUT, url=None, transport=None, transport_options=None, retry_options=None, worker_expiry=pr.EXPIRES_AFTER):
        self._uuid = uuid
        self._ongoing_requests = {}
        self._ongoing_requests_lock = threading.RLock()
        self._transition_timeout = transition_timeout
        self._proxy = proxy.Proxy(uuid, exchange, on_wait=self._on_wait, url=url, transport=transport, transport_options=transport_options, retry_options=retry_options)
        self._finder = wt.ProxyWorkerFinder(uuid, self._proxy, topics, worker_expiry=worker_expiry)
        self._proxy.dispatcher.type_handlers.update({pr.RESPONSE: dispatcher.Handler(self._process_response, validator=pr.Response.validate), pr.NOTIFY: dispatcher.Handler(self._finder.process_response, validator=functools.partial(pr.Notify.validate, response=True))})
        self._helper = None
        self._messages_processed = {'finder': self._finder.messages_processed}

    def _process_response(self, response, message):
        """Process response from remote side."""
        LOG.debug("Started processing response message '%s'", ku.DelayedPretty(message))
        try:
            request_uuid = message.properties['correlation_id']
        except KeyError:
            LOG.warning("The 'correlation_id' message property is missing in message '%s'", ku.DelayedPretty(message))
        else:
            request = self._ongoing_requests.get(request_uuid)
            if request is not None:
                response = pr.Response.from_dict(response)
                LOG.debug("Extracted response '%s' and matched it to request '%s'", response, request)
        ...
        if waiting_requests:
            ...
            if new_messages_processed > last_messages_processed:
                while waiting_requests:
                    ...
                    if worker is not None and request.transition_and_log_error(pr.PENDING, logger=LOG):
    ...
    def _submit_task(self, task, task_uuid, action, arguments, progress_callback=None, result=pr.NO_RESULT, failures=None):
        """Submit task request to a worker."""
        request = pr.Request(task, task_uuid, action, arguments, timeout=self._transition_timeout, result=result, failures=failures)
        if progress_callback is not None and task.notifier.can_be_registered(EVENT_UPDATE_PROGRESS):
            task.notifier.register(EVENT_UPDATE_PROGRESS, progress_callback)
            request.future.add_done_callback(lambda _fut: task.notifier.deregister(EVENT_UPDATE_PROGRESS, progress_callback))
        ...
        if worker is not None:
            ...
            LOG.debug("Delaying submission of '%s', no currently known worker/s available to process it", request)
            with self._ongoing_requests_lock:
                self._ongoing_requests[request.uuid] = request
    ...
    def execute_task(self, task, task_uuid, arguments, progress_callback=None):
        return self._submit_task(task, task_uuid, pr.EXECUTE, arguments, progress_callback=progress_callback)

    def revert_task(self, task, task_uuid, arguments, result, failures, progress_callback=None):
        return self._submit_task(task, task_uuid, pr.REVERT, arguments, result=result, failures=failures, progress_callback=progress_callback)
    ...
    def start(self):
        ...
        if self._helper is not None:
            raise RuntimeError('Worker executor must be stopped before it can be started')
