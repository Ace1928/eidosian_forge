...
class Dhcp_relay(ConfigBase):
    ...
    def get_delete_commands_requests_for_replaced_overridden(self, want, have, state):
        ...
        """
        default_value = {'circuit_id': DEFAULT_CIRCUIT_ID, 'max_hop_count': DEFAULT_MAX_HOP_COUNT, 'policy_action': DEFAULT_POLICY_ACTION}
        commands = []
        requests = []
        if not have:
            return (commands, requests)
        for conf in have:
            ...
            if ipv4_conf:
                ...
                if not match_ipv4:
                    ...
                    if not have_server_addresses.intersection(server_addresses) or (ipv4_conf.get('vrf_name') and match_ipv4.get('vrf_name') is None):
                        command['ipv4'] = ipv4_conf
                        requests.append(self.get_delete_all_dhcp_relay_intf_request(intf_name))
                    else:
                        ipv4_command = {}
                        ...
                        for option in ('circuit_id', 'max_hop_count', 'policy_action'):
                            if ipv4_conf.get(option) and match_ipv4.get(option) is None and (ipv4_conf[option] != default_value[option]):
                                ipv4_command[option] = ipv4_conf[option]
                        if ipv4_command:
            ...
            if ipv6_conf:
                ...
                if not match_ipv6:
                    ...
                    if not have_server_addresses.intersection(server_addresses) or (ipv6_conf.get('vrf_name') and match_ipv6.get('vrf_name') is None):
                        command['ipv6'] = ipv6_conf
                        requests.append(self.get_delete_all_dhcpv6_relay_intf_request(intf_name))
                    else:
                        ipv6_command = {}
                        ...
                        if ipv6_conf.get('max_hop_count') and match_ipv6.get('max_hop_count') is None and (ipv6_conf['max_hop_count'] != default_value['max_hop_count']):
