...
def _autobegin_t(self, begin: bool=False) -> SessionTransaction:
    if self._transaction is None:
        if not begin and (not self.autobegin):
            raise sa_exc.InvalidRequestError('Autobegin is disabled on this Session; please call session.begin() to start a new transaction')
        trans = SessionTransaction(self, SessionTransactionOrigin.BEGIN if begin else SessionTransactionOrigin.AUTOBEGIN)
