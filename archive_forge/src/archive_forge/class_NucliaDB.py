...
class NucliaDB(VectorStore):
    ...
    def __init__(self, knowledge_box: str, local: bool, api_key: Optional[str]=None, backend: Optional[str]=None) -> None:
        ...
        try:
            ...
            raise ValueError('nuclia python package not found. Please install it with `pip install nuclia`.')
    ...
    def add_texts(self, texts: Iterable[str], metadatas: Optional[List[dict]]=None, **kwargs: Any) -> List[str]:
        ...
        for i, text in enumerate(texts):
            ...
            id = factory.create(texts={'text': {'body': text}}, extra=extra, url=self.kb_url, api_key=self._config['TOKEN'])
            ids.append(id)
        return ids
    ...
    def similarity_search(self, query: str, k: int=4, **kwargs: Any) -> List[Document]:
        ...
        request = FindRequest(query=query, page_size=k, show=[ResourceProperties.VALUES, ResourceProperties.EXTRA])
        search = NucliaSearch()
        ...
        for resource in results.resources.values():
            for field in resource.fields.values():
                for paragraph_id, paragraph in field.paragraphs.items():
                    ...
                    paragraphs.append({'text': paragraph.text, 'metadata': {'extra': getattr(getattr(resource, 'extra', {}), 'metadata', None), 'value': value}, 'order': paragraph.order})
