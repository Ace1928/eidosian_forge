...
class AndroidConfig(Config):
    ...
    def __init__(self, config_file: Path, source_file: Path, python_exe: Path, dry_run: bool, android_data, existing_config_file: bool=False, extra_ignore_dirs: List[str]=None):
        super().__init__(config_file=config_file, source_file=source_file, python_exe=python_exe, dry_run=dry_run, existing_config_file=existing_config_file)
        ...
        if android_data.ndk_path:
            ...
            if ndk_path_temp:
                ...
                ndk_path_temp = ANDROID_DEPLOY_CACHE / 'android-ndk' / f'android-ndk-r{ANDROID_NDK_VERSION}'
                if ndk_path_temp.exists():
                    self.ndk_path = ndk_path_temp
        if self.ndk_path:
            print(f'Using Android NDK: {str(self.ndk_path)}')
        else:
            raise FileNotFoundError('[DEPLOY] Unable to find Android NDK. Please pass the NDK path either from the CLI or from pysidedeploy.spec')
        self.sdk_path = None
