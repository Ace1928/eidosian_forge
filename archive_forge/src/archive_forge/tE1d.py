    ...
    for path in module_paths:
        ...
        try:
            module = loaded_modules[
                module_name
            ]  # Attempt to access the dynamically loaded module
            for attr in dir(module):
                if attr.startswith(
                    "is_"
                ):  # Assuming validation functions start with 'is_'
                    rules[attr] = getattr(module, attr)
        except KeyError:
            logging.warning(
                f"Module {module_name} not found in loaded_modules. Continuing without loading its rules."
            )
            # No raise here, continue to the next module
    return rules
