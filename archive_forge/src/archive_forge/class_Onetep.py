...
class Onetep(FileIOCalculator):
    ...
    _dummy_parameters = ['ngwf_radius', 'xc', 'species_ngwf_radius', 'species_ngwf_number', 'species_solver', 'ngwf_radius_cond', 'pseudo_suffix', 'species_pseudo', 'species_core_wf', 'species_solver_cond', 'species_ngwf_number_cond', 'species_ngwf_radius_cond']
    _path_parameters = ['bsunfld_kpoint_path', 'bs_kpoint_path']
    _group_parameters = ['species_bsunfld_groups', 'species_ldos_groups', 'species_locdipole_groups', 'species_bsunfld_projatoms', 'species_pdos_groups', 'species_tddft_ct', 'species_tddft_kernel', 'nbo_write_species', 'species_ngwf_plot']
    _block_parameters = _path_parameters + _group_parameters + ['species_constraints', 'nbo_species_ngwflabel', 'ddec_rmse_vdw', 'vdw_params', 'sol_ions', 'swri']
    default_parameters = {'cutoff_energy': '1000 eV', 'kernel_cutoff': '1000 bohr', 'ngwf_radius': 12.0, 'ngwf_radius_cond': -1.0}
    name = 'onetep'

    def __init__(self, restart=None, ignore_bad_restart_file=FileIOCalculator._deprecated, label=None, command=None, atoms=None, **kwargs):
    ...
    def read(self, label):
        ...
        if not (read_lattice and read_species and read_positions):
            raise ReadError('Failed to read in essential calculation data from output file "%s"' % onetep_file)
