...
class SoftwareComponent(sc.SoftwareConfig):
    ...
    PROPERTIES = CONFIGS, INPUTS, OUTPUTS, OPTIONS = ('configs', sc.SoftwareConfig.INPUTS, sc.SoftwareConfig.OUTPUTS, sc.SoftwareConfig.OPTIONS)
    CONFIG_PROPERTIES = CONFIG_ACTIONS, CONFIG_CONFIG, CONFIG_TOOL = ('actions', 'config', 'tool')
    ATTRIBUTES = CONFIGS_ATTR, = ('configs',)
    config_schema = properties.Schema(properties.Schema.MAP, schema={CONFIG_ACTIONS: properties.Schema(properties.Schema.LIST, _('Lifecycle actions to which the configuration applies. The string values provided for this property can include the standard resource actions CREATE, DELETE, UPDATE, SUSPEND and RESUME supported by Heat.'), default=[resource.Resource.CREATE, resource.Resource.UPDATE], schema=properties.Schema(properties.Schema.STRING), constraints=[constr.Length(min=1)]), CONFIG_CONFIG: sc.SoftwareConfig.properties_schema[sc.SoftwareConfig.CONFIG], CONFIG_TOOL: properties.Schema(properties.Schema.STRING, _('The configuration tool used to actually apply the configuration on a server. This string property has to be understood by in-instance tools running inside deployed servers.'), required=True)})
    properties_schema = {CONFIGS: properties.Schema(properties.Schema.LIST, _('The list of configurations for the different lifecycle actions of the represented software component.'), schema=config_schema, constraints=[constr.Length(min=1)], required=True), INPUTS: sc.SoftwareConfig.properties_schema[sc.SoftwareConfig.INPUTS], OUTPUTS: sc.SoftwareConfig.properties_schema[sc.SoftwareConfig.OUTPUTS], OPTIONS: sc.SoftwareConfig.properties_schema[sc.SoftwareConfig.OPTIONS]}
    ...
    def _resolve_attribute(self, name):
        ...
        if name == self.CONFIGS_ATTR and self.resource_id:
            with self.rpc_client().ignore_error_by_name('NotFound'):
                sc = self.rpc_client().show_software_config(self.context, self.resource_id)
    ...
    def validate(self):
        ...
        for config in configs:
            ...
            if any((action in config_actions for action in actions)):
                msg = _('Defining more than one configuration for the same action in SoftwareComponent "%s" is not allowed.') % self.name
