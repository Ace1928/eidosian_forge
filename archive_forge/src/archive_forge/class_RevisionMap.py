...
class RevisionMap:
    ...
    def get_current_head(self, branch_label: Optional[str]=None) -> Optional[str]:
        ...
        if len(current_heads) > 1:
            raise MultipleHeads(current_heads, '%s@head' % branch_label if branch_label else 'head')
        if current_heads:
                ...
                try:
                    ...
                    if rint < 0:
                        ...
                        if branch_label is not None:
                            select_heads = tuple((head for head in select_heads if branch_label in is_revision(head).branch_labels))
    ...
    def iterate_revisions(self, upper: _RevisionIdentifierType, lower: _RevisionIdentifierType, implicit_base: bool=False, inclusive: bool=False, assert_relative_length: bool=True, select_for_downgrade: bool=False) -> Iterator[Revision]:
        ...
        revisions, heads = fn(upper, lower, inclusive=inclusive, implicit_base=implicit_base, assert_relative_length=assert_relative_length)
    ...
    def _walk(self, start: Optional[Union[str, Revision]], steps: int, branch_label: Optional[str]=None, no_overwalk: bool=True) -> Optional[_RevisionOrBase]:
        """
        ...
        for _ in range(abs(steps)):
            if steps > 0:
                assert initial != 'base'
                walk_up = [is_revision(rev) for rev in self.get_revisions(self.bases if initial is None else initial.nextrev)]
                if branch_label:
