...
class RemoteFsConnectorTestCase(test_connector.ConnectorTestCase):
    ...
    def setUp(self):
        ...
        self.connection_properties = {'export': self.TEST_DEV, 'name': self.TEST_NAME}
        self.connector = remotefs.RemoteFsConnector('nfs', root_helper='sudo', nfs_mount_point_base=self.TEST_BASE, nfs_mount_options='vers=3')
    ...
    def test_get_connector_properties(self):
        props = remotefs.RemoteFsConnector.get_connector_properties('sudo', multipath=True, enforce_multipath=True)
        expected_props = {}
    ...
    def test_get_volume_paths(self, mock_mount):
        path = '%(path)s/%(name)s' % {'path': self.TEST_PATH, 'name': self.TEST_NAME}
        expected = [path]
        actual = self.connector.get_volume_paths(self.connection_properties)
        self.assertEqual(expected, actual)

    @mock.patch.object(remotefs_client.RemoteFsClient, 'mount')
    @mock.patch.object(remotefs_client.RemoteFsClient, 'get_mount_point', return_value='something')
