...
class MetricsCollectorClient(object):
    ...
    def __enter__(self):
        ...
        send_method = getattr(self, self.metrics_type + '_invocation_start_total')
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        if self.conf.metrics_enabled:
            ...
            send_method = getattr(self, self.metrics_type + '_processing_seconds')
            send_method(duration=duration, **self.args)
            send_method = getattr(self, self.metrics_type + '_invocation_end_total')
            send_method(**self.args)
    ...
    def rpc_client_invocation_start_total(self, target, method, call_type, timeout=None):
        self.put_rpc_client_metrics_to_txqueue('rpc_client_invocation_start_total', message_type.MetricAction('inc', None), target, method, call_type, timeout)

    def rpc_client_invocation_end_total(self, target, method, call_type, timeout=None):
        self.put_rpc_client_metrics_to_txqueue('rpc_client_invocation_end_total', message_type.MetricAction('inc', None), target, method, call_type, timeout)
    ...
    def rpc_server_invocation_start_total(self, target, endpoint, ns, ver, method):
        self.put_rpc_server_metrics_to_txqueue('rpc_server_invocation_start_total', message_type.MetricAction('inc', None), target, endpoint, ns, ver, method)

    def rpc_server_invocation_end_total(self, target, endpoint, ns, ver, method):
        self.put_rpc_server_metrics_to_txqueue('rpc_server_invocation_end_total', message_type.MetricAction('inc', None), target, endpoint, ns, ver, method)
