...
class AzureRMVirtualMachineScaleSetInstance(AzureRMModuleBase):
    ...
    def __init__(self):
        self.module_arg_spec = dict(resource_group=dict(type='str', required=True), vmss_name=dict(type='str', required=True), instance_id=dict(type='str', required=True), latest_model=dict(type='bool'), power_state=dict(type='str', choices=['running', 'stopped', 'deallocated']), protect_from_scale_in=dict(type='bool'), protect_from_scale_set_actions=dict(type='bool'), state=dict(type='str', default='present', choices=['present', 'absent']))
        ...
        if self.state == 'absent':
            ...
            if self.protect_from_scale_in is not None or self.protect_from_scale_set_actions is not None:
                for item in instances:
                    ...
                    if protection_policy is None or self.protect_from_scale_in != protection_policy['protect_from_scale_in'] or self.protect_from_scale_set_actions != protection_policy['protect_from_scale_set_actions']:
                        if not self.check_mode:
    ...
    def delete(self, instance_id):
        try:
            self.mgmt_client.virtual_machine_scale_set_vms.begin_delete(resource_group_name=self.resource_group, vm_scale_set_name=self.vmss_name, instance_id=instance_id)
    ...
    def start(self, instance_id):
        try:
            self.mgmt_client.virtual_machine_scale_set_vms.begin_start(resource_group_name=self.resource_group, vm_scale_set_name=self.vmss_name, instance_id=instance_id)
        except Exception as e:
    ...
    def stop(self, instance_id):
        try:
            self.mgmt_client.virtual_machine_scale_set_vms.begin_power_off(resource_group_name=self.resource_group, vm_scale_set_name=self.vmss_name, instance_id=instance_id)
        except Exception as e:
    ...
    def update_protection_policy(self, instance_id, protect_from_scale_in, protect_from_scale_set_actions):
        try:
            ...
            instance = self.mgmt_client.virtual_machine_scale_set_vms.get(resource_group_name=self.resource_group, vm_scale_set_name=self.vmss_name, instance_id=instance_id)
            instance.protection_policy = protection_policy
            poller = self.mgmt_client.virtual_machine_scale_set_vms.begin_update(resource_group_name=self.resource_group, vm_scale_set_name=self.vmss_name, instance_id=instance_id, parameters=instance)
            self.get_poller_result(poller)
        except Exception as e:
    ...
    def format_response(self, item):
        ...
        for index in range(len(iv['statuses'])):
            ...
            if code[0] == 'PowerState':
                power_state = code[1]
                break
        d = {'id': d.get('id'), 'tags': d.get('tags'), 'instance_id': d.get('instance_id'), 'latest_model': d.get('latest_model_applied'), 'power_state': power_state, 'protection_policy': d.get('protection_policy')}
        return d