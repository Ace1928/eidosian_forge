...
def test_COCu111_2(testdir):
    ...
    a = 3.6
    d = a / sqrt(2)
    fcc111 = Atoms(symbols='Cu', cell=[(d, 0, 0), (d / 2, d * sqrt(3) / 2, 0), (d / 2, d * sqrt(3) / 6, -a / sqrt(3))], pbc=True)
    ...
    with Optimizer(final, logfile=logfile) as dyn:
        dyn.run(fmax=0.1)
    neb = NEB([initial, initial.copy(), initial.copy(), final], allow_shared_calculator=True)
    neb.interpolate()
    neb.set_calculators(EMT())
    with Optimizer(neb, maxstep=0.04, trajectory='mep_2coarse.traj', logfile=logfile) as dyn:
        dyn.run(fmax=0.1)
    ...
    neb.set_calculators(EMT())
    with Optimizer(neb, maxstep=0.04, trajectory='mep_2fine.traj', logfile=logfile) as dyn:
        dyn.run(fmax=0.1)
