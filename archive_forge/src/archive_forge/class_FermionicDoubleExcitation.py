...
class FermionicDoubleExcitation(Operation):
    ...
    def __init__(self, weight, wires1=None, wires2=None, id=None):
        if len(wires1) < 2:
            raise ValueError(f'expected at least two wires representing the occupied orbitals; got {len(wires1)}')
        if len(wires2) < 2:
            raise ValueError(f'expected at least two wires representing the unoccupied orbitals; got {len(wires2)}')
