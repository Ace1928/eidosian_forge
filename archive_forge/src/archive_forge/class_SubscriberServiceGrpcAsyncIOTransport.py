...
class SubscriberServiceGrpcAsyncIOTransport(SubscriberServiceTransport):
    ...
    @property
    def subscribe(self) -> Callable[[subscriber.SubscribeRequest], Awaitable[subscriber.SubscribeResponse]]:
        """Return a callable for the subscribe method over gRPC.
        ...
        if 'subscribe' not in self._stubs:
            self._stubs['subscribe'] = self.grpc_channel.stream_stream('/google.cloud.pubsublite.v1.SubscriberService/Subscribe', request_serializer=subscriber.SubscribeRequest.serialize, response_deserializer=subscriber.SubscribeResponse.deserialize)
