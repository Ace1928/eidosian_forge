...
class GCENodeDriverTest(GoogleTestCase, TestCaseMixin):
    ...
    def test_build_disk_gce_struct(self):
        ...
        is_boot = True
        d = self.driver._build_disk_gce_struct(device_name=device_name, source=source, disk_name=disk_name, is_boot=is_boot)
        ...
        source = None
        is_boot = True
        d = self.driver._build_disk_gce_struct(device_name=device_name, disk_type=disk_type, image=image, is_boot=is_boot)
        self.assertEqual('READ_WRITE', d['mode'])
        self.assertEqual('PERSISTENT', d['type'])
        self.assertTrue('initializeParams' in d and isinstance(d['initializeParams'], dict))
        self.assertTrue(all((k in d['initializeParams'] for k in ['sourceImage', 'diskType', 'diskName'])))
    ...
    def test_ex_get_serial_output(self):
        ...
        self.assertTrue(self.driver.ex_get_serial_output(node), 'This is some serial\r\noutput for you.')
    ...
    def test_ex_instancegroup_set_named_ports(self):
        ...
        self.assertTrue(self.driver.ex_instancegroup_set_named_ports(gceobj, []))
        self.assertRaises(ValueError, self.driver.ex_instancegroup_set_named_ports, gceobj, 'foobar')
        self.assertRaises(ValueError, self.driver.ex_instancegroup_set_named_ports, gceobj, {'name': 'foo', 'port': 4444})
    ...
    def test_list_sizes(self):
        ...
        self.assertEqual(sizes[1].extra['accelerators'][0]['guestAcceleratorType'], 'nvidia-tesla-v100')
    ...
    def test_ex_create_firewall_egress(self):
        ...
        network = 'default'
        firewall = self.driver.ex_create_firewall(name, allowed, description=description, network=network, priority=priority, direction=direction, target_ranges=target_ranges, target_service_accounts=target_service_accounts)
        self.assertTrue(isinstance(firewall, GCEFirewall))
        self.assertEqual(firewall.name, name)
    ...
    def test_ex_create_forwarding_rule(self):
        ...
        fwr = self.driver.ex_create_forwarding_rule(fwr_name, targetpool, region=region, address=address, port_range=port_range, description=description)
    ...
    def test_ex_create_multiple_nodes(self):
        ...
        number = 2
        disk_size = '25'
        nodes = self.driver.ex_create_multiple_nodes(base_name, size, image, number, ex_disk_size=disk_size, poll_interval=0.1)
    ...
    def test_ex_create_multiple_nodes_image_family(self):
        ...
        size = self.driver.ex_get_size('n1-standard-1')
        number = 2
        nodes = self.driver.ex_create_multiple_nodes(base_name, size, image, number, ex_image_family='coreos-stable', poll_interval=0.1)
        self.assertEqual(len(nodes), 2)
        self.assertTrue(isinstance(nodes[0], Node))
        self.assertTrue(isinstance(nodes[1], Node))
        self.assertEqual(nodes[0].name, '%s-000' % base_name)
        self.assertEqual(nodes[1].name, '%s-001' % base_name)
        image = self.driver.ex_get_image('debian-7')
        self.assertRaises(ValueError, self.driver.ex_create_multiple_nodes, base_name, size, image, number, ex_image_family='coreos-stable')
    ...
    def test_ex_get_node(self):
        ...
        self.assertRaises(ResourceNotFoundError, self.driver.ex_get_node, removed_node, 'us-central1-b')
