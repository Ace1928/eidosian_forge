...
class NetAppOntapVlan:
    ...
    def __init__(self):
        ...
        self.module = AnsibleModule(argument_spec=self.argument_spec, required_together=[['broadcast_domain', 'ipspace']], supports_check_mode=True)
    ...
    def create_vlan(self):
        ...
        if self.use_rest:
            ...
            body = {'type': 'vlan', 'node': {'name': self.parameters['node']}, 'vlan': {'base_port': {'name': self.parameters['parent_interface'], 'node': {'name': self.parameters['node']}}, 'tag': self.parameters['vlanid']}}
    ...
    def get_vlan(self):
        ...
        query = {'query': {'vlan-info': {'interface-name': self.parameters['interface_name'], 'node': self.parameters['node']}}}
        ...
        if result.get_child_by_name('num-records') and int(result.get_child_content('num-records')) == 1:
            ...
            current = {'interface_name': vlan_info.get_child_content('interface-name'), 'node': vlan_info.get_child_content('node')}
            return current
        return None
