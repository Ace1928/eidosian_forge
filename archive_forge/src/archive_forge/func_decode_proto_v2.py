...
def decode_proto_v2(bytes: _atypes.TensorFuzzingAnnotation[_atypes.String], message_type: str, field_names, output_types, descriptor_source: str='local://', message_format: str='binary', sanitize: bool=False, name=None):
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'DecodeProtoV2', name, bytes, 'message_type', message_type, 'field_names', field_names, 'output_types', output_types, 'descriptor_source', descriptor_source, 'message_format', message_format, 'sanitize', sanitize)
            ...
            pass
        try:
            _result = _dispatcher_for_decode_proto_v2((bytes, message_type, field_names, output_types, descriptor_source, message_format, sanitize, name), None)
            if _result is not NotImplemented:
                return _result
            return decode_proto_v2_eager_fallback(bytes, message_type=message_type, field_names=field_names, output_types=output_types, descriptor_source=descriptor_source, message_format=message_format, sanitize=sanitize, name=name, ctx=_ctx)
        except _core._SymbolicException:
            pass
        except (TypeError, ValueError):
            _result = _dispatch.dispatch(decode_proto_v2, (), dict(bytes=bytes, message_type=message_type, field_names=field_names, output_types=output_types, descriptor_source=descriptor_source, message_format=message_format, sanitize=sanitize, name=name))
    ...
    try:
        _, _, _op, _outputs = _op_def_library._apply_op_helper('DecodeProtoV2', bytes=bytes, message_type=message_type, field_names=field_names, output_types=output_types, descriptor_source=descriptor_source, message_format=message_format, sanitize=sanitize, name=name)
    except (TypeError, ValueError):
        _result = _dispatch.dispatch(decode_proto_v2, (), dict(bytes=bytes, message_type=message_type, field_names=field_names, output_types=output_types, descriptor_source=descriptor_source, message_format=message_format, sanitize=sanitize, name=name))
        if _result is not _dispatch.OpDispatcher.NOT_SUPPORTED:
            return _result
    ...
    if _execute.must_record_gradient():
        _attrs = ('message_type', _op.get_attr('message_type'), 'field_names', _op.get_attr('field_names'), 'output_types', _op.get_attr('output_types'), 'descriptor_source', _op.get_attr('descriptor_source'), 'message_format', _op.get_attr('message_format'), 'sanitize', _op._get_attr_bool('sanitize'))
