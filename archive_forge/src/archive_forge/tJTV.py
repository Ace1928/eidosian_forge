...
class ImageCompressorApp(tk.Tk):
    ...
    def async_decompress_and_show(self):
        try:
            encrypted_data, image_format = asyncio.run(
                retrieve_compressed_image(self.img_hash)
            )
    ...
    def update_image_display(self, metadata_list):
        ...
        for metadata, label in zip(metadata_list, self.image_labels):
            try:
                img = Image.open(
                    io.BytesIO(metadata["image_data"])
                )  # Adjust according to your metadata
    ...
    async def async_browse_stored_images_coroutine(self):
        ...
        try:
            ...
            self.update_image_display(metadata_list)
        except Exception as e:
    ...
    def async_browse_stored_images(self):
        Thread(
            target=lambda: asyncio.run(self.async_browse_stored_images_coroutine()),
            daemon=True,
        ).start()
