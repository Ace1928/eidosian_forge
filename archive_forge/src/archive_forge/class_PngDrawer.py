...
class PngDrawer:
    ...
    def add_node(self, viz: Any, node: str) -> None:
        viz.add_node(node, label=self.get_node_label(node), style='filled', fillcolor='yellow', fontsize=15, fontname=self.fontname)

    def add_edge(self, viz: Any, source: str, target: str, label: Optional[str]=None, conditional: bool=False) -> None:
        viz.add_edge(source, target, label=self.get_edge_label(label) if label else '', fontsize=12, fontname=self.fontname, style='dotted' if conditional else 'solid')
    ...
    def draw(self, graph: Graph, output_path: Optional[str]=None) -> Optional[bytes]:
        ...
        try:
            ...
            raise ImportError('Install pygraphviz to draw graphs: `pip install pygraphviz`.') from exc
