...
@keras_export('keras.layers.Discretization', 'keras.layers.experimental.preprocessing.Discretization')
class Discretization(base_preprocessing_layer.PreprocessingLayer):
    ...
    def __init__(self, bin_boundaries=None, num_bins=None, epsilon=0.01, output_mode='int', sparse=False, **kwargs):
        if 'bins' in kwargs:
            logging.warning('bins is deprecated, please use bin_boundaries or num_bins instead.')
        ...
        base_preprocessing_layer.keras_kpl_gauge.get_cell('Discretization').set(True)
        ...
        layer_utils.validate_string_arg(output_mode, allowable_strings=(INT, ONE_HOT, MULTI_HOT, COUNT), layer_name=self.__class__.__name__, arg_name='output_mode')
        ...
        if num_bins is not None and num_bins < 0:
            raise ValueError('`num_bins` must be greater than or equal to 0. You passed `num_bins={}`'.format(num_bins))
        if num_bins is not None and bin_boundaries is not None:
            raise ValueError('Both `num_bins` and `bin_boundaries` should not be set. You passed `num_bins={}` and `bin_boundaries={}`'.format(num_bins, bin_boundaries))
    ...
    def build(self, input_shape):
        ...
        if self.input_bin_boundaries is not None:
            return
        self.summary = self.add_weight(name='summary', shape=(2, None), dtype=tf.float32, initializer=lambda shape, dtype: [[], []], trainable=False)
    ...
    def update_state(self, data):
        if self.input_bin_boundaries is not None:
            raise ValueError('Cannot adapt a Discretization layer that has been initialized with `bin_boundaries`, use `num_bins` instead. You passed `bin_boundaries={}`.'.format(self.input_bin_boundaries))
        if not self.built:
    ...
    def finalize_state(self):
        if self.input_bin_boundaries is not None or not self.built:
            return
        self.bin_boundaries = utils.listify_tensors(get_bin_boundaries(self.summary, self.num_bins))
    ...
    def get_config(self):
        ...
        config.update({'bin_boundaries': self.input_bin_boundaries, 'num_bins': self.num_bins, 'epsilon': self.epsilon, 'output_mode': self.output_mode, 'sparse': self.sparse})
        return config
    ...
    def call(self, inputs):
        ...
        if tf_utils.is_ragged(inputs):
            ...
            indices = tf.SparseTensor(indices=tf.identity(inputs.indices), values=bucketize(inputs.values), dense_shape=tf.identity(inputs.dense_shape))
        else:
            indices = bucketize(inputs)
        return utils.encode_categorical_inputs(indices, output_mode=self.output_mode, depth=len(self.bin_boundaries) + 1, sparse=self.sparse, dtype=self.compute_dtype)