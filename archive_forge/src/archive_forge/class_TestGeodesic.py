...
class TestGeodesic:
    ...
    def setup_class(self):
        ...
        data = np.array([(0.0, 36.530042355, 176.1258751622, 5.7623446947, -48.1642707791, 175.3343083163, 9398502.0434687), (0.0, 20.8766024619, 6.9012827094, 163.9792202999, 64.2764863397, 165.0440144913, 10462971.2273697), (0.0, 59.7405712203, 80.9569174535, 80.196995466, 30.9857449391, 144.4488137288, 6549489.1863671), (0.0, 38.6508883588, 18.3455177945, 23.5931524958, 66.3457305181, 37.7145989984, 3425212.476799), (0.0, 23.2214345509, 165.5720618611, 148.3625110902, -68.8453788967, 39.2692310682, 14506511.2971898), (0.0, 31.2989275984, 155.7723493796, 93.8764112107, -69.2776346668, 98.5250397385, 13370814.5013951), (0.0, 49.6823298563, 1.0175398481, 5.3554086646, 83.8681965431, 6.1667605618, 3815028.2543705), (0.0, 32.7651878215, 98.6494285944, 70.3527194957, 2.477749177, 123.5999412794, 8030520.7178933), (0.0, 46.3648067071, 94.9148631993, 56.5676529172, 25.2581951337, 130.4405565458, 5485075.9286327), (0.0, 33.7321188396, 147.9041907517, 33.1346935645, -26.3211288531, 150.4502346224, 7512675.5414637)], dtype=[('start_lon', np.float64), ('start_lat', np.float64), ('start_azi', np.float64), ('end_lon', np.float64), ('end_lat', np.float64), ('end_azi', np.float64), ('dist', np.float64)])
        ...
        self.direct_solution = np.column_stack([self.data.end_lon, self.data.end_lat, self.data.end_azi])
        self.inverse_solution = np.column_stack([self.data.dist, self.data.start_azi, self.data.end_azi])
    ...
    def test_circle(self):
        ...
        assert_almost_equal(geod_circle, np.array([[40.0, 54.49349757], [34.23766162, 47.60355349], [45.76233838, 47.60355349]]), decimal=5)
