...
class _IonQClient:
    ...
    def __init__(self, remote_host: str, api_key: str, default_target: Optional[str]=None, api_version: str='v0.3', max_retry_seconds: int=3600, verbose: bool=False):
        ...
        assert url.scheme and url.netloc, f'Specified remote_host {remote_host} is not a valid url, for example http://example.com'
        assert api_version in self.SUPPORTED_VERSIONS, f'Only api v0.3 is accepted but was {api_version}'
        assert default_target is None or default_target in self.SUPPORTED_TARGETS, f'Target can only be one of {self.SUPPORTED_TARGETS} but was {default_target}.'
    ...
    def create_job(self, serialized_program: cirq_ionq.SerializedProgram, repetitions: Optional[int]=None, target: Optional[str]=None, name: Optional[str]=None, extra_query_params: Optional[dict]=None) -> dict:
        """Create a job.
    ...
    def get_results(self, job_id: str, sharpen: Optional[bool]=None, extra_query_params: Optional[dict]=None):
    ...
    def list_jobs(self, status: Optional[str]=None, limit: int=100, batch_size: int=1000) -> List[Dict[str, Any]]:
    ...
    def list_calibrations(self, start: Optional[datetime.datetime]=None, end: Optional[datetime.datetime]=None, limit: int=100, batch_size: int=1000) -> List[dict]:
    ...
    def _target(self, target: Optional[str]) -> str:
        ...
        """
        assert target is not None or self.default_target is not None, 'One must specify a target on this call, or a default_target on the service/client, but neither were set.'
        ...
        while True:
            try:
                ...
                if response.status_code == requests.codes.unauthorized:
                    raise ionq_exceptions.IonQException('"Not authorized" returned by IonQ API. Check to ensure you have supplied the correct API key.', response.status_code)
                if response.status_code == requests.codes.not_found:
                    raise ionq_exceptions.IonQNotFoundException('IonQ could not find requested resource.')
                if not _is_retriable(response.status_code, response.request.method):
                    ...
                    try:
                        ...
                        pass
                    raise ionq_exceptions.IonQException(f'Non-retry-able error making request to IonQ API. Request Body: {json} Response Body: {error} Status: {response.status_code} Error:{response.reason}', response.status_code)
    ...
    def _list(self, resource_path: str, params: dict, response_key: str, limit: int, batch_size: int) -> List[Dict]:
        ...
        while len(results) < limit:
            ...
            def request():
                return requests.get(f'{self.url}/{resource_path}', headers=self.headers, json=json, params=full_params)
