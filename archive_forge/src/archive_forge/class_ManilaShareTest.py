...
class ManilaShareTest(common.HeatTestCase):
    ...
    def setUp(self):
        ...
        self.available_share = mock.MagicMock(id='test_share_id', status=mshare.ManilaShare.STATUS_AVAILABLE)
        self.failed_share = mock.MagicMock(id='test_share_id', status=mshare.ManilaShare.STATUS_ERROR)
        self.deleting_share = mock.MagicMock(id='test_share_id', status=mshare.ManilaShare.STATUS_DELETING)
    ...
    def test_share_create(self):
        ...
        self.assertEqual(self.fake_share.id, share.resource_id, 'Expected share ID was not propagated to share')
        share.client().shares.allow.assert_called_once_with(access='127.0.0.1', access_level='ro', share=share.resource_id, access_type='ip')
        args, kwargs = share.client().shares.create.call_args
        message_end = ' parameter was not passed to manila client'
        self.assertEqual(u'NFS', kwargs['share_proto'], 'Share protocol' + message_end)
        self.assertEqual(1, kwargs['size'], 'Share size' + message_end)
        self.assertEqual('basic_test_share', kwargs['name'], 'Share name' + message_end)
        self.assertEqual('basic test share', kwargs['description'], 'Share description' + message_end)
        self.assertEqual({u'key': u'value'}, kwargs['metadata'], 'Metadata' + message_end)
    ...
    def test_share_update(self):
        ...
        kwargs = {'display_name': 'name', 'display_description': 'desc'}
        share.client().shares.update.assert_called_once_with(share.resource_id, **kwargs)
    ...
    def test_attributes(self):
        ...
        share.client().share_export_locations.list.return_value = [DummyShareExportLocation()]
