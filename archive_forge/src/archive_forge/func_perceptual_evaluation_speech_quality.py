...
def perceptual_evaluation_speech_quality(preds: Tensor, target: Tensor, fs: int, mode: str, keep_same_device: bool=False, n_processes: int=1) -> Tensor:
    ...
    if not _PESQ_AVAILABLE:
        raise ModuleNotFoundError('PESQ metric requires that pesq is installed. Either install as `pip install torchmetrics[audio]` or `pip install pesq`.')
