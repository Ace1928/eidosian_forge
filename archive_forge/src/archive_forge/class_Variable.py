...
class Variable(trackable.Trackable, metaclass=VariableMetaclass):
  ...
  """
    ...
    def __init__(self, initial_value=None, trainable=None, validate_shape=True, caching_device=None, name=None, variable_def=None, dtype=None, import_scope=None, constraint=None, synchronization=VariableSynchronization.AUTO, aggregation=VariableAggregation.NONE, shape=None, experimental_enable_variable_lifting=True):
    ...
    def __hash__(self):
        if tensor_lib.Tensor._USE_EQUALITY and ops.executing_eagerly_outside_functions():
            raise TypeError(f'Variable is unhashable. Instead, use variable.ref() as the key. (Variable: {self})')
        else:
            return id(self)
