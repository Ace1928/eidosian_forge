...
class AzureRMPostgreSQLFirewallRulesInfo(AzureRMModuleBase):
    ...
    def exec_module(self, **kwargs):
        ...
        if is_old_facts:
            self.module.deprecate("The 'azure_rm_postgresqlfirewallrule_facts' module has been renamed to 'azure_rm_postgresqlfirewallrule_info'", version=(2.9,))
    ...
    def list_by_server(self):
        ...
        results = []
        try:
            response = self.postgresql_client.firewall_rules.list_by_server(resource_group_name=self.resource_group, server_name=self.server_name)
            self.log('Response : {0}'.format(response))
