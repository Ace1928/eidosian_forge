...
class NovaHostAggregateTest(common.HeatTestCase):
    ...
    def setUp(self):
        ...
        self.stack = stack.Stack(self.ctx, 'nova_aggregate_test_stack', template.Template(AGGREGATE_TEMPLATE))
    ...
    def test_aggregate_get_live_state(self):
        value = mock.MagicMock()
        value.to_dict.return_value = {'availability_zone': 'nova', 'name': 'test', 'hosts': ['ubuntu'], 'metadata': {'test': 'test', 'availability_zone': 'nova'}}
        self.aggregates.get.return_value = value
        reality = self.my_aggregate.get_live_state(self.my_aggregate.properties)
        expected = {'availability_zone': 'nova', 'name': 'test', 'hosts': ['ubuntu'], 'metadata': {'test': 'test', 'availability_zone': 'nova'}}
