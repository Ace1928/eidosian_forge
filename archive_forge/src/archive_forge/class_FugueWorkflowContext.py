...
class FugueWorkflowContext(WorkflowContext):
    ...
    def __init__(self, engine: ExecutionEngine, compile_conf: Any=None, cache: Any=NoOpCache, workflow_engine: Any=None, hooks: Any=WorkflowHooks):
        ...
        if workflow_engine is None:
            workflow_engine = ParallelExecutionEngine(conf.get_or_throw(FUGUE_CONF_WORKFLOW_CONCURRENCY, int), self)
        super().__init__(cache=cache, engine=workflow_engine, hooks=hooks, logger=self.execution_engine.log, config=conf)
