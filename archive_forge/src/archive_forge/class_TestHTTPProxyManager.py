...
class TestHTTPProxyManager(HTTPDummyProxyTestCase):
    ...
    def test_redirect(self):
        with proxy_from_url(self.proxy_url) as http:
            r = http.request('GET', '%s/redirect' % self.http_url, fields={'target': '%s/' % self.http_url}, redirect=False)
            assert r.status == 303
            r = http.request('GET', '%s/redirect' % self.http_url, fields={'target': '%s/' % self.http_url})
            assert r.status == 200
    ...
    def test_cross_protocol_redirect(self):
        with proxy_from_url(self.proxy_url, ca_certs=DEFAULT_CA) as http:
            ...
            with pytest.raises(MaxRetryError):
                http.request('GET', '%s/redirect' % self.http_url, fields={'target': cross_protocol_location}, retries=0)
            r = http.request('GET', '%s/redirect' % self.http_url, fields={'target': '%s/echo?a=b' % self.https_url}, retries=1)
    ...
    @requires_network
    @pytest.mark.parametrize(['proxy_scheme', 'target_scheme', 'use_forwarding_for_https'], [('http', 'http', False), ('https', 'http', False), ('https', 'https', True)])
    def test_forwarding_proxy_connect_timeout(self, proxy_scheme, target_scheme, use_forwarding_for_https):
        _should_skip_https_in_https(proxy_scheme, target_scheme, use_forwarding_for_https)
