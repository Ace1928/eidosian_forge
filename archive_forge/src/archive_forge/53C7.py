...
async def resize_images_parallel(
    ...
    """
    Asynchronously resizes a list of images to a specified maximum size in parallel.
    ...
    async def process_image(image_data: bytes) -> Optional[Image.Image]:
        try:
            ...
            LoggingManager.error(
                f"Error processing image during resizing: {e}", exc_info=True
            )
            return None
    ...
    try:
        ...
        tasks = [
            asyncio.to_thread(process_image, image_data)
            for image_data in image_data_list
        ]
        resized_images = await asyncio.gather(*tasks, return_exceptions=True)
        valid_images = [image for image in resized_images if image is not None]
        ...
        LoggingManager.error(
            f"Unhandled exception in resize_images_parallel: {e}", exc_info=True
        )
        return []
...
class TestImageProcessing(unittest.TestCase):
    ...
    def test_resize_images_parallel(self):
        try:
            ...
            LoggingManager.error(f"Error executing test_resize_images_parallel: {e}")
