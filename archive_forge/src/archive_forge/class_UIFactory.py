...
class UIFactory:
    ...
    Attributes:
      suppressed_warnings: Identifiers for user warnings that should
                           no be emitted.
    """
    _user_warning_templates = dict(cross_format_fetch='Doing on-the-fly conversion from %(from_format)s to %(to_format)s.\nThis may take some time. Upgrade the repositories to the same format for better performance.', experimental_format_fetch='Fetching into experimental format %(to_format)s.\nThis format may be unreliable or change in the future without an upgrade path.\n', deprecated_command="The command 'brz %(deprecated_name)s' has been deprecated in brz %(deprecated_in_version)s. Please use 'brz %(recommended_name)s' instead.", deprecated_command_option="The option '%(deprecated_name)s' to 'brz %(command)s' has been deprecated in brz %(deprecated_in_version)s. Please use '%(recommended_name)s' instead.", recommend_upgrade='%(current_format_name)s is deprecated and a better format is available.\nIt is recommended that you upgrade by running the command\n  brz upgrade %(basedir)s', locks_steal_dead='Stole dead lock %(lock_url)s %(other_holder_info)s.', not_checking_ssl_cert='Not checking SSL certificate for %(host)s.')
    ...
    def _make_output_stream_explicit(self, encoding, encoding_type):
        raise NotImplementedError("%s doesn't support make_output_stream" % self.__class__.__name__)
