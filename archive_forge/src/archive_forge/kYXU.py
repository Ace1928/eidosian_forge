...
async def remove_vocals(input_file_path: str, output_file_path: str):
...
async def main() -> None:
    """
    Asynchronously orchestrates the process of converting an MP3 file to WAV, removing vocals from the WAV file,
    extracting lyrics, calculating the duration of the vocal-removed audio, and creating subtitles based on the extracted lyrics.

    This function demonstrates an example usage of the process with hardcoded file paths for simplicity.
    In a production environment, these paths would likely be dynamically determined or passed as arguments.

    Raises:
        FileNotFoundError: If the specified MP3 file does not exist.
    """
    # Example usage with hardcoded file paths for demonstration purposes.
    mp3_file_path: Optional[str] = (
        "/home/lloyd/EVIE/scripts/INDEGO_project_development/media/testing_song_1.mp3"
    )

    # Validate the existence of the MP3 file before proceeding.
    if not os.path.exists(mp3_file_path):
        raise FileNotFoundError(
            f"The specified MP3 file was not found: {mp3_file_path}"
        )

    # Define the paths for the WAV file, the vocal-removed WAV file, and the subtitles file.
    wav_file_path: str = (
        "/home/lloyd/EVIE/scripts/INDEGO_project_development/media/testing_song_1.wav"
    )
    vocal_removed_file_path: str = (
    ...
    if mp3_file_path and wav_file_path is not None:
        await convert_mp3_to_wav(mp3_file_path, wav_file_path)
    else 
        print("No mp3 file path provided")
    # Remove vocals from the WAV file.
    if wav_file_path and vocal_removed_file_path is not None:
        await remove_vocals(wav_file_path, vocal_removed_file_path)
    else
        print("No wav file path provided")
    # Extract lyrics from the original WAV file.
    if wav_file_path is not None:
        lyrics: str = await extract_lyrics(wav_file_path)
    else
        print("No wav file path provided")

    # Calculate the duration of the vocal-removed audio file.
    if vocal_removed_file_path is not None:
        duration: float = await get_audio_duration(vocal_removed_file_path)
    else
        print("No vocal removed file path provided")

    # Create subtitles based on the extracted lyrics and the duration of the audio.
    if lyrics is not None:
        subtitles: str = await create_subtitles(lyrics, duration)
    else
