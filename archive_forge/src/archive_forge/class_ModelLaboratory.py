    ...
    def __init__(self, chains: Sequence[Chain], names: Optional[List[str]]=None):
        ...
        for chain in chains:
            if not isinstance(chain, Chain):
                raise ValueError('ModelLaboratory should now be initialized with Chains. If you want to initialize with LLMs, use the `from_llms` method instead (`ModelLaboratory.from_llms(...)`)')
            if len(chain.input_keys) != 1:
                raise ValueError(f'Currently only support chains with one input variable, got {chain.input_keys}')
            if len(chain.output_keys) != 1:
                raise ValueError(f'Currently only support chains with one output variable, got {chain.output_keys}')
