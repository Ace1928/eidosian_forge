        ...
        if self.restarting:
            ...
            if trainer.num_training_batches != float('inf'):
                ...
                if self.global_step % expected_steps != 0 and (not is_resumable_loader):
                    rank_zero_warn("You're resuming from a checkpoint that ended before the epoch ended and your dataloader is not resumable. This can cause unreliable results if further training is done. Consider using an end-of-epoch checkpoint or make your dataloader resumable by implementing the `state_dict` / `load_state_dict` interface.", category=PossibleUserWarning)
        else:
