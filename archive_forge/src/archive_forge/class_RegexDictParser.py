...
class RegexDictParser(BaseOutputParser):
    ...
    def parse(self, text: str) -> Dict[str, str]:
        ...
        for output_key, expected_format in self.output_key_to_format.items():
            ...
            if not matches:
                raise ValueError(f'No match found for output key: {output_key} with expected format                         {expected_format} on text {text}')
            elif len(matches) > 1:
                raise ValueError(f'Multiple matches found for output key: {output_key} with                         expected format {expected_format} on text {text}')
