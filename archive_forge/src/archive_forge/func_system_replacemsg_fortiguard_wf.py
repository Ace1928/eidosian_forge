...
def system_replacemsg_fortiguard_wf(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('system.replacemsg', 'fortiguard-wf', filtered_data, vdom=vdom)
        current_data = fos.get('system.replacemsg', 'fortiguard-wf', vdom=vdom, mkey=mkey)
    ...
    if state == 'present' or state is True:
        return fos.set('system.replacemsg', 'fortiguard-wf', data=filtered_data, vdom=vdom)
    elif state == 'absent':
        return fos.delete('system.replacemsg', 'fortiguard-wf', mkey=filtered_data['msg-type'], vdom=vdom)
    else:
