...
def VerifyKubernetesFlags(args, release_track, product):
    ...
    error_msg = 'The `{flag}` flag is not supported with Cloud Run for Anthos deployed on VMware. Specify `--platform {platform}` or run `gcloud config set run/platform {platform}` to work with {platform_desc}.'
    if FlagIsExplicitlySet(args, 'allow_unauthenticated'):
        raise serverless_exceptions.ConfigurationError('The `--[no-]allow-unauthenticated` flag is not supported with Cloud Run for Anthos deployed on VMware. All deployed services allow unauthenticated requests. The `--connectivity` flag can limit which network a service is available on to reduce access.')
    ...
    if FlagIsExplicitlySet(args, 'vpc_connector'):
        raise serverless_exceptions.ConfigurationError(error_msg.format(flag='--vpc-connector', platform='managed', platform_desc=platforms.PLATFORM_SHORT_DESCRIPTIONS['managed']))
