...
class CfnTemplateBase(template_common.CommonTemplate):
    ...
    SECTIONS = VERSION, ALTERNATE_VERSION, DESCRIPTION, MAPPINGS, PARAMETERS, RESOURCES, OUTPUTS = ('AWSTemplateFormatVersion', 'HeatTemplateFormatVersion', 'Description', 'Mappings', 'Parameters', 'Resources', 'Outputs')
    ...
    _RESOURCE_KEYS = RES_TYPE, RES_PROPERTIES, RES_METADATA, RES_DEPENDS_ON, RES_DELETION_POLICY, RES_UPDATE_POLICY, RES_DESCRIPTION = ('Type', 'Properties', 'Metadata', 'DependsOn', 'DeletionPolicy', 'UpdatePolicy', 'Description')
    functions = {'Fn::FindInMap': cfn_funcs.FindInMap, 'Fn::GetAZs': cfn_funcs.GetAZs, 'Ref': cfn_funcs.Ref, 'Fn::GetAtt': cfn_funcs.GetAtt, 'Fn::Select': cfn_funcs.Select, 'Fn::Join': cfn_funcs.Join, 'Fn::Base64': cfn_funcs.Base64}
    ...
    HOT_TO_CFN_RES_ATTRS = {'type': RES_TYPE, 'properties': RES_PROPERTIES, 'metadata': RES_METADATA, 'depends_on': RES_DEPENDS_ON, 'deletion_policy': RES_DELETION_POLICY, 'update_policy': RES_UPDATE_POLICY}
    HOT_TO_CFN_OUTPUT_ATTRS = {'description': OUTPUT_DESCRIPTION, 'value': OUTPUT_VALUE}
    ...
    def param_schemata(self, param_defaults=None):
        ...
        return dict(((name, parameters.Schema.from_dict(name, schema)) for name, schema in params.items()))

    def get_section_name(self, section):
        return section

    def parameters(self, stack_identifier, user_params, param_defaults=None):
        return cfn_params.CfnParameters(stack_identifier, self, user_params=user_params, param_defaults=param_defaults)
    ...
    def add_resource(self, definition, name=None):
        ...
        cfn_tmpl = dict(((self.HOT_TO_CFN_RES_ATTRS[k], v) for k, v in hot_tmpl.items()))
        ...
        if len(dep_list) == 1:
            ...
            cfn_tmpl[self.RES_DEPENDS_ON] = [d for d in dep_list if d in self.t[self.RESOURCES]]
        self.t[self.RESOURCES][name] = cfn_tmpl

    def add_output(self, definition):
        hot_op = definition.render_hot()
        cfn_op = dict(((self.HOT_TO_CFN_OUTPUT_ATTRS[k], v) for k, v in hot_op.items()))
