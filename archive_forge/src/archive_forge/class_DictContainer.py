...
class DictContainer(IHomogeneousContainer, collections.abc.MutableMapping):
    ...
    def __init__(self, *args, **kwds):
        ...
        if len(args) > 0:
            if len(args) > 1:
                raise TypeError('%s expected at most 1 arguments, got %s' % (self.__class__.__name__, len(args)))
            self.update(args[0])
        if len(kwds):
            self.update(**kwds)
        ...
        if item.ctype is self.ctype:
            if item._parent is None:
                if key in self._data:
                    logger.warning('Implicitly replacing the entry %s (type=%s) with a new object (type=%s). This is usually indicative of a modeling error. To avoid this warning, delete the original object from the container before assigning a new object.' % (self[key].name, self[key].__class__.__name__, item.__class__.__name__))
                    self._data[key]._clear_parent_and_storage_key()
                self._fast_insert(key, item)
                return
            elif key in self._data and self._data[key] is item:
                return
            raise ValueError("Invalid assignment to %s type with name '%s' at key %s. A parent container has already been assigned to the object being inserted: %s" % (self.__class__.__name__, self.name, key, item.parent.name))
        else:
            raise TypeError('Invalid assignment to type %s with index %s. The object being inserted has the wrong category type: %s' % (self.__class__.__name__, key, item.ctype))
    ...
    def __eq__(self, other):
        if not isinstance(other, collections.abc.Mapping):
            return False
        return {key: (type(val), id(val)) for key, val in self.items()} == {key: (type(val), id(val)) for key, val in other.items()}
