...
class Ntp(ConfigBase):
    ...
    def execute_module(self):
        ...
        if self._module.check_mode:
            ...
            new_config = get_new_config(commands, existing_ntp_facts, TEST_KEYS_formatted_diff)
            result['after(generated)'] = new_config
        if self._module._diff:
            result['diff'] = get_formatted_config_diff(existing_ntp_facts, new_config, self._module._verbosity)
    ...
    def _state_deleted(self, want, have):
        ...
        """
        diff = get_diff(want, have, TEST_KEYS)
        want_none = {'enable_ntp_auth': None, 'ntp_keys': None, 'servers': None, 'source_interfaces': [], 'trusted_keys': None, 'vrf': None}
