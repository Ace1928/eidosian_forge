...
On Sun, May 5, 2024 at 5:05 PM Lloyd Handyside <ace1928@gmail.com> wrote:
import pygame as pg
import sys
...
logging.basicConfig(filename='snake_game_log.txt', level=logging.DEBUG, format='%(asctime)s:%(levelname)s:%(message)s')
...
def heuristic(
    ...
    if last_dir:
        ...
        if current_dir == last_dir:
            direction_penalty = 5 * (1 - len(self.snake) / (GRID_SIZE**2))

    # Boundary Awareness: Dynamically adjust penalty based on snake's proximity to boundaries
    boundary_threshold = max(2, int(0.1 * GRID_SIZE))
    boundary_penalty = 0
    if (
        ...
        boundary_penalty = 10 * (1 - len(self.snake) / (GRID_SIZE**2))
        boundary_penalty *= (
            1
            - min(a[0], a[1], GRID_SIZE - a[0] - 1, GRID_SIZE - a[1] - 1)
            / boundary_threshold
        )
    ...
    for dx, dy in [
        ...
        if neighbor in self.obstacles:
            obstacle_penalty += 5 * (1 - len(self.snake) / (GRID_SIZE**2))
    ...
    escape_route_bonus = available_neighbors * -2 * (len(self.snake) / (GRID_SIZE**2))
    ...
    dense_packing_bonus *= len(self.snake) / (GRID_SIZE**2)
    ...
    if is_return_path:
        ...
        return_path_bonus = -tail_distance * (1 - len(self.snake) / (GRID_SIZE**2))
    ...
    if not is_return_path:
        ...
        food_seeking_bonus = -food_distance * (1 - len(self.snake) / (GRID_SIZE**2))

    # Real-time Adaptation: Dynamically adjust heuristic weights based on game state
    snake_length_ratio = len(self.snake) / (GRID_SIZE**2)
    ...
    if not self._is_explored(a):
        exploration_bonus = 10 * (1 - len(self.snake) / (GRID_SIZE**2))
...
class Snake:
    def __init__(self):
        ...
        self.fruit = Fruit()
        self.path: List[Tuple[int, int]] = a_star(self.body[0], self.fruit.position, list(self.body))
