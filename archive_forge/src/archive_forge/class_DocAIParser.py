...
class DocAIParser(BaseBlobParser):
    ...
    def __init__(self, *, client: Optional['DocumentProcessorServiceClient']=None, location: Optional[str]=None, gcs_output_path: Optional[str]=None, processor_name: Optional[str]=None):
        ...
        if processor_name and (not re.fullmatch(pattern, processor_name)):
            raise ValueError(f'Processor name {processor_name} has the wrong format. If your prediction endpoint looks like https://us-documentai.googleapis.com/v1/projects/PROJECT_ID/locations/us/processors/PROCESSOR_ID:process, use only projects/PROJECT_ID/locations/us/processors/PROCESSOR_ID part.')
        ...
        if client:
            ...
            self._client = DocumentProcessorServiceClient(client_options=options, client_info=get_client_info(module='document-ai'))
