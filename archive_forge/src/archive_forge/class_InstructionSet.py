    ...
    def inverse(self, annotated: bool=False):
        ...
        for i, instruction in enumerate(self._instructions):
            if isinstance(instruction, CircuitInstruction):
                self._instructions[i] = instruction.replace(operation=instruction.operation.inverse(annotated=annotated))
            else:
                data, idx = instruction
                instruction = data[idx]
                data[idx] = instruction.replace(operation=instruction.operation.inverse(annotated=annotated))
        return self
        ...
        for instruction in self._instructions:
            if isinstance(instruction, CircuitInstruction):
                ...
                if updated is not instruction.operation:
                    raise CircuitError('SingletonGate instances can only be added to InstructionSet via _add_ref')
            else:
                data, idx = instruction
                instruction = data[idx]
                data[idx] = instruction.replace(operation=instruction.operation.c_if(classical, val))
        return self
