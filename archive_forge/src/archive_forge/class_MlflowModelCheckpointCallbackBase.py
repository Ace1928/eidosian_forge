...
class MlflowModelCheckpointCallbackBase(metaclass=ExceptionSafeAbstractClass):
    ...
    def __init__(self, checkpoint_file_suffix, monitor, mode, save_best_only, save_weights_only, save_freq):
        ...
        if self.save_best_only:
            if self.monitor is None:
                raise MlflowException("If checkpoint 'save_best_only' config is set to True, you need to set 'monitor' config as well.")
            if self.mode not in ['min', 'max']:
                raise MlflowException(f"If checkpoint 'save_best_only' config is set to True, you need to set 'mode' config and available modes includes 'min' and 'max', but you set 'mode' to '{self.mode}'.")
