    ...
    def __init__(self, name: str, run: Union[str, Callable, Type], *, stop: Optional[Union[Mapping, Stopper, Callable[[str, Mapping], bool]]]=None, time_budget_s: Optional[Union[int, float, datetime.timedelta]]=None, config: Optional[Dict[str, Any]]=None, resources_per_trial: Union[None, Mapping[str, Union[float, int, Mapping]], 'PlacementGroupFactory']=None, num_samples: int=1, storage_path: Optional[str]=None, storage_filesystem: Optional['pyarrow.fs.FileSystem']=None, sync_config: Optional[Union[SyncConfig, dict]]=None, checkpoint_config: Optional[Union[CheckpointConfig, dict]]=None, trial_name_creator: Optional[Callable[['Trial'], str]]=None, trial_dirname_creator: Optional[Callable[['Trial'], str]]=None, log_to_file: bool=False, export_formats: Optional[Sequence]=None, max_failures: int=0, restore: Optional[str]=None, local_dir: Optional[str]=None):
        ...
        if is_function_trainable(run):
            if checkpoint_config.checkpoint_at_end:
                raise ValueError("'checkpoint_at_end' cannot be used with a function trainable. You should include one last call to `ray.train.report(metrics=..., checkpoint=...)` at the end of your training loop to get this behavior.")
            if checkpoint_config.checkpoint_frequency:
                raise ValueError("'checkpoint_frequency' cannot be set for a function trainable. You will need to report a checkpoint every `checkpoint_frequency` iterations within your training loop using `ray.train.report(metrics=..., checkpoint=...)` to get this behavior.")
        try:
            self._run_identifier = Experiment.register_if_needed(run)
        except RpcError as e:
            if e.rpc_code == ray._raylet.GRPC_STATUS_CODE_RESOURCE_EXHAUSTED:
                raise TuneError(f'The Trainable/training function is too large for grpc resource limit. Check that its definition is not implicitly capturing a large array or other object in scope. Tip: use tune.with_parameters() to put large objects in the Ray object store. \nOriginal exception: {traceback.format_exc()}')
            else:
                raise e
        if not name:
            name = StorageContext.get_experiment_dir_name(run)
        self.storage = self._storage_context_cls(storage_path=storage_path, storage_filesystem=storage_filesystem, sync_config=sync_config, experiment_dir_name=name)
        ...
        if not stop:
            ...
            if bad_stoppers:
                ...
                raise ValueError(f'If you pass a list as the `stop` argument to `train.RunConfig()`, each element must be an instance of `tune.stopper.Stopper`. Got {stopper_types}.')
            ...
            if FunctionStopper.is_valid_function(stop):
                ...
                raise ValueError(f'Provided stop object must be either a dict, a function, or a subclass of `ray.tune.Stopper`. Got {type(stop)}.')
        else:
            raise ValueError(f'Invalid stop criteria: {stop}. Must be a callable or dict. Got {type(stop)}.')
        if time_budget_s:
            if self._stopper:
                self._stopper = CombinedStopper(self._stopper, TimeoutStopper(time_budget_s))
            else:
                self._stopper = TimeoutStopper(time_budget_s)
        stdout_file, stderr_file = _validate_log_to_file(log_to_file)
        spec = {'run': self._run_identifier, 'stop': stopping_criteria, 'time_budget_s': time_budget_s, 'config': config, 'resources_per_trial': resources_per_trial, 'num_samples': num_samples, 'checkpoint_config': checkpoint_config, 'trial_name_creator': trial_name_creator, 'trial_dirname_creator': trial_dirname_creator, 'log_to_file': (stdout_file, stderr_file), 'export_formats': export_formats or [], 'max_failures': max_failures, 'restore': Path(restore).expanduser().absolute().as_posix() if restore else None, 'storage': self.storage}
        self.spec = spec
