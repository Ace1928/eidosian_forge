...
class NetAppOntapLUNMap:
    ...
    def __init__(self):
        ...
        self.argument_spec.update(dict(state=dict(required=False, type='str', choices=['present', 'absent'], default='present'), initiator_group_name=dict(required=True, type='str'), path=dict(required=True, type='str'), vserver=dict(required=True, type='str'), lun_id=dict(required=False, type='str', default=None)))
    ...
    def get_lun_rest(self):
        api = 'storage/luns'
        params = {'name': self.parameters['path'], 'svm.name': self.parameters['vserver'], 'fields': 'name,os_type,serial_number,status.state,space.size,uuid,lun_maps'}
        record, error = rest_generic.get_one_record(self.rest_api, api, params)
        if error:
            self.module.fail_json(msg='Error getting lun %s: %s' % (self.parameters['path'], error))
        if record:
            return {'lun_ostype': self.na_helper.safe_get(record, ['os_type']), 'lun_serial': self.na_helper.safe_get(record, ['serial_number']), 'lun_naa_id': self.return_naa_id(self.na_helper.safe_get(record, ['serial_number'])), 'lun_state': self.na_helper.safe_get(record, ['status', 'state']), 'lun_size': self.na_helper.safe_get(record, ['space', 'size'])}
        return None
