...
def doctor(mask_envs=False):
    ...
    .. code-block:: text
        ...
        MLflow dependencies:
          ...
          sqlparse: 0.4.3
    """
    items = [('System information', ' '.join((platform.system(), platform.version()))), ('Python version', platform.python_version()), ('MLflow version', mlflow.__version__), ('MLflow module location', mlflow.__file__), ('Tracking URI', mlflow.get_tracking_uri()), ('Registry URI', mlflow.get_registry_uri())]
    if (runtime := get_databricks_runtime()) is not None:
        items.append(('Databricks runtime version', runtime))
    active_run = mlflow.active_run()
    if active_run:
        items.extend([('Active experiment ID', active_run.info.experiment_id), ('Active run ID', active_run.info.run_id), ('Active run artifact URI', active_run.info.artifact_uri)])
    mlflow_envs = {k: '***' if mask_envs else v for k, v in os.environ.items() if k.startswith('MLFLOW_')}
    if mlflow_envs:
        items.append(('MLflow environment variables', yaml.dump({'_': mlflow_envs}, indent=2).replace("'", '').lstrip('_:').rstrip('\n')))
    mlflow_dependencies = {}
    ...
    items.append(('MLflow dependencies', yaml.dump({'_': mlflow_dependencies}, indent=2).replace("'", '').lstrip('_:').rstrip('\n')))
