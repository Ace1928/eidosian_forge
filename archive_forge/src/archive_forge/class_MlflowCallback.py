...
class MlflowCallback(keras.callbacks.Callback, metaclass=ExceptionSafeClass):
    ...
    def __init__(self, log_every_epoch=True, log_every_n_steps=None):
        ...
        if log_every_epoch and log_every_n_steps is not None:
            raise ValueError(f'`log_every_n_steps` must be None if `log_every_epoch=True`, received `log_every_epoch={log_every_epoch}` and `log_every_n_steps={log_every_n_steps}`.')
        if not log_every_epoch and log_every_n_steps is None:
            raise ValueError('`log_every_n_steps` must be specified if `log_every_epoch=False`, received`log_every_n_steps=False` and `log_every_n_steps=None`.')
