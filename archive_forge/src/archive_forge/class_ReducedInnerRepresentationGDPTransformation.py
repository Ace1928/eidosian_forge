...
@TransformationFactory.register('contrib.piecewise.reduced_inner_repn_gdp', doc='Convert piecewise-linear model to a GDP using an inner representation of the simplices that are the domains of the linear functions.')
class ReducedInnerRepresentationGDPTransformation(PiecewiseLinearToGDP):
    ...
    def _transform_pw_linear_expr(self, pw_expr, pw_linear_func, transformation_block):
        ...
        transBlock.convex_combo = Constraint(expr=sum((transBlock.lambdas[i] for i in range(num_extreme_pts))) == 1)
        transBlock.linear_func = Constraint(expr=sum((linear_func_by_extreme_pt[j](*pt) * transBlock.lambdas[j] for j, pt in enumerate(pw_linear_func._points))) == substitute_var)
