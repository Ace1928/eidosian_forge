...
class DesignateRecordSetTest(common.HeatTestCase):
    ...
    def test_resource_handle_create(self):
        ...
        mock_resource = self._get_mock_resource()
        mock_record_create.return_value = mock_resource
        self.test_resource.properties = args = dict(name='test-record.com', description='Test record', ttl=3600, type='A', records=['1.1.1.1'], zone='1234567')
        self.test_resource.handle_create()
        args['type_'] = args.pop('type')
        mock_record_create.assert_called_with(**args)
    ...
    def test_resource_handle_update(self):
        ...
        prop_diff = args = {recordset.DesignateRecordSet.DESCRIPTION: 'updated description', recordset.DesignateRecordSet.TTL: 4200, recordset.DesignateRecordSet.TYPE: 'B', recordset.DesignateRecordSet.RECORDS: ['2.2.2.2']}
        self.test_resource.handle_update(json_snippet=None, tmpl_diff=None, prop_diff=prop_diff)
        args['type_'] = args.pop('type')
        mock_record_update.assert_called_with(zone='1234567', recordset='477e8273-60a7-4c41-b683-fdb0bc7cd151', values=args)
    ...
    def test_resource_handle_delete(self):
        ...
        self.assertEqual(self.test_resource.resource_id, self.test_resource.handle_delete())
        mock_record_delete.assert_called_once_with(zone='1234567', recordset=self.test_resource.resource_id)
    ...
    def test_resource_show_resource(self):
        args = dict(name='test-record.com', description='Test record', ttl=3600, type='A', records=['1.1.1.1'])
        mock_get = self.test_client.recordsets.get
        mock_get.return_value = args
        self.assertEqual(args, self.test_resource._show_resource(), 'Failed to show resource')