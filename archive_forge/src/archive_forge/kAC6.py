...
class Food:
    def __init__(self, position=None):
        ...
        """
        self.position = (
            position if position is not None else self.generate_random_position()
        )
...
class ThetaStar:
    ...
    async def find_path(self, start, end):
        ...
        while not open_set.empty():
            ...
            if current == end:
                ...
                logging.info(
                    f"Successfully computed shortest path from {start} to {end}: {' -> '.join(map(str, path))}"
                )
                return path
        ...
        logging.warning(
            f"Pathfinding from {start} to {end} using Theta* concluded with no path found."
        )
        return []
    ...
    def get_neighbors(self, position):
        ...
        return [
            ...
            if 0 <= neighbor[0] < self.grid.width
            and 0 <= neighbor[1] < self.grid.height
            and not self.grid.is_obstacle(neighbor)
        ]
...
class GameManager:
    def __init__(self, snake=Snake(), grid=Grid(WIDTH, HEIGHT), food=Food(None)):
        ...
        self.pathfinderCDP = ConstrainedDelaunayPathfinder(
            Grid.get_points(self.grid), Grid.get_obstacles(self.grid)
        )
        self.pathfinderAHP = AmoebaHamiltonianPathfinder(
            self.snake, self.grid, self.food
        )
        self.score = 0
        self.high_score = 0
        self.moves = 0
