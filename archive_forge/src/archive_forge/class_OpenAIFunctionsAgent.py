...
class OpenAIFunctionsAgent(BaseSingleActionAgent):
    ...
    def plan(self, intermediate_steps: List[Tuple[AgentAction, str]], callbacks: Callbacks=None, with_functions: bool=True, **kwargs: Any) -> Union[AgentAction, AgentFinish]:
    ...
    def return_stopped_response(self, early_stopping_method: str, intermediate_steps: List[Tuple[AgentAction, str]], **kwargs: Any) -> AgentFinish:
        ...
        if early_stopping_method == 'force':
            ...
            raise ValueError(f'early_stopping_method should be one of `force` or `generate`, got {early_stopping_method}')

    @classmethod
    def create_prompt(cls, system_message: Optional[SystemMessage]=SystemMessage(content='You are a helpful AI assistant.'), extra_prompt_messages: Optional[List[BaseMessagePromptTemplate]]=None) -> ChatPromptTemplate:
        ...
        if system_message:
            ...
            messages = []
        messages.extend([*_prompts, HumanMessagePromptTemplate.from_template('{input}'), MessagesPlaceholder(variable_name='agent_scratchpad')])
    ...
    def from_llm_and_tools(cls, llm: BaseLanguageModel, tools: Sequence[BaseTool], callback_manager: Optional[BaseCallbackManager]=None, extra_prompt_messages: Optional[List[BaseMessagePromptTemplate]]=None, system_message: Optional[SystemMessage]=SystemMessage(content='You are a helpful AI assistant.'), **kwargs: Any) -> BaseSingleActionAgent:
        ...
        return cls(llm=llm, prompt=prompt, tools=tools, callback_manager=callback_manager, **kwargs)