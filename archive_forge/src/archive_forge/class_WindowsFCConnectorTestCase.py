...
class WindowsFCConnectorTestCase(test_base.WindowsConnectorTestBase):
    ...
    def setUp(self):
        ...
        self._connector = fc.WindowsFCConnector(device_scan_interval=mock.sentinel.rescan_interval)
    ...
    def test_connect_volume(self, mock_get_vol_paths, mock_get_scsi_wwn):
        ...
        expected_device_info = dict(type='block', path=mock.sentinel.dev_name, number=mock.sentinel.dev_num, scsi_wwn=mock_get_scsi_wwn.return_value)
    ...
    def test_get_volume_paths(self, mock_get_disk_paths_by_scsi_id, mock_get_fc_mappings, mock_sleep, volume_mappings, expected_paths, scsi_id_side_eff=None, use_multipath=False, is_mpio_disk=False):
        mock_get_dev_num = self._diskutils.get_device_number_from_device_name
        ...
        expected_try_count = 1 if expected_paths else self._connector.device_scan_attempts
        self._diskutils.rescan_disks.assert_has_calls([mock.call()] * expected_try_count)
        mock_get_fc_mappings.assert_has_calls([mock.call(mock.sentinel.conn_props)] * expected_try_count)
        mock_sleep.assert_has_calls([mock.call(mock.sentinel.rescan_interval)] * (expected_try_count - 1))
        dev_names = [mapping['device_name'] for mapping in volume_mappings if mapping['device_name']]
        if volume_mappings and (not dev_names):
            mock_get_disk_paths_by_scsi_id.assert_any_call(mock.sentinel.conn_props, volume_mappings[0]['fcp_lun'])
        if expected_paths and use_multipath:
            mock_get_dev_num.assert_called_once_with(expected_paths[0])
            self._diskutils.is_mpio_disk.assert_any_call(mock_get_dev_num.return_value)
