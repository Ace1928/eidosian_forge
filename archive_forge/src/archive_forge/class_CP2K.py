...
class CP2K(Calculator):
    ...
    command = None
    default_parameters = dict(auto_write=False, basis_set='DZVP-MOLOPT-SR-GTH', basis_set_file='BASIS_MOLOPT', charge=0, cutoff=400 * Rydberg, force_eval_method='Quickstep', inp='', max_scf=50, potential_file='POTENTIAL', pseudo_potential='auto', stress_tensor=True, uks=False, poisson_solver='auto', xc='LDA', print_level='LOW')

    def __init__(self, restart=None, ignore_bad_restart_file=Calculator._deprecated, label='cp2k', atoms=None, command=None, debug=False, **kwargs):
        ...
        if command is not None:
            ...
            self.command = 'cp2k_shell'
        Calculator.__init__(self, restart=restart, ignore_bad_restart_file=ignore_bad_restart_file, label=label, atoms=atoms, **kwargs)
    ...
    def set(self, **kwargs):
        ...
        msg = '"%s" is not a known keyword for the CP2K calculator. To access all features of CP2K by means of an input template, consider using the "inp" keyword instead.'
        for key in kwargs:
