...
import pandas as pd
def load_tsv(filename, cell_axis='row', delimiter='\t', gene_names=True, cell_names=True, sparse=False, **kwargs):
    """Load a tsv file.
    ...
    **kwargs : optional arguments for `pd.read_csv`.
    ...
    """
    return load_csv(filename, cell_axis=cell_axis, delimiter=delimiter, gene_names=gene_names, cell_names=cell_names, sparse=sparse, **kwargs)