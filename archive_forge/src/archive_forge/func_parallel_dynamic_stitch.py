...
def parallel_dynamic_stitch(indices: List[_atypes.TensorFuzzingAnnotation[_atypes.Int32]], data: List[_atypes.TensorFuzzingAnnotation[TV_ParallelDynamicStitch_T]], name=None) -> _atypes.TensorFuzzingAnnotation[TV_ParallelDynamicStitch_T]:
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'ParallelDynamicStitch', name, indices, data)
            return _result
            ...
            pass
        try:
            return parallel_dynamic_stitch_eager_fallback(indices, data, name=name, ctx=_ctx)
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('ParallelDynamicStitch', indices=indices, data=data, name=name)
    _result = _outputs[:]
