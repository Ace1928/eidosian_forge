...
class DuckDaskExecutionEngine(DuckExecutionEngine):
    ...
    def __init__(self, conf: Any=None, connection: Optional[DuckDBPyConnection]=None, dask_client: Optional[Client]=None):
    ...
    def to_df(self, df: Any, schema: Any=None) -> DuckDataFrame:
        if isinstance(df, (dd.DataFrame, DaskDataFrame)):
            ...
            if all((not pa.types.is_nested(f.type) for f in ddf.schema.fields)):
                ...
                res = DuckDataFrame(duckdb.from_arrow(ddf.as_arrow(), connection=self.connection))
            if ddf.has_metadata:
    ...
    def save_df(self, df: DataFrame, path: str, format_hint: Any=None, mode: str='overwrite', partition_spec: Optional[PartitionSpec]=None, force_single: bool=False, **kwargs: Any) -> None:
        ...
        if isinstance(df, DaskDataFrame) or not partition_spec.empty:
            return self._dask_engine.save_df(self._to_dask_df(df), path=path, format_hint=format_hint, mode=mode, partition_spec=partition_spec, force_single=force_single, **kwargs)
        return super().save_df(df, path=path, format_hint=format_hint, mode=mode, partition_spec=partition_spec, force_single=force_single, **kwargs)
