    ...
    for channel in schedule.channels:
        if channel not in self._timeslots:
            if time == 0:
                ...
                self._timeslots[channel] = [(i[0] + time, i[1] + time) for i in other_timeslots[channel]]
            continue
        for idx, interval in enumerate(other_timeslots[channel]):
            if interval[0] + time >= self._timeslots[channel][-1][1]:
                self._timeslots[channel].extend([(i[0] + time, i[1] + time) for i in other_timeslots[channel][idx:]])
                break
            try:
                ...
                raise PulseError("Schedule(name='{new}') cannot be inserted into Schedule(name='{old}') at time {time} because its instruction on channel {ch} scheduled from time {t0} to {tf} overlaps with an existing instruction.".format(new=schedule.name or '', old=self.name or '', time=time, ch=channel, t0=interval[0], tf=interval[1])) from ex
