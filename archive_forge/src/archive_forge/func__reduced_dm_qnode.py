...
def _reduced_dm_qnode(self, qnode, targs, tkwargs):
    if tkwargs.get('device', False):
        raise ValueError("Cannot provide a 'device' value directly to the reduced_dm decorator when transforming a QNode.")
    if tkwargs.get('device_wires', None):
        raise ValueError("Cannot provide a 'device_wires' value directly to the reduced_dm decorator when transforming a QNode.")
