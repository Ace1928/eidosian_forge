...
class Yellowbrick(VectorStore):
    ...
    def __init__(self, embedding: Embeddings, connection_string: str, table: str) -> None:
    ...
    def create_table_if_not_exists(self) -> None:
        ...
        cursor.execute(sql.SQL('CREATE TABLE IF NOT EXISTS {} (             id UUID,             embedding_id INTEGER,             text VARCHAR(60000),             metadata VARCHAR(1024),             embedding FLOAT)').format(sql.Identifier(self._table)))
    ...
    def add_texts(self, texts: Iterable[str], metadatas: Optional[List[dict]]=None, **kwargs: Any) -> List[str]:
        ...
        for id in range(len(embeddings)):
            ...
            data_input = [(str(id), embedding_id, text, json.dumps(metadata), embedding) for id, embedding_id, text, metadata, embedding in zip(repeat(doc_uuid), range(len(embeddings[id])), repeat(texts[id]), repeat(metadatas[id]), embeddings[id])]
    ...
    @classmethod
    def from_texts(cls: Type[Yellowbrick], texts: List[str], embedding: Embeddings, metadatas: Optional[List[dict]]=None, connection_string: str='', table: str='langchain', **kwargs: Any) -> Yellowbrick:
    ...
    def similarity_search_with_score_by_vector(self, embedding: List[float], k: int=4, **kwargs: Any) -> List[Tuple[Document, float]]:
        ...
        cursor.execute(sql.SQL('CREATE TEMPORARY TABLE {} (             embedding_id INTEGER, embedding FLOAT)').format(sql.Identifier(tmp_table)))
        self._connection.commit()
        data_input = [(embedding_id, embedding) for embedding_id, embedding in zip(range(len(embedding)), embedding)]
        flattened_input = [val for sublist in data_input for val in sublist]
        insert_query = sql.SQL('INSERT INTO {t}             (embedding_id, embedding) VALUES {v}').format(t=sql.Identifier(tmp_table), v=sql.SQL(',').join([sql.SQL('(%s,%s)') for _ in range(len(embedding))]))
        cursor.execute(insert_query, flattened_input)
        self._connection.commit()
        sql_query = sql.SQL('SELECT text,             metadata,             sum(v1.embedding * v2.embedding) /             ( sqrt(sum(v1.embedding * v1.embedding)) *             sqrt(sum(v2.embedding * v2.embedding))) AS score             FROM {v1} v1 INNER JOIN {v2} v2             ON v1.embedding_id = v2.embedding_id             GROUP BY v2.id, v2.text, v2.metadata             ORDER BY score DESC             LIMIT %s').format(v1=sql.Identifier(tmp_table), v2=sql.Identifier(self._table))
