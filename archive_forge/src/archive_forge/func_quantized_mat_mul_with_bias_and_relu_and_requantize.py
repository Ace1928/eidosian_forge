...
add and relu and requantize fusion.
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'QuantizedMatMulWithBiasAndReluAndRequantize', name, a, b, bias, min_a, max_a, min_b, max_b, min_freezed_output, max_freezed_output, 'Toutput', Toutput, 'transpose_a', transpose_a, 'transpose_b', transpose_b, 'input_quant_mode', input_quant_mode)
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('QuantizedMatMulWithBiasAndReluAndRequantize', a=a, b=b, bias=bias, min_a=min_a, max_a=max_a, min_b=min_b, max_b=max_b, min_freezed_output=min_freezed_output, max_freezed_output=max_freezed_output, Toutput=Toutput, transpose_a=transpose_a, transpose_b=transpose_b, input_quant_mode=input_quant_mode, name=name)
    _result = _outputs[:]
