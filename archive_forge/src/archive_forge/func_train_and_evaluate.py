...
def train_and_evaluate(estimator, train_spec, eval_spec):
  ...
  """
    ...
    estimator_lib._estimator_api_gauge.get_cell('train_and_evaluate').set(True)
    ...
    if config.task_type == run_config_lib.TaskType.EVALUATOR and config.task_id > 0:
        raise ValueError('For distributed training, there can only be one `evaluator` task (with task id 0).  Given task id {}'.format(config.task_id))
    return executor.run()