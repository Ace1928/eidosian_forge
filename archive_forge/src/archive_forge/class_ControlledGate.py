...
class ControlledGate(raw_types.Gate):
    ...
    def __init__(self, sub_gate: 'cirq.Gate', num_controls: Optional[int]=None, control_values: Optional[Union[cv.AbstractControlValues, Sequence[Union[int, Collection[int]]]]]=None, control_qid_shape: Optional[Sequence[int]]=None) -> None:
        ...
        if num_controls is None:
            if control_values is not None:
                num_controls = control_values._num_qubits_() if isinstance(control_values, cv.AbstractControlValues) else len(control_values)
    ...
    def _decompose_(self, qubits: Tuple['cirq.Qid', ...]) -> Union[None, NotImplementedType, 'cirq.OP_TREE']:
        return self._decompose_with_context_(qubits)

    def _decompose_with_context_(self, qubits: Tuple['cirq.Qid', ...], context: Optional['cirq.DecompositionContext']=None) -> Union[None, NotImplementedType, 'cirq.OP_TREE']:
        if protocols.has_unitary(self.sub_gate) and protocols.num_qubits(self.sub_gate) == 1 and (self._qid_shape_() == (2,) * len(self._qid_shape_())) and isinstance(self.control_values, cv.ProductOfSums):
            ...
            decomposed_ops = controlled_gate_decomposition.decompose_multi_controlled_rotation(protocols.unitary(self.sub_gate), control_qubits, qubits[-1])
            return invert_ops + decomposed_ops + invert_ops
        if isinstance(self.sub_gate, common_gates.CZPowGate):
            z_sub_gate = common_gates.ZPowGate(exponent=self.sub_gate.exponent, global_shift=self.sub_gate.global_shift)
            ...
            controlled_z = z_sub_gate.controlled(num_controls=num_controls, control_values=control_values, control_qid_shape=control_qid_shape) if protocols.is_parameterized(self) else ControlledGate(z_sub_gate, num_controls=num_controls, control_values=control_values, control_qid_shape=control_qid_shape)
            if self != controlled_z:
                return protocols.decompose_once_with_qubits(controlled_z, qubits, NotImplemented, context=context)
        if isinstance(self.sub_gate, matrix_gates.MatrixGate):
            return NotImplemented
        result = protocols.decompose_once_with_qubits(self.sub_gate, qubits[self.num_controls():], NotImplemented, flatten=False, context=context)
        if result is NotImplemented:
            return NotImplemented
        return op_tree.transform_op_tree(result, lambda op: op.controlled_by(*qubits[:self.num_controls()], control_values=self.control_values))
    ...
    def on(self, *qubits: 'cirq.Qid') -> cop.ControlledOperation:
        ...
        return cop.ControlledOperation(qubits[:self.num_controls()], self.sub_gate.on(*qubits[self.num_controls():]), self.control_values)
    ...
    def __pow__(self, exponent: Any) -> 'ControlledGate':
        ...
        if new_sub_gate is NotImplemented:
            return NotImplemented
        return ControlledGate(new_sub_gate, self.num_controls(), control_values=self.control_values, control_qid_shape=self.control_qid_shape)
    ...
    def _circuit_diagram_info_(self, args: 'cirq.CircuitDiagramInfoArgs') -> 'cirq.CircuitDiagramInfo':
        sub_args = protocols.CircuitDiagramInfoArgs(known_qubit_count=args.known_qubit_count - self.num_controls() if args.known_qubit_count is not None else None, known_qubits=args.known_qubits[self.num_controls():] if args.known_qubits is not None else None, use_unicode_characters=args.use_unicode_characters, precision=args.precision, label_map=args.label_map)
        ...
        return protocols.CircuitDiagramInfo(wire_symbols=(*cv_info.wire_symbols, *sub_info.wire_symbols), exponent=sub_info.exponent)
    ...
    def __repr__(self) -> str:
        ...
        if self.control_values.is_trivial and set(self.control_qid_shape) == {2}:
            return f'cirq.ControlledGate(sub_gate={self.sub_gate!r}, num_controls={self.num_controls()!r})'
        return f'cirq.ControlledGate(sub_gate={self.sub_gate!r}, control_values={self.control_values!r},control_qid_shape={self.control_qid_shape!r})'

    def _json_dict_(self) -> Dict[str, Any]:
        return {'control_values': self.control_values, 'control_qid_shape': self.control_qid_shape, 'sub_gate': self.sub_gate}