...
def export_resize_upsample_sizes_nearest_axes_2_3() -> None:
    axes = [2, 3]
    node = onnx.helper.make_node('Resize', inputs=['X', '', '', 'sizes'], outputs=['Y'], mode='nearest', axes=axes)
    data = np.array([[[[1, 2], [3, 4]]]], dtype=np.float32)
    sizes = np.array([7, 8], dtype=np.int64)
    output = interpolate_nd(data, lambda x, _: nearest_coeffs(x), output_size=sizes, axes=axes).astype(np.float32)
    expect(node, inputs=[data, sizes], outputs=[output], name='test_resize_upsample_sizes_nearest_axes_2_3')