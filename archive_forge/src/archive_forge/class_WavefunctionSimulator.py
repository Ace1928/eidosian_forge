...
class WavefunctionSimulator:
    ...
    def __init__(self, *, gate_noise: Optional[Tuple[float, float, float]]=None, measurement_noise: Optional[Tuple[float, float, float]]=None, random_seed: Optional[int]=None, timeout: float=10.0, client_configuration: Optional[QCSClientConfiguration]=None) -> None:
        """
    ...
    def wavefunction(self, quil_program: Program, memory_map: Optional[Dict[str, List[Union[int, float]]]]=None) -> Wavefunction:
        """
    ...
    def expectation(self, prep_prog: Program, pauli_terms: Union[PauliSum, List[PauliTerm]], memory_map: Optional[Dict[str, List[Union[int, float]]]]=None) -> Union[float, np.ndarray]:
        """
    ...
    def _expectation(self, prep_prog: Program, operator_programs: Iterable[Program]) -> np.ndarray:
        if isinstance(operator_programs, Program):
            warnings.warn('You have provided a Program rather than a list of Programs. The results from expectation will be line-wise expectation values of the operator_programs.', SyntaxWarning)
        request = self._expectation_request(prep_prog=prep_prog, operator_programs=operator_programs)
        response = self._qvm_client.measure_expectation(request)
        return np.asarray(response.expectations)

    def run_and_measure(self, quil_program: Program, qubits: Optional[List[int]]=None, trials: int=1, memory_map: Optional[Dict[str, List[Union[int, float]]]]=None) -> np.ndarray:
        """
        ...
        if memory_map is not None:
            quil_program = self.augment_program_with_memory_values(quil_program, memory_map)
        request = self._run_and_measure_request(quil_program=quil_program, qubits=qubits, trials=trials)
    ...
    def _run_and_measure_request(self, *, quil_program: Program, qubits: Sequence[int], trials: int) -> RunAndMeasureProgramRequest:
        if not quil_program:
        ...
        return RunAndMeasureProgramRequest(program=quil_program.out(calibrations=False), qubits=list(qubits), trials=trials, measurement_noise=self.measurement_noise, gate_noise=self.gate_noise, seed=self.random_seed)
