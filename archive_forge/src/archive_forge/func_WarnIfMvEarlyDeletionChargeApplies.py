...
def WarnIfMvEarlyDeletionChargeApplies(src_url, src_obj_metadata, logger):
  ...
  """
    if src_url.scheme == 'gs' and src_obj_metadata and src_obj_metadata.timeCreated and src_obj_metadata.storageClass:
        object_storage_class = src_obj_metadata.storageClass.lower()
        early_deletion_cutoff_seconds = EARLY_DELETION_MINIMUM_LIFETIME.get(object_storage_class, None)
        if early_deletion_cutoff_seconds:
            minimum_delete_age = early_deletion_cutoff_seconds + ConvertDatetimeToPOSIX(src_obj_metadata.timeCreated)
            if time.time() < minimum_delete_age:
                logger.warn('Warning: moving %s object %s may incur an early deletion charge, because the original object is less than %s days old according to the local system time.', object_storage_class, src_url.url_string, early_deletion_cutoff_seconds // SECONDS_PER_DAY)