...
class RequiredOptional:
    ...
    def validate_attrs(self, *, data: Dict[str, Any], excludes: Optional[List[str]]=None) -> None:
        if excludes is None:
            excludes = []
        if self.required:
        ...
        if self.exclusive:
            ...
            if not exclusives:
                raise AttributeError(f'Must provide one of these attributes: {', '.join(self.exclusive)}')