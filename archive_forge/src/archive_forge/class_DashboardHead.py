...
class DashboardHead:
    ...
    def __init__(self, http_host: str, http_port: int, http_port_retries: int, gcs_address: str, node_ip_address: str, grpc_port: int, log_dir: str, temp_dir: str, session_dir: str, minimal: bool, serve_frontend: bool, modules_to_load: Optional[Set[str]]=None):
        """
        Args:
    ...
    async def _configure_http_server(self, modules):
        ...
        http_server = HttpServerDashboardHead(self.ip, self.http_host, self.http_port, self.http_port_retries, self.gcs_address, self.gcs_client, self.session_name, self.metrics)
    ...
    async def _gcs_check_alive(self):
        ...
        try:
            is_alive = await asyncio.wait_for(check_future, dashboard_consts.GCS_CHECK_ALIVE_RPC_TIMEOUT + 1)
        ...
        if is_alive:
            ...
            if self._gcs_rpc_error_counter > dashboard_consts.GCS_CHECK_ALIVE_MAX_COUNT_OF_RPC_ERROR:
                logger.error('Dashboard exiting because it received too many GCS RPC errors count: %s, threshold is %s.', self._gcs_rpc_error_counter, dashboard_consts.GCS_CHECK_ALIVE_MAX_COUNT_OF_RPC_ERROR)
                os._exit(-1)
    ...
    def _load_modules(self, modules_to_load: Optional[Set[str]]=None):
        ...
        if loaded_modules != modules_to_load:
            assert False, "Actual loaded modules, {}, doesn't match the requested modules to load, {}".format(loaded_modules, modules_to_load)
    ...
    async def _setup_metrics(self, gcs_aio_client):
        ...
        await gcs_aio_client.internal_kv_put('DashboardMetricsAddress'.encode(), address.encode(), True, namespace=None)
        if prometheus_client:
            try:
                logger.info('Starting dashboard metrics server on port {}'.format(DASHBOARD_METRIC_PORT))
                kwargs = {'addr': '127.0.0.1'} if self.ip == '127.0.0.1' else {}
                prometheus_client.start_http_server(port=DASHBOARD_METRIC_PORT, registry=metrics.registry, **kwargs)
            except Exception:
    ...
    async def run(self):
        ...
        dashboard_http_host = self.ip if self.http_host != ray_constants.DEFAULT_DASHBOARD_IP else http_host
        (self.gcs_client.internal_kv_put(ray_constants.DASHBOARD_ADDRESS.encode(), f'{dashboard_http_host}:{http_port}'.encode(), True, namespace=ray_constants.KV_NAMESPACE_DASHBOARD),)
        self.gcs_client.internal_kv_put(dashboard_consts.DASHBOARD_RPC_ADDRESS.encode(), f'{self.ip}:{self.grpc_port}'.encode(), True, namespace=ray_constants.KV_NAMESPACE_DASHBOARD)
        dashboard_utils.SignalManager.freeze()
        concurrent_tasks = [self._gcs_check_alive(), _async_notify(), DataOrganizer.purge(), DataOrganizer.organize()]
