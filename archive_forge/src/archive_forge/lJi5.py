...
class ImageCompressorApp(tk.Tk):
    ...
    async def async_compress_and_store_image(self) -> None:
        """
        ...
        try:
            ...
            compressed_data = await compress(
                self.original_image_data, image_format
            )  # Assuming compress is asynchronous
            ...
            self.after(0, lambda: self.compress_btn.config(state="normal"))
    ...
    async def async_decompress_and_show(self):
        try:
            ...
            decompressed_data, image_format = await decompress(decrypted_data)

            # Correctly handling the image format when opening the image
            img = Image.open(io.BytesIO(decompressed_data))
            img.format = image_format  # Explicitly setting the format

            tk_img = ImageTk.PhotoImage(img)
            self.after(0, lambda: self.decompressed_img_label.config(image=tk_img))
            self.after(0, lambda: setattr(self.decompressed_img_label, "image", tk_img))
            self.after(
                0, lambda: self.update_status("Image decompressed successfully.")
            )
        except Exception as e:
