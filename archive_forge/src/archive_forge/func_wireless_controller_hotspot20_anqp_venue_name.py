...
def wireless_controller_hotspot20_anqp_venue_name(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('wireless-controller.hotspot20', 'anqp-venue-name', filtered_data, vdom=vdom)
        current_data = fos.get('wireless-controller.hotspot20', 'anqp-venue-name', vdom=vdom, mkey=mkey)
    ...
    if state == 'present' or state is True:
        return fos.set('wireless-controller.hotspot20', 'anqp-venue-name', data=filtered_data, vdom=vdom)
    elif state == 'absent':
        return fos.delete('wireless-controller.hotspot20', 'anqp-venue-name', mkey=filtered_data['name'], vdom=vdom)
    else:
