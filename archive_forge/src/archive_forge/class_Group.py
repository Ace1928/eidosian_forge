...
class Group(HLObject, MutableMappingHDF5):
    ...
    _gcpl_crt_order.set_link_creation_order(h5p.CRT_ORDER_TRACKED | h5p.CRT_ORDER_INDEXED)
    _gcpl_crt_order.set_attr_creation_order(h5p.CRT_ORDER_TRACKED | h5p.CRT_ORDER_INDEXED)
    ...
    def __getitem__(self, name):
        ...
        if isinstance(name, h5r.Reference):
            ...
            raise TypeError('Accessing a group is done with bytes or str, not {}'.format(type(name)))
    ...
    def get(self, name, default=None, getclass=False, getlink=False):
        ...
        with phil:
            ...
            if name not in self:
                ...
                try:
                    return {h5o.TYPE_GROUP: Group, h5o.TYPE_DATASET: dataset.Dataset, h5o.TYPE_NAMED_DATATYPE: datatype.Datatype}[typecode]
                except KeyError:
                    raise TypeError('Unknown object type')
            elif getlink:
    ...
    def __setitem__(self, name, obj):
        ...
        with phil:
            ...
            if isinstance(obj, HLObject):
                ...
                self.id.links.create_external(name, fn, self._e(obj.path), lcpl=lcpl, lapl=self._lapl)
       ...
       Example:
        ...
        with phil:
            ...
            if flags:
                ...
                copypl = None
            h5o.copy(source.id, self._e(source_path), dest.id, self._e(dest_path), copypl, base.dlcpl)
    ...
    def move(self, source, dest):
        ...
        with phil:
            if source == dest:
                return
            self.id.links.move(self._e(source), self.id, self._e(dest), lapl=self._lapl, lcpl=self._lcpl)
    ...
    def __repr__(self):
        if not self:
            ...
            namestr = '"%s"' % self.name if self.name is not None else u'(anonymous)'
