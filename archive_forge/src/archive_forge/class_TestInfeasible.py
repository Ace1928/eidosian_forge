...
class TestInfeasible(unittest.TestCase):
    ...
    def build_model(self):
        ...
        m.yy = Var(bounds=(0, 1), initialize=1e-07, doc='Close to lower bound')
        m.y3 = Var(bounds=(0, 1e-07), initialize=0, doc='Bounds too close')
    ...
    def test_log_infeasible_constraints(self):
        ...
        self.assertEqual('log_infeasible_constraints() called with a logger whose effective level is higher than logging.INFO: no output will be logged regardless of constraint feasibility', LOG.getvalue().strip())
        output = StringIO()
        with LoggingIntercept(output, 'pyomo.util.infeasible', logging.INFO):
            log_infeasible_constraints(m)
        expected_output = ['CONSTR c1: 2.0 </= 1', 'CONSTR c2: 1 =/= 4.0', 'CONSTR c3: 1 </= 0.0', 'CONSTR c5: 5.0 <?= evaluation error <?= 10.0', 'CONSTR c7: evaluation error =?= 6.0', 'CONSTR c8: 3.0 </= 1 <= 6.0', 'CONSTR c9: 0.0 <= 1 </= 0.5']
