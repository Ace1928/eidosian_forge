...
def switch_controller_qos_queue_policy(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('switch-controller.qos', 'queue-policy', filtered_data, vdom=vdom)
        current_data = fos.get('switch-controller.qos', 'queue-policy', vdom=vdom, mkey=mkey)
    ...
    if state == 'present' or state is True:
        return fos.set('switch-controller.qos', 'queue-policy', data=filtered_data, vdom=vdom)
    elif state == 'absent':
        return fos.delete('switch-controller.qos', 'queue-policy', mkey=filtered_data['name'], vdom=vdom)
    else:
