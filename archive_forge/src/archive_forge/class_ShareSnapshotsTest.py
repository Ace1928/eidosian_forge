...
class ShareSnapshotsTest(utils.TestCase):
    ...
    def test_manage_snapshot(self):
        ...
        expected_body = {'share_id': share_id, 'provider_location': provider_location, 'driver_options': driver_options, 'name': name, 'description': description}
        version = api_versions.APIVersion('2.12')
        mock_microversion = mock.Mock(api_version=version)
        manager = share_snapshots.ShareSnapshotManager(api=mock_microversion)
        with mock.patch.object(manager, '_create', mock.Mock(return_value='fake')):
            result = manager.manage(share_id, provider_location, driver_options=driver_options, name=name, description=description)
            self.assertEqual(manager._create.return_value, result)
            manager._create.assert_called_once_with('/snapshots/manage', {'snapshot': expected_body}, 'snapshot')
    ...
    def test_deny_access(self):
        ...
        with mock.patch.object(manager, '_action'):
            ...
            manager._action.assert_called_once_with('deny_access', snapshot, {'access_id': access_id})
    ...
    def test_set_metadata(self):
        ...
        cs.assert_called('POST', '/snapshots/1234/metadata', {'metadata': {'k1': 'v2'}})
    ...
    def test_metadata_update_all(self, snapshot):
        ...
        cs.assert_called('PUT', '/snapshots/1234/metadata', {'metadata': {'k1': 'v1'}})