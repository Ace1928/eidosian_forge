...
class ExpandBuiltins(Transformation):
    ...
    def visit_Name(self, node):
        ...
        if isinstance(node.ctx, ast.Load) and s not in self.locals[node] and (s not in self.globals) and (s in MODULES['builtins']):
            if s == 'getattr':
                raise PythranSyntaxError('You fool! Trying a getattr?', node)
            self.update = True
            return ast.Attribute(ast.Name('builtins', ast.Load(), None, None), s, node.ctx)
        else:
            return node