...
def plot_energy(ax, energy, kind, bfmi, figsize, textsize, fill_alpha, fill_color, fill_kwargs, plot_kwargs, bw, legend, backend_kwargs, show):
    """Bokeh energy plot."""
    ...
    _colors = [prop for _, prop in zip(range(10), cycle(mpl_rcParams['axes.prop_cycle'].by_key()['color']))]
    if (fill_color[0].startswith('C') and len(fill_color[0]) == 2) and (fill_color[1].startswith('C') and len(fill_color[1]) == 2):
    ...
    if kind == 'kde':
        for alpha, color, label, value in series:
            ...
            _, glyph = plot_kde(value, bw=bw, label=label, fill_kwargs=fill_kwargs, plot_kwargs=plot_kwargs, ax=ax, legend=legend, backend='bokeh', backend_kwargs={}, show=False, return_glyph=True)
        ...
        for alpha, color, label, value in series:
            ...
            _histplot_bokeh_op(value.flatten(), values2=None, rotated=False, ax=ax, hist_kwargs=hist_kwargs, is_circular=False)
    else:
        raise ValueError(f'Plot type {kind} not recognized.')
    if bfmi:
        for idx, val in enumerate(e_bfmi(energy)):
            bfmi_info = Label(x=int(figsize[0] * dpi * 0.58), y=int(figsize[1] * dpi * 0.73) - 20 * idx, x_units='screen', y_units='screen', text=f'chain {idx:>2} BFMI = {val:.2f}', border_line_color=None, border_line_alpha=0.0, background_fill_color='white', background_fill_alpha=1.0)
