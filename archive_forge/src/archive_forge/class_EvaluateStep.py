...
class EvaluateStep(BaseStep):
    ...
    def _validate_and_apply_step_config(self):
        ...
        self.user_defined_custom_metrics = {metric.name: metric for metric in _get_custom_metrics(self.step_config, self.extended_task)}
        self.evaluation_metrics = {metric.name: metric for metric in _get_builtin_metrics(self.extended_task)}
    ...
    def _validate_validation_criteria(self):
        ...
        if undefined_metrics:
            raise MlflowException(f'Validation criteria contain undefined metrics: {sorted(undefined_metrics)}', error_code=INVALID_PARAMETER_VALUE)
    ...
    def _run(self, output_directory):
        ...
        try:
            ...
            test_df_path = get_step_output_path(recipe_root_path=self.recipe_root, step_name='split', relative_path='test.parquet')
            test_df = pd.read_parquet(test_df_path)
            validate_classification_config(self.task, self.positive_class, test_df, self.target_col)
            validation_df_path = get_step_output_path(recipe_root_path=self.recipe_root, step_name='split', relative_path='validation.parquet')
            validation_df = pd.read_parquet(validation_df_path)
            run_id_path = get_step_output_path(recipe_root_path=self.recipe_root, step_name='train', relative_path='run_id')
            run_id = Path(run_id_path).read_text()
            model_uri = get_step_output_path(recipe_root_path=self.recipe_root, step_name='train', relative_path=TrainStep.SKLEARN_MODEL_ARTIFACT_RELATIVE_PATH)
            ...
            _set_experiment_primary_metric(exp_id, f'test_{self.primary_metric}', primary_metric_greater_is_better)
            with mlflow.start_run(run_id=run_id):
                eval_metrics = {}
                for dataset_name, dataset, evaluator_config in (('validation', validation_df, {'explainability_algorithm': 'kernel', 'explainability_nsamples': 10, 'metric_prefix': _VALIDATION_METRIC_PREFIX}), ('test', test_df, {'log_model_explainability': False, 'metric_prefix': 'test_'})):
                    if self.extended_task == 'classification/binary':
                        evaluator_config['pos_label'] = self.positive_class
                    eval_result = mlflow.evaluate(model=model_uri, data=dataset, targets=self.target_col, model_type=_get_model_type_from_template(self.recipe), evaluators='default', extra_metrics=_load_custom_metrics(self.recipe_root, self.evaluation_metrics.values()), evaluator_config=evaluator_config)
                    eval_result.save(os.path.join(output_directory, f'eval_{dataset_name}'))
                    eval_metrics[dataset_name] = {transform_multiclass_metric(strip_prefix(k, evaluator_config['metric_prefix']), self.extended_task): v for k, v in eval_result.metrics.items()}
                validation_results = self._validate_model(eval_metrics, output_directory)
            card = self._build_profiles_and_card(run_id, model_uri, eval_metrics, validation_results, output_directory)
    ...
    def _build_profiles_and_card(self, run_id, model_uri, eval_metrics, validation_results, output_directory):
        """
        ...
        metric_df = get_merged_eval_metrics(eval_metrics, ordered_metric_names=[self.primary_metric, *self.user_defined_custom_metrics]).reset_index().rename(columns={'index': 'Metric'})
        ...
        card.add_tab('Model Performance (Test)', "<h3 class='section-title'>Summary Metrics</h3><b>NOTE</b>: Use evaluation metrics over test dataset with care. Fine-tuning model over the test dataset is not advised.{{ METRICS }} ").add_html('METRICS', metric_table_html)
        if validation_results is not None:

            def get_icon(validated):
                return '✅' if validated else '❌'
            result_df = pd.DataFrame(validation_results).assign(validated=lambda df: df['validated'].map(get_icon))
            criteria_html = BaseCard.render_table(result_df.style.format({'value': '{:.6g}', 'threshold': '{:.6g}'}))
            card.add_tab('Model Validation', '{{ METRIC_VALIDATION_RESULTS }}').add_html('METRIC_VALIDATION_RESULTS', "<h3 class='section-title'>Model Validation Results (Test Dataset)</h3> " + criteria_html)
        if self.recipe == 'classification/v1':
            classifiers_plot_tab = card.add_tab('Model Performance Plots', '{{ CONFUSION_MATRIX }} {{CONFUSION_MATRIX_PLOT}}' + '{{ LIFT_CURVE }} {{LIFT_CURVE_PLOT}}' + '{{ PR_CURVE }} {{PR_CURVE_PLOT}}' + '{{ ROC_CURVE }} {{ROC_CURVE_PLOT}}')
            confusion_matrix_path = os.path.join(output_directory, 'eval_validation/artifacts', f'{_VALIDATION_METRIC_PREFIX}confusion_matrix.png')
            if os.path.exists(confusion_matrix_path):
                classifiers_plot_tab.add_html('CONFUSION_MATRIX', '<h3 class="section-title">Confusion Matrix Plot</h3>')
                classifiers_plot_tab.add_image('CONFUSION_MATRIX_PLOT', confusion_matrix_path, width=400)
            lift_curve_path = os.path.join(output_directory, 'eval_validation/artifacts', f'{_VALIDATION_METRIC_PREFIX}lift_curve_plot.png')
            if os.path.exists(lift_curve_path):
                classifiers_plot_tab.add_html('LIFT_CURVE', '<h3 class="section-title">Lift Curve Plot</h3>')
                classifiers_plot_tab.add_image('LIFT_CURVE_PLOT', lift_curve_path, width=400)
            pr_curve_path = os.path.join(output_directory, 'eval_validation/artifacts', f'{_VALIDATION_METRIC_PREFIX}precision_recall_curve_plot.png')
            if os.path.exists(pr_curve_path):
                classifiers_plot_tab.add_html('PR_CURVE', '<h3 class="section-title">Precision Recall Curve Plot</h3>')
                classifiers_plot_tab.add_image('PR_CURVE_PLOT', pr_curve_path, width=400)
            roc_curve_path = os.path.join(output_directory, 'eval_validation/artifacts', f'{_VALIDATION_METRIC_PREFIX}roc_curve_plot.png')
            if os.path.exists(roc_curve_path):
                classifiers_plot_tab.add_html('ROC_CURVE', '<h3 class="section-title">ROC Curve Plot</h3>')
        ...
        try:
            ...
            _logger.warning('SHAP or matplotlib package is not installed, so shap plots will not be added.')
        ...
        run_summary_card_tab = card.add_tab('Run Summary', '{{ RUN_ID }} ' + '{{ MODEL_URI }}' + '{{ VALIDATION_STATUS }}' + '{{ EXE_DURATION }}' + '{{ LAST_UPDATE_TIME }}').add_markdown('VALIDATION_STATUS', f'**Validation status:** `{self.model_validation_status}`')
        ...
        model_url = get_databricks_run_url(tracking_uri=mlflow.get_tracking_uri(), run_id=run_id, artifact_path=f'train/{TrainStep.MODEL_ARTIFACT_RELATIVE_PATH}')
        if run_url is not None:
            run_summary_card_tab.add_html('RUN_ID', f'<b>MLflow Run ID:</b> <a href={run_url}>{run_id}</a><br><br>')
        else:
            run_summary_card_tab.add_markdown('RUN_ID', f'**MLflow Run ID:** `{run_id}`')
        if model_url is not None:
            run_summary_card_tab.add_html('MODEL_URI', f'<b>MLflow Model URI:</b> <a href={model_url}>{model_uri}</a>')
        else:
