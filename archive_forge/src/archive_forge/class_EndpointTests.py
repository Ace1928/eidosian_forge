...
class EndpointTests(utils.ClientTestCase):
    ...
    def test_create_with_optional_params_as_none(self):
        ...
        self.stub_url('POST', ['endpoints'], json=resp_body)
        endpoint_without_defaults = self.client.endpoints.create(region=req_body_without_defaults['endpoint']['region'], publicurl=req_body_without_defaults['endpoint']['publicurl'], service_id=req_body_without_defaults['endpoint']['service_id'], adminurl=None, internalurl=None)
    ...
    def test_list(self):
        ...
        [self.assertIsInstance(r, endpoints.Endpoint) for r in endpoint_list]