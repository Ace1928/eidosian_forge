...
def StreamWithFilter(self, build_ref, backoff, output_filter=None):
    ...
    while build.status in working_statuses:
        if poll_build_logs:
            try:
                ...
                log.warning('Failed to fetch cloud build logs: {}. Waiting for build to complete.'.format(encoding_util.Decode(e.content)))
    ...
    if poll_build_logs:
        try:
            ...
            log.warning('Failed to fetch cloud build logs: {}.'.format(encoding_util.Decode(e.content)))
        except apitools_exceptions.HttpError as e:
            log.warning('Failed to fetch cloud build logs: {}.'.format(encoding_util.Decode(e.content)))
    return build