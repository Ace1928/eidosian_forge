...
class WavLMSelfAttention(nn.Module):
    ...
    def __init__(self, embed_dim: int, num_heads: int, dropout: float=0.0, bias: bool=True, has_relative_attention_bias: bool=False, num_buckets: int=32, max_distance: int=128, gru_rel_pos: bool=True):
        super().__init__()
    ...
    def forward(self, query: Tensor, key_padding_mask: Optional[Tensor]=None, attention_mask: Optional[Tensor]=None, position_bias: Optional[Tensor]=None) -> Tuple[Tensor, Optional[Tensor]]:
        """
        Args:
        ...
        if position_bias is not None:
            ...
            if self.gru_rel_pos:
                ...
                gate_a, gate_b = torch.sigmoid(self.gru_rel_pos_linear(query_layer).view(bsz, self.num_heads, seq_len, 2, 4).sum(-1, keepdim=False)).chunk(2, dim=-1)
        ...
        if attn_mask_rel_pos is not None and key_padding_mask is not None:
            ...
            key_padding_mask = torch.nn.functional._canonical_mask(mask=key_padding_mask, mask_name='key_padding_mask', other_type=torch.nn.functional._none_or_dtype(attn_mask_rel_pos), other_name='', target_type=query.dtype)
        if attn_mask_rel_pos is not None and key_padding_mask is not None:
        ...
        query = query.view(shape).transpose(2, 1)
        key = key.view(shape).transpose(2, 1)
        value = value.view(shape).transpose(2, 1)
        dropout = self.dropout if self.training else 0.0
        attn_output = torch.nn.functional.scaled_dot_product_attention(query, key, value, attn_mask=attn_mask_rel_pos, dropout_p=dropout, is_causal=False)
