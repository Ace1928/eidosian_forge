...
class KernelInceptionDistance(Metric):
    ...
    def __init__(self, feature: Union[str, int, Module]=2048, subsets: int=100, subset_size: int=1000, degree: int=3, gamma: Optional[float]=None, coef: float=1.0, reset_real_features: bool=True, normalize: bool=False, **kwargs: Any) -> None:
        super().__init__(**kwargs)
        rank_zero_warn('Metric `Kernel Inception Distance` will save all extracted features in buffer. For large datasets this may lead to large memory footprint.', UserWarning)
        if isinstance(feature, (str, int)):
            if not _TORCH_FIDELITY_AVAILABLE:
                raise ModuleNotFoundError('Kernel Inception Distance metric requires that `Torch-fidelity` is installed. Either install as `pip install torchmetrics[image]` or `pip install torch-fidelity`.')
