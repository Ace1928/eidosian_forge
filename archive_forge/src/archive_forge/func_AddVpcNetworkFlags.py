...
def AddVpcNetworkFlags(parser, resource_kind='service'):
    ...
    parser.add_argument('--network', metavar='NETWORK', help='The VPC network that the Cloud Run {kind} will be able to send traffic to. If --subnet is also specified, subnet must be a subnetwork of the network specified by this --network flag. To clear existing VPC network settings, use --clear-network.'.format(kind=resource_kind))