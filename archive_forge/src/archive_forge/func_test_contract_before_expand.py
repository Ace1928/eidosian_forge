...
def test_contract_before_expand(self, mock_validate_engine, mock_get_alembic_branch_head, mock_get_current_alembic_heads):
    ...
    self.assertIn('Database contraction did not run. Database contraction cannot be run before database expansion. Run database expansion first using "glance-manage db expand"', exit.code)