...
def _vn_entropy_qnode(self, qnode, targs, tkwargs):
    if tkwargs.get('device', False):
        raise ValueError("Cannot provide a 'device' value directly to the vn_entropy decorator when transforming a QNode.")
    if tkwargs.get('device_wires', None):
        raise ValueError("Cannot provide a 'device_wires' value directly to the vn_entropy decorator when transforming a QNode.")
