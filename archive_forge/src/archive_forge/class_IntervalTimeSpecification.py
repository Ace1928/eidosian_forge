...
class IntervalTimeSpecification(TimeSpecification):
  ...
  """
    ...
    def __init__(self, interval, period, synchronized=False, start_time_string='', end_time_string='', timezone=None):
        ...
        if self.synchronized:
            if start_time_string:
                raise ValueError('start_time_string may not be specified if synchronized is true')
            if end_time_string:
                raise ValueError('end_time_string may not be specified if synchronized is true')
            if self.seconds > 86400 or 86400 % self.seconds != 0:
                raise groc.GrocException('can only use synchronized for periods that divide evenly into 24 hours')
            self.start_time = datetime.time(0, 0)
            self.end_time = datetime.time(23, 59)
        elif start_time_string:
            if not end_time_string:
                raise ValueError('end_time_string must be specified if start_time_string is')
            self.start_time = _GetTime(start_time_string)
            self.end_time = _GetTime(end_time_string)
        else:
            if end_time_string:
                raise ValueError('start_time_string must be specified if end_time_string is')
    ...
    """
        ...
        interval_time = start_time + datetime.timedelta(seconds=num_intervals * self.seconds)
        if self.timezone:
            interval_time = self.timezone.normalize(interval_time)
        next_start_time = self._GetNextDateTime(t, self.start_time, self.timezone)
        if self._TimeIsInRange(t) and self._TimeIsInRange(interval_time) and (interval_time < next_start_time):
