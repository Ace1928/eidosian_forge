...
class XsdAssert(XsdComponent, ElementPathMixin[Union['XsdAssert', SchemaElementType]]):
    ...
    def __init__(self, elem: ElementType, schema: SchemaType, parent: 'XsdComplexType', base_type: 'XsdComplexType') -> None:
    ...
    def build(self) -> None:
        self.parser = XPath2Parser(namespaces=self.namespaces, variable_types={'value': self.base_type.sequence_type}, strict=False, default_namespace=self.xpath_default_namespace, schema=self.xpath_proxy)
        try:
    ...
    def __call__(self, elem: ElementType, value: Any=None, namespaces: Optional[NamespacesType]=None, source: Optional['XMLResource']=None, **kwargs: Any) -> Iterator[XMLSchemaValidationError]:
        ...
        if source is not None:
            context = XPathContext(root=source.get_xpath_node(elem), namespaces=_namespaces, variables=variables)
        else:
