...
class OProc(object):
    ...
    def __init__(self, command, parent_log, cmd, stdin, stdout, stderr, call_args, pipe, process_assign_lock):
        ...
        if self.pid == 0:
            ...
            if self._stdin_parent_fd:
                ...
                self._stdin_stream = StreamWriter(log, self._stdin_parent_fd, self.stdin, ca['in_bufsize'], ca['encoding'], ca['tty_in'])
            stdout_pipe = None
            ...
            if not pipe_out and self._stdout_parent_fd:
                ...
                self._stdout_stream = StreamReader(self.log.get_child('streamreader', 'stdout'), self._stdout_parent_fd, stdout, self._stdout, ca['out_bufsize'], ca['encoding'], ca['decode_errors'], stdout_pipe, save_data=save_stdout)
            ...
            if stderr is not OProc.STDOUT and (not single_tty) and (not pipe_err) and self._stderr_parent_fd:
                ...
                self._stderr_stream = StreamReader(Logger('streamreader'), self._stderr_parent_fd, stderr, self._stderr, ca['err_bufsize'], ca['encoding'], ca['decode_errors'], stderr_pipe, save_data=save_stderr)
            ...
            self._output_thread = _start_daemon_thread(output_thread, thread_name, self._output_thread_exc_queue, self.log, self._stdout_stream, self._stderr_stream, self._timeout_event, self.is_alive, self._quit_threads, self._stop_output_event, output_complete)
    ...
    def is_alive(self):
        ...
        acquired = self._wait_lock.acquire(timeout=1e-05)
        if not acquired:
            if self.exit_code is not None:
                return (False, self.exit_code)
