...
class ImageCompressorApp(tk.Tk):
    ...
    async def async_decompress_and_show(self):
        try:
            [image_data, image_format] = await retrieve_compressed_image(self.img_hash)
            ...
            self.decompressed_img_label.image = (
                tk_img  # Keep a reference to avoid garbage collection
            )
