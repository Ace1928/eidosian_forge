...
class ChatHunyuan(BaseChatModel):
    ...
    def lc_secrets(self) -> Dict[str, str]:
        return {'hunyuan_app_id': 'HUNYUAN_APP_ID', 'hunyuan_secret_id': 'HUNYUAN_SECRET_ID', 'hunyuan_secret_key': 'HUNYUAN_SECRET_KEY'}
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        values['hunyuan_api_base'] = get_from_dict_or_env(values, 'hunyuan_api_base', 'HUNYUAN_API_BASE', DEFAULT_API_BASE)
        values['hunyuan_app_id'] = get_from_dict_or_env(values, 'hunyuan_app_id', 'HUNYUAN_APP_ID')
        values['hunyuan_secret_id'] = get_from_dict_or_env(values, 'hunyuan_secret_id', 'HUNYUAN_SECRET_ID')
        values['hunyuan_secret_key'] = convert_to_secret_str(get_from_dict_or_env(values, 'hunyuan_secret_key', 'HUNYUAN_SECRET_KEY'))
        return values

    @property
    def _default_params(self) -> Dict[str, Any]:
        """Get the default parameters for calling Hunyuan API."""
        normal_params = {'app_id': self.hunyuan_app_id, 'secret_id': self.hunyuan_secret_id, 'temperature': self.temperature, 'top_p': self.top_p}
    ...
    def _chat(self, messages: List[BaseMessage], **kwargs: Any) -> requests.Response:
        ...
        payload = {'timestamp': timestamp, 'expired': expired, 'messages': [_convert_message_to_dict(m) for m in messages], **parameters}
        if self.streaming:
            payload['stream'] = 1
        url = self.hunyuan_api_base + DEFAULT_PATH
        res = requests.post(url=url, timeout=self.request_timeout, headers={'Content-Type': 'application/json', 'Authorization': _signature(secret_key=self.hunyuan_secret_key, url=url, payload=payload), **headers}, json=payload, stream=self.streaming)
        return res
