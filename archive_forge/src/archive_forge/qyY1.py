class ASCIIFormatter:
    ...
    def __call__(
        ...
        # Function to generate color codes with systematic names
        def generate_color_codes() -> ColorCodesList:
            ...
            Additionally, it includes the ANSI escape character and hex code for each color.

            Returns:
                ColorCodesList: A list of tuples containing color names, their RGB values, ANSI escape codes, and hex codes.
            """
        ...
        def rgb_to_ansi(rgb: RGBValue) -> str:
            """
        ...
        # Check if the color codes file exists, load it if it does, generate and save it if it doesn't
        color_codes_file = "color_codes.txt"
        if os.path.exists(color_codes_file):
            with open(color_codes_file, "r") as file:
                color_codes: ColorCodesDict = eval(file.read())  # Load color codes into RAM
        else:
            color_codes_list: ColorCodesList = generate_color_codes()
            color_codes: ColorCodesDict = {
                name: (rgb, rgb_to_ansi(rgb), rgb_to_hex(rgb)) for name, rgb in color_codes_list
            }
            with open(color_codes_file, "w") as file:
                file.write(str(color_codes))  # Save generated color codes to file
