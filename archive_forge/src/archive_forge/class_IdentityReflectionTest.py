...
class IdentityReflectionTest(fixtures.TablesTest):
    ...
    def define_tables(cls, metadata):
        Table('t1', metadata, Column('normal', Integer), Column('id1', Integer, Identity()))
        Table('t2', metadata, Column('id2', Integer, Identity(always=True, start=2, increment=3, minvalue=-2, maxvalue=42, cycle=True, cache=4)))
        if testing.requires.schemas.enabled:
            Table('t1', metadata, Column('normal', Integer), Column('id1', Integer, Identity(always=True, start=20)), schema=config.test_schema)

    def check(self, value, exp, approx):
        if testing.requires.identity_columns_standard.enabled:
            common_keys = ('always', 'start', 'increment', 'minvalue', 'maxvalue', 'cycle', 'cache')
            for k in list(value):
