...
class DerivativeVar(Var):
    ...
    def __init__(self, sVar, **kwds):
        if not isinstance(sVar, Var):
            raise DAE_Error('%s is not a variable. Can only take the derivative of a Varcomponent.' % sVar)
        if 'wrt' in kwds and 'withrespectto' in kwds:
            raise TypeError("Cannot specify both 'wrt' and 'withrespectto keywords in a DerivativeVar")
        ...
        try:
            ...
            if sVar.dim() == 0:
                ...
                for i, s in enumerate(sidx_sets):
                    ...
                    _dim = s.dimen
                    if _dim is None:
                        raise DAE_Error('The variable %s is indexed by a Set (%s) with a non-fixed dimension.  A DerivativeVar may only be indexed by Sets with constant dimension' % (sVar, s.name))
                    elif _dim is UnknownSetDimen:
                        raise DAE_Error('The variable %s is indexed by a Set (%s) with an unknown dimension.  A DerivativeVar may only be indexed by Sets with known constant dimension' % (sVar, s.name))
                    loc += s.dimen
            num_contset = len(sVar._contset)
        if num_contset == 0:
            raise DAE_Error('The variable %s is not indexed by any ContinuousSets. A derivative may only be taken with respect to a continuous domain' % sVar)
        if wrt is None:
            if num_contset != 1:
                raise DAE_Error("The variable %s is indexed by multiple ContinuousSets. The desired ContinuousSet must be specified using the keyword argument 'wrt'" % sVar)
            wrt = [next(iter(sVar._contset.keys()))]
        elif type(wrt) is ContinuousSet:
            if wrt not in sVar._contset:
                raise DAE_Error('Invalid derivative: The variable %s is not indexed by the ContinuousSet %s' % (sVar, wrt))
            wrt = [wrt]
        elif type(wrt) is tuple or type(wrt) is list:
            for i in wrt:
                if type(i) is not ContinuousSet:
                    raise DAE_Error('Cannot take the derivative with respect to %s. Expected a ContinuousSet or a tuple of ContinuousSets' % i)
                if i not in sVar._contset:
                    raise DAE_Error('Invalid derivative: The variable %s is not indexed by the ContinuousSet %s' % (sVar, i))
            wrt = list(wrt)
        else:
            raise DAE_Error('Cannot take the derivative with respect to %s. Expected a ContinuousSet or a tuple of ContinuousSets' % i)
        ...
        if wrtkey in sVar._derivative:
            raise DAE_Error('Cannot create a new derivative variable for variable %s: derivative already defined as %s' % (sVar.name, sVar._derivative[wrtkey]().name))
