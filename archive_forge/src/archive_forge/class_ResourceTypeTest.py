...
class ResourceTypeTest(common.HeatTestCase):
    ...
    @mock.patch.object(res.Resource, 'is_service_available')
    def test_list_resource_types_supported(self, mock_is_service_available):
        mock_is_service_available.return_value = (True, None)
    ...
    def test_list_resource_types_with_descr(self, mock_is_service_available):
        mock_is_service_available.return_value = (True, None)
        resources = self.eng.list_resource_types(self.ctx, with_description=True)
        self.assertIsInstance(resources, list)
        self.assertIn({'resource_type': 'AWS::RDS::DBInstance', 'description': 'Builtin AWS::RDS::DBInstance'}, resources)
        self.assertIn({'resource_type': 'AWS::EC2::Instance', 'description': 'No description available'}, resources)

    def test_resource_schema(self):
        type_name = 'ResourceWithPropsType'
        expected = {'resource_type': type_name, 'properties': {'Foo': {'type': 'string', 'required': False, 'update_allowed': False, 'immutable': False}, 'FooInt': {'type': 'integer', 'required': False, 'update_allowed': False, 'immutable': False}}, 'attributes': {'foo': {'description': 'A generic attribute'}, 'Foo': {'description': 'Another generic attribute'}, 'show': {'description': 'Detailed information about resource.', 'type': 'map'}}, 'support_status': {'status': 'SUPPORTED', 'version': None, 'message': None, 'previous_status': None}, 'description': 'No description available'}
        schema = self.eng.resource_schema(self.ctx, type_name=type_name, with_description=True)
    ...
    def test_resource_schema_nonexist(self):
        ex = self.assertRaises(exception.EntityNotFound, self.eng.resource_schema, self.ctx, type_name='Bogus')
