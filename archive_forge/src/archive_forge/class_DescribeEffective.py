...
class DescribeEffective(base.DescribeCommand):
  ...
  """
    ...
    @staticmethod
    def Args(parser):
        flags.CreateModuleIdOrNameArg(module_type=constants.CustomModuleType.EFFECTIVE_ETD).AddToParser(parser)
        flags.CreateParentFlag(required=False).AddToParser(parser)

    def Run(self, args):
        name = parsing.GetModuleNameFromArgs(args, module_type=constants.CustomModuleType.EFFECTIVE_ETD)
