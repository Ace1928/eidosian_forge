...
class HeatTestCase(testscenarios.WithScenarios, testtools.TestCase, FakeLogMixin):

    def setUp(self, mock_keystone=True, mock_resource_policy=True, quieten_logging=True, mock_find_file=True):
        super().setUp()
        ...
        template_dir = os.path.join(project_dir, 'etc', 'heat', 'templates')
        ...
        for name in tri_names:
            tris.append(resources.global_env().get_resource_info(name, registry_type=environment.TemplateResourceInfo))
        for tri in tris:
            if tri is not None:
        ...
        if mock_find_file:
            self.mock_find_file = self.patchobject(cfg.ConfigOpts, 'find_file')
        if mock_keystone:
            self.stub_keystoneclient()
        if mock_resource_policy:
            self.mock_resource_policy = self.patchobject(policy.ResourceEnforcer, 'enforce')
    ...
    def patchobject(self, obj, attr, **kwargs):
        mockfixture = self.useFixture(fixtures.MockPatchObject(obj, attr, **kwargs))
    ...
    def stub_auth(self, ctx=None, **kwargs):
        auth = self.patchobject(ctx or context.RequestContext, '_create_auth_plugin')
    ...
    def stub_SnapshotConstraint_validate(self):
        validate = self.patchobject(cinder.VolumeSnapshotConstraint, 'validate')
        validate.return_value = True
    ...
    def stub_KeystoneProjectConstraint(self):
        validate = self.patchobject(ks_constr.KeystoneProjectConstraint, 'validate')
        validate.return_value = True
