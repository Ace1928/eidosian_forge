...
class XsdValidatorTestCase(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.default_namespaces = {'xsi': 'http://www.w3.org/2001/XMLSchema-instance', 'tns': 'http://xmlschema.test/ns', 'ns': 'ns'}
    ...
    def get_element(self, name, **attrib):
        source = '<xs:element name="{}" {}/>'.format(name, ' '.join(('%s="%s"' % (k, v) for k, v in attrib.items())))
