...
import six
@staticmethod
def _PatchStatefulInternalIPs(messages, per_instance_config, ips_to_update, ips_to_remove):
    """Patch and return the updated list of stateful internal IPs."""
    existing_ips = per_instance_config.preservedState.internalIPs.additionalProperties if per_instance_config.preservedState.internalIPs else []
    ...
    UpdateGA._VerifyStatefulIPsToRemoveSet('--remove-stateful-internal-ips', existing_ips, ips_to_remove)
    ...
    new_stateful_ips.extend(UpdateGA._CreateInternalIPs(messages, remaining_ips_to_update))
    per_instance_config.preservedState.internalIPs = messages.PreservedState.InternalIPsValue(additionalProperties=new_stateful_ips)