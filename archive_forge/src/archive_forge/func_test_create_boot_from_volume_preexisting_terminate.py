...
def test_create_boot_from_volume_preexisting_terminate(self):
    ...
    server = self.user_cloud.create_server(name=self.server_name, image=None, flavor=self.flavor, boot_volume=volume, terminate_volume=True, volume_size=1, wait=True)
