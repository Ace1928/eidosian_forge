...
class CmdStanLaplace:
    ...
    def __init__(self, runset: RunSet, mode: CmdStanMLE) -> None:
        ...
        if not runset.method == Method.LAPLACE:
            raise ValueError('Wrong runset method, expecting laplace runset, found method {}'.format(runset.method))
        self._runset = runset
        self._mode = mode
    ...
    def draws_pd(self, vars: Union[List[str], str, None]=None) -> pd.DataFrame:
        if vars is not None:
    ...
    def draws_xr(self, vars: Union[str, List[str], None]=None) -> 'xr.Dataset':
        """
        ...
        attrs: MutableMapping[Hashable, Any] = {'stan_version': f'{meta['stan_version_major']}.{meta['stan_version_minor']}.{meta['stan_version_patch']}', 'model': meta['model']}
        data: MutableMapping[Hashable, Any] = {}
        coordinates: MutableMapping[Hashable, Any] = {'draw': np.arange(self._draws.shape[0])}
        for var in vars_list:
            build_xarray_data(data, self._metadata.stan_vars[var], self._draws[:, np.newaxis, :])
    ...
    def __repr__(self) -> str:
        ...
        rep = 'CmdStanLaplace: model={} \nmode=({})\n{}'.format(self._runset.model, mode, self._runset._args.method_args.compose(0, cmd=[]))
