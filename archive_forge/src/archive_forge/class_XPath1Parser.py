...
class XPath1Parser(Parser[XPathToken]):
    ...
    literals_pattern = re.compile('\'(?:[^\']|\'\')*\'|"(?:[^"]|"")*"|(?:\\d+|\\.\\d+)(?:\\.\\d*)?(?:[Ee][+-]?\\d+)?')
    name_pattern = re.compile('[^\\d\\W][\\w.\\-\\xb7\\u0300-\\u036F\\u203F\\u2040]*')
    RESERVED_FUNCTION_NAMES = {'comment', 'element', 'node', 'processing-instruction', 'text'}
    ...
    PATH_STEP_SYMBOLS: ClassVar[Set[str]] = {'(integer)', '(string)', '(float)', '(decimal)', '(name)', '*', '@', '..', '.', '{'}
    ...
    def __init__(self, namespaces: Optional[NamespacesType]=None, strict: bool=True) -> None:
    ...
    def axis(cls, symbol: str, reverse_axis: bool=False, bp: int=80) -> Type[XPathAxis]:
        ...
        token_class = cls.register(symbol, label='axis', bases=(XPathAxis,), reverse_axis=reverse_axis, lbp=bp, rbp=bp)
        return cast(Type[XPathAxis], token_class)

    @classmethod
    def function(cls, symbol: str, prefix: Optional[str]=None, label: str='function', nargs: NargsType=None, sequence_types: Tuple[str, ...]=(), bp: int=90) -> Type[XPathFunction]:
        """
        Registers a token class for a symbol that represents an XPath function.
        """
        kwargs = {'bases': (XPathFunction,), 'label': label, 'nargs': nargs, 'lbp': bp, 'rbp': bp}
        ...
        if prefix:
            ...
            kwargs['class_name'] = '_%s%s%s' % (prefix.capitalize(), symbol.capitalize(), str(label).title().replace(' ', ''))
        ...
        if sequence_types:
            ...
            if nargs is None:
                ...
                cls.function_signatures[qname, nargs] = 'function({}) as {}'.format(', '.join(sequence_types[:-1]), sequence_types[-1])
            elif nargs[1] is None:
                assert len(sequence_types) == nargs[0] + 1
                cls.function_signatures[qname, nargs[0]] = 'function({}, ...) as {}'.format(', '.join(sequence_types[:-1]), sequence_types[-1])
            else:
                assert len(sequence_types) == nargs[1] + 1
                for arity in range(nargs[0], nargs[1] + 1):
                    cls.function_signatures[qname, arity] = 'function({}) as {}'.format(', '.join(sequence_types[:arity]), sequence_types[-1])
        return cast(Type[XPathFunction], cls.register(symbol, **kwargs))
    ...
    def check_variables(self, values: MutableMapping[str, Any]) -> None:
        ...
        for varname, value in values.items():
            if not match_sequence_type(value, 'item()*' if isinstance(value, list) else 'item()', self):
                message = 'Unmatched sequence type for variable {!r}'.format(varname)
                raise xpath_error('XPDY0050', message)

    def get_function(self, name: str, arity: Optional[int], context: ContextArgType=None) -> XPathFunction:
        """
