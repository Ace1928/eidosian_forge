...
class SOCSpillage:
    ...
    @staticmethod
    def isclose(n1, n2, rel_tol=1e-07):
    ...
    def overlap_so_spinpol(self):
        ...
        so_nkpts = len(so_k)
        nelec_list = []
        for nk1 in range(1, n_kpts_noso + 1):
            knoso = noso_kvecs[nk1 - 1, :]
            for nk2 in range(1, so_nkpts + 1):
                kso = so_kvecs[nk2 - 1, :]
                if self.isclose(kso[0], knoso[0]) and self.isclose(kso[1], knoso[1]) and self.isclose(kso[2], knoso[2]):
        ...
        for nk1 in range(1, n_kpts_noso + 1):
            ...
            for nk2 in range(1, so_nkpts + 1):
                ...
                if self.isclose(kso[0], knoso[0]) and self.isclose(kso[1], knoso[1]) and self.isclose(kso[2], knoso[2]):
                    ...
                    kpoints.append(kso)
                    Mmn = 0.0
                    vnoso = np.array(no_so.coeffs[0][nk1 - 1][0])
                    n_noso1 = vnoso.shape[0]
                    vnoso = np.array(no_so.coeffs[1][nk1 - 1][0])
        ...
        print('no spin-orbit gaps', f'{float(noso_lumo - noso_homo):+.3f}', f'{noso_direct:+.3f}', '   ', [noso_homo, noso_lumo])
        print('spin-orbit gaps   ', f'{float(so_lumo - so_homo):+.3f}', f'{so_direct:+.3f}', '   ', [so_homo, so_lumo])
