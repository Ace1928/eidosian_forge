...
def firewall_wildcard_fqdn_custom(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('firewall.wildcard-fqdn', 'custom', filtered_data, vdom=vdom)
    ...
    if state == 'present' or state is True:
        return fos.set('firewall.wildcard-fqdn', 'custom', data=filtered_data, vdom=vdom)
    elif state == 'absent':
        return fos.delete('firewall.wildcard-fqdn', 'custom', mkey=filtered_data['name'], vdom=vdom)
    else:
