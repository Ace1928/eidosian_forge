...
def setup_google_sheets(
    ...
    client = gspread.authorize(
        ServiceAccountCredentials.from_json_keyfile_name(
            ...
            [
                ...
                "/home/lloyd/EVIE/scripts/trading_bot/client_secret_1071711940804-3itcqem02rcs9ni302f0papvp3454567.apps.googleusercontent.com.json",
            ],
        )
    )
    sheet_title = f"{ticker.replace(':', '_')} Stock Data"
    sh = client.create(sheet_title)  # Create a new spreadsheet
    ...
    worksheet.update(
        "A1", finance_formula
    )  # Write the GOOGLEFINANCE function to cell A1
    return sh.url
...
def load_credentials_gui():
    def save_credentials():
        credentials = {
            ...
            "private_key_id": private_key_id.get(),
            ...
            "client_x509_cert_url": client_x509_cert_url.get(),
        }
    ...
    private_key_id = StringVar()
    private_key = StringVar()
    client_email = StringVar()
    client_id = StringVar()
    client_x509_cert_url = StringVar()

    Label(root, text="Project ID").pack()
    Entry(root, textvariable=project_id).pack()
    Label(root, text="Private Key ID").pack()
    Entry(root, textvariable=private_key_id).pack()
    Label(root, text="Private Key").pack()
    Entry(root, textvariable=private_key).pack()
    Label(root, text="Client Email").pack()
    Entry(root, textvariable=client_email).pack()
    Label(root, text="Client ID").pack()
    Entry(root, textvariable=client_id).pack()
    Label(root, text="Client X509 Cert URL").pack()
    Entry(root, textvariable=client_x509_cert_url).pack()

    Button(root, text="Save Credentials", command=save_credentials).pack()
    root.mainloop()
