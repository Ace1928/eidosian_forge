...
class Ipopt(PersistentSolver):
    ...
    def version(self):
        results = subprocess.run([str(self.config.executable), '--version'], timeout=1, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, universal_newlines=True)
    ...
    def _apply_solver(self, timer: HierarchicalTimer):
        ...
        cmd = [str(config.executable), self._filename + '.nl', '-AMPL', 'option_file_name=' + self._filename + '.opt']
        if 'option_file_name' in self.ipopt_options:
            raise ValueError('Use Ipopt.config.filename to specify the name of the options file. Do not use Ipopt.ipopt_options["option_file_name"].')
        ...
        with TeeStream(*ostreams) as t:
            ...
            cp = subprocess.run(cmd, timeout=timeout, stdout=t.STDOUT, stderr=t.STDERR, env=env, universal_newlines=True)
