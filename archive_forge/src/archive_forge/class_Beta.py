  ...
  * `Gamma` is the [gamma function](
  ...
  """
    ...
    def _param_shapes(sample_shape):
        return dict(zip(['concentration1', 'concentration0'], [ops.convert_to_tensor(sample_shape, dtype=dtypes.int32)] * 2))
    ...
    def _sample_n(self, n, seed=None):
        expanded_concentration1 = array_ops.ones_like(self.total_concentration, dtype=self.dtype) * self.concentration1
        expanded_concentration0 = array_ops.ones_like(self.total_concentration, dtype=self.dtype) * self.concentration0
        gamma1_sample = random_ops.random_gamma(shape=[n], alpha=expanded_concentration1, dtype=self.dtype, seed=seed)
        gamma2_sample = random_ops.random_gamma(shape=[n], alpha=expanded_concentration0, dtype=self.dtype, seed=distribution_util.gen_new_seed(seed, 'beta'))
    ...
    def _log_unnormalized_prob(self, x):
        ...
        return math_ops.xlogy(self.concentration1 - 1.0, x) + (self.concentration0 - 1.0) * math_ops.log1p(-x)

    def _log_normalization(self):
        return math_ops.lgamma(self.concentration1) + math_ops.lgamma(self.concentration0) - math_ops.lgamma(self.total_concentration)
