...
class ViewRequirement:
    ...
    def __post_init__(self):
        ...
        self.shfit_arr = None
        if self.shift_from:
            self.shift_arr = np.arange(self.shift_from, self.shift_to + 1, self.shift_step)
    ...
    def to_dict(self) -> Dict:
        ...
        return {'data_col': self.data_col, 'space': gym_space_to_dict(self.space), 'shift': self.shift, 'index': self.index, 'batch_repeat_value': self.batch_repeat_value, 'used_for_training': self.used_for_training, 'used_for_compute_actions': self.used_for_compute_actions}
