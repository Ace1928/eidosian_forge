...
class SpectralAngleMapper(Metric):
    ...
    def __init__(self, reduction: Optional[Literal['elementwise_mean', 'sum', 'none']]='elementwise_mean', **kwargs: Any) -> None:
        super().__init__(**kwargs)
        if reduction not in ('elementwise_mean', 'sum', 'none', None):
            raise ValueError(f'The `reduction` {reduction} is not valid. Valid options are `elementwise_mean`, `sum`, `none`, None.')
        if reduction == 'none' or reduction is None:
            rank_zero_warn("Metric `SpectralAngleMapper` will save all targets and predictions in the buffer when using`reduction=None` or `reduction='none'. For large datasets, this may lead to a large memory footprint.")
