...
class ExtraRoute(neutron.NeutronResource):
    ...
    support_status = support.SupportStatus(status=support.UNSUPPORTED, message=_('Use this resource at your own risk.'))
    PROPERTIES = ROUTER_ID, DESTINATION, NEXTHOP = ('router_id', 'destination', 'nexthop')
    properties_schema = {ROUTER_ID: properties.Schema(properties.Schema.STRING, description=_('The router id.'), required=True, constraints=[constraints.CustomConstraint('neutron.router')]), DESTINATION: properties.Schema(properties.Schema.STRING, description=_('Network in CIDR notation.'), required=True), NEXTHOP: properties.Schema(properties.Schema.STRING, description=_('Nexthop IP address.'), required=True)}
    ...
    def add_dependencies(self, deps):
        ...
        for resource in self.stack.values():
            if resource.has_interface('OS::Neutron::RouterInterface'):
                ...
                try:
                    router_id = self.properties[self.ROUTER_ID]
                    dep_router_id = resource.properties.get(router.RouterGateway.ROUTER_ID)
                except (ValueError, TypeError):
                    continue
                if dep_router_id == router_id:
                    deps += (self, resource)
    ...
    def handle_create(self):
        ...
        routes = self.client().show_router(router_id).get('router').get('routes')
        if not routes:
            routes = []
        new_route = {'destination': self.properties[self.DESTINATION], 'nexthop': self.properties[self.NEXTHOP]}
        if new_route in routes:
            msg = _('Route duplicates an existing route.')
            raise exception.Error(msg)
        routes.append(new_route.copy())
        self.client().update_router(router_id, {'router': {'routes': routes}})
        new_route['router_id'] = router_id
        self.resource_id_set('%(router_id)s:%(destination)s:%(nexthop)s' % new_route)
    ...
    def handle_delete(self):
        ...
        with self.client_plugin().ignore_not_found:
            routes = self.client().show_router(router_id).get('router').get('routes', [])
            try:
                routes.remove({'destination': self.properties[self.DESTINATION], 'nexthop': self.properties[self.NEXTHOP]})
            except ValueError:
                return
            self.client().update_router(router_id, {'router': {'routes': routes}})