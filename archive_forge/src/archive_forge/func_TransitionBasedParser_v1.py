...
def TransitionBasedParser_v1(tok2vec: Model[List[Doc], List[Floats2d]], state_type: Literal['parser', 'ner'], extra_state_tokens: bool, hidden_width: int, maxout_pieces: int, use_upper: bool=True, nO: Optional[int]=None) -> Model:
    ...
    lower = PrecomputableAffine(nO=hidden_width if use_upper else nO, nF=nr_feature_tokens, nI=tok2vec.get_dim('nO'), nP=maxout_pieces)
    if use_upper:
        with use_ops('numpy'):
