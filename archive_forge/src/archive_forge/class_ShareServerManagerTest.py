...
class ShareServerManagerTest(utils.TestCase):
    ...
    def test_list(self):
        with mock.patch.object(self.manager, '_list', mock.Mock(return_value=None)):
            self.manager.list()
            self.manager._list.assert_called_once_with(share_servers.RESOURCES_PATH, share_servers.RESOURCES_NAME)
    ...
    def test_manage(self, driver_options):
        ...
        if driver_options is None:
            driver_options = {}
        expected_body = {'host': host, 'share_network_id': share_network_id, 'identifier': identifier, 'driver_options': driver_options, 'share_network_subnet_id': share_network_subnet_id}
        with mock.patch.object(self.manager, '_create', mock.Mock(return_value='fake')):
            result = self.manager.manage(host, share_network_id, identifier, driver_options=driver_options, share_network_subnet_id=share_network_subnet_id)
            self.manager._create.assert_called_once_with(share_servers.RESOURCES_PATH + '/manage', {'share_server': expected_body}, 'share_server')
            self.assertEqual('fake', result)

    @ddt.data(True, False)
    def test_unmanage(self, force):
        share_server = {'id': 'fake'}
        with mock.patch.object(self.manager, '_action', mock.Mock(return_value='fake')):
            result = self.manager.unmanage(share_server, force)
            self.manager._action.assert_called_once_with('unmanage', share_server, {'force': force})
            self.assertEqual('fake', result)
    ...
    def test_reset_state(self):
        ...
        with mock.patch.object(self.manager, '_action', mock.Mock(return_value='fake')):
            ...
            self.manager._action.assert_called_once_with('reset_status', share_server, {'status': state})
            self.assertEqual('fake', result)
    ...
    def test_delete(self):
        ...
        with mock.patch.object(self.manager, '_delete', mock.Mock()):
            ...
            self.manager._delete.assert_called_once_with(share_servers.RESOURCE_PATH % share_server_id)

    def test_get(self):
        server = FakeShareServer()
        with mock.patch.object(self.manager, '_get', mock.Mock(return_value=server)):
            share_server_id = 'fake_share_server_id'
            self.manager.get(share_server_id)
            self.manager._get.assert_called_once_with('%s/%s' % (share_servers.RESOURCES_PATH, share_server_id), share_servers.RESOURCE_NAME)
            for key in ['details:fake_key1', 'details:fake_key2']:
                self.assertIn(key, list(server._info))

    def test_details(self):
        with mock.patch.object(self.manager, '_get', mock.Mock(return_value=None)):
            share_server_id = 'fake_share_server_id'
            self.manager.details(share_server_id)
            self.manager._get.assert_called_once_with('%s/%s/details' % (share_servers.RESOURCES_PATH, share_server_id), 'details')

    def test_migration_check(self):
        share_server = 'fake_share_server'
        host = 'fake_host'
        returned = {'compatible': True, 'capacity': True, 'capability': True, 'writable': True, 'nondisruptive': True, 'preserve_snapshots': True}
        with mock.patch.object(self.manager, '_action', mock.Mock(return_value=['200', returned])):
            result = self.manager.migration_check(share_server, host, writable=True, nondisruptive=True, preserve_snapshots=True)
            self.manager._action.assert_called_once_with('migration_check', share_server, {'host': host, 'writable': True, 'nondisruptive': True, 'preserve_snapshots': True, 'new_share_network_id': None})
    ...
    def test_migration_complete(self):
        ...
        with mock.patch.object(self.manager, '_action', mock.Mock(return_value=['200', returned])):
            ...
            self.manager._action.assert_called_once_with('migration_complete', share_server)
            self.assertEqual(returned, result)
    ...
    def test_migration_cancel(self):
        ...
        with mock.patch.object(self.manager, '_action', mock.Mock(return_value=returned)):
            ...
            self.manager._action.assert_called_once_with('migration_cancel', share_server)
            self.assertEqual(returned, result)