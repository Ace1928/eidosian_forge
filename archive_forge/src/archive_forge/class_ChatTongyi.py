...
class ChatTongyi(BaseChatModel):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        values['dashscope_api_key'] = convert_to_secret_str(get_from_dict_or_env(values, 'dashscope_api_key', 'DASHSCOPE_API_KEY'))
        try:
            import dashscope
        except ImportError:
            raise ImportError('Could not import dashscope python package. Please install it with `pip install dashscope --upgrade`.')
        try:
    ...
    def _default_params(self) -> Dict[str, Any]:
        ...
        return {'model': self.model_name, 'top_p': self.top_p, 'api_key': cast(SecretStr, self.dashscope_api_key).get_secret_value(), 'result_format': 'message', **self.model_kwargs}
    ...
    def _generate(self, messages: List[BaseMessage], stop: Optional[List[str]]=None, run_manager: Optional[CallbackManagerForLLMRun]=None, **kwargs: Any) -> ChatResult:
        ...
        return ChatResult(generations=generations, llm_output={'model_name': self.model_name})
    ...
    async def _agenerate(self, messages: List[BaseMessage], stop: Optional[List[str]]=None, run_manager: Optional[AsyncCallbackManagerForLLMRun]=None, **kwargs: Any) -> ChatResult:
        ...
        if self.streaming:
            ...
            params: Dict[str, Any] = self._invocation_params(messages=messages, stop=stop, **kwargs)
            resp = await asyncio.get_running_loop().run_in_executor(None, functools.partial(self.completion_with_retry, **params))
            generations.append(ChatGeneration(**self._chat_generation_from_qwen_resp(resp)))
        return ChatResult(generations=generations, llm_output={'model_name': self.model_name})
    ...
    def _stream(self, messages: List[BaseMessage], stop: Optional[List[str]]=None, run_manager: Optional[CallbackManagerForLLMRun]=None, **kwargs: Any) -> Iterator[ChatGenerationChunk]:
        ...
        for stream_resp, is_last_chunk in generate_with_last_element_mark(self.stream_completion_with_retry(**params)):
            chunk = ChatGenerationChunk(**self._chat_generation_from_qwen_resp(stream_resp, is_chunk=True, is_last_chunk=is_last_chunk))
            if run_manager:
    ...
    def _chunk_to_generation(chunk: ChatGenerationChunk) -> ChatGeneration:
        return ChatGeneration(message=convert_message_chunk_to_message(chunk.message), generation_info=chunk.generation_info)