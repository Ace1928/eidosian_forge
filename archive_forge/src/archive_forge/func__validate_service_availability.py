...
def _validate_service_availability(cls, context, resource_type):
    try:
        ...
        LOG.exception('Resource type %s unavailable', resource_type)
        ex = exception.ResourceTypeUnavailable(resource_type=resource_type, service_name=cls.default_client_name, reason=str(exc))
        raise ex
    else:
        if not svc_available:
            ex = exception.ResourceTypeUnavailable(resource_type=resource_type, service_name=cls.default_client_name, reason=reason)
            LOG.info(str(ex))
            raise ex