...
class JsonHomeTest(unit.BaseTestCase):
    ...
    def test_build_v3_extension_resource_relation(self):
        ...
        resource_name = self.getUniqueString()
        relation = json_home.build_v3_extension_resource_relation(extension_name, extension_version, resource_name)
        exp_relation = 'https://docs.openstack.org/api/openstack-identity/3/ext/%s/%s/rel/%s' % (extension_name, extension_version, resource_name)
        self.assertThat(relation, matchers.Equals(exp_relation))
