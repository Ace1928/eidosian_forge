...
def AddEnableKubeletReadonlyPortFlag(parser, hidden=True):
    ...
    parser.add_argument('--enable-insecure-kubelet-readonly-port', default=None, action='store_true', hidden=hidden, help=textwrap.dedent("      Enables the Kubelet's insecure read only port.\n\n      To disable the readonly port on a cluster or node-pool set the flag to\n      `--no-enable-insecure-kubelet-readonly-port`.\n      "))