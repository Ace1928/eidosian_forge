...
class ConfluenceLoader(BaseLoader):
    ...
    def __init__(self, url: str, api_key: Optional[str]=None, username: Optional[str]=None, session: Optional[requests.Session]=None, oauth2: Optional[dict]=None, token: Optional[str]=None, cloud: Optional[bool]=True, number_of_retries: Optional[int]=3, min_retry_seconds: Optional[int]=2, max_retry_seconds: Optional[int]=10, confluence_kwargs: Optional[dict]=None, *, space_key: Optional[str]=None, page_ids: Optional[List[str]]=None, label: Optional[str]=None, cql: Optional[str]=None, include_restricted_content: bool=False, include_archived_content: bool=False, include_attachments: bool=False, include_comments: bool=False, content_format: ContentFormat=ContentFormat.STORAGE, limit: Optional[int]=50, max_pages: Optional[int]=1000, ocr_languages: Optional[str]=None, keep_markdown_format: bool=False, keep_newlines: bool=False):
        ...
        errors = ConfluenceLoader.validate_init_args(url=url, api_key=api_key, username=username, session=session, oauth2=oauth2, token=token)
        if errors:
    ...
    def _lazy_load(self, **kwargs: Any) -> Iterator[Document]:
        ...
        if space_key:
            pages = self.paginate_request(self.confluence.get_all_pages_from_space, space=space_key, limit=limit, max_pages=max_pages, status='any' if include_archived_content else 'current', expand=f'{content_format.value},version')
            yield from self.process_pages(pages, include_restricted_content, include_attachments, include_comments, content_format, ocr_languages=ocr_languages, keep_markdown_format=keep_markdown_format, keep_newlines=keep_newlines)
        if label:
            pages = self.paginate_request(self.confluence.get_all_pages_by_label, label=label, limit=limit, max_pages=max_pages)
        ...
        if cql:
            pages = self.paginate_request(self._search_content_by_cql, cql=cql, limit=limit, max_pages=max_pages, include_archived_spaces=include_archived_content, expand=f'{content_format.value},version')
            yield from self.process_pages(pages, include_restricted_content, include_attachments, include_comments, content_format, ocr_languages, keep_markdown_format)
        if page_ids:
            for page_id in page_ids:
                get_page = retry(reraise=True, stop=stop_after_attempt(self.number_of_retries), wait=wait_exponential(multiplier=1, min=self.min_retry_seconds, max=self.max_retry_seconds), before_sleep=before_sleep_log(logger, logging.WARNING))(self.confluence.get_page_by_id)
                page = get_page(page_id=page_id, expand=f'{content_format.value},version')
                if not include_restricted_content and (not self.is_public_page(page)):
                    continue
                yield self.process_page(page, include_attachments, include_comments, content_format, ocr_languages, keep_markdown_format)
    ...
    def process_page(self, page: dict, include_attachments: bool, include_comments: bool, content_format: ContentFormat, ocr_languages: Optional[str]=None, keep_markdown_format: Optional[bool]=False, keep_newlines: bool=False) -> Document:
        ...
        if include_comments:
            ...
            comment_texts = [BeautifulSoup(comment['body']['view']['value'], 'lxml').get_text(' ', strip=True) for comment in comments]
            text = text + ''.join(comment_texts)
        metadata = {'title': page['title'], 'id': page['id'], 'source': self.base_url.strip('/') + page['_links']['webui']}
