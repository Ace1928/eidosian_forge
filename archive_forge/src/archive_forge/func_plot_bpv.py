...
def plot_bpv(ax, length_plotters, rows, cols, obs_plotters, pp_plotters, total_pp_samples, kind, t_stat, bpv, plot_mean, reference, mse, n_ref, hdi_prob, color, figsize, textsize, labeller, plot_ref_kwargs, backend_kwargs, show):
    """Bokeh bpv plot."""
    ...
    figsize, ax_labelsize, _, _, linewidth, markersize = _scale_fig_size(figsize, textsize, rows, cols)
    if ax is None:
        ...
        if kind == 'p_value':
            ...
            if reference is not None:
                ...
                if reference == 'analytical':
                    ...
                    ax_i.multi_line(list(x_ss.T), list(u_dens.T), line_width=linewidth, **plot_ref_kwargs)
        elif kind == 'u_value':
            ...
            if reference is not None:
                if reference == 'analytical':
                    ...
                    ax_i.add_layout(BoxAnnotation(bottom=hdi_odds[1], top=hdi_odds[0], fill_alpha=plot_ref_kwargs.pop('alpha'), fill_color=plot_ref_kwargs.pop('line_color'), **plot_ref_kwargs))
            ...
            if plot_mean:
                ax_i.scatter(obs_vals.mean(), 0, fill_color=color, line_color='black', size=markersize, marker='circle')
        _title = Title()
