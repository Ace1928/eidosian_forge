...
class AbstractPutTests:
    ...
    def test_put_file_to_file_in_new_directory(self, fs, fs_join, fs_target, local_join, local_bulk_operations_scenario_0):
        source = local_bulk_operations_scenario_0
        target = fs_target
        fs.mkdir(target)
        fs.put(local_join(source, 'subdir', 'subfile1'), fs_join(target, 'newdir', 'newfile'))
        assert fs.isdir(fs_join(target, 'newdir'))
    ...
    def test_put_directory_to_new_directory(self, fs, fs_join, fs_target, local_bulk_operations_scenario_0, supports_empty_directories):
        source = local_bulk_operations_scenario_0
        target = fs_target
        fs.mkdir(target)
        for source_slash, target_slash in zip([False, True], [False, True]):
    ...
    def test_put_glob_to_existing_directory(self, fs, fs_join, fs_target, local_join, supports_empty_directories, local_bulk_operations_scenario_0):
        source = local_bulk_operations_scenario_0
        target = fs_target
        fs.mkdir(target)
        if not supports_empty_directories:
        ...
        for target_slash in [False, True]:
            ...
            for glob, recursive in zip(['*', '**'], [True, False]):
                ...
                fs.rm([fs_join(target, 'subfile1'), fs_join(target, 'subfile2'), fs_join(target, 'nesteddir')], recursive=True)
                assert fs.ls(target, detail=False) == ([] if supports_empty_directories else [dummy])
                fs.put(local_join(source, 'subdir', glob), t, recursive=recursive, maxdepth=1)
                assert fs.isfile(fs_join(target, 'subfile1'))
                assert fs.isfile(fs_join(target, 'subfile2'))
                assert not fs.exists(fs_join(target, 'nesteddir'))
                assert not fs.exists(fs_join(target, 'subdir'))
    ...
    def test_put_directory_without_files_with_same_name_prefix(self, fs, fs_join, fs_target, local_join, local_dir_and_file_with_same_name_prefix, supports_empty_directories):
