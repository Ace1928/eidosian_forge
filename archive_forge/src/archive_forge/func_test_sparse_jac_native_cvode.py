...
@pycvodes_klu
def test_sparse_jac_native_cvode(nu=0.01, k=1.0, m=1.0, x0=1.0, atol=1e-12, rtol=1e-12):
    w0 = (k / m) ** 0.5

    def f(t, y, p):
        return [y[1], -w0 ** 2 * y[0] - nu * y[1]]
    odesys = NativeSys.from_callback(f, 2, 0, jac=True, sparse=True)
    tout, yout, info = odesys.integrate(100, [x0, 0], integrator='cvode', linear_solver='klu', with_jacobian=True, atol=atol, rtol=rtol, nsteps=20000)
    w = (w0 ** 2 - nu ** 2 / 4.0) ** 0.5
