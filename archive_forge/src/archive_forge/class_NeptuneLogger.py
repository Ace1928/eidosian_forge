...
class NeptuneLogger(Logger):
    ...
    **Details about Neptune run structure**
    ...
    def __init__(self, *, api_key: Optional[str]=None, project: Optional[str]=None, name: Optional[str]=None, run: Optional[Union['Run', 'Handler']]=None, log_model_checkpoints: Optional[bool]=True, prefix: str='training', **neptune_run_kwargs: Any):
    ...
    def after_save_checkpoint(self, checkpoint_callback: Checkpoint) -> None:
        ...
        if hasattr(checkpoint_callback, 'best_model_score') and checkpoint_callback.best_model_score:
            self.run[self._construct_path_with_prefix('model/best_model_score')] = checkpoint_callback.best_model_score.cpu().detach().numpy()
