...
def compute_spectral_features(signal: np.ndarray, sr: int) -> dict:
    ...
    try:
        spectral_centroid = np.sum(np.arange(len(signal)) * np.abs(signal)) / np.sum(np.abs(signal))
        spectral_bandwidth = np.sqrt(
            np.sum((np.arange(len(signal)) - spectral_centroid) ** 2 * np.abs(signal)) / np.sum(np.abs(signal))
        )
...
def compute_spectral_features_tensor_wave(signal: np.ndarray, sr: int) -> np.ndarray:
    ...
    try:
        ...
        logging.error(f"Error computing spectral features tensor wave: {e}", exc_info=True)
...
def compute_3d_surface(signal: np.ndarray, sr: int) -> np.ndarray:
    ...
    try:
        ...
        X, Y = np.meshgrid(np.arange(spectrogram.shape[1]), np.arange(spectrogram.shape[0]))
        Z = spectrogram
...
def create_tensor_wave(signal: np.ndarray, sr: int) -> dict:
    ...
    try:
        tensor_waves["spectral_features"] = compute_spectral_features_tensor_wave(signal, sr)
    except Exception as e:
...
def save_visualization(
    ...
    try:
        if viz_type == "original_signal":
            ...
            fig.add_trace(
                go.Scatter(
                    x=time, y=spectral_features[0], mode="lines", name="Spectral Centroid"
                )
            )
            fig.add_trace(
                go.Scatter(
                    x=time, y=spectral_features[1], mode="lines", name="Spectral Bandwidth"
                )
            )
            fig.add_trace(
                go.Scatter(
                    x=time, y=spectral_features[2], mode="lines", name="Spectral Rolloff"
                )
            )
            fig.add_trace(
                go.Scatter(
                    x=time, y=spectral_features[3], mode="lines", name="Zero Crossing Rate"
                )
            )
            fig.update_layout(
    ...
    if viz_type == "original_signal":
        ...
        fig = px.imshow(spectrogram, aspect="auto", color_continuous_scale="viridis")
        fig.update_layout(
            title=f"{title} - Spectrogram", xaxis_title="Time", yaxis_title="Frequency"
        )
        ...
        fig.update_layout(title=f"{title} - Tonnetz", xaxis_title="", yaxis_title="")
