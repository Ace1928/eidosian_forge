...
def _parse_request(self, enc_request, request_cls, service, binding, relay_state=None, sigalg=None, signature=None):
    """Parse a Request
    ...
    if only_valid_cert:
        must = True
    _request = _request.loads(xmlstr, binding, origdoc=enc_request, must=must, only_valid_cert=only_valid_cert, relay_state=relay_state, sigalg=sigalg, signature=signature)
