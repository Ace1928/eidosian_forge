def test_standardcomparator():
    from ase.ga.standard_comparators import InteratomicDistanceComparator, EnergyComparator, RawScoreComparator, SequentialComparator
    ...
    comp1 = InteratomicDistanceComparator(n_top=3, pair_cor_cum_diff=0.03, pair_cor_max=0.7, dE=0.3)
    assert comp1.looks_like(a1, a2)
    comp2 = InteratomicDistanceComparator(n_top=3, pair_cor_cum_diff=0.03, pair_cor_max=0.7, dE=0.15)
    assert not comp2.looks_like(a1, a2)
    comp3 = InteratomicDistanceComparator(n_top=3, pair_cor_cum_diff=0.02, pair_cor_max=0.7, dE=0.3)
