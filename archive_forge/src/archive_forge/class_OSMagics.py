...
class OSMagics(Magics):
    ...
    def __init__(self, shell=None, **kwargs):
        ...
        if os.name == 'posix':
            ...
            try:
                self.execre = re.compile('(.*)\\.(%s)$' % winext, re.IGNORECASE)
            except re.error:
                warn('Seems like your pathext environmental variable is malformed. Please check it to enable a proper handle of file extensions managed for your system')
                winext = 'exe|com|bat|py'
                self.execre = re.compile('(.*)\\.(%s)$' % winext, re.IGNORECASE)
    ...
    def cd(self, parameter_s=''):
        ...
        elif not os.path.isdir(ps) or 'b' in opts:
