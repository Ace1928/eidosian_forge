...
def softsign_grad(gradients: _atypes.TensorFuzzingAnnotation[TV_SoftsignGrad_T], features: _atypes.TensorFuzzingAnnotation[TV_SoftsignGrad_T], name=None) -> _atypes.TensorFuzzingAnnotation[TV_SoftsignGrad_T]:
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'SoftsignGrad', name, gradients, features)
            return _result
        ...
        try:
            ...
            pass
    _, _, _op, _outputs = _op_def_library._apply_op_helper('SoftsignGrad', gradients=gradients, features=features, name=name)
    _result = _outputs[:]
