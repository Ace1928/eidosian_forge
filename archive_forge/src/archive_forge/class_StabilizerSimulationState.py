...
class StabilizerSimulationState(SimulationState[TStabilizerState], Generic[TStabilizerState], metaclass=abc.ABCMeta):
    ...
    def __init__(self, *, state: TStabilizerState, prng: Optional[np.random.RandomState]=None, qubits: Optional[Sequence['cirq.Qid']]=None, classical_data: Optional['cirq.ClassicalDataStore']=None):
    ...
    def _act_on_fallback_(self, action: Any, qubits: Sequence['cirq.Qid'], allow_decompose: bool=True) -> Union[bool, NotImplementedType]:
    ...
    def _swap(self, control_axis: int, target_axis: int, exponent: float=1, global_shift: float=0):
        """Apply a SWAP gate"""
        if exponent % 1 != 0:
