...
def _append_row_groups(metadata, md):
    ...
    try:
        ...
        if 'requires equal schemas' in str(err):
            raise RuntimeError('Schemas are inconsistent, try using `to_parquet(..., schema="infer")`, or pass an explicit pyarrow schema. Such as `to_parquet(..., schema={"column1": pa.string()})`') from err
        else:
            raise err