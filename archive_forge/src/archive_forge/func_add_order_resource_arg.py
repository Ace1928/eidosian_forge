...
def add_order_resource_arg(parser, verb):
  ...
  """
    concept_parsers.ConceptParser.ForResource('order', get_order_resource_spec(), 'The order to {}.'.format(verb.value), flag_name_overrides={'region': ''}, prefixes=True, required=True).AddToParser(parser)
