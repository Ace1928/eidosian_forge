...
class DirectSolver(DirectOrPersistentSolver):
    ...
    def _presolve(self, *args, **kwds):
        ...
        model = args[0]
        if len(args) != 1:
            msg = ("The {0} plugin method '_presolve' must be supplied a single problem instance - {1} were " + 'supplied.').format(type(self), len(args))
        ...
        for arg in args:
            if isinstance(arg, (_BlockData, IBlock)):
                ...
                if isinstance(arg, _BlockData):
                    ...
                    model_suffixes = list((comp.storage_key for comp in import_suffix_generator(arg, active=True, descend_into=False)))
