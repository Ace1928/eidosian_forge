...
class RestController(object):
    ...
    def __new__(cls, *args, **kwargs):
        ...
        for name, value in getmembers(cls):
            if iscontroller(value) and getattr(value, 'custom_route', None):
                raise ValueError('Path segments cannot be used in combination with pecan.rest.RestController.  Remove the `route` argument to @pecan.expose on %s.%s.%s' % (cls.__module__, cls.__name__, value.__name__))
    ...
    def _route(self, args, request=None):
        ...
        if result:
            return result
        handler = getattr(self, '_handle_%s' % method, self._handle_unknown_method)
        try:
        ...
        except (exc.HTTPClientError, exc.HTTPNotFound, exc.HTTPMethodNotAllowed) as e:
            _lookup_result = self._handle_lookup(args, request)
            if _lookup_result:
                return _lookup_result
