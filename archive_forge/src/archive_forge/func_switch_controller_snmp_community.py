...
def switch_controller_snmp_community(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('switch-controller', 'snmp-community', filtered_data, vdom=vdom)
        current_data = fos.get('switch-controller', 'snmp-community', vdom=vdom, mkey=mkey)
    ...
    if state == 'present' or state is True:
        return fos.set('switch-controller', 'snmp-community', data=filtered_data, vdom=vdom)
    elif state == 'absent':
        return fos.delete('switch-controller', 'snmp-community', mkey=filtered_data['id'], vdom=vdom)
    else:
