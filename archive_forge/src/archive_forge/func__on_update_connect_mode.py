...
def _on_update_connect_mode(self, mode):
    if mode is not CONNECT_MODE_PASSIVE and 'peer.connect_loop' not in self._child_thread_map:
        LOG.debug('start connect loop. (mode: %s)', mode)
        self._spawn('peer.connect_loop', self._connect_loop, self._client_factory)
