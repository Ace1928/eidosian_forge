...
class XArrayInterface(GridInterface):
    ...
    def init(cls, eltype, data, kdims, vdims):
        ...
        if isinstance(data, xr.DataArray):
            ...
            if vdim_len > 1 and kdim_len == len(data.dims) - 1 and (data.shape[-1] == vdim_len):
                ...
                if vdim.name in data.dims:
                    raise DataError("xarray DataArray does not define a name, and the default of '%s' clashes with a coordinate dimension. Give the DataArray a name or supply an explicit value dimension." % vdim.name, cls)
            else:
                raise DataError('xarray DataArray does not define a name and %s does not define a default value dimension. Give the DataArray a name or supply an explicit vdim.' % eltype.__name__, cls)
            if not packed:
        ...
        if not isinstance(data, (xr.Dataset, xr.DataArray)):
            ...
            if kdims is None:
                ...
                kdims = [name for name in data.indexes.keys() if isinstance(data[name].data, np.ndarray)]
        ...
        not_found = []
        for d in kdims:
            if not any((d.name == k or (isinstance(v, xr.DataArray) and d.name in v.dims) for k, v in data.coords.items())):
                not_found.append(d)
        if not isinstance(data, (xr.Dataset, xr.DataArray)):
            raise TypeError('Data must be be an xarray Dataset type.')
        elif not_found:
            raise DataError('xarray Dataset must define coordinates for all defined kdims, %s coordinates not found.' % not_found, cls)
        for vdim in vdims:
            if packed:
                continue
            da = data[vdim.name]
    ...
    def validate(cls, dataset, vdims=True):
        ...
        if irregular:
            nonmatching = [f'{kd}: {dims}' for kd, dims in irregular[1:] if set(dims) != set(irregular[0][1])]
            if nonmatching:
                nonmatching = ['{}: {}'.format(*irregular[0])] + nonmatching
                raise DataError('The dimensions of coordinate arrays on irregular data must match. The following kdims were found to have non-matching array dimensions:\n\n%s' % '\n'.join(nonmatching), cls)
    ...
    def values(cls, dataset, dim, expanded=True, flat=True, compute=True, keep_index=False):
        ...
        if dim in dataset.vdims or irregular:
            ...
            if not keep_index:
                data = cls.canonicalize(dataset, data, data_coords=data_coords, virtual_coords=virtual_coords)
    ...
    def ndloc(cls, dataset, indices):
        ...
        sampled = all((isinstance(ind, np.ndarray) and ind.dtype.kind != 'b' for ind in adjusted_indices)) and len(indices) == len(kdims)
    ...
    def concat_dim(cls, datasets, dim, vdims):
        import xarray as xr
        return xr.concat([ds.assign_coords(**{dim.name: c}) for c, ds in datasets.items()], dim=dim.name)
    ...
    def reindex(cls, dataset, kdims=None, vdims=None):
        ...
        if len(constant) == len(dropped_kdims):
            dropped = dataset.data.sel(**{k: v for k, v in constant.items() if k in dataset.data.dims})
    ...
    def select(cls, dataset, selection_mask=None, **selection):
        ...
        dropped = dict(((d.name, np.atleast_1d(data[d.name])) for d in dataset.kdims if not data[d.name].data.shape))
    ...
    def dframe(cls, dataset, dimensions):
        import xarray as xr
        if cls.packed(dataset):
            bands = {vd.name: dataset.data[..., i].drop_vars('band') for i, vd in enumerate(dataset.vdims)}
    ...
    def sample(cls, dataset, samples=None):
        ...
        samples = [dataset.data.sel(**{k: [v] for k, v in zip(names, s)}).to_dataframe().reset_index() for s in samples]
    ...
    def add_dimension(cls, dataset, dimension, dim_pos, values, vdim):
        ...
        arr = xr.DataArray(values, coords=coords, name=dim, dims=tuple((d.name for d in dataset.kdims[::-1])))
