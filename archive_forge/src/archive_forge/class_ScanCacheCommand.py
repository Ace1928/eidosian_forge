...
class ScanCacheCommand(BaseHuggingfaceCLICommand):
    ...
    def register_subcommand(parser: _SubParsersAction):
        ...
        scan_cache_parser.add_argument('--dir', type=str, default=None, help='cache directory to scan (optional). Default to the default HuggingFace cache.')
        scan_cache_parser.add_argument('-v', '--verbose', action='count', default=0, help='show a more verbose output')
    ...
    def run(self):
        ...
        print(f'\nDone in {round(t1 - t0, 1)}s. Scanned {len(hf_cache_info.repos)} repo(s) for a total of {ANSI.red(hf_cache_info.size_on_disk_str)}.')
