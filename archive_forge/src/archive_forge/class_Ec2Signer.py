...
class Ec2Signer(object):
    ...
    def _v4_creds(self, credentials):
        ...
        try:
            ...
            try:
                if credentials['params']['X-Amz-Algorithm'] == 'AWS4-HMAC-SHA256':
                    return True
            except KeyError:
                pass
        return False
    ...
    def generate(self, credentials):
        ...
        if signature_version == '2':
            return self._calc_signature_2(credentials['params'], credentials['verb'], credentials['host'], credentials['path'])
        if self._v4_creds(credentials):
            return self._calc_signature_4(credentials['params'], credentials['verb'], credentials['host'], credentials['path'], credentials['headers'], credentials['body_hash'])
        if signature_version is not None:
            raise Exception(_('Unknown signature version: %s') % signature_version)
        else:
