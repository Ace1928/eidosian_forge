...
class TestInputDataStandardizer(unittest.TestCase):
    ...
    def test_standardizer_indexed_component(self):
        ...
        standardizer_output = standardizer_func(standardizer_input)
        self.assertIsInstance(standardizer_output, list, msg=f'Standardized output should be of type list, but is of type {standardizer_output.__class__.__name__}.')
        self.assertEqual(len(standardizer_output), 2, msg='Length of standardizer output is not as expected.')
    ...
    def test_standardizer_iterable_with_invalid_type(self):
        ...
        mdl.v = Var([0, 1])
        standardizer_func = InputDataStandardizer(Var, _VarData)
        exc_str = 'Input object .*entry of iterable.*is not of valid component type.*'
        with self.assertRaisesRegex(TypeError, exc_str):
