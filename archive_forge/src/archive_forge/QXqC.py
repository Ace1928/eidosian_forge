...
class UniversalGUIBuilder:
    ...
    def drag_start(self, event: tk.Event, widget_type: str) -> None:
        ...
        """
        self.dragged_widget = {"type": widget_type, "x": event.x, "y": event.y}
    ...
    def drag_motion(self, event: tk.Event) -> None:
        ...
        if self.dragged_widget:
            ...
            self.canvas.move(self.dragged_widget["id"], dx, dy)
            self.drag_start_x = event.x
            self.drag_start_y = event.y
    ...
    def canvas_drag(self, event: tk.Event) -> None:
        """
        Handle dragging a widget on the canvas.

        :param event: The tkinter event that triggered the canvas drag.
        :type event: tk.Event
        """
        if self.selected_widget:
            ...
            self.selected_widget["x"] += dx
            self.selected_widget["y"] += dy
            self.drag_start_x = event.x
            self.drag_start_y = event.y

    def canvas_release(self, event: tk.Event) -> None:
        """
        Handle releasing a dragged widget on the canvas.

        :param event: The tkinter event that triggered the canvas release.
        :type event: tk.Event
        """
        if self.selected_widget:
    ...
    def select_widget(self, widget: dict) -> None:
        ...
        self.canvas.itemconfig(widget["id"], outline="blue", width=2)
        self.drag_start_x = widget["x"] + widget["width"] // 2
        self.drag_start_y = widget["y"] + widget["height"] // 2
    ...
    def deselect_widget(self) -> None:
        ...
        """
        if self.selected_widget:
            self.canvas.itemconfig(self.selected_widget["id"], outline="", width=0)
            self.selected_widget = None
    ...
    def display_widget_properties(self) -> None:
        ...
        if self.selected_widget:
            for key, value in self.selected_widget.items():
                if key not in ["id", "x", "y", "width", "height"]:
    ...
    def save_widget_properties(self) -> None:
        ...
        if self.selected_widget:
            ...
            self.update_widget_config()
            self.redraw_widget(self.selected_widget)

    def add_widget(self, widget_type: str, x: int = 0, y: int = 0, **kwargs) -> None:
        """
        ...
        widget_id = self.draw_widget(widget_config)
        widget_config["id"] = widget_id

    def draw_widget(self, widget_config: dict) -> int:
        """
        ...
        :return: The ID of the drawn widget on the canvas.
        :rtype: int
        """
    ...
    def redraw_widget(self, widget_config: dict) -> None:
        """
        Redraw a widget on the canvas based on its updated configuration.

        :param widget_config: The updated widget configuration dictionary.
        :type widget_config: dict
        """
        self.canvas.delete(widget_config["id"])
        widget_id = self.draw_widget(widget_config)
        widget_config["id"] = widget_id
    ...
    def update_widget_config(self) -> None:
        ...
        """
        if self.selected_widget:
            self.selected_widget["x"] = self.canvas.coords(self.selected_widget["id"])[
                0
            ]
            self.selected_widget["y"] = self.canvas.coords(self.selected_widget["id"])[
                1
            ]
            self.selected_widget["width"] = (
                self.canvas.coords(self.selected_widget["id"])[2]
                - self.canvas.coords(self.selected_widget["id"])[0]
            )
            self.selected_widget["height"] = (
                self.canvas.coords(self.selected_widget["id"])[3]
                - self.canvas.coords(self.selected_widget["id"])[1]
            )
