...
class CloudSearchConnection(AWSQueryConnection):
    ...
    _faults = {'InvalidTypeException': exceptions.InvalidTypeException, 'LimitExceededException': exceptions.LimitExceededException, 'InternalException': exceptions.InternalException, 'DisabledOperationException': exceptions.DisabledOperationException, 'ResourceNotFoundException': exceptions.ResourceNotFoundException, 'BaseException': exceptions.BaseException}
    ...
    def describe_index_fields(self, domain_name, field_names=None, deployed=None):
        ...
        if deployed is not None:
            params['Deployed'] = str(deployed).lower()
        return self._make_request(action='DescribeIndexFields', verb='POST', path='/', params=params)
