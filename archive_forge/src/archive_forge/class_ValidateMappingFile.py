...
class ValidateMappingFile:
    ...
    def run(self, conf_file):
        print('%s%sValidate variable mapping file for oVirt ansible disaster recovery%s' % (INFO, PREFIX, END))
        self._set_dr_conf_variables(conf_file)
        print("%s%sVar File: '%s'%s" % (INFO, PREFIX, self.var_file, END))
        python_vars = self._read_var_file()
        if not self._validate_lists_in_mapping_file(python_vars) or not self._validate_duplicate_keys(python_vars) or (not self._entity_validator(python_vars)) or (not self._validate_failback_leftovers()):
    ...
    def _is_list(self, mapping_vars, mapping):
        ...
        if not isinstance(map_file, list) and map_file is not None:
            print("%s%s%s is not a list: '%s'. Please check your mapping file%s" % (FAIL, PREFIX, mapping, map_file, END))
            return False
        return True
    ...
    def _get_affinity_labels(self, conn):
        affinity_labels = set()
        affinity_labels_service = conn.system_service().affinity_labels_service()
        ...
        for vnic_profile_item in vnic_profile_list:
            ...
            for network_item in networks_list:
                if network_item.id == vnic_profile_item.network.id:
                    ...
                    dc_name = conn.system_service().data_centers_service().data_center_service(network_item.data_center.id).get()._name
                    break
    ...
    def _validate_networks(self, var_file, networks_setup, setup):
        ...
        for mapping in _mappings:
            map_key = mapping[keys[0]] + '_' + mapping[keys[1]] + '_' + (mapping[keys[2]] if keys[2] in mapping else '')
            if map_key in dups:
                if keys[2] not in mapping:
                    print("%s%sVnic profile name '%s' and network name '%s' are related to multiple data centers in the %s setup. Please specify the data center name in the mapping var file.%s" % (FAIL, PREFIX, mapping[keys[0]], mapping[keys[1]], setup, END))
                    return False
        print("%s%sFinished validation for 'dr_network_mappings' for %s setup with success.%s" % (INFO, PREFIX, setup, END))
        return True
