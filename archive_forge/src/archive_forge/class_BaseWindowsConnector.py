...
class BaseWindowsConnector(initiator_connector.InitiatorConnector):
    ...
    def __init__(self, root_helper=None, *args, **kwargs):
        warnings.warn('Support for Windows OS has been deprecated.', category=DeprecationWarning, stacklevel=2)
        kwargs['executor'] = kwargs.get('executor') or putils.execute
        super(BaseWindowsConnector, self).__init__(root_helper, *args, **kwargs)
        self.device_scan_interval = kwargs.pop('device_scan_interval', self.DEFAULT_DEVICE_SCAN_INTERVAL)
    ...
    def get_connector_properties(*args, **kwargs):
        ...
        props = {}
        props['multipath'] = multipath and BaseWindowsConnector.check_multipath_support(enforce_multipath)
        return props
    ...
    def extend_volume(self, connection_properties):
        ...
        device_number = self._diskutils.get_device_number_from_device_name(device_path)
