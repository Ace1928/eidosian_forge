...
class UpstashRedisChatMessageHistory(BaseChatMessageHistory):
    ...
    def __init__(self, session_id: str, url: str='', token: str='', key_prefix: str='message_store:', ttl: Optional[int]=None):
        try:
            from upstash_redis import Redis
        except ImportError:
            raise ImportError('Could not import upstash redis python package. Please install it with `pip install upstash_redis`.')
        if url == '' or token == '':
