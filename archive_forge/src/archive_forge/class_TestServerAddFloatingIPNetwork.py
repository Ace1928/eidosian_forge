...
class TestServerAddFloatingIPNetwork(TestServer, network_fakes.TestNetworkV2):
    ...
    def test_server_add_floating_ip_with_fixed_ip(self):
        ...
        self.network_client.find_ip = mock.Mock(return_value=_floating_ip)
        self.network_client.ports = mock.Mock(return_value=[_port])
        arglist = ['--fixed-ip-address', _port.fixed_ips[0]['ip_address'], _server.id, _floating_ip['floating_ip_address']]
        verifylist = [('fixed_ip_address', _port.fixed_ips[0]['ip_address']), ('server', _server.id), ('ip_address', _floating_ip['floating_ip_address'])]
        parsed_args = self.check_parser(self.cmd, arglist, verifylist)
        self.cmd.take_action(parsed_args)
        attrs = {'port_id': _port.id, 'fixed_ip_address': _port.fixed_ips[0]['ip_address']}
        self.network_client.find_ip.assert_called_once_with(_floating_ip['floating_ip_address'], ignore_missing=False)
        self.network_client.ports.assert_called_once_with(device_id=_server.id)
        self.network_client.update_ip.assert_called_once_with(_floating_ip, **attrs)
