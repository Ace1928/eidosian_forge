...
class Geodesic:
    ...
    def __str__(self):
        return f'<Geodesic: radius={self.radius:0.3f}, flattening=1/{1 / self.flattening:0.3f}>'
    ...
    def inverse(self, points, endpoints):
        ...
        if epts.shape[0] == 1:
            ...
            epts[:, :] = orig_pts
        start_azims, end_azims, dists = self.geod.inv(pts[:, 0], pts[:, 1], epts[:, 0], epts[:, 1])
    ...
    def circle(self, lon, lat, radius, n_samples=180, endpoint=False):
        ...
        azimuths = np.linspace(360.0, 0.0, n_samples, endpoint=endpoint).astype(np.double)
