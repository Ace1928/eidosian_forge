...
def _get_metric_name_list():
    ...
    for metric_method_name in metrics.__all__:
        ...
        if metric_method_name not in _metric_api_excluding_list and (not inspect.isclass(metric_method)) and callable(metric_method) and (not metric_method_name.startswith('plot_')):
