...
def _disambiguate_shift(reference_image, moving_image, shift):
    ...
    if max_slice is None:
        warnings.warn(f'Could not determine real-space shift for periodic shift {shift!r} as requested by `disambiguate=True` (disambiguation is degenerate).', stacklevel=3)
        return shift
    real_shift_acc = []
