...
def _ImportGsutil(gcs_bucket, source, destination):
    ...
    destination_ref = storage_util.ObjectReference.FromBucketRef(gcs_bucket, destination)
    try:
        retval = storage_util.RunGsutilCommand('cp', command_args=['-r', source, destination_ref.ToUrl()], run_concurrent=True, out_func=log.out.write, err_func=log.err.write)
