...
class RetryingConnection(Connection[Request, Response], PermanentFailable):
            ...
            while not self.error():
                try:
                    ...
                    if self.error():
                        return
                    e = adapt_error(e)
                    logging.debug('Saw a stream failure. Cause: \n%s', traceback.format_exc())
                    ...
                    self._read_queue = asyncio.Queue(maxsize=1)
                    self._write_queue = asyncio.Queue(maxsize=1)
                    await wait_ignore_cancelled(asyncio.sleep(min(_MAX_BACKOFF_SECS, _MIN_BACKOFF_SECS * 2 ** bad_retries)))
                    bad_retries += 1
        except Exception as e:
            logging.error('Saw a stream failure which was unhandled. Cause: \n%s', traceback.format_exc())
    ...
    async def _loop_connection(self, connection: Connection[Request, Response]):
        ...
        write_task: 'Future[WorkItem[Request, None]]' = asyncio.ensure_future(self._write_queue.get())
        try:
            while True:
