...
class H11Handshake:
    ...
    def _accept(self, event: AcceptConnection) -> bytes:
        ...
        headers = [(b'Upgrade', b'WebSocket'), (b'Connection', b'Upgrade'), (b'Sec-WebSocket-Accept', accept_token)]
        ...
        if event.extensions:
            accepts = server_extensions_handshake(cast(Sequence[str], self._initiating_request.extensions), event.extensions)
            if accepts:
        ...
        self._connection = Connection(ConnectionType.CLIENT if self.client else ConnectionType.SERVER, event.extensions)
    ...
    def _establish_client_connection(self, event: h11.InformationalResponse) -> AcceptConnection:
        assert self._initiating_request is not None
        ...
        if subprotocol is not None:
            if subprotocol not in self._initiating_request.subprotocols:
                raise RemoteProtocolError(f'unrecognized subprotocol {subprotocol}', event_hint=RejectConnection())
        extensions = client_extensions_handshake(accepts, cast(Sequence[Extension], self._initiating_request.extensions))
        self._connection = Connection(ConnectionType.CLIENT if self.client else ConnectionType.SERVER, extensions, self._h11_connection.trailing_data[0])
        self._state = ConnectionState.OPEN
