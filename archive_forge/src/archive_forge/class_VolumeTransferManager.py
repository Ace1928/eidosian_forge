...
class VolumeTransferManager(base.ManagerWithFind):
    ...
    def create(self, volume_id, name=None, no_snapshots=False):
        ...
        """
        body = {'transfer': {'volume_id': volume_id, 'name': name}}
    ...
    def accept(self, transfer_id, auth_key):
        ...
        if self.api_version.matches('3.55'):
            return self._create('/volume-transfers/%s/accept' % transfer_id, body, 'transfer')
        return self._create('/os-volume-transfer/%s/accept' % transfer_id, body, 'transfer')
    ...
    def list(self, detailed=True, search_opts=None, sort=None):
        ...
        url = self._build_list_url(resource_type, detailed=detailed, search_opts=search_opts, sort=sort)
