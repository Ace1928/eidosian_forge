...
class DialogueNliTeacher(FixedDialogTeacher):
    ...
    def __init__(self, opt, shared=None, extras=False):
        ...
        if suffix.startswith('train'):
            ...
            suffix = 'dev'
        if extras:
            datapath = os.path.join(opt['datapath'], 'dialogue_nli', 'dnli', 'dialogue_nli_extra', 'dialogue_nli_EXTRA_uu_' + suffix + '.jsonl')
        else:
            datapath = os.path.join(opt['datapath'], 'dialogue_nli', 'dnli', 'dialogue_nli', 'dialogue_nli_' + suffix + '.jsonl')
    ...
    def get(self, episode_idx, entry_idx=0):
        ...
        entry['text'], entry['labels'], entry['label_candidates'] = convert_to_dialogData(premise_raw=entry[DNLI_PREMISE_KEY], hypo_raw=entry[DNLI_HYPO_KEY], answer_raw=DNLI_LABEL_DICT[entry[DNLI_ANSWER_KEY]], dialog_format=self.dialog_format, binary_classes=self.binary_classes)
