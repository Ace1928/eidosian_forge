...
class Stride2D(nn.Module):
    ...
    def __init__(self, width, height, stride_w, stride_h):
        ...
        self.zeros = torch.zeros(size=(self.width * self.stride_w - (self.stride_w - 1), self.height * self.stride_h - (self.stride_h - 1)), dtype=torch.float32)
        self.out_width, self.out_height = (self.zeros.shape[0], self.zeros.shape[1])
        self.zeros = self.zeros.unsqueeze(0).unsqueeze(0)
        self.where_template = torch.zeros((self.stride_w, self.stride_h), dtype=torch.float32)
        self.where_template[0][0] = 1.0
        self.where_template = self.where_template.repeat((self.height, self.width))[:-(self.stride_w - 1), :-(self.stride_h - 1)]
        self.where_template = self.where_template.unsqueeze(0).unsqueeze(0).bool()

    def forward(self, x):
        repeated_x = x.repeat_interleave(self.stride_w, dim=-2).repeat_interleave(self.stride_h, dim=-1)[:, :, :-(self.stride_w - 1), :-(self.stride_h - 1)]
