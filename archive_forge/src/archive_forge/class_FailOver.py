...
class FailOver:
    ...
    def run(self, conf_file, log_file, log_level):
        ...
        dr_tag = 'fail_over'
        extra_vars = ' dr_target_host=' + target_host + ' dr_source_map=' + source_map + ' dr_report_file=' + report
        command = ['ansible-playbook', ansible_play_file, '-t', dr_tag, '-e', '@' + var_file, '-e', '@' + vault_file, '-e', extra_vars, '--vault-password-file', 'vault_secret.sh', '-vvv']
    ...
    def _handle_result(self, command):
        try:
            ...
            print('%sException: %s\n\nfailover operation failed, please check log file for further details.%s' % (FAIL, e, END))
            sys.exit()
