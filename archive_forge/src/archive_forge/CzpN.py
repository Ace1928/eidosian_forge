...
class ImageCompressorApp(tk.Tk):
    ...
    async def async_compress_and_store_image(self):
        try:
            await insert_compressed_image(
                self.img_hash, "PNG", self.original_image_data
            )
            self.after(
                0,
    ...
    async def async_decompress_and_show(self):
        try:
            image_data, _ = await retrieve_compressed_image(self.img_hash)
            _, image_format = await retrieve_compressed_image(self.img_hash)

            # Correctly handling the image format when opening the image
            img = Image.open(io.BytesIO(image_data, _))
            # While PIL automatically detects the format, ensuring consistency in logging or further processing
            LoggingManager.debug(
                f"Image format {image_format} extracted and image loaded successfully."
            )
