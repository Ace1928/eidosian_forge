...
class EdenAI(LLM):
    ...
    resolution: Optional[Literal['256x256', '512x512', '1024x1024']] = None
    ...
    def build_extra(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for field_name in list(values):
            if field_name not in all_required_field_names:
                ...
                logger.warning(f'{field_name} was transferred to model_kwargs.\n                    Please confirm that {field_name} is what you intended.')
    ...
    def _call(self, prompt: str, stop: Optional[List[str]]=None, run_manager: Optional[CallbackManagerForLLMRun]=None, **kwargs: Any) -> str:
        ...
        payload: Dict[str, Any] = {'providers': self.provider, 'text': prompt, 'max_tokens': self.max_tokens, 'temperature': self.temperature, 'resolution': self.resolution, **self.params, **kwargs, 'num_images': 1}
        ...
        if response.status_code >= 500:
            ...
            raise Exception(f'EdenAI returned an unexpected response with status {response.status_code}: {response.text}')
        data = response.json()
        ...
        async with ClientSession() as session:
            async with session.post(url, json=payload, headers=headers) as response:
                if response.status >= 500:
                    ...
                    raise Exception(f'EdenAI returned an unexpected response with status {response.status}: {response.text}')
