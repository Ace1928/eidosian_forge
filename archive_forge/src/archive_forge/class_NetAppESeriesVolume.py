...
class NetAppESeriesVolume(NetAppESeriesModule):
    ...
    def __init__(self):
        ...
        if self.thin_provision:
            ...
            if self.size_b > 256 * 1024 ** 4:
                self.module.fail_json(msg='Thin provisioned volumes must be less than or equal to 256tb is size. Attempted size [%sg]' % (self.size_b * 1024 ** 3))
            if self.thin_volume_repo_size_b and self.thin_volume_max_repo_size_b and (self.thin_volume_repo_size_b > self.thin_volume_max_repo_size_b):
                self.module.fail_json(msg='The initial size of the thin volume must not be larger than the maximum repository size. Array [%s].' % self.ssid)
            if self.thin_volume_growth_alert_threshold < 10 or self.thin_volume_growth_alert_threshold > 99:
                self.module.fail_json(msg='thin_volume_growth_alert_threshold must be between or equal to 10 and 99.thin_volume_growth_alert_threshold [%s]. Array [%s].' % (self.thin_volume_growth_alert_threshold, self.ssid))
    ...
    def get_volume(self):
        ...
        try:
            ...
            self.module.fail_json(msg='Failed to obtain list of thick volumes.  Array Id [%s]. Error[%s].' % (self.ssid, to_native(err)))
        try:
            rc, thin_volumes = self.request('storage-systems/%s/thin-volumes' % self.ssid)
        except Exception as err:
            self.module.fail_json(msg='Failed to obtain list of thin volumes.  Array Id [%s]. Error[%s].' % (self.ssid, to_native(err)))
    ...
    def create_volume(self):
        ...
        body = dict(name=self.name, poolId=self.pool_detail['id'], sizeUnit='bytes', dataAssuranceEnabled=self.data_assurance_enabled)
        if self.thin_provision:
            body.update(dict(virtualSize=self.size_b, repositorySize=self.thin_volume_repo_size_b, maximumRepositorySize=self.thin_volume_max_repo_size_b, expansionPolicy=self.thin_volume_expansion_policy, growthAlertThreshold=self.thin_volume_growth_alert_threshold))
            try:
                rc, volume = self.request('storage-systems/%s/thin-volumes' % self.ssid, data=body, method='POST')
            except Exception as error:
                self.module.fail_json(msg='Failed to create thin volume.  Volume [%s].  Array Id [%s]. Error[%s].' % (self.name, self.ssid, to_native(error)))
            ...
            try:
                ...
                self.module.fail_json(msg='Failed to create volume.  Volume [%s].  Array Id [%s]. Error[%s].' % (self.name, self.ssid, to_native(error)))
    ...
    def delete_volume(self):
        ...
        if self.thin_provision:
            try:
                rc, resp = self.request('storage-systems/%s/thin-volumes/%s' % (self.ssid, self.volume_detail['id']), method='DELETE')
            except Exception as error:
                self.module.fail_json(msg='Failed to delete thin volume. Volume [%s]. Array Id [%s]. Error[%s].' % (self.name, self.ssid, to_native(error)))
            self.module.log('Thin volume deleted [%s].' % self.name)
        else:
            try:
                rc, resp = self.request('storage-systems/%s/volumes/%s' % (self.ssid, self.volume_detail['id']), method='DELETE')
            except Exception as error:
                self.module.fail_json(msg='Failed to delete volume. Volume [%s]. Array Id [%s]. Error[%s].' % (self.name, self.ssid, to_native(error)))
    ...
    def apply(self):
        ...
        if self.volume_detail:
            ...
            if self.thin_provision and (self.thin_volume_repo_size_b < 4 * 1024 ** 3 or self.thin_volume_repo_size_b > 256 * 1024 ** 3 or self.thin_volume_repo_size_b % (4 * 1024 ** 3) != 0):
                self.module.fail_json(msg='The initial thin volume repository size must be between 4gb and 256gb in increments of 4gb. Attempted size [%sg].' % (self.thin_volume_repo_size_b * 1024 ** 3))
            change = True
