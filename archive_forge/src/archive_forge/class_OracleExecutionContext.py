...
class OracleExecutionContext(default.DefaultExecutionContext):
    ...
    def fire_sequence(self, seq, type_):
        return self._execute_scalar('SELECT ' + self.identifier_preparer.format_sequence(seq) + '.nextval FROM DUAL', type_)

    def pre_exec(self):
        if self.statement and '_oracle_dblink' in self.execution_options:
            self.statement = self.statement.replace(dictionary.DB_LINK_PLACEHOLDER, self.execution_options['_oracle_dblink'])