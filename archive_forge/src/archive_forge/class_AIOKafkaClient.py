...
class AIOKafkaClient:
    ...
    def __init__(self, *, loop=None, bootstrap_servers='localhost', client_id='aiokafka-' + __version__, metadata_max_age_ms=300000, request_timeout_ms=40000, retry_backoff_ms=100, ssl_context=None, security_protocol='PLAINTEXT', api_version='auto', connections_max_idle_ms=540000, sasl_mechanism='PLAIN', sasl_plain_username=None, sasl_plain_password=None, sasl_kerberos_service_name='kafka', sasl_kerberos_domain_name=None, sasl_oauth_token_provider=None):
        if loop is None:
        ...
        if security_protocol in ['SASL_SSL', 'SASL_PLAINTEXT']:
            if sasl_mechanism not in ('PLAIN', 'GSSAPI', 'SCRAM-SHA-256', 'SCRAM-SHA-512', 'OAUTHBEARER'):
                raise ValueError('only `PLAIN`, `GSSAPI`, `SCRAM-SHA-256`, `SCRAM-SHA-512` and `OAUTHBEARER`sasl_mechanism are supported at the moment')
            if sasl_mechanism == 'PLAIN' and (sasl_plain_username is None or sasl_plain_password is None):
                raise ValueError('sasl_plain_username and sasl_plain_password required for PLAIN sasl')
    ...
    async def bootstrap(self):
        ...
        for host, port, _ in self.hosts:
            ...
            try:
                bootstrap_conn = await create_conn(host, port, client_id=self._client_id, request_timeout_ms=self._request_timeout_ms, ssl_context=self._ssl_context, security_protocol=self._security_protocol, max_idle_ms=self._connections_max_idle_ms, sasl_mechanism=self._sasl_mechanism, sasl_plain_username=self._sasl_plain_username, sasl_plain_password=self._sasl_plain_password, sasl_kerberos_service_name=self._sasl_kerberos_service_name, sasl_kerberos_domain_name=self._sasl_kerberos_domain_name, sasl_oauth_token_provider=self._sasl_oauth_token_provider, version_hint=version_hint)
            ...
            try:
                ...
                log.warning('Unable to request metadata from "%s:%s": %s', host, port, err)
    ...
    async def _md_synchronizer(self):
        ...
        while True:
            await asyncio.wait([self._md_update_waiter], timeout=self._metadata_max_age_ms / 1000)
            topics = self._topics
    ...
    async def _metadata_update(self, cluster_metadata, topics):
        ...
        for node_id in nodeids:
            ...
            if conn is None:
                continue
            log.debug('Sending metadata request %s to node %s', metadata_request, node_id)
            try:
                metadata = await conn.send(metadata_request)
            except (KafkaError, asyncio.TimeoutError) as err:
                log.error('Unable to request metadata from node with id %s: %r', node_id, err)
                continue
    ...
    def _on_connection_closed(self, conn, reason):
        ...
        """
        if reason == CloseReason.CONNECTION_BROKEN or reason == CloseReason.CONNECTION_TIMEOUT:
    ...
    async def _get_conn(self, node_id, *, group=ConnectionGroup.DEFAULT, no_hint=False):
        ...
        try:
            if group == ConnectionGroup.DEFAULT:
                ...
                if broker is None:
                    raise StaleMetadata('Broker id %s not in current metadata' % node_id)
            else:
    ...
    async def send(self, node_id, request, *, group=ConnectionGroup.DEFAULT):
        ...
        if not await self.ready(node_id, group=group):
            raise NodeNotReadyError('Attempt to send a request to node which is not ready (node id {}).'.format(node_id))
        expect_response = True
        if isinstance(request, tuple(ProduceRequest)) and request.required_acks == 0:
            expect_response = False
        future = self._conns[node_id, group].send(request, expect_response=expect_response)
        try:
            result = await future
    ...
    async def check_version(self, node_id=None):
        ...
        if node_id is None:
            default_group_conns = [n_id for n_id, group in self._conns.keys() if group == ConnectionGroup.DEFAULT]
        ...
        from aiokafka.protocol.admin import ListGroupsRequest_v0, ApiVersionRequest_v0
        from aiokafka.protocol.commit import OffsetFetchRequest_v0, GroupCoordinatorRequest_v0
        from aiokafka.protocol.metadata import MetadataRequest_v0
        test_cases = [((0, 10), ApiVersionRequest_v0()), ((0, 9), ListGroupsRequest_v0()), ((0, 8, 2), GroupCoordinatorRequest_v0('aiokafka-default-group')), ((0, 8, 1), OffsetFetchRequest_v0('aiokafka-default-group', [])), ((0, 8, 0), MetadataRequest_v0([]))]
    ...
    async def coordinator_lookup(self, coordinator_type, coordinator_key):
        ...
        log.debug('Sending FindCoordinator request for key %s to broker %s', coordinator_key, node_id)
        if self.api_version > (0, 11):
            request = FindCoordinatorRequest[1](coordinator_key, coordinator_type)
        else:
            assert coordinator_type == CoordinationType.GROUP, 'No transactions for older brokers'
        ...
        if error_type is not Errors.NoError:
            err = error_type()
            raise err
        self.cluster.add_coordinator(resp.coordinator_id, resp.host, resp.port, rack=None, purpose=(coordinator_type, coordinator_key))
