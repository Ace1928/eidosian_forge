...
class Pep8Test(SanitySingleVersion):
    ...
    def test(self, args: SanityConfig, targets: SanityTargets, python: PythonConfig) -> TestResult:
        ...
        cmd = [python.path, '-m', 'pycodestyle', '--max-line-length', '160', '--config', '/dev/null', '--ignore', ','.join(sorted(current_ignore))] + paths
        if paths:
            try:
        ...
        if stdout:
            ...
            results = []
        messages = [SanityMessage(message=r['message'], path=r['path'], line=int(r['line']), column=int(r['column']), level='warning' if r['code'].startswith('W') else 'error', code=r['code']) for r in results]
