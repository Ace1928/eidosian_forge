...
class URIReference(namedtuple('URIReference', misc.URI_COMPONENTS), URIMixin):
    ...
    def normalize(self):
        ...
        :rtype: URIReference
        """
        return URIReference(normalizers.normalize_scheme(self.scheme or ''), normalizers.normalize_authority((self.userinfo, self.host, self.port)), normalizers.normalize_path(self.path or ''), normalizers.normalize_query(self.query), normalizers.normalize_fragment(self.fragment), self.encoding)
