...
def cudnn_rnnv3(input: _atypes.TensorFuzzingAnnotation[TV_CudnnRNNV3_T], input_h: _atypes.TensorFuzzingAnnotation[TV_CudnnRNNV3_T], input_c: _atypes.TensorFuzzingAnnotation[TV_CudnnRNNV3_T], params: _atypes.TensorFuzzingAnnotation[TV_CudnnRNNV3_T], sequence_lengths: _atypes.TensorFuzzingAnnotation[_atypes.Int32], rnn_mode: str='lstm', input_mode: str='linear_input', direction: str='unidirectional', dropout: float=0, seed: int=0, seed2: int=0, num_proj: int=0, is_training: bool=True, time_major: bool=True, name=None):
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'CudnnRNNV3', name, input, input_h, input_c, params, sequence_lengths, 'rnn_mode', rnn_mode, 'input_mode', input_mode, 'direction', direction, 'dropout', dropout, 'seed', seed, 'seed2', seed2, 'num_proj', num_proj, 'is_training', is_training, 'time_major', time_major)
            ...
            pass
        try:
            return cudnn_rnnv3_eager_fallback(input, input_h, input_c, params, sequence_lengths, rnn_mode=rnn_mode, input_mode=input_mode, direction=direction, dropout=dropout, seed=seed, seed2=seed2, num_proj=num_proj, is_training=is_training, time_major=time_major, name=name, ctx=_ctx)
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('CudnnRNNV3', input=input, input_h=input_h, input_c=input_c, params=params, sequence_lengths=sequence_lengths, rnn_mode=rnn_mode, input_mode=input_mode, direction=direction, dropout=dropout, seed=seed, seed2=seed2, num_proj=num_proj, is_training=is_training, time_major=time_major, name=name)
    _result = _outputs[:]
    if _execute.must_record_gradient():
        _attrs = ('T', _op._get_attr_type('T'), 'rnn_mode', _op.get_attr('rnn_mode'), 'input_mode', _op.get_attr('input_mode'), 'direction', _op.get_attr('direction'), 'dropout', _op.get_attr('dropout'), 'seed', _op._get_attr_int('seed'), 'seed2', _op._get_attr_int('seed2'), 'num_proj', _op._get_attr_int('num_proj'), 'is_training', _op._get_attr_bool('is_training'), 'time_major', _op._get_attr_bool('time_major'))
        _inputs_flat = _op.inputs
        _execute.record_gradient('CudnnRNNV3', _inputs_flat, _attrs, _result)
    _result = _CudnnRNNV3Output._make(_result)
    return _result