...
class CometLoggerCallback(LoggerCallback):
    ...
    """
    _exclude_autolog = ['auto_output_logging', 'log_git_metadata', 'log_git_patch', 'log_env_cpu', 'log_env_gpu']
    ...
    def __init__(self, online: bool=True, tags: List[str]=None, save_checkpoints: bool=False, **experiment_kwargs):
        _import_comet()
        self.online = online
        self.tags = tags
    ...
    def log_trial_save(self, trial: 'Trial'):
        ...
        if self.save_checkpoints and trial.checkpoint:
            experiment = self._trial_experiments[trial]
            artifact = comet_ml.Artifact(name=f'checkpoint_{str(trial)}', artifact_type='model')
