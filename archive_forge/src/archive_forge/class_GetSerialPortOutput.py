...
class GetSerialPortOutput(base.Command):
  ...
  """
    ...
    def Run(self, args):
        ...
        request = (client.apitools_client.instances, 'GetSerialPortOutput', client.messages.ComputeInstancesGetSerialPortOutputRequest(instance=instance_ref.Name(), project=instance_ref.project, port=args.port, start=args.start, zone=instance_ref.zone))
        errors = []
        objects = client.MakeRequests(requests=[request], errors_to_collect=errors)
        if errors:
            raise GetSerialPortOutputException('Could not fetch serial port output: ' + ','.join([error[1] for error in errors]))
        response = objects[0]
    ...
    def Epilog(self, unused_resources_were_displayed):
        if self._start and self._response.start != self._start:
            log.warning('Some serial port output was lost due to a limited buffer. The oldest byte of output returned was at offset {0}.'.format(self._response.start))
        log.status.Print('\nSpecify --start={0} in the next get-serial-port-output invocation to get only the new output starting from here.'.format(self._response.next))