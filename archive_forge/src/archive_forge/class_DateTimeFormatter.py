...
class DateTimeFormatter:
    _FORMAT_RE: Final[Pattern[str]] = re.compile('(\\[(?:(?=(?P<literal>[^]]))(?P=literal))*\\]|YYY?Y?|MM?M?M?|Do|DD?D?D?|d?dd?d?|HH?|hh?|mm?|ss?|SS?S?S?S?S?|ZZ?Z?|a|A|X|x|W)')
    ...
    def _format_token(self, dt: datetime, token: Optional[str]) -> Optional[str]:
        ...
        if token == 'x':
            return f'{dt.timestamp() * 1000000:.0f}'
        if token == 'ZZZ':
            return dt.tzname()
        if token in ['ZZ', 'Z']:
