...
class VoilaHandler(BaseVoilaHandler):
    ...
    async def get_generator(self, path=None):
        ...
        try:
            ...
            pool_size = 0
        if self.should_use_rendered_notebook(current_notebook_data, pool_size, template_arg, theme_arg, self.request.arguments):
            render_task, rendered_cache, kernel_id = await self.kernel_manager.get_rendered_notebook(notebook_name=notebook_path)
            ...
            supported_file_extensions.extend([x.lower() for x in self.voila_configuration.extension_language_mapping.keys()])
            file_extenstion = Path(notebook_path).suffix.lower()
            if file_extenstion not in supported_file_extensions:
                self.redirect_to_file(path)
                return
            gen = NotebookRenderer(request_handler=self, voila_configuration=self.voila_configuration, traitlet_config=self.traitlet_config, notebook_path=notebook_path, template_paths=self.template_paths, config_manager=self.config_manager, contents_manager=self.contents_manager, base_url=self.base_url, kernel_spec_manager=self.kernel_spec_manager, prelaunch_hook=self.prelaunch_hook)
            ...
            kernel_id = await ensure_async(self.kernel_manager.start_kernel(kernel_name=gen.notebook.metadata.kernelspec.name, path=cwd, env=kernel_env))
