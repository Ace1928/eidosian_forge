        ...
        for i, state in enumerate(state_batch):
            ...
            if not qml.math.is_abstract(state):
                if any((bit not in [0, 1] for bit in state)):
                    raise ValueError(f'Basis states must only consist of 0s and 1s; state {i} is {state}')
    ...
    def compute_decomposition(basis_state, wires):
        ...
        **Example**
        ...
        if len(qml.math.shape(basis_state)) > 1:
            raise ValueError('Broadcasting with BasisStatePreparation is not supported. Please use the qml.transforms.broadcast_expand transform to use broadcasting with BasisStatePreparation.')
