...
class RewritingParser(HTMLParser):
    ...
    def _get_text(self):
        try:
            return ''.join((t for t in self._content if not isinstance(t, tuple)))
        except UnicodeDecodeError as e:
            if self.data_is_str:
                e.reason += ' the form was passed in as a byte string, but some data or error messages were normal strings; the form should be passed in as a string'
            else:
                e.reason += ' the form was passed in as a string, but some data or error message was encoded; the data and error messages should be passed in as strings'
            raise