...
def crypto_aead_chacha20poly1305_ietf_decrypt(ciphertext: bytes, aad: Optional[bytes], nonce: bytes, key: bytes) -> bytes:
    ...
    clen = len(ciphertext)
    ensure(clen <= _aead_chacha20poly1305_ietf_CRYPTBYTES_MAX, 'Ciphertext must be at most {} bytes long'.format(_aead_chacha20poly1305_ietf_CRYPTBYTES_MAX), raising=exc.ValueError)
    ...
    if aad:
        ...
        _aad = ffi.NULL
        aalen = 0
    res = lib.crypto_aead_chacha20poly1305_ietf_decrypt(message, mlen, ffi.NULL, ciphertext, clen, _aad, aalen, nonce, key)
