...
class DPOTrainerTester(unittest.TestCase):
    ...
    def test_dpo_trainer_padding_token_is_none(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            with self.assertRaisesRegex(ValueError, expected_regex='Padding is enabled, but the tokenizer is not configured with a padding token. Explicitly set `tokenizer.pad_token` \\(e.g. `tokenizer.pad_token = tokenizer.eos_token`\\) before calling the trainer.'):
                trainer = DPOTrainer(model=self.model, ref_model=None, beta=0.1, args=training_args, tokenizer=tokenizer, train_dataset=dummy_dataset, eval_dataset=dummy_dataset)
                trainer.train()
    ...
    def test_dpo_lora_tags(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            training_args = TrainingArguments(output_dir=tmp_dir, per_device_train_batch_size=2, max_steps=3, remove_unused_columns=False, gradient_accumulation_steps=4, learning_rate=0.9, evaluation_strategy='steps')
            dummy_dataset = self._init_dummy_dataset()
            trainer = DPOTrainer(model=model, ref_model=None, beta=0.1, args=training_args, tokenizer=tokenizer, train_dataset=dummy_dataset, eval_dataset=dummy_dataset, peft_config=lora_config)
