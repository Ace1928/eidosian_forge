...
class Snake:
    def __init__(self):
        ...
        self.path: List[Tuple[int, int]] = self._a_star_search_helper(
            self.body[0], self.fruit.position, is_return_path=False
        )
    ...
    def heuristic(
        ...
        for dx, dy in [
            ...
            if (
                ...
                and neighbor not in self.body
                and neighbor not in self.obstacles
            ):
                available_neighbors += 1
        escape_route_bonus = (
            available_neighbors * -2 * (len(self.body) / (BLOCK_SIZE**2))
        )
        ...
        if is_return_path:
            tail_distance = math.sqrt(
                (a[0] - self.body[-1][0]) ** 2 + (a[1] - self.body[-1][1]) ** 2
            )
        ...
        if not is_return_path:
            food_distance = math.sqrt(
                (a[0] - self.fruit.position[0]) ** 2
                + (a[1] - self.fruit.position[1]) ** 2
            )
    ...
    def a_star_search(
        ...
        path_to_tail = self._a_star_search_helper(
    ...
    def move(self) -> bool:
        ...
        """
        if not self.path:
            self.path = self._a_star_search_helper(
                self.body[0], self.fruit.position, is_return_path=False
            )
        if self.path:
