...
class NestedStack(stack_resource.StackResource):
    ...
    PROPERTIES = TEMPLATE_URL, TIMEOUT_IN_MINS, PARAMETERS = ('TemplateURL', 'TimeoutInMinutes', 'Parameters')
    properties_schema = {TEMPLATE_URL: properties.Schema(properties.Schema.STRING, _('The URL of a template that specifies the stack to be created as a resource.'), required=True, update_allowed=True), TIMEOUT_IN_MINS: properties.Schema(properties.Schema.INTEGER, _('The length of time, in minutes, to wait for the nested stack creation.'), update_allowed=True), PARAMETERS: properties.Schema(properties.Schema.MAP, _('The set of parameters passed to this nested stack.'), update_allowed=True)}
    ...
    def child_template(self):
        ...
        try:
            ...
            raise ValueError(_("Could not fetch remote template '%(url)s': %(exc)s") % {'url': template_url, 'exc': r_exc})
    ...
    def handle_update(self, json_snippet, tmpl_diff, prop_diff):
        ...
        return self.update_with_template(self.child_template(), self.properties[self.PARAMETERS], self.properties[self.TIMEOUT_IN_MINS])