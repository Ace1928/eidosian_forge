...
from .._utils import (
def output_cotransformer(**validation_rules: Any) -> Callable[[Any], '_FuncAsCoTransformer']:
    ...
    def deco(func: Callable) -> '_FuncAsOutputCoTransformer':
        assert_or_throw(not is_class_method(func), NotImplementedError("output_cotransformer decorator can't be used on class methods"))
        return _FuncAsOutputCoTransformer.from_func(func, schema=None, validation_rules=to_validation_rules(validation_rules))
    return deco