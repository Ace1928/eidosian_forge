...
@classmethod
def _create_distinct(cls, expr: _ColumnExpressionArgument[_T]) -> UnaryExpression[_T]:
    col_expr: ColumnElement[_T] = coercions.expect(roles.ExpressionElementRole, expr)
    return UnaryExpression(col_expr, operator=operators.distinct_op, type_=col_expr.type, wraps_column_expression=False)