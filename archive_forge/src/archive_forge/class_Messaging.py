...
class Messaging(base.Driver):
    ...
    def __init__(self, connection_str, project=None, service=None, host=None, context=None, conf=None, transport_url=None, idle_timeout=1, **kwargs):
        ...
        if not self.oslo_messaging:
            raise ValueError('Oslo.messaging library is required for messaging driver')
        super(Messaging, self).__init__(connection_str, project=project, service=service, host=host)
        self.context = context
        if not conf:
            oslo_config = importutils.try_import('oslo_config')
            if not oslo_config:
                raise ValueError('Oslo.config library is required for messaging driver')
            conf = oslo_config.cfg.CONF
        transport_kwargs = {}
        if transport_url:
            transport_kwargs['url'] = transport_url
        self.transport = self.oslo_messaging.get_notification_transport(conf, **transport_kwargs)
        self.client = self.oslo_messaging.Notifier(self.transport, publisher_id=self.host, driver='messaging', topics=['profiler'], retry=0)
    ...
    def notify(self, info, context=None):
        ...
        self.client.info(context or self.context, 'profiler.%s' % info['service'], info)
    ...
    def get_report(self, base_id):
        ...
        server = self.oslo_messaging.notify.get_notification_listener(self.transport, targets, endpoints, executor='threading')
        ...
        try:
            ...
            print('Execution interrupted while trying to connect to messaging server. No data was collected.')
            return {}
        try:
            state['running'] = True
            while state['running']:
        ...
        if not events:
            print('No events are collected for Trace UUID %s. Please note that osprofiler has read ALL events from profiler topic, but has not found any for specified Trace UUID.' % base_id)
        for n in events:
            trace_id = n['trace_id']
