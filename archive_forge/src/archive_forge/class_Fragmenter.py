...
class Fragmenter(MSONable):
    ...
    def __init__(self, molecule: Molecule, edges: list | None=None, depth: int=1, open_rings: bool=False, use_metal_edge_extender: bool=False, opt_steps: int=10000, prev_unique_frag_dict: dict | None=None, assume_previous_thoroughness: bool=True):
        """
        ...
        if depth == 0:
            ...
            for level in range(depth):
                if level == 0:
                    ...
                    self.fragments_by_level['0'] = self._fragment_one_level({f'{alph_formula} E{len(self.mol_graph.graph.edges())}': [self.mol_graph]})
                else:
