...
class IPKernelApp(BaseIPythonApplication, InteractiveShellApp, ConnectionFileMixin):
    ...
    kernel_class = Type('ipykernel.ipkernel.IPythonKernel', klass='ipykernel.kernelbase.Kernel', help="The Kernel subclass to be used.\n\n    This should allow easy reuse of the IPKernelApp entry point\n    to configure and launch kernels other than IPython's own.\n    ").tag(config=True)
    kernel = Any()
    poller = Any()
    ...
    control_thread = Any()
    _ports = Dict()
    subcommands = {'install': ('ipykernel.kernelspec.InstallIPythonKernelSpecApp', 'Install the IPython kernel')}
    ...
    outstream_class = DottedObjectName('ipykernel.iostream.OutStream', help='The importstring for the OutStream factory', allow_none=True).tag(config=True)
    displayhook_class = DottedObjectName('ipykernel.displayhook.ZMQDisplayHook', help='The importstring for the DisplayHook factory').tag(config=True)
    capture_fd_output = Bool(True, help='Attempt to capture and forward low-level output, e.g. produced by Extension libraries.\n    ').tag(config=True)
    parent_handle = Integer(int(os.environ.get('JPY_PARENT_PID') or 0), help='kill this process if its parent dies.  On Windows, the argument\n        specifies the HANDLE of the parent process, otherwise it is simply boolean.\n        ').tag(config=True)
    interrupt = Integer(int(os.environ.get('JPY_INTERRUPT_EVENT') or 0), help='ONLY USED ON WINDOWS\n        Interrupt this process when the parent is signaled.\n        ').tag(config=True)
    ...
    def write_connection_file(self):
        ...
        connection_info = dict(ip=self.ip, key=self.session.key, transport=self.transport, shell_port=self.shell_port, stdin_port=self.stdin_port, hb_port=self.hb_port, iopub_port=self.iopub_port, control_port=self.control_port)
        if Path(cf).exists():
    ...
    def init_connection_file(self):
        ...
        try:
            ...
            Path(self.abs_connection_file).parent.mkdir(mode=448, exist_ok=True, parents=True)
    ...
    def log_connection_info(self):
        ...
        lines = ['To connect another client to this kernel, use:', '    --existing %s' % tail]
        for line in lines:
            self.log.info(line)
        ...
        self._ports = dict(shell=self.shell_port, iopub=self.iopub_port, stdin=self.stdin_port, hb=self.hb_port, control=self.control_port)
            ...
            if hasattr(sys.stderr, '_original_stdstream_copy'):
                for handler in self.log.handlers:
                    if isinstance(handler, StreamHandler) and handler.stream.buffer.fileno() == 2:
                        ...
                        handler.stream = TextIOWrapper(FileIO(sys.stderr._original_stdstream_copy, 'w'))
