...
class DigitalOcean_v2_NodeDriver(DigitalOcean_v2_BaseDriver, DigitalOceanNodeDriver):
    ...
    """
    NODE_STATE_MAP = {'new': NodeState.PENDING, 'off': NodeState.STOPPED, 'active': NodeState.RUNNING, 'archive': NodeState.TERMINATED}
    ...
    def create_node(self, name, size, image, location, ex_create_attr=None, ex_ssh_key_ids=None, ex_user_data=None):
        """
        Create a node.
        ...
        * `ssh_keys`: ``list`` of ``int`` key ids or ``str`` fingerprints
        ...
        :rtype: :class:`Node`
        """
        attr = {'name': name, 'size': size.name, 'image': image.id, 'region': location.id, 'user_data': ex_user_data}
        if ex_ssh_key_ids:
            warnings.warn('The ex_ssh_key_ids parameter has been deprecated in favor of the ex_create_attr parameter.')
    ...
    def attach_volume(self, node, volume, device=None):
        ...
        :rytpe: ``bool``
        """
        attr = {'type': 'attach', 'droplet_id': node.id, 'volume_name': volume.name, 'region': volume.extra['region_slug']}
    ...
    def detach_volume(self, volume):
        ...
        :rtype: ``bool``
        """
        attr = {'type': 'detach', 'volume_name': volume.name, 'region': volume.extra['region_slug']}
        responses = []
    ...
    def create_volume_snapshot(self, volume, name):
        ...
        """
        attr = {'name': name}
        res = self.connection.request('/v2/volumes/%s/snapshots' % volume.id, data=json.dumps(attr), method='POST')
    ...
    def _to_node(self, data):
        extra_keys = ['memory', 'vcpus', 'disk', 'image', 'size', 'size_slug', 'locked', 'created_at', 'networks', 'kernel', 'backup_ids', 'snapshot_ids', 'features', 'tags']
        if 'status' in data:
        ...
        if resolve_data:
            ...
            size = None
        node = Node(id=data['id'], name=data['name'], state=state, image=image, size=size, public_ips=public_ips, private_ips=private_ips, created_at=created, driver=self, extra=extra)
        return node

    def _to_image(self, data):
        extra = {'distribution': data['distribution'], 'public': data['public'], 'slug': data['slug'], 'regions': data['regions'], 'min_disk_size': data['min_disk_size'], 'created_at': data['created_at']}
        return NodeImage(id=data['id'], name=data['name'], driver=self, extra=extra)

    def _to_volume(self, data):
        extra = {'created_at': data['created_at'], 'droplet_ids': data['droplet_ids'], 'region': data['region'], 'region_slug': data['region']['slug']}
        return StorageVolume(id=data['id'], name=data['name'], size=data['size_gigabytes'], driver=self, extra=extra)

    def _to_location(self, data):
        extra = data.get('features', [])
        return NodeLocation(id=data['slug'], name=data['name'], country=None, extra=extra, driver=self)

    def _to_size(self, data):
        extra = {'vcpus': data['vcpus'], 'regions': data['regions'], 'price_monthly': data['price_monthly']}
        return NodeSize(id=data['slug'], name=data['slug'], ram=data['memory'], disk=data['disk'], bandwidth=data['transfer'], price=data['price_hourly'], driver=self, extra=extra)

    def _to_key_pair(self, data):
        extra = {'id': data['id']}
        return KeyPair(name=data['name'], fingerprint=data['fingerprint'], public_key=data['public_key'], private_key=None, driver=self, extra=extra)
