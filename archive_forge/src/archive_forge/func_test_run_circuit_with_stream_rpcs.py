...
def test_run_circuit_with_stream_rpcs(client):
    ...
    engine = cg.Engine(project_id='proj', context=EngineContext(service_args={'client_info': 1}, enable_streaming=True))
    ...
    client().run_job_over_stream.assert_called_once_with(project_id='proj', program_id='prog', code=mock.ANY, job_id='job-id', run_context=util.pack_any(v2.run_context_pb2.RunContext(parameter_sweeps=[v2.run_context_pb2.ParameterSweep(repetitions=1)])), program_description=None, program_labels=None, job_description=None, job_labels=None, processor_id='mysim', run_name='', device_config_name='')