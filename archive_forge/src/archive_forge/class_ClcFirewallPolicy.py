...
class ClcFirewallPolicy:
    ...
    def process_request(self):
        ...
        self.firewall_dict = {'location': location, 'source_account_alias': source_account_alias, 'destination_account_alias': destination_account_alias, 'firewall_policy_id': firewall_policy_id, 'ports': ports, 'source': source, 'destination': destination, 'wait': wait, 'state': state, 'enabled': enabled}
        self._set_clc_credentials_from_env()
        if state == 'absent':
            changed, firewall_policy_id, firewall_policy = self._ensure_firewall_policy_is_absent(source_account_alias, location, self.firewall_dict)
        elif state == 'present':
            changed, firewall_policy_id, firewall_policy = self._ensure_firewall_policy_is_present(source_account_alias, location, self.firewall_dict)
        return self.module.exit_json(changed=changed, firewall_policy_id=firewall_policy_id, firewall_policy=firewall_policy)
    ...
    def _delete_firewall_policy(self, source_account_alias, location, firewall_policy_id):
        """
        ...
        """
        try:
            response = self.clc.v2.API.Call('DELETE', '/v2-experimental/firewallPolicies/%s/%s/%s' % (source_account_alias, location, firewall_policy_id))
        except APIFailedResponse as e:
            return self.module.fail_json(msg='Unable to delete the firewall policy id : {0}. {1}'.format(firewall_policy_id, str(e.response_text)))
        return response
    ...
    def _get_firewall_policy(self, source_account_alias, location, firewall_policy_id):
        """
        ...
        response = None
        try:
            response = self.clc.v2.API.Call('GET', '/v2-experimental/firewallPolicies/%s/%s/%s' % (source_account_alias, location, firewall_policy_id))
        except APIFailedResponse as e:
            if e.response_status_code != 404:
                self.module.fail_json(msg='Unable to fetch the firewall policy with id : {0}. {1}'.format(firewall_policy_id, str(e.response_text)))
        return response
