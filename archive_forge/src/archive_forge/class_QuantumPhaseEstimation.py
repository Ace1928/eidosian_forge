...
class QuantumPhaseEstimation(Operation):
    ...
    def __init__(self, unitary, target_wires=None, estimation_wires=None, id=None):
        if isinstance(unitary, Operator):
            if target_wires is not None:
                raise qml.QuantumFunctionError('The unitary is expressed as an operator, which already has target wires defined, do not additionally specify target wires.')
            target_wires = unitary.wires
        elif target_wires is None:
            raise qml.QuantumFunctionError('Target wires must be specified if the unitary is expressed as a matrix.')
        else:
        ...
        if any((wire in target_wires for wire in estimation_wires)):
            raise qml.QuantumFunctionError('The target wires and estimation wires must not overlap.')
        self._hyperparameters = {'unitary': unitary, 'target_wires': target_wires, 'estimation_wires': estimation_wires}
    ...
    def map_wires(self, wire_map: dict):
        ...
        new_op._hyperparameters['estimation_wires'] = [wire_map.get(wire, wire) for wire in self.estimation_wires]
        new_op._hyperparameters['target_wires'] = [wire_map.get(wire, wire) for wire in self.target_wires]
        return new_op
