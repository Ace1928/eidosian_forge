...
def compress_and_encrypt_image_data(
    ...
    try:
        ...
        logging.debug(
            f"Compression and encryption successful with checksum. Original image format: {image_format}, Checksum: {checksum}"
        )
...
def decrypt_and_decompress_image_data(
    ...
    try:
        ...
        logging.debug(
            f"Decryption and decompression successful with checksum verification. Decompressed image format: {image_format.decode()}, Verified Checksum: {verify_checksum(image_data, checksum.decode())}"
        )
