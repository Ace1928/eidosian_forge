...
class AtmosDriver(StorageDriver):
    ...
    def get_object(self, container_name, object_name):
        ...
        return Object(object_name, int(system_meta['size']), data_hash, extra, user_meta, container, self)
    ...
    def upload_object(self, file_path, container, object_name, extra=None, verify_hash=True, headers=None):
        ...
        try:
            ...
            if e.code != 1003:
                raise
            method = 'POST'
        result_dict = self._upload_object(object_name=object_name, content_type=content_type, request_path=request_path, request_method=method, headers={}, file_path=file_path)
        ...
        self.connection.request(request_path + '?metadata/user', method='POST', headers={'x-emc-meta': user_meta})
        result = self.connection.request(request_path + '?metadata/system')
        meta = self._emc_meta(result)
        del meta_data['md5']
        extra = {'object_id': meta['objectid'], 'meta_data': meta_data}
        return Object(object_name, bytes_transferred, result_dict['data_hash'], extra, meta_data, container, self)
    ...
    def upload_object_via_stream(self, iterator, container, object_name, extra=None, headers=None):
        ...
        while True:
            ...
            headers.update({'x-emc-meta': 'md5=' + data_hash.hexdigest(), 'Content-Type': content_type})
    ...
    def get_object_cdn_url(self, obj, expiry=None, use_object=False):
        ...
        params = [('uid', self.key), ('expires', expiry)]
