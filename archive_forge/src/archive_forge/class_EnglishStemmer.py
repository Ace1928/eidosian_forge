...
class EnglishStemmer(_StandardStemmer):
    ...
    __step2_suffixes = ('ization', 'ational', 'fulness', 'ousness', 'iveness', 'tional', 'biliti', 'lessli', 'entli', 'ation', 'alism', 'aliti', 'ousli', 'iviti', 'fulli', 'enci', 'anci', 'abli', 'izer', 'ator', 'alli', 'bli', 'ogi', 'li')
    __step3_suffixes = ('ational', 'tional', 'alize', 'icate', 'iciti', 'ative', 'ical', 'ness', 'ful')
    __step4_suffixes = ('ement', 'ance', 'ence', 'able', 'ible', 'ment', 'ant', 'ent', 'ism', 'ate', 'iti', 'ous', 'ive', 'ize', 'ion', 'al', 'er', 'ic')
    ...
    def stem(self, word):
                ...
                if suffix in ('eed', 'eedly'):
                    ...
                    if step1b_vowel_found:
                        ...
                        if word.endswith(('at', 'bl', 'iz')):
                            ...
                            r1 = r1[:-1]
                            r2 = r2[:-1]
                        elif r1 == '' and len(word) >= 3 and (word[-1] not in self.__vowels) and (word[-1] not in 'wxY') and (word[-2] in self.__vowels) and (word[-3] not in self.__vowels) or (r1 == '' and len(word) == 2 and (word[0] in self.__vowels) and (word[1] not in self.__vowels)):
                            word = ''.join((word, 'e'))
                            if len(r1) > 0:
                                r1 = ''.join((r1, 'e'))
                            if len(r2) > 0:
        ...
        if r2.endswith('l') and word[-2] == 'l':
            ...
            if len(word) >= 4 and (word[-2] in self.__vowels or word[-2] in 'wxY' or word[-3] not in self.__vowels or (word[-4] in self.__vowels)):
                word = word[:-1]
