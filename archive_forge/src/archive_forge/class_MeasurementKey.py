...
class MeasurementKey:
    ...
    def __post_init__(self):
        ...
        if MEASUREMENT_KEY_SEPARATOR in self.name:
            raise ValueError(f'Invalid key name: {self.name}\n{MEASUREMENT_KEY_SEPARATOR} is not allowed in MeasurementKey. If this is a nested key string, use `MeasurementKey.parse_serialized` for correct behavior.')
    ...
    def __str__(self):
        if self._str is None:
            object.__setattr__(self, '_str', MEASUREMENT_KEY_SEPARATOR.join(self.path + (self.name,)))
        return self._str
