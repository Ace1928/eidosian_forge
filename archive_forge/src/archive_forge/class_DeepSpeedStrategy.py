...
class DeepSpeedStrategy(DDPStrategy, _Sharded):
    ...
    @override
    def setup_module_and_optimizers(self, module: Module, optimizers: List[Optimizer]) -> Tuple['DeepSpeedEngine', List[Optimizer]]:
        ...
        """
        if len(optimizers) != 1:
            raise ValueError(f'Currently only one optimizer is supported with DeepSpeed. Got {len(optimizers)} optimizers instead.')
    ...
    def module_init_context(self, empty_init: Optional[bool]=None) -> ContextManager:
        if self.zero_stage_3 and empty_init is False:
            raise NotImplementedError(f'`empty_init={empty_init!r}` is not a valid choice with `DeepSpeedStrategy` when ZeRO stage 3 is enabled.')
    ...
    def module_sharded_context(self) -> ContextManager:
        ...
        return deepspeed.zero.Init(enabled=self.zero_stage_3, remote_device=self.remote_device, config_dict_or_path=self.config)
    ...
    def save_checkpoint(self, path: _PATH, state: Dict[str, Union[Module, Optimizer, Any]], storage_options: Optional[Any]=None, filter: Optional[Dict[str, Callable[[str, Any], bool]]]=None) -> None:
        ...
        if storage_options is not None:
            raise TypeError('`DeepSpeedStrategy.save_checkpoint(..., storage_options=...)` is not supported because `DeepSpeedStrategy` does not use the `CheckpointIO`.')
        if filter is not None:
            raise TypeError('`DeepSpeedStrategy.save_checkpoint(..., filter=...)` is not supported because `DeepSpeedStrategy` manages the state serialization internally.')
        engines = _get_deepspeed_engines_from_state(state)
        if len(engines) == 0:
            raise ValueError("Could not find a DeepSpeed model in the provided checkpoint state. Please provide the model as part of the state like so: `save_checkpoint(..., state={'model': model, ...})`. Make sure you set up the model (and optimizers if any) through the strategy before saving the checkpoint.")
        if len(engines) > 1:
            raise ValueError('Found multiple DeepSpeed engine modules in the given state. Saving checkpoints with DeepSpeed is currently limited to a single model per checkpoint. To save multiple models, call the save method for each model separately with a different path.')
        engine = engines[0]
    ...
    def load_checkpoint(self, path: _PATH, state: Optional[Union[Module, Optimizer, Dict[str, Union[Module, Optimizer, Any]]]]=None, strict: bool=True) -> Dict[str, Any]:
        ...
        if not state:
            raise ValueError(f"Got DeepSpeedStrategy.load_checkpoint(..., state={state!r}) but a state with at least  a model instance to reload is required. Pass it in like so: DeepSpeedStrategy.load_checkpoint(..., state={{'model': model, ...}})")
        _validate_checkpoint_directory(path)
        engines = _get_deepspeed_engines_from_state(state)
        if len(engines) == 0:
            raise ValueError("Could not find a DeepSpeed model in the provided checkpoint state. Please provide the model as part of the state like so: `load_checkpoint(..., state={'model': model, ...})`. Make sure you set up the model (and optimizers if any) through the strategy before loading the checkpoint.")
        if len(engines) > 1:
            raise ValueError('Found multiple DeepSpeed engine modules in the given state. Saving and loading checkpoints with DeepSpeed is currently limited to a single model per checkpoint. To load multiple model states, call the load method for each model checkpoint separately.')
        engine = engines[0]
        from deepspeed.runtime import DeepSpeedOptimizer
        optimzer_state_requested = any((isinstance(item, (Optimizer, DeepSpeedOptimizer)) for item in state.values()))
        torch.cuda.empty_cache()
        _, client_state = engine.load_checkpoint(path, tag='checkpoint', load_optimizer_states=optimzer_state_requested, load_lr_scheduler_states=False, load_module_strict=strict)
        if client_state is None:
            raise RuntimeError('DeepSpeed was unable to load the checkpoint. Ensure you passed in a DeepSpeed compatible checkpoint or a single checkpoint file by setting `DeepSpeedStrategy(..., load_full_weights=True)`.')
        keys = set(client_state) & set(state) - {'optimizer', 'lr_scheduler'}
        _move_state_into(source=client_state, destination=state, keys=keys)
        return client_state

    @override
    def clip_gradients_norm(self, module: 'DeepSpeedEngine', optimizer: Optimizer, max_norm: Union[float, int], norm_type: Union[float, int]=2.0, error_if_nonfinite: bool=True) -> torch.Tensor:
        raise NotImplementedError('DeepSpeed handles gradient clipping automatically within the optimizer. Make sure to set the `gradient_clipping` value in your Config.')

    @override
    def clip_gradients_value(self, module: 'DeepSpeedEngine', optimizer: Optimizer, clip_val: Union[float, int]) -> None:
        raise NotImplementedError('DeepSpeed handles gradient clipping automatically within the optimizer. Make sure to set the `gradient_clipping` value in your Config.')
    ...
    def register_strategies(cls, strategy_registry: _StrategyRegistry) -> None:
        ...
        strategy_registry.register('deepspeed_stage_1_offload', cls, description='DeepSpeed with ZeRO Stage 1 and optimizer CPU Offload', stage=1, offload_optimizer=True)
        strategy_registry.register('deepspeed_stage_2', cls, description='DeepSpeed with ZeRO Stage 2 enabled', stage=2)
        strategy_registry.register('deepspeed_stage_2_offload', cls, description='DeepSpeed ZeRO Stage 2 and CPU Offload', stage=2, offload_optimizer=True)
        strategy_registry.register('deepspeed_stage_3', cls, description='DeepSpeed ZeRO Stage 3', stage=3)
        strategy_registry.register('deepspeed_stage_3_offload', cls, description='DeepSpeed ZeRO Stage 3 and CPU Offload', stage=3, offload_optimizer=True, offload_parameters=True)
        strategy_registry.register('deepspeed_stage_3_offload_nvme', cls, description='DeepSpeed ZeRO Stage 3 and NVMe Offload', stage=3, offload_optimizer=True, offload_parameters=True, remote_device='nvme', offload_params_device='nvme', offload_optimizer_device='nvme')
    ...
    def setup_environment(self) -> None:
        if not isinstance(self.accelerator, CUDAAccelerator):
            raise RuntimeError(f'The DeepSpeed strategy is only supported on CUDA GPUs but `{self.accelerator.__class__.__name__}` is used.')
    ...
    def _load_config(self, config: Optional[Union[_PATH, Dict[str, Any]]]) -> Optional[Dict[str, Any]]:
        ...
        if isinstance(config, (str, Path)):
            if not os.path.isfile(config):
                raise FileNotFoundError(f'You passed in a path to a DeepSpeed config but the path does not exist: {config}')
