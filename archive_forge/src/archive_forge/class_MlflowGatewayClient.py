...
class MlflowGatewayClient:
    ...
    def _call_endpoint(self, method: str, route: str, json_body: Optional[str]=None):
        ...
        response = http_request(host_creds=self._host_creds, endpoint=route, method=method, timeout=MLFLOW_GATEWAY_CLIENT_QUERY_TIMEOUT_SECONDS, retry_codes=MLFLOW_GATEWAY_CLIENT_QUERY_RETRY_CODES, raise_on_status=False, **call_kwargs)
    ...
    def search_routes(self, page_token: Optional[str]=None) -> PagedList[Route]:
        ...
        response_json = self._call_endpoint('GET', MLFLOW_GATEWAY_CRUD_ROUTE_BASE, json_body=json.dumps(request_parameters)).json()
        routes = [Route(**{**resp, 'route_url': resolve_route_url(self._gateway_uri, resp['route_url'])}) for resp in response_json.get('routes', [])]
        next_page_token = response_json.get('next_page_token')
        return PagedList(routes, next_page_token)

    @gateway_deprecated
    def create_route(self, name: str, route_type: Optional[str]=None, model: Optional[Dict[str, Any]]=None) -> Route:
        """
        ...
        if not self._is_databricks_host():
            raise MlflowException('The create_route API is only available when running within Databricks. Route creation is handled through creating a configuration YAML file during startup or through updating a running Gateway server.', error_code=BAD_REQUEST)
        payload = {'name': name, 'route_type': route_type, 'model': model}
        response = self._call_endpoint('POST', MLFLOW_GATEWAY_CRUD_ROUTE_BASE, json.dumps(payload)).json()
        return Route(**response)
    ...
    def query(self, route: str, data: Dict[str, Any]):
        ...
        try:
            ...
            if isinstance(e.__cause__, requests.exceptions.Timeout):
                timeout_message = f'The provider has timed out while generating a response to your query. Please evaluate the available parameters for the query that you are submitting. Some parameter values and inputs can increase the computation time beyond the allowable route timeout of {MLFLOW_GATEWAY_CLIENT_QUERY_TIMEOUT_SECONDS} seconds.'
    ...
    def set_limits(self, route: str, limits: List[Dict[str, Any]]) -> LimitsConfig:
        ...
        """
        payload = {'route': route, 'limits': limits}
        response = self._call_endpoint('POST', MLFLOW_GATEWAY_LIMITS_BASE, json.dumps(payload)).json()
