...
class NamedTraitObserver:
    ...
    def __hash__(self):
        return hash((type(self).__name__, self.name, self.notify, self.optional))

    def __eq__(self, other):
        return type(self) is type(other) and self.name == other.name and (self.notify == other.notify) and (self.optional == other.optional)

    def __repr__(self):
        formatted_args = [f'name={self.name!r}', f'notify={self.notify!r}', f'optional={self.optional!r}']
    ...
    def get_notifier(self, handler, target, dispatcher):
        ...
        """
        return TraitEventNotifier(handler=handler, target=target, dispatcher=dispatcher, event_factory=trait_event_factory, prevent_event=ctrait_prevent_event)
    ...
    def get_maintainer(self, graph, handler, target, dispatcher):
        ...
        """
        return ObserverChangeNotifier(observer_handler=observer_change_handler, event_factory=trait_event_factory, prevent_event=lambda event: False, graph=graph, handler=handler, target=target, dispatcher=dispatcher)
