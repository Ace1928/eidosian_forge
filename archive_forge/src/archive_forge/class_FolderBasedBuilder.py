...
class FolderBasedBuilder(datasets.GeneratorBasedBuilder):
    ...
    def _split_generators(self, dl_manager):
            ...
            if os.path.isfile(downloaded_files_or_dirs[0]):
                ...
                for original_file, downloaded_file in zip(original_files, downloaded_files):
                    ...
                    if original_file_ext.lower() in self.EXTENSIONS:
                        ...
                        logger.debug(f"The file '{original_file_name}' was ignored: it is not an image, and is not {self.METADATA_FILENAMES} either.")
            else:
                ...
                for archive, downloaded_dir in zip(archives, downloaded_dirs):
                    ...
                    for downloaded_dir_file in dl_manager.iter_files(downloaded_dir):
                        ...
                        if downloaded_dir_file_ext in self.EXTENSIONS:
                            ...
                            logger.debug(f"The file '{original_file_name}' from the archive '{archive_file_name}' was ignored: it is not an {self.BASE_COLUMN_NAME}, and is not {self.METADATA_FILENAMES} either.")
        ...
        for split_name, files in data_files.items():
            ...
            splits.append(datasets.SplitGenerator(name=split_name, gen_kwargs={'files': list(zip(files, downloaded_files)) + [(None, dl_manager.iter_files(downloaded_dir)) for downloaded_dir in downloaded_dirs], 'metadata_files': metadata_files, 'split_name': split_name, 'add_labels': add_labels, 'add_metadata': add_metadata}))
        if add_metadata:
            features_per_metadata_file: List[Tuple[str, datasets.Features]] = []
            metadata_ext = {os.path.splitext(original_metadata_file)[-1] for original_metadata_file, _ in itertools.chain.from_iterable(metadata_files.values())}
        ...
        if self.config.features is None:
            ...
            if add_metadata:
                ...
                if duplicated_keys:
                    logger.warning(f'Ignoring metadata columns {list(duplicated_keys)} as they are already present in the features dictionary.')
                self.info.features.update({feature: metadata_features[feature] for feature in metadata_features if feature not in duplicated_keys})
        return splits
