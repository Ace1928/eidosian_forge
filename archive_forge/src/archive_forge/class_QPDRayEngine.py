...
class QPDRayEngine(QPDEngine):
    ...
    def to_native(self, df: DataFrame) -> Any:
        ...
        assert_or_throw(len(series) > 0, ValueError("DataFrame doesn't contain any series"))
        ddf = series[0].to_frame()
        if ddf.empty:
            assert_or_throw(len(values) == 0, ValueError('series are empty but values are not'))
    ...
    def group_agg(self, df: DataFrame, keys: List[str], agg_map: Dict[str, Tuple[str, AggFunctionSpec]]) -> DataFrame:
        if len(keys) > 0:
            return self._group_agg_with_keys(df, keys, agg_map)
        else:
            return self._group_agg_no_keys(df, agg_map)

    def window(self, df: DataFrame, func: WindowFunctionSpec, args: List[ArgumentSpec], dest_col_name: str) -> DataFrame:
        if func.name == 'row_number':
            return self._window_row_number(df, func, args, dest_col_name)
        if func.name == 'sum':
            return self._window_simple_agg(lambda x: x.sum(min_count=1), df, func, args, dest_col_name)
        if func.name == 'min':
            return self._window_simple_agg(lambda x: x.min(), df, func, args, dest_col_name)
        if func.name == 'max':
            return self._window_simple_agg(lambda x: x.max(), df, func, args, dest_col_name)
        if func.name == 'count':
            return self._window_simple_agg(lambda x: x.count(), df, func, args, dest_col_name)
        if func.name in ['avg', 'mean']:
            return self._window_simple_agg(lambda x: x.mean(), df, func, args, dest_col_name)
        if func.name in ['rank']:
            return self._window_ranks(lambda x: x.rank(method='min'), df, func, args, dest_col_name)
        if func.name in ['dense_rank']:
            return self._window_ranks(lambda x: x.rank(method='dense'), df, func, args, dest_col_name)
        if func.name in ['percentile_rank', 'percent_rank']:
            ...
            def pct_rank(s: Any) -> Any:
                ...
                if m == 0:
                    m = 1.0
                return ranks / m
            return self._window_ranks(lambda x: pct_rank(x) if isinstance(x, pandas.Series) else x.apply(pct_rank), df, func, args, dest_col_name)
        if func.name == 'lead':
            return self._window_lead_lag(lambda x, delta, default: x.shift(-delta, fill_value=default), df, func, args, dest_col_name)
        if func.name == 'lag':
            return self._window_lead_lag(lambda x, delta, default: x.shift(delta, fill_value=default), df, func, args, dest_col_name)
    ...
    def _window_row_number(self, df: DataFrame, func: WindowFunctionSpec, args: List[ArgumentSpec], dest_col_name: str) -> DataFrame:
        ...
        assert_or_throw(not (func.has_order_by and (not func.has_partition_by)), ValueError('for ray engine, order by requires partition by'))
        assert_or_throw(func.has_partition_by, ValueError('for ray engine, partition by is required for row_number()'))
        keys = list(df.keys())
        ndf = self.to_native(df)
    ...
    def _window_ranks(self, rank_func: Callable, df: DataFrame, func: WindowFunctionSpec, args: List[ArgumentSpec], dest_col_name: str) -> DataFrame:
        ...
        assert_or_throw(not (func.has_order_by and (not func.has_partition_by)), ValueError('for ray engine, order by requires partition by'))
        assert_or_throw(func.has_partition_by, ValueError('for ray engine, partition by is required for ranking'))
        ndf = self.to_native(df)
        keys = list(df.keys())
        ndf, sort_keys, asc = self._prepare_sort(ndf, func.window.order_by)
        gp = self._safe_groupby(ndf, func.window.partition_keys)
    ...
    def _window_lead_lag(self, l_func: Callable, df: DataFrame, func: WindowFunctionSpec, args: List[ArgumentSpec], dest_col_name: str) -> DataFrame:
        assert_or_throw(not (func.has_order_by and (not func.has_partition_by)), ValueError('for ray engine, order by requires partition by'))
        assert_or_throw(func.has_partition_by, ValueError('for ray engine, partition by is required for lead/lag'))
    ...
    def _window_simple_agg(self, agg_func: Callable, df: DataFrame, func: WindowFunctionSpec, args: List[ArgumentSpec], dest_col_name: str) -> DataFrame:
        assert_or_throw(not (func.has_order_by and (not func.has_partition_by)), ValueError('for ray engine, order by requires partition by'))
        assert_or_throw(func.has_partition_by, ValueError('for ray engine, partition by is required for window aggregation'))
    ...
    def _agg_gp(self, key: str, func: AggFunctionSpec, gp: '_LazyGrouper') -> Any:
    ...
    def _count_unique(self, gp: '_LazyGrouper', cols: str, func: AggFunctionSpec) -> Any:
