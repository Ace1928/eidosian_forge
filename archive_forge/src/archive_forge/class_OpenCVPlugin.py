...
class OpenCVPlugin(PluginV3):
    ...
    def read(self, *, index: int=None, colorspace: Union[int, str]=None, flags: int=cv2.IMREAD_COLOR) -> np.ndarray:
    ...
    def iter(self, colorspace: Union[int, str]=None, flags: int=cv2.IMREAD_COLOR) -> np.ndarray:
    ...
    def write(self, ndimage: Union[ArrayLike, List[ArrayLike]], is_batch: bool=False, params: List[int]=None) -> Optional[bytes]:
    ...
    def properties(self, index: int=None, colorspace: Union[int, str]=None, flags: int=cv2.IMREAD_COLOR) -> ImageProperties:
        ...
        if is_batch:
            ...
            return ImageProperties(shape=(n_images, *img.shape), dtype=img.dtype, n_images=n_images, is_batch=True)
