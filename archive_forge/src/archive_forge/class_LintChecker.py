    ...
    """
        ...
        for lineno, line in enumerate(lines):
            ...
            if len(line) > cfg.format.line_width:
                local_ctx.record_lint('C0301', len(line), cfg.format.line_width, location=(lineno,))
            if line.endswith('\r'):
                if cfg.format.line_ending == 'unix':
                    local_ctx.record_lint('C0327', 'windows', location=(lineno,))
                line = line[:-1]
            elif cfg.format.line_ending == 'windows':
                local_ctx.record_lint('C0327', 'unix', location=(lineno,))
            ...
            if not cfg.format.use_tabchars:
                if '\t' in indentation:
                    ...
                    local_ctx.record_lint('C0306', 'tab', 'space', location=(lineno, colno))
            elif cfg.format.fractional_tab_policy == 'round-up':
                if ' ' in indentation:
                    colno = indentation.find(' ')
                    local_ctx.record_lint('C0306', 'space', 'tab', location=(lineno, colno))
            else:
