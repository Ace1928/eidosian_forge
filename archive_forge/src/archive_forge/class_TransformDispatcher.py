...
class TransformDispatcher:
    """Converts a transform that has the signature ``(tape -> Sequence(tape), fn)`` to a transform dispatcher
    ...
    def __new__(cls, *args, **kwargs):
        if os.environ.get('SPHINX_BUILD') == '1':
            warnings.warn("Transforms have been disabled, as a Sphinx build has been detected via SPHINX_BUILD='1'. If this is not the case, please set the environment variable SPHINX_BUILD='0'.", UserWarning)
            args[0].custom_qnode_transform = lambda x: x
            return args[0]
        return super().__new__(cls)

    def __init__(self, transform, expand_transform=None, classical_cotransform=None, is_informative=False, final_transform=False, use_argnum_in_expand=False):
