...
class UniversalImageQualityIndex(Metric):
    ...
    def __init__(self, kernel_size: Sequence[int]=(11, 11), sigma: Sequence[float]=(1.5, 1.5), reduction: Literal['elementwise_mean', 'sum', 'none', None]='elementwise_mean', **kwargs: Any) -> None:
        ...
        if reduction is None or reduction == 'none':
            rank_zero_warn("Metric `UniversalImageQualityIndex` will save all targets and predictions in the buffer when using`reduction=None` or `reduction='none'. For large datasets, this may lead to a large memory footprint.")
