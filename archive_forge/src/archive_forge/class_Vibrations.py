...
class Vibrations(AtomicDisplacements):
    ...
    def run(self):
        ...
        if not self.cache.writable:
            raise RuntimeError('Cannot run calculation.  Cache must be removed or split in order to have only one sort of data structure at a time.')
    ...
    def clean(self, empty_files=False, combined=True):
        ...
        if world.rank != 0:
            return 0
        if empty_files:
            return self.cache.strip_empties()
    ...
    def read(self, method='standard', direction='central'):
        ...
        if any(masses[self.indices] == 0):
            raise RuntimeError('Zero mass encountered in one or more of the vibrated atoms. Use Atoms.set_masses() to set all masses to non-zero values.')
    ...
    def get_energies(self, method='standard', direction='central', **kw):
        ...
        return self.get_vibrations(method=method, direction=direction, **kw).get_energies()
    ...
    def summary(self, method='standard', direction='central', freq=None, log=sys.stdout):
        if freq is not None:
    ...
    def fold(self, frequencies, intensities, start=800.0, end=4000.0, npts=None, width=4.0, type='Gaussian', normalize=False):
        ...
        for i, energy in enumerate(energies):
            ...
            if lctype == 'lorentzian':
                ...
                spectrum[i] = (intensities * np.exp(-(frequencies - energy) ** 2 / 2.0 / sigma ** 2)).sum()
