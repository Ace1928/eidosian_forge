...
class AdminSite:
    ...
    def get_urls(self):
        ...
        if self.final_catch_all_view:
            urlpatterns.append(re_path('(?P<url>.*)$', wrap(self.catch_all_view)))
        return urlpatterns
    ...
    def app_index(self, request, app_label, extra_context=None):
        ...
        context = {**self.each_context(request), 'title': _('%(app)s administration') % {'app': app_list[0]['name']}, 'subtitle': None, 'app_list': app_list, 'app_label': app_label, **(extra_context or {})}
        request.current_app = self.name
        return TemplateResponse(request, self.app_index_template or ['admin/%s/app_index.html' % app_label, 'admin/app_index.html'], context)
