...
class SparkIbisEngine(IbisEngine):
    ...
    def __init__(self, execution_engine: ExecutionEngine) -> None:
        assert_or_throw(isinstance(execution_engine, SparkExecutionEngine), lambda: ValueError(f'SparkIbisEngine must use SparkExecutionEngine ({execution_engine})'))
        super().__init__(execution_engine)

    def select(self, dfs: DataFrames, ibis_func: Callable[[ibis.BaseBackend], IbisTable]) -> DataFrame:
        for k, v in dfs.items():
            self.execution_engine.register(v, k)
        con = ibis.pyspark.connect(self.execution_engine.spark_session)
        expr = ibis_func(con)
        schema = to_schema(expr.schema())
        result = expr.compile()
        assert_or_throw(isinstance(result, PySparkDataFrame), lambda: ValueError(f'result must be a PySpark DataFrame ({type(result)})'))
