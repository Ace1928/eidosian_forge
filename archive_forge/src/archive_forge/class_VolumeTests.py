...
class VolumeTests(unittest.TestCase):
    ...
    def test_endElement_sets_correct_attributes_with_values(self):
        for arguments in [('volumeId', 'some value', 'id'), ('createTime', 'some time', 'create_time'), ('status', 'some status', 'status'), ('size', 5, 'size'), ('snapshotId', 1, 'snapshot_id'), ('availabilityZone', 'some zone', 'zone'), ('someName', 'some value', 'someName'), ('encrypted', 'true', 'encrypted', True)]:
    ...
    def test_update_with_result_set_greater_than_0_updates_dict(self):
        ...
        assert all([self.volume_two.create_time == 5, self.volume_two.status == 'one_status', self.volume_two.size == 'one_size', self.volume_two.snapshot_id == 1, self.volume_two.attach_data == self.attach_data, self.volume_two.zone == 'one_zone'])
    ...
    def test_detach_with_no_attach_data(self):
        ...
        self.volume_two.connection.detach_volume.assert_called_with(1, None, None, False, dry_run=False)
