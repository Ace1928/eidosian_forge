...
class NeuralDBVectorStore(VectorStore):
    ...
    @classmethod
    def from_checkpoint(cls, checkpoint: Union[str, Path], thirdai_key: Optional[str]=None):
        """
    ...
    @classmethod
    def from_texts(cls, texts: List[str], embedding: Embeddings, metadatas: Optional[List[dict]]=None, **kwargs: Any) -> 'NeuralDBVectorStore':
    ...
    def validate_environments(cls, values: Dict) -> Dict:
        ...
        values['thirdai_key'] = convert_to_secret_str(get_from_dict_or_env(values, 'thirdai_key', 'THIRDAI_KEY'))
        return values

    def insert(self, sources: List[Any], train: bool=True, fast_mode: bool=True, **kwargs):
        ...
        self.db.insert(sources=sources, train=train, fast_approximation=fast_mode, **kwargs)
    ...
    def similarity_search(self, query: str, k: int=10, **kwargs: Any) -> List[Document]:
        ...
        try:
            ...
            return [Document(page_content=ref.text, metadata={'id': ref.id, 'upvote_ids': ref.upvote_ids, 'source': ref.source, 'metadata': ref.metadata, 'score': ref.score, 'context': ref.context(1)}) for ref in references]
