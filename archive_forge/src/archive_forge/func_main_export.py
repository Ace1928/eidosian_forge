...
def main_export(model_name_or_path: str, output: Union[str, Path], task: str='auto', opset: Optional[int]=None, device: str='cpu', dtype: Optional[str]=None, fp16: Optional[bool]=False, optimize: Optional[str]=None, monolith: bool=False, no_post_process: bool=False, framework: Optional[str]=None, atol: Optional[float]=None, cache_dir: Optional[str]=None, trust_remote_code: bool=False, pad_token_id: Optional[int]=None, subfolder: str='', revision: str='main', force_download: bool=False, local_files_only: bool=False, use_auth_token: Optional[Union[bool, str]]=None, for_ort: bool=False, do_validation: bool=True, model_kwargs: Optional[Dict[str, Any]]=None, custom_onnx_configs: Optional[Dict[str, 'OnnxConfig']]=None, fn_get_submodels: Optional[Callable]=None, use_subprocess: bool=False, _variant: str='default', library_name: Optional[str]=None, legacy: bool=False, no_dynamic_axes: bool=False, do_constant_folding: bool=True, **kwargs_shapes):
    ...
    if optimize == 'O4' and device != 'cuda':
        raise ValueError('Requested O4 optimization, but this optimization requires to do the export on GPU. Please pass the argument `--device cuda`.')
    ...
    if dtype == 'fp16' and device == 'cpu':
        raise ValueError('FP16 export is supported only when exporting on GPU. Please pass the option `--device cuda`.')
    if for_ort:
        logger.warning('The option --for-ort was passed, but its behavior is now the default in the ONNX exporter and passing it is not required anymore.')
    original_task = task
    ...
    if task.endswith('-with-past') and monolith:
        ...
        raise ValueError(f'The task {task} is not compatible with the --monolith argument. Please either use `--task {task_non_past} --monolith`, or `--task {task}` without the monolith argument.')
    if task == 'auto':
        try:
            task = TasksManager.infer_task_from_model(model_name_or_path)
        except KeyError as e:
            raise KeyError(f'The task could not be automatically inferred. Please provide the argument --task with the relevant task from {', '.join(TasksManager.get_all_tasks())}. Detailed error: {e}')
        except RequestsConnectionError as e:
            raise RequestsConnectionError(f'The task could not be automatically inferred as this is available only for models hosted on the Hugging Face Hub. Please provide the argument --task with the relevant task from {', '.join(TasksManager.get_all_tasks())}. Detailed error: {e}')
    custom_architecture = False
    loading_kwargs = {}
    if library_name == 'transformers':
        config = AutoConfig.from_pretrained(model_name_or_path, subfolder=subfolder, revision=revision, cache_dir=cache_dir, use_auth_token=use_auth_token, local_files_only=local_files_only, force_download=force_download, trust_remote_code=trust_remote_code)
        model_type = config.model_type.replace('_', '-')
        if model_type not in TasksManager._SUPPORTED_MODEL_TYPE:
            custom_architecture = True
        elif task not in TasksManager.get_supported_tasks_for_model_type(model_type, 'onnx', library_name=library_name):
            if original_task == 'auto':
                autodetected_message = ' (auto-detected)'
            else:
                autodetected_message = ''
            model_tasks = TasksManager.get_supported_tasks_for_model_type(model_type, exporter='onnx', library_name=library_name)
            raise ValueError(f'Asked to export a {model_type} model for the task {task}{autodetected_message}, but the Optimum ONNX exporter only supports the tasks {', '.join(model_tasks.keys())} for {model_type}. Please use a supported task. Please open an issue at https://github.com/huggingface/optimum/issues if you would like the task {task} to be supported in the ONNX export for {model_type}.')
        if model_type in SDPA_ARCHS_ONNX_EXPORT_NOT_SUPPORTED and _transformers_version >= version.parse('4.35.99'):
            loading_kwargs['attn_implementation'] = 'eager'
    model = TasksManager.get_model_from_task(task, model_name_or_path, subfolder=subfolder, revision=revision, cache_dir=cache_dir, use_auth_token=use_auth_token, local_files_only=local_files_only, force_download=force_download, trust_remote_code=trust_remote_code, framework=framework, torch_dtype=torch_dtype, device=device, library_name=library_name, **loading_kwargs)
    ...
    if not custom_architecture and library_name != 'diffusers' and (task + '-with-past' in TasksManager.get_supported_tasks_for_model_type(model_type, 'onnx', library_name=library_name)):
        if original_task == 'auto' and (not monolith):
            task = task + '-with-past'
        else:
            logger.info(f'The task `{task}` was manually specified, and past key values will not be reused in the decoding. if needed, please pass `--task {task}-with-past` to export using the past key values.')
    ...
    preprocessors = maybe_load_preprocessors(model_name_or_path, subfolder=subfolder, trust_remote_code=trust_remote_code)
    onnx_export_from_model(model=model, output=output, opset=opset, optimize=optimize, monolith=monolith, no_post_process=no_post_process, atol=atol, do_validation=do_validation, model_kwargs=model_kwargs, custom_onnx_configs=custom_onnx_configs, fn_get_submodels=fn_get_submodels, _variant=_variant, legacy=legacy, preprocessors=preprocessors, device=device, no_dynamic_axes=no_dynamic_axes, task=task, use_subprocess=use_subprocess, do_constant_folding=do_constant_folding, **kwargs_shapes)