...
def _create_experiment():
    request_message = _get_request_message(CreateExperiment(), schema={'name': [_assert_required, _assert_string], 'artifact_location': [_assert_string], 'tags': [_assert_array]})
    ...
    if parsed_artifact_location.fragment or parsed_artifact_location.params:
        raise MlflowException("'artifact_location' URL can't include fragments or params.", error_code=INVALID_PARAMETER_VALUE)
    validate_query_string(parsed_artifact_location.query)
    experiment_id = _get_tracking_store().create_experiment(request_message.name, request_message.artifact_location, tags)
