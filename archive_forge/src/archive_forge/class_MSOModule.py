...
class MSOModule(object):
    ...
    def login(self):
        ...
        resp, auth = fetch_url(self.module, self.url, data=json.dumps(payload), method='POST', headers=self.headers, timeout=self.params.get('timeout'), use_proxy=self.params.get('use_proxy'))
    ...
    def request(self, path, method=None, data=None, qs=None, api_version='v1'):
        ...
        if self.module._socket_path:
            ...
            resp, info = fetch_url(self.module, self.url, headers=self.headers, data=json.dumps(data), method=self.method, timeout=self.params.get('timeout'), use_proxy=self.params.get('use_proxy'))
        self.response = info.get('msg')
    ...
    def dict_from_ref(self, data):
        if data and data != '':
            ref_regex = re.compile('\\/schemas\\/(.*)\\/templates\\/(.*?)\\/(.*?)\\/(.*)')
            ...
            if dic is not None:
                ...
                name = dic.group(4)
                uri_map = {'vrfs': ['vrfName', 'schemaId', 'templateName'], 'bds': ['bdName', 'schemaId', 'templateName'], 'filters': ['filterName', 'schemaId', 'templateName'], 'contracts': ['contractName', 'schemaId', 'templateName'], 'l3outs': ['l3outName', 'schemaId', 'templateName'], 'anps': ['anpName', 'schemaId', 'templateName'], 'serviceGraphs': ['serviceGraphName', 'schemaId', 'templateName'], 'serviceNode': ['serviceNodeName', 'schemaId', 'templateName', 'serviceGraphName']}
                result = {uri_map[category][1]: schema_id, uri_map[category][2]: template_name}
    ...
    def recursive_dict_from_ref_regex(self, data, result, category):
        continued_ref_regex = re.compile('(.*?)\\/([a-zA-Z]+.*)')
        section_ref_regex = re.compile('([a-zA-Z]+)\\/(.*)')
    ...
    def make_reference(self, data, reftype, schema_id, template):
        ...
        return {refname: data.get('name'), 'schemaId': schema_id, 'templateName': template}
    ...
    def exit_json(self, **kwargs):
        ...
        if self.module._diff and self.result.get('changed') is True:
            self.result['diff'] = dict(before=self.previous, after=self.existing)
