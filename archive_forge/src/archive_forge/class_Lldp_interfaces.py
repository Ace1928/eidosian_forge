        ...
        if want_location_type['coordinate_based']:
            ...
            if is_dict_element_present(have_location_type, 'civic_based'):
                ...
                if want_dict['country_code'] != have_dict['country_code']:
                    commands.append(self._compute_command(set_cmd + location_type, 'country-code', str(want_dict['country_code'])))
            else:
                commands.append(self._compute_command(set_cmd + location_type, 'country-code', str(want_dict['country_code'])))
            ...
            if is_dict_element_present(have_location_type, 'elin'):
                if want_location_type.get('elin') != have_location_type.get('elin'):
                    commands.append(self._compute_command(set_cmd + location_type, value=str(want_location_type['elin'])))
            else:
                commands.append(self._compute_command(set_cmd + location_type, value=str(want_location_type['elin'])))
        return commands
        ...
        if want_location_type['coordinate_based']:
            ...
            if is_dict_element_present(have_location_type, 'coordinate_based'):
                ...
                for key, value in iteritems(have_dict):
                    ...
                    if not only_in_have:
                        commands.append(self._compute_command(del_cmd + location_type, key, str(value), True))
            else:
            ...
            if is_dict_element_present(have_location_type, 'civic_based'):
                ...
                commands.append(self._compute_command(del_cmd, remove=True))
        elif is_dict_element_present(have_location_type, 'elin'):
            if want_location_type.get('elin') != have_location_type.get('elin'):
                commands.append(self._compute_command(del_cmd, remove=True))
        else:
            commands.append(self._compute_command(del_cmd, remove=True))
