...
async def main():
    ...
    llm_interrupt_event = asyncio.Event()
    tts_stop_event = asyncio.Event()
    partial_response_buffer = ""
    is_user_speaking = False
    llm_processing_lock = asyncio.Lock()
    transcription_buffer = ""
    last_transcription_time = asyncio.get_event_loop().time()
    transcription_interval = 0.3  # Reduced interval for more responsive processing.
    chat_history: List[ChatMessage] = []

    async def generate_and_play_response(input_text: str):
        """Generates LLM response and plays it via TTS."""
        nonlocal chat_history, partial_response_buffer
        async with llm_processing_lock:
            logging.info(f"Main - Generating and playing response for: '{input_text}'")
            response_generator = await get_llm_response(
                input_text,
                chat_history=chat_history,
                interrupt_event=llm_interrupt_event,
            )
            partial_response_buffer = ""

            async def stream_audio(chunk_stream: Optional[AsyncIterator[ChatResponse]]):
                """Streams audio chunks from the LLM and plays them."""
                nonlocal partial_response_buffer
                if chunk_stream is None:
                    logging.warning("Main - LLM response stream is None.")
                    return
                async for chunk in chunk_stream:
                    if llm_interrupt_event.is_set():
                        logging.info("Main - LLM response streaming interrupted.")
                        break
                    if chunk and chunk.message and chunk.message.content:
                        await play_audio(chunk.message.content, tts_stop_event)
                        partial_response_buffer += chunk.message.content

                full_response = partial_response_buffer

                if not llm_interrupt_event.is_set() and full_response.strip():
                    console.print(f"[cyan]Assistant: {full_response}")
                    chat_history.append(
                        ChatMessage(role="assistant", content=full_response)
                    )
                    logging.info(f"Main - Assistant response: '{full_response}'")

            if response_generator.achat_stream:
                await stream_audio(response_generator.achat_stream)
            else:
                logging.warning("Main - LLM response stream is None.")

            if input_text and not llm_interrupt_event.is_set():
                chat_history.append(ChatMessage(role="user", content=input_text))
                logging.info(f"Main - User input added to history: '{input_text}'")

    async def process_transcription():
        """Processes transcribed text and sends it to the LLM."""
        nonlocal transcription_buffer, last_transcription_time, is_user_speaking
        while not audio_processor.stop_event.is_set():
            try:
                text_chunk = await audio_processor.get_transcription_chunk(
                    timeout=0.1
                )  # Reduced timeout for more responsiveness
                if text_chunk:
                    is_user_speaking = True
                    transcription_buffer += " " + text_chunk
                    console.print(
                        f"[yellow]You (Partial): {transcription_buffer.strip()}"
                    )
                    logging.info(
                        f"Main - Partial transcription: '{transcription_buffer.strip()}'"
                    )
                    last_transcription_time = asyncio.get_event_loop().time()

                if transcription_buffer and (
                    not audio_processor._is_speaking
                    or (
                        asyncio.get_event_loop().time() - last_transcription_time
                        >= transcription_interval
                    )
                ):
                    final_transcript = transcription_buffer.strip()
                    if final_transcript:
                        console.print(f"[yellow]You (Full): {final_transcript}")
                        logging.info(f"Main - Full transcription: '{final_transcript}'")
                        llm_interrupt_event.set()
                        tts_stop_event.set()
                        stop_audio()
                        await generate_and_play_response(final_transcript)
                    transcription_buffer = ""
                    is_user_speaking = False
                await asyncio.sleep(
                    0.01
                )  # Further reduced sleep interval for maximum responsiveness.
            except Exception as e:
                logging.error(f"Error in process_transcription: {e}", exc_info=True)

    async def manage_conversation():
        """Manages the main conversation flow, including starting and stopping audio processing."""
        try:
            with Live(console=console, screen=True, refresh_per_second=10):
                ...
                async def update_display():
                    ...
                    table.add_row(
                        ...
                        (
                            "🟢 Active"
                            if audio_processor.microphone_active
                            else "🔴 Inactive"
                        ),
                        "",
                    )
            ...
            await asyncio.gather(audio_processor.start(), process_transcription())
        except Exception as e:
            logging.error(f"Error in manage_conversation: {e}", exc_info=True)
        finally:
            await audio_processor.stop()

    try:
        await manage_conversation()

        # Keep the main loop running to allow background tasks to continue.
        while True:
