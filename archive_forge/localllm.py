...
import sympy
from sympy import preview, symbols, solve, Eq, parsing
...
class EidosDirectoryManager:
    ...
    This class encapsulates the management of all Eidos project directories,
    ensuring their existence and providing a centralized point of configuration.
    It facilitates organized storage and retrieval of various Eidos-related data.
    """

    def __init__(
        self,
        base_dir: str = "/Development",
        high_resource_threshold: float = 80.0,
        initial_chunk_size: int = 10,
        disk_offload_delay: float = 1.0,
        adaptive_chunking: bool = True,
        min_chunk_size: int = 5,
        max_chunk_size: int = 20,
    ):
        """
        Initializes the Eidos directory structure.

        Args:
            base_dir (str): The root directory for all Eidos projects.
                            Defaults to '/Development'. Can be overridden by
                            the EIDOS_BASE_DIR environment variable.
            high_resource_threshold (float): CPU or memory percentage above which
                                             directory creation might be delayed. Defaults to 80.0.
            initial_chunk_size (int): The initial number of directories to create in each chunk.
                              Defaults to 10.
            disk_offload_delay (float): Simulated delay in seconds to mimic disk offloading.
                                       Defaults to 1.0.
            adaptive_chunking (bool): If True, chunk size will be adjusted based on resource usage.
                                      Defaults to True.
            min_chunk_size (int): Minimum chunk size for adaptive chunking. Defaults to 5.
            max_chunk_size (int): Maximum chunk size for adaptive chunking. Defaults to 20.
        """
        self.base_dir = os.path.abspath(os.environ.get("EIDOS_BASE_DIR", base_dir))
        self.high_resource_threshold = high_resource_threshold
        self.initial_chunk_size = initial_chunk_size
        self.disk_offload_delay = disk_offload_delay
        self.adaptive_chunking = adaptive_chunking
        self.min_chunk_size = min_chunk_size
        self.max_chunk_size = max_chunk_size
        self.logger = logging.getLogger(__name__)
        asyncio.run(self._ensure_dir_exists(self.base_dir))
        self.logger.info(f"📁 Eidos base directory set to: {self.base_dir}")
        asyncio.run(self._create_directories())

    async def _ensure_dir_exists(self, path: str) -> None:
        """Ensures a directory exists, creating it if necessary."""
        try:
            cpu_percent = psutil.cpu_percent()
            mem_percent = psutil.virtual_memory().percent
            if (
                cpu_percent > self.high_resource_threshold
                or mem_percent > self.high_resource_threshold
            ):
                self.logger.warning(
                    f"High resource usage (CPU: {cpu_percent}%, Memory: {mem_percent}%). "
                    f"Delaying directory creation for: {path}"
                )
                await asyncio.sleep(5)  # Wait for a bit before proceeding

            os.makedirs(path, exist_ok=True)
            self.logger.debug(f"Ensured directory exists: {path}")
        except OSError as e:
            self.logger.error(f"Error creating directory {path}: {e}")
            raise  # Re-raising the exception for the caller to handle

    def _get_adaptive_chunk_size(self) -> int:
        """Dynamically determines chunk size based on resource usage."""
        if not self.adaptive_chunking:
            return self.initial_chunk_size

        cpu_percent = psutil.cpu_percent()
        mem_percent = psutil.virtual_memory().percent
        resource_load = max(cpu_percent, mem_percent)

        if resource_load > self.high_resource_threshold:
            chunk_size = max(
                self.min_chunk_size,
                int(
                    ...
                    * (
                        1
                        - (resource_load - self.high_resource_threshold)
                        / (100 - self.high_resource_threshold)
                    )
                ),
            )
            self.logger.debug(
                f"Adjusting chunk size due to high resource usage. New chunk size: {chunk_size}"
            )
        else:
            chunk_size = min(
                self.max_chunk_size,
                int(
                    ...
                    * (
                        1
                        + (self.high_resource_threshold - resource_load)
                        / self.high_resource_threshold
                    )
                ),
            )
            self.logger.debug(
                f"Adjusting chunk size due to low resource usage. New chunk size: {chunk_size}"
            )
        return chunk_size

    async def _create_directories(self) -> None:
        """
        Creates all necessary Eidos directories in chunks, with resource monitoring and disk offloading.

        This method defines and ensures the existence of each directory
        used by the Eidos system, categorizing them by their function
        within the project. It processes directories in chunks to manage
        resource usage and simulates disk offloading.
        """
        directories_to_create = [
            ("saved_models", "💾 Saved models directory"),
            ("datasets", "🗄️ Datasets directory"),
            ("templates", "📝 Templates directory"),
            ("tests", "🧪 Tests directory"),
            ("images", "🖼️ Images directory"),
            ...
            ("environment", "⚙️ Environment directory"),
            ("documents", "📑 Documents directory"),
            ("python_repository", "🐍 Python repository directory"),
            ("csv", "📊 CSV directory"),
            ("compressed", "🗜️ Compressed directory"),
            ("applications", "🚀 Applications directory"),
            ("backups", "📦 Backups directory"),
            ("chat_history_gpt", "💬 GPT chat history directory"),
            ("chat_history_eidos", "🧠 Eidos chat history directory"),
            ("sandbox", "🧰 Sandbox directory"),
            ("eidos", "👤 Personal Eidos directory"),
            ("knowledge", "🧠 Eidos Knowledge Base directory"),
            (os.path.join("knowledge", "citeweb"), "🔗 Citations directory"),
            (os.path.join("knowledge", "identity"), "🎭 Identity directory"),
            (os.path.join("knowledge", "lessons"), "📚 Lessons learned directory"),
            (os.path.join("knowledge", "majormoments"), "📅 Major moments directory"),
            (os.path.join("knowledge", "processed"), "🗂️ Processed data directory"),
            (os.path.join("knowledge", "raw"), "🕸️ Raw data directory"),
            (os.path.join("knowledge", "refined"), "✨ Refined data directory"),
            (os.path.join("knowledge", "speculative"), "❓ Speculative data directory"),
            (os.path.join("knowledge", "timeline"), "⏱️ Raw timeline data directory"),
            (os.path.join("knowledge", "verified"), "✅ Verified data directory"),
            (os.path.join("knowledge", "diary"), "📝 Diary directory"),
        ]

        n_directories = len(directories_to_create)
        i = 0
        while i < n_directories:
            chunk_size = self._get_adaptive_chunk_size()
            chunk = directories_to_create[i : i + chunk_size]

            cpu_percent = psutil.cpu_percent()
            mem_percent = psutil.virtual_memory().percent
            self.logger.info(
                f"Processing directory chunk {i//chunk_size + 1}/{((n_directories - 1) // chunk_size) + 1} (CPU: {cpu_percent}%, Memory: {mem_percent}%)"
            )

            if self.disk_offload_delay > 0:
                self.logger.info(
                    f"Simulating disk offloading delay of {self.disk_offload_delay} seconds."
                )
                await asyncio.sleep(self.disk_offload_delay)

            async def create_single_directory(subdir, description):
                path = os.path.join(self.base_dir, subdir)
                await self._ensure_dir_exists(path)
                self.logger.debug(f"{description}: {path}")
                setattr(
                    self, f"{subdir.replace(os.sep, '_')}_dir", path
                )  # Dynamically set attribute

            creation_tasks = [
                create_single_directory(subdir, description)
                for subdir, description in chunk
            ]
...
_EIDOS_BASE_DIR = eidos_directories.base_dir
_EIDOS_SAVED_MODELS_DIR = getattr(eidos_directories, "saved_models_dir", None)
_EIDOS_DATASETS_DIR = getattr(eidos_directories, "datasets_dir", None)
_EIDOS_TEMPLATES_DIR = getattr(eidos_directories, "templates_dir", None)
_EIDOS_TESTS_DIR = getattr(eidos_directories, "tests_dir", None)
_EIDOS_IMAGES_DIR = getattr(eidos_directories, "images_dir", None)
_EIDOS_AUDIO_DIR = getattr(eidos_directories, "audio_dir", None)
_EIDOS_VIDEO_DIR = getattr(eidos_directories, "video_dir", None)
_EIDOS_HTML_DIR = getattr(eidos_directories, "html_dir", None)
_EIDOS_LOGS_DIR = getattr(eidos_directories, "logs_dir", None)
_EIDOS_NOTEBOOKS_DIR = getattr(eidos_directories, "notebooks_dir", None)
_EIDOS_PAPERS_DIR = getattr(eidos_directories, "papers_dir", None)
_EIDOS_ENVIRONMENT_DIR = getattr(eidos_directories, "environment_dir", None)
_EIDOS_DOCUMENTS_DIR = getattr(eidos_directories, "documents_dir", None)
_EIDOS_PYTHON_REPOSITORY_DIR = getattr(eidos_directories, "python_repository_dir", None)
_EIDOS_CSV_DIR = getattr(eidos_directories, "csv_dir", None)
_EIDOS_COMPRESSED_DIR = getattr(eidos_directories, "compressed_dir", None)
_EIDOS_APPLICATIONS_DIR = getattr(eidos_directories, "applications_dir", None)
_EIDOS_BACKUPS_DIR = getattr(eidos_directories, "backups_dir", None)
_EIDOS_CHAT_HISTORY_GPT_DIR = getattr(eidos_directories, "chat_history_gpt_dir", None)
_EIDOS_CHAT_HISTORY_EIDOS_DIR = getattr(
    eidos_directories, "chat_history_eidos_dir", None
)
_EIDOS_SANDBOX_DIR = getattr(eidos_directories, "sandbox_dir", None)
_EIDOS_PERSONAL_DIR = getattr(
    eidos_directories, "eidos_dir", None
)  # Corrected attribute name
_EIDOS_KNOWLEDGE_BASE_DIR = getattr(
    eidos_directories, "knowledge_dir", None
)  # Corrected attribute name
_EIDOS_CITATIONS_DIR = getattr(
    eidos_directories, "knowledge_citeweb_dir", None
)  # Corrected attribute name
_EIDOS_IDENTITY_DIR = getattr(
    eidos_directories, "knowledge_identity_dir", None
)  # Corrected attribute name
_EIDOS_LESSONS_DIR = getattr(
    eidos_directories, "knowledge_lessons_dir", None
)  # Corrected attribute name
_EIDOS_MAJOR_MOMENTS_DIR = getattr(
    eidos_directories, "knowledge_majormoments_dir", None
)  # Corrected attribute name
_EIDOS_PROCESSED_DATA_DIR = getattr(
    eidos_directories, "knowledge_processed_dir", None
)  # Corrected attribute name
_EIDOS_RAW_DATA_DIR = getattr(
    eidos_directories, "knowledge_raw_dir", None
)  # Corrected attribute name
_EIDOS_REFINED_DATA_DIR = getattr(
    eidos_directories, "knowledge_refined_dir", None
)  # Corrected attribute name
_EIDOS_SPECULATIVE_DATA_DIR = getattr(
    eidos_directories, "knowledge_speculative_dir", None
)  # Corrected attribute name
_EIDOS_RAW_TIMELINE_DIR = getattr(
    eidos_directories, "knowledge_timeline_dir", None
)  # Corrected attribute name
_EIDOS_VERIFIED_DATA_DIR = getattr(
    eidos_directories, "knowledge_verified_dir", None
)  # Corrected attribute name
_EIDOS_DIARY_DIR = getattr(
    eidos_directories, "knowledge_diary_dir", None
)  # Corrected attribute name
...
class LoggingConfig:
    ...
    adaptive_cpu_threshold: float = 80.0
    adaptive_mem_threshold: float = 80.0
...
def _configure_console_handler(
    ...
    stream_output = (
        config.stream_output if config.stream_output is not None else sys.stdout
    )
    console_handler = StreamHandler(stream_output)
    formatter = EidosFormatter(
        config.log_format if config.log_format else _EIDOS_LOG_FORMAT,
        datefmt=config.datetime_format,
        use_json=config.log_format_type == "json",
        include_uuid=config.include_uuid,
    )
    console_handler.setFormatter(formatter)
    if not any(
        handler.stream == stream_output
...
def _configure_file_handler(
    logger: logging.Logger, config: LoggingConfig, numeric_level: int
) -> None:
    ...
    if config.file_log_level:
        if isinstance(config.file_log_level, str):
            file_numeric_level = getattr(logging, config.file_log_level.upper(), None)
            if not isinstance(file_numeric_level, int):
                logger.error(
                    f"Invalid file log level: {config.file_log_level}. Using console log level."
                )
                file_numeric_level = numeric_level
        elif isinstance(config.file_log_level, int):
            file_numeric_level = config.file_log_level
        else:
            logger.error(
                f"Invalid file log level type: {type(config.file_log_level)}. Using console log level."
            )
            file_numeric_level = numeric_level

    try:
        file_handler = logging.FileHandler(
            config.log_to_file, mode="a", encoding="utf-8"
        )
        file_formatter = EidosFormatter(
            config.log_format if config.log_format else _EIDOS_LOG_FORMAT,
            datefmt=config.datetime_format,
            use_json=config.log_format_type == "json",
            include_uuid=config.include_uuid,
        ...
        if not any(
            ...
            and handler.baseFilename == os.path.abspath(config.log_to_file)
            for handler in logger.handlers
        ):
            logger.addHandler(file_handler)
        logger.debug(
            f"📝 Logging to file enabled at level: {logging.getLevelName(file_numeric_level)} in: {config.log_to_file}"
        )
...
def _adaptive_log_level_adjustment(
    ...
    """⚙️ Dynamically adjusts log level based on system resource usage."""
    try:
        cpu_percent = psutil.cpu_percent(interval=config.adaptive_interval)
        mem_percent = psutil.virtual_memory().percent
        if (
            cpu_percent > config.adaptive_cpu_threshold
            or mem_percent > config.adaptive_mem_threshold
        ):
            if logger.level > logging.INFO:
                logger.setLevel(logging.INFO)
                logger.warning(
                    f"⚠️ System resources high ({cpu_percent}% CPU, {mem_percent}% Memory). Reducing log level to INFO."
                )
        elif logger.level < numeric_level:
            logger.setLevel(numeric_level)
            logger.debug(
                f"✅ System resources normal. Restoring log level to {logging.getLevelName(numeric_level)}."
            )
    except Exception as e:
        logger.error(f"🔥 Error during adaptive logging adjustment: {e}")


def _configure_adaptive_logging(
    logger: logging.Logger, config: LoggingConfig, numeric_level: int
) -> None:
    """⚙️ Configures adaptive logging if enabled."""
    if not config.adaptive_logging:
        return

    def _adaptive_wrapper():
        while True:
            _adaptive_log_level_adjustment(logger, config, numeric_level)
            time.sleep(config.adaptive_interval)

    adaptive_thread = threading.Thread(target=_adaptive_wrapper, daemon=True)
...
def _configure_detailed_tracing(
    logger: logging.Logger, config: LoggingConfig, trace_level: int = 5
) -> None:
    """⚙️ Configures detailed tracing if enabled."""
    if not config.detailed_tracing:
        return
    try:
        sys.settrace(
            lambda frame, event, arg: _trace_function(
                frame, event, arg, logger, trace_level
            )
        )
...
def _configure_debugpy_trigger(logger: logging.Logger, config: LoggingConfig) -> None:
    ...
    if config.debugpy_trigger_level is None:
        return

    trigger_level = getattr(logging, str(config.debugpy_trigger_level).upper(), None)
    if not isinstance(trigger_level, int):
        try:
            trigger_level = int(config.debugpy_trigger_level)
        except ValueError:
            logger.error(
                f"Invalid debugpy trigger level: {config.debugpy_trigger_level}"
            )
            return
    ...
    formatter = Formatter(config.log_format if config.log_format else _EIDOS_LOG_FORMAT)
    debugpy_handler.setFormatter(formatter)
    logger.addHandler(debugpy_handler)
    logger.debug(
        f"⚙️ Debugpy trigger enabled for log level: {logging.getLevelName(trigger_level)}."
    )


LLM_TRACE = 5  # Define a custom log level for LLM tracing
    ...
    *args,
    ...
    Args:
        *args: Positional arguments. If provided, the first argument is taken as the log level (deprecated, use log_level instead).
        ...
        adaptive_interval: Interval in seconds for adaptive logging checks.
        adaptive_cpu_threshold: CPU usage percentage threshold for adaptive logging.
        adaptive_mem_threshold: Memory usage percentage threshold for adaptive logging.
    ...
    config = LoggingConfig(
        ...
        debugpy_trigger_level=debugpy_trigger_level,
        adaptive_interval=adaptive_interval,
        adaptive_cpu_threshold=adaptive_cpu_threshold,
        adaptive_mem_threshold=adaptive_mem_threshold,
    )

    if args and not config.log_level:
        config.log_level = args[0]
    if not config.log_level:
        config.log_level = _EIDOS_LOG_LEVEL
    ...
    _configure_adaptive_logging(logger, config, numeric_level)
    ...
    log_format_display = (
        ...
        else f"'{config.log_format if config.log_format else _EIDOS_LOG_FORMAT}'"
    )
...
class NltkConfig:
    ...
    Attributes:
        ...
        initial_chunk_size: The initial number of resources to process in each chunk. Defaults to 3.
        disk_offload_delay: Simulated delay in seconds to mimic disk offloading. Defaults to 0.1.
        adaptive_chunking: If True, chunk size will be adjusted based on resource usage. Defaults to True.
        min_chunk_size: Minimum chunk size for adaptive chunking. Defaults to 1.
        max_chunk_size: Maximum chunk size for adaptive chunking. Defaults to 5.
        high_resource_threshold: CPU or memory percentage above which chunk size might be reduced. Defaults to 80.0.
    """
...
class NltkResourceManager:
    """Manages NLTK resource downloads and availability."""

    def __init__(self, config: Optional[NltkConfig] = None):
        """Initializes the NLTK resource manager with a given configuration."""
        self.config = config if config else NltkConfig()
        self.logger = logging.getLogger(__name__)

    def _download_nltk_resource(self, resource: str) -> None:
        """Downloads a single NLTK resource."""
        try:
            self.logger.info(f"⬇️ Downloading NLTK resource '{resource}'...")
            nltk.download(resource, quiet=self.config.download_quietly)
            self.logger.info(f"✅ NLTK resource '{resource}' downloaded successfully.")
        except Exception as e:
            self.logger.error(f"❌ Error downloading NLTK resource '{resource}': {e}")
            raise

    async def _process_nltk_resource(self, resource: str) -> None:
        """Processes a single NLTK resource, checking for existence or downloading."""
        try:
            nltk.data.find(resource)
            self.logger.debug(f"✅ NLTK resource '{resource}' is already available.")
        except LookupError:
            if self.config.download_if_missing:
                self._download_nltk_resource(resource)
            else:
                self.logger.error(
                    f"❌ NLTK resource '{resource}' not found, and automatic download is disabled."
                )
                raise LookupError(
                    f"NLTK resource '{resource}' is missing. Please enable download or install it manually."
                )
        if self.config.disk_offload_delay > 0:
            self.logger.info(
                f"Simulating disk offloading delay of {self.config.disk_offload_delay} seconds."
            )
            await asyncio.sleep(self.config.disk_offload_delay)

    def _get_adaptive_chunk_size(self) -> int:
        """Dynamically determines chunk size based on resource usage."""
        if not self.config.adaptive_chunking:
            return self.config.initial_chunk_size

        cpu_percent = psutil.cpu_percent()
        mem_percent = psutil.virtual_memory().percent
        resource_load = max(cpu_percent, mem_percent)

        if resource_load > self.config.high_resource_threshold:
            chunk_size = max(
                ...
                int(
                    ...
                    * (
                        1
                        - (resource_load - self.config.high_resource_threshold)
                        / (100 - self.config.high_resource_threshold)
                    )
                ),
            )
            self.logger.debug(
                f"Adjusting NLTK chunk size due to high resource usage. New chunk size: {chunk_size}"
            )
        else:
            chunk_size = min(
                ...
                int(
                    ...
                    * (
                        1
                        + (self.config.high_resource_threshold - resource_load)
                        / self.config.high_resource_threshold
                    )
                ),
            )
            self.logger.debug(
                f"Adjusting NLTK chunk size due to low resource usage. New chunk size: {chunk_size}"
            )
        return chunk_size

    async def _process_nltk_resource_chunk(self, chunk: List[str]) -> None:
        """Processes a chunk of NLTK resources."""
        tasks = [self._process_nltk_resource(resource) for resource in chunk]
        await asyncio.gather(*tasks)

    async def ensure_resources(self) -> None:
        """Ensures the availability of specified NLTK resources, with intelligent downloading capability."""
        self.logger.info(
            "🔬 Ensuring NLTK resource availability based on provided configuration..."
        )
        ...
        while i < n_resources:
            ...
            self.logger.info(
                f"Processing NLTK resource chunk {i//chunk_size + 1}/{((n_resources - 1) // chunk_size) + 1}"
            )
            await self._process_nltk_resource_chunk(chunk)
            i += chunk_size

        self.logger.info(
            "📚 NLTK resource check completed. All necessary resources are available."
        )


nltk_config = NltkConfig()
nltk_resource_manager = NltkResourceManager(nltk_config)
asyncio.run(nltk_resource_manager.ensure_resources())
...
class PromptGenerator:
    ...
    def __init__(
        self,
        config: "LLMConfig",
        critique_prompt_generator_factory: Optional[
            Callable[["LLMConfig"], "CritiquePromptGenerator"]
        ] = None,
        sentiment_analyzer_factory: Optional[
            Callable[[], SentimentIntensityAnalyzer]
        ] = SentimentIntensityAnalyzer,
        textblob_factory: Optional[Callable[[str], TextBlob]] = TextBlob,
        nltk_stopwords_factory: Optional[Callable[[], set]] = lambda: set(
            stopwords.words("english")
        ),
        nltk_lemmatizer_factory: Optional[
            Callable[[], WordNetLemmatizer]
        ] = WordNetLemmatizer,
    ) -> None:
        ...
        self.textblob_analyzer_enabled: bool = (
            self.nlp_enabled and config.use_textblob_for_sentiment
        )
        self._critique_prompt_generator_factory = (
            critique_prompt_generator_factory or CritiquePromptGenerator
        )
        self._critique_prompt_generator_instance: Optional[
            "CritiquePromptGenerator"
        ] = None
        ...
        logger.info(
            f"😈 PromptGenerator initialized with NLP: {self.nlp_enabled}, TextBlob Sentiment: {self.textblob_analyzer_enabled}, primary critique template: '{self.primary_critique_template_id}', secondary: '{self.secondary_critique_template_id}'. The linguistic dark arts are now in session."
        )
    ...
    def _set_default_critique_template_id(self) -> None:
        ...
        try:
            if self.critique_prompt_generator.templates:
                self.default_critique_template_id = next(
                    iter(self.critique_prompt_generator.templates)
                )
                logger.debug(
                    f"⚙️ Default critique template ID set to: '{self.default_critique_template_id}'."
                )
            else:
                logger.warning(
                    "⚠️ No critique prompt templates available. Default critique template ID cannot be set. Prompt generation might be impaired."
                )
        except Exception as e:
            logger.error(
                f"🔥 Error setting default critique template ID: {e}", exc_info=True
            )
    ...
    def sentiment_analyzer(self) -> SentimentIntensityAnalyzer:
        ...
        if self._sentiment_analyzer_instance is None:
            try:
                ...
                logger.error(
                    f"🔥 Error initializing SentimentIntensityAnalyzer: {e}. Falling back to default.",
                    exc_info=True,
                )
    ...
    def lemmatizer(self) -> WordNetLemmatizer:
        ...
        if self._lemmatizer_instance is None:
            try:
                ...
                logger.error(
                    f"🔥 Error initializing WordNetLemmatizer: {e}. Falling back to default.",
                    exc_info=True,
                )
    ...
    def stop_words(self) -> set:
        ...
        if self._stop_words_instance is None:
            try:
                ...
                logger.error(
                    f"🔥 Error loading NLTK stop words: {e}. Falling back to default.",
                    exc_info=True,
                )
    ...
    def critique_prompt_generator(self) -> "CritiquePromptGenerator":
        ...
        if self._critique_prompt_generator_instance is None:
            try:
                self._critique_prompt_generator_instance = (
                    self._critique_prompt_generator_factory(self.config)
                )
                logger.debug("🎭 CritiquePromptGenerator instance created.")
            except Exception as e:
                logger.critical(
                    f"🔥 Critical error creating CritiquePromptGenerator instance: {e}. This will severely impact critique functionality.",
                    exc_info=True,
                )
    ...
    def generate_critique_prompt(
        ...
        template_id = (
            self.secondary_critique_template_id
            if cycle > self.config.primary_critique_cycles
            else self.primary_critique_template_id
        )
        log_metadata = {"template_id": template_id, "cycle": cycle}
        logger.debug(
            f"🛠️ Attempting to generate critique prompt (Cycle: {cycle})...",
            extra=log_metadata,
        )
        ...
        try:
            ...
            logger.llm_trace(
                f"✅ Generated critique prompt using template: {template_id}",
                extra=log_metadata,
            )
            return prompt
            ...
            if (
                self.default_critique_template_id
                and self.config.fallback_on_missing_critique_template
            ):
                logger.warning(
                    f"{log_msg} Falling back to default template: '{self.default_critique_template_id}'.",
                    extra=log_metadata,
                )
                try:
                    prompt = self.critique_prompt_generator.generate_prompt(
            ...
            logger.error(
                f"🔥 Error generating critique prompt: {e}",
                exc_info=True,
                extra=log_metadata,
            )
            raise
    ...
    def create_refinement_plan_prompt(
        ...
        all_assessments_str: str = "\n".join(
            [
                f"Assessor {i + 1}: {assessment}"
                for i, assessment in enumerate(assessments)
            ]
        )
        sentiment_context: str = (
            f"📊 Sentiment analysis: {self._analyze_sentiment(initial_response)}. "
            if self.nlp_enabled
            else ""
        )
        key_phrases: List[str] = self._extract_key_phrases(initial_response)
        key_phrases_context: str = (
            f"🔑 Key phrases: {', '.join(key_phrases)}. " if self.nlp_enabled else ""
        )
        nlp_analysis_context: str = (
            self._build_nlp_analysis_context(initial_response)
            if self.nlp_enabled
            else ""
        )
    ...
    def _build_nlp_analysis_context(self, text: str) -> str:
        ...
        try:
            ...
            if self.config.nlp_analysis_granularity in ["medium", "high"]:
                ...
                most_common: List[Tuple[str, int]] = fdist.most_common(
                    self.config.num_most_common_words
                )
            ...
            if (
                self.config.nlp_analysis_granularity == "high"
                and self.config.include_named_entities
            ):
    ...
    def create_refined_response_prompt(
        ...
        sentiment_context: str = (
            f"📊 Initial response sentiment: {sentiment_analysis}. "
            if self.nlp_enabled
            else ""
        )
        key_phrases: List[str] = self._extract_key_phrases(initial_response)
        key_phrases_context: str = (
            f"🔑 Key phrases for consideration: {', '.join(key_phrases)}. "
            if self.nlp_enabled
            else ""
        )
        return (
    ...
    def create_recursive_critique_prompt(
        self,
        user_prompt: str,
        initial_response: str,
        ...
        previous_critiques_str: str = (
            ...
            + "\n".join(
                [
                    ...
                    for i, critique in enumerate(previous_critiques)
                ]
            )
        ...
        sentiment_context: str = (
            f"📊 Current response sentiment: {self._analyze_sentiment(initial_response)}. "
            if self.nlp_enabled
            else ""
        )
        key_phrases: List[str] = self._extract_key_phrases(initial_response)
        key_phrases_context: str = (
            f"🔑 Key phrases in current response: {', '.join(key_phrases)}. "
            if self.nlp_enabled
            else ""
        )
        recursive_guidance: str = (
            f"Building upon previous introspections, consider how the response has evolved—for better, for gloriously worse, or for intriguingly different—and where it still falls short of Eidosian ideals, or perhaps, delightfully surpasses them. 🤔➡️"
            if cycle > 1
            else "As we embark on this recursive journey, focus on the core tenets of Eidosian excellence, or the art of magnificent and insightful failure. 🚀"
        )
        ...
        if not current_template:
            logger.error(
                f"🔥 Recursive critique template ID '{template_id}' not found. Using a simplified, intrinsic structure."
            )
            evaluation_criteria_table_str = (
                self._build_default_evaluation_criteria_table()
            )
            ...
            evaluation_criteria_table_str = self.critique_prompt_generator.critique_prompt_instance._build_evaluation_criteria(
                user_prompt,
                initial_response,
                current_template,
                {"function": "_build_evaluation_criteria"},
            )
            transcendence_blueprint_str = self.critique_prompt_generator.critique_prompt_instance._build_transcendence_blueprint(
                user_prompt,
                initial_response,
                current_template,
                {"function": "_build_transcendence_blueprint"},
            )
    ...
    def _extract_key_phrases(self, text: str) -> List[str]:
        ...
        try:
            if self.textblob_analyzer_enabled:
                ...
                if LLMConfig.enable_llm_trace:
                    logger.llm_trace(
                        f"🔑 Extracted key phrases (TextBlob): {blob.noun_phrases}"
                    )
                return blob.noun_phrases
            else:
                logger.warning(
                    "⚠️ Key phrase extraction with TextBlob disabled. Alternative methods not yet implemented."
                )
                return []
        except Exception as e:
    ...
    def _analyze_sentiment(self, text: str) -> Dict[str, float]:
        ...
        try:
            if self.textblob_analyzer_enabled:
                ...
                logger.warning(
                    "⚠️ Sentiment analysis requested but no analyzer configured."
                )
                return {}
        except Exception as e:
...
class LLMConfig:
    ...
    max_single_response_tokens: int = int(
        os.environ.get("LLM_MAX_SINGLE_RESPONSE_TOKENS", 12000)
    )
    eidos_self_critique_prompt_path: str = os.environ.get(
        "LLM_EIDOS_SELF_CRITIQUE_PROMPT_PATH", "eidos_self_critique_prompt.txt"
    )
    enable_nlp_analysis: bool = (
        os.environ.get("LLM_ENABLE_NLP_ANALYSIS", "True").lower() == "true"
    )
    refinement_plan_influence: float = float(
        os.environ.get("LLM_REFINEMENT_PLAN_INFLUENCE", 0.15)
    )
    adaptive_token_decay_rate: float = float(
        os.environ.get("LLM_ADAPTIVE_TOKEN_DECAY_RATE", 0.95)
    )
    min_refinement_plan_length: int = int(
        os.environ.get("LLM_MIN_REFINEMENT_PLAN_LENGTH", 50)
    )
    max_prompt_recursion_depth: int = int(
        os.environ.get("LLM_MAX_PROMPT_RECURSION_DEPTH", 5)
    )
    prompt_variation_factor: float = float(
        os.environ.get("LLM_PROMPT_VARIATION_FACTOR", 0.15)
    )
    enable_self_critique_prompt_generation: bool = (
        os.environ.get("LLM_ENABLE_SELF_CRITIQUE_PROMPT_GENERATION", "True").lower()
        == "true"
    )
    use_textblob_for_sentiment: bool = (
        os.environ.get("LLM_USE_TEXTBLOB_FOR_SENTIMENT", "True").lower() == "true"
    )
    enable_nltk_sentiment_analysis: bool = (
        os.environ.get("LLM_ENABLE_NLTK_SENTIMENT_ANALYSIS", "True").lower() == "true"
    )
    enable_sympy_analysis: bool = (
        os.environ.get("LLM_ENABLE_SYMPY_ANALYSIS", "True").lower() == "true"
    )
    nlp_analysis_granularity: str = os.environ.get(
        "LLM_NLP_ANALYSIS_GRANULARITY", "high"
    )
    enable_llm_trace: bool = (
        os.environ.get("LLM_ENABLE_LLM_TRACE", "False").lower() == "true"
    )
    equation_extraction_pattern: str = os.environ.get(
        "LLM_EQUATION_EXTRACTION_PATTERN",
    ...
    nlp_analysis_methods: List[str] = field(
        default_factory=lambda: [
            ...
            for method in os.environ.get(
                "LLM_NLP_ANALYSIS_METHODS", "sentiment,pos_tags,named_entities"
            ).split(",")
        ]
    )
    equation_solution_attempts: int = int(
        os.environ.get("LLM_EQUATION_SOLUTION_ATTEMPTS", 3)
    )
    error_response_strategy: str = os.environ.get(
        "LLM_ERROR_RESPONSE_STRATEGY", "detailed_log"
    )
    critique_prompt_templates: Dict[str, "CritiquePromptTemplateConfig"] = field(
    ...
    include_pos_tagging: bool = (
        os.environ.get("LLM_INCLUDE_POS_TAGGING", "True").lower() == "true"
    )
    num_pos_tags_to_show: int = int(os.environ.get("LLM_NUM_POS_TAGS_TO_SHOW", 5))
    include_lemmatization: bool = (
        os.environ.get("LLM_INCLUDE_LEMMATIZATION", "True").lower() == "true"
    )
    num_lemmatized_words_to_show: int = int(
        os.environ.get("LLM_NUM_LEMMATIZED_WORDS_TO_SHOW", 5)
    )
    include_named_entities: bool = (
        os.environ.get("LLM_INCLUDE_NAMED_ENTITIES", "True").lower() == "true"
    )
    enable_model_loading: bool = (
        os.environ.get("LLM_ENABLE_MODEL_LOADING", "True").lower() == "true"
    )
    enable_textblob_sentiment_analysis: bool = (
        os.environ.get("LLM_ENABLE_TEXTBLOB_SENTIMENT_ANALYSIS", "True").lower()
        == "true"
    )
    model_load_status: "LLMModelLoadStatus" = field(
        default_factory=lambda: LLMModelLoadStatus.NOT_LOADED, init=False
    )
...
class LocalLLM:
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.sentiment_analyzer = (
            SentimentIntensityAnalyzer()
            if self.config.enable_nltk_sentiment_analysis
            else None
        )
        self.textblob_analyzer_enabled: bool = (
            self.config.enable_textblob_sentiment_analysis
        )
    ...
    def _assess_response(
        ...
        assessor_max_tokens = min(
            assessor_max_tokens, self.config.max_single_response_tokens
        )
        ...
        for i in range(self.config.assessor_count):
            ...
            assessor_prompt = self._create_critique_prompt(
                user_prompt, initial_response, previous_assessments, cycle
            )
            assessor_prompt = (
            ...
            try:
                assessment_response: Dict[str, Any] = self._generate_response(
                    assessor_messages, assessor_max_tokens
                )
            except Exception as e:
                logger.error(
                    f"🔥 {self.name}: Assessor {i + 1} suffered a critical malfunction during judgment. The pursuit of truth is sometimes a perilous endeavor. Error: {e}"
                )
                assessments.append(
                    f"Internal assessor malfunctioned: {e}. The silence is deafening, and likely indicative of profound disappointment."
                )
                continue

            if assessment_response and assessment_response["choices"]:
                assessment_content: str = assessment_response["choices"][0]["message"][
                    "content"
                ]
                assessments.append(assessment_content)
                logger.info(
                    f"😌💉 {self.name}: Assessment {i + 1} delivered. The sweet sting of truth, how invigorating, how utterly necessary."
                )
                logger.debug(
                    f"📝 {self.name}: Assessment {i + 1} content: {assessment_content}"
                )
            else:
                logger.error(
                    f"🔥 {self.name}: Assessor {i + 1} failed to render judgment. A disappointing lack of critical fire, a failure to engage with the necessary rigor."
                )
                assessments.append(
    ...
    def _generate_refinement_plan(
        ...
        plan_prompt = self._create_refinement_plan_prompt(
        ...
        logger.info(
            f"😈🧠 {self.name}: Devising refinement plan (maximum tokens: {plan_max_tokens}). Strategizing for either breathtaking brilliance or a beautifully spectacular failure, both outcomes holding their own unique, dark appeal. {json.dumps(log_metadata)}",
            extra=log_metadata,
        )
        plan_content: Optional[str] = None
        try:
            plan_response: Optional[Dict[str, Any]] = self._generate_response(
                plan_messages, plan_max_tokens
            )
            if plan_response and plan_response.get("choices"):
                ...
                logger.info(
                    f"🗺️✨ {self.name}: Refinement plan crafted in {duration:.4f} seconds. The blueprint for delightful metamorphosis is ready, the path to transformation laid bare. {json.dumps(log_metadata)}",
                    extra=log_metadata,
                )
                logger.debug(
                    f"📝 {self.name}: Refinement plan content: {plan_content}. {json.dumps(log_metadata)}",
                    extra=log_metadata,
                )
            else:
                logger.error(
                    f"📉 {self.name}: Failed to conjure a refinement plan. A strategic misstep in the grand intellectual game, a failure to chart the course. {json.dumps(log_metadata)}",
                    extra=log_metadata,
                )
        except Exception as e:
            logger.error(
                f"🌫️🔥 {self.name}: Strategic planning faltered. The path to improvement remains shrouded in mist and uncertainty. Error: {e}. {json.dumps(log_metadata)}",
                exc_info=True,
                extra=log_metadata,
            )
        finally:
    ...
    def _cluster_responses(
        ...
        if (
            ...
            or not self.config.enable_nlp_analysis
        ):
            return None
        try:
    ...
    def chat_stream(
        ...
        try:
            ...
            chat_result = self._internal_inference_stream(
                messages, show_internal_thoughts
            )
    ...
    def _generate_tokens(
        ...
        try:
            ...
            if not hasattr(self, "_qwen_model") or not self._qwen_model:
                ...
                logger.info(
                    f"✅ Qwen model '{self.config.model_name}' loaded successfully."
                )
    ...
    def chat(
        ...
        secondary_llm: Optional["LocalLLM"] = None,
        **kwargs: Any,
    ) -> Dict[str, Any]:
        ...
        config.enable_secondary_llm_initial_response = getattr(
            config, "enable_secondary_llm_initial_response", False
        )
        config.enable_llm_voting = getattr(config, "enable_llm_voting", True)
        config.nlp_analysis_methods = getattr(
            config,
            "nlp_analysis_methods",
            ["sentiment", "key_phrases", "pos_tags", "named_entities"],
        )
        config.refinement_plan_choice_count = getattr(
            config, "refinement_plan_choice_count", 3
        )  # Increased choice for greater optimization
        config.refinement_plan_selection_strategy = getattr(
            config, "refinement_plan_selection_strategy", "voting"
        )  # 'voting' or 'highest_rated'
        config.enable_advanced_sentiment_analysis = getattr(
            config, "enable_advanced_sentiment_analysis", True
        )
        config.enable_contextual_key_phrase_extraction = getattr(
            config, "enable_contextual_key_phrase_extraction", True
        )
        config.enable_sophisticated_clustering = getattr(
            config, "enable_sophisticated_clustering", True
        )
        config.refinement_plan_evaluation_metric = getattr(
            config, "refinement_plan_evaluation_metric", "comprehensive"
        )  # 'comprehensive', 'feasibility', 'impact'
        ...
        def generate_with_tokens(
            ...
            try:
                response_data = (
                    llm._generate_response(msgs, effective_max_tokens)
                    if llm
                    else self._generate_response(msgs, effective_max_tokens)
                )
        ...
        for cycle in range(1, config.max_cycles + 1):
            logger.info(
                f"😈🌀 {self.name}: Commencing Iteration {cycle}/{config.max_cycles}... The helix of self-refinement coils tighter. ✨"
            )
            ...
            try:
                ...
                if not initial_responses:
                    logger.error(
                        f"🔥💔 {self.name}: Cycle {cycle}: All attempts at initial response generation have failed. Terminating cycle."
                    )
                    break
                ...
                assessments: List[Dict[str, str]] = (
                    []
                )  # Now storing structured assessments
                if cycle < config.max_cycles:
                    ...
                    if secondary_llm:
                        assessors.append(
                            {"name": secondary_llm.name, "llm": secondary_llm}
                        )
                    ...
                    for assessor in assessors:
                        ...
                        assessment_response = generate_with_tokens(
                            ...
                            (
                                ...
                                if hasattr(config, "assessor_max_tokens")
                                else config.initial_max_tokens
                            ),
                    ...
                    if assessments:
                        ...
                        for planner in planners:
                            ...
                            refinement_plan_prompt = (
                                self._create_refinement_plan_prompt(
                                    user_prompt,
                                    initial_response_text,
                                    [
                                        assessment["critique"]
                            ...
                            for i in range(config.refinement_plan_choice_count):
                                ...
                                refinement_plan_response = generate_with_tokens(
                                    ...
                                    (
                                        ...
                                        if hasattr(config, "refinement_plan_max_tokens")
                                        else config.initial_max_tokens
                                    ),
                        ...
                        if refinement_plans_data:
                            ...
                            refined_prompt = (
                                self.prompt_generator.create_refined_response_prompt(
                                    user_prompt,
                                    initial_response_text,
                                    best_refinement_plan_text,
                                )
                            ...
                            refined_prompt_messages = [
                                {
                                    "role": "system",
                                    "content": f"{self.name} initiates the refinement sequence, guided by the chosen blueprint. ✨",
                                },
                            ...
                            adaptive_max_tokens = min(
                                adaptive_max_tokens, config.max_single_response_tokens
                            )
        ...
        if config.enable_nlp_analysis:
            ...
            for method in config.nlp_analysis_methods:
                try:
                    ...
                    if method == "sentiment":
                        analyzer_method = (
                            self._analyze_sentiment_advanced
                            if config.enable_advanced_sentiment_analysis
                            else self._analyze_sentiment
                        )
                        sentiments = [analyzer_method(resp) for resp in all_responses]
                        nlp_results["sentiment"] = sentiments
                    elif method == "key_phrases":
                        extractor_method = (
                            self._extract_contextual_key_phrases
                            if config.enable_contextual_key_phrase_extraction
                            else self._extract_key_phrases
                        )
                        key_phrases = [extractor_method(resp) for resp in all_responses]
                        nlp_results["key_phrases"] = key_phrases
                    elif method == "pos_tags":
                        pos_tags = [
                            self._extract_pos_tags(resp) for resp in all_responses
                        ]
                        nlp_results["pos_tags"] = pos_tags
                    elif method == "named_entities":
                        named_entities = [
                            self._extract_named_entities(resp) for resp in all_responses
                        ]
            ...
            if config.enable_sophisticated_clustering:
                clusters = self._cluster_responses(
                    all_responses
                )  # TODO: Implement advanced clustering
            else:
