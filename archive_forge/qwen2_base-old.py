...
from typing import Any, List, Optional, Dict, Tuple, Callable, Union
...
import networkx as nx
from networkx import graph, Graph
import os
import logging
from huggingface_hub import hf_hub_download
from eidos_config import DEFAULT_HF_TOKEN
...
class GraphRAGStore(SimplePropertyGraphStore):
    """
    A custom graph store that extends SimplePropertyGraphStore with community detection and summarization.

    This class enhances the basic graph store by adding functionality to build communities within the graph
    and generate summaries for each community using an LLM.

    Args:
        community_summary (Dict[Any, str]): A dictionary to store community summaries.
        max_cluster_size (int): The maximum size of a cluster for community detection.
    ...
    def generate_community_summary(self, text: str) -> str:
        ...
        This function takes a text representing a set of relationships and uses an LLM
        to generate a concise summary of these relationships.

        Args:
            text (str): The text containing the relationships to be summarized.

        Returns:
            str: A summary of the relationships generated by the LLM.
        ...
        messages = [
            ...
            # Define the user message with the input text
            ChatMessage(role="user", content=text),
        ]
        # Get the LLM's response
        response = llm.chat(messages)
        # Clean the response by removing the "assistant:" prefix and stripping whitespace
    ...
    def build_communities(self) -> None:
        ...
        This function converts the internal graph representation to a NetworkX graph,
        detects communities using hierarchical Leiden clustering, collects information
        about each community, and then generates summaries for each community.

        [all]
        """
        # Convert the internal graph to a NetworkX graph
        nx_graph = self._create_nx_graph()
        # Detect communities using hierarchical Leiden clustering
        community_hierarchical_clusters = hierarchical_leiden(
            nx_graph, max_cluster_size=self.max_cluster_size
        )
        # Collect information about each community
    ...
    def _create_nx_graph(self) -> nx.Graph:
        ...
        This function iterates through the nodes and relations in the internal graph
        representation and creates a corresponding NetworkX graph.

        Returns:
            nx.Graph: A NetworkX graph representing the internal graph.
        ...
        # Return the created NetworkX graph
        return nx_graph

    def _collect_community_info(
        self, nx_graph: nx.Graph, clusters: List[Any]
    ) -> Dict[Any, List[str]]:
        """Collect detailed information for each node based on their community.

        This function maps each node to its community, then collects detailed information
        about the relationships within each community.

        Args:
            nx_graph (nx.Graph): The NetworkX graph representing the knowledge graph.
            clusters (List[Any]): A list of cluster objects, each containing a node and its cluster ID.

        Returns:
            Dict[Any, List[str]]: A dictionary where keys are community IDs and values are lists of
            strings, each string representing a relationship within the community.
        ...
        for item in clusters:
            # Get the cluster ID and node
            cluster_id = item.cluster
            node = item.node
            # If the cluster ID is not in the community info dictionary, initialize it
            if cluster_id not in community_info:
                community_info[cluster_id] = []

            # Iterate through the neighbors of the node
            for neighbor in nx_graph.neighbors(node):
                # Check if the neighbor belongs to the same community
                if community_mapping[neighbor] == cluster_id:
                    # Get the edge data between the node and its neighbor
                    edge_data = nx_graph.get_edge_data(node, neighbor)
                    # If edge data exists, format the relationship and add it to the community info
                    if edge_data:
                        detail = f"{node} -> {neighbor} -> {edge_data['relationship']} -> {edge_data['description']}"
                        community_info[cluster_id].append(detail)
        # Return the collected community information
        return community_info

    def _summarize_communities(self, community_info: Dict[Any, List[str]]) -> None:
        """Generate and store summaries for each community.

        This function takes the collected community information and generates a summary
        for each community using the LLM. The summaries are stored in the `community_summary` attribute.

        Args:
            community_info (Dict[Any, List[str]]): A dictionary where keys are community IDs and values
            are lists of strings, each string representing a relationship within the community.

        [all]
        """
        # Iterate through each community and its details
        for community_id, details in community_info.items():
            # Join the details into a single text string and ensure it ends with a period
            details_text = "\n".join(details) + "."
            # Generate a summary for the community using the LLM and store it
    ...
    def get_community_summaries(self) -> Dict[Any, str]:
        ...
        This function checks if community summaries have been generated. If not, it
        builds the communities and generates the summaries. It then returns the summaries.

        Returns:
            Dict[Any, str]: A dictionary where keys are community IDs and values are the
            corresponding community summaries.

        [all]
        """
        # Check if community summaries have been generated
        if not self.community_summary:
            # If not, build the communities and generate summaries
            self.build_communities()
        # Return the community summaries
        return self.community_summary


class GraphRAGQueryEngine(CustomQueryEngine):
    """
    A custom query engine that uses community summaries to answer queries.

    This class extends CustomQueryEngine to provide a query engine that leverages
    community summaries generated by the GraphRAGStore to answer user queries.

    Args:
        graph_store (GraphRAGStore): The graph store containing the community summaries.
        llm (HuggingFaceLLM): The language model to use for generating answers.
    ...
    def custom_query(self, query_str: str) -> str:
        ...
        This function retrieves community summaries from the graph store, generates
        an answer for each summary based on the query, and then aggregates these answers
        ...
        Returns:
            str: The final aggregated answer to the query.
...
def parse_fn(
    ...
    """
    Parses the LLM response string to extract entities and relationships.
    ...
    Returns:
        Tuple[List[Tuple[str, str, str]], List[Tuple[str, str, str, str]]]:
            ...
            - The first list contains tuples representing entities (entity_name, entity_type, entity_description).
            - The second list contains tuples representing relationships
    ...
    logging.debug(
        f"Parsed {len(entities)} entities and {len(relationships)} relationships"
    )
...
class GraphRAGExtractor(TransformComponent):
    """
    Extracts triples (paths) from text in a set of BaseNodes using an LLM.

    The GraphRAGExtractor uses a prompt template to query an LLM with each node’s text,
    then parses the result to collect entity references and relationships (triplets).
    These are appended to the node’s metadata under KG_NODES_KEY and KG_RELATIONS_KEY.

    Args:
        llm (LLM): The language model to use.
        extract_prompt (Union[str, PromptTemplate], optional):
            The prompt to use for extracting triples. Defaults to DEFAULT_KG_TRIPLET_EXTRACT_PROMPT.
        parse_fn (Callable, optional):
            A function to parse the LLM's output into (entities, relationships).
            Defaults to default_parse_triplets_fn.
        max_paths_per_chunk (int, optional):
            The maximum number of paths (triplets) to extract per chunk. Defaults to 10.
        num_workers (int, optional):
            Number of async workers for concurrency. Defaults to 4.
    """
    ...
    def __init__(
        self,
        llm: Optional[HuggingFaceLLM] = llm,
        extract_prompt: Optional[Union[str, PromptTemplate]] = None,
        parse_fn: Callable = default_parse_triplets_fn,
        max_paths_per_chunk: int = 10,
        num_workers: int = 4,
    ) -> None:
        # Lazy import to avoid cyclical references
    ...
    def __call__(
        ...
        """
        Synchronous entrypoint to extract triples from a list of nodes.
        """
    ...
    async def acall(
        ...
        """
        Asynchronous entrypoint to extract triples from a list of nodes.
        """
    ...
    async def _aextract(self, node: BaseNode) -> BaseNode:
        """Helper method that runs the LLM extraction on a single node."""
        text = node.get_content(metadata_mode=MetadataMode.LLM)
        try:
            # Feed text into LLM with the extraction prompt
            ...
            # If parsing fails, gracefully handle by skipping
            entities, entities_relationship = [], []

        # Collect or initialize the node’s stored KG data
        existing_nodes = node.metadata.pop(KG_NODES_KEY, [])
        existing_relations = node.metadata.pop(KG_RELATIONS_KEY, [])

        # Create new EntityNodes
        metadata_copy = node.metadata.copy()
        for entity, entity_type, description in entities:
            metadata_copy["entity_description"] = description
            entity_node = EntityNode(
                name=entity,
                label=entity_type,
                properties=metadata_copy,
            )
        ...
        for subj, rel, obj, description in entities_relationship:
            subj_node = EntityNode(name=subj, properties=metadata_copy)
            obj_node = EntityNode(name=obj, properties=metadata_copy)
            metadata_copy["relationship_description"] = description
            rel_node = Relation(
                label=rel,
        ...
        # Attach our new/updated KG data back to the node
...
# Initialize kg_extractor with the documented parse_fn
