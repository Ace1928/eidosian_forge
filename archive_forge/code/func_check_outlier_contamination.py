...
def check_outlier_contamination(name, estimator_orig):
    ...
    for constraint in contamination_constraints:
        if isinstance(constraint, Interval):
            assert constraint.type == Real and constraint.left >= 0.0 and (constraint.right <= 0.5) and (constraint.left > 0 or constraint.closed in {'right', 'neither'}), 'contamination constraint should be an interval in (0, 0.5]'