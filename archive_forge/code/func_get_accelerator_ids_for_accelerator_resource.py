...
def get_accelerator_ids_for_accelerator_resource(self, resource_name: str, resource_regex: str) -> List[str]:
    ...
    if self.original_visible_accelerator_ids.get(resource_name, None) is not None:
        ...
        if self.mode == LOCAL_MODE:
            if resource_name == ray_constants.GPU:
                ...
                max_accelerators = self.node.get_resource_spec().resources.get(resource_name, None)
            if max_accelerators:
