...
CacheValueType = Any
CacheData = Dict[CacheKeyType, CacheValueType]
CacheInstanceType = Optional[AsyncKeyDB]
SerializedCacheData = bytes

# Global cache instance
global_cache: CacheInstanceType = None
...
CACHE_RETRY_LIMIT: int = 3
CACHE_RETRY_DELAY: int = 1
CACHE_FILE_PATH: str = "cache_data.pkl"
...
def async_cache(
    ...
    cache_instance: CacheInstanceType = None,
    ...
    def decorator(func: DecoratedCallable) -> Awaitable[Any]:
        ...
        async def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            for attempt in range(retry_limit):
                try:
                    cached_result: Optional[SerializedCacheData] = (
                        await cache_instance.get(cache_key_hash)
                    )
...
async def close_cache(cache_instance: CacheInstanceType = None) -> None:
    """
    Close the KeyDB cache instance.

    Args:
        cache_instance (CacheInstanceType, optional): The KeyDB cache instance to close. Defaults to None.
    """
    global global_cache
...
async def load_cache_from_file(
    ...
    Returns:
        Optional[CacheData]: The loaded cache data, or None if the file doesn't exist or an error occurs.
    """
    try:
        with open(file_path, "rb") as file:
            cache_data: CacheData = pickle.load(file)
...
async def save_cache_to_file(
    cache_data: CacheData, file_path: str = CACHE_FILE_PATH
) -> None:
    """
    Save the cache data to a file.

    Args:
        cache_data (CacheData): The cache data to save.
...
async def consolidate_cache_instances(
    ...
    """
    consolidated_cache_data: CacheData = {}

    for cache_instance in cache_instances:
        try:
            cache_data: CacheData = await cache_instance.get_all()
...
async def main() -> None:
    ...
    try:
        # Initialize cache
        cache: AsyncKeyDB = await initialize_cache()

        # Load cache data from file
        loaded_cache_data: Optional[CacheData] = await load_cache_from_file()
        if loaded_cache_data:
            logger.info(f"Loaded cache data: {loaded_cache_data}")

        # Call the cached function
        result: Dict[str, Any] = await fetch_data("example")
