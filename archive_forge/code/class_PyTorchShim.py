...
class PyTorchShim(Shim):
    ...
    *not* a Thinc Model subclass itself.
    ...
    def __init__(self, model: Any, config=None, optimizer: Any=None, mixed_precision: bool=False, grad_scaler: Optional[PyTorchGradScaler]=None, device: Optional['torch.device']=None, serialize_model: Optional[Callable[[Any], bytes]]=None, deserialize_model: Optional[Callable[[Any, bytes, 'torch.device'], Any]]=None):
    ...
    def begin_update(self, inputs: ArgsKwargs):
        ...
        def backprop(grads):
            grads.kwargs['grad_tensors'] = self._grad_scaler.scale(grads.kwargs['grad_tensors'], inplace=True)
            torch.autograd.backward(*grads.args, **grads.kwargs)
            grad_tensors = []
            for torch_data in itertools.chain(self._model.parameters(), iterate_recursive(lambda x: hasattr(x, 'grad'), inputs)):
        ...
        for name, torch_data in self._model.named_parameters():
            if torch_data.grad is not None:
                if not self._grad_scaler.found_inf:
                    param, grad = optimizer((self.id, name), cast(FloatsXd, torch2xp(torch_data.data)), cast(FloatsXd, torch2xp(torch_data.grad)))
                    torch_data.data = xp2torch(param, requires_grad=True, device=torch_data.device)
