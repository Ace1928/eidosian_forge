...
class BaseStep(metaclass=abc.ABCMeta):
    ...
    def run(self, output_directory: str):
        ...
        try:
            ...
            self._update_status(status=StepStatus.FAILED, output_directory=output_directory, stack_trace=stack_trace)
            self.step_card = FailureCard(recipe_name=self.recipe_name, step_name=self.name, failure_traceback=stack_trace, output_directory=output_directory)
            raise
        finally:
    ...
    def inspect(self, output_directory: str):
        ...
        if not os.path.exists(card_path):
            _logger.info("Unable to locate runtime info for step '%s'. Re-run the step before inspect.", self.name)
            return None
    ...
    def _update_status(self, status: StepStatus, output_directory: str, stack_trace: Optional[str]=None) -> None:
    ...
    @staticmethod
    def _generate_worst_examples_dataframe(dataframe, predictions, error, target_col, worst_k=10):
        """
        ...
        result_df = dataframe.iloc[worst_k_indexes].assign(prediction=predictions[worst_k_indexes], absolute_error=abs_error[worst_k_indexes])
