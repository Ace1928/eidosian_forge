...
class AsyncioAuthenticator(Authenticator):
    ...
    def __init__(self, context: Optional['zmq.Context']=None, loop: Any=None, encoding: str='utf-8', log: Any=None):
        super().__init__(context, encoding, log)
        if loop is not None:
            warnings.warn(f'{self.__class__.__name__}(loop) is deprecated and ignored', DeprecationWarning, stacklevel=2)
        self.__poller = None
        self.__task = None
