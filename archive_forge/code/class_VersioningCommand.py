...
class VersioningCommand(Command):
    ...
    help_spec = Command.HelpSpec(help_name='versioning', help_name_aliases=['getversioning', 'setversioning'], help_type='command_help', help_one_line_summary='Enable or suspend versioning for one or more buckets', help_text=_DETAILED_HELP_TEXT, subcommand_help_text={'get': _get_help_text, 'set': _set_help_text})
    gcloud_storage_map = GcloudStorageMap(gcloud_command={'get': GcloudStorageMap(gcloud_command=['storage', 'buckets', 'list', '--raw', _GCLOUD_FORMAT_STRING], flag_map={}, supports_output_translation=True), 'set': GcloudStorageMap(gcloud_command={'on': GcloudStorageMap(gcloud_command=['storage', 'buckets', 'update', '--versioning'], flag_map={}), 'off': GcloudStorageMap(gcloud_command=['storage', 'buckets', 'update', '--no-versioning'], flag_map={})}, flag_map={})}, flag_map={})
    ...
    def _SetVersioning(self):
        ...
        for url_str in url_args:
            ...
            for blr in bucket_iter:
                url = blr.storage_url
                some_matched = True
                bucket_metadata = apitools_messages.Bucket(versioning=apitools_messages.Bucket.VersioningValue())
    ...
    def _GetVersioning(self):
        ...
        some_matched = False
        for url_str in url_args:
            bucket_iter = self.GetBucketUrlIterFromArg(url_str, bucket_fields=['versioning'])
            for blr in bucket_iter:
                some_matched = True
    ...
    def RunCommand(self):
        ...
        if action_subcommand == 'get':
            ...
            if versioning_arg in ('on', 'off'):
                metrics.LogCommandParams(subcommands=[action_subcommand, versioning_arg])
        else:
