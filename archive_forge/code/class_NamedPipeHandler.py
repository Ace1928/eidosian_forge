...
class NamedPipeHandler(object):
    ...
    def __init__(self, pipe_name, input_queue=None, output_queue=None, connect_event=None, log_file=None):
    ...
    def start(self):
        try:
            ...
            msg = _('Named pipe handler failed to initialize. Pipe Name: %(pipe_name)s Error: %(err)s') % {'pipe_name': self._pipe_name, 'err': err}
            LOG.error(msg)
            self.stop()
    ...
    def stop(self):
        ...
        for worker in self._workers:
            while worker.is_alive() and worker is not threading.current_thread():
                self._cancel_io()
                worker.join(0.5)
        self._workers = []
    ...
    def _read_callback(self, num_bytes):
        data = self._ioutils.get_buffer_data(self._r_buffer, num_bytes)
