...
class Calibrator:
  ...
  """
    ...
    def __init__(self, model_content, custom_op_registerers_by_name=None, custom_op_registerers_by_func=None):
        """Constructor.
    ...
    """
        ...
        try:
            self._calibrator = _calibration_wrapper.CalibrationWrapper(model_content, custom_op_registerers_by_name, custom_op_registerers_by_func)
    ...
    def calibrate_and_quantize(self, dataset_gen, input_type, output_type, allow_float, activations_type=dtypes.int8, bias_type=dtypes.int32, resize_input=True, disable_per_channel=False):
    ...
    """
        ...
        return self._calibrator.QuantizeModel(np.dtype(input_type.as_numpy_dtype()).num, np.dtype(output_type.as_numpy_dtype()).num, allow_float, np.dtype(activations_type.as_numpy_dtype()).num, np.dtype(bias_type.as_numpy_dtype()).num, disable_per_channel)
