...
class SankeyPlot(GraphPlot):
    ...
    label_position = param.ObjectSelector(default='right', objects=['left', 'right', 'outer', 'inner'], doc='\n        Whether node labels should be placed to the left, right, outer or inner.')
    ...
    label_index = param.ClassSelector(default=2, class_=(str, int), allow_None=True, doc='\n        Index of the dimension from which the node labels will be drawn')
    ...
    def _compute_labels(self, element, data, mapping):
        ...
        if is_outer_inner:
            ...
            mapping['text_2'] = dict(text='text', x='x', y='y', text_baseline='middle', text_align=align_2)
