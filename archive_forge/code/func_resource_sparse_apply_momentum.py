...
def resource_sparse_apply_momentum(var: _atypes.TensorFuzzingAnnotation[_atypes.Resource], accum: _atypes.TensorFuzzingAnnotation[_atypes.Resource], lr: _atypes.TensorFuzzingAnnotation[TV_ResourceSparseApplyMomentum_T], grad: _atypes.TensorFuzzingAnnotation[TV_ResourceSparseApplyMomentum_T], indices: _atypes.TensorFuzzingAnnotation[TV_ResourceSparseApplyMomentum_Tindices], momentum: _atypes.TensorFuzzingAnnotation[TV_ResourceSparseApplyMomentum_T], use_locking: bool=False, use_nesterov: bool=False, name=None):
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'ResourceSparseApplyMomentum', name, var, accum, lr, grad, indices, momentum, 'use_locking', use_locking, 'use_nesterov', use_nesterov)
            return _result
            ...
            pass
        try:
            return resource_sparse_apply_momentum_eager_fallback(var, accum, lr, grad, indices, momentum, use_locking=use_locking, use_nesterov=use_nesterov, name=name, ctx=_ctx)
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('ResourceSparseApplyMomentum', var=var, accum=accum, lr=lr, grad=grad, indices=indices, momentum=momentum, use_locking=use_locking, use_nesterov=use_nesterov, name=name)
    return _op