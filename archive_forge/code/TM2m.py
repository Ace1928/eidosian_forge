...
from collections import (
    defaultdict,
)  # Importing defaultdict from collections to provide a dictionary with default values for missing keys, simplifying handling of missing data.
from typing import (
    List,
    Tuple,
    Dict,
    Set,
)  # Importing specific types from the typing module to support type hinting, enhancing code readability and maintainability by specifying the expected data types.
from functools import (
    lru_cache,
)  # Importing lru_cache from functools to decorate functions whose return values should be cached for optimized performance by storing the results of expensive function calls.

# Initialize logging with maximum verbosity and detail
logging.basicConfig(
    level=logging.DEBUG,  # Setting the logging level to DEBUG to capture all levels of log messages, ensuring that every detail of the execution process is logged.
    format="%(asctime)s - %(levelname)s - %(message)s",  # Defining the format of log messages to include the time, the level of severity, and the message, providing a comprehensive and detailed logging output.
    datefmt="%Y-%m-%d %H:%M:%S",  # Adding a date format to the logging setup to ensure that each log entry is timestamped with precision to the second, aiding in precise tracking of events.
    handlers=[
        logging.FileHandler("debug.log"),
        logging.StreamHandler(),
    ],  # Adding multiple handlers to direct the logs to both a file and the console, ensuring that logs are accessible both during runtime and for later review.
)
...
def create_grid(size: int) -> np.ndarray:
    ...
    # Utilizing numpy to create a two-dimensional array with all elements initialized to zero
    grid = np.zeros((size, size), dtype=int)

    # Logging the creation of the grid with detailed information
    logging.debug(
        f"Grid of size {size}x{size} created with all elements initialized to zero using NumPy."
    )

    # Returning the created grid as a NumPy array
    return grid
