...
def avg_pool3d_grad(orig_input_shape: _atypes.TensorFuzzingAnnotation[_atypes.Int32], grad: _atypes.TensorFuzzingAnnotation[TV_AvgPool3DGrad_T], ksize, strides, padding: str, data_format: str='NDHWC', name=None) -> _atypes.TensorFuzzingAnnotation[TV_AvgPool3DGrad_T]:
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'AvgPool3DGrad', name, orig_input_shape, grad, 'ksize', ksize, 'strides', strides, 'padding', padding, 'data_format', data_format)
            return _result
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('AvgPool3DGrad', orig_input_shape=orig_input_shape, grad=grad, ksize=ksize, strides=strides, padding=padding, data_format=data_format, name=name)
    _result = _outputs[:]
