...
class TestNetworkLocalIpAssociation(TestNetworkProxy):
    ...
    def test_local_ip_association_create_attrs(self):
        self.verify_create(self.proxy.create_local_ip_association, local_ip_association.LocalIPAssociation, method_kwargs={'local_ip': LOCAL_IP_ID}, expected_kwargs={'local_ip_id': LOCAL_IP_ID})

    def test_local_ip_association_delete(self):
        self.verify_delete(self.proxy.delete_local_ip_association, local_ip_association.LocalIPAssociation, ignore_missing=False, method_args=[LOCAL_IP_ID, 'resource_or_id'], expected_args=['resource_or_id'], expected_kwargs={'if_revision': None, 'local_ip_id': LOCAL_IP_ID})

    def test_local_ip_association_delete_ignore(self):
        self.verify_delete(self.proxy.delete_local_ip_association, local_ip_association.LocalIPAssociation, ignore_missing=True, method_args=[LOCAL_IP_ID, 'resource_or_id'], expected_args=['resource_or_id'], expected_kwargs={'if_revision': None, 'local_ip_id': LOCAL_IP_ID})

    def test_local_ip_association_find(self):
        lip = local_ip.LocalIP.new(id=LOCAL_IP_ID)
        self._verify('openstack.proxy.Proxy._find', self.proxy.find_local_ip_association, method_args=['local_ip_association_id', lip], expected_args=[local_ip_association.LocalIPAssociation, 'local_ip_association_id'], expected_kwargs={'ignore_missing': True, 'local_ip_id': LOCAL_IP_ID})

    def test_local_ip_association_get(self):
        lip = local_ip.LocalIP.new(id=LOCAL_IP_ID)
        self._verify('openstack.proxy.Proxy._get', self.proxy.get_local_ip_association, method_args=['local_ip_association_id', lip], expected_args=[local_ip_association.LocalIPAssociation, 'local_ip_association_id'], expected_kwargs={'local_ip_id': LOCAL_IP_ID})

    def test_local_ip_associations(self):
        self.verify_list(self.proxy.local_ip_associations, local_ip_association.LocalIPAssociation, method_kwargs={'local_ip': LOCAL_IP_ID}, expected_kwargs={'local_ip_id': LOCAL_IP_ID})