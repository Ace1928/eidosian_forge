    ...
    **Note:** This layer is safe to use inside a `tf.data` pipeline
    ...
    def call(self, inputs):
        ...
        if init_height is None or init_width is None:
            raise ValueError(f'At this time, CenterCrop can only process images with a static spatial shape. Received: inputs.shape={inputs.shape}')
        ...
        if h_diff >= 0 and w_diff >= 0:
            if len(inputs.shape) == 4:
                if self.data_format == 'channels_first':
                    return inputs[:, :, h_start:h_start + self.height, w_start:w_start + self.width]
                return inputs[:, h_start:h_start + self.height, w_start:w_start + self.width, :]
            elif len(inputs.shape) == 3:
                if self.data_format == 'channels_first':
                    return inputs[:, h_start:h_start + self.height, w_start:w_start + self.width]
                return inputs[h_start:h_start + self.height, w_start:w_start + self.width, :]
        return image_utils.smart_resize(inputs, [self.height, self.width], data_format=self.data_format, backend_module=self.backend)
    ...
    def get_config(self):
        ...
        config = {'height': self.height, 'width': self.width, 'data_format': self.data_format}
