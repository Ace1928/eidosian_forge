...
def AuthorizeProjectToUseTestingKmsKey(key_name=KmsTestingResources.CONSTANT_KEY_NAME):
    """Ensures test keys exist and that the service agent is authorized."""
    kms_api = KmsApi(logging.getLogger())
    keyring_fully_qualified_name = kms_api.CreateKeyRing(PopulateProjectId(None), KmsTestingResources.KEYRING_NAME, location=KmsTestingResources.KEYRING_LOCATION)
    key_fully_qualified_name = kms_api.CreateCryptoKey(keyring_fully_qualified_name, key_name)
    cmd = GetGsutilCommand(['kms', 'authorize', '-k', key_fully_qualified_name], force_gsutil=True)
