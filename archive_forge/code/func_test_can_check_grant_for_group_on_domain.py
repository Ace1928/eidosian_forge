...
def test_can_check_grant_for_group_on_domain(self):
    ...
    with self.test_client() as c:
        c.get('/v3/domains/%s/groups/%s/roles/%s' % (self.domain_id, group['id'], self.bootstrapper.reader_role_id), headers=self.headers, expected_status_code=http.client.NO_CONTENT)