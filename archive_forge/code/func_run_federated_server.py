...
def run_federated_server(port: int, world_size: int, server_key_path: str='', server_cert_path: str='', client_cert_path: str='') -> None:
    ...
    if build_info()['USE_FEDERATED']:
        if not server_key_path or not server_cert_path or (not client_cert_path):
            ...
            _check_call(_LIB.XGBRunFederatedServer(port, world_size, c_str(server_key_path), c_str(server_cert_path), c_str(client_cert_path)))
    else:
        raise XGBoostError('XGBoost needs to be built with the federated learning plugin enabled in order to use this module')