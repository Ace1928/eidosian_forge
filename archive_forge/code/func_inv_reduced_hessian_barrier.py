...
def inv_reduced_hessian_barrier(model, independent_variables, bound_tolerance=1e-06, solver_options=None, tee=False):
    """
    ...
    if len(estimated_mu) == 0:
        ...
        if any([abs(mu - estmu) > 1e-07 for estmu in estimated_mu]):
    ...
    for v in ind_vardatas:
        if v.has_lb() and pyo.value(v) - v.lb <= bound_tolerance or (v.has_ub() and v.ub - pyo.value(v) <= bound_tolerance):
            raise ValueError('Independent variable: {} has a solution value that is near its bound (according to tolerance). The reduced hessian computation does not support this at this time. All independent variables should be in their interior.'.format(v))
