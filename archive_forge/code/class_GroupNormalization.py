...
class GroupNormalization(Layer):
    ...
    def __init__(self, groups=32, axis=-1, epsilon=0.001, center=True, scale=True, beta_initializer='zeros', gamma_initializer='ones', beta_regularizer=None, gamma_regularizer=None, beta_constraint=None, gamma_constraint=None, **kwargs):
    ...
    def build(self, input_shape):
        ...
        if self.groups == -1:
            self.groups = dim
        if dim < self.groups:
            raise ValueError(f'Number of groups ({self.groups}) cannot be more than the number of channels ({dim}).')
        if dim % self.groups != 0:
            raise ValueError(f'Number of groups ({self.groups}) must be a multiple of the number of channels ({dim}).')
        self.input_spec = InputSpec(ndim=len(input_shape), axes={self.axis: dim})
        if self.scale:
            self.gamma = self.add_weight(shape=(dim,), name='gamma', initializer=self.gamma_initializer, regularizer=self.gamma_regularizer, constraint=self.gamma_constraint)
        else:
            self.gamma = None
        if self.center:
            self.beta = self.add_weight(shape=(dim,), name='beta', initializer=self.beta_initializer, regularizer=self.beta_regularizer, constraint=self.beta_constraint)
        else:
            self.beta = None
    ...
    def call(self, inputs, mask=None):
        ...
        normalized_inputs = self._apply_normalization(reshaped_inputs=reshaped_inputs, input_shape=input_shape, reshaped_mask=reshaped_mask)
    ...
    def _apply_normalization(self, *, reshaped_inputs, reshaped_mask, input_shape):
        ...
        mean, variance = tf.nn.weighted_moments(reshaped_inputs, axes=group_reduction_axes, frequency_weights=mask_weights, keepdims=True)
