    ...
    **Examples:**
    ...
    def step_and_cost(self, qnode, *args, grad_fn=None, recompute_tensor=True, metric_tensor_fn=None, **kwargs):
        ...
        if not isinstance(qnode, qml.QNode) and metric_tensor_fn is None:
            raise ValueError('The objective function must be encoded as a single QNode for the natural gradient to be automatically computed. Otherwise, metric_tensor_fn must be explicitly provided to the optimizer.')
        if recompute_tensor or self.metric_tensor is None:
            ...
            self.metric_tensor = self.metric_tensor + self.lam * qml.math.eye(size, like=_metric_tensor)
    ...
    def step(self, qnode, *args, grad_fn=None, recompute_tensor=True, metric_tensor_fn=None, **kwargs):
        ...
        """
        new_args, _ = self.step_and_cost(qnode, *args, grad_fn=grad_fn, recompute_tensor=recompute_tensor, metric_tensor_fn=metric_tensor_fn, **kwargs)
        return new_args
