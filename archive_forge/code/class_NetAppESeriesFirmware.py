...
class NetAppESeriesFirmware(NetAppESeriesModule):
    ...
    def __init__(self):
        ansible_options = dict(nvsram=dict(type='str', required=False), firmware=dict(type='str', required=True), wait_for_completion=dict(type='bool', default=False), clear_mel_events=dict(type='bool', default=False))
        super(NetAppESeriesFirmware, self).__init__(ansible_options=ansible_options, web_services_version='02.00.0000.0000', supports_check_mode=True)
    ...
    def wait_for_web_services(self):
        ...
        for count in range(int(self.REBOOT_TIMEOUT_SEC / 5)):
            try:
                if self.is_firmware_bundled():
                    firmware_rc, firmware_version = self.request("storage-systems/%s/graph/xpath-filter?query=/controller/codeVersions[codeModule='bundleDisplay']" % self.ssid, log_request=False)
                    current_firmware_version = six.b(firmware_version[0]['versionString'])
                else:
                    firmware_rc, firmware_version = self.request('storage-systems/%s/graph/xpath-filter?query=/sa/saData/fwVersion' % self.ssid, log_request=False)
