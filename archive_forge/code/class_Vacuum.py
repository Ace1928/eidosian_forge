...
class Vacuum(ParlaiScript):
    ...
    @classmethod
    def setup_args(cls):
        parser = ParlaiParser(False, False, description='Shrink a model file for release.')
        parser.add_argument('-mf', '--model-file', dest='path', help='Path to model file.')
        return parser
    ...
    def run(self):
        ...
        states = torch.load(model_file, map_location=lambda cpu, _: cpu, pickle_module=parlai.utils.pickle)
        logging.info(f'Backing up {model_file} to {model_file}.unvacuumed')
        os.rename(model_file, model_file + '.unvacuumed')
        for key in ['optimizer', 'optimizer_type', 'lr_scheduler', 'lr_scheduler_type', 'warmup_scheduler', 'number_training_updates']:
            if key in states:
