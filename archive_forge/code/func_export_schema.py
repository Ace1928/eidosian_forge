...
def export_schema(obj: 'XMLSchemaBase', target_dir: str, save_remote: bool=False, remove_residuals: bool=True) -> None:
    ...
    while True:
        ...
        for schema in list(exports):
            ...
            pattern = '\\bschemaLocation\\s*=\\s*[\\\'\\"](.*)[\\\'"]'
            schema_locations = set((x.strip() for x in re.findall(pattern, exports[schema][1])))
            for location, ref_schema in chain(schema.includes.items(), imports_items):
                ...
                if is_remote_url(location):
                    ...
                    path = _PurePath(parts.scheme).joinpath(parts.netloc).joinpath(parts.path.lstrip('/'))
                else:
