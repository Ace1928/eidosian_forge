...
class NetAppONTAPPortset:
    ...
    def portset_get(self):
        ...
        try:
            ...
            self.module.fail_json(msg='Error fetching portset %s: %s' % (self.parameters['name'], to_native(error)), exception=traceback.format_exc())
    ...
    def create_portset(self):
        ...
        try:
            self.server.invoke_successfully(portset_info, enable_tunneling=True)
        except netapp_utils.zapi.NaApiError as error:
            self.module.fail_json(msg='Error creating portset %s: %s' % (self.parameters['name'], to_native(error)), exception=traceback.format_exc())
    ...
    def delete_portset(self):
        ...
        try:
            ...
            self.module.fail_json(msg='Error deleting portset %s: %s' % (self.parameters['name'], to_native(error)), exception=traceback.format_exc())
    ...
    def portset_get_rest(self):
        ...
        if error:
            self.module.fail_json(msg='Error fetching portset %s: %s' % (self.parameters['name'], to_native(error)))
        portset_info = None
        if record:
    ...
    def delete_portset_rest(self):
        ...
        if error:
            self.module.fail_json(msg='Error deleting portset %s: %s' % (self.parameters['name'], to_native(error)))
    ...
    def get_san_lifs_rest(self, san_lifs):
        ...
        if missing_lifs and self.parameters['state'] == 'present':
            error_msg = 'Error: lifs: %s of type %s not found in vserver %s' % (', '.join(missing_lifs), self.parameters.get('type', 'fcp or iscsi'), self.parameters['vserver'])
    ...
    def apply(self):
        ...
        if cd_action is None and self.parameters['state'] == 'present':
            if self.parameters.get('type') and self.parameters['type'] != current['type']:
                self.module.fail_json(msg='modify protocol(type) not supported and %s already exists in vserver %s under different type' % (self.parameters['name'], self.parameters['vserver']))
