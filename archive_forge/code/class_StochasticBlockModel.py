...
class StochasticBlockModel(Graph):
    ...
    def __init__(self, N=1024, k=5, z=None, M=None, p=0.7, q=None, directed=False, self_loops=False, connected=False, max_iter=10, seed=None, **kwargs):
        ...
        if M is None:
            ...
            if p.size == 1:
                p = p * np.ones(k)
            if p.shape != (k,):
                raise ValueError('Optional parameter p is neither a scalar nor a vector of length k.')
            if q is None:
                q = 0.3 / k
            q = np.asarray(q)
            if q.size == 1:
                q = q * np.ones((k, k))
            if q.shape != (k, k):
                raise ValueError('Optional parameter q is neither a scalar nor a matrix of size k x k.')
            M = q
            M.flat[::k + 1] = p
        ...
        for nb_iter in range(max_iter):
            ...
            if nb_iter == max_iter - 1:
                raise ValueError('The graph could not be connected after {} trials. Increase the connection probability or the number of trials.'.format(max_iter))
