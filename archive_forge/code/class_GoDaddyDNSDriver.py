...
class GoDaddyDNSDriver(DNSDriver):
    ...
    RECORD_TYPE_MAP = {RecordType.A: 'A', RecordType.AAAA: 'AAAA', RecordType.CNAME: 'CNAME', RecordType.MX: 'MX', RecordType.NS: 'SPF', RecordType.SRV: 'SRV', RecordType.TXT: 'TXT'}
    ...
    def __init__(self, shopper_id, key, secret, secure=True, host=None, port=None):
        ...
        super().__init__(key=key, secret=secret, secure=secure, host=host, port=port, shopper_id=str(shopper_id))
    ...
    def create_record(self, name, zone, type, data, extra=None):
        ...
        self.connection.request('/v1/domains/%s/records' % zone.domain, method='PATCH', data=json.dumps([new_record]))
        id = self._get_id_of_record(name, type)
        return Record(id=id, name=name, type=type, data=data, zone=zone, driver=self, ttl=new_record['ttl'], extra=extra)
    ...
    def update_record(self, record, name, type, data, extra=None):
        ...
        """
        new_record = self._format_record(name, type, data, extra)
        self.connection.request('/v1/domains/{}/records/{}/{}'.format(record.zone.domain, record.type, record.name), method='PUT', data=json.dumps([new_record]))
        id = self._get_id_of_record(name, type)
        return Record(id=id, name=name, type=type, data=data, zone=record.zone, driver=self, ttl=new_record['ttl'], extra=extra)
    ...
    def get_record(self, zone_id, record_id):
        ...
        result = self.connection.request('/v1/domains/{}/records/{}/{}'.format(zone_id, parts[1], parts[0])).object
        if len(result) == 0:
    ...
    def _to_zone(self, item):
        ...
        zone = Zone(id=item['domainId'], domain=item['domain'], type='master', ttl=None, driver=self, extra=extra)
        return zone
    ...
    def _to_record(self, item, zone=None):
        ...
        name = item['name']
        id = self._get_id_of_record(name, type)
        record = Record(id=id, name=name, type=type, data=item['data'], zone=zone, driver=self, ttl=ttl)
        return record
