...
class ShufflerIterDataPipe(IterDataPipe[T_co]):
    ...
    def __init__(self, datapipe: MapDataPipe[T_co], *, indices: Optional[List]=None) -> None:
        super().__init__()
    ...
    def __getstate__(self):
        state = (self.datapipe, self.indices, self._enabled, self._seed, self._rng.getstate(), self._shuffled_indices, self._valid_iterator_id, self._number_of_samples_yielded)
        if IterDataPipe.getstate_hook is not None:
            return IterDataPipe.getstate_hook(state)
        return state

    def __setstate__(self, state):
        self.datapipe, self.indices, self._enabled, self._seed, rng_state, self._shuffled_indices, self._valid_iterator_id, self._number_of_samples_yielded = state
        self._rng = random.Random()
