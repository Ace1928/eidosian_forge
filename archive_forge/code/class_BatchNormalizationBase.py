...
class BatchNormalizationBase(Layer):
    ...
    *after having been trained on data that has similar statistics as the
    ...
    def __init__(self, axis=-1, momentum=0.99, epsilon=0.001, center=True, scale=True, beta_initializer='zeros', gamma_initializer='ones', moving_mean_initializer='zeros', moving_variance_initializer='ones', beta_regularizer=None, gamma_regularizer=None, beta_constraint=None, gamma_constraint=None, renorm=False, renorm_clipping=None, renorm_momentum=0.99, fused=None, trainable=True, virtual_batch_size=None, adjustment=None, name=None, synchronized=False, **kwargs):
        ...
        if synchronized and fused:
            raise ValueError('`fused=True` is not supported when `synchronized=True`.')
        ...
        if renorm:
            ...
            if set(renorm_clipping) - set(keys):
                raise ValueError(f'Received invalid keys for `renorm_clipping` argument: {renorm_clipping}. Supported values: {keys}.')
    ...
    def build(self, input_shape):
        ...
        if self.virtual_batch_size is not None:
            if self.virtual_batch_size <= 0:
                raise ValueError(f'`virtual_batch_size` must be a positive integer that divides the true batch size of the input tensor. Received: virtual_batch_size={self.virtual_batch_size}')
            if 0 in self.axis:
                raise ValueError(f'When using `virtual_batch_size`, the batch dimension must be 0 and thus axis cannot include 0. Received axis={self.axis}')
            if self.adjustment is not None:
        ...
        if self.fused in (None, True):
            if self._USE_V2_BEHAVIOR:
                if self.fused is None:
                    ...
                    raise ValueError(f'Batch normalization layers with `fused=True` only support 4D or 5D input tensors. Received tensor with shape: {tuple(input_shape)}')
            else:
        ...
        if self.fused:
            if self.axis == [1] and rank == 4:
                ...
                raise ValueError(f'Unsupported axis. The use of `fused=True` is only possible with `axis=1` or `axis=3` for 4D input tensors. Received: axis={tuple(self.axis)}')
            else:
                raise ValueError(f'Unsupported axis. The use of `fused=True` is only possible with `axis=1` or `axis=4` for 5D input tensors. Received: axis={tuple(self.axis)}')
        axis_to_dim = {x: input_shape.dims[x].value for x in self.axis}
        for x in axis_to_dim:
            if axis_to_dim[x] is None:
                raise ValueError(f'Input has undefined `axis` dimension. Received input with shape {tuple(input_shape)} and axis={tuple(self.axis)}')
        self.input_spec = InputSpec(ndim=rank, axes=axis_to_dim)
        if len(axis_to_dim) == 1 and self.virtual_batch_size is None:
            param_shape = (list(axis_to_dim.values())[0],)
        else:
            param_shape = [axis_to_dim[i] if i in axis_to_dim else 1 for i in range(rank)]
            if self.virtual_batch_size is not None:
        ...
        if self.scale:
            self.gamma = self.add_weight(name='gamma', shape=param_shape, dtype=self._param_dtype, initializer=self.gamma_initializer, regularizer=self.gamma_regularizer, constraint=self.gamma_constraint, trainable=True, experimental_autocast=False)
        else:
            self.gamma = None
        if self.center:
            self.beta = self.add_weight(name='beta', shape=param_shape, dtype=self._param_dtype, initializer=self.beta_initializer, regularizer=self.beta_regularizer, constraint=self.beta_constraint, trainable=True, experimental_autocast=False)
        else:
            self.beta = None
        try:
            if hasattr(self, '_scope') and self._scope:
                ...
                partitioner = None
            self.moving_mean = self.add_weight(name='moving_mean', shape=param_shape, dtype=self._param_dtype, initializer=self.moving_mean_initializer, synchronization=tf.VariableSynchronization.ON_READ, trainable=False, aggregation=tf.VariableAggregation.MEAN, experimental_autocast=False)
            self.moving_variance = self.add_weight(name='moving_variance', shape=param_shape, dtype=self._param_dtype, initializer=self.moving_variance_initializer, synchronization=tf.VariableSynchronization.ON_READ, trainable=False, aggregation=tf.VariableAggregation.MEAN, experimental_autocast=False)
            if self.renorm:

                def moving_stddev_initializer(*args, **kwargs):
                    return tf.sqrt(self.moving_variance_initializer(*args, **kwargs))
                with tf.distribute.get_strategy().extended.colocate_vars_with(self.moving_variance):
                    self.moving_stddev = self.add_weight(name='moving_stddev', shape=param_shape, dtype=self._param_dtype, initializer=moving_stddev_initializer, synchronization=tf.VariableSynchronization.ON_READ, trainable=False, aggregation=tf.VariableAggregation.MEAN, experimental_autocast=False)

                def _renorm_variable(name, shape, initializer='zeros'):
                    """Create a renorm variable."""
                    var = self.add_weight(name=name, shape=shape, dtype=self._param_dtype, initializer=initializer, synchronization=tf.VariableSynchronization.ON_READ, trainable=False, aggregation=tf.VariableAggregation.MEAN, experimental_autocast=False)
                    return var
                with tf.distribute.get_strategy().extended.colocate_vars_with(self.moving_mean):
                    self.renorm_mean = _renorm_variable('renorm_mean', param_shape, self.moving_mean_initializer)
                with tf.distribute.get_strategy().extended.colocate_vars_with(self.moving_stddev):
                    self.renorm_stddev = _renorm_variable('renorm_stddev', param_shape, moving_stddev_initializer)
        finally:
            if partitioner:
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        _raise_for_non_sync_bn_with_renorm_and_dtensor_strategy(synchronized=self.synchronized, training=training, renorm=self.renorm)
        if self.virtual_batch_size is not None:
            original_shape = tf.shape(inputs)
            original_shape = tf.concat([tf.constant([-1]), original_shape[1:]], axis=0)
            if tf.__internal__.tf2.enabled():
                expanded_shape = [self.virtual_batch_size, -1] if training_value else [-1, 1]
                expanded_shape = tf.concat([tf.constant(expanded_shape), original_shape[1:]], axis=0)
            else:
                expanded_shape = tf.concat([tf.constant([self.virtual_batch_size, -1]), original_shape[1:]], axis=0)
        ...
        if training_value == False:
            ...
            if self.adjustment:
                ...
                adj_scale = control_flow_util.smart_cond(training, lambda: adj_scale, lambda: tf.ones_like(adj_scale))
                adj_bias = control_flow_util.smart_cond(training, lambda: adj_bias, lambda: tf.zeros_like(adj_bias))
                scale, offset = _compose_transforms(adj_scale, adj_bias, scale, offset)
            keep_dims = self.virtual_batch_size is not None or len(self.axis) > 1
            mean, variance = self._moments(tf.cast(inputs, self._param_dtype), reduction_axes, keep_dims=keep_dims, mask=mask)
            moving_mean = self.moving_mean
            moving_variance = self.moving_variance
            mean = control_flow_util.smart_cond(training, lambda: mean, lambda: tf.convert_to_tensor(moving_mean))
            variance = control_flow_util.smart_cond(training, lambda: variance, lambda: tf.convert_to_tensor(moving_variance))
            if self.virtual_batch_size is not None:
                new_mean = tf.reduce_mean(mean, axis=1, keepdims=True)
                new_variance = tf.reduce_mean(variance, axis=1, keepdims=True)
            elif utils.running_with_dtensor_strategy() and (not self.synchronized):
            ...
            if self.renorm:
                r, d, new_mean, new_variance = self._renorm_correction_and_moments(new_mean, new_variance, training, input_batch_size)
    ...
    def get_config(self):
        ...
        if self.adjustment is not None:
            logging.warning('The `adjustment` function of this `BatchNormalization` layer cannot be serialized and has been omitted from the layer config. It will not be included when re-creating the layer from the saved config.')
    ...
    def _fused_batch_norm(self, inputs, mask, training):
        ...
        if mask is not None:
            warnings.warn('Masking is not supported with `fused=True`. You should either turn off fusing (`fused=False`) or you should not pass a `mask` argument when calling the layer. For the moment `mask` will be ignored for the normalization.')
        if self.center:
            beta = self.beta
        else:
        ...
        if self._support_zero_size_input():
            ...
            input_batch_size = None
        use_fused_avg_updates = tf.compat.v1.executing_eagerly_outside_functions() and isinstance(self.momentum, (float, int)) and (get_enclosing_xla_context() is None)
        ...
        def _fused_batch_norm_inference():
            return tf.compat.v1.nn.fused_batch_norm(inputs, gamma, beta, mean=self.moving_mean, variance=self.moving_variance, epsilon=self.epsilon, is_training=False, data_format=self._data_format)
        output, mean, variance = control_flow_util.smart_cond(training, _fused_batch_norm_training, _fused_batch_norm_inference)
        variance = _maybe_add_or_remove_bessels_correction(variance, remove=True)
        training_value = control_flow_util.constant_value(training)
        if training_value or training_value is None:
            if not use_fused_avg_updates:
                if training_value is None:
                    momentum = control_flow_util.smart_cond(training, lambda: self.momentum, lambda: 1.0)
                else:
            ...
            def variance_update():
                ...
                if use_fused_avg_updates:
                    if input_batch_size is not None:
                        new_variance = control_flow_util.smart_cond(input_batch_size > 0, lambda: variance, lambda: self.moving_variance)
                    else:
    ...
    def _calculate_mean_and_var(self, inputs, reduction_axes, keep_dims, mask=None):
        if self.synchronized:
            return self._sync_calculate_mean_and_var(inputs, reduction_axes, keep_dims, mask=mask)
        return self._no_sync_calculate_mean_and_var(inputs, reduction_axes, keep_dims, mask=mask)
    ...
    def _dtensor_calculate_mean_and_var(self, inputs, reduction_axes, keep_dims, mask=None):
        if self.synchronized:
            return self._dtensor_sync_calculate_mean_and_var(inputs, reduction_axes, keep_dims, mask=mask)
        return self._dtensor_no_sync_calculate_mean_and_var(inputs, reduction_axes, keep_dims, mask=mask)
    ...
    def _dtensor_sync_calculate_mean_and_var(self, inputs, reduction_axes, keep_dims, mask=None):
        return self._no_sync_calculate_mean_and_var(inputs, reduction_axes, keep_dims, mask)

    def _moments(self, inputs, reduction_axes, keep_dims, mask=None):
        if utils.running_with_dtensor_strategy():
            mean, variance = self._dtensor_calculate_mean_and_var(inputs, reduction_axes, keep_dims, mask=mask)
        else:
            mean, variance = self._calculate_mean_and_var(inputs, reduction_axes, keep_dims, mask=mask)
        if self._support_zero_size_input():
            input_batch_size = tf.shape(inputs)[0]
