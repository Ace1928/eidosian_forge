...
def stage_eager_fallback(values, capacity: int, memory_limit: int, container: str, shared_name: str, name, ctx):
    ...
    _attrs = ('capacity', capacity, 'memory_limit', memory_limit, 'dtypes', _attr_dtypes, 'container', container, 'shared_name', shared_name)
    _result = _execute.execute(b'Stage', 0, inputs=_inputs_flat, attrs=_attrs, ctx=ctx, name=name)
    _result = None
    return _result