        ...
        while attempts < self.retries:
            try:
                ...
                if self.enable_performance_logging:
                    log_performance(func, start_time, end_time)
                ...
                if self.dynamic_retry_enabled:
                    dynamic_retries, dynamic_delay = dynamic_retry_strategy(exception=e)
                    self.retries = (
...
async def ensure_logging_config_exists(path: FILE_PATH) -> None:
    ...
    try:
        if not path.exists():
            ...
            with aiofiles.open(path, mode="w", encoding="utf-8") as config_file:
                json.dump(
...
class AsyncValidationException(ValueError):
    """A specific exception type for async validation failures."""

    def __init__(self, argument: str, value: Any, *args: Any) -> None:
...
async def validate_async_rules(func: T, *args: Any, **kwargs: Any) -> None:
    """
...
async def async_log_decorator(
    ...
    """
    A decorator that logs the entry and exit of functions. It supports both asynchronous and synchronous functions,
    including those that return values.

    Args:
        func (Callable[..., Union[T, Coroutine[Any, Any, T]]]): The function to decorate.

    Returns:
        Callable[..., Union[T, Coroutine[Any, Any, T]]]: The wrapped function.
    """
