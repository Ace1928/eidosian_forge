...
@TransformationFactory.register('contrib.strip_var_bounds', doc='Strip bounds from variables.')
...
class VariableBoundStripper(NonIsomorphicTransformation):
    ...
    """
    CONFIG = ConfigBlock()
    CONFIG.declare('strip_domains', ConfigValue(default=True, domain=bool, description='strip the domain for discrete variables as well'))
    CONFIG.declare('reversible', ConfigValue(default=False, domain=bool, description='Whether the bound stripping will be temporary. If so, store information for reversion.'))

    def _apply_to(self, instance, **kwds):
        config = self.CONFIG(kwds)
        if config.reversible:
            if any((hasattr(instance, map_name) for map_name in ['_tmp_var_bound_strip_lb', '_tmp_var_bound_strip_ub', '_tmp_var_bound_strip_domain'])):
                raise RuntimeError('Variable stripping reversion component maps already exist. Did you already apply a temporary transformation without a subsequent reversion?')
