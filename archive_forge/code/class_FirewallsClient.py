...
class FirewallsClient(ClientEntityBase):
    ...
    def get_actions_list(self, firewall: Firewall | BoundFirewall, status: list[str] | None=None, sort: list[str] | None=None, page: int | None=None, per_page: int | None=None) -> ActionsPageResult:
        ...
        response = self._client.request(url=f'/firewalls/{firewall.id}/actions', method='GET', params=params)
        actions = [BoundAction(self._client.actions, action_data) for action_data in response['actions']]
        return ActionsPageResult(actions, Meta.parse_meta(response))

    def get_actions(self, firewall: Firewall | BoundFirewall, status: list[str] | None=None, sort: list[str] | None=None) -> list[BoundAction]:
        """Returns all action objects for a Firewall.
        ...
        """
        return self._iter_pages(self.get_actions_list, firewall, status=status, sort=sort)
    ...
    def get_list(self, label_selector: str | None=None, page: int | None=None, per_page: int | None=None, name: str | None=None, sort: list[str] | None=None) -> FirewallsPageResult:
    ...
    def get_all(self, label_selector: str | None=None, name: str | None=None, sort: list[str] | None=None) -> list[BoundFirewall]:
        ...
        """
        return self._iter_pages(self.get_list, label_selector=label_selector, name=name, sort=sort)
    ...
    def create(self, name: str, rules: list[FirewallRule] | None=None, labels: str | None=None, resources: list[FirewallResource] | None=None) -> CreateFirewallResponse:
        ...
        if response.get('actions') is not None:
            actions = [BoundAction(self._client.actions, action_data) for action_data in response['actions']]
        result = CreateFirewallResponse(firewall=BoundFirewall(self, response['firewall']), actions=actions)
        return result

    def update(self, firewall: Firewall | BoundFirewall, labels: dict[str, str] | None=None, name: str | None=None) -> BoundFirewall:
        ...
        if name is not None:
            data['name'] = name
        response = self._client.request(url=f'/firewalls/{firewall.id}', method='PUT', json=data)
        return BoundFirewall(self, response['firewall'])
    ...
    def delete(self, firewall: Firewall | BoundFirewall) -> bool:
        ...
        :return: boolean
        """
        self._client.request(url=f'/firewalls/{firewall.id}', method='DELETE')
        return True

    def set_rules(self, firewall: Firewall | BoundFirewall, rules: list[FirewallRule]) -> list[BoundAction]:
        ...
        for rule in rules:
            data['rules'].append(rule.to_payload())
        response = self._client.request(url=f'/firewalls/{firewall.id}/actions/set_rules', method='POST', json=data)
