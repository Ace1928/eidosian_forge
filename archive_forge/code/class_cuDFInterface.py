...
class cuDFInterface(PandasInterface):
    ...
    def init(cls, eltype, data, kdims, vdims):
        ...
        if kdims and vdims is None:
            ...
            if vdims is None:
                vdims = [d for d in columns[ndim:ndim + nvdim if nvdim else None] if d not in kdims]
        ...
        if any((isinstance(d, (np.int64, int)) for d in kdims + vdims)):
            raise DataError('cudf DataFrame column names used as dimensions must be strings not integers.', cls)
        if kdims:
