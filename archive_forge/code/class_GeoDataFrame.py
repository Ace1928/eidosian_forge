...
class GeoDataFrame(GeoPandasBase, DataFrame):
    ...
    def __init__(self, data=None, *args, geometry=None, crs=None, **kwargs):
        with compat.ignore_shapely2_warnings():
            if kwargs.get('copy') is None and isinstance(data, DataFrame) and (not isinstance(data, GeoDataFrame)):
        ...
        if geometry is None and self.columns.nlevels == 1 and ('geometry' in self.columns):
            if (self.columns == 'geometry').sum() > 1:
                raise ValueError("GeoDataFrame does not support multiple columns using the geometry column name 'geometry'.")
            try:
                if hasattr(self['geometry'].values, 'crs') and self['geometry'].values.crs and crs and (not self['geometry'].values.crs == crs):
                    raise ValueError(crs_mismatch_error)
        ...
        if geometry is not None:
            if hasattr(geometry, 'crs') and geometry.crs and crs and (not geometry.crs == crs):
                raise ValueError(crs_mismatch_error)
            self.set_geometry(geometry, inplace=True, crs=crs)
        if geometry is None and crs:
            raise ValueError("Assigning CRS to a GeoDataFrame without a geometry column is not supported. Supply geometry using the 'geometry=' keyword argument, or by providing a DataFrame with column name 'geometry'")
    ...
    geometry = property(fget=_get_geometry, fset=_set_geometry, doc='Geometry data for GeoDataFrame')
    ...
    def crs(self):
        ...
        try:
            ...
            raise AttributeError('The CRS attribute of a GeoDataFrame without an active geometry column is not defined. Use GeoDataFrame.set_geometry to set the active geometry column.')

    @crs.setter
    def crs(self, value):
        """Sets the value of the crs"""
        if self._geometry_column_name is None:
            raise ValueError('Assigning CRS to a GeoDataFrame without a geometry column is not supported. Use GeoDataFrame.set_geometry to set the active geometry column.')
        if hasattr(self.geometry.values, 'crs'):
            self.geometry.values.crs = value
        else:
            raise ValueError('Assigning CRS to a GeoDataFrame without an active geometry column is not supported. Use GeoDataFrame.set_geometry to set the active geometry column.')
    ...
    def to_json(self, na='null', show_bbox=False, drop_id=False, to_wgs84=False, **kwargs):
        ...
        if to_wgs84:
            if self.crs:
                ...
                raise ValueError('CRS is not set. Cannot re-project to WGS84 (EPSG:4326).')
        else:
            df = self
        ...
        if df.crs is not None and (not df.crs.equals('epsg:4326')):
            ...
            if auth_crsdef is None or auth_crsdef[0] not in allowed_authorities:
                warnings.warn("GeoDataFrame's CRS is not representable in URN OGC format. Resulting JSON will contain no CRS information.", stacklevel=2)
            else:
    ...
    def to_parquet(self, path, index=None, compression='snappy', schema_version=None, **kwargs):
        ...
        if engine not in ('auto', 'pyarrow'):
            raise ValueError(f'GeoPandas only supports using pyarrow as the engine for to_parquet: {engine!r} passed instead.')
        from geopandas.io.arrow import _to_parquet
        _to_parquet(self, path, compression=compression, index=index, schema_version=schema_version, **kwargs)

    def to_feather(self, path, index=None, compression=None, schema_version=None, **kwargs):
        ...
        _to_feather(self, path, index=index, compression=compression, schema_version=schema_version, **kwargs)
    ...
    def set_crs(self, crs=None, epsg=None, inplace=False, allow_override=False):
        ...
        if not inplace:
            ...
            df = self
        df.geometry = df.geometry.set_crs(crs=crs, epsg=epsg, allow_override=allow_override, inplace=True)
        return df
    ...
    def __getitem__(self, key):
        ...
        if pd.api.types.is_scalar(key) and key == '' and isinstance(self.columns, pd.MultiIndex) and isinstance(result, Series) and (not is_geometry_type(result)):
    ...
    def __setitem__(self, key, value):
        ...
        """
        if not pd.api.types.is_list_like(key) and (key == self._geometry_column_name or (key == 'geometry' and self._geometry_column_name is None)):
            ...
            try:
                ...
                warnings.warn('Geometry column does not contain geometry.', stacklevel=2)
...
.DataFrame.merge.html
    ...
    def apply(self, func, axis=0, raw=False, result_type=None, args=(), **kwargs):
        result = super().apply(func, axis=axis, raw=raw, result_type=result_type, args=args, **kwargs)
        if not compat.PANDAS_GE_14 and isinstance(result, GeoDataFrame) and (result._geometry_column_name is None):
            result._geometry_column_name = self._geometry_column_name
        if isinstance(result, DataFrame) and self._geometry_column_name in result.columns:
            try:
    ...
    def _constructor_from_mgr(self, mgr, axes):
        if not any((isinstance(block.dtype, GeometryDtype) for block in mgr.blocks)):
            return _geodataframe_constructor_with_fallback(pd.DataFrame._from_mgr(mgr, axes))
    ...
    def __finalize__(self, other, method=None, **kwargs):
        ...
        if method == 'merge':
            ...
            if (self.columns == self._geometry_column_name).sum() > 1:
                raise ValueError(f"Concat operation has resulted in multiple columns using the geometry column name '{self._geometry_column_name}'.\nPlease ensure this column from the first DataFrame is not repeated.")
        elif method == 'unstack':
            self._geometry_column_name = None
            self._crs = None
        return self

    def dissolve(self, by=None, aggfunc='first', as_index=True, level=None, sort=True, observed=False, dropna=True, **kwargs):
        """
        ...
        **kwargs :
        ...
        groupby_kwargs = {'by': by, 'level': level, 'sort': sort, 'observed': observed, 'dropna': dropna}
        ...
        for w in record:
            if str(w.message).startswith('The default value of numeric_only'):
                msg = f"The default value of numeric_only in aggfunc='{aggfunc}' within pandas.DataFrameGroupBy.agg used in dissolve is deprecated. In pandas 2.0, numeric_only will default to False. Either specify numeric_only as additional argument in dissolve() or select only columns which should be valid for the function."
        ...
        def merge_geometries(block):
            ...
            return merged_geom
        g = self.groupby(group_keys=False, **groupby_kwargs)[self.geometry.name].agg(merge_geometries)
    ...
    def explode(self, column=None, ignore_index=False, index_parts=None, **kwargs):
        ...
        if index_parts is None:
            if not ignore_index:
                warnings.warn('Currently, index_parts defaults to True, but in the future, it will default to False to be consistent with Pandas. Use `index_parts=True` to keep the current behavior and True/False to silence the warning.', FutureWarning, stacklevel=2)
            index_parts = True
        exploded_geom = self.geometry.reset_index(drop=True).explode(index_parts=True)
        df = self.drop(self._geometry_column_name, axis=1).take(exploded_geom.index.droplevel(-1))
        df[exploded_geom.name] = exploded_geom.values
        df = df.set_geometry(self._geometry_column_name).__finalize__(self)
        if ignore_index:
            df.reset_index(inplace=True, drop=True)
        elif index_parts:
            df = df.set_index(exploded_geom.index.droplevel(list(range(exploded_geom.index.nlevels - 1))), append=True)
        return df
    ...
    def convert_dtypes(self, *args, **kwargs):
        ...
        """
        return GeoDataFrame(super().convert_dtypes(*args, **kwargs), geometry=self.geometry.name, crs=self.crs)
    ...
    def __xor__(self, other):
        ...
        warnings.warn("'^' operator will be deprecated. Use the 'symmetric_difference' method instead.", FutureWarning, stacklevel=2)
        return self.geometry.symmetric_difference(other)

    def __or__(self, other):
        """Implement | operator as for builtin set type"""
        warnings.warn("'|' operator will be deprecated. Use the 'union' method instead.", FutureWarning, stacklevel=2)
        return self.geometry.union(other)

    def __and__(self, other):
        """Implement & operator as for builtin set type"""
        warnings.warn("'&' operator will be deprecated. Use the 'intersection' method instead.", FutureWarning, stacklevel=2)
        return self.geometry.intersection(other)

    def __sub__(self, other):
        """Implement - operator as for builtin set type"""
        warnings.warn("'-' operator will be deprecated. Use the 'difference' method instead.", FutureWarning, stacklevel=2)
    ...
    def sjoin(self, df, *args, **kwargs):
        ...
        """
        return geopandas.sjoin(*args, left_df=self, right_df=df, **kwargs)

    def sjoin_nearest(self, right, how='inner', max_distance=None, lsuffix='left', rsuffix='right', distance_col=None, exclusive=False):
        """
        ...
        dimension is not taken into account.
        """
        return geopandas.sjoin_nearest(self, right, how=how, max_distance=max_distance, lsuffix=lsuffix, rsuffix=rsuffix, distance_col=distance_col, exclusive=exclusive)
    ...
    def overlay(self, right, how='intersection', keep_geom_type=None, make_valid=True):
        ...
        dimension is not taken into account.
        """
        return geopandas.overlay(self, right, how=how, keep_geom_type=keep_geom_type, make_valid=make_valid)