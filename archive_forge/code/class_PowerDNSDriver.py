...
class PowerDNSDriver(DNSDriver):
    ...
    def create_record(self, name, zone, type, data, extra=None):
        ...
        if self._pdns_version() == 3:
            record = {'content': data, 'disabled': False, 'name': name, 'ttl': extra['ttl'], 'type': type}
            payload = {'rrsets': [{'name': name, 'type': type, 'changetype': 'REPLACE', 'records': [record]}]}
        elif self._pdns_version() == 4:
            record = {'content': data, 'disabled': extra.get('disabled', False), 'set-ptr': False}
            payload = {'rrsets': [{'name': name, 'type': type, 'changetype': 'REPLACE', 'ttl': extra['ttl'], 'records': [record]}]}
            if 'comment' in extra:
        ...
        try:
            ...
            raise e
        return Record(id=None, name=name, data=data, type=type, zone=zone, driver=self, ttl=extra['ttl'])
    ...
    def create_zone(self, domain, type=None, ttl=None, extra={}):
        ...
        try:
            ...
            if e.code == httplib.UNPROCESSABLE_ENTITY and e.message.startswith("Domain '%s' already exists" % domain):
    ...
    def delete_record(self, record):
        ...
        :rtype: ``bool``
        """
        action = '{}/servers/{}/zones/{}'.format(self.api_root, self.ex_server, record.zone.id)
    ...
    def update_record(self, record, name, type, data, extra=None):
        ...
        if extra is None or extra.get('ttl', None) is None:
            raise ValueError('PowerDNS requires a ttl value for every record')
        if self._pdns_version() == 3:
            updated_record = {'content': data, 'disabled': False, 'name': name, 'ttl': extra['ttl'], 'type': type}
            payload = {'rrsets': [{'name': record.name, 'type': record.type, 'changetype': 'DELETE'}, {'name': name, 'type': type, 'changetype': 'REPLACE', 'records': [updated_record]}]}
        elif self._pdns_version() == 4:
            disabled = False
            if 'disabled' in extra:
                disabled = extra['disabled']
            updated_record = {'content': data, 'disabled': disabled, 'set-ptr': False}
            payload = {'rrsets': [{'name': name, 'type': type, 'changetype': 'REPLACE', 'ttl': extra['ttl'], 'records': [updated_record]}]}
            if 'comment' in extra:
                payload['rrsets'][0]['comments'] = extra['comment']
        try:
            self.connection.request(action=action, data=json.dumps(payload), method='PATCH')
    ...
    def _to_zone(self, item):
        extra = {}
        for e in ['kind', 'dnssec', 'account', 'masters', 'serial', 'notified_serial', 'last_check']:
            extra[e] = item[e]
        return Zone(id=item['id'], domain=item['name'], type=None, ttl=None, driver=self, extra=extra)
    ...
    def _to_record(self, item, zone, record=None):
        if record is None:
            ...
            data = record['content']
        return Record(id=None, name=item['name'], data=data, type=item['type'], zone=zone, driver=self, ttl=item['ttl'])
