...
def TestCpMvPOSIXLocalToBucketNoErrors(cls, bucket_uri, is_cp=True):
  ...
  """
    ...
    for obj_name, attrs_dict in six.iteritems(test_params):
        ...
        cls.RunGsUtil(['cp' if is_cp else 'mv', '-P', fpath, suri(bucket_uri, obj_name)])
        if uid != NA_ID:
            cls.VerifyObjectCustomAttribute(bucket_uri.bucket_name, obj_name, UID_ATTR, str(uid))
        if gid != NA_ID:
            cls.VerifyObjectCustomAttribute(bucket_uri.bucket_name, obj_name, GID_ATTR, str(gid))
        if mode != NA_MODE:
            cls.VerifyObjectCustomAttribute(bucket_uri.bucket_name, obj_name, MODE_ATTR, str(mode))