...
class ArtifactFileCache:
    ...
    def cleanup(self, target_size: Optional[int]=None, remove_temp: bool=False, target_fraction: Optional[float]=None) -> int:
        ...
        for entry in files_in(self._temp_dir):
            ...
            if remove_temp:
                ...
                temp_size += size
        if temp_size:
            wandb.termwarn(f'Cache contains {util.to_human_size(temp_size)} of temporary files. Run `wandb artifact cleanup --remove-temp` to remove them.')
        entries = []
        ...
        if total_size > target_size:
            wandb.termerror(f'Failed to reclaim enough space in {self._cache_dir}. Try running `wandb artifact cleanup --remove-temp` to remove temporary files.')
        return bytes_reclaimed
