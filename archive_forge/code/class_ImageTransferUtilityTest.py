...
class ImageTransferUtilityTest(base.TestCase):
    ...
    def test_download_stream_optimized_image_invalid_ova(self):
        self._test_download_stream_optimized_image(container='ova', invalid_ova=True)
    ...
    def test_upload_image(self, fake_transfer, fake_rw_handles_VmdkReadHandle):
        ...
        image_transfer.upload_image(context, timeout_secs, image_service, image_id, owner_id, session=session, host=host, port=port, vm=vm, vmdk_file_path=file_path, vmdk_size=image_size, is_public=is_public, image_name=image_name, image_version=image_version, store_id=store_id, base_image_ref=base_image_ref)
        fake_rw_handles_VmdkReadHandle.assert_called_once_with(session, host, port, vm, file_path, image_size)
        ver_str = str(image_version)
        image_metadata = {'disk_format': 'vmdk', 'name': image_name, 'properties': {'vmware_image_version': ver_str, 'vmware_disktype': 'streamOptimized', 'owner_id': owner_id}}
        image_service.update.assert_called_once_with(context, image_id, image_metadata, data=fake_VmdkReadHandle, store_id=store_id, base_image_ref=base_image_ref)