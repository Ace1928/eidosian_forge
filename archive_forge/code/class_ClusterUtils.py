...
class ClusterUtils(baseutils.BaseUtils):
    ...
    def check_cluster_state(self):
        if len(list(self._get_cluster_nodes())) < 1:
            raise exceptions.HyperVClusterException(_('Not enough cluster nodes.'))
    ...
    def _lookup_vm_group(self, vm_name):
        return self._lookup_res(self._conn_cluster.MSCluster_ResourceGroup, vm_name)
    ...
    def add_vm_to_cluster(self, vm_name, max_failover_count=1, failover_period=6, auto_failback=True):
    ...
    def cluster_enum(self, object_type):
        with self._cmgr.open_cluster_enum(object_type) as enum_handle:
            object_count = self._clusapi_utils.cluster_get_enum_count(enum_handle)
            for idx in range(object_count):
                item = self._clusapi_utils.cluster_enum(enum_handle, idx)
                item_dict = dict(version=item.dwVersion, type=item.dwType, id=item.lpszId, name=item.lpszName)
                yield item_dict
    ...
    def live_migrate_vm(self, vm_name, new_host, timeout=None):
        self._migrate_vm(vm_name, new_host, self._LIVE_MIGRATION_TYPE, constants.CLUSTER_GROUP_ONLINE, timeout)
    ...
    def _is_migration_pending(self, group_state, group_status_info, expected_state):
        migration_pending = group_state != expected_state or self._is_migration_queued(group_status_info)
    ...
    def get_cluster_group_type(self, group_name):
        with self._cmgr.open_cluster_group(group_name) as group_handle:
            ...
            return self._clusapi_utils.get_cluster_group_type(ctypes.byref(buff), buff_sz)
