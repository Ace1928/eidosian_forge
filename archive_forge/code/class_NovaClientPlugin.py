...
class NovaClientPlugin(microversion_mixin.MicroversionMixin, client_plugin.ClientPlugin):
    ...
    def _get_args(self, version):
        ...
        return {'session': self.context.keystone_session, 'endpoint_type': endpoint_type, 'service_type': self.COMPUTE, 'region_name': self._get_region_name(), 'connect_retries': cfg.CONF.client_retry_limit, 'http_log_debug': self._get_client_option(CLIENT_NAME, 'http_log_debug')}
    ...
    def is_unprocessable_entity(self, ex):
        http_status = getattr(ex, 'http_status', None) or getattr(ex, 'code', None)
        return isinstance(ex, exceptions.ClientException) and http_status == 422
    ...
    def rebuild(self, server_id, image_id, password=None, preserve_ephemeral=False, meta=None, files=None):
        """Rebuild the server and call check_rebuild to verify."""
        server = self.fetch_server(server_id)
        if server:
            server.rebuild(image_id, password=password, preserve_ephemeral=preserve_ephemeral, meta=meta, files=files)
            return True
        else:
            return False
    ...
    def attach_volume(self, server_id, volume_id, device):
        try:
            va = self.client().volumes.create_server_volume(server_id=server_id, volume_id=volume_id, device=device)
        except Exception as ex:
            if self.is_conflict(ex):
                return False
            elif self.is_client_exception(ex):
                raise exception.Error(_('Failed to attach volume %(vol)s to server %(srv)s - %(err)s') % {'vol': volume_id, 'srv': server_id, 'err': ex})
            else:
                raise
        return va.id
    ...
    def detach_volume(self, server_id, attach_id):
        try:
            ...
            if self.is_conflict(ex):
                return False
            elif not (self.is_not_found(ex) or self.is_bad_request(ex)):
                raise exception.Error(_('Could not detach attachment %(att)s from server %(srv)s.') % {'srv': server_id, 'att': attach_id})
        return True
    ...
    def dissociate_floatingip(self, floatingip_id):
        request_body = {'floatingip': {'port_id': None, 'fixed_ip_address': None}}
        self.clients.client('neutron').update_floatingip(floatingip_id, request_body)
    ...
    def interface_attach(self, server_id, port_id=None, net_id=None, fip=None, security_groups=None):
        server = self.fetch_server(server_id)
        if server:
            ...
            if not port_id and security_groups:
                ...
                self.clients.client('neutron').update_port(attachment.port_id, {'port': props})
            return True
        else:
            return False
