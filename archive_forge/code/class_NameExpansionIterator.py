...
class NameExpansionIterator:
  ...
  """
    ...
    def __init__(self, urls_iterable, fields_scope=cloud_api.FieldsScope.NO_ACL, ignore_symlinks=False, include_buckets=BucketSetting.NO, managed_folder_setting=folder_util.ManagedFolderSetting.DO_NOT_LIST, object_state=cloud_api.ObjectState.LIVE, preserve_symlinks=False, raise_error_for_unmatched_urls=True, raise_managed_folder_precondition_errors=True, recursion_requested=RecursionSetting.NO_WITH_WARNING, url_found_match_tracker=None):
    ...
    """
        ...
        self._urls_iterator = plurality_checkable_iterator.PluralityCheckableIterator(urls_iterable)
        ...
        self._raise_managed_folder_precondition_errors = raise_managed_folder_precondition_errors
        ...
        self._top_level_iterator = plurality_checkable_iterator.PluralityCheckableIterator(self._get_top_level_iterator())
        self._has_multiple_top_level_resources = None

    def _get_wildcard_iterator(self, url, managed_folder_setting=folder_util.ManagedFolderSetting.DO_NOT_LIST):
        """Returns get_wildcard_iterator with instance variables as args."""
        return wildcard_iterator.get_wildcard_iterator(url, fetch_encrypted_object_hashes=True, fields_scope=self._fields_scope, ignore_symlinks=self._ignore_symlinks, managed_folder_setting=managed_folder_setting, object_state=self.object_state, preserve_symlinks=self._preserve_symlinks, raise_managed_folder_precondition_errors=self._raise_managed_folder_precondition_errors)
    ...
    def _get_name_expansion_result(self, resource, expanded_url, original_url):
        ...
        keep_generation_in_url = self.object_state is cloud_api.ObjectState.LIVE_AND_NONCURRENT or original_url.generation
        if not keep_generation_in_url:
            new_storage_url = storage_url.storage_url_from_string(resource.storage_url.versionless_url_string)
    ...
    """
        ...
        for input_url, name_expansion_result in self._top_level_iterator:
            self._raise_error_if_multiple_sources_include_stdin(name_expansion_result.expanded_url)
            should_return_bucket = self._include_buckets is BucketSetting.YES and name_expansion_result.resource.storage_url.is_bucket()
            should_return_managed_folder = self._managed_folder_setting in {folder_util.ManagedFolderSetting.LIST_WITH_OBJECTS, folder_util.ManagedFolderSetting.LIST_WITHOUT_OBJECTS} and isinstance(name_expansion_result.resource, resource_reference.ManagedFolderResource)
            if not resource_reference.is_container_or_has_container_url(name_expansion_result.resource) or should_return_bucket or should_return_managed_folder:
                self._url_found_match_tracker[input_url] = True
                yield name_expansion_result
            if resource_reference.is_container_or_has_container_url(name_expansion_result.resource):
                if self._recursion_requested is RecursionSetting.YES:
                    for nested_name_expansion_result in self._get_nested_objects_iterator(name_expansion_result):
                        self._url_found_match_tracker[input_url] = True
                    ...
                    if self._recursion_requested is RecursionSetting.NO_WITH_WARNING:
                        log.warning('Omitting {} because it is a container, and recursion is not enabled.'.format(name_expansion_result.resource))
