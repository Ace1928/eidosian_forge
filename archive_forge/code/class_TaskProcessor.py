...
class TaskProcessor(ABC):
    ...
    def __init__(self, config: 'PretrainedConfig', preprocessor: Preprocessor, preprocessor_kwargs: Optional[Dict[str, Any]]=None):
        """
        ...
        if not isinstance(preprocessor, self.ACCEPTED_PREPROCESSOR_CLASSES):
            raise ValueError(f'Preprocessor is incorrect, provided an instance of {type(preprocessor)} but expected one of the following type: {', '.join((cls_.__name__ for cls_ in self.ACCEPTED_PREPROCESSOR_CLASSES))}.')
        self.config = config
        self.preprocessor = preprocessor
        self.defaults, self.preprocessor_kwargs = self.create_defaults_and_kwargs_from_preprocessor_kwargs(preprocessor_kwargs)

    def create_defaults_and_kwargs_from_preprocessor_kwargs(self, preprocessor_kwargs: Optional[Dict[str, Any]]) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        """
    ...
    def load_dataset(self, path: str, data_keys: Optional[Dict[str, str]]=None, ref_keys: Optional[List[str]]=None, only_keep_necessary_columns: bool=False, load_smallest_split: bool=False, num_samples: Optional[int]=None, shuffle: bool=False, **load_dataset_kwargs) -> Union[DatasetDict, Dataset]:
        ...
        if isinstance(dataset, DatasetDict) and load_smallest_split:
            ...
            if split is not None:
                raise ValueError(f'A split name was provided ({split}) but load_smallest_split is True, use either one or the other.')
            smallest_split = min(dataset.items(), key=lambda item: item[1].num_rows)[0]
            logger.info(f'Since no split was explicitely provided and load_smallest_split=True, using the smallest split of the dataset called "{smallest_split}".')
        ...
        if isinstance(dataset, Dataset) and num_samples is not None:
            if num_samples > dataset.num_rows:
                raise ValueError(f'There are only {dataset.num_rows} examples in the dataset, but it was requested to select {num_samples} examples.')
        ...
        if data_keys is None:
            ...
            if data_keys is None:
                raise ValueError(f'Data keys need to be specified manually since they could not be guessed from {', '.join(column_names)}')
            else:
                logger.warning(f'Guessed the following data keys: {data_keys}')
        elif not set(data_keys.keys()) <= self.ALLOWED_DATA_KEY_NAMES:
            raise ValueError(f'data_keys contains unallowed keys {set(data_keys.keys())}, allowed_keys: {self.ALLOWED_DATA_KEY_NAMES}.')
        if ref_keys is None:
