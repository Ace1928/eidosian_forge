...
class NetworksSwitchAlternateManagementInterface(object):
    ...
    def __init__(self, params, meraki):
        self.meraki = meraki
        self.new_object = dict(enabled=params.get('enabled'), vlanId=params.get('vlanId'), protocols=params.get('protocols'), switches=params.get('switches'), network_id=params.get('networkId'))
    ...
    def update_all_params(self):
        ...
        if self.new_object.get('switches') is not None or self.new_object.get('switches') is not None:
            new_object_params['switches'] = self.new_object.get('switches') or self.new_object.get('switches')
        if self.new_object.get('networkId') is not None or self.new_object.get('network_id') is not None:
            new_object_params['networkId'] = self.new_object.get('networkId') or self.new_object.get('network_id')
        return new_object_params
    ...
    def get_object_by_name(self, name):
        result = None
        try:
            items = self.meraki.exec_meraki(family='switch', function='getNetworkSwitchAlternateManagementInterface', params=self.get_all_params(name=name))
    ...
    def requires_update(self, current_obj):
        ...
        obj_params = [('enabled', 'enabled'), ('vlanId', 'vlanId'), ('protocols', 'protocols'), ('switches', 'switches'), ('networkId', 'networkId')]
    ...
    def update(self):
        ...
        name = self.new_object.get('name')
        result = None
        result = self.meraki.exec_meraki(family='switch', function='updateNetworkSwitchAlternateManagementInterface', params=self.update_all_params(), op_modifies=True)
        return result