...
class RMSprop(optimizer_v2.OptimizerV2):
  ...
  """
    ...
    def __init__(self, learning_rate=0.001, rho=0.9, momentum=0.0, epsilon=1e-07, centered=False, name='RMSprop', **kwargs):
    ...
    def _resource_apply_dense(self, grad, var, apply_state=None):
        ...
        if self._momentum:
            ...
            if self.centered:
                ...
                return gen_training_ops.ResourceApplyCenteredRMSProp(var=var.handle, mg=mg.handle, ms=rms.handle, mom=mom.handle, lr=coefficients['lr_t'], rho=coefficients['rho'], momentum=coefficients['momentum'], epsilon=coefficients['epsilon'], grad=grad, use_locking=self._use_locking)
            else:
                return gen_training_ops.ResourceApplyRMSProp(var=var.handle, ms=rms.handle, mom=mom.handle, lr=coefficients['lr_t'], rho=coefficients['rho'], momentum=coefficients['momentum'], epsilon=coefficients['epsilon'], grad=grad, use_locking=self._use_locking)
        else:
    ...
    def _resource_apply_sparse(self, grad, var, indices, apply_state=None):
        ...
        if self._momentum:
            ...
            if self.centered:
                mg = self.get_slot(var, 'mg')
                return gen_training_ops.ResourceSparseApplyCenteredRMSProp(var=var.handle, mg=mg.handle, ms=rms.handle, mom=mom.handle, lr=coefficients['lr_t'], rho=coefficients['rho'], momentum=coefficients['momentum'], epsilon=coefficients['epsilon'], grad=grad, indices=indices, use_locking=self._use_locking)
            else:
                return gen_training_ops.ResourceSparseApplyRMSProp(var=var.handle, ms=rms.handle, mom=mom.handle, lr=coefficients['lr_t'], rho=coefficients['rho'], momentum=coefficients['momentum'], epsilon=coefficients['epsilon'], grad=grad, indices=indices, use_locking=self._use_locking)
        else:
    ...
    def get_config(self):
        ...
        config.update({'learning_rate': self._serialize_hyperparameter('learning_rate'), 'decay': self._initial_decay, 'rho': self._serialize_hyperparameter('rho'), 'momentum': self._serialize_hyperparameter('momentum'), 'epsilon': self.epsilon, 'centered': self.centered})
        return config