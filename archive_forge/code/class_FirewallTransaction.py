...
class FirewallTransaction(object):
    ...
    def __init__(self, module, action_args=(), zone=None, desired_state=None, permanent=False, immediate=False, enabled_values=None, disabled_values=None):
        """
    ...
    def action_handler(self, action_func, action_func_args):
        ...
        try:
            ...
            if len(self.msgs) > 0:
                self.module.fail_json(msg='ERROR: Exception caught: %s %s' % (e, ', '.join(self.msgs)))
            else:
    ...
    def get_fw_zone_settings(self):
        if self.fw_offline:
            ...
            fw_settings = FirewallClientZoneSettings(list(self.fw.config.get_zone_config(fw_zone)))
        else:
    ...
    def run(self):
        ...
        if self.immediate and self.permanent:
            is_enabled_permanent = self.action_handler(self.get_enabled_permanent, self.action_args)
            is_enabled_immediate = self.action_handler(self.get_enabled_immediate, self.action_args)
            ...
            is_enabled = self.action_handler(self.get_enabled_permanent, self.action_args)
            ...
            is_enabled = self.action_handler(self.get_enabled_immediate, self.action_args)
    ...
    def sanity_check(module):
        ...
        if import_failure:
            module.fail_json(msg=missing_required_lib('firewall') + '. Version 0.2.11 or newer required (0.3.9 or newer for offline operations)')