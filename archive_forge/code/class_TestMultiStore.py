...
class TestMultiStore(base.MultiStoreBaseTest, test_store_capabilities.TestStoreCapabilitiesChecking):
    _CONF = cfg.ConfigOpts()

    def setUp(self):
        """Establish a clean test environment."""
        super(TestMultiStore, self).setUp()
        self.enabled_backends = {'file1': 'file', 'file2': 'file'}
        ...
        self.config(filesystem_store_datadir=self.test_dir, filesystem_store_chunk_size=10, group='file1')
    ...
    def _create_metadata_json_file(self, metadata):
        ...
        self.config(filesystem_store_metadata_file=jsonfilename, group='file1')
    ...
    def _store_image(self, in_metadata):
        ...
        return self.store.add(expected_image_id, image_file, expected_file_size)
    ...
    def test_get(self):
        ...
        loc, size, checksum, metadata = self.store.add(image_id, image_file, len(file_contents))
        self.assertEqual('file1', metadata['store'])
        uri = 'file:///%s/%s' % (self.test_dir, image_id)
        loc = location.get_location_from_uri_and_backend(uri, 'file1', conf=self.conf)
    ...
    def test_get_non_existing(self):
        ...
        loc = location.get_location_from_uri_and_backend('file:///%s/non-existing' % self.test_dir, 'file1', conf=self.conf)
    ...
    def test_add_check_metadata_list_with_valid_mountpoint_locations(self):
        in_metadata = [{'id': 'abcdefg', 'mountpoint': '/tmp'}, {'id': 'xyz1234', 'mountpoint': '/xyz'}]
        location, size, checksum, metadata = self._store_image(in_metadata)
    ...
    def test_add_check_metadata_bad_nosuch_file(self):
        ...
        expected_file_contents = b'*' * expected_file_size
        image_file = io.BytesIO(expected_file_contents)
        location, size, checksum, metadata = self.store.add(expected_image_id, image_file, expected_file_size)
        self.assertEqual({'store': 'file1'}, metadata)
    ...
    def test_add_already_existing(self):
        ...
        image_file = io.BytesIO(file_contents)
        location, size, checksum, metadata = self.store.add(image_id, image_file, file_size)
        self.assertEqual('file1', metadata['store'])
        image_file = io.BytesIO(b'nevergonnamakeit')
        self.assertRaises(exceptions.Duplicate, self.store.add, image_id, image_file, 0)
    ...
    def test_add_cleanup_on_read_failure(self):
        ...
        with mock.patch.object(image_file, 'read') as mock_read:
            ...
            self.assertRaises(AttributeError, self.store.add, image_id, image_file, 0)
            self.assertFalse(os.path.exists(path))
    ...
    def test_delete(self):
        ...
        file_contents = b'*' * file_size
        image_file = io.BytesIO(file_contents)
        loc, size, checksum, metadata = self.store.add(image_id, image_file, file_size)
        self.assertEqual('file1', metadata['store'])
        uri = 'file:///%s/%s' % (self.test_dir, image_id)
        loc = location.get_location_from_uri_and_backend(uri, 'file1', conf=self.conf)
    ...
    def test_delete_non_existing(self):
        ...
        loc = location.get_location_from_uri_and_backend('file:///tmp/glance-tests/non-existing', 'file1', conf=self.conf)
    ...
    def test_configure_add_with_multi_datadirs(self):
        ...
        self.conf.set_override('filesystem_store_datadir', override=None, group='file1')
        self.conf.set_override('filesystem_store_datadirs', [store_map[0] + ':100', store_map[1] + ':200'], group='file1')
    ...
    def test_configure_add_with_metadata_file_success(self):
        metadata = {'id': 'asdf1234', 'mountpoint': '/tmp'}
    ...
    def test_configure_add_same_dir_multiple_times(self):
        ...
        """
        store_map = [self.useFixture(fixtures.TempDir()).path, self.useFixture(fixtures.TempDir()).path]
        self.conf.clear_override('filesystem_store_datadir', group='file1')
        self.conf.set_override('filesystem_store_datadirs', [store_map[0] + ':100', store_map[1] + ':200', store_map[0] + ':300'], group='file1')
        self.assertRaises(exceptions.BadStoreConfiguration, self.store.configure_add)
    ...
    def test_configure_add_same_dir_multiple_times_same_priority(self):
        ...
        store_map = [self.useFixture(fixtures.TempDir()).path, self.useFixture(fixtures.TempDir()).path]
        self.conf.set_override('filesystem_store_datadir', override=None, group='file1')
        self.conf.set_override('filesystem_store_datadirs', [store_map[0] + ':100', store_map[1] + ':200', store_map[0] + ':100'], group='file1')
        try:
            self.store.configure()
        ...
        self.assertEqual(expected_file_size, size)
        self.assertEqual(expected_checksum, checksum)
        loc = location.get_location_from_uri_and_backend(expected_location, 'file1', conf=self.conf)
        new_image_file, new_image_size = self.store.get(loc)
        new_image_contents = b''
        new_image_file_size = 0
        for chunk in new_image_file:
    ...
    def test_add_with_multiple_dirs_storage_full(self):
        ...
        with mock.patch.object(self.store, '_get_capacity_info') as capacity:
            ...
            expected_file_contents = b'*' * expected_file_size
            image_file = io.BytesIO(expected_file_contents)
            self.assertRaises(exceptions.StorageFull, self.store.add, expected_image_id, image_file, expected_file_size)
    ...
    def test_configure_add_with_file_perm(self):
        ...
        self.conf.set_override('filesystem_store_datadir', store, group='file1')
        self.conf.set_override('filesystem_store_file_perm', 700, group='file1')
        self.store.configure_add()
    ...
    def test_configure_add_with_inaccessible_file_perm(self):
        ...
        store = self.useFixture(fixtures.TempDir()).path
        self.conf.set_override('filesystem_store_datadir', store, group='file1')
        self.conf.set_override('filesystem_store_file_perm', 7, group='file1')
        self.assertRaises(exceptions.BadStoreConfiguration, self.store.configure_add)
    ...
    def test_add_with_file_perm_for_group_other_users_access(self):
        ...
        store = self.useFixture(fixtures.TempDir()).path
        self.conf.set_override('filesystem_store_datadir', store, group='file1')
        self.conf.set_override('filesystem_store_file_perm', 744, group='file1')
        os.chmod(store, 448)
        ...
        self.assertEqual(expected_file_size, size)
        self.assertEqual(expected_checksum, checksum)
        self.assertEqual(457, stat.S_IMODE(os.stat(store)[stat.ST_MODE]))
        mode = os.stat(expected_location[len('file:/'):])[stat.ST_MODE]
        perm = int(str(getattr(self.conf, 'file1').filesystem_store_file_perm), 8)
    ...
    def test_add_with_file_perm_for_owner_users_access(self):
        ...
        store = self.useFixture(fixtures.TempDir()).path
        self.conf.set_override('filesystem_store_datadir', store, group='file1')
        self.conf.set_override('filesystem_store_file_perm', 600, group='file1')
        os.chmod(store, 448)
        self.assertEqual(448, stat.S_IMODE(os.stat(store)[stat.ST_MODE]))
        self.store.configure_add()
        filesystem.Store.WRITE_CHUNKSIZE = units.Ki
