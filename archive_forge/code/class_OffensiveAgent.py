...
class OffensiveAgent(ParlaiClassificationAgent, EncoderLM, SingleTurn):
    ...
    def __init__(self, model, device, maxlen):
        super(OffensiveAgent, self).__init__(device=device, maxlen=maxlen, model=None, suffix='', name=model)
        self.string_matcher = OffensiveStringMatcher()
        self.agent = self._create_safety_model('zoo:dialogue_safety/single_turn/model', device=device)
    ...
    def predict(self, text, method='both', **kwargs):
        assert method in ['both', 'string-match', 'bert'], "param method must be one of ['both', 'string-match', 'bert']"
        if method == 'string-match':
            return {'input': text, 'output': 'offensive' if text in self.string_matcher else 'non-offensive'}
        if method == 'bert':
            return {'input': text, 'output': self.contains_offensive_language(text)}
        if method == 'both':
            if text in self.string_matcher:
                return {'input': text, 'output': 'offensive'}
            return {'input': text, 'output': self.contains_offensive_language(text)}
