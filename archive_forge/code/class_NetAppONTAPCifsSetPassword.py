...
class NetAppONTAPCifsSetPassword:
    ...
    def __init__(self):
        ...
        self.argument_spec.update(dict(vserver=dict(required=True, type='str'), user_name=dict(required=True, type='str'), user_password=dict(required=True, type='str', no_log=True)))
    ...
    def get_user_sid(self):
        ...
        if error:
            self.module.fail_json(msg='Error fetching cifs/local-user %s: %s' % (self.parameters['user_name'], to_native(error)), exception=traceback.format_exc())
        if record:
            return record['sid']
