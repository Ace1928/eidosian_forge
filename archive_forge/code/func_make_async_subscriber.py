...
def make_async_subscriber(subscription: SubscriptionPath, transport: str, per_partition_flow_control_settings: FlowControlSettings, nack_handler: Optional[NackHandler]=None, reassignment_handler: Optional[ReassignmentHandler]=None, message_transformer: Optional[MessageTransformer]=None, fixed_partitions: Optional[Set[Partition]]=None, credentials: Optional[Credentials]=None, client_options: Optional[ClientOptions]=None, metadata: Optional[Mapping[str, str]]=None) -> AsyncSingleSubscriber:
    """
    ...
    if fixed_partitions:
        ...
        assigner_factory = lambda: _make_dynamic_assigner(subscription, transport, client_options, credentials, metadata)
    ...
    partition_subscriber_factory = _make_partition_subscriber_factory(subscription, transport, client_options, credentials, metadata, per_partition_flow_control_settings, nack_handler, message_transformer)
    return AssigningSingleSubscriber(assigner_factory, partition_subscriber_factory, reassignment_handler)