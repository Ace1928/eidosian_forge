...
class table__h_m_t_x(DefaultTable.DefaultTable):
    ...
    def decompile(self, data, ttFont):
        ...
        for i in range(numberOfMetrics):
            ...
            if advanceWidth > 32767:
                log.warning('Glyph %r has a huge advance %s (%d); is it intentional or an (invalid) negative value?', glyphName, self.advanceName, advanceWidth)
    ...
    def compile(self, ttFont):
        ...
        try:
            ...
            if 'out of range' in str(e) and hasNegativeAdvances:
                raise ttLib.TTLibError("'%s' table can't contain negative advance %ss" % (self.tableTag, self.advanceName))
            else:
                raise
    ...
    def toXML(self, writer, ttFont):
        ...
        for glyphName in names:
            ...
            writer.simpletag('mtx', [('name', glyphName), (self.advanceName, advance), (self.sideBearingName, sb)])
            writer.newline()
    ...
    def fromXML(self, name, attrs, content, ttFont):
        if not hasattr(self, 'metrics'):
            self.metrics = {}
        if name == 'mtx':
            self.metrics[attrs['name']] = (safeEval(attrs[self.advanceName]), safeEval(attrs[self.sideBearingName]))
