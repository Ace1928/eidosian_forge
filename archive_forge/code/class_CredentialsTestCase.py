...
class CredentialsTestCase(base.V3ClientTestCase):
    ...
    def test_create_credential_of_cert_type(self):
        ...
        credential_ref = {'user': user.id, 'type': 'cert', 'blob': uuid.uuid4().hex}
    ...
    def test_get_credential(self):
        ...
        for credential_type in ['cert', 'ec2', 'totp']:
            credential = fixtures.Credential(self.client, user=user.id, type=credential_type, project=project.id)
    ...
    def test_list_credentials(self):
        user = fixtures.User(self.client, self.test_domain.id)
        self.useFixture(user)
        cert_credential = fixtures.Credential(self.client, user=user.id, type='cert')
        self.useFixture(cert_credential)
        project = fixtures.Project(self.client, self.test_domain.id)
        self.useFixture(project)
        ec2_credential = fixtures.Credential(self.client, user=user.id, type='ec2', project=project.id)
        self.useFixture(ec2_credential)
        totp_credential = fixtures.Credential(self.client, user=user.id, type='totp')
    ...
    def test_update_credential(self):
        ...
        credential = fixtures.Credential(self.client, user=user.id, type='cert')
        self.useFixture(credential)
        new_type = 'ec2'
        new_blob = '{"access":"' + uuid.uuid4().hex + '","secret":"secretKey"}'
        credential_ret = self.client.credentials.update(credential.id, user=new_user.id, type=new_type, blob=new_blob, project=new_project.id)
        credential.ref.update({'user': new_user.id, 'type': new_type, 'blob': new_blob, 'project': new_project.id})
        self.check_credential(credential_ret, credential.ref)
    ...
    def test_delete_credential(self):
        ...
        for credential_type in ['cert', 'ec2', 'totp']:
            if credential_type == 'ec2':
                blob_value = '{"access":"' + uuid.uuid4().hex + '","secret":"secretKey"}'
            else:
                blob_value = uuid.uuid4().hex
            credential = self.client.credentials.create(user=user.id, type=credential_type, blob=blob_value, project=project.id)
            self.client.credentials.delete(credential.id)
            self.assertRaises(http.NotFound, self.client.credentials.get, credential.id)