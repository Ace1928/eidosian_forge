...
class ClcGroup(object):
    ...
    def process_request(self):
        ...
        if state == 'absent':
            changed, group, requests = self._ensure_group_is_absent(group_name=group_name, parent_name=parent_name)
            if requests:
                self._wait_for_requests_to_complete(requests)
        else:
            changed, group = self._ensure_group_is_present(group_name=group_name, parent_name=parent_name, group_description=group_description)
        try:
            group = group.data
    ...
    def _define_module_argument_spec():
        ...
        """
        argument_spec = dict(name=dict(required=True), description=dict(), parent=dict(), location=dict(), state=dict(default='present', choices=['present', 'absent']), wait=dict(type='bool', default=True))
    ...
    def _delete_group(self, group_name):
        ...
        try:
            ...
            self.module.fail_json(msg='Failed to delete group :{0}. {1}'.format(group_name, ex.response_text))
        return response
    ...
    def _create_group(self, group, parent, description):
        ...
        try:
            ...
            self.module.fail_json(msg='Failed to create group :{0}. {1}'.format(group, ex.response_text))
        return response
