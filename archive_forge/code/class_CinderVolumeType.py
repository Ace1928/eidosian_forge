...
class CinderVolumeType(resource.Resource):
    ...
    def handle_create(self):
        args = {'name': self.properties[self.NAME], 'is_public': self.properties[self.IS_PUBLIC], 'description': self.properties[self.DESCRIPTION]}
    ...
    def handle_update(self, json_snippet, tmpl_diff, prop_diff):
        ...
        if self.PROJECTS in prop_diff and (not is_public):
            old_access_list = self.client().volume_type_access.list(self.resource_id)
            old_projects = [ac.to_dict()['project_id'] for ac in old_access_list]
            new_projects = prop_diff.get(self.PROJECTS)
            for project_id in set(old_projects) - set(new_projects):
                self.client().volume_type_access.remove_project_access(self.resource_id, project_id)
    ...
    def validate(self):
        ...
        if self.properties[self.PROJECTS]:
            if self.properties[self.IS_PUBLIC]:
                msg = _('Can not specify property "%s" if the volume type is public.') % self.PROJECTS
    ...
    def parse_live_resource_data(self, resource_properties, resource_data):
        ...
        resource_reality.update({self.NAME: resource_data.get(self.NAME), self.DESCRIPTION: resource_data.get(self.DESCRIPTION)})
