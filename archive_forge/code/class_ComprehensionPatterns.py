...
class ComprehensionPatterns(Transformation):
    ...
    def visit_ListComp(self, node):
        ...
        def makeattr(*args):
            r = ast.Attribute(value=ast.Name(id='builtins', ctx=ast.Load(), annotation=None, type_comment=None), attr='map', ctx=ast.Load())
            r = ast.Call(r, list(args), [])
            r = ast.Call(ast.Attribute(ast.Name('builtins', ast.Load(), None, None), 'list', ast.Load()), [r], [])
            return r
            ...
            if not gen.ifs and isinstance(gen.iter, ast.Call):
                try:
                    ...
                    if path == range_path and len(gen.iter.args) == 1:
                        self.update = True
                        return ast.BinOp(ast.List([node.elt], ast.Load()), ast.Mult(), ast.Call(path_to_attr(('builtins', 'len')), [gen.iter], []))
                except TypeError:
                    pass
