...
class IntersectionOverUnion(Metric):
    ...
    def __init__(self, box_format: str='xyxy', iou_threshold: Optional[float]=None, class_metrics: bool=False, respect_labels: bool=True, **kwargs: Any) -> None:
        super().__init__(**kwargs)
        if not _TORCHVISION_GREATER_EQUAL_0_8:
            raise ModuleNotFoundError(f'Metric `{self._iou_type.upper()}` requires that `torchvision` version 0.8.0 or newer is installed. Please install with `pip install torchvision>=0.8` or `pip install torchmetrics[detection]`.')
    ...
    def update(self, preds: List[Dict[str, Tensor]], target: List[Dict[str, Tensor]]) -> None:
        ...
        for p, t in zip(preds, target):
            ...
            if self.respect_labels:
                label_eq = p['labels'].unsqueeze(1) == t['labels'].unsqueeze(0)
