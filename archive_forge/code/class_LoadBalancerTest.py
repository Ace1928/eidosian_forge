...
class LoadBalancerTest(common.HeatTestCase):
    ...
    def test_resource_mapping(self):
        ...
        self.assertEqual(loadbalancer.LoadBalancer, mapping['OS::Octavia::LoadBalancer'])
    ...
    def _create_stack(self, tmpl=inline_templates.LB_TEMPLATE):
        ...
        self.patchobject(OctaviaClientPlugin, 'get_flavor', return_value='f123')
        self.lb.client_plugin().client = mock.MagicMock(return_value=self.octavia_client)
        self.lb.translate_properties(self.lb.properties)
        self.lb.resource_id_set('1234')

    def test_create(self):
        self._create_stack()
        expected = {'loadbalancer': {'name': 'my_lb', 'description': 'my loadbalancer', 'vip_address': '10.0.0.4', 'vip_subnet_id': '123', 'provider': 'octavia', 'project_id': '1234', 'admin_state_up': True, 'flavor_id': 'f123', 'availability_zone': 'nova'}}
        self.lb.handle_create()
        self.octavia_client.load_balancer_create.assert_called_with(json=expected)
    ...
    def test_update(self):
        ...
        self.octavia_client.load_balancer_set.assert_called_once_with('1234', json={'loadbalancer': prop_diff})
    ...
    def test_delete(self):
        self._create_stack()
        self.octavia_client.load_balancer_show.side_effect = [{'provisioning_status': 'DELETE_PENDING'}, {'provisioning_status': 'DELETE_PENDING'}, {'provisioning_status': 'DELETED'}]
        self.octavia_client.load_balancer_delete.side_effect = [exceptions.Conflict(409), None]
        ...
        self.assertEqual(2, self.octavia_client.load_balancer_delete.call_count)

    def test_delete_error(self):
        self._create_stack()
        self.octavia_client.load_balancer_show.side_effect = [{'provisioning_status': 'DELETE_PENDING'}]
        self.octavia_client.load_balancer_delete.side_effect = [exceptions.Conflict(409), exceptions.NotFound(404)]
        self.lb.handle_delete()
        self.assertFalse(self.lb.check_delete_complete(None))
        self.assertTrue(self.lb.check_delete_complete(None))
        self.octavia_client.load_balancer_delete.assert_called_with('1234')
    ...
    def test_delete_failed(self):
        self._create_stack()
        self.octavia_client.load_balancer_delete.side_effect = exceptions.Unauthorized(403)
        self.lb.handle_delete()
        self.assertRaises(exceptions.Unauthorized, self.lb.check_delete_complete, None)