...
class BuildozerConfig(BaseConfig):
    ...
    def __init__(self, buildozer_spec_file: Path, pysidedeploy_config: Config):
        ...
        self.set_value('app', 'package.domain', f'org.{pysidedeploy_config.title}')
        ...
        platform_map = {'aarch64': 'arm64-v8a', 'armv7a': 'armeabi-v7a', 'i686': 'x86', 'x86_64': 'x86_64'}
        ...
        self.qt_libs_path: zipfile.Path = find_qtlibs_in_wheel(wheel_pyside=pysidedeploy_config.wheel_pyside)
        ...
        dependency_files = self.__get_dependency_files(modules=pysidedeploy_config.modules, arch=self.arch)
        dependent_plugins = []
        ...
        if not pysidedeploy_config.recipes_exist() and (not pysidedeploy_config.dry_run):
            ...
            create_recipe(version=version, component=f'PySide{MAJOR_VERSION}', wheel_path=pysidedeploy_config.wheel_pyside, generated_files_path=pysidedeploy_config.generated_files_path, qt_modules=pysidedeploy_config.modules, local_libs=pysidedeploy_config.local_libs, plugins=pysidedeploy_config.qt_plugins)
            create_recipe(version=version, component=f'shiboken{MAJOR_VERSION}', wheel_path=pysidedeploy_config.wheel_shiboken, generated_files_path=pysidedeploy_config.generated_files_path)
            pysidedeploy_config.recipe_dir = (pysidedeploy_config.generated_files_path / 'recipes').resolve()
        ...
        extra_args = f'--qt-libs={modules} --load-local-libs={local_libs} --init-classes={init_classes}'
