...
def _validate_llm_inference_task_type(llm_inference_task: str, pipeline: Pipeline) -> None:
    ...
    """
    supported_llm_inference_tasks = _SUPPORTED_LLM_INFERENCE_TASK_TYPES_BY_PIPELINE_TASK.get(pipeline.task, [])
    if llm_inference_task not in supported_llm_inference_tasks:
        raise MlflowException(f"The task provided is invalid. '{llm_inference_task}' is not a supported task for the {pipeline.task} pipeline. Must be one of {supported_llm_inference_tasks}", error_code=INVALID_PARAMETER_VALUE)