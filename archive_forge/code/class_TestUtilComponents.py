...
class TestUtilComponents(unittest.TestCase):
    ...
    def test_iter_component_kernel(self):
        ...
        model.con = pmo.constraint_dict(((i, pmo.constraint(expr=model.x[i - 1] + model.z == i)) for i in [1, 2, 3]))
        ...
        self.assertSameComponents(list(iter_component(model.param_t)), list(model.param_t))
