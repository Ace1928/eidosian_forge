...
def fused_batch_norm_v3(x: _atypes.TensorFuzzingAnnotation[TV_FusedBatchNormV3_T], scale: _atypes.TensorFuzzingAnnotation[TV_FusedBatchNormV3_U], offset: _atypes.TensorFuzzingAnnotation[TV_FusedBatchNormV3_U], mean: _atypes.TensorFuzzingAnnotation[TV_FusedBatchNormV3_U], variance: _atypes.TensorFuzzingAnnotation[TV_FusedBatchNormV3_U], epsilon: float=0.0001, exponential_avg_factor: float=1, data_format: str='NHWC', is_training: bool=True, name=None):
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'FusedBatchNormV3', name, x, scale, offset, mean, variance, 'epsilon', epsilon, 'exponential_avg_factor', exponential_avg_factor, 'data_format', data_format, 'is_training', is_training)
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('FusedBatchNormV3', x=x, scale=scale, offset=offset, mean=mean, variance=variance, epsilon=epsilon, exponential_avg_factor=exponential_avg_factor, data_format=data_format, is_training=is_training, name=name)
    _result = _outputs[:]
