  ...
  *{command}* downloads a named set from a Compute Engine router.
  """
    ...
    def Args(cls, parser):
        ...
        parser.add_argument('--set-name', required=True, help='Name of the named set to download.')
        parser.add_argument('--file-name', required=True, help='The name of the file to download the named set config to.')
    ...
    def Run(self, args):
        ...
        router_ref = DownloadNamedSet.ROUTER_ARG.ResolveAsResource(args, holder.resources, scope_lister=compute_flags.GetDefaultScopeLister(client))
