...
class SetVolumeType(command.Command):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument('--name', metavar='<name>', help=_('Set volume type name'))
        parser.add_argument('--description', metavar='<description>', help=_('Set volume type description'))
        ...
        parser.add_argument('--project', metavar='<project>', help=_('Set volume type access to project (name or ID) (admin only)'))
        ...
        parser.add_argument('--encryption-provider', metavar='<provider>', help=_('Set the encryption provider format for this volume type (e.g "luks" or "plain") (admin only) (This option is required when setting encryption type of a volume for the first time. Consider using other encryption options such as: "--encryption-cipher", "--encryption-key-size" and "--encryption-control-location")'))
    ...
    def take_action(self, parsed_args):
        ...
        if kwargs:
            try:
                ...
                LOG.error(_('Failed to update volume type name or description: %s'), e)
                result += 1
        properties = {}
        properties = {}
        ...
        if parsed_args.project:
            ...
            try:
                ...
                volume_client.volume_type_access.add_project_access(volume_type.id, project_info.id)
            except Exception as e:
                LOG.error(_('Failed to set volume type access to project: %s'), e)
                result += 1
