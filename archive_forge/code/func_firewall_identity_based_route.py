...
def firewall_identity_based_route(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('firewall', 'identity-based-route', filtered_data, vdom=vdom)
    ...
    if state == 'present' or state is True:
        return fos.set('firewall', 'identity-based-route', data=filtered_data, vdom=vdom)
    elif state == 'absent':
        return fos.delete('firewall', 'identity-based-route', mkey=filtered_data['name'], vdom=vdom)
    else:
