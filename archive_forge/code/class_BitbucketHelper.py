...
class BitbucketHelper:
    ...
    def fetch_access_token(self):
        if self.module.params['client_id'] and self.module.params['client_secret']:
            headers = {'Authorization': basic_auth_header(self.module.params['client_id'], self.module.params['client_secret'])}
            info, content = self.request(api_url='https://bitbucket.org/site/oauth2/access_token', method='POST', data='grant_type=client_credentials', headers=headers)
            if info['status'] == 200:
    ...
    def request(self, api_url, method, data=None, headers=None):
        ...
        if self.access_token:
            headers.update({'Authorization': 'Bearer {0}'.format(self.access_token)})
        elif self.module.params['user'] and self.module.params['password']:
            headers.update({'Authorization': basic_auth_header(self.module.params['user'], self.module.params['password'])})
        if isinstance(data, dict):
            data = self.module.jsonify(data)
            headers.update({'Content-type': 'application/json'})
        response, info = fetch_url(module=self.module, url=api_url, method=method, headers=headers, data=data, force=True)
        content = {}
