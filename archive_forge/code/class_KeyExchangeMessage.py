...
class KeyExchangeMessage:
    INITIATE_FLAG = 1
    RESPONSE_FLAG = 2
    SIMULTAENOUS_INITIATE_FLAG = 4

    def __init__(self, messageVersion=None, sequence=None, flags=None, baseKey=None, baseKeySignature=None, ratchetKey=None, identityKey=None, serialized=None):
        """
        ...
        if serialized:
            try:
                ...
                if not message.HasField('id') or not message.HasField('baseKey') or (not message.HasField('ratchetKey')) or (not message.HasField('identityKey')) or (not message.HasField('baseKeySignature')):
                    raise InvalidMessageException('Some required fields are missing!')
                self.sequence = message.id >> 5
                self.flags = message.id & 31
