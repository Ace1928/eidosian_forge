...
class NetworksApplianceSingleLan(object):
    ...
    def __init__(self, params, meraki):
        self.meraki = meraki
        self.new_object = dict(subnet=params.get('subnet'), applianceIp=params.get('applianceIp'), ipv6=params.get('ipv6'), mandatoryDhcp=params.get('mandatoryDhcp'), network_id=params.get('networkId'))
    ...
    def update_all_params(self):
        ...
        if self.new_object.get('applianceIp') is not None or self.new_object.get('appliance_ip') is not None:
            new_object_params['applianceIp'] = self.new_object.get('applianceIp') or self.new_object.get('appliance_ip')
        ...
        if self.new_object.get('mandatoryDhcp') is not None or self.new_object.get('mandatory_dhcp') is not None:
            new_object_params['mandatoryDhcp'] = self.new_object.get('mandatoryDhcp') or self.new_object.get('mandatory_dhcp')
        if self.new_object.get('networkId') is not None or self.new_object.get('network_id') is not None:
            new_object_params['networkId'] = self.new_object.get('networkId') or self.new_object.get('network_id')
        return new_object_params
    ...
    def get_object_by_name(self, name):
        result = None
        try:
            items = self.meraki.exec_meraki(family='appliance', function='getNetworkApplianceSingleLan', params=self.get_all_params(name=name))
    ...
    def requires_update(self, current_obj):
        ...
        obj_params = [('subnet', 'subnet'), ('applianceIp', 'applianceIp'), ('ipv6', 'ipv6'), ('mandatoryDhcp', 'mandatoryDhcp'), ('networkId', 'networkId')]
    ...
    def update(self):
        ...
        name = self.new_object.get('name')
        result = None
        result = self.meraki.exec_meraki(family='appliance', function='updateNetworkApplianceSingleLan', params=self.update_all_params(), op_modifies=True)
        return result