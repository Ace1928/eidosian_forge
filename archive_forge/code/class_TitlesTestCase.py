...
class TitlesTestCase(test.TestCase):
    specs_path = os.path.join(os.path.dirname(__file__), os.pardir, os.pardir, os.pardir, os.pardir, 'doc', 'specs')
    ...
    def test_template(self):
        ...
        for d in ['implemented', 'in-progress']:
            ...
            self.assertTrue(os.path.isdir(spec_dir), '%s is not a directory' % spec_dir)
            for filename in glob.glob(spec_dir + '/*'):
                if filename.endswith('README.rst'):
                    continue
                self.assertTrue(filename.endswith('.rst'), "spec's file must have .rst ext. Found: %s" % filename)
