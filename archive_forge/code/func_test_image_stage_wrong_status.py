...
def test_image_stage_wrong_status(self):
    ...
    exc = self.assertRaises(exceptions.SDKException, self.proxy.stage_image, image, data='data')
    self.assertIn('Image stage is only possible for images in the queued state.', str(exc))
