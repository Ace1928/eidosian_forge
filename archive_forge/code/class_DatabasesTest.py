...
class DatabasesTest(testtools.TestCase):
    ...
    def test_delete(self):
        ...
        self.assertEqual('/instances/27/databases/%s' % self.fakedb1[0], self._url)
        self.databases.delete(self.instance_with_id, self.fakedb1[0])
        self.assertEqual('/instances/%s/databases/%s' % (self.instance_with_id.id, self.fakedb1[0]), self._url)
    ...
    def test_list(self):
        ...
        page_mock.assert_called_with('/instances/instance1/databases', 'databases', None, None)
        limit = 'test-limit'
        marker = 'test-marker'
        self.databases.list('instance1', limit, marker)
        page_mock.assert_called_with('/instances/instance1/databases', 'databases', limit, marker)