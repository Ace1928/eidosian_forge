...
class DurableDNSTests(LibcloudTestCase):
    ...
    def test_list_zones(self):
        extra = {'ns': 'ns1.durabledns.com.', 'mbox': 'mail.myzone.com', 'serial': '1437473456', 'refresh': '13000', 'retry': 7200, 'expire': 1300, 'minimum': 13, 'xfer': '127.0.0.1', 'update_acl': '127.0.0.1'}
        zone = Zone(id='myzone.com.', domain='myzone.com.', type='master', ttl=1300, driver=self.driver, extra=extra)
    ...
    def test_create_record_with_extra_param(self):
        zone = self.driver.list_zones()[0]
        DurableDNSMockHttp.type = 'WITH_EXTRA_PARAMS'
        record = self.driver.create_record(name='record1', zone=zone, type=RecordType.A, data='1.2.3.4', extra={'ttl': 4000})
        self.assertEqual(record.id, '353367855')
        self.assertEqual(record.name, 'record1')
        self.assertEqual(record.zone, zone)
        self.assertEqual(record.type, RecordType.A)
    ...
    def test_update_zone(self):
        ...
        updated_zone = self.driver.update_zone(zone, zone.domain, type=zone.type, ttl=4000, extra=new_extra)
    ...
    def test_update_record(self):
        ...
        updated_record = self.driver.update_record(record, record.name, record.type, record.data, extra=new_extra)
