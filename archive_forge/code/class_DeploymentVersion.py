...
class DeploymentVersion:
    ...
    def __init__(self, code_version: Optional[str], deployment_config: DeploymentConfig, ray_actor_options: Optional[Dict], placement_group_bundles: Optional[List[Dict[str, float]]]=None, placement_group_strategy: Optional[str]=None, max_replicas_per_node: Optional[int]=None):
    ...
    def to_proto(self) -> bytes:
        return DeploymentVersionProto(code_version=self.code_version, deployment_config=self.deployment_config.to_proto(), ray_actor_options=json.dumps(self.ray_actor_options), placement_group_bundles=json.dumps(self.placement_group_bundles) if self.placement_group_bundles is not None else '', placement_group_strategy=self.placement_group_strategy if self.placement_group_strategy is not None else '', max_replicas_per_node=self.max_replicas_per_node if self.max_replicas_per_node is not None else 0)

    @classmethod
    def from_proto(cls, proto: DeploymentVersionProto):
        return DeploymentVersion(proto.code_version, DeploymentConfig.from_proto(proto.deployment_config), json.loads(proto.ray_actor_options), placement_group_bundles=json.loads(proto.placement_group_bundles) if proto.placement_group_bundles else None, placement_group_version=proto.placement_group_version if proto.placement_group_version else None, max_replicas_per_node=proto.max_replicas_per_node if proto.max_replicas_per_node else None)
