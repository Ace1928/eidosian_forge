    ...
    **Example**
    ...
    def __init__(self, coeffs, observables):
        if len(coeffs) != len(observables):
            raise ValueError(f'Could not create valid Hamiltonian; number of coefficients and operators does not match.Got len(coeffs) = {len(coeffs)} and len(observables) = {len(observables)}.')
        self.coeffs_fixed = []
        ...
        self.wires = Wires.all_wires([op.wires for op in self.ops_fixed] + [op.wires for op in self.ops_parametrized])

    def __call__(self, params, t):
        if len(params) != len(self.coeffs_parametrized):
            raise ValueError(f'The length of the params argument and the number of scalar-valued functions must be the same. Received len(params) = {len(params)} parameters but expected {len(self.coeffs_parametrized)} parameters.')
    ...
    def map_wires(self, wire_map):
        ...
        new_ph.wires = Wires.all_wires([op.wires for op in new_ph.ops_fixed] + [op.wires for op in new_ph.ops_parametrized])
        return new_ph
    ...
    def __mul__(self, other):
        ...
        if isinstance(other, TensorLike) and qml.math.ndim(other) == 0:
            ...
            coeffs_parametrized = [lambda p, t, new_c=c: other * new_c(p, t) for c in coeffs_parametrized]
