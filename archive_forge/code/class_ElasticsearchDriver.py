...
class ElasticsearchDriver(base.Driver):
    ...
    def __init__(self, connection_str, index_name='osprofiler-notifications', project=None, service=None, host=None, conf=cfg.CONF, **kwargs):
        """Elasticsearch driver for OSProfiler."""
        super(ElasticsearchDriver, self).__init__(connection_str, project=project, service=service, host=host, conf=conf, **kwargs)
        try:
            from elasticsearch import Elasticsearch
        except ImportError:
            raise exc.CommandError('To use OSProfiler with ElasticSearch driver, please install `elasticsearch` library. To install with pip:\n `pip install elasticsearch`.')
        client_url = parser.urlunparse(parser.urlparse(self.connection_str)._replace(scheme='http'))
        self.conf = conf
    ...
    def notify(self, info):
        ...
        self.client.index(index=self.index_name, doc_type=self.conf.profiler.es_doc_type, body=info)
        if self.filter_error_trace and info.get('info', {}).get('etype') is not None:
    ...
    def list_traces(self, fields=None):
        ...
        response = self.client.search(index=self.index_name, doc_type=self.conf.profiler.es_doc_type, size=self.conf.profiler.es_scroll_size, scroll=self.conf.profiler.es_scroll_time, body={'_source': fields, 'query': query, 'sort': [{'timestamp': 'asc'}]})
        return self._hits(response)

    def list_error_traces(self):
        """Returns all traces that have error/exception."""
        response = self.client.search(index=self.index_name_error, doc_type=self.conf.profiler.es_doc_type, size=self.conf.profiler.es_scroll_size, scroll=self.conf.profiler.es_scroll_time, body={'_source': self.default_trace_fields, 'query': {'match_all': {}}, 'sort': [{'timestamp': 'asc'}]})
        return self._hits(response)
    ...
    def get_report(self, base_id):
        ...
        """
        response = self.client.search(index=self.index_name, doc_type=self.conf.profiler.es_doc_type, size=self.conf.profiler.es_scroll_size, scroll=self.conf.profiler.es_scroll_time, body={'query': {'match': {'base_id': base_id}}})
        for n in self._hits(response):
            ...
            self._append_results(trace_id, parent_id, name, project, service, host, timestamp, n)
