...
class LowStretchTree(Graph):
    ...
    def __init__(self, k=6, **kwargs):
        ...
        for p in range(1, k):
            ii = np.concatenate((ii, ii + 4 ** p, ii + 2 * 4 ** p, ii + 3 * 4 ** p, [4 ** p - 1], [4 ** p - 1], [4 ** p + (4 ** (p + 1) + 2) / 3.0 - 1], [5 / 3.0 * 4 ** p + 1 / 3.0 - 1], [4 ** p + (4 ** (p + 1) + 2) / 3.0 - 1], [3 * 4 ** p]))
            jj = np.concatenate((jj, jj + 4 ** p, jj + 2 * 4 ** p, jj + 3 * 4 ** p, [5.0 / 3 * 4 ** p + 1 / 3.0 - 1], [4 ** p + (4 ** (p + 1) + 2) / 3.0 - 1], [3 * 4 ** p], [4 ** p - 1], [4 ** p - 1], [4 ** p + (4 ** (p + 1) + 2) / 3.0 - 1]))
        ...
        coords = np.concatenate((XCoords[:, np.newaxis], YCoords[:, np.newaxis]), axis=1)
        self.root = 4 ** (k - 1)
        plotting = {'edges_width': 1.25, 'vertex_size': 75, 'limits': np.array([0, 2 ** k + 1, 0, 2 ** k + 1])}
        super(LowStretchTree, self).__init__(W=W, coords=coords, plotting=plotting, gtype='low stretch tree', **kwargs)