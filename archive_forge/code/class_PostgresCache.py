...
class PostgresCache(BaseCache):
    ...
    def __init__(self, *, timeout=POSTGRES_TIMEOUT, **kwargs):
        ...
        if not asyncpglogger.hasHandlers():
    ...
    async def setup(self):
        queries = ['CREATE TABLE IF NOT EXISTS proactive_fetch_ts (id serial primary key, last_fetch_ts integer)', 'CREATE TABLE IF NOT EXISTS sts_policy_cache (id serial primary key, domain text, ts integer, pol_id text, pol_body jsonb)', 'CREATE UNIQUE INDEX IF NOT EXISTS sts_policy_domain ON sts_policy_cache (domain)', 'CREATE INDEX IF NOT EXISTS sts_policy_domain_ts ON sts_policy_cache (domain, ts)']
    ...
    async def get_proactive_fetch_ts(self):
        async with self._pool.acquire(timeout=self._timeout) as conn, conn.transaction():
            cur = await conn.cursor('SELECT last_fetch_ts FROM proactive_fetch_ts where id = $1', self._last_proactive_fetch_ts_id)
    ...
    async def get(self, key):
        async with self._pool.acquire(timeout=self._timeout) as conn, conn.transaction():
            cur = await conn.cursor('SELECT ts, pol_id, pol_body FROM sts_policy_cache WHERE domain=$1', key)
            res = await cur.fetchrow()
        if res is not None:
    ...
    async def scan(self, token, amount_hint):
        if token is None:
            token = 1
        async with self._pool.acquire(timeout=self._timeout) as conn, conn.transaction():
            res = await conn.fetch('SELECT id, ts, pol_id, pol_body, domain FROM sts_policy_cache WHERE id >= $1 ORDER BY id ASC LIMIT $2', token, amount_hint)
        if res:
            result = []
            new_token = token
            for row in res:
