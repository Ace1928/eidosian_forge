...
class VibrationsData:
    ...
    def __init__(self, atoms: Atoms, hessian: Union[RealSequence4D, np.ndarray], indices: Union[Sequence[int], np.ndarray]=None) -> None:
        if indices is None:
            self._indices = np.arange(len(atoms), dtype=int)
        else:
            self._indices = np.array(indices, dtype=int)
        n_atoms = self._check_dimensions(atoms, np.asarray(hessian), indices=self._indices)
        self._atoms = atoms.copy()
        self._hessian2d = np.asarray(hessian).reshape(3 * n_atoms, 3 * n_atoms).copy()
    _setter_error = 'VibrationsData properties cannot be modified: construct a new VibrationsData with consistent atoms, Hessian and (optionally) indices/mask.'
    ...
    def todict(self) -> Dict[str, Any]:
        ...
        return {'atoms': self.get_atoms(), 'hessian': self.get_hessian(), 'indices': indices}
    ...
    def _energies_and_modes(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if not np.all(masses):
            raise ValueError('Zero mass encountered in one or more of the vibrated atoms. Use Atoms.set_masses() to set all masses to non-zero values.')
        mass_weights = np.repeat(masses ** (-0.5), 3)
        omega2, vectors = np.linalg.eigh(mass_weights * self.get_hessian_2d() * mass_weights[:, np.newaxis])
    ...
    def tabulate(self, im_tol: float=1e-08) -> str:
    ...
    def write_jmol(self, filename: str='vib.xyz', ir_intensities: Union[Sequence[float], np.ndarray]=None) -> None:
        ...
        """
        all_images = list(self._get_jmol_images(atoms=self.get_atoms(), energies=self.get_energies(), modes=self.get_modes(all_atoms=True), ir_intensities=ir_intensities))
