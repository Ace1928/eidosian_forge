...
def _valid_data(self, proposal):
    ...
    if np.ndim(array) < 2 or np.ndim(array) > 3:
        raise TraitError('Data needs to have two or three dimensions. Given shape was: %r' % (np.shape(array),))
    old = self._trait_values.get(proposal['trait'].name, None)
    if old is not None and np.ndim(old) > 0 and (np.shape(old) != np.shape(array)):
        raise TraitError('Cannot change shape of previously initialized DataTexture. %r vs %r' % (np.shape(old), np.shape(array)))
    return value