...
def test_update_convergence_checks_resource_class(self):
    ...
    new_temp.store(ctx)
    new_stack = parser.Stack(ctx, 'test_stack', new_temp, stack_id=self.stack.id, convergence=True)
    tr = scheduler.TaskRunner(res.update_convergence, new_temp.id, set(), 'engine-007', -1, new_stack, self.dummy_event)
