...
class MultiVolume(io.RawIOBase, contextlib.AbstractContextManager):
    ...
    def __init__(self, basename: Union[pathlib.Path, str], mode: Optional[str]='r', *, volume: Optional[int]=None, ext_digits: Optional[int]=4, hex: Optional[bool]=False, ext_start: Optional[int]=1):
        self._mode = mode
        self._closed = False
        self._files = []
        self._fileinfo = []
        ...
        if mode in ['rb', 'r', 'rt']:
            ...
            if volume is None:
                self._volume_size = 10 * 1024 * 1024
            else:
    ...
    def write(self, b: Union[bytes, bytearray, memoryview, Container[Any], mmap]) -> None:
        ...
        if pos + len(b) > self._volume_size:
            ...
            file.seek(0)
            self.write(b[self._volume_size - pos:])
        else:
            file.write(b)
