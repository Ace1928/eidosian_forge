...
class ChatFeed(ListPanel):
    ...
    def __init__(self, *objects, **params):
        ...
        linked_params = dict(design=self.param.design, sizing_mode=self.param.sizing_mode, width=self.param.width, max_width=self.param.max_width, min_width=self.param.min_width, visible=self.param.visible)
        self._chat_log = Feed(*self.objects, load_buffer=self.load_buffer, auto_scroll_limit=self.auto_scroll_limit, scroll_button_threshold=self.scroll_button_threshold, view_latest=self.view_latest, css_classes=['chat-feed-log'], stylesheets=self._stylesheets, **linked_params)
        self._chat_log.height = None
        card_params = linked_params.copy()
        card_stylesheets = self._stylesheets + self.param.stylesheets.rx() + self.param.card_params.rx().get('stylesheets', [])
        card_params.update(margin=self.param.margin, align=self.param.align, header=self.param.header, height=self.param.height, hide_header=self.param.header.rx().rx.in_((None, '')), collapsible=False, css_classes=['chat-feed'] + self.param.css_classes.rx(), header_css_classes=['chat-feed-header'], max_height=self.param.max_height, min_height=self.param.min_height, title_css_classes=['chat-feed-title'], styles={'padding': '0px'}, stylesheets=card_stylesheets)
        ...
        self._card = Card(self._chat_log, VSpacer(), **card_params)
    ...
    def _update_model(self, events: Dict[str, param.parameterized.Event], msg: Dict[str, Any], root: Model, model: Model, doc: Document, comm: Comm | None) -> None:
        return
    ...
    def _build_message(self, value: dict, user: str | None=None, avatar: str | bytes | BytesIO | None=None) -> ChatMessage | None:
        """
        ...
        if 'value' in value and 'object' in value:
            ...
            raise ValueError(f"If 'value' is a dict, it must contain an 'object' key, e.g. {{'object': 'Hello World'}}; got {value!r}")
    ...
    async def _schedule_placeholder(self, task: asyncio.Task, num_entries: int) -> None:
        """
    ...
    async def _prepare_response(self, *_) -> None:
        ...
        try:
            ...
            await asyncio.gather(self._schedule_placeholder(future, num_entries), future)
        except StopCallback:
            ...
            if self.callback_exception == 'summary':
                self.send(f"Encountered `{e!r}`. Set `callback_exception='verbose'` to see the full traceback.", **send_kwargs)
    ...
    def send(self, value: ChatMessage | dict | Any, user: str | None=None, avatar: str | bytes | BytesIO | None=None, respond: bool=True) -> ChatMessage | None:
        """
    ...
    def stream(self, value: str | dict | ChatMessage, user: str | None=None, avatar: str | bytes | BytesIO | None=None, message: ChatMessage | None=None, replace: bool=False) -> ChatMessage | None:
        """
    ...
    def _serialize_for_transformers(self, messages: List[ChatMessage], role_names: Dict[str, str | List[str]] | None=None, default_role: str | None='assistant', custom_serializer: Callable=None) -> List[Dict[str, Any]]:
        ...
        if role_names is None:
            role_names = {'user': ['user'], 'assistant': [self.callback_user]}
        names_role = {}
        ...
        for message in messages:
            ...
            if lowercase_name not in names_role and (not default_role):
                raise ValueError(f'User {message.user!r} not found in role_names; got {role_names!r}.')
            role = names_role.get(lowercase_name, default_role)
            if custom_serializer:
                content = custom_serializer(message.object)
                if not isinstance(content, str):
                    raise ValueError(f'The provided custom_serializer must return a string; it returned a {type(content)} type')
            else:
                content = str(message)
            serialized_messages.append({'role': role, 'content': content})
        return serialized_messages

    def serialize(self, exclude_users: List[str] | None=None, filter_by: Callable | None=None, format: Literal['transformers']='transformers', custom_serializer: Callable | None=None, **serialize_kwargs):
        """
        ...
        **serialize_kwargs
        ...
        messages = [message for message in self._chat_log.objects if message.user.lower() not in exclude_users]
        if filter_by is not None:
            messages = filter_by(messages)
        if format == 'transformers':
            return self._serialize_for_transformers(messages, custom_serializer=custom_serializer, **serialize_kwargs)
    ...
    def select(self, selector=None):
        ...
        """
        selected = []
        if selector is None or (isinstance(selector, type) and isinstance(self, selector)) or (callable(selector) and (not isinstance(selector, type)) and selector(self)):
