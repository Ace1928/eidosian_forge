...
class NTriangulationForPtolemy(NTriangulation):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not self.isOriented():
            raise Exception('Only oriented triangulations are supported.Use orient method.')
    ...
    def ptolemy_variety(self, N, obstruction_class=None, simplify=True, eliminate_fixed_ptolemys=False):
        """
        ...
        * Stavros Garoufalidis, Matthias Goerner, Christian K. Zickert:
        ...
        """
        return manifoldMethods.get_ptolemy_variety(self, N, obstruction_class, simplify=simplify, eliminate_fixed_ptolemys=eliminate_fixed_ptolemys)
    ...
    def _to_string(self):
        ...
        filename = f.name
        f.close()
        writeXMLFile(filename, self, False)
        f = open(filename, 'rb')
        s = f.read()
        f.close()
        os.unlink(filename)
