...
class SelectedMajoranaFermionGate(unary_iteration_gate.UnaryIterationGate):
    ...
    """
    selection_regs: Tuple[infra.SelectionRegister, ...] = attr.field(converter=lambda v: (v,) if isinstance(v, infra.SelectionRegister) else tuple(v))
    control_regs: Tuple[infra.Register, ...] = attr.field(converter=lambda v: (v,) if isinstance(v, infra.Register) else tuple(v))
    ...
    @classmethod
    def make_on(cls, *, target_gate=cirq.Y, **quregs: Union[Sequence[cirq.Qid], NDArray[cirq.Qid]]) -> cirq.Operation:
        """Helper constructor to automatically deduce selection_regs attribute."""
        return SelectedMajoranaFermionGate(selection_regs=infra.SelectionRegister('selection', len(quregs['selection']), len(quregs['target'])), target_gate=target_gate).on_registers(**quregs)
    ...
    def decompose_from_registers(self, context: cirq.DecompositionContext, **quregs: NDArray[cirq.Qid]) -> cirq.OP_TREE:
        ...
        control = quregs[self.control_regs[0].name] if infra.total_bits(self.control_registers) else []
        yield cirq.X(*quregs['accumulator']).controlled_by(*control)
        yield super(SelectedMajoranaFermionGate, self).decompose_from_registers(context=context, **quregs)
    ...
    def nth_operation(self, context: cirq.DecompositionContext, control: cirq.Qid, target: Sequence[cirq.Qid], accumulator: Sequence[cirq.Qid], **selection_indices: int) -> cirq.OP_TREE:
