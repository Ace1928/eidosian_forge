...
class CompileUnit(object):
    ...
    def get_abbrev_table(self):
        ...
        if self._abbrev_table is None:
            self._abbrev_table = self.dwarfinfo.get_abbrev_table(self['debug_abbrev_offset'])
    ...
    def get_DIE_from_refaddr(self, refaddr):
        ...
        """
        dwarf_assert(self.cu_die_offset <= refaddr < self.cu_offset + self.size, 'refaddr %s not in DIE range of CU %s' % (refaddr, self.cu_offset))
