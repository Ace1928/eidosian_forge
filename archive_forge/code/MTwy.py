...
class A_STAR(Algorithm):
    ...
    def run_algorithm(self, snake: "Snake") -> Optional["Node"]:
        ...
        for future in as_completed(futures):
            neighbor_node = future.result()
            if neighbor_node and neighbor_node not in self.explored_set:
                heapq.heappush(self.frontier, (neighbor_node.f, neighbor_node))
                logging.debug(
                    f"Processed neighbor {neighbor_node} has been added to the frontier."
                )
        ...
        while self.frontier:
            ...
            # Process each neighbor asynchronously
            futures = [
