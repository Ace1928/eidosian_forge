...
class ItalianLemmatizer(Lemmatizer):
    ...
    def get_lookups_config(cls, mode: str) -> Tuple[List[str], List[str]]:
        if mode == 'pos_lookup':
            required = ['lemma_lookup_num', 'lemma_lookup_det', 'lemma_lookup_adp', 'lemma_lookup_adj', 'lemma_lookup_noun', 'lemma_lookup_pron', 'lemma_lookup_verb', 'lemma_lookup_aux', 'lemma_lookup_adv', 'lemma_lookup_other', 'lemma_lookup']
            return (required, [])
        else:
    ...
    def lemmatize_det(self, string: str, morphology: dict, lookup_table: Dict[str, str]) -> List[str]:
    ...
    def lemmatize_pron(self, string: str, morphology: dict, lookup_table: Dict[str, str]) -> List[str]:
    ...
    def lemmatize_adp(self, string: str, morphology: dict, lookup_table: Dict[str, str]) -> List[str]:
        if string == "d'":
            return ['di']
        return [lookup_table.get(string, string)]
