...
@experimental
def make_genai_metric(name: str, definition: str, grading_prompt: str, examples: Optional[List[EvaluationExample]]=None, version: Optional[str]=_get_latest_metric_version(), model: Optional[str]=_get_default_model(), grading_context_columns: Optional[Union[str, List[str]]]=[], include_input: bool=True, parameters: Optional[Dict[str, Any]]=None, aggregations: Optional[List[str]]=['mean', 'variance', 'p90'], greater_is_better: bool=True, max_workers: int=10) -> EvaluationMetric:
    """
    ...
    def process_example(example):
        ...
        if set(grading_context.keys()) != set(grading_context_columns):
            raise MlflowException.invalid_parameter_value(f'Example grading context does not contain required columns.\n Example grading context columns: {list(grading_context.keys())}\n Required grading context columns: {grading_context_columns}\n')
        if not include_input:
            return EvaluationExample(output=example.output, score=example.score, justification=example.justification, grading_context=example.grading_context)
        return example
    ...
    try:
        ...
        raise MlflowException(f'Failed to find evaluation model for version {version}. Please check the correctness of the version', error_code=INVALID_PARAMETER_VALUE) from None
    except Exception as e:
        raise MlflowException(f'Failed to construct evaluation model {version}. Error: {e!r}', error_code=INTERNAL_ERROR) from None
    evaluation_context = evaluation_model_class_module(name, definition, grading_prompt, examples, model, *((parameters,) if parameters is not None else ())).to_dict()
    ...
    signature_parameters = [Parameter('predictions', Parameter.POSITIONAL_OR_KEYWORD, annotation='pd.Series'), Parameter('metrics', Parameter.POSITIONAL_OR_KEYWORD, annotation=Dict[str, MetricValue]), Parameter('inputs', Parameter.POSITIONAL_OR_KEYWORD, annotation='pd.Series')]
    ...
    return make_metric(eval_fn=eval_fn, greater_is_better=greater_is_better, name=name, version=version, metric_details=evaluation_context['eval_prompt'].__str__())