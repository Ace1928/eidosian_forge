...
def _FusedMulAddSubGrad(op, grad, sgn):
    ...
    return [gx1 if not must_reduce_x1 else math_ops.reduce_sum(gx1, rx1), gy1 if not must_reduce_y1 else math_ops.reduce_sum(gy1, ry1), gx2 if not must_reduce_x2 else math_ops.reduce_sum(gx2, rx2)]