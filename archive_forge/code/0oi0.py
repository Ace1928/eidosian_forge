...
def statistics(data: List[Dict[str, Any]]) -> Dict[str, int]:
    ...
    """
    num_classes: int = len(data)
    num_methods: int = sum(len(cls["methods"]) for cls in data)
    logging.info(
        f"Statistics: {num_classes} classes and {num_methods} methods extracted."
    )
    return {"classes": num_classes, "methods": num_methods}


class Application(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Python Documentation Generator")
    ...
    def create_widgets(self):
        ...
        self.label = Label(
            frame,
            text="Select Python files and generate documentation.",
            wraplength=480,
        )
        self.label.pack(pady=10, fill="x", expand=True)

        # Buttons
        browse_button = Button(frame, text="Browse Files", command=self.browse_files)
        browse_button.pack(side="left", expand=True)

        generate_button = Button(
            frame, text="Generate Documentation", command=self.generate_documentation
        )
        generate_button.pack(side="right", expand=True)
