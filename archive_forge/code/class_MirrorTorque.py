...
class MirrorTorque(FixConstraint):
    ...
    def adjust_forces(self, atoms, forces):
        angle = atoms.get_dihedral(self.indices[0], self.indices[1], self.indices[2], self.indices[3])
        angle *= np.pi / 180.0
        ...
        d1 = np.cross(np.cross(p[1] - p0, p[0] - p[1]), p[1] - p0) / (p[1] - p0).dot(p[1] - p0)
        d2 = np.cross(np.cross(p[2] - p0, p[3] - p[2]), p[2] - p0) / (p[2] - p0).dot(p[2] - p0)
        omegap1 = (np.cross(d1, fff[0]) / d1.dot(d1)).dot(p[1] - p0) / np.linalg.norm(p[1] - p0)
        omegap2 = (np.cross(d2, fff[3]) / d2.dot(d2)).dot(p[2] - p0) / np.linalg.norm(p[2] - p0)
        ...
        if (forces_copy ** 2).sum(axis=1).max() < self.fmax ** 2:
            ...
            factor = 0.0
        df1 = -(1 + factor) / 2.0 * omegap * np.cross(p[1] - p0, d1) / np.linalg.norm(p[1] - p0)
        df2 = -(1 + factor) / 2.0 * omegap * np.cross(p[2] - p0, d2) / np.linalg.norm(p[2] - p0)
    ...
    def __repr__(self):
        return 'MirrorTorque(%d, %d, %d, %d, %f, %f, %f)' % (self.indices[0], self.indices[1], self.indices[2], self.indices[3], self.max_angle, self.min_angle, self.fmax)

    def todict(self):
        return {'name': 'MirrorTorque', 'kwargs': {'a1': self.indices[0], 'a2': self.indices[1], 'a3': self.indices[2], 'a4': self.indices[3], 'max_angle': self.max_angle, 'min_angle': self.min_angle, 'fmax': self.fmax}}