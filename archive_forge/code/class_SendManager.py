...
class SendManager:
    ...
    def setup(cls, root_dir: str, resume: Union[None, bool, str]) -> 'SendManager':
        ...
        settings = wandb.Settings(files_dir=files_dir, root_dir=root_dir, resume=resume, _sync=True, disable_job_creation=False, _async_upload_concurrency_limit=None, _file_stream_timeout_seconds=0)
        record_q: Queue[Record] = queue.Queue()
        result_q: Queue[Result] = queue.Queue()
        publish_interface = InterfaceQueue(record_q=record_q)
        context_keeper = context.ContextKeeper()
        return SendManager(settings=SettingsStatic(settings.to_proto()), record_q=record_q, result_q=result_q, interface=publish_interface, context_keeper=context_keeper)
    ...
    def __exit__(self, exc_type: Optional[Type[BaseException]], exc_value: Optional[BaseException], exc_traceback: Optional[traceback.TracebackException]) -> Literal[False]:
        while self:
            data = next(self)
            self.send(data)
        self.finish()
    ...
    def send_request_job_info(self, record: 'Record') -> None:
        ...
        result = proto_util._result_from_record(record)
        result.response.job_info_response.sequenceId = self._job_builder._job_seq_id or ''
        result.response.job_info_response.version = self._job_builder._job_version_alias or ''
        self._respond_result(result)
    ...
    def _config_backend_dict(self) -> sender_config.BackendConfigDict:
        ...
        return config.to_backend_dict(telemetry_record=self._telemetry_obj, framework=self._telemetry_get_framework(), start_time_millis=self._start_time, metric_pbdicts=self._config_metric_pbdict_list)
    ...
    def send_run(self, record: 'Record', file_dir: Optional[str]=None) -> None:
        ...
        self._start_time = run.start_time.ToMicroseconds() // 1000000.0
        if run.telemetry:
    ...
    def send_artifact(self, record: 'Record') -> None:
        ...
        try:
            ...
            logger.error('send_artifact: failed for artifact "{}/{}": {}'.format(artifact.type, artifact.name, e))
