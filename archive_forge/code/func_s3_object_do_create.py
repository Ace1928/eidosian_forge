...
def s3_object_do_create(module, connection, connection_v4, s3_vars):
    ...
    if key_check(module, connection, s3_vars['bucket'], s3_vars['object']):
        module.exit_json(msg=f'Bucket {s3_vars['bucket']} and key {s3_vars['object']} already exists.', changed=False)
    if not s3_vars['acl_disabled']:
        s3_vars['permission'] = s3_vars['object_acl']
    create_dirkey(module, connection, s3_vars['bucket'], s3_vars['object'], s3_vars['encrypt'], s3_vars['expiry'])