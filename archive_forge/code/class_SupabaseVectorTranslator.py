...
class SupabaseVectorTranslator(Visitor):
    ...
    allowed_comparators = [Comparator.EQ, Comparator.NE, Comparator.GT, Comparator.GTE, Comparator.LT, Comparator.LTE, Comparator.LIKE]
    ...
    def visit_comparison(self, comparison: Comparison) -> str:
        if isinstance(comparison.value, list):
            return self.visit_operation(Operation(operator=Operator.AND, arguments=[Comparison(comparator=comparison.comparator, attribute=comparison.attribute, value=value) for value in comparison.value]))
        return '.'.join([f'{self.metadata_column}{self._get_json_operator(comparison.value)}{comparison.attribute}', f'{self._map_comparator(comparison.comparator)}', f'{comparison.value}'])
