...
class QPUCompiler(AbstractCompiler):
    ...
    def __init__(self, *, quantum_processor_id: str, quantum_processor: AbstractQuantumProcessor, timeout: float=10.0, client_configuration: Optional[QCSClientConfiguration]=None) -> None:
        """
    ...
    def native_quil_to_executable(self, nq_program: Program) -> QuantumExecutable:
        ...
        request = TranslateNativeQuilToEncryptedBinaryRequest(quil=arithmetic_response.quil, num_shots=nq_program.num_shots)
        with self._qcs_client() as qcs_client:
            response = translate_native_quil_to_encrypted_binary(client=qcs_client, quantum_processor_id=self.quantum_processor_id, json_body=request).parsed
