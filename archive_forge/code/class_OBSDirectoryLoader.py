...
class OBSDirectoryLoader(BaseLoader):
    ...
    def __init__(self, bucket: str, endpoint: str, config: Optional[dict]=None, prefix: str=''):
        ...
        if config.get('get_token_from_ecs'):
            ...
            self.client = ObsClient(access_key_id=config.get('ak'), secret_access_key=config.get('sk'), security_token=config.get('token'), server=endpoint)
        self.bucket = bucket
        self.prefix = prefix
