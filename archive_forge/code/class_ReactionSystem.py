...
class ReactionSystem(object):
    ...
    def __init__(self, rxns, substances=None, name=None, checks=None, dont_check=None, substance_factory=Substance, missing_substances_from_keys=False, sort_substances=None):
        self.rxns = list(rxns)
        ...
        if isinstance(substances, OrderedDict):
            ...
            self.substances = OrderedDict([(s, substance_factory(s)) for s in substances])
        elif all((isinstance(s, Substance) for s in substances)):
    ...
    def split(self, **kwargs):
        ...
        while True:
            ...
            if i >= len(groups):
                break
        return [self.__class__([self.rxns[ri] for ri in gr], OrderedDict([(k, v) for k, v in self.substances.items() if k in gs]), **kwargs) for gr, gs in groups]
    ...
    def _category_colors(self, checks=()):
        ...
        for k in categories['accumulated']:
            colors[k] = ('90ee90', '008000')
        for k in categories['depleted']:
            colors[k] = ('ffb6c1', 'c71585')
        return colors

    def html(self, with_param=True, with_name=True, checks=(), color_categories=True, split=True, print_fn=None):
        ...
        if split:
            ...
            if len(parts) > 1:
                return '<br><hl><br>'.join((rs.html(with_param=with_param, with_name=with_name) for rs in parts))
    ...
    def check_balance(self, strict=False, throw=False):
        ...
        for rxn in self.rxns:
            for net, k in zip(*rxn.composition_violation(self.substances, composition_keys=True)):
                if net != 0:
                    if throw:
                        raise ValueError('Composition violation (%s: %s) in %s' % (k, net, rxn.string(with_param=False, with_name=False)))
                    else:
                        return False
        return True
    ...
    @classmethod
    def from_string(cls, s, substances=None, rxn_parse_kwargs=None, comment_tokens=('#',), **kwargs):
    ...
    def subset(self, pred, checks=()):
        ...
        return tuple((self.__class__(coll, substances=new_substances(coll), checks=checks) for coll in yes_no))
