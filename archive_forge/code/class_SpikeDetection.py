...
class SpikeDetection:
    ...
    def __init__(self, mode: Literal['min', 'max']='min', window: int=10, warmup: int=1, atol: Optional[float]=None, rtol: Optional[float]=2.0, exclude_batches_path: Optional[_PATH]=None, finite_only: bool=True):
    ...
    def on_train_batch_end(self, fabric: 'Fabric', loss: torch.Tensor, batch: Any, batch_idx: int) -> None:
        ...
        if is_spike_global:
            ...
            is_finite_all = self.finite_only or fabric.strategy.reduce_boolean_decision(bool(torch.isfinite(loss).all()), all=True)
            if is_finite_all:
