...
class RandomFourierFeatures(base_layer.Layer):
    ...
    **Usage:** Typically, this layer is used to "kernelize" linear models by
    ...
    def __init__(self, output_dim, kernel_initializer='gaussian', scale=None, trainable=False, name=None, **kwargs):
        if output_dim <= 0:
            raise ValueError(f'`output_dim` should be a positive integer. Received: {output_dim}')
        if isinstance(kernel_initializer, str):
            if kernel_initializer.lower() not in _SUPPORTED_RBF_KERNEL_TYPES:
                raise ValueError(f'Unsupported `kernel_initializer`: {kernel_initializer} Expected one of: {_SUPPORTED_RBF_KERNEL_TYPES}')
        if scale is not None and scale <= 0.0:
            raise ValueError(f'When provided, `scale` should be a positive float. Received: {scale}')
    ...
    def build(self, input_shape):
        ...
        if input_shape.rank != 2:
            raise ValueError(f'The rank of the input tensor should be 2. Received input with rank {input_shape.ndims} instead. Full input shape received: {input_shape}')
        if input_shape.dims[1].value is None:
            raise ValueError(f'The last dimension of the input tensor should be defined. Found `None`. Full input shape received: {input_shape}')
        self.input_spec = input_spec.InputSpec(ndim=2, axes={1: input_shape.dims[1].value})
        ...
        self.unscaled_kernel = self.add_weight(name='unscaled_kernel', shape=(input_dim, self.output_dim), dtype=tf.float32, initializer=kernel_initializer, trainable=False)
        self.bias = self.add_weight(name='bias', shape=(self.output_dim,), dtype=tf.float32, initializer=initializers.RandomUniform(minval=0.0, maxval=2 * np.pi), trainable=False)
        if self.scale is None:
            self.scale = _get_default_scale(self.kernel_initializer, input_dim)
        self.kernel_scale = self.add_weight(name='kernel_scale', shape=(1,), dtype=tf.float32, initializer=tf.compat.v1.constant_initializer(self.scale), trainable=True, constraint='NonNeg')
    ...
    def get_config(self):
        ...
        config = {'output_dim': self.output_dim, 'kernel_initializer': kernel_initializer, 'scale': self.scale}
