...
class CreateDistroCommand(Command):
    ...
    def command(self):
        ...
        if self.verbose:
            ...
            for tmpl_name, tmpl in templates:
                print('  %s%s  %s' % (tmpl_name, ' ' * (max_tmpl_name - len(tmpl_name)), tmpl.summary))
            print()
        if not self.args:
            if self.interactive:
        ...
        vars = {'project': dist_name, 'package': pkg_name, 'egg': pluginlib.egg_name(dist_name)}
        ...
        if found_setup_py:
            ...
            if len(egg_plugins) and (not os.path.exists(plugins_path) or os.path.getmtime(plugins_path) == paster_plugins_mtime):
                if self.verbose:
                    print('Manually creating paster_plugins.txt (deprecated! pass a paster_plugins keyword to setup() instead)', file=sys.stderr)
    ...
    def setup_svn_repository(self, output_dir, dist_name):
        ...
        if not self.simulate:
            os.system(cmd)
    ignore_egg_info_files = ['top_level.txt', 'entry_points.txt', 'requires.txt', 'PKG-INFO', 'namespace_packages.txt', 'SOURCES.txt', 'dependency_links.txt', 'not-zip-safe']
    ...
    def list_templates(self):
        ...
        for entry in self.all_entry_points():
            try:
                ...
                print('Warning: could not load entry point %s (%s: %s)' % (entry.name, e.__class__.__name__, e))
        max_name = max([len(t.name) for t in templates])
        templates = sorted(templates, key=lambda a: a.name)
        print('Available templates:')
        for template in templates:
            print('  %s:%s  %s' % (template.name, ' ' * (max_name - len(template.name)), template.summary))
