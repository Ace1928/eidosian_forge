...
class ComplexInputNetwork(TorchModelV2, nn.Module):
    ...
    def __init__(self, obs_space, action_space, num_outputs, model_config, name):
        ...
        for i, component in enumerate(self.flattened_input_space):
            ...
            if len(component.shape) == 3 and isinstance(component, Box):
                ...
                self.cnns[i] = ModelCatalog.get_model_v2(component, action_space, num_outputs=None, model_config=config, framework='torch', name='cnn_{}'.format(i))
                ...
                self.one_hot[i] = ModelCatalog.get_model_v2(Box(-1.0, 1.0, (size,), np.float32), action_space, num_outputs=None, model_config=config, framework='torch', name='one_hot_{}'.format(i))
                ...
                config = {'fcnet_hiddens': model_config['fcnet_hiddens'], 'fcnet_activation': model_config.get('fcnet_activation'), 'post_fcnet_hiddens': []}
                self.flatten[i] = ModelCatalog.get_model_v2(Box(-1.0, 1.0, (size,), np.float32), action_space, num_outputs=None, model_config=config, framework='torch', name='flatten_{}'.format(i))
        ...
        self.post_fc_stack = ModelCatalog.get_model_v2(Box(float('-inf'), float('inf'), shape=(concat_size,), dtype=np.float32), self.action_space, None, post_fc_stack_config, framework='torch', name='post_fc_stack')
        self.logits_layer = None
        self.value_layer = None
        self._value_out = None
        if num_outputs:
            self.logits_layer = SlimFC(in_size=self.post_fc_stack.num_outputs, out_size=num_outputs, activation_fn=None, initializer=torch_normc_initializer(0.01))
            self.value_layer = SlimFC(in_size=self.post_fc_stack.num_outputs, out_size=1, activation_fn=None, initializer=torch_normc_initializer(0.01))
        else:
    ...
    def forward(self, input_dict, state, seq_lens):
        if SampleBatch.OBS in input_dict and 'obs_flat' in input_dict:
            ...
            orig_obs = restore_original_dimensions(input_dict[SampleBatch.OBS], self.processed_obs_space, tensorlib='torch')
        outs = []
        for i, component in enumerate(tree.flatten(orig_obs)):
            ...
            if i in self.cnns:
                ...
                if component.dtype in [torch.int8, torch.int16, torch.int32, torch.int64, torch.uint8]:
                    one_hot_in = {SampleBatch.OBS: one_hot(component, self.flattened_input_space[int(i)])}
                else:
                    one_hot_in = {SampleBatch.OBS: component}
                one_hot_out, _ = self.one_hot[i](SampleBatch(one_hot_in))
                outs.append(one_hot_out)
            else:
                nn_out, _ = self.flatten[i](SampleBatch({SampleBatch.OBS: torch.reshape(component, [-1, self.flatten_dims[i]])}))
                outs.append(nn_out)
