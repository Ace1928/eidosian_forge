...
class TuneController:
    ...
    def __init__(self, *, search_alg: Optional[SearchAlgorithm]=None, placeholder_resolvers: Optional[Dict[Tuple, Any]]=None, scheduler: Optional[TrialScheduler]=None, stopper: Optional[Stopper]=None, resume: Union[str, bool]=False, fail_fast: bool=False, checkpoint_period: Union[str, int]=None, callbacks: Optional[List[Callback]]=None, metric: Optional[str]=None, trial_checkpoint_config: Optional[CheckpointConfig]=None, storage: Optional[StorageContext]=None, reuse_actors: bool=False, resource_manager_factory: Optional[Callable[[], ResourceManager]]=None, _trainer_api: bool=False):
    ...
    def _process_trial_failure(self, trial: Trial, exception: Union[TuneError, RayTaskError, RayActorError]):
    ...
    def _try_recover(self, trial: Trial, exc: Union[TuneError, RayTaskError, RayActorError]):
    ...
    def __getstate__(self):
        ...
        for k in ['_trials', '_live_trials', '_stop_queue', '_search_alg', '_placeholder_resolvers', '_scheduler_alg', '_pending_trial_queue_times', '_callbacks', '_checkpoint_manager', '_storage', '_insufficient_resources_manager', '_actor_manager', '_class_cache', '_resource_updater', '_trials_to_cache', '_trial_metadata', '_actor_to_trial', '_trial_to_actor', '_resources_to_pending_trials', '_pending_trials', '_pending_trials_list', '_running_trials', '_paused_trials', '_stopped_trials', '_failed_trials', '_resetting_trials', '_started_actors', '_stopping_actors', '_staged_trials', '_actor_cache']:
            del state[k]
        return state
