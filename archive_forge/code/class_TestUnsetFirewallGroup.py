...
class TestUnsetFirewallGroup(TestFirewallGroup, common.TestUnsetFWaaS):
    ...
    def test_unset_ingress_policy(self):
        ...
        arglist = [target, '--ingress-firewall-policy']
        verifylist = [(self.res, target), ('ingress_firewall_policy', True)]
    ...
    def test_unset_egress_policy(self):
        target = self.resource['id']
        arglist = [target, '--egress-firewall-policy']
        verifylist = [(self.res, target), ('egress_firewall_policy', True)]
        parsed_args = self.check_parser(self.cmd, arglist, verifylist)
        result = self.cmd.take_action(parsed_args)
    ...
    def test_unset_enable(self):
        ...
        parsed_args = self.check_parser(self.cmd, arglist, verifylist)
        result = self.cmd.take_action(parsed_args)
        self.mocked.assert_called_once_with(target, **{'admin_state_up': False})
        self.assertIsNone(result)
    ...
    def test_unset_all_port(self):
        target = self.resource['id']
        arglist = [target, '--all-port']
        verifylist = [(self.res, target), ('all_port', True)]
        parsed_args = self.check_parser(self.cmd, arglist, verifylist)
        result = self.cmd.take_action(parsed_args)
        self.mocked.assert_called_once_with(target, **{'ports': []})
        self.assertIsNone(result)