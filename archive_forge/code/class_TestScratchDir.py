...
class TestScratchDir:
    ...
    def test_with_copy(self):
        with open('pre_scratch_text', 'w') as f:
            f.write('write')
        with ScratchDir(self.scratch_root, copy_from_current_on_enter=True, copy_to_current_on_exit=True) as d:
    ...
    def test_symlink(self):
        if os.name != 'nt':
            with ScratchDir(self.scratch_root, copy_from_current_on_enter=False, copy_to_current_on_exit=False, create_symbolic_link=True) as d:
                with open('scratch_text', 'w') as f:
                    f.write('write')
                files = os.listdir(d)
                assert 'scratch_text' in files
