...
def connect_patch_namespaced_service_proxy_with_http_info(self, name, namespace, **kwargs):
    ...
    for key, val in iteritems(params['kwargs']):
        if key not in all_params:
            raise TypeError("Got an unexpected keyword argument '%s' to method connect_patch_namespaced_service_proxy" % key)
        params[key] = val
    del params['kwargs']
    if 'name' not in params or params['name'] is None:
        raise ValueError('Missing the required parameter `name` when calling `connect_patch_namespaced_service_proxy`')
    if 'namespace' not in params or params['namespace'] is None:
        raise ValueError('Missing the required parameter `namespace` when calling `connect_patch_namespaced_service_proxy`')
    collection_formats = {}
    path_params = {}
    if 'name' in params:
