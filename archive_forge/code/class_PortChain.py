...
class PortChain(neutron.NeutronResource):
    ...
    PROPERTIES = NAME, DESCRIPTION, PORT_PAIR_GROUPS, FLOW_CLASSIFIERS, CHAIN_PARAMETERS = ('name', 'description', 'port_pair_groups', 'flow_classifiers', 'chain_parameters')
    properties_schema = {NAME: properties.Schema(properties.Schema.STRING, _('Name of the Port Chain.'), update_allowed=True), DESCRIPTION: properties.Schema(properties.Schema.STRING, _('Description for the Port Chain.'), update_allowed=True), PORT_PAIR_GROUPS: properties.Schema(properties.Schema.LIST, _('A list of port pair groups to apply to the Port Chain.'), update_allowed=True, required=True, schema=properties.Schema(properties.Schema.STRING, _('Port Pair Group ID or Name .'), constraints=[constraints.CustomConstraint('neutron.port_pair_group')])), FLOW_CLASSIFIERS: properties.Schema(properties.Schema.LIST, _('A list of flow classifiers to apply to the Port Chain.'), default=[], update_allowed=True, schema=properties.Schema(properties.Schema.STRING, _('Flow Classifier ID or Name .'), constraints=[constraints.CustomConstraint('neutron.flow_classifier')])), CHAIN_PARAMETERS: properties.Schema(properties.Schema.MAP, _('Dictionary of chain parameters. Currently, only correlation=mpls is supported by default.'), default={'correlation': 'mpls'})}
    ...
    def _show_resource(self):
        return self.client_plugin().show_ext_resource('port_chain', self.resource_id)

    def handle_create(self):
        props = self.prepare_properties(self.properties, self.physical_resource_name())
        port_chain = self.client_plugin().create_ext_resource('port_chain', props)
        self.resource_id_set(port_chain['id'])

    def handle_update(self, json_snippet, tmpl_diff, prop_diff):
        if prop_diff:
            self.prepare_update_properties(prop_diff)
            self.client_plugin().update_ext_resource('port_chain', prop_diff, self.resource_id)

    def handle_delete(self):
        if self.resource_id is None:
            return
        with self.client_plugin().ignore_not_found:
            self.client_plugin().delete_ext_resource('port_chain', self.resource_id)