...
class ScaleIOConnectorTestCase(test_connector.ConnectorTestCase):
    ...
    vol = {'id': 'vol1', 'name': 'test_volume', 'provider_id': 'vol1'}
    fake_guid = '013a5304-d053-4b30-a34f-ee3ad983236d'

    def setUp(self):
        super(ScaleIOConnectorTestCase, self).setUp()
        self.fake_connection_properties = {'hostIP': test_connector.MY_IP, 'serverIP': test_connector.MY_IP, 'scaleIO_volname': self.vol['name'], 'scaleIO_volume_id': self.vol['provider_id'], 'serverPort': 443, 'serverUsername': 'test', 'config_group': 'test', 'failed_over': False, 'iopsLimit': None, 'bandwidthLimit': None}
        self.action_format = 'instances/Volume::{}/action/{{}}'.format(self.vol['id'])
        self.get_volume_api = 'types/Volume/instances/getByName::{}'.format(self.vol['name'])
        self.mock_calls = {self.get_volume_api: self.MockHTTPSResponse(json.dumps(self.vol['id'])), self.action_format.format('addMappedSdc'): self.MockHTTPSResponse(''), self.action_format.format('setMappedSdcLimits'): self.MockHTTPSResponse(''), self.action_format.format('removeMappedSdc'): self.MockHTTPSResponse('')}
        self.error_404 = self.MockHTTPSResponse(content=dict(errorCode=0, message='HTTP 404'), status_code=404)
        ...
        self.mock_object(os, 'listdir', return_value=['emc-vol-{}'.format(self.vol['id'])])
        self.get_password_mock = self.mock_object(scaleio.priv_scaleio, 'get_connector_password', return_value='fake_password')
        self.get_guid_mock = self.mock_object(scaleio.priv_scaleio, 'get_guid', return_value=self.fake_guid)
        self.rescan_vols_mock = self.mock_object(scaleio.priv_scaleio, 'rescan_vols')
        self.connector = scaleio.ScaleIOConnector('sudo', execute=self.fake_execute)
    ...
    def test_get_volume_paths(self, mock_wait_for_path, mock_exists):
        ...
        actual = self.connector.get_volume_paths(self.fake_connection_properties)
        self.assertEqual(expected, actual)

    def test_get_connector_properties(self):
        props = scaleio.ScaleIOConnector.get_connector_properties('sudo', multipath=True, enforce_multipath=True)
        expected_props = {}
    ...
    def test_error_map_volume(self):
        ...
        self.mock_calls[self.action_format.format('addMappedSdc')] = self.MockHTTPSResponse(dict(errorCode=self.connector.VOLUME_NOT_MAPPED_ERROR, message='Test error map volume'), 500)
        self.assertRaises(exception.BrickException, self.test_connect_volume)
    ...
    def test_map_volume_already_mapped_v4(self):
        """Ignore REST API failure for volume already mapped"""
        self.mock_calls[self.action_format.format('addMappedSdc')] = self.MockHTTPSResponse(dict(errorCode=self.connector.VOLUME_ALREADY_MAPPED_ERROR_v4, message='Test error map volume'), 500)
        self.test_connect_volume()
    ...
    def test_disconnect_volume_not_mapped(self):
        ...
        self.mock_calls[self.action_format.format('removeMappedSdc')] = self.MockHTTPSResponse(dict(errorCode=self.connector.VOLUME_NOT_MAPPED_ERROR, message='Test error map volume'), 500)
    ...
    def test_extend_volume(self, mock_device_size, mock_find_volume_path, mock_exists):
        mock_device_size.return_value = 16
        mock_find_volume_path.return_value = 'emc-vol-vol1'
        extended_size = self.connector.extend_volume(self.fake_connection_properties)
        self.assertEqual(extended_size, mock_device_size.return_value)
        self.rescan_vols_mock.assert_called_once_with(self.connector.RESCAN_VOLS_OP_CODE)
