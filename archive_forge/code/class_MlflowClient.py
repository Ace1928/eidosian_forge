...
class MlflowClient:
    ...
    def create_run(self, experiment_id: str, start_time: Optional[int]=None, tags: Optional[Dict[str, Any]]=None, run_name: Optional[str]=None) -> Run:
        """
    ...
    def search_experiments(self, view_type: int=ViewType.ACTIVE_ONLY, max_results: Optional[int]=SEARCH_MAX_RESULTS_DEFAULT, filter_string: Optional[str]=None, order_by: Optional[List[str]]=None, page_token=None) -> PagedList[Experiment]:
        """
        ...
        """
        return self._tracking_client.search_experiments(view_type=view_type, max_results=max_results, filter_string=filter_string, order_by=order_by, page_token=page_token)
    ...
    def log_metric(self, run_id: str, key: str, value: float, timestamp: Optional[int]=None, step: Optional[int]=None, synchronous: bool=True) -> Optional[RunOperations]:
        """
        ...
        .. code-block:: text
            ...
            status: FINISHED
        """
        return self._tracking_client.log_metric(run_id, key, value, timestamp, step, synchronous=synchronous)
    ...
    def set_tag(self, run_id: str, key: str, value: Any, synchronous: bool=True) -> Optional[RunOperations]:
        """
    ...
    def log_batch(self, run_id: str, metrics: Sequence[Metric]=(), params: Sequence[Param]=(), tags: Sequence[RunTag]=(), synchronous: bool=True) -> Optional[RunOperations]:
        """
        ...
        """
        return self._tracking_client.log_batch(run_id, metrics, params, tags, synchronous=synchronous)

    def log_inputs(self, run_id: str, datasets: Optional[Sequence[DatasetInput]]=None) -> None:
        """
    ...
    def log_figure(self, run_id: str, figure: Union['matplotlib.figure.Figure', 'plotly.graph_objects.Figure'], artifact_file: str, *, save_kwargs: Optional[Dict[str, Any]]=None) -> None:
    ...
    def log_image(self, run_id: str, image: Union['numpy.ndarray', 'PIL.Image.Image', 'mlflow.Image'], artifact_file: Optional[str]=None, key: Optional[str]=None, step: Optional[int]=None, timestamp: Optional[int]=None) -> None:
        """
        ...
        if artifact_file is not None and any((arg is not None for arg in [key, step, timestamp])):
            raise TypeError('The `artifact_file` parameter cannot be used in conjunction with `key`, `step`, or `timestamp` parameters. Please ensure that `artifact_file` is specified alone, without any of these conflicting parameters.')
        elif artifact_file is None and key is None:
            raise TypeError('Invalid arguments: Please specify exactly one of `artifact_file` or `key`. Use `key` to log dynamic image charts or `artifact_file` for saving static images. ')
        import numpy as np
        if isinstance(image, np.ndarray):
            ...
            if not isinstance(image, PIL.Image.Image):
                raise TypeError(f'Unsupported image object type: {type(image)}. `image` must be one of numpy.ndarray, PIL.Image.Image, and mlflow.Image.')
        if artifact_file is not None:
            with self._log_artifact_helper(run_id, artifact_file) as tmp_path:
                image.save(tmp_path)
        elif key is not None:
            if not re.match('^[a-zA-Z0-9_\\-./ ]+$', key):
                raise ValueError(f'The `key` parameter may only contain alphanumerics, underscores (_), dashes (-), periods (.), spaces ( ), and slashes (/).The provided key `{key}` contains invalid characters.')
            step = step or 0
            ...
            with self._log_artifact_helper(run_id, metadata_filepath) as tmp_path:
                with open(tmp_path, 'w+') as f:
                    json.dump({'filepath': image_filepath, 'key': key, 'step': step, 'timestamp': timestamp}, f)
    ...
    @experimental
    def log_table(self, run_id: str, data: Union[Dict[str, Any], 'pandas.DataFrame'], artifact_file: str) -> None:
        """
        ...
        if not artifact_file.endswith('.json'):
            raise ValueError(f"The provided artifact file '{artifact_file}' does not have the required '.json' extension. Please ensure the file you are trying to log as a table is a JSON file with the '.json' extension. ")
        ...
        if len(data.select_dtypes(include=['object']).columns) > 0:
            ...
            def process_image(image):
                ...
                return {'type': 'image', 'filepath': image_filepath, 'compressed_filepath': compressed_image_filepath}
            ...
            for column in data.columns:
                ...
                if any(isImage) and (not all(isImage)):
                    raise ValueError(f'Column `{column}` contains a mix of images and non-images. Please ensure that all elements in the column are of the same type.')
                elif all(isImage):
        ...
        if artifact_file in artifacts:
            ...
            _logger.info(f'Appending new table to already existing artifact {artifact_file} for run {run_id}.')
        with self._log_artifact_helper(run_id, artifact_file) as artifact_path:
            try:
                ...
                raise MlflowException(f'Failed to save {data} as table as the data is not JSON serializable. Error: {e}')
    ...
    @experimental
    def load_table(self, experiment_id: str, artifact_file: str, run_ids: Optional[List[str]]=None, extra_columns: Optional[List[str]]=None) -> 'pandas.DataFrame':
        """
    ...
    def search_runs(self, experiment_ids: List[str], filter_string: str='', run_view_type: int=ViewType.ACTIVE_ONLY, max_results: int=SEARCH_MAX_RESULTS_DEFAULT, order_by: Optional[List[str]]=None, page_token: Optional[str]=None) -> PagedList[Run]:
        """
    ...
    def create_registered_model(self, name: str, tags: Optional[Dict[str, Any]]=None, description: Optional[str]=None) -> RegisteredModel:
        """
    ...
    def search_registered_models(self, filter_string: Optional[str]=None, max_results: int=SEARCH_REGISTERED_MODEL_MAX_RESULTS_DEFAULT, order_by: Optional[List[str]]=None, page_token: Optional[str]=None) -> PagedList[RegisteredModel]:
        """
    ...
    def create_model_version(self, name: str, source: str, run_id: Optional[str]=None, tags: Optional[Dict[str, Any]]=None, run_link: Optional[str]=None, description: Optional[str]=None, await_creation_for: int=DEFAULT_AWAIT_MAX_SLEEP_SECONDS) -> ModelVersion:
        """
        ...
        .. code-block:: text
            ...
            Stage: None
        """
        return self._create_model_version(name=name, source=source, run_id=run_id, tags=tags, run_link=run_link, description=description, await_creation_for=await_creation_for)
    ...
    def copy_model_version(self, src_model_uri, dst_name) -> ModelVersion:
        ...
        if urllib.parse.urlparse(src_model_uri).scheme != 'models':
            raise MlflowException(f"Unsupported source model URI: '{src_model_uri}'. The `copy_model_version` API only copies models stored in the 'models:/' scheme.")
        ...
        try:
            ...
            raise MlflowException(f"Failed to fetch model version from source model URI: '{src_model_uri}'. Error: {e}") from e
    ...
    def transition_model_version_stage(self, name: str, version: str, stage: str, archive_existing_versions: bool=False) -> ModelVersion:
        ...
        .. code-block:: text
            ...
            Stage: Staging
        """
        return self._get_registry_client().transition_model_version_stage(name, version, stage, archive_existing_versions)
    ...
    def search_model_versions(self, filter_string: Optional[str]=None, max_results: int=SEARCH_MODEL_VERSION_MAX_RESULTS_DEFAULT, order_by: Optional[List[str]]=None, page_token: Optional[str]=None) -> PagedList[ModelVersion]:
        """
    ...
    def set_model_version_tag(self, name: str, version: Optional[str]=None, key: Optional[str]=None, value: Any=None, stage: Optional[str]=None) -> None:
        ...
        if stage:
            warnings.warn('The `stage` parameter of the `set_model_version_tag` API is deprecated. ' + _STAGES_DEPRECATION_WARNING, category=FutureWarning, stacklevel=2)
    ...
    def delete_model_version_tag(self, name: str, version: Optional[str]=None, key: Optional[str]=None, stage: Optional[str]=None) -> None:
        ...
        _validate_model_version_or_stage_exists(version, stage)
        if stage:
            warnings.warn('The `stage` parameter of the `delete_model_version_tag` API is deprecated. ' + _STAGES_DEPRECATION_WARNING, category=FutureWarning, stacklevel=2)
            latest_versions = self.get_latest_versions(name, stages=[stage])
            if not latest_versions:
                raise MlflowException(f'Could not find any model version for {stage} stage')
            version = latest_versions[0].version
