...
import six
def _compare_metadata_and_return_copy_needed(source_resource, destination_resource, source_mtime, destination_mtime, compare_only_hashes=False, is_cloud_source_and_destination=False):
    ...
    if not skip_mtime_comparison and source_mtime is not None and (destination_mtime is not None):
        return not (source_mtime == destination_mtime and source_resource.size == destination_resource.size)
    return not _compute_hashes_and_return_match(source_resource, destination_resource)