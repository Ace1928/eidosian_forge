...
class MaxUnpool(OpRun):
    ...
    def _run(self, X, indices, output_shape=None, kernel_shape=None, pads=None, strides=None):
        pooling_dims = len(X.shape) - 2
        if pooling_dims > 3:
            raise NotImplementedError(f'Unsupported pooling size {pooling_dims} for operator MaxUnpool.')
        ...
        for dim in range(0, len(kernel_shape)):
            inferred_shape[dim + 2] = (X.shape[dim + 2] - 1) * strides[dim] - (pads[dim] + pads[len(kernel_shape) + dim]) + kernel_shape[dim]
