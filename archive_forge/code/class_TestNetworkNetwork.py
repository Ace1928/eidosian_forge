...
class TestNetworkNetwork(TestNetworkProxy):
    ...
    def test_network_delete(self):
        self.verify_delete(self.proxy.delete_network, network.Network, False, expected_kwargs={'if_revision': None})

    def test_network_delete_ignore(self):
        self.verify_delete(self.proxy.delete_network, network.Network, True, expected_kwargs={'if_revision': None})

    def test_network_delete_if_revision(self):
        self.verify_delete(self.proxy.delete_network, network.Network, True, method_kwargs={'if_revision': 42}, expected_kwargs={'if_revision': 42})

    def test_network_find(self):
        self.verify_find(self.proxy.find_network, network.Network)

    def test_network_find_with_filter(self):
        self._verify('openstack.proxy.Proxy._find', self.proxy.find_network, method_args=['net1'], method_kwargs={'project_id': '1'}, expected_args=[network.Network, 'net1'], expected_kwargs={'project_id': '1', 'ignore_missing': True})
    ...
    def test_network_update(self):
        self.verify_update(self.proxy.update_network, network.Network, expected_kwargs={'x': 1, 'y': 2, 'z': 3, 'if_revision': None})

    def test_network_update_if_revision(self):
        self.verify_update(self.proxy.update_network, network.Network, method_kwargs={'x': 1, 'y': 2, 'z': 3, 'if_revision': 42}, expected_kwargs={'x': 1, 'y': 2, 'z': 3, 'if_revision': 42})