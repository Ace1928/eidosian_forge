...
class TestYAMLFormatter(base.TestBase):
    ...
    def test_list(self):
        ...
        expected = [{'a': 'A1', 'b': 'B1', 'c': 'C1'}, {'a': 'A2', 'b': 'B2', 'c': 'C2'}, {'a': 'A3', 'b': 'B3', 'c': 'C3'}]
        output = StringIO()
        args = mock.Mock()
        sf.add_argument_group(args)
    ...
    def test_list_custom_object(self):
        sf = yaml_format.YAMLFormatter()
        c = ('a', 'toDict', 'to_dict')
        d = (('A1', _toDict(B=1), _to_Dict(C=1)), ('A2', _toDict(B=2), _to_Dict(C=2)), ('A3', _toDict(B=3), _to_Dict(C=3)))
        expected = [{'a': 'A1', 'toDict': {'B': 1}, 'to_dict': {'C': 1}}, {'a': 'A2', 'toDict': {'B': 2}, 'to_dict': {'C': 2}}, {'a': 'A3', 'toDict': {'B': 3}, 'to_dict': {'C': 3}}]
        output = StringIO()
        args = mock.Mock()
        sf.add_argument_group(args)
        sf.emit_list(c, d, output, args)
