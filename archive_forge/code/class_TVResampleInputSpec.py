...
class TVResampleInputSpec(CommandLineInputSpec):
    in_file = File(desc='tensor volume to resample', exists=True, mandatory=True, argstr='-in %s')
    ...
    interpolation = traits.Enum('LEI', 'EI', argstr='-interp %s', desc='Log Euclidean Interpolation')
