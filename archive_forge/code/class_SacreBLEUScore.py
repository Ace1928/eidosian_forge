...
class SacreBLEUScore(BLEUScore):
    ...
    def __init__(self, n_gram: int=4, smooth: bool=False, tokenize: _TokenizersLiteral='13a', lowercase: bool=False, weights: Optional[Sequence[float]]=None, **kwargs: Any) -> None:
        super().__init__(n_gram=n_gram, smooth=smooth, weights=weights, **kwargs)
        self.tokenizer = _SacreBLEUTokenizer(tokenize, lowercase)

    def update(self, preds: Sequence[str], target: Sequence[Sequence[str]]) -> None:
        """Update state with predictions and targets."""
        self.preds_len, self.target_len = _bleu_score_update(preds, target, self.numerator, self.denominator, self.preds_len, self.target_len, self.n_gram, self.tokenizer)
