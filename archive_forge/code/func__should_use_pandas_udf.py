...
def _should_use_pandas_udf(self, input_schema: Schema, output_schema: Schema) -> bool:
    if self.is_spark_connect:
        return True
    compatible = pandas_udf_can_accept(input_schema, is_input=True) and pandas_udf_can_accept(output_schema, is_input=False)
    enabled = self.execution_engine.conf.get_or_throw(FUGUE_SPARK_CONF_USE_PANDAS_UDF, bool)
    if enabled and (not compatible):
        self.log.warning(f'{FUGUE_SPARK_CONF_USE_PANDAS_UDF} is enabled but {input_schema} or {output_schema} is not compatible with pandas udf, using RDD instead')
        return False
    return enabled