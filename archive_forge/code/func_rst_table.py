import numpy as np
def rst_table(cell_values, row_names=None, col_names=None, title='', val_fmt='{0:5.2f}', format_chars=None):
    ...
    table_strs = []
    if title != '':
        table_strs += [title_heading * len(title), title, title_heading * len(title), '']
    ...
    crossed_thick_line = cross_thick_starter + cross_thick_joiner.join(thick_long_headings) + cross_thick_ender
    table_strs += [crossed_line, down_starter + col_header + down_ender, crossed_thick_line]
