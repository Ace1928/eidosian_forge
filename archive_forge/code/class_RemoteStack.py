...
class RemoteStack(resource.Resource):
    ...
    entity = 'stacks'
    PROPERTIES = CONTEXT, TEMPLATE, TIMEOUT, PARAMETERS = ('context', 'template', 'timeout', 'parameters')
    ATTRIBUTES = NAME_ATTR, OUTPUTS = ('stack_name', 'outputs')
    _CONTEXT_KEYS = REGION_NAME, CREDENTIAL_SECRET_ID, CA_CERT, SSL_INSECURE = ('region_name', 'credential_secret_id', 'ca_cert', 'insecure')
    properties_schema = {CONTEXT: properties.Schema(properties.Schema.MAP, _('Context for this stack.'), update_allowed=True, schema={REGION_NAME: properties.Schema(properties.Schema.STRING, _('Region name in which this stack will be created.'), required=False), CREDENTIAL_SECRET_ID: properties.Schema(properties.Schema.STRING, _('A Barbican secret ID. The Barbican secret should contain an OpenStack credential that can be used to access a remote cloud.'), required=False, update_allowed=True, support_status=support.SupportStatus(version='12.0.0')), CA_CERT: properties.Schema(properties.Schema.STRING, _('CA Cert for SSL.'), required=False, update_allowed=True, support_status=support.SupportStatus(version='12.0.0')), SSL_INSECURE: properties.Schema(properties.Schema.BOOLEAN, _("If set, then the server's certificate will not be verified."), default=False, required=False, update_allowed=True, support_status=support.SupportStatus(version='12.0.0'))}), TEMPLATE: properties.Schema(properties.Schema.STRING, _('Template that specifies the stack to be created as a resource.'), required=True, update_allowed=True), TIMEOUT: properties.Schema(properties.Schema.INTEGER, _('Number of minutes to wait for this stack creation.'), update_allowed=True), PARAMETERS: properties.Schema(properties.Schema.MAP, _('Set of parameters passed to this stack.'), default={}, update_allowed=True)}
    attributes_schema = {NAME_ATTR: attributes.Schema(_('Name of the stack.'), type=attributes.Schema.STRING), OUTPUTS: attributes.Schema(_('A dict of key-value pairs output from the stack.'), type=attributes.Schema.MAP)}
    ...
    def _context(self, cacert_path=None):
        ...
        if ctx_props:
            ...
            self._region_name = ctx_props[self.REGION_NAME] if ctx_props[self.REGION_NAME] else self.context.region_name
            _insecure = ctx_props[self.SSL_INSECURE]
            _ssl_verify = False if _insecure else cacert_path or True
    ...
    def validate(self):
        ...
        try:
            ...
            msg = _('Cannot establish connection to Heat endpoint at %(location)s due to "%(exc)s"') % exc_info
        ...
        try:
            ...
            args = {'template': tmpl, 'files': self.stack.t.files, 'environment': env.user_env_as_dict()}
            with TempCACertFile(self.cacert) as cacert_path:
            ...
            if self._credential:
                ...
                location = 'region "%s"' % self._region_name
            exc_info = dict(location=location, exc=str(ex))
            msg = _('Failed validating stack template using Heat endpoint at %(location)s due to "%(exc)s"') % exc_info
            raise exception.StackValidationFailed(message=msg)
    ...
    def handle_create(self):
        ...
        env = environment.get_child_environment(self.stack.env, params)
        tmpl = template_format.parse(self.properties[self.TEMPLATE])
        args = {'stack_name': self.physical_resource_name_or_FnGetRefId(), 'template': tmpl, 'timeout_mins': self.properties[self.TIMEOUT], 'disable_rollback': True, 'parameters': params, 'files': self.stack.t.files, 'environment': env.user_env_as_dict()}
        with TempCACertFile(self.cacert) as cacert_path:
            remote_stack_id = self.heat(cacert_path).stacks.create(**args)['stack']['id']
    ...
    def handle_delete(self):
        if self.resource_id is not None:
            with self.client_plugin().ignore_not_found:
                with TempCACertFile(self.cacert) as cacert_path:
                    self.heat(cacert_path).stacks.delete(stack_id=self.resource_id)
                    return self.resource_id
        return None

    def handle_resume(self):
        if self.resource_id is None:
            raise exception.Error(_('Cannot resume %s, resource not found') % self.name)
        with TempCACertFile(self.cacert) as cacert_path:
            self.heat(cacert_path).actions.resume(stack_id=self.resource_id)

    def handle_suspend(self):
        if self.resource_id is None:
            raise exception.Error(_('Cannot suspend %s, resource not found') % self.name)
        with TempCACertFile(self.cacert) as cacert_path:
            self.heat(cacert_path).actions.suspend(stack_id=self.resource_id)

    def handle_snapshot(self):
        with TempCACertFile(self.cacert) as cacert_path:
            snapshot = self.heat(cacert_path).stacks.snapshot(stack_id=self.resource_id)
        self.data_set('snapshot_id', snapshot['id'])

    def handle_restore(self, defn, restore_data):
        snapshot_id = restore_data['resource_data']['snapshot_id']
        with TempCACertFile(self.cacert) as cacert_path:
            snapshot = self.heat(cacert_path).stacks.snapshot_show(self.resource_id, snapshot_id)
        ...
        props = dict(((k, v) for k, v in self.properties.items() if k in self.properties.data))
    ...
    def handle_update(self, json_snippet, tmpl_diff, prop_diff):
        if self.resource_id:
            ...
            env = environment.get_child_environment(self.stack.env, params)
            tmpl = template_format.parse(self.properties[self.TEMPLATE])
            fields = {'stack_id': self.resource_id, 'parameters': params, 'template': tmpl, 'timeout_mins': self.properties[self.TIMEOUT], 'disable_rollback': self.stack.disable_rollback, 'files': self.stack.t.files, 'environment': env.user_env_as_dict()}
            with TempCACertFile(self.cacert) as cacert_path:
    ...
    def _resolve_attribute(self, name):
        ...
        if name == self.OUTPUTS:
            ...
            return dict(((output['output_key'], output['output_value']) for output in outputs))
