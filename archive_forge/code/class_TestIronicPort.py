...
class TestIronicPort(common.HeatTestCase):
    ...
    def setUp(self):
        ...
        self.fake_local_link_connection = {'switch_info': 'brbm', 'port_id': 'ovs-node-1i1', 'switch_id': '70:4d:7b:88:ff:3a'}
        ...
        self.m_fgn = self.patchobject(ic.IronicClientPlugin, 'get_node')
        self.m_fgpg = self.patchobject(ic.IronicClientPlugin, 'get_portgroup')
    ...
    def test_port_check_create_complete_with_not_found(self):
        b = self._create_resource('port', self.rsrc_defn, self.stack, get_exception=ic_exc.NotFound)
    ...
    def test_port_check_create_complete_with_non_not_found_exception(self):
        b = self._create_resource('port', self.rsrc_defn, self.stack, get_exception=ic_exc.Conflict())
        exc = self.assertRaises(ic_exc.Conflict, b.check_create_complete, self.resource_id)
    ...
    def test_port_check_delete_complete_with_exception(self):
        b = self._create_resource('port', self.rsrc_defn, self.stack, get_exception=ic_exc.Conflict())
        exc = self.assertRaises(ic_exc.Conflict, b.check_delete_complete, self.resource_id)
        self.assertIn('Conflict', str(exc))