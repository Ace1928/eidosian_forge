...
class RelatedFieldWidgetWrapper(forms.Widget):
    ...
    def get_context(self, name, value, attrs):
        ...
        url_params = '&'.join(('%s=%s' % param for param in [(TO_FIELD_VAR, related_field_name), (IS_POPUP_VAR, 1)]))
        context = {'rendered_widget': self.widget.render(name, value, attrs), 'is_hidden': self.is_hidden, 'name': name, 'url_params': url_params, 'model': rel_opts.verbose_name, 'can_add_related': self.can_add_related, 'can_change_related': self.can_change_related, 'can_delete_related': self.can_delete_related, 'can_view_related': self.can_view_related, 'model_has_limit_choices_to': self.rel.limit_choices_to}
