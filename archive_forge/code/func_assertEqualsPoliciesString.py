...
def assertEqualsPoliciesString(self, a, b):
    ...
    expected = [bvle(members=binding_dict['members'], role=binding_dict['role']) for binding_dict in json.loads(a)['bindings']]
    result = [bvle(members=binding_dict['members'], role=binding_dict['role']) for binding_dict in json.loads(b)['bindings']]
