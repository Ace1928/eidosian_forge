    ...
    def parse_content(file_content: str) -> List[Dict[str, str]]:
        ...
        for line in file_content.split("\n"):
            ...
            if line.startswith(USER_PREFIX):
                ...
                if (
                    capture_mode == "user" and user_input
                ):  # Only switch to assistant mode if we were capturing user input
                    capture_mode = "assistant"
                else:
                    logger.warning(
                        "Found assistant response without preceding user input."
                    )
...
if __name__ == "__main__":
    ...
    try:
        MarkdownProcessor.process_directory(input_directory, output_directory)
...
class TestMarkdownConversion(unittest.TestCase):
    ...
    def test_parse_md_content(self):
        conversations = MarkdownParser.parse_content(self.file_content)
    ...
    def test_consecutive_conversations(self):
        ...
        """
        conversations = MarkdownParser.parse_content(file_content)
        self.assertEqual(len(conversations), 2, "Expected two separate conversations")
        self.assertEqual(
            conversations[0]["input"],
            "What's your name?",
            "First conversation input mismatch",
