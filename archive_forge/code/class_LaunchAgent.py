...
class LaunchAgent:
    ...
    def __init__(self, api: Api, config: Dict[str, Any]):
        ...
        self._gorilla_supports_fail_run_queue_items = self._api.fail_run_queue_item_introspection()
        self._queues: List[str] = config.get('queues', ['default'])
        sent_config = config.copy()
        if 'project' in sent_config:
            del sent_config['project']
        create_response = self._api.create_launch_agent(self._entity, self._project, self._queues, sent_config, self.version, self.gorilla_supports_agents)
        self._id = create_response['launchAgentId']
        if self._api.entity_is_team(self._entity):
            wandb.termwarn(f'{LOG_PREFIX}Agent is running on team entity ({self._entity}). Members of this team will be able to run code on this device.')
    ...
    async def fail_run_queue_item(self, run_queue_item_id: str, message: str, phase: str, files: Optional[List[str]]=None) -> None:
    ...
    async def update_status(self, status: str) -> None:
        ...
        update_ret = await _update_status(self._id, status, self.gorilla_supports_agents)
    ...
    async def finish_thread_id(self, thread_id: int, exception: Optional[Union[Exception, LaunchDockerError]]=None) -> None:
        ...
        if job_and_run_status.entity is not None and job_and_run_status.entity != self._entity:
            _logger.info('Skipping check for completed run status because run is on a different entity than agent')
            ...
            _logger.error(f'called finish_thread_id on thread whose tracker has no project or run id. RunQueueItemID: {job_and_run_status.run_queue_item_id}')
            wandb.termerror('Missing project or run id on thread called finish thread id')
                        ...
                        try:
                            ...
                            if _is_scheduler_job(job.get('runSpec', {})):
                                if self.num_running_schedulers >= self._max_schedulers:
                                    wandb.termwarn(f'{LOG_PREFIX}Agent already running the maximum number of sweep schedulers: {self._max_schedulers}. To set this value use `max_schedulers` key in the agent config')
                                    continue
    ...
    async def task_run_job(self, launch_spec: Dict[str, Any], job: Dict[str, Any], default_config: Dict[str, Any], api: Api, job_tracker: JobAndRunStatusTracker) -> None:
        ...
        try:
            ...
            wandb.termerror(f'{LOG_PREFIX}agent {self._name} encountered an issue while starting Docker, see above output for details.')
            exception = e
            wandb._sentry.exception(e)
    ...
    async def _task_run_job(self, launch_spec: Dict[str, Any], job: Dict[str, Any], default_config: Dict[str, Any], api: Api, thread_id: int, job_tracker: JobAndRunStatusTracker) -> None:
        ...
        if _is_scheduler_job(launch_spec):
            ...
            wandb.termlog(f'{LOG_PREFIX}Preparing to run sweep scheduler ({self.num_running_schedulers}/{self._max_schedulers})')
        if not run:
            with self._jobs_lock:
        ...
        while self._jobs_event.is_set():
            ...
            if await job_tracker.check_wandb_run_stopped(self._api):
                ...
                elif time.time() - stopped_time > MAX_WAIT_RUN_STOPPED:
                    await run.cancel()
            await asyncio.sleep(AGENT_POLLING_INTERVAL)
        ...
        try:
            ...
            if status == 'preempted' and job_tracker.entity == self._entity:
                ...
                if config['_resume_count'] > MAX_RESUME_COUNT:
                    wandb.termlog(f'{LOG_PREFIX}Run {job_tracker.run_id} has already resumed {MAX_RESUME_COUNT} times.')
                    return True
                wandb.termlog(f'{LOG_PREFIX}Run {job_tracker.run_id} was preempted, requeueing...')
            ...
            wandb.termerror(f'{LOG_PREFIX}Terminating job {run.id} because it failed to start: {str(e)}')
            known_error = True
            with self._jobs_lock:
                job_tracker.failed_to_start = True
        except Exception as e:
