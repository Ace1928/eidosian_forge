...
class ConfigurationLoader:
    ...
    **1.5 Configuration Manager (`config_manager.py`):**
    ...
    def load_configuration_from_file(self, configuration_file_path: str, file_format: str='json') -> dict:
        """
        ...
        """
        try:
            self.logger.debug(f'Attempting to load configuration from {configuration_file_path} as {file_format}.')
            if file_format == 'json':
                ...
                raise ValueError("Unsupported configuration file format specified. Use 'json' or 'xml'.")
            self.logger.debug(f'Configuration Data: {configuration}')
            return configuration
        except FileNotFoundError:
            self.logger.error(f'The configuration file at {configuration_file_path} was not found.')
            raise
