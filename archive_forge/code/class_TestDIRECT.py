...
class TestDIRECT:
    ...
    def test_direct(self, locally_biased):
        ...
        assert_allclose(res.x, self.optimum_sphere_pos, rtol=0.001, atol=0.001)
        assert_allclose(res.fun, self.optimum_sphere, atol=1e-05, rtol=1e-05)
    ...
    def test_bounds_variants(self, locally_biased):
        ...
        assert_allclose(res_new_bounds.x, x_opt, rtol=0.01)

    @pytest.mark.parametrize('locally_biased', [True, False])
    @pytest.mark.parametrize('eps', [1e-05, 0.0001, 0.001])
    def test_epsilon(self, eps, locally_biased):
        result = direct(self.styblinski_tang, self.bounds_stylinski_tang, eps=eps, vol_tol=1e-06, locally_biased=locally_biased)
