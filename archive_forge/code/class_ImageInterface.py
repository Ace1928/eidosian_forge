...
class ImageInterface(GridInterface):
    ...
    def init(cls, eltype, data, kdims, vdims):
        ...
        if isinstance(data, dict):
            ...
            if shape != expected and (not (not expected and shape == (1,))):
                raise error(f'Key dimension values and value array {vdims[0]} shapes do not match. Expected shape {expected}, actual shape: {shape}', cls)
        if not isinstance(data, np.ndarray) or data.ndim not in [2, 3]:
            ...
            raise ValueError('ImageInterface may only be used on elements that subclass SheetCoordinateSystem.')
    ...
    def select(cls, dataset, selection_mask=None, **selection):
        ...
        """
        selection = {k: slice(*sel) if isinstance(sel, tuple) else sel for k, sel in selection.items()}
    ...
    def groupby(cls, dataset, dim_names, container_type, group_type, **kwargs):
        ...
        if len(dimensions) == 1:
            ...
            samples = [(i, slice(None)) if didx else (slice(None), i) for i in range(dataset.data.shape[abs(didx - 1)])]
            data = np.flipud(dataset.data)
            groups = [(c, group_type((xvals, data[s]), **group_kwargs)) for s, c in zip(samples, coords)]
        else:
            data = zip(*[dataset.dimension_values(i) for i in range(len(dataset.dimensions()))])
            groups = [(g[:dataset.ndims], group_type([g[dataset.ndims:]], **group_kwargs)) for g in data]
    ...
    def aggregate(cls, dataset, kdims, function, **kwargs):
        ...
        if not kdims:
            ...
            return ({kdims[0]: cls.values(dataset, axes[0], expanded=False), dataset.vdims[0].name: data[::-1] if axes[0] else data}, [])