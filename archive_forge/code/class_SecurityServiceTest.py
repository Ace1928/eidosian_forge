...
class SecurityServiceTest(utils.TestCase):
    ...
    def setUp(self):
        ...
        self.manager = security_services.SecurityServiceManager(fakes.FakeClient())
    ...
    def test_create_some_fields(self):
        values = {'type': 'ldap', 'dns_ip': 'fake dns ip', 'server': 'fake.ldap.server', 'domain': 'fake.ldap.domain', 'user': 'fake user'}
        with mock.patch.object(self.manager, '_create', fakes.fake_create):
            result = self.manager.create(**values)
            self.assertEqual(result['url'], security_services.RESOURCES_PATH)
            self.assertEqual(result['resp_key'], security_services.RESOURCE_NAME)
    ...
    def test_delete(self):
        ...
        with mock.patch.object(self.manager, '_delete', mock.Mock()):
            ...
            self.manager._delete.assert_called_once_with(security_services.RESOURCE_PATH % security_service)
    ...
    def test_get(self):
        ...
        with mock.patch.object(self.manager, '_get', mock.Mock()):
            ...
            self.manager._get.assert_called_once_with(security_services.RESOURCE_PATH % security_service, security_services.RESOURCE_NAME)
    ...
    def test_list_with_filters(self):
        ...
        with mock.patch.object(self.manager, '_list', mock.Mock(return_value=None)):
            ...
            self.manager._list.assert_called_once_with(security_services.RESOURCES_PATH + expected_postfix, security_services.RESOURCES_NAME)

    def test_update(self):
        security_service = 'fake service'
        values = {'dns_ip': 'new dns ip', 'ou': 'new ou', 'server': 'new.ldap.server', 'domain': 'new.ldap.domain', 'user': 'new user', 'password': 'fake password'}
        with mock.patch.object(self.manager, '_update', fakes.fake_update):
            result = self.manager.update(security_service, **values)
            self.assertEqual(result['url'], security_services.RESOURCE_PATH % security_service)
            self.assertEqual(result['resp_key'], security_services.RESOURCE_NAME)
            self.assertEqual(result['body'][security_services.RESOURCE_NAME], values)
