...
class KAN:
    ...
    def _activation_derivative(self, activation: Callable, x: np.ndarray) -> np.ndarray:
        ...
        """
        if activation == kan.tanh:
            return 1 - np.tanh(x) ** 2
        elif activation == kan.relu:
            return np.where(x > 0, 1, 0)
        elif activation == kan.sigmoid:
...
if __name__ == "__main__":
    ...
    # Initialize the KAN
    kan_instance = KAN(input_size, output_size, depth, activation)

    # Create dummy data for training
    dataset = np.random.randn(100, input_size).astype(np.float32)
    targets = np.random.randn(100, output_size).astype(np.float32)

    # Train the KAN
    kan_instance.train(dataset, targets, learning_rate=0.01, epochs=10)

    # Save the trained model
    kan_instance.save("kan_model.pkl")

    # Load the model
    loaded_kan = KAN.load("kan_model.pkl")
