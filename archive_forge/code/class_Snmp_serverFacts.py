                    ...
                    for param, val in iteritems(k):
                        if param in ['clients', 'networks']:
                            if None in val:
                                val.remove(None)
                            val.sort()
                objs['communities'] = sorted(list(objs['communities'].values()), key=lambda k, sk='name': k[sk])
            if 'listen_addresses' in objs:
                objs['listen_addresses'] = sorted(list(objs['listen_addresses'].values()), key=lambda k, sk='address': k[sk])
            if 'snmp_v3' in objs:
                if 'groups' in objs['snmp_v3']:
                    objs['snmp_v3']['groups'] = sorted(list(objs['snmp_v3']['groups'].values()), key=lambda k, sk='group': k[sk])
                if 'trap_targets' in objs['snmp_v3']:
                    objs['snmp_v3']['trap_targets'] = sorted(list(objs['snmp_v3']['trap_targets'].values()), key=lambda k, sk='address': k[sk])
                if 'users' in objs['snmp_v3']:
                    objs['snmp_v3']['users'] = sorted(list(objs['snmp_v3']['users'].values()), key=lambda k, sk='user': k[sk])
                if 'views' in objs['snmp_v3']:
                    objs['snmp_v3']['views'] = sorted(list(objs['snmp_v3']['views'].values()), key=lambda k, sk='view': k[sk])
        else:
            objs = {}
        ansible_facts['ansible_network_resources'].pop('snmp_server', None)
        params = utils.remove_empties(snmp_server_parser.validate_config(self.argument_spec, {'config': objs}, redact=True))
