  ...
  *{command}* is used to remove an exclusion configuration for preconfigured WAF
  ...
  """
    ...
    def Args(cls, parser):
        ...
        cls.NAME_ARG = flags.PriorityArgument('remove the exclusion configuration for preconfigured WAF evaluation')
        cls.NAME_ARG.AddArgument(parser, operation_type='remove the exclusion configuration for preconfigured WAF evaluation', cust_metavar='PRIORITY')
        flags.AddRegionFlag(parser, 'remove the exclusion configuration for preconfigured WAF evaluation')
    ...
    def Run(cls, release_track, args):
        ...
        if args.target_rule_set == '*':
            if args.IsSpecified('target_rule_ids') or args.IsSpecified('request_header_to_exclude') or args.IsSpecified('request_cookie_to_exclude') or args.IsSpecified('request_query_param_to_exclude') or args.IsSpecified('request_uri_to_exclude'):
                raise exceptions.InvalidArgumentException('target-rule-set', 'Arguments in [--target-rule-ids, --request-header-to-exclude, --request-cookie-to-exclude, --request-query-param-to-exclude, --request-uri-to-exclude] cannot be specified when --target-rule-set is set to *.')
