...
class HeatWaitConditionTest(common.HeatTestCase):
    ...
    def create_stack(self, stack_id=None, template=test_template_heat_waitcondition_count, params={}, stub=True, stub_status=True):
        ...
        with utils.UUIDStub(self.stack_id):
            stack.store()
        if stub:
            id = identifier.ResourceIdentifier('test_tenant', stack.name, stack.id, '', 'wait_handle')
            self.patchobject(h_wch.HeatWaitConditionHandle, 'identifier', return_value=id)
        if stub_status:
    ...
    def test_post_complete_to_handle(self):
        ...
        mock_get_status.side_effect = [['SUCCESS'], ['SUCCESS', 'SUCCESS'], ['SUCCESS', 'SUCCESS', 'SUCCESS']]
        self.stack.create()
        rsrc = self.stack['wait_condition']
        self.assertEqual((rsrc.CREATE, rsrc.COMPLETE), rsrc.state)
        r = resource_objects.Resource.get_by_name_and_stack(self.stack.context, 'wait_handle', self.stack.id)
    ...
    def test_post_failed_to_handle(self):
        self.stack = self.create_stack()
        mock_get_status = h_wch.HeatWaitConditionHandle.get_status
        mock_get_status.side_effect = [['SUCCESS'], ['SUCCESS', 'SUCCESS'], ['SUCCESS', 'SUCCESS', 'FAILURE']]
        self.stack.create()
        rsrc = self.stack['wait_condition']
    ...
    def test_wait_handle_bad_reference(self):
        self._test_wait_handle_invalid(test_template_waithandle_bad_reference, 'wait_handel')
    ...
    def test_timeout(self):
        ...
        h_wch.HeatWaitConditionHandle.get_status().MultipleTimes().AndReturn([])
        self.stack.create()
        rsrc = self.stack['wait_condition']
        self.assertEqual((rsrc.CREATE, rsrc.FAILED), rsrc.state)
        reason = rsrc.status_reason
    ...
    def test_data(self):
        ...
        test_metadata = {'data': 'foo', 'reason': 'bar', 'status': 'SUCCESS', 'id': '123'}
        ...
        test_metadata = {'data': 'dog', 'reason': 'cat', 'status': 'SUCCESS', 'id': '456'}
        ret = handle.handle_signal(details=test_metadata)
        wc_att = rsrc.FnGetAtt('data')
        self.assertEqual(json.loads(u'{"123": "foo", "456": "dog"}'), json.loads(wc_att))
        self.assertEqual('status:SUCCESS reason:cat', ret)
        mock_get_status.assert_called_once()

    def test_data_noid(self):
        rsrc, handle, mock_get_status = self._create_heat_wc_and_handle()
        test_metadata = {'data': 'foo', 'reason': 'bar', 'status': 'SUCCESS'}
        ret = handle.handle_signal(details=test_metadata)
        wc_att = rsrc.FnGetAtt('data')
        self.assertEqual('{"1": "foo"}', wc_att)
        self.assertEqual('status:SUCCESS reason:bar', ret)
        test_metadata = {'data': 'dog', 'reason': 'cat', 'status': 'SUCCESS'}
        ret = handle.handle_signal(details=test_metadata)
        wc_att = rsrc.FnGetAtt('data')
        self.assertEqual(json.loads(u'{"1": "foo", "2": "dog"}'), json.loads(wc_att))
        self.assertEqual('status:SUCCESS reason:cat', ret)
        mock_get_status.assert_called_once()
    ...
    def test_data_nodata(self):
        ...
        wc_att = rsrc.FnGetAtt('data')
        self.assertEqual(json.loads(u'{"1": null, "2": null}'), json.loads(wc_att))
        mock_get_status.assert_called_once()
    ...
    def test_get_status_none_complete(self):
        ...
        md_expected = {'1': {'data': None, 'reason': 'Signal 1 received', 'status': 'SUCCESS'}}
    ...
    def test_get_status_failure(self):
        ...
        md_expected = {'1': {'data': None, 'reason': 'Signal 1 received', 'status': 'FAILURE'}}
        self.assertEqual(md_expected, handle.metadata_get())
    ...
    def test_getatt_endpoint(self):
        ...
        handle = self._create_heat_handle()
        expected = 'foo/aprojectid/stacks/test_stack/%s/resources/wait_handle/signal' % self.stack_id
        self.assertEqual(expected, handle.FnGetAtt('endpoint'))

    def test_getatt_curl_cli(self):
        self.patchobject(heat_plugin.HeatClientPlugin, 'get_heat_url', return_value='foo/%s' % self.tenant_id)
        self.patchobject(heat_plugin.HeatClientPlugin, 'get_insecure_option', return_value=False)
        handle = self._create_heat_handle()
        expected = "curl -i -X POST -H 'X-Auth-Token: adomainusertoken' -H 'Content-Type: application/json' -H 'Accept: application/json' foo/aprojectid/stacks/test_stack/%s/resources/wait_handle/signal" % self.stack_id
    ...
    def test_getatt_signal_zaqar(self, mock_signed_url):
        handle = self._create_heat_handle(template=test_template_heat_waithandle_zaqar)
        self.assertIsNone(handle.FnGetAtt('token'))
        self.assertIsNone(handle.FnGetAtt('endpoint'))
        self.assertIsNone(handle.FnGetAtt('curl_cli'))
        signal = json.loads(handle.FnGetAtt('signal'))
    ...
    def test_getatt_signal_none(self):
        handle = self._create_heat_handle(template=test_template_heat_waithandle_none)
        self.assertIsNone(handle.FnGetAtt('token'))
        self.assertIsNone(handle.FnGetAtt('endpoint'))
        self.assertIsNone(handle.FnGetAtt('curl_cli'))
    ...
    def test_create_update_updatehandle(self):
        self.stack = self.create_stack(template=test_template_update_waithandle, stub_status=False)
        self.stack.create()
        handle = self.stack['update_wait_handle']
        self.assertEqual((handle.CREATE, handle.COMPLETE), handle.state)
        self.assertRaises(resource.UpdateReplace, handle.update, None, None)