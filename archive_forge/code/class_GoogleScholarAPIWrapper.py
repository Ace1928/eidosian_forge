...
class GoogleScholarAPIWrapper(BaseModel):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        try:
            ...
            raise ImportError('google-search-results is not installed. Please install it with `pip install google-search-results>=2.4.2`')
    ...
    def run(self, query: str) -> str:
        ...
        while page < max(self.top_k_results - 20, 1):
            results = self.google_scholar_engine({'q': query, 'start': page, 'hl': self.hl, 'num': min(self.top_k_results, 20), 'lr': self.lr}).get_dict().get('organic_results', [])
            total_results.extend(results)
            if not results:
                break
            page += 20
        if self.top_k_results % 20 != 0 and page > 20 and total_results:
            results = self.google_scholar_engine({'q': query, 'start': page, 'num': self.top_k_results % 20, 'hl': self.hl, 'lr': self.lr}).get_dict().get('organic_results', [])
            total_results.extend(results)
        if not total_results:
            return 'No good Google Scholar Result was found'
        docs = [f'Title: {result.get('title', '')}\nAuthors: {','.join([author.get('name') for author in result.get('publication_info', {}).get('authors', [])])}\nSummary: {result.get('publication_info', {}).get('summary', '')}\nTotal-Citations: {result.get('inline_links', {}).get('cited_by', {}).get('total', '')}' for result in total_results]
        return '\n\n'.join(docs)