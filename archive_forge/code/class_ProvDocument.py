...
class ProvDocument(ProvBundle):
    ...
    def flattened(self):
        ...
        if self._bundles:
            ...
            bundled_records = itertools.chain(*[b.get_records() for b in self._bundles.values()])
    ...
    def update(self, other):
        ...
        if isinstance(other, ProvBundle):
            ...
            raise ProvException('ProvDocument.update(): The other is not a ProvDocument or ProvBundle instance (%s)' % type(other))
    ...
    def add_bundle(self, bundle, identifier=None):
        ...
        if bundle.is_document():
            if bundle.has_bundles():
                raise ProvException('Cannot add a document with nested bundles as a bundle.')
    ...
    def bundle(self, identifier):
        ...
        if valid_id is None:
            raise ProvException('The provided identifier "%s" is not valid' % identifier)
        if valid_id in self._bundles:
            raise ProvException('A bundle with that identifier already exists')
