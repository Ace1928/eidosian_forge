...
class Reencrypt(base.Command):
    ...
    """
        ...
        if not console_io.PromptContinue("WARNING: Reencryption will restart your instance if the primary key version is different from the instance's key version."):
            return None
        try:
            result = sql_client.instances.Reencrypt(sql_messages.SqlInstancesReencryptRequest(instance=instance_ref.instance, project=instance_ref.project))
            ...
            operations.OperationsV1Beta4.WaitForOperation(sql_client, operation_ref, 'Reencrypting Cloud SQL instance')
