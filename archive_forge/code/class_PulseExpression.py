...
class PulseExpression(ast.NodeTransformer):
    ...
    _math_ops: dict[str, Callable | float] = {'acos': cmath.acos, 'acosh': cmath.acosh, 'asin': cmath.asin, 'asinh': cmath.asinh, 'atan': cmath.atan, 'atanh': cmath.atanh, 'cos': cmath.cos, 'cosh': cmath.cosh, 'exp': cmath.exp, 'log': cmath.log, 'log10': cmath.log10, 'sin': cmath.sin, 'sinh': cmath.sinh, 'sqrt': cmath.sqrt, 'tan': cmath.tan, 'tanh': cmath.tanh, 'pi': cmath.pi, 'e': cmath.e}
            ...
            for key, val in kwargs.items():
                if key in self.params:
                    if key not in self._locals_dict.keys():
                        ...
                        raise PulseError("%s got multiple values for argument '%s'" % (self.__class__.__name__, key))
                else:
                    raise PulseError("%s got an unexpected keyword argument '%s'" % (self.__class__.__name__, key))
        ...
        if node.id in self._math_ops:
            ...
            if not isinstance(_val, ParameterExpression):
                try:
                    ...
                    raise PulseError(f'Invalid parameter value {node.id} = {self._locals_dict[node.id]} is specified.') from ex
    ...
    def visit_BinOp(self, node: ast.BinOp) -> ast.BinOp | ast.Constant:
        ...
        if isinstance(node.left, ast.Constant) and isinstance(node.right, ast.Constant):
            val = ast.Constant(self._match_ops(node.op, self._binary_ops, node.left.value, node.right.value))
            return ast.copy_location(val, node)
        return node
