...
class Snake:
    ...
    def __init__(
        ...
        self.collision_detection_strategy: DefaultCollisionDetectionStrategy = (
...
def run() -> None:
    """
    Run the AI snake game with automatic restarts.
    """
    # Initialize game objects
    snake: Snake = Snake(Position(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2))
    fruit: Fruit = Fruit(Position(0, 0))
    game_grid: Grid = Grid(SCREEN_WIDTH, SCREEN_HEIGHT, BLOCK_SIZE, snake, fruit)

    # Initialize pathfinding algorithms
    pathfinding_algorithms: List[Callable] = [
        game_grid.find_path_theta_star,
        game_grid.find_path_adaptive_theta_star,
        game_grid.find_path_hierarchical_theta_star,
        game_grid.find_path_advanced_theta_star,
        game_grid.find_path_advanced_adaptive_theta_star,
        game_grid.find_path_advanced_hierarchical_theta_star,
    ]

    # Initialize learning algorithms
    learning_algorithms: List[Callable] = [
        decide_by_q_learning,
        decide_by_sarsa,
        decide_by_monte_carlo,
    ]

    while True:
        game_over: bool = False
        snake.reset()
        fruit.respawn(game_grid.grid)

        while not game_over:
            # Choose the next action based on the current game state
            state: Position = snake.head
            goal: Position = fruit.position

            action: Optional[Position] = None
            path: List[Position] = []

            # Use pathfinding algorithms first
            for pathfinding_func in pathfinding_algorithms:
                try:
                    path = pathfinding_func(state, goal, game_grid, snake, fruit)
                    if path:
                        action = path[1] - state
                        break
            ...
            if action is None:
                for learning_func in learning_algorithms:
                    try:
            ...
            if action is None:
                action = decide_by_neural_network(state, goal)
            ...
            if game_grid.is_collision(next_state):
                game_over = True
                reward: float = -1.0
            elif next_state == fruit.position:
                # Snake ate the fruit
                snake.length += 1
                snake.score += 1
                reward = 1.0
            ...
            if path:
                train_q_learning(state, action, reward, next_state)
                train_neural_network(state, goal, path)

            # Update algorithm priorities based on game state
            game_state: Dict[str, Any] = {
                "step": snake.score,
                "snake_length": snake.length,
                "fruit_eaten": snake.score,
            }
            update_algorithm_priorities(game_state)

            # Delay to control game speed
            time.sleep(0.1)

        logging.info(f"Game over. Final score: {snake.score}")
