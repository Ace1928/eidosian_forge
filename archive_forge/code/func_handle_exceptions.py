    ...
    async def wrapper(*args, **kwargs):
        ...
        while retries < config.max_retries:
            try:
                ...
                retries += 1
                logger.warning(f'Retry {retries}/{config.max_retries}: Function {func.__name__} timed out after {config.timeout} seconds. Retrying...')
            except Exception as e:
                retries += 1
                logger.warning(f'Retry {retries}/{config.max_retries}: An exception occurred in function {func.__name__}: {str(e)}. Retrying in {config.retry_delay} seconds...')
                await asyncio.sleep(config.retry_delay)
        logger.error(f'Max retries exceeded. Raising exception...')
        raise RuntimeError(f'Function {func.__name__} failed after {config.max_retries} retries')
    return wrapper