...
class SdaFabricControlPlaneDevice(object):
    ...
    def __init__(self, params, dnac):
        self.dnac = dnac
        self.new_object = dict(deviceManagementIpAddress=params.get('deviceManagementIpAddress'), siteNameHierarchy=params.get('siteNameHierarchy'), routeDistributionProtocol=params.get('routeDistributionProtocol'), device_management_ip_address=params.get('deviceManagementIpAddress'))
    ...
    def get_object_by_name(self, name, is_absent=False):
        result = None
        try:
            items = self.dnac.exec(family='sda', function='get_control_plane_device', params=self.get_all_params(name=name))
    ...
    def requires_update(self, current_obj):
        ...
        obj_params = [('deviceManagementIpAddress', 'deviceManagementIpAddress'), ('siteNameHierarchy', 'siteNameHierarchy'), ('routeDistributionProtocol', 'routeDistributionProtocol'), ('deviceManagementIpAddress', 'device_management_ip_address')]
    ...
    def create(self):
        result = self.dnac.exec(family='sda', function='add_control_plane_device', params=self.create_params(), op_modifies=True)
    ...
    def delete(self):
        ...
        name = self.new_object.get('name')
        result = None
        result = self.dnac.exec(family='sda', function='delete_control_plane_device', params=self.delete_all_params())
        return result