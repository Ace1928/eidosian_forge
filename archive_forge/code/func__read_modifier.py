...
def _read_modifier(file_obj):
    modifier = formats.MODIFIER_DEF._make(struct.unpack(formats.MODIFIER_DEF_PACK, file_obj.read(formats.MODIFIER_DEF_SIZE)))
    if modifier.type == b'i':
        return InverseModifier()
    elif modifier.type == b'c':
        return ControlModifier(num_ctrl_qubits=modifier.num_ctrl_qubits, ctrl_state=modifier.ctrl_state)
