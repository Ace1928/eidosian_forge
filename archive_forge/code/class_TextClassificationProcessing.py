...
class TextClassificationProcessing(TaskProcessor):
    ...
    def dataset_processing_func(self, example: Dict[str, Any], data_keys: Dict[str, str], ref_keys: Optional[List[str]]=None) -> Dict[str, Any]:
        tokenized_inputs = self.preprocessor(text=example[data_keys['primary']], text_pair=example[data_keys['secondary']] if 'secondary' in data_keys else None, **self.defaults, **self.preprocessor_kwargs)
    ...
    def try_to_guess_data_keys(self, column_names: List[str]) -> Optional[Dict[str, str]]:
        ...
        if primary_key_name is None:
            ...
            logger.info('Could not infer the secondary key in the dataset, if it does contain one, please provide it manually.')
    ...
    def load_dataset(self, path: str, data_keys: Optional[Dict[str, str]]=None, ref_keys: Optional[List[str]]=None, only_keep_necessary_columns: bool=False, load_smallest_split: bool=False, num_samples: Optional[int]=None, shuffle: bool=False, **load_dataset_kwargs) -> Union['DatasetDict', 'Dataset']:
        dataset = super().load_dataset(path, data_keys=data_keys, ref_keys=ref_keys, only_keep_necessary_columns=only_keep_necessary_columns, load_smallest_split=load_smallest_split, num_samples=num_samples, shuffle=shuffle, **load_dataset_kwargs)
        return dataset