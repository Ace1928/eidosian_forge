...
class CloudflareWorkersAIEmbeddings(BaseModel, Embeddings):
    ...
    def embed_documents(self, texts: List[str]) -> List[List[float]]:
        ...
        for batch in batches:
            response = requests.post(f'{self.api_base_url}/{self.account_id}/ai/run/{self.model_name}', headers=self.headers, json={'text': batch})
    ...
    def embed_query(self, text: str) -> List[float]:
        ...
        response = requests.post(f'{self.api_base_url}/{self.account_id}/ai/run/{self.model_name}', headers=self.headers, json={'text': [text]})
