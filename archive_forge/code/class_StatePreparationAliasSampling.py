...
class StatePreparationAliasSampling(select_and_prepare.PrepareOracle):
    ...
    """
    selection_registers: Tuple[infra.SelectionRegister, ...] = attr.field(converter=lambda v: (v,) if isinstance(v, infra.SelectionRegister) else tuple(v))
    alt: NDArray[np.int_]
    ...
    def junk_registers(self) -> Tuple[infra.Register, ...]:
        return tuple(infra.Signature.build(sigma_mu=self.sigma_mu_bitsize, alt=self.alternates_bitsize, keep=self.keep_bitsize, less_than_equal=1))

    def _value_equality_values_(self):
        return (self.selection_registers, tuple(self.alt.ravel()), tuple(self.keep.ravel()), self.mu)

    def __repr__(self) -> str:
        alt_repr = cirq._compat.proper_repr(self.alt)
        keep_repr = cirq._compat.proper_repr(self.keep)
        return f'cirq_ft.StatePreparationAliasSampling({self.selection_registers}, {alt_repr}, {keep_repr}, {self.mu})'
    ...
    def decompose_from_registers(self, *, context: cirq.DecompositionContext, **quregs: NDArray[cirq.Qid]) -> cirq.OP_TREE:
        ...
        qrom_gate = qrom.QROM([self.alt, self.keep], (self.selection_bitsize,), (self.alternates_bitsize, self.keep_bitsize))
        ...
        yield swap_network.MultiTargetCSwap.make_on(control=less_than_equal, target_x=alt, target_y=selection)