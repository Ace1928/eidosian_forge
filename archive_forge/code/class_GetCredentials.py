...
class GetCredentials(base.Command):
    ...
    detailed_help = {'EXAMPLES': kubeconfig.COMMAND_EXAMPLE, 'DESCRIPTION': kubeconfig.COMMAND_DESCRIPTION.format(cluster_type='Attached cluster')}
    ...
    def Run(self, args):
        ...
        with endpoint_util.GkemulticloudEndpointOverride(resource_args.ParseAttachedClusterResourceArg(args).locationsId, self.ReleaseTrack()):
            ...
            if resp.state != util.GetMessagesModule().GoogleCloudGkemulticloudV1AttachedCluster.StateValueValuesEnum.RUNNING:
                log.warning(kubeconfig.NOT_RUNNING_MSG.format(cluster_ref.attachedClustersId))
            context = kubeconfig.GenerateContext('attached', cluster_ref.projectsId, cluster_ref.locationsId, cluster_ref.attachedClustersId)
            kubeconfig.GenerateAttachedKubeConfig(resp, cluster_ref.attachedClustersId, context, args.auth_provider_cmd_path)