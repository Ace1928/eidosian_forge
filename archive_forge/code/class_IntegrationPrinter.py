...
class IntegrationPrinter(cp.CustomPrinterBase):
    ...
    def Transform(self, record: base.Record) -> cp._Marker:
        ...
        component_block = formatter.TransformComponentStatus(record) if record.status.resourceComponentStatuses else 'Status not available'
        lines = [self.Header(record), ' ', self._DeploymentProgress(record.latest_deployment, formatter), config_block, ' ', cp.Labeled([cp.Lines(['Integration Components', component_block])])]
    ...
    """
        ...
        return con.Emphasize('{} Integration status: {} in region {}'.format(symbol, record.name, record.region))
