...
class Pathfinder:
    ...
    def heuristic(
        ...
        heuristic_value += self.calculate_euclidean_distance(
            current_position, goal_position
        )

        # If a secondary goal position is provided, add a weighted distance to the heuristic value
        if secondary_goal_position is not None:
            heuristic_value += 0.5 * self.calculate_euclidean_distance(
                current_position, secondary_goal_position
            )

        # If a tertiary goal position is provided, add a weighted distance to the heuristic value
        if tertiary_goal_position is not None:
            heuristic_value += 0.3 * self.calculate_euclidean_distance(
                current_position, tertiary_goal_position
            )

        # If a quaternary goal position is provided, add a weighted distance to the heuristic value
        if quaternary_goal_position is not None:
            heuristic_value += 0.1 * self.calculate_euclidean_distance(
                current_position, quaternary_goal_position
            )

        # Calculate the obstacle proximity penalty and add it to the heuristic value
        heuristic_value += self.calculate_obstacle_proximity_penalty(
            current_position, space_around_obstacles
        )

        # Calculate the boundary proximity penalty and add it to the heuristic value
        heuristic_value += self.calculate_boundary_proximity_penalty(
        ...
        if body_size_adaptations:
            heuristic_value += self.calculate_body_position_proximity_penalty(
