...
class kUpCCGSD(Operation):
    ...
    def _flatten(self):
        hyperparameters = (('k', self.hyperparameters['k']), ('delta_sz', self.hyperparameters['delta_sz']), ('init_state', tuple(self.hyperparameters['init_state'])))
    ...
    def __init__(self, weights, wires, k=1, delta_sz=0, init_state=None, id=None):
        ...
        if shape != (k, len(s_wires) + len(d_wires)):
            raise ValueError(f'Weights tensor must be of shape {(k, len(s_wires) + len(d_wires))}; got {shape}.')
        ...
        self._hyperparameters = {'init_state': init_state, 's_wires': s_wires, 'd_wires': d_wires, 'k': k, 'delta_sz': delta_sz}
    ...
    def compute_decomposition(weights, wires, s_wires, d_wires, k, init_state, delta_sz=None):
        ...
        for layer in range(k):
            for i, (w1, w2) in enumerate(d_wires):
                op_list.append(qml.FermionicDoubleExcitation(weights[layer][len(s_wires) + i], wires1=w1, wires2=w2))
    ...
    def shape(k, n_wires, delta_sz):
        ...
        """
        if n_wires < 4:
            raise ValueError(f"This template requires the number of qubits to be greater than four; got 'n_wires' = {n_wires}")
        if n_wires % 2:
            raise ValueError(f"This template requires an even number of qubits; got 'n_wires' = {n_wires}")
