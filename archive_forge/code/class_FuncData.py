...
class FuncData:
    ...
    def __init__(self, func, data, param_columns, result_columns=None, result_func=None, rtol=None, atol=None, param_filter=None, knownfailure=None, dataname=None, nan_ok=False, vectorized=True, ignore_inf_sign=False, distinguish_nan_and_inf=True):
        self.func = func
        self.data = data
    ...
    def __repr__(self):
        ...
        if np.any(list(map(np.iscomplexobj, self.param_columns))):
            ...
            is_complex = ''
        if self.dataname:
            return '<Data for {}{}: {}>'.format(self.func.__name__, is_complex, os.path.basename(self.dataname))
        else:
