...
class StandardDecorator:
    ...
    async def validate_arguments(self, func: F, *args, **kwargs) -> None:
        ...
        # Adjust for methods by checking if the function is bound
        if inspect.ismethod(func):
            # For bound methods, the first argument is always 'self' or 'cls' for class methods
            bound_args = inspect.signature(func).bind(*args, **kwargs)
        else:
            # For functions and static methods, bind directly
