...
class FugueSQLWorkflow(FugueWorkflow):
    ...
    def __call__(self, code: str, *args: Any, **kwargs: Any) -> None:
        ...
        variables = self._sql(code, self._sql_vars, global_vars, local_vars, *args, **kwargs)
        for k, v in variables.items():
            if isinstance(v, WorkflowDataFrame) and v.workflow is self:
                self._sql_vars[k] = v

    def _sql(self, code: str, *args: Any, **kwargs: Any) -> Dict[str, Tuple[WorkflowDataFrame, WorkflowDataFrames, LazyWorkflowDataFrame]]:
        FugueSQLParser = import_fsql_dependency('fugue_sql_antlr').FugueSQLParser
        from ._visitors import FugueSQLHooks, _Extensions
        params: Dict[str, Any] = {}
        for a in args:
            assert_or_throw(isinstance(a, Dict), lambda: f'args can only have dict: {a}')
            params.update(a)
        params.update(kwargs)
        params, dfs = self._split_params(params)
        code = fill_sql_template(code, params)
        sql = FugueSQLParser(code, 'fugueLanguage', ignore_case=self.conf.get_or_throw(FUGUE_CONF_SQL_IGNORE_CASE, bool), parse_mode='auto')
        v = _Extensions(sql, FugueSQLHooks(), self, dialect=self.conf.get_or_throw(FUGUE_CONF_SQL_DIALECT, str), variables=dfs, local_vars=params)
        v.visit(sql.tree)
        return v.variables
