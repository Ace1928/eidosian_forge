...
def switch_controller_switch_profile(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('switch-controller', 'switch-profile', filtered_data, vdom=vdom)
        current_data = fos.get('switch-controller', 'switch-profile', vdom=vdom, mkey=mkey)
    ...
    if state == 'present' or state is True:
        return fos.set('switch-controller', 'switch-profile', data=filtered_data, vdom=vdom)
    elif state == 'absent':
        return fos.delete('switch-controller', 'switch-profile', mkey=filtered_data['name'], vdom=vdom)
    else:
