...
class ExportLogs(base_classes.BaseCommand):
  ...
  """
    ...
    def Args(cls, parser):
        ...
        parser.add_argument('--collect-process-traces', action='store_true', help='Collect a 10 minute trace of the running system. On Windows, this utilizes Windows Performance Recorder. It records CPU, disk, file, and network activity during that time.')
    ...
    def Run(self, args):
        ...
        url = self._CreateResumableSignedUrl(service_account, expiration_time, bucket, log_path)
        diagnostics_entry = self._ConstructDiagnosticsKeyEntry(url, args.collect_process_traces)
        self._diagnose_client.UpdateMetadata(project, instance_ref, _DIAGNOSTICS_METADATA_KEY, diagnostics_entry)
    ...
    """
        ...
        instance_ref = instance_flags.INSTANCE_ARG.ResolveAsResource(args, resources, scope_lister=instance_flags.GetInstanceZoneScopeLister(compute_client))
        return instance_ref
