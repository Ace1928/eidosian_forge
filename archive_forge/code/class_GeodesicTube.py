...
class GeodesicTube:
    ...
    def __init__(self, mcomplex: Mcomplex, geodesic: GeodesicInfo):
        ...
        if geodesic.line is None:
            raise ValueError('GeodesicTube expected GeodesicInfo with line set to start developing a tube about the geodesic.')
        if not geodesic.lifted_tetrahedra:
            raise ValueError('GeodesicTube expected GeodesicInfo with lifted_tetrahedra set to start developing a tube about the geodesic.')
        ...
        for lifted_tetrahedron in geodesic.lifted_tetrahedra:
            heapq.heappush(self._pending_pieces, _PendingPiece(lifted_tetrahedron, mcomplex.RF(0)))
        self._visited_lifted_tetrahedra = ZQuotientLiftedTetrahedronSet(mcomplex, balance_end_points_of_line(geodesic.line, geodesic.unnormalised_start_point))
