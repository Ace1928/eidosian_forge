...
class NFSNDNSDriver(DNSDriver):
    ...
    RECORD_TYPE_MAP = {RecordType.A: 'A', RecordType.AAAA: 'AAAA', RecordType.CNAME: 'CNAME', RecordType.MX: 'MX', RecordType.NS: 'NS', RecordType.SRV: 'SRV', RecordType.TXT: 'TXT', RecordType.PTR: 'PTR'}
    ...
    def create_record(self, name, zone, type, data, extra=None):
        ...
        try:
            ...
            exists_re = re.compile('That RR already exists on the domain')
    ...
    def _to_record(self, item, zone):
        ttl = int(item['ttl'])
        return Record(id=None, name=item['name'], data=item['data'], type=item['type'], zone=zone, driver=self, ttl=ttl)
