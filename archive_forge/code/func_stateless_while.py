...
def stateless_while(input, cond, body, output_shapes=[], parallel_iterations: int=10, name=None):
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'StatelessWhile', name, input, 'cond', cond, 'body', body, 'output_shapes', output_shapes, 'parallel_iterations', parallel_iterations)
            return _result
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('StatelessWhile', input=input, cond=cond, body=body, output_shapes=output_shapes, parallel_iterations=parallel_iterations, name=name)
    _result = _outputs[:]
