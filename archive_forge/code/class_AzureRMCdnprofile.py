...
class AzureRMCdnprofile(AzureRMModuleBase):
    ...
    def create_cdnprofile(self):
        ...
        parameters = Profile(location=self.location, sku=Sku(name=self.sku), tags=self.tags)
        xid = str(uuid.uuid1())
        try:
            poller = self.cdn_client.profiles.begin_create(self.resource_group, self.name, parameters)
