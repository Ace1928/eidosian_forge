...
class RadiusNeighborsClassifier(RadiusNeighborsMixin, ClassifierMixin, NeighborsBase):
    ...
    """
    _parameter_constraints: dict = {**NeighborsBase._parameter_constraints, 'weights': [StrOptions({'uniform', 'distance'}), callable, None], 'outlier_label': [Integral, str, 'array-like', None]}
    ...
    def predict_proba(self, X):
        ...
        if self.weights == 'uniform' and self._fit_method == 'brute' and (not self.outputs_2d_) and RadiusNeighborsClassMode.is_usable_for(X, self._fit_X, metric):
            probabilities = RadiusNeighborsClassMode.compute(X=X, Y=self._fit_X, radius=self.radius, weights=self.weights, Y_labels=self._y, unique_Y_labels=self.classes_, outlier_label=self.outlier_label, metric=metric, metric_kwargs=metric_kwargs, strategy='parallel_on_X')
