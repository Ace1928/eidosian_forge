...
class ORTEncoderForPix2Struct(ORTEncoder):
    ...
    def forward(self, flattened_patches: torch.FloatTensor, attention_mask: torch.LongTensor, **kwargs) -> BaseModelOutput:
        ...
        if self.parent_model.device.type == 'cuda' and self.parent_model.use_io_binding:
            model_inputs = [flattened_patches, attention_mask] if 'attention_mask' in self.input_names else [flattened_patches]
