...
def generate_mcmc_like_dataset(seed=None):
    ...
    ds = xr.Dataset({'x_plot': (['plot_dim'], np.linspace(0, 10, 20)), 'mu': (['chain', 'draw', 'team'], rng.exponential(size=(4, 10, 6))), 'sigma': (['chain', 'draw'], rng.exponential(size=(4, 10))), 'score': (['chain', 'draw', 'match'], rng.poisson(size=(4, 10, 12)))}, coords={'team': list('abcdef'), 'chain': np.arange(4), 'draw': np.arange(10)})
    return ds