...
class NumpyTrainer(base_trainer.Trainer):
    ...
    def _symbolic_build(self, data_batch):
        ...
        compile_metrics_unbuilt = self._compile_metrics is not None and (not self._compile_metrics.built)
        if model_unbuilt or compile_metrics_unbuilt:
            ...
            x, y, sample_weight = data_adapter_utils.unpack_x_y_sample_weight(data_batch)
            try:
                y_pred = backend.compute_output_spec(self, x)
            except:
                raise RuntimeError("Unable to automatically build the model. Please build it yourself before calling fit/evaluate/predict. A model is 'built' when its variables have been created and its `self.built` attribute is True. Usually, calling the model on a batch of data is the right way to build it.")
            if compile_metrics_unbuilt:
                backend.compute_output_spec(self.compute_metrics, x, y, y_pred, sample_weight=sample_weight)
        self._post_build()
    ...
    def predict(self, x, batch_size=None, verbose='auto', steps=None, callbacks=None):
        epoch_iterator = EpochIterator(x=x, batch_size=batch_size, steps_per_epoch=steps, shuffle=False, steps_per_execution=self.steps_per_execution)
    ...
    def evaluate(self, x=None, y=None, batch_size=None, verbose='auto', sample_weight=None, steps=None, callbacks=None, return_dict=False, **kwargs):
        ...
        if use_cached_eval_dataset:
            ...
            epoch_iterator = EpochIterator(x=x, y=y, sample_weight=sample_weight, batch_size=batch_size, steps_per_epoch=steps, shuffle=False, steps_per_execution=self.steps_per_execution)
