...
class GameEnvironmentInitializer(ShowBase):
    ...
    def enablePlayerMovement(self):
        self.accept("arrow_up", self.applyMovement, [Vec3(0, 50, 0)])
        self.accept("arrow_down", self.applyMovement, [Vec3(0, -50, 0)])
        self.accept("arrow_left", self.applyMovement, [Vec3(-50, 0, 0)])
        self.accept("arrow_right", self.applyMovement, [Vec3(50, 0, 0)])
    ...
    def controlCamera(self, task):
        if self.mouseWatcherNode.hasMouse():
            ...
            self.camera.setP(
                self.camera.getP() - (mpos.getY() * 30)
            )  # Adjust pitch based on vertical mouse movement
            self.camera.setH(
                self.camera.getH() - (mpos.getX() * 30)
            )  # Adjust heading based on horizontal mouse movement
            self.mouseWatcherNode.setMouse(0, 0)  # Reset mouse position to center
        return task.cont
