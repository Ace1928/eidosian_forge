...
class BatchReslicer(Generic[T]):
    ...
    def __init__(self, row_limit: Optional[int]=None, size_limit: Any=None) -> None:
    ...
    def reslice(self, batches: Iterable[T]) -> Iterable[T]:
        ...
        for batch in batches:
            ...
            if self._row_limit > 0 and self._size_limit <= 0:
                ...
                row_limit = min(self._row_limit, max(math.floor(self._size_limit / row_size), 1))
            if cache_rows >= row_limit:
            ...
            if cache_rows + batch_rows < row_limit:
                ...
                slices, remain = self._slice_rows(batch_rows, row_limit - cache_rows, slice_rows=row_limit)
