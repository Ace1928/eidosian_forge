...
class BaseIO:
    ...
    def from_ray(cls, ray_obj):
        ...
        """
        raise RuntimeError('Modin Dataframe can only be converted to a Ray Dataset if Modin uses a Ray engine.')
    ...
    def from_dask(cls, dask_obj):
        ...
        If another engine is used, the runtime exception will be raised.
        """
        raise RuntimeError('Modin DataFrame can only be converted to a Dask DataFrame if Modin uses a Dask engine.')
    ...
    def read_gbq(cls, query: str, project_id=None, index_col=None, col_order=None, reauth=False, auth_local_webserver=False, dialect=None, location=None, configuration=None, credentials=None, use_bqstorage_api=None, private_key=None, verbose=None, progress_bar_type=None, max_results=None):
        ErrorMessage.default_to_pandas('`read_gbq`')
        return cls.from_pandas(pandas.read_gbq(query, project_id=project_id, index_col=index_col, col_order=col_order, reauth=reauth, auth_local_webserver=auth_local_webserver, dialect=dialect, location=location, configuration=configuration, credentials=credentials, use_bqstorage_api=use_bqstorage_api, progress_bar_type=progress_bar_type, max_results=max_results))
    ...
    def read_html(cls, io, *, match='.+', flavor=None, header=None, index_col=None, skiprows=None, attrs=None, parse_dates=False, thousands=',', encoding=None, decimal='.', converters=None, na_values=None, keep_default_na=True, displayed_only=True, **kwargs):
        ErrorMessage.default_to_pandas('`read_html`')
        result = pandas.read_html(io=io, match=match, flavor=flavor, header=header, index_col=index_col, skiprows=skiprows, attrs=attrs, parse_dates=parse_dates, thousands=thousands, encoding=encoding, decimal=decimal, converters=converters, na_values=na_values, keep_default_na=keep_default_na, displayed_only=displayed_only, **kwargs)
    ...
    def read_clipboard(cls, sep='\\s+', **kwargs):
    ...
    def read_hdf(cls, path_or_buf, key=None, mode: str='r', errors: str='strict', where=None, start=None, stop=None, columns=None, iterator=False, chunksize=None, **kwargs):
        ...
        df = pandas.read_hdf(path_or_buf, key=key, mode=mode, columns=columns, errors=errors, where=where, start=start, stop=stop, iterator=iterator, chunksize=chunksize, **kwargs)
        if modin_store:
    ...
    def read_feather(cls, path, **kwargs):
    ...
    def read_stata(cls, filepath_or_buffer, **kwargs):
    ...
    def read_sas(cls, filepath_or_buffer, *, format=None, index=None, encoding=None, chunksize=None, iterator=False, **kwargs):
        ErrorMessage.default_to_pandas('`read_sas`')
        return cls.from_pandas(pandas.read_sas(filepath_or_buffer, format=format, index=index, encoding=encoding, chunksize=chunksize, iterator=iterator, **kwargs))
    ...
    def read_pickle(cls, filepath_or_buffer, **kwargs):
    ...
    def read_sql(cls, sql, con, index_col=None, coerce_float=True, params=None, parse_dates=None, columns=None, chunksize=None, dtype_backend=no_default, dtype=None):
        ErrorMessage.default_to_pandas('`read_sql`')
        if isinstance(con, ModinDatabaseConnection):
            con = con.get_connection()
        result = pandas.read_sql(sql, con, index_col=index_col, coerce_float=coerce_float, params=params, parse_dates=parse_dates, columns=columns, chunksize=chunksize, dtype_backend=dtype_backend, dtype=dtype)
    ...
    def read_fwf(cls, filepath_or_buffer, *, colspecs='infer', widths=None, infer_nrows=100, dtype_backend=no_default, iterator=False, chunksize=None, **kwds):
        ErrorMessage.default_to_pandas('`read_fwf`')
        pd_obj = pandas.read_fwf(filepath_or_buffer, colspecs=colspecs, widths=widths, infer_nrows=infer_nrows, dtype_backend=dtype_backend, iterator=iterator, chunksize=chunksize, **kwds)
        if isinstance(pd_obj, pandas.DataFrame):
            return cls.from_pandas(pd_obj)
        if isinstance(pd_obj, pandas.io.parsers.TextFileReader):
            pd_read = pd_obj.read
    ...
    def read_sql_table(cls, table_name, con, schema=None, index_col=None, coerce_float=True, parse_dates=None, columns=None, chunksize=None, dtype_backend=no_default):
        ErrorMessage.default_to_pandas('`read_sql_table`')
        return cls.from_pandas(pandas.read_sql_table(table_name, con, schema=schema, index_col=index_col, coerce_float=coerce_float, parse_dates=parse_dates, columns=columns, chunksize=chunksize, dtype_backend=dtype_backend))
    ...
    def read_spss(cls, path, usecols, convert_categoricals, dtype_backend):
        ...
        return cls.from_pandas(pandas.read_spss(path, usecols=usecols, convert_categoricals=convert_categoricals, dtype_backend=dtype_backend))

    @classmethod
    @_inherit_docstrings(pandas.DataFrame.to_sql, apilink='pandas.DataFrame.to_sql')
    def to_sql(cls, qc, name, con, schema=None, if_exists='fail', index=True, index_label=None, chunksize=None, dtype=None, method=None):
        """
        ...
        df = qc.to_pandas()
        df.to_sql(name=name, con=con, schema=schema, if_exists=if_exists, index=index, index_label=index_label, chunksize=chunksize, dtype=dtype, method=method)
