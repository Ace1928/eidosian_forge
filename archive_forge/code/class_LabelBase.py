...
class LabelBase(object):
    ...
    def __init__(self, text='', font_size=12, font_name=DEFAULT_FONT, bold=False, italic=False, underline=False, strikethrough=False, font_family=None, halign='left', valign='bottom', shorten=False, text_size=None, mipmap=False, color=None, line_height=1.0, strip=False, strip_reflow=True, shorten_from='center', split_str=' ', unicode_errors='replace', font_hinting='normal', font_kerning=True, font_blended=True, outline_width=None, outline_color=None, font_context=None, font_features=None, base_direction=None, font_direction='ltr', font_script_name='Latin', text_language=None, limit_render_to_text_bbox=False, **kwargs):
        ...
        if isinstance(options['padding'], (int, float)):
            ...
            if len(options['padding']) == 1:
                ...
                raise ValueError(f'padding should be int|float or a list|tuple with 1, 2 or 4 elements, got {type(options['padding'])} with {len(options['padding'])} elements.')
        ...
        for padding_option in ('padding_x', 'padding_y'):
            if kwargs_get(padding_option):
                Logger.warning(f'LabelBase: The use of the {padding_option} parameter is deprecated, and will be removed in future versions. Use padding instead.')
        if 'size' in kwargs:
    ...
    def render(self, real=False):
        ...
        options['strip'] = strip = options['strip'] or options['halign'] == 'justify'
        ...
        if not text:
            return (0, 0)
        if uh is not None and (options['valign'] == 'middle' or options['valign'] == 'center'):
            center = -1
            if len(text) > 1:
                ...
                if l != -1 and r != -1:
                    ...
                    center = r
            if center != -1:
                w, h, clipped = layout_text(text[center + 1:], lines, (0, 0), (uw, uh / 2), options, self.get_cached_extents(), True, True)
                w, h, clipped = layout_text(text[:center + 1], lines, (w, h), (uw, uh), options, self.get_cached_extents(), False, True)
            else:
                w, h, clipped = layout_text(text, lines, (0, 0), (uw, None), options, self.get_cached_extents(), True, True)
        else:
            w, h, clipped = layout_text(text, lines, (0, 0), (uw, uh), options, self.get_cached_extents(), options['valign'] == 'top', True)
    ...
    def refresh(self):
        ...
        if texture is None or width != texture.width or height != texture.height:
            texture = Texture.create(size=(width, height), mipmap=self.options['mipmap'], callback=self._texture_fill)
    ...
    def content_width(self):
        ...
        if self.texture is None:
            return 0
        return self.texture.width - (self.options['padding'][0] + self.options['padding'][2])
    ...
    def content_height(self):
        ...
        if self.texture is None:
            return 0
        return self.texture.height - (self.options['padding'][1] + self.options['padding'][3])
    ...
    def fontid(self):
        ...
        return str([self.options[x] for x in ('font_size', 'font_name_r', 'bold', 'italic', 'underline', 'strikethrough')])
    ...
    def _set_text_size(self, x):
        self._text_size = x
    text_size = property(_get_text_size, _set_text_size, doc="Get/set the (width, height) of the '\n                         'contrained rendering box")
