...
class PDLP(ConicSolver):
    ...
    def solve_via_data(self, data: Dict[str, Any], warm_start: bool, verbose: bool, solver_opts: Dict[str, Any], solver_cache: Dict=None) -> Solution:
        ...
        request = linear_solver_pb2.MPModelRequest(model=data[self.MODEL_PROTO], enable_internal_solver_output=verbose, solver_type=pdlp_solver)
        ...
        if Version(ortools.__version__) < Version('9.7.0'):
            from ortools.linear_solver.python import pywrap_model_builder_helper as model_builder_helper
        else:
