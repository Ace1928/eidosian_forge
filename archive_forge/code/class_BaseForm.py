...
class BaseForm(RenderableFormMixin):
    ...
    def __repr__(self):
        ...
        return '<%(cls)s bound=%(bound)s, valid=%(valid)s, fields=(%(fields)s)>' % {'cls': self.__class__.__name__, 'bound': self.is_bound, 'valid': is_valid, 'fields': ';'.join(self.fields)}
    ...
    def get_context(self):
        ...
        for name, bf in self._bound_items():
            ...
            if bf.is_hidden:
                if bf_errors:
                    top_errors += [_('(Hidden field %(name)s) %(error)s') % {'name': name, 'error': str(e)} for e in bf_errors]
                hidden_fields.append(bf)
            else:
                errors_str = str(bf_errors)
                fields.append((bf, errors_str))
        return {'form': self, 'fields': fields, 'hidden_fields': hidden_fields, 'errors': top_errors}
    ...
    def has_error(self, field, code=None):
        return field in self.errors and (code is None or any((error.code == code for error in self.errors.as_data()[field])))
