...
class OutputMessage(object):
  ...
  """
    ...
    def __init__(self, timestamp, body=None, version=SCHEMA_VERSION, resource_body=None, error_details=None, as_json=False, level=None):
        if (body or resource_body) and error_details or (not body and (not resource_body) and (not error_details)):
            raise InvalidMessageError('Invalid OutputMessage: Must contain one and only one of: body (+resource_body) or error_details: body=[{}], resource_body=[{}], error_details=[{}]'.format(body, resource_body, error_details))
        self._body = body
        ...
        if error_details:
            if not isinstance(error_details, dict):
                raise InvalidMessageError('ErrorDetails must be a dict object:[{}]'.format(error_details))
            err = self.ErrorDetails(error_details.get('error'), error_details.get('context'), as_json, level)
        else:
            err = None
        self._err = err
    ...
    def AsDict(self):
        ...
        out = collections.OrderedDict(version=self.version, timestamp=self.timestamp)
        if self.body:
            out['body'] = self.body
    ...
    def __eq__(self, other):
        if not isinstance(other, OutputMessage):
            return False
        return self.error_details == other.error_details and self.body == other.body and (self.resource_body == other.resource_body) and (self.version == other.version) and (self.timestamp == other.timestamp)
    ...
    def FromString(cls, input_str, as_json=False):
        ...
        return cls(body=yaml_msg.get('body'), resource_body=yaml_msg.get('resource_body'), error_details=yaml_msg.get('error_details'), version=yaml_msg.get('version'), timestamp=yaml_msg.get('timestamp'), level=yaml_msg.get('level'), as_json=as_json)