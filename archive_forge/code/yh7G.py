...
class PhaserEffect(SoundModule):
    ...
    def __init__(
        self, rate: float = 1.0, depth: float = 0.5, feedback: float = 0.5
    ) -> None:
        """
...
class Equalizer(SoundModule):
    ...
    def process_sound(
        self, sound: np.ndarray, frequency_bands: dict = {}
    ) -> np.ndarray:
        """
...
class DistortionEffect(SoundModule):
    ...
    def process_sound(
        self, sound: np.ndarray, drive: float = 0.5, tone: float = 0.0
    ) -> np.ndarray:
        """
...
class StereoPanning(SoundModule):
    ...
    def process_sound(
        self, sound: np.ndarray, new_rate: int = SAMPLE_RATE
    ) -> np.ndarray:
        """
    ...
    def process_sound(
        self, sound: np.ndarray = np.array([]), new_depth: int = 16
    ) -> np.ndarray:
        """
    ...
    def validate_parameters(
        self, parameters: Dict[str, Any] = {"new_depth": 16}
    ) -> None:
        """
        Validates the parameters provided to the module to ensure they meet expected criteria.
        This method can be overridden by subclasses to implement module-specific validation logic.
...
class FormantAdjustment(SoundModule):
    ...
    def __init__(self, default_formant_shifts: dict = None) -> None:
        ...
        if default_formant_shifts is None:
            default_formant_shifts = {
                1: 0,
                2: 0,
                3: 0,
            }  # Default shifts for three formants
        self.default_formant_shifts = default_formant_shifts

    def process_sound(
        self, sound: np.ndarray = np.array([]), formant_shifts: dict = None
    ) -> np.ndarray:
        """
