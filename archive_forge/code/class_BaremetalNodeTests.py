...
class BaremetalNodeTests(base.TestCase):
    ...
    def test_create_name_uuid(self):
        ...
        node_info = self.node_create(name=name, params='--uuid {0}'.format(uuid))
    ...
    def test_set(self, key):
        ...
        self.openstack('baremetal node set --name {0} {1}'.format(new_name, self.node[key]))
    ...
    def test_unset(self, key):
        ...
        """
        self.openstack('baremetal node unset --name {0}'.format(self.node[key]))
        show_prop = self.node_show(self.node['uuid'], ['name'])
    ...
    def test_show(self, key):
        ...
        """
        node = self.node_show(self.node[key], ['name', 'uuid', 'driver'])
