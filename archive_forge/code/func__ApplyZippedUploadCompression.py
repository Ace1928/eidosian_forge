  ...
  """
    ...
    try:
        if src_url.IsStream() or src_url.IsFifo():
            raise CommandException('gzip compression is not currently supported on streaming uploads. Remove the compression flag or save the streamed output temporarily to a file before uploading.')
        if src_obj_size is not None and CheckFreeSpace(gzip_path) < 2 * int(src_obj_size):
            raise CommandException('Inadequate temp space available to compress %s. See the CHANGING TEMP DIRECTORIES section of "gsutil help cp" for more info.' % src_url)
        compression_level = config.getint('GSUtil', 'gzip_compression_level', DEFAULT_GZIP_COMPRESSION_LEVEL)
