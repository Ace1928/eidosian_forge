...
class Vrrp(object):
    ...
    def get_virtual_ip_info(self):
        ...
        if '<data/>' in xml_str:
            ...
            vrrp_virtual_ip_infos = root.findall('vrrp/vrrpGroups/vrrpGroup/virtualIps/virtualIp')
    ...
    def get_vrrp_global_info(self):
        ...
        if '<data/>' in xml_str:
            ...
            xml_str = xml_str.replace('\r', '').replace('\n', '').replace('xmlns="urn:ietf:params:xml:ns:netconf:base:1.0"', '').replace('xmlns="http://www.huawei.com/netconf/vrp"', '')
            root = ElementTree.fromstring(xml_str)
            global_info = root.findall('vrrp/vrrpGlobalCfg')
            if global_info:
    ...
    def create_virtual_ip(self):
        ...
        if self.is_virtual_ip_change():
            conf_str = CE_NC_CREATE_VRRP_VIRTUAL_IP_INFO % (self.vrid, self.interface, self.virtual_ip)
            recv_xml = set_nc_config(self.module, conf_str)
            if '<ok/>' not in recv_xml:
                self.module.fail_json(msg='Error: create virtual ip info failed.')
            self.updates_cmd.append('interface %s' % self.interface)
            self.updates_cmd.append('vrrp vrid %s virtual-ip %s' % (self.vrid, self.virtual_ip))
            self.changed = True
    ...
    def set_vrrp_group(self):
        ...
        if self.is_vrrp_group_info_change():
            ...
            recv_xml = set_nc_config(self.module, conf_str)
            if '<ok/>' not in recv_xml:
                self.module.fail_json(msg='Error: set vrrp group attribute info failed.')
    ...
    def get_proposed(self):
        ...
        if self.gratuitous_arp_interval:
            self.proposed['gratuitous_arp_interval'] = self.gratuitous_arp_interval
        if self.recover_delay:
    ...
    def get_existing(self):
        ...
        if self.vrrp_group_info:
            ...
            if self.vrrp_type == 'admin':
                self.existing['admin_ignore_if_down'] = self.vrrp_group_info['adminIgnoreIfDown']
            if self.admin_vrid and self.admin_interface:
    ...
    def get_end_state(self):
        ...
        if self.vrrp_group_info:
            ...
            if self.vrrp_type == 'admin':
                self.end_state['admin_ignore_if_down'] = self.vrrp_group_info['adminIgnoreIfDown']
            if self.admin_vrid and self.admin_interface:
                self.end_state['admin_vrid'] = self.vrrp_group_info['adminVrrpId']
