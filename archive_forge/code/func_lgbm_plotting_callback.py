...
def lgbm_plotting_callback():
    ...
    def _init(env):
        ...
        nonlocal plotter
        plotter = MetricsPlotter(train_metrics, test_metrics, env.end_iteration - env.begin_iteration)
    ...
    def _callback(env):
        ...
        for item in env.evaluation_result_list:
            ...
            if 'train' in data_name.lower():
                ...
                raise Exception('Unexpected sample name during evaluation')
        plotter.log(env.iteration - env.begin_iteration, train=True, metrics=metrics['train'])
        plotter.log(env.iteration - env.begin_iteration, train=False, metrics=metrics['test'])
    _callback.order = 20
    return _callback