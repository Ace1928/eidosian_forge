...
def dense_bincount(input: _atypes.TensorFuzzingAnnotation[TV_DenseBincount_Tidx], size: _atypes.TensorFuzzingAnnotation[TV_DenseBincount_Tidx], weights: _atypes.TensorFuzzingAnnotation[TV_DenseBincount_T], binary_output: bool=False, name=None) -> _atypes.TensorFuzzingAnnotation[TV_DenseBincount_T]:
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'DenseBincount', name, input, size, weights, 'binary_output', binary_output)
            return _result
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('DenseBincount', input=input, size=size, weights=weights, binary_output=binary_output, name=name)
    _result = _outputs[:]
