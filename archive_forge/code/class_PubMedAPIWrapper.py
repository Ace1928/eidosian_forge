...
class PubMedAPIWrapper(BaseModel):
    ...
    def run(self, query: str) -> str:
        ...
        """
        try:
            docs = [f'Published: {result['Published']}\nTitle: {result['Title']}\nCopyright Information: {result['Copyright Information']}\nSummary::\n{result['Summary']}' for result in self.load(query[:self.MAX_QUERY_LENGTH])]
    ...
    def lazy_load(self, query: str) -> Iterator[dict]:
        ...
        """
        url = self.base_url_esearch + 'db=pubmed&term=' + str({urllib.parse.quote(query)}) + f'&retmode=json&retmax={self.top_k_results}&usehistory=y'
