...
def get_plotnine_all(use_clipboard=True) -> Optional[str]:
    ...
    modules = ('coords', 'facets', 'geoms', 'ggplot', 'guides', 'labels', 'mapping', 'positions', 'qplot', 'scales', 'stats', 'themes', 'themes.elements', 'watermark')
    comma_join = ',\n'.join
    ...
    lst = indent(comma_join((get_all_from_module(name) for name in modules)), ' ' * 4)
    _all = f'__all__ = (\n{lst},\n)'
    content = f'{_imports}\n\n{_all}'
    if use_clipboard:
