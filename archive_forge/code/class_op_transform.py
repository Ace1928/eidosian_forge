    ...
    **Example**
    ...
    def __new__(cls, *args, **kwargs):
        if os.environ.get('SPHINX_BUILD') == '1':
            warnings.warn("Operator transformations have been disabled, as a Sphinx build has been detected via SPHINX_BUILD='1'. If this is not the case, please set the environment variable SPHINX_BUILD='0'.", UserWarning)
    ...
    def __init__(self, fn):
        if not callable(fn):
            raise OperationTransformError(f'The operator function to register, {fn}, does not appear to be a valid Python function or callable.')
        warnings.warn('Use of `op_transform` to create a custom transform is deprecated. Instead switch to using the new qml.transform function. Follow the instructions here for further details: https://docs.pennylane.ai/en/stable/code/qml_transforms.html#custom-transforms.', qml.PennyLaneDeprecationWarning)
        self._fn = fn
    ...
    def fn(self, obj, *args, **kwargs):
        ...
        try:
            ...
            except (AttributeError, qml.operation.OperatorPropertyUndefined, OperationTransformError) as e:
                raise e1 from e
