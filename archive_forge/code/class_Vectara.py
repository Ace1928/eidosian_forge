...
class Vectara(VectorStore):
    ...
    def __init__(self, vectara_customer_id: Optional[str]=None, vectara_corpus_id: Optional[str]=None, vectara_api_key: Optional[str]=None, vectara_api_timeout: int=120, source: str='langchain'):
    ...
    def _delete_doc(self, doc_id: str) -> bool:
        ...
        response = self._session.post('https://api.vectara.io/v1/delete-doc', data=json.dumps(body), verify=True, headers=self._get_post_headers(), timeout=self.vectara_api_timeout)
    ...
    def add_files(self, files_list: Iterable[str], metadatas: Optional[List[dict]]=None, **kwargs: Any) -> List[str]:
        """
        ...
        for inx, file in enumerate(files_list):
            ...
            response = self._session.post(f'https://api.vectara.io/upload?c={self._vectara_customer_id}&o={self._vectara_corpus_id}&d=True', files=files, verify=True, headers=headers, timeout=self.vectara_api_timeout)
    ...
    def add_texts(self, texts: Iterable[str], metadatas: Optional[List[dict]]=None, doc_metadata: Optional[dict]=None, **kwargs: Any) -> List[str]:
        ...
        doc = {'document_id': doc_id, 'metadataJson': json.dumps(doc_metadata), section_key: [{'text': text, 'metadataJson': json.dumps(md)} for text, md in zip(texts, metadatas)]}
        ...
        if success_str == 'E_ALREADY_EXISTS':
            ...
            print('No permissions to add document to Vectara. \n                Check your corpus ID, customer ID and API key')
        return [doc_id]
    ...
    def max_marginal_relevance_search(self, query: str, fetch_k: int=50, lambda_mult: float=0.5, **kwargs: Any) -> List[Document]:
        ...
        """
        kwargs['mmr_config'] = MMRConfig(is_enabled=True, mmr_k=fetch_k, diversity_bias=1 - lambda_mult)
        return self.similarity_search(query, **kwargs)

    @classmethod
    def from_texts(cls: Type[Vectara], texts: List[str], embedding: Optional[Embeddings]=None, metadatas: Optional[List[dict]]=None, **kwargs: Any) -> Vectara:
