...
def clifford_optimized_circuit(circuit: circuits.Circuit, atol: float=1e-08) -> circuits.Circuit:
    c_cliff = transformers.optimize_for_target_gateset(circuit, gateset=CliffordTargetGateset(atol=atol))
    ...
    def try_merge_cz(cz_op: ops.GateOperation, start_i: int) -> int:
        ...
        for i in reversed(range(start_i)):
            ...
            if not set(cz_op.qubits) & set(op.qubits):
                continue
            elif not (isinstance(op, ops.GateOperation) and isinstance(op.gate, ops.CZPowGate) and (op.gate.exponent == 1)):
                return 0
            elif cz_op == op:
                all_ops.pop(start_i)
                all_ops.pop(i)
    ...
    while i < len(all_ops):
        ...
        if isinstance(op, ops.GateOperation) and isinstance(op.gate, ops.SingleQubitCliffordGate):
            if try_merge_clifford(op, i):
                i -= 1
        elif isinstance(op, ops.GateOperation) and isinstance(op.gate, ops.CZPowGate) and (op.gate.exponent == 1):
