...
def _exec_drop_col_fk_constraint(element: _ExecDropFKConstraint, compiler: MSSQLCompiler, **kw) -> str:
    ...
    return "declare @const_name varchar(256)\nselect @const_name = QUOTENAME([name]) from\nsys.foreign_keys fk join sys.foreign_key_columns fkc\non fk.object_id=fkc.constraint_object_id\nwhere fkc.parent_object_id = object_id('%(schema_dot)s%(tname)s')\nand col_name(fkc.parent_object_id, fkc.parent_column_id) = '%(colname)s'\nexec('alter table %(tname_quoted)s drop constraint ' + @const_name)" % {'tname': tname, 'colname': colname, 'tname_quoted': format_table_name(compiler, tname, schema), 'schema_dot': schema + '.' if schema else ''}