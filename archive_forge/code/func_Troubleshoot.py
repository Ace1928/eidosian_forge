...
def Troubleshoot(client, instance_ref, release_track, analyze_logs=False):
    ...
    log.Print('OS Config troubleshooter tool is checking if there are issues with the VM Manager setup for this VM instance.\n')
    service_enablement_response = service_enablement.Check(instance_ref, release_track)
    ...
    try:
        ...
        exception = e
    metadata_setup_response = metadata_setup.Check(project, instance, release_track, exception)
    log.Print(metadata_setup_response.response_message)
    if not metadata_setup_response.continue_flag:
        return
    agent_freshness_response = agent_freshness.Check(project, instance, instance_ref.zone, release_track)
    ...
    if analyze_logs:
        ...
        log_analysis.CheckSerialLogOutput(client, project, instance, instance_ref.zone)
