...
class AdaScale(Optimizer):
    ...
    def __init__(self, optimizer: torch.optim.Optimizer, world_size: Optional[int]=None, scale: Optional[float]=None, smoothing: float=None, num_gradients_to_accumulate: int=1, is_scaled_loss: bool=True, debias_ewma: bool=True):
        self._hook_handles: List[Any] = []
        self._optimizer = optimizer
        self._local_grad_sqr: Optional[torch.Tensor] = None
        self._world_size: int = world_size if world_size is not None else dist.get_world_size() if dist.is_initialized() else 1
        ...
        self._optimizer.state.setdefault('adascale', {'grad_sqr_avg': np.ones(len(optimizer.param_groups)), 'grad_var_avg': np.zeros(len(optimizer.param_groups))})
        self._scale = 1.0
    ...
    def set_scale(self, scale: float, update_estimate: bool=True) -> None:
        ...
        if update_estimate and hasattr(self, '_scale'):
            ...
            elif 'grad_var_avg_total' in self._state:
    ...
    def _backward_hook(self, pg_idx: int, grad: torch.Tensor) -> None:
        if self._local_grad_sqr is None:
            self._local_grad_sqr = torch.zeros(len(self._optimizer.param_groups), device=grad.device, requires_grad=False)
