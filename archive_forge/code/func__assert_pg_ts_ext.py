...
def _assert_pg_ts_ext(self, element, **kw):
    if not isinstance(element, _regconfig_fn):
        raise exc.CompileError(f'''Can't compile "{element.name}()" full text search function construct that does not originate from the "sqlalchemy.dialects.postgresql" package.  Please ensure "import sqlalchemy.dialects.postgresql" is called before constructing "sqlalchemy.func.{element.name}()" to ensure registration of the correct argument and return types.''')
