...
class TorchVisionPreprocessor(Preprocessor):
    ...
    def __init__(self, columns: List[str], transform: Callable[[Union['np.ndarray', 'torch.Tensor']], 'torch.Tensor'], output_columns: Optional[List[str]]=None, batched: bool=False):
        if not output_columns:
            output_columns = columns
        if len(columns) != len(output_columns):
            raise ValueError(f'The length of columns should match the length of output_columns: {columns} vs {output_columns}.')
    ...
    def __repr__(self) -> str:
        return f'{self.__class__.__name__}(columns={self._columns}, output_columns={self._output_columns}, transform={self._torchvision_transform!r})'

    def _transform_numpy(self, data_batch: Dict[str, 'np.ndarray']) -> Dict[str, 'np.ndarray']:
        import torch
        ...
        def transform_batch(batch: np.ndarray) -> np.ndarray:
            ...
            return _create_possibly_ragged_ndarray([apply_torchvision_transform(array) for array in batch])
