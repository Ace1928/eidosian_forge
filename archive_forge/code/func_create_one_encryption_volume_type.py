...
def create_one_encryption_volume_type(attrs=None):
    ...
    """
    attrs = attrs or {}
    encryption_info = {'volume_type_id': 'type-id-' + uuid.uuid4().hex, 'provider': 'LuksEncryptor', 'cipher': None, 'key_size': None, 'control_location': 'front-end'}
    encryption_info.update(attrs)
    encryption_type = fakes.FakeResource(info=copy.deepcopy(encryption_info), loaded=True)
