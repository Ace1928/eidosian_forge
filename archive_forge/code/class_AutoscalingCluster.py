...
class AutoscalingCluster:
    ...
    def start(self, ray_head_ip, ray_head_port, ray_client_server_port, ray_temp_dir, dashboard_options, head_node_options, collect_log_to_path):
        """Start the cluster.
        ...
        ray_head_node_cmd = [sys.executable, '-m', 'ray.util.spark.start_ray_node', f'--temp-dir={ray_temp_dir}', '--block', '--head', f'--node-ip-address={ray_head_ip}', f'--port={ray_head_port}', f'--ray-client-server-port={ray_client_server_port}', f'--autoscaling-config={autoscale_config}', f'--min-worker-port={worker_port_range_begin}', f'--max-worker-port={worker_port_range_end - 1}', *dashboard_options]
