...
class RSAKey(PKey):
    ...
    def sign_ssh_data(self, data, algorithm=None):
        if algorithm is None:
            algorithm = self.name
        sig = self.key.sign(data, padding=padding.PKCS1v15(), algorithm=self.HASHES[algorithm]())
        m = Message()
    ...
    def _decode_key(self, data):
        ...
        if pkformat == self._PRIVATE_KEY_FORMAT_ORIGINAL:
            ...
            key = rsa.RSAPrivateNumbers(p=p, q=q, d=d, dmp1=d % (p - 1), dmq1=d % (q - 1), iqmp=iqmp, public_numbers=public_numbers).private_key(default_backend())
        else:
