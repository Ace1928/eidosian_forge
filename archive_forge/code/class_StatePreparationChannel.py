...
class StatePreparationChannel(raw_types.Gate):
    ...
    @classmethod
    def _from_json_dict_(cls, target_state: np.ndarray, name: str, **kwargs) -> 'StatePreparationChannel':
    ...
    def _circuit_diagram_info_(self, _args: 'cirq.CircuitDiagramInfoArgs') -> 'cirq.CircuitDiagramInfo':
        ...
        symbols = [self._name] if self._num_qubits == 1 else [f'{self._name}[{i + 1}]' for i in range(self._num_qubits)]
