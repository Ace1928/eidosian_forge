...
class DataFrameTests(object):
    ...
    class Tests(TestCase):
        ...
        def df(self, data: Any=None, schema: Any=None) -> Any:
        ...
        def test_drop_columns(self):
            ...
            raises(FugueDataFrameOperationError, lambda: fi.drop_columns(df, ['b']))
            raises(FugueDataFrameOperationError, lambda: fi.drop_columns(df, ['x']))
        ...
        def test_select(self):
            ...
            raises(FugueDataFrameOperationError, lambda: fi.select_columns(df, []))
            raises(FugueDataFrameOperationError, lambda: fi.select_columns(df, ['a']))
            raises(FugueDataFrameOperationError, lambda: fi.select_columns(df, ['a']))
            ...
            df_eq(fi.as_fugue_df(fi.select_columns(df, ['c', 'a'])), [[2, 'a']], 'a:str,c:int')
        ...
        def test_as_array(self):
            for func in [lambda df, *args, **kwargs: fi.as_array(df, *args, **kwargs, type_safe=True), lambda df, *args, **kwargs: list(fi.as_array_iterable(df, *args, **kwargs, type_safe=True))]:
