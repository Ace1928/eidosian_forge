...
class ImageItem(GraphicsObject):
    ...
    **Bases:** :class:`GraphicsObject <pyqtgraph.GraphicsObject>`
    ...
    def __init__(self, image=None, **kargs):
        ...
        self._update_data_transforms(self.axisOrder)
    ...
    def render(self):
        ...
        if self._imageHasNans is None:
            self._imageHasNans = image.dtype.kind == 'f' and self._xp.isnan(image.min())
        qimage = None
        if lut is not None and lut.dtype != self._xp.uint8:
            warnings.warn('Using non-uint8 LUTs is an undocumented accidental feature and may be removed at some point in the future. Please open an issue if you instead believe this to be worthy of protected inclusion in pyqtgraph.', DeprecationWarning, stacklevel=2)
    ...
    def getHistogram(self, bins='auto', step='auto', perChannel=False, targetImageSize=200, targetHistogramSize=500, **kwds):
        """
        ...
        if self.image is None or self.image.size == 0:
            return (None, None)
        if step == 'auto':
            step = (max(1, int(self._xp.ceil(self.image.shape[0] / targetImageSize))), max(1, int(self._xp.ceil(self.image.shape[1] / targetImageSize))))
        ...
        if isinstance(bins, str) and bins == 'auto':
            ...
            if stepData.dtype.kind in 'ui':
                step = int(self._xp.ceil((mx - mn) / 500.0))
                bins = []
                if step > 0.0:
