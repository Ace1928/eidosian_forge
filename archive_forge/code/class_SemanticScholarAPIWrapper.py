...
class SemanticScholarAPIWrapper(BaseModel):
    ...
    returned_fields = ['title', 'abstract', 'venue', 'year', 'paperId', 'citationCount', 'openAccessPdf', 'authors', 'externalIds']
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        try:
            ...
            raise ImportError('Could not import Semanticscholar python package. Please install it with `pip install semanticscholar`.')
        return values
    ...
    def run(self, query: str) -> str:
        ...
        for item in results[:self.top_k_results]:
            ...
            documents.append(f'Published year: {getattr(item, 'year', None)}\nTitle: {getattr(item, 'title', None)}\nAuthors: {authors}\nAstract: {getattr(item, 'abstract', None)}\n')
        if documents:
