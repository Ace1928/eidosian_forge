...
class Vlan(object):
    ...
    def get_vlans_list(self):
        ...
        if '<data/>' in xml_str:
            ...
            vlan_list = re.findall('.*<vlanId>(.*)</vlanId>.*', xml_str)
            return vlan_list
    ...
    def check_params(self):
        ...
        if not self.vlan_id and self.description:
            self.module.fail_json(msg='Error: Vlan description could be set only at one vlan.')
        if not self.vlan_id and self.name:
            self.module.fail_json(msg='Error: Vlan name could be set only at one vlan.')
        if self.vlan_id:
            if not self.vlan_id.isdigit():
                self.module.fail_json(msg='Error: Vlan id is not digit.')
            if int(self.vlan_id) <= 0 or int(self.vlan_id) > 4094:
                self.module.fail_json(msg='Error: Vlan id is not in the range from 1 to 4094.')
        if self.description:
            if len(self.description) > 81 or len(self.description.replace(' ', '')) < 1:
                self.module.fail_json(msg='Error: vlan description is not in the range from 1 to 80.')
        if self.name:
            if len(self.name) > 31 or len(self.name.replace(' ', '')) < 1:
                self.module.fail_json(msg='Error: Vlan name is not in the range from 1 to 31.')
    ...
    def get_proposed(self):
        ...
        if self.vlans_list_change:
            if self.state == 'present':
                ...
                self.results['proposed_vlans_list'] = list(set(proposed_vlans_tmp))
            else:
                self.results['proposed_vlans_list'] = list(set(self.vlans_list_exist) - set(self.vlans_list_change))
            self.results['proposed_vlans_list'].sort()
        else:
            self.results['proposed_vlans_list'] = self.vlans_list_exist
        if self.vlan_id:
            if self.state == 'present':
                self.results['proposed'] = dict(vlan_id=self.vlan_id, name=self.name, description=self.description)
            else:
    ...
    def get_existing(self):
        ...
        if self.vlan_id:
            if self.vlan_attr_exist:
                self.results['existing'] = dict(vlan_id=self.vlan_attr_exist['vlan_id'], name=self.vlan_attr_exist['name'], description=self.vlan_attr_exist['description'])
            else:
    ...
    def get_end_state(self):
        ...
        if self.vlan_id:
            if self.vlan_attr_end:
                self.results['end_state'] = dict(vlan_id=self.vlan_attr_end['vlan_id'], name=self.vlan_attr_end['name'], description=self.vlan_attr_end['description'])
            else:
    ...
    def work(self):
        ...
        if self.vlan_range:
            ...
            if self.state == 'present':
                self.vlans_list_change = list(set(new_vlans_tmp) - set(self.vlans_list_exist))
            else:
                self.vlans_list_change = [val for val in new_vlans_tmp if val in self.vlans_list_exist]
        if self.state == 'present':
            if self.vlan_id:
