...
class MatrixGate(raw_types.Gate):
    ...
    def __init__(self, matrix: np.ndarray, *, name: Optional[str]=None, qid_shape: Optional[Iterable[int]]=None, unitary_check: bool=True, unitary_check_rtol: float=1e-05, unitary_check_atol: float=1e-08) -> None:
        ...
        if qid_shape is None:
            ...
            if 2 ** n != matrix.shape[0]:
                raise ValueError(f'Matrix width ({matrix.shape[0]}) is not a power of 2 and qid_shape is not specified.')
            qid_shape = (2,) * n
        self._matrix = matrix
        ...
        if self._matrix.shape != (m, m):
            raise ValueError(f'Wrong matrix shape for qid_shape.\nMatrix shape: {self._matrix.shape}\nqid_shape: {self._qid_shape}\n')
        if unitary_check and (not linalg.is_unitary(matrix, rtol=unitary_check_rtol, atol=unitary_check_atol)):
    ...
    def _json_dict_(self) -> Dict[str, Any]:
        return {'matrix': self._matrix.tolist(), 'qid_shape': self._qid_shape, **({'name': self._name} if self._name is not None else {})}
    ...
    def _decompose_(self, qubits: Tuple['cirq.Qid', ...]) -> 'cirq.OP_TREE':
        if self._qid_shape == (2,):
            return [g.on(qubits[0]) for g in single_qubit_decompositions.single_qubit_matrix_to_gates(self._matrix)]
        if self._qid_shape == (2,) * 2:
            return two_qubit_to_cz.two_qubit_matrix_to_cz_operations(*qubits, self._matrix, allow_partial_czs=True)
    ...
    def _circuit_diagram_info_(self, args: 'cirq.CircuitDiagramInfoArgs') -> 'cirq.CircuitDiagramInfo':
        ...
        if self._name is not None:
            symbols = [self._name] if n_qubits == 1 else [f'{self._name}[{i + 1}]' for i in range(n_qubits)]
    ...
    def _qasm_(self, args: 'cirq.QasmArgs', qubits: Tuple['cirq.Qid', ...]) -> Optional[str]:
        ...
        if self._qid_shape == (2,):
            return protocols.qasm(phased_x_z_gate.PhasedXZGate.from_matrix(self._matrix), args=args, qubits=qubits)
        return None
