...
class NetAppESeriesProxyFirmwareUpload(NetAppESeriesModule):
    ...
    def __init__(self):
        ...
        super(NetAppESeriesProxyFirmwareUpload, self).__init__(ansible_options=ansible_options, web_services_version='02.00.0000.0000', supports_check_mode=True, proxy_specific_task=True)
    ...
    def apply(self):
        ...
        if self.upload_failures:
            self.module.fail_json(msg='Some file failed to be uploaded! changed=%s, Files_added [%s]. Files_removed [%s]. Upload_failures [%s]' % (change_required, self.add_files, self.remove_files, self.upload_failures))
