...
class SphinxComponentRegistry:
    ...
    def add_builder(self, builder: Type[Builder], override: bool=False) -> None:
        ...
        if builder.name in self.builders and (not override):
            raise ExtensionError(__('Builder %r already exists (in module %s)') % (builder.name, self.builders[builder.name].__module__))
    ...
    def preload_builder(self, app: 'Sphinx', name: str) -> None:
        ...
        if name not in self.builders:
            ...
            try:
                ...
                raise SphinxError(__('Builder name %s not registered or available through entry point') % name) from exc
            self.load_extension(app, entry_point.module)

    def create_builder(self, app: 'Sphinx', name: str, env: Optional[BuildEnvironment]=None) -> Builder:
        if name not in self.builders:
            raise SphinxError(__('Builder name %s not registered') % name)
        try:
            return self.builders[name](app, env)
        except TypeError:
            warnings.warn(f"The custom builder {name} defines a custom __init__ method without the 'env'argument. Report this bug to the developers of your custom builder, this is likely not a issue with Sphinx. The 'env' argument will be required from Sphinx 7.", RemovedInSphinx70Warning, stacklevel=2)
    ...
    def add_object_type(self, directivename: str, rolename: str, indextemplate: str='', parse_node: Optional[Callable]=None, ref_nodeclass: Optional[Type[TextElement]]=None, objname: str='', doc_field_types: List=[], override: bool=False) -> None:
        logger.debug('[app] adding object type: %r', (directivename, rolename, indextemplate, parse_node, ref_nodeclass, objname, doc_field_types))
        directive = type(directivename, (GenericObject, object), {'indextemplate': indextemplate, 'parse_node': staticmethod(parse_node), 'doc_field_types': doc_field_types})
    ...
    def get_source_input(self, filetype: str) -> Optional[Type[Input]]:
        warnings.warn('SphinxComponentRegistry.get_source_input() is deprecated.', RemovedInSphinx60Warning)
        try:
    ...
    def add_enumerable_node(self, node: Type[Node], figtype: str, title_getter: Optional[TitleGetter]=None, override: bool=False) -> None:
    ...
    def get_envversion(self, app: 'Sphinx') -> Dict[str, str]:
        ...
        envversion = {ext.name: ext.metadata['env_version'] for ext in app.extensions.values() if ext.metadata.get('env_version')}
