...
class DevicesCameraCustomAnalytics(object):
    ...
    def __init__(self, params, meraki):
        self.meraki = meraki
        self.new_object = dict(enabled=params.get('enabled'), artifactId=params.get('artifactId'), parameters=params.get('parameters'), serial=params.get('serial'))
    ...
    def update_all_params(self):
        ...
        if self.new_object.get('parameters') is not None or self.new_object.get('parameters') is not None:
            new_object_params['parameters'] = self.new_object.get('parameters') or self.new_object.get('parameters')
        if self.new_object.get('serial') is not None or self.new_object.get('serial') is not None:
    ...
    def get_object_by_name(self, name):
        result = None
        try:
            items = self.meraki.exec_meraki(family='camera', function='getDeviceCameraCustomAnalytics', params=self.get_all_params(name=name))
    ...
    def requires_update(self, current_obj):
        ...
        obj_params = [('enabled', 'enabled'), ('artifactId', 'artifactId'), ('parameters', 'parameters'), ('serial', 'serial')]
    ...
    def update(self):
        ...
        name = self.new_object.get('name')
        result = None
        result = self.meraki.exec_meraki(family='camera', function='updateDeviceCameraCustomAnalytics', params=self.update_all_params(), op_modifies=True)
        return result