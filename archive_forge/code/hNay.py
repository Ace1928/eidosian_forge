...
def process_file_or_folder():
    """
    Process a selected file or all files within a selected directory based on user input.
    If the user selects the "Process Directory" checkbox, the entire directory is processed.
    Otherwise, only the selected file is processed.
    """
    # Create a dialog to ask for file or directory
    process_directory = tk.BooleanVar()
    checkbox = tk.Checkbutton(
        window, text="Process Directory", variable=process_directory
    )
    checkbox.pack()

    path = (
        filedialog.askopenfilename()
        if not process_directory.get()
        else filedialog.askdirectory()
    )

    if path:
        if process_directory.get():
...
# Create buttons
process_button = tk.Button(window, text="Process File", command=process_file_or_folder)
