...
class CloudStackCommonTestCase(TestCaseMixin):
    ...
    def test_create_node_ex_networks(self):
        ...
        node = self.driver.create_node(name='deploynetworks', location=location, image=image, size=size, networks=networks)
    ...
    def test_create_node_ex_start_vm_false(self):
        ...
        location = self.driver.list_locations()[0]
        networks = [nw for nw in self.driver.ex_list_networks() if str(nw.zoneid) == str(location.id)]
        node = self.driver.create_node(name='stopped_vm', location=location, image=image, size=size, networks=networks, ex_start_vm=False)
    ...
    def test_create_node_ex_keyname(self):
        ...
        node = self.driver.create_node(name='test', location=location, image=image, size=size, ex_keyname='foobar')
        self.assertEqual(node.name, 'test')
    ...
    def test_create_node_project(self):
        ...
        node = self.driver.create_node(name='test', location=location, image=image, size=size, project=project)
    ...
    def test_ex_create_network(self):
        ...
        location = self.driver.list_locations()[0]
        network = self.driver.ex_create_network(display_text='test', name='test', network_offering=netoffer, location=location, gateway='10.1.1.1', netmask='255.255.255.0', network_domain='cloud.local', vpc_id='2', project_id='2')
    ...
    def test_list_nodes(self):
        ...
        self.assertEqual('772fd410-6649-43ed-befa-77be986b8906', nodes[0].extra['ip_forwarding_rules'][0].id)
        self.assertEqual(1, len(nodes[0].extra['port_forwarding_rules']))
        self.assertEqual('bc7ea3ee-a2c3-4b86-a53f-01bdaa1b2e32', nodes[0].extra['port_forwarding_rules'][0].id)
    ...
    def test_ex_create_affinity_group_already_exists(self):
        self.assertRaises(LibcloudError, self.driver.ex_create_affinity_group, 'MyAG', CloudStackAffinityGroupType('MyAGType'))
    ...
    def test_ex_create_firewall_rule(self):
        ...
        start_port = 33
        end_port = 34
        rule = self.driver.ex_create_firewall_rule(address, cidr_list, protocol, start_port=start_port, end_port=end_port)
        self.assertEqual(rule.address, address)
        self.assertEqual(rule.protocol, protocol)
        self.assertIsNone(rule.icmp_code)
        self.assertIsNone(rule.icmp_type)
    ...
    def test_node_ex_create_port_forwarding_rule(self):
        ...
        openfirewall = True
        protocol = 'TCP'
        rule = node.ex_create_port_forwarding_rule(address, private_port, public_port, protocol, public_end_port, private_end_port, openfirewall)
        self.assertEqual(rule.address, address)
        self.assertEqual(rule.protocol, protocol)
        self.assertEqual(rule.public_port, public_port)
        self.assertEqual(rule.public_end_port, public_end_port)
