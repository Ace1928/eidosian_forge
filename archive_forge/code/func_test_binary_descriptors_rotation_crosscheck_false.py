...
def test_binary_descriptors_rotation_crosscheck_false():
    ...
    exp_matches2 = np.array([0, 2, 1, 3, 4, 5, 7, 8, 14, 9, 11, 13, 23, 15, 16, 22, 17, 19, 37, 18, 24, 27, 30, 25, 26, 32, 28, 35, 37, 42, 29, 38, 33, 40, 36, 39, 10, 36, 43, 15, 35, 41, 6, 37, 32, 24, 8])
    assert_equal(matches[:, 0], exp_matches1)
    assert_equal(matches[:, 1], exp_matches2)
    matches = match_descriptors(descriptors1, descriptors2, metric='minkowski', cross_check=False)
    assert_equal(matches[:, 0], exp_matches1)
    assert_equal(matches[:, 1], exp_matches2)
    matches = match_descriptors(descriptors1, descriptors2, metric='minkowski', p=4, cross_check=False)
    assert_equal(matches[:, 0], exp_matches1)
    assert_equal(matches[:, 1], exp_matches2)