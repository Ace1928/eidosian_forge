...
class SolverResults(MapContainer):
    ...
    def __init__(self):
        ...
        self._sections = []
        self._descriptions = {}
        self.add('problem', ListContainer(pyomo.opt.results.problem.ProblemInformation), False, 'Problem Information')
        self.add('solver', ListContainer(pyomo.opt.results.solver.SolverInformation), False, 'Solver Information')
        self.add('solution', pyomo.opt.results.solution.SolutionSet(), False, 'Solution Information')
    ...
    def write(self, **kwds):
        ...
        if fname:
            ...
            normalized_ext = {'json': 'json', 'jsn': 'json', 'yaml': 'yaml', 'yml': 'yaml'}.get(ext, None)
            if not _fmt:
                _fmt = normalized_ext
            elif normalized_ext and _fmt != normalized_ext:
                logger.warning('writing results to file (%s) using what appears to be an incompatible format (%s)' % (fname, _fmt))
