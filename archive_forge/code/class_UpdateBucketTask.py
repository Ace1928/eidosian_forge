...
class UpdateBucketTask(task.Task):
    ...
    def __eq__(self, other):
        ...
        return self._bucket_resource == other._bucket_resource and self._user_request_args == other._user_request_args
    ...
    def execute(self, task_status_queue=None):
        ...
        request_config = request_config_factory.get_request_config(self._bucket_resource.storage_url, user_request_args=self._user_request_args)
        provider = self._bucket_resource.storage_url.scheme
        api_client = api_factory.get_api(provider)
        try:
            bucket_metadata = api_client.patch_bucket(self._bucket_resource, request_config=request_config)
        except errors.GcsApiError as e:
            if e.payload.status_code == 403 and request_config.resource_args.default_encryption_key:
                service_agent = api_client.get_service_agent()
                requests.AddCryptoKeyPermission(request_config.resource_args.default_encryption_key, 'serviceAccount:' + service_agent)
                bucket_metadata = api_client.patch_bucket(self._bucket_resource, request_config=request_config)
            else:
                raise
        if getattr(request_config.resource_args, 'retention_period_to_be_locked', None):
            self._confirm_and_lock_retention_policy(api_client, bucket_metadata, request_config)
