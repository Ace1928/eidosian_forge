...
class Lock:
    ...
    def __init__(self, redis: 'Redis', name: Union[str, bytes, memoryview], timeout: Optional[float]=None, sleep: float=0.1, blocking: bool=True, blocking_timeout: Optional[float]=None, thread_local: bool=True):
        """
    ...
    async def do_extend(self, additional_time, replace_ttl) -> bool:
        ...
        if not bool(await self.lua_extend(keys=[self.name], args=[self.local.token, additional_time, replace_ttl and '1' or '0'], client=self.redis)):
