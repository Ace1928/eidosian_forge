...
class BackupPoliciesClient(object):
    ...
    def DeleteBackupPolicy(self, backuppolicy_ref, async_):
        ...
        request = self.messages.NetappProjectsLocationsBackupPoliciesDeleteRequest(name=backuppolicy_ref.RelativeName())
    ...
    def UpdateBackupPolicy(self, backuppolicy_ref, backup_policy, update_mask, async_):
    ...
    """
        update_op = self._adapter.UpdateBackupPolicy(backuppolicy_ref, backup_policy, update_mask)
        if async_:
            return update_op
        operation_ref = resources.REGISTRY.ParseRelativeName(update_op.name, collection=constants.OPERATIONS_COLLECTION)
        return self.WaitForOperation(operation_ref)

    def ParseUpdatedBackupPolicy(self, backup_policy, enabled=None, daily_backup_limit=None, weekly_backup_limit=None, monthly_backup_limit=None, description=None, labels=None):
        """Parses updates into an Backup Policy."""
        return self._adapter.ParseUpdatedBackupPolicy(backup_policy=backup_policy, enabled=enabled, daily_backup_limit=daily_backup_limit, weekly_backup_limit=weekly_backup_limit, monthly_backup_limit=monthly_backup_limit, description=description, labels=labels)