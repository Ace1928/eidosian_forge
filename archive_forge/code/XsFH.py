...
class KANWrapper:
    ...
    def _initialize_parameters(self) -> None:
        ...
        def _initialize_param(param: torch.Tensor) -> None:
            """
            Initialize a model parameter.

            Args:
                param (torch.Tensor): The parameter to initialize.
            """
            param.data = _initialize_tensor(param)
        ...
        def _initialize_learnable_param(key: str, value: Any) -> None:
            ...
            if isinstance(value, torch.Tensor):
                self.learnable_params[key] = _initialize_tensor(value)
            else:
                self.learnable_params[key] = _initialize_tensor(torch.tensor(value))
