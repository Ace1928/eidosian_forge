...
class NetAppOntapDiskOptions:
    ...
    def __init__(self):
        ...
        self.argument_spec.update(dict(node=dict(required=True, type='str'), bkg_firmware_update=dict(required=False, type='bool'), autocopy=dict(required=False, type='bool'), autoassign=dict(required=False, type='bool'), autoassign_policy=dict(required=False, type='str', choices=['default', 'bay', 'shelf', 'stack'])))
    ...
    def get_disk_options(self):
        ...
        query = {'fields': 'node,autoassign,bkg-firmware-update,autocopy,autoassign-policy', 'node': self.parameters['node']}
        record, error = rest_generic.get_one_record(self.rest_api, api, query)
        if error:
            self.module.fail_json(msg='Error %s' % error)
        if record is None:
            self.module.fail_json(msg='Error on GET %s, no record.' % api)
        return {'node': record['node'], 'bkg_firmware_update': self.convert_to_bool(record, 'bkg_firmware_update'), 'autocopy': self.convert_to_bool(record, 'autocopy'), 'autoassign': self.convert_to_bool(record, 'autoassign'), 'autoassign_policy': record['autoassign_policy']}
