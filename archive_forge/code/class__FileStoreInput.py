...
class _FileStoreInput(NamedTuple):
    ...
    def write_yaml(self, root: str, file_name: str):
        dict_for_yaml = {'source_type': InputVertexType.Name(self.source_type), 'source_id': self.source_id, 'destination_type': InputVertexType.Name(self.destination_type), 'destination_id': self.source_id, 'tags': self.tags}
    ...
    def from_yaml(cls, root, file_name):
        ...
        return cls(source_type=InputVertexType.Value(dict_from_yaml['source_type']), source_id=dict_from_yaml['source_id'], destination_type=InputVertexType.Value(dict_from_yaml['destination_type']), destination_id=dict_from_yaml['destination_id'], tags=dict_from_yaml['tags'])