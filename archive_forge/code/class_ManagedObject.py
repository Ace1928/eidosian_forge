...
class ManagedObject(object, metaclass=abc.ABCMeta):
    ...
    def __init__(self, name=None, created=None, id=None, consumers=[]):
        ...
        if not created or isinstance(created, int):
            ...
            raise ValueError('created must be of long type, actual type %s' % type(created))
        self._id = id
    ...
    @classmethod
    def from_dict(cls, dict_fields, id=None, metadata_only=False, consumers=[]):
        ...
        try:
            ...
            return cls(value, name=dict_fields['name'], created=dict_fields['created'], id=id, consumers=consumers)
    ...
    def to_dict(self, metadata_only=False):
        ...
        return {'type': self.managed_type(), 'name': self.name, 'created': self.created, 'value': value, 'consumers': self.consumers}