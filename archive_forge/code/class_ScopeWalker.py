...
class ScopeWalker(object):
    ...
    def __init__(self, compilation, atom, names_only=False):
        ...
        if self._node is None:
            raise ValueError("Unable to find atom '%s' in compilation hierarchy" % atom)
        self._level_cache = {}
        self._atom = atom
    ...
    def __iter__(self):
        ...
        if self._predecessors is None:
            predecessors = set((node for node in graph.bfs_predecessors_iter(self._atom) if graph.nodes[node]['kind'] in co.ATOMS))
        ...
        for lvl, parent in enumerate(self._node.path_iter(include_self=False)):
            ...
            try:
                ...
                visible = []
                removals = set()
                atom_it = tr.depth_first_reverse_iterate(parent, start_from_idx=last_idx)
                for atom in atom_it:
                ...
                if LOG.isEnabledFor(logging.TRACE):
                    ...
                    LOG.trace("Scope visible to '%s' (limited by parent '%s' index < %s) is: %s", self._atom, parent.item.name, last_idx, visible_names)
            if self._names_only:
