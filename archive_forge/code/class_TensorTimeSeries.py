...
class TensorTimeSeries(_TimeSeries):
    ...
    def __repr__(self):
        return 'TensorTimeSeries(%s)' % ', '.join(('max_step=%r' % (self._max_step,), 'max_wall_time=%r' % (self._max_wall_time,), 'plugin_content=%r' % (self._plugin_content,), 'description=%r' % (self._description,), 'display_name=%r' % (self._display_name,)))