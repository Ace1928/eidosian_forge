...
def test_boot_image_bdms_v2(self):
    self.run_command('boot --flavor 1 --image %s --block-device id=fake-id,source=volume,dest=volume,device=vda,size=1,format=ext4,type=disk,shutdown=preserve some-server' % FAKE_UUID_1)
