...
class AliyunNodeProvider(NodeProvider):
    ...
    def __init__(self, provider_config, cluster_name):
        ...
        self.acs = AcsClient(access_key=provider_config['access_key'], access_key_secret=provider_config['access_key_secret'], region_id=provider_config['region'], max_retries=BOTO_MAX_RETRIES)
    ...
    def non_terminated_nodes(self, tag_filters: Dict[str, str]) -> List[str]:
        tags = [{'Key': TAG_RAY_CLUSTER_NAME, 'Value': self.cluster_name}]
    ...
    def create_node(self, node_config: Dict[str, Any], tags: Dict[str, str], count: int) -> Optional[Dict[str, Any]]:
        filter_tags = [{'Key': TAG_RAY_CLUSTER_NAME, 'Value': self.cluster_name}, {'Key': TAG_RAY_NODE_KIND, 'Value': tags[TAG_RAY_NODE_KIND]}, {'Key': TAG_RAY_USER_NODE_TYPE, 'Value': tags[TAG_RAY_USER_NODE_TYPE]}, {'Key': TAG_RAY_LAUNCH_CONFIG, 'Value': tags[TAG_RAY_LAUNCH_CONFIG]}, {'Key': TAG_RAY_NODE_NAME, 'Value': tags[TAG_RAY_NODE_NAME]}]
        reused_nodes_dict = {}
        ...
        created_nodes_dict = {}
        if count > 0:
            filter_tags.append({'Key': TAG_RAY_NODE_STATUS, 'Value': tags[TAG_RAY_NODE_STATUS]})
            instance_id_sets = self.acs.run_instances(instance_type=node_config['InstanceType'], image_id=node_config['ImageId'], tags=filter_tags, amount=count, vswitch_id=self.provider_config['v_switch_id'], security_group_id=self.provider_config['security_group_id'], key_pair_name=self.provider_config['key_name'])
    ...
    def terminate_node(self, node_id: str) -> None:
        ...
        if self.cache_stopped_nodes:
            logger.info('Stopping instance {} (to terminate instead, set `cache_stopped_nodes: False` under `provider` in the cluster configuration)').format(node_id)
    ...
    def terminate_nodes(self, node_ids: List[str]) -> None:
        if not node_ids:
            return
        if self.cache_stopped_nodes:
            logger.info('Stopping instances {} (to terminate instead, set `cache_stopped_nodes: False` under `provider` in the cluster configuration)'.format(node_ids))
