  ...
  """
    ...
    def __setitem__(self, name, flag):
        ...
        if name in fl and (not flag.allow_override) and (not fl[name].allow_override):
            ...
            if self.find_module_defining_flag(name) == module_name and id(module) != self.find_module_id_defining_flag(name):
                return
            raise _exceptions.DuplicateFlagError.from_flag(name, self)
        short_name = flag.short_name
        flags_to_cleanup = set()
        if short_name is not None:
            if short_name in fl and (not flag.allow_override) and (not fl[short_name].allow_override):
    ...
    def __getattr__(self, name):
        ...
        if self.__dict__['__flags_parsed'] or fl[name].present:
            ...
            error_message = 'Trying to access flag --%s before flags were parsed.' % name
            if six.PY2:
    ...
    """
        if _helpers.is_bytes_or_string(argv):
            raise TypeError('argv should be a tuple/list of strings, not bytes or string.')
        if not argv:
            raise ValueError('argv cannot be an empty list, and must contain the program name as the first element.')
        program_name = argv[0]
        ...
        for name, value in unknown_flags:
            ...
            raise _exceptions.UnrecognizedFlagError(name, value, suggestions=suggestions)
        self.mark_as_parsed()
    ...
    def __copy__(self):
        raise TypeError('FlagValues does not support shallow copies. Use absl.testing.flagsaver or copy.deepcopy instead.')
            ...
            if retired_flag_func and (not flag):
                ...
                if is_retired:
                    if not is_bool and value is None:
                        get_value()
                    logging.error('Flag "%s" is retired and should no longer be specified. See go/totw/90.', name)
                    continue
            if flag:
                flag.parse(value)
