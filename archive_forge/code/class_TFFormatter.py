...
class TFFormatter(TensorFormatter[Mapping, 'tf.Tensor', Mapping]):
    ...
    def _consolidate(self, column):
        ...
        if isinstance(column, list) and column:
            if all((isinstance(x, tf.Tensor) and x.shape == column[0].shape and (x.dtype == column[0].dtype) for x in column)):
                return tf.stack(column)
            elif all((isinstance(x, (tf.Tensor, tf.RaggedTensor)) and x.ndim == 1 and (x.dtype == column[0].dtype) for x in column)):
