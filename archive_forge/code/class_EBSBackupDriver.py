...
class EBSBackupDriver(BackupDriver):
    ...
    def create_target_from_node(self, node, type=BackupTargetType.VIRTUAL, extra=None):
        ...
        if device_mapping is not None:
            return self.create_target(name=node.name, address=device_mapping['ebs'][0]['volume_id'], type=BackupTargetType.VOLUME, extra=None)
        else:
    ...
    def list_recovery_points(self, target, start_date=None, end_date=None):
        ...
        """
        params = {'Action': 'DescribeSnapshots', 'Filter.1.Name': 'volume-id', 'Filter.1.Value': target.extra['volume-id']}
    ...
    def list_target_jobs(self, target):
        ...
        """
        params = {'Action': 'DescribeSnapshots', 'Filter.1.Name': 'volume-id', 'Filter.1.Value': target.extra['volume-id'], 'Filter.2.Name': 'status', 'Filter.2.Value': 'pending'}
        data = self.connection.request(ROOT, params=params).object
    ...
    def _to_job(self, el):
        ...
        job = BackupTargetJob(id=id, status=BackupTargetJobStatusType.PENDING, progress=int(progress), target=target, driver=self.connection.driver, extra={})
        return job
