...
class AzureRMWebAppVnetConnection(AzureRMModuleBase):
    ...
    def __init__(self):
        self.module_arg_spec = dict(name=dict(type='str', required=True), resource_group=dict(type='str', required=True), state=dict(type='str', default='present', choices=['present', 'absent']), vnet_name=dict(type='str'), subnet=dict(type='str'), vnet_resource_group=dict(type='str'))
        ...
        super(AzureRMWebAppVnetConnection, self).__init__(self.module_arg_spec, supports_check_mode=True, supports_tags=False)
