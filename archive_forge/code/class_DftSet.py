...
class DftSet(Cp2kInput):
    ...
    def __init__(self, structure: Structure | Molecule, project_name: str='CP2K', basis_and_potential: dict | None=None, xc_functionals: list | str | None=None, multiplicity: int=0, ot: bool=True, energy_gap: float=-1, qs_method: str='GPW', eps_default: float=1e-12, eps_scf: float=1e-06, max_scf: int | None=None, minimizer: str='DIIS', preconditioner: str='FULL_SINGLE_INVERSE', algorithm: str='IRAC', linesearch: str='2PNT', rotation: bool=True, occupation_preconditioner: bool=False, cutoff: float | None=None, rel_cutoff: int=50, ngrids: int=5, progression_factor: int=3, override_default_params: dict | None=None, wfn_restart_file_name: str | None=None, kpoints: VaspKpoints | None=None, smearing: bool=False, **kwargs) -> None:
        ...
        if self.kpoints:
            if self.kpoints.style in [KpointsSupportedModes.Gamma, KpointsSupportedModes.Monkhorst] and np.array_equal(self.kpoints.kpts[0], (1, 1, 1)) or (self.kpoints.style in [KpointsSupportedModes.Reciprocal, KpointsSupportedModes.Cartesian] and np.array_equal(self.kpoints.kpts[0], (0, 0, 0))):
                self.kpoints = None
            if ot and self.kpoints:
                warnings.warn('As of 2022.1, kpoints not supported with OT. Defaulting to diagonalization')
                ot = False
        global_sec = Global(project_name=self.kwargs.get('project_name', 'CP2K'), run_type=self.kwargs.get('run_type', 'ENERGY_FORCE'))
        self.insert(global_sec)
        qs = QS(method=self.qs_method, eps_default=eps_default, eps_pgf_orb=kwargs.get('eps_pgf_orb'))
        max_scf = max_scf or 20 if ot else 400
        scf = Scf(eps_scf=eps_scf, max_scf=max_scf, subsections={})
        if ot:
            scf.insert(OrbitalTransformation(minimizer=minimizer, preconditioner=preconditioner, energy_gap=energy_gap, algorithm=algorithm, linesearch=linesearch, rotation=rotation, occupation_preconditioner=occupation_preconditioner))
            scf.insert(Section('OUTER_SCF', subsections={}, keywords={'MAX_SCF': Keyword('MAX_SCF', kwargs.get('outer_max_scf', 20)), 'EPS_SCF': Keyword('EPS_SCF', kwargs.get('outer_eps_scf', eps_scf))}))
        else:
            scf.insert(Section('DIAGONALIZATION', subsections={}))
            mixing_kwds = {'ALPHA': Keyword('ALPHA', kwargs.get('alpha', 0.05)), 'BETA': Keyword('BETA', kwargs.get('beta', 0.01)), 'NBUFFER': Keyword('NBUFFER', kwargs.get('nbuffer', 10)), 'N_SIMPLE_MIX': Keyword('N_SIMPLE_MIX', kwargs.get('n_simple_mix', 3)), 'METHOD': Keyword('METHOD', kwargs.get('mixing_method', 'BROYDEN_MIXING'))}
        ...
        mgrid = Mgrid(cutoff=self.cutoff, rel_cutoff=rel_cutoff, ngrids=ngrids, progression_factor=progression_factor)
        ...
        dft = Dft(MULTIPLICITY=self.multiplicity, CHARGE=self.charge, uks=self.kwargs.get('spin_polarized', True), basis_set_filenames=self.basis_set_file_names or [], potential_filename=self.potential_file_name, subsections={'QS': qs, 'SCF': scf, 'MGRID': mgrid}, wfn_restart_file_name=wfn_restart_file_name)
        if self.kpoints:
    ...
    def activate_localize(self, states='OCCUPIED', preconditioner='FULL_ALL', restart=False) -> None:
        ...
        self['FORCE_EVAL']['PROPERTIES']['LINRES'].insert(Section('LOCALIZE', PRECONDITIONER=preconditioner, STATES=states, RESTART=restart))
    ...
    def activate_very_strict_minimization(self) -> None:
        ...
        """
        ot = OrbitalTransformation(minimizer='CG', preconditioner='FULL_ALL', algorithm='STRICT', linesearch='GOLD')
        self.update({'FORCE_EVAL': {'DFT': {'SCF': {'OT': ot}}}})
    ...
    def modify_dft_print_iters(self, iters, add_last='no'):
        ...
        run_type = self['global'].get('run_type', Keyword('run_type', 'energy')).values[0].upper()
        if run_type not in ['ENERGY_FORCE', 'ENERGY', 'WAVEFUNCTION_OPTIMIZATION', 'WFN_OPT'] and self.check('FORCE_EVAL/DFT/PRINT'):
    ...
    def validate(self):
        ...
        for val in self['force_eval']['subsys'].subsections.values():
            if val.name.upper() == 'KIND' and val['POTENTIAL'].values[0].upper() == 'ALL' and (self['force_eval']['dft']['qs']['method'].values[0].upper() != 'GAPW'):
