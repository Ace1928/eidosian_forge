...
class DesktopEntry(IniFile):
    ...
    def checkGroup(self, group):
        if not (group == self.defaultGroup or re.match('^Desktop Action [a-zA-Z0-9-]+$', group) or (re.match('^X-', group) and is_ascii(group))):
            self.errors.append('Invalid Group name: %s' % group)
        elif 'OnlyShowIn' in self.content[group] and 'NotShowIn' in self.content[group]:
