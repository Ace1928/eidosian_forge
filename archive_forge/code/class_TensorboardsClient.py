...
class TensorboardsClient(object):
    ...
    def CreateBeta(self, location_ref, args):
        """Create a new Tensorboard."""
        kms_key_name = common_validation.GetAndValidateKmsKey(args)
        labels = labels_util.ParseCreateArgs(args, self.messages.GoogleCloudAiplatformV1beta1Tensorboard.LabelsValue)
        tensorboard = self.messages.GoogleCloudAiplatformV1beta1Tensorboard(displayName=args.display_name, description=args.description, labels=labels)
        if kms_key_name is not None:
            tensorboard.encryptionSpec = api_util.GetMessage('EncryptionSpec', self._version)(kmsKeyName=kms_key_name)
        request = self.messages.AiplatformProjectsLocationsTensorboardsCreateRequest(parent=location_ref.RelativeName(), googleCloudAiplatformV1beta1Tensorboard=tensorboard)
        return self._service.Create(request)

    def Get(self, tensorboard_ref):
        request = self.messages.AiplatformProjectsLocationsTensorboardsGetRequest(name=tensorboard_ref.RelativeName())
        return self._service.Get(request)

    def List(self, limit=1000, page_size=50, region_ref=None, sort_by=None):
        request = self.messages.AiplatformProjectsLocationsTensorboardsListRequest(parent=region_ref.RelativeName(), orderBy=common_args.ParseSortByArg(sort_by))
        return list_pager.YieldFromList(self._service, request, field='tensorboards', batch_size_attribute='pageSize', batch_size=page_size, limit=limit)

    def Delete(self, tensorboard_ref):
        request = self.messages.AiplatformProjectsLocationsTensorboardsDeleteRequest(name=tensorboard_ref.RelativeName())
    ...
    def Patch(self, tensorboard_ref, args):
        ...
        labels_update = labels_util.ProcessUpdateArgsLazy(args, labels_value, GetLabels)
        ...
        if self._version == constants.GA_VERSION:
            req = self.messages.AiplatformProjectsLocationsTensorboardsPatchRequest(name=tensorboard_ref.RelativeName(), googleCloudAiplatformV1Tensorboard=tensorboard, updateMask=','.join(update_mask))
        else:
            req = self.messages.AiplatformProjectsLocationsTensorboardsPatchRequest(name=tensorboard_ref.RelativeName(), googleCloudAiplatformV1beta1Tensorboard=tensorboard, updateMask=','.join(update_mask))
