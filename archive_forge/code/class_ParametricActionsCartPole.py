...
class ParametricActionsCartPole(gym.Env):
    ...
    def __init__(self, max_avail_actions):
        ...
        self.observation_space = Dict({'action_mask': Box(0, 1, shape=(max_avail_actions,), dtype=np.int8), 'avail_actions': Box(-10, 10, shape=(max_avail_actions, 2)), 'cart': self.wrapped.observation_space})
    ...
    def reset(self, *, seed=None, options=None):
        ...
        return ({'action_mask': self.action_mask, 'avail_actions': self.action_assignments, 'cart': obs}, infos)
    ...
    def step(self, action):
        if action == self.left_idx:
            ...
            raise ValueError('Chosen action was not one of the non-zero action embeddings', action, self.action_assignments, self.action_mask, self.left_idx, self.right_idx)
        orig_obs, rew, done, truncated, info = self.wrapped.step(actual_action)
        self.update_avail_actions()
        self.action_mask = self.action_mask.astype(np.int8)
        obs = {'action_mask': self.action_mask, 'avail_actions': self.action_assignments, 'cart': orig_obs}
