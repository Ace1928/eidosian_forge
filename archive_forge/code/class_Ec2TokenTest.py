...
class Ec2TokenTest(common.HeatTestCase):
    ...
    def test_conf_get_ssl_opts(self):
        cfg.CONF.set_default('auth_uri', 'https://192.0.2.9/v2.0/', group='ec2authtoken')
        cfg.CONF.set_default('ca_file', '/home/user/cacert.pem', group='ec2authtoken')
        cfg.CONF.set_default('insecure', 'false', group='ec2authtoken')
        cfg.CONF.set_default('cert_file', '/home/user/mycert', group='ec2authtoken')
        cfg.CONF.set_default('key_file', '/home/user/mykey', group='ec2authtoken')
        ec2 = ec2token.EC2Token(app=None, conf={})
        self.assertEqual('/home/user/cacert.pem', ec2.ssl_options['verify'])
        self.assertEqual(('/home/user/mycert', '/home/user/mykey'), ec2.ssl_options['cert'])
    ...
    def test_get_signature_header_notlast(self):
        req_env = {'HTTP_AUTHORIZATION': 'Authorization: foo  Credential=foo/bar, Signature=xyz,SignedHeaders=content-type;host;x-amz-date '}
        dummy_req = self._dummy_GET_request(environ=req_env)
        ec2 = ec2token.EC2Token(app=None, conf={})
        self.assertEqual('xyz', ec2._get_signature(dummy_req))
    ...
    def test_get_access_header_last(self):
        req_env = {'HTTP_AUTHORIZATION': 'Authorization: foo SignedHeaders=content-type;host;x-amz-date,Signature=xyz,Credential=foo/bar'}
        dummy_req = self._dummy_GET_request(environ=req_env)
        ec2 = ec2token.EC2Token(app=None, conf={})
        self.assertEqual('foo', ec2._get_access(dummy_req))
    ...
    def test_call_auth_nouser(self):
        req_env = {'HTTP_AUTHORIZATION': 'Authorization: foo SignedHeaders=content-type;host;x-amz-date,Signature=xyz'}
        dummy_req = self._dummy_GET_request(environ=req_env)
        ec2 = ec2token.EC2Token(app='xyz', conf={})
    ...
    def test_call_err_signature(self):
        ...
        err_resp = json.dumps({'error': {'message': err_msg}})
        self._stub_http_connection(headers={'Authorization': auth_str}, response=err_resp)
        self.assertRaises(exception.HeatSignatureError, ec2.__call__, dummy_req)
        requests.post.assert_called_once_with(self.verify_req_url, data=self.verify_data, verify=self.verify_verify, cert=self.verify_cert, headers=self.verify_req_headers)
    ...
    def test_call_badconf_no_authuri(self):
        ...
        req_env = {'SERVER_NAME': 'heat', 'SERVER_PORT': '8000', 'PATH_INFO': '/v1'}
        dummy_req = self._dummy_GET_request(params, req_env)
        ex = self.assertRaises(exception.HeatInternalFailureError, ec2.__call__, dummy_req)
    ...
    def test_call_ok_auth_uri_ks_authtoken(self):
        ...
        dummy_url = 'http://123:5000/v2.0'
        try:
            cfg.CONF.set_override('www_authenticate_uri', dummy_url, group='keystone_authtoken')
