...
def cli_bc(arguments: List[str]):
    parser = argparse.ArgumentParser(description='BC script checker for Charset-Normalizer with Chardet')
    parser.add_argument('-c', '--coverage', action='store', default=85, type=int, dest='coverage', help='Define the minimum acceptable coverage to succeed')
    ...
    for tbt_path in sorted(glob('./char-dataset/**/*.*')):
        ...
        if calc_eq >= 0.98:
            success_count += 1
            print("️✅ ️'{}' (got '{}' but eq {} WITH {} %)".format(tbt_path, charset_normalizer_encoding, chardet_encoding, round(calc_eq * 100.0, 3)))
            continue
        print("⚡⚡ '{}' (BC-Break) New('{}') vs Legacy('{}')".format(tbt_path, charset_normalizer_encoding, chardet_encoding))
