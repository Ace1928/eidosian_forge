...
class LiquidWebDNSDriver(DNSDriver):
    ...
    def create_record(self, name, zone, type, data, extra=None):
        ...
        to_post = {'params': {'name': name, 'rdata': data, 'type': type, 'zone': zone.domain, 'zone_id': zone.id}}
        if extra is not None:
    ...
    def _to_zone(self, item):
        ...
        zone = Zone(domain=item['name'], id=item['id'], type=item['type'], ttl=None, driver=self, extra=extra)
        return zone
    ...
    def _to_record(self, item, zone):
        ...
        for key in item:
            if key not in common_attr:
                extra[key] = item.get(key)
        record = Record(id=item['id'], name=item['name'], type=item['type'], data=item['rdata'], zone=zone, driver=self, extra=extra)
        return record
