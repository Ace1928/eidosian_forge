...
class BasisRotation(Operation):
    ...
    def __init__(self, wires, unitary_matrix, check=False, id=None):
        ...
        if check:
            ...
            if not np.allclose(umat @ umat.conj().T, np.eye(M, dtype=complex), atol=1e-06):
        ...
        self._hyperparameters = {'unitary_matrix': unitary_matrix}
    ...
    @staticmethod
    def compute_decomposition(wires, unitary_matrix, check=False):
        ...
        if check:
            umat = qml.math.toarray(unitary_matrix)
            if not np.allclose(umat @ umat.conj().T, np.eye(M, dtype=complex), atol=0.0001):
                raise ValueError('The provided transformation matrix should be unitary.')
        if len(wires) < 2:
            raise ValueError(f'This template requires at least two wires, got {len(wires)}')
        op_list = []
        ...
        for grot_mat, indices in givens_list:
            ...
            op_list.append(qml.SingleExcitation(2 * theta, wires=[wires[indices[0]], wires[indices[1]]]))
