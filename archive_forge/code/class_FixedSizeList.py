...
class FixedSizeList(FunctionAnalysis):
    ...
    def __init__(self):
        ...
        super(FixedSizeList, self).__init__(Aliases, DefUseChains, Ancestors, ArgumentEffects)
    ...
    def visit_Assign(self, node):
        ...
        for target in node.targets:
            ...
            if not isinstance(target, ast.Name):
                continue
            if sum((1 for d in self.def_use_chains.locals[self.ctx.function] if d.name() == target.id)) > 1:
                break
        else:
