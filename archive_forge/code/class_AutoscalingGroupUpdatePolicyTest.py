...
class AutoscalingGroupUpdatePolicyTest(AutoscalingGroupTest):
    ...
    def ig_tmpl_with_updt_policy(self):
        ...
        up = {'AutoScalingRollingUpdate': {'MinInstancesInService': '1', 'MaxBatchSize': '2', 'PauseTime': 'PT1S'}}
        templ['Resources']['JobServerGroup']['UpdatePolicy'] = up
        return templ

    def update_instance_group(self, updt_template, num_updates_expected_on_updt, num_creates_expected_on_updt, num_deletes_expected_on_updt):
        files = {'provider.yaml': self.instance_template}
        size = 10
        env = {'resource_registry': {'AWS::EC2::Instance': 'provider.yaml'}, 'parameters': {'size': size, 'image': self.conf.minimal_image_ref, 'flavor': self.conf.instance_type}}
    ...
    def test_instance_group_update_replace(self):
        ...
        self.update_instance_group(updt_template, num_updates_expected_on_updt=10, num_creates_expected_on_updt=0, num_deletes_expected_on_updt=0)
    ...
    def test_instance_group_update_replace_with_adjusted_capacity(self):
        ...
        config = updt_template['Resources']['JobServerConfig']
        config['Properties']['UserData'] = 'new data'
        self.update_instance_group(updt_template, num_updates_expected_on_updt=8, num_creates_expected_on_updt=2, num_deletes_expected_on_updt=2)
    ...
    def test_instance_group_update_replace_huge_min_in_service(self):
        ...
        config = updt_template['Resources']['JobServerConfig']
        config['Properties']['UserData'] = 'new data'
        self.update_instance_group(updt_template, num_updates_expected_on_updt=9, num_creates_expected_on_updt=1, num_deletes_expected_on_updt=1)
