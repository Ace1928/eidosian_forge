...
def _get_news_api(llm: BaseLanguageModel, **kwargs: Any) -> BaseTool:
    ...
    chain = APIChain.from_llm_and_api_docs(llm, news_docs.NEWS_DOCS, headers={'X-Api-Key': news_api_key}, limit_to_domains=['https://newsapi.org/'])
    return Tool(name='News-API', description='Use this when you want to get information about the top headlines of current news stories. The input should be a question in natural language that this API can answer.', func=chain.run)