...
def test_cannot_check_grant_for_group_own_domain_on_own_domain_with_role_other_domain(self):
    ...
    with self.test_client() as c:
        c.get('/v3/domains/%s/groups/%s/roles/%s' % (domain_id, group['id'], role['id']), headers=self.headers, expected_status_code=http.client.FORBIDDEN)