...
class QdrantSparseVectorRetriever(BaseRetriever):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        try:
            ...
            raise ImportError('Could not import qdrant-client python package. Please install it with `pip install qdrant-client`.')
        client = values['client']
        if not isinstance(client, QdrantClient):
            raise ValueError(f'client should be an instance of qdrant_client.QdrantClient, got {type(client)}')
        filter = values['filter']
        if filter is not None and (not isinstance(filter, models.Filter)):
            raise ValueError(f'filter should be an instance of qdrant_client.models.Filter, got {type(filter)}')
        ...
        try:
            ...
            if sparse_vector_name not in sparse_vectors_config:
                raise QdrantException(f'Existing Qdrant collection {collection_name} does not contain sparse vector named {sparse_vector_name}.Did you mean one of {', '.join(sparse_vectors_config.keys())}?')
    ...
    def _get_relevant_documents(self, query: str, *, run_manager: CallbackManagerForRetrieverRun) -> List[Document]:
        ...
        results = client.search(self.collection_name, query_filter=self.filter, query_vector=models.NamedSparseVector(name=self.sparse_vector_name, vector=models.SparseVector(indices=query_indices, values=query_values)), limit=self.k, with_vectors=False, **self.search_options)
        return [Qdrant._document_from_scored_point(point, self.collection_name, self.content_payload_key, self.metadata_payload_key) for point in results]
    ...
    def add_texts(self, texts: Iterable[str], metadatas: Optional[List[dict]]=None, ids: Optional[Sequence[str]]=None, batch_size: int=64, **kwargs: Any) -> List[str]:
