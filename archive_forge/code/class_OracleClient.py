...
class OracleClient:
    ...
    def __getattr__(self, name):
        whitelisted_attrs = {'objective', 'max_trials', 'allow_new_entries', 'tune_new_entries'}
    ...
    def get_space(self):
        response = self.stub.GetSpace(protos.get_service().GetSpaceRequest(), wait_for_ready=True, timeout=TIMEOUT)
        return hp_module.HyperParameters.from_proto(response.hyperparameters)

    def update_space(self, hyperparameters):
        if self.should_report:
            self.stub.UpdateSpace(protos.get_service().UpdateSpaceRequest(hyperparameters=hyperparameters.to_proto()), wait_for_ready=True, timeout=TIMEOUT)

    def create_trial(self, tuner_id):
        response = self.stub.CreateTrial(protos.get_service().CreateTrialRequest(tuner_id=tuner_id), wait_for_ready=True, timeout=TIMEOUT)
        return trial_module.Trial.from_proto(response.trial)

    def update_trial(self, trial_id, metrics, step=0):
        if self.should_report:
            response = self.stub.UpdateTrial(protos.get_service().UpdateTrialRequest(trial_id=trial_id, metrics=metrics, step=step), wait_for_ready=True, timeout=TIMEOUT)
    ...
    def end_trial(self, trial):
        if self.should_report:
            self.stub.EndTrial(protos.get_service().EndTrialRequest(trial=trial.to_proto()), wait_for_ready=True, timeout=TIMEOUT)

    def get_trial(self, trial_id):
        response = self.stub.GetTrial(protos.get_service().GetTrialRequest(trial_id=trial_id), wait_for_ready=True, timeout=TIMEOUT)
        return trial_module.Trial.from_proto(response.trial)

    def get_best_trials(self, num_trials=1):
        response = self.stub.GetBestTrials(protos.get_service().GetBestTrialsRequest(num_trials=num_trials), wait_for_ready=True, timeout=TIMEOUT)
