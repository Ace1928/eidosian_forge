...
def convert_to_inference_data(obj, *, group='posterior', coords=None, dims=None, **kwargs):
    ...
    elif obj.__class__.__name__ in {'StanFit4Model', 'CmdStanMCMC'} or obj.__class__.__module__ == 'stan.fit':
        if group == 'sample_stats':
            kwargs['posterior'] = kwargs.pop(group)
        elif group == 'sample_stats_prior':
            kwargs['prior'] = kwargs.pop(group)
    ...
    elif obj.__class__.__name__ == 'EnsembleSampler':
    ...
    if isinstance(obj, xr.Dataset):
        ...
        allowable_types = ('xarray dataarray', 'xarray dataset', 'dict', 'pytree', 'netcdf filename', 'numpy array', 'pystan fit', 'emcee fit', 'pyro mcmc fit', 'numpyro mcmc fit', 'cmdstan fit csv filename', 'cmdstanpy fit')
        raise ValueError(f'Can only convert {', '.join(allowable_types)} to InferenceData, not {obj.__class__.__name__}')
