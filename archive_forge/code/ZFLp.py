...
PARTICLE_COUNT = 1000  # 10x more particles
PARTICLE_SIZE = 1  # 10x smaller size
...
class Particle:
    ...
    def draw(self):
        ...
        pygame.draw.circle(screen, color, self.pos.astype(int), PARTICLE_SIZE)
...
def update_particles(particles):
    with Pool(processes=4) as pool:
        particles = pool.map(
            functools.partial(Particle.update, particles=particles), particles
        )
    return particles
