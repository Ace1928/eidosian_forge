...
class BaseRetriever(RunnableSerializable[RetrieverInput, RetrieverOutput], ABC):
    ...
    def __init_subclass__(cls, **kwargs: Any) -> None:
        ...
        if hasattr(cls, 'aget_relevant_documents') and cls.aget_relevant_documents != BaseRetriever.aget_relevant_documents:
            warnings.warn('Retrievers must implement abstract `_aget_relevant_documents` method instead of `aget_relevant_documents`', DeprecationWarning)
    ...
    async def ainvoke(self, input: str, config: Optional[RunnableConfig]=None, **kwargs: Any) -> List[Document]:
        ...
        """
        config = ensure_config(config)
        return await self.aget_relevant_documents(input, callbacks=config.get('callbacks'), tags=config.get('tags'), metadata=config.get('metadata'), run_name=config.get('run_name'), **kwargs)
    ...
    async def _aget_relevant_documents(self, query: str, *, run_manager: AsyncCallbackManagerForRetrieverRun) -> List[Document]:
        ...
        Returns:
            List of relevant documents
        """
        return await run_in_executor(None, self._get_relevant_documents, query, run_manager=run_manager.get_sync())
    ...
    async def aget_relevant_documents(self, query: str, *, callbacks: Callbacks=None, tags: Optional[List[str]]=None, metadata: Optional[Dict[str, Any]]=None, run_name: Optional[str]=None, **kwargs: Any) -> List[Document]:
        """Asynchronously get documents relevant to a query.
        ...
        callback_manager = AsyncCallbackManager.configure(callbacks, None, verbose=kwargs.get('verbose', False), inheritable_tags=tags, local_tags=self.tags, inheritable_metadata=metadata, local_metadata=self.metadata)
        run_manager = await callback_manager.on_retriever_start(dumpd(self), query, name=run_name, run_id=kwargs.pop('run_id', None))
        try:
            _kwargs = kwargs if self._expects_other_args else {}
            if self._new_arg_supported:
                result = await self._aget_relevant_documents(query, run_manager=run_manager, **_kwargs)
            else:
