...
class _JSONForceHTTPErrorCopyCallbackHandler(object):
    ...
    def call(self, total_bytes_transferred, total_size):
        ...
        if total_bytes_transferred >= self._startover_at_byte and (not self.started_over_once):
            sys.stderr.write('Forcing HTTP error %s after byte %s. %s/%s transferred.\r\n' % (self._http_error_num, self._startover_at_byte, MakeHumanReadable(total_bytes_transferred), MakeHumanReadable(total_size)))
            self.started_over_once = True
            raise apitools_exceptions.HttpError({'status': self._http_error_num}, None, None)