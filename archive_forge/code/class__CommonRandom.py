...
class _CommonRandom(OpRun):
    ...
    def __init__(self, onnx_node, run_params):
        ...
        if hasattr(self, 'shape') and len(self.shape) == 0:
            raise ValueError(f'shape cannot be empty for operator {self.__class__.__name__}.')
    ...
    def _dtype(*data, dtype=None, dtype_first=False):
        ...
        if dtype_first and numpy_type is not None:
            ...
            if data:
                return data[0].dtype
            raise RuntimeError(f'dtype cannot be None for a random operator {_CommonRandom.__name__!r}, numpy_type={numpy_type}, len(data)={len(data)}.')
        res = None
        if not data:
            res = numpy_type
            ...
            res = data[0].dtype
        if res is None:
            raise RuntimeError(f'dtype cannot be None, numpy_type={numpy_type}, type(data[0])={type(data[0])}.')
        return res
