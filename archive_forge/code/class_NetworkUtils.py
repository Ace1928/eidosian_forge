...
class NetworkUtils(baseutils.BaseUtilsVirt):
    ...
    def get_vnic_ids(self):
        return set((p.ElementName for p in self._conn.Msvm_SyntheticEthernetPortSettingData() if p.ElementName is not None))
    ...
    def _prepare_vlan_sd_trunk_mode(self, vlan_settings, vlan_id, trunk_vlans):
        if vlan_settings:
            ...
            if vlan_settings.OperationMode == constants.VLAN_MODE_TRUNK and vlan_settings.NativeVlanId == vlan_id and (sorted(vlan_settings.TrunkVlanIdArray) == trunk_vlans):
                return None
        vlan_settings = self._create_default_setting_data(self._PORT_VLAN_SET_DATA)
    ...
    def _get_profile_setting_data_from_port_alloc(self, port_alloc):
        return self._get_setting_data_from_port_alloc(port_alloc, self._profile_sds, self._PORT_PROFILE_SET_DATA)
    ...
    def _get_security_setting_data_from_port_alloc(self, port_alloc):
        return self._get_setting_data_from_port_alloc(port_alloc, self._vsid_sds, self._PORT_SECURITY_SET_DATA)

    def _get_hw_offload_sd_from_port_alloc(self, port_alloc):
        return self._get_setting_data_from_port_alloc(port_alloc, self._hw_offload_sds, self._PORT_HW_OFFLOAD_SET_DATA)

    def _get_bandwidth_setting_data_from_port_alloc(self, port_alloc):
        return self._get_setting_data_from_port_alloc(port_alloc, self._bandwidth_sds, self._PORT_BANDWIDTH_SET_DATA)
