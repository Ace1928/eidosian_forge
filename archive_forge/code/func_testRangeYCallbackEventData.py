...
def testRangeYCallbackEventData(self):
    for viewport in [{'xaxis.range': [1, 4], 'yaxis.range': [-1, 5]}, {'xaxis.range[0]': 1, 'xaxis.range[1]': 4, 'yaxis.range[0]': -1, 'yaxis.range[1]': 5}]:
        event_data = RangeYCallback.get_event_data_from_property_update('viewport', viewport, self.fig_dict)
        self.assertEqual(event_data, {'first': {'y_range': (-1, 5)}, 'second': {'y_range': (-1, 5)}})