...
class power(Elementwise):
    """ Elementwise power function :math:`f(x) = x^p`.
    ...
    def __init__(self, x, p, max_denom: int=1024) -> None:
        ...
        if not (isinstance(self.p, cvxtypes.constant()) or isinstance(self.p, cvxtypes.parameter())):
            raise ValueError('The exponent `p` must be either a Constant or a Parameter; received ', type(p))
    ...
    def _grad(self, values):
        ...
        if self.p_rational is not None:
            ...
            raise ValueError('Cannot compute grad of parametrized power when parameter value is unspecified.')
        if p == 0:
    ...
    def _domain(self) -> List[Constraint]:
        ...
        if not isinstance(self._p_orig, cvxtypes.expression()):
            ...
            p = self.p.value
        if p is None:
            raise ValueError('Cannot compute domain of parametrized power when parameter value is unspecified.')
