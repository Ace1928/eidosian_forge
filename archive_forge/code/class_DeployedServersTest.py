...
class DeployedServersTest(common.HeatTestCase):
    ...
    def _server_create_software_config_poll_temp_url(self, server_name='server'):
    ...
    def test_server_create_deployment_swift_data(self):
        server_name = 'server'
        stack_name = '%s_s' % server_name
        tmpl, stack = self._setup_test_stack(stack_name, ds_deployment_data_tmpl)
        props = tmpl.t['resources']['server']['properties']
        props['software_config_transport'] = 'POLL_TEMP_URL'
        self.server_props = props
        resource_defns = tmpl.resource_definitions(stack)
    ...
    def test_server_create_deployment_swift_data_bad_object(self):
        server_name = 'server'
        stack_name = '%s_s' % server_name
        tmpl, stack = self._setup_test_stack(stack_name, ds_deployment_data_bad_object_tmpl)
        props = tmpl.t['resources']['server']['properties']
        props['software_config_transport'] = 'POLL_TEMP_URL'
        self.server_props = props
        resource_defns = tmpl.resource_definitions(stack)
    ...
    def test_server_create_deployment_swift_data_none_container(self):
        server_name = 'server'
        stack_name = '%s_s' % server_name
        tmpl, stack = self._setup_test_stack(stack_name, ds_deployment_data_none_container_tmpl)
        props = tmpl.t['resources']['server']['properties']
        props['software_config_transport'] = 'POLL_TEMP_URL'
        self.server_props = props
        resource_defns = tmpl.resource_definitions(stack)
    ...
    def test_server_create_software_config_metadata(self, fake_url):
        ...
        self.assertEqual({'os-collect-config': {'cfn': {'access_key_id': '4567', 'metadata_url': 'the-cfn-url/v1/', 'path': 'server.Metadata', 'secret_access_key': '8901', 'stack_name': 'server_sc_s'}, 'collectors': ['cfn', 'local'], 'polling_interval': 10}, 'deployments': []}, server.metadata_get())
    ...
    def test_server_create_software_config_zaqar(self):
        ...
        self.assertEqual({'os-collect-config': {'zaqar': {'user_id': '1234', 'password': server.password, 'auth_url': 'http://server.test:5000/v2.0', 'project_id': '8888', 'region_name': 'RegionOne', 'queue_id': queue_id}, 'collectors': ['zaqar', 'local']}, 'deployments': []}, server.metadata_get())
