    ...
    for note, base_freq in NOTE_FREQUENCIES.items():
        for octave in OCTAVE_RANGE:
            if base_freq * (2**octave) == freq:
                return note, octave
    return None, None  # if no matching note is found
...
def load_or_define_frequencies() -> Dict[int, float]:
    ...
    try:
        ...
        frequencies: Dict[int, float] = {
            pygame.K_a
            + i: NOTE_FREQUENCIES[notes_sequence[i % len(notes_sequence)]]
            * (2 ** (4 + i // len(notes_sequence)))
            for i in range(26)  # for letters a to z
        }
...
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            key = event.key
            note_name, octave = frequency_to_note_name_and_octave(key_to_frequency[key])
            filename: str = f"sounds/{note_name}{octave}.wav"
            if filename in sound_cache:
                sound_obj: pygame.mixer.Sound = sound_cache[filename]
                threading.Thread(
                    target=sound_obj.play, args=(-1,)
                ).start()  # Play sound indefinitely in a separate thread
