...
def build_server_cmd(cli_module: str, settings: 'AppSettings', host: Optional[str]=None, port: Optional[int]=None, num_server_workers: Optional[int]=1, server_backend: Optional[str]='uvicorn', server_class: Optional[str]='uvloop', reload_enabled: Optional[bool]=False) -> str:
    """
    ...
    if server_backend == 'uvicorn':
        ...
        if reload_enabled:
            server_cmd += f' --reload --reload-dir {settings.module_path.as_posix()}'
        if server_class and server_class in {'auto', 'asyncio', 'uvloop'}:
            server_cmd += f' --loop {server_class}'
    elif server_backend == 'hypercorn':
        server_cmd += f'{cli_module} --bind {host}:{port} --workers {num_server_workers}'
        if reload_enabled:
            server_cmd += ' --reload'
        if server_class and server_class in {'asyncio', 'uvloop', 'trio'}:
            server_cmd += f' --worker-class {server_class}'
    elif server_backend == 'gunicorn':
        server_cmd += f'--bind {host}:{port} --threads {num_server_workers}'
        if reload_enabled:
            server_cmd += ' --reload'
        if server_class:
            if server_class == 'uvicorn':
                server_cmd += ' --worker-class uvicorn.workers.UvicornWorker'
            elif server_class in {'sync', 'eventlet', 'gevent', 'tornado', 'gthread', 'gaiohttp'}:
                server_cmd += f' --worker-class {server_class}'
