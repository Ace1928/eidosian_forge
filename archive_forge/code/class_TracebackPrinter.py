...
class TracebackPrinter(object):
    ...
    def __init__(self, color_error: Union[str, int]='red', color_tb: Union[str, int]='blue', color_highlight: Union[str, int]='yellow', indent: int=2, tb_base: Optional[str]=None, tb_exclude: Tuple=tuple(), tb_range_start: int=-5, tb_range_end: int=-2):
    ...
    def _get_traceback(self, tb, highlight):
        ...
        tb_range = tb[self.tb_range_start:self.tb_range_end] if self.tb_range_end is not None else tb[self.tb_range_start:]
        tb_list = [self._format_traceback(path, line, fn, text, i, len(tb_range), highlight) for i, (path, line, fn, text) in enumerate(tb_range)]
        tb_data = '\n'.join(tb_list).strip()
        title = 'Traceback:'
        if self.supports_ansi:
            title = color(title, fg=self.color_tb, bold=True)
        return '\n\n{indent}{title}\n{indent}{tb}'.format(title=title, tb=tb_data, indent=self.indent)
