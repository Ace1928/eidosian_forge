...
def _SetContentTypeFromFile(src_url, dst_obj_metadata):
  ...
  Args:
    ...
    dst_obj_metadata: Object-specific metadata that should be overidden during
                     the copy.
  """
    if dst_obj_metadata.contentType is None and src_url.IsFileUrl() and (not src_url.IsStream()) and (not src_url.IsFifo()):
        ...
        if object_name != '-':
            ...
            if config.getbool('GSUtil', 'use_magicfile', False) and (not IS_WINDOWS):
                try:
                    p = subprocess.Popen(['file', '-b', '--mime', real_file_path], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                    output, error = p.communicate()
                    p.stdout.close()
                    p.stderr.close()
                    if p.returncode != 0 or error:
                        raise CommandException('Encountered error running "file -b --mime %s" (returncode=%d).\n%s' % (real_file_path, p.returncode, error))
                    content_type = output.rstrip()
                    content_type = six.ensure_str(content_type)
                except OSError as e:
                    raise CommandException('Encountered OSError running "file -b --mime %s"\n%s' % (real_file_path, e))
            else:
