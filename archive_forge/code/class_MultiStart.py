...
class MultiStart(object):
    ...
    CONFIG.declare('strategy', ConfigValue(default='rand', domain=In(strategies.keys()), description='Specify the restart strategy. Defaults to rand.', doc='Specify the restart strategy.\n\n        - "rand": random choice between variable bounds\n        - "midpoint_guess_and_bound": midpoint between current value and farthest bound\n        - "rand_guess_and_bound": random choice between current value and farthest bound\n        - "rand_distributed": random choice among evenly distributed values\n        - "midpoint": exact midpoint between the bounds. If using this option, multiple iterations are useless.\n        '))
    CONFIG.declare('solver', ConfigValue(default='ipopt', description='solver to use, defaults to ipopt'))
    CONFIG.declare('solver_args', ConfigValue(default={}, description='Dictionary of keyword arguments to pass to the solver.'))
    CONFIG.declare('iterations', ConfigValue(default=10, description='Specify the number of iterations, defaults to 10. If -1 is specified, the high confidence stopping rule will be used'))
    CONFIG.declare('stopping_mass', ConfigValue(default=0.5, description='Maximum allowable estimated missing mass of optima.', doc='Maximum allowable estimated missing mass of optima for the\n        high confidence stopping rule, only used with the random strategy.\n        The lower the parameter, the stricter the rule.\n        Value bounded in (0, 1].'))
    CONFIG.declare('stopping_delta', ConfigValue(default=0.5, description='1 minus the confidence level required for the stopping rule.', doc='1 minus the confidence level required for the stopping rule for the\n        high confidence stopping rule, only used with the random strategy.\n        The lower the parameter, the stricter the rule.\n        Value bounded in (0, 1].'))
    CONFIG.declare('suppress_unbounded_warning', ConfigValue(default=False, domain=bool, description='True to suppress warning for skipping unbounded variables.'))
    CONFIG.declare('HCS_max_iterations', ConfigValue(default=1000, description='Maximum number of iterations before interrupting the high confidence stopping rule.'))
    CONFIG.declare('HCS_tolerance', ConfigValue(default=0, description='Tolerance on HCS objective value equality. Defaults to Python float equality precision.'))
    ...
    def solve(self, model, **kwds):
        ...
        if obj.polynomial_degree() == 0:
            raise RuntimeError('Multistart solver received model with constant objective')
        objectives = []
        ...
        try:
            ...
            setattr(model, tmp_var_list_name, list(model.component_data_objects(ctype=Var, descend_into=True)))
            best_result = result = solver.solve(model, **config.solver_args)
            if result.solver.status is SolverStatus.ok and result.solver.termination_condition is tc.optimal:
            ...
            HCS_completed = False
            if using_HCS:
                assert config.strategy == 'rand', 'High confidence stopping rule requires rand strategy.'
                max_iter = config.HCS_max_iterations
            while num_iter < max_iter:
                if using_HCS and should_stop(objectives, config.stopping_mass, config.stopping_delta, config.HCS_tolerance):
                    HCS_completed = True
                    break
                num_iter += 1
            ...
            if using_HCS and (not HCS_completed):
                logger.warning('High confidence stopping rule was unable to complete after %s iterations. To increase this limit, change the HCS_max_iterations flag.' % num_iter)
