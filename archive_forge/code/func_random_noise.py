...
def random_noise(image, mode='gaussian', rng=None, clip=True, **kwargs):
    ...
    kwdefaults = {'mean': 0.0, 'var': 0.01, 'amount': 0.05, 'salt_vs_pepper': 0.5, 'local_vars': np.zeros_like(image) + 0.01}
    ...
    for key in kwargs:
        if key not in allowedkwargs[allowedtypes[mode]]:
            raise ValueError(f'{key} keyword not in allowed keywords {allowedkwargs[allowedtypes[mode]]}')
    ...
    if mode == 'gaussian':
        ...
        out = random_noise(image, mode='s&p', rng=rng, amount=kwargs['amount'], salt_vs_pepper=1.0)
    elif mode == 'pepper':
        out = random_noise(image, mode='s&p', rng=rng, amount=kwargs['amount'], salt_vs_pepper=0.0)
    elif mode == 's&p':
        out = image.copy()
        p = kwargs['amount']
