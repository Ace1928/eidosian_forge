...
class MultipleBackendFunctionalTest(test_utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.api_server_multiple_backend = ApiServerForMultipleBackend(self.test_dir, self.api_port, self.policy_file, sock=api_sock)
        self.scrubber_daemon = ScrubberDaemon(self.test_dir, self.policy_file)
        self.pid_files = [self.api_server_multiple_backend.pid_file, self.scrubber_daemon.pid_file]
        self.files_to_destroy = []
        self.launched_servers = []
        self._attached_server_logs = []
        self.addOnException(self.add_log_details_on_exception)
        if not self.disabled:
            self.addCleanup(self._reset_database, self.api_server_multiple_backend.sql_connection)
            self.addCleanup(self.cleanup)
            self._reset_database(self.api_server_multiple_backend.sql_connection)
    ...
    def cleanup(self):
        ...
        """
        servers = [self.api_server_multiple_backend, self.scrubber_daemon]
        for s in servers:
            try:
                s.stop()
            except Exception:
