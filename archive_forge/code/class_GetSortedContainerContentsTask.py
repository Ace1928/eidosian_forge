...
class GetSortedContainerContentsTask(task.Task):
    ...
    def __init__(self, container, output_path, exclude_pattern_strings=None, managed_folders_only=False, ignore_symlinks=True, recurse=False):
        """Initializes task.
    ...
    """
        ...
        self._container_query_path = container.storage_url.join('**' if recurse else '*').url_string
        ...
        if exclude_pattern_strings:
            ...
            if isinstance(container_url_trailing_delimiter, storage_url.FileUrl):
                ...
                container_prefix = container_url_trailing_delimiter.versionless_url_string
            self._exclude_patterns = regex_util.Patterns(exclude_pattern_strings, ignore_prefix_length=len(container_prefix))
        else:
            self._exclude_patterns = None
        self._managed_folders_only = managed_folders_only
        self._ignore_symlinks = ignore_symlinks
        self._worker_id = 'process {} thread {}'.format(os.getpid(), threading.get_ident())
    ...
    def execute(self, task_status_queue=None):
        ...
        file_iterator = iter(wildcard_iterator.get_wildcard_iterator(self._container_query_path, exclude_patterns=self._exclude_patterns, fields_scope=cloud_api.FieldsScope.RSYNC, files_only=not self._managed_folders_only, force_include_hidden_files=True, ignore_symlinks=self._ignore_symlinks, managed_folder_setting=managed_folder_setting))
        ...
        try:
            while True:
                ...
                log.status.Print('At {}, worker {} listed {}...'.format(self._container_query_path, self._worker_id, file_count))
                chunk_file_paths.append(rsync_command_util.get_hashed_list_file_path(self._container_query_path, chunk_count, is_managed_folder_list=self._managed_folders_only))
                sorted_encoded_chunk = sorted([rsync_command_util.get_csv_line_from_resource(x) for x in resources_chunk])
                sorted_encoded_chunk.append('')
                files.WriteFileContents(chunk_file_paths[-1], '\n'.join(sorted_encoded_chunk))
            ...
            if e.errno == errno.EMFILE:
                raise errors.Error('Too many open chunk files. Try increasing the size with `gcloud config set storage/rsync_list_chunk_size`. The current size is {}.'.format(chunk_size))
            raise e
        finally:
    ...
    def __eq__(self, other):
        ...
        return self._container_query_path == other._container_query_path and self._exclude_patterns == other._exclude_patterns and (self._managed_folders_only == other._managed_folders_only) and (self._ignore_symlinks == other._ignore_symlinks) and (self._output_path == other._output_path)