...
class DevicesCameraQualityAndRetention(object):
    ...
    def __init__(self, params, meraki):
        self.meraki = meraki
        self.new_object = dict(profileId=params.get('profileId'), motionBasedRetentionEnabled=params.get('motionBasedRetentionEnabled'), audioRecordingEnabled=params.get('audioRecordingEnabled'), restrictedBandwidthModeEnabled=params.get('restrictedBandwidthModeEnabled'), quality=params.get('quality'), resolution=params.get('resolution'), motionDetectorVersion=params.get('motionDetectorVersion'), serial=params.get('serial'))
    ...
    def update_all_params(self):
        ...
        if self.new_object.get('quality') is not None or self.new_object.get('quality') is not None:
            new_object_params['quality'] = self.new_object.get('quality') or self.new_object.get('quality')
        if self.new_object.get('resolution') is not None or self.new_object.get('resolution') is not None:
            new_object_params['resolution'] = self.new_object.get('resolution') or self.new_object.get('resolution')
    ...
    def get_object_by_name(self, name):
        result = None
        try:
            items = self.meraki.exec_meraki(family='camera', function='getDeviceCameraQualityAndRetention', params=self.get_all_params(name=name))
    ...
    def requires_update(self, current_obj):
        ...
        obj_params = [('profileId', 'profileId'), ('motionBasedRetentionEnabled', 'motionBasedRetentionEnabled'), ('audioRecordingEnabled', 'audioRecordingEnabled'), ('restrictedBandwidthModeEnabled', 'restrictedBandwidthModeEnabled'), ('quality', 'quality'), ('resolution', 'resolution'), ('motionDetectorVersion', 'motionDetectorVersion'), ('serial', 'serial')]
    ...
    def update(self):
        ...
        name = self.new_object.get('name')
        result = None
        result = self.meraki.exec_meraki(family='camera', function='updateDeviceCameraQualityAndRetention', params=self.update_all_params(), op_modifies=True)
        return result