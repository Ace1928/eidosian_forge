...
class AuthorizationCodeGrant(GrantTypeBase):
    ...
    def validate_token_request(self, request):
        ...
        if not self.request_validator.validate_code(request.client_id, request.code, request.client, request):
            log.debug('Client, %r (%r), is not allowed access to scopes %r.', request.client_id, request.client, request.scopes)
        ...
        if challenge is not None:
            ...
            if challenge_method not in self._code_challenge_methods:
                raise errors.ServerError(description='code_challenge_method {} is not supported.'.format(challenge_method), request=request)
        ...
        if not self.request_validator.confirm_redirect_uri(request.client_id, request.code, request.redirect_uri, request.client, request):
