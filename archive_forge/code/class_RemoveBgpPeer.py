...
class RemoveBgpPeer(base.UpdateCommand):
  ...
  *{command}* is used to delete a BGP peer from a Distributed Cloud
  ...
  """
    ...
    @staticmethod
    def Args(parser):
        resource_args.AddRouterResourceArg(parser, 'from which we delete a BGP peer', True)
        bgp_peer_parser = parser.add_mutually_exclusive_group(required=True)
        bgp_peer_parser.add_argument('--peer-names', type=arg_parsers.ArgList(), metavar='PEER_NAME', help='The list of names for peers being removed.\n        Only single value allowed currently.\n        ')
        bgp_peer_parser.add_argument('--peer-name', help='The name of the BGP peer being removed.')
