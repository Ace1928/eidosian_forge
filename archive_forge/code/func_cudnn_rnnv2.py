...
def cudnn_rnnv2(input: _atypes.TensorFuzzingAnnotation[TV_CudnnRNNV2_T], input_h: _atypes.TensorFuzzingAnnotation[TV_CudnnRNNV2_T], input_c: _atypes.TensorFuzzingAnnotation[TV_CudnnRNNV2_T], params: _atypes.TensorFuzzingAnnotation[TV_CudnnRNNV2_T], rnn_mode: str='lstm', input_mode: str='linear_input', direction: str='unidirectional', dropout: float=0, seed: int=0, seed2: int=0, is_training: bool=True, name=None):
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'CudnnRNNV2', name, input, input_h, input_c, params, 'rnn_mode', rnn_mode, 'input_mode', input_mode, 'direction', direction, 'dropout', dropout, 'seed', seed, 'seed2', seed2, 'is_training', is_training)
            ...
            pass
        try:
            return cudnn_rnnv2_eager_fallback(input, input_h, input_c, params, rnn_mode=rnn_mode, input_mode=input_mode, direction=direction, dropout=dropout, seed=seed, seed2=seed2, is_training=is_training, name=name, ctx=_ctx)
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('CudnnRNNV2', input=input, input_h=input_h, input_c=input_c, params=params, rnn_mode=rnn_mode, input_mode=input_mode, direction=direction, dropout=dropout, seed=seed, seed2=seed2, is_training=is_training, name=name)
    _result = _outputs[:]
