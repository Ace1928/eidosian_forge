...
class VmwareHostTcpipStack(PyVmomi):
    ...
    def __init__(self, module):
        ...
        self.net_stack_instance_keys = {'default': 'defaultTcpipStack', 'vmotion': 'vmotion', 'provisioning': 'vSphereProvisioning', 'vxlan': 'vxlan'}

    def check_enabled_net_stack_instance(self):
        """
        Make sure if enabled each the tcpip stack item in ESXi host.
        """
        self.enabled_net_stack_instance = {'default': False, 'vmotion': False, 'provisioning': False, 'vxlan': False}
    ...
    def execute(self):
        ...
        result = dict(changed=self.change_flag, diff=dict(before=OrderedDict(sorted(self.diff_config['before'].items())), after=OrderedDict(sorted(self.diff_config['after'].items()))))
        for key, value in self.enabled_net_stack_instance.items():
            if value:
