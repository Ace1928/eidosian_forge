...
class VespaStore(VectorStore):
    ...
    def __init__(self, app: Any, embedding_function: Optional[Embeddings]=None, page_content_field: Optional[str]=None, embedding_field: Optional[str]=None, input_field: Optional[str]=None, metadata_fields: Optional[List[str]]=None) -> None:
        """
    ...
    def add_texts(self, texts: Iterable[str], metadatas: Optional[List[dict]]=None, ids: Optional[List[str]]=None, **kwargs: Any) -> List[str]:
        ...
        for result in results:
            if not str(result.status_code).startswith('2'):
                raise RuntimeError(f'Could not add document to Vespa. Error code: {result.status_code}. Message: {result.json['message']}')
        return ids
    ...
    def _create_query(self, query_embedding: List[float], k: int=4, **kwargs: Any) -> Dict:
        ...
        if filter is not None:
            yql += f' and {filter}'
        query = {'yql': yql, f'input.query({input_embedding_field})': query_embedding, 'ranking': ranking_function, 'hits': hits}
        return query
    ...
    @classmethod
    def from_texts(cls: Type[VespaStore], texts: List[str], embedding: Embeddings, metadatas: Optional[List[dict]]=None, ids: Optional[List[str]]=None, **kwargs: Any) -> VespaStore:
