...
class Octopus(FileIOCalculator, EigenvalOccupationMixin):
    ...
    def __init__(self, restart=None, label=None, directory=None, atoms=None, command=None, **kwargs):
        """Create Octopus calculator.

        Label is always taken as a subdirectory.
        Restart is taken to be a label."""
        kwargs.pop('check_keywords', None)
        ...
        if directory is None:
            directory = 'ink-pool'
        self.kwargs = {}
        FileIOCalculator.__init__(self, restart=restart, directory=directory, atoms=atoms, command=command, **kwargs)
    ...
    def read_results(self):
        ...
        try:
            ...
            self.results.update(eigenvalues=eigs, occupations=occs, ibz_k_points=kpts, k_point_weights=kpt_weights)
    ...
    def recipe(cls, **kwargs):
        ...
        try:
            ...
            raise OctopusIOError('Expected recipe, but found useful physical output!')