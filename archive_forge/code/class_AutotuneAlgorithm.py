...
class AutotuneAlgorithm(enum.Enum):
  ...
  """
    ...
    def _to_proto(cls, obj):
        ...
        raise ValueError(f'Invalid `obj.` Supported values include `DEFAULT`, `HILL_CLIMB` `GRADIENT_DESCENT`, and `STAGE_BASED`. Got {obj.name}.')
    ...
    def _from_proto(cls, pb):
        ...
        raise ValueError(f'Invalid `pb.` Supported values include `DEFAULT`, `HILL_CLIMB`, `GRADIENT_DESCENT` and `STAGE_BASED`. Got {pb}.')