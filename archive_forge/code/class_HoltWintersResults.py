...
class HoltWintersResults(Results):
    ...
    def forecast(self, steps=1):
        ...
        try:
            ...
            if not isinstance(freq, int) and isinstance(self.model._index, (pd.DatetimeIndex, pd.PeriodIndex)):
    ...
    def summary(self):
        ...
        lookup = {'add': 'Additive', 'additive': 'Additive', 'mul': 'Multiplicative', 'multiplicative': 'Multiplicative', None: 'None'}
        ...
        top_left = [('Dep. Variable:', [dep_variable]), ('Model:', [model.__class__.__name__]), ('Optimized:', [str(np.any(self.optimized))]), ('Trend:', [lookup[self.model.trend]]), ('Seasonal:', [lookup[self.model.seasonal]]), ('Seasonal Periods:', [str(seasonal_periods)]), ('Box-Cox:', [str(box_cox_transform)]), ('Box-Cox Coeff.:', [str(box_cox_coeff)])]
        top_right = [('No. Observations:', [str(len(self.model.endog))]), ('SSE', [f'{self.sse:5.3f}']), ('AIC', [f'{self.aic:5.3f}']), ('BIC', [f'{self.bic:5.3f}']), ('AICC', [f'{self.aicc:5.3f}']), ('Date:', None), ('Time:', None)]
        smry = Summary()
        smry.add_table_2cols(self, gleft=top_left, gright=top_right, title=title)
        formatted = self.params_formatted
        ...
        for _, vals in formatted.iterrows():
            tab.append([_fmt(vals.iloc[1]), f'{vals.iloc[0]:>20}', f'{str(bool(vals.iloc[2])):>20}'])
        params_table = SimpleTable(tab, headers=['coeff', 'code', 'optimized'], title='', stubs=list(formatted.index))
    ...
    def simulate(self, nsimulations, anchor=None, repetitions=1, error='add', random_errors=None, random_state=None):
        ...
        if isinstance(random_errors, np.ndarray):
            if random_errors.shape != (nsimulations, repetitions):
                raise ValueError('If random_errors is an ndarray, it must have shape (nsimulations, repetitions)')
            eps = random_errors
