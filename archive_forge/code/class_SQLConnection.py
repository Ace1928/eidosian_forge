...
class SQLConnection(BaseConnection['Engine']):
    ...
    def query(self, sql: str, *, show_spinner: bool | str='Running `sql.query(...)`.', ttl: float | int | timedelta | None=None, index_col: str | list[str] | None=None, chunksize: int | None=None, params=None, **kwargs) -> DataFrame:
        ...
        **kwargs: dict
        ...
        def _query(sql: str, index_col=None, chunksize=None, params=None, **kwargs) -> DataFrame:
            import pandas as pd
        ...
        ttl_str = str(ttl).replace('.', '_')
        _query.__qualname__ = f'{_query.__qualname__}_{self._connection_name}_{ttl_str}'
        _query = cache_data(show_spinner=show_spinner, ttl=ttl)(_query)
