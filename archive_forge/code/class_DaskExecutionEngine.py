...
class DaskExecutionEngine(ExecutionEngine):
    ...
    def repartition(self, df: DataFrame, partition_spec: PartitionSpec) -> DaskDataFrame:
        df = self.to_df(df)
        if partition_spec.empty:
            return df
        if len(partition_spec.partition_by) > 0 and partition_spec.algo == 'default':
            return df
        p = partition_spec.get_num_partitions(**{KEYWORD_ROWCOUNT: lambda: df.persist().count(), KEYWORD_PARALLELISM: lambda: self.get_current_parallelism()})
        if partition_spec.algo == 'default':
            ddf: dd.DataFrame = df.native.repartition(npartitions=p) if p > 0 else df.native
        elif partition_spec.algo in ['hash', 'coarse']:
            ddf = hash_repartition(df.native, num=p if p > 0 else self.get_current_parallelism() * 2, cols=partition_spec.partition_by)
            ...
            ddf = rand_repartition(df.native, num=p if p > 0 else self.get_current_parallelism() * 2, cols=partition_spec.partition_by, seed=0)
        else:
            raise NotImplementedError(partition_spec.algo + ' partitioning is not supported')
    ...
    def join(self, df1: DataFrame, df2: DataFrame, how: str, on: Optional[List[str]]=None) -> DataFrame:
        ...
        if ntp1 != ntp2:
            ntp = key_schema.to_pandas_dtype(use_extension_types=True, use_arrow_dtype=FUGUE_DASK_USE_ARROW)
            if ntp1 != ntp:
    ...
    def union(self, df1: DataFrame, df2: DataFrame, distinct: bool=True) -> DataFrame:
        ...
        d = self.pl_utils.union(self.to_df(df1).native, self.to_df(df2).native, unique=distinct)
    ...
    def subtract(self, df1: DataFrame, df2: DataFrame, distinct: bool=True) -> DataFrame:
        assert_or_throw(distinct, NotImplementedError('EXCEPT ALL for DaskExecutionEngine'))
        assert_or_throw(df1.schema == df2.schema, lambda: ValueError(f'{df1.schema} != {df2.schema}'))
        d = self.pl_utils.except_df(self.to_df(df1).native, self.to_df(df2).native, unique=distinct)
        return DaskDataFrame(d, df1.schema, type_safe=False)

    def intersect(self, df1: DataFrame, df2: DataFrame, distinct: bool=True) -> DataFrame:
        assert_or_throw(distinct, NotImplementedError('INTERSECT ALL for DaskExecutionEngine'))
        assert_or_throw(df1.schema == df2.schema, lambda: ValueError(f'{df1.schema} != {df2.schema}'))
        d = self.pl_utils.intersect(self.to_df(df1).native, self.to_df(df2).native, unique=distinct)
        return DaskDataFrame(d, df1.schema, type_safe=False)
    ...
    def fillna(self, df: DataFrame, value: Any, subset: List[str]=None) -> DataFrame:
        assert_or_throw(not isinstance(value, list) and value is not None, ValueError('fillna value can not be a list or None'))
    ...
    def sample(self, df: DataFrame, n: Optional[int]=None, frac: Optional[float]=None, replace: bool=False, seed: Optional[int]=None) -> DataFrame:
        ...
        d = self.to_df(df).native.sample(n=n, frac=frac, replace=replace, random_state=seed)
        return DaskDataFrame(d, df.schema, type_safe=False)
    ...
    def take(self, df: DataFrame, n: int, presort: str, na_position: str='last', partition_spec: Optional[PartitionSpec]=None) -> DataFrame:
        ...
        if len(partition_spec.partition_by) == 0:
            if len(_presort.keys()) == 0:
                ...
                d = d.map_partitions(_partition_take, n, _presort, meta=meta).reset_index(drop=True).compute()
                d = d.sort_values(list(_presort.keys()), ascending=list(_presort.values()), na_position=na_position).head(n)
        else:
            d = d.groupby(partition_spec.partition_by, dropna=False).apply(_partition_take, n=n, presort=_presort, meta=meta).reset_index(drop=True)
        return DaskDataFrame(d, df.schema, type_safe=False)

    def load_df(self, path: Union[str, List[str]], format_hint: Any=None, columns: Any=None, **kwargs: Any) -> DaskDataFrame:
    ...
    def save_df(self, df: DataFrame, path: str, format_hint: Any=None, mode: str='overwrite', partition_spec: Optional[PartitionSpec]=None, force_single: bool=False, **kwargs: Any) -> None:
        partition_spec = partition_spec or PartitionSpec()
        if force_single:
            self._native.save_df(df, path=path, format_hint=format_hint, mode=mode, partition_spec=partition_spec, force_single=force_single, **kwargs)
        else:
