...
class SoftwareDeploymentController(object):
    ...
    def index(self, req):
        ...
        param_types = {'server_id': util.PARAM_TYPE_SINGLE}
    ...
    def metadata(self, req, server_id):
        ...
        """
        sds = self.rpc_client.metadata_software_deployments(req.context, server_id=server_id)
        return {'metadata': sds}

    @util.registered_policy_enforce
    def show(self, req, deployment_id):
        """Gets detailed information for a software deployment."""
        sd = self.rpc_client.show_software_deployment(req.context, deployment_id)
        return {'software_deployment': sd}

    @util.registered_policy_enforce
    def create(self, req, body):
        """Create a new software deployment."""
        create_data = dict(((k, body.get(k)) for k in ('config_id', 'server_id', 'input_values', 'action', 'status', 'status_reason', 'stack_user_project_id')))
        sd = self.rpc_client.create_software_deployment(req.context, **create_data)
        return {'software_deployment': sd}

    @util.registered_policy_enforce
    def update(self, req, deployment_id, body):
        """Update an existing software deployment."""
        update_data = dict(((k, body.get(k)) for k in ('config_id', 'input_values', 'output_values', 'action', 'status', 'status_reason') if body.get(k, None) is not None))
        sd = self.rpc_client.update_software_deployment(req.context, deployment_id, **update_data)
        return {'software_deployment': sd}

    @util.registered_policy_enforce
    def delete(self, req, deployment_id):
        """Delete an existing software deployment."""
        res = self.rpc_client.delete_software_deployment(req.context, deployment_id)
        if res is not None:
