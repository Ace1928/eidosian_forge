...
class SFTPFile(BufferedFile):
    ...
    def _write(self, data):
        ...
        sftp_async_request = self.sftp._async_request(type(None), CMD_WRITE, self.handle, int64(self._realpos), data[:chunk])
    ...
    def check(self, hash_algorithm, offset=0, length=0, block_size=0):
        ...
        .. versionadded:: 1.4
        """
        t, msg = self.sftp._request(CMD_EXTENDED, 'check-file', self.handle, hash_algorithm, int64(offset), int64(length), block_size)
        msg.get_text()
        msg.get_text()
