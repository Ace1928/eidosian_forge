...
class ProjectFileManager(GetMixin, CreateMixin, UpdateMixin, DeleteMixin, RESTManager):
    ...
    _create_attrs = RequiredOptional(required=('file_path', 'branch', 'content', 'commit_message'), optional=('encoding', 'author_email', 'author_name'))
    _update_attrs = RequiredOptional(required=('file_path', 'branch', 'content', 'commit_message'), optional=('encoding', 'author_email', 'author_name'))
    ...
    @cli.register_custom_action('ProjectFileManager', ('file_path', 'branch', 'content', 'commit_message'), ('encoding', 'author_email', 'author_name'))
    @exc.on_http_error(exc.GitlabCreateError)
    def create(self, data: Optional[Dict[str, Any]]=None, **kwargs: Any) -> ProjectFile:
        """Create a new object.
    ...
    def update(self, file_path: str, new_data: Optional[Dict[str, Any]]=None, **kwargs: Any) -> Dict[str, Any]:
    ...
    @cli.register_custom_action('ProjectFileManager', ('file_path', 'branch', 'commit_message'))
    @exc.on_http_error(exc.GitlabDeleteError)
    def delete(self, file_path: str, branch: str, commit_message: str, **kwargs: Any) -> None:
    ...
    def raw(self, file_path: str, ref: str, streamed: bool=False, action: Optional[Callable[..., Any]]=None, chunk_size: int=1024, *, iterator: bool=False, **kwargs: Any) -> Optional[Union[bytes, Iterator[Any]]]:
        ...
        query_data = {'ref': ref}
        result = self.gitlab.http_get(path, query_data=query_data, streamed=streamed, raw=True, **kwargs)
        if TYPE_CHECKING:
