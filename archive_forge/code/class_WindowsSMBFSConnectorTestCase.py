...
class WindowsSMBFSConnectorTestCase(test_base.WindowsConnectorTestBase):
    ...
    def test_connect_volume(self, mock_ensure_mounted, mock_get_disk_path):
        device_info = self._connector.connect_volume(mock.sentinel.conn_props)
        expected_info = dict(type='file', path=mock_get_disk_path.return_value)
    ...
    def test_disconnect_volume(self, mock_get_export_path, mock_get_disk_path):
        self._connector.disconnect_volume(mock.sentinel.conn_props, mock.sentinel.dev_info)
        mock_get_disk_path.assert_called_once_with(mock.sentinel.conn_props)
        self._vhdutils.detach_virtual_disk.assert_called_once_with(mock_get_disk_path.return_value)
    ...
    def test_get_search_path(self):
        ...
        self.assertEqual(search_path, self._remotefs.get_mount_base.return_value)
