...
def wireless_controller_hotspot20_anqp_nai_realm(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('wireless-controller.hotspot20', 'anqp-nai-realm', filtered_data, vdom=vdom)
        current_data = fos.get('wireless-controller.hotspot20', 'anqp-nai-realm', vdom=vdom, mkey=mkey)
    ...
    if state == 'present' or state is True:
        return fos.set('wireless-controller.hotspot20', 'anqp-nai-realm', data=filtered_data, vdom=vdom)
    elif state == 'absent':
        return fos.delete('wireless-controller.hotspot20', 'anqp-nai-realm', mkey=filtered_data['name'], vdom=vdom)
    else:
