...
def plot_pair(ax, plotters, numvars, figsize, textsize, kind, scatter_kwargs, kde_kwargs, hexbin_kwargs, gridsize, colorbar, divergences, diverging_mask, divergences_kwargs, flat_var_names, backend_kwargs, marginal_kwargs, show, marginals, point_estimate, point_estimate_kwargs, point_estimate_marker_kwargs, reference_values, reference_values_kwargs):
    """Bokeh pair plot."""
    ...
    if reference_values:
        ...
        if difference:
            ...
            warnings.warn('Argument reference_values does not include reference value for: {}'.format(', '.join(warn)), UserWarning)
    if reference_values:
        reference_values_copy = {}
        label = []
        for variable in list(reference_values.keys()):
    ...
    if vars_to_plot < numvars:
        warnings.warn("rcParams['plot.max_subplots'] ({max_plots}) is smaller than the number of resulting pair plots with these variables, generating only a {side}x{side} grid".format(max_plots=max_plots, side=vars_to_plot), UserWarning)
        numvars = vars_to_plot
    if numvars == 2:
        offset = 1
    else:
        offset = 2
    figsize, _, _, _, _, markersize = _scale_fig_size(figsize, textsize, numvars - offset, numvars - offset)
    ...
    if divergences:
        source_nondiv = CDSView(source=source, filters=[GroupFilter(column_name=divergenve_name, group='0')])
        source_div = CDSView(source=source, filters=[GroupFilter(column_name=divergenve_name, group='1')])
    ...
    if ax is None:
        ...
        for row in range(numvars - marginals_offset):
            row_ax = []
            var1 = flat_var_names[row + marginals_offset] if tmp_flat_var_names is None else tmp_flat_var_names[row + marginals_offset]
            for col in range(numvars - marginals_offset):
                var2 = flat_var_names[col] if tmp_flat_var_names is None else tmp_flat_var_names[col]
                backend_kwargs_copy = backend_kwargs.copy()
                if 'scatter' in kind:
                    tooltips = [(var2, f'@{{{var2}}}'), (var1, f'@{{{var1}}}')]
    ...
    for i in range(0, numvars - marginals_offset):
        ...
        for j in range(0, numvars - marginals_offset):
            var2 = flat_var_names[j + marginals_offset] if tmp_flat_var_names is None else tmp_flat_var_names[j + marginals_offset]
            if j == i and marginals:
                rotate = numvars == 2 and j == 1
                var1_dist = plotters[i][-1].flatten()
                plot_dist(var1_dist, ax=ax[j, i], show=False, backend='bokeh', rotated=rotate, **marginal_kwargs)
                ...
                if 'kde' in kind:
                    ...
                    plot_kde(var1_kde, var2_kde, ax=ax[j, i], backend='bokeh', backend_kwargs={}, show=False, **deepcopy(kde_kwargs))
                if 'hexbin' in kind:
                    var1_hexbin = plotters[i][-1].flatten()
                    var2_hexbin = plotters[j + marginals_offset][-1].flatten()
                    ax[j, i].grid.visible = False
                    ax[j, i].hexbin(var1_hexbin, var2_hexbin, **hexbin_kwargs)
                if divergences:
                    ax[j, i].circle(var1, var2, source=source, view=source_div, **divergences_kwargs)
                if point_estimate:
                    ...
                    if marginals:
                        ...
                        ax_pe_vline = Span(location=pe_last, dimension='height', **point_estimate_kwargs)
                        ax[-1, -1].add_layout(ax_pe_vline)
                        if numvars == 2:
                            ax_pe_hline = Span(location=pe_last, dimension='width', **point_estimate_kwargs)
