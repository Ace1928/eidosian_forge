...
class VCLNodeDriver(NodeDriver):
    ...
    :type      host: ``str``
    """
    NODE_STATE_MAP = {'ready': NodeState.RUNNING, 'failed': NodeState.TERMINATED, 'timedout': NodeState.TERMINATED, 'loading': NodeState.PENDING, 'time': NodeState.PENDING, 'future': NodeState.PENDING, 'error': NodeState.UNKNOWN, 'notready': NodeState.PENDING, 'notavailable': NodeState.TERMINATED, 'success': NodeState.PENDING}
    ...
    def __init__(self, key, secret, secure=True, host=None, port=None, *args, **kwargs):
        ...
        """
        if not host:
            raise Exception('When instantiating VCL driver directly ' + 'you also need to provide host')
        super().__init__(key, secret, *args, secure=True, host=None, port=None, **kwargs)
    ...
    def create_node(self, image, start=None, length='60'):
        ...
        return Node(id=res['requestid'], name=image.name, state=self.NODE_STATE_MAP[res['status']], public_ips=[], private_ips=[], driver=self, image=image.name)
    ...
    def _to_status(self, requestid, imagename, ipaddr):
        ...
        return Node(id=requestid, name=imagename, state=self.NODE_STATE_MAP[res['status']], public_ips=public_ips, private_ips=[], driver=self, image=imagename, extra=extra)
