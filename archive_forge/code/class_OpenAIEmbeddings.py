...
@deprecated(since='0.0.9', removal='0.2.0', alternative_import='langchain_openai.OpenAIEmbeddings')
class OpenAIEmbeddings(BaseModel, Embeddings):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        values['openai_api_type'] = get_from_dict_or_env(values, 'openai_api_type', 'OPENAI_API_TYPE', default='')
        ...
        if values['openai_api_type'] in ('azure', 'azure_ad', 'azuread'):
            ...
            default_api_version = ''
        values['openai_api_version'] = get_from_dict_or_env(values, 'openai_api_version', 'OPENAI_API_VERSION', default=default_api_version)
        ...
        try:
            ...
            if is_openai_v1():
                if values['openai_api_type'] in ('azure', 'azure_ad', 'azuread'):
                    warnings.warn('If you have openai>=1.0.0 installed and are using Azure, please use the `AzureOpenAIEmbeddings` class.')
                client_params = {'api_key': values['openai_api_key'], 'organization': values['openai_organization'], 'base_url': values['openai_api_base'], 'timeout': values['request_timeout'], 'max_retries': values['max_retries'], 'default_headers': values['default_headers'], 'default_query': values['default_query'], 'http_client': values['http_client']}
    ...
    def _invocation_params(self) -> Dict[str, Any]:
        if is_openai_v1():
            ...
            openai_args = {'model': self.model, 'request_timeout': self.request_timeout, 'headers': self.headers, 'api_key': self.openai_api_key, 'organization': self.openai_organization, 'api_base': self.openai_api_base, 'api_type': self.openai_api_type, 'api_version': self.openai_api_version, **self.model_kwargs}
    ...
    async def _aget_len_safe_embeddings(self, texts: List[str], *, engine: str, chunk_size: Optional[int]=None) -> List[List[float]]:
        """
        ...
        for i in range(0, len(tokens), _chunk_size):
            response = await async_embed_with_retry(self, input=tokens[i:i + _chunk_size], **self._invocation_params)
            if not isinstance(response, dict):
                response = response.dict()
            batched_embeddings.extend((r['embedding'] for r in response['data']))
        results: List[List[List[float]]] = [[] for _ in range(len(texts))]
