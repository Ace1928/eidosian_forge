  ...
  """
    ...
    def __call__(self, step):
        with ops.name_scope_v2(self.name or 'PolynomialDecay') as name:
            ...
            if self.cycle:
                multiplier = array_ops.where_v2(math_ops.equal(global_step_recomp, 0), 1.0, math_ops.ceil(global_step_recomp / self.decay_steps))
                decay_steps_recomp = math_ops.multiply(decay_steps_recomp, multiplier)
            else:
                global_step_recomp = math_ops.minimum(global_step_recomp, decay_steps_recomp)
            p = math_ops.divide(global_step_recomp, decay_steps_recomp)
            return math_ops.add(math_ops.multiply(initial_learning_rate - end_learning_rate, math_ops.pow(1 - p, power)), end_learning_rate, name=name)

    def get_config(self):
        return {'initial_learning_rate': self.initial_learning_rate, 'decay_steps': self.decay_steps, 'end_learning_rate': self.end_learning_rate, 'power': self.power, 'cycle': self.cycle, 'name': self.name}