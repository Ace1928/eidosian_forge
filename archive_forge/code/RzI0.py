...
def create_tensor_wave(signal: np.ndarray, sr: int) -> dict:
    ...
    try:
        tensor_waves = {
            ...
            "chromagram": compute_chromagram(signal, sr),
            "mel_spectrogram": compute_mel_spectrogram(signal, sr),
            "power_spectrum": compute_power_spectrum(signal),
            "rms_energy": compute_rms_energy(signal),
            "spectrogram": compute_spectrogram(signal, sr),
            "tempogram": compute_tempogram(signal, sr),
            "tonnetz": compute_tonnetz(signal, sr),
            "3d_surface": compute_3d_surface(signal, sr),
            "pca": compute_pca(signal),
            "kmeans": compute_kmeans(signal),
        }
