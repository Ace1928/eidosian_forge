...
def publisher_factory(partition: Partition):
    ...
    def connection_factory(requests: AsyncIterator[PublishRequest]):
        ...
        return client_cache.get().publish(requests, metadata=list(final_metadata.items()))
    initial_request = InitialPublishRequest(topic=str(topic), partition=partition.value)
    if client_id:
        initial_request.client_id = client_id.value
    return SinglePartitionPublisher(initial_request, per_partition_batching_settings, GapicConnectionFactory(connection_factory), PublishSequenceNumber(0))