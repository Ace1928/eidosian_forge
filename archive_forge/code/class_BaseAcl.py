...
class BaseAcl(object):
    ...
    def get_wildcard_mask(self):
        ...
        if value < 8:
            mask_int[0] = str(int(~(255 << 8 - value % 8) & 255))
        if value >= 8:
            mask_int[0] = '0'
            mask_int[1] = str(int(~(255 << 16 - value % 16) & 255))
        if value >= 16:
            mask_int[1] = '0'
            mask_int[2] = str(int(~(255 << 24 - value % 24) & 255))
        if value >= 24:
            mask_int[2] = '0'
            mask_int[3] = str(int(~(255 << 32 - value % 32) & 255))
        if value == 32:
            mask_int[3] = '0'
        ...
        if self.acl_name:
            if self.acl_name.isdigit():
                if int(self.acl_name) < 2000 or int(self.acl_name) > 2999:
                    self.module.fail_json(msg='Error: The value of acl_name is out of [2000-2999] for base ACL.')
                if self.acl_num:
    ...
    def get_existing(self):
        ...
        self.existing['base_rule_info'] = self.cur_base_rule_cfg['base_rule_info']
    ...
    def get_end_state(self):
        ...
        self.end_state['base_rule_info'] = self.cur_base_rule_cfg['base_rule_info']
    ...
    def delete_base_rule(self):
        ...
        conf_str = CE_DELETE_ACL_BASE_RULE_HEADER % (self.acl_name, self.rule_name)
        if self.rule_id:
            conf_str += '<aclRuleID>%s</aclRuleID>' % self.rule_id
        if self.rule_action:
            conf_str += '<aclAction>%s</aclAction>' % self.rule_action
