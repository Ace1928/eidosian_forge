...
class NetAppOntapSubnet:
    ...
    def get_subnet(self, name=None):
        ...
        if result.get_child_by_name('num-records') and int(result.get_child_content('num-records')) == 1:
            ...
            return_value = {'name': name, 'broadcast_domain': broadcast_domain, 'gateway': gateway, 'ip_ranges': ip_ranges, 'ipspace': ipspace, 'subnet': subnet}
        return return_value
    ...
    def create_subnet(self):
        ...
        try:
            ...
            self.module.fail_json(msg='Error creating subnet %s: %s' % (self.parameters.get('name'), to_native(error)), exception=traceback.format_exc())
    ...
    def delete_subnet(self):
        ...
        subnet_delete = netapp_utils.zapi.NaElement.create_node_with_children('net-subnet-destroy', **{'subnet-name': self.parameters.get('name')})
        if self.parameters.get('ipspace'):
            subnet_delete.add_new_child('ipspace', self.parameters.get('ipspace'))
        try:
            self.server.invoke_successfully(subnet_delete, True)
        except netapp_utils.zapi.NaApiError as error:
            self.module.fail_json(msg='Error deleting subnet %s: %s' % (self.parameters.get('name'), to_native(error)), exception=traceback.format_exc())
    ...
    def modify_subnet(self, modify):
        ...
        try:
            ...
            self.module.fail_json(msg='Error modifying subnet %s: %s' % (self.parameters.get('name'), to_native(error)), exception=traceback.format_exc())
    ...
    def apply(self):
        ...
        if cd_action == 'create' and self.parameters.get('from_name'):
            ...
            if current is None:
                self.module.fail_json(msg='Error renaming: subnet %s does not exist' % self.parameters.get('from_name'))
            rename = True
            cd_action = None
            if self.use_rest:
                rename = False
        if self.parameters['state'] == 'present' and current:
            ...
            if 'broadcast_domain' in modify:
                self.module.fail_json(msg='Error modifying subnet %s: cannot modify broadcast_domain parameter, desired "%s", currrent "%s"' % (self.parameters.get('name'), self.parameters.get('broadcast_domain'), current.get('broadcast_domain')))
        if cd_action == 'create':
