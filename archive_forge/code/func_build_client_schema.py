...
def build_client_schema(introspection):
    ...
    def get_named_type(type_name):
        ...
        if not type_introspection:
            raise Exception('Invalid or incomplete schema, unknown type: {}. Ensure that a full introspection query is used in order to build a client schema.'.format(type_name))
    ...
    def build_type(type):
        ...
        if not handler:
            raise Exception('Invalid or incomplete schema, unknown kind: {}. Ensure that a full introspection query is used in order to build a client schema.'.format(type_kind))
        return handler(type)
    ...
    type_builders = {TypeKind.SCALAR: build_scalar_def, TypeKind.OBJECT: build_object_def, TypeKind.INTERFACE: build_interface_def, TypeKind.UNION: build_union_def, TypeKind.ENUM: build_enum_def, TypeKind.INPUT_OBJECT: build_input_object_def}
    ...
    def build_input_value(input_value_introspection, argument_type):
        input_value = argument_type(description=input_value_introspection['description'], type=get_input_type(input_value_introspection['type']), default_value=build_default_value(input_value_introspection))
        return input_value
    ...
    def build_directive(directive_introspection):
        ...
        return GraphQLDirective(name=directive_introspection['name'], description=directive_introspection.get('description'), args=build_input_value_def_map(directive_introspection.get('args', {}), GraphQLArgument), locations=locations)
    ...
    subscription_type = get_object_type(schema_introspection['subscriptionType']) if schema_introspection.get('subscriptionType') else None
    directives = [build_directive(d) for d in schema_introspection['directives']] if schema_introspection['directives'] else []
