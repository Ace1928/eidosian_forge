...
class NetAppOntapLicense:
    ...
    def remove_unused_licenses(self):
        ...
        try:
            self.server.invoke_successfully(remove_unused, enable_tunneling=False)
        except netapp_utils.zapi.NaApiError as error:
            self.module.fail_json(msg='Error removing unused licenses: %s' % to_native(error), exception=traceback.format_exc())
    ...
    def get_key(self, error, body):
        needle = 'Failed to install the license at index (\\d+)'
    ...
    def apply(self):
        ...
        if changed and (not self.module.check_mode):
            ...
            if not changed_keys:
                changed = False
        if error:
            error = 'Error: ' + ('some licenses were updated, but others were in conflict: ' if changed_keys else 'adding licenses: ') + error
