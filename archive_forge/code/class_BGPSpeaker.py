...
class BGPSpeaker(object):
    ...
    def __init__(self, as_number, router_id, bgp_server_hosts=DEFAULT_BGP_SERVER_HOSTS, bgp_server_port=DEFAULT_BGP_SERVER_PORT, refresh_stalepath_time=DEFAULT_REFRESH_STALEPATH_TIME, refresh_max_eor_time=DEFAULT_REFRESH_MAX_EOR_TIME, best_path_change_handler=None, adj_rib_in_change_handler=None, peer_down_handler=None, peer_up_handler=None, ssh_console=False, ssh_port=None, ssh_host=None, ssh_host_key=None, label_range=DEFAULT_LABEL_RANGE, allow_local_as_in_count=0, cluster_id=None, local_pref=DEFAULT_LOCAL_PREF):
        super(BGPSpeaker, self).__init__()
        settings = {LOCAL_AS: as_number, ROUTER_ID: router_id, BGP_SERVER_HOSTS: bgp_server_hosts, BGP_SERVER_PORT: bgp_server_port, REFRESH_STALEPATH_TIME: refresh_stalepath_time, REFRESH_MAX_EOR_TIME: refresh_max_eor_time, LABEL_RANGE: label_range, ALLOW_LOCAL_AS_IN_COUNT: allow_local_as_in_count, CLUSTER_ID: cluster_id, LOCAL_PREF: local_pref}
        ...
        if ssh_console:
            ...
            ssh_settings = {ssh.SSH_PORT: ssh_port or ssh.DEFAULT_SSH_PORT, ssh.SSH_HOST: ssh_host or ssh.DEFAULT_SSH_HOST, ssh.SSH_HOST_KEY: ssh_host_key or ssh.DEFAULT_SSH_HOST_KEY}
    ...
    def _notify_best_path_changed(self, path, is_withdraw):
        if not path.source or not isinstance(path, (Ipv4Path, Ipv6Path, Vpnv4Path, Vpnv6Path, EvpnPath)):
            return
    ...
    def _notify_adj_rib_in_changed(self, peer, route):
        ...
        if self._adj_rib_in_change_handler:
            self._adj_rib_in_change_handler(EventPrefix(route.path, route.path.is_withdraw), peer.ip_address, peer.remote_as)

    def _init_signal_listeners(self):
        CORE_MANAGER.get_core_service()._signal_bus.register_listener(BgpSignalBus.BGP_BEST_PATH_CHANGED, lambda _, info: self._notify_best_path_changed(info['path'], info['is_withdraw']))
        CORE_MANAGER.get_core_service()._signal_bus.register_listener(BgpSignalBus.BGP_ADJ_RIB_IN_CHANGED, lambda _, info: self._notify_adj_rib_in_changed(info['peer'], info['received_route']))
        CORE_MANAGER.get_core_service()._signal_bus.register_listener(BgpSignalBus.BGP_ADJ_DOWN, lambda _, info: self._notify_peer_down(info['peer']))
        CORE_MANAGER.get_core_service()._signal_bus.register_listener(BgpSignalBus.BGP_ADJ_UP, lambda _, info: self._notify_peer_up(info['peer']))
    ...
    def neighbor_state_get(self, address=None, format='json'):
        ...
        - 'cli'
        """
        show = {'params': ['neighbor', 'summary'], 'format': format}
        if address:
    ...
    def evpn_prefix_del(self, route_type, route_dist, esi=0, ethernet_tag_id=None, mac_addr=None, ip_addr=None, ip_prefix=None):
        ...
        if route_type == EVPN_ETH_AUTO_DISCOVERY:
            ...
            kwargs.update({EVPN_ETHERNET_TAG_ID: ethernet_tag_id, MAC_ADDR: mac_addr, IP_ADDR: ip_addr})
        elif route_type == EVPN_MULTICAST_ETAG_ROUTE:
            kwargs.update({EVPN_ETHERNET_TAG_ID: ethernet_tag_id, IP_ADDR: ip_addr})
        elif route_type == EVPN_ETH_SEGMENT:
            kwargs.update({EVPN_ESI: esi, IP_ADDR: ip_addr})
        elif route_type == EVPN_IP_PREFIX_ROUTE:
            kwargs.update({EVPN_ETHERNET_TAG_ID: ethernet_tag_id, IP_PREFIX: ip_prefix})
        else:
            raise ValueError('Unsupported EVPN route type: %s' % route_type)
        call(func_name, **kwargs)
    ...
    def neighbors_get(self, format='json'):
        ...
        - 'cli'
        """
        show = {'params': ['neighbor'], 'format': format}
        return call('operator.show', **show)
