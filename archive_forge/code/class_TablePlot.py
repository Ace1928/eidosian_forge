...
class TablePlot(ElementPlot):
    ...
    def get_data(self, element, ranges, style, **kwargs):
        ...
        cells = dict(values=[[d.pprint_value(v) for v in element.dimension_values(d)] for d in element.dimensions()])
    ...
    def init_layout(self, key, element, ranges, **kwargs):
        return dict(width=self.width, height=self.height, title=self._format_title(key, separator=' '), plot_bgcolor=self.bgcolor)