...
class BaseTrainer(abc.ABC):
    ...
    **How do I create a new Trainer?**
    ...
    def __init__(self, *, scaling_config: Optional[ScalingConfig]=None, run_config: Optional[RunConfig]=None, datasets: Optional[Dict[str, GenDataset]]=None, metadata: Optional[Dict[str, Any]]=None, resume_from_checkpoint: Optional[Checkpoint]=None, preprocessor: Optional['Preprocessor']=None):
        self.scaling_config = scaling_config if scaling_config is not None else ScalingConfig()
    ...
    @classmethod
    def restore(cls: Type['BaseTrainer'], path: Union[str, os.PathLike], storage_filesystem: Optional[pyarrow.fs.FileSystem]=None, datasets: Optional[Dict[str, GenDataset]]=None, preprocessor: Optional['Preprocessor']=None, scaling_config: Optional[ScalingConfig]=None, **kwargs) -> 'BaseTrainer':
        ...
        if not cls.can_restore(path, storage_filesystem):
            raise ValueError(f'Invalid restore path: {path}. Make sure that this path exists and is the experiment directory that results from a call to `trainer.fit()`.')
        ...
        if trainer_cls is not cls:
            warnings.warn(f'Invalid trainer type. You are attempting to restore a trainer of type {trainer_cls} with `{cls.__name__}.restore`, which will most likely fail. Use `{trainer_cls.__name__}.restore` instead.')
        original_datasets = param_dict.pop('datasets', {})
        if original_datasets and (not datasets):
            raise ValueError(f'The following datasets need to be provided again on restore: {list(original_datasets.keys())}\nUse {cls.__name__}.restore(..., datasets=datasets) with the datasets that were provided to the original trainer.')
        datasets = datasets or {}
        if set(original_datasets) != set(datasets):
            raise ValueError(f"The provided datasets don't match the original dataset keys.\n  Expected datasets for the keys: {list(original_datasets.keys())}\n  Actual datasets provided: {list(datasets.keys())}")
        ...
        try:
            ...
            raise ValueError(f'Trainer restoration failed (see above for the stack trace). Make sure that you use the right trainer class to restore: `{cls.__name__}.restore`\n') from e
    ...
    @PublicAPI(stability='alpha')
    @classmethod
    def can_restore(cls: Type['BaseTrainer'], path: Union[str, os.PathLike], storage_filesystem: Optional[pyarrow.fs.FileSystem]=None) -> bool:
    ...
    def __repr__(self):
        default_values: Dict[str, Any] = {'scaling_config': ScalingConfig(), 'run_config': RunConfig(), 'datasets': {}, 'starting_checkpoint': None}
    ...
    def _validate_attributes(self):
        ...
        if not isinstance(self.run_config, RunConfig):
            raise ValueError(f'`run_config` should be an instance of `ray.train.RunConfig`, found {type(self.run_config)} with value `{self.run_config}`.')
        if not isinstance(self.scaling_config, ScalingConfig):
            raise ValueError(f'`scaling_config` should be an instance of `ScalingConfig`, found {type(self.scaling_config)} with value `{self.scaling_config}`.')
        if not isinstance(self.datasets, dict):
            raise ValueError(f'`datasets` should be a dict mapping from a string to `ray.data.Dataset` objects, found {type(self.datasets)} with value `{self.datasets}`.')
        else:
            for key, dataset in self.datasets.items():
                if not isinstance(dataset, ray.data.Dataset) and (not callable(dataset)):
                    raise ValueError(f"The Dataset under '{key}' key is not a `ray.data.Dataset`. Received {dataset} instead.")
        self.metadata = self.metadata or {}
        if not isinstance(self.metadata, dict):
            raise TypeError(f'The provided metadata must be a dict, was {type(self.metadata)}.')
        try:
            self.metadata = json.loads(json.dumps(self.metadata))
        except Exception as e:
            raise ValueError(f'The provided metadata must be JSON-serializable: {self.metadata}: {e}')
        if self.preprocessor is not None and (not isinstance(self.preprocessor, ray.data.Preprocessor)):
            raise ValueError(f'`preprocessor` should be an instance of `ray.data.Preprocessor`, found {type(self.preprocessor)} with value `{self.preprocessor}`.')
        if self.starting_checkpoint is not None and (not isinstance(self.starting_checkpoint, Checkpoint)):
            raise ValueError(f'`resume_from_checkpoint` should be an instance of `ray.train.Checkpoint`, found {type(self.starting_checkpoint)} with value `{self.starting_checkpoint}`.')

    @classmethod
    def _validate_scaling_config(cls, scaling_config: ScalingConfig) -> ScalingConfig:
        """Returns scaling config dataclass after validating updated keys."""
        ensure_only_allowed_dataclass_keys_updated(dataclass=scaling_config, allowed_keys=cls._scaling_config_allowed_keys)
    ...
    def preprocess_datasets(self) -> None:
        ...
        if self.preprocessor:
            ...
            if train_dataset and self.preprocessor.fit_status() in (ray.data.Preprocessor.FitStatus.NOT_FITTED, ray.data.Preprocessor.FitStatus.PARTIALLY_FITTED):
    ...
    def fit(self) -> Result:
        ...
        if self._restore_path:
            tuner = Tuner.restore(path=self._restore_path, trainable=trainable, param_space=param_space, resume_unfinished=True, resume_errored=True, storage_filesystem=self._restore_storage_filesystem)
        else:
            tuner = Tuner(trainable=trainable, param_space=param_space, run_config=self.run_config, _entrypoint=AirEntrypoint.TRAINER)
        experiment_local_path, _ = TunerInternal.setup_create_experiment_checkpoint_dir(trainable, self.run_config)
        experiment_local_path = Path(experiment_local_path)
        self._save(experiment_local_path)
        restore_msg = TrainingFailedError._RESTORE_MSG.format(trainer_cls_name=self.__class__.__name__, path=str(experiment_local_path))
        try:
        ...
        if result.error:
            raise TrainingFailedError('\n'.join([restore_msg, TrainingFailedError._FAILURE_CONFIG_MSG])) from result.error
        return result
    ...
    def _save(self, experiment_path: Union[str, Path]):
        ...
        def raise_fn():
            raise RuntimeError
        if datasets:
            param_dict['datasets'] = {dataset_name: raise_fn for dataset_name in datasets}
