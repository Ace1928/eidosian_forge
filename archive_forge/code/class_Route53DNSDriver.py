...
class Route53DNSDriver(DNSDriver):
    ...
    RECORD_TYPE_MAP = {RecordType.A: 'A', RecordType.AAAA: 'AAAA', RecordType.CNAME: 'CNAME', RecordType.MX: 'MX', RecordType.NS: 'NS', RecordType.PTR: 'PTR', RecordType.SOA: 'SOA', RecordType.SPF: 'SPF', RecordType.SRV: 'SRV', RecordType.TXT: 'TXT'}
    ...
    def create_record(self, name, zone, type, data, extra=None):
        ...
        return Record(id=id, name=name, type=type, data=data, zone=zone, driver=self, ttl=extra.get('ttl', None), extra=extra)
    ...
    def update_record(self, record, name=None, type=None, data=None, extra=None):
        ...
        if multiple_value_record and other_records:
            self._update_multi_value_record(record=record, name=name, type=type, data=data, extra=extra)
        else:
            self._update_single_value_record(record=record, name=name, type=type, data=data, extra=extra)
        id = ':'.join((self.RECORD_TYPE_MAP[type], name))
        return Record(id=id, name=name, type=type, data=data, zone=record.zone, driver=self, ttl=extra.get('ttl', None), extra=extra)
    ...
    def ex_create_multi_value_record(self, name, zone, type, data, extra=None):
        ...
        records = []
        for value in values:
            record = Record(id=id, name=name, type=type, data=value, zone=zone, driver=self, ttl=extra.get('ttl', None), extra=extra)
    ...
    def _to_zone(self, elem):
        ...
        resource_record_count = int(findtext(element=elem, xpath='ResourceRecordSetCount', namespace=NAMESPACE))
    ...
    def _to_records(self, data, zone):
        records = []
        elems = data.findall(fixxpath(xpath='ResourceRecordSets/ResourceRecordSet', namespace=NAMESPACE))
        for elem in elems:
            record_set = elem.findall(fixxpath(xpath='ResourceRecords/ResourceRecord', namespace=NAMESPACE))
            ...
            if multiple_value_record:
                for index in range(0, len(record_set_records)):
                    ...
                    for other_index, other_record in enumerate(record_set_records):
                        ...
                        item = {'name': other_record.name, 'data': other_record.data, 'type': other_record.type, 'extra': extra}
    ...
    def _to_record(self, elem, zone, index=0):
        ...
        type = self._string_to_record_type(findtext(element=elem, xpath='Type', namespace=NAMESPACE))
        ttl = findtext(element=elem, xpath='TTL', namespace=NAMESPACE)
        if ttl is not None:
            ttl = int(ttl)
        value_elem = elem.findall(fixxpath(xpath='ResourceRecords/ResourceRecord', namespace=NAMESPACE))[index]
        ...
        if type == 'MX':
            ...
            extra['port'] = int(port)
        id = ':'.join((self.RECORD_TYPE_MAP[type], name))
        record = Record(id=id, name=name, type=type, data=data, zone=zone, driver=self, ttl=extra.get('ttl', None), extra=extra)
        return record
    ...
    def _get_data(self, rtype, last_key, **kwargs):
        ...
        if response.status == httplib.OK:
            is_truncated = findtext(element=response.object, xpath='IsTruncated', namespace=NAMESPACE)
            exhausted = is_truncated != 'true'
            last_key = findtext(element=response.object, xpath='NextRecordName', namespace=NAMESPACE)
