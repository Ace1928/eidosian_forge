...
def lower_gates(circuit: QuantumCircuit, schedule_config: ScheduleConfig, backend: Optional[Union[BackendV1, BackendV2]]=None) -> List[CircuitPulseDef]:
    """
    ...
    for instruction in circuit.data:
        ...
        if isinstance(instruction.operation, Barrier):
            circ_pulse_defs.append(CircuitPulseDef(schedule=instruction.operation, qubits=inst_qubits))
            ...
            for qubit in inst_qubits:
                for channel in [DriveChannel]:
                    sched += pulse_inst.Delay(duration=instruction.operation.duration, channel=channel(qubit))
            ...
            if len(inst_qubits) != 1 and len(instruction.clbits) != 1:
                raise QiskitError("Qubit '{}' or classical bit '{}' errored because the circuit Measure instruction only takes one of each.".format(inst_qubits, instruction.clbits))
            ...
            try:
                ...
                schedule = gate_cals[tuple(inst_qubits), tuple((p if getattr(p, 'parameters', None) else float(p) for p in instruction.operation.params))]
                ...
                pass
            try:
                schedule = inst_map.get(instruction.operation, inst_qubits, *instruction.operation.params)
                schedule = target_qobj_transform(schedule)
                circ_pulse_defs.append(CircuitPulseDef(schedule=schedule, qubits=inst_qubits))
            except PulseError as ex:
                raise QiskitError(f"Operation '{instruction.operation.name}' on qubit(s) {inst_qubits} not supported by the backend command definition. Did you remember to transpile your input circuit for the same backend?") from ex
    if qubit_mem_slots:
        circ_pulse_defs.append(get_measure_schedule(qubit_mem_slots))
