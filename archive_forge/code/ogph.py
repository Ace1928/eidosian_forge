...
class DynamicActivationNeuron(nn.Module):
    ...
    The output can be scaled by a learnable parameter. Guided by the Kalmogorov-Arnold theorem.
    """
    ...
    def __init__(
        ...
        logger.info(
            f"DynamicActivationNeuron initialized with input_size={input_size}, output_size={output_size}, activation={activation}, neuron_type={neuron_type}, scale_output={scale_output}"
        )
...
class DynamicActivationSynapse(nn.Module):
    ...
    Guide by the Kalmogorov-Arnold theorem.
    """
...
def main():
    ...
    # Create a model with a reasonable depth and width
    model = nn.Sequential(
        DynamicActivationNeuron(
            input_size, 216, activation="relu", neuron_type="standard"
        ),
        DynamicActivationSynapse(216, 216, activation="tanh"),
        DynamicActivationNeuron(216, 72, activation="sigmoid", neuron_type="spiking"),
        DynamicActivationSynapse(72, 72, activation="standard"),
        DynamicActivationNeuron(
            36, output_size, activation="relu", neuron_type="graph"
        ),
    )
    ...
    plt.figure(figsize=(10, 5))
    plt.scatter(range(num_samples), y.numpy(), label="True Values")
    plt.scatter(range(num_samples), test_outputs.numpy(), label="Predicted Values")
