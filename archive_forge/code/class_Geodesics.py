...
class Geodesics:
    ...
    def __init__(self, manifold, words):
        ...
        self.mcomplex = None
        self.geodesic_tube_infos = [GeodesicTubeInfo(self.get_mcomplex(), word, index) for index, word in enumerate(words)]
    ...
    def get_uniform_bindings(self):
        return {'geodesics.geodesicHeads': ('vec4[]', self.data_heads), 'geodesics.geodesicTails': ('vec4[]', self.data_tails), 'geodesics.geodesicIndex': ('int[]', self.data_indices), 'geodesics.geodesicTubeRadiusParam': ('float[]', self.data_radius_params), 'geodesics.geodesicOffsets': ('int[]', self.data_offsets)}
    ...
    def get_compile_time_constants(self):
        if self.data_heads:
            ...
            num = 0
        return {b'##num_geodesic_segments##': num}

    def add_length_spectrum(self, l):
        try:
            L = self.manifold.length_spectrum(l, grouped=False, include_words=True)
    ...
    def add_word(self, word, is_primitive=None):
        geodesic_tube_info = GeodesicTubeInfo(self.get_mcomplex(), word, index=len(self.geodesic_tube_infos), is_primitive=is_primitive)
