...
class MongoDB(base.Driver):
    ...
    def __init__(self, connection_str, db_name='osprofiler', project=None, service=None, host=None, **kwargs):
        """MongoDB driver for OSProfiler."""
        super(MongoDB, self).__init__(connection_str, project=project, service=service, host=host, **kwargs)
        try:
            from pymongo import MongoClient
        except ImportError:
            raise exc.CommandError('To use OSProfiler with MongoDB driver, please install `pymongo` library. To install with pip:\n `pip install pymongo`.')
    ...
    def notify(self, info):
        ...
        if self.filter_error_trace and data.get('info', {}).get('etype') is not None:
            self.notify_error_trace(data)

    def notify_error_trace(self, data):
        """Store base_id and timestamp of error trace to a separate db."""
        self.db.profiler_error.update({'base_id': data['base_id']}, {'base_id': data['base_id'], 'timestamp': data['timestamp']}, upsert=True)
    ...
    def list_traces(self, fields=None):
        ...
        return [self.db.profiler.find({'base_id': i}, out_format).sort('timestamp')[0] for i in ids]
