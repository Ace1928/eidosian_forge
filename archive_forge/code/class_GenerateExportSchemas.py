  ...
  *gcloud* commands that have "too many" *create*/*update* command flags may
  ...
  """
    ...
    def Args(parser):
        ...
        parser.add_argument('message', help='The name of the message to generate the YAML export schemas for.')
        parser.add_argument('--directory', help='The path name of the directory to create the YAML export schema files in. If not specified then the files are created in the current directory.')
    ...
    def Run(self, args):
        ...
        try:
            ...
            raise exceptions.InvalidArgumentException('message', 'Message [{}] does not exist for API [{} {}]'.format(args.message, args.api, api.version))
        message_spec = arg_utils.GetRecursiveMessageSpec(message)
        export.GenerateExportSchemas(api, args.message, message_spec, args.directory)