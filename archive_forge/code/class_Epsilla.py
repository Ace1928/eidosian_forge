...
class Epsilla(VectorStore):
    ...
    def __init__(self, client: Any, embeddings: Embeddings, db_path: Optional[str]=_LANGCHAIN_DEFAULT_DB_PATH, db_name: Optional[str]=_LANGCHAIN_DEFAULT_DB_NAME):
        """Initialize with necessary components."""
        try:
            import pyepsilla
        except ImportError as e:
            raise ImportError('Could not import pyepsilla python package. Please install pyepsilla package with `pip install pyepsilla`.') from e
        if not isinstance(client, pyepsilla.vectordb.Client):
            raise TypeError(f'client should be an instance of pyepsilla.vectordb.Client, got {type(client)}')
    ...
    def _create_collection(self, table_name: str, embeddings: list, metadatas: Optional[list[dict]]=None) -> None:
        ...
        fields: List[dict] = [{'name': 'id', 'dataType': 'INT'}, {'name': 'text', 'dataType': 'STRING'}, {'name': 'embeddings', 'dataType': 'VECTOR_FLOAT', 'dimensions': dim}]
    ...
    def add_texts(self, texts: Iterable[str], metadatas: Optional[List[dict]]=None, collection_name: Optional[str]='', drop_old: Optional[bool]=False, **kwargs: Any) -> List[str]:
        """
    ...
    def similarity_search(self, query: str, k: int=4, collection_name: str='', **kwargs: Any) -> List[Document]:
        ...
        status_code, response = self._client.query(table_name=collection_name, query_field='embeddings', query_vector=query_vector, limit=k)
        if status_code != 200:
    ...
    def from_texts(cls: Type[Epsilla], texts: List[str], embedding: Embeddings, metadatas: Optional[List[dict]]=None, client: Any=None, db_path: Optional[str]=_LANGCHAIN_DEFAULT_DB_PATH, db_name: Optional[str]=_LANGCHAIN_DEFAULT_DB_NAME, collection_name: Optional[str]=_LANGCHAIN_DEFAULT_TABLE_NAME, drop_old: Optional[bool]=False, **kwargs: Any) -> Epsilla:
        ...
        instance.add_texts(texts, metadatas=metadatas, collection_name=collection_name, drop_old=drop_old, **kwargs)
        return instance

    @classmethod
    def from_documents(cls: Type[Epsilla], documents: List[Document], embedding: Embeddings, client: Any=None, db_path: Optional[str]=_LANGCHAIN_DEFAULT_DB_PATH, db_name: Optional[str]=_LANGCHAIN_DEFAULT_DB_NAME, collection_name: Optional[str]=_LANGCHAIN_DEFAULT_TABLE_NAME, drop_old: Optional[bool]=False, **kwargs: Any) -> Epsilla:
        ...
        return cls.from_texts(texts, embedding, metadatas=metadatas, client=client, db_path=db_path, db_name=db_name, collection_name=collection_name, drop_old=drop_old, **kwargs)