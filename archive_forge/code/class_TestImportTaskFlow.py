...
class TestImportTaskFlow(test_utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.config(enabled_import_methods=['glance-direct', 'web-download', 'copy-image'])
        self.config(node_staging_uri='file:///tmp/staging')
        store.create_stores(CONF)
        self.base_flow = ['ImageLock', 'ConfigureStaging', 'ImportToStore', 'DeleteFromFS', 'VerifyImageState', 'CompleteTask']
        self.import_plugins = ['Convert_Image', 'Decompress_Image', 'InjectMetadataProperties']
