...
class CZTargetGateset(compilation_target_gateset.TwoQubitCompilationTargetGateset):
    ...
    def __init__(self, *, atol: float=1e-08, allow_partial_czs: bool=False, additional_gates: Sequence[Union[Type['cirq.Gate'], 'cirq.Gate', 'cirq.GateFamily']]=()) -> None:
        ...
        """
        super().__init__(ops.CZPowGate if allow_partial_czs else ops.CZ, ops.MeasurementGate, ops.PhasedXZGate, ops.GlobalPhaseGate, *additional_gates, name='CZPowTargetGateset' if allow_partial_czs else 'CZTargetGateset')
    ...
    def _decompose_two_qubit_operation(self, op: 'cirq.Operation', _) -> 'cirq.OP_TREE':
        ...
        return two_qubit_to_cz.two_qubit_matrix_to_cz_operations(op.qubits[0], op.qubits[1], protocols.unitary(op), allow_partial_czs=self.allow_partial_czs, atol=self.atol)

    def __repr__(self) -> str:
        return f'cirq.CZTargetGateset(atol={self.atol}, allow_partial_czs={self.allow_partial_czs}, additional_gates=[{self._additional_gates_repr_str}])'
    ...
    def _from_json_dict_(cls, atol, allow_partial_czs, additional_gates=(), **kwargs):
        return cls(atol=atol, allow_partial_czs=allow_partial_czs, additional_gates=additional_gates)