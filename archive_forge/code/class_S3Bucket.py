...
class S3Bucket(resource.Resource):
    PROPERTIES = ACCESS_CONTROL, WEBSITE_CONFIGURATION, TAGS = ('AccessControl', 'WebsiteConfiguration', 'Tags')
    _WEBSITE_CONFIGURATION_KEYS = WEBSITE_CONFIGURATION_INDEX_DOCUMENT, WEBSITE_CONFIGURATION_ERROR_DOCUMENT = ('IndexDocument', 'ErrorDocument')
    _TAG_KEYS = TAG_KEY, TAG_VALUE = ('Key', 'Value')
    ATTRIBUTES = DOMAIN_NAME, WEBSITE_URL = ('DomainName', 'WebsiteURL')
    properties_schema = {ACCESS_CONTROL: properties.Schema(properties.Schema.STRING, _('A predefined access control list (ACL) that grants permissions on the bucket.'), constraints=[constraints.AllowedValues(['Private', 'PublicRead', 'PublicReadWrite', 'AuthenticatedRead', 'BucketOwnerRead', 'BucketOwnerFullControl'])]), WEBSITE_CONFIGURATION: properties.Schema(properties.Schema.MAP, _('Information used to configure the bucket as a static website.'), schema={WEBSITE_CONFIGURATION_INDEX_DOCUMENT: properties.Schema(properties.Schema.STRING, _('The name of the index document.')), WEBSITE_CONFIGURATION_ERROR_DOCUMENT: properties.Schema(properties.Schema.STRING, _('The name of the error document.'))}), TAGS: properties.Schema(properties.Schema.LIST, _('Tags to attach to the bucket.'), schema=properties.Schema(properties.Schema.MAP, schema={TAG_KEY: properties.Schema(properties.Schema.STRING, _('The tag key name.'), required=True), TAG_VALUE: properties.Schema(properties.Schema.STRING, _('The tag value.'), required=True)}))}
    attributes_schema = {DOMAIN_NAME: attributes.Schema(_('The DNS name of the specified bucket.'), type=attributes.Schema.STRING), WEBSITE_URL: attributes.Schema(_('The website endpoint for the specified bucket.'), type=attributes.Schema.STRING)}
    ...
    def handle_delete(self):
        ...
        try:
            ...
            if self.client_plugin().is_conflict(ex):
                ...
                if objects:
                    msg = _('The bucket you tried to delete is not empty (%s).') % self.resource_id
