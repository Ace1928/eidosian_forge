...
import logging
@TransformationFactory.register('gdp.bound_pretransformation', doc='Partially transforms a GDP to a MIP by finding all disjunctive constraints with common left-hand sides and transforming them according to the formulation in Balas 1988')
class BoundPretransformation(Transformation):
    ...
    def _apply_to(self, instance, **kwds):
        ...
        for t in whole_tree.topological_sort():
            if t.ctype is Disjunction and whole_tree.in_degree(t) == 0:
                self._transform_disjunction(t, instance, bound_dict, transformation_blocks)
    ...
    def _add_transformation_block(self, disjunction, transformation_blocks):
        ...
        trans_block_name = unique_component_name(to_block, '_pyomo_gdp_%s_reformulation' % self.transformation_name)
        transformation_blocks[to_block] = trans_block = Block()
        to_block.add_component(trans_block_name, trans_block)
        trans_block.transformed_bound_constraints = Constraint(Any * NonNegativeIntegers)
        return trans_block

    def get_transformed_constraints(self, v, disjunction):
        if self.transformation_name not in disjunction._transformation_map:
            logger.debug("No variable on Disjunction '%s' was transformed with the gdp.%s transformation" % (disjunction.name, self.transformation_name))
            return []
        trans_map = disjunction._transformation_map[self.transformation_name]
        if v not in trans_map:
            logger.debug("Constraint bounding variable '%s' on Disjunction '%s' was not transformed by the 'gdp.%s' transformation" % (v.name, disjunction.name, self.transformation_name))
            return []
        return trans_map[v]