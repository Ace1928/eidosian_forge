...
class Repo:
    ...
    def __init__(self, path: Optional[PathLike]=None, odbt: Type[LooseObjectDB]=GitCmdObjectDB, search_parent_directories: bool=False, expand_vars: bool=True) -> None:
    ...
    def create_tag(self, path: PathLike, ref: Union[str, 'SymbolicReference']='HEAD', message: Optional[str]=None, force: bool=False, **kwargs: Any) -> TagReference:
    ...
    def _set_daemon_export(self, value: object) -> None:
        ...
        if value and (not fileexists):
            ...
            os.unlink(filename)
    daemon_export = property(_get_daemon_export, _set_daemon_export, doc='If True, git-daemon may export this repository')
    ...
    @classmethod
    def init(cls, path: Union[PathLike, None]=None, mkdir: bool=True, odbt: Type[GitCmdObjectDB]=GitCmdObjectDB, expand_vars: bool=True, **kwargs: Any) -> 'Repo':
    ...
    @classmethod
    def _clone(cls, git: 'Git', url: PathLike, path: PathLike, odb_default_type: Type[GitCmdObjectDB], progress: Union['RemoteProgress', 'UpdateProgress', Callable[..., 'RemoteProgress'], None]=None, multi_options: Optional[List[str]]=None, allow_unsafe_protocols: bool=False, allow_unsafe_options: bool=False, **kwargs: Any) -> 'Repo':
        ...
        proc = git.clone(multi, '--', Git.polish_url(str(url)), clone_path, with_extended_output=True, as_process=True, v=True, universal_newlines=True, **add_progress(kwargs, git, progress))
        if progress:
            handle_process_output(proc, None, to_progress_instance(progress).new_message_handler(), finalize_process, decode_streams=False)
        else:
