...
def _set_model_version_tag():
    request_message = _get_request_message(SetModelVersionTag(), schema={'name': [_assert_string, _assert_required], 'version': [_assert_string, _assert_required], 'key': [_assert_string, _assert_required], 'value': [_assert_string]})
    tag = ModelVersionTag(key=request_message.key, value=request_message.value)
    _get_model_registry_store().set_model_version_tag(name=request_message.name, version=request_message.version, tag=tag)
