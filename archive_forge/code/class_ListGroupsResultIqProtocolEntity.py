...
class ListGroupsResultIqProtocolEntity(ResultIqProtocolEntity):
    ...
    def setProps(self, groupsList):
        assert type(groupsList) is list and (len(groupsList) == 0 or groupsList[0].__class__ is Group), 'groupList must be a list of Group instances'
    ...
    def toProtocolTreeNode(self):
        ...
        for group in self.groupsList:
            groupNode = ProtocolTreeNode('group', {'id': group.getId(), 'creator': group.getCreator(), 'subject': group.getSubject(), 's_o': group.getSubjectOwner(), 's_t': str(group.getSubjectTime()), 'creation': str(group.getCreationTime())})
            participants = []
