...
class SESConnection(AWSAuthConnection):
    ...
    def __init__(self, aws_access_key_id=None, aws_secret_access_key=None, is_secure=True, port=None, proxy=None, proxy_port=None, proxy_user=None, proxy_pass=None, debug=0, https_connection_factory=None, region=None, path='/', security_token=None, validate_certs=True, profile_name=None):
        ...
        self.region = region
        super(SESConnection, self).__init__(self.region.endpoint, aws_access_key_id, aws_secret_access_key, is_secure, port, proxy, proxy_port, proxy_user, proxy_pass, debug, https_connection_factory, path, security_token=security_token, validate_certs=validate_certs, profile_name=profile_name)
    ...
    def _make_request(self, action, params=None):
        ...
        response = super(SESConnection, self).make_request('POST', '/', headers=headers, data=urllib.parse.urlencode(params))
        body = response.read().decode('utf-8')
        if response.status == 200:
            list_markers = ('VerifiedEmailAddresses', 'Identities', 'DkimTokens', 'DkimAttributes', 'VerificationAttributes', 'SendDataPoints')
    ...
    def send_email(self, source, subject, body, to_addresses, cc_addresses=None, bcc_addresses=None, format='text', reply_addresses=None, return_path=None, text_body=None, html_body=None):
        ...
        if body is not None:
            if format == 'text':
                ...
                html_body = body
        params = {'Source': source, 'Message.Subject.Data': subject}
        if return_path:
        ...
        if cc_addresses:
            self._build_list_params(params, cc_addresses, 'Destination.CcAddresses.member')
        if bcc_addresses:
            self._build_list_params(params, bcc_addresses, 'Destination.BccAddresses.member')
        if reply_addresses:
