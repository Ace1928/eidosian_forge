...
class GridDeviceMetadata(device.DeviceMetadata):
    ...
    def __init__(self, qubit_pairs: Iterable[Tuple['cirq.GridQubit', 'cirq.GridQubit']], gateset: 'cirq.Gateset', gate_durations: Optional[Mapping['cirq.GateFamily', 'cirq.Duration']]=None, all_qubits: Optional[Iterable['cirq.GridQubit']]=None, compilation_target_gatesets: Iterable['cirq.CompilationTargetGateset']=()):
        ...
        for q in node_set:
            if q not in all_qubits:
                raise ValueError(f'Qubit {q} found in node_set and not in all_qubits. all_qubits must contain at least all the qubits found in all_qubits.')
        ...
        if gate_durations is not None:
            ...
            if not working_gatefamilies.issubset(gateset.gates):
                raise ValueError(f'Some gate_durations keys are not found in gateset. gate_durations={gate_durations} gateset.gates={gateset.gates}')
    ...
    def _value_equality_values_(self):
        ...
        return (self._qubit_pairs, self._gateset, tuple(duration_equality), tuple(sorted(self.qubit_set)), frozenset(self._compilation_target_gatesets))

    def __repr__(self) -> str:
        qubit_pair_tuples = frozenset({tuple(sorted(p)) for p in self._qubit_pairs})
        return f'cirq.GridDeviceMetadata({repr(qubit_pair_tuples)}, {repr(self._gateset)}, {repr(self._gate_durations)}, {repr(self.qubit_set)}, {repr(self._compilation_target_gatesets)})'

    def _json_dict_(self):
        duration_payload = None
        if self._gate_durations is not None:
            duration_payload = sorted(self._gate_durations.items(), key=lambda x: repr(x[0]))
        return {'qubit_pairs': sorted([sorted(pair) for pair in self._qubit_pairs]), 'gateset': self._gateset, 'gate_durations': duration_payload, 'all_qubits': sorted(list(self.qubit_set)), 'compilation_target_gatesets': list(self._compilation_target_gatesets)}

    @classmethod
    def _from_json_dict_(cls, qubit_pairs, gateset, gate_durations, all_qubits, compilation_target_gatesets=(), **kwargs):
        return cls(qubit_pairs, gateset, dict(gate_durations) if gate_durations is not None else None, all_qubits, compilation_target_gatesets)