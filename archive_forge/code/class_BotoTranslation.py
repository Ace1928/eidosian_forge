...
class BotoTranslation(CloudApi):
  ...
  """
    ...
    """
        super(BotoTranslation, self).__init__(bucket_storage_uri_class, logger, status_queue, provider=provider, debug=debug, http_headers=http_headers, trace_token=trace_token, perf_trace_token=perf_trace_token)
        _ = credentials
    ...
    def GetBucket(self, bucket_name, provider=None, fields=None):
        ...
        try:
            return self._BotoBucketToBucket(bucket_uri.get_bucket(validate=True, headers=headers), fields=fields)
    ...
    def ListBuckets(self, project_id=None, provider=None, fields=None):
        ...
        try:
            provider_uri = boto.storage_uri('%s://' % self.provider, suppress_consec_slashes=False, bucket_storage_uri_class=self.bucket_storage_uri_class, debug=self.debug)
    ...
    def PatchBucket(self, bucket_name, metadata, canned_acl=None, canned_def_acl=None, preconditions=None, provider=None, fields=None):
        ...
        try:
            ...
            if canned_def_acl:
                ...
                bucket_uri.set_def_acl(canned_def_acl, bucket_uri.object_name, headers=headers)
            if metadata.cors:
            ...
            if metadata.lifecycle:
                boto_lifecycle = LifecycleTranslation.BotoLifecycleFromMessage(metadata.lifecycle)
            ...
            if metadata.logging:
                if metadata.logging.logBucket and metadata.logging.logObjectPrefix:
                    bucket_uri.enable_logging(metadata.logging.logBucket, metadata.logging.logObjectPrefix, False, headers=headers)
                else:
            ...
            if metadata.versioning:
                bucket_uri.configure_versioning(metadata.versioning.enabled, headers=headers)
            if metadata.website:
                main_page_suffix = metadata.website.mainPageSuffix
                error_page = metadata.website.notFoundPage
                bucket_uri.set_website_config(main_page_suffix, error_page, headers=headers)
    ...
    def CreateBucket(self, bucket_name, project_id=None, metadata=None, provider=None, fields=None):
        ...
        if bucket_uri.scheme == 'gs':
            ...
            if metadata and metadata.retentionPolicy and metadata.retentionPolicy.retentionPeriod:
                headers['x-goog-bucket-retention-period'] = str(metadata.retentionPolicy.retentionPeriod)
            try:
                bucket_uri.create_bucket(headers=headers, location=location, storage_class=storage_class)
            except TRANSLATABLE_BOTO_EXCEPTIONS as e:
                self._TranslateExceptionAndRaise(e, bucket_name=bucket_name)
        else:
            try:
    ...
    def DeleteBucket(self, bucket_name, preconditions=None, provider=None):
        ...
        try:
            ...
            translated_exception = self._TranslateBotoException(e, bucket_name=bucket_name)
            if translated_exception and 'BucketNotEmpty' in translated_exception.reason:
                try:
                    if bucket_uri.get_versioning_config():
                        if self.provider == 's3':
                            raise NotEmptyException('VersionedBucketNotEmpty (%s). Currently, gsutil does not support listing or removing S3 DeleteMarkers, so you may need to delete these using another tool to successfully delete this bucket.' % bucket_name, status=e.status)
                        raise NotEmptyException('VersionedBucketNotEmpty (%s)' % bucket_name, status=e.status)
                    else:
                        raise NotEmptyException('BucketNotEmpty (%s)' % bucket_name, status=e.status)
    ...
    def ListObjects(self, bucket_name, prefix=None, delimiter=None, all_versions=None, provider=None, fields=None):
        ...
        yield_objects = fields is None or any((field.startswith('items/') for field in fields))
        try:
            objects_iter = bucket_uri.list_bucket(prefix=prefix or '', delimiter=delimiter or '', all_versions=all_versions, headers=headers)
        except TRANSLATABLE_BOTO_EXCEPTIONS as e:
            self._TranslateExceptionAndRaise(e, bucket_name=bucket_name)
        try:
            for key in objects_iter:
                if yield_prefixes and isinstance(key, Prefix):
                    yield CloudApi.CsObjectOrPrefix(key.name, CloudApi.CsObjectOrPrefixType.PREFIX)
                elif yield_objects:
                    key_to_convert = key
                    key_http_fields = set(['bucket', 'etag', 'name', 'updated', 'generation', 'metageneration', 'size'])
                    if not get_fields or (get_fields and (not get_fields.issubset(key_http_fields))):
                        generation = None
                        if getattr(key, 'generation', None):
                            generation = key.generation
                        if getattr(key, 'version_id', None):
                            generation = key.version_id
                        key_to_convert = self._GetBotoKey(bucket_name, key.name, generation=generation)
                    return_object = self._BotoKeyToObject(key_to_convert, fields=get_fields)
                    yield CloudApi.CsObjectOrPrefix(return_object, CloudApi.CsObjectOrPrefixType.OBJECT)
        except TRANSLATABLE_BOTO_EXCEPTIONS as e:
            self._TranslateExceptionAndRaise(e, bucket_name=bucket_name)
    ...
    def GetObjectMetadata(self, bucket_name, object_name, generation=None, provider=None, fields=None):
        ...
        _ = provider
        try:
            return self._BotoKeyToObject(self._GetBotoKey(bucket_name, object_name, generation=generation), fields=fields)
        except TRANSLATABLE_BOTO_EXCEPTIONS as e:
    ...
    def GetObjectMedia(self, bucket_name, object_name, download_stream, provider=None, generation=None, object_size=None, compressed_encoding=False, download_strategy=CloudApi.DownloadStrategy.ONE_SHOT, start_byte=0, end_byte=None, progress_callback=None, serialization_data=None, digesters=None, decryption_tuple=None):
        ...
        if total_size:
            num_progress_callbacks = max(int(total_size) / TWO_MIB, XML_PROGRESS_CALLBACKS)
        else:
            num_progress_callbacks = XML_PROGRESS_CALLBACKS
        try:
            if download_strategy is CloudApi.DownloadStrategy.RESUMABLE:
                self._PerformResumableDownload(download_stream, start_byte, end_byte, key, headers=headers, callback=progress_callback, num_callbacks=num_progress_callbacks, hash_algs=hash_algs)
            elif download_strategy is CloudApi.DownloadStrategy.ONE_SHOT:
                self._PerformSimpleDownload(download_stream, key, progress_callback=progress_callback, num_progress_callbacks=num_progress_callbacks, headers=headers, hash_algs=hash_algs)
            else:
                raise ArgumentException('Unsupported DownloadStrategy: %s' % download_strategy)
        except TRANSLATABLE_BOTO_EXCEPTIONS as e:
            self._TranslateExceptionAndRaise(e, bucket_name=bucket_name, object_name=object_name, generation=generation)
        if self.provider == 's3':
            if digesters:
          ...
          """
                ...
                for alg_name in digesters:
                    if download_strategy == CloudApi.DownloadStrategy.RESUMABLE and start_byte != 0 or not (getattr(key, 'local_hashes', None) and alg_name in key.local_hashes):
                        digesters[alg_name] = None
                    else:
                        digesters[alg_name] = HashToDigester(key.local_hashes[alg_name])

    def _PerformSimpleDownload(self, download_stream, key, progress_callback=None, num_progress_callbacks=XML_PROGRESS_CALLBACKS, headers=None, hash_algs=None):
        try:
            key.get_contents_to_file(download_stream, cb=progress_callback, num_cb=num_progress_callbacks, headers=headers, hash_algs=hash_algs)
        except TypeError:
            key.get_contents_to_file(download_stream, cb=progress_callback, num_cb=num_progress_callbacks, headers=headers)

    def _PerformResumableDownload(self, fp, start_byte, end_byte, key, headers=None, callback=None, num_callbacks=XML_PROGRESS_CALLBACKS, hash_algs=None):
    ...
    """
        ...
        while True:
            try:
                ...
                try:
                    key.get_file(fp, headers, cb_handler.call, num_callbacks, override_num_retries=0, hash_algs=hash_algs)
                except TypeError:
                    key.get_file(fp, headers, cb_handler.call, num_callbacks, override_num_retries=0)
                fp.flush()
                return
                ...
                if e.disposition == boto.exception.ResumableTransferDisposition.ABORT_CUR_PROCESS:
                    raise ResumableDownloadException(e.message)
                elif debug >= 1:
                    self.logger.info('Caught boto.exception.ResumableDownloadException (%s) - will retry', e.message)
            start_byte = fp.tell()
            ...
            if progress_less_iterations > num_retries:
                raise ResumableDownloadException('Too many resumable download attempts failed without progress. You might try this download again later')
            try:
                key.close()
            except http_client.IncompleteRead:
                pass
            sleep_time_secs = min(random.random() * 2 ** progress_less_iterations, GetMaxRetryDelay())
            if debug >= 1:
                self.logger.info('Got retryable failure (%d progress-less in a row).\nSleeping %d seconds before re-trying', progress_less_iterations, sleep_time_secs)
            time.sleep(sleep_time_secs)

    def PatchObjectMetadata(self, bucket_name, object_name, metadata, canned_acl=None, generation=None, preconditions=None, provider=None, fields=None):
        """See CloudApi class for function doc strings."""
        _ = provider
        object_uri = self._StorageUriForObject(bucket_name, object_name, generation=generation)
        headers = self._CreateBaseHeaders()
        ...
        if metadata_changed:
            try:
                object_uri.set_metadata(metadata_plus, metadata_minus, False, headers=headers)
            except TRANSLATABLE_BOTO_EXCEPTIONS as e:
                self._TranslateExceptionAndRaise(e, bucket_name=bucket_name, object_name=object_name, generation=generation)
        if metadata.acl:
            boto_acl = AclTranslation.BotoAclFromMessage(metadata.acl)
            try:
                object_uri.set_xml_acl(boto_acl.to_xml(), key_name=object_name, headers=headers)
            except TRANSLATABLE_BOTO_EXCEPTIONS as e:
                self._TranslateExceptionAndRaise(e, bucket_name=bucket_name, object_name=object_name, generation=generation)
        if canned_acl:
        ...
        return self.GetObjectMetadata(bucket_name, object_name, generation=generation, fields=fields)

    def _PerformSimpleUpload(self, dst_uri, upload_stream, md5=None, canned_acl=None, progress_callback=None, headers=None):
        dst_uri.set_contents_from_file(upload_stream, md5=md5, policy=canned_acl, cb=progress_callback, headers=headers, rewind=True)

    def _PerformStreamingUpload(self, dst_uri, upload_stream, canned_acl=None, progress_callback=None, headers=None):
        if dst_uri.get_provider().supports_chunked_transfer():
            dst_uri.set_contents_from_stream(upload_stream, policy=canned_acl, cb=progress_callback, headers=headers)
        else:
            ...
            try:
                ...
                with open(temp_path, 'rb') as in_fp:
                    dst_uri.set_contents_from_file(in_fp, policy=canned_acl, headers=headers)
            finally:
                os.close(temp_fh)
                os.unlink(temp_path)

    def _PerformResumableUpload(self, key, upload_stream, upload_size, tracker_callback, canned_acl=None, serialization_data=None, progress_callback=None, headers=None):
        resumable_upload = BotoResumableUpload(tracker_callback, self.logger, resume_url=serialization_data)
        resumable_upload.SendFile(key, upload_stream, upload_size, canned_acl=canned_acl, cb=progress_callback, headers=headers)
    ...
    """
        try:
            ...
            if e.message and NON_EXISTENT_OBJECT_REGEX.match(e.message.encode(UTF8)):
                raise CommandException('\n'.join(textwrap.wrap('Uploaded object (%s) was deleted or overwritten immediately after it was uploaded. This can happen if you attempt to upload to the same object multiple times concurrently.' % dst_uri.uri)))
            else:
                raise
    ...
    def UploadObjectResumable(self, upload_stream, object_metadata, canned_acl=None, preconditions=None, size=None, serialization_data=None, tracker_callback=None, progress_callback=None, encryption_tuple=None, provider=None, fields=None, gzip_encoded=False):
        ...
        if self.provider == 's3':
            return self.UploadObject(upload_stream, object_metadata, canned_acl=canned_acl, preconditions=preconditions, fields=fields, size=size)
        headers, dst_uri = self._UploadSetup(object_metadata, preconditions=preconditions)
        if not tracker_callback:
            raise ArgumentException('No tracker callback function set for resumable upload of %s' % dst_uri)
        try:
            self._PerformResumableUpload(dst_uri.new_key(headers=headers), upload_stream, size, tracker_callback, canned_acl=canned_acl, serialization_data=serialization_data, progress_callback=progress_callback, headers=headers)
    ...
    def UploadObjectStreaming(self, upload_stream, object_metadata, canned_acl=None, progress_callback=None, preconditions=None, encryption_tuple=None, provider=None, fields=None, gzip_encoded=False):
        """See CloudApi class for function doc strings."""
        if gzip_encoded:
            raise NotImplementedError('XML API does not suport gzip-encoded uploads.')
        headers, dst_uri = self._UploadSetup(object_metadata, preconditions=preconditions)
        try:
            self._PerformStreamingUpload(dst_uri, upload_stream, canned_acl=canned_acl, progress_callback=progress_callback, headers=headers)
            return self._HandleSuccessfulUpload(dst_uri, object_metadata, fields=fields)
        except TRANSLATABLE_BOTO_EXCEPTIONS as e:
            not_found_exception = CreateNotFoundExceptionForObjectWrite(self.provider, object_metadata.bucket)
            self._TranslateExceptionAndRaise(e, bucket_name=object_metadata.bucket, object_name=object_metadata.name, not_found_exception=not_found_exception)

    def UploadObject(self, upload_stream, object_metadata, canned_acl=None, preconditions=None, size=None, progress_callback=None, encryption_tuple=None, provider=None, fields=None, gzip_encoded=False):
        """See CloudApi class for function doc strings."""
        if gzip_encoded:
            raise NotImplementedError('XML API does not suport gzip-encoded uploads.')
        headers, dst_uri = self._UploadSetup(object_metadata, preconditions=preconditions)
        try:
            md5 = None
            if object_metadata.md5Hash:
                md5 = [Base64ToHexHash(object_metadata.md5Hash), object_metadata.md5Hash.strip('\n"\'')]
            self._PerformSimpleUpload(dst_uri, upload_stream, md5=md5, canned_acl=canned_acl, progress_callback=progress_callback, headers=headers)
            return self._HandleSuccessfulUpload(dst_uri, object_metadata, fields=fields)
        except TRANSLATABLE_BOTO_EXCEPTIONS as e:
            not_found_exception = CreateNotFoundExceptionForObjectWrite(self.provider, object_metadata.bucket)
            self._TranslateExceptionAndRaise(e, bucket_name=object_metadata.bucket, object_name=object_metadata.name, not_found_exception=not_found_exception)
    ...
    def CopyObject(self, src_obj_metadata, dst_obj_metadata, src_generation=None, canned_acl=None, preconditions=None, progress_callback=None, max_bytes_per_call=None, encryption_tuple=None, decryption_tuple=None, provider=None, fields=None):
        ...
        dst_uri = self._StorageUriForObject(dst_obj_metadata.bucket, dst_obj_metadata.name)
        src_version_id = None
        if self.provider == 's3':
        ...
        try:
            new_key = dst_uri.copy_key(src_obj_metadata.bucket, src_obj_metadata.name, preserve_acl=preserve_acl, headers=headers, storage_class=dst_storage_class, src_version_id=src_version_id, src_generation=src_generation)
    ...
    def ComposeObject(self, src_objs_metadata, dst_obj_metadata, preconditions=None, encryption_tuple=None, provider=None, fields=None):
        ...
        for src_obj in src_objs_metadata:
            src_uri = self._StorageUriForObject(dst_bucket_name, src_obj.name, generation=src_obj.generation)
        ...
        try:
            ...
            self._TranslateExceptionAndRaise(e, dst_obj_metadata.bucket, dst_obj_metadata.name)
    ...
    def _AddPreconditionsToHeaders(self, preconditions, headers):
        ...
        if preconditions and self.provider == 'gs':
            ...
            if preconditions.meta_gen_match is not None:
                headers['x-goog-if-metageneration-match'] = str(preconditions.meta_gen_match)
    ...
    """
        return boto.storage_uri('%s://%s' % (self.provider, bucket), suppress_consec_slashes=False, bucket_storage_uri_class=self.bucket_storage_uri_class, debug=self.debug, validate=False)
    ...
    """
        ...
        try:
            ...
            if not key:
                raise CreateObjectNotFoundException('404', self.provider, bucket_name, object_name, generation=generation)
            return key
        except TRANSLATABLE_BOTO_EXCEPTIONS as e:
            self._TranslateExceptionAndRaise(e, bucket_name=bucket_name, object_name=object_name, generation=generation)
        ...
        if getattr(key, 'metadata', None):
            ...
            for k, v in six.iteritems(key.metadata):
                if k.lower() == 'content-language':
                    continue
                custom_metadata.additionalProperties.append(apitools_messages.Object.MetadataValue.AdditionalProperty(key=k, value=v))
    ...
    def _TranslateDeleteMarker(self, key, cloud_api_object):
        ...
        if isinstance(key, DeleteMarker):
            ...
            cloud_api_object.metadata.additionalProperties.append(apitools_messages.Object.MetadataValue.AdditionalProperty(key=S3_DELETE_MARKER_GUID, value=True))
    ...
    def _TranslateBotoKeyAcl(self, key, cloud_api_object, generation=None):
        ...
        storage_uri_for_key = self._StorageUriForObject(key.bucket.name, key.name, generation=generation)
        headers = self._CreateBaseHeaders()
        try:
            if self.provider == 'gs':
    ...
    Raises:
      ...
      translatable.
    """
        translated_exception = self._TranslateBotoException(e, bucket_name=bucket_name, object_name=object_name, generation=generation, not_found_exception=not_found_exception)
    ...
    def XmlPassThroughGetAcl(self, storage_url, def_obj_acl=False):
        ...
        try:
            uri = boto.storage_uri(storage_url.url_string, suppress_consec_slashes=False, bucket_storage_uri_class=self.bucket_storage_uri_class, debug=self.debug)
            if def_obj_acl:
    ...
    def XmlPassThroughSetAcl(self, acl_text, storage_url, canned=True, def_obj_acl=False):
        """See CloudApiDelegator class for function doc strings."""
        headers = self._CreateBaseHeaders()
        try:
            uri = boto.storage_uri(storage_url.url_string, suppress_consec_slashes=False, bucket_storage_uri_class=self.bucket_storage_uri_class, debug=self.debug)
    ...
    def XmlPassThroughSetCors(self, cors_text, storage_url):
        ...
        try:
            ...
            raise CommandException('Requested CORS is invalid: %s at line %s, column %s' % (e.getMessage(), e.getLineNumber(), e.getColumnNumber()))
        try:
            uri = boto.storage_uri(storage_url.url_string, suppress_consec_slashes=False, bucket_storage_uri_class=self.bucket_storage_uri_class, debug=self.debug)
    ...
    def XmlPassThroughSetLifecycle(self, lifecycle_text, storage_url):
        ...
        try:
            ...
            raise CommandException('Requested lifecycle config is invalid: %s at line %s, column %s' % (e.getMessage(), e.getLineNumber(), e.getColumnNumber()))
        try:
            uri = boto.storage_uri(storage_url.url_string, suppress_consec_slashes=False, bucket_storage_uri_class=self.bucket_storage_uri_class, debug=self.debug)
