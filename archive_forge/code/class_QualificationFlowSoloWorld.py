...
class QualificationFlowSoloWorld(MTurkTaskWorld):
    ...
    """
    test_set = [['What is 1+1?', '2'], ['What is 3+2?', '5'], ['What is 6+6?', '12'], ['What is 5-3?', '2'], ['What is 6*4?', '24']]
    ...
    def parley(self):
        if self.curr_question == len(self.questions):
            ad = {'episode_done': True, 'id': self.__class__.collector_agent_id, 'text': 'Thank you for your answers!'}
            self.mturk_agent.observe(validate(ad))
            self.episodeDone = True
        else:
            ad = {'episode_done': True, 'id': self.__class__.collector_agent_id, 'text': self.questions[self.curr_question][0]}
            self.mturk_agent.observe(validate(ad))
    ...
    def shutdown(self):
        ...
        if self.firstTime and self.correct != len(self.questions):
            mturk_utils.give_worker_qualification(self.mturk_agent.worker_id, self.qualification_id, is_sandbox=self.opt['is_sandbox'])
