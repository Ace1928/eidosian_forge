...
class TestCrosstab:
    ...
    def test_crosstab_normalize_multiple_columns(self):
        ...
        expected = DataFrame(np.array([0] * 29 + [1], dtype=float).reshape(10, 3), columns=Index(['bar', 'foo', 'All'], name='C'), index=MultiIndex.from_tuples([('one', 'A'), ('one', 'B'), ('one', 'C'), ('three', 'A'), ('three', 'B'), ('three', 'C'), ('two', 'A'), ('two', 'B'), ('two', 'C'), ('All', '')], names=['A', 'B']))
        tm.assert_frame_equal(result, expected)
