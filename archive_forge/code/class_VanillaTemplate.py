...
class VanillaTemplate(BasicTemplate):
    ...
    """
    design = param.ClassSelector(class_=Design, default=Native, is_instance=False, instantiate=False, doc='\n        A Design applies a specific design system to a template.')
    _css = pathlib.Path(__file__).parent / 'vanilla.css'
    _resources: ClassVar[Dict[str, Dict[str, str]]] = {'css': {'lato': 'https://fonts.googleapis.com/css?family=Lato&subset=latin,latin-ext'}}
