...
class AuthZeroSettings(BaseSettings):
    ...
    enable_multi_audiences: Optional[bool] = None
    ...
    user_data_expiration: Optional[int] = 60 * 60 * 24 * 7
    user_session_expiration: Optional[int] = 60 * 60 * 24 * 30
    ...
    def jwks(self) -> Dict[str, Any]:
        ...
        """
        if self.jwks_dict is not None:
            return self.jwks_dict
        if self.domain is None:
            raise ValueError('AuthZero Domain must be set')
    ...
    def decrypt_api_key(self, api_key: str, prefix: Optional[str]=None, suffix: Optional[str]=None) -> Optional[str]:
        """
        Decrypts an API Key
        """
        if not self.api_key_enabled:
    ...
    async def acreate_api_key(self, user_id: str, prefix: Optional[str]=None, suffix: Optional[str]=None) -> Optional[str]:
        """
        Creates an API Key
        """
        if not self.api_key_enabled:
    ...
    async def adecrypt_api_key(self, api_key: str, prefix: Optional[str]=None, suffix: Optional[str]=None) -> Optional[str]:
        """
        Decrypts an API Key
        """
        if not self.api_key_enabled:
    ...
    def validate_app_scopes(self):
        ...
        if self.app_scopes is None:
            ...
            return
        if not isinstance(self.app_scopes, list):
            self.app_scopes = [self.app_scopes]
        else:
            self.app_scopes = list(set(self.app_scopes))

    def validate_config(self):
        """
        Handles the validation of the configuration after updates / changes
        """
        if self.has_completed_validation:
            return
        if self.client_id is None:
            raise ValueError('AuthZero Client ID must be set')
        if self.client_secret is None:
            raise ValueError('AuthZero Client Secret must be set')
        if self.domain is None:
            raise ValueError('AuthZero Domain must be set')
        tmp_data = get_az_temp_data()
        if not self.api_key_secret_key:
            if self.is_production_env:
                raise ValueError('API Key Secret Key must be set in production')
        ...
        elif len(self.api_key_secret_key) != 16:
            raise ValueError('API Key Secret Key must be 16 characters')
        if not self.api_key_access_key:
            if self.is_production_env:
                raise ValueError('API Key Access Key must be set in production')
        ...
        elif len(self.api_key_access_key) != 16:
            raise ValueError('API Key Access Key must be 16 characters')
        ...
        if self.session_cookie_key is None:
            if self.app_name and self.app_env:
                self.session_cookie_key = f'{self.app_name}-{self.app_env.name}-session'.replace(' ', '-').lower()
            elif self.app_ingress:
                self.session_cookie_key = f'{self.app_ingress}-session'.split('://', 1)[-1].replace(' ', '-').replace('.', '').lower()
    ...
    def validate_authzero_config(self):
        ...
        if self.data_dir is None:
            self.data_dir = lib_path.joinpath('data')
        if self.app_env is None:
            self.app_env = AppEnv.from_module_name('authzero')
        if self.ctx is None:
            self.ctx = AuthZeroContext
    ...
    @overload
    def configure(self, client_id: Optional[str]=None, client_secret: Optional[str]=None, audience: Optional[str]=None, audiences: Optional[List[str]]=None, apikey_header: Optional[str]=None, authorization_header: Optional[str]=None, authorization_scheme: Optional[str]=None, domain: Optional[str]=None, jwks_dict: Optional[Dict[str, Any]]=None, cache_key_prefix: Optional[str]=None, data_dir: Optional[Path]=None, api_key_prefix: Optional[str]=None, api_key_secret_key: Optional[str]=None, api_key_access_key: Optional[str]=None, api_key_suffix: Optional[str]=None, api_key_hashes: Optional[List[str]]=None, enabled: Optional[bool]=None, session_enabled: Optional[bool]=None, enable_multi_audiences: Optional[bool]=None, api_client_id_header_key: Optional[str]=None, api_client_env_header_key: Optional[str]=None, api_client_api_key_prefix: Optional[str]=None, app_name: Optional[str]=None, app_env: Optional[Union[AppEnv, str]]=None, app_ingress: Optional[str]=None, **kwargs):
        """
