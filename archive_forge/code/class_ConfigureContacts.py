...
class ConfigureContacts(base.UpdateCommand):
  ...
  """
    ...
    @staticmethod
    def Args(parser):
        resource_args.AddRegistrationResourceArg(parser, 'to configure contact settings for')
        flags.AddConfigureContactsSettingsFlagsToParser(parser)
        flags.AddValidateOnlyFlagToParser(parser, 'configure contact settings of the')
    ...
    def Run(self, args):
        ...
        public_contacts_ack = contacts_util.ParsePublicContactsAck(api_version, args.notices)
        if contacts is None:
            contacts = contacts_util.PromptForContacts(api_version, registration.contactSettings)
        if contact_privacy is None:
            choices = list(map(flags.ContactPrivacyEnumMapper(client.messages).GetChoiceForEnum, registration.supportedPrivacy))
            contact_privacy = contacts_util.PromptForContactPrivacy(api_version, choices, registration.contactSettings.privacy)
        ...
        if not public_contacts_ack and new_privacy == privacy.PUBLIC_CONTACT_DATA:
            merged_contacts = contacts_util.MergeContacts(api_version, prev_contacts=registration.contactSettings, new_contacts=contacts)
            if registration.contactSettings.privacy != privacy.PUBLIC_CONTACT_DATA:
                public_contacts_ack = contacts_util.PromptForPublicContactsUpdateAck(registration.domainName, merged_contacts)
            else:
                public_contacts_ack = contacts_util.PromptForPublicContactsAck(registration.domainName, merged_contacts)
        response = client.ConfigureContacts(registration_ref, contacts, contact_privacy, public_contacts_ack, validate_only=args.validate_only)
        if args.validate_only:
            ...
            note = None
            if not args.async_ and self.CheckPendingContacts(client, registration_ref):
            ...
            log.UpdatedResource(registration_ref.Name(), 'registration', args.async_, details=note)
        return response