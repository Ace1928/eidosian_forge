    ...
    def run(self, dag):
        ...
        if isinstance(self.layout, list):
            if len(self.layout) != len(dag.qubits):
                raise InvalidLayoutError(f'The length of the layout is different than the size of the circuit: {len(self.layout)} <> {len(dag.qubits)}')
            if len(set(self.layout)) != len(self.layout):
                raise InvalidLayoutError(f'The provided layout {self.layout} contains duplicate qubits')
            ...
            raise InvalidLayoutError(f'SetLayout was intialized with the layout type: {type(self.layout)}')
