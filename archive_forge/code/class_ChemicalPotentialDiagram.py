...
@due.dcite(Doi('10.1021/jacs.1c06229'), description='Chemical potential diagram', path='pymatgen.analysis.chempot_diagram.ChemicalPotentialDiagram')
class ChemicalPotentialDiagram(MSONable):
    ...
    def __init__(self, entries: list[PDEntry], limits: dict[Element, tuple[float, float]] | None=None, default_min_limit: float=-50.0, formal_chempots: bool=True) -> None:
        """
        Args:
    ...
    def get_plot(self, elements: list[Element | str] | None=None, label_stable: bool | None=True, formulas_to_draw: list[str] | None=None, draw_formula_meshes: bool | None=True, draw_formula_lines: bool | None=True, formula_colors: list[str]=px.colors.qualitative.Dark2, element_padding: float | None=1.0) -> Figure:
        """
        ...
        if len(elems) == 2 and self.dim == 2:
            ...
            cpd = ChemicalPotentialDiagram(entries=entries, limits=self.limits, default_min_limit=self.default_min_limit, formal_chempots=self.formal_chempots)
            fig = cpd.get_plot(elements=elems, label_stable=label_stable)
        else:
            fig = self._get_3d_plot(elements=elems, label_stable=label_stable, formulas_to_draw=formulas_to_draw, draw_formula_meshes=draw_formula_meshes, draw_formula_lines=draw_formula_lines, formula_colors=formula_colors, element_padding=element_padding)
        return fig
    ...
    def _get_hyperplanes_and_entries(self) -> tuple[np.ndarray, list[PDEntry]]:
        ...
        entries.
        """
        data = np.array([[entry.composition.get_atomic_fraction(el) for el in self.elements] + [entry.energy_per_atom] for entry in self._min_entries])
    ...
    def _get_3d_domain_lines(domains: dict[str, list[Simplex] | None]) -> list[Scatter3d]:
        ...
        return [Scatter3d(x=x, y=y, z=z, mode='lines', line={'color': 'black', 'width': 4.5}, showlegend=False)]
