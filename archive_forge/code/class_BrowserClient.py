...
class BrowserClient(BaseGlobalClient):
    ...
    async def ainit(self):
        ...
        if self._abrowser is None:
            self._abrowser = await self.aclient.chromium.launch(headless=True, downloads_path=self.settings.module_path.joinpath('data').as_posix())
    ...
    def close(self):
        ...
        """
        if self._abrowser:
            self.pooler.create_background(self._abrowser.close)
            self._abrowser = None
        if self._browser:
            self._browser.close()
            self._browser = None
        if self._apw_client:
            self.pooler.create_background(self._apw_client.stop)
