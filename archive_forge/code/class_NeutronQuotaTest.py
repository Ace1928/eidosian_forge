...
class NeutronQuotaTest(common.HeatTestCase):
    ...
    def setUp(self):
        ...
        self.patchobject(n_plugin.NeutronClientPlugin, 'has_extension', return_value=True)
        self.patchobject(n_plugin.NeutronClientPlugin, 'ignore_not_found', return_value=None)
        ...
        self.stack = parser.Stack(self.ctx, 'neutron_quota_test_stack', template.Template(tpl))
    ...
    def test_miss_all_quotas(self):
        ...
        my_quota.reparse()
        msg = 'At least one of the following properties must be specified: floatingip, network, port, rbac_policy, router, security_group, security_group_rule, subnet, subnetpool.'
    ...
    def test_quota_handle_create(self):
        ...
        body = {'quota': {'subnet': 5, 'network': 5, 'floatingip': 5, 'security_group_rule': 5, 'security_group': 5, 'router': 5, 'port': 5, 'subnetpool': 5, 'rbac_policy': 5}}
        self.update_quota.assert_called_once_with('some_project_id', body)
    ...
    def test_quota_handle_update(self):
        ...
        props = {'project': 'some_project_id', 'floatingip': 1, 'security_group': 4, 'rbac_policy': 8}
        json_snippet = rsrc_defn.ResourceDefinition(self.my_quota.name, 'OS::Neutron::Quota', properties=props)
        self.my_quota.reparse()
        self.my_quota.handle_update(json_snippet, tmpl_diff, prop_diff)
        body = {'quota': {'floatingip': 1, 'security_group': 4, 'rbac_policy': 8}}
        self.update_quota.assert_called_once_with('some_project_id', body)
