...
class TestResumableStreamingJsonUploadWrapper(testcase.GsUtilUnitTestCase):
    ...
    def _GetTestFile(self):
        if not self._temp_test_file:
            self._temp_test_file_contents = pkgutil.get_data('gslib', 'tests/test_data/%s' % _TEST_FILE)
            self._temp_test_file = self.CreateTempFile(file_name=_TEST_FILE, contents=self._temp_test_file_contents)
    ...
    """
        ...
        self.assertGreaterEqual(buffer_size, seek_back_amount, 'seek_back_amount must be less than initial position %s (but was actually: %s)' % (buffer_size, seek_back_amount))
        self.assertLess(initial_position, self._temp_test_file_len, 'initial_position must be less than test file size %s (but was actually: %s)' % (self._temp_test_file_len, initial_position))
        with open(tmp_file, 'rb') as stream:
            wrapper = ResumableStreamingJsonUploadWrapper(stream, buffer_size, test_small_buffer=True)
            position = 0
            for read_size in initial_reads:
            ...
            data = wrapper.read()
            self.assertEqual(self._temp_test_file_len - (initial_position - seek_back_amount), len(data), 'Unexpected data length with initial pos %s seek_back_amount %s. Expected: %s, actual: %s.' % (initial_position, seek_back_amount, self._temp_test_file_len - (initial_position - seek_back_amount), len(data)))
    ...
    def testSeekEnd(self):
        tmp_file = self._GetTestFile()
        for buffer_size in (TRANSFER_BUFFER_SIZE - 1, TRANSFER_BUFFER_SIZE, TRANSFER_BUFFER_SIZE + 1):
            for seek_back in (TRANSFER_BUFFER_SIZE - 1, TRANSFER_BUFFER_SIZE, TRANSFER_BUFFER_SIZE + 1):
                ...
                with open(tmp_file, 'rb') as stream:
                    ...
                    try:
                        ...
                        if expect_exception:
                            self.fail('Did not get expected CommandException for seek_back size %s, buffer size %s' % (seek_back, buffer_size))
                    except CommandException as e:
                        if not expect_exception:
                            self.fail('Got unexpected CommandException "%s" for seek_back size %s, buffer size %s' % (str(e), seek_back, buffer_size))