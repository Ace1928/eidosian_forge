...
class AbstractGenerator(ABC, metaclass=GeneratorMeta):
    ...
    :param searchpath: additional search path for custom templates.     If provided the search path has priority over searchpaths defined     in generator class.
    ...
    builtin_types = {'anyType': '', 'anySimpleType': ''}
    ...
    def __init__(self, schema, searchpath=None, types_map=None):
        ...
        if self.searchpaths:
            file_loaders.extend((FileSystemLoader(str(path)) for path in reversed(self.searchpaths)))
        if not file_loaders:
    ...
    def map_type(self, obj):
        ...
        :param obj: an XSD type or another type-related declaration as         an attribute or an element.
    ...
    def is_derived(self, xsd_type, *names, derivation=None):
        ...
        :param names: positional argument with the names of other         XSD types.
        :param derivation: the type of derivation, that can be         *extension* or *restriction*, or both with a space separator.         If no value is provided it only checks if it is derived from         or if it is the XSD type itself.
        """
