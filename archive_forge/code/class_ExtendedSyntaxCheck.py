...
class ExtendedSyntaxCheck(ModuleAnalysis):
    ...
    def visit_FunctionDef(self, node):
        if node.name in self.functions:
            raise PythranSyntaxError('Function {} redefined'.format(node.name), node)
        else:
    ...
    def check_assert_with_side_effect(self, node, arg):
        if self.inassert:
            raise PythranSyntaxError('Cannot call a function with side effect in an assert', node)
    ...
    def visit_arguments(self, node):
        ...
        for arg_default in node.defaults:
            if not self.is_immutable_constant(arg_default):
                raise PythranSyntaxError("Pythran does not support mutable default values. Use a `None' default and set the value at runtime instead.", arg_default)
    ...
    def visit_Call(self, node):
        ...
        try:
            ...
            raise PythranSyntaxError("Call to unknown function `{}`, it's a trap!".format(getattr(func, 'id', None) or func), node)
        ...
        for alias in aliases:
            ...
            if lbound == ubound:
                ...
                msg = msg.format(alias.name, len(alias.args.args), len(node.args))
            else:
                msg = 'Invalid {} call: expected between {} and {} arguments, got {}'
