...
def update_extension(target: str, vcs_ref: Optional[str]=None, interactive: bool=True) -> None:
    ...
    if (target / 'jupyter-config').exists():
        ...
        kind = 'theme'
    has_test = (target / 'conftest.py').exists() or (target / 'jest.config.js').exists() or (target / 'ui-tests').exists()
    extra_context = {'kind': kind, 'author_name': author_name, 'author_email': author_email, 'labextension_name': data['name'], 'python_name': python_name, 'project_short_description': data.get('description', '<description>'), 'has_settings': bool(data.get('jupyterlab', {}).get('schemaDir', '')), 'has_binder': bool((target / 'binder').exists()), 'test': bool(has_test), 'repository': data.get('repository', {}).get('url', '<repository')}
    ...
    linters = {'eslintConfig': '.eslintrc.js', 'eslintIgnore': '.eslintignore', 'prettier': '.prettierrc', 'stylelint': '.stylelintrc'}
        ...
        if (target / 'setup.cfg').exists():
            try:
                ...
                if requirements_raw is not None:
                    requirements = list(filter(lambda r: r and JUPYTER_SERVER_REQUIREMENT.match(r) is None, requirements_raw.splitlines()))
                else:
                    requirements = []
