...
def _extract_and_handle_mpbgp_new_paths(self, update_msg):
    ...
    if msg_rf not in SUPPORTED_GLOBAL_RF:
        LOG.info('Received route for route family %s which is not supported. Ignoring paths from this UPDATE: %s' % (msg_rf, update_msg))
        return
    if msg_rf in (RF_IPv4_VPN, RF_IPv6_VPN):
        ext_comm_attr = umsg_pattrs.get(BGP_ATTR_TYPE_EXTENDED_COMMUNITIES)
        if not ext_comm_attr:
            LOG.info('Missing Extended Communities Attribute. Ignoring paths from this UPDATE: %s', update_msg)
            return
        msg_rts = ext_comm_attr.rt_list
        if not msg_rts:
            LOG.info('Received route with no RTs. Ignoring paths in this UPDATE: %s', update_msg)
            return
        interested_rts = self._core_service.global_interested_rts
        if not interested_rts.intersection(msg_rts):
            LOG.info('Received route with RT %s that is of no interest to any VRFs or Peers %s. Ignoring paths from this UPDATE: %s', msg_rts, interested_rts, update_msg)
            return
    ...
    for msg_nlri in msg_nlri_list:
        ...
        if not block:
            if msg_rf == RF_RTC_UC and self._init_rtc_nlri_path is not None:
