...
class MomentoVectorIndex(VectorStore):
    ...
    def __init__(self, embedding: Embeddings, client: 'PreviewVectorIndexClient', index_name: str='default', distance_strategy: DistanceStrategy=DistanceStrategy.COSINE, text_field: str='text', ensure_index_exists: bool=True, **kwargs: Any):
    ...
    def add_texts(self, texts: Iterable[str], metadatas: Optional[List[dict]]=None, **kwargs: Any) -> List[str]:
        ...
        for i in range(0, len(embeddings), batch_size):
            ...
            items = [Item(id=id, vector=vector, metadata=metadata) for id, vector, metadata in zip(ids[start:end], embeddings[start:end], metadatas[start:end])]
    ...
    def similarity_search_with_score_by_vector(self, embedding: List[float], k: int=4, **kwargs: Any) -> List[Tuple[Document, float]]:
        ...
        response = self._client.search(self.index_name, embedding, top_k=k, metadata_fields=ALL_METADATA, filter_expression=filter_expression)
    ...
    def max_marginal_relevance_search_by_vector(self, embedding: List[float], k: int=4, fetch_k: int=20, lambda_mult: float=0.5, **kwargs: Any) -> List[Document]:
        ...
        filter_expression = kwargs.get('filter_expression', None)
        response = self._client.search_and_fetch_vectors(self.index_name, embedding, top_k=fetch_k, metadata_fields=ALL_METADATA, filter_expression=filter_expression)
        if isinstance(response, SearchAndFetchVectors.Success):
            ...
            logger.error(f'Unexpected response: {response}')
            raise Exception(f'Unexpected response: {response}')
        mmr_selected = maximal_marginal_relevance(query_embedding=np.array([embedding], dtype=np.float32), embedding_list=[hit.vector for hit in response.hits], lambda_mult=lambda_mult, k=k)
    ...
    def from_texts(cls: Type[VST], texts: List[str], embedding: Embeddings, metadatas: Optional[List[dict]]=None, **kwargs: Any) -> VST:
        ...
        if 'client' in kwargs:
            ...
            client = PreviewVectorIndexClient(configuration=VectorIndexConfigurations.Default.latest(), credential_provider=CredentialProvider.from_string(api_key))
