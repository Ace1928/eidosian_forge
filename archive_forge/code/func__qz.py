...
def _qz(A, B, output='real', lwork=None, sort=None, overwrite_a=False, overwrite_b=False, check_finite=True):
    ...
    if info < 0:
        ...
        warnings.warn(f'The QZ iteration failed. (a,b) are not in Schur form, but ALPHAR(j), ALPHAI(j), and BETA(j) should be correct for J={info - 1},...,N', LinAlgWarning, stacklevel=3)
    elif info == a_n + 1:
