...
def test_search_cv_verbose_3(capsys, return_train_score):
    ...
    grid = {'C': [0.1]}
    GridSearchCV(clf, grid, scoring='accuracy', verbose=3, cv=3, return_train_score=return_train_score).fit(X, y)
    captured = capsys.readouterr().out
    if return_train_score:
        match = re.findall('score=\\(train=[\\d\\.]+, test=[\\d.]+\\)', captured)
    else:
        match = re.findall('score=[\\d\\.]+', captured)
    assert len(match) == 3