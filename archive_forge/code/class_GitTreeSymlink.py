...
class GitTreeSymlink(_mod_tree.TreeLink):
    ...
    def __repr__(self):
        return '{}(file_id={!r}, name={!r}, parent_id={!r}, symlink_target={!r})'.format(type(self).__name__, self.file_id, self.name, self.parent_id, self.symlink_target)

    def __eq__(self, other):
        return self.kind == other.kind and self.file_id == other.file_id and (self.name == other.name) and (self.parent_id == other.parent_id) and (self.symlink_target == other.symlink_target)

    def copy(self):
        return self.__class__(self.file_id, self.name, self.parent_id, self.symlink_target)