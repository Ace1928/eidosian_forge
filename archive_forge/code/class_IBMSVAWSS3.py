...
class IBMSVAWSS3:
    ...
    def __init__(self):
        ...
        argument_spec.update(dict(state=dict(type='str', choices=['present', 'absent'], required=True), name=dict(type='str', required=True), old_name=dict(type='str'), bucketprefix=dict(type='str'), accesskeyid=dict(type='str', no_log=False), secretaccesskey=dict(type='str', no_log=True), upbandwidthmbits=dict(type='str'), downbandwidthmbits=dict(type='str'), region=dict(type='str'), encrypt=dict(type='str', choices=['yes', 'no']), ignorefailures=dict(type='bool'), mode=dict(type='str', choices=['import', 'normal']), importsystem=dict(type='str'), refresh=dict(type='bool'), resetusagehistory=dict(type='bool')))
    ...
    def basic_checks(self):
        ...
        if self.state == 'present':
            ...
            invalids = ('bucketprefix', 'accesskeyid', 'secretaccesskey', 'upbandwidthmbits', 'downbandwidthmbits', 'region', 'encrypt', 'ignorefailures', 'mode', 'importsystem', 'refresh', 'resetusagehistory', 'old_name')
    ...
    def create_validation(self):
        ...
        invalids = ('ignorefailures', 'mode', 'importsystem', 'refresh', 'resetusagehistory')
        invalid_exists = ', '.join((var for var in invalids if getattr(self, var) not in {'', None}))
        if invalid_exists:
    ...
    def rename_validation(self, updates):
        if self.old_name and self.name:
            ...
            if existing:
                if new:
                    self.module.fail_json(msg='Cloud account ({0}) already exists for the given new name.'.format(self.name))
                else:
                    updates['name'] = self.name
            elif not new:
                self.module.fail_json(msg='Cloud account ({0}) does not exists for the given old name.'.format(self.old_name))
            else:
                self.module.exit_json(msg='Cloud account ({0}) already renamed. No modifications done.'.format(self.name))
