...
class SearxSearchWrapper(BaseModel):
    ...
    def validate_params(cls, values: Dict) -> Dict:
        ...
        if not searx_host.startswith('http'):
            print(f'Warning: missing the url scheme on host                 ! assuming secure https://{searx_host} ')
    ...
    async def _asearx_api_query(self, params: dict) -> SearxResults:
        if not self.aiosession:
            async with aiohttp.ClientSession() as session:
                async with session.get(self.searx_host, headers=self.headers, params=params, ssl=(lambda: False if self.unsecure else None)()) as response:
                    if not response.ok:
                        raise ValueError('Searx API returned an error: ', response.text)
                    result = SearxResults(await response.text())
                    self._result = result
        else:
            async with self.aiosession.get(self.searx_host, headers=self.headers, params=params, verify=not self.unsecure) as response:
                if not response.ok:
                    raise ValueError('Searx API returned an error: ', response.text)
                result = SearxResults(await response.text())
                self._result = result
    ...
    def run(self, query: str, engines: Optional[List[str]]=None, categories: Optional[List[str]]=None, query_suffix: Optional[str]='', **kwargs: Any) -> str:
    ...
    async def arun(self, query: str, engines: Optional[List[str]]=None, query_suffix: Optional[str]='', **kwargs: Any) -> str:
    ...
    def results(self, query: str, num_results: int, engines: Optional[List[str]]=None, categories: Optional[List[str]]=None, query_suffix: Optional[str]='', **kwargs: Any) -> List[Dict]:
        ...
        return [{'snippet': result.get('content', ''), 'title': result['title'], 'link': result['url'], 'engines': result['engines'], 'category': result['category']} for result in results]

    async def aresults(self, query: str, num_results: int, engines: Optional[List[str]]=None, query_suffix: Optional[str]='', **kwargs: Any) -> List[Dict]:
