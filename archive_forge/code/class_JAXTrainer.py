...
class JAXTrainer(base_trainer.Trainer):
    ...
    def train_step(self, state, data):
        ...
        (loss, aux), grads = grad_fn(trainable_variables, non_trainable_variables, x, y, sample_weight, training=True, optimizer_variables=optimizer_variables)
        ...
        with backend.StatelessScope(state_mapping=[(ref_v, v) for ref_v, v in zip(self.metrics_variables, metrics_variables)]) as scope:
        ...
        state = self._enforce_jax_state_sharding(trainable_variables, non_trainable_variables, optimizer_variables, metrics_variables)
        return (logs, state)

    def test_step(self, state, data):
        trainable_variables, non_trainable_variables, metrics_variables = state
        x, y, sample_weight = data_adapter_utils.unpack_x_y_sample_weight(data)
        loss, aux = self.compute_loss_and_updates(trainable_variables, non_trainable_variables, x, y, sample_weight, training=False)
        unscaled_loss, y_pred, non_trainable_variables = aux
        with backend.StatelessScope(state_mapping=[(ref_v, v) for ref_v, v in zip(self.metrics_variables, metrics_variables)]) as scope:
            self._loss_tracker.update_state(unscaled_loss)
            logs = self.compute_metrics(x, y, y_pred, sample_weight)
        ...
        for ref_v in self.metrics_variables:
            ...
            new_metrics_variables.append(new_v)
        metrics_variables = new_metrics_variables
        trainable_variables, non_trainable_variables, _, metrics_variables = self._enforce_jax_state_sharding(trainable_variables=trainable_variables, non_trainable_variables=non_trainable_variables, optimizer_variables=None, metrics_variables=metrics_variables)
    ...
    def predict_step(self, state, data):
        ...
        outputs, non_trainable_variables = self.stateless_call(trainable_variables, non_trainable_variables, x, **kwargs)
        _, non_trainable_variables, _, _ = self._enforce_jax_state_sharding(trainable_variables=None, non_trainable_variables=non_trainable_variables, optimizer_variables=None, metrics_variables=None)
    ...
    def make_predict_function(self, force=False):
        ...
        def multi_predict_steps(state, data):
            ...
            for single_step_data in data[1:]:
                step_outputs, trainable_variables = one_predict_step(state, [single_step_data])
                outputs = tree.map_structure(lambda t1, t2: jax.numpy.concatenate([t1, t2]), outputs, step_outputs)
    ...
    def fit(self, x=None, y=None, batch_size=None, epochs=1, verbose='auto', callbacks=None, validation_split=0.0, validation_data=None, shuffle=True, class_weight=None, sample_weight=None, initial_epoch=0, steps_per_epoch=None, validation_steps=None, validation_batch_size=None, validation_freq=1):
        ...
        epoch_iterator = JAXEpochIterator(x=x, y=y, sample_weight=sample_weight, batch_size=batch_size, steps_per_epoch=steps_per_epoch, shuffle=shuffle, class_weight=class_weight, steps_per_execution=self.steps_per_execution)
        ...
        for epoch in range(initial_epoch, epochs):
            ...
            for step, data in epoch_iterator.enumerate_epoch():
                ...
                if self._jax_state_synced:
                    state = self._get_jax_state(trainable_variables=True, non_trainable_variables=True, optimizer_variables=True, metrics_variables=True, purge_model_variables=True)
                    self._jax_state_synced = False
                logs, state = self.train_function(state, data)
                trainable_variables, non_trainable_variables, optimizer_variables, metrics_variables = state
                self._jax_state = {'trainable_variables': trainable_variables, 'non_trainable_variables': non_trainable_variables, 'optimizer_variables': optimizer_variables, 'metrics_variables': metrics_variables}
            ...
            if validation_data is not None and self._should_eval(epoch, validation_freq):
                if getattr(self, '_eval_epoch_iterator', None) is None:
                    self._eval_epoch_iterator = JAXEpochIterator(x=val_x, y=val_y, sample_weight=val_sample_weight, batch_size=validation_batch_size or batch_size, steps_per_execution=self.steps_per_execution, steps_per_epoch=validation_steps, shuffle=False)
    ...
    def evaluate(self, x=None, y=None, batch_size=None, verbose='auto', sample_weight=None, steps=None, callbacks=None, return_dict=False, **kwargs):
        ...
        if use_cached_eval_dataset:
            ...
            epoch_iterator = JAXEpochIterator(x=x, y=y, sample_weight=sample_weight, batch_size=batch_size, steps_per_epoch=steps, shuffle=False, steps_per_execution=self.steps_per_execution)
        self._symbolic_build(iterator=epoch_iterator)
        if not isinstance(callbacks, callbacks_module.CallbackList):
        ...
        for step, data in epoch_iterator.enumerate_epoch():
            ...
            if self._jax_state_synced:
                state = self._get_jax_state(trainable_variables=True, non_trainable_variables=True, metrics_variables=True, purge_model_variables=True)
                self._jax_state_synced = False
            logs, state = self.test_function(state, data)
            trainable_variables, non_trainable_variables, metrics_variables = state
            self._jax_state = {'trainable_variables': trainable_variables, 'non_trainable_variables': non_trainable_variables, 'metrics_variables': metrics_variables}
    ...
    def predict(self, x, batch_size=None, verbose='auto', steps=None, callbacks=None):
        epoch_iterator = JAXEpochIterator(x=x, batch_size=batch_size, steps_per_epoch=steps, shuffle=False, steps_per_execution=self.steps_per_execution)
        ...
        def append_to_outputs(batch_outputs, outputs):
            if outputs is None:
                outputs = tree.map_structure(lambda batch_output: [batch_output], batch_outputs)
            else:
        ...
        for step, x in epoch_iterator.enumerate_epoch():
            ...
            if self._jax_state_synced:
                state = self._get_jax_state(trainable_variables=True, non_trainable_variables=True)
            ...
            if self.stop_predicting:
                break
        self._jax_state = {'non_trainable_variables': non_trainable_variables}
        self.jax_state_sync()
    ...
    def train_on_batch(self, x, y=None, sample_weight=None, class_weight=None, return_dict=False):
        ...
        self._record_training_state_sharding_spec()
        self.make_train_function()
        state = self._get_jax_state(trainable_variables=True, non_trainable_variables=True, optimizer_variables=True, metrics_variables=True, purge_model_variables=False)
        self._jax_state_synced = False
        logs, state = self.train_function(state, [data])
        trainable_variables, non_trainable_variables, optimizer_variables, metrics_variables = state
        self._jax_state = {'trainable_variables': trainable_variables, 'non_trainable_variables': non_trainable_variables, 'optimizer_variables': optimizer_variables, 'metrics_variables': metrics_variables}
    ...
    def test_on_batch(self, x, y=None, sample_weight=None, return_dict=False):
        ...
        self._record_training_state_sharding_spec()
        self.make_test_function()
        state = self._get_jax_state(trainable_variables=True, non_trainable_variables=True, metrics_variables=True, purge_model_variables=False)
        self._jax_state_synced = False
        logs, state = self.test_function(state, [data])
        trainable_variables, non_trainable_variables, metrics_variables = state
        self._jax_state = {'trainable_variables': trainable_variables, 'non_trainable_variables': non_trainable_variables, 'metrics_variables': metrics_variables}
    ...
    def predict_on_batch(self, x):
        ...
        self._record_training_state_sharding_spec()
        self.make_predict_function()
        state = self._get_jax_state(trainable_variables=True, non_trainable_variables=True, metrics_variables=False, purge_model_variables=False)
        self._jax_state_synced = False
        batch_outputs, non_trainable_variables = self.predict_function(state, [(x,)])
        self._jax_state = {'non_trainable_variables': non_trainable_variables}
        self.jax_state_sync()
