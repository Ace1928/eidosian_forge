...
@deprecated(since='0.0.12', removal='0.2.0', alternative_import='langchain_google_vertexai.VectorSearchVectorStore')
class MatchingEngine(VectorStore):
    ...
    def __init__(self, project_id: str, index: MatchingEngineIndex, endpoint: MatchingEngineIndexEndpoint, embedding: Embeddings, gcs_client: storage.Client, gcs_bucket_name: str, credentials: Optional[Credentials]=None, *, document_id_key: Optional[str]=None):
    ...
    def add_texts(self, texts: Iterable[str], metadatas: Optional[List[dict]]=None, **kwargs: Any) -> List[str]:
        ...
        if metadatas is not None and len(texts) != len(metadatas):
            raise ValueError(f'texts and metadatas do not have the same length. Received {len(texts)} texts and {len(metadatas)} metadatas.')
        ...
        logger.debug(f'Uploaded updated json with embeddings to {self.gcs_bucket_name}/{filename}.')
    ...
    def similarity_search_with_score(self, query: str, k: int=4, filter: Optional[List[Namespace]]=None) -> List[Tuple[Document, float]]:
    ...
    def similarity_search_by_vector_with_score(self, embedding: List[float], k: int=4, filter: Optional[List[Namespace]]=None) -> List[Tuple[Document, float]]:
        ...
        if hasattr(self.endpoint, '_public_match_client') and self.endpoint._public_match_client:
            response = self.endpoint.find_neighbors(deployed_index_id=self._get_index_id(), queries=[embedding], num_neighbors=k, filter=filter)
        else:
            response = self.endpoint.match(deployed_index_id=self._get_index_id(), queries=[embedding], num_neighbors=k, filter=filter)
    ...
    def similarity_search(self, query: str, k: int=4, filter: Optional[List[Namespace]]=None, **kwargs: Any) -> List[Document]:
    ...
    def similarity_search_by_vector(self, embedding: List[float], k: int=4, filter: Optional[List[Namespace]]=None, **kwargs: Any) -> List[Document]:
    ...
    @classmethod
    def from_texts(cls: Type['MatchingEngine'], texts: List[str], embedding: Embeddings, metadatas: Optional[List[dict]]=None, **kwargs: Any) -> 'MatchingEngine':
        """Use from components instead."""
        raise NotImplementedError('This method is not implemented. Instead, you should initialize the class with `MatchingEngine.from_components(...)` and then call `add_texts`')

    @classmethod
    def from_components(cls: Type['MatchingEngine'], project_id: str, region: str, gcs_bucket_name: str, index_id: str, endpoint_id: str, credentials_path: Optional[str]=None, embedding: Optional[Embeddings]=None, **kwargs: Any) -> 'MatchingEngine':
        ...
        endpoint = cls._create_endpoint_by_id(endpoint_id, project_id, region, credentials)
        gcs_client = cls._get_gcs_client(credentials, project_id)
        cls._init_aiplatform(project_id, region, gcs_bucket_name, credentials)
        return cls(project_id=project_id, index=index, endpoint=endpoint, embedding=embedding or cls._get_default_embeddings(), gcs_client=gcs_client, credentials=credentials, gcs_bucket_name=gcs_bucket_name, **kwargs)
    ...
    @classmethod
    def _create_index_by_id(cls, index_id: str, project_id: str, region: str, credentials: 'Credentials') -> MatchingEngineIndex:
        ...
        return aiplatform.MatchingEngineIndex(index_name=index_id, project=project_id, location=region, credentials=credentials)
