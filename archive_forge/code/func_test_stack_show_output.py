...
def test_stack_show_output(self):
    ...
    self.assertEqual({'output_key': 'test', 'output_value': 'first', 'description': 'sec'}, output)
    ex = self.assertRaises(dispatcher.ExpectedException, self.eng.show_output, self.ctx, mock.ANY, 'bunny')
    self.assertEqual(exception.NotFound, ex.exc_info[0])
    self.assertEqual('Specified output key bunny not found.', str(ex.exc_info[1]))