...
def cli_coverage(arguments: List[str]):
    parser = argparse.ArgumentParser(description='Embedded detection success coverage script checker for Charset-Normalizer')
    parser.add_argument('-p', '--with-preemptive', action='store_true', default=False, dest='preemptive', help='Enable the preemptive scan behaviour during coverage check')
    parser.add_argument('-c', '--coverage', action='store', default=90, type=int, dest='coverage', help='Define the minimum acceptable coverage to succeed')
    ...
    for tbt_path in sorted(glob('./char-dataset/**/*.*')):
        ...
        if calc_eq >= 0.98:
            success_count += 1
            print("️✅ ️'{}' (got '{}' but equivalence {} %)".format(tbt_path, result.encoding, round(calc_eq * 100.0, 3)))
            continue
