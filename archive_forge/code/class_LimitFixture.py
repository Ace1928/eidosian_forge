...
class LimitFixture(fixtures.Fixture):
    ...
    def setUp(self):
        ...
        self.useFixture(fixtures.MockPatch('oslo_limit.limit._SDK_CONNECTION', new=self.mock_conn))
        mock_gem = self.useFixture(fixtures.MockPatch('oslo_limit.limit.Enforcer._get_enforcement_model')).mock
        ...
        def fake_limits(service_id, region_id, resource_name, project_id=None):
            ...
            if project_id is None:
                ...
                this_limit.resource_limit = self.projlimits.get(project_id, {}).get(resource_name)
