...
class AdjustInstanceSplit(base.Command):
    ...
    """
        ...
        if not changes:
            raise exceptions.NoConfigurationChangeError('No instance split configuration change requested.')
        ...
        with serverless_operations.Connect(conn_context) as client:
            ...
            try:
                with progress_tracker.StagedProgressTracker('Updating instance split...', instance_split_stages, failure_message='Updating intance split failed', suppress_output=args.async_) as tracker:
                    worker = client.UpdateInstanceSplit(worker_ref, changes, tracker, args.async_)
            except Exception:
                worker = client.GetWorker(worker_ref)
                if worker:
                    resources = traffic_pair.GetTrafficTargetPairs(worker.spec_traffic, worker.status_traffic, True, worker.status.latestReadyRevisionName, worker.status.url)
