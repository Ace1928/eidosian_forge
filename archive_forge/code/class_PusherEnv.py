...
class PusherEnv(MujocoEnv, utils.EzPickle):
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def __init__(self, **kwargs):
        ...
        MujocoEnv.__init__(self, 'pusher.xml', 5, observation_space=observation_space, **kwargs)
    ...
    def reset_model(self):
        ...
        while True:
            self.cylinder_pos = np.concatenate([self.np_random.uniform(low=-0.3, high=0, size=1), self.np_random.uniform(low=-0.2, high=0.2, size=1)])
    ...
    def _get_obs(self):
        return np.concatenate([self.data.qpos.flat[:7], self.data.qvel.flat[:7], self.get_body_com('tips_arm'), self.get_body_com('object'), self.get_body_com('goal')])