...
class EcloudESVectorStore(VectorStore):
    ...
    def __init__(self, index_name: str, es_url: str, user: Optional[str]=None, password: Optional[str]=None, embedding: Optional[Embeddings]=None, **kwargs: Optional[dict]) -> None:
        ...
        key_list = ['text_field', 'vector_field', 'vector_type', 'vector_params', 'index_settings']
    ...
    def _search(self, query: Optional[str]=None, query_vector: Union[List[float], None]=None, filter: Optional[dict]=None, custom_query: Optional[Callable[[Dict, Union[str, None]], Dict]]=None, search_params: Dict={}) -> List[Tuple[Document, float]]:
        ...
        docs_and_scores = [(Document(page_content=hit['_source'][search_params.get('text_field', self.text_field)], metadata=hit['_source']['metadata']), hit['_score']) for hit in hits]
    ...
    @classmethod
    def from_documents(cls, documents: List[Document], embedding: Optional[Embeddings]=None, **kwargs: Any) -> 'EcloudESVectorStore':
    ...
    @classmethod
    def from_texts(cls, texts: List[str], embedding: Optional[Embeddings]=None, metadatas: Optional[List[Dict[str, Any]]]=None, **kwargs: Any) -> 'EcloudESVectorStore':
    ...
    def add_texts(self, texts: Iterable[str], metadatas: Optional[List[Dict[Any, Any]]]=None, **kwargs: Any) -> List[str]:
        ...
        if self.embedding is not None:
            ...
            for i, (text, vector) in enumerate(zip(texts, embeddings)):
                ...
                doc = {'_op_type': 'index', '_index': self.index_name, self.text_field: text, 'metadata': metadata, '_id': ids[i]}
                if self.vector_type == 'knn_dense_float_vector':
                    doc[self.vector_field] = vector
                elif self.vector_type == 'knn_sparse_bool_vector':
                    doc[self.vector_field] = {'true_indices': vector, 'total_indices': len(vector)}
                requests.append(doc)
        else:
            if create_index_if_not_exists:
            ...
            for i, text in enumerate(texts):
                metadata = metadatas[i] if metadatas else {}
                requests.append({'_op_type': 'index', '_index': self.index_name, self.text_field: text, 'metadata': metadata, '_id': ids[i]})
        if len(requests) > 0:
            try:
