...
def GetDecryptionCSEK(src_url, src_obj_metadata):
  ...
  """
    if src_obj_metadata.customerEncryption:
        decryption_key = FindMatchingCSEKInBotoConfig(src_obj_metadata.customerEncryption.keySha256, config)
        if not decryption_key:
            raise EncryptionException('Missing decryption key with SHA256 hash %s. No decryption key matches object %s' % (src_obj_metadata.customerEncryption.keySha256, src_url))
