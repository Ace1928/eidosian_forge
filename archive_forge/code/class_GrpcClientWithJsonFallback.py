...
class GrpcClientWithJsonFallback(gcs_json_client.JsonClient):
    ...
    ALLOWED_PREDFINED_DESTINATION_ACL_VALUES = ('authenticatedRead', 'bucketOwnerFullControl', 'bucketOwnerRead', 'private', 'projectPrivate', 'publicRead')
    ...
    def delete_object(self, object_url, request_config):
        ...
        if object_url.generation is not None:
            ...
            generation = None
        self._get_gapic_client()
        request = self._gapic_client.types.DeleteObjectRequest(bucket=object_url.bucket_name, object=object_url.object_name, generation=generation, if_generation_match=request_config.precondition_generation_match, if_metageneration_match=request_config.precondition_metageneration_match)
    ...
    def restore_object(self, url, request_config):
        ...
        if request_config.resource_args:
            ...
            preserve_acl = None
        self._get_gapic_client()
        object_metadata = self._gapic_client.storage.restore_object(self._gapic_client.types.RestoreObjectRequest(bucket=url.bucket_name, object=url.object_name, generation=int(url.generation), if_generation_match=request_config.precondition_generation_match, if_metageneration_match=request_config.precondition_metageneration_match, copy_source_acl=preserve_acl))
    ...
    def download_object(self, cloud_resource, download_stream, request_config, digesters=None, do_not_decompress=False, download_strategy=cloud_api.DownloadStrategy.RESUMABLE, progress_callback=None, start_byte=0, end_byte=None):
        ...
        if download_util.return_and_report_if_nothing_to_download(cloud_resource, progress_callback):
            return None
        if request_config.resource_args is not None and request_config.resource_args.decryption_key is not None:
            decryption_key = request_config.resource_args.decryption_key
        else:
            decryption_key = None
        downloader = download.GrpcDownload(gapic_client=self._get_gapic_client(), cloud_resource=cloud_resource, download_stream=download_stream, start_byte=start_byte, end_byte=end_byte, digesters=digesters, progress_callback=progress_callback, download_strategy=download_strategy, decryption_key=decryption_key)
        downloader.run()
        return None
