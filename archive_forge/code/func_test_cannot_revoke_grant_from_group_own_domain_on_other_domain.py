...
def test_cannot_revoke_grant_from_group_own_domain_on_other_domain(self):
    ...
    with self.test_client() as c:
        c.delete('/v3/domains/%s/groups/%s/roles/%s' % (domain_id, group['id'], self.bootstrapper.reader_role_id), headers=self.headers, expected_status_code=http.client.FORBIDDEN)