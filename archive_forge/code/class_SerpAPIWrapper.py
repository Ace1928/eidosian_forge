...
class SerpAPIWrapper(BaseModel):
    ...
    """
    search_engine: Any
    params: dict = Field(default={'engine': 'google', 'google_domain': 'google.com', 'gl': 'us', 'hl': 'en'})
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        try:
            ...
            raise ValueError('Could not import serpapi python package. Please install it with `pip install google-search-results`.')
        return values
    ...
    def _process_response(res: dict) -> str:
        ...
        if 'knowledge_graph' in res.keys():
            ...
            for key, value in knowledge_graph.items():
                if isinstance(key, str) and isinstance(value, str) and (key not in ['title', 'description']) and (not key.endswith('_stick')) and (not key.endswith('_link')) and (not value.startswith('http')):
