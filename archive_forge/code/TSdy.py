...
def heuristic_evaluation(board: np.ndarray) -> float:
    """
    Evaluates the board state based on a comprehensive heuristic that combines several factors.

    This heuristic emphasizes the importance of empty tiles, the highest tile value, smoothness,
    monotonicity, and a snake pattern for tile arrangement. It also penalizes states where the
    highest tile is not located in a corner.

    Args:
        board (np.ndarray): The current state of the game board.

    Returns:
        float: The heuristic value of the board, combining all considered factors.
    """
    # Define the snake pattern for heuristic evaluation
