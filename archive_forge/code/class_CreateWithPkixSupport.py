...
class CreateWithPkixSupport(base.CreateCommand):
  ...
  """
    ...
    def Args(cls, parser):
        ...
        parser.add_argument('--signature-file', required=True, type=str, help=textwrap.dedent('          Path to file containing the signature to store, or `-` to read\n          signature from stdin.'))
        parser.add_argument('--payload-file', required=False, type=str, help=textwrap.dedent('          Path to file containing the payload over which the signature was\n          calculated.\n\n          This defaults to the output of the standard payload command:\n\n              $ {grandparent_command} create-signature-payload\n\n          NOTE: If you sign a payload with e.g. different whitespace or\n          formatting, you must explicitly provide the payload content via this\n          flag.\n          '))
        exclusive_group = parser.add_mutually_exclusive_group()
        group = exclusive_group.add_group()
        flags.AddConcepts(group, flags.GetAttestorPresentationSpec(base_name='attestor', required=False, positional=False, use_global_project_flag=False, group_help=textwrap.dedent("              The Attestor whose Container Analysis Note will be used to host\n              the created attestation. In order to successfully attach the\n              attestation, the active gcloud account (core/account) must\n              be able to read this attestor and must have the\n              `containeranalysis.notes.attachOccurrence` permission for the\n              Attestor's underlying Note resource (usually via the\n              `containeranalysis.notes.attacher` role).")))
        flags.AddConcepts(exclusive_group, flags.GetNotePresentationSpec(base_name='note', required=False, positional=False, group_help=textwrap.dedent('          The Container Analysis Note which will be used to host\n          the created attestation. In order to successfully attach the\n          attestation, the active gcloud account (core/account) must have the\n          `containeranalysis.notes.attachOccurrence` permission for the\n          Note (usually via the `containeranalysis.notes.attacher` role).')))
        parser.add_argument('--public-key-id', required=True, type=str, help=textwrap.dedent('          The ID of the public key that will be used to verify the signature\n          of the created Attestation. This ID must match the one found on the\n          Attestor resource(s) which will verify this Attestation.\n\n          For PKIX keys, this will be the URI-formatted `id` field of the\n          associated Attestor public key.\n\n          For PGP keys, this must be the version 4, full 160-bit fingerprint,\n          expressed as a 40 character hexadecimal string. See\n          https://tools.ietf.org/html/rfc4880#section-12.2 for details.'))
        group.add_argument('--validate', action='store_true', default=False, help=textwrap.dedent('          Whether to validate that the Attestation can be verified by the\n          provided Attestor.\n        '))

    def Run(self, args):
        project_ref = resources.REGISTRY.Parse(properties.VALUES.core.project.Get(required=True), collection='cloudresourcemanager.projects')
        artifact_url_without_scheme = binauthz_command_util.RemoveArtifactUrlScheme(args.artifact_url)
        ...
        if args.payload_file:
            ...
            payload = binauthz_command_util.MakeSignaturePayload(artifact_url_without_scheme)
        if args.attestor:
            attestor_ref = args.CONCEPTS.attestor.Parse()
            api_version = apis.GetApiVersion(self.ReleaseTrack())
            client = attestors.Client(api_version)
            attestor = client.Get(attestor_ref)
            validation_callback = functools.partial(validation.validate_attestation, attestor_ref=attestor_ref, api_version=api_version)
            return containeranalysis.Client().CreateAttestationOccurrence(project_ref=project_ref, note_ref=resources.REGISTRY.ParseResourceId('containeranalysis.projects.notes', client.GetNoteAttr(attestor).noteReference, {}), artifact_url=artifact_url_without_scheme, public_key_id=args.public_key_id, signature=signature, plaintext=payload, validation_callback=validation_callback if 'validate' in args and args.validate else None)
        elif args.note:
            note_ref = args.CONCEPTS.note.Parse()
            return containeranalysis.Client(ca_apis.GetApiVersion(self.ReleaseTrack())).CreateAttestationOccurrence(project_ref=project_ref, note_ref=note_ref, artifact_url=artifact_url_without_scheme, public_key_id=args.public_key_id, signature=signature, plaintext=payload, validation_callback=None)
        else:
            raise exceptions.InvalidArgumentError('One of --attestor or --note must be provided.')