...
class StorPoolConnector(base.BaseLinuxConnector):
    ...
    def __init__(self, root_helper, driver=None, *args, **kwargs):
        super(StorPoolConnector, self).__init__(root_helper, *args, driver=driver, **kwargs)
        if spopenstack is not None:
            try:
                self._attach = spopenstack.AttachDB(log=LOG)
            except Exception as e:
                raise exception.BrickException('Could not initialize the StorPool API bindings: %s' % e)
        else:
            self._attach = None
    ...
    def connect_volume(self, connection_properties):
        ...
        if client_id is None:
            raise exception.BrickException('Invalid StorPool connection data, no client ID specified.')
        volume_id = connection_properties.get('volume', None)
        if volume_id is None:
            raise exception.BrickException('Invalid StorPool connection data, no volume ID specified.')
        ...
        if mode is None or mode not in ('rw', 'ro'):
            raise exception.BrickException('Invalid access_mode specified in the connection data.')
        req_id = 'brick-%s-%s' % (client_id, volume_id)
        self._attach.add(req_id, {'volume': volume, 'type': 'brick', 'id': req_id, 'rights': 1 if mode == 'ro' else 2, 'volsnap': False})
    ...
    def get_volume_paths(self, connection_properties):
        ...
        if dpath is not None and dpath != path:
            raise exception.BrickException('Internal error: StorPool volume path {path} does not match device path {dpath}', {path: path, dpath: dpath})
        return [path]
    ...
    def get_all_available_volumes(self, connection_properties=None):
        ...
        if os.path.isdir('/dev/storpool'):
            ...
            for entry in files:
                ...
                if entry.startswith(prefix) and os.path.islink(full) and (not os.path.isdir(full)):
                    names.append(entry[prefixlen:])
        return names

    def _get_device_size(self, device):
        """Get the size in bytes of a volume."""
        out, _err = self._execute('blockdev', '--getsize64', device, run_as_root=True, root_helper=self._root_helper)
        var = str(out).strip()
        if var.isnumeric():
            return int(var)
        else:
    ...
    def extend_volume(self, connection_properties):
        ...
        if volume_id is None:
            raise exception.BrickException('Invalid StorPool connection data, no volume ID specified.')
        volume = self._attach.volumeName(volume_id)
        LOG.debug('Querying the StorPool API for the size of %(vol)s', {'vol': volume})
