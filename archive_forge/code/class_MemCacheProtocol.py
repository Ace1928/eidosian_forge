...
class MemCacheProtocol(LineReceiver, TimeoutMixin):
    ...
    def _set(self, cmd, key, val, flags, expireTime, cas):
        ...
        if not isinstance(val, bytes):
            return fail(ClientError(f'Invalid type for value: {type(val)}, expecting bytes'))
        if cas:
            cas = b' ' + cas
        length = len(val)
        fullcmd = b' '.join([cmd, key, networkString('%d %d %d' % (flags, expireTime, length))]) + cas
        self.sendLine(fullcmd)
        self.sendLine(val)
    ...
    def _get(self, keys, withIdentifier, multiple):
        ...
        if multiple:
            ...
            cmdObj = Command(cmd, key=keys[0], value=None, flags=0, cas=b'', multiple=False)
        self._current.append(cmdObj)
        return cmdObj._deferred
