...
class MountFS(FS):
    ...
    _meta = {'virtual': True, 'read_only': False, 'unicode_paths': True, 'case_insensitive': False, 'invalid_path_chars': '\x00'}
    ...
    def open(self, path, mode='r', buffering=-1, encoding=None, errors=None, newline='', **options):
        validate_open_mode(mode)
        self.check()
        fs, _path = self._delegate(path)
        return fs.open(_path, mode=mode, buffering=buffering, encoding=encoding, errors=errors, newline=newline, **options)
