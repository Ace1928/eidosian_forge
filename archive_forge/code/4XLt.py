...
class A_STAR(Algorithm):
    ...
    async def find_neighbor(self, node: "Node", direction: Vector2) -> Optional["Node"]:
        ...
        # Use a thread pool executor to handle the coordinate validation concurrently.
        future = self.executor.submit(validate_coordinates)
        result = await asyncio.wrap_future(future)
        # Return the result of the asynchronous validation.
        return result

    async def process_neighbor(
        self, snake: "Snake", current_node: "Node", neighbor: "Node", goal_state: "Node"
    ) -> Optional["Node"]:
        """
        Asynchronously process a neighbor node for the A* algorithm, ensuring it is valid and accessible.
        This method leverages concurrency to efficiently validate and compute the necessary metrics for pathfinding.
        """
        # Asynchronously check if the neighbor is a valid position and not an obstacle
        if not await self.valid_position(neighbor) or await self.is_obstacle(neighbor):
            logging.debug(f"Neighbor {neighbor} is either invalid or an obstacle.")
            return None

        # Asynchronously calculate the cost to reach the neighbor from the current node
        await self.calculate_cost(current_node, neighbor)

        # Asynchronously calculate the heuristic value from the neighbor to the goal state
        neighbor_position = neighbor.x, neighbor.y
        neighbor.h = await self.calculate_heuristic(goal_state, neighbor)
    ...
    async def calculate_cost(self, current_node: "Node", neighbor: "Node") -> None:
        """
        Asynchronously calculate the cost to reach the neighbor from the current node.
        """
        # Asynchronously calculate the cost to reach the neighbor from the current node
    ...
    async def is_obstacle(self, node: "Node") -> bool:
        """
        Asynchronously check if the node is an obstacle.
        """
        # Asynchronously check if the node is in the explored set
        if node in self.explored_set:
            logging.debug(f"Node {node} is an obstacle.")
            return True
        return False

    async def valid_position(self, node: "Node") -> bool:
        """
        Asynchronously check if the node is a valid position.
        """
        # Asynchronously check if the node is not in the explored set
    ...
    async def reconstruct_path(self, node: Node) -> List[Node]:
        """
        Asynchronously reconstruct the path from the start node to the goal node.
        """
        path = []
        while node:
            path.append(node)
