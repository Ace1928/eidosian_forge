...
class ProjectArtifactManager(RESTManager):
    ...
    @cli.register_custom_action('ProjectArtifactManager', ('ref_name', 'job'), ('job_token',))
    @exc.on_http_error(exc.GitlabGetError)
    def download(self, ref_name: str, job: str, streamed: bool=False, action: Optional[Callable[[bytes], None]]=None, chunk_size: int=1024, *, iterator: bool=False, **kwargs: Any) -> Optional[Union[bytes, Iterator[Any]]]:
        ...
        result = self.gitlab.http_get(path, job=job, streamed=streamed, raw=True, **kwargs)
        if TYPE_CHECKING:
