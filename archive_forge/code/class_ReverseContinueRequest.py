...
class ReverseContinueRequest(BaseSchema):
    ...
    """
    __props__ = {'seq': {'type': 'integer', 'description': "Sequence number (also known as message ID). For protocol messages of type 'request' this ID can be used to cancel the request."}, 'type': {'type': 'string', 'enum': ['request']}, 'command': {'type': 'string', 'enum': ['reverseContinue']}, 'arguments': {'type': 'ReverseContinueArguments'}}
