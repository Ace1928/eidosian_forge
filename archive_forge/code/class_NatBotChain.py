...
class NatBotChain(Chain):
    ...
    **Security Note**: This toolkit provides code to control a web-browser.
    ...
    def raise_deprecation(cls, values: Dict) -> Dict:
        if 'llm' in values:
            warnings.warn('Directly instantiating an NatBotChain with an llm is deprecated. Please instantiate with llm_chain argument or using the from_llm class method.')
    ...
    def _call(self, inputs: Dict[str, str], run_manager: Optional[CallbackManagerForChainRun]=None) -> Dict[str, str]:
        ...
        llm_cmd = self.llm_chain.predict(objective=self.objective, url=url[:100], previous_command=self.previous_command, browser_content=browser_content[:4500], callbacks=_run_manager.get_child())
    ...
    def execute(self, url: str, browser_content: str) -> str:
        ...
        """
        _inputs = {self.input_url_key: url, self.input_browser_content_key: browser_content}
