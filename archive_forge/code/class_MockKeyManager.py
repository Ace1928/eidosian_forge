...
class MockKeyManager(key_manager.KeyManager):
    ...
    def create_key_pair(self, context, algorithm, length, expiration=None, name=None):
        ...
        if length not in valid_lengths:
            msg = 'Invalid bit length: {}, only {} supported'.format(length, valid_lengths)
            raise ValueError(msg)
        private_key, public_key = self._generate_public_and_private_key(length, name)
    ...
    def remove_consumer(self, context, managed_object_id, consumer_data):
        ...
        if managed_object_id not in self.keys:
            raise exception.ManagedObjectNotFoundError()
        self.keys[managed_object_id].consumers = [c for c in self.keys[managed_object_id].consumers if c != consumer_data]
