...
class MotorheadMemory(BaseChatMemory):
    ...
    async def init(self) -> None:
        res = requests.get(f'{self.url}/sessions/{self.session_id}/memory', timeout=self.timeout, headers=self.__get_headers())
        res_data = res.json()
    ...
    def save_context(self, inputs: Dict[str, Any], outputs: Dict[str, str]) -> None:
        ...
        requests.post(f'{self.url}/sessions/{self.session_id}/memory', timeout=self.timeout, json={'messages': [{'role': 'Human', 'content': f'{input_str}'}, {'role': 'AI', 'content': f'{output_str}'}]}, headers=self.__get_headers())
