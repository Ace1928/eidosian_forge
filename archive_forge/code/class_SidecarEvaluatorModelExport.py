...
class SidecarEvaluatorModelExport(ModelCheckpoint):
    ...
    def __init__(self, export_filepath, checkpoint_filepath, **kwargs):
        super().__init__(filepath=export_filepath, save_best_only=True, **kwargs)
        self._checkpoint_filepath = checkpoint_filepath

    def on_test_begin(self, logs=None):
        """Updates export_index to the latest checkpoint."""
        most_recent_filepath = self._get_most_recently_modified_file_matching_pattern(self._checkpoint_filepath)
        if most_recent_filepath is not None:
            self.export_index = int(re.match('.*ckpt-(?P<ckpt>\\d+)', most_recent_filepath)['ckpt']) - 1
        else:
            self.export_index = 0
