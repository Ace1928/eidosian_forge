        ...
        with open(submitjobsfile, 'wt') as fp:
            ...
            for idx, pyscript in enumerate(pyfiles):
                ...
                if cache_doneness_per_node.get(idx, False):
                    ...
                    template, sbatch_args = self._get_args(node, ['template', 'sbatch_args'])
                    ...
                    full_line = "{jobNm}=$(sbatch {outFileOption} {errFileOption} {extraSBatchArgs} {dependantIndex} -J {jobNm} {batchscript} | awk '/^Submitted/ {{print $4}}')\n".format(jobNm=jobname, outFileOption=stdoutFile, errFileOption=stderrFile, extraSBatchArgs=sbatch_args, dependantIndex=deps, batchscript=batchscriptfile)
