...
class StrictTwoLevelModel(base.ModelBase):
    ...
    def _check_limit(self, resource_name, service_id, region_id, resource_limit, domain_id=None, parent_id=None):
        ...
        """
        if parent_id:
            parent_limit_value = self._get_specified_limit_value(resource_name, service_id, region_id, domain_id=parent_id)
            if parent_limit_value and resource_limit > parent_limit_value:
                raise exception.InvalidLimit(reason='Limit is bigger than parent.')
        else:
            sub_projects = PROVIDERS.resource_api.list_projects_in_subtree(domain_id)
            for sub_project in sub_projects:
                sub_limit_value = self._get_specified_limit_value(resource_name, service_id, region_id, project_id=sub_project['id'])
                if sub_limit_value and resource_limit < sub_limit_value:
                    raise exception.InvalidLimit(reason='Limit is smaller than child.')
