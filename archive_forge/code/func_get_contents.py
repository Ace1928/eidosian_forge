...
def get_contents(self, path: str, ref: Opt[str]=NotSet) -> list[ContentFile] | ContentFile:
    ...
    headers, data = self._requester.requestJsonAndCheck('GET', f'{self.url}/contents/{urllib.parse.quote(path)}', parameters=url_parameters)
    ...
    if isinstance(data, list):
        return [github.ContentFile.ContentFile(self._requester, headers, item, completed=item['type'] != 'file') for item in data]
