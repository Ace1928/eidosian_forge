...
class SamplerArgs:
    ...
    def validate(self, chains: Optional[int]) -> None:
        ...
        * length of per-chain lists equals specified # of chains
        ...
        if not (self.adapt_delta is None and self.adapt_init_phase is None and (self.adapt_metric_window is None) and (self.adapt_step_size is None)):
        ...
        if self.iter_warmup is not None:
            if self.iter_warmup < 0 or not isinstance(self.iter_warmup, (int, np.integer)):
                raise ValueError('Value for iter_warmup must be a non-negative integer, found {}.'.format(self.iter_warmup))
        ...
        if self.iter_sampling is not None:
            if self.iter_sampling < 0 or not isinstance(self.iter_sampling, (int, np.integer)):
                raise ValueError('Argument "iter_sampling" must be a non-negative integer, found {}.'.format(self.iter_sampling))
        ...
        if self.step_size is not None:
            if isinstance(self.step_size, (float, int, np.integer, np.floating)):
                if self.step_size <= 0:
                    raise ValueError('Argument "step_size" must be > 0, found {}.'.format(self.step_size))
            else:
                if len(self.step_size) != chains:
                    raise ValueError('Expecting {} per-chain step_size specifications,  found {}.'.format(chains, len(self.step_size)))
                for i, step_size in enumerate(self.step_size):
                    if step_size < 0:
                        raise ValueError('Argument "step_size" must be > 0, chain {}, found {}.'.format(i + 1, step_size))
        if self.metric is not None:
            if isinstance(self.metric, str):
                ...
                if len(self.metric) != chains:
                    raise ValueError('Number of metric files must match number of chains, found {} metric files for {} chains.'.format(len(self.metric), chains))
                if all((isinstance(elem, dict) for elem in self.metric)):
                    ...
                    for i, metric in enumerate(self.metric):
                        ...
                        if 'inv_metric' not in metric_dict:
                            raise ValueError('Entry "inv_metric" not found in metric dict for chain {}.'.format(i + 1))
                        if i == 0:
                            dims = list(np.asarray(metric_dict['inv_metric']).shape)
                        else:
                            dims2 = list(np.asarray(metric_dict['inv_metric']).shape)
                            if dims != dims2:
                                raise ValueError('Found inconsistent "inv_metric" entry for chain {}: entry has dims {}, expected {}.'.format(i + 1, dims, dims2))
                        dict_file = create_named_text_file(dir=_TMPDIR, prefix='metric', suffix='.json')
                    ...
                    for i, metric in enumerate(self.metric):
                        ...
                        if i == 0:
                            ...
                            if len(dims) != len(dims2):
                                raise ValueError('Metrics files {}, {}, inconsistent metrics'.format(self.metric[0], metric))
                            if dims != dims2:
                                raise ValueError('Metrics files {}, {}, inconsistent metrics'.format(self.metric[0], metric))
                    ...
                    raise ValueError('Argument "metric" must be a list of pathnames or Python dicts, found list of {}.'.format(type(self.metric[0])))
            else:
                raise ValueError('Invalid metric specified, not a recognized metric type, must be either a metric type name, a filepath, dict, or list of per-chain filepaths or dicts.  Found an object of type {}.'.format(type(self.metric)))
        if self.adapt_delta is not None:
            if not 0 < self.adapt_delta < 1:
                raise ValueError('Argument "adapt_delta" must be between 0 and 1, found {}'.format(self.adapt_delta))
        if self.adapt_init_phase is not None:
            if self.adapt_init_phase < 0 or not isinstance(self.adapt_init_phase, (int, np.integer)):
                raise ValueError('Argument "adapt_init_phase" must be a non-negative integer, found {}'.format(self.adapt_init_phase))
        if self.adapt_metric_window is not None:
            if self.adapt_metric_window < 0 or not isinstance(self.adapt_metric_window, (int, np.integer)):
                raise ValueError('Argument "adapt_metric_window" must be a non-negative  integer, found {}'.format(self.adapt_metric_window))
        if self.adapt_step_size is not None:
            if self.adapt_step_size < 0 or not isinstance(self.adapt_step_size, (int, np.integer)):
                raise ValueError('Argument "adapt_step_size" must be a non-negative integer,found {}'.format(self.adapt_step_size))
        positive_int(self.num_chains, 'num_chains')
        if self.fixed_param and (self.max_treedepth is not None or self.metric is not None or self.step_size is not None or (not (self.adapt_delta is None and self.adapt_init_phase is None and (self.adapt_metric_window is None) and (self.adapt_step_size is None)))):
