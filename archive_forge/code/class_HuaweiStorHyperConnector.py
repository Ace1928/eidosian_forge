...
class HuaweiStorHyperConnector(base.BaseLinuxConnector):
    ...
    def __init__(self, root_helper, driver=None, *args, **kwargs):
        ...
        if not self.cli_path:
            ...
            LOG.debug('CLI path is not configured, using default %s.', self.cli_path)
        if not os.path.isfile(self.cli_path):
            self.iscliexist = False
            LOG.error('SDS CLI file not found, HuaweiStorHyperConnector init failed.')
        super(HuaweiStorHyperConnector, self).__init__(root_helper, *args, driver=driver, **kwargs)
    ...
    def connect_volume(self, connection_properties):
        ...
        :returns: dict
        """
        LOG.debug('Connect_volume connection properties: %s.', connection_properties)
        out = self._attach_volume(connection_properties['volume_id'])
        if not out or int(out['ret_code']) not in (self.attached_success_code, self.has_been_attached_code, self.attach_mnid_done_code):
            msg = _('Attach volume failed, error code is %s') % out['ret_code']
            raise exception.BrickException(message=msg)
        try:
            volume_path = self._get_volume_path(connection_properties)
        except Exception:
    ...
    def disconnect_volume(self, connection_properties, device_info, force=False, ignore_errors=False):
        ...
        if not out or int(out['ret_code']) not in (self.attached_success_code, self.vbs_unnormal_code, self.not_mount_node_code):
            msg = _('Disconnect_volume failed, error code is %s') % out['ret_code']
            raise exception.BrickException(message=msg)
