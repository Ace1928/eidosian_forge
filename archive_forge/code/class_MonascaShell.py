...
class MonascaShell(shell.OpenStackShell):
    ...
    def __init__(self):
        super(MonascaShell, self).__init__(description=__doc__.strip(), version=mc_version.version_string)
        self.cloud_config = None

    def initialize_app(self, argv):
        super(MonascaShell, self).initialize_app(argv)
        self.cloud_config = cloud_config.OSC_Config(override_defaults={'interface': None, 'auth_type': self._auth_type})

    def build_option_parser(self, description, version):
        parser = super(MonascaShell, self).build_option_parser(description, version)
    ...
    def _append_monasca_args(parser):
        parser.add_argument('--monasca-api-url', default=utils.env('MONASCA_API_URL'), help='Defaults to env[MONASCA_API_URL].')
        parser.add_argument('--monasca_api_url', help=argparse.SUPPRESS)
        parser.add_argument('--monasca-api-version', default=utils.env('MONASCA_API_VERSION', default='2_0'), help='Defaults to env[MONASCA_API_VERSION] or 2_0')
        parser.add_argument('--monasca_api_version', help=argparse.SUPPRESS)
        return parser

    def _load_commands(self):
        version = self.options.monasca_api_version
        submodule = importutils.import_versioned_module('monascaclient', version, 'shell')
