...
def quantized_conv2d_with_bias_and_relu_and_requantize(input: _atypes.TensorFuzzingAnnotation[TV_QuantizedConv2DWithBiasAndReluAndRequantize_Tinput], filter: _atypes.TensorFuzzingAnnotation[TV_QuantizedConv2DWithBiasAndReluAndRequantize_Tfilter], bias: _atypes.TensorFuzzingAnnotation[TV_QuantizedConv2DWithBiasAndReluAndRequantize_Tbias], min_input: _atypes.TensorFuzzingAnnotation[_atypes.Float32], max_input: _atypes.TensorFuzzingAnnotation[_atypes.Float32], min_filter: _atypes.TensorFuzzingAnnotation[_atypes.Float32], max_filter: _atypes.TensorFuzzingAnnotation[_atypes.Float32], min_freezed_output: _atypes.TensorFuzzingAnnotation[_atypes.Float32], max_freezed_output: _atypes.TensorFuzzingAnnotation[_atypes.Float32], strides, padding: str, out_type: TV_QuantizedConv2DWithBiasAndReluAndRequantize_out_type=_dtypes.quint8, dilations=[1, 1, 1, 1], padding_list=[], name=None):
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'QuantizedConv2DWithBiasAndReluAndRequantize', name, input, filter, bias, min_input, max_input, min_filter, max_filter, min_freezed_output, max_freezed_output, 'out_type', out_type, 'strides', strides, 'padding', padding, 'dilations', dilations, 'padding_list', padding_list)
    ...
    if not isinstance(strides, (list, tuple)):
        raise TypeError("Expected list for 'strides' argument to 'quantized_conv2d_with_bias_and_relu_and_requantize' Op, not %r." % strides)
