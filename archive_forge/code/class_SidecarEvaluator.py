...
class SidecarEvaluator:
    ...
    def __init__(self, model, data, checkpoint_dir, steps=None, max_evaluations=None, callbacks=None):
        ...
        self._iterations = tf.Variable(name='iterations', initial_value=_ITERATIONS_UNINITIALIZED, dtype=tf.int64)
    ...
    def start(self):
        ...
        if self.model.optimizer and isinstance(self.model.optimizer, optimizer.Optimizer):
            ...
            checkpoint = tf.train.Checkpoint(model=self.model, optimizer=optimizer_checkpoint)
        for latest_checkpoint in tf.train.checkpoints_iterator(self.checkpoint_dir, timeout=_CHECKPOINT_TIMEOUT_SEC, timeout_fn=self._timeout_fn):
            try:
                ...
                if self.model.optimizer and (not isinstance(self.model.optimizer, optimizer.Optimizer)):
                    self.model.optimizer.iterations.assign(self._iterations)
            except (tf.errors.OpError,) as e:
                if isinstance(e, tf.errors.UnavailableError):
                    raise e
                logging.info(f'SidecarEvaluator encountered an error when loading the checkpoint at {latest_checkpoint}. Retrying. Error: {e.__class__.__name__}: {e}')
                continue
            if self._iterations.numpy() == _ITERATIONS_UNINITIALIZED and (not isinstance(self.model.optimizer, optimizer.Optimizer)):
                raise RuntimeError(f'Variable `iterations` cannot be loaded from the checkpoint file at {self.checkpoint_dir}. Please ensure `iterations` is included in the checkpoint saved during training.')
            logging.info(f'Evaluation starts: Model weights loaded from latest checkpoint file {latest_checkpoint}')
            self.model.evaluate(self.data, steps=self.steps, callbacks=self.callbacks, verbose=2)
            return_metrics = {}
            ...
            logging.info('End of evaluation. Metrics: %s', ' '.join([f'{name}={value.numpy()}' for name, value in return_metrics.items()]))
            if self.max_evaluations and self.max_evaluations <= int(latest_checkpoint.split('-')[-1]):
