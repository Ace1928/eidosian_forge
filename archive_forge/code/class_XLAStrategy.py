...
class XLAStrategy(ParallelStrategy):
    ...
    def __init__(self, accelerator: Optional[Accelerator]=None, parallel_devices: Optional[List[torch.device]]=None, checkpoint_io: Optional[XLACheckpointIO]=None, precision: Optional[XLAPrecision]=None, sync_module_states: bool=True) -> None:
    ...
    def all_reduce(self, output: Union[Tensor, Any], group: Optional[Any]=None, reduce_op: Optional[Union[ReduceOp, str]]=None) -> Tensor:
        ...
        if invalid_reduce_op or invalid_reduce_op_str:
            raise ValueError(f'Currently, the XLAStrategy only supports `sum`, `mean`, `avg` for the reduce operation, got: {reduce_op}')
        import torch_xla.core.xla_model as xm
