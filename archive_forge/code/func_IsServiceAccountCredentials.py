...
def IsServiceAccountCredentials(creds):
    if IsOauth2ClientCredentials(creds):
        ...
        return cred_type in (CredentialType.SERVICE_ACCOUNT, CredentialType.P12_SERVICE_ACCOUNT)
    else:
        cred_type = CredentialTypeGoogleAuth.FromCredentials(creds)
        return cred_type in (CredentialTypeGoogleAuth.SERVICE_ACCOUNT, CredentialTypeGoogleAuth.P12_SERVICE_ACCOUNT)