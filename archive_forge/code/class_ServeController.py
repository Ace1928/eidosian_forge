...
class ServeController:
    ...
    async def __init__(self, controller_name: str, *, http_config: HTTPOptions, global_logging_config: LoggingConfig, grpc_options: Optional[gRPCOptions]=None):
        self._controller_node_id = ray.get_runtime_context().get_node_id()
        assert self._controller_node_id == get_head_node_id(), 'Controller must be on the head node.'
        ...
        configure_component_memory_profiler(component_name='controller', component_id=str(os.getpid()))
        self.cpu_profiler, self.cpu_profiler_log = configure_component_cpu_profiler(component_name='controller', component_id=str(os.getpid()))
        if RAY_SERVE_CONTROLLER_CALLBACK_IMPORT_PATH:
            logger.info(f'Calling user-provided callback from import path {RAY_SERVE_CONTROLLER_CALLBACK_IMPORT_PATH}.')
        ...
        self.proxy_state_manager = ProxyStateManager(controller_name, http_config, self._controller_node_id, self.cluster_node_info_cache, self.global_logging_config, grpc_options)
        self.endpoint_state = EndpointState(self.kv_store, self.long_poll_host)
        all_current_actors = ray.util.list_named_actors(all_namespaces=True)
        all_serve_actor_names = [actor['name'] for actor in all_current_actors if actor['namespace'] == SERVE_NAMESPACE]
        self.deployment_state_manager = DeploymentStateManager(controller_name, self.kv_store, self.long_poll_host, all_serve_actor_names, get_all_live_placement_group_names(), self.cluster_node_info_cache)
        self.application_state_manager = ApplicationStateManager(self.deployment_state_manager, self.endpoint_state, self.kv_store)
        self._actor_details = ServeActorDetails(node_id=ray.get_runtime_context().get_node_id(), node_ip=ray.util.get_node_ip_address(), actor_id=ray.get_runtime_context().get_actor_id(), actor_name=self.controller_name, worker_id=ray.get_runtime_context().get_worker_id(), log_file_path=get_component_logger_file_path())
        ...
        metrics.Counter('serve_controller_num_starts', description='The number of times that controller has started.').inc()
    ...
    def record_autoscaling_metrics(self, data: Dict[str, float], send_timestamp: float):
        logger.debug(f'Received autoscaling metrics: {data} at timestamp {send_timestamp}')
    ...
    def _stop_one_running_replica_for_testing(self, deployment_name):
        self.deployment_state_manager._deployment_states[deployment_name]._stop_one_running_replica_for_testing()
    ...
    async def listen_for_change_java(self, keys_to_snapshot_ids_bytes: bytes):
        ...
        if not self.done_recovering_event.is_set():
            await self.done_recovering_event.wait()
        return await self.long_poll_host.listen_for_change_java(keys_to_snapshot_ids_bytes)
    ...
    def get_all_endpoints_java(self) -> bytes:
        ...
        data = {endpoint_tag.name: EndpointInfoProto(route=endppint_dict['route']) for endpoint_tag, endppint_dict in endpoints.items()}
    ...
    def get_proxy_names(self) -> bytes:
        ...
        if self.proxy_state_manager is None:
            return None
        actor_name_list = ActorNameList(names=self.proxy_state_manager.get_proxy_names().values())
    ...
    async def run_control_loop(self) -> None:
        ...
        while True:
            ...
            if not self.done_recovering_event.is_set() and time.time() - start_time > recovering_timeout:
                logger.warning(f'Replicas still recovering after {recovering_timeout}s, setting done recovering event to broadcast long poll updates.')
            ...
            try:
                ...
                if not self.done_recovering_event.is_set() and (not any_recovering):
                    self.done_recovering_event.set()
                    logger.info(f'Finished recovering deployments after {time.time() - start_time:.2f}s.', extra={'log_to_stderr': False})
            except Exception:
            ...
            if self.proxy_state_manager and self.done_recovering_event.is_set():
                try:
                    ...
                    self.proxy_update_duration_gauge_s.set(time.time() - proxy_update_start_time)
                except Exception:
                    logger.exception('Exception updating proxy state.')
            loop_duration = time.time() - loop_start_time
            if loop_duration > 10:
                logger.warning(f'The last control loop was slow (took {loop_duration}s). This is likely caused by running a large number of replicas in a single Ray cluster. Consider using multiple Ray clusters.', extra={'log_to_stderr': False})
    ...
    def _create_control_loop_metrics(self):
        self.node_update_duration_gauge_s = metrics.Gauge('serve_controller_node_update_duration_s', description='The control loop time spent on collecting proxy node info.')
        self.proxy_update_duration_gauge_s = metrics.Gauge('serve_controller_proxy_state_update_duration_s', description='The control loop time spent on updating proxy state.')
        self.dsm_update_duration_gauge_s = metrics.Gauge('serve_controller_deployment_state_update_duration_s', description='The control loop time spent on updating deployment state.')
        self.asm_update_duration_gauge_s = metrics.Gauge('serve_controller_application_state_update_duration_s', description='The control loop time spent on updating application state.')
        self.sleep_duration_gauge_s = metrics.Gauge('serve_controller_sleep_duration_s', description="The duration of the last control loop's sleep.")
        self.control_loop_duration_gauge_s = metrics.Gauge('serve_controller_control_loop_duration_s', description='The duration of the last control loop.')
        self.num_control_loops_gauge = metrics.Gauge('serve_controller_num_control_loops', description="The number of control loops performed by the controller. Increases monotonically over the controller's lifetime.", tag_keys=('actor_id',))
        self.num_control_loops_gauge.set_default_tags({'actor_id': ray.get_runtime_context().get_actor_id()})
    ...
    def get_root_url(self):
        ...
        if http_config.root_url == '':
            if SERVE_ROOT_URL_ENV_KEY in os.environ:
                ...
                return f'http://{http_config.host}:{http_config.port}{http_config.root_path}'
    ...
    def shutdown(self):
        ...
        proxy_state_is_shutdown = self.proxy_state_manager is None or self.proxy_state_manager.is_ready_for_shutdown()
        if config_checkpoint_deleted and application_is_shutdown and deployment_is_shutdown and endpoint_is_shutdown and proxy_state_is_shutdown:
            logger.warning('All resources have shut down, shutting down controller!', extra={'log_to_stderr': False})
            ...
            if not config_checkpoint_deleted:
                logger.warning(f'{CONFIG_CHECKPOINT_KEY} not yet deleted', extra={'log_to_stderr': False})
            if not application_is_shutdown:
                logger.warning('application not yet shutdown', extra={'log_to_stderr': False})
            if not deployment_is_shutdown:
                logger.warning('deployment not yet shutdown', extra={'log_to_stderr': False})
            if not endpoint_is_shutdown:
                logger.warning('endpoint not yet shutdown', extra={'log_to_stderr': False})
            if not proxy_state_is_shutdown:
                logger.warning('proxy_state not yet shutdown', extra={'log_to_stderr': False})

    def deploy(self, name: str, deployment_config_proto_bytes: bytes, replica_config_proto_bytes: bytes, route_prefix: Optional[str], deployer_job_id: Union[str, bytes], docs_path: Optional[str]=None, is_deployed_from_python: bool=False) -> bool:
        """Deploys a deployment. This should only be used for 1.x deployments."""
        if route_prefix is not None:
            assert route_prefix.startswith('/')
        if docs_path is not None:
            assert docs_path.startswith('/')
        deployment_info = deploy_args_to_deployment_info(deployment_name=name, deployment_config_proto_bytes=deployment_config_proto_bytes, replica_config_proto_bytes=replica_config_proto_bytes, deployer_job_id=deployer_job_id, route_prefix=route_prefix, docs_path=docs_path, app_name='')
        updating = self.deployment_state_manager.deploy(DeploymentID(name, ''), deployment_info)
        if route_prefix is not None:
            endpoint_info = EndpointInfo(route=route_prefix, app_is_cross_language=not is_deployed_from_python)
    ...
    def deploy_application(self, name: str, deployment_args_list: List[bytes]) -> None:
        ...
        for deployment_args_bytes in deployment_args_list:
            ...
            deployment_args_deserialized.append({'deployment_name': deployment_args.deployment_name, 'deployment_config_proto_bytes': deployment_args.deployment_config, 'replica_config_proto_bytes': deployment_args.replica_config, 'deployer_job_id': deployment_args.deployer_job_id, 'route_prefix': deployment_args.route_prefix if deployment_args.HasField('route_prefix') else None, 'ingress': deployment_args.ingress, 'docs_path': deployment_args.docs_path if deployment_args.HasField('docs_path') else None})
        self.application_state_manager.apply_deployment_args(name, deployment_args_deserialized)

    def deploy_config(self, config: ServeDeploySchema, deployment_time: float=0.0) -> None:
        ...
        self._target_capacity_direction = calculate_target_capacity_direction(curr_config=curr_config, new_config=config, curr_target_capacity_direction=self._target_capacity_direction)
        log_target_capacity_change(self._target_capacity, config.target_capacity, self._target_capacity_direction)
        ...
        for app_config in config.applications:
            for deployments in app_config.deployments:
                if deployments.route_prefix != DEFAULT.VALUE:
                    logger.warning('Specifying route prefix for a deployment is deprecated. Please specify route prefix at an application level in the Serve config instead.')
            ...
            self.application_state_manager.deploy_config(app_config.name, app_config, deployment_time=deployment_time, target_capacity=self._target_capacity, target_capacity_direction=self._target_capacity_direction)
        self.kv_store.put(CONFIG_CHECKPOINT_KEY, pickle.dumps((deployment_time, self._target_capacity, self._target_capacity_direction, new_config_checkpoint)))
        existing_applications = set(self.application_state_manager._application_states.keys())
    ...
    def list_deployments_v1(self) -> bytes:
        ...
        for deployment_id, (deployment_info, route_prefix) in self.list_deployments_internal().items():
            if deployment_id.app:
                continue
            deployment_info_proto = deployment_info.to_proto()
            deployment_info_proto.name = deployment_id.name
            deployment_route_list.deployment_routes.append(DeploymentRoute(deployment_info=deployment_info_proto, route=route_prefix))
    ...
    def list_deployments(self) -> Dict[DeploymentID, DeploymentInfo]:
        ...
        return {deployment_id: deployment_info for deployment_id, (deployment_info, _) in self.list_deployments_internal().items()}
    ...
    def get_serve_status(self, name: str=SERVE_DEFAULT_APP_NAME) -> bytes:
        ...
        deployment_statuses = self.application_state_manager.get_deployments_statuses(name)
        status_info = StatusOverview(name=name, app_status=app_status, deployment_statuses=deployment_statuses)
    ...
    def get_deployment_status(self, name: str, app_name: str='') -> Union[None, bytes]:
