...
class SPARQLConnector:
    ...
    def __init__(self, query_endpoint: Optional[str]=None, update_endpoint: Optional[str]=None, returnFormat: str='xml', method: "te.Literal['GET', 'POST', 'POST_FORM']"='GET', auth: Optional[Tuple[str, str]]=None, **kwargs):
        """
    ...
    def query(self, query: str, default_graph: Optional[str]=None, named_graph: Optional[str]=None) -> 'Result':
        ...
        return Result.parse(BytesIO(res.read()), content_type=res.headers['Content-Type'].split(';')[0])

    def update(self, query: str, default_graph: Optional[str]=None, named_graph: Optional[str]=None) -> None:
        ...
        headers = {'Accept': _response_mime_types[self.returnFormat], 'Content-Type': 'application/sparql-update; charset=UTF-8'}
        args = copy.deepcopy(self.kwargs)
        args.setdefault('params', {})
        args['params'].update(params)
        args.setdefault('headers', {})
