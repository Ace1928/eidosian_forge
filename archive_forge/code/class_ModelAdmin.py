...
class ModelAdmin(BaseModelAdmin):
    ...
    def get_urls(self):
        ...
        return [path('', wrap(self.changelist_view), name='%s_%s_changelist' % info), path('add/', wrap(self.add_view), name='%s_%s_add' % info), path('<path:object_id>/history/', wrap(self.history_view), name='%s_%s_history' % info), path('<path:object_id>/delete/', wrap(self.delete_view), name='%s_%s_delete' % info), path('<path:object_id>/change/', wrap(self.change_view), name='%s_%s_change' % info), path('<path:object_id>/', wrap(RedirectView.as_view(pattern_name='%s:%s_%s_change' % ((self.admin_site.name,) + info))))]
    ...
    def media(self):
        ...
        js = ['vendor/jquery/jquery%s.js' % extra, 'jquery.init.js', 'core.js', 'admin/RelatedObjectLookups.js', 'actions.js', 'urlify.js', 'prepopulate.js', 'vendor/xregexp/xregexp%s.js' % extra]
    ...
    def get_form(self, request, obj=None, change=False, **kwargs):
        ...
        defaults = {'form': form, 'fields': fields, 'exclude': exclude, 'formfield_callback': partial(self.formfield_for_dbfield, request=request), **kwargs}
    ...
    def get_changelist_instance(self, request):
        ...
        return ChangeList(request, self.model, list_display, list_display_links, self.get_list_filter(request), self.date_hierarchy, self.get_search_fields(request), self.get_list_select_related(request), self.list_per_page, self.list_max_show_all, self.list_editable, self, sortable_by, self.search_help_text)
    ...
    def log_addition(self, request, obj, message):
        ...
        return LogEntry.objects.log_action(user_id=request.user.pk, content_type_id=get_content_type_for_model(obj).pk, object_id=obj.pk, object_repr=str(obj), action_flag=ADDITION, change_message=message)
    ...
    def log_change(self, request, obj, message):
        ...
        return LogEntry.objects.log_action(user_id=request.user.pk, content_type_id=get_content_type_for_model(obj).pk, object_id=obj.pk, object_repr=str(obj), action_flag=CHANGE, change_message=message)
    ...
    def log_deletion(self, request, obj, object_repr):
        ...
        return LogEntry.objects.log_action(user_id=request.user.pk, content_type_id=get_content_type_for_model(obj).pk, object_id=obj.pk, object_repr=object_repr, action_flag=DELETION)

    def action_checkbox(self, obj):
        """
        A list_display column containing a checkbox widget.
        """
        attrs = {'class': 'action-select', 'aria-label': format_html(_('Select this object for an action - {}'), obj)}
    ...
    def response_add(self, request, obj, post_url_continue=None):
        ...
        if IS_POPUP_VAR in request.POST:
            ...
            post_url_continue = add_preserved_filters({'preserved_filters': preserved_filters, 'preserved_qsl': preserved_qsl, 'opts': opts}, post_url_continue)
    ...
    def _changeform_view(self, request, object_id, form_url, extra_context):
        ...
        if not add and (not self.has_change_permission(request, obj)):
            ...
            readonly_fields = self.get_readonly_fields(request, obj)
        admin_form = helpers.AdminForm(form, list(fieldsets), self.get_prepopulated_fields(request, obj) if add or self.has_change_permission(request, obj) else {}, readonly_fields, model_admin=self)
        ...
        if add:
            ...
            title = _('View %s')
        context = {**self.admin_site.each_context(request), 'title': title % self.opts.verbose_name, 'subtitle': str(obj) if obj else None, 'adminform': admin_form, 'object_id': object_id, 'original': obj, 'is_popup': IS_POPUP_VAR in request.POST or IS_POPUP_VAR in request.GET, 'to_field': to_field, 'media': media, 'inline_admin_formsets': inline_formsets, 'errors': helpers.AdminErrorList(form, formsets), 'preserved_filters': self.get_preserved_filters(request)}
    ...
    def get_formset_kwargs(self, request, obj, inline, prefix):
        ...
        if request.method == 'POST':
            formset_params.update({'data': request.POST.copy(), 'files': request.FILES, 'save_as_new': '_saveasnew' in request.POST})
