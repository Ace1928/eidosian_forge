...
class DashVector(VectorStore):
    ...
    def __init__(self, collection: Any, embedding: Embeddings, text_field: str):
        ...
        try:
            ...
            raise ValueError('Could not import dashvector python package. Please install it with `pip install dashvector`.')
        if not isinstance(collection, dashvector.Collection):
            raise ValueError(f'collection should be an instance of dashvector.Collection, bug got {type(collection)}')
    ...
    def add_texts(self, texts: Iterable[str], metadatas: Optional[List[dict]]=None, ids: Optional[List[str]]=None, batch_size: int=25, partition: str='default', **kwargs: Any) -> List[str]:
        ...
        for i in range(0, len(text_list), batch_size):
            ...
            if not ret:
                raise ValueError(f'Fail to upsert docs to dashvector vector database,Error: {ret.message}')
        return ids
    ...
    def similarity_search(self, query: str, k: int=4, filter: Optional[str]=None, partition: str='default', **kwargs: Any) -> List[Document]:
    ...
    def similarity_search_with_relevance_scores(self, query: str, k: int=4, filter: Optional[str]=None, partition: str='default', **kwargs: Any) -> List[Tuple[Document, float]]:
    ...
    def similarity_search_by_vector(self, embedding: List[float], k: int=4, filter: Optional[str]=None, partition: str='default', **kwargs: Any) -> List[Document]:
    ...
    def max_marginal_relevance_search(self, query: str, k: int=4, fetch_k: int=20, lambda_mult: float=0.5, filter: Optional[dict]=None, partition: str='default', **kwargs: Any) -> List[Document]:
    ...
    def max_marginal_relevance_search_by_vector(self, embedding: List[float], k: int=4, fetch_k: int=20, lambda_mult: float=0.5, filter: Optional[dict]=None, partition: str='default', **kwargs: Any) -> List[Document]:
        """Return docs selected using the maximal marginal relevance.
        ...
        Returns:
            List of Documents selected by maximal marginal relevance.
        """
        ret = self._collection.query(embedding, topk=fetch_k, filter=filter, partition=partition, include_vector=True)
        if not ret:
            raise ValueError(f'Fail to query docs by vector, error {self._collection.message}')
    ...
    @classmethod
    def from_texts(cls, texts: List[str], embedding: Embeddings, metadatas: Optional[List[dict]]=None, dashvector_api_key: Optional[str]=None, dashvector_endpoint: Optional[str]=None, collection_name: str='langchain', text_field: str='text', batch_size: int=25, ids: Optional[List[str]]=None, **kwargs: Any) -> DashVector:
        ...
        dashvector_endpoint = dashvector_endpoint or get_from_env('dashvector_endpoint', 'DASHVECTOR_ENDPOINT', default='dashvector.cn-hangzhou.aliyuncs.com')
