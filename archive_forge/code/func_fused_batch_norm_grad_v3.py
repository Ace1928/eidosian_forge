...
def fused_batch_norm_grad_v3(y_backprop: _atypes.TensorFuzzingAnnotation[TV_FusedBatchNormGradV3_T], x: _atypes.TensorFuzzingAnnotation[TV_FusedBatchNormGradV3_T], scale: _atypes.TensorFuzzingAnnotation[_atypes.Float32], reserve_space_1: _atypes.TensorFuzzingAnnotation[TV_FusedBatchNormGradV3_U], reserve_space_2: _atypes.TensorFuzzingAnnotation[TV_FusedBatchNormGradV3_U], reserve_space_3: _atypes.TensorFuzzingAnnotation[TV_FusedBatchNormGradV3_U], epsilon: float=0.0001, data_format: str='NHWC', is_training: bool=True, name=None):
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'FusedBatchNormGradV3', name, y_backprop, x, scale, reserve_space_1, reserve_space_2, reserve_space_3, 'epsilon', epsilon, 'data_format', data_format, 'is_training', is_training)
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('FusedBatchNormGradV3', y_backprop=y_backprop, x=x, scale=scale, reserve_space_1=reserve_space_1, reserve_space_2=reserve_space_2, reserve_space_3=reserve_space_3, epsilon=epsilon, data_format=data_format, is_training=is_training, name=name)
    _result = _outputs[:]
    if _execute.must_record_gradient():
        _attrs = ('T', _op._get_attr_type('T'), 'U', _op._get_attr_type('U'), 'epsilon', _op.get_attr('epsilon'), 'data_format', _op.get_attr('data_format'), 'is_training', _op._get_attr_bool('is_training'))
