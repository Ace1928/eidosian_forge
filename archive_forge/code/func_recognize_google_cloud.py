...
def recognize_google_cloud(self, audio_data, credentials_json=None, language='en-US', preferred_phrases=None, show_all=False):
    ...
    flac_data = audio_data.get_flac_data(convert_rate=None if 8000 <= audio_data.sample_rate <= 48000 else max(8000, min(audio_data.sample_rate, 48000)), convert_width=2)
    audio = speech.RecognitionAudio(content=flac_data)
    config = {'encoding': speech.RecognitionConfig.AudioEncoding.FLAC, 'sample_rate_hertz': audio_data.sample_rate, 'language_code': language}
