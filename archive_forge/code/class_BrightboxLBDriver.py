...
class BrightboxLBDriver(Driver):
    ...
    LB_STATE_MAP = {'creating': State.PENDING, 'active': State.RUNNING, 'deleting': State.UNKNOWN, 'deleted': State.UNKNOWN, 'failing': State.UNKNOWN, 'failed': State.UNKNOWN}
    _VALUE_TO_ALGORITHM_MAP = {'round-robin': Algorithm.ROUND_ROBIN, 'least-connections': Algorithm.LEAST_CONNECTIONS}
    ...
    def create_balancer(self, name, port, protocol, algorithm, members):
        response = self._post('/%s/load_balancers' % API_VERSION, {'name': name, 'nodes': list(map(self._member_to_node, members)), 'policy': self._algorithm_to_value(algorithm), 'listeners': [{'in': port, 'out': port, 'protocol': protocol}], 'healthcheck': {'type': protocol, 'port': port}})
        return self._to_balancer(response.object)

    def destroy_balancer(self, balancer):
        response = self.connection.request('/{}/load_balancers/{}'.format(API_VERSION, balancer.id), method='DELETE')
        return response.status == httplib.ACCEPTED

    def get_balancer(self, balancer_id):
        data = self.connection.request('/{}/load_balancers/{}'.format(API_VERSION, balancer_id)).object
    ...
    def _to_balancer(self, data):
        return LoadBalancer(id=data['id'], name=data['name'], state=self.LB_STATE_MAP.get(data['status'], State.UNKNOWN), ip=self._public_ip(data), port=data['listeners'][0]['in'], driver=self.connection.driver)
