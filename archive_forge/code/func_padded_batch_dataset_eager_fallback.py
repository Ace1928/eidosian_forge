...
def padded_batch_dataset_eager_fallback(input_dataset: _atypes.TensorFuzzingAnnotation[_atypes.Variant], batch_size: _atypes.TensorFuzzingAnnotation[_atypes.Int64], padded_shapes: List[_atypes.TensorFuzzingAnnotation[_atypes.Int64]], padding_values, output_shapes, metadata: str, name, ctx) -> _atypes.TensorFuzzingAnnotation[_atypes.Variant]:
    ...
    _attrs = ('Toutput_types', _attr_Toutput_types, 'output_shapes', output_shapes, 'N', _attr_N, 'metadata', metadata)
    _result = _execute.execute(b'PaddedBatchDataset', 1, inputs=_inputs_flat, attrs=_attrs, ctx=ctx, name=name)
