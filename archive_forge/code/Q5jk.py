...
class EmbeddingDatabaseManager:
    ...
    def insert_embedding(
        ...
        if self.is_file_processed(file_hash):
            logging.info(f"File already processed, skipping insertion: {file_hash}")
            return

        try:
            self.connection_manager.cursor.execute(
                "INSERT INTO embeddings (file_hash, imports, function, docstring, embedding) VALUES (?, ?, ?, ?, ?)",
                (file_hash, imports, function, docstring, embedding),
            )
        except sqlite3.IntegrityError as e:
            logging.error(f"Error inserting embedding due to integrity constraint: {e}")
        except sqlite3.Error as e:
...
class FileManager:
    ...
    def process_file(
        ...
        This method ensures that all parts of the file are processed and saved before marking the file as processed.
        """
        file_hash = self.hash_file(file_path)
        if embedding_database_manager.is_file_processed(file_hash):
        ...
        try:
            ...
            # Start a transaction
            embedding_database_manager.connection_manager.connection.begin()
            ...
            for name, class_content in classes:
                docstring = self.find_docstring(class_content)
                embedding = self.generate_embedding(class_content, embedding_model)
                embedding_database_manager.insert_embedding(
                    file_hash, imports, class_content, docstring, embedding
                )

            for doc_content in docs:
                embedding = self.generate_embedding(doc_content, embedding_model)
                embedding_database_manager.insert_embedding(
                    file_hash, imports, doc_content, doc_content, embedding
                )

            # Commit the transaction after all embeddings are inserted
            embedding_database_manager.connection_manager.connection.commit()

            # Update the processed files record only after successful commit
            self.processed_files[file_hash] = {"path": file_path}
            self.save_processed_files()

        except SyntaxError as e:
            logging.error(f"Syntax error in file {file_path}: {e}")
            embedding_database_manager.connection_manager.connection.rollback()
        except Exception as e:
            logging.error(f"Error processing file {file_path}: {e}")
            embedding_database_manager.connection_manager.connection.rollback()
