...
class Test_calc_var(unittest.TestCase):
    ...
    def test_nonlinear(self):
        ...
        for mode in all_diff_modes:
            m.x.set_value(1.0)
            calculate_variable_from_constraint(m.x, m.c, linesearch=False, diff_mode=mode)
            self.assertEqual(value(m.x), 4)
        m.d = Constraint(expr=m.x ** 2 == -1)
        for mode in all_diff_modes:
            m.x.set_value(1.25)
            with self.assertRaisesRegex(IterationLimitError, 'Iteration limit \\(10\\) reached'):
                calculate_variable_from_constraint(m.x, m.d, iterlim=10, linesearch=False, diff_mode=mode)
        for mode in all_diff_modes:
            m.x.set_value(1.25)
            with self.assertRaisesRegex(IterationLimitError, 'Linesearch iteration limit reached'):
                calculate_variable_from_constraint(m.x, m.d, iterlim=10, linesearch=True, diff_mode=mode)
        for mode in all_diff_modes:
            m.x = 0
            with self.assertRaisesRegex(ValueError, "Initial value for variable 'x' results in a derivative value for constraint 'c' that is very close to zero."):
                calculate_variable_from_constraint(m.x, m.c, diff_mode=mode)
        for mode in all_diff_modes:
            if mode == differentiate.Modes.reverse_numeric:
                with self.assertRaisesRegex(ValueError, "Initial value for variable 'y' results in a derivative value for constraint 'c' that is very close to zero."):
                    calculate_variable_from_constraint(m.y, m.c, diff_mode=mode)
            else:
                with self.assertRaisesRegex(ValueError, "Variable 'y' derivative == 0 in constraint 'c'"):
                    calculate_variable_from_constraint(m.y, m.c, diff_mode=mode)
        m.e = Constraint(expr=(m.x - 2.0) ** 2 - 1 == 0)
        for mode in all_diff_modes:
            m.x.set_value(3.1)
            calculate_variable_from_constraint(m.x, m.e, linesearch=False, diff_mode=mode)
            self.assertAlmostEqual(value(m.x), 3)
        for mode in all_diff_modes:
            m.x.set_value(3.1)
            calculate_variable_from_constraint(m.x, m.e, linesearch=True, diff_mode=mode)
            self.assertAlmostEqual(value(m.x), 3)
        m.f = Constraint(expr=1.0 / (1.0 + exp(-m.x)) - 0.5 == 0)
        for mode in all_diff_modes:
            m.x.set_value(3.0)
            calculate_variable_from_constraint(m.x, m.f, linesearch=True, diff_mode=mode)
            self.assertAlmostEqual(value(m.x), 0)
        for mode in all_diff_modes:
            m.x.set_value(3.0)
            with self.assertRaisesRegex(RuntimeError, "Newton's method encountered a derivative of constraint 'f' with respect to variable 'x' that was too close to zero"):
                calculate_variable_from_constraint(m.x, m.f, linesearch=False, diff_mode=mode)
        m = ConcreteModel()
        m.x = Var()
        m.pc = 4890000.0
        m.tc = 562.2
        ...
        @m.Constraint()
        def f(m):
            return m.pc * exp((m.psc['A'] * (1 - m.x / m.tc) + m.psc['B'] * (1 - m.x / m.tc) ** 1.5 + m.psc['C'] * (1 - m.x / m.tc) ** 3 + m.psc['D'] * (1 - m.x / m.tc) ** 6) / (1 - (1 - m.x / m.tc))) - m.p == 0
        ...
        with LoggingIntercept(output, 'pyomo', logging.WARNING):
            with self.assertRaises(TypeError):
                calculate_variable_from_constraint(m.x, m.f, linesearch=False)
        self.assertIn('Encountered an error evaluating the expression at the initial guess', output.getvalue())
        m = ConcreteModel()
        m.x = Var()
        ...
        self.assertIn("Newton's method encountered an error evaluating the expression for constraint 'c'.", output.getvalue())
        m = ConcreteModel()
        m.x = Var()
        m.c = Constraint(expr=m.x ** 0.5 == -1e-08)
        m.x = 1e-08
        with self.assertRaisesRegex(IterationLimitError, "Linesearch iteration limit reached solving for variable 'x' using constraint 'c'; remaining residual = {function evaluation error}"):
