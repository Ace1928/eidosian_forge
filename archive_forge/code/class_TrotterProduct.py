    ...
    **Example**
    ...
    def __init__(self, hamiltonian, time, n=1, order=1, check_hermitian=True, id=None):
        ...
        if isinstance(hamiltonian, qml.Hamiltonian):
            ...
            if len(coeffs) < 2:
                raise ValueError('There should be at least 2 terms in the Hamiltonian. Otherwise use `qml.exp`')
        ...
        if not isinstance(hamiltonian, Sum):
            raise TypeError(f'The given operator must be a PennyLane ~.Hamiltonian, ~.Sum or ~.SProd, got {hamiltonian}')
        if check_hermitian:
            for op in hamiltonian.operands:
                if not op.is_hermitian:
                    raise ValueError('One or more of the terms in the Hamiltonian may not be Hermitian')
        self._hyperparameters = {'n': n, 'order': order, 'base': hamiltonian, 'check_hermitian': check_hermitian}
    ...
    def _flatten(self):
        ...
        **Example:**
        ...
        hashable_hyperparameters = tuple(((key, value) for key, value in self.hyperparameters.items() if key != 'base'))
