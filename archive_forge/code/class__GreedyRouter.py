...
class _GreedyRouter:
    ...
    def __init__(self, circuit, device_graph: nx.Graph, *, max_search_radius: int=1, max_num_empty_steps: int=5, initial_mapping: Optional[Dict[ops.Qid, ops.Qid]]=None, can_reorder: Callable[[ops.Operation, ops.Operation], bool]=lambda op1, op2: not set(op1.qubits) & set(op2.qubits), random_state: 'cirq.RANDOM_STATE_OR_SEED_LIKE'=None):
        self.prng = value.parse_random_state(random_state)
        self.device_graph = device_graph
        self.physical_distances: Dict[QidPair, int] = {(a, b): d for a, neighbor_distances in nx.shortest_path_length(device_graph) for b, d in neighbor_distances.items()}
