...
class PadDynamicalDecoupling(BasePadding):
    ...
    def __init__(self, durations: InstructionDurations=None, dd_sequence: list[Gate]=None, qubits: list[int] | None=None, spacing: list[float] | None=None, skip_reset_qubits: bool=True, pulse_alignment: int=1, extra_slack_distribution: str='middle', target: Target=None):
    ...
    def _pad(self, dag: DAGCircuit, qubit: Qubit, t_start: int, t_end: int, next_node: DAGNode, prev_node: DAGNode):
        ...
        if time_interval % self._alignment != 0:
            raise TranspilerError(f'Time interval {time_interval} is not divisible by alignment {self._alignment} between DAGNode {prev_node.name} on qargs {prev_node.qargs} and {next_node.name} on qargs {next_node.qargs}.')
        if not self.__is_dd_qubit(dag.qubits.index(qubit)):
            self._apply_scheduled_op(dag, t_start, Delay(time_interval, dag.unit), qubit)
            return
        if self._skip_reset_qubits and (isinstance(prev_node, DAGInNode) or isinstance(prev_node.op, Reset)):
            self._apply_scheduled_op(dag, t_start, Delay(time_interval, dag.unit), qubit)
            return
        ...
        if len(self._dd_sequence) == 1:
            ...
            if isinstance(next_node, DAGOpNode) and isinstance(next_node.op, (UGate, U3Gate)):
                ...
                next_node.op.params = Optimize1qGates.compose_u3(theta_r, phi_r, lam_r, theta, phi, lam)
                sequence_gphase += phase
            elif isinstance(prev_node, DAGOpNode) and isinstance(prev_node.op, (UGate, U3Gate)):
                theta_l, phi_l, lam_l = prev_node.op.params
                prev_node.op.params = Optimize1qGates.compose_u3(theta, phi, lam, theta_l, phi_l, lam_l)
                sequence_gphase += phase
            else:
                self._apply_scheduled_op(dag, t_start, Delay(time_interval, dag.unit), qubit)
                return
        ...
        if self._extra_slack_distribution == 'middle':
            ...
            raise TranspilerError(f'Option extra_slack_distribution = {self._extra_slack_distribution} is invalid.')
