...
class GSClient(Client):
    ...
    def __init__(self, application_credentials: Optional[Union[str, os.PathLike]]=None, credentials: Optional['Credentials']=None, project: Optional[str]=None, storage_client: Optional['StorageClient']=None, file_cache_mode: Optional[Union[str, FileCacheMode]]=None, local_cache_dir: Optional[Union[str, os.PathLike]]=None, content_type_method: Optional[Callable]=mimetypes.guess_type):
    ...
    def _get_metadata(self, cloud_path: GSPath) -> Optional[Dict[str, Any]]:
        ...
        if blob is None:
            ...
            return {'etag': blob.etag, 'size': blob.size, 'updated': blob.updated, 'content_type': blob.content_type}
    ...
    def _list_dir(self, cloud_path: GSPath, recursive=False) -> Iterable[Tuple[GSPath, bool]]:
        if not cloud_path.bucket:
            ...
            yield from ((self.CloudPath(f'gs://{str(b)}'), True) for b in self.client.list_buckets())
            return
        bucket = self.client.bucket(cloud_path.bucket)
        prefix = cloud_path.blob
        if prefix and (not prefix.endswith('/')):
            ...
            for o in bucket.list_blobs(prefix=prefix):
                for parent in PurePosixPath(o.name[len(prefix):]).parents:
                    if parent not in yielded_dirs and str(parent) != '.':
                        yield (self.CloudPath(f'gs://{cloud_path.bucket}/{prefix}{parent}'), True)
                        yielded_dirs.add(parent)
                yield (self.CloudPath(f'gs://{cloud_path.bucket}/{o.name}'), False)
        else:
            iterator = bucket.list_blobs(delimiter='/', prefix=prefix)
            for file in iterator:
                yield (self.CloudPath(f'gs://{cloud_path.bucket}/{file.name}'), False)
            for directory in iterator.prefixes:
                yield (self.CloudPath(f'gs://{cloud_path.bucket}/{directory}'), True)
