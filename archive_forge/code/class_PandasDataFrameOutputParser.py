...
class PandasDataFrameOutputParser(BaseOutputParser):
    ...
    def validate_dataframe(cls, val: Any) -> Any:
        ...
        raise TypeError("Wrong type for 'dataframe', must be a subclass                 of Pandas DataFrame (pd.DataFrame)")
    ...
    def parse_array(self, array: str, original_request_params: str) -> Tuple[List[Union[int, str]], str]:
        ...
        if re.match('\\[\\d+(,\\s*\\d+)*\\]', array):
            ...
            if match:
                ...
                raise OutputParserException(f'Unable to parse the array provided in {array}.                         Please check the format instructions.')
            ...
            if match:
                ...
                raise OutputParserException(f'Unable to parse the array provided in {array}.                         Please check the format instructions.')
        if not parsed_array:
            raise OutputParserException(f"Invalid array format in '{original_request_params}'.                     Please check the format instructions.")
        elif isinstance(parsed_array[0], int) and parsed_array[-1] > self.dataframe.index.max():
            raise OutputParserException(f'The maximum index {parsed_array[-1]} exceeds the maximum index of                     the Pandas DataFrame {self.dataframe.index.max()}.')
    ...
    def parse(self, request: str) -> Dict[str, Any]:
        ...
        if len(splitted_request) != 2:
            raise OutputParserException(f"Request '{request}' is not correctly formatted.                     Please refer to the format instructions.")
        result = {}
        try:
            ...
            if request_type not in {'column', 'row'}:
                raise OutputParserException(f"Unsupported request type '{request_type}'.                         Please check the format instructions.")
            raise OutputParserException(f'Requested index {(request_params if stripped_request_params is None else stripped_request_params)} is out of bounds.')
        return result
