...
class FastSpeech2ConformerTokenizer(PreTrainedTokenizer):
    ...
    def decode(self, token_ids, **kwargs):
        logger.warn('Phonemes cannot be reliably converted to a string due to the one-many mapping, converting to tokens instead.')
        return self.convert_ids_to_tokens(token_ids)

    def convert_tokens_to_string(self, tokens, **kwargs):
        logger.warn('Phonemes cannot be reliably converted to a string due to the one-many mapping, returning the tokens.')
        return tokens
