...
class TensorFlowDataset(Dataset, PyFuncConvertibleDatasetMixin):
    ...
    def __init__(self, features, source: DatasetSource, targets=None, name: Optional[str]=None, digest: Optional[str]=None):
        """
        Args:
        ...
        if not isinstance(features, tf.data.Dataset) and (not tf.is_tensor(features)):
            raise MlflowException(f"'features' must be an instance of tf.data.Dataset or a TensorFlow Tensor. Found: {type(features)}.", INVALID_PARAMETER_VALUE)
        if tf.is_tensor(features) and targets is not None and (not tf.is_tensor(targets)):
            raise MlflowException(f"If 'features' is a TensorFlow Tensor, then 'targets' must also be a TensorFlow Tensor. Found: {type(targets)}.", INVALID_PARAMETER_VALUE)
        if isinstance(features, tf.data.Dataset) and targets is not None and (not isinstance(targets, tf.data.Dataset)):
            raise MlflowException(f"If 'features' is an instance of tf.data.Dataset, then 'targets' must also be an instance of tf.data.Dataset. Found: {type(targets)}.", INVALID_PARAMETER_VALUE)
    ...
    def profile(self) -> Optional[Any]:
        ...
        """
        import tensorflow as tf
        profile = {'features_cardinality': int(self._features.cardinality().numpy()) if isinstance(self._features, tf.data.Dataset) else int(tf.size(self._features).numpy())}
        if self._targets is not None:
            profile.update({'targets_cardinality': int(self._targets.cardinality().numpy()) if isinstance(self._targets, tf.data.Dataset) else int(tf.size(self._targets).numpy())})
        return profile
    ...
    def _get_schema_from_tf_dataset_tuple_numpy_data(numpy_data: Tuple[Any]) -> Schema:
        if not all((isinstance(data_element, np.ndarray) for data_element in numpy_data)):
            raise MlflowException('Failed to infer schema for tf.data.Dataset. Schemas can only be inferred if the dataset consists of tensors. Ragged tensors, tensor arrays, and other types are not supported. Additionally, datasets with nested tensors are not supported.', INVALID_PARAMETER_VALUE)
        return _infer_schema({str(i): data_element for i, data_element in enumerate(numpy_data)})
    ...
    def to_evaluation_dataset(self, path=None, feature_names=None) -> EvaluationDataset:
        ...
        return EvaluationDataset(data=self._features.numpy(), targets=self._targets.numpy() if self._targets is not None else None, path=path, feature_names=feature_names)