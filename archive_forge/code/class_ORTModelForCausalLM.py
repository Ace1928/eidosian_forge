...
class ORTModelForCausalLM(ORTModel, GenerationMixin):
    ...
    def __init__(self, model: onnxruntime.InferenceSession, config: 'PretrainedConfig', use_io_binding: Optional[bool]=None, model_save_dir: Optional[Union[str, Path, TemporaryDirectory]]=None, preprocessors: Optional[List]=None, generation_config: Optional[GenerationConfig]=None, use_cache: Optional[bool]=None, **kwargs):
        ...
        if model_type in MODEL_TYPES_REQUIRING_POSITION_IDS and 'position_ids' not in self.inputs_names:
            logger.warning(f'ORTModelForCausalLM loaded a legacy ONNX model with no position_ids input, although this input is required for batched generation for the architecture {model_type}. We strongly encourage to re-export the model with optimum>=1.14 for position_ids and batched inference support.')
        if use_cache ^ self.use_cache:
            raise ValueError(f'`use_cache` was set to `{use_cache}` but the loaded model only supports `use_cache={self.use_cache}`. Please load your current model with `use_cache={self.use_cache}` or export the original model once again with `use_cache={use_cache}` when calling the `from_pretrained` method. To export your model, simply set `export=True`.')
        if use_io_binding and (not use_cache):
            raise ValueError('The parameters combination use_cache=False, use_io_binding=True is not supported. Please either pass use_cache=True, use_io_binding=True (default), or use_cache=False, use_io_binding=False.')

    @add_start_docstrings_to_model_forward(CAUSALLM_ONNX_MODEL_DOCSTRING.format('batch_size, sequence_length') + TEXT_GENERATION_EXAMPLE.format(processor_class=_TOKENIZER_FOR_DOC, model_class='ORTModelForCausalLM', checkpoint='optimum/gpt2'))
    def forward(self, input_ids: torch.LongTensor, attention_mask: Optional[torch.FloatTensor]=None, position_ids: Optional[torch.LongTensor]=None, past_key_values: Optional[Tuple[Tuple[torch.Tensor]]]=None, labels: Optional[torch.LongTensor]=None, use_cache_branch: bool=None, **kwargs) -> CausalLMOutputWithPast:
        ...
        if self.use_cache:
            ...
            use_cache_branch, past_key_values, known_output_shapes = self.prepare_past_key_values(input_ids, past_key_values, use_torch)
        if self.use_io_binding:
            ...
            io_binding, output_shapes, output_buffers = self._prepare_io_binding(self.model, *model_inputs, known_output_shapes=known_output_shapes, ordered_input_names=self._ordered_input_names)
            ...
            if self.use_cache:
                past_key_values = tuple((torch.from_numpy(outputs[self.output_names[key]]).to(self.device) for key in self.key_value_output_names))
    ...
    def _from_pretrained(cls, model_id: Union[str, Path], config: 'PretrainedConfig', use_auth_token: Optional[Union[bool, str]]=None, revision: Optional[str]=None, force_download: bool=False, cache_dir: Optional[str]=None, file_name: Optional[str]=None, subfolder: str='', use_cache: bool=True, local_files_only: bool=False, use_merged: Optional[bool]=None, provider: str='CPUExecutionProvider', session_options: Optional[onnxruntime.SessionOptions]=None, provider_options: Optional[Dict[str, Any]]=None, use_io_binding: Optional[bool]=None, model_save_dir: Optional[Union[str, Path, TemporaryDirectory]]=None, **kwargs) -> 'ORTModelForCausalLM':
        ...
        if file_name is None:
            ...
            if use_merged is not False:
                try:
                    decoder_path = ORTModelForCausalLM.infer_onnx_filename(model_id, [DECODER_MERGED_ONNX_FILE_PATTERN], argument_name=None, subfolder=subfolder, use_auth_token=use_auth_token, revision=revision)
                    use_merged = True
            ...
            if use_merged is False:
                ...
                decoder_path = ORTModelForCausalLM.infer_onnx_filename(model_id, ['^((?!decoder).)*.onnx', pattern], argument_name=None, subfolder=subfolder, use_auth_token=use_auth_token, revision=revision)
                file_name = decoder_path.name
            ...
            if file_name not in regular_file_names:
                logger.warning(f'The ONNX file {file_name} is not a regular name used in optimum.onnxruntime that are {regular_file_names}, the {cls.__name__} might not behave as expected.')
        model_cache_path, preprocessors = cls._cached_file(model_path=model_path, use_auth_token=use_auth_token, revision=revision, force_download=force_download, cache_dir=cache_dir, file_name=file_name, subfolder=subfolder, local_files_only=local_files_only)
        ...
        if input_dims['input_ids'][1] == 1:
            ...
            onnx.save(onnx_model, str(model_cache_path), save_as_external_data=model_uses_external_data, all_tensors_to_one_file=True, location=model_cache_path.name + '_data', size_threshold=0)
        del onnx_model
        model = ORTModel.load_model(model_cache_path, provider=provider, session_options=session_options, provider_options=provider_options)
        ...
        return init_cls(model=model, config=config, use_io_binding=use_io_binding, model_save_dir=model_save_dir, preprocessors=preprocessors, use_cache=use_cache)
    ...
    def prepare_inputs_for_generation(self, input_ids, past_key_values=None, **kwargs):
        ...
        return {'input_ids': input_ids, 'past_key_values': past_key_values, 'use_cache': use_cache, 'position_ids': position_ids, 'attention_mask': attention_mask}
