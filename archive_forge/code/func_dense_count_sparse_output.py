...
def dense_count_sparse_output(values: _atypes.TensorFuzzingAnnotation[TV_DenseCountSparseOutput_T], weights: _atypes.TensorFuzzingAnnotation[TV_DenseCountSparseOutput_output_type], binary_output: bool, minlength: int=-1, maxlength: int=-1, name=None):
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'DenseCountSparseOutput', name, values, weights, 'minlength', minlength, 'maxlength', maxlength, 'binary_output', binary_output)
            ...
            pass
        try:
            return dense_count_sparse_output_eager_fallback(values, weights, minlength=minlength, maxlength=maxlength, binary_output=binary_output, name=name, ctx=_ctx)
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('DenseCountSparseOutput', values=values, weights=weights, binary_output=binary_output, minlength=minlength, maxlength=maxlength, name=name)
    _result = _outputs[:]
    if _execute.must_record_gradient():
        ...
        _execute.record_gradient('DenseCountSparseOutput', _inputs_flat, _attrs, _result)
    _result = _DenseCountSparseOutputOutput._make(_result)
    return _result