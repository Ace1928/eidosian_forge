...
class Ospfv3(ConfigBase):
    ...
    def _render_list_dict_param(self, attr, want, have, cmd=None, opr=True):
        ...
        commands = []
        h = []
        name = {'redistribute': 'route_type', 'range': 'address'}
        leaf_dict = {'redistribute': ('route_map', 'route_type'), 'range': ('address', 'advertise', 'not_advertise')}
        leaf = leaf_dict[attr]
        w = want.get(attr) or []
        if have:
            h = have.get(attr) or []
            ...
            for w_item in w:
                for key, val in iteritems(w_item):
                    ...
                    if opr and key in leaf and (not _is_w_same(w_item, h_item, key)):
                        if key == 'route_type' or (key == 'address' and 'advertise' not in w_item and ('not-advertise' not in w_item)):
                            if not val:
