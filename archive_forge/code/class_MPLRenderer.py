...
class MPLRenderer(Renderer):
    ...
    fig = param.ObjectSelector(default='auto', objects=['png', 'svg', 'pdf', 'pgf', 'html', None, 'auto'], doc='\n        Output render format for static figures. If None, no figure\n        rendering will occur. ')
    holomap = param.ObjectSelector(default='auto', objects=['widgets', 'scrubber', 'webm', 'mp4', 'gif', None, 'auto'], doc='\n        Output render multi-frame (typically animated) format. If\n        None, no multi-frame rendering will occur.')
    ...
    mode_formats = {'fig': ['png', 'svg', 'pdf', 'pgf', 'html', None, 'auto'], 'holomap': ['widgets', 'scrubber', 'webm', 'mp4', 'gif', 'html', None, 'auto']}
    counter = 0
    ...
    def _figure_data(self, plot, fmt, bbox_inches='tight', as_script=False, **kwargs):
        ...
        if fmt in ['gif', 'mp4', 'webm']:
            ...
            extra_artists = list(chain.from_iterable((artists for artists in plot.traverse(traverse_fn) if artists is not None)))
            kw = dict(format=fmt, facecolor=fig.get_facecolor(), edgecolor=fig.get_edgecolor(), dpi=self.dpi, bbox_inches=bbox_inches, bbox_extra_artists=extra_artists)
            kw.update(kwargs)
            try:
