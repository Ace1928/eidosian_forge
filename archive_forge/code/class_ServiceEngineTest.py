...
class ServiceEngineTest(common.HeatTestCase):
    ...
    def test_service_manage_report_start(self, mock_admin_context, mock_service_create, mock_service_update):
        ...
        mock_service_create.assert_called_once_with(self.ctx, dict(host=self.eng.host, hostname=self.eng.hostname, binary=self.eng.binary, engine_id=self.eng.engine_id, topic=self.eng.topic, report_interval=cfg.CONF.periodic_interval))
        self.assertEqual(srv['id'], self.eng.service_id)
        mock_service_update.assert_called_once_with(self.ctx, self.eng.service_id, dict(deleted_at=None))
    ...
    @mock.patch('heat.common.service_utils.generate_engine_id', return_value='sample-uuid')
    ...
    def test_engine_service_start_in_non_convergence_mode(self, configure_db_conn_pool_size, thread_group_class, worker_service_class, engine_listener_class, thread_group_manager_class, sample_uuid_method, rpc_client_class, target_class, rpc_server_method):
    ...
    @mock.patch.object(worker.WorkerService, 'stop')
    ...
    def test_engine_service_stop_in_convergence_mode(self, service_delete_method, admin_context_method, worker_service_stop):
