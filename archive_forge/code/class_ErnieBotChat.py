...
@deprecated(since='0.0.13', alternative='langchain_community.chat_models.QianfanChatEndpoint')
class ErnieBotChat(BaseChatModel):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        values['ernie_client_id'] = get_from_dict_or_env(values, 'ernie_client_id', 'ERNIE_CLIENT_ID')
        values['ernie_client_secret'] = get_from_dict_or_env(values, 'ernie_client_secret', 'ERNIE_CLIENT_SECRET')
        return values

    def _chat(self, payload: object) -> dict:
        base_url = f'{self.ernie_api_base}/rpc/2.0/ai_custom/v1/wenxinworkshop/chat'
        model_paths = {'ERNIE-Bot-turbo': 'eb-instant', 'ERNIE-Bot': 'completions', 'ERNIE-Bot-8K': 'ernie_bot_8k', 'ERNIE-Bot-4': 'completions_pro', 'ERNIE-Bot-turbo-AI': 'ai_apaas', 'BLOOMZ-7B': 'bloomz_7b1', 'Llama-2-7b-chat': 'llama_2_7b', 'Llama-2-13b-chat': 'llama_2_13b', 'Llama-2-70b-chat': 'llama_2_70b'}
        ...
        resp = requests.post(url, timeout=self.request_timeout, headers={'Content-Type': 'application/json'}, params={'access_token': self.access_token}, json=payload)
        return resp.json()
    ...
    def _refresh_access_token_with_lock(self) -> None:
        with self._lock:
            ...
            resp = requests.post(base_url, timeout=10, headers={'Content-Type': 'application/json', 'Accept': 'application/json'}, params={'grant_type': 'client_credentials', 'client_id': self.ernie_client_id, 'client_secret': self.ernie_client_secret})
    ...
    def _generate(self, messages: List[BaseMessage], stop: Optional[List[str]]=None, run_manager: Optional[CallbackManagerForLLMRun]=None, **kwargs: Any) -> ChatResult:
        ...
        payload = {'messages': [_convert_message_to_dict(m) for m in messages], 'top_p': self.top_p, 'temperature': self.temperature, 'penalty_score': self.penalty_score, 'system': self.system, **kwargs}
    ...
    def _create_chat_result(self, response: Mapping[str, Any]) -> ChatResult:
        if 'function_call' in response:
            ...
            additional_kwargs = {}
        generations = [ChatGeneration(message=AIMessage(content=response.get('result', ''), additional_kwargs={**additional_kwargs}))]
