...
class Insight(base.ClientBase):
    ...
    def _CreateMarkRequest(self, name, state, state_metadata, etag):
        ...
        return self._GetMessage('RecommenderProjectsLocationsInsightTypesInsightsMark{}Request'.format(state))(**kwargs)
    ...
    Returns:
      The Insight message.
    """
        request = self._messages.RecommenderProjectsLocationsInsightTypesInsightsGetRequest(name=name)
    ...
    """
        request = self._messages.RecommenderProjectsLocationsInsightTypesInsightsListRequest(parent=parent_name, filter=request_filter)
        return list_pager.YieldFromList(self._service, request, batch_size_attribute='pageSize', batch_size=page_size, limit=limit, field='insights')
