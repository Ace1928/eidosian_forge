...
class NetAppCloudManagerCVOAZURE:
    ...
    def __init__(self):
        ...
        self.changeable_params = ['svm_password', 'svm_name', 'azure_tag', 'tier_level', 'ontap_version', 'instance_type', 'license_type', 'writing_speed_state']
    ...
    def get_extra_azure_tags(self, rest_api, headers):
        ...
        return [{'tag_key': 'DeployedByOccm', 'tag_value': we['userTags']['DeployedByOccm']}] if 'DeployedByOccm' in we['userTags'] else []
    ...
    def validate_cvo_params(self):
        ...
        if self.parameters['is_ha'] and self.parameters['license_type'] == 'azure-ha-cot-premium-byol':
            if self.parameters.get('platform_serial_number_node1') is None or self.parameters.get('platform_serial_number_node2') is None:
                self.module.fail_json(msg='both platform_serial_number_node1 and platform_serial_number_node2 parameters are requiredwhen having ha type as true and license_type as azure-ha-cot-premium-byol')
    ...
    def apply(self):
        ...
        modify = None
        current, dummy = self.na_helper.get_working_environment_details_by_name(self.rest_api, self.headers, self.parameters['name'], 'azure')
        if current:
