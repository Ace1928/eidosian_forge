...
def heuristic(
    ...
    return base_distance + body_proximity_penalty
...
class Snake:
    ...
    def calculate_path(self) -> List[Tuple[int, int]]:
        ...
        """
        return a_star(
            self.fruit.position, self.body[-1], set(self.body), list(self.body)[:-1]
        )
    ...
    def move(self) -> bool:
        ...
        if self.path:
            ...
            if self.is_collision(next_pos):
                ...
                return False
            # Implementing zigzag movement
            if self.body:
                last_segment = self.body[0]
                direction_x = (next_pos[0] - last_segment[0]) // BLOCK_SIZE
                direction_y = (next_pos[1] - last_segment[1]) // BLOCK_SIZE
                intermediate_pos = (
                    last_segment[0] + direction_x * BLOCK_SIZE,
                    last_segment[1],
                )
                if not self.is_collision(intermediate_pos):
                    self.body.appendleft(intermediate_pos)
                intermediate_pos = (
                    intermediate_pos[0],
                    intermediate_pos[1] + direction_y * BLOCK_SIZE,
                )
                if not self.is_collision(intermediate_pos):
                    self.body.appendleft(intermediate_pos)
