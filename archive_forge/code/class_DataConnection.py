...
class DataConnection:
    ...
    def add_relaxed_candidate(self, a, find_neighbors=None, perform_parametrization=None):
        """After a candidate is relaxed it must be marked
        ...
        *not* been used.
        ...
        if perform_parametrization is not None:
            set_parametrization(a, perform_parametrization(a))
        relax_id = self.c.write(a, relaxed=1, key_value_pairs=a.info['key_value_pairs'], data=a.info['data'])
    ...
    def add_unrelaxed_candidate(self, candidate, description):
        ...
        gaid = self.c.write(candidate, key_value_pairs=candidate.info['key_value_pairs'], data=candidate.info['data'], **kwargs)
