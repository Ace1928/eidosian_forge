...
def create_tensor_wave(signal: np.ndarray, sr: int) -> np.ndarray:
    """Create a complex 2D tensor wave representation of the input signal."""
    ...
    try:
        ...
        # Ensure all arrays have the same length along the concatenation axis
        min_length = min(len(magnitude_spectrum), len(phase_spectrum), len(mfccs))
        magnitude_spectrum = magnitude_spectrum[:min_length]
        phase_spectrum = phase_spectrum[:min_length]
        spectral_centroid = np.full(min_length, spectral_centroid)
        spectral_bandwidth = np.full(min_length, spectral_bandwidth)
        spectral_rolloff = np.full(min_length, spectral_rolloff)
        zero_crossing_rate = np.full(min_length, zero_crossing_rate)
        mfccs = mfccs[:min_length]

        # Construct a complex 2D tensor representation
        logging.info("Constructing complex 2D tensor representation")
        ...
        logging.info("Complex 2D tensor representation construction complete")

        # Normalize and scale tensor representations
        logging.info("Normalizing and scaling tensor representations")
        scaler = MinMaxScaler()
        tensor_wave = scaler.fit_transform(tensor_wave.T).T
