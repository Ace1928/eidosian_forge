    ...
    **How do I create a new Predictor?**
    ...
    def predict(self, data: DataBatchType, **kwargs) -> DataBatchType:
        ...
        if not hasattr(self, '_preprocessor'):
            raise NotImplementedError('Subclasses of Predictor must call Predictor.__init__(preprocessor).')
        try:
            batch_format = TYPE_TO_ENUM[type(data)]
        except KeyError:
            raise RuntimeError(f'Invalid input data type of {type(data)}, supported types: {list(TYPE_TO_ENUM.keys())}')
        ...
        if batch_format == BatchFormat.PANDAS:
            if batch_format_to_use == BatchFormat.PANDAS:
                return self._predict_pandas(_convert_batch_type_to_pandas(data), **kwargs)
            elif batch_format_to_use == BatchFormat.NUMPY:
                return _convert_batch_type_to_pandas(self._predict_numpy(_convert_batch_type_to_numpy(data), **kwargs))
        elif batch_format == BatchFormat.NUMPY:
            if batch_format_to_use == BatchFormat.PANDAS:
                return _convert_batch_type_to_numpy(self._predict_pandas(_convert_batch_type_to_pandas(data), **kwargs))
            elif batch_format_to_use == BatchFormat.NUMPY:
    ...
    def __reduce__(self):
        raise PredictorNotSerializableException('Predictor instances are not serializable. Instead, you may want to serialize a checkpoint and initialize the Predictor with Predictor.from_checkpoint.')