...
class InfoLM(Metric):
    ...
    def update(self, preds: Union[str, Sequence[str]], target: Union[str, Sequence[str]]) -> None:
        ...
        preds_input_ids, preds_attention_mask, target_input_ids, target_attention_mask = _infolm_update(preds, target, self.tokenizer, self.max_length)
    ...
    def compute(self) -> Union[Tensor, Tuple[Tensor, Tensor]]:
        ...
        preds_dataloader = _get_dataloader(input_ids=dim_zero_cat(self.preds_input_ids), attention_mask=dim_zero_cat(self.preds_attention_mask), idf=self.idf, batch_size=self.batch_size, num_workers=self.num_threads)
        target_dataloader = _get_dataloader(input_ids=dim_zero_cat(self.target_input_ids), attention_mask=dim_zero_cat(self.target_attention_mask), idf=self.idf, batch_size=self.batch_size, num_workers=self.num_threads)
        info_lm_score = _infolm_compute(self.model, preds_dataloader, target_dataloader, self.temperature, self.idf, self.information_measure_cls, self.special_tokens_map, self.verbose)
