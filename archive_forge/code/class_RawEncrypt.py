  ...
  """
    ...
    def _CreateRawEncryptRequest(self, args):
        if args.initialization_vector_file and args.additional_authenticated_data_file:
            raise exceptions.InvalidArgumentException('--initialization-vector-file and --additional-authenticated-data-file', 'both parameters cannot be provided simultaneously.')
        if args.plaintext_file == '-' and (args.initialization_vector_file == '-' or args.additional_authenticated_data_file == '-'):
            raise exceptions.InvalidArgumentException('--plaintext-file', 'multiple parameters cannot be read from stdin.')
        try:
        ...
        req = messages.CloudkmsProjectsLocationsKeyRingsCryptoKeysCryptoKeyVersionsRawEncryptRequest(name=crypto_key_ref.RelativeName())
        if self._PerformIntegrityVerification(args):
            ...
            req.rawEncryptRequest = messages.RawEncryptRequest(plaintext=plaintext, initializationVector=iv, additionalAuthenticatedData=aad, plaintextCrc32c=plaintext_crc32c, initializationVectorCrc32c=iv_crc32c, additionalAuthenticatedDataCrc32c=aad_crc32c)
        else:
            req.rawEncryptRequest = messages.RawEncryptRequest(plaintext=plaintext, initializationVector=iv, additionalAuthenticatedData=aad)
        return req
    ...
    def Run(self, args):
        ...
        resp = None
        try:
            resp = client.projects_locations_keyRings_cryptoKeys_cryptoKeyVersions.RawEncrypt(req)
        ...
        try:
            log.WriteToFileOrStdout(args.ciphertext_file, resp.ciphertext, binary=True, overwrite=True)
            if not args.initialization_vector_file and resp.initializationVector:
                iv_file_name = './inialization_vector_' + str(uuid.uuid4())[:8]
                files.WriteBinaryFileContents(iv_file_name, resp.initializationVector, overwrite=True)
        except files.Error as e:
