...
class ShardedTensor(ShardedTensorBase):
    ...
    def __init__(self, sharding_spec: shard_spec.ShardingSpec, *size, dtype=None, layout=torch.strided, requires_grad=False, pin_memory=False, memory_format=torch.contiguous_format, process_group=None, init_rrefs=False):
        ...
        for shard_metadata in self._metadata.shards_metadata:
            ...
            if rank == current_rank:
                local_tensor = _create_tensor_from_params(shard_metadata.shard_sizes, local_device=device, tensor_properties=self._metadata.tensor_properties)
    ...
    def _post_init(self):
        if self._init_rrefs:
            ...
            if not rpc._is_current_rpc_agent_set():
                raise RuntimeError('RPC Framework needs to be initialized using torch.distributed.rpc.init_rpc if init_rrefs is set to True')
            self._init_rpc()
    ...
    def gather(self, dst: int=0, out: Optional[torch.Tensor]=None, enforce_dtype: bool=False, dtype: Optional[torch.dtype]=None) -> None:
        """
        ...
        dist.gather(tensor=data, gather_list=gather_list, dst=dst, group=self._process_group)
        if rank != dst:
            return
        ...
        for shard_md in self.metadata().shards_metadata:
            ...
            for dim in range(dims):
                out_narrow_view = out_narrow_view.narrow(dim, shard_md.shard_offsets[dim], shard_md.shard_sizes[dim])
    ...
    def cuda(self, device=None, non_blocking=False, memory_format=torch.preserve_format, process_group=None) -> ShardedTensor:
        """
    ...
    def to(self, *args, **kwargs) -> ShardedTensor:
        ...
        if device_to.type == 'cuda':
            ...
            if device_to.index != current_idx:
                warnings.warn('ShardedTensor.to only move tensor to its current deviceIf you want to put to different device, use `reshard` instead.')
        ...
        list_shards: List[Shard] = []
        for shard in self._local_shards:
            new_tensor = shard.tensor.to(device=device_to, dtype=dtype_to, non_blocking=non_blocking, copy=copy_tensor, memory_format=memory_format)
            metadata = copy.deepcopy(shard.metadata)
        ...
        for meta in st_meta.shards_metadata:
            meta.placement._device = device_to
        pg = self._process_group if process_group is None else process_group
        st_to = ShardedTensor._init_from_local_shards_and_global_metadata(list_shards, sharded_tensor_metadata=st_meta, process_group=pg, init_rrefs=self._init_rrefs)
        return st_to
    ...
    def reshard(self, resharding_spec: shard_spec.ShardingSpec) -> ShardedTensor:
        ...
        """
        warnings.warn(DEPRECATE_MSG)
        if not isinstance(resharding_spec, shard_spec.ChunkShardingSpec) or not isinstance(self._sharding_spec, shard_spec.ChunkShardingSpec):
        ...
        if self._sharding_spec.dim == resharding_spec.dim:
            if self._sharding_spec.placements == resharding_spec.placements:
                ...
                local_shards, shards_metadata = reshuffle_local_shard(self.local_tensor(), self.size(), self._sharding_spec, resharding_spec, self._process_group)
        else:
            local_shards, shards_metadata = reshard_local_shard(self.local_tensor(), self.size(), self._sharding_spec, resharding_spec, self._process_group)
    ...
    def __torch_function__(cls, func, types, args=(), kwargs=None):
        ...
        def dispatch(st: ShardedTensor, func: Callable):
            ...
            raise RuntimeError(f"torch function '{func.__name__}', with args: {args} and kwargs: {kwargs} not supported for ShardedTensor!")
        warnings.warn(DEPRECATE_MSG)
        st_instance = None
    ...
    def __getstate__(self):
        pg_state = ShardedTensor.ProcessGroupState(distributed_c10d.get_rank(self._process_group), distributed_c10d.get_rank(), distributed_c10d.get_world_size(self._process_group), distributed_c10d.get_world_size())
    ...
    def __setstate__(self, state):
        ...
        if not distributed_c10d.is_initialized():
            raise RuntimeError('Need to initialize default process group using "init_process_group" before loading ShardedTensor')
        ...
        if pg_state.local_rank != local_rank:
            raise RuntimeError(f'Local rank at save time was {pg_state.local_rank}, but at load time was {local_rank}')
        global_rank = distributed_c10d.get_rank()
        if pg_state.global_rank != global_rank:
            raise RuntimeError(f'Global rank at save time was {pg_state.global_rank}, but at load time was {global_rank}')
        local_world_size = distributed_c10d.get_world_size(self._process_group)
        if pg_state.local_world_size != local_world_size:
            raise RuntimeError(f'Local world size at save time was {pg_state.local_world_size}, but at load time was {local_world_size}')
        global_world_size = distributed_c10d.get_world_size()
        if pg_state.global_world_size != global_world_size:
            raise RuntimeError(f'Global world size at save time was {pg_state.global_world_size}, but at load time was {global_world_size}')
        self._post_init()