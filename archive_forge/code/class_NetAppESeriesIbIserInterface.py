...
class NetAppESeriesIbIserInterface(NetAppESeriesModule):
    ...
    def __init__(self):
        ansible_options = dict(controller=dict(type='str', required=True, choices=['A', 'B']), channel=dict(type='int'), address=dict(type='str', required=True))
        super(NetAppESeriesIbIserInterface, self).__init__(ansible_options=ansible_options, web_services_version='02.00.0000.0000', supports_check_mode=True)
    ...
    def get_interfaces(self):
        ...
        ib_iser_ifaces = []
        for iface in ifaces:
            if iface['ioInterfaceTypeData']['interfaceType'] == 'iscsi' and iface['ioInterfaceTypeData']['iscsi']['interfaceData']['type'] == 'infiniband' and iface['ioInterfaceTypeData']['iscsi']['interfaceData']['infinibandData']['isIser'] or (iface['ioInterfaceTypeData']['interfaceType'] == 'ib' and iface['ioInterfaceTypeData']['ib']['isISERSupported']):
    ...
    def update(self):
        ...
        if update_required and (not self.check_mode):
            try:
                rc, result = self.request('storage-systems/%s/symbol/setIscsiInterfaceProperties' % self.ssid, method='POST', data=self.make_request_body())
            except Exception as error:
                self.module.fail_json(msg='Failed to modify the interface! Array Id [%s]. Error [%s].' % (self.ssid, to_native(error)))
