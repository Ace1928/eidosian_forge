...
class ComposeObjectsTask(task.Task):
    ...
    def __init__(self, source_resources, destination_resource, original_source_resource=None, posix_to_set=None, print_status_message=False, user_request_args=None):
        """Initializes task.
    ...
    def execute(self, task_status_queue=None):
        ...
        if cloud_api.Capability.COMPOSE_OBJECTS not in api.capabilities:
            raise command_errors.Error('Compose is not available with requested provider: {}'.format(provider))
        for source_resource in self._source_resources:
            if source_resource.storage_url.bucket_name != self._destination_resource.storage_url.bucket_name:
                raise command_errors.Error('Inter-bucket composing not supported')
        request_config = request_config_factory.get_request_config(self._destination_resource.storage_url, user_request_args=self._user_request_args)
        if self._print_status_message:
            log.status.write('Composing {} from {} component object(s).\n'.format(self._destination_resource, len(self._source_resources)))
        created_resource = api.compose_objects(self._source_resources, self._destination_resource, request_config, original_source_resource=self._original_source_resource, posix_to_set=self._posix_to_set)
        return task.Output(messages=[task.Message(topic=task.Topic.CREATED_RESOURCE, payload=created_resource)], additional_task_iterators=[])

    def __eq__(self, other):
        if not isinstance(other, ComposeObjectsTask):
            return NotImplemented
        return self._source_resources == other._source_resources and self._destination_resource == other._destination_resource and (self._user_request_args == other._user_request_args)