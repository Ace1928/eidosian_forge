...
def _validate_prompt_template(prompt_template):
    ...
    if not isinstance(prompt_template, str):
        raise MlflowException(f'Argument `prompt_template` must be a string, received {type(prompt_template)}', INVALID_PARAMETER_VALUE)
    format_args = [tup[1] for tup in string.Formatter().parse(prompt_template) if tup[1] is not None]
    if format_args != ['prompt']:
        raise MlflowException.invalid_parameter_value(f"Argument `prompt_template` must be a string with a single format arg, 'prompt'. For example: 'Answer the following question in a friendly tone. Q: {{prompt}}. A:'\nReceived {prompt_template}. ")