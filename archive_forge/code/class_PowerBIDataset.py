...
class PowerBIDataset(BaseModel):
    ...
    def request_url(self) -> str:
        """Get the request url."""
        if self.group_id:
            return f'{BASE_URL}/groups/{self.group_id}/datasets/{self.dataset_id}/executeQueries'
    ...
    def _create_json_content(self, command: str) -> dict[str, Any]:
        ...
        return {'queries': [{'query': f'{command}'}], 'impersonatedUserName': self.impersonated_user_name, 'serializerSettings': {'includeNulls': True}}

    def run(self, command: str) -> Any:
        """Execute a DAX command and return a json representing the results."""
        logger.debug('Running command: %s', command)
        response = requests.post(self.request_url, json=self._create_json_content(command), headers=self.headers, timeout=10)
    ...
    async def arun(self, command: str) -> Any:
        ...
        logger.debug('Running command: %s', command)
        if self.aiosession:
            async with self.aiosession.post(self.request_url, headers=self.headers, json=self._create_json_content(command), timeout=10) as response:
                if response.status == 403:
                    return 'TokenError: Could not login to PowerBI, please check your credentials.'
                response_json = await response.json(content_type=response.content_type)
                return response_json
        async with aiohttp.ClientSession() as session:
            async with session.post(self.request_url, headers=self.headers, json=self._create_json_content(command), timeout=10) as response:
                if response.status == 403:
                    return 'TokenError: Could not login to PowerBI, please check your credentials.'
                response_json = await response.json(content_type=response.content_type)
                return response_json