...
class Rockset(VectorStore):
    ...
    def __init__(self, client: Any, embeddings: Embeddings, collection_name: str, text_key: str, embedding_key: str, workspace: str='commons'):
        ...
        if not isinstance(client, RocksetClient):
            raise ValueError(f'client should be an instance of rockset.RocksetClient, got {type(client)}')
        self._client = client
    ...
    def add_texts(self, texts: Iterable[str], metadatas: Optional[List[dict]]=None, ids: Optional[List[str]]=None, batch_size: int=32, **kwargs: Any) -> List[str]:
    ...
    @classmethod
    def from_texts(cls, texts: List[str], embedding: Embeddings, metadatas: Optional[List[dict]]=None, client: Any=None, collection_name: str='', text_key: str='', embedding_key: str='', ids: Optional[List[str]]=None, batch_size: int=32, **kwargs: Any) -> Rockset:
    ...
    def similarity_search_with_relevance_scores(self, query: str, k: int=4, distance_func: DistanceFunction=DistanceFunction.COSINE_SIM, where_str: Optional[str]=None, **kwargs: Any) -> List[Tuple[Document, float]]:
    ...
    def similarity_search(self, query: str, k: int=4, distance_func: DistanceFunction=DistanceFunction.COSINE_SIM, where_str: Optional[str]=None, **kwargs: Any) -> List[Document]:
        """Same as `similarity_search_with_relevance_scores` but
        doesn't return the scores.
        """
        return self.similarity_search_by_vector(self._embeddings.embed_query(query), k, distance_func, where_str, **kwargs)

    def similarity_search_by_vector(self, embedding: List[float], k: int=4, distance_func: DistanceFunction=DistanceFunction.COSINE_SIM, where_str: Optional[str]=None, **kwargs: Any) -> List[Document]:
        """Accepts a query_embedding (vector), and returns documents with
        similar embeddings."""
        docs_and_scores = self.similarity_search_by_vector_with_relevance_scores(embedding, k, distance_func, where_str, **kwargs)
        return [doc for doc, _ in docs_and_scores]

    def similarity_search_by_vector_with_relevance_scores(self, embedding: List[float], k: int=4, distance_func: DistanceFunction=DistanceFunction.COSINE_SIM, where_str: Optional[str]=None, **kwargs: Any) -> List[Tuple[Document, float]]:
        """Accepts a query_embedding (vector), and returns documents with
        ...
        for document in query_response.results:
            metadata = {}
            assert isinstance(document, dict), 'document should be of type `dict[str,Any]`. But found: `{}`'.format(type(document))
            for k, v in document.items():
                if k == self._text_key:
                    assert isinstance(v, str), 'page content stored in column `{}` must be of type `str`. But found: `{}`'.format(self._text_key, type(v))
                    page_content = v
                elif k == 'dist':
                    assert isinstance(v, float), 'Computed distance between vectors must of type `float`. But found {}'.format(type(v))
                    score = v
                elif k not in ['_id', '_event_time', '_meta']:
                    metadata[k] = v
            finalResult.append((Document(page_content=page_content, metadata=metadata), score))
        return finalResult

    def max_marginal_relevance_search(self, query: str, k: int=4, fetch_k: int=20, lambda_mult: float=0.5, *, where_str: Optional[str]=None, **kwargs: Any) -> List[Document]:
        ...
        initial_docs = self.similarity_search_by_vector(query_embedding, k=fetch_k, where_str=where_str, exclude_embeddings=False, **kwargs)
        embeddings = [doc.metadata[self._embedding_key] for doc in initial_docs]
        selected_indices = maximal_marginal_relevance(np.array(query_embedding), embeddings, lambda_mult=lambda_mult, k=k)
        for i in selected_indices:
            del initial_docs[i].metadata[self._embedding_key]
        return [initial_docs[i] for i in selected_indices]

    def _build_query_sql(self, query_embedding: List[float], distance_func: DistanceFunction, k: int=4, where_str: Optional[str]=None, exclude_embeddings: bool=True) -> str:
        ...
        return f'SELECT *{select_embedding} {distance_str}\nFROM {self._workspace}.{self._collection_name}\n{where_str}ORDER BY dist {distance_func.order_by()}\nLIMIT {str(k)}\n'
