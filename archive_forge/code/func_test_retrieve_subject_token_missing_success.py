...
def test_retrieve_subject_token_missing_success(self):
    EXECUTABLE_SUCCESSFUL_OIDC_RESPONSE = {'version': 1, 'token_type': 'urn:ietf:params:oauth:token-type:id_token', 'id_token': self.EXECUTABLE_OIDC_TOKEN, 'expiration_time': 9999999999}
    with mock.patch('subprocess.run', return_value=subprocess.CompletedProcess(args=[], stdout=json.dumps(EXECUTABLE_SUCCESSFUL_OIDC_RESPONSE).encode('UTF-8'), returncode=0)):
        ...
        assert excinfo.match('The executable response is missing the success field.')