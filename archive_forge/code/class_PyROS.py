...
@SolverFactory.register('pyros', doc='Robust optimization (RO) solver implementing the generalized robust cutting-set algorithm (GRCS)')
class PyROS(object):
    """
    ...
    @document_kwargs_from_configdict(config=CONFIG, section='Keyword Arguments', indent_spacing=4, width=72, visibility=0)
    def solve(self, model, first_stage_variables, second_stage_variables, uncertain_params, uncertainty_set, local_solver, global_solver, **kwds):
        """Solve a model.
        ...
        """
        kwds.update(dict(first_stage_variables=first_stage_variables, second_stage_variables=second_stage_variables, uncertain_params=uncertain_params, uncertainty_set=uncertainty_set, local_solver=local_solver, global_solver=global_solver))
        ...
        with time_code(timing_data_obj=model_data.timing, code_block_name='main', is_main_timer=True):
            ...
            self._log_config(logger=config.progress_logger, config=config, exclude_options=None, level=logging.INFO)
            ...
            active_objs = list(model_data.working_model.component_data_objects(Objective, active=True, descend_into=True))
            ...
            replace_uncertain_bounds_with_constraints(model_data.working_model, model_data.working_model.util.uncertain_params)
            ...
            config.progress_logger.info(f'Done preprocessing; required wall time of {preprocessing_time:.3f}s.')
            ...
            if pyros_soln is not None and final_iter_separation_solns is not None:
                if config.load_solution and (pyros_soln.pyros_termination_condition is pyrosTerminationCondition.robust_optimal or pyros_soln.pyros_termination_condition is pyrosTerminationCondition.robust_feasible):
                    load_final_solution(model_data, pyros_soln.master_soln, config)
                if config.objective_focus == ObjectiveType.nominal:
                    return_soln.final_objective_value = active_obj_original_sense * value(pyros_soln.master_soln.master_model.obj)
                elif config.objective_focus == ObjectiveType.worst_case:
                    return_soln.final_objective_value = active_obj_original_sense * value(pyros_soln.master_soln.master_model.zeta)
