...
def firewall_decrypted_traffic_mirror(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('firewall', 'decrypted-traffic-mirror', filtered_data, vdom=vdom)
        current_data = fos.get('firewall', 'decrypted-traffic-mirror', vdom=vdom, mkey=mkey)
    ...
    if state == 'present' or state is True:
        return fos.set('firewall', 'decrypted-traffic-mirror', data=filtered_data, vdom=vdom)
    elif state == 'absent':
        return fos.delete('firewall', 'decrypted-traffic-mirror', mkey=filtered_data['name'], vdom=vdom)
    else:
