...
class LLMEngine:
    ...
    def __init__(self, model_config: ModelConfig, cache_config: CacheConfig, parallel_config: ParallelConfig, scheduler_config: SchedulerConfig, device_config: DeviceConfig, lora_config: Optional[LoRAConfig], placement_group: Optional['PlacementGroup'], log_stats: bool) -> None:
        logger.info(f'Initializing an LLM engine with config: model={model_config.model!r}, tokenizer={model_config.tokenizer!r}, tokenizer_mode={model_config.tokenizer_mode}, revision={model_config.revision}, tokenizer_revision={model_config.tokenizer_revision}, trust_remote_code={model_config.trust_remote_code}, dtype={model_config.dtype}, max_seq_len={model_config.max_model_len}, download_dir={model_config.download_dir!r}, load_format={model_config.load_format}, tensor_parallel_size={parallel_config.tensor_parallel_size}, disable_custom_all_reduce={parallel_config.disable_custom_all_reduce}, quantization={model_config.quantization}, enforce_eager={model_config.enforce_eager}, kv_cache_dtype={cache_config.cache_dtype}, device_config={device_config.device}, seed={model_config.seed})')
        ...
        if self.log_stats:
            self.stat_logger = StatLogger(local_interval=_LOCAL_LOGGING_INTERVAL_SEC, labels=dict(model_name=model_config.model))
    ...
    def _dispatch_worker(self):
        worker_module = DEVICE_TO_WORKER_MODULE_MAP[self.device_config.device_type]
    ...
    def encode_request(self, request_id: str, prompt: Optional[str], prompt_token_ids: Optional[List[int]]=None, lora_request: Optional[LoRARequest]=None):
    ...
    def _get_stats(self, scheduler_outputs: Optional[SchedulerOutputs]) -> Stats:
        ...
        if scheduler_outputs is not None:
            ...
            if prompt_run:
                num_prompt_tokens = sum((len(seq_group.prompt_token_ids) for seq_group in scheduler_outputs.scheduled_seq_groups))
                num_generation_tokens = sum((seq_group.num_seqs() for seq_group in scheduler_outputs.scheduled_seq_groups))
            else:
    ...
    def add_lora(self, lora_request: LoRARequest) -> bool:
        ...
        return self._run_workers('add_lora', lora_request=lora_request)

    def remove_lora(self, lora_id: int) -> bool:
        assert lora_id > 0, 'lora_id must be greater than 0.'
        return self._run_workers('remove_lora', lora_id=lora_id)
