...
class ConsoleCLI(CLI, cmd.Cmd):
    ...
    modules = []
    ARGUMENTS = {'host-pattern': 'A name of a group in the inventory, a shell-like glob selecting hosts in inventory or any combination of the two separated by commas.'}
    ...
    def init_parser(self):
        super(ConsoleCLI, self).init_parser(desc='REPL console for executing Ansible tasks.', epilog='This is not a live session/connection: each task is executed in the background and returns its results.')
    ...
    def default(self, line, forceshell=False):
        ...
        try:
            ...
            play_ds = dict(name='Ansible Shell', hosts=self.cwd, gather_facts='no', tasks=[task], remote_user=self.remote_user, become=self.become, become_user=self.become_user, become_method=self.become_method, check_mode=self.check_mode, diff=self.diff, collections=self.collections)
        ...
        try:
            self._tqm = None
            try:
                self._tqm = TaskQueueManager(inventory=self.inventory, variable_manager=self.variable_manager, loader=self.loader, passwords=self.passwords, stdout_callback=cb, run_additional_callbacks=C.DEFAULT_LOAD_CALLBACK_PLUGINS, run_tree=False, forks=self.forks)
