...
class DynamicQuantizeLinear(OpRun):
    ...
    def _run(self, x):
        ...
        y_scale = np.float32(1.0 if maxx == minx else maxx - minx) / np.float32(qmax - qmin)
        ...
        return (y.astype(dtype), np.array(y_scale.astype(x.dtype)), np.array(zpi.astype(dtype)))