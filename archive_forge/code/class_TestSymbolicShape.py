...
class TestSymbolicShape(unittest.TestCase):
    ...
    def test_unknown_shape(self) -> None:
        concat = helper.make_node('Concat', inputs=['A', 'B'], outputs=['C'], name='Concat', axis=1)
        cast = onnx.helper.make_node('Cast', inputs=['C'], outputs=['output'], to=TensorProto.FLOAT)
        graph_def = helper.make_graph(name='test_graph', nodes=[concat, cast], inputs=[helper.make_tensor_value_info('A', TensorProto.FLOAT, [3, None]), helper.make_tensor_value_info('B', TensorProto.FLOAT, [3, None])], outputs=[helper.make_tensor_value_info('output', TensorProto.FLOAT, [3, None])])
        onnx_model = make_model(graph_def)
        inferred_model = onnx.shape_inference.infer_shapes(onnx_model, strict_mode=True)
