...
class CurlClient(BaseClient):
    ...
    def _process_queue(self):
        while 1:
            ...
            while self._free_list and self._pending:
                ...
                buf = BytesIO()
                curl.info = {'headers': headers, 'buffer': buf, 'request': request, 'curl_start_time': time()}
    ...
    def _process(self, curl, errno=None, reason=None, _pycurl=pycurl):
        ...
        request.on_ready(self.Response(request=request, code=code, headers=info['headers'], buffer=buffer, effective_url=effective_url, error=error))
