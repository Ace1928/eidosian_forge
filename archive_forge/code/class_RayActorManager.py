...
class RayActorManager:
    ...
    def __init__(self, resource_manager: ResourceManager):
        ...
        self._tracked_actors_to_task_futures: Dict[TrackedActor, Set[ray.ObjectRef]] = defaultdict(set)
        self._tracked_actors_to_state_futures: Dict[TrackedActor, Set[ray.ObjectRef]] = defaultdict(set)
        self._resource_request_to_pending_actors: Dict[ResourceRequest, List[TrackedActor]] = defaultdict(list)
        self._pending_actors_to_attrs: Dict[TrackedActor, Tuple[Type, Dict[str, Any], ResourceRequest]] = {}
        self._pending_actors_to_enqueued_actor_tasks: Dict[TrackedActor, List[Tuple[TrackedActorTask, str, Tuple[Any], Dict[str, Any]]]] = defaultdict(list)
        self._live_actors_to_ray_actors_resources: Dict[TrackedActor, Tuple[ray.actor.ActorHandle, AcquiredResources]] = {}
    ...
    def next(self, timeout: Optional[Union[int, float]]=None) -> bool:
        ...
        if future in actor_state_futures:
            ...
            return self.next(timeout=max(1e-09, timeout - time_taken) if timeout is not None else None)
        self._try_start_actors()
        return True
    ...
    def _actor_task_failed(self, tracked_actor_task: TrackedActorTask, exception: Exception):
        ...
        if isinstance(exception, RayActorError):
            ...
            raise RuntimeError(f'Caught unexpected exception: {exception}') from exception
    ...
    def _schedule_tracked_actor_task(self, tracked_actor_task: TrackedActorTask, method_name: str, *, args: Optional[Tuple]=None, kwargs: Optional[Dict]=None, _return_future: bool=False) -> Union[TrackedActorTask, Tuple[TrackedActorTask, ray.ObjectRef]]:
        tracked_actor = tracked_actor_task._tracked_actor
        ray_actor, _ = self._live_actors_to_ray_actors_resources[tracked_actor]
        try:
            remote_fn = getattr(ray_actor, method_name)
        except AttributeError as e:
            raise AttributeError(f'Remote function `{method_name}()` does not exist for this actor.') from e

        def on_result(result: Any):
            self._actor_task_resolved(tracked_actor_task=tracked_actor_task, result=result)

        def on_error(exception: Exception):
            self._actor_task_failed(tracked_actor_task=tracked_actor_task, exception=exception)
        future = remote_fn.remote(*args, **kwargs)
        self._actor_task_events.track_future(future=future, on_result=on_result, on_error=on_error)
    ...
    def cleanup(self):
        for actor, acquired_resources in self._live_actors_to_ray_actors_resources.values():
            ray.kill(actor)
            self._resource_manager.free_resources(acquired_resources)
        for resource_request, pending_actors in self._resource_request_to_pending_actors.items():
