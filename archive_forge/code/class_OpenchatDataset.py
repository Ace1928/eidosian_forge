...
class OpenchatDataset(IterableDataset):
    ...
    def __init__(self, dataset_filename, batch_max_length, rank, num_replicas):
        ...
        self.BATCH_KEYS = {'seqlens': torch.int32, 'nz_input_ids': torch.long, 'nz_position_ids': torch.long, 'nz_shifted_label_ids': torch.long, 'nz_shifted_loss_weights': torch.bfloat16}
        ...
        del table
        self.sampler = MultipackDistributedSampler(lengths=self.dataset['total_length'], numseqs=self.dataset['num_seqs'], batch_max_length=batch_max_length, rank=rank, num_replicas=num_replicas, seed=0)
        self._epoch = 0
