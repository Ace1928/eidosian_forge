...
class TestFillNA:
    ...
    def test_fillna_with_columns_and_limit(self):
        ...
        expected = DataFrame({'A': Series([100, 3, 100, 100], dtype='float64'), 'B': [2, 4, np.nan, 3], 'C': [np.nan, 100, np.nan, np.nan], 'D': Series([0, 1, 5, 4], dtype='float64')}, index=[0, 1, 2, 3])
        expected2 = DataFrame({'A': Series([100, 3, 100, 100], dtype='float64'), 'B': Series([2, 4, 100, 3], dtype='float64'), 'C': [100, 100, np.nan, 100], 'D': Series([0, 1, 5, 4], dtype='float64')}, index=[0, 1, 2, 3])
        tm.assert_frame_equal(result, expected)
    ...
    def test_single_block_df_with_horizontal_axis(self):
        df = DataFrame({'col1': [5, 0, np.nan, 10, np.nan], 'col2': [7, np.nan, np.nan, 5, 3], 'col3': [12, np.nan, 1, 2, 0], 'col4': [np.nan, 1, 1, np.nan, 18]})
        result = df.fillna(50, limit=1, axis=1)
        expected = DataFrame([[5.0, 7.0, 12.0, 50.0], [0.0, 50.0, np.nan, 1.0], [50.0, np.nan, 1.0, 1.0], [10.0, 5.0, 2.0, 50.0], [50.0, 3.0, 0.0, 18.0]], columns=['col1', 'col2', 'col3', 'col4'])
        tm.assert_frame_equal(result, expected)
