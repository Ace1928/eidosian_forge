...
def ravel_multi_index(multi_index, dims, mode='wrap', order='C'):
    ...
    if len(multi_index) != ndim:
        raise ValueError('parameter multi_index must be a sequence of length {}'.format(ndim))
    for d in dims:
        if not isinstance(d, numbers.Integral):
            raise TypeError('{} object cannot be interpreted as an integer'.format(type(d)))
    if isinstance(mode, str):
        mode = (mode,) * ndim
    if functools.reduce(operator.mul, dims) > cupy.iinfo(cupy.int64).max:
        raise ValueError('invalid dims: array size defined by dims is larger than the maximum possible size')
    s = 1
    ...
    for d, stride, idx, _mode in zip(dims, ravel_strides, multi_index, mode):
        ...
        if not cupy.can_cast(idx, cupy.int64, 'same_kind'):
            raise TypeError("multi_index entries could not be cast from dtype('{}') to dtype('{}') according to the rule 'same_kind'".format(idx.dtype, cupy.int64().dtype))
