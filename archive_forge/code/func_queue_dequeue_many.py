...
def queue_dequeue_many(handle: _atypes.TensorFuzzingAnnotation[_atypes.String], n: _atypes.TensorFuzzingAnnotation[_atypes.Int32], component_types, timeout_ms: int=-1, name=None):
  ...
  """
    ...
    if not isinstance(component_types, (list, tuple)):
        raise TypeError("Expected list for 'component_types' argument to 'queue_dequeue_many' Op, not %r." % component_types)
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('QueueDequeueMany', handle=handle, n=n, component_types=component_types, timeout_ms=timeout_ms, name=name)
    _result = _outputs[:]
    if _execute.must_record_gradient():
        _attrs = ('component_types', _op.get_attr('component_types'), 'timeout_ms', _op._get_attr_int('timeout_ms'))
        _inputs_flat = _op.inputs
        _execute.record_gradient('QueueDequeueMany', _inputs_flat, _attrs, _result)
    return _result