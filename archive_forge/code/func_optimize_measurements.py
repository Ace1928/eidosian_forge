...
def optimize_measurements(observables, coefficients=None, grouping='qwc', colouring_method='rlf'):
    ...
    **Example**
    ...
    if coefficients is None:
        grouped_obs = group_observables(observables, grouping_type=grouping, method=colouring_method)
    else:
        grouped_obs, grouped_coeffs = group_observables(observables, coefficients, grouping_type=grouping, method=colouring_method)
    if grouping.lower() == 'qwc':
        post_rotations, diagonalized_groupings = diagonalize_qwc_groupings(grouped_obs)
    else:
        raise NotImplementedError(f"Measurement reduction by '{grouping.lower()}' grouping not implemented.")
    if coefficients is None:
