...
class GraphLowering(torch.fx.Interpreter):
    ...
    def __init__(self, gm: torch.fx.GraphModule, example_inputs: Optional[List[torch.Tensor]]=None, shape_env=None, num_static_inputs=None, graph_id=None, cpp_wrapper=False, aot_mode=False, user_visible_outputs=frozenset(), layout_opt=None, extern_node_serializer=None, is_inference=False):
        super().__init__(gm)
        self.example_inputs = example_inputs
        self.layout_opt = layout_opt if layout_opt is not None else self.decide_layout_opt(gm, is_inference=is_inference)
    ...
    def get_dtype(self, buffer_name: str):
        ...
        m = re.match('(as_strided|reinterpret_tensor)\\(([a-zA-Z0-9_]+),', buffer_name)
        if m:
    ...
    def register_users_of(self, node_output):
        ...
        def register(value):
            ...
            if isinstance(value, ir.IRNode):
                if not hasattr(value, 'data') or not isinstance(value.data, ir.IRNode) or (not (hasattr(value.data, 'data') and isinstance(value.data.data, ir.IRNode))):
                    return
    ...
    def placeholder(self, target: str, args, kwargs):
        ...
        tensor = TensorBox.create(InputBuffer(target, FixedLayout(example.device, example.dtype, sizes, strides)))
    ...
    def call_function(self, target, args, kwargs):
        ...
        if target not in lowerings:
            ...
            if base_name in FALLBACK_ALLOW_LIST:
                ...
                log.info('Creating implicit fallback for:\n%s', error.operator_str(target, args, kwargs))
                make_fallback(target)
    ...
    def output(self, target, args, kwargs):
        ...
        assert all((isinstance(x, (TensorBox, ir.Constant, type(None), ir.ConstantBuffer, sympy.Expr, sympy.logic.boolalg.Boolean, int)) for x in result)), result
        ...
        for name, value in self.graph_inputs.items():
            ...
            if not isinstance(value, InputBuffer) or value.get_name() != name:
                ...
                try:
                    ...
                    pass
        self.finalize()
        log.debug('Force channels last inputs for %d conv for the current graph with id %d', self.num_channels_last_conv, self.graph_id if self.graph_id is not None else -1)
    ...
    def run_node(self, n: torch.fx.Node):
        ...
        with ir.IRNode.current_origins(origins), self.set_current_node(n), V.set_current_node(n):
            if n.op == 'call_function' and n.target is not operator.getitem and fallback_node_due_to_unsupported_type(n):
            ...
            if (is_output or is_input_for_as_strided) and isinstance(n.meta['val'], torch.Tensor):
                ...
                if dense and len(strides):
                    ...
                    if len(result.get_size()) == 4 and n in self.nodes_prefer_channels_last and (n.name not in self.user_visible_outputs) and (not is_input_for_as_strided):
    ...
    def get_output_names(self):
        return [node.get_name() for node in self.graph_outputs if not isinstance(node, ir.NoneAsConstantBuffer) and (not isinstance(node, ir.ShapeAsConstantBuffer))]

    def is_unspec_arg(self, name: str):
        return name in self.graph_inputs.keys() and self.graph_inputs[name].get_numel() == 1 and (self.graph_inputs[name].get_device().type == 'cpu')