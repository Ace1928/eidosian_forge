...
import torch
import torchaudio
def play_audio(waveform: torch.Tensor, sample_rate: Optional[float], device: Optional[str]=None) -> None:
    ...
    if num_channels > 2:
        warnings.warn(f'Expected up to 2 channels, got {num_channels} channels instead. Only the first 2 channels will be played.', stacklevel=2)
