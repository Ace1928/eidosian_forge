...
class ImageCompressorApp(tk.Tk):
    ...
    async def async_compress_and_store_image(self) -> None:
        ...
        try:
            ...
            encrypted_data = await encrypt(
                compressed_data, self.cipher_suite
            )  # Corrected usage
    ...
    async def async_decompress_and_show(self) -> None:
        ...
        try:
            ...
            decrypted_data = await decrypt(encrypted_data, self.cipher_suite)
            ...
            self.after(
                0, lambda: setattr(self.decompressed_img_label, "image", tk_img)
            )  # Keep a reference
            self.after(
