...
class TestBlockStorageVolumeManage(TestBlockStorageManage):
    ...
    def setUp(self):
        super().setUp()
        self.volumes_mock.list_manageable.return_value = self.volume_manage_list
        self.cmd = block_storage_manage.BlockStorageManageVolumes(self.app, None)
    ...
    def test_block_storage_volume_manage_list(self):
        ...
        for volume_record in self.volume_manage_list:
            manage_details = (volume_record.reference, volume_record.size, volume_record.safe_to_manage)
            datalist.append(manage_details)
        datalist = tuple(datalist)
        self.assertEqual(expected_columns, columns)
        self.assertEqual(datalist, tuple(data))
        self.volumes_mock.list_manageable.assert_called_with(host='fake_host', detailed=False, marker=None, limit=None, offset=None, sort=None, cluster=None)
