...
class LatexPreprocessor(Preprocessor):
    ...
    """
    date = Unicode(None, help='Date of the LaTeX document', allow_none=True).tag(config=True)
    title = Unicode(None, help='Title of the LaTeX document', allow_none=True).tag(config=True)
    author_names = List(Unicode(), default_value=None, help='Author names to list in the LaTeX document', allow_none=True).tag(config=True)
