...
class Aliases(ModuleAnalysis):
    ...
    def visit_Set(self, node):
        ...
        """
        if node.elts:
            elts_aliases = {ContainerOf({alias for elt in node.elts for alias in self.visit(elt)})}
        else:
            elts_aliases = None
    ...
    def visit_Call(self, node):
        ...
        if isinstance(f, ast.Attribute) and f.attr == 'partial':
            ...
            for value in return_alias:
                if isinstance(value, (ContainerOf, ast.FunctionDef, Intrinsic)):
    ...
    def visit_Subscript(self, node):
        ...
        if isinstance(node.slice, ast.Tuple):
            ...
            for alias in value_aliases:
                ...
                elif isinstance(getattr(alias, 'ctx', None), (ast.Param, ast.Store)):
    ...
    def visit_FunctionDef(self, node):
        ...
        self.aliases.update(((k, {v}) for k, v in self.global_declarations.items()))
        ...
        if Aliases.RetId in self.aliases:
            ...
            def parametrize(exp):
                if isinstance(exp, (ast.Constant, Intrinsic, ast.FunctionDef)):
                    ...
                    return lambda _: self.get_unbound_value_set()
            return_aliases = [parametrize(ret_alias) for ret_alias in self.aliases[Aliases.RetId]]

            def merge_return_aliases(args):
                return {ra for return_alias in return_aliases for ra in return_alias(args)}
    ...
    def visit_For(self, node):
        ...
        if all((isinstance(x, ContainerOf) for x in iter_aliases)):
            target_aliases = set().union(*[iter_alias.containees for iter_alias in iter_aliases])
        else:
