...
class SQLExpressionGenerator:
    ...
    def where(self, condition: ColumnExpr, table: str) -> Iterable[Tuple[bool, str]]:
        ...
        """
        assert_or_throw(not is_agg(condition), lambda: ValueError(f'{condition} has aggregation functions'))
        cond = self.generate(condition.alias(''))
        yield (False, 'SELECT * FROM')
        yield (True, table)
        yield (False, f'WHERE {cond}')

    def select(self, columns: SelectColumns, table: str, where: Optional[ColumnExpr]=None, having: Optional[ColumnExpr]=None) -> Iterable[Tuple[bool, str]]:
        ...
        def _where() -> str:
            if where is None:
                return ''
            assert_or_throw(not is_agg(where), lambda: ValueError(f'{where} has aggregation functions'))
        ...
        if len(columns.literals) == 0:
            ...
            no_lit = [x for x in columns.all_cols if not isinstance(x, _LiteralColumnExpr)]
            sub = self.select(SelectColumns(*no_lit), table, where=where, having=having)
            names = [self.generate(x) if isinstance(x, _LiteralColumnExpr) else x.output_name for x in columns.all_cols]
            expr = ', '.join(names)
