...
def relu_grad(gradients: _atypes.TensorFuzzingAnnotation[TV_ReluGrad_T], features: _atypes.TensorFuzzingAnnotation[TV_ReluGrad_T], name=None) -> _atypes.TensorFuzzingAnnotation[TV_ReluGrad_T]:
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'ReluGrad', name, gradients, features)
            return _result
        ...
        try:
            ...
            pass
    _, _, _op, _outputs = _op_def_library._apply_op_helper('ReluGrad', gradients=gradients, features=features, name=name)
    _result = _outputs[:]
