...
class GraphQLSession(HTTPTransport):
    ...
    def __init__(self, url: str, auth: Optional[Union[Tuple[str, str], Callable]]=None, use_json: bool=False, timeout: Optional[Union[int, float]]=None, proxies: Optional[Dict[str, str]]=None, **kwargs: Any) -> None:
    ...
    def execute(self, document: ast.Node, variable_values: Optional[Dict]=None, timeout: Optional[Union[int, float]]=None) -> ExecutionResult:
        query_str = print_ast(document)
        payload = {'query': query_str, 'variables': variable_values or {}}
        data_key = 'json' if self.use_json else 'data'
        post_args = {'headers': self.headers, 'cookies': self.cookies, 'timeout': timeout or self.default_timeout, data_key: payload}
