...
def standardize_lag_order(order, title=None):
    ...
    if order.ndim == 2 and order.shape[1] == 1:
        ...
        raise ValueError('Invalid %s. Must be an integer or 1-dimensional array-like object (e.g. list, ndarray, etc.). Got %s.' % (title, order))
    if order.ndim == 0:
        order = order.item()
    elif len(order) == 0:
        order = 0
        ...
        if has_zeros or has_multiple_ones:
            if has_gt_one:
                raise ValueError('Invalid %s. Appears to be a boolean list (since it contains a 0 element and/or multiple elements) but also contains elements greater than 1 like a list of lag orders.' % title)
