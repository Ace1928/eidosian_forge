...
def validate_record_geometry(self, record):
    """Compares the record's geometry to the collection's schema.
        ...
        """
    if self.driver == 'ESRI Shapefile' and 'Point' not in record['geometry']['type']:
        return record['geometry']['type'].lstrip('Multi') == self.schema['geometry'].lstrip('3D ').lstrip('Multi')
    else:
