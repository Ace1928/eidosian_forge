...
def test_default_environ_external_credentials_with_user_and_default_scopes_and_quota_project_id(get_project_id, monkeypatch, tmpdir):
    config_file = tmpdir.join('config.json')
    config_file.write(json.dumps(IDENTITY_POOL_DATA))
    monkeypatch.setenv(environment_vars.CREDENTIALS, str(config_file))
    credentials, project_id = _default.default(scopes=['https://www.google.com/calendar/feeds'], default_scopes=['https://www.googleapis.com/auth/cloud-platform'], quota_project_id='project-foo')
