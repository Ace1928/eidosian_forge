...
class DatadogLogsLoader(BaseLoader):
    ...
    def __init__(self, query: str, api_key: str, app_key: str, from_time: Optional[int]=None, to_time: Optional[int]=None, limit: int=100) -> None:
        ...
        try:
            ...
            raise ImportError('Could not import datadog_api_client python package. Please install it with `pip install datadog_api_client`.') from ex
        self.query = query
    ...
    def load(self) -> List[Document]:
        ...
        body = LogsListRequest(filter=LogsQueryFilter(query=self.query, _from=_from, to=f'{(self.to_time if self.to_time is not None else now_timestamp)}'), sort=LogsSort.TIMESTAMP_ASCENDING, page=LogsListRequestPage(limit=self.limit))
