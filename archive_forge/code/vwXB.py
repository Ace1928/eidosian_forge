...
if __name__ == "__main__":
    ...
    # Create a substantially larger multidimensional training dataset
    dataset = torch.normal(0, 1, size=(10000, 2))
    targets = torch.normal(0, 1, size=(10000, 3))

    # Train the KANWrapper with detailed logging and metrics
    learning_rate = 0.001
    epochs = 200
    for epoch in range(epochs):
        kan_wrapper.train(dataset, targets, learning_rate=learning_rate, epochs=1)

        # Calculate additional metrics
        y_pred = kan_wrapper.forward(dataset)
        rmse = torch.sqrt(torch.mean((y_pred - targets) ** 2)).item()
        mape = torch.mean(torch.abs((targets - y_pred) / targets)).item() * 100
        variance = torch.var(y_pred).item()
        std_dev = torch.std(y_pred).item()

        # Log metrics
        logging.info(
            f"Epoch {epoch+1}/{epochs}, RMSE: {rmse}, MAPE: {mape}%, Variance: {variance}, Std Dev: {std_dev}"
        )

        # Plot and save results
        plt.figure()
        plt.plot(targets.detach().numpy(), label="Targets")
        plt.plot(y_pred.detach().numpy(), label="Predictions")
        plt.legend()
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        plt.savefig(f"outputs/kan_model_epoch_{epoch+1}_{timestamp}.png")
        plt.close()

    # Save the trained model
    kan_wrapper.save("kan_model_trained.pth")

    # Load the model
    loaded_kan_wrapper = KANWrapper.load("kan_model_trained.pth")
    ...
    # Plot final results
    y_pred = loaded_kan_wrapper.forward(dataset)
    loaded_kan_wrapper.plot_results(dataset, targets, y_pred)
