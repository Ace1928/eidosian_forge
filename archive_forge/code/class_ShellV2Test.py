...
class ShellV2Test(testtools.TestCase):
    ...
    stores_info_response = {'stores': [{'default': 'true', 'id': 'ceph1', 'description': 'RBD backend for glance.'}, {'id': 'file2', 'description': 'Filesystem backend for glance.'}, {'id': 'file1', 'description': 'Filesystem backend for gkance.'}, {'id': 'ceph2', 'description': 'RBD backend for glance.'}]}
    stores_info_detail_response = {'stores': [{'default': 'true', 'id': 'ceph1', 'type': 'rbd', 'description': 'RBD backend for glance.', 'properties': {'pool': 'pool1', 'chunk_size': '4'}}, {'id': 'file2', 'type': 'file', 'description': 'Filesystem backend for glance.', 'properties': {}}, {'id': 'file1', 'type': 'file', 'description': 'Filesystem backend for gkance.', 'properties': {}}, {'id': 'ceph2', 'type': 'rbd', 'description': 'RBD backend for glance.', 'properties': {'pool': 'pool2', 'chunk_size': '4'}}]}

    def test_do_stores_info(self):
        args = self._make_args({'detail': False})
        with mock.patch.object(self.gc.images, 'get_stores_info') as mocked_list:
    ...
    def test_image_create_missing_container_format(self, __):
        e = self.assertRaises(exc.CommandError, self._run_command, '--os-image-api-version 2 image-create ' + '--file fake_src --disk-format qcow2')
        self.assertEqual('error: Must provide --container-format when using --file.', e.message)
    ...
    def test_image_create_missing_disk_format_stdin_data(self, __):
        self.mock_get_data_file.return_value = io.StringIO()
        e = self.assertRaises(exc.CommandError, self._run_command, '--os-image-api-version 2 image-create --container-format bare')
    ...
    def test_create_via_import_glance_direct_missing_disk_format(self, __):
        e = self.assertRaises(exc.CommandError, self._run_command, '--os-image-api-version 2 image-create-via-import --file fake_src --container-format bare')
        self.assertEqual('error: Must provide --disk-format when using --file.', e.message)

    @mock.patch('sys.stderr')
    def test_create_via_import_glance_direct_missing_container_format(self, __):
        e = self.assertRaises(exc.CommandError, self._run_command, '--os-image-api-version 2 image-create-via-import --file fake_src --disk-format qcow2')
        self.assertEqual('error: Must provide --container-format when using --file.', e.message)

    @mock.patch('sys.stderr')
    def test_create_via_import_web_download_missing_disk_format(self, __):
        e = self.assertRaises(exc.CommandError, self._run_command, '--os-image-api-version 2 image-create-via-import ' + '--import-method web-download ' + '--uri fake_uri --container-format bare')
        self.assertEqual('error: Must provide --disk-format when using --uri.', e.message)

    @mock.patch('sys.stderr')
    def test_create_via_import_web_download_missing_container_format(self, __):
        e = self.assertRaises(exc.CommandError, self._run_command, '--os-image-api-version 2 image-create-via-import --import-method web-download --uri fake_uri --disk-format qcow2')
        self.assertEqual('error: Must provide --container-format when using --uri.', e.message)

    def test_do_image_list(self):
        input = {'limit': None, 'page_size': 18, 'visibility': True, 'member_status': 'Fake', 'owner': 'test', 'checksum': 'fake_checksum', 'tag': 'fake tag', 'properties': [], 'sort_key': ['name', 'id'], 'sort_dir': ['desc', 'asc'], 'sort': None, 'verbose': False, 'include_stores': False, 'os_hash_value': None, 'os_hidden': False}
    ...
    def test_do_image_list_verbose(self):
        input = {'limit': None, 'page_size': 18, 'visibility': True, 'member_status': 'Fake', 'owner': 'test', 'checksum': 'fake_checksum', 'tag': 'fake tag', 'properties': [], 'sort_key': ['name', 'id'], 'sort_dir': ['desc', 'asc'], 'sort': None, 'verbose': True, 'include_stores': False, 'os_hash_value': None, 'os_hidden': False}
        args = self._make_args(input)
        with mock.patch.object(self.gc.images, 'list') as mocked_list:
            mocked_list.return_value = {}
            test_shell.do_image_list(self.gc, args)
            utils.print_list.assert_called_once_with({}, ['ID', 'Name', 'Disk_format', 'Container_format', 'Size', 'Status', 'Owner'])
    ...
    def test_do_image_list_with_include_stores_true(self):
        input = {'limit': None, 'page_size': 18, 'visibility': True, 'member_status': 'Fake', 'owner': 'test', 'checksum': 'fake_checksum', 'tag': 'fake tag', 'properties': [], 'sort_key': ['name', 'id'], 'sort_dir': ['desc', 'asc'], 'sort': None, 'verbose': False, 'include_stores': True, 'os_hash_value': None, 'os_hidden': False}
        args = self._make_args(input)
        with mock.patch.object(self.gc.images, 'list') as mocked_list:
            mocked_list.return_value = {}
            test_shell.do_image_list(self.gc, args)
            utils.print_list.assert_called_once_with({}, ['ID', 'Name', 'Stores'])

    def test_do_image_list_verbose_with_include_stores_true(self):
        input = {'limit': None, 'page_size': 18, 'visibility': True, 'member_status': 'Fake', 'owner': 'test', 'checksum': 'fake_checksum', 'tag': 'fake tag', 'properties': [], 'sort_key': ['name', 'id'], 'sort_dir': ['desc', 'asc'], 'sort': None, 'verbose': True, 'include_stores': True, 'os_hash_value': None, 'os_hidden': False}
        args = self._make_args(input)
        with mock.patch.object(self.gc.images, 'list') as mocked_list:
            mocked_list.return_value = {}
            test_shell.do_image_list(self.gc, args)
            utils.print_list.assert_called_once_with({}, ['ID', 'Name', 'Disk_format', 'Container_format', 'Size', 'Status', 'Owner', 'Stores'])
    ...
    def test_do_image_list_with_property_filter(self):
        input = {'limit': None, 'page_size': 1, 'visibility': True, 'member_status': 'Fake', 'owner': 'test', 'checksum': 'fake_checksum', 'tag': 'fake tag', 'properties': ['os_distro=NixOS', 'architecture=x86_64'], 'sort_key': ['name'], 'sort_dir': ['desc'], 'sort': None, 'verbose': False, 'include_stores': False, 'os_hash_value': None, 'os_hidden': False}
        args = self._make_args(input)
        with mock.patch.object(self.gc.images, 'list') as mocked_list:
            mocked_list.return_value = {}
            test_shell.do_image_list(self.gc, args)
            exp_img_filters = {'owner': 'test', 'member_status': 'Fake', 'visibility': True, 'checksum': 'fake_checksum', 'tag': 'fake tag', 'os_distro': 'NixOS', 'architecture': 'x86_64', 'os_hidden': False}
            mocked_list.assert_called_once_with(page_size=1, sort_key=['name'], sort_dir=['desc'], filters=exp_img_filters)
            utils.print_list.assert_called_once_with({}, ['ID', 'Name'])

    def test_do_image_show_human_readable(self):
        args = self._make_args({'id': 'pass', 'page_size': 18, 'human_readable': True, 'max_column_width': 120})
        with mock.patch.object(self.gc.images, 'get') as mocked_list:
            ...
            utils.print_dict.assert_called_once_with({'id': 'pass', 'size': '1kB'}, max_column_width=120)
    ...
    def test_usage(self):
        with mock.patch.object(self.gc.info, 'get_usage') as mock_usage:
            mock_usage.return_value = {'quota1': {'limit': 10, 'usage': 0}, 'quota2': {'limit': 20, 'usage': 5}}
            test_shell.do_usage(self.gc, [])
            utils.print_dict_list.assert_called_once_with([{'quota': 'quota1', 'limit': 10, 'usage': 0}, {'quota': 'quota2', 'limit': 20, 'usage': 5}], ['Quota', 'Limit', 'Usage'])

    @mock.patch('sys.stdin', autospec=True)
    def test_do_image_create_no_user_props(self, mock_stdin):
        args = self._make_args({'name': 'IMG-01', 'disk_format': 'vhd', 'container_format': 'bare', 'file': None})
    ...
    def test_do_image_create_for_none_multi_hash(self, mock_stdin):
        ...
        with mock.patch.object(self.gc.images, 'create') as mocked_create:
            ...
            test_shell.do_image_create(self.gc, args)
            mocked_create.assert_called_once_with(name='IMG-01', disk_format='vhd', container_format='bare')
            utils.print_dict.assert_called_once_with({'id': 'pass', 'name': 'IMG-01', 'disk_format': 'vhd', 'container_format': 'bare', 'os_hash_algo': None, 'os_hash_value': None})
    ...
    def test_neg_do_image_create_invalid_store(self, mock_utils_exit):
        ...
        with mock.patch.object(self.gc.images, 'get_stores_info') as mock_stores_info:
            ...
            try:
                ...
                pass
        mock_utils_exit.assert_called_once_with(expected_msg)
    base_args = {'name': 'Mortimer', 'disk_format': 'raw', 'container_format': 'bare', 'progress': False, 'file': None, 'uri': None, 'remote_region': None, 'import_method': None}
    import_info_response = {'import-methods': {'type': 'array', 'description': 'Import methods available.', 'value': ['glance-direct', 'web-download', 'copy-image', 'glance-download']}}
    ...
    @mock.patch('os.access')
    @mock.patch('sys.stdin', autospec=True)
    def test_neg_image_create_via_import_no_method_with_file_and_stdin(self, mock_stdin, mock_access, mock_utils_exit):
        expected_msg = 'You cannot use both --file and stdin with the glance-direct import method.'
    ...
    @mock.patch('glanceclient.common.utils.exit')
    def test_neg_image_create_via_import_copy_image(self, mock_utils_exit):
        expected_msg = "Import method 'copy-image' cannot be used while creating the image."
        mock_utils_exit.side_effect = self._mock_utils_exit
        my_args = self.base_args.copy()
        my_args.update({'id': 'IMG-01', 'import_method': 'copy-image'})
        args = self._make_args(my_args)
        with mock.patch.object(self.gc.images, 'get_import_info') as mocked_info:
            mocked_info.return_value = self.import_info_response
            try:
    ...
    @mock.patch('glanceclient.common.utils.exit')
    @mock.patch('sys.stdin', autospec=True)
    def test_neg_image_create_via_import_all_stores_without_file(self, mock_stdin, mock_utils_exit):
        expected_msg = '--all-stores option should only be provided with --file option or stdin for the glance-direct import method.'
        mock_utils_exit.side_effect = self._mock_utils_exit
        mock_stdin.isatty = lambda: True
        my_args = self.base_args.copy()
        my_args.update({'id': 'IMG-01', 'import_method': 'glance-direct', 'os_all_stores': True, 'disk_format': 'raw', 'container_format': 'bare'})
        args = self._make_args(my_args)
        with mock.patch.object(self.gc.images, 'get_import_info') as mocked_info:
            mocked_info.return_value = self.import_info_response
            try:
    ...
    @mock.patch('os.access')
    @mock.patch('sys.stdin', autospec=True)
    def test_neg_image_create_via_import_no_file_and_stdin_with_store(self, mock_stdin, mock_access, mock_utils_exit):
        expected_msg = '--store option should only be provided with --file option or stdin for the glance-direct import method.'
        my_args = self.base_args.copy()
    ...
    @mock.patch('os.access')
    @mock.patch('sys.stdin', autospec=True)
    def test_neg_image_create_via_import_invalid_store(self, mock_stdin, mock_access, mock_utils_exit):
        expected_msg = "Store 'dummy' is not valid for this cloud. Valid values can be retrieved with stores-info command."
        my_args = self.base_args.copy()
        my_args['import_method'] = 'glance-direct'
        my_args['store'] = 'dummy'
    ...
    @mock.patch('glanceclient.common.utils.exit')
    @mock.patch('sys.stdin', autospec=True)
    def test_neg_image_create_via_import_no_method_passing_uri(self, mock_stdin, mock_utils_exit):
        expected_msg = 'You cannot use --uri without specifying an import method.'
        my_args = self.base_args.copy()
    ...
    @mock.patch('glanceclient.common.utils.exit')
    @mock.patch('sys.stdin', autospec=True)
    def test_neg_image_create_via_import_glance_direct_no_data(self, mock_stdin, mock_utils_exit):
        expected_msg = 'You must specify a --file or provide data via stdin for the glance-direct import method.'
        my_args = self.base_args.copy()
        my_args['import_method'] = 'glance-direct'
        args = self._make_args(my_args)
        mock_stdin.isatty = lambda: True
    ...
    @mock.patch('glanceclient.common.utils.exit')
    @mock.patch('sys.stdin', autospec=True)
    def test_neg_image_create_via_import_glance_direct_with_uri(self, mock_stdin, mock_utils_exit):
        expected_msg = 'You cannot specify a --uri with the glance-direct import method.'
        my_args = self.base_args.copy()
        my_args['import_method'] = 'glance-direct'
    ...
    @mock.patch('os.access')
    @mock.patch('sys.stdin', autospec=True)
    def test_neg_image_create_via_import_glance_direct_with_file_and_uri(self, mock_stdin, mock_access, mock_utils_exit):
        expected_msg = 'You cannot specify a --uri with the glance-direct import method.'
        my_args = self.base_args.copy()
        my_args['import_method'] = 'glance-direct'
        my_args['uri'] = 'https://example.com/some/stuff'
    ...
    @mock.patch('glanceclient.common.utils.exit')
    @mock.patch('sys.stdin', autospec=True)
    def test_neg_image_create_via_import_glance_direct_with_data_and_uri(self, mock_stdin, mock_utils_exit):
        expected_msg = 'You cannot specify a --uri with the glance-direct import method.'
        my_args = self.base_args.copy()
        my_args['import_method'] = 'glance-direct'
        my_args['uri'] = 'https://example.com/some/stuff'
    ...
    @mock.patch('glanceclient.common.utils.exit')
    @mock.patch('sys.stdin', autospec=True)
    def test_neg_image_create_via_import_web_download_no_uri(self, mock_stdin, mock_utils_exit):
        expected_msg = "URI is required for web-download import method. Please use '--uri <uri>'."
        my_args = self.base_args.copy()
        my_args['import_method'] = 'web-download'
        args = self._make_args(my_args)
        mock_stdin.isatty = lambda: True
    ...
    @mock.patch('glanceclient.common.utils.exit')
    def test_neg_image_create_via_import_stores_without_uri(self, mock_utils_exit):
        expected_msg = '--stores option should only be provided with --uri option for the web-download import method.'
        mock_utils_exit.side_effect = self._mock_utils_exit
        my_args = self.base_args.copy()
        my_args.update({'id': 'IMG-01', 'import_method': 'web-download', 'stores': 'file1,file2', 'disk_format': 'raw', 'container_format': 'bare'})
        args = self._make_args(my_args)
        with mock.patch.object(self.gc.images, 'get_import_info') as mocked_info:
            with mock.patch.object(self.gc.images, 'get_stores_info') as mocked_stores_info:
                mocked_stores_info.return_value = self.stores_info_response
    ...
    @mock.patch('glanceclient.common.utils.exit')
    @mock.patch('sys.stdin', autospec=True)
    def test_neg_image_create_via_import_web_download_no_uri_with_file(self, mock_stdin, mock_utils_exit):
        expected_msg = "URI is required for web-download import method. Please use '--uri <uri>'."
        my_args = self.base_args.copy()
        my_args['import_method'] = 'web-download'
        my_args['file'] = 'my.browncow'
    ...
    @mock.patch('glanceclient.common.utils.exit')
    @mock.patch('sys.stdin', autospec=True)
    def test_neg_image_create_via_import_web_download_no_uri_with_data(self, mock_stdin, mock_utils_exit):
        expected_msg = "URI is required for web-download import method. Please use '--uri <uri>'."
        my_args = self.base_args.copy()
        my_args['import_method'] = 'web-download'
        my_args['file'] = 'my.browncow'
    ...
    @mock.patch('glanceclient.common.utils.exit')
    @mock.patch('sys.stdin', autospec=True)
    def test_neg_image_create_via_import_web_download_with_data_and_uri(self, mock_stdin, mock_utils_exit):
        expected_msg = 'You cannot pass data via stdin with the web-download import method.'
        my_args = self.base_args.copy()
        my_args['import_method'] = 'web-download'
        my_args['uri'] = 'https://example.com/some/stuff'
        args = self._make_args(my_args)
    ...
    @mock.patch('glanceclient.common.utils.exit')
    @mock.patch('sys.stdin', autospec=True)
    def test_neg_image_create_via_import_glance_download_no_region_and_id(self, mock_stdin, mock_utils_exit):
        expected_msg = 'REMOTE GlANCE REGION and REMOTE IMAGE ID are required for glance-download import method. Please use --remote-region <region> and --remote-image-id <remote-image-id>.'
        my_args = self.base_args.copy()
    ...
    @mock.patch('glanceclient.common.utils.exit')
    @mock.patch('sys.stdin', autospec=True)
    def test_neg_image_create_via_import_glance_download_with_uri(self, mock_stdin, mock_utils_exit):
        expected_msg = 'You cannot specify a --uri with the glance-download import method.'
        my_args = self.base_args.copy()
        my_args['import_method'] = 'glance-download'
    ...
    @mock.patch('glanceclient.common.utils.exit')
    @mock.patch('sys.stdin', autospec=True)
    def test_neg_image_create_via_import_glance_download_with_file(self, mock_stdin, mock_utils_exit):
        expected_msg = 'You cannot specify a --file with the glance-download import method.'
        my_args = self.base_args.copy()
        my_args['import_method'] = 'glance-download'
        my_args['remote_region'] = 'REGION2'
        my_args['remote_image_id'] = 'IMG2'
    ...
    @mock.patch('glanceclient.common.utils.exit')
    @mock.patch('sys.stdin', autospec=True)
    def test_neg_image_create_via_import_bad_method(self, mock_stdin, mock_utils_exit):
        expected_msg = "Import method 'swift-party-time' is not valid for this cloud. Valid values can be retrieved with import-info command."
        my_args = self.base_args.copy()
    ...
    @mock.patch('glanceclient.common.utils.exit')
    @mock.patch('sys.stdin', autospec=True)
    def test_neg_image_create_via_import_no_method_with_data_and_method_NA(self, mock_stdin, mock_utils_exit):
    ...
    @mock.patch('sys.stdin', autospec=True)
    def test_image_create_via_import_no_method_with_stdin(self, mock_stdin, mock_do_stage, mock_do_import):
        ...
        with mock.patch.object(self.gc.images, 'create') as mocked_create:
            with mock.patch.object(self.gc.images, 'get') as mocked_get:
                with mock.patch.object(self.gc.images, 'get_import_info') as mocked_info:
                    ...
                    utils.print_dict.assert_called_with({'id': 'via-stdin', 'name': 'Mortimer', 'disk_format': 'raw', 'container_format': 'bare'})
    ...
    @mock.patch('glanceclient.v2.shell.do_image_stage')
    @mock.patch('sys.stdin', autospec=True)
    def test_do_image_create_via_import_with_no_method_no_data(self, mock_stdin, mock_do_image_stage, mock_do_image_import):
        """Create an image record without calling do_stage or do_import"""
        img_create_args = {'name': 'IMG-11', 'os_architecture': 'powerpc', 'id': 'watch-out-for-ossn-0075', 'progress': False}
        client_args = {'import_method': None, 'file': None, 'uri': None}
        ...
        with mock.patch.object(self.gc.images, 'create') as mocked_create:
            with mock.patch.object(self.gc.images, 'get') as mocked_get:
                with mock.patch.object(self.gc.images, 'get_import_info') as mocked_info:
                    ...
                    utils.print_dict.assert_called_with({'name': 'IMG-11', 'os_architecture': 'powerpc', 'id': 'watch-out-for-ossn-0075'})

    @mock.patch('glanceclient.v2.shell.do_image_import')
    @mock.patch('glanceclient.v2.shell.do_image_stage')
    @mock.patch('sys.stdin', autospec=True)
    def test_do_image_create_via_import_with_web_download(self, mock_stdin, mock_do_image_stage, mock_do_image_import):
        temp_args = {'name': 'IMG-01', 'disk_format': 'vhd', 'container_format': 'bare', 'uri': 'http://example.com/image.qcow', 'import_method': 'web-download', 'progress': False}
        args = self._make_args(temp_args)
        with mock.patch.object(self.gc.images, 'create') as mocked_create:
            with mock.patch.object(self.gc.images, 'get') as mocked_get:
                with mock.patch.object(self.gc.images, 'get_import_info') as mocked_info:
    ...
    def test_do_image_update_revert_hide_image(self):
        ...
        with mock.patch.object(self.gc.images, 'update') as mocked_update:
            ...
            mocked_update.return_value = expect_image
            test_shell.do_image_update(self.gc, args)
            mocked_update.assert_called_once_with('pass', None, os_hidden='false')
            utils.print_dict.assert_called_once_with({'id': 'pass', 'name': 'IMG-01', 'disk_format': 'vhd', 'container_format': 'bare', 'os_hidden': False})

    def test_do_image_update_with_user_props(self):
        args = self._make_args({'id': 'pass', 'name': 'IMG-01', 'property': ['myprop=myval']})
        with mock.patch.object(self.gc.images, 'update') as mocked_update:
            ignore_fields = ['self', 'access', 'file', 'schema']
            expect_image = dict([(field, field) for field in ignore_fields])
            expect_image['id'] = 'pass'
            ...
            mocked_update.return_value = expect_image
            test_shell.do_image_update(self.gc, args)
            mocked_update.assert_called_once_with('pass', None, name='IMG-01', myprop='myval')
            utils.print_dict.assert_called_once_with({'id': 'pass', 'name': 'IMG-01', 'myprop': 'myval'})

    def test_do_image_update_with_remove_props(self):
        args = self._make_args({'id': 'pass', 'name': 'IMG-01', 'disk_format': 'vhd', 'remove-property': ['container_format']})
        with mock.patch.object(self.gc.images, 'update') as mocked_update:
            ignore_fields = ['self', 'access', 'file', 'schema']
            expect_image = dict([(field, field) for field in ignore_fields])
            expect_image['id'] = 'pass'
            ...
            mocked_update.return_value = expect_image
            test_shell.do_image_update(self.gc, args)
            mocked_update.assert_called_once_with('pass', ['container_format'], name='IMG-01', disk_format='vhd')
            utils.print_dict.assert_called_once_with({'id': 'pass', 'name': 'IMG-01', 'disk_format': 'vhd'})
    ...
    def test_image_upload(self):
        args = self._make_args({'id': 'IMG-01', 'file': 'test', 'size': 1024, 'progress': False})
        with mock.patch.object(self.gc.images, 'upload') as mocked_upload:
            ...
            mocked_upload.assert_called_once_with('IMG-01', 'testfile', 1024, backend=None)
    ...
    def test_neg_image_import_not_available(self, mock_utils_exit):
        ...
        mock_utils_exit.side_effect = self._mock_utils_exit
        args = self._make_args({'id': 'IMG-01', 'import_method': 'smarty-pants', 'uri': None})
    ...
    def test_neg_image_import_bad_method(self, mock_utils_exit):
        expected_msg = "Import method 'smarty-pants' is not valid for this cloud. Valid values can be retrieved with import-info command."
        mock_utils_exit.side_effect = self._mock_utils_exit
        args = self._make_args({'id': 'IMG-01', 'import_method': 'smarty-pants', 'uri': None})
        with mock.patch.object(self.gc.images, 'get_import_info') as mocked_info:
            mocked_info.return_value = self.import_info_response
    ...
    @mock.patch('glanceclient.common.utils.exit')
    def test_neg_image_import_web_download_image_not_queued_status(self, mock_utils_exit):
        expected_msg = "The 'web-download' import method can only be applied to an image in status 'queued'"
        mock_utils_exit.side_effect = self._mock_utils_exit
    ...
    @mock.patch('glanceclient.common.utils.exit')
    def test_neg_image_import_image_no_container_format(self, mock_utils_exit):
        expected_msg = "The 'container_format' and 'disk_format' properties must be set on an image before it can be imported."
        mock_utils_exit.side_effect = self._mock_utils_exit
        args = self._make_args({'id': 'IMG-01', 'import_method': 'web-download', 'uri': 'http://joes-image-shack.com/funky.qcow2'})
        with mock.patch.object(self.gc.images, 'get_import_info') as mocked_info:
            with mock.patch.object(self.gc.images, 'get') as mocked_get:
                mocked_get.return_value = {'status': 'uploading', 'disk_format': 'raw'}
                mocked_info.return_value = self.import_info_response
                try:
                    test_shell.do_image_import(self.gc, args)
                    self.fail('utils.exit should have been called')
    ...
    def test_image_import_web_download(self):
        args = self._make_args({'id': 'IMG-01', 'uri': 'http://example.com/image.qcow', 'import_method': 'web-download'})
        with mock.patch.object(self.gc.images, 'image_import') as mock_import:
            with mock.patch.object(self.gc.images, 'get') as mocked_get:
                with mock.patch.object(self.gc.images, 'get_import_info') as mocked_info:
                    mocked_get.return_value = {'status': 'queued', 'container_format': 'bare', 'disk_format': 'raw'}
                    ...
                    mock_import.return_value = None
                    test_shell.do_image_import(self.gc, args)
                    mock_import.assert_called_once_with('IMG-01', 'web-download', uri='http://example.com/image.qcow', remote_region=None, remote_image_id=None, remote_service_interface=None, all_stores=None, allow_failure=True, backend=None, stores=None)

    def test_image_import_glance_download(self):
        args = self._make_args({'id': 'IMG-01', 'uri': None, 'remote_region': 'REGION2', 'remote_image_id': 'IMG-02', 'import_method': 'glance-download', 'remote_service_interface': 'public'})
        with mock.patch.object(self.gc.images, 'image_import') as mock_import:
            with mock.patch.object(self.gc.images, 'get') as mocked_get:
                with mock.patch.object(self.gc.images, 'get_import_info') as mocked_info:
                    mocked_get.return_value = {'status': 'queued', 'container_format': 'bare', 'disk_format': 'raw'}
                    ...
                    mock_import.return_value = None
                    test_shell.do_image_import(self.gc, args)
                    mock_import.assert_called_once_with('IMG-01', 'glance-download', uri=None, remote_region='REGION2', remote_image_id='IMG-02', remote_service_interface='public', all_stores=None, allow_failure=True, backend=None, stores=None)

    @mock.patch('glanceclient.common.utils.exit')
    def test_image_import_neg_no_glance_download_with_remote_region(self, mock_utils_exit):
        expected_msg = "Import method should be 'glance-download' if REMOTE REGION is provided."
        my_args = self.base_args.copy()
        my_args['id'] = 'IMG-01'
        my_args['remote_region'] = 'REGION2'
        my_args['import_method'] = 'web-download'
    ...
    @mock.patch('glanceclient.common.utils.print_image')
    @mock.patch('glanceclient.v2.shell._validate_backend')
    def test_image_import_copy_image(self, mocked_utils_print_image, msvb):
        args = self._make_args({'id': 'IMG-02', 'uri': None, 'import_method': 'copy-image', 'from_create': False, 'stores': 'file1,file2'})
        with mock.patch.object(self.gc.images, 'image_import') as mock_import:
            with mock.patch.object(self.gc.images, 'get') as mocked_get:
                with mock.patch.object(self.gc.images, 'get_import_info') as mocked_info:
                    mocked_get.return_value = {'status': 'active', 'container_format': 'bare', 'disk_format': 'raw'}
                    mocked_info.return_value = self.import_info_response
                    mock_import.return_value = None
                    test_shell.do_image_import(self.gc, args)
                    mock_import.assert_called_once_with('IMG-02', 'copy-image', uri=None, remote_region=None, remote_image_id=None, remote_service_interface=None, all_stores=None, allow_failure=True, stores=['file1', 'file2'], backend=None)
    ...
    @mock.patch('glanceclient.common.utils.exit')
    def test_neg_image_import_stores_all_stores_not_specified(self, mock_utils_exit):
        expected_msg = "Provide either --stores or --all-stores for 'copy-image' import method."
        mock_utils_exit.side_effect = self._mock_utils_exit
        my_args = self.base_args.copy()
        my_args.update({'id': 'IMG-01', 'import_method': 'copy-image', 'disk_format': 'raw', 'container_format': 'bare'})
        args = self._make_args(my_args)
        with mock.patch.object(self.gc.images, 'get_import_info') as mocked_info:
            mocked_info.return_value = self.import_info_response
            try:
    ...
    def test_image_download(self):
        args = self._make_args({'id': 'IMG-01', 'file': 'test', 'progress': True, 'allow_md5_fallback': False})
        with mock.patch.object(self.gc.images, 'data') as mocked_data, mock.patch.object(utils, '_extract_request_id'):
            mocked_data.return_value = utils.RequestIdProxy([c for c in 'abcdef'])
            test_shell.do_image_download(self.gc, args)
            mocked_data.assert_called_once_with('IMG-01', allow_md5_fallback=False)
        args.allow_md5_fallback = True
        with mock.patch.object(self.gc.images, 'data') as mocked_data, mock.patch.object(utils, '_extract_request_id'):
            mocked_data.return_value = utils.RequestIdProxy([c for c in 'abcdef'])
            test_shell.do_image_download(self.gc, args)
            mocked_data.assert_called_once_with('IMG-01', allow_md5_fallback=True)
    ...
    def test_do_image_delete_with_invalid_ids(self, mocked_print_err, mocked_utils_exit):
        args = argparse.Namespace(id=['image1', 'image2'])
        with mock.patch.object(self.gc.images, 'delete') as mocked_delete:
    ...
    @mock.patch.object(utils, 'exit')
    @mock.patch.object(utils, 'print_err')
    def test_do_image_delete_with_image_in_use(self, mocked_print_err, mocked_utils_exit):
        args = argparse.Namespace(id=['image1', 'image2'])
        with mock.patch.object(self.gc.images, 'delete') as mocked_delete:
    ...
    def test_do_member_create_with_few_arguments(self):
        ...
        self.assert_exits_with_msg(func=test_shell.do_member_create, func_args=args, err_msg=msg)
    ...
    def test_do_md_namespace_list_one_filter(self):
        args = self._make_args({'resource_types': ['OS::Compute::Aggregate'], 'visibility': None, 'page_size': None})
        with mock.patch.object(self.gc.metadefs_namespace, 'list') as mocked_list:
            expect_namespaces = [{'namespace': 'MyNamespace'}]
            mocked_list.return_value = expect_namespaces
            test_shell.do_md_namespace_list(self.gc, args)
            mocked_list.assert_called_once_with(filters={'resource_types': ['OS::Compute::Aggregate']})
            utils.print_list.assert_called_once_with(expect_namespaces, ['namespace'])
    ...
    def test_do_md_namespace_list_unknown_filter(self):
        args = self._make_args({'resource_type': None, 'visibility': None, 'some_arg': 'some_value', 'page_size': None})
        with mock.patch.object(self.gc.metadefs_namespace, 'list') as mocked_list:
            expect_namespaces = [{'namespace': 'MyNamespace'}]
            mocked_list.return_value = expect_namespaces
            test_shell.do_md_namespace_list(self.gc, args)
    ...
    def test_do_md_resource_type_deassociate(self):
        args = self._make_args({'namespace': 'MyNamespace', 'resource_type': 'MyResourceType'})
        with mock.patch.object(self.gc.metadefs_resource_type, 'deassociate') as mocked_deassociate:
            test_shell.do_md_resource_type_deassociate(self.gc, args)
            mocked_deassociate.assert_called_once_with('MyNamespace', 'MyResourceType')
    ...
    def test_do_md_property_create_invalid_schema(self):
        args = self._make_args({'namespace': 'MyNamespace', 'name': 'MyProperty', 'title': 'Title', 'type': 'boolean', 'schema': 'Invalid'})
        self.assertRaises(SystemExit, test_shell.do_md_property_create, self.gc, args)
    ...
    def test_do_md_object_update_invalid_schema(self):
        args = self._make_args({'namespace': 'MyNamespace', 'object': 'MyObject', 'name': 'MyObject', 'schema': 'Invalid'})
        self.assertRaises(SystemExit, test_shell.do_md_object_update, self.gc, args)

    def test_do_md_object_show(self):
        args = self._make_args({'namespace': 'MyNamespace', 'object': 'MyObject', 'max_column_width': 80})
        with mock.patch.object(self.gc.metadefs_object, 'get') as mocked_get:
            expect_object = {'namespace': 'MyNamespace', 'object': 'MyObject'}
    ...
    def test_do_md_namespace_objects_delete(self):
        args = self._make_args({'namespace': 'MyNamespace'})
        with mock.patch.object(self.gc.metadefs_object, 'delete_all') as mocked_delete_all:
    ...
    def test_do_md_object_list(self):
        ...
        with mock.patch.object(self.gc.metadefs_object, 'list') as mocked_list:
            ...
            mocked_list.assert_called_once_with('MyNamespace')
            utils.print_list.assert_called_once_with(expect_objects, ['name', 'description'], field_settings={'description': {'align': 'l', 'max_width': 50}})
    ...
    def test_do_md_tag_show(self):
        args = self._make_args({'namespace': 'MyNamespace', 'tag': 'MyTag', 'sort_dir': 'desc'})
        with mock.patch.object(self.gc.metadefs_tag, 'get') as mocked_get:
            expect_tag = {'namespace': 'MyNamespace', 'tag': 'MyTag'}
    ...
    def test_do_md_tag_delete(self):
        args = self._make_args({'namespace': 'MyNamespace', 'tag': 'MyTag'})
        with mock.patch.object(self.gc.metadefs_tag, 'delete') as mocked_delete:
    ...
    def test_do_md_namespace_tags_delete(self):
        args = self._make_args({'namespace': 'MyNamespace'})
        with mock.patch.object(self.gc.metadefs_tag, 'delete_all') as mocked_delete_all:
    ...
    def test_do_md_tag_create_multiple_with_append(self):
        args = self._make_args({'namespace': 'MyNamespace', 'delim': ',', 'names': 'MyTag1, MyTag2', 'append': True})
        with mock.patch.object(self.gc.metadefs_tag, 'create_multiple') as mocked_create_tags:
            expect_tags = [{'tags': [{'name': 'MyTag1'}, {'name': 'MyTag2'}]}]
            mocked_create_tags.return_value = expect_tags
            test_shell.do_md_tag_create_multiple(self.gc, args)
            mocked_create_tags.assert_called_once_with('MyNamespace', tags=['MyTag1', 'MyTag2'], append=True)
            utils.print_list.assert_called_once_with(expect_tags, ['name'], field_settings={'description': {'align': 'l', 'max_width': 50}})
    ...
    def test_do_cache_list_unsupported(self):
        self.assertRaises(exc.HTTPNotImplemented, self._test_do_cache_list, supported=False)
    ...
    def test_do_cache_queue_unsupported(self):
        with mock.patch('glanceclient.common.utils.print_err') as mock_print_err:
            ...
            mock_print_err.assert_called_once_with("'HTTP HTTPNotImplemented': Unable to queue image 'image1' for caching.")

    def test_do_cache_queue_forbidden(self):
        with mock.patch('glanceclient.common.utils.print_err') as mock_print_err:
            self._test_cache_queue(forbidden=True)
            mock_print_err.assert_called_once_with("You are not permitted to queue the image 'image1' for caching.")
    ...
    def test_do_cache_delete_unsupported(self):
        with mock.patch('glanceclient.common.utils.print_err') as mock_print_err:
            ...
            mock_print_err.assert_called_once_with("'HTTP HTTPNotImplemented': Unable to delete image 'image1' from cache.")
