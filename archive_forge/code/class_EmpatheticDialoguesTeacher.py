...
class EmpatheticDialoguesTeacher(FixedDialogTeacher):
    ...
    def __init__(self, opt, shared=None):
        ...
        self.datapath = os.path.join(self.opt['datapath'], 'empatheticdialogues', 'empatheticdialogues', base_datatype + '.csv')
        self.experiencer_side_only = opt.get('train_experiencer_only', DEFAULT_TRAIN_EXPERIENCER_ONLY) and base_datatype == 'train' or base_datatype != 'train'
        if not shared:
            print(f'[EmpatheticDialoguesTeacher] Only use experiencer side? {self.experiencer_side_only}, datatype: {self.datatype}')
        self.remove_political_convos = opt.get('remove_political_convos', DEFAULT_REMOVE_POLITICAL_CONVOS)
        if shared:
    ...
    def add_cmdline_args(cls, argparser):
        ...
        agent.add_argument('--train-experiencer-only', type='bool', default=DEFAULT_TRAIN_EXPERIENCER_ONLY, help='In the train set, only use Speaker (experiencer) utterances as text and Listener (responder) utterances as labels.')
        agent.add_argument('--remove-political-convos', type='bool', default=DEFAULT_REMOVE_POLITICAL_CONVOS, help='Remove all conversations containing an utterance marked as political')
    ...
    def _setup_data(self, base_datatype):
        ...
        for i in range(1, len(df)):
            ...
            if cparts[0] == sparts[0]:
                turn_idx += 1
                assert int(cparts[1]) + 1 == int(sparts[1]) and int(sparts[1]) == turn_idx
                ...
                if len(sparts) == 9:
                    if sparts[8] != '':
                        inline_label_candidates = [cand.replace('_comma_', ',').replace('_pipe_', '|') for cand in sparts[8].split('|')]
                    else:
                ...
                dialogue_parts = [contextt, label, prompt, sit, context_emb, cand_emb, ft_ctx, ft_cand, inline_label_candidates, is_political]
                if int(sparts[1]) % 2 == 0:
                ...
                turn_idx = 1
                self.data += self._select_dialogues_to_add(experiencer_text_dialogue, responder_text_dialogue)
                experiencer_text_dialogue = []
                responder_text_dialogue = []
        self.data += self._select_dialogues_to_add(experiencer_text_dialogue, responder_text_dialogue)
    ...
    def get(self, episode_idx, entry_idx=0):
        ...
        action = {'situation': ep_i[3], 'emotion': ep_i[2], 'text': ep_i[0], 'labels': [ep_i[1]], 'prepend_ctx': ep_i[6], 'prepend_cand': ep_i[7], 'deepmoji_ctx': ep_i[4], 'deepmoji_cand': ep_i[5], 'episode_done': episode_done, 'label_candidates': ep_i[8]}
        return action
