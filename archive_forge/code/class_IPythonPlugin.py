...
class IPythonPlugin(DistributedPluginBase):
    ...
    def __init__(self, plugin_args=None):
        ...
        valid_args = ('url_file', 'profile', 'cluster_id', 'context', 'debug', 'timeout', 'config', 'username', 'sshserver', 'sshkey', 'password', 'paramiko')
        self.client_args = {arg: plugin_args[arg] for arg in valid_args if arg in plugin_args}
        self.iparallel = None
    ...
    def run(self, graph, config, updatehash=False):
        ...
        try:
            ...
            raise ImportError('ipyparallel not found. Parallel execution will be unavailable') from e
        try:
    ...
    def _submit_job(self, node, updatehash=False):
        ...
        result_object = self.taskclient.load_balanced_view().apply(execute_task, pckld_node, node.config, updatehash)
        self._taskid += 1
