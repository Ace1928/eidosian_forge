...
class ChatFriendli(BaseChatModel, BaseFriendli):
    ...
    async def _astream(self, messages: List[BaseMessage], stop: Optional[List[str]]=None, run_manager: Optional[AsyncCallbackManagerForLLMRun]=None, **kwargs: Any) -> AsyncIterator[ChatGenerationChunk]:
        params = self._get_invocation_params(stop=stop, **kwargs)
        stream = await self.async_client.chat.completions.create(**get_chat_request(messages), stream=True, model=self.model, **params)
    ...
    def _generate(self, messages: List[BaseMessage], stop: Optional[List[str]]=None, run_manager: Optional[CallbackManagerForLLMRun]=None, **kwargs: Any) -> ChatResult:
        ...
        params = self._get_invocation_params(stop=stop, **kwargs)
        response = self.client.chat.completions.create(messages=[{'role': get_role(message), 'content': message.content} for message in messages], stream=False, model=self.model, **params)
    ...
    async def _agenerate(self, messages: List[BaseMessage], stop: Optional[List[str]]=None, run_manager: Optional[AsyncCallbackManagerForLLMRun]=None, **kwargs: Any) -> ChatResult:
        ...
        params = self._get_invocation_params(stop=stop, **kwargs)
        response = await self.async_client.chat.completions.create(messages=[{'role': get_role(message), 'content': message.content} for message in messages], stream=False, model=self.model, **params)
        message = AIMessage(content=response.choices[0].message.content)
        return ChatResult(generations=[ChatGeneration(message=message)])