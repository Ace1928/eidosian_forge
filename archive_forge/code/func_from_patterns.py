...
def from_patterns(cls, patterns: Dict[str, List[str]], allowed_extensions: Optional[List[str]]=None) -> 'DataFilesPatternsDict':
    ...
    for key, patterns_for_key in patterns.items():
        out[key] = DataFilesPatternsList.from_patterns(patterns_for_key, allowed_extensions=allowed_extensions) if not isinstance(patterns_for_key, DataFilesPatternsList) else patterns_for_key
    return out