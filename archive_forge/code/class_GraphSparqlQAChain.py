...
class GraphSparqlQAChain(Chain):
    ...
    *Security note*: Make sure that the database connection uses credentials
    ...
    def from_llm(cls, llm: BaseLanguageModel, *, qa_prompt: BasePromptTemplate=SPARQL_QA_PROMPT, sparql_select_prompt: BasePromptTemplate=SPARQL_GENERATION_SELECT_PROMPT, sparql_update_prompt: BasePromptTemplate=SPARQL_GENERATION_UPDATE_PROMPT, sparql_intent_prompt: BasePromptTemplate=SPARQL_INTENT_PROMPT, **kwargs: Any) -> GraphSparqlQAChain:
        ...
        return cls(qa_chain=qa_chain, sparql_generation_select_chain=sparql_generation_select_chain, sparql_generation_update_chain=sparql_generation_update_chain, sparql_intent_chain=sparql_intent_chain, **kwargs)
    ...
    def _call(self, inputs: Dict[str, Any], run_manager: Optional[CallbackManagerForChainRun]=None) -> Dict[str, str]:
        ...
        if 'SELECT' in intent and 'UPDATE' not in intent:
            ...
            raise ValueError('I am sorry, but this prompt seems to fit none of the currently supported SPARQL query types, i.e., SELECT and UPDATE.')
        _run_manager.on_text('Identified intent:', end='\n', verbose=self.verbose)
        _run_manager.on_text(intent, color='green', end='\n', verbose=self.verbose)
        generated_sparql = sparql_generation_chain.run({'prompt': prompt, 'schema': self.graph.get_schema}, callbacks=callbacks)
        ...
        if intent == 'SELECT':
            ...
            result = self.qa_chain({'prompt': prompt, 'context': context}, callbacks=callbacks)
