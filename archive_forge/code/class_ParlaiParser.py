...
class ParlaiParser(argparse.ArgumentParser):
    ...
    def __init__(self, add_parlai_args=True, add_model_args=False, description=None, **kwargs):
        ...
        super().__init__(description=description, allow_abbrev=False, conflict_handler='resolve', add_help=True, **kwargs)
    ...
    def add_extra_args(self, args=None):
        ...
        if task is not None:
            self.add_world_args(task, parsed.get('interactive_task', False), parsed.get('selfchat_task', False))
        try:
            self.set_defaults(**self._defaults)
        except AttributeError:
            raise RuntimeError('Please file an issue on github that argparse got an attribute error when parsing.')
    ...
    def parse_args(self, args=None, namespace=None, **kwargs):
        ...
        if 'print_args' in kwargs:
            logging.error('You gave the print_args flag to parser.parse_args, but this is no longer supported. Use opt.log() to print the arguments')
    ...
    def add_argument_group(self, *args, **kwargs):
        ...
        def ag_add_argument(*args, **kwargs):
            ...
            for k, v in newattr.items():
                setattr(action, k, v)
            return action
        arg_group.add_argument = ag_add_argument
