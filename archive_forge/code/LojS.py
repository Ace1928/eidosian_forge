...
trajectory_information = TrajectoryInformation(
    data_path + env_name + "/" + trajectory_name
)
final_chain = trajectory_information.to_old_chain_format(
    items=trajectory_information.chain, return_time_indexes=False
)
...
for episode_step in range(0, 2000000):
    ...
    if "inventory" in observation:
        ...
        inventory_channel = np.zeros(
            shape=list(observation["pov"].shape[:-1]) + [1],
            dtype=observation["pov"].dtype,
        )
        for key_idx, key in enumerate(observation["inventory"].keys()):
            item_scaled = np.clip(
                1 - 1 / (observation["inventory"][key] + 1), 0, 1  # Inversed
            )
            item_channel = (
                np.ones(shape=[rs, rs, 1], dtype=observation["pov"].dtype) * item_scaled
            )
    ...
    while True:
        try:
            ...
            env_output = {
                "env_id": np.array([arguments.env_id]),
                "reward": reward / 10.0,
                "done": done,
            ...
            if "inventory" in observation_1:
                ...
                inventory_channel_1 = np.zeros(
                    shape=list(observation_1["pov"].shape[:-1]) + [1],
                ...
                for key_idx, key in enumerate(observation_1["inventory"].keys()):
                    ...
                    item_scaled = np.clip(
                        1 - 1 / (observation_1["inventory"][key] + 1), 0, 1  # Inversed
                    )
                    # print("item_scaled : " + str(item_scaled))
                    item_channel = (
                        np.ones(shape=[rs, rs, 1], dtype=observation_1["pov"].dtype)
                        * item_scaled
                    )
                    width_low = (key_idx % num_element_width) * rs
                    height_low = (key_idx // num_element_width) * rs
            ...
            next_state = np.concatenate(
                (observation_1["pov"] / 255.0, inventory_channel_1), axis=2
            )
            ...
            if done:
                if arguments.env_id == 0:
                    ...
                    with writer.as_default():
                        tf.summary.scalar(
                            "average_reward", average[-1], step=episode_step
                        )
                        writer.flush()
            ...
            logging.info(
                "Inference call failed. This is normal at the end of training."
            )
