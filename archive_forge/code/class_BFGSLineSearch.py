...
class BFGSLineSearch(Optimizer):
    ...
    def __init__(self, atoms, restart=None, logfile='-', maxstep=None, trajectory=None, c1=0.23, c2=0.46, alpha=10.0, stpmax=50.0, master=None, force_consistent=None):
    ...
    def step(self, f=None):
        ...
        if isinstance(atoms, NEB):
            raise TypeError('NEB calculations cannot use the BFGSLineSearch optimizer. Use BFGS or another optimizer.')
        ...
        ls = LineSearch()
        self.alpha_k, e, self.e0, self.no_update = ls._line_search(self.func, self.fprime, r, self.p, g, e, self.e0, maxstep=self.maxstep, c1=self.c1, c2=self.c2, stpmax=self.stpmax)
    ...
    def update(self, r, g, r0, g0, p0):
        ...
        if self.H is None:
            ...
            dr = r - r0
            dg = g - g0
            if not ((self.alpha_k or 0) > 0 and abs(np.dot(g, p0)) - abs(np.dot(g0, p0)) < 0 or self.replay):
                return
            ...
            self.H = np.dot(A1, np.dot(self.H, A2)) + rhok * dr[:, np.newaxis] * dr[np.newaxis, :]

    def func(self, x):
        """Objective function for use of the optimizers"""
        self.atoms.set_positions(x.reshape(-1, 3))
        self.function_calls += 1
        return self.atoms.get_potential_energy(force_consistent=self.force_consistent) / self.alpha
    ...
    def log(self, forces=None):
        ...
        w('%s:  %3d[%3d] %02d:%02d:%02d %15.6f%1s %12.4f\n' % (name, self.nsteps, self.force_calls, T[3], T[4], T[5], e, {1: '*', 0: ''}[self.force_consistent], fmax))
        self.logfile.flush()