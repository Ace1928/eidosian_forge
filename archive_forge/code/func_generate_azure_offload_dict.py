...
def generate_azure_offload_dict(module, array):
    ...
    if P53_API_VERSION in api_version:
        ...
        for target in range(0, len(offload)):
            ...
            offload_info[offloadt] = {'status': offload[target]['status'], 'account_name': offload[target]['account_name'], 'protocol': offload[target]['protocol'], 'secret_access_key': offload[target]['secret_access_key'], 'container_name': offload[target]['container_name']}
