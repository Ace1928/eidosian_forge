...
class UserTests(utils.ClientTestCase):
    ...
    def setUp(self):
        ...
        self.TEST_USERS = {'users': {'values': [{'email': 'None', 'enabled': True, 'id': self.ADMIN_USER_ID, 'name': 'admin'}, {'email': 'None', 'enabled': True, 'id': self.DEMO_USER_ID, 'name': 'demo'}]}}

    def test_create(self):
        tenant_id = uuid.uuid4().hex
        user_id = uuid.uuid4().hex
        password = uuid.uuid4().hex
        req_body = {'user': {'name': 'gabriel', 'password': password, 'tenantId': tenant_id, 'email': 'test@example.com', 'enabled': True}}
        resp_body = {'user': {'name': 'gabriel', 'enabled': True, 'tenantId': tenant_id, 'id': user_id, 'password': password, 'email': 'test@example.com'}}
        self.stub_url('POST', ['users'], json=resp_body)
        user = self.client.users.create(req_body['user']['name'], req_body['user']['password'], req_body['user']['email'], tenant_id=req_body['user']['tenantId'], enabled=req_body['user']['enabled'])
    ...
    def test_create_user_without_password(self):
        ...
        tenant_id = uuid.uuid4().hex
        user_enabled = True
        req_body = {'user': {'name': user_name, 'password': None, 'tenantId': tenant_id, 'enabled': user_enabled, 'email': None}}
        resp_body = {'user': {'name': user_name, 'enabled': user_enabled, 'tenantId': tenant_id, 'id': user_id}}
        self.stub_url('POST', ['users'], json=resp_body)
        user = self.client.users.create(user_name, tenant_id=tenant_id, enabled=user_enabled)
        self.assertIsInstance(user, users.User)
    ...
    def test_get(self):
        self.stub_url('GET', ['users', self.ADMIN_USER_ID], json={'user': self.TEST_USERS['users']['values'][0]})
    ...
    def test_update(self):
        req_1 = {'user': {'email': 'gabriel@example.com', 'name': 'gabriel'}}
        password = uuid.uuid4().hex
        req_2 = {'user': {'password': password}}
        tenant_id = uuid.uuid4().hex
        req_3 = {'user': {'tenantId': tenant_id}}
        req_4 = {'user': {'enabled': False}}
        self.stub_url('PUT', ['users', self.DEMO_USER_ID], json=req_1)
        self.stub_url('PUT', ['users', self.DEMO_USER_ID, 'OS-KSADM', 'password'], json=req_2)
        self.stub_url('PUT', ['users', self.DEMO_USER_ID, 'OS-KSADM', 'tenant'], json=req_3)
        self.stub_url('PUT', ['users', self.DEMO_USER_ID, 'OS-KSADM', 'enabled'], json=req_4)
        self.client.users.update(self.DEMO_USER_ID, name='gabriel', email='gabriel@example.com')
