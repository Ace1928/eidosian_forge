...
class Meilisearch(VectorStore):
    ...
    def __init__(self, embedding: Embeddings, client: Optional[Client]=None, url: Optional[str]=None, api_key: Optional[str]=None, index_name: str='langchain-demo', text_key: str='text', metadata_key: str='metadata', *, embedders: Optional[Dict[str, Any]]=None):
    ...
    def add_texts(self, texts: Iterable[str], metadatas: Optional[List[dict]]=None, ids: Optional[List[str]]=None, embedder_name: Optional[str]='default', **kwargs: Any) -> List[str]:
        ...
        for i, text in enumerate(texts):
            ...
            docs.append({'id': id, '_vectors': {f'{embedder_name}': embedding}, f'{self._metadata_key}': metadata})
        self._client.index(str(self._index_name)).add_documents(docs)
        return ids

    def similarity_search(self, query: str, k: int=4, filter: Optional[Dict[str, str]]=None, embedder_name: Optional[str]='default', **kwargs: Any) -> List[Document]:
        ...
        """
        docs_and_scores = self.similarity_search_with_score(query=query, embedder_name=embedder_name, k=k, filter=filter, kwargs=kwargs)
        return [doc for doc, _ in docs_and_scores]

    def similarity_search_with_score(self, query: str, k: int=4, filter: Optional[Dict[str, str]]=None, embedder_name: Optional[str]='default', **kwargs: Any) -> List[Tuple[Document, float]]:
        ...
        docs = self.similarity_search_by_vector_with_scores(embedding=_query, embedder_name=embedder_name, k=k, filter=filter, kwargs=kwargs)
        return docs
    ...
    def similarity_search_by_vector(self, embedding: List[float], k: int=4, filter: Optional[Dict[str, str]]=None, embedder_name: Optional[str]='default', **kwargs: Any) -> List[Document]:
        """Return meilisearch documents most similar to embedding vector.
    ...
    def from_texts(cls: Type[Meilisearch], texts: List[str], embedding: Embeddings, metadatas: Optional[List[dict]]=None, client: Optional[Client]=None, url: Optional[str]=None, api_key: Optional[str]=None, index_name: str='langchain-demo', ids: Optional[List[str]]=None, text_key: Optional[str]='text', metadata_key: Optional[str]='metadata', embedders: Dict[str, Any]={}, embedder_name: Optional[str]='default', **kwargs: Any) -> Meilisearch:
        ...
        """
        client = _create_client(client=client, url=url, api_key=api_key)
        vectorstore = cls(embedding=embedding, embedders=embedders, client=client, index_name=index_name)
        vectorstore.add_texts(texts=texts, embedder_name=embedder_name, metadatas=metadatas, ids=ids, text_key=text_key, metadata_key=metadata_key)
        return vectorstore