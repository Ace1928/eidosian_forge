...
class NewCmd(appcommands.Cmd):
    ...
    def __init__(self, name: str, flag_values):
        ...
        if self._new_style:
            ...
            self._star_args = bool(code.co_flags & 4)
            self._star_kwds = bool(code.co_flags & 8)
            if self._star_args:
                self._max_args = sys.maxsize
            self.surface_in_shell = True
            self.__doc__ = self.RunWithArgs.__doc__
        elif hasattr(self.Run, '__func__') and self.Run.__func__ is NewCmd.Run.__func__:
            raise appcommands.AppCommandsError('Subclasses of NewCmd must override Run or RunWithArgs')
    ...
    """
        ...
        original_values = {name: self._command_flags[name].value for name in self._command_flags}
        original_presence = {name: self._command_flags[name].present for name in self._command_flags}
        try:
