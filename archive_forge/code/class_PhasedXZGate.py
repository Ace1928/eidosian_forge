...
class PhasedXZGate(raw_types.Gate):
    ...
    def __init__(self, *, x_exponent: Union[float, sympy.Expr], z_exponent: Union[float, sympy.Expr], axis_phase_exponent: Union[float, sympy.Expr]) -> None:
        """Inits PhasedXZGate.
    ...
    def _value_equality_values_(self):
        c = self._canonical()
        return (value.PeriodicValue(c._x_exponent, 2), value.PeriodicValue(c._z_exponent, 2), value.PeriodicValue(c._axis_phase_exponent, 2))
    ...
    def _qasm_(self, args: 'cirq.QasmArgs', qubits: Tuple['cirq.Qid', ...]) -> Optional[str]:
        ...
        qasm_gate = qasm_output.QasmUGate(lmda=0.5 - self._axis_phase_exponent, theta=self._x_exponent, phi=self._z_exponent + self._axis_phase_exponent - 0.5)
    ...
    def __pow__(self, exponent: Union[float, int]) -> 'PhasedXZGate':
        if exponent == 1:
            return self
        if exponent == -1:
            return PhasedXZGate(x_exponent=-self._x_exponent, z_exponent=-self._z_exponent, axis_phase_exponent=self._z_exponent + self.axis_phase_exponent)
        return NotImplemented

    def _is_parameterized_(self) -> bool:
        """See `cirq.SupportsParameterization`."""
        return protocols.is_parameterized(self._x_exponent) or protocols.is_parameterized(self._z_exponent) or protocols.is_parameterized(self._axis_phase_exponent)

    def _parameter_names_(self) -> AbstractSet[str]:
        """See `cirq.SupportsParameterization`."""
        return protocols.parameter_names(self._x_exponent) | protocols.parameter_names(self._z_exponent) | protocols.parameter_names(self._axis_phase_exponent)

    def _resolve_parameters_(self, resolver: 'cirq.ParamResolver', recursive: bool) -> 'cirq.PhasedXZGate':
        """See `cirq.SupportsParameterization`."""
        ...
        if isinstance(axis_phase_exponent, (complex, numbers.Complex)):
            if isinstance(axis_phase_exponent, numbers.Real):
                ...
                raise ValueError(f'Complex exponent {axis_phase_exponent} not allowed in cirq.PhasedXZGate')
        return PhasedXZGate(z_exponent=z_exponent, x_exponent=x_exponent, axis_phase_exponent=axis_phase_exponent)

    def _phase_by_(self, phase_turns, qubit_index) -> 'cirq.PhasedXZGate':
        """See `cirq.SupportsPhase`."""
        assert qubit_index == 0
        return PhasedXZGate(x_exponent=self._x_exponent, z_exponent=self._z_exponent, axis_phase_exponent=self._axis_phase_exponent + phase_turns * 2)
    ...
    def _pauli_expansion_(self) -> 'cirq.LinearDict[str]':
        ...
        cx = np.cos(x_angle)
        sx = np.sin(x_angle)
        return value.LinearDict({'I': phase * cx * np.cos(z_angle), 'X': -1j * phase * sx * np.cos(z_angle + axis_angle), 'Y': -1j * phase * sx * np.sin(z_angle + axis_angle), 'Z': -1j * phase * cx * np.sin(z_angle)})

    def _circuit_diagram_info_(self, args: 'cirq.CircuitDiagramInfoArgs') -> str:
        """See `cirq.SupportsCircuitDiagramInfo`."""
        return f'PhXZ(a={args.format_real(self._axis_phase_exponent)},x={args.format_real(self._x_exponent)},z={args.format_real(self._z_exponent)})'

    def __str__(self) -> str:
        return protocols.circuit_diagram_info(self).wire_symbols[0]

    def __repr__(self) -> str:
        return f'cirq.PhasedXZGate(axis_phase_exponent={proper_repr(self._axis_phase_exponent)}, x_exponent={proper_repr(self._x_exponent)}, z_exponent={proper_repr(self._z_exponent)})'

    def _json_dict_(self) -> Dict[str, Any]:
        return protocols.obj_to_dict_helper(self, ['axis_phase_exponent', 'x_exponent', 'z_exponent'])