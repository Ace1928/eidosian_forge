...
class DeliveryPipelinesClient(object):
    ...
    """
        request = self.messages.ClouddeployProjectsLocationsDeliveryPipelinesGetRequest(name=name)
        return self._service.Get(request)

    def List(self, location, filter_str=None, order_by=None, page_size=0, limit=None):
    ...
    """
        list_req = self.messages.ClouddeployProjectsLocationsDeliveryPipelinesListRequest(parent=location, filter=filter_str, orderBy=order_by)
        return list_pager.YieldFromList(self._service, list_req, field='deliveryPipelines', batch_size=page_size, limit=limit, batch_size_attribute='pageSize')
    ...
    """
        msg = self.messages.ClouddeployProjectsLocationsDeliveryPipelinesRollbackTargetRequest(name=name, rollbackTargetRequest=request)
