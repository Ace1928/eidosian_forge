...
class DevicesLiveToolsPing(object):
    ...
    def __init__(self, params, meraki):
        self.meraki = meraki
        self.new_object = dict(target=params.get('target'), count=params.get('count'), serial=params.get('serial'), id=params.get('id'))
    ...
    def create_params(self):
        ...
        if self.new_object.get('target') is not None or self.new_object.get('target') is not None:
            new_object_params['target'] = self.new_object.get('target') or self.new_object.get('target')
        if self.new_object.get('count') is not None or self.new_object.get('count') is not None:
            new_object_params['count'] = self.new_object.get('count') or self.new_object.get('count')
        if self.new_object.get('serial') is not None or self.new_object.get('serial') is not None:
    ...
    def get_object_by_id(self, id):
        result = None
        try:
            items = self.meraki.exec_meraki(family='devices', function='getDeviceLiveToolsPing', params={'id': id})
    ...
    def requires_update(self, current_obj):
        ...
        obj_params = [('target', 'target'), ('count', 'count'), ('serial', 'serial'), ('id', 'id')]
    ...
    def create(self):
        result = self.meraki.exec_meraki(family='devices', function='createDeviceLiveToolsPing', params=self.create_params(), op_modifies=True)
        return result