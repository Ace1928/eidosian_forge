...
def _get_arg_position(self, func: F, arg_name: str) -> int:
    ...
    logging.debug(f'Getting arg position for {arg_name} in {func.__name__} at {start_time}')
    ...
    end_time = time.time()
    logging.debug(f'Getting arg position for {arg_name} in {func.__name__} completed at {end_time}')
    return result