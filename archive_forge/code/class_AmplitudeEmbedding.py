    ...
    def _preprocess(features, wires, pad_with, normalize):
        ...
        * If normalize is false, check that last dimension of features is normalised to one. Else, normalise the
        ...
        if len(shape) not in (1, 2):
            raise ValueError(f'Features must be a one-dimensional tensor, or two-dimensional with batching; got shape {shape}.')
        n_features = shape[-1]
        dim = 2 ** len(wires)
        if pad_with is None and n_features != dim:
            raise ValueError(f"Features must be of length {dim}; got length {n_features}. Use the 'pad_with' argument for automated padding.")
        if pad_with is not None:
            if n_features > dim:
                raise ValueError(f'Features must be of length {dim} or smaller to be padded; got length {n_features}.')
            if n_features < dim:
        ...
        if qml.math.is_abstract(norm):
            ...
            if normalize or pad_with:
                ...
                raise ValueError(f"Features must be a vector of norm 1.0; got norm {norm}. Use 'normalize=True' to automatically normalize.")
        return features