...
class TinyAsyncOpenAIInfinityEmbeddingClient:
    ...
    def __init__(self, host: str='http://localhost:7797/v1', aiosession: Optional[aiohttp.ClientSession]=None) -> None:
        self.host = host
    ...
    def _kwargs_post_request(self, model: str, texts: List[str]) -> Dict[str, Any]:
        ...
        """
        return dict(url=f'{self.host}/embeddings', headers={'content-type': 'application/json'}, json=dict(input=texts, model=model))
    ...
    async def _async_request(self, session: aiohttp.ClientSession, kwargs: Dict[str, Any]) -> List[List[float]]:
        async with session.post(**kwargs) as response:
            if response.status != 200:
                raise Exception(f'Infinity returned an unexpected response with status {response.status}: {response.text}')
    ...
    async def aembed(self, model: str, texts: List[str]) -> List[List[float]]:
        ...
        if self.aiosession is None:
            self.aiosession = aiohttp.ClientSession(trust_env=True, connector=aiohttp.TCPConnector(limit=32))
        async with self.aiosession as session:
            embeddings_batch_perm = await asyncio.gather(*[self._async_request(session=session, **self._kwargs_post_request(model=model, texts=t)) for t in perm_texts_batched])
        embeddings_perm = self._unbatch(embeddings_batch_perm)
        embeddings = unpermute_func(embeddings_perm)
        return embeddings