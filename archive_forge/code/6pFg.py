...
from typing import Any, Callable, Dict, Optional, Tuple, OrderedDict
...
import scripts.trading_bot.indehandler as afh
...
class AsyncCache:
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            # Pass `func` as the first argument to `_default_cache_key_strategy`
            key = self._default_cache_key_strategy(func, *args, **kwargs)
            cached_result = await self.cache_logic(key, func, *args, **kwargs)
            if cached_result is not None:
                return cached_result
            result = await func(*args, **kwargs)
            await self.cache_logic(key, func, *args, **kwargs)
            return result
        return wrapper
