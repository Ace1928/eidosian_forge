...
def request_route_refresh(self, *route_families):
    ...
    if not self.in_established:
        raise ValueError('Peer not in established state to satisfy this request.')
    ...
    for route_family in route_families:
        if skip_validation or (route_family in SUPPORTED_GLOBAL_RF and self._protocol.is_mbgp_cap_valid(route_family)):
            rr_req = BGPRouteRefresh(route_family.afi, route_family.safi)
            self.enque_outgoing_msg(rr_req)
            LOG.debug('Enqueued Route Refresh message to peer %s for rf: %s', self, route_family)