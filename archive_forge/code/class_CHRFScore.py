...
class CHRFScore(Metric):
    ...
    def update(self, preds: Sequence[str], target: Sequence[Sequence[str]]) -> None:
        ...
        n_grams_dicts_tuple = _chrf_score_update(preds, target, *self._convert_states_to_dicts(), self.n_char_order, self.n_word_order, self.n_order, self.beta, self.lowercase, self.whitespace, self.sentence_chrf_score if self.return_sentence_level_score else None)
    ...
    def compute(self) -> Union[Tensor, Tuple[Tensor, Tensor]]:
        ...
        if self.sentence_chrf_score is not None:
            return (_chrf_score_compute(*self._convert_states_to_dicts(), self.n_order, self.beta), torch.cat(self.sentence_chrf_score))
    ...
    def _convert_states_to_dicts(self) -> _DICT_STATES_TYPES:
        ...
        n_grams_dicts: Dict[str, Dict[int, Tensor]] = dict(zip(_DICT_STATES_NAMES, _prepare_n_grams_dicts(self.n_char_order, self.n_word_order)))
        for (n_gram_level, n_gram_order), text in self._get_text_n_gram_iterator():
            for n in range(1, n_gram_order + 1):
