...
class ShellTestResourceTypes(ShellBase):
    ...
    def test_resource_type_template_json(self):
        self.register_keystone_auth_fixture()
        resp_dict = {'AWSTemplateFormatVersion': '2013-05-23', 'Parameters': {}, 'Resources': {}, 'Outputs': {}}
        self.mock_request_get('/resource_types/OS%3A%3ANova%3A%3AKeyPair/template?template_type=cfn', resp_dict)
        show_text = self.shell('resource-type-template -F json OS::Nova::KeyPair')
        required = ['{', '  "AWSTemplateFormatVersion": "2013-05-23"', '  "Outputs": {}', '  "Resources": {}', '  "Parameters": {}', '}']
        for r in required:
            self.assertRegex(show_text, r)