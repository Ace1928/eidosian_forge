...
class _TestTupleContainerBase(object):
    ...
    def test_pickle(self):
        index = range(5)
        ctuple = self._container_type([self._ctype_factory() for i in index] + [self._container_type()])
        index = list(index)
        index = index + [len(index)]
        for i in index:
            self.assertTrue(ctuple[i].parent is ctuple)
        pickled_ctuple = pickle.loads(pickle.dumps(ctuple, protocol=_pickle_test_protocol))
    ...
    def test_components(self):
        ...
        self.assertEqual(list((id(_c) for _c in csubtuple.components())), list((id(_c) for _c in csubtupleflattened)))
        self.assertEqual(len(set((id(_c) for _c in csubtuple.components()))), len(list((id(_c) for _c in csubtuple.components()))))
        self.assertEqual(len(set((id(_c) for _c in csubtuple.components()))), 3)
        ctuple = self._container_type([ctupleflattened[0], ctupleflattened[1], csubtuple])
        self.assertEqual(list((id(_c) for _c in ctuple.components())), list((id(_c) for _c in ctupleflattened)))
        self.assertEqual(len(set((id(_c) for _c in ctuple.components()))), len(list((id(_c) for _c in ctuple.components()))))
        self.assertEqual(len(set((id(_c) for _c in ctuple.components()))), 5)

    def test_preorder_traversal(self):
        csubtuple = self._container_type([self._ctype_factory()])
        ctuple = self._container_type([self._ctype_factory(), csubtuple, self._ctype_factory()])
        traversal = []
        ...
        self.assertEqual([c.name for c in traversal], [c.name for c in pmo.preorder_traversal(ctuple)])
        self.assertEqual([id(c) for c in traversal], [id(c) for c in pmo.preorder_traversal(ctuple)])
