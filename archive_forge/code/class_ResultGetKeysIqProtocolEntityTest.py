...
class ResultGetKeysIqProtocolEntityTest(ResultIqProtocolEntityTest):
    ...
    def setUp(self):
        ...
        for i in range(0, 1):
            ...
            registrationNode = ProtocolTreeNode('registration', data=ResultGetKeysIqProtocolEntity._intToBytes(KeyHelper.generateRegistrationId()))
            ...
            signedPreKeyNode_idNode = ProtocolTreeNode('id', data=ResultGetKeysIqProtocolEntity._intToBytes(signedPreKey.getId()))
            signedPreKeyNode_valueNode = ProtocolTreeNode('value', data=signedPreKey.getKeyPair().getPublicKey().getPublicKey())
            signedPreKeyNode_sigNode = ProtocolTreeNode('signature', data=signedPreKey.getSignature())
            ...
            userNode.addChildren([registrationNode, typeNode, identityNode, signedPreKeyNode, preKeyNode])