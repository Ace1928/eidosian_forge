...
class RequirementPreparer:
    ...
    def __init__(self, build_dir: str, download_dir: Optional[str], src_dir: str, build_isolation: bool, check_build_deps: bool, build_tracker: BuildTracker, session: PipSession, progress_bar: str, finder: PackageFinder, require_hashes: bool, use_user_site: bool, lazy_wheel: bool, verbosity: int, legacy_resolver: bool) -> None:
        super().__init__()
    ...
    def _ensure_link_req_src_dir(self, req: InstallRequirement, parallel_builds: bool) -> None:
        ...
        if req.link.is_existing_dir():
            ...
            return
        req.ensure_has_source_dir(self.build_dir, autodelete=True, parallel_builds=parallel_builds)
    ...
    def _fetch_metadata_only(self, req: InstallRequirement) -> Optional[BaseDistribution]:
        if self.legacy_resolver:
            logger.debug('Metadata-only fetching is not used in the legacy resolver')
            return None
        if self.require_hashes:
            logger.debug('Metadata-only fetching is not used as hash checking is required')
            return None
