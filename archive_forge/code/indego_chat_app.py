...
def app():
    ...
    response = "No response yet"  # Default response
    ...
    with st.sidebar:
        ...
        api_key = st.text_input(
            "API Key", type="password", value=config_manager.settings.get("api_key", "")
        )
        ...
        selected_model = st.selectbox(
            ...
            index=(
                model_options.index(
                    config_manager.settings.get("model", model_options[0])
                )
                if model_options
                else 0
            ),
        ...
        max_tokens = st.number_input(
            ...
            min_value=256,
            max_value=4096,
            value=config_manager.settings.get("max_tokens", 2048),
        )
        ...
        temperature = st.slider(
            ...
            min_value=0.0,
            max_value=1.0,
            value=config_manager.settings.get("temperature", 0.95),
        )
        ...
        frequency_penalty = st.slider(
            ...
            min_value=0.0,
            max_value=1.0,
            value=config_manager.settings.get("frequency_penalty", 0.1),
        )
        ...
        presence_penalty = st.slider(
            ...
            min_value=0.0,
            max_value=1.0,
            value=config_manager.settings.get("presence_penalty", 0.05),
        )
        config_manager.settings["presence_penalty"] = presence_penalty

    if st.button("Initialize API Interface"):
        api_interface = OpenAIInterface(api_key=api_key)  # Initialize API interface
        ...
        if service_type == "Text Completion":
            ...
            )  # Output field for text response
        elif service_type == "Image Generation":
            prompt = st.text_input("Image Prompt", value="")
            if st.button("Generate"):
                image_url = api_interface.generate_image(prompt)
                st.image(image_url, caption="Generated Image")
            st.text(image_url)  # Output field for image URL
            ...
            st.text_area(
                "Transcription", response_text, height=300
            )  # Output field for audio transcription
