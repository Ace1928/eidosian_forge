...
class QuantizeLinear(Base):
    ...
    def export() -> None:
        ...
        expect(node, inputs=[x, y_scale, y_zero_point], outputs=[y], name='test_quantizelinear')
    ...
    def export_e4m3fn() -> None:
        ...
        expect(node, inputs=[x, y_scale, y_zero_point], outputs=[y], name='test_quantizelinear_e4m3fn')
    ...
    def export_int4() -> None:
        ...
        x = np.array([[0.0, 2.5, 4.8, 8.6], [-30, -20, 6, 9], [12, 15, 16, 40]]).astype(np.float32)
        y_scale = np.asarray([2.0, 3.0, 4.0], dtype=np.float32)
        y_zero_point = make_tensor('zero_point', TensorProto.INT4, y_scale.shape, np.ones_like(y_scale))
        y = make_tensor('y', TensorProto.INT4, x.shape, [1, 2, 3, 5, -8, -6, 3, 4, 4, 5, 5, 7])
        expect(node, inputs=[x, y_scale, y_zero_point], outputs=[y], name='test_quantizelinear_int4')
