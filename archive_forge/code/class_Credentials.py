...
class Credentials(credentials.Scoped, credentials.CredentialsWithQuotaProject, credentials.CredentialsWithTokenUri, metaclass=abc.ABCMeta):
    ...
    def __init__(self, audience, subject_token_type, token_url, credential_source, service_account_impersonation_url=None, service_account_impersonation_options=None, client_id=None, client_secret=None, token_info_url=None, quota_project_id=None, scopes=None, default_scopes=None, workforce_pool_user_project=None, universe_domain=credentials.DEFAULT_UNIVERSE_DOMAIN, trust_boundary=None):
        ...
        if not self.is_workforce_pool and self._workforce_pool_user_project:
            raise exceptions.InvalidValue('workforce_pool_user_project should not be set for non-workforce pool credentials')
    ...
    def info(self):
        ...
        config_info.update(type=_EXTERNAL_ACCOUNT_JSON_TYPE, service_account_impersonation=config_info.pop('service_account_impersonation_options', None))
