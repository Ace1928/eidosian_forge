...
class BasicPrinter:
    ...
    _CONSTANT_LOOKUP = {ast.Constant.PI: 'pi', ast.Constant.EULER: 'euler', ast.Constant.TAU: 'tau'}
    _MODIFIER_LOOKUP = {ast.QuantumGateModifierName.CTRL: 'ctrl', ast.QuantumGateModifierName.NEGCTRL: 'negctrl', ast.QuantumGateModifierName.INV: 'inv', ast.QuantumGateModifierName.POW: 'pow'}
    ...
    def visit(self, node: ast.ASTNode) -> None:
        ...
        for cls_ in type(node).mro():
            ...
            raise RuntimeError(f"This visitor does not know how to handle AST nodes of type '{node_type}', but was given '{node}'.")
        visitor(node)
