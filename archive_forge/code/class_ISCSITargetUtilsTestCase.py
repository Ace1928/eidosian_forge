...
class ISCSITargetUtilsTestCase(test_base.OsWinBaseTestCase):
    _autospec_classes = [tg_utils.pathutils.PathUtils, tg_utils.hostutils.HostUtils, tg_utils.win32utils.Win32Utils]

    @mock.patch.object(tg_utils.ISCSITargetUtils, '_ensure_wt_provider_available')
    ...
    def _test_get_portal_locations(self, available_only=False, fail_if_none_found=False):
        mock_portal = mock.Mock(Listen=False, Address=mock.sentinel.address, Port=mock.sentinel.port)
        mock_portal_location = '%s:%s' % (mock.sentinel.address, mock.sentinel.port)
        ...
        if available_only and fail_if_none_found:
            self.assertRaises(exceptions.ISCSITargetException, self._tgutils.get_portal_locations, available_only=available_only, fail_if_none_found=fail_if_none_found)
        else:
            portals = self._tgutils.get_portal_locations(available_only=available_only, fail_if_none_found=fail_if_none_found)
            expected_retrieved_portals = []
            if not available_only:
                expected_retrieved_portals.append(mock_portal_location)
            self.assertEqual(expected_retrieved_portals, portals)
    ...
    def test_get_wt_disk_not_found_exception(self):
        self._test_get_wt_disk(disk_found=False, fail_if_not_found=True)
    ...
    def test_get_wt_snap_not_found_exception(self):
        self._test_get_wt_snap(snap_found=False, fail_if_not_found=True)
    ...
    def test_create_iscsi_target_already_exists_failing(self):
        self._test_create_iscsi_target_exception(target_exists=True, fail_if_exists=True)
    ...
    def _test_iscsi_target_exists(self, mock_get_wt_host, target_exists=True):
        mock_get_wt_host.return_value = mock.sentinel.wt_host if target_exists else None
