...
class ChatEdenAI(BaseChatModel):
    ...
    def _stream(self, messages: List[BaseMessage], stop: Optional[List[str]]=None, run_manager: Optional[CallbackManagerForLLMRun]=None, **kwargs: Any) -> Iterator[ChatGenerationChunk]:
        ...
        headers = {'Authorization': f'Bearer {self._api_key}', 'User-Agent': self.get_user_agent()}
        formatted_data = _format_edenai_messages(messages=messages)
        payload: Dict[str, Any] = {'providers': self.provider, 'max_tokens': self.max_tokens, 'temperature': self.temperature, 'fallback_providers': self.fallback_providers, **formatted_data, **kwargs}
    ...
    def _generate(self, messages: List[BaseMessage], stop: Optional[List[str]]=None, run_manager: Optional[CallbackManagerForLLMRun]=None, **kwargs: Any) -> ChatResult:
        ...
        return ChatResult(generations=[ChatGeneration(message=AIMessage(content=provider_response['generated_text']))], llm_output=data)
