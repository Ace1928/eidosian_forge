...
class EpsilonGreedy(Exploration):
    ...
    def __init__(self, action_space: gym.spaces.Space, *, framework: str, initial_epsilon: float=1.0, final_epsilon: float=0.05, warmup_timesteps: int=0, epsilon_timesteps: int=int(100000.0), epsilon_schedule: Optional[Schedule]=None, **kwargs):
        ...
        self.epsilon_schedule = from_config(Schedule, epsilon_schedule, framework=framework) or PiecewiseSchedule(endpoints=[(0, initial_epsilon), (warmup_timesteps, initial_epsilon), (warmup_timesteps + epsilon_timesteps, final_epsilon)], outside_value=final_epsilon, framework=self.framework)
    ...
    def _get_tf_exploration_action_op(self, action_distribution: ActionDistribution, explore: Union[bool, TensorType], timestep: Union[int, TensorType]) -> 'tf.Tensor':
        ...
        random_valid_action_logits = tf.where(tf.equal(q_values, tf.float32.min), tf.ones_like(q_values) * tf.float32.min, tf.ones_like(q_values))
        random_actions = tf.squeeze(tf.random.categorical(random_valid_action_logits, 1), axis=1)
        chose_random = tf.random.uniform(tf.stack([batch_size]), minval=0, maxval=1, dtype=tf.float32) < epsilon
        action = tf.cond(pred=tf.constant(explore, dtype=tf.bool) if isinstance(explore, bool) else explore, true_fn=lambda: tf.where(chose_random, random_actions, exploit_action), false_fn=lambda: exploit_action)
    ...
    def get_state(self, sess: Optional['tf.Session']=None):
        ...
        return {'cur_epsilon': convert_to_numpy(eps) if self.framework != 'tf' else eps, 'last_timestep': convert_to_numpy(self.last_timestep) if self.framework != 'tf' else self.last_timestep}
