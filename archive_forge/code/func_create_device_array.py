...
def create_device_array(shape, device_type):
    ...
    return np.asarray([tf_device.DeviceSpec(job='localhost/replica:0/task:0', device_type=device_type, device_index=i) for i in range(device_count)]).reshape(shape)