...
class PasqalSampler(cirq.work.Sampler):
    ...
    def __init__(self, remote_host: str, access_token: str='', device: Optional[cirq_pasqal.PasqalDevice]=None) -> None:
        """Inits PasqalSampler.
    ...
    def _serialize_circuit(self, circuit: cirq.circuits.AbstractCircuit, param_resolver: cirq.study.ParamResolverOrSimilarType) -> str:
    ...
    def run_sweep(self, program: cirq.AbstractCircuit, params: cirq.study.Sweepable, repetitions: int=1) -> List[cirq.study.Result]:
        ...
        """
        device = self._device
        assert isinstance(device, cirq_pasqal.PasqalDevice), 'Device must inherit from cirq.PasqalDevice.'
        ...
        for param_resolver in cirq.study.to_resolvers(params):
            ...
            results = self._send_serialized_circuit(serialization_str=json_str, repetitions=repetitions)
