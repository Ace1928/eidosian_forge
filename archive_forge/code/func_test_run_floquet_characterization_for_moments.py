...
def test_run_floquet_characterization_for_moments():
    ...
    circuit_with_calibration, requests = workflow.run_floquet_characterization_for_moments(circuit, engine, 'qproc', options=options)
    assert requests == [PhasedFSimCalibrationResult(parameters={(q_00, q_01): PhasedFSimCharacterization(theta=0.1, zeta=0.2, chi=None, gamma=None, phi=0.3), (q_02, q_03): PhasedFSimCharacterization(theta=0.4, zeta=0.5, chi=None, gamma=None, phi=0.6)}, gate=gate, options=options, project_id='project_id', program_id='program_id', job_id='job_id')]
