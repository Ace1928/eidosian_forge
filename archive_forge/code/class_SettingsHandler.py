...
class SettingsHandler(ExtensionHandlerMixin, ExtensionHandlerJinjaMixin, SchemaHandler):
    ...
    def initialize(self, name: str, app_settings_dir: str, schemas_dir: str, settings_dir: str, labextensions_path: list[str], **kwargs: Any) -> None:
        """Initialize the handler."""
        SchemaHandler.initialize(self, app_settings_dir, schemas_dir, settings_dir, labextensions_path)
    ...
    def get(self, schema_name: str='') -> Any:
        ...
        result, warnings = get_settings(self.app_settings_dir, self.schemas_dir, self.settings_dir, labextensions_path=self.labextensions_path, schema_name=schema_name, overrides=self.overrides, translator=translator.translate_schema, ids_only=ids_only)
        for w in warnings:
            if w:
                self.log.warning(w)
    ...
    def put(self, schema_name: str) -> None:
        ...
        try:
            ...
            save_settings(schemas_dir, settings_dir, schema_name, raw_settings, overrides, self.labextensions_path)
