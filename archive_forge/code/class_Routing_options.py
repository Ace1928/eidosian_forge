...
class Routing_options(ConfigBase):
    ...
    def execute_module(self):
        ...
        if state == 'gathered':
            ...
            result['parsed'] = self.get_routing_options_facts(data=running_config)
    ...
    def set_state(self, want, have):
        ...
        self.routing_options = build_child_xml_node(self.root, 'routing-options')
        state = self._module.params['state']
    ...
    def _state_merged(self, want, have):
        ...
        if 'autonomous_system' in want.keys():
            ...
            if as_num.get('as_number'):
                as_node = build_child_xml_node(self.routing_options, 'autonomous-system', as_num.get('as_number'))
                if as_num.get('loops'):
        ...
        if 'router_id' in want.keys():
            build_child_xml_node(self.routing_options, 'router-id', want.get('router_id'))
        return routing_xml
    ...
    def _state_deleted(self, want, have):
        ...
        if have is not None:
            if 'autonomous_system' in have.keys():
                build_child_xml_node(self.routing_options, 'autonomous-system', None, delete)
            if 'router_id' in have.keys():
                build_child_xml_node(self.routing_options, 'router-id', None, delete)