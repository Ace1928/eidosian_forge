...
class AnsibleCloudscaleApi(object):
    ...
    def _get(self, api_call):
        resp, info = fetch_url(self._module, self._api_url + api_call, headers=self._auth_header, timeout=self._module.params['api_timeout'])
        if info['status'] == 200:
    ...
    def _delete(self, api_call):
        if self._api_url not in api_call:
            ...
            api_endpoint = api_call
        resp, info = fetch_url(self._module, api_endpoint, headers=self._auth_header, method='DELETE', timeout=self._module.params['api_timeout'])
        if info['status'] == 204:
            return None
        else:
            self._module.fail_json(msg='Failure while calling the cloudscale.ch API with DELETE for "%s".' % api_endpoint, fetch_url_info=info)