...
class OpenAIRequestResponseResolver:
    ...
    def __call__(self, args: Sequence[Any], kwargs: Dict[str, Any], response: Response, start_time: float, time_elapsed: float) -> Optional[Dict[str, Any]]:
        ...
        try:
            if response.get('object') == 'edit':
                ...
                logger.info(f'Unsupported OpenAI response object: {response.get('object')}')
    ...
    def _resolve_chat_completion(self, request: Dict[str, Any], response: Response, time_elapsed: float) -> Dict[str, Any]:
        """Resolves the request and response objects for `openai.Completion`."""
        prompt = io.StringIO()
