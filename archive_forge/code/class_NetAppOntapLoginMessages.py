...
class NetAppOntapLoginMessages:
    ...
    def __init__(self):
        ...
        self.argument_spec.update(dict(vserver=dict(type='str'), banner=dict(type='str'), motd_message=dict(type='str', aliases=['message']), show_cluster_motd=dict(default=True, type='bool')))
        self.module = AnsibleModule(argument_spec=self.argument_spec, supports_check_mode=True, required_one_of=[['show_cluster_motd', 'banner', 'motd_message']])
    ...
    def form_current(self, record):
        ...
        return_result = {'banner': '', 'motd_message': '', 'uuid': record['uuid'] if record else self.get_svm_uuid(self.parameters.get('vserver')), 'show_cluster_motd': show_cluster_motd}
    ...
    def get_login_banner_zapi(self):
        ...
        try:
            ...
            self.module.fail_json(msg='Error fetching login_banner info: %s' % to_native(error), exception=traceback.format_exc())
        if result.get_child_by_name('num-records') and int(result.get_child_content('num-records')) > 0:
            login_banner_info = result.get_child_by_name('attributes-list').get_child_by_name('vserver-login-banner-info')
    ...
    def modify_motd(self, modify):
        ...
        try:
            ...
            self.module.fail_json(msg='Error modifying motd: %s' % to_native(err), exception=traceback.format_exc())
    ...
    def get_svm_uuid(self, vserver):
        ...
        if uuid is None:
            self.module.fail_json(msg="Error fetching vserver %s. Please make sure vserver name is correct. For cluster vserver, don't set vserver." % vserver)
        return uuid
