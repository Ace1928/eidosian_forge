...
class DuCommand(Command):
    ...
    help_spec = Command.HelpSpec(help_name='du', help_name_aliases=[], help_type='command_help', help_one_line_summary='Display object size usage', help_text=_DETAILED_HELP_TEXT, subcommand_help_text={})
    gcloud_storage_map = GcloudStorageMap(gcloud_command=['storage', 'du'], flag_map={'-0': GcloudStorageFlag('--zero-terminator'), '-a': GcloudStorageFlag('--all-versions'), '-c': GcloudStorageFlag('--total'), '-e': GcloudStorageFlag('--exclude-name-pattern'), '-h': GcloudStorageFlag('--readable-sizes'), '-s': GcloudStorageFlag('--summarize'), '-X': GcloudStorageFlag('--exclude-name-pattern-file')})
    ...
    def RunCommand(self):
        ...
        for url_arg in self.args:
            ...
            listing_helper = ls_helper.LsHelper(self.WildcardIterator, self.logger, print_object_func=_PrintObjectLong, print_dir_func=_PrintNothing, print_dir_header_func=_PrintNothing, print_dir_summary_func=_PrintDirectory, print_newline_func=_PrintNothing, all_versions=self.all_versions, should_recurse=True, exclude_patterns=self.exclude_patterns, fields=bucket_listing_fields)
            if top_level_storage_url.IsProvider():
                top_level_iter = self.WildcardIterator('%s://*' % top_level_storage_url.scheme).IterBuckets(bucket_fields=['id'])
            elif top_level_storage_url.IsBucket():
                top_level_iter = self.WildcardIterator('%s://%s' % (top_level_storage_url.scheme, top_level_storage_url.bucket_name)).IterBuckets(bucket_fields=['id'])
            else:
            ...
            for blr in top_level_iter:
                ...
                if storage_url.IsBucket() and self.summary_only:
                    storage_url = StorageUrlFromString(storage_url.CreatePrefixUrl(wildcard_suffix='**'))
                _, exp_objs, exp_bytes = listing_helper.ExpandUrlAndPrint(storage_url)
                if storage_url.IsObject() and exp_objs == 0 and ContainsWildcard(url_arg) and (not self.exclude_patterns):
                    got_nomatch_errors = True
                total_bytes += exp_bytes
                if self.summary_only:
                    self._PrintSummaryLine(exp_bytes, blr.url_string.rstrip('/').encode(UTF8))
