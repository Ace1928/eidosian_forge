...
class VideoRecorder:
    ...
    def __init__(self, env, path: Optional[str]=None, metadata: Optional[dict]=None, enabled: bool=True, base_path: Optional[str]=None):
        ...
        try:
            ...
            raise error.DependencyNotInstalled('MoviePy is not installed, run `pip install moviepy`')
        ...
        if 'rgb_array_list' != self.render_mode and 'rgb_array' != self.render_mode:
            logger.warn(f'Disabling video recorder because environment {env} was not initialized with any compatible video mode between `rgb_array` and `rgb_array_list`')
            self.enabled = False
        if not self.enabled:
            return
