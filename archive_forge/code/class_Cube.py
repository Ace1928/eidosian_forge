    ...
    def __init__(self, radius=1, nb_pts=300, nb_dim=3, sampling='random', seed=None, **kwargs):
        ...
        if self.sampling == 'random':
            if self.nb_dim == 2:
                ...
                pts[2 * n:3 * n, :] = np.concatenate((rs.rand(n, 1), np.zeros((n, 1)), rs.rand(n, 1)), axis=1)
                pts[3 * n:4 * n, :] = np.concatenate((rs.rand(n, 1), np.ones((n, 1)), rs.rand(n, 1)), axis=1)
                pts[4 * n:5 * n, :2] = rs.rand(n, 2)
                pts[5 * n:6 * n, :] = np.concatenate((rs.rand(n, 2), np.ones((n, 1))), axis=1)
        else:
            raise ValueError('Unknown sampling !')
        plotting = {'vertex_size': 80, 'elevation': 15, 'azimuth': 0, 'distance': 7}
        super(Cube, self).__init__(Xin=pts, k=10, gtype='Cube', plotting=plotting, **kwargs)