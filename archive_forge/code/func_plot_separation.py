...
def plot_separation(y, y_hat, y_hat_line, label_y_hat, expected_events, figsize, textsize, color, legend, locs, width, ax, plot_kwargs, y_hat_line_kwargs, exp_events_kwargs, backend_kwargs, show):
    ...
    if legend:
        ...
        exp_events_kwargs.setdefault('legend_label', 'Expected events')
    ...
    for i, loc in enumerate(locs):
        ...
        ax.vbar(loc, top=1, width=width, fill_alpha=alpha, line_alpha=alpha, **plot_kwargs)
    if y_hat_line:
        ax.line(np.linspace(0, 1, len(y_hat)), y_hat[idx], **y_hat_line_kwargs)
    if expected_events:
        expected_events = int(np.round(np.sum(y_hat)))
        ax.triangle(y_hat[idx][len(y_hat) - expected_events - 1], 0, **exp_events_kwargs)
