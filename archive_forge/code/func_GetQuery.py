...
def GetQuery(self):
    ...
    return datastore_query.Query(app=self.__app, namespace=self.__namespace, kind=self.__kind, ancestor=self.__ancestor_pb, filter_predicate=self.GetFilterPredicate(), order=self.GetOrder(), group_by=self.__group_by, read_time_us=self.__read_time_us)