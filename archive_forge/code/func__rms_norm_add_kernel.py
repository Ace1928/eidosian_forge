...
def _rms_norm_add_kernel(x_ptr, y_ptr, h1_ptr, w_ptr, eps, stride, N_COLS: tl.constexpr, BLOCK_SIZE: tl.constexpr, INCLUDE_WEIGHT: tl.constexpr):
    ...
    for offset in range(0, N_COLS, BLOCK_SIZE):
        ...
        mask = cols < N_COLS
        ax = tl.load(x_ptr + cols, mask=mask, other=0.0, eviction_policy='evict_last').to(tl.float32)
        ay = tl.load(y_ptr + cols, mask=mask, other=0.0, eviction_policy='evict_first').to(tl.float32)
        a = ax + ay
