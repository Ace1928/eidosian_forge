...
class DBClient(object):
    ...
    def run_command(self, args):
        _COMMANDS = {'list-dbs': self._list_dbs, 'get-schema': self._get_schema, 'get-schema-version': self._get_schema_version, 'list-tables': self._list_tables, 'list-columns': self._list_columns, 'transact': self._transact, 'monitor': self._monitor, 'dump': self._dump}
        command = args[0]
        args = args[1:]
        error, stream_ = stream.Stream.open_block(stream.Stream.open(self.remote))
        if error:
            raise RuntimeError('can not open socket to %s: %s' % (self.remote, os.strerror(error)))
    ...
    def _fetch_schema(self, rpc, database):
        return schema.DbSchema.from_json(self._fetch_schema_json(rpc, database))
    ...
    def _list_columns(self, rpc, *args):
        ...
        if table_name is None:
            ...
            tables = [table for table in schema_.tables.values() if table.name == table_name]
        columns = []
        for table in tables:
