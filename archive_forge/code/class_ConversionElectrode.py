...
class ConversionElectrode(AbstractElectrode):
    ...
    def get_sub_electrodes(self, adjacent_only=True):
        ...
        """
        if adjacent_only:
            return [ConversionElectrode(voltage_pairs=self.voltage_pairs[i:i + 1], working_ion_entry=self.working_ion_entry, initial_comp_formula=self.initial_comp_formula, framework_formula=self.framework_formula) for i in range(len(self.voltage_pairs))]
        sub_electrodes = []
        for i in range(len(self.voltage_pairs)):
            for j in range(i, len(self.voltage_pairs)):
                sub_electrodes.append(ConversionElectrode(voltage_pairs=self.voltage_pairs[i:j + 1], working_ion_entry=self.working_ion_entry, initial_comp_formula=self.initial_comp_formula, framework_formula=self.framework_formula))
    ...
    def __repr__(self):
        ...
        output = [f'{cls_name} with formula={formula!r} and n_steps={n_steps!r}, avg_voltage={avg_voltage:.3f} V, min_voltage={min_voltage:.3f} V, max_voltage={max_voltage:.3f} V', f'Capacity (grav.) {self.get_capacity_grav():.3f} mAh/g, capacity (vol.) {self.get_capacity_vol():.3f} Ah/l', f'Specific energy {self.get_specific_energy():.3f} Wh/kg, energy density {self.get_energy_density():.3f} Wh/l']
        return '\n'.join(output)
    ...
    def get_summary_dict(self, print_subelectrodes=True) -> dict:
        ...
        for pair in self.voltage_pairs:
            ...
            for idx, coeff in enumerate(rxn.coeffs):
                if abs(coeff) > 1e-05 and rxn.all_comp[idx] not in comps:
                    comps.append(rxn.all_comp[idx])
                if abs(coeff) > 1e-05 and rxn.all_comp[idx].reduced_formula != dct['working_ion']:
