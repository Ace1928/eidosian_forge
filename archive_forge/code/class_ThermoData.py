...
class ThermoData:
    ...
    def __init__(self, data_type, cpdname, phaseinfo, formula, value, ref='', method='', temp_range=(298, 298), uncertainty=None):
        """
        Args:
    ...
    def from_dict(cls, dct: dict) -> Self:
        ...
        Returns:
            ThermoData
        """
        return cls(dct['type'], dct['compound_name'], dct['phaseinfo'], dct['formula'], dct['value'], dct['ref'], dct['method'], dct['temp_range'], dct.get('uncertainty'))

    def as_dict(self):
        """Returns: MSONable dict."""
        return {'@module': type(self).__module__, '@class': type(self).__name__, 'type': self.type, 'formula': self.formula, 'compound_name': self.compound_name, 'phaseinfo': self.phaseinfo, 'value': self.value, 'temp_range': self.temp_range, 'method': self.method, 'ref': self.ref, 'uncertainty': self.uncertainty}
    ...
    def __str__(self):
        return f'{self.type}_{self.formula}_{self.phaseinfo} = {self.value}, Valid T : {self.temp_range}, Ref = {self.ref}'