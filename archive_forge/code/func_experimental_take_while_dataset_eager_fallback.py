...
def experimental_take_while_dataset_eager_fallback(input_dataset: _atypes.TensorFuzzingAnnotation[_atypes.Variant], other_arguments, predicate, output_types, output_shapes, name, ctx) -> _atypes.TensorFuzzingAnnotation[_atypes.Variant]:
    ...
    _attrs = ('predicate', predicate, 'Targuments', _attr_Targuments, 'output_types', output_types, 'output_shapes', output_shapes)
    _result = _execute.execute(b'ExperimentalTakeWhileDataset', 1, inputs=_inputs_flat, attrs=_attrs, ctx=ctx, name=name)
