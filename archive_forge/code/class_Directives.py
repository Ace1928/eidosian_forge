...
class Directives:
    ...
    def __init__(self, start_block_pattern, end_block_pattern):
        self.__directives_block_pattern = re.compile(start_block_pattern + ' beautify( \\w+[:]\\w+)+ ' + end_block_pattern)
        self.__directive_pattern = re.compile(' (\\w+)[:](\\w+)')
        self.__directives_end_ignore_pattern = re.compile(start_block_pattern + '\\sbeautify\\signore:end\\s' + end_block_pattern)
