...
class BertClassifierAgent(TorchClassifierAgent):
    ...
    def add_cmdline_args(parser):
        ...
        parser.add_argument('--type-optimization', type=str, default='all_encoder_layers', choices=['additional_layers', 'top_layer', 'top4_layers', 'all_encoder_layers', 'all'], help='which part of the encoders do we optimize (defaults to all layers)')
        parser.add_argument('--add-cls-token', type='bool', default=True, help='add [CLS] token to text vec')
        parser.add_argument('--sep-last-utt', type='bool', default=False, help='separate the last utterance into a differentsegment with [SEP] token in between')
        parser.set_defaults(dict_maxexs=0)
