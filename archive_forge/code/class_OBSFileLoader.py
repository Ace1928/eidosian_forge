...
class OBSFileLoader(BaseLoader):
    ...
    def __init__(self, bucket: str, key: str, client: Any=None, endpoint: str='', config: Optional[dict]=None) -> None:
        ...
        try:
            ...
            raise ImportError('Could not import esdk-obs-python python package. Please install it with `pip install esdk-obs-python`.')
        if not client:
            if not endpoint:
            ...
            if config.get('get_token_from_ecs'):
                ...
                client = ObsClient(access_key_id=config.get('ak'), secret_access_key=config.get('sk'), security_token=config.get('token'), server=endpoint)
