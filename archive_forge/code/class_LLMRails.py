...
class LLMRails(VectorStore):
    ...
    def __init__(self, datastore_id: Optional[str]=None, api_key: Optional[str]=None):
    ...
    def add_texts(self, texts: Iterable[str], metadatas: Optional[List[dict]]=None, **kwargs: Any) -> List[str]:
        ...
        for text in texts:
            ...
            response = self._session.post(f'{self.base_url}/datastores/{self._datastore_id}/text', json={'name': doc_name, 'text': text}, verify=True, headers=self._get_post_headers())
            if response.status_code != 200:
                logging.error(f'Create request failed for doc_name = {doc_name} with status code {response.status_code}, reason {response.reason}, text {response.text}')
                return names
            names.append(doc_name)
        return names

    def add_files(self, files_list: Iterable[str], metadatas: Optional[List[dict]]=None, **kwargs: Any) -> bool:
        """
        ...
        response = self._session.post(f'{self.base_url}/datastores/{self._datastore_id}/file', files=files, verify=True, headers=self._get_post_headers())
        if response.status_code != 200:
            logging.error(f'Create request failed for datastore = {self._datastore_id} with status code {response.status_code}, reason {response.reason}, text {response.text}')
            return False
        return True
    ...
    def similarity_search_with_score(self, query: str, k: int=5) -> List[Tuple[Document, float]]:
        ...
        """
        response = self._session.post(headers=self._get_post_headers(), url=f'{self.base_url}/datastores/{self._datastore_id}/search', data=json.dumps({'k': k, 'text': query}), timeout=10)
        if response.status_code != 200:
            logging.error('Query failed %s', f'(code {response.status_code}, reason {response.reason}, details {response.text})')
            return []
        results = response.json()['results']
        docs = [(Document(page_content=x['text'], metadata={key: value for key, value in x['metadata'].items() if key != 'score'}), x['metadata']['score']) for x in results]
        return docs
    ...
    @classmethod
    def from_texts(cls, texts: List[str], embedding: Optional[Embeddings]=None, metadatas: Optional[List[dict]]=None, **kwargs: Any) -> LLMRails:
