...
class Saml2Client(Base):
    ...
    def do_logout(self, name_id, entity_ids, reason, expire, sign=None, expected_binding=None, sign_alg=None, digest_alg=None, **kwargs):
        """
        ...
        for entity_id in entity_ids:
            ...
            bindings_slo_preferred_and_supported = (binding for binding in bindings_slo_preferred if binding in bindings_slo_supported)
            bindings_slo_choices = filter(lambda x: x, (expected_binding, *bindings_slo_preferred_and_supported, *bindings_slo_supported))
            binding = next(bindings_slo_choices, None)
            if not binding:
                logger.info({'message': 'Entity does not support SLO', 'entity': entity_id})
                continue
            service_info = bindings_slo_supported[binding]
            service_location = next(locations(service_info), None)
            if not service_location:
                logger.info({'message': 'Entity SLO service does not have a location', 'entity': entity_id, 'service_location': service_location})
                continue
            try:
            ...
            sign_redirect = sign and binding == BINDING_HTTP_REDIRECT
            sign_post = sign and (not sign_redirect)
            log_report = {'message': 'Invoking SLO on entity', 'entity': entity_id, 'binding': binding, 'location': service_location, 'session_indexes': session_indexes, 'sign': sign}
            logger.info(log_report)
            req_id, request = self.create_logout_request(service_location, entity_id, name_id=name_id, reason=reason, expire=expire, session_indexes=session_indexes, sign=sign_post, sign_alg=sign_alg, digest_alg=digest_alg)
            relay_state = self._relay_state(req_id)
            http_info = self.apply_binding(binding, str(request), service_location, relay_state, sign=sign_redirect, sigalg=sign_alg)
            if binding == BINDING_SOAP:
                response = self.send(**http_info)
                if response and response.status_code == 200:
                    not_done.remove(entity_id)
                    response_text = response.text
                    log_report_response = {**log_report, 'message': 'Response from SLO service', 'response_text': response_text}
                    logger.debug(log_report_response)
                    res = self.parse_logout_request_response(response_text, binding)
                    responses[entity_id] = res
                else:
                    log_report_response = {**log_report, 'message': 'Bad status_code response from SLO service', 'status_code': response and response.status_code}
                    logger.info(log_report_response)
            else:
                self.state[req_id] = {'entity_id': entity_id, 'operation': 'SLO', 'entity_ids': entity_ids, 'name_id': code(name_id), 'reason': reason, 'not_on_or_after': expire, 'sign': sign}
