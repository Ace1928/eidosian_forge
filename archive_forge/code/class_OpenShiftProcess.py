    ...
    def execute_module(self):
        ...
        if src or definition:
            ...
            try:
                ...
                self.fail_json(msg="Failed to retrieve Template with name '{0}' in namespace '{1}': {2}".format(name, namespace, exc.body), error=exc.status, status=exc.status, reason=exc.reason)
            except Exception as exc:
                self.fail_json(msg="Failed to retrieve Template with name '{0}' in namespace '{1}': {2}".format(name, namespace, to_native(exc)), error='', status='', reason='')
        else:
        ...
        try:
            ...
            self.fail_json(msg='Server failed to render the Template: {0}'.format(exc.body), error=exc.status, status=exc.status, reason=exc.reason)
        except Exception as exc:
            self.fail_json(msg='Server failed to render the Template: {0}'.format(to_native(exc)), error='', status='', reason='')
        result['message'] = ''
