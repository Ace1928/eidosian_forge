...
class RedisStatefulBackend(BaseStatefulBackend):
    ...
    def __init__(self, name: Optional[str]=None, expiration: Optional[int]=None, hset_disabled: Optional[bool]=False, keyjoin: Optional[str]=None, serializer: Optional[str]=None, serializer_kwargs: Optional[Dict[str, Any]]=None, base_key: Optional[str]=None, async_enabled: Optional[bool]=False, settings: Optional['BaseSettings']=None, **kwargs):
        """
        ...
        super().__init__(serializer=serializer, serializer_kwargs=serializer_kwargs, base_key=base_key, async_enabled=async_enabled, settings=settings, **kwargs)
        if name is not None:
            self.name = name
        if expiration is not None:
            self.expiration = expiration
        self.hset_enabled = not hset_disabled and self.base_key is not None
        if keyjoin is not None:
            self.keyjoin = keyjoin
        self.cache: 'KeyDBSession' = get_keydb_session(name=self.name, **_kdb_kwargs)
    ...
    def set(self, key: str, value: Any, ex: Optional[int]=None, _raw: Optional[bool]=None, **kwargs) -> None:
        ...
        if self.hset_enabled:
            ...
            if ex is not None:
                self.cache.expire(self.base_key, ex)
        else:
    ...
    def clear(self, *keys, **kwargs):
        ...
        if self.hset_enabled:
            if keys:
                ...
                self.cache.delete(self.base_key)
        elif keys:
    ...
    async def aget(self, key: str, default: Optional[Any]=None, _raw: Optional[bool]=None, **kwargs) -> Optional[Any]:
        ...
        """
        if self.hset_enabled:
            value = await self.cache.async_hget(self.base_key, key)
        else:
            value = await self.cache.async_client.get(self.get_key(key))
        if value is None:
            return default
        try:
            return self.decode_value(value, _raw=_raw, **kwargs)
    ...
    async def aclear(self, *keys, **kwargs):
        ...
        if self.hset_enabled:
            if keys:
                await self.cache.async_hdel(self.base_key, *keys)
            else:
                await self.cache.async_delete(self.base_key)
        elif keys:
            keys = [self.get_key(key) for key in keys]
            ...
            if keys:
                await self.cache.async_client.delete(*keys)
    ...
    async def acontains(self, key, **kwargs):
        ...
        """
        if self.hset_enabled:
            return await self.cache.async_hexists(self.base_key, key)
