...
def quantized_resize_bilinear(images: _atypes.TensorFuzzingAnnotation[TV_QuantizedResizeBilinear_T], size: _atypes.TensorFuzzingAnnotation[_atypes.Int32], min: _atypes.TensorFuzzingAnnotation[_atypes.Float32], max: _atypes.TensorFuzzingAnnotation[_atypes.Float32], align_corners: bool=False, half_pixel_centers: bool=False, name=None):
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'QuantizedResizeBilinear', name, images, size, min, max, 'align_corners', align_corners, 'half_pixel_centers', half_pixel_centers)
            ...
            pass
        try:
            return quantized_resize_bilinear_eager_fallback(images, size, min, max, align_corners=align_corners, half_pixel_centers=half_pixel_centers, name=name, ctx=_ctx)
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('QuantizedResizeBilinear', images=images, size=size, min=min, max=max, align_corners=align_corners, half_pixel_centers=half_pixel_centers, name=name)
    _result = _outputs[:]
