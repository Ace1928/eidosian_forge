...
class PandasLikeUtils(Generic[T, ColT]):
    ...
    def ensure_compatible(self, df: T) -> None:
        ...
        if self.empty(df):
            return
        raise ValueError(f'pandas like datafame must have default index, but got {type(df.index)}')
    ...
    def join(self, ndf1: T, ndf2: T, join_type: str, on: List[str], anti_indicator_col: str=_ANTI_INDICATOR, cross_indicator_col: str=_CROSS_INDICATOR) -> T:
        """Join two dataframes
        ...
        if join_type == 'inner':
            ...
            joined = joined[joined[anti_indicator_col].isnull()].drop([anti_indicator_col], axis=1)
            ...
            assert_or_throw(len(on) == 0, ValueError(f"cross join can't have join keys {on}"))
            ndf1 = self._with_indicator(ndf1, cross_indicator_col)
            ndf2 = self._with_indicator(ndf2, cross_indicator_col)
            joined = ndf1.merge(ndf2, how='inner', on=[cross_indicator_col]).drop([cross_indicator_col], axis=1)
        else:
            raise NotImplementedError(join_type)
        return joined

    def concat_dfs(self, *dfs: T) -> T:
