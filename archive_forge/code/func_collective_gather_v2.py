...
def collective_gather_v2(input: _atypes.TensorFuzzingAnnotation[TV_CollectiveGatherV2_T], group_size: _atypes.TensorFuzzingAnnotation[_atypes.Int32], group_key: _atypes.TensorFuzzingAnnotation[_atypes.Int32], instance_key: _atypes.TensorFuzzingAnnotation[_atypes.Int32], ordering_token: List[_atypes.TensorFuzzingAnnotation[_atypes.Resource]], communication_hint: str='auto', timeout_seconds: float=0, name=None) -> _atypes.TensorFuzzingAnnotation[TV_CollectiveGatherV2_T]:
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'CollectiveGatherV2', name, input, group_size, group_key, instance_key, ordering_token, 'communication_hint', communication_hint, 'timeout_seconds', timeout_seconds)
            return _result
            ...
            pass
        try:
            return collective_gather_v2_eager_fallback(input, group_size, group_key, instance_key, ordering_token, communication_hint=communication_hint, timeout_seconds=timeout_seconds, name=name, ctx=_ctx)
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('CollectiveGatherV2', input=input, group_size=group_size, group_key=group_key, instance_key=instance_key, ordering_token=ordering_token, communication_hint=communication_hint, timeout_seconds=timeout_seconds, name=name)
    _result = _outputs[:]
