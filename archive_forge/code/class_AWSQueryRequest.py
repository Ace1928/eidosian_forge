...
class AWSQueryRequest(object):
    ...
    Filters = []
    Response = {}
    CLITypeMap = {'string': 'string', 'integer': 'int', 'int': 'int', 'enum': 'choice', 'datetime': 'string', 'dateTime': 'string', 'file': 'string', 'boolean': None}
            ...
            for p in self.Params + self.Args:
                if p.name in required:
                    if p.short_name and p.long_name:
                        l.append('(%s, %s)' % (p.optparse_short_name, p.optparse_long_name))
                    elif p.short_name:
