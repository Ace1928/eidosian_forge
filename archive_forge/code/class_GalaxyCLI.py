...
class GalaxyCLI(CLI):
    ...
    def __init__(self, args):
        ...
        if len(args) > 1:
            ...
            if args[1:3] == ['role', 'login']:
                display.error('The login command was removed in late 2020. An API key is now required to publish roles or collections to Galaxy. The key can be found at https://galaxy.ansible.com/me/preferences, and passed to the ansible-galaxy CLI via a file at {0} or (insecurely) via the `--token` command-line argument.'.format(to_text(C.GALAXY_TOKEN_PATH)))
                sys.exit(1)
        self.api_servers = []
        self.galaxy = None
    ...
    def init_parser(self):
        ...
        common.add_argument('--api-version', type=int, choices=[2, 3], help=argparse.SUPPRESS)
        common.add_argument('--token', '--api-key', dest='api_key', help='The Ansible Galaxy API key which can be found at https://galaxy.ansible.com/me/preferences.')
        ...
        common.add_argument('--timeout', dest='timeout', type=int, help='The time to wait for operations against the galaxy server, defaults to 60s.')
        ...
        collections_path.add_argument('-p', '--collections-path', dest='collections_path', type=opt_help.unfrack_path(pathsep=True), action=opt_help.PrependListAction, help="One or more directories to search for collections in addition to the default COLLECTIONS_PATHS. Separate multiple paths with '{0}'.".format(os.path.pathsep))
        cache_options = opt_help.ArgumentParser(add_help=False)
        cache_options.add_argument('--clear-response-cache', dest='clear_response_cache', action='store_true', default=False, help='Clear the existing server response cache.')
        cache_options.add_argument('--no-cache', dest='no_cache', action='store_true', default=False, help='Do not use the server response cache.')
        type_parser = self.parser.add_subparsers(metavar='TYPE', dest='type')
        type_parser.required = True
        collection = type_parser.add_parser('collection', help='Manage an Ansible Galaxy collection.')
        collection.set_defaults(func=self.execute_collection)
        ...
        role.set_defaults(func=self.execute_role)
    ...
    def add_list_options(self, parser, parents=None):
        ...
        list_parser = parser.add_parser('list', parents=parents, help='Show the name and version of each {0} installed in the {0}s_path.'.format(galaxy_type))
        list_parser.set_defaults(func=self.execute_list)
        list_parser.add_argument(galaxy_type, help=galaxy_type.capitalize(), nargs='?', metavar=galaxy_type)
        if galaxy_type == 'collection':
            list_parser.add_argument('--format', dest='output_format', choices=('human', 'yaml', 'json'), default='human', help='Format to display the list of collections in.')
    ...
    def add_verify_options(self, parser, parents=None):
        galaxy_type = 'collection'
        verify_parser = parser.add_parser('verify', parents=parents, help='Compare checksums with the collection(s) found on the server and the installed copy. This does not verify dependencies.')
        verify_parser.set_defaults(func=self.execute_verify)
        verify_parser.add_argument('args', metavar='{0}_name'.format(galaxy_type), nargs='*', help='The installed collection(s) name. This is mutually exclusive with --requirements-file.')
        verify_parser.add_argument('-i', '--ignore-errors', dest='ignore_errors', action='store_true', default=False, help='Ignore errors during verification and continue with the next specified collection.')
        verify_parser.add_argument('--offline', dest='offline', action='store_true', default=False, help='Validate collection integrity locally without contacting server for canonical manifest hash.')
        verify_parser.add_argument('-r', '--requirements-file', dest='requirements', help='A file containing a list of collections to be verified.')
        verify_parser.add_argument('--keyring', dest='keyring', default=C.GALAXY_GPG_KEYRING, help='The keyring used during signature verification')
        verify_parser.add_argument('--signature', dest='signatures', action='append', help='An additional signature source to verify the authenticity of the MANIFEST.json before using it to verify the rest of the contents of a collection from a Galaxy server. Use in conjunction with a positional collection name (mutually exclusive with --requirements-file).')
        valid_signature_count_help = 'The number of signatures that must successfully verify the collection. This should be a positive integer or all to signify that all signatures must be used to verify the collection. Prepend the value with + to fail if no valid signatures are found for the collection (e.g. +all).'
        ...
        verify_parser.add_argument('--required-valid-signature-count', dest='required_valid_signature_count', type=validate_signature_count, help=valid_signature_count_help, default=C.GALAXY_REQUIRED_VALID_SIGNATURE_COUNT)
        verify_parser.add_argument('--ignore-signature-status-code', dest='ignore_gpg_errors', type=str, action='append', help=opt_help.argparse.SUPPRESS, default=C.GALAXY_IGNORE_INVALID_SIGNATURE_STATUS_CODES, choices=list(GPG_ERROR_MAP.keys()))
        verify_parser.add_argument('--ignore-signature-status-codes', dest='ignore_gpg_errors', type=str, action='extend', nargs='+', help=ignore_gpg_status_help, default=C.GALAXY_IGNORE_INVALID_SIGNATURE_STATUS_CODES, choices=list(GPG_ERROR_MAP.keys()))
    ...
    def add_build_options(self, parser, parents=None):
        build_parser = parser.add_parser('build', parents=parents, help='Build an Ansible collection artifact that can be published to Ansible Galaxy.')
    ...
    def run(self):
        ...
        for server_priority, server_key in enumerate(server_list, start=1):
            ...
            if api_version:
                display.warning(f'The specified "api_version" configuration for the galaxy server "{server_key}" is not a public configuration, and may be removed at any time without warning.')
        ...
        if context.CLIARGS['api_version']:
            ...
            display.warning('The --api-version is not a public argument, and may be removed at any time without warning.')
        ...
        if cmd_server:
            ...
            if config_server:
                ...
                self.api_servers.append(GalaxyAPI(self.galaxy, 'cmd_arg', cmd_server, token=cmd_token, priority=len(config_servers) + 1, validate_certs=validate_certs, timeout=default_server_timeout, **galaxy_options))
        else:
            self.api_servers = config_servers
        if len(self.api_servers) == 0:
            self.api_servers.append(GalaxyAPI(self.galaxy, 'default', C.GALAXY_SERVER, token=cmd_token, priority=0, validate_certs=validate_certs, timeout=default_server_timeout, **galaxy_options))
    ...
    def _parse_requirements_file(self, requirements_file, allow_old_format=True, artifacts_manager=None, validate_signature_options=True):
        ...
        if isinstance(file_requirements, list):
            ...
            for role_req in file_requirements.get('roles') or []:
                requirements['roles'] += parse_role_req(role_req)
            requirements['collections'] = [Requirement.from_requirement_dict(self._init_coll_req_dict(collection_req), artifacts_manager, validate_signature_options) for collection_req in file_requirements.get('collections') or []]
        else:
    ...
    def execute_build(self):
        ...
        for collection_path in context.CLIARGS['args']:
            ...
            build_collection(to_text(collection_path, errors='surrogate_or_strict'), to_text(output_path, errors='surrogate_or_strict'), force)
