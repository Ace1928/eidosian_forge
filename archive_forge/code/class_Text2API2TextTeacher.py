...
class Text2API2TextTeacher(DialogTeacher):
    ...
    def __init__(self, opt: Opt, shared=None):
        ...
        if shared is None:
            warn_once('Google SGD is a beta dataset, and format may significantly change.')
            build_.build(opt)
    ...
    def setup_data(self, fold):
        ...
        for dialog in dialogs:
            ...
            for turn_id in range(0, num_turns, 2):
                ...
                api_call, api_results = self._get_api_call_and_results(sys_turn, schema_lookup)
                call_str = self._api_dict_to_str(api_call)
                resp_str = self._api_dict_to_str(api_results)
                if not api_call and (not api_results):
                    yield ({'text': user_turn['utterance'], 'label': sys_turn['utterance'], 'type': 'text'}, is_first_turn)
                elif not api_call and api_results:
                    yield ({'text': f'{user_turn['utterance']} api_resp: {resp_str}', 'label': sys_turn['utterance'], 'type': 'apiresp', 'slots': api_results}, is_first_turn)
                elif api_call and api_results:
                    yield ({'text': user_turn['utterance'], 'label': f'apicall: {call_str}', 'type': 'apicall', 'slots': api_call}, is_first_turn)
                    yield ({'text': f'api_resp: {resp_str}', 'label': sys_turn['utterance'], 'type': 'apiresp', 'slots': api_results}, False)
                else:
                    assert api_call and api_results, 'API call without API results! Check Dataset!'