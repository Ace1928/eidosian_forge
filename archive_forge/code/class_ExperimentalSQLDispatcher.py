...
class ExperimentalSQLDispatcher(SQLDispatcher):
    ...
    def _read(cls, sql, con, index_col, coerce_float, params, parse_dates, columns, chunksize, dtype_backend, dtype, partition_column, lower_bound, upper_bound, max_sessions):
        ...
        if not is_distributed(partition_column, lower_bound, upper_bound):
            ...
            return cls.base_read(sql, con, index_col, coerce_float=coerce_float, params=params, parse_dates=parse_dates, columns=columns, chunksize=chunksize, dtype_backend=dtype_backend, dtype=dtype)
