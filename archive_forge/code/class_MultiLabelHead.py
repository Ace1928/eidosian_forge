...
class MultiLabelHead(base_head.Head):
  ...
  * If `label_vocabulary` is given, a string `SparseTensor`. The `dense_shape`
  ...
  """
    ...
    def __init__(self, n_classes, weight_column=None, thresholds=None, label_vocabulary=None, loss_reduction=tf.losses.Reduction.SUM_OVER_BATCH_SIZE, loss_fn=None, classes_for_class_based_metrics=None, name=None):
        if n_classes is None or n_classes < 2:
            raise ValueError('n_classes must be > 1 for multi-label classification. Given: {}'.format(n_classes))
        thresholds = tuple(thresholds) if thresholds else tuple()
        for threshold in thresholds:
            if threshold <= 0.0 or threshold >= 1.0:
                raise ValueError('thresholds must be in (0, 1) range. Given: {}'.format(threshold))
        if label_vocabulary is not None:
            if not isinstance(label_vocabulary, (list, tuple)):
                raise ValueError('label_vocabulary must be a list or tuple. Given type: {}'.format(type(label_vocabulary)))
            if len(label_vocabulary) != n_classes:
                raise ValueError('Length of label_vocabulary must be n_classes ({}). Given: {}'.format(n_classes, len(label_vocabulary)))
        if loss_fn:
        ...
        if classes_for_class_based_metrics:
            ...
            if isinstance(classes_for_class_based_metrics[0], six.string_types):
                if not label_vocabulary:
                    raise ValueError('label_vocabulary must be provided when classes_for_class_based_metrics are strings.')
                class_ids = []
                ...
                for class_id in classes_for_class_based_metrics:
                    if class_id < 0 or class_id >= n_classes:
                        raise ValueError('All classes_for_class_based_metrics must be in range [0, {}]. Given: {}'.format(n_classes - 1, class_id))
        else:
        ...
        for class_id in self._classes_for_class_based_metrics:
            if self._label_vocabulary is None:
                ...
                prob_key = keys.PROBABILITY_MEAN_AT_NAME % self._label_vocabulary[class_id]
    ...
    def loss(self, labels, logits, features=None, mode=None, regularization_losses=None):
        ...
        with ops.name_scope('losses', values=(logits, labels, regularization_losses, features)):
            ...
            unweighted_loss, weights = self._unweighted_loss_and_weights(logits, processed_labels, features)
    ...
    """
        ...
        with ops.name_scope('predictions', values=(logits,)):
            ...
            if pred_keys.CLASSES in keys:
                predictions[pred_keys.CLASSES] = base_head.all_classes(logits, self._n_classes, self._label_vocabulary)
            return predictions
    ...
    def metrics(self, regularization_losses=None):
        ...
        with ops.name_scope(None, 'metrics', (regularization_losses,)):
            ...
            for i in range(len(self._classes_for_class_based_metrics)):
                eval_metrics[self._prob_keys[i]] = tf.keras.metrics.Mean(name=self._prob_keys[i])
                eval_metrics[self._auc_keys[i]] = tf.keras.metrics.AUC(name=self._auc_keys[i])
                eval_metrics[self._auc_pr_keys[i]] = tf.keras.metrics.AUC(curve='PR', name=self._auc_pr_keys[i])
        return eval_metrics
    ...
    def update_metrics(self, eval_metrics, features, logits, labels, regularization_losses=None):
        ...
        for i in range(len(self._thresholds)):
            eval_metrics[self._accuracy_keys[i]].update_state(y_true=processed_labels, y_pred=probabilities, sample_weight=weights)
            eval_metrics[self._precision_keys[i]].update_state(y_true=processed_labels, y_pred=probabilities, sample_weight=weights)
            eval_metrics[self._recall_keys[i]].update_state(y_true=processed_labels, y_pred=probabilities, sample_weight=weights)
        for i, class_id in enumerate(self._classes_for_class_based_metrics):
            batch_rank = tf.rank(probabilities) - 1
            begin = tf.concat([tf.zeros([batch_rank], dtype=tf.dtypes.int32), [class_id]], axis=0)
            size = tf.concat([-1 * tf.ones([batch_rank], dtype=tf.dtypes.int32), [1]], axis=0)
            ...
            base_head.update_metric_with_broadcast_weights(eval_metrics[self._prob_keys[i]], class_probabilities, weights)
            eval_metrics[self._auc_keys[i]].update_state(y_true=class_labels, y_pred=class_probabilities, sample_weight=weights)
            eval_metrics[self._auc_pr_keys[i]].update_state(y_true=class_labels, y_pred=class_probabilities, sample_weight=weights)
        return eval_metrics
    ...
    """
        with ops.name_scope(self._name, 'head'):
            ...
            if mode == ModeKeys.PREDICT:
                ...
                classifier_output = base_head.classification_output(scores=probabilities, n_classes=self._n_classes, label_vocabulary=self._label_vocabulary)
                return model_fn._TPUEstimatorSpec(mode=ModeKeys.PREDICT, predictions=predictions, export_outputs={base_head.DEFAULT_SERVING_KEY: classifier_output, base_head.CLASSIFY_SERVING_KEY: classifier_output, base_head.PREDICT_SERVING_KEY: export_output.PredictOutput(predictions)})
