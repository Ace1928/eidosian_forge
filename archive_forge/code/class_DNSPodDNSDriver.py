...
class DNSPodDNSDriver(DNSDriver):
    ...
    def create_record(self, name, zone, type, data, extra=None):
        ...
        data = {'sub_domain': name, 'value': data, 'record_type': type, 'domain_id': zone.id}
        if extra is not None:
            data.update(extra)
        try:
            response = self._make_request(action=action, method='POST', data=data)
    ...
    def _to_zone(self, item):
        ...
        zone = Zone(domain=item.get('name') or item.get('domain'), id=item.get('id'), type=None, extra=extra, ttl=item.get('ttl'), driver=self)
        return zone
    ...
    def _to_record(self, item, zone):
        ...
        for key in item:
            if key not in common_attr:
                extra[key] = item.get(key)
        record = Record(id=item.get('id'), name=item.get('name') or item.get('sub_domain'), type=item.get('type') or item.get('record_type'), data=item.get('value'), zone=zone, driver=self, extra=extra)
        return record
