...
class BaseTraitedSpec(traits.HasTraits):
    ...
    * get_hashval : returns a tuple containing the state of the trait as a dict
    ...
    def _deprecated_warn(self, obj, name, old, new):
        ...
        if isdefined(new):
            trait_spec = self.traits()[name]
            msg1 = 'Input %s in interface %s is deprecated.' % (name, self.__class__.__name__.split('InputSpec')[0])
            msg2 = 'Will be removed or raise an error as of release %s' % trait_spec.deprecated
            if trait_spec.new_name:
                if trait_spec.new_name not in self.copyable_trait_names():
                    raise TraitError(msg1 + ' Replacement trait %s not found' % trait_spec.new_name)
            ...
            if Version(str(trait_spec.deprecated)) < self.package_version:
                ...
                if trait_spec.new_name:
                    msg += 'Unsetting old value %s; setting new value %s.' % (name, trait_spec.new_name)
                warn(msg)
                if trait_spec.new_name:
                    self.trait_set(trait_change_notify=False, **{'%s' % name: Undefined, '%s' % trait_spec.new_name: new})
