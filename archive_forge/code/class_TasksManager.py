...
class TasksManager:
    ...
    @staticmethod
    def get_model_files(model_name_or_path: Union[str, Path], subfolder: str='', cache_dir: str=huggingface_hub.constants.HUGGINGFACE_HUB_CACHE):
        request_exception = None
        full_model_path = Path(model_name_or_path) / subfolder
        if full_model_path.is_dir():
            all_files = [os.path.relpath(os.path.join(dirpath, file), full_model_path) for dirpath, _, filenames in os.walk(full_model_path) for file in filenames]
        else:
            try:
                ...
                if full_model_path.is_dir():
                    ...
                    all_files = [os.path.relpath(os.path.join(dirpath, file), cached_path) for dirpath, _, filenames in os.walk(cached_path) for file in filenames]
        return (all_files, request_exception)

    @staticmethod
    def determine_framework(model_name_or_path: Union[str, Path], subfolder: str='', framework: Optional[str]=None, cache_dir: str=huggingface_hub.constants.HUGGINGFACE_HUB_CACHE) -> str:
        """
        ...
        is_pt_weight_file = [file.startswith(pt_weight_name) and file.endswith(pt_weight_extension) or (file.startswith(safe_weight_name) and file.endswith(safe_weight_extension)) for file in all_files]
        ...
        if any(is_pt_weight_file):
            ...
            framework = 'pt'
        elif request_exception is not None:
            raise RequestsConnectionError(f'The framework could not be automatically inferred. If using the command-line, please provide the argument --framework (pt,tf) Detailed error: {request_exception}')
        else:
            raise FileNotFoundError(f'Cannot determine framework from given checkpoint location. There should be a {Path(WEIGHTS_NAME).stem}*{Path(WEIGHTS_NAME).suffix} for PyTorch or {Path(TF2_WEIGHTS_NAME).stem}*{Path(TF2_WEIGHTS_NAME).suffix} for TensorFlow.')
        if is_torch_available():
