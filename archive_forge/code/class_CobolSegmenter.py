...
class CobolSegmenter(CodeSegmenter):
    ...
    PARAGRAPH_PATTERN = re.compile('^[A-Z0-9\\-]+(\\s+.*)?\\.$', re.IGNORECASE)
    DIVISION_PATTERN = re.compile('^\\s*(IDENTIFICATION|DATA|PROCEDURE|ENVIRONMENT)\\s+DIVISION.*$', re.IGNORECASE)
    SECTION_PATTERN = re.compile('^\\s*[A-Z0-9\\-]+\\s+SECTION.$', re.IGNORECASE)
    ...
    def simplify_code(self) -> str:
        ...
        for line in self.source_lines:
            is_header = 'PROCEDURE DIVISION' in line or 'DATA DIVISION' in line or 'IDENTIFICATION DIVISION' in line or self.PARAGRAPH_PATTERN.match(line.strip().split(' ')[0]) or self.SECTION_PATTERN.match(line.strip())
            if is_header:
                inside_relevant_section = True
                omitted_code_added = False
