...
def leaky_relu_grad(gradients: _atypes.TensorFuzzingAnnotation[TV_LeakyReluGrad_T], features: _atypes.TensorFuzzingAnnotation[TV_LeakyReluGrad_T], alpha: float=0.2, name=None) -> _atypes.TensorFuzzingAnnotation[TV_LeakyReluGrad_T]:
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'LeakyReluGrad', name, gradients, features, 'alpha', alpha)
            return _result
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('LeakyReluGrad', gradients=gradients, features=features, alpha=alpha, name=name)
    _result = _outputs[:]
