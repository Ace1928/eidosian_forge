...
def get_actions(self, indices: Union[int, List[int]]=-1, global_ts: bool=True, as_list: bool=False) -> Union[MultiAgentDict, List[MultiAgentDict]]:
    ...
    if global_ts:
        if isinstance(indices, list):
            indices = [self.t + idx + 1 if idx < 0 else idx + self.ts_carriage_return for idx in indices]
        else:
            indices = [self.t + indices + 1] if indices < 0 else [indices + self.ts_carriage_return]
    ...
    for agent_id, agent_global_action_t in self.global_actions_t.items():
        ...
        if agent_global_action_t and (last_action_index in indices or -1 in indices) and self.agent_buffers[agent_id]['actions'].full():
            buffered_actions[agent_id] = [self.agent_buffers[agent_id]['actions'].queue[0]]
        else:
            buffered_actions[agent_id] = []
    actions = self._getattr_by_index('actions', indices=indices, has_initial_value=True, global_ts=global_ts, global_ts_mapping=self.global_actions_t, as_list=as_list, buffered_values=buffered_actions)
    return actions