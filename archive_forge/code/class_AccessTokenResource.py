...
class AccessTokenResource(ks_flask.ResourceBase):
    ...
    def post(self):
        ...
        if grant_type is None:
            error = exception.OAuth2InvalidRequest(int(http.client.BAD_REQUEST), http.client.responses[http.client.BAD_REQUEST], _('The parameter grant_type is required.'))
            LOG.info(f'Get OAuth2.0 Access Token API: {error.message_format}')
            raise error
        if grant_type != 'client_credentials':
            error = exception.OAuth2UnsupportedGrantType(int(http.client.BAD_REQUEST), http.client.responses[http.client.BAD_REQUEST], _('The parameter grant_type %s is not supported.') % grant_type)
            LOG.info(f'Get OAuth2.0 Access Token API: {error.message_format}')
            raise error
        auth_method = ''
        ...
        if not client_id:
            ...
            LOG.info('Get OAuth2.0 Access Token API: failed to get a client_id from the request.')
            raise error
        if client_cert:
        ...
        error = exception.OAuth2InvalidClient(int(http.client.UNAUTHORIZED), http.client.responses[http.client.UNAUTHORIZED], _('Client authentication failed.'))
        LOG.info('Get OAuth2.0 Access Token API: failed to get client credentials from the request.')
        raise error
