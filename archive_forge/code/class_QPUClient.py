...
class QPUClient:
    ...
    def __init__(self, *, quantum_processor_id: str, engagement_manager: EngagementManager, endpoint_id: Optional[str]=None, request_timeout: float=10.0) -> None:
        """
    ...
    def get_buffers(self, request: GetBuffersRequest) -> GetBuffersResponse:
        ...
        Get job buffers.
        """
        buffs = self._rpcq_request('get_buffers', job_id=request.job_id, wait=request.wait)
        return GetBuffersResponse(buffers={name: BufferResponse(shape=cast(Tuple[int, int], tuple(val['shape'])), dtype=val['dtype'], data=val['data']) for name, val in buffs.items()})
