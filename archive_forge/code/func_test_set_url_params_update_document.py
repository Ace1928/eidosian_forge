...
def test_set_url_params_update_document(page):
    ...
    expected_location = {'protocol': 'http:', 'hostname': 'localhost', 'port': f'{port}', 'pathname': '/', 'hash': '', 'reload': None}
    verify_document_location(expected_location, page)
    wait_until(lambda: parse_query(page.evaluate('() => document.location')['search']) == {'range_value': [1, 2], 'slider_value': 2, 'text_value': 'Simple Text'}, page)