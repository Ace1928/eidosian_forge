...
class Lantern(VectorStore):
    ...
    def __init__(self, connection_string: str, embedding_function: Embeddings, distance_strategy: DistanceStrategy=DEFAULT_DISTANCE_STRATEGY, collection_name: str=_LANGCHAIN_DEFAULT_COLLECTION_NAME, collection_metadata: Optional[dict]=None, pre_delete_collection: bool=False, logger: Optional[logging.Logger]=None, relevance_score_fn: Optional[Callable[[float], float]]=None) -> None:
    ...
    def _select_relevance_score_fn(self) -> Callable[[float], float]:
        ...
        if self.distance_strategy == DistanceStrategy.COSINE:
            ...
            raise ValueError(f'No supported normalization function for distance_strategy of {self._distance_strategy}.Consider providing relevance_score_fn to Lantern constructor.')
    ...
    def create_hnsw_index(self, dims: int=ADA_TOKEN_COUNT, m: int=16, ef_construction: int=64, ef_search: int=64, **_kwargs: Any) -> None:
        ...
        """
        create_index_query = sqlalchemy.text('CREATE INDEX IF NOT EXISTS {} ON {} USING hnsw (embedding {}) WITH (dim = :dim, m = :m, ef_construction = :ef_construction, ef = :ef);'.format(quoted_name(self._index_name, True), quoted_name(self.collection_name, True), self._get_op_class()))
        with Session(self._conn) as session:
            session.execute(create_index_query, {'dim': dims, 'm': m, 'ef_construction': ef_construction, 'ef': ef_search})
            session.commit()
    ...
    @classmethod
    def _initialize_from_embeddings(cls, texts: List[str], embeddings: List[List[float]], embedding: Embeddings, metadatas: Optional[List[dict]]=None, ids: Optional[List[str]]=None, collection_name: str=_LANGCHAIN_DEFAULT_COLLECTION_NAME, distance_strategy: DistanceStrategy=DEFAULT_DISTANCE_STRATEGY, pre_delete_collection: bool=False, **kwargs: Any) -> Lantern:
        """
        ...
        store = cls(connection_string=connection_string, collection_name=collection_name, embedding_function=embedding, pre_delete_collection=pre_delete_collection, distance_strategy=distance_strategy)
        store.add_embeddings(texts=texts, embeddings=embeddings, metadatas=metadatas, ids=ids, **kwargs)
        store.create_hnsw_index(**kwargs)
        return store

    def add_embeddings(self, texts: List[str], embeddings: List[List[float]], metadatas: List[dict], ids: List[str], **kwargs: Any) -> None:
        with Session(self._conn) as session:
            for text, metadata, embedding, id in zip(texts, metadatas, embeddings, ids):
                embedding_store = self.EmbeddingStore(embedding=embedding, document=text, cmetadata=metadata, custom_id=id)
    ...
    def __query_collection(self, embedding: List[float], k: int=4, filter: Optional[dict]=None) -> List[Any]:
        with Session(self._conn) as session:
            ...
            query = session.query(self.EmbeddingStore, getattr(func, self.distance_function)(self.EmbeddingStore.embedding, embedding).label('distance'))
            if filter_by is not None:
                query = query.filter(filter_by)
            results: List[QueryResult] = query.order_by(self.EmbeddingStore.embedding.op(self._get_operator())(embedding)).limit(k).all()
        return results
    ...
    @classmethod
    def from_texts(cls: Type[Lantern], texts: List[str], embedding: Embeddings, metadatas: Optional[List[dict]]=None, collection_name: str=_LANGCHAIN_DEFAULT_COLLECTION_NAME, distance_strategy: DistanceStrategy=DEFAULT_DISTANCE_STRATEGY, ids: Optional[List[str]]=None, pre_delete_collection: bool=False, **kwargs: Any) -> Lantern:
        """
        ...
        return cls._initialize_from_embeddings(texts, embeddings, embedding, metadatas=metadatas, ids=ids, collection_name=collection_name, pre_delete_collection=pre_delete_collection, distance_strategy=distance_strategy, **kwargs)

    @classmethod
    def from_embeddings(cls, text_embeddings: List[Tuple[str, List[float]]], embedding: Embeddings, metadatas: Optional[List[dict]]=None, collection_name: str=_LANGCHAIN_DEFAULT_COLLECTION_NAME, ids: Optional[List[str]]=None, pre_delete_collection: bool=False, distance_strategy: DistanceStrategy=DEFAULT_DISTANCE_STRATEGY, **kwargs: Any) -> Lantern:
    ...
    @classmethod
    def from_existing_index(cls: Type[Lantern], embedding: Embeddings, collection_name: str=_LANGCHAIN_DEFAULT_COLLECTION_NAME, pre_delete_collection: bool=False, distance_strategy: DistanceStrategy=DEFAULT_DISTANCE_STRATEGY, **kwargs: Any) -> Lantern:
        """
    ...
    @classmethod
    def from_documents(cls: Type[Lantern], documents: List[Document], embedding: Embeddings, collection_name: str=_LANGCHAIN_DEFAULT_COLLECTION_NAME, distance_strategy: DistanceStrategy=DEFAULT_DISTANCE_STRATEGY, ids: Optional[List[str]]=None, pre_delete_collection: bool=False, **kwargs: Any) -> Lantern:
        """
        ...
        return cls.from_texts(texts=texts, pre_delete_collection=pre_delete_collection, embedding=embedding, metadatas=metadatas, ids=ids, collection_name=collection_name, distance_strategy=distance_strategy, **kwargs)
