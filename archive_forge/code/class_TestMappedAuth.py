...
class TestMappedAuth(base.TestCase):
    ...
    def test_project_scoped_mapped_auth(self):
        ...
        plugin = kerberos.MappedKerberos(auth_url=self.TEST_V3_URL, protocol=self.protocol, identity_provider=self.identity_provider, project_id=scoped_body.project_id)
        sess = session.Session()
        tok = plugin.get_token(sess)
    ...
    def test_authenticate_with_mutual_authentication_disabled(self):
        ...
        scoped_body.set_project_scope()
        self.requests_mock.post('%s/auth/tokens' % self.TEST_V3_URL, json=scoped_body, headers={'X-Subject-Token': scoped_id, 'Content-Type': 'application/json'})
        plugin = kerberos.MappedKerberos(auth_url=self.TEST_V3_URL, protocol=self.protocol, identity_provider=self.identity_provider, project_id=scoped_body.project_id, mutual_auth='disabled')
        sess = session.Session()
        tok = plugin.get_token(sess)
        proj = plugin.get_project_id(sess)
        self.assertEqual(scoped_id, tok)
