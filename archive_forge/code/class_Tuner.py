...
class Tuner:
    ...
    def __init__(self, trainable: Optional[Union[str, Callable, Type[Trainable], 'BaseTrainer']]=None, *, param_space: Optional[Dict[str, Any]]=None, tune_config: Optional[TuneConfig]=None, run_config: Optional[RunConfig]=None, _tuner_kwargs: Optional[Dict]=None, _tuner_internal: Optional[TunerInternal]=None, _entrypoint: AirEntrypoint=AirEntrypoint.TUNER):
        ...
        if _tuner_internal:
            ...
            if not self._is_ray_client:
                ...
                self._remote_tuner = _force_on_current_node(ray.remote(num_cpus=0)(TunerInternal)).remote(**kwargs)

    @classmethod
    def restore(cls, path: str, trainable: Union[str, Callable, Type[Trainable], 'BaseTrainer'], resume_unfinished: bool=True, resume_errored: bool=False, restart_errored: bool=False, param_space: Optional[Dict[str, Any]]=None, storage_filesystem: Optional[pyarrow.fs.FileSystem]=None) -> 'Tuner':
        ...
        if not ray.util.client.ray.is_connected():
            tuner_internal = TunerInternal(restore_path=path, resume_config=resume_config, trainable=trainable, param_space=param_space, storage_filesystem=storage_filesystem)
            return Tuner(_tuner_internal=tuner_internal)
        else:
            tuner_internal = _force_on_current_node(ray.remote(num_cpus=0)(TunerInternal)).remote(restore_path=path, resume_config=resume_config, trainable=trainable, param_space=param_space, storage_filesystem=storage_filesystem)
            return Tuner(_tuner_internal=tuner_internal)

    @classmethod
    def can_restore(cls, path: Union[str, os.PathLike], storage_filesystem: Optional[pyarrow.fs.FileSystem]=None) -> bool:
    ...
    def fit(self) -> ResultGrid:
        ...
        if not self._is_ray_client:
            ...
            progress_reporter, string_queue = self._prepare_remote_tuner_for_jupyter_progress_reporting()
            try:
                fit_future = self._remote_tuner.fit.remote()
                _stream_client_output(fit_future, progress_reporter, string_queue)
    ...
    def get_results(self) -> ResultGrid:
        ...
        if not self._is_ray_client:
            ...
            _stream_client_output(get_results_future, progress_reporter, string_queue)
            return ray.get(get_results_future)

    def __getattribute__(self, item):
        if item == 'restore':
            raise AttributeError('`Tuner.restore()` is a classmethod and cannot be called on an instance. Use `tuner = Tuner.restore(...)` to instantiate the Tuner instead.')
