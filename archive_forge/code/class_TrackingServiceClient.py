...
class TrackingServiceClient:
    ...
    def get_metric_history(self, run_id, key):
        ...
        """
        history = self.store.get_metric_history(run_id=run_id, metric_key=key, max_results=GET_METRIC_HISTORY_MAX_RESULTS, page_token=None)
        token = history.token
        while token is not None:
            paged_history = self.store.get_metric_history(run_id=run_id, metric_key=key, max_results=GET_METRIC_HISTORY_MAX_RESULTS, page_token=token)
    ...
    def create_run(self, experiment_id, start_time=None, tags=None, run_name=None):
        ...
        return self.store.create_run(experiment_id=experiment_id, user_id=user_id, start_time=start_time or get_current_time_millis(), tags=[RunTag(key, value) for key, value in tags.items()], run_name=run_name)

    def search_experiments(self, view_type=ViewType.ACTIVE_ONLY, max_results=SEARCH_MAX_RESULTS_DEFAULT, filter_string=None, order_by=None, page_token=None):
        ...
        """
        return self.store.search_experiments(view_type=view_type, max_results=max_results, filter_string=filter_string, order_by=order_by, page_token=page_token)
    ...
    def create_experiment(self, name, artifact_location=None, tags=None):
        ...
        return self.store.create_experiment(name=name, artifact_location=artifact_location, tags=[ExperimentTag(key, value) for key, value in tags.items()] if tags else [])
    ...
    def update_run(self, run_id, status=None, name=None):
        ...
        self.store.update_run_info(run_id=run_id, run_status=RunStatus.from_string(status), end_time=run.info.end_time, run_name=name)
    ...
    def set_terminated(self, run_id, status=None, end_time=None):
        ...
        self.store.update_run_info(run_id, run_status=RunStatus.from_string(status), end_time=end_time, run_name=None)
