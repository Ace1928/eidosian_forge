...
class PrepareUniformSuperposition(infra.GateWithRegisters):
    ...
    def decompose_from_registers(self, *, context: cirq.DecompositionContext, **quregs: NDArray[cirq.Qid]) -> cirq.OP_TREE:
        ...
        yield [op.controlled_by(*controls, control_values=self.cv) for op in cirq.H.on_each(*target)]
        ...
        and_op = and_gate.And((0,) * logL + self.cv).on_registers(ctrl=np.asarray([*logL_qubits, *controls])[:, np.newaxis], junk=np.asarray(and_ancilla)[:, np.newaxis], target=ancilla)
        yield and_op
        yield cirq.Rz(rads=theta)(*ancilla)
