...
class MagneticSpaceGroup(SymmetryGroup):
    ...
    def __init__(self, label, setting_transformation='a,b,c;0,0,0'):
        ...
        self._data['og_label'] = raw_data[7]
        ...
        def _parse_operators(b):
            ...
            for r in raw_symops:
                ...
                time_reversal = r[5]
                op = MagSymmOp.from_rotation_and_translation_and_time_reversal(rotation_matrix=point_operator['matrix'], translation_vec=translation_vec, time_reversal=time_reversal)
                seitz = f'({point_operator['symbol']}|{Fraction(translation_vec[0])},{Fraction(translation_vec[1])},{Fraction(translation_vec[2])})'
                if time_reversal == -1:
                    seitz += "'"
        ...
        for vec in lattice_vectors:
            if not (np.array_equal(vec, [1, 0, 0]) or np.array_equal(vec, [0, 1, 0]) or np.array_equal(vec, [0, 0, 1])):
                for op in ops:
                    ...
                    new_op = MagSymmOp.from_rotation_and_translation_and_time_reversal(op.rotation_matrix, translation_vec=new_vec, time_reversal=op.time_reversal)
                    centered_ops.append(new_op)
        ops = ops + centered_ops
        return [self.jf.transform_symmop(op) for op in ops]

    def get_orbit(self, p, magmom, tol: float=1e-05):
    ...
    def is_compatible(self, lattice: Lattice, tol: float=1e-05, angle_tol: float=5) -> bool:
