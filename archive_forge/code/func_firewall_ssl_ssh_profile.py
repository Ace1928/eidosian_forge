...
def firewall_ssl_ssh_profile(data, fos, check_mode=False):
    ...
    firewall_ssl_ssh_profile_data = flatten_multilists_attributes(firewall_ssl_ssh_profile_data)
    ...
    if state == 'present' or state is True:
        ...
        return fos.delete('firewall', 'ssl-ssh-profile', mkey=filtered_data['name'], vdom=vdom)
    else:
