...
def type5_pw(password, salt=None):
    if not isinstance(password, string_types):
        _raise_error('type5_pw password input should be a string, but was given a input of %s' % type(password).__name__)
    ...
    if salt is not None and (not isinstance(salt, string_types)):
        _raise_error('type5_pw salt input should be a string, but was given a input of %s' % type(salt).__name__)
    elif not salt:
