...
class SparkExecutionEngine(ExecutionEngine):
    ...
    def __init__(self, spark_session: Optional[SparkSession]=None, conf: Any=None):
        ...
        self._broadcast_func = RunOnce(self._broadcast, lambda *args, **kwargs: id(args[0]))
    ...
    def spark_session(self) -> SparkSession:
        ...
        """
        assert_or_throw(self._spark_session is not None, 'SparkExecutionEngine is not started')
    ...
    def to_df(self, df: Any, schema: Any=None) -> SparkDataFrame:
    ...
    def repartition(self, df: DataFrame, partition_spec: PartitionSpec) -> DataFrame:
        ...
        num_funcs = {KEYWORD_ROWCOUNT: lambda: _persist_and_count(df), KEYWORD_PARALLELISM: lambda: self.get_current_parallelism()}
        num = partition_spec.get_num_partitions(**num_funcs)
        if partition_spec.algo in ['default', 'hash', 'coarse']:
            sdf = hash_repartition(self.spark_session, df.native, num, partition_spec.partition_by)
        elif partition_spec.algo == 'rand':
            sdf = rand_repartition(self.spark_session, df.native, num, partition_spec.partition_by)
        elif partition_spec.algo == 'even':
            df = self.persist(df)
            sdf = even_repartition(self.spark_session, df.native, num, partition_spec.partition_by)
        else:
            raise NotImplementedError(partition_spec.algo + ' is not supported')
        sorts = partition_spec.get_sorts(df.schema)
        if len(sorts) > 0:
            sdf = sdf.sortWithinPartitions(*sorts.keys(), ascending=list(sorts.values()))
    ...
    def persist(self, df: DataFrame, lazy: bool=False, **kwargs: Any) -> SparkDataFrame:
        res = self._persist_func(self._to_spark_df(df), lazy=lazy, level=kwargs.get('level', None))
        res.reset_metadata(df.metadata)
        return res
    ...
    def join(self, df1: DataFrame, df2: DataFrame, how: str, on: Optional[List[str]]=None) -> DataFrame:
        ...
        assert_or_throw(how in _TO_SPARK_JOIN_MAP, ValueError(f'{how} is not supported as a join type'))
    ...
    def sample(self, df: DataFrame, n: Optional[int]=None, frac: Optional[float]=None, replace: bool=False, seed: Optional[int]=None) -> DataFrame:
        ...
        if frac is not None:
            d = self._to_spark_df(df).native.sample(fraction=frac, withReplacement=replace, seed=seed)
            return self._to_spark_df(d, df.schema)
        else:
            assert_or_throw(seed is None, NotImplementedError("seed can't be set when sampling by row count"))
            assert_or_throw(not replace, NotImplementedError('replacement is not supported when sampling by row count'))
            temp_name = '__temp_' + str(uuid4()).split('-')[-1]
            self._to_spark_df(df).native.createOrReplaceTempView(temp_name)
            d = self.spark_session.sql(f'SELECT * FROM {temp_name} TABLESAMPLE({n} ROWS)')
            return self._to_spark_df(d, df.schema)
    ...
    def take(self, df: DataFrame, n: int, presort: str, na_position: str='last', partition_spec: Optional[PartitionSpec]=None) -> DataFrame:
        ...
        if len(partition_spec.partition_by) == 0:
            if len(_presort.keys()) > 0:
                d = d.orderBy([_presort_to_col(_col, _presort[_col]) for _col in _presort.keys()])
            d = d.limit(n)
        else:
            w = Window.partitionBy([col(x) for x in partition_spec.partition_by])
            if len(_presort.keys()) > 0:
                w = w.orderBy([_presort_to_col(_col, _presort[_col]) for _col in _presort.keys()])
            else:
                w = w.orderBy(lit(1))
            d = d.select(col('*'), row_number().over(w).alias('__row_number__')).filter(col('__row_number__') <= n).drop('__row_number__')
        return self._to_spark_df(d, df.schema)
    ...
    def _persist(self, df: SparkDataFrame, lazy: bool, level: Any) -> SparkDataFrame:
        ...
        if isinstance(level, StorageLevel):
            ...
            return df
        raise ValueError(f'{level} is not supported persist type')

    def _to_spark_df(self, df: Any, schema: Any=None, create_view: bool=False) -> SparkDataFrame:
