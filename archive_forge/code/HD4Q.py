...
t = threading.Thread(target=start_async_loop, args=(async_loop,))
t.start()


def run_async_coroutine(coroutine):
    asyncio.run_coroutine_threadsafe(coroutine, async_loop)
...
QUANTUM_FORCE_CONSTANT = 0.00001
NUCLEAR_FORCE_RANGE = 10
...
class Particle:
    ...
    Attributes:
        ...
        pos (numpy.ndarray): Position of the particle in 2D space.
        vel (numpy.ndarray): Velocity of the particle in 2D space.
    ...
    def __init__(self, x, y, vx, vy, element="hydrogen"):
        """
        ...
        self.strong_coupling = self.properties[
            "strong_coupling"
        ]  # Strong nuclear force coupling constant of the particle
        self.strong_radius = self.properties[
            "strong_radius"
        ]  # Strong nuclear force radius of the particle
    ...
    def calculate_force(self, other):
        ...
        Returns:
            numpy.ndarray: The force vector.
        """
        displacement = (
            other.pos - self.pos
        ...
        distance = (
            np.sqrt(distance_sq) + EPSILON
        )  # Calculate the distance between the particles with a small epsilon to avoid division by zero

        force = np.zeros(2, dtype=np.float64)  # Initialize the force vector to zero
        ...
        force += displacement * em_force_magnitude
        ...
        if distance < self.strong_radius + other.strong_radius:
            ...
            force -= displacement * strong_force_magnitude

        # Repulsive force (simplified Coulomb's Law)
        repulsive_force_magnitude = (
            self.em_charge * other.em_charge * REPULSIVE_CONSTANT
        ) / distance_sq
        force += (
            displacement * repulsive_force_magnitude / distance
    ...
    def apply_forces(self, particles):
        ...
        force = np.zeros(
            2, dtype=np.float64
        )  # Initialize the total force vector to zero
        for other in particles:
            if other.id != self.id:
                force += self.calculate_force(
                    other
                )  # Calculate and accumulate the force from each other particle

        # Dark matter force
        direction_to_center = (
            center - self.pos
        )  # Calculate the direction vector from the particle to the center of the simulation
        force += direction_to_center * DARK_MATTER_PULL  # Apply the dark matter force

        # Quantum fluctuations
        force += (
            np.random.randn(2) * QUANTUM_FORCE_CONSTANT
        ...
        self.pos += (
            self.vel * DT + 0.5 * force / self.mass * DT**2
        )  # Update the particle's position using Verlet integration
        self.pos = np.clip(
            self.pos, 0, [width, height]
    ...
    def draw(self):
        ...
        """
        pygame.draw.circle(
            screen, self.color, self.pos.astype(int), PARTICLE_SIZE
...
async def process_particles(particles):
    ...
    This function is designed to work both in environments where an event loop
    is already running (e.g., Jupyter Notebooks) and where it is not (e.g., standalone scripts).
    ...
    with ThreadPoolExecutor() as executor:  # Create a thread pool executor
        forces = await asyncio.gather(
            *[
                loop.run_in_executor(executor, particle.apply_forces, particles)
                for particle in particles
            ]
    ...
    for particle, force in zip(particles, forces):
        if force is not None:
            particle.vel += force / particle.mass * DT
            particle.pos += particle.vel * DT + 0.5 * force / particle.mass * DT**2
        particle.pos = np.clip(
            particle.pos, 0, [width, height]
        )  # Ensure the particle stays within the simulation boundaries
...
def run():
    ...
    while running:
        ...
        # Schedule the asynchronous task without using asyncio.run()
        loop = asyncio.get_event_loop()
        if loop.is_running():
            # If the loop is running (e.g., Jupyter notebooks), use create_task() or ensure_future()
            task = run_async_coroutine(process_particles(particles))
        else:
            # If the loop is not running, run the coroutine manually
            loop.run_until_complete(run_async_coroutine(process_particles(particles)))

        # Your particle processing and drawing code here
        ...
        # Assuming draw_legend() and save_particles() are defined elsewhere
