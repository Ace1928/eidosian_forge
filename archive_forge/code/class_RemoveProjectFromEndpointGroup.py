...
class RemoveProjectFromEndpointGroup(command.Command):
    ...
    def get_parser(self, prog_name):
        parser = super(RemoveProjectFromEndpointGroup, self).get_parser(prog_name)
        parser.add_argument('endpointgroup', metavar='<endpoint-group>', help=_('Endpoint group (name or ID)'))
        parser.add_argument('project', metavar='<project>', help=_('Project to remove (name or ID)'))
    ...
    def take_action(self, parsed_args):
        ...
        client.endpoint_filter.delete_endpoint_group_from_project(endpoint_group=endpointgroup.id, project=project.id)