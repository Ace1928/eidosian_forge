...
class FitAslInputSpec(CommandLineInputSpec):
    ...
    source_file = File(position=1, exists=True, argstr='-source %s', mandatory=True, desc=desc)
    ...
    cbf_file = File(name_source=['source_file'], name_template='%s_cbf.nii.gz', argstr='-cbf %s', desc=desc)
    error_file = File(name_source=['source_file'], name_template='%s_error.nii.gz', argstr='-error %s', desc='Filename of the CBF error map.')
    ...
    desc = 'Filename of a [1,2,5]s Inversion Recovery volume (T1/M0 fitting carried out internally).'
    ...
    t1_art_cmp = traits.Float(desc='T1 of arterial component [1650ms].', argstr='-T1a %f')
    desc = 'Single plasma/tissue partition coefficient [0.9ml/g].'
    plasma_coeff = traits.Float(desc=desc, argstr='-L %f')
    desc = 'Labelling efficiency [0.99 (pasl), 0.85 (pcasl)], ensure any background suppression pulses are included in -eff'
    eff = traits.Float(desc=desc, argstr='-eff %f')
    desc = 'Outlier rejection for multi CL volumes (enter z-score threshold (e.g. 2.5)) [off].'
    ...
    gm_plasma = traits.Float(desc='Plasma/GM water partition [0.95ml/g].', argstr='-gmL %f')
    gm_ttt = traits.Float(desc='Time to GM [ATT+0ms].', argstr='-gmTTT %f')
    wm_t1 = traits.Float(desc='T1 of WM [800ms].', argstr='-wmT1 %f')
    wm_plasma = traits.Float(desc='Plasma/WM water partition [0.82ml/g].', argstr='-wmL %f')
    ...
    desc = 'Simple PV correction (CBF=vg*CBFg + vw*CBFw, with CBFw=f*CBFg) [0.25].'
    pv0 = traits.Int(desc=desc, argstr='-pv0 %d')
    pv2 = traits.Int(desc='In plane PV kernel size [3x3].', argstr='-pv2 %d')
    pv3 = traits.Tuple(traits.Int, traits.Int, traits.Int, desc='3D kernel size [3x3x1].', argstr='-pv3 %d %d %d')
