...
class WSRoot(object):
    ...
    def __init__(self, protocols=[], webpath='', transaction=None, scan_api=scan_api):
        self._debug = True
    ...
    def _do_call(self, protocol, context):
        ...
        try:
            ...
            if context.path is None:
                raise ClientSideError('The %s protocol was unable to extract a function path from the request' % protocol.name)
            context.func, context.funcdef, args = self._lookup_function(context.path)
    ...
    def _handle_request(self, request):
        ...
        if protocol is None:
            if not msg:
                msg = 'None of the following protocols can handle this request : %s' % ','.join((p.name for p in self.protocols))
        ...
        try:
            ...
            body = prepare_response_body(request, (self._do_call(protocol, context) for context in protocol.iter_calls(request)))
            ...
            if len(request.calls) == 1:
                ...
                elif request.client_errorcount == 1:
