...
class _OpenStackCloudMixin:
    ...
    def get_session_endpoint(self, service_key, **kwargs):
        ...
        try:
            ...
            raise exceptions.SDKException('Error getting {service} endpoint on {cloud}:{region}: {error}'.format(service=service_key, cloud=self.name, region=self.config.get_region_name(service_key), error=str(e)))
        return endpoint
    ...
    def has_service(self, service_key, version=None):
        if not self.config.has_service(service_key):
            if not (service_key in self._disable_warnings and self._disable_warnings[service_key]):
                self.log.debug('Disabling %(service_key)s entry in catalog per config', {'service_key': service_key})
