    ...
    def __init__(self, module):
        ...
        if self.params.get('state') == 'present':
            ...
            for available_vswitch in self.network_mgr.networkInfo.vswitch:
                ...
                self.available_vswitches[available_vswitch.name] = dict(pnic=used_pnic, mtu=available_vswitch.mtu, num_ports=available_vswitch.spec.numPorts)
            for desired_pnic in self.nics:
                if desired_pnic not in available_pnic:
                    self.module.fail_json(msg="Specified Physical NIC '%s' does not exists on given ESXi '%s'." % (desired_pnic, self.host_system.name))
                for vswitch in self.available_vswitches:
                    if desired_pnic in self.available_vswitches[vswitch]['pnic'] and vswitch != self.switch:
                        self.module.fail_json(msg="Specified Physical NIC '%s' is already used by vSwitch '%s'." % (desired_pnic, vswitch))

    def process_state(self):
        """
        Manage internal state of vSwitch
        """
        vswitch_states = {'absent': {'present': self.state_destroy_vswitch, 'absent': self.state_exit_unchanged}, 'present': {'present': self.state_update_vswitch, 'absent': self.state_create_vswitch}}
        try:
