...
class AI21(LLM):
    ...
    def _default_params(self) -> Dict[str, Any]:
        ...
        return {'temperature': self.temperature, 'maxTokens': self.maxTokens, 'minTokens': self.minTokens, 'topP': self.topP, 'presencePenalty': self.presencePenalty.dict(), 'countPenalty': self.countPenalty.dict(), 'frequencyPenalty': self.frequencyPenalty.dict(), 'numResults': self.numResults, 'logitBias': self.logitBias}
    ...
    def _call(self, prompt: str, stop: Optional[List[str]]=None, run_manager: Optional[CallbackManagerForLLMRun]=None, **kwargs: Any) -> str:
        ...
        response = requests.post(url=f'{base_url}/{self.model}/complete', headers={'Authorization': f'Bearer {self.ai21_api_key.get_secret_value()}'}, json={'prompt': prompt, 'stopSequences': stop, **params})
