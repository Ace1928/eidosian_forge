...
class LanguageServerSession(LoggingConfigurable):
    ...
    handlers = Set(trait=Instance(WebSocketHandler), default_value=[], help='the currently subscribed websockets')
    ...
    def __repr__(self):
        return '<LanguageServerSession(language_server={language_server}, argv={argv})>'.format(language_server=self.language_server, **self.spec)

    def to_json(self):
        return dict(handler_count=len(self.handlers), status=self.status.value, last_server_message_at=self.last_server_message_at.isoformat() if self.last_server_message_at else None, last_handler_message_at=self.last_handler_message_at.isoformat() if self.last_handler_message_at else None, spec=censored_spec(self.spec))
    ...
    def init_process(self):
        ...
        self.process = subprocess.Popen(self.spec['argv'], stdin=subprocess.PIPE, stdout=subprocess.PIPE, env=self.substitute_env(self.spec.get('env', {}), os.environ), bufsize=0)
