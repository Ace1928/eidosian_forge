...
def _ShouldDoSlicedDownload(download_strategy, src_obj_metadata, allow_splitting, logger):
  ...
  """
    sliced_object_download_threshold = HumanReadableToBytes(config.get('GSUtil', 'sliced_object_download_threshold', DEFAULT_SLICED_OBJECT_DOWNLOAD_THRESHOLD))
    ...
    use_slice = allow_splitting and download_strategy is not CloudApi.DownloadStrategy.ONE_SHOT and (max_components > 1) and hashing_okay and (sliced_object_download_threshold > 0) and (src_obj_metadata.size >= sliced_object_download_threshold)
    if not use_slice and src_obj_metadata.size >= PARALLEL_COMPOSITE_SUGGESTION_THRESHOLD and (not UsingCrcmodExtension()) and (check_hashes_config != CHECK_HASH_NEVER):
