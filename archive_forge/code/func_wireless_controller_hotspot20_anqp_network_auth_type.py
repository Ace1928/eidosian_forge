...
def wireless_controller_hotspot20_anqp_network_auth_type(data, fos, check_mode=False):
    ...
    filtered_data = underscore_to_hyphen(filter_wireless_controller_hotspot20_anqp_network_auth_type_data(wireless_controller_hotspot20_anqp_network_auth_type_data))
    if check_mode:
    ...
    if state == 'present' or state is True:
        return fos.set('wireless-controller.hotspot20', 'anqp-network-auth-type', data=filtered_data, vdom=vdom)
    elif state == 'absent':
        return fos.delete('wireless-controller.hotspot20', 'anqp-network-auth-type', mkey=filtered_data['name'], vdom=vdom)
    else:
