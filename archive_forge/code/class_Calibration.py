...
class Calibration(abc.Mapping):
    ...
    def __init__(self, calibration: v2.metrics_pb2.MetricsSnapshot=v2.metrics_pb2.MetricsSnapshot(), metrics: Optional[ALL_METRICS]=None) -> None:
    ...
    def to_proto(self) -> v2.metrics_pb2.MetricsSnapshot:
        ...
        for key in self._metric_dict:
            for targets, value_list in self._metric_dict[key].items():
                ...
                current_metric.targets.extend([target if isinstance(target, str) else v2.qubit_to_proto_id(target) for target in targets])
                for value in value_list:
                    ...
                    if isinstance(value, float):
                        ...
                        raise ValueError(f'Unsupported metric value {value}. Must be int, float, or str to convert to proto.')
        return proto
    ...
    def heatmap(self, key: str) -> cirq.Heatmap:
        ...
        if not all((len(k) == 1 for k in metrics.values())):
            raise ValueError('Heatmaps are only supported if all values in a metric are single metric values.' + f'{key} has metric values {metrics.values()}')
        ...
        raise ValueError('Heatmaps are only supported if all the targets in a metric are one or two qubits.' + f'{key} has target qubits {value_map.keys()}')
    ...
    def plot(self, key: str, fig: Optional[mpl.figure.Figure]=None) -> Tuple[mpl.figure.Figure, List[plt.Axes]]:
