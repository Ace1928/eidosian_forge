...
class BaseAPI(object):
    ...
    * Provide basic endpoint handling when a Service Catalog is not
    ...
    def list(self, path, session=None, body=None, detailed=False, headers=None, **params):
        ...
        if body:
            ret = self._request('POST', path, json=body, params=params, headers=headers)
        else:
            ret = self._request('GET', path, params=params, headers=headers)
        try:
            return ret.json()
        except json.JSONDecodeError:
            return ret
    ...
    def find(self, path, value=None, attr=None, headers=None):
        ...
        def raise_not_found():
            ...
            raise exceptions.NotFound(404, msg)
        try:
            ret = self._request('GET', '/%s/%s' % (path, value), headers=headers).json()
            if isinstance(ret, dict):
                key = list(ret.keys())[0]
                ret = ret[key]
        except (ksa_exceptions.NotFound, ksa_exceptions.BadRequest):
            if attr:
                kwargs = {attr: value}
                try:
                    ret = self.find_one(path, headers=headers, **kwargs)
                except (exceptions.NotFound, ksa_exceptions.NotFound):
                    raise_not_found()
            else:
                raise_not_found()
        return ret