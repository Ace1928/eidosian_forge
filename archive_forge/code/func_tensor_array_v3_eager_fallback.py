...
def tensor_array_v3_eager_fallback(size: _atypes.TensorFuzzingAnnotation[_atypes.Int32], dtype: TV_TensorArrayV3_dtype, element_shape, dynamic_size: bool, clear_after_read: bool, identical_element_shapes: bool, tensor_array_name: str, name, ctx):
    ...
    _inputs_flat = [size]
    _attrs = ('dtype', dtype, 'element_shape', element_shape, 'dynamic_size', dynamic_size, 'clear_after_read', clear_after_read, 'identical_element_shapes', identical_element_shapes, 'tensor_array_name', tensor_array_name)
    _result = _execute.execute(b'TensorArrayV3', 2, inputs=_inputs_flat, attrs=_attrs, ctx=ctx, name=name)
