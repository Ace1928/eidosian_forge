...
class Snake:
    def __init__(self, window: pg.Surface) -> None:
        ...
        This constructor meticulously sets up the snake's body, direction, associates a fruit object, and calculates the initial path using the A* algorithm, ensuring all components are perfectly integrated and functional.
        """
        # Initialize the snake's body with predefined coordinates
        self.body: Deque[Tuple[int, int]] = deque(
            [
                (160, 160),  # Head of the snake at the start
                (140, 160),  # Second segment of the snake
                (120, 160),  # Third segment of the snake
            ]
        )
        ...
        # Associate a fruit object which the snake will aim to capture
        self.fruit: Fruit = Fruit(window)

        # Store the reference to the game window for drawing purposes
        self.window = window

        # Calculate the initial path from the snake's head to the fruit using the A* algorithm
        self.path: List[Tuple[int, int]] = self.calculate_path()

        # Log the successful initialization of the snake with all its components
        logging.info(
            "Snake initialized with body, direction, fruit, and path, ensuring full integration and functionality."
        )
    ...
    def move(self) -> bool:
        ...
        if self.path:
            ...
            if next_pos == self.fruit.position:
                ...
                self.path = (
                    self.calculate_path()
                )  # Recalculate path after eating the fruit
            else:
                self.body.pop()
