        ...
        for name, value in namespace.items():
            ...
            for base in bases:
                ...
                if not base_class_method or not callable(base_class_method):
                    continue
                assert is_override, 'Method %s overrides but does not have @overrides decorator' % name
                assert not getattr(base_class_method, '__finalized__', False), 'Method %s is finalized in %s, it cannot be overridden' % (base_class_method, base)
        return cls
