...
class GUI:
    ...
    def _create_scoreboard(self, position, initial_text):
        """
        Create a dynamic scoreboard to display the current score and other game-related information.

        Args:
            position (Tuple[int, int]): The position on the screen where the scoreboard will be placed.
            initial_text (str): The initial text to display on the scoreboard.

        Returns:
            pygame_gui.elements.UILabel: A configured scoreboard element.
        """
        return pygame_gui.elements.UILabel(
            relative_rect=pygame.Rect(position, (200, 30)),
            text=initial_text,
            manager=self.manager,
            object_id="scoreboard_label",
        )
