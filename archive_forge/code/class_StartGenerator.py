...
class StartGenerator:
    ...
    def __init__(self, slab, blocks, blmin, number_of_variable_cell_vectors=0, box_to_place_in=None, box_volume=None, splits=None, cellbounds=None, test_dist_to_slab=True, test_too_far=True, rng=np.random):
        self.slab = slab
        self.blocks = []
        for item in blocks:
        ...
        if isinstance(blmin, dict):
            ...
            self.blmin = closest_distances_generator(numbers, ratio_of_covalent_radii=blmin)
    ...
    def get_new_candidate(self, maxiter=None):
        ...
        while maxiter is None or niter < maxiter:
            ...
            for i in range(N_blocks):
                ...
                while maxiter is None or niter < maxiter:
                    ...
                    if len(atoms) > 1:
                        ...
                        atoms.euler_rotate(phi=phi, theta=0.5 * theta, psi=psi, center=pos)
            ...
            assert not atoms_too_close(cand, blmin, use_tags=True), 'This is not supposed to happen; please report this bug'
            ...
            if self.test_too_far:
                ...
                for tag in np.unique(tags):
                    ...
                    too_far = not atoms_too_close_two_sets(cand[indices_i], cand[indices_j], blmin_too_far)
                    if too_far and len(self.slab) > 0:
                        too_far = not atoms_too_close_two_sets(cand[indices_i], self.slab, blmin_too_far)
                    if too_far:
                        break
                else:
                    too_far = False
