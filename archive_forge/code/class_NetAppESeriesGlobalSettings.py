...
class NetAppESeriesGlobalSettings(NetAppESeriesModule):
    ...
    def __init__(self):
        ...
        ansible_options = dict(cache_block_size=dict(type='int', require=False), cache_flush_threshold=dict(type='int', required=False), default_host_type=dict(type='str', require=False), automatic_load_balancing=dict(type='str', choices=['enabled', 'disabled'], required=False), host_connectivity_reporting=dict(type='str', choices=['enabled', 'disabled'], required=False), name=dict(type='str', required=False, aliases=['label']), login_banner_message=dict(type='str', required=False), controller_shelf_id=dict(type='int', required=False, default=0))
        super(NetAppESeriesGlobalSettings, self).__init__(ansible_options=ansible_options, web_services_version=version, supports_check_mode=True)
