...
class WindowsRemotefsClientTestCase(base.TestCase):
    ...
    _FAKE_SHARE = '\\\\%s\\%s' % (_FAKE_SHARE_SERVER, _FAKE_SHARE_NAME)
    ...
    def setUp(self, mock_utilsfactory):
        ...
        self._remotefs = windows_remotefs.WindowsRemoteFsClient(mount_type='smbfs')
    ...
    @ddt.data(True, False)
    @mock.patch.object(windows_remotefs.WindowsRemoteFsClient, '_create_mount_point')
    def test_mount(self, is_local_share, mock_create_mount_point):
        ...
        if is_local_share:
            ...
            self._smbutils.check_smb_mapping.assert_called_once_with(self._FAKE_SHARE)
            self._smbutils.mount_smb_share.assert_called_once_with(self._FAKE_SHARE, username='username', password='password')
        mock_create_mount_point.assert_called_once_with(self._FAKE_SHARE, is_local_share)
