...
def to_numeric(arg, errors='raise', meta=None):
    ...
    if not any([is_series, is_array, is_scalar]):
        raise TypeError('arg must be a list, tuple, dask.array.Array, or dask.dataframe.Series')
    if meta is not None:
        if is_scalar:
            raise KeyError('``meta`` is not allowed when input is a scalar.')
    elif is_series or is_array:
        meta = pd.to_numeric(arg._meta)
    if is_series:
        return arg.map_partitions(pd.to_numeric, token=arg._name + '-to_numeric', meta=meta, enforce_metadata=False, errors=errors)
    if is_array:
        return arg.map_blocks(pd.to_numeric, name=arg._name + '-to_numeric', meta=meta, errors=errors)
    if is_scalar:
