...
class PBSPlugin(SGELikeBatchManagerBase):
    ...
    def _is_pending(self, taskid):
        result = CommandLine('qstat -f {}'.format(taskid), environ=dict(os.environ), terminal_output='file_split', resource_monitor=False, ignore_exception=True).run()
        ...
        while True:
            try:
                ...
                if tries < self._max_tries:
                    ...
                    raise RuntimeError('Could not submit pbs task for node {}\n{}'.format(node._id, e))
            else:
                break
        iflogger.setLevel(oldlevel)
