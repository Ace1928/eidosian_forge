...
class VolcanoEmbeddings(BaseModel, Embeddings):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        """
        values['volcano_ak'] = get_from_dict_or_env(values, 'volcano_ak', 'VOLC_ACCESSKEY')
        values['volcano_sk'] = get_from_dict_or_env(values, 'volcano_sk', 'VOLC_SECRETKEY')
        try:
    ...
    def embed_documents(self, texts: List[str]) -> List[List[float]]:
        ...
        for chunk in text_in_chunks:
            req = {'model': {'name': self.model, 'version': self.version}, 'input': chunk}
            try:
