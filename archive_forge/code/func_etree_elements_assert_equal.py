...
def etree_elements_assert_equal(elem: Element, other: Element, strict: bool=True, skip_comments: bool=True, unordered: bool=False) -> None:
    """
    ...
    if unordered:
        ...
        other_children = iter(sorted(other, key=lambda x: '' if callable(x.tag) else x.tag))
    else:
        children = elem
