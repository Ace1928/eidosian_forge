...
class TestDiagnostics:
    ...
    def test_deterministic(self):
        ...
        reference = pd.read_csv(reference_path, index_col=0, float_precision='high').sort_index(axis=1).sort_index(axis=0)
        funcs = {'rhat_rank': lambda x: rhat(x, method='rank'), 'rhat_raw': lambda x: rhat(x, method='identity'), 'ess_bulk': lambda x: ess(x, method='bulk'), 'ess_tail': lambda x: ess(x, method='tail'), 'ess_mean': lambda x: ess(x, method='mean'), 'ess_sd': lambda x: ess(x, method='sd'), 'ess_median': lambda x: ess(x, method='median'), 'ess_raw': lambda x: ess(x, method='identity'), 'ess_quantile01': lambda x: ess(x, method='quantile', prob=0.01), 'ess_quantile10': lambda x: ess(x, method='quantile', prob=0.1), 'ess_quantile30': lambda x: ess(x, method='quantile', prob=0.3), 'mcse_mean': lambda x: mcse(x, method='mean'), 'mcse_sd': lambda x: mcse(x, method='sd'), 'mcse_median': lambda x: mcse(x, method='median'), 'mcse_quantile01': lambda x: mcse(x, method='quantile', prob=0.01), 'mcse_quantile10': lambda x: mcse(x, method='quantile', prob=0.1), 'mcse_quantile30': lambda x: mcse(x, method='quantile', prob=0.3)}
        results = {}
        ...
        assert (abs(reference - arviz_data).values < 1e-08).all(None)
