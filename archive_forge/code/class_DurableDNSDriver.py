...
class DurableDNSDriver(DNSDriver):
    ...
    RECORD_TYPE_MAP = {RecordType.A: 'A', RecordType.AAAA: 'AAAA', RecordType.CNAME: 'CNAME', RecordType.HINFO: 'HINFO', RecordType.MX: 'MX', RecordType.NS: 'NS', RecordType.PTR: 'PTR', RecordType.RP: 'RP', RecordType.SRV: 'SRV', RecordType.TXT: 'TXT'}
    ...
    def list_zones(self):
        ...
        schema = api_schema_builder(schema_params.get('urn_nid'), schema_params.get('method'), attributes)
        ...
        response = self.connection.request(action=action, params=params, data=req_data, method='POST', headers=headers)
        zones = []
    ...
    def get_record(self, zone_id, record_id):
        ...
        attributes = schema_params.get('attributes')
        schema = api_schema_builder(schema_params.get('urn_nid'), schema_params.get('method'), attributes)
        params = {'apiuser': self.key, 'apikey': self.secret, 'zonename': zone_id, 'recordid': record_id}
        urn = list(schema)[0]
        for child in urn:
            key = child.tag.split(':')[2]
            if key in attributes:
    ...
    def create_zone(self, domain, type='master', ttl=None, extra=None):
        ...
        attributes = schema_params.get('attributes')
        schema = api_schema_builder(schema_params.get('urn_nid'), schema_params.get('method'), attributes)
        params = {'apiuser': self.key, 'apikey': self.secret, 'zonename': domain, 'ttl': ttl or DEFAULT_TTL}
        params.update(extra)
        urn = list(schema)[0]
        for child in urn:
            key = child.tag.split(':')[2]
            if key in attributes:
                if isinstance(params.get(key), int):
                    ...
                    child.text = '%s'
        skel = ensure_string(tostring(schema))
        req_data = skel % (self.key, self.secret, domain, extra.get('ns'), extra.get('mbox'), extra.get('refresh'), extra.get('retry'), extra.get('expire'), extra.get('minimum'), ttl or DEFAULT_TTL, extra.get('xfer'), extra.get('update_acl'))
        action = '/services/dns/createZone.php?'
        params = {}
        headers = {'SOAPAction': 'urn:createZonewsdl#createZone'}
        try:
            self.connection.request(action=action, params=params, data=req_data, method='POST', headers=headers)
        except DurableDNSException as e:
    ...
    def create_record(self, name, zone, type, data, extra=None):
        ...
        attributes = schema_params.get('attributes')
        schema = api_schema_builder(schema_params.get('urn_nid'), schema_params.get('method'), attributes)
        params = {'apiuser': self.key, 'apikey': self.secret, 'zonename': zone.id, 'name': name, 'type': type, 'data': data}
        params.update(extra)
        urn = list(schema)[0]
        for child in urn:
            key = child.tag.split(':')[2]
            if key in attributes:
                if isinstance(params.get(key), int):
                    ...
                    child.text = '%s'
        skel = ensure_string(tostring(schema))
        req_data = skel % (self.key, self.secret, zone.id, name, type, data, extra.get('aux'), extra.get('ttl'), extra.get('ddns_enabled'))
        action = '/services/dns/createRecord.php?'
        headers = {'SOAPAction': 'urn:createRecordwsdl#createRecord'}
        try:
            response = self.connection.request(action=action, data=req_data, method='POST', headers=headers)
    ...
    def update_zone(self, zone, domain, type='master', ttl=None, extra=None):
        ...
        attributes = schema_params.get('attributes')
        schema = api_schema_builder(schema_params.get('urn_nid'), schema_params.get('method'), attributes)
        params = {'apiuser': self.key, 'apikey': self.secret, 'zonename': domain, 'ttl': ttl}
        params.update(extra)
        urn = list(schema)[0]
        for child in urn:
            key = child.tag.split(':')[2]
            if key in attributes:
                if isinstance(params.get(key), int):
                    ...
                    child.text = '%s'
        skel = ensure_string(tostring(schema))
        req_data = skel % (self.key, self.secret, domain, extra['ns'], extra['mbox'], extra['refresh'], extra['retry'], extra['expire'], extra['minimum'], ttl, extra['xfer'], extra['update_acl'])
    ...
    def update_record(self, record, name, type, data, extra=None):
        ...
        attributes = schema_params.get('attributes')
        schema = api_schema_builder(schema_params.get('urn_nid'), schema_params.get('method'), attributes)
        params = {'apiuser': self.key, 'apikey': self.secret, 'zonename': zone.id, 'id': record.id, 'name': name, 'data': data}
        params.update(extra)
        urn = list(schema)[0]
        for child in urn:
            key = child.tag.split(':')[2]
            if key in attributes:
                if isinstance(params.get(key), int):
                    ...
                    child.text = '%s'
        skel = ensure_string(tostring(schema))
        req_data = skel % (self.key, self.secret, zone.id, record.id, name, extra.get('aux'), data, extra.get('ttl'), extra.get('ddns_enabled'))
    ...
    def delete_record(self, record):
        ...
        attributes = schema_params.get('attributes')
        schema = api_schema_builder(schema_params.get('urn_nid'), schema_params.get('method'), attributes)
        params = {'apiuser': self.key, 'apikey': self.secret, 'zonename': record.zone.id, 'id': record.id}
        urn = list(schema)[0]
        for child in urn:
            key = child.tag.split(':')[2]
            if key in attributes:
        ...
        try:
            response = self.connection.request(action=action, data=req_data, headers=headers, method='POST')
        except DurableDNSException as e:
    ...
    def _to_zone(self, item):
        ...
        zone = Zone(id=item.get('id'), type='master', domain=item.get('id'), ttl=item.get('ttl'), driver=self, extra=extra)
        return zone
    ...
    def _to_record(self, item, zone=None):
        ...
        record = Record(id=item.get('id'), type=item.get('type'), zone=zone, name=item.get('name'), data=item.get('data'), driver=self, ttl=item.get('ttl', None), extra=extra)
        return record
