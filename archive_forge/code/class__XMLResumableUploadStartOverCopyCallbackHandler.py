...
class _XMLResumableUploadStartOverCopyCallbackHandler(object):
    ...
    def call(self, total_bytes_transferred, total_size):
        ...
        if total_bytes_transferred >= self._startover_at_byte and (not self.started_over_once):
            sys.stderr.write('Forcing ResumableUpload start over error after byte %s. %s/%s transferred.\r\n' % (self._startover_at_byte, MakeHumanReadable(total_bytes_transferred), MakeHumanReadable(total_size)))
            self.started_over_once = True
            raise boto.exception.ResumableUploadException('Forcing upload start over', ResumableTransferDisposition.START_OVER)