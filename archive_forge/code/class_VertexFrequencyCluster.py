...
class VertexFrequencyCluster(BaseEstimator):
    ...
    **kwargs
    ...
    def __init__(self, n_clusters=10, likelihood_bias=1, window_count=9, window_sizes=None, sparse=False, suppress=False, random_state=None, **kwargs):
    ...
    def fit(self, G):
        ...
        if np.all(np.diff(np.log2(self.window_sizes)) == 1):
            ...
            for t in self.window_sizes:
                self.windows.append(self._compute_window(self._basewindow, t=t).astype(float))
    ...
    def transform(self, sample_indicator, likelihood=None, center=True):
        ...
        if likelihood is not None and (not isinstance(self.likelihood, (list, tuple, np.ndarray, pd.Series, pd.DataFrame))):
        ...
        if likelihood is not None:
            ...
            if likelihood.shape != sample_indicator.shape:
                raise ValueError('`sample_indicator` and `likelihood` must have the same shape. Got sample_indicator: {} and likelihood: {}'.format(str(sample_indicator.shape), str(likelihood.shape)))
        ...
        if self.likelihood is not None:
            self.combined_spectrogram = self._combine_spectrogram_likelihood(spectrogram=self.spectrogram, likelihood=self.likelihood)
    ...
    def predict(self, n_clusters=None, **kwargs):
        ...
        if not self.isfit:
            raise ValueError('Estimator is not fit. Call VertexFrequencyCluster.fit().')
        if self.spectrogram is None:
            raise ValueError('Estimator is not transformed. Call VertexFrequencyCluster.transform().')
