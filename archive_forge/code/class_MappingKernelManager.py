...
class MappingKernelManager(MultiKernelManager):
    ...
    _kernel_ports: dict[str, list[int]] = Dict()
    ...
    def _update_root_dir(self, proposal):
        ...
        return value
    cull_idle_timeout = Integer(0, config=True, help='Timeout (in seconds) after which a kernel is considered idle and ready to be culled.\n        Values of 0 or lower disable culling. Very short timeouts may result in kernels being culled\n        for users with poor network connections.')
    cull_interval_default = 300
    cull_interval = Integer(cull_interval_default, config=True, help='The interval (in seconds) on which to check for idle kernels exceeding the cull timeout value.')
    cull_connected = Bool(False, config=True, help='Whether to consider culling kernels which have one or more connections.\n        Only effective if cull_idle_timeout > 0.')
    cull_busy = Bool(False, config=True, help='Whether to consider culling kernels which are busy.\n        Only effective if cull_idle_timeout > 0.')
    buffer_offline_messages = Bool(True, config=True, help='Whether messages from kernels whose frontends have disconnected should be buffered in-memory.\n\n        When True (default), messages are buffered and replayed on reconnect,\n        avoiding lost messages due to interrupted connectivity.\n\n        Disable if long-running kernels will produce too much output while\n        no frontends are connected.\n        ')
    kernel_info_timeout = Float(60, config=True, help='Timeout for giving up on a kernel (in seconds).\n\n        On starting and restarting kernels, we check whether the\n        kernel is running and responsive by sending kernel_info_requests.\n        This sets the timeout in seconds for how long the kernel can take\n        before being presumed dead.\n        This affects the MappingKernelManager (which handles kernel restarts)\n        and the ZMQChannelsHandler (which handles the startup).\n        ')
    _kernel_buffers = Any()
    ...
    allowed_message_types = List(trait=Unicode(), config=True, help='White list of allowed kernel message types.\n        When the list is empty, all message types are allowed.\n        ')
    allow_tracebacks = Bool(True, config=True, help='Whether to send tracebacks to clients on exceptions.')
    traceback_replacement_message = Unicode('An exception occurred at runtime, which is not shown due to security reasons.', config=True, help='Message to print when allow_tracebacks is False, and an exception occurs')
    ...
    async def _async_start_kernel(self, *, kernel_id: str | None=None, path: ApiPath | None=None, **kwargs: str) -> str:
    ...
    async def _finish_kernel_start(self, kernel_id):
        ...
        self.add_restart_callback(kernel_id, lambda: self._handle_kernel_died(kernel_id), 'dead')
    ...
    async def cull_kernels(self):
        ...
        self.log.debug('Polling every %s seconds for kernels idle > %s seconds...', self.cull_interval, self.cull_idle_timeout)
        ...
        for kernel_id in list(self._kernels):
            try:
                ...
                self.log.exception('The following exception was encountered while checking the idle duration of kernel %s: %s', kernel_id, e)

    async def cull_kernel_if_idle(self, kernel_id):
        """Cull a kernel if it is idle."""
        kernel = self._kernels[kernel_id]
        if getattr(kernel, 'execution_state', None) == 'dead':
            self.log.warning("Culling '%s' dead kernel '%s' (%s).", kernel.execution_state, kernel.kernel_name, kernel_id)
        ...
        if hasattr(kernel, 'last_activity'):
            ...
            if is_idle_time and is_idle_execute and is_idle_connected:
                ...
                self.log.warning("Culling '%s' kernel '%s' (%s) with %d connections due to %s seconds of inactivity.", kernel.execution_state, kernel.kernel_name, kernel_id, connections, idle_duration)
                await ensure_async(self.shutdown_kernel(kernel_id))