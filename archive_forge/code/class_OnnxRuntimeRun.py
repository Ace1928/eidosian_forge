...
class OnnxRuntimeRun(Run):
    ...
    def __init__(self, run_config):
        ...
        if self.static_quantization:
            ...
            calibrator = OnnxRuntimeCalibrator(calibration_dataset, quantizer, self.model_path, qconfig, calibration_params=run_config['calibration'], node_exclusion=run_config['node_exclusion'])
            ranges, quantization_preprocessor = calibrator.fit()
        quantizer.quantize(save_dir='./', calibration_tensors_range=ranges, quantization_config=qconfig, preprocessor=quantization_preprocessor)
        ...
        self.return_body['model_type'] = self.torch_model.config.model_type
    ...
    def _launch_time(self, trial):
        ...
        ort_benchmark = TimeBenchmark(self.ort_model, input_length=input_length, batch_size=batch_size, model_input_names=model_input_names, warmup_runs=self.time_benchmark_args['warmup_runs'], duration=self.time_benchmark_args['duration'])
        optimized_time_metrics = ort_benchmark.execute()
        print('Running Pytorch time benchmark.')
        torch_benchmark = TimeBenchmark(self.torch_model, input_length=input_length, batch_size=batch_size, model_input_names=model_input_names, warmup_runs=self.time_benchmark_args['warmup_runs'], duration=self.time_benchmark_args['duration'])
        baseline_time_metrics = torch_benchmark.execute()
        time_evaluation = {'batch_size': batch_size, 'input_length': input_length, 'baseline': baseline_time_metrics, 'optimized': optimized_time_metrics}
