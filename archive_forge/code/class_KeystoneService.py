...
class KeystoneService(resource.Resource):
    ...
    entity = 'services'
    PROPERTIES = NAME, DESCRIPTION, TYPE, ENABLED = ('name', 'description', 'type', 'enabled')
    properties_schema = {NAME: properties.Schema(properties.Schema.STRING, _('Name of keystone service.'), update_allowed=True), DESCRIPTION: properties.Schema(properties.Schema.STRING, _('Description of keystone service.'), update_allowed=True), TYPE: properties.Schema(properties.Schema.STRING, _('Type of keystone Service.'), update_allowed=True, required=True), ENABLED: properties.Schema(properties.Schema.BOOLEAN, _('This service is enabled or disabled.'), default=True, update_allowed=True, support_status=support.SupportStatus(version='6.0.0'))}
    ...
    def handle_create(self):
        ...
        service = self.client().services.create(name=name, description=description, type=type, enabled=enabled)
    ...
    def handle_update(self, json_snippet, tmpl_diff, prop_diff):
        if prop_diff:
            ...
            self.client().services.update(service=self.resource_id, name=name, description=description, type=type, enabled=enabled)