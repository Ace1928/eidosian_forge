...
class TestGeneratorsBipartite:
    ...
    def test_complete_bipartite_graph(self):
        ...
        assert mG.is_multigraph()
        assert sorted(mG.edges()) == sorted(G.edges())
        mG = complete_bipartite_graph(7, 3)
        assert sorted(mG.edges()) == sorted(G.edges())
    ...
    def test_configuration_model(self):
        ...
        pytest.raises(nx.NetworkXError, configuration_model, aseq, bseq, create_using=nx.DiGraph)
        pytest.raises(nx.NetworkXError, configuration_model, aseq, bseq, create_using=nx.MultiDiGraph)
    ...
    def test_reverse_havel_hakimi_graph(self):
        ...
        assert not G.is_multigraph()
        assert not G.is_directed()
        pytest.raises(nx.NetworkXError, reverse_havel_hakimi_graph, aseq, bseq, create_using=nx.DiGraph)
        pytest.raises(nx.NetworkXError, reverse_havel_hakimi_graph, aseq, bseq, create_using=nx.DiGraph)
        pytest.raises(nx.NetworkXError, reverse_havel_hakimi_graph, aseq, bseq, create_using=nx.MultiDiGraph)
