...
class ResourceDefinition(object):
    ...
    class Diff(object):
        ...
        def __bool__(self):
            ...
            return self.properties_changed() or self.metadata_changed() or self.update_policy_changed()
        __nonzero__ = __bool__
    DELETION_POLICIES = DELETE, RETAIN, SNAPSHOT = ('Delete', 'Retain', 'Snapshot')

    def __init__(self, name, resource_type, properties=None, metadata=None, depends=None, deletion_policy=None, update_policy=None, description=None, external_id=None, condition=None):
        ...
        if properties is not None:
            assert isinstance(properties, (collections.abc.Mapping, function.Function))
            self._hash ^= _hash_data(properties)
        if metadata is not None:
            assert isinstance(metadata, (collections.abc.Mapping, function.Function))
            self._hash ^= _hash_data(metadata)
        if depends is not None:
            assert isinstance(depends, (collections.abc.Sequence, function.Function))
        ...
        if update_policy is not None:
            assert isinstance(update_policy, (collections.abc.Mapping, function.Function))
            self._hash ^= _hash_data(update_policy)
        if external_id is not None:
            assert isinstance(external_id, (str, function.Function))
            self._hash ^= _hash_data(external_id)
            self._deletion_policy = self.RETAIN
        if condition is not None:
            assert isinstance(condition, (str, bool, function.Function))
    ...
    def freeze(self, **overrides):
        ...
        def arg_item(attr_name):
            ...
            return (name, value)
        args = ('name', 'resource_type', '_properties', '_metadata', '_depends', '_deletion_policy', '_update_policy', 'description', '_external_id', '_condition')
    ...
    def dep_attrs(self, resource_name, load_all=False):
        ...
        if self._all_dep_attrs is None and load_all:
            ...
            atts = itertools.chain(function.all_dep_attrs(self._properties), function.all_dep_attrs(self._metadata))
        ...
        return itertools.chain(function.dep_attrs(self._properties, resource_name), function.dep_attrs(self._metadata, resource_name))
        ...
        def get_resource(res_name):
            if res_name not in stack:
                if res_name in stack.defn.all_rsrc_names():
                    return
                raise exception.InvalidTemplateReference(resource=res_name, key=self.name)
            res = stack[res_name]
    ...
    def properties(self, schema, context=None):
        ...
        """
        props = properties.Properties(schema, self._properties or {}, function.resolve, context=context, section=PROPERTIES, rsrc_description=self.description)
    ...
    def update_policy(self, schema, context=None):
        ...
        require a context to validate constraints.
        """
        props = properties.Properties(schema, self._update_policy or {}, function.resolve, context=context, section=UPDATE_POLICY)
        props.update_translation(self._rules, self._client_resolve)
        return props
    ...
    def __repr__(self):
        ...
        args = ('properties', 'metadata', 'depends', 'deletion_policy', 'update_policy', 'condition')
        data = {'classname': type(self).__name__, 'name': repr(self.name), 'type': repr(self.resource_type), 'args': ', '.join((arg_repr(n) for n in args))}
