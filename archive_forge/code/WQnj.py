...
class Node:
    def __init__(self, x: int, y: int, f: int = 0, g: int = 0, h: int = 0):
        ...
        self.parent: Optional["Node"] = (
            None  # Reference to the parent node, initialized to None
        )
        logging.debug(
    ...
    async def print_node(self) -> None:
        """
        Asynchronously print the coordinates and costs of the node.
        """
        logging.info(
            f"Node position - x: {self.x}, y: {self.y}, f: {self.f}, g: {self.g}, h: {self.h}"
        )

    async def is_equal(self, other_node: "Node") -> bool:
        """
        Asynchronously check if two nodes are at the same coordinates.
...
class Grid:
    def __init__(self, cells=NO_OF_CELLS):
        """
    ...
    async def get_grid(self) -> list:
        """
        Asynchronously get the grid of nodes.

        Returns:
        list: The grid of nodes.
        """
        return self.grid
