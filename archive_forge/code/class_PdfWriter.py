...
class PdfWriter(PdfDocCommon):
    ...
    def add_uri(self, page_number: int, uri: str, rect: RectangleObject, border: Optional[ArrayObject]=None) -> None:
        ...
        lnk.update({NameObject(AA.Type): NameObject('/Annot'), NameObject(AA.Subtype): NameObject('/Link'), NameObject(AA.P): page_link, NameObject(AA.Rect): rect, NameObject('/H'): NameObject('/I'), NameObject(AA.Border): ArrayObject(border_arr), NameObject('/A'): lnk2})
    ...
    def merge(self, position: Optional[int], fileobj: Union[Path, StrByteType, PdfReader], outline_item: Optional[str]=None, pages: Optional[Union[PageRangeSpec, List[PageObject]]]=None, import_outline: bool=True, excluded_fields: Optional[Union[List[str], Tuple[str, ...]]]=()) -> None:
        ...
        if '/AcroForm' in _ro and _ro['/AcroForm'] is not None:
            if '/AcroForm' not in self._root_object:
                self._root_object[NameObject('/AcroForm')] = self._add_object(cast(DictionaryObject, reader.root_object['/AcroForm']).clone(self, False, ('/Fields',)))
                arr = ArrayObject()
            else:
