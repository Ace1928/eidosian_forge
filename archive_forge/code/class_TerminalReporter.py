...
class TerminalReporter:
    ...
    def wrap_write(self, content: str, *, flush: bool=False, margin: int=8, line_sep: str='\n', **markup: bool) -> None:
        ...
        wrapped = line_sep.join(textwrap.wrap(' ' * width_of_current_line + content, width=self._screen_width - margin, drop_whitespace=True, replace_whitespace=False))
    ...
    def write_sep(self, sep: str, title: Optional[str]=None, fullwidth: Optional[int]=None, **markup: bool) -> None:
        self.ensure_newline()
    ...
    def pytest_collection_finish(self, session: 'Session') -> None:
        ...
        lines = self.config.hook.pytest_report_collectionfinish(config=self.config, start_path=self.startpath, items=session.items)
        self._write_report_lines_from_hooks(lines)
    ...
    def pytest_sessionfinish(self, session: 'Session', exitstatus: Union[int, ExitCode]) -> Generator[None, None, None]:
        result = (yield)
        self._tw.line('')
        summary_exit_codes = (ExitCode.OK, ExitCode.TESTS_FAILED, ExitCode.INTERRUPTED, ExitCode.USAGE_ERROR, ExitCode.NO_TESTS_COLLECTED)
