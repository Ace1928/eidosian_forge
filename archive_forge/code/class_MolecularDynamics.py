...
class MolecularDynamics(Dynamics):
    ...
    def __init__(self, atoms, timestep, trajectory, logfile=None, loginterval=1, append_trajectory=False):
        ...
        self.max_steps = None
        if 0 in self.masses:
            warnings.warn('Zero mass encountered in atoms; this will likely lead to errors if the massless atoms are unconstrained.')
        ...
        if trajectory is not None:
            if isinstance(trajectory, str):
                ...
                trajectory = self.closelater(Trajectory(trajectory, mode=mode, atoms=atoms))
    ...
    def todict(self):
        return {'type': 'molecular-dynamics', 'md-type': self.__class__.__name__, 'timestep': self.dt}
