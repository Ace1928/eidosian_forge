...
class VMUtilsTestCase(test_base.OsWinBaseTestCase):
    ...
    _autospec_classes = [vmutils.jobutils.JobUtils, vmutils.pathutils.PathUtils]
    ...
    _FAKE_INSTANCE = {'name': _FAKE_VM_NAME, 'uuid': _FAKE_VM_UUID}
    ...
    _FAKE_MEMORY_INFO = {'DynamicMemoryEnabled': True, 'Reservation': 1024, 'Limit': 4096, 'Weight': 5000, 'MaxMemoryBlocksPerNumaNode': 2048}
    _FAKE_SUMMARY_INFO = {'NumberOfProcessors': 4, 'EnabledState': 2, 'MemoryUsage': 2, 'UpTime': 1}
    ...
    def test_lookup_vm_multiple(self):
        ...
        self.assertRaises(exceptions.HyperVException, self._vmutils._lookup_vm_check, self._FAKE_VM_NAME, as_vssd=False)
    ...
    @mock.patch.object(_wqlutils, 'get_element_associated_class')
    def _test_set_vm_memory_dynamic(self, mock_get_element_associated_class, dynamic_memory_ratio, mem_per_numa_node=None):
        mock_s = mock.MagicMock()
        mock_get_element_associated_class.return_value = [mock_s]
        self._vmutils._set_vm_memory(mock_s, self._FAKE_MEMORY_MB, mem_per_numa_node, dynamic_memory_ratio)
        self._vmutils._jobutils.modify_virt_resource.assert_called_once_with(mock_s)
        if mem_per_numa_node:
            self.assertEqual(mem_per_numa_node, mock_s.MaxMemoryBlocksPerNumaNode)
    ...
    def test_get_vm_ide_controller(self, mock_get_element_associated_class):
        self._prepare_get_vm_controller(self._vmutils._IDE_CTRL_RES_SUB_TYPE, mock_get_element_associated_class)
        path = self._vmutils.get_vm_ide_controller(mock.sentinel.FAKE_VM_SETTINGS, self._FAKE_ADDRESS)
        self.assertEqual(self._FAKE_RES_PATH, path)

    @mock.patch.object(_wqlutils, 'get_element_associated_class')
    def test_get_vm_ide_controller_none(self, mock_get_element_associated_class):
        self._prepare_get_vm_controller(self._vmutils._IDE_CTRL_RES_SUB_TYPE, mock_get_element_associated_class)
        path = self._vmutils.get_vm_ide_controller(mock.sentinel.FAKE_VM_SETTINGS, mock.sentinel.FAKE_NOT_FOUND_ADDR)
    ...
    def test_get_scsi_ctrl_addr(self, mock_get_wmi_obj, mock_get_ctrls):
        ...
        mock_get_wmi_obj.return_value = mock_rasds
        mock_scsi_ctrls = ['someCtrl', self._FAKE_CTRL_PATH.upper(), 'someOtherCtrl']
        exp_ctrl_addr = 1
        ...
        mock_get_ctrls.assert_called_once_with(mock.sentinel.vmsettings, self._vmutils._SCSI_CTRL_RES_SUB_TYPE)
    ...
    def test_get_vm_physical_disk_mapping(self, mock_get_vm_disks):
        ...
        expected_mapping = {expected_serial: {'resource_path': mock_phys_disk.path_.return_value, 'mounted_disk_path': mock_phys_disk.HostResource[0]}}
    ...
    def test_list_instances(self):
        ...
        self._vmutils._conn.Msvm_VirtualSystemSettingData.assert_called_with(['ElementName'], VirtualSystemType=self._vmutils._VIRTUAL_SYSTEM_TYPE_REALIZED)
    ...
    def test_get_event_wql_query(self):
        ...
        timeframe = 10
        filtered_states = [constants.HYPERV_VM_STATE_ENABLED, constants.HYPERV_VM_STATE_DISABLED]
        expected_checks = ' OR '.join(["TargetInstance.%s = '%s'" % (field, state) for state in filtered_states])
        expected_query = "SELECT %(field)s, TargetInstance FROM __InstanceModificationEvent WITHIN %(timeframe)s WHERE TargetInstance ISA '%(class)s' AND TargetInstance.%(field)s != PreviousInstance.%(field)s AND (%(checks)s)" % {'class': cls, 'field': field, 'timeframe': timeframe, 'checks': expected_checks}
        query = self._vmutils._get_event_wql_query(cls=cls, field=field, timeframe=timeframe, filtered_states=filtered_states)
    ...
    def test_add_pci_device(self):
        self.assertRaises(NotImplementedError, self._vmutils.add_pci_device, mock.sentinel.vm_name, mock.sentinel.vendor_id, mock.sentinel.product_id)
    ...
    def test_add_vtpm(self):
        self.assertRaises(NotImplementedError, self._vmutils.add_vtpm, mock.sentinel.vm_name, mock.sentinel.pdk_filepath, mock.sentinel.shielded)
