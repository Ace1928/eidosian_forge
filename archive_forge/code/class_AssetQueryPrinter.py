...
class AssetQueryPrinter(cp.CustomPrinterBase):
    ...
    def Register(parser):
        ...
        resource_printer.RegisterFormatter(ASSET_QUERY_PRINTER_FORMAT, AssetQueryPrinter, hidden=True)
        ...
        for field, v in zip(new_schema, values):
            ...
            if field['type'].upper() == 'RECORD':
                ...
                if field.get('mode', 'NULLABLE').upper() == 'REPEATED':
                    result[field['field']] = [self._ConvertFromFV(subfields, subvalue.get('v', ''), True) for subvalue in v]
                else:
