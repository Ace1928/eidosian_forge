...
def wireless_controller_wids_profile(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('wireless-controller', 'wids-profile', filtered_data, vdom=vdom)
        current_data = fos.get('wireless-controller', 'wids-profile', vdom=vdom, mkey=mkey)
    ...
    if state == 'present' or state is True:
        return fos.set('wireless-controller', 'wids-profile', data=filtered_data, vdom=vdom)
    elif state == 'absent':
        return fos.delete('wireless-controller', 'wids-profile', mkey=filtered_data['name'], vdom=vdom)
    else:
