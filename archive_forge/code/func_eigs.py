...
def eigs(A, k=6, M=None, sigma=None, which='LM', v0=None, ncv=None, maxiter=None, tol=0, return_eigenvectors=True, Minv=None, OPinv=None, OPpart=None):
    ...
    if M is not None:
        ...
        if np.dtype(M.dtype).char.lower() != np.dtype(A.dtype).char.lower():
            warnings.warn('M does not have the same type precision as A. This may adversely affect ARPACK convergence', stacklevel=2)
    n = A.shape[0]
    if k <= 0:
        raise ValueError('k=%d must be greater than 0.' % k)
    if k >= n - 1:
        warnings.warn('k >= N - 1 for N * N square matrix. Attempting to use scipy.linalg.eig instead.', RuntimeWarning, stacklevel=2)
        if issparse(A):
