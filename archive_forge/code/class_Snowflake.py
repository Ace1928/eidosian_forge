...
class Snowflake(Dialect):
    ...
    TIME_MAPPING = {'YYYY': '%Y', 'yyyy': '%Y', 'YY': '%y', 'yy': '%y', 'MMMM': '%B', 'mmmm': '%B', 'MON': '%b', 'mon': '%b', 'MM': '%m', 'mm': '%m', 'DD': '%d', 'dd': '%-d', 'DY': '%a', 'dy': '%w', 'HH24': '%H', 'hh24': '%H', 'HH12': '%I', 'hh12': '%I', 'MI': '%M', 'mi': '%M', 'SS': '%S', 'ss': '%S', 'FF': '%f', 'ff': '%f', 'FF6': '%f', 'ff6': '%f'}
    ...
    class Parser(parser.Parser):
        ...
        ID_VAR_TOKENS = {*parser.Parser.ID_VAR_TOKENS, TokenType.MATCH_CONDITION}
        TABLE_ALIAS_TOKENS = parser.Parser.TABLE_ALIAS_TOKENS | {TokenType.WINDOW}
        TABLE_ALIAS_TOKENS.discard(TokenType.MATCH_CONDITION)
        FUNCTIONS = {**parser.Parser.FUNCTIONS, 'ARRAYAGG': exp.ArrayAgg.from_arg_list, 'ARRAY_CONSTRUCT': exp.Array.from_arg_list, 'ARRAY_CONTAINS': lambda args: exp.ArrayContains(this=seq_get(args, 1), expression=seq_get(args, 0)), 'ARRAY_GENERATE_RANGE': lambda args: exp.GenerateSeries(start=seq_get(args, 0), end=exp.Sub(this=seq_get(args, 1), expression=exp.Literal.number(1)), step=seq_get(args, 2)), 'BITXOR': binary_from_function(exp.BitwiseXor), 'BIT_XOR': binary_from_function(exp.BitwiseXor), 'BOOLXOR': binary_from_function(exp.Xor), 'CONVERT_TIMEZONE': _build_convert_timezone, 'DATE': _build_datetime('DATE', exp.DataType.Type.DATE), 'DATE_TRUNC': _date_trunc_to_time, 'DATEADD': _build_date_time_add(exp.DateAdd), 'DATEDIFF': _build_datediff, 'DIV0': _build_if_from_div0, 'FLATTEN': exp.Explode.from_arg_list, 'GET_PATH': lambda args, dialect: exp.JSONExtract(this=seq_get(args, 0), expression=dialect.to_json_path(seq_get(args, 1))), 'IFF': exp.If.from_arg_list, 'LAST_DAY': lambda args: exp.LastDay(this=seq_get(args, 0), unit=_map_date_part(seq_get(args, 1))), 'LISTAGG': exp.GroupConcat.from_arg_list, 'MEDIAN': lambda args: exp.PercentileCont(this=seq_get(args, 0), expression=exp.Literal.number(0.5)), 'NULLIFZERO': _build_if_from_nullifzero, 'OBJECT_CONSTRUCT': _build_object_construct, 'REGEXP_REPLACE': _build_regexp_replace, 'REGEXP_SUBSTR': exp.RegexpExtract.from_arg_list, 'RLIKE': exp.RegexpLike.from_arg_list, 'SQUARE': lambda args: exp.Pow(this=seq_get(args, 0), expression=exp.Literal.number(2)), 'TIMEADD': _build_date_time_add(exp.TimeAdd), 'TIMEDIFF': _build_datediff, 'TIMESTAMPADD': _build_date_time_add(exp.DateAdd), 'TIMESTAMPDIFF': _build_datediff, 'TIMESTAMPFROMPARTS': _build_timestamp_from_parts, 'TIMESTAMP_FROM_PARTS': _build_timestamp_from_parts, 'TRY_TO_DATE': _build_datetime('TRY_TO_DATE', exp.DataType.Type.DATE, safe=True), 'TO_DATE': _build_datetime('TO_DATE', exp.DataType.Type.DATE), 'TO_NUMBER': lambda args: exp.ToNumber(this=seq_get(args, 0), format=seq_get(args, 1), precision=seq_get(args, 2), scale=seq_get(args, 3)), 'TO_TIME': _build_datetime('TO_TIME', exp.DataType.Type.TIME), 'TO_TIMESTAMP': _build_datetime('TO_TIMESTAMP', exp.DataType.Type.TIMESTAMP), 'TO_TIMESTAMP_LTZ': _build_datetime('TO_TIMESTAMP_LTZ', exp.DataType.Type.TIMESTAMPLTZ), 'TO_TIMESTAMP_NTZ': _build_datetime('TO_TIMESTAMP_NTZ', exp.DataType.Type.TIMESTAMP), 'TO_TIMESTAMP_TZ': _build_datetime('TO_TIMESTAMP_TZ', exp.DataType.Type.TIMESTAMPTZ), 'TO_VARCHAR': exp.ToChar.from_arg_list, 'ZEROIFNULL': _build_if_from_zeroifnull}
        FUNCTION_PARSERS = {**parser.Parser.FUNCTION_PARSERS, 'DATE_PART': lambda self: self._parse_date_part(), 'OBJECT_CONSTRUCT_KEEP_NULL': lambda self: self._parse_json_object()}
        FUNCTION_PARSERS.pop('TRIM')
        TIMESTAMPS = parser.Parser.TIMESTAMPS - {TokenType.TIME}
        RANGE_PARSERS = {**parser.Parser.RANGE_PARSERS, TokenType.LIKE_ANY: parser.binary_range_parser(exp.LikeAny), TokenType.ILIKE_ANY: parser.binary_range_parser(exp.ILikeAny)}
        ALTER_PARSERS = {**parser.Parser.ALTER_PARSERS, 'SET': lambda self: self._parse_set(tag=self._match_text_seq('TAG')), 'UNSET': lambda self: self.expression(exp.Set, tag=self._match_text_seq('TAG'), expressions=self._parse_csv(self._parse_id_var), unset=True), 'SWAP': lambda self: self._parse_alter_table_swap()}
        STATEMENT_PARSERS = {**parser.Parser.STATEMENT_PARSERS, TokenType.SHOW: lambda self: self._parse_show()}
        PROPERTY_PARSERS = {**parser.Parser.PROPERTY_PARSERS, 'LOCATION': lambda self: self._parse_location()}
        SHOW_PARSERS = {'SCHEMAS': _show_parser('SCHEMAS'), 'TERSE SCHEMAS': _show_parser('SCHEMAS'), 'OBJECTS': _show_parser('OBJECTS'), 'TERSE OBJECTS': _show_parser('OBJECTS'), 'TABLES': _show_parser('TABLES'), 'TERSE TABLES': _show_parser('TABLES'), 'VIEWS': _show_parser('VIEWS'), 'TERSE VIEWS': _show_parser('VIEWS'), 'PRIMARY KEYS': _show_parser('PRIMARY KEYS'), 'TERSE PRIMARY KEYS': _show_parser('PRIMARY KEYS'), 'IMPORTED KEYS': _show_parser('IMPORTED KEYS'), 'TERSE IMPORTED KEYS': _show_parser('IMPORTED KEYS'), 'UNIQUE KEYS': _show_parser('UNIQUE KEYS'), 'TERSE UNIQUE KEYS': _show_parser('UNIQUE KEYS'), 'SEQUENCES': _show_parser('SEQUENCES'), 'TERSE SEQUENCES': _show_parser('SEQUENCES'), 'COLUMNS': _show_parser('COLUMNS'), 'USERS': _show_parser('USERS'), 'TERSE USERS': _show_parser('USERS')}
        STAGED_FILE_SINGLE_TOKENS = {TokenType.DOT, TokenType.MOD, TokenType.SLASH}
    ...
    class Generator(generator.Generator):
        ...
        def unnest_sql(self, expression: exp.Unnest) -> str:
            ...
            columns = [exp.to_identifier('seq'), exp.to_identifier('key'), exp.to_identifier('path'), offset.pop() if isinstance(offset, exp.Expression) else exp.to_identifier('index'), seq_get(unnest_alias.columns if unnest_alias else [], 0) or exp.to_identifier('value'), exp.to_identifier('this')]
            if unnest_alias:
