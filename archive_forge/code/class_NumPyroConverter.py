...
class NumPyroConverter:
    ...
    def __init__(self, *, posterior=None, prior=None, posterior_predictive=None, predictions=None, constant_data=None, predictions_constant_data=None, log_likelihood=None, index_origin=None, coords=None, dims=None, pred_dims=None, num_chains=1):
        ...
        if posterior is not None:
            ...
            self.nchains, self.ndraws = (posterior.num_chains, posterior.num_samples // posterior.thinning)
            self.model = self.posterior.sampler.model
            self._args = self.posterior._args
            self._kwargs = self.posterior._kwargs
        else:
            ...
            if get_from is None and constant_data is None and (predictions_constant_data is None):
                raise ValueError('When constructing InferenceData must have at least one of posterior, prior, posterior_predictive or predictions.')
            if get_from is not None:
                aelem = arbitrary_element(get_from)
                self.ndraws = aelem.shape[0] // self.nchains
        observations = {}
        if self.model is not None:
            seeded_model = numpyro.handlers.substitute(numpyro.handlers.seed(self.model, jax.random.PRNGKey(0)), substitute_fn=numpyro.infer.init_to_sample)
    ...
    def posterior_to_xarray(self):
        ...
        data = self._samples
        return dict_to_dataset(data, library=self.numpyro, coords=self.coords, dims=self.dims, index_origin=self.index_origin)

    @requires('posterior')
    def sample_stats_to_xarray(self):
        """Extract sample_stats from NumPyro posterior."""
        rename_key = {'potential_energy': 'lp', 'adapt_state.step_size': 'step_size', 'num_steps': 'n_steps', 'accept_prob': 'acceptance_rate'}
        data = {}
        ...
        return dict_to_dataset(data, library=self.numpyro, dims=None, coords=self.coords, index_origin=self.index_origin)
    ...
    def log_likelihood_to_xarray(self):
        ...
        if self.observations is not None:
            ...
            if hasattr(samples, '_asdict'):
                samples = samples._asdict()
            log_likelihood_dict = self.numpyro.infer.log_likelihood(self.model, samples, *self._args, **self._kwargs)
        ...
        return dict_to_dataset(data, library=self.numpyro, dims=self.dims, coords=self.coords, index_origin=self.index_origin, skip_event_dims=True)
    ...
    def translate_posterior_predictive_dict_to_xarray(self, dct, dims):
        ...
        return dict_to_dataset(data, library=self.numpyro, coords=self.coords, dims=dims, index_origin=self.index_origin)

    @requires('posterior_predictive')
    def posterior_predictive_to_xarray(self):
        """Convert posterior_predictive samples to xarray."""
        return self.translate_posterior_predictive_dict_to_xarray(self.posterior_predictive, self.dims)
    ...
    def priors_to_xarray(self):
        ...
        priors_dict = {group: None if var_names is None else dict_to_dataset({k: utils.expand_dims(self.prior[k]) for k in var_names}, library=self.numpyro, coords=self.coords, dims=self.dims, index_origin=self.index_origin) for group, var_names in zip(('prior', 'prior_predictive'), (prior_vars, prior_predictive_vars))}
        return priors_dict
    ...
    def observed_data_to_xarray(self):
        ...
        return dict_to_dataset(self.observations, library=self.numpyro, dims=self.dims, coords=self.coords, default_dims=[], index_origin=self.index_origin)

    @requires('constant_data')
    def constant_data_to_xarray(self):
        """Convert constant_data to xarray."""
        return dict_to_dataset(self.constant_data, library=self.numpyro, dims=self.dims, coords=self.coords, default_dims=[], index_origin=self.index_origin)

    @requires('predictions_constant_data')
    def predictions_constant_data_to_xarray(self):
        """Convert predictions_constant_data to xarray."""
        return dict_to_dataset(self.predictions_constant_data, library=self.numpyro, dims=self.pred_dims, coords=self.coords, default_dims=[], index_origin=self.index_origin)
