...
class ClearMetadataPreprocessor(Preprocessor):
    ...
    """
    clear_cell_metadata = Bool(True, help='Flag to choose if cell metadata is to be cleared in addition to notebook metadata.').tag(config=True)
    clear_notebook_metadata = Bool(True, help='Flag to choose if notebook metadata is to be cleared in addition to cell metadata.').tag(config=True)
    preserve_nb_metadata_mask = Set([('language_info', 'name')], help='Indicates the key paths to preserve when deleting metadata across both cells and notebook metadata fields. Tuples of keys can be passed to preserved specific nested values').tag(config=True)
    preserve_cell_metadata_mask = Set(help='Indicates the key paths to preserve when deleting metadata across both cells and notebook metadata fields. Tuples of keys can be passed to preserved specific nested values').tag(config=True)
