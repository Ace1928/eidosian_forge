...
# Check if the model and tokenizer are already downloaded, if not, download and save them
...
# Move models to the specified device (CPU)
model.to(device)
embedding_model.to(device)
logging.info(f"Model moved to device: {device}")


# Cache model loading for efficient performance
...
def load_embedding_model(embedding_model_path):
    model = T5ForConditionalGeneration.from_pretrained(embedding_model_path)
    model.to(device)
    return model
