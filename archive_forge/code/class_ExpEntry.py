...
class ExpEntry(PDEntry, MSONable):
    ...
    def as_dict(self):
        """MSONable dict."""
        return {'@module': type(self).__module__, '@class': type(self).__name__, 'thermodata': [td.as_dict() for td in self._thermodata], 'composition': self.composition.as_dict(), 'temperature': self.temperature}