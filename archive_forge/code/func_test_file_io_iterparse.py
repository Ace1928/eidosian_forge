        ...
        with funcIO(f.read()) as b:
            if mode == 'r' and parser == 'lxml':
                with pytest.raises(TypeError, match='reading file objects must return bytes objects'):
                    read_xml(b, parser=parser, iterparse={'book': ['category', 'title', 'year', 'author', 'price']})
                return None
            else:
                df_fileio = read_xml(b, parser=parser, iterparse={'book': ['category', 'title', 'year', 'author', 'price']})
