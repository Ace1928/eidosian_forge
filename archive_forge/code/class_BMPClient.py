...
class BMPClient(Activity):
    ...
    def __init__(self, core_service, host, port):
        ...
        self._core_service.signal_bus.register_listener(BgpSignalBus.BGP_ADJ_RIB_IN_CHANGED, lambda _, data: self.on_adj_rib_in_changed(data))
        self._core_service.signal_bus.register_listener(BgpSignalBus.BGP_ADJ_UP, lambda _, data: self.on_adj_up(data))
        self._core_service.signal_bus.register_listener(BgpSignalBus.BGP_ADJ_DOWN, lambda _, data: self.on_adj_down(data))
        self._socket = None
    ...
    def _run(self):
        ...
        while True:
            ...
            try:
                ...
                self._connect_tcp(self.server_address, self._handle_bmp_session)
            except socket.error:
                self._connect_retry_event.set()
                LOG.info('Will try to reconnect to %s after %s secs: %s', self.server_address, self._connect_retry_time, self._connect_retry_event.is_set())
    ...
    def _construct_update(self, path):
        ...
        if path.is_withdraw:
            if isinstance(path, Ipv4Path):
                return BGPUpdate(withdrawn_routes=[path.nlri], path_attributes=new_pathattr)
            else:
        ...
        elif isinstance(path, Ipv4Path):
            return BGPUpdate(nlri=[path.nlri], path_attributes=new_pathattr)
