...
class TestVolumeSnapshotDelete(TestVolumeSnapshot):
    ...
    def test_snapshot_delete(self):
        ...
        self.snapshots_mock.delete.assert_called_with(self.snapshots[0].id, False)
    ...
    def test_snapshot_delete_with_force(self):
        ...
        parsed_args = self.check_parser(self.cmd, arglist, verifylist)
        result = self.cmd.take_action(parsed_args)
        self.snapshots_mock.delete.assert_called_with(self.snapshots[0].id, True)
        self.assertIsNone(result)
    ...
    def test_delete_multiple_snapshots_with_exception(self):
        arglist = [self.snapshots[0].id, 'unexist_snapshot']
        verifylist = [('snapshots', arglist)]
        parsed_args = self.check_parser(self.cmd, arglist, verifylist)
        ...
        with mock.patch.object(utils, 'find_resource', side_effect=find_mock_result) as find_mock:
            ...
            find_mock.assert_any_call(self.snapshots_mock, self.snapshots[0].id)
            find_mock.assert_any_call(self.snapshots_mock, 'unexist_snapshot')
            self.assertEqual(2, find_mock.call_count)
            self.snapshots_mock.delete.assert_called_once_with(self.snapshots[0].id, False)