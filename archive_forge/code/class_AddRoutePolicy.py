  ...
  *{command}* adds an empty route policy to a Compute Engine router.
  """
    ...
    def Args(cls, parser):
        ...
        parser.add_argument('--policy-name', help='Name of the route policy to add.', required=True)
        parser.add_argument('--policy-type', type=arg_utils.ChoiceToEnumName, choices={'IMPORT': 'The Route Policy is an Import Policy.', 'EXPORT': 'The Route Policy is an Export Policy.'}, help='Type of the route policy to add.', required=True)
    ...
    """
        ...
        router_ref = AddRoutePolicy.ROUTER_ARG.ResolveAsResource(args, holder.resources, scope_lister=compute_flags.GetDefaultScopeLister(client))
        route_policy = client.messages.RoutePolicy(name=args.policy_name, type=client.messages.RoutePolicy.TypeValueValuesEnum(self.ConvertRouteType(args.policy_type)))
        self.RequireRoutePolicyDoesNotExist(client, router_ref, args.policy_name)
        request = (client.apitools_client.routers, 'UpdateRoutePolicy', client.messages.ComputeRoutersUpdateRoutePolicyRequest(**router_ref.AsDict(), routePolicy=route_policy))
