...
class LocalProcessRunner(AbstractRunner):
    ...
    async def run(self, launch_project: LaunchProject, *args, **kwargs) -> Optional[AbstractRun]:
        if args is not None:
        ...
        if launch_project.uri and _is_wandb_uri(launch_project.uri):
            ...
            run_requirements_file = download_wandb_python_deps(source_entity, source_project, run_name, self._api, launch_project.project_dir)
            validate_wandb_python_deps(run_requirements_file, launch_project.project_dir)
