...
class SpatiaLiteOperations(BaseSpatialOperations, DatabaseOperations):
    ...
    gis_operators = {'equals': SpatialiteNullCheckOperator(func='Equals'), 'disjoint': SpatialiteNullCheckOperator(func='Disjoint'), 'touches': SpatialiteNullCheckOperator(func='Touches'), 'crosses': SpatialiteNullCheckOperator(func='Crosses'), 'within': SpatialiteNullCheckOperator(func='Within'), 'overlaps': SpatialiteNullCheckOperator(func='Overlaps'), 'contains': SpatialiteNullCheckOperator(func='Contains'), 'intersects': SpatialiteNullCheckOperator(func='Intersects'), 'relate': SpatialiteNullCheckOperator(func='Relate'), 'coveredby': SpatialiteNullCheckOperator(func='CoveredBy'), 'covers': SpatialiteNullCheckOperator(func='Covers'), 'contained': SpatialOperator(func='MbrWithin'), 'bbcontains': SpatialOperator(func='MbrContains'), 'bboverlaps': SpatialOperator(func='MbrOverlaps'), 'same_as': SpatialiteNullCheckOperator(func='Equals'), 'exact': SpatialiteNullCheckOperator(func='Equals'), 'dwithin': SpatialOperator(func='PtDistWithin')}
    disallowed_aggregates = (models.Extent3D,)
    select = 'CAST (AsEWKB(%s) AS BLOB)'
    function_names = {'AsWKB': 'St_AsBinary', 'ForcePolygonCW': 'ST_ForceLHR', 'FromWKB': 'ST_GeomFromWKB', 'FromWKT': 'ST_GeomFromText', 'Length': 'ST_Length', 'LineLocatePoint': 'ST_Line_Locate_Point', 'NumPoints': 'ST_NPoints', 'Reverse': 'ST_Reverse', 'Scale': 'ScaleCoords', 'Translate': 'ST_Translate', 'Union': 'ST_Union'}
    ...
    def spatial_version(self):
        ...
        try:
            ...
            raise ImproperlyConfigured('Cannot determine the SpatiaLite version for the "%s" database. Was the SpatiaLite initialization SQL loaded on this database?' % (self.connection.settings_dict['NAME'],)) from exc
        if version < (4, 3, 0):
