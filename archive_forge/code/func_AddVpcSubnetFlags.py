...
def AddVpcSubnetFlags(parser, resource_kind='service'):
    ...
    parser.add_argument('--subnet', metavar='SUBNET', help='The VPC subnetwork that the Cloud Run {kind} will get IPs from. The subnetwork must be `/26` or larger. If --network is also specified, subnet must be a subnetwork of the network specified by the --network flag. If --network is not specified, network will be looked up from this subnetwork. To clear existing VPC network settings, use --clear-network.'.format(kind=resource_kind))