...
class SinkWorker(threading.Thread):
    ...
    def _download(self, url):
        ...
        wrote = 0
        enc_data = b''
        for data in self._create_progress_iterator(response.iter_content(block_size), math.ceil(total_size // block_size) + 1, 'Download       '):
    ...
    def run(self):
        logger.debug('SinkWorker started, storage_dir=%s' % self._storage_dir)
        while True:
            ...
            if isinstance(media_message_protocolentity, DownloadableMediaMessageProtocolEntity):
                logger.info('Processing [url=%s, media_key=%s]' % (media_message_protocolentity.url, base64.b64encode(media_message_protocolentity.media_key)))
            else:
