...
class AcquireSsrsLease(base.Command):
    ...
    """
        ...
        request = sql_messages.SqlInstancesAcquireSsrsLeaseRequest(instance=instance_ref.instance, project=instance_ref.project, instancesAcquireSsrsLeaseRequest=sql_messages.InstancesAcquireSsrsLeaseRequest(acquireSsrsLeaseContext=sql_messages.AcquireSsrsLeaseContext(setupLogin=args.setup_login, serviceLogin=args.service_login, reportDatabase=args.report_database, duration=duration_str)))
        result = sql_client.instances.AcquireSsrsLease(request)
        operation_ref = client.resource_parser.Create('sql.operations', operation=result.operationId, project=instance_ref.project)
        operations.OperationsV1Beta4.WaitForOperation(sql_client, operation_ref, 'Acquiring SSRS lease')
