...
def _validate_new_param_value(self, param_path, param_key, run_id, new_value):
    ...
    if current_value != new_value:
        raise MlflowException(f"Changing param values is not allowed. Param with key='{param_key}' was already logged with value='{current_value}' for run ID='{run_id}'. Attempted logging new value '{new_value}'.", databricks_pb2.INVALID_PARAMETER_VALUE)