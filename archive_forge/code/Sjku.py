...
def hash_file(filepath: str) -> str:
    ...
    try:
        ...
        logging.error(f"Unable to open or read file {filepath}: {e}")
        raise
...
def store_embeddings(text: str, file_hash: str, filename: str) -> None:
    ...
    try:
        ...
        logging.error(f"Failed to store embeddings for {filename}: {e}")
        raise
...
def analyze_text(text: str, file_hash: str) -> None:
    ...
    try:
        ...
        logging.error(f"Failed to analyze text for {file_hash}: {e}")
        raise
...
def search_embeddings(query: str, num_results: int = 5) -> list:
    ...
    try:
        ...
        if not embeddings:
            logging.error(
                "No embeddings found. Ensure that documents are processed and embeddings are stored."
            )
            return []
...
def display_wordcloud(tab: ttk.Frame) -> None:
    ...
    if not wordcloud_data:
        label = Label(tab, text="No wordcloud data available.")
        label.pack()
    else:
