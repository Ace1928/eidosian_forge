...
class BaremetalPortTests(base.TestCase):
    ...
    def test_list(self):
        ...
        self.assertIn(self.port['address'], [port['Address'] for port in port_list])
        self.assertIn(self.port['uuid'], [port['UUID'] for port in port_list])
    ...
    def test_show_addr(self):
        ...
        3) Check port fields in output.
        """
        port = self.port_show(uuid='', params='--address {}'.format(self.port['address']))
        self.assertEqual(self.port['address'], port['address'])
        self.assertEqual(self.port['uuid'], port['uuid'])
        self.assertEqual(self.port['node_uuid'], self.node['uuid'])
    ...
    def test_delete(self):
        ...
        port_list = self.port_list()
        self.assertNotIn(self.port['address'], [port['Address'] for port in port_list])
        self.assertNotIn(self.port['uuid'], [port['UUID'] for port in port_list])
    ...
    def test_set_unset_extra(self):
        ...
        self.openstack('baremetal port set --extra {0}={1} {2}'.format(extra_key, extra_value, self.port['uuid']))
        show_prop = self.port_show(self.port['uuid'], ['extra'])
        self.assertEqual(extra_value, show_prop['extra'][extra_key])
        self.openstack('baremetal port unset --extra {0} {1}'.format(extra_key, self.port['uuid']))
        show_prop = self.port_show(self.port['uuid'], ['extra'])
