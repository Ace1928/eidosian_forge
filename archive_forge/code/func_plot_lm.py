...
def plot_lm(x, y, y_model, y_hat, num_samples, kind_pp, kind_model, length_plotters, xjitter, rows, cols, y_kwargs, y_hat_plot_kwargs, y_hat_fill_kwargs, y_model_plot_kwargs, y_model_fill_kwargs, y_model_mean_kwargs, backend_kwargs, show, figsize, textsize, axes, legend, grid):
    """Bokeh linreg plot."""
            ...
            if kind_pp == 'samples':
                ...
                for j in range(num_samples):
                    if xjitter is True:
                        ...
                        x_plotters_jitter = x_plotters + np.random.uniform(low=-scale_high, high=scale_high, size=len(x_plotters))
                        posterior_circle = ax_i.circle(x_plotters_jitter, y_hat_plotters[..., j], alpha=0.2, **y_hat_plot_kwargs)
                    else:
                        posterior_circle = ax_i.circle(x_plotters, y_hat_plotters[..., j], alpha=0.2, **y_hat_plot_kwargs)
                    posterior_legend.append(posterior_circle)
                legend_it.append(('Posterior predictive samples', posterior_legend))
            else:
                plot_hdi(x_plotters, y_hat_plotters, ax=ax_i, backend='bokeh', fill_kwargs=y_hat_fill_kwargs, show=False)
        if y_model is not None:
            _, _, _, y_model_plotters = y_model[i]
            if kind_model == 'lines':
                model_legend = ax_i.multi_line([np.tile(np.array(x_plotters, dtype=object), (num_samples, 1))], [np.transpose(y_model_plotters)], **y_model_plot_kwargs)
                legend_it.append(('Uncertainty in mean', [model_legend]))
                y_model_mean = np.mean(y_model_plotters, axis=1)
            else:
                plot_hdi(x_plotters, y_model_plotters, fill_kwargs=y_model_fill_kwargs, ax=ax_i, backend='bokeh', show=False)
