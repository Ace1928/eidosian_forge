...
class ElementSWVolumePair(object):
    ...
    def get_account_id(self, account, type):
        ...
        try:
            ...
            self.module.fail_json(msg='Error: either account %s or %s does not exist' % (self.parameters['src_account'], self.parameters['dest_account']), exception=to_native(err))
    ...
    def get_volume_id(self, volume, type):
        ...
        if type == 'src':
            ...
            if self.parameters['src_vol_id'] is None:
                self.module.fail_json(msg='Error: source volume %s does not exist' % self.parameters['src_volume'])
        elif type == 'dest':
            ...
            if self.parameters['dest_vol_id'] is None:
                self.module.fail_json(msg='Error: destination volume %s does not exist' % self.parameters['dest_volume'])
    ...
    def apply(self):
        ...
        """
        self.get_ids()
        paired = self.pairing_exists(self.parameters['src_vol_id'], self.parameters['dest_vol_id'])
