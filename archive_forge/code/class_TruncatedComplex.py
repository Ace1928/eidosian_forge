...
class TruncatedComplex:
    ...
    class Edge(SubcomplexBase):
        ...
        def reverse(self):
            ...
            """
            return TruncatedComplex.Edge(self.subcomplex_type, self.tet_and_perm_of_end())

        def __repr__(self):
            return 'TruncatedComplex.Edge(%r, %r)' % (self.subcomplex_type, self.tet_and_perm)
    ...
    def __init__(self, mcomplex):
        ...
        self.tet_and_perm_to_end_of_edge = {(tet.Index, p2.tuple()): end for i, edge in enumerate(mcomplex.Edges) for tet, perm in edge.embeddings() for end, p in [(0, perm), (1, perm * t3m.Perm4([1, 0, 2, 3]))] for p2 in [p, p * t3m.Perm4([0, 1, 3, 2])]}
    ...
    def get_key(self, tet_and_perm):
        ...
        return min(to_tuple(tet_and_perm), to_tuple(self.get_glued_tet_and_perm(tet_and_perm)))
