...
class EmbaasEmbeddings(BaseModel, Embeddings):
    ...
    def _handle_request(self, payload: EmbaasEmbeddingsPayload) -> List[List[float]]:
        ...
        headers = {'Authorization': f'Bearer {self.embaas_api_key.get_secret_value()}', 'Content-Type': 'application/json'}
        session = requests.Session()
        retries = Retry(total=self.max_retries, backoff_factor=0.5, allowed_methods=['POST'], raise_on_status=True)
