...
class AddPort(command.Command):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument('server', metavar='<server>', help=_('Server to add the port to (name or ID)'))
        parser.add_argument('port', metavar='<port>', help=_('Port to add to the server (name or ID)'))
        parser.add_argument('--tag', metavar='<tag>', help=_('Tag for the attached interface (supported by --os-compute-api-version 2.49 or later)'))
        return parser
