...
class SequentialDecomposition(FOQUSGraph):
    ...
    def __init__(self, **kwds):
        ...
        options['almost_equal_tol'] = 1e-08
            ...
            for sccIndex in lev:
                ...
                for ei in tset:
                    if ei in sccEdges[sccIndex]:
                        tears.append(ei)
                kwds = dict(G=G, order=order, function=function, tears=tears, iterLim=self.options['iterLim'], tol=self.options['tol'], tol_type=self.options['tol_type'], report_diffs=self.options['report_diffs'], outEdges=outEdges[sccIndex])
    ...
    def create_graph(self, model):
        ...
        for blk in model.block_data_objects(descend_into=True, active=True):
            for arc in blk.component_data_objects(Arc, descend_into=False):
                if not arc.directed:
                    raise ValueError("All Arcs must be directed when creating a graph for a model. Found undirected Arc: '%s'" % arc.name)
                if arc.expanded_block is None:
                    raise ValueError("All Arcs must be expanded when creating a graph for a model. Found unexpanded Arc: '%s'" % arc.name)
