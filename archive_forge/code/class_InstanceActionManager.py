...
class InstanceActionManager(base.ManagerWithFind):
    ...
    def get(self, server, request_id):
        ...
        """
        return self._get('/servers/%s/os-instance-actions/%s' % (base.getid(server), request_id), 'instanceAction')
    ...
    def list(self, server):
        ...
        """
        return self._list('/servers/%s/os-instance-actions' % base.getid(server), 'instanceActions')
    ...
    def list(self, server, marker=None, limit=None, changes_since=None, changes_before=None):
        """
        Get a list of actions performed on a server.
