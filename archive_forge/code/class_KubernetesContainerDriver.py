...
class KubernetesContainerDriver(KubernetesDriverMixin, ContainerDriver):
    ...
    def delete_namespace(self, namespace: KubernetesNamespace) -> bool:
        ...
        :rtype: ``bool``
        """
        self.connection.request(ROOT_URL + 'v1/namespaces/%s' % namespace.id, method='DELETE').object
        return True
    ...
    def create_namespace(self, name: str) -> KubernetesNamespace:
        ...
        result = self.connection.request(ROOT_URL + 'v1/namespaces', method='POST', data=json.dumps(request)).object
        return self._to_namespace(result)

    def deploy_container(self, name: str, image: ContainerImage, namespace: KubernetesNamespace=None, parameters: Optional[str]=None, start: Optional[bool]=True):
        """
        ...
        request = {'metadata': {'name': name}, 'spec': {'containers': [{'name': name, 'image': image.name}]}}
        result = self.connection.request(ROOT_URL + 'v1/namespaces/%s/pods' % namespace, method='POST', data=json.dumps(request)).object
        return self._to_namespace(result)
    ...
    def _to_container(self, data, container_status, pod_data):
        ...
        extra = {'pod': pod_data['metadata']['name'], 'namespace': pod_data['metadata']['namespace']}
        resources = data.get('resources')
        if resources:
            extra['resources'] = resources
        return Container(id=container_status.get('containerID') or data['name'], name=data['name'], image=ContainerImage(id=container_status.get('imageID') or data['image'], name=data['image'], path=None, version=None, driver=self.connection.driver), ip_addresses=None, state=ContainerState.RUNNING if container_status else ContainerState.UNKNOWN, driver=self.connection.driver, created_at=created_at, extra=extra)
