...
class GitHubAPIWrapper(BaseModel):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        if not installation:
            raise ValueError(f'Please make sure to install the created github app with id {github_app_id} on the repo: {github_repository}More instructions can be found at https://docs.github.com/en/apps/using-github-apps/installing-your-own-github-app')
        try:
        ...
        github_base_branch = get_from_dict_or_env(values, 'github_base_branch', 'GITHUB_BASE_BRANCH', default=repo.default_branch)
        active_branch = get_from_dict_or_env(values, 'active_branch', 'ACTIVE_BRANCH', default=repo.default_branch)
        values['github'] = g
    ...
    def get_issue(self, issue_number: int) -> Dict[str, Any]:
        ...
        return {'number': issue_number, 'title': issue.title, 'body': issue.body, 'comments': str(comments), 'opened_by': str(opened_by)}
...
made changes to add info"
        ...
        if self.github_base_branch == self.active_branch:
            ...
            try:
                ...
                pr = self.github_repo_instance.create_pull(title=title, body=body, head=self.active_branch, base=self.github_base_branch)
...
# Hello World!"
        ...
        try:
            try:
                ...
                pass
            self.github_repo_instance.create_file(path=file_path, message='Create ' + file_path, content=file_contents, branch=self.active_branch)
    ...
    def update_file(self, file_query: str) -> str:
        ...
        """
        if self.active_branch == self.github_base_branch:
            return f"You're attempting to commit to the directlyto the {self.github_base_branch} branch, which is protected. Please create a new branch and try again."
        try:
            ...
            if file_content == updated_file_content:
                return 'File content was not updated because old content was not found.It may be helpful to use the read_file action to get the current file contents.'
            self.github_repo_instance.update_file(path=file_path, message='Update ' + str(file_path), content=updated_file_content, branch=self.active_branch, sha=self.github_repo_instance.get_contents(file_path, ref=self.active_branch).sha)
    ...
    def delete_file(self, file_path: str) -> str:
        ...
        if self.active_branch == self.github_base_branch:
            return f"You're attempting to commit to the directlyto the {self.github_base_branch} branch, which is protected. Please create a new branch and try again."
        try:
            self.github_repo_instance.delete_file(path=file_path, message='Delete ' + file_path, branch=self.active_branch, sha=self.github_repo_instance.get_contents(file_path, ref=self.active_branch).sha)
