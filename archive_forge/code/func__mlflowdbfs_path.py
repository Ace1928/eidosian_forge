...
def _mlflowdbfs_path(run_id, artifact_path):
    if artifact_path.startswith('/'):
        raise MlflowException(f'artifact_path should be relative, found: {artifact_path}', INVALID_PARAMETER_VALUE)
    return '{}:///artifacts?run_id={}&path=/{}'.format(_MLFLOWDBFS_SCHEME, run_id, posixpath.join(artifact_path, _SPARK_MODEL_PATH_SUB))