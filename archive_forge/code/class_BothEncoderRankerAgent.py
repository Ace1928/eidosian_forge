...
class BothEncoderRankerAgent(TorchAgent):
    ...
    def add_cmdline_args(parser):
        ...
        parser.add_argument('--biencoder-model-file', type=str, default=None, help='path to biencoder model. Default to model-file_bi')
        parser.add_argument('--biencoder-top-n', type=int, default=10, help='default number of elements to keep from the biencoder response')
        parser.add_argument('--crossencoder-model-file', type=str, default=None, help='path to crossencoder model. Default to model-file_cross')
        parser.add_argument('--crossencoder-batchsize', type=int, default=-1, help='crossencoder will be fed those many elements at train or eval time.')
    ...
    def train_step(self, batch):
        ...
        for start in range(0, len(batch.text_vec), step):
            mbatch = Batch(text_vec=batch.text_vec[start:start + step], label_vec=batch.label_vec[start:start + step], candidate_vecs=batch.candidate_vecs[start:start + step], candidates=batch.candidates[start:start + step])
    ...
    def eval_step(self, batch):
        ...
        if output_biencoder is None:
            return None
        new_candidate_vecs = [self.biencoder.vectorize_fixed_candidates(cands[0:self.top_n_bi]) for cands in output_biencoder.text_candidates]
        new_candidates = [[c for c in cands[0:self.top_n_bi]] for cands in output_biencoder.text_candidates if cands is not None]
        copy_batch = Batch(text_vec=batch.text_vec, candidate_vecs=new_candidate_vecs, candidates=new_candidates)
