...
class ImageEditor(Component):
    ...
    def __init__(self, value: EditorValue | ImageType | None=None, *, height: int | str | None=None, width: int | str | None=None, image_mode: Literal['1', 'L', 'P', 'RGB', 'RGBA', 'CMYK', 'YCbCr', 'LAB', 'HSV', 'I', 'F']='RGBA', sources: Iterable[Literal['upload', 'webcam', 'clipboard']]=('upload', 'webcam', 'clipboard'), type: Literal['numpy', 'pil', 'filepath']='numpy', label: str | None=None, every: float | None=None, show_label: bool | None=None, show_download_button: bool=True, container: bool=True, scale: int | None=None, min_width: int=160, interactive: bool | None=None, visible: bool=True, elem_id: str | None=None, elem_classes: list[str] | str | None=None, render: bool=True, mirror_webcam: bool=True, show_share_button: bool | None=None, _selectable: bool=False, crop_size: tuple[int | float, int | float] | str | None=None, transforms: Iterable[Literal['crop']]=('crop',), eraser: Eraser | None | Literal[False]=None, brush: Brush | None | Literal[False]=None, format: str='webp', layers: bool=True):
        ...
        for source in sources:
            if source not in valid_sources:
                raise ValueError(f'`sources` must be a list consisting of elements in {valid_sources}')
    ...
    def convert_and_format_image(self, file: FileData | None | bytes) -> np.ndarray | PIL.Image.Image | str | None:
        if file is None:
            return None
        im = PIL.Image.open(file.path) if isinstance(file, FileData) else PIL.Image.open(BytesIO(file))
        ...
        return image_utils.format_image(im, cast(Literal['numpy', 'pil', 'filepath'], self.type), self.GRADIO_CACHE, format=suffix, name=name)
    ...
    def preprocess(self, payload: EditorData | None) -> EditorValue | None:
        ...
        if payload is not None and payload.id is not None:
            ...
            _payload = EditorDataBlobs(background=cached.background, layers=cached.layers, composite=cached.composite) if cached else None
        ...
        if _payload is not None:
            ...
            layers = [self.convert_and_format_image(layer) for layer in _payload.layers] if _payload.layers else None
            composite = self.convert_and_format_image(_payload.composite)
        if payload is not None and payload.id is not None:
            self.blob_storage.pop(payload.id)
        return {'background': bg, 'layers': [x for x in layers if x is not None] if layers else [], 'composite': composite}
    ...
    def postprocess(self, value: EditorValue | ImageType | None) -> EditorData | None:
        ...
        layers = [FileData(path=image_utils.save_image(cast(Union[np.ndarray, PIL.Image.Image, str], layer), self.GRADIO_CACHE, format=self.format)) for layer in value['layers']] if value['layers'] else []
    ...
    def example_payload(self) -> Any:
        return {'background': file('https://raw.githubusercontent.com/gradio-app/gradio/main/test/test_files/bus.png'), 'layers': [], 'composite': None}

    def example_value(self) -> Any:
        return {'background': 'https://raw.githubusercontent.com/gradio-app/gradio/main/test/test_files/bus.png', 'layers': [], 'composite': None}
