...
class SnapshotServiceTest(common.HeatTestCase):
    ...
    def test_delete_snapshot_not_belong_to_stack(self, mock_load):
        ...
        ex = self.assertRaises(dispatcher.ExpectedException, self.engine.delete_snapshot, self.ctx, stk2.identifier(), snapshot_id)
        expected = 'The Snapshot (%(snapshot)s) for Stack (%(stack)s) could not be found' % {'snapshot': snapshot_id, 'stack': stk2.name}
        self.assertEqual(exception.SnapshotNotFound, ex.exc_info[0])
        self.assertIn(expected, str(ex.exc_info[1]))
        mock_load.assert_called_once_with(self.ctx, stack=mock.ANY)
    ...
    def test_list_snapshots(self, mock_load):
        ...
        self.assertIsNotNone(snapshot['id'])
        self.assertEqual('IN_PROGRESS', snapshot['status'])
        snapshots = self.engine.stack_list_snapshots(self.ctx, stk.identifier())
        expected = {'id': snapshot['id'], 'name': 'snap1', 'status': 'COMPLETE', 'status_reason': 'Stack SNAPSHOT completed successfully', 'data': stk.prepare_abandon(), 'creation_time': snapshot['creation_time']}
