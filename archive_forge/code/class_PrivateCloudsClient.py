...
class PrivateCloudsClient(util.VmwareClientBase):
    ...
    def Get(self, resource):
        request = self.messages.VmwareengineProjectsLocationsPrivateCloudsGetRequest(name=resource.RelativeName())
        response = self.service.Get(request)
        return response

    def Create(self, resource, cluster_id, nodes_configs, network_cidr, vmware_engine_network_id, private_cloud_type, description=None, secondary_zone=None, preferred_zone=None, autoscaling_settings=None):
        ...
        network_config = self.messages.NetworkConfig(managementCidr=network_cidr, vmwareEngineNetwork=ven.name)
        management_cluster = self.messages.ManagementCluster(clusterId=cluster_id)
        management_cluster.nodeTypeConfigs = util.ConstructNodeParameterConfigMessage(self.messages.ManagementCluster.NodeTypeConfigsValue, self.messages.NodeTypeConfig, nodes_configs)
        if private_cloud.type is self.messages.PrivateCloud.TypeValueValuesEnum.STRETCHED:
            if not preferred_zone:
                raise PreferredZoneNotProvidedError()
            if not secondary_zone:
                raise SecondaryZoneNotProvidedError()
            management_cluster.stretchedClusterConfig = self.messages.StretchedClusterConfig(preferredLocation=preferred_zone, secondaryLocation=secondary_zone)
        management_cluster.autoscalingSettings = util.ConstructAutoscalingSettingsMessage(self.messages.AutoscalingSettings, self.messages.AutoscalingPolicy, self.messages.Thresholds, autoscaling_settings)
        private_cloud.managementCluster = management_cluster
        private_cloud.networkConfig = network_config
        request = self.messages.VmwareengineProjectsLocationsPrivateCloudsCreateRequest(parent=parent, privateCloudId=private_cloud_id, privateCloud=private_cloud)
    ...
    def Update(self, resource, description):
        ...
        request = self.messages.VmwareengineProjectsLocationsPrivateCloudsPatchRequest(privateCloud=private_cloud, name=resource.RelativeName(), updateMask=','.join(update_mask))
    ...
    def Delete(self, resource, delay_hours=None):
        return self.service.Delete(self.messages.VmwareengineProjectsLocationsPrivateCloudsDeleteRequest(name=resource.RelativeName(), delayHours=delay_hours))

    def List(self, location_resource):
        location = location_resource.RelativeName()
        request = self.messages.VmwareengineProjectsLocationsPrivateCloudsListRequest(parent=location)
        return list_pager.YieldFromList(self.service, request, batch_size_attribute='pageSize', field='privateClouds')

    def GetDnsForwarding(self, resource):
        request = self.messages.VmwareengineProjectsLocationsPrivateCloudsGetDnsForwardingRequest(name=resource.RelativeName() + '/dnsForwarding')
    ...
    def UpdateDnsForwarding(self, resource, args_rules):
        ...
        request = self.messages.VmwareengineProjectsLocationsPrivateCloudsUpdateDnsForwardingRequest(name=resource.RelativeName() + '/dnsForwarding', dnsForwarding=dns_forwarding, updateMask=update_mask)
    ...
    def _ParseRule(self, rule):
        return self.messages.ForwardingRule(domain=rule['domain'], nameServers=rule['name-servers'])
    ...
    def ResetNsxCredentials(self, resource):
        request = self.messages.VmwareengineProjectsLocationsPrivateCloudsResetNsxCredentialsRequest(privateCloud=resource.RelativeName())
    ...
    def ResetVcenterCredentials(self, resource, username=None):
        ...
        request = self.messages.VmwareengineProjectsLocationsPrivateCloudsResetVcenterCredentialsRequest(privateCloud=resource.RelativeName(), resetVcenterCredentialsRequest=vcenter)
