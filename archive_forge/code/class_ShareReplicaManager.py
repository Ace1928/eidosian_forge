...
class ShareReplicaManager(base.ManagerWithFind):
    ...
    @api_versions.wraps(constants.REPLICA_GRADUATION_VERSION, '2.74')
    def promote(self, replica):
        """Promote the provided replica.
    ...
    @api_versions.wraps('2.75')
    def promote(self, replica, quiesce_wait_time=None):
        """Promote the provided replica.
    ...
    def create(self, share, availability_zone=None):
        return self._create_share_replica(share, availability_zone=availability_zone)

    @api_versions.wraps(constants.REPLICA_GRADUATION_VERSION, '2.66')
    def create(self, share, availability_zone=None):
        return self._create_share_replica(share, availability_zone=availability_zone)

    @api_versions.wraps('2.67', '2.71')
    def create(self, share, availability_zone=None, scheduler_hints=None):
        return self._create_share_replica(share, availability_zone=availability_zone, scheduler_hints=scheduler_hints)

    @api_versions.wraps('2.72')
    def create(self, share, availability_zone=None, scheduler_hints=None, share_network=None):
        return self._create_share_replica(share, availability_zone=availability_zone, scheduler_hints=scheduler_hints, share_network=share_network)
