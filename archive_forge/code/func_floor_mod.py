...
def floor_mod(x: _atypes.TensorFuzzingAnnotation[TV_FloorMod_T], y: _atypes.TensorFuzzingAnnotation[TV_FloorMod_T], name=None) -> _atypes.TensorFuzzingAnnotation[TV_FloorMod_T]:
  ...
  *NOTE*: `math.floormod` supports broadcasting. More about broadcasting
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'FloorMod', name, x, y)
            return _result
            ...
            pass
        try:
            _result = _dispatcher_for_floor_mod((x, y, name), None)
            ...
            _result = _dispatch.dispatch(floor_mod, (), dict(x=x, y=y, name=name))
        ...
        if _result is not NotImplemented:
            return _result
    try:
        _, _, _op, _outputs = _op_def_library._apply_op_helper('FloorMod', x=x, y=y, name=name)
    except (TypeError, ValueError):
        _result = _dispatch.dispatch(floor_mod, (), dict(x=x, y=y, name=name))
        if _result is not _dispatch.OpDispatcher.NOT_SUPPORTED:
            return _result
