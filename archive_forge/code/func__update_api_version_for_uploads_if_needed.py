...
def _update_api_version_for_uploads_if_needed(client):
    ...
    insert_config = client.objects._upload_configs.get('Insert')
    if insert_config is None:
        return
    insert_config.simple_path = insert_config.simple_path.replace('v1', api_version)
    insert_config.resumable_path = insert_config.resumable_path.replace('v1', api_version)