...
class SubprocessSSHVendorTests(TestCase):
    ...
    def test_run_command_password(self):
        vendor = SubprocessSSHVendor()
        self.assertRaises(NotImplementedError, vendor.run_command, 'host', 'git-clone-url', password='12345')

    def test_run_command_password_and_privkey(self):
        vendor = SubprocessSSHVendor()
        self.assertRaises(NotImplementedError, vendor.run_command, 'host', 'git-clone-url', password='12345', key_filename='/tmp/id_rsa')

    def test_run_command_with_port_username_and_privkey(self):
        expected = ['ssh', '-x', '-p', '2200', '-i', '/tmp/id_rsa', 'user@host', 'git-clone-url']
        vendor = SubprocessSSHVendor()
    ...
    def test_run_with_ssh_command(self):
        expected = ['/path/to/ssh', '-o', 'Option=Value', '-x', 'host', 'git-clone-url']
        vendor = SubprocessSSHVendor()
        command = vendor.run_command('host', 'git-clone-url', ssh_command='/path/to/ssh -o Option=Value')
        args = command.proc.args
        self.assertListEqual(expected, args[0])