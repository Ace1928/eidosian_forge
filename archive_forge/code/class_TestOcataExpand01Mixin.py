...
class TestOcataExpand01Mixin(test_migrations.AlembicMigrationsMixin):
    ...
    def _check_ocata_expand01(self, engine, data):
        ...
        with engine.connect() as conn:
            rows = conn.execute(images.select().where(images.c.id.like('%_before_expand')).order_by(images.c.id)).fetchall()
    ...
    def _test_trigger_old_to_new(self, engine, images):
        now = datetime.datetime.now()
        public_temp = dict(deleted=False, created_at=now, status='active', is_public=True, min_disk=0, min_ram=0, id='public_id_old_to_new')
        with engine.connect() as conn, conn.begin():
            conn.execute(images.insert().values(public_temp))
        shared_temp = dict(deleted=False, created_at=now, status='active', is_public=False, min_disk=0, min_ram=0, id='private_id_old_to_new')
        with engine.connect() as conn, conn.begin():
            conn.execute(images.insert().values(shared_temp))
        with engine.connect() as conn:
            rows = conn.execute(images.select().where(images.c.id.like('%_old_to_new')).order_by(images.c.id)).fetchall()
        self.assertEqual(2, len(rows))
        self.assertEqual(0, rows[0].is_public)
