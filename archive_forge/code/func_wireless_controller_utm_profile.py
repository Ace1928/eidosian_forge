...
def wireless_controller_utm_profile(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('wireless-controller', 'utm-profile', filtered_data, vdom=vdom)
        current_data = fos.get('wireless-controller', 'utm-profile', vdom=vdom, mkey=mkey)
    ...
    if state == 'present' or state is True:
        return fos.set('wireless-controller', 'utm-profile', data=filtered_data, vdom=vdom)
    elif state == 'absent':
        return fos.delete('wireless-controller', 'utm-profile', mkey=filtered_data['name'], vdom=vdom)
    else:
