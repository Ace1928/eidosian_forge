...
class CinderVolume(vb.BaseVolume, sh.SchedulerHintsMixin):
    ...
    def _check_extend_volume_complete(self):
        ...
        if vol.status != 'available':
            LOG.info('Resize failed: Volume %(vol)s is in %(status)s state.', {'vol': vol.id, 'status': vol.status})
            raise exception.ResourceUnknownStatus(resource_status=vol.status, result=_('Volume resize failed'))
    ...
    def handle_update(self, json_snippet, tmpl_diff, prop_diff):
        ...
        if self.NAME in prop_diff or self.DESCRIPTION in prop_diff:
            vol = cinder.volumes.get(self.resource_id)
            update_name = prop_diff.get(self.NAME) or self.properties[self.NAME]
            update_description = prop_diff.get(self.DESCRIPTION) or self.properties[self.DESCRIPTION]
            kwargs = self._fetch_name_and_description(update_name, update_description)
        ...
        if self.READ_ONLY in prop_diff:
            ...
            prg_access = progress.VolumeUpdateAccessModeProgress(read_only=flag)
            prg_detach, prg_attach = self._detach_attach_progress(vol)
        if self.BACKUP_ID in prop_diff:
            if not vol:
                vol = cinder.volumes.get(self.resource_id)
            prg_restore = progress.VolumeBackupRestoreProgress(vol_id=self.resource_id, backup_id=prop_diff.get(self.BACKUP_ID))
            prg_detach, prg_attach = self._detach_attach_progress(vol)
    ...
    def _detach_attach_progress(self, vol):
        ...
        if vol.attachments:
            ...
            prg_detach = progress.VolumeDetachProgress(server_id, vol.id, attachment_id)
            prg_attach = progress.VolumeAttachProgress(server_id, vol.id, device)
    ...
    def check_update_complete(self, checkers):
        ...
        if prg_restore:
            if not prg_restore.called:
                prg_restore.called = self._backup_restore(prg_restore.vol_id, prg_restore.backup_id)
                return False
        ...
        if prg_access:
            if not prg_access.called:
                prg_access.called = self._update_read_only(prg_access.read_only)
                return False
        if prg_attach:
    ...
    def parse_live_resource_data(self, resource_properties, resource_data):
        volume_reality = {}
        if resource_data.get(self.METADATA) and resource_data.get(self.METADATA).get('readonly') is not None:
        ...
        props_keys = [self.SIZE, self.NAME, self.DESCRIPTION, self.METADATA, self.BACKUP_ID]
