  ...
  *{command}* downloads a route policy from a Compute Engine router.
  """
    ...
    def Args(cls, parser):
        ...
        parser.add_argument('--policy-name', required=True, help='Name of the route policy to download.')
        parser.add_argument('--file-name', required=True, help='The name of the file to download the route policy config to.')
    ...
    def Run(self, args):
        ...
        router_ref = DownloadRoutePolicy.ROUTER_ARG.ResolveAsResource(args, holder.resources, scope_lister=compute_flags.GetDefaultScopeLister(client))
    ...
    def WriteToFile(self, message, file_name, file_format):
        ...
        with files.FileWriter(file_name) as export_file:
            resource_printer.Print(resources=message, print_format=file_format, out=export_file)