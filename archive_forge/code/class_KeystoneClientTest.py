...
class KeystoneClientTest(common.HeatTestCase):
    ...
    def setUp(self):
        ...
        self.mock_ks_v3_client_domain_mngr = self.patchobject(kc_v3_domains, 'DomainManager', spec=kc_v3_domains.DomainManager)
        self.mock_ks_v3_client = mock.Mock()
        self.mock_ks_v3_client.domains = self.mock_ks_v3_client_domain_mngr.return_value
        self.m_client = self.patchobject(kc_v3, 'Client', return_value=self.mock_ks_v3_client)
        ...
        self.m_load_auth = self.patchobject(ks_loading, 'load_auth_from_conf_options')
        cfg.CONF.set_override('auth_uri', 'http://server.test:5000/v2.0', group='keystone_authtoken')
    ...
    def _validate_stub_domain_admin_client(self):
        self.m_password.assert_called_once_with(auth_url='http://server.test:5000/v3', password='adminsecret', domain_id='adomain123', domain_name=None, user_domain_id='adomain123', user_domain_name=None, username='adminuser123')
    ...
    def test_create_stack_user_error_norole(self):
        ...
        heat_ks_client = heat_keystoneclient.KeystoneClient(ctx)
        err = self.assertRaises(exception.Error, heat_ks_client.create_stack_user, 'auser', password='password')
    ...
    def test_create_stack_domain_user(self):
        ...
        ctx.trust_id = None
        user_options = dict(ignore_password_expiry=True, ignore_change_password_upon_first_use=True, ignore_lockout_failure_attempts=True)
        self._stub_domain_admin_client()
        self.mock_ks_v3_client.users.create.return_value.id = 'duser123'
        self.mock_ks_v3_client.roles.list.return_value = self._mock_roles_list()
        self.mock_ks_v3_client.roles.grant.return_value = None
        heat_ks_client = heat_keystoneclient.KeystoneClient(ctx)
        heat_ks_client.create_stack_domain_user(username='duser', project_id='aproject')
        self._validate_stub_domain_admin_client()
        self.mock_ks_v3_client.users.create.assert_called_once_with(name='duser', password=None, default_project='aproject', domain='adomain123', options=user_options)
        self.mock_ks_v3_client.roles.grant.assert_called_once_with(project='aproject', role='4546', user='duser123')
        self.mock_ks_v3_client.roles.list.assert_called_once_with(name='heat_stack_user')
    ...
    def test_delete_stack_domain_user(self):
        ...
        self.mock_ks_v3_client.users.get.side_effect = [mock_user, kc_exception.NotFound]
        heat_ks_client = heat_keystoneclient.KeystoneClient(ctx)
        heat_ks_client.delete_stack_domain_user(user_id='duser123', project_id='aproject')
        heat_ks_client.delete_stack_domain_user(user_id='duser123', project_id='aproject')
        self._validate_stub_domain_admin_client()
    ...
    def test_delete_stack_domain_user_error_domain(self):
        ...
        ctx.trust_id = None
        self._stub_domain_admin_client()
        self._stubs_get_user(user_id='duser123', domain_id='notadomain123', default_project_id='aproject')
        heat_ks_client = heat_keystoneclient.KeystoneClient(ctx)
        err = self.assertRaises(ValueError, heat_ks_client.delete_stack_domain_user, user_id='duser123', project_id='aproject')
    ...
    def test_delete_stack_domain_user_error_project(self):
        ...
        ctx.trust_id = None
        self._stub_domain_admin_client()
        self._stubs_get_user(user_id='duser123', domain_id='adomain123', default_project_id='notaproject')
        heat_ks_client = heat_keystoneclient.KeystoneClient(ctx)
        err = self.assertRaises(ValueError, heat_ks_client.delete_stack_domain_user, user_id='duser123', project_id='aproject')
    ...
    def test_init_v3_token_auth_ref_v2(self):
        ...
        expected_auth_ref = {'token': {'id': 'ctx_token', 'expires': '123'}, 'version': 'v2.0'}
        self._stubs_auth(method='auth_ref', auth_ref=expected_auth_ref, version=2)
        ctx = utils.dummy_context()
        ctx.username = None
        ctx.password = None
        ctx.trust_id = None
        ctx.auth_token = 'ctx_token'
        ctx.auth_token_info = {'access': {'token': {'id': 'abcd1234', 'expires': '123'}}}
        heat_ks_client = heat_keystoneclient.KeystoneClient(ctx)
        heat_ks_client.client
        self.assertIsNotNone(heat_ks_client._client)
        self._validate_stub_auth()
    ...
    def test_init_v3_token_auth_ref_v3(self):
        ...
        expected_auth_ref = {'auth_token': 'ctx_token', 'expires': '456', 'version': 'v3', 'methods': []}
    ...
    def _test_create_trust_context_trust_create(self, delegate_roles=None, redelegate=False):
        ...
        self.assertEqual('5678', trust_context.trustor_user_id)
        self.m_load_auth.assert_called_once_with(cfg.CONF, 'trustee', trust_id=None)
        self.mock_ks_v3_client.trusts.create.assert_called_once_with(allow_redelegation=redelegate, trustor_user='5678', trustee_user='1234', project='42', impersonation=True, role_names=trustee_roles)
    ...
    def test_disable_stack_user(self):
        ...
        self.mock_ks_v3_client.users.update.assert_called_once_with(user='atestuser', enabled=False)
        self._validate_stub_auth()
    ...
    def test_enable_stack_user(self):
        ...
        self.mock_ks_v3_client.users.update.assert_called_once_with(user='atestuser', enabled=True)
        self._validate_stub_auth()
    ...
    def test_enable_stack_domain_user(self):
        ...
        heat_ks_client = heat_keystoneclient.KeystoneClient(ctx)
        heat_ks_client.enable_stack_domain_user(user_id='duser123', project_id='aproject')
        self._validate_stub_domain_admin_client()
        self.mock_ks_v3_client.users.get.assert_called_once_with('duser123')
        self.mock_ks_v3_client.users.update.assert_called_once_with(user='duser123', enabled=True)
    ...
    def test_enable_stack_domain_user_legacy_fallback(self):
        ...
        self._stubs_auth()
        heat_ks_client = heat_keystoneclient.KeystoneClient(ctx)
        heat_ks_client.enable_stack_domain_user(user_id='user123', project_id='aproject')
        self.mock_ks_v3_client.users.update.assert_called_once_with(user='user123', enabled=True)
        self._validate_stub_auth()
    ...
    def test_enable_stack_domain_user_error_project(self):
        ...
        heat_ks_client = heat_keystoneclient.KeystoneClient(ctx)
        self.assertRaises(ValueError, heat_ks_client.enable_stack_domain_user, user_id='duser123', project_id='aproject')
        self._validate_stub_domain_admin_client()
        self.mock_ks_v3_client.users.get.assert_called_once_with('duser123')
    ...
    def test_disable_stack_domain_user(self):
        ...
        self._stubs_get_user('duser123', 'adomain123', 'aproject')
        heat_ks_client = heat_keystoneclient.KeystoneClient(ctx)
        heat_ks_client.disable_stack_domain_user(user_id='duser123', project_id='aproject')
        self._validate_stub_domain_admin_client()
        self.mock_ks_v3_client.users.get.assert_called_once_with('duser123')
        self.mock_ks_v3_client.users.update.assert_called_once_with(user='duser123', enabled=False)
    ...
    def test_disable_stack_domain_user_legacy_fallback(self):
        ...
        self._stubs_auth()
        heat_ks_client = heat_keystoneclient.KeystoneClient(ctx)
        heat_ks_client.disable_stack_domain_user(user_id='user123', project_id='aproject')
        self.mock_ks_v3_client.users.update.assert_called_once_with(user='user123', enabled=False)
        self._validate_stub_auth()
    ...
    def test_disable_stack_domain_user_error_project(self):
        ...
        self._stubs_get_user('duser123', 'adomain123', 'notaproject')
        heat_ks_client = heat_keystoneclient.KeystoneClient(ctx)
        self.assertRaises(ValueError, heat_ks_client.disable_stack_domain_user, user_id='duser123', project_id='aproject')
        self._validate_stub_domain_admin_client()
        self.mock_ks_v3_client.users.get.assert_called_once_with('duser123')
    ...
    def test_delete_stack_domain_user_keypair(self):
        ...
        self._validate_stub_domain_admin_client()
        self.mock_ks_v3_client.users.get.assert_called_with('duser123')
        self.mock_ks_v3_client.credentials.delete.assert_called_with('acredentialid')
    ...
    def test_delete_stack_domain_user_keypair_legacy_fallback(self):
        ...
        self._stubs_auth()
        heat_ks_client = heat_keystoneclient.KeystoneClient(ctx)
        heat_ks_client.delete_stack_domain_user_keypair(user_id='user123', project_id='aproject', credential_id='acredentialid')
        self.mock_ks_v3_client.credentials.delete.assert_called_once_with('acredentialid')
        self._validate_stub_auth()
    ...
    def test_delete_stack_domain_user_keypair_error_project(self):
        ...
        self._stubs_get_user('duser123', 'adomain123', 'notaproject')
        heat_ks_client = heat_keystoneclient.KeystoneClient(ctx)
        self.assertRaises(ValueError, heat_ks_client.delete_stack_domain_user_keypair, user_id='duser123', project_id='aproject', credential_id='acredentialid')
        self._validate_stub_domain_admin_client()
        self.mock_ks_v3_client.users.get.assert_called_once_with('duser123')
    ...
    def _stub_gen_creds(self, access, secret):
        ...
        self.patchobject(password_gen, 'generate_openstack_password', return_value=secret)
    ...
    def test_create_ec2_keypair(self):
        ...
        ctx = utils.dummy_context()
        ctx.trust_id = None
        ex_data = {'access': 'dummy_access', 'secret': 'dummy_secret'}
        ...
        self.mock_ks_v3_client.credentials.create.assert_called_once_with(user='atestuser', type='ec2', blob=ex_data_json, project=ctx.tenant_id)
        self._validate_stub_auth()
    ...
    def test_create_stack_domain_user_keypair(self):
        ...
        self._validate_stub_domain_admin_client()
        self.mock_ks_v3_client.credentials.create.assert_called_once_with(user='atestuser2', type='ec2', blob=ex_data_json, project='aproject')
    ...
    def _mock_credential_list(self, user_id):
        ...
        for x in (1, 2, 3):
            ...
            mock_credential.blob = json.dumps({'access': 'access%s' % x, 'secret': 'secret%s' % x})
    ...
    def test_create_stack_domain_project(self):
        ...
        heat_ks_client = heat_keystoneclient.KeystoneClient(ctx)
        self.assertEqual('aproject123', heat_ks_client.create_stack_domain_project('astack'))
        self._validate_stub_domain_admin_client()
        self.mock_ks_v3_client.projects.create.assert_called_once_with(name=expected_name, domain='adomain123', description='Heat stack user project')
    ...
    def test_stack_domain_user_token_err_nodomain(self):
        ...
        ctx.trust_id = None
        heat_ks_client = heat_keystoneclient.KeystoneClient(ctx)
        self.assertRaises(exception.Error, heat_ks_client.stack_domain_user_token, user_id='user', project_id='aproject', password='password')
    ...
    def test_delete_stack_domain_project_legacy_fallback(self):
        ...
        ctx.trust_id = None
        heat_ks_client = heat_keystoneclient.KeystoneClient(ctx)
        self.assertIsNone(heat_ks_client.delete_stack_domain_project(project_id='aprojectid'))
    ...
    def test_server_keystone_endpoint_url_auth_exception(self):
        ...
        heat_ks_client.context.auth_plugin.get_access.side_effect = kc_exception.Unauthorized
        fallback_url = 'http://server.fallback.test:5000/v3'
        self.assertEqual(heat_ks_client.server_keystone_endpoint_url(fallback_url), fallback_url)
        cfg.CONF.clear_override('server_keystone_endpoint_type')