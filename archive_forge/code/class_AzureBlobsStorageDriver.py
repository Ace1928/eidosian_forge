...
class AzureBlobsStorageDriver(StorageDriver):
    ...
    def __init__(self, key, secret=None, secure=True, host=None, port=None, tenant_id=None, identity=None, auth_type=None, cloud_environment='default', **kwargs):
        self._host = host
    ...
    def _ex_connection_class_kwargs(self):
        ...
        if self._host is None:
            ...
            return kwargs
        try:
            host_suffix = next((host_suffix for host_suffix in (AZURE_STORAGE_HOST_SUFFIX_CHINA, AZURE_STORAGE_HOST_SUFFIX_GOVERNMENT, AZURE_STORAGE_HOST_SUFFIX_PRIVATELINK) if self._host.endswith(host_suffix)))
    ...
    def iterate_containers(self):
        ...
        """
        params = {'comp': 'list', 'maxresults': RESPONSES_PER_REQUEST, 'include': 'metadata'}
        while True:
    ...
    def iterate_container_objects(self, container, prefix=None, ex_prefix=None):
        ...
        params = {'restype': 'container', 'comp': 'list', 'maxresults': RESPONSES_PER_REQUEST, 'include': 'metadata'}
        if prefix:
    ...
    def get_container(self, container_name):
        ...
        if response.status == httplib.NOT_FOUND:
            raise ContainerDoesNotExistError('Container %s does not exist' % container_name, driver=self, container_name=container_name)
        elif response.status != httplib.OK:
            raise LibcloudError('Unexpected status code: %s' % response.status, driver=self)
    ...
    def get_object_cdn_url(self, obj, ex_expiry=AZURE_STORAGE_CDN_URL_EXPIRY_HOURS):
        ...
        params = {'st': start.strftime(AZURE_STORAGE_CDN_URL_DATE_FORMAT), 'se': expiry.strftime(AZURE_STORAGE_CDN_URL_DATE_FORMAT), 'sp': 'r', 'spr': 'https' if self.secure else 'http,https', 'sv': self.connectionCls.API_VERSION, 'sr': 'b'}
        string_to_sign = '\n'.join((params['sp'], params['st'], params['se'], '/blob/{}{}'.format(self.key, object_path), '', '', params['spr'], params['sv'], params['sr'], '', '', '', '', '', ''))
        params['sig'] = base64.b64encode(hmac.new(self.secret, string_to_sign.encode('utf-8'), hashlib.sha256).digest()).decode('utf-8')
        return '{scheme}://{host}:{port}{action}?{sas_token}'.format(scheme='https' if self.secure else 'http', host=self.connection.host, port=self.connection.port, action=self.connection.morph_action_hook(object_path), sas_token=urlencode(params))
    ...
    def create_container(self, container_name):
        ...
        if response.status == httplib.CREATED:
            ...
            raise ContainerAlreadyExistsError(value='Container with this name already exists. The name must be unique among all the containers in the system', container_name=container_name, driver=self)
    ...
    def download_object_as_stream(self, obj, chunk_size=None):
        ...
        return self._get_object(obj=obj, callback=read_in_chunks, response=response, callback_kwargs={'iterator': iterator, 'chunk_size': chunk_size}, success_status_code=httplib.OK)
    ...
    def download_object_range(self, obj, destination_path, start_bytes, end_bytes=None, overwrite_existing=False, delete_on_failure=True):
        ...
        return self._get_object(obj=obj, callback=self._save_object, response=response, callback_kwargs={'obj': obj, 'response': response.response, 'destination_path': destination_path, 'overwrite_existing': overwrite_existing, 'delete_on_failure': delete_on_failure, 'partial_download': True}, success_status_code=success_status_codes)
    ...
    def download_object_range_as_stream(self, obj, start_bytes, end_bytes=None, chunk_size=None):
        ...
        iterator = response.iter_content(AZURE_DOWNLOAD_CHUNK_SIZE)
        success_status_codes = [httplib.OK, httplib.PARTIAL_CONTENT]
        return self._get_object(obj=obj, callback=read_in_chunks, response=response, callback_kwargs={'iterator': iterator, 'chunk_size': chunk_size}, success_status_code=success_status_codes)
    ...
    def upload_object(self, file_path, container, object_name, verify_hash=True, extra=None, headers=None, ex_use_lease=False, **deprecated_kwargs):
        """
        ...
        if deprecated_kwargs:
            raise ValueError('Support for arguments was removed: %s' % ', '.join(deprecated_kwargs.keys()))
        blob_size = os.stat(file_path).st_size
        with open(file_path, 'rb') as fobj:
            return self._put_object(container=container, object_name=object_name, extra=extra, verify_hash=verify_hash, use_lease=ex_use_lease, headers=headers, blob_size=blob_size, file_path=file_path, stream=fobj)

    def upload_object_via_stream(self, iterator, container, object_name, verify_hash=True, extra=None, headers=None, ex_use_lease=False, **deprecated_kwargs):
        """
        ...
        if deprecated_kwargs:
            raise ValueError('Support for arguments was removed: %s' % ', '.join(deprecated_kwargs.keys()))
        return self._put_object(container=container, object_name=object_name, extra=extra, verify_hash=verify_hash, use_lease=ex_use_lease, headers=headers, blob_size=None, stream=iterator)
    ...
    def _put_object(self, container, object_name, stream, extra=None, verify_hash=True, headers=None, blob_size=None, file_path=None, use_lease=False):
        """
        ...
        with AzureBlobLease(self, object_path, use_lease) as lease:
            if blob_size is not None and blob_size <= AZURE_UPLOAD_CHUNK_SIZE:
                result_dict = self._upload_directly(stream=stream, object_path=object_path, lease=lease, blob_size=blob_size, meta_data=meta_data, headers=headers, content_type=content_type, object_name=object_name, file_path=file_path)
            else:
                result_dict = self._upload_in_chunks(stream=stream, object_path=object_path, lease=lease, meta_data=meta_data, headers=headers, content_type=content_type, object_name=object_name, file_path=file_path, verify_hash=verify_hash)
        ...
        if verify_hash and server_hash and (data_hash != server_hash):
            raise ObjectHashMismatchError(value='MD5 hash checksum does not match', object_name=object_name, driver=self)
        return Object(name=object_name, size=bytes_transferred, hash=headers['etag'], extra=None, meta_data=meta_data, container=container, driver=self)
