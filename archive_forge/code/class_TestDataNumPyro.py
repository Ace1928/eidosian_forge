...
class TestDataNumPyro:
    ...
    def predictions_data(self, data, predictions_params):
        ...
        predictions = Predictive(model, posterior_samples)(PRNGKey(2), predictions_params['J'], predictions_params['sigma'])
        return predictions

    def get_inference_data(self, data, eight_schools_params, predictions_data, predictions_params):
        posterior_samples = data.obj.get_samples()
        model = data.obj.sampler.model
        posterior_predictive = Predictive(model, posterior_samples)(PRNGKey(1), eight_schools_params['J'], eight_schools_params['sigma'])
        prior = Predictive(model, num_samples=500)(PRNGKey(2), eight_schools_params['J'], eight_schools_params['sigma'])
        predictions = predictions_data
        return from_numpyro(posterior=data.obj, prior=prior, posterior_predictive=posterior_predictive, predictions=predictions, coords={'school': np.arange(eight_schools_params['J']), 'school_pred': np.arange(predictions_params['J'])}, dims={'theta': ['school'], 'eta': ['school'], 'obs': ['school']}, pred_dims={'theta': ['school_pred'], 'eta': ['school_pred'], 'obs': ['school_pred']})
    ...
    def test_inference_data(self, data, eight_schools_params, predictions_data, predictions_params):
        ...
        test_dict = {'posterior': ['mu', 'tau', 'eta'], 'sample_stats': ['diverging'], 'log_likelihood': ['obs'], 'posterior_predictive': ['obs'], 'predictions': ['obs'], 'prior': ['mu', 'tau', 'eta'], 'prior_predictive': ['obs'], 'observed_data': ['obs']}
    ...
    def test_inference_data_no_posterior(self, data, eight_schools_params, predictions_data, predictions_params):
        ...
        idata = from_numpyro(prior=prior, posterior_predictive=posterior_predictive, coords={'school': np.arange(eight_schools_params['J'])}, dims={'theta': ['school'], 'eta': ['school']})
    ...
    def test_inference_data_only_posterior(self, data):
        ...
        test_dict = {'posterior': ['mu', 'tau', 'eta'], 'sample_stats': ['diverging'], 'log_likelihood': ['obs']}
        fails = check_multiple_attrs(test_dict, idata)
        assert not fails
    ...
    def test_inference_data_constant_data(self):
        ...
        inference_data = from_numpyro(mcmc, posterior_predictive=posterior_predictive, predictions=predictions, constant_data={'x1': x1}, predictions_constant_data={'x2': x2})
