...
class L2Gateway(neutron.NeutronResource):
    ...
    PROPERTIES = NAME, DEVICES = ('name', 'devices')
    _DEVICE_KEYS = DEVICE_NAME, INTERFACES = ('device_name', 'interfaces')
    _INTERFACE_KEYS = INTERFACE_NAME, SEGMENTATION_ID = ('name', 'segmentation_id')
    _interface_schema = {INTERFACE_NAME: properties.Schema(properties.Schema.STRING, _('The name of the interface on the gateway device.'), required=True), SEGMENTATION_ID: properties.Schema(properties.Schema.LIST, _('A list of segmentation ids of the interface.'))}
    _device_schema = {DEVICE_NAME: properties.Schema(properties.Schema.STRING, _('The name of the gateway device.'), required=True), INTERFACES: properties.Schema(properties.Schema.LIST, _('List of gateway device interfaces.'), schema=properties.Schema(properties.Schema.MAP, schema=_interface_schema), required=True)}
    properties_schema = {NAME: properties.Schema(properties.Schema.STRING, _('A symbolic name for the l2-gateway, which is not required to be unique.'), required=True, update_allowed=True), DEVICES: properties.Schema(properties.Schema.LIST, _('List of gateway devices.'), schema=properties.Schema(properties.Schema.MAP, schema=_device_schema), required=True, update_allowed=True)}
    ...
    def handle_create(self):
        ...
        l2gw = self.client().create_l2_gateway({'l2_gateway': props})['l2_gateway']
    ...
    def handle_update(self, json_snippet, tmpl_diff, prop_diff):
        if prop_diff:
            ...
            self.client().update_l2_gateway(self.resource_id, {'l2_gateway': prop_diff})