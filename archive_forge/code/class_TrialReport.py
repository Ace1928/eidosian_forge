...
class TrialReport:
    ...
    def __init__(self, trial: Trial, metric: Any, params: Any=None, metadata: Optional[Dict[str, Any]]=None, cost: float=1.0, rung: int=0, sort_metric: Any=None, log_time: Any=None):
    ...
    def copy(self) -> 'TrialReport':
        ...
        .. note::
            ...
            of TrialReport.
        """
        return TrialReport(trial=self.trial, metric=self.metric, params=self._params, metadata=self._metadata, cost=self._cost, rung=self._rung, sort_metric=self._sort_metric, log_time=self.log_time)

    def __repr__(self) -> str:
        return repr(dict(trial=self.trial, metric=self.metric, params=self._params, metadata=self._metadata, cost=self._cost, rung=self._rung, sort_metric=self._sort_metric, log_time=self.log_time))
    ...
    def generate_sort_metric(self, min_better: bool, digits: int) -> 'TrialReport':
        ...
        """
        t = self.copy()
        t._sort_metric = round(self.metric, digits) if min_better else -round(self.metric, digits)
        return t

    def __getstate__(self) -> Dict[str, Any]:
        keys = ['_trial', '_metric', '_params', '_metadata', '_cost', '_rung', '_sort_metric', '_log_time']
