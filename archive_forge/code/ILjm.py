...
Module Structure:
        ...
        indeutils.py - Data handling logic
            - Data Input:
                validate - Validation - Validate The Input Data for Type, Syntax, Expected Values, and Expected Data Types and all requirements, packages, modules, classes, functions, methods, objects, arguments, keyword arguments, logic, sequence, operations, intermediate products and outputs. Maximum possible verbosity for anything decorated and all context etc. captured (providing not captured prior).
                    inspect - Create unique specific generic identifiers for all information and keep all actual types/values/parameters verbatim and all other information (metadata) to store in the metadata cache/file.
                hash - Hashing - Create a unique hash using all of the metadata to verify integrity and uniqueness and ensure no redundancy or duplication.
