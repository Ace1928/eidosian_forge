...
class PlotnineLayoutEngine(LayoutEngine):
    ...
    def execute(self, fig: Figure):
        ...
        from ._plotnine_tight_layout import get_plotnine_tight_layout, set_figure_artist_positions
        pack = self.setup()
        ...
        def get_target(name: str) -> Any:
            ...
            if self.theme.T.is_blank(name):
                ...
                if isinstance(t, Text) and t.get_text() == '':
                    return None
                return t
        return LayoutPack(axs=self.plot.axs, figure=self.plot.figure, renderer=self.plot.figure._get_renderer(), theme=self.theme, facet=self.plot.facet, axis_title_x=get_target('axis_title_x'), axis_title_y=get_target('axis_title_y'), legends=get_target('legends'), plot_caption=get_target('plot_caption'), plot_subtitle=get_target('plot_subtitle'), plot_title=get_target('plot_title'), strip_text_x=get_target('strip_text_x'), strip_text_y=get_target('strip_text_y'))