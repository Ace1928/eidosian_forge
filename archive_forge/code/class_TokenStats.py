...
class TokenStats(ParlaiScript):
    ...
    def setup_args(cls):
        ...
        parser.set_defaults(no_cuda=True, model='test_agents/null', datatype='train:stream:ordered')
        return parser

    def _compute_stats(self, lengths):
        lengths = np.array(lengths)
        return {'exs': len(lengths), 'min': np.min(lengths), 'max': np.max(lengths), 'mean': np.median(lengths), 'p01': np.quantile(lengths, 0.05), 'p05': np.quantile(lengths, 0.05), 'p10': np.quantile(lengths, 0.1), 'p25': np.quantile(lengths, 0.25), 'p50': np.quantile(lengths, 0.5), 'p75': np.quantile(lengths, 0.75), 'p90': np.quantile(lengths, 0.9), 'p95': np.quantile(lengths, 0.95), 'p99': np.quantile(lengths, 0.99), 'p@128': np.mean(lengths <= 128)}
    ...
    def run(self):
        ...
        while not teacher.epoch_done() and cnt < num_examples:
            ...
            if text_vec is not None and (not self.opt['final_only'] or act.get('episode_done')):
                cnt += 1
