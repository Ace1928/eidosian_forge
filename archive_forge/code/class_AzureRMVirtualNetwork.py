...
class AzureRMVirtualNetwork(AzureRMModuleBase):
    ...
    def __init__(self):
        self.module_arg_spec = dict(resource_group=dict(type='str', required=True), name=dict(type='str', required=True), state=dict(type='str', default='present', choices=['present', 'absent']), location=dict(type='str'), address_prefixes_cidr=dict(type='list', elements='str', aliases=['address_prefixes']), dns_servers=dict(type='list', elements='str'), purge_address_prefixes=dict(type='bool', default=False, aliases=['purge']), purge_dns_servers=dict(type='bool', default=False), flow_timeout_in_minutes=dict(type='int'))
        mutually_exclusive = [('dns_servers', 'purge_dns_servers')]
        required_if = [('purge_address_prefixes', True, ['address_prefixes_cidr'])]
        ...
        super(AzureRMVirtualNetwork, self).__init__(self.module_arg_spec, mutually_exclusive=mutually_exclusive, required_if=required_if, supports_check_mode=True)
        ...
        if changed:
            if self.state == 'present':
                if not results:
                    ...
                    vnet_param = self.network_models.VirtualNetwork(location=self.location, flow_timeout_in_minutes=self.flow_timeout_in_minutes, address_space=self.network_models.AddressSpace(address_prefixes=self.address_prefixes_cidr))
                    if self.dns_servers:
                        vnet_param.dhcp_options = self.network_models.DhcpOptions(dns_servers=self.dns_servers)
                    if self.tags:
                    ...
                    vnet_param = self.network_models.VirtualNetwork(location=results['location'], address_space=self.network_models.AddressSpace(address_prefixes=results['address_prefixes']), tags=results['tags'], subnets=vnet.subnets)
                    if results.get('dns_servers'):
                        vnet_param.dhcp_options = self.network_models.DhcpOptions(dns_servers=results['dns_servers'])
