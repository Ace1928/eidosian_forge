...
def gateway_proxy_handler():
    ...
    if not gateway_path:
        raise MlflowException(message='Deployments proxy request must specify a gateway_path.', error_code=INVALID_PARAMETER_VALUE)
    ...
    if response.status_code == 200:
        ...
        raise MlflowException(message=f'Deployments proxy request failed with error code {response.status_code}. Error message: {response.text}', error_code=response.status_code)