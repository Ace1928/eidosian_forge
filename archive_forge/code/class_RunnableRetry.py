...
class RunnableRetry(RunnableBindingBase[Input, Output]):
    ...
    def _invoke(self, input: Input, run_manager: 'CallbackManagerForChainRun', config: RunnableConfig, **kwargs: Any) -> Output:
        for attempt in self._sync_retrying(reraise=True):
            with attempt:
                result = super().invoke(input, self._patch_config(config, run_manager, attempt.retry_state), **kwargs)
    ...
    async def _ainvoke(self, input: Input, run_manager: 'AsyncCallbackManagerForChainRun', config: RunnableConfig, **kwargs: Any) -> Output:
        async for attempt in self._async_retrying(reraise=True):
            with attempt:
                result = await super().ainvoke(input, self._patch_config(config, run_manager, attempt.retry_state), **kwargs)
            if attempt.retry_state.outcome and (not attempt.retry_state.outcome.failed):
                attempt.retry_state.set_result(result)
        return result
    ...
    def _batch(self, inputs: List[Input], run_manager: List['CallbackManagerForChainRun'], config: List[RunnableConfig], **kwargs: Any) -> List[Union[Output, Exception]]:
        ...
        try:
            for attempt in self._sync_retrying():
                with attempt:
                    result = super().batch(pending(inputs), self._patch_config_list(pending(config), pending(run_manager), attempt.retry_state), return_exceptions=True, **kwargs)
    ...
    async def _abatch(self, inputs: List[Input], run_manager: List['AsyncCallbackManagerForChainRun'], config: List[RunnableConfig], **kwargs: Any) -> List[Union[Output, Exception]]:
        results_map: Dict[int, Output] = {}
        ...
        try:
            async for attempt in self._async_retrying():
                with attempt:
                    result = await super().abatch(pending(inputs), self._patch_config_list(pending(config), pending(run_manager), attempt.retry_state), return_exceptions=True, **kwargs)
                    first_exception = None
                    for i, r in enumerate(result):
                        if isinstance(r, Exception):
                            if not first_exception:
