...
class ProjectCiLintManager(GetWithoutIdMixin, CreateMixin, RESTManager):
    ...
    _create_attrs = RequiredOptional(required=('content',), optional=('dry_run', 'include_jobs', 'ref'))

    def get(self, **kwargs: Any) -> ProjectCiLint:
        return cast(ProjectCiLint, super().get(**kwargs))

    @register_custom_action('ProjectCiLintManager', ('content',), optional=('dry_run', 'include_jobs', 'ref'))
