...
class SOSConstraint(ActiveIndexedComponent):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if 'sos' in kwargs and 'level' in kwargs:
            raise TypeError("Specify only one of 'sos' and 'level' -- they are equivalent keyword arguments")
        sosLevel = kwargs.pop('sos', None)
        sosLevel = kwargs.pop('level', sosLevel)
        if sosLevel is None:
            raise TypeError("SOSConstraint() requires that either the 'sos' or 'level' keyword arguments be set to indicate the type of SOS.")
        ...
        if self._rule is None:
            ...
            for index in self._index_set:
                try:
                    ...
                    err = sys.exc_info()[1]
                    logger.error('Rule failed when generating expression for sos constraint %s with index %s:\n%s: %s' % (self.name, str(index), type(err).__name__, err))
                    raise
                if tmp is None:
