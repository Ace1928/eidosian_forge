...
class GoldenQueryAPIWrapper(BaseModel):
    ...
    def run(self, query: str) -> str:
        ...
        response = requests.post(f'{GOLDEN_BASE_URL}/api/v2/public/queries/', json={'prompt': query}, headers=headers, timeout=GOLDEN_TIMEOUT)
        if response.status_code != 201:
            return response.text
        content = json.loads(response.content)
        query_id = content['id']
        response = requests.get(f'{GOLDEN_BASE_URL}/api/v2/public/queries/{query_id}/results/?pageSize=10', headers=headers, timeout=GOLDEN_TIMEOUT)
        return response.text