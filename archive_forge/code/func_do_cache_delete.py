...
@utils.arg('id', metavar='<IMAGE_ID>', nargs='+', help=_('ID of image(s) to delete from cache/queue.'))
def do_cache_delete(gc, args):
    ...
    for args_id in args.id:
        try:
            ...
            msg = _("You are not permitted to delete the image '%s' from cache." % args_id)
            utils.print_err(msg)
        except exc.HTTPException as e:
            msg = _("'%s': Unable to delete image '%s' from cache." % (e, args_id))
            utils.print_err(msg)