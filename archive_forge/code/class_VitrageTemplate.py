...
class VitrageTemplate(resource.Resource):
    ...
    PROPERTIES = TEMPLATE_FILE, TEMPLATE_PARAMS = ('template_file', 'template_params')
    properties_schema = {TEMPLATE_FILE: properties.Schema(properties.Schema.STRING, _('Path of the Vitrage template to use.'), required=True), TEMPLATE_PARAMS: properties.Schema(properties.Schema.MAP, _('Input parameters for the Vitrage template.'), required=True)}
    ...
    def handle_create(self):
        ...
        added_templates = self.client().template.add(template_str=self.properties[self.TEMPLATE_FILE], params=params)
        if added_templates and len(added_templates) > 0:
            if added_templates[0].get('status') == 'LOADING':
                ...
                LOG.debug('Added Vitrage template: %s', str(added_templates[0].get('uuid')))
            else:
                LOG.warning('Failed to add template to Vitrage: %s', added_templates[0].get('status details'))
        else:
    ...
    def validate(self):
        ...
        try:
            ...
            validation = self.client().template.validate(template_str=self.properties[self.TEMPLATE_FILE], params=params)
        except Exception as e:
            msg = _('Exception when calling Vitrage template validate: %s') % e.message
            raise exception.StackValidationFailed(message=msg)
        if not validation or not validation.get('results') or len(validation['results']) != 1 or ('status code' not in validation['results'][0]):
            msg = _('Failed to validate Vitrage template %s') % self.TEMPLATE_FILE
            raise exception.StackValidationFailed(message=msg)
        result = validation['results'][0]
        if result['status code'] != 0:
            msg = _('Failed to validate Vitrage template. Error: %s') % result.get('message')
            raise exception.StackValidationFailed(message=msg)