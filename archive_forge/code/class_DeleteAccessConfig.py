...
class DeleteAccessConfig(base.SilentCommand):
    ...
    def Args(parser):
        ...
        parser.add_argument('--access-config-name', default=constants.DEFAULT_ACCESS_CONFIG_NAME, help="        Specifies the name of the access configuration to delete.\n        ``{0}'' is used as the default if this flag is not provided.\n        ".format(constants.DEFAULT_ACCESS_CONFIG_NAME))
        parser.add_argument('--network-interface', default=constants.DEFAULT_NETWORK_INTERFACE, action=arg_parsers.StoreOnceAction, help="        Specifies the name of the network interface from which to delete the\n        access configuration. If this is not provided, then ``nic0'' is used\n        as the default.\n        ")
    ...
    def Run(self, args):
        ...
        request = client.messages.ComputeInstancesDeleteAccessConfigRequest(accessConfig=args.access_config_name, instance=instance_ref.Name(), networkInterface=args.network_interface, project=instance_ref.project, zone=instance_ref.zone)
        return client.MakeRequests([(client.apitools_client.instances, 'DeleteAccessConfig', request)])