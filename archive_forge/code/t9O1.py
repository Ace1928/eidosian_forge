    ...
    def recursive_read(directory):
        for root, _, files in os.walk(directory):
            for filename in files:
                if filename.endswith((".txt", ".py", ".md")):
                    file_path = os.path.join(root, filename)
                    try:
                        with open(file_path, "r", encoding="utf-8") as file:
                            content = file.read().strip()
                            if content:  # Ensure the file content is not empty
                                texts.append(content)
                    except Exception as e:
                        logging.error(f"Error reading {file_path}: {e}")

    recursive_read(directory_path)
    return texts


def chunk_texts(texts, tokenizer, min_length=4096, max_length=256000):
    chunks = []
    for text in texts:
...
def save_dataset(chunks, file_path):
    ...
    def load_data(self):
        ...
        if directory_path:
            ...
            if not texts:  # Check if the list is empty
                messagebox.showinfo(
                    "Info", "No text files found in the selected directory."
                )
                return  # Exit the function if no texts are found
            chunks = chunk_texts(texts, self.tokenizer)
            save_dataset(chunks, "/home/lloyd/Downloads/processed_dataset.pkl")
            self.dataset = TextDataset(chunks, self.tokenizer)
            self.dataloader = DataLoader(self.dataset, batch_size=8, shuffle=True)
            messagebox.showinfo(
                "Info", f"Loaded and processed {len(texts)} text files."
            )
