...
class CircuitOperation(ops.Operation):
    ...
    def __init__(self, circuit: 'cirq.FrozenCircuit', repetitions: INT_TYPE=1, qubit_map: Optional[Dict['cirq.Qid', 'cirq.Qid']]=None, measurement_key_map: Optional[Dict[str, str]]=None, param_resolver: Optional[study.ParamResolverOrSimilarType]=None, repetition_ids: Optional[Sequence[str]]=None, parent_path: Tuple[str, ...]=(), extern_keys: FrozenSet['cirq.MeasurementKey']=frozenset(), use_repetition_ids: bool=True, repeat_until: Optional['cirq.Condition']=None):
        ...
        if isinstance(self._repetitions, INT_CLASSES):
            ...
            if not self._repetition_ids:
                ...
                raise ValueError(f'Expected repetition_ids to be a list of length {loop_size}, got: {self._repetition_ids}')
        ...
        for mapped_key in self._measurement_key_map.values():
            if value.MEASUREMENT_KEY_SEPARATOR in mapped_key:
                raise ValueError(f'Mapping to invalid key: {mapped_key}. "{value.MEASUREMENT_KEY_SEPARATOR}" is not allowed for measurement keys in a CircuitOperation')
        ...
        if self._repeat_until:
            ...
            if protocols.measurement_key_objs(self._mapped_single_loop()).isdisjoint(self._repeat_until.keys):
    ...
    def replace(self, **changes) -> 'cirq.CircuitOperation':
        ...
        kwargs = {'circuit': self.circuit, 'repetitions': self.repetitions, 'qubit_map': self.qubit_map, 'measurement_key_map': self.measurement_key_map, 'param_resolver': self.param_resolver, 'repetition_ids': self.repetition_ids, 'parent_path': self.parent_path, 'extern_keys': self._extern_keys, 'use_repetition_ids': self.use_repetition_ids, 'repeat_until': self.repeat_until, **changes}
        return CircuitOperation(**kwargs)

    def __eq__(self, other) -> bool:
        if not isinstance(other, type(self)):
            return NotImplemented
        return self.circuit == other.circuit and self.qubit_map == other.qubit_map and (self.measurement_key_map == other.measurement_key_map) and (self.param_resolver == other.param_resolver) and (self.repetitions == other.repetitions) and (self.repetition_ids == other.repetition_ids) and (self.parent_path == other.parent_path) and (self.use_repetition_ids == other.use_repetition_ids) and (self.repeat_until == other.repeat_until)
    ...
    @cached_property
    def _hash(self) -> int:
        return hash((self.circuit, self.repetitions, frozenset(self.qubit_map.items()), frozenset(self.measurement_key_map.items()), self.param_resolver, self.parent_path, () if self.repetition_ids is None else tuple(self.repetition_ids), self.use_repetition_ids))

    def __hash__(self) -> int:
        return self._hash

    def _json_dict_(self):
        resp = {'circuit': self.circuit, 'repetitions': self.repetitions, 'qubit_map': sorted(self.qubit_map.items()), 'measurement_key_map': self.measurement_key_map, 'param_resolver': self.param_resolver, 'repetition_ids': self.repetition_ids, 'parent_path': self.parent_path}
        if not self.use_repetition_ids:
    ...
    @classmethod
    def _from_json_dict_(cls, circuit, repetitions, qubit_map, measurement_key_map, param_resolver, repetition_ids, parent_path=(), use_repetition_ids=True, repeat_until=None, **kwargs):
        return cls(circuit=circuit, repetitions=repetitions, repetition_ids=repetition_ids, use_repetition_ids=use_repetition_ids, repeat_until=repeat_until, param_resolver=param_resolver, qubit_map=dict(qubit_map), measurement_key_map=measurement_key_map, parent_path=tuple(parent_path))

    def repeat(self, repetitions: Optional[IntParam]=None, repetition_ids: Optional[Sequence[str]]=None) -> 'CircuitOperation':
        ...
        if isinstance(repetitions, INT_CLASSES):
            ...
            if repetition_ids is None:
                ...
                raise ValueError(f'Expected repetition_ids={repetition_ids} length to be {expected_repetition_id_length}')
    ...
    def with_measurement_key_mapping(self, key_map: Mapping[str, str]) -> 'cirq.CircuitOperation':
        ...
        if len(protocols.measurement_keys_touched(new_op)) != len(protocols.measurement_keys_touched(self)):
            raise ValueError(f'Collision in measurement key map composition. Original map:\n{self.measurement_key_map}\nApplied changes: {key_map}')
        return new_op
    ...
    def _resolve_parameters_(self, resolver: 'cirq.ParamResolver', recursive: bool) -> 'cirq.CircuitOperation':
        resolved = self.with_params(resolver.param_dict, recursive)
        return resolved.replace(repetitions=resolver.value_of(cast('cirq.TParamVal', self.repetitions), recursive))