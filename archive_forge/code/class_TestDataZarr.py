...
class TestDataZarr:
    ...
    def get_inference_data(self, data, eight_schools_params, fill_attrs):
        return from_dict(posterior=data.obj, posterior_predictive=data.obj, sample_stats=data.obj, prior=data.obj, prior_predictive=data.obj, sample_stats_prior=data.obj, observed_data=eight_schools_params, coords={'school': np.arange(8)}, dims={'theta': ['school'], 'eta': ['school']}, attrs={'test': 1} if fill_attrs else None)
    ...
    def test_io_method(self, data, eight_schools_params, store, fill_attrs):
        inference_data = self.get_inference_data(data, eight_schools_params, fill_attrs)
    ...
    def test_io_function(self, data, eight_schools_params):
        inference_data = self.get_inference_data(data, eight_schools_params, fill_attrs=True)
        test_dict = {'posterior': ['eta', 'theta', 'mu', 'tau'], 'posterior_predictive': ['eta', 'theta', 'mu', 'tau'], 'sample_stats': ['eta', 'theta', 'mu', 'tau'], 'prior': ['eta', 'theta', 'mu', 'tau'], 'prior_predictive': ['eta', 'theta', 'mu', 'tau'], 'sample_stats_prior': ['eta', 'theta', 'mu', 'tau'], 'observed_data': ['J', 'y', 'sigma']}
        fails = check_multiple_attrs(test_dict, inference_data)
        assert not fails
        assert inference_data.attrs['test'] == 1
