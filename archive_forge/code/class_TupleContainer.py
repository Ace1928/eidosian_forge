...
class TupleContainer(IHomogeneousContainer, collections.abc.Sequence):
    ...
    def _insert(self, i, item):
        if item.ctype is self.ctype:
            if item._parent is None:
                ...
                return
            raise ValueError('Invalid assignment to type %s with index %s. A parent container has already been assigned to the object being inserted: %s' % (self.__class__.__name__, i, item.parent.name))
        else:
            raise TypeError('Invalid assignment to type %s with index %s. The object being inserted has the wrong category type: %s' % (self.__class__.__name__, i, item.ctype))
    ...
    def __eq__(self, other):
        if not isinstance(other, (collections.abc.Set, collections.abc.Sequence)):
            return False
        return tuple(((type(val), id(val)) for val in self)) == tuple(((type(val), id(val)) for val in other))
