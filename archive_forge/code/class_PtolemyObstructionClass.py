class PtolemyObstructionClass:
    ...
    def __init__(self, manifold, index, H2_element, explain_basis, identified_face_classes):
        ...
        self.identified_variables = [H2_element_entry_to_identified_variable(entry, variable) for entry, variable in zip(H2_element, explain_basis)] + [(+1, 0, var1, var2) for sign, power, var1, var2 in self._identified_face_classes]

    def _checkManifoldAndN(self, manifold, N):
        if self._manifold is not None:
            assert manifold == self._manifold, 'PtolemyObstructionClass for wrong manifold'
