...
def test_diff_append(n):
    ...
    assert_eq(da.diff(a, n, axis=1, append=[[0], [0], [0], [0]]), np.diff(x, n, axis=1, append=[[0], [0], [0], [0]]))
    assert_eq(da.diff(a, n, axis=0, append=0), np.diff(x, n, axis=0, append=0))
    assert_eq(da.diff(a, n, axis=0, append=[[0, 0, 0, 0]]), np.diff(x, n, axis=0, append=[[0, 0, 0, 0]]))
    if n > 0:
