...
class OpenChat(object):
    ...
    def __init__(self, model, device, maxlen=-1, environment='interactive'):
        draw_openchat()
        self.agent = self.check_agent(model)
        self.agent = self.create_agent_by_name(name=self.agent, device=device, maxlen=maxlen)
    ...
    def check_environment(self, env):
        ...
        assert env in available_envs, f'param `environment` must be one of {available_envs}'
        return env
    ...
    def available_models():
        agents = [BlenderGenerationAgent, DialoGPTAgent, GPTNeoAgent, DodecathlonAgent, RedditAgent, SensitiveAgent, OffensiveAgent, UnlikelihoodAgent, WizardOfWikipediaGenerationAgent]
        available_models = []
        for agent in agents:
