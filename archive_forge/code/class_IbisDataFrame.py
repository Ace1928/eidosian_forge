...
class IbisDataFrame(DataFrame):
    ...
    def _to_local_df(self, table: IbisTable, schema: Any=None) -> LocalBoundedDataFrame:
        raise NotImplementedError

    def _to_iterable_df(self, table: IbisTable, sdhema: Any=None) -> IterableDataFrame:
        raise NotImplementedError
    ...
    def alter_columns(self, columns: Any) -> DataFrame:
        ...
        if new_schema == self.schema:
            return self
        return self._to_new_df(self._alter_table_columns(self._table, new_schema), schema=new_schema)
    ...
    def as_array_iterable(self, columns: Optional[List[str]]=None, type_safe: bool=False) -> Iterable[Any]:
        if columns is not None:
            ...
            yield from self._to_iterable_df(self._table).as_array_iterable(type_safe=type_safe)
