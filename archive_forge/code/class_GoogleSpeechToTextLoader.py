...
@deprecated(since='0.0.32', removal='0.2.0', alternative_import='langchain_google_community.SpeechToTextLoader')
class GoogleSpeechToTextLoader(BaseLoader):
    ...
    def __init__(self, project_id: str, file_path: str, location: str='us-central1', recognizer_id: str='_', config: Optional[RecognitionConfig]=None, config_mask: Optional[FieldMask]=None):
        """
        ...
        try:
            ...
            raise ImportError('Could not import google-cloud-speech python package. Please install it with `pip install google-cloud-speech`.') from exc
        self.project_id = project_id
        self.file_path = file_path
        self.location = location
        self.recognizer_id = recognizer_id
        self.config = config or RecognitionConfig(auto_decoding_config=AutoDetectDecodingConfig(), language_codes=['en-US'], model='chirp', features=RecognitionFeatures(enable_automatic_punctuation=True))
        self.config_mask = config_mask
        self._client = SpeechClient(client_info=get_client_info(module='speech-to-text'), client_options=ClientOptions(api_endpoint=f'{location}-speech.googleapis.com') if location != 'global' else None)
    ...
    def load(self) -> List[Document]:
        ...
        try:
            ...
            raise ImportError('Could not import google-cloud-speech python package. Please install it with `pip install google-cloud-speech`.') from exc
        request = RecognizeRequest(recognizer=self._recognizer_path, config=self.config, config_mask=self.config_mask)
        ...
        return [Document(page_content=result.alternatives[0].transcript, metadata={'language_code': result.language_code, 'result_end_offset': result.result_end_offset}) for result in response.results]