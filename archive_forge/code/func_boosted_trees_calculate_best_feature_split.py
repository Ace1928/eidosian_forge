...
def boosted_trees_calculate_best_feature_split(node_id_range: _atypes.TensorFuzzingAnnotation[_atypes.Int32], stats_summary: _atypes.TensorFuzzingAnnotation[_atypes.Float32], l1: _atypes.TensorFuzzingAnnotation[_atypes.Float32], l2: _atypes.TensorFuzzingAnnotation[_atypes.Float32], tree_complexity: _atypes.TensorFuzzingAnnotation[_atypes.Float32], min_node_weight: _atypes.TensorFuzzingAnnotation[_atypes.Float32], logits_dimension: int, split_type: str='inequality', name=None):
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'BoostedTreesCalculateBestFeatureSplit', name, node_id_range, stats_summary, l1, l2, tree_complexity, min_node_weight, 'logits_dimension', logits_dimension, 'split_type', split_type)
            ...
            pass
        try:
            return boosted_trees_calculate_best_feature_split_eager_fallback(node_id_range, stats_summary, l1, l2, tree_complexity, min_node_weight, logits_dimension=logits_dimension, split_type=split_type, name=name, ctx=_ctx)
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('BoostedTreesCalculateBestFeatureSplit', node_id_range=node_id_range, stats_summary=stats_summary, l1=l1, l2=l2, tree_complexity=tree_complexity, min_node_weight=min_node_weight, logits_dimension=logits_dimension, split_type=split_type, name=name)
    _result = _outputs[:]
    if _execute.must_record_gradient():
        _attrs = ('logits_dimension', _op._get_attr_int('logits_dimension'), 'split_type', _op.get_attr('split_type'))
        _inputs_flat = _op.inputs
        _execute.record_gradient('BoostedTreesCalculateBestFeatureSplit', _inputs_flat, _attrs, _result)
    _result = _BoostedTreesCalculateBestFeatureSplitOutput._make(_result)
    return _result