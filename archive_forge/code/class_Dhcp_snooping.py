...
class Dhcp_snooping(ConfigBase):
    ...
    """
    test_keys = [{'afis': {'afi': ''}}, {'source_bindings': {'mac_addr': ''}}, {'trusted': {'intf_name': ''}}]
    ipv4_key = 'ipv4'
    ipv6_key = 'ipv6'
    delete_method_value = 'delete'
    patch_method_value = 'patch'
    gather_subset = ['!all', '!min']
    gather_network_resources = ['dhcp_snooping']
    ...
    def get_delete_vlans_requests(self, afi):
        ...
        if afi.get('vlans'):
            for vlan_id in afi.get('vlans'):
                requests.append({'path': self.vlans_uri.format(vlan_name='Vlan' + vlan_id, v=self.afi_to_vnum(afi)), 'method': self.delete_method_value})
        return requests
