...
class JobUtilsTestCase(test_base.OsWinBaseTestCase):
    ...
    def test_get_pending_jobs_reraised(self, mock_is_not_found_exc):
        ...
        type(mock_not_found_mapping).AffectingElement = mock.PropertyMock(side_effect=exceptions.x_wmi)
        self.jobutils._conn.Msvm_AffectedJobElement.return_value = [mock_not_found_mapping]
        self.assertRaises(exceptions.x_wmi, self.jobutils._get_pending_jobs_affecting_element, mock.MagicMock())
    ...
    def test_modify_virt_resource(self):
        side_effect = [(self._FAKE_JOB_PATH, mock.MagicMock(), self._FAKE_RET_VAL)]
    ...
    def test_remove_multiple_virt_resources_not_found(self, mock_check_ret):
        ...
        mock_check_ret.side_effect = exceptions.x_wmi('expected error', com_error=mock.Mock(excepinfo=excepinfo))
        ...
        self.assertRaises(exceptions.NotFound, self.jobutils.remove_virt_resource, mock_virt_res)
        vsms_method.assert_called_once_with(ResourceSettings=[mock_virt_res.path_.return_value])
        mock_check_ret.assert_called_once_with(mock.sentinel.ret_val, mock.sentinel.job)