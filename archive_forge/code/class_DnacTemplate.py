...
class DnacTemplate(DnacBase):
    ...
    def get_template(self, config):
        ...
        """
        result = None
        items = self.dnac_apply['exec'](family='configuration_templates', function='get_template_details', params={'template_id': config.get('templateId')})
        if items:
            result = items
    ...
    def get_have(self, config):
        ...
        self.msg = 'Successfully collected all project and template                     parameters from dnac for comparison'
        self.status = 'success'
        return self
    ...
    def requires_update(self):
        ...
        if self.have_template.get('isCommitPending'):
            self.log("Template '{0}' is in saved state and needs to be updated and committed.".format(self.have_template.get('template').get('name')), 'DEBUG')
            return True
    ...
    def get_export_template_values(self, export_values):
        ...
        Returns:
            self
        """
        template_details = self.dnac._exec(family='configuration_templates', function='get_projects_details')
        for values in export_values:
            ...
            all_template_details = get_dict_result(template_details, 'name', project_name)
            self.log('Template details under the project name {0}: {1}'.format(project_name, all_template_details), 'DEBUG')
            all_template_details = all_template_details.get('templates')
            self.log('Template details under the project name {0}: {1}'.format(project_name, all_template_details), 'DEBUG')
            template_name = values.get('template_name')
            template_detail = get_dict_result(all_template_details, 'name', template_name)
            self.log('Template details with template name {0}: {1}'.format(template_name, template_detail), 'DEBUG')
    ...
    def get_diff_deleted(self, config):
        ...
        if configuration_templates:
            ...
            if template_params.get('name'):
                ...
                self.log("Template name is empty, deleting the project '{0}' and associated templates".format(config.get('configuration_templates').get('project_name')), 'INFO')
    ...
    def verify_diff_merged(self, config):
        ...
        if config.get('configuration_templates') is not None:
            ...
            template_params = ['language', 'name', 'projectName', 'softwareType', 'softwareVariant', 'templateContent']
            for item in template_params:
