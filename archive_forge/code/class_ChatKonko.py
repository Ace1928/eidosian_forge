...
class ChatKonko(ChatOpenAI):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        try:
            ...
            raise ValueError('Could not import konko python package. Please install it with `pip install konko`.')
        try:
            if is_openai_v1():
            ...
            raise ValueError('`konko` has no `ChatCompletion` attribute, this is likely due to an old version of the konko package. Try upgrading it with `pip install --upgrade konko`.')
        if not hasattr(konko, '_is_legacy_openai'):
            warnings.warn("You are using an older version of the 'konko' package. Please consider upgrading to access new features.")
        if values['n'] < 1:
    ...
    def _default_params(self) -> Dict[str, Any]:
        ...
        return {'model': self.model, 'max_tokens': self.max_tokens, 'stream': self.streaming, 'n': self.n, 'temperature': self.temperature, **self.model_kwargs}

    @staticmethod
    def get_available_models(konko_api_key: Union[str, SecretStr, None]=None, openai_api_key: Union[str, SecretStr, None]=None, konko_api_base: str=DEFAULT_API_BASE) -> Set[str]:
        ...
        if not konko_api_key:
            try:
                ...
                raise ValueError('Konko API key must be passed as keyword argument or set in environment variable KONKO_API_KEY.')
    ...
    def _get_invocation_params(self, stop: Optional[List[str]]=None, **kwargs: Any) -> Dict[str, Any]:
        ...
        return {'model': self.model, **super()._get_invocation_params(stop=stop), **self._default_params, **kwargs}
