...
class ArtifactManifestEntry:
    ...
    def __init__(self, path: StrPath, digest: Union[B64MD5, URIStr, FilePathStr, ETag], ref: Optional[Union[FilePathStr, URIStr]]=None, birth_artifact_id: Optional[str]=None, size: Optional[int]=None, extra: Optional[Dict]=None, local_path: Optional[StrPath]=None) -> None:
    ...
    def __repr__(self) -> str:
        ...
        birth_artifact_id = f', birth_artifact_id={self.birth_artifact_id!r}' if self.birth_artifact_id is not None else ''
    ...
    def __eq__(self, other: object) -> bool:
        ...
        if not isinstance(other, ArtifactManifestEntry):
            return False
        return self.path == other.path and self.digest == other.digest and (self.ref == other.ref) and (self.birth_artifact_id == other.birth_artifact_id) and (self.size == other.size) and (self.extra == other.extra) and (self.local_path == other.local_path)
    ...
    def download(self, root: Optional[str]=None, skip_cache: Optional[bool]=None) -> FilePathStr:
        ...
        if self.ref is not None:
            cache_path = self._parent_artifact.manifest.storage_policy.load_reference(self, local=True, dest_path=override_cache_path)
        else:
            cache_path = self._parent_artifact.manifest.storage_policy.load_file(self._parent_artifact, self, dest_path=override_cache_path)
        if skip_cache:
            return FilePathStr(dest_path)
        else:
            return FilePathStr(str(filesystem.copy_or_overwrite_changed(cache_path, dest_path)))
