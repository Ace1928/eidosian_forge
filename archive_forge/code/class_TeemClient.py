...
class TeemClient(object):
    ...
    def prepare_request(self):
        ...
        headers = {'F5-ApiKey': TEEM_KEY, 'F5-DigitalAssetId': str(dai), 'F5-TraceId': str(uuid.uuid4()), 'User-Agent': user_agent}
        headers.update(BASE_HEADERS)
        data = {'digitalAssetName': self.coll_name, 'digitalAssetVersion': CURRENT_COLL_VERSION, 'digitalAssetId': str(dai), 'documentType': '{0} Ansible Collection'.format(self.coll_name), 'documentVersion': '1', 'observationStartTime': self.start_time, 'observationEndTime': datetime.now().isoformat(), 'epochTime': time(), 'telemetryId': str(uuid.uuid4()), 'telemetryRecords': telemetry}
    ...
    def send(self):
        ...
        try:
            response = open_url(url=url, method='POST', headers=headers, timeout=TEEM_TIMEOUT, validate_certs=TEEM_VERIFY, data=payload)
        except (HTTPError, URLError, SSLError, RemoteDisconnected):
            return None
        ok = re.search('20[01-4]', str(response.code))
        if ok:
            return True
        return False
