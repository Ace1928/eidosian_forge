...
class ListRoleAssignment(command.Lister):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument('--user', metavar='<user>', help='User to filter (name or ID)')
        parser.add_argument('--project', metavar='<project>', help='Project to filter (name or ID)')
        parser.add_argument('--names', action='store_true', help='Display names instead of IDs')
        parser.add_argument('--auth-user', action='store_true', dest='authuser', help='Only list assignments for the authenticated user')
        parser.add_argument('--auth-project', action='store_true', dest='authproject', help="Only list assignments for the project to which the authenticated user's token is scoped")
        return parser
    ...
    def take_action(self, parsed_args):
        ...
        if parsed_args.user:
            ...
            if auth_ref:
                user = utils.find_resource(identity_client.users, auth_ref.user_id)
        project = None
        if parsed_args.project:
            project = utils.find_resource(identity_client.projects, parsed_args.project)
        elif parsed_args.authproject:
            if auth_ref:
                project = utils.find_resource(identity_client.projects, auth_ref.project_id)
