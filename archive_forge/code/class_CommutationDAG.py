...
class CommutationDAG:
    ...
    **Reference:**
    ...
    def add_node(self, operation):
        ...
        new_node = CommutationDAGNode(op=operation, wires=operation.wires.tolist(), target_wires=target_wires, control_wires=operation.control_wires.tolist(), successors=[], predecessors=[])
    ...
    def draw(self, filename='dag.png'):
        ...
        for node in self.get_nodes():
            ...
            label = 'ID: ' + str(node[0]) + '\n' + 'Op: ' + node[1].op.name + '\n' + 'Wires: [' + wires[1:] + ']'
            draw_graph.add_node(node[0], label=label, color='blue', style='filled', fillcolor='lightblue')
    ...
    def _update_edges(self):
        ...
        for prev_node_id in range(max_node_id - 1, -1, -1):
            if self.get_node(prev_node_id).reachable and (not qml.is_commuting(self.get_node(prev_node_id).op, max_node)):
