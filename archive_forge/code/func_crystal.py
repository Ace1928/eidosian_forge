...
def crystal(symbols=None, basis=None, occupancies=None, spacegroup=1, setting=1, cell=None, cellpar=None, ab_normal=(0, 0, 1), a_direction=None, size=(1, 1, 1), onduplicates='warn', symprec=0.001, pbc=True, primitive_cell=False, **kwargs) -> ase.Atoms:
    ...
    if not isinstance(symbols, str) and hasattr(symbols, '__getitem__') and (len(symbols) > 0) and isinstance(symbols[0], ase.Atom):
    ...
    sites, kinds = sg.equivalent_sites(basis_coords, onduplicates=onduplicates, symprec=symprec)
    masses = None
    if 'masses' in kwargs:
    ...
    kwargs['info'] = info
    atoms = ase.Atoms(symbols, scaled_positions=sites, cell=cell, pbc=pbc, masses=masses, **kwargs)
    if isinstance(basis, ase.Atoms):
        for name in basis.arrays:
            if not atoms.has(name):
                array = basis.get_array(name)
                atoms.new_array(name, [array[i] for i in kinds], dtype=array.dtype, shape=array.shape[1:])
    if kinds:
