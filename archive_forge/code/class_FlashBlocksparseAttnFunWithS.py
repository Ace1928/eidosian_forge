...
class FlashBlocksparseAttnFunWithS(torch.autograd.Function):
    ...
    def forward(ctx, qkv, cu_seqlens, blockmask, dropout_p, max_s, softmax_scale, causal):
        ...
        context, softmax_lse, S_dmask = _flash_blocksparse_attn_forward(qkv, cu_seqlens, blockmask, dropout_p, max_s, softmax_scale, causal=causal, return_softmax=True)
    ...
    def backward(ctx, dout, _dS_dmask_ignored, _dsoftmax_sum_ignored):
        ...
        dqkv = _flash_blocksparse_attn_backward(dout, qkv, context, S_dmask, softmax_lse, cu_seqlens, blockmask, ctx.dropout_p, ctx.max_s, ctx.softmax_scale, ctx.causal)
        if rng_state is not None:
