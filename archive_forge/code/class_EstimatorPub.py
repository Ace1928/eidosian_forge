...
class EstimatorPub(ShapedMixin):
    ...
    def __init__(self, circuit: QuantumCircuit, observables: ObservablesArray, parameter_values: BindingsArray | None=None, precision: float | None=None, validate: bool=True):
        ...
        try:
            ...
            raise ValueError(f'The observables shape {self.observables.shape} and the parameter values shape {self.parameter_values.shape} are not broadcastable.') from ex
        if validate:
            self.validate()
    ...
    def coerce(cls, pub: EstimatorPubLike, precision: float | None=None) -> EstimatorPub:
        ...
        if isinstance(pub, EstimatorPub):
            if pub.precision is None and precision is not None:
                return cls(circuit=pub.circuit, observables=pub.observables, parameter_values=pub.parameter_values, precision=precision, validate=False)
            return pub
        ...
        if len(pub) > 3 and pub[3] is not None:
            precision = pub[3]
        return cls(circuit=circuit, observables=observables, parameter_values=parameter_values, precision=precision, validate=True)
    ...
    def validate(self):
        ...
        for i, observable in np.ndenumerate(self.observables):
            ...
            if self.circuit.num_qubits != num_qubits:
                raise ValueError(f'The number of qubits of the circuit ({self.circuit.num_qubits}) does not match the number of qubits of the {i}-th observable ({num_qubits}).')
        num_parameters = self.parameter_values.num_parameters
        if num_parameters != self.circuit.num_parameters:
            raise ValueError(f'The number of values ({num_parameters}) does not match the number of parameters ({self.circuit.num_parameters}) for the circuit.')