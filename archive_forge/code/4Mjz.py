...
def run() -> None:
    ...
    while True:
        game_over = False
        snake.reset()
        fruit.respawn(self.game_grid.grid)
        ...
        while not game_over:
            ...
            snake.move(action)
            game_grid.update(snake, fruit)

            # Check for collision with walls or snake body
            if (
                not (0 <= next_state.x < game_grid.width)
                or not (0 <= next_state.y < game_grid.height)
                or game_grid.grid[next_state.x][next_state.y] == 1
            ):
                game_over = True
                reward: float = -1.0
