...
class ODESys(object):
    ...
    def __init__(self, f, jac=None, dfdx=None, jtimes=None, first_step_cb=None, roots_cb=None, nroots=None, band=None, names=(), param_names=(), indep_name=None, description=None, dep_by_name=False, par_by_name=False, latex_names=(), latex_param_names=(), latex_indep_name=None, taken_names=None, pre_processors=None, post_processors=None, append_iv=False, autonomous_interface=None, to_arrays_callbacks=None, autonomous_exprs=None, _indep_autonomous_key=None, numpy=None, nnz=-1, **kwargs):
        ...
        elif autonomous_interface is None and self.autonomous_exprs and (len(self.post_processors) == 0) and (len(self.pre_processors) == 0):
    ...
    def to_arrays(self, x, y, p, callbacks=None, reshape=True):
        ...
        arrs = [arr.T if tp else arr for tp, arr in zip([False, tp_y, tp_p], map(self.numpy.atleast_1d, (_x, _y, _p)))]
        if reshape:
            extra_shape = None
            for a in arrs:
                if a.ndim == 1:
    ...
    def adaptive(self, y0, x0, xend, params=(), **kwargs):
        ...
        """
        return self.integrate((x0, xend), y0, params=params, **kwargs)
    ...
    def integrate(self, x, y0, params=(), atol=1e-08, rtol=1e-08, **kwargs):
        ...
        if isinstance(integrator, str):
            ...
            nfo = self._integrate(integrator.integrate_adaptive, integrator.integrate_predefined, *args, **kwargs)
        if twodim:
            _xout = [d['internal_xout'] for d in nfo]
            _yout = [d['internal_yout'] for d in nfo]
            _params = [d['internal_params'] for d in nfo]
            res = [Result(*self.post_process(_xout[i], _yout[i], _params[i]) + (nfo[i], self)) for i in range(len(nfo))]
        else:
    ...
    def _integrate(self, adaptive, predefined, intern_xout, intern_y0, intern_p, atol=1e-08, rtol=1e-08, first_step=0.0, with_jacobian=None, force_predefined=False, **kwargs):
        nx = intern_xout.shape[-1]
        results = []
        for _xout, _y0, _p in zip(intern_xout, intern_y0, intern_p):
            new_kwargs = dict(dx0=first_step, atol=atol, rtol=rtol, check_indexing=False)
    ...
    def _plot(self, cb, internal_xout=None, internal_yout=None, internal_params=None, **kwargs):
        ...
        x, y, p = (_default(internal_xout, _internal[0]), _default(internal_yout, _internal[1]), _default(internal_params, _internal[2]))
        for post_processor in self.post_processors:
            x, y, p = post_processor(x, y, p)
        if 'names' not in kwargs:
            kwargs['names'] = getattr(self, 'names', None)
        elif 'indices' not in kwargs and getattr(self, 'names', None) is not None:
