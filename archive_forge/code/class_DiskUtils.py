...
class DiskUtils(baseutils.BaseUtils):
    ...
    def __init__(self):
        ...
        self._phys_dev_name_regex = re.compile('\\\\\\\\.*\\\\[a-zA-Z]*([\\d]+)')
    ...
    def is_mpio_disk(self, disk_number):
        disk = self._get_disk_by_number(disk_number)
        return disk.Path.lower().startswith('\\\\?\\mpio')
    ...
    def set_new_disk_policy(self, policy):
        ...
        """
        self._conn_storage.MSFT_StorageSetting.Set(NewDiskPolicy=policy)
    ...
    def set_disk_readonly_status(self, disk_number, read_only):
        ...
        if err_code:
            err_msg = _("Failed to set disk '%(disk_number)s' read-only status to '%(read_only)s'. Error code: %(err_code)s.") % dict(disk_number=disk_number, err_code=err_code, read_only=bool(read_only))
            raise exceptions.DiskUpdateError(message=err_msg)