...
class GurobiPersistent(PersistentSolver, GurobiDirect):
    ...
    def cbLazy(self, con):
        ...
        if con.has_ub():
            if not is_fixed(con.upper):
                raise ValueError('Upper bound of constraint {0} is not constant.'.format(con))
        if con.equality:
            self._solver_model.cbLazy(lhs=gurobi_expr, sense=gurobipy.GRB.EQUAL, rhs=value(con.lower))
        elif con.has_lb() and value(con.lower) > -float('inf'):
            self._solver_model.cbLazy(lhs=gurobi_expr, sense=gurobipy.GRB.GREATER_EQUAL, rhs=value(con.lower))
        elif con.has_ub() and value(con.upper) < float('inf'):
            self._solver_model.cbLazy(lhs=gurobi_expr, sense=gurobipy.GRB.LESS_EQUAL, rhs=value(con.upper))
        else:
            raise ValueError('Constraint does not have a lower or an upper bound {0} \n'.format(con))
    ...
    def _add_column(self, var, obj_coef, constraints, coefficients):
        ...
        vtype = self._gurobi_vtype_from_var(var)
        lb, ub = self._gurobi_lb_ub_from_var(var)
        gurobipy_var = self._solver_model.addVar(obj=obj_coef, lb=lb, ub=ub, vtype=vtype, name=varname, column=gurobipy.Column(coeffs=coefficients, constrs=constraints))
