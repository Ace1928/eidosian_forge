...
class ProblemWriter_mps(AbstractProblemWriter):
    ...
    def __call__(self, model, output_filename, solver_capability, io_options):
        ...
        if len(io_options):
            raise ValueError('ProblemWriter_mps passed unrecognized io_options:\n\t' + '\n\t'.join(('%s = %s' % (k, v) for k, v in io_options.items())))
        if symbolic_solver_labels and labeler is not None:
            raise ValueError("ProblemWriter_mps: Using both the 'symbolic_solver_labels' and 'labeler' I/O options is forbidden")
        ...
        with PauseGC() as pgc:
            with open(output_filename, 'w') as output_file:
                symbol_map = self._print_model_MPS(model, output_file, solver_capability, labeler, output_fixed_variable_bounds=output_fixed_variable_bounds, file_determinism=file_determinism, row_order=row_order, column_order=column_order, skip_trivial_constraints=skip_trivial_constraints, force_objective_constant=force_objective_constant, include_all_variable_bounds=include_all_variable_bounds, skip_objective_sense=skip_objective_sense)
        self._referenced_variable_ids.clear()
