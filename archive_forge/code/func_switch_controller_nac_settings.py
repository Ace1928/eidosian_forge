...
def switch_controller_nac_settings(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('switch-controller', 'nac-settings', filtered_data, vdom=vdom)
        current_data = fos.get('switch-controller', 'nac-settings', vdom=vdom, mkey=mkey)
    ...
    if state == 'present' or state is True:
        return fos.set('switch-controller', 'nac-settings', data=filtered_data, vdom=vdom)
    elif state == 'absent':
        return fos.delete('switch-controller', 'nac-settings', mkey=filtered_data['name'], vdom=vdom)
    else:
