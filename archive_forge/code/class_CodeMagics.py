...
class CodeMagics(Magics):
    ...
    def pastebin(self, parameter_s=''):
        ...
        if expiry_days < 1 or expiry_days > 365:
            ...
            return
        post_data = urlencode({'title': opts.get('d', 'Pasted from IPython'), 'syntax': 'python', 'content': code, 'expiry_days': expiry_days}).encode('utf-8')
        request = Request('https://dpaste.com/api/v2/', headers={'User-Agent': 'IPython v{}'.format(version)})
    ...
    def load(self, arg_s):
        ...
        if l > 200000 and 'y' not in opts:
            try:
                ans = self.shell.ask_yes_no("The text you're trying to load seems pretty big (%d characters). Continue (y/[N]) ?" % l, default='n')
            except StdinNotImplementedError:
                ans = True
            if ans is False:
                print('Operation cancelled.')
