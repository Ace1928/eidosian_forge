...
class TestCors(testcase.GsUtilIntegrationTestCase):
    ...
    empty_doc1 = '[]'
    empty_doc2 = '[ {} ]'
    cors_bad = '[{"origin": ["http://origin1.example.com", "http://origin2.example.com"], "responseHeader": ["foo", "bar"], "badmethod": ["GET", "PUT", "POST"], "maxAgeSeconds": 3600},{"origin": ["http://origin3.example.com"], "responseHeader": ["foo2", "bar2"], "method": ["GET", "DELETE"]}])'
    xml_cors_doc = parseString('<CorsConfig><Cors><Origins><Origin>http://origin1.example.com</Origin><Origin>http://origin2.example.com</Origin></Origins><Methods><Method>GET</Method><Method>PUT</Method><Method>POST</Method></Methods><ResponseHeaders><ResponseHeader>foo</ResponseHeader><ResponseHeader>bar</ResponseHeader></ResponseHeaders><MaxAgeSec>3600</MaxAgeSec></Cors><Cors><Origins><Origin>http://origin3.example.com</Origin></Origins><Methods><Method>GET</Method><Method>DELETE</Method></Methods><ResponseHeaders><ResponseHeader>foo2</ResponseHeader><ResponseHeader>bar2</ResponseHeader></ResponseHeaders></Cors></CorsConfig>').toprettyxml(indent='    ')
    cors_doc = '[{"origin": ["http://origin1.example.com", "http://origin2.example.com"], "responseHeader": ["foo", "bar"], "method": ["GET", "PUT", "POST"], "maxAgeSeconds": 3600},{"origin": ["http://origin3.example.com"], "responseHeader": ["foo2", "bar2"], "method": ["GET", "DELETE"]}]\n'
    cors_json_obj = json.loads(cors_doc)
    cors_doc_not_nested_in_list = '{"origin": ["http://origin.example.com", "http://origin2.example.com"], "responseHeader": ["foo", "bar"], "method": ["GET", "PUT", "POST"], "maxAgeSeconds": 3600}'
    cors_doc2 = '[{"origin": ["http://origin1.example.com", "http://origin2.example.com"], "responseHeader": ["foo", "bar"], "method": ["GET", "PUT", "POST"]}]\n'
    cors_json_obj2 = json.loads(cors_doc2)

    def setUp(self):
        super(TestCors, self).setUp()
        self.no_cors = '[]' if self._use_gcloud_storage else 'has no CORS configuration'
    ...
    def testTooFewArgumentsFails(self):
        ...
        stderr = self.RunGsUtil(self._get_cmd_prefix, return_stderr=True, expected_status=1)
        self.assertIn('command requires at least', stderr)
        stderr = self.RunGsUtil(self._set_cmd_prefix, return_stderr=True, expected_status=1)
        self.assertIn('command requires at least', stderr)
