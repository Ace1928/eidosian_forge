...
class UniqueConstraint(BaseConstraint):
    ...
    def __init__(self, *expressions, fields=(), name=None, condition=None, deferrable=None, include=None, opclasses=(), nulls_distinct=None, violation_error_code=None, violation_error_message=None):
        if not name:
    ...
    def constraint_sql(self, model, schema_editor):
        ...
        return schema_editor._unique_sql(model, fields, self.name, condition=condition, deferrable=self.deferrable, include=include, opclasses=self.opclasses, expressions=expressions, nulls_distinct=self.nulls_distinct)
    ...
    def create_sql(self, model, schema_editor):
        ...
        condition = self._get_condition_sql(model, schema_editor)
        expressions = self._get_index_expressions(model, schema_editor)
        return schema_editor._create_unique_sql(model, fields, self.name, condition=condition, deferrable=self.deferrable, include=include, opclasses=self.opclasses, expressions=expressions, nulls_distinct=self.nulls_distinct)

    def remove_sql(self, model, schema_editor):
        condition = self._get_condition_sql(model, schema_editor)
        include = [model._meta.get_field(field_name).column for field_name in self.include]
        expressions = self._get_index_expressions(model, schema_editor)
        return schema_editor._delete_unique_sql(model, self.name, condition=condition, deferrable=self.deferrable, include=include, opclasses=self.opclasses, expressions=expressions, nulls_distinct=self.nulls_distinct)

    def __repr__(self):
        return '<%s:%s%s%s%s%s%s%s%s%s%s>' % (self.__class__.__qualname__, '' if not self.fields else ' fields=%s' % repr(self.fields), '' if not self.expressions else ' expressions=%s' % repr(self.expressions), ' name=%s' % repr(self.name), '' if self.condition is None else ' condition=%s' % self.condition, '' if self.deferrable is None else ' deferrable=%r' % self.deferrable, '' if not self.include else ' include=%s' % repr(self.include), '' if not self.opclasses else ' opclasses=%s' % repr(self.opclasses), '' if self.nulls_distinct is None else ' nulls_distinct=%r' % self.nulls_distinct, '' if self.violation_error_code is None else ' violation_error_code=%r' % self.violation_error_code, '' if self.violation_error_message is None or self.violation_error_message == self.default_violation_error_message else ' violation_error_message=%r' % self.violation_error_message)

    def __eq__(self, other):
        if isinstance(other, UniqueConstraint):
            return self.name == other.name and self.fields == other.fields and (self.condition == other.condition) and (self.deferrable == other.deferrable) and (self.include == other.include) and (self.opclasses == other.opclasses) and (self.expressions == other.expressions) and (self.nulls_distinct is other.nulls_distinct) and (self.violation_error_code == other.violation_error_code) and (self.violation_error_message == other.violation_error_message)
    ...
    def validate(self, model, instance, exclude=None, using=DEFAULT_DB_ALIAS):
        ...
        if self.fields:
            ...
            for field_name in self.fields:
                ...
                if self.nulls_distinct is not False and lookup_value is None or (lookup_value == '' and connections[using].features.interprets_empty_strings_as_nulls):
                    return
