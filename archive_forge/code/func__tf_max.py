...
def _tf_max(*args, **kwargs):
    ...
    if len(args) == 1:
        ...
        raise ValueError('max(arg) currently support only tensor with rank 1, but got a tensor with rank {}'.format(rank))
    for arg in args:
        rank = arg.shape.rank
        if rank != 0:
            raise ValueError('max(arg1, arg2, *args) currently support only scalar tensor, but got a tensor with shape {}'.format(rank))
    return math_ops.reduce_max(args, axis=0)