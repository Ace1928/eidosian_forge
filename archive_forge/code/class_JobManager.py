...
class JobManager:
    ...
    def _get_actor_for_job(self, job_id: str) -> Optional[ActorHandle]:
        try:
            return ray.get_actor(JOB_ACTOR_NAME_TEMPLATE.format(job_id=job_id), namespace=SUPERVISOR_ACTOR_RAY_NAMESPACE)
        except ValueError:
            return None

    async def _monitor_job(self, job_id: str, job_supervisor: Optional[ActorHandle]=None):
    ...
    async def _monitor_job_internal(self, job_id: str, job_supervisor: Optional[ActorHandle]=None):
        timeout = float(os.environ.get(RAY_JOB_START_TIMEOUT_SECONDS_ENV_VAR, DEFAULT_JOB_START_TIMEOUT_SECONDS))
        is_alive = True
        while is_alive:
            try:
                ...
                if job_status == JobStatus.PENDING:
                    ...
                    if time.time() - job_info.start_time / 1000 > timeout:
                        err_msg = f'Job supervisor actor failed to start within {timeout} seconds. This timeout can be configured by setting the environment variable {RAY_JOB_START_TIMEOUT_SECONDS_ENV_VAR}.'
                        resources_specified = job_info.entrypoint_num_cpus is not None and job_info.entrypoint_num_cpus > 0 or (job_info.entrypoint_num_gpus is not None and job_info.entrypoint_num_gpus > 0) or (job_info.entrypoint_memory is not None and job_info.entrypoint_memory > 0) or (job_info.entrypoint_resources is not None and len(job_info.entrypoint_resources) > 0)
                        if resources_specified:
                            err_msg += " This may be because the job entrypoint's specified resources (entrypoint_num_cpus, entrypoint_num_gpus, entrypoint_resources, entrypoint_memory)aren't available on the cluster. Try checking the cluster's available resources with `ray status` and specifying fewer resources for the job entrypoint."
                        await self._job_info_client.put_status(job_id, JobStatus.FAILED, message=err_msg)
                        is_alive = False
                ...
                if job_supervisor is None:
                    if job_status == JobStatus.PENDING:
                        ...
                        await self._job_info_client.put_status(job_id, JobStatus.FAILED, message='Unexpected error occurred: failed to get job supervisor.')
                        is_alive = False
                        continue
                ...
                if job_status == JobStatus.FAILED:
                    job_error_message = 'See more details from the dashboard `Job` page or the state API `ray list jobs`.'
                job_error_message = ''
                if job_status.is_terminal():
                    ...
                    await self._job_info_client.put_status(job_id, job_status, message=job_error_message)
                elif isinstance(e, ActorUnschedulableError):
                    logger.info(f'Failed to schedule job {job_id} because the supervisor actor could not be scheduled: {e}')
                    job_error_message = f'Job supervisor actor could not be scheduled: {e}'
                    await self._job_info_client.put_status(job_id, JobStatus.FAILED, message=job_error_message)
                else:
                    logger.warning(f'Job supervisor for job {job_id} failed unexpectedly: {e}.')
    ...
    async def _get_scheduling_strategy(self, resources_specified: bool) -> SchedulingStrategyT:
        ...
        if os.environ.get(RAY_JOB_ALLOW_DRIVER_ON_WORKER_NODES_ENV_VAR, '0') == '1':
            logger.info(f"{RAY_JOB_ALLOW_DRIVER_ON_WORKER_NODES_ENV_VAR} was set to 1. Using Ray's default actor scheduling strategy for the job driver instead of running it on the head node.")
            return 'DEFAULT'
        head_node_id_bytes = await self._gcs_aio_client.internal_kv_get('head_node_id'.encode(), namespace=ray_constants.KV_NAMESPACE_JOB, timeout=30)
        if head_node_id_bytes is None:
            logger.info("Head node ID not found in GCS. Using Ray's default actor scheduling strategy for the job driver instead of running it on the head node.")
            scheduling_strategy = 'DEFAULT'
        else:
            head_node_id = head_node_id_bytes.decode()
            logger.info(f'Head node ID found in GCS; scheduling job driver on head node {head_node_id}')
            scheduling_strategy = NodeAffinitySchedulingStrategy(node_id=head_node_id, soft=False)
        return scheduling_strategy

    async def submit_job(self, *, entrypoint: str, submission_id: Optional[str]=None, runtime_env: Optional[Dict[str, Any]]=None, metadata: Optional[Dict[str, str]]=None, entrypoint_num_cpus: Optional[Union[int, float]]=None, entrypoint_num_gpus: Optional[Union[int, float]]=None, entrypoint_memory: Optional[int]=None, entrypoint_resources: Optional[Dict[str, float]]=None, _start_signal_actor: Optional[ActorHandle]=None) -> str:
        """
        ...
        job_info = JobInfo(entrypoint=entrypoint, status=JobStatus.PENDING, start_time=int(time.time() * 1000), metadata=metadata, runtime_env=runtime_env, entrypoint_num_cpus=entrypoint_num_cpus, entrypoint_num_gpus=entrypoint_num_gpus, entrypoint_memory=entrypoint_memory, entrypoint_resources=entrypoint_resources)
        new_key_added = await self._job_info_client.put_info(submission_id, job_info, overwrite=False)
        if not new_key_added:
            raise ValueError(f'Job with submission_id {submission_id} already exists. Please use a different submission_id.')
        try:
            resources_specified = any([entrypoint_num_cpus is not None and entrypoint_num_cpus > 0, entrypoint_num_gpus is not None and entrypoint_num_gpus > 0, entrypoint_memory is not None and entrypoint_memory > 0, entrypoint_resources not in [None, {}]])
            scheduling_strategy = await self._get_scheduling_strategy(resources_specified)
            if self.event_logger:
                self.event_logger.info(f'Started a ray job {submission_id}.', submission_id=submission_id)
            supervisor = self._supervisor_actor_cls.options(lifetime='detached', name=JOB_ACTOR_NAME_TEMPLATE.format(job_id=submission_id), num_cpus=entrypoint_num_cpus, num_gpus=entrypoint_num_gpus, memory=entrypoint_memory, resources=entrypoint_resources, scheduling_strategy=scheduling_strategy, runtime_env=self._get_supervisor_runtime_env(runtime_env, resources_specified), namespace=SUPERVISOR_ACTOR_RAY_NAMESPACE).remote(submission_id, entrypoint, metadata or {}, self._gcs_address)
            supervisor.run.remote(_start_signal_actor=_start_signal_actor, resources_specified=resources_specified)
            run_background_task(self._monitor_job(submission_id, job_supervisor=supervisor))
        except Exception as e:
            logger.warning(f"Failed to start supervisor actor for job {submission_id}: '{e}'")
            await self._job_info_client.put_status(submission_id, JobStatus.FAILED, message=f"Failed to start supervisor actor {submission_id}: '{e}'")
    ...
    async def delete_job(self, job_id):
        ...
        if job_status is None or not job_status.is_terminal():
            raise RuntimeError(f"Attempted to delete job '{job_id}', but it is in a non-terminal state {job_status}.")
