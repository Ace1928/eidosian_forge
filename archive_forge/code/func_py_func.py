...
def py_func(input, token: str, Tout, name=None):
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'PyFunc', name, input, 'token', token, 'Tout', Tout)
            return _result
            ...
            pass
        try:
            return py_func_eager_fallback(input, token=token, Tout=Tout, name=name, ctx=_ctx)
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('PyFunc', input=input, token=token, Tout=Tout, name=name)
    _result = _outputs[:]
    if not _result:
        return _op
