...
class WebGLRenderer(RenderableWidget):
    ...
    def __init__(self, antialias=False, alpha=False, webgl_version=2, **kwargs):
        super(WebGLRenderer, self).__init__(_antialias=antialias, _alpha=alpha, _webgl_version=webgl_version, **kwargs)
        link((self, 'width'), (self, '_width'))
        link((self, 'height'), (self, '_height'))

    def render(self, scene, camera):
        content = {'type': 'render', 'scene': to_json(scene, None), 'camera': to_json(camera, None)}
        self.send(content)
