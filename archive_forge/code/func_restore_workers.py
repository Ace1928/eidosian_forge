...
def restore_workers(self, workers: WorkerSet) -> None:
    """Try syncing previously failed and restarted workers with local, if necessary.
        ...
        Args:
            workers: The WorkerSet to restore. This may be Rollout or Evaluation
                workers.
        """
    if not workers or (not workers.local_worker() and (not self.workers.local_worker())):
        return
    ...
    if restored:
        ...
        workers.foreach_worker(func=lambda w: w.set_state(ray.get(state_ref)), remote_worker_ids=restored, local_worker=False, timeout_seconds=self.config.worker_restore_timeout_s, mark_healthy=True)
        self.callbacks.on_workers_recreated(algorithm=self, worker_set=workers, worker_ids=restored, is_evaluation=workers.local_worker().config.in_evaluation)