...
class notebook_extension(extension):
    ...
    def __call__(self, *args, **params):
        ...
        if 'html' not in p.display_formats and len(p.display_formats) > 1:
            msg = 'Output magic unable to control displayed format as IPython notebook uses fixed precedence between %r' % p.display_formats
        ...
        if hasattr(ip, 'kernel') and (not loaded):
            Renderer.comm_manager.get_client_comm(notebook_extension._process_comm_msg, 'hv-extension-comm')
        if not same_cell_execution and p.logo:
            self.load_logo(logo=p.logo, bokeh_logo=p.logo and 'bokeh' in resources, mpl_logo=p.logo and ('matplotlib' in resources or resources == ['holoviews']), plotly_logo=p.logo and 'plotly' in resources)
