...
class NetAppOntapVolumeEfficiency(object):
    ...
    def __init__(self):
        ...
        self.module = AnsibleModule(argument_spec=self.argument_spec, supports_check_mode=True, required_if=[('start_ve_scan_all', True, ['start_ve_scan_old_data'])], required_one_of=[('path', 'volume_name')], mutually_exclusive=[('policy', 'schedule'), ('path', 'volume_name')])
        ...
        partially_supported_rest_properties = [['policy', (9, 7)], ['storage_efficiency_mode', (9, 10, 1)], ['path', (9, 9, 1)], ['volume_efficiency', (9, 11, 1)], ['start_ve_scan_old_data', (9, 11, 1)]]
        unsupported_rest_properties = ['schedule', 'start_ve_scan_all', 'start_ve_build_metadata', 'start_ve_delete_checkpoint', 'start_ve_queue_operation', 'start_ve_qos_policy', 'stop_ve_all_operations']
    ...
    def disable_volume_efficiency(self):
        ...
        try:
            ...
            self.module.fail_json(msg='Error disabling storage efficiency for path %s: %s' % (self.parameters['path'], to_native(error)), exception=traceback.format_exc())
