...
class Albrecht(ResonantRaman):
    ...
    def electronic_me_Qcc(self, omega, gamma):
        ...
        if approx == 'albrecht a' or approx == 'albrecht':
            ...
            with np.errstate(divide='ignore'):
                Vel_Qcc *= np.where(self.vib01_Q > 0, 1.0 / self.vib01_Q, 0)[:, None, None]
    ...
    def summary(self, omega=0, gamma=0, method='standard', direction='central', log=sys.stdout):
        ...
        for n, e in enumerate(om_v):
            if e.imag != 0:
                ...
                c = ' '
                e = e.real
            parprint('%3d %6.1f   %7.1f%s  %9.1f' % (n, 1000 * e, e / u.invcm, c, intensities[n]), file=log)
        parprint('-------------------------------------', file=log)
