...
class TestMultiCinderStore(base.MultiStoreBaseTest, test_cap.TestStoreCapabilitiesChecking, test_cinder_base.TestCinderStoreBase):
    ...
    def setUp(self):
        ...
        fake_sc = [{'endpoints': [{'publicURL': 'http://foo/public_url'}], 'endpoints_links': [], 'name': 'cinder', 'type': 'volumev3'}]
        self.context = mock.MagicMock(service_catalog=fake_sc, user_id='fake_user', auth_token='fake_token', project_id='fake_project')
        self.hash_algo = 'sha256'
        self.fake_admin_context = mock.MagicMock()
        self.fake_admin_context.elevated.return_value = mock.MagicMock(service_catalog=fake_sc, user_id='admin_user', auth_token='admin_token', project_id='admin_project')
        cinder._reset_cinder_session()
        self.config(cinder_mount_point_base=None, group='cinder1')
        self.volume_id = str(uuid.uuid4())
        specs = {'scheme': 'cinder', 'volume_id': self.volume_id}
        self.location = cinder.StoreLocation(specs, self.conf, backend_group='cinder1')
    ...
    def test_get_cinderclient_with_user_overriden_and_region(self):
        self._test_get_cinderclient_with_user_overriden_and_region(group='cinder1')
    ...
    def test_open_cinder_volume_multipath_disabled(self):
        ...
        self._test_open_cinder_volume('wb', 'rw', None, multipath_supported=False)
    ...
    def test_configure_add_invalid_type(self):
        self.config(cinder_volume_type='some_random_type', group=self.store.backend_group)
    ...
    def test_is_image_associated_with_store(self):
        with mock.patch.object(self.store, 'get_cinderclient') as mocked_cc:
            ...
            mocked_cc.return_value = mock.MagicMock(volumes=mock.MagicMock(get=lambda volume_id: mock.MagicMock(volume_type='some_type')), volume_types=mock.MagicMock(default=lambda: mock_default))
            self.config(cinder_volume_type='some_type', group=self.store.backend_group)
            fake_vol_id = str(uuid.uuid4())
            type_match = self.store.is_image_associated_with_store(self.context, fake_vol_id)
            self.assertTrue(type_match)
            self.config(cinder_volume_type=None, group=self.store.backend_group)
            type_match = self.store.is_image_associated_with_store(self.context, fake_vol_id)
            self.assertTrue(type_match)
            mocked_cc.return_value.volume_types = mock.MagicMock(default=lambda: {'name': 'random_type'})
            type_match = self.store.is_image_associated_with_store(self.context, fake_vol_id)
    ...
    def test_cinder_add_with_verifier(self):
        ...
        self._test_cinder_add(fake_volume, volume_file, 1, verifier, is_multi_store=True)
    ...
    def test_cinder_add_volume_full(self):
        ...
        with mock.patch.object(volume_file, 'write', side_effect=e):
            self.assertRaises(exceptions.StorageFull, self._test_cinder_add, fake_volume, volume_file, is_multi_store=True)
    ...
    def test_get_cinderclient_cinder_endpoint_template(self):
        self._test_get_cinderclient_cinder_endpoint_template(group='cinder1')