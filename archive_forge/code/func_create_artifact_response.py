...
def create_artifact_response(self, request, artifact, bindings=None, status=None, sign=None, issuer=None, sign_alg=None, digest_alg=None):
    """
        Create an ArtifactResponse
        :return:
        """
    rinfo = self.response_args(request, bindings)
    response = self._status_response(ArtifactResponse, issuer, status, sign=sign, sign_alg=sign_alg, digest_alg=digest_alg, **rinfo)
