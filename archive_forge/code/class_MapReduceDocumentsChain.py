...
class MapReduceDocumentsChain(BaseCombineDocumentsChain):
    ...
    def get_output_schema(self, config: Optional[RunnableConfig]=None) -> Type[BaseModel]:
        if self.return_intermediate_steps:
            return create_model('MapReduceDocumentsOutput', **{self.output_key: (str, None), 'intermediate_steps': (List[str], None)})
    ...
    def combine_docs(self, docs: List[Document], token_max: Optional[int]=None, callbacks: Callbacks=None, **kwargs: Any) -> Tuple[str, dict]:
        ...
        """
        map_results = self.llm_chain.apply([{self.document_variable_name: d.page_content, **kwargs} for d in docs], callbacks=callbacks)
        question_result_key = self.llm_chain.output_key
        result_docs = [Document(page_content=r[question_result_key], metadata=docs[i].metadata) for i, r in enumerate(map_results)]
    ...
    async def acombine_docs(self, docs: List[Document], token_max: Optional[int]=None, callbacks: Callbacks=None, **kwargs: Any) -> Tuple[str, dict]:
        ...
        This reducing can be done recursively if needed (if there are many documents).
        """
        map_results = await self.llm_chain.aapply([{**{self.document_variable_name: d.page_content}, **kwargs} for d in docs], callbacks=callbacks)
        question_result_key = self.llm_chain.output_key
        result_docs = [Document(page_content=r[question_result_key], metadata=docs[i].metadata) for i, r in enumerate(map_results)]
