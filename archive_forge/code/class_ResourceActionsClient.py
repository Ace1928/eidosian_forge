...
class ResourceActionsClient(ClientEntityBase):
    ...
    def get_by_id(self, id: int) -> BoundAction:
        ...
        """
        response = self._client.request(url=f'{self._resource}/actions/{id}', method='GET')
    ...
    def get_list(self, status: list[str] | None=None, sort: list[str] | None=None, page: int | None=None, per_page: int | None=None) -> ActionsPageResult:
        ...
        response = self._client.request(url=f'{self._resource}/actions', method='GET', params=params)
