...
def ValidatePublicPtrFlags(args):
    ...
    if public_ptr is True:
        if network_interface is not None and network_interface != constants.DEFAULT_NETWORK_INTERFACE:
            raise compute_exceptions.ArgumentError("Public PTR can only be enabled for default network interface '{0}' rather than '{1}'.".format(constants.DEFAULT_NETWORK_INTERFACE, network_interface))
    if args.public_ptr_domain is not None and args.no_public_ptr is True:
        raise exceptions.ConflictingArgumentsException('--public-ptr-domain', '--no-public-ptr')