...
class AI21LabsProvider(BaseProvider):
    ...
    async def completions(self, payload: completions.RequestPayload) -> completions.ResponsePayload:
        ...
        key_mapping = {'stop': 'stopSequences', 'n': 'numResults', 'max_tokens': 'maxTokens'}
        ...
        resp = await send_request(headers=self.headers, base_url=self.base_url, path='complete', payload=payload)
        return completions.ResponsePayload(created=int(time.time()), object='text_completion', model=self.config.model.name, choices=[completions.Choice(index=idx, text=c['data']['text'], finish_reason=c['finishReason']['reason']) for idx, c in enumerate(resp['completions'])], usage=completions.CompletionsUsage(prompt_tokens=None, completion_tokens=None, total_tokens=None))