...
class LLMLinguaCompressor(BaseDocumentCompressor):
    ...
    """
    _pattern_beginning = re.compile('\\A(?:<#)?(?:ref)?(\\d+)(?:#>?)?')
    _pattern_ending = re.compile('(?:<#)?(?:ref)?(\\d+)(?:#>?)?\\Z')
    ...
    additional_compress_kwargs: dict = {'condition_compare': True, 'condition_in_question': 'after', 'context_budget': '+100', 'reorder_context': 'sort', 'dynamic_context_compression_ratio': 0.4}
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        if not values.get('lingua'):
            values['lingua'] = PromptCompressor(model_name=values.get('model_name', {}), device_map=values.get('device_map', {}), model_config=values.get('model_config', {}), open_api_config=values.get('open_api_config', {}))
        return values
    ...
    def compress_documents(self, documents: Sequence[Document], query: str, callbacks: Optional[Callbacks]=None) -> Sequence[Document]:
        ...
        if len(documents) == 0:
            return []
        compressed_prompt = self.lingua.compress_prompt(context=self._format_context(documents), instruction=self.instruction, question=query, target_token=self.target_token, rank_method=self.rank_method, concate_question=False, add_instruction=True, **self.additional_compress_kwargs)
