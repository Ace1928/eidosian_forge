...
class Qwen2MoeFlashAttention2(Qwen2MoeAttention):
    ...
    def forward(self, hidden_states: torch.Tensor, attention_mask: Optional[torch.Tensor]=None, position_ids: Optional[torch.LongTensor]=None, past_key_value: Optional[Cache]=None, output_attentions: bool=False, use_cache: bool=False, **kwargs):
        ...
        use_sliding_windows = _flash_supports_window_size and getattr(self.config, 'sliding_window', None) is not None and (kv_seq_len > self.config.sliding_window) and self.config.use_sliding_window
