...
class ProfilerMiddleware:
    ...
    def __init__(self, app: WSGIApplication, stream: t.IO[str] | None=sys.stdout, sort_by: t.Iterable[str]=('time', 'calls'), restrictions: t.Iterable[str | int | float]=(), profile_dir: str | None=None, filename_format: str='{method}.{path}.{elapsed:.0f}ms.{time:.0f}.prof') -> None:
        self._app = app
        self._stream = stream
    ...
    def __call__(self, environ: WSGIEnvironment, start_response: StartResponse) -> t.Iterable[bytes]:
        ...
        if self._profile_dir is not None:
            if callable(self._filename_format):
                environ['werkzeug.profiler'] = {'elapsed': elapsed * 1000.0, 'time': time.time()}
                filename = self._filename_format(environ)
            else:
                filename = self._filename_format.format(method=environ['REQUEST_METHOD'], path=environ['PATH_INFO'].strip('/').replace('/', '.') or 'root', elapsed=elapsed * 1000.0, time=time.time())
