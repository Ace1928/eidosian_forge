...
class PersistentSolver(DirectOrPersistentSolver):
    ...
    def add_column(self, model, var, obj_coef, constraints, coefficients):
        ...
        if id(self._pyomo_model) != id(model):
            raise RuntimeError('The pyomo model which the column is being added to must be the same as the pyomo model attached to this PersistentSolver instance; i.e., the same pyomo model used in set_instance.')
        if id(self._pyomo_model) != id(var.model()):
            raise RuntimeError('The pyomo var must be attached to the solver model')
        if var in self._pyomo_var_to_solver_var_map:
            raise RuntimeError('The pyomo var must not have been already added to the solver model')
        if len(constraints) != len(coefficients):
            raise RuntimeError('The list of constraints and the list of coefficients be of equal length')
        obj_coef, constraints, coefficients = self._add_and_collect_column_data(var, obj_coef, constraints, coefficients)
    ...
    def remove_var(self, var):
        ...
        if self._referenced_variables[var] != 0:
            raise ValueError('Cannot remove Var {0} because it is still referenced by the '.format(var) + 'objective or one or more constraints')
    ...
    def solve(self, *args, **kwds):
        ...
        if isinstance(self._pyomo_model, _BlockData):
            ...
            model_suffixes = list((comp.storage_key for comp in import_suffix_generator(self._pyomo_model, active=True, descend_into=False)))
