...
class FTPArtifactRepository(ArtifactRepository):
    ...
    def __init__(self, artifact_uri):
        ...
        self.config = {'host': parsed.hostname, 'port': 21 if parsed.port is None else parsed.port, 'username': parsed.username, 'password': parsed.password}
    ...
    def _download_file(self, remote_file_path, local_path):
        remote_full_path = posixpath.join(self.path, remote_file_path) if remote_file_path else self.path
        with self.get_ftp_client() as ftp:
