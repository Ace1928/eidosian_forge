...
class LegacyPlugin(PluginV3):
    ...
    def __init__(self, request, legacy_plugin):
        ...
        source = '<bytes>' if isinstance(self._request.raw_uri, bytes) else self._request.raw_uri
        if self._request.mode.io_mode == IOMode.read:
            if not self._format.can_read(request):
                raise InitializationError(f'`{self._format.name}` can not read `{source}`.')
        elif not self._format.can_write(request):
            raise InitializationError(f'`{self._format.name}` can not write to `{source}`.')
    ...
    def write(self, ndimage, *, is_batch=None, metadata=None, **kwargs):
        ...
        with self.legacy_get_writer(**kwargs) as writer:
            for image in ndimage:
                ...
                if not np.issubdtype(image.dtype, np.number) and (not np.issubdtype(image.dtype, bool)):
                    raise ValueError(f'All images have to be numeric, and not `{image.dtype}`.')
    ...
    def properties(self, index=None):
        ...
        if index is Ellipsis:
            ...
            return ImageProperties(shape=(n_images, *image.shape), dtype=image.dtype, n_images=n_images, is_batch=True)
        image = self.read(index=index)
        return ImageProperties(shape=image.shape, dtype=image.dtype, is_batch=False)
