...
class ProgrammableRotationGateArrayBase(infra.GateWithRegisters):
    ...
    def signature(self) -> infra.Signature:
        return infra.Signature([*self.selection_registers, *self.kappa_load_target, *self.rotations_target, *self.interleaved_unitary_target])
    ...
    def decompose_from_registers(self, *, context: cirq.DecompositionContext, **quregs: NDArray[cirq.Qid]) -> cirq.OP_TREE:
        ...
        for st in range(0, num_bits, self.kappa):
            ...
            yield qrom.QROM([data], selection_bitsizes=tuple(selection_bitsizes), target_bitsizes=(self.kappa,)).on_registers(selection=selection, target0=kappa_load_target)
            data = angles_bits[:, st:en].dot(power_of_2s[:en - st])
            for cqid, bpow, idx in zip(kappa_load_target, angles_bit_pow[st:en], angles_idx[st:en]):
                if idx != last_id:
                    yield self.interleaved_unitary(last_id, rotations_target=rotations_target, **interleaved_unitary_target)
                    last_id = idx
