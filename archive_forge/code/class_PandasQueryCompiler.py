...
class PandasQueryCompiler(BaseQueryCompiler):
    ...
    def add_suffix(self, suffix, axis=1):
        if axis == 1:
            return self.__constructor__(self._modin_frame.rename(new_col_labels=lambda x: f'{x}{suffix}'))
        else:
            return self.__constructor__(self._modin_frame.rename(new_row_labels=lambda x: f'{x}{suffix}'))
    ...
    combine_first = Binary.register(pandas.DataFrame.combine_first, infer_dtypes='common_cast')
    eq = Binary.register(pandas.DataFrame.eq, infer_dtypes='bool')
    equals = Binary.register(lambda df, other: pandas.DataFrame([[df.equals(other)]]), join_type=None, labels='drop', infer_dtypes='bool')
    ...
    df_update = Binary.register(copy_df_for_func(pandas.DataFrame.update, display_name='update'), join_type='left')
    series_update = Binary.register(copy_df_for_func(lambda x, y: pandas.Series.update(x.squeeze(axis=1), y.squeeze(axis=1)), display_name='update'), join_type='left')
    _logical_and = Binary.register(lambda df, other, *args, **kwargs: pandas.DataFrame(np.logical_and(df, other, *args, **kwargs)), infer_dtypes='bool')
    _logical_or = Binary.register(lambda df, other, *args, **kwargs: pandas.DataFrame(np.logical_or(df, other, *args, **kwargs)), infer_dtypes='bool')
    _logical_xor = Binary.register(lambda df, other, *args, **kwargs: pandas.DataFrame(np.logical_xor(df, other, *args, **kwargs)), infer_dtypes='bool')

    def where(self, cond, other, **kwargs):
        assert isinstance(cond, type(self)), 'Must have the same QueryCompiler subclass to perform this operation'
        if isinstance(other, type(self)) and other._shape_hint is not None:
            other = other.to_pandas()
        if isinstance(other, type(self)):
            new_modin_frame = self._modin_frame.n_ary_op(lambda df, cond, other: df.where(cond, other, **kwargs), [cond._modin_frame, other._modin_frame], join_type=None)
        else:

            def where_builder_series(df, cond):
                return df.where(cond, other, **kwargs)
            new_modin_frame = self._modin_frame.n_ary_op(where_builder_series, [cond._modin_frame], join_type='left')
    ...
    def merge(self, right, **kwargs):
        if RangePartitioning.get():
            try:
                ...
                message = f"Can't use range-partitioning merge implementation because of: {e}" + '\nFalling back to a row-axis implementation.'
    ...
    def join(self, right, **kwargs):
        ...
        if how in ['left', 'inner']:
            ...
            new_self = self.__constructor__(self._modin_frame.broadcast_apply_full_axis(axis=1, func=map_func, keep_partitioning=False, num_splits=merge_partitioning(self._modin_frame, right._modin_frame, axis=1), other=right_to_broadcast))
    ...
    def reindex(self, axis, labels, **kwargs):
        ...
        new_dtypes = None
        if self._modin_frame.has_materialized_dtypes and kwargs.get('method', None) is None:
        ...
        new_modin_frame = self._modin_frame.apply_full_axis(axis, lambda df: df.reindex(labels=labels, axis=axis, **kwargs), new_index=new_index, new_columns=new_columns, dtypes=new_dtypes)
        return self.__constructor__(new_modin_frame)
    ...
    def reset_index(self, **kwargs):
        if self.lazy_execution:
            ...
            if kwargs['drop']:
                ...
                try:
                    dtypes = ModinDtypes.concat([index_dtypes, self._modin_frame._dtypes])
                except NotImplementedError:
                    dtypes = None
                if self._modin_frame.has_materialized_columns and index_dtypes is not None:
                    empty_index = pandas.Index([0], name=index_dtypes.index[0]) if len(index_dtypes) == 1 else pandas.MultiIndex.from_arrays([[i] for i in range(len(index_dtypes))], names=index_dtypes.index)
                    new_columns = pandas.DataFrame(columns=self.columns, index=empty_index).reset_index(**kwargs).columns
            return self.__constructor__(self._modin_frame.apply_full_axis(axis=1, func=_reset, enumerate_partitions=True, new_columns=new_columns, dtypes=dtypes, sync_labels=False, pass_axis_lengths_to_partitions=True))
        ...
        if allow_duplicates not in (lib.no_default, False) or names is not None:
            return self.default_to_pandas(pandas.DataFrame.reset_index, allow_duplicates=allow_duplicates, names=names, **kwargs)
        ...
        if level is not None:
            ...
            if len(uniq_sorted_level) < self.index.nlevels:
                new_index = self.index.droplevel(uniq_sorted_level) if len(level) < self.index.nlevels else pandas.RangeIndex(len(self.index))
        elif not drop:
        ...
        if not drop:
            if len(uniq_sorted_level) < self.index.nlevels:
                ...
                new_copy.index.names = ['level_{}'.format(level_value) if new_copy.index.names[level_index] is None else new_copy.index.names[level_index] for level_index, level_value in enumerate(uniq_sorted_level)]
            ...
            if isinstance(new_modin_frame.columns, pandas.MultiIndex):
                ...
                if col_level != 0 or col_fill != '':
                    levels_names_list = [f'level_{level_index}' if level_name is None else level_name for level_index, level_name in enumerate(self.index.names)]
                    if col_fill is None:
                        last_col_name = levels_names_list[uniq_sorted_level[-1]]
                        last_col_name = list(last_col_name) if isinstance(last_col_name, tuple) else [last_col_name]
                        if len(last_col_name) not in (1, self.columns.nlevels):
                            raise ValueError('col_fill=None is incompatible ' + f'with incomplete column name {last_col_name}')
                    ...
                    for level_index, level_value in enumerate(uniq_sorted_level):
                        ...
                        middle_level = list(level_name) if isinstance(level_name, tuple) else [level_name]
                        bottom_level = [col_fill] * (self.columns.nlevels - (col_level + len(middle_level)))
                        item = tuple(top_level + middle_level + bottom_level)
                        if len(item) > self.columns.nlevels:
                            raise ValueError('Item must have length equal to number of levels.')
                        columns_list[level_index] = item
                    new_modin_frame.columns = pandas.MultiIndex.from_tuples(columns_list, names=self.columns.names)
            new_self = self.__constructor__(new_modin_frame)
        else:
            new_self = self.copy()
            new_self.index = pandas.RangeIndex(sum(new_self._modin_frame.row_lengths)) if new_index is None else new_index
        return new_self
    ...
    def _dtypes_sum(dtypes: pandas.Series, *func_args, **func_kwargs):
    ...
    _memory_usage_without_index = TreeReduce.register(pandas.DataFrame.memory_usage, lambda x, *args, **kwargs: pandas.DataFrame.sum(x), axis=0)

    def memory_usage(self, **kwargs):
        index = kwargs.get('index', True)
        deep = kwargs.get('deep', False)
        usage_without_index = self._memory_usage_without_index(index=False, deep=deep)
        return self.from_pandas(pandas.DataFrame([self.index.memory_usage()], columns=['Index'], index=[MODIN_UNNAMED_SERIES_LABEL]), data_cls=type(self._modin_frame)).concat(axis=1, other=[usage_without_index]) if index else usage_without_index
    ...
    def mean(self, axis, **kwargs):
        ...
        def map_fn(df, numeric_only=False, **kwargs):
            ...
            """
            result = pandas.DataFrame({'sum': df.sum(axis=axis, skipna=skipna, numeric_only=numeric_only), 'count': df.count(axis=axis, numeric_only=numeric_only)})
        ...
        def compute_dtypes_fn(dtypes, axis, **kwargs):
            ...
            """
            if axis == 1 and any((is_bool_dtype(t) for t in dtypes)) and any((is_numeric_dtype(t) for t in dtypes)):
                return 'object'
            return 'float64'
        return TreeReduce.register(map_fn, reduce_fn, compute_dtypes=compute_dtypes_fn)(self, axis=axis, **kwargs)
    ...
    def nunique(self, axis=0, dropna=True):
        ...
        if len(unsupported_message) > 0:
            message = f"Can't use range-partitioning implementation for 'nunique' because:\n{unsupported_message}" + 'Falling back to a full-axis reduce implementation.'
            get_logger().info(message)
            ErrorMessage.warn(message)
            return Reduce.register(pandas.DataFrame.nunique)(self, axis=axis, dropna=dropna)
        new_modin_frame = self._modin_frame._apply_func_to_range_partitioning(key_columns=self.columns.tolist(), func=lambda df: df.nunique(dropna=dropna).to_frame())
    ...
    def to_datetime(self, *args, **kwargs):
        if len(self.columns) == 1:
            return Map.register(lambda df, *args, **kwargs: pandas.to_datetime(df.squeeze(axis=1), *args, **kwargs).to_frame(), shape_hint='column')(self, *args, **kwargs)
        else:
            return Reduce.register(pandas.to_datetime, axis=1, shape_hint='column')(self, *args, **kwargs)
    ...
    def resample_get_group(self, resample_kwargs, name, obj):
        return self._resample_func(resample_kwargs, 'get_group', name=name, allow_range_impl=False, obj=obj)

    def resample_app_ser(self, resample_kwargs, func, *args, **kwargs):
        return self._resample_func(resample_kwargs, 'apply', *args, df_op=lambda df: df.squeeze(axis=1), func=func, **kwargs)

    def resample_app_df(self, resample_kwargs, func, *args, **kwargs):
        return self._resample_func(resample_kwargs, 'apply', *args, func=func, **kwargs)

    def resample_agg_ser(self, resample_kwargs, func, *args, **kwargs):
        return self._resample_func(resample_kwargs, 'aggregate', *args, df_op=lambda df: df.squeeze(axis=1), func=func, **kwargs)

    def resample_agg_df(self, resample_kwargs, func, *args, **kwargs):
        return self._resample_func(resample_kwargs, 'aggregate', *args, func=func, **kwargs)

    def resample_transform(self, resample_kwargs, arg, *args, **kwargs):
        return self._resample_func(resample_kwargs, 'transform', *args, arg=arg, allow_range_impl=False, **kwargs)

    def resample_pipe(self, resample_kwargs, func, *args, **kwargs):
        return self._resample_func(resample_kwargs, 'pipe', *args, func=func, **kwargs)

    def resample_ffill(self, resample_kwargs, limit):
        return self._resample_func(resample_kwargs, 'ffill', limit=limit, allow_range_impl=False)

    def resample_bfill(self, resample_kwargs, limit):
        return self._resample_func(resample_kwargs, 'bfill', limit=limit, allow_range_impl=False)

    def resample_nearest(self, resample_kwargs, limit):
        return self._resample_func(resample_kwargs, 'nearest', limit=limit, allow_range_impl=False)

    def resample_fillna(self, resample_kwargs, method, limit):
        return self._resample_func(resample_kwargs, 'fillna', method=method, limit=limit, allow_range_impl=method is None)
    ...
    def resample_interpolate(self, resample_kwargs, method, axis, limit, inplace, limit_direction, limit_area, downcast, **kwargs):
        return self._resample_func(resample_kwargs, 'interpolate', axis=axis, limit=limit, inplace=inplace, limit_direction=limit_direction, limit_area=limit_area, downcast=downcast, allow_range_impl=False, **kwargs)
    ...
    def resample_first(self, resample_kwargs, *args, **kwargs):
        return self._resample_func(resample_kwargs, 'first', *args, allow_range_impl=False, **kwargs)

    def resample_last(self, resample_kwargs, *args, **kwargs):
        return self._resample_func(resample_kwargs, 'last', *args, allow_range_impl=False, **kwargs)
    ...
    def resample_ohlc_ser(self, resample_kwargs, *args, **kwargs):
        return self._resample_func(resample_kwargs, 'ohlc', *args, df_op=lambda df: df.squeeze(axis=1), **kwargs)
    ...
    def resample_prod(self, resample_kwargs, min_count, *args, **kwargs):
        return self._resample_func(resample_kwargs, 'prod', *args, min_count=min_count, **kwargs)

    def resample_size(self, resample_kwargs):
        return self._resample_func(resample_kwargs, 'size', new_columns=[MODIN_UNNAMED_SERIES_LABEL], allow_range_impl=False)
    ...
    def resample_std(self, resample_kwargs, ddof, *args, **kwargs):
        return self._resample_func(resample_kwargs, 'std', *args, ddof=ddof, **kwargs)

    def resample_sum(self, resample_kwargs, min_count, *args, **kwargs):
        return self._resample_func(resample_kwargs, 'sum', *args, min_count=min_count, **kwargs)

    def resample_var(self, resample_kwargs, ddof, *args, **kwargs):
        return self._resample_func(resample_kwargs, 'var', *args, ddof=ddof, **kwargs)
    ...
    def expanding_aggregate(self, axis, expanding_args, func, *args, **kwargs):
        new_modin_frame = self._modin_frame.apply_full_axis(axis, lambda df: pandas.DataFrame(df.expanding(*expanding_args).aggregate(*args, func=func, **kwargs)), new_index=self.index)
        return self.__constructor__(new_modin_frame)
    expanding_sum = Fold.register(lambda df, expanding_args, *args, **kwargs: pandas.DataFrame(df.expanding(*expanding_args).sum(*args, **kwargs)))
    expanding_min = Fold.register(lambda df, expanding_args, *args, **kwargs: pandas.DataFrame(df.expanding(*expanding_args).min(*args, **kwargs)))
    expanding_max = Fold.register(lambda df, expanding_args, *args, **kwargs: pandas.DataFrame(df.expanding(*expanding_args).max(*args, **kwargs)))
    expanding_mean = Fold.register(lambda df, expanding_args, *args, **kwargs: pandas.DataFrame(df.expanding(*expanding_args).mean(*args, **kwargs)))
    expanding_median = Fold.register(lambda df, expanding_args, *args, **kwargs: pandas.DataFrame(df.expanding(*expanding_args).median(*args, **kwargs)))
    expanding_var = Fold.register(lambda df, expanding_args, *args, **kwargs: pandas.DataFrame(df.expanding(*expanding_args).var(*args, **kwargs)))
    expanding_std = Fold.register(lambda df, expanding_args, *args, **kwargs: pandas.DataFrame(df.expanding(*expanding_args).std(*args, **kwargs)))
    expanding_count = Fold.register(lambda df, expanding_args, *args, **kwargs: pandas.DataFrame(df.expanding(*expanding_args).count(*args, **kwargs)))

    def expanding_cov(self, fold_axis, expanding_args, squeeze_self, squeeze_other, other=None, pairwise=None, ddof=1, numeric_only=False, **kwargs):
        other_for_pandas = other if other is None else other.to_pandas().squeeze(axis=1) if squeeze_other else other.to_pandas()
        if len(self.columns) > 1:
            return self.default_to_pandas(lambda df: pandas.DataFrame.expanding(df, *expanding_args).cov(other=other_for_pandas, pairwise=pairwise, ddof=ddof, numeric_only=numeric_only, **kwargs))
        return Fold.register(lambda df, expanding_args, *args, **kwargs: pandas.DataFrame((df.squeeze(axis=1) if squeeze_self else df).expanding(*expanding_args).cov(*args, **kwargs)))(self, fold_axis, expanding_args, other=other_for_pandas, pairwise=pairwise, ddof=ddof, numeric_only=numeric_only, **kwargs)
    ...
    def expanding_corr(self, fold_axis, expanding_args, squeeze_self, squeeze_other, other=None, pairwise=None, ddof=1, numeric_only=False, **kwargs):
        other_for_pandas = other if other is None else other.to_pandas().squeeze(axis=1) if squeeze_other else other.to_pandas()
        if len(self.columns) > 1:
            return self.default_to_pandas(lambda df: pandas.DataFrame.expanding(df, *expanding_args).corr(other=other_for_pandas, pairwise=pairwise, ddof=ddof, numeric_only=numeric_only, **kwargs))
        return Fold.register(lambda df, expanding_args, *args, **kwargs: pandas.DataFrame((df.squeeze(axis=1) if squeeze_self else df).expanding(*expanding_args).corr(*args, **kwargs)))(self, fold_axis, expanding_args, other=other_for_pandas, pairwise=pairwise, ddof=ddof, numeric_only=numeric_only, **kwargs)
    expanding_quantile = Fold.register(lambda df, expanding_args, *args, **kwargs: pandas.DataFrame(df.expanding(*expanding_args).quantile(*args, **kwargs)))
    expanding_sem = Fold.register(lambda df, expanding_args, *args, **kwargs: pandas.DataFrame(df.expanding(*expanding_args).sem(*args, **kwargs)))
    expanding_kurt = Fold.register(lambda df, expanding_args, *args, **kwargs: pandas.DataFrame(df.expanding(*expanding_args).kurt(*args, **kwargs)))
    expanding_skew = Fold.register(lambda df, expanding_args, *args, **kwargs: pandas.DataFrame(df.expanding(*expanding_args).skew(*args, **kwargs)))
    expanding_rank = Fold.register(lambda df, expanding_args, *args, **kwargs: pandas.DataFrame(df.expanding(*expanding_args).rank(*args, **kwargs)))
    window_mean = Fold.register(lambda df, rolling_kwargs, *args, **kwargs: pandas.DataFrame(df.rolling(**rolling_kwargs).mean(*args, **kwargs)))
    window_sum = Fold.register(lambda df, rolling_kwargs, *args, **kwargs: pandas.DataFrame(df.rolling(**rolling_kwargs).sum(*args, **kwargs)))
    window_var = Fold.register(lambda df, rolling_kwargs, ddof, *args, **kwargs: pandas.DataFrame(df.rolling(**rolling_kwargs).var(*args, ddof=ddof, **kwargs)))
    window_std = Fold.register(lambda df, rolling_kwargs, ddof, *args, **kwargs: pandas.DataFrame(df.rolling(**rolling_kwargs).std(*args, ddof=ddof, **kwargs)))
    rolling_count = Fold.register(lambda df, rolling_kwargs: pandas.DataFrame(df.rolling(**rolling_kwargs).count()))
    rolling_sum = Fold.register(lambda df, rolling_kwargs, *args, **kwargs: pandas.DataFrame(df.rolling(**rolling_kwargs).sum(*args, **kwargs)))
    rolling_sem = Fold.register(lambda df, rolling_kwargs, *args, **kwargs: pandas.DataFrame(df.rolling(**rolling_kwargs).sem(*args, **kwargs)))
    rolling_mean = Fold.register(lambda df, rolling_kwargs, *args, **kwargs: pandas.DataFrame(df.rolling(**rolling_kwargs).mean(*args, **kwargs)))
    rolling_median = Fold.register(lambda df, rolling_kwargs, **kwargs: pandas.DataFrame(df.rolling(**rolling_kwargs).median(**kwargs)))
    rolling_var = Fold.register(lambda df, rolling_kwargs, ddof, *args, **kwargs: pandas.DataFrame(df.rolling(**rolling_kwargs).var(*args, ddof=ddof, **kwargs)))
    rolling_std = Fold.register(lambda df, rolling_kwargs, ddof, *args, **kwargs: pandas.DataFrame(df.rolling(**rolling_kwargs).std(*args, ddof=ddof, **kwargs)))
    rolling_min = Fold.register(lambda df, rolling_kwargs, *args, **kwargs: pandas.DataFrame(df.rolling(**rolling_kwargs).min(*args, **kwargs)))
    rolling_max = Fold.register(lambda df, rolling_kwargs, *args, **kwargs: pandas.DataFrame(df.rolling(**rolling_kwargs).max(*args, **kwargs)))
    rolling_skew = Fold.register(lambda df, rolling_kwargs, **kwargs: pandas.DataFrame(df.rolling(**rolling_kwargs).skew(**kwargs)))
    rolling_kurt = Fold.register(lambda df, rolling_kwargs, **kwargs: pandas.DataFrame(df.rolling(**rolling_kwargs).kurt(**kwargs)))
    rolling_apply = Fold.register(lambda df, rolling_kwargs, func, raw, engine, engine_kwargs, args, kwargs: pandas.DataFrame(df.rolling(**rolling_kwargs).apply(func=func, raw=raw, engine=engine, engine_kwargs=engine_kwargs, args=args, kwargs=kwargs)))
    rolling_quantile = Fold.register(lambda df, rolling_kwargs, q, interpolation, **kwargs: pandas.DataFrame(df.rolling(**rolling_kwargs).quantile(q=q, interpolation=interpolation, **kwargs)))
    rolling_rank = Fold.register(lambda df, rolling_kwargs, method, ascending, pct, numeric_only, **kwargs: pandas.DataFrame(df.rolling(**rolling_kwargs).rank(method=method, ascending=ascending, pct=pct, numeric_only=numeric_only, **kwargs)))

    def rolling_corr(self, axis, rolling_kwargs, other, pairwise, *args, **kwargs):
        if len(self.columns) > 1:
            return self.default_to_pandas(lambda df: pandas.DataFrame.rolling(df, **rolling_kwargs).corr(*args, other=other, pairwise=pairwise, **kwargs))
        else:
            return Fold.register(lambda df: pandas.DataFrame(df.rolling(**rolling_kwargs).corr(*args, other=other, pairwise=pairwise, **kwargs)))(self, axis)

    def rolling_cov(self, axis, rolling_kwargs, other, pairwise, ddof, **kwargs):
        if len(self.columns) > 1:
            return self.default_to_pandas(lambda df: pandas.DataFrame.rolling(df, **rolling_kwargs).cov(other=other, pairwise=pairwise, ddof=ddof, **kwargs))
        else:
            return Fold.register(lambda df: pandas.DataFrame(df.rolling(**rolling_kwargs).cov(other=other, pairwise=pairwise, ddof=ddof, **kwargs)))(self, axis)

    def rolling_aggregate(self, axis, rolling_kwargs, func, *args, **kwargs):
        new_modin_frame = self._modin_frame.apply_full_axis(axis, lambda df: pandas.DataFrame(df.rolling(**rolling_kwargs).aggregate(*args, func=func, **kwargs)), new_index=self.index)
        return self.__constructor__(new_modin_frame)

    def unstack(self, level, fill_value):
        if not isinstance(self.index, pandas.MultiIndex) or (isinstance(self.index, pandas.MultiIndex) and is_list_like(level) and (len(level) == self.index.nlevels)):
            axis = 1
        ...
        if isinstance(self.index, pandas.MultiIndex) and isinstance(self.columns, pandas.MultiIndex) and is_list_like(level) and (len(level) == self.index.nlevels) and is_tree_like_or_1d_index and is_tree_like_or_1d_cols:
            ...
            obj = self
        new_modin_frame = obj._modin_frame.apply_full_axis(axis, map_func, new_columns=new_columns)
        result = self.__constructor__(new_modin_frame)
        ...
        def compute_index(index, columns, consider_index=True, consider_columns=True):
            ...
            def get_unique_level_values(index):
                return [index.get_level_values(lvl).unique() for lvl in np.arange(index.nlevels)]
            new_index = get_unique_level_values(index) if consider_index else index if isinstance(index, list) else [index]
            new_columns = get_unique_level_values(columns) if consider_columns else [columns]
        ...
        if is_all_multi_list and is_tree_like_or_1d_index and is_tree_like_or_1d_cols:
            ...
            result.index = compute_index(index_level_values, real_cols_bkp, consider_index=False)
            return result
        if need_reindex:
            if is_tree_like_or_1d_index and is_tree_like_or_1d_cols:
                is_recompute_index = isinstance(self.index, pandas.MultiIndex)
                is_recompute_columns = not is_recompute_index and isinstance(self.columns, pandas.MultiIndex)
                new_index = compute_index(self.index, self.columns, is_recompute_index, is_recompute_columns)
            elif is_tree_like_or_1d_index != is_tree_like_or_1d_cols:
                if isinstance(self.columns, pandas.MultiIndex) or not isinstance(self.index, pandas.MultiIndex):
                    return result
                else:
                    index = self.index.sortlevel()[0] if is_tree_like_or_1d_index and (not is_tree_like_or_1d_cols) and isinstance(self.index, pandas.MultiIndex) else self.index
                    index = pandas.MultiIndex.from_tuples(list(index) * len(self.columns))
                    columns = self.columns.repeat(len(self.index))
                    index_levels = [index.get_level_values(i) for i in range(index.nlevels)]
                    new_index = pandas.MultiIndex.from_arrays([columns] + index_levels, names=self.columns.names + self.index.names)
            else:
                return result
    ...
    def stack(self, level, dropna):
        if not isinstance(self.columns, pandas.MultiIndex) or (isinstance(self.columns, pandas.MultiIndex) and is_list_like(level) and (len(level) == self.columns.nlevels)):
            new_columns = [MODIN_UNNAMED_SERIES_LABEL]
        else:
            new_columns = None
        new_modin_frame = self._modin_frame.apply_full_axis(1, lambda df: pandas.DataFrame(df.stack(level=level, dropna=dropna)), new_columns=new_columns)
        return self.__constructor__(new_modin_frame)
    ...
    def isin(self, values, ignore_indices=False):
        ...
        def isin_func(df, values):
            ...
            if isinstance(res, pandas.Series):
                res = res.to_frame(MODIN_UNNAMED_SERIES_LABEL if res.name is None else res.name)
            return res
        return Map.register(isin_func, shape_hint=shape_hint, dtypes=np.bool_)(self, values)
    ...
    _isfinite = Map.register(lambda df, *args, **kwargs: pandas.DataFrame(np.isfinite(df, *args, **kwargs)), dtypes=np.bool_)
    _isinf = Map.register(lambda df, *args, **kwargs: pandas.DataFrame(np.isinf(df, *args, **kwargs)), dtypes=np.bool_)
    _isnat = Map.register(lambda df, *args, **kwargs: pandas.DataFrame(np.isnat(df, *args, **kwargs)), dtypes=np.bool_)
    _isneginf = Map.register(lambda df, *args, **kwargs: pandas.DataFrame(np.isneginf(df, *args, **kwargs)), dtypes=np.bool_)
    _isposinf = Map.register(lambda df, *args, **kwargs: pandas.DataFrame(np.isposinf(df, *args, **kwargs)), dtypes=np.bool_)
    _iscomplex = Map.register(lambda df, *args, **kwargs: pandas.DataFrame(np.iscomplex(df, *args, **kwargs)), dtypes=np.bool_)
    _isreal = Map.register(lambda df, *args, **kwargs: pandas.DataFrame(np.isreal(df, *args, **kwargs)), dtypes=np.bool_)
    _logical_not = Map.register(np.logical_not, dtypes=np.bool_)
    _tanh = Map.register(lambda df, *args, **kwargs: pandas.DataFrame(np.tanh(df, *args, **kwargs)))
    _sqrt = Map.register(lambda df, *args, **kwargs: pandas.DataFrame(np.sqrt(df, *args, **kwargs)))
    _exp = Map.register(lambda df, *args, **kwargs: pandas.DataFrame(np.exp(df, *args, **kwargs)))
    ...
    series_view = Map.register(lambda df, *args, **kwargs: pandas.DataFrame(df.squeeze(axis=1).view(*args, **kwargs)))
    to_numeric = Map.register(lambda df, *args, **kwargs: pandas.DataFrame(pandas.to_numeric(df.squeeze(axis=1), *args, **kwargs)))
    to_timedelta = Map.register(lambda s, *args, **kwargs: pandas.to_timedelta(s.squeeze(axis=1), *args, **kwargs).to_frame(), dtypes='timedelta64[ns]')
    ...
    _str_rpartition = Map.register(_str_map('rpartition'), dtypes='copy', shape_hint='column')
    ...
    str_slice_replace = Map.register(_str_map('slice_replace'), dtypes='copy', shape_hint='column')
    ...
    str_startswith = Map.register(_str_map('startswith'), dtypes=np.bool_, shape_hint='column')
    str_strip = Map.register(_str_map('strip'), dtypes='copy', shape_hint='column')
    str_swapcase = Map.register(_str_map('swapcase'), dtypes='copy', shape_hint='column')
    str_title = Map.register(_str_map('title'), dtypes='copy', shape_hint='column')
    str_translate = Map.register(_str_map('translate'), dtypes='copy', shape_hint='column')
    ...
    str___getitem__ = Map.register(_str_map('__getitem__'), dtypes='copy', shape_hint='column')

    def unique(self, keep='first', ignore_index=True, subset=None):
        can_use_unique_kernel = subset is None and ignore_index and (len(self.columns) == 1) and (keep is not False)
        ...
        if RangePartitioning.get():
            new_modin_frame = self._modin_frame._apply_func_to_range_partitioning(key_columns=self.columns.tolist() if subset is None else subset, func=(lambda df: pandas.DataFrame(df.squeeze(axis=1).unique(), columns=['__reduced__'])) if can_use_unique_kernel else lambda df: df.drop_duplicates(keep=keep, ignore_index=ignore_index, subset=subset), preserve_columns=True)
        else:
            new_modin_frame = self._modin_frame.apply_full_axis(0, lambda x: x.squeeze(axis=1).unique(), new_columns=self.columns)
    ...
    dt_is_quarter_start = Map.register(_dt_prop_map('is_quarter_start'), dtypes=np.bool_)
    ...
    def astype(self, col_dtypes, errors: str='raise'):
        return self.__constructor__(self._modin_frame.astype(col_dtypes, errors=errors), shape_hint=self._shape_hint)
    ...
    def first_valid_index(self):
        ...
        def first_valid_index_builder(df):
            ...
            return df.set_axis(pandas.RangeIndex(len(df.index)), axis='index').apply(lambda df: df.first_valid_index())
        first_result = self.__constructor__(self._modin_frame.reduce(0, first_valid_index_builder)).min(axis=1).to_pandas().squeeze()
    ...
    def last_valid_index(self):
        ...
        def last_valid_index_builder(df):
            ...
            return df.set_axis(pandas.RangeIndex(len(df.index)), axis='index').apply(lambda df: df.last_valid_index())
        first_result = self.__constructor__(self._modin_frame.reduce(0, last_valid_index_builder)).max(axis=1).to_pandas().squeeze()
        return self.index[first_result]

    def describe(self, percentiles: np.ndarray):
        empty_df = pandas.DataFrame(columns=self.columns).astype(self.dtypes).describe(percentiles, include='all')
        ...
        def describe_builder(df, internal_indices=[]):
            ...
            return df.iloc[:, internal_indices].describe(percentiles=percentiles, include='all').reindex(new_index)
        return self.__constructor__(self._modin_frame.apply_full_axis_select_indices(0, describe_builder, empty_df.columns, new_index=new_index, new_columns=empty_df.columns))
    ...
    def dot(self, other, squeeze_self=None, squeeze_other=None):
        if isinstance(other, PandasQueryCompiler):
            other = other.to_pandas().squeeze(axis=1) if squeeze_other else other.to_pandas()
        num_cols = other.shape[1] if len(other.shape) > 1 else 1
        if len(self.columns) == 1:
            new_index = [MODIN_UNNAMED_SERIES_LABEL] if (len(self.index) == 1 or squeeze_self) and num_cols == 1 else None
            new_columns = [MODIN_UNNAMED_SERIES_LABEL] if squeeze_self and num_cols == 1 else None
            axis = 0
        else:
            new_index = self.index
            new_columns = [MODIN_UNNAMED_SERIES_LABEL] if num_cols == 1 else None
            axis = 1
        align_index = isinstance(new_index, list) and new_index == [MODIN_UNNAMED_SERIES_LABEL]
        ...
        def map_func(df, other=other, squeeze_self=squeeze_self):
            ...
            return res
        new_modin_frame = self._modin_frame.apply_full_axis(axis, map_func, new_index=new_index, new_columns=new_columns, sync_labels=False)
        return self.__constructor__(new_modin_frame)
    ...
    def _nsort(self, n, columns=None, keep='first', sort_type='nsmallest'):
        ...
        def map_func(df, n=n, keep=keep, columns=columns):
            ...
            if columns is None:
                return pandas.DataFrame(getattr(pandas.Series, sort_type)(df.squeeze(axis=1), n=n, keep=keep))
    ...
    def nsmallest(self, *args, **kwargs):
        return self._nsort(*args, sort_type='nsmallest', **kwargs)

    def nlargest(self, *args, **kwargs):
        return self._nsort(*args, sort_type='nlargest', **kwargs)
    ...
    def eval(self, expr, **kwargs):
        ...
        if isinstance(empty_eval, pandas.Series):
            new_columns = [empty_eval.name] if empty_eval.name is not None else [MODIN_UNNAMED_SERIES_LABEL]
        else:
            new_columns = empty_eval.columns
        new_modin_frame = self._modin_frame.apply_full_axis(1, lambda df: pandas.DataFrame(df.eval(expr, inplace=False, **kwargs)), new_index=self.index, new_columns=new_columns)
        return self.__constructor__(new_modin_frame)
    ...
    def mode(self, **kwargs):
        ...
        new_modin_frame = self._modin_frame.apply_full_axis(axis, mode_builder, new_index=new_index, new_columns=new_columns)
        ...
        if isinstance(value, BaseQueryCompiler):
            if squeeze_self:
                if full_axis:
                    ...
                    new_modin_frame = self._modin_frame.apply_full_axis(0, fillna_builder)
                else:
                    ...
                    new_modin_frame = self._modin_frame.n_ary_op(fillna_builder, [value._modin_frame], join_type='left', copartition_along_columns=False)
                return self.__constructor__(new_modin_frame)
            elif squeeze_value:
                value = value.to_pandas().squeeze(axis=1)
                ...
                new_modin_frame = self._modin_frame.broadcast_apply(0, fillna_builder, value._modin_frame)
                return self.__constructor__(new_modin_frame)
        elif isinstance(value, dict):
            if squeeze_self:

                def fillna(df):
                    return pandas.DataFrame(df.squeeze(axis=1).fillna(value=value, **kwargs))
            else:

                def fillna(df):
                    func_dict = {col: val for col, val in value.items() if col in df.columns}
                    return df.fillna(value=func_dict, **kwargs)
                if self._modin_frame.has_materialized_dtypes:
                    dtypes = self._modin_frame.dtypes
                    value_dtypes = pandas.DataFrame({k: [v] for k, v in value.items()}).dtypes
                    if all((find_common_type([dtypes[col], dtype]) == dtypes[col] for col, dtype in value_dtypes.items() if col in dtypes)):
                        new_dtypes = dtypes
        else:
            if self._modin_frame.has_materialized_dtypes:
                dtype = pandas.Series(value).dtype
                if all((find_common_type([t, dtype]) == t for t in self._modin_frame.dtypes)):
    ...
    def quantile_for_list_of_values(self, **kwargs):
        ...
        if numeric_only:
            ...
            new_columns = [col for col, dtype in zip(self.columns, self.dtypes) if is_numeric_dtype(dtype) or lib.is_np_dtype(dtype, 'mM')]
        if axis == 1:
        ...
        new_modin_frame = query_compiler._modin_frame.apply_full_axis(axis, lambda df: quantile_builder(df, **kwargs), new_index=q_index, new_columns=new_columns, dtypes=np.float64)
        result = self.__constructor__(new_modin_frame)
    ...
    def rank(self, **kwargs):
        ...
        new_modin_frame = self._modin_frame.apply_full_axis(axis, lambda df: df.rank(**kwargs), new_index=self._modin_frame.copy_index_cache(copy_lengths=True), new_columns=self._modin_frame.copy_columns_cache(copy_lengths=True) if not numeric_only else None, dtypes=np.float64, sync_labels=False)
        return self.__constructor__(new_modin_frame)
    ...
    def sort_index(self, **kwargs):
        ...
        if level is not None or self.has_multiindex(axis=axis):
            return self.default_to_pandas(pandas.DataFrame.sort_index, axis=axis, level=level, sort_remaining=sort_remaining, **kwargs)
        ...
        if axis:
            ...
            new_columns = self.columns
        new_modin_frame = self._modin_frame.apply_full_axis(axis, lambda df: df.sort_index(axis=axis, level=level, sort_remaining=sort_remaining, **kwargs), new_index, new_columns, dtypes='copy' if axis == 0 else None)
        return self.__constructor__(new_modin_frame)

    def melt(self, id_vars=None, value_vars=None, var_name=None, value_name='value', col_level=None, ignore_index=True):
        ErrorMessage.mismatch_with_pandas(operation='melt', message='Order of rows could be different from pandas')
        if var_name is None:
            var_name = 'variable'
        ...
        def applyier(df, internal_indices, other=[], internal_other_indices=[]):
            ...
            return df.melt(id_vars=id_vars, value_vars=df.columns[internal_indices], var_name=var_name, value_name=value_name, col_level=col_level)
        inconsistent_frame = self._modin_frame.broadcast_apply_select_indices(axis=0, apply_indices=value_vars, func=applyier, other=to_broadcast, new_index=['dummy_index'] * len(id_vars), new_columns=['dummy_index'] * len(id_vars))
        new_parts = np.array([np.array([x]) for x in np.concatenate(inconsistent_frame._partitions.T)])
        new_index = pandas.RangeIndex(len(self.index) * len(value_vars))
        new_modin_frame = self._modin_frame.__constructor__(new_parts, index=new_index, columns=id_vars + [var_name, value_name])
        result = self.__constructor__(new_modin_frame)
        result.index = new_index
        return result
    __getitem_bool = Binary.register(lambda df, r: df[[r]] if is_scalar(r) else df[r], join_type='left', labels='drop')
    ...
    def setitem_bool(self, row_loc, col_loc, item):
        ...
        if self._modin_frame.has_materialized_dtypes and is_scalar(item):
            ...
            if isinstance(old_dtypes, pandas.Series):
                new_dtypes[col_loc] = [find_common_type([dtype, item_type]) for dtype in old_dtypes.values]
            else:
                new_dtypes[col_loc] = find_common_type([old_dtypes, item_type])
        else:
            new_dtypes = None
        new_modin_frame = self._modin_frame.broadcast_apply_full_axis(axis=1, func=_set_item, other=row_loc._modin_frame, new_index=self._modin_frame.copy_index_cache(copy_lengths=True), new_columns=self._modin_frame.copy_columns_cache(), keep_partitioning=False, dtypes=new_dtypes)
        return self.__constructor__(new_modin_frame)

    def __validate_bool_indexer(self, indexer):
        if len(indexer) != len(self.index):
            raise ValueError(f'Item wrong length {len(indexer)} instead of {len(self.index)}.')
        if isinstance(indexer, pandas.Series) and (not indexer.equals(self.index)):
            warnings.warn('Boolean Series key will be reindexed to match DataFrame index.', PendingDeprecationWarning, stacklevel=4)
    ...
    def getitem_array(self, key):
        ...
        if is_bool_indexer(key):
            ...
            if len(key):
                ...
                return self.from_pandas(pandas.DataFrame(columns=self.columns), type(self._modin_frame))
        else:
    ...
    def getitem_row_array(self, key):
        return self.__constructor__(self._modin_frame.take_2d_labels_or_positional(row_positions=key))
    ...
    def _setitem(self, axis, key, value, how='inner'):
        ...
        if axis == 0:
            ...
            new_dtypes = ModinDtypes.concat([old_dtypes, DtypesDescriptor({key: value_dtype}, cols_with_unknown_dtypes=[])]).lazy_get(self.columns)
        else:
            new_dtypes = None
        if is_list_like(value):
            new_modin_frame = self._modin_frame.apply_full_axis_select_indices(axis, setitem_builder, [key], new_index=self.index, new_columns=self.columns, keep_remaining=True, new_dtypes=new_dtypes)
        else:
            new_modin_frame = self._modin_frame.apply_select_indices(axis, setitem_builder, [key], new_index=self.index, new_columns=self.columns, new_dtypes=new_dtypes, keep_remaining=True)
        return self.__constructor__(new_modin_frame)

    def dropna(self, **kwargs):
        is_column_wise = kwargs.get('axis', 0) == 1
        no_thresh_passed = kwargs.get('thresh', lib.no_default) in (lib.no_default, None)
        ...
        if is_column_wise and no_thresh_passed and processable_amount_of_partitions:
            ...
            condition = lambda df: getattr(df, how)()

            def mapper(df: pandas.DataFrame):
                """Compute a mask indicating whether there are all/any NaN values in each column."""
                if subset is not None:
                    subset_mask = condition(df.loc[df.index.intersection(subset)].isna())
                    mask = pandas.Series(np.zeros(df.shape[1], dtype=bool), index=df.columns)
                    mask.update(subset_mask)
                else:
                    mask = condition(df.isna())
                return mask.to_frame().T
            masks = self._modin_frame.apply_full_axis(func=mapper, axis=1, keep_partitioning=True)
            ...
            def reduce(df: pandas.DataFrame, mask: pandas.DataFrame):
                ...
                for col, value in to_take_mask.items():
                    if value and col in df:
                        to_take.append(col)
                return df[to_take]
            result = self._modin_frame.broadcast_apply(axis=1, func=reduce, other=masks, copartition=False, labels='drop')
            return self.__constructor__(result, shape_hint=self._shape_hint)
        return self.__constructor__(self._modin_frame.filter(kwargs.get('axis', 0) ^ 1, lambda df: pandas.DataFrame.dropna(df, **kwargs)), shape_hint=self._shape_hint)
    ...
    def drop(self, index=None, columns=None, errors: str='raise'):
        ...
        if columns is not None:
            columns = np.sort(self.columns.get_indexer_for(self.columns.difference(columns)))
        new_modin_frame = self._modin_frame.take_2d_labels_or_positional(row_positions=index, col_positions=columns)
        return self.__constructor__(new_modin_frame)

    def duplicated(self, **kwargs):

        def _compute_hash(df):
            result = df.apply(lambda s: hashlib.new('md5', str(tuple(s)).encode()).hexdigest(), axis=1)
        ...
        if self._modin_frame._partitions.shape[1] > 1:
            hashed_modin_frame = self._modin_frame.reduce(axis=1, function=_compute_hash, dtypes=pandas.api.types.pandas_dtype('O'))
        else:
            hashed_modin_frame = self._modin_frame
        new_modin_frame = hashed_modin_frame.apply_full_axis(axis=0, func=_compute_duplicated, new_index=self._modin_frame.copy_index_cache(), new_columns=[MODIN_UNNAMED_SERIES_LABEL], dtypes=np.bool_, keep_partitioning=True)
        return self.__constructor__(new_modin_frame, shape_hint='column')
    ...
    def insert(self, loc, column, value):
        ...
        new_dtypes = ModinDtypes.concat([self._modin_frame._dtypes, DtypesDescriptor({column: value_dtype}, cols_with_unknown_dtypes=[])]).lazy_get(new_columns)
        new_modin_frame = self._modin_frame.apply_full_axis_select_indices(0, insert, numeric_indices=[loc], keep_remaining=True, new_index=self.index, new_columns=new_columns, new_dtypes=new_dtypes)
        return self.__constructor__(new_modin_frame)
    ...
    def _wrap_column_data(self, data):
        ...
        if is_list_like(data):
            return self.from_pandas(pandas.DataFrame(pandas.Series(data, index=self.index)), data_cls=type(self._modin_frame))
        return data

    def explode(self, column):
        return self.__constructor__(self._modin_frame.explode(1, lambda df: df.explode(column)))
    ...
    def apply_on_series(self, func, *args, **kwargs):
        ...
        return self.__constructor__(self._modin_frame.apply_full_axis(1, lambda df: df.squeeze(axis=1).apply(func, *args, **kwargs)))
    ...
    def _callable_func(self, func, axis, *args, **kwargs):
        ...
        **kwargs : kwargs
        ...
        new_modin_frame = self._modin_frame.apply_full_axis(axis, lambda df: df.apply(func, *args, axis=axis, **kwargs))
        return self.__constructor__(new_modin_frame)
    ...
    def groupby_nth(self, by, axis, groupby_kwargs, agg_args, agg_kwargs, drop=False):
        result = super().groupby_nth(by, axis, groupby_kwargs, agg_args, agg_kwargs, drop)
    ...
    def groupby_mean(self, by, axis, groupby_kwargs, agg_args, agg_kwargs, drop=False):
        if use_range_partitioning_groupby():
            try:
                return self._groupby_shuffle(by=by, agg_func='mean', axis=axis, groupby_kwargs=groupby_kwargs, agg_args=agg_args, agg_kwargs=agg_kwargs, drop=drop)
            except NotImplementedError as e:
        ...
        datetime_cols = {col: dtype for col, dtype in zip(self.dtypes.index, self.dtypes) if is_datetime64_any_dtype(dtype) and col not in internal_by} if not numeric_only else dict()
        if len(datetime_cols) > 0:
            datetime_qc = self.getitem_array(datetime_cols)
            if datetime_qc.isna().any().any(axis=1).to_pandas().squeeze():
                return super().groupby_mean(by=by, axis=axis, groupby_kwargs=groupby_kwargs, agg_args=agg_args, agg_kwargs=agg_kwargs, drop=drop)
        qc_with_converted_datetime_cols = self.astype({col: 'int64' for col in datetime_cols.keys()}) if len(datetime_cols) > 0 else self
        result = GroupbyReduceImpl.build_qc_method('mean')(query_compiler=qc_with_converted_datetime_cols, by=by, axis=axis, groupby_kwargs=groupby_kwargs, agg_args=agg_args, agg_kwargs=agg_kwargs, drop=drop)
        if len(datetime_cols) > 0:
    ...
    def groupby_size(self, by, axis, groupby_kwargs, agg_args, agg_kwargs, drop=False):
        if use_range_partitioning_groupby():
            try:
                return self._groupby_shuffle(by=by, agg_func='size', axis=axis, groupby_kwargs=groupby_kwargs, agg_args=agg_args, agg_kwargs=agg_kwargs, drop=drop)
            except NotImplementedError as e:
                ErrorMessage.warn(f"Can't use range-partitioning groupby implementation because of: {e}" + '\nFalling back to a TreeReduce implementation.')
        result = self._groupby_dict_reduce(by=by, axis=axis, agg_func={self.columns[0]: [('__size_col__', 'size')]}, agg_args=agg_args, agg_kwargs=agg_kwargs, groupby_kwargs=groupby_kwargs, drop=drop, method='size', default_to_pandas_func=lambda grp: grp.size())
        if groupby_kwargs.get('as_index', True):
            ...
            result.columns = result.columns[:-1].droplevel(-1).append(pandas.Index(['size']))
        return result
    ...
    def groupby_dtypes(self, by, axis, groupby_kwargs, agg_args, agg_kwargs, drop=False):
        return self.groupby_agg(by=by, axis=axis, agg_func=lambda df: df.dtypes, how='group_wise', agg_args=agg_args, agg_kwargs=agg_kwargs, groupby_kwargs=groupby_kwargs, drop=drop)
    ...
    def groupby_corr(self, by, axis, groupby_kwargs, agg_args, agg_kwargs, drop=False):
        ErrorMessage.default_to_pandas('`GroupBy.corr`')
        return super().groupby_agg(by=by, agg_func='corr', axis=axis, groupby_kwargs=groupby_kwargs, agg_args=agg_args, agg_kwargs=agg_kwargs, drop=drop)

    def groupby_cov(self, by, axis, groupby_kwargs, agg_args, agg_kwargs, drop=False):
        ErrorMessage.default_to_pandas('`GroupBy.cov`')
        return super().groupby_agg(by=by, agg_func='cov', axis=axis, groupby_kwargs=groupby_kwargs, agg_args=agg_args, agg_kwargs=agg_kwargs, drop=drop)

    def groupby_rolling(self, by, agg_func, axis, groupby_kwargs, rolling_kwargs, agg_args, agg_kwargs, drop=False):
        unsupported_groupby = agg_func in ('corr', 'cov') or rolling_kwargs.get('on') is not None
        ...
        kwargs = {'by': by, 'agg_func': lambda grp, *args, **kwargs: agg_func(grp.rolling(**rolling_kwargs), *args, **kwargs), 'axis': axis, 'groupby_kwargs': groupby_kwargs, 'agg_args': agg_args, 'agg_kwargs': agg_kwargs, 'how': 'direct', 'drop': drop}
        if unsupported_groupby:
            return super(PandasQueryCompiler, self).groupby_agg(**kwargs)
        try:
            return self._groupby_shuffle(**kwargs)
        except NotImplementedError as e:
            get_logger().info(f"Can't use range-partitioning groupby implementation because of: {e}" + '\nFalling back to a full-axis implementation.')
    ...
    def groupby_agg(self, by, agg_func, axis, groupby_kwargs, agg_args, agg_kwargs, how='axis_wise', drop=False, series_groupby=False):
        ...
        if how == 'group_wise' or use_range_partitioning_groupby():
            try:
                return self._groupby_shuffle(by=by, agg_func=agg_func, axis=axis, groupby_kwargs=groupby_kwargs, agg_args=agg_args, agg_kwargs=agg_kwargs, drop=drop, how=how, series_groupby=series_groupby)
            except NotImplementedError as e:
                message = f"Can't use range-partitioning groupby implementation because of: {e}" + '\nFalling back to a full-axis implementation.'
                get_logger().info(message)
                if use_range_partitioning_groupby():
                    ErrorMessage.warn(message)
        if isinstance(agg_func, dict) and GroupbyReduceImpl.has_impl_for(agg_func):
            return self._groupby_dict_reduce(by, agg_func, axis, groupby_kwargs, agg_args, agg_kwargs, drop)
        ...
        internal_qc = [self.getitem_column_array(internal_by)] if len(internal_by) else []
        ...
        if isinstance(original_agg_func, dict):
            ...
            apply_indices = None
        if agg_kwargs.get('as_index', True) and len(not_broadcastable_by) == 0 and (len(broadcastable_by) == 1) and broadcastable_by[0].has_materialized_dtypes:
            new_index = ModinIndex(value=None, axis=0, dtypes=broadcastable_by[0].dtypes)
        else:
            new_index = None
        new_modin_frame = self._modin_frame.broadcast_apply_full_axis(axis=axis, func=lambda df, by=None, partition_idx=None: groupby_agg_builder(df, by, drop, partition_idx), other=broadcastable_by, new_index=new_index, apply_indices=apply_indices, enumerate_partitions=True)
        result = self.__constructor__(new_modin_frame)
        if len(result.columns) == 0 and len(self.columns) != 0 and agg_kwargs.get('numeric_only', False):
    ...
    def pivot(self, index, columns, values):
        ...
        def __convert_by(by):
            ...
            if isinstance(by, pandas.Index):
                by = list(by)
            by = _convert_by(by)
            if len(by) > 0 and (not is_list_like(by[0]) or isinstance(by[0], tuple)) and (not all([key in self.columns for key in by])):
                by = [by]
            return by
        index, columns, values = map(__convert_by, [index, columns, values])
        is_custom_index = len(index) == 1 and is_list_like(index[0]) and (not isinstance(index[0], tuple))
        ...
        if is_custom_index:
            obj.index = index
        reindexed = self.__constructor__(obj._modin_frame.apply_full_axis(1, lambda df: df.set_index(to_reindex, append=len(to_reindex) == 1), new_columns=obj.columns.drop(to_reindex)))
    ...
    def pivot_table(self, index, values, columns, aggfunc, fill_value, margins, dropna, margins_name, observed, sort):
        ErrorMessage.mismatch_with_pandas(operation='pivot_table', message='Order of columns could be different from pandas')
        from pandas.core.reshape.pivot import _convert_by
        ...
        kwargs = {'qc': self, 'unique_keys': unique_keys, 'drop_column_level': drop_column_level, 'pivot_kwargs': {'index': index, 'values': values, 'columns': columns, 'aggfunc': aggfunc, 'fill_value': fill_value, 'margins': margins, 'dropna': dropna, 'margins_name': margins_name, 'observed': observed, 'sort': sort}}
        try:
            return PivotTableImpl.map_reduce_impl(**kwargs)
        except NotImplementedError as e:
            message = f"Can't use MapReduce 'pivot_table' implementation because of: {e}" + '\nFalling back to a range-partitioning implementation.'
            get_logger().info(message)
        try:
            return PivotTableImpl.range_partition_impl(**kwargs)
        except NotImplementedError as e:
            message = f"Can't use range-partitioning 'pivot_table' implementation because of: {e}" + '\nFalling back to a full-axis implementation.'
            get_logger().info(message)
    ...
    def get_dummies(self, columns, **kwargs):
        ...
        if len(columns) == len(self.columns):
            new_modin_frame = self._modin_frame.apply_full_axis(0, map_fn, new_index=self.index, dtypes=bool)
            untouched_frame = None
        else:
            new_modin_frame = self._modin_frame.take_2d_labels_or_positional(col_labels=columns).apply_full_axis(0, map_fn, new_index=self.index, dtypes=bool)
            untouched_frame = self.drop(columns=columns)
        if len(columns) != len(self.columns):
            new_modin_frame = untouched_frame._modin_frame.concat(1, [new_modin_frame], how='left', sort=False)
        return self.__constructor__(new_modin_frame)
    ...
    def write_items(self, row_numeric_index, col_numeric_index, item, need_columns_reindex=True):
        ...
        if not is_scalar(item):
            broadcasted_item, broadcasted_dtypes = broadcast_item(self, row_numeric_index, col_numeric_index, item, need_columns_reindex=need_columns_reindex)
        else:
            broadcasted_item, broadcasted_dtypes = (item, pandas.Series([np.array(item).dtype] * len(col_numeric_index)))
        new_dtypes = None
        if isinstance(row_numeric_index, slice) and row_numeric_index == slice(None) and self._modin_frame.has_materialized_dtypes:
            new_dtypes = self.dtypes.copy()
            new_dtypes.iloc[col_numeric_index] = broadcasted_dtypes.values
        new_modin_frame = self._modin_frame.apply_select_indices(axis=None, func=iloc_mut, row_labels=row_numeric_index, col_labels=col_numeric_index, new_index=self.index, new_columns=self.columns, new_dtypes=new_dtypes, keep_remaining=True, item_to_distribute=broadcasted_item)
        return self.__constructor__(new_modin_frame)
    ...
    def sort_columns_by_row_values(self, rows, ascending=True, **kwargs):
        ...
        broadcast_value_list = [self.getitem_row_array([row]).to_pandas() for row in rows]
        index_builder = list(zip(broadcast_value_list, rows))
        broadcast_values = pandas.concat([row for row, idx in index_builder], copy=False)
        broadcast_values.columns = self.columns
        new_columns = broadcast_values.sort_values(by=rows, axis=1, ascending=ascending, **kwargs).columns
    ...
    def compare(self, other, **kwargs):
        return self.__constructor__(self._modin_frame.broadcast_apply_full_axis(0, lambda left, right: pandas.DataFrame.compare(left, other=right, **kwargs), other._modin_frame))

    def case_when(self, caselist):
        qc_type = type(self)
        caselist = [tuple((data._modin_frame if isinstance(data, qc_type) else data for data in case_tuple)) for case_tuple in caselist]
        return self.__constructor__(self._modin_frame.case_when(caselist), shape_hint=self._shape_hint)