...
class CythonCompiler:
    ...
    @classmethod
    async def creat(cls, obj: Type) -> "CythonCompiler":
        """
        ...
        """
        source_code: str = await cls._extract_source_code(
            obj
        )  # This is now awaited properly.
    ...
    @async_log_decorator
    def _extract_source_code(self, CythonCompiler: Type) -> str:
        """
        ...
        try:
            ...
            for _, member in members:
                if member is not obj:
                    source += self._extract_source_code(member)
            return source
