...
class DimensionDataBackupDriver(BackupDriver):
    ...
    def create_target_from_node(self, node, type=BackupTargetType.VIRTUAL, extra=None):
        ...
        :rtype: Instance of :class:`BackupTarget`
        """
        return self.create_target(name=node.name, address=node.id, type=BackupTargetType.VIRTUAL, extra=extra)
    ...
    def update_target(self, target, name=None, address=None, extra=None):
        ...
        self.connection.request_with_orgId_api_1('server/%s/backup/modify' % server_id, method='POST', data=ET.tostring(request)).object
    ...
    def _to_backup_details(self, object, target):
        return DimensionDataBackupDetails(asset_id=object.get('assetId'), service_plan=object.get('servicePlan'), status=object.get('state'), clients=self._to_clients(object, target))
    ...
    def _to_target(self, element):
        ...
        if len(backup) == 0:
            return
        extra = {'description': findtext(element, 'description', TYPES_URN), 'sourceImageId': findtext(element, 'sourceImageId', TYPES_URN), 'datacenterId': element.get('datacenterId'), 'deployedTime': findtext(element, 'createTime', TYPES_URN), 'servicePlan': backup[0].get('servicePlan')}
        n = BackupTarget(id=backup[0].get('assetId'), name=findtext(element, 'name', TYPES_URN), address=element.get('id'), driver=self.connection.driver, type=BackupTargetType.VIRTUAL, extra=extra)
        return n
