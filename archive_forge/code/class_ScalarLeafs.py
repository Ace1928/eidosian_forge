    ...
    def enter_Field(self, node, key, parent, path, ancestors):
        ...
        if is_leaf_type(get_named_type(type)):
            if node.selection_set:
                self.context.report_error(GraphQLError(self.no_subselection_allowed_message(node.name.value, type), [node.selection_set]))
        elif not node.selection_set:
            self.context.report_error(GraphQLError(self.required_subselection_message(node.name.value, type), [node]))
