...
class GUFunc(serialize.ReduceMixin):
    ...
    def __init__(self, py_func, signature, identity=None, cache=None, is_dynamic=False, targetoptions={}, writable_args=()):
        self.ufunc = None
        self._frozen = False
    ...
    def _rebuild(cls, py_func, signature, identity, cache, is_dynamic, targetoptions, writable_args, typesigs, frozen):
        self = cls(py_func=py_func, signature=signature, identity=identity, cache=cache, is_dynamic=is_dynamic, targetoptions=targetoptions, writable_args=writable_args)
        for sig in typesigs:
            self.add(sig)
        self.build_ufunc()
        self._frozen = frozen
    ...
    def __call__(self, *args, **kwargs):
        ...
        if self._num_args_match(*args) is False:
            msg = f"Too few arguments for function '{self.__name__}'. Note that the pattern `out = gufunc(Arg1, Arg2, ..., ArgN)` is not allowed. Use `gufunc(Arg1, Arg2, ..., ArgN, out) instead."
            raise TypeError(msg)
