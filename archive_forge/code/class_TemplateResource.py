...
class TemplateResource(stack_resource.StackResource):
    ...
    def get_schemas(tmpl, param_defaults):
        return (properties.Properties.schema_from_params(tmpl.param_schemata(param_defaults)), attributes.Attributes.schema_from_outputs(tmpl[tmpl.OUTPUTS]))
    ...
    def _generate_schema(self):
        ...
        self.properties_schema, self.attributes_schema = self.get_schemas(tmpl, self.stack.env.param_defaults)
            ...
            if val is not None:
                if pval.type() == properties.Schema.LIST:
                    if len(val) == 0:
                        ...
                        params[pname] = ','.join((x if x is not None else '' for x in val))
                else:
                    params[pname] = val
        return params
    ...
    def child_template(self):
        if not self._parsed_nested:
            self._parsed_nested = template_format.parse(self.template_data(), self.template_url)
    ...
    def validate(self):
        ...
        try:
            ...
            raise exception.StackValidationFailed(message=msg)
        try:
            fri = self.stack.env.get_resource_info(self.type(), resource_name=self.name, ignore=self.resource_info)
        except exception.EntityNotFound:
            pass
        else:
    ...
    def handle_adopt(self, resource_data=None):
        return self.create_with_template(self.child_template(), self.child_params(), adopt_data=resource_data)

    def handle_create(self):
        return self.create_with_template(self.child_template(), self.child_params())
    ...
    def handle_update(self, json_snippet, tmpl_diff, prop_diff):
        ...
        return self.update_with_template(self.child_template(), self.child_params())
    ...
    def get_reference_id(self):
        ...
        try:
            if self._outputs is not None:
                ...
                output = self.rpc_client().show_output(self.context, dict(stack_identity), STACK_ID_OUTPUT)
                if rpc_api.OUTPUT_ERROR in output:
                    raise exception.TemplateOutputError(resource=self.name, attribute=STACK_ID_OUTPUT, message=output[rpc_api.OUTPUT_ERROR])
