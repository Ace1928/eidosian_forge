...
class Routing_instances(ConfigBase):
    ...
    def execute_module(self):
        ...
        if state == 'gathered':
            ...
            result['parsed'] = self.get_routing_instances_facts(data=running_config)
    ...
    def _state_merged(self, want, have):
        ...
        for instance in want:
            ...
            if instance.get('description'):
                build_child_xml_node(rinst_node, 'description', instance['description'])
            if instance.get('instance_role'):
                build_child_xml_node(rinst_node, 'instance-role', instance['insatnce_role'])
            if instance.get('type'):
                build_child_xml_node(rinst_node, 'instance-type', instance['type'])
            if instance.get('interfaces'):
                ...
                for interface in interfaces:
                    ...
                    if interface.get('protect_interface'):
                        build_child_xml_node(int_node, 'protect-interface', interface['protect-interface'])
                    build_child_xml_node(int_node, 'name', interface['name'])
            if instance.get('l2vpn_id'):
                build_child_xml_node(rinst_node, 'l2vpn-id', instance['l2vpn_id'])
            ...
            if instance.get('route_distinguisher'):
                rd_instance = build_child_xml_node(rinst_node, 'route-distinguisher')
                build_child_xml_node(rd_instance, 'rd-type', instance.get('route_distinguisher'))
            ...
            if want:
                for instance in want:
                    if instance['name'] not in existing_rinsts:
                        continue
                    rinstance_node = build_root_xml_node('instance')
                    build_child_xml_node(rinstance_node, 'name', instance['name'])
                    rinstance_node.attrib.update(delete)
                    rinst_xml.append(rinstance_node)
            else:
                rinstance_node = build_root_xml_node('instance')
