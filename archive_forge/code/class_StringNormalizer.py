...
class StringNormalizer(OpRun):
    ...
    def _run(self, x, case_change_action=None, is_case_sensitive=None, locale=None, stopwords=None):
        slocale = locale
        ...
        if len(x.shape) == 2:
            for i in range(0, x.shape[1]):
                self._run_column(x[:, i], res[:, i], slocale=slocale, stops=stops, raw_stops=raw_stops, is_case_sensitive=is_case_sensitive, case_change_action=case_change_action)
        elif len(x.shape) == 1:
            self._run_column(x, res, slocale=slocale, stops=stops, raw_stops=raw_stops, is_case_sensitive=is_case_sensitive, case_change_action=case_change_action)
        else:
