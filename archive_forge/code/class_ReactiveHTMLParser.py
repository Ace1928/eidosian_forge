    ...
    def handle_starttag(self, tag, attrs):
        ...
        if not dom_id:
            for attr, value in attrs.items():
                ...
                if methods:
                    raise ValueError(f'DOM nodes with an attached callback must declare an id. Found <{tag}> node with the `{attr}` callback referencing the `{methods[0]}` method. Add an id attribute like this: <{tag} id="{tag}" {attr}="${{{methods[0]}}}>...</{tag}>.')
                elif params:
                    literal = value.replace(f'${{{params[0]}}}', f'{{{{{params[0]}}}}}')
                    raise ValueError(f'DOM node with a linked parameter declaration must declare an id. Found <{tag}> node with the `{attr}` attribute referencing the `{params[0]}` parameter. Either declare an id on the node, i.e. <{tag} id="{tag}" {attr}="{value}">...</{tag}>, or insert the value as a literal: <{tag} {attr}="{literal}">...</{tag}>.')
            return
    ...
    def handle_data(self, data):
        ...
        if nloops > 1 and nloops and self._open_for:
            ...
            if var in self.cls.param:
                raise ValueError(f'Loop variable {var} clashes with parameter name. Ensure loop variables have a unique name. Relevant template section:\n\n{data}')
            self.loop_map[var] = obj
        open_for = re.search('{%-? for', data)
        end_for = re.search(endfor, data)
        if open_for:
            if self._current_node is None:
                node = self._node_stack[-1][0]
                raise ValueError(f'Loops may only be used inside a DOM node with an assigned ID. The following loop could not be expanded because the <{node}> node did not have an assigned id:\n\n    {data.strip()}')
            self._open_for = True
        ...
        for match in matches:
            ...
            if match not in self.cls.param and '.' not in match:
                ...
                raise ValueError(f"{self.cls.__name__} HTML template references unknown parameter '{match}', similar parameters include {params}.")
