...
class PauliSumExponential:
    ...
    def __init__(self, pauli_sum_like: 'cirq.PauliSumLike', exponent: Union[int, float, sympy.Expr]=1, atol: float=1e-08):
        ...
        for pauli_string in pauli_sum:
            ...
            if abs(coeff.real) > atol and abs(coeff.imag) > atol or curr_multiplier != self._multiplier:
                raise ValueError(pauli_sum, 'PauliSum should be either hermitian or anti-hermitian.')
        if not self._multiplier:
    ...
    def _resolve_parameters_(self, resolver: 'cirq.ParamResolver', recursive: bool) -> 'PauliSumExponential':
        return PauliSumExponential(self._pauli_sum, exponent=protocols.resolve_parameters(self._exponent, resolver, recursive))
    ...
    def __iter__(self) -> Iterator['cirq.PauliStringPhasor']:
        for pauli_string in self._pauli_sum:
            ...
            if isinstance(theta, complex):
                theta = theta.real
            yield pauli_string_phasor.PauliStringPhasor(pauli_string.with_coefficient(1.0), exponent_neg=-theta, exponent_pos=theta)
