...
class KeystoneProject(resource.Resource):
    ...
    entity = 'projects'
    PROPERTIES = NAME, DOMAIN, DESCRIPTION, ENABLED, PARENT, TAGS = ('name', 'domain', 'description', 'enabled', 'parent', 'tags')
    properties_schema = {NAME: properties.Schema(properties.Schema.STRING, _('Name of keystone project.'), update_allowed=True), DOMAIN: properties.Schema(properties.Schema.STRING, _('Name or id of keystone domain.'), default='default', update_allowed=True, constraints=[constraints.CustomConstraint('keystone.domain')]), DESCRIPTION: properties.Schema(properties.Schema.STRING, _('Description of keystone project.'), default='', update_allowed=True), ENABLED: properties.Schema(properties.Schema.BOOLEAN, _('This project is enabled or disabled.'), default=True, update_allowed=True), PARENT: properties.Schema(properties.Schema.STRING, _('The name or ID of parent of this keystone project in hierarchy.'), support_status=support.SupportStatus(version='6.0.0'), constraints=[constraints.CustomConstraint('keystone.project')]), TAGS: properties.Schema(properties.Schema.LIST, _('A list of tags for labeling and sorting projects.'), support_status=support.SupportStatus(version='10.0.0'), default=[], update_allowed=True)}
    ATTRIBUTES = NAME_ATTR, PARENT_ATTR, DOMAIN_ATTR, ENABLED_ATTR, IS_DOMAIN_ATTR = ('name', 'parent_id', 'domain_id', 'enabled', 'is_domain')
    attributes_schema = {NAME_ATTR: attributes.Schema(_('Project name.'), support_status=support.SupportStatus(version='10.0.0'), type=attributes.Schema.STRING), PARENT_ATTR: attributes.Schema(_('Parent project id.'), support_status=support.SupportStatus(version='10.0.0'), type=attributes.Schema.STRING), DOMAIN_ATTR: attributes.Schema(_('Domain id for project.'), support_status=support.SupportStatus(version='10.0.0'), type=attributes.Schema.STRING), ENABLED_ATTR: attributes.Schema(_('Flag of enable project.'), support_status=support.SupportStatus(version='10.0.0'), type=attributes.Schema.BOOLEAN), IS_DOMAIN_ATTR: attributes.Schema(_('Indicates whether the project also acts as a domain.'), support_status=support.SupportStatus(version='10.0.0'), type=attributes.Schema.BOOLEAN)}
    ...
    def translation_rules(self, properties):
        return [translation.TranslationRule(properties, translation.TranslationRule.RESOLVE, [self.DOMAIN], client_plugin=self.client_plugin(), finder='get_domain_id'), translation.TranslationRule(properties, translation.TranslationRule.RESOLVE, [self.PARENT], client_plugin=self.client_plugin(), finder='get_project_id')]

    def client(self):
        return super(KeystoneProject, self).client().client

    def handle_create(self):
        project_name = self.properties[self.NAME] or self.physical_resource_name()
        ...
        project = self.client().projects.create(name=project_name, domain=domain, description=description, enabled=enabled, parent=parent, tags=tags)
    ...
    def handle_update(self, json_snippet, tmpl_diff, prop_diff):
        if prop_diff:
            ...
            tags = prop_diff.get(self.TAGS) or self.properties[self.TAGS]
            self.client().projects.update(project=self.resource_id, name=name, description=description, enabled=enabled, domain=domain, tags=tags)

    def parse_live_resource_data(self, resource_properties, resource_data):
        result = super(KeystoneProject, self).parse_live_resource_data(resource_properties, resource_data)
    ...
    def handle_delete(self):
        if self.resource_id:
            ...
            default_sec_groups = nclient.list_security_groups(project_id=self.resource_id, name=default_sec_group_name)['security_groups']
