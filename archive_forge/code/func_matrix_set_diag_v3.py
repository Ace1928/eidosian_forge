...
def matrix_set_diag_v3(input: _atypes.TensorFuzzingAnnotation[TV_MatrixSetDiagV3_T], diagonal: _atypes.TensorFuzzingAnnotation[TV_MatrixSetDiagV3_T], k: _atypes.TensorFuzzingAnnotation[_atypes.Int32], align: str='RIGHT_LEFT', name=None) -> _atypes.TensorFuzzingAnnotation[TV_MatrixSetDiagV3_T]:
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'MatrixSetDiagV3', name, input, diagonal, k, 'align', align)
            return _result
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('MatrixSetDiagV3', input=input, diagonal=diagonal, k=k, align=align, name=name)
    _result = _outputs[:]
