...
def _no_limit_offset(self, meth: str) -> None:
    ...
    if self._limit_clause is not None or self._offset_clause is not None:
        raise sa_exc.InvalidRequestError('Query.%s() being called on a Query which already has LIMIT or OFFSET applied.  Call %s() before limit() or offset() are applied.' % (meth, meth))