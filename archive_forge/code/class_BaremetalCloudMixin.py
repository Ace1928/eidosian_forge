...
class BaremetalCloudMixin:
    ...
    def update_machine(self, name_or_id, **attrs):
        ...
        if not machine:
            raise exceptions.SDKException('Machine update failed to find Machine: %s. ' % name_or_id)
        new_config = dict(machine._to_munch(), **attrs)
        try:
            patch = jsonpatch.JsonPatch.from_diff(machine._to_munch(), new_config)
        except Exception as e:
            raise exceptions.SDKException('Machine update failed - Error generating JSON patch object for submission to the API. Machine: %s Error: %s' % (name_or_id, e))
        if not patch:
    ...
    def validate_node(self, uuid):
        warnings.warn('validate_node is deprecated, please use validate_machine instead', os_warnings.OpenStackDeprecationWarning)
