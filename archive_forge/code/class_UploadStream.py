...
class UploadStream:
  ...
  """
    ...
    def __init__(self, stream, length=None, digesters=None, progress_callback=None):
    ...
    def _catch_up_digesters(self, new_absolute_index):
        ...
        if new_absolute_index < self._checkpoint_absolute_index:
            ...
            self._checkpoint_digesters = hash_util.copy_digesters(self._checkpoint_digesters)
        elif new_absolute_index == self._get_absolute_position():
            return
        self._save_digesters_checkpoint()
        while True:
            data = self._get_data(min(new_absolute_index - self._get_absolute_position(), installers.WRITE_BUFFER_SIZE))
            if not data:
                break
    ...
    """
        if whence == os.SEEK_END:
            if self._length:
                ...
                raise errors.Error('SEEK_END is not supported if the length of the stream is unknown.')
