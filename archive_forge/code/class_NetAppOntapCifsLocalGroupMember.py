...
class NetAppOntapCifsLocalGroupMember:
    ...
    def __init__(self):
        ...
        self.argument_spec.update(dict(state=dict(required=False, choices=['present', 'absent'], default='present'), vserver=dict(required=True, type='str'), group=dict(required=True, type='str'), member=dict(required=True, type='str')))
    ...
    def remove_cifs_local_group_member(self):
        ...
        if self.use_rest:
            ...
            if error:
                self.module.fail_json(msg='Error removing member %s from cifs local group %s on vserver %s: %s' % (self.parameters['member'], self.parameters['group'], self.parameters['vserver'], to_native(error)), exception=traceback.format_exc())
        else:
