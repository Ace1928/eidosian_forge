...
def qnode_spectrum(qnode, encoding_args=None, argnum=None, decimals=8, validation_kwargs=None):
    ...
    **Example**
    ...
    def wrapper(*args, **kwargs):
        ...
        jac_fn = qml.gradients.classical_jacobian(qnode, argnum=argnum, expand_fn=qml.transforms.expand_multipar)
        if not qml.math.is_independent(jac_fn, qnode.interface, args, kwargs, **validation_kwargs):
            raise ValueError('The Jacobian of the classical preprocessing in the provided QNode is not constant; only linear classical preprocessing is supported.')
        for m in qnode.qtape.measurements:
            if not isinstance(m, (qml.measurements.ExpectationMP, qml.measurements.ProbabilityMP)):
                raise ValueError(f'The measurement {m.__class__.__name__} is not supported as it likely does not admit a Fourier spectrum.')
        ...
        for jac_idx, cjac in enumerate(cjacs):
            ...
            for op_idx, jac_of_op in enumerate(np.round(cjac, decimals=decimals)):
                ...
                if len(op.parameters) != 1:
                    raise ValueError(f'Can only consider one-parameter gates as data-encoding gates; got {op.name}.')
