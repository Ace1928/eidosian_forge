...
class TrubricsCallbackHandler(BaseCallbackHandler):
    ...
    def __init__(self, project: str='default', email: Optional[str]=None, password: Optional[str]=None, **kwargs: Any) -> None:
        super().__init__()
        try:
            from trubrics import Trubrics
        except ImportError:
            raise ImportError('The TrubricsCallbackHandler requires installation of the trubrics package. Please install it with `pip install trubrics`.')
        self.trubrics = Trubrics(project=project, email=email or os.environ['TRUBRICS_EMAIL'], password=password or os.environ['TRUBRICS_PASSWORD'])
    ...
    def on_llm_end(self, response: LLMResult, run_id: UUID, **kwargs: Any) -> None:
        ...
        for generation in response.generations:
            self.trubrics.log_prompt(config_model={'model': response.llm_output.get('model_name') if response.llm_output else 'NA'}, prompt=self.prompt, generation=generation[0].text, user_id=user_id, session_id=session_id, tags=tags, metadata=metadata)