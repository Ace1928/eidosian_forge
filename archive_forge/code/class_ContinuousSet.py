...
@ModelComponentFactory.register('A bounded continuous numerical range optionally containing discrete points of interest.')
class ContinuousSet(SortedScalarSet):
    ...
    def set_changed(self, newvalue):
        ...
        if newvalue is not True and newvalue is not False:
            raise ValueError('The _changed attribute on a ContinuousSet may only be set to True or False')
    ...
    def construct(self, values=None):
        ...
        if None in self.bounds():
            raise ValueError("ContinuousSet '%s' must have at least two values indicating the range over which a differential equation is to be discretized" % self.name)
        if len(self) < 2:
            raise ValueError("ContinuousSet '%s' must have at least two values indicating the range over which a differential equation is to be discretized" % self.name)
        self._fe = list(self)
        timer.report()
    ...
    def find_nearest_index(self, target, tolerance=None):
        ...
        if i == lo:
            ...
            delta, nearest_index = min(((abs(target - self.at(j)), j) for j in [i, i + 1]))
