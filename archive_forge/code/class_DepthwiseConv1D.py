...
class DepthwiseConv1D(DepthwiseConv):
    ...
    def call(self, inputs):
        ...
        outputs = tf.nn.depthwise_conv2d(inputs, depthwise_kernel, strides=strides, padding=self.padding.upper(), dilations=dilation_rate, data_format=conv_utils.convert_data_format(self.data_format, ndim=4))
        if self.use_bias:
    ...
    def compute_output_shape(self, input_shape):
        ...
        input_dim = conv_utils.conv_output_length(input_dim, self.kernel_size[0], self.padding, self.strides[0], self.dilation_rate[0])
        if self.data_format == 'channels_first':
