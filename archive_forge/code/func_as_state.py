...
def as_state(self) -> History:
    return History([c is not None and instance_state(c) or None for c in self.added], [c is not None and instance_state(c) or None for c in self.unchanged], [c is not None and instance_state(c) or None for c in self.deleted])