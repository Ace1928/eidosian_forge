...
This module provides database functionalities for the Image Interconversion GUI application. It includes operations such as loading configurations, connecting to the database, executing queries, and managing image data (compression, encryption, storage, and retrieval).

The module utilizes asynchronous programming techniques with the aiosqlite library to interact with a SQLite database. It also incorporates encryption and compression mechanisms to securely store and retrieve image data.
...
async def load_database_configurations():
    ...
    Raises:
        Exception: If loading configurations fails for any reason.
    """
...
@asynccontextmanager
async def get_db_connection() -> asynccontextmanager:
    """
