...
class BlockStorageCloudMixin:
    ...
    def create_volume(self, size, wait=True, timeout=None, image=None, bootable=None, **kwargs):
        ...
        if image:
            ...
            if not image_obj:
                raise exceptions.SDKException('Image {image} was requested as the basis for a new volume, but was not found on the cloud'.format(image=image))
    ...
    def delete_volume(self, name_or_id=None, wait=True, timeout=None, force=False):
        ...
        if not volume:
            self.log.debug('Volume %(name_or_id)s does not exist', {'name_or_id': name_or_id}, exc_info=True)
            return False
        try:
    ...
    def detach_volume(self, server, volume, wait=True, timeout=None):
        ...
        :raises: :class:`~openstack.exceptions.SDKException` on operation
            error.
        """
        self.compute.delete_volume_attachment(server=server['id'], volume=volume['id'], ignore_missing=False)
        if wait:
    ...
    def attach_volume(self, server, volume, device=None, wait=True, timeout=None):
        ...
        if dev:
            raise exceptions.SDKException('Volume %s already attached to server %s on device %s' % (volume['id'], server['id'], dev))
        if volume['status'] != 'available':
            raise exceptions.SDKException("Volume %s is not available. Status is '%s'" % (volume['id'], volume['status']))
        payload = {}
        if device:
            payload['device'] = device
        attachment = self.compute.create_volume_attachment(server=server['id'], volume=volume['id'], **payload)
        if wait:
