...
class DR_VCTK(Dataset):
    ...
    def __init__(self, root: Union[str, Path], subset: str='train', *, download: bool=False, url: str=_URL) -> None:
        if subset not in _SUPPORTED_SUBSETS:
            raise RuntimeError(f"The subset '{subset}' does not match any of the supported subsets: {_SUPPORTED_SUBSETS}")
