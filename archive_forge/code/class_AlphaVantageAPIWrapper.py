...
class AlphaVantageAPIWrapper(BaseModel):
    ...
    def _get_exchange_rate(self, from_currency: str, to_currency: str) -> Dict[str, Any]:
        ...
        response = requests.get('https://www.alphavantage.co/query/', params={'function': 'CURRENCY_EXCHANGE_RATE', 'from_currency': from_currency, 'to_currency': to_currency, 'apikey': self.alphavantage_api_key})
        response.raise_for_status()
        data = response.json()
        if 'Error Message' in data:
            raise ValueError(f'API Error: {data['Error Message']}')
