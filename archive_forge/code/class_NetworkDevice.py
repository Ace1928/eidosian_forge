...
class NetworkDevice(object):
    ...
    def get_object_by_name(self, name):
        result = None
        try:
            items = self.dnac.exec(family='devices', function='get_device_list', params=self.get_all_params(name=name))
    ...
    def get_object_by_id(self, id):
        result = None
        try:
            items = self.dnac.exec(family='devices', function='get_device_by_id', params={'id': id})
            if isinstance(items, dict):
                if 'response' in items:
                    items = items.get('response')
    ...
    def requires_update(self, current_obj):
        ...
        obj_params = [('cliTransport', 'cliTransport'), ('computeDevice', 'computeDevice'), ('enablePassword', 'enablePassword'), ('extendedDiscoveryInfo', 'extendedDiscoveryInfo'), ('httpPassword', 'httpPassword'), ('httpPort', 'httpPort'), ('httpSecure', 'httpSecure'), ('httpUserName', 'httpUserName'), ('ipAddress', 'ipAddress'), ('merakiOrgId', 'merakiOrgId'), ('netconfPort', 'netconfPort'), ('serialNumber', 'serialNumber'), ('snmpAuthPassphrase', 'snmpAuthPassphrase'), ('snmpAuthProtocol', 'snmpAuthProtocol'), ('snmpMode', 'snmpMode'), ('snmpPrivPassphrase', 'snmpPrivPassphrase'), ('snmpPrivProtocol', 'snmpPrivProtocol'), ('snmpROCommunity', 'snmpROCommunity'), ('snmpRWCommunity', 'snmpRWCommunity'), ('snmpRetry', 'snmpRetry'), ('snmpTimeout', 'snmpTimeout'), ('snmpUserName', 'snmpUserName'), ('snmpVersion', 'snmpVersion'), ('type', 'type'), ('updateMgmtIPaddressList', 'updateMgmtIPaddressList'), ('userName', 'userName'), ('id', 'id'), ('cleanConfig', 'clean_config')]
        return any((not dnac_compare_equality2(current_obj.get(dnac_param), requested_obj.get(ansible_param)) for dnac_param, ansible_param in obj_params))

    def create(self):
        result = self.dnac.exec(family='devices', function='add_device', params=self.create_params(), op_modifies=True)
        return result
    ...
    def update(self):
        ...
        if isinstance(name, list) and len(name) > 0:
            name = name[0]
        result = None
        result = self.dnac.exec(family='devices', function='sync_devices', params=self.update_all_params(), op_modifies=True)
        return result
    ...
    def delete(self):
        ...
        result = self.dnac.exec(family='devices', function='delete_device_by_id', params=self.delete_by_id_params())
        return result