...
class TitanTakeoffEmbed(Embeddings):
    ...
    def __init__(self, base_url: str='http://localhost', port: int=3000, mgmt_port: int=3001, models: List[ReaderConfig]=[]):
        ...
        try:
            ...
            raise ImportError("takeoff-client is required for TitanTakeoff. Please install it with `pip install 'takeoff-client==0.4.0'`.")
    ...
    def _embed(self, input: Union[List[str], str], consumer_group: Optional[str]) -> dict:
        """Embed text.
        ...
        if not consumer_group:
            if len(self.embed_consumer_groups) == 1:
                ...
                raise MissingConsumerGroup('TakeoffEmbedding was initialized with multiple embedding readergroups, you must specify which one to use.')
            else:
                raise MissingConsumerGroup('You must specify what consumer group you want to send embeddingresponse to as TitanTakeoffEmbed was not initialized with an embedding reader.')
