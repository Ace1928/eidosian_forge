...
class WhatsAppChatLoader(BaseChatLoader):
    ...
    def __init__(self, path: str):
        ...
        """
        self.path = path
        ignore_lines = ['This message was deleted', '<Media omitted>', 'image omitted', 'Messages and calls are end-to-end encrypted. No one outside of this chat, not even WhatsApp, can read or listen to them.']
        self._ignore_lines = re.compile('(' + '|'.join(['\\u200E*' + line for line in ignore_lines]) + ')', flags=re.IGNORECASE)
        self._message_line_regex = re.compile('\\u200E*\\[?(\\d{1,2}/\\d{1,2}/\\d{2,4}, \\d{1,2}:\\d{2}:\\d{2} (?:AM|PM))\\]?[ \\u200E]*([^:]+): (.+)', flags=re.IGNORECASE)
        ...
        for line in chat_lines:
            ...
            if result:
                ...
                if not self._ignore_lines.match(text.strip()):
                    results.append(HumanMessage(role=sender, content=text, additional_kwargs={'sender': sender, 'events': [{'message_time': timestamp}]}))
            else:
