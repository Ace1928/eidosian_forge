...
class NetAppONTAPCifsShare:
    ...
    def __init__(self):
        ...
        partially_supported_rest_properties = [['continuously_available', (9, 10, 1)], ['namespace_caching', (9, 10, 1)], ['show_snapshot', (9, 10, 1)], ['allow_unencrypted_access', (9, 11)], ['browsable', (9, 13, 1)], ['show_previous_versions', (9, 13, 1)]]
        unsupported_rest_properties = ['share_properties', 'symlink_properties', 'vscan_fileop_profile']
        self.use_rest = self.rest_api.is_rest_supported_properties(self.parameters, unsupported_rest_properties, partially_supported_rest_properties)
        self.unsupported_zapi_properties = ['unix_symlink', 'access_based_enumeration', 'change_notify', 'encryption', 'home_directory', 'oplocks', 'continuously_available', 'show_snapshot', 'namespace_caching', 'allow_unencrypted_access', 'browsable', 'show_previous_versions']
        self.svm_uuid = None
    ...
    def get_cifs_share(self):
        ...
        if result.get_child_by_name('num-records') and int(result.get_child_content('num-records')) == 1:
            properties_list = []
            symlink_list = []
            cifs_attrs = result.get_child_by_name('attributes-list').get_child_by_name('cifs-share')
            ...
            return_value = {'share': cifs_attrs.get_child_content('share-name'), 'path': cifs_attrs.get_child_content('path'), 'share_properties': properties_list, 'symlink_properties': symlink_list}
    ...
    def get_cifs_share_rest(self):
        ...
        """
        options = {'svm.name': self.parameters.get('vserver'), 'name': self.parameters.get('name'), 'fields': 'svm.uuid,name,path,comment,unix_symlink,access_based_enumeration,change_notify,encryption,oplocks,'}
    ...
    def create_modify_body_rest(self, params=None):
        ...
        options = ['path', 'comment', 'unix_symlink', 'access_based_enumeration', 'change_notify', 'encryption', 'home_directory', 'oplocks', 'continuously_available', 'show_snapshot', 'namespace_caching', 'allow_unencrypted_access', 'browsable', 'show_previous_versions']
        for key in options:
            if key in params:
                body[key] = params[key]
        return body
    ...
    def apply(self):
        ...
        if not self.use_rest and cd_action is None and ('show-previous-versions' in self.parameters.get('share_properties', [])) and current and ('show_previous_versions' in current.get('share_properties', [])):
