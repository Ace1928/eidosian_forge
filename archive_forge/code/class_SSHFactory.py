...
class SSHFactory(protocol.Factory):
    ...
    def buildProtocol(self, addr):
        ...
        t.supportedPublicKeys = list(chain.from_iterable((key.supportedSignatureAlgorithms() for key in self.privateKeys.values())))
        if not self.primes:
            self._log.info('disabling non-fixed-group key exchange algorithms because we cannot find moduli file')
            t.supportedKeyExchanges = [kexAlgorithm for kexAlgorithm in t.supportedKeyExchanges if _kex.isFixedGroup(kexAlgorithm) or _kex.isEllipticCurve(kexAlgorithm)]
        return t
