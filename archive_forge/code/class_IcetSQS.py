...
class IcetSQS:
    ...
    """
    sqs_kwarg_names: dict[str, tuple[str, ...]] = {'monte_carlo': ('include_smaller_cells', 'pbc', 'T_start', 'T_stop', 'n_steps', 'optimality_weight', 'random_seed', 'tol'), 'enumeration': ('include_smaller_cells', 'pbc', 'optimality_weight', 'tol')}
    _sqs_kwarg_defaults: dict[str, Any] = {'optimality_weight': None, 'tol': 1e-05, 'include_smaller_cells': False, 'pbc': (True, True, True)}
    sqs_methods: tuple[str, ...] = ('enumeration', 'monte_carlo')

    def __init__(self, structure: Structure, scaling: int, instances: int | None, cluster_cutoffs: dict[int, float], sqs_method: str | None=None, sqs_kwargs: dict | None=None) -> None:
        """
        ...
        _ordered_structure.replace_species({species: dummy_comp for species in original_composition if species != dummy_comp})
        ...
        self.sqs_kwargs = {key: value for key, value in self.sqs_kwargs.items() if key in self.sqs_kwarg_names[sqs_method]}
        if sqs_method == 'monte_carlo':
            self.sqs_getter = self.monte_carlo_sqs_structures
            if self.sqs_kwargs.get('random_seed') is None:
                self.sqs_kwargs['random_seed'] = int(1000000.0 * time())
        elif sqs_method == 'enumeration':
            self.sqs_getter = self.enumerate_sqs_structures
        else:
            raise ValueError(f'Unknown sqs_method={sqs_method!r}! Must be one of {self.sqs_methods}')
        ...
        self.target_concentrations = _validate_concentrations(concentrations=self.composition, cluster_space=cluster_space)
        self.sqs_vector = _get_sqs_cluster_vector(cluster_space=cluster_space, target_concentrations=self.target_concentrations)
    ...
    def run(self) -> Sqs:
        ...
        return Sqs(bestsqs=sqs_structures[0]['structure'], objective_function=sqs_structures[0]['objective_function'], allsqs=sqs_structures, directory='./', clusters=str(self._get_cluster_space()))
