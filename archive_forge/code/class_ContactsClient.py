...
class ContactsClient:
    ...
    Returns:
      The created contact.
    """
        contact = self.contact_message(email=email, notificationCategorySubscriptions=notification_categories, languageTag=language_tag)
        args = {'parent': parent_name, self.contact_param_name: contact}
        if parent_name.startswith('folders'):
            create_req = self._messages.EssentialcontactsFoldersContactsCreateRequest(**args)
            return self._folders_service.Create(create_req)
        if parent_name.startswith('organizations'):
            create_req = self._messages.EssentialcontactsOrganizationsContactsCreateRequest(**args)
            return self._organizations_service.Create(create_req)
        create_req = self._messages.EssentialcontactsProjectsContactsCreateRequest(**args)
    ...
    """
        ...
        contact = self.contact_message(notificationCategorySubscriptions=notification_categories, languageTag=language_tag)
        args = {'name': contact_name, 'updateMask': update_mask, self.contact_param_name: contact}
        if contact_name.startswith('folders'):
            update_req = self._messages.EssentialcontactsFoldersContactsPatchRequest(**args)
            return self._folders_service.Patch(update_req)
        if contact_name.startswith('organizations'):
            update_req = self._messages.EssentialcontactsOrganizationsContactsPatchRequest(**args)
            return self._organizations_service.Patch(update_req)
        update_req = self._messages.EssentialcontactsProjectsContactsPatchRequest(**args)
    ...
    """
        if contact_name.startswith('folders'):
            delete_req = self._messages.EssentialcontactsFoldersContactsDeleteRequest(name=contact_name)
            return self._folders_service.Delete(delete_req)
        if contact_name.startswith('organizations'):
            delete_req = self._messages.EssentialcontactsOrganizationsContactsDeleteRequest(name=contact_name)
            return self._organizations_service.Delete(delete_req)
        delete_req = self._messages.EssentialcontactsProjectsContactsDeleteRequest(name=contact_name)
    ...
    """
        if contact_name.startswith('folders'):
            describe_req = self._messages.EssentialcontactsFoldersContactsGetRequest(name=contact_name)
            return self._folders_service.Get(describe_req)
        if contact_name.startswith('organizations'):
            describe_req = self._messages.EssentialcontactsOrganizationsContactsGetRequest(name=contact_name)
            return self._organizations_service.Get(describe_req)
        describe_req = self._messages.EssentialcontactsProjectsContactsGetRequest(name=contact_name)
    ...
    """
        ...
        if parent_name.startswith('folders'):
            ...
            list_req = self._messages.EssentialcontactsFoldersContactsListRequest(parent=parent_name)
        elif parent_name.startswith('organizations'):
            service = self._organizations_service
            list_req = self._messages.EssentialcontactsOrganizationsContactsListRequest(parent=parent_name)
        else:
            service = self._projects_service
            list_req = self._messages.EssentialcontactsProjectsContactsListRequest(parent=parent_name)
        return list_pager.YieldFromList(service, list_req, batch_size=page_size, limit=limit, field='contacts', batch_size_attribute='pageSize')

    def Compute(self, parent_name, notification_categories, page_size=50, limit=None):
    ...
    """
        ...
        if parent_name.startswith('folders'):
            service = self._folders_service
            compute_req = self._messages.EssentialcontactsFoldersContactsComputeRequest(parent=parent_name, notificationCategories=notification_categories)
        elif parent_name.startswith('organizations'):
            service = self._organizations_service
            compute_req = self._messages.EssentialcontactsOrganizationsContactsComputeRequest(parent=parent_name, notificationCategories=notification_categories)
        else:
            service = self._projects_service
            compute_req = self._messages.EssentialcontactsProjectsContactsComputeRequest(parent=parent_name, notificationCategories=notification_categories)
        return list_pager.YieldFromList(service, compute_req, batch_size=page_size, limit=limit, method='Compute', field='contacts', batch_size_attribute='pageSize')