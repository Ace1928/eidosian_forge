...
class SpatialDistortionIndex(Metric):
    ...
    def __init__(self, norm_order: int=1, window_size: int=7, reduction: Literal['elementwise_mean', 'sum', 'none']='elementwise_mean', **kwargs: Any) -> None:
        super().__init__(**kwargs)
        rank_zero_warn('Metric `SpatialDistortionIndex` will save all targets and predictions in buffer. For large datasets this may lead to large memory footprint.')
    ...
    def compute(self) -> Tensor:
        ...
        return _spatial_distortion_index_compute(preds, ms, pan, pan_lr, self.norm_order, self.window_size, self.reduction)
