...
class Flag(_Symbol):
    ...
    def demo(self):
        ...
        D.add(fx)
        labelFontSize = 10
        D.add(String(fx.x + fx.size / 2.0, fx.y - 1.2 * labelFontSize, name, fillColor=colors.black, textAnchor='middle', fontSize=labelFontSize))
        labelFontSize = int(fx.size / 4.0)
        D.add(String(fx.x + fx.size, fx.y + fx.size / 2.0, 'SAMPLE', fillColor=colors.gold, textAnchor='middle', fontSize=labelFontSize, fontName='Helvetica-Bold'))
        return D
