...
class DigitalOceanDNSDriver(DigitalOcean_v2_BaseDriver, DNSDriver):
    ...
    RECORD_TYPE_MAP = {RecordType.NS: 'NS', RecordType.A: 'A', RecordType.AAAA: 'AAAA', RecordType.CNAME: 'CNAME', RecordType.MX: 'MX', RecordType.TXT: 'TXT', RecordType.SRV: 'SRV'}
    ...
    def get_record(self, zone_id, record_id):
        ...
        :rtype: :class:`Record`
        """
        data = self.connection.request('/v2/domains/{}/records/{}'.format(zone_id, record_id)).object['domain_record']
    ...
    def create_zone(self, domain, type='master', ttl=None, extra=None):
        ...
        return Zone(id=res.object['domain']['name'], domain=res.object['domain']['name'], type='master', ttl=1800, driver=self, extra={})
    ...
    def create_record(self, name, zone, type, data, extra=None):
        ...
        res = self.connection.request('/v2/domains/%s/records' % zone.id, data=json.dumps(params), method='POST')
        return Record(id=res.object['domain_record']['id'], name=res.object['domain_record']['name'], type=type, data=data, zone=zone, ttl=res.object['domain_record'].get('ttl', None), driver=self, extra=extra)
    ...
    def update_record(self, record, name=None, type=None, data=None, extra=None):
        ...
        if extra:
            ...
            if 'ttl' in extra:
                params['ttl'] = extra['ttl']
        res = self.connection.request('/v2/domains/{}/records/{}'.format(record.zone.id, record.id), data=json.dumps(params), method='PUT')
        return Record(id=res.object['domain_record']['id'], name=res.object['domain_record']['name'], type=record.type, data=data, zone=record.zone, ttl=res.object['domain_record'].get('ttl', None), driver=self, extra=extra)
    ...
    def delete_record(self, record):
        ...
        """
        params = {}
        res = self.connection.request('/v2/domains/{}/records/{}'.format(record.zone.id, record.id), params=params, method='DELETE')
        return res.status == httplib.NO_CONTENT

    def _to_record(self, data, zone=None):
        extra = {'port': data['port'], 'priority': data['priority'], 'weight': data['weight']}
        return Record(id=data['id'], name=data['name'], type=self._string_to_record_type(data['type']), data=data['data'], zone=zone, ttl=data.get('ttl', None), driver=self, extra=extra)

    def _to_zone(self, data):
        extra = {'zone_file': data['zone_file']}
        return Zone(id=data['name'], domain=data['name'], type='master', ttl=data['ttl'], driver=self, extra=extra)