...
class NeutronSubnetTest(common.HeatTestCase):
    ...
    def setUp(self):
        ...
        self.create_mock = self.patchobject(neutronclient.Client, 'create_subnet')
        self.delete_mock = self.patchobject(neutronclient.Client, 'delete_subnet')
        self.show_mock = self.patchobject(neutronclient.Client, 'show_subnet')
        self.update_mock = self.patchobject(neutronclient.Client, 'update_subnet')
        self.patchobject(neutron.NeutronClientPlugin, 'has_extension', return_value=True)
        self.patchobject(openstacksdk.OpenStackSDKPlugin, 'find_network_segment', return_value='fc68ea2c-b60b-4b4f-bd82-94ec81110766')
        self.patchobject(neutronV20, 'find_resourceid_by_name_or_id', return_value='fc68ea2c-b60b-4b4f-bd82-94ec81110766')

    def create_subnet(self, t, stack, resource_name):
        resource_defns = stack.t.resource_definitions(stack)
        rsrc = subnet.Subnet('test_subnet', resource_defns[resource_name], stack)
        return rsrc

    def _setup_mock(self, stack_name=None, use_deprecated_templ=False, tags=None):
    ...
    def test_update_subnet_with_value_specs(self):
        update_props = {'subnet': {'name': 'mysubnet', 'value_specs': {'enable_dhcp': True}}}
    ...
    def test_subnet_with_segment(self):
        subnet_dict = {'subnet': {'allocation_pools': [{'start': '10.0.3.20', 'end': '10.0.3.150'}], 'host_routes': [{'destination': '10.0.4.0/24', 'nexthop': '10.0.3.20'}], 'segment_id': 'fc68ea2c-b60b-4b4f-bd82-94ec81110766', 'prefixlen': 24, 'dns_nameservers': ['8.8.8.8'], 'enable_dhcp': True, 'gateway_ip': '10.0.3.1', 'id': '91e47a57-7508-46fe-afc9-fc454e8580e1', 'ip_version': 4, 'name': 'name', 'network_id': 'fc68ea2c-b60b-4b4f-bd82-94ec81110766', 'tenant_id': 'c1210485b2424d48804aad5d39c61b8f'}}
        self.create_mock.return_value = subnet_dict
        self.show_mock.side_effect = [qe.NeutronClientException(status_code=404)]
        t = template_format.parse(neutron_template)
        del t['resources']['sub_net']['properties']['cidr']
        t['resources']['sub_net']['properties']['segment'] = 'fc68ea2c-b60b-4b4f-bd82-94ec81110766'
        t['resources']['sub_net']['properties']['prefixlen'] = 24
        t['resources']['sub_net']['properties']['name'] = 'mysubnet'
        stack = utils.parse_stack(t)
        self.patchobject(stack['net'], 'FnGetRefId', return_value='fc68ea2c-b60b-4b4f-bd82-94ec81110766')
    ...
    def test_null_gateway_ip(self):
        ...
        p = {'foo': 'bar', 'gateway_ip': '198.51.100.0'}
        subnet.Subnet._null_gateway_ip(p)
        self.assertEqual({'foo': 'bar', 'gateway_ip': '198.51.100.0'}, p)
        p = {'foo': 'bar', 'gateway_ip': ''}
        subnet.Subnet._null_gateway_ip(p)
        self.assertEqual({'foo': 'bar', 'gateway_ip': None}, p)
        p = {'foo': 'bar', 'gateway_ip': None}
        subnet.Subnet._null_gateway_ip(p)
        self.assertEqual({'foo': 'bar', 'gateway_ip': None}, p)
    ...
    def test_ipv6_validate_ra_mode(self):
        ...
        rsrc = stack['sub_net']
        ex = self.assertRaises(exception.StackValidationFailed, rsrc.validate)
        self.assertEqual('When both ipv6_ra_mode and ipv6_address_mode are set, they must be equal.', str(ex))
    ...
    def test_ipv6_validate_ip_version(self):
        ...
        rsrc = stack['sub_net']
        ex = self.assertRaises(exception.StackValidationFailed, rsrc.validate)
        self.assertEqual('ipv6_ra_mode and ipv6_address_mode are not supported for ipv4.', str(ex))
    ...
    def test_validate_none_subnetpool_cidr(self):
        ...
        self.patchobject(stack['net'], 'FnGetRefId', return_value='fc68ea2c-b60b-4b4f-bd82-94ec81110766')
        rsrc = stack['sub_net']
        ex = self.assertRaises(exception.PropertyUnspecifiedError, rsrc.validate)
        msg = 'At least one of the following properties must be specified: subnetpool, cidr.'
        self.assertEqual(msg, str(ex))
    ...
    def test_validate_subnetpool_ref_with_cidr(self):
        ...
        props = t['resources']['sub_net']['properties']
        stack = utils.parse_stack(t)
        snippet = rsrc_defn.ResourceDefinition('subnetpool', 'OS::Neutron::SubnetPool')
        res = resource.Resource('subnetpool', snippet, stack)
        stack.add_resource(res)
        self.patchobject(stack['subnetpool'], 'FnGetRefId', return_value=None)
        self.patchobject(stack['net'], 'FnGetRefId', return_value='fc68ea2c-b60b-4b4f-bd82-94ec81110766')
        rsrc = stack['sub_net']
        ex = self.assertRaises(exception.ResourcePropertyConflict, rsrc.validate)
        msg = 'Cannot define the following properties at the same time: subnetpool, cidr.'
