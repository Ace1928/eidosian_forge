...
class AzureNodeProvider(NodeProvider):
    ...
    def external_ip(self, node_id):
        ...
        ip = self._get_cached_node(node_id=node_id)['external_ip'] or self._get_node(node_id=node_id)['external_ip']
        return ip

    def internal_ip(self, node_id):
        """Returns the internal ip (Ray ip) of the given node."""
        ip = self._get_cached_node(node_id=node_id)['internal_ip'] or self._get_node(node_id=node_id)['internal_ip']
        return ip

    def create_node(self, node_config, tags, count):
        resource_group = self.provider_config['resource_group']
        if self.cache_stopped_nodes:
            VALIDITY_TAGS = [TAG_RAY_CLUSTER_NAME, TAG_RAY_NODE_KIND, TAG_RAY_LAUNCH_CONFIG, TAG_RAY_USER_NODE_TYPE]
            filters = {tag: tags[tag] for tag in VALIDITY_TAGS if tag in tags}
            reuse_nodes = self.stopped_nodes(filters)[:count]
            logger.info(f'Reusing nodes {list(reuse_nodes)}. To disable reuse, set `cache_stopped_nodes: False` under `provider` in the cluster configuration.')
            start = get_azure_sdk_function(client=self.compute_client.virtual_machines, function_name='start')
    ...
    def set_node_tags(self, node_id, tags):
        ...
        update = get_azure_sdk_function(client=self.compute_client.virtual_machines, function_name='update')
        update(resource_group_name=self.provider_config['resource_group'], vm_name=node_id, parameters={'tags': node_tags})
    ...
    def terminate_node(self, node_id):
        ...
        if self.cache_stopped_nodes:
            try:
                logger.info(f'Stopping instance {node_id}(to fully terminate instead, set `cache_stopped_nodes: False` under `provider` in the cluster configuration)')
                stop = get_azure_sdk_function(client=self.compute_client.virtual_machines, function_name='deallocate')
                stop(resource_group_name=resource_group, vm_name=node_id)
            except Exception as e:
                logger.warning('Failed to stop VM: {}'.format(e))
        else:
            vm = self.compute_client.virtual_machines.get(resource_group_name=resource_group, vm_name=node_id)
            disks = {d.name for d in vm.storage_profile.data_disks}
            disks.add(vm.storage_profile.os_disk.name)
            try:
                delete = get_azure_sdk_function(client=self.compute_client.virtual_machines, function_name='delete')
                delete(resource_group_name=resource_group, vm_name=node_id)
            except Exception as e:
                logger.warning('Failed to delete VM: {}'.format(e))
            try:
                delete = get_azure_sdk_function(client=self.network_client.network_interfaces, function_name='delete')
                delete(resource_group_name=resource_group, network_interface_name=metadata['nic_name'])
            except Exception as e:
                logger.warning('Failed to delete nic: {}'.format(e))
            if 'public_ip_name' in metadata:
                try:
                    delete = get_azure_sdk_function(client=self.network_client.public_ip_addresses, function_name='delete')
                    delete(resource_group_name=resource_group, public_ip_address_name=metadata['public_ip_name'])
                except Exception as e:
                    logger.warning('Failed to delete public ip: {}'.format(e))
            for disk in disks:
                try:
                    delete = get_azure_sdk_function(client=self.compute_client.disks, function_name='delete')
