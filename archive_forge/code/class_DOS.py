...
class DOS:
    ...
    def read_dos(self, method='Tetrahedron', pdos=False, atom_index=1, orbital='', spin_polarization=False):
        """
    ...
    def get_dos(self, atom_index_list=None, method='Tetrahedron', gaussian_width=0.1, pdos=False, orbital_list=None, spin_polarization=None, density=True, cum=False, erange=(-25, 20), file_format=None, atoms=None, fermi_level=True):
        """
        ...
        if pdos:
            ...
            for atom_index in atom_index_list:
                for orbital in orbital_list:
                    self.read_dos(method=method, atom_index=atom_index, pdos=pdos, orbital=orbital, spin_polarization=spin_polarization)
        else:
            self.read_dos(method=method, spin_polarization=spin_polarization)
        return self.plot_dos(density=density, cum=cum, atoms=atoms, atom_index_list=atom_index_list, pdos=pdos, orbital_list=orbital_list, erange=erange, spin_polarization=spin_polarization, file_format=file_format, method=method, fermi_level=fermi_level)