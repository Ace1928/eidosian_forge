  ...
  """
    ...
    """
        ...
        if not vars_with_grad:
            raise ValueError('No gradients provided for any variable, check your graph for ops that do not support gradients, between variables %s and loss %s.' % ([str(v) for _, v in grads_and_vars], loss))
        return self.apply_gradients(grads_and_vars, global_step=global_step, name=name)

    def compute_gradients(self, loss, var_list=None, gate_gradients=GATE_OP, aggregation_method=None, colocate_gradients_with_ops=False, grad_loss=None):
    ...
    def apply_gradients(self, grads_and_vars, global_step=None, name=None, skip_gradients_aggregation=False):
    ...
    """
        if distribute_lib.has_strategy() and (not skip_gradients_aggregation):
            if distribute_lib.in_cross_replica_context():
                raise RuntimeError('Use `_distributed_apply()` instead of `apply_gradients()` in a cross-replica context.')
            grads_and_vars = get_filtered_grad_fn(lambda: grads_and_vars)()
            return distribute_lib.get_replica_context().merge_call(self._distributed_apply, args=(grads_and_vars, global_step, name))
        ...
        if not var_list:
            raise ValueError('No gradients provided for any variable: %s.' % ([str(v) for _, v, _ in converted_grads_and_vars],))
        with ops.init_scope():
        ...
        with ops.name_scope(name, self._name, skip_on_eager=False) as name:
            ...
            for grad, var, processor in converted_grads_and_vars:
                if grad is None:
                    continue
                if context.executing_eagerly() or (resource_variable_ops.is_resource_variable(var) and (not var._in_graph_mode)):
                    scope_name = ''
                else:
                    scope_name = var.op.name
                with ops.name_scope('update_' + scope_name, skip_on_eager=False), ops.colocate_with(var):
                ...
                with ops.control_dependencies([self._finish(update_ops, 'update')]):
                    with ops.colocate_with(global_step):
                        if isinstance(global_step, resource_variable_ops.BaseResourceVariable):
                            apply_updates = resource_variable_ops.assign_add_variable_op(global_step.handle, ops.convert_to_tensor(1, dtype=global_step.dtype), name=name)
                        else:
    ...
    def _get_or_make_slot_with_initializer(self, var, initializer, shape, dtype, slot_name, op_name):
