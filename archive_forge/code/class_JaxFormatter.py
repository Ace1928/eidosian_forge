...
class JaxFormatter(TensorFormatter[Mapping, 'jax.Array', Mapping]):
    ...
    def __init__(self, features=None, device=None, **jnp_array_kwargs):
        ...
        if isinstance(device, Device):
            raise ValueError(f'Expected {device} to be a `str` not {type(device)}, as `jaxlib.xla_extension.Device` is not serializable neither with `pickle` nor with `dill`. Instead you can surround the device with `str()` to get its string identifier that will be internally mapped to the actual `jaxlib.xla_extension.Device`.')
        ...
        if self.device not in list(DEVICE_MAPPING.keys()):
            logger.warning(f'Device with string identifier {self.device} not listed among the available devices: {list(DEVICE_MAPPING.keys())}, so falling back to the default device: {str(jax.devices()[0])}.')
    ...
    def _consolidate(self, column):
        ...
        if isinstance(column, list) and column:
            if all((isinstance(x, jax.Array) and x.shape == column[0].shape and (x.dtype == column[0].dtype) for x in column)):
