...
class TkTerm:
    ...
    def __init__(self, shell, name='TkTerm'):
        ...
        self.text = text = Tk_.Text(frame, width=85, height=24, foreground='Black', borderwidth=3, background='#ec0fffec0', insertbackground='#ff0000', highlightthickness=0, relief=Tk_.FLAT)
        ...
        if keysym == 'Right':
            ...
            if insert_pos < self._prompt_size:
                ...
                if self.text.compare(Tk_.INSERT, '>=', '%s-2c' % Tk_.END):
                    ...
                    self.text.mark_set(Tk_.INSERT, '%d.%d' % (insert_line + 1, self._prompt_size))
                return 'break'
            return
        if event.state & 4:
            if keysym == 'c':
    ...
    def write(self, string, style=('output',), advance=True, mark='output_end', see=True):
        """
