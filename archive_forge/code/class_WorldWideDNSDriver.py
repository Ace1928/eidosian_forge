...
class WorldWideDNSDriver(DNSDriver):
    ...
    RECORD_TYPE_MAP = {RecordType.MX: 'MX', RecordType.CNAME: 'CNAME', RecordType.A: 'A', RecordType.NS: 'NS', RecordType.SRV: 'SRV', RecordType.TXT: 'TXT'}

    def __init__(self, key, secret=None, reseller_id=None, secure=True, host=None, port=None, **kwargs):
        """
    ...
    def get_record(self, zone_id, record_id):
        ...
        try:
            if int(record_id) not in range(1, MAX_RECORD_ENTRIES + 1):
                raise RecordDoesNotExistError(value="Record doesn't exists", driver=zone.driver, record_id=record_id)
        except ValueError:
    ...
    def update_zone(self, zone, domain, type='master', ttl=None, extra=None, ex_raw=False):
        ...
        if ex_raw:
            ...
            method = 'GET'
        response = self.connection.request(action, params=params, method=method)
    ...
    def update_record(self, record, name, type, data, extra=None):
        ...
        if type not in self.RECORD_TYPE_MAP:
            raise RecordError(value='Record type is not allowed', driver=record.zone.driver, record_id=name)
        zone = record.zone
        extra = {'S%s' % record_id: name, 'T%s' % record_id: type, 'D%s' % record_id: data}
    ...
    def create_record(self, name, zone, type, data, extra=None):
        ...
        if name == '':
            name = '@'
        if type not in self.RECORD_TYPE_MAP:
            raise RecordError(value='Record type is not allowed', driver=zone.driver, record_id=record_id)
        extra = {'S%s' % record_id: name, 'T%s' % record_id: type, 'D%s' % record_id: data}
        zone = self.update_zone(zone, zone.domain, extra=extra)
        record = self.get_record(zone.id, record_id)
        return record
    ...
    def _to_zone(self, line):
        ...
        if data[1] == 'P':
            ...
            extra = {'HOSTMASTER': soa_block[0], 'REFRESH': soa_block[1], 'RETRY': soa_block[2], 'EXPIRE': soa_block[3], 'SECURE': soa_block[5]}
            ttl = soa_block[4]
