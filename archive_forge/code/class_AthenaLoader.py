...
class AthenaLoader(BaseLoader):
    ...
    def __init__(self, query: str, database: str, s3_output_uri: str, profile_name: str, metadata_columns: Optional[List[str]]=None):
    ...
    def _execute_query(self) -> List[Dict[str, Any]]:
        response = self.athena_client.start_query_execution(QueryString=self.query, QueryExecutionContext={'Database': self.database}, ResultConfiguration={'OutputLocation': self.s3_output_uri})
    ...
    def _get_result_set(self, query_execution_id: str) -> Any:
        try:
            ...
            raise ModuleNotFoundError('Could not import pandas python package. Please install it with `pip install pandas`.')
