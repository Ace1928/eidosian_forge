...
class ISIS_View(object):
    ...
    def __init_module__(self):
        """init module"""
        mutually_exclusive = [['stdlevel1cost', 'stdlevel2cost'], ['aclnum_or_name', 'ip_prefix_name', 'import_routepolicy_name'], ['export_aclnumorname', 'import_ipprefix', 'import_routepolicyname']]
        required_together = [('ip_address', 'weight')]
        self.module = AnsibleModule(argument_spec=self.spec, mutually_exclusive=mutually_exclusive, required_together=required_together, supports_check_mode=True)
    ...
    def get_isis_dict(self):
        ...
        if self.netentity:
            conf_str = CE_NC_GET_ISIS % (CE_NC_GET_ISIS_ENTITY % self.instance_id)
        if self.route_policy_name or self.preference_value:
            conf_str = CE_NC_GET_ISIS % (CE_NC_GET_ISIS_PREFERENCE % self.instance_id)
        if self.max_load:
            conf_str = CE_NC_GET_ISIS % (CE_NC_GET_ISIS_MAXLOAD % self.instance_id)
        if self.ip_address:
            conf_str = CE_NC_GET_ISIS % (CE_NC_GET_ISIS_NEXTHOP % self.instance_id)
        if self.penetration_direct and self.penetration_direct == 'level2-level1':
            conf_str = CE_NC_GET_ISIS % (CE_NC_GET_ISIS_LEAKROUTELEVEL2 % self.instance_id)
        elif self.penetration_direct and self.penetration_direct == 'level1-level2':
            conf_str = CE_NC_GET_ISIS % (CE_NC_GET_ISIS_LEAKROUTELEVEL1 % self.instance_id)
        elif self.defaultmode:
            conf_str = CE_NC_GET_ISIS % (CE_NC_GET_ISIS_DEFAULTROUTE % self.instance_id)
        elif self.protocol:
            conf_str = CE_NC_GET_ISIS % (CE_NC_GET_ISIS_IMPORTROUTE % self.instance_id)
        elif self.export_protocol:
            conf_str = CE_NC_GET_ISIS % (CE_NC_GET_ISIS_EXPORTROUTE % self.instance_id)
        elif self.bfd_min_rx or self.bfd_min_tx or self.bfd_multiplier_num:
            conf_str = CE_NC_GET_ISIS % (CE_NC_GET_ISIS_BFDLINK % self.instance_id)
        elif self.import_aclnumorname or self.import_ipprefix or self.import_ipprefix:
            conf_str = CE_NC_GET_ISIS % (CE_NC_GET_ISIS_IMPORTIPROUTE % self.instance_id)
        ...
        if self.netentity:
            ...
            glb = root.find('isiscomm/isSites/isSite/isSiteMTs/isSiteMT/isLeakRouteLevel1ToLevel2s/isLeakRouteLevel1ToLevel2')
        elif self.defaultmode:
            glb = root.find('isiscomm/isSites/isSite/isSiteMTs/isSiteMT/isDefaultRoutes/isDefaultRoute')
        elif self.protocol:
            glb = root.find('isiscomm/isSites/isSite/isSiteMTs/isSiteMT/isImportRoutes/isImportRoute')
        elif self.export_protocol:
            glb = root.find('isiscomm/isSites/isSite/isSiteMTs/isSiteMT/isFilterExports/isFilterExport')
        elif self.bfd_min_rx or self.bfd_min_tx or self.bfd_multiplier_num:
            glb = root.find('isiscomm/isSites/isSite/isSiteMTs/isSiteMT')
        elif self.import_aclnumorname or self.import_ipprefix or self.import_ipprefix:
            glb = root.find('isiscomm/isSites/isSite/isSiteMTs/isSiteMT/isFilterImports/isFilterImport')
        else:
    ...
    def config_session(self):
        ...
        elif self.netentity or self.preference_value or self.route_policy_name or self.max_load or self.ip_address or self.defaultmode or self.bfd_min_rx or self.bfd_min_tx or (self.bfd_multiplier_num is not None):
            return xml_str
        else:
