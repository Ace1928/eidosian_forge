...
def create_tensor_wave(signal: np.ndarray, sr: int) -> dict:
    ...
    print("Ensuring tensor shapes are suitable for image data")
    for key in tensor_wave_dict:
        if tensor_wave_dict[key].ndim == 1:
            tensor_wave_dict[key] = tensor_wave_dict[key].reshape(1, -1)
        elif tensor_wave_dict[key].ndim == 2:
            tensor_wave_dict[key] = tensor_wave_dict[key].reshape(
                1, *tensor_wave_dict[key].shape
            )
        logging.info(f"{key} reshaped to {tensor_wave_dict[key].shape}")
        print(f"{key} reshaped to {tensor_wave_dict[key].shape}")
