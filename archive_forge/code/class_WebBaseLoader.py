...
class WebBaseLoader(BaseLoader):
    ...
    def __init__(self, web_path: Union[str, Sequence[str]]='', header_template: Optional[dict]=None, verify_ssl: bool=True, proxies: Optional[dict]=None, continue_on_failure: bool=False, autoset_encoding: bool=True, encoding: Optional[str]=None, web_paths: Sequence[str]=(), requests_per_second: int=2, default_parser: str='html.parser', requests_kwargs: Optional[Dict[str, Any]]=None, raise_for_status: bool=False, bs_get_text_kwargs: Optional[Dict[str, Any]]=None, bs_kwargs: Optional[Dict[str, Any]]=None, session: Any=None) -> None:
        ...
        if web_path and web_paths:
            raise ValueError('Received web_path and web_paths. Only one can be specified. web_path is deprecated, web_paths should be used.')
        if web_paths:
            ...
            raise TypeError(f'web_path must be str or Sequence[str] got ({type(web_path)}) or web_paths must be Sequence[str] got ({type(web_paths)})')
        ...
        if session:
            ...
            if not header_template.get('User-Agent'):
                try:
                    ...
                    logger.info('fake_useragent not found, using default user agent.To get a realistic header for requests, `pip install fake_useragent`.')
    ...
    async def _fetch(self, url: str, retries: int=3, cooldown: int=2, backoff: float=1.5) -> str:
        async with aiohttp.ClientSession() as session:
            for i in range(retries):
                try:
                    async with session.get(url, headers=self.session.headers, ssl=None if self.session.verify else False, cookies=self.session.cookies.get_dict()) as response:
        ...
        async with semaphore:
            try:
                ...
                if self.continue_on_failure:
                    ...
                    return ''
                logger.exception(f'Error fetching {url} and aborting, use continue_on_failure=True to continue loading urls after encountering an error.')
                raise e
