...
def generate_new_hardware_dict(array):
    hw_info = {'fans': {}, 'controllers': {}, 'temps': {}, 'drives': {}, 'interfaces': {}, 'power': {}, 'chassis': {}, 'tempatures': {}}
    ...
    for component in range(0, len(components)):
        ...
        if components[component].type == 'chassis':
            hw_info['chassis'][component_name] = {'status': components[component].status, 'serial': components[component].serial, 'model': components[component].model, 'identify_enabled': components[component].identify_enabled}
        if components[component].type == 'controller':
            hw_info['controllers'][component_name] = {'status': components[component].status, 'serial': components[component].serial, 'model': components[component].model, 'identify_enabled': components[component].identify_enabled}
        if components[component].type == 'cooling':
            hw_info['fans'][component_name] = {'status': components[component].status}
        if components[component].type == 'temp_sensor':
            hw_info['controllers'][component_name] = {'status': components[component].status, 'temperature': components[component].temperature}
        if components[component].type == 'drive_bay':
            hw_info['drives'][component_name] = {'status': components[component].status, 'identify_enabled': components[component].identify_enabled, 'serial': getattr(components[component], 'serial', None)}
        if components[component].type in ['sas_port', 'fc_port', 'eth_port', 'ib_port']:
            hw_info['interfaces'][component_name] = {'type': components[component].type, 'status': components[component].status, 'speed': components[component].speed, 'connector_type': None, 'rx_los': None, 'rx_power': None, 'static': {}, 'temperature': None, 'tx_bias': None, 'tx_fault': None, 'tx_power': None, 'voltage': None}
        if components[component].type == 'power_supply':
            hw_info['power'][component_name] = {'status': components[component].status, 'voltage': components[component].voltage, 'serial': components[component].serial, 'model': components[component].model}
    drives = list(array.get_drives().items)
    for drive in range(0, len(drives)):
        drive_name = drives[drive].name
        hw_info['drives'][drive_name] = {'capacity': drives[drive].capacity, 'status': drives[drive].status, 'protocol': getattr(drives[drive], 'protocol', None), 'type': drives[drive].type}
