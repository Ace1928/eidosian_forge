...
class ORTTrainer(Trainer):
    ...
    def __init__(self, model: Union[PreTrainedModel, nn.Module]=None, args: ORTTrainingArguments=None, data_collator: Optional[DataCollator]=None, train_dataset: Optional[Dataset]=None, eval_dataset: Optional[Union[Dataset, Dict[str, Dataset]]]=None, tokenizer: Optional[PreTrainedTokenizerBase]=None, model_init: Optional[Callable[[], PreTrainedModel]]=None, compute_metrics: Optional[Callable[[EvalPrediction], Dict]]=None, callbacks: Optional[List[TrainerCallback]]=None, optimizers: Tuple[torch.optim.Optimizer, torch.optim.lr_scheduler.LambdaLR]=(None, None), preprocess_logits_for_metrics: Optional[Callable[[torch.Tensor, torch.Tensor], torch.Tensor]]=None):
    ...
    def train(self, resume_from_checkpoint: Optional[Union[str, bool]]=None, trial: Union['optuna.Trial', Dict[str, Any]]=None, ignore_keys_for_eval: Optional[List[str]]=None, **kwargs):
        ...
        if not is_onnxruntime_training_available():
            raise ImportError('You need to install `onnxruntime-training` to use `ORTTrainer` for training. Check out https://huggingface.co/docs/optimum/onnxruntime/usage_guides/trainer#install-onnx-runtime.')
        ...
        if resume_from_checkpoint is not None and (not is_sagemaker_mp_enabled()) and (not self.is_deepspeed_enabled) and (not self.is_fsdp_enabled):
    ...
    def _inner_training_loop(self, batch_size=None, args=None, resume_from_checkpoint=None, trial=None, ignore_keys_for_eval=None):
        ...
        if self.is_deepspeed_enabled:
            if is_deepspeed_zero3_enabled():
                raise NotImplementedError('`ORTTrainer` does not support ZeRO stage 3 for the moment. Please use DeepSpeed stage 1 or 2 instead.')
            if args.bf16:
                warnings.warn("ONNX Runtime doesn't support BF16 when executing some operators. The execution will fail if there are any op which doesn't support BF16 in the IR.", RuntimeWarning)
            self.model = model
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if self.control.should_epoch_stop or self.control.should_training_stop:
                    break
            if step < 0:
                logger.warning(f"There seems to be not a single sample in your train dataloader, stopping training at step {self.state.global_step}! This is expected if you're using an IterableDataset and set num_steps ({max_steps}) higher than the number of available samples.")
                self.control.should_training_stop = True
            self.control = self.callback_handler.on_epoch_end(args, self.state, self.control)
            self._maybe_log_save_evaluate(tr_loss, model, trial, epoch, ignore_keys_for_eval)
            if DebugOption.TPU_METRICS_DEBUG in self.args.debug:
                logger.warning('You enabled PyTorch/XLA debug metrics which is not supported by ONNX Runtime. Check your training configuration if this is unexpected.')
    ...
    def _wrap_model(self, model, training=True, dataloader=None):
        ...
        if is_sagemaker_mp_enabled():
            raise NotImplementedError("Sagemaker's distrubuted data parallel features are not supported by `ORTTrainer`.")
        ...
        if self.is_fsdp_xla_enabled:
            ...
            raise NotImplementedError("Sagemaker's distrubuted data parallel features are not supported by `ORTTrainer` yet.")
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            optimizer_grouped_parameters = [{'params': [p for n, p in opt_model.named_parameters() if n in decay_parameters], 'weight_decay': self.args.weight_decay}, {'params': [p for n, p in opt_model.named_parameters() if n not in decay_parameters], 'weight_decay': 0.0}]
        ...
        if is_sagemaker_mp_enabled():
            raise NotImplementedError("Sagemaker's distributed data parallel features are not supported by `ORTTrainer` yet.")
