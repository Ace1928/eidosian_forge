...
class ClcModifyServer:
    ...
    def process_request(self):
        ...
        if state == 'absent' and (cpu or memory):
            return self.module.fail_json(msg="'absent' state is not supported for 'cpu' and 'memory' arguments")
        server_ids = p['server_ids']
        if not isinstance(server_ids, list):
            return self.module.fail_json(msg='server_ids needs to be a list of instances to modify: %s' % server_ids)
        changed, server_dict_array, changed_server_ids = self._modify_servers(server_ids=server_ids)
        self.module.exit_json(changed=changed, server_ids=changed_server_ids, servers=server_dict_array)
    ...
    def _define_module_argument_spec():
        ...
        """
        argument_spec = dict(server_ids=dict(type='list', required=True, elements='str'), state=dict(default='present', choices=['present', 'absent']), cpu=dict(), memory=dict(), anti_affinity_policy_id=dict(), anti_affinity_policy_name=dict(), alert_policy_id=dict(), alert_policy_name=dict(), wait=dict(type='bool', default=True))
        mutually_exclusive = [['anti_affinity_policy_id', 'anti_affinity_policy_name'], ['alert_policy_id', 'alert_policy_name']]
    ...
    def _wait_for_requests(module, request_list):
        ...
        if wait:
            ...
            if failed_requests_count > 0:
                module.fail_json(msg='Unable to process modify server request')
    ...
    def _delete_aa_policy(clc, module, acct_alias, server_id):
        ...
        if not module.check_mode:
            try:
                result = clc.v2.API.Call('DELETE', 'servers/%s/%s/antiAffinityPolicy' % (acct_alias, server_id), json.dumps({}))
            except APIFailedResponse as ex:
                module.fail_json(msg='Unable to delete anti affinity policy to server : "{0}". {1}'.format(server_id, str(ex.response_text)))
        return result
    ...
    def _get_aa_policy_id_of_server(clc, module, alias, server_id):
        ...
        aa_policy_id = None
        try:
            result = clc.v2.API.Call(method='GET', url='servers/%s/%s/antiAffinityPolicy' % (alias, server_id))
            aa_policy_id = result.get('id')
        except APIFailedResponse as ex:
            if ex.response_status_code != 404:
                module.fail_json(msg='Unable to fetch anti affinity policy for server "{0}". {1}'.format(server_id, str(ex.response_text)))
        return aa_policy_id

    def _ensure_alert_policy_present(self, server, server_params):
        """
        ...
        if alert_policy_id and (not self._alert_policy_exists(server, alert_policy_id)):
            self._add_alert_policy_to_server(self.clc, self.module, acct_alias, server.id, alert_policy_id)
            changed = True
        return changed
    ...
    def _add_alert_policy_to_server(clc, module, acct_alias, server_id, alert_policy_id):
        ...
        if not module.check_mode:
            try:
                result = clc.v2.API.Call('POST', 'servers/%s/%s/alertPolicies' % (acct_alias, server_id), json.dumps({'id': alert_policy_id}))
            except APIFailedResponse as ex:
                module.fail_json(msg='Unable to set alert policy to the server : "{0}". {1}'.format(server_id, str(ex.response_text)))
        return result
    ...
    def _get_alert_policy_id_by_name(clc, module, alias, alert_policy_name):
        ...
        try:
            alert_policies = clc.v2.API.Call(method='GET', url='alertPolicies/%s' % alias)
        except APIFailedResponse as ex:
            return module.fail_json(msg='Unable to fetch alert policies for account : "{0}". {1}'.format(alias, str(ex.response_text)))
