...
class AttrTree:
    ...
    def filter(self, path_filters):
        ...
        if not path_filters:
            return self
        path_filters = [tuple(pf.split('.')) if not isinstance(pf, tuple) else pf for pf in path_filters]
    ...
    def _propagate(self, path, val):
        ...
        if val == '_DELETE':
            if path in self.data:
                ...
                items = [(key, v) for key, v in self.data.items() if not all((k == p for k, p in zip(key, path)))]
