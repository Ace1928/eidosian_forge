...
class NLPCloud(LLM):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        values['nlpcloud_api_key'] = convert_to_secret_str(get_from_dict_or_env(values, 'nlpcloud_api_key', 'NLPCLOUD_API_KEY'))
        try:
            import nlpcloud
            values['client'] = nlpcloud.Client(values['model_name'], values['nlpcloud_api_key'].get_secret_value(), gpu=values['gpu'], lang=values['lang'])
        except ImportError:
    ...
    def _default_params(self) -> Mapping[str, Any]:
        ...
        return {'temperature': self.temperature, 'max_length': self.max_length, 'length_no_input': self.length_no_input, 'remove_input': self.remove_input, 'remove_end_sequence': self.remove_end_sequence, 'bad_words': self.bad_words, 'top_p': self.top_p, 'top_k': self.top_k, 'repetition_penalty': self.repetition_penalty, 'num_beams': self.num_beams, 'num_return_sequences': self.num_return_sequences}
