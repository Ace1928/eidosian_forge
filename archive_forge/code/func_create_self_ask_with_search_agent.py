...
def create_self_ask_with_search_agent(llm: BaseLanguageModel, tools: Sequence[BaseTool], prompt: BasePromptTemplate) -> Runnable:
    ...
    agent = RunnablePassthrough.assign(agent_scratchpad=lambda x: format_log_to_str(x['intermediate_steps'], observation_prefix='\nIntermediate answer: ', llm_prefix=''), chat_history=lambda x: x.get('chat_history', '')) | prompt | llm_with_stop | SelfAskOutputParser()
    return agent