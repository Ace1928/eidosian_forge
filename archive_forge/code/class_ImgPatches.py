...
class ImgPatches(NNGraph):
    ...
    def __init__(self, img, patch_shape=(3, 3), **kwargs):
        ...
        try:
            ...
            r = patch_shape[0]
            c = r
        pad_width = [(int((r - 0.5) / 2.0), int((r + 0.5) / 2.0)), (int((c - 0.5) / 2.0), int((c + 0.5) / 2.0))]
        if d == 0:
            window_shape = (r, c)
            d = 1
        else:
        ...
        try:
            ...
            raise ImportError('Cannot import skimage, which is needed to extract patches. Try to install it with pip (or conda) install scikit-image.')
        patches = skimage.util.view_as_windows(img, window_shape=window_shape)
        patches = patches.reshape((h * w, r * c * d))
        super(ImgPatches, self).__init__(patches, gtype='patch-graph', **kwargs)