...
def _build_extend_area_to_line_axis1_x_constant(draw_trapezoid_y, expand_aggs_and_cols):
    ...
    def perform_extend(i, j, sx, tx, sy, ty, xmin, xmax, ymin, ymax, xs, ys0, ys1, *aggs_and_cols):
        ...
        y2 = ys1[i, j + 1]
        y3 = ys0[i, j + 1]
        trapezoid_start = j == 0 or isnull(xs[j - 1]) or isnull(ys0[i, j - 1]) or isnull(ys1[i, j - 1])
        stacked = True
