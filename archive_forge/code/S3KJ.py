...
def cache_result(func: DecoratedCallable, config: DecoratorConfig) -> DecoratedCallable:
    ...
    async def wrapper(*args, **kwargs) -> T:
        ...
        try:
            try:
                # Ensure the function and its arguments are correctly passed to asyncio.to_thread
                # Inside the cache_result decorator, replace the problematic line with:
                cached_result = await asyncio.to_thread(
                    lambda: config.cache.get(cache_key)
                )
