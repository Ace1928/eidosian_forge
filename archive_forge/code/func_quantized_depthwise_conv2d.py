...
def quantized_depthwise_conv2d(input: _atypes.TensorFuzzingAnnotation[TV_QuantizedDepthwiseConv2D_Tinput], filter: _atypes.TensorFuzzingAnnotation[TV_QuantizedDepthwiseConv2D_Tfilter], min_input: _atypes.TensorFuzzingAnnotation[_atypes.Float32], max_input: _atypes.TensorFuzzingAnnotation[_atypes.Float32], min_filter: _atypes.TensorFuzzingAnnotation[_atypes.Float32], max_filter: _atypes.TensorFuzzingAnnotation[_atypes.Float32], strides, padding: str, out_type: TV_QuantizedDepthwiseConv2D_out_type=_dtypes.qint32, dilations=[1, 1, 1, 1], name=None):
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'QuantizedDepthwiseConv2D', name, input, filter, min_input, max_input, min_filter, max_filter, 'out_type', out_type, 'strides', strides, 'padding', padding, 'dilations', dilations)
    ...
    if not isinstance(strides, (list, tuple)):
        raise TypeError("Expected list for 'strides' argument to 'quantized_depthwise_conv2d' Op, not %r." % strides)
    ...
    if not isinstance(dilations, (list, tuple)):
        raise TypeError("Expected list for 'dilations' argument to 'quantized_depthwise_conv2d' Op, not %r." % dilations)
    dilations = [_execute.make_int(_i, 'dilations') for _i in dilations]
    _, _, _op, _outputs = _op_def_library._apply_op_helper('QuantizedDepthwiseConv2D', input=input, filter=filter, min_input=min_input, max_input=max_input, min_filter=min_filter, max_filter=max_filter, strides=strides, padding=padding, out_type=out_type, dilations=dilations, name=name)
    _result = _outputs[:]
