...
class MergerRetriever(BaseRetriever):
    ...
    async def amerge_documents(self, query: str, run_manager: AsyncCallbackManagerForRetrieverRun) -> List[Document]:
        ...
        Returns:
            A list of merged documents.
        """
        retriever_docs = await asyncio.gather(*(retriever.aget_relevant_documents(query, callbacks=run_manager.get_child('retriever_{}'.format(i + 1))) for i, retriever in enumerate(self.retrievers)))
        merged_documents = []
        max_docs = max(map(len, retriever_docs), default=0)
        for i in range(max_docs):
            for retriever, doc in zip(self.retrievers, retriever_docs):
