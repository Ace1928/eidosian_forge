...
@TransformationFactory.register('contrib.detect_fixed_vars', doc='Detect variables that are de-facto fixed but not considered fixed.')
...
class FixedVarDetector(IsomorphicTransformation):
    ...
    CONFIG.declare('tmp', ConfigValue(default=False, domain=bool, description='True to store the set of transformed variables and their old values so that they can be restored.'))
    CONFIG.declare('tolerance', ConfigValue(default=1e-13, domain=NonNegativeFloat, description='tolerance on bound equality (LB == UB)'))
