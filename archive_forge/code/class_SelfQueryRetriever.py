...
class SelfQueryRetriever(BaseRetriever):
    ...
    async def _aget_relevant_documents(self, query: str, *, run_manager: AsyncCallbackManagerForRetrieverRun) -> List[Document]:
        ...
        Returns:
            List of relevant documents
        """
        structured_query = await self.query_constructor.ainvoke({'query': query}, config={'callbacks': run_manager.get_child()})
        if self.verbose:
            logger.info(f'Generated Query: {structured_query}')
        new_query, search_kwargs = self._prepare_query(query, structured_query)
    ...
    @classmethod
    def from_llm(cls, llm: BaseLanguageModel, vectorstore: VectorStore, document_contents: str, metadata_field_info: Sequence[Union[AttributeInfo, dict]], structured_query_translator: Optional[Visitor]=None, chain_kwargs: Optional[Dict]=None, enable_limit: bool=False, use_original_query: bool=False, **kwargs: Any) -> 'SelfQueryRetriever':
