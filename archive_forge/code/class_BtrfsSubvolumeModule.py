...
class BtrfsSubvolumeModule(object):
    ...
    def __load_filesystem(self):
        ...
        if not filesystem.is_mounted():
            if not self.__automount:
                raise BtrfsModuleException('Target filesystem uuid=%s is not currently mounted and automount=False.Mount explicitly before module execution or pass automount=True' % filesystem.uuid)
            elif self.module.check_mode:
                raise BtrfsModuleException('Target filesystem uuid=%s is not currently mounted. Unable to validate the currentstate while running with check_mode=True' % filesystem.uuid)
            else:
    ...
    def get_results(self):
        ...
        return dict(changed=len(self.__completed_work) > 0, filesystem=self.__filesystem.get_summary(), modifications=self.__get_formatted_modifications(), target_subvolume_id=target.id if target is not None else None)
