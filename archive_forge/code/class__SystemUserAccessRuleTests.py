...
class _SystemUserAccessRuleTests(object):
    ...
    def test_user_can_list_access_rules_for_other_users(self):
        ...
        with self.test_client() as c:
            r = c.get('/v3/users/%s/access_rules' % user['id'], headers=self.headers)
    ...
    def test_user_cannot_get_non_existent_access_rule_not_found(self):
        ...
        user = PROVIDERS.identity_api.create_user(user)
        with self.test_client() as c:
            c.get('/v3/users/%s/access_rules/%s' % (user['id'], uuid.uuid4().hex), headers=self.headers, expected_status_code=http.client.NOT_FOUND)