...
class RegressionHead(base_head.Head):
  ...
  """
    ...
    def __init__(self, label_dimension=1, weight_column=None, loss_reduction=tf.losses.Reduction.SUM_OVER_BATCH_SIZE, loss_fn=None, inverse_link_fn=None, name=None):
    ...
    def _processed_labels(self, logits, labels):
        labels = base_head.check_dense_labels_match_logits_and_reshape(labels=labels, logits=logits, expected_labels_dimension=self._logits_dimension)
    ...
    def _unweighted_loss_and_weights(self, logits, labels, features):
        ...
        if self._loss_fn:
            unweighted_loss = base_head.call_loss_fn(loss_fn=self._loss_fn, labels=labels, logits=logits, features=features, expected_loss_dim=self._logits_dimension)
        else:
        ...
        weights = base_head.get_weights_and_check_match_logits(features=features, weight_column=self._weight_column, logits=logits, allow_per_logit_weights=True)
    ...
    """
        ...
        with ops.name_scope('predictions', values=(logits,)):
            if self._inverse_link_fn:
                ...
                predictions = {pred_keys.PREDICTIONS: predicted_value, pred_keys.LOGITS: logits}
            else:
    ...
    def update_metrics(self, eval_metrics, features, logits, labels, regularization_losses=None):
        ...
        base_head.update_metric_with_broadcast_weights(eval_metrics[self._prediction_mean_key], predicted_value, weights)
        if regularization_losses is not None:
    ...
    """
        with ops.name_scope(self._name, 'head'):
            ...
            if mode == ModeKeys.PREDICT:
                ...
                regression_output = export_output.RegressionOutput(value=predictions[keys.PREDICTIONS])
                return model_fn._TPUEstimatorSpec(mode=ModeKeys.PREDICT, predictions=predictions, export_outputs={base_head.DEFAULT_SERVING_KEY: regression_output, base_head.REGRESS_SERVING_KEY: regression_output, base_head.PREDICT_SERVING_KEY: export_output.PredictOutput(predictions)})
