...
class NovaQuotaTest(common.HeatTestCase):
    ...
    def setUp(self):
        ...
        self.stack = parser.Stack(self.ctx, 'nova_quota_test_stack', template.Template(tpl))
    ...
    def test_miss_all_quotas(self):
        ...
        my_quota.t = my_quota.t.freeze(properties=props)
        my_quota.reparse()
        msg = 'At least one of the following properties must be specified: cores, fixed_ips, floating_ips, injected_file_content_bytes, injected_file_path_bytes, injected_files, instances, key_pairs, metadata_items, ram, security_group_rules, security_groups, server_group_members, server_groups.'
    ...
    def test_quota_handle_create(self):
        ...
        self.quotas.update.assert_called_once_with('some_project_id', cores=5, fixed_ips=5, floating_ips=5, instances=5, injected_files=5, injected_file_content_bytes=5, injected_file_path_bytes=5, key_pairs=5, metadata_items=5, ram=5, security_groups=5, security_group_rules=5, server_groups=5, server_group_members=5)
    ...
    def test_quota_handle_update(self):
        ...
        props = {'project': 'some_project_id', 'cores': 1, 'fixed_ips': 2, 'instances': 3, 'injected_file_content_bytes': 4, 'ram': 200}
        json_snippet = rsrc_defn.ResourceDefinition(self.my_quota.name, 'OS::Nova::Quota', properties=props)
        self.my_quota.reparse()
        self.my_quota.handle_update(json_snippet, tmpl_diff, prop_diff)
        self.quotas.update.assert_called_once_with('some_project_id', cores=1, fixed_ips=2, instances=3, injected_file_content_bytes=4, ram=200)
