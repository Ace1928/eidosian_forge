...
class CapabilityConfig(ProtocolBuffer.ProtocolMessage):
    ...
    DISABLED = 3
    UNKNOWN = 4
    _Status_NAMES = {0: 'DEFAULT', 1: 'ENABLED', 2: 'SCHEDULED', 3: 'DISABLED', 4: 'UNKNOWN'}
    ...
    def has_scheduled_time(self):
        return self.has_scheduled_time_

    def internal_message(self):
        return self.internal_message_
    ...
    def has_internal_message(self):
        return self.has_internal_message_

    def admin_message(self):
        return self.admin_message_
    ...
    def has_admin_message(self):
        return self.has_admin_message_
    ...
    def MergeFrom(self, x):
        ...
        if x.has_package():
            self.set_package(x.package())
        if x.has_capability():
            self.set_capability(x.capability())
        if x.has_status():
            self.set_status(x.status())
        if x.has_scheduled_time():
            self.set_scheduled_time(x.scheduled_time())
        if x.has_internal_message():
            self.set_internal_message(x.internal_message())
        if x.has_admin_message():
            self.set_admin_message(x.admin_message())
    ...
    def Equals(self, x):
        if x is self:
            return 1
        if self.has_package_ != x.has_package_:
            return 0
        if self.has_package_ and self.package_ != x.package_:
            return 0
        if self.has_capability_ != x.has_capability_:
            return 0
        if self.has_capability_ and self.capability_ != x.capability_:
            return 0
        if self.has_status_ != x.has_status_:
            return 0
        if self.has_status_ and self.status_ != x.status_:
            return 0
        if self.has_scheduled_time_ != x.has_scheduled_time_:
            return 0
        if self.has_scheduled_time_ and self.scheduled_time_ != x.scheduled_time_:
            return 0
        if self.has_internal_message_ != x.has_internal_message_:
            return 0
        if self.has_internal_message_ and self.internal_message_ != x.internal_message_:
            return 0
        if self.has_admin_message_ != x.has_admin_message_:
            return 0
        if self.has_admin_message_ and self.admin_message_ != x.admin_message_:
            return 0
    ...
    def ByteSize(self):
        ...
        if self.has_internal_message_:
            n += 1 + self.lengthString(len(self.internal_message_))
        if self.has_admin_message_:
            n += 1 + self.lengthString(len(self.admin_message_))
        if self.has_error_message_:
    ...
    kadmin_message = 5
    kerror_message = 6
    _TEXT = _BuildTagLookupTable({0: 'ErrorCode', 1: 'package', 2: 'capability', 3: 'status', 4: 'internal_message', 5: 'admin_message', 6: 'error_message', 7: 'scheduled_time'}, 7)
    _TYPES = _BuildTagLookupTable({0: ProtocolBuffer.Encoder.NUMERIC, 1: ProtocolBuffer.Encoder.STRING, 2: ProtocolBuffer.Encoder.STRING, 3: ProtocolBuffer.Encoder.NUMERIC, 4: ProtocolBuffer.Encoder.STRING, 5: ProtocolBuffer.Encoder.STRING, 6: ProtocolBuffer.Encoder.STRING, 7: ProtocolBuffer.Encoder.STRING}, 7, ProtocolBuffer.Encoder.MAX_TYPE)
    _STYLE = ''
    _STYLE_CONTENT_TYPE = ''
