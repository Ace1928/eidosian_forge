...
class ServerlessOperations(object):
    ...
    def ListServices(self, namespace_ref):
        ...
        messages = self.messages_module
        request = messages.RunNamespacesServicesListRequest(parent=namespace_ref.RelativeName())
        try:
    ...
    def ListConfigurations(self, namespace_ref):
        ...
        messages = self.messages_module
        request = messages.RunNamespacesConfigurationsListRequest(parent=namespace_ref.RelativeName())
        try:
            with metrics.RecordDuration(metric_names.LIST_CONFIGURATIONS):
                response = self._client.namespaces_configurations.List(request)
            return [configuration.Configuration(item, messages) for item in response.items]
        except api_exceptions.InvalidDataFromServerError as e:
            serverless_exceptions.MaybeRaiseCustomFieldMismatch(e)
    ...
    def ListRoutes(self, namespace_ref):
        ...
        messages = self.messages_module
        request = messages.RunNamespacesRoutesListRequest(parent=namespace_ref.RelativeName())
    ...
    def GetService(self, service_ref):
        ...
        messages = self.messages_module
        service_get_request = messages.RunNamespacesServicesGetRequest(name=service_ref.RelativeName())
        try:
            with metrics.RecordDuration(metric_names.GET_SERVICE):
                service_get_response = self._client.namespaces_services.Get(service_get_request)
    ...
    def GetConfiguration(self, service_or_configuration_ref):
        ...
        if hasattr(service_or_configuration_ref, 'servicesId'):
            name = self._registry.Parse(service_or_configuration_ref.servicesId, params={'namespacesId': service_or_configuration_ref.namespacesId}, collection='run.namespaces.configurations').RelativeName()
        else:
            name = service_or_configuration_ref.RelativeName()
        configuration_get_request = messages.RunNamespacesConfigurationsGetRequest(name=name)
        try:
            with metrics.RecordDuration(metric_names.GET_CONFIGURATION):
                configuration_get_response = self._client.namespaces_configurations.Get(configuration_get_request)
    ...
    def GetRoute(self, service_or_route_ref):
        ...
        if hasattr(service_or_route_ref, 'servicesId'):
            name = self._registry.Parse(service_or_route_ref.servicesId, params={'namespacesId': service_or_route_ref.namespacesId}, collection='run.namespaces.routes').RelativeName()
        else:
        ...
        try:
            with metrics.RecordDuration(metric_names.GET_ROUTE):
                route_get_response = self._client.namespaces_routes.Get(route_get_request)
    ...
    """
        ...
        service_delete_request = messages.RunNamespacesServicesDeleteRequest(name=service_name)
        try:
    ...
    """
        ...
        try:
            ...
            raise serverless_exceptions.RevisionNotFoundError('Revision [{}] could not be found.'.format(revision_ref.revisionsId))

    def GetRevisionsByNonce(self, namespace_ref, nonce):
        """Return all revisions with the given nonce."""
        messages = self.messages_module
        request = messages.RunNamespacesRevisionsListRequest(parent=namespace_ref.RelativeName(), labelSelector='{} = {}'.format(revision.NONCE_LABEL, nonce))
        try:
    ...
    def _EnsureImageDigest(self, serv, config_changes):
        ...
        if not _IsDigest(serv.template.image):
            base_revision = self._GetBaseRevision(serv.template, serv.metadata, serv.status)
            if base_revision:
    ...
    def _AddRevisionForcingChange(self, serv, config_changes):
        ...
        revision_suffix = '{}-{}'.format(str(curr_generation + 1).zfill(5), name_generator.GenerateName())
    ...
    """
        ...
        worker_delete_request = messages.RunNamespacesServicesDeleteRequest(name=worker_name)
        try:
            with metrics.RecordDuration(metric_names.DELETE_SERVICE):
                self._client.namespaces_services.Delete(worker_delete_request)
        except api_exceptions.HttpNotFoundError:
            raise serverless_exceptions.WorkerNotFoundError('Worker [{}] could not be found.'.format(worker_ref.servicesId))
    ...
    def ListWorkerRevisions(self, namespace_ref, worker_name, limit=None, page_size=100):
    ...
    """
        messages = self.messages_module
        request = messages.RunNamespacesDomainmappingsListRequest(parent=namespace_ref.RelativeName())
    ...
    """
        messages = self.messages_module
        request = messages.RunNamespacesDomainmappingsDeleteRequest(name=domain_mapping_ref.RelativeName())
    ...
    def GetTask(self, task_ref):
        ...
        messages = self.messages_module
        get_request = messages.RunNamespacesTasksGetRequest(name=task_ref.RelativeName())
        try:
    ...
    def GetExecution(self, execution_ref):
        ...
        messages = self.messages_module
        get_request = messages.RunNamespacesExecutionsGetRequest(name=execution_ref.RelativeName())
        try:
    ...
    def ListJobs(self, namespace_ref):
        ...
        messages = self.messages_module
        request = messages.RunNamespacesJobsListRequest(parent=namespace_ref.RelativeName())
        try:
    ...
    """
        messages = self.messages_module
        request = messages.RunNamespacesExecutionsDeleteRequest(name=execution_ref.RelativeName())
        try:
            with metrics.RecordDuration(metric_names.DELETE_EXECUTION):
                self._client.namespaces_executions.Delete(request)
        except api_exceptions.HttpNotFoundError:
            raise serverless_exceptions.ExecutionNotFoundError('Execution [{}] could not be found.'.format(execution_ref.Name()))
    ...
    """
        messages = self.messages_module
        request = messages.RunNamespacesExecutionsCancelRequest(name=execution_ref.RelativeName())
        try:
    ...
    def _GetIamPolicy(self, service_name):
        ...
        messages = self.messages_module
        request = messages.RunProjectsLocationsServicesGetIamPolicyRequest(resource=six.text_type(service_name))
    ...
    def GetExecutionOverrides(self, tasks, task_timeout, container_overrides):
        return self.messages_module.Overrides(containerOverrides=container_overrides, taskCount=tasks, timeoutSeconds=task_timeout)

    def GetContainerOverrides(self, update_env_vars, args, clear_args):
        container_overrides = []
        env_vars = self._GetEnvVarList(update_env_vars)
        container_overrides.append(self.messages_module.ContainerOverride(args=args or [], env=env_vars, clearArgs=clear_args))
