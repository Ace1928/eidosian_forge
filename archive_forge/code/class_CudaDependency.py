...
class CudaDependency(SystemDependency):
    supported_languages = ['cuda', 'cpp', 'c']
    ...
    def _cuda_paths(self) -> T.List[T.Tuple[str, bool]]:
        return ([(os.environ[self.env_var], True)] if self.env_var else []) + (self._cuda_paths_win() if self._is_windows() else self._cuda_paths_nix())
    ...
    toolkit_version_regex = re.compile('^CUDA Version\\s+(.*)$')
    path_version_win_regex = re.compile('^v(.*)$')
    path_version_nix_regex = re.compile('^cuda-(.*)$')
    cudart_version_regex = re.compile('#define\\s+CUDART_VERSION\\s+([0-9]+)')
