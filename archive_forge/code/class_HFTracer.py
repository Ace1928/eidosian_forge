...
class HFTracer(Tracer):
    ...
    def __init__(self, autowrap_modules=(math,), autowrap_functions=()):
        ...
        if not is_torch_fx_available():
            raise ImportError(f'Found an incompatible version of torch. Found version {get_torch_version()}, but only version {TORCH_FX_REQUIRED_VERSION} is supported.')
    ...
    def trace(self, root: Union[torch.nn.Module, Callable[..., Any]], concrete_args: Optional[Dict[str, Any]]=None, dummy_inputs: Optional[Dict[str, Any]]=None, complete_concrete_args_with_inputs_not_in_dummy_inputs: bool=True) -> Graph:
        ...
        for input_name in input_names:
            if input_name in inputs:
                continue
            if isinstance(root, self.supported_archs) or type(root).__qualname__.startswith(('_deserialize_graph_module', '_CodeOnlyModule')):
