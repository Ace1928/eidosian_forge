...
class Simplex(gym.Space):
    ...
    def __init__(self, shape, concentration=None, dtype=np.float32):
        ...
        if concentration is not None:
            assert concentration.shape == shape[:-1], f'{concentration.shape} vs {shape[:-1]}'
            self.concentration = concentration
        else:
            self.concentration = np.array([1] * self.dim)

    def sample(self):
        return np.random.dirichlet(self.concentration, size=self.shape[:-1]).astype(self.dtype)

    def contains(self, x):
        return x.shape == self.shape and np.allclose(np.sum(x, axis=-1), np.ones_like(x[..., 0]))
    ...
    def __eq__(self, other):
        return np.allclose(self.concentration, other.concentration) and self.shape == other.shape