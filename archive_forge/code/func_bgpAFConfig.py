...
def bgpAFConfig(module, cmd, prompt, answer):
    ...
    if bgpAFArg1 == 'aggregate-address':
        ...
        value = cnos.checkSanityofVariable(deviceType, 'bgp_aggregate_prefix', bgpAFArg2)
        if value == 'ok':
        ...
        if bgpAFArg2 == 'route-map':
            command = command + bgpAFArg2 + ' '
            value = cnos.checkSanityofVariable(deviceType, 'addrfamily_routemap_name', bgpAFArg3)
            if value == 'ok':
            ...
            value = cnos.checkSanityofVariable(deviceType, 'reachability_half_life', bgpAFArg2)
            if value == 'ok':
                command = command + bgpAFArg2 + ' '
                if bgpAFArg3 is not None:
                    value1 = cnos.checkSanityofVariable(deviceType, 'start_reuse_route_value', bgpAFArg3)
                    value2 = cnos.checkSanityofVariable(deviceType, 'start_suppress_route_value', bgpAFArg4)
                    value3 = cnos.checkSanityofVariable(deviceType, 'max_duration_to_suppress_route', bgpAFArg5)
                    if value1 == 'ok' and value2 == 'ok' and (value3 == 'ok'):
                        command = command + bgpAFArg3 + ' ' + bgpAFArg4 + ' ' + bgpAFArg5 + ' '
                        if bgpAFArg6 is not None:
                            value = cnos.checkSanityofVariable(deviceType, 'unreachability_halftime_for_penalty', bgpAFArg6)
                            if value == 'ok':
        ...
        command = command + bgpAFArg1 + ' '
        value = cnos.checkSanityofVariable(deviceType, 'distance_external_AS', bgpAFArg2)
        if value == 'ok':
            command = command + bgpAFArg2 + ' '
            value = cnos.checkSanityofVariable(deviceType, 'distance_internal_AS', bgpAFArg3)
            if value == 'ok':
                command = command + bgpAFArg3 + ' '
                value = cnos.checkSanityofVariable(deviceType, 'distance_local_routes', bgpAFArg4)
                if value == 'ok':
        ...
        if value == 'ok':
            command = command + bgpAFArg2 + ' '
            value = cnos.checkSanityofVariable(deviceType, 'maxpath_numbers', bgpAFArg3)
            if value == 'ok':
                command = command + bgpAFArg3
            else:
                retVal = 'Error-199'
        ...
        if bgpAFArg2 == 'synchronization':
            ...
            value = cnos.checkSanityofVariable(deviceType, 'network_ip_prefix_with_mask', bgpAFArg2)
            if value == 'ok':
                command = command + bgpAFArg2 + ' '
                if bgpAFArg3 is not None and bgpAFArg3 == 'backdoor':
                    ...
                    command = command + bgpAFArg3
                    value = cnos.checkSanityofVariable(deviceType, 'addrfamily_routemap_name', bgpAFArg4)
                    if value == 'ok':
                        command = command + bgpAFArg4 + ' '
                ...
                value = cnos.checkSanityofVariable(deviceType, 'network_ip_prefix_value', bgpAFArg2)
                if value == 'ok':
                    command = command + bgpAFArg2 + ' '
                    if bgpAFArg3 is not None and bgpAFArg3 == 'backdoor':
                        command = command + bgpAFArg3
                        ...
                        command = command + bgpAFArg3
                        value = cnos.checkSanityofVariable(deviceType, 'network_ip_prefix_mask', bgpAFArg4)
                        if value == 'ok':
                            command = command + bgpAFArg4 + ' '
                        else:
                            retVal = 'Error-299'
        ...
        value = cnos.checkSanityofVariable(deviceType, 'nexthop_crtitical_delay', bgpAFArg2)
        if value == 'ok':
            command = command + bgpAFArg2 + ' '
            value = cnos.checkSanityofVariable(deviceType, 'nexthop_noncrtitical_delay', bgpAFArg3)
            if value == 'ok':
                command = command + bgpAFArg3 + ' '
            else:
                retVal = 'Error-198'
        ...
        command = command + bgpAFArg1 + ' '
        value = cnos.checkSanityofVariable(deviceType, 'addrfamily_redistribute_option', bgpAFArg2)
        if value == 'ok':
