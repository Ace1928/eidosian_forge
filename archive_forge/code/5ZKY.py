...
class Algorithm(ABC):
    ...
    async def get_initstate_and_goalstate(self, snake=None) -> Tuple[Node, Node]:
        """
        ...
        async def determine_states(snake):
            with self.lock:
                # Assuming snake.get_x(), snake.get_y(), snake.get_fruit().x, and snake.get_fruit().y are async functions
                initial_x = await snake.get_x()
                initial_y = await snake.get_y()
                goal_x = await snake.get_fruit().x
                goal_y = await snake.get_fruit().y

                initial_state = Node(initial_x, initial_y)
                goal_state = Node(goal_x, goal_y)
