    ...
    def _get_converter(self, x=None, y=None, kind=None, **kwds):
        ...
        if isinstance(self._data, (pl.LazyFrame, pl.DataFrame)):
            if params.get('hover_cols') == 'all':
                ...
                possible_columns = [[v] if isinstance(v, str) else v for v in params.values() if isinstance(v, (str, list))]
                columns = set(self._data.columns) & set(itertools.chain(*possible_columns)) or {self._data.columns[0]}
                if y is None:
