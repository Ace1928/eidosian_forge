...
class PreconFIRE(Optimizer):
    ...
    def __init__(self, atoms, restart=None, logfile='-', trajectory=None, dt=0.1, maxmove=0.2, dtmax=1.0, Nmin=5, finc=1.1, fdec=0.5, astart=0.1, fa=0.99, a=0.1, theta=0.1, master=None, precon=None, use_armijo=True, variable_cell=False):
        """
            ...
            if not self.skip_flag:
                ...
                if v_f > 0.0:
                    if self.precon is None:
                        ...
                        self.v = (1.0 - self.a) * self.v + self.a * (np.sqrt(self.precon.dot(self.v.reshape(-1), self.v.reshape(-1))) / np.sqrt(np.dot(f.reshape(-1), invP_f.reshape(-1))) * invP_f)
