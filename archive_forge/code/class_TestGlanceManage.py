...
class TestGlanceManage(functional.FunctionalTest):
    ...
    def setUp(self):
        ...
        self.connection = 'sql_connection = sqlite:///%s' % self.db_filepath
        db_options.set_defaults(CONF, connection='sqlite:///%s' % self.db_filepath)
    ...
    def _db_command(self, db_method):
        with open(self.conf_filepath, 'w') as conf_file:
            ...
            conf_file.flush()
        cmd = '%s -m glance.cmd.manage --config-file %s db %s' % (sys.executable, self.conf_filepath, db_method)
    ...
    def test_sync(self):
        ...
        cmd = 'sqlite3 {0} "SELECT version_num FROM alembic_version"'.format(self.db_filepath)
        exitcode, out, err = execute(cmd, raise_error=True)
    ...
    def test_expand(self):
        ...
        self.assertIn('Database expansion is up to date. No expansion needed.', str(out))
    ...
    def test_migrate(self):
        ...
        exitcode, out, err = execute(cmd, raise_error=True)
        self.assertEqual(expand_head, out.rstrip().decode('utf-8'))
        self.assertEqual(False, data_migrations.has_pending_migrations(db_api.get_engine()))
        if data_migrations.has_pending_migrations(db_api.get_engine()):
            exitcode, out, err = self._db_command(db_method='migrate')
            self.assertIn('Database migration is up to date. No migration needed.', str(out))
    ...
    def test_contract(self):
        ...
        self.assertIn('Database is up to date. No migrations needed.', str(out))