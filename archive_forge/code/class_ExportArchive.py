    ...
    **Note on resource tracking:**
    ...
    def track(self, resource):
        ...
        if not isinstance(resource, tf.__internal__.tracking.Trackable):
            raise ValueError(f"Invalid resource type. Expected an instance of a TensorFlow `Trackable` (such as a Keras `Layer` or `Model`). Received instead an object of type '{type(resource)}'. Object received: {resource}")
