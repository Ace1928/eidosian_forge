...
class UpdateBitbucketServer(base.UpdateCommand):
    ...
    """
        ...
        flag_config.add_argument('--repo-slug', help='Bitbucket Server repository slug.')
        flag_config.add_argument('--project-key', help='Bitbucket Server project key.')
        flag_config.add_argument('--bitbucket-server-config-resource', help='Bitbucket Server config resource name.')
        ...
        pr_config.add_argument('--pull-request-pattern', metavar='REGEX', help='A regular expression specifying which base git branch to match for\npull request events.\n\nThis pattern is used as a regular expression search for the base branch (the\nbranch you are trying to merge into) for pull request updates.\nFor example, --pull-request-pattern=foo will match "foo", "foobar", and "barfoo".\n\nThe syntax of the regular expressions accepted is the syntax accepted by\nRE2 and described at https://github.com/google/re2/wiki/Syntax.\n')
        pr_config.add_argument('--comment-control', choices={'COMMENTS_DISABLED': '\nDo not require comments on Pull Requests before builds are triggered.', 'COMMENTS_ENABLED': '\nEnforce that repository owners or collaborators must comment on Pull Requests\nbefore builds are triggered.', 'COMMENTS_ENABLED_FOR_EXTERNAL_CONTRIBUTORS_ONLY': "\nEnforce that repository owners or collaborators must comment on external\ncontributors' Pull Requests before builds are triggered."}, default='COMMENTS_ENABLED', help="Require a repository collaborator or owner to comment '/gcbrun' on a pull\nrequest before running the build.\n")
        trigger_utils.AddBuildConfigArgsForUpdate(flag_config, has_build_config=True, require_docker_image=True)
    ...
    """
        ...
        if done:
            return trigger
        bbs = messages.BitbucketServerTriggerConfig(repoSlug=args.repo_slug, projectKey=args.project_key, bitbucketServerConfigResource=args.bitbucket_server_config_resource)
        if args.pull_request_pattern:
            bbs.pullRequest = messages.PullRequestFilter(branch=args.pull_request_pattern)
            if args.comment_control:
                bbs.pullRequest.commentControl = messages.PullRequestFilter.CommentControlValueValuesEnum(args.comment_control)
        else:
            bbs.push = messages.PushFilter(branch=args.branch_pattern, tag=args.tag_pattern)
        trigger.bitbucketServerTriggerConfig = bbs
        trigger_utils.ParseBuildConfigArgsForUpdate(trigger, old_trigger, args, messages, update_mask, None, has_build_config=True)
