...
class IGCTeacher(AbstractImageTeacher):
    ...
    def add_cmdline_args(cls, argparser):
        ...
        agent.add_argument('--igc-multi-ref', type='bool', default=False, help='specify to evaluate on multi-reference labels')
    ...
    def get_image_features_path(self, task, image_model_name, dt):
        ...
        return os.path.join(image_features_path, f'{image_model_name}_{dt}_features_dict')
    ...
    def get(self, episode_idx: int, entry_idx: int=0) -> dict:
        ...
        action = {'text': text, 'image_id': image_id, 'episode_done': episode_done, 'image': image_features, 'labels': labels}
        return action
    ...
    def load_data(self, data_path: str, opt: Opt) -> List[Dict[str, Any]]:
        ...
        if not os.path.exists(dp):
            raise RuntimeError('Please download the IGC Dataset from https://www.microsoft.com/en-us/download/details.aspx?id=55324. Then, make sure to put the two .csv files in {}'.format(self.get_data_path(opt)))
