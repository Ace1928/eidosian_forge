...
class AxolotlSendLayer(AxolotlBaseLayer):
        ...
        if not self.processIqRegistry(protocolTreeNode):
            if protocolTreeNode.tag == 'receipt':
                ...
                if not messageNode:
                    ...
                    self.getKeysFor([protocolTreeNode['participant'] or protocolTreeNode['from']], lambda successJids, errors: on_get_keys_success(messageNode, retryReceiptEntity, successJids, errors))
                else:
                    self.toUpper(protocolTreeNode)
    ...
    def on_get_keys_process_errors(self, errors):
        for jid, error in errors.items():
            if isinstance(error, MissingParametersException):
                logger.error('Failed to create prekeybundle for %s, user had missing parameters: %s, is that a valid user?' % (jid, error.parameters))
    ...
    def sendEncEntities(self, node, encEntities, participant=None):
        ...
        messageEntity = EncryptedMessageProtocolEntity(encEntities, node['type'], message_attrs)
    ...
    def sendToGroup(self, node, retryReceiptEntity=None):
        ...
        """
        logger.debug('sendToGroup(node=[omitted], retryReceiptEntity=[%s])' % ('[retry_count=%s, retry_jid=%s]' % (retryReceiptEntity.getRetryCount(), retryReceiptEntity.getRetryJid())) if retryReceiptEntity is not None else None)
        groupJid = node['to']
