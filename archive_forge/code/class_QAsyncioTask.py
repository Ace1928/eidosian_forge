...
class QAsyncioTask(futures.QAsyncioFuture):
    ...
    def __init__(self, coro: typing.Union[collections.abc.Generator, collections.abc.Coroutine], *, loop: typing.Optional['events.QAsyncioEventLoop']=None, name: typing.Optional[str]=None, context: typing.Optional[contextvars.Context]=None) -> None:
    ...
    def _step(self, exception_or_future: typing.Union[BaseException, futures.QAsyncioFuture, None]=None) -> None:
        if self.done():
            return
        result = None
        ...
        try:
            ...
            if asyncio.futures.isfuture(result):
                result.add_done_callback(self._step, context=self._context)
            ...
            if self._exception:
                self._loop.call_exception_handler({'message': str(self._exception) if self._exception else 'An exception occurred during task execution', 'exception': self._exception, 'task': self, 'future': exception_or_future if asyncio.futures.isfuture(exception_or_future) else None})
            if self.done():
                self._schedule_callbacks()
                asyncio._unregister_task(self)
