...
class TensorflowDatasets(BaseModel):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        try:
            ...
            raise ImportError('Could not import tensorflow python package. Please install it with `pip install tensorflow`.')
        try:
            import tensorflow_datasets
        except ImportError:
            raise ImportError('Could not import tensorflow_datasets python package. Please install it with `pip install tensorflow-datasets`.')
        if values['sample_to_document_function'] is None:
            raise ValueError('sample_to_document_function is None. Please provide a function that converts a dataset sample to  a Document.')
    ...
    def lazy_load(self) -> Iterator[Document]:
        ...
        """
        return (self.sample_to_document_function(s) for s in self.dataset.take(self.load_max_docs) if self.sample_to_document_function is not None)
