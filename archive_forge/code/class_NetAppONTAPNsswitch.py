...
class NetAppONTAPNsswitch:
    ...
    def __init__(self):
        ...
        self.argument_spec.update(dict(state=dict(required=False, type='str', choices=['present', 'absent'], default='present'), vserver=dict(required=True, type='str'), database_type=dict(required=True, type='str', choices=['hosts', 'group', 'passwd', 'netgroup', 'namemap']), sources=dict(required=False, type='list', elements='str')))
    ...
    def modify_name_service_switch(self, modify):
        ...
        try:
            ...
            self.module.fail_json(msg='Error on modifying name service switch config on vserver %s: %s' % (self.parameters['vserver'], to_native(error)), exception=traceback.format_exc())
