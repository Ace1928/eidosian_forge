...
def create_sql_agent(llm: BaseLanguageModel, toolkit: Optional[SQLDatabaseToolkit]=None, agent_type: Optional[Union[AgentType, Literal['openai-tools']]]=None, callback_manager: Optional[BaseCallbackManager]=None, prefix: Optional[str]=None, suffix: Optional[str]=None, format_instructions: Optional[str]=None, input_variables: Optional[List[str]]=None, top_k: int=10, max_iterations: Optional[int]=15, max_execution_time: Optional[float]=None, early_stopping_method: str='force', verbose: bool=False, agent_executor_kwargs: Optional[Dict[str, Any]]=None, extra_tools: Sequence[BaseTool]=(), *, db: Optional[SQLDatabase]=None, prompt: Optional[BasePromptTemplate]=None, **kwargs: Any) -> AgentExecutor:
    ...
    """
    from langchain.agents import create_openai_functions_agent, create_openai_tools_agent, create_react_agent
    ...
    if agent_type == AgentType.ZERO_SHOT_REACT_DESCRIPTION:
        if prompt is None:
            ...
            template = '\n\n'.join([react_prompt.PREFIX, '{tools}', format_instructions, react_prompt.SUFFIX])
            prompt = PromptTemplate.from_template(template)
        agent = RunnableAgent(runnable=create_react_agent(llm, tools, prompt), input_keys_arg=['input'], return_keys_arg=['output'], **kwargs)
    elif agent_type == AgentType.OPENAI_FUNCTIONS:
        if prompt is None:
            messages: List = [SystemMessage(content=cast(str, prefix)), HumanMessagePromptTemplate.from_template('{input}'), AIMessage(content=suffix or SQL_FUNCTIONS_SUFFIX), MessagesPlaceholder(variable_name='agent_scratchpad')]
            prompt = ChatPromptTemplate.from_messages(messages)
        agent = RunnableAgent(runnable=create_openai_functions_agent(llm, tools, prompt), input_keys_arg=['input'], return_keys_arg=['output'], **kwargs)
    elif agent_type == 'openai-tools':
        if prompt is None:
            messages = [SystemMessage(content=cast(str, prefix)), HumanMessagePromptTemplate.from_template('{input}'), AIMessage(content=suffix or SQL_FUNCTIONS_SUFFIX), MessagesPlaceholder(variable_name='agent_scratchpad')]
            prompt = ChatPromptTemplate.from_messages(messages)
        agent = RunnableMultiActionAgent(runnable=create_openai_tools_agent(llm, tools, prompt), input_keys_arg=['input'], return_keys_arg=['output'], **kwargs)
    else:
        raise ValueError(f"Agent type {agent_type} not supported at the moment. Must be one of 'openai-tools', 'openai-functions', or 'zero-shot-react-description'.")
    return AgentExecutor(name='SQL Agent Executor', agent=agent, tools=tools, callback_manager=callback_manager, verbose=verbose, max_iterations=max_iterations, max_execution_time=max_execution_time, early_stopping_method=early_stopping_method, **agent_executor_kwargs or {})