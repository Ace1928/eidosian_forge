...
class AzureRMSqlElasticPool(AzureRMModuleBase):
    ...
    def __init__(self):
        ...
        super(AzureRMSqlElasticPool, self).__init__(derived_arg_spec=self.module_arg_spec, supports_check_mode=True, supports_tags=True)
    ...
    def create_elastic_pool(self, parameters):
        ...
        self.log('Creating / Updating the SQL Elastic Pool instance {0}'.format(self.name))
        try:
            response = self.sql_client.elastic_pools.begin_create_or_update(resource_group_name=self.resource_group, server_name=self.server_name, elastic_pool_name=self.name, parameters=parameters)
            if isinstance(response, LROPoller):
                response = self.get_poller_result(response)
        except Exception as exc:
            self.log('Error attempting to create the SQL Elastic Pool instance.')
    ...
    def delete_elastic_pool(self):
        ...
        try:
            response = self.sql_client.elastic_pools.begin_delete(resource_group_name=self.resource_group, server_name=self.server_name, elastic_pool_name=self.name)
            if isinstance(response, LROPoller):
                response = self.get_poller_result(response)
