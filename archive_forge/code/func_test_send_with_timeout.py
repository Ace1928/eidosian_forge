...
def test_send_with_timeout(self, fake_publish):
    ...
    with transport._driver._get_connection(driver_common.PURPOSE_SEND) as pool_conn:
        ...
        conn._publish(exchange_mock, 'msg', routing_key='routing_key', timeout=1)
    fake_publish.assert_called_with('msg', expiration=1, exchange=exchange_mock, compression=self.conf.oslo_messaging_rabbit.kombu_compression, mandatory=False, routing_key='routing_key')