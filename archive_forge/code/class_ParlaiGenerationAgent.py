...
class ParlaiGenerationAgent(ParlaiAgent):
    ...
    @torch.no_grad()
    def predict(self, text, method='top_k', num_beams=5, top_k=20, top_p=None, no_repeat_ngram_size=4, length_penalty: int=0.65) -> Dict[str, str]:
        ...
        message = Message({'text': text, 'full_text': text})
        ...
        tokens = self.model._generate(batch=batch, beam_size=num_beams, max_ts=self.maxlen)[0][0][0]
        return {'input': text, 'output': self.model._v2t(tokens.tolist())}