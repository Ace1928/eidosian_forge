...
def _test_chained_multi_native(NativeSys, integrator='cvode', rtol_close=0.02, atol=1e-10, rtol=1e-14, steps_fact=1, **kwargs):
    ...
    tend = 100000000000.0
    _yref_1e11 = (2.083340149701255e-08, 8.333360770334713e-14, 0.999999979166505)
    lin_s = SymbolicSys.from_callback(get_ode_exprs(logc=False, logt=False)[0], ny, nk, lower_bounds=[0] * ny if nonnegative else None)
    logexp = (sp.log, sp.exp)
    if reduced:
        if logc or logt:
            PartSolvSys = PartiallySolvedSystem
        else:
    ...
    for sys_iter, kw in [([our_sys, ori_sys], {'nsteps': [100 * steps_fact, 1613 * 1.05 * steps_fact], 'return_on_error': [True, False]}), ([ori_sys], {'nsteps': [1705 * 1.01 * steps_fact]})]:
        results = integrate_chained(sys_iter, kw, [(zero_time, tend)] * 3, [init_conc] * 3, [k] * 3, integrator=integrator, atol=atol, rtol=rtol, **kwargs)
        for res in results:
            x, y, nfo = res
