...
class OpenStackAuditApi(object):
    ...
    def __init__(self, cfg_file, log=logging.getLogger(__name__)):
        ...
        if cfg_file:
            try:
                ...
                try:
                    default_target_endpoint_type = map_conf.get('DEFAULT', 'target_endpoint_type')
    ...
    def _build_target(self, req, service):
        ...
        target_typeURI = self._build_typeURI(req, service.type) if service.type != taxonomy.UNKNOWN else service.type
        target = resource.Resource(typeURI=target_typeURI, id=service.id, name=service.name)
    ...
    def _create_event(self, req):
        ...
        initiator = ClientResource(typeURI=taxonomy.ACCOUNT_USER, id=req.environ.get('HTTP_X_USER_ID', taxonomy.UNKNOWN), name=req.environ.get('HTTP_X_USER_NAME', taxonomy.UNKNOWN), host=host.Host(address=req.client_addr, agent=req.user_agent), credential=KeystoneCredential(token=req.environ.get('HTTP_X_AUTH_TOKEN', ''), identity_status=req.environ.get('HTTP_X_IDENTITY_STATUS', taxonomy.UNKNOWN)), project_id=req.environ.get('HTTP_X_PROJECT_ID', taxonomy.UNKNOWN), request_id=req.environ.get('openstack.request_id'), global_request_id=req.environ.get('openstack.global_request_id'))
        target = self.get_target_resource(req)
        event = factory.EventFactory().new_event(eventType=cadftype.EVENTTYPE_ACTIVITY, outcome=taxonomy.OUTCOME_PENDING, action=action, initiator=initiator, target=target, observer=resource.Resource(id='target'))
