...
class AzureRMNetAppCapacityPool(AzureRMNetAppModuleBase):
    ...
    def __init__(self):
        self.module_arg_spec = dict(resource_group=dict(type='str', required=True), name=dict(type='str', required=True), account_name=dict(type='str', required=True), location=dict(type='str', required=False), state=dict(choices=['present', 'absent'], default='present', type='str'), size=dict(type='int', required=False, default=1), service_level=dict(type='str', required=False, choices=['Standard', 'Premium', 'Ultra']))
        self.na_helper = NetAppModule()
        self.parameters = dict()
        super(AzureRMNetAppCapacityPool, self).__init__(derived_arg_spec=self.module_arg_spec, required_if=[('state', 'present', ['location', 'service_level'])], supports_check_mode=True)
    ...
    def create_azure_netapp_capacity_pool(self):
        ...
        try:
            response = self.get_method('pools', 'create_or_update')(body=capacity_pool_body, resource_group_name=self.parameters['resource_group'], account_name=self.parameters['account_name'], pool_name=self.parameters['name'])
            while response.done() is not True:
                response.result(10)
        except (CloudError, AzureError) as error:
            self.module.fail_json(msg='Error creating capacity pool %s for Azure NetApp account %s: %s' % (self.parameters['name'], self.parameters['account_name'], to_native(error)), exception=traceback.format_exc())
