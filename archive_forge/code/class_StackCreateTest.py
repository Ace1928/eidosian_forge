...
class StackCreateTest(common.HeatTestCase):
    ...
    def test_stack_create_with_environment_files(self):
        ...
        self._test_stack_create(stack_name, environment_files=environment_files)
    ...
    def test_stack_create_exceeds_max_per_tenant(self):
        ...
        ex = self.assertRaises(dispatcher.ExpectedException, self._test_stack_create, stack_name)
        self.assertEqual(exception.RequestLimitExceeded, ex.exc_info[0])
        self.assertIn('You have reached the maximum stacks per tenant', str(ex.exc_info[1]))
    ...
    def test_stack_create_invalid_stack_name(self):
        ...
        self.assertRaises(dispatcher.ExpectedException, self.man.create_stack, self.ctx, stack_name, stack.t.t, {}, None, {})
    ...
    def test_stack_create_nested(self, mock_validate, mock_tg):
        ...
        mock_tg.return_value = tools.DummyThreadGroup()
        stk = tools.get_stack(stack_name, self.ctx, with_params=True, owner_id=owner_id, nested_depth=1)
        tmpl_id = stk.t.store(self.ctx)
        mock_load = self.patchobject(templatem.Template, 'load', return_value=stk.t)
        mock_stack = self.patchobject(stack, 'Stack', return_value=stk)
        result = self.man.create_stack(self.ctx, stack_name, None, None, None, {}, owner_id=owner_id, nested_depth=1, template_id=tmpl_id)
        self.assertEqual(stk.identifier(), result)
        self.assertIsInstance(result, dict)
        self.assertTrue(result['stack_id'])
        mock_load.assert_called_once_with(self.ctx, tmpl_id)
        mock_stack.assert_called_once_with(self.ctx, stack_name, stk.t, owner_id=owner_id, nested_depth=1, user_creds_id=None, stack_user_project_id=None, convergence=convergence_engine, parent_resource=None)
        mock_validate.assert_called_once_with()
