...
class PredictStep(BaseStep):
    ...
    def _validate_and_apply_step_config(self):
        ...
        for key in required_configuration_keys:
            if key not in self.step_config:
                raise MlflowException(f'The `{key}` configuration key must be specified for the predict step.', error_code=INVALID_PARAMETER_VALUE)
        if self.step_config['using'] not in {'parquet', 'delta', 'table'}:
            raise MlflowException('Invalid `using` in predict step configuration.', error_code=INVALID_PARAMETER_VALUE)
        if 'model_uri' not in self.step_config:
            try:
                register_config = self.step_config['model_registry']
                model_name = register_config['model_name']
            except KeyError:
                raise MlflowException('No model specified for batch scoring: model_registry does not have `model_uri` and does not have `model_name` configuration key.', error_code=INVALID_PARAMETER_VALUE)
            else:
    ...
    def _build_profiles_and_card(self, scored_sdf) -> BaseCard:
        ...
        if not self.skip_data_profiling:
            ...
            scored_dataset_profile = get_pandas_data_profiles([['Profile of Scored Dataset', scored_df]])
            card.add_tab('Scored Data Profile', '{{PROFILE}}').add_pandas_profile('PROFILE', scored_dataset_profile)
        card.add_tab('Run Summary', '\n                {{ SCORED_DATA_NUM_ROWS }}\n                {{ EXE_DURATION }}\n                {{ LAST_UPDATE_TIME }}\n                ').add_markdown('SCORED_DATA_NUM_ROWS', f'**Number of scored dataset rows:** `{scored_size}`')
        return card
    ...
    def _run(self, output_directory):
        ...
        try:
            ...
            raise MlflowException(message='Encountered an error while getting or creating an active Spark session to score dataset with spark UDF.', error_code=BAD_REQUEST) from e
        ingested_data_path = get_step_output_path(recipe_root_path=self.recipe_root, step_name='ingest_scoring', relative_path=_INPUT_FILE_NAME)
        ...
        if _PREDICTION_COLUMN_NAME in input_sdf.columns:
            _logger.warning(f"Input scoring dataframe already contains a column '{_PREDICTION_COLUMN_NAME}'. This column will be dropped in favor of the predict output column name.")
        model_uri = self.step_config['model_uri']
        registered_model_file_path = get_step_output_path(recipe_root_path=self.recipe_root, step_name='register', relative_path=_REGISTERED_MV_INFO_FILE)
        ...
        predict = mlflow.pyfunc.spark_udf(spark, model_uri, result_type=result_type, env_manager=_ENV_MANAGER)
        ...
        if self.save_mode in ['default', 'error', 'errorifexists']:
            if output_format == 'parquet' or output_format == 'delta':
                ...
                try:
                    ...
                    pass
        if output_populated:
            raise MlflowException(message=f'Output location `{output_location}` using format `{output_format}` is already populated. To overwrite, please change the spark `save_mode` in the predict step configuration.', error_code=BAD_REQUEST)
        if output_format == 'table':
            ...
            if output_populated:
                ...
                spark.sql(f"ALTER TABLE {output_location} SET TBLPROPERTIES ('delta.columnMapping.mode'='name','delta.minReaderVersion'='2','delta.minWriterVersion'='5')")
            else:
                _logger.info(f'Creating a new table at {output_location}')
                from delta.tables import DeltaTable
                DeltaTable.create().addColumns(scored_sdf.schema).property('delta.minReaderVersion', '2').property('delta.minWriterVersion', '5').property('delta.columnMapping.mode', 'name').tableName(output_location).execute()
                self.save_mode = 'append'
        if output_format in ['parquet', 'delta']:
            scored_sdf.coalesce(1).write.format(output_format).mode(self.save_mode).save(output_location)
        else:
            scored_sdf.write.format('delta').mode(self.save_mode).saveAsTable(output_location)
        write_spark_dataframe_to_parquet_on_local_disk(scored_sdf, os.path.join(output_directory, _SCORED_OUTPUT_FILE_NAME))
    ...
    def from_recipe_config(cls, recipe_config, recipe_root):
        ...
        if recipe_config.get('model_registry', {}).get('registry_uri') is not None:
            step_config['registry_uri'] = recipe_config.get('model_registry', {}).get('registry_uri')
    ...
    def get_artifacts(self):
        return [DataframeArtifact('scored_data', self.recipe_root, self.name, _SCORED_OUTPUT_FILE_NAME)]
