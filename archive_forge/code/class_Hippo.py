...
class Hippo(VectorStore):
    ...
    def __init__(self, embedding_function: Embeddings, table_name: str='test', database_name: str='default', number_of_shards: int=1, number_of_replicas: int=1, connection_args: Optional[Dict[str, Any]]=None, index_params: Optional[dict]=None, drop_old: Optional[bool]=False):
    ...
    def _create_collection(self, embeddings: list, metadatas: Optional[List[dict]]=None) -> None:
        ...
        fields.append(HippoField(self._vector_field, False, HippoType.FLOAT_VECTOR, type_params={'dimension': dim}))
        if metadatas:
        ...
        self.hc.create_table(name=self.table_name, auto_id=True, fields=fields, database_name=self.database_name, number_of_shards=self.number_of_shards, number_of_replicas=self.number_of_replicas)
        self.col = self.hc.get_table(self.table_name, self.database_name)
        ...
        if isinstance(self.col, HippoTable) and self._get_index() is None:
            if self._get_index() is None:
                if self.index_params is None:
                    self.index_params = {'index_name': 'langchain_auto_create', 'metric_type': MetricType.L2, 'index_type': IndexType.IVF_FLAT, 'nlist': 10}
                    self.col.create_index(self._vector_field, self.index_params['index_name'], self.index_params['index_type'], self.index_params['metric_type'], nlist=self.index_params['nlist'])
                    logger.debug(self.col.activate_index(self.index_params['index_name']))
                    logger.info('create index successfully')
                else:
                    index_dict = {'IVF_FLAT': IndexType.IVF_FLAT, 'FLAT': IndexType.FLAT, 'IVF_SQ': IndexType.IVF_SQ, 'IVF_PQ': IndexType.IVF_PQ, 'HNSW': IndexType.HNSW}
                    metric_dict = {'ip': MetricType.IP, 'IP': MetricType.IP, 'l2': MetricType.L2, 'L2': MetricType.L2}
                    ...
                    if self.index_params['index_type'] == 'FLAT':
                        ...
                        self.col.create_index(self._vector_field, self.index_params['index_name'], self.index_params['index_type'], self.index_params['metric_type'])
                        logger.debug(self.col.activate_index(self.index_params['index_name']))
                    elif self.index_params['index_type'] == 'IVF_FLAT' or self.index_params['index_type'] == 'IVF_SQ':
                        self.index_params['index_type'] = index_dict[self.index_params['index_type']]
                        self.col.create_index(self._vector_field, self.index_params['index_name'], self.index_params['index_type'], self.index_params['metric_type'], nlist=self.index_params.get('nlist', 10), nprobe=self.index_params.get('nprobe', 10))
                        logger.debug(self.col.activate_index(self.index_params['index_name']))
                    elif self.index_params['index_type'] == 'IVF_PQ':
                        self.index_params['index_type'] = index_dict[self.index_params['index_type']]
                        self.col.create_index(self._vector_field, self.index_params['index_name'], self.index_params['index_type'], self.index_params['metric_type'], nlist=self.index_params.get('nlist', 10), nprobe=self.index_params.get('nprobe', 10), nbits=self.index_params.get('nbits', 8), m=self.index_params.get('m'))
                        logger.debug(self.col.activate_index(self.index_params['index_name']))
                    elif self.index_params['index_type'] == 'HNSW':
                        self.index_params['index_type'] = index_dict[self.index_params['index_type']]
                        self.col.create_index(self._vector_field, self.index_params['index_name'], self.index_params['index_type'], self.index_params['metric_type'], M=self.index_params.get('M'), ef_construction=self.index_params.get('ef_construction'), ef_search=self.index_params.get('ef_search'))
                        logger.debug(self.col.activate_index(self.index_params['index_name']))
                    else:
    ...
    def similarity_search_with_score(self, query: str, k: int=4, param: Optional[dict]=None, expr: Optional[str]=None, timeout: Optional[int]=None, **kwargs: Any) -> List[Tuple[Document, float]]:
        ...
        ret = self.similarity_search_with_score_by_vector(embedding=embedding, k=k, param=param, expr=expr, timeout=timeout, **kwargs)
        return ret
    ...
    def similarity_search_with_score_by_vector(self, embedding: List[float], k: int=4, param: Optional[dict]=None, expr: Optional[str]=None, timeout: Optional[int]=None, **kwargs: Any) -> List[Tuple[Document, float]]:
        ...
        res = self.col.query(search_field=self._vector_field, vectors=[embedding], output_fields=output_fields, topk=k, dsl=expr)
    ...
    @classmethod
    def from_texts(cls, texts: List[str], embedding: Embeddings, metadatas: Optional[List[dict]]=None, table_name: str='test', database_name: str='default', connection_args: Dict[str, Any]=DEFAULT_HIPPO_CONNECTION, index_params: Optional[Dict[Any, Any]]=None, search_params: Optional[Dict[str, Any]]=None, drop_old: bool=False, **kwargs: Any) -> 'Hippo':
        """
        ...
        vector_db = cls(embedding_function=embedding, table_name=table_name, database_name=database_name, connection_args=connection_args, index_params=index_params, drop_old=drop_old, **kwargs)
