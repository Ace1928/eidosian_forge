...
class MHA(nn.Module):
    ...
    def _update_kvcache_attention(self, q, kv, inference_params):
        ...
        if inference_params.seqlen_offset == 0 or flash_attn_with_kvcache is None or (not self.use_flash_attn):
            ...
            cache_seqlens = inference_params.lengths_per_sample[:batch] if inference_params.lengths_per_sample is not None else inference_params.seqlen_offset
            alibi_slopes = getattr(self.inner_cross_attn, 'alibi_slopes', None)
            return flash_attn_with_kvcache(q, kv_cache[:, :, 0], kv_cache[:, :, 1], kv[:, :, 0], kv[:, :, 1], cache_seqlens=cache_seqlens, softmax_scale=self.inner_cross_attn.softmax_scale, causal=self.inner_cross_attn.causal, alibi_slopes=alibi_slopes)

    def forward(self, x, x_kv=None, key_padding_mask=None, cu_seqlens=None, max_seqlen=None, mixer_subset=None, inference_params=None, **kwargs):
        """
        Arguments:
        ...
        if inference_params is not None:
            ...
            assert not self.dwconv
        kwargs = {'cu_seqlens': cu_seqlens, 'max_seqlen': max_seqlen, **kwargs} if self.use_flash_attn else {'key_padding_mask': key_padding_mask, **kwargs}
