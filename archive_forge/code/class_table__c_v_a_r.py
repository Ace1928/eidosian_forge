...
class table__c_v_a_r(DefaultTable.DefaultTable):
    ...
    def compile(self, ttFont, useSharedPoints=False):
        tupleVariationCount, tuples, data = compileTupleVariationStore(variations=[v for v in self.variations if v.hasImpact()], pointCount=len(ttFont['cvt '].values), axisTags=[axis.axisTag for axis in ttFont['fvar'].axes], sharedTupleIndices={}, useSharedPoints=useSharedPoints)
        header = {'majorVersion': self.majorVersion, 'minorVersion': self.minorVersion, 'tupleVariationCount': tupleVariationCount, 'offsetToData': CVAR_HEADER_SIZE + len(tuples)}
    ...
    def decompile(self, data, ttFont):
        ...
        self.variations = decompileTupleVariationStore(tableTag=self.tableTag, axisTags=axisTags, tupleVariationCount=header['tupleVariationCount'], pointCount=len(ttFont['cvt '].values), sharedTuples=None, data=data, pos=CVAR_HEADER_SIZE, dataPos=header['offsetToData'])
