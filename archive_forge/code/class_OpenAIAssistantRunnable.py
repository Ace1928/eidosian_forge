...
class OpenAIAssistantRunnable(RunnableSerializable[Dict, OutputType]):
    ...
    def invoke(self, input: dict, config: Optional[RunnableConfig]=None) -> OutputType:
        """Invoke assistant.
        ...
        callback_manager = CallbackManager.configure(inheritable_callbacks=config.get('callbacks'), inheritable_tags=config.get('tags'), inheritable_metadata=config.get('metadata'))
        ...
        try:
            if self.as_agent and input.get('intermediate_steps'):
                ...
                thread = {'messages': [{'role': 'user', 'content': input['content'], 'file_ids': input.get('file_ids', []), 'metadata': input.get('message_metadata')}], 'metadata': input.get('thread_metadata')}
                run = self._create_thread_and_run(input, thread)
            elif 'run_id' not in input:
                _ = self.client.beta.threads.messages.create(input['thread_id'], content=input['content'], role='user', file_ids=input.get('file_ids', []), metadata=input.get('message_metadata'))
    ...
    @classmethod
    async def acreate_assistant(cls, name: str, instructions: str, tools: Sequence[Union[BaseTool, dict]], model: str, *, async_client: Optional[Union[openai.AsyncOpenAI, openai.AsyncAzureOpenAI]]=None, **kwargs: Any) -> OpenAIAssistantRunnable:
        ...
        assistant = await async_client.beta.assistants.create(name=name, instructions=instructions, tools=openai_tools, model=model, file_ids=kwargs.get('file_ids'))
    ...
    async def ainvoke(self, input: dict, config: Optional[RunnableConfig]=None, **kwargs: Any) -> OutputType:
        ...
        try:
            if self.as_agent and input.get('intermediate_steps'):
                ...
                _ = await self.async_client.beta.threads.messages.create(input['thread_id'], content=input['content'], role='user', file_ids=input.get('file_ids', []), metadata=input.get('message_metadata'))
    ...
    def _create_run(self, input: dict) -> Any:
        ...
        return self.client.beta.threads.runs.create(input['thread_id'], assistant_id=self.assistant_id, **params)

    def _create_thread_and_run(self, input: dict, thread: dict) -> Any:
        params = {k: v for k, v in input.items() if k in ('instructions', 'model', 'tools', 'run_metadata')}
        run = self.client.beta.threads.create_and_run(assistant_id=self.assistant_id, thread=thread, **params)
        return run
    ...
    def _get_response(self, run: Any) -> Any:
        if run.status == 'completed':
            ...
            if all((isinstance(content, openai.types.beta.threads.TextContentBlock) if version_gte_1_14 else isinstance(content, openai.types.beta.threads.MessageContentText) for content in answer)):
                answer = '\n'.join((content.text.value for content in answer))
            return OpenAIAssistantFinish(return_values={'output': answer, 'thread_id': run.thread_id, 'run_id': run.id}, log='', run_id=run.id, thread_id=run.thread_id)
            ...
            for tool_call in run.required_action.submit_tool_outputs.tool_calls:
                ...
                try:
                    ...
                    raise ValueError(f'Received invalid JSON function arguments: {function.arguments} for function {function.name}') from e
                if len(args) == 1 and '__arg1' in args:
                    args = args['__arg1']
                actions.append(OpenAIAssistantAction(tool=function.name, tool_input=args, tool_call_id=tool_call.id, log='', run_id=run.id, thread_id=run.thread_id))
            return actions
        else:
    ...
    async def _acreate_run(self, input: dict) -> Any:
        params = {k: v for k, v in input.items() if k in ('instructions', 'model', 'tools', 'run_metadata')}
        return await self.async_client.beta.threads.runs.create(input['thread_id'], assistant_id=self.assistant_id, **params)

    async def _acreate_thread_and_run(self, input: dict, thread: dict) -> Any:
        params = {k: v for k, v in input.items() if k in ('instructions', 'model', 'tools', 'run_metadata')}
        run = await self.async_client.beta.threads.create_and_run(assistant_id=self.assistant_id, thread=thread, **params)
        return run
