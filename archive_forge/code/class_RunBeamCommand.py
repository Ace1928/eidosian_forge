...
class RunBeamCommand(BaseDatasetsCLICommand):
    ...
    def register_subcommand(parser: ArgumentParser):
        ...
        run_beam_parser.add_argument('--cache_dir', type=str, default=None, help='Cache directory where the datasets are stored')
        run_beam_parser.add_argument('--beam_pipeline_options', type=str, default='', help='Beam pipeline options, separated by commas. Example:: `--beam_pipeline_options=job_name=my-job,project=my-project`')
        run_beam_parser.add_argument('--data_dir', type=str, default=None, help='Can be used to specify a manual directory to get the files from')
        ...
        run_beam_parser.add_argument('--ignore_verifications', action='store_true', help='Run the test without checksums and splits checks')
    ...
    def __init__(self, dataset: str, name: str, cache_dir: str, beam_pipeline_options: str, data_dir: str, all_configs: bool, save_infos: bool, ignore_verifications: bool, force_redownload: bool, **config_kwargs):
    ...
    def run(self):
        ...
        if self._beam_pipeline_options:
            beam_options = beam.options.pipeline_options.PipelineOptions(flags=[f'--{opt.strip()}' for opt in self._beam_pipeline_options.split(',') if opt])
        else:
            beam_options = None
        if self._all_configs and len(builder_cls.BUILDER_CONFIGS) > 0:
            for builder_config in builder_cls.BUILDER_CONFIGS:
                builders.append(builder_cls(config_name=builder_config.name, data_dir=self._data_dir, hash=dataset_module.hash, beam_options=beam_options, cache_dir=self._cache_dir, base_path=dataset_module.builder_kwargs.get('base_path')))
        else:
            builders.append(builder_cls(config_name=config_name, data_dir=self._data_dir, beam_options=beam_options, cache_dir=self._cache_dir, base_path=dataset_module.builder_kwargs.get('base_path'), **self._config_kwargs))
        for builder in builders:
            builder.download_and_prepare(download_mode=DownloadMode.REUSE_CACHE_IF_EXISTS if not self._force_redownload else DownloadMode.FORCE_REDOWNLOAD, download_config=DownloadConfig(cache_dir=config.DOWNLOADED_DATASETS_PATH), verification_mode=VerificationMode.NO_CHECKS if self._ignore_verifications else VerificationMode.ALL_CHECKS, try_from_hf_gcs=False)
            if self._save_infos:
                builder._save_infos()
