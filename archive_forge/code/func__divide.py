...
def _divide(self, block_idx: int) -> ('Dataset', 'Dataset'):
    ...
    l_ds = Dataset(ExecutionPlan(left, self._plan.stats(), run_by_consumer=block_list._owned_by_consumer))
    r_ds = Dataset(ExecutionPlan(right, self._plan.stats(), run_by_consumer=block_list._owned_by_consumer))
    return (l_ds, r_ds)