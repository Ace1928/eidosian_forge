...
def _NormalizeMaintenanceExclusionsForPolicy(self, policy):
    ...
    if policy is None:
        policy = self.messages.MaintenancePolicy(window=self.messages.MaintenanceWindow(maintenanceExclusions=empty_excl))
    elif policy.window is None:
        policy.window = self.messages.MaintenanceWindow(maintenanceExclusions=empty_excl)
