...
class TestSeriesToInterval(unittest.TestCase):
    ...
    def test_singleton(self):
        ...
        pred_data = IntervalData({m.var[:, 'A']: [0.5], m.var[:, 'B']: [2.0]}, [(0.1, 0.1)])
    ...
    def test_convert(self):
        ...
        series_data = TimeSeriesData(data, time_points)
        interval_data = series_to_interval(series_data)
        pred_data = IntervalData({m.var[:, 'A']: [2.0, 3.0, 4.0, 5.0], m.var[:, 'B']: [7.0, 8.0, 9.0, 10.0]}, [(0.1, 0.2), (0.2, 0.3), (0.3, 0.4), (0.4, 0.5)])
