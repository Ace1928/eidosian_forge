...
class ListTestablePermissions(base.Command):
  ...
  """
    ...
    @staticmethod
    def Args(parser):
        flags.GetResourceNameFlag('get the testable permissions for').AddToParser(parser)
    ...
    def Run(self, args):
        ...
        if args.resource.startswith('//'):
            ...
            resource = iam_util.GetFullResourceName(resources.REGISTRY.Parse(args.resource))
        if not resource:
            raise exceptions.InvalidResourceException('The given resource is not a valid full resource name or URL.')
        client, messages = util.GetClientAndMessages()
        return list_pager.YieldFromList(client.permissions, messages.QueryTestablePermissionsRequest(fullResourceName=resource), field='permissions', method='QueryTestablePermissions', batch_size_attribute='pageSize')