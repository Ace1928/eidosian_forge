...
def gcps(ctx, input, geojson_type, projection, precision, use_rs, indent, compact):
    ...
    if geojson_type == 'feature' and indent and (not use_rs):
        raise click.BadParameter('Pretty-printing a sequence of Features requires the --rs option')
    with ctx.obj['env'], rasterio.open(input) as src:
        ...
        if geojson_type == 'collection':
            ...
            click.echo(json.dumps({'type': 'FeatureCollection', 'features': features}, separators=(',', ':') if compact else None, indent=indent))
        else:
            for p in gcps:
                if use_rs:
                ...
                click.echo(json.dumps(feat, separators=(',', ':') if compact else None, indent=indent))