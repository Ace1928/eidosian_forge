...
class ChineseTokenizer(DummyTokenizer):
    ...
    def __init__(self, vocab: Vocab, segmenter: Segmenter=Segmenter.char):
        self.vocab = vocab
        self.segmenter = segmenter.value if isinstance(segmenter, Segmenter) else segmenter
        self.pkuseg_seg = None
        self.jieba_seg = None
        if self.segmenter not in Segmenter.values():
            warn_msg = Warnings.W103.format(lang='Chinese', segmenter=self.segmenter, supported=', '.join(Segmenter.values()), default="'char' (character segmentation)")
            warnings.warn(warn_msg)
            self.segmenter = Segmenter.char
        if self.segmenter == Segmenter.jieba:
            self.jieba_seg = try_jieba_import()

    def initialize(self, get_examples: Optional[Callable[[], Iterable[Example]]]=None, *, nlp: Optional[Language]=None, pkuseg_model: Optional[str]=None, pkuseg_user_dict: Optional[str]='default'):
        if self.segmenter == Segmenter.pkuseg:
            if pkuseg_user_dict is None:
                pkuseg_user_dict = pkuseg_model
            self.pkuseg_seg = try_pkuseg_import(pkuseg_model=pkuseg_model, pkuseg_user_dict=pkuseg_user_dict)
    ...
    def _get_config(self) -> Dict[str, Any]:
        return {'segmenter': self.segmenter}
    ...
    def to_bytes(self, **kwargs):
        ...
        if self.pkuseg_seg:
            ...
            pkuseg_processors_data = (_get_pkuseg_trie_data(self.pkuseg_seg.preprocesser.trie), self.pkuseg_seg.postprocesser.do_process, sorted(list(self.pkuseg_seg.postprocesser.common_words)), sorted(list(self.pkuseg_seg.postprocesser.other_words)))
        serializers = {'cfg': lambda: srsly.json_dumps(self._get_config()), 'pkuseg_features': lambda: pkuseg_features_b, 'pkuseg_weights': lambda: pkuseg_weights_b, 'pkuseg_processors': lambda: srsly.msgpack_dumps(pkuseg_processors_data)}
    ...
    def from_bytes(self, data, **kwargs):
        ...
        deserializers = {'cfg': lambda b: self._set_config(srsly.json_loads(b)), 'pkuseg_features': deserialize_pkuseg_features, 'pkuseg_weights': deserialize_pkuseg_weights, 'pkuseg_processors': deserialize_pkuseg_processors}
    ...
    def to_disk(self, path, **kwargs):
        ...
        serializers = {'cfg': lambda p: srsly.write_json(p, self._get_config()), 'pkuseg_model': lambda p: save_pkuseg_model(p), 'pkuseg_processors': lambda p: save_pkuseg_processors(p)}
    ...
    def from_disk(self, path, **kwargs):
        ...
        def load_pkuseg_processors(path):
            ...
            if self.segmenter == Segmenter.pkuseg:
                ...
                self.pkuseg_seg.postprocesser.common_words = set(common_words)
                self.pkuseg_seg.postprocesser.other_words = set(other_words)
        serializers = {'cfg': lambda p: self._set_config(srsly.read_json(p)), 'pkuseg_model': lambda p: load_pkuseg_model(p), 'pkuseg_processors': lambda p: load_pkuseg_processors(p)}
