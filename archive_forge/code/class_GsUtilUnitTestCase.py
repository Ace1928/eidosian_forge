...
class GsUtilUnitTestCase(base.GsUtilTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.command_runner = CommandRunner(bucket_storage_uri_class=cls.mock_bucket_storage_uri, gsutil_api_class_map_factory=cls.mock_gsutil_api_class_map_factory)
    ...
    def MakeGsUtilApi(cls, debug=0):
        gsutil_api_map = {ApiMapConstants.API_MAP: cls.mock_gsutil_api_class_map_factory.GetClassMap(), ApiMapConstants.SUPPORT_MAP: {'gs': [ApiSelector.XML, ApiSelector.JSON], 's3': [ApiSelector.XML]}, ApiMapConstants.DEFAULT_MAP: {'gs': ApiSelector.JSON, 's3': ApiSelector.XML}}
        return CloudApiDelegator(cls.mock_bucket_storage_uri, gsutil_api_map, cls.logger, DiscardMessagesQueue(), debug=debug)
