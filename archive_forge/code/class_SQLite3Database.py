...
class SQLite3Database(Database):
    ...
    def _initialize(self, con):
        ...
        if self.version > VERSION:
            raise IOError('Can not read new ase.db format (version {}).  Please update to latest ASE.'.format(self.version))
        if self.version < 5 and (not self._allow_reading_old_format):
            raise IOError('Please convert to new format. ' + 'Use: python -m ase.db.convert ' + self.filename)
    ...
    def _write(self, atoms, key_value_pairs, data, id):
        ...
        if constraints:
            ...
            constraints = None
        values = (row.unique_id, row.ctime, mtime, row.user, blob(row.numbers), blob(row.positions), blob(row.cell), int(np.dot(row.pbc, [1, 2, 4])), blob(row.get('initial_magmoms')), blob(row.get('initial_charges')), blob(row.get('masses')), blob(row.get('tags')), blob(row.get('momenta')), constraints)
        ...
        with self.managed_connection() as con:
            ...
            values += (row.get('energy'), row.get('free_energy'), blob(row.get('forces')), blob(row.get('stress')), blob(row.get('dipole')), blob(row.get('magmoms')), row.get('magmom'), blob(row.get('charges')), encode(key_value_pairs), data, len(row.numbers), float_if_not_none(row.get('fmax')), float_if_not_none(row.get('smax')), float_if_not_none(row.get('volume')), float(row.mass), float(row.charge))
            cur = con.cursor()
            if id is None:
    ...
    def _update(self, id, key_value_pairs, data=None):
        ...
        with self.managed_connection() as con:
            cur = con.cursor()
            cur.execute('UPDATE systems SET mtime=?, key_value_pairs=? WHERE id=?', (mtime, encode(key_value_pairs), id))
            if data:
                if not isinstance(data, (str, bytes)):
                    data = encode(data, binary=self.version >= 9)
    ...
    def create_select_statement(self, keys, cmps, sort=None, order=None, sort_table=None, what='systems.*'):
        ...
        for key, op, value in cmps:
            if key in ['id', 'energy', 'magmom', 'ctime', 'user', 'calculator', 'natoms', 'pbc', 'unique_id', 'fmax', 'smax', 'volume', 'mass', 'charge']:
                if key == 'user':
                    key = 'username'
                elif key == 'pbc':
                    assert op in ['=', '!=']
                ...
                if self.type == 'postgresql':
                    where.append('cardinality(array_positions(' + 'numbers::int[], ?)){}?'.format(op))
                    args += [key, value]
                elif bad[key]:
                    where.append('systems.id not in (select id from species ' + 'where Z=? and n{}?)'.format(invop[op]))
                    args += [key, value]
                else:
                    where.append('systems.id in (select id from species ' + 'where Z=? and n{}?)'.format(op))
                    args += [key, value]
                ...
                where.append('systems.id in (select id from text_key_values ' + 'where key=? and value{}?)'.format(op))
                args += [key, value]
            else:
                where.append('systems.id in (select id from number_key_values ' + 'where key=? and value{}?)'.format(op))
    ...
    def _delete(self, cur, ids, tables=None):
        ...
        for table in tables:
            cur.execute('DELETE FROM {} WHERE id in ({});'.format(table, ', '.join([str(id) for id in ids])))
    ...
    def metadata(self, dct):
        ...
        with self.managed_connection() as con:
            ...
            if cur.fetchone()[0]:
                cur.execute("UPDATE information SET value=? WHERE name='metadata'", [md])
            else:
    ...
    def _create_table_if_not_exists(self, name, dtype):
        ...
        if name in taken_names:
            raise ValueError('External table can not be any of {}'.format(taken_names))
