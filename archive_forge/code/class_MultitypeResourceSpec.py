...
class MultitypeResourceSpec(concepts.ConceptSpec):
  ...
  """
        ...
        for spec in self._concept_specs:
            for i, attribute in enumerate(spec.attributes):
                ...
                if attr_name in attr_map and attribute != attr_map[attr_name][1]:
                    raise ConfigurationError(f'Multiple non-equivalent attributes found with name [{attribute.name}]')
                attr_map[attr_name] = (i, attribute)
                self._attribute_to_types_map.setdefault(attr_name, []).append(self.type_enum[self._ConceptToName(spec)])
    ...
    """
        active_fallthroughs_map = {attr: [f for f in fallthroughs if f.active] for attr, fallthroughs in full_fallthroughs_map.items()}
        actively_specified = self._GetSpecifiedAttributes(active_fallthroughs_map, parsed_args=parsed_args)
        actively_specified_resources = self._FilterTypesByAttribute(actively_specified, self._GetParsedResources(active_fallthroughs_map, parsed_args))
        if len(actively_specified_resources) == 1:
            return actively_specified_resources[0]
        all_specified = self._GetSpecifiedAttributes(full_fallthroughs_map, parsed_args=parsed_args)
        parsed_resources = self._GetParsedResources(full_fallthroughs_map, parsed_args)
        if not parsed_resources:
            raise InitializationError(self.name, self._concept_specs, all_specified, full_fallthroughs_map)
        specified_resources = self._FilterTypesByAttribute(actively_specified, parsed_resources)
        if len(specified_resources) == 1:
            return specified_resources[0]
        else:
            return self._PromptOrErrorConflictingTypes(all_specified, full_fallthroughs_map, parsed_resources)
    ...
    """
        ...
        if not plural:
            return self._ParseFromValue(attribute_to_args_map, base_fallthroughs_map, parsed_args, allow_empty)
        else:
            return self._ParseFromPluralValue(attribute_to_args_map, base_fallthroughs_map, parsed_args, allow_empty)

    def BuildFullFallthroughsMap(self, attribute_to_args_map, base_fallthroughs_map, parsed_args=None):
    ...
    """
        ...
        for resource_spec in self._concept_specs:
            deps_map_util.AddFlagFallthroughs(fallthroughs_map, resource_spec.attributes, attribute_to_args_map)
            deps_map_util.UpdateWithValueFallthrough(fallthroughs_map, resource_spec.anchor.name, parsed_args)
        map_without_anchors = {**fallthroughs_map}
        for resource_spec in self._concept_specs:
            deps_map_util.AddAnchorFallthroughs(fallthroughs_map, resource_spec.attributes, resource_spec.anchor, resource_spec.collection_info, map_without_anchors.get(resource_spec.anchor.name, []))
        return fallthroughs_map

    def _BuildFullFallthroughsMapList(self, anchor, attribute_to_args_map, base_fallthroughs_map, parsed_args=None):
    ...
    """
        ...
        map_list = deps_map_util.CreateValueFallthroughMapList(fallthroughs_map, anchor.name, parsed_args)
        for full_map in map_list:
            for spec in self._concept_specs:
                if spec.anchor.name != anchor.name:
                    continue
                deps_map_util.AddAnchorFallthroughs(full_map, spec.attributes, spec.anchor, spec.collection_info, full_map.get(spec.anchor.name, []))
        return map_list
    ...
    def _ParseFromPluralValue(self, attribute_to_args_map, base_fallthroughs_map, parsed_args, allow_empty=False):
        ...
        for attribute in self.attributes:
            if self.IsLeafAnchor(attribute):
                results += self._ParseFromPluralLeaf(attribute_to_args_map, base_fallthroughs_map, attribute, parsed_args=parsed_args)
        if results:
            return results
        parent = self._ParseFromValue(attribute_to_args_map, base_fallthroughs_map, parsed_args, allow_empty=allow_empty)
    ...
    def _GetSpecifiedAttributes(self, fallthroughs_map, parsed_args=None):
        ...
        for attribute in self.attributes:
            try:
                value = deps_lib.Get(attribute.name, fallthroughs_map, parsed_args=parsed_args)
    ...
    def _GetUniqueNameForSpec(self, resource_spec, final_names):
        ...
        other_collection_names = [spec.collection for spec in self._concept_specs]
        ...
        if any((resource_spec.collection == n for n in other_collection_names)):
            raise ValueError('Attempting to create a multitype spec with duplicate collections. Collection name: [{}]'.format(resource_spec.collection))
        else:
