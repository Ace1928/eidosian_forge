...
class ResourceParameterInfo(parameter_info_lib.ParameterInfoByConvention):
    ...
    def __init__(self, resource_info, parsed_args, argument, **kwargs):
        ...
        super(ResourceParameterInfo, self).__init__(parsed_args, argument, **kwargs)
    ...
    """
        del check_properties
        attribute_name = self.resource_info.resource_spec.AttributeName(parameter_name)
        current = properties.VALUES.core.disable_prompts.GetBool()
        properties.VALUES.core.disable_prompts.Set(True)
        try:
            return deps.Get(attribute_name, self.resource_info.BuildFullFallthroughsMap(), parsed_args=self.parsed_args) if attribute_name else None
