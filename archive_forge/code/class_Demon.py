...
class Demon(FileIOCalculator):
    ...
    def calculate(self, atoms=None, properties=['energy'], system_changes=all_changes):
        ...
        if self.command is None:
            raise RuntimeError('Please set $%s environment variable ' % 'DEMON_COMMAND' + 'or supply the command keyword')
        command = self.command
        basis_path = self.parameters['basis_path']
        if basis_path is None:
            basis_path = os.environ.get('DEMON_BASIS_PATH')
        if basis_path is None:
            raise RuntimeError('Please set basis_path keyword,' + ' or the DEMON_BASIS_PATH' + ' environment variable')
        value = self.parameters['guess']
        if value.upper() == 'RESTART':
            value2 = self.parameters['deMon_restart_path']
            if op.exists(self.directory + '/deMon.rst') or op.islink(self.directory + '/deMon.rst'):
    ...
    def read(self, restart_path):
        ...
        if not op.exists(restart_path + '/deMon.inp'):
            raise ReadError('The restart_path file {0} does not exist'.format(restart_path))
