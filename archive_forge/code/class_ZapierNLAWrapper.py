...
class ZapierNLAWrapper(BaseModel):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        zapier_nla_api_key = get_from_dict_or_env(values, 'zapier_nla_api_key', 'ZAPIER_NLA_API_KEY', zapier_nla_api_key_default)
    ...
    async def arun(self, action_id: str, instructions: str, params: Optional[Dict]=None) -> Dict:
        ...
        call.
        """
        response = await self._arequest('POST', self._create_action_url(action_id), json=self._create_action_payload(instructions, params))
    ...
    async def apreview(self, action_id: str, instructions: str, params: Optional[Dict]=None) -> Dict:
        ...
        instead return a preview of params that have been guessed by the AI in
        case you need to explicitly review before executing."""
        response = await self._arequest('POST', self._create_action_url(action_id), json=self._create_action_payload(instructions, params, preview_only=True))
        return response['result']
    ...
    async def apreview_as_str(self, *args, **kwargs) -> str:
        """Same as preview, but returns a stringified version of the JSON for
        insertting back into an LLM."""
