...
class CompositeCell(Cell):
    ...
    def __init__(self, height: int, sub_cell_cols_generator: Iterable[List[Optional[Cell]]], *, gate_count: int):
        """Inits CompositeCell.
    ...
    def _transform_cells(self, func: Callable[[Cell], Cell]) -> 'CompositeCell':
        return CompositeCell(height=self.height, sub_cell_cols_generator=_iterator_to_iterable(([None if cell is None else func(cell) for cell in col] for col in self._sub_cell_cols_generator)), gate_count=self._gate_count)
    ...
    def circuit(self) -> 'cirq.Circuit':
        ...
        for col in self._sub_cell_cols_sealed():
            ...
            if body:
                basis_change = circuits.Circuit((cell.basis_change() for cell in col if cell is not None))
                result += basis_change
                result += body
