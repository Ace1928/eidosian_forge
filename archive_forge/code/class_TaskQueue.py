...
class TaskQueue:
    ...
    def __init__(self, queue_name: typing.Optional[str]=None, prefix: typing.Optional[str]=None, db: typing.Optional[int]=None, serializer: typing.Optional[typing.Union[object, SerializerType]]=None, max_concurrency: typing.Optional[int]=None, max_broadcast_concurrency: typing.Optional[int]=None, client_mode: typing.Optional[bool]=False, debug_enabled: typing.Optional[bool]=None, uuid: typing.Optional[str]=None, metrics_func: typing.Optional[typing.Callable]=None, verbose_results: typing.Optional[bool]=False, truncate_logs: typing.Optional[bool]=True, logging_max_length: typing.Optional[int]=500, silenced_functions: typing.Optional[typing.List[typing.Union[str, typing.Tuple[str, typing.List[str]]]]]=None, heartbeat_ttl: typing.Optional[int]=None, is_leader_process: typing.Optional[bool]=None, function_tracker_enabled: typing.Optional[bool]=None, function_tracker_ttl: typing.Optional[int]=None, push_to_queue_enabled: typing.Optional[bool]=None, push_to_queue_key: typing.Optional[str]=None, push_to_queue_ttl: typing.Optional[int]=None, **kwargs):
        ...
        self._ctx_kwargs = {'db_id': db if db is not None else self.settings.worker.db, **kwargs}
        ...
        self._stats = QueueStats(name=self.queue_name, prefix=self.prefix, uuid=uuid)
        ...
        self._push_queue_kwargs = {'push_to_queue_enabled': push_to_queue_enabled, 'push_to_queue_key': push_to_queue_key, 'push_to_queue_ttl': push_to_queue_ttl, **kwargs}
    ...
    def ctx(self) -> KeyDBSession:
        ...
        return KeyDBClient.create_session(name=self.queue_name, serializer=False, cache_enabled=False, decode_responses=False, auto_pubsub=False, set_current=False, max_connections=self.max_concurrency * 10, pool_class=ConnectionPool, amax_connections=self.max_concurrency ** 2, apool_class=AsyncConnectionPool, **self._ctx_kwargs)
    ...
    async def _get_stats(self, log_stats: bool=False, include_jobs: bool=True, include_conn_kwargs: bool=True, include_retries: bool=False):
        ...
        _stats['workers'] = {'num': self.num_workers, 'active': self.active_workers}
    ...
    def is_silenced_function(self, name: str, stage: typing.Optional[str]=None) -> bool:
        """
    ...
    def logger(self, job: 'Job'=None, kind: str='enqueue', job_id: typing.Optional[str]=None):
        ...
        """
        if job:
            return logger.bind(job_id=job.id, status=job.status, worker_name=self._worker_name, queue_name=getattr(job.queue, 'queue_name', None) or 'unknown queue', kind=kind)
        elif job_id:
            return logger.bind(job_id=job_id, worker_name=self._worker_name, queue_name=self.log_name, kind=kind)
        else:
            return logger.bind(worker_name=self._worker_name, queue_name=self.log_name, kind=kind)
    ...
    def deserialize(self, job_bytes: bytes):
        ...
        assert job_dict.pop('queue') == self.queue_name, f'Job {job_dict} fetched by wrong queue: {self.queue_name}'
    ...
    async def prepare_server(self):
        ...
        if 'maxclients' not in info:
            if self._should_debug_log:
                logger.warning(f'Unable to configure the maxclients to {self.max_concurrency}: maxclients not supported: {info}')
            return
        ...
        new_max_connections = max(min_connections, curr_connected + min_connections, curr_max_connections)
    ...
    async def schedule(self, lock: int=1, worker_id: typing.Optional[str]=None):
        ...
        return await self._schedule_script(keys=[scheduled_key, incomplete_key, queued_key], args=[lock, seconds(now())])
    ...
    async def sweep(self, lock: int=60, worker_id: typing.Optional[str]=None):
        ...
        incomplete_key = self.incomplete_key if worker_id is None else f'{self.incomplete_key}:{worker_id}'
        job_ids = await self._cleanup_script(keys=[sweep_key, active_key], args=[lock], client=self.ctx.async_client)
        swept = []
        if job_ids:
            for job_id, job_bytes in zip(job_ids, await self.ctx.async_mget(job_ids)):
                ...
                if not job:
                    ...
                    if self.is_silenced_function(job.function, stage='sweep'):
                        ...
                        job_result = f'{str(job.result)[:self.logging_max_length]}...' if self.logging_max_length else str(job.result)
    ...
    async def abort(self, job: Job, error: typing.Any, ttl: int=5):
        ...
        async with self.op_sephamore(job.bypass_lock if job else False):
            async with self.ctx.async_client.pipeline(transaction=True, retryable=True) as pipe:
                dequeued, *_ = await pipe.lrem(job.queued_key, 0, job.id).zrem(job.incomplete_key, job.id).expire(job.id, ttl + 1).setex(job.abort_id, ttl, error).execute()
            if dequeued:
    ...
    async def finish(self, job: Job, status: typing.Union[JobStatus, str], *, result: typing.Any=None, error: typing.Any=None):
        """
    ...
    async def _dequeue_job_id(self, timeout: int=0, postfix: str=None):
        """
        ...
        if self.version < (6, 2, 0):
            return await self.ctx.async_brpoplpush(queued_key, active_key, timeout)
        return await self.ctx.async_blmove(source_list=queued_key, destination_list=active_key, source='RIGHT', destination='LEFT', timeout=timeout)

    async def dequeue(self, timeout=0, worker_id: str=None, worker_name: str=None):
        """
        Dequeue a job from the queue.
        """
        job_id = await self._dequeue_job_id(timeout) if worker_id is None and worker_name is None else None
    ...
    async def _deferred(self, job_or_func: typing.Union[Job, str], wait_time: float=10.0, **kwargs):
        """
    ...
    async def defer(self, job_or_func: typing.Union[Job, str], wait_time: float=10.0, **kwargs):
        """
    ...
    @overload
    async def enqueue(self, job_or_func: typing.Union[Job, str, typing.Callable], key: typing.Optional[str]=None, timeout: typing.Optional[int]=None, retries: typing.Optional[int]=None, ttl: typing.Optional[int]=None, retry_delay: typing.Optional[int]=None, retry_backoff: typing.Optional[int]=None, worker_id: typing.Optional[str]=None, worker_name: typing.Optional[str]=None, job_callback: typing.Optional[typing.Callable]=None, job_callback_kwargs: typing.Optional[typing.Dict]=None, _return_existing_job: bool=False, **kwargs) -> typing.Optional[Job]:
        """
    ...
    async def enqueue(self, job_or_func: typing.Union[Job, str, typing.Callable], _return_existing_job: bool=False, **kwargs) -> typing.Optional[Job]:
        """
        Enqueue a job by instance or string.
        ...
        async with self.op_sephamore(job.bypass_lock):
            if not await self._enqueue_script(keys=[job.incomplete_key, job.id, job.queued_key, job.abort_id], args=[self.serialize(job), job.scheduled], client=self.ctx.async_client):
        ...
        if not self.client_mode and self.debug_enabled:
            ...
            self.logger(job=job, kind='enqueue').info(f'→ duration={now() - job.queued}ms, node={self.node_name}, func={job.function}, timeout={job.timeout}, kwargs={job.kwargs}')
        elif self.truncate_logs:
            job_kwargs = f'{str({k: str(v)[:self.logging_max_length] for k, v in job.kwargs.items()})[:self.logging_max_length]}...' if job.kwargs else ''
            self.logger(job=job, kind='enqueue').info(f'→ duration={now() - job.queued}ms, node={self.node_name}, func={job.function}, timeout={job.timeout}, kwargs={job_kwargs}')
        else:
            self.logger(job=job, kind='enqueue').info(f'→ duration={now() - job.queued}ms, node={self.node_name}, func={job.function}, timeout={job.timeout}')
        return job

    @overload
    async def apply(self, job_or_func: typing.Union[Job, str, typing.Callable], key: typing.Optional[str]=None, timeout: typing.Optional[int]=None, retries: typing.Optional[int]=None, ttl: typing.Optional[int]=None, retry_delay: typing.Optional[int]=None, retry_backoff: typing.Optional[int]=None, worker_id: typing.Optional[str]=None, worker_name: typing.Optional[str]=None, job_callback: typing.Optional[typing.Callable]=None, job_callback_kwargs: typing.Optional[typing.Dict]=None, broadcast: typing.Optional[bool]=None, worker_names: typing.Optional[typing.List[str]]=None, worker_selector: typing.Optional[typing.Callable]=None, worker_selector_args: typing.Optional[typing.List]=None, worker_selector_kwargs: typing.Optional[typing.Dict]=None, workers_selected: typing.Optional[typing.List[typing.Dict[str, str]]]=None, return_all_results: typing.Optional[bool]=False, **kwargs) -> typing.Optional[typing.Any]:
        """
    ...
    async def apply(self, job_or_func: typing.Union[Job, str], timeout: int=None, broadcast: typing.Optional[bool]=None, worker_names: typing.Optional[typing.List[str]]=None, worker_selector: typing.Optional[typing.Callable]=None, worker_selector_args: typing.Optional[typing.List]=None, worker_selector_kwargs: typing.Optional[typing.Dict]=None, workers_selected: typing.Optional[typing.List[typing.Dict[str, str]]]=None, return_all_results: typing.Optional[bool]=False, **kwargs):
        """
        Enqueue a job and wait for its result.
        ...
        """
        results = await self.map(job_or_func, timeout=timeout, iter_kwargs=[kwargs], broadcast=broadcast, worker_names=worker_names, worker_selector=worker_selector, worker_selector_args=worker_selector_args, worker_selector_kwargs=worker_selector_kwargs, workers_selected=workers_selected)
        return results if return_all_results else results[0]

    async def select_workers(self, func: typing.Callable, *args, **kwargs) -> typing.List[typing.Dict[str, str]]:
        """
        ...
        return await func(*args, worker_attributes=self.worker_attributes, **kwargs)

    async def prepare_job_kws(self, iter_kwargs: typing.Iterable[typing.Dict], timeout: int=None, broadcast: typing.Optional[bool]=False, worker_names: typing.Optional[typing.List[str]]=None, worker_selector: typing.Optional[typing.Callable]=None, worker_selector_args: typing.Optional[typing.List]=None, worker_selector_kwargs: typing.Optional[typing.Dict]=None, workers_selected: typing.Optional[typing.List[typing.Dict[str, str]]]=None, **kwargs) -> typing.Tuple[typing.List[typing.Dict[str, typing.Any]], typing.Optional[typing.List[typing.Dict[str, str]]]]:
        """
        ...
        """
        if not broadcast and (not worker_selector) and (not worker_names) and (not workers_selected):
            return ([{'timeout': timeout, 'key': kwargs.get('key') or get_default_job_key(), **kwargs, **kw} for kw in iter_kwargs], None)
        if workers_selected:
            worker_kws = workers_selected
            ...
            worker_kws = [{'worker_name': worker_name} for worker_name in worker_names]
        else:
            await self.prepare_for_broadcast()
            worker_kws = [{'worker_id': worker_id} for worker_id in await self.get_worker_ids()]
        broadcast_kwargs = []
        for kw in iter_kwargs:
            broadcast_kwargs.extend(({'timeout': timeout, 'key': get_default_job_key(), **kwargs, **kw, **worker_kw} for worker_kw in worker_kws))
        return (broadcast_kwargs, worker_kws)

    async def broadcast(self, job_or_func: typing.Union[Job, str], timeout: int=None, enqueue: typing.Optional[bool]=True, worker_names: typing.Optional[typing.List[str]]=None, worker_selector: typing.Optional[typing.Callable]=None, worker_selector_args: typing.Optional[typing.List]=None, worker_selector_kwargs: typing.Optional[typing.Dict]=None, workers_selected: typing.Optional[typing.List[typing.Dict[str, str]]]=None, **kwargs):
        """
        ...
        """
        if not enqueue:
            return await self.map(job_or_func, timeout=timeout, iter_kwargs=[kwargs], broadcast=True, worker_names=worker_names, worker_selector=worker_selector, worker_selector_args=worker_selector_args, worker_selector_kwargs=worker_selector_kwargs, workers_selected=workers_selected)
        iter_kwargs, worker_kws = await self.prepare_job_kws(iter_kwargs=[kwargs], timeout=timeout, broadcast=True, worker_names=worker_names, worker_selector=worker_selector, worker_selector_args=worker_selector_args, worker_selector_kwargs=worker_selector_kwargs, workers_selected=workers_selected, **kwargs)
        jobs: typing.List[Job] = []
        for kw in iter_kwargs:
            jobs.append(await self.enqueue(job_or_func, **kw))
        return jobs

    async def map(self, job_or_func: typing.Union[Job, str], iter_kwargs: typing.Iterable[typing.Dict], timeout: int=None, return_exceptions: bool=False, broadcast: typing.Optional[bool]=False, worker_names: typing.Optional[typing.List[str]]=None, worker_selector: typing.Optional[typing.Callable]=None, worker_selector_args: typing.Optional[typing.List]=None, worker_selector_kwargs: typing.Optional[typing.Dict]=None, workers_selected: typing.Optional[typing.List[typing.Dict[str, str]]]=None, **kwargs):
        """
        ...
        """
        iter_kwargs, worker_kws = await self.prepare_job_kws(iter_kwargs=iter_kwargs, timeout=timeout, broadcast=broadcast, worker_names=worker_names, worker_selector=worker_selector, workers_selected=workers_selected, worker_selector_args=worker_selector_args, worker_selector_kwargs=worker_selector_kwargs, **kwargs)
    ...
    async def batch_map(self, batch_func_kwargs: typing.Union[typing.List, typing.Dict], timeout: int=None, return_exceptions: bool=False, **kwargs):
        """
        ...
        if isinstance(batch_func_kwargs, dict):
            return await self.batch_map_dict(batch_func_kwargs, timeout=timeout, return_exceptions=return_exceptions, **kwargs)
        return await self.batch_map_list(batch_func_kwargs, timeout=timeout, return_exceptions=return_exceptions, **kwargs)

    async def batch_map_dict(self, batch_func_kwargs: typing.Dict[str, typing.Tuple[str, typing.Dict]], timeout: int=None, return_exceptions: bool=False, **kwargs):
        """
        Enqueue multiple individual as a result of multiple funcs collect all of their results.
        ...
        kwargs: Default kwargs for all jobs. These will be overridden by those in iter_kwargs.
        """
        batch_kwargs = {key: {'job_or_func': func, 'timeout': timeout, 'key': kw.get('key') or get_default_job_key(), **kwargs, **kw} for key, (func, kw) in batch_func_kwargs.items()}
        ...
        task = asyncio.create_task(self.listen(pending_job_keys, callback, timeout=None))
        try:
            await asyncio.gather(*[self.enqueue(**kw) for kw in list(batch_kwargs.values())])
        except:
            task.cancel()
            raise
        await asyncio.wait_for(task, timeout=timeout)
        ...
        for job, job_key in zip(jobs, job_dict):
            if job is None:
                ...
                results[job_dict[job.key]] = job.result
        return results

    async def batch_map_list(self, batch_func_kwargs: typing.List[typing.Tuple[str, typing.Dict[str, typing.Any]]], timeout: int=None, return_exceptions: bool=False, **kwargs):
        """
        Enqueue multiple individual as a result of multiple funcs collect all of their results.
        ...
        kwargs: Default kwargs for all jobs. These will be overridden by those in iter_kwargs.
        """
        batch_kwargs = [{'job_or_func': func, 'timeout': timeout, 'key': kw.get('key') or get_default_job_key(), **kwargs, **kw} for func, kw in batch_func_kwargs]
        ...
        task = asyncio.create_task(self.listen(pending_job_keys, callback, timeout=None))
        try:
            await asyncio.gather(*[self.enqueue(**kw) for kw in batch_kwargs])
        except:
            task.cancel()
            raise
        await asyncio.wait_for(task, timeout=timeout)
    ...
    async def batch(self):
        ...
        try:
            ...
            await asyncio.gather(*[self.abort(child, 'cancelled') for child in children], return_exceptions=True)
            raise
        finally:
            self.unregister_before_enqueue(track_child)
    '\n    New Helper Methods\n    '

    async def wait_for_jobs(self, jobs: typing.List[Job], source_job: typing.Optional[Job]=None, verbose: typing.Optional[bool]=False, raise_exceptions: typing.Optional[bool]=False, refresh_interval: typing.Optional[float]=0.5, **kwargs) -> typing.List[typing.Any]:
        """
        ...
        while jobs:
            for job in jobs:
                try:
                    ...
                    if verbose:
                        logger.error(f'Unable to refresh job {job}')
                    if raise_exceptions:
                        raise e
                    jobs.remove(job)
                if job.status == JobStatus.COMPLETE:
                    results.append(job.result)
                    if source_job:
                        await source_job.incr_progress()
                    jobs.remove(job)
        ...
        if verbose:
            if source_job:
                logger.info(f'Completed {source_job.id} w/ {len(results)}/{num_jobs} in {timer(t):.2f}s')
            else:
                logger.info(f'Completed {len(results)}/{num_jobs} jobs in {timer(t):.2f}s')
        return results

    async def as_jobs_complete(self, jobs: typing.List[Job], source_job: typing.Optional[Job]=None, verbose: typing.Optional[bool]=False, raise_exceptions: typing.Optional[bool]=False, refresh_interval: typing.Optional[float]=0.5, return_results: typing.Optional[bool]=True, cancel_func: typing.Optional[typing.Callable]=None, **kwargs) -> typing.AsyncGenerator[typing.Any, None]:
        """
        ...
        if verbose:
            if source_job:
                logger.info(f'Completed {source_job.id} w/ {num_results}/{num_jobs} in {timer(t):.2f}s')
            else:
                logger.info(f'Completed {num_results}/{num_jobs} jobs in {timer(t):.2f}s')
    ...
    async def batch_enqueue_iterator(self, job_or_func: typing.Union[Job, str], iterable: typing.Iterable[typing.Any], iterable_key: typing.Optional[str]=None, concurrency_limit: typing.Optional[int]=None, enqueue_func: typing.Optional[typing.Callable]=None, _verbose: typing.Optional[bool]=False, _raise_exceptions: typing.Optional[bool]=False, _return_results: typing.Optional[bool]=True, _key_func: typing.Optional[typing.Callable]=None, _cancel_func: typing.Optional[typing.Callable]=None, **kwargs) -> typing.AsyncGenerator[typing.Any, None]:
        """
        ...
        for batch in build_batches(iterable, concurrency_limit, fixed_batch_size=True):
            ...
            for item in batch:
                if _key_func:
                    kwargs['key'] = _key_func(item)
                if iterable_key:
                    kwargs[iterable_key] = item
                batch_jobs.append(await enqueue_func(job_or_func, *(() if iterable_key else item), **kwargs))
            async for completed_job in self.as_jobs_complete(jobs=batch_jobs, verbose=_verbose, raise_exceptions=_raise_exceptions, return_results=_return_results, cancel_func=_cancel_func):
                yield completed_job
                n_items += 1
            if _verbose:
                logger.info(f'[{job_or_func}] Completed {len(batch_jobs)} Batch Results in {timer(bt):.2f}s ({len(batch_jobs) / timer(bt):.2f} items/s)')
        if _verbose:
            logger.info(f'[{job_or_func}] Completed {n_items} Results in {timer(t):.2f}s ({n_items / timer(t):.2f} items/s)')
    ...
    async def _reset_queue_jobs(self, error: typing.Optional[typing.Union[str, Exception]]='Resetting Queue', ttl: typing.Optional[int]=5, status: typing.Optional[JobStatus]=JobStatus.ABORTED, verbose: typing.Optional[bool]=True, **kwargs):
        """
        ...
        if verbose:
            logger.info(f'Reset {len(job_keys)} jobs in {self.queue_name}')

    async def _reset_queue_trackers(self, verbose: typing.Optional[bool]=True, **kwargs):
        """
        ...
        if verbose:
            logger.info(f'Reset {len(tracker_keys)} function trackers in {self.queue_name}')

    async def reset_queue(self, jobs: typing.Optional[bool]=False, tracker: typing.Optional[bool]=False, verbose: typing.Optional[bool]=True, **kwargs):
        """
        Purges all keys from the queue
        """
        if jobs:
            await self._reset_queue_jobs(verbose=verbose, **kwargs)
        if tracker:
            await self._reset_queue_trackers(verbose=verbose, **kwargs)
    '\n    Queue Metadata\n    '

    async def fetch_job_info(self, offset: int=0, limit: int=10, **kwargs) -> typing.List[typing.Union[Job, typing.Dict]]:
        """
        Fetch all job info
        """
        job_info = []
        for job_bytes in await self.ctx.async_mget(await self.ctx.async_lrange(self.active_key, offset, limit - 1) + await self.ctx.async_lrange(self.queued_key, offset, limit - 1)):
    ...
    async def info(self, jobs: bool=False, offset: int=0, limit: int=10):
        ...
        worker_stats = await self.ctx.async_mget((self.create_namespace(f'stats:{worker_uuid}') for worker_uuid in worker_uuids))
        worker_info = {}
        ...
        try:
            async with anyio.fail_after(10):
                ...
                async with self.ctx.async_client.pipeline(transaction=True, retryable=True) as pipe:
                    ...
                    await pipe.setex(key, ttl, json.dumps(stats)).zremrangebyscore(self.stats_key, 0, current).zadd(self.stats_key, {key: current + millis(ttl)}).expire(self.stats_key, ttl).execute()
                return stats
    ...
    async def add_heartbeat(self, worker_id: str, worker_name: str, worker_attributes: typing.Dict[str, typing.Any]=None, heartbeat_ttl: typing.Optional[int]=None):
        """
        ...
        async with self.ctx.async_client.pipeline(transaction=True, retryable=True) as pipe:
            ...
            await pipe.setex(key, heartbeat_ttl, json.dumps(worker_attributes)).zremrangebyscore(self.heartbeat_key, 0, current).zadd(self.heartbeat_key, {worker_id: current + millis(heartbeat_ttl)}).expire(self.heartbeat_key, heartbeat_ttl).zremrangebyscore(self.worker_names_key, 0, current).zadd(self.worker_names_key, {worker_name: current + millis(heartbeat_ttl)}).expire(self.worker_names_key, heartbeat_ttl).hmset(self.worker_map_key, {worker_name: worker_id}).expire(self.worker_map_key, heartbeat_ttl).execute()
    ...
    def _queue_kind_key_map(self) -> typing.Dict[str, str]:
        ...
        """
        return {'queued': self.queued_key, 'active': self.active_key, 'scheduled': self.scheduled_key, 'incomplete': self.incomplete_key}
    ...
    async def _all_queue_jobs(self) -> typing.Dict[str, JobStatus]:
        ...
        if not job_ids:
            return {}
        return {job_id.decode('utf-8'): JobStatus(job_status.decode('utf-8')) for job_id, job_status in job_ids.items()}
    ...
    async def sweep_jobs_by_status(self, status: JobStatus, kind: typing.Optional[str]='active', key: typing.Optional[str]=None, postfix: typing.Optional[str]=None, verbose: typing.Optional[bool]=None, **kwargs):
        ...
        if not job_ids:
            if verbose:
                logger.info(f'No jobs found in {kind} queue')
            return
        if verbose:
            logger.info(f'Sweeping {len(job_ids)} jobs from {kind} queue with status {status}')
        ...
        async def _sweep_job(job_id: str):
            ...
            if job.status == status:
                if verbose:
                    logger.info(f'Sweeping job {job_id} with status {status}')
                await self.sweep_job(job=job, job_id=job_id, verbose=verbose)
            elif verbose:
                logger.info(f'Skipping job {job_id} with status {job.status}')
        async for _ in concurrent_map(_sweep_job, job_ids):
            pass
        return job_ids
    '\n    Metrics Handler\n    '
    ...
    async def prepare_for_broadcast(self, ttl: typing.Optional[int]=None):
        ...
        """
        worker_data = await self.ctx.async_mget((self.create_namespace(f'worker:attr:{worker_id}') for worker_id in self.active_workers))
        self.worker_attributes = {worker_id: json.loads(worker_attr.decode('UTF-8')) for worker_id, worker_attr in zip(self.active_workers, worker_data) if worker_attr}

    async def prepare_queue_metrics(self, worker_info: typing.Optional[typing.Dict[str, typing.Any]]=None, **kwargs) -> typing.Dict[str, typing.Any]:
        """
        Parse worker metrics from worker info.
        """
        return worker_info

    async def prepare_worker_metrics(self, **kwargs) -> typing.Dict:
        """
        Parse worker metrics from worker info.
        """
        pass
    ...
    async def _get_function_tracker(self, function: str, none_ok: typing.Optional[bool]=True) -> typing.Optional[FunctionTracker]:
        ...
        """
        if not self.function_tracker_enabled:
            return None
    ...
    async def anum_workers(self) -> int:
        ...
        """
        return await self.ctx.async_client.zcount(self.heartbeat_key, 1, 'inf')
    ...
    def node_name(self) -> str:
        return self._stats.node_name

    @lazyproperty
    def complete(self) -> int:
        return self._stats.complete
    ...
    def retried(self) -> int:
        return self._stats.retried

    @lazyproperty
    def aborted(self) -> int:
        return self._stats.aborted

    @lazyproperty
    def num_workers(self) -> int:
        return self._stats.num_workers

    @lazyproperty
    def active_key(self) -> str:
        return self._stats.active_key

    @lazyproperty
    def queued_key(self) -> str:
        return self._stats.queued_key

    @lazyproperty
    def scheduled_key(self) -> str:
        return self._stats.scheduled_key

    @lazyproperty
    def incomplete_key(self) -> str:
        return self._stats.incomplete_key

    @lazyproperty
    def function_tracker_key(self) -> str:
        return self._stats.function_tracker_key
    ...
    def sweep_key(self) -> str:
        return self._stats.sweep_key

    @lazyproperty
    def queue_info_key(self) -> str:
        return self._stats.queue_info_key

    @lazyproperty
    def worker_token_key(self) -> str:
        return self._stats.worker_token_key

    @lazyproperty
    def management_queue_key(self) -> str:
        return self._stats.management_queue_key

    @lazyproperty
    def heartbeat_key(self) -> str:
        return self._stats.heartbeat_key

    @lazyproperty
    def worker_names_key(self) -> str:
        return self._stats.worker_names_key

    @lazyproperty
    def worker_map_key(self) -> str:
        return self._stats.worker_map_key

    @lazyproperty
    def abort_id_prefix(self) -> str:
        return self._stats.abort_id_prefix

    @lazyproperty
    def active_workers(self) -> typing.List[str]:
        return self._stats.active_workers

    @lazyproperty
    def worker_attributes(self) -> typing.Dict[str, typing.Dict[str, typing.Any]]:
        return self._stats.worker_attributes

    def create_namespace(self, key: str) -> str:
        return self._stats.create_namespace(key)