...
class NetAppOntapVscanOnAccessPolicy:
    ...
    def __init__(self):
        ...
        self.module = AnsibleModule(argument_spec=self.argument_spec, supports_check_mode=True, mutually_exclusive=[['filters', 'scan_readonly_volumes'], ['filters', 'only_execute_access']])
    ...
    def set_playbook_zapi_key_map(self):
        self.na_helper.zapi_int_keys = {'max_file_size': 'max-file-size'}
        self.na_helper.zapi_str_keys = {'vserver': 'vserver', 'policy_name': 'policy-name'}
        self.na_helper.zapi_bool_keys = {'is_scan_mandatory': 'is-scan-mandatory', 'policy_status': 'is-policy-enabled', 'scan_files_with_no_ext': 'scan-files-with-no-ext'}
        self.na_helper.zapi_list_keys = {'file_ext_to_exclude': 'file-ext-to-exclude', 'file_ext_to_include': 'file-ext-to-include', 'paths_to_exclude': 'paths-to-exclude', 'filters': 'filters'}
    ...
    def create_on_access_policy(self):
        ...
        try:
            ...
            self.module.fail_json(msg='Error creating Vscan on Access Policy %s: %s' % (self.parameters['policy_name'], to_native(error)), exception=traceback.format_exc())
    ...
    def modify_on_access_policy(self, modify=None):
        ...
        try:
            ...
            self.module.fail_json(msg='Error Modifying Vscan on Access Policy %s: %s' % (self.parameters['policy_name'], to_native(error)), exception=traceback.format_exc())
    ...
    def modify_on_access_policy_rest(self, modify):
        ...
        if error:
            self.module.fail_json(msg='Error Modifying Vscan on Access Policy %s: %s' % (self.parameters['policy_name'], to_native(error)))
    ...
    def delete_on_access_policy_rest(self):
        ...
        if error:
            self.module.fail_json(msg='Error Deleting Vscan on Access Policy %s: %s' % (self.parameters['policy_name'], to_native(error)))
    ...
    def apply(self):
        ...
        result = netapp_utils.generate_result(self.na_helper.changed, cd_action, modify, extra_responses={'modify_policy_state': modify_policy_state})
