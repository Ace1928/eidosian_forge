...
def ParseCapacityCommitmentPath(path: str) -> Tuple[Optional[str], Optional[str], Optional[str]]:
  ...
  """
    ...
    if not match:
        raise bq_error.BigqueryError('Could not parse capacity commitment path: %s' % path)
