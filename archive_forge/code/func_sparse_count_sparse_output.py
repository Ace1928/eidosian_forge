...
def sparse_count_sparse_output(indices: _atypes.TensorFuzzingAnnotation[_atypes.Int64], values: _atypes.TensorFuzzingAnnotation[TV_SparseCountSparseOutput_T], dense_shape: _atypes.TensorFuzzingAnnotation[_atypes.Int64], weights: _atypes.TensorFuzzingAnnotation[TV_SparseCountSparseOutput_output_type], binary_output: bool, minlength: int=-1, maxlength: int=-1, name=None):
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'SparseCountSparseOutput', name, indices, values, dense_shape, weights, 'minlength', minlength, 'maxlength', maxlength, 'binary_output', binary_output)
            ...
            pass
        try:
            return sparse_count_sparse_output_eager_fallback(indices, values, dense_shape, weights, minlength=minlength, maxlength=maxlength, binary_output=binary_output, name=name, ctx=_ctx)
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('SparseCountSparseOutput', indices=indices, values=values, dense_shape=dense_shape, weights=weights, binary_output=binary_output, minlength=minlength, maxlength=maxlength, name=name)
    _result = _outputs[:]
    if _execute.must_record_gradient():
        ...
        _execute.record_gradient('SparseCountSparseOutput', _inputs_flat, _attrs, _result)
    _result = _SparseCountSparseOutputOutput._make(_result)
    return _result