...
@experimental
def answer_similarity(model: Optional[str]=None, metric_version: Optional[str]=None, examples: Optional[List[EvaluationExample]]=None) -> EvaluationMetric:
    """
    ...
    try:
        ...
        raise MlflowException(f'Failed to find answer similarity metric for version {metric_version}. Please check the version', error_code=INVALID_PARAMETER_VALUE) from None
    except Exception as e:
        raise MlflowException(f'Failed to construct answer similarity metric {metric_version}. Error: {e!r}', error_code=INTERNAL_ERROR) from None
    if examples is None:
