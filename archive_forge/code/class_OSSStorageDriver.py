...
class OSSStorageDriver(StorageDriver):
    ...
    def download_object(self, obj, destination_path, overwrite_existing=False, delete_on_failure=True):
        ...
        response = self.connection.request(obj_path, method='GET', raw=True, container=obj.container)
    ...
    def download_object_as_stream(self, obj, chunk_size=None):
        obj_path = self._get_object_path(obj.container, obj.name)
        response = self.connection.request(obj_path, method='GET', raw=True, container=obj.container)
        return self._get_object(obj=obj, callback=read_in_chunks, response=response, callback_kwargs={'iterator': response.response, 'chunk_size': chunk_size}, success_status_code=httplib.OK)

    def upload_object(self, file_path, container, object_name, extra=None, verify_hash=True, headers=None):
        return self._put_object(container=container, object_name=object_name, extra=extra, file_path=file_path, verify_hash=verify_hash)
    ...
    def upload_object_via_stream(self, iterator, container, object_name, extra=None, headers=None):
        ...
        if self.supports_multipart_upload:
            pass
        return self._put_object(container=container, object_name=object_name, extra=extra, method=method, query_args=params, stream=iterator, verify_hash=False, headers=headers)
    ...
    def ex_iterate_multipart_uploads(self, container, prefix=None, delimiter=None, max_uploads=MAX_UPLOADS_PER_RESPONSE):
        """
    ...
    def _upload_object(self, object_name, content_type, request_path, request_method='PUT', headers=None, file_path=None, stream=None, chunked=False, multipart=False, container=None):
        """
        ...
        if stream:
            response = self.connection.request(request_path, method=request_method, data=stream, headers=headers, raw=True, container=container)
            stream_hash, stream_length = self._hash_buffered_stream(stream, self._get_hash_function())
        else:
            with open(file_path, 'rb') as file_stream:
                response = self.connection.request(request_path, method=request_method, data=file_stream, headers=headers, raw=True, container=container)
            with open(file_path, 'rb') as file_stream:
    ...
    def _put_object(self, container, object_name, method='PUT', query_args=None, extra=None, file_path=None, stream=None, verify_hash=False, headers=None):
        """
        ...
        result_dict = self._upload_object(object_name=object_name, content_type=content_type, request_path=request_path, request_method=method, headers=headers, file_path=file_path, stream=stream, container=container)
    ...
    def _commit_multipart(self, object_path, upload_id, chunks, container=None):
        ...
        params = {'uploadId': upload_id}
        request_path = '?'.join((object_path, urlencode(params)))
        response = self.connection.request(request_path, data=data, method='POST', container=container)
        if response.status != httplib.OK:
    ...
    def _to_container(self, element):
        extra = {'creation_date': findtext(element=element, xpath='CreationDate', namespace=self.namespace), 'location': findtext(element=element, xpath='Location', namespace=self.namespace)}
    ...
    def _to_obj(self, element, container):
        ...
        meta_data = {'owner': {'id': owner_id, 'display_name': self._safe_decode(owner_display_name)}}
        ...
        obj = Object(name=name, size=int(findtext(element=element, xpath='Size', namespace=self.namespace)), hash=findtext(element=element, xpath='ETag', namespace=self.namespace).replace('"', ''), extra=extra, meta_data=meta_data, container=container, driver=self)
        return obj
    ...
    def _headers_to_object(self, object_name, container, headers):
        ...
        for key, value in headers.items():
            ...
            meta_data[key] = value
        obj = Object(name=object_name, size=int(headers['content-length']), hash=hash, extra=extra, meta_data=meta_data, container=container, driver=self)
        return obj