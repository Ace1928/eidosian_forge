...
class NonIterativeObjectiveLocalOptimizerTests:
    ...
    class Tests(TestCase):
        ...
        def test_transition_choice(self):
            d = dict(a=1, b=1, c=1)
            values = self._generate_values(TransitionChoice('a', 'b', 'c'), lambda x: d[x])
            assert len(values) > 0
            assert all((x in ['a', 'b', 'c'] for x in values))
            assert all((c in values for c in ['a', 'b', 'c']))
        ...
        def test_rand(self):
            ...
            values = self._generate_values(Rand(-2.0, 3.0, q=2.5, include_high=False), lambda x: x ** 2)
            assert_close(values, [-2.0, 0.5])
            for ih in [True, False]:
                values = self._generate_values(Rand(-2.0, 3.0, q=3.0, include_high=ih), lambda x: x ** 2)
            ...
            values = self._generate_values(Rand(1.0, 6.0, q=2.5, log=True), lambda x: x ** 2)
            assert_close(values, [1.0, 3.5, 6.0])
            values = self._generate_values(Rand(1.0, 6.0, q=2.5, log=True, include_high=False), lambda x: x ** 2)
            assert_close(values, [1.0, 3.5])
            for ih in [True, False]:
                values = self._generate_values(Rand(1.0, 6.0, q=3.0, log=True, include_high=ih), lambda x: x ** 2)
        ...
        def test_randint(self):
            for log in [True, False]:
                ...
                values = self._generate_values(RandInt(1, 3, include_high=False, log=log), lambda x: x ** 2)
                assert_close(values, [1, 2])
                for ih in [True, False]:
                    values = self._generate_values(RandInt(1, 6, 2, include_high=ih, log=log), lambda x: x ** 2)
                    assert_close(values, [1, 3, 5])
                    assert all((isinstance(x, int) for x in values))
                values = self._generate_values(RandInt(1, 5, 2, log=log), lambda x: x ** 2)
                assert_close(values, [1, 3, 5])
                values = self._generate_values(RandInt(1, 5, 2, include_high=False, log=log), lambda x: x ** 2)
