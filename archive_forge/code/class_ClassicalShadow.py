    ...
    **Example**
    ...
    def __init__(self, bits, recipes, wire_map=None):
        ...
        if bits.shape != recipes.shape:
            raise ValueError(f'Bits and recipes but have the same shape, got {bits.shape} and {recipes.shape}.')
        if bits.shape[1] != len(self.wire_map):
            raise ValueError(f'The 1st axis of bits must have the same size as wire_map, got {bits.shape[1]} and {len(self.wire_map)}.')
        self.observables = [qml.matrix(qml.X(0)), qml.matrix(qml.Y(0)), qml.matrix(qml.Z(0))]
    ...
    def expval(self, H, k=1):
        ...
        **Example**
        ...
        expvals = pauli_expval(self.bits, self.recipes, np.array([word for cw in coeffs_and_words for _, word in cw]))
