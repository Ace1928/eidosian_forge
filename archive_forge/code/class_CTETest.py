...
class CTETest(fixtures.TablesTest):
    ...
    def define_tables(cls, metadata):
        Table('some_table', metadata, Column('id', Integer, primary_key=True), Column('data', String(50)), Column('parent_id', ForeignKey('some_table.id')))
        Table('some_other_table', metadata, Column('id', Integer, primary_key=True), Column('data', String(50)), Column('parent_id', Integer))

    @classmethod
    def insert_data(cls, connection):
        connection.execute(cls.tables.some_table.insert(), [{'id': 1, 'data': 'd1', 'parent_id': None}, {'id': 2, 'data': 'd2', 'parent_id': 1}, {'id': 3, 'data': 'd3', 'parent_id': 1}, {'id': 4, 'data': 'd4', 'parent_id': 3}, {'id': 5, 'data': 'd5', 'parent_id': 3}])
    ...
    def test_update_from_round_trip(self, connection):
        ...
        eq_(connection.execute(select(some_other_table).order_by(some_other_table.c.id)).fetchall(), [(1, 'd1', None), (2, 'd2', 5), (3, 'd3', 5), (4, 'd4', 5), (5, 'd5', 3)])
