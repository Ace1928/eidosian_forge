...
class PartitionCountWatchingPublisher(Publisher):
    ...
    def __init__(self, watcher: PartitionCountWatcher, publisher_factory: Callable[[Partition], Publisher], policy_factory: Callable[[int], RoutingPolicy]):
        self._publishers = {}
    ...
    async def __aenter__(self):
        try:
            ...
            raise
        self._partition_count_poller = asyncio.ensure_future(self._watch_partition_count())
        return self
    ...
    async def _handle_partition_count_update(self, partition_count: int):
        ...
        if current_count > partition_count:
            return
        new_publishers = {Partition(index): self._publisher_factory(Partition(index)) for index in range(current_count, partition_count)}
