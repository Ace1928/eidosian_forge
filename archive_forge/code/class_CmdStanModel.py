...
class CmdStanModel:
    ...
    def __init__(self, model_name: Optional[str]=None, stan_file: OptionalPath=None, exe_file: OptionalPath=None, force_compile: bool=False, stanc_options: Optional[Dict[str, Any]]=None, cpp_options: Optional[Dict[str, Any]]=None, user_header: OptionalPath=None, *, compile: Union[bool, Literal['force'], None]=None) -> None:
        """
        ...
        self._exe_file = None
        self._compiler_options = compilation.CompilerOptions(stanc_options=stanc_options, cpp_options=cpp_options, user_header=user_header)
        self._compiler_options.validate()
        self._fixed_param = False
        if compile is None:
            compile = True
        else:
            get_logger().warning('CmdStanModel(compile=...) is deprecated and will be removed in the next major version. The constructor will always ensure a model has a compiled executable.\nIf you wish to force recompilation, use force_compile=True instead.')
        if force_compile:
            compile = 'force'
        if model_name is not None:
            get_logger().warning('CmdStanModel(model_name=...) is deprecated and will be removed in the next major version.')
            if not model_name.strip():
                raise ValueError('Invalid value for argument model name, found "{}"'.format(model_name))
            self._name = model_name.strip()
        if stan_file is None:
            if exe_file is None:
                raise ValueError('Missing model file arguments, you must specify either Stan source or executable program file or both.')
        else:
        ...
        if exe_file is not None:
            ...
            elif self._name != os.path.splitext(exename)[0]:
                raise ValueError('Name mismatch between Stan file and compiled executable, expecting basename: {} found: {}.'.format(self._name, exename))
        if platform.system() == 'Windows':
            try:
                ...
                os.environ['PATH'] = ';'.join(list(OrderedDict.fromkeys([libtbb] + os.environ.get('PATH', '').split(';'))))
            else:
    ...
    def format(self, overwrite_file: bool=False, canonicalize: Union[bool, str, Iterable[str]]=False, max_line_length: int=78, *, backup: bool=True) -> None:
        """
        ...
        """
        get_logger().warning('CmdStanModel.format() is deprecated and will be removed in the next major version.\nUse cmdstanpy.format_stan_file() instead.')
        if self.stan_file is None:
            raise ValueError('No Stan file found for this module')
        compilation.format_stan_file(self.stan_file, overwrite_file=overwrite_file, max_line_length=max_line_length, canonicalize=canonicalize, backup=backup, stanc_options=self.stanc_options)
    ...
    def compile(self, force: bool=False, stanc_options: Optional[Dict[str, Any]]=None, cpp_options: Optional[Dict[str, Any]]=None, user_header: OptionalPath=None, override_options: bool=False, *, _internal: bool=False) -> None:
        """
        ...
        """
        if not _internal:
            get_logger().warning('CmdStanModel.compile() is deprecated and will be removed in the next major version. To compile a model, use the CmdStanModel() constructor or cmdstanpy.compile_stan_file().')
        if not self._stan_file:
            raise RuntimeError('Please specify source file')
        compiler_options = None
        if stanc_options is not None or cpp_options is not None or user_header is not None:
            compiler_options = compilation.CompilerOptions(stanc_options=stanc_options, cpp_options=cpp_options, user_header=user_header)
        ...
        self._exe_file = compilation.compile_stan_file(str(self.stan_file), force=force, stanc_options=self._compiler_options.stanc_options, cpp_options=self._compiler_options.cpp_options, user_header=self._compiler_options.user_header)

    def optimize(self, data: Union[Mapping[str, Any], str, os.PathLike, None]=None, seed: Optional[int]=None, inits: Union[Mapping[str, Any], float, str, os.PathLike, None]=None, output_dir: OptionalPath=None, sig_figs: Optional[int]=None, save_profile: bool=False, algorithm: Optional[str]=None, init_alpha: Optional[float]=None, tol_obj: Optional[float]=None, tol_rel_obj: Optional[float]=None, tol_grad: Optional[float]=None, tol_rel_grad: Optional[float]=None, tol_param: Optional[float]=None, history_size: Optional[int]=None, iter: Optional[int]=None, save_iterations: bool=False, require_converged: bool=True, show_console: bool=False, refresh: Optional[int]=None, time_fmt: str='%Y%m%d%H%M%S', timeout: Optional[float]=None, jacobian: bool=False) -> CmdStanMLE:
        """
        ...
        """
        optimize_args = OptimizeArgs(algorithm=algorithm, init_alpha=init_alpha, tol_obj=tol_obj, tol_rel_obj=tol_rel_obj, tol_grad=tol_grad, tol_rel_grad=tol_rel_grad, tol_param=tol_param, history_size=history_size, iter=iter, save_iterations=save_iterations, jacobian=jacobian)
        if jacobian and cmdstan_version_before(2, 32, self.exe_info()):
            raise ValueError('Jacobian adjustment for optimization is only supported in CmdStan 2.32 and above.')
        with temp_single_json(data) as _data, temp_inits(inits, allow_multiple=False) as _inits:
            args = CmdStanArgs(self._name, self._exe_file, chain_ids=None, data=_data, seed=seed, inits=_inits, output_dir=output_dir, sig_figs=sig_figs, save_profile=save_profile, method_args=optimize_args, refresh=refresh)
            dummy_chain_id = 0
        ...
        if not runset._check_retcodes():
            msg = "Error during optimization! Command '{}' failed: {}".format(' '.join(runset.cmd(0)), runset.get_err_msgs())
    ...
    def sample(self, data: Union[Mapping[str, Any], str, os.PathLike, None]=None, chains: Optional[int]=None, parallel_chains: Optional[int]=None, threads_per_chain: Optional[int]=None, seed: Union[int, List[int], None]=None, chain_ids: Union[int, List[int], None]=None, inits: Union[Mapping[str, Any], float, str, List[str], List[Mapping[str, Any]], None]=None, iter_warmup: Optional[int]=None, iter_sampling: Optional[int]=None, save_warmup: bool=False, thin: Optional[int]=None, max_treedepth: Optional[int]=None, metric: Union[str, Dict[str, Any], List[str], List[Dict[str, Any]], None]=None, step_size: Union[float, List[float], None]=None, adapt_engaged: bool=True, adapt_delta: Optional[float]=None, adapt_init_phase: Optional[int]=None, adapt_metric_window: Optional[int]=None, adapt_step_size: Optional[int]=None, fixed_param: bool=False, output_dir: OptionalPath=None, sig_figs: Optional[int]=None, save_latent_dynamics: bool=False, save_profile: bool=False, show_progress: bool=True, show_console: bool=False, refresh: Optional[int]=None, time_fmt: str='%Y%m%d%H%M%S', timeout: Optional[float]=None, *, force_one_process_per_chain: Optional[bool]=None) -> CmdStanMCMC:
        ...
        if parallel_chains is None:
            ...
            get_logger().info('Requested %u parallel_chains but only %u required, will run all chains in parallel.', parallel_chains, chains)
            parallel_chains = chains
        elif parallel_chains < 1:
            raise ValueError('Argument parallel_chains must be a positive integer, found {}.'.format(parallel_chains))
        if threads_per_chain is None:
            threads_per_chain = 1
        if threads_per_chain < 1:
            raise ValueError('Argument threads_per_chain must be a positive integer, found {}.'.format(threads_per_chain))
        ...
        if force_one_process_per_chain is None and (not cmdstan_version_before(2, 28, info_dict)) and (stan_threads == 'true'):
        ...
        if force_one_process_per_chain is False:
            if not cmdstan_version_before(2, 28, info_dict):
                ...
                if stan_threads == 'false':
                    get_logger().warning('Stan program not compiled for threading, process will run chains sequentially. For multi-chain parallelization, recompile the model with argument "cpp_options={\'STAN_THREADS\':\'TRUE\'}.')
            else:
                get_logger().warning('Installed version of CmdStan cannot multi-process chains, will run %d processes. Run "install_cmdstan" to upgrade to latest version.', chains)
        os.environ['STAN_NUM_THREADS'] = str(num_threads)
        if chain_ids is None:
            chain_ids = [i + 1 for i in range(chains)]
        elif isinstance(chain_ids, int):
            if chain_ids < 1:
                raise ValueError('Chain_id must be a positive integer value, found {}.'.format(chain_ids))
            chain_ids = [i + chain_ids for i in range(chains)]
        else:
            if not one_process_per_chain:
                for i, j in zip(chain_ids, chain_ids[1:]):
                    if i != j - 1:
                        raise ValueError('chain_ids must be sequential list of integers, found {}.'.format(chain_ids))
            if not len(chain_ids) == chains:
                raise ValueError('Chain_ids must correspond to number of chains specified {} chains, found {} chain_ids.'.format(chains, len(chain_ids)))
            for chain_id in chain_ids:
                if chain_id < 0:
                    raise ValueError('Chain_id must be a non-negative integer value, found {}.'.format(chain_id))
        sampler_args = SamplerArgs(num_chains=1 if one_process_per_chain else chains, iter_warmup=iter_warmup, iter_sampling=iter_sampling, save_warmup=save_warmup, thin=thin, max_treedepth=max_treedepth, metric=metric, step_size=step_size, adapt_engaged=adapt_engaged, adapt_delta=adapt_delta, adapt_init_phase=adapt_init_phase, adapt_metric_window=adapt_metric_window, adapt_step_size=adapt_step_size, fixed_param=fixed_param)
        with temp_single_json(data) as _data, temp_inits(inits, id=chain_ids[0]) as _inits:
            ...
            if one_process_per_chain and isinstance(inits, list):
                cmdstan_inits = [f'{_inits[:-5]}_{i}.json' for i in chain_ids]
            else:
                cmdstan_inits = _inits
            args = CmdStanArgs(self._name, self._exe_file, chain_ids=chain_ids, data=_data, seed=seed, inits=cmdstan_inits, output_dir=output_dir, sig_figs=sig_figs, save_latent_dynamics=save_latent_dynamics, save_profile=save_profile, method_args=sampler_args, refresh=refresh)
            if show_console:
                show_progress = False
            else:
            ...
            if show_progress:
                ...
                progress_hook = self._wrap_sampler_progress_hook(chain_ids=chain_ids, total=iter_total)
            runset = RunSet(args=args, chains=chains, chain_ids=chain_ids, time_fmt=time_fmt, one_process_per_chain=one_process_per_chain)
            with ThreadPoolExecutor(max_workers=parallel_procs) as executor:
                for i in range(runset.num_procs):
                    executor.submit(self._run_cmdstan, runset=runset, idx=i, show_progress=show_progress, show_console=show_console, progress_hook=progress_hook, timeout=timeout)
            if show_progress and progress_hook is not None:
                progress_hook('Done', -1)
    ...
    def log_prob(self, params: Union[Dict[str, Any], str, os.PathLike], data: Union[Mapping[str, Any], str, os.PathLike, None]=None, *, jacobian: bool=True, sig_figs: Optional[int]=None) -> pd.DataFrame:
        """
        ...
        with temp_single_json(data) as _data, temp_single_json(params) as _params:
            cmd = [str(self.exe_file), 'log_prob', f'constrained_params={_params}', f'jacobian={int(jacobian)}']
            ...
            if proc.returncode:
                get_logger().error("'log_prob' command failed!\nstdout:%s\nstderr:%s", proc.stdout, proc.stderr)
                raise RuntimeError("Method 'log_prob' failed with return code " + str(proc.returncode))
            result = pd.read_csv(output, comment='#')
            return result

    def laplace_sample(self, data: Union[Mapping[str, Any], str, os.PathLike, None]=None, mode: Union[CmdStanMLE, str, os.PathLike, None]=None, draws: Optional[int]=None, *, jacobian: bool=True, seed: Optional[int]=None, output_dir: OptionalPath=None, sig_figs: Optional[int]=None, save_profile: bool=False, show_console: bool=False, refresh: Optional[int]=None, time_fmt: str='%Y%m%d%H%M%S', timeout: Optional[float]=None, opt_args: Optional[Dict[str, Any]]=None) -> CmdStanLaplace:
        """
        ...
        if cmdstan_version_before(2, 32, self.exe_info()):
            raise ValueError("Method 'laplace_sample' not available for CmdStan versions before 2.32")
        if opt_args is not None and mode is not None:
            raise ValueError("Cannot specify both 'opt_args' and 'mode' arguments")
        if mode is None:
            optimize_args = {'seed': seed, 'sig_figs': sig_figs, 'jacobian': jacobian, 'save_profile': save_profile, 'show_console': show_console, 'refresh': refresh, 'time_fmt': time_fmt, 'timeout': timeout, 'output_dir': output_dir}
            optimize_args.update(opt_args or {})
            optimize_args['time_fmt'] = 'opt-' + time_fmt
            try:
                cmdstan_mode: CmdStanMLE = self.optimize(data=data, **optimize_args)
            except Exception as e:
                raise RuntimeError('Failed to run optimizer on model. Consider supplying a mode or additional optimizer args') from e
        elif not isinstance(mode, CmdStanMLE):
            cmdstan_mode = from_csv(mode)
        else:
            cmdstan_mode = mode
        if cmdstan_mode.runset.method != Method.OPTIMIZE:
            raise ValueError('Mode must be a CmdStanMLE or a path to an optimize CSV')
        mode_jacobian = cmdstan_mode.runset._args.method_args.jacobian
        if mode_jacobian != jacobian:
            raise ValueError(f'Jacobian argument to optimize and laplace must match!\nLaplace was run with jacobian={jacobian},\nbut optimize was run with jacobian={mode_jacobian}')
        laplace_args = LaplaceArgs(cmdstan_mode.runset.csv_files[0], draws, jacobian)
        with temp_single_json(data) as _data:
            args = CmdStanArgs(self._name, self._exe_file, chain_ids=None, data=_data, seed=seed, output_dir=output_dir, sig_figs=sig_figs, save_profile=save_profile, method_args=laplace_args, refresh=refresh)
            dummy_chain_id = 0
            runset = RunSet(args=args, chains=1, time_fmt=time_fmt)
            self._run_cmdstan(runset, dummy_chain_id, show_console=show_console, timeout=timeout)
        runset.raise_for_timeouts()
