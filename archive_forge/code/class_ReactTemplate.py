...
class ReactTemplate(BasicTemplate):
    ...
    dimensions = param.Dict(default={'minW': 0, 'maxW': float('inf'), 'minH': 0, 'maxH': float('inf')}, doc='A dictionary of minimum/maximum width/height in grid units.')
    ...
    _resources = {'js': {'react': f'{config.npm_cdn}/react@17/umd/react.production.min.js', 'react-dom': f'{config.npm_cdn}/react-dom@17/umd/react-dom.production.min.js', 'babel': f'{config.npm_cdn}/babel-standalone@latest/babel.min.js', 'react-grid': f'{config.npm_cdn}/react-grid-layout@1.3.4/dist/react-grid-layout.min.js'}, 'css': {'font-awesome': CSS_URLS['font-awesome']}}
    ...
    @depends('cols', 'breakpoints', 'row_height', 'compact', 'dimensions', 'prevent_collision', 'save_layout', watch=True)
