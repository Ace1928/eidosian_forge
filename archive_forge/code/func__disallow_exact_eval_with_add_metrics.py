...
def _disallow_exact_eval_with_add_metrics(self):
    ...
    if any([metric not in compiled_metrics for metric in metrics_from_add_metric]):
        raise ValueError('Detected that a metric was added to this model via `Model.add_metric`. This is not currently supported when using exact evaluation with `tf.distribute.ParameterServerStrategy`.')