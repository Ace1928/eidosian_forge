...
class CMSTest(utils.TestCase, testresources.ResourcedTestCase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        process = subprocess.Popen(['openssl', 'version'], stdout=subprocess.PIPE)
        out, err = process.communicate()
        openssl_version = out.split()[1]
        if err or openssl_version.startswith(b'0'):
            raise Exception('Your version of OpenSSL is not supported. You will need to update it to 1.0 or later.')
    ...
    def test_cms_verify_token_scoped_expired(self):
        cms_content = cms.token_to_cms(self.examples.SIGNED_TOKEN_SCOPED_EXPIRED)
        self.assertTrue(cms.cms_verify(cms_content, self.examples.SIGNING_CERT_FILE, self.examples.SIGNING_CA_FILE))
