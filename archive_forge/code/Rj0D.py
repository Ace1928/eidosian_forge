...
class KANWrapper:
    def __init__(
        self, width: List[int], grid: int, k: int, learnable_params: dict = None
    ) -> None:
        """
        ...
        Args:
            ...
            learnable_params (dict): Optional dictionary of additional learnable parameters.
        """
        self.width = width
        self.grid = grid
        self.k = k
    ...
    def _initialize_parameters(self) -> None:
        ...
        def _initialize_param(param: torch.nn.Parameter) -> None:
            """
            Initialize a model parameter.

            Args:
                param (torch.nn.Parameter): The parameter to initialize.
            """
            param.data.copy_(_initialize_tensor(param))

        def _initialize_learnable_param(key: str, value: Any) -> None:
            """
            Initialize a learnable parameter.

            Args:
                key (str): The key of the learnable parameter.
                value (Any): The value of the learnable parameter.
            """
            tensor_value = (
                value if isinstance(value, torch.Tensor) else torch.tensor(value)
            )
            self.learnable_params[key] = _initialize_tensor(tensor_value)

        # Initialize model parameters
        for param in self.model.parameters():
            _initialize_param(param)

        # Initialize learnable parameters
        for key, value in self.learnable_params.items():
            _initialize_learnable_param(key, value)
    ...
    def _initialize_missing_keys(self) -> None:
        ...
        def _initialize_key(key: str) -> None:
            """
            Initialize a missing key in learnable_params with a log-normal distribution.
        ...
        missing_keys = [
            key for key in required_keys if key not in self.learnable_params
        ]
