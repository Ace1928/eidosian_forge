...
def _update_impl(self, state: InstanceState[Any], revert_deletion: bool=False) -> None:
    ...
    if state._deleted:
        if revert_deletion:
            ...
            raise sa_exc.InvalidRequestError("Instance '%s' has been deleted.  Use the make_transient() function to send this object back to the transient state." % state_str(state))
    obj = state.obj()
    if obj is None:
        return
