...
def reloo(wrapper, loo_orig=None, k_thresh=0.7, scale=None, verbose=True):
    ...
    if not_implemented:
        raise TypeError(f'Passed wrapper instance does not implement all methods required for reloo to work. Check the documentation of SamplingWrapper. {not_implemented} must be implemented and were not found.')
    if loo_orig is None:
