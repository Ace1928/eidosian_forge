  ...
  *{command}* is used to remove a backend from a backend
  ...
  """
    ...
    def Args(cls, parser):
        ...
        flags.AddInstanceGroupAndNetworkEndpointGroupArgs(parser, 'remove from', support_global_neg=cls.support_global_neg, support_region_neg=cls.support_region_neg)
    ...
    def Modify(self, client, resources, backend_service_ref, args, existing):
        ...
        if backend_idx is None:
            ...
            if scope_value is None:
                ...
                scope = 'region'
            raise exceptions.ArgumentError('Backend [{0}] in {1} [{2}] is not a backend of backend service [{3}].'.format(group_ref.Name(), scope, scope_value, backend_service_ref.Name()))
        else:
    ...
    def Run(self, args):
        ...
        new_object = self.Modify(client, holder.resources, backend_service_ref, args, objects[0])
        return client.MakeRequests([self.GetSetRequest(client, backend_service_ref, new_object)])