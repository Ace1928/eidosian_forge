...
class KernelRestarter(LoggingConfigurable):
    ...
    debug = Bool(False, config=True, help='Whether to include every poll event in debugging output.\n\n        Has to be set explicitly, because there will be *a lot* of output.\n        ')
    time_to_dead = Float(3.0, config=True, help='Kernel heartbeat interval in seconds.')
    stable_start_time = Float(10.0, config=True, help='The time in seconds to consider the kernel to have completed a stable start up.')
    restart_limit = Integer(5, config=True, help='The number of consecutive autorestarts before the kernel is presumed dead.')
    random_ports_until_alive = Bool(True, config=True, help='Whether to choose new random ports when restarting before the kernel is alive.')
    ...
    def poll(self) -> None:
        ...
        if not self.kernel_manager.is_alive():
            ...
            if self._restart_count > self.restart_limit:
                ...
                self.log.info('KernelRestarter: restarting kernel (%i/%i), %s random ports', self._restart_count, self.restart_limit, 'new' if newports else 'keep')
