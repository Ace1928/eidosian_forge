...
class OnAppDNSDriver(DNSDriver):
    ...
    RECORD_TYPE_MAP = {RecordType.SOA: 'SOA', RecordType.NS: 'NS', RecordType.A: 'A', RecordType.AAAA: 'AAAA', RecordType.CNAME: 'CNAME', RecordType.MX: 'MX', RecordType.TXT: 'TXT', RecordType.SRV: 'SRV'}
    ...
    def create_zone(self, domain, type='master', ttl=None, extra=None):
        ...
        response = self.connection.request('/dns_zones.json', method='POST', headers={'Content-type': 'application/json'}, data=dns_zone_data)
        zone = self._to_zone(response.object)
        return zone
    ...
    def get_record(self, zone_id, record_id):
        ...
        :rtype: :class:`Record`
        """
        response = self.connection.request('/dns_zones/{}/records/{}.json'.format(zone_id, record_id))
    ...
    def create_record(self, name, zone, type, data, extra=None):
        ...
        response = self.connection.request('/dns_zones/%s/records.json' % zone.id, method='POST', headers={'Content-type': 'application/json'}, data=dns_record_data)
    ...
    def update_record(self, record, name, type, data, extra=None):
        ...
        dns_record = self._format_record(name, type, data, extra)
        dns_record_data = json.dumps({'dns_record': dns_record})
        self.connection.request('/dns_zones/{}/records/{}.json'.format(zone.id, record.id), method='PUT', headers={'Content-type': 'application/json'}, data=dns_record_data)
    ...
    def delete_record(self, record):
        ...
        self.connection.request('/dns_zones/{}/records/{}.json'.format(zone_id, record.id), method='DELETE')
        return True
    ...
    def _format_record(self, name, type, data, extra):
        ...
        new_record = {'name': name, 'ttl': extra.get('ttl', DEFAULT_ZONE_TTL), 'type': record_type}
        if type == RecordType.MX:
            additions = {'priority': extra.get('priority', 1), 'hostname': extra.get('hostname')}
        elif type == RecordType.SRV:
            additions = {'port': extra.get('port'), 'weight': extra.get('weight', 1), 'priority': extra.get('priority', 1), 'hostname': extra.get('hostname')}
    ...
    def _to_zone(self, data):
        ...
        extra = {'user_id': dns_zone.get('user_id'), 'cdn_reference': dns_zone.get('cdn_reference'), 'created_at': dns_zone.get('created_at'), 'updated_at': dns_zone.get('updated_at')}
        type = 'master'
        return Zone(id=id, domain=name, type=type, ttl=DEFAULT_ZONE_TTL, driver=self, extra=extra)
    ...
    def _to_record(self, data, zone_id=None, zone=None):
        ...
        return Record(id=id, name=name, type=type, data=record, zone=zone, driver=self, ttl=ttl, extra={})