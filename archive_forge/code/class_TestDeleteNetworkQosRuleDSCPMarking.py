...
class TestDeleteNetworkQosRuleDSCPMarking(TestNetworkQosRule):
    ...
    def setUp(self):
        ...
        self.network_client.delete_qos_dscp_marking_rule = mock.Mock(return_value=None)
        self.network_client.find_qos_dscp_marking_rule = network_fakes.FakeNetworkQosRule.get_qos_rules(qos_rules=self.new_rule)
    ...
    def test_qos_policy_delete(self):
        ...
        self.network_client.delete_qos_dscp_marking_rule.assert_called_once_with(self.new_rule.id, self.qos_policy.id)
        self.assertIsNone(result)

    def test_qos_policy_delete_error(self):
        arglist = [self.new_rule.qos_policy_id, self.new_rule.id]
        verifylist = [('qos_policy', self.new_rule.qos_policy_id), ('id', self.new_rule.id)]
        self.network_client.delete_qos_dscp_marking_rule.side_effect = Exception('Error message')
        try:
            parsed_args = self.check_parser(self.cmd, arglist, verifylist)
