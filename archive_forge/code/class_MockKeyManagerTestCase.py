...
class MockKeyManagerTestCase(test_key_mgr.KeyManagerTestCase):
    ...
    def test_create_key_null_context(self):
        self.assertRaises(exception.Forbidden, self.key_mgr.create_key, None)

    def test_create_key_pair(self):
        for length in [2048, 3072, 4096]:
            name = str(length) + ' key'
            private_key_uuid, public_key_uuid = self.key_mgr.create_key_pair(self.context, 'RSA', length, name=name)
    ...
    def test_create_key_pair_invalid_algorithm(self):
        self.assertRaises(ValueError, self.key_mgr.create_key_pair, self.context, 'DSA', 2048)
    ...
    def test_store_key_and_get_metadata_and_get_key(self):
        ...
        self.assertIsNone(actual_key.get_encoded())
        self.assertTrue(actual_key.is_metadata_only())
        actual_key = self.key_mgr.get(self.context, key_id, metadata_only=False)
    ...
    def test_get_unknown_key(self):
        self.assertRaises(exception.ManagedObjectNotFoundError, self.key_mgr.get, self.context, None)

    def test_delete_key(self):
        key_id = self.key_mgr.create_key(self.context)
        self.key_mgr.delete(self.context, key_id)
        self.assertRaises(exception.ManagedObjectNotFoundError, self.key_mgr.get, self.context, key_id)

    def test_delete_null_context(self):
        self.assertRaises(exception.Forbidden, self.key_mgr.delete, None, None)

    def test_delete_unknown_key(self):
        self.assertRaises(exception.ManagedObjectNotFoundError, self.key_mgr.delete, self.context, None)
