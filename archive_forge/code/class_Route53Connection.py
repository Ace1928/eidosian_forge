...
class Route53Connection(AWSAuthConnection):
    ...
    def __init__(self, aws_access_key_id=None, aws_secret_access_key=None, port=None, proxy=None, proxy_port=None, host=DefaultHost, debug=0, security_token=None, validate_certs=True, https_connection_factory=None, profile_name=None):
        super(Route53Connection, self).__init__(host, aws_access_key_id, aws_secret_access_key, True, port, proxy, proxy_port, debug=debug, security_token=security_token, validate_certs=validate_certs, https_connection_factory=https_connection_factory, profile_name=profile_name)
    ...
    def make_request(self, action, path, headers=None, data='', params=None):
        if params:
            ...
            path += '?' + '&'.join(pairs)
        return super(Route53Connection, self).make_request(action, path, headers, data, retry_handler=self._retry_handler)
    ...
    def _retry_handler(self, response, i, next_sleep):
        ...
        if response.status == 400:
            ...
            if err.error_code:
                if not err.error_code in ('PriorRequestNotComplete', 'Throttling', 'ServiceUnavailable', 'RequestExpired'):
                    return status
