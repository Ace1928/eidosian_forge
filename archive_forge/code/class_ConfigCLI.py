...
class ConfigCLI(CLI):
    ...
    def init_parser(self):
        super(ConfigCLI, self).init_parser(desc='View ansible configuration.')
        common = opt_help.ArgumentParser(add_help=False)
        opt_help.add_verbosity_options(common)
        common.add_argument('-c', '--config', dest='config_file', help='path to configuration file, defaults to first file found in precedence.')
        common.add_argument('-t', '--type', action='store', default='base', dest='type', choices=['all', 'base'] + list(C.CONFIGURABLE_PLUGINS), help='Filter down to a specific plugin type.')
        ...
        list_parser.add_argument('--format', '-f', dest='format', action='store', choices=['json', 'yaml'], default='yaml', help='Output format for list')
        ...
        dump_parser.add_argument('--only-changed', '--changed-only', dest='only_changed', action='store_true', help='Only show configurations that have changed from the default')
        dump_parser.add_argument('--format', '-f', dest='format', action='store', choices=['json', 'yaml', 'display'], default='display', help='Output format for dump')
