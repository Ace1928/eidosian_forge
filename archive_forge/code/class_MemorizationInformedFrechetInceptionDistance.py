...
class MemorizationInformedFrechetInceptionDistance(Metric):
    ...
    def __init__(self, feature: Union[int, Module]=2048, reset_real_features: bool=True, normalize: bool=False, cosine_distance_eps: float=0.1, **kwargs: Any) -> None:
        super().__init__(**kwargs)
        if isinstance(feature, int):
            if not _TORCH_FIDELITY_AVAILABLE:
                raise ModuleNotFoundError('MemorizationInformedFrechetInceptionDistance metric requires that `Torch-fidelity` is installed. Either install as `pip install torchmetrics[image]` or `pip install torch-fidelity`.')
    ...
    def compute(self) -> Tensor:
        ...
        return _mifid_compute(mean_real, cov_real, real_features, mean_fake, cov_fake, fake_features, cosine_distance_eps=self.cosine_distance_eps).to(self.orig_dtype)
