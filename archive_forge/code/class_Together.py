...
@deprecated(since='0.0.12', removal='0.2', alternative_import='langchain_together.Together')
class Together(LLM):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        values['together_api_key'] = convert_to_secret_str(get_from_dict_or_env(values, 'together_api_key', 'TOGETHER_API_KEY'))
        return values
    ...
    def _call(self, prompt: str, stop: Optional[List[str]]=None, run_manager: Optional[CallbackManagerForLLMRun]=None, **kwargs: Any) -> str:
        ...
        """
        headers = {'Authorization': f'Bearer {self.together_api_key.get_secret_value()}', 'Content-Type': 'application/json'}
        ...
        if response.status_code >= 500:
            ...
            raise Exception(f'Together returned an unexpected response with status {response.status_code}: {response.text}')
        data = response.json()
        ...
        async with ClientSession() as session:
            async with session.post(self.base_url, json=payload, headers=headers) as response:
                if response.status >= 500:
                    ...
                    raise Exception(f'Together returned an unexpected response with status {response.status}: {response.text}')
