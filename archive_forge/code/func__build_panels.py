...
Returns
    ...
    panels = pd.DataFrame(dict(axes=_nones, relsize=_nones, lift=_nones, height=_nones, used2nd=[False] * len(pset), title=_nones, y_on_right=_nones), index=pset)
    ...
    if isinstance(scale_padding, dict):
        if 'left' in scale_padding:
            scale_left = scale_padding['left']
        if 'right' in scale_padding:
            scale_right = scale_padding['right']
        if 'top' in scale_padding:
            scale_top = scale_padding['top']
        if 'bottom' in scale_padding:
            scale_bot = scale_padding['bottom']
    else:
