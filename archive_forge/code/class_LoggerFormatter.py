...
class LoggerFormatter:
    ...
    def default_formatter(cls, record: Dict[str, Union[Dict[str, Any], Any]]) -> str:
        ...
        if 'result=tensor([' not in str(record['message']):
            return '<level>{level: <8}</> <green>{time:YYYY-MM-DD HH:mm:ss.SSS}</>: ' + extra + '<level>{message}</level>' + RESET_COLOR + '\n'
        msg = str(record['message'])[:100].replace('{', '(').replace('}', ')')
        return '<level>{level: <8}</> <green>{time:YYYY-MM-DD HH:mm:ss.SSS}</>: ' + extra + '<level>' + msg + f'</level>{RESET_COLOR}\n'