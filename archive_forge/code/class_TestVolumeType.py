...
class TestVolumeType(base.BaseFunctionalTest):
    ...
    def setUp(self):
        ...
        volume_type = {'name': 'test-volume-type', 'description': None, 'os-volume-type-access:is_public': False}
    ...
    def tearDown(self):
        ...
        if ret.get('id'):
            self.operator_cloud.block_storage.delete('/types/{volume_type_id}'.format(volume_type_id=ret.id))
    ...
    def test_remove_volume_type_access_missing_volume(self):
        with testtools.ExpectedException(exceptions.SDKException, 'VolumeType not found.*'):
            self.operator_cloud.remove_volume_type_access('MISSING_VOLUME_TYPE', self.operator_cloud.current_project_id)
