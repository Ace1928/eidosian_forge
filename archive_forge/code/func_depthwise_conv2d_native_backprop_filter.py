...
def depthwise_conv2d_native_backprop_filter(input: _atypes.TensorFuzzingAnnotation[TV_DepthwiseConv2dNativeBackpropFilter_T], filter_sizes: _atypes.TensorFuzzingAnnotation[_atypes.Int32], out_backprop: _atypes.TensorFuzzingAnnotation[TV_DepthwiseConv2dNativeBackpropFilter_T], strides, padding: str, explicit_paddings=[], data_format: str='NHWC', dilations=[1, 1, 1, 1], name=None) -> _atypes.TensorFuzzingAnnotation[TV_DepthwiseConv2dNativeBackpropFilter_T]:
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'DepthwiseConv2dNativeBackpropFilter', name, input, filter_sizes, out_backprop, 'strides', strides, 'padding', padding, 'explicit_paddings', explicit_paddings, 'data_format', data_format, 'dilations', dilations)
            return _result
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('DepthwiseConv2dNativeBackpropFilter', input=input, filter_sizes=filter_sizes, out_backprop=out_backprop, strides=strides, padding=padding, explicit_paddings=explicit_paddings, data_format=data_format, dilations=dilations, name=name)
    _result = _outputs[:]
