...
class AzureAiServicesDocumentIntelligenceTool(BaseTool):
    ...
    description: str = 'A wrapper around Azure AI Services Document Intelligence. Useful for when you need to extract text, tables, and key-value pairs from documents. Input should be a url to a document.'
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        try:
            ...
            values['doc_analysis_client'] = DocumentAnalysisClient(endpoint=azure_ai_services_endpoint, credential=AzureKeyCredential(azure_ai_services_key))
        except ImportError:
            raise ImportError('azure-ai-formrecognizer is not installed. Run `pip install azure-ai-formrecognizer` to install.')
        return values
