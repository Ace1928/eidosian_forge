...
@keras_export('keras.layers.Hashing', 'keras.layers.experimental.preprocessing.Hashing')
class Hashing(base_layer.Layer):
    ...
    **Example (Siphash64 with a single integer, same as `salt=[133, 133]`)**
    ...
    def __init__(self, num_bins, mask_value=None, salt=None, output_mode='int', sparse=False, **kwargs):
        if num_bins is None or num_bins <= 0:
            raise ValueError(f'The `num_bins` for `Hashing` cannot be `None` or non-positive values. Received: num_bins={num_bins}.')
        ...
        if salt is not None:
            if isinstance(salt, (tuple, list)) and len(salt) == 2:
                ...
                raise ValueError(f'The `salt` argument for `Hashing` can only be a tuple of size 2 integers, or a single integer. Received: salt={salt}.')

    def call(self, inputs):
        inputs = utils.ensure_tensor(inputs)
        if isinstance(inputs, tf.SparseTensor):
            indices = tf.SparseTensor(indices=inputs.indices, values=self._hash_values_to_bins(inputs.values), dense_shape=inputs.dense_shape)
        else:
            indices = self._hash_values_to_bins(inputs)
        return utils.encode_categorical_inputs(indices, output_mode=self.output_mode, depth=self.num_bins, sparse=self.sparse, dtype=self.compute_dtype)
    ...
    def get_config(self):
        ...
        config.update({'num_bins': self.num_bins, 'salt': self.salt, 'mask_value': self.mask_value, 'output_mode': self.output_mode, 'sparse': self.sparse})
        return config