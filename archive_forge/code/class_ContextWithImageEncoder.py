...
class ContextWithImageEncoder(TransformerEncoder):
    ...
    def __init__(self, n_heads, n_layers, embedding_size, ffn_size, vocabulary_size, embedding=None, dropout=0.0, attention_dropout=0.0, relu_dropout=0.0, padding_idx=0, learn_positional_embeddings=False, embeddings_scale=False, n_positions=1024, activation='relu', variant='aiayn', n_segments=0, output_scaling=1.0, image_encoder_num_layers=1, image_features_dim=2048, image_combination_mode='append', n_image_tokens=1, fusion='late', n_image_channels=1):
        """
        ...
        if self.image_combination_mode == 'add' and self.n_image_tokens > 1:
            raise ValueError('Image encoding cannot be added to context encoding if there is more than one image token!')
        if self.fusion is FusionType.EARLY:
            assert n_segments == 2, 'must use segment embeddings for early fusion'
        reduction_type = None
        super().__init__(n_heads=n_heads, n_layers=n_layers, embedding_size=embedding_size, ffn_size=ffn_size, vocabulary_size=vocabulary_size, embedding=embedding, dropout=dropout, attention_dropout=attention_dropout, relu_dropout=relu_dropout, padding_idx=padding_idx, learn_positional_embeddings=learn_positional_embeddings, embeddings_scale=embeddings_scale, reduction_type=reduction_type, n_positions=n_positions, activation=activation, variant=variant, n_segments=n_segments, output_scaling=output_scaling)
    ...
    def _build_image_encoder(self):
        ...
        for _ in range(self.n_img_layers - 1):
            image_layers += [nn.ReLU(), nn.Dropout(p=self.dropout_frac), nn.Linear(self.full_embedding_size, self.full_embedding_size)]
        self.image_encoder = nn.Sequential(*image_layers)

    def encode_images(self, images: Union[List[object], torch.Tensor], positions: Optional[torch.LongTensor]=None, segments: Optional[torch.LongTensor]=None) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Encode Images.
        ...
        valid_inds = [i for i, img in enumerate(images) if img is not None and isinstance(img, torch.Tensor)]
        if valid_inds:
            image_mask_list = []
            image_encoded_list = []
            ...
            for i in range(len(images)):
                if i in valid_inds:
                    ...
                    image_encoded_list.append(self._add([valid_img_enc[img_num, :], positions[i], segments[i]]))
                    img_num += 1
                else:
                    image_mask_list.append(~self.ones_mask)
                    image_encoded_list.append(self.dummy_image_enc)
            image_masks = torch.stack(image_mask_list)
            image_encoded = torch.stack(image_encoded_list).reshape((len(images), self.n_image_tokens * self.n_image_channels, self.embedding_size))
        else:
            image_encoded = torch.stack([self.dummy_image_enc] * len(images)).reshape(len(images), self.n_image_tokens * self.n_image_channels, self.embedding_size)
    ...
    def forward(self, src_tokens: Optional[torch.LongTensor], image_features: Optional[Union[List[object], torch.Tensor]]) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        """
    ...
    def _fix_for_fp16(self, full_enc: torch.Tensor, full_mask: Optional[torch.Tensor]) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        ...
        if num_tokens_to_remove == 0:
            ...
            enc_extension = full_enc.new_full(size=(full_enc.size(0), num_tokens_to_add, full_enc.size(2)), fill_value=self.padding_idx)
            mask_extension = full_mask.new_full(size=(full_mask.size(0), num_tokens_to_add), fill_value=self.padding_idx)
