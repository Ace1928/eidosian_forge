...
class DatabaseCache(BaseDatabaseCache):
    ...
    def has_key(self, key, version=None):
        ...
        with connection.cursor() as cursor:
            cursor.execute('SELECT %s FROM %s WHERE %s = %%s and %s > %%s' % (quote_name('cache_key'), quote_name(self._table), quote_name('cache_key'), quote_name('expires')), [key, connection.ops.adapt_datetimefield_value(now)])
    ...
    def _cull(self, db, cursor, now, num):
        if self._cull_frequency == 0:
            ...
            cursor.execute('DELETE FROM %s WHERE %s < %%s' % (table, connection.ops.quote_name('expires')), [connection.ops.adapt_datetimefield_value(now)])
            ...
            if remaining_num > self._max_entries:
                ...
                if last_cache_key:
                    cursor.execute('DELETE FROM %s WHERE %s < %%s' % (table, connection.ops.quote_name('cache_key')), [last_cache_key[0]])
