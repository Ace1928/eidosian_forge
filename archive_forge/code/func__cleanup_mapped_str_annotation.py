...
def _cleanup_mapped_str_annotation(annotation: str, originating_module: str) -> str:
    ...
    try:
        ...
        raise _CleanupError(f'For annotation "{annotation}", could not resolve container type "{mm.group(1)}".  Please ensure this type is imported at the module level outside of TYPE_CHECKING blocks') from ne
    ...
    while True:
        ...
        if inner is None:
            stack.append(g2)
            break
    if not re.match('^["\'].*["\']$', stack[-1]) and (not re.match('.*\\[.*\\]', stack[-1])):
        stripchars = '"\' '
