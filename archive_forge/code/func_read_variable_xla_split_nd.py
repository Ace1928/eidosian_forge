...
def read_variable_xla_split_nd(resource: _atypes.TensorFuzzingAnnotation[_atypes.Resource], T: TV_ReadVariableXlaSplitND_T, N: int, num_splits, paddings=[], name=None):
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'ReadVariableXlaSplitND', name, resource, 'T', T, 'N', N, 'num_splits', num_splits, 'paddings', paddings)
            return _result
            ...
            pass
        try:
            return read_variable_xla_split_nd_eager_fallback(resource, T=T, N=N, num_splits=num_splits, paddings=paddings, name=name, ctx=_ctx)
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('ReadVariableXlaSplitND', resource=resource, T=T, N=N, num_splits=num_splits, paddings=paddings, name=name)
    _result = _outputs[:]
    if not _result:
        return _op
