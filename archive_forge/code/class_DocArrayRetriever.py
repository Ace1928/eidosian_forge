...
class DocArrayRetriever(BaseRetriever):
    ...
    def _get_relevant_documents(self, query: str, *, run_manager: CallbackManagerForRetrieverRun) -> List[Document]:
        ...
        if self.search_type == SearchType.similarity:
            ...
            raise ValueError(f"Search type {self.search_type} does not exist. Choose either 'similarity' or 'mmr'.")
        return results
    ...
    def _search(self, query_emb: np.ndarray, top_k: int) -> List[Union[Dict[str, Any], Any]]:
        ...
        if self.filters:
            query = self.index.build_query().find(query=query_emb, search_field=search_field).filter(**filter_args).build(limit=top_k)
