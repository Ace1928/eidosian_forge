...
class ReduceL2(Base):
    ...
    def export_do_not_keepdims() -> None:
        ...
        expect(node, inputs=[data, axes], outputs=[reduced], name='test_reduce_l2_do_not_keepdims_example')
        np.random.seed(0)
        data = np.random.uniform(-10, 10, shape).astype(np.float32)
        reduced = np.sqrt(np.sum(a=np.square(data), axis=tuple(axes), keepdims=keepdims == 1))
        expect(node, inputs=[data, axes], outputs=[reduced], name='test_reduce_l2_do_not_keepdims_random')
    ...
    def export_keepdims() -> None:
        ...
        data = np.reshape(np.arange(1, np.prod(shape) + 1, dtype=np.float32), shape)
        reduced = np.sqrt(np.sum(a=np.square(data), axis=tuple(axes), keepdims=keepdims == 1))
        expect(node, inputs=[data, axes], outputs=[reduced], name='test_reduce_l2_keep_dims_example')
        np.random.seed(0)
        data = np.random.uniform(-10, 10, shape).astype(np.float32)
        reduced = np.sqrt(np.sum(a=np.square(data), axis=tuple(axes), keepdims=keepdims == 1))
        expect(node, inputs=[data, axes], outputs=[reduced], name='test_reduce_l2_keep_dims_random')
    ...
    def export_default_axes_keepdims() -> None:
        ...
        expect(node, inputs=[data, axes], outputs=[reduced], name='test_reduce_l2_default_axes_keepdims_example')
        np.random.seed(0)
        data = np.random.uniform(-10, 10, shape).astype(np.float32)
        reduced = np.sqrt(np.sum(a=np.square(data), axis=None, keepdims=keepdims == 1))
        expect(node, inputs=[data, axes], outputs=[reduced], name='test_reduce_l2_default_axes_keepdims_random')
