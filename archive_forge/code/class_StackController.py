...
class StackController(object):
    ...
    def describe(self, req):
        ...
        def format_stack(s):
            ...
            result['Parameters'] = [{'ParameterKey': k, 'ParameterValue': v} for k, v in result['Parameters'].items()]
            return self._id_format(result)
        con = req.context
        try:
    ...
    def _get_template(self, req):
        ...
        return None
    CREATE_OR_UPDATE_ACTION = CREATE_STACK, UPDATE_STACK = ('CreateStack', 'UpdateStack')
    ...
    def estimate_template_cost(self, req):
        ...
        return api_utils.format_response('EstimateTemplateCost', {'Url': 'http://en.wikipedia.org/wiki/Gratis'})
    ...
    def describe_stack_resource(self, req):
        ...
        try:
            identity = self._get_identity(con, req.params['StackName'])
            resource_details = self.rpc_client.describe_stack_resource(con, stack_identity=identity, resource_name=req.params.get('LogicalResourceId'))
        except Exception as ex:
            return exception.map_remote_error(ex)
        result = format_resource_detail(resource_details)
        return api_utils.format_response('DescribeStackResource', {'StackResourceDetail': result})
    ...
    def describe_stack_resources(self, req):
        ...
        def format_stack_resource(r):
            ...
            keymap = {rpc_api.RES_DESCRIPTION: 'Description', rpc_api.RES_NAME: 'LogicalResourceId', rpc_api.RES_PHYSICAL_ID: 'PhysicalResourceId', rpc_api.RES_STATUS_DATA: 'ResourceStatusReason', rpc_api.RES_TYPE: 'ResourceType', rpc_api.RES_STACK_ID: 'StackId', rpc_api.RES_STACK_NAME: 'StackName', rpc_api.RES_UPDATED_TIME: 'Timestamp'}
            result = api_utils.reformat_dict_keys(keymap, r)
            result['ResourceStatus'] = self._resource_status(r)
            return self._id_format(result)
        con = req.context
        ...
        try:
            ...
            resources = self.rpc_client.describe_stack_resources(con, stack_identity=identity, resource_name=req.params.get('LogicalResourceId'))
        except Exception as ex:
            return exception.map_remote_error(ex)
