...
class RemoveComprehension(Transformation):
    ...
    def visit_Module(self, node):
        ...
        if self.has_dist_comp:
            node.body.insert(0, ast.Import([ast.alias('__dispatch__', mangle('__dispatch__'))]))
        return node
    ...
    def visit_AnyComp(self, node, comp_type, *path):
        ...
        self.count_iter = 0
        starget = '__target'
        body = reduce(self.nest_reducer, reversed(node.generators), ast.Expr(ast.Call(reduce(lambda x, y: ast.Attribute(x, y, ast.Load()), path[1:], ast.Name(path[0], ast.Load(), None, None)), [ast.Name(starget, ast.Load(), None, None), node.elt], [])))
        metadata.add(body, metadata.Comprehension(starget))
        init = ast.Assign([ast.Name(starget, ast.Store(), None, None)], ast.Call(ast.Attribute(ast.Name('builtins', ast.Load(), None, None), comp_type, ast.Load()), [], []), None)
        result = ast.Return(ast.Name(starget, ast.Load(), None, None))
        sargs = [ast.Name(arg, ast.Param(), None, None) for arg in args]
        fd = ast.FunctionDef(name, ast.arguments(sargs, [], None, [], [], None, []), [init, body, result], [], None, None)
        metadata.add(fd, metadata.Local())
        self.ctx.module.body.append(fd)
        return ast.Call(ast.Name(name, ast.Load(), None, None), [ast.Name(arg.id, ast.Load(), None, None) for arg in sargs], [])

    def visit_ListComp(self, node):
        return self.visit_AnyComp(node, 'list', 'builtins', 'list', 'append')

    def visit_SetComp(self, node):
        return self.visit_AnyComp(node, 'set', 'builtins', 'set', 'add')

    def visit_DictComp(self, node):
        node.elt = ast.List([ast.Tuple([node.key, node.value], ast.Load())], ast.Load())
        self.has_dist_comp = True
        return self.visit_AnyComp(node, 'dict', mangle('__dispatch__'), 'update')
    ...
    def visit_GeneratorExp(self, node):
        ...
        args = self.gather(ImportedIds, node)
        self.count_iter = 0
        body = reduce(self.nest_reducer, reversed(node.generators), ast.Expr(ast.Yield(node.elt)))
        sargs = [ast.Name(arg, ast.Param(), None, None) for arg in args]
        fd = ast.FunctionDef(name, ast.arguments(sargs, [], None, [], [], None, []), [body], [], None, None)
        metadata.add(fd, metadata.Local())
        self.ctx.module.body.append(fd)
        return ast.Call(ast.Name(name, ast.Load(), None, None), [ast.Name(arg.id, ast.Load(), None, None) for arg in sargs], [])