...
class LoggingCmd(Command):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.subcommands = {'on': self.On, 'off': self.Off, 'level': self.Level}
    ...
    class Level(Command):
        ...
        def action(self, params):
            lvls = {'debug': logging.DEBUG, 'error': logging.ERROR, 'info': logging.INFO}
            if len(params) == 1 and params[0] in lvls:
                self.api.log_handler.setLevel(lvls.get(params[0], logging.ERROR))
                return CommandsResponse(STATUS_OK, True)
            else:
