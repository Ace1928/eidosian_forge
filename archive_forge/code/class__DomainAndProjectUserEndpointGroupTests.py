...
class _DomainAndProjectUserEndpointGroupTests(object):
    ...
    def test_user_cannot_list_endpoints_associated_with_endpoint_groups(self):
        ...
        endpoint_group = PROVIDERS.catalog_api.create_endpoint_group(endpoint_group['id'], endpoint_group)
        with self.test_client() as c:
            c.get('/v3/OS-EP-FILTER/endpoint_groups/%s/endpoints' % endpoint_group['id'], headers=self.headers, expected_status_code=http.client.FORBIDDEN)
    ...
    def test_user_cannot_get_endpoints_associated_with_endpoint_groups(self):
        ...
        PROVIDERS.catalog_api.add_endpoint_group_to_project(endpoint_group['id'], project['id'])
        with self.test_client() as c:
            c.get('/v3/OS-EP-FILTER/endpoint_groups/%s/projects/%s' % (endpoint_group['id'], project['id']), headers=self.headers, expected_status_code=http.client.FORBIDDEN)
    ...
    def test_user_cannot_list_endpoint_groups_with_their_projects(self):
        ...
        PROVIDERS.catalog_api.add_endpoint_group_to_project(endpoint_group['id'], project['id'])
        with self.test_client() as c:
            c.get('/v3/OS-EP-FILTER/projects/%s/endpoint_groups' % project['id'], headers=self.headers, expected_status_code=http.client.FORBIDDEN)
