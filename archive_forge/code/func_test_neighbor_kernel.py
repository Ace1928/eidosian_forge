...
def test_neighbor_kernel():
    tol = 1e-07
    a = ase.Atoms('CC', positions=[[0.5, 0.5, 0.5], [1, 1, 1]], cell=[10, 10, 10], pbc=True)
    ...
    for pbc in [True, False, [True, False, True]]:
        ...
        a.set_scaled_positions(np.transpose([np.random.random(len(a)), np.random.random(len(a)), np.random.random(len(a))]))
    ...
    a = ase.Atoms('CC', positions=[[0.5, 0.5, 0.5], [1.1, 0.5, 0.5]], cell=[1, 1, 1], pbc=False)
    ...
    a = ase.Atoms('CC', positions=[[9.5, 0.5, 0.5], [10.1, 0.5, 0.5]], cell=[10, 10, 10], pbc=False)
    i1, j1, r1 = neighbor_list('ijd', a, 1.1)
    a.set_cell([20, 10, 10])
    i2, j2, r2 = neighbor_list('ijd', a, 1.1)
    assert (i1 == i2).all()
    ...
    assert np.abs(r1 - r2).max() < tol
    for sx in range(3):
        a = ase.lattice.hexagonal.Graphite('C', latticeconstant=(2.5, 10.0), size=[sx + 1, sx + 1, 1])
    ...
    nat = 10
    atoms = ase.Atoms(numbers=range(nat), cell=[(0.2, 1.2, 1.4), (1.4, 0.1, 1.6), (1.3, 2.0, -0.1)])
    ...
    for p1 in range(2):
        for p2 in range(2):
            for p3 in range(2):
                ...
                assert not (c2 - c).any()
    c = 0.0058
    i, j, d = primitive_neighbor_list('ijd', [True, True, True], np.eye(3) * 7.56, np.array([[0, 0, 0], [0, 0, 0.99875]]), [c, c], self_interaction=False, use_scaled_positions=True)
    assert np.all(i == [0, 1])
    assert np.all(j == [1, 0])
