...
class GroundControlPoint:
    ...
    def __repr__(self):
        args = ', '.join(['{}={}'.format(att, repr(getattr(self, att))) for att in ('row', 'col', 'x', 'y', 'z', 'id', 'info') if getattr(self, att) is not None])
        return 'GroundControlPoint({})'.format(args)

    def asdict(self):
        """A dict representation of the GCP"""
        return {'id': self.id, 'info': self.info, 'row': self.row, 'col': self.col, 'x': self.x, 'y': self.y, 'z': self.z}
    ...
    def __geo_interface__(self):
        ...
        return {'id': self.id, 'type': 'Feature', 'geometry': {'type': 'Point', 'coordinates': tuple(coords)}, 'properties': self.asdict()}