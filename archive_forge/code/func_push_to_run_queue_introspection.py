...
def push_to_run_queue_introspection(self) -> Tuple[bool, bool]:
    ...
    if self.server_supports_template_variables is None or self.server_push_to_run_queue_supports_priority is None:
        ...
        res = self.gql(query)
        self.server_supports_template_variables = 'templateVariableValues' in [x['name'] for x in res.get('PushToRunQueueInputType', {}).get('inputFields', [{}])]
        self.server_push_to_run_queue_supports_priority = 'priority' in [x['name'] for x in res.get('PushToRunQueueInputType', {}).get('inputFields', [{}])]
