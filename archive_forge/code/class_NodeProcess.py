...
class NodeProcess(BaseModel):
    ...
    @classmethod
    def get_process(cls, pid: Optional[int]=None, runtime: Optional[str]='uvicorn', entrypoint: Optional[str]='main:app', exact_match: Optional[bool]=False, is_worker: Optional[bool]=False, worker_entrypoint: Optional[str]=None, has_workers: Optional[bool]=False) -> 'NodeProcess':
        """
        ...
        process = cls(pid=node_pid, name=node_process.name(), exe=node_process.exe(), cmdline=node_process.cmdline(), has_workers=has_workers, is_worker=is_worker, runtime=runtime, entrypoint=entrypoint, exact_match=exact_match, worker_entrypoint=worker_entrypoint)
        for p in psutil.process_iter(attrs=['pid', 'name', 'exe', 'cmdline']):
            if p.info['pid'] == node_pid:
                continue
            if p.info['exe'] != process.exe:
                continue
            if not is_worker and any((runtime in cmd_arg for cmd_arg in p.info['cmdline'])) and any((entrypoint == cmd_arg if exact_match else entrypoint in cmd_arg for cmd_arg in p.info['cmdline'])):
                process.linked.append(p)
                process.parent = p
                continue
            if is_worker and worker_entrypoint and all((runtime not in cmd_arg for cmd_arg in p.info['cmdline'])) and any((worker_entrypoint == cmd_arg if exact_match else worker_entrypoint in cmd_arg for cmd_arg in p.info['cmdline'])) and (not process.parent):
                process.parent = p
                continue
    ...
    def reconfigure(self):
        ...
        """
        self = self.get_process(pid=self.pid, runtime=self.runtime, entrypoint=self.entrypoint, exact_match=self.exact_match, is_worker=self.is_worker, worker_entrypoint=self.worker_entrypoint)