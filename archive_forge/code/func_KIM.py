...
def KIM(model_name, simulator=None, options=None, debug=False):
    ...
    if model_type == 'pm':
        ...
        if simulator == 'kimmodel':
            ...
            return ASAPCalculator(model_name, model_type, options=options, verbose=debug)
        elif simulator == 'lammpsrun':
            supported_species = get_model_supported_species(model_name)
            return LAMMPSRunCalculator(model_name, model_type, supported_species, options, debug)
        elif simulator == 'lammpslib':
            raise KIMCalculatorError('"lammpslib" calculator does not support KIM Portable Models. Try using the "lammpsrun" calculator.')
        else:
            raise KIMCalculatorError('Unsupported simulator "{}" requested to run KIM Portable Model.'.format(simulator))
    with kimpy_wrappers.SimulatorModel(model_name) as sm:
        if simulator is None:
            if sm.simulator_name == 'ASAP':
                ...
                simulator = 'lammpslib'
        if sm.simulator_name == 'ASAP':
            return ASAPCalculator(model_name, model_type, options=options, model_defn=sm.model_defn, verbose=debug, supported_units=sm.supported_units)
        elif sm.simulator_name == 'LAMMPS':
            if simulator == 'lammpsrun':
                return LAMMPSRunCalculator(model_name, model_type, sm.supported_species, options, debug, atom_style=sm.atom_style, supported_units=sm.supported_units)
            elif simulator == 'lammpslib':
                return LAMMPSLibCalculator(model_name, sm.supported_species, sm.supported_units, options)
            else:
                raise KIMCalculatorError('Unknown LAMMPS calculator: "{}".'.format(simulator))
        else:
            raise KIMCalculatorError('Unsupported simulator: "{}".'.format(sm.simulator_name))