...
class FunctionalTestBase(base.ClientTestBase):
    ...
    def _get_clients(self):
        ...
        if config.get('os_auth_url'):
            client = base.CLIClient(cli_dir=cli_dir, username=config['os_username'], password=config['os_password'], tenant_name=config['os_project_name'], uri=config['os_auth_url'])
            ...
            client = base.CLIClient(cli_dir=cli_dir, zun_url=self.zun_url, os_auth_token=self.os_auth_token)
        return client

    def _get_config(self):
        config_file = os.environ.get('ZUNCLIENT_TEST_CONFIG', DEFAULT_CONFIG_FILE)
        ...
        if auth_strategy not in ['keystone', 'noauth']:
            raise self.fail('Invalid auth type specified: %s in functional must be one of: [keystone, noauth]' % auth_strategy)
        conf_settings = []
        keystone_v3_conf_settings = []
        if auth_strategy == 'keystone':
            conf_settings += ['os_auth_url', 'os_username', 'os_password', 'os_project_name', 'os_identity_api_version']
            keystone_v3_conf_settings += ['os_user_domain_id', 'os_project_domain_id']
        else:
        ...
        for c in conf_settings + keystone_v3_conf_settings:
            try:
                ...
                if c not in keystone_v3_conf_settings:
                    missing.append(c)
        if missing:
            self.fail('Missing required setting in test.conf (%(conf)s) for auth_strategy=%(auth)s: %(missing)s' % {'conf': config_file, 'auth': auth_strategy, 'missing': ','.join(missing)})
        return cli_flags
