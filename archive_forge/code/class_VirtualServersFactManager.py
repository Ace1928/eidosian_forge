...
class VirtualServersFactManager(BaseManager):
    ...
    def read_collection_from_device(self, skip=0):
        uri = 'https://{0}:{1}/mgmt/tm/ltm/virtual'.format(self.client.provider['server'], self.client.provider['server_port'])
        query = '?expandSubcollections=true&$top={0}&$skip={1}&$filter=partition+eq+{2}'.format(self.module.params['data_increment'], skip, self.module.params['partition'])
    ...
    def read_stats_from_device(self, full_path):
        uri = 'https://{0}:{1}/mgmt/tm/ltm/virtual/{2}/stats'.format(self.client.provider['server'], self.client.provider['server_port'], transform_name(name=full_path))
