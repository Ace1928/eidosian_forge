...
def quantized_conv2d_with_bias_and_requantize(input: _atypes.TensorFuzzingAnnotation[TV_QuantizedConv2DWithBiasAndRequantize_Tinput], filter: _atypes.TensorFuzzingAnnotation[TV_QuantizedConv2DWithBiasAndRequantize_Tfilter], bias: _atypes.TensorFuzzingAnnotation[TV_QuantizedConv2DWithBiasAndRequantize_Tbias], min_input: _atypes.TensorFuzzingAnnotation[_atypes.Float32], max_input: _atypes.TensorFuzzingAnnotation[_atypes.Float32], min_filter: _atypes.TensorFuzzingAnnotation[_atypes.Float32], max_filter: _atypes.TensorFuzzingAnnotation[_atypes.Float32], min_freezed_output: _atypes.TensorFuzzingAnnotation[_atypes.Float32], max_freezed_output: _atypes.TensorFuzzingAnnotation[_atypes.Float32], strides, padding: str, out_type: TV_QuantizedConv2DWithBiasAndRequantize_out_type=_dtypes.qint8, dilations=[1, 1, 1, 1], padding_list=[], name=None):
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'QuantizedConv2DWithBiasAndRequantize', name, input, filter, bias, min_input, max_input, min_filter, max_filter, min_freezed_output, max_freezed_output, 'out_type', out_type, 'strides', strides, 'padding', padding, 'dilations', dilations, 'padding_list', padding_list)
    ...
    if not isinstance(strides, (list, tuple)):
        raise TypeError("Expected list for 'strides' argument to 'quantized_conv2d_with_bias_and_requantize' Op, not %r." % strides)
    ...
    if not isinstance(dilations, (list, tuple)):
        raise TypeError("Expected list for 'dilations' argument to 'quantized_conv2d_with_bias_and_requantize' Op, not %r." % dilations)
    dilations = [_execute.make_int(_i, 'dilations') for _i in dilations]
    if padding_list is None:
        padding_list = []
    if not isinstance(padding_list, (list, tuple)):
        raise TypeError("Expected list for 'padding_list' argument to 'quantized_conv2d_with_bias_and_requantize' Op, not %r." % padding_list)
    padding_list = [_execute.make_int(_i, 'padding_list') for _i in padding_list]
    _, _, _op, _outputs = _op_def_library._apply_op_helper('QuantizedConv2DWithBiasAndRequantize', input=input, filter=filter, bias=bias, min_input=min_input, max_input=max_input, min_filter=min_filter, max_filter=max_filter, min_freezed_output=min_freezed_output, max_freezed_output=max_freezed_output, strides=strides, padding=padding, out_type=out_type, dilations=dilations, padding_list=padding_list, name=name)
    _result = _outputs[:]
