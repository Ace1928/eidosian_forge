...
class InterfacesFacts(object):
    ...
    def populate_facts(self, connection, ansible_facts, data=None):
        ...
        objs = []
        interface_names = findall("^set interfaces (?:ethernet|bonding|bridge|dummy|tunnel|vti|loopback|vxlan|openvpn|wireguard) (?:\\'*)(\\S+)(?:\\'*)", data, M)
        if interface_names:
            for interface in set(interface_names):
                intf_regex = ' %s .+$' % interface.strip("'")
    ...
    def parse_vifs(self, conf):
        vif_names = findall("vif (?:\\'*)(\\d+)(?:\\'*)", conf, M)
        vifs_list = None
        if vif_names:
            vifs_list = []
