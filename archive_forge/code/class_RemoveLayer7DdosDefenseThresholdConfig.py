  ...
  *{command}* is used to remove layer7 ddos defense threshold configs from security policies.
  ...
  """
    ...
    def Run(self, args):
        ...
        if adaptive_protection_config is None or adaptive_protection_config.layer7DdosDefenseConfig is None or (not adaptive_protection_config.layer7DdosDefenseConfig.thresholdConfigs):
            raise exceptions.InvalidArgumentException('--threshold-config-name', "There's no existing layer 7 ddos defense threshold config to remove")
        existing_threshold_configs = adaptive_protection_config.layer7DdosDefenseConfig.thresholdConfigs
        new_threshold_configs = [threshold_config for threshold_config in existing_threshold_configs if threshold_config.name != args.threshold_config_name]
        if len(existing_threshold_configs) == len(new_threshold_configs):
            raise exceptions.InvalidArgumentException('--threshold-config-name', 'layer 7 ddos defense threshold config "%s" does not exist in this policy.' % args.threshold_config_name)
        adaptive_protection_config.layer7DdosDefenseConfig.thresholdConfigs = new_threshold_configs
