...
class Running(WrapperMetric):
    ...
    def __init__(self, base_metric: Metric, window: int=5) -> None:
        ...
        if not isinstance(base_metric, Metric):
            raise ValueError(f'Expected argument `metric` to be an instance of `torchmetrics.Metric` but got {base_metric}')
        ...
        if base_metric.full_state_update is not False:
            raise ValueError(f'Expected attribute `full_state_update` set to `False` but got {base_metric.full_state_update}')
        self._num_vals_seen = 0
        for key in base_metric._defaults:
            for i in range(window):
                self.add_state(name=key + f'_{i}', default=base_metric._defaults[key], dist_reduce_fx=base_metric._reductions[key])
