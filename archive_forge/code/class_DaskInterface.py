...
class DaskInterface(PandasInterface):
    ...
    def aggregate(cls, dataset, dimensions, function, **kwargs):
        ...
        dtypes = data.dtypes
        numeric = [c for c, dtype in zip(dtypes.index, dtypes.values) if dtype.kind in 'iufc' and c in vdims]
        reindexed = data[cols + numeric]
        inbuilts = {'amin': 'min', 'amax': 'max', 'mean': 'mean', 'std': 'std', 'sum': 'sum', 'var': 'var'}
        if len(dimensions):
