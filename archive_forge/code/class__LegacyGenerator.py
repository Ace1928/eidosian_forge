...
class _LegacyGenerator(object):
  ...
  """
    ...
    def __init__(self, account, credentials, scopes=None):
        ...
        if self._cred_type not in (c_creds.USER_ACCOUNT_CREDS_NAME, c_creds.SERVICE_ACCOUNT_CREDS_NAME, c_creds.EXTERNAL_ACCOUNT_CREDS_NAME, c_creds.EXTERNAL_ACCOUNT_USER_CREDS_NAME, c_creds.EXTERNAL_ACCOUNT_AUTHORIZED_USER_CREDS_NAME, c_creds.P12_SERVICE_ACCOUNT_CREDS_NAME):
    ...
    def _cred_type(self):
        if self._is_oauth2client:
            ...
            return c_creds.CredentialTypeGoogleAuth.FromCredentials(self.credentials).key
