...
class EmbeddingDatabaseManager:
    ...
    def insert_embedding(
        ...
        This method utilizes an atomic approach to check and insert in a single operation to avoid duplicate entries.
        Parameters:
        ...
        try:
            ...
            SELECT ?, ?, ?, ?, ?
            WHERE NOT EXISTS (
                SELECT 1 FROM embeddings WHERE file_hash = ?
            )
            """
            self.connection_manager.cursor.execute(
                insert_query,
                (file_hash, imports, function, docstring, embedding, file_hash),
            )
            if self.connection_manager.cursor.rowcount == 0:
                logging.info(
                    f"Skipped insertion for already processed hash: {file_hash}"
                )
            else:
                logging.info(f"Embedding inserted for file hash: {file_hash}")
        except sqlite3.IntegrityError as e:
            logging.error(f"Failed to insert due to integrity error: {e}")
...
class FileManager:
    ...
    def process_file(
        ...
        """
        Processes a single Python file, extracting functions, imports, and classes, and storing embeddings in a database.
        This method now includes a more robust and universal hashing process that incorporates file size to ensure uniqueness.
        ...
        try:
            ...
            unique_identifier = f"{file_stat.st_size}-{file_stat.st_mtime}-{content}"
            ...
            # Decode content to process with the code parser
