...
def test_create_flavor_extra_specs(self):
    self._verify('openstack.compute.v2.flavor.Flavor.create_extra_specs', self.proxy.create_flavor_extra_specs, method_args=['value', {'a': 'b'}], expected_args=[self.proxy], expected_kwargs={'specs': {'a': 'b'}})