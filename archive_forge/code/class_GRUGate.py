...
class GRUGate(nn.Module):
    ...
    def forward(self, inputs: TensorType, **kwargs) -> TensorType:
        h, X = inputs
        r = torch.tensordot(X, self._w_r, dims=1) + torch.tensordot(h, self._u_r, dims=1)
        r = torch.sigmoid(r)
        z = torch.tensordot(X, self._w_z, dims=1) + torch.tensordot(h, self._u_z, dims=1) - self._bias_z
        z = torch.sigmoid(z)
        h_next = torch.tensordot(X, self._w_h, dims=1) + torch.tensordot(h * r, self._u_h, dims=1)
