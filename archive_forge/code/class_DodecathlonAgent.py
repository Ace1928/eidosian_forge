...
class DodecathlonAgent(ParlaiGenerationAgent, Seq2SeqLM):
    ...
    def __init__(self, model, device, maxlen):
        ...
        super().__init__(name=model, suffix='\n', device=device, maxlen=maxlen, model=create_agent_from_opt_file(option))
    ...
    def available_models():
        return ['dodecathlon.all_tasks_mt', 'dodecathlon.convai2', 'dodecathlon.wizard_of_wikipedia', 'dodecathlon.empathetic_dialoguesdodecathlon.eli5', 'dodecathlon.reddit', 'dodecathlon.twitter', 'dodecathlon.ubuntu', 'dodecathlon.image_chat', 'dodecathlon.cornell_movie', 'dodecathlon.light_dialog', 'dodecathlon.daily_dialog']

    def set_options(self, name, device):
        option = {'n_image_tokens': 1, 'n_image_channels': 1, 'image_fusion_type': 'late'}
