...
class DiskUtilsTestCase(test_base.OsWinBaseTestCase):
    _autospec_classes = [diskutils.pathutils.PathUtils, diskutils.win32utils.Win32Utils]
    ...
    @ddt.data({}, {'exists': False}, {'same_file': False})
    @ddt.unpack
    @mock.patch('os.path.exists')
    @mock.patch.object(diskutils.DiskUtils, 'get_attached_virtual_disk_files')
    def test_is_virtual_disk_file_attached(self, mock_get_disks, mock_exists, exists=True, same_file=True):
        ...
        attached = self._diskutils.is_virtual_disk_file_attached(mock.sentinel.path)
        self.assertEqual(exists and same_file, attached)
        if exists:
            mock_get_disks.assert_called_once_with()
            self._pathutils.is_same_file.assert_called_once_with(mock.sentinel.path, mock.sentinel.other_path)
        else:
    ...
    def test_get_device_name_by_device_number(self, mock_get_disk):
        dev_name = self._diskutils.get_device_name_by_device_number(mock.sentinel.disk_number)
        self.assertEqual(mock_get_disk.return_value.Name, dev_name)
        mock_get_disk.assert_called_once_with(mock.sentinel.disk_number, msft_disk_cls=False)
    ...
    def test_get_disk_capacity_raised_exc(self):
        self._test_get_disk_capacity(raised_exc=exceptions.Win32Exception)
    ...
    def test_set_disk_online(self, err_code, mock_get_disk):
        mock_disk = mock_get_disk.return_value
        mock_disk.Online.return_value = (mock.sentinel.ext_err_info, err_code)
        if err_code:
            self.assertRaises(exceptions.DiskUpdateError, self._diskutils.set_disk_online, mock.sentinel.disk_number)
        else:
