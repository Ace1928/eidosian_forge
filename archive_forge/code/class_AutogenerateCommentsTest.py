...
class AutogenerateCommentsTest(AutogenFixtureTest, TestBase):
    ...
    def test_existing_table_comment_no_change(self):
        ...
        Table('some_table', m2, Column('test', String(10), primary_key=True), comment='this is some table')
    ...
    def test_alter_table_comment(self):
        ...
        m2 = MetaData()
        Table('some_table', m1, Column('test', String(10), primary_key=True), comment='this is some table')
        Table('some_table', m2, Column('test', String(10), primary_key=True), comment='this is also some table')
        diffs = self._fixture(m1, m2)
        eq_(diffs[0][0], 'add_table_comment')
    ...
    def test_existing_column_comment_no_change(self):
        m1 = MetaData()
        m2 = MetaData()
        Table('some_table', m1, Column('test', String(10), primary_key=True), Column('amount', Float, comment='the amount'))
    ...
    def test_remove_column_comment(self):
        ...
        m2 = MetaData()
        Table('some_table', m1, Column('test', String(10), primary_key=True), Column('amount', Float, comment='the amount'))
        Table('some_table', m2, Column('test', String(10), primary_key=True), Column('amount', Float))
        diffs = self._fixture(m1, m2)
        eq_(diffs, [[('modify_comment', None, 'some_table', 'amount', {'existing_nullable': True, 'existing_type': mock.ANY, 'existing_server_default': False}, 'the amount', None)]])

    def test_alter_column_comment(self):
        m1 = MetaData()
        m2 = MetaData()
        Table('some_table', m1, Column('test', String(10), primary_key=True), Column('amount', Float, comment='the amount'))
        Table('some_table', m2, Column('test', String(10), primary_key=True), Column('amount', Float, comment='the adjusted amount'))
        diffs = self._fixture(m1, m2)
        eq_(diffs, [[('modify_comment', None, 'some_table', 'amount', {'existing_nullable': True, 'existing_type': mock.ANY, 'existing_server_default': False}, 'the amount', 'the adjusted amount')]])