...
class PubsubApi(object):
    ...
    """
        ...
        self.host_base = config.get('Credentials', 'gs_pubsub_host', 'pubsub.googleapis.com')
        ...
        self.api_client = apitools_client.PubsubV1(url=self.url_base, http=self.http, log_request=log_request, log_response=log_response, credentials=self.credentials)
    ...
    def GetTopicIamPolicy(self, topic_name):
        request = apitools_messages.PubsubProjectsTopicsGetIamPolicyRequest(resource=topic_name)
        try:
    ...
    def SetTopicIamPolicy(self, topic_name, policy):
        ...
        request = apitools_messages.PubsubProjectsTopicsSetIamPolicyRequest(resource=topic_name, setIamPolicyRequest=policy_request)
        try:
    ...
    """
        ...
        translated_exception = self._TranslateApitoolsException(e, topic_name=topic_name)
        ...
        if isinstance(e, apitools_exceptions.HttpError):
            ...
            if e.status_code == 400:
                ...
                if 'The account for the specified project has been disabled' in str(e):
                    ...
                    return AccessDeniedException('Access Not Configured. Please go to the Google Cloud Platform Console (https://cloud.google.com/console#/project) for your project, select APIs and Auth and enable the Google Cloud Pub/Sub API.', status=e.status_code)
                elif 'insufficient_scope' in str(e):
                    return AccessDeniedException(_INSUFFICIENT_OAUTH2_SCOPE_MESSAGE, status=e.status_code, body=self._GetAcceptableScopesFromHttpError(e))
                else:
                    return AccessDeniedException(message or e.message, status=e.status_code)
            elif e.status_code == 404:
                return NotFoundException(message, status=e.status_code)
            elif e.status_code == 409 and topic_name:
                return ServiceException('The topic %s already exists.' % topic_name, status=e.status_code)
