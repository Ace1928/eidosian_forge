...
def build_ast_schema(document):
    ...
    schema_def = None
    type_asts = (ast.ScalarTypeDefinition, ast.ObjectTypeDefinition, ast.InterfaceTypeDefinition, ast.EnumTypeDefinition, ast.UnionTypeDefinition, ast.InputObjectTypeDefinition)
    type_defs = []
    directive_defs = []
    ...
    inner_type_map = OrderedDict([('String', GraphQLString), ('Int', GraphQLInt), ('Float', GraphQLFloat), ('Boolean', GraphQLBoolean), ('ID', GraphQLID), ('__Schema', __Schema), ('__Directive', __Directive), ('__DirectiveLocation', __DirectiveLocation), ('__Type', __Type), ('__Field', __Field), ('__InputValue', __InputValue), ('__EnumValue', __EnumValue), ('__TypeKind', __TypeKind)])

    def get_directive(directive_ast):
        return GraphQLDirective(name=directive_ast.name.value, locations=[node.value for node in directive_ast.locations], args=make_input_values(directive_ast.arguments, GraphQLArgument))
    ...
    _schema_def_handlers = {ast.ObjectTypeDefinition: make_type_def, ast.InterfaceTypeDefinition: make_interface_def, ast.EnumTypeDefinition: make_enum_def, ast.UnionTypeDefinition: make_union_def, ast.ScalarTypeDefinition: make_scalar_def, ast.InputObjectTypeDefinition: make_input_object_def}
