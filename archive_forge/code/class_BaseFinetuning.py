...
class BaseFinetuning(Callback):
    ...
    def state_dict(self) -> Dict[str, Any]:
        return {'internal_optimizer_metadata': self._internal_optimizer_metadata}
    ...
    def on_fit_start(self, trainer: 'pl.Trainer', pl_module: 'pl.LightningModule') -> None:
        if self._restarting:
            ...
            for opt_idx, optimizer in enumerate(trainer.optimizers):
                param_groups = self._apply_mapping_to_param_groups(self._internal_optimizer_metadata[opt_idx], named_parameters)
    ...
    @staticmethod
    def filter_params(modules: Union[Module, Iterable[Union[Module, Iterable]]], train_bn: bool=True, requires_grad: bool=True) -> Generator:
    ...
    def setup(self, trainer: 'pl.Trainer', pl_module: 'pl.LightningModule', stage: str) -> None:
        ...
        if isinstance(trainer.strategy, DeepSpeedStrategy):
            raise NotImplementedError('The Finetuning callback does not support running with the DeepSpeed strategy. Choose a different strategy or disable the callback.')
    ...
    def _store(self, pl_module: 'pl.LightningModule', opt_idx: int, num_param_groups: int, current_param_groups: List[Dict[str, Any]]) -> None:
        mapping = {p: n for n, p in pl_module.named_parameters()}
        if opt_idx not in self._internal_optimizer_metadata:
            self._internal_optimizer_metadata[opt_idx] = self._apply_mapping_to_param_groups(current_param_groups, mapping)
        elif num_param_groups != len(current_param_groups):
            self._internal_optimizer_metadata[opt_idx].extend(self._apply_mapping_to_param_groups(current_param_groups[num_param_groups:], mapping))
