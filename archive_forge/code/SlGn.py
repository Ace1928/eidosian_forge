...
CHARGE_CONSTANT = 0.0001
QUANTUM_FORCE_CONSTANT = 0.001
NUCLEAR_FORCE_RANGE = 10
NUCLEAR_FORCE_STRENGTH = 0.02
WEAK_FORCE_STRENGTH = 0.01
...
class Particle:
    ...
    def apply_gravity(self):
        ...
        gravity_force = direction_to_center * (
            GRAVITY_STRENGTH * self.mass / max(np.linalg.norm(direction_to_center), 1)
        )
    ...
    def draw(self):
        ...
        pygame.draw.circle(screen, color, self.pos.astype(int), 10)


def initialize_particles():
    particles = []
    for _ in range(100):  # Example number of particles
