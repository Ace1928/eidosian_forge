...
class Weaviate(VectorStore):
    ...
    def __init__(self, client: Any, index_name: str, text_key: str, embedding: Optional[Embeddings]=None, attributes: Optional[List[str]]=None, relevance_score_fn: Optional[Callable[[float], float]]=_default_score_normalizer, by_text: bool=True):
        ...
        with self._client.batch as batch:
            for i, text in enumerate(texts):
                ...
                if 'uuids' in kwargs:
                    ...
                    _id = kwargs['ids'][i]
                batch.add_data_object(data_object=data_properties, class_name=self._index_name, uuid=_id, vector=embeddings[i] if embeddings else None, tenant=kwargs.get('tenant'))
                ids.append(_id)
        return ids
    ...
    def from_texts(cls, texts: List[str], embedding: Embeddings, metadatas: Optional[List[dict]]=None, *, client: Optional[weaviate.Client]=None, weaviate_url: Optional[str]=None, weaviate_api_key: Optional[str]=None, batch_size: Optional[int]=None, index_name: Optional[str]=None, text_key: str='text', by_text: bool=False, relevance_score_fn: Optional[Callable[[float], float]]=_default_score_normalizer, **kwargs: Any) -> Weaviate:
        ...
        try:
            ...
            raise ImportError('Could not import weaviate python  package. Please install it with `pip install weaviate-client`') from e
        client = client or _create_weaviate_client(url=weaviate_url, api_key=weaviate_api_key)
        if batch_size:
        ...
        with client.batch as batch:
            for i, text in enumerate(texts):
                ...
                _id = uuids[i]
                params = {'uuid': _id, 'data_object': data_properties, 'class_name': index_name}
                if embeddings is not None:
                    params['vector'] = embeddings[i]
                batch.add_data_object(**params)
            batch.flush()
        return cls(client, index_name, text_key, embedding=embedding, attributes=attributes, relevance_score_fn=relevance_score_fn, by_text=by_text, **kwargs)
