...
from ._util import (
@app.command('assemble', context_settings={'allow_extra_args': True, 'ignore_unknown_options': True})
def assemble_cli(ctx: typer.Context, config_path: Path=Arg(..., help='Path to config file', exists=True, allow_dash=True), output_path: Path=Arg(..., help='Output directory to store assembled pipeline in'), code_path: Optional[Path]=Opt(None, '--code', '-c', help='Path to Python file with additional code (registered functions) to be imported'), verbose: bool=Opt(False, '--verbose', '-V', '-VV', help='Display more information for debugging purposes')):
    """
