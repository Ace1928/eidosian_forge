...
class RegionResource(ks_flask.ResourceBase):
    ...
    def post(self):
        ...
        ref = PROVIDERS.catalog_api.create_region(region, initiator=self.audit_initiator)
    ...
    def put(self, region_id):
        ...
        if 'id' not in region:
            ...
            raise exception.ValidationError(_('Conflicting region IDs specified: "%(url_id)s" != "%(ref_id)s"') % {'url_id': region_id, 'ref_id': region['id']})
        ref = PROVIDERS.catalog_api.create_region(region, initiator=self.audit_initiator)
        return (self.wrap_member(ref), http.client.CREATED)
    ...
    def patch(self, region_id):
        ...
        return self.wrap_member(PROVIDERS.catalog_api.update_region(region_id, region, initiator=self.audit_initiator))

    def delete(self, region_id):
        ENFORCER.enforce_call(action='identity:delete_region')
        return (PROVIDERS.catalog_api.delete_region(region_id, initiator=self.audit_initiator), http.client.NO_CONTENT)