...
class STRtree:
    ...
    def query(self, geometry, predicate=None, distance=None):
        ...
        if predicate is None:
            ...
            if lib.geos_version < (3, 10, 0):
                raise UnsupportedGEOSVersionError('dwithin predicate requires GEOS >= 3.10')
            if distance is None:
                raise ValueError('distance parameter must be provided for dwithin predicate')
    ...
    def nearest(self, geometry) -> Union[Any, None]:
        ...
        if is_missing(geometry_arr).any() or is_empty(geometry_arr).any():
            raise ValueError('Cannot determine nearest geometry for empty geometry or missing value (None).')
