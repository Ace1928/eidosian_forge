...
def test_create_custom_credential_type(run_module, admin_user, silence_deprecation):
    result = run_module('credential_type', dict(name='Nexus', description='Credentials type for Nexus', kind='cloud', inputs={'fields': [{'id': 'server', 'type': 'string', 'default': '', 'label': ''}], 'required': []}, injectors={'extra_vars': {'nexus_credential': 'test'}}, state='present'), admin_user)
