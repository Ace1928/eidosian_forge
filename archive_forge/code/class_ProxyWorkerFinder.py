...
class ProxyWorkerFinder(object):
    ...
    def __init__(self, uuid, proxy, topics, beat_periodicity=pr.NOTIFY_PERIOD, worker_expiry=pr.EXPIRES_AFTER):
    ...
    def process_response(self, data, message):
        ...
        LOG.debug("Started processing notify response message '%s'", ku.DelayedPretty(message))
        ...
        with self._cond:
            ...
            if new_or_updated:
                LOG.debug("Updated worker '%s' (%s total workers are currently known)", worker, self.total_workers)
    ...
    def clean(self):
        ...
        """
        if not self._workers or (self._worker_expiry is None or self._worker_expiry <= 0):
            return 0
        dead_workers = {}
        with self._cond:
            now = timeutils.now()
        ...
        if dead_workers and LOG.isEnabledFor(logging.INFO):
            for worker, secs_since_last_seen in dead_workers.values():
                LOG.info("Removed worker '%s' as it has not responded to notification requests in %0.3f seconds", worker, secs_since_last_seen)
