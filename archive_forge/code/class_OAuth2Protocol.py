...
class OAuth2Protocol(AuthProtocol):
    ...
    def process_request(self, request):
        ...
        if not self._is_valid_access_token(request) or 'keystone.token_info' not in request.environ or 'token' not in request.environ['keystone.token_info']:
            ...
            raise webob.exc.HTTPUnauthorized(body=jsonutils.dumps(body), headers=self._reject_auth_headers, charset='UTF-8', content_type='application/json')
