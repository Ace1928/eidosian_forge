...
class TestClaim(base.TestCase):
    ...
    def test_get(self, mock_uuid):
        ...
        sot._translate_response = mock.Mock()
        res = sot.fetch(sess)
        url = 'queues/%(queue)s/claims/%(claim)s' % {'queue': FAKE1['queue_name'], 'claim': FAKE1['id']}
        headers = {'Client-ID': 'NEW_CLIENT_ID', 'X-PROJECT-ID': 'NEW_PROJECT_ID'}
    ...
    def test_get_client_id_project_id_exist(self):
        ...
        sot._translate_response = mock.Mock()
        res = sot.fetch(sess)
        url = 'queues/%(queue)s/claims/%(claim)s' % {'queue': FAKE2['queue_name'], 'claim': FAKE2['id']}
    ...
    def test_update(self, mock_uuid):
        ...
        sot = claim.Claim(**FAKE1)
        res = sot.commit(sess)
        url = 'queues/%(queue)s/claims/%(claim)s' % {'queue': FAKE.pop('queue_name'), 'claim': FAKE['id']}
        headers = {'Client-ID': 'NEW_CLIENT_ID', 'X-PROJECT-ID': 'NEW_PROJECT_ID'}
