...
class BusConnection(Connection):
    ...
    def get_object(self, bus_name, object_path, introspect=True, follow_name_owner_changes=False, **kwargs):
        ...
        if named_service is not None:
            ...
            bus_name = named_service
        if kwargs:
            raise TypeError('get_object does not take these keyword arguments: %s' % ', '.join(kwargs.keys()))
    ...
    def list_names(self):
        ...
        :Since: 0.81.0
        """
        return self.call_blocking(BUS_DAEMON_NAME, BUS_DAEMON_PATH, BUS_DAEMON_IFACE, 'ListNames', '', ())
