...
class SelfRegisteredPortal(object):
    ...
    def get_object_by_name(self, name):
        result = None
        gen_items_responses = self.ise.exec(family='self_registered_portal', function='get_self_registered_portals_generator')
        try:
    ...
    def get_object_by_id(self, id):
        try:
            result = self.ise.exec(family='self_registered_portal', function='get_self_registered_portal_by_id', handle_func_exception=False, params={'id': id}).response['SelfRegPortal']
        except (TypeError, AttributeError) as e:
            self.ise.fail_json(msg='An error occured when executing operation. Check the configuration of your API Settings and API Gateway settings on your ISE server. This collection assumes that the API Gateway, the ERS APIs and OpenAPIs are enabled. You may want to enable the (ise_debug: True) argument. The error was: {error}'.format(error=e))
        except Exception:
            result = None
    ...
    def create(self):
        result = self.ise.exec(family='self_registered_portal', function='create_self_registered_portal', params=self.new_object).response
        return result
    ...
    def update(self):
        ...
        result = self.ise.exec(family='self_registered_portal', function='update_self_registered_portal_by_id', params=self.new_object).response
        return result
    ...
    def delete(self):
        ...
        if not id:
            id_ = self.get_object_by_name(name).get('id')
            self.new_object.update(dict(id=id_))
        result = self.ise.exec(family='self_registered_portal', function='delete_self_registered_portal_by_id', params=self.new_object).response
        return result