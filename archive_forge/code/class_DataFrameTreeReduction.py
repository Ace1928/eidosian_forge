...
class DataFrameTreeReduction(Layer):
    ...
    def __init__(self, name: str, name_input: str, npartitions_input: int, concat_func: Callable, tree_node_func: Callable, finalize_func: Callable | None=None, split_every: int=32, split_out: int | None=None, output_partitions: list[int] | None=None, tree_node_name: str | None=None, annotations: dict[str, Any] | None=None):
        ...
        self.output_partitions = list(range(self.split_out or 1)) if output_partitions is None else output_partitions
        ...
        if self.split_out:
            ...
            for s in self.output_partitions:
                for p in range(self.npartitions_input):
                    dsk[self._make_key(name_input_use, p, split=s)] = (operator.getitem, (self.name_input, p), s)
        if self.height >= 2:
            for s in self.output_partitions:
                for depth in range(1, self.height):
                    for group in range(self.widths[depth]):
                        ...
                        if depth == 1:
                            input_keys = [self._make_key(name_input_use, p, split=s) for p in range(lstart, lstop)]
                        else:
                            input_keys = [self._make_key(self.tree_node_name, p, depth - 1, split=s) for p in range(lstart, lstop)]
                        if depth == self.height - 1:
                            assert group == 0, f'group = {group}, not 0 for final tree reduction task'
                            dsk[self.name, s] = self._define_task(input_keys, final_task=True)
                        else:
                            dsk[self._make_key(self.tree_node_name, group, depth, split=s)] = self._define_task(input_keys, final_task=False)
        else:
            for s in self.output_partitions:
    ...
    def __repr__(self):
        return "DataFrameTreeReduction<name='{}', input_name={}, split_out={}>".format(self.name, self.name_input, self.split_out)
    ...
    def _cull(self, output_partitions):
        return DataFrameTreeReduction(self.name, self.name_input, self.npartitions_input, self.concat_func, self.tree_node_func, finalize_func=self.finalize_func, split_every=self.split_every, split_out=self.split_out, output_partitions=output_partitions, tree_node_name=self.tree_node_name, annotations=self.annotations)

    def cull(self, keys, all_keys):
        """Cull a DataFrameTreeReduction HighLevelGraph layer"""
        deps = {(self.name, 0): {(self.name_input, i) for i in range(self.npartitions_input)}}
