            ...
            for httpsReadCredential in all_httpsRead:
                ...
                if httpsReadOldDescription and httpsReadOldUsername and (not httpsReadDetail):
                    for item in httpsRead_details:
                        if item.get('description') == httpsReadOldDescription and item.get('username') == httpsReadOldUsername:
                            if httpsReadDetail:
                                self.msg = 'More than one httpsRead credential with same                                             old_description and old_username. Pass ID.'
                                self.status = 'failed'
                                return self
                ...
                if httpsReadDescription and httpsReadUsername and (not httpsReadDetail):
                    for item in httpsRead_details:
                        if item.get('description') == httpsReadDescription and item.get('username') == httpsReadUsername:
                            if httpsReadDetail:
                                self.msg = 'More than one httpsRead credential with same                                             description and username. Pass ID.'
                                self.status = 'failed'
                                return self
                            httpsReadDetail = item
    ...
    def get_want_device_credentials(self, CredentialDetails):
                ...
                if not have_cliCredential or have_cliCredential[have_cli_ptr] is None:
                    ...
                    for i in range(0, 3):
                        if item.get(values[i]):
                            create_credential[create_cli_ptr].update({values[i]: item.get(values[i])})
                        else:
                            self.msg = values[i] + ' is mandatory for creating                                        cliCredential ' + str(have_cli_ptr)
                            self.status = 'failed'
                            return self
                    if item.get('enable_password'):
                        create_credential[create_cli_ptr].update({'enablePassword': item.get('enable_password')})
                    ...
                    if item.get('password'):
                        update_credential[update_cli_ptr].update({'password': item.get('password')})
                    else:
                    ...
                    for i in range(1, 4):
                        if item.get(values[i]):
                            update_credential[update_cli_ptr].update({values[i]: item.get(values[i])})
                        else:
                            update_credential[update_cli_ptr].update({values[i]: self.have.get('globalCredential').get('cliCredential')[have_cli_ptr].get(values[i])})
                    if item.get('enable_password'):
                        update_credential[update_cli_ptr].update({'enablePassword': item.get('enable_password')})
        ...
        if CredentialDetails.get('snmp_v2c_read'):
            ...
            for item in snmpV2cRead:
                if not have_snmpV2cRead or have_snmpV2cRead[have_snmpv2cread_ptr] is None:
                    ...
                    for i in range(0, 2):
                        if item.get(values[i]):
                            create_credential[create_snmpv2cread_ptr].update({keys[i]: item.get(values[i])})
                        else:
                    ...
                    if item.get('read_community'):
                        update_credential[update_snmpv2cread_ptr].update({'readCommunity': item.get('read_community')})
                    else:
                    ...
                    for i in range(1, 3):
                        if item.get(values[i]):
                            update_credential[update_snmpv2cread_ptr].update({values[i]: item.get(values[i])})
                        else:
                            update_credential[update_snmpv2cread_ptr].update({values[i]: self.have.get('globalCredential').get('snmpV2cRead')[have_snmpv2cread_ptr].get(values[i])})
        ...
        if CredentialDetails.get('snmp_v2c_write'):
            ...
            for item in snmpV2cWrite:
                if not have_snmpV2cWrite or have_snmpV2cWrite[have_snmpv2cwrite_ptr] is None:
                    ...
                    for i in range(0, 2):
                        if item.get(values[i]):
                            create_credential[create_snmpv2cwrite_ptr].update({keys[i]: item.get(values[i])})
                        else:
                    ...
                    if item.get('write_community'):
                        update_credential[update_snmpv2cwrite_ptr].update({'writeCommunity': item.get('write_community')})
                    else:
                    ...
                    for i in range(1, 3):
                        if item.get(values[i]):
                            update_credential[update_snmpv2cwrite_ptr].update({values[i]: item.get(values[i])})
                        else:
                            update_credential[update_snmpv2cwrite_ptr].update({values[i]: self.have.get('globalCredential').get('snmpV2cWrite')[have_snmpv2cwrite_ptr].get(values[i])})
        ...
        if CredentialDetails.get('https_read'):
            ...
            for item in httpsRead:
                self.log('Global credentials details: {0}'.format(self.have.get('globalCredential')), 'DEBUG')
                if not have_httpsRead or have_httpsRead[have_httpsread_ptr] is None:
                    ...
                    for i in range(0, 3):
                        if item.get(values[i]):
                            create_credential[create_httpsread_ptr].update({values[i]: item.get(values[i])})
                        else:
                            self.msg = values[i] + ' is mandatory for creating                                        httpsRead ' + str(have_httpsread_ptr)
                            self.status = 'failed'
                            return self
                    if item.get('port'):
                        create_credential[create_httpsread_ptr].update({'port': item.get('port')})
                    else:
                    ...
                    update_credential.append({})
                    if item.get('password'):
                        update_credential[update_httpsread_ptr].update({'password': item.get('password')})
                    else:
                    ...
                    for i in range(1, 5):
                        if item.get(values[i]):
                            update_credential[update_httpsread_ptr].update({values[i]: item.get(values[i])})
                        else:
                            update_credential[update_httpsread_ptr].update({values[i]: self.have.get('globalCredential').get('httpsRead')[have_httpsread_ptr].get(values[i])})
        ...
        if CredentialDetails.get('https_write'):
            ...
            for item in httpsWrite:
                if not have_httpsWrite or have_httpsWrite[have_httpswrite_ptr] is None:
                    ...
                    for i in range(0, 3):
                        if item.get(values[i]):
                            create_credential[create_httpswrite_ptr].update({values[i]: item.get(values[i])})
                        else:
                            self.msg = values[i] + ' is mandatory for creating                                        httpsWrite ' + str(have_httpswrite_ptr)
                            self.status = 'failed'
                            return self
                    if item.get('port'):
                        create_credential[create_httpswrite_ptr].update({'port': item.get('port')})
                    else:
                    ...
                    update_credential.append({})
                    if item.get('password'):
                        update_credential[update_httpswrite_ptr].update({'password': item.get('password')})
                    else:
                    ...
                    for i in range(1, 5):
                        if item.get(values[i]):
                            update_credential[update_httpswrite_ptr].update({values[i]: item.get(values[i])})
                        else:
                            update_credential[update_httpswrite_ptr].update({values[i]: self.have.get('globalCredential').get('httpsWrite')[have_httpswrite_ptr].get(values[i])})
        ...
        if CredentialDetails.get('snmp_v3'):
            ...
            for item in snmpV3:
                if not have_snmpV3 or have_snmpV3[have_snmpv3_ptr] is None:
                    ...
                    for i in range(0, 2):
                        if item.get(values[i]):
                            create_credential[create_snmpv3_ptr].update({values[i]: item.get(values[i])})
                        else:
                            self.msg = values[i] + ' is mandatory for creating                                        snmpV3 ' + str(have_snmpv3_ptr)
                            self.status = 'failed'
                            return self
                    if item.get('snmp_mode'):
                        create_credential[create_snmpv3_ptr].update({'snmpMode': item.get('snmp_mode')})
                    else:
                        create_credential[create_snmpv3_ptr].update({'snmpMode': 'AUTHPRIV'})
                    if create_credential[create_snmpv3_ptr].get('snmpMode') == 'AUTHNOPRIV' or create_credential[create_snmpv3_ptr].get('snmpMode') == 'AUTHPRIV':
                        auths = ['auth_password', 'auth_type']
                        keys = {'auth_password': 'authPassword', 'auth_type': 'authType'}
                        for auth in auths:
                            if item.get(auth):
                                create_credential[create_snmpv3_ptr].update({keys[auth]: item.get(auth)})
                            else:
                        ...
                        if len(item.get('auth_password')) < 8:
                            ...
                            self.status = 'failed'
                            return self
                        self.log('snmp_mode: {0}'.format(create_credential[create_snmpv3_ptr].get('snmpMode')), 'DEBUG')
                    if create_credential[create_snmpv3_ptr].get('snmpMode') == 'AUTHPRIV':
                        privs = ['privacy_password', 'privacy_type']
                        key = {'privacy_password': 'privacyPassword', 'privacy_type': 'privacyType'}
                        for priv in privs:
                            if item.get(priv):
                                create_credential[create_snmpv3_ptr].update({key[priv]: item.get(priv)})
                            else:
                    ...
                    for value in values:
                        if item.get(value):
                            update_credential[update_snmpv3_ptr].update({value: item.get(value)})
                        else:
                            update_credential[update_snmpv3_ptr].update({value: self.have.get('globalCredential').get('snmpV3')[have_snmpv3_ptr].get(value)})
                    if item.get('snmp_mode'):
                        update_credential[update_snmpv3_ptr].update({'snmpMode': item.get('snmp_mode')})
                    if update_credential[update_snmpv3_ptr].get('snmpMode') == 'AUTHNOPRIV' or update_credential[update_snmpv3_ptr].get('snmpMode') == 'AUTHPRIV':
                        if item.get('auth_type'):
                            update_credential[update_snmpv3_ptr].update({'authType': item.get('auth_type')})
                        elif self.have.get('globalCredential').get('snmpMode')[have_snmpv3_ptr].get('authType'):
                            update_credential[update_snmpv3_ptr].update({'authType': self.have.get('globalCredential').get('snmpMode')[have_snmpv3_ptr].get('authType')})
                        else:
                            self.msg = 'auth_type is required for updating snmpV3 ' + str(have_snmpv3_ptr)
                            self.status = 'failed'
                            return self
                        if item.get('auth_password'):
                            update_credential[update_snmpv3_ptr].update({'authPassword': item.get('auth_password')})
                        else:
                            self.msg = 'auth_password is required for updating snmpV3 ' + str(have_snmpv3_ptr)
                            self.status = 'failed'
                            return self
                        if len(item.get('auth_password')) < 8:
                            self.msg = 'auth_password length should be greater than 8'
                        ...
                        elif self.have.get('globalCredential').get('snmpMode')[have_snmpv3_ptr].get('privacyType'):
                            update_credential[update_snmpv3_ptr].update({'privacyType': self.have.get('globalCredential').get('snmpMode')[have_snmpv3_ptr].get('privacyType')})
                        else:
                            self.msg = 'privacy_type is required for updating snmpV3 ' + str(have_snmpv3_ptr)
                            self.status = 'failed'
                            return self
                        if item.get('privacy_password'):
                            update_credential[update_snmpv3_ptr].update({'privacyPassword': item.get('privacy_password')})
                        else:
                            self.msg = 'privacy_password is required for updating snmpV3 ' + str(have_snmpv3_ptr)
                            self.status = 'failed'
                            return self
                        if len(item.get('privacy_password')) < 8:
    ...
    def verify_diff_merged(self, config):
        ...
        if config.get('global_credential_details') is not None:
            ...
            if self.want.get('want_update'):
                credential_types = ['cliCredential', 'snmpV2cRead', 'snmpV2cWrite', 'httpsRead', 'httpsWrite', 'snmpV3']
                value_mapping = {'cliCredential': ['username', 'description', 'id'], 'snmpV2cRead': ['description', 'id'], 'snmpV2cWrite': ['description', 'id'], 'httpsRead': ['description', 'username', 'port', 'id'], 'httpsWrite': ['description', 'username', 'port', 'id'], 'snmpV3': ['username', 'description', 'snmpMode', 'id']}
        ...
        if config.get('global_credential_details') is not None:
            ...
            for credential_type in credential_types:
                for item in have_global_credential.get(credential_type):
                    if item is not None:
                        self.msg = 'Delete Global Device Credentials config                                     is not applied to the config'
                        self.status = 'failed'
                        return self
