...
class LayoutsMixin:
    ...
    def container(self, *, height: int | None=None, border: bool | None=None) -> DeltaGenerator:
    ...
    def columns(self, spec: SpecType, *, gap: str | None='small') -> list[DeltaGenerator]:
        ...
        """
        weights = spec
        weights_exception = StreamlitAPIException('The input argument to st.columns must be either a ' + 'positive integer or a list of positive numeric weights. ' + 'See [documentation](https://docs.streamlit.io/library/api-reference/layout/st.columns) ' + 'for more information.')
    ...
    def tabs(self, tabs: Sequence[str]) -> Sequence[DeltaGenerator]:
        ...
        """
        if not tabs:
            raise StreamlitAPIException('The input argument to st.tabs must contain at least one tab label.')
        if any((isinstance(tab, str) == False for tab in tabs)):
            raise StreamlitAPIException('The tabs input list to st.tabs is only allowed to contain strings.')
    ...
    def status(self, label: str, *, expanded: bool=False, state: Literal['running', 'complete', 'error']='running') -> StatusContainer:
        ...
        return StatusContainer._create(self.dg, label=label, expanded=expanded, state=state)
