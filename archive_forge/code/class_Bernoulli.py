...
class Bernoulli(distribution.Distribution):
  ...
  """
    ...
    def _sample_n(self, n, seed=None):
        ...
        uniform = random_ops.random_uniform(new_shape, seed=seed, dtype=self.probs.dtype)
        sample = math_ops.less(uniform, self.probs)
        return math_ops.cast(sample, self.dtype)

    def _log_prob(self, event):
        if self.validate_args:
            event = distribution_util.embed_check_integer_casting_closed(event, target_dtype=dtypes.bool)
        event = math_ops.cast(event, self.logits.dtype)
        logits = self.logits

        def _broadcast(logits, event):
            return (array_ops.ones_like(event) * logits, array_ops.ones_like(logits) * event)
        if not (event.get_shape().is_fully_defined() and logits.get_shape().is_fully_defined() and (event.get_shape() == logits.get_shape())):
            logits, event = _broadcast(logits, event)
        return -nn.sigmoid_cross_entropy_with_logits(labels=event, logits=logits)

    def _entropy(self):
        return -self.logits * (math_ops.sigmoid(self.logits) - 1) + nn.softplus(-self.logits)
