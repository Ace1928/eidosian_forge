...
class SparkLLMTextEmbeddings(BaseModel, Embeddings):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        cls.spark_app_id = convert_to_secret_str(get_from_dict_or_env(values, 'spark_app_id', 'SPARK_APP_ID'))
        cls.spark_api_key = convert_to_secret_str(get_from_dict_or_env(values, 'spark_api_key', 'SPARK_API_KEY'))
        cls.spark_api_secret = convert_to_secret_str(get_from_dict_or_env(values, 'spark_api_secret', 'SPARK_API_SECRET'))
        return values

    def _embed(self, texts: List[str], host: str) -> Optional[List[List[float]]]:
        url = self._assemble_ws_auth_url(request_url=host, method='POST', api_key=self.spark_api_key.get_secret_value(), api_secret=self.spark_api_secret.get_secret_value())
        embed_result: list = []
        for text in texts:
    ...
    def _get_body(appid: str, text: dict) -> Dict[str, Any]:
        body = {'header': {'app_id': appid, 'uid': '39769795890', 'status': 3}, 'parameter': {'emb': {'feature': {'encoding': 'utf8'}}}, 'payload': {'messages': {'text': base64.b64encode(json.dumps(text).encode('utf-8')).decode()}}}
        return body
