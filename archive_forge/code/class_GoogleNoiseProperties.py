...
class GoogleNoiseProperties(devices.SuperconductingQubitsNoiseProperties):
    ...
    def __eq__(self, other):
        ...
        if set(self.readout_errors) != set(other.readout_errors):
            return False
        return all([self.gate_times_ns == other.gate_times_ns, self.t1_ns == other.t1_ns, self.tphi_ns == other.tphi_ns, all((np.allclose(self.readout_errors[q], other.readout_errors[q]) for q in self.readout_errors)), self.gate_pauli_errors == other.gate_pauli_errors, self.validate == other.validate, self.fsim_errors == other.fsim_errors])

    def with_params(self, *, gate_times_ns: Union[None, float, Dict[Type['cirq.Gate'], float]]=None, t1_ns: Union[None, float, Dict['cirq.Qid', float]]=None, tphi_ns: Union[None, float, Dict['cirq.Qid', float]]=None, readout_errors: Union[None, Sequence[float], Dict['cirq.Qid', Sequence[float]]]=None, gate_pauli_errors: Union[None, float, Dict[Union[Type['cirq.Gate'], noise_utils.OpIdentifier], float]]=None, fsim_errors: Union[None, 'cirq.PhasedFSimGate', Dict[Union[Type['cirq.Gate'], noise_utils.OpIdentifier], 'cirq.PhasedFSimGate']]=None):
        ...
        if readout_errors is not None:
            if isinstance(readout_errors, dict):
                replace_args['readout_errors'] = _with_values(self.readout_errors, {k: np.array(v) for k, v in readout_errors.items()})
            else:
                replace_args['readout_errors'] = _with_values(self.readout_errors, np.array(readout_errors))
        if gate_pauli_errors is not None:
            ...
            replace_args['gate_pauli_errors'] = _with_values(self.gate_pauli_errors, gate_pauli_errors)
    ...
    def symmetric_two_qubit_gates(cls) -> Set[type]:
        return {cirq_google.SycamoreGate, cirq.FSimGate, cirq.PhasedFSimGate, cirq.ISwapPowGate, cirq.CZPowGate}
    ...
    def __repr__(self):
        gate_times_repr = ', '.join((f'{key.__module__}.{key.__qualname__}: {val}' for key, val in self.gate_times_ns.items()))
        args = [f'gate_times_ns={{{gate_times_repr}}}', f't1_ns={self.t1_ns!r}', f'tphi_ns={self.tphi_ns!r}', f'readout_errors={_compat.proper_repr(self.readout_errors)}', f'gate_pauli_errors={self.gate_pauli_errors!r}', f'fsim_errors={self.fsim_errors!r}', f'validate={self.validate!r}']
        arglines = ',\n'.join(args)
        return f'cirq_google.GoogleNoiseProperties({arglines})'

    def _json_dict_(self):
        storage_gate_times = {cirq.json_cirq_type(key): val for key, val in self.gate_times_ns.items()}
        return {'gate_times_ns': tuple(storage_gate_times.items()), 't1_ns': tuple(self.t1_ns.items()), 'tphi_ns': tuple(self.tphi_ns.items()), 'readout_errors': tuple(((k, v.tolist()) for k, v in self.readout_errors.items())), 'gate_pauli_errors': tuple(self.gate_pauli_errors.items()), 'fsim_errors': tuple(self.fsim_errors.items()), 'validate': self.validate}

    @classmethod
    def _from_json_dict_(cls, gate_times_ns, t1_ns, tphi_ns, readout_errors, gate_pauli_errors, fsim_errors, validate, **kwargs):
        gate_type_times = {cirq.cirq_type_from_json(gate): val for gate, val in gate_times_ns}
        return GoogleNoiseProperties(gate_times_ns=gate_type_times, t1_ns=dict(t1_ns), tphi_ns=dict(tphi_ns), readout_errors={k: np.array(v) for k, v in readout_errors}, gate_pauli_errors=dict(gate_pauli_errors), fsim_errors=dict(fsim_errors), validate=validate)