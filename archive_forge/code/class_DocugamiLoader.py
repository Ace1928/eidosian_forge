...
@deprecated(since='0.0.24', removal='0.2.0', alternative_import='docugami_langchain.DocugamiLoader')
    ...
    def load(self) -> List[Document]:
        ...
        if self.access_token and self.docset_id:
            ...
            for doc in _document_details:
                ...
                chunks += self._load_chunks_for_document(document_id=doc_id, docset_id=self.docset_id, document_name=doc_name, additional_metadata=doc_metadata)
        elif self.file_paths:
            for path in self.file_paths:
                path = Path(path)
                with open(path, 'rb') as file:
                    chunks += self._parse_dgml(content=file.read(), document_name=path.name)
        return chunks