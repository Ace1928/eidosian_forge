...
class VMBackupPolicy(AzureRMModuleBaseExt):
    ...
    def __init__(self):
        self.module_arg_spec = dict(resource_group=dict(type='str', required=True), name=dict(type='str', required=True), vault_name=dict(type='str', required=True), time=dict(type='str', default='12:00'), weekdays=dict(type='list', elements='str', default=['Monday']), weeks=dict(type='list', elements='str', default=['First']), months=dict(type='list', elements='str', default=['January']), count=dict(type='int', default=1), state=dict(type='str', default='present', choices=['present', 'absent']))
        ...
        super(VMBackupPolicy, self).__init__(derived_arg_spec=self.module_arg_spec, supports_check_mode=True, supports_tags=True)
