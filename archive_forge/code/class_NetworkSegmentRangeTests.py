...
class NetworkSegmentRangeTests(common.NetworkTests):
    ...
    def test_network_segment_range_list(self):
        name = uuid.uuid4().hex
        json_output = self.openstack(' network segment range create ' + '--shared ' + '--network-type geneve ' + '--minimum 2013 ' + '--maximum 2017 ' + name, parse_output=True)
        network_segment_range_id = json_output.get('id')
        network_segment_range_name = json_output.get('name')
        self.addCleanup(self.openstack, 'network segment range delete ' + network_segment_range_id)
        self.assertEqual(name, json_output['name'])
        json_output = self.openstack('network segment range list', parse_output=True)
