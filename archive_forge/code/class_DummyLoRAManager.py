...
class DummyLoRAManager:
    ...
    def init_packed_lora(self, module_name: str, input_dim: int, output_dims: List[int], noop_lora_index: List[int]=None, rank=8):
        base_loras = []
        noop_lora_index = set(noop_lora_index or [])
        for i, out_dim in enumerate(output_dims):
            base_lora = self.init_lora(module_name + '_000_' + str(i), input_dim, out_dim, rank=rank, noop=i in noop_lora_index)
