...
class LuadnsDNSDriver(DNSDriver):
    ...
    def _to_zone(self, item):
        ...
        zone = Zone(domain=item['name'], id=item['id'], type=None, ttl=None, driver=self, extra=extra)
        return zone
    ...
    def _to_record(self, item, zone):
        ...
        for key in item:
            if key not in common_attr:
                extra[key] = item.get(key)
        record = Record(id=item['id'], name=item['name'], type=item['type'], data=item['content'], zone=zone, driver=self, extra=extra)
        return record
