...
def test_turbomole_2h2o():
    ...
    while any([conv[key][-1] > thres[key] for key in conv]):
        ...
        conv['e'].append((abs(new1 - old1) + abs(new2 - old2)) / (abs(old1) + abs(old2)))
        new1, old1 = (prop['c1'][-1], prop['c1'][-2])
        new2, old2 = (prop['c2'][-1], prop['c2'][-2])
        conv['c'].append((norm(new1 - old1) + norm(new2 - old2)) / (norm(old1) + norm(old2)))
        fmt = 'iteration {0:d}: convergence of energy {1:10e}; of charges {2:10e}'
        print(fmt.format(iteration, conv['e'][-1], norm(conv['c'][-1])))
    ref = {'e1': -2077.7082947500003, 'e2': -2077.3347674372353, 'c1': [-0.133033, 0.238218, -0.105186], 'c2': [-0.844336, 0.422151, 0.422184]}
    dev = {}
    for key in ref:
        val = prop[key][-1]
    ...
    assert all([dev[key] < 0.05 for key in dev]), 'deviation too large'