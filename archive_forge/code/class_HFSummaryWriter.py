...
class HFSummaryWriter(SummaryWriter):
    ...
    def __new__(cls, *args, **kwargs) -> 'HFSummaryWriter':
        if not is_tensorboard_available():
            raise ImportError('You must have `tensorboard` installed to use `HFSummaryWriter`. Please run `pip install --upgrade tensorboardX` first.')
        return super().__new__(cls)

    def __init__(self, repo_id: str, *, logdir: Optional[str]=None, commit_every: Union[int, float]=5, squash_history: bool=False, repo_type: Optional[str]=None, repo_revision: Optional[str]=None, repo_private: bool=False, path_in_repo: Optional[str]='tensorboard', repo_allow_patterns: Optional[Union[List[str], str]]='*.tfevents.*', repo_ignore_patterns: Optional[Union[List[str], str]]=None, token: Optional[str]=None, **kwargs):
        ...
        self.scheduler = CommitScheduler(folder_path=self.logdir, path_in_repo=path_in_repo, repo_id=repo_id, repo_type=repo_type, revision=repo_revision, private=repo_private, token=token, allow_patterns=repo_allow_patterns, ignore_patterns=repo_ignore_patterns, every=commit_every, squash_history=squash_history)
