...
class PromptLayerCallbackHandler(BaseCallbackHandler):
    ...
    def __init__(self, pl_id_callback: Optional[Callable[..., Any]]=None, pl_tags: Optional[List[str]]=None) -> None:
    ...
    def on_chat_model_start(self, serialized: Dict[str, Any], messages: List[List[BaseMessage]], *, run_id: UUID, parent_run_id: Optional[UUID]=None, tags: Optional[List[str]]=None, **kwargs: Any) -> Any:
        self.runs[run_id] = {'messages': [self._create_message_dicts(m)[0] for m in messages], 'invocation_params': kwargs.get('invocation_params', {}), 'name': '.'.join(serialized['id']), 'request_start_time': datetime.datetime.now().timestamp(), 'tags': tags}

    def on_llm_start(self, serialized: Dict[str, Any], prompts: List[str], *, run_id: UUID, parent_run_id: Optional[UUID]=None, tags: Optional[List[str]]=None, **kwargs: Any) -> Any:
        self.runs[run_id] = {'prompts': prompts, 'invocation_params': kwargs.get('invocation_params', {}), 'name': '.'.join(serialized['id']), 'request_start_time': datetime.datetime.now().timestamp(), 'tags': tags}
    ...
    def on_llm_end(self, response: LLMResult, *, run_id: UUID, parent_run_id: Optional[UUID]=None, **kwargs: Any) -> None:
        ...
        for i in range(len(response.generations)):
            ...
            model_response = [self._convert_message_to_dict(generation.message)] if is_chat_model and isinstance(generation, ChatGeneration) else resp
            pl_request_id = promptlayer_api_request(run_info.get('name'), 'langchain', model_input, model_params, self.pl_tags, model_response, run_info.get('request_start_time'), run_info.get('request_end_time'), get_api_key(), return_pl_id=bool(self.pl_id_callback is not None), metadata={'_langchain_run_id': str(run_id), '_langchain_parent_run_id': str(parent_run_id), '_langchain_tags': str(run_info.get('tags', []))})
