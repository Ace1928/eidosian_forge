...
class KeystoneAuth(base.AuthBackend):
    ...
    def authenticate(self, api_version, request):
        ...
        if not token or not request.endpoint:
            ks_kwargs = {}
            keys = ('username', 'password', 'project_id', 'project_name', 'auth_url', 'insecure', 'cacert', 'region_name', 'user_domain_name', 'user_domain_id', 'project_domain_name', 'project_domain_id')
            for k in keys:
                ks_kwargs.update({k: get_options(k)})
            ks_session = request.session or self._get_keystone_session(**ks_kwargs)
            if not token:
