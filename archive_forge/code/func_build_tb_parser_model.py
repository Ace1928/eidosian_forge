...
def build_tb_parser_model(tok2vec: Model[List[Doc], List[Floats2d]], state_type: Literal['parser', 'ner'], extra_state_tokens: bool, hidden_width: int, maxout_pieces: int, use_upper: bool, nO: Optional[int]=None) -> Model:
    """
    ...
    * upper (optional): A feed-forward network that predicts scores from the
    ...
    lower = _define_lower(nO=hidden_width if use_upper else nO, nF=nr_feature_tokens, nI=tok2vec.get_dim('nO'), nP=maxout_pieces)
    upper = None
    if use_upper:
        with use_ops('cpu'):
