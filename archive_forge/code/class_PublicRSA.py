...
class PublicRSA(CryptographyPublicKey):
    ...
    def encode_key_bytes(self) -> bytes:
        ...
        if _exp_len > 255:
            exp_header = b'\x00' + struct.pack('!H', _exp_len)
        else:
    ...
    def from_dnskey(cls, key: DNSKEY) -> 'PublicRSA':
        ...
        rsa_n = keyptr[bytes_:]
        return cls(key=rsa.RSAPublicNumbers(int.from_bytes(rsa_e, 'big'), int.from_bytes(rsa_n, 'big')).public_key(default_backend()))