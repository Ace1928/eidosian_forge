...
class BaseAggregator(Metric):
    ...
    def __init__(self, fn: Union[Callable, str], default_value: Union[Tensor, List], nan_strategy: Union[str, float]='error', state_name: str='value', **kwargs: Any) -> None:
        super().__init__(**kwargs)
        allowed_nan_strategy = ('error', 'warn', 'ignore')
        if nan_strategy not in allowed_nan_strategy and (not isinstance(nan_strategy, float)):
            raise ValueError(f'Arg `nan_strategy` should either be a float or one of {allowed_nan_strategy} but got {nan_strategy}.')
