...
class AlarmManager(base.Manager):
    ...
    def list(self, filters=None, limit=None, marker=None, sorts=None):
        ...
        filter_string = self._filtersdict_to_url(filters) if filters else ''
        url = self.url
        options = []
        if filter_string:
    ...
    def query(self, query=None):
        ...
        query = {'filter': query}
        url = 'v2/query/alarms'
        return self._post(url, headers={'Content-Type': 'application/json'}, data=jsonutils.dumps(query)).json()
    ...
    def create(self, alarm):
        ...
        return self._post(self.url, headers={'Content-Type': 'application/json'}, data=jsonutils.dumps(alarm)).json()
    ...
    def update(self, alarm_id, alarm_update):
        ...
        if 'prometheus_rule' in alarm_update:
            ...
            if 'type' in alarm_update and alarm_update['type'] != alarm['type']:
            ...
            if 'type' in alarm_update and alarm_update['type'] != alarm['type']:
                alarm.pop('%s_rule' % alarm['type'], None)
                alarm['gnocchi_resources_threshold_rule'] = alarm_update['gnocchi_resources_threshold_rule']
            else:
                alarm['gnocchi_resources_threshold_rule'].update(alarm_update.get('gnocchi_resources_threshold_rule'))
            alarm_update.pop('gnocchi_resources_threshold_rule')
        elif 'gnocchi_aggregation_by_metrics_threshold_rule' in alarm_update:
            if 'type' in alarm_update and alarm_update['type'] != alarm['type']:
                alarm.pop('%s_rule' % alarm['type'], None)
                alarm['gnocchi_aggregation_by_metrics_threshold_rule'] = alarm_update['gnocchi_aggregation_by_metrics_threshold_rule']
            else:
                alarm['gnocchi_aggregation_by_metrics_threshold_rule'].update(alarm_update.get('gnocchi_aggregation_by_metrics_threshold_rule'))
            ...
            if 'type' in alarm_update and alarm_update['type'] != alarm['type']:
                alarm.pop('%s_rule' % alarm['type'], None)
                alarm['gnocchi_aggregation_by_resources_threshold_rule'] = alarm_update['gnocchi_aggregation_by_resources_threshold_rule']
            else:
                alarm['gnocchi_aggregation_by_resources_threshold_rule'].update(alarm_update.get('gnocchi_aggregation_by_resources_threshold_rule'))
            alarm_update.pop('gnocchi_aggregation_by_resources_threshold_rule')
            ...
            if alarm_update['loadbalancer_member_health_rule'] is not None:
                alarm['loadbalancer_member_health_rule'] = alarm_update['loadbalancer_member_health_rule']
            alarm_update.pop('loadbalancer_member_health_rule')
        alarm.update(alarm_update)
        return self._put(self.url + '/' + alarm_id, headers={'Content-Type': 'application/json'}, data=jsonutils.dumps(alarm)).json()
    ...
    def set_state(self, alarm_id, state):
        ...
        :type state: str
        """
        return self._put(self.url + '/' + alarm_id + '/state', headers={'Content-Type': 'application/json'}, data='"%s"' % state).json()