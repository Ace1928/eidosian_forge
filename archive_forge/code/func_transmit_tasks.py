    ...
    def wrapper(*args, **kwargs):
        ...
        for dataset in datasets:
            if self.info.task_templates is not None:
                dataset.info.task_templates = [template for template in self.info.task_templates if all((dataset._info.features.get(k) == self._info.features.get(k) for k in template.column_mapping.keys()))]
        return out
