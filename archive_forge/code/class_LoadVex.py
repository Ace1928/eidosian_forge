...
class LoadVex(base.Command):
  ...
  """
    detailed_help = {'DESCRIPTION': '{description}', 'EXAMPLES': '       To load a CSAF security advisory file given an artifact in Artifact Registry and the file on disk, run:\n\n        $ {command} --uri=us-east1-docker.pkg.dev/project123/repository123/someimage@sha256:49765698074d6d7baa82f --source=/path/to/vex/file\n\nTo load a CSAF security advisory file given an artifact with a tag and a file on disk, run:\n\n        $ {command} --uri=us-east1-docker.pkg.dev/project123/repository123/someimage:latest --source=/path/to/vex/file\n    '}
    ca_client = None
    ca_messages = None

    @staticmethod
    def Args(parser):
        parser.add_argument('--uri', required=True, help="The path of the artifact in Artifact Registry. A 'gcr.io' image can also be used if redirection is enabled in Artifact Registry. Make sure 'artifactregistry.projectsettings.get' permission is granted to the current gcloud user to verify the redirection status.")
        parser.add_argument('--source', required=True, help='The path of the VEX file.')
        parser.add_argument('--project', required=False, help='The parent project to load security advisory into.')
        return
