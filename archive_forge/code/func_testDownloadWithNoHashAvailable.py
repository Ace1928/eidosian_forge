...
def testDownloadWithNoHashAvailable(self):
    ...
    object_uri.get_key().etag = '12345'
    dst_dir = self.CreateTempDir()
    log_handler = self.RunCommand('cp', [suri(object_uri), dst_dir], return_log_handler=True)
    warning_messages = log_handler.messages['warning']
    self.assertEqual(2, len(warning_messages))
    self.assertRegex(warning_messages[0], 'Non-MD5 etag \\(12345\\) present for key .*, data integrity checks are not possible')
