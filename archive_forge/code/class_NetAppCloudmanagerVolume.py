...
class NetAppCloudmanagerVolume(object):
    ...
    def __init__(self):
        ...
        self.parameters['working_environment_id'] = working_environment_detail['publicId'] if working_environment_detail.get('publicId') else working_environment_detail['id']
        ...
        if self.parameters.get('svm_name') is None:
            ...
            if self.is_fsx:
                fsx_path = '/svms'
            response, err, dummy = self.rest_api.send_request('GET', '%s/working-environments/%s%s' % (self.rest_api.api_root_path, self.parameters['working_environment_id'], fsx_path), None, None, header=self.headers)
            if err is not None:
        ...
        if self.parameters['volume_protocol'] == 'nfs':
            ...
            if len(extra_options) > 0:
                self.module.fail_json(msg='Error: The following options are not allowed when volume_protocol is nfs:  %s' % extra_options)
            ...
            for option in ['export_policy_type', 'export_policy_ip', 'export_policy_nfs_version', 'igroups', 'os_name', 'initiator']:
                if self.parameters.get(option) is not None:
                    extra_options.append(option)
            if len(extra_options) > 0:
                self.module.fail_json(msg='Error: The following options are not allowed when volume_protocol is cifs: %s' % extra_options)
        else:
            extra_options = []
            for option in ['export_policy_type', 'export_policy_ip', 'export_policy_nfs_version', 'share_name', 'permission', 'users']:
                if self.parameters.get(option) is not None:
                    extra_options.append(option)
            if len(extra_options) > 0:
                self.module.fail_json(msg='Error: The following options are not allowed when volume_protocol is iscsi: %s' % extra_options)
        if self.parameters.get('igroups'):
            ...
            if any((isinstance(x, dict) for x in current_igroups)) and None in current_igroups:
                self.module.fail_json(changed=False, msg='Error: can not specify existingigroup and new igroup together.')
    ...
    def get_volume(self):
        ...
        response, err, dummy = self.rest_api.send_request('GET', '%s/volumes?%s=%s' % (self.rest_api.api_root_path, query_param, self.parameters['working_environment_id']), None, header=self.headers)
        if err is not None:
    ...
    def create_volume(self):
        exclude_list = ['client_id', 'size_unit', 'export_policy_name', 'export_policy_type', 'export_policy_ip', 'export_policy_nfs_version', 'capacity_tier']
        ...
        if not self.is_fsx:
            response, err, dummy = self.rest_api.send_request('POST', '%s/volumes/quote' % self.rest_api.api_root_path, None, quote, header=self.headers)
            if err is not None:
        ...
        response, err, on_cloud_request_id = self.rest_api.send_request('POST', '%s/volumes?createAggregateIfNotFound=%s' % (self.rest_api.api_root_path, create_aggregate_if_not_exists), None, quote, header=self.headers)
        if err is not None:
    ...
    def modify_volume(self, modify):
        ...
        response, err, dummy = self.rest_api.send_request('PUT', '%s/volumes/%s/%s/%s' % (self.rest_api.api_root_path, self.parameters['working_environment_id'], self.parameters['svm_name'], self.parameters['name']), None, vol, header=self.headers)
        if err is not None:
            self.module.fail_json(changed=False, msg='Error: unexpected response on modifying volume: %s, %s' % (str(err), str(response)))

    def delete_volume(self):
        response, err, dummy = self.rest_api.send_request('DELETE', '%s/volumes/%s/%s/%s' % (self.rest_api.api_root_path, self.parameters['working_environment_id'], self.parameters['svm_name'], self.parameters['name']), None, None, header=self.headers)
        if err is not None:
    ...
    def get_initiator(self, alias_name):
        response, err, dummy = self.rest_api.send_request('GET', '%s/volumes/initiator' % self.rest_api.api_root_path, None, header=self.headers)
        if err is not None:
    ...
    def create_initiator(self, initiator):
        ...
        response, err, dummy = self.rest_api.send_request('POST', '%s/volumes/initiator' % self.rest_api.api_root_path, None, ini, header=self.headers)
        if err is not None:
    ...
    def get_igroup(self, igroup_name):
        response, err, dummy = self.rest_api.send_request('GET', '%s/volumes/igroups/%s/%s' % (self.rest_api.api_root_path, self.parameters['working_environment_id'], self.parameters['svm_name']), None, None, header=self.headers)
        if err is not None:
    ...
    def apply(self):
        ...
        if cd_action is None:
            ...
            unmodifiable = []
            for attr in modify:
                if attr not in ['export_policy_ip', 'export_policy_nfs_version', 'snapshot_policy_name', 'users', 'permission', 'tiering_policy', 'snapshot_policy_name']:
