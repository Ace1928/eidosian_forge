...
class NormalizeTypeIs(Transformation):
    ...
    def visit_Compare(self, node):
        ...
        typecalls = [expr for i, expr in enumerate(comparators) if i != typecall_index]
        return reduce(lambda x, y: ast.BinOp(x, ast.BitAnd(), ast.Call(ast.Attribute(ast.Name('builtins', ast.Load(), None, None), 'isinstance', ast.Load()), [checked_value, y], [])), typecalls[1:], ast.Call(ast.Attribute(ast.Name('builtins', ast.Load(), None, None), 'isinstance', ast.Load()), [checked_value, typecalls[0]], []))