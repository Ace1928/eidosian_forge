...
class Sflow(object):
    ...
    def config_agent(self):
        ...
        elif self.agent_ip == self.sflow_dict['agent'].get('ipv4Addr') or self.agent_ip == self.sflow_dict['agent'].get('ipv6Addr'):
    ...
    def config_source(self):
        ...
        for src in self.sflow_dict['source']:
            if src.get('family') == self.source_version:
                ...
                break
        if self.state == 'present':
            if self.source_ip != src_dict.get('ipv4Addr') and self.source_ip != src_dict.get('ipv6Addr'):
                ...
                if self.source_version == 'ipv4':
                    ...
                    self.updates_cmd.append('sflow source ipv6 %s' % self.source_ip)
                xml_str += '</source ></sources>'
        elif self.source_ip == src_dict.get('ipv4Addr'):
