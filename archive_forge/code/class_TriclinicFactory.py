...
class TriclinicFactory(Bravais):
    ...
    def make_crystal_basis(self):
        ...
        lattice = np.array([[a, 0, 0], [b * cosg, b * sing, 0], [c * cosb, c * (cosa - cosb * cosg) / sing, c * np.sqrt(sinb ** 2 - ((cosa - cosb * cosg) / sing) ** 2)]])
        ...
        assert abs(np.dot(lattice[0], lattice[1]) - a * b * cosg) < 1e-05
        assert abs(np.dot(lattice[0], lattice[2]) - a * c * cosb) < 1e-05
        assert abs(np.dot(lattice[1], lattice[2]) - b * c * cosa) < 1e-05
        assert abs(np.dot(lattice[0], lattice[0]) - a * a) < 1e-05
        assert abs(np.dot(lattice[1], lattice[1]) - b * b) < 1e-05
        assert abs(np.dot(lattice[2], lattice[2]) - c * c) < 1e-05