...
class WorkflowManager(base.ResourceManager):
    ...
    def create(self, definition, namespace='', scope='private'):
        ...
        return self._create('/workflows?scope=%s&namespace=%s' % (scope, namespace), definition, response_key='workflows', dump_json=False, headers={'content-type': 'text/plain'}, is_iter_resp=True)
    ...
    def update(self, definition, namespace='', scope='private', id=None):
        ...
        if id:
            is_iter_resp = False
            response_key = None
        return self._update('%s?namespace=%s&scope=%s' % (url_pre, namespace, scope), definition, response_key=response_key, dump_json=False, headers={'content-type': 'text/plain'}, is_iter_resp=is_iter_resp)
    ...
    def list(self, namespace='', marker='', limit=None, sort_keys='', sort_dirs='', fields='', **filters):
        ...
        return self._list('/workflows%s' % query_string, response_key='workflows')

    def get(self, identifier, namespace=''):
        self._ensure_not_empty(identifier=identifier)
        return self._get('/workflows/%s?namespace=%s' % (identifier, namespace))
    ...
    def validate(self, definition):
        self._ensure_not_empty(definition=definition)
        definition = self.get_contents_if_file(definition)
        return self._validate('/workflows/validate', definition, dump_json=False, headers={'content-type': 'text/plain'})