...
class TransresnetMultimodalAgent(TransresnetAgent):
    ...
    def add_cmdline_args(argparser):
        ...
        argparser.add_argument('--personality-override', type=str, default=None, help='for use in other tasks where no personality is given. This will give the model a personality (whichever is specifed).')
        argparser.add_argument('--personalities-path', type=str, default=None, help='Path to personalities list')
        DictionaryAgent.add_cmdline_args(argparser)
        return arg_group

    def __init__(self, opt, shared=None):
        super().__init__(opt, shared)
        self.metrics = {k: {'hits@1/100': 0.0, 'loss': 0.0, 'num_samples': 0, 'med_rank': []} for k in ['first_round', 'second_round', 'third_round+']}
        if shared is None:
            self.optimizer = optim.Adam(filter(lambda p: p.requires_grad, self.model.parameters()), self.opt['learningrate'])
        else:
    ...
    def _build_model(self, path=None):
        ...
        self.model = TransresnetMultimodalModel(self.opt, self.personalities_list, self.dict)
    ...
    def train_step(self, valid_obs, image_feats, personalities, dialogue_histories):
        ...
        loss, num_correct, _ = self.model(image_feats, personalities, dialogue_histories, labels, batchsize=len(valid_obs))
    ...
    def eval_step(self, valid_obs, image_feats, personalities, dialogue_histories):
        ...
        chosen_responses = self.model.choose_best_response(image_feats, personalities, dialogue_histories, candidates, candidates_encoded=candidates_encoded, k=-1 if self.fixed_cands is None else 100, batchsize=len(valid_obs))
        loss = -1
        if self.fixed_cands is not None:
            num_correct = 0
        else:
            labels = [v.get('eval_labels') for v in valid_obs]
            equality_list = [1 if chosen_responses[i][0] in labels[i] else 0 for i in range(len(labels))]
            med_rank = []
    ...
    def batch_act(self, observations):
        ...
        if is_training:
            ...
            loss, num_correct = self.train_step(valid_obs, image_feats, personalities, dialogue_histories)
            loss.backward()
            self.optimizer.step()
        else:
            loss, num_correct, med_rank, chosen_responses = self.eval_step(valid_obs, image_feats, personalities, dialogue_histories)
        self.update_metrics(loss, num_correct, len(valid_obs), dialogue_round, med_rank)
        result = [{'text': 'No Response During Traiing', 'id': self.getID()} for _ in range(len(observations))]
    ...
    def update_metrics(self, loss, num_correct, num_samples, dialogue_round, med_rank=None):
        """
        Update Metrics.
    ...
    def receive_metrics(self, metrics_dict):
        ...
        if self.freeze_patience != -1 and self.is_frozen:
            m_key = 'hits@1/100'
            ms = [metrics_dict[r].get(m_key, -1) for r in ['first_round', 'second_round', 'third_round+']]
    ...
    def report(self):
        ...
        for k, v in self.metrics.items():
            ...
            if v['num_samples'] > 0:
                m[f'{k}/hits@1/100'] = round_sigfigs(v['hits@1/100'] / v['num_samples'], 4)
