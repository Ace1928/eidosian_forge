...
@staticmethod
def _footer_exit_status_info(exit_code: Optional[int], *, settings: 'Settings', printer: Union['PrinterTerm', 'PrinterJupyter']) -> None:
    if settings.silent:
        return
    status = '(success).' if not exit_code else f'(failed {exit_code}).'
    info = [f'Waiting for W&B process to finish... {printer.status(status, bool(exit_code))}']
