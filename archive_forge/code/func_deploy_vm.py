...
def deploy_vm(self):
    ...
    if task.info.state == 'error':
        ...
        kwargs = {'changed': self.change_applied, 'failed': True, 'msg': task.info.error.msg, 'clonespec': clonespec_json, 'configspec': configspec_json, 'clone_method': clone_method}
        return kwargs
    else:
        vm = task.info.result
        ...
        if self.params['wait_for_ip_address'] or self.params['wait_for_customization'] or self.params['state'] in ['poweredon', 'powered-on', 'restarted']:
            ...
            if self.params['wait_for_customization']:
                ...
                if not is_customization_ok:
                    ...
                    return {'changed': self.change_applied, 'failed': True, 'msg': 'Customization failed. For detailed information see warnings', 'instance': vm_facts, 'op': 'customization'}
        vm_facts = self.gather_facts(vm)
