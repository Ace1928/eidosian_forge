...
class RedisLock(utils.LockBase):
    ...
    def acquire(self, timeout: typing.Optional[float]=None, check_interval: typing.Optional[float]=None, fail_when_locked: typing.Optional[bool]=None):
        ...
        for _ in timeout_generator:
            subscribers = connection.pubsub_numsub(self.channel)[0][1]
            if subscribers:
                logger.debug('Found %d lock subscribers for %s', subscribers, self.channel)
                if self.check_or_kill_lock(connection, self.unavailable_timeout):
                    continue
                else:
                    subscribers = 0
            if not subscribers:
