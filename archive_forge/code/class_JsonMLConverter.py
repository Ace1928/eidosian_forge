...
class JsonMLConverter(XMLSchemaConverter):
    ...
    def __init__(self, namespaces: Optional[NamespacesType]=None, dict_class: Optional[Type[Dict[str, Any]]]=None, list_class: Optional[Type[List[Any]]]=None, **kwargs: Any) -> None:
        kwargs.update(attr_prefix='', text_key='', cdata_prefix='')
        super(JsonMLConverter, self).__init__(namespaces, dict_class, list_class, **kwargs)
    ...
    def element_decode(self, data: ElementData, xsd_element: 'XsdElement', xsd_type: Optional[BaseXsdType]=None, level: int=0) -> Any:
        ...
        if xsd_type.model_group is not None:
            result_list.extend([value if value is not None else self.list([name]) for name, value, _ in self.map_content(data.content)])
        ...
        if level == 0 and xsd_element.is_global() and (not self.strip_namespaces) and self:
            attributes.update(((f'xmlns:{k}' if k else 'xmlns', v) for k, v in self._namespaces.items()))
        if attributes:
    ...
    def element_encode(self, obj: Any, xsd_element: 'XsdElement', level: int=0) -> ElementData:
        ...
        elif data_len == content_index + 1 and (xsd_element.type.simple_type is not None or (not xsd_element.type.content and xsd_element.type.mixed)):
            return ElementData(xsd_element.name, obj[content_index], [], attributes)
        else:
            cdata_num = iter(range(1, data_len))
            content = [(self.unmap_qname(e[0]), e) if isinstance(e, MutableSequence) else (next(cdata_num), e) for e in obj[content_index:]]
