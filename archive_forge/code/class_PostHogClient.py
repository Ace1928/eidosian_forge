...
class PostHogClient(abc.ABC):
    ...
    def __init__(self, endpoint: Optional[str]=None, enabled: Optional[bool]=None, api_key: Optional[str]=None, project_id: Optional[str]=None, client_timeout: Optional[float]=None, batch_size: Optional[int]=None, batch_interval: Optional[float]=None, debug_enabled: Optional[bool]=None, default_retries: Optional[int]=None, batched: Optional[bool]=None, num_workers: Optional[int]=None, upkeep_interval: Optional[float]=0.1, dryrun: Optional[bool]=None, **kwargs):
        """
        ...
        self.dryrun = dryrun
        self._kwargs = kwargs
        self.configure(endpoint=endpoint, enabled=enabled, api_key=api_key, project_id=project_id, client_timeout=client_timeout, batch_size=batch_size, batch_interval=batch_interval, debug_enabled=debug_enabled, default_retries=default_retries, batched=batched, num_workers=num_workers)
    ...
    def configure(self, endpoint: Optional[str]=None, enabled: Optional[bool]=None, api_key: Optional[str]=None, project_id: Optional[str]=None, client_timeout: Optional[float]=None, batch_size: Optional[int]=None, batch_interval: Optional[float]=None, debug_enabled: Optional[bool]=None, default_retries: Optional[int]=None, batched: Optional[bool]=None, num_workers: Optional[int]=None, upkeep_interval: Optional[float]=0.1, dryrun: Optional[bool]=None, **kwargs):
        """
        ...
        if project_id is not None:
            self.settings.project_id = project_id
        if client_timeout is not None:
            self.settings.client_timeout = client_timeout
        if batch_size is not None:
            self.settings.batch_size = batch_size
        if batch_interval is not None:
            self.settings.batch_interval = batch_interval
        if debug_enabled is not None:
            self.settings.debug_enabled = debug_enabled
        if batched is not None:
            self.settings.batched = batched
        if num_workers is not None:
            self.settings.num_workers = num_workers
        if default_retries is not None:
            self.settings.default_retries = default_retries
        if upkeep_interval is not None:
    ...
    def get_session_kwargs(self, **kwargs) -> Dict[str, Any]:
        ...
        """
        return {'pool_connections': self._kwargs.get('pool_connections', 10), 'pool_maxsize': self._kwargs.get('pool_maxsize', 10), 'retries': self._kwargs.get('retries', self.settings.default_retries), 'resolver': self._kwargs.get('resolver', 'doh+cloudflare://'), 'multiplexed': True, **kwargs}
    ...
    def asession(self) -> 'niquests.AsyncSession':
        ...
        if self._asession is None:
            self._asession = niquests.AsyncSession(**self.get_session_kwargs())
    ...
    async def send_events(self, force: Optional[bool]=None, batched: Optional[bool]=None) -> Tuple[List[niquests.Response], int]:
        """
        Sends the events
        """
                ...
                if self.event_queue.capture_events:
                    ...
                    if batched:
                        if self.dryrun:
                            ...
                            responses.append(await req_func(self.endpoint.batch, json={'batch': events, **base_json}))
                    else:
                        for event in events:
                            if self.dryrun:
    ...
    async def upkeep(self, worker_n: Optional[int]=None):
        ...
        while not self.event.is_set():
            try:
                ...
                await asyncio.sleep(self.upkeep_interval)
    ...
    def capture(self, event: Optional[str]=None, distinct_id: Optional[str]=None, properties: Optional[Dict[str, Any]]=None, **_kwargs: Any) -> Callable[..., RT]:
        """
    ...
    def flush(self, force: Optional[bool]=True) -> None:
        ...
        Flushes the events
        """
        self.tasks.add(asyncio.create_task(self.send_events(force=force)))
