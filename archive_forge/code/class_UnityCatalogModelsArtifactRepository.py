...
class UnityCatalogModelsArtifactRepository(ArtifactRepository):
    ...
    def __init__(self, artifact_uri, registry_uri):
        if not is_databricks_unity_catalog_uri(registry_uri):
            raise MlflowException(message=f"Attempted to instantiate an artifact repo to access models in the Unity Catalog with non-Unity Catalog registry URI '{registry_uri}'. Please specify a Unity Catalog registry URI of the form '{_DATABRICKS_UNITY_CATALOG_SCHEME}[://profile]', e.g. by calling mlflow.set_registry_uri('{_DATABRICKS_UNITY_CATALOG_SCHEME}') if using the MLflow Python client", error_code=INVALID_PARAMETER_VALUE)
        super().__init__(artifact_uri)
        from mlflow.tracking.client import MlflowClient
        registry_uri_from_artifact_uri = get_databricks_profile_uri_from_artifact_uri(artifact_uri, result_scheme=_DATABRICKS_UNITY_CATALOG_SCHEME)
        ...
        if key_prefix is not None:
            raise MlflowException("Remote model registry access via model URIs of the form 'models://<scope>@<prefix>/<model_name>/<version_or_stage>' is unsupported for models in the Unity Catalog. We recommend that you access the Unity Catalog from the current Databricks workspace instead.")
    ...
    def _get_scoped_token(self):
        ...
        req_body = message_to_json(GenerateTemporaryModelVersionCredentialsRequest(name=self.model_name, version=self.model_version, operation=MODEL_VERSION_OPERATION_READ))
        response_proto = GenerateTemporaryModelVersionCredentialsResponse()
        return call_endpoint(host_creds=db_creds, endpoint=endpoint, method=method, json_body=req_body, response_proto=response_proto).credentials
    ...
    def _get_artifact_repo(self):
        ...
        if MLFLOW_UNITY_CATALOG_PRESIGNED_URLS_ENABLED.get():
            return PresignedUrlArtifactRepository(get_databricks_host_creds(self.registry_uri), self.model_name, self.model_version)
        scoped_token = self._get_scoped_token()
        blob_storage_path = self._get_blob_storage_path()
        return get_artifact_repo_from_storage_info(storage_location=blob_storage_path, scoped_token=scoped_token)
