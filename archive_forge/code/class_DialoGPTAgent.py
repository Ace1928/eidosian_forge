...
class DialoGPTAgent(HuggingfaceAgent, DecoderLM):
    ...
    def __init__(self, model, device, maxlen):
        ...
        super().__init__(name=model, suffix='<|endoftext|>', device=device, maxlen=maxlen, model=GPT2LMHeadModel.from_pretrained(name).to(device).eval(), tokenizer=tokenizer)

    @staticmethod
    def available_models():
        return ['dialogpt.small', 'dialogpt.medium', 'dialogpt.large']
