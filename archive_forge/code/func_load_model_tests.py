...
def load_model_tests(data_dir: str=DATA_DIR, kind: Optional[str]=None) -> List[TestCase]:
    ...
    for test_name in os.listdir(kind_dir):
        ...
        if os.path.exists(os.path.join(case_dir, 'model.onnx')):
            ...
            with open(os.path.join(case_dir, 'data.json')) as f:
                ...
                rtol = data.get('rtol', 0.001)
                atol = data.get('atol', 1e-07)
                model_dir = None
        testcases.append(TestCase(name=test_name, url=url, model_name=model_name, model_dir=model_dir, model=None, data_sets=None, kind=kind, rtol=rtol, atol=atol))
    return testcases