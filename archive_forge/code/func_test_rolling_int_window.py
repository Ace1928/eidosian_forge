...
def test_rolling_int_window(center, closed, as_index):
    ...
    md_window = md_df.groupby('by', as_index=as_index).rolling(3, center=center, closed=closed)
    pd_window = pd_df.groupby('by', as_index=as_index).rolling(3, center=center, closed=closed)
