...
def test_disable_validate_required_param(self):
    ...
    self.stack = stack.Stack(self.ctx, 'stack_with_reqd_param', template.Template(tmpl))
    ex = self.assertRaises(exception.UserParameterMissing, self.stack.validate)
    self.assertIn('The Parameter (aparam) was not provided', str(ex))
