...
class FBetaScore(base_metric.Metric):
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):
        ...
        if self.threshold is None:
            ...
            y_pred = tf.logical_and(y_pred >= threshold, tf.abs(y_pred) > 1e-09)
        else:
    ...
    def result(self):
        ...
        if self.average == 'weighted':
            weights = tf.math.divide_no_nan(self.intermediate_weights, tf.reduce_sum(self.intermediate_weights))
    ...
    def get_config(self):
        ...
        config = {'average': self.average, 'beta': self.beta, 'threshold': self.threshold}
