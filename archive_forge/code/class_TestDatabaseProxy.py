...
class TestDatabaseProxy(test_proxy_base.TestProxyBase):
    ...
    def test_database_delete(self):
        self.verify_delete(self.proxy.delete_database, database.Database, ignore_missing=False, method_kwargs={'instance': 'test_id'}, expected_kwargs={'instance_id': 'test_id'})
    ...
    def test_database_find(self):
        self._verify('openstack.proxy.Proxy._find', self.proxy.find_database, method_args=['db', 'instance'], expected_args=[database.Database, 'db'], expected_kwargs={'instance_id': 'instance', 'ignore_missing': True})
    ...
    def test_user_create_attrs(self):
        self.verify_create(self.proxy.create_user, user.User, method_kwargs={'instance': 'id'}, expected_kwargs={'instance_id': 'id'})

    def test_user_delete(self):
        self.verify_delete(self.proxy.delete_user, user.User, False, method_kwargs={'instance': 'id'}, expected_kwargs={'instance_id': 'id'})

    def test_user_delete_ignore(self):
        self.verify_delete(self.proxy.delete_user, user.User, True, method_kwargs={'instance': 'id'}, expected_kwargs={'instance_id': 'id'})

    def test_user_find(self):
        self._verify('openstack.proxy.Proxy._find', self.proxy.find_user, method_args=['user', 'instance'], expected_args=[user.User, 'user'], expected_kwargs={'instance_id': 'instance', 'ignore_missing': True})

    def test_users(self):
        self.verify_list(self.proxy.users, user.User, method_args=['test_instance'], expected_args=[], expected_kwargs={'instance_id': 'test_instance'})
