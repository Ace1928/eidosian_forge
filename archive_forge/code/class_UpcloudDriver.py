...
class UpcloudDriver(NodeDriver):
    ...
    NODE_STATE_MAP = {'started': NodeState.RUNNING, 'stopped': NodeState.STOPPED, 'maintenance': NodeState.RECONFIGURING, 'error': NodeState.ERROR}
    ...
    def list_sizes(self, location=None):
        ...
        return self._to_node_sizes(response.object['plans']['plan'], prices_response.object['prices']['zone'], location)
    ...
    def create_node(self, name, size, image, location, auth=None, ex_hostname='localhost', ex_username='root'):
        """
        ...
        :rtype: :class:`.Node`
        """
        body = UpcloudCreateNodeRequestBody(name=name, size=size, image=image, location=location, auth=auth, ex_hostname=ex_hostname, ex_username=ex_username)
    ...
    def reboot_node(self, node):
        ...
        self.connection.request('1.2/server/{}/restart'.format(node.id), method='POST', data=json.dumps(body))
        return True
    ...
    def _to_node(self, server, state=None):
        ...
        return Node(id=server['uuid'], name=server['title'], state=state or self.NODE_STATE_MAP[server['state']], public_ips=public_ips, private_ips=private_ips, driver=self, extra=extra)
    ...
    def _to_node_size(self, plan, plan_price, location):
        ...
        return NodeSize(id=plan['name'], name=plan['name'], ram=plan['memory_amount'], disk=plan['storage_size'], bandwidth=plan['public_traffic_out'], price=plan_price.get_price(plan['name'], location), driver=self, extra=extra)
