...
def max_pool(input: _atypes.TensorFuzzingAnnotation[TV_MaxPool_T], ksize, strides, padding: str, explicit_paddings=[], data_format: str='NHWC', name=None) -> _atypes.TensorFuzzingAnnotation[TV_MaxPool_T]:
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'MaxPool', name, input, 'ksize', ksize, 'strides', strides, 'padding', padding, 'explicit_paddings', explicit_paddings, 'data_format', data_format)
            return _result
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('MaxPool', input=input, ksize=ksize, strides=strides, padding=padding, explicit_paddings=explicit_paddings, data_format=data_format, name=name)
    _result = _outputs[:]
    if _execute.must_record_gradient():
        _attrs = ('T', _op._get_attr_type('T'), 'ksize', _op.get_attr('ksize'), 'strides', _op.get_attr('strides'), 'padding', _op.get_attr('padding'), 'explicit_paddings', _op.get_attr('explicit_paddings'), 'data_format', _op.get_attr('data_format'))
