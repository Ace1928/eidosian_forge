    ...
    **Mixed**
    ...
    def from_components(cls, k_states, a=None, Pstar=None, Pinf=None, A=None, R0=None, Q0=None):
        ...
        if Pstar is not None and (R0 is not None or Q0 is not None):
            raise ValueError("Cannot specify the initial state covariance both as `Pstar` and as the components R0 and Q0  (because `Pstar` is defined such that `Pstar=R0 Q0 R0'`).")
        if Pinf is not None and A is not None:
            raise ValueError("Cannot specify both the diffuse covariance matrix `Pinf` and the selection matrix for diffuse elements, A, (because Pinf is defined such that `Pinf=A A'`).")
        elif A is not None:
            Pinf = np.dot(A, A.T)
        if a is None:
            a = np.zeros(k_states)
        if len(a) != k_states:
            raise ValueError('Must provide constant initialization vector for the entire state vector.')
        if R0 is not None or Q0 is not None:
            if R0 is None or Q0 is None:
                raise ValueError('If specifying either of R0 or Q0 then you must specify both R0 and Q0.')
            ...
            if Pstar is not None:
                for i in diffuse_ix:
                    if not (np.all(Pstar[i] == 0) and np.all(Pstar[:, i] == 0)):
                        raise ValueError(f'The state at position {i} was specified as diffuse in Pinf, but also contains a non-diffuse diagonal or off-diagonal in Pstar.')
        ...
        init = cls(k_states)
        if nondiffuse_ix:
            nondiffuse_groups = np.split(nondiffuse_ix, np.where(np.diff(nondiffuse_ix) != 1)[0] + 1)
        else:
            nondiffuse_groups = []
    ...
    def set(self, index, initialization_type, constant=None, stationary_cov=None, approximate_diffuse_variance=None):
        ...
        if self.initialization_type is not None and (not index == self._states):
            raise ValueError('Cannot set initialization for the block of  states %s because initialization was previously performed globally. You must either re-initialize globally or else unset the global initialization before initializing specific blocks of states.' % str(index))
        uninitialized = np.equal(self._initialization[index,], None)
        if index not in self.blocks and (not np.all(uninitialized)):
            raise ValueError('Cannot set initialization for the state(s) %s because they are a subset of a previously initialized block. You must either re-initialize the entire block as a whole or else unset the entire block before re-initializing the subset.' % str(np.array(index)[~uninitialized]))
        k_states = len(index)
        if k_states == self.k_states:
            self.initialization_type = initialization_type
            if approximate_diffuse_variance is not None and (not initialization_type == 'approximate_diffuse'):
                raise ValueError('`approximate_diffuse_variance` can only be provided when using approximate diffuse initialization.')
            if stationary_cov is not None and (not initialization_type == 'known'):
                raise ValueError('`stationary_cov` can only be provided when using known initialization.')
            if initialization_type == 'known':
                if constant is None and stationary_cov is None:
                    raise ValueError('Must specify either the constant vector or the stationary covariance matrix (or both) if using known initialization.')
                ...
                if not stationary_cov.shape == (k_states, k_states):
                    raise ValueError('Invalid stationary covariance matrix; given shape %s but require shape %s.' % (str(stationary_cov.shape), str((k_states, k_states))))
                self.stationary_cov = stationary_cov
            elif initialization_type == 'diffuse':
                if constant is not None:
                    warnings.warn('Constant values provided, but they are ignored in exact diffuse initialization.')
                ...
                if constant is not None:
                    raise ValueError('Constant values cannot be provided for stationary initialization.')
            else:
            ...
            if not constant.shape == (k_states,):
                raise ValueError('Invalid constant vector; given shape %s but require shape %s.' % (str(constant.shape), str((k_states,))))
            ...
            if isinstance(initialization_type, Initialization):
                ...
                init = Initialization(k_states, initialization_type, constant=constant, stationary_cov=stationary_cov, approximate_diffuse_variance=approximate_diffuse_variance)
    ...
    def unset(self, index):
        ...
        if k_states == self.k_states and self.initialization_type is not None:
            ...
            raise ValueError('The given index does not correspond to a previously initialized block.')
    ...
    def initialized(self):
        return not (self.initialization_type is None and np.any(np.equal(self._initialization, None)))
