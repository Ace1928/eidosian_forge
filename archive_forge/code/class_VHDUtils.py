...
class VHDUtils(object):
    ...
    def _open(self, vhd_path, open_flag=0, open_access_mask=w_const.VIRTUAL_DISK_ACCESS_ALL, open_params=None):
        device_id = self._get_vhd_device_id(vhd_path)
        vst = vdisk_struct.VIRTUAL_STORAGE_TYPE(DeviceId=device_id, VendorId=w_const.VIRTUAL_STORAGE_TYPE_VENDOR_MICROSOFT)
        handle = wintypes.HANDLE()
        self._run_and_check_output(virtdisk.OpenVirtualDisk, ctypes.byref(vst), ctypes.c_wchar_p(vhd_path), open_access_mask, open_flag, open_params, ctypes.byref(handle))
        return handle
    ...
    def create_dynamic_vhd(self, path, max_internal_size):
        self.create_vhd(path, constants.VHD_TYPE_DYNAMIC, max_internal_size=max_internal_size)
    ...
    def get_vhd_info(self, vhd_path, info_members=None, open_parents=False):
        ...
        open_flag = w_const.OPEN_VIRTUAL_DISK_FLAG_NO_PARENTS if not open_parents else 0
    ...
    def get_vhd_parent_path(self, vhd_path):
        vhd_info = self.get_vhd_info(vhd_path, [w_const.GET_VIRTUAL_DISK_INFO_PARENT_LOCATION])
    ...
    def set_vhd_guid(self, vhd_path, guid):
        ...
        open_params.Version = w_const.OPEN_VIRTUAL_DISK_VERSION_2
        open_params.Version2.GetInfoOnly = False
        handle = self._open(vhd_path, open_flag=w_const.OPEN_VIRTUAL_DISK_FLAG_NO_PARENTS, open_access_mask=0, open_params=ctypes.byref(open_params))
        params = vdisk_struct.SET_VIRTUAL_DISK_INFO()
