...
class SgGridUser(object):
    ...
    def __init__(self):
        ...
        self.module = AnsibleModule(argument_spec=self.argument_spec, required_if=[('state', 'present', ['full_name', 'unique_name'])], supports_check_mode=True)
        ...
        if re_local_user.match(self.parameters['unique_name']) is None and re_fed_user.match(self.parameters['unique_name']) is None:
    ...
    def apply(self):
        ...
        if cd_action is None and self.parameters['state'] == 'present':
            ...
            if grid_user['memberOf'] is None:
                ...
                member_of_diff = [i for i in self.data['memberOf'] + grid_user['memberOf'] if i not in self.data['memberOf'] or i not in grid_user['memberOf']]
            if member_of_diff:
                update = True
        ...
        if self.pw_change:
            if self.module.check_mode:
                pass
            elif cd_action == 'create' or self.parameters['update_password'] == 'always':
