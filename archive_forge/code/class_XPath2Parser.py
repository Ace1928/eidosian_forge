...
class XPath2Parser(XPath1Parser):
    ...
    DEFAULT_NAMESPACES: ClassVar[Dict[str, str]] = {'xml': XML_NAMESPACE, 'xs': XSD_NAMESPACE, 'fn': XPATH_FUNCTIONS_NAMESPACE, 'err': XQT_ERRORS_NAMESPACE}
    PATH_STEP_LABELS = ('axis', 'function', 'kind test')
    PATH_STEP_SYMBOLS = {'(integer)', '(string)', '(float)', '(decimal)', '(name)', '*', '@', '..', '.', '(', '{'}
    RESERVED_FUNCTION_NAMES = {'attribute', 'comment', 'document-node', 'element', 'empty-sequence', 'if', 'item', 'node', 'processing-instruction', 'schema-attribute', 'schema-element', 'text', 'typeswitch'}
    ...
    def __init__(self, namespaces: Optional[NamespacesType]=None, strict: bool=True, compatibility_mode: bool=False, default_collation: Optional[str]=None, default_namespace: Optional[str]=None, function_namespace: Optional[str]=None, xsd_version: Optional[str]=None, schema: Optional[AbstractSchemaProxy]=None, base_uri: Optional[str]=None, variable_types: Optional[Dict[str, str]]=None, document_types: Optional[Dict[str, str]]=None, collection_types: Optional[Dict[str, str]]=None, default_collection_type: str='node()*') -> None:
        ...
        if not is_sequence_type(default_collection_type, self):
            raise ElementPathValueError('invalid sequence type for default_collection_type argument')
    ...
    @classmethod
    def constructor(cls, symbol: str, bp: int=90, nargs: NargsType=1, sequence_types: Union[Tuple[()], Tuple[str, ...], List[str]]=(), label: Union[str, Tuple[str, ...]]='constructor function') -> Callable[[Callable[..., Any]], Callable[..., Any]]:
        ...
        def evaluate_(self: XPathConstructor, context: Optional[XPathContext]=None) -> Union[List[None], AtomicValueType]:
        ...
        token_class = cls.register(symbol, nargs=nargs, sequence_types=sequence_types, label=label, bases=(XPathConstructor,), lbp=bp, rbp=bp, nud=nud_, evaluate=evaluate_)
