...
class XpressDirect(DirectSolver):
    ...
    def _get_mip_results(self, results, soln):
        ...
        if status == xp.mip_not_loaded:
        ...
        elif status == xp.mip_lp_not_optimal or status == xp.mip_lp_optimal or status == xp.mip_no_sol_found:
            results.solver.status = SolverStatus.aborted
            results.solver.termination_message = 'Model is loaded, but no solution information is available.'
            results.solver.termination_condition = TerminationCondition.error
            soln.status = SolutionStatus.unknown
        elif status == xp.mip_solution:
            results.solver.status = SolverStatus.warning
            results.solver.termination_message = 'Unable to satisfy optimality tolerances; a sub-optimal solution is available.'
            ...
            results.solver.status = SolverStatus.warning
            results.solver.termination_message = 'LP relaxation was proven to be unbounded, but a solution is available.'
    ...
    def _set_instance(self, model, kwds={}):
        ...
        try:
            ...
            e = sys.exc_info()[1]
            msg = 'Unable to create Xpress model. Have you installed the Python bindings for Xpress?\n\n\t' + 'Error message: {0}'.format(e)
            raise Exception(msg)
    ...
    def _add_constraint(self, con):
        ...
        if con._linear_canonical_form:
            xpress_expr, referenced_vars = self._get_expr_from_pyomo_repn(con.canonical_form(), self._max_constraint_degree)
        else:
            xpress_expr, referenced_vars = self._get_expr_from_pyomo_expr(con.body, self._max_constraint_degree)
        if con.has_lb():
        ...
        if con.equality:
            xpress_con = xpress.constraint(body=xpress_expr, sense=xpress.eq, rhs=value(con.lower), name=conname)
        elif con.has_lb() and con.has_ub():
            xpress_con = xpress.constraint(body=xpress_expr, sense=xpress.rng, lb=value(con.lower), ub=value(con.upper), name=conname)
            self._range_constraints.add(xpress_con)
        elif con.has_lb():
            xpress_con = xpress.constraint(body=xpress_expr, sense=xpress.geq, rhs=value(con.lower), name=conname)
        elif con.has_ub():
            xpress_con = xpress.constraint(body=xpress_expr, sense=xpress.leq, rhs=value(con.upper), name=conname)
        else:
