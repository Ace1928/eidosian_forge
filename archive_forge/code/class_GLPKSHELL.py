...
@SolverFactory.register('_glpk_shell', doc='Shell interface to the GNU Linear Programming Kit')
class GLPKSHELL(SystemCallSolver):
    ...
    def __init__(self, **kwargs):
        ...
        self._valid_result_formats = {ProblemFormat.mod: ResultsFormat.soln, ProblemFormat.cpxlp: ResultsFormat.soln, ProblemFormat.mps: ResultsFormat.soln}
    ...
    def _default_executable(self):
        ...
        if not executable:
            msg = "Could not locate the 'glpsol' executable, which is required for solver '%s'"
    ...
    def _get_version(self, executable=None):
        ...
        result = subprocess.run([executable, '--version'], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, timeout=1, universal_newlines=True)
    ...
    def create_command_line(self, executable, problem_files):
        ...
        if not _ver or _ver < (4, 58):
            raise RuntimeError('Pyomo only supports versions of GLPK since 4.58; found version %s.  Please upgrade your installation of GLPK' % ('.'.join(map(str, _ver)),))
    ...
    def process_soln_file(self, results):
        ...
        try:
            ...
            pcols = int(pcols)
            pnonz = int(pnonz)
            if pprob != 'p' or ptype not in ('lp', 'mip') or psense not in ('max', 'min') or (prows < 0) or (pcols < 0) or (pnonz < 0):
                raise ValueError
