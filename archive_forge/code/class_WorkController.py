...
class WorkController:
    ...
    def setup_instance(self, queues=None, ready_callback=None, pidfile=None, include=None, use_eventloop=None, exclude_queues=None, **kwargs):
        ...
        self.blueprint = self.Blueprint(steps=self.app.steps['worker'], on_start=self.on_start, on_close=self.on_close, on_stopped=self.on_stopped)
    ...
    def should_use_eventloop(self):
        return detect_environment() == 'default' and self._conninfo.transport.implements.asynchronous and (not self.app.IS_WINDOWS)
    ...
    def _reload_modules(self, modules=None, **kwargs):
        return (self._maybe_reload_module(m, **kwargs) for m in set(self.app.loader.task_modules if modules is None else modules or ()))
    ...
    def info(self):
        ...
        return {'total': self.state.total_count, 'pid': os.getpid(), 'clock': str(self.app.clock), 'uptime': round(uptime.total_seconds())}
    ...
    def __repr__(self):
        """``repr(worker)``."""
        return '<Worker: {self.hostname} ({state})>'.format(self=self, state=self.blueprint.human_state() if self.blueprint else 'INIT')
