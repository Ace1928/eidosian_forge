...
def nnls(A, b, maxiter=None, *, atol=None):
    ...
    if len(A.shape) != 2:
        raise ValueError('Expected a two-dimensional array (matrix)' + f', but the shape of A is {A.shape}')
    if len(b.shape) != 1:
        raise ValueError('Expected a one-dimensional array (vector)' + f', but the shape of b is {b.shape}')
    m, n = A.shape
    if m != b.shape[0]:
