...
class GCSBackend(KeyValueStoreBackend):
    ...
    def __init__(self, **kwargs):
        ...
        if self.ttl < 0:
            ...
            if not self._is_bucket_lifecycle_rule_exists():
                raise ImproperlyConfigured(f'Missing lifecycle rule to use gcs backend with ttl on bucket: {self.bucket_name}')
    ...
    def client(self):
        ...
        with self._lock:
            ...
            self._pid = getpid()
            adapter = requests.adapters.HTTPAdapter(pool_connections=self._threadpool_maxsize, pool_maxsize=self._threadpool_maxsize, max_retries=3)
    ...
    def _params_from_url(self):
        ...
        return {'gcs_bucket': url_parts.hostname, 'gcs_base_path': url_parts.path, **url_parts.query}