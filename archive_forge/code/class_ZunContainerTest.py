...
class ZunContainerTest(common.HeatTestCase):
    ...
    def setUp(self):
        ...
        self.fake_restart_policy = {'MaximumRetryCount': '2', 'Name': 'on-failure'}
        ...
        self.fake_mounts = [{'volume_id': None, 'volume_size': 1, 'mount_path': '/data'}, {'volume_id': '6ec29ba3-bf2c-4276-a88e-3670ea5abc80', 'volume_size': None, 'mount_path': '/data2'}]
        self.fake_mounts_args = [{'size': 1, 'destination': '/data'}, {'source': '6ec29ba3-bf2c-4276-a88e-3670ea5abc80', 'destination': '/data2'}]
        self.fake_networks = [{'network': 'mynet', 'port': None, 'fixed_ip': '10.0.0.4'}, {'network': 'mynet2', 'port': None, 'fixed_ip': 'fe80::3'}, {'network': None, 'port': 'myport', 'fixed_ip': None}]
        self.fake_networks_args = [{'network': 'mynet', 'v4-fixed-ip': '10.0.0.4'}, {'network': 'mynet2', 'v6-fixed-ip': 'fe80::3'}, {'port': 'myport'}]
        self.fake_network_id = '9c11d847-99ce-4a83-82da-9827362a68e8'
        self.fake_network_name = 'private'
        self.fake_networks_attr = {'networks': [{'id': self.fake_network_id, 'name': self.fake_network_name}]}
        self.fake_address = {'version': 4, 'addr': '10.0.0.12', 'port': 'ab5c12d8-f414-48a3-b765-8ce34a6714d2'}
        self.fake_addresses = {self.fake_network_id: [self.fake_address]}
        self.fake_extended_addresses = {self.fake_network_id: [self.fake_address], self.fake_network_name: [self.fake_address]}
        t = template_format.parse(zun_template)
        self.stack = utils.parse_stack(t)
        resource_defns = self.stack.t.resource_definitions(self.stack)
        self.rsrc_defn = resource_defns[self.fake_name]
        self.client = mock.Mock()
        self.patchobject(container.Container, 'client', return_value=self.client)
        self.neutron_client = mock.Mock()
        self.patchobject(container.Container, 'neutron', return_value=self.neutron_client)
        self.stub_VolumeConstraint_validate()
        self.mock_update = self.patchobject(zun.ZunClientPlugin, 'update_container')
        self.stub_PortConstraint_validate()
        self.mock_find = self.patchobject(neutron.NeutronClientPlugin, 'find_resourceid_by_name_or_id', side_effect=lambda x, y: y)
        self.mock_attach = self.patchobject(zun.ZunClientPlugin, 'network_attach')
        self.mock_detach = self.patchobject(zun.ZunClientPlugin, 'network_detach')
        self.mock_attach_check = self.patchobject(zun.ZunClientPlugin, 'check_network_attach', return_value=True)
        self.mock_detach_check = self.patchobject(zun.ZunClientPlugin, 'check_network_detach', return_value=True)
    ...
    def test_create(self):
        ...
        self.assertEqual(self.fake_name, c.properties.get(container.Container.NAME))
        self.assertEqual(self.fake_image, c.properties.get(container.Container.IMAGE))
        self.assertEqual(self.fake_command, c.properties.get(container.Container.COMMAND))
        self.assertEqual(self.fake_cpu, c.properties.get(container.Container.CPU))
        self.assertEqual(self.fake_memory, c.properties.get(container.Container.MEMORY))
        self.assertEqual(self.fake_env, c.properties.get(container.Container.ENVIRONMENT))
        self.assertEqual(self.fake_workdir, c.properties.get(container.Container.WORKDIR))
        self.assertEqual(self.fake_labels, c.properties.get(container.Container.LABELS))
        self.assertEqual(self.fake_image_policy, c.properties.get(container.Container.IMAGE_PULL_POLICY))
        self.assertEqual('on-failure:2', c.properties.get(container.Container.RESTART_POLICY))
        self.assertEqual(self.fake_interactive, c.properties.get(container.Container.INTERACTIVE))
        self.assertEqual(self.fake_tty, c.properties.get(container.Container.TTY))
        self.assertEqual(self.fake_image_driver, c.properties.get(container.Container.IMAGE_DRIVER))
        self.assertEqual(self.fake_hints, c.properties.get(container.Container.HINTS))
        self.assertEqual(self.fake_hostname, c.properties.get(container.Container.HOSTNAME))
        self.assertEqual(self.fake_security_groups, c.properties.get(container.Container.SECURITY_GROUPS))
        self.assertEqual(self.fake_mounts, c.properties.get(container.Container.MOUNTS))
        self.assertEqual(self.fake_networks, c.properties.get(container.Container.NETWORKS))
        ...
        self.client.containers.run.assert_called_once_with(name=self.fake_name, image=self.fake_image, command=self.fake_command.split(), cpu=self.fake_cpu, memory=self.fake_memory, environment=self.fake_env, workdir=self.fake_workdir, labels=self.fake_labels, image_pull_policy=self.fake_image_policy, restart_policy=self.fake_restart_policy, interactive=self.fake_interactive, tty=self.fake_tty, image_driver=self.fake_image_driver, hints=self.fake_hints, hostname=self.fake_hostname, security_groups=self.fake_security_groups, mounts=self.fake_mounts_args, nets=self.fake_networks_args)
    ...
    def test_container_update(self):
        ...
        new_t['resources'][self.fake_name]['properties']['name'] = 'fake-container'
        ...
        self.mock_update.assert_called_once_with(self.resource_id, cpu=10, memory=10, name='fake-container')
    ...
    def test_container_update_networks_with_complex_parameters(self):
        new_networks = [{'network': 'mynet', 'fixed_ip': '10.0.0.4'}, {'port': '2a60cbaa-3d33-4af6-a9ce-83594ac546fc'}]
    ...
    def test_container_update_remove_network_non_empty(self):
        new_networks = [{'network': 'mynet', 'fixed_ip': '10.0.0.4'}, {'port': 'myport'}]
        self._test_container_update_networks(new_networks)
        self.assertEqual(1, self.mock_detach.call_count)
        self.assertEqual(0, self.mock_attach.call_count)
        self.assertEqual(1, self.mock_detach_check.call_count)
    ...
    def test_container_delete(self):
        c = self._create_resource('container', self.rsrc_defn, self.stack)
        scheduler.TaskRunner(c.create)()
        self.patchobject(self.client.containers, 'get', side_effect=[c, zc_exc.NotFound('Not Found')])
    ...
    def test_resolve_attributes(self):
        self.neutron_client.list_networks.return_value = self.fake_networks_attr
        c = self._create_resource('container', self.rsrc_defn, self.stack)
        scheduler.TaskRunner(c.create)()
        self._mock_get_client()
        self.assertEqual(self.fake_name, c._resolve_attribute(container.Container.NAME))
        self.assertEqual(self.fake_extended_addresses, c._resolve_attribute(container.Container.ADDRESSES))
