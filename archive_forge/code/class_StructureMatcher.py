...
class StructureMatcher(MSONable):
    ...
    def fit(self, struct1: Structure, struct2: Structure, symmetric: bool=False, skip_structure_reduction: bool=False) -> bool:
        """
        Fit two structures.
        ...
        if not self._subset and (not hash_match):
            return False
        if not symmetric:
            struct1, struct2, fu, s1_supercell = self._preprocess(struct1, struct2, skip_structure_reduction=skip_structure_reduction)
    ...
    def _match(self, struct1, struct2, fu, s1_supercell=True, use_rms=False, break_on_match=False) -> tuple[float, float, np.ndarray, float, Mapping] | None:
        """Matches one struct onto the other."""
        ratio = fu if s1_supercell else 1 / fu
        if len(struct1) * ratio >= len(struct2):
            return self._strict_match(struct1, struct2, fu, s1_supercell=s1_supercell, break_on_match=break_on_match, use_rms=use_rms)
        return self._strict_match(struct2, struct1, fu, s1_supercell=not s1_supercell, break_on_match=break_on_match, use_rms=use_rms)
    ...
    def as_dict(self):
        """MSONable dict"""
        return {'version': __version__, '@module': type(self).__module__, '@class': type(self).__name__, 'comparator': self._comparator.as_dict(), 'stol': self.stol, 'ltol': self.ltol, 'angle_tol': self.angle_tol, 'primitive_cell': self._primitive_cell, 'scale': self._scale, 'attempt_supercell': self._supercell, 'allow_subset': self._subset, 'supercell_size': self._supercell_size, 'ignored_species': self._ignored_species}
