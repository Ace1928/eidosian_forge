...
class Node:
  ...
  """
    ...
    def __init__(self, layer, call_args=None, call_kwargs=None, outputs=None):
        ...
        self._single_positional_tensor_passed = not self.call_kwargs and len(self.call_args) == 1 and tensor_util.is_tf_type(self.call_args[0])
        if not ops.executing_eagerly_outside_functions():
            for obj in self._flat_arguments:
                if isinstance(obj, tensor_lib.Tensor) and base_layer_utils.needs_keras_history(obj, ignore_call_context=True):
        ...
        for i, tensor in enumerate(nest.flatten(outputs)):
            tensor._keras_history = KerasHistory(layer=layer, node_index=node_index, tensor_index=i)
    ...
    def serialize(self, make_node_key, node_conversion_map):
        ...
        try:
            ...
            raise TypeError('Layer ' + self.layer.name + ' was passed non-JSON-serializable arguments. ' + 'Arguments had types: ' + str(kwarg_types) + '. They cannot be serialized out when saving the model.')
        ...
        if not nest.is_nested(data) and (not self.layer._preserve_input_structure_in_config):
            data = [data]
    ...
    def inbound_layers(self):
        if self.is_input:
            return []
        inbound_layers = nest.map_structure(lambda t: t._keras_history.layer, self.call_args[0])
