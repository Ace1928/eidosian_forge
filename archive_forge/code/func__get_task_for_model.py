...
def _get_task_for_model(model_name_or_path: str, default_task=None) -> str:
    ...
    try:
        ...
        if default_task:
            return default_task
        raise MlflowException('The task could not be inferred from the model. If you are saving a custom local model that is not available in the Hugging Face hub, please provide the `task` argument to the `log_model` or `save_model` function.', error_code=INVALID_PARAMETER_VALUE) from e