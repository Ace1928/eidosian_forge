...
@deprecated(since='0.0.25', removal='0.2.0', alternative_import='langchain_astradb.AstraDBChatMessageHistory')
class AstraDBChatMessageHistory(BaseChatMessageHistory):

    def __init__(self, *, session_id: str, collection_name: str=DEFAULT_COLLECTION_NAME, token: Optional[str]=None, api_endpoint: Optional[str]=None, astra_db_client: Optional[AstraDB]=None, async_astra_db_client: Optional[AsyncAstraDB]=None, namespace: Optional[str]=None, setup_mode: SetupMode=SetupMode.SYNC, pre_delete_collection: bool=False) -> None:
    ...
    def messages(self) -> List[BaseMessage]:
        ...
        message_blobs = [doc['body_blob'] for doc in sorted(self.collection.paginated_find(filter={'session_id': self.session_id}, projection={'timestamp': 1, 'body_blob': 1}), key=lambda _doc: _doc['timestamp'])]
    ...
    async def aget_messages(self) -> List[BaseMessage]:
        ...
        docs = self.async_collection.paginated_find(filter={'session_id': self.session_id}, projection={'timestamp': 1, 'body_blob': 1})
        sorted_docs = sorted([doc async for doc in docs], key=lambda _doc: _doc['timestamp'])
    ...
    def add_messages(self, messages: Sequence[BaseMessage]) -> None:
        self.astra_env.ensure_db_setup()
        docs = [{'timestamp': time.time(), 'session_id': self.session_id, 'body_blob': json.dumps(message_to_dict(message))} for message in messages]
