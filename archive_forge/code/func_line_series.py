...
import wandb
def line_series(xs: t.Union[t.Iterable, t.Iterable[t.Iterable]], ys: t.Iterable[t.Iterable], keys: t.Optional[t.Iterable]=None, title: t.Optional[str]=None, xname: t.Optional[str]=None, split_table: t.Optional[bool]=False):
    ...
    for y in ys:
        if not isinstance(y, Iterable):
            raise TypeError(f'Expected ys to be an array of arrays instead got {type(y)}')
    ...
    data = [[x, f'key_{i}' if keys is None else keys[i], y] for i, (xx, yy) in enumerate(zip(xs, ys)) for x, y in zip(xx, yy)]
    table = wandb.Table(data=data, columns=['step', 'lineKey', 'lineVal'])
    return wandb.plot_table('wandb/lineseries/v0', table, {'step': 'step', 'lineKey': 'lineKey', 'lineVal': 'lineVal'}, {'title': title, 'xname': xname or 'x'}, split_table=split_table)