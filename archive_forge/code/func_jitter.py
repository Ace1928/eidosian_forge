...
def jitter(labels, values, sigma=0.1, c=None, cmap=None, cmap_scale='linear', s=None, mask=None, plot_means=True, means_s=100, means_c='lightgrey', discrete=None, ax=None, legend=None, colorbar=None, shuffle=True, figsize=None, ticks=True, xticks=None, yticks=None, ticklabels=True, xticklabels=None, yticklabels=None, xlabel=None, ylabel=None, title=None, fontsize=None, legend_title=None, legend_loc='best', legend_anchor=None, vmin=None, vmax=None, filename=None, dpi=None, **plot_kwargs):
    ...
    **plot_kwargs : keyword arguments
    ...
    with temp_fontsize(fontsize):
        params = _JitterParams(labels, values, c=c, discrete=discrete, cmap=cmap, cmap_scale=cmap_scale, vmin=vmin, vmax=vmax, s=s, mask=mask, legend=legend, colorbar=colorbar, xlabel=xlabel, ylabel=ylabel)
        fig, ax, show_fig = _get_figure(ax, figsize, subplot_kw=params.subplot_kw)
        sc = ax.scatter(params.x_coords + np.random.normal(0, sigma, params.size)[params.plot_idx], params.y, c=params.c, cmap=params.cmap, norm=params.norm, s=params.s, vmin=params.vmin, vmax=params.vmax, **plot_kwargs)
        if plot_means:
            plot_x_means = [i for i in range(len(params.x_labels)) if np.any(params.x_coords == i)]
            ax.scatter(plot_x_means, [np.nanmean(params.y[params.x_coords == i]) for i in plot_x_means], c=means_c, edgecolors='black', lw=1.5, marker='o', zorder=3, s=means_s)
        ...
        label_axis(ax.yaxis, _with_default(yticks, ticks), _with_default(yticklabels, ticklabels), params.ylabel)
        ...
        if params.legend:
            if params.discrete:
                generate_legend({params.labels[i]: sc.cmap(sc.norm(i)) for i in range(len(params.labels))}, ax=ax, loc=legend_loc, bbox_to_anchor=legend_anchor, title=legend_title)
            else:
                generate_colorbar(params.cmap, ax=ax, vmin=params.vmin, vmax=params.vmax, title=legend_title, extend=params.extend, scale=sc.norm)
        if show_fig:
            show(fig)
