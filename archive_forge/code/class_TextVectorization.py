...
@keras_export('keras.layers.TextVectorization', 'keras.layers.experimental.preprocessing.TextVectorization', v1=[])
class TextVectorization(base_preprocessing_layer.PreprocessingLayer):
    ...
    def __init__(self, max_tokens=None, standardize='lower_and_strip_punctuation', split='whitespace', ngrams=None, output_mode='int', output_sequence_length=None, pad_to_max_tokens=False, vocabulary=None, idf_weights=None, sparse=False, ragged=False, encoding='utf-8', **kwargs):
        if 'dtype' in kwargs and kwargs['dtype'] != tf.string:
            raise ValueError(f'`TextVectorization` may only have a dtype of string. Received dtype: {kwargs['dtype']}.')
        elif 'dtype' not in kwargs:
            kwargs['dtype'] = tf.string
        layer_utils.validate_string_arg(standardize, allowable_strings=(LOWER_AND_STRIP_PUNCTUATION, LOWER, STRIP_PUNCTUATION), layer_name='TextVectorization', arg_name='standardize', allow_none=True, allow_callables=True)
        layer_utils.validate_string_arg(split, allowable_strings=(WHITESPACE, CHARACTER), layer_name='TextVectorization', arg_name='split', allow_none=True, allow_callables=True)
        if output_mode == 'binary':
            output_mode = MULTI_HOT
        if output_mode == 'tf-idf':
            output_mode = TF_IDF
        layer_utils.validate_string_arg(output_mode, allowable_strings=(INT, COUNT, MULTI_HOT, TF_IDF), layer_name='TextVectorization', arg_name='output_mode', allow_none=True)
        ...
        base_preprocessing_layer.keras_kpl_gauge.get_cell('TextVectorization').set(True)
    ...
    def compute_output_signature(self, input_spec):
        ...
        output_dtype = tf.int64 if self._output_mode == INT else backend.floatx()
    ...
    def from_config(cls, config):
        if config['standardize'] not in (LOWER_AND_STRIP_PUNCTUATION, LOWER, STRIP_PUNCTUATION):
            config['standardize'] = deserialize_keras_object(config['standardize'])
    ...
    def _preprocess(self, inputs):
        ...
        if self._split is not None:
            ...
            if self._split == WHITESPACE:
                ...
                raise ValueError("%s is not a supported splitting.TextVectorization supports the following options for `split`: None, 'whitespace', or a Callable." % self._split)
