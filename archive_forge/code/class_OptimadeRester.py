...
@due.dcite(Doi('10.1038/s41597-021-00974-z'), description='OPTIMADE, an API for exchanging materials data')
class OptimadeRester:
    ...
    def __init__(self, aliases_or_resource_urls: str | list[str] | None=None, refresh_aliases: bool=False, timeout: int=5):
        ...
        if not aliases_or_resource_urls:
            ...
            _logger.warning(f'Connecting to all known OPTIMADE providers, this will be slow. Please connect to only the OPTIMADE providers you want to query. Choose from: {', '.join(self.aliases)}')
    ...
    @staticmethod
    def _build_filter(elements: str | list[str] | None=None, nelements: int | None=None, nsites: int | None=None, chemical_formula_anonymous: str | None=None, chemical_formula_hill: str | None=None):
        ...
        if chemical_formula_hill:
            filters.append(f'(chemical_formula_hill={chemical_formula_hill!r})')
        return ' AND '.join(filters)

    def get_structures(self, elements: list[str] | str | None=None, nelements: int | None=None, nsites: int | None=None, chemical_formula_anonymous: str | None=None, chemical_formula_hill: str | None=None) -> dict[str, dict[str, Structure]]:
