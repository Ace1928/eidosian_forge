...
class StackResourcesServiceTest(common.HeatTestCase):
    ...
    @tools.stack_context('service_resource_describe_noncreated_test_stack', create_res=False)
    ...
    def test_stack_resources_describe(self, mock_load):
        mock_load.return_value = self.stack
        resources = self.eng.describe_stack_resources(self.ctx, self.stack.identifier(), 'WebServer')
    ...
    def test_stack_resources_describe_bad_lookup(self, mock_get):
        ...
        self.assertRaises(TypeError, self.eng.describe_stack_resources, self.ctx, None, 'WebServer')
    ...
    def test_find_physical_resource_nonexist(self):
        ex = self.assertRaises(dispatcher.ExpectedException, self.eng.find_physical_resource, self.ctx, 'foo')
        self.assertEqual(exception.EntityNotFound, ex.exc_info[0])
    ...
    def test_stack_resources_list_with_depth(self, mock_load):
        ...
        self.eng.list_stack_resources(self.ctx, self.stack.identifier(), 2)
        self.stack.iter_resources.assert_called_once_with(2, filters=None)
    ...
    def test_stack_resources_list_with_max_depth(self, mock_load):
        ...
        resources = self.stack.values()
        self.stack.iter_resources = mock.Mock(return_value=resources)
        self.eng.list_stack_resources(self.ctx, self.stack.identifier(), 99)
        max_depth = cfg.CONF.max_nested_stack_depth
        self.stack.iter_resources.assert_called_once_with(max_depth, filters=None)
    ...
    def test_stack_resources_list_nonexist_stack(self, mock_get):
        non_exist_identifier = identifier.HeatIdentifier(self.ctx.tenant_id, 'wibble', '18d06e2e-44d3-4bef-9fbf-52480d604b02')
        mock_get.side_effect = exception.EntityNotFound(entity='Stack', name='test')
        ex = self.assertRaises(dispatcher.ExpectedException, self.eng.list_stack_resources, self.ctx, non_exist_identifier)
        self.assertEqual(exception.EntityNotFound, ex.exc_info[0])
        mock_get.assert_called_once_with(self.ctx, non_exist_identifier, show_deleted=True)
    ...
    def test_signal_reception_async(self):
        ...
        test_data = {'food': 'yum'}
        self.eng.resource_signal(self.ctx, dict(self.stack.identifier()), 'WebServerScaleDownPolicy', test_data)
        self.assertEqual([(self.stack.id, mock.ANY)], self.eng.thread_group_mgr.started)
    ...
    def test_signal_reception_sync(self, mock_signal):
        ...
        self.stack = self._stack_create(stack_name)
        test_data = {'food': 'yum'}
        self.eng.resource_signal(self.ctx, dict(self.stack.identifier()), 'WebServerScaleDownPolicy', test_data, sync_call=True)
    ...
    def test_signal_reception_no_resource(self):
        ...
        self.stack = self._stack_create(stack_name)
        test_data = {'food': 'yum'}
        ex = self.assertRaises(dispatcher.ExpectedException, self.eng.resource_signal, self.ctx, dict(self.stack.identifier()), 'resource_does_not_exist', test_data)
        self.assertEqual(exception.ResourceNotFound, ex.exc_info[0])
    ...
    def test_signal_reception_unavailable_resource(self, mock_get, mock_load):
        ...
        mock_load.assert_called_once_with(self.ctx, stack=mock.ANY, use_stored_context=mock.ANY)
    ...
    def test_signal_returns_metadata(self, mock_signal):
        ...
        md = self.eng.resource_signal(self.ctx, dict(self.stack.identifier()), 'WebServerScaleDownPolicy', None, sync_call=True)
    ...
    def test_signal_unset_invalid_hook(self):
        ...
        ex = self.assertRaises(dispatcher.ExpectedException, self.eng.resource_signal, self.ctx, dict(self.stack.identifier()), 'WebServerScaleDownPolicy', details)
        msg = 'Invalid hook type "invalid_hook"'
        self.assertIn(msg, str(ex.exc_info[1]))
        self.assertEqual(exception.InvalidBreakPointHook, ex.exc_info[0])
    ...
    def test_signal_unset_not_defined_hook(self):
        ...
        ex = self.assertRaises(dispatcher.ExpectedException, self.eng.resource_signal, self.ctx, dict(self.stack.identifier()), 'WebServerScaleDownPolicy', details)
        msg = 'The "pre-update" hook is not defined on AWSScalingPolicy "WebServerScaleDownPolicy"'
        self.assertIn(msg, str(ex.exc_info[1]))
        self.assertEqual(exception.InvalidBreakPointHook, ex.exc_info[0])
    ...
    @mock.patch.object(res.Resource, 'signal')
    @mock.patch.object(service.EngineService, '_get_stack')
    def test_signal_no_calls_metadata_update(self, mock_get, mock_signal, mock_update):
        self.patchobject(keystone.KeystoneClientPlugin, '_create', return_value=fake_ks.FakeKeystoneClient())
        stk = tools.get_stack('signal_reception', self.ctx, policy_template)
        self.stack = stk
        stk.store()
    ...
    def test_find_resource_logical_name(self):
        ...
        physical_rsrc = self.eng._find_resource_in_stack(self.ctx, 'WebServer', self.stack)
    ...
    def test_find_resource_physical_id(self):
        rsrc = self.stack['WebServer']
        physical_rsrc = self.eng._find_resource_in_stack(self.ctx, rsrc.resource_id, self.stack)
        self.assertEqual(rsrc.id, physical_rsrc.id)
    ...
    def test_mark_healthy_in_create_complete(self):
        self.eng.resource_mark_unhealthy(self.ctx, self.stack.identifier(), 'WebServer', False, resource_status_reason='noop')
        self._test_mark_healthy_asserts(action='CREATE', status='COMPLETE')
    ...
    def test_mark_healthy_check_failed(self):
        ...
        reason = 'Good Reason'
        self.eng.resource_mark_unhealthy(self.ctx, self.stack.identifier(), 'WebServer', False, resource_status_reason=reason)
        self._test_mark_healthy_asserts(status='COMPLETE', reason=reason, meta=meta)
    ...
    def test_mark_unhealthy_check_failed(self):
        ...
        self.eng.resource_mark_unhealthy(self.ctx, self.stack.identifier(), 'WebServer', True, resource_status_reason=new_reason)
    ...
    def test_mark_unhealthy_none_reason(self):
        self.eng.resource_mark_unhealthy(self.ctx, self.stack.identifier(), 'WebServer', True)
    ...
    @tools.stack_context('service_mark_unhealthy_lockexc_converge_test_stk', convergence=True)
    def test_mark_unhealthy_stack_lock_exc_convergence(self):
        ...
        self.patchobject(res.Resource, '_store_with_lock', return_value=None, side_effect=exception.UpdateInProgress(self.stack.name))
