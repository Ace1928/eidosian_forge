...
class Community(Graph):
    ...
    def __init__(self, N=256, Nc=None, min_comm=None, min_deg=0, comm_sizes=None, size_ratio=1, world_density=None, comm_density=None, k_neigh=None, epsilon=None, seed=None, **kwargs):
        if Nc is None:
        ...
        info = {'node_com': None, 'comm_sizes': None, 'world_rad': None, 'world_density': world_density, 'min_comm': min_comm}
        if comm_sizes is None:
            mandatory_labels = np.tile(np.arange(Nc), (min_comm,))
            remaining_labels = rs.choice(Nc, N - min_comm * Nc)
        ...
        if comm_density is not None:
            ...
            comm_density = comm_density if 0.0 <= comm_density <= 1.0 else 0.1
        ...
        info['com_coords'] = info['world_rad'] * np.array(list(zip(np.cos(2 * np.pi * np.arange(1, Nc + 1) / Nc), np.sin(2 * np.pi * np.arange(1, Nc + 1) / Nc))))
        coords = rs.rand(N, 2)
        coords = np.array([[elem[0] * np.cos(2 * np.pi * elem[1]), elem[0] * np.sin(2 * np.pi * elem[1])] for elem in coords])
        for i in range(N):
