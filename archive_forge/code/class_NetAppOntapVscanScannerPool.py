...
class NetAppOntapVscanScannerPool(object):
    ...
    def get_svm_uuid(self):
        ...
        if error is not None:
            self.module.fail_json(msg='Error fetching vserver %s: %s' % (self.parameters['vserver'], to_native(error)), exception=traceback.format_exc())
        if record is None:
            self.module.fail_json(msg='Error fetching vserver %s. Please make sure vserver name is correct.' % self.parameters['vserver'], exception=traceback.format_exc())
