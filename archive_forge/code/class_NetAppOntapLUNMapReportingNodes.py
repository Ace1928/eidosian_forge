...
class NetAppOntapLUNMapReportingNodes:
    ...
    def __init__(self):
        ...
        self.argument_spec.update(dict(state=dict(required=False, type='str', choices=['present', 'absent'], default='present'), initiator_group_name=dict(required=True, type='str'), path=dict(required=True, type='str'), vserver=dict(required=True, type='str'), nodes=dict(required=True, type='list', elements='str')))
    ...
    def remove_lun_map_reporting_nodes(self, nodes):
        ...
        try:
            ...
            self.module.fail_json(msg='Error deleting LUN map reporting nodes for %s: %s' % (self.parameters['initiator_group_name'], to_native(error)), exception=traceback.format_exc())
    ...
    def add_lun_map_reporting_nodes_rest(self, node):
        ...
        if error:
            self.module.fail_json(msg='Error creating LUN map reporting node for %s: %s' % (self.parameters['initiator_group_name'], to_native(error)))
    ...
    def apply(self):
        ...
        if reporting_nodes is None:
            self.module.fail_json(msg='Error: LUN map not found for vserver %s, LUN path: %s, igroup: %s' % (self.parameters['vserver'], self.parameters['path'], self.parameters['initiator_group_name']))
        ...
        result = netapp_utils.generate_result(changed, cd_action, extra_responses={'reporting_nodes': reporting_nodes, 'nodes_to_add': nodes_to_add, 'nodes_to_delete': nodes_to_delete})
