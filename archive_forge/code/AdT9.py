...
from convokit import Corpus, TextParser, PolitenessStrategies
import pandas as pd
...
utterances_df = huggingface_corpus.get_utterances_dataframe()
conversations_df = huggingface_corpus.get_conversations_dataframe()
speakers_df = huggingface_corpus.get_speakers_dataframe()
...
utterances_df.to_csv(
    os.path.join(PROCESSED_DATA_DIR, "universal_utterances.csv"), index=False
)
conversations_df.to_csv(
    os.path.join(PROCESSED_DATA_DIR, "universal_conversations.csv"), index=False
)
speakers_df.to_csv(
    os.path.join(PROCESSED_DATA_DIR, "universal_speakers.csv"), index=False
)
