...
class WorkloadsClient(object):
    ...
    def __init__(self, release_track=base.ReleaseTrack.ALPHA):
        ...
        self._app_workloads_client = self.client.projects_locations_applications_workloads
        self._poller = waiter.CloudOperationPoller(self._app_workloads_client, self.client.projects_locations_operations)
    ...
    """
        describe_req = self.messages.ApphubProjectsLocationsApplicationsWorkloadsGetRequest(name=workload)
    ...
    """
        list_req = self.messages.ApphubProjectsLocationsApplicationsWorkloadsListRequest(parent=parent)
        return list_pager.YieldFromList(self._app_workloads_client, list_req, field='workloads', batch_size=page_size, limit=limit, batch_size_attribute='pageSize')
    ...
    """
        delete_req = self.messages.ApphubProjectsLocationsApplicationsWorkloadsDeleteRequest(name=workload)
        operation = self._app_workloads_client.Delete(delete_req)
        if async_flag:
            return operation
        delete_response = api_lib_utils.WaitForOperation(self._delete_poller, operation, api_lib_consts.DeleteApplicationWorkload.WAIT_FOR_DELETE_MESSAGE, api_lib_consts.DeleteApplicationWorkload.DELETE_TIMELIMIT_SEC)
        return delete_response

    def Create(self, workload_id, parent, async_flag, discovered_workload, display_name, description, attributes):
    ...
    """
        workload = self.messages.Workload(description=description, displayName=display_name, discoveredWorkload=discovered_workload, attributes=attributes)
        create_req = self.messages.ApphubProjectsLocationsApplicationsWorkloadsCreateRequest(workload=workload, workloadId=workload_id, parent=parent)
        operation = self._app_workloads_client.Create(create_req)
        if async_flag:
            return operation
        create_response = api_lib_utils.WaitForOperation(self._poller, operation, api_lib_consts.CreateApplicationWorkload.WAIT_FOR_CREATE_MESSAGE, api_lib_consts.CreateApplicationWorkload.CREATE_TIMELIMIT_SEC)
    ...
    def Update(self, workload_id, async_flag, attributes, args):
        ...
        if not update_mask:
            log.status.Print(api_lib_consts.UpdateApplicationWorkload.EMPTY_UPDATE_HELP_TEXT)
            return
        update_request = self.messages.ApphubProjectsLocationsApplicationsWorkloadsPatchRequest(name=workload_id, workload=workload, updateMask=update_mask)
        operation = self._app_workloads_client.Patch(update_request)
        if async_flag:
            return operation
        update_response = api_lib_utils.WaitForOperation(self._poller, operation, api_lib_consts.UpdateApplicationWorkload.WAIT_FOR_UPDATE_MESSAGE, api_lib_consts.UpdateApplicationWorkload.UPDATE_TIMELIMIT_SEC)
