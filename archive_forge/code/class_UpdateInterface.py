  ...
  *{command}* is used to update an interface on a Compute Engine
  ...
  """
    ...
    def _Args(cls, parser, enable_ipv6_bgp=False):
        ...
        cls.VPN_TUNNEL_ARG = vpn_tunnel_flags.VpnTunnelArgumentForRouter(required=False, operation_type='updated')
        cls.VPN_TUNNEL_ARG.AddArgument(link_parser)
        cls.INTERCONNECT_ATTACHMENT_ARG = attachment_flags.InterconnectAttachmentArgumentForRouter(required=False, operation_type='updated')
        cls.INTERCONNECT_ATTACHMENT_ARG.AddArgument(link_parser)
        router_flags.AddInterfaceArgs(parser, for_update=True, enable_ipv6_bgp=enable_ipv6_bgp)
    ...
    def GetGetRequest(self, client, router_ref):
        return (client.apitools_client.routers, 'Get', client.messages.ComputeRoutersGetRequest(router=router_ref.Name(), region=router_ref.region, project=router_ref.project))

    def GetSetRequest(self, client, router_ref, replacement):
        return (client.apitools_client.routers, 'Patch', client.messages.ComputeRoutersPatchRequest(router=router_ref.Name(), routerResource=replacement, region=router_ref.region, project=router_ref.project))
    ...
    def Modify(self, client, resources, args, existing, enable_ipv6_bgp=False):
        ...
        if enable_ipv6_bgp and args.ip_version is not None:
            iface.ipVersion = client.messages.RouterInterface.IpVersionValueValuesEnum(args.ip_version)
        if not args.vpn_tunnel_region:
            args.vpn_tunnel_region = replacement.region
        if args.vpn_tunnel is not None:
            vpn_ref = self.VPN_TUNNEL_ARG.ResolveAsResource(args, resources, scope_lister=compute_flags.GetDefaultScopeLister(client))
        ...
        if args.interconnect_attachment is not None:
            attachment_ref = self.INTERCONNECT_ATTACHMENT_ARG.ResolveAsResource(args, resources)
            iface.linkedInterconnectAttachment = attachment_ref.SelfLink()
        if iface.linkedVpnTunnel is not None and iface.linkedInterconnectAttachment is not None:
            raise parser_errors.ArgumentException('cannot have both vpn-tunnel and interconnect-attachment for the interface.')
        return replacement
    ...
    def _Run(self, args, enable_ipv6_bgp=False):
        ...
        new_object = self.Modify(client, holder.resources, args, objects[0], enable_ipv6_bgp=enable_ipv6_bgp)
        if objects[0] == new_object:
            log.status.Print('No change requested; skipping update for [{0}].'.format(objects[0].name))
            return objects
        return client.MakeRequests([self.GetSetRequest(client, router_ref, new_object)])
