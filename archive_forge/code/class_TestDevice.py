...
class TestDevice(BaseZMQTestCase):
    ...
    def test_proxy_bind_to_random_with_args(self):
        ...
        ports = []
        min, max = (5000, 5050)
        ports.extend([dev.bind_in_to_random_port(iface, min_port=min, max_port=max), dev.bind_out_to_random_port(iface, min_port=min, max_port=max), dev.bind_mon_to_random_port(iface, min_port=min, max_port=max)])
        for port in ports:
            if port < min or port > max:
                self.fail('Unexpected port number: %i' % port)