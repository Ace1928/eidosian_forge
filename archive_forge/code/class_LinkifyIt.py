...
class LinkifyIt:
    ...
    def __init__(self, schemas=None, options=None):
        self.default_options = {'fuzzy_link': True, 'fuzzy_email': True, 'fuzzy_ip': False}
        self.default_schemas = {'http:': {'validate': self._validate_http}, 'https:': 'http:', 'ftp:': 'http:', '//': {'validate': self._validate_double_slash}, 'mailto:': {'validate': self._validate_mailto}}
        ...
        if self._opts.get('fuzzy_email') and self._compiled.get('mailto:'):
            ...
            if at_pos >= 0:
                ...
                if me:
                    ...
                    if self._index < 0 or shift < self._index or (shift == self._index and next_shift > self._last_index):
                        self._schema = 'mailto:'
                        self._index = shift
