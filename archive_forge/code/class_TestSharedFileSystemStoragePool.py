...
class TestSharedFileSystemStoragePool(TestSharedFileSystemProxy):
    ...
    def test_storage_pool_detailed(self):
        self.verify_list(self.proxy.storage_pools, storage_pool.StoragePool, method_kwargs={'details': True, 'backend': 'alpha'}, expected_kwargs={'backend': 'alpha'})

    def test_storage_pool_not_detailed(self):
        self.verify_list(self.proxy.storage_pools, storage_pool.StoragePool, method_kwargs={'details': False, 'backend': 'alpha'}, expected_kwargs={'backend': 'alpha'})