...
class Itersine(Filter):
    ...
    def __init__(self, G, Nf=6, overlap=2.0, **kwargs):

        def k(x):
            return np.sin(0.5 * np.pi * np.power(np.cos(x * np.pi), 2)) * ((x >= -0.5) * (x <= 0.5))
        scale = G.lmax / (Nf - overlap + 1.0) * overlap
        g = []
        for i in range(1, Nf + 1):
            g.append(lambda x, ind=i: k(x / scale - (ind - overlap / 2.0) / overlap) / np.sqrt(overlap) * np.sqrt(2))
