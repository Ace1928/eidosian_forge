...
@deprecated(since='0.0.25', removal='0.2.0', alternative_import='langchain_mongodb.MongoDBAtlasVectorSearch')
class MongoDBAtlasVectorSearch(VectorStore):
    ...
    def __init__(self, collection: Collection[MongoDBDocumentType], embedding: Embeddings, *, index_name: str='default', text_key: str='text', embedding_key: str='embedding', relevance_score_fn: str='cosine'):
        """
        Args:
    ...
    def similarity_search_with_score(self, query: str, k: int=4, pre_filter: Optional[Dict]=None, post_filter_pipeline: Optional[List[Dict]]=None) -> List[Tuple[Document, float]]:
        ...
        docs = self._similarity_search_with_score(embedding, k=k, pre_filter=pre_filter, post_filter_pipeline=post_filter_pipeline)
        return docs
    ...
    def similarity_search(self, query: str, k: int=4, pre_filter: Optional[Dict]=None, post_filter_pipeline: Optional[List[Dict]]=None, **kwargs: Any) -> List[Document]:
        ...
        docs_and_scores = self.similarity_search_with_score(query, k=k, pre_filter=pre_filter, post_filter_pipeline=post_filter_pipeline)
        if additional and 'similarity_score' in additional:
            for doc, score in docs_and_scores:
                doc.metadata['score'] = score
        return [doc for doc, _ in docs_and_scores]

    def max_marginal_relevance_search(self, query: str, k: int=4, fetch_k: int=20, lambda_mult: float=0.5, pre_filter: Optional[Dict]=None, post_filter_pipeline: Optional[List[Dict]]=None, **kwargs: Any) -> List[Document]:
        ...
        docs = self._similarity_search_with_score(query_embedding, k=fetch_k, pre_filter=pre_filter, post_filter_pipeline=post_filter_pipeline)
        mmr_doc_indexes = maximal_marginal_relevance(np.array(query_embedding), [doc.metadata[self._embedding_key] for doc, _ in docs], k=k, lambda_mult=lambda_mult)
