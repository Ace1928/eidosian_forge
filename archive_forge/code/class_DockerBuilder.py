...
class DockerBuilder(AbstractBuilder):
    ...
    def from_config(cls, config: Dict[str, Any], environment: AbstractEnvironment, registry: AbstractRegistry) -> 'DockerBuilder':
        ...
        if image_uri:
            if registry is not None:
                wandb.termwarn(f'{LOG_PREFIX}Overriding registry from registry config with {image_uri} from builder config.')
    ...
    async def build_image(self, launch_project: LaunchProject, entrypoint: EntryPoint, job_tracker: Optional[JobAndRunStatusTracker]=None) -> str:
        ...
        await self.verify()
        await self.login()
        dockerfile_str = generate_dockerfile(launch_project=launch_project, entry_point=entrypoint, runner_type=launch_project.resource, builder_type='docker', dockerfile=launch_project.override_dockerfile)
