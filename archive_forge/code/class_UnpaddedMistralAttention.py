...
class UnpaddedMistralAttention(nn.Module):
    ...
    def forward(self, cos_sin: Tuple[torch.Tensor, torch.Tensor], nz_hidden_states: torch.Tensor, nz_position_ids: torch.LongTensor, cu_seqlens: torch.Tensor, max_seqlen: int) -> torch.Tensor:
        ...
        attn_output = flash_attn_varlen_func(q=query_states, k=key_states, v=value_states, cu_seqlens_q=cu_seqlens, cu_seqlens_k=cu_seqlens, max_seqlen_q=max_seqlen, max_seqlen_k=max_seqlen, dropout_p=0.0, causal=True, window_size=(self.sliding_window, self.sliding_window))
