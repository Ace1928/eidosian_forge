...
def block_lstmv2(seq_len_max: _atypes.TensorFuzzingAnnotation[_atypes.Int64], x: _atypes.TensorFuzzingAnnotation[TV_BlockLSTMV2_T], cs_prev: _atypes.TensorFuzzingAnnotation[TV_BlockLSTMV2_T], h_prev: _atypes.TensorFuzzingAnnotation[TV_BlockLSTMV2_T], w: _atypes.TensorFuzzingAnnotation[TV_BlockLSTMV2_T], wci: _atypes.TensorFuzzingAnnotation[TV_BlockLSTMV2_T], wcf: _atypes.TensorFuzzingAnnotation[TV_BlockLSTMV2_T], wco: _atypes.TensorFuzzingAnnotation[TV_BlockLSTMV2_T], b: _atypes.TensorFuzzingAnnotation[TV_BlockLSTMV2_T], cell_clip: float=0, use_peephole: bool=False, name=None):
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'BlockLSTMV2', name, seq_len_max, x, cs_prev, h_prev, w, wci, wcf, wco, b, 'cell_clip', cell_clip, 'use_peephole', use_peephole)
            ...
            pass
        try:
            return block_lstmv2_eager_fallback(seq_len_max, x, cs_prev, h_prev, w, wci, wcf, wco, b, cell_clip=cell_clip, use_peephole=use_peephole, name=name, ctx=_ctx)
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('BlockLSTMV2', seq_len_max=seq_len_max, x=x, cs_prev=cs_prev, h_prev=h_prev, w=w, wci=wci, wcf=wcf, wco=wco, b=b, cell_clip=cell_clip, use_peephole=use_peephole, name=name)
    _result = _outputs[:]
    if _execute.must_record_gradient():
        _attrs = ('cell_clip', _op.get_attr('cell_clip'), 'use_peephole', _op._get_attr_bool('use_peephole'), 'T', _op._get_attr_type('T'))
