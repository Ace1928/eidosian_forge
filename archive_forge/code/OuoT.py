...
def create_grid(size: int) -> List[List[int]]:
    ...
    logging.debug(
        f"Grid of size {size}x{size} created with all elements initialized to zero."
    )
    return grid
        ...
        for j in range(size):
            ...
            for di, dj in directions:
                ...
                if 0 <= ni < size and 0 <= nj < size:
                    ...
                    connections.append(((ni, nj), weight))
                    logging.debug(
                        f"Edge added from {(i, j)} to {(ni, nj)} with weight {weight}."
                    )
            graph[(i, j)] = connections
            logging.debug(
                f"Node {(i, j)} connected to its neighbors with randomly assigned weights."
            )
...
def prim_mst(
    ...
    heapq.heapify(edges)
    mst = [start_vertex]
    logging.debug(f"Starting Prim's algorithm from vertex {start_vertex}.")
    ...
    while edges:
        ...
        if to not in visited:
            visited.add(to)
            mst.append(to)
            logging.debug(f"Vertex {to} added to MST with edge weight {weight}.")
            for neighbor, weight in graph[to]:
                if neighbor not in visited:
                    heapq.heappush(edges, (weight, to, neighbor))
                    logging.debug(
                        f"Edge from {to} to {neighbor} with weight {weight} added to priority queue."
                    )

    logging.info(f"MST completed with vertices: {mst}")
    return mst
