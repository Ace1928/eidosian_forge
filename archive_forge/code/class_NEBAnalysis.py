...
class NEBAnalysis(MSONable):
    ...
    def get_plot(self, normalize_rxn_coordinate: bool=True, label_barrier: bool=True) -> plt.Axes:
        ...
        if label_barrier:
            ...
            ax.annotate(f'{np.max(ys) - np.min(ys):.0f} meV', xy=(barrier[0] / 2, barrier[1] * 1.02), xytext=(barrier[0] / 2, barrier[1] * 1.02), horizontalalignment='center', fontsize=18)
        plt.tight_layout()
        return ax
    ...
    def as_dict(self):
        ...
        """
        return {'@module': type(self).__module__, '@class': type(self).__name__, 'r': jsanitize(self.r), 'energies': jsanitize(self.energies), 'forces': jsanitize(self.forces), 'structures': [s.as_dict() for s in self.structures]}