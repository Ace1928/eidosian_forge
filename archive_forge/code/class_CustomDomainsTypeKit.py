...
class CustomDomainsTypeKit(base.TypeKit):
    ...
    """
        ...
        if 'set-mapping' in parameters:
            ...
            svc_id = runapps_v1alpha1_messages.ResourceID(name=service, type='service')
            ...
            if domain_config is None:
                ...
                domain_config = runapps_v1alpha1_messages.Resource(id=runapps_v1alpha1_messages.ResourceID(type=DOMAIN_TYPE, name=domain_res_name), config=encoding.DictToMessage({'domain': domain}, runapps_v1alpha1_messages.Resource.ConfigValue))
            ...
            bindings = base.FindBindings(domain_config, types_utils.SERVICE_TYPE, service)
            if bindings:
                binding = bindings[0]
                cfg = encoding.MessageToDict(binding.config)
                cfg.setdefault('paths', []).append(path)
                binding.config = encoding.DictToMessage(cfg, runapps_v1alpha1_messages.Binding.ConfigValue)
            else:
                domain_config.bindings.append(runapps_v1alpha1_messages.Binding(targetRef=runapps_v1alpha1_messages.ResourceRef(id=svc_id), config=encoding.DictToMessage({'paths': [path]}, runapps_v1alpha1_messages.Binding.ConfigValue)))
        elif 'remove-mapping' in parameters:
            url = parameters.get('remove-mapping')
            if ':' in url:
                raise exceptions.ArgumentError('Service notion not allowed in remove-mapping')
            domain, path = self._ParseDomainPath(url)
            domain_config = self._FindDomainConfig(resource.subresources, domain)
            if domain_config is None:
                raise exceptions.ArgumentError('Domain "{}" does not exist'.format(domain))
            if path == '/*':
                if len(domain_config.bindings) > 1:
                    raise exceptions.ArgumentError(('Can not remove root route of domain "{}" ' + 'because there are other routes configured.').format(domain))
                else:
        ...
        if not resource.subresources:
            raise exceptions.ArgumentError('Can not remove the last domain. ' + 'Use "gcloud run integrations delete custom-domains" instead.')
        return services
    ...
    """
        raise exceptions.ArgumentError('--add-service is not supported in custom-domains integration')
    ...
    Raises:
      ...
      service is not supported in DomainRouting integration.
    """
        raise exceptions.ArgumentError('--remove-service is not supported in custom-domains integration')
