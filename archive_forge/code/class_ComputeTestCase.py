...
class ComputeTestCase(base.TestCase):
    ...
    def get_image(cls):
        ...
        server_image = None
        for image in images:
            if image['Name'].startswith('cirros-') and (image['Name'].endswith('-uec') or image['Name'].endswith('-disk')):
    ...
    def get_network(cls):
        try:
            cmd_output = cls.openstack('network show private', parse_output=True)
    ...
    def wait_for_status(self, name, expected_status='ACTIVE', wait=900, interval=10):
        ...
        while total_sleep < wait:
            ...
            if status == expected_status:
                ...
                break
            print('Server {}: Waiting for {}, current status: {}'.format(name, expected_status, status))
