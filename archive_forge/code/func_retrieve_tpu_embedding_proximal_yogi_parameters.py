...
def retrieve_tpu_embedding_proximal_yogi_parameters(num_shards: int, shard_id: int, table_id: int=-1, table_name: str='', config: str='', name=None):
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'RetrieveTPUEmbeddingProximalYogiParameters', name, 'table_id', table_id, 'table_name', table_name, 'num_shards', num_shards, 'shard_id', shard_id, 'config', config)
            ...
            pass
        try:
            return retrieve_tpu_embedding_proximal_yogi_parameters_eager_fallback(table_id=table_id, table_name=table_name, num_shards=num_shards, shard_id=shard_id, config=config, name=name, ctx=_ctx)
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('RetrieveTPUEmbeddingProximalYogiParameters', num_shards=num_shards, shard_id=shard_id, table_id=table_id, table_name=table_name, config=config, name=name)
    _result = _outputs[:]
    if _execute.must_record_gradient():
        ...
        _execute.record_gradient('RetrieveTPUEmbeddingProximalYogiParameters', _inputs_flat, _attrs, _result)
    _result = _RetrieveTPUEmbeddingProximalYogiParametersOutput._make(_result)
    return _result