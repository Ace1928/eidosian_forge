...
class ArangoGraphQAChain(Chain):
    ...
    *Security note*: Make sure that the database connection uses credentials
    ...
    @classmethod
    def from_llm(cls, llm: BaseLanguageModel, *, qa_prompt: BasePromptTemplate=AQL_QA_PROMPT, aql_generation_prompt: BasePromptTemplate=AQL_GENERATION_PROMPT, aql_fix_prompt: BasePromptTemplate=AQL_FIX_PROMPT, **kwargs: Any) -> ArangoGraphQAChain:
        """Initialize from LLM."""
        ...
        return cls(qa_chain=qa_chain, aql_generation_chain=aql_generation_chain, aql_fix_chain=aql_fix_chain, **kwargs)
    ...
    def _call(self, inputs: Dict[str, Any], run_manager: Optional[CallbackManagerForChainRun]=None) -> Dict[str, Any]:
        ...
        aql_generation_output = self.aql_generation_chain.run({'adb_schema': self.graph.schema, 'aql_examples': self.aql_examples, 'user_input': user_input}, callbacks=callbacks)
        aql_query = ''
        aql_error = ''
        aql_result = None
        ...
        while aql_result is None and aql_generation_attempt < self.max_aql_generation_attempts + 1:
            ...
            try:
                ...
                aql_generation_output = self.aql_fix_chain.run({'adb_schema': self.graph.schema, 'aql_query': aql_query, 'aql_error': aql_error}, callbacks=callbacks)
            aql_generation_attempt += 1
        ...
        result = self.qa_chain({'adb_schema': self.graph.schema, 'user_input': user_input, 'aql_query': aql_query, 'aql_result': aql_result}, callbacks=callbacks)
