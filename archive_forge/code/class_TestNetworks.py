...
class TestNetworks(base.TestCase):
    ...
    def test_delete_network_exception(self):
        ...
        network_name = 'network'
        network = {'id': network_id, 'name': network_name}
        self.register_uris([dict(method='GET', uri=self.get_mock_url('network', 'public', append=['v2.0', 'networks', network_name]), status_code=404), dict(method='GET', uri=self.get_mock_url('network', 'public', append=['v2.0', 'networks'], qs_elements=['name=%s' % network_name]), json={'networks': [network]}), dict(method='DELETE', uri=self.get_mock_url('network', 'public', append=['v2.0', 'networks', network_id]), status_code=503)])
        self.assertRaises(exceptions.SDKException, self.cloud.delete_network, network_name)
        self.assert_calls()
