...
class HuggingFaceEndpoint(LLM):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        try:
            ...
            raise ImportError('Could not import huggingface_hub python package. Please install it with `pip install huggingface_hub`.')
        try:
        ...
        values['client'] = InferenceClient(model=values['model'], timeout=values['timeout'], token=huggingfacehub_api_token, **values['server_kwargs'])
        values['async_client'] = AsyncInferenceClient(model=values['model'], timeout=values['timeout'], token=huggingfacehub_api_token, **values['server_kwargs'])
        return values

    @property
    def _default_params(self) -> Dict[str, Any]:
        """Get the default parameters for calling text generation inference API."""
        return {'max_new_tokens': self.max_new_tokens, 'top_k': self.top_k, 'top_p': self.top_p, 'typical_p': self.typical_p, 'temperature': self.temperature, 'repetition_penalty': self.repetition_penalty, 'return_full_text': self.return_full_text, 'truncate': self.truncate, 'stop_sequences': self.stop_sequences, 'seed': self.seed, 'do_sample': self.do_sample, 'watermark': self.watermark, **self.model_kwargs}

    @property
    def _identifying_params(self) -> Mapping[str, Any]:
        """Get the identifying parameters."""
        _model_kwargs = self.model_kwargs or {}
        return {**{'endpoint_url': self.endpoint_url, 'task': self.task}, **{'model_kwargs': _model_kwargs}}
    ...
    def _call(self, prompt: str, stop: Optional[List[str]]=None, run_manager: Optional[CallbackManagerForLLMRun]=None, **kwargs: Any) -> str:
        ...
        if self.streaming:
            ...
            response = self.client.post(json={'inputs': prompt, 'parameters': invocation_params}, stream=False, task=self.task)
    ...
    async def _acall(self, prompt: str, stop: Optional[List[str]]=None, run_manager: Optional[AsyncCallbackManagerForLLMRun]=None, **kwargs: Any) -> str:
        ...
        if self.streaming:
            ...
            invocation_params['stop'] = invocation_params['stop_sequences']
            response = await self.async_client.post(json={'inputs': prompt, 'parameters': invocation_params}, stream=False, task=self.task)
            response_text = json.loads(response.decode())[0]['generated_text']
            for stop_seq in invocation_params['stop_sequences']:
                if response_text[-len(stop_seq):] == stop_seq:
                    response_text = response_text[:-len(stop_seq)]
