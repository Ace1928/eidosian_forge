...
class NetworksCellularGatewayDhcp(object):
    ...
    def __init__(self, params, meraki):
        self.meraki = meraki
        self.new_object = dict(dhcpLeaseTime=params.get('dhcpLeaseTime'), dnsNameservers=params.get('dnsNameservers'), dnsCustomNameservers=params.get('dnsCustomNameservers'), network_id=params.get('networkId'))
    ...
    def update_all_params(self):
        ...
        if self.new_object.get('dnsNameservers') is not None or self.new_object.get('dns_nameservers') is not None:
            new_object_params['dnsNameservers'] = self.new_object.get('dnsNameservers') or self.new_object.get('dns_nameservers')
    ...
    def get_object_by_name(self, name):
        result = None
        try:
            items = self.meraki.exec_meraki(family='cellulargateway', function='getNetworkCellularGatewayDhcp', params=self.get_all_params(name=name))
    ...
    def requires_update(self, current_obj):
        ...
        obj_params = [('dhcpLeaseTime', 'dhcpLeaseTime'), ('dnsNameservers', 'dnsNameservers'), ('dnsCustomNameservers', 'dnsCustomNameservers'), ('networkId', 'networkId')]
    ...
    def update(self):
        ...
        name = self.new_object.get('name')
        result = None
        result = self.meraki.exec_meraki(family='cellulargateway', function='updateNetworkCellularGatewayDhcp', params=self.update_all_params(), op_modifies=True)
        return result