...
async def main() -> None:
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            try:
                if not os.path.exists("assets/sound_font/soundfont.json"):
                    await downloader.download_file(
                        Config.SOUNDFONT_JSON_URL, "assets/sound_font/soundfont.json"
                    )
                else:
                    async with aiofiles.open(
                        "assets/sound_font/soundfont.json", "rb"
                    ) as file:
            ...
            if soundfont_data is not None:
                for instrument_id, instrument_name in soundfont_data[
                    ...
                    os.makedirs(f"assets/sound_font/{instrument_name}", exist_ok=True)
                    instrument_json: bytes = b""
                    instrument_path = f"{Config.SOUNDFONT_PATH}/{instrument_name}"
                    try:
                        if not os.path.exists(
                            f"assets/sound_font/{instrument_name}/instrument.json"
                        ):
                            instrument_json_url = (
                    ...
                    if instrument_data is not None:
                        for velocity in instrument_data["velocities"]:
                            ...
                            for pitch in pitches:
                                ...
                                await downloader.download_file(
                                    file_url,
                                    f"assets/sound_font/{instrument_name}/{file_name}",
                                )
                                logging.info(
