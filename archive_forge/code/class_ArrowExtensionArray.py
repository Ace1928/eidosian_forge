...
class ArrowExtensionArray(OpsMixin, ExtensionArraySupportsAnyAll, ArrowStringArrayMixin, BaseStringArrayMethods):
    ...
    def __getitem__(self, item: PositionalIndexer):
        ...
        if isinstance(item, np.ndarray):
            if not len(item):
                if self._dtype.name == 'string' and self._dtype.storage in ('pyarrow', 'pyarrow_numpy'):
                    pa_dtype = pa.string()
                else:
    ...
    def _rank(self, *, axis: AxisInt=0, method: str='average', na_option: str='keep', ascending: bool=True, pct: bool=False):
        ...
        """
        return type(self)(self._rank_calc(axis=axis, method=method, na_option=na_option, ascending=ascending, pct=pct))
    ...
    def _str_find(self, sub: str, start: int=0, end: int | None=None):
        if start != 0 and end is not None:
            ...
            raise NotImplementedError(f'find not implemented with sub={sub!r}, start={start!r}, end={end!r}')
        return type(self)(result)
    ...
    @property
    def _dt_days(self):
        return type(self)(pa.array(self._to_timedeltaarray().days, from_pandas=True, type=pa.int32()))

    @property
    def _dt_hours(self):
        return type(self)(pa.array([td.components.hours if td is not NaT else None for td in self._to_timedeltaarray()], type=pa.int32()))

    @property
    def _dt_minutes(self):
        return type(self)(pa.array([td.components.minutes if td is not NaT else None for td in self._to_timedeltaarray()], type=pa.int32()))

    @property
    def _dt_seconds(self):
        return type(self)(pa.array(self._to_timedeltaarray().seconds, from_pandas=True, type=pa.int32()))

    @property
    def _dt_milliseconds(self):
        return type(self)(pa.array([td.components.milliseconds if td is not NaT else None for td in self._to_timedeltaarray()], type=pa.int32()))

    @property
    def _dt_microseconds(self):
        return type(self)(pa.array(self._to_timedeltaarray().microseconds, from_pandas=True, type=pa.int32()))

    @property
    def _dt_nanoseconds(self):
        return type(self)(pa.array(self._to_timedeltaarray().nanoseconds, from_pandas=True, type=pa.int32()))
