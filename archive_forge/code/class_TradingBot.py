...
class TradingBot:
    ...
    def fetch_data(self, symbol: str, timeframe: str='1d', limit: int=500, source: str='ccxt') -> pd.DataFrame:
        """Fetch historical price data from exchange or Yahoo Finance."""
        logging.info(f'Fetching data for {symbol} with timeframe {timeframe}, limit {limit}, from {source}.')
        if source == 'ccxt':
    ...
    async def async_fetch_data(self, symbol: str, timeframe: str='1d', limit: int=500, source: str='ccxt') -> pd.DataFrame:
        """Fetch historical price data from exchange asynchronously."""
        logging.info(f'Asynchronously fetching data for {symbol} with timeframe {timeframe}, limit {limit}, from {source}.')
        if source == 'ccxt':
            ...
            df = await asyncio.to_thread(yf.download, symbol, period=f'{limit}d', interval=timeframe)
            df.reset_index(inplace=True)
            df.rename(columns={'Date': 'timestamp', 'Open': 'open', 'High': 'high', 'Low': 'low', 'Close': 'close', 'Volume': 'volume'}, inplace=True)
        self.data = df
