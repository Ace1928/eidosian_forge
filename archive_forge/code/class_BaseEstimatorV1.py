...
class BaseEstimatorV1(BasePrimitive, Generic[T]):
    ...
    * parameter values (:math:`\\theta_k`): list of sets of values
    ...
    def run(self, circuits: Sequence[QuantumCircuit] | QuantumCircuit, observables: Sequence[BaseOperator | str] | BaseOperator | str, parameter_values: Sequence[Sequence[float]] | Sequence[float] | float | None=None, **run_options) -> T:
        ...
        """
        circuits, observables, parameter_values = validation._validate_estimator_args(circuits, observables, parameter_values)
        run_opts = copy(self.options)
        run_opts.update_options(**run_options)
        return self._run(circuits, observables, parameter_values, **run_opts.__dict__)

    @abstractmethod
    def _run(self, circuits: tuple[QuantumCircuit, ...], observables: tuple[SparsePauliOp, ...], parameter_values: tuple[tuple[float, ...], ...], **run_options) -> T:
