...
class SenderKeyMessage(CiphertextMessage):
    ...
    def __init__(self, keyId=None, iteration=None, ciphertext=None, signatureKey=None, serialized=None):
        assert bool(keyId is not None and iteration is not None and (ciphertext is not None) and (signatureKey is not None)) ^ bool(serialized), 'Either pass arguments or serialized data'
        if serialized:
            try:
                messageParts = ByteUtil.split(serialized, 1, len(serialized) - 1 - self.__class__.SIGNATURE_LENGTH, self.__class__.SIGNATURE_LENGTH)
                ...
                if senderKeyMessage.id is None or senderKeyMessage.iteration is None or senderKeyMessage.ciphertext is None:
