...
class DocumentDBVectorSearch(VectorStore):
    ...
    def __init__(self, collection: Collection[DocumentDBDocumentType], embedding: Embeddings, *, index_name: str='vectorSearchIndex', text_key: str='textContent', embedding_key: str='vectorContent'):
    ...
    @classmethod
    def from_connection_string(cls, connection_string: str, namespace: str, embedding: Embeddings, **kwargs: Any) -> DocumentDBVectorSearch:
    ...
    def create_index(self, dimensions: int=1536, similarity: DocumentDBSimilarityType=DocumentDBSimilarityType.COS, m: int=16, ef_construction: int=64) -> dict[str, Any]:
        ...
        create_index_commands = {'createIndexes': self._collection.name, 'indexes': [{'name': self._index_name, 'key': {self._embedding_key: 'vector'}, 'vectorOptions': {'type': 'hnsw', 'similarity': similarity, 'dimensions': dimensions, 'm': m, 'efConstruction': ef_construction}}]}
    ...
    @classmethod
    def from_texts(cls, texts: List[str], embedding: Embeddings, metadatas: Optional[List[dict]]=None, collection: Optional[Collection[DocumentDBDocumentType]]=None, **kwargs: Any) -> DocumentDBVectorSearch:
    ...
    def similarity_search(self, query: str, k: int=4, ef_search: int=40, **kwargs: Any) -> List[Document]:
        embeddings = self._embedding.embed_query(query)
        docs = self._similarity_search_without_score(embeddings=embeddings, k=k, ef_search=ef_search)
