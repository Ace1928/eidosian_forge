...
class CommonMarkParser(parsers.Parser):
    ...
    def add_section(self, section, level):
        parent_level = max((section_level for section_level in self._level_to_elem if level > section_level))
        parent = self._level_to_elem[parent_level]
        parent.append(section)
        self._level_to_elem[level] = section
        self._level_to_elem = dict(((section_level, section) for section_level, section in self._level_to_elem.items() if section_level <= level))
