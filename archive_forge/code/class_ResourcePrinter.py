...
class ResourcePrinter(object):
  ...
  """
    ...
    def __init__(self, out=None, name=None, projector=None, by_columns=False, ignore_default_transforms=False, process_record=None, non_empty_projection_required=False, printer=None, console_attr=None, retain_none_values=False):
        """Constructor.
    ...
    """
        if 'disable' in self.attributes:
            self._empty = False
            return
        if self._non_empty_projection_required and (not self.column_attributes or not self.column_attributes.Columns()):
            raise ProjectionRequiredError('Format [{0}] requires a non-empty projection. Use key parameters to specify a projection like so `{0}(foo, bar.baz)`'.format(self._name))
        try:
            if resources:
