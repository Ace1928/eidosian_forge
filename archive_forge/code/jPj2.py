...
class Particle:
    ...
    def calculate_force(self, other: "Particle") -> np.ndarray:
        ...
        force += displacement * repulsive_force_magnitude / distance
...
@njit
def apply_forces_njit(particle_data, all_particles_data, DT, width, height):
    """
    Apply forces to a particle using numba for JIT compilation.

    Args:
        particle_data (numpy.ndarray): Data for the current particle.
        all_particles_data (numpy.ndarray): Data for all particles.
        DT (float): Time step for the simulation.
        width (int): Width of the simulation area.
        height (int): Height of the simulation area.

    Returns:
        numpy.ndarray: Updated particle data.
    """
    # Extract particle properties
    id, x, y, vx, vy, mass, em_charge, em_radius, strong_coupling, strong_radius = (
        particle_data
    )

    # Placeholder for force calculation (simplified)
    force_x, force_y = (
        0,
        0,
    )  # This should be replaced with actual force computation logic

    # Update velocity and position based on the force
    vx += force_x / mass * DT
    vy += force_y / mass * DT
    x += vx * DT + 0.5 * force_x / mass * DT**2
    y += vy * DT + 0.5 * force_y / mass * DT**2
...
async def process_particles(particles):
    """
    Process particles asynchronously using a thread pool executor and numba for efficient computation.

    Args:
        particles (list): List of particles to process.
    """
    # Convert particles to a structured numpy array for efficient processing
    dtype = [
        ("id", "i4"),
        ("x", "f8"),
        ("y", "f8"),
    ...
    # Update original particle objects with new data
    for particle, new_data in zip(particles, results):
        particle.pos = np.array([new_data["x"], new_data["y"]])
        particle.vel = np.array([new_data["vx"], new_data["vy"]])
