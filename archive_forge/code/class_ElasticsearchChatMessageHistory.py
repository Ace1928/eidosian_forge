...
class ElasticsearchChatMessageHistory(BaseChatMessageHistory):
    ...
    def __init__(self, index: str, session_id: str, *, es_connection: Optional['Elasticsearch']=None, es_url: Optional[str]=None, es_cloud_id: Optional[str]=None, es_user: Optional[str]=None, es_api_key: Optional[str]=None, es_password: Optional[str]=None, esnsure_ascii: Optional[bool]=True):
        ...
        if es_connection is not None:
            ...
            self.client = ElasticsearchChatMessageHistory.connect_to_elasticsearch(es_url=es_url, username=es_user, password=es_password, cloud_id=es_cloud_id, api_key=es_api_key)
        else:
        ...
        if self.client.indices.exists(index=index):
            ...
            self.client.indices.create(index=index, mappings={'properties': {'session_id': {'type': 'keyword'}, 'created_at': {'type': 'date'}, 'history': {'type': 'text'}}})
    ...
    def messages(self) -> List[BaseMessage]:
        ...
        try:
            ...
            result = self.client.search(index=self.index, query={'term': {'session_id': self.session_id}}, sort='created_at:asc')
    ...
    def add_message(self, message: BaseMessage) -> None:
        ...
        try:
            from elasticsearch import ApiError
            self.client.index(index=self.index, document={'session_id': self.session_id, 'created_at': round(time() * 1000), 'history': json.dumps(message_to_dict(message), ensure_ascii=bool(self.ensure_ascii))}, refresh=True)
        except ApiError as err:
    ...
    def clear(self) -> None:
        ...
        try:
            from elasticsearch import ApiError
            self.client.delete_by_query(index=self.index, query={'term': {'session_id': self.session_id}}, refresh=True)
        except ApiError as err:
