...
class AudioEffector:
    ...
    def __init__(self, effect: Optional[str]=None, format: Optional[str]=None, *, encoder: Optional[str]=None, codec_config: Optional[CodecConfig]=None, pad_end: bool=True):
        if format is None:
    ...
    def _get_reader(self, waveform, sample_rate, output_sample_rate, frames_per_chunk=None):
        ...
        if frames_per_chunk is None:
            ...
            src = _AudioStreamingEncoder(waveform, sample_rate, self.effect, muxer, encoder, self.codec_config, frames_per_chunk)
    ...
    def stream(self, waveform: Tensor, sample_rate: int, frames_per_chunk: int, output_sample_rate: Optional[int]=None) -> Iterator[Tensor]:
