...
class StockDatabase:
    ...
    def __init__(self, db_path: str = DB_PATH):
        ...
        self.conn = sqlite3.Connection(DB_PATH)  # Create a connection object
        self.conn.close()  # Close the connection to the DB if it has been left open
    ...
    def initialize_database(self):
        """Create tables if they do not exist. After forcibly closing the connection to the DB, the tables will be created again if they dont exist."""
        self.conn.close()  # Close the connection to the DB if it has been left open
        tables = {
            "Market_Data": """
        ...
        for table_name, query in tables.items():
            try:
                self.execute_query(query)  # Execute the creation query
