class Alarm(object):
    ...
    def list(self, vitrage_id, all_tenants=False, limit=1000, sort_by=['start_timestamp', 'vitrage_id'], sort_dirs=['asc', 'asc'], filter_by=None, filter_vals=None, next_page=True, marker=None):
        ...
        """
        params = dict(vitrage_id=vitrage_id, all_tenants=all_tenants, limit=limit, sort_by=sort_by, sort_dirs=sort_dirs, filter_by=filter_by, filter_vals=filter_vals, next_page=next_page, marker=marker)
        return self.api.get(self.url, params=params).json()

    def history(self, all_tenants=False, start=None, end=None, limit=1000, sort_by=('start_timestamp', 'vitrage_id'), sort_dirs=('asc', 'asc'), filter_by=None, filter_vals=None, next_page=True, marker=None):
        ...
        marker=None if the request is for the first page.
        """
        params = dict(all_tenants=all_tenants, start=start, end=end, limit=limit, sort_by=sort_by, sort_dirs=sort_dirs, filter_by=filter_by, filter_vals=filter_vals, next_page=next_page, marker=marker)
