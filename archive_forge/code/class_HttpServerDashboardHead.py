...
class HttpServerDashboardHead:
    ...
    async def path_clean_middleware(self, request, handler):
        if request.path.startswith('/static') or request.path.startswith('/logs'):
            parent = pathlib.PurePosixPath('/logs' if request.path.startswith('/logs') else '/static')
            request_path = pathlib.PurePosixPath(pathlib.posixpath.realpath(request.path))
            if request_path != parent and parent not in request_path.parents:
                logger.info(f'Rejecting request_path={request_path!r} because it is not relative to parent={parent!r}')
                raise aiohttp.web.HTTPForbidden()
        return await handler(request)

    @aiohttp.web.middleware
    async def browsers_no_post_put_middleware(self, request, handler):
        if request.headers['User-Agent'].startswith('Mozilla') and request.method in [hdrs.METH_POST, hdrs.METH_PUT]:
            return aiohttp.web.Response(status=405, text='Method Not Allowed for browser traffic.')
        return await handler(request)
    ...
    async def run(self, modules):
        ...
        app = aiohttp.web.Application(client_max_size=100 * 1024 ** 2, middlewares=[self.metrics_middleware, self.path_clean_middleware, self.browsers_no_post_put_middleware])
