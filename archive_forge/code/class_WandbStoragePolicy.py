...
class WandbStoragePolicy(StoragePolicy):
    ...
    @classmethod
    def from_config(cls, config: Dict, api: Optional[InternalApi]=None) -> 'WandbStoragePolicy':
        return cls(config=config, api=api)

    def __init__(self, config: Optional[Dict]=None, cache: Optional[ArtifactFileCache]=None, api: Optional[InternalApi]=None) -> None:
        ...
        self._handler = MultiHandler(handlers=[s3, gcs, azure, http, https, artifact, local_artifact, file_handler], default_handler=TrackingHandler())
    ...
    def store_reference(self, artifact: 'Artifact', path: Union[URIStr, FilePathStr], name: Optional[str]=None, checksum: bool=True, max_objects: Optional[int]=None) -> Sequence['ArtifactManifestEntry']:
    ...
    async def store_file_async(self, artifact_id: str, artifact_manifest_id: str, entry: 'ArtifactManifestEntry', preparer: 'StepPrepare', progress_callback: Optional['progress.ProgressFn']=None) -> bool:
        """Async equivalent to `store_file_sync`."""
        resp = await preparer.prepare_async({'artifactID': artifact_id, 'artifactManifestID': artifact_manifest_id, 'name': entry.path, 'md5': entry.digest})
        entry.birth_artifact_id = resp.birth_artifact_id
        if resp.upload_url is None:
            return True
        if entry.local_path is None:
        ...
        with open(entry.local_path, 'rb') as file:
            await self._api.upload_file_retry_async(resp.upload_url, file, progress_callback, extra_headers={header.split(':', 1)[0]: header.split(':', 1)[1] for header in resp.upload_headers or {}})
        self._write_cache(entry)
        return False
    ...
    def _write_cache(self, entry: 'ArtifactManifestEntry') -> None:
        if entry.local_path is None:
            return
        _, hit, cache_open = self._cache.check_md5_obj_path(B64MD5(entry.digest), entry.size if entry.size is not None else 0)
        if not hit:
            try:
