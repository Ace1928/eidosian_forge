...
class DeleteOverride(base.UpdateCommand):
    ...
    def Run(self, args):
        ...
        response = client.DeleteOverride(security_profile.RelativeName(), overrides, update_mask, labels=labels_update.GetOrNone())
        if is_async:
            operation_id = response.name
            log.status.Print('Check for operation completion status using operation ID:', operation_id)
            return response
        return client.WaitForOperation(operation_ref=client.GetOperationsRef(response), message='Waiting for delete override from security-profile [{}] operation to complete.'.format(security_profile.RelativeName()), has_result=True)