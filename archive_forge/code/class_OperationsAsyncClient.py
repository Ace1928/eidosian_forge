...
class OperationsAsyncClient:
    ...
    def __init__(self, channel, client_config=None):
        ...
        default_retry = retries.AsyncRetry(initial=0.1, maximum=60.0, multiplier=1.3, predicate=retries.if_exception_type(core_exceptions.DeadlineExceeded, core_exceptions.ServiceUnavailable), timeout=600.0)
    ...
    async def get_operation(self, name, retry=gapic_v1.method_async.DEFAULT, timeout=gapic_v1.method_async.DEFAULT, compression=gapic_v1.method_async.DEFAULT, metadata=None):
        ...
        return await self._get_operation(request, retry=retry, timeout=timeout, compression=compression, metadata=metadata)

    async def list_operations(self, name, filter_, retry=gapic_v1.method_async.DEFAULT, timeout=gapic_v1.method_async.DEFAULT, compression=gapic_v1.method_async.DEFAULT, metadata=None):
        """
        ...
        metadata = metadata or []
        metadata.append(gapic_v1.routing_header.to_grpc_metadata({'name': name}))
        method = functools.partial(self._list_operations, retry=retry, timeout=timeout, compression=compression, metadata=metadata)
        iterator = page_iterator_async.AsyncGRPCIterator(client=None, method=method, request=request, items_field='operations', request_token_field='page_token', response_token_field='next_page_token')
        return iterator

    async def cancel_operation(self, name, retry=gapic_v1.method_async.DEFAULT, timeout=gapic_v1.method_async.DEFAULT, compression=gapic_v1.method_async.DEFAULT, metadata=None):
        ...
        metadata = metadata or []
        metadata.append(gapic_v1.routing_header.to_grpc_metadata({'name': name}))
        await self._cancel_operation(request, retry=retry, timeout=timeout, compression=compression, metadata=metadata)

    async def delete_operation(self, name, retry=gapic_v1.method_async.DEFAULT, timeout=gapic_v1.method_async.DEFAULT, compression=gapic_v1.method_async.DEFAULT, metadata=None):
        ...
        metadata = metadata or []
        metadata.append(gapic_v1.routing_header.to_grpc_metadata({'name': name}))
        await self._delete_operation(request, retry=retry, timeout=timeout, compression=compression, metadata=metadata)