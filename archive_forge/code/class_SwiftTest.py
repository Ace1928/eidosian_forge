...
class SwiftTest(common.HeatTestCase):
    ...
    def _create_container(self, stack, definition_name='SwiftContainer'):
        ...
        container = swift_c.SwiftContainer('test_resource', resource_defns[definition_name], stack)
        runner = scheduler.TaskRunner(container.create)
        runner()
        self.assertEqual((container.CREATE, container.COMPLETE), container.state)
        return container
    ...
    def test_attributes(self, mock_put, mock_head):
        headers = {'content-length': '0', 'x-container-object-count': '82', 'accept-ranges': 'bytes', 'x-trans-id': 'tx08ea48ef2fa24e6da3d2f5c188fd938b', 'date': 'Wed, 23 Jan 2013 22:48:05 GMT', 'x-timestamp': '1358980499.84298', 'x-container-read': '.r:*', 'x-container-bytes-used': '17680980', 'content-type': 'text/plain; charset=utf-8'}
        ...
        self.assertRaises(exception.InvalidTemplateAttribute, container.FnGetAtt, 'Foo')
    ...
    def test_container_headers(self, mock_put):
        stack = utils.parse_stack(self.t)
        container_name = utils.PhysName(stack.name, 'test_resource')
        self._create_container(stack, definition_name='SwiftContainerWebsite')
        expected = {'X-Container-Meta-Web-Error': 'error.html', 'X-Container-Meta-Web-Index': 'index.html', 'X-Container-Read': '.r:*'}
        mock_put.assert_called_once_with(container_name, expected)
    ...
    def test_delete_not_found(self, mock_put, mock_get, mock_delete):
        stack = utils.parse_stack(self.t)
        container_name = utils.PhysName(stack.name, 'test_resource')
        mock_delete.side_effect = sc.ClientException('missing', http_status=404)
        mock_get.return_value = ({'name': container_name}, [])
        container = self._create_container(stack)
        runner = scheduler.TaskRunner(container.delete)
        runner()
    ...
    def test_check(self, mock_put, mock_get):
        ...
        runner()
        self.assertEqual((container.CHECK, container.COMPLETE), container.state)
    ...
    @mock.patch('swiftclient.client.Connection.head_container')
    @mock.patch('swiftclient.client.Connection.put_container')
    def test_parse_live_resource_data(self, mock_put, mock_container, mock_account):
        stack = utils.parse_stack(self.t)
        container = self._create_container(stack, definition_name='SwiftContainerWebsite')
        mock_container.return_value = {'x-container-read': '.r:*', 'x-container-meta-web-index': 'index.html', 'x-container-meta-web-error': 'error.html', 'x-container-meta-login': 'login.html'}
        mock_account.return_value = {}
        live_state = container.parse_live_resource_data(container.properties, container.get_live_resource_data())
