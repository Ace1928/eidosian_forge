...
class ArizeCallbackHandler(BaseCallbackHandler):
    ...
    def __init__(self, model_id: Optional[str]=None, model_version: Optional[str]=None, SPACE_KEY: Optional[str]=None, API_KEY: Optional[str]=None) -> None:
    ...
    def on_llm_end(self, response: LLMResult, **kwargs: Any) -> None:
        ...
        if response.llm_output and 'token_usage' in response.llm_output:
            ...
            self.prompt_tokens = self.total_tokens = self.completion_tokens = 0
        for generations in response.generations:
            for generation in generations:
                ...
                schema = Schema(timestamp_column_name='prediction_ts', tag_column_names=['prompt_token', 'completion_token', 'total_token'], prompt_column_names=prompt_columns, response_column_names=response_columns)
                response_from_arize = self.arize_client.log(dataframe=df, schema=schema, model_id=self.model_id, model_version=self.model_version, model_type=ModelTypes.GENERATIVE_LLM, environment=Environments.PRODUCTION)
