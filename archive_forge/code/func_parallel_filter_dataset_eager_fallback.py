...
def parallel_filter_dataset_eager_fallback(input_dataset: _atypes.TensorFuzzingAnnotation[_atypes.Variant], other_arguments, num_parallel_calls: _atypes.TensorFuzzingAnnotation[_atypes.Int64], predicate, output_types, output_shapes, deterministic: str, metadata: str, name, ctx) -> _atypes.TensorFuzzingAnnotation[_atypes.Variant]:
    ...
    _attrs = ('predicate', predicate, 'deterministic', deterministic, 'Targuments', _attr_Targuments, 'output_types', output_types, 'output_shapes', output_shapes, 'metadata', metadata)
    _result = _execute.execute(b'ParallelFilterDataset', 1, inputs=_inputs_flat, attrs=_attrs, ctx=ctx, name=name)
