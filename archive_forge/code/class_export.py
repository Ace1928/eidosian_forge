    ...
    **Note:** All export packages must start with the package name.
    ...
    def __init__(self, package, path):
        if isinstance(path, str):
            ...
            raise ValueError(f"Invalid type for `path` argument: Received '{path}' of type {type(path)}")
        for p in export_paths:
            if not p.startswith(package + '.'):
                raise ValueError(f"All `export_path` values should start with '{package}.'. Received: path={path}")
        self.package = package
        self.path = path

    def __call__(self, symbol):
        if hasattr(symbol, '_api_export_path') and symbol._api_export_symbol_id == id(symbol):
            raise ValueError(f"Symbol {symbol} is already exported as '{symbol._api_export_path}'. Cannot also export it to '{self.path}'.")
