...
class ClearMLCallbackHandler(BaseMetadataCallbackHandler, BaseCallbackHandler):
    ...
    def __init__(self, task_type: Optional[str]='inference', project_name: Optional[str]='langchain_callback_demo', tags: Optional[Sequence]=None, task_name: Optional[str]=None, visualize: bool=False, complexity_metrics: bool=False, stream_logs: bool=False) -> None:
        ...
        if clearml.Task.current_task():
            ...
            self.task = clearml.Task.init(task_type=self.task_type, project_name=self.project_name, tags=self.tags, task_name=self.task_name, output_uri=True)
        self.logger = self.task.get_logger()
        warning = 'The clearml callback is currently in beta and is subject to change based on updates to `langchain`. Please report any issues to https://github.com/allegroai/clearml/issues with the tag `langchain`.'
