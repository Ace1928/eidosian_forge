...
class Pathfinder:
    ...
    def heuristic(
        ...
        # If a secondary goal position is provided, calculate the weighted distance and add it to the heuristic value
        ...
        # If a tertiary goal position is provided, calculate the weighted distance and add it to the heuristic value
        ...
        # If a quaternary goal position is provided, calculate the weighted distance and add it to the heuristic value
        ...
        # Calculate the obstacle proximity penalty and add it to the heuristic value
        ...
        # Calculate the boundary proximity penalty and add it to the heuristic value
        ...
        # If body size adaptations are enabled, calculate the body position proximity penalty and add it to the heuristic value
        ...
        # If escape route availability is considered, evaluate the escape routes and add the score to the heuristic value
        ...
        # If enhancements are provided, apply each enhancement to the heuristic value
    ...
    def astar_search(
        self, start_position: Vector2, goal_position: Vector2
    ) -> List[Vector2]:
        """
        ...
        Args:
            start_position (Vector2): The starting position of the search.
            goal_position (Vector2): The target position to reach.

        Returns:
            List[Vector2]: The optimal path from start to goal as a list of positions, or an empty list if no path is found.
        """
        # Initialize the open set with the start position and its costs
        open_set: List[Tuple[float, float, Vector2]] = [(0, 0, start_position)]

        # Initialize dictionaries to store the path and cost information
        came_from: Dict[Vector2, Optional[Vector2]] = {start_position: None}
        g_cost: Dict[Vector2, float] = {start_position: 0}
        ...
        while open_set:
            ...
            if current_position == goal_position:
                ...
                return self.reconstruct_path(came_from, start_position, goal_position)
    ...
    def reconstruct_path(
        self,
        came_from: Dict[Vector2, Optional[Vector2]],
        ...
        Args:
            came_from (Dict[Vector2, Optional[Vector2]]): A dictionary mapping each position to its parent position in the optimal path.
            start_position (Vector2): The starting position of the path.
            goal_position (Vector2): The target position of the path.

        Returns:
            List[Vector2]: The reconstructed optimal path from start to goal as a list of positions.
        """
        # Initialize an empty path
        path: List[Vector2] = []

        # Start from the goal position
        current_position: Vector2 = goal_position
    ...
    def get_neighbors(self, position: Vector2) -> List[Vector2]:
        """
        ...
        Args:
            position (Vector2): The position for which to get the neighbors.

        Returns:
            List[Vector2]: A list of valid neighboring positions.
        """
        ...
        for direction in directions:
            ...
            new_position: Vector2 = position + direction

            # Check if the new position is within the grid boundaries and not an obstacle
            if self.is_within_boundaries(new_position) and not self.is_obstacle(
                new_position
            ):
