...
def fake_quant_with_min_max_vars_per_channel_gradient(gradients: _atypes.TensorFuzzingAnnotation[_atypes.Float32], inputs: _atypes.TensorFuzzingAnnotation[_atypes.Float32], min: _atypes.TensorFuzzingAnnotation[_atypes.Float32], max: _atypes.TensorFuzzingAnnotation[_atypes.Float32], num_bits: int=8, narrow_range: bool=False, name=None):
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'FakeQuantWithMinMaxVarsPerChannelGradient', name, gradients, inputs, min, max, 'num_bits', num_bits, 'narrow_range', narrow_range)
            ...
            pass
        try:
            _result = _dispatcher_for_fake_quant_with_min_max_vars_per_channel_gradient((gradients, inputs, min, max, num_bits, narrow_range, name), None)
            ...
            _result = _dispatch.dispatch(fake_quant_with_min_max_vars_per_channel_gradient, (), dict(gradients=gradients, inputs=inputs, min=min, max=max, num_bits=num_bits, narrow_range=narrow_range, name=name))
    ...
    try:
        _, _, _op, _outputs = _op_def_library._apply_op_helper('FakeQuantWithMinMaxVarsPerChannelGradient', gradients=gradients, inputs=inputs, min=min, max=max, num_bits=num_bits, narrow_range=narrow_range, name=name)
    except (TypeError, ValueError):
        _result = _dispatch.dispatch(fake_quant_with_min_max_vars_per_channel_gradient, (), dict(gradients=gradients, inputs=inputs, min=min, max=max, num_bits=num_bits, narrow_range=narrow_range, name=name))
        if _result is not _dispatch.OpDispatcher.NOT_SUPPORTED:
            return _result
