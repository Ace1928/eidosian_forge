...
class DownloadManager:
    ...
    def download(self, url_or_urls):
        ...
        with stack_multiprocessing_download_progress_bars():
            downloaded_path_or_paths = map_nested(download_func, url_or_urls, map_tuple=True, num_proc=download_config.num_proc, desc='Downloading data files')
    ...
    def extract(self, path_or_paths, num_proc='deprecated'):
        ...
        if num_proc != 'deprecated':
            warnings.warn("'num_proc' was deprecated in version 2.6.2 and will be removed in 3.0.0. Pass `DownloadConfig(num_proc=<num_proc>)` to the initializer instead.", FutureWarning)
        download_config = self.download_config.copy()
