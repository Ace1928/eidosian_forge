...
class DictInterface(Interface):
    ...
    def init(cls, eltype, data, kdims, vdims):
        ...
        if isinstance(data, list) and all((isinstance(d, dict) for d in data)) and (not all((c in d for d in data for c in dimensions))):
        ...
        elif isinstance(data, list) and isinstance(data[0], tuple) and (len(data[0]) == 2) and any((isinstance(v, tuple) for v in data[0])):
            dict_data = zip(*(util.wrap_tuple(k) + util.wrap_tuple(v) for k, v in data))
        ...
        elif isinstance(data, dict) and (not any((isinstance(v, np.ndarray) for v in data.values()))) and (not any((d in data or any((d in k for k in data if isinstance(k, tuple))) for d in dimensions))):
            dict_data = sorted(data.items())
            k, v = dict_data[0]
            if len(util.wrap_tuple(k)) != len(kdims) or len(util.wrap_tuple(v)) != len(vdims):
                raise ValueError('Dictionary data not understood, should contain a column per dimension or a mapping between key and value dimension values.')
            dict_data = zip(*(util.wrap_tuple(k) + util.wrap_tuple(v) for k, v in dict_data))
            data = {k: np.array(v) for k, v in zip(dimensions, dict_data)}
    ...
    def validate(cls, dataset, vdims=True):
        ...
        if not_found:
            raise DataError('Following columns specified as dimensions but not found in data: %s' % not_found, cls)
        lengths = [(dim, 1 if isscalar(dataset.data[dim]) else len(dataset.data[dim])) for dim in dimensions]
        if len({l for d, l in lengths if l > 1}) > 1:
            lengths = ', '.join(['%s: %d' % l for l in sorted(lengths)])
            raise DataError('Length of columns must be equal or scalar, columns have lengths: %s' % lengths, cls)
    ...
    def length(cls, dataset):
        lengths = [len(vals) for d, vals in dataset.data.items() if d in dataset.dimensions() and (not isscalar(vals))]
    ...
    def array(cls, dataset, dimensions):
        ...
        return np.column_stack([np.full(len(dataset), arr) if isscalar(arr) else arr for arr in arrays])
    ...
    def sort(cls, dataset, by=None, reverse=False):
        ...
        return dict([(d, v if isscalar(v) else v[sorting][::-1] if reverse else v[sorting]) for d, v in dataset.data.items()])
    ...
    def groupby(cls, dataset, dimensions, container_type, group_type, **kwargs):
        ...
        keys = (tuple((dataset.data[d.name] if isscalar(dataset.data[d.name]) else dataset.data[d.name][i] for d in dimensions)) for i in range(len(dataset)))
        grouped_data = []
        for unique_key in util.unique_iterator(keys):
            mask = cls.select_mask(dataset, dict(zip(dimensions, unique_key)))
            group_data = dict(((d.name, dataset.data[d.name] if isscalar(dataset.data[d.name]) else dataset.data[d.name][mask]) for d in kdims + vdims))
    ...
    def sample(cls, dataset, samples=None):
        ...
        for sample in samples:
            ...
            mask |= sample_mask
        return {k: col if isscalar(col) else np.array(col)[mask] for k, col in dataset.data.items()}
