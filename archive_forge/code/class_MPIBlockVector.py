...
class MPIBlockVector(np.ndarray, BaseBlockVector):
    ...
    def __init__(self, nblocks, rank_owner, mpi_comm, assert_correct_owners=False):
        if assert_correct_owners:
            assert self._assert_correct_owners(), 'rank_owner must be the same in all processors'
    ...
    def __array_ufunc__(self, ufunc, method, *inputs, **kwargs):
        ...
        unary_funcs = [np.log10, np.sin, np.cos, np.exp, np.ceil, np.floor, np.tan, np.arctan, np.arcsin, np.arccos, np.sinh, np.cosh, np.abs, np.tanh, np.arccosh, np.arcsinh, np.arctanh, np.fabs, np.sqrt, np.log, np.log2, np.absolute, np.isfinite, np.isinf, np.isnan, np.log1p, np.logical_not, np.expm1, np.exp2, np.sign, np.rint, np.square, np.positive, np.negative, np.rad2deg, np.deg2rad, np.conjugate, np.reciprocal, np.signbit]
        binary_funcs = [np.add, np.multiply, np.divide, np.subtract, np.greater, np.greater_equal, np.less, np.less_equal, np.not_equal, np.maximum, np.minimum, np.fmax, np.fmin, np.equal, np.logical_and, np.logical_or, np.logical_xor, np.logaddexp, np.logaddexp2, np.remainder, np.heaviside, np.hypot]
    ...
    def _unary_operation(self, ufunc, method, *args, **kwargs):
        ...
        if isinstance(x, MPIBlockVector):
            ...
            return super(MPIBlockVector, self).__array_ufunc__(ufunc, method, *args, **kwargs)
        else:
    ...
    def broadcast_block_sizes(self):
        ...
        for ndx in self.owned_blocks:
            if ndx in self._undefined_brows:
                raise NotFullyDefinedBlockVectorError('Block {ndx} is owned by rank {rank}, but the dimensions for block {ndx} have not yet been specified in rank {rank}. Please specify all owned blocks.'.format(ndx=ndx, rank=rank))
    ...
    def nonzero(self):
        ...
        """
        result = MPIBlockVector(nblocks=self.nblocks, rank_owner=self.rank_ownership, mpi_comm=self.mpi_comm, assert_correct_owners=False)
        assert_block_structure(self)
        for i in self._owned_blocks:
    ...
    def dot(self, other, out=None):
        ...
        if isinstance(other, MPIBlockVector):
            ...
            assert self.nblocks == other.nblocks, 'Number of blocks mismatch: {} != {}'.format(self.nblocks, other.nblocks)
    ...
    def _binary_operation_helper(self, other, operation):
        ...
        if isinstance(other, MPIBlockVector) or isinstance(other, BlockVector):
            assert self.nblocks == other.nblocks, 'Number of blocks mismatch: {} != {}'.format(self.nblocks, other.nblocks)
            if isinstance(other, MPIBlockVector):
                assert np.array_equal(self._rank_owner, other._rank_owner) or self._mpiw.Get_size() == 1, 'MPIBlockVectors must be distributed in same processors'
                assert self._mpiw == other._mpiw, 'Need to have same communicator'
            for i in self._owned_blocks:
    ...
    def set_block(self, key, value):
        owner = self._rank_owner[key]
        rank = self._mpiw.Get_rank()
        assert owner == rank or owner < 0, 'Block {} not owned by processor {}'.format(key, rank)
    ...
    def __setitem__(self, key, value):
        if not (self._has_equal_structure(key) and (self._has_equal_structure(value) or np.isscalar(value))):
            raise ValueError('MPIBlockVector.__setitem__ only accepts slices in the form of MPIBlockVectors of the same structure')
    ...
    def pprint(self, root=0):
        ...
        for bid in range(self.nblocks):
            ...
            repn = 'Owned by {} Shape({},){}'.format(disp_owner, self._brow_lengths[bid], is_none)
            msg += '{}: {}\n'.format(bid, repn)
