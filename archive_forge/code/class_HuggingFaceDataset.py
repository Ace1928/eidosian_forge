...
class HuggingFaceDataset(Dataset, PyFuncConvertibleDatasetMixin):
    ...
    def __init__(self, ds: 'datasets.Dataset', source: HuggingFaceDatasetSource, targets: Optional[str]=None, name: Optional[str]=None, digest: Optional[str]=None):
        """
        Args:
        ...
        if targets is not None and targets not in ds.column_names:
            raise MlflowException(f"The specified Hugging Face dataset does not contain the specified targets column '{targets}'.", INVALID_PARAMETER_VALUE)
        self._ds = ds
    ...
    def profile(self) -> Optional[Any]:
        ...
        """
        return {'num_rows': self._ds.num_rows, 'dataset_size': self._ds.dataset_size, 'size_in_bytes': self._ds.size_in_bytes}
    ...
    def to_pyfunc(self) -> PyFuncInputsOutputs:
        ...
        if self._targets is not None:
            if self._targets not in df.columns:
                raise MlflowException(f"Failed to convert Hugging Face dataset to pyfunc inputs and outputs because the pandas representation of the Hugging Face dataset does not contain the specified targets column '{self._targets}'.", INTERNAL_ERROR)
    ...
    def to_evaluation_dataset(self, path=None, feature_names=None) -> EvaluationDataset:
        ...
        """
        return EvaluationDataset(data=self._ds.to_pandas(), targets=self._targets, path=path, feature_names=feature_names)