...
class K8sAnsibleMixin(object):
    ...
    def __init__(self, module, pyyaml_required=True, *args, **kwargs):
        module.deprecate(msg='The K8sAnsibleMixin class has been deprecated and refactored into the module_utils/k8s/ directory.', version='4.0.0', collection_name='kubernetes.core')
        if not HAS_K8S_MODULE_HELPER:
            module.fail_json(msg=missing_required_lib('kubernetes'), exception=K8S_IMP_ERR, error=to_native(k8s_import_exception))
    ...
    def find_resource(self, kind, api_version, fail=False):
        msg = 'Failed to find exact match for {0}.{1} by [kind, name, singularName, shortNames]'.format(api_version, kind)
        try:
            if api_version == 'v1':
    ...
    def wait(self, resource, definition, sleep, timeout, state='present', condition=None, label_selectors=None):
        ...
        def _deployment_ready(deployment):
            return deployment.status and deployment.spec.replicas == (deployment.status.replicas or 0) and (deployment.status.availableReplicas == deployment.status.replicas) and (deployment.status.observedGeneration == deployment.metadata.generation) and (not deployment.status.unavailableReplicas)
        ...
        waiter = dict(StatefulSet=_statefulset_ready, Deployment=_deployment_ready, DaemonSet=_daemonset_ready, Pod=_pod_ready)
    ...
    def set_resource_definitions(self, module):
        ...
        if not resource_definition and (not src):
            implicit_definition = dict(kind=module.params['kind'], apiVersion=module.params['api_version'], metadata=dict(name=module.params['name']))
        ...
        if state == 'absent':
            ...
            if label_selectors:
                ...
                if not filter_selector.isMatching(definition):
                    ...
                    result['msg'] = "resource 'kind={kind},name={name},namespace={namespace}' filtered by label_selectors.".format(kind=definition['kind'], name=origin_name, namespace=namespace)
                    return result
            if apply:
                if self.check_mode and (not self.supports_dry_run):
            ...
            if not existing:
                if state == 'patched':
                    result['changed'] = False
                    result['warning'] = "resource 'kind={kind},name={name}' was not found but will not be created as 'state'                                        parameter has been set to '{state}'".format(kind=definition['kind'], name=origin_name, state=state)
                    return result
