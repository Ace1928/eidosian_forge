...
def ols(trendline_options, x_raw, x, y, x_label, y_label, non_missing):
    ...
    for k in trendline_options.keys():
        if k not in valid_options:
            raise ValueError("OLS trendline_options keys must be one of [%s] but got '%s'" % (', '.join(valid_options), k))
    ...
    if len(fit_results.params) == 2:
        hover_header += '%s = %g * %s + %g<br>' % (y_label, fit_results.params[1], x_label, fit_results.params[0])
