...
class ConnectedComponent(MSONable):
    ...
    def __init__(self, environments=None, links=None, environments_data=None, links_data=None, graph=None) -> None:
        """
        ...
        if graph is None:
            ...
            for edge in links:
                ...
                if not self._connected_subgraph.has_node(env_node1) or not self._connected_subgraph.has_node(env_node2):
                    raise ChemenvError(type(self).__name__, '__init__', 'Trying to add edge with some unexistent node ...')
    ...
    def as_dict(self):
        ...
        return {'@module': type(self).__module__, '@class': type(self).__name__, 'nodes': {strindex: (node.as_dict(), data) for strindex, (node, data) in nodes.items()}, 'graph': new_dict_of_dicts}
    ...
    def from_dict(cls, dct: dict) -> Self:
        ...
        """
        nodes_map = {inode_str: EnvironmentNode.from_dict(nodedict) for inode_str, (nodedict, nodedata) in dct['nodes'].items()}
        ...
        for e1, e1dict in dct['graph'].items():
            ...
            for e2, e2dict in e1dict.items():
                nested_dict[e1][e2] = {cls._edgedictkey_to_edgekey(ied): cls._retuplify_edgedata(edata) for ied, edata in e2dict.items()}
    ...
    def description(self, full=False):
        ...
        for en in sorted(self.graph.nodes()):
            ...
            for en_neighb in sorted(en_neighbs):
                ...
                all_deltas = sorted((get_delta(node1=en, node2=en_neighb, edge_data=edge_data).tolist() for iedge, edge_data in self.graph[en][en_neighb].items()))
