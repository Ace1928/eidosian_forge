...
def create_sql_query_chain(llm: BaseLanguageModel, db: SQLDatabase, prompt: Optional[BasePromptTemplate]=None, k: int=5) -> Runnable[Union[SQLInput, SQLInputWithTables, Dict[str, Any]], str]:
...
SQLQuery: " is passed here.
    ...
    if {'input', 'top_k', 'table_info'}.difference(prompt_to_use.input_variables):
        raise ValueError(f"Prompt must have input variables: 'input', 'top_k', 'table_info'. Received prompt with input variables: {prompt_to_use.input_variables}. Full prompt:\n\n{prompt_to_use}")
    if 'dialect' in prompt_to_use.input_variables:
        prompt_to_use = prompt_to_use.partial(dialect=db.dialect)
    inputs = {'input': lambda x: x['question'] + '\nSQLQuery: ', 'table_info': lambda x: db.get_table_info(table_names=x.get('table_names_to_use'))}
    return RunnablePassthrough.assign(**inputs) | (lambda x: {k: v for k, v in x.items() if k not in ('question', 'table_names_to_use')}) | prompt_to_use.partial(top_k=str(k)) | llm.bind(stop=['\nSQLResult:']) | StrOutputParser() | _strip