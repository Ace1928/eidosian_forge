...
@options.output_opt
@options.format_opt
@click.option('--like', type=click.Path(exists=True), help='Raster dataset to use as a template for obtaining affine transform (bounds and resolution), and crs.')
@click.option('--dst-crs', default=None, help='Target coordinate reference system.')
@options.dimensions_opt
@click.option('--src-bounds', nargs=4, type=float, default=None, help='Determine output extent from source bounds: left bottom right top . Cannot be used with destination --bounds')
@click.option('--bounds', '--dst-bounds', 'dst_bounds', nargs=4, type=float, default=None, help='Determine output extent from destination bounds: left bottom right top')
@options.resolution_opt
@click.option('--resampling', type=click.Choice([r.name for r in SUPPORTED_RESAMPLING]), default='nearest', help='Resampling method.', show_default=True)
@click.option('--src-nodata', default=None, show_default=True, type=float, help='Manually override source nodata')
@click.option('--dst-nodata', default=None, show_default=True, type=float, help='Manually override destination nodata')
@click.option('--threads', type=int, default=1, help='Number of processing threads.')
@click.option('--check-invert-proj/--no-check-invert-proj', default=True, help='Constrain output to valid coordinate region in dst-crs')
@click.option('--target-aligned-pixels/--no-target-aligned-pixels', default=False, help='align the output bounds based on the resolution')
@options.overwrite_opt
@options.creation_options
@click.option('--to', '--wo', '--transformer-option', '--warper-option', 'warper_options', metavar='NAME=VALUE', multiple=True, callback=_cb_key_val, help='GDAL warper and coordinate transformer options.')
@click.pass_context
def warp(ctx, files, output, driver, like, dst_crs, dimensions, src_bounds, dst_bounds, res, resampling, src_nodata, dst_nodata, threads, check_invert_proj, overwrite, creation_options, target_aligned_pixels, warper_options):
    """
    ...
    if target_aligned_pixels:
        if not res:
            raise click.BadParameter('--target-aligned-pixels requires a specified resolution')
    if like:
        invalid_combos = (dimensions, dst_bounds, dst_crs, res)
        if any((p for p in invalid_combos if p is not None)):
            raise click.BadParameter('--like cannot be used with any of --dimensions, --bounds, --dst-crs, or --res')
    elif dimensions:
        invalid_combos = (dst_bounds, res)
        if any((p for p in invalid_combos if p is not None)):
            raise click.BadParameter('--dimensions cannot be used with --bounds or --res')
    with ctx.obj['env']:
        ...
        with rasterio.open(files[0]) as src:
            ...
            if src_bounds and dst_bounds:
                raise click.BadParameter('--src-bounds and destination --bounds may not be specified simultaneously.')
            if like:
                ...
                try:
                    ...
                    raise click.BadParameter(str(err), param='dst_crs', param_hint='dst_crs')
                if dimensions:
                    ...
                    try:
                        ...
                        dst_transform, dst_width, dst_height = calcdt(src_crs, dst_crs, src.width, src.height, resolution=res, **kwargs, **warper_options)
                    except CRSError as err:
                        raise click.BadParameter(str(err), param='dst_crs', param_hint='dst_crs')
            elif dimensions:
                dst_crs = src.crs
            ...
            if dst_nodata is not None:
                if src_nodata is None and src.meta['nodata'] is None:
                    raise click.BadParameter('--src-nodata must be provided because dst-nodata is not None')
                else:
                    out_kwargs.update(nodata=dst_nodata)
            if dst_width < 0 or dst_height < 0 or dst_width > MAX_OUTPUT_WIDTH or (dst_height > MAX_OUTPUT_HEIGHT):
                raise click.BadParameter('Invalid output dimensions: {0}.'.format((dst_width, dst_height)))
            out_kwargs.update(crs=dst_crs, transform=dst_transform, width=dst_width, height=dst_height)
            ...
            if 'blockysize' in out_kwargs and dst_height < int(out_kwargs['blockysize']):
                ...
                logger.warning('Blockxsize removed from creation options to accomodate small output height')
