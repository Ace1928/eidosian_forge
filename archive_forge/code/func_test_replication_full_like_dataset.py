...
def test_replication_full_like_dataset(variant, dtype):
    ...
    variants = {'data': ((unit_registry.s, unit_registry.Pa), 1, 1), 'dims': ((1, 1), unit, 1), 'coords': ((1, 1), 1, unit)}
    ...
    expected = attach_units(xr.full_like(strip_units(ds), fill_value=strip_units(fill_value)), units)
