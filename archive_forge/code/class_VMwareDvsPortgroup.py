...
class VMwareDvsPortgroup(PyVmomi):
    ...
    def __init__(self, module):
        ...
        if self.port_allocation == 'elastic' and self.module.params['port_binding'] == 'ephemeral':
            self.module.fail_json(msg="'elastic' port allocation is not supported on an 'ephemeral' portgroup.")
    ...
    def build_config(self):
        ...
        if self.module.params['vlan_trunk']:
            ...
            if self.module.params['vlan_private']:
                ...
                if not pvlan_exists:
                    self.module.fail_json(msg='No private vlan with id %s in distributed vSwitch %s' % (self.module.params['vlan_id'], self.module.params['switch_name']))
    ...
    def process_state(self):
        dvspg_states = {'absent': {'update': self.state_destroy_dvspg, 'present': self.state_destroy_dvspg, 'absent': self.state_exit_unchanged}, 'present': {'update': self.state_update_dvspg, 'present': self.state_exit_unchanged, 'absent': self.state_create_dvspg}}
        try:
