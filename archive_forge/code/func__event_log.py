...
def _event_log(model, model_graph, constraint_set, type_of_graph, with_objective):
    ...
    if number_of_nodes > 0:
        ...
        if type_of_graph == 'bipartite':
            ...
            if constraint_density == variable_density == 1.0:
                logger.info('The bipartite graph constructed from the model is complete (graph density equals 1)')
            else:
                logger.info('For the bipartite graph constructed from the model, the density for constraint nodes is %s' % constraint_density)
                logger.info('For the bipartite graph constructed from the model, the density for variable nodes is %s' % variable_density)
        else:
