...
class FileUploadTask(copy_util.ObjectCopyTaskWithExitHandler):
    ...
    def __init__(self, source_resource, destination_resource, delete_source=False, is_composite_upload_eligible=False, posix_to_set=None, print_created_message=False, print_source_version=False, user_request_args=None, verbose=False):
        """Initializes task.
    ...
    """
        super(FileUploadTask, self).__init__(source_resource, destination_resource, posix_to_set=posix_to_set, print_created_message=print_created_message, print_source_version=print_source_version, user_request_args=user_request_args, verbose=verbose)
    ...
    def _handle_symlink_placeholder_transform(self, source_path, temporary_paths_to_clean_up):
    ...
    Returns:
      The path to the symlink placeholder if one was created. Otherwise, returns
        source_path.
    """
        should_create_symlink_placeholder = symlink_util.get_preserve_symlink_from_user_request(self._user_request_args) and self._source_resource.is_symlink
        if should_create_symlink_placeholder:
            symlink_path = symlink_util.get_symlink_placeholder_file(self._source_resource.storage_url.object_name)
    ...
    def execute(self, task_status_queue=None):
        ...
        if source_url.is_stream:
            ...
            symlink_transformed_path = self._handle_symlink_placeholder_transform(source_url.object_name, temporary_paths_to_clean_up)
            source_path = self._handle_gzip_transform(symlink_transformed_path, temporary_paths_to_clean_up)
            size = os.path.getsize(source_path)
        component_count = copy_component_util.get_component_count(size, properties.VALUES.storage.parallel_composite_upload_component_size.Get(), api_client.MAX_OBJECTS_PER_COMPOSE_CALL)
        should_perform_single_transfer = not self._is_composite_upload_eligible or not task_util.should_use_parallelism() or component_count <= 1
        if should_perform_single_transfer:
            self._perform_single_transfer(size, source_path, task_status_queue, temporary_paths_to_clean_up)
        else:
            return self._perform_composite_upload(api_client, component_count, size, source_path, task_status_queue, temporary_paths_to_clean_up)