    ...
    def test_with_context(self):
        ...
        class OutOfTea(errors.BzrError):
            ...
            def __init__(self, branch, urgency):
                self.branch = branch
                self.urgency = urgency
        remote.error_translators.register(b'OutOfTea', lambda err, find, path: OutOfTea(err.error_args[0].decode('utf-8'), find('branch')))
        transport = MemoryTransport()
        client = FakeClient(transport.base)
        client.add_expected_call(b'Branch.get_stacked_on_url', (b'quack/',), b'error', (b'NotStacked',))
        client.add_expected_call(b'Branch.last_revision_info', (b'quack/',), b'error', (b'OutOfTea', b'low'))
        transport.mkdir('quack')
        transport = transport.clone('quack')
        branch = self.make_remote_branch(transport, client)
