...
class NetCommandFactoryJson(NetCommandFactory):
    ...
    def make_thread_created_message(self, thread):
        msg = pydevd_schema.ThreadEvent(pydevd_schema.ThreadEventBody('started', get_thread_id(thread)))
    ...
    def make_custom_frame_created_message(self, frame_id, frame_description):
        ...
        msg = pydevd_schema.ThreadEvent(pydevd_schema.ThreadEventBody('started', frame_id))
        return NetCommand(CMD_THREAD_CREATE, 0, msg, is_json=True)
    ...
    def make_thread_killed_message(self, tid):
        ...
        msg = pydevd_schema.ThreadEvent(pydevd_schema.ThreadEventBody('exited', tid))
    ...
    def make_get_completions_message(self, seq, completions, qualifier, start):
        ...
        for completion in completions:
            ...
            if label.lower().startswith(qualifier):
                completion = pydevd_schema.CompletionItem(label=label, type=COMPLETION_TYPE_LOOK_UP[completion[3]], start=start, length=qualifier_len)
    ...
    def make_get_thread_stack_message(self, py_db, seq, thread_id, topmost_frame, fmt, must_be_suspended=False, start_frame=0, levels=0):
        ...
        try:
            ...
            for frame_id, frame, method_name, original_filename, filename_in_utf8, lineno, applied_mapping, show_as_current_frame, line_col_info in self._iter_visible_frames_info(py_db, frames_list, flatten_chained=True):
                try:
                    ...
                    try:
                        ...
                        if line_text:
                            ...
                            if line_col_info.lineno == line_col_info.end_lineno:
                                endcol = endcolno + 1
                frames.append(pydevd_schema.StackFrame(frame_id, formatted_name, lineno, column=column, endColumn=endcol, source={'path': filename_in_utf8, 'sourceReference': source_reference}, presentationHint=presentation_hint).to_dict())
        finally:
            topmost_frame = None
        ...
        response = pydevd_schema.StackTraceResponse(request_seq=seq, success=True, command='stackTrace', body=pydevd_schema.StackTraceResponseBody(stackFrames=stack_frames, totalFrames=total_frames))
        return NetCommand(CMD_RETURN, 0, response, is_json=True)
    ...
    def make_console_message(self, msg):
        ...
        event = OutputEvent(body)
        return NetCommand(CMD_WRITE_TO_CONSOLE, 0, event, is_json=True)
    _STEP_REASONS = set([CMD_STEP_INTO, CMD_STEP_INTO_MY_CODE, CMD_STEP_OVER, CMD_STEP_OVER_MY_CODE, CMD_STEP_RETURN, CMD_STEP_RETURN_MY_CODE, CMD_STEP_INTO_MY_CODE, CMD_STOP_ON_START, CMD_STEP_INTO_COROUTINE, CMD_SMART_STEP_INTO])
    _EXCEPTION_REASONS = set([CMD_STEP_CAUGHT_EXCEPTION, CMD_ADD_EXCEPTION_BREAK])
    ...
    def make_thread_suspend_single_notification(self, py_db, thread_id, thread, stop_reason):
        ...
        if stop_reason == 'exception':
            exception_info_response = build_exception_info_response(py_db, thread_id, thread, -1, set_additional_thread_info, self._iter_visible_frames_info, max_frames=-1)
        ...
        body = pydevd_schema.StoppedEventBody(reason=stop_reason, description=exc_desc, threadId=thread_id, text=exc_name, allThreadsStopped=True, preserveFocusHint=preserve_focus_hint)
    ...
    def make_set_next_stmnt_status_message(self, seq, is_success, exception_msg):
        response = pydevd_schema.GotoResponse(request_seq=int(seq), success=is_success, command='goto', body={}, message=None if is_success else exception_msg)
        return NetCommand(CMD_RETURN, 0, response, is_json=True)
