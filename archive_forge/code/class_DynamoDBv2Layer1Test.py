...
class DynamoDBv2Layer1Test(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.read_units = 5
        self.write_units = 5
        self.attributes = [{'AttributeName': self.hash_key_name, 'AttributeType': self.hash_key_type}, {'AttributeName': self.range_key_name, 'AttributeType': self.range_key_type}]
        self.schema = [{'AttributeName': self.hash_key_name, 'KeyType': 'HASH'}, {'AttributeName': self.range_key_name, 'KeyType': 'RANGE'}]
        self.provisioned_throughput = {'ReadCapacityUnits': self.read_units, 'WriteCapacityUnits': self.write_units}
        self.lsi = [{'IndexName': 'MostRecentIndex', 'KeySchema': [{'AttributeName': self.hash_key_name, 'KeyType': 'HASH'}, {'AttributeName': self.range_key_name, 'KeyType': 'RANGE'}], 'Projection': {'ProjectionType': 'KEYS_ONLY'}}]

    def create_table(self, table_name, attributes, schema, provisioned_throughput, lsi=None, wait=True):
        result = self.dynamodb.create_table(attributes, table_name, schema, provisioned_throughput, local_secondary_indexes=lsi)
    ...
    def test_recursive(self):
        ...
        description = self.dynamodb.describe_table(self.table_name)
        self.assertEqual(description['Table']['ItemCount'], 0)
        record_1_data = {'username': {'S': 'johndoe'}, 'first_name': {'S': 'John'}, 'last_name': {'S': 'Doe'}, 'date_joined': {'N': '1366056668'}, 'friend_count': {'N': '3'}, 'friend_data': {'M': {'username': {'S': 'alice'}, 'friend_count': {'N': '4'}}}}
        r1_result = self.dynamodb.put_item(self.table_name, record_1_data)
        record_1 = self.dynamodb.get_item(self.table_name, key={'username': {'S': 'johndoe'}, 'date_joined': {'N': '1366056668'}}, consistent_read=True)
        self.assertEqual(record_1['Item']['username']['S'], 'johndoe')
        self.assertEqual(record_1['Item']['first_name']['S'], 'John')
