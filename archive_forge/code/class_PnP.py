...
class PnP(DnacBase):
    ...
    def get_claim_params(self):
        ...
        configinfo = {'configId': self.have.get('template_id'), 'configParameters': [{'key': '', 'value': ''}]}
        ...
        claim_params = {'deviceId': self.have.get('device_id'), 'siteId': self.have.get('site_id'), 'type': self.want.get('pnp_type'), 'hostname': self.want.get('hostname'), 'imageInfo': imageinfo, 'configInfo': configinfo}
        if claim_params['type'] == 'CatalystWLC':
            ...
            if not self.validated_config[0].get('subnet_mask'):
                msg = 'Please provide a subnet mask to claim a wireless controller. This information is mandatory for the configuration.'
                self.pnp_cred_failure(msg=msg)
            ...
            if not self.validated_config[0].get('ip_interface_name'):
                msg = 'Please provide the Interface Name to claim a wireless controller. This information is necessary for making it a logical interface post claiming which can used to help manage the Wireless SSIDs broadcasted by the access points, manage the controller, access point and user data, plus more.'
                self.pnp_cred_failure(msg=msg)
            if not self.validated_config[0].get('vlan_id'):
                msg = 'Please provide the Vlan ID to claim a wireless controller. This is a required field for the process to create and set the specified port as trunk during PnP.'
                self.pnp_cred_failure(msg=msg)
    ...
    def get_have(self):
        ...
        if len(self.want.get('pnp_params')) == 1:
            device_response = self.dnac_apply['exec'](family='device_onboarding_pnp', function='get_device_list', params={'serial_number': self.want.get('serial_number')})
            self.log("Device details for the device with serial                 number '{0}': {1}".format(self.want.get('serial_number'), str(device_response)), 'DEBUG')
            ...
            if self.params.get('state') == 'merged':
                image_response = self.dnac_apply['exec'](family='software_image_management_swim', function='get_software_image_details', params=self.want.get('image_params'))
                ...
                template_list = self.dnac_apply['exec'](family='configuration_templates', function='gets_the_templates_available', params={'project_names': self.want.get('project_name')})
                self.log("List of templates under the project '{0}': {1}".format(self.want.get('project_name'), str(template_list)), 'DEBUG')
                dev_details_response = self.dnac_apply['exec'](family='device_onboarding_pnp', function='get_device_by_id', params={'id': device_response[0].get('id')})
                ...
                site_exists = False
                if not isinstance(self.want.get('site_name'), str) and (not self.want.get('pnp_params')[0].get('deviceInfo')):
                ...
                if site_exists:
                    ...
                    if self.want.get('pnp_type') == 'AccessPoint':
                        if self.get_site_type() != 'floor':
                            self.msg = "Please ensure that the site type is specified as 'floor' when claiming an AP. The site type is given as '{0}'. Please change the 'site_type' into 'floor' to proceed.".format(self.get_site_type())
                            self.log(str(self.msg), 'ERROR')
                            self.status = 'failed'
                            return self
                    if len(image_list) == 0:
                        self.msg = "The image '{0}' is either not present or not tagged as 'Golden' in the Cisco Catalyst Center. Please verify its existence and its tag status.".format(self.validated_config[0].get('image_name'))
                        self.log(self.msg, 'CRITICAL')
                        self.status = 'failed'
                        return self
                    if len(image_list) == 1:
                        if install_mode != 'INSTALL':
                            self.msg = "The system must be in INSTALL mode to upgrade the image. The current mode is '{0}'. Please switch to INSTALL mode to proceed.".format(install_mode)
                    ...
                    if template_name:
                        ...
                        if template_details:
                            ...
                            self.status = 'failed'
                            return self
                elif not self.want.get('pnp_params')[0].get('deviceInfo'):
                    self.msg = 'Either Site Name or Device details must be added.'
                    self.log(self.msg, 'ERROR')
                    self.status = 'failed'
                    return self
        self.msg = 'Successfully collected all project and template                     parameters from Cisco Catalyst Center for comparison'
    ...
    def get_want(self, config):
        ...
        """
        self.want = {'image_params': self.get_image_params(config), 'pnp_params': self.get_pnp_params(config), 'pnp_type': config.get('pnp_type'), 'site_name': config.get('site_name'), 'project_name': config.get('project_name'), 'template_name': config.get('template_name')}
        if len(self.want.get('pnp_params')) == 1:
            self.want['serial_number'] = self.want['pnp_params'][0]['deviceInfo'].get('serialNumber')
            self.want['hostname'] = self.want['pnp_params'][0]['deviceInfo'].get('hostname')
    ...
    def get_diff_merged(self):
        ...
        if len(self.want.get('pnp_params')) > 1:
            ...
            for device in self.want.get('pnp_params'):
                multi_device_response = self.dnac_apply['exec'](family='device_onboarding_pnp', function='get_device_list', params={'serial_number': device['deviceInfo']['serialNumber']})
                self.log("Device details for serial number {0}                         obtained from the API 'get_device_list': {1}".format(device['deviceInfo']['serialNumber'], str(multi_device_response)), 'DEBUG')
            ...
            if len(self.want.get('pnp_params')) - len(devices_added) == 0:
                ...
                return self
            bulk_list = [device for device in self.want.get('pnp_params') if device not in devices_added]
            bulk_params = self.dnac_apply['exec'](family='device_onboarding_pnp', function='import_devices_in_bulk', params={'payload': bulk_list}, op_modifies=True)
            ...
            if len(bulk_params.get('successList')) > 0:
                self.result['msg'] = '{0} device(s) imported successfully'.format(len(bulk_params.get('successList')))
            ...
            self.status = 'failed'
            return self
        provisioned_count_params = {'serial_number': self.want.get('serial_number'), 'state': 'Provisioned'}
        planned_count_params = {'serial_number': self.want.get('serial_number'), 'state': 'Planned'}
        if not self.have.get('device_found'):
            ...
            if not self.want['site_name']:
                ...
                dev_add_response = self.dnac_apply['exec'](family='device_onboarding_pnp', function='add_device', params=self.want.get('pnp_params')[0], op_modifies=True)
                ...
                claim_response = self.dnac_apply['exec'](family='device_onboarding_pnp', function='claim_a_device_to_a_site', op_modifies=True, params=claim_params)
                ...
                if claim_response.get('response') == 'Device Claimed' and self.have['deviceInfo']:
                    ...
                    self.status = 'failed'
                return self
        prov_dev_response = self.dnac_apply['exec'](family='device_onboarding_pnp', function='get_device_count', op_modifies=True, params=provisioned_count_params)
        self.log("Response from 'get device count' API for provisioned devices: {0}".format(str(prov_dev_response)), 'DEBUG')
        plan_dev_response = self.dnac_apply['exec'](family='device_onboarding_pnp', function='get_device_count', op_modifies=True, params=planned_count_params)
        self.log("Response from 'get_device_count' API for devices in planned state: {0}".format(str(plan_dev_response)), 'DEBUG')
        dev_details_response = self.dnac_apply['exec'](family='device_onboarding_pnp', function='get_device_by_id', params={'id': self.have['device_id']})
        ...
        update_response = self.dnac_apply['exec'](family='device_onboarding_pnp', function='update_device', params={'id': self.have['device_id'], 'payload': update_payload}, op_modifies=True)
        ...
        if pnp_state == 'Error':
            ...
            reset_response = self.dnac_apply['exec'](family='device_onboarding_pnp', function='reset_device', params={'payload': reset_paramters}, op_modifies=True)
            ...
            self.result['changed'] = True
            return self
        if not (prov_dev_response.get('response') == 0 and plan_dev_response.get('response') == 0 and (pnp_state == 'Unclaimed')):
            self.result['response'] = self.have.get('device_found')
    ...
    def get_diff_deleted(self):
        ...
        for device in devices_to_delete:
            ...
            if multi_device_response and len(multi_device_response) == 1:
                ...
                response = self.dnac_apply['exec'](family='device_onboarding_pnp', function='delete_device_by_id_from_pnp', op_modifies=True, params={'id': device_id})
    ...
    def verify_diff_merged(self, config):
        ...
        for device in self.want.get('pnp_params'):
            device_response = self.dnac_apply['exec'](family='device_onboarding_pnp', function='get_device_list', params={'serial_number': device['deviceInfo']['serialNumber']})
            if device_response and len(device_response) == 1:
                msg = 'Requested Device with Serial No. {0} is present in Cisco Catalyst Center and addition verified.'.format(device['deviceInfo']['serialNumber'])
                self.log(msg, 'INFO')
            else:
                msg = 'Requested Device with Serial No. {0} is not present in Cisco Catalyst CenterCenter'.format(device['deviceInfo']['serialNumber'])
    ...
    def verify_diff_deleted(self, config):
        ...
        for device in self.want.get('pnp_params'):
            device_response = self.dnac_apply['exec'](family='device_onboarding_pnp', function='get_device_list', params={'serial_number': device['deviceInfo']['serialNumber']})
            if not (device_response and len(device_response) == 1):
                msg = 'Requested Device with Serial No. {0} is not present in the Cisco DNACenter.'.format(device['deviceInfo']['serialNumber'])
                self.log(msg, 'INFO')
            else:
                msg = 'Requested Device with Serial No. {0} is present in Cisco Catalyst Center'.format(device['deviceInfo']['serialNumber'])
                self.log(msg, 'WARNING')
        self.status = 'success'
        return self