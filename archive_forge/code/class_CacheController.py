...
class CacheController(object):
    ...
    def _enforce(self, req, image=None, new_policy=None):
        ...
        try:
            api_policy.CacheImageAPIPolicy(req.context, image=image, enforcer=self.policy, policy_str=new_policy).manage_image_cache()
    ...
    def clear_cache(self, req):
        ...
        if target == '':
            res = dict(cache_deleted=self.cache.delete_all_cached_images(), queue_deleted=self.cache.delete_all_queued_images())
        elif target == 'cache':
            res = dict(cache_deleted=self.cache.delete_all_cached_images())
        elif target == 'queue':
            res = dict(queue_deleted=self.cache.delete_all_queued_images())
        else:
            reason = _("If provided 'x-image-cache-clear-target' must be 'cache', 'queue' or empty string.")
            raise webob.exc.HTTPBadRequest(explanation=reason, request=req, content_type='text/plain')
        return res
    ...
    def queue_image_from_api(self, req, image_id):
        ...
        if image.status != 'active':
            msg = _('Only images with status active can be targeted for queueing')
