...
class TestTrainingTool(unittest.TestCase):
    ...
    def test_training_info_proto(self) -> None:
        ...
        inference_node = helper.make_node('MatMul', inputs=[A_name, B_name], outputs=[C_name])
        inference_graph = helper.make_graph([inference_node], 'simple_inference', [A_value_info, B_value_info], [C_value_info], [A_initializer, B_initializer])
        X_shape = [2, 2]
        X_name = 'X'
        ...
        node = helper.make_node('MatMul', inputs=[X_name, C_name], outputs=[Y_name])
        training_graph = helper.make_graph([node], 'simple_training', [X_value_info], [Y_value_info], [X_initializer])
        training_info = helper.make_training_info(training_graph, [(B_name, Y_name)], None, None)
