...
class PortChainTest(common.HeatTestCase):
    ...
    def setUp(self):
        ...
        self.stack = stack.Stack(self.ctx, 'test_stack', template.Template(port_chain_template))
        ...
        self.patchobject(self.test_client_plugin, 'resolve_ext_resource').return_value = 'port_pair_group_1'
        self.patchobject(self.test_client_plugin, 'resolve_ext_resource').return_value = 'flow_classifier1'

    def test_resource_mapping(self):
        mapping = port_chain.resource_mapping()
        self.assertEqual(port_chain.PortChain, mapping['OS::Neutron::PortChain'])
    ...
    def test_resource_handle_create(self):
        ...
        self.assertEqual('test_port_chain', self.test_resource.properties.get(port_chain.PortChain.NAME))
        self.assertEqual('port_chain_desc', self.test_resource.properties.get(port_chain.PortChain.DESCRIPTION))
        self.assertEqual(['port_pair_group_1'], self.test_resource.properties.get(port_chain.PortChain.PORT_PAIR_GROUPS))
        self.assertEqual(['flow_classifier1'], self.test_resource.properties.get(port_chain.PortChain.FLOW_CLASSIFIERS))
        self.assertEqual({'correlation': 'mpls'}, self.test_resource.properties.get(port_chain.PortChain.CHAIN_PARAMETERS))
        self.test_resource.data_set = mock.Mock()
        self.test_resource.handle_create()
        mock_pc_create.assert_called_once_with('port_chain', {'name': 'test_port_chain', 'description': 'port_chain_desc', 'port_pair_groups': ['port_pair_group_1'], 'flow_classifiers': ['flow_classifier1'], 'chain_parameters': {'correlation': 'mpls'}})
    ...
    def test_resource_show_resource(self):
        ...
        self.assertIsNone(self.test_resource._show_resource(), 'Failed to show resource')

    def test_resource_handle_update(self):
        mock_ppg_patch = self.test_client_plugin.update_ext_resource
        self.test_resource.resource_id = '477e8273-60a7-4c41-b683-fdb0bc7cd151'
        prop_diff = {'name': 'name-updated', 'description': 'description-updated', 'port_pair_groups': ['port_pair_group_2'], 'flow_classifiers': ['flow_classifier2']}
        self.test_resource.handle_update(json_snippet=None, tmpl_diff=None, prop_diff=prop_diff)
        mock_ppg_patch.assert_called_once_with('port_chain', {'name': 'name-updated', 'description': 'description-updated', 'port_pair_groups': ['port_pair_group_2'], 'flow_classifiers': ['flow_classifier2']}, self.test_resource.resource_id)