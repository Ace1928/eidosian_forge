...
def AddReservationAffinityFlags(parser, for_node_pool=False):
    ...
    group_text = 'Specifies the reservation for the {}.'.format(target)
    group = parser.add_group(help=group_text)
    affinity_text = 'The type of the reservation for the {}.'.format(target)
    group.add_argument('--reservation-affinity', choices=['any', 'none', 'specific'], default=None, help=affinity_text)
    group.add_argument('--reservation', default=None, help='\nThe name of the reservation, required when `--reservation-affinity=specific`.\n')