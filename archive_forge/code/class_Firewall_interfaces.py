...
class Firewall_interfaces(ConfigBase):
    ...
    gather_network_resources = ['firewall_interfaces']
                            ...
                            for h in have_rules:
                                if key in h:
                                    ...
                                    if not w or key not in w or ('name' in h and w and ('name' not in w)):
                                        commands.append(self._compute_command(afi=h_rule['afi'], name=h_ar['name'], attrib=h[key], opr=False))
        commands.extend(self._state_merged(want, have))
        return commands
    ...
    def _delete_rules(self, name, want, have, opr=False):
        ...
        for w in w_rules:
            ...
            if key in w and h and (key in h) and ('name' in w) and ('name' in h) and (w['name'] == h['name']):
                commands.append(self._compute_command(afi=want['afi'], name=name, attrib=w[key], value=w['name'], opr=opr))
        return commands
