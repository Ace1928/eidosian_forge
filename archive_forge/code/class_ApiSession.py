...
class ApiSession(Session):
    ...
    def __init__(self, controller_ip=None, username=None, password=None, token=None, tenant=None, tenant_uuid=None, verify=False, port=None, timeout=60, api_version=None, retry_conxn_errors=True, data_log=False, avi_credentials=None, session_id=None, csrftoken=None, lazy_authentication=False, max_api_retries=None):
        """
        ...
        if not avi_credentials:
            ...
            self.avi_credentials = AviCredentials(controller=controller_ip, username=username, password=password, api_version=api_version, tenant=tenant, tenant_uuid=tenant_uuid, token=token, port=port, timeout=timeout, session_id=session_id, csrftoken=csrftoken)
        else:
        ...
        self.max_session_retries = self.MAX_API_RETRIES if max_api_retries is None else int(max_api_retries)
        ...
        if self.avi_credentials.controller.startswith('http'):
            ...
            if self.avi_credentials.port is None or self.avi_credentials.port == 80:
                ...
                self.prefix = '{x}:{y}'.format(x=self.avi_credentials.controller, y=self.avi_credentials.port)
        elif port is None or port == 443:
            self.prefix = 'https://{x}'.format(x=self.avi_credentials.controller)
        else:
            self.prefix = 'https://{x}:{y}'.format(x=self.avi_credentials.controller, y=self.avi_credentials.port)
        self.timeout = timeout
        self.key = '%s:%s:%s' % (self.avi_credentials.controller, self.avi_credentials.username, k_port)
        if self.avi_credentials.csrftoken:
            sessionDict[self.key] = {'api': self, 'csrftoken': self.avi_credentials.csrftoken, 'session_id': self.avi_credentials.session_id, 'last_used': datetime.utcnow()}
        elif lazy_authentication:
            sessionDict.get(self.key, {}).update({'api': self, 'last_used': datetime.utcnow()})
        else:
    ...
    def get_context(self):
        return {'session_id': sessionDict[self.key]['session_id'], 'csrftoken': sessionDict[self.key]['csrftoken']}
    ...
    @staticmethod
    def get_session(controller_ip=None, username=None, password=None, token=None, tenant=None, tenant_uuid=None, verify=False, port=None, timeout=60, retry_conxn_errors=True, api_version=None, data_log=False, avi_credentials=None, session_id=None, csrftoken=None, lazy_authentication=False, max_api_retries=None):
        """
        ...
        if not avi_credentials:
            tenant = tenant if tenant else 'admin'
            avi_credentials = AviCredentials(controller=controller_ip, username=username, password=password, api_version=api_version, tenant=tenant, tenant_uuid=tenant_uuid, token=token, port=port, timeout=timeout, session_id=session_id, csrftoken=csrftoken)
        ...
        key = '%s:%s:%s' % (avi_credentials.controller, avi_credentials.username, k_port)
    ...
    def get(self, path, tenant='', tenant_uuid='', timeout=None, params=None, api_version=None, **kwargs):
        """
        ...
        """
        return self._api('get', path, tenant, tenant_uuid, timeout=timeout, params=params, api_version=api_version, **kwargs)

    def get_object_by_name(self, path, name, tenant='', tenant_uuid='', timeout=None, params=None, api_version=None, **kwargs):
        """
        ...
        if not params:
            params = {}
        params['name'] = name
        resp = self.get(path, tenant=tenant, tenant_uuid=tenant_uuid, timeout=timeout, params=params, api_version=api_version, **kwargs)
        if resp.status_code in (401, 419):
            ApiSession.reset_session(self)
            resp = self.get_object_by_name(path, name, tenant, tenant_uuid, timeout=timeout, params=params, **kwargs)
        if resp.status_code > 499 or 'Invalid version' in resp.text:
            logger.error('Error in get object by name for %s named %s. Error: %s', path, name, resp.text)
    ...
    def post(self, path, data=None, tenant='', tenant_uuid='', timeout=None, force_uuid=None, params=None, api_version=None, **kwargs):
        """
        It extends the Session Library interface to add AVI API prefixes,
        handle session exceptions related to authentication and update
        the global user session cache.
        ...
        return self._api('post', path, tenant, tenant_uuid, data=data, timeout=timeout, params=params, api_version=api_version, **kwargs)

    def put(self, path, data=None, tenant='', tenant_uuid='', timeout=None, params=None, api_version=None, **kwargs):
        """
        It extends the Session Library interface to add AVI API prefixes,
        handle session exceptions related to authentication and update
        the global user session cache.
        ...
        returns session's response object
        """
        return self._api('put', path, tenant, tenant_uuid, data=data, timeout=timeout, params=params, api_version=api_version, **kwargs)

    def patch(self, path, data=None, tenant='', tenant_uuid='', timeout=None, params=None, api_version=None, **kwargs):
        """
        It extends the Session Library interface to add AVI API prefixes,
        handle session exceptions related to authentication and update
        the global user session cache.
        ...
        returns session's response object
        """
        return self._api('patch', path, tenant, tenant_uuid, data=data, timeout=timeout, params=params, api_version=api_version, **kwargs)
    ...
    def delete(self, path, tenant='', tenant_uuid='', timeout=None, params=None, data=None, api_version=None, **kwargs):
        """
        It extends the Session Library interface to add AVI API prefixes,
        handle session exceptions related to authentication and update
        the global user session cache.
        ...
        returns session's response object
        """
        return self._api('delete', path, tenant, tenant_uuid, data=data, timeout=timeout, params=params, api_version=api_version, **kwargs)
