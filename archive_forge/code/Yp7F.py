...
class KANWrapper:
    ...
    def _initialize_parameters(self) -> None:
        ...
        def initialize_tensor(tensor: torch.Tensor) -> torch.Tensor:
            """
        ...
        for param in self.model.parameters():
            param.data = initialize_tensor(param)

        # Initialize learnable parameters
        for key, value in self.learnable_params.items():
            if isinstance(value, torch.Tensor):
                self.learnable_params[key] = initialize_tensor(value)
            else:
                self.learnable_params[key] = initialize_tensor(torch.tensor(value))
