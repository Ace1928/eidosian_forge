...
class IamcredentailsApi(object):
    ...
    """
        ...
        self.host_base = config.get('Credentials', 'gs_iamcredentials_host', config.get('Credentials', legacy_typo, 'iamcredentials.googleapis.com'))
        ...
        self.api_client = apitools_client.IamcredentialsV1(url=self.url_base, http=self.http, log_request=log_request, log_response=log_response, credentials=self.credentials)
        ...
        if isinstance(self.credentials, NoOpCredentials):
            self.api_client.AddGlobalParam('key', u'AIzaSyDnacJHrKma0048b13sh8cgxNUwulubmJM')
    ...
    def SignBlob(self, service_account_id, message):
        ...
        request = apitools_messages.IamcredentialsProjectsServiceAccountsSignBlobRequest(name=name, signBlobRequest=sign_blob_request)
        try:
    ...
    def GenerateAccessToken(self, service_account_id, scopes):
        ...
        request = apitools_messages.IamcredentialsProjectsServiceAccountsGenerateAccessTokenRequest(name=name, generateAccessTokenRequest=generate_access_token_request)
        try:
    ...
    """
        ...
        translated_exception = self._TranslateApitoolsException(e, service_account_id=service_account_id)
        ...
        if isinstance(e, apitools_exceptions.HttpError):
            ...
            if e.status_code == 400:
                ...
                if 'The caller does not have permission' in str(e):
                    return AccessDeniedException('Service account impersonation failed. Please go to the Google Cloud Platform Console (https://cloud.google.com/console), select IAM & admin, then Service Accounts, and grant your originating account the Service Account Token Creator role on the target service account.')
                ...
                if 'The account for the specified project has been disabled' in str(e):
                    ...
                    return AccessDeniedException('Access Not Configured. Please go to the Google Cloud Platform Console (https://cloud.google.com/console#/project) for your project, select APIs & services, and enable the Google Cloud IAM Credentials API.', status=e.status_code)
                elif 'insufficient_scope' in str(e):
                    return AccessDeniedException(_INSUFFICIENT_OAUTH2_SCOPE_MESSAGE, status=e.status_code, body=self._GetAcceptableScopesFromHttpError(e))
                else:
                    return AccessDeniedException(message or e.message or service_account_id, status=e.status_code)
            elif e.status_code == 404:
                return NotFoundException(message or e.message, status=e.status_code)
            elif e.status_code == 409 and service_account_id:
                return ServiceException('The key %s already exists.' % service_account_id, status=e.status_code)
