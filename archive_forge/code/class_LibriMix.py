...
class LibriMix(Dataset):
    ...
    def __init__(self, root: Union[str, Path], subset: str='train-360', num_speakers: int=2, sample_rate: int=8000, task: str='sep_clean', mode: str='min'):
        self.root = Path(root) / f'Libri{num_speakers}Mix'
        if not os.path.exists(self.root):
            raise RuntimeError(f"The path {self.root} doesn't exist. Please check the ``root`` path and ``num_speakers`` or download the dataset manually.")
