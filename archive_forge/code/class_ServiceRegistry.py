...
class ServiceRegistry(object):
    ...
    def __init__(self, client_info, message_registry, names, root_package, base_files_package, unelidable_request_methods):
    ...
    def __PrintDocstring(self, printer, method_info, method_name, name):
        ...
        printer('  global_params: (StandardQueryParameters, default: None) global arguments')
    ...
    def WriteProtoFile(self, printer):
        ...
        printer('// Generated services for %s version %s.', client_info.package, client_info.version)
        printer()
    ...
    def WriteFile(self, printer):
        ...
        self.Validate()
        client_info = self.__client_info
        printer('"""Generated client library for %s version %s."""', client_info.package, client_info.version)
        printer('# NOTE: This file is autogenerated and should not be edited by hand.')
        printer()
        ...
        if self.__root_package:
            ...
            import_prefix = ''
        printer('%simport %s as messages', import_prefix, client_info.messages_rule_name)
        printer()
        printer()
        printer('class %s(base_api.BaseApiClient):', client_info.client_class_name)
        with printer.Indent():
            printer('"""Generated client library for service %s version %s."""', client_info.package, client_info.version)
            printer()
            ...
            printer('_SCOPES = {0!r}'.format(client_info.scopes or ['https://www.googleapis.com/auth/userinfo.email']))
            ...
            with printer.Indent():
                ...
                for name in self.__service_method_info_map.keys():
                    printer('self.%s = self.%s(self)', name, self.__GetServiceClassName(name))
            for name, method_info in self.__service_method_info_map.items():
                self.__WriteSingleService(printer, name, method_info, client_info.client_class_name)
    ...
    def __ComputeUploadConfig(self, media_upload_config, method_id):
        ...
        if 'accept' not in media_upload_config:
            logging.warn('No accept types found for upload configuration in method %s, using */*', method_id)
