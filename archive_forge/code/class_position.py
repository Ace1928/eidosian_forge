...
class position(ABC, metaclass=Register):
    ...
    def setup_data(self, data: pd.DataFrame, params: dict[str, Any]) -> pd.DataFrame:
        ...
        Verify & return data
        """
        check_required_aesthetics(self.REQUIRED_AES, data.columns, self.__class__.__name__)
        return data

    @classmethod
    def compute_layer(cls, data: pd.DataFrame, params: dict[str, Any], layout: Layout):
        """
    ...
    @classmethod
    def compute_panel(cls, data: pd.DataFrame, scales: pos_scales, params: dict[str, Any]) -> pd.DataFrame:
        """
