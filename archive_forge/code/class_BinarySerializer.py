...
class BinarySerializer(BaseSerializer):
    ...
    def serialize(self, macaroon):
        return urlsafe_b64encode(self.serialize_raw(macaroon)).decode('ascii').rstrip('=')
    ...
    def _serialize_v1(self, macaroon):
        ...
        for caveat in macaroon.caveats:
            ...
            if caveat._verification_key_id and caveat._location:
                combined += self._packetize(b'vid', caveat._verification_key_id)
                combined += self._packetize(b'cl', caveat._location)
        combined += self._packetize(b'signature', binascii.unhexlify(macaroon.signature_bytes))
        return combined
    ...
    def _serialize_v2(self, macaroon):
        ...
        if macaroon.location is not None:
            self._append_packet(data, self._LOCATION, convert_to_bytes(macaroon.location))
            self._append_packet(data, self._IDENTIFIER, macaroon.identifier_bytes)
        self._append_packet(data, self._EOS)
        for c in macaroon.caveats:
            if c.location is not None:
                self._append_packet(data, self._LOCATION, convert_to_bytes(c.location))
            self._append_packet(data, self._IDENTIFIER, c.caveat_id_bytes)
            if c.verification_key_id is not None:
                self._append_packet(data, self._VID, convert_to_bytes(c.verification_key_id))
            self._append_packet(data, self._EOS)
        self._append_packet(data, self._EOS)
        self._append_packet(data, self._SIGNATURE, binascii.unhexlify(macaroon.signature_bytes))
        return bytes(data)
    ...
    def _deserialize_v1(self, decoded):
        ...
        while index < len(decoded):
            packet_length = int(struct.unpack(b'4s', decoded[index:index + self.PACKET_PREFIX_LENGTH])[0], 16)
            packet = decoded[index + self.PACKET_PREFIX_LENGTH:index + packet_length]
            ...
            if key == b'location':
                ...
                macaroon.caveats.append(Caveat(caveat_id=value, version=MACAROON_V1))
            elif key == b'vid':
                ...
                raise MacaroonDeserializationException('Key {key} not valid key for this format. Value: {value}'.format(key=key, value=value))
    ...
    def _deserialize_v2(self, serialized):
        ...
        macaroon = Macaroon(identifier=section[0].data, location=loc, version=MACAROON_V2)
        while True:
            ...
            if len(section) == 0 or section[0].field_type != self._IDENTIFIER:
                raise MacaroonDeserializationException('no identifier in caveat')
            cav.caveat_id = section[0].data
            section = section[1:]
            if len(section) == 0:
                if cav.location is not None:
                    raise MacaroonDeserializationException('location not allowed in first party caveat')
                macaroon.caveats.append(cav)
                continue
            if len(section) != 1:
                raise MacaroonDeserializationException('extra fields found in caveat')
            if section[0].field_type != self._VID:
                raise MacaroonDeserializationException('invalid field found in caveat')
        ...
        if packet.field_type != self._SIGNATURE:
            raise MacaroonDeserializationException('unexpected field found instead of signature')
