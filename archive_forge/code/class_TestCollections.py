...
class TestCollections(yaql.tests.TestCase):
    ...
    def test_dict(self):
        self.assertEqual({'b c': 13, 'a': 2, 4: 5, None: None, True: False, 8: 8}, self.eval("dict(a => 2, 'b c' => 13, 4 => 5, null => null, true => false, 2+6=>8)"))
    ...
    def test_to_dict(self):
        self.assertEqual({1: 1, 2: 4, 3: 9}, self.eval('$.toDict($, $*$)', data=[1, 2, 3]))
    ...
    def test_dict_set(self):
        data = {'a': 12, 'b c': 44}
        self.assertEqual({'a': 99, 'b c': 44, 'x': None}, self.eval('$.set(a, 99).set(x, null)', data=data))
    ...
    def test_dict_items(self):
        data = {'a': 12, 'b': 44}
        self.assertCountEqual([['a', 12], ['b', 44]], self.eval('$.items()', data=data))
    ...
    def test_in(self):
        ...
        self.assertRaises(exceptions.NoMatchingFunctionException, self.eval, 'a in $', data=data)
    ...
    def test_contains(self):
        ...
        self.assertTrue(self.eval('$.values().select(2*$).contains(24)', data=data))
        self.assertTrue(self.eval('set(1, 2, 5).contains(5)'))
        self.assertTrue(self.eval('[1, 2, 5].contains(5)'))
        self.assertTrue(self.eval('{{a => 2} => {b => 3}}.containsKey({a => 2})'))
    ...
    def test_delete(self):
        self.assertEqual([2, 3, 4], self.eval('[1, 2, 3, 4].delete(0)'))
        self.assertEqual([3, 4], self.eval('[1, 2, 3, 4].delete(0, 2)'))
        self.assertEqual([4], self.eval('[1, 2, 3, 4].delete(0, 2).delete(0)'))
        self.assertEqual([1], self.eval('[1, 2, 3, 4].delete(1, -1)'))
        self.assertEqual([1, 2, 3, 4], self.eval('[1, 2, 3, 4].delete(0, 0)'))
        self.assertEqual([], self.eval('[1, 2, 3, 4].delete(0, -1)'))

    def test_insert(self):
        self.assertEqual([1, 'a', 2], self.eval('[1, 2].insert(1, a)'))
        self.assertEqual([1, ['a', 'b'], 2], self.eval('[1, 2].insert(1, [a, b])'))
        self.assertEqual([1, 'a', 2], self.eval('[1, 2].insert(-1, a)'))
        self.assertEqual([1, 2, 'a'], self.eval('[1, 2].insert(100, a)'))
        self.assertEqual(['b', 'a'], self.eval('[].insert(0, a).insert(0, b)'))
        self.assertRaises(exceptions.NoMatchingMethodException, self.eval, 'set(a, b).insert(1, a)')
    ...
    def test_insert_many(self):
        self.assertEqual([1, 'a', 'b', 2], self.eval('[1, 2].insertMany(1, [a, b])'))
        self.assertEqual(['a', 'b', 1, 2], self.eval('[1, 2].insertMany(-1, [a, b])'))
        self.assertEqual([1, 2, 'a', 'b'], self.eval('[1, 2].insertMany(100, [a, b])'))
        self.assertEqual(['a', 'a', 'b', 'b'], self.eval('[].insertMany(0, [a, b]).insertMany(1, [a, b])'))

    def test_replace(self):
        self.assertEqual([None, 2, 3, 4], self.eval('[1, 2, 3, 4].replace(0, null)'))
        self.assertEqual([None, 3, 4], self.eval('[1, 2, 3, 4].replace(0, null, 2)'))
        self.assertEqual([1, 7], self.eval('[1, 2, 3, 4].replace(1, 7, -1)'))
        self.assertEqual(7, self.eval('set(1, 2, 3, 4).replace(1, 7)')[1])
        self.assertEqual([1, 7, 3, 4], self.eval('set(4, 2, 3, 1).orderBy($).replace(1, 7)'))
    ...
    def test_delete_all(self):
        data = {'a': 1, 'b': 2, 'c': 3, 'd': 4}
        self.assertEqual({'a': 1, 'd': 4}, self.eval('$.deleteAll([b, c])', data=data))
    ...
    def test_set(self):
        ...
        self.assertEqual([{'a': {'b': 'c'}}], self.eval('set({a => {b => c}})'))
