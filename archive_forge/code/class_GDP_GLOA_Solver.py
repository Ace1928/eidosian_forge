...
@SolverFactory.register('gdpopt.gloa', doc='The GLOA (global logic-based outer approximation) Generalized Disjunctive Programming (GDP) solver')
class GDP_GLOA_Solver(_GDPoptAlgorithm, _OAAlgorithmMixIn):
    ...
    def _solve_gdp(self, original_model, config):
        ...
        discrete_obj = next(discrete.component_data_objects(Objective, active=True, descend_into=True))
        ...
        while not config.iterlim or self.iteration < config.iterlim:
            ...
            with time_code(self.timing, 'mip'):
                ...
                self._update_bounds_after_discrete_problem_solve(mip_feasible, discrete_obj, logger)
    ...
    def _add_cuts_to_discrete_problem(self, subproblem_util_block, discrete_problem_util_block, objective_sense, config, timing):
        ...
        for constr in self._get_active_untransformed_constraints(discrete_problem_util_block, config):
            ...
            cut_body = sum((ccSlope[var] * (var - var.value) for var in vars_in_constr if not var.fixed))
            if not is_potentially_variable(cut_body):
                if cut_body + ccStart >= lb_int - config.constraint_tolerance and cut_body + cvStart <= ub_int + config.constraint_tolerance:
