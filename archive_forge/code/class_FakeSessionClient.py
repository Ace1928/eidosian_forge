...
class FakeSessionClient(base_client.SessionClient):
    ...
    def request(self, url, method, **kwargs):
        ...
        munged_url = munged_url.strip('/').replace('/', '_').replace('.', '_').replace(' ', '_')
    ...
    def get_flavors(self, **kw):
        return (200, {'flavors': [{'id': 1, 'name': '256 MB Server', 'ram': 256, 'disk': 10, 'OS-FLV-EXT-DATA:ephemeral': 10}, {'id': 2, 'name': 'm1.small', 'ram': 512, 'disk': 20, 'OS-FLV-EXT-DATA:ephemeral': 20}, {'id': 3, 'name': 'm1.large', 'ram': 512, 'disk': 20, 'OS-FLV-EXT-DATA:ephemeral': 30}]})
    ...
    def get_os_keypairs_test(self, *kw):
        return (200, {'keypair': {'fingerprint': 'FAKE_KEYPAIR', 'name': 'test', 'public_key': 'foo'}})
    ...
    def get_limits(self, *kw):
        return (200, {'limits': {'absolute': {'maxServerMeta': 3, 'maxPersonalitySize': 10240, 'maxPersonality': 5}}})