...
class GitWorkingTree(MutableGitIndexTree, workingtree.WorkingTree):
    ...
    def _get_submodule_index(self, relpath):
        ...
        try:
            ...
            submodule_transport = self.control_transport.clone(posixpath.join('modules', decode_git_path(info[1])))
            try:
    ...
    def set_parent_ids(self, revision_ids, allow_leftmost_as_ghost=False):
        ...
        with self.lock_tree_write():
            self._check_parents_for_ghosts(revision_ids, allow_leftmost_as_ghost=allow_leftmost_as_ghost)
    ...
    def remove(self, files, verbose=False, to_file=None, keep_files=True, force=False):
        ...
        def backup(file_to_backup):
            ...
            return 'removed {} (but kept a copy: {})'.format(file_to_backup, backup_name)
        files_to_backup = []
        all_files = set()
        ...
        with self.lock_tree_write():
            ...
            if not keep_files and (not force):
                for change in self.iter_changes(self.basis_tree(), include_unchanged=True, require_versioned=False, want_unversioned=True, specific_files=files):
                    if change.versioned[0] is False:
                        files_to_backup.append(change.path[1])
                        files_to_backup.extend(osutils.parent_directories(change.path[1]))
                    elif change.changed_content and change.kind[1] is not None and osutils.is_inside_any(files, change.path[1]):
                        files_to_backup.append(change.path[1])
                        files_to_backup.extend(osutils.parent_directories(change.path[1]))
            for f in files:
                if f == '':
    ...
    def _iter_files_recursive(self, from_dir=None, include_dirs=False, recurse_nested=False):
        if from_dir is None:
            from_dir = ''
    ...
    def extras(self):
        ...
        """
        with self.lock_read():
            index_paths = {decode_git_path(p) for p, sha, mode in self.iter_git_objects()}
    ...
    def list_files(self, include_root=False, from_dir=None, recursive=True, recurse_nested=False):
        if from_dir is None or from_dir == '.':
            from_dir = ''
        dir_ids = {}
        fk_entries = {'directory': tree.TreeDirectory, 'file': tree.TreeFile, 'symlink': tree.TreeLink, 'tree-reference': tree.TreeReference}
        with self.lock_read():
            ...
            if recursive:
                path_iterator = sorted(self._iter_files_recursive(from_dir, include_dirs=True, recurse_nested=recurse_nested))
            else:
                encoded_from_dir = os.fsencode(self.abspath(from_dir))
                path_iterator = sorted([os.path.join(from_dir, os.fsdecode(name)) for name in os.listdir(encoded_from_dir) if not self.controldir.is_control_filename(os.fsdecode(name)) and (not self.mapping.is_special_file(os.fsdecode(name)))])
        ...
        with self.lock_read():
            ...
            for item_path, value in self.index.iteritems():
                ...
                if '/' in subpath:
                    ...
                    file_ie = self._get_file_ie(name, decoded_item_path, value, parent_id)
                yield file_ie
    ...
    def walkdirs(self, prefix=''):
        ...
        try:
            ...
            if not (e.errno == errno.ENOENT or (sys.platform == 'win32' and e.errno == ERROR_PATH_NOT_FOUND)):
                raise
            current_disk = None
            disk_finished = True
        try:
        ...
        while not inv_finished or not disk_finished:
            if current_disk:
                (cur_disk_dir_relpath, cur_disk_dir_path_from_top), cur_disk_dir_content = current_disk
            else:
                (cur_disk_dir_relpath, cur_disk_dir_path_from_top), cur_disk_dir_content = ((None, None), None)
            if not disk_finished:
                if cur_disk_dir_path_from_top[top_strip_len:] == '' and len(cur_disk_dir_content) > 0:
                    bzrdir_loc = bisect_left(cur_disk_dir_content, ('.git', '.git'))
                    if bzrdir_loc < len(cur_disk_dir_content) and self.controldir.is_control_filename(cur_disk_dir_content[bzrdir_loc][0]):
            ...
            if inv_finished:
                ...
                direction = (current_inv[0][0] > cur_disk_dir_relpath) - (current_inv[0][0] < cur_disk_dir_relpath)
            if direction > 0:
                dirblock = [(relpath, basename, kind, stat, None) for relpath, basename, kind, stat, top_path in cur_disk_dir_content]
                ...
                dirblock = [(relpath, basename, 'unknown', None, kind) for relpath, basename, dkind, stat, fileid, kind in current_inv[1]]
                ...
                dirblock = []
                for relpath, subiterator in itertools.groupby(sorted(current_inv[1] + cur_disk_dir_content, key=operator.itemgetter(0)), operator.itemgetter(1)):
                    path_elements = list(subiterator)
                    if len(path_elements) == 2:
                        inv_row, disk_row = path_elements
                        dirblock.append((inv_row[0], inv_row[1], disk_row[2], disk_row[3], inv_row[5]))
                    elif len(path_elements[0]) == 5:
                        dirblock.append((path_elements[0][0], path_elements[0][1], path_elements[0][2], path_elements[0][3], None))
                    elif len(path_elements[0]) == 6:
                        dirblock.append((path_elements[0][0], path_elements[0][1], 'unknown', None, path_elements[0][5]))
                    else:
        ...
        with self.lock_read():
            ...
            for parent_id in self.get_parent_ids():
                ...
                with parent_tree.lock_read():
                    ...
                    if kind != 'file':
                        continue
                    parent_text_key = (parent_path, parent_tree.get_file_revision(parent_path))
                    if parent_text_key not in maybe_file_parent_keys:
                        maybe_file_parent_keys.append(parent_text_key)
            from breezy.annotate import Annotator
            from .annotate import AnnotateProvider
            annotate_provider = AnnotateProvider(self.branch.repository._file_change_scanner)
    ...
    def reset_state(self, revision_ids=None):
        ...
        with self.lock_tree_write():
            ...
            if self.branch.head is not None:
                for entry in iter_tree_contents(self.store, self.store[self.branch.head].tree):
                    ...
                    if S_ISGITLINK(entry.mode):
                        ...
                        try:
                            ...
                            st = os.stat_result((entry.mode, 0, 0, 0, 0, 0, len(obj.as_raw_string()), 0, 0, 0))
    ...
    def pull(self, source, overwrite=False, stop_revision=None, change_reporter=None, possible_transports=None, local=False, show_base=False, tag_selector=None):
        with self.lock_write(), source.lock_read():
            old_revision = self.branch.last_revision()
            count = self.branch.pull(source, overwrite=overwrite, stop_revision=stop_revision, possible_transports=possible_transports, local=local, tag_selector=tag_selector)
            self._update_git_tree(old_revision=old_revision, new_revision=self.branch.last_revision(), change_reporter=change_reporter, show_base=show_base)
            return count
    ...
    def _update_tree(self, old_tip=None, change_reporter=None, revision=None, show_base=False):
        ...
        with self.lock_tree_write():
            ...
            if not _mod_revision.is_null(old_tip) and old_tip != last_rev:
                ...
                nb_conflicts = merge_inner(self.branch, other_tree, base_tree, this_tree=self, change_reporter=change_reporter, show_base=show_base)
                if nb_conflicts:
            ...
            if last_rev != revision:
                ...
                nb_conflicts = merge_inner(self.branch, to_tree, base_tree, this_tree=self, change_reporter=change_reporter, show_base=show_base)
                self.set_last_revision(revision)
                parent_trees = [(revision, to_tree)]
                merges = self.get_parent_ids()[1:]
                for parent in merges:
                    parent_trees.append((parent, self.branch.repository.revision_tree(parent)))
                if not _mod_revision.is_null(old_tip):
                    parent_trees.append((old_tip, self.branch.repository.revision_tree(old_tip)))
