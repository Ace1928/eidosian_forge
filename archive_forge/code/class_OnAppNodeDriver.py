...
class OnAppNodeDriver(NodeDriver):
    ...
    def create_node(self, name, ex_memory, ex_cpus, ex_cpu_shares, ex_hostname, ex_template_id, ex_primary_disk_size, ex_swap_disk_size, ex_required_virtual_machine_build=1, ex_required_ip_address_assignment=1, **kwargs):
        """
        Add a VS
        ...
        """
        server_params = dict(label=name, memory=ex_memory, cpus=ex_cpus, cpu_shares=ex_cpu_shares, hostname=ex_hostname, template_id=ex_template_id, primary_disk_size=ex_primary_disk_size, swap_disk_size=ex_swap_disk_size, required_virtual_machine_build=ex_required_virtual_machine_build, required_ip_address_assignment=ex_required_ip_address_assignment, rate_limit=kwargs.get('rate_limit'))
        server_params.update(OnAppNodeDriver._create_args_to_params(**kwargs))
        data = json.dumps({'virtual_machine': server_params})
        response = self.connection.request('/virtual_machines.json', data=data, headers={'Content-type': 'application/json'}, method='POST')
    ...
    def destroy_node(self, node, ex_convert_last_backup=0, ex_destroy_all_backups=0):
        ...
        """
        server_params = {'convert_last_backup': ex_convert_last_backup, 'destroy_all_backups': ex_destroy_all_backups}
    ...
    def import_key_pair_from_string(self, name, key_material):
        ...
        user_id = self.connection.request('/profile.json').object['user']['id']
        response = self.connection.request('/users/%s/ssh_keys.json' % user_id, data=data, headers={'Content-type': 'application/json'}, method='POST')
        return self._to_key_pair(response.object['ssh_key'])
    ...
    def _to_key_pair(self, data):
        ...
        return KeyPair(name=data['id'], fingerprint=None, public_key=data['key'], private_key=None, driver=self, extra=extra)

    def _to_image(self, template):
        extra = {'distribution': template['operating_system_distro'], 'operating_system': template['operating_system'], 'operating_system_arch': template['operating_system_arch'], 'allow_resize_without_reboot': template['allow_resize_without_reboot'], 'allowed_hot_migrate': template['allowed_hot_migrate'], 'allowed_swap': template['allowed_swap'], 'min_disk_size': template['min_disk_size'], 'min_memory_size': template['min_memory_size'], 'created_at': template['created_at']}
    ...
    def _create_args_to_params(**kwargs):
        ...
        :return: ``dict``
        """
        params = ['ex_cpu_sockets', 'ex_cpu_threads', 'ex_enable_autoscale', 'ex_data_store_group_primary_id', 'ex_data_store_group_swap_id', 'ex_hypervisor_group_id', 'ex_hypervisor_id', 'ex_initial_root_password', 'ex_note', 'ex_primary_disk_min_iops', 'ex_primary_network_id', 'ex_primary_network_group_id', 'ex_recipe_ids', 'ex_required_automatic_backup', 'ex_required_virtual_machine_startup', 'ex_required_virtual_machine_startup', 'ex_selected_ip_address_id', 'ex_swap_disk_min_iops', 'ex_type_of_format', 'ex_custom_recipe_variables', 'ex_licensing_key', 'ex_licensing_server_id', 'ex_licensing_type']
        server_params = {}
        for p in params:
            value = kwargs.get(p)
            if value:
