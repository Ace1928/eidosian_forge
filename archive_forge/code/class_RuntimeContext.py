...
class RuntimeContext(object):
    ...
    @Deprecated(message='Use get_xxx_id() methods to get relevant ids instead', warning=True)
    def get(self) -> Dict[str, Any]:
        ...
        """
        context = {'job_id': self.job_id, 'node_id': self.node_id, 'namespace': self.namespace}
    ...
    def get_node_id(self) -> str:
        ...
        Raises:
            AssertionError: If not called in a driver or worker. Generally,
                this means that ray.init() was not called.
        """
        assert ray.is_initialized(), 'Node ID is not available because Ray has not been initialized.'
        node_id = self.worker.current_node_id
        return node_id.hex()
