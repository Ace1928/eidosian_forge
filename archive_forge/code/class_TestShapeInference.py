...
class TestShapeInference(TestShapeInferenceHelper):
    ...
    def test_transpose(self, _, version) -> None:
        graph = self._make_graph([('X', TensorProto.FLOAT, (2, 3, 4))], [make_node('Transpose', ['X'], ['Y'], perm=[1, 0, 2])], [])
        self._assert_inferred(graph, [make_tensor_value_info('Y', TensorProto.FLOAT, (3, 2, 4))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, version)])

    @parameterized.expand(all_versions_for('Transpose'))
    def test_transpose_preexisting(self, _, version) -> None:
        graph = self._make_graph([('X', TensorProto.FLOAT, (2, 3, 4))], [make_node('Transpose', ['X'], ['Y'], perm=[1, 0, 2])], [make_tensor_value_info('Y', TensorProto.FLOAT, None)])
        self._assert_inferred(graph, [make_tensor_value_info('Y', TensorProto.FLOAT, (3, 2, 4))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, version)])
    ...
    def test_transpose_partial(self, _, version) -> None:
        graph = self._make_graph([('X', TensorProto.FLOAT, (2, 3, 4))], [make_node('Transpose', ['X'], ['Y'], perm=[1, 0, 2])], [make_tensor_value_info('Y', TensorProto.UNDEFINED, (3, 'a', 'b'))])
        self._assert_inferred(graph, [make_tensor_value_info('Y', TensorProto.FLOAT, (3, 2, 4))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, version)])

    @parameterized.expand(all_versions_for('Transpose'))
    def test_transpose_preexisting_incorrect_shape(self, *_) -> None:
        graph = self._make_graph([('X', TensorProto.FLOAT, (2, 3, 4))], [make_node('Transpose', ['X'], ['Y'], perm=[1, 0, 2])], [make_tensor_value_info('Y', TensorProto.FLOAT, (5, 5, 5))])
        self.assertRaises(onnx.shape_inference.InferenceError, self._inferred, graph)

    @parameterized.expand(all_versions_for('Transpose'))
    def test_transpose_preexisting_incorrect_type(self, *_) -> None:
        graph = self._make_graph([('X', TensorProto.FLOAT, (2, 3, 4))], [make_node('Transpose', ['X'], ['Y'], perm=[1, 0, 2])], [make_tensor_value_info('Y', TensorProto.STRING, (3, 2, 4))])
        self.assertRaises(onnx.shape_inference.InferenceError, self._inferred, graph)
    ...
    def test_cast(self, _, version) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (2, 4, 3))], [make_node('Cast', ['x'], ['y'], to=TensorProto.UINT8)], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.UINT8, (2, 4, 3))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, version)])

    @parameterized.expand(all_versions_for('Cast'))
    @unittest.skip('Issue #5960')
    def test_cast_to_complex(self, _, version) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (2, 4, 3))], [make_node('Cast', ['x'], ['y'], to=TensorProto.COMPLEX128)], [])
        self.assertRaises(onnx.shape_inference.InferenceError, self._inferred, graph)

    @parameterized.expand(all_versions_for('CastLike'))
    def test_cast_like(self, _, version) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (2, 4, 3)), ('t', TensorProto.FLOAT16, ('N',))], [make_node('CastLike', ['x', 't'], ['y'])], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.FLOAT16, (2, 4, 3))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, version)])

    @parameterized.expand(all_versions_for('Col2Im'))
    def test_col2im(self, _, version) -> None:
        graph = self._make_graph([('input', TensorProto.FLOAT, (1, 5, 5)), ('output_shape', TensorProto.INT64, (2,)), ('kernel_shape', TensorProto.INT64, (2,))], [make_node('Col2Im', ['input', 'output_shape', 'kernel_shape'], ['output'])], [], initializer=[make_tensor('output_shape', TensorProto.INT64, (2,), (5, 5)), make_tensor('kernel_shape', TensorProto.INT64, (2,), (1, 5))])
    ...
    def test_col2im_dilations(self, _, version) -> None:
        graph = self._make_graph([('input', TensorProto.FLOAT, (1, 4, 5)), ('output_shape', TensorProto.INT64, (2,)), ('kernel_shape', TensorProto.INT64, (2,))], [make_node('Col2Im', ['input', 'output_shape', 'kernel_shape'], ['output'], dilations=[1, 5])], [], initializer=[make_tensor('output_shape', TensorProto.INT64, (2,), (6, 6)), make_tensor('kernel_shape', TensorProto.INT64, (2,), (2, 2))])
        self._assert_inferred(graph, [make_tensor_value_info('output', TensorProto.FLOAT, (1, 1, 6, 6))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, version)])

    @parameterized.expand(all_versions_for('Col2Im'))
    def test_col2im_5d(self, _, version) -> None:
        graph = self._make_graph([('input', TensorProto.FLOAT, (1, 10, 12)), ('output_shape', TensorProto.INT64, (3,)), ('kernel_shape', TensorProto.INT64, (3,))], [make_node('Col2Im', ['input', 'output_shape', 'kernel_shape'], ['output'])], [], initializer=[make_tensor('output_shape', TensorProto.INT64, (3,), (3, 4, 5)), make_tensor('kernel_shape', TensorProto.INT64, (3,), (1, 1, 5))])
        self._assert_inferred(graph, [make_tensor_value_info('output', TensorProto.FLOAT, (1, 2, 3, 4, 5))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, version)])

    @parameterized.expand(all_versions_for('Concat'))
    def test_concat(self, _, version) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (2, 4, 3)), ('y', TensorProto.FLOAT, (7, 4, 3))], [make_node('Concat', ['x', 'y'], ['z'], axis=0)], [])
        self._assert_inferred(graph, [make_tensor_value_info('z', TensorProto.FLOAT, (9, 4, 3))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, version)])
    ...
    def test_concat_3d_axis_2(self, _, version) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (2, 2, 2)), ('y', TensorProto.FLOAT, (2, 2, 2))], [make_node('Concat', ['x', 'y'], ['z'], axis=2)], [])
        self._assert_inferred(graph, [make_tensor_value_info('z', TensorProto.FLOAT, (2, 2, 4))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, version)])
    ...
    def test_reshape_dynamic_shape_symbolic(self, _, version) -> None:
        graph = self._make_graph([('x', TensorProto.UINT8, (2, 4, 3)), ('shape', TensorProto.INT64, ('M',))], [make_node('Reshape', ['x', 'shape'], ['y'])], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.UINT8, None)], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, version)])

    @parameterized.expand(all_versions_for('Reshape'))
    def test_reshape_dynamic_unknown_shape(self, _, version) -> None:
        graph = self._make_graph([('x', TensorProto.UINT8, (2, 4, 3)), ('shape', TensorProto.INT64, None)], [make_node('Reshape', ['x', 'shape'], ['y'])], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.UINT8, None)], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, version)])
    ...
    def test_reshape_static_shape_zero(self, _, version) -> None:
        graph = self._make_graph([('x', TensorProto.UINT8, (1, 1, 1)), ('shape', TensorProto.INT64, (3,))], [make_node('Reshape', ['x', 'shape'], ['y'])], [], initializer=[make_tensor('shape', TensorProto.INT64, (3,), (0, 1, 1))])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.UINT8, (1, 1, 1))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, version)])
    ...
    def test_upsample(self, _, version) -> None:
        if version == 7:
            graph = self._make_graph([('x', TensorProto.INT32, (2, 4, 3, 5))], [make_node('Upsample', ['x'], ['y'], scales=[1.0, 1.1, 1.3, 1.9])], [])
            ...
            graph = self._make_graph([('x', TensorProto.INT32, (2, 4, 3, 5)), ('scales', TensorProto.FLOAT, (4,))], [make_node('Upsample', ['x', 'scales'], ['y'])], [], initializer=[make_tensor('scales', TensorProto.FLOAT, (4,), (1.0, 1.1, 1.3, 1.9))])
    ...
    def test_expand(self, _, version) -> None:
        graph = self._make_graph([('x', TensorProto.INT32, (3, 1)), ('shape', TensorProto.INT64, (3,))], [make_node('Expand', ['x', 'shape'], ['y'])], [], initializer=[make_tensor('shape', TensorProto.INT64, (3,), (2, 1, 6))])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.INT32, (2, 3, 6))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, version)])
    ...
    def test_expand_dynamic_shape(self, _, version) -> None:
        graph = self._make_graph([('x', TensorProto.INT32, (1, 2, None)), ('shape', TensorProto.INT64, (3,))], [make_node('Expand', ['x', 'shape'], ['y'])], [], initializer=[])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.INT32, (None, 2, None))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, version)])
    ...
    def test_resize_size_axes_3_2(self, _, version) -> None:
        self.skipIf(version < 18, 'axes is from Version 18')
        graph = self._make_graph([('x', TensorProto.INT32, (2, 4, 3, 5)), ('roi', TensorProto.FLOAT, (4,)), ('sizes', TensorProto.INT64, (2,))], [make_node('Resize', ['x', 'roi', '', 'sizes'], ['y'], axes=(3, 2))], [], initializer=[make_tensor('sizes', TensorProto.INT64, (2,), (6, 7))])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.INT32, (2, 4, 7, 6))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, version)])

    @parameterized.expand(all_versions_for('Resize'))
    def test_resize_size_not_larger(self, _, version) -> None:
        self.skipIf(version < 18, 'keep_aspect_ratio_policy is from Version 18')
        graph = self._make_graph([('x', TensorProto.INT32, (3, 5)), ('roi', TensorProto.FLOAT, (4,)), ('sizes', TensorProto.INT64, (2,))], [make_node('Resize', ['x', 'roi', '', 'sizes'], ['y'], keep_aspect_ratio_policy='not_larger')], [], initializer=[make_tensor('sizes', TensorProto.INT64, (2,), (6, 6))])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.INT32, (4, 6))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, version)])
    ...
    def test_resize_size_axes_2_3_not_larger(self, _, version) -> None:
        ...
        graph = self._make_graph([('x', TensorProto.INT32, (2, 4, 3, 5)), ('roi', TensorProto.FLOAT, (4,)), ('sizes', TensorProto.INT64, (2,))], [make_node('Resize', ['x', 'roi', '', 'sizes'], ['y'], axes=(2, 3), keep_aspect_ratio_policy='not_larger')], [], initializer=[make_tensor('sizes', TensorProto.INT64, (2,), (6, 6))])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.INT32, (2, 4, 4, 6))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, version)])
    ...
    def test_resize_scale_axes_2_3(self, _, version) -> None:
        self.skipIf(version < 18, 'axes is from Version 18')
        graph = self._make_graph([('x', TensorProto.INT32, (2, 4, 3, 5)), ('roi', TensorProto.FLOAT, (8,)), ('scales', TensorProto.FLOAT, (2,))], [make_node('Resize', ['x', 'roi', 'scales'], ['y'], axes=(2, 3))], [], initializer=[make_tensor('scales', TensorProto.FLOAT, (2,), (1.3, 1.9))])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.INT32, (2, 4, 3, 9))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, version)])

    @parameterized.expand(all_versions_for('Resize'))
    def test_resize_scale_axes_3_2(self, _, version) -> None:
        self.skipIf(version < 18, 'axes is from Version 18')
        graph = self._make_graph([('x', TensorProto.INT32, (2, 4, 3, 5)), ('roi', TensorProto.FLOAT, (8,)), ('scales', TensorProto.FLOAT, (2,))], [make_node('Resize', ['x', 'roi', 'scales'], ['y'], axes=(3, 2))], [], initializer=[make_tensor('scales', TensorProto.FLOAT, (2,), (1.9, 1.3))])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.INT32, (2, 4, 3, 9))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, version)])

    @parameterized.expand(all_versions_for('Resize'))
    def test_resize_scale_raw_data(self, _, version) -> None:
        self.skipIf(version < 11, 'roi input is from Version 11')
        graph = self._make_graph([('x', TensorProto.INT32, (1, 3, 4, 5)), ('roi', TensorProto.FLOAT, (8,)), ('scales', TensorProto.FLOAT, (4,))], [make_node('Resize', ['x', 'roi', 'scales'], ['y'])], [], initializer=[make_tensor('scales', TensorProto.FLOAT, (4,), vals=np.array([2.0, 1.1, 2.3, 1.9], dtype='<f4').tobytes(), raw=True)])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.INT32, (2, 3, 9, 9))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, version)])
    ...
    def test_shape(self, _, version) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (2, 4, 3))], [make_node('Shape', ['x'], ['y'])], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.INT64, (3,))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, version)])
    ...
    def test_shape_end_1(self, _, version) -> None:
        self.skipIf(version < 15, 'start and end are from Version 15')
        graph = self._make_graph([('x', TensorProto.FLOAT, (2, 4, 3))], [make_node('Shape', ['x'], ['y'], end=1)], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.INT64, (1,))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, version)])
    ...
    def test_shape_clip1(self, _, version) -> None:
        self.skipIf(version < 15, 'start and end are from Version 15')
        graph = self._make_graph([('x', TensorProto.FLOAT, (2, 4, 3))], [make_node('Shape', ['x'], ['y'], start=-5)], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.INT64, (3,))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, version)])

    @parameterized.expand(all_versions_for('Shape'))
    def test_shape_clip2(self, _, version) -> None:
        self.skipIf(version < 15, 'start and end are from Version 15')
        graph = self._make_graph([('x', TensorProto.FLOAT, (2, 4, 3))], [make_node('Shape', ['x'], ['y'], end=10)], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.INT64, (3,))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, version)])

    @parameterized.expand(all_versions_for('Size'))
    def test_size(self, _, version) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (2, 4, 3))], [make_node('Size', ['x'], ['y'])], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.INT64, ())], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, version)])

    @parameterized.expand(all_versions_for('Gather'))
    def test_gather(self, _, version) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (4, 3)), ('i', TensorProto.INT64, (2,))], [make_node('Gather', ['x', 'i'], ['y'])], [])
    ...
    def test_gather_axis1(self, _, version) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (4, 3, 5)), ('i', TensorProto.INT64, (1, 2))], [make_node('Gather', ['x', 'i'], ['y'], axis=1)], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.FLOAT, (4, 1, 2, 5))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, version)])
    ...
    def test_gather_elements(self, _, version) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (2, 2)), ('i', TensorProto.INT64, (2, 2))], [make_node('GatherElements', ['x', 'i'], ['y'], axis=1)], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.FLOAT, (2, 2))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, version)])
    ...
    def _test_scatter(self, version) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (3, 3)), ('i', TensorProto.INT64, (2, 3)), ('u', TensorProto.FLOAT, (2, 3))], [make_node('Scatter', ['x', 'i', 'u'], ['y'])], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.FLOAT, (3, 3))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, version)])
    ...
    def _test_scatter_axis1(self, version) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (1, 5)), ('i', TensorProto.INT64, (1, 2)), ('u', TensorProto.FLOAT, (1, 2))], [make_node('Scatter', ['x', 'i', 'u'], ['y'], axis=1)], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.FLOAT, (1, 5))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, version)])

    @parameterized.expand(all_versions_for('ScatterElements'))
    def test_scatter_elements(self, _, version) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (3, 3)), ('i', TensorProto.INT64, (2, 3)), ('u', TensorProto.FLOAT, (2, 3))], [make_node('ScatterElements', ['x', 'i', 'u'], ['y'])], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.FLOAT, (3, 3))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, version)])

    @parameterized.expand(all_versions_for('ScatterElements'))
    def test_scatter_elements_axis1(self, _, version) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (1, 5)), ('i', TensorProto.INT64, (1, 2)), ('u', TensorProto.FLOAT, (1, 2))], [make_node('ScatterElements', ['x', 'i', 'u'], ['y'], axis=1)], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.FLOAT, (1, 5))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, version)])

    @parameterized.expand(all_versions_for('ScatterND'))
    def test_scatternd(self, _, version) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (4, 5, 6)), ('indices', TensorProto.INT64, (3, 3, 2)), ('updates', TensorProto.FLOAT, (3, 3, 6))], [make_node('ScatterND', ['x', 'indices', 'updates'], ['y'])], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.FLOAT, (4, 5, 6))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, version)])
    ...
    def test_squeeze(self, _, version) -> None:
        if version == 11:
            graph = self._make_graph([('x', TensorProto.FLOAT, (1, 3, 1, 1, 2, 1))], [make_node('Squeeze', 'x', 'y', axes=[0, 2, 3, 5])], [])
            self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.FLOAT, (3, 2))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, version)])
        else:
            graph = self._make_graph([('x', TensorProto.FLOAT, (1, 3, 1, 1, 2, 1)), ('axes', TensorProto.INT64, (4,))], [make_node('Squeeze', ['x', 'axes'], 'y')], [], initializer=[make_tensor('axes', TensorProto.INT64, (4,), (0, 2, 3, 5))])
            self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.FLOAT, (3, 2))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, version)])
    ...
    def test_stringconcat_broadcasting(self, _, version) -> None:
        graph = self._make_graph([('x', TensorProto.STRING, (2, 3, 4)), ('y', TensorProto.STRING, (1, 3, 1))], [make_node('StringConcat', ['x', 'y'], 'z')], [])
        self._assert_inferred(graph, [make_tensor_value_info('z', TensorProto.STRING, (2, 3, 4))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, version)])

    @parameterized.expand(all_versions_for('RegexFullMatch'))
    def test_regex_full_match(self, _, version) -> None:
        graph = self._make_graph([('x', TensorProto.STRING, (2, 4, 3, 9))], [make_node('RegexFullMatch', ['x'], ['y'], pattern='^[A-Z][a-z]*$')], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.BOOL, (2, 4, 3, 9))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, version)])
    ...
    def test_unsqueeze_regular(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (3, 2)), ('axes', TensorProto.INT64, (4,))], [make_node('Unsqueeze', ['x', 'axes'], 'y')], [], initializer=[make_tensor('axes', TensorProto.INT64, (4,), (0, 1, 3, 5))])
    ...
    def test_unsqueeze_unsorted_axes(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (3, 4, 5)), ('axes', TensorProto.INT64, (2,))], [make_node('Unsqueeze', ['x', 'axes'], 'y')], [], initializer=[make_tensor('axes', TensorProto.INT64, (2,), (4, 0))])
    ...
    def test_unsqueeze_negative_axes(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (3, 4, 5)), ('axes', TensorProto.INT64, (2,))], [make_node('Unsqueeze', ['x', 'axes'], 'y')], [], initializer=[make_tensor('axes', TensorProto.INT64, (2,), (0, -1))])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.FLOAT, (1, 3, 4, 5, 1))])
    ...
    def test_slice_unsorted_axes(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (3, 2)), ('starts', TensorProto.INT64, (2,)), ('ends', TensorProto.INT64, (2,)), ('axes', TensorProto.INT64, (2,))], [make_node('Slice', ['x', 'starts', 'ends', 'axes'], 'y')], [], initializer=[make_tensor('starts', TensorProto.INT64, (2,), (1, 0)), make_tensor('ends', TensorProto.INT64, (2,), (2, 2)), make_tensor('axes', TensorProto.INT64, (2,), (1, 0))])
    ...
    def test_slice_negative_end(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (3, 2)), ('starts', TensorProto.INT64, (2,)), ('ends', TensorProto.INT64, (2,)), ('axes', TensorProto.INT64, (2,))], [make_node('Slice', ['x', 'starts', 'ends', 'axes'], 'y')], [], initializer=[make_tensor('starts', TensorProto.INT64, (2,), (1, 0)), make_tensor('ends', TensorProto.INT64, (2,), (200, -1)), make_tensor('axes', TensorProto.INT64, (2,), (0, 1))])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.FLOAT, (2, 1))])

    def test_slice_negative_start(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (3, 2)), ('starts', TensorProto.INT64, (2,)), ('ends', TensorProto.INT64, (2,)), ('axes', TensorProto.INT64, (2,))], [make_node('Slice', ['x', 'starts', 'ends', 'axes'], 'y')], [], initializer=[make_tensor('starts', TensorProto.INT64, (2,), (1, -2)), make_tensor('ends', TensorProto.INT64, (2,), (200, 3)), make_tensor('axes', TensorProto.INT64, (2,), (0, 1))])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.FLOAT, (2, 2))])

    def test_slice_negative_step(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (3, 4)), ('starts', TensorProto.INT64, (2,)), ('ends', TensorProto.INT64, (2,)), ('axes', TensorProto.INT64, (2,)), ('steps', TensorProto.INT64, (2,))], [make_node('Slice', ['x', 'starts', 'ends', 'axes', 'steps'], 'y')], [], initializer=[make_tensor('starts', TensorProto.INT64, (2,), (1, 4)), make_tensor('ends', TensorProto.INT64, (2,), (200, 0)), make_tensor('axes', TensorProto.INT64, (2,), (0, 1)), make_tensor('steps', TensorProto.INT64, (2,), (1, -1))])
    ...
    def test_conv(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (3, 4, 5, 6, 7)), ('y', TensorProto.FLOAT, (5, 4, 2, 4, 3))], [make_node('Conv', ['x', 'y'], 'z', pads=[0, 1, 1, 0, 0, 1], dilations=[1, 2, 2], strides=[1, 1, 2])], [])
        self._assert_inferred(graph, [make_tensor_value_info('z', TensorProto.FLOAT, (3, 5, 4, 1, 3))])

    def test_conv_1d_simple(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (30, 4, 5)), ('y', TensorProto.FLOAT, (50, 4, 2))], [make_node('Conv', ['x', 'y'], 'z', dilations=[1])], [])
        self._assert_inferred(graph, [make_tensor_value_info('z', TensorProto.FLOAT, (30, 50, 4))])

    def test_conv_dilations(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (30, 4, 8, 8, 8)), ('y', TensorProto.FLOAT, (50, 4, 3, 3, 3))], [make_node('Conv', ['x', 'y'], 'z', dilations=[1, 2, 3])], [])
        self._assert_inferred(graph, [make_tensor_value_info('z', TensorProto.FLOAT, (30, 50, 6, 4, 2))])

    def test_conv_strides(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (30, 4, 8, 8, 8)), ('y', TensorProto.FLOAT, (50, 4, 3, 3, 3))], [make_node('Conv', ['x', 'y'], 'z', strides=[1, 2, 3])], [])
        self._assert_inferred(graph, [make_tensor_value_info('z', TensorProto.FLOAT, (30, 50, 6, 3, 2))])

    def test_conv_pads(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (30, 4, 7, 6, 4)), ('y', TensorProto.FLOAT, (50, 4, 3, 3, 3))], [make_node('Conv', ['x', 'y'], 'z', pads=[1, 1, 2, 0, 1, 2])], [])
        self._assert_inferred(graph, [make_tensor_value_info('z', TensorProto.FLOAT, (30, 50, 6, 6, 6))])

    def test_conv_auto_pad(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (30, 4, 7, 6, 4)), ('y', TensorProto.FLOAT, (50, 4, 4, 3, 2))], [make_node('Conv', ['x', 'y'], 'z', auto_pad='SAME_UPPER')], [])
        self._assert_inferred(graph, [make_tensor_value_info('z', TensorProto.FLOAT, (30, 50, 7, 6, 4))])

    def test_conv_auto_pads(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (30, 4, 7, 6, 4)), ('y', TensorProto.FLOAT, (50, 4, 4, 3, 2))], [make_node('Conv', ['x', 'y'], 'z', auto_pad='SAME_UPPER', strides=[2, 2, 1])], [])
        self._assert_inferred(graph, [make_tensor_value_info('z', TensorProto.FLOAT, (30, 50, 4, 3, 4))])
    ...
    def test_conv_group(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (30, 4, 8, 8, 8)), ('y', TensorProto.FLOAT, (4, 1, 8, 8, 8))], [make_node('Conv', ['x', 'y'], 'z', group=4)], [])
        self._assert_inferred(graph, [make_tensor_value_info('z', TensorProto.FLOAT, (30, 4, 1, 1, 1))])

    def test_conv_only_one_pos(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (30, 4, 5)), ('y', TensorProto.FLOAT, (50, 4, 5))], [make_node('Conv', ['x', 'y'], 'z', strides=[2])], [])
    ...
    def test_conv_partial_missing_shape(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (30, 4, None, 6, 4)), ('y', TensorProto.FLOAT, (50, 4, 3, 3, 3))], [make_node('Conv', ['x', 'y'], 'z', pads=[1, 1, 2, 0, 1, 2])], [])
    ...
    def test_average_pool_with_dilations(self) -> None:
        graph = self._make_graph([('X', TensorProto.FLOAT, (5, 3, 4, 4))], [make_node('AveragePool', ['X'], ['Y'], kernel_shape=[2, 2], dilations=[2, 2])], [])
    ...
    def test_average_pool_with_same_upper_padding_and_stride_and_dilation(self) -> None:
        graph = self._make_graph([('X', TensorProto.FLOAT, (5, 3, 4, 4))], [make_node('AveragePool', ['X'], ['Y'], auto_pad='SAME_UPPER', kernel_shape=[2, 2], strides=[2, 2], dilations=[2, 3])], [])
        self._assert_inferred(graph, [make_tensor_value_info('Y', TensorProto.FLOAT, (5, 3, 2, 2))])
    ...
    def test_identity_sequence(self) -> None:
        graph = self._make_graph([('input1', TensorProto.FLOAT, (2, 3, 4)), ('input2', TensorProto.FLOAT, (2, 3, 4)), ('input3', TensorProto.FLOAT, (2, 5, 4))], [make_node('SequenceConstruct', ['input1', 'input2', 'input3'], ['in_sequence']), make_node('Identity', ['in_sequence'], ['output_sequence'])], [])
        self._assert_inferred(graph, [make_tensor_sequence_value_info('in_sequence', TensorProto.FLOAT, (2, None, 4)), make_tensor_sequence_value_info('output_sequence', TensorProto.FLOAT, (2, None, 4))])
    ...
    def test_add(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (30, 4, 5)), ('y', TensorProto.FLOAT, (30, 4, 5))], [make_node('Add', ['x', 'y'], 'z')], [])
        self._assert_inferred(graph, [make_tensor_value_info('z', TensorProto.FLOAT, (30, 4, 5))])

    def test_pow(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (30, 4, 5)), ('y', TensorProto.FLOAT, (30, 4, 5))], [make_node('Pow', ['x', 'y'], 'z')], [])
        self._assert_inferred(graph, [make_tensor_value_info('z', TensorProto.FLOAT, (30, 4, 5))])
    ...
    def test_bitshift_broadcast_to_first(self) -> None:
        graph = self._make_graph([('x', TensorProto.UINT32, (16, 4, 1)), ('y', TensorProto.UINT32, (1,))], [make_node('BitShift', ['x', 'y'], 'z', direction='RIGHT')], [])
    ...
    def test_bitshift_broadcast_to_second(self) -> None:
        graph = self._make_graph([('x', TensorProto.UINT32, (1,)), ('y', TensorProto.UINT32, (2, 3, 1))], [make_node('BitShift', ['x', 'y'], 'z', direction='RIGHT')], [])
        self._assert_inferred(graph, [make_tensor_value_info('z', TensorProto.UINT32, (2, 3, 1))])
    ...
    def test_random_normal(self) -> None:
        graph = self._make_graph([], [make_node('RandomNormal', [], ['out'], dtype=TensorProto.DOUBLE, shape=(3, 4, 5))], [])
        self._assert_inferred(graph, [make_tensor_value_info('out', TensorProto.DOUBLE, (3, 4, 5))])

    def test_random_normal_like(self) -> None:
        graph = self._make_graph([('X', TensorProto.FLOAT, (2, 3, 4))], [make_node('RandomNormalLike', ['X'], ['out'])], [])
    ...
    def test_bernoulli(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (3, 4))], [make_node('Bernoulli', ['x'], ['out'])], [])
        self._assert_inferred(graph, [make_tensor_value_info('out', TensorProto.FLOAT, (3, 4))])
    ...
    def _logical_binary_op(self, op: str, input_type: TensorProto.DataType) -> None:
        graph = self._make_graph([('x', input_type, (30, 4, 5)), ('y', input_type, (30, 4, 5))], [make_node(op, ['x', 'y'], 'z')], [])
    ...
    def test_logical_not(self) -> None:
        graph = self._make_graph([('x', TensorProto.BOOL, (30, 4, 5))], [make_node('Not', ['x'], 'z')], [])
        self._assert_inferred(graph, [make_tensor_value_info('z', TensorProto.BOOL, (30, 4, 5))])
    ...
    def test_flatten(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (2, 3, 4, 5))], [make_node('Flatten', ['x'], ['z'], axis=2)], [])
    ...
    def test_flatten_unknown_dim(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (2, 'N', 4, 5))], [make_node('Flatten', ['x'], ['z'], axis=2)], [])
        self._assert_inferred(graph, [make_tensor_value_info('z', TensorProto.FLOAT, (None, 20))])

    def test_space_to_depth(self) -> None:
        b = 10
        graph = self._make_graph([('x', TensorProto.FLOAT, (2, 3, 100, 100))], [make_node('SpaceToDepth', ['x'], ['z'], blocksize=b)], [])
    ...
    def _rnn_bidirectional(self, seqlen: int, batchsize: int, inpsize: int, hiddensize: int) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (seqlen, batchsize, inpsize)), ('w', TensorProto.FLOAT, (2, hiddensize, inpsize)), ('r', TensorProto.FLOAT, (2, hiddensize, hiddensize))], [make_node('RNN', ['x', 'w', 'r'], ['all', 'last'], hidden_size=hiddensize, direction='bidirectional')], [])
        self._assert_inferred(graph, [make_tensor_value_info('all', TensorProto.FLOAT, (seqlen, 2, batchsize, hiddensize)), make_tensor_value_info('last', TensorProto.FLOAT, (2, batchsize, hiddensize))])
    ...
    def _lstm_forward(self, seqlen: int, batchsize: int, inpsize: int, hiddensize: int) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (seqlen, batchsize, inpsize)), ('w', TensorProto.FLOAT, (1, 4 * hiddensize, inpsize)), ('r', TensorProto.FLOAT, (1, 4 * hiddensize, hiddensize))], [make_node('LSTM', ['x', 'w', 'r'], ['all', 'hidden', 'last'], hidden_size=hiddensize)], [])
        self._assert_inferred(graph, [make_tensor_value_info('all', TensorProto.FLOAT, (seqlen, 1, batchsize, hiddensize)), make_tensor_value_info('hidden', TensorProto.FLOAT, (1, batchsize, hiddensize)), make_tensor_value_info('last', TensorProto.FLOAT, (1, batchsize, hiddensize))])

    def test_lstm_forward(self) -> None:
        self._lstm_forward(64, 32, 10, 4)

    def test_topk_default_axis(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (3, 4, 5, 10))], [make_node('TopK', ['x', 'k'], ['y', 'z'])], [], initializer=[make_tensor('k', TensorProto.INT64, (1,), (2,))])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.FLOAT, (3, 4, 5, 2)), make_tensor_value_info('z', TensorProto.INT64, (3, 4, 5, 2))])

    def test_topk(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (3, 4, 5, 10))], [make_node('TopK', ['x', 'k'], ['y', 'z'], axis=2)], [], initializer=[make_tensor('k', TensorProto.INT64, (1,), (2,))])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.FLOAT, (3, 4, 2, 10)), make_tensor_value_info('z', TensorProto.INT64, (3, 4, 2, 10))])
    ...
    def test_gemm(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (7, 5)), ('y', TensorProto.FLOAT, (5, 11)), ('z', TensorProto.FLOAT, None)], [make_node('Gemm', ['x', 'y', 'z'], ['out'])], [])
        self._assert_inferred(graph, [make_tensor_value_info('out', TensorProto.FLOAT, (7, 11))])

    def test_gemm_transA(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (5, 7)), ('y', TensorProto.FLOAT, (5, 11)), ('z', TensorProto.FLOAT, None)], [make_node('Gemm', ['x', 'y', 'z'], ['out'], transA=1)], [])
        self._assert_inferred(graph, [make_tensor_value_info('out', TensorProto.FLOAT, (7, 11))])

    def test_gemm_transB(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (7, 5)), ('y', TensorProto.FLOAT, (11, 5)), ('z', TensorProto.FLOAT, None)], [make_node('Gemm', ['x', 'y', 'z'], ['out'], transB=1)], [])
        self._assert_inferred(graph, [make_tensor_value_info('out', TensorProto.FLOAT, (7, 11))])

    def test_gemm_transA_and_transB(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (5, 7)), ('y', TensorProto.FLOAT, (11, 5)), ('z', TensorProto.FLOAT, None)], [make_node('Gemm', ['x', 'y', 'z'], ['out'], transA=1, transB=1)], [])
        self._assert_inferred(graph, [make_tensor_value_info('out', TensorProto.FLOAT, (7, 11))])
    ...
    def test_reduce_op_shape_2_axis_opset18(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (24, 4, 11)), ('axes', TensorProto.INT64, (2,))], [make_node('ReduceL1', ['x', 'axes'], 'y', keepdims=0)], [], initializer=[make_tensor('axes', TensorProto.INT64, (2,), (1, 2))])
        operatorsetid = OperatorSetIdProto()
        operatorsetid.domain = ''
    ...
    def test_reduce_op_shape_keep_dims_opset13(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (24, 4, 11))], [make_node('ReduceL1', 'x', 'y', axes=(1, 2), keepdims=1)], [], initializer=[make_tensor('axes', TensorProto.INT64, (2,), (1, 2))])
        operatorsetid = OperatorSetIdProto()
        operatorsetid.domain = ''
        operatorsetid.version = 13
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.FLOAT, (24, 1, 1))], opset_imports=[operatorsetid])

    def test_reduce_op_shape_keep_dims_opset18(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (24, 4, 11)), ('axes', TensorProto.INT64, (2,))], [make_node('ReduceL1', ['x', 'axes'], 'y', keepdims=1)], [], initializer=[make_tensor('axes', TensorProto.INT64, (2,), (1, 2))])
        operatorsetid = OperatorSetIdProto()
        operatorsetid.domain = ''
        operatorsetid.version = 18
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.FLOAT, (24, 1, 1))], opset_imports=[operatorsetid])
    ...
    def test_reduce_op_shape_default_value(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (24, 4, 11))], [make_node('ReduceL1', 'x', 'y')], [])
    ...
    def test_reduce_op_shape_no_axes_do_not_keep_dims(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (24, 4, 11))], [make_node('ReduceL1', 'x', 'y', keepdims=0)], [])
    ...
    def test_argmax_shape(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (24, 4, 11))], [make_node('ArgMax', 'x', 'y', axis=1, keepdims=1)], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.INT64, (24, 1, 11))])

    def test_argmax_shape_keepdims(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (24, 4, 11))], [make_node('ArgMax', 'x', 'y', axis=0, keepdims=0)], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.INT64, (4, 11))])

    def test_argmax_shape_default_value(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (24, 4, 11))], [make_node('ArgMax', 'x', 'y')], [])
    ...
    def test_argmax_shape_negative_axis(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (24, 4, 11))], [make_node('ArgMax', 'x', 'y', axis=-2)], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.INT64, (24, 1, 11))])

    def test_dropout(self) -> None:
        graph = self._make_graph([('data', TensorProto.FLOAT, (3, 4, 5)), ('ratio', TensorProto.FLOAT, ())], [make_node('Dropout', ['data', 'ratio'], ['out'])], [])
    ...
    def test_batch_norm(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (3, 4, 5, 6, 7)), ('scale', TensorProto.FLOAT, (4,)), ('b', TensorProto.FLOAT, (4,)), ('mean', TensorProto.FLOAT, (4,)), ('var', TensorProto.FLOAT, (4,))], [make_node('BatchNormalization', ['x', 'scale', 'b', 'mean', 'var'], ['out'])], [])
    ...
    def test_split_from_GLU(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (5, 6, 7))], [make_node('Split', ['x'], ['y', 'z'], axis=1, num_outputs=2)], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.FLOAT, (5, 3, 7)), make_tensor_value_info('z', TensorProto.FLOAT, (5, 3, 7))])

    def test_split_uneven_split_2d(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (8, 2))], [make_node('Split', ['x'], ['y', 'z', 'a'], axis=0, num_outputs=3)], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.FLOAT, (3, 2)), make_tensor_value_info('z', TensorProto.FLOAT, (3, 2)), make_tensor_value_info('a', TensorProto.FLOAT, (2, 2))])

    def test_split_uneven_split_3d(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (2, 7, 3))], [make_node('Split', ['x'], ['y', 'z', 'a'], axis=1, num_outputs=3)], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.FLOAT, (2, 3, 3)), make_tensor_value_info('z', TensorProto.FLOAT, (2, 3, 3)), make_tensor_value_info('a', TensorProto.FLOAT, (2, 1, 3))])

    def test_GLU_partial(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (5, 6, 7))], [make_node('Split', ['x'], ['y', 'z'], axis=1, num_outputs=2), make_node('Sigmoid', ['z'], ['a'])], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.FLOAT, (5, 3, 7)), make_tensor_value_info('z', TensorProto.FLOAT, (5, 3, 7)), make_tensor_value_info('a', TensorProto.FLOAT, (5, 3, 7))])
    ...
    def test_logsoftmax_2d(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (4, 5))], [make_node('LogSoftmax', ['x'], 'z')], [])
        self._assert_inferred(graph, [make_tensor_value_info('z', TensorProto.FLOAT, (4, 5))])

    def test_logsoftmax_3d(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (4, 5, 6))], [make_node('LogSoftmax', ['x'], 'z')], [])
        self._assert_inferred(graph, [make_tensor_value_info('z', TensorProto.FLOAT, (4, 5, 6))])

    def test_logsoftmax_3d_negative_axis(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (4, 5, 6))], [make_node('LogSoftmax', ['x'], 'z', axis=-1)], [])
        self._assert_inferred(graph, [make_tensor_value_info('z', TensorProto.FLOAT, (4, 5, 6))])

    def test_maxpool(self) -> None:
        graph = self._make_graph([('X', TensorProto.FLOAT, (5, 3, 4, 4))], [make_node('MaxPool', ['X'], ['Y'], kernel_shape=[2, 2])], [])
        self._assert_inferred(graph, [make_tensor_value_info('Y', TensorProto.FLOAT, (5, 3, 3, 3))])

    def test_maxpool_with_indices(self) -> None:
        graph = self._make_graph([('X', TensorProto.FLOAT, (5, 3, 4, 4))], [make_node('MaxPool', ['X'], ['Y', 'Z'], kernel_shape=[2, 2])], [])
        self._assert_inferred(graph, [make_tensor_value_info('Y', TensorProto.FLOAT, (5, 3, 3, 3)), make_tensor_value_info('Z', TensorProto.INT64, (5, 3, 3, 3))])

    def test_maxpool_3D(self) -> None:
        graph = self._make_graph([('X', TensorProto.FLOAT, (5, 3, 4, 4, 4))], [make_node('MaxPool', ['X'], ['Y'], kernel_shape=[2, 2, 2])], [])
        self._assert_inferred(graph, [make_tensor_value_info('Y', TensorProto.FLOAT, (5, 3, 3, 3, 3))])

    def test_maxpool_with_padding(self) -> None:
        graph = self._make_graph([('X', TensorProto.FLOAT, (5, 3, 4, 4))], [make_node('MaxPool', ['X'], ['Y'], kernel_shape=[2, 2], pads=[1, 1, 2, 2])], [])
    ...
    def test_maxpool_with_padding_and_stride(self) -> None:
        graph = self._make_graph([('X', TensorProto.FLOAT, (5, 3, 4, 4))], [make_node('MaxPool', ['X'], ['Y'], kernel_shape=[2, 2], pads=[1, 1, 2, 2], strides=[2, 2])], [])
        self._assert_inferred(graph, [make_tensor_value_info('Y', TensorProto.FLOAT, (5, 3, 3, 3))])
    ...
    def test_maxpool_with_ceil_mode(self) -> None:
        graph = self._make_graph([('X', TensorProto.FLOAT, (32, 288, 35, 35))], [make_node('MaxPool', ['X'], ['Y'], kernel_shape=[2, 2], strides=[2, 2], ceil_mode=True)], [])
        self._assert_inferred(graph, [make_tensor_value_info('Y', TensorProto.FLOAT, (32, 288, 18, 18))])

    def test_maxpool_ceil(self) -> None:
        graph = self._make_graph([('X', TensorProto.FLOAT, (1, 1, 4, 4))], [make_node('MaxPool', ['X'], ['Y'], kernel_shape=[3, 3], strides=[2, 2], ceil_mode=True)], [])
        self._assert_inferred(graph, [make_tensor_value_info('Y', TensorProto.FLOAT, (1, 1, 2, 2))])

    def test_maxpool_with_dilations(self) -> None:
        graph = self._make_graph([('X', TensorProto.FLOAT, (5, 3, 4, 4))], [make_node('MaxPool', ['X'], ['Y'], kernel_shape=[2, 2], dilations=[2, 2])], [])
        self._assert_inferred(graph, [make_tensor_value_info('Y', TensorProto.FLOAT, (5, 3, 2, 2))])

    def test_maxpool_with_same_upper_padding_and_stride(self) -> None:
        graph = self._make_graph([('X', TensorProto.FLOAT, (5, 3, 4, 4))], [make_node('MaxPool', ['X'], ['Y'], auto_pad='SAME_UPPER', kernel_shape=[2, 2], strides=[2, 2])], [])
        self._assert_inferred(graph, [make_tensor_value_info('Y', TensorProto.FLOAT, (5, 3, 2, 2))])

    def test_maxpool_with_same_upper_padding_and_stride_and_dilation(self) -> None:
        graph = self._make_graph([('X', TensorProto.FLOAT, (5, 3, 4, 4))], [make_node('MaxPool', ['X'], ['Y'], auto_pad='SAME_UPPER', kernel_shape=[2, 2], strides=[2, 2], dilations=[2, 3])], [])
        self._assert_inferred(graph, [make_tensor_value_info('Y', TensorProto.FLOAT, (5, 3, 2, 2))])
    ...
    def test_maxpool_with_same_lower_padding_and_stride(self) -> None:
        graph = self._make_graph([('X', TensorProto.FLOAT, (5, 3, 9, 9))], [make_node('MaxPool', ['X'], ['Y'], auto_pad='SAME_LOWER', kernel_shape=[2, 2], strides=[2, 2])], [])
    ...
    def test_maxpool_with_same_lower_padding_and_stride_and_dilation(self) -> None:
        graph = self._make_graph([('X', TensorProto.FLOAT, (5, 3, 9, 9))], [make_node('MaxPool', ['X'], ['Y'], auto_pad='SAME_LOWER', kernel_shape=[2, 2], strides=[2, 2], dilations=[2, 3])], [])
        self._assert_inferred(graph, [make_tensor_value_info('Y', TensorProto.FLOAT, (5, 3, 5, 5))])

    def test_maxpool_with_same_lower_padding_and_big_stride(self) -> None:
        graph = self._make_graph([('X', TensorProto.FLOAT, (5, 3, 4, 4))], [make_node('MaxPool', ['X'], ['Y'], auto_pad='SAME_LOWER', kernel_shape=[2, 2], strides=[4, 4])], [])
        self._assert_inferred(graph, [make_tensor_value_info('Y', TensorProto.FLOAT, (5, 3, 1, 1))])

    def test_averagepool(self) -> None:
        graph = self._make_graph([('X', TensorProto.FLOAT, (5, 3, 4, 4))], [make_node('AveragePool', ['X'], ['Y'], kernel_shape=[2, 2])], [])
        self._assert_inferred(graph, [make_tensor_value_info('Y', TensorProto.FLOAT, (5, 3, 3, 3))])

    def test_averagepool_3D(self) -> None:
        graph = self._make_graph([('X', TensorProto.FLOAT, (5, 3, 4, 4, 4))], [make_node('AveragePool', ['X'], ['Y'], kernel_shape=[2, 2, 2])], [])
        self._assert_inferred(graph, [make_tensor_value_info('Y', TensorProto.FLOAT, (5, 3, 3, 3, 3))])

    def test_averagepool_with_padding(self) -> None:
        graph = self._make_graph([('X', TensorProto.FLOAT, (5, 3, 4, 4))], [make_node('AveragePool', ['X'], ['Y'], kernel_shape=[2, 2], pads=[1, 1, 2, 2])], [])
        self._assert_inferred(graph, [make_tensor_value_info('Y', TensorProto.FLOAT, (5, 3, 6, 6))])

    def test_averagepool_with_padding_and_stride(self) -> None:
        graph = self._make_graph([('X', TensorProto.FLOAT, (5, 3, 4, 4))], [make_node('AveragePool', ['X'], ['Y'], kernel_shape=[2, 2], pads=[1, 1, 2, 2], strides=[2, 2])], [])
        self._assert_inferred(graph, [make_tensor_value_info('Y', TensorProto.FLOAT, (5, 3, 3, 3))])
    ...
    def test_lppool(self) -> None:
        graph = self._make_graph([('X', TensorProto.FLOAT, (5, 3, 4, 4))], [make_node('LpPool', ['X'], ['Y'], kernel_shape=[2, 2])], [])
        self._assert_inferred(graph, [make_tensor_value_info('Y', TensorProto.FLOAT, (5, 3, 3, 3))])
    ...
    def test_lppool_with_padding(self) -> None:
        graph = self._make_graph([('X', TensorProto.FLOAT, (5, 3, 4, 4))], [make_node('LpPool', ['X'], ['Y'], kernel_shape=[2, 2], pads=[1, 1, 2, 2])], [])
        self._assert_inferred(graph, [make_tensor_value_info('Y', TensorProto.FLOAT, (5, 3, 6, 6))])

    def test_lppool_with_padding_and_stride(self) -> None:
        graph = self._make_graph([('X', TensorProto.FLOAT, (5, 3, 4, 4))], [make_node('LpPool', ['X'], ['Y'], kernel_shape=[2, 2], pads=[1, 1, 2, 2], strides=[2, 2])], [])
        self._assert_inferred(graph, [make_tensor_value_info('Y', TensorProto.FLOAT, (5, 3, 3, 3))])

    def test_lppool_with_dilations(self) -> None:
        graph = self._make_graph([('X', TensorProto.FLOAT, (5, 3, 4, 4))], [make_node('LpPool', ['X'], ['Y'], kernel_shape=[2, 2], dilations=[2, 2])], [])
        self._assert_inferred(graph, [make_tensor_value_info('Y', TensorProto.FLOAT, (5, 3, 2, 2))])

    def test_lppool_with_same_upper_padding_and_stride_and_dilation(self) -> None:
        graph = self._make_graph([('X', TensorProto.FLOAT, (5, 3, 4, 4))], [make_node('LpPool', ['X'], ['Y'], auto_pad='SAME_UPPER', kernel_shape=[2, 2], strides=[2, 2], dilations=[2, 3])], [])
        self._assert_inferred(graph, [make_tensor_value_info('Y', TensorProto.FLOAT, (5, 3, 2, 2))])
    ...
    def test_global_maxpool(self) -> None:
        graph = self._make_graph([('X', TensorProto.FLOAT, (5, 3, 4, 4))], [make_node('GlobalMaxPool', ['X'], ['Y'])], [])
        self._assert_inferred(graph, [make_tensor_value_info('Y', TensorProto.FLOAT, (5, 3, 1, 1))])

    def test_global_averagepool(self) -> None:
        graph = self._make_graph([('X', TensorProto.FLOAT, (5, 3, 4, 4))], [make_node('GlobalAveragePool', ['X'], ['Y'])], [])
        self._assert_inferred(graph, [make_tensor_value_info('Y', TensorProto.FLOAT, (5, 3, 1, 1))])

    def test_global_lppool(self) -> None:
        graph = self._make_graph([('X', TensorProto.FLOAT, (5, 3, 4, 4))], [make_node('GlobalLpPool', ['X'], ['Y'])], [])
        self._assert_inferred(graph, [make_tensor_value_info('Y', TensorProto.FLOAT, (5, 3, 1, 1))])

    def test_conv_transpose(self) -> None:
        graph = self._make_graph([('X', TensorProto.FLOAT, (25, 48, 16, 16)), ('W', TensorProto.FLOAT, (48, 32, 3, 3))], [make_node('ConvTranspose', ['X', 'W'], 'Y', strides=[2, 2])], [])
    ...
    def test_conv_transpose_with_pads(self) -> None:
        graph = self._make_graph([('X', TensorProto.FLOAT, (25, 48, 16, 16)), ('W', TensorProto.FLOAT, (48, 32, 3, 3))], [make_node('ConvTranspose', ['X', 'W'], 'Y', strides=[2, 2], pads=[1, 1, 2, 2])], [])
    ...
    def test_conv_transpose_with_output_shape(self) -> None:
        graph = self._make_graph([('X', TensorProto.FLOAT, (25, 48, 16, 16)), ('W', TensorProto.FLOAT, (48, 32, 3, 3))], [make_node('ConvTranspose', ['X', 'W'], 'Y', strides=[2, 2], pads=[1, 1, 2, 2], output_shape=[36, 36])], [])
    ...
    def test_conv_transpose_with_dilations(self) -> None:
        graph = self._make_graph([('X', TensorProto.FLOAT, (25, 48, 16, 16)), ('W', TensorProto.FLOAT, (48, 32, 3, 3))], [make_node('ConvTranspose', ['X', 'W'], 'Y', strides=[2, 2], pads=[1, 1, 2, 2], dilations=[3, 3])], [])
    ...
    def test_conv_transpose_with_group_and_output_shape(self) -> None:
        graph = self._make_graph([('X', TensorProto.FLOAT, (25, 48, 16, 16)), ('W', TensorProto.FLOAT, (48, 32, 3, 3))], [make_node('ConvTranspose', ['X', 'W'], 'Y', strides=[2, 2], pads=[1, 1, 2, 2], group=2, output_shape=[36, 36])], [])
    ...
    def test_scan(self) -> None:
        ...
        graph = self._make_graph([('loop_state_orig', TensorProto.FLOAT, (batch_size, loop_state_size)), ('scan_input', TensorProto.FLOAT, (batch_size, seq_len, input_size))], [make_node('Scan', ['', 'loop_state_orig', 'scan_input'], ['loop_state_final', 'scan_output'], num_scan_inputs=1, body=subgraph)], [])
        self._assert_inferred(graph, [make_tensor_value_info('loop_state_final', TensorProto.FLOAT, (batch_size, loop_state_size)), make_tensor_value_info('scan_output', TensorProto.FLOAT, (batch_size, seq_len, input_size))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, 8)])
    ...
    def test_scan_opset9(self) -> None:
        ...
        output_value_infos = [make_tensor_value_info('loop_state_out', TensorProto.UNDEFINED, None), make_tensor_value_info('output', TensorProto.UNDEFINED, None)]
        subgraph = helper.make_graph([make_node('Identity', ['loop_state_in'], ['loop_state_out']), make_node('Identity', ['input'], ['output'])], 'subgraph', input_value_infos, output_value_infos)
        graph = self._make_graph([('loop_state_orig', TensorProto.FLOAT, (loop_state_size,)), ('scan_input', TensorProto.FLOAT, (seq_len, input_size))], [make_node('Scan', ['loop_state_orig', 'scan_input'], ['loop_state_final', 'scan_output'], num_scan_inputs=1, body=subgraph)], [])
        self._assert_inferred(graph, [make_tensor_value_info('loop_state_final', TensorProto.FLOAT, (loop_state_size,)), make_tensor_value_info('scan_output', TensorProto.FLOAT, (seq_len, input_size))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, 9)])
    ...
    def test_scan_opset9_output_axes(self) -> None:
        ...
        output_value_infos = [make_tensor_value_info('loop_state_out', TensorProto.UNDEFINED, None), make_tensor_value_info('output', TensorProto.UNDEFINED, None)]
        subgraph = helper.make_graph([make_node('Identity', ['loop_state_in'], ['loop_state_out']), make_node('Identity', ['input'], ['output'])], 'subgraph', input_value_infos, output_value_infos)
        graph = self._make_graph([('loop_state_orig', TensorProto.FLOAT, (loop_state_size,)), ('scan_input', TensorProto.FLOAT, (axis_0_len, seq_len, input_size))], [make_node('Scan', ['loop_state_orig', 'scan_input'], ['loop_state_final', 'scan_output'], num_scan_inputs=1, body=subgraph, scan_input_axes=[1], scan_output_axes=[1])], [])
        self._assert_inferred(graph, [make_tensor_value_info('loop_state_final', TensorProto.FLOAT, (loop_state_size,)), make_tensor_value_info('scan_output', TensorProto.FLOAT, (axis_0_len, seq_len, input_size))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, 9)])
    ...
    def test_scan_opset9_negative_axes(self) -> None:
        ...
        output_value_infos = [make_tensor_value_info('loop_state_out', TensorProto.UNDEFINED, None), make_tensor_value_info('output', TensorProto.UNDEFINED, None)]
        subgraph = helper.make_graph([make_node('Identity', ['loop_state_in'], ['loop_state_out']), make_node('Identity', ['input'], ['output'])], 'subgraph', input_value_infos, output_value_infos)
        graph = self._make_graph([('loop_state_orig', TensorProto.FLOAT, (loop_state_size,)), ('scan_input', TensorProto.FLOAT, (axis_0_len, seq_len, input_size))], [make_node('Scan', ['loop_state_orig', 'scan_input'], ['loop_state_final', 'scan_output'], num_scan_inputs=1, body=subgraph, scan_input_axes=[-2], scan_output_axes=[-2])], [])
        self._assert_inferred(graph, [make_tensor_value_info('loop_state_final', TensorProto.FLOAT, (loop_state_size,)), make_tensor_value_info('scan_output', TensorProto.FLOAT, (axis_0_len, seq_len, input_size))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, 9)])

    def test_if_ver1(self) -> None:
        then_subgraph = helper.make_graph([make_node('Add', ['current_value', 'add_value'], ['then_output'])], 'then_subgraph', [], [make_tensor_value_info('then_output', TensorProto.UNDEFINED, None)])
        else_subgraph = helper.make_graph([make_node('Sub', ['current_value', 'sub_value'], ['else_output'])], 'else_subgraph', [], [make_tensor_value_info('else_output', TensorProto.UNDEFINED, None)])
        graph = self._make_graph([('cond', TensorProto.BOOL, (1,)), ('current_value', TensorProto.FLOAT, (1,)), ('add_value', TensorProto.FLOAT, (1,)), ('sub_value', TensorProto.FLOAT, (1,))], [make_node('If', ['cond'], ['if_output'], then_branch=then_subgraph, else_branch=else_subgraph)], [])
        self._assert_inferred(graph, [make_tensor_value_info('if_output', TensorProto.FLOAT, (1,))], opset_imports=[make_opsetid(ONNX_DOMAIN, 10)])
    ...
    def test_maxunpool_shape_with_output_shape(self) -> None:
        graph = self._make_graph([('xT', TensorProto.FLOAT, (1, 1, 2, 2)), ('xI', TensorProto.FLOAT, (1, 1, 2, 2)), ('output_shape', TensorProto.FLOAT, (4,))], [make_node('MaxUnpool', ['xT', 'xI', 'output_shape'], 'Y', kernel_shape=[2, 2], strides=[2, 2])], [make_tensor_value_info('Y', TensorProto.FLOAT, None)])
        self._assert_inferred(graph, [make_tensor_value_info('Y', TensorProto.FLOAT, None)])

    def test_onehot_without_axis(self) -> None:
        graph = self._make_graph([('indices', TensorProto.INT64, (2, 2)), ('depth', TensorProto.INT64, ()), ('values', TensorProto.FLOAT, (2,))], [make_node('OneHot', ['indices', 'depth', 'values'], 'Y')], [])
    ...
    def test_loop(self) -> None:
        input_value_infos = [make_tensor_value_info('iter_num_in', TensorProto.INT64, (1,)), make_tensor_value_info('cond_in', TensorProto.UNDEFINED, None), make_tensor_value_info('loop_state_in', TensorProto.UNDEFINED, ())]
        output_value_infos = [make_tensor_value_info('cond_out', TensorProto.UNDEFINED, None), make_tensor_value_info('loop_state_out', TensorProto.UNDEFINED, None), make_tensor_value_info('output', TensorProto.FLOAT, (3,))]
        subgraph = helper.make_graph([make_node('Identity', ['cond_in'], ['cond_out']), make_node('Identity', ['loop_state_in'], ['loop_state_out']), make_node('Identity', ['outer_scope_input'], ['output'])], 'subgraph', input_value_infos, output_value_infos)
        graph = self._make_graph([('max_trip_count', TensorProto.INT64, (1,)), ('cond_orig', TensorProto.FLOAT, (1,)), ('loop_state_orig', TensorProto.FLOAT, (2,)), ('outer_scope_input', TensorProto.FLOAT, (3,))], [make_node('Loop', ['max_trip_count', 'cond_orig', 'loop_state_orig'], ['loop_state_final', 'loop_output'], body=subgraph)], [])
        self._assert_inferred(graph, [make_tensor_value_info('loop_state_final', TensorProto.FLOAT, None), make_tensor_value_info('loop_output', TensorProto.FLOAT, (None, 3))])
    ...
    def test_constantofshape_without_input_shape(self) -> None:
        graph = self._make_graph([('shape', TensorProto.INT64, (3,))], [make_node('ConstantOfShape', ['shape'], ['y'], value=make_tensor('value', TensorProto.UINT8, (1,), (2,)))], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.UINT8, (None, None, None))])
    ...
    def test_convinteger_strides(self) -> None:
        graph = self._make_graph([('x', TensorProto.INT8, (30, 4, 8, 8, 8)), ('y', TensorProto.INT8, (50, 4, 3, 3, 3)), ('x_zero_point', TensorProto.UINT8, ()), ('y_zero_point', TensorProto.UINT8, ())], [make_node('ConvInteger', ['x', 'y', 'x_zero_point', 'y_zero_point'], 'z', strides=[1, 2, 3])], [])
        self._assert_inferred(graph, [make_tensor_value_info('z', TensorProto.INT32, (30, 50, 6, 3, 2))])

    def test_convineteger_pads(self) -> None:
        graph = self._make_graph([('x', TensorProto.UINT8, (30, 4, 7, 6, 4)), ('y', TensorProto.INT8, (50, 4, 3, 3, 3))], [make_node('ConvInteger', ['x', 'y'], 'z', pads=[1, 1, 2, 0, 1, 2])], [])
    ...
    def test_convineteger_partial_missing_shape(self) -> None:
        graph = self._make_graph([('x', TensorProto.UINT8, (30, 4, None, 6, 4)), ('y', TensorProto.UINT8, (50, 4, 3, 3, 3)), ('x_zero_point', TensorProto.UINT8, ()), ('y_zero_point', TensorProto.UINT8, ())], [make_node('ConvInteger', ['x', 'y', 'x_zero_point', 'y_zero_point'], 'z', pads=[1, 1, 2, 0, 1, 2])], [])
        self._assert_inferred(graph, [make_tensor_value_info('z', TensorProto.INT32, (30, 50, None, 6, 6))])
    ...
    def test_qlinearconv(self) -> None:
        graph = self._make_graph([('x', TensorProto.UINT8, (3, 4, 5, 6, 7)), ('x_scale', TensorProto.FLOAT, ()), ('x_zero_point', TensorProto.UINT8, ()), ('w', TensorProto.UINT8, (5, 4, 2, 4, 3)), ('w_scale', TensorProto.FLOAT, ()), ('w_zero_point', TensorProto.UINT8, ()), ('y_scale', TensorProto.FLOAT, ()), ('y_zero_point', TensorProto.UINT8, ())], [make_node('QLinearConv', ['x', 'x_scale', 'x_zero_point', 'w', 'w_scale', 'w_zero_point', 'y_scale', 'y_zero_point'], 'y', pads=[0, 1, 1, 0, 0, 1], dilations=[1, 2, 2], strides=[1, 1, 2])], [])
    ...
    def test_qlinearconv_strides(self) -> None:
        graph = self._make_graph([('x', TensorProto.INT8, (30, 4, 8, 8, 8)), ('x_scale', TensorProto.FLOAT, ()), ('x_zero_point', TensorProto.INT8, ()), ('w', TensorProto.INT8, (50, 4, 3, 3, 3)), ('w_scale', TensorProto.FLOAT, ()), ('w_zero_point', TensorProto.INT8, ()), ('y_scale', TensorProto.FLOAT, ()), ('y_zero_point', TensorProto.INT8, ())], [make_node('QLinearConv', ['x', 'x_scale', 'x_zero_point', 'w', 'w_scale', 'w_zero_point', 'y_scale', 'y_zero_point'], 'y', strides=[1, 2, 3])], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.INT8, (30, 50, 6, 3, 2))])

    def test_qlinearconv_pads(self) -> None:
        graph = self._make_graph([('x', TensorProto.UINT8, (30, 4, 7, 6, 4)), ('x_scale', TensorProto.FLOAT, ()), ('x_zero_point', TensorProto.UINT8, ()), ('w', TensorProto.INT8, (50, 4, 3, 3, 3)), ('w_scale', TensorProto.FLOAT, ()), ('w_zero_point', TensorProto.INT8, ()), ('y_scale', TensorProto.FLOAT, ()), ('y_zero_point', TensorProto.UINT8, ())], [make_node('QLinearConv', ['x', 'x_scale', 'x_zero_point', 'w', 'w_scale', 'w_zero_point', 'y_scale', 'y_zero_point'], 'y', pads=[1, 1, 2, 0, 1, 2])], [])
    ...
    def test_qlinearconv_partial_missing_shape(self) -> None:
        graph = self._make_graph([('x', TensorProto.UINT8, (30, 4, None, 6, 4)), ('x_scale', TensorProto.FLOAT, ()), ('x_zero_point', TensorProto.UINT8, ()), ('w', TensorProto.UINT8, (50, 4, 3, 3, 3)), ('w_scale', TensorProto.FLOAT, ()), ('w_zero_point', TensorProto.UINT8, ()), ('y_scale', TensorProto.FLOAT, ()), ('y_zero_point', TensorProto.UINT8, ())], [make_node('QLinearConv', ['x', 'x_scale', 'x_zero_point', 'w', 'w_scale', 'w_zero_point', 'y_scale', 'y_zero_point'], 'y', pads=[1, 1, 2, 0, 1, 2])], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.UINT8, (30, 50, None, 6, 6))])

    def test_qlinearconv_partial_missing_weight_shape(self) -> None:
        graph = self._make_graph([('x', TensorProto.UINT8, (30, 4, 7, 6, 4)), ('x_scale', TensorProto.FLOAT, ()), ('x_zero_point', TensorProto.UINT8, ()), ('w', TensorProto.UINT8, (50, 4, None, 3, 3)), ('w_scale', TensorProto.FLOAT, ()), ('w_zero_point', TensorProto.UINT8, ()), ('y_scale', TensorProto.FLOAT, ()), ('y_zero_point', TensorProto.UINT8, ())], [make_node('QLinearConv', ['x', 'x_scale', 'x_zero_point', 'w', 'w_scale', 'w_zero_point', 'y_scale', 'y_zero_point'], 'y', pads=[1, 1, 2, 0, 1, 2])], [])
    ...
    def test_quantizelinear_default_zp(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (30, 4, 5)), ('y_scale', TensorProto.FLOAT, ())], [make_node('QuantizeLinear', ['x', 'y_scale'], ['y'])], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.UINT8, (30, 4, 5))])
    ...
    def test_quantizelinear_output_dtype(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (3, 4, 5)), ('y_scale', TensorProto.FLOAT, ())], [make_node('QuantizeLinear', ['x', 'y_scale'], ['y'], output_dtype=TensorProto.UINT4)], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.UINT4, (3, 4, 5))])

    def test_quantizelinear_zp_output_dtype(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (3, 4, 5)), ('y_scale', TensorProto.FLOAT, ()), ('y_zero_point', TensorProto.UINT16, ())], [make_node('QuantizeLinear', ['x', 'y_scale', 'y_zero_point'], ['y'], output_dtype=TensorProto.UINT16)], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.UINT16, (3, 4, 5))])
    ...
    def test_dequantizelinear(self, elem_type) -> None:
        graph = self._make_graph([('x', TensorProto.UINT8, (30, 4, 5)), ('x_scale', elem_type, ()), ('x_zero_point', TensorProto.UINT8, ())], [make_node('DequantizeLinear', ['x', 'x_scale', 'x_zero_point'], ['y'])], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', elem_type, (30, 4, 5))])
    ...
    def test_reversesequence(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (4, 5, 6)), ('sequence_lens', TensorProto.INT64, (5,))], [make_node('ReverseSequence', ['x', 'sequence_lens'], ['y'])], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.FLOAT, (4, 5, 6))])

    def test_unique_without_axis(self) -> None:
        graph = self._make_graph([('X', TensorProto.FLOAT, (2, 4, 2))], [make_node('Unique', ['X'], ['Y', 'indices', 'inverse_indices', 'counts'])], [])
        self._assert_inferred(graph, [make_tensor_value_info('Y', TensorProto.FLOAT, (None,)), make_tensor_value_info('indices', TensorProto.INT64, (None,)), make_tensor_value_info('inverse_indices', TensorProto.INT64, (None,)), make_tensor_value_info('counts', TensorProto.INT64, (None,))])
    ...
    def test_det(self) -> None:
        graph = self._make_graph([('X', TensorProto.FLOAT, (3, 3))], [make_node('Det', ['X'], ['Y'])], [])
        self._assert_inferred(graph, [make_tensor_value_info('Y', TensorProto.FLOAT, ())])
        graph = self._make_graph([('X', TensorProto.FLOAT, (4, 5, 6, 7, 7))], [make_node('Det', ['X'], ['Y'])], [])
        self._assert_inferred(graph, [make_tensor_value_info('Y', TensorProto.FLOAT, (4, 5, 6))])
    ...
    def test_tile_raw_input_data(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (4, 5, 6)), ('repeats', TensorProto.INT64, (3,))], [make_node('Tile', ['x', 'repeats'], ['y'])], [], initializer=[make_tensor('repeats', TensorProto.INT64, (3,), vals=np.array([1, 2, 3], dtype='<i8').tobytes(), raw=True)])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.FLOAT, (4, 10, 18))])
    ...
    def test_linearclassifier_1D_input(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (5,))], [make_node('LinearClassifier', ['x'], ['y', 'z'], domain=ONNX_ML_DOMAIN, coefficients=[0.0008, -0.0008], intercepts=[2.0, 2.0], classlabels_ints=[1, 2])], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.INT64, (1,)), make_tensor_value_info('z', TensorProto.FLOAT, (1, 2))], opset_imports=[make_opsetid(ONNX_ML_DOMAIN, 1), make_opsetid(ONNX_DOMAIN, 11)])
    ...
    def test_roialign_num_rois(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, ('N', 'C', 'H', 'W')), ('rois', TensorProto.FLOAT, ('num_rois', 4)), ('batch_indices', TensorProto.INT64, (15,))], [make_node('RoiAlign', ['x', 'rois', 'batch_indices'], ['y'])], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.FLOAT, (15, 'C', 1, 1))])
    ...
    def make_sparse(self, shape: Sequence[int], values: Sequence[int], indices_shape: Sequence[int], indices: Sequence[int]) -> SparseTensorProto:
    ...
    def test_constant_value_float(self) -> None:
        graph = self._make_graph([], [make_node('Constant', [], ['y'], value_float=1.42)], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.FLOAT, [])])
    ...
    def test_constant_value_string(self) -> None:
        graph = self._make_graph([], [make_node('Constant', [], ['y'], value_string='String value')], [])
    ...
    def test_range(self) -> None:
        graph = self._make_graph([('start', TensorProto.FLOAT, ()), ('limit', TensorProto.FLOAT, ()), ('delta', TensorProto.FLOAT, ())], [make_node('Range', ['start', 'limit', 'delta'], ['output'])], [], initializer=[make_tensor('start', TensorProto.FLOAT, (), (1,)), make_tensor('limit', TensorProto.FLOAT, (), (5,)), make_tensor('delta', TensorProto.FLOAT, (), (2,))])
    ...
    def test_gathernd(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (4, 5, 6)), ('indices', TensorProto.INT64, (2,))], [make_node('GatherND', ['x', 'indices'], ['y'])], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.FLOAT, (6,))])

    def test_gathernd_batchdim_1(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (2, 2, 2)), ('indices', TensorProto.INT64, (2, 1))], [make_node('GatherND', ['x', 'indices'], ['y'], batch_dims=1)], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.FLOAT, (2, 2))])

    def test_cumsum(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (2, 3)), ('axis', TensorProto.FLOAT, (1,))], [make_node('CumSum', ['x', 'axis'], 'z')], [])
    ...
    def test_sequence_construct(self) -> None:
        graph = self._make_graph([('input1', TensorProto.FLOAT, (2, 3, 4)), ('input2', TensorProto.FLOAT, (2, 3, 4)), ('input3', TensorProto.FLOAT, (2, 3, 4))], [make_node('SequenceConstruct', ['input1', 'input2', 'input3'], ['output_sequence'])], [])
    ...
    def test_sequence_construct_diff_rank(self) -> None:
        graph = self._make_graph([('input1', TensorProto.FLOAT, (2, 3, 4)), ('input2', TensorProto.FLOAT, (2, 3)), ('input3', TensorProto.FLOAT, (2, 3))], [make_node('SequenceConstruct', ['input1', 'input2', 'input3'], ['output_sequence'])], [])
        self._assert_inferred(graph, [make_tensor_sequence_value_info('output_sequence', TensorProto.FLOAT, None)])
    ...
    def test_sequence_insert(self) -> None:
        graph = self._make_graph([('input1', TensorProto.FLOAT, (2, 3, 4)), ('input2', TensorProto.FLOAT, (2, 3, 4)), ('input3', TensorProto.FLOAT, (2, 3, 4)), ('input4', TensorProto.FLOAT, (2, 3, 4))], [make_node('SequenceConstruct', ['input1', 'input2', 'input3'], ['in_sequence']), make_node('SequenceInsert', ['in_sequence', 'input4'], ['output_sequence'])], [])
        self._assert_inferred(graph, [make_tensor_sequence_value_info('in_sequence', TensorProto.FLOAT, (2, 3, 4)), make_tensor_sequence_value_info('output_sequence', TensorProto.FLOAT, (2, 3, 4))])

    def test_sequence_insert_diff_rank(self) -> None:
        graph = self._make_graph([('input1', TensorProto.FLOAT, (2, 3, 4)), ('input2', TensorProto.FLOAT, (2, 3, 4)), ('input3', TensorProto.FLOAT, (2, 3, 4)), ('input4', TensorProto.FLOAT, (2, 3))], [make_node('SequenceConstruct', ['input1', 'input2', 'input3'], ['in_sequence']), make_node('SequenceInsert', ['in_sequence', 'input4'], ['output_sequence'])], [])
        self._assert_inferred(graph, [make_tensor_sequence_value_info('in_sequence', TensorProto.FLOAT, (2, 3, 4)), make_tensor_sequence_value_info('output_sequence', TensorProto.FLOAT, None)])

    def test_sequence_insert_diff_shape(self) -> None:
        graph = self._make_graph([('input1', TensorProto.FLOAT, (2, 3, 4)), ('input2', TensorProto.FLOAT, (2, 3, 4)), ('input3', TensorProto.FLOAT, (2, 5, 4)), ('input4', TensorProto.FLOAT, (2, 5, 2))], [make_node('SequenceConstruct', ['input1', 'input2', 'input3'], ['in_sequence']), make_node('SequenceInsert', ['in_sequence', 'input4'], ['output_sequence'])], [])
        self._assert_inferred(graph, [make_tensor_sequence_value_info('in_sequence', TensorProto.FLOAT, (2, None, 4)), make_tensor_sequence_value_info('output_sequence', TensorProto.FLOAT, (2, None, None))])

    def test_sequence_at(self) -> None:
        graph = self._make_graph([('input1', TensorProto.FLOAT, (2, 3, 4)), ('input2', TensorProto.FLOAT, (2, 3, 4)), ('input3', TensorProto.FLOAT, (2, 3, 4)), ('ind', TensorProto.INT64, ())], [make_node('SequenceConstruct', ['input1', 'input2', 'input3'], ['in_sequence']), make_node('SequenceAt', ['in_sequence', 'ind'], ['output'])], [])
        self._assert_inferred(graph, [make_tensor_sequence_value_info('in_sequence', TensorProto.FLOAT, (2, 3, 4)), make_tensor_value_info('output', TensorProto.FLOAT, (2, 3, 4))])
    ...
    def test_sequence_at_unknown_dim_size(self) -> None:
        graph = self._make_graph([('input1', TensorProto.FLOAT, (2, 3, 4)), ('input2', TensorProto.FLOAT, (2, 3, 5)), ('input3', TensorProto.FLOAT, (2, 3, 4)), ('ind', TensorProto.INT64, ())], [make_node('SequenceConstruct', ['input1', 'input2', 'input3'], ['in_sequence']), make_node('SequenceAt', ['in_sequence', 'ind'], ['output'])], [])
        self._assert_inferred(graph, [make_tensor_sequence_value_info('in_sequence', TensorProto.FLOAT, (2, 3, None)), make_tensor_value_info('output', TensorProto.FLOAT, (2, 3, None))])

    def test_sequence_erase(self) -> None:
        graph = self._make_graph([('input1', TensorProto.FLOAT, (2, 3, 4)), ('input2', TensorProto.FLOAT, (2, 3, 4)), ('input3', TensorProto.FLOAT, (2, 3, 4)), ('ind', TensorProto.INT64, ())], [make_node('SequenceConstruct', ['input1', 'input2', 'input3'], ['in_sequence']), make_node('SequenceErase', ['in_sequence', 'ind'], ['output_sequence'])], [])
        self._assert_inferred(graph, [make_tensor_sequence_value_info('in_sequence', TensorProto.FLOAT, (2, 3, 4)), make_tensor_sequence_value_info('output_sequence', TensorProto.FLOAT, (2, 3, 4))])
    ...
    def test_split_to_sequence(self) -> None:
        graph = self._make_graph([('input', TensorProto.FLOAT, (6, 4)), ('split', TensorProto.INT32, (2,))], [make_node('SplitToSequence', ['input', 'split'], ['output_sequence'])], [], initializer=[make_tensor('split', TensorProto.INT32, (2,), (3, 3))])
    ...
    def test_split_to_sequence_scalar(self) -> None:
        graph = self._make_graph([('input', TensorProto.FLOAT, (6, 4)), ('split', TensorProto.INT32, ())], [make_node('SplitToSequence', ['input', 'split'], ['output_sequence'])], [], initializer=[make_tensor('split', TensorProto.INT32, (), (2,))])
        self._assert_inferred(graph, [make_tensor_sequence_value_info('output_sequence', TensorProto.FLOAT, (2, 4))])

    def test_split_to_sequence_keepdims(self) -> None:
        graph = self._make_graph([('input', TensorProto.FLOAT, (6, 4))], [make_node('SplitToSequence', ['input'], ['output_sequence'], keepdims=1)], [])
    ...
    def test_split_to_sequence_not_keepdims(self) -> None:
        graph = self._make_graph([('input', TensorProto.FLOAT, (6, 4))], [make_node('SplitToSequence', ['input'], ['output_sequence'], keepdims=0)], [])
        self._assert_inferred(graph, [make_tensor_sequence_value_info('output_sequence', TensorProto.FLOAT, (4,))])
    ...
    def test_split_to_sequence_axis(self) -> None:
        graph = self._make_graph([('input', TensorProto.FLOAT, (6, 4))], [make_node('SplitToSequence', ['input'], ['output_sequence'], axis=1)], [])
        self._assert_inferred(graph, [make_tensor_sequence_value_info('output_sequence', TensorProto.FLOAT, (6, 1))])
    ...
    def test_split_to_sequence_split_sizes(self) -> None:
        graph = self._make_graph([('input', TensorProto.FLOAT, (6, 4)), ('split', TensorProto.INT32, (3,))], [make_node('SplitToSequence', ['input', 'split'], ['output_sequence'])], [], initializer=[make_tensor('split', TensorProto.INT32, (3,), (2, 1, 3))])
        self._assert_inferred(graph, [make_tensor_sequence_value_info('output_sequence', TensorProto.FLOAT, (None, 4))])

    def test_split_to_sequence_non_divisible(self) -> None:
        graph = self._make_graph([('input', TensorProto.FLOAT, (6, 4)), ('split', TensorProto.INT32, ())], [make_node('SplitToSequence', ['input', 'split'], ['output_sequence'])], [], initializer=[make_tensor('split', TensorProto.INT32, (), (4,))])
        self._assert_inferred(graph, [make_tensor_sequence_value_info('output_sequence', TensorProto.FLOAT, (None, 4))])

    def test_concat_from_sequence(self) -> None:
        graph = self._make_graph([('input1', TensorProto.FLOAT, (2, 3, 'x')), ('input2', TensorProto.FLOAT, (2, 3, 'x')), ('input3', TensorProto.FLOAT, (2, 3, 'x'))], [make_node('SequenceConstruct', ['input1', 'input2', 'input3'], ['in_sequence']), make_node('ConcatFromSequence', ['in_sequence'], ['out'], axis=0)], [])
        self._assert_inferred(graph, [make_tensor_sequence_value_info('in_sequence', TensorProto.FLOAT, (2, 3, 'x')), make_tensor_value_info('out', TensorProto.FLOAT, (None, 3, 'x'))])

    def test_concat_from_sequence_unknown_shape(self) -> None:
        graph = self._make_graph([('input1', TensorProto.FLOAT, (2, 3, 'x')), ('input2', TensorProto.FLOAT, (2, 3)), ('input3', TensorProto.FLOAT, (2, 3, 'x'))], [make_node('SequenceConstruct', ['input1', 'input2', 'input3'], ['in_sequence']), make_node('ConcatFromSequence', ['in_sequence'], ['out'], axis=0)], [])
        self._assert_inferred(graph, [make_tensor_sequence_value_info('in_sequence', TensorProto.FLOAT, None), make_tensor_value_info('out', TensorProto.FLOAT, None)])

    def test_concat_from_sequence_unknown_dim_size(self) -> None:
        graph = self._make_graph([('input1', TensorProto.FLOAT, (2, 3, 'x')), ('input2', TensorProto.FLOAT, (2, 4, 'x')), ('input3', TensorProto.FLOAT, (2, 3, 'x'))], [make_node('SequenceConstruct', ['input1', 'input2', 'input3'], ['in_sequence']), make_node('ConcatFromSequence', ['in_sequence'], ['out'], axis=0)], [])
        self._assert_inferred(graph, [make_tensor_sequence_value_info('in_sequence', TensorProto.FLOAT, (2, None, 'x')), make_tensor_value_info('out', TensorProto.FLOAT, (None, None, 'x'))])

    def test_concat_from_sequence_axis(self) -> None:
        graph = self._make_graph([('input1', TensorProto.FLOAT, (2, 3, 'x')), ('input2', TensorProto.FLOAT, (2, 4, 'x')), ('input3', TensorProto.FLOAT, (2, 3, 'x'))], [make_node('SequenceConstruct', ['input1', 'input2', 'input3'], ['in_sequence']), make_node('ConcatFromSequence', ['in_sequence'], ['out'], axis=2)], [])
        self._assert_inferred(graph, [make_tensor_sequence_value_info('in_sequence', TensorProto.FLOAT, (2, None, 'x')), make_tensor_value_info('out', TensorProto.FLOAT, (2, None, None))])

    def test_concat_from_sequence_neg_axis(self) -> None:
        graph = self._make_graph([('input1', TensorProto.FLOAT, (2, 3, 'x')), ('input2', TensorProto.FLOAT, (2, 4, 'x')), ('input3', TensorProto.FLOAT, (2, 3, 'x'))], [make_node('SequenceConstruct', ['input1', 'input2', 'input3'], ['in_sequence']), make_node('ConcatFromSequence', ['in_sequence'], ['out'], axis=-3)], [])
        self._assert_inferred(graph, [make_tensor_sequence_value_info('in_sequence', TensorProto.FLOAT, (2, None, 'x')), make_tensor_value_info('out', TensorProto.FLOAT, (None, None, 'x'))])
    ...
    def test_concat_from_sequence_neg_new_axis(self) -> None:
        graph = self._make_graph([('input1', TensorProto.FLOAT, (2, 3, 'x')), ('input2', TensorProto.FLOAT, (2, 3, 'x')), ('input3', TensorProto.FLOAT, (2, 3, 'x'))], [make_node('SequenceConstruct', ['input1', 'input2', 'input3'], ['in_sequence']), make_node('ConcatFromSequence', ['in_sequence'], ['out'], axis=-1, new_axis=1)], [])
        self._assert_inferred(graph, [make_tensor_sequence_value_info('in_sequence', TensorProto.FLOAT, (2, 3, 'x')), make_tensor_value_info('out', TensorProto.FLOAT, (2, 3, 'x', None))])

    def test_adagrad(self) -> None:
        graph = self._make_graph([('R', TensorProto.FLOAT, ()), ('T', TensorProto.INT64, ()), ('X', TensorProto.FLOAT, (1, 2)), ('G', TensorProto.FLOAT, (1, 2)), ('H', TensorProto.FLOAT, (1, 2))], [make_node('Adagrad', ['R', 'T', 'X', 'G', 'H'], ['X_new', 'H_new'], domain=AI_ONNX_PREVIEW_TRAINING_DOMAIN)], [])
        self._assert_inferred(graph, [make_tensor_value_info('X_new', TensorProto.FLOAT, (1, 2)), make_tensor_value_info('H_new', TensorProto.FLOAT, (1, 2))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, 12), helper.make_opsetid(AI_ONNX_PREVIEW_TRAINING_DOMAIN, 1)])
    ...
    def test_momentum_multiple(self) -> None:
        graph = self._make_graph([('R', TensorProto.FLOAT, ()), ('T', TensorProto.INT64, ()), ('X1', TensorProto.FLOAT, (1, 2)), ('X2', TensorProto.FLOAT, (3, 4)), ('G1', TensorProto.FLOAT, (1, 2)), ('G2', TensorProto.FLOAT, (3, 4)), ('V1', TensorProto.FLOAT, (1, 2)), ('V2', TensorProto.FLOAT, (3, 4))], [make_node('Momentum', ['R', 'T', 'X1', 'X2', 'G1', 'G2', 'V1', 'V2'], ['X1_new', 'X2_new', 'V1_new', 'V2_new'], alpha=0.9, beta=1.0, norm_coefficient=0.02, mode='nesterov', domain=AI_ONNX_PREVIEW_TRAINING_DOMAIN)], [])
        self._assert_inferred(graph, [make_tensor_value_info('X1_new', TensorProto.FLOAT, (1, 2)), make_tensor_value_info('X2_new', TensorProto.FLOAT, (3, 4)), make_tensor_value_info('V1_new', TensorProto.FLOAT, (1, 2)), make_tensor_value_info('V2_new', TensorProto.FLOAT, (3, 4))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, 12), helper.make_opsetid(AI_ONNX_PREVIEW_TRAINING_DOMAIN, 1)])
    ...
    def test_pad_opset10(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (1, None, 2))], [make_node('Pad', 'x', 'y', pads=[1, 3, 1, 1, 0, 1])], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.FLOAT, (3, None, 4))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, 10)])
    ...
    def test_pad(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (1, None, 2)), ('pads', TensorProto.INT64, (6,))], [make_node('Pad', ['x', 'pads'], 'y')], [], initializer=[make_tensor('pads', TensorProto.INT64, (6,), (1, 3, 1, 1, 0, 1))])
    ...
    def test_gatherelements_indices_missing_shape(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (6,)), ('indices', TensorProto.INT64, None)], [make_node('GatherElements', ['x', 'indices'], ['y'])], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.FLOAT, None)])

    def test_einsum_transpose(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (3, 4))], [make_node('Einsum', ['x'], ['y'], equation='ij->ji')], [])
    ...
    def test_einsum_sum_along_dim(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (3, 4))], [make_node('Einsum', ['x'], ['y'], equation='i j->i ')], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.FLOAT, (None,))])
    ...
    def test_einsum_ellipsis_2(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (2, 2, 2)), ('y', TensorProto.FLOAT, (2, 2, 2))], [make_node('Einsum', ['x', 'y'], ['z'], equation='...ij,...jk->...ik')], [])
    ...
    def test_einsum_contraction(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (5, 6, 7, 8)), ('y', TensorProto.FLOAT, (8, 9, 10))], [make_node('Einsum', ['x', 'y'], ['z'], equation='abcd,dfg->abcfg')], [])
        self._assert_inferred(graph, [make_tensor_value_info('z', TensorProto.FLOAT, (None, None, None, None, None))])
    ...
    def test_einsum_batch_matmul(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (5, 2, 3)), ('y', TensorProto.FLOAT, (5, 3, 4))], [make_node('Einsum', ['x', 'y'], ['z'], equation='bij , b jk-> bik')], [])
        self._assert_inferred(graph, [make_tensor_value_info('z', TensorProto.FLOAT, (None, None, None))])
    ...
    def test_negative_log_likehood_shape_is_NC_reduction_mean(self) -> None:
        N, C = (3, 4)
        graph = self._make_graph([('input', TensorProto.FLOAT, (N, C)), ('target', TensorProto.INT64, (N,))], [make_node('NegativeLogLikelihoodLoss', ['input', 'target'], ['loss'], reduction='mean')], [])
        self._assert_inferred(graph, [make_tensor_value_info('loss', TensorProto.FLOAT, ())])

    def test_negative_log_likehood_shape_is_NC_with_weight_reduction_mean(self) -> None:
        N, C = (3, 4)
        graph = self._make_graph([('input', TensorProto.FLOAT, (N, C)), ('target', TensorProto.INT64, (N,)), ('weight', TensorProto.FLOAT, (C,))], [make_node('NegativeLogLikelihoodLoss', ['input', 'target', 'weight'], ['loss'], reduction='mean')], [])
        self._assert_inferred(graph, [make_tensor_value_info('loss', TensorProto.FLOAT, ())])

    def test_negative_log_likehood_shape_is_NCd1d2(self) -> None:
        N, C, d1, d2 = (3, 4, 5, 6)
        graph = self._make_graph([('input', TensorProto.FLOAT, (N, C, d1, d2)), ('target', TensorProto.INT64, (N, d1, d2))], [make_node('NegativeLogLikelihoodLoss', ['input', 'target'], ['loss'], reduction='none')], [])
    ...
    def test_negative_log_likehood_shape_is_NCd1d2_reduction_sum(self) -> None:
        N, C, d1, d2 = (3, 4, 5, 6)
        graph = self._make_graph([('input', TensorProto.FLOAT, (N, C, d1, d2)), ('target', TensorProto.INT64, (N, d1, d2))], [make_node('NegativeLogLikelihoodLoss', ['input', 'target'], ['loss'], reduction='sum')], [])
        self._assert_inferred(graph, [make_tensor_value_info('loss', TensorProto.FLOAT, ())])

    def test_negative_log_likehood_shape_is_NCd1d2_with_weight_reduction_mean(self) -> None:
        N, C, d1, d2 = (3, 4, 5, 6)
        graph = self._make_graph([('input', TensorProto.FLOAT, (N, C, d1, d2)), ('target', TensorProto.INT64, (N, d1, d2)), ('weight', TensorProto.FLOAT, (C,))], [make_node('NegativeLogLikelihoodLoss', ['input', 'target', 'weight'], ['loss'], reduction='mean')], [])
        self._assert_inferred(graph, [make_tensor_value_info('loss', TensorProto.FLOAT, ())])

    def test_negative_log_likehood_input_target_shape_mismatch(self) -> None:
        N, C, d1, d2 = (3, 4, 5, 6)
        graph = self._make_graph([('input', TensorProto.FLOAT, (N, d1, d2)), ('target', TensorProto.INT64, (N, d1 + 1, d2)), ('weight', TensorProto.FLOAT, (C,)), ('loss', TensorProto.FLOAT, ())], [make_node('NegativeLogLikelihoodLoss', ['input', 'target', 'weight'], ['loss'], reduction='mean')], [])
        self.assertRaises(onnx.shape_inference.InferenceError, self._inferred, graph)

    def test_negative_log_likehood_input_weight_shape_mismatch(self) -> None:
        N, C, d1, d2 = (3, 4, 5, 6)
        graph = self._make_graph([('input', TensorProto.FLOAT, (N, C, d1, d2)), ('target', TensorProto.INT64, (N, d1, d2)), ('weight', TensorProto.FLOAT, (C + 1,)), ('loss', TensorProto.FLOAT, (N, d1, d2))], [make_node('NegativeLogLikelihoodLoss', ['input', 'target', 'weight'], ['loss'], reduction='none')], [])
        self.assertRaises(checker.ValidationError, self._inferred, graph)

    def test_softmax_cross_entropy_none(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (2, 3)), ('y', TensorProto.FLOAT, (2,))], [make_node('SoftmaxCrossEntropyLoss', ['x', 'y'], ['z'], reduction='none')], [])
        self._assert_inferred(graph, [make_tensor_value_info('z', TensorProto.FLOAT, (2,))])

    def test_softmax_cross_entropy_mean(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (2, 3)), ('y', TensorProto.FLOAT, (2,))], [make_node('SoftmaxCrossEntropyLoss', ['x', 'y'], ['z'], reduction='mean')], [])
        self._assert_inferred(graph, [make_tensor_value_info('z', TensorProto.FLOAT, ())])
    ...
    def test_softmax_cross_entropy_mean_NCD1D2(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (2, 3, 4, 5)), ('y', TensorProto.FLOAT, (2, 4, 5))], [make_node('SoftmaxCrossEntropyLoss', ['x', 'y'], ['z'], reduction='mean')], [])
        self._assert_inferred(graph, [make_tensor_value_info('z', TensorProto.FLOAT, ())])

    def test_celu_function_output_shape(self) -> None:
        graph = self._make_graph([('X', TensorProto.FLOAT, (25, 48, 16, 16))], [make_node('Celu', ['X'], ['Y'], alpha=2.0)], [])
        self._assert_inferred(graph, [make_tensor_value_info('Y', TensorProto.FLOAT, (25, 48, 16, 16))])
    ...
    def test_trilu_upper(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (3, 4, 5)), ('k', TensorProto.INT64, ())], [make_node('Trilu', ['x', 'k'], ['y'])], [], initializer=[make_tensor('k', TensorProto.INT64, (), (2,))])
    ...
    def test_batch_norm_train(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (3, 4, 5, 6, 7)), ('scale', TensorProto.FLOAT, (4,)), ('b', TensorProto.FLOAT, (4,)), ('input_mean', TensorProto.FLOAT, (4,)), ('input_var', TensorProto.FLOAT, (4,))], [make_node('BatchNormalization', ['x', 'scale', 'b', 'input_mean', 'input_var'], ['out', 'output_mean', 'output_var'], training_mode=1)], [])
        self._assert_inferred(graph, [make_tensor_value_info('out', TensorProto.FLOAT, (3, 4, 5, 6, 7)), make_tensor_value_info('output_mean', TensorProto.FLOAT, (4,)), make_tensor_value_info('output_var', TensorProto.FLOAT, (4,))])
    ...
    def test_batch_norm_train_with_diff_type(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT16, (3, 4, 5, 6, 7)), ('scale', TensorProto.FLOAT16, (4,)), ('b', TensorProto.FLOAT16, (4,)), ('input_mean', TensorProto.FLOAT, (4,)), ('input_var', TensorProto.FLOAT, (4,))], [make_node('BatchNormalization', ['x', 'scale', 'b', 'input_mean', 'input_var'], ['out', 'output_mean', 'output_var'], training_mode=1)], [])
        self._assert_inferred(graph, [make_tensor_value_info('out', TensorProto.FLOAT16, (3, 4, 5, 6, 7)), make_tensor_value_info('output_mean', TensorProto.FLOAT, (4,)), make_tensor_value_info('output_var', TensorProto.FLOAT, (4,))])
    ...
    def test_nonzero_scalar(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, ())], [make_node('NonZero', ['x'], ['out'])], [])
        self._assert_inferred(graph, [make_tensor_value_info('out', TensorProto.INT64, (0, None))])
    ...
    def test_optional_construct_empty_tensor(self) -> None:
        ...
        optional_type_proto = helper.make_optional_type_proto(tensor_type_proto)
        optional_val_info = helper.make_value_info(name='output', type_proto=optional_type_proto)
        graph = self._make_graph([], [make_node('Optional', [], ['output'], type=tensor_type_proto)], [])
    ...
    def test_optional_construct_tensor(self) -> None:
        ...
        optional_type_proto = helper.make_optional_type_proto(tensor_type_proto)
        optional_val_info = helper.make_value_info(name='output', type_proto=optional_type_proto)
        graph = self._make_graph([('input1', TensorProto.FLOAT, (2, 3, 4))], [make_node('Optional', ['input1'], ['output'])], [])
        self._assert_inferred(graph, [optional_val_info])
    ...
    def test_string_split_basic(self, _, version) -> None:
        substrings = make_tensor_value_info('substrings', TensorProto.STRING, (2, None))
        length = make_tensor_value_info('length', TensorProto.INT64, (2,))
        graph = self._make_graph([('x', TensorProto.STRING, (2,))], [make_node('StringSplit', ['x'], ['substrings', 'length'])], [substrings, length])
    ...
    def test_string_split_zero_dimensional_input(self, _, version) -> None:
        ...
        graph = self._make_graph([('x', TensorProto.STRING, ())], [make_node('StringSplit', ['x'], ['substrings', 'length'], maxsplit=2)], [substrings, length])
        self._assert_inferred(graph, [substrings, length], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, version)])

    @parameterized.expand(all_versions_for('StringSplit'))
    def test_string_split_empty_input(self, _, version) -> None:
        substrings = make_tensor_value_info('substrings', TensorProto.STRING, ('M', 3, 0, None))
        length = make_tensor_value_info('length', TensorProto.INT64, ('M', 3, 0))
        graph = self._make_graph([('x', TensorProto.STRING, ('M', 3, 0))], [make_node('StringSplit', ['x'], ['substrings', 'length'], maxsplit=2)], [substrings, length])
        self._assert_inferred(graph, [substrings, length], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, version)])
    ...
    def test_parse_data_with_unsupported_tensor_type(self) -> None:
        model = helper.make_model(graph=helper.make_graph(name='graph_with_unsupported_type', inputs=[], outputs=[helper.make_tensor_value_info('y', TensorProto.FLOAT, shape=None)], nodes=[make_node('ConstantOfShape', ['x'], ['y'])], initializer=[numpy_helper.from_array(np.array([4, 3], dtype=np.int32), name='x')]))
    ...
    def test_gridsample_2d(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, (1, 1, 3, 3)), ('grid', TensorProto.INT64, (1, 3, 3, 2))], [make_node('GridSample', ['x', 'grid'], ['y'], mode='nearest', padding_mode='border', align_corners=1)], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.FLOAT, (1, 1, 3, 3))])
    ...
    def test_gridsample_3d_defaults(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, ('N', 'C', 'D', 'H', 'W')), ('grid', TensorProto.FLOAT, ('N', 'D_out', 'H_out', 'W_out', 3))], [make_node('GridSample', ['x', 'grid'], ['y'])], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.FLOAT, ('N', 'C', 'D_out', 'H_out', 'W_out'))])
    ...
    def test_gridsample_3d_no_dim(self) -> None:
        graph = self._make_graph([('x', TensorProto.FLOAT, ('N', 'C', None, None, None)), ('grid', TensorProto.FLOAT, ('N', None, None, None, 3))], [make_node('GridSample', ['x', 'grid'], ['y'], mode='linear', padding_mode='border')], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.FLOAT, ('N', 'C', None, None, None))])
    ...
    def test_sequence_map_slice_outs_unknown_dims(self):
        ...
        graph = self._make_graph([('input1', TensorProto.FLOAT, (220, 310, 3)), ('input2', TensorProto.FLOAT, (110, 210, 3)), ('input3', TensorProto.FLOAT, (90, 110, 3))], [make_node('SequenceConstruct', ['input1', 'input2', 'input3'], ['in_sequence']), make_node('SequenceMap', ['in_sequence'], ['out_sequence1', 'out_sequence2'], body=body_graph)], [])
        self._assert_inferred(graph, [make_tensor_sequence_value_info('in_sequence', TensorProto.FLOAT, (None, None, 3)), make_tensor_sequence_value_info('out_sequence1', TensorProto.FLOAT, (None, None, 3)), make_tensor_sequence_value_info('out_sequence2', TensorProto.FLOAT, (None, None, 3))])
    ...
    def test_hammingwindow(self):
        graph = self._make_graph([], [make_node('Constant', [], ['shape'], value=make_tensor('shape', TensorProto.INT64, (), (10,))), make_node('HammingWindow', ['shape'], ['y'])], [])
        self._assert_inferred(graph, [make_tensor_value_info('shape', TensorProto.INT64, ()), make_tensor_value_info('y', TensorProto.FLOAT, (10,))])
        graph = self._make_graph([], [make_node('Constant', [], ['shape'], value=make_tensor('shape', TensorProto.INT64, (), (10,))), make_node('HammingWindow', ['shape'], ['y'], periodic=0)], [])
        self._assert_inferred(graph, [make_tensor_value_info('shape', TensorProto.INT64, ()), make_tensor_value_info('y', TensorProto.FLOAT, (10,))])

    def test_hannwindow(self):
        graph = self._make_graph([], [make_node('Constant', [], ['shape'], value=make_tensor('shape', TensorProto.INT64, (), (10,))), make_node('HannWindow', ['shape'], ['y'])], [])
        self._assert_inferred(graph, [make_tensor_value_info('shape', TensorProto.INT64, ()), make_tensor_value_info('y', TensorProto.FLOAT, (10,))])
        graph = self._make_graph([], [make_node('Constant', [], ['shape'], value=make_tensor('shape', TensorProto.INT64, (), (10,))), make_node('HannWindow', ['shape'], ['y'], periodic=0)], [])
        self._assert_inferred(graph, [make_tensor_value_info('shape', TensorProto.INT64, ()), make_tensor_value_info('y', TensorProto.FLOAT, (10,))])
    ...
    def test_dft(self, _: str, version: int, _test_aspect: str, input_shape: tuple[int], axis: int | None, onesided: int | None, inverse: int | None, expected_shape: tuple[int]) -> None:
        ...
        if version < 20:
            ...
            assert version >= 20
            if axis is not None:
                nodes = [make_node('Constant', [], ['axis'], value=make_tensor('axis', TensorProto.INT64, (), (axis,))), make_node('DFT', ['input', '', 'axis'], ['output'], **attributes)]
    ...
    def test_dft_invalid_axis_opset20(self, _: str, axis: int) -> None:
        graph = self._make_graph([], [make_node('Constant', [], ['input'], value=make_tensor('input', TensorProto.FLOAT, (2, 5, 5, 2), np.ones((2, 5, 5, 2), dtype=np.float32).flatten())), make_node('Constant', [], ['axis'], value=make_tensor('axis', TensorProto.INT64, (), (axis,))), make_node('DFT', ['input', '', 'axis'], ['output'])], [])
        with self.assertRaises(onnx.shape_inference.InferenceError):
            self._assert_inferred(graph, [make_tensor_value_info('input', TensorProto.FLOAT, (2, 5, 5, 2)), make_tensor_value_info('axis', TensorProto.INT64, ()), make_tensor_value_info('output', TensorProto.FLOAT, (2, 3, 5, 2))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, 20)])

    @parameterized.expand([('real', (2, 5, 5, 1)), ('complex', (2, 5, 5, 2))])
    def test_dft_dynamic_axis_opset20(self, _: str, shape: tuple[int, ...]) -> None:
        graph = self._make_graph([('axis', TensorProto.INT64, ())], [make_node('Constant', [], ['input'], value=make_tensor('input', TensorProto.FLOAT, shape, np.ones(shape, dtype=np.float32).flatten())), make_node('DFT', ['input', '', 'axis'], ['output'])], [])
        self._assert_inferred(graph, [make_tensor_value_info('input', TensorProto.FLOAT, shape), make_tensor_value_info('output', TensorProto.FLOAT, (2, 5, 5, 2))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, 20)])
    ...
    def test_dft_onesided_default_axis_opset17(self) -> None:
        graph = self._make_graph([], [make_node('Constant', [], ['input'], value=make_tensor('input', TensorProto.FLOAT, (2, 5, 5, 2), np.ones((2, 5, 5, 2), dtype=np.float32).flatten())), make_node('DFT', ['input', ''], ['output'], onesided=1)], [])
        self._assert_inferred(graph, [make_tensor_value_info('input', TensorProto.FLOAT, (2, 5, 5, 2)), make_tensor_value_info('output', TensorProto.FLOAT, (2, 3, 5, 2))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, 17)])

    def test_dft_onesided_default_axis_opset20(self) -> None:
        graph = self._make_graph([], [make_node('Constant', [], ['input'], value=make_tensor('input', TensorProto.FLOAT, (2, 5, 5, 2), np.ones((2, 5, 5, 2), dtype=np.float32).flatten())), make_node('DFT', ['input', '', ''], ['output'], onesided=1)], [])
        self._assert_inferred(graph, [make_tensor_value_info('input', TensorProto.FLOAT, (2, 5, 5, 2)), make_tensor_value_info('output', TensorProto.FLOAT, (2, 5, 3, 2))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, 20)])
    ...
    def test_center_crop_pad_hwc_crop(self):
        graph = self._make_graph([('input_data', TensorProto.FLOAT, (20, 10, 3)), ('shape', TensorProto.INT64, (2,))], [make_node('CenterCropPad', ['input_data', 'shape'], ['y'], axes=[0, 1])], [], initializer=[make_tensor('shape', TensorProto.INT64, (2,), (10, 8))])
    ...
    def test_center_crop_pad_hwc_croppad(self):
        graph = self._make_graph([('input_data', TensorProto.FLOAT, (10, 10, 3)), ('shape', TensorProto.INT64, (2,))], [make_node('CenterCropPad', ['input_data', 'shape'], ['y'], axes=[0, 1])], [], initializer=[make_tensor('shape', TensorProto.INT64, (2,), (20, 8))])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.FLOAT, (20, 8, 3))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, 18)])

    def test_center_crop_pad_chw_croppad(self):
        graph = self._make_graph([('input_data', TensorProto.FLOAT, (3, 10, 10)), ('shape', TensorProto.INT64, (2,))], [make_node('CenterCropPad', ['input_data', 'shape'], ['y'], axes=[1, 2])], [], initializer=[make_tensor('shape', TensorProto.INT64, (2,), (20, 8))])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.FLOAT, (3, 20, 8))], opset_imports=[helper.make_opsetid(ONNX_DOMAIN, 18)])
    ...
    def test_binarizer(self) -> None:
        node = make_node('Binarizer', ['x'], ['y'], domain=ONNX_ML_DOMAIN)
        graph = self._make_graph([('x', TensorProto.INT64, (3, 4, 5))], [node], [])
        self._assert_inferred(graph, [make_tensor_value_info('y', TensorProto.INT64, (3, 4, 5))], opset_imports=[make_opsetid(ONNX_ML_DOMAIN, 3), make_opsetid(ONNX_DOMAIN, 18)])

    @unittest.skipUnless(ONNX_ML, 'ONNX_ML required to test ai.onnx.ml operators')
    def test_one_hot_encoder(self) -> None:
        graph = self._make_graph([('input', TensorProto.INT64, (2, 'N', 3))], [make_node('OneHotEncoder', ['input'], ['output'], cats_int64s=[1, 2, 3, 4], domain='ai.onnx.ml')], [])
        self._assert_inferred(graph, [make_tensor_value_info('output', TensorProto.FLOAT, (2, 'N', 3, 4))], opset_imports=[make_opsetid(ONNX_ML_DOMAIN, 1), make_opsetid(ONNX_DOMAIN, 18)])
    ...
    def test_compress_with_axis(self) -> None:
        graph = self._make_graph([('input', TensorProto.INT64, (2, 'N', 3, 4)), ('condition', TensorProto.BOOL, (None,))], [make_node('Compress', ['input', 'condition'], ['output'], axis=-1)], [])
        self._assert_inferred(graph, [make_tensor_value_info('output', TensorProto.INT64, (2, 'N', 3, None))])
