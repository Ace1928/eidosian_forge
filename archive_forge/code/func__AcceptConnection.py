...
def _AcceptConnection(self, client, _):
    ...
    server = websocket.WebSocketApp('wss://%s/_workstation/tcp/%d' % (self.host, self.port), header={'Authorization': 'Bearer %s' % self.access_token}, on_open=lambda ws: self._ForwardClientToServer(client, ws), on_data=lambda ws, data, op, finished: client.send(data), on_error=lambda ws, e: self._OnWebsocketError(client, e))

    def Run():
        server.run_forever(sslopt={'cert_reqs': ssl.CERT_REQUIRED, 'ca_certs': ca_certs})
