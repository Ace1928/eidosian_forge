...
class AzureRMDevTestLabVirtualNetwork(AzureRMModuleBase):
    ...
    def __init__(self):
        self.module_arg_spec = dict(resource_group=dict(type='str', required=True), lab_name=dict(type='str', required=True), name=dict(type='str', required=True), location=dict(type='str'), description=dict(type='str'), state=dict(type='str', default='present', choices=['present', 'absent']))
        ...
        super(AzureRMDevTestLabVirtualNetwork, self).__init__(derived_arg_spec=self.module_arg_spec, supports_check_mode=True, supports_tags=True)
    ...
    def exec_module(self, **kwargs):
        ...
        response = None
        self.mgmt_client = self.get_mgmt_svc_client(DevTestLabsClient, base_url=self._cloud_environment.endpoints.resource_manager, is_track2=True, api_version='2018-10-15')
        ...
        subnet_id = template.format(self.subscription_id, self.resource_group, self.name, self.name + 'Subnet')
        self.virtual_network['subnet_overrides'] = [{'resource_id': subnet_id, 'lab_subnet_name': self.name + 'Subnet', 'use_in_vm_creation_permission': 'Allow', 'use_public_ip_address_permission': 'Allow'}]
        ...
        if self.state == 'present':
            self.results.update({'id': response.get('id', None), 'external_provider_resource_id': response.get('external_provider_resource_id', None)})
        return self.results
