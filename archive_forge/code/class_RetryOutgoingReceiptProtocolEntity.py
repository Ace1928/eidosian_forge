...
class RetryOutgoingReceiptProtocolEntity(OutgoingReceiptProtocolEntity):
    ...
    def toProtocolTreeNode(self):
        ...
        retry_attrs = {'count': str(self.count), 'id': self.getId(), 'v': str(self.v), 't': str(self.retry_timestamp)}
        retry = ProtocolTreeNode('retry', retry_attrs)
        node.addChild(retry)
        registration = ProtocolTreeNode('registration', data=ResultGetKeysIqProtocolEntity._intToBytes(self.local_registration_id))
