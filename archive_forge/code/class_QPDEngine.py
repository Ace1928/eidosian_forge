...
class QPDEngine(ABC):
    ...
    def is_value(self, col: Column, is_value: IsValueSpec) -> Column:
        ...
        if v == 'true':
            if is_value.positive:
                return Column(col.native == True)
            else:
                return Column(col.native != True)
        if v == 'false':
            if is_value.positive:
                return Column(col.native == False)
            else:
                return Column(col.native != False)
    ...
    def is_between(self, col: Column, lower: Column, upper: Column, positive: bool) -> Column:
        ln = lower.native
        cn = col.native
        un = upper.native
        if positive:
    ...
    def join(self, df1: DataFrame, df2: DataFrame, join_type: str, on: List[str]) -> DataFrame:
        ndf1 = self.to_native(df1)
        ndf2 = self.to_native(df2)
    ...
    def _preprocess_set_op(self, df1: DataFrame, df2: DataFrame) -> Tuple[Any, Any]:
        assert_or_throw(len(df1) == len(df2), ValueError('two dataframes have different number of columns'))
        ndf1 = self.to_native(df1)
        ndf2 = self.to_native(df2)
