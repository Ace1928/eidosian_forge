...
class ReplicationsClient(object):
    ...
    """
        request = self.messages.NetappProjectsLocationsVolumesReplicationsListRequest(parent=volume_ref)
        ...
        return list_pager.YieldFromList(self.client.projects_locations_volumes_replications, request, field=constants.REPLICATION_RESOURCE, limit=limit, batch_size_attribute='pageSize')
    ...
    def _DeleteReplication(self, async_, request):
        delete_op = self.client.projects_locations_volumes_replications.Delete(request)
        if async_:
            return delete_op
    ...
    """
        return self._adapter.ParseUpdatedReplicationConfig(replication_config, description=description, labels=labels, replication_schedule=replication_schedule)

    def UpdateReplication(self, replication_ref, replication_config, update_mask, async_):
    ...
    """
        update_op = self._adapter.UpdateReplication(replication_ref, replication_config, update_mask)
        if async_:
            return update_op
        operation_ref = resources.REGISTRY.ParseRelativeName(update_op.name, collection=constants.OPERATIONS_COLLECTION)
        return self.WaitForOperation(operation_ref)

    def ResumeReplication(self, replication_ref, async_):
    ...
    """
        ...
        if async_:
            return resume_op
        operation_ref = resources.REGISTRY.ParseRelativeName(resume_op.name, collection=constants.OPERATIONS_COLLECTION)
        return self.WaitForOperation(operation_ref)

    def ReverseReplication(self, replication_ref, async_):
    ...
    """
        ...
        if async_:
            return reverse_op
        operation_ref = resources.REGISTRY.ParseRelativeName(reverse_op.name, collection=constants.OPERATIONS_COLLECTION)
        return self.WaitForOperation(operation_ref)
    ...
    """
        ...
        if async_:
            return stop_op
        operation_ref = resources.REGISTRY.ParseRelativeName(stop_op.name, collection=constants.OPERATIONS_COLLECTION)
        return self.WaitForOperation(operation_ref)