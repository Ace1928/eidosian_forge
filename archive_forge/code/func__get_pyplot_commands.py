...
def _get_pyplot_commands() -> list[str]:
    ...
    return sorted((name for name, obj in globals().items() if not name.startswith('_') and name not in exclude and inspect.isfunction(obj) and (inspect.getmodule(obj) is this_module)))