...
class Types(ModuleAnalysis):
    ...
    def __init__(self):
        self.max_seq_size = cfg.getint('typing', 'max_heterogeneous_sequence_size')
        ...
        ModuleAnalysis.__init__(self, Reorder, StrictAliases, LazynessAnalysis, Immediates, RangeValues, Ancestors)
    ...
    def visit_FunctionDef(self, node):
        ...
        for delayed_node in self.delayed_types:
            ...
            all_types = ordered_set((self.result[ty] for ty in self.name_to_nodes[delayed_node.id]))
        ...
        return_type = self.result.get(node, self.builder.NamedType('pythonic::types::none_type'))
        self.result[node] = (self.builder.Returnable(return_type), reduce(list.__add__, self.typedefs.values(), []))
    ...
    def visit_BinOp(self, node):
        self.generic_visit(node)
        self.update_type(node, self.builder.ExpressionType, operator_to_lambda[type(node.op)], self.result[node.left], self.result[node.right])

    def visit_UnaryOp(self, node):
        self.generic_visit(node)
        self.update_type(node, self.builder.ExpressionType, operator_to_lambda[type(node.op)], self.result[node.operand])
    ...
    def visit_Compare(self, node):
        self.generic_visit(node)
        all_compare = zip(node.ops, [node.left] + node.comparators[:-1], node.comparators)
        for op, left, right in all_compare:
            self.update_type(node, self.builder.ExpressionType, operator_to_lambda[type(op)], self.result[left], self.result[right])
    ...
    def visit_Call(self, node):
        ...
        if isinstance(func, ast.Attribute) and func.attr == 'getattr':
            self.update_type(node, self.builder.GetAttr, self.result[node.args[0]], node.args[1].value)
        else:
            self.update_type(node, self.builder.ReturnType, self.result[func], *[self.result[arg] for arg in node.args])
    ...
    def visit_Constant(self, node):
        ...
        if node in self.immediates:
            sty = 'std::integral_constant<%s, %s>' % (sty, str(node.value).lower())
    ...
    def visit_Slice(self, node):
        ...
        if nstep is None or (isnum(nstep) and nstep.value > 0):
            if nstep is None or nstep.value == 1:
                if all((self.range_values[p].low >= 0 for p in (node.lower, node.upper))):
            ...
            self.result[node] = self.builder.NamedType('pythonic::types::slice')
    ...
    def visit_Subscript(self, node):
        ...
        if isextslice(node.slice):
            ...
            self.update_type(node, self.builder.ExpressionType, lambda a, *b: '{0}({1})'.format(a, ', '.join(b)), self.result[node.value], *[self.result[d] for d in node.slice.elts])
        elif isnum(node.slice) and node.slice.value >= 0:
            self.update_type(node, self.builder.ElementType, node.slice.value, self.result[node.value])
        else:
            self.visit(node.slice)
            self.update_type(node, self.builder.ExpressionType, '{0}[{1}]'.format, self.result[node.value], self.result[node.slice])
    ...
    def delayed(self, node):
        fallback_type = self.combined(*[self.result[n] for n in self.name_to_nodes[node.id]])
    ...
    def visit_Name(self, node):
        if node.id in self.name_to_nodes:
            ...
            newtype = self.builder.NamedType(self.current_global_declarations[node.id].name)
            if node not in self.result:
    ...
    def visit_List(self, node):
        ...
        if node.elts:
            ...
            self.update_type(node, self.builder.NamedType, 'pythonic::types::empty_list')
    ...
    def visit_Set(self, node):
        ...
        if node.elts:
            ...
            self.update_type(node, self.builder.NamedType, 'pythonic::types::empty_set')

    def visit_Dict(self, node):
        """ Define set type from all elements type (or empty_dict type). """
        self.generic_visit(node)
        if node.keys:
            for key, value in islice(zip(node.keys, node.values), self.max_seq_size):
                self.update_type(node, self.builder.DictType, self.result[key], self.result[value])
        else:
            self.update_type(node, self.builder.NamedType, 'pythonic::types::empty_dict')

    def visit_ExceptHandler(self, node):
        if node.type and node.name:
            if not isinstance(node.type, ast.Tuple):
                tname = self.builder.NamedType('pythonic::types::{0}'.format(node.type.attr))
    ...
    def visit_Tuple(self, node):
        ...
        self.update_type(node, self.builder.TupleType, *types)
