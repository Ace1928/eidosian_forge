...
class NotebookAppTraits(HasTraits):
    ignore_minified_js = Bool(False, config=True, help=_i18n('Deprecated: Use minified JS file or not, mainly use during dev to avoid JS recompilation'))
    ...
    enable_mathjax = Bool(True, config=True, help='Whether to enable MathJax for typesetting math/TeX\n\n        MathJax is the javascript library Jupyter uses to render math/LaTeX. It is\n        very large, so you may want to disable it if you have a slow internet\n        connection, or for offline use of the notebook.\n\n        When disabled, equations etc. will appear as their untransformed TeX source.\n        ')
    ...
    extra_nbextensions_path = List(Unicode(), config=True, help=_i18n('extra paths to look for Javascript notebook extensions'))
    ...
    def nbextensions_path(self):
        ...
        return path
    mathjax_url = Unicode('', config=True, help='A custom url for MathJax.js.\n        Should be in the form of a case-sensitive url to MathJax,\n        for example:  /static/components/MathJax/MathJax.js\n        ')
    ...
    mathjax_config = Unicode('TeX-AMS-MML_HTMLorMML-full,Safe', config=True, help=_i18n('The MathJax.js configuration file that is to be used.'))

    @observe('mathjax_config')
    def _update_mathjax_config(self, change):
        self.log.info(_i18n('Using MathJax configuration file: %s'), change['new'])
    quit_button = Bool(True, config=True, help='If True, display a button in the dashboard to quit\n        (shutdown the notebook server).')
    nbserver_extensions = Dict({}, config=True, help=_i18n('Dict of Python modules to load as notebook server extensions.Entry values can be used to enable and disable the loading ofthe extensions. The extensions will be loaded in alphabetical order.'))