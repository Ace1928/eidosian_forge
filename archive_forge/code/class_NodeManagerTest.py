...
class NodeManagerTest(testtools.TestCase):
    ...
    def test_node_list_pagination_no_limit_path_prefix(self):
        self.api = utils.FakeAPI(fake_responses_pagination_path_prefix, path_prefix='/baremetal')
        self.mgr = node.NodeManager(self.api)
        nodes = self.mgr.list(limit=0)
        expect = [('GET', '/baremetal/v1/nodes', {}, None), ('GET', '/baremetal/v1/nodes/?limit=1', {}, None)]
        self.assertEqual(expect, self.api.calls)
        self.assertEqual(2, len(nodes))
    ...
    def test_node_list_associated(self):
        ...
        expect = [('GET', '/v1/nodes/?associated=True', {}, None)]
        self.assertEqual(expect, self.api.calls)
        self.assertThat(nodes, HasLength(1))
    ...
    def test_node_list_maintenance(self):
        ...
        expect = [('GET', '/v1/nodes/?maintenance=True', {}, None)]
        self.assertEqual(expect, self.api.calls)
        self.assertThat(nodes, HasLength(1))
        self.assertEqual(NODE2['uuid'], getattr(nodes[0], 'uuid'))
    ...
    def test_node_list_not_retired(self):
        ...
        expect = [('GET', '/v1/nodes/?retired=False', {}, None)]
        self.assertEqual(expect, self.api.calls)
        self.assertThat(nodes, HasLength(1))
        self.assertEqual(NODE1['uuid'], getattr(nodes[0], 'uuid'))
    ...
    def test_node_list_provision_state(self):
        ...
        expect = [('GET', '/v1/nodes/?provision_state=available', {}, None)]
        self.assertEqual(expect, self.api.calls)
        self.assertThat(nodes, HasLength(1))
        self.assertEqual(NODE1['uuid'], getattr(nodes[0], 'uuid'))
    ...
    def test_node_list_owner(self):
        ...
        expect = [('GET', '/v1/nodes/?owner=%s' % NODE2['owner'], {}, None)]
        self.assertEqual(expect, self.api.calls)
        self.assertThat(nodes, HasLength(1))
    ...
    def test_node_list_no_maintenance(self):
        ...
        expect = [('GET', '/v1/nodes/?maintenance=False', {}, None)]
        self.assertEqual(expect, self.api.calls)
        self.assertThat(nodes, HasLength(1))
        self.assertEqual(NODE1['uuid'], getattr(nodes[0], 'uuid'))
    ...
    def test_node_list_associated_and_maintenance(self):
        ...
        expect = [('GET', '/v1/nodes/?associated=True&maintenance=True', {}, None)]
        self.assertEqual(expect, self.api.calls)
        self.assertThat(nodes, HasLength(1))
        self.assertEqual(NODE2['uuid'], getattr(nodes[0], 'uuid'))
    ...
    def test_node_list_associated_and_retired(self):
        ...
        expect = [('GET', '/v1/nodes/?associated=True&retired=True', {}, None)]
        self.assertEqual(expect, self.api.calls)
        self.assertThat(nodes, HasLength(1))
        self.assertEqual(NODE2['uuid'], getattr(nodes[0], 'uuid'))
    ...
    def test_node_list_with_conductor(self):
        ...
        expect = [('GET', '/v1/nodes/?conductor=fake-conductor', {}, None)]
        self.assertEqual(expect, self.api.calls)
        self.assertThat(nodes, HasLength(1))
        self.assertEqual(NODE2['uuid'], getattr(nodes[0], 'uuid'))
    ...
    def test_node_list_not_sharded(self):
        ...
        expect = [('GET', '/v1/nodes/?sharded=False', {}, None)]
        self.assertEqual(expect, self.api.calls)
        self.assertThat(nodes, HasLength(1))
        self.assertEqual(NODE1['uuid'], getattr(nodes[0], 'uuid'))
    ...
    def test_node_list_nodes_by_parent_node(self):
        ...
        expect = [('GET', '/v1/nodes/?parent_node=%s' % NODE1['uuid'], {}, None)]
        self.assertEqual(expect, self.api.calls)
        self.assertThat(nodes, HasLength(1))
        self.assertEqual(NODE2['uuid'], getattr(nodes[0], 'uuid'))
    ...
    def test_node_list_children_of_node(self):
        ...
        expect = [('GET', '/v1/nodes/%s/children' % NODE1['uuid'], {}, None)]
        self.assertEqual(expect, self.api.calls)
    ...
    def test_node_list_detail(self):
        ...
        expect = [('GET', '/v1/nodes/detail', {}, None)]
        self.assertEqual(expect, self.api.calls)
        self.assertEqual(2, len(nodes))
    ...
    def test_node_show(self):
        ...
        expect = [('GET', '/v1/nodes/%s' % NODE1['uuid'], {}, None)]
        self.assertEqual(expect, self.api.calls)
    ...
    def test_node_show_fields(self):
        ...
        expect = [('GET', '/v1/nodes/%s?fields=uuid,extra' % NODE1['uuid'], {}, None)]
        self.assertEqual(expect, self.api.calls)
        self.assertEqual(NODE1['uuid'], node.uuid)

    def test_create(self):
        node = self.mgr.create(**CREATE_NODE)
        expect = [('POST', '/v1/nodes', {}, CREATE_NODE)]
        self.assertEqual(expect, self.api.calls)
        self.assertTrue(node)

    def test_create_with_uuid(self):
        node = self.mgr.create(**CREATE_WITH_UUID)
        expect = [('POST', '/v1/nodes', {}, CREATE_WITH_UUID)]
        self.assertEqual(expect, self.api.calls)
        self.assertTrue(node)

    def test_delete(self):
        node = self.mgr.delete(node_id=NODE1['uuid'])
        expect = [('DELETE', '/v1/nodes/%s' % NODE1['uuid'], {}, None)]
        self.assertEqual(expect, self.api.calls)
    ...
    def test_update(self):
        ...
        expect = [('PATCH', '/v1/nodes/%s' % NODE1['uuid'], {}, patch)]
        self.assertEqual(expect, self.api.calls)
    ...
    def test_node_port_list_with_uuid(self):
        ...
        expect = [('GET', '/v1/nodes/%s/ports' % NODE1['uuid'], {}, None)]
        self.assertEqual(expect, self.api.calls)
    ...
    def test_node_port_list_with_name(self):
        ...
        expect = [('GET', '/v1/nodes/%s/ports' % NODE1['name'], {}, None)]
        self.assertEqual(expect, self.api.calls)
        self.assertThat(ports, HasLength(1))
        self.assertEqual(PORT['uuid'], ports[0].uuid)
        self.assertEqual(PORT['address'], ports[0].address)
    ...
    def test_node_port_list_marker(self):
        ...
        expect = [('GET', '/v1/nodes/%s/ports?marker=%s' % (NODE1['uuid'], PORT['uuid']), {}, None)]
        self.assertEqual(expect, self.api.calls)
        self.assertThat(ports, HasLength(1))
    ...
    def test_node_port_list_sort_dir(self):
        ...
        expect = [('GET', '/v1/nodes/%s/ports?sort_dir=desc' % NODE1['uuid'], {}, None)]
        self.assertEqual(expect, self.api.calls)
        self.assertThat(ports, HasLength(1))
        self.assertEqual(PORT['uuid'], ports[0].uuid)
        self.assertEqual(PORT['address'], ports[0].address)
    ...
    def test_node_port_list_fields(self):
        ...
        expect = [('GET', '/v1/nodes/%s/ports?fields=uuid,address' % NODE1['uuid'], {}, None)]
        self.assertEqual(expect, self.api.calls)
        self.assertThat(ports, HasLength(1))

    def test_node_port_list_detail_and_fields_fail(self):
        self.assertRaises(exc.InvalidAttribute, self.mgr.list_ports, NODE1['uuid'], detail=True, fields=['uuid', 'extra'])
    ...
    def test_node_volume_connector_list_marker(self):
        self.api = utils.FakeAPI(fake_responses_pagination)
        self.mgr = node.NodeManager(self.api)
        volume_connectors = self.mgr.list_volume_connectors(NODE1['uuid'], marker=CONNECTOR['uuid'])
        expect = [('GET', '/v1/nodes/%s/volume/connectors?marker=%s' % (NODE1['uuid'], CONNECTOR['uuid']), {}, None)]
        self._validate_node_volume_connector_list(expect, volume_connectors)

    def test_node_volume_connector_list_sort_key(self):
        self.api = utils.FakeAPI(fake_responses_sorting)
        self.mgr = node.NodeManager(self.api)
        volume_connectors = self.mgr.list_volume_connectors(NODE1['uuid'], sort_key='updated_at')
        expect = [('GET', '/v1/nodes/%s/volume/connectors?sort_key=updated_at' % NODE1['uuid'], {}, None)]
        self._validate_node_volume_connector_list(expect, volume_connectors)

    def test_node_volume_connector_list_sort_dir(self):
        self.api = utils.FakeAPI(fake_responses_sorting)
        self.mgr = node.NodeManager(self.api)
        volume_connectors = self.mgr.list_volume_connectors(NODE1['uuid'], sort_dir='desc')
        expect = [('GET', '/v1/nodes/%s/volume/connectors?sort_dir=desc' % NODE1['uuid'], {}, None)]
        self._validate_node_volume_connector_list(expect, volume_connectors)

    def test_node_volume_connector_list_detail(self):
        volume_connectors = self.mgr.list_volume_connectors(NODE1['uuid'], detail=True)
        expect = [('GET', '/v1/nodes/%s/volume/connectors?detail=True' % NODE1['uuid'], {}, None)]
        self._validate_node_volume_connector_list(expect, volume_connectors)
    ...
    def test_node_volume_connector_list_detail_and_fields_fail(self):
        self.assertRaises(exc.InvalidAttribute, self.mgr.list_volume_connectors, NODE1['uuid'], detail=True, fields=['uuid', 'extra'])
    ...
    def test_node_volume_target_list(self):
        ...
        expect = [('GET', '/v1/nodes/%s/volume/targets' % NODE1['uuid'], {}, None)]
    ...
    def test_node_volume_target_list_limit(self):
        ...
        expect = [('GET', '/v1/nodes/%s/volume/targets?limit=1' % NODE1['uuid'], {}, None)]
        self._validate_node_volume_target_list(expect, volume_targets)
    ...
    def test_node_volume_target_list_detail(self):
        volume_targets = self.mgr.list_volume_targets(NODE1['uuid'], detail=True)
        expect = [('GET', '/v1/nodes/%s/volume/targets?detail=True' % NODE1['uuid'], {}, None)]
        self._validate_node_volume_target_list(expect, volume_targets)
    ...
    def test_node_set_maintenance_false(self):
        ...
        expect = [('DELETE', '/v1/nodes/%s/maintenance' % NODE1['uuid'], {}, None)]
        self.assertEqual(expect, self.api.calls)
        self.assertIsNone(maintenance)

    def test_node_set_maintenance_on(self):
        maintenance = self.mgr.set_maintenance(NODE1['uuid'], 'on', maint_reason='reason')
        body = {'reason': 'reason'}
        expect = [('PUT', '/v1/nodes/%s/maintenance' % NODE1['uuid'], {}, body)]
        self.assertEqual(expect, self.api.calls)
        self.assertIsNone(maintenance)
    ...
    def test_node_set_maintenance_bad(self):
        self.assertRaises(exc.InvalidAttribute, self.mgr.set_maintenance, NODE1['uuid'], 'bad')
    ...
    def test_node_set_power_state_soft_fail(self):
        self.assertRaises(ValueError, self.mgr.set_power_state, NODE1['uuid'], 'on', soft=True)
    ...
    def test_node_set_power_state_on_timeout_type_error(self):
        self.assertRaises(ValueError, self.mgr.set_power_state, NODE1['uuid'], 'off', soft=False, timeout='a')
    ...
    def test_node_set_boot_mode_invalid(self):
        self.assertRaises(ValueError, self.mgr.set_boot_mode, NODE1['uuid'], 'ancient-bios')
    ...
    def test_node_set_secure_boot_bad(self):
        self.assertRaises(exc.InvalidAttribute, self.mgr.set_secure_boot, NODE1['uuid'], 'band')
    ...
    def test_node_validate(self):
        ...
        expect = [('GET', '/v1/nodes/%s/validate' % NODE1['uuid'], {}, None)]
        self.assertEqual(expect, self.api.calls)
    ...
    def test_node_set_provision_state_microversion_override(self):
        target_state = 'active'
        self.mgr.set_provision_state(NODE1['uuid'], target_state, os_ironic_api_version='1.35')
        body = {'target': target_state}
        expect = [('PUT', '/v1/nodes/%s/states/provision' % NODE1['uuid'], {'X-OpenStack-Ironic-API-Version': '1.35'}, body)]
        self.assertEqual(expect, self.api.calls)

    def test_node_set_provision_state_with_configdrive(self):
        target_state = 'active'
        self.mgr.set_provision_state(NODE1['uuid'], target_state, configdrive=b'foo')
    ...
    def test_node_set_provision_state_with_configdrive_file(self):
        ...
        with tempfile.NamedTemporaryFile() as f:
            ...
            f.flush()
            self.mgr.set_provision_state(NODE1['uuid'], target_state, configdrive=f.name)
        body = {'target': target_state, 'configdrive': file_content.decode('utf-8')}
        expect = [('PUT', '/v1/nodes/%s/states/provision' % NODE1['uuid'], {}, body)]
        self.assertEqual(expect, self.api.calls)
    ...
    def test_node_set_provision_state_with_configdrive_json_file(self):
        ...
        with tempfile.NamedTemporaryFile() as f:
            ...
            f.flush()
            self.mgr.set_provision_state(NODE1['uuid'], target_state, configdrive=f.name)
        body = {'target': target_state, 'configdrive': {'user_data': 'foo bar'}}
        expect = [('PUT', '/v1/nodes/%s/states/provision' % NODE1['uuid'], {}, body)]
        self.assertEqual(expect, self.api.calls)
    ...
    def test_node_set_provision_state_with_deploysteps(self):
        ...
        target_state = 'active'
        self.mgr.set_provision_state(NODE1['uuid'], target_state, deploysteps=deploysteps)
    ...
    def test_node_set_provision_state_with_configdrive_and_deploysteps(self):
        deploysteps = [{'step': 'upgrade', 'interface': 'deploy'}]
        target_state = 'active'
        self.mgr.set_provision_state(NODE1['uuid'], target_state, configdrive={'user_data': ''}, deploysteps=deploysteps)
        body = {'target': target_state, 'configdrive': {'user_data': ''}, 'deploy_steps': deploysteps}
        expect = [('PUT', '/v1/nodes/%s/states/provision' % NODE1['uuid'], {}, body)]
        self.assertEqual(expect, self.api.calls)
    ...
    def test_node_states(self):
        ...
        expect = [('GET', '/v1/nodes/%s/states' % NODE1['uuid'], {}, None)]
        self.assertEqual(expect, self.api.calls)
        expected_fields = ['boot_mode', 'last_error', 'power_state', 'provision_state', 'target_power_state', 'target_provision_state', 'secure_boot']
        self.assertEqual(sorted(expected_fields), sorted(states.to_dict().keys()))
    ...
    def test_vendor_passthru_update(self, update_mock):
        ...
        kwargs = {'node_id': 'node_uuid', 'method': 'method', 'args': vendor_passthru_args}
    ...
    def test_vendor_passthru_get(self, get_mock):
        kwargs = {'node_id': 'node_uuid', 'method': 'method', 'http_method': 'GET'}
        final_path = 'node_uuid/vendor_passthru/method'
        self.mgr.vendor_passthru(**kwargs)
    ...
    def test_vendor_passthru_delete(self, delete_mock):
        kwargs = {'node_id': 'node_uuid', 'method': 'method', 'http_method': 'DELETE'}
        final_path = 'node_uuid/vendor_passthru/method'
        self.mgr.vendor_passthru(**kwargs)
    ...
    def test_vendor_passthru_unknown_http_method(self, delete_mock):
        kwargs = {'node_id': 'node_uuid', 'method': 'method', 'http_method': 'UNKNOWN'}
    ...
    def test_vif_attach_custom_fields(self, update_mock):
        kwargs = {'node_ident': NODE1['uuid'], 'vif_id': 'vif_id', 'foo': 'bar'}
        final_path = '%s/vifs' % NODE1['uuid']
        self.mgr.vif_attach(**kwargs)
        update_mock.assert_called_once_with(mock.ANY, final_path, {'id': 'vif_id', 'foo': 'bar'}, http_method='POST', os_ironic_api_version=None, global_request_id=None)
    ...
    def test_node_get_boot_device(self):
        ...
        expect = [('GET', '/v1/nodes/%s/management/boot_device' % NODE1['uuid'], {}, None)]
        self.assertEqual(expect, self.api.calls)
        self.assertEqual(BOOT_DEVICE, boot_device)

    def test_node_inject_nmi(self):
        self.mgr.inject_nmi(NODE1['uuid'])
        expect = [('PUT', '/v1/nodes/%s/management/inject_nmi' % NODE1['uuid'], {}, {})]
        self.assertEqual(expect, self.api.calls)
    ...
    def test_wait_for_provision_state_error(self, mock_get, mock_sleep):
        mock_get.side_effect = [self._fake_node_for_wait('deploying', target='active'), self._fake_node_for_wait('deploy failed', error='boom')]
        self.assertRaisesRegex(exc.StateTransitionFailed, 'boom', self.mgr.wait_for_provision_state, 'node', 'active')
        mock_get.assert_called_with(self.mgr, 'node', os_ironic_api_version=None, global_request_id=None)
    ...
    @mock.patch.object(time, 'sleep', autospec=True)
    @mock.patch.object(node.NodeManager, 'get', autospec=True)
    def test_wait_for_provision_state_unexpected_stable_state(self, mock_get, mock_sleep):
        mock_get.side_effect = [self._fake_node_for_wait('deploying', target='active'), self._fake_node_for_wait('available')]
        self.assertRaisesRegex(exc.StateTransitionFailed, 'available', self.mgr.wait_for_provision_state, 'node', 'active')
        mock_get.assert_called_with(self.mgr, 'node', os_ironic_api_version=None, global_request_id=None)
        self.assertEqual(2, mock_get.call_count)
        mock_sleep.assert_called_with(node._DEFAULT_POLL_INTERVAL)
        self.assertEqual(1, mock_sleep.call_count)
    ...
    @mock.patch.object(time, 'sleep', autospec=True)
    @mock.patch.object(node.NodeManager, 'get', autospec=True)
    def test_wait_for_provision_state_unexpected_stable_state_allowed(self, mock_get, mock_sleep):
        mock_get.side_effect = [self._fake_node_for_wait('deploying', target='active'), self._fake_node_for_wait('available'), self._fake_node_for_wait('deploying', target='active'), self._fake_node_for_wait('active')]
        self.mgr.wait_for_provision_state('node', 'active', fail_on_unexpected_state=False)
        mock_get.assert_called_with(self.mgr, 'node', os_ironic_api_version=None, global_request_id=None)
    ...
    def test_wait_for_provision_state_one_failed(self, mock_get, mock_sleep):
        mock_get.side_effect = [self._fake_node_for_wait('deploying', target='active'), self._fake_node_for_wait('deploying', target='active'), self._fake_node_for_wait('active'), self._fake_node_for_wait('deploy failed', error='boom')]
        self.assertRaisesRegex(exc.StateTransitionFailed, 'boom', self.mgr.wait_for_provision_state, ['node1', 'node2'], 'active')
        mock_get.assert_has_calls([mock.call(self.mgr, 'node1', os_ironic_api_version=None, global_request_id=None), mock.call(self.mgr, 'node2', os_ironic_api_version=None, global_request_id=None)], any_order=True)
        self.assertEqual(4, mock_get.call_count)
        mock_sleep.assert_called_with(node._DEFAULT_POLL_INTERVAL)
        self.assertEqual(1, mock_sleep.call_count)
    ...
    def test_node_add_trait(self):
        ...
        expect = [('PUT', '/v1/nodes/%s/traits/%s' % (NODE1['uuid'], trait), {}, None)]
        self.assertEqual(expect, self.api.calls)
    ...
    def test_node_remove_all_traits(self):
        ...
        expect = [('DELETE', '/v1/nodes/%s/traits' % NODE1['uuid'], {}, None)]
        self.assertEqual(expect, self.api.calls)
        self.assertIsNone(resp)
    ...
    def test_node_get_inventory(self):
        ...
        expect = [('GET', '/v1/nodes/%s/inventory' % NODE1['uuid'], {}, None)]
        self.assertEqual(expect, self.api.calls)
