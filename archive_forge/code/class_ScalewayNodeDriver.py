...
class ScalewayNodeDriver(NodeDriver):
    ...
    SNAPSHOT_STATE_MAP = {'snapshotting': VolumeSnapshotState.CREATING, 'available': VolumeSnapshotState.AVAILABLE, 'error': VolumeSnapshotState.ERROR}
    ...
    def list_sizes(self, region=None):
        ...
        return sorted((self._to_size(name, sizes[name], availability[name]) for name in sizes), key=lambda x: x.name)

    def _to_size(self, name, size, availability):
        min_disk = _to_lib_size(size['volumes_constraint']['min_size'] or 0) if size['volumes_constraint'] else 25
        max_disk = _to_lib_size(size['volumes_constraint']['max_size'] or 0) if size['volumes_constraint'] else min_disk
        extra = {'cores': size['ncpus'], 'monthly': size['monthly_price'], 'arch': size['arch'], 'baremetal': size['baremetal'], 'availability': availability['availability'], 'max_disk': max_disk, 'internal_bandwidth': int((size['network']['sum_internal_bandwidth'] or 0) / (1024 * 1024)), 'ipv6': size['network']['ipv6_support'], 'alt_names': size['alt_names']}
        return NodeSize(id=name, name=name, ram=int((size['ram'] or 0) / (1024 * 1024)), disk=min_disk, bandwidth=int((size['network']['sum_internet_bandwidth'] or 0) / (1024 * 1024)), price=size['hourly_price'], driver=self, extra=extra)
    ...
    def create_image(self, node, name, region=None):
        ...
        """
        data = {'organization': self.key, 'name': name, 'arch': node.extra['arch'], 'root_volume': node.extra['volumes']['0']['id']}
    ...
    def delete_image(self, node_image, region=None):
        ...
        :rtype: ``bool``
        """
        return self.connection.request('/images/%s' % node_image.id, region=region, method='DELETE').success()
    ...
    def _to_image(self, image):
        extra = {'arch': image['arch'], 'size': _to_lib_size(image.get('root_volume', {}).get('size', 0)) or 50, 'creation_date': parse_date(image['creation_date']), 'modification_date': parse_date(image['modification_date']), 'organization': image['organization']}
    ...
    def create_node(self, name, size, image, ex_volumes=None, ex_tags=None, region=None):
        ...
        :rtype: :class:`.Node`
        """
        data = {'name': name, 'organization': self.key, 'image': image.id, 'volumes': ex_volumes or {}, 'commercial_type': size.id, 'tags': ex_tags or []}
        ...
        while allocate_space < size.disk:
            ...
            data['volumes'][str(vol_num)] = {'name': '%s-%d' % (name, vol_num), 'organization': self.key, 'size': _to_api_size(bump), 'volume_type': 'l_ssd'}
            allocate_space += bump
        if allocate_space > size.extra.get('max_disk', size.disk):
            range = 'of %dGB' % size.disk if size.extra.get('max_disk', size.disk) == size.disk else 'between %dGB and %dGB' % (size.extra.get('max_disk', size.disk), size.disk)
            raise ProviderError(value='%s only supports a total volume size %s; tried %dGB' % (size.id, range, allocate_space), http_code=400, driver=self)
        response = self.connection.request('/servers', data=json.dumps(data), region=region, method='POST')
    ...
    def _action(self, server_id, action, region=None):
        return self.connection.request('/servers/%s/action' % server_id, region=region, data=json.dumps({'action': action}), method='POST').success()
    ...
    def _to_volume(self, volume):
        extra = {'organization': volume['organization'], 'volume_type': volume['volume_type'], 'creation_date': parse_date(volume['creation_date']), 'modification_date': parse_date(volume['modification_date'])}
        return StorageVolume(id=volume['id'], name=volume['name'], size=_to_lib_size(volume['size']), driver=self, extra=extra)
    ...
    def create_volume(self, size, name, region=None):
        ...
        """
        data = {'name': name, 'organization': self.key, 'volume_type': 'l_ssd', 'size': _to_api_size(size)}
    ...
    def create_volume_snapshot(self, volume, name, region=None):
        ...
        response = self.connection.request('/snapshots', region=region, data=json.dumps(data), method='POST')
    ...
    def destroy_volume(self, volume, region=None):
        ...
        :rtype: ``bool``
        """
        return self.connection.request('/volumes/%s' % volume.id, region=region, method='DELETE').success()
