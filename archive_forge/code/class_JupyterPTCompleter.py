...
class JupyterPTCompleter(Completer):
    ...
    def get_completions(self, document, complete_event):
        if not document.current_line.strip():
            return
        content = self.jup_completer.complete_request(code=document.text, cursor_pos=document.cursor_position)
        ...
        for m in content['matches']:
            yield Completion(m, start_position=start_pos, display_meta=meta.get(m, '?'))