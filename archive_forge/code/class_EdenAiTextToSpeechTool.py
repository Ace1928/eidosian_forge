...
class EdenAiTextToSpeechTool(EdenaiTool):
    ...
    description = 'A wrapper around edenai Services text to speech.Useful for when you need to convert text to speech.the output is a string representing the URL of the audio file,\n        or the path to the downloaded wav file '
    ...
    def _run(self, query: str, run_manager: Optional[CallbackManagerForToolRun]=None) -> str:
        """Use the tool."""
        all_params = {'text': query, 'language': self.language, 'option': self.voice, 'return_type': self.return_type, 'rate': self.rate, 'pitch': self.pitch, 'volume': self.volume, 'audio_format': self.audio_format, 'sampling_rate': self.sampling_rate, 'settings': self.voice_models}
