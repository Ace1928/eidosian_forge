...
class WatchDog(object):
    ...
    def __init__(self, timeout=-1, traceback_file=sys.stdout, on_triggered=None):
        if os.environ.get('TF_CLUSTER_COORDINATOR_WATCH_DOG_TIMEOUT', '').isnumeric():
        ...
        self._stopped = False
        if timeout > 0:
            self._watchdog_thread = threading.Thread(target=self._watchdog_function, name='WatchDog', daemon=True)
