...
class ForwardSubstitution(Transformation):
    ...
    def __init__(self):
        ...
        super(ForwardSubstitution, self).__init__(LazynessAnalysis, UseDefChains, DefUseChains, Ancestors, CFG, Literals)
        self.to_remove = None
    ...
    def visit_Name(self, node):
        ...
        if node not in self.use_def_chains:
            if __debug__:
                ...
                assert any((isinstance(p, OMPDirective) for p in self.ancestors[node]))
            return node
        ...
        if sum((1 for d in self.locals if d.name() == dnode.id)) > 1:
            return node
        fwd = dnode in self.literals and isfinite(self.lazyness_analysis[dnode.id])
        ...
        if isinstance(parent, ast.Assign):
            ...
            if dnode in self.literals:
                ...
                node_stmt = next(reversed([s for s in self.ancestors[node] if isinstance(s, ast.stmt)]))
                all_paths = graph.all_simple_paths(self.cfg, parent, node_stmt)
                for path in all_paths:
                    for stmt in path[1:-1]:
                        assigned_ids = {n.id for n in self.gather(IsAssigned, stmt)}
