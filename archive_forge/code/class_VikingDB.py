...
class VikingDB(VectorStore):
    ...
    def __init__(self, embedding_function: Embeddings, collection_name: str='LangChainCollection', connection_args: Optional[VikingDBConfig]=None, index_params: Optional[dict]=None, drop_old: Optional[bool]=False, **kwargs: Any):
        try:
            from volcengine.viking_db import Collection, VikingDBService
        except ImportError:
            raise ValueError('Could not import volcengine python package. Please install it with `pip install --upgrade volcengine`.')
        ...
        self.service = VikingDBService(connection_args.host, connection_args.region, connection_args.ak, connection_args.sk, connection_args.scheme)
        try:
    ...
    def _create_index(self) -> None:
        ...
        self.index = self.service.create_index(self.collection_name, self.index_name, vector_index=vector_index, cpu_quota=cpu_quota, partition_by=partition_by, scalar_index=scalar_index)

    def add_texts(self, texts: List[str], metadatas: Optional[List[dict]]=None, batch_size: int=1000, **kwargs: Any) -> List[str]:
        ...
        for index in range(len(embeddings)):
            ...
            field = {'text': texts[index], 'primary_key': primary_key, 'vector': embeddings[index]}
    ...
    def similarity_search(self, query: str, params: Optional[dict]=None, **kwargs: Any) -> List[Document]:
        """Perform a similarity search against the query string."""
        res = self.similarity_search_with_score(query=query, params=params, **kwargs)
        return [doc for doc, _ in res]

    def similarity_search_with_score(self, query: str, params: Optional[dict]=None, **kwargs: Any) -> List[Tuple[Document, float]]:
        """Perform a search on a query string and return results with score."""
        embedding = self.embedding_func.embed_query(query)
        res = self.similarity_search_with_score_by_vector(embedding=embedding, params=params, **kwargs)
        return res

    def similarity_search_by_vector(self, embedding: List[float], params: Optional[dict]=None, **kwargs: Any) -> List[Document]:
        """Perform a similarity search against the query string."""
        res = self.similarity_search_with_score_by_vector(embedding=embedding, params=params, **kwargs)
        return [doc for doc, _ in res]

    def similarity_search_with_score_by_vector(self, embedding: List[float], params: Optional[dict]=None, **kwargs: Any) -> List[Tuple[Document, float]]:
        """Perform a search on a query string and return results with score."""
        if self.collection is None:
        ...
        res = self.index.search_by_vector(embedding, filter=filter, limit=limit, output_fields=output_fields, partition=partition)
        ret = []
        for item in res:
    ...
    def max_marginal_relevance_search(self, query: str, k: int=4, lambda_mult: float=0.5, params: Optional[dict]=None, **kwargs: Any) -> List[Document]:
        """Perform a search and return results that are reordered by MMR."""
        embedding = self.embedding_func.embed_query(query)
        return self.max_marginal_relevance_search_by_vector(embedding=embedding, k=k, lambda_mult=lambda_mult, params=params, **kwargs)

    def max_marginal_relevance_search_by_vector(self, embedding: List[float], k: int=4, lambda_mult: float=0.5, params: Optional[dict]=None, **kwargs: Any) -> List[Document]:
        """Perform a search and return results that are reordered by MMR."""
        if self.collection is None:
            logger.debug('No existing collection to search.')
            return []
    ...
    @classmethod
    def from_texts(cls, texts: List[str], embedding: Embeddings, connection_args: Optional[VikingDBConfig]=None, metadatas: Optional[List[dict]]=None, collection_name: str='LangChainCollection', index_params: Optional[dict]=None, drop_old: bool=False, **kwargs: Any):
        """Create a collection, indexes it and insert data."""
        if connection_args is None:
            raise Exception('VikingDBConfig does not exists')
        vector_db = cls(embedding_function=embedding, collection_name=collection_name, connection_args=connection_args, index_params=index_params, drop_old=drop_old, **kwargs)
