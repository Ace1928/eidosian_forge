...
class KeyTests(unittest.TestCase):
    ...
    def setUp(self):
        self.rsaObj = keys.Key._fromRSAComponents(n=keydata.RSAData['n'], e=keydata.RSAData['e'], d=keydata.RSAData['d'], p=keydata.RSAData['p'], q=keydata.RSAData['q'], u=keydata.RSAData['u'])._keyObject
        self.dsaObj = keys.Key._fromDSAComponents(y=keydata.DSAData['y'], p=keydata.DSAData['p'], q=keydata.DSAData['q'], g=keydata.DSAData['g'], x=keydata.DSAData['x'])._keyObject
        self.ecObj = keys.Key._fromECComponents(x=keydata.ECDatanistp256['x'], y=keydata.ECDatanistp256['y'], privateValue=keydata.ECDatanistp256['privateValue'], curve=keydata.ECDatanistp256['curve'])._keyObject
        self.ecObj384 = keys.Key._fromECComponents(x=keydata.ECDatanistp384['x'], y=keydata.ECDatanistp384['y'], privateValue=keydata.ECDatanistp384['privateValue'], curve=keydata.ECDatanistp384['curve'])._keyObject
        self.ecObj521 = keys.Key._fromECComponents(x=keydata.ECDatanistp521['x'], y=keydata.ECDatanistp521['y'], privateValue=keydata.ECDatanistp521['privateValue'], curve=keydata.ECDatanistp521['curve'])._keyObject
        if ED25519_SUPPORTED:
            self.ed25519Obj = keys.Key._fromEd25519Components(a=keydata.Ed25519Data['a'], k=keydata.Ed25519Data['k'])._keyObject
        self.rsaSignature = b"\x00\x00\x00\x07ssh-rsa\x00\x00\x01\x00~Y\xa3\xd7\xfdW\xc6pu@\xd81\xa1S\xf3O\xdaE\xf4/\x1ex\x1d\xf1\x9a\xe1G3\xd9\xd6U\x1f\x8c\xd9\x1b\x8b\x90\x0e\x8a\xc1\x91\xd8\x0cd\xc9\x0c\xe7\xb2\xc9,'=\x15\x1cQg\xe7x\xb5j\xdbI\xc0\xde\xafb\xd7@\xcar\x0b\xce\xa3zM\x151q5\xde\xfa\x0c{wjKN\x88\xcbC\xe5\x89\xc3\xf9i\x96\x91\xdb\xca}\xdbR\x1a\x13T\xf9\x0cDJH\x0b\x06\xcfl\xf3\x13[\x82\xa2\x9d\x93\xfd\x8e\xce|\xfb^n\xd4\xed\xe2\xd1\x8a\xb7aY\x9bB\x8f\xa4\xc7\xbe7\xb5\x0b9j\xa4.\x87\x13\xf7\xf0\xda\xd7\xd2\xf9\x1f9p\xfd?\x18\x0f\xf2N\x9b\xcf/\x1e)\n>A\x19\xc2\xb5j\xf9UW\xd4\xae\x87B\xe6\x99t\xa2y\x90\x98\xa2\xaaf\xcb\x86\xe5k\xe3\xce\xe0u\x1c\xeb\x93\x1aN\x88\xc9\x93Y\xc3.V\xb1L44`C\xc7\xa66\xaf\xfa\x7f\x04Y\x92\xfa\xa4\x1a\x18%\x19\xd5 4^\xb9rY\xba \x01\xf9.\x89%H\xbe\x1c\x83A\x96"
        self.dsaSignature = b'\x00\x00\x00\x07ssh-dss\x00\x00\x00(?\xc7\xeb\x86;\xd5TFA\xb4\xdf\x0c\xc4E@4,d\xbc\t\xd9\xae\xdd[\xed-\x82nQ\x8cf\x9b\xe8\xe1jrg\x84p<'
    ...
    def test_fromOpenSSH_with_whitespace(self):
        ...
        self.assertEqual(keys.Key.fromString(privateDSAData), keys.Key.fromString(privateDSAData + b'\n'))
    ...
    def test_fingerprint_md5_hex(self):
        ...
        """
        self.assertEqual(keys.Key(self.rsaObj).fingerprint(keys.FingerprintFormats.MD5_HEX), '85:25:04:32:58:55:96:9f:57:ee:fb:a8:1a:ea:69:da')
        self.assertEqual(keys.Key(self.dsaObj).fingerprint(keys.FingerprintFormats.MD5_HEX), '63:15:b3:0e:e6:4f:50:de:91:48:3d:01:6b:b3:13:c1')
    ...
    def test_type(self):
        ...
        self.assertEqual(keys.Key(self.ecObj).sshType(), keydata.ECDatanistp256['curve'])
        if ED25519_SUPPORTED:
    ...
    def test_privateBlobEd25519(self):
        ...
        from cryptography.hazmat.primitives import serialization
        publicBytes = self.ed25519Obj.public_key().public_bytes(serialization.Encoding.Raw, serialization.PublicFormat.Raw)
        privateBytes = self.ed25519Obj.private_bytes(serialization.Encoding.Raw, serialization.PrivateFormat.Raw, serialization.NoEncryption())
        self.assertEqual(keys.Key(self.ed25519Obj).privateBlob(), common.NS(b'ssh-ed25519') + common.NS(publicBytes) + common.NS(privateBytes + publicBytes))
    ...
    def test_signAndVerifyRSASHA512(self):
        ...
        self.assertIsNone(key._keyObject.public_key().verify(common.getNS(signature)[0], data, padding.PKCS1v15(), hashes.SHA512()))
