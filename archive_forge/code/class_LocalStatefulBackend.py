...
class LocalStatefulBackend(BaseStatefulBackend):
    ...
    def __init__(self, name: Optional[str]=None, file_path: Optional[File]=None, encoding: Optional[str]=None, serializer: Optional[str]=None, serializer_kwargs: Optional[Dict[str, Any]]=None, base_key: Optional[str]=None, async_enabled: Optional[bool]=False, settings: Optional['BaseSettings']=None, **kwargs):
        """
    ...
    def get_all_keys(self, exclude_base_key: Optional[bool]=False, **kwargs) -> List[str]:
        ...
        """
        self.sync()
        keys = [key for key in self.cache.keys() if key.startswith(self.base_key)] if self.base_key else list(self.cache.keys())
        if exclude_base_key:
    ...
    def get_all_data(self, exclude_base_key: Optional[bool]=False, **kwargs) -> Dict[str, Any]:
        ...
        data = {key: self.decode_value(value) for key, value in self.cache.items() if key in all_keys}
        if exclude_base_key:
    ...
    def get_all_values(self, **kwargs) -> Iterable[Any]:
        ...
        return [self.decode_value(value, **kwargs) for key, value in self.cache.items() if key in all_keys]
