...
class CircuitSerializer(serializer.Serializer):
    ...
    def serialize(self, program: cirq.AbstractCircuit, msg: Optional[v2.program_pb2.Program]=None, *, arg_function_language: Optional[str]=None) -> v2.program_pb2.Program:
        ...
        msg.language.arg_function_language = arg_function_language or arg_func_langs.MOST_PERMISSIVE_LANGUAGE
        self._serialize_circuit(program, msg.circuit, arg_function_language=arg_function_language, constants=msg.constants, raw_constants=raw_constants)
        return msg
    ...
    def deserialize(self, proto: v2.program_pb2.Program) -> cirq.Circuit:
        ...
        if proto.language.gate_set != self.name:
            raise ValueError(f'Gate set in proto was {proto.language.gate_set} but expected {self.name}')
        which = proto.WhichOneof('program')
        arg_func_language = proto.language.arg_function_language or arg_func_langs.MOST_PERMISSIVE_LANGUAGE
        if which == 'circuit':
            ...
            for constant in proto.constants:
                ...
                if which_const == 'string_value':
                    ...
                    circuit = self._deserialize_circuit(constant.circuit_value, arg_function_language=arg_func_language, constants=proto.constants, deserialized_constants=deserialized_constants)
            ...
            circuit = self._deserialize_circuit(proto.circuit, arg_function_language=arg_func_language, constants=proto.constants, deserialized_constants=deserialized_constants)
            return circuit
        if which == 'schedule':
