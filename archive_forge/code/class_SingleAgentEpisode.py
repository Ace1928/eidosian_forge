...
class SingleAgentEpisode:
    ...
    def __init__(self, id_: Optional[str]=None, *, observations: List[ObsType]=None, observation_space: Optional[gym.Space]=None, actions: List[ActType]=None, action_space: Optional[gym.Space]=None, rewards: List[SupportsFloat]=None, infos: List[Dict]=None, terminated: bool=False, truncated: bool=False, extra_model_outputs: Optional[Dict[str, Any]]=None, render_images: Optional[List[np.ndarray]]=None, t_started: Optional[int]=None, len_lookback_buffer: Optional[int]=0) -> 'SingleAgentEpisode':
        ...
        self.observations = BufferWithInfiniteLookback(data=observations, lookback=self._len_lookback_buffer, space=observation_space)
        self.action_space = action_space
        self.actions = BufferWithInfiniteLookback(data=actions, lookback=self._len_lookback_buffer, space=action_space)
        self.rewards = BufferWithInfiniteLookback(data=rewards, lookback=self._len_lookback_buffer, space=gym.spaces.Box(float('-inf'), float('inf'), (), np.float32))
        self.infos = BufferWithInfiniteLookback(data=infos, lookback=self._len_lookback_buffer)
        self.is_terminated = terminated
        self.is_truncated = truncated
        self.extra_model_outputs = defaultdict(functools.partial(BufferWithInfiniteLookback, lookback=self._len_lookback_buffer))
        ...
        self.t = (len(rewards) if rewards is not None else 0) - self._len_lookback_buffer + self.t_started
        self.validate()
    ...
    def concat_episode(self, episode_chunk: 'SingleAgentEpisode'):
        ...
        for model_out_key in episode_chunk.extra_model_outputs.keys():
            self.extra_model_outputs[model_out_key].extend(episode_chunk.get_extra_model_outputs(model_out_key))
        self.validate()

    def add_env_reset(self, observation: ObsType, infos: Optional[Dict]=None, *, render_image: Optional[np.ndarray]=None) -> None:
        ...
        if self.observation_space is not None:
            assert self.observation_space.contains(observation), f"`observation` {observation} does NOT fit SingleAgentEpisode's observation_space: {self.observation_space}!"
    ...
    def add_env_step(self, observation: ObsType, action: ActType, reward: SupportsFloat, infos: Optional[Dict[str, Any]]=None, *, terminated: bool=False, truncated: bool=False, render_image: Optional[np.ndarray]=None, extra_model_outputs: Optional[Dict[str, Any]]=None) -> None:
        ...
        """
        assert not self.is_done, 'The agent is already done: no data can be added to its episode.'
        self.observations.append(observation)
        ...
        if self.t % 50:
            ...
            if self.action_space is not None:
                assert self.action_space.contains(action), f"`action` {action} does NOT fit SingleAgentEpisode's action_space: {self.action_space}!"
    ...
    def cut(self, len_lookback_buffer: int=0) -> 'SingleAgentEpisode':
        ...
        indices_rest = slice(-len_lookback_buffer, None) if len_lookback_buffer > 0 else slice(None, 0)
        return SingleAgentEpisode(id_=self.id_, observations=self.get_observations(indices=indices_obs_and_infos), infos=self.get_infos(indices=indices_obs_and_infos), actions=self.get_actions(indices=indices_rest), rewards=self.get_rewards(indices=indices_rest), extra_model_outputs={k: self.get_extra_model_outputs(k, indices_rest) for k in self.extra_model_outputs.keys()}, t_started=self.t)

    def get_observations(self, indices: Optional[Union[int, List[int], slice]]=None, *, neg_indices_left_of_zero: bool=False, fill: Optional[float]=None, one_hot_discrete: bool=False) -> Any:
        ...
        """
        return self.observations.get(indices=indices, neg_indices_left_of_zero=neg_indices_left_of_zero, fill=fill, one_hot_discrete=one_hot_discrete)

    def get_infos(self, indices: Optional[Union[int, List[int], slice]]=None, *, neg_indices_left_of_zero: bool=False, fill: Optional[Any]=None) -> Any:
        ...
        Returns:
            ...
            As single item (B=0 -> no additional 0-axis) if `indices` is a single int.
        """
        return self.infos.get(indices=indices, neg_indices_left_of_zero=neg_indices_left_of_zero, fill=fill)

    def get_actions(self, indices: Optional[Union[int, List[int], slice]]=None, *, neg_indices_left_of_zero: bool=False, fill: Optional[float]=None, one_hot_discrete: bool=False) -> Any:
        ...
        Returns:
            ...
            As single item (B=0 -> no additional 0-axis) if `indices` is a single int.
        """
        return self.actions.get(indices=indices, neg_indices_left_of_zero=neg_indices_left_of_zero, fill=fill, one_hot_discrete=one_hot_discrete)

    def get_rewards(self, indices: Optional[Union[int, List[int], slice]]=None, *, neg_indices_left_of_zero: bool=False, fill: Optional[Any]=None) -> Any:
        ...
        Returns:
            ...
            As single item (B=0 -> no additional 0-axis) if `indices` is a single int.
        """
        return self.rewards.get(indices=indices, neg_indices_left_of_zero=neg_indices_left_of_zero, fill=fill)

    def get_extra_model_outputs(self, key: str, indices: Optional[Union[int, List[int], slice]]=None, *, neg_indices_left_of_zero: bool=False, fill: Optional[Any]=None) -> Any:
        ...
        if isinstance(value, BufferWithInfiniteLookback):
            return value.get(indices=indices, neg_indices_left_of_zero=neg_indices_left_of_zero, fill=fill)
        return BufferWithInfiniteLookback(value).get(indices, fill=fill, neg_indices_left_of_zero=neg_indices_left_of_zero)
    ...
    def slice(self, slice_: slice) -> 'SingleAgentEpisode':
        ...
        slice_ = slice((slice_.start or 0) - self._len_lookback_buffer, slice_.stop, slice_.step)
        slice_obs_infos = slice(slice_.start, ((slice_.stop if slice_.stop != -1 else len(self) - 1) or len(self)) + 1, slice_.step)
        return SingleAgentEpisode(id_=self.id_, observations=self.get_observations(slice_obs_infos, neg_indices_left_of_zero=neg_indices_left_of_zero), infos=self.get_infos(slice_obs_infos, neg_indices_left_of_zero=neg_indices_left_of_zero), actions=self.get_actions(slice_, neg_indices_left_of_zero=neg_indices_left_of_zero), rewards=self.get_rewards(slice_, neg_indices_left_of_zero=neg_indices_left_of_zero), extra_model_outputs={k: self.get_extra_model_outputs(k, slice_, neg_indices_left_of_zero=neg_indices_left_of_zero) for k in self.extra_model_outputs}, terminated=self.is_terminated if keep_done else False, truncated=self.is_truncated if keep_done else False, t_started=t_started, len_lookback_buffer=self._len_lookback_buffer)
    ...
    def get_data_dict(self):
        ...
        return dict({SampleBatch.TERMINATEDS: terminateds, SampleBatch.TRUNCATEDS: truncateds, SampleBatch.T: t, SampleBatch.EPS_ID: eps_id, SampleBatch.OBS: self.get_observations(slice(None, -1)), SampleBatch.INFOS: self.get_infos(), SampleBatch.ACTIONS: self.get_actions(), SampleBatch.REWARDS: self.get_rewards()}, **{k: self.get_extra_model_outputs(k) for k in self.extra_model_outputs.keys()})
    ...
    def __getitem__(self, item: slice) -> 'SingleAgentEpisode':
        ...
        if isinstance(item, slice):
            ...
            raise NotImplementedError(f"SingleAgentEpisode does not support getting item '{item}'! Only slice objects allowed with the syntax: `episode[a:b]`.")