...
class CompositeOp(Operator):
    ...
    def __repr__(self):
        return f' {self._op_symbol} '.join([f'({op})' if op.arithmetic_depth > 0 else f'{op}' for op in self])
    ...
    def eigvals(self):
        ...
        for ops in self.overlapping_ops:
            if len(ops) == 1:
                eigvals.append(qml.utils.expand_vector(ops[0].eigvals(), list(ops[0].wires), list(self.wires)))
            else:
                tmp_composite = self.__class__(*ops)
                eigvals.append(qml.utils.expand_vector(tmp_composite.eigendecomposition['eigval'], list(tmp_composite.wires), list(self.wires)))
    ...
    def label(self, decimals=None, base_label=None, cache=None):
        ...
        **Example (using the Sum composite operator)**
        ...
        if base_label is not None:
            ...
            return self._op_symbol.join((_label(op, decimals, lbl, cache) for op, lbl in zip(self, base_label)))
    ...
    def hash(self):
        if self._hash is None:
            self._hash = hash((str(self.name), str([factor.hash for factor in self._sort(self.operands)])))
        return self._hash
