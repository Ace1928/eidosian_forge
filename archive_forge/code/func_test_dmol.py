def test_dmol():
    ...
    def check(atoms, ref_atoms, dist_tol=1e-06):
        assert all(atoms.pbc == ref_atoms.pbc), (atoms.pbc, ref_atoms.pbc)
        if all(atoms.pbc):
            assert abs(atoms.cell - ref_atoms.cell).max() < dist_tol, atoms.cell - ref_atoms.cell
        assert abs(atoms.positions - ref_atoms.positions).max() < dist_tol, atoms.positions - ref_atoms.positions
