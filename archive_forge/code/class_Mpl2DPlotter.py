...
class Mpl2DPlotter(BasePlotter):
    ...
    def draw(self):
        ...
        axis_config = self.canvas.layout['time_axis_map'](time_window=self.canvas.time_range, axis_breaks=self.canvas.time_breaks, dt=self.canvas.device.dt)
        current_y = 0
        ...
        for chart in self.canvas.charts:
            ...
            for _, data in chart.collections:
                ...
                if isinstance(data, drawings.LineData):
                    ...
                    text = f'${data.latex}$' if data.latex else data.text
                    ...
                    xy = (x[0], y[0])
                    box = Rectangle(xy, width=x[1] - x[0], height=y[1] - y[0], fill=True, **data.styles)
                    self.ax.add_patch(box)
                else:
                    raise VisualizationError('Data {name} is not supported by {plotter}'.format(name=data, plotter=self.__class__.__name__))
            for pos in axis_config.axis_break_pos:
                self.ax.text(x=pos, y=current_y, s='//', ha='center', va='center', zorder=self.canvas.formatter['layer.axis_label'], fontsize=self.canvas.formatter['text_size.axis_break_symbol'], rotation=180)
            current_y += chart.vmin - margin_y
        current_y += margin_y
        y_max = self.canvas.formatter['margin.top']
        y_min = current_y - self.canvas.formatter['margin.bottom']
        for pos in axis_config.axis_break_pos:
            self.ax.plot([pos, pos], [y_min, y_max], zorder=self.canvas.formatter['layer.fill_waveform'] + 1, linewidth=self.canvas.formatter['line_width.axis_break'], color=self.canvas.formatter['color.background'])
        ...
        if self.canvas.fig_title:
            self.ax.text(x=axis_config.window[0], y=y_max, s=self.canvas.fig_title, ha='left', va='bottom', zorder=self.canvas.formatter['layer.fig_title'], color=self.canvas.formatter['color.fig_title'], size=self.canvas.formatter['text_size.fig_title'])
