...
def handle_migration(conf, key_mgr):
    try:
        ...
        pass
    if conf.key_manager.fixed_key is not None and (not conf.key_manager.backend.endswith('ConfKeyManager')):
        LOG.warning('Using MigrationKeyManager to provide support for legacy fixed_key encryption')
