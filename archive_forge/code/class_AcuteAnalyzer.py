...
class AcuteAnalyzer(object):
    ...
    def __init__(self, opt: Dict, remove_failed: bool=True):
        ...
        self.db_path = os.path.join(os.path.dirname(parlai_filepath), 'mturk', 'run_data', f'pmt_{('sb' if opt['is_sandbox'] else '')}data.db')
    ...
    def save_results(self, path: str=None):
        ...
        _print_progress(f'To visualize signficance result, try cat {_path('significance.csv')} | column -t -s, | less -S')
        with open(_path('grid.csv'), 'w') as f:
            f.write(self.get_win_fractions().to_csv(index=True))
        _print_progress(f"To visualize grid result, try cat {_path('grid.csv')} | sed 's/,/ ,/g' | column -t -s, | less -S")
        if os.path.exists(self.pairings_filepath):
            ...
            _print_progress(f'To visualize only conversations with reasons provided, open {_path('reason.html')} in a browser')
            with open(_path('all.html'), 'w') as f:
                f.write(self.rendered_without_reasons.data)
            _print_progress(f'To visualize all conversations, open {_path('all.html')} in a browser')