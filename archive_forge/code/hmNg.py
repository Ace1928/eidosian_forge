...
class SnakeGameAI:
    ...
    def get_next_direction(self) -> Tuple[int, int]:
        """
        Determine the next direction for the snake to move based on advanced pathfinding strategies.
        This method utilizes the A* search algorithm to dynamically calculate the optimal path to the food
        and then to the tail, considering the need to avoid collisions with the snake itself and the boundaries,
        while also attempting to pack the snake as densely as possible in a zigzag pattern to maximize space usage.

        Returns:
            Tuple[int, int]: The direction vector (dx, dy) indicating the next move.
        """
        # Refresh paths if not already calculated
        if not self.path:
            self.path = self.a_star_search(self.snake[0], self.food)
            self.tail_path = self.a_star_search(self.food, self.snake[-1])

        # Determine next position from the path to food
        if len(self.path) > 1:
            next_pos = self.path.pop(0)
            dx, dy = next_pos[0] - self.snake[0][0], next_pos[1] - self.snake[0][1]
            return dx, dy

        # If path to food is exhausted, use the path to tail
        if len(self.tail_path) > 1:
            next_pos = self.tail_path.pop(0)
            dx, dy = next_pos[0] - self.snake[0][0], next_pos[1] - self.snake[0][1]
            return dx, dy

        # If all paths are exhausted, recalculate both paths
        self.path = self.a_star_search(self.snake[0], self.food)
        self.tail_path = self.a_star_search(self.food, self.snake[-1])

        # Attempt to move again using the new paths
        if len(self.path) > 1:
            next_pos = self.path.pop(0)
            dx, dy = next_pos[0] - self.snake[0][0], next_pos[1] - self.snake[0][1]
            return dx, dy

        # Default to the current direction if no valid move is found
