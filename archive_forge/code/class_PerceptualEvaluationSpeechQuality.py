...
class PerceptualEvaluationSpeechQuality(Metric):
    ...
    def __init__(self, fs: int, mode: str, n_processes: int=1, **kwargs: Any) -> None:
        ...
        if not _PESQ_AVAILABLE:
            raise ModuleNotFoundError('PerceptualEvaluationSpeechQuality metric requires that `pesq` is installed. Either install as `pip install torchmetrics[audio]` or `pip install pesq`.')
    ...
    def update(self, preds: Tensor, target: Tensor) -> None:
        ...
        pesq_batch = perceptual_evaluation_speech_quality(preds, target, self.fs, self.mode, False, self.n_processes).to(self.sum_pesq.device)
