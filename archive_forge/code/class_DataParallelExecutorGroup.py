...
class DataParallelExecutorGroup(object):
    ...
    def __init__(self, symbol, contexts, workload, data_shapes, label_shapes, param_names, for_training, inputs_need_grad, shared_group=None, logger=logging, fixed_param_names=None, grad_req='write', state_names=None, group2ctxs=None):
        ...
        self.output_layouts = [DataDesc.get_batch_axis(self.symbol[index].attr('__layout__')) for index in range(self.num_outputs)]
    ...
    def decide_slices(self, data_shapes):
        ...
        for (name, shape), axis in zip(data_shapes, major_axis):
            ...
            if self.batch_size is not None:
                assert batch_size == self.batch_size, 'all data must have the same batch size: ' + 'batch_size = %d, but ' % self.batch_size + '%s has shape %s' % (name, shape)
            else:
    ...
    def get_outputs(self, merge_multi_context=True, begin=0, end=None):
        ...
        outputs = [[exec_.outputs[i] for exec_ in self.execs] for i in range(begin, end)]
            ...
            if not pre_sliced:
                ...
                for label, axis in zip(labels, self.label_layouts):
                    if axis == 0:
                        ...
                        label_my_slice = nd.slice_axis(label, axis=axis, begin=islice.start, end=islice.stop).as_in_context(label.context)
