...
def endpoint_control_registered_forticlient(data, fos, check_mode=False):
    ...
    if state == 'present' or state is True:
        return fos.set('endpoint-control', 'registered-forticlient', data=filtered_data, vdom=vdom)
    elif state == 'absent':
        return fos.delete('endpoint-control', 'registered-forticlient', mkey=filtered_data['uid'], vdom=vdom)
    else:
