...
class IBMSVCMigrate(object):
    ...
    def __init__(self):
        ...
        argument_spec.update(dict(type_of_migration=dict(type='str', required=False, default='across_clusters', choices=['across_clusters', 'across_pools']), new_pool=dict(type='str', required=False), source_volume=dict(type='str', required=False), target_volume=dict(type='str', required=False), state=dict(type='str', choices=['initiate', 'switch', 'cleanup']), remote_pool=dict(type='str', required=False), replicate_hosts=dict(type='bool', default=False), relationship_name=dict(type='str', required=False), remote_cluster=dict(type='str', required=False), remote_token=dict(type='str', required=False, no_log=True), remote_validate_certs=dict(type='bool', default=False), remote_username=dict(type='str', required=False), remote_password=dict(type='str', required=False, no_log=True)))
    ...
    def basic_checks(self):
        ...
        valid_params = {}
        valid_params['initiate'] = ['source_volume', 'remote_cluster', 'target_volume', 'replicate_hosts', 'remote_username', 'remote_password', 'relationship_name', 'remote_token', 'remote_pool', 'remote_validate_certs']
                    ...
                    if param == 'remote_username' or param == 'remote_password':
                        if not self.remote_username or not self.remote_password:
                            if self.remote_token:
                                ...
                                self.module.fail_json(msg='You must pass in either pre-acquired remote_token or remote_username/remote_password to generate new token.')
    ...
    def delete(self):
        ...
        if result == '':
            ...
            self.changed = True
            self.log('delete the source volume %s with result message %s', self.source_volume, result['message'])
        else:
            self.module.fail_json(msg='Failed to delete the volume [%s]' % self.source_volume)
