...
def legacy_search_issues(self, state: str, keyword: str) -> list[Issue]:
    ...
    headers, data = self._requester.requestJsonAndCheck('GET', f'/legacy/issues/search/{self.owner.login}/{self.name}/{state}/{urllib.parse.quote(keyword)}')
    return [github.Issue.Issue(self._requester, headers, self._legacy_convert_issue(element), completed=False) for element in data['issues']]