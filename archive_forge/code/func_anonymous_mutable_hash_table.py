...
def anonymous_mutable_hash_table(key_dtype: TV_AnonymousMutableHashTable_key_dtype, value_dtype: TV_AnonymousMutableHashTable_value_dtype, name=None) -> _atypes.TensorFuzzingAnnotation[_atypes.Resource]:
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'AnonymousMutableHashTable', name, 'key_dtype', key_dtype, 'value_dtype', value_dtype)
            return _result
            ...
            pass
        try:
            return anonymous_mutable_hash_table_eager_fallback(key_dtype=key_dtype, value_dtype=value_dtype, name=name, ctx=_ctx)
        except _core._SymbolicException:
            pass
    key_dtype = _execute.make_type(key_dtype, 'key_dtype')
    value_dtype = _execute.make_type(value_dtype, 'value_dtype')
    _, _, _op, _outputs = _op_def_library._apply_op_helper('AnonymousMutableHashTable', key_dtype=key_dtype, value_dtype=value_dtype, name=name)
    _result = _outputs[:]
    if _execute.must_record_gradient():
        _attrs = ('key_dtype', _op._get_attr_type('key_dtype'), 'value_dtype', _op._get_attr_type('value_dtype'))
        _inputs_flat = _op.inputs
        _execute.record_gradient('AnonymousMutableHashTable', _inputs_flat, _attrs, _result)
    _result, = _result
    return _result