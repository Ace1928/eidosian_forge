...
class PBES2(object):
    ...
    def encrypt(data, passphrase, protection, prot_params=None, randfunc=None):
        ...
        pattern = re.compile('^(PBKDF2WithHMAC-([0-9A-Z-]+)|scrypt)And([0-9A-Z-]+)$')
        ...
        if pbkdf == 'pbkdf2':
            ...
            kdf_info = DerSequence([DerObjectId(_OID_PBKDF2), pbkdf2_params])
        elif pbkdf == 'scrypt':
            ...
            kdf_info = DerSequence([DerObjectId(_OID_SCRYPT), DerSequence([DerOctetString(salt), DerInteger(count), DerInteger(scrypt_r), DerInteger(scrypt_p)])])
        else:
        ...
        enc_private_key_info = DerSequence([DerSequence([DerObjectId(_OID_PBES2), DerSequence([kdf_info, enc_info])]), DerOctetString(encrypted_data)])
