...
class Prefix_lists(ResourceModule):
    ...
    def __init__(self, module):
        super(Prefix_lists, self).__init__(empty_fact_val=[], facts_module=Facts(module), module=module, resource='prefix_lists', tmplt=Prefix_listsTemplate())
        self.plist_parsers = ['name', 'description']
        self.entries_parsers = ['sequence', 'action', 'rule_description', 'ge', 'le', 'prefix']
    ...
    def _compare(self, want, have):
        ...
        if self.state in ['overridden', 'deleted']:
            for h in hplists.values():
                self.commands.append('delete policy prefix-{0} {1}'.format('list' if h['afi'] == 'ipv4' else 'list6', h['name']))

    def _compare_plists(self, want, have):
        for wk, wentry in iteritems(want):
            hentry = have.pop(wk, {})
            self.compare(parsers=self.plist_parsers, want=wentry, have=hentry)
    ...
    def _compare_rules(self, want, have):
        for wr, wrule in iteritems(want):
            hrule = have.pop(wr, {})
            self.compare(parsers=self.entries_parsers, want=wrule, have=hrule)
        for hr in have.values():
            self.commands.append('delete policy prefix-{0} {1} rule {2}'.format('list' if hr['afi'] == 'ipv4' else 'list6', hr['name'], hr['sequence']))
