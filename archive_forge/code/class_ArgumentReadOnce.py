...
class ArgumentReadOnce(ModuleAnalysis):
    ...
    def visit_Assign(self, node):
        dep = self.generic_visit(node)
        local = [self.local_effect(t, 2) for t in node.targets if isinstance(t, ast.Subscript)]
    ...
    def visit_For(self, node):
        ...
        return lambda ctx: iter_local(ctx) + iter_deps(ctx) + 2 * sum((l(ctx) for l in body_deps)) + sum((l(ctx) for l in else_deps))

    def visit_While(self, node):
        test_deps = self.visit(node.test)
        body_deps = [self.visit(stmt) for stmt in node.body]
        else_deps = [self.visit(stmt) for stmt in node.orelse]
        return lambda ctx: test_deps(ctx) + 2 * sum((l(ctx) for l in body_deps)) + sum((l(ctx) for l in else_deps))

    def visit_If(self, node):
        test_deps = self.visit(node.test)
        body_deps = [self.visit(stmt) for stmt in node.body]
        else_deps = [self.visit(stmt) for stmt in node.orelse]
        return lambda ctx: test_deps(ctx) + max(sum((l(ctx) for l in body_deps)), sum((l(ctx) for l in else_deps)))
