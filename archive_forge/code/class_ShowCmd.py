...
class ShowCmd(Command):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.subcommands = {'count': self.Count, 'logging': self.Logging, 'rib': self.Rib, 'vrf': self.Vrf, 'memory': self.Memory, 'neighbor': self.Neighbor, 'importmap': self.Importmap}
    ...
    class Logging(Command):
        ...
        def action(self, params):
            if self.api.check_logging():
                ret = {'logging': self.api.check_logging(), 'level': self.api.check_logging_level()}
            else:
