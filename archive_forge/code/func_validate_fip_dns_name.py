...
def validate_fip_dns_name(data, max_len=db_constants.FQDN_FIELD_SIZE):
    ...
    length = len(data)
    if length > max_len - 3:
        msg = _("'%(data)s' contains %(length)s characters. Adding a domain name will cause it to exceed the maximum length of a FQDN of '%(max_len)s'") % {'data': data, 'length': length, 'max_len': max_len}
        return msg