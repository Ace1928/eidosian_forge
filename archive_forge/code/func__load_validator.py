...
def _load_validator(engine, data, criteria, plugin_vars=None, cls_name='Validate', kwargs=None):
    ...
    validatorlib = 'ansible_collections.{corg}.{cname}.plugins.sub_plugins.validate.{plugin}'.format(**cref)
    try:
        validatorcls = getattr(import_module(validatorlib), cls_name)
        validator = validatorcls(data=data, criteria=criteria, engine=engine, plugin_vars=plugin_vars, kwargs=kwargs)
        return (validator, result)
    except Exception as exc:
        result['failed'] = True
        result['msg'] = "For engine '{engine}' error loading the corresponding validate plugin: {err}".format(engine=engine, err=to_native(exc))
        return (None, result)