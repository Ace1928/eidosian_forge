...
class MELD(GraphEstimator):
    ...
    """
    beta = attribute('beta', doc='Amount of smoothing to apply. Default value of 60 determined throughanalysis of simulated data using Splatter', default=40, on_set=graphtools.utils.check_positive)
    offset = attribute('offset', doc='Amount to shift the MELD filter in the eigenvalue spectrum.Recommend using an eigenvalue from the graph based on thespectral distribution. Should be in interval [0,1]', default=0)
    order = attribute('order', doc='Falloff and smoothness of the filter.High order leads to square-like filters.', default=1)
    filter = attribute('filter', default='heat', doc="Filter type to use. Should be in ['heat', 'laplacian']", on_set=partial(graphtools.utils.check_in, ['heat', 'laplacian']))
    solver = attribute('solver', default='chebyshev', doc="Method to solve convex problem.'chebyshev' uses a chebyshev polynomial approximation of the correspondingfilter. 'exact' uses the eigenvalue solution to the problem", on_set=partial(graphtools.utils.check_in, ['chebyshev', 'exact']))
    chebyshev_order = attribute('chebyshev_order', default=30, doc='Order of chebyshev approximation to use.', on_set=[graphtools.utils.check_int, graphtools.utils.check_positive])
    lap_type = attribute('lap_type', default='combinatorial', doc='The kind of Laplacian to calculate', on_set=partial(graphtools.utils.check_in, ['combinatorial', 'normalized']))
    sample_densities = attribute('sample_densities', doc='Density associated with each sample')

    def __init__(self, beta=60, offset=0, order=1, filter='heat', solver='chebyshev', chebyshev_order=50, lap_type='combinatorial', sample_normalize=True, anisotropy=1, n_landmark=None, **kwargs):
        self.beta = beta
        self.offset = offset
        self.order = order
        self.solver = solver
    ...
    def set_params(self, **params):
        for p in ['beta', 'offset', 'order', 'solver', 'chebyshev_order', 'lap_type', 'filter']:
    ...
    def transform(self, sample_labels):
        ...
        if sample_labels.shape[0] != self.graph.N:
            raise ValueError('Input data ({}) and input graph ({}) are not of the same size'.format(sample_labels.shape, self.graph.N))
        if len(np.unique(sample_labels)) == 1:
            raise ValueError('Found only one unqiue sample label. Cannot estimate density of a single sample.')
        ...
        densities = filter.filter(signal=self.sample_indicators, graph=self.graph, filter=self.filter, beta=self.beta, offset=self.offset, order=self.order, solver=self.solver, chebyshev_order=self.chebyshev_order)
