...
class TestDatasetQuiverPlots(PlotTestCase):
    ...
    def setUp(self) -> None:
        das = [DataArray(np.random.randn(3, 3, 4, 4), dims=['x', 'y', 'row', 'col'], coords=[range(k) for k in [3, 3, 4, 4]]) for _ in [1, 2]]
    ...
    def test_quiver(self) -> None:
        ...
        with pytest.raises(ValueError, match='hue_style'):
            self.ds.isel(row=0, col=0).plot.quiver(x='x', y='y', u='u', v='v', hue='mag', hue_style='discrete')

    def test_facetgrid(self) -> None:
        with figure_context():
            fg = self.ds.plot.quiver(x='x', y='y', u='u', v='v', row='row', col='col', scale=1, hue='mag')
        ...
        with figure_context():
            fg = self.ds.plot.quiver(x='x', y='y', u='u', v='v', row='row', col='col', scale=1, hue='mag', add_guide=False)
            assert fg.quiverkey is None
        with pytest.raises(ValueError, match='Please provide scale'):
            self.ds.plot.quiver(x='x', y='y', u='u', v='v', row='row', col='col')

    @pytest.mark.parametrize('add_guide, hue_style, legend, colorbar', [(None, None, False, True), (False, None, False, False), (True, None, False, True), (True, 'continuous', False, True)])
    def test_add_guide(self, add_guide, hue_style, legend, colorbar):
        meta_data = _infer_meta_data(self.ds, x='x', y='y', hue='mag', hue_style=hue_style, add_guide=add_guide, funcname='quiver')
