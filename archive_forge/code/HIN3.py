...
class GameController:
    ...
    async def keep_moving(self) -> Tuple[int, int]:
        """
        Continues the snake's movement in the last known direction asynchronously.
        ...
        async with asyncio.Lock():
            x: float = self.snake.body[0].x
            y: float = self.snake.body[0].y
    ...
    async def update_GA_ai(self) -> None:
        """
        Updates the state of the game when using a genetic algorithm for AI control asynchronously.
        """
        logging.debug("Updating GA AI")
        async with asyncio.Lock():
            if not self.snake and not self.model_loaded:
                if await self.algo.done():
                    if await self.algo.next_generation():
                        self.snakes = self.algo.population.snakes
                ...
                for snake in self.snakes:
                    x, y = await self.algo.run_algorithm(snake)

                    await snake.move_ai(x, y)
                    await self.algo.died(snake)
                    await self.ate_fruit_GA(snake)
            else:
                x, y = await self.algo.run_algorithm(self.snake)
                await self.snake.move_ai(x, y)
                await self.died()
                await self.ate_fruit()

    async def update_path_finding_algo(self, pos: Optional[Node]) -> None:
        """
        Updates the state of the game when using a path-finding algorithm for AI control asynchronously.
