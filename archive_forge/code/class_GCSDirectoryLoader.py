...
@deprecated(since='0.0.32', removal='0.2.0', alternative_import='langchain_google_community.GCSDirectoryLoader')
class GCSDirectoryLoader(BaseLoader):
    """Load from GCS directory."""

    def __init__(self, project_name: str, bucket: str, prefix: str='', loader_func: Optional[Callable[[str], BaseLoader]]=None, continue_on_failure: bool=False):
    ...
    def load(self) -> List[Document]:
        ...
        try:
            ...
            raise ImportError('Could not import google-cloud-storage python package. Please install it with `pip install google-cloud-storage`.')
        client = storage.Client(project=self.project_name, client_info=get_client_info(module='google-cloud-storage'))
        docs = []
        for blob in client.list_blobs(self.bucket, prefix=self.prefix):
            if blob.name.endswith('/'):
                continue
            try:
                loader = GCSFileLoader(self.project_name, self.bucket, blob.name, loader_func=self._loader_func)
