...
class StackControllerTest(tools.ControllerTest, common.HeatTestCase):
    ...
    def test_index_limit_negative(self, mock_call, mock_enforce):
        ...
        self.assertEqual("Value '-1' is invalid for 'limit' which only accepts non-negative integer.", str(ex))
    ...
    def test_index_returns_stack_count_if_with_count_is_true(self, mock_enforce):
        self._mock_enforce_setup(mock_enforce, 'index', True)
    ...
    def test_index_doesnt_return_stack_count_if_with_count_is_false(self, mock_enforce):
        self._mock_enforce_setup(mock_enforce, 'index', True)
    ...
    def test_global_index_show_nested_false(self, mock_enforce):
        ...
        req = self._get('/stacks', params=params)
        self.controller.index(req, tenant_id=self.tenant)
        rpc_client.list_stacks.assert_called_once_with(mock.ANY, filters=mock.ANY, show_nested=False)
    ...
    def test_global_index_show_nested_true(self, mock_enforce):
        ...
        req = self._get('/stacks', params=params)
        self.controller.index(req, tenant_id=self.tenant)
        rpc_client.list_stacks.assert_called_once_with(mock.ANY, filters=mock.ANY, show_nested=True)
    ...
    def test_global_index_show_hidden_true(self, mock_enforce):
        ...
        req = self._get('/stacks', params=params)
        self.controller.index(req, tenant_id=self.tenant)
        rpc_client.list_stacks.assert_called_once_with(mock.ANY, filters=mock.ANY, show_hidden=True)
    ...
    def test_global_index_show_hidden_false(self, mock_enforce):
        ...
        req = self._get('/stacks', params=params)
        self.controller.index(req, tenant_id=self.tenant)
        rpc_client.list_stacks.assert_called_once_with(mock.ANY, filters=mock.ANY, show_hidden=False)

    def test_index_show_deleted_True_with_count_false(self, mock_enforce):
        rpc_client = self.controller.rpc_client
        rpc_client.list_stacks = mock.Mock(return_value=[])
        rpc_client.count_stacks = mock.Mock()
        params = {'show_deleted': 'True', 'with_count': 'false'}
        req = self._get('/stacks', params=params)
        result = self.controller.index(req, tenant_id=self.tenant)
        self.assertNotIn('count', result)
        rpc_client.list_stacks.assert_called_once_with(mock.ANY, filters=mock.ANY, show_deleted=True)
    ...
    def test_index_rmt_aterr(self, mock_call, mock_enforce):
        ...
        resp = tools.request_with_middleware(fault.FaultWrapper, self.controller.index, req, tenant_id=self.tenant)
        self.assertEqual(400, resp.json['code'])
        self.assertEqual('AttributeError', resp.json['error']['type'])
        mock_call.assert_called_once_with(req.context, ('list_stacks', mock.ANY), version='1.33')
    ...
    def test_create(self, mock_enforce):
        ...
        body = {'template': template, 'stack_name': identity.stack_name, 'parameters': parameters, 'environment_files': ['foo.yaml'], 'timeout_mins': 30}
    ...
    def test_adopt(self, mock_enforce):
        ...
        adopt_data = {'status': 'COMPLETE', 'name': 'rtrove1', 'parameters': parameters, 'template': template, 'action': 'CREATE', 'id': '8532f0d3-ea84-444e-b2bb-2543bb1496a4', 'resources': {'res1': {'status': 'COMPLETE', 'name': 'database_password', 'resource_id': 'yBpuUROjfGQ2gKOD', 'action': 'CREATE', 'type': 'GenericResourceType', 'metadata': {}}}}
    ...
    def test_create_err_rpcerr(self, mock_enforce):
        ...
        mock_call = self.patchobject(rpc_client.EngineClient, 'call', side_effect=[tools.to_remote_error(AttributeError()), tools.to_remote_error(unknown_parameter), tools.to_remote_error(missing_parameter)])
        resp = tools.request_with_middleware(fault.FaultWrapper, self.controller.create, req, tenant_id=self.tenant, body=body)
        self.assertEqual(400, resp.json['code'])
        self.assertEqual('AttributeError', resp.json['error']['type'])
        resp = tools.request_with_middleware(fault.FaultWrapper, self.controller.create, req, tenant_id=self.tenant, body=body)
    ...
    def test_create_err_circulardep(self, mock_enforce):
        ...
        stack_name = 'foobar'
        template = {u'Foo': u'bar'}
        body = {'template': template, 'stack_name': stack_name, 'parameters': {}, 'timeout_mins': 30}
        req = self._post('/stacks', json.dumps(body))
        error = heat_exc.CircularDependencyException(cycle='some data')
        mock_call = self.patchobject(rpc_client.EngineClient, 'call', side_effect=tools.to_remote_error(error))
        resp = tools.request_with_middleware(fault.FaultWrapper, self.controller.create, req, tenant_id=self.tenant, body=body)
        self.assertEqual(400, resp.json['code'])
        self.assertEqual('CircularDependencyException', resp.json['error']['type'])
    ...
    def test_preview_update_stack_patch(self, mock_enforce):
        ...
        identity = identifier.HeatIdentifier(self.tenant, 'wordpress', '6')
        parameters = {u'InstanceType': u'm1.xlarge'}
        body = {'template': None, 'parameters': parameters, 'files': {}, 'timeout_mins': 30}
        req = self._patch('/stacks/%(stack_name)s/%(stack_id)s/preview' % identity, json.dumps(body))
        resource_changes = {'updated': [], 'deleted': [], 'unchanged': [], 'added': [], 'replaced': []}
        mock_call = self.patchobject(rpc_client.EngineClient, 'call', return_value=resource_changes)
        result = self.controller.preview_update_patch(req, tenant_id=identity.tenant, stack_name=identity.stack_name, stack_id=identity.stack_id, body=body)
        self.assertEqual({'resource_changes': resource_changes}, result)
        mock_call.assert_called_once_with(req.context, ('preview_update_stack', {'stack_identity': dict(identity), 'template': None, 'params': {'parameters': parameters, 'encrypted_param_names': [], 'parameter_defaults': {}, 'event_sinks': [], 'resource_registry': {}}, 'files': {}, 'environment_files': None, 'files_container': None, 'args': {rpc_api.PARAM_EXISTING: True, 'timeout_mins': 30}}), version='1.36')
    ...
    def test_lookup(self, mock_enforce):
        ...
        mock_call = self.patchobject(rpc_client.EngineClient, 'call', return_value=dict(identity))
        found = self.assertRaises(webob.exc.HTTPFound, self.controller.lookup, req, tenant_id=identity.tenant, stack_name=identity.stack_name)
        self.assertEqual(self._url(identity), found.location)
        mock_call.assert_called_once_with(req.context, ('identify_stack', {'stack_name': identity.stack_name}))

    def test_lookup_arn(self, mock_enforce):
        self._mock_enforce_setup(mock_enforce, 'lookup', True)
        identity = identifier.HeatIdentifier(self.tenant, 'wordpress', '1')
        req = self._get('/stacks%s' % identity.arn_url_path())
        found = self.assertRaises(webob.exc.HTTPFound, self.controller.lookup, req, tenant_id=identity.tenant, stack_name=identity.arn())
        self.assertEqual(self._url(identity), found.location)

    def test_lookup_nonexistent(self, mock_enforce):
        self._mock_enforce_setup(mock_enforce, 'lookup', True)
        stack_name = 'wibble'
        req = self._get('/stacks/%(stack_name)s' % {'stack_name': stack_name})
        error = heat_exc.EntityNotFound(entity='Stack', name='a')
        mock_call = self.patchobject(rpc_client.EngineClient, 'call', side_effect=tools.to_remote_error(error))
        resp = tools.request_with_middleware(fault.FaultWrapper, self.controller.lookup, req, tenant_id=self.tenant, stack_name=stack_name)
    ...
    def test_lookup_resource(self, mock_enforce):
        ...
        mock_call = self.patchobject(rpc_client.EngineClient, 'call', return_value=dict(identity))
        found = self.assertRaises(webob.exc.HTTPFound, self.controller.lookup, req, tenant_id=identity.tenant, stack_name=identity.stack_name, path='resources')
        self.assertEqual(self._url(identity) + '/resources', found.location)
        mock_call.assert_called_once_with(req.context, ('identify_stack', {'stack_name': identity.stack_name}))
    ...
    def test_show(self, mock_enforce):
        ...
        identity = identifier.HeatIdentifier(self.tenant, 'wordpress', '6')
        req = self._get('/stacks/%(stack_name)s/%(stack_id)s' % identity, params={'resolve_outputs': True})
        ...
        expected = {'stack': {'links': [{'href': self._url(identity), 'rel': 'self'}], 'id': '6', u'updated_time': u'2012-07-09T09:13:11Z', u'parameters': parameters, u'outputs': outputs, u'description': u'blah', u'stack_status_reason': u'Stack successfully created', u'creation_time': u'2012-07-09T09:12:45Z', u'stack_name': identity.stack_name, u'stack_status': u'CREATE_COMPLETE', u'capabilities': [], u'notification_topics': [], u'disable_rollback': True, u'timeout_mins': 60}}
        self.assertEqual(expected, response)
        mock_call.assert_called_once_with(req.context, ('show_stack', {'stack_identity': dict(identity), 'resolve_outputs': True}), version='1.20')
    ...
    def test_get_template(self, mock_enforce):
        ...
        req = self._get('/stacks/%(stack_name)s/%(stack_id)s' % identity)
        template = {u'Foo': u'bar'}
        mock_call = self.patchobject(rpc_client.EngineClient, 'call', return_value=template)
        response = self.controller.template(req, tenant_id=identity.tenant, stack_name=identity.stack_name, stack_id=identity.stack_id)
        self.assertEqual(template, response)
        mock_call.assert_called_once_with(req.context, ('get_template', {'stack_identity': dict(identity)}))
    ...
    def test_get_environment(self, mock_enforce):
        ...
        mock_call = self.patchobject(rpc_client.EngineClient, 'call', return_value=env)
        response = self.controller.environment(req, tenant_id=identity.tenant, stack_name=identity.stack_name, stack_id=identity.stack_id)
        self.assertEqual(env, response)
        mock_call.assert_called_once_with(req.context, ('get_environment', {'stack_identity': dict(identity)}), version='1.28')
    ...
    def test_update_timeout_negative(self, mock_enforce):
        ...
        template = {u'Foo': u'bar'}
        parameters = {u'InstanceType': u'm1.xlarge'}
        body = {'template': template, 'parameters': parameters, 'files': {}, 'timeout_mins': -1}
        req = self._put('/stacks/%(stack_name)s/%(stack_id)s' % identity, json.dumps(body))
        mock_call = self.patchobject(rpc_client.EngineClient, 'call')
        ...
        self.assertEqual("Value '-1' is invalid for 'timeout_mins' which only accepts non-negative integer.", str(ex))
        self.assertFalse(mock_call.called)
    ...
    def test_update_with_existing_template(self, mock_enforce):
        ...
        identity = identifier.HeatIdentifier(self.tenant, 'wordpress', '6')
        body = {'template': None, 'parameters': {}, 'files': {}, 'timeout_mins': 30}
        ...
        mock_call.assert_called_once_with(req.context, ('update_stack', {'stack_identity': dict(identity), 'template': None, 'params': {'parameters': {}, 'encrypted_param_names': [], 'parameter_defaults': {}, 'event_sinks': [], 'resource_registry': {}}, 'files': {}, 'environment_files': None, 'files_container': None, 'args': {rpc_api.PARAM_EXISTING: True, 'timeout_mins': 30}, 'template_id': None}), version='1.36')
    ...
    def test_update_with_existing_parameters_with_tags(self, mock_enforce):
        ...
        identity = identifier.HeatIdentifier(self.tenant, 'wordpress', '6')
        template = {u'Foo': u'bar'}
        body = {'template': template, 'parameters': {}, 'files': {}, 'tags': 'tag1,tag2', 'timeout_mins': 30}
        req = self._patch('/stacks/%(stack_name)s/%(stack_id)s' % identity, json.dumps(body))
        mock_call = self.patchobject(rpc_client.EngineClient, 'call', return_value=dict(identity))
        self.assertRaises(webob.exc.HTTPAccepted, self.controller.update_patch, req, tenant_id=identity.tenant, stack_name=identity.stack_name, stack_id=identity.stack_id, body=body)
        mock_call.assert_called_once_with(req.context, ('update_stack', {'stack_identity': dict(identity), 'template': template, 'params': {'parameters': {}, 'encrypted_param_names': [], 'parameter_defaults': {}, 'event_sinks': [], 'resource_registry': {}}, 'files': {}, 'environment_files': None, 'files_container': None, 'args': {rpc_api.PARAM_EXISTING: True, 'timeout_mins': 30, 'tags': ['tag1', 'tag2']}, 'template_id': None}), version='1.36')
    ...
    def test_update_with_patched_existing_parameters(self, mock_enforce):
        ...
        mock_call = self.patchobject(rpc_client.EngineClient, 'call', return_value=dict(identity))
        self.assertRaises(webob.exc.HTTPAccepted, self.controller.update_patch, req, tenant_id=identity.tenant, stack_name=identity.stack_name, stack_id=identity.stack_id, body=body)
        mock_call.assert_called_once_with(req.context, ('update_stack', {'stack_identity': dict(identity), 'template': template, 'params': {'parameters': parameters, 'encrypted_param_names': [], 'parameter_defaults': {}, 'event_sinks': [], 'resource_registry': {}}, 'files': {}, 'environment_files': None, 'files_container': None, 'args': {rpc_api.PARAM_EXISTING: True, 'timeout_mins': 30}, 'template_id': None}), version='1.36')
    ...
    def test_update_with_patch_timeout_negative(self, mock_enforce):
        ...
        req = self._patch('/stacks/%(stack_name)s/%(stack_id)s' % identity, json.dumps(body))
        mock_call = self.patchobject(rpc_client.EngineClient, 'call')
        ex = self.assertRaises(webob.exc.HTTPBadRequest, self.controller.update_patch, req, tenant_id=identity.tenant, stack_name=identity.stack_name, stack_id=identity.stack_id, body=body)
        self.assertEqual("Value '-1' is invalid for 'timeout_mins' which only accepts non-negative integer.", str(ex))
        self.assertFalse(mock_call.called)
    ...
    def test_update_with_existing_and_default_parameters(self, mock_enforce):
        self._mock_enforce_setup(mock_enforce, 'update_patch', True)
        identity = identifier.HeatIdentifier(self.tenant, 'wordpress', '6')
        template = {u'Foo': u'bar'}
        clear_params = [u'DBUsername', u'DBPassword', u'LinuxDistribution']
        body = {'template': template, 'parameters': {}, 'clear_parameters': clear_params, 'files': {}, 'timeout_mins': 30}
        req = self._patch('/stacks/%(stack_name)s/%(stack_id)s' % identity, json.dumps(body))
        mock_call = self.patchobject(rpc_client.EngineClient, 'call', return_value=dict(identity))
        self.assertRaises(webob.exc.HTTPAccepted, self.controller.update_patch, req, tenant_id=identity.tenant, stack_name=identity.stack_name, stack_id=identity.stack_id, body=body)
        mock_call.assert_called_once_with(req.context, ('update_stack', {'stack_identity': dict(identity), 'template': template, 'params': {'parameters': {}, 'encrypted_param_names': [], 'parameter_defaults': {}, 'event_sinks': [], 'resource_registry': {}}, 'files': {}, 'environment_files': None, 'files_container': None, 'args': {rpc_api.PARAM_EXISTING: True, 'clear_parameters': clear_params, 'timeout_mins': 30}, 'template_id': None}), version='1.36')

    def test_update_with_patched_and_default_parameters(self, mock_enforce):
        self._mock_enforce_setup(mock_enforce, 'update_patch', True)
        identity = identifier.HeatIdentifier(self.tenant, 'wordpress', '6')
        template = {u'Foo': u'bar'}
        parameters = {u'InstanceType': u'm1.xlarge'}
        clear_params = [u'DBUsername', u'DBPassword', u'LinuxDistribution']
        body = {'template': template, 'parameters': parameters, 'clear_parameters': clear_params, 'files': {}, 'timeout_mins': 30}
        req = self._patch('/stacks/%(stack_name)s/%(stack_id)s' % identity, json.dumps(body))
        mock_call = self.patchobject(rpc_client.EngineClient, 'call', return_value=dict(identity))
        self.assertRaises(webob.exc.HTTPAccepted, self.controller.update_patch, req, tenant_id=identity.tenant, stack_name=identity.stack_name, stack_id=identity.stack_id, body=body)
        mock_call.assert_called_once_with(req.context, ('update_stack', {'stack_identity': dict(identity), 'template': template, 'params': {'parameters': parameters, 'encrypted_param_names': [], 'parameter_defaults': {}, 'event_sinks': [], 'resource_registry': {}}, 'files': {}, 'environment_files': None, 'files_container': None, 'args': {rpc_api.PARAM_EXISTING: True, 'clear_parameters': clear_params, 'timeout_mins': 30}, 'template_id': None}), version='1.36')
    ...
    def test_delete(self, mock_enforce):
        ...
        self.assertRaises(webob.exc.HTTPNoContent, self.controller.delete, req, tenant_id=identity.tenant, stack_name=identity.stack_name, stack_id=identity.stack_id)
        mock_call.assert_called_once_with(req.context, ('delete_stack', {'stack_identity': dict(identity)}))

    def test_delete_err_denied_policy(self, mock_enforce):
        self._mock_enforce_setup(mock_enforce, 'delete', False)
        identity = identifier.HeatIdentifier(self.tenant, 'wordpress', '6')
        req = self._delete('/stacks/%(stack_name)s/%(stack_id)s' % identity)
        resp = tools.request_with_middleware(fault.FaultWrapper, self.controller.delete, req, tenant_id=self.tenant, stack_name=identity.stack_name, stack_id=identity.stack_id)
        self.assertEqual(403, resp.status_int)
        self.assertIn('403 Forbidden', str(resp))
    ...
    def test_export(self, mock_enforce):
        ...
        identity = identifier.HeatIdentifier(self.tenant, 'wordpress', '6')
        req = self._get('/stacks/%(stack_name)s/%(stack_id)s/export' % identity)
        expected = {'name': 'test', 'id': '123'}
        mock_call = self.patchobject(rpc_client.EngineClient, 'call', return_value=expected)
        ret = self.controller.export(req, tenant_id=identity.tenant, stack_name=identity.stack_name, stack_id=identity.stack_id)
        self.assertEqual(expected, ret)
        mock_call.assert_called_once_with(req.context, ('export_stack', {'stack_identity': dict(identity)}), version='1.22')
    ...
    def test_abandon(self, mock_enforce):
        ...
        expected = {'name': 'test', 'id': '123'}
        mock_call = self.patchobject(rpc_client.EngineClient, 'call', return_value=expected)
        ret = self.controller.abandon(req, tenant_id=identity.tenant, stack_name=identity.stack_name, stack_id=identity.stack_id)
        self.assertEqual(expected, ret)
        mock_call.assert_called_once_with(req.context, ('abandon_stack', {'stack_identity': dict(identity)}))

    def test_abandon_err_denied_policy(self, mock_enforce):
        self._mock_enforce_setup(mock_enforce, 'abandon', False)
        identity = identifier.HeatIdentifier(self.tenant, 'wordpress', '6')
        req = self._abandon('/stacks/%(stack_name)s/%(stack_id)s' % identity)
        resp = tools.request_with_middleware(fault.FaultWrapper, self.controller.abandon, req, tenant_id=self.tenant, stack_name=identity.stack_name, stack_id=identity.stack_id)
        self.assertEqual(403, resp.status_int)
        self.assertIn('403 Forbidden', str(resp))
    ...
    def test_delete_bad_name(self, mock_enforce):
        ...
        error = heat_exc.EntityNotFound(entity='Stack', name='a')
        mock_call = self.patchobject(rpc_client.EngineClient, 'call', side_effect=tools.to_remote_error(error))
        resp = tools.request_with_middleware(fault.FaultWrapper, self.controller.delete, req, tenant_id=identity.tenant, stack_name=identity.stack_name, stack_id=identity.stack_id)
        self.assertEqual(404, resp.json['code'])
        self.assertEqual('EntityNotFound', resp.json['error']['type'])
        mock_call.assert_called_once_with(req.context, ('delete_stack', {'stack_identity': dict(identity)}))
    ...
    def test_validate_template(self, mock_enforce):
        ...
        engine_response = {u'Description': u'blah', u'Parameters': [{u'NoEcho': u'false', u'ParameterKey': u'InstanceType', u'Description': u'Instance type'}]}
        mock_call = self.patchobject(rpc_client.EngineClient, 'call', return_value=engine_response)
        response = self.controller.validate_template(req, tenant_id=self.tenant, body=body)
        self.assertEqual(engine_response, response)
        mock_call.assert_called_once_with(req.context, ('validate_template', {'template': template, 'params': {'parameters': {}, 'encrypted_param_names': [], 'parameter_defaults': {}, 'event_sinks': [], 'resource_registry': {}}, 'files': {}, 'environment_files': None, 'files_container': None, 'show_nested': False, 'ignorable_errors': None}), version='1.36')
    ...
    def test_validate_template_error(self, mock_enforce):
        ...
        body = {'template': template}
        req = self._post('/validate', json.dumps(body))
        mock_call = self.patchobject(rpc_client.EngineClient, 'call', return_value={'Error': 'fubar'})
        self.assertRaises(webob.exc.HTTPBadRequest, self.controller.validate_template, req, tenant_id=self.tenant, body=body)
        mock_call.assert_called_once_with(req.context, ('validate_template', {'template': template, 'params': {'parameters': {}, 'encrypted_param_names': [], 'parameter_defaults': {}, 'event_sinks': [], 'resource_registry': {}}, 'files': {}, 'environment_files': None, 'files_container': None, 'show_nested': False, 'ignorable_errors': None}), version='1.36')
    ...
    def test_list_resource_types(self, mock_enforce):
        ...
        engine_response = ['AWS::EC2::Instance', 'AWS::EC2::EIP', 'AWS::EC2::EIPAssociation']
        mock_call = self.patchobject(rpc_client.EngineClient, 'call', return_value=engine_response)
        response = self.controller.list_resource_types(req, tenant_id=self.tenant)
    ...
    def test_list_outputs(self, mock_enforce):
        ...
        identity = identifier.HeatIdentifier(self.tenant, 'wordpress', '6')
        req = self._get('/stacks/%(stack_name)s/%(stack_id)s' % identity)
        outputs = [{'output_key': 'key1', 'description': 'description'}, {'output_key': 'key2', 'description': 'description1'}]
        mock_call = self.patchobject(rpc_client.EngineClient, 'call', return_value=outputs)
        response = self.controller.list_outputs(req, tenant_id=identity.tenant, stack_name=identity.stack_name, stack_id=identity.stack_id)
        self.assertEqual({'outputs': outputs}, response)
        mock_call.assert_called_once_with(req.context, ('list_outputs', {'stack_identity': dict(identity)}), version='1.19')
    ...
    def test_list_template_functions(self, mock_enforce):
        ...
        engine_response = [{'functions': 'func1', 'description': 'desc1'}]
    ...
    def test_generate_template_err_denied_policy(self, mock_enforce):
        ...
        req = self._get('/resource_types/NOT_FOUND/template')
        resp = tools.request_with_middleware(fault.FaultWrapper, self.controller.generate_template, req, tenant_id=self.tenant, type_name='blah')
        self.assertEqual(403, resp.status_int)
        self.assertIn('403 Forbidden', str(resp))