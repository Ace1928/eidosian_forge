...
class ColumnarConverter(XMLSchemaConverter):
    ...
    def __init__(self, namespaces: Optional[NamespacesType]=None, dict_class: Optional[Type[Dict[str, Any]]]=None, list_class: Optional[Type[List[Any]]]=None, attr_prefix: Optional[str]='', **kwargs: Any) -> None:
        kwargs.update(text_key=None, cdata_prefix=None)
        super(ColumnarConverter, self).__init__(namespaces, dict_class, list_class, attr_prefix=attr_prefix, **kwargs)
    ...
    def element_decode(self, data: ElementData, xsd_element: 'XsdElement', xsd_type: Optional[BaseXsdType]=None, level: int=0) -> Any:
        ...
        if data.content:
            for name, value, xsd_child in self.map_content(data.content):
                ...
                elif xsd_child.type is not None and xsd_child.type.simple_type is not None and (not xsd_child.attributes):
