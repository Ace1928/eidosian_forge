...
class StochasticSwap(TransformationPass):
    ...
    def _mapper(self, circuit_graph, coupling_graph, trials=20):
        ...
        for i, layer in enumerate(layerlist):
            if not layer['graph'].op_nodes(op=ControlFlowOp):
                success_flag, best_circuit, best_depth, best_layout = self._layer_permutation(circuit_graph, layer['partition'], layout, qubit_subset, coupling_graph, trials)
                logger.debug('mapper: layer %d', i)
                logger.debug('mapper: success_flag=%s,best_depth=%s', success_flag, str(best_depth))
                if success_flag:
                    layout = best_layout
                    if not self.fake_run:
                        self._layer_update(dagcircuit_output, layerlist[i], best_layout, best_depth, best_circuit)
                    continue
            ...
            for j, serial_layer in enumerate(layer['graph'].serial_layers()):
                ...
                if isinstance(op_node.op, ControlFlowOp):
                    layout = self._controlflow_layer_update(dagcircuit_output, layer_dag, layout, circuit_graph)
                else:
                    success_flag, best_circuit, best_depth, best_layout = self._layer_permutation(circuit_graph, serial_layer['partition'], layout, qubit_subset, coupling_graph, trials)
                    logger.debug('mapper: layer %d, sublayer %d', i, j)
                    logger.debug('mapper: success_flag=%s,best_depth=%s,', success_flag, str(best_depth))
                    if not success_flag:
                        raise TranspilerError('swap mapper failed: ' + 'layer %d, sublayer %d' % (i, j))
                    layout = best_layout
                    if not self.fake_run:
                        self._layer_update(dagcircuit_output, serial_layer, best_layout, best_depth, best_circuit)
    ...
    def _controlflow_layer_update(self, dagcircuit_output, layer_dag, current_layout, root_dag):
        ...
        if _controlflow_exhaustive_acyclic(node.op):
            final_layout = max(zip(block_layouts, block_dags), key=lambda x: x[1].depth(recurse=True))[0]
        else:
        ...
        for layout, updated_dag_block in zip(block_layouts, block_dags):
            swap_dag, swap_qubits = get_swap_map_dag(root_dag, self.coupling_map, layout, final_layout, seed=self._new_seed())
