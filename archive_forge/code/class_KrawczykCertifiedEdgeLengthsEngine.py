...
class KrawczykCertifiedEdgeLengthsEngine:
    ...
    def __init__(self, approx_hyperbolic_structure, bits_prec=53):
        if not (approx_hyperbolic_structure.exact_edges and approx_hyperbolic_structure.var_edges):
            raise Exception('Did not pick exact/var edges: call HyperbolicStructure.pick_exact_and_var_edges.')
        ...
        self.initial_edge_lengths = vector([self.RIF(e) for e in approx_hyperbolic_structure.edge_lengths])
        self.approx_inverse = approx_hyperbolic_structure.full_rank_jacobian_submatrix().change_ring(RealDoubleField()).inverse().change_ring(self.RIF)
