...
class CreateBitbucketDataCenter(base.CreateCommand):
    ...
    """
        ...
        if args.pull_request_pattern:
            cfg.pullRequest = messages.PullRequestFilter(branch=args.pull_request_pattern)
            if args.comment_control:
                cfg.pullRequest.commentControl = messages.PullRequestFilter.CommentControlValueValuesEnum(args.comment_control)
        else:
            cfg.push = messages.PushFilter(branch=args.branch_pattern, tag=args.tag_pattern)
