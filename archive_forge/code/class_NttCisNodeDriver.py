...
class NttCisNodeDriver(NodeDriver):
    ...
    def list_nodes(self, ex_location=None, ex_name=None, ex_ipv6=None, ex_ipv4=None, ex_vlan=None, ex_image=None, ex_deployed=None, ex_started=None, ex_state=None, ex_network_domain=None, ex_snaphots=None):
        """
        ...
        """
        node_list = []
        for nodes in self.ex_list_nodes_paginated(location=ex_location, name=ex_name, ipv6=ex_ipv6, ipv4=ex_ipv4, vlan=ex_vlan, image=ex_image, deployed=ex_deployed, started=ex_started, state=ex_state, network_domain=ex_network_domain):
            if nodes:
                node_list = nodes
        return node_list
    ...
    def list_networks(self, location=None):
        ...
        return self._to_networks(self.connection.request_with_orgId_api_2('networkWithLocation%s' % url_ext).object)
    ...
    def list_snapshots(self, node, page_size=None):
        ...
        return self._to_snapshots(self.connection.request_with_orgId_api_2('snapshot/snapshot', params=params).object)
    ...
    def get_snapshot(self, snapshot_id):
        ...
        :rtype: ``dict``
        """
        return self._to_snapshot(self.connection.request_with_orgId_api_2('snapshot/snapshot/%s' % snapshot_id).object)
    ...
    def ex_disable_snapshots(self, node):
        ...
        update_node.set('serverId', node)
        result = self.connection.request_with_orgId_api_2('snapshot/disableSnapshotService', method='POST', data=ET.tostring(update_node)).object
        response_code = findtext(result, 'responseCode', TYPES_URN)
        return response_code in ['IN_PROGRESS', 'OK']
    ...
    def ex_create_anti_affinity_rule(self, node_list):
        ...
        result = self.connection.request_with_orgId_api_2('server/createAntiAffinityRule', method='POST', data=ET.tostring(anti_affinity_xml_request)).object
        response_code = findtext(result, 'responseCode', TYPES_URN)
        return response_code in ['IN_PROGRESS', 'SUCCESS']
    ...
    def ex_list_reserved_ipv6(self, vlan=None, datacenter_id=None):
        if vlan is not None:
            vlan_id = self._vlan_to_vlan_id(vlan)
            params = {'vlanId': vlan_id}
            response = self.connection.request_with_orgId_api_2('network/reservedIpv6Address', params=params).object
        elif datacenter_id is not None:
            params = {'datacenterId': datacenter_id}
            response = self.connection.request_with_orgId_api_2('network/reservedIpv6Address', params=params).object
        else:
            response = self.connection.request_with_orgId_api_2('network/reservedIpv6Address').object
    ...
    def ex_create_firewall_rule(self, network_domain, name, action, ip_version, protocol, source_addr, dest_addr, position, enabled=1, position_relative_to_rule=None):
        """
    ...
    def ex_get_consistency_group(self, consistency_group_id):
        ...
        :rtype: :class:`NttCisConsistencygroup`
        """
        response = self.connection.request_with_orgId_api_2('consistencyGroup/consistencyGroup/%s' % consistency_group_id).object
    ...
    def _to_network(self, element, locations):
        ...
        location = list(filter(lambda x: x.id == location_id, locations))[0]
        return NttCisNetwork(id=findtext(element, 'id', NETWORK_NS), name=findtext(element, 'name', NETWORK_NS), description=findtext(element, 'description', NETWORK_NS), location=location, private_net=findtext(element, 'privateNet', NETWORK_NS), multicast=multicast, status=status)
    ...
    def _to_disk(self, element):
        return NttCisServerDisk(id=element.get('id'), scsi_id=int(element.get('scsiId')), size_gb=int(element.get('sizeGb')), speed=element.get('speed'), state=element.get('state'))
    ...
    def _to_snapshot(self, element):
        return {'id': element.get('id'), 'start_time': findtext(element, 'startTime', TYPES_URN), 'end_time': findtext(element, 'endTime', TYPES_URN), 'expiry_time': findtext(element, 'expiryTime', TYPES_URN), 'type': findtext(element, 'type', TYPES_URN), 'state': findtext(element, 'state', TYPES_URN)}
    ...
    def _to_node(self, element):
        ...
        if LooseVersion(self.connection.active_api_version) < LooseVersion('2.4'):
            ...
            if vmtools_elm is not None:
                ...
                vmware_tools = NttCisServerVMWareTools(status=None, version_status=None, api_version=None)
