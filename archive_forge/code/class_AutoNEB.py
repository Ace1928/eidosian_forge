...
class AutoNEB:
    ...
    def __init__(self, attach_calculators, prefix, n_simul, n_max, iter_folder='AutoNEB_iter', fmax=0.025, maxsteps=10000, k=0.1, climb=True, method='eb', optimizer='FIRE', remove_rotation_and_translation=False, space_energy_ratio=0.5, world=None, parallel=True, smooth_curve=False, interpolate_method='idpp'):
    ...
    def run(self):
        ...
        while n_cur < self.n_max:
            if isinstance(self.k, (float, int)):
                self.k = [self.k] * (len(self.all_images) - 1)
            if self.world.rank == 0:
                print('****Now adding another image until n_max is reached', '({0}/{1})****'.format(n_cur, self.n_max))
            spring_lengths = []
            for j in range(n_cur - 1):
                spring_vec = self.all_images[j + 1].get_positions() - self.all_images[j].get_positions()
                spring_lengths.append(np.linalg.norm(spring_vec))
            ...
            if self.world.rank == 0:
                print('Adding image between {0} and'.format(jmax), '{0}. New image point is selected'.format(jmax + 1), 'on the basis of the biggest ' + t)
            toInterpolate = [self.all_images[jmax]]
            toInterpolate += [toInterpolate[0].copy()]
            toInterpolate += [self.all_images[jmax + 1]]
            neb = NEB(toInterpolate)
        ...
        x2 = []
        for i in range(peak):
            v = (self.all_images[i].get_positions() + self.all_images[i + 1].get_positions()) / 2 - self.all_images[0].get_positions()
