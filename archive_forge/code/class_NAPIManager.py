...
class NAPIManager(object):
    ...
    def __init__(self, jrpc_urls, perobject_jrpc_urls, module_primary_key, url_params, module, conn, top_level_schema_name=None):
        ...
        if YAML_IMPORT_ERROR:
            raise_from(Exception('YAML must be installed to use this plugin'), YAML_IMPORT_ERROR)
    ...
    def _version_matched(self, v_ranges):
        ...
        for vr in v_ranges:
            min_v = vr[0].split('.')
            min_vn = int(min_v[0]) * 10000 + int(min_v[1]) * 100 + int(min_v[2])
            ...
            if vr[1] == '':
                ...
                break
            max_v = vr[1].split('.')
            max_vn = int(max_v[0]) * 10000 + int(max_v[1]) * 100 + int(max_v[2])
        ...
        return (False, 'Current FortiManager version %s.%s.%s do not support this feature. Supported version range: %s.' % (self.system_status['Major'], self.system_status['Minor'], self.system_status['Patch'], supported_v))
    ...
    def update_object(self, mvalue):
        ...
        params = {'url': url_updating, self.top_level_schema_name: raw_attributes}
    ...
    def create_object(self):
        ...
        raw_attributes = remove_aliases(self.module.params, self.metadata)
        raw_attributes = raw_attributes.get(self.module_level2_name, {})
        params = {'url': url_creating, self.top_level_schema_name: raw_attributes}
    ...
    def do_exit(self, response, changed=True):
        ...
        if 'url' in response_data:
            ...
            if response_data['url'] == '/sys/hitcount':
                ...
                if 'taskid' in response_data and isinstance(result['response_data'], dict) and ('task' not in result['response_data']):
