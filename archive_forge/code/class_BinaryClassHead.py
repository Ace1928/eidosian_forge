...
class BinaryClassHead(base_head.Head):
  ...
  """
    ...
    def __init__(self, weight_column=None, thresholds=None, label_vocabulary=None, loss_reduction=tf.losses.Reduction.SUM_OVER_BATCH_SIZE, loss_fn=None, name=None):
        ...
        for threshold in self._thresholds:
            accuracy_keys.append(self._summary_key(keys.ACCURACY_AT_THRESHOLD % threshold))
            precision_keys.append(self._summary_key(keys.PRECISION_AT_THRESHOLD % threshold))
            recall_keys.append(self._summary_key(keys.RECALL_AT_THRESHOLD % threshold))
    ...
    def _processed_labels(self, logits, labels):
        ...
        labels = base_head.check_dense_labels_match_logits_and_reshape(labels=labels, logits=logits, expected_labels_dimension=1)
    ...
    def _unweighted_loss_and_weights(self, logits, labels, features):
        ...
        if self._loss_fn:
            unweighted_loss = base_head.call_loss_fn(loss_fn=self._loss_fn, labels=labels, logits=logits, features=features, expected_loss_dim=1)
        else:
    ...
    def loss(self, labels, logits, features=None, mode=None, regularization_losses=None):
        ...
        del mode
        with ops.name_scope('losses', values=(logits, labels, regularization_losses, features)):
            logits = base_head.check_logits_final_dim(logits, self.logits_dimension)
            labels = self._processed_labels(logits, labels)
            unweighted_loss, weights = self._unweighted_loss_and_weights(logits, labels, features)
            training_loss = tf.compat.v2.keras.__internal__.losses.compute_weighted_loss(unweighted_loss, sample_weight=weights, reduction=self._loss_reduction)
            regularization_loss = tf.math.add_n(regularization_losses) if regularization_losses is not None else None
            regularized_training_loss = training_loss + regularization_loss if regularization_loss is not None else training_loss
    ...
    """
        ...
        valid_keys = [pred_keys.LOGITS, pred_keys.LOGISTIC, pred_keys.PROBABILITIES, pred_keys.CLASS_IDS, pred_keys.CLASSES, pred_keys.ALL_CLASS_IDS, pred_keys.ALL_CLASSES]
        if keys:
        ...
        with ops.name_scope('predictions', values=(logits,)):
            ...
            if pred_keys.ALL_CLASS_IDS in keys:
                predictions[pred_keys.ALL_CLASS_IDS] = base_head.all_class_ids(logits, n_classes=2)
            if pred_keys.ALL_CLASSES in keys:
                predictions[pred_keys.ALL_CLASSES] = base_head.all_classes(logits, n_classes=2, label_vocabulary=self._label_vocabulary)
            return predictions
    ...
    def metrics(self, regularization_losses=None):
        ...
        with ops.name_scope('metrics', values=(regularization_losses,)):
            eval_metrics = {}
            eval_metrics[self._loss_mean_key] = tf.keras.metrics.Mean(name=keys.LOSS_MEAN)
            eval_metrics[self._accuracy_key] = tf.keras.metrics.Accuracy(name=keys.ACCURACY)
            eval_metrics[self._precision_key] = tf.keras.metrics.Precision(name=keys.PRECISION)
            eval_metrics[self._recall_key] = tf.keras.metrics.Recall(name=keys.RECALL)
            eval_metrics[self._prediction_mean_key] = tf.keras.metrics.Mean(name=keys.PREDICTION_MEAN)
            eval_metrics[self._label_mean_key] = tf.keras.metrics.Mean(name=keys.LABEL_MEAN)
            eval_metrics[self._accuracy_baseline_key] = tf.keras.metrics.Mean(name=keys.ACCURACY_BASELINE)
            eval_metrics[self._auc_key] = tf.keras.metrics.AUC(name=keys.AUC)
            eval_metrics[self._auc_pr_key] = tf.keras.metrics.AUC(curve='PR', name=keys.AUC_PR)
            if regularization_losses is not None:
                eval_metrics[self._loss_regularization_key] = tf.keras.metrics.Mean(name=keys.LOSS_REGULARIZATION)
            for i, threshold in enumerate(self._thresholds):
                eval_metrics[self._accuracy_keys[i]] = tf.keras.metrics.BinaryAccuracy(name=self._accuracy_keys[i], threshold=threshold)
                eval_metrics[self._precision_keys[i]] = tf.keras.metrics.Precision(name=self._precision_keys[i], thresholds=threshold)
                eval_metrics[self._recall_keys[i]] = tf.keras.metrics.Recall(name=self._recall_keys[i], thresholds=threshold)
        return eval_metrics
    ...
    def update_metrics(self, eval_metrics, features, logits, labels, regularization_losses=None):
        ...
        labels = self._processed_labels(logits, labels)
        unweighted_loss, weights = self._unweighted_loss_and_weights(logits, labels, features)
        eval_metrics[self._loss_mean_key].update_state(values=unweighted_loss, sample_weight=weights)
        ...
        base_head.update_metric_with_broadcast_weights(eval_metrics[self._prediction_mean_key], logistic, weights)
        base_head.update_metric_with_broadcast_weights(eval_metrics[self._label_mean_key], labels, weights)
        self._update_accuracy_baseline(eval_metrics)
        self._update_auc(auc_metric=eval_metrics[self._auc_key], labels=labels, predictions=logistic, weights=weights)
        self._update_auc(auc_metric=eval_metrics[self._auc_pr_key], labels=labels, predictions=logistic, weights=weights)
        if regularization_losses is not None:
        ...
        for i in range(len(self._thresholds)):
            eval_metrics[self._accuracy_keys[i]].update_state(y_true=labels, y_pred=logistic, sample_weight=weights)
            eval_metrics[self._precision_keys[i]].update_state(y_true=labels, y_pred=logistic, sample_weight=weights)
            eval_metrics[self._recall_keys[i]].update_state(y_true=labels, y_pred=logistic, sample_weight=weights)
        return eval_metrics
    ...
    """
        with ops.name_scope(self._name, 'head'):
            ...
            if mode == ModeKeys.PREDICT:
                ...
                classifier_output = base_head.classification_output(scores=probabilities, n_classes=2, label_vocabulary=self._label_vocabulary)
                return model_fn._TPUEstimatorSpec(mode=ModeKeys.PREDICT, predictions=predictions, export_outputs={base_head.DEFAULT_SERVING_KEY: classifier_output, base_head.CLASSIFY_SERVING_KEY: classifier_output, base_head.REGRESS_SERVING_KEY: export_output.RegressionOutput(value=logistic), base_head.PREDICT_SERVING_KEY: export_output.PredictOutput(predictions)})
            regularized_training_loss = self.loss(logits=logits, labels=labels, features=features, mode=mode, regularization_losses=regularization_losses)
            if mode == ModeKeys.EVAL:
                eval_metrics = self.metrics(regularization_losses=regularization_losses)
                return model_fn._TPUEstimatorSpec(mode=ModeKeys.EVAL, predictions=predictions, loss=regularized_training_loss, eval_metrics=base_head.create_eval_metrics_tuple(self.update_metrics, {'eval_metrics': eval_metrics, 'features': features, 'logits': logits, 'labels': labels, 'regularization_losses': regularization_losses}))
            train_op = base_head.create_estimator_spec_train_op(head_name=self._name, optimizer=optimizer, train_op_fn=train_op_fn, update_ops=update_ops, trainable_variables=trainable_variables, regularized_training_loss=regularized_training_loss, loss_reduction=self._loss_reduction)
        base_head.create_estimator_spec_summary(regularized_training_loss=regularized_training_loss, regularization_losses=regularization_losses, summary_key_fn=self._summary_key)
