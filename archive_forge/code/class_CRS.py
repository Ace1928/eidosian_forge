...
class CRS:
    ...
    * `opendatacube <https://github.com/opendatacube/datacube-core/blob/83bae20d2a2469a6417097168fd4ede37fd2abe5/datacube/utils/geometry/_base.py>`_  # noqa: E501
    ...
    def to_cf(self, wkt_version: Union[WktVersion, str]=WktVersion.WKT2_2019, errcheck: bool=False) -> dict:
        """
        .. versionadded:: 2.2.0
        ...
        if self.is_bound and self.coordinate_operation and self.coordinate_operation.towgs84 and self.source_crs:
            sub_cf: dict[str, Any] = self.source_crs.to_cf(wkt_version=wkt_version, errcheck=errcheck)
            sub_cf.pop('crs_wkt')
        ...
        if self.is_vertical:
            ...
            return cf_dict
        if self.ellipsoid:
            cf_dict.update(semi_major_axis=self.ellipsoid.semi_major_metre, semi_minor_axis=self.ellipsoid.semi_minor_metre, inverse_flattening=self.ellipsoid.inverse_flattening)
        ...
        if self.is_geographic:
            if self.coordinate_operation:
                if self.coordinate_operation.method_name.lower() not in _INVERSE_GEOGRAPHIC_GRID_MAPPING_NAME_MAP:
                    if errcheck:
                        warnings.warn(f'Unsupported coordinate operation: {self.coordinate_operation.method_name}')
                    return {'crs_wkt': cf_dict['crs_wkt']}
                cf_dict.update(_INVERSE_GEOGRAPHIC_GRID_MAPPING_NAME_MAP[self.coordinate_operation.method_name.lower()](self.coordinate_operation))
            else:
        ...
        coordinate_operation_name = None if not coordinate_operation else coordinate_operation.method_name.lower().replace(' ', '_')
        if coordinate_operation_name not in _INVERSE_GRID_MAPPING_NAME_MAP:
            if errcheck:
                if coordinate_operation:
                    warnings.warn(f'Unsupported coordinate operation: {coordinate_operation.method_name}')
                else:
                    warnings.warn('Coordinate operation not found.')
            return {'crs_wkt': cf_dict['crs_wkt']}
        cf_dict.update(_INVERSE_GRID_MAPPING_NAME_MAP[coordinate_operation_name](coordinate_operation))
        return cf_dict
    ...
    def cs_to_cf(self) -> list[dict]:
        ...
        if self.type_name == 'Temporal CRS' and self.datum:
            ...
            cf_temporal_axis = {'standard_name': 'time', 'long_name': 'time', 'calendar': datum_json.get('calendar', 'proleptic_gregorian').lower().replace(' ', '_'), 'axis': 'T'}
        ...
        if self.coordinate_system:
            cf_axis_list.extend(self.coordinate_system.to_cf(rotated_pole=rotated_pole(self)))
        elif self.is_bound and self.source_crs and self.source_crs.coordinate_system:
            cf_axis_list.extend(self.source_crs.coordinate_system.to_cf(rotated_pole=rotated_pole(self.source_crs)))
        else:
            for sub_crs in self.sub_crs_list:
    ...
    def geodetic_crs(self) -> Optional['CRS']:
        ...
        """
        return None if self._crs.geodetic_crs is None else self.__class__(self._crs.geodetic_crs)
    ...
    def source_crs(self) -> Optional['CRS']:
        ...
        CRS
        """
        return None if self._crs.source_crs is None else self.__class__(self._crs.source_crs)
