...
class FER2013(VisionDataset):
    ...
    """
    _RESOURCES = {'train': ('train.csv', '3f0dfb3d3fd99c811a1299cb947e3131'), 'test': ('test.csv', 'b02c2298636a634e8c2faabbf3ea9a23')}

    def __init__(self, root: str, split: str='train', transform: Optional[Callable]=None, target_transform: Optional[Callable]=None) -> None:
        ...
        with open(data_file, 'r', newline='') as file:
            self._samples = [(torch.tensor([int(idx) for idx in row['pixels'].split()], dtype=torch.uint8).reshape(48, 48), int(row['emotion']) if 'emotion' in row else None) for row in csv.DictReader(file)]
