...
class CometLogger(Logger):
    ...
    **Access the Comet Experiment object:**
    ...
    def __init__(self, api_key: Optional[str]=None, save_dir: Optional[str]=None, project_name: Optional[str]=None, rest_api_key: Optional[str]=None, experiment_name: Optional[str]=None, experiment_key: Optional[str]=None, offline: bool=False, prefix: str='', **kwargs: Any):
    ...
    def experiment(self) -> Union['Experiment', 'ExistingExperiment', 'OfflineExperiment']:
        """Actual Comet object. To use Comet features in your :class:`~pytorch_lightning.core.LightningModule` do the
        following.
        ...
        try:
            if self.mode == 'online':
                if self._experiment_key is None:
                    ...
                    self._experiment = ExistingExperiment(api_key=self.api_key, project_name=self._project_name, previous_experiment=self._experiment_key, **self._kwargs)
            else:
                self._experiment = OfflineExperiment(offline_directory=self.save_dir, project_name=self._project_name, **self._kwargs)
    ...
    def finalize(self, status: str) -> None:
        """When calling ``self.experiment.end()``, that experiment won't log any more data to Comet. That's why, if you
