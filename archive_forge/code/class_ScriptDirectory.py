...
class ScriptDirectory:
    ...
    def __init__(self, dir: str, file_template: str=_default_file_template, truncate_slug_length: Optional[int]=40, version_locations: Optional[List[str]]=None, sourceless: bool=False, output_encoding: str='utf-8', timezone: Optional[str]=None, hook_config: Optional[Mapping[str, str]]=None, recursive_version_locations: bool=False, messaging_opts: MessagingOptions=cast('MessagingOptions', util.EMPTY_DICT)) -> None:
        ...
        if not os.access(dir, os.F_OK):
            raise util.CommandError("Path doesn't exist: %r.  Please use the 'init' command to create a new scripts folder." % os.path.abspath(dir))
    ...
    def from_config(cls, config: Config) -> ScriptDirectory:
        ...
        if version_locations_str:
            ...
            split_on_path = {None: None, 'space': ' ', 'os': os.pathsep, ':': ':', ';': ';'}
            try:
                split_char: Optional[str] = split_on_path[version_path_separator]
            except KeyError as ke:
                raise ValueError("'%s' is not a valid value for version_path_separator; expected 'space', 'os', ':', ';'" % version_path_separator) from ke
            else:
                if split_char is None:
                    version_locations = _split_on_space_comma.split(version_locations_str)
                else:
                    version_locations = [x for x in version_locations_str.split(split_char) if x]
        else:
        ...
        return ScriptDirectory(util.coerce_resource_to_filename(script_location), file_template=config.get_main_option('file_template', _default_file_template), truncate_slug_length=truncate_slug_length, sourceless=config.get_main_option('sourceless') == 'true', output_encoding=config.get_main_option('output_encoding', 'utf-8'), version_locations=version_locations, timezone=config.get_main_option('timezone'), hook_config=config.get_section('post_write_hooks', {}), recursive_version_locations=rvl, messaging_opts=config.messaging_opts)
    ...
    def get_current_head(self) -> Optional[str]:
        ...
        """
        with self._catch_revision_errors(multiple_heads='The script directory has multiple heads (due to branching).Please use get_heads(), or merge the branches using alembic merge.'):
    ...
    def _stamp_revs(self, revision: _RevIdType, heads: _RevIdType) -> List[StampStep]:
        with self._catch_revision_errors(multiple_heads='Multiple heads are present; please specify a single target revision'):
            ...
            for rev in util.to_tuple(revision):
                if rev:
                    filtered_heads.extend(self.revision_map.filter_for_lineage(cast(Sequence[Script], heads_revs), rev, include_dependencies=True))
            filtered_heads = util.unique_list(filtered_heads)
            dests = self.get_revisions(revision) or [None]
            for dest in dests:
                if dest is None:
                    steps.extend([migration.StampStep(head.revision, None, False, True, self.revision_map) for head in filtered_heads])
                    continue
                ...
                if descendants.intersection(filtered_heads):
                    ...
                    step = migration.StampStep(todo_heads, dest.revision, False, False, self.revision_map)
                    steps.append(step)
                    continue
                elif ancestors.intersection(filtered_heads):
                    todo_heads = [head.revision for head in filtered_heads]
                    step = migration.StampStep(todo_heads, dest.revision, True, False, self.revision_map)
                    steps.append(step)
                    continue
                else:
