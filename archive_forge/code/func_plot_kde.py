...
def plot_kde(density, lower, upper, density_q, xmin, xmax, ymin, ymax, gridsize, values, values2, rug, label, quantiles, rotated, contour, fill_last, figsize, textsize, plot_kwargs, fill_kwargs, rug_kwargs, contour_kwargs, contourf_kwargs, pcolormesh_kwargs, is_circular, ax, legend, backend_kwargs, show, return_glyph):
    """Bokeh kde plot."""
    ...
    if values2 is None:
        ...
        if quantiles is not None:
            ...
            for quant_0, quant_1 in zip(quantiles[:-1], quantiles[1:]):
                ...
                if idx.sum():
                    ...
                    patch_y = np.concatenate((np.zeros_like(density[idx]), [density[idx][-1]], density[idx][::-1], [0]))
                    if not rotated:
            ...
            if fill_kwargs.get('fill_alpha', False):
                ...
                patch_y = np.concatenate((np.zeros_like(density), [density[-1]], density[::-1], [0]))
                if not rotated:
                    patch = ax.patch(patch_x, patch_y, **fill_kwargs)
                else:
                    patch = ax.patch(patch_y, patch_x, **fill_kwargs)
        ...
        if contour:
            ...
            contour_generator = contourpy.contour_generator(x=x_x, y=y_y, z=scaled_density, **contourpy_kwargs)
            levels = 9
            ...
            for i, (level, level_upper, color) in enumerate(zip(levels_scaled[:-1], levels_scaled[1:], colors[1:])):
            ...
            if isinstance(cmap, Callable):
                ...
                colors = cmap
            image = ax.image(image=[density.T], x=xmin, y=ymin, dw=(xmax - xmin) / density.shape[0], dh=(ymax - ymin) / density.shape[1], palette=colors, **pcolormesh_kwargs)
            glyphs.append(image)
