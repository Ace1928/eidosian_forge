...
class TestFileUtils(unittest.TestCase):
    ...
    def test_this_file(self):
        ...
        self.assertIn(subprocess.run([sys.executable, '-c', 'from pyomo.common.fileutils import this_file;print(this_file())'], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, universal_newlines=True).stdout.strip(), ['<string>', '<module>'])
        self.assertEqual(subprocess.run([sys.executable], input='from pyomo.common.fileutils import this_file;print(this_file())', stdout=subprocess.PIPE, stderr=subprocess.STDOUT, universal_newlines=True).stdout.strip(), '<stdin>')
    ...
    def test_find_executable(self):
        ...
        if _system() in ('windows', 'cygwin'):
            ...
            self.assertIsNone(found)
        self._check_file(find_executable(f_in_cwd_ldlib_path), os.path.join(self.tmpdir, f_in_cwd_ldlib_path))
        self._check_file(os.path.join(pathdir, f_in_cwd_ldlib_path), find_executable(f_in_cwd_ldlib_path, cwd=False))
        self._check_file(find_executable(f_in_path_extension), os.path.join(pathdir, f_in_path_extension) + exeExt)
        ...
        self._check_file(find_executable(f_in_path, pathlist=os.pathsep + pathdir + os.pathsep), os.path.join(pathdir, f_in_path))
        self._check_file(os.path.join(ldlibdir, f_in_cwd_ldlib_path), find_executable(f_in_cwd_ldlib_path, cwd=False, pathlist=[ldlibdir]))
        self._check_file(os.path.join(config_bindir, f_in_configbin), find_executable(f_in_configbin))
