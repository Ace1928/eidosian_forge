...
def test_worker_apply_lora(sql_lora_files):
    worker = Worker(model_config=ModelConfig('meta-llama/Llama-2-7b-hf', 'meta-llama/Llama-2-7b-hf', tokenizer_mode='auto', trust_remote_code=False, download_dir=None, load_format='dummy', seed=0, dtype='float16', revision=None), parallel_config=ParallelConfig(1, 1, False), scheduler_config=SchedulerConfig(32, 32, 32, 256), device_config=DeviceConfig('cuda'), local_rank=0, rank=0, lora_config=LoRAConfig(max_lora_rank=8, max_cpu_loras=32, max_loras=32), distributed_init_method=f'file://{tempfile.mkstemp()[1]}')
    worker.init_model()
    worker.load_model()
