...
def test_list_policies_on_cluster(self):
    ...
    cluster_policy = self.user_cloud.get_policy_on_cluster(name_or_id=cluster['cluster']['id'], policy_name_or_id=policy['id'])
    policy_list = {'cluster_policies': [cluster_policy]}
    cluster_policies = self.user_cloud.list_policies_on_cluster(cluster['cluster']['id'])
