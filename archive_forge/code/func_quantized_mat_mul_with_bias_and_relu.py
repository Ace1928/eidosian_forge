...
add and relu fusion.
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'QuantizedMatMulWithBiasAndRelu', name, a, b, bias, min_a, max_a, min_b, max_b, 'Toutput', Toutput, 'transpose_a', transpose_a, 'transpose_b', transpose_b, 'input_quant_mode', input_quant_mode)
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('QuantizedMatMulWithBiasAndRelu', a=a, b=b, bias=bias, min_a=min_a, max_a=max_a, min_b=min_b, max_b=max_b, Toutput=Toutput, transpose_a=transpose_a, transpose_b=transpose_b, input_quant_mode=input_quant_mode, name=name)
    _result = _outputs[:]
    if _execute.must_record_gradient():
        _attrs = ('T1', _op._get_attr_type('T1'), 'T2', _op._get_attr_type('T2'), 'Toutput', _op._get_attr_type('Toutput'), 'transpose_a', _op._get_attr_bool('transpose_a'), 'transpose_b', _op._get_attr_bool('transpose_b'), 'input_quant_mode', _op.get_attr('input_quant_mode'))
