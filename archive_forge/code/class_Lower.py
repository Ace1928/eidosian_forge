...
class Lower(BaseLower):
    ...
    def lower_yield(self, retty, inst):
        ...
        retval = self.context.get_return_value(self.builder, actual_rettyp, yret)
        self.call_conv.return_value(self.builder, retval)
    ...
    def lower_binop(self, resty, expr, op):
        ...
        def try_static_impl(tys, args):
            ...
            try:
                ...
                return None
        res = try_static_impl((_lit_or_omitted(static_lhs), _lit_or_omitted(static_rhs)), (static_lhs, static_rhs))
        if res is not None:
    ...
    def lower_expr(self, resty, expr):
        if expr.op == 'binop':
            ...
            signature = typing.signature(resty, self.typeof(expr.value.name), _lit_or_omitted(expr.index))
            try:
            ...
            signature = typing.signature(resty, self.typeof(expr.value.name), self.typeof(expr.index.name))
            ...
            castvals = [self.context.cast(self.builder, val, fromty, toty) for val, toty, fromty in zip(itemvals, resty, itemtys)]
            ...
            if isinstance(resty, types.LiteralList):
                castvals = [self.context.cast(self.builder, val, fromty, toty) for val, toty, fromty in zip(itemvals, resty.types, itemtys)]
                tup = self.context.make_tuple(self.builder, types.Tuple(resty.types), castvals)
                self.incref(resty, tup)
                return tup
            else:
                castvals = [self.context.cast(self.builder, val, fromty, resty.dtype) for val, fromty in zip(itemvals, itemtys)]
            ...
            return self.context.build_map(self.builder, resty, list(zip(key_types, value_types)), list(zip(keys, values)))
        elif expr.op == 'cast':
            val = self.loadvar(expr.value.name)
            ty = self.typeof(expr.value.name)
    ...
    def storevar(self, value, name, argidx=None):
        ...
        if name in self._singly_assigned_vars and (not self._disable_sroa_like_opt):
            ...
            if value.type != ptr.type.pointee:
                msg = "Storing {value.type} to ptr of {ptr.type.pointee} ('{name}'). FE type {fetype}".format(value=value, ptr=ptr, fetype=fetype, name=name)
            ...
            if argidx is not None:
                ...
                self.debuginfo.mark_variable(self.builder, ptr, name=name, lltype=lltype, size=sizeof, line=loc.line, datamodel=datamodel, argidx=argidx)
            else:
                self.builder.store(value, ptr)
