...
class SqlCacheConfig(BaseModel):
    ...
    cull_limit: int = 10
    sqlite_auto_vacuum: int = 1
    cache_size: int = 2 ** 13
    sqlite_journal_mode: str = 'wal'
    mmap_size: ByteSize = Field(default=OPTIMIZED_SETTINGS['standard']['mmap_size'])
    sqlite_synchronous: int = 1
    disk_min_file_size: ByteSize = Field(default=OPTIMIZED_SETTINGS['standard']['min_file_size'])
    ...
    def sql_settings(self) -> Dict[str, Any]:
        return {'statistics': self.statistics, 'tag_index': self.tag_index, 'eviction_policy': self.eviction_policy, 'size_limit': self.size_limit, 'cull_limit': self.cull_limit, 'sqlite_auto_vacuum': self.sqlite_auto_vacuum, 'sqlite_cache_size': self.cache_size, 'sqlite_journal_mode': self.sqlite_journal_mode, 'sqlite_mmap_size': self.mmap_size, 'sqlite_synchronous': self.sqlite_synchronous, 'disk_min_file_size': self.disk_min_file_size, 'disk_pickle_protocol': self.disk_pickle_protocol}
