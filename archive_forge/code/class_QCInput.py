...
class QCInput(InputFile):
    ...
    def __init__(self, molecule: Molecule | list[Molecule] | Literal['read'], rem: dict, opt: dict[str, list] | None=None, pcm: dict | None=None, solvent: dict | None=None, smx: dict | None=None, scan: dict[str, list] | None=None, van_der_waals: dict[str, float] | None=None, vdw_mode: str='atomic', plots: dict | None=None, nbo: dict | None=None, geom_opt: dict | None=None, cdft: list[list[dict]] | None=None, almo_coupling: list[list[tuple[int, int]]] | None=None, svp: dict | None=None, pcm_nonels: dict | None=None):
        """
        Args:
        ...
        valid_job_types = ['opt', 'optimization', 'sp', 'freq', 'frequency', 'force', 'nmr', 'ts', 'pes_scan']
    ...
    def from_str(cls, string: str) -> Self:
        ...
        return cls(molecule, rem, opt=opt, solvent=solvent, pcm=pcm, smx=smx, scan=scan, van_der_waals=vdw, vdw_mode=vdw_mode, plots=plots, nbo=nbo, geom_opt=geom_opt, cdft=cdft, almo_coupling=almo_coupling, svp=svp, pcm_nonels=pcm_nonels)
    ...
    def read_scan(string: str) -> dict[str, list]:
        ...
        """
        header = '^\\s*\\$scan'
        row = '\\s*(stre|bend|tors|STRE|BEND|TORS)\\s+((?:[\\-\\.0-9]+\\s*)+)'
        footer = '^\\s*\\$end'
