...
class NetAppOntapNtfsSd(object):
    ...
    def __init__(self):
        ...
        self.argument_spec.update(dict(state=dict(required=False, choices=['present', 'absent'], default='present'), vserver=dict(required=True, type='str'), name=dict(required=True, type='str'), owner=dict(required=False, type='str'), group=dict(required=False, type='str'), control_flags_raw=dict(required=False, type='int')))
    ...
    def modify_ntfs_sd(self):
        ...
        try:
            ...
            self.module.fail_json(msg='Error modifying NTFS security descriptor %s: %s' % (self.parameters['name'], to_native(error)), exception=traceback.format_exc())
