...
class DualLogger:
    def __init__(self, name: str, file_path: str, level: Union[int, str] = logging.INFO, formatter: Optional[FormatterType] = None) -> None:
        self.logger: logging.Logger = logging.getLogger(name)
        self.logger.setLevel(level)
        self.file_handler: logging.FileHandler = logging.FileHandler(file_path)
        self.stream_handler: logging.StreamHandler = logging.StreamHandler()
        self.formatter: logging.Formatter = logging.Formatter(formatter if formatter else '%(asctime)s - %(name)s - %(levelname)s - %(message)s')
        self.file_handler.setFormatter(self.formatter)
        self.stream_handler.setFormatter(self.formatter)
        self.logger.addHandler(self.file_handler)
        self.logger.addHandler(self.stream_handler)

    def log(self, level: Union[int, str], message: LogMessage) -> None:
    ...
    def format_log_message(self, message: Dict[str, Any]) -> str:
        formatted_message: str = " | ".join(f"{key}: {value}" for key, value in message.items())
