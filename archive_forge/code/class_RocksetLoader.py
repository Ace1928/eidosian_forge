...
class RocksetLoader(BaseLoader):
    ...
    def __init__(self, client: Any, query: Any, content_keys: List[str], metadata_keys: Optional[List[str]]=None, content_columns_joiner: Callable[[List[Tuple[str, Any]]], str]=default_joiner):
        ...
        if not isinstance(query, QueryRequestSql):
            raise ValueError(f'query should be an instance of rockset.model.QueryRequestSql, got {type(query)}')
        self.client = client
        self.query = query
    ...
    def lazy_load(self) -> Iterator[Document]:
        query_results = self.client.Queries.query(sql=self.query).results
        for doc in query_results:
            try:
                yield Document(page_content=self.content_columns_joiner([(col, doc[col]) for col in self.content_keys]), metadata={col: doc[col] for col in self.metadata_keys} if self.metadata_keys is not None else doc)
            except KeyError as e:
                raise ColumnNotFoundError(e.args[0], self.query)