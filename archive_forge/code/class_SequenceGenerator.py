...
class SequenceGenerator:
    ...
    def __call__(self, prompts: Union[str, List[str]], max_tokens: Optional[int]=None, stop_at: Optional[Union[str, List[str]]]=None, rng: Optional[torch.Generator]=None) -> Union[FormattedOutput, List[FormattedOutput], List[List[FormattedOutput]]]:
        ...
        weights = torch.zeros(batch_size * num_samples, dtype=torch.float, device=self.device)
        states = sequence_generator(self.model, self.sampler, fsms, prompt_token_ids, weights, attention_masks, fsm_states, rng=rng)
        while True:
            try:
                ...
                if max_tokens or stop_sequences:
                    ...
                    if max_tokens and len(generated_token_ids[0]) >= max_tokens:
                        break
                    if stop_sequences and self.is_stop_sequence_found(self.tokenizer.decode(generated_token_ids), stop_sequences):
                        break
        ...
        stripped = [self.strip_stop_sequences(sequence, stop_sequences) for sequence in generated]
    ...
    def stream(self, prompts: Union[str, List[str]], max_tokens: Optional[int]=None, stop_at: Optional[Union[str, List[str]]]=None, rng: Optional[torch.Generator]=None) -> Iterator[Union[List[str], str, List[List[str]]]]:
        ...
        fsm_states = [0 for _ in range(batch_size * num_samples)]
        fsms = [self.fsm.copy() for _ in range(batch_size * num_samples)]
        weights = torch.zeros(batch_size * num_samples, dtype=torch.float, device=prompt_token_ids.device)
        if rng is None:
