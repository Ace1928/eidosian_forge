...
class RetryWithErrorOutputParser(BaseOutputParser[T]):
    ...
    @classmethod
    def from_llm(cls, llm: BaseLanguageModel, parser: BaseOutputParser[T], prompt: BasePromptTemplate=NAIVE_RETRY_WITH_ERROR_PROMPT, max_retries: int=1) -> RetryWithErrorOutputParser[T]:
        ...
        while retries <= self.max_retries:
            try:
                ...
                if retries == self.max_retries:
                    ...
                    retries += 1
                    completion = self.retry_chain.run(prompt=prompt_value.to_string(), completion=completion, error=repr(e))
        ...
        while retries <= self.max_retries:
            try:
                ...
                if retries == self.max_retries:
                    ...
                    retries += 1
                    completion = await self.retry_chain.arun(prompt=prompt_value.to_string(), completion=completion, error=repr(e))
        raise OutputParserException('Failed to parse')
