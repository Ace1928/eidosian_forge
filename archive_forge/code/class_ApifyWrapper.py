...
class ApifyWrapper(BaseModel):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        try:
            ...
            raise ImportError('Could not import apify-client Python package. Please install it with `pip install apify-client`.')
        return values
    ...
    async def acall_actor(self, actor_id: str, run_input: Dict, dataset_mapping_function: Callable[[Dict], Document], *, build: Optional[str]=None, memory_mbytes: Optional[int]=None, timeout_secs: Optional[int]=None) -> 'ApifyDatasetLoader':
        """Run an Actor on the Apify platform and wait for results to be ready.
        Args:
            actor_id (str): The ID or name of the Actor on the Apify platform.
            run_input (Dict): The input object of the Actor that you're trying to run.
        ...
        """
        from langchain_community.document_loaders import ApifyDatasetLoader
        actor_call = await self.apify_client_async.actor(actor_id).call(run_input=run_input, build=build, memory_mbytes=memory_mbytes, timeout_secs=timeout_secs)
        return ApifyDatasetLoader(dataset_id=actor_call['defaultDatasetId'], dataset_mapping_function=dataset_mapping_function)
    ...
    async def acall_actor_task(self, task_id: str, task_input: Dict, dataset_mapping_function: Callable[[Dict], Document], *, build: Optional[str]=None, memory_mbytes: Optional[int]=None, timeout_secs: Optional[int]=None) -> 'ApifyDatasetLoader':
        """Run a saved Actor task on Apify and wait for results to be ready.
        Args:
            task_id (str): The ID or name of the task on the Apify platform.
            task_input (Dict): The input object of the task that you're trying to run.
        ...
        """
        from langchain_community.document_loaders import ApifyDatasetLoader
        task_call = await self.apify_client_async.task(task_id).call(task_input=task_input, build=build, memory_mbytes=memory_mbytes, timeout_secs=timeout_secs)
        return ApifyDatasetLoader(dataset_id=task_call['defaultDatasetId'], dataset_mapping_function=dataset_mapping_function)