...
class ManilaShareType(resource.Resource):
    ...
    PROPERTIES = NAME, IS_PUBLIC, DRIVER_HANDLES_SHARE_SERVERS, EXTRA_SPECS, SNAPSHOT_SUPPORT = ('name', 'is_public', 'driver_handles_share_servers', 'extra_specs', 'snapshot_support')
    properties_schema = {NAME: properties.Schema(properties.Schema.STRING, _('Name of the share type.'), required=True), IS_PUBLIC: properties.Schema(properties.Schema.BOOLEAN, _('Defines if share type is accessible to the public.'), default=True), DRIVER_HANDLES_SHARE_SERVERS: properties.Schema(properties.Schema.BOOLEAN, _('Required extra specification. Defines if share drivers handles share servers.'), required=True), EXTRA_SPECS: properties.Schema(properties.Schema.MAP, _('Extra specs key-value pairs defined for share type.'), update_allowed=True), SNAPSHOT_SUPPORT: properties.Schema(properties.Schema.BOOLEAN, _('Boolean extra spec that used for filtering of backends by their capability to create share snapshots.'), support_status=support.SupportStatus(version='6.0.0'), default=True)}
    default_client_name = 'manila'
    entity = 'share_types'

    def handle_create(self):
        share_type = self.client().share_types.create(name=self.properties.get(self.NAME), spec_driver_handles_share_servers=self.properties.get(self.DRIVER_HANDLES_SHARE_SERVERS), is_public=self.properties.get(self.IS_PUBLIC), spec_snapshot_support=self.properties.get(self.SNAPSHOT_SUPPORT))
