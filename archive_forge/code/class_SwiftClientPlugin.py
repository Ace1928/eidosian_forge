...
class SwiftClientPlugin(client_plugin.ClientPlugin):
    ...
    def _create(self):
        ...
        os_options = {'endpoint_type': endpoint_type, 'service_type': self.OBJECT_STORE, 'region_name': self._get_region_name()}
    ...
    def is_not_found(self, ex):
        return isinstance(ex, exceptions.ClientException) and ex.http_status == 404

    def is_over_limit(self, ex):
        return isinstance(ex, exceptions.ClientException) and ex.http_status == 413

    def is_conflict(self, ex):
        return isinstance(ex, exceptions.ClientException) and ex.http_status == 409
