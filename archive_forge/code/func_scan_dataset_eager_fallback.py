...
def scan_dataset_eager_fallback(input_dataset: _atypes.TensorFuzzingAnnotation[_atypes.Variant], initial_state, other_arguments, f, output_types, output_shapes, preserve_cardinality: bool, use_default_device: bool, metadata: str, name, ctx) -> _atypes.TensorFuzzingAnnotation[_atypes.Variant]:
    ...
    _attrs = ('f', f, 'Tstate', _attr_Tstate, 'Targuments', _attr_Targuments, 'output_types', output_types, 'output_shapes', output_shapes, 'preserve_cardinality', preserve_cardinality, 'use_default_device', use_default_device, 'metadata', metadata)
    _result = _execute.execute(b'ScanDataset', 1, inputs=_inputs_flat, attrs=_attrs, ctx=ctx, name=name)
