...
class PlotlyRenderer(Renderer):
    ...
    def open_figure(self, fig, props):
        ...
        self.mpl_fig = fig
        self.plotly_fig['layout'] = go.Layout(width=int(props['figwidth'] * props['dpi']), height=int(props['figheight'] * props['dpi']), autosize=False, hovermode='closest')
        self.mpl_x_bounds, self.mpl_y_bounds = mpltools.get_axes_bounds(fig)
        margin = go.layout.Margin(l=int(self.mpl_x_bounds[0] * self.plotly_fig['layout']['width']), r=int((1 - self.mpl_x_bounds[1]) * self.plotly_fig['layout']['width']), t=int((1 - self.mpl_y_bounds[1]) * self.plotly_fig['layout']['height']), b=int(self.mpl_y_bounds[0] * self.plotly_fig['layout']['height']), pad=0)
    ...
    def open_axes(self, ax, props):
        ...
        self.bar_containers = [c for c in ax.containers if c.__class__.__name__ == 'BarContainer']
        self.current_bars = []
        self.axis_ct += 1
        ...
        mpl_xaxis, mpl_yaxis = mpltools.prep_xy_axis(ax=ax, props=props, x_bounds=self.mpl_x_bounds, y_bounds=self.mpl_y_bounds)
    ...
    def draw_marked_line(self, **props):
        ...
        if props['linestyle']:
            ...
            if props['coordinates'] == 'data':
                line = go.scatter.Line(color=color, width=props['linestyle']['linewidth'], dash=mpltools.convert_dash(props['linestyle']['dasharray']))
            else:
                shape = dict(line=dict(color=color, width=props['linestyle']['linewidth'], dash=mpltools.convert_dash(props['linestyle']['dasharray'])))
        if props['markerstyle']:
            if props['coordinates'] == 'data':
                marker = go.scatter.Marker(opacity=props['markerstyle']['alpha'], color=props['markerstyle']['facecolor'], symbol=mpltools.convert_symbol(props['markerstyle']['marker']), size=props['markerstyle']['markersize'], line=dict(color=props['markerstyle']['edgecolor'], width=props['markerstyle']['edgewidth']))
            else:
                shape = dict(opacity=props['markerstyle']['alpha'], fillcolor=props['markerstyle']['facecolor'], symbol=mpltools.convert_symbol(props['markerstyle']['marker']), size=props['markerstyle']['markersize'], line=dict(color=props['markerstyle']['edgecolor'], width=props['markerstyle']['edgewidth']))
        if props['coordinates'] == 'data':
            marked_line = go.Scatter(mode=mode, name=str(props['label']) if isinstance(props['label'], str) else props['label'], x=[xy_pair[0] for xy_pair in props['data']], y=[xy_pair[1] for xy_pair in props['data']], xaxis='x{0}'.format(self.axis_ct), yaxis='y{0}'.format(self.axis_ct), line=line, marker=marker)
            if self.x_is_mpl_date:
                formatter = self.current_mpl_ax.get_xaxis().get_major_formatter().__class__.__name__
            ...
            warnings.warn("Bummer! Plotly can currently only draw Line2D objects from matplotlib that are in 'data' coordinates!")
    ...
    def draw_image(self, **props):
        ...
        warnings.warn("Aw. Snap! You're gonna have to hold off on the selfies for now. Plotly can't import images from matplotlib yet!")
    ...
    def draw_path_collection(self, **props):
        ...
        if props['offset_coordinates'] == 'data':
            ...
            scatter_props = {'coordinates': 'data', 'data': props['offsets'], 'label': None, 'markerstyle': markerstyle, 'linestyle': None}
            ...
            warnings.warn("Dang! That path collection is out of this world. I totally don't know what to do with it yet! Plotly can only import path collections linked to 'data' coordinates")
    ...
    def draw_text(self, **props):
        ...
        if props['text_type'] == 'xlabel':
            ...
            if props['coordinates'] != 'data':
                ...
                xanchor = props['style']['halign']
                yanchor = mpltools.convert_va(props['style']['valign'])
            annotation = go.layout.Annotation(text=str(props['text']) if isinstance(props['text'], str) else props['text'], opacity=props['style']['alpha'], x=x, y=y, xref=xref, yref=yref, align=align, xanchor=xanchor, yanchor=yanchor, showarrow=False, font=go.layout.annotation.Font(color=props['style']['color'], size=props['style']['fontsize']))
