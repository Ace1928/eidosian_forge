...
class CommandLoop(cmd.Cmd):
    ...
    def __init__(self, commands, prompt=None):
        ...
        for name, command in commands.items():
            if name not in self._special_command_names and isinstance(command, bigquery_command.NewCmd) and command.surface_in_shell:
    ...
    def do_set(self, line: str) -> int:
        ...
        if name not in ('project_id', 'dataset_id') or not 1 <= len(line.split(' ')) <= 2:
