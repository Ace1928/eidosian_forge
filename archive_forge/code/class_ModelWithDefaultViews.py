...
class ModelWithDefaultViews(base_model.ReportModel):
    ...
    def __init__(self, *args, **kwargs):
        self.views = {'text': textviews.KeyValueView(), 'json': jsonviews.KeyValueView(), 'xml': xmlviews.KeyValueView()}
    ...
    def __getattr__(self, attrname):
        if attrname[:3] == 'to_':
            if self.views[attrname[3:]] is not None:
                ...
                raise NotImplementedError(('Model {cn.__module__}.{cn.__name__} does not have' + ' a default view for {tp}').format(cn=type(self), tp=attrname[3:]))
        else:
