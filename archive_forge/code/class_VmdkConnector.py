...
class VmdkConnector(initiator_connector.InitiatorConnector):
    ...
    def __init__(self, *args, **kwargs):
        if vim_util is None:
            message = _('Missing oslo_vmware python module, ensure oslo.vmware library is installed and available.')
    ...
    def _create_session(self):
        return api.VMwareAPISession(self._ip, self._username, self._password, self._api_retry_count, self._task_poll_interval, port=self._port, cacert=self._ca_file, insecure=self._insecure)

    def _create_temp_file(self, *args, **kwargs):
        fileutils.ensure_tree(self._tmp_dir)
        fd, tmp = tempfile.mkstemp(*args, dir=self._tmp_dir, **kwargs)
        os.close(fd)
        return tmp
    ...
    def connect_volume(self, connection_properties):
        ...
        tmp_file_path = self._create_temp_file(suffix='.vmdk', prefix=connection_properties['volume_id'])
        backing = vim_util.get_moref(connection_properties['volume'], 'VirtualMachine')
        vmdk_path = connection_properties['vmdk_path']
        vmdk_size = connection_properties['vmdk_size']
        try:
            self._download_vmdk(tmp_file_path, session, backing, vmdk_path, vmdk_size)
        finally:
            session.logout()
    ...
    def _disconnect(self, backing, tmp_file_path, session, ds_ref, dc_ref, vmdk_path):
        dstore = datastore.get_datastore_by_ref(session, ds_ref)
        ds_path = dstore.build_path(VmdkConnector.TMP_IMAGES_DATASTORE_FOLDER_PATH, os.path.basename(tmp_file_path))
        self._create_temp_ds_folder(session, str(ds_path.parent), dc_ref)
        with open(tmp_file_path, 'rb') as tmp_file:
            dc_name = session.invoke_api(vim_util, 'get_object_property', session.vim, dc_ref, 'name')
            cookies = session.vim.client.cookiejar
            cacerts = self._ca_file if self._ca_file else not self._insecure
            self._upload_vmdk(tmp_file, self._ip, self._port, dc_name, dstore.name, cookies, ds_path.rel_path, os.path.getsize(tmp_file_path), cacerts, self._timeout)
        disk_device = self._get_disk_device(session, backing)
        self._detach_disk_from_backing(session, backing, disk_device)
        src = str(ds_path)
        LOG.debug('Copying %(src)s to %(dest)s', {'src': src, 'dest': vmdk_path})
        disk_mgr = session.vim.service_content.virtualDiskManager
        task = session.invoke_api(session.vim, 'CopyVirtualDisk_Task', disk_mgr, sourceName=src, sourceDatacenter=dc_ref, destName=vmdk_path, destDatacenter=dc_ref)
        ...
        task = session.invoke_api(session.vim, 'DeleteDatastoreFile_Task', file_mgr, name=src, datacenter=dc_ref)
        session.wait_for_task(task)
        ...
        try:
            if os.path.getmtime(tmp_file_path) > device_info['last_modified']:
                ...
                if self._snapshot_exists(session, backing):
                    msg = _('Backing of volume: %s contains one or more snapshots; cannot disconnect.') % connection_properties['volume_id']
                    raise exception.BrickException(message=msg)
                ds_ref = vim_util.get_moref(connection_properties['datastore'], 'Datastore')
                dc_ref = vim_util.get_moref(connection_properties['datacenter'], 'Datacenter')
                vmdk_path = connection_properties['vmdk_path']
                self._disconnect(backing, tmp_file_path, session, ds_ref, dc_ref, vmdk_path)
        finally:
