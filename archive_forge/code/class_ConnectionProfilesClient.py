...
class ConnectionProfilesClient(object):
    ...
    def _GetSslConfig(self, args):
        return self.messages.SslConfig(clientKey=args.private_key, clientCertificate=args.GetValue(self._ClientCertificateArgName()), caCertificate=args.ca_certificate)
    ...
    def _UpdateMySqlSslConfig(self, connection_profile, args, update_fields):
        ...
        if args.IsSpecified(self._ClientCertificateArgName()):
            connection_profile.mysql.ssl.clientCertificate = args.GetValue(self._ClientCertificateArgName())
    ...
    def _GetMySqlConnectionProfile(self, args):
        ...
        return self.messages.MySqlConnectionProfile(host=args.host, port=args.port, username=args.username, password=args.password, ssl=ssl_config, cloudSqlId=args.GetValue(self._InstanceArgName()))
    ...
    """
        if version == 'MYSQL_8_0_36':
            raise calliope_exceptions.BadArgumentException('database-version', 'The requested connection profile contains unsupported database version.')
    ...
    def _GetAuthorizedNetworks(self, networks):
        ...
        return [acl_entry(value=network) for network in networks]
    ...
    """
        ...
        user_labels_value = labels_util.ParseCreateArgs(args, cp_type.UserLabelsValue, 'user_labels')
        database_flags = labels_util.ParseCreateArgs(args, cp_type.DatabaseFlagsValue, 'database_flags')
        cloud_sql_settings = self.messages.CloudSqlSettings(databaseVersion=self._GetDatabaseVersion(cp_type, args.database_version), userLabels=user_labels_value, tier=args.tier, storageAutoResizeLimit=args.storage_auto_resize_limit, activationPolicy=self._GetActivationPolicy(cp_type, args.activation_policy), ipConfig=self._GetIpConfig(args), autoStorageIncrease=args.auto_storage_increase, databaseFlags=database_flags, dataDiskType=self._GetDataDiskType(cp_type, args.data_disk_type), dataDiskSizeGb=args.data_disk_size, zone=args.zone, rootPassword=args.root_password, sourceId=source_id)
        if self._api_version == 'v1':
            cloud_sql_settings.availabilityType = self._GetAvailabilityType(cp_type, args.availability_type)
        ...
        if self._release_track == base.ReleaseTrack.GA and args.CONCEPTS.cmek_key.Parse() is not None:
            cloud_sql_settings.cmekKeyName = args.CONCEPTS.cmek_key.Parse().RelativeName()
    ...
    def _GetAlloyDBDatabaseVersion(self, args):
        if args.IsKnownAndSpecified('database_version'):
            return self.messages.AlloyDbSettings.DatabaseVersionValueValuesEnum.lookup_by_name(args.database_version)
        else:
            return None
    ...
    def _GetExistingConnectionProfile(self, name):
        get_req = self.messages.DatamigrationProjectsLocationsConnectionProfilesGetRequest(name=name)
    ...
    """
        ...
        connection_profile = self._GetConnectionProfile(cp_type, args, connection_profile_id)
        ...
        create_req = create_req_type(connectionProfile=connection_profile, connectionProfileId=connection_profile_id, parent=parent_ref, requestId=request_id)
    ...
    """
        ...
        updated_cp, update_fields = self._GetUpdatedConnectionProfile(current_cp, args)
        request_id = api_util.GenerateRequestId()
        update_req_type = self.messages.DatamigrationProjectsLocationsConnectionProfilesPatchRequest
        update_req = update_req_type(connectionProfile=updated_cp, name=updated_cp.name, updateMask=','.join(update_fields), requestId=request_id)
    ...
    """
        location_ref = self.resource_parser.Create('datamigration.projects.locations', projectsId=project_id, locationsId=args.region if args.IsKnownAndSpecified('region') else '-')
        list_req_type = self.messages.DatamigrationProjectsLocationsConnectionProfilesListRequest
        list_req = list_req_type(parent=location_ref.RelativeName(), filter=args.filter, orderBy=','.join(args.sort_by) if args.sort_by else None)
        return list_pager.YieldFromList(service=self.client.projects_locations_connectionProfiles, request=list_req, limit=args.limit, batch_size=args.page_size, field='connectionProfiles', batch_size_attribute='pageSize')
    ...
    """
        uri = self.resource_parser.ParseRelativeName(name, collection='datamigration.projects.locations.connectionProfiles')
        return uri.SelfLink()