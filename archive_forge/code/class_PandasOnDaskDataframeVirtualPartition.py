...
class PandasOnDaskDataframeVirtualPartition(PandasDataframeAxisPartition):
    ...
    def deploy_splitting_func(cls, axis, func, f_args, f_kwargs, num_splits, *partitions, extract_metadata=False):
        return DaskWrapper.deploy(func=_deploy_dask_func, f_args=(PandasDataframeAxisPartition.deploy_splitting_func, axis, func, f_args, f_kwargs, num_splits, *partitions), f_kwargs={'extract_metadata': extract_metadata}, num_returns=num_splits * (1 + cls._PARTITIONS_METADATA_LEN) if extract_metadata else num_splits, pure=False)

    @classmethod
    def deploy_axis_func(cls, axis, func, f_args, f_kwargs, num_splits, maintain_partitioning, *partitions, min_block_size, lengths=None, manual_partition=False):
        """
    ...
    def deploy_func_between_two_axis_partitions(cls, axis, func, f_args, f_kwargs, num_splits, len_of_left, other_shape, *partitions, min_block_size):
        ...
        *partitions : iterable
        ...
        list
            A list of distributed.Future.
        """
        return DaskWrapper.deploy(func=_deploy_dask_func, f_args=(PandasDataframeAxisPartition.deploy_func_between_two_axis_partitions, axis, func, f_args, f_kwargs, num_splits, len_of_left, other_shape, *partitions), f_kwargs={'min_block_size': min_block_size}, num_returns=num_splits * (1 + cls._PARTITIONS_METADATA_LEN), pure=False)
