...
class NetAppOntapQosPolicyGroup:
    ...
    def __init__(self):
        ...
        if self.use_rest and self.parameters['state'] == 'present':
            if not self.rest_api.meets_rest_minimum_version(self.use_rest, 9, 8) and self.na_helper.safe_get(self.parameters, ['fixed_qos_options', 'min_throughput_mbps']):
            ...
            if not self.rest_api.meets_rest_minimum_version(self.use_rest, 9, 10, 1) and any((self.na_helper.safe_get(self.parameters, ['adaptive_qos_options', option]) for option in ontap_9_10_adaptive_options)):
    ...
    def get_policy_group(self, policy_group_name=None):
        ...
        try:
            ...
            self.module.fail_json(msg='Error fetching qos policy group %s: %s' % (self.parameters['name'], to_native(error)), exception=traceback.format_exc())
    ...
    def create_policy_group(self):
        ...
        try:
            ...
            self.module.fail_json(msg='Error creating qos policy group %s: %s' % (self.parameters['name'], to_native(error)), exception=traceback.format_exc())
    ...
    def delete_policy_group(self, policy_group=None):
        ...
        try:
            ...
            self.module.fail_json(msg='Error deleting qos policy group %s: %s' % (policy_group, to_native(error)), exception=traceback.format_exc())

    def delete_policy_group_rest(self):
        api = 'storage/qos/policies'
        dummy, error = rest_generic.delete_async(self.rest_api, api, self.uuid)
        if error:
            self.module.fail_json(msg='Error deleting qos policy group %s: %s' % (self.parameters['name'], error))
    ...
    def modify_policy_group(self, modify):
        ...
        try:
            ...
            self.module.fail_json(msg='Error modifying qos policy group %s: %s' % (self.parameters['name'], to_native(error)), exception=traceback.format_exc())
    ...
    def rename_policy_group(self):
        ...
        try:
            ...
            self.module.fail_json(msg='Error renaming qos policy group %s: %s' % (self.parameters['from_name'], to_native(error)), exception=traceback.format_exc())
    ...
    def modify_helper(self, modify):
        ...
        """
        if any((attribute in modify for attribute in ['max_throughput', 'min_throughput', 'fixed_qos_options', 'adaptive_qos_options'])):
    ...
    def apply(self):
        ...
        if cd_action == 'create' and self.parameters.get('from_name'):
            ...
            if rename:
                ...
                cd_action = None
            if rename is None:
                self.module.fail_json(msg='Error renaming qos policy group: cannot find %s' % self.parameters['from_name'])
        ...
        if 'is_shared' in modify or self.na_helper.safe_get(modify, ['fixed_qos_options', 'capacity_shared']) is not None:
            self.module.fail_json(msg="Error cannot modify '%s' attribute." % ('is_shared' if 'is_shared' in modify else 'fixed_qos_options.capacity_shared'))
