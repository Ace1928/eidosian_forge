...
class SyncAsyncLauncher:
    ...
    async def async_sync_wrapper(func):
        ...
        validate = validate._validate_func_signature(func, args, kwargs) # Validate the function signature
        start_time = alogging.start_time() # Get the start time
        end_time = alogging.end_time() # Get the end time
        log_performance = alogging.log_performance(func, start_time, end_time) # Log the performance of the function
        result = wrapper_logic.wrapper_logic(func, True, args, kwargs) # Execute the function
        ...
        async def async_wrapper(*args, **kwargs) -> Any:
            ...
            try:
                ...
                alogging.error(f"Exception in async call to {function_name}: {exception}")
                raise
            finally:
                end_time
        ...
        def sync_wrapper(*args, **kwargs) -> Any:
            ...
            asyncio.run_coroutine_threadsafe(validate._validate_func_signature(func, *args, **kwargs))
            try:
                ...
                end_time = asyncio.run_coroutine_threadsafe(alogging.end_time(), asyncio.get_event_loop()).result()
