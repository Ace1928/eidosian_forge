...
class Geod(_Geod):
    ...
    def __init__(self, initstring: Optional[str]=None, **kwargs) -> None:
        ...
        sphere = False
        if 'ellps' in kwargs:
            semi_major_axis, semi_minor_axis, flattening, eccentricity_squared, sphere = _params_from_ellps_map(kwargs['ellps'])
        else:
            semi_major_axis, semi_minor_axis, flattening, eccentricity_squared = _params_from_kwargs(kwargs)
        if math.fabs(flattening) < 1e-08:
            sphere = True
        super().__init__(semi_major_axis, flattening, sphere, semi_minor_axis, eccentricity_squared)

    def fwd(self, lons: Any, lats: Any, az: Any, dist: Any, radians: bool=False, inplace: bool=False, return_back_azimuth: bool=True) -> tuple[Any, Any, Any]:
        """
        ...
        """
        try:
            return self._fwd_point(lons, lats, az, dist, radians=radians, return_back_azimuth=return_back_azimuth)
        except TypeError:
            pass
        ...
        self._fwd(inx, iny, inz, ind, radians=radians, return_back_azimuth=return_back_azimuth)
        outx = _convertback(x_data_type, inx)
        outy = _convertback(y_data_type, iny)
        outz = _convertback(z_data_type, inz)
        return (outx, outy, outz)

    def inv(self, lons1: Any, lats1: Any, lons2: Any, lats2: Any, radians: bool=False, inplace: bool=False, return_back_azimuth: bool=True) -> tuple[Any, Any, Any]:
        """
        ...
        """
        try:
            return self._inv_point(lons1, lats1, lons2, lats2, radians=radians, return_back_azimuth=return_back_azimuth)
        except TypeError:
            pass
        ...
        self._inv(inx, iny, inz, ind, radians=radians, return_back_azimuth=return_back_azimuth)
        outx = _convertback(x_data_type, inx)
        outy = _convertback(y_data_type, iny)
        outz = _convertback(z_data_type, inz)
        return (outx, outy, outz)

    def npts(self, lon1: float, lat1: float, lon2: float, lat2: float, npts: int, radians: bool=False, initial_idx: int=1, terminus_idx: int=1) -> list:
        """
        ...
        """
        res = self._inv_or_fwd_intermediate(lon1=lon1, lat1=lat1, lon2_or_azi1=lon2, lat2=lat2, npts=npts, del_s=0, radians=radians, initial_idx=initial_idx, terminus_idx=terminus_idx, flags=GeodIntermediateFlag.AZIS_DISCARD, out_lons=None, out_lats=None, out_azis=None, return_back_azimuth=False, is_fwd=False)
    ...
    def inv_intermediate(self, lon1: float, lat1: float, lon2: float, lat2: float, npts: int=0, del_s: float=0, initial_idx: int=1, terminus_idx: int=1, radians: bool=False, flags: GeodIntermediateFlag=GeodIntermediateFlag.DEFAULT, out_lons: Optional[Any]=None, out_lats: Optional[Any]=None, out_azis: Optional[Any]=None, return_back_azimuth: Optional[bool]=None) -> GeodIntermediateReturn:
        ...
        if return_back_azimuth is None:
            ...
            warnings.warn('Back azimuth is being returned by default to be compatible with fwd()This is a breaking change for pyproj 3.5+.To avoid this warning, set return_back_azimuth=True.Otherwise, to restore old behaviour, set return_back_azimuth=False.This warning will be removed in future version.')
        return super()._inv_or_fwd_intermediate(lon1=lon1, lat1=lat1, lon2_or_azi1=lon2, lat2=lat2, npts=npts, del_s=del_s, radians=radians, initial_idx=initial_idx, terminus_idx=terminus_idx, flags=int(flags), out_lons=out_lons, out_lats=out_lats, out_azis=out_azis, return_back_azimuth=return_back_azimuth, is_fwd=False)
    ...
    def __repr__(self) -> str:
        for ellps, vals in pj_ellps.items():
            if self.a == vals['a']:
                if self.b == vals.get('b') or (not self.sphere and 1.0 / self.f == vals.get('rf')):
