...
def serialize_in_chunks(g: Graph, max_triples: int=10000, max_file_size_kb: Optional[int]=None, file_name_stem: str='chunk', output_dir: Optional[Path]=None, write_prefixes: bool=False) -> None:
    """
    ...
    if not output_dir.is_dir():
        raise ValueError('If you specify an output_dir, it must actually be a directory!')
    ...
    def _serialize_prefixes(g: Graph) -> str:
        ...
        return '\n'.join(sorted(pres)) + '\n'
    if write_prefixes:
        with open(Path(output_dir) / f'{file_name_stem}_000000.ttl', 'w', encoding='utf-8') as fh:
        ...
        if max_file_size_kb is not None:
            ...
            for i, t in enumerate(g.triples((None, None, None))):
                ...
                if len(row_bytes) > max_file_size:
                    raise ValueError(f"cannot write triple {t!r} as it's serialized size of {row_bytes / 1000} exceeds max_file_size_kb = {max_file_size_kb}")
                if i == 0:
            ...
            if graph_length <= max_triples:
                g.serialize(destination=Path(output_dir) / f'{file_name_stem}_all.nt', format='nt')
            else:
                file_no = 1 if write_prefixes else 0
                for i, t in enumerate(g.triples((None, None, None))):
                    if i % max_triples == 0:
