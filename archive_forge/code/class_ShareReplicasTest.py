...
class ShareReplicasTest(utils.TestCase):
    ...
    def setUp(self):
        ...
        self.manager = share_replicas.ShareReplicaManager(fakes.FakeClient(api_version=microversion))

    @ddt.data('2.11', constants.REPLICA_PRE_GRADUATION_VERSION, constants.REPLICA_GRADUATION_VERSION)
    def test_create(self, microversion):
        api_version = api_versions.APIVersion(microversion)
        values = {'availability_zone': 'az1', 'share': 's1'}
        manager = share_replicas.ShareReplicaManager(fakes.FakeClient(api_version=api_version))
    ...
    def test_create_with_share_network(self, microversion):
        api_version = api_versions.APIVersion(microversion)
        values = {'availability_zone': 'az1', 'share': 's1', 'share_network': 'sn1'}
        manager = share_replicas.ShareReplicaManager(fakes.FakeClient(api_version=api_version))
        with mock.patch.object(manager, '_create', fakes.fake_create):
            result = manager.create(**values)
            values['share_id'] = values.pop('share')
    ...
    def test_delete_str(self):
        with mock.patch.object(self.manager, '_delete', mock.Mock()):
            ...
            self.manager._delete.assert_called_once_with(share_replicas.RESOURCE_PATH % FAKE_REPLICA)

    def test_delete_obj(self):
        replica = self._FakeShareReplica
        with mock.patch.object(self.manager, '_delete', mock.Mock()):
            self.manager.delete(replica)
            self.manager._delete.assert_called_once_with(share_replicas.RESOURCE_PATH % replica.id)

    def test_delete_with_force(self):
        with mock.patch.object(self.manager, '_action', mock.Mock()):
            self.manager.delete(FAKE_REPLICA, force=True)
            self.manager._action.assert_called_once_with('force_delete', FAKE_REPLICA)

    def test_get(self):
        with mock.patch.object(self.manager, '_get', mock.Mock()):
            self.manager.get(FAKE_REPLICA)
            self.manager._get.assert_called_once_with(share_replicas.RESOURCE_PATH % FAKE_REPLICA, share_replicas.RESOURCE_NAME)

    def test_promote(self):
        with mock.patch.object(self.manager, '_action', mock.Mock()):
            self.manager.promote(FAKE_REPLICA)
            self.manager._action.assert_called_once_with('promote', FAKE_REPLICA)

    def test_list(self):
        with mock.patch.object(self.manager, '_list', mock.Mock()):
            self.manager.list(search_opts=None)
            self.manager._list.assert_called_once_with(share_replicas.RESOURCES_PATH + '/detail', share_replicas.RESOURCES_NAME)

    def test_list_with_share(self):
        with mock.patch.object(self.manager, '_list', mock.Mock()):
            self.manager.list('share_id')
            share_uri = '?share_id=share_id'
            self.manager._list.assert_called_once_with(share_replicas.RESOURCES_PATH + '/detail' + share_uri, share_replicas.RESOURCES_NAME)
