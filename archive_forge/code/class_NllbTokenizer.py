...
class NllbTokenizer(PreTrainedTokenizer):
    ...
    def __init__(self, vocab_file, bos_token='<s>', eos_token='</s>', sep_token='</s>', cls_token='<s>', unk_token='<unk>', pad_token='<pad>', mask_token='<mask>', tokenizer_file=None, src_lang=None, tgt_lang=None, sp_model_kwargs: Optional[Dict[str, Any]]=None, additional_special_tokens=None, legacy_behaviour=False, **kwargs):
        ...
        self._lang_code_to_id = {code: self.sp_model_size + i + self.fairseq_offset for i, code in enumerate(language_codes)}
    ...
    def fairseq_ids_to_tokens(self):
        logger.warning_once('the `_fairseq_ids_to_tokens` attribute is deprecated. The logic is natively handled in the `tokenizer.adder_tokens_decoder` this attribute will be removed in `transformers` v4.38')
