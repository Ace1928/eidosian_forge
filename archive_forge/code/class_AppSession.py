...
class AppSession:
    ...
    def __init__(self, script_data: ScriptData, uploaded_file_manager: UploadedFileManager, script_cache: ScriptCache, message_enqueued_callback: Callable[[], None] | None, local_sources_watcher: LocalSourcesWatcher, user_info: dict[str, str | None], session_id_override: str | None=None) -> None:
    ...
    def request_rerun(self, client_state: ClientState | None) -> None:
        ...
        if client_state:
            ...
            rerun_data = RerunData(client_state.query_string, client_state.widget_states, client_state.page_script_hash, client_state.page_name, fragment_id_queue=[fragment_id] if fragment_id else [])
        else:
            rerun_data = RerunData()
        if self._scriptrunner is not None:
            if bool(config.get_option('runner.fastReruns')) and (not rerun_data.fragment_id_queue):
    ...
    def _create_scriptrunner(self, initial_rerun_data: RerunData) -> None:
        ...
        self._scriptrunner = ScriptRunner(session_id=self.id, main_script_path=self._script_data.main_script_path, session_state=self._session_state, uploaded_file_mgr=self._uploaded_file_mgr, script_cache=self._script_cache, initial_rerun_data=initial_rerun_data, user_info=self._user_info, fragment_storage=self._fragment_storage)
    ...
    def _handle_rerun_script_request(self, client_state: ClientState | None=None) -> None:
