...
class LiveMigrationUtilsTestCase(test_base.OsWinBaseTestCase):
    ...
    _autospec_classes = [vmutils.VMUtils, jobutils.JobUtils]
    ...
    def test_get_conn_v2(self):
        self.liveutils._get_wmi_obj.side_effect = exceptions.x_wmi(com_error=mock.Mock())
        self.assertRaises(exceptions.HyperVException, self.liveutils._get_conn_v2, '.')
        self.liveutils._get_wmi_obj.assert_called_once_with(self.liveutils._wmi_namespace % '.', compatibility_mode=True)
    ...
    def test_get_vm_not_found(self):
        ...
        self.assertRaises(exceptions.HyperVVMNotFoundException, self.liveutils._get_vm, mock_conn_v2, self._FAKE_VM_NAME)
