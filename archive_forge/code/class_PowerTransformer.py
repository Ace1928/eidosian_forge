    ...
    * `Box-Cox <https://en.wikipedia.org/wiki/Power_transform#Box%E2%80%93Cox_transformation>`_
    ...
    def __init__(self, columns: List[str], power: float, method: str='yeo-johnson'):
        ...
        if method not in self._valid_methods:
            raise ValueError(f'Method {method} is not supported.Supported values are: {self._valid_methods}')
    ...
    def _transform_pandas(self, df: pd.DataFrame):
        ...
        def column_power_transformer(s: pd.Series):
            if self.method == 'yeo-johnson':
                ...
                return np.log(s)
        df.loc[:, self.columns] = df.loc[:, self.columns].transform(column_power_transformer)
        return df

    def __repr__(self):
        return f'{self.__class__.__name__}(columns={self.columns!r}, power={self.power!r}, method={self.method!r})'