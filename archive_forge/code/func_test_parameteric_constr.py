def test_parameteric_constr():
    ...
    from ase.constraints import dict2constraint, FixScaledParametricRelations, FixCartesianParametricRelations
    from ase.calculators.emt import EMT
    a = bulk('Ni', cubic=True)
    a.calc = EMT()
    cell = a.cell + 0.01
    param_lat = ['a']
    expr_lat = ['a', '0', '0', '0', 'a', '0', '0', '0', 'a']
    constr_lat = FixCartesianParametricRelations.from_expressions(indices=[0, 1, 2], params=param_lat, expressions=expr_lat, use_cell=True)
    ...
    constr_atom = FixScaledParametricRelations([0, 1, 2, 3], np.ndarray((12, 0)), a.get_scaled_positions().flatten())
    ...
    param_atom = ['dis']
    expr_atom = ['dis', 'dis', 'dis', 'dis', '-0.5', '0.5', '0.5', 'dis', '0.5', '0.5', '0.5', 'dis']
    constr_atom = FixScaledParametricRelations.from_expressions(indices=[0, 1, 2, 3], params=param_atom, expressions=expr_atom)
    pos += 0.01 * a.cell[0, 0]
    constr_atom.adjust_positions(a, pos)
    scaled_pos = a.cell.scaled_positions(pos)
    pos_diff = (scaled_pos - a.get_scaled_positions()).flatten()
    expected_pos_diff = np.array([0.01, 0.01, 0.01, 0.01, 0.0, 0.0, 0.0, 0.01, 0.0, 0.0, 0.0, 0.01])
    ...
    for const_expr, passed_expr in zip(current_expression, expr_atom):
        assert const_expr == passed_expr
    expr_atom = ['dis', 'dis', 'dis', 'dis', '1.76', '1.76', '1.76', 'dis', '1.76', '1.76', '1.76', 'dis']
    constr_atom = FixCartesianParametricRelations.from_expressions(indices=[0, 1, 2, 3], params=param_atom, expressions=expr_atom)
    a.set_positions(pos)
    pos += 0.01
    constr_atom.adjust_positions(a, pos)
