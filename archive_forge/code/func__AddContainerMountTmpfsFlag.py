...
def _AddContainerMountTmpfsFlag(parser):
    ...
    parser.add_argument('--container-mount-tmpfs', metavar='mount-path=MOUNTPATH', type=arg_parsers.ArgDict(spec={'mount-path': str}), action='append', help='      Mounts empty tmpfs into container at MOUNTPATH.\n\n      *mount-path*::: Path on container to mount to. Mount paths with spaces\n      and commas (and other special characters) are not supported by this\n      command.\n      ')