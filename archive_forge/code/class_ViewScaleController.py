...
class ViewScaleController:
    ...
    def __init__(self, uniform_dict, scale, label0, label1, update_function):
    ...
    def scale_command(self, value):
        ...
        if not parameter_interval:
            return
        parameter = _linear_remap(value, (self.scale.left_end, self.scale.right_end), parameter_interval)
        self.uniform_dict['viewScale'][1] = float(_scale_function[perspective_type](parameter))
        self.update_label()
    ...
    def update_scale(self):
        ...
        self.scale.set_value(_linear_remap(parameter, parameter_interval, (self.scale.left_end, self.scale.right_end)))
