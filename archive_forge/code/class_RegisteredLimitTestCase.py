...
class RegisteredLimitTestCase(common.IdentityTests):
    ...
    def test_registered_limit_create_with_options(self):
        ...
        params = {'service_name': service_name, 'resource_name': 'cores', 'default_limit': 10, 'description': 'default limit for cores', 'region_id': region_id}
        raw_output = self.openstack("registered limit create --description '%(description)s' --region %(region_id)s --service %(service_name)s --default-limit %(default_limit)s %(resource_name)s" % params, cloud=SYSTEM_CLOUD)
        items = self.parse_show(raw_output)
        registered_limit_id = self._extract_value_from_items('id', items)
        self.addCleanup(self.openstack, 'registered limit delete %(registered_limit_id)s' % {'registered_limit_id': registered_limit_id}, cloud=SYSTEM_CLOUD)
        self.assert_show_fields(items, self.REGISTERED_LIMIT_FIELDS)
    ...
    def test_registered_limit_show(self):
        ...
        raw_output = self.openstack('registered limit show %(registered_limit_id)s' % {'registered_limit_id': registered_limit_id})
        items = self.parse_show(raw_output)
        self.assert_show_fields(items, self.REGISTERED_LIMIT_FIELDS)
    ...
    def test_registered_limit_set_description(self):
        registered_limit_id = self._create_dummy_registered_limit()
        params = {'registered_limit_id': registered_limit_id, 'description': 'updated description'}
        raw_output = self.openstack("registered limit set %(registered_limit_id)s --description '%(description)s'" % params, cloud=SYSTEM_CLOUD)
        items = self.parse_show(raw_output)
        self.assert_show_fields(items, self.REGISTERED_LIMIT_FIELDS)
    ...
    def test_registered_limit_set_service(self):
        registered_limit_id = self._create_dummy_registered_limit()
        service_name = self._create_dummy_service()
        params = {'registered_limit_id': registered_limit_id, 'service': service_name}
        raw_output = self.openstack('registered limit set %(registered_limit_id)s --service %(service)s' % params, cloud=SYSTEM_CLOUD)
        items = self.parse_show(raw_output)
        self.assert_show_fields(items, self.REGISTERED_LIMIT_FIELDS)

    def test_registered_limit_set_default_limit(self):
        registered_limit_id = self._create_dummy_registered_limit()
        params = {'registered_limit_id': registered_limit_id, 'default_limit': 20}
        raw_output = self.openstack('registered limit set %(registered_limit_id)s --default-limit %(default_limit)s' % params, cloud=SYSTEM_CLOUD)
        items = self.parse_show(raw_output)
        self.assert_show_fields(items, self.REGISTERED_LIMIT_FIELDS)
    ...
    def test_registered_limit_set_resource_name(self):
        ...
        params = {'registered_limit_id': registered_limit_id, 'resource_name': resource_name}
        raw_output = self.openstack('registered limit set %(registered_limit_id)s --resource-name %(resource_name)s' % params, cloud=SYSTEM_CLOUD)
        items = self.parse_show(raw_output)
        self.assert_show_fields(items, self.REGISTERED_LIMIT_FIELDS)
    ...
    def test_registered_limit_delete(self):
        registered_limit_id = self._create_dummy_registered_limit(add_clean_up=False)
        raw_output = self.openstack('registered limit delete %(registered_limit_id)s' % {'registered_limit_id': registered_limit_id}, cloud=SYSTEM_CLOUD)
