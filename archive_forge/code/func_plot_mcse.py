...
def plot_mcse(ax, plotters, length_plotters, rows, cols, figsize, errorbar, rug, data, probs, kwargs, extra_methods, mean_mcse, sd_mcse, textsize, labeller, text_kwargs, rug_kwargs, extra_kwargs, idata, rug_kind, backend_kwargs, show):
    """Bokeh mcse plot."""
    ...
    for (var_name, selection, isel, x), ax_ in zip(plotters, (item for item in ax.flatten() if item is not None)):
        ...
        if errorbar:
            ...
            ax_.multi_line(list(zip(probs, probs)), [(quant - err, quant + err) for quant, err in zip(quantile_values, x)])
        else:
            ax_.circle(probs, x)
            if extra_methods:
                mean_mcse_i = mean_mcse[var_name].sel(**selection).values.item()
                sd_mcse_i = sd_mcse[var_name].sel(**selection).values.item()
                hline_mean = Span(location=mean_mcse_i, dimension='width', line_color=extra_kwargs['color'], line_width=extra_kwargs['linewidth'] * 2, line_alpha=extra_kwargs['alpha'])
                ax_.renderers.append(hline_mean)
                hline_sd = Span(location=sd_mcse_i, dimension='width', line_color='black', line_width=extra_kwargs['linewidth'], line_alpha=extra_kwargs['alpha'])
        ...
        if rug:
            ...
            if errorbar:
                rug_x, rug_y = (values / (len(mask) - 1), np.full_like(values, min(0, min(quantile_values) - (max(quantile_values) - min(quantile_values)) * 0.05)))
                hline = Span(location=min(0, min(quantile_values) - (max(quantile_values) - min(quantile_values)) * 0.05), dimension='width', line_color='black', line_width=_linewidth, line_alpha=0.7)
            else:
                rug_x, rug_y = (values / (len(mask) - 1), np.full_like(values, 0))
                hline = Span(location=0, dimension='width', line_color='black', line_width=_linewidth, line_alpha=0.7)
        ...
        ax_.yaxis.axis_label = 'Value $\\pm$ MCSE for quantiles' if errorbar else 'MCSE for quantiles'
        if not errorbar:
            ax_.y_range._property_values['start'] = -0.05
            ax_.y_range._property_values['end'] = 1
    show_layout(ax, show)
    return ax