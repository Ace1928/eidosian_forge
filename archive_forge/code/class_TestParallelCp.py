...
class TestParallelCp(testcase.GsUtilIntegrationTestCase):
    ...
    def testCopyingAbsolutePathDirToBucket(self):
        ...
        self.assertEqual(suri(dst_bucket_uri, src_tmpdir, 'dir0', 'dir1', 'nested'), lines[0])
    ...
    """
        ...
        self.RunGsUtil(['cp', '-R', os.path.join(src_dir, 'dir0', 'dir1'), suri(dst_bucket_uri)])
        lines = self.AssertNObjectsInBucket(dst_bucket_uri, 1)
    ...
    def testCopyingObjsAndFilesToBucket(self):
        ...
        dst_bucket_uri = self.CreateBucket()
        self.RunGsUtil(['cp', '-R', suri(src_bucket_uri, '**'), '%s%s**' % (src_dir, os.sep), suri(dst_bucket_uri)])
        lines = self.AssertNObjectsInBucket(dst_bucket_uri, 2)
    ...
    def testCopyingWildcardedFilesToBucketSubDir(self):
        ...
        for final_dst_char in ('', '/'):
            dst_bucket_uri = self.CreateBucket()
            self.CreateObject(dst_bucket_uri, object_name='subdir0/existing', contents=b'foo')
            self.CreateObject(dst_bucket_uri, object_name='subdir1/existing', contents=b'foo')
            src_dir = self.CreateTempDir(test_files=['f0', 'f1', 'f2'])
            for i in range(2):
                self.RunGsUtil(['cp', os.path.join(src_dir, 'f?'), suri(dst_bucket_uri, 'subdir%d' % i) + final_dst_char])

                @Retry(AssertionError, tries=3, timeout_secs=1)
                def _Check1():
                    """Validate files were copied to the correct destinations."""
                    stdout = self.RunGsUtil(['ls', suri(dst_bucket_uri, 'subdir%d' % i, '**')], return_stdout=True)
                    lines = stdout.split('\n')
                    self.assertEqual(5, len(lines))
                    self.assertEqual(suri(dst_bucket_uri, 'subdir%d' % i, 'existing'), lines[0])
    ...
    def testCopyingOneNestedFileToBucketSubDir(self):
        ...
        for final_dst_char in ('', '/'):
            dst_bucket_uri = self.CreateBucket()
            self.CreateObject(dst_bucket_uri, object_name='d0/placeholder', contents=b'foo')
            self.CreateObject(dst_bucket_uri, object_name='d1/placeholder', contents=b'foo')
            for i in range(2):
                src_dir = self.CreateTempDir(test_files=[('d3', 'd4', 'nested', 'f1')])
                self.RunGsUtil(['cp', '-r', suri(src_dir, 'd3'), suri(dst_bucket_uri, 'd%d' % i) + final_dst_char])
            lines = self.AssertNObjectsInBucket(dst_bucket_uri, 4)
            self.assertEqual(suri(dst_bucket_uri, 'd0', 'd3', 'd4', 'nested', 'f1'), lines[0])
            self.assertEqual(suri(dst_bucket_uri, 'd0', 'placeholder'), lines[1])
            self.assertEqual(suri(dst_bucket_uri, 'd1', 'd3', 'd4', 'nested', 'f1'), lines[2])
