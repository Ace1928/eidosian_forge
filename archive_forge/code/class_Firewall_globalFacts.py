...
class Firewall_globalFacts(object):
    ...
    def populate_facts(self, connection, ansible_facts, data=None):
        ...
        objs = {}
        firewalls = findall('^set firewall .*$', data, M)
        if firewalls:
    ...
    def render_config(self, conf):
        ...
        f_sub = {'ping': self.parse_ping(conf), 'group': self.parse_group(conf), 'route_redirects': self.route_redirects(conf), 'state_policy': self.parse_state_policy(conf)}
    ...
    def parse_group(self, conf):
        ...
        cfg_dict['address_group'] = self.parse_group_lst(conf, 'address-group') + self.parse_group_lst(conf, 'ipv6-address-group')
        cfg_dict['network_group'] = self.parse_group_lst(conf, 'network-group') + self.parse_group_lst(conf, 'ipv6-network-group')
        return cfg_dict
    ...
    def is_bool(self, attrib):
        ...
        :return: True/False
        """
        bool_set = ('all', 'log', 'send', 'receive', 'broadcast', 'config_trap', 'log_martians', 'syn_cookies', 'ip_src_route', 'twa_hazards_protection')
