...
class MockTurkManager:
    ...
    def ready_to_accept_workers(self, timeout_seconds=None):
        ...
        """
        self.id_to_agent = {agent_id: MockTurkAgent(self.opt, self, 'hit_id_{}'.format(agent_id), 'assignment_id_{}'.format(agent_id), agent_id) for agent_id in self.mturk_agent_ids}
    ...
    def worker_alive(self, worker_id, hit_id, assign_id):
        ...
        elif agent.status in [AssignState.STATUS_ONBOARDING, AssignState.STATUS_IN_TASK]:
            pass
        elif agent.status == AssignState.STATUS_DISCONNECT or agent.status == AssignState.STATUS_DONE or agent.status == AssignState.STATUS_EXPIRED or (agent.status == AssignState.STATUS_RETURNED) or (agent.status == AssignState.STATUS_PARTNER_DISCONNECT):
    ...
    def send_message(self, receiver_id, assignment_id, data, blocking=True, ack_func=None):
        ...
        packet = Packet(event_id, Packet.TYPE_MESSAGE, 'world', receiver_id, assignment_id, data, conversation_id=conversation_id, blocking=blocking, ack_func=ack_func)
