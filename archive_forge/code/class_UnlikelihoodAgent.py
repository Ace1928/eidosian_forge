...
class UnlikelihoodAgent(ParlaiGenerationAgent, Seq2SeqLM):
    ...
    def __init__(self, model, device, maxlen=-1):
        ...
        option, model_class = self.set_options(name=f'zoo:dialogue_unlikelihood/{name}/model', path='projects.dialogue_unlikelihood.agents', class_name='RepetitionUnlikelihoodAgent', device=device)
        super().__init__(device=device, name=model, maxlen=maxlen, suffix='\n', model=create_agent_from_opt_file_and_model_class(opt=option, model_class=model_class))
