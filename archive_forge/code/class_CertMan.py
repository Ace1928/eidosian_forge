...
class CertMan(object):
    ...
    def __init__(self):
        self._pubkeys = {'WhatsAppLongTerm1': bytearray([20, 35, 117, 87, 77, 10, 88, 113, 102, 170, 231, 30, 190, 81, 100, 55, 196, 162, 139, 115, 227, 105, 92, 108, 225, 247, 249, 84, 93, 168, 238, 107])}
    ...
    def is_valid(self, rs, certificate_data):
        ...
        logger.debug("NoiseCertificate(signature=[%d bytes], serial=%d, issuer='%s', expires=%d, subject='%s', key=[%d bytes])" % (len(cert.signature), cert_details.serial, cert_details.issuer, cert_details.expires, cert_details.subject, len(cert_details.key)))
