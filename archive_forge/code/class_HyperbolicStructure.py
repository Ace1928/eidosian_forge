...
class HyperbolicStructure:
    ...
    def __init__(self, mcomplex, edge_lengths, exact_edges=None, var_edges=None):
        """
        ...
        self.vertex_gram_matrices = [_compute_vertex_gram_matrix(tet, edge_lengths) for tet in mcomplex.Tetrahedra]
        self.vertex_gram_adjoints = [m.adjugate() for m in self.vertex_gram_matrices]
        self.dihedral_angles = [[[_dihedral_angle(vertex_gram_adjoint, i, j) for i in range(4)] for j in range(4)] for vertex_gram_adjoint in self.vertex_gram_adjoints]
        self.angle_sums = [self._angle_sum(edge) for edge in mcomplex.Edges]
    ...
    def jacobian(self):
        ...
        for tet in self.mcomplex.Tetrahedra:
            cofactor_matrices = _cofactor_matrices_for_submatrices(self.vertex_gram_matrices[tet.Index])
            ...
            for angle_edge, (i, j) in _OneSubsimplicesWithVertexIndices:
                ...
                for length_edge, (m, n) in _OneSubsimplicesWithVertexIndices:
                    ...
                    result[a, l] += dcij * _cofactor_derivative(cofactor_matrices, i, j, m, n) + dcii * _cofactor_derivative(cofactor_matrices, i, i, m, n) + dcjj * _cofactor_derivative(cofactor_matrices, j, j, m, n)
        return result
