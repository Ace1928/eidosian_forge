...
class AIOKafkaAdminClient:
    ...
    def __init__(self, *, loop=None, bootstrap_servers: str='localhost', client_id: str='aiokafka-' + __version__, request_timeout_ms: int=40000, connections_max_idle_ms: int=540000, retry_backoff_ms: int=100, metadata_max_age_ms: int=300000, security_protocol: str='PLAINTEXT', ssl_context: Optional[SSLContext]=None, api_version: str='auto', sasl_mechanism: str='PLAIN', sasl_plain_username: Optional[str]=None, sasl_plain_password: Optional[str]=None, sasl_kerberos_service_name: str='kafka', sasl_kerberos_domain_name: Optional[str]=None, sasl_oauth_token_provider: Optional[str]=None):
        self._closed = False
        self._started = False
        self._version_info = {}
        self._request_timeout_ms = request_timeout_ms
        self._client = AIOKafkaClient(loop=loop, bootstrap_servers=bootstrap_servers, client_id=client_id, metadata_max_age_ms=metadata_max_age_ms, request_timeout_ms=request_timeout_ms, retry_backoff_ms=retry_backoff_ms, api_version=api_version, ssl_context=ssl_context, security_protocol=security_protocol, connections_max_idle_ms=connections_max_idle_ms, sasl_mechanism=sasl_mechanism, sasl_plain_username=sasl_plain_username, sasl_plain_password=sasl_plain_password, sasl_kerberos_service_name=sasl_kerberos_service_name, sasl_kerberos_domain_name=sasl_kerberos_domain_name, sasl_oauth_token_provider=sasl_oauth_token_provider)
    ...
    async def _send_request(self, request: Request, node_id: Optional[int]=None) -> Response:
        if node_id is None:
    ...
    async def create_topics(self, new_topics: List[NewTopic], timeout_ms: Optional[int]=None, validate_only: bool=False) -> Response:
        ...
        if version == 0:
            if validate_only:
                raise IncompatibleBrokerVersion('validate_only requires CreateTopicsRequest >= v1, which is not supported by Kafka {}.'.format(self._client.api_version))
            request = CreateTopicsRequest[version](create_topic_requests=topics, timeout=timeout_ms)
        elif version <= 3:
            request = CreateTopicsRequest[version](create_topic_requests=topics, timeout=timeout_ms, validate_only=validate_only)
        else:
            raise NotImplementedError('Support for CreateTopics v{} has not yet been added to AIOKafkaAdminClient.'.format(version))
        response = await self._client.send(self._client.get_random_node(), request)
        return response

    async def delete_topics(self, topics: List[str], timeout_ms: Optional[int]=None) -> Response:
    ...
    async def _get_cluster_metadata(self, topics: Optional[List[str]]=None) -> Response:
        """
    ...
    async def describe_topics(self, topics: Optional[List[str]]=None) -> List[Any]:
    ...
    async def describe_configs(self, config_resources: List[ConfigResource], include_synonyms: bool=False) -> List[Response]:
        ...
        if version == 0 and include_synonyms:
            raise IncompatibleBrokerVersion('include_synonyms requires DescribeConfigsRequest >= v1, which is not supported by Kafka {}.'.format(self._client.api_version))
        broker_res, topic_res = self._convert_config_resources(config_resources, 'describe')
        req_cls = DescribeConfigsRequest[version]
        for broker_id in broker_res:
            if version == 0:
                req = req_cls(resources=broker_res[broker_id])
            else:
                req = req_cls(resources=broker_res[broker_id], include_synonyms=include_synonyms)
    ...
    async def alter_configs(self, config_resources: List[ConfigResource]) -> Response:
        ...
        broker_resources, topic_resources = self._convert_config_resources(config_resources, 'alter')
    ...
    def _convert_alter_config_resource_request(config_resource):
        return (config_resource.resource_type, config_resource.name, list(config_resource.configs.items()))
    ...
    async def create_partitions(self, topic_partitions: Dict[str, TopicPartition], timeout_ms: Optional[int]=None, validate_only: bool=False) -> Response:
        ...
        req = req_class(topic_partitions=converted_partitions, timeout=timeout_ms or self._request_timeout_ms, validate_only=validate_only)
    ...
    async def describe_consumer_groups(self, group_ids: List[str], group_coordinator_id: Optional[int]=None, include_authorized_operations: bool=False) -> List[Response]:
        ...
        if version < 3 and include_authorized_operations:
            raise IncompatibleBrokerVersion('include_authorized_operations requests DescribeGroupsRequest >= v3, which is not supported by Kafka {}'.format(version))
        ...
        for node_id, groups in node_to_groups.items():
            if include_authorized_operations:
                req = req_class(groups=list(groups), include_authorized_operations=include_authorized_operations)
            else:
    ...
    async def list_consumer_groups(self, broker_ids: Optional[List[int]]=None) -> List[Tuple[Any, ...]]:
        ...
        for broker_id in broker_ids:
            response = await self._send_request(ListGroupsRequest[self._matching_api_version(ListGroupsRequest)](), broker_id)
    ...
    async def find_coordinator(self, group_id: str, coordinator_type: int=0) -> int:
        ...
        if version == 0 and coordinator_type:
            raise IncompatibleBrokerVersion('Cannot query for transaction id on current broker version')
    ...
    async def list_consumer_group_offsets(self, group_id: str, group_coordinator_id: Optional[int]=None, partitions: Optional[List[TopicPartition]]=None) -> Dict[TopicPartition, OffsetAndMetadata]:
