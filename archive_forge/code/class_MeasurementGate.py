...
class MeasurementGate(raw_types.Gate):
    ...
    def __init__(self, num_qubits: Optional[int]=None, key: Union[str, 'cirq.MeasurementKey']='', invert_mask: Tuple[bool, ...]=(), qid_shape: Optional[Tuple[int, ...]]=None, confusion_map: Optional[Dict[Tuple[int, ...], np.ndarray]]=None) -> None:
        ...
        self._mkey = key if isinstance(key, value.MeasurementKey) else value.MeasurementKey(name=key)
    ...
    def with_key(self, key: Union[str, 'cirq.MeasurementKey']) -> 'MeasurementGate':
        ...
        if key == self.key:
            return self
        return MeasurementGate(self.num_qubits(), key=key, invert_mask=self.invert_mask, qid_shape=self._qid_shape, confusion_map=self.confusion_map)
    ...
    def _op_repr_(self, qubits: Sequence['cirq.Qid']) -> str:
        ...
        if self.confusion_map:
            proper_map_str = ', '.join((f'{k!r}: {_compat.proper_repr(v)}' for k, v in self.confusion_map.items()))
    ...
    def _value_equality_values_(self) -> Any:
        hashable_cmap = frozenset(((idxs, tuple((v for _, v in np.ndenumerate(cmap)))) for idxs, cmap in self._confusion_map.items()))
    ...
    def _json_dict_(self) -> Dict[str, Any]:
        ...
        return {'num_qubits': len(self._qid_shape), 'key': self.key, 'invert_mask': self.invert_mask, **other}

    @classmethod
    def _from_json_dict_(cls, num_qubits, key, invert_mask, qid_shape=None, confusion_map=None, **kwargs):
        return cls(num_qubits=num_qubits, key=value.MeasurementKey.parse_serialized(key), invert_mask=tuple(invert_mask), qid_shape=None if qid_shape is None else tuple(qid_shape), confusion_map={tuple(k): np.array(v) for k, v in confusion_map or []})
