...
class TimeoutStopper(Stopper):
    ...
    def __init__(self, timeout: Union[int, float, datetime.timedelta]):
        ...
        if isinstance(timeout, timedelta):
            ...
            raise ValueError('`timeout` parameter has to be either a number or a `datetime.timedelta` object. Found: {}'.format(type(timeout)))
    ...
    def stop_all(self):
        ...
        if self._budget <= 0:
            logger.info(f'Reached timeout of {self._timeout_seconds} seconds. Stopping all trials.')
            return True
        return False
