...
class NetAppONTAPMetroClusterDRGroup(object):
    ...
    def __init__(self):
        ...
        self.argument_spec.update(dict(state=dict(choices=['present', 'absent'], default='present'), dr_pairs=dict(required=True, type='list', elements='dict', options=dict(node_name=dict(required=True, type='str'), partner_node_name=dict(required=True, type='str'))), partner_cluster_name=dict(required=True, type='str')))
        ...
        if not self.use_rest:
            self.module.fail_json(msg=self.rest_api.requires_ontap_version('na_ontap_metrocluster_dr_group', version='9.8'))
