...
def sparse_reduce_sum_sparse_eager_fallback(input_indices: _atypes.TensorFuzzingAnnotation[_atypes.Int64], input_values: _atypes.TensorFuzzingAnnotation[TV_SparseReduceSumSparse_T], input_shape: _atypes.TensorFuzzingAnnotation[_atypes.Int64], reduction_axes: _atypes.TensorFuzzingAnnotation[_atypes.Int32], keep_dims: bool, name, ctx):
    ...
    _result = _execute.execute(b'SparseReduceSumSparse', 3, inputs=_inputs_flat, attrs=_attrs, ctx=ctx, name=name)
    if _execute.must_record_gradient():
        _execute.record_gradient('SparseReduceSumSparse', _inputs_flat, _attrs, _result)
