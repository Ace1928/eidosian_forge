...
class Dnacprovision(DnacBase):
    ...
    def validate_input(self):
        ...
        if not self.config:
            ...
            self.status = 'success'
            return self
        provision_spec = {'management_ip_address': {'type': 'str', 'required': True}, 'site_name': {'type': 'str', 'required': True}, 'managed_ap_locations': {'type': 'list', 'required': False, 'elements': 'str'}, 'dynamic_interfaces': {'type': 'list', 'required': False, 'elements': 'dict'}}
        valid_provision, invalid_params = validate_list_of_dicts(self.config, provision_spec)
        if invalid_params:
    ...
    def get_dev_type(self):
        ...
        Example:
          ...
          type of the device.
        """
        dev_response = self.dnac_apply['exec'](family='devices', function='get_network_device_by_ip', params={'ip_address': self.validated_config[0]['management_ip_address']})
    ...
    def get_wired_params(self):
        ...
        """
        wired_params = {'deviceManagementIpAddress': self.validated_config[0]['management_ip_address'], 'siteNameHierarchy': self.validated_config[0].get('site_name')}
        return wired_params
    ...
    def get_wireless_params(self):
        ...
        Example:
          ...
          parameters in other APIs.
        """
        wireless_params = [{'site': self.validated_config[0].get('site_name'), 'managedAPLocations': self.validated_config[0].get('managed_ap_locations')}]
    ...
    def get_diff_merged(self):
        ...
        if device_type == 'wired':
            try:
                status_response = self.dnac_apply['exec'](family='sda', function='get_provisioned_wired_device', op_modifies=True, params={'device_management_                        ip_address': self.validated_config[0]['management_ip_address']})
            except Exception:
                status_response = {}
            status = status_response.get('status')
            if status == 'success':
                response = self.dnac_apply['exec'](family='sda', function='re_provision_wired_device', op_modifies=True, params=self.want['prov_params'])
            else:
                response = self.dnac_apply['exec'](family='sda', function='provision_wired_device', op_modifies=True, params=self.want['prov_params'])
        elif device_type == 'wireless':
            response = self.dnac_apply['exec'](family='wireless', function='provision', op_modifies=True, params=self.want['prov_params'])
        else:
    ...
    def get_diff_deleted(self):
        ...
        if status != 'success':
            ...
            self.result['response'] = self.want['prov_params']
            return self
        response = self.dnac_apply['exec'](family='sda', function='delete_provisioned_wired_device', op_modifies=True, params={'device_management_                ip_address': self.validated_config[0]['management_ip_address']})
        task_id = response.get('taskId')
