...
mpl.use("TkAgg")  # Use the 'TkAgg' backend for plotting
...
from async_cache import async_cache  # Async cache decorator
...
def fetch_stock_data(
    ...
    if source == "yahoo":
        ...
        url: str = (
            f'https://finnhub.io/api/v1/stock/candle?symbol={ticker}&resolution=D&from={int(dt.datetime.strptime(start_date, "%Y-%m-%d").timestamp())}&to={int(dt.datetime.strptime(end_date, "%Y-%m-%d").timestamp())}&token={api_key}'
        )
        r: requests.Response = requests.get(url)
        if r.status_code == 200:
            json_data = r.json()
...
def plot_donchian_channel(data: pd.DataFrame, window: int = 300) -> None:
    ...
    """
    # Check if 'ticker' column exists and get a sample ticker value
    ...
    plt.savefig(
        f"{sample_ticker}_donchian_channel_{time.time()}.png"
    )  # Save the plot as an image
    try:
        plt.close()  # Close the plot window
    except Exception as e:
        # If an error occurs, print the error message and continue gracefully
            ...
            if mode.lower() == "auto":
                ...
                while True:
                    try:
                        ...
                        print(
                            f"Data updated. Waiting for {sleep_interval} before the next update..."
                        )
...
if __name__ == "__main__":
    # Setting up logging to capture errors with timestamps
    logging.basicConfig(
    ...
    # Redirecting stdout to both the console and a file with a custom header
    sys.stdout = DualOutput("output_log.txt")
    print(
        "\n\n"
        + "-" * 20
        + f" Execution started at {dt.now().strftime('%Y-%m-%d %H:%M:%S')} "
        + "-" * 20
        + "\n\n"
    )
    ...
    print("\n\n" + "-" * 20 + " Program completed successfully " + "-" * 20 + "\n\n")
