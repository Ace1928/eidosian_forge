...
class _DomainAndProjectUserProjectEndpointTests(object):
    ...
    def test_user_cannot_list_projects_for_endpoint(self):
        ...
        with self.test_client() as c:
            c.get('/v3/OS-EP-FILTER/endpoints/%s/projects' % endpoint['id'], headers=self.headers, expected_status_code=http.client.FORBIDDEN)
    ...
    def test_user_cannot_check_endpoint_in_project(self):
        ...
        PROVIDERS.catalog_api.add_endpoint_to_project(endpoint['id'], project['id'])
        with self.test_client() as c:
            c.get('/v3/OS-EP-FILTER/projects/%s/endpoints/%s' % (project['id'], endpoint['id']), headers=self.headers, expected_status_code=http.client.FORBIDDEN)
    ...
    def test_user_cannot_list_endpoints_for_project(self):
        ...
        PROVIDERS.catalog_api.add_endpoint_to_project(endpoint['id'], project['id'])
        with self.test_client() as c:
            c.get('/v3/OS-EP-FILTER/projects/%s/endpoints' % project['id'], headers=self.headers, expected_status_code=http.client.FORBIDDEN)