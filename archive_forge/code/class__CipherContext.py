...
class _CipherContext:
    ...
    def __init__(self, backend: Backend, cipher, mode, operation: int) -> None:
        ...
        if res == 0 and (not lib.CRYPTOGRAPHY_IS_LIBRESSL and errors[0]._lib_reason_match(lib.ERR_LIB_EVP, lib.EVP_R_XTS_DUPLICATED_KEYS) or (lib.Cryptography_HAS_PROVIDERS and errors[0]._lib_reason_match(lib.ERR_LIB_PROV, lib.PROV_R_XTS_DUPLICATED_KEYS))):
    ...
    def finalize(self) -> bytes:
        if self._operation == self._DECRYPT and isinstance(self._mode, modes.ModeWithAuthenticationTag) and (self.tag is None):
        ...
        if res == 0:
            ...
            if not errors and isinstance(self._mode, modes.GCM):
                raise InvalidTag
            lib = self._backend._lib
            self._backend.openssl_assert(errors[0]._lib_reason_match(lib.ERR_LIB_EVP, lib.EVP_R_DATA_NOT_MULTIPLE_OF_BLOCK_LENGTH) or (lib.Cryptography_HAS_PROVIDERS and errors[0]._lib_reason_match(lib.ERR_LIB_PROV, lib.PROV_R_WRONG_FINAL_BLOCK_LENGTH)) or (lib.CRYPTOGRAPHY_IS_BORINGSSL and errors[0].reason == lib.CIPHER_R_DATA_NOT_MULTIPLE_OF_BLOCK_LENGTH), errors=errors)
