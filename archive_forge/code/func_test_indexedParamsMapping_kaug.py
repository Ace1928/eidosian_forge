...
def test_indexedParamsMapping_kaug(self):
    ...
    m_kaug = sensitivity_calculation('k_aug', m, [m.eps, m.qq, m.aa], [m.epsDelta, m.qqDelta, m.aaDelta])
    ...
    self.assertEqual(m_kaug._SENSITIVITY_TOOLBOX_DATA.paramConst[1].body.to_string(), '_SENSITIVITY_TOOLBOX_DATA.eps - eps')
    ...
    self.assertEqual(m_kaug._SENSITIVITY_TOOLBOX_DATA.paramConst[6].body.to_string(), '_SENSITIVITY_TOOLBOX_DATA.qq[2,0] - qq[2,0]')
    ...
    self.assertEqual(m_kaug._SENSITIVITY_TOOLBOX_DATA.paramConst[10].body.to_string(), '_SENSITIVITY_TOOLBOX_DATA.aa - aa')