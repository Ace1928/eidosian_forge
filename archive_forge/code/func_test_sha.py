...
def test_sha(tmpdir):
    ...
    obj = F()
    res = optimize_by_sha(obj, dataset, plan=[[1.0, 3], [1.0, 2], [1.0, 1], [1.0, 1]], checkpoint_path=str(tmpdir))
    res.result().output(assert_metric, dict(metric=4.0, ct=1))
    res = optimize_by_sha(obj, dataset, plan=[[2.0, 2], [1.0, 1], [1.0, 1]], checkpoint_path=str(tmpdir))
