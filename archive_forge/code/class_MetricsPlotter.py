...
class MetricsPlotter:
    ...
    def __init__(self, train_metrics: List[Union[str, tp.Dict[str, str]]], test_metrics: Optional[List[Union[str, tp.Dict[str, str]]]]=None, total_iterations: Optional[int]=None) -> None:
        """
        ...
        self._values = {'iterations': [], 'meta': {'launch_mode': 'Train', 'parameters': '', 'name': 'experiment', 'iteration_count': None, 'learn_sets': ['learn'], 'learn_metrics': None, 'test_sets': ['test'], 'test_metrics': None}}
        self._content = {'passed_iterations': 0, 'total_iterations': None, 'data': self._values}
        self._data = {'test_path': {'path': 'test_path', 'name': 'experiment', 'content': self._content}}
        ...
        self._train_metrics_positions = {meta['name']: pos for pos, meta in enumerate(train_metrics_meta)}
        self._test_metrics_positions = {meta['name']: pos for pos, meta in enumerate(test_metrics_meta)}
        self._values['meta'].update({'learn_metrics': train_metrics_meta, 'test_metrics': test_metrics_meta})
    ...
    def __exit__(self, exc_type, exc_value, traceback) -> Any:
        if exc_type == KeyboardInterrupt:
            logger.info(f'Learning was stopped manually after {self.passed_iterations} epochs')
            return True
    ...
    def log(self, epoch: int, train: bool, metrics: tp.Dict[str, float]) -> None:
        ...
        self._values['meta']['iteration_count'] = total_iterations
        assert len(self._values['iterations']) in (epoch, epoch + 1), 'Data for epochs should be passed successively (wrong epoch number)'
        ...
        if len(self._values['iterations']) == epoch:
            self._values['iterations'].append({'learn': [], 'test': [], 'iteration': epoch, 'passed_time': time_from_start})
        ...
        value: List[float] = self.construct_metrics_array(self._train_metrics_positions if train else self._test_metrics_positions, metrics)
