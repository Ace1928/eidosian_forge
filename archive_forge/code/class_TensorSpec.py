  ...
  """
    ...
    def is_subtype_of(self, other):
        if not isinstance(other, TensorSpec):
            return False
        return (not self.name or self.name == other.name) and self.shape.is_subtype_of(other.shape) and self.dtype.is_subtype_of(other.dtype)
    ...
    def _to_tensors(self, value):
        ...
        if not value.shape.is_subtype_of(self.shape):
            raise TypeError(f'Received tensor of shape {value.shape} instead of {self.shape}')
        return [value]
    ...
    def _batch(self, batch_size):
        return TensorSpec(tensor_shape.TensorShape([batch_size]).concatenate(self._shape), self._dtype)
