...
class TestRebuildServer(base.TestCase):
    ...
    def test_rebuild_server_with_admin_pass_no_wait(self):
        ...
        self.register_uris([self.get_nova_discovery_mock_dict(), dict(method='POST', uri=self.get_mock_url('compute', 'public', append=['servers', self.server_id, 'action']), json={'server': rebuild_server}, validate=dict(json={'rebuild': {'imageRef': 'a', 'adminPass': password}})), dict(method='GET', uri=self.get_mock_url('network', 'public', append=['v2.0', 'networks']), json={'networks': []})])
        self.assertEqual(password, self.cloud.rebuild_server(self.fake_server['id'], 'a', admin_pass=password)['adminPass'])
        self.assert_calls()
