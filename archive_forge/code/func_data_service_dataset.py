  ...
  """
    ...
    if tld.is_eager:
        try:
            ...
            pass
        try:
            return data_service_dataset_eager_fallback(dataset_id, processing_mode, address, protocol, job_name, max_outstanding_requests, iteration_counter, task_refresh_interval_hint_ms=task_refresh_interval_hint_ms, output_types=output_types, output_shapes=output_shapes, data_transfer_protocol=data_transfer_protocol, target_workers=target_workers, cross_trainer_cache_options=cross_trainer_cache_options, name=name, ctx=_ctx)
        except _core._SymbolicException:
            pass
    if not isinstance(output_types, (list, tuple)):
        raise TypeError("Expected list for 'output_types' argument to 'data_service_dataset' Op, not %r." % output_types)
    output_types = [_execute.make_type(_t, 'output_types') for _t in output_types]
    if not isinstance(output_shapes, (list, tuple)):
        raise TypeError("Expected list for 'output_shapes' argument to 'data_service_dataset' Op, not %r." % output_shapes)
    ...
    if _execute.must_record_gradient():
        ...
        _execute.record_gradient('DataServiceDataset', _inputs_flat, _attrs, _result)
    _result, = _result
    return _result