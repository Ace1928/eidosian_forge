...
class MonascaNotificationTest(common.HeatTestCase):
    ...
    def test_validate_no_scheme_address_for_get_attr(self):
        ...
        get_att = cfn_funcs.GetAtt(self.stack, 'Fn::GetAtt', ['ResourceA', 'abc'])
    ...
    def test_resource_handle_create(self):
        ...
        self.assertEqual('test-notification', self.test_resource.properties.get(notification.MonascaNotification.NAME))
        self.assertEqual('webhook', self.test_resource.properties.get(notification.MonascaNotification.TYPE))
        self.assertEqual('http://localhost:80/', self.test_resource.properties.get(notification.MonascaNotification.ADDRESS))
        self.assertEqual(60, self.test_resource.properties.get(notification.MonascaNotification.PERIOD))
    ...
    def test_resource_handle_create_no_period(self):
        ...
        mock_notification_create = self.test_client.notifications.create
        self.test_resource.handle_create()
        args = dict(name='test-notification', type='email', address='abc@def.com')
        mock_notification_create.assert_called_once_with(**args)

    def test_resource_handle_update(self):
        mock_notification_update = self.test_client.notifications.update
        self.test_resource.resource_id = '477e8273-60a7-4c41-b683-fdb0bc7cd151'
        prop_diff = {notification.MonascaNotification.ADDRESS: 'http://localhost:1234/', notification.MonascaNotification.NAME: 'name-updated', notification.MonascaNotification.TYPE: 'webhook', notification.MonascaNotification.PERIOD: 0}
        self.test_resource.handle_update(json_snippet=None, tmpl_diff=None, prop_diff=prop_diff)
        args = dict(notification_id=self.test_resource.resource_id, name='name-updated', type='webhook', address='http://localhost:1234/', period=0)
    ...
    def test_resource_handle_delete(self):
        ...
        mock_notification_delete.assert_called_once_with(notification_id=self.test_resource.resource_id)
