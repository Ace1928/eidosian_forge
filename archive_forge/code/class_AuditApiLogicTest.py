...
class AuditApiLogicTest(base.BaseAuditMiddlewareTest):
    ...
    def get_payload(self, method, url, audit_map=None, body=None, environ=None):
        ...
        req = webob.Request.blank(url, body=body, method=method, environ=environ, remote_addr='192.168.0.1')
    ...
    def test_custom_action(self):
        url = 'http://admin_host:8774/v2/%s/os-hosts/%s/reboot' % (uuid.uuid4().hex, uuid.uuid4().hex)
        payload = self.get_payload('GET', url)
        self.assertEqual(payload['target']['typeURI'], 'service/compute/os-hosts/host/reboot')
    ...
    def test_no_auth_token(self):
        ...
        self.assertEqual(payload['initiator']['credential']['identity_status'], 'Invalid')
        self.assertNotIn('reason', payload)
        self.assertNotIn('reporterchain', payload)
        self.assertEqual(payload['observer']['id'], 'target')
        self.assertEqual(path, payload['requestPath'])
