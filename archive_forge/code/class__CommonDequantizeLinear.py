...
class _CommonDequantizeLinear(OpRun):
    ...
    def _run(self, x: np.ndarray, x_scale: np.ndarray, x_zero_point: Optional[np.ndarray]=None, axis: Optional[int]=None, block_size: Optional[int]=None):
        x_type = self.get_x_type(x)
        fp8_type = x_type in {TensorProto.FLOAT8E4M3FN, TensorProto.FLOAT8E4M3FNUZ, TensorProto.FLOAT8E5M2, TensorProto.FLOAT8E5M2FNUZ}
        if x_zero_point is not None and (not fp8_type):
            zero_type = self.get_x_type(x_zero_point)
            if x_type != zero_type:
                raise ValueError(f'Type mismatch {x_type} != {zero_type} in DequantizeLinear.')
            dx = x.astype(np.float32) - reshape_input(x_zero_point, x.shape, axis, block_size)
        else:
            if fp8_type and x_zero_point is not None:
                ...
                if umi != uma or umi != np.uint8(0):
                    raise ValueError('x_zero_point is not null but should be zero for float8 types.')
