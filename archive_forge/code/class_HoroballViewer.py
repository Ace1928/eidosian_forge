...
class HoroballViewer(ttk.Frame):
    ...
    def __init__(self, parent, nbhd=None, which_cusp=0, cutoff=None, title='Horoball Viewer', settings={'cusp_horoballs': True, 'cusp_triangulation': True, 'cusp_ford_domain': True, 'cusp_labels': True, 'cusp_parallelogram': True, 'cusp_cutoff': '0.1000'}, bgcolor=None, main_window=None):
        ...
        self.pgram_var = pgram_var = Tk_.IntVar(self, value=settings['cusp_parallelogram'])
        self.Ford_var = Ford_var = Tk_.IntVar(self, value=settings['cusp_ford_domain'])
        self.tri_var = tri_var = Tk_.IntVar(self, value=settings['cusp_triangulation'])
        self.horo_var = horo_var = Tk_.IntVar(self, value=settings['cusp_horoballs'])
        self.label_var = label_var = Tk_.IntVar(self, value=settings['cusp_labels'])
        ...
        self.cutoff_var = cutoff_var = Tk_.StringVar(self, value='%.4f' % self.cutoff)
        ...
        self.build_menus()
        self.scene = HoroballScene(nbhd, pgram_var, Ford_var, tri_var, horo_var, label_var, flipped=self.flip_var.get(), cutoff=self.cutoff, which_cusp=self.which_cusp, togl_widget=self.widget)
