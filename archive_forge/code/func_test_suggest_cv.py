...
def test_suggest_cv(tmpdir):
    ...
    result = suggest_sk_models_by_cv(space1 + space3, train, 'neg_mean_absolute_error', top_n=0, distributed=False, local_optimizer=HyperoptLocalOptimizer(max_iter=10, seed=0))
    assert 4 == len(result)
    assert 50 > result[0].sort_metric
    result = suggest_sk_models_by_cv(space1 + space2, train, 'neg_mean_absolute_error', top_n=0, partition_keys=['sex'], temp_path=str(tmpdir), save_model=True)
    assert 16 == len(result)
    assert 50 > result[0].sort_metric
    result = suggest_sk_models_by_cv(space1 + space2, train, 'neg_mean_absolute_error', top_n=1, partition_keys=['sex'], execution_engine=DaskExecutionEngine)
    assert 2 == len(result)
    assert 50 > result[0].sort_metric