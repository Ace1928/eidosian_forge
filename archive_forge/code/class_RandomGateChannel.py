...
class RandomGateChannel(raw_types.Gate):
    ...
    def __init__(self, *, sub_gate: 'cirq.Gate', probability: 'cirq.TParamVal'):
        if isinstance(probability, numbers.Number) and (not 0 <= float(cast(SupportsFloat, probability)) <= 1):
    ...
    def _is_parameterized_(self) -> bool:
        return protocols.is_parameterized(self.probability) or protocols.is_parameterized(self.sub_gate)

    def _parameter_names_(self) -> AbstractSet[str]:
        return protocols.parameter_names(self.probability) | protocols.parameter_names(self.sub_gate)

    def _resolve_parameters_(self, resolver: 'cirq.ParamResolver', recursive: bool) -> 'RandomGateChannel':
        return RandomGateChannel(sub_gate=protocols.resolve_parameters(self.sub_gate, resolver, recursive), probability=protocols.resolve_parameters(self.probability, resolver, recursive))
    ...
    def _mixture_(self):
        ...
        if mixture is None:
            return None
        do_nothing = np.eye(np.prod(protocols.qid_shape(self.sub_gate), dtype=np.int64), dtype=np.float64)
    ...
    def _circuit_diagram_info_(self, args: 'cirq.CircuitDiagramInfoArgs') -> Optional['cirq.CircuitDiagramInfo']:
