...
class NetAppONTAPDebug(object):
    ...
    def __init__(self):
        ...
        self.argument_spec.update(dict(vserver=dict(required=False, type='str')))
    ...
    def import_lib(self):
        if not netapp_utils.has_netapp_lib():
            msgs = ['Error importing netapp-lib or a dependency: %s.' % str(netapp_utils.IMPORT_EXCEPTION), 'Install the python netapp-lib module or a missing dependency.', 'Additional diagnostic information:', 'Python Executable Path: %s.' % sys.executable, 'Python Version: %s.' % sys.version, 'System Path: %s.' % ','.join(sys.path)]
    ...
    def check_connection(self, connection_type):
        ...
        if error_string is not None:
            summary_msg = None
            error_patterns = ['Connection timed out', 'Resource temporarily unavailable', 'ConnectTimeoutError', 'Network is unreachable']
    ...
    def list_users(self, vserver_name=None, user_name=None):
        ...
        if error:
            ...
            msg = 'Error: no unlocked user for %s on vserver: %s' if vserver_name else 'Error: %s is not enabled for user %s'
            if not one_http:
