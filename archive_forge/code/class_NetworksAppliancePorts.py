...
class NetworksAppliancePorts(object):
    ...
    def __init__(self, params, meraki):
        self.meraki = meraki
        self.new_object = dict(enabled=params.get('enabled'), dropUntaggedTraffic=params.get('dropUntaggedTraffic'), type=params.get('type'), vlan=params.get('vlan'), allowedVlans=params.get('allowedVlans'), accessPolicy=params.get('accessPolicy'), network_id=params.get('networkId'), port_id=params.get('portId'))
    ...
    def get_object_by_name(self, name):
        result = None
        try:
            items = self.meraki.exec_meraki(family='appliance', function='getNetworkAppliancePorts', params=self.get_all_params(name=name))
    ...
    def get_object_by_id(self, id):
        result = None
        try:
            items = self.meraki.exec_meraki(family='appliance', function='getNetworkAppliancePort', params=self.get_params_by_id())
            if isinstance(items, dict):
                if 'response' in items:
                    items = items.get('response')
            result = items
    ...
    def requires_update(self, current_obj):
        ...
        obj_params = [('enabled', 'enabled'), ('dropUntaggedTraffic', 'dropUntaggedTraffic'), ('type', 'type'), ('vlan', 'vlan'), ('allowedVlans', 'allowedVlans'), ('accessPolicy', 'accessPolicy'), ('networkId', 'networkId'), ('portId', 'portId')]
    ...
    def update(self):
        ...
        result = self.meraki.exec_meraki(family='appliance', function='updateNetworkAppliancePort', params=self.update_by_id_params(), op_modifies=True)
        return result