...
def _disable_unless_serve_artifacts(func):
    ...
    def wrapper(*args, **kwargs):
        if not _is_serving_proxied_artifacts():
            return Response(f'Endpoint: {request.url_rule} disabled due to the mlflow server running with `--no-serve-artifacts`. To enable artifacts server functionality, run `mlflow server` with `--serve-artifacts`', 503)
