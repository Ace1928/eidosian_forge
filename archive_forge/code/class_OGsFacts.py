...
class OGsFacts(object):
    ...
    def populate_facts(self, connection, ansible_facts, data=None):
        ...
        current = rmmod.parse()
        ogs = []
        object_groups = {'icmp-type': 'icmp_type', 'network': 'network_object', 'protocol': 'protocol_object', 'security': 'security_group', 'service': 'service_object', 'user': 'user_object'}
        if current.get('ogs'):
            for k, v in iteritems(current.get('ogs')):
                ...
                for each in iteritems(v):
                    ...
                    obj_gp = {}
                config_dict['object_groups'] = sorted(config_dict['object_groups'], key=lambda k, sk='name': str(k[sk]))
                ogs.append(config_dict)
        ogs = sorted(ogs, key=lambda i: i['object_type'])
        facts = {}
        params = utils.remove_empties(utils.validate_config(self.argument_spec, {'config': ogs}))
