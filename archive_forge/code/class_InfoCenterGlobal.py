...
class InfoCenterGlobal(object):
    ...
    def config_merge_out_direct(self, out_direct, channel_id):
        ...
        if not self.is_exist_out_direct(out_direct, channel_id):
            ...
            self.updates_cmd.append('info-center %s channel %s' % (out_direct, channel_id))
            self.changed = True
    ...
    def config_merge_loghost(self):
        ...
        if self.is_default_vpn is True:
            is_default_vpn = 'true'
        if self.ip_type:
            conf_str = CE_NC_MERGE_SERVER_IP_INFO_HEADER % (self.ip_type, self.server_ip, self.vrf_name, is_default_vpn)
        elif self.server_domain:
            conf_str = CE_NC_MERGE_SERVER_DNS_INFO_HEADER % (self.server_domain, self.vrf_name, is_default_vpn)
        if self.level:
        ...
        if syslog_dnss:
            for syslog_dns in syslog_dnss:
                ...
                for ele in syslog_dns:
                    if ele.tag in ['serverDomain', 'vrfName', 'level', 'serverPort', 'facility', 'chnlId', 'chnlName', 'timestamp', 'transportMode', 'sslPolicyName', 'isDefaultVpn', 'sourceIP', 'isBriefFmt']:
    ...
    def check_params(self):
        ...
        if self.packet_priority:
            if not self.packet_priority.isdigit():
                self.module.fail_json(msg='Error: The parameter of packet priority is invalid.')
            if int(self.packet_priority) > 7 or int(self.packet_priority) < 0:
                self.module.fail_json(msg='Error: The packet priority must be an integer between 0 and 7.')
        if self.logfile_max_num:
            if not self.logfile_max_num.isdigit():
                self.module.fail_json(msg='Error: The parameter of logfile_max_num is invalid.')
            if int(self.logfile_max_num) > 500 or int(self.logfile_max_num) < 3:
                self.module.fail_json(msg='Error: The logfile_max_num must be an integer between 3 and 500.')
        if self.channel_id:
            if not self.channel_id.isdigit():
                self.module.fail_json(msg='Error: The parameter of channel_id is invalid.')
            if int(self.channel_id) > 9 or int(self.channel_id) < 0:
                self.module.fail_json(msg='Error: The channel_id must be an integer between 0 and 9.')
        if self.channel_cfg_name:
            if len(self.channel_cfg_name) > 30 or len(self.channel_cfg_name.replace(' ', '')) < 1:
                self.module.fail_json(msg='Error: channel_cfg_name is not in the range from 1 to 30.')
        if self.filter_feature_name:
            if len(self.filter_feature_name) > 31 or len(self.filter_feature_name.replace(' ', '')) < 1:
                self.module.fail_json(msg='Error: filter_feature_name is not in the range from 1 to 31.')
        if self.filter_log_name:
            if len(self.filter_log_name) > 63 or len(self.filter_log_name.replace(' ', '')) < 1:
                self.module.fail_json(msg='Error: filter_log_name is not in the range from 1 to 63.')
        if self.server_ip:
            if not check_ip_addr(self.server_ip):
                self.module.fail_json(msg='Error: The %s is not a valid ip address' % self.server_ip)
        if self.source_ip:
            if not check_ip_addr(self.source_ip):
                self.module.fail_json(msg='Error: The %s is not a valid ip address' % self.source_ip)
        if self.server_domain:
            if len(self.server_domain) > 255 or len(self.server_domain.replace(' ', '')) < 1:
                self.module.fail_json(msg='Error: server_domain is not in the range from 1 to 255.')
        if self.vrf_name:
            if len(self.vrf_name) > 31 or len(self.vrf_name.replace(' ', '')) < 1:
                self.module.fail_json(msg='Error: vrf_name is not in the range from 1 to 31.')
        if self.server_port:
            if not self.server_port.isdigit():
                self.module.fail_json(msg='Error: The parameter of server_port is invalid.')
            if int(self.server_port) > 65535 or int(self.server_port) < 1:
                self.module.fail_json(msg='Error: The server_port must be an integer between 1 and 65535.')
        if self.channel_name:
            if len(self.channel_name) > 31 or len(self.channel_name.replace(' ', '')) < 1:
                self.module.fail_json(msg='Error: channel_name is not in the range from 1 to 30.')
        if self.ssl_policy_name:
            if len(self.ssl_policy_name) > 23 or len(self.ssl_policy_name.replace(' ', '')) < 1:
                self.module.fail_json(msg='Error: ssl_policy_name is not in the range from 1 to 23.')
    ...
    def get_existing(self):
        """get existing info"""
        if self.info_center_enable:
            self.existing['info_center_enable'] = self.cur_global_info['icEnable']
        if self.packet_priority:
            self.existing['packet_priority'] = self.cur_global_info['packetPriority']
        if self.suppress_enable:
            self.existing['suppress_enable'] = self.cur_global_info['suppressEnable']
        if self.logfile_max_num:
            self.existing['logfile_max_num'] = self.cur_logfile_info['maxFileNum']
        if self.logfile_max_size:
            self.existing['logfile_max_size'] = self.cur_logfile_info['maxFileSize']
        if self.channel_id and self.channel_cfg_name:
            if self.channel_info:
                self.existing['channel_id_info'] = self.channel_info['channelInfos']
        if self.channel_out_direct and self.channel_id:
            if self.channel_direct_info:
                self.existing['channel_out_direct_info'] = self.channel_direct_info['channelDirectInfos']
        if self.filter_feature_name and self.filter_log_name:
            if self.filter_info:
                self.existing['filter_id_info'] = self.filter_info['filterInfos']
        if self.ip_type:
            if self.server_ip_info:
                self.existing['server_ip_info'] = self.server_ip_info['serverIpInfos']
        if self.server_domain:
            if self.server_domain_info:
                self.existing['server_domain_info'] = self.server_domain_info['serverAddressInfos']
    ...
    def work(self):
        ...
        if self.server_ip:
            if not self.ip_type:
                self.module.fail_json(msg='Error: ip_type and server_ip must be exist at the same time.')
        if self.ip_type:
        ...
        if self.ip_type or self.server_domain or self.channel_id or self.filter_feature_name:
            if self.ip_type and self.server_domain:
                self.module.fail_json(msg='Error: ip_type and server_domain can not be exist at the same time.')
            if self.channel_id and self.channel_name:
                self.module.fail_json(msg='Error: channel_id and channel_name can not be exist at the same time.')
            if self.ssl_policy_name:
                if self.transport_mode == 'udp':
                    self.module.fail_json(msg='Error: transport_mode: udp does not support ssl_policy.')
                if not self.transport_mode:
                    self.module.fail_json(msg='Error: transport_mode, ssl_policy_name must be exist at the same time.')
            if self.ip_type == 'ipv6':
                if self.vrf_name and self.vrf_name != '_public_':
                    self.module.fail_json(msg='Error: ipType:ipv6 only support default vpn:_public_.')
            if self.is_default_vpn is True:
                if self.vrf_name:
                    if self.vrf_name != '_public_':
                        self.module.fail_json(msg='Error: vrf_name should be _public_ when is_default_vpn is True.')
                else:
                    self.vrf_name = '_public_'
            elif self.vrf_name == '_public_':
                self.module.fail_json(msg='Error: The default vpn value is _public_, but is_default_vpn is False.')
            if self.state == 'present':
                if self.channel_id and self.channel_cfg_name:
                    self.config_merge_syslog_channel(self.channel_id, self.channel_cfg_name)
                if self.channel_out_direct and self.channel_id:
                    self.config_merge_out_direct(self.channel_out_direct, self.channel_id)
                if self.filter_feature_name and self.filter_log_name:
                    self.config_merge_filter(self.filter_feature_name, self.filter_log_name)
                if self.ip_type and self.server_ip:
                    if not self.vrf_name:
                        self.vrf_name = '_public_'
                ...
                if self.channel_id:
                    self.delete_merge_syslog_channel(self.channel_id, self.channel_cfg_name)
                if self.channel_out_direct:
                    self.delete_merge_out_direct(self.channel_out_direct, self.channel_id)
                if self.filter_feature_name and self.filter_log_name:
                    self.delete_merge_filter(self.filter_feature_name, self.filter_log_name)
                if self.ip_type and self.server_ip:
                    if not self.vrf_name:
                        self.vrf_name = '_public_'
                    if self.check_need_loghost_cfg():
