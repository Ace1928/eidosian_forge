...
class Pathfinder:
    def __init__(self, 
                 ...
                 space_around_goals: Dict[str, int] = {'primary': 1, 'secondary': 1, 'tertiary': 1, 'quaternary': 1},
    ...
    def neighbors(self, node: Tuple[int, int]) -> List[Tuple[int, int]]:
        ...
        for dx, dy in directions:
            ...
            if 0 <= nx <= self.bounds[2] and 0 <= ny <= self.bounds[3] and (nx, ny) not in self.body_occupied:
                result.append((nx, ny))
        return result
    ...
    def smooth_path(self, path: List[Tuple[int, int]]):
        ...
        for i in range(1, len(path) - 1):
            if not (path[i - 1][0] == path[i + 1][0] or path[i - 1][1] == path[i + 1][1]):
