...
def _add_implicit_dependencies(self, deps, ignore_errors=True):
    ...
    for res in self.resources.values():
        try:
            ...
            if not (ignore_errors or isinstance(exc, (ValueError, TypeError))):
                raise
            else:
                LOG.warning('Ignoring error adding implicit dependencies for %(res)s: %(err)s', {'res': str(res), 'err': str(exc)})