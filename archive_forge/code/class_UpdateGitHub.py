...
class UpdateGitHub(base.UpdateCommand):
    ...
    """
        ...
        v1_config.add_argument('--repo-owner', help='Owner of the GitHub Repository.')
        v1_config.add_argument('--repo-name', help='Name of the GitHub Repository.')
    ...
    """
        ...
        if done:
            return trigger
        gh = messages.GitHubEventsConfig(owner=args.repo_owner, name=args.repo_name, enterpriseConfigResourceName=args.enterprise_config)
        if args.pull_request_pattern:
            gh.pullRequest = messages.PullRequestFilter(branch=args.pull_request_pattern)
            if args.comment_control:
                gh.pullRequest.commentControl = messages.PullRequestFilter.CommentControlValueValuesEnum(args.comment_control)
        else:
            gh.push = messages.PushFilter(branch=args.branch_pattern, tag=args.tag_pattern)
        trigger.github = gh
