...
def test_get_policy_on_cluster_by_id(self):
    ...
    wait = wait_for_status(self.user_cloud.get_cluster_by_id, {'name_or_id': cluster['cluster']['id']}, 'policies', ['{policy}'.format(policy=policy['id'])])
    cluster_policy_get = self.user_cloud.get_policy_on_cluster(cluster['cluster']['id'], policy['id'])
    self.assertEqual(cluster_policy_get['cluster_id'], cluster['cluster']['id'])
    self.assertEqual(cluster_policy_get['cluster_name'], cluster['cluster']['name'])
