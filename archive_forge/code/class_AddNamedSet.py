  ...
  *{command}* adds an empty named set to a Compute Engine router.
  """
    ...
    def Args(cls, parser):
        ...
        parser.add_argument('--set-name', help='Name of the named set to add.', required=True)
        parser.add_argument('--set-type', type=arg_utils.ChoiceToEnumName, choices={'PREFIX': 'The Named Set is a Prefix Named Set.', 'COMMUNITY': 'The Named Set is a Community Named Set.'}, help="Type of the set's elements.", required=True)
    ...
    """
        ...
        router_ref = AddNamedSet.ROUTER_ARG.ResolveAsResource(args, holder.resources, scope_lister=compute_flags.GetDefaultScopeLister(client))
        named_set = client.messages.NamedSet(name=args.set_name, type=client.messages.NamedSet.TypeValueValuesEnum(self.ConvertSetType(args.set_type)))
