...
class ParlAIQuickAcute(object):
    ...
    def _get_task_conversion_config(self, config_id: str) -> Dict[str, Any]:
        ...
        config = CONFIG[config_id]
        config.update({'outfile': outfile, 'num_episodes': NUM_TASK_DATA_EPISODES, 'speaker_0_id': f'{config_id}_as_human', 'speaker_1_id': config_id})
        return config
    ...
    def _get_selfchat_log_path(self, config_id: str) -> str:
        ...
        return os.path.join(self_chat_dir, f'{config_id}.{self.task.replace(':', '_')}.jsonl')

    def _acutify_convo(self, conversation: Conversation, config_id: str) -> Dict[str, List]:
        """
        ...
        acute_conversation: Dict[str, List] = {'context': [], 'dialogue': [], 'speakers': []}
        for i, ex in enumerate(conversation):
            ...
            acute_conversation['dialogue'].append({'id': speaker_id, 'text': normalize_reply(ex['text'])})
    ...
    def _load_pairings_file(self):
        ...
        if not os.path.exists(pairings_filepath):
            ...
            self._print_progress(f'Pairings already exist {pairings_filepath}. Last modified {time.ctime(modify_time)}')
            answer = ''
    ...
    def analyze_results(self):
        ...
        opt.update({'run_id': self.run_id, 'outdir': self.results_path, 'pairings_filepath': self.pairings_filepath})
