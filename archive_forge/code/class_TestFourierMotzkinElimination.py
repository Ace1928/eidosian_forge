...
class TestFourierMotzkinElimination(unittest.TestCase):
    ...
    def test_no_vars_specified(self):
        m = self.makeModel()
        self.assertRaisesRegex(RuntimeError, 'The Fourier-Motzkin Elimination transformation requires the argument vars_to_eliminate, a list of Vars to be projected out of the model.', TransformationFactory('contrib.fourier_motzkin_elimination').apply_to, m)
    ...
    def test_transformed_constraints_indexed_var_arg(self):
        m = self.makeModel()
        TransformationFactory('contrib.fourier_motzkin_elimination').apply_to(m, vars_to_eliminate=m.lamb, constraint_filtering_callback=None)
    ...
    def test_nonlinear_error(self):
        ...
        self.assertRaisesRegex(RuntimeError, 'Variable x appears in a nonlinear constraint. The Fourier-Motzkin Elimination transformation can only be used to eliminate variables which only appear linearly.', TransformationFactory('contrib.fourier_motzkin_elimination').apply_to, m, vars_to_eliminate=m.x)
    ...
    def test_noninteger_coefficients_of_vars_not_being_projected_error(self):
        ...
        fme = TransformationFactory('contrib.fourier_motzkin_elimination')
        self.assertRaisesRegex(ValueError, 'The do_integer_arithmetic flag was set to True, but the coefficient of y is non-integer within the specified tolerance, with value 0.5. \nPlease set do_integer_arithmetic=False, increase integer_tolerance, or make your data integer.', fme.apply_to, m, vars_to_eliminate=m.x, do_integer_arithmetic=True)
