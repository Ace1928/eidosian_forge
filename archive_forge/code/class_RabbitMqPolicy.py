...
class RabbitMqPolicy(object):
    ...
    def _exec(self, args, force_exec_in_check_mode=False, split_lines=True, add_vhost=True):
    ...
    def has_modifications(self):
        if self._pattern is None or self._tags is None:
            self._module.fail_json(msg='pattern and tags are required for state=present')
        if self._version and self._version >= Version('3.7.0'):
            return not any((self._policy_check(policy, apply_to_fno=3, pattern_fno=2) for policy in self._list_policies()))
        else:
            return not any((self._policy_check(policy) for policy in self._list_policies()))
