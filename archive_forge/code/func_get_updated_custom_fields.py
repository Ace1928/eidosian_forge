...
import six
def get_updated_custom_fields(existing_custom_fields, request_config, attributes_resource=None, known_posix=None):
  ...
  """
    ...
    if not resource_args:
        return
    file_resource = attributes_resource if isinstance(attributes_resource, resource_reference.FileObjectResource) else None
    if existing_custom_fields and file_resource:
        raise errors.Error('Existing custom fields should not exist when updating custom fields using local source.')
    if not has_updated_custom_fields(resource_args, request_config.preserve_posix, request_config.preserve_symlinks, attributes_resource=attributes_resource, known_posix=known_posix):
        return
    ...
    if known_posix or should_parse_file_posix:
        if known_posix:
            ...
            posix_attributes = posix_util.get_posix_attributes_from_resource(file_resource, preserve_symlinks=request_config.preserve_symlinks)
        posix_util.update_custom_metadata_dict_with_posix_attributes(posix_metadata, posix_attributes)
    if should_parse_symlinks:
        symlink_util.update_custom_metadata_dict_with_symlink_attributes(posix_metadata, file_resource.is_symlink)
