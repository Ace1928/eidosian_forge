  ...
  """
    ...
    def Run(self, args):
        ...
        if args.docker_config_dir and (not args.use_docker_creds):
            raise util.Error('Cannot specify --docker-config-dir without --use-docker-creds.')
        if args.resource:
            resource_obj = parsing.LoadResourceFile(args.resource)
        else:
            resource_obj = util.GeneratePodSpecFromImages(args.image)
        response = platform_policy.Client('v1').Evaluate(policy_ref, resource_obj, True)
        ...
        if not args.no_upload:
            for attestation in response.attestations:
                ...
                sigstore_image.UploadAttestationToRegistry(image_url=image_url, attestation=sigstore_image.StandardOrUrlsafeBase64Decode(attestation), use_docker_creds=args.use_docker_creds, docker_config_dir=args.docker_config_dir)
        if args.output_file:
            modified_resource = util.AddInlineAttestationsToResource(resource_obj, response.attestations)
            if parsing.GetResourceFileType(args.resource) == parsing.ResourceFileType.YAML:
                modified_resource = yaml.dump(modified_resource)
            log.WriteToFileOrStdout(args.output_file, modified_resource, overwrite=True, binary=False, private=True)
        return response