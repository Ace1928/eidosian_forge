...
class CmdStanMCMC:
    ...
    def __init__(self, runset: RunSet) -> None:
        ...
        if not runset.method == Method.SAMPLE:
            raise ValueError('Wrong runset method, expecting sample runset, found method {}'.format(runset.method))
        self.runset = runset
        sampler_args = self.runset._args.method_args
        assert isinstance(sampler_args, SamplerArgs)
    ...
    def __repr__(self) -> str:
        repr = 'CmdStanMCMC: model={} chains={}{}'.format(self.runset.model, self.runset.chains, self.runset._args.method_args.compose(0, cmd=[]))
    ...
    def _validate_csv_files(self) -> Dict[str, Any]:
        ...
        for i in range(self.chains):
            if i == 0:
                dzero = check_sampler_csv(path=self.runset.csv_files[i], is_fixed_param=self._is_fixed_param, iter_sampling=self._iter_sampling, iter_warmup=self._iter_warmup, save_warmup=self._save_warmup, thin=self._thin)
                if not self._is_fixed_param:
                    self._divergences[i] = dzero['ct_divergences']
                    self._max_treedepths[i] = dzero['ct_max_treedepth']
            else:
                drest = check_sampler_csv(path=self.runset.csv_files[i], is_fixed_param=self._is_fixed_param, iter_sampling=self._iter_sampling, iter_warmup=self._iter_warmup, save_warmup=self._save_warmup, thin=self._thin)
                for key in dzero:
                    if key in ['stan_version_major', 'stan_version_minor', 'stan_version_patch', 'stanc_version', 'model', 'num_samples', 'num_warmup', 'save_warmup', 'thin', 'refresh'] and dzero[key] != drest[key]:
    ...
    def _assemble_draws(self) -> None:
        ...
        self._draws = np.empty((num_draws, self.chains, len(self.column_names)), dtype=float, order='F')
                    ...
                    if self._metadata.cmdstan_config['metric'] != 'unit_e':
                        ...
                        if chain == 0:
                            if self.metric_type == 'diag_e':
                                self._metric = np.empty((self.chains, num_unconstrained_params), dtype=float)
                            else:
                                self._metric = np.empty((self.chains, num_unconstrained_params, num_unconstrained_params), dtype=float)
                        if self.metric_type == 'diag_e':
                            xs = line.split(',')
    ...
    def summary(self, percentiles: Sequence[int]=(5, 50, 95), sig_figs: int=6) -> pd.DataFrame:
        """
        ...
        if len(percentiles) == 0:
            raise ValueError('Invalid percentiles argument, must be ordered non-empty list from (1, 99), inclusive.')
        cur_pct = 0
        for pct in percentiles:
            if pct > 99 or not pct > cur_pct:
                raise ValueError('Invalid percentiles spec, must be ordered non-empty list from (1, 99), inclusive.')
            cur_pct = pct
        percentiles_str = f'--percentiles= {','.join((str(x) for x in percentiles))}'
        if not isinstance(sig_figs, int) or sig_figs < 1 or sig_figs > 18:
            raise ValueError('Keyword "sig_figs" must be an integer between 1 and 18, found {}'.format(sig_figs))
        csv_sig_figs = self._sig_figs or 6
        if sig_figs > csv_sig_figs:
            get_logger().warning('Requesting %d significant digits of output, but CSV files only have %d digits of precision.', sig_figs, csv_sig_figs)
        ...
        tmp_csv_path = create_named_text_file(dir=_TMPDIR, prefix=tmp_csv_file, suffix='.csv', name_only=True)
        csv_str = '--csv_filename={}'.format(tmp_csv_path)
        if cmdstan_version_before(2, 24):
            csv_str = '--csv_file={}'.format(tmp_csv_path)
        cmd = [cmd_path, percentiles_str, sig_figs_str, csv_str] + self.runset.csv_files
        do_command(cmd, fd_out=None)
        with open(tmp_csv_path, 'rb') as fd:
            summary_data = pd.read_csv(fd, delimiter=',', header=0, index_col=0, comment='#', float_precision='high')
        mask = [not x.endswith('__') for x in summary_data.index] if self._is_fixed_param else [x == 'lp__' or not x.endswith('__') for x in summary_data.index]
    ...
    def draws_pd(self, vars: Union[List[str], str, None]=None, inc_warmup: bool=False) -> pd.DataFrame:
        """
        ...
        if vars is not None:
            if isinstance(vars, str):
                ...
                vars_list = vars
        if inc_warmup and (not self._save_warmup):
            get_logger().warning('Draws from warmup iterations not available, must run sampler with "save_warmup=True".')
        self._assemble_draws()
        cols = []
        if vars is not None:
        ...
        return pd.DataFrame(data=flatten_chains(draws), columns=['chain__', 'iter__', 'draw__'] + list(self.column_names))[cols]

    def draws_xr(self, vars: Union[str, List[str], None]=None, inc_warmup: bool=False) -> 'xr.Dataset':
        """
        ...
        attrs: MutableMapping[Hashable, Any] = {'stan_version': f'{meta['stan_version_major']}.{meta['stan_version_minor']}.{meta['stan_version_patch']}', 'model': meta['model'], 'num_draws_sampling': num_draws}
