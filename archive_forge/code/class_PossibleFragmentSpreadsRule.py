...
class PossibleFragmentSpreadsRule(ValidationRule):
    ...
    def enter_inline_fragment(self, node: InlineFragmentNode, *_args: Any) -> None:
        ...
        if is_composite_type(frag_type) and is_composite_type(parent_type) and (not do_types_overlap(context.schema, cast(GraphQLCompositeType, frag_type), cast(GraphQLCompositeType, parent_type))):
            context.report_error(GraphQLError(f"Fragment cannot be spread here as objects of type '{parent_type}' can never be of type '{frag_type}'.", node))
    ...
    def enter_fragment_spread(self, node: FragmentSpreadNode, *_args: Any) -> None:
        ...
        if frag_type and parent_type and (not do_types_overlap(context.schema, frag_type, parent_type)):
            context.report_error(GraphQLError(f"Fragment '{frag_name}' cannot be spread here as objects of type '{parent_type}' can never be of type '{frag_type}'.", node))
