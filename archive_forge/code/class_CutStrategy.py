    ...
    **Example**
    ...
    def __post_init__(self, devices):
        ...
        if devices is not None:
            if not isinstance(devices, SequenceType) or any((not isinstance(d, (qml.Device, qml.devices.Device)) for d in devices)):
                raise ValueError('Argument `devices` must be a list or tuple containing elements of type `qml.Device` or `qml.devices.Device`')
        ...
        if self.imbalance_tolerance is not None and (not (isinstance(self.imbalance_tolerance, (float, int)) and self.imbalance_tolerance >= 0)):
            raise ValueError(f'The overall `imbalance_tolerance` is expected to be a non-negative number, got {type(self.imbalance_tolerance)} with value {self.imbalance_tolerance}.')
    ...
    def _validate_input(max_wires_by_fragment, max_gates_by_fragment):
        ...
        if max_wires_by_fragment is not None:
            if not isinstance(max_wires_by_fragment, (list, tuple)):
                raise ValueError(f'`max_wires_by_fragment` is expected to be a list or tuple, but got {type(max_gates_by_fragment)}.')
            if any((not (isinstance(i, int) and i > 0) for i in max_wires_by_fragment)):
                raise ValueError('`max_wires_by_fragment` is expected to contain positive integers only.')
        if max_gates_by_fragment is not None:
            if not isinstance(max_gates_by_fragment, (list, tuple)):
                raise ValueError(f'`max_gates_by_fragment` is expected to be a list or tuple, but got {type(max_gates_by_fragment)}.')
            if any((not (isinstance(i, int) and i > 0) for i in max_gates_by_fragment)):
                raise ValueError('`max_gates_by_fragment` is expected to contain positive integers only.')
        if max_wires_by_fragment is not None and max_gates_by_fragment is not None:
            if len(max_wires_by_fragment) != len(max_gates_by_fragment):
                raise ValueError(f'The lengths of `max_wires_by_fragment` and `max_gates_by_fragment` should be equal, but got {len(max_wires_by_fragment)} and {len(max_gates_by_fragment)}.')
