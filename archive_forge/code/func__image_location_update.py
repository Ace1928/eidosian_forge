...
def _image_location_update(context, session, image_id, location):
    ...
    try:
        location_ref = session.query(models.ImageLocation).filter_by(id=loc_id).filter_by(image_id=image_id).one()
        ...
        location_ref.update({'value': location['url'], 'meta_data': location['metadata'], 'status': location['status'], 'deleted': deleted, 'updated_at': updated_time, 'deleted_at': delete_time})
