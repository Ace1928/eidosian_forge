...
class TestPacket(unittest.TestCase):
    ...
    payload = b'\x06\x06GP\x00\x00\x00\x00' + b'\xcd\xc5\x00\x00\x00\x00\x00\x00' + b'\x10\x11\x12\x13\x14\x15\x16\x17' + b'\x18\x19\x1a\x1b\x1c\x1d\x1e\x1f'
    ...
    def test_vlan_arp(self):
        e = ethernet.ethernet(self.dst_mac, self.src_mac, ether.ETH_TYPE_8021Q)
        v = vlan.vlan(7, 1, 3, ether.ETH_TYPE_ARP)
        a = arp.arp(1, ether.ETH_TYPE_IP, 6, 4, 2, self.src_mac, self.src_ip, self.dst_mac, self.dst_ip)
        p = packet.Packet()
        p.add_protocol(e)
        p.add_protocol(v)
        p.add_protocol(a)
        p.serialize()
        e_buf = self.dst_mac_bin + self.src_mac_bin + b'\x81\x00'
        v_buf = b'\xf0\x03' + b'\x08\x06'
        a_buf = b'\x00\x01' + b'\x08\x00' + b'\x06' + b'\x04' + b'\x00\x02' + self.src_mac_bin + self.src_ip_bin + self.dst_mac_bin + self.dst_ip_bin
        ...
        self.assertEqual(7, p_vlan.pcp)
        self.assertEqual(1, p_vlan.cfi)
        ...
        self.assertEqual(self.dst_mac, p_arp.dst_mac)
        self.assertEqual(self.dst_ip, p_arp.dst_ip)
        eth_values = {'dst': self.dst_mac, 'src': self.src_mac, 'ethertype': ether.ETH_TYPE_8021Q}
        _eth_str = ','.join(['%s=%s' % (k, repr(eth_values[k])) for k, v in inspect.getmembers(p_eth) if k in eth_values])
        eth_str = '%s(%s)' % (ethernet.ethernet.__name__, _eth_str)
        vlan_values = {'pcp': 7, 'cfi': 1, 'vid': 3, 'ethertype': ether.ETH_TYPE_ARP}
        _vlan_str = ','.join(['%s=%s' % (k, repr(vlan_values[k])) for k, v in inspect.getmembers(p_vlan) if k in vlan_values])
