...
def draw_path(screen: pg.Surface, path: List[Node], block_size: int) -> None:
    ...
    This function iterates through each node in the provided path list and draws a rectangle
    on the Pygame screen at the node's position with dimensions defined by the block size.
    The color of the rectangle is determined by the global COLOR_PATH.
    ...
    """
    for node in path:
        # Calculate the rectangle's position and size based on the node's position and the block size
        rect = pg.Rect(
            node.position.x * block_size,  # X position
            node.position.y * block_size,  # Y position
            block_size,  # Width of the rectangle
            block_size,  # Height of the rectangle
        )
        # Draw the rectangle on the screen with the color specified for the path
