...
class AbstractLocalProgram(AbstractProgram):
    ...
    def list_jobs(self, created_before: Optional[Union[datetime.datetime, datetime.date]]=None, created_after: Optional[Union[datetime.datetime, datetime.date]]=None, has_labels: Optional[Dict[str, str]]=None, execution_states: Optional[Set[quantum.ExecutionStatus.State]]=None) -> Sequence['AbstractLocalJob']:
        ...
        for job in self._jobs.values():
            ...
            if has_labels:
                ...
                if not all((label in job_labels and job_labels[label] == has_labels[label] for label in has_labels)):
                    continue
            job_list.append(job)
        return job_list
