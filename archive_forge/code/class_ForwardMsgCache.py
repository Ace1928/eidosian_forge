...
class ForwardMsgCache(CacheStatsProvider):
    ...
    class Entry:
        ...
        def __init__(self, msg: ForwardMsg | None):
            self.msg = msg
            self._session_script_run_counts: MutableMapping[AppSession, int] = WeakKeyDictionary()
    ...
    def add_message(self, msg: ForwardMsg, session: AppSession, script_run_count: int) -> None:
    ...
    def has_message_reference(self, msg: ForwardMsg, session: AppSession, script_run_count: int) -> bool:
    ...
    def remove_expired_entries_for_session(self, session: AppSession, script_run_count: int) -> None:
        ...
        for msg_hash, entry in self._entries.copy().items():
            ...
            age = entry.get_session_ref_age(session, script_run_count)
            if age > max_age:
                _LOGGER.debug('Removing expired entry [session=%s, hash=%s, age=%s]', id(session), msg_hash, age)
                entry.remove_session_ref(session)
                if not entry.has_refs():
                    del self._entries[msg_hash]
    ...
    def get_stats(self) -> list[CacheStat]:
        ...
        for entry_hash, entry in self._entries.items():
            stats.append(CacheStat(category_name='ForwardMessageCache', cache_name='', byte_length=entry.msg.ByteSize() if entry.msg is not None else 0))
