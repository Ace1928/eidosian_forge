...
def _AddRestrictedServices(parser, include_set=True):
    repeated.AddPrimitiveArgs(parser, 'perimeter', 'restricted-services', 'restricted services', metavar='SERVICE', additional_help='The perimeter boundary DOES apply to these services (for example, `storage.googleapis.com`).', include_set=include_set)