...
@TransformationFactory.register('contrib.piecewise.inner_repn_gdp', doc='Convert piecewise-linear model to a GDP using an inner representation of the simplices that are the domains of the linear functions.')
class InnerRepresentationGDPTransformation(PiecewiseLinearToGDP):
    ...
    def _transform_pw_linear_expr(self, pw_expr, pw_linear_func, transformation_block):
        ...
        for simplex, linear_func in zip(pw_linear_func._simplices, pw_linear_func._linear_functions):
            ...
            disj.convex_combo = Constraint(expr=sum((disj.lambdas[i] for i in range(len(extreme_pts)))) == 1)
            ...
            def linear_combo(disj, i):
                return pw_expr.args[i] == sum((disj.lambdas[j] * pt[i] for j, pt in enumerate(extreme_pts)))
