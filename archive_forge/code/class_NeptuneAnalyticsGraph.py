...
class NeptuneAnalyticsGraph(BaseNeptuneGraph):
    ...
    *Security note*: Make sure that the database connection uses credentials
    ...
    def __init__(self, graph_identifier: str, client: Any=None, credentials_profile_name: Optional[str]=None, region_name: Optional[str]=None) -> None:
        ...
        try:
            ...
            if type(e).__name__ == 'UnknownServiceError':
                raise ModuleNotFoundError('NeptuneGraph requires a boto3 version 1.34.40 or greater.Please install it with `pip install -U boto3`.') from e
            else:
    ...
    def query(self, query: str, params: dict={}) -> Dict[str, Any]:
        ...
        try:
            resp = self.client.execute_query(graphIdentifier=self.graph_identifier, queryString=query, parameters=params, language='OPEN_CYPHER')
    ...
    def _get_summary(self) -> Dict:
        try:
            ...
            raise NeptuneQueryException({'message': 'Summary API error occurred on Neptune Analytics', 'details': str(e)})
        try:
