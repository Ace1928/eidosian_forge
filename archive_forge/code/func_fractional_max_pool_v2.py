...
def fractional_max_pool_v2(value, pooling_ratio, pseudo_random=False, overlapping=False, seed=0, name=None):
  ...
  """
    if isinstance(pooling_ratio, (list, tuple)):
        if pooling_ratio[0] != 1.0 or pooling_ratio[-1] != 1.0:
            raise ValueError(f'`pooling_ratio` should have first and last elements with value 1.0. Received: pooling_ratio={pooling_ratio}')
        for element in pooling_ratio:
            if element < 1.0:
                raise ValueError(f'`pooling_ratio` elements should be >= 1.0. Received: pooling_ratio={pooling_ratio}')
    elif isinstance(pooling_ratio, (int, float)):
        if pooling_ratio < 1.0:
            raise ValueError(f'`pooling_ratio` should be >= 1.0. Received: pooling_ratio={pooling_ratio}')
    else:
        raise ValueError(f'`pooling_ratio` should be an int or a list of ints. Received: pooling_ratio={pooling_ratio}')
    pooling_ratio = _get_sequence(pooling_ratio, 2, 3, 'pooling_ratio')
    if seed == 0:
        if config.is_op_determinism_enabled():
            raise ValueError(f'tf.nn.fractional_max_pool requires a non-zero seed to be passed in when determinism is enabled, but got seed={seed}. Please pass in a non-zero seed, e.g. by passing "seed=1".')
        return gen_nn_ops.fractional_max_pool(value, pooling_ratio, pseudo_random, overlapping, deterministic=False, seed=0, seed2=0, name=name)
    else:
        seed1, seed2 = random_seed.get_seed(seed)
        return gen_nn_ops.fractional_max_pool(value, pooling_ratio, pseudo_random, overlapping, deterministic=True, seed=seed1, seed2=seed2, name=name)