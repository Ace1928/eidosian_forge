...
def _IsUrlValidParentDir(url):
  ...
  """
    if not url.IsFileUrl():
        return True
    url_string_minus_trailing_delimiter = url.versionless_url_string.rstrip(url.delim)
    _, _, after_last_delimiter = url_string_minus_trailing_delimiter.rpartition(url.delim)
    return after_last_delimiter not in storage_url.RELATIVE_PATH_SYMBOLS and after_last_delimiter not in [url.scheme + '://' + symbol for symbol in storage_url.RELATIVE_PATH_SYMBOLS]