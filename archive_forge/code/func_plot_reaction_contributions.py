...
def plot_reaction_contributions(xyp, rsys, rate_exprs_cb, substance_keys=None, varied=None, axes=None, total=False, linthreshy=1e-09, relative=False, xscale='log', yscale='symlog', xlabel='Time', ylabel=None, combine_equilibria=False, selection=slice(None), unit_registry=None):
    ...
    for sk, ax in zip(substance_keys, axes):
        ...
        if ylabel is None:
            ax.set_ylabel('$\\frac{d}{dt}\\left[%s\\right]\\ /\\ M\\cdot s^{-1}$' % ttl)
        else:
