...
class facet_grid(facet):
    ...
    def __init__(self, rows: Optional[str | Sequence[str]]=None, cols: Optional[str | Sequence[str]]=None, *, margins: bool | list[str]=False, scales: Literal['fixed', 'free', 'free_x', 'free_y']='fixed', space: Literal['fixed', 'free', 'free_x', 'free_y'] | FacetSpaceRatios='fixed', shrink: bool=True, labeller: Literal['label_value', 'label_both', 'label_context']='label_value', as_table: bool=True, drop: bool=True):
    ...
    def compute_layout(self, data: list[pd.DataFrame]) -> pd.DataFrame:
        if not self.rows and (not self.cols):
            ...
            return layout_null()
        base_rows = combine_vars(data, self.environment, self.rows, drop=self.drop)
        if not self.as_table:
            base_rows = base_rows[::-1]
        base_cols = combine_vars(data, self.environment, self.cols, drop=self.drop)
