...
def switch_controller_switch_group(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('switch-controller', 'switch-group', filtered_data, vdom=vdom)
        current_data = fos.get('switch-controller', 'switch-group', vdom=vdom, mkey=mkey)
    ...
    if state == 'present' or state is True:
        return fos.set('switch-controller', 'switch-group', data=filtered_data, vdom=vdom)
    elif state == 'absent':
        return fos.delete('switch-controller', 'switch-group', mkey=filtered_data['name'], vdom=vdom)
    else:
