...
@transform
def merge_rotations(tape: QuantumTape, atol=1e-08, include_gates=None) -> (Sequence[QuantumTape], Callable):
            ...
            if current_gate.name == next_gate.name and current_gate.wires == next_gate.wires:
                ...
                if current_gate.name == 'Rot':
                    if is_abstract(cumulative_angles):
                        ...
                        cumulative_angles = fuse_rot_angles(cumulative_angles, cast_like(stack(next_gate.parameters, like=interface), cumulative_angles))
                elif is_abstract(cumulative_angles):
                    cumulative_angles = cumulative_angles + stack(next_gate.parameters)
                else:
                    cumulative_angles = cumulative_angles + cast_like(stack(next_gate.parameters, like=interface), cumulative_angles)
            else:
                break
            next_gate_idx = find_next_gate(current_gate.wires, list_copy[1:])
        if is_abstract(cumulative_angles):
            with QueuingManager.stop_recording():
                new_operations.append(current_gate.__class__(*cumulative_angles, wires=current_gate.wires))
        elif not allclose(cumulative_angles, zeros(len(cumulative_angles)), atol=atol, rtol=0):
            with QueuingManager.stop_recording():
                new_operations.append(current_gate.__class__(*cumulative_angles, wires=current_gate.wires))
        list_copy.pop(0)
