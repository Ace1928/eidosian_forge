...
def _train_on_tpu_system(ctx, model_fn_wrapper, dequeue_fn):
    ...
    def multi_tpu_train_steps_on_single_shard(replica_id):
        ...
        with tpu_context._TPUEstimatorReplicaContext(replica_id):
            outputs = training_loop.while_loop(lambda i, loss: i < iterations_per_loop_var, lambda i, loss: [i + 1, single_tpu_train_step(i)], inputs=[0, _INITIAL_LOSS])
            return outputs[1:]
    replica_id_inputs = []
    replica_id_inputs.append([tf.constant(i) for i in range(ctx.num_replicas)])
    compile_op, loss = tpu.split_compile_and_shard(multi_tpu_train_steps_on_single_shard, inputs=replica_id_inputs, num_shards=ctx.num_replicas, outputs_from_all_shards=False, device_assignment=ctx.device_assignment)
    loss = loss[0]
    return (compile_op, loss, host_call, captured_scaffold_fn, captured_training_hooks.get())