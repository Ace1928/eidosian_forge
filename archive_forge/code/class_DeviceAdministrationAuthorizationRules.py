...
class DeviceAdministrationAuthorizationRules(object):
    ...
    def get_object_by_name(self, name, policy_id):
        result = None
        items = self.ise.exec(family='device_administration_authorization_rules', function='get_device_admin_authorization_rules', params={'policy_id': policy_id}).response.get('response', []) or []
        for item in items:
    ...
    def get_object_by_id(self, id, policy_id):
        try:
            result = self.ise.exec(family='device_administration_authorization_rules', function='get_device_admin_authorization_rule_by_id', handle_func_exception=False, params={'id': id, 'policy_id': policy_id}).response['response']
    ...
    def create(self):
        result = self.ise.exec(family='device_administration_authorization_rules', function='create_device_admin_authorization_rule', params=self.new_object).response
        return result
    ...
    def update(self):
        ...
        result = self.ise.exec(family='device_administration_authorization_rules', function='update_device_admin_authorization_rule_by_id', params=self.new_object).response
        return result
    ...
    def delete(self):
        ...
        if not id:
            ...
            rule.update(dict(id=id_))
            self.new_object.update(dict(rule=rule, id=id_))
        result = self.ise.exec(family='device_administration_authorization_rules', function='delete_device_admin_authorization_rule_by_id', params=self.new_object).response
        return result