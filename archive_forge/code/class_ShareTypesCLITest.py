...
class ShareTypesCLITest(base.OSCClientTestBase):
    ...
    def test_share_type_create(self):
        ...
        share_type = self.create_share_type(name=name, description=description)
        ...
        self.assertIn(share_type['id'], [item['ID'] for item in share_types_list])
    ...
    def test_share_type_delete(self):
        ...
        self.openstack(f'share type delete {share_type_1['id']} {share_type_2['id']}')
    ...
    def test_share_type_show(self):
        share_type = self.create_share_type(extra_specs={'foo': 'bar'})
        result = json.loads(self.openstack(f'share type show {share_type['id']} -f json'))
        self.assertEqual(share_type['name'], result['name'])
        self.assertEqual(share_type['visibility'], result['visibility'])
        self.assertEqual(share_type['is_default'], str(result['is_default']))
        self.assertEqual('bar', result['optional_extra_specs']['foo'])