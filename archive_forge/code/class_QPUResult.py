...
class QPUResult:
    ...
    def __init__(self, counts: Dict[int, int], num_qubits: int, measurement_dict: Dict[str, Sequence[int]]):
    ...
    def to_cirq_result(self, params: Optional[cirq.ParamResolver]=None) -> cirq.Result:
        ...
        for key, targets in self.measurement_dict().items():
            ...
            measurements[key] = np.array(list((cirq.big_endian_int_to_bits(x, bit_count=len(targets)) for x in qpu_results)))
    ...
    def __eq__(self, other):
        ...
        return self._counts == other._counts and self._num_qubits == other._num_qubits and (self._measurement_dict == other._measurement_dict) and (self._repetitions == other._repetitions)
