...
def holonomy_from_shape_intervals(manifold, shape_intervals, fundamental_group_args=[], lift_to_SL2=True):
    ...
    f = FundamentalPolyhedronEngine.from_manifold_and_shapes(M, shape_intervals, normalize_matrices=True)
    mats = f.matrices_for_presentation(G, match_kernel=True)
    PG = ManifoldGroup(G.generators(), G.relators(), G.peripheral_curves(), mats)
    if lift_to_SL2:
        PG.lift_to_SL2C()
    return PG