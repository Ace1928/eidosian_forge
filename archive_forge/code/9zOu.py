...
def download_missing_package(detected_language_code):
    for package in available_packages:
        if package.from_code == detected_language_code and package.to_code == "en":
            if not os.path.exists(
                f"{package.from_code}_to_{package.to_code}.argosmodel"
            ):
                print(
...
def detect_language(text, installed_languages):
...
source_lang = detect_language(detected_language, installed_languages)
