...
class SKLearnVectorStore(VectorStore):
    ...
    def __init__(self, embedding: Embeddings, *, persist_path: Optional[str]=None, serializer: Literal['json', 'bson', 'parquet']='json', metric: str='cosine', **kwargs: Any) -> None:
    ...
    def persist(self) -> None:
        ...
        data = {'ids': self._ids, 'texts': self._texts, 'metadatas': self._metadatas, 'embeddings': self._embeddings}
    ...
    def similarity_search_with_score(self, query: str, *, k: int=DEFAULT_K, **kwargs: Any) -> List[Tuple[Document, float]]:
        ...
        return [(Document(page_content=self._texts[idx], metadata={'id': self._ids[idx], **self._metadatas[idx]}), dist) for idx, dist in indices_dists]
    ...
    def _similarity_search_with_relevance_scores(self, query: str, k: int=DEFAULT_K, **kwargs: Any) -> List[Tuple[Document, float]]:
    ...
    def max_marginal_relevance_search_by_vector(self, embedding: List[float], k: int=DEFAULT_K, fetch_k: int=DEFAULT_FETCH_K, lambda_mult: float=0.5, **kwargs: Any) -> List[Document]:
        ...
        mmr_selected = maximal_marginal_relevance(self._np.array(embedding, dtype=self._np.float32), result_embeddings, k=k, lambda_mult=lambda_mult)
        mmr_indices = [indices[i] for i in mmr_selected]
        return [Document(page_content=self._texts[idx], metadata={'id': self._ids[idx], **self._metadatas[idx]}) for idx in mmr_indices]

    def max_marginal_relevance_search(self, query: str, k: int=DEFAULT_K, fetch_k: int=DEFAULT_FETCH_K, lambda_mult: float=0.5, **kwargs: Any) -> List[Document]:
        """Return docs selected using the maximal marginal relevance.
        Maximal marginal relevance optimizes for similarity to query AND diversity
        among selected documents.
        Args:
    ...
    @classmethod
    def from_texts(cls, texts: List[str], embedding: Embeddings, metadatas: Optional[List[dict]]=None, ids: Optional[List[str]]=None, persist_path: Optional[str]=None, **kwargs: Any) -> 'SKLearnVectorStore':
