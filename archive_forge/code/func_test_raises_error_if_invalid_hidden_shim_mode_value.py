...
def test_raises_error_if_invalid_hidden_shim_mode_value(self):
    with util.SetBotoConfigForTest([('GSUtil', 'hidden_shim_mode', 'invalid')]):
        with self.assertRaisesRegex(exception.CommandException, 'CommandException: Invalid option specified for GSUtil:use_gcloud_storage config setting. Should be one of: no_fallback | dry_run | none'):
