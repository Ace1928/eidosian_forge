...
class DataobjImage(FileBasedImage):
    ...
    def __init__(self, dataobj: ArrayLike, header: FileBasedHeader | ty.Mapping | None=None, extra: ty.Mapping | None=None, file_map: FileMap | None=None):
    ...
    @deprecate_with_version('get_data() is deprecated in favor of get_fdata(), which has a more predictable return type. To obtain get_data() behavior going forward, use numpy.asanyarray(img.dataobj).', '3.0', '5.0')
    ...
    def get_fdata(self, caching: ty.Literal['fill', 'unchanged']='fill', dtype: npt.DTypeLike=np.float64) -> np.ndarray[ty.Any, np.dtype[np.floating]]:
    ...
    def in_memory(self) -> bool:
        ...
        """
        return isinstance(self._dataobj, np.ndarray) or self._fdata_cache is not None or self._data_cache is not None
    ...
    @classmethod
    def from_file_map(klass: type[ArrayImgT], file_map: FileMap, *, mmap: bool | ty.Literal['c', 'r']=True, keep_file_open: bool | None=None) -> ArrayImgT:
    ...
    @classmethod
    def from_filename(klass: type[ArrayImgT], filename: FileSpec, *, mmap: bool | ty.Literal['c', 'r']=True, keep_file_open: bool | None=None) -> ArrayImgT:
