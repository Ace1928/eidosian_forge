...
class TestRoleAssignmentList(TestRoleAssignment):
    ...
    def setUp(self):
        ...
        self.roles_mock.roles_for_user.return_value = [fakes.FakeResource(None, copy.deepcopy(identity_fakes.ROLE), loaded=True)]
    ...
    def test_role_assignment_list_by_name_project_and_user(self):
        self.roles_mock.roles_for_user.return_value = [fakes.FakeResource(None, copy.deepcopy(identity_fakes.ROLE), loaded=True), fakes.FakeResource(None, copy.deepcopy(identity_fakes.ROLE_2), loaded=True)]
        arglist = ['--project', identity_fakes.project_name, '--user', identity_fakes.user_name, '--names']
        verifylist = [('user', identity_fakes.user_name), ('project', identity_fakes.project_name), ('names', True)]
        parsed_args = self.check_parser(self.cmd, arglist, verifylist)
        columns, data = self.cmd.take_action(parsed_args)
        self.roles_mock.roles_for_user.assert_called_with(identity_fakes.user_id, identity_fakes.project_id)
        self.assertEqual(self.columns, columns)
        datalist = ((identity_fakes.role_name, identity_fakes.user_name, identity_fakes.project_name), (identity_fakes.ROLE_2['name'], identity_fakes.user_name, identity_fakes.project_name))
        self.assertEqual(datalist, tuple(data))