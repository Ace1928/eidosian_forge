...
class LinearModelDecisionTreeExample(CommonTests):
    ...
    def test_logical_constraints_on_disjuncts(self):
        ...
        transformed = mbm.get_transformed_constraints(m.d1._logical_to_disjunctive.transformed_constraints[1])
        ...
        assertExpressionsStructurallyEqual(self, simplified, -m.d2.binary_indicator_var - m.d3.binary_indicator_var + z1 + y - z - 1)
        transformed = mbm.get_transformed_constraints(m.d1._logical_to_disjunctive.transformed_constraints[2])
        self.assertEqual(len(transformed), 1)
        c = transformed[0]
        check_obj_in_active_tree(self, c)
        self.assertIsNone(c.lower)
        ...
        self.assertTrue(repn.is_linear())
        simplified = repn.constant + sum((repn.linear_coefs[i] * repn.linear_vars[i] for i in range(len(repn.linear_vars))))
        assertExpressionsStructurallyEqual(self, simplified, -m.d2.binary_indicator_var - m.d3.binary_indicator_var - y - z1 + 1)
        transformed = mbm.get_transformed_constraints(m.d1._logical_to_disjunctive.transformed_constraints[3])
        self.assertEqual(len(transformed), 1)
        c = transformed[0]
        check_obj_in_active_tree(self, c)
        self.assertIsNone(c.lower)
        ...
        self.assertTrue(repn.is_linear())
        simplified = repn.constant + sum((repn.linear_coefs[i] * repn.linear_vars[i] for i in range(len(repn.linear_vars))))
        assertExpressionsStructurallyEqual(self, simplified, -m.d2.binary_indicator_var - m.d3.binary_indicator_var + z - z1)
    ...
    def test_only_multiple_bigm_bound_constraints(self):
        ...
        self.check_pretty_bound_constraints(cons[0], m.x2, {m.d1: 0.75, m.d2: 3, m.d3: 0.55}, lb=True)
        self.check_pretty_bound_constraints(cons[1], m.x2, {m.d1: 3, m.d2: 10, m.d3: 1}, lb=False)
        self.check_traditionally_bigmed_constraints(m, mbm, {m.d1: (-1030.0, 1030.0), m.d2: (-1093.0, 1107.0), m.d3: (-1113.0, 1107.0)})
