...
class KeystoneAuthHandler(mistral_auth.AuthHandler):
    ...
    def authenticate(self, req, session=None):
        ...
        try:
            ...
            if 'Cannot use v2 authentication with domain scope' in str(e):
                LOG.warning('Client tried to use v2 authentication with domain scope. Domain parameters are assumed to be erroneously set. Retrying authentication without them. Request parameters: %s' % str(reqs))
                domainless_reqs = [reqs[0], self._remove_domain(reqs[1])]
    ...
    @staticmethod
    def _get_auth(api_key=None, auth_token=None, auth_url=None, project_domain_id=None, project_domain_name=None, project_id=None, project_name=None, user_domain_id=None, user_domain_name=None, user_id=None, username=None, **kwargs):
        if project_name and project_id:
            project_name = None
            LOG.warning('Only one of project_name or project_id should be set,project_name will be ignored')
        if username and user_id:
            username = None
            LOG.warning('Only one of username or user_id should be set,username will be ignored')
        auth = {}
        if auth_token:
            auth = auth_plugin.Token(auth_url=auth_url, project_domain_id=project_domain_id, project_domain_name=project_domain_name, project_id=project_id, project_name=project_name, token=auth_token)
        elif api_key and (username or user_id):
            auth = auth_plugin.Password(auth_url=auth_url, password=api_key, project_domain_id=project_domain_id, project_domain_name=project_domain_name, project_id=project_id, project_name=project_name, user_domain_id=user_domain_id, user_domain_name=user_domain_name, user_id=user_id, username=username)
        return auth
    ...
    def _authenticate(self, reqs, session=None):
        ...
        if session:
            if not mistral_url:
                try:
                    mistral_url = session.get_endpoint(service_type=service_type, interface=endpoint_type, region_name=region_name)
                except Exception:
                    mistral_url = None
        ...
        if 'auth_url' in target_req:
            ...
            if target_auth:
                ...
                verify = self._verification_needed(cacert, insecure)
                target_session = ks_session.Session(auth=target_auth, verify=verify)
                target_auth_headers = target_session.get_auth_headers() or {}
                target_auth_token = target_auth_headers.get('X-Auth-Token')
                auth_response.update({api.TARGET_AUTH_TOKEN: target_auth_token, api.TARGET_PROJECT_ID: target_session.get_project_id(), api.TARGET_USER_ID: target_session.get_user_id(), api.TARGET_AUTH_URI: target_auth._plugin.auth_url})
                ...
                if self._is_service_catalog_v2(service_catalog):
                    ...
                    if not len(access_data['serviceCatalog']):
                        LOG.warning('Service Catalog empty, some authenticationcredentials may be missing. This can causemalfunction in the Mistral action executions.')
                    sc_json = jsonutils.dumps(access_data)
                    auth_response[api.TARGET_SERVICE_CATALOG] = sc_json
        if not auth_response:
            LOG.debug('No valid token or password + user provided. Continuing without authentication')
            return {}
