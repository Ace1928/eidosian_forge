...
class StreamsClient:
    ...
    def _ParseGcsDestinationConfigBeta(self, gcs_destination_config_file):
        ...
        data = console_io.ReadFromFileOrStdin(gcs_destination_config_file, binary=False)
        try:
            gcs_destination_head_config_data = yaml.load(data)
        except yaml.YAMLParseError as e:
            raise ds_exceptions.ParseError('Cannot parse YAML:[{0}]'.format(e))
        gcs_destination_config_data_object = gcs_destination_head_config_data.get('gcs_destination_config')
        ...
        file_rotation_interval = gcs_destination_config_data.get('file_rotation_interval', {})
        gcs_dest_config_msg = self._messages.GcsDestinationConfig(path=path, fileRotationMb=file_rotation_mb, fileRotationInterval=file_rotation_interval)
        if 'avro_file_format' in gcs_destination_config_data:
            ...
            json_file_format_data = gcs_destination_config_data.get('json_file_format')
            gcs_dest_config_msg.jsonFileFormat = self._messages.JsonFileFormat(compression=json_file_format_data.get('compression'), schemaFileFormat=json_file_format_data.get('schema_file_format'))
        else:
            raise ds_exceptions.ParseError('Cannot parse YAML: missing file format.')
    ...
    def _ParseBigqueryDestinationConfig(self, config_file):
        ...
        return util.ParseMessageAndValidateSchema(config_file, 'BigQueryDestinationConfig', self._messages.BigQueryDestinationConfig)

    def _GetStream(self, stream_id, release_track, args):
        """Returns a stream object."""
        labels = labels_util.ParseCreateArgs(args, self._messages.Stream.LabelsValue)
        stream_obj = self._messages.Stream(name=stream_id, labels=labels, displayName=args.display_name)
        if release_track == base.ReleaseTrack.BETA:
        ...
        stream_source_config.sourceConnectionProfile = source_connection_profile_ref.RelativeName()
        if args.oracle_source_config:
            stream_source_config.oracleSourceConfig = self._ParseOracleSourceConfig(args.oracle_source_config, release_track)
        elif args.mysql_source_config:
            stream_source_config.mysqlSourceConfig = self._ParseMysqlSourceConfig(args.mysql_source_config, release_track)
        elif args.postgresql_source_config:
            stream_source_config.postgresqlSourceConfig = self._ParsePostgresqlSourceConfig(args.postgresql_source_config)
        ...
        stream_destination_config.destinationConnectionProfile = destination_connection_profile_ref.RelativeName()
        if args.gcs_destination_config:
            stream_destination_config.gcsDestinationConfig = self._ParseGcsDestinationConfig(args.gcs_destination_config, release_track)
        elif args.bigquery_destination_config:
            stream_destination_config.bigqueryDestinationConfig = self._ParseBigqueryDestinationConfig(args.bigquery_destination_config)
    ...
    def _UpdateLabels(self, stream, args):
        ...
        update_result = labels_util.Diff(additions=add_labels, subtractions=remove_labels, clear=args.clear_labels).Apply(value_type, stream.labels)
    ...
    """
        ...
        create_req = create_req_type(stream=stream, streamId=stream.name, parent=parent_ref, requestId=request_id, validateOnly=validate_only, force=force)
    ...
    """
        ...
        updated_stream, update_fields = self._GetUpdatedStream(current_stream, release_track, args)
        request_id = util.GenerateRequestId()
        update_req_type = self._messages.DatastreamProjectsLocationsStreamsPatchRequest
        update_req = update_req_type(stream=updated_stream, name=updated_stream.name, requestId=request_id, validateOnly=validate_only, force=force)
        if args.update_mask:
