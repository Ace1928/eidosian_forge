...
class SLURMPlugin(SGELikeBatchManagerBase):
    ...
    def _is_pending(self, taskid):
        try:
            res = CommandLine('squeue', args=' '.join(['-j', '%s' % taskid]), resource_monitor=False, terminal_output='allatonce').run()
            return res.runtime.stdout.find(str(taskid)) > -1
        except RuntimeError as e:
            if any((ss in str(e) for ss in ['Socket timed out', 'not available at the moment'])):
                logger.warning('SLURM timeout encountered while checking job status, treating job %d as pending', taskid)
                return True
    ...
    def _submit_batchtask(self, scriptfile, node):
        ...
        """
        cmd = CommandLine('sbatch', environ=dict(os.environ), resource_monitor=False, terminal_output='allatonce')
        ...
        while True:
            try:
                ...
                if tries < self._max_tries:
                    ...
                    raise RuntimeError('\n'.join(('Could not submit sbatch task for node %s' % node._id, str(e))))
            else:
                break
