...
class ChatServiceMessageSocket:
    ...
    def _send_world_alive(self):
        ...
        """
        self._safe_send(json.dumps({'type': 'world_alive', 'content': {'id': 'WORLD_ALIVE', 'sender_id': 'world'}}), force=True)
    ...
    def _setup_socket(self):
        ...
        def on_error(ws, error):
            try:
                if error.errno == errno.ECONNREFUSED:
                    ...
                    log_utils.print_and_log(logging.WARN, 'Socket logged error: {}'.format(repr(error)))
            except BaseException:
                if type(error) is websocket.WebSocketConnectionClosedException:
                    return
                log_utils.print_and_log(logging.WARN, 'Socket logged error: {} Restarting'.format(repr(error)))
                self._ensure_closed()
        ...
        def on_disconnect(*args):
            ...
            on a retry.
            """
            log_utils.print_and_log(logging.INFO, 'World server disconnected: {}'.format(args))
            self.alive = False
            self._ensure_closed()
        ...
        def on_message(*args):
            ...
            log_utils.print_and_log(logging.DEBUG, 'Message data received: {}'.format(message_data))
        ...
        def run_socket(*args):
            ...
            while self.keep_running:
                try:
                    ...
                    log_utils.print_and_log(logging.WARN, 'Socket error {}, attempting restart'.format(repr(e)))
                time.sleep(0.2)
        self.listen_thread = threading.Thread(target=run_socket, name='Main-Socket-Thread')
