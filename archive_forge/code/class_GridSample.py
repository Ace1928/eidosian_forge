...
class GridSample(OpRun):
    ...
    def _gs_cubic_interpolation_1d_with_x(self, data, x, border, padding_mode):
        ...
        v[0] = self._pixel_at_array(array=data, i=x_minus_1, border=border, padding_mode=padding_mode)
        v[1] = self._pixel_at_array(array=data, i=x_0, border=border, padding_mode=padding_mode)
        v[2] = self._pixel_at_array(array=data, i=x_1, border=border, padding_mode=padding_mode)
        v[3] = self._pixel_at_array(array=data, i=x_2, border=border, padding_mode=padding_mode)
        return coeffs @ v
    ...
    def _gs_cubic_interpolation_nd_with_x(self, data, x, border, padding_mode):
        ...
        assert num_dims == len(x) == int(len(border) / 2)
        if num_dims == 1:
            return self._gs_cubic_interpolation_1d_with_x(data=data, x=x[0], border=border, padding_mode=padding_mode)
        res1d = []
        for i in range(data.shape[0]):
            r = self._gs_cubic_interpolation_nd_with_x(data=data[i], x=x[1:], border=list(border[1:num_dims]) + list(border[1 + num_dims:2 * num_dims]), padding_mode=padding_mode)
            res1d.append(r)
        res1d = np.array(res1d)
        return self._gs_cubic_interpolation_1d_with_x(data=res1d, x=x[0], border=[border[0], border[num_dims]], padding_mode=padding_mode)
        ...
        for n in range(N):
            ...
            for c in range(C):
                ...
                for ox in _get_all_coords(Y[n, c]):
                    ...
                    nx = nx[::-1]
                    x = self._gs_denormalize_coordinates(n=nx, dims=dims, align_corners=align_corners)
                    if mode == 'nearest':
                        x = np.rint(x)
                    ...
                    if mode == 'nearest':
                        x = x.astype(np.int32)
                        Y[n][c][tuple(ox)] = self._pixel_at_ndarray(ndarray=X_data, x=x, border=border, padding_mode=padding_mode)
                    elif mode == 'linear':
                        Y[n][c][tuple(ox)] = self._gs_linear_interpolation_nd_with_x(data=X_data, x=x, border=border, padding_mode=padding_mode)
                    elif mode == 'cubic':
                        Y[n][c][tuple(ox)] = self._gs_cubic_interpolation_nd_with_x(data=X_data, x=x, border=border, padding_mode=padding_mode)
                    else:
                        raise RuntimeError('GridSample interpolation only supports nearest, linear, and cubic modes.')
