...
def _need_filtering(filters, partition_keys):
    partition_cols = {v[0] for v in flatten(partition_keys, container=list) if len(v)} if partition_keys else set()
    filtered_cols = {v[0] for v in flatten(filters, container=list) if len(v)} if filters else set()
