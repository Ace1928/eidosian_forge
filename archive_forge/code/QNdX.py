...
def play_tone(frequency: float, duration: float, volume: float = 0.5) -> None:
    ...
    """
    logging.debug(
        f"Playing tone: Frequency={frequency}, Duration={duration}, Volume={volume}"
    )
    ...
    sound_obj.stop()
    logging.info("Tone playback completed.")
...
def load_or_define_frequencies() -> Dict[int, float]:
    ...
    if os.path.exists(path):
        with open(path, "r") as file:
            ...
            logging.info("Loaded key frequencies from file.")
    else:
        key_to_frequency = {
        ...
        with open(path, "w") as file:
            ...
            logging.info("Default key frequencies set and saved.")
...
logging.info("Graphical interface initialized.")
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_ESCAPE:
                    ...
                    try:
                        ...
                        logging.info(
                            f"Key {pygame.key.name(event.key)} frequency updated to {new_freq} Hz."
                        )
                    except ValueError:
                        print("Invalid input. Please enter a valid numeric frequency.")
                        logging.warning("Invalid frequency input received.")
