...
def test_coefficient_matching_nonlinear_expr(self):
    ...
    with self.assertRaises(ValueError, msg='ValueError should be raised for general nonlinear expressions in h(x,z,q)=0 constraints.'):
        results = pyros_solver.solve(model=m, first_stage_variables=[m.x1], second_stage_variables=[m.x2], uncertain_params=[m.u], uncertainty_set=interval, local_solver=local_subsolver, global_solver=global_subsolver, options={'objective_focus': ObjectiveType.worst_case, 'solve_master_globally': True, 'decision_rule_order': 1})