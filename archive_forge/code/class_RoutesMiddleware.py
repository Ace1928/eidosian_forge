...
class RoutesMiddleware(object):
    ...
    def __call__(self, environ, start_response):
        ...
        if route and route.redirect:
            ...
            log.debug("Using redirect route, redirect to '%s' with statuscode: %s", location, route.redirect_status)
            start_response(route.redirect_status, [('Content-Type', 'text/plain; charset=utf8'), ('Location', location)])
            return []
        if self.path_info and 'path_info' in match:
            ...
            environ['SCRIPT_NAME'] += re.sub('^(.*?)/' + re.escape(newpath) + '$', '\\1', oldpath)
