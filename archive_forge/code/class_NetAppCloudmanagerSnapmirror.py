...
class NetAppCloudmanagerSnapmirror:
    ...
    def __init__(self):
        ...
        self.module = AnsibleModule(argument_spec=self.argument_spec, required_one_of=[['source_working_environment_id', 'source_working_environment_name'], ['refresh_token', 'sa_client_id']], required_together=(['source_working_environment_id', 'destination_working_environment_id'], ['source_working_environment_name', 'destination_working_environment_name'], ['sa_client_id', 'sa_secret_key']), supports_check_mode=True)
    ...
    def get_volumes(self, working_environment_detail, name):
        ...
        response, err, dummy = self.rest_api.send_request('GET', '%s/volumes?workingEnvironmentId=%s&name=%s' % (self.rest_api.api_root_path, working_environment_detail['publicId'], name), None, header=self.headers)
        if err is not None:
    ...
    def delete_snapmirror(self, sm_detail):
        api_delete = '/occm/api/replication/%s/%s/%s' % (sm_detail['destination_working_environment_id'], sm_detail['destination_svm_name'], self.parameters['destination_volume_name'])
