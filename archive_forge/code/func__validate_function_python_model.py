...
def _validate_function_python_model(python_model):
    if not (isinstance(python_model, PythonModel) or callable(python_model)):
        raise MlflowException('`python_model` must be a PythonModel instance or a callable object', error_code=INVALID_PARAMETER_VALUE)
    if callable(python_model):
        num_args = len(inspect.signature(python_model).parameters)
        if num_args != 1:
            raise MlflowException(f'When `python_model` is a callable object, it must accept exactly one argument. Found {num_args} arguments.', error_code=INVALID_PARAMETER_VALUE)