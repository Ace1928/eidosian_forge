...
class LoggingNotificationHandlerTestCase(utils.SkipIfNoTransportURL):
    ...
    """
    _priority = [('debug', dict(priority='debug')), ('info', dict(priority='info')), ('warn', dict(priority='warn')), ('error', dict(priority='error')), ('critical', dict(priority='critical'))]
    _driver = [('messaging', dict(driver='messaging')), ('messagingv2', dict(driver='messagingv2'))]

    @classmethod
    def generate_scenarios(cls):
        cls.scenarios = testscenarios.multiply_scenarios(cls._priority, cls._driver)

    def test_logging(self):
        topic = 'test_logging_%s_driver_%s' % (self.priority, self.driver)
        if self.notify_url.startswith('kafka://'):
            self.conf.set_override('consumer_group', str(uuid.uuid4()), group='oslo_messaging_kafka')
        self.config(driver=[self.driver], topics=[topic], group='oslo_messaging_notifications')
        listener = self.useFixture(utils.NotificationFixture(self.conf, self.notify_url, [topic]))
        ...
        for key in ['name', 'thread', 'extra', 'process', 'funcName', 'levelno', 'processName', 'pathname', 'lineno', 'msg', 'exc_info', 'levelname']:
