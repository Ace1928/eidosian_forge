...
class InferenceEndpoint:
    ...
    def wait(self, timeout: Optional[int]=None, refresh_every: int=5) -> 'InferenceEndpoint':
        ...
        while True:
            ...
            if self.status == InferenceEndpointStatus.FAILED:
                raise InferenceEndpointError(f'Inference Endpoint {self.name} failed to deploy. Please check the logs for more information.')
    ...
    def update(self, *, accelerator: Optional[str]=None, instance_size: Optional[str]=None, instance_type: Optional[str]=None, min_replica: Optional[int]=None, max_replica: Optional[int]=None, repository: Optional[str]=None, framework: Optional[str]=None, revision: Optional[str]=None, task: Optional[str]=None) -> 'InferenceEndpoint':
        ...
        Returns:
            [`InferenceEndpoint`]: the same Inference Endpoint, mutated in place with the latest data.
        """
        obj = self._api.update_inference_endpoint(name=self.name, namespace=self.namespace, accelerator=accelerator, instance_size=instance_size, instance_type=instance_type, min_replica=min_replica, max_replica=max_replica, repository=repository, framework=framework, revision=revision, task=task, token=self._token)
        self.raw = obj.raw
        self._populate_from_raw()
        return self
