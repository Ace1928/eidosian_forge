...
class FileDownloader(object):
    ...
    def __init__(self, insecure=False, cacert=None):
        ...
        if cacert is not None:
            if not self.cacert or not os.path.isfile(self.cacert):
                raise RuntimeError("cacert='%s' does not refer to a valid file." % (self.cacert,))
    ...
    def get_os_version(cls, normalize=True):
        ...
        _map = {'almalinux': 'rhel', 'centos': 'rhel', 'rocky': 'rhel', 'scientific': 'rhel', 'kubuntu': 'ubuntu', 'linuxmint': 'ubuntu', 'xubuntu': 'ubuntu'}
        if _os in _map:
            _os = _map[_os]
    ...
    def create_parser(self, parser=None):
        ...
        parser.add_argument('--insecure', action='store_true', dest='insecure', default=False, help='Disable all SSL verification')
        parser.add_argument('--cacert', action='store', dest='cacert', default=None, help='Use CACERT as the file of certificate authorities to verify peers.')
        parser.add_argument('-v', '--verbose', action='store_true', dest='verbose', default=False, help='Verbose output when download fails')
        return parser

    def parse_args(self, argv):
        parser = self.create_parser()
        parser.add_argument('target', nargs='?', default=None, help='Target destination directory or filename')
