    ...
    def work(self):
        ...
        if not self._exhausted:
            ...
            if remaining > self._buffered_data_comfortable_limit:
                return
            data = self._get_and_compensate_audio_data(self.source.audio_format.align(self._buffered_data_ideal_size - remaining), self._pseudo_play_cursor)
            if data is None:
                ...
                return
        if self._pseudo_play_cursor >= self._pseudo_write_cursor and self._exhausted and (not self._dispatched_on_eos):
