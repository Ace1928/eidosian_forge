...
class BufferedUploadStream(upload_stream.UploadStream):
    ...
    def __init__(self, stream, max_buffer_size, digesters=None, progress_callback=None):
    ...
    """
        super(__class__, self).__init__(stream=stream, length=None, digesters=digesters, progress_callback=progress_callback)
    ...
    def seek(self, offset, whence=os.SEEK_SET):
        ...
        if whence == os.SEEK_SET:
            if offset < self._buffer_start or offset > self._buffer_end:
                raise errors.Error('Unable to recover from an upload error because limited buffering is available for streaming uploads. Offset {} was requested, but only data from {} to {} is buffered.'.format(offset, self._buffer_start, self._buffer_end))
            new_position = offset
        elif whence == os.SEEK_END:
            if abs(offset) > self._max_buffer_size:
                raise errors.Error('Invalid SEEK_END offset {} on streaming upload. Only {} bytes can be buffered.'.format(offset, self._max_buffer_size))
            while self.read(self._max_buffer_size):
                pass
            new_position = self._position + offset
        else:
            raise errors.Error('Invalid seek mode on streaming upload. Mode: {}, offset: {}'.format(whence, offset))
