...
def ValidatePublicDnsFlags(args):
    ...
    if public_dns:
        if network_interface is not None and network_interface != constants.DEFAULT_NETWORK_INTERFACE:
            raise compute_exceptions.ArgumentError("Public DNS can only be enabled for default network interface '{0}' rather than '{1}'.".format(constants.DEFAULT_NETWORK_INTERFACE, network_interface))