...
class EdenAiSpeechToTextTool(EdenaiTool):
    ...
    description = 'A wrapper around edenai Services speech to text Useful for when you have to convert audio to text.Input should be a url to an audio file.'
    is_async = True
    ...
    def _run(self, query: str, run_manager: Optional[CallbackManagerForToolRun]=None) -> str:
        """Use the tool."""
        all_params = {'file_url': query, 'language': self.language, 'speakers': self.speakers, 'profanity_filter': self.profanity_filter, 'custom_vocabulary': self.custom_vocabulary}
