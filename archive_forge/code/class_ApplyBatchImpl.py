...
class ApplyBatchImpl:
    ...
    def __init__(self, impl: DefaultImpl, table: Table, table_args: tuple, table_kwargs: Dict[str, Any], reflected: bool, partial_reordering: tuple=()) -> None:
        ...
        self.add_col_ordering: Tuple[Tuple[str, str], ...] = ()
    ...
    def _gather_indexes_from_both_tables(self) -> List[Index]:
        ...
        for idx_existing in self.indexes.values():
            if idx_existing._column_flag:
                continue
            idx_copy = Index(idx_existing.name, *[_copy_expression(expr, self.new_table) for expr in _idx_table_bound_expressions(idx_existing)], unique=idx_existing.unique, _table=self.new_table, **idx_existing.kwargs)
            idx.append(idx_copy)
        for index in self.new_indexes.values():
            idx.append(Index(index.name, *[self.new_table.c[col] for col in index.columns.keys()], unique=index.unique, **index.kwargs))
        return idx
    ...
    def _create(self, op_impl: DefaultImpl) -> None:
        ...
        try:
            op_impl._exec(_insert_inline(self.new_table).from_select(list((k for k, transfer in self.column_transfers.items() if 'expr' in transfer)), _select(*[transfer['expr'] for transfer in self.column_transfers.values() if 'expr' in transfer])))
    ...
    def alter_column(self, table_name: str, column_name: str, nullable: Optional[bool]=None, server_default: Optional[Union[Function[Any], str, bool]]=False, name: Optional[str]=None, type_: Optional[TypeEngine]=None, autoincrement: Optional[Union[bool, Literal['auto']]]=None, comment: Union[str, Literal[False]]=False, **kw) -> None:
        ...
        if name is not None and name != column_name:
            ...
            if existing_type:
                ...
                if isinstance(resolved_existing_type, SchemaEventTarget) and resolved_existing_type.name:
                    self.named_constraints.pop(resolved_existing_type.name, None)
    ...
    def add_column(self, table_name: str, column: Column[Any], insert_before: Optional[str]=None, insert_after: Optional[str]=None, **kw) -> None:
