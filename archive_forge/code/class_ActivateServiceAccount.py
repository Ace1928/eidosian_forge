...
class ActivateServiceAccount(base.SilentCommand):
  ...
  """
    ...
    def Args(parser):
        ...
        parser.add_argument('account', nargs='?', help='E-mail address of the service account.')
        parser.add_argument('--key-file', help='Path to the private key file.', required=True)
        group = parser.add_mutually_exclusive_group()
        group.add_argument('--password-file', help='Path to a file containing the password for the service account private key (only for a .p12 file).')
        group.add_argument('--prompt-for-password', action='store_true', help='Prompt for the password for the service account private key (only for a .p12 file).')

    def Run(self, args):
        """Create service account credentials."""
        file_content, is_json = _IsJsonFile(args.key_file)
        if is_json:
            cred = auth_service_account.CredentialsFromAdcDictGoogleAuth(file_content)
            if args.password_file or args.prompt_for_password:
                raise c_exc.InvalidArgumentException('--password-file', 'A .json service account key does not require a password.')
            account = cred.service_account_email
            if args.account and args.account != account:
                raise c_exc.InvalidArgumentException('ACCOUNT', 'The given account name does not match the account name in the key file.  This argument can be omitted when using .json keys.')
        else:
            account = args.account
            if not account:
                raise c_exc.RequiredArgumentException('ACCOUNT', 'An account is required when using .p12 keys')
            password = None
            ...
            cred = auth_service_account.CredentialsFromP12Key(file_content, account, password=password)
        try:
        ...
        log.status.Print('Activated service account credentials for: [{0}]'.format(account))