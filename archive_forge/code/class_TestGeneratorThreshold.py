...
class TestGeneratorThreshold:
    ...
    def test_shortest_path_length(self):
        ...
        assert nxt.shortest_path_length(['d', 'd', 'd', 'i', 'd', 'd'], 1) == [1, 0, 1, 2, 1, 1]
        assert nxt.shortest_path_length(('d', 'd', 'd', 'i', 'd', 'd'), 1) == [1, 0, 1, 2, 1, 1]
    ...
    def test_create_using(self):
        cs = 'ddiiddid'
        G = nxt.threshold_graph(cs)
        assert pytest.raises(nx.exception.NetworkXError, nxt.threshold_graph, cs, create_using=nx.DiGraph())
