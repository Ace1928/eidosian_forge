...
class ActivationFunctionManager:
    ...
    def initialize_activation_types(self):
        ...
        self.activation_types = {
            ...
            "Softmax": lambda x: (
                cmath.exp(x) / sum(cmath.exp(item) for item in x)
                if isinstance(x, (list, tuple))
                else cmath.exp(x) / cmath.exp(x)
            ),
            "Linear": lambda x: x,
            "ELU": lambda x: x if x > 0 else 0.01 * (cmath.exp(x) - 1),
            "Swish": lambda x: x / (1 + cmath.exp(-x)),
            "Leaky ReLU": lambda x: x if x > 0 else 0.01 * x,
            "Parametric ReLU": lambda x, a=0.01: x if x > 0 else a * x,
            "ELU-PA": lambda x, a=0.01: x if x > 0 else a * (cmath.exp(x) - 1),
            "GELU": lambda x: 0.5
            * x
            * (
                1
                + cmath.tanh(
                    cmath.sqrt(2 / cmath.pi) * (x + 0.044715 * cmath.pow(x, 3))
                )
            ),
            "Softplus": lambda x: cmath.log(1 + cmath.exp(x)),
            "Softsign": lambda x: x / (1 + abs(x)),
            "Bent Identity": lambda x: x + (cmath.sqrt(x**2 + 1) - 1) / 2,
            "Hard Sigmoid": lambda x: max(0, min(1, (x + 1) / 2)),
        }
