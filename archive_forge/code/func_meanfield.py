...
def meanfield(symbols, coordinates, name='molecule', charge=0, mult=1, basis='sto-3g', package='pyscf', outpath='.'):
    ...
    **Example**
    ...
    if coordinates.size != 3 * len(symbols):
        raise ValueError(f"The size of the array 'coordinates' has to be 3*len(symbols) = {3 * len(symbols)}; got 'coordinates.size' = {coordinates.size}")
    package = package.strip().lower()
    if package not in 'pyscf':
        error_message = f"Integration with quantum chemistry package '{package}' is not available. \n Please set 'package' to 'pyscf'."
        raise TypeError(error_message)
    filename = name + '_' + package.lower() + '_' + basis.strip()
    path_to_file = os.path.join(outpath.strip(), filename)
    geometry = [[symbol, tuple(np.array(coordinates)[3 * i:3 * i + 3] * bohr_angs)] for i, symbol in enumerate(symbols)]
