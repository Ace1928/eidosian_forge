...
class Git(LazyMixin):
        ...
        if not has_git:
            ...
            if old_git is None:
                ...
                if mode in quiet:
                    ...
                    err = dedent('                        %s\n                        All git commands will error until this is rectified.\n\n                        This initial message can be silenced or aggravated in the future by setting the\n                        $%s environment variable. Use one of the following values:\n                            - %s: for no message or exception\n                            - %s: for a warning message (logged at level CRITICAL, displayed by default)\n                            - %s: for a raised exception\n\n                        Example:\n                            export %s=%s\n                        ') % (err, cls._refresh_env_var, '|'.join(quiet), '|'.join(warn), '|'.join(error), cls._refresh_env_var, quiet[0])
                    if mode in warn:
                        _logger.critical(err)
                    else:
                        raise ImportError(err)
                else:
                    err = dedent('                        %s environment variable has been set but it has been set with an invalid value.\n\n                        Use only the following values:\n                            - %s: for no message or exception\n                            - %s: for a warning message (logged at level CRITICAL, displayed by default)\n                            - %s: for a raised exception\n                        ') % (cls._refresh_env_var, '|'.join(quiet), '|'.join(warn), '|'.join(error))
                    raise ImportError(err)
    ...
    def _set_cache_(self, attr: str) -> None:
        if attr == '_version_info':
            ...
            self._version_info = cast(Tuple[int, int, int, int], tuple((int(n) for n in version_numbers.split('.')[:4] if n.isdigit())))
        else:
    ...
    @overload
    def execute(self, command: Union[str, Sequence[Any]], *, as_process: Literal[False]=False, stdout_as_string: Literal[True]) -> Union[str, Tuple[int, str, str]]:
        ...

    @overload
    def execute(self, command: Union[str, Sequence[Any]], *, as_process: Literal[False]=False, stdout_as_string: Literal[False]=False) -> Union[bytes, Tuple[int, bytes, str]]:
        ...

    @overload
    def execute(self, command: Union[str, Sequence[Any]], *, with_extended_output: Literal[False], as_process: Literal[False], stdout_as_string: Literal[True]) -> str:
        ...

    @overload
    def execute(self, command: Union[str, Sequence[Any]], *, with_extended_output: Literal[False], as_process: Literal[False], stdout_as_string: Literal[False]) -> bytes:
        ...

    def execute(self, command: Union[str, Sequence[Any]], istream: Union[None, BinaryIO]=None, with_extended_output: bool=False, with_exceptions: bool=True, as_process: bool=False, output_stream: Union[None, BinaryIO]=None, stdout_as_string: bool=True, kill_after_timeout: Union[None, float]=None, with_stdout: bool=True, universal_newlines: bool=False, shell: Union[None, bool]=None, env: Union[None, Mapping[str, str]]=None, max_chunk_size: int=io.DEFAULT_BUFFER_SIZE, strip_newline_in_stdout: bool=True, **subprocess_kwargs: Any) -> Union[str, bytes, Tuple[int, Union[str, bytes], str], AutoInterrupt]:
        """Handle executing the command, and consume and return the returned
        information (stdout).
        ...
        _logger.debug('Popen(%s, cwd=%s, stdin=%s, shell=%s, universal_newlines=%s)', redacted_command, cwd, '<valid stream>' if istream else 'None', shell, universal_newlines)
        try:
            proc = safer_popen(command, env=env, cwd=cwd, bufsize=-1, stdin=istream or DEVNULL, stderr=PIPE, stdout=stdout_sink, shell=shell, universal_newlines=universal_newlines, **subprocess_kwargs)
            ...
            if output_stream is None:
                ...
                if kill_after_timeout is not None:
                    ...
                    if kill_check.is_set():
                        stderr_value = 'Timeout: the command "%s" did not complete in %d secs.' % (' '.join(redacted_command), kill_after_timeout)
        ...
        if self.GIT_PYTHON_TRACE == 'full':
            ...
            if stderr_value:
                _logger.info("%s -> %d; stdout: '%s'; stderr: '%s'", cmdstr, status, as_text(stdout_value), safe_decode(stderr_value))
            elif stdout_value:
    ...
    @overload
    def _call_process(self, method: str, istream: int, as_process: Literal[True], *args: Any, **kwargs: Any) -> 'Git.AutoInterrupt':
        ...
