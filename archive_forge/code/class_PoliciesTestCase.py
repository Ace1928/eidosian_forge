...
class PoliciesTestCase(base.V3ClientTestCase):
    ...
    def test_create_policy(self):
        policy_ref = {'blob': uuid.uuid4().hex, 'type': uuid.uuid4().hex}
    ...
    def test_update_policy(self):
        ...
        policy_ret = self.client.policies.update(policy.id, blob=new_blob, type=new_type)
        policy.ref.update({'blob': new_blob, 'type': new_type})
        self.check_policy(policy_ret, policy.ref)

    def test_delete_policy(self):
        policy = self.client.policies.create(blob=uuid.uuid4().hex, type=uuid.uuid4().hex)
        self.client.policies.delete(policy.id)
        self.assertRaises(http.NotFound, self.client.policies.get, policy.id)