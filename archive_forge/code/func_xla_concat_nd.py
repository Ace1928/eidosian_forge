...
def xla_concat_nd(inputs: List[_atypes.TensorFuzzingAnnotation[TV_XlaConcatND_T]], num_concats, paddings=[], name=None) -> _atypes.TensorFuzzingAnnotation[TV_XlaConcatND_T]:
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'XlaConcatND', name, inputs, 'num_concats', num_concats, 'paddings', paddings)
            return _result
            ...
            pass
        try:
            return xla_concat_nd_eager_fallback(inputs, num_concats=num_concats, paddings=paddings, name=name, ctx=_ctx)
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('XlaConcatND', inputs=inputs, num_concats=num_concats, paddings=paddings, name=name)
    _result = _outputs[:]
    if _execute.must_record_gradient():
        _attrs = ('T', _op._get_attr_type('T'), 'N', _op._get_attr_int('N'), 'num_concats', _op.get_attr('num_concats'), 'paddings', _op.get_attr('paddings'))
