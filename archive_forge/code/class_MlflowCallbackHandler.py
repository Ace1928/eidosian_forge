...
class MlflowCallbackHandler(BaseMetadataCallbackHandler, BaseCallbackHandler):
    ...
    def __init__(self, name: Optional[str]='langchainrun-%', experiment: Optional[str]='langchain', tags: Optional[Dict]=None, tracking_uri: Optional[str]=None, run_id: Optional[str]=None, artifacts_dir: str='') -> None:
        ...
        self.mlflg = MlflowLogger(tracking_uri=self.tracking_uri, experiment_name=self.experiment, run_name=self.name, run_tags=self.tags, run_id=self.run_id, artifacts_dir=self.artifacts_dir)
        ...
        try:
            ...
            try:
                ...
                logger.warning('Run `python -m spacy download en_core_web_sm` to download en_core_web_sm model for text visualization.')
        try:
    ...
    def on_retriever_end(self, documents: Sequence[Document], **kwargs: Any) -> Any:
        ...
        resp: Dict[str, Any] = {}
        retriever_documents = [{'page_content': doc.page_content, 'metadata': {k: str(v) if not isinstance(v, list) else ','.join((str(x) for x in v)) for k, v in doc.metadata.items()}} for doc in documents]
    ...
    def _create_session_analysis_df(self) -> Any:
        ...
        if 'name' in on_llm_start_records_df.columns:
            ...
            llm_input_columns.append('name')
        llm_input_prompts_df = on_llm_start_records_df[llm_input_columns].dropna(axis=1).rename({'step': 'prompt_step'}, axis=1)
        ...
        token_usage_columns = ['token_usage_total_tokens', 'token_usage_prompt_tokens', 'token_usage_completion_tokens']
        token_usage_columns = [x for x in token_usage_columns if x in on_llm_end_records_df.columns]
        llm_outputs_df = on_llm_end_records_df[['step', 'text'] + token_usage_columns + complexity_metrics_columns + visualizations_columns].dropna(axis=1).rename({'step': 'output_step', 'text': 'output'}, axis=1)
        session_analysis_df = pd.concat([llm_input_prompts_df, llm_outputs_df], axis=1)
        session_analysis_df['chat_html'] = session_analysis_df[['prompt', 'output']].apply(lambda row: construct_html_from_prompt_and_generation(row['prompt'], row['output']), axis=1)
