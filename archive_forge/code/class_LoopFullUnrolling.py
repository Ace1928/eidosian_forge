...
class LoopFullUnrolling(Transformation):
    ...
    def visit_For(self, node):
        ...
        if isinstance(node.iter, (ast.Tuple, ast.List)):
            ...
            if issmall:
                self.update = True
                return sum([unroll(elt, dc(node.body, i, elts_count)) for i, elt in enumerate(node.iter.elts)], [])
        ast.fix_missing_locations(node.iter)
        code = compile(ast.gast_to_ast(ast.Expression(node.iter)), '<loop unrolling>', 'eval')
        try:
        ...
        if issmall:
            try:
                new_node = sum([unroll(to_ast(elt), dc(node.body, i, values_count)) for i, elt in enumerate(values)], [])
                self.update = True
                return new_node
            except Exception:
                return node
