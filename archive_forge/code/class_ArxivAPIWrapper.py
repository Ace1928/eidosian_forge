...
class ArxivAPIWrapper(BaseModel):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        try:
            ...
            values['arxiv_exceptions'] = (arxiv.ArxivError, arxiv.UnexpectedEmptyPageError, arxiv.HTTPError)
    ...
    def lazy_load(self, query: str) -> Iterator[Document]:
        ...
        try:
            ...
            if self.is_arxiv_identifier(query):
                results = self.arxiv_search(id_list=query[:self.ARXIV_MAX_QUERY_LENGTH].split(), max_results=self.load_max_docs).results()
            else:
                results = self.arxiv_search(query[:self.ARXIV_MAX_QUERY_LENGTH], max_results=self.load_max_docs).results()
        except self.arxiv_exceptions as ex:
        ...
        for result in results:
            ...
            if self.load_all_available_meta:
                extra_metadata = {'entry_id': result.entry_id, 'published_first_time': str(result.published.date()), 'comment': result.comment, 'journal_ref': result.journal_ref, 'doi': result.doi, 'primary_category': result.primary_category, 'categories': result.categories, 'links': [link.href for link in result.links]}
            else:
                extra_metadata = {}
            metadata = {'Published': str(result.updated.date()), 'Title': result.title, 'Authors': ', '.join((a.name for a in result.authors)), 'Summary': result.summary, **extra_metadata}
