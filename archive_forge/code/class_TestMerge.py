...
class TestMerge(tests.TestCaseWithTransport):
    ...
    def example_branch(self, path='.'):
        ...
        self.build_tree_contents([(osutils.pathjoin(path, 'hello'), b'foo'), (osutils.pathjoin(path, 'goodbye'), b'baz')])
        tree.add('hello')
    ...
    def test_merge(self):
        ...
        self.run_bzr_error(['Show-base is not supported for this merge type'], 'merge ../b -r last:1..last:1 --merge-type weave --show-base', working_dir='a')
        a_tree.revert(backups=False)
        ...
        self.run_bzr('merge -r revno:%d:./..revno:%d:../b' % (ancestor, b.revno()), working_dir='a')
        self.assertEqual(a.get_parent_ids(), [a.branch.last_revision(), b.last_revision()])
        self.check_file_contents('a/goodbye', b'quux')
        a_tree.revert(backups=False)
    ...
    def test_merge_bundle(self):
        ...
        testament_a = Testament.from_revision(tree_a.branch.repository, tree_b.get_parent_ids()[0])
        testament_b = Testament.from_revision(tree_b.branch.repository, tree_b.get_parent_ids()[0])
