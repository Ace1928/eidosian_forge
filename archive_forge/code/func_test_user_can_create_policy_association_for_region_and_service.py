...
def test_user_can_create_policy_association_for_region_and_service(self):
    ...
    with self.test_client() as c:
        c.put('/v3/policies/%s/OS-ENDPOINT-POLICY/services/%s/regions/%s' % (policy['id'], service['id'], region['id']), headers=self.headers, expected_status_code=http.client.NO_CONTENT)