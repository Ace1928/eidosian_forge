...
class PublisherClient(object):
    ...
    def Publish(self, topic_resource, message=None, ordering_key=None, attributes=None, event_time=None):
    ...
    """
        if not message and (not attributes):
            raise topics.EmptyMessageException('You cannot send an empty message. You must specify either a MESSAGE, one or more ATTRIBUTE, or both.')
        topic_path = self._TopicResourceToPath(topic_resource)
        attributes = attributes or {}
        if event_time:
            attributes[message_transforms.PUBSUB_LITE_EVENT_TIME] = message_transforms.encode_attribute_event_time(event_time)
        try:
            return types.MessageMetadata.decode(self._client.publish(topic_path, http_encoding.Encode(message), ordering_key, **attributes).result())
        except Exception as e:
            raise topics.PublishOperationException('Publish operation failed with error: {error}'.format(error=e))