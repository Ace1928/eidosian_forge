...
class SearchUtils:
    ...
    NUMERIC_ATTRIBUTES = set(list(_BUILTIN_NUMERIC_ATTRIBUTES) + list(_ALTERNATE_NUMERIC_ATTRIBUTES))
    ...
    _IDENTIFIERS = [_METRIC_IDENTIFIER, _PARAM_IDENTIFIER, _TAG_IDENTIFIER, _ATTRIBUTE_IDENTIFIER, _DATASET_IDENTIFIER]
    _VALID_IDENTIFIERS = set(_IDENTIFIERS + list(_ALTERNATE_METRIC_IDENTIFIERS) + list(_ALTERNATE_PARAM_IDENTIFIERS) + list(_ALTERNATE_TAG_IDENTIFIERS) + list(_ALTERNATE_ATTRIBUTE_IDENTIFIERS) + list(_ALTERNATE_DATASET_IDENTIFIERS))
    ...
    VALID_ORDER_BY_KEYS_REGISTERED_MODELS = {ORDER_BY_KEY_TIMESTAMP, ORDER_BY_KEY_LAST_UPDATED_TIMESTAMP, ORDER_BY_KEY_MODEL_NAME}
    ...
    def _strip_quotes(cls, value, expect_quoted_value=False):
        ...
        if cls._is_quoted(value, "'") or cls._is_quoted(value, '"'):
            ...
            raise MlflowException('Parameter value is either not quoted or unidentified quote types used for string value %s. Use either single or double quotes.' % value, error_code=INVALID_PARAMETER_VALUE)
        else:
            return value
    ...
    def _get_identifier(cls, identifier, valid_attributes):
        try:
            ...
            raise MlflowException("Invalid identifier '%s'. Columns should be specified as 'attribute.<key>', 'metric.<key>', 'tag.<key>', 'dataset.<key>', or 'param.'." % identifier, error_code=INVALID_PARAMETER_VALUE)
        ...
        if identifier == cls._ATTRIBUTE_IDENTIFIER and key not in valid_attributes:
            ...
            raise MlflowException.invalid_parameter_value(f"Invalid dataset key '{key}' specified. Valid keys are '{cls.DATASET_ATTRIBUTES}'")
    ...
    def _get_value(cls, identifier_type, key, token):
        if identifier_type == cls._METRIC_IDENTIFIER:
            if token.ttype not in cls.NUMERIC_VALUE_TYPES:
                raise MlflowException(f'Expected numeric value type for metric. Found {token.value}', error_code=INVALID_PARAMETER_VALUE)
            return token.value
            ...
            if key in cls.NUMERIC_ATTRIBUTES:
                if token.ttype not in cls.NUMERIC_VALUE_TYPES:
                    raise MlflowException(f'Expected numeric value type for numeric attribute: {key}. Found {token.value}', error_code=INVALID_PARAMETER_VALUE)
                return token.value
                ...
                raise MlflowException(f'Expected a quoted string value for attributes. Got value {token.value}', error_code=INVALID_PARAMETER_VALUE)
        elif identifier_type == cls._DATASET_IDENTIFIER:
            if key in cls.DATASET_ATTRIBUTES and (token.ttype in cls.STRING_VALUE_TYPES or isinstance(token, Identifier)):
                return cls._strip_quotes(token.value, expect_quoted_value=True)
            elif isinstance(token, Parenthesis):
                if key not in ('name', 'digest', 'context'):
                    raise MlflowException("Only the dataset 'name' and 'digest' supports comparison with a list of quoted string values.", error_code=INVALID_PARAMETER_VALUE)
                return cls._parse_run_ids(token)
            else:
                raise MlflowException(f'Expected a quoted string value for dataset attributes. Got value {token.value}', error_code=INVALID_PARAMETER_VALUE)
        else:
            raise MlflowException(f'Invalid identifier type. Expected one of {[cls._METRIC_IDENTIFIER, cls._PARAM_IDENTIFIER]}.')
    ...
    def _process_statement(cls, statement):
        ...
        if len(invalids) > 0:
            ...
            raise MlflowException(f'Invalid clause(s) in filter string: {invalid_clauses}', error_code=INVALID_PARAMETER_VALUE)
    ...
    def is_metric(cls, key_type, comparator):
        if key_type == cls._METRIC_IDENTIFIER:
            if comparator not in cls.VALID_METRIC_COMPARATORS:
                raise MlflowException(f"Invalid comparator '{comparator}' not one of '{cls.VALID_METRIC_COMPARATORS}", error_code=INVALID_PARAMETER_VALUE)
            return True
        return False
    ...
    def is_param(cls, key_type, comparator):
        if key_type == cls._PARAM_IDENTIFIER:
            if comparator not in cls.VALID_PARAM_COMPARATORS:
                raise MlflowException(f"Invalid comparator '{comparator}' not one of '{cls.VALID_PARAM_COMPARATORS}'", error_code=INVALID_PARAMETER_VALUE)
            return True
        return False
    ...
    def is_tag(cls, key_type, comparator):
        if key_type == cls._TAG_IDENTIFIER:
            if comparator not in cls.VALID_TAG_COMPARATORS:
                raise MlflowException(f"Invalid comparator '{comparator}' not one of '{cls.VALID_TAG_COMPARATORS}")
            return True
        return False
    ...
    def is_dataset(cls, key_type, comparator):
        if key_type == cls._DATASET_IDENTIFIER:
            if comparator not in cls.VALID_DATASET_COMPARATORS:
                raise MlflowException(f"Invalid comparator '{comparator}' not one of '{cls.VALID_DATASET_COMPARATORS}")
            return True
        return False
    ...
    def parse_order_by_for_search_registered_models(cls, order_by):
        ...
        if token_value not in cls.VALID_ORDER_BY_KEYS_REGISTERED_MODELS:
            raise MlflowException(f"Invalid order by key '{token_value}' specified. Valid keys are '{cls.RECOMMENDED_ORDER_BY_KEYS_REGISTERED_MODELS}'", error_code=INVALID_PARAMETER_VALUE)
        return (token_value, is_ascending)
    ...
    def sort(cls, runs, order_by_list):
        ...
        for order_by_clause in reversed(order_by_list):
            ...
            runs = sorted(runs, key=lambda run: cls._get_value_for_sort(run, key_type, key, ascending), reverse=not ascending)
        return runs
