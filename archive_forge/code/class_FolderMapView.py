...
class FolderMapView(typing.Mapping[str, Union['FolderMapView', DataPath]]):
    ...
    def find(self, data_name: str, missing_default: Optional[ParamArg]=ParamArg.DEFAULT, **params: Union[typing.Iterable[ParamVal], ParamArg]) -> List[Tuple[Description, DataPath]]:
        ...
        curr: List[Tuple[Description, Union[FolderMapView, DataPath]]] = [(Description(()), self[data_name])]
            ...
            while curr:
                ...
                for next_param in next_params:
                    try:
                        ...
                        continue
                    todo.append((Description((*curr_description.items(), (param_name, next_param))), fmap_next))
            if len(todo) == 0:
                param_arg_repr = repr([param_arg]) if isinstance(param_arg, (str, ParamArg)) else repr(list(param_arg))
                raise ValueError(f'{param_name} value(s) {param_arg_repr} are not available. Available values are: {list(curr_level)}')
            curr, todo = (todo, curr)
        done.extend(curr)
        return done

    def __getitem__(self, __key: Union[str, Literal[ParamArg.DEFAULT]]) -> Union['FolderMapView', DataPath]:
