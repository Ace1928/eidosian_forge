...
class batch_transform:
    """Class for registering a tape transform that takes a tape, and outputs
    ...
    **Example**
    ...
    def __new__(cls, *args, **kwargs):
        if os.environ.get('SPHINX_BUILD') == '1':
            warnings.warn("Batch transformations have been disabled, as a Sphinx build has been detected via SPHINX_BUILD='1'. If this is not the case, please set the environment variable SPHINX_BUILD='0'.", UserWarning)
    ...
    def __init__(self, transform_fn, expand_fn=None, differentiable=True):
        if not callable(transform_fn):
            raise ValueError(f'The batch transform function to register, {transform_fn}, does not appear to be a valid Python function or callable.')
        warnings.warn('Use of `batch_transform` to create a custom transform is deprecated. Instead switch to using the new qml.transform function. Follow the instructions here for further details: https://docs.pennylane.ai/en/stable/code/qml_transforms.html#custom-transforms.', qml.PennyLaneDeprecationWarning)
    ...
    def default_qnode_wrapper(self, qnode, targs, tkwargs):
        ...
        def _wrapper(*args, **kwargs):
            ...
            res = qml.execute(tapes, device=qnode.device, gradient_fn=gradient_fn, interface=interface, max_diff=max_diff, override_shots=shots, gradient_kwargs=gradient_kwargs, **execute_kwargs)
