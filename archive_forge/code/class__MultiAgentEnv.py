...
class _MultiAgentEnv(gym.Env):
    ...
    def __init__(self, env_spec: EnvSpec, instances: Optional[List[MinecraftInstance]]=None, is_fault_tolerant: bool=True, verbose: bool=False, _xml_mutator_to_be_deprecated: Optional[Callable]=None, refresh_instances_every: Optional[int]=None):
        """
    ...
    def _process_action(self, actor_name, action_in) -> str:
        ...
        while isinstance(bottom_env_spec, EnvWrapper):
            bottom_env_spec = bottom_env_spec.env_to_wrap
        act_space = bottom_env_spec.action_space[actor_name] if self.task.agent_count > 1 else bottom_env_spec.action_space
        action_str = []
    ...
    def step(self, actions) -> Tuple[Dict[str, Dict[str, Any]], Dict[str, float], bool, Dict[str, Dict[str, Any]]]:
        if not self.done:
            ...
            for role, (actor_name, instance) in enumerate(zip(self.task.agent_names, self.instances)):
                try:
                    if not self.has_finished[actor_name]:
                        ...
                        step_message = '<StepClient' + str(STEP_OPTIONS) + '>' + malmo_command + '</StepClient' + str(STEP_OPTIONS) + ' >'
                    ...
                    self.done = True
                    logger.error(f"Failed to take a step (error {e}). Terminating episode and sending random observation, be aware. To account for this failure case in your code check to see if `'error' in info` where info is the info dictionary returned by the step function.")
                    logger.error(traceback.format_exc())
                    return ({agent: self.observation_space.sample() for agent in actions}, {agent: 0 for agent in actions}, self.done, {agent: {'error': 'Connection timed out!'} for agent in actions})
            ...
            try:
                ...
                self.done = True
                logger.error("Failed to take a step (timeout or error). Terminating episode and sending random observation, be aware. To account for this failure case in your code check to see if `'error' in info` where info is the info dictionary returned by the step function.")
    ...
    def reset(self) -> Any:
        ...
        try:
            ...
            self._send_mission(self.instances[0], agent_xmls[0], self._get_token(0, ep_uid))
            if self.task.agent_count > 1:
                ...
                for slave_instance, slave_xml, role in list(zip(self.instances, agent_xmls, range(1, self.task.agent_count + 1)))[1:]:
    ...
    def _setup_slave_master_connection_info(self, slave_xml: etree.Element, mc_server_ip: str, mc_server_port: str):
        e = etree.Element(NS + 'MinecraftServerConnection', attrib={'address': str(mc_server_ip), 'port': str(mc_server_port)})
