...
def priority_queue(shapes, component_types=[], capacity: int=-1, container: str='', shared_name: str='', name=None) -> _atypes.TensorFuzzingAnnotation[_atypes.String]:
  ...
  """
    ...
    if not isinstance(shapes, (list, tuple)):
        raise TypeError("Expected list for 'shapes' argument to 'priority_queue' Op, not %r." % shapes)
    shapes = [_execute.make_shape(_s, 'shapes') for _s in shapes]
    if component_types is None:
        component_types = []
    if not isinstance(component_types, (list, tuple)):
        raise TypeError("Expected list for 'component_types' argument to 'priority_queue' Op, not %r." % component_types)
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('PriorityQueue', shapes=shapes, component_types=component_types, capacity=capacity, container=container, shared_name=shared_name, name=name)
    _result = _outputs[:]
    if _execute.must_record_gradient():
        ...
        _execute.record_gradient('PriorityQueue', _inputs_flat, _attrs, _result)
    _result, = _result
    return _result