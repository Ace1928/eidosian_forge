...
class SymmetricKeyTestCase(base.KeyTestCase):
    ...
    def _create_key(self):
        return sym_key.SymmetricKey(self.algorithm, self.bit_length, self.encoded, self.name, self.created, consumers=self.consumers)
    ...
    def test_is_only_metadata(self):
        k = sym_key.SymmetricKey(self.algorithm, self.bit_length, None, self.name, self.created, consumers=self.consumers)
    ...
    def test_get_created_none(self):
        created = None
        key = sym_key.SymmetricKey(self.algorithm, self.bit_length, self.encoded, self.name, created, consumers=self.consumers)
    ...
    def test___eq__(self):
        ...
        other_key = sym_key.SymmetricKey(self.algorithm, self.bit_length, self.encoded, consumers=self.consumers)
    ...
    def test___ne___algorithm(self):
        other_key = sym_key.SymmetricKey('DES', self.bit_length, self.encoded, self.name, consumers=self.consumers)
        self.assertTrue(self.key != other_key)

    def test___ne___bit_length(self):
        other_key = sym_key.SymmetricKey(self.algorithm, self.bit_length * 2, self.encoded, self.name, consumers=self.consumers)
        self.assertTrue(self.key != other_key)

    def test___ne___encoded(self):
        different_encoded = self.encoded * 2
        other_key = sym_key.SymmetricKey(self.algorithm, self.bit_length, different_encoded, self.name, consumers=self.consumers)
        self.assertTrue(self.key != other_key)

    def test___ne___consumers(self):
        different_consumers = [{'service': 'other_service', 'resource_type': 'other_type', 'resource_id': 'other_id'}]
        other_key = sym_key.SymmetricKey(self.algorithm, self.bit_length, self.encoded, self.name, consumers=different_consumers)
