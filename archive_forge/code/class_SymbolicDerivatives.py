...
class SymbolicDerivatives(unittest.TestCase):
    ...
    def test_errors(self):
        m = ConcreteModel()
        m.x = Var()
        self.assertRaisesRegex(ValueError, 'Must specify exactly one of wrt and wrt_list', differentiate, m.x, wrt=m.x, wrt_list=[m.x])
        ...
        class bogus(object):
            ...
            def __init__(self):
                ...
                self.func = type(self)
        self.assertRaisesRegex(DeveloperError, "sympy expression .*bogus'> not found in the operator map", sympy2pyomo_expression, bogus(), obj_map, normalize_whitespace=True)