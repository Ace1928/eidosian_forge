...
class QCircuitImage:
    ...
    def __init__(self, qubits, clbits, nodes, scale, style=None, reverse_bits=False, plot_barriers=True, initial_state=False, cregbundle=None, with_layout=False, circuit=None):
        ...
        for node in itertools.chain.from_iterable(self._nodes):
            if node.cargs and node.op.name != 'measure':
                if cregbundle:
                    warn('Cregbundle set to False since an instruction needs to refer to individual classical wire', RuntimeWarning, 2)
    ...
    def latex(self):
        ...
        header_1 = '\\documentclass[border=2px]{standalone}' + '\n'
        ...
        if self._global_phase:
            output.write('{$\\mathrm{%s} \\mathrm{%s}$}' % ('global\\,phase:\\,', pi_check(self._global_phase, output='latex')))
    ...
    def _build_measure(self, node, col):
        ...
        if self._cregbundle:
            ...
            self._latex[wire2][col] = '\\dstick{_{_{\\hspace{%sem}%s}}} \\cw \\ar @{<=} [-%s,0]' % (cond_offset, idx_str, str(wire2 - wire1))
        else:
            wire2 = self._wire_map[node.cargs[0]]
    ...
    def _add_condition(self, op, wire_list, col):
        ...
        if cond_is_bit or self._cregbundle:
            cwire = self._wire_map[cond_reg] if self._cregbundle else self._wire_map[op.condition[0]]
            gap = cwire - max(wire_list)
            control = '\\control' if op.condition[1] else '\\controlo'
            self._latex[cwire][col] = f'{control}' + ' \\cw^(%s){^{\\mathtt{%s}}} \\cwx[-%s]' % (meas_offset, label, str(gap))
        else:
            cond_wires = []
            cond_bits = []
            for wire in self._wire_map:
            ...
            self._latex[cond_wires[-1]][col] = (f'{control}' + ' \\cw^(%s){^{\\mathtt{%s}}} \\cwx[-%s]') % (meas_offset, label, str(gap))
