...
class SQLRecordManager(RecordManager):
    ...
    def __init__(self, namespace: str, *, engine: Optional[Union[Engine, AsyncEngine]]=None, db_url: Union[None, str, URL]=None, engine_kwargs: Optional[Dict[str, Any]]=None, async_mode: bool=False) -> None:
    ...
    def update(self, keys: Sequence[str], *, group_ids: Optional[Sequence[Optional[str]]]=None, time_at_least: Optional[float]=None) -> None:
        """Upsert records into the SQLite database."""
        if group_ids is None:
            group_ids = [None] * len(keys)
        if len(keys) != len(group_ids):
            raise ValueError(f'Number of keys ({len(keys)}) does not match number of group_ids ({len(group_ids)})')
        update_time = self.get_time()
        if time_at_least and update_time < time_at_least:
            raise AssertionError(f'Time sync issue: {update_time} < {time_at_least}')
        records_to_upsert = [{'key': key, 'namespace': self.namespace, 'updated_at': update_time, 'group_id': group_id} for key, group_id in zip(keys, group_ids)]
        with self._make_session() as session:
            if self.dialect == 'sqlite':
                ...
                stmt = sqlite_insert_stmt.on_conflict_do_update([UpsertionRecord.key, UpsertionRecord.namespace], set_=dict(updated_at=sqlite_insert_stmt.excluded.updated_at, group_id=sqlite_insert_stmt.excluded.group_id))
            elif self.dialect == 'postgresql':
                ...
                stmt = pg_insert_stmt.on_conflict_do_update('uix_key_namespace', set_=dict(updated_at=pg_insert_stmt.excluded.updated_at, group_id=pg_insert_stmt.excluded.group_id))
            else:
                raise NotImplementedError(f'Unsupported dialect {self.dialect}')
            session.execute(stmt)
            session.commit()

    async def aupdate(self, keys: Sequence[str], *, group_ids: Optional[Sequence[Optional[str]]]=None, time_at_least: Optional[float]=None) -> None:
        """Upsert records into the SQLite database."""
        if group_ids is None:
            group_ids = [None] * len(keys)
        if len(keys) != len(group_ids):
    ...
    def exists(self, keys: Sequence[str]) -> List[bool]:
        ...
        session: Session
        with self._make_session() as session:
            filtered_query: Query = session.query(UpsertionRecord.key).filter(and_(UpsertionRecord.key.in_(keys), UpsertionRecord.namespace == self.namespace))
    ...
    async def aexists(self, keys: Sequence[str]) -> List[bool]:
        """Check if the given keys exist in the SQLite database."""
        async with self._amake_session() as session:
            records = (await session.execute(select(UpsertionRecord.key).where(and_(UpsertionRecord.key.in_(keys), UpsertionRecord.namespace == self.namespace)))).scalars().all()
    ...
    async def alist_keys(self, *, before: Optional[float]=None, after: Optional[float]=None, group_ids: Optional[Sequence[str]]=None, limit: Optional[int]=None) -> List[str]:
        """List records in the SQLite database based on the provided date range."""
        session: AsyncSession
        async with self._amake_session() as session:
            query: Query = select(UpsertionRecord.key).filter(UpsertionRecord.namespace == self.namespace)
            if after:
                query = query.filter(UpsertionRecord.updated_at > after)
            if before:
                query = query.filter(UpsertionRecord.updated_at < before)
    ...
    def delete_keys(self, keys: Sequence[str]) -> None:
        ...
        session: Session
        with self._make_session() as session:
            filtered_query: Query = session.query(UpsertionRecord).filter(and_(UpsertionRecord.key.in_(keys), UpsertionRecord.namespace == self.namespace))
    ...
    async def adelete_keys(self, keys: Sequence[str]) -> None:
        """Delete records from the SQLite database."""
        async with self._amake_session() as session:
            await session.execute(delete(UpsertionRecord).where(and_(UpsertionRecord.key.in_(keys), UpsertionRecord.namespace == self.namespace)))
            await session.commit()