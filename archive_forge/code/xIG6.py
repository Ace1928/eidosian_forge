...
class GameGUI:
    ...
    async def draw_elements(self):
        ...
        await self.draw_banner()
        await self.draw_game_stats()
        logging.debug("Banner and game stats drawn.")

        if (
            self.curr_menu.state != "GA" or self.controller.model_loaded
        ):  # Path Ai or trained GA
            fruit = await self.controller.get_fruit_pos()
            snake = self.controller.snake

            await self.draw_fruit(fruit)
            await self.draw_snake(snake)
            await self.draw_score()
            logging.debug("Fruit, snake, and score drawn.")

            if not self.controller.model_loaded:
                await self.draw_path()  # only path Ai has a path
                logging.debug("Path drawn.")

        else:  # training a GA model
            await self.draw_all_snakes_GA()
            logging.debug("All GA snakes drawn.")

    async def draw_game_stats(self):
        """
        Asynchronously draws the game statistics and instructions on the screen based on the current game state,
        ensuring thread safety and maximum concurrency.
        """
        async with asyncio.Lock():
            if self.curr_menu.state != "GA":  # path Ai algo
                ...
                instruction: str = (
                    "Space to hide all snakes, W to speed up, Q to go back"
                )
                curr_gen: str = str(await self.controller.curr_gen())
                best_score: str = str(await self.controller.best_GA_score())

                stats_gen: str = (
                    f"Generation: {curr_gen}/{await self.controller.algo.generation}"
                )
                stats_score: str = f"Best score: {best_score}"
                stats_hidden_node: str = (
                    f"Hidden nodes {await self.controller.algo.population.hidden_node}"
                )
                logging.debug(
    ...
    async def draw_all_snakes_GA(self):
        """
        Asynchronously draws all snakes during the GA training phase, unless the view path flag is toggled,
        ensuring thread safety and maximum concurrency.
        """
        async with asyncio.Lock():
            if not self.view_path:  # have all snakes visible by default
                ...
                for snake in self.controller.snakes:  # for each snake in list
                    ...
                    # fruit of each snake
                    await self.draw_fruit(snake.get_fruit())
                    logging.debug(f"Snake and its fruit drawn for GA training: {snake}")

    async def draw_path(self):
        """
        Asynchronously draws the path of the AI snake if the view path flag is toggled and an algorithm is active,
        ensuring thread safety and maximum concurrency.
        """
        async with asyncio.Lock():
    ...
    async def draw_snake_head(self, snake):
        """
        Asynchronously draws the head of the snake on the display, ensuring thread safety and maximum concurrency.
        """
        head = snake.body[0]
        await self.draw_rect(head, color=SNAKE_HEAD_COLOR)
        logging.debug(f"Snake head drawn asynchronously at {head}.")

    async def draw_snake_body(self, body):
        """
        Asynchronously draws a body segment of the snake on the display, ensuring thread safety and maximum concurrency.
        """
        await self.draw_rect(body, color=SNAKE_COLOR, border=True)
        logging.debug(f"Snake body segment drawn asynchronously at {body}.")

    async def draw_rect(self, element, color, border=False):
        """
        Asynchronously draws a rectangle on the display representing either a snake body segment or the snake head,
        ensuring thread safety and maximum concurrency.
        """
        ...
        async with asyncio.Lock():
            ...
            if border:
                ...
                logging.debug(f"Rect drawn with border asynchronously at {x}, {y}.")

    async def draw_snake(self, snake):
        """
        Asynchronously draws the entire snake on the display, including its head and body segments,
        ensuring thread safety and maximum concurrency.
        """
        await self.draw_snake_head(snake)  # draw head asynchronously
        logging.debug("Snake head drawn asynchronously.")

        for body in snake.body[1:]:
            await self.draw_snake_body(body)  # draw body asynchronously
            logging.debug("Snake body drawn asynchronously.")

    async def draw_fruit(self, fruit):
        """
        Asynchronously draws the fruit on the display, which the snake aims to eat, ensuring thread safety and maximum concurrency.
        """
        ...
        async with asyncio.Lock():
            ...
            logging.debug(f"Fruit drawn asynchronously at {x}, {y}.")

    async def draw_banner(self):
        """
        Asynchronously draws the banner at the top of the display, ensuring thread safety and maximum concurrency.
        """
        banner: pygame.Rect = pygame.Rect(0, 0, self.SIZE, BANNER_HEIGHT * CELL_SIZE)
        async with asyncio.Lock():
            pygame.draw.rect(self.display, BANNER_COLOR, banner)
            logging.debug("Banner drawn asynchronously.")

    async def draw_score(self):
        """
        Asynchronously draws the current score of the game on the display, ensuring thread safety and maximum concurrency.
        """
        score_text: str = "Score: " + str(self.controller.get_score())
        score_x: int = self.SIZE - (CELL_SIZE + 2 * len(score_text))
        score_y: int = CELL_SIZE
        await self.draw_text(score_text, 20, score_x, score_y, WINDOW_COLOR)
        logging.debug("Score drawn asynchronously.")

    async def game_over(self):
        """
        Handles the game over state asynchronously, allowing the user to save the model or restart the game,
        ensuring thread safety and maximum concurrency.
        """
        again: bool = False
        ...
        while not again:
            ...
            if self.curr_menu.state == "GA" and not self.controller.model_loaded:
                best_score: int = await self.controller.best_GA_score()
                best_gen: int = await self.controller.best_GA_gen()
    ...
    async def is_quit(self, event):
        """
        Asynchronously checks if the quit event is triggered by the user, ensuring thread safety.
        """
    ...
    async def event_handler(self):
        """
        Asynchronously handles all events during the game loop, including key presses and custom events,
        ensuring thread safety and maximum concurrency.
        """
        events = pygame.event.get()
        tasks = [self.process_event(event) for event in events]
        await asyncio.gather(*tasks)
        logging.debug("All events processed concurrently.")

    async def process_event(self, event):
        """
        Asynchronously processes individual events, ensuring each event is handled in a thread-safe manner.
        """
        if await self.is_quit(event):
            print("Bye :)")
            pygame.quit()
            ...
            await self.handle_keydown_event(event)

    async def handle_user_event(self, event):
        """
        Handles custom user events related to game logic, particularly for AI and model interactions.
        """
        if self.load_model:  # user load model
            await self.controller.load_model()
            self.load_model = False
            logging.debug("Model loaded asynchronously.")

        await self.controller.ai_play(self.curr_menu.state)  # play

        if self.controller.end:  # Only path ai and trained model
            self.playing = False
            await self.game_over()  # show game over stats
            logging.debug("Game over initiated asynchronously.")

    async def handle_keydown_event(self, event):
        """
        Handles keydown events to control game settings and actions.
        """
        if event.key == pygame.K_RETURN:  # on Enter
            ...
            self.BACK = True
            await self.controller.reset()
            logging.debug("Q key pressed and controller reset asynchronously.")
            ...
            pygame.time.set_timer(self.SCREEN_UPDATE, self.speed)
            logging.debug(f"Speed adjusted to {self.speed} asynchronously.")

    async def reset_keys(self):
        """
        Asynchronously resets all key flags to False after handling them in the game loop,
        ensuring thread safety and concurrency.
        """
        self.UPKEY, self.DOWNKEY, self.START, self.BACK = False, False, False, False
        logging.debug("All keys reset asynchronously.")

    async def draw_text(self, text, size, x, y, color=WINDOW_COLOR):
        """
        Asynchronously draws text on the display at the specified location and with the specified properties,
        ensuring thread safety and concurrency.
        """
        ...
        logging.debug(f"Text '{text}' drawn at {x}, {y} asynchronously.")
