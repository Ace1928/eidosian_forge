...
class DimensionDataNodeDriver(NodeDriver):
    ...
    def create_node(self, name, image, auth, ex_network_domain=None, ex_primary_nic_private_ipv4=None, ex_primary_nic_vlan=None, ex_primary_nic_network_adapter=None, ex_additional_nics=None, ex_description=None, ex_disks=None, ex_cpu_specification=None, ex_memory_gb=None, ex_is_started=True, ex_primary_dns=None, ex_secondary_dns=None, ex_ipv4_gateway=None, ex_microsoft_time_zone=None, **kwargs):
        ...
        if ex_network_domain is None and 'ex_network' not in kwargs:
            raise ValueError('You must provide either ex_network_domain for MCP2 or ex_network for legacy MCP1')
        if ex_network_domain is not None and 'ex_network' in kwargs:
            raise ValueError('You can only supply either ex_network_domain for MCP2 or ex_network for legacy MCP1')
        if not isinstance(ex_is_started, bool):
            ex_is_started = True
        if 'ex_network' in kwargs:
            new_node = self._create_node_mcp1(name=name, image=image, auth=auth, ex_network=kwargs.get('ex_network'), ex_description=ex_description, ex_memory_gb=ex_memory_gb, ex_cpu_specification=ex_cpu_specification, ex_is_started=ex_is_started, ex_primary_ipv4=ex_primary_nic_private_ipv4, ex_disks=ex_disks, ex_additional_nics_vlan=kwargs.get('ex_additional_nics_vlan'), ex_additional_nics_ipv4=kwargs.get('ex_additional_nics_ipv4'), ex_primary_dns=ex_primary_dns, ex_secondary_dns=ex_secondary_dns)
        else:
    ...
    def destroy_node(self, node):
        ...
        body = self.connection.request_with_orgId_api_2('server/deleteServer', method='POST', data=ET.tostring(request_elm)).object
    ...
    def reboot_node(self, node):
        ...
        body = self.connection.request_with_orgId_api_2('server/rebootServer', method='POST', data=ET.tostring(request_elm)).object
        response_code = findtext(body, 'responseCode', TYPES_URN)
        return response_code in ['IN_PROGRESS', 'OK']

    def list_nodes(self, ex_location=None, ex_name=None, ex_ipv6=None, ex_ipv4=None, ex_vlan=None, ex_image=None, ex_deployed=None, ex_started=None, ex_state=None, ex_network=None, ex_network_domain=None):
        """
        ...
        """
        node_list = []
        for nodes in self.ex_list_nodes_paginated(location=ex_location, name=ex_name, ipv6=ex_ipv6, ipv4=ex_ipv4, vlan=ex_vlan, image=ex_image, deployed=ex_deployed, started=ex_started, state=ex_state, network=ex_network, network_domain=ex_network_domain):
    ...
    def list_images(self, location=None):
        ...
        return self._to_images(self.connection.request_with_orgId_api_2('image/osImage', params=params).object)
    ...
    def list_locations(self, ex_id=None):
        ...
        if ex_id is not None:
            params['id'] = ex_id
        return self._to_locations(self.connection.request_with_orgId_api_2('infrastructure/datacenter', params=params).object)
    ...
    def list_networks(self, location=None):
        ...
        return self._to_networks(self.connection.request_with_orgId_api_1('networkWithLocation%s' % url_ext).object)

    def import_image(self, ovf_package_name, name, cluster_id=None, datacenter_id=None, description=None, is_guest_os_customization=None, tagkey_name_value_dictionaries=None):
        """
        Import image
        ...
        if LooseVersion(self.connection.active_api_version) < LooseVersion('2.4'):
            raise Exception('import image is feature is NOT supported in  api version earlier than 2.4')
            ...
            raise ValueError('Cannot accept both cluster_id and datacenter_id. Please provide either one')
        else:
            ...
            if is_guest_os_customization is not None:
                ET.SubElement(import_image_elem, 'urn:guestOsCustomization').text = is_guest_os_customization
        ...
        response = self.connection.request_with_orgId_api_2('image/importImage', method='POST', data=ET.tostring(import_image_elem)).object
    ...
    def start_node(self, node):
        ...
        body = self.connection.request_with_orgId_api_2('server/startServer', method='POST', data=ET.tostring(request_elm)).object
        response_code = findtext(body, 'responseCode', TYPES_URN)
        return response_code in ['IN_PROGRESS', 'OK']
    ...
    def stop_node(self, node):
        ...
        body = self.connection.request_with_orgId_api_2('server/shutdownServer', method='POST', data=ET.tostring(request_elm)).object
        response_code = findtext(body, 'responseCode', TYPES_URN)
        return response_code in ['IN_PROGRESS', 'OK']
    ...
    def ex_reset(self, node):
        ...
        body = self.connection.request_with_orgId_api_2('server/resetServer', method='POST', data=ET.tostring(request_elm)).object
        response_code = findtext(body, 'responseCode', TYPES_URN)
        return response_code in ['IN_PROGRESS', 'OK']
    ...
    def ex_update_node(self, node, name=None, description=None, cpu_count=None, ram_mb=None):
        ...
        body = self.connection.request_with_orgId_api_1('server/%s' % node.id, method='POST', data=urlencode(data, True)).object
    ...
    def ex_create_anti_affinity_rule(self, node_list):
        ...
        result = self.connection.request_with_orgId_api_1('antiAffinityRule', method='POST', data=ET.tostring(anti_affinity_xml_request)).object
    ...
    def ex_delete_anti_affinity_rule(self, anti_affinity_rule):
        ...
        result = self.connection.request_with_orgId_api_1('antiAffinityRule/%s?delete' % rule_id, method='GET').object
        response_code = findtext(result, 'result', GENERAL_NS)
        return response_code in ['IN_PROGRESS', 'SUCCESS']
    ...
    def ex_create_network(self, location, name, description=None):
        ...
        self.connection.request_with_orgId_api_1('networkWithLocation', method='POST', data=ET.tostring(create_node))
    ...
    def ex_delete_network(self, network):
        ...
        :rtype: ``bool``
        """
        response = self.connection.request_with_orgId_api_1('network/%s?delete' % network.id, method='GET').object
    ...
    def ex_create_network_domain(self, location, name, service_plan, description=None):
        ...
        return DimensionDataNetworkDomain(id=network_domain_id, name=name, description=description, location=location, status=NodeState.RUNNING, plan=service_plan)
    ...
    def ex_create_vlan(self, network_domain, name, private_ipv4_base_address, description=None, private_ipv4_prefix_size=24):
        """
        ...
        response = self.connection.request_with_orgId_api_2('network/deployVlan', method='POST', data=ET.tostring(create_node)).object
        vlan_id = None
        for info in findall(response, 'info', TYPES_URN):
    ...
    def ex_get_public_ip_block(self, block_id):
        locations = self.list_locations()
        block = self.connection.request_with_orgId_api_2('network/publicIpBlock/%s' % block_id).object
    ...
    def ex_list_firewall_rules(self, network_domain, page_size=50, page_number=1):
        ...
        params['networkDomainId'] = self._network_domain_to_network_domain_id(network_domain)
        response = self.connection.request_with_orgId_api_2('network/firewallRule', params=params).object
        return self._to_firewall_rules(response, network_domain)

    def ex_create_firewall_rule(self, network_domain, rule, position, position_relative_to_rule=None):
        """
        ...
        ET.SubElement(create_node, 'networkDomainId').text = self._network_domain_to_network_domain_id(network_domain)
        ...
        response = self.connection.request_with_orgId_api_2('network/createFirewallRule', method='POST', data=ET.tostring(create_node)).object
        rule_id = None
        for info in findall(response, 'info', TYPES_URN):
    ...
    def ex_delete_firewall_rule(self, rule):
        ...
        update_node.set('id', rule.id)
        result = self.connection.request_with_orgId_api_2('network/deleteFirewallRule', method='POST', data=ET.tostring(update_node)).object
        response_code = findtext(result, 'responseCode', TYPES_URN)
        return response_code in ['IN_PROGRESS', 'OK']
    ...
    def ex_create_nat_rule(self, network_domain, internal_ip, external_ip):
        ...
        for info in findall(result, 'info', TYPES_URN):
            if info.get('name') == 'natRuleId':
                rule_id = info.get('value')
        return DimensionDataNatRule(id=rule_id, network_domain=network_domain, internal_ip=internal_ip, external_ip=external_ip, status=NodeState.RUNNING)
    ...
    def ex_change_storage_speed(self, node, disk_id, speed):
        ...
        result = self.connection.request_with_orgId_api_1('server/{}/disk/{}/changeSpeed'.format(node.id, disk_id), method='POST', data=ET.tostring(create_node)).object
        response_code = findtext(result, 'result', GENERAL_NS)
        return response_code in ['IN_PROGRESS', 'SUCCESS']
    ...
    def ex_change_storage_size(self, node, disk_id, size):
        ...
        result = self.connection.request_with_orgId_api_1('server/{}/disk/{}/changeSize'.format(node.id, disk_id), method='POST', data=ET.tostring(create_node)).object
        response_code = findtext(result, 'result', GENERAL_NS)
        return response_code in ['IN_PROGRESS', 'SUCCESS']
    ...
    def ex_clone_node_to_image(self, node, image_name, image_description=None, cluster_id=None, is_guest_Os_Customization=None, tag_key_id=None, tag_value=None):
        ...
        if LooseVersion(self.connection.active_api_version) < LooseVersion('2.4'):
            response = self.connection.request_with_orgId_api_1('server/{}?clone={}&desc={}'.format(node_id, image_name, image_description)).object
        else:
    ...
    def ex_list_tag_keys(self, id=None, name=None, value_required=None, display_on_report=None):
        ...
        paged_result = self.connection.paginated_request_with_orgId_api_2('tag/tagKey', method='GET', params=params)
        tag_keys = []
        for result in paged_result:
    ...
    def ex_list_tags(self, asset_id=None, asset_type=None, location=None, tag_key_name=None, tag_key_id=None, value=None, value_required=None, display_on_report=None):
        """
        ...
        if display_on_report is not None:
            params['displayOnReport'] = str(display_on_report).lower()
        paged_result = self.connection.paginated_request_with_orgId_api_2('tag/tag', method='GET', params=params)
        tags = []
        for result in paged_result:
    ...
    def ex_summary_usage_report(self, start_date, end_date):
        ...
        :rtype: ``list`` of ``list``
        """
        result = self.connection.raw_request_with_orgId_api_1('report/usage?startDate={}&endDate={}'.format(start_date, end_date))
    ...
    def ex_detailed_usage_report(self, start_date, end_date):
        ...
        :rtype: ``list`` of ``list``
        """
        result = self.connection.raw_request_with_orgId_api_1('report/usageDetailed?startDate={}&endDate={}'.format(start_date, end_date))
        return self._format_csv(result.response)
    ...
    def ex_software_usage_report(self, start_date, end_date):
        ...
        :rtype: ``list`` of ``list``
        """
        result = self.connection.raw_request_with_orgId_api_1('report/usageSoftwareUnits?startDate={}&endDate={}'.format(start_date, end_date))
        return self._format_csv(result.response)
    ...
    def ex_edit_ip_address_list(self, ex_ip_address_list, description, ip_address_collection, child_ip_address_lists=None):
        """
    ...
    def _to_tag(self, element):
        ...
        return DimensionDataTag(asset_type=findtext(element, 'assetType', TYPES_URN), asset_id=findtext(element, 'assetId', TYPES_URN), asset_name=findtext(element, 'assetName', TYPES_URN), datacenter=findtext(element, 'datacenterId', TYPES_URN), key=tag_key, value=findtext(element, 'value', TYPES_URN))
    ...
    def _to_tag_key(self, element, from_tag_api=False):
        ...
        return DimensionDataTagKey(id=id, name=name, description=findtext(element, 'description', TYPES_URN), value_required=self._str2bool(findtext(element, 'valueRequired', TYPES_URN)), display_on_report=self._str2bool(findtext(element, 'displayOnReport', TYPES_URN)))
    ...
    def _to_nat_rule(self, element, network_domain):
        return DimensionDataNatRule(id=element.get('id'), network_domain=network_domain, internal_ip=findtext(element, 'internalIp', TYPES_URN), external_ip=findtext(element, 'externalIp', TYPES_URN), status=findtext(element, 'state', TYPES_URN))
    ...
    def _to_firewall_address(self, element):
        ...
        if address_list is None:
            return DimensionDataFirewallAddress(any_ip=ip.get('address') == 'ANY', ip_address=ip.get('address'), ip_prefix_size=ip.get('prefixSize'), port_begin=port.get('begin') if port is not None else None, port_end=port.get('end') if port is not None else None, port_list_id=port_list.get('id', None) if port_list is not None else None, address_list_id=address_list.get('id') if address_list is not None else None)
        else:
            return DimensionDataFirewallAddress(any_ip=False, ip_address=None, ip_prefix_size=None, port_begin=None, port_end=None, port_list_id=port_list.get('id', None) if port_list is not None else None, address_list_id=address_list.get('id') if address_list is not None else None)
    ...
    def _to_ip_block(self, element, locations):
        location_id = element.get('datacenterId')
        location = list(filter(lambda x: x.id == location_id, locations))[0]
        return DimensionDataPublicIpBlock(id=element.get('id'), network_domain=self.ex_get_network_domain(findtext(element, 'networkDomainId', TYPES_URN)), base_ip=findtext(element, 'baseIp', TYPES_URN), size=findtext(element, 'size', TYPES_URN), location=location, status=findtext(element, 'state', TYPES_URN))
    ...
    def _to_network(self, element, locations):
        ...
        location = list(filter(lambda x: x.id == location_id, locations))[0]
        return DimensionDataNetwork(id=findtext(element, 'id', NETWORK_NS), name=findtext(element, 'name', NETWORK_NS), description=findtext(element, 'description', NETWORK_NS), location=location, private_net=findtext(element, 'privateNet', NETWORK_NS), multicast=multicast, status=status)
    ...
    def _to_network_domain(self, element, locations):
        ...
        return DimensionDataNetworkDomain(id=element.get('id'), name=findtext(element, 'name', TYPES_URN), description=findtext(element, 'description', TYPES_URN), plan=plan_type, location=location, status=findtext(element, 'state', TYPES_URN))
    ...
    def _to_location(self, element):
        loc = NodeLocation(id=element.get('id'), name=findtext(element, 'displayName', TYPES_URN), country=findtext(element, 'country', TYPES_URN), driver=self)
        return loc

    def _to_cpu_spec(self, element):
        return DimensionDataServerCpuSpecification(cpu_count=int(element.get('count')), cores_per_socket=int(element.get('coresPerSocket')), performance=element.get('speed'))
    ...
    def _to_vmware_tools(self, element):
        ...
        return DimensionDataServerVMWareTools(status=status, version_status=version_status, api_version=api_version)
    ...
    def _to_disk(self, element):
        return DimensionDataServerDisk(id=element.get('id'), scsi_id=int(element.get('scsiId')), size_gb=int(element.get('sizeGb')), speed=element.get('speed'), state=element.get('state'))
    ...
    def _to_node(self, element):
        ...
        if LooseVersion(self.connection.active_api_version) < LooseVersion('2.4'):
            ...
            if vmtools_elm is not None:
                ...
                vmware_tools = DimensionDataServerVMWareTools(status=None, version_status=None, api_version=None)
        ...
        private_ip = element.find(fixxpath('networkInfo/primaryNic', TYPES_URN)).get('privateIpv4') if has_network_info else element.find(fixxpath('nic', TYPES_URN)).get('privateIpv4')
        extra['ipv6'] = element.find(fixxpath('networkInfo/primaryNic', TYPES_URN)).get('ipv6') if has_network_info else element.find(fixxpath('nic', TYPES_URN)).get('ipv6')
        n = Node(id=element.get('id'), name=findtext(element, 'name', TYPES_URN), state=node_state, public_ips=[public_ip] if public_ip is not None else [], private_ips=[private_ip] if private_ip is not None else [], size=self.list_sizes()[0], image=NodeImage(extra['sourceImageId'], extra['OS_displayName'], self.connection.driver), driver=self.connection.driver, extra=extra)
        return n
    ...
    def _to_ip_address(self, element):
        return DimensionDataIpAddress(begin=element.get('begin'), end=element.get('end'), prefix_size=element.get('prefixSize'))
    ...
    def _to_port_list(self, element):
        ...
        return DimensionDataPortList(id=element.get('id'), name=findtext(element, 'name', TYPES_URN), description=findtext(element, 'description', TYPES_URN), port_collection=ports, child_portlist_list=child_portlist_list, state=findtext(element, 'state', TYPES_URN), create_time=findtext(element, 'createTime', TYPES_URN))
