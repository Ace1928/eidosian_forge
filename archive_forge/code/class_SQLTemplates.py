...
class SQLTemplates(abc.ABC):
    ...
    def __init__(self, name: Optional[str]=None, base_path: Union[str, Path]=None, file_suffix: Optional[str]=None, settings: Optional['AppSettings']=None, module_name: Optional[str]=None):
        """
    ...
    def j2(self) -> 'jinja2.Environment':
        ...
        """
        if self._j2 is None:
            self._j2 = self.settings.ctx.get_j2_ctx(path=self.base_path or 'assets/sql', name=self.name, enable_async=False, comment_start_string='/*', comment_end_string='*/')
        return self._j2
    ...
    def __call__(self, *parts: str, enable_async: bool=False, **kwargs) -> str:
        """
        Renders the template
        """
        if enable_async:
