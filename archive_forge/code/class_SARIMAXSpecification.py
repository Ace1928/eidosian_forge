...
class SARIMAXSpecification:
    ...
    def k_params(self):
        ...
        k_params = self.k_exog_params + self.k_ar_params + self.k_ma_params + self.k_seasonal_ar_params + self.k_seasonal_ma_params
    ...
    def param_names(self):
        ...
        names = self.exog_names + self.ar_names + self.ma_names + self.seasonal_ar_names + self.seasonal_ma_names
        if not self.concentrate_scale:
            names.append('sigma2')
        return names
    ...
    def construct_trend_data(self, nobs, offset=1):
        if self.trend_order is None:
            ...
            trend_data = prepare_trend_data(self.trend_poly, int(np.sum(self.trend_poly)), nobs, offset)
        return trend_data
