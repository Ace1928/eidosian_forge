...
class ProjectForkManager(CreateMixin, ListMixin, RESTManager):
    ...
    _list_filters = ('archived', 'visibility', 'order_by', 'sort', 'search', 'simple', 'owned', 'membership', 'starred', 'statistics', 'with_custom_attributes', 'with_issues_enabled', 'with_merge_requests_enabled')
    _create_attrs = RequiredOptional(optional=('namespace',))

    def create(self, data: Optional[Dict[str, Any]]=None, **kwargs: Any) -> ProjectFork:
