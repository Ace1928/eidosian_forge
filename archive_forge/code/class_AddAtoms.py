...
class AddAtoms:
    ...
    def __init__(self, gui):
        ...
        spinners = [ui.SpinBox(0.0, -1000.0, 1000.0, 0.1, rounding=2, width=3) for __ in range(3)]
        win.add([_('Coordinates:')] + spinners)
        self.spinners = spinners
        win.add(_('Coordinates are relative to the center of the selection, if any, else absolute.'))
    ...
    def get_atoms(self):
        ...
        if os.path.exists(val):
            return self.readfile(val)
        ui.showerror(_('Cannot add atoms'), _('{} is neither atom, molecule, nor file').format(val))
        return None
    ...
    def add(self):
        ...
        if len(atoms) and self.picky.value:
            ...
            mindist = dists.min()
            if mindist < 0.5:
                ui.showerror(_('Bad positions'), _('Atom would be less than 0.5 Å from an existing atom.  To override, uncheck the check positions option.'))
                return
