    ...
    def __init__(self):
        ...
        for parser, suffix, available, library, parser_args in sources:
            ...
            if not os.path.exists(cfg_file):
                continue
            if not available:
                logger.warning('Default configuration file (%s) cannot be loaded; %s is not available' % (cfg_file, library))
                continue
            fp = open(cfg_file)
            try:
                data = parser.load(fp, **parser_args)
            except:
                logger.error("Error parsing the user's default configuration file\n\t%s." % (cfg_file,))
