...
def recognize_whisper(self, audio_data, model='base', show_dict=False, load_options=None, language=None, translate=False, **transcribe_options):
    ...
    result = self.whisper_model[model].transcribe(audio_array, language=language, task='translate' if translate else None, fp16=torch.cuda.is_available(), **transcribe_options)
    if show_dict:
        return result
    else:
        return result['text']