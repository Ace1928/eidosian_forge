...
class InstancesTest(common.HeatTestCase):
    ...
    def _setup_test_stack(self, stack_name):
        ...
        tmpl = template.Template(t, env=environment.Environment({'KeyName': 'test'}))
    ...
    def _mock_get_image_id_fail(self, image_id, exp):
        self.patchobject(glance.GlanceClientPlugin, 'find_image_by_name_or_id', side_effect=exp)

    def _get_test_template(self, stack_name, image_id=None, volumes=False):
        tmpl, stack = self._setup_test_stack(stack_name)
        tmpl.t['Resources']['WebServer']['Properties']['ImageId'] = image_id or 'CentOS 5.2'
        tmpl.t['Resources']['WebServer']['Properties']['InstanceType'] = '256 MB Server'
        if not volumes:
            tmpl.t['Resources']['WebServer']['Properties']['Volumes'] = []
        return (tmpl, stack)

    def _setup_test_instance(self, return_server, name, image_id=None, volumes=False):
        stack_name = '%s_s' % name
        tmpl, self.stack = self._get_test_template(stack_name, image_id, volumes=volumes)
        self.instance_props = tmpl.t['Resources']['WebServer']['Properties']
        resource_defns = tmpl.resource_definitions(self.stack)
        instance = instances.Instance(name, resource_defns['WebServer'], self.stack)
    ...
    def test_build_block_device_mapping(self):
        return_server = self.fc.servers.list()[1]
        instance = self._create_test_instance(return_server, 'test_build_bdm')
        self.assertIsNone(instance._build_block_device_mapping([]))
        self.assertIsNone(instance._build_block_device_mapping(None))
        self.assertEqual({'vdb': '1234:snap:', 'vdc': '5678:snap::False'}, instance._build_block_device_mapping([{'DeviceName': 'vdb', 'Ebs': {'SnapshotId': '1234'}}, {'DeviceName': 'vdc', 'Ebs': {'SnapshotId': '5678', 'DeleteOnTermination': False}}]))
        self.assertEqual({'vdb': '1234:snap:1', 'vdc': '5678:snap:2:True'}, instance._build_block_device_mapping([{'DeviceName': 'vdb', 'Ebs': {'SnapshotId': '1234', 'VolumeSize': '1'}}, {'DeviceName': 'vdc', 'Ebs': {'SnapshotId': '5678', 'VolumeSize': '2', 'DeleteOnTermination': True}}]))
    ...
    def test_validate_Volumes_property(self):
        ...
        instance = instances.Instance('validate_volumes', resource_defns['WebServer'], stack)
        self.stub_ImageConstraint_validate()
        self.stub_SnapshotConstraint_validate()
        self.stub_FlavorConstraint_validate()
        self.stub_KeypairConstraint_validate()
        ex = exception.EntityNotFound(entity='Volume', name='1234')
        mock_get_vol = self.patchobject(cinder.CinderClientPlugin, 'get_volume', side_effect=ex)
        exc = self.assertRaises(exception.StackValidationFailed, instance.validate)
        self.assertIn("WebServer.Properties.Volumes[0].VolumeId: Error validating value '1234': The Volume (1234) could not be found.", str(exc))
    ...
    def test_validate_BlockDeviceMappings_without_Ebs_property(self):
        ...
        wsp['Volumes'] = []
        resource_defns = tmpl.resource_definitions(stack)
        instance = instances.Instance('validate_without_Ebs', resource_defns['WebServer'], stack)
        self._mock_get_image_id_success('F17-x86_64-gold', 1)
        self.patchobject(nova.NovaClientPlugin, 'client', return_value=self.fc)
        exc = self.assertRaises(exception.StackValidationFailed, instance.validate)
        self.assertIn('Ebs is missing, this is required', str(exc))

    def test_validate_BlockDeviceMappings_without_SnapshotId_property(self):
        stack_name = 'without_SnapshotId'
        tmpl, stack = self._setup_test_stack(stack_name)
        bdm = [{'DeviceName': 'vdb', 'Ebs': {'VolumeSize': '1'}}]
        wsp = tmpl.t['Resources']['WebServer']['Properties']
        wsp['BlockDeviceMappings'] = bdm
        wsp['Volumes'] = []
        resource_defns = tmpl.resource_definitions(stack)
        instance = instances.Instance('validate_without_SnapshotId', resource_defns['WebServer'], stack)
        self._mock_get_image_id_success('F17-x86_64-gold', 1)
        self.patchobject(nova.NovaClientPlugin, 'client', return_value=self.fc)
        exc = self.assertRaises(exception.StackValidationFailed, instance.validate)
        self.assertIn('SnapshotId is missing, this is required', str(exc))

    def test_validate_BlockDeviceMappings_without_DeviceName_property(self):
        stack_name = 'without_DeviceName'
        tmpl, stack = self._setup_test_stack(stack_name)
        bdm = [{'Ebs': {'SnapshotId': '1234', 'VolumeSize': '1'}}]
        wsp = tmpl.t['Resources']['WebServer']['Properties']
        wsp['BlockDeviceMappings'] = bdm
        resource_defns = tmpl.resource_definitions(stack)
        instance = instances.Instance('validate_without_DeviceName', resource_defns['WebServer'], stack)
        self.stub_VolumeConstraint_validate()
        self.stub_SnapshotConstraint_validate()
        self.stub_ImageConstraint_validate()
        self.stub_KeypairConstraint_validate()
        self.stub_FlavorConstraint_validate()
        exc = self.assertRaises(exception.StackValidationFailed, instance.validate)
        excepted_error = 'Property error: Resources.WebServer.Properties.BlockDeviceMappings[0]: Property DeviceName not assigned'
        self.assertIn(excepted_error, str(exc))

    def test_instance_create_with_image_id(self):
        return_server = self.fc.servers.list()[1]
        instance = self._setup_test_instance(return_server, 'in_create_imgid', image_id='1')
        scheduler.TaskRunner(instance.create)()
        self.assertGreater(instance.id, 0)
        expected_ip = return_server.networks['public'][0]
        expected_az = getattr(return_server, 'OS-EXT-AZ:availability_zone')
    ...
    def test_instance_create_image_id_err(self):
        ...
        resource_defns = tmpl.resource_definitions(stack)
        instance = instances.Instance('instance_create_image_err', resource_defns['WebServer'], stack)
        self._mock_get_image_id_fail('1', glance.client_exception.EntityMatchNotFound(entity='image', args='1'))
        self.stub_VolumeConstraint_validate()
        self.stub_FlavorConstraint_validate()
        self.stub_KeypairConstraint_validate()
        self.stub_SnapshotConstraint_validate()
        create = scheduler.TaskRunner(instance.create)
        error = self.assertRaises(exception.ResourceFailure, create)
        self.assertEqual("StackValidationFailed: resources.instance_create_image_err: Property error: WebServer.Properties.ImageId: Error validating value '1': No image matching 1.", str(error))
    ...
    def test_handle_check_raises_exception_if_instance_not_active(self):
        ...
        self.patchobject(nova.NovaClientPlugin, '_check_active', return_value=False)
    ...
    def test_instance_update_network_interfaces(self):
        ...
        self._stub_glance_for_update()
        old_interfaces = [{'NetworkInterfaceId': 'ea29f957-cd35-4364-98fb-57ce9732c10d', 'DeviceIndex': '2'}, {'NetworkInterfaceId': 'd1e9c73c-04fe-4e9e-983c-d5ef94cd1a46', 'DeviceIndex': '1'}]
        ...
        return_server.interface_attach.assert_called_once_with('34b752ec-14de-416a-8722-9531015e04a5', None, None)
    ...
    def test_instance_update_network_interfaces_old_include_new(self):
        ...
        self._stub_glance_for_update()
        old_interfaces = [{'NetworkInterfaceId': 'ea29f957-cd35-4364-98fb-57ce9732c10d', 'DeviceIndex': '2'}, {'NetworkInterfaceId': 'd1e9c73c-04fe-4e9e-983c-d5ef94cd1a46', 'DeviceIndex': '1'}]
        new_interfaces = [{'NetworkInterfaceId': 'ea29f957-cd35-4364-98fb-57ce9732c10d', 'DeviceIndex': '2'}]
        before_props = self.instance_props.copy()
        before_props['NetworkInterfaces'] = old_interfaces
        update_props = self.instance_props.copy()
        update_props['NetworkInterfaces'] = new_interfaces
    ...
    def test_instance_update_network_interfaces_new_include_old(self):
        ...
        new_interfaces = [{'NetworkInterfaceId': 'ea29f957-cd35-4364-98fb-57ce9732c10d', 'DeviceIndex': '2'}, {'NetworkInterfaceId': 'd1e9c73c-04fe-4e9e-983c-d5ef94cd1a46', 'DeviceIndex': '1'}]
        before_props = self.instance_props.copy()
        before_props['NetworkInterfaces'] = old_interfaces
        update_props = self.instance_props.copy()
        update_props['NetworkInterfaces'] = new_interfaces
        ...
        self.assertEqual((instance.UPDATE, instance.COMPLETE), instance.state)
        self.fc.servers.get.assert_called_with('1234')
        return_server.interface_attach.assert_called_once_with('d1e9c73c-04fe-4e9e-983c-d5ef94cd1a46', None, None)
    ...
    def test_instance_update_network_interfaces_empty_new_with_subnet(self):
        ...
        self._test_instance_update_with_subnet(stack_name, new_interfaces=[])
    ...
    def test_instance_status_build(self):
        ...
        instance = self._setup_test_instance(return_server, 'in_sts_build')
        instance.resource_id = '1234'
    ...
    def _test_instance_status_suspend(self, name, state=('CREATE', 'COMPLETE')):
        return_server = self.fc.servers.list()[1]
    ...
    def test_instance_suspend_in_create_complete(self):
        self._test_instance_status_suspend(name='test_suspend_in_create_complete')

    def test_instance_suspend_in_suspend_failed(self):
        self._test_instance_status_suspend(name='test_suspend_in_suspend_failed', state=('SUSPEND', 'FAILED'))

    def test_instance_suspend_in_suspend_complete(self):
        self._test_instance_status_suspend(name='test_suspend_in_suspend_complete', state=('SUSPEND', 'COMPLETE'))
    ...
    def test_instance_resume_in_suspend_complete(self):
        self._test_instance_status_resume(name='test_resume_in_suspend_complete')

    def test_instance_resume_in_resume_failed(self):
        self._test_instance_status_resume(name='test_resume_in_resume_failed', state=('RESUME', 'FAILED'))

    def test_instance_resume_in_resume_complete(self):
        self._test_instance_status_resume(name='test_resume_in_resume_complete', state=('RESUME', 'COMPLETE'))
    ...
    def test_build_nics(self):
        return_server = self.fc.servers.list()[1]
        instance = self._create_test_instance(return_server, 'build_nics')
        self.assertIsNone(instance._build_nics([]))
        self.assertIsNone(instance._build_nics(None))
        self.assertEqual([{'port-id': 'id3'}, {'port-id': 'id1'}, {'port-id': 'id2'}], instance._build_nics(['id3', 'id1', 'id2']))
        self.assertEqual([{'port-id': 'id1'}, {'port-id': 'id2'}, {'port-id': 'id3'}], instance._build_nics([{'NetworkInterfaceId': 'id3', 'DeviceIndex': '3'}, {'NetworkInterfaceId': 'id1', 'DeviceIndex': '1'}, {'NetworkInterfaceId': 'id2', 'DeviceIndex': 2}]))
        self.assertEqual([{'port-id': 'id1'}, {'port-id': 'id2'}, {'port-id': 'id3'}, {'port-id': 'id4'}, {'port-id': 'id5'}], instance._build_nics([{'NetworkInterfaceId': 'id3', 'DeviceIndex': '3'}, {'NetworkInterfaceId': 'id1', 'DeviceIndex': '1'}, {'NetworkInterfaceId': 'id2', 'DeviceIndex': 2}, 'id4', 'id5']))
    ...
    def test_instance_without_ip_address(self):
        ...
        instance = self._create_test_instance(return_server, 'wo_ipaddr')
    ...
    def test_default_instance_user(self):
        ...
        self.patchobject(nova.NovaClientPlugin, 'build_userdata', return_value=None)
        scheduler.TaskRunner(instance.create)()
        nova.NovaClientPlugin.build_userdata.assert_called_once_with(metadata, 'wordpress', 'ec2-user')
