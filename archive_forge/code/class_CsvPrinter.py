...
class CsvPrinter(resource_printer_base.ResourcePrinter):
  ...
  """
    ...
    def __init__(self, *args, **kwargs):
        super(CsvPrinter, self).__init__(*args, by_columns=True, non_empty_projection_required=True, **kwargs)
    ...
    """
        if not self._heading_printed:
            ...
            if 'no-heading' not in self.attributes:
                ...
                if labels:
                    self._out.write(self._separator.join([self._QuoteField(label) for label in self.RemoveHiddenColumns(labels)]) + self._terminator)
        line = []
        for col in self.RemoveHiddenColumns(record):
            if col is None:
                val = ''
            elif isinstance(col, dict):
                val = self._delimiter.join([self._QuoteField('{0}={1}'.format(encoding.Decode(k), encoding.Decode(v))) for k, v in sorted(six.iteritems(col))])
            elif isinstance(col, list):
                val = self._delimiter.join([self._QuoteField(encoding.Decode(x)) if x else '' for x in col])
