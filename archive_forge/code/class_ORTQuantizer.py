...
class ORTQuantizer(OptimumQuantizer):
    ...
    def __init__(self, onnx_model_path: Path, config: Optional['PretrainedConfig']=None):
        ...
        if self.config is None:
            try:
                ...
                LOGGER.warning(f'Could not load the config for {self.onnx_model_path} automatically, this might make the quantized model harder to use because it will not be able to be loaded by an ORTModel without having to specify the configuration explicitly.')
        self._calibrator = None

    @classmethod
    def from_pretrained(cls, model_or_path: Union['ORTModel', str, Path], file_name: Optional[str]=None) -> 'ORTQuantizer':
        """
    ...
    def fit(self, dataset: Dataset, calibration_config: CalibrationConfig, onnx_augmented_model_name: Union[str, Path]='augmented_model.onnx', operators_to_quantize: Optional[List[str]]=None, batch_size: int=1, use_external_data_format: bool=False, use_gpu: bool=False, force_symmetric_range: bool=False) -> Dict[str, Tuple[float, float]]:
        ...
        """
        LOGGER.info(f'Using static quantization schema (dataset: {calibration_config.dataset_name}, method: {calibration_config.method})')
        self.partial_fit(dataset, calibration_config, onnx_augmented_model_name, operators_to_quantize, batch_size, use_external_data_format, use_gpu, force_symmetric_range)
        return self.compute_ranges()

    def partial_fit(self, dataset: Dataset, calibration_config: CalibrationConfig, onnx_augmented_model_name: Union[str, Path]='augmented_model.onnx', operators_to_quantize: Optional[List[str]]=None, batch_size: int=1, use_external_data_format: bool=False, use_gpu: bool=False, force_symmetric_range: bool=False):
        """
        ...
        if calibration_config.method is not None:
            ...
            self._calibrator = calibration_config.create_calibrator(onnx_model_path=self.onnx_model_path.as_posix(), use_external_data_format=use_external_data_format, augmented_model_name=onnx_augmented_model_name, operators_to_quantize=operators_to_quantize, force_symmetric_range=force_symmetric_range)
        if use_gpu:
    ...
    def compute_ranges(self) -> Dict[str, Tuple[float, float]]:
        ...
        if self._calibrator is None:
            raise ValueError('Calibrator is None, please call `partial_fit` or `fit` method at least ones to compute ranges.')
    ...
    def quantize(self, quantization_config: QuantizationConfig, save_dir: Union[str, Path], file_suffix: Optional[str]='quantized', calibration_tensors_range: Optional[Dict[str, Tuple[float, float]]]=None, use_external_data_format: bool=False, preprocessor: Optional[QuantizationPreprocessor]=None) -> Path:
        """
        ...
        if quantization_config.is_static and calibration_tensors_range is None:
            raise ValueError('Requested static quantization in the QuantizationConfig, but no calibration ranges were provided. Please run calibration first using the quantizer fit method, or use dynamic quantization.')
        if not quantization_config.is_static:
            if quantization_config.mode != QuantizationMode.IntegerOps:
                LOGGER.warning(f'ONNX Runtime dynamic quantization mode should be QuantizationMode.IntegerOps (got: {quantization_config.mode}).')
            if quantization_config.activations_dtype != QuantType.QUInt8:
                LOGGER.warning(f'ONNX Runtime dynamic quantization activations data type should be QuantType.QUInt8 (got: {quantization_config.activations_dtype}).')
        LOGGER.info(f'Creating {('static' if quantization_config.is_static else 'dynamic')} quantizer: {quantization_config}')
