...
import pandas as pd
def load_csv(filename, cell_axis='row', delimiter=',', gene_names=True, cell_names=True, sparse=False, chunksize=10000, **kwargs):
    """Load a csv file.
    ...
    **kwargs : optional arguments for `pd.read_csv`.
    ...
    if cell_axis not in ['row', 'column', 'col']:
        raise ValueError("cell_axis {} not recognized. Expected 'row' or 'column'".format(cell_axis))
    ...
    data = read_fun(filename, delimiter=delimiter, header=header, index_col=index_col, **kwargs)
