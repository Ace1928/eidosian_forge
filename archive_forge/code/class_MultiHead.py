  ...
  """
    ...
    def __init__(self, heads, head_weights=None):
        ...
        if head_weights:
            if len(head_weights) != len(heads):
                raise ValueError('heads and head_weights must have the same size. Given len(heads): {}. Given len(head_weights): {}.'.format(len(heads), len(head_weights)))
    ...
    """
        with ops.name_scope(self.name, 'multi_head'):
            ...
            all_estimator_spec = []
            for head in self._heads:
                all_estimator_spec.append(head.create_estimator_spec(features=features, mode=mode, logits=logits_dict[head.name], labels=labels[head.name] if labels else None, train_op_fn=_no_op_train_fn))
            predictions = self.predictions(logits)
            if mode == ModeKeys.PREDICT:
                export_outputs = self._merge_predict_export_outputs(all_estimator_spec)
                return model_fn.EstimatorSpec(mode=ModeKeys.PREDICT, predictions=predictions, export_outputs=export_outputs)
            ...
            if mode == ModeKeys.EVAL:
                ...
                return model_fn.EstimatorSpec(mode=ModeKeys.EVAL, predictions=predictions, loss=loss, eval_metric_ops=updated_metrics)
            if mode == ModeKeys.TRAIN:
                train_op = base_head.create_estimator_spec_train_op(head_name=self.name, optimizer=optimizer, train_op_fn=train_op_fn, update_ops=update_ops, trainable_variables=trainable_variables, regularized_training_loss=loss, loss_reduction=self.loss_reduction)
                ...
                return model_fn.EstimatorSpec(mode=ModeKeys.TRAIN, loss=loss, train_op=train_op, predictions=predictions, eval_metric_ops=eval_metrics)
    ...
    """
        export_outputs = {base_head.DEFAULT_SERVING_KEY: _default_export_output(all_estimator_spec[0].export_outputs, self._heads[0].name)}
        merged_predict_outputs = {}
        for head, spec in zip(self._heads, all_estimator_spec):
            for k, v in six.iteritems(spec.export_outputs):
                key = head.name if k == base_head.DEFAULT_SERVING_KEY else '{}/{}'.format(head.name, k)
                export_outputs[key] = v
                if k == base_head.PREDICT_SERVING_KEY and isinstance(v, export_output.PredictOutput):
                    for kp, vp in six.iteritems(v.outputs):
                        merged_predict_outputs['{}/{}'.format(head.name, kp)] = vp
        export_outputs[base_head.PREDICT_SERVING_KEY] = export_output.PredictOutput(merged_predict_outputs)
