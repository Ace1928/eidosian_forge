...
class SymmetrizedStructure(Structure):
    ...
    def __init__(self, structure: Structure, spacegroup: SpacegroupOperations, equivalent_positions: Sequence[int], wyckoff_letters: Sequence[str]) -> None:
        """
        Args:
        ...
        super().__init__(structure.lattice, [site.species for site in structure], structure.frac_coords, site_properties=structure.site_properties, properties=structure.properties, labels=structure.labels)
    ...
    def copy(self):
        """Copy of structure."""
        return SymmetrizedStructure(self, spacegroup=self.spacegroup, equivalent_positions=self.site_labels, wyckoff_letters=self.wyckoff_letters)
    ...
    def __str__(self) -> str:
        outs = ['SymmetrizedStructure', f'Full Formula ({self.formula})', f'Reduced Formula: {self.reduced_formula}', f'Spacegroup: {self.spacegroup.int_symbol} ({self.spacegroup.int_number})', f'abc   : {' '.join((f'{val:>10.6f}' for val in self.lattice.abc))}', f'angles: {' '.join((f'{val:>10.6f}' for val in self.lattice.angles))}']
        if self._charge:
    ...
    def as_dict(self):
        ...
        return {'structure': structure.as_dict(), 'spacegroup': self.spacegroup, 'equivalent_positions': self.site_labels, 'wyckoff_letters': self.wyckoff_letters}
    ...
    def from_dict(cls, dct: dict) -> Self:
        ...
        """
        return cls(Structure.from_dict(dct['structure']), spacegroup=dct['spacegroup'], equivalent_positions=dct['equivalent_positions'], wyckoff_letters=dct['wyckoff_letters'])