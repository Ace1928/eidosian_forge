...
class HdkProtocolColumn(ProtocolColumn):
    ...
    def dtype(self) -> Tuple[DTypeKind, int, str, str]:
        ...
        elif pandas.api.types.is_datetime64_dtype(dtype) or isinstance(dtype, pandas.CategoricalDtype):
            return self._dtype_from_pyarrow(self._arrow_dtype)
        elif pandas.api.types.is_string_dtype(dtype):
            return (DTypeKind.STRING, 8, pandas_dtype_to_arrow_c(dtype), Endianness.NATIVE)
        else:
    ...
    def describe_categorical(self) -> CategoricalDescription:
        ...
        col = col.chunks[0]
        cat_frame = HdkOnNativeDataframe.from_arrow(pa.Table.from_pydict({next(iter(self._col.column_names())): col.dictionary}))
        from .dataframe import HdkProtocolDataframe
        return {'is_ordered': ordered, 'is_dictionary': True, 'categories': HdkProtocolColumn(HdkProtocolDataframe(cat_frame, self._col._nan_as_null, self._col._allow_copy))}
    ...
    def _get_data_buffer(self, arr: pa.Array) -> Tuple[HdkProtocolBuffer, Tuple[DTypeKind, int, str, str]]:
        """
        ...
        buff_size = self._get_buffer_size(bit_width=arrow_type[1]) if self.dtype[0] != DTypeKind.STRING else None
        return (HdkProtocolBuffer(arr.buffers()[-1], buff_size), arrow_type)

    def _get_validity_buffer(self, arr: pa.Array) -> Optional[Tuple[HdkProtocolBuffer, Tuple[DTypeKind, int, str, str]]]:
        """
        ...
        return (HdkProtocolBuffer(validity_buffer, data_size), (DTypeKind.BOOL, 1, ArrowCTypes.BOOL, Endianness.NATIVE))

    def _get_offsets_buffer(self, arr: pa.Array) -> Optional[Tuple[HdkProtocolBuffer, Tuple[DTypeKind, int, str, str]]]:
        """
        ...
        return (HdkProtocolBuffer(offset_buff, self._get_buffer_size(bit_width=dtype[1], is_offset_buffer=True)), dtype)
    ...
    def _combine_chunks(self):
        ...
        self._col = type(self._col)(self._col._df.from_arrow(contiguous_at), self._col._nan_as_null, self._col._allow_copy)