...
class CondorPlugin(SGELikeBatchManagerBase):
    ...
    def _is_pending(self, taskid):
        cmd = CommandLine('condor_q', resource_monitor=False, terminal_output='allatonce')
    ...
    def _submit_batchtask(self, scriptfile, node):
        cmd = CommandLine('condor_qsub', environ=dict(os.environ), resource_monitor=False, terminal_output='allatonce')
        ...
        while True:
            try:
                ...
                if tries < self._max_tries:
                    ...
                    iflogger.setLevel(oldlevel)
                    raise RuntimeError('\n'.join(('Could not submit condor cluster for node %s' % node._id, str(e))))
            else:
                break
        iflogger.setLevel(oldlevel)
