...
class XpressPersistent(PersistentSolver, XpressDirect):
    ...
    def _add_column(self, var, obj_coef, constraints, coefficients):
        ...
        lb, ub = self._xpress_lb_ub_from_var(var)
        self._solver_model.addcols(objx=[obj_coef], mstart=[0, len(coefficients)], mrwind=constraints, dmatval=coefficients, bdl=[lb], bdu=[ub], names=[varname], types=[vartype])
        xpress_var = self._solver_model.getVariable(index=self._solver_model.getIndexFromName(type=2, name=varname))
