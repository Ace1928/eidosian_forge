...
class _AxesBase(martist.Artist):
    ...
    def __init__(self, fig, *args, facecolor=None, frameon=True, sharex=None, sharey=None, label='', xscale=None, yscale=None, box_aspect=None, **kwargs):
        """
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        if 'cla' in cls.__dict__:
            _api.warn_deprecated('3.6', pending=True, message=f'Overriding `Axes.cla` in {cls.__qualname__} is pending deprecation in %(since)s and will be fully deprecated in favor of `Axes.clear` in the future. Please report this to the {cls.__module__!r} author.')
    ...
    @property
    def artists(self):
        return self.ArtistList(self, 'artists', invalid_types=(mcoll.Collection, mimage.AxesImage, mlines.Line2D, mpatches.Patch, mtable.Table, mtext.Text))
    ...
    def apply_aspect(self, position=None):
        ...
        if shared_x and shared_y:
            raise RuntimeError("set_aspect(..., adjustable='datalim') or axis('equal') are not allowed when both axes are shared.  Try set_aspect(..., adjustable='box').")
        if shared_y:
            adjust_y = False
        else:
        ...
        if isinstance(arg, (str, bool)):
            ...
            if arg is not None:
                if len(arg) != 2 * len(self._axis_names):
                    raise TypeError('The first argument to axis() must be an iterable of the form [{}]'.format(', '.join((f'{name}min, {name}max' for name in self._axis_names))))
    ...
    def autoscale_view(self, tight=None, scalex=True, scaley=True):
        ...
        if self.use_sticky_edges:
            if self._xmargin and scalex and self.get_autoscalex_on():
                x_stickies = np.sort(np.concatenate([artist.sticky_edges.x for ax in self._shared_axes['x'].get_siblings(self) for artist in ax.get_children()]))
            if self._ymargin and scaley and self.get_autoscaley_on():
                y_stickies = np.sort(np.concatenate([artist.sticky_edges.y for ax in self._shared_axes['y'].get_siblings(self) for artist in ax.get_children()]))
    ...
    def get_children(self):
        return [*self._children, *self.spines.values(), *self._axis_map.values(), self.title, self._left_title, self._right_title, *self.child_axes, *([self.legend_] if self.legend_ is not None else []), self.patch]
    ...
    def label_outer(self, remove_inner_ticks=False):
        ...
        self._label_outer_yaxis(skip_non_rectangular_axes=False, remove_inner_ticks=remove_inner_ticks)
