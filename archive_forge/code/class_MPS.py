...
class MPS(Operation):
    ...
    def __init__(self, wires, n_block_wires, block, n_params_block=0, template_weights=None, offset=None, id=None, **kwargs):
        ...
        if template_weights is not None:
            shape = qml.math.shape(template_weights)
            if shape[0] != n_blocks:
                raise ValueError(f'Weights tensor must have first dimension of length {n_blocks}; got {shape[0]}')
            if shape[-1] != n_params_block:
                raise ValueError(f'Weights tensor must have last dimension of length {n_params_block}; got {shape[-1]}')
    ...
    def compute_decomposition(weights=None, wires=None, ind_gates=None, block=None, **kwargs):
        ...
        for w in ind_gates:
            ...
            decomp += block_gen(wires=w, **kwargs) if weight is None else block_gen(weights=weight, wires=w, **kwargs)
    ...
    def get_n_blocks(wires, n_block_wires, offset=None):
        ...
        if offset is None and (not n_block_wires % 2) and (n_wires % (n_block_wires // 2) > 0):
            warnings.warn(f'The number of wires should be a multiple of {int(n_block_wires / 2)}; got {n_wires}')
        ...
        if offset < 1 or offset > n_block_wires - 1:
            raise ValueError(f'Provided offset is outside the expected range; the expected range for n_block_wires = {n_block_wires} is range{(1, n_block_wires - 1)}')
        n_step = offset
