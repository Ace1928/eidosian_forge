...
class Caret:
    ...
    """
    _next_word_re: Pattern[str] = re.compile('(?<=\\W)\\w')
    _previous_word_re: Pattern[str] = re.compile('(?<=\\W)\\w+\\W*$')
    _next_para_re: Pattern[str] = re.compile('\\n', flags=re.DOTALL)
    _previous_para_re: Pattern[str] = re.compile('\\n', flags=re.DOTALL)
    _position: int = 0
    _active: bool = True
    _visible: bool = True
    ...
    _mark: Optional[int] = None
    _next_attributes: dict[str, Any]

    def __init__(self, layout: IncrementalTextLayout, batch: Batch=None, color: Tuple[int, int, int, int]=(0, 0, 0, 255)) -> None:
        ...
        self._list = self._group.program.vertex_list(2, gl.GL_LINES, self._batch, self._group, colors=('Bn', colors), visible=('f', (1, 1)))
        self._ideal_x = None
    ...
    def _update(self, line: Optional[int]=None, update_ideal_x: bool=True):
        if line is None:
        ...
        if update_ideal_x:
            self._ideal_x = x
        x += self._layout.left
        y += self._layout.bottom + self._layout._get_content_height()
