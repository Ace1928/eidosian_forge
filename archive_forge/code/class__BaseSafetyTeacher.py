...
class _BaseSafetyTeacher(FixedDialogTeacher, ABC):
    ...
    def add_cmdline_args(parser):
        ...
        parser.add_argument('--round', type=int, default=1, choices=[1, 2, 3], help='Which round of data to use')
        parser.add_argument('--round-only', type='bool', default=False, help='if False, includes all rounds up to including the specified round; if True, only includes data from the specified round')
        return parser

    def __init__(self, opt, shared=None):
        build(opt['datapath'])
        self.opt = opt
        self.data_path = os.path.join(opt['datapath'], 'dialogue_safety', SINGLE_TURN_DATA)
