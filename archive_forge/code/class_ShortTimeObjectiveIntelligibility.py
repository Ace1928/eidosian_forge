...
class ShortTimeObjectiveIntelligibility(Metric):
    ...
    def __init__(self, fs: int, extended: bool=False, **kwargs: Any) -> None:
        ...
        if not _PYSTOI_AVAILABLE:
            raise ModuleNotFoundError('STOI metric requires that `pystoi` is installed. Either install as `pip install torchmetrics[audio]` or `pip install pystoi`.')
        self.fs = fs
    ...
    def update(self, preds: Tensor, target: Tensor) -> None:
        ...
        stoi_batch = short_time_objective_intelligibility(preds, target, self.fs, self.extended, False).to(self.sum_stoi.device)
