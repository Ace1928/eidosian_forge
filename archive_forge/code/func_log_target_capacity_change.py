...
def log_target_capacity_change(curr_target_capacity: Optional[float], next_target_capacity: Optional[float], next_target_capacity_direction: Optional[TargetCapacityDirection]):
    ...
    if curr_target_capacity != next_target_capacity:
        if isinstance(next_target_capacity_direction, TargetCapacityDirection):
            logger.info(f'Target capacity scaling {next_target_capacity_direction.value.lower()} from {curr_target_capacity} to {next_target_capacity}.')
        else:
