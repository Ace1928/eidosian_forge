...
class SelfFeedingModel(nn.Module):
    ...
    def add_cmdline_args(cls, argparser):
        ...
        model.add_argument('-shl', '--sat-head-layers', type=int, default=1, help='The number of linear layers in the satisfaction task head')
        model.add_argument('-sfeeemb', '--share-fee-embeddings', type='bool', default=True, help='If True, the feedback task shares the dialog embeddings')
        model.add_argument('-sfeexenc', '--share-fee-x-encoder', type='bool', default=True, help='If True, the feedback task shares the dialog x encoder')
        model.add_argument('-sfeeyenc', '--share-fee-y-encoder', type='bool', default=True, help='If True, the feedback task shares the dialog y encoder')
        model.add_argument('-ssatemb', '--share-sat-embeddings', type='bool', default=False, help='If True, the satisfaction task shares the dialog embeddings')
        model.add_argument('-ssatenc', '--share-sat-encoder', type='bool', default=False, help='If True, the satisfaction task shares the dialog encoder')
    ...
    def __init__(self, opt, dictionary):
        ...
        if 'satisfaction' in self.opt['subtasks']:
            ...
            self.x_sat_head = self.build_head(opt, outdim=1, num_layers=self.opt['sat_head_layers'])
    ...
    def build_encoder(self, opt, embeddings):
        return TransformerEncoder(n_heads=opt['n_heads'], n_layers=opt['n_layers'], embedding_size=opt['embedding_size'], ffn_size=opt['ffn_size'], vocabulary_size=self.vocab_size, embedding=embeddings, attention_dropout=opt['attention_dropout'], relu_dropout=opt['relu_dropout'], padding_idx=self.pad_idx, learn_positional_embeddings=opt.get('learn_positional_embeddings', False), embeddings_scale=opt['embeddings_scale'])
