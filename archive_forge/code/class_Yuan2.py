...
class Yuan2(LLM):
    ...
    def _default_params(self) -> Dict[str, Any]:
        return {'do_sample': self.do_sample, 'infer_api': self.infer_api, 'max_tokens': self.max_tokens, 'repeat_penalty': self.repeat_penalty, 'temp': self.temp, 'top_k': self.top_k, 'top_p': self.top_p, 'use_history': self.use_history}

    @property
    def _identifying_params(self) -> Mapping[str, Any]:
        """Get the identifying parameters."""
        return {'model': self._llm_type, **self._default_params(), **{k: v for k, v in self.__dict__.items() if k in self._model_param_names()}}
    ...
    def _call(self, prompt: str, stop: Optional[List[str]]=None, run_manager: Optional[CallbackManagerForLLMRun]=None, **kwargs: Any) -> str:
        ...
        data = json.dumps({'ques_list': [{'id': '000', 'ques': input}], 'tokens_to_generate': self.max_tokens, 'temperature': self.temp, 'top_p': self.top_p, 'top_k': self.top_k, 'do_sample': self.do_sample})
        ...
        try:
            resp = response.json()
            if resp['errCode'] != '0':
                raise ValueError(f'Failed with error code [{resp['errCode']}], error message: [{resp['exceptionMsg']}]')
            if 'resData' in resp:
            ...
            raise ValueError(f'Error raised during decoding response from inference api: {e}.\nResponse: {response.text}')
        if stop is not None:
