...
class UcModelRegistryStore(BaseRestStore):
    ...
    def _get_response_from_method(self, method):
        method_to_response = {CreateRegisteredModelRequest: CreateRegisteredModelResponse, UpdateRegisteredModelRequest: UpdateRegisteredModelResponse, DeleteRegisteredModelRequest: DeleteRegisteredModelResponse, CreateModelVersionRequest: CreateModelVersionResponse, FinalizeModelVersionRequest: FinalizeModelVersionResponse, UpdateModelVersionRequest: UpdateModelVersionResponse, DeleteModelVersionRequest: DeleteModelVersionResponse, GetModelVersionDownloadUriRequest: GetModelVersionDownloadUriResponse, SearchModelVersionsRequest: SearchModelVersionsResponse, GetRegisteredModelRequest: GetRegisteredModelResponse, GetModelVersionRequest: GetModelVersionResponse, SearchRegisteredModelsRequest: SearchRegisteredModelsResponse, GenerateTemporaryModelVersionCredentialsRequest: GenerateTemporaryModelVersionCredentialsResponse, GetRun: GetRun.Response, SetRegisteredModelAliasRequest: SetRegisteredModelAliasResponse, DeleteRegisteredModelAliasRequest: DeleteRegisteredModelAliasResponse, SetRegisteredModelTagRequest: SetRegisteredModelTagResponse, DeleteRegisteredModelTagRequest: DeleteRegisteredModelTagResponse, SetModelVersionTagRequest: SetModelVersionTagResponse, DeleteModelVersionTagRequest: DeleteModelVersionTagResponse, GetModelVersionByAliasRequest: GetModelVersionByAliasResponse}
    ...
    def create_registered_model(self, name, tags=None, description=None):
        ...
        req_body = message_to_json(CreateRegisteredModelRequest(name=full_name, description=description, tags=uc_registered_model_tag_from_mlflow_tags(tags)))
    ...
    def update_registered_model(self, name, description):
        ...
        """
        full_name = get_full_name_from_sc(name, self.spark)
        req_body = message_to_json(UpdateRegisteredModelRequest(name=full_name, description=description))
    ...
    def rename_registered_model(self, name, new_name):
        ...
        Returns:
            A single updated :py:class:`mlflow.entities.model_registry.RegisteredModel` object.
        """
        _raise_unsupported_method(method='rename_registered_model', message='Use the Unity Catalog REST API to rename registered models')
    ...
    def search_registered_models(self, filter_string=None, max_results=None, order_by=None, page_token=None):
        ...
        req_body = message_to_json(SearchRegisteredModelsRequest(max_results=max_results, page_token=page_token))
        response_proto = self._call_endpoint(SearchRegisteredModelsRequest, req_body)
        registered_models = [registered_model_from_uc_proto(registered_model) for registered_model in response_proto.registered_models]
    ...
    def get_latest_versions(self, name, stages=None):
        ...
        if stages is None:
            message = f'To load the latest version of a model in Unity Catalog, you can set an alias on the model version and load it by alias. See {alias_doc_url} for details.'
        else:
            message = f"Detected attempt to load latest model version in stages {stages}. You may see this error because:\n1) You're attempting to load a model version by stage. Setting stages and loading model versions by stage is unsupported in Unity Catalog. Instead, use aliases for flexible model deployment. See {alias_doc_url} for details.\n2) You're attempting to load a model version by alias. Use syntax 'models:/your_model_name@your_alias_name'\n3) You're attempting load a model version by version number. Verify that the version number is a valid integer"
        _raise_unsupported_method(method='get_latest_versions', message=message)
    ...
    def set_registered_model_tag(self, name, tag):
        ...
        """
        full_name = get_full_name_from_sc(name, self.spark)
        req_body = message_to_json(SetRegisteredModelTagRequest(name=full_name, key=tag.key, value=tag.value))
    ...
    def _get_workspace_id(self, headers):
        if headers is None or _DATABRICKS_ORG_ID_HEADER not in headers:
            _logger.warning("Unable to get model version source run's workspace ID from request headers. No run link will be recorded for the model version")
            return None
    ...
    def create_model_version(self, name, source, run_id=None, tags=None, run_link=None, description=None, local_model_path=None):
        """
        ...
        with self._local_model_dir(source, local_model_path) as local_model_dir:
            ...
            req_body = message_to_json(CreateModelVersionRequest(name=full_name, source=source, run_id=run_id, description=description, tags=uc_model_version_tag_from_mlflow_tags(tags), run_tracking_server_id=source_workspace_id, feature_deps=feature_deps, model_version_dependencies=other_model_deps))
            model_version = self._call_endpoint(CreateModelVersionRequest, req_body, extra_headers=extra_headers).model_version
    ...
    def _get_artifact_repo(self, model_version):
        if MLFLOW_UNITY_CATALOG_PRESIGNED_URLS_ENABLED.get():
            return PresignedUrlArtifactRepository(self.get_host_creds(), model_version.name, model_version.version)
        scoped_token = self._get_temporary_model_version_write_credentials(name=model_version.name, version=model_version.version)
        return get_artifact_repo_from_storage_info(storage_location=model_version.storage_location, scoped_token=scoped_token)
    ...
    def update_model_version(self, name, version, description):
        ...
        """
        full_name = get_full_name_from_sc(name, self.spark)
        req_body = message_to_json(UpdateModelVersionRequest(name=full_name, version=str(version), description=description))
    ...
    def get_model_version_download_uri(self, name, version):
        ...
        """
        full_name = get_full_name_from_sc(name, self.spark)
        req_body = message_to_json(GetModelVersionDownloadUriRequest(name=full_name, version=str(version)))
    ...
    def search_model_versions(self, filter_string=None, max_results=None, order_by=None, page_token=None):
        ...
        req_body = message_to_json(SearchModelVersionsRequest(filter=filter_string, page_token=page_token, max_results=max_results))
    ...
    def set_model_version_tag(self, name, version, tag):
        ...
        """
        full_name = get_full_name_from_sc(name, self.spark)
        req_body = message_to_json(SetModelVersionTagRequest(name=full_name, version=version, key=tag.key, value=tag.value))
    ...
    def delete_model_version_tag(self, name, version, key):
        ...
        """
        full_name = get_full_name_from_sc(name, self.spark)
        req_body = message_to_json(DeleteModelVersionTagRequest(name=full_name, version=version, key=key))
    ...
    def set_registered_model_alias(self, name, alias, version):
        ...
        """
        full_name = get_full_name_from_sc(name, self.spark)
        req_body = message_to_json(SetRegisteredModelAliasRequest(name=full_name, alias=alias, version=str(version)))
