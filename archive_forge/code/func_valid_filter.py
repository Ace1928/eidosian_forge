...
def valid_filter(filter_):
    ...
    if 'type' not in filter_:
        raise ConfigTypeError(desc="Invalid filter: %s, needs 'type' field" % filter_)
    if not filter_['type'] in SUPPORTED_FILTER_VALIDATORS:
        raise ConfigTypeError(desc='Invalid filter type: %s, supported filter types are %s' % (filter_['type'], list(SUPPORTED_FILTER_VALIDATORS.keys())))
