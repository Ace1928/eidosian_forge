...
class TestTreeTransform(TestCaseWithWorkingTree):
    ...
    def test_apply_retains_root_directory(self):
        ...
        with transform:
            transform.delete_contents(root)
            e = self.assertRaises(AssertionError, self.assertRaises, TransformRenameFailed, transform.apply)
    ...
    def test_conflicts(self):
        ...
        self.assertEqual(transform.find_raw_conflicts(), [('duplicate', trans_id, trans_id2, 'name')])
        self.assertRaises(MalformedTransform, transform.apply)
        transform.adjust_path('name', trans_id, trans_id2)
        self.assertEqual(transform.find_raw_conflicts(), [('non-directory parent', trans_id)])
        tinman_id = transform.trans_id_tree_path('tinman')
        transform.adjust_path('name', tinman_id, trans_id2)
        if self.wt.has_versioned_directories():
            self.assertEqual(transform.find_raw_conflicts(), [('unversioned parent', tinman_id), ('missing parent', tinman_id)])
        else:
            self.assertEqual(transform.find_raw_conflicts(), [('missing parent', tinman_id)])
        ...
        if self.wt.has_versioned_directories():
            self.assertEqual(transform.find_raw_conflicts(), [('unversioned parent', lion_id), ('missing parent', lion_id)])
        else:
            self.assertEqual(transform.find_raw_conflicts(), [('missing parent', lion_id)])
        transform.version_file(lion_id, file_id=b'Courage')
        self.assertEqual(transform.find_raw_conflicts(), [('missing parent', lion_id), ('versioning no contents', lion_id)])
        transform.adjust_path('name2', root, trans_id2)
        self.assertEqual(transform.find_raw_conflicts(), [('versioning no contents', lion_id)])
        transform.create_file([b'Contents, okay?'], lion_id)
        transform.adjust_path('name2', trans_id2, trans_id2)
        self.assertEqual(transform.find_raw_conflicts(), [('parent loop', trans_id2), ('non-directory parent', trans_id2)])
        transform.adjust_path('name2', root, trans_id2)
        oz_id = transform.new_directory('oz', root)
        transform.set_executability(True, oz_id)
        self.assertEqual(transform.find_raw_conflicts(), [('unversioned executability', oz_id)])
        transform.version_file(oz_id, file_id=b'oz-id')
        self.assertEqual(transform.find_raw_conflicts(), [('non-file executability', oz_id)])
    ...
    def test_symlinks_unicode(self):
        ...
        self._test_symlinks('€wizard', 'wizard-targ€t', '€wizard2', 'b€hind_curtain')
    ...
    def test_iter_changes(self):
        ...
        try:
            ...
            self.assertTreeChanges(transform, [TreeChange(('old', None), False, (True, False), ('old', 'old'), ('file', 'file'), (True, True), False)])
            transform.new_directory('new', root, b'id-1')
            if transform._tree.supports_setting_file_ids():
                self.assertTreeChanges(transform, [TreeChange(('old', 'new'), True, (True, True), ('old', 'new'), ('file', 'directory'), (True, False), False)])
            else:
                self.assertTreeChanges(transform, [TreeChange((None, 'new'), False, (False, True), (None, 'new'), (None, 'directory'), (False, False), False), TreeChange(('old', None), False, (True, False), ('old', 'old'), ('file', 'file'), (True, True), False)])
        finally:
            transform.finalize()
