...
class _FeedParserMixin(_base.Namespace, cc.Namespace, dc.Namespace, georss.Namespace, itunes.Namespace, mediarss.Namespace, psc.Namespace):
    ...
    _matchnamespaces = {}
    can_be_relative_uri = {'comments', 'docs', 'href', 'icon', 'id', 'link', 'logo', 'url', 'wfw_comment', 'wfw_commentrss'}
    can_contain_relative_uris = {'content', 'copyright', 'description', 'info', 'rights', 'subtitle', 'summary', 'tagline', 'title'}
    can_contain_dangerous_markup = {'content', 'copyright', 'description', 'info', 'rights', 'subtitle', 'summary', 'tagline', 'title'}
    ...
    @staticmethod
    def strattrs(attrs):
        return ''.join((' %s="%s"' % (t[0], xml.sax.saxutils.escape(t[1], {'"': '&quot;'})) for t in attrs))
