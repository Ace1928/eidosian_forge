...
SCRAPE_DIR = "/home/lloyd/Downloads/scrapedata/"
INDEX_FILE = os.path.join(SCRAPE_DIR, "index.json")
...
class BrowserUI(QMainWindow):
    ...
    def handle_scrape_result(self, html: str):
        """
        Handles the result of the page scrape by saving the HTML content to a file if it has not been scraped since the last update.
        ...
        try:
            ...
            metadata = {"url": url, "timestamp": timestamp, "html_length": len(html)}
            ...
            # Check if the page has been updated since the last scrape
            if url in index and index[url]["html_length"] == len(html):
                logging.info("Page has not been updated since the last scrape.")
                return
            ...
            logging.info(f"Scraped page content saved to {filepath}")
        except Exception as e:
...
class WebPageHandler:
    ...
    def on_load_progress(self, progress: int):
        ...
        """
        self.browser.parent().showMessage(f"Loading... {progress}%")
