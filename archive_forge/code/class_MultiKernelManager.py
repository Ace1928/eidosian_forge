...
class MultiKernelManager(LoggingConfigurable):
    ...
    kernel_manager_class = DottedObjectName('jupyter_client.ioloop.IOLoopKernelManager', help='The kernel manager class.  This is configurable to allow\n        subclassing of the KernelManager for customized behavior.\n        ').tag(config=True)
    ...
    shared_context = Bool(True, help='Share a single zmq.Context to talk to all my kernels').tag(config=True)
    ...
    async def _async_start_kernel(self, *, kernel_name: str | None=None, **kwargs: t.Any) -> str:
        ...
        if not isinstance(km, KernelManager):
            self.log.warning("Kernel manager class ({km_class}) is not an instance of 'KernelManager'!".format(km_class=self.kernel_manager_class.__class__))
    ...
    async def _async_shutdown_kernel(self, kernel_id: str, now: bool | None=False, restart: bool | None=False) -> None:
