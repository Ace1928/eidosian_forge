...
class Vearch(VectorStore):
    ...
    def __init__(self, embedding_function: Embeddings, path_or_url: Optional[str]=None, table_name: str=_DEFAULT_TABLE_NAME, db_name: str=_DEFAULT_CLUSTER_DB_NAME, flag: int=_DEFAULT_VERSION, **kwargs: Any) -> None:
        ...
        try:
            ...
            raise ValueError('Could not import suitable python package. Please install it with `pip install vearch or vearch_cluster`.')
        if flag:
    ...
    @classmethod
    def from_documents(cls: Type[Vearch], documents: List[Document], embedding: Embeddings, path_or_url: Optional[str]=None, table_name: str=_DEFAULT_TABLE_NAME, db_name: str=_DEFAULT_CLUSTER_DB_NAME, flag: int=_DEFAULT_VERSION, **kwargs: Any) -> Vearch:
        """Return Vearch VectorStore"""
        texts = [d.page_content for d in documents]
        metadatas = [d.metadata for d in documents]
        return cls.from_texts(texts=texts, embedding=embedding, metadatas=metadatas, path_or_url=path_or_url, table_name=table_name, db_name=db_name, flag=flag, **kwargs)

    @classmethod
    def from_texts(cls: Type[Vearch], texts: List[str], embedding: Embeddings, metadatas: Optional[List[dict]]=None, path_or_url: Optional[str]=None, table_name: str=_DEFAULT_TABLE_NAME, db_name: str=_DEFAULT_CLUSTER_DB_NAME, flag: int=_DEFAULT_VERSION, **kwargs: Any) -> Vearch:
        """Return Vearch VectorStore"""
        vearch_db = cls(embedding_function=embedding, embedding=embedding, path_or_url=path_or_url, db_name=db_name, table_name=table_name, flag=flag)
        vearch_db.add_texts(texts=texts, metadatas=metadatas)
        return vearch_db

    def _create_table(self, dim: int=1024, field_list: List[dict]=[{'field': 'text', 'type': 'str'}, {'field': 'metadata', 'type': 'str'}]) -> int:
        """
        ...
        engine_info = {'index_size': 10000, 'retrieval_type': 'IVFPQ', 'retrieval_param': {'ncentroids': 2048, 'nsubvector': 32}}
        fields = [vearch.GammaFieldInfo(fi['field'], type_dict[fi['type']]) for fi in field_list]
        vector_field = vearch.GammaVectorInfo(name='text_embedding', type=vearch.dataType.VECTOR, is_index=True, dimension=dim, model_id='', store_type='MemoryOnly', store_param={'cache_size': 10000}, has_source=False)
        response_code = self.vearch.create_table(engine_info, name=self.using_table_name, fields=fields, vector_field=vector_field)
    ...
    @classmethod
    def load_local(cls, embedding: Embeddings, path_or_url: Optional[str]=None, table_name: str=_DEFAULT_TABLE_NAME, db_name: str=_DEFAULT_CLUSTER_DB_NAME, flag: int=_DEFAULT_VERSION, **kwargs: Any) -> Vearch:
        ...
        vearch_db = cls(embedding_function=embedding, path_or_url=path_or_url, table_name=table_name, db_name=db_name, flag=flag)
        vearch_db._load()
        return vearch_db

    def similarity_search(self, query: str, k: int=DEFAULT_TOPN, **kwargs: Any) -> List[Document]:
        """
    ...
    def similarity_search_by_vector(self, embedding: List[float], k: int=DEFAULT_TOPN, **kwargs: Any) -> List[Document]:
        ...
        if self.flag:
            query_data = {'query': {'sum': [{'field': 'text_embedding', 'feature': (embed / np.linalg.norm(embed)).tolist()}]}, 'size': k, 'fields': ['text', 'metadata']}
            query_result = self.vearch.search(self.using_db_name, self.using_table_name, query_data)
            res = query_result['hits']['hits']
        else:
            query_data = {'vector': [{'field': 'text_embedding', 'feature': embed / np.linalg.norm(embed)}], 'fields': [], 'is_brute_search': 1, 'retrieval_param': {'metric_type': 'InnerProduct', 'nprobe': 20}, 'topn': k}
    ...
    def similarity_search_with_score(self, query: str, k: int=DEFAULT_TOPN, **kwargs: Any) -> List[Tuple[Document, float]]:
        """The most k similar documents and scores of the specified query.
        Args:
            embeddings: embedding vector of the query.
            k: The k most similar documents to the text query.
        ...
        if self.flag:
            query_data = {'query': {'sum': [{'field': 'text_embedding', 'feature': (embed / np.linalg.norm(embed)).tolist()}]}, 'size': k, 'fields': ['text_embedding', 'text', 'metadata']}
            query_result = self.vearch.search(self.using_db_name, self.using_table_name, query_data)
            res = query_result['hits']['hits']
        else:
            query_data = {'vector': [{'field': 'text_embedding', 'feature': embed / np.linalg.norm(embed)}], 'fields': [], 'is_brute_search': 1, 'retrieval_param': {'metric_type': 'InnerProduct', 'nprobe': 20}, 'topn': k}
    ...
    def get(self, ids: Optional[List[str]]=None, **kwargs: Any) -> Dict[str, Document]:
