...
class SharpnessAwareMinimization(Model):
    ...
    def train_step(self, data):
        ...
        for x_batch, y_batch in zip(x_split, y_split):
            ...
            for gradient, variable in zip(gradients, trainable_variables):
                ...
                self._distributed_apply_epsilon_w(variable, epsilon_w, tf.distribute.get_strategy())
            ...
            if len(gradients_all_batches) == 0:
                ...
                for gradient, gradient_all_batches in zip(gradients, gradients_all_batches):
    ...
    def get_config(self):
        ...
        config.update({'model': serialize_keras_object(self.model), 'rho': self.rho})
        return config
