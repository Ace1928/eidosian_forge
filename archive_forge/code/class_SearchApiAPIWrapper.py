...
class SearchApiAPIWrapper(BaseModel):
    ...
    def _prepare_request(self, query: str, **kwargs: Any) -> dict:
        return {'url': 'https://www.searchapi.io/api/v1/search', 'headers': {'Authorization': f'Bearer {self.searchapi_api_key}'}, 'params': {'engine': self.engine, 'q': query, **{key: value for key, value in kwargs.items() if value is not None}}}
    ...
    async def _async_search_api_results(self, query: str, **kwargs: Any) -> dict:
        ...
        if not self.aiosession:
            async with aiohttp.ClientSession() as session:
                async with session.get(url=request_details['url'], headers=request_details['headers'], params=request_details['params'], raise_for_status=True) as response:
                    results = await response.json()
        else:
            async with self.aiosession.get(url=request_details['url'], headers=request_details['headers'], params=request_details['params'], raise_for_status=True) as response:
                results = await response.json()
        return results
