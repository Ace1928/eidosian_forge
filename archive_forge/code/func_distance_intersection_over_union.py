...
def distance_intersection_over_union(preds: torch.Tensor, target: torch.Tensor, iou_threshold: Optional[float]=None, replacement_val: float=0, aggregate: bool=True) -> torch.Tensor:
    ...
    if not _TORCHVISION_GREATER_EQUAL_0_13:
        raise ModuleNotFoundError(f'`{distance_intersection_over_union.__name__}` requires that `torchvision` version 0.13.0 or newer is installed. Please install with `pip install torchvision>=0.13` or `pip install torchmetrics[detection]`.')
