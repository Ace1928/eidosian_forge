    ...
    def _get_learner_bundles(cf: AlgorithmConfig) -> List[Dict[str, int]]:
        ...
        if cf.num_learner_workers > 0:
            if cf.num_gpus_per_learner_worker:
                learner_bundles = [{'GPU': cf.num_learner_workers * cf.num_gpus_per_learner_worker}]
            elif cf.num_cpus_per_learner_worker:
                learner_bundles = [{'CPU': cf.num_cpus_per_learner_worker * cf.num_learner_workers}]
        else:
            learner_bundles = [{'CPU': max(cf.num_cpus_per_learner_worker, cf.num_cpus_for_local_worker), 'GPU': cf.num_gpus_per_learner_worker}]
