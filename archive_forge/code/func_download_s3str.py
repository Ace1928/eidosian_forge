...
def download_s3str(module, s3, bucket, obj, version=None):
    ...
    try:
        if version:
            contents = to_native(s3.get_object(aws_retry=True, Bucket=bucket, Key=obj, VersionId=version)['Body'].read())
        else:
        ...
        module.fail_json_aws(e, msg=f'Failed while getting contents of object {obj} as a string.')
