...
class RFuncBackend(FlavorBackend):
    ...
    def predict(self, model_uri, input_path, output_path, content_type, pip_requirements_override=None):
        ...
        str_cmd = "mlflow:::mlflow_rfunc_predict(model_path = '{0}', input_path = {1}, output_path = {2}, content_type = {3})"
        command = str_cmd.format(quote(model_path), _str_optional(input_path), _str_optional(output_path), _str_optional(content_type))
        _execute(command)
    ...
    def serve(self, model_uri, port, host, timeout, enable_mlserver, synchronous=True, stdout=None, stderr=None):
        ...
        model_path = _download_artifact_from_uri(model_uri)
        command = "mlflow::mlflow_rfunc_serve('{}', port = {}, host = '{}')".format(quote(model_path), port, host)
        _execute(command)
