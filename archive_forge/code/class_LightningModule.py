...
class LightningModule(_DeviceDtypeModuleMixin, HyperparametersMixin, ModelHooks, DataHooks, CheckpointHooks, Module):
    __jit_unused_properties__: List[str] = ['example_input_array', 'on_gpu', 'current_epoch', 'global_step', 'global_rank', 'local_rank', 'logger', 'loggers', 'automatic_optimization', 'trainer', 'fabric', 'strict_loading'] + _DeviceDtypeModuleMixin.__jit_unused_properties__ + HyperparametersMixin.__jit_unused_properties__
    ...
    def trainer(self) -> 'pl.Trainer':
        if self._fabric is not None:
            return _TrainerFabricShim(fabric=self._fabric)
    ...
    def _apply_batch_transfer_handler(self, batch: Any, device: Optional[torch.device]=None, dataloader_idx: int=0) -> Any:
    ...
    def log(self, name: str, value: _METRIC, prog_bar: bool=False, logger: Optional[bool]=None, on_step: Optional[bool]=None, on_epoch: Optional[bool]=None, reduce_fx: Union[str, Callable]='mean', enable_graph: bool=False, sync_dist: bool=False, sync_dist_group: Optional[Any]=None, add_dataloader_idx: bool=True, batch_size: Optional[int]=None, metric_attribute: Optional[str]=None, rank_zero_only: bool=False) -> None:
        ...
        apply_to_collection(value, object, self.__check_allowed, name, value, wrong_dtype=(numbers.Number, Metric, Tensor))
        trainer = self._trainer
        if trainer is None:
            rank_zero_warn("You are trying to `self.log()` but the `self.trainer` reference is not registered on the model yet. This is most likely because the model hasn't been passed to the `Trainer`")
            return
        if trainer.barebones:
            rank_zero_warn('You are trying to `self.log()` but `Trainer(barebones=True)` is configured. Logging can impact raw speed so it is disabled under this setting.')
            return
        results = trainer._results
        if results is None:
            raise MisconfigurationException("You are trying to `self.log()` but the loop's result collection is not registered yet. This is most likely because you are trying to log in a `predict` hook, but it doesn't support logging")
        if self._current_fx_name is None:
            raise MisconfigurationException('You are trying to `self.log()` but it is not managed by the `Trainer` control flow')
        on_step, on_epoch = _FxValidator.check_logging_and_get_default_levels(self._current_fx_name, on_step=on_step, on_epoch=on_epoch)
        if '/dataloader_idx_' in name:
            raise MisconfigurationException(f'You called `self.log` with the key `{name}` but it should not contain information about `dataloader_idx`')
        ...
        if metric_attribute is None and isinstance(value, Metric):
            if self._metric_attributes is None:
                self._metric_attributes = {id(module): name for name, module in self.named_modules() if isinstance(module, Metric)}
                if not self._metric_attributes:
                    raise MisconfigurationException('Could not find the `LightningModule` attribute for the `torchmetrics.Metric` logged. You can fix this by setting an attribute for the metric in your `LightningModule`.')
            metric_attribute = self._metric_attributes.get(id(value), None)
            if metric_attribute is None:
                raise MisconfigurationException(f'Could not find the `LightningModule` attribute for the `torchmetrics.Metric` logged. You can fix this by calling `self.log({name}, ..., metric_attribute=name)` where `name` is one of {list(self._metric_attributes.values())}')
        if trainer.training and is_param_in_hook_signature(self.training_step, 'dataloader_iter', explicit=True) and (batch_size is None):
            raise MisconfigurationException('With `def training_step(self, dataloader_iter)`, `self.log(..., batch_size=...)` should be provided.')
        if logger and trainer.logger is None:
            rank_zero_warn(f'You called `self.log({name!r}, ..., logger=True)` but have no logger configured. You can enable one by doing `Trainer(logger=ALogger(...))`')
        if logger is None:
            logger = True
        results.log(self._current_fx_name, name, value, prog_bar=prog_bar, logger=logger, on_step=on_step, on_epoch=on_epoch, reduce_fx=reduce_fx, enable_graph=enable_graph, add_dataloader_idx=add_dataloader_idx, batch_size=batch_size, sync_dist=sync_dist and trainer._accelerator_connector.is_distributed, sync_dist_fn=trainer.strategy.reduce, sync_dist_group=sync_dist_group, metric_attribute=metric_attribute, rank_zero_only=rank_zero_only)
    ...
    def log_dict(self, dictionary: Union[Mapping[str, _METRIC], MetricCollection], prog_bar: bool=False, logger: Optional[bool]=None, on_step: Optional[bool]=None, on_epoch: Optional[bool]=None, reduce_fx: Union[str, Callable]='mean', enable_graph: bool=False, sync_dist: bool=False, sync_dist_group: Optional[Any]=None, add_dataloader_idx: bool=True, batch_size: Optional[int]=None, rank_zero_only: bool=False) -> None:
        ...
        for k, v in dictionary.items(**kwargs):
            self.log(name=k, value=v, prog_bar=prog_bar, logger=logger, on_step=on_step, on_epoch=on_epoch, reduce_fx=reduce_fx, enable_graph=enable_graph, sync_dist=sync_dist, sync_dist_group=sync_dist_group, add_dataloader_idx=add_dataloader_idx, batch_size=batch_size, rank_zero_only=rank_zero_only)
        return None

    def _log_dict_through_fabric(self, dictionary: Union[Mapping[str, _METRIC], MetricCollection], logger: Optional[bool]=None) -> None:
        if logger is False:
            return
    ...
    def __to_tensor(self, value: Union[Tensor, numbers.Number], name: str) -> Tensor:
        value = value.clone().detach() if isinstance(value, Tensor) else torch.tensor(value, device=self.device, dtype=_get_default_dtype())
        if not torch.numel(value) == 1:
            raise ValueError(f'`self.log({name}, {value})` was called, but the tensor must have a single element. You can try doing `self.log({name}, {value}.mean())`')
        value = value.squeeze()
        return value

    def all_gather(self, data: Union[Tensor, Dict, List, Tuple], group: Optional[Any]=None, sync_grads: bool=False) -> Union[Tensor, Dict, List, Tuple]:
    ...
    def clip_gradients(self, optimizer: Optimizer, gradient_clip_val: Optional[Union[int, float]]=None, gradient_clip_algorithm: Optional[str]=None) -> None:
        ...
        if self.fabric is not None:
            self.fabric.clip_gradients(self, optimizer, clip_val=gradient_clip_val if gradient_clip_algorithm == GradClipAlgorithmType.VALUE else None, max_norm=None if gradient_clip_algorithm == GradClipAlgorithmType.VALUE else gradient_clip_val)
            return
        if gradient_clip_val is None:
            ...
            raise MisconfigurationException(f'You have set `Trainer(gradient_clip_val={self.trainer.gradient_clip_val!r})` and have passed `clip_gradients(gradient_clip_val={gradient_clip_val!r})`. Please use only one of them.')
        if gradient_clip_algorithm is None:
            ...
            if self.trainer.gradient_clip_algorithm is not None and self.trainer.gradient_clip_algorithm != gradient_clip_algorithm:
                raise MisconfigurationException(f'You have set `Trainer(gradient_clip_algorithm={self.trainer.gradient_clip_algorithm.value!r})` and have passed `clip_gradients(gradient_clip_algorithm={gradient_clip_algorithm!r}) Please use only one of them.')
        ...
        if not GradClipAlgorithmType.supported_type(gradient_clip_algorithm.lower()):
            raise MisconfigurationException(f'`gradient_clip_algorithm` {gradient_clip_algorithm} is invalid. Allowed algorithms: {GradClipAlgorithmType.supported_types()}.')
    ...
    def optimizer_step(self, epoch: int, batch_idx: int, optimizer: Union[Optimizer, LightningOptimizer], optimizer_closure: Optional[Callable[[], Any]]=None) -> None:
        """Override this method to adjust the default way the :class:`~pytorch_lightning.trainer.trainer.Trainer` calls
        the optimizer.
    ...
    def _verify_is_manual_optimization(self, fn_name: str) -> None:
        if self.automatic_optimization:
            raise MisconfigurationException(f'to use {fn_name}, please disable automatic optimization: set model property `automatic_optimization` as False')
    ...
    def load_from_checkpoint(cls, checkpoint_path: Union[_PATH, IO], map_location: _MAP_LOCATION_TYPE=None, hparams_file: Optional[_PATH]=None, strict: Optional[bool]=None, **kwargs: Any) -> Self:
        ...
        """
        loaded = _load_from_checkpoint(cls, checkpoint_path, map_location, hparams_file, strict, **kwargs)
