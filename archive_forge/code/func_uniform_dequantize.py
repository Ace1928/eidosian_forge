...
def uniform_dequantize(input: _atypes.TensorFuzzingAnnotation[TV_UniformDequantize_Tin], scales: _atypes.TensorFuzzingAnnotation[_atypes.Float32], zero_points: _atypes.TensorFuzzingAnnotation[_atypes.Int32], Tout: TV_UniformDequantize_Tout, quantization_min_val: int, quantization_max_val: int, quantization_axis: int=-1, name=None) -> _atypes.TensorFuzzingAnnotation[TV_UniformDequantize_Tout]:
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'UniformDequantize', name, input, scales, zero_points, 'Tout', Tout, 'quantization_axis', quantization_axis, 'quantization_min_val', quantization_min_val, 'quantization_max_val', quantization_max_val)
            return _result
            ...
            pass
        try:
            return uniform_dequantize_eager_fallback(input, scales, zero_points, Tout=Tout, quantization_axis=quantization_axis, quantization_min_val=quantization_min_val, quantization_max_val=quantization_max_val, name=name, ctx=_ctx)
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('UniformDequantize', input=input, scales=scales, zero_points=zero_points, Tout=Tout, quantization_min_val=quantization_min_val, quantization_max_val=quantization_max_val, quantization_axis=quantization_axis, name=name)
    _result = _outputs[:]
    if _execute.must_record_gradient():
        ...
        _execute.record_gradient('UniformDequantize', _inputs_flat, _attrs, _result)
    _result, = _result
    return _result