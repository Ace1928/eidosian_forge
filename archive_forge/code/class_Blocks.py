...
class Blocks(BlockContext, BlocksEvents, metaclass=BlocksMeta):
    ...
    def __init__(self, theme: Theme | str | None=None, analytics_enabled: bool | None=None, mode: str='blocks', title: str='Gradio', css: str | None=None, js: str | None=None, head: str | None=None, fill_height: bool=False, delete_cache: tuple[int, int] | None=None, **kwargs):
        """
        Parameters:
        ...
        with Blocks(theme=theme) as blocks:
            ...
            for dependency, fn in zip(config['dependencies'], fns):
                ...
                if not isinstance(_targets[0], int) and _targets[0][1] == 'then':
                    if len(_targets) != 1:
                        raise ValueError('This logic assumes that .then() events are not combined with other events in a single gr.on() event')
                    is_then_event = True
    ...
    def unload(self, fn: Callable):
        ...
        Example:
            ...
            demo.launch()
        """
        self.set_event_trigger(targets=[EventListenerMethod(None, 'unload')], fn=fn, inputs=None, outputs=None, preprocess=False, postprocess=False, show_progress='hidden', api_name=None, js=None, no_target=True, queue=None, batch=False, max_batch_size=4, cancels=None, every=None, collects_event_data=None, trigger_after=None, trigger_only_on_success=False, trigger_mode='once', concurrency_limit='default', concurrency_id=None, show_api=False)

    def set_event_trigger(self, targets: Sequence[EventListenerMethod], fn: Callable | None, inputs: Component | list[Component] | set[Component] | None, outputs: Component | list[Component] | None, preprocess: bool=True, postprocess: bool=True, scroll_to_output: bool=False, show_progress: Literal['full', 'minimal', 'hidden']='full', api_name: str | None | Literal[False]=None, js: str | None=None, no_target: bool=False, queue: bool | None=None, batch: bool=False, max_batch_size: int=4, cancels: list[int] | None=None, every: float | None=None, collects_event_data: bool | None=None, trigger_after: int | None=None, trigger_only_on_success: bool=False, trigger_mode: Literal['once', 'multiple', 'always_last'] | None='once', concurrency_limit: int | None | Literal['default']='default', concurrency_id: str | None=None, show_api: bool=True) -> tuple[dict[str, Any], int]:
        """
        ...
        """
        _targets = [(target.block._id if not no_target and target.block else None, target.event_name) for target in targets]
        ...
        if every and concurrency_limit is not None:
            if concurrency_limit == 'default':
                ...
                raise ValueError('Cannot set a value for `concurrency_limit` with `every`.')
        ...
        if api_name is None or (isinstance(api_name, str) and api_name.strip() == ''):
            if fn is not None:
                if not hasattr(fn, '__name__'):
                    ...
                    name = fn.__name__
                api_name = ''.join([s for s in name if s not in set(string.punctuation) - {'-', '_'}])
            elif js is not None:
                api_name = 'js_fn'
                show_api = False
            else:
    ...
    def __call__(self, *inputs, fn_index: int=0, api_name: str | None=None):
        ...
        *inputs: the parameters to pass to the function
        ...
        outputs = client_utils.synchronize_async(self.process_api, fn_index=fn_index, inputs=processed_inputs, request=None, state={}, explicit_call=True)
    ...
    def serialize_data(self, fn_index: int, inputs: list[Any]) -> list[Any]:
        ...
        for i, input_id in enumerate(dependency['inputs']):
            ...
            if client_utils.value_is_file(api_info):
                serialized_input = client_utils.traverse(inputs[i], format_file, lambda s: client_utils.is_filepath(s) or client_utils.is_http_url_like(s))
            else:
    ...
    async def preprocess_data(self, fn_index: int, inputs: list[Any], state: SessionState | None, explicit_call: bool=False):
        ...
        if block_fn.preprocess:
            ...
            for i, input_id in enumerate(dependency['inputs']):
                ...
                if getattr(block, 'stateful', False):
                    ...
                    inputs_cached = await processing_utils.async_move_files_to_cache(inputs[i], block, check_in_upload_folder=not explicit_call)
    ...
    async def postprocess_data(self, fn_index: int, predictions: list | dict, state: SessionState | None):
        state = state or SessionState(self)
        block_fn = self.fns[fn_index]
        dependency = self.dependencies[fn_index]
        ...
        for i, output_id in enumerate(dependency['outputs']):
            ...
            if block.stateful:
                ...
                outputs_cached = await processing_utils.async_move_files_to_cache(prediction_value, block, postprocess=True)
                output.append(outputs_cached)
        return output

    async def handle_streaming_outputs(self, fn_index: int, data: list, session_hash: str | None, run: int | None, root_path: str | None=None) -> list:
        ...
        for i, output_id in enumerate(self.dependencies[fn_index]['outputs']):
            ...
            if isinstance(block, components.StreamingOutput) and block.streaming:
                ...
                output_data = await processing_utils.async_move_files_to_cache(output_data, block, postprocess=True)
    ...
    async def process_api(self, fn_index: int, inputs: list[Any], state: SessionState | None=None, request: routes.Request | list[routes.Request] | None=None, iterator: AsyncIterator | None=None, session_hash: str | None=None, event_id: str | None=None, event_data: EventData | None=None, in_event_listener: bool=True, simple_format: bool=False, explicit_call: bool=False, root_path: str | None=None) -> dict[str, Any]:
        ...
        if batch:
            ...
            inputs = [await self.preprocess_data(fn_index, list(i), state, explicit_call) for i in zip(*inputs)]
            result = await self.call_function(fn_index, list(zip(*inputs)), None, request, event_id, event_data, in_event_listener)
            preds = result['prediction']
            data = [await self.postprocess_data(fn_index, list(o), state) for o in zip(*preds)]
            if root_path is not None:
            ...
            if old_iterator:
                ...
                inputs = await self.preprocess_data(fn_index, inputs, state, explicit_call)
            ...
            if is_generating or was_generating:
                ...
                data = await self.handle_streaming_outputs(fn_index, data, session_hash=session_hash, run=run, root_path=root_path)
                data = self.handle_streaming_diffs(fn_index, data, session_hash=session_hash, run=run, final=not is_generating, simple_format=simple_format)
    ...
    @document()
    def queue(self, status_update_rate: float | Literal['auto']='auto', api_open: bool | None=None, max_size: int | None=None, concurrency_count: int | None=None, *, default_concurrency_limit: int | None | Literal['not_set']='not_set'):
        """
        ...
        self._queue = queueing.Queue(live_updates=status_update_rate == 'auto', concurrency_count=self.max_threads, update_intervals=status_update_rate if status_update_rate != 'auto' else 1, max_size=max_size, block_fns=self.fns, default_concurrency_limit=default_concurrency_limit)
        self.config = self.get_config_file()
        self.app = routes.App.create_app(self)
        return self
    ...
    def launch(self, inline: bool | None=None, inbrowser: bool=False, share: bool | None=None, debug: bool=False, max_threads: int=40, auth: Callable | tuple[str, str] | list[tuple[str, str]] | None=None, auth_message: str | None=None, prevent_thread_lock: bool=False, show_error: bool=False, server_name: str | None=None, server_port: int | None=None, *, height: int=500, width: int | str='100%', favicon_path: str | None=None, ssl_keyfile: str | None=None, ssl_certfile: str | None=None, ssl_keyfile_password: str | None=None, ssl_verify: bool=True, quiet: bool=False, show_api: bool=True, allowed_paths: list[str] | None=None, blocked_paths: list[str] | None=None, root_path: str | None=None, app_kwargs: dict[str, Any] | None=None, state_session_capacity: int=10000, share_server_address: str | None=None, share_server_protocol: Literal['http', 'https'] | None=None, auth_dependency: Callable[[fastapi.Request], str | None] | None=None, _frontend: bool=True) -> tuple[FastAPI, str, str]:
        ...
        if auth is not None and auth_dependency is not None:
            raise ValueError('You cannot provide both `auth` and `auth_dependency` in launch(). Please choose one.')
        ...
        if self.is_running:
            ...
            if wasm_utils.IS_WASM:
                ...
                app = App.create_app(self, auth_dependency=auth_dependency, app_kwargs=app_kwargs)
                wasm_utils.register_app(app)
            else:
                from gradio import http_server
                server_name, server_port, local_url, app, server = http_server.start_server(self, server_name, server_port, ssl_keyfile, ssl_certfile, ssl_keyfile_password, app_kwargs=app_kwargs)
            ...
            if not wasm_utils.IS_WASM:
                httpx.get(f'{self.local_url}startup-events', verify=ssl_verify, timeout=None)
            else:
                self.startup_events()
    ...
    def get_api_info(self):
        ...
        for dependency, fn in zip(config['dependencies'], self.fns):
            ...
            for index, input_id in enumerate(inputs):
                ...
                if dependency['backend_fn'] and index < len(fn_info) and (fn_info[index][0] not in ['api_name', 'fn_index', 'result_callbacks']):
                ...
                elif dependency['backend_fn'] and index < len(fn_info) and fn_info[index][1] and (fn_info[index][2] is None):
                    parameter_has_default = True
                    parameter_default = None
                else:
                    parameter_has_default = False
                    parameter_default = None
                dependency_info['parameters'].append({'label': label, 'parameter_name': parameter_name, 'parameter_has_default': parameter_has_default, 'parameter_default': parameter_default, 'type': info, 'python_type': {'type': python_type, 'description': info.get('description', '')}, 'component': type.capitalize(), 'example_input': example})
