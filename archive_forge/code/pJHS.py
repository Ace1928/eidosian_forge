...
CENTRAL_MASS: float = 1.989e30  # Mass of a central body, e.g., the Sun
CENTRAL_POSITION: np.ndarray = np.array(
    [400, 300], dtype=np.float64
)  # Position for gravity pull
COLLISION_THRESHOLD: float = 10  # Threshold distance for collision detection
...
class Body:
    ...
    def __init__(self, mass: float, position: np.ndarray, velocity: np.ndarray):
        ...
        self.position: np.ndarray = position.astype(
            np.float64
        )  # Ensuring type consistency
        self.velocity: np.ndarray = velocity.astype(np.float64)

    def __repr__(self) -> str:
        return f"Body(mass={self.mass}, position={self.position.tolist()}, velocity={self.velocity.tolist()})"
...
class Simulation:
    ...
    def compute_gravitational_force(
        ...
        """Compute gravitational forces between all bodies and a central massive body."""
