...
class PandasInterface(Interface, PandasAPI):
    ...
    def init(cls, eltype, data, kdims, vdims):
        ...
        if util.is_dataframe(data):
            ...
            if kdims and vdims is None:
                ...
                if vdims is None:
                    vdims = [d for d in data.columns[ndim:ndim + nvdim if nvdim else None] if d not in kdims]
            ...
            if any((not isinstance(d, (str, Dimension)) for d in kdims + vdims)):
                raise DataError('Having a non-string as a column name in a DataFrame is not supported.')
            for kd in kdims:
            ...
            for d in kdims + vdims:
                ...
                if len([c for c in data.columns if c == d]) > 1:
                    raise DataError('Dimensions may not reference duplicated DataFrame columns (found duplicate %r columns). If you want to plot a column against itself simply declare two dimensions with the same name. ' % d, cls)
        else:
            ...
            if isinstance(data, dict) and all((c in data for c in columns)):
                ...
                data = None
            elif isinstance(data, dict) and (not all((d in data for d in columns))) and (not any((isinstance(v, np.ndarray) for v in data.values()))):
                ...
                column_data = zip(*(util.wrap_tuple(k) + util.wrap_tuple(v) for k, v in column_data))
            ...
            if isinstance(data, tuple):
                ...
                if any((d.ndim > 1 for d in data)):
                    ...
                    raise DataError('Data contains fewer columns than the %s element expects. Expected at least %d columns but found only %d columns.' % (eltype.__name__, min_dims, len(data)))
            ...
            elif isinstance(data, dict) and any((c not in data for c in columns)) or (isinstance(data, list) and any((isinstance(d, dict) and c not in d for d in data for c in columns))):
    ...
    def range(cls, dataset, dimension):
        ...
        if column.dtype.kind == 'O':
            if not isinstance(dataset.data, pd.DataFrame) or util.pandas_version < Version('0.17.0'):
            ...
            if column.dtype.kind == 'M' and getattr(column.dtype, 'tz', None):
                return (cmin.to_pydatetime().replace(tzinfo=None), cmax.to_pydatetime().replace(tzinfo=None))
            return (cmin, cmax)
    ...
    def groupby(cls, dataset, dimensions, container_type, group_type, **kwargs):
        ...
        if len(group_by) == 1 and util.pandas_version >= Version('1.5.0'):
            group_by = group_by[0]
        data = [(k, group_type(v, **group_kwargs)) for k, v in dataset.data.groupby(group_by, sort=False)]
    ...
    def aggregate(cls, dataset, dimensions, function, **kwargs):
        ...
        if len(dimensions):
            if function in [np.size]:
                ...
                numeric_cols = [c for c, d in zip(reindexed.columns, reindexed.dtypes) if is_numeric_dtype(d) and c not in cols]
