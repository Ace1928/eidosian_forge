...
def add_tag_option_to_parser_for_unset(parser, resource_name, enhance_help=lambda _h: _h):
    ...
    tag_group.add_argument('--tag', action='append', dest='tags', metavar='<tag>', help=enhance_help(_('Tag to be removed from the %s (repeat option to remove multiple tags)') % resource_name))
    tag_group.add_argument('--all-tag', action='store_true', help=enhance_help(_('Clear all tags associated with the %s') % resource_name))