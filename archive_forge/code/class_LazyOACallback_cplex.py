...
class LazyOACallback_cplex(cplex.callbacks.LazyConstraintCallback if cplex_available else object):
    ...
    def __call__(self):
        ...
        mindtpy_solver = self.mindtpy_solver
        config.logger.info('Solution source: {} (111 node_solution, 117 heuristic_solution, 119 mipstart_solution)'.format(self.get_solution_source()))
        if self.get_solution_source() != cplex.callbacks.SolutionSource.mipstart_solution and len(mindtpy_solver.mip_start_lazy_oa_cuts) > 0:
        ...
        if config.add_cuts_at_incumbent:
            self.copy_lazy_var_list_values(opt, main_mip.MindtPy_utils.variable_list, mindtpy_solver.mip.MindtPy_utils.variable_list, config)
            if config.strategy == 'OA':
                try:
                    self.add_lazy_oa_cuts(mindtpy_solver.mip, None, mindtpy_solver, config, opt)
                except ValueError as e:
                    config.logger.error(e, exc_info=True)
                    config.logger.error('Usually this error is caused by the MIP start solution causing a math domain error. We will skip it.')
                    return
        ...
        if abs(mindtpy_solver.primal_bound - mindtpy_solver.dual_bound) <= config.absolute_bound_tolerance:
            config.logger.info('MindtPy exiting on bound convergence. |Primal Bound: {} - Dual Bound: {}| <= (absolute tolerance {})  \n'.format(mindtpy_solver.primal_bound, mindtpy_solver.dual_bound, config.absolute_bound_tolerance))
        ...
        if mindtpy_solver.curr_int_sol in set(mindtpy_solver.integer_list):
            ...
            if config.strategy == 'GOA':
                if config.add_no_good_cuts:
                    var_values = list((v.value for v in mindtpy_solver.working_model.MindtPy_utils.variable_list))
                    self.add_lazy_no_good_cuts(var_values, mindtpy_solver, config, opt)
                return
        ...
        if fixed_nlp_result.solver.termination_condition in {tc.optimal, tc.locallyOptimal, tc.feasible}:
            ...
            self.handle_lazy_subproblem_infeasible(fixed_nlp, mindtpy_solver, config, opt)
        else:
            self.handle_lazy_subproblem_other_termination(fixed_nlp, fixed_nlp_result.solver.termination_condition, mindtpy_solver, config)