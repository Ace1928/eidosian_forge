            ...
            async with self._request_lock:
                if self._connection is None:
                    ...
                    if http2_negotiated or (self._http2 and (not self._http1)):
                        ...
                        self._connection = AsyncHTTP2Connection(origin=self._origin, stream=stream, keepalive_expiry=self._keepalive_expiry)
                    else:
                        self._connection = AsyncHTTP11Connection(origin=self._origin, stream=stream, keepalive_expiry=self._keepalive_expiry)
        ...
        while True:
            try:
                ...
                if self._origin.scheme in (b'https', b'wss'):
                    ...
                    kwargs = {'ssl_context': ssl_context, 'server_hostname': sni_hostname or self._origin.host.decode('ascii'), 'timeout': timeout}
