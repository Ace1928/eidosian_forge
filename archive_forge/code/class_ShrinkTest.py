...
class ShrinkTest(Base):
    ...
    @staticmethod
    def export() -> None:
        node = onnx.helper.make_node('Shrink', ['x'], ['y'], lambd=1.5, bias=1.5)
        graph = onnx.helper.make_graph(nodes=[node], name='Shrink', inputs=[onnx.helper.make_tensor_value_info('x', onnx.TensorProto.FLOAT, [5])], outputs=[onnx.helper.make_tensor_value_info('y', onnx.TensorProto.FLOAT, [5])])
