...
class InferenceApi:
    ...
    def __init__(self, repo_id: str, task: Optional[str]=None, token: Optional[str]=None, gpu: bool=False):
        ...
        if not model_info.pipeline_tag and (not task):
            raise ValueError('Task not specified in the repository. Please add it to the model card using pipeline_tag (https://huggingface.co/docs#how-is-a-models-type-of-inference-api-and-widget-determined)')
        if task and task != model_info.pipeline_tag:
            if task not in ALL_TASKS:
                raise ValueError(f"Invalid task {task}. Make sure it's valid.")
            logger.warning("You're using a different task than the one specified in the repository. Be sure to know what you're doing :)")
            self.task = task
        else:
    ...
    def __call__(self, inputs: Optional[Union[str, Dict, List[str], List[List[str]]]]=None, params: Optional[Dict]=None, data: Optional[bytes]=None, raw_response: bool=False) -> Any:
        ...
        """
        payload: Dict[str, Any] = {'options': self.options}
        if inputs:
        ...
        if content_type.startswith('image'):
            if not is_pillow_available():
                raise ImportError(f"Task '{self.task}' returned as image but Pillow is not installed. Please install it (`pip install Pillow`) or pass `raw_response=True` to get the raw `Response` object and parse the image by yourself.")
            from PIL import Image
            return Image.open(io.BytesIO(response.content))
        elif content_type == 'application/json':
            return response.json()
        else:
            raise NotImplementedError(f'{content_type} output type is not implemented yet. You can pass `raw_response=True` to get the raw `Response` object and parse the output by yourself.')