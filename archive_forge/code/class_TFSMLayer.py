    ...
    * If you need training-time behavior to differ from inference-time behavior
    ...
    def __init__(self, filepath, call_endpoint='serve', call_training_endpoint=None, trainable=True, name=None, dtype=None):
        if backend.backend() != 'tensorflow':
            raise NotImplementedError('The TFSMLayer is only currently supported with the TensorFlow backend.')
        ...
        if hasattr(self._reloaded_obj, call_endpoint):
            ...
            raise ValueError(f"The endpoint '{call_endpoint}' is neither an attribute of the reloaded SavedModel, nor an entry in the `signatures` field of the reloaded SavedModel. Select another endpoint via the `call_endpoint` argument. Available endpoints for this SavedModel: {list(self._reloaded_obj.signatures.keys())}")
        if call_training_endpoint:
            if hasattr(self._reloaded_obj, call_training_endpoint):
                ...
                raise ValueError(f"The endpoint '{call_training_endpoint}' is neither an attribute of the reloaded SavedModel, nor an entry in the `signatures` field of the reloaded SavedModel. Available endpoints for this SavedModel: {list(self._reloaded_obj.signatures.keys())}")
