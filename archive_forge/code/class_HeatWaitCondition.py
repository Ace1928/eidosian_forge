...
class HeatWaitCondition(resource.Resource):
    ...
    properties_schema = {HANDLE: properties.Schema(properties.Schema.STRING, _('A reference to the wait condition handle used to signal this wait condition.'), required=True), TIMEOUT: properties.Schema(properties.Schema.NUMBER, _('The number of seconds to wait for the correct number of signals to arrive.'), required=True, constraints=[constraints.Range(1, 43200)]), COUNT: properties.Schema(properties.Schema.INTEGER, _('The number of success signals that must be received before the stack creation process continues.'), constraints=[constraints.Range(min=1)], default=1, update_allowed=True)}
    ...
    def _wait(self, handle, started_at, timeout_in):
        if timeutils.is_older_than(started_at, timeout_in):
            ...
            LOG.info('%(name)s Timed out (%(timeout)s)', {'name': str(self), 'timeout': str(exc)})
            raise exc
    ...
    def _resolve_attribute(self, key):
        ...
        if key == self.DATA:
            ...
            LOG.debug('%(name)s.GetAtt(%(key)s) == %(res)s' % {'name': self.name, 'key': key, 'res': res})
