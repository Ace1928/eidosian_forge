...
class NttCisLBDriver(Driver):
    ...
    api_version = 1.0
    _VALUE_TO_ALGORITHM_MAP = {'ROUND_ROBIN': Algorithm.ROUND_ROBIN, 'LEAST_CONNECTIONS_MEMBER': Algorithm.LEAST_CONNECTIONS_MEMBER, 'LEAST_CONNECTIONS_NODE': Algorithm.LEAST_CONNECTIONS_NODE, 'OBSERVED_MEMBER': Algorithm.OBSERVED_MEMBER, 'OBSERVED_NODE': Algorithm.OBSERVED_NODE, 'PREDICTIVE_MEMBER': Algorithm.PREDICTIVE_MEMBER, 'PREDICTIVE_NODE': Algorithm.PREDICTIVE_NODE}
    _ALGORITHM_TO_VALUE_MAP = reverse_dict(_VALUE_TO_ALGORITHM_MAP)
    _VALUE_TO_STATE_MAP = {'NORMAL': State.RUNNING, 'PENDING_ADD': State.PENDING, 'PENDING_CHANGE': State.PENDING, 'PENDING_DELETE': State.PENDING, 'FAILED_ADD': State.ERROR, 'FAILED_CHANGE': State.ERROR, 'FAILED_DELETE': State.ERROR, 'REQUIRES_SUPPORT': State.ERROR}

    def __init__(self, key, network_domain_id, secret=None, secure=True, host=None, port=None, api_version=None, region=DEFAULT_REGION, **kwargs):
    ...
    def create_balancer(self, name, listener_port=None, port=None, protocol=None, algorithm=None, members=None, optimization_profile='TCP', ex_listener_ip_address=None):
        """
        ...
        if members is not None:
            for member in members:
                if not isinstance(member, Member):
                    member = self.ex_create_node(network_domain_id=network_domain_id, name=member.name, ip=member.private_ips[0], ex_description=None)
                self.ex_create_pool_member(pool=pool, node=member, port=port)
        listener = self.ex_create_virtual_listener(network_domain_id=network_domain_id, name=name, ex_description=name, port=listener_port, pool=pool, protocol=protocol, optimization_profile=optimization_profile, listener_ip_address=ex_listener_ip_address)
    ...
    def ex_delete_ssl_certificate_chain(self, cert_chain_id):
        ...
        :return ``bool``
        """
        del_cert_chain_elem = ET.Element('deleteSslCertificateChain', {'id': cert_chain_id, 'xmlns': TYPES_URN})
        result = self.connection.request_with_orgId_api_2('networkDomainVip/deleteSslCertificateChain', method='POST', data=ET.tostring(del_cert_chain_elem)).object
        response_code = findtext(result, 'responseCode', TYPES_URN)
        return response_code in ['IN_PROGRESS', 'OK']
    ...
    def ex_get_default_health_monitors(self, network_domain):
        ...
        """
        result = self.connection.request_with_orgId_api_2(action='networkDomainVip/defaultHealthMonitor', params={'networkDomainId': network_domain}, method='GET').object
    ...
    def _to_health_monitor(self, element):
        return NttCisDefaultHealthMonitor(id=element.get('id'), name=findtext(element, 'name', TYPES_URN), node_compatible=bool(findtext(element, 'nodeCompatible', TYPES_URN) == 'true'), pool_compatible=bool(findtext(element, 'poolCompatible', TYPES_URN) == 'true'))
    ...
    def _to_node(self, element):
        ...
        try:
            ...
            hm = None
        node = NttCisVIPNode(id=element.get('id'), name=name, status=self._VALUE_TO_STATE_MAP.get(findtext(element, 'state', TYPES_URN), State.UNKNOWN), health_monitor=hm, connection_rate_limit=findtext(element, 'connectionRateLimit', TYPES_URN), connection_limit=findtext(element, 'connectionLimit', TYPES_URN), ip=ipaddress)
        return node
    ...
    def _to_balancer(self, element):
        ...
        balancer = LoadBalancer(id=element.get('id'), name=name, state=self._VALUE_TO_STATE_MAP.get(findtext(element, 'state', TYPES_URN), State.UNKNOWN), ip=ipaddress, port=port, driver=self.connection.driver, extra=extra)
        return balancer
    ...
    def _to_member(self, element):
        ...
        if port is not None:
            port = int(port)
        pool_member = NttCisPoolMember(id=element.get('id'), name=element.find(fixxpath('node', TYPES_URN)).get('name'), status=findtext(element, 'state', TYPES_URN), node_id=element.find(fixxpath('node', TYPES_URN)).get('id'), ip=element.find(fixxpath('node', TYPES_URN)).get('ipAddress'), port=port)
        return pool_member
