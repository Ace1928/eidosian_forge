...
class SQSConnection(AWSQueryConnection):
    ...
    DefaultRegionEndpoint = boto.config.get('Boto', 'sqs_region_endpoint', 'queue.amazonaws.com')
    ...
    def __init__(self, aws_access_key_id=None, aws_secret_access_key=None, is_secure=True, port=None, proxy=None, proxy_port=None, proxy_user=None, proxy_pass=None, debug=0, https_connection_factory=None, region=None, path='/', security_token=None, validate_certs=True, profile_name=None):
        ...
        self.region = region
        super(SQSConnection, self).__init__(aws_access_key_id, aws_secret_access_key, is_secure, port, proxy, proxy_port, proxy_user, proxy_pass, self.region.endpoint, debug, https_connection_factory, path, security_token=security_token, validate_certs=validate_certs, profile_name=profile_name)
        ...
        if message_attributes is not None:
            ...
            for i, name in enumerate(keys, start=1):
                ...
                if 'data_type' in attribute:
                    params['MessageAttribute.%s.Value.DataType' % i] = attribute['data_type']
                if 'string_value' in attribute:
                    params['MessageAttribute.%s.Value.StringValue' % i] = attribute['string_value']
                if 'binary_value' in attribute:
                    params['MessageAttribute.%s.Value.BinaryValue' % i] = attribute['binary_value']
                if 'string_list_value' in attribute:
                    params['MessageAttribute.%s.Value.StringListValue' % i] = attribute['string_list_value']
                if 'binary_list_value' in attribute:
                    params['MessageAttribute.%s.Value.BinaryListValue' % i] = attribute['binary_list_value']
