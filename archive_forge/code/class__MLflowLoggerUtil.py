...
class _MLflowLoggerUtil:
    ...
    def setup_mlflow(self, tracking_uri: Optional[str]=None, registry_uri: Optional[str]=None, experiment_id: Optional[str]=None, experiment_name: Optional[str]=None, tracking_token: Optional[str]=None, artifact_location: Optional[str]=None, create_experiment_if_not_exists: bool=True):
        """
        Sets up MLflow.
        ...
        experiment_id = experiment_id if experiment_id is not None else os.environ.get('MLFLOW_EXPERIMENT_ID')
        if experiment_id is not None:
            from mlflow.exceptions import MlflowException
            try:
                self._mlflow.get_experiment(experiment_id=experiment_id)
                logger.debug(f'Experiment with provided id {experiment_id} exists. Setting that as the experiment.')
                self.experiment_id = experiment_id
                return
            except MlflowException:
                pass
        experiment_name = experiment_name if experiment_name is not None else os.environ.get('MLFLOW_EXPERIMENT_NAME')
        if experiment_name is not None and self._mlflow.get_experiment_by_name(name=experiment_name):
            logger.debug(f'Experiment with provided name {experiment_name} exists. Setting that as the experiment.')
            self.experiment_id = self._mlflow.get_experiment_by_name(experiment_name).experiment_id
            return
        if experiment_name and create_experiment_if_not_exists:
            logger.debug(f'Existing experiment not found. Creating new experiment with name: {experiment_name}')
            self.experiment_id = self._mlflow.create_experiment(name=experiment_name, artifact_location=artifact_location)
            return
        if create_experiment_if_not_exists:
            raise ValueError(f'Experiment with the provided experiment_id: {experiment_id} does not exist and no experiment_name provided. At least one of these has to be provided.')
        else:
            raise ValueError(f'Experiment with the provided experiment_id: {experiment_id} or experiment_name: {experiment_name} does not exist. Please create an MLflow experiment and provide either its id or name.')
