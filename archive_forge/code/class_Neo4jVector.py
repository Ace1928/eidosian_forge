...
class Neo4jVector(VectorStore):
    ...
    def __init__(self, embedding: Embeddings, *, search_type: SearchType=SearchType.VECTOR, username: Optional[str]=None, password: Optional[str]=None, url: Optional[str]=None, keyword_index_name: Optional[str]='keyword', database: Optional[str]=None, index_name: str='vector', node_label: str='Chunk', embedding_node_property: str='embedding', text_node_property: str='text', distance_strategy: DistanceStrategy=DEFAULT_DISTANCE_STRATEGY, logger: Optional[logging.Logger]=None, pre_delete_collection: bool=False, retrieval_query: str='', relevance_score_fn: Optional[Callable[[float], float]]=None, index_type: IndexType=DEFAULT_INDEX_TYPE) -> None:
        ...
        self.verify_version()
        check_if_not_null(['index_name', 'node_label', 'embedding_node_property', 'text_node_property'], [index_name, node_label, embedding_node_property, text_node_property])
        ...
        if pre_delete_collection:
            ...
            self.query(f'MATCH (n:`{self.node_label}`) CALL {{ WITH n DETACH DELETE n }} IN TRANSACTIONS OF 10000 ROWS;')
            try:
    ...
    def retrieve_existing_fts_index(self, text_node_properties: List[str]=[]) -> Optional[str]:
        ...
        """
        index_information = self.query("SHOW INDEXES YIELD name, type, labelsOrTypes, properties, options WHERE type = 'FULLTEXT' AND (name = $keyword_index_name OR (labelsOrTypes = [$node_label] AND properties = $text_node_property)) RETURN name, labelsOrTypes, properties, options ", params={'keyword_index_name': self.keyword_index_name, 'node_label': self.node_label, 'text_node_property': text_node_properties or [self.text_node_property]})
        index_information = sort_by_index_name(index_information, self.index_name)
        try:
    ...
    @classmethod
    def __from(cls, texts: List[str], embeddings: List[List[float]], embedding: Embeddings, metadatas: Optional[List[dict]]=None, ids: Optional[List[str]]=None, create_id_index: bool=True, search_type: SearchType=SearchType.VECTOR, **kwargs: Any) -> Neo4jVector:
        if ids is None:
        ...
        if create_id_index:
            store.query(f'CREATE CONSTRAINT IF NOT EXISTS FOR (n:`{store.node_label}`) REQUIRE n.id IS UNIQUE;')
    ...
    def add_embeddings(self, texts: Iterable[str], embeddings: List[List[float]], metadatas: Optional[List[dict]]=None, ids: Optional[List[str]]=None, **kwargs: Any) -> List[str]:
        ...
        if not metadatas:
            metadatas = [{} for _ in texts]
        import_query = f"UNWIND $data AS row CALL {{ WITH row MERGE (c:`{self.node_label}` {{id: row.id}}) WITH c, row CALL db.create.setVectorProperty(c, '{self.embedding_node_property}', row.embedding) YIELD node SET c.`{self.text_node_property}` = row.text SET c += row.metadata }} IN TRANSACTIONS OF 1000 ROWS"
        parameters = {'data': [{'text': text, 'metadata': metadata, 'embedding': embedding, 'id': id} for text, metadata, embedding, id in zip(texts, metadatas, embeddings, ids)]}
    ...
    def similarity_search(self, query: str, k: int=4, params: Dict[str, Any]={}, filter: Optional[Dict[str, Any]]=None, **kwargs: Any) -> List[Document]:
        ...
        return self.similarity_search_by_vector(embedding=embedding, k=k, query=query, params=params, filter=filter, **kwargs)

    def similarity_search_with_score(self, query: str, k: int=4, params: Dict[str, Any]={}, filter: Optional[Dict[str, Any]]=None, **kwargs: Any) -> List[Tuple[Document, float]]:
        ...
        docs = self.similarity_search_with_score_by_vector(embedding=embedding, k=k, query=query, params=params, filter=filter, **kwargs)
        return docs

    def similarity_search_with_score_by_vector(self, embedding: List[float], k: int=4, filter: Optional[Dict[str, Any]]=None, params: Dict[str, Any]={}, **kwargs: Any) -> List[Tuple[Document, float]]:
        """
        ...
        if filter:
            ...
            base_index_query = parallel_query + f'MATCH (n:`{self.node_label}`) WHERE n.`{self.embedding_node_property}` IS NOT NULL AND size(n.`{self.embedding_node_property}`) = toInteger({self.embedding_dimension}) AND '
            base_cosine_query = f' WITH n as node, vector.similarity.cosine(n.`{self.embedding_node_property}`, $embedding) AS score ORDER BY score DESC LIMIT toInteger($k) '
        ...
        if self._index_type == IndexType.RELATIONSHIP:
            default_retrieval = f'RETURN relationship.`{self.text_node_property}` AS text, score, relationship {{.*, `{self.text_node_property}`: Null, `{self.embedding_node_property}`: Null, id: Null }} AS metadata'
        else:
            default_retrieval = f'RETURN node.`{self.text_node_property}` AS text, score, node {{.*, `{self.text_node_property}`: Null, `{self.embedding_node_property}`: Null, id: Null }} AS metadata'
        ...
        parameters = {'index': self.index_name, 'k': k, 'embedding': embedding, 'keyword_index': self.keyword_index_name, 'query': remove_lucene_chars(kwargs['query']), **params, **filter_params}
        results = self.query(read_query, params=parameters)
        docs = [(Document(page_content=dict_to_yaml_str(result['text']) if isinstance(result['text'], dict) else result['text'], metadata={k: v for k, v in result['metadata'].items() if v is not None}), result['score']) for result in results]
        return docs
    ...
    def similarity_search_by_vector(self, embedding: List[float], k: int=4, filter: Optional[Dict[str, Any]]=None, **kwargs: Any) -> List[Document]:
        ...
        """
        docs_and_scores = self.similarity_search_with_score_by_vector(embedding=embedding, k=k, filter=filter, **kwargs)
        return [doc for doc, _ in docs_and_scores]

    @classmethod
    def from_texts(cls: Type[Neo4jVector], texts: List[str], embedding: Embeddings, metadatas: Optional[List[dict]]=None, distance_strategy: DistanceStrategy=DEFAULT_DISTANCE_STRATEGY, ids: Optional[List[str]]=None, **kwargs: Any) -> Neo4jVector:
        """
        ...
        return cls.__from(texts, embeddings, embedding, metadatas=metadatas, ids=ids, distance_strategy=distance_strategy, **kwargs)

    @classmethod
    def from_embeddings(cls, text_embeddings: List[Tuple[str, List[float]]], embedding: Embeddings, metadatas: Optional[List[dict]]=None, distance_strategy: DistanceStrategy=DEFAULT_DISTANCE_STRATEGY, ids: Optional[List[str]]=None, pre_delete_collection: bool=False, **kwargs: Any) -> Neo4jVector:
        ...
        return cls.__from(texts, embeddings, embedding, metadatas=metadatas, ids=ids, distance_strategy=distance_strategy, pre_delete_collection=pre_delete_collection, **kwargs)

    @classmethod
    def from_existing_index(cls: Type[Neo4jVector], embedding: Embeddings, index_name: str, search_type: SearchType=DEFAULT_SEARCH_TYPE, keyword_index_name: Optional[str]=None, **kwargs: Any) -> Neo4jVector:
        """
        ...
        store = cls(embedding=embedding, index_name=index_name, keyword_index_name=keyword_index_name, search_type=search_type, **kwargs)
        embedding_dimension, index_type = store.retrieve_existing_index()
        if index_type == 'RELATIONSHIP':
            raise ValueError('Relationship vector index is not supported with `from_existing_index` method. Please use the `from_existing_relationship_index` method.')
        if not embedding_dimension:
        ...
        if search_type == SearchType.HYBRID:
            fts_node_label = store.retrieve_existing_fts_index()
            if not fts_node_label:
                raise ValueError('The specified keyword index name does not exist. Make sure to check if you spelled it correctly')
            elif not fts_node_label == store.node_label:
                raise ValueError("Vector and keyword index don't index the same node label")
        return store
    ...
    @classmethod
    def from_documents(cls: Type[Neo4jVector], documents: List[Document], embedding: Embeddings, distance_strategy: DistanceStrategy=DEFAULT_DISTANCE_STRATEGY, ids: Optional[List[str]]=None, **kwargs: Any) -> Neo4jVector:
        """
        Return Neo4jVector initialized from documents and embeddings.
        Neo4j credentials are required in the form of `url`, `username`,
        and `password` and optional `database` parameters.
        ...
        return cls.from_texts(texts=texts, embedding=embedding, distance_strategy=distance_strategy, metadatas=metadatas, ids=ids, **kwargs)
