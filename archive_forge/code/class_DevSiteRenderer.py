...
class DevSiteRenderer(html_renderer.HTMLRenderer):
  ...
  """
    ...
    """
        ...
        self._out.write('\n<section id="{document_id}">\n<dt>{heading}</dt>\n<dd class="sectionbody">\n'.format(document_id=self.GetDocumentID(heading), heading=heading))
    ...
    """
        ...
        if not self._example:
            ...
            if not self._lang:
                ...
                self._out.write('<pre class="prettyprint lang-{lang} wrap-code">\n'.format(lang=self._lang))
        indent = len(line)
        line = line.lstrip()
        indent -= len(line)
        command_pattern = re.compile('\\A\\$\\s+')
    ...
    """
        if target != self.command[0] and ('/' not in target or ':' in target or '#' in target or target.startswith('www.') or target.endswith('/..')):
        ...
        if len(target_parts) > 1 and target_parts[1] == 'meta':
            return target + ' --help'
        return '<a href="/sdk/{head}/{tail}">{text}</a>'.format(head=target_parts[0], tail='/'.join(['reference'] + target_parts[1:]), text=text or target)
    ...
    """
        return re.sub('(--[-a-z]+)', '<code><a href="/sdk/{}/reference/#\\1">\\1</a></code>'.format(self.command[0]), line)