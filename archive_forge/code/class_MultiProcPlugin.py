...
class MultiProcPlugin(DistributedPluginBase):
    ...
    def __init__(self, plugin_args=None):
        ...
        logger.debug('[MultiProc] Starting (n_procs=%d, mem_gb=%0.2f, cwd=%s)', self.processors, self.memory_gb, self._cwd)
        try:
            mp_context = mp.get_context(self.plugin_args.get('mp_context'))
            self.pool = ProcessPoolExecutor(max_workers=self.processors, initializer=process_initializer, initargs=(self._cwd,), mp_context=mp_context)
    ...
    def _send_procs_to_workers(self, updatehash=False, graph=None):
        ...
        if self._stats != stats:
            ...
            logger.info('[MultiProc] Running %d tasks, and %d jobs ready. Free memory (GB): %0.2f/%0.2f, Free processors: %d/%d.%s', len(self.pending_tasks), len(jobids), free_memory_gb, self.memory_gb, free_processors, self.processors, tasks_list_msg)
            self._stats = stats
