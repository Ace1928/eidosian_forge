...
class CinderQuota(resource.Resource):
    ...
    PROPERTIES = PROJECT, GIGABYTES, VOLUMES, BACKUPS, BACKUPS_GIGABYTES, SNAPSHOTS = ('project', 'gigabytes', 'volumes', 'backups', 'backup_gigabytes', 'snapshots')
    properties_schema = {PROJECT: properties.Schema(properties.Schema.STRING, _('OpenStack Keystone Project.'), required=True, constraints=[constraints.CustomConstraint('keystone.project')]), GIGABYTES: properties.Schema(properties.Schema.INTEGER, _('Quota for the amount of disk space (in Gigabytes). Setting the value to -1 removes the limit.'), constraints=[constraints.Range(min=-1)], update_allowed=True), VOLUMES: properties.Schema(properties.Schema.INTEGER, _('Quota for the number of volumes. Setting the value to -1 removes the limit.'), constraints=[constraints.Range(min=-1)], update_allowed=True), BACKUPS: properties.Schema(properties.Schema.INTEGER, _('Quota for the number of backups. Setting the value to -1 removes the limit.'), support_status=support.SupportStatus(version='16.0.0'), constraints=[constraints.Range(min=-1)], update_allowed=True), BACKUPS_GIGABYTES: properties.Schema(properties.Schema.INTEGER, _('Quota for the amount of backups disk space (in Gigabytes). Setting the value to -1 removes the limit.'), support_status=support.SupportStatus(version='16.0.0'), constraints=[constraints.Range(min=-1)], update_allowed=True), SNAPSHOTS: properties.Schema(properties.Schema.INTEGER, _('Quota for the number of snapshots. Setting the value to -1 removes the limit.'), constraints=[constraints.Range(min=-1)], update_allowed=True)}
    ...
    def validate(self):
        ...
        if sum((1 for p in self.properties.values() if p is not None)) <= 1:
            raise exception.PropertyUnspecifiedError(self.GIGABYTES, self.SNAPSHOTS, self.VOLUMES, self.BACKUPS, self.BACKUPS_GIGABYTES)