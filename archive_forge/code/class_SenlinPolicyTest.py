...
class SenlinPolicyTest(common.HeatTestCase):
    ...
    def setUp(self):
        ...
        self.patchobject(senlin.PolicyTypeConstraint, 'validate', return_value=True)
        self.senlin_mock = mock.MagicMock(spec=Proxy)
        self.senlin_mock.get_cluster.return_value = mock.Mock(id='c1_id')
        self.patchobject(policy.Policy, 'client', return_value=self.senlin_mock)
    ...
    def _create_policy(self, template):
        ...
        self.senlin_mock.attach_policy_to_cluster.return_value = {'action': 'fake_action'}
        self.senlin_mock.get_action.return_value = mock.Mock(status='SUCCEEDED')
        scheduler.TaskRunner(policy.create)()
        self.assertEqual((policy.CREATE, policy.COMPLETE), policy.state)
        self.assertEqual(self.fake_p.id, policy.resource_id)
        self.senlin_mock.attach_policy_to_cluster.assert_called_once_with('c1_id', policy.resource_id, enabled=True)
        self.senlin_mock.get_action.assert_called_once_with('fake_action')
        return policy

    def test_policy_create(self):
        self._create_policy(self.t)
        expect_kwargs = {'name': 'SenlinPolicy', 'spec': policy_spec}
        self.senlin_mock.create_policy.assert_called_once_with(**expect_kwargs)
    ...
    def test_policy_create_fail(self):
        ...
        self.senlin_mock.create_policy.return_value = self.fake_p
        self.senlin_mock.attach_policy_to_cluster.return_value = {'action': 'fake_action'}
        self.senlin_mock.get_action.return_value = mock.Mock(status='FAILED', status_reason='oops', action='CLUSTER_ATTACH_POLICY')
        create_task = scheduler.TaskRunner(policy.create)
        self.assertRaises(exception.ResourceFailure, create_task)
        self.assertEqual((policy.CREATE, policy.FAILED), policy.state)
        err_msg = 'ResourceInError: resources.senlin-policy: Went to status FAILED due to "Failed to execute CLUSTER_ATTACH_POLICY for c1_id: oops"'
    ...
    def test_policy_update(self):
        policy = self._create_policy(self.t)
        self.senlin_mock.get_cluster.side_effect = [mock.Mock(id='c2_id'), mock.Mock(id='c1_id'), mock.Mock(id='c2_id')]
        ...
        self.senlin_mock.attach_policy_to_cluster.return_value = {'action': 'fake_action1'}
        self.senlin_mock.detach_policy_from_cluster.return_value = {'action': 'fake_action2'}
        ...
        self.senlin_mock.update_policy.assert_called_once_with(self.fake_p, name='new_name')
        self.senlin_mock.detach_policy_from_cluster.assert_called_once_with('c1_id', policy.resource_id)
        self.senlin_mock.attach_policy_to_cluster.assert_called_with('c2_id', policy.resource_id, enabled=True)
