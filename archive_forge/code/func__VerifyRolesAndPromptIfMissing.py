...
def _VerifyRolesAndPromptIfMissing(project_id, account, applied_roles, required_roles):
  ...
  """
    ...
    add_roles = console_io.PromptContinue(message=prompt_message, prompt_string='Would you like to add the permissions', throw_if_unattended=True)
    if not add_roles:
        return
    for role in sorted(missing_roles):
        ...
        try:
            ...
            log.warning('Your account does not have permission to add roles to the service account {0}. If import fails, ensure "{0}" has the roles "{1}" before retrying.'.format(account, required_roles))
            return