...
class TestPortForwarding(base.BaseFunctionalTest):
    ...
    def setUp(self):
        ...
        self.FIP_ID = fip.id
        pf = self.user_cloud.network.create_port_forwarding(floatingip_id=self.FIP_ID, internal_port_id=self.INTERNAL_PORT_ID, internal_ip_address=self.INTERNAL_IP_ADDRESS, internal_port=self.INTERNAL_PORT, external_port=self.EXTERNAL_PORT, protocol=self.PROTOCOL, description=self.DESCRIPTION)
        assert isinstance(pf, _port_forwarding.PortForwarding)
        self.PF = pf

    def tearDown(self):
        sot = self.user_cloud.network.delete_port_forwarding(self.PF, self.FIP_ID, ignore_missing=False)
    ...
    def test_update(self):
        ...
        sot = self.user_cloud.network.update_port_forwarding(self.PF.id, self.FIP_ID, external_port=NEW_EXTERNAL_PORT)
