...
class HealthChecker:
    ...
    def __init__(self, gcs_aio_client: GcsAioClient, local_node_address: Optional[str]=None):
    ...
    async def check_local_raylet_liveness(self) -> bool:
        if self._local_node_address is None:
            return False
        liveness = await self._gcs_aio_client.check_alive([self._local_node_address.encode()], 0.1)
        return liveness[0]
