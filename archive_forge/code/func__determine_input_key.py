...
def _determine_input_key(config: smith_eval.RunEvalConfig, run_inputs: Optional[List[str]]) -> Optional[str]:
    input_key = None
    if config.input_key:
        input_key = config.input_key
        if run_inputs and input_key not in run_inputs:
            logger.warning(f"Input key {input_key} not in chain's specified input keys {run_inputs}. Evaluation behavior may be undefined.")
    elif run_inputs and len(run_inputs) == 1:
        input_key = run_inputs[0]
    elif run_inputs is not None and len(run_inputs) > 1:
        logger.warning(f'Chain expects multiple input keys: {run_inputs}, Evaluator is likely to fail. Evaluation behavior may be undefined. Specify an input_key in the RunEvalConfig to avoid this warning.')
    return input_key