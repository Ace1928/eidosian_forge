...
@async_error_handler
async def download_audio_track(save_path: AudioPath) -> None:
    """
    ...
    Args:
        save_path (AudioPath): The path where the audio track will be saved.
    """
    app = QApplication([])
...
async def convert_mp3_to_wav(
    mp3_file_path: AudioPath, wav_file_path: AudioPath
) -> None:
    """
    Converts an MP3 file to WAV format using the pydub library, with added logging for tracking.

    Args:
        mp3_file_path (AudioPath): The path to the MP3 file.
        wav_file_path (AudioPath): The path where the WAV file will be saved.
    """
...
async def remove_vocals(
    input_file_path: AudioPath, output_file_path: AudioPath
) -> None:
    """
    Removes vocals from an audio file using the Open-Unmix model loaded via torch.hub.

    Args:
        input_file_path (AudioPath): The path to the input audio file.
        output_file_path (AudioPath): The path where the vocal-removed audio will be saved.
...
@async_error_handler
async def extract_lyrics(input_file_path: AudioPath) -> Lyrics:
    """
    Extracts lyrics from an audio file using the Google Speech Recognition API, with enhanced error handling.

    Args:
        input_file_path (AudioPath): The path to the audio file.

    Returns:
        Lyrics: The extracted lyrics or an error message.
    """
    r = sr.Recognizer()
    with sr.AudioFile(input_file_path) as source:
        audio = r.record(source)
    try:
        lyrics: Lyrics = r.recognize_google(audio, language="en-US")
...
@async_error_handler
async def get_audio_duration(wav_file_path: AudioPath) -> Duration:
    """
    Calculates the duration of an audio file in seconds, with precise logging for the calculated duration.

    Args:
        wav_file_path (AudioPath): The path to the WAV file.
    ...
    with contextlib.closing(wave.open(wav_file_path, "r")) as f:
        frames = f.getnframes()
        rate = f.getframerate()
        duration: Duration = frames / float(rate)
        logging.info(f"Audio duration calculated: {duration} seconds")
        return duration


@async_error_handler
async def create_subtitles(lyrics: Lyrics, duration: Duration) -> Subtitles:
    """
    Creates subtitles from the extracted lyrics based on the duration of the audio, with detailed logging.

    Args:
        lyrics (Lyrics): The extracted lyrics.
        duration (Duration): The duration of the audio in seconds.

    Returns:
        Subtitles: The generated subtitles in SRT format.
    """
    ...
    subtitles: Subtitles = srt.compose(subs)
...
async def main() -> None:
    ...
    """
    # Select MP3 file
    mp3_file_path: AudioPath = select_file_gui("Select MP3 File", "MP3 files (*.mp3)")
    if not mp3_file_path:
        logging.info("No MP3 file selected.")
        return

    # Specify paths for WAV and vocal-removed files
    wav_file_path: AudioPath = save_file_gui("Save WAV File As", "WAV files (*.wav)")
    vocal_removed_file_path: AudioPath = save_file_gui(
        "Save Vocal-Removed File As", "WAV files (*.wav)"
    )
    subtitle_file_path: SubtitlePath = save_file_gui(
        "Save Subtitles File As", "SRT files (*.srt)"
    )
    ...
    lyrics: Lyrics = await extract_lyrics(wav_file_path)
    duration: Duration = await get_audio_duration(vocal_removed_file_path)
    subtitles: Subtitles = await create_subtitles(lyrics, duration)
