...
class PrivateConnectionsClient(util.VmwareClientBase):
    ...
    def Get(self, resource):
        request = self.messages.VmwareengineProjectsLocationsPrivateConnectionsGetRequest(name=resource.RelativeName())
        response = self.service.Get(request)
        return response

    def Create(self, resource, vmware_engine_network, service_project, private_connection_type, routing_mode=None, description=None, service_network=None):
        parent = resource.Parent().RelativeName()
        project = resource.Parent().Parent().Name()
        private_connection_id = resource.Name()
        private_connection = self.messages.PrivateConnection(description=description)
        ...
        type_enum = arg_utils.ChoiceEnumMapper(arg_name='type', message_enum=self.messages.PrivateConnection.TypeValueValuesEnum, include_filter=lambda x: 'TYPE_UNSPECIFIED' not in x).GetEnumForChoice(arg_utils.EnumNameToChoice(private_connection_type))
        private_connection.type = type_enum
        service_network = self.GetServiceNetwork(type_enum, service_network)
        private_connection.serviceNetwork = 'projects/{project}/global/networks/{network_id}'.format(project=service_project, network_id=service_network)
        request = self.messages.VmwareengineProjectsLocationsPrivateConnectionsCreateRequest(parent=parent, privateConnection=private_connection, privateConnectionId=private_connection_id)
    ...
    def Update(self, resource, description=None, routing_mode=None):
        ...
        request = self.messages.VmwareengineProjectsLocationsPrivateConnectionsPatchRequest(privateConnection=private_connection, name=resource.RelativeName(), updateMask=','.join(update_mask))
        return self.service.Patch(request)

    def Delete(self, resource):
        request = self.messages.VmwareengineProjectsLocationsPrivateConnectionsDeleteRequest(name=resource.RelativeName())
        return self.service.Delete(request)

    def List(self, location_resource):
        location = location_resource.RelativeName()
        request = self.messages.VmwareengineProjectsLocationsPrivateConnectionsListRequest(parent=location)
        return list_pager.YieldFromList(self.service, request, field='privateConnections', batch_size_attribute='pageSize')
