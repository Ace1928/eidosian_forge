...
class ClassicallyControlledOperation(raw_types.Operation):
    ...
    def __init__(self, sub_operation: 'cirq.Operation', conditions: Sequence[Union[str, 'cirq.MeasurementKey', 'cirq.Condition', sympy.Basic]]):
        ...
        if protocols.measurement_key_objs(sub_operation):
            raise ValueError(f'Cannot conditionally run operations with measurements: {sub_operation}')
    ...
    def with_qubits(self, *new_qubits):
        return self._sub_operation.with_qubits(*new_qubits).with_classical_controls(*self._conditions)

    def _decompose_(self):
        return self._decompose_with_context_()

    def _decompose_with_context_(self, context: Optional['cirq.DecompositionContext']=None):
        result = protocols.decompose_once(self._sub_operation, NotImplemented, flatten=False, context=context)
        if result is NotImplemented:
            return NotImplemented
        return op_tree.transform_op_tree(result, lambda op: ClassicallyControlledOperation(op, self._conditions))
    ...
    def __repr__(self):
        return f'cirq.ClassicallyControlledOperation({self._sub_operation!r}, {list(self._conditions)!r})'
    ...
    def _resolve_parameters_(self, resolver: 'cirq.ParamResolver', recursive: bool) -> 'ClassicallyControlledOperation':
    ...
    def _circuit_diagram_info_(self, args: 'cirq.CircuitDiagramInfoArgs') -> Optional['protocols.CircuitDiagramInfo']:
        sub_args = protocols.CircuitDiagramInfoArgs(known_qubit_count=args.known_qubit_count, known_qubits=args.known_qubits, use_unicode_characters=args.use_unicode_characters, precision=args.precision, label_map=args.label_map)
        ...
        if control_label_count == 0 or any((not isinstance(c, value.KeyCondition) for c in self._conditions)):
            wire_symbols = (wire_symbols[0] + '(conditions=[' + ', '.join((str(c) for c in self._conditions)) + '])',) + wire_symbols[1:]
    ...
    def _with_measurement_key_mapping_(self, key_map: Mapping[str, str]) -> 'ClassicallyControlledOperation':
    ...
    def _with_rescoped_keys_(self, path: Tuple[str, ...], bindable_keys: FrozenSet['cirq.MeasurementKey']) -> 'ClassicallyControlledOperation':
    ...
    def _control_keys_(self) -> FrozenSet['cirq.MeasurementKey']:
        local_keys: FrozenSet['cirq.MeasurementKey'] = frozenset((k for condition in self._conditions for k in condition.keys))
