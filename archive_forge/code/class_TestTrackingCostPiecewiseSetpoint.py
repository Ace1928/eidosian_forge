...
class TestTrackingCostPiecewiseSetpoint(unittest.TestCase):
    ...
    def test_piecewise_penalty_no_weights(self):
        ...
        m.var_set, m.tracking_cost = get_penalty_from_piecewise_constant_target(variables, m.time, setpoint_data)
        for i in m.time:
            for j in m.var_set:
                if i <= 2:
                    pred_expr = (m.var[i, 'A'] - 2.0) ** 2 if j == 0 else (m.var[i, 'B'] - 3.0) ** 2
                else:
                    pred_expr = (m.var[i, 'A'] - 2.5) ** 2 if j == 0 else (m.var[i, 'B'] - 3.5) ** 2
