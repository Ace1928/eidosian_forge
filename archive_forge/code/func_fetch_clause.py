...
def fetch_clause(self, select, **kw):
    ...
    if select._fetch_clause is not None:
        text += '\n FETCH FIRST (%s)%s ROWS %s' % (self.process(select._fetch_clause, **kw), ' PERCENT' if select._fetch_clause_options['percent'] else '', 'WITH TIES' if select._fetch_clause_options['with_ties'] else 'ONLY')
    return text