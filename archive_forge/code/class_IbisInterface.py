...
class IbisInterface(Interface):
    ...
    zero_indexed_backend_modules = ['ibis.backends.omniscidb.client']
    ...
    def init(cls, eltype, data, keys, values):
        ...
        if keys and values is None:
            ...
            if values is None:
                values = [key for key in data.columns[ndim:ndim + nvdim if nvdim else None] if key not in keys]
    ...
    def values(cls, dataset, dimension, expanded=True, flat=True, compute=True, keep_index=False):
        ...
        if ibis_version() > Version('3') and isinstance(data, ibis.expr.types.AnyColumn) and (not expanded):
            data = dataset.data[[dimension.name]]
        if not expanded:
            data = data.distinct()
    ...
    def histogram(cls, expr, bins, density=True, weights=None):
        ...
        if weights is not None:
            raise NotImplementedError('Weighted histograms currently not implemented for IbisInterface.')
        if density:
    ...
    def groupby(cls, dataset, dimensions, container_type, group_type, **kwargs):
        ...
        data = [(tuple(s.values.tolist()), group_type(dataset.data.filter([dataset.data[k] == v for k, v in s.to_dict().items()]), **group_kwargs)) for i, s in groups.iterrows()]
    ...
    def add_dimension(cls, dataset, dimension, dim_pos, values, vdim):
        ...
        if dimension.name not in data.columns:
            if not isinstance(values, ibis.Expr) and (not np.isscalar(values)):
                raise ValueError('Cannot assign %s type as a Ibis table column, expecting either ibis.Expr or scalar.' % type(values).__name__)
    ...
    def isscalar(cls, dataset, dim):
        return dataset.data[dataset.get_dimension(dim, strict=True).name].distinct().count().compute() == 1
    ...
    def aggregate(cls, dataset, dimensions, function, **kwargs):
        ...
        if len(dimensions):
            ...
            if function is np.count_nonzero:
                aggregation = selection.aggregate(**{x: ibis.expr.operations.Count(new[x], where=new[x] != 0).to_expr() for x in new.columns if x not in columns})
            else:
                aggregation = selection.aggregate(**{x: function(new[x]).to_expr() for x in new.columns if x not in columns})
        else:
