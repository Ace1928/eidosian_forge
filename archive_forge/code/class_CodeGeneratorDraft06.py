...
class CodeGeneratorDraft06(CodeGeneratorDraft04):
    FORMAT_REGEXS = dict(CodeGeneratorDraft04.FORMAT_REGEXS, **{'json-pointer': '^(/(([^/~])|(~[01]))*)*\\Z', 'uri-reference': '^(\\w+:(\\/?\\/?))?[^#\\\\\\s]*(#[^\\\\\\s]*)?\\Z', 'uri-template': '^(?:(?:[^\\x00-\\x20\\"\\\'<>%\\\\^`{|}]|%[0-9a-f]{2})|\\{[+#./;?&=,!@|]?(?:[a-z0-9_]|%[0-9a-f]{2})+(?::[1-9][0-9]{0,3}|\\*)?(?:,(?:[a-z0-9_]|%[0-9a-f]{2})+(?::[1-9][0-9]{0,3}|\\*)?)*\\})*\\Z'})
    ...
    def __init__(self, definition, resolver=None, formats={}, use_default=True, use_formats=True):
        ...
        self._json_keywords_to_function.update((('exclusiveMinimum', self.generate_exclusive_minimum), ('exclusiveMaximum', self.generate_exclusive_maximum), ('propertyNames', self.generate_property_names), ('contains', self.generate_contains), ('const', self.generate_const)))
