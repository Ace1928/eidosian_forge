...
class TestMistralWorkflow(common.HeatTestCase):
    ...
    def setUp(self):
        ...
        self.patchobject(workflow.Workflow, 'client', return_value=self.mistral)
        self.patches = []
        self.patches.append(mock.patch.object(stack_user.StackUser, '_create_user'))
        self.patches.append(mock.patch.object(signal_responder.SignalResponder, '_create_keypair'))
        self.patches.append(mock.patch.object(client.MistralClientPlugin, '_create'))
    ...
    def _create_resource(self, name, template=workflow_template):
        ...
        self.mistral.workflows.create.return_value = [FakeWorkflow('test_stack-workflow-b5fiekfci3yc')]
    ...
    def test_attributes(self):
        wf = self._create_resource('workflow')
        self.mistral.workflows.get.return_value = FakeWorkflow('test_stack-workflow-b5fiekfci3yc')
        self.assertEqual({'name': 'test_stack-workflow-b5fiekfci3yc', 'input': None}, wf.FnGetAtt('data'))
    ...
    def test_wrong_params_using(self):
        error_msg = "Mistral resource validation error: workflow.properties.params: 'task_name' is not assigned in 'params' in case of reverse type workflow."
    ...
    def test_update_replace(self):
        ...
        err = self.assertRaises(resource.UpdateReplace, scheduler.TaskRunner(wf.update, new_workflow))
    ...
    def test_update_failed_no_replace(self):
        ...
        new_wf = rsrc_defns['workflow']
        self.mistral.workflows.get.return_value = FakeWorkflow('test_stack-workflow-b5fiekfci3yc')
        self.mistral.workflows.update.side_effect = [Exception('boom!'), [FakeWorkflow('test_stack-workflow-b5fiekfci3yc')]]
        self.assertRaises(exception.ResourceFailure, scheduler.TaskRunner(wf.update, new_wf))
        self.assertEqual((wf.UPDATE, wf.FAILED), wf.state)
        scheduler.TaskRunner(wf.update, new_wf)()
        self.assertTrue(self.mistral.workflows.update.called)
    ...
    def test_delete_executions_not_found(self):
        wf = self._create_resource('workflow')
        self.mistral.executuions.delete.side_effect = [self.mistral.mistral_base.APIException(error_code=404), None]
        wf._data = {'executions': '1234,5678'}
        data_delete = self.patchobject(resource.Resource, 'data_delete')
        wf._delete_executions()
        self.assertEqual(2, self.mistral.executions.delete.call_count)
    ...
    def test_signal_wrong_input_and_params_type(self):
        ...
        details = {'input': '3'}
        err = self.assertRaises(exception.ResourceFailure, scheduler.TaskRunner(wf.signal, details))
        error_message = "StackValidationFailed: resources.create_vm: Signal data error: Input in signal data must be a map, find a <class 'str'>"
        self.assertEqual(error_message, str(err))
        details = {'params': '3'}
        err = self.assertRaises(exception.ResourceFailure, scheduler.TaskRunner(wf.signal, details))
        error_message = "StackValidationFailed: resources.create_vm: Signal data error: Params must be a map, find a <class 'str'>"
        self.assertEqual(error_message, str(err))
    ...
    def test_signal_wrong_input_key(self):
        ...
        details = {'input': {'1': '3'}}
        err = self.assertRaises(exception.ResourceFailure, scheduler.TaskRunner(wf.signal, details))
        error_message = 'StackValidationFailed: resources.create_vm: Signal data error: Unknown input 1'
        self.assertEqual(error_message, str(err))

    def test_signal_with_body_as_input_and_delete_with_executions(self):
        tmpl = template_format.parse(workflow_template_full)
        stack = utils.parse_stack(tmpl, params={'parameters': {'use_request_body_as_input': 'true'}})
        rsrc_defns = stack.t.resource_definitions(stack)['create_vm']
        wf = workflow.Workflow('create_vm', rsrc_defns, stack)
        self.mistral.workflows.create.return_value = [FakeWorkflow('create_vm')]
        scheduler.TaskRunner(wf.create)()
        ...
        expected_args = '{"image": "31d8eeaf-686e-4e95-bb27-765014b9f20b", "name": "create_test_server", "flavor": "3"}'
        ...
        self.mistral.workflows.update.return_value = [FakeWorkflow('test_stack-workflow-b5fiekdsa355')]
        ...
        call_args = self.mistral.executions.create.call_args
        args, kwargs = call_args
        expected_args = '{"image": "31d8eeaf-686e-4e95-bb27-765014b9f20b", "name": "create_test_server", "flavor": "4"}'
        self.validate_json_inputs(kwargs['workflow_input'], expected_args)
        self.assertEqual({'executions': '54321,12345', 'name': 'test_stack-workflow-b5fiekdsa355'}, wf.data())
        scheduler.TaskRunner(wf.delete)()
        self.assertEqual(2, self.mistral.executions.delete.call_count)
        self.assertEqual((wf.DELETE, wf.COMPLETE), wf.state)
    ...
    def test_workflow_params(self):
        ...
        self.mistral.workflows.create.return_value = [FakeWorkflow('create_vm')]
        scheduler.TaskRunner(wf.create)()
        details = {'input': {'flavor': '3'}, 'params': {'test': 'param_value', 'test1': 'param_value_1'}}
        execution = mock.Mock()
        execution.id = '12345'
    ...
    def test_mistral_workflow_refid_convergence_cache_data(self):
        tmpl = template_format.parse(workflow_template)
        cache_data = {'workflow': node_data.NodeData.from_dict({'uuid': mock.ANY, 'id': mock.ANY, 'action': 'CREATE', 'status': 'COMPLETE', 'reference_id': 'convg_xyz'})}
        stack = utils.parse_stack(tmpl, stack_name='test', cache_data=cache_data)
