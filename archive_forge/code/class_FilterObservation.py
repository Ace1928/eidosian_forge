...
class FilterObservation(gym.ObservationWrapper):
    ...
    def __init__(self, env: gym.Env, filter_keys: Sequence[str]=None):
        ...
        if not isinstance(wrapped_observation_space, spaces.Dict):
            raise ValueError(f'FilterObservationWrapper is only usable with dict observations, environment observation space is {type(wrapped_observation_space)}')
        ...
        if missing_keys:
            raise ValueError(f'All the filter_keys must be included in the original observation space.\nFilter keys: {filter_keys}\nObservation keys: {observation_keys}\nMissing keys: {missing_keys}')
        self.observation_space = type(wrapped_observation_space)([(name, copy.deepcopy(space)) for name, space in wrapped_observation_space.spaces.items() if name in filter_keys])
        self._env = env
