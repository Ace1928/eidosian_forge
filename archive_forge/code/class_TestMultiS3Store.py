...
class TestMultiS3Store(base.MultiStoreBaseTest, test_store_capabilities.TestStoreCapabilitiesChecking):
    _CONF = cfg.ConfigOpts()

    def setUp(self):
        """Establish a clean test environment."""
        super(TestMultiS3Store, self).setUp()
        enabled_backends = {'s3_region1': 's3', 's3_region2': 's3'}
        ...
        self.config(group='s3_region1', s3_store_access_key='user', s3_store_secret_key='key', s3_store_host='https://s3-region1.com', s3_store_region_name='custom_region_name', s3_store_bucket='glance', s3_store_large_object_size=S3_CONF['s3_store_large_object_size'], s3_store_large_object_chunk_size=6)
        self.config(group='s3_region2', s3_store_access_key='user', s3_store_secret_key='key', s3_store_host='http://s3-region2.com', s3_store_bucket='glance', s3_store_large_object_size=S3_CONF['s3_store_large_object_size'], s3_store_large_object_chunk_size=6)
    ...
    def test_get_invalid_bucket_name(self):
        ...
        for bucket in invalid_buckets:
            loc = location.get_location_from_uri_and_backend('s3+https://user:key@auth_address/%s/key' % bucket, 's3_region1', conf=self.conf)
            self.assertRaises(boto_exceptions.InvalidDNSNameError, self.store.get, loc)
    ...
    def test_client_custom_region_name(self, mock_client, mock_loc):
        ...
        mock_client.assert_called_with(config=mock.ANY, endpoint_url='https://s3-region1.com', region_name='custom_region_name', service_name='s3', use_ssl=False)

    @mock.patch.object(boto3.session.Session, 'client')
    def test_get(self, mock_client):
        """Test a "normal" retrieval of an image in chunks."""
        bucket, key = ('glance', FAKE_UUID)
        fixture_object = {'Body': io.BytesIO(b'*' * FIVE_KB), 'ContentLength': FIVE_KB}
        fake_s3_client = botocore.session.get_session().create_client('s3')
        with stub.Stubber(fake_s3_client) as stubber:
            stubber.add_response(method='head_object', service_response={}, expected_params={'Bucket': bucket, 'Key': key})
            stubber.add_response(method='get_object', service_response=fixture_object, expected_params={'Bucket': bucket, 'Key': key})
            mock_client.return_value = fake_s3_client
            loc = location.get_location_from_uri_and_backend('s3+https://user:key@auth_address/%s/%s' % (bucket, key), 's3_region1', conf=self.conf)
    ...
    def test_partial_get(self):
        loc = location.get_location_from_uri_and_backend('s3+https://user:key@auth_address/glance/%s' % FAKE_UUID, 's3_region1', conf=self.conf)
    ...
    def test_get_non_existing(self, mock_client):
        ...
        with stub.Stubber(fake_s3_client) as stubber:
            ...
            loc = location.get_location_from_uri_and_backend(uri, 's3_region1', conf=self.conf)
    ...
    def test_add_multipart(self, mock_client):
        ...
        with stub.Stubber(fake_s3_client) as stubber:
            stubber.add_response(method='head_bucket', service_response={}, expected_params={'Bucket': S3_CONF['s3_store_bucket']})
            stubber.add_client_error(method='head_object', service_error_code='404', service_message='', expected_params={'Bucket': S3_CONF['s3_store_bucket'], 'Key': expected_image_id})
            stubber.add_response(method='create_multipart_upload', service_response={'Bucket': S3_CONF['s3_store_bucket'], 'Key': expected_image_id, 'UploadId': 'UploadId'}, expected_params={'Bucket': S3_CONF['s3_store_bucket'], 'Key': expected_image_id})
            parts = []
            remaining_image_size = expected_s3_size
            chunk_size = S3_CONF['s3_store_large_object_chunk_size'] * units.Mi
            for i in range(num_parts):
                part_number = i + 1
                stubber.add_response(method='upload_part', service_response={'ETag': 'ETag'}, expected_params={'Bucket': S3_CONF['s3_store_bucket'], 'Key': expected_image_id, 'Body': botocore.stub.ANY, 'ContentLength': chunk_size, 'PartNumber': part_number, 'UploadId': 'UploadId'})
                parts.append({'ETag': 'ETag', 'PartNumber': part_number})
                remaining_image_size -= chunk_size
                if remaining_image_size < chunk_size:
                    chunk_size = remaining_image_size
            stubber.add_response(method='complete_multipart_upload', service_response={'Bucket': S3_CONF['s3_store_bucket'], 'Key': expected_image_id, 'ETag': 'ETag'}, expected_params={'Bucket': S3_CONF['s3_store_bucket'], 'Key': expected_image_id, 'MultipartUpload': {'Parts': parts}, 'UploadId': 'UploadId'})
            mock_client.return_value = fake_s3_client
            loc, size, checksum, multihash, metadata = self.store.add(expected_image_id, image_s3, expected_s3_size, self.hash_algo)
            self.assertEqual('s3_region1', metadata['store'])
            self.assertEqual(expected_location, loc)
