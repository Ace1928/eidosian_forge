...
class ZipFileSystem(AbstractArchiveFileSystem):
    ...
    def _get_dirs(self):
        if self.dir_cache is None or self.mode in set('wa'):
            ...
            self.dir_cache = {dirname.rstrip('/'): {'name': dirname.rstrip('/'), 'size': 0, 'type': 'directory'} for dirname in self._all_dirnames(self.zip.namelist())}
            for z in files:
                f = {s: getattr(z, s, None) for s in zipfile.ZipInfo.__slots__}
                f.update({'name': z.filename.rstrip('/'), 'size': z.file_size, 'type': 'directory' if z.is_dir() else 'file'})
