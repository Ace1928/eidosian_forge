...
def plot_ess(ax, plotters, xdata, ess_tail_dataset, mean_ess, sd_ess, idata, data, kind, extra_methods, textsize, rows, cols, figsize, kwargs, extra_kwargs, text_kwargs, n_samples, relative, min_ess, labeller, ylabel, rug, rug_kind, rug_kwargs, hline_kwargs, backend_kwargs, show):
    """Bokeh essplot."""
    ...
    if ax is None:
        ax = create_axes_grid(len(plotters), rows, cols, figsize=figsize, backend_kwargs=backend_kwargs)
    else:
        ax = np.atleast_2d(ax)
    for (var_name, selection, isel, x), ax_ in zip(plotters, (item for item in ax.flatten() if item is not None)):
        ...
        if extra_methods:
            ...
            hline = Span(location=mean_ess_i, dimension='width', line_color='black', line_width=2, line_dash='dashed', line_alpha=1.0)
            ax_.renderers.append(hline)
            hline = Span(location=sd_ess_i, dimension='width', line_color='black', line_width=1, line_dash='dashed', line_alpha=1.0)
            ax_.renderers.append(hline)
        if relative and kind == 'evolution':
            ...
            hline = Span(location=min_ess / n_samples if relative else min_ess, dimension='width', **hline_kwargs)
            ax_.renderers.append(hline)
        if kind == 'evolution':
