...
class _ExportPassBase(PassBase):
    ...
    def call_map(self, f: torch.fx.GraphModule, num_args: int, args: List[ProxyValue], meta: NodeMetadata) -> ProxyValue:
        ...
        return self._fx('call_function', torch.ops.higher_order.map_impl, (f_branch.graph_module, num_args, *args), {}, meta)
    ...
    def call(self, graph_module: fx.GraphModule) -> PassResult:
        if not getattr(self, '_initialized', False):
            raise ExportPassBaseError('ExportPass is not initialized with __init__().')
        inputs = self.inputs(graph_module)
        fake_tensor_mode = None
        for i in inputs:
            if isinstance(i, FakeTensor):
                assert fake_tensor_mode is None or fake_tensor_mode is i.fake_mode, 'Multiple fake tensor mode detected.'
