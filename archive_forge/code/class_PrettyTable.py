...
class PrettyTable:
    ...
    def __init__(self, schema: Schema, data: List[Any], best_width: int, truncate_width: int=500):
        raw: List[List[str]] = []
        ...
        for row in data:
            ...
            for i in range(len(schema)):
                ...
                raw_row.append(d)
            raw.append(raw_row)
        dt = sorted(filter(lambda x: x[0] > 0, [(w - col_width_min[i], i) for i, w in enumerate(col_width_max)]))
        if len(dt) > 0:
        ...
        self.data = [[self._wrap(row[i], self.col_width[i]) for i in range(len(schema))] for row in raw]
    ...
    def to_string(self) -> Iterable[str]:
        yield '|'.join((self.titles[i].ljust(self.col_width[i]) for i in range(len(self.titles))))
        yield '+'.join((''.ljust(self.col_width[i], '-') for i in range(len(self.titles))))
        for row in self.data:
            for tp in zip(*row):
                yield '|'.join(tp)
