...
class ModelPruning(Callback):
    ...
    def __init__(self, pruning_fn: Union[Callable, str], parameters_to_prune: _PARAM_LIST=(), parameter_names: Optional[List[str]]=None, use_global_unstructured: bool=True, amount: Union[int, float, Callable[[int], Union[int, float]]]=0.5, apply_pruning: Union[bool, Callable[[int], bool]]=True, make_pruning_permanent: bool=True, use_lottery_ticket_hypothesis: Union[bool, Callable[[int], bool]]=True, resample_parameters: bool=False, pruning_dim: Optional[int]=None, pruning_norm: Optional[int]=None, verbose: int=0, prune_on_train_epoch_end: bool=True) -> None:
        ...
        for name in self._parameter_names:
            if name not in self.PARAMETER_NAMES:
                raise MisconfigurationException(f"The provided `parameter_names` name: {name} isn't in {self.PARAMETER_NAMES}")
        if isinstance(pruning_fn, str):
            pruning_kwargs = {}
            pruning_fn = pruning_fn.lower()
            if pruning_fn not in _PYTORCH_PRUNING_FUNCTIONS:
                raise MisconfigurationException(f"The provided `pruning_fn` {pruning_fn} isn't available in PyTorch's built-in functions: {list(_PYTORCH_PRUNING_FUNCTIONS.keys())} ")
            if pruning_fn.endswith('_structured'):
                if pruning_dim is None:
                    raise MisconfigurationException('When requesting `structured` pruning, the `pruning_dim` should be provided.')
                if pruning_fn == 'ln_structured':
                    if pruning_norm is None:
                        raise MisconfigurationException('When requesting `ln_structured` pruning, the `pruning_norm` should be provided.')
            ...
            if not use_global_unstructured:
                raise MisconfigurationException('PyTorch `BasePruningMethod` is currently only supported with `use_global_unstructured=True`.')
        else:
            raise MisconfigurationException(f'`pruning_fn` is expected to be a str in {list(_PYTORCH_PRUNING_FUNCTIONS.keys())} or a PyTorch `BasePruningMethod`. Found: {pruning_fn}. HINT: if passing a `BasePruningMethod`, pass the the class, not an instance')
        if use_global_unstructured and pruning_fn.PRUNING_TYPE != 'unstructured':
            raise MisconfigurationException(f'Only the "unstructured" PRUNING_TYPE is supported with `use_global_unstructured=True`. Found method {pruning_fn} of type {pruning_fn.PRUNING_TYPE}. ')
        ...
        if not (isinstance(amount, (int, float)) or callable(amount)):
            raise MisconfigurationException('`amount` should be provided and be either an int, a float or Callable function.')
        self.amount = amount
    ...
    def setup(self, trainer: 'pl.Trainer', pl_module: LightningModule, stage: str) -> None:
        parameters_to_prune = self.sanitize_parameters_to_prune(pl_module, self._parameters_to_prune, parameter_names=self._parameter_names)
    ...
    @staticmethod
    def sanitize_parameters_to_prune(pl_module: LightningModule, parameters_to_prune: _PARAM_LIST=(), parameter_names: Sequence[str]=()) -> _PARAM_LIST:
        ...
        if not parameters_to_prune:
            parameters_to_prune = [(m, p) for p in parameters for m in current_modules if getattr(m, p, None) is not None]
        elif isinstance(parameters_to_prune, (list, tuple)) and len(parameters_to_prune) > 0 and all((len(p) == 2 for p in parameters_to_prune)) and all((isinstance(a, nn.Module) and isinstance(b, str) for a, b in parameters_to_prune)):
            ...
            if missing_modules or missing_parameters:
                raise MisconfigurationException(f"Some provided `parameters_to_prune` don't exist in the model. Found missing modules: {missing_modules} and missing parameters: {missing_parameters}")
        else:
            raise MisconfigurationException('The provided `parameters_to_prune` should either be list of tuple with 2 elements: (nn.Module, parameter_name_to_prune) or None')
        return parameters_to_prune
