...
class NextArguments(BaseSchema):
    ...
    """
    __props__ = {'threadId': {'type': 'integer', 'description': 'Specifies the thread for which to resume execution for one step (of the given granularity).'}, 'singleThread': {'type': 'boolean', 'description': 'If this optional flag is true, all other suspended threads are not resumed.'}, 'granularity': {'description': "Optional granularity to step. If no granularity is specified, a granularity of 'statement' is assumed.", 'type': 'SteppingGranularity'}}
