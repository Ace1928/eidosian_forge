...
class HfApi:
    ...
    def __init__(self, endpoint: Optional[str]=None, token: Optional[str]=None, library_name: Optional[str]=None, library_version: Optional[str]=None, user_agent: Union[Dict, str, None]=None) -> None:
    ...
    def list_datasets(self, *, filter: Union[DatasetFilter, str, Iterable[str], None]=None, author: Optional[str]=None, benchmark: Optional[Union[str, List[str]]]=None, dataset_name: Optional[str]=None, language_creators: Optional[Union[str, List[str]]]=None, language: Optional[Union[str, List[str]]]=None, multilinguality: Optional[Union[str, List[str]]]=None, size_categories: Optional[Union[str, List[str]]]=None, task_categories: Optional[Union[str, List[str]]]=None, task_ids: Optional[Union[str, List[str]]]=None, search: Optional[str]=None, sort: Optional[Union[Literal['last_modified'], str]]=None, direction: Optional[Literal[-1]]=None, limit: Optional[int]=None, full: Optional[bool]=None, token: Optional[str]=None) -> Iterable[DatasetInfo]:
        ...
        for attr in (benchmark, language_creators, language, multilinguality, size_categories, task_categories, task_ids):
            if attr:
    ...
    @validate_hf_hub_args
    def unlike(self, repo_id: str, *, token: Optional[str]=None, repo_type: Optional[str]=None) -> None:
        """
        ...
        if repo_type is None:
            repo_type = REPO_TYPE_MODEL
        response = get_session().delete(url=f'{self.endpoint}/api/{repo_type}s/{repo_id}/like', headers=self._build_hf_headers(token=token))
        hf_raise_for_status(response)
    ...
    @validate_hf_hub_args
    def space_info(self, repo_id: str, *, revision: Optional[str]=None, timeout: Optional[float]=None, files_metadata: bool=False, token: Optional[Union[bool, str]]=None) -> SpaceInfo:
        """
        ...
        """
        headers = self._build_hf_headers(token=token)
        path = f'{self.endpoint}/api/spaces/{repo_id}' if revision is None else f'{self.endpoint}/api/spaces/{repo_id}/revision/{quote(revision, safe='')}'
        params = {}
        if files_metadata:
            params['blobs'] = True
        r = get_session().get(path, headers=headers, timeout=timeout, params=params)
    ...
    @validate_hf_hub_args
    def repo_info(self, repo_id: str, *, revision: Optional[str]=None, repo_type: Optional[str]=None, timeout: Optional[float]=None, files_metadata: bool=False, token: Optional[Union[bool, str]]=None) -> Union[ModelInfo, DatasetInfo, SpaceInfo]:
        """
        ...
        return method(repo_id, revision=revision, token=token, timeout=timeout, files_metadata=files_metadata)
    ...
    @validate_hf_hub_args
    def revision_exists(self, repo_id: str, revision: str, *, repo_type: Optional[str]=None, token: Optional[str]=None) -> bool:
        """
    ...
    @validate_hf_hub_args
    def file_exists(self, repo_id: str, filename: str, *, repo_type: Optional[str]=None, revision: Optional[str]=None, token: Optional[str]=None) -> bool:
        """
        ...
        Examples:
            ...
            ```
        """
        url = hf_hub_url(repo_id=repo_id, repo_type=repo_type, revision=revision, filename=filename, endpoint=self.endpoint)
        try:
            if token is None:
                token = self.token
    ...
    def list_files_info(self, repo_id: str, paths: Union[List[str], str, None]=None, *, expand: bool=False, revision: Optional[str]=None, repo_type: Optional[str]=None, token: Optional[Union[bool, str]]=None) -> Iterable[RepoFile]:
        """
    ...
    @validate_hf_hub_args
    def create_repo(self, repo_id: str, *, token: Optional[str]=None, private: bool=False, repo_type: Optional[str]=None, exist_ok: bool=False, space_sdk: Optional[str]=None, space_hardware: Optional[SpaceHardware]=None, space_storage: Optional[SpaceStorage]=None, space_sleep_time: Optional[int]=None, space_secrets: Optional[List[Dict[str, str]]]=None, space_variables: Optional[List[Dict[str, str]]]=None) -> RepoUrl:
        ...
        if repo_type == 'space':
            if space_sdk is None:
                raise ValueError(f"No space_sdk provided. `create_repo` expects space_sdk to be one of {SPACES_SDK_TYPES} when repo_type is 'space'`")
    ...
    def create_commit(self, repo_id: str, operations: Iterable[CommitOperation], *, commit_message: str, commit_description: Optional[str]=None, token: Optional[str]=None, repo_type: Optional[str]=None, revision: Optional[str]=None, create_pr: Optional[bool]=None, num_threads: int=5, parent_commit: Optional[str]=None, run_as_future: bool=False) -> Union[CommitInfo, Future[CommitInfo]]:
        ...
        if parent_commit is not None and (not REGEX_COMMIT_OID.fullmatch(parent_commit)):
            raise ValueError(f'`parent_commit` is not a valid commit OID. It must match the following regex: {REGEX_COMMIT_OID}')
        ...
        for addition in additions:
            if addition._is_committed:
                raise ValueError(f'CommitOperationAdd {addition} has already being committed and cannot be reused. Please create a new CommitOperationAdd object if you want to create a new commit.')
        ...
        for addition in additions:
            if addition.path_in_repo == 'README.md':
                with addition.as_file() as file:
                    response = get_session().post(f'{ENDPOINT}/api/validate-yaml', json={'content': file.read().decode(), 'repoType': repo_type}, headers=self._build_hf_headers(token=token))
        ...
        self.preupload_lfs_files(repo_id=repo_id, additions=additions, token=token, repo_type=repo_type, revision=unquoted_revision, create_pr=create_pr, num_threads=num_threads, free_memory=False)
        files_to_copy = _fetch_files_to_copy(copies=copies, repo_type=repo_type, repo_id=repo_id, token=token or self.token, revision=revision, endpoint=self.endpoint)
        commit_payload = _prepare_commit_payload(operations=operations, files_to_copy=files_to_copy, commit_message=commit_message, commit_description=commit_description, parent_commit=parent_commit)
        ...
        def _payload_as_ndjson() -> Iterable[bytes]:
            for item in commit_payload:
                ...
                yield b'\n'
        headers = {'Content-Type': 'application/x-ndjson', **self._build_hf_headers(token=token, is_write_action=True)}
        ...
        try:
            ...
            if nb_deletions > 0 and "A file with this name doesn't exist" in str(e):
                e.append_to_message("\nMake sure to differentiate file and folder paths in delete operations with a trailing '/' or using `is_folder=True/False`.")
            raise
        for addition in additions:
            addition._is_committed = True
        commit_data = commit_resp.json()
        return CommitInfo(commit_url=commit_data['commitUrl'], commit_message=commit_message, commit_description=commit_description, oid=commit_data['commitOid'], pr_url=commit_data['pullRequestUrl'] if create_pr else None)
    ...
    def create_commits_on_pr(self, *, repo_id: str, addition_commits: List[List[CommitOperationAdd]], deletion_commits: List[List[CommitOperationDelete]], commit_message: str, commit_description: Optional[str]=None, token: Optional[str]=None, repo_type: Optional[str]=None, merge_pr: bool=True, num_threads: int=5, verbose: bool=False) -> str:
        ...
        logger.info(f'Will create {len(deletion_commits)} deletion commit(s) and {len(addition_commits)} addition commit(s), totalling {sum((len(ops) for ops in addition_commits + deletion_commits))} atomic operations.')
        strategy = MultiCommitStrategy(addition_commits=[MultiCommitStep(operations=operations) for operations in addition_commits], deletion_commits=[MultiCommitStep(operations=operations) for operations in deletion_commits])
        ...
        for discussion in self.get_repo_discussions(repo_id=repo_id, repo_type=repo_type, token=token):
            if discussion.is_pull_request and discussion.status == 'draft' and (strategy.id in discussion.title):
                pr = self.get_discussion_details(repo_id=repo_id, discussion_num=discussion.num, repo_type=repo_type, token=token)
                logger.info(f'PR already exists: {pr.url}. Will resume process where it stopped.')
                break
        else:
            pr = multi_commit_create_pull_request(self, repo_id=repo_id, commit_message=commit_message, commit_description=commit_description, strategy=strategy, token=token, repo_type=repo_type)
        ...
        if len(description_commits) != len(strategy.all_steps):
            raise MultiCommitException(f'Corrupted multi-commit PR #{pr.num}: got {len(description_commits)} steps in description but {len(strategy.all_steps)} in strategy.')
        ...
        commits_on_main_branch = {commit.commit_id for commit in self.list_repo_commits(repo_id=repo_id, repo_type=repo_type, token=token, revision=DEFAULT_REVISION)}
        pr_commits = [commit for commit in self.list_repo_commits(repo_id=repo_id, repo_type=repo_type, token=token, revision=pr.git_reference) if commit.commit_id not in commits_on_main_branch]
        if len(pr_commits) > 0:
            logger.info(f'Found {len(pr_commits)} existing commits on the PR.')
        if len(pr_commits) > len(strategy.all_steps):
            raise MultiCommitException(f'Corrupted multi-commit PR #{pr.num}: scheduled {len(strategy.all_steps)} steps but {len(pr_commits)} commits have already been pushed to the PR.')
        ...
        if len(remaining_deletions) > 0 and len(remaining_additions) < len(strategy.addition_commits):
            raise MultiCommitException(f'Corrupted multi-commit PR #{pr.num}: some addition commits have already been pushed to the PR but deletion commits are not all completed yet.')
        nb_remaining = len(remaining_deletions) + len(remaining_additions)
        if len(pr_commits) > 0:
            logger.info(f'{nb_remaining} commits remaining ({len(remaining_deletions)} deletion commits and {len(remaining_additions)} addition commits)')
        for step in list(remaining_deletions.values()) + list(remaining_additions.values()):
            self.create_commit(repo_id=repo_id, repo_type=repo_type, token=token, commit_message=step.id, revision=pr.git_reference, num_threads=num_threads, operations=step.operations, create_pr=False)
            step.completed = True
            nb_remaining -= 1
            logger.info(f'  step {step.id} completed (still {nb_remaining} to go).')
            self.edit_discussion_comment(repo_id=repo_id, repo_type=repo_type, token=token, discussion_num=pr.num, comment_id=pr_comment.id, new_content=multi_commit_generate_comment(commit_message=commit_message, commit_description=commit_description, strategy=strategy))
        logger.info('All commits have been pushed.')
        self.rename_discussion(repo_id=repo_id, repo_type=repo_type, token=token, discussion_num=pr.num, new_title=commit_message)
        self.change_discussion_status(repo_id=repo_id, repo_type=repo_type, token=token, discussion_num=pr.num, new_status='open', comment=MULTI_COMMIT_PR_COMPLETION_COMMENT_TEMPLATE)
        logger.info('PR is now open for reviews.')
        if merge_pr:
            try:
                self.merge_pull_request(repo_id=repo_id, repo_type=repo_type, token=token, discussion_num=pr.num, comment=MULTI_COMMIT_PR_CLOSING_COMMENT_TEMPLATE)
                ...
                if error.server_message is not None and 'no associated changes' in error.server_message:
                    self.change_discussion_status(repo_id=repo_id, repo_type=repo_type, token=token, discussion_num=pr.num, comment=MULTI_COMMIT_PR_CLOSE_COMMENT_FAILURE_NO_CHANGES_TEMPLATE, new_status='closed')
                    logger.warning("Couldn't merge the PR: no associated changes.")
                else:
                    self.comment_discussion(repo_id=repo_id, repo_type=repo_type, token=token, discussion_num=pr.num, comment=MULTI_COMMIT_PR_CLOSE_COMMENT_FAILURE_BAD_REQUEST_TEMPLATE.format(error_message=error.server_message))
                    raise MultiCommitException(f"Couldn't merge Pull Request in multi-commit: {error.server_message}") from error
        return pr.url

    def preupload_lfs_files(self, repo_id: str, additions: Iterable[CommitOperationAdd], *, token: Optional[str]=None, repo_type: Optional[str]=None, revision: Optional[str]=None, create_pr: Optional[bool]=None, num_threads: int=5, free_memory: bool=True, gitignore_content: Optional[str]=None):
        ...
        try:
            _fetch_upload_modes(additions=new_additions, repo_type=repo_type, repo_id=repo_id, token=token or self.token, revision=revision, endpoint=self.endpoint, create_pr=create_pr or False, gitignore_content=gitignore_content)
        except RepositoryNotFoundError as e:
            e.append_to_message(_CREATE_COMMIT_NO_REPO_ERROR_MESSAGE)
            raise
        ...
        if len(new_lfs_additions) != len(new_lfs_additions_to_upload):
            logger.info(f'Skipped upload for {len(new_lfs_additions) - len(new_lfs_additions_to_upload)} LFS file(s) (ignored by gitignore file).')
        _upload_lfs_files(additions=new_lfs_additions_to_upload, repo_type=repo_type, repo_id=repo_id, token=token or self.token, endpoint=self.endpoint, num_threads=num_threads, revision=revision if not create_pr else None)
    ...
    def upload_file(self, *, path_or_fileobj: Union[str, Path, bytes, BinaryIO], path_in_repo: str, repo_id: str, token: Optional[str]=None, repo_type: Optional[str]=None, revision: Optional[str]=None, commit_message: Optional[str]=None, commit_description: Optional[str]=None, create_pr: Optional[bool]=None, parent_commit: Optional[str]=None, run_as_future: bool=False) -> Union[CommitInfo, Future[CommitInfo]]:
        ...
        commit_info = self.create_commit(repo_id=repo_id, repo_type=repo_type, operations=[operation], commit_message=commit_message, commit_description=commit_description, token=token, revision=revision, create_pr=create_pr, parent_commit=parent_commit)
        ...
        return CommitInfo(commit_url=commit_info.commit_url, commit_message=commit_info.commit_message, commit_description=commit_info.commit_description, oid=commit_info.oid, pr_url=commit_info.pr_url, _url=f'{self.endpoint}/{repo_id}/blob/{revision}/{path_in_repo}')
    ...
    def upload_folder(self, *, repo_id: str, folder_path: Union[str, Path], path_in_repo: Optional[str]=None, commit_message: Optional[str]=None, commit_description: Optional[str]=None, token: Optional[str]=None, repo_type: Optional[str]=None, revision: Optional[str]=None, create_pr: Optional[bool]=None, parent_commit: Optional[str]=None, allow_patterns: Optional[Union[List[str], str]]=None, ignore_patterns: Optional[Union[List[str], str]]=None, delete_patterns: Optional[Union[List[str], str]]=None, multi_commits: bool=False, multi_commits_verbose: bool=False, run_as_future: bool=False) -> Union[CommitInfo, str, Future[CommitInfo], Future[str]]:
        ...
        delete_operations = self._prepare_upload_folder_deletions(repo_id=repo_id, repo_type=repo_type, revision=DEFAULT_REVISION if create_pr else revision, token=token, path_in_repo=path_in_repo, delete_patterns=delete_patterns)
        add_operations = _prepare_upload_folder_additions(folder_path, path_in_repo, allow_patterns=allow_patterns, ignore_patterns=ignore_patterns)
        if len(add_operations) > 0:
            added_paths = set((op.path_in_repo for op in add_operations))
            delete_operations = [delete_op for delete_op in delete_operations if delete_op.path_in_repo not in added_paths]
        ...
        if multi_commits:
            ...
            pr_url = self.create_commits_on_pr(repo_id=repo_id, repo_type=repo_type, addition_commits=addition_commits, deletion_commits=deletion_commits, commit_message=commit_message, commit_description=commit_description, token=token, merge_pr=not create_pr, verbose=multi_commits_verbose)
            return pr_url
        commit_info = self.create_commit(repo_type=repo_type, repo_id=repo_id, operations=commit_operations, commit_message=commit_message, commit_description=commit_description, token=token, revision=revision, create_pr=create_pr, parent_commit=parent_commit)
        if create_pr and commit_info.pr_url is not None:
            revision = quote(_parse_revision_from_pr_url(commit_info.pr_url), safe='')
        if repo_type in REPO_TYPES_URL_PREFIXES:
            repo_id = REPO_TYPES_URL_PREFIXES[repo_type] + repo_id
        revision = revision if revision is not None else DEFAULT_REVISION
        return CommitInfo(commit_url=commit_info.commit_url, commit_message=commit_info.commit_message, commit_description=commit_info.commit_description, oid=commit_info.oid, pr_url=commit_info.pr_url, _url=f'{self.endpoint}/{repo_id}/tree/{revision}/{path_in_repo}')

    @validate_hf_hub_args
    def delete_file(self, path_in_repo: str, repo_id: str, *, token: Optional[str]=None, repo_type: Optional[str]=None, revision: Optional[str]=None, commit_message: Optional[str]=None, commit_description: Optional[str]=None, create_pr: Optional[bool]=None, parent_commit: Optional[str]=None) -> CommitInfo:
        """
    ...
    @validate_hf_hub_args
    def delete_folder(self, path_in_repo: str, repo_id: str, *, token: Optional[str]=None, repo_type: Optional[str]=None, revision: Optional[str]=None, commit_message: Optional[str]=None, commit_description: Optional[str]=None, create_pr: Optional[bool]=None, parent_commit: Optional[str]=None) -> CommitInfo:
        """
        ...
        Args:
            ...
            parent_commit (`str`, *optional*):
                ...
                especially useful if the repo is updated / committed to concurrently.
        """
        return self.create_commit(repo_id=repo_id, repo_type=repo_type, token=token, operations=[CommitOperationDelete(path_in_repo=path_in_repo, is_folder=True)], revision=revision, commit_message=commit_message if commit_message is not None else f'Delete folder {path_in_repo} with huggingface_hub', commit_description=commit_description, create_pr=create_pr, parent_commit=parent_commit)

    @validate_hf_hub_args
    def get_hf_file_metadata(self, *, url: str, token: Union[bool, str, None]=None, proxies: Optional[Dict]=None, timeout: Optional[float]=DEFAULT_REQUEST_TIMEOUT) -> HfFileMetadata:
        ...
        if token is None:
            token = self.token
        return get_hf_file_metadata(url=url, token=token, proxies=proxies, timeout=timeout, library_name=self.library_name, library_version=self.library_version, user_agent=self.user_agent)

    @validate_hf_hub_args
    def hf_hub_download(self, repo_id: str, filename: str, *, subfolder: Optional[str]=None, repo_type: Optional[str]=None, revision: Optional[str]=None, cache_dir: Union[str, Path, None]=None, local_dir: Union[str, Path, None]=None, local_dir_use_symlinks: Union[bool, Literal['auto']]='auto', force_download: bool=False, force_filename: Optional[str]=None, proxies: Optional[Dict]=None, etag_timeout: float=DEFAULT_ETAG_TIMEOUT, resume_download: bool=False, token: Optional[Union[str, bool]]=None, local_files_only: bool=False, legacy_cache_layout: bool=False) -> str:
        ...
        if token is None:
            token = self.token
        return hf_hub_download(repo_id=repo_id, filename=filename, subfolder=subfolder, repo_type=repo_type, revision=revision, endpoint=self.endpoint, library_name=self.library_name, library_version=self.library_version, cache_dir=cache_dir, local_dir=local_dir, local_dir_use_symlinks=local_dir_use_symlinks, user_agent=self.user_agent, force_download=force_download, force_filename=force_filename, proxies=proxies, etag_timeout=etag_timeout, resume_download=resume_download, token=token, local_files_only=local_files_only, legacy_cache_layout=legacy_cache_layout)
    ...
    def snapshot_download(self, repo_id: str, *, repo_type: Optional[str]=None, revision: Optional[str]=None, cache_dir: Union[str, Path, None]=None, local_dir: Union[str, Path, None]=None, local_dir_use_symlinks: Union[bool, Literal['auto']]='auto', proxies: Optional[Dict]=None, etag_timeout: float=DEFAULT_ETAG_TIMEOUT, resume_download: bool=False, force_download: bool=False, token: Optional[Union[str, bool]]=None, local_files_only: bool=False, allow_patterns: Optional[Union[List[str], str]]=None, ignore_patterns: Optional[Union[List[str], str]]=None, max_workers: int=8, tqdm_class: Optional[base_tqdm]=None) -> str:
        ...
        if token is None:
            token = self.token
        return snapshot_download(repo_id=repo_id, repo_type=repo_type, revision=revision, endpoint=self.endpoint, cache_dir=cache_dir, local_dir=local_dir, local_dir_use_symlinks=local_dir_use_symlinks, library_name=self.library_name, library_version=self.library_version, user_agent=self.user_agent, proxies=proxies, etag_timeout=etag_timeout, resume_download=resume_download, force_download=force_download, token=token, local_files_only=local_files_only, allow_patterns=allow_patterns, ignore_patterns=ignore_patterns, max_workers=max_workers, tqdm_class=tqdm_class)

    def get_safetensors_metadata(self, repo_id: str, *, repo_type: Optional[str]=None, revision: Optional[str]=None, token: Optional[str]=None) -> SafetensorsRepoMetadata:
        """
        ...
        Example:
            ...
            ```
        """
        if self.file_exists(repo_id=repo_id, filename=SAFETENSORS_SINGLE_FILE, repo_type=repo_type, revision=revision, token=token):
            file_metadata = self.parse_safetensors_file_metadata(repo_id=repo_id, filename=SAFETENSORS_SINGLE_FILE, repo_type=repo_type, revision=revision, token=token)
            return SafetensorsRepoMetadata(metadata=None, sharded=False, weight_map={tensor_name: SAFETENSORS_SINGLE_FILE for tensor_name in file_metadata.tensors.keys()}, files_metadata={SAFETENSORS_SINGLE_FILE: file_metadata})
        elif self.file_exists(repo_id=repo_id, filename=SAFETENSORS_INDEX_FILE, repo_type=repo_type, revision=revision, token=token):
            index_file = self.hf_hub_download(repo_id=repo_id, filename=SAFETENSORS_INDEX_FILE, repo_type=repo_type, revision=revision, token=token)
            ...
            def _parse(filename: str) -> None:
                files_metadata[filename] = self.parse_safetensors_file_metadata(repo_id=repo_id, filename=filename, repo_type=repo_type, revision=revision, token=token)
            thread_map(_parse, set(weight_map.values()), desc='Parse safetensors files', tqdm_class=hf_tqdm)
            return SafetensorsRepoMetadata(metadata=index.get('metadata', None), sharded=True, weight_map=weight_map, files_metadata=files_metadata)
        else:
            raise NotASafetensorsRepoError(f"'{repo_id}' is not a safetensors repo. Couldn't find '{SAFETENSORS_INDEX_FILE}' or '{SAFETENSORS_SINGLE_FILE}' files.")
    ...
    @validate_hf_hub_args
    def create_branch(self, repo_id: str, *, branch: str, revision: Optional[str]=None, token: Optional[str]=None, repo_type: Optional[str]=None, exist_ok: bool=False) -> None:
        """
    ...
    @validate_hf_hub_args
    def create_tag(self, repo_id: str, *, tag: str, tag_message: Optional[str]=None, revision: Optional[str]=None, token: Optional[str]=None, repo_type: Optional[str]=None, exist_ok: bool=False) -> None:
        """
    ...
    @validate_hf_hub_args
    def delete_tag(self, repo_id: str, *, tag: str, token: Optional[str]=None, repo_type: Optional[str]=None) -> None:
        """
    ...
    @validate_hf_hub_args
    def comment_discussion(self, repo_id: str, discussion_num: int, comment: str, *, token: Optional[str]=None, repo_type: Optional[str]=None) -> DiscussionComment:
        ...
        '''
        resp = self._post_discussion_changes(repo_id=repo_id, repo_type=repo_type, discussion_num=discussion_num, token=token, resource='comment', body={'comment': comment})
        return deserialize_event(resp.json()['newMessage'])

    @validate_hf_hub_args
    def rename_discussion(self, repo_id: str, discussion_num: int, new_title: str, *, token: Optional[str]=None, repo_type: Optional[str]=None) -> DiscussionTitleChange:
        ...
        """
        resp = self._post_discussion_changes(repo_id=repo_id, repo_type=repo_type, discussion_num=discussion_num, token=token, resource='title', body={'title': new_title})
        return deserialize_event(resp.json()['newTitle'])

    @validate_hf_hub_args
    def change_discussion_status(self, repo_id: str, discussion_num: int, new_status: Literal['open', 'closed'], *, token: Optional[str]=None, comment: Optional[str]=None, repo_type: Optional[str]=None) -> DiscussionStatusChange:
        ...
        resp = self._post_discussion_changes(repo_id=repo_id, repo_type=repo_type, discussion_num=discussion_num, token=token, resource='status', body=body)
        return deserialize_event(resp.json()['newStatus'])

    @validate_hf_hub_args
    def merge_pull_request(self, repo_id: str, discussion_num: int, *, token: Optional[str]=None, comment: Optional[str]=None, repo_type: Optional[str]=None):
        ...
        """
        self._post_discussion_changes(repo_id=repo_id, repo_type=repo_type, discussion_num=discussion_num, token=token, resource='merge', body={'comment': comment.strip()} if comment and comment.strip() else None)

    @validate_hf_hub_args
    def edit_discussion_comment(self, repo_id: str, discussion_num: int, comment_id: str, new_content: str, *, token: Optional[str]=None, repo_type: Optional[str]=None) -> DiscussionComment:
        ...
        """
        resp = self._post_discussion_changes(repo_id=repo_id, repo_type=repo_type, discussion_num=discussion_num, token=token, resource=f'comment/{comment_id.lower()}/edit', body={'content': new_content})
    ...
    def get_space_variables(self, repo_id: str, *, token: Optional[str]=None) -> Dict[str, SpaceVariable]:
        ...
        Args:
            ...
            token (`str`, *optional*):
                Hugging Face token. Will default to the locally saved token if not provided.
        """
        r = get_session().get(f'{self.endpoint}/api/spaces/{repo_id}/variables', headers=self._build_hf_headers(token=token))
        hf_raise_for_status(r)
        return {k: SpaceVariable(k, v) for k, v in r.json().items()}

    @validate_hf_hub_args
    def add_space_variable(self, repo_id: str, key: str, value: str, *, description: Optional[str]=None, token: Optional[str]=None) -> Dict[str, SpaceVariable]:
        ...
        if description is not None:
            payload['description'] = description
        r = get_session().post(f'{self.endpoint}/api/spaces/{repo_id}/variables', headers=self._build_hf_headers(token=token), json=payload)
        hf_raise_for_status(r)
        return {k: SpaceVariable(k, v) for k, v in r.json().items()}
    ...
    def get_space_runtime(self, repo_id: str, *, token: Optional[str]=None) -> SpaceRuntime:
        ...
        """
        r = get_session().get(f'{self.endpoint}/api/spaces/{repo_id}/runtime', headers=self._build_hf_headers(token=token))
        hf_raise_for_status(r)
    ...
    @validate_hf_hub_args
    def restart_space(self, repo_id: str, *, token: Optional[str]=None, factory_reboot: bool=False) -> SpaceRuntime:
        """Restart your Space.
        ...
        if factory_reboot:
            params['factory'] = 'true'
        r = get_session().post(f'{self.endpoint}/api/spaces/{repo_id}/restart', headers=self._build_hf_headers(token=token), params=params)
        hf_raise_for_status(r)
        return SpaceRuntime(r.json())
    ...
    def list_inference_endpoints(self, namespace: Optional[str]=None, *, token: Optional[str]=None) -> List[InferenceEndpoint]:
        ...
        response = get_session().get(f'{INFERENCE_ENDPOINTS_ENDPOINT}/endpoint/{namespace}', headers=self._build_hf_headers(token=token))
        hf_raise_for_status(response)
        return [InferenceEndpoint.from_raw(endpoint, namespace=namespace, token=token) for endpoint in response.json()['items']]
    ...
    def pause_inference_endpoint(self, name: str, *, namespace: Optional[str]=None, token: Optional[str]=None) -> InferenceEndpoint:
        ...
        """
        namespace = namespace or self._get_namespace(token=token)
        response = get_session().post(f'{INFERENCE_ENDPOINTS_ENDPOINT}/endpoint/{namespace}/{name}/pause', headers=self._build_hf_headers(token=token))
        hf_raise_for_status(response)
        return InferenceEndpoint.from_raw(response.json(), namespace=namespace, token=token)

    def resume_inference_endpoint(self, name: str, *, namespace: Optional[str]=None, token: Optional[str]=None) -> InferenceEndpoint:
        ...
        """
        namespace = namespace or self._get_namespace(token=token)
        response = get_session().post(f'{INFERENCE_ENDPOINTS_ENDPOINT}/endpoint/{namespace}/{name}/resume', headers=self._build_hf_headers(token=token))
        hf_raise_for_status(response)
        return InferenceEndpoint.from_raw(response.json(), namespace=namespace, token=token)
    ...
    def _get_namespace(self, token: Optional[str]=None) -> str:
        ...
        if me['type'] == 'user':
            ...
            raise ValueError("Cannot determine default namespace. You must provide a 'namespace' as input or be logged in as a user.")
    ...
    def get_collection(self, collection_slug: str, *, token: Optional[str]=None) -> Collection:
        ...
        ```
        """
        r = get_session().get(f'{self.endpoint}/api/collections/{collection_slug}', headers=self._build_hf_headers(token=token))
        hf_raise_for_status(r)
    ...
    @validate_hf_hub_args
    def list_pending_access_requests(self, repo_id: str, *, repo_type: Optional[str]=None, token: Optional[str]=None) -> List[AccessRequest]:
        """
    ...
    @validate_hf_hub_args
    def list_accepted_access_requests(self, repo_id: str, *, repo_type: Optional[str]=None, token: Optional[str]=None) -> List[AccessRequest]:
        """
    ...
    @validate_hf_hub_args
    def accept_access_request(self, repo_id: str, user: str, *, repo_type: Optional[str]=None, token: Optional[str]=None) -> None:
        """
