...
def firewall_internet_service_group(data, fos, check_mode=False):
    ...
    if check_mode:
        ...
        mkey = fos.get_mkey('firewall', 'internet-service-group', filtered_data, vdom=vdom)
        current_data = fos.get('firewall', 'internet-service-group', vdom=vdom, mkey=mkey)
    ...
    if state == 'present' or state is True:
        return fos.set('firewall', 'internet-service-group', data=filtered_data, vdom=vdom)
    elif state == 'absent':
        return fos.delete('firewall', 'internet-service-group', mkey=filtered_data['name'], vdom=vdom)
    else:
