...
class ISCSITargetUtils(baseutils.BaseUtils):
    ...
    def get_supported_disk_format(self):
        return constants.DISK_FORMAT_VHDX if self._win_gteq_6_3 else constants.DISK_FORMAT_VHD
    ...
    def associate_initiator_with_iscsi_target(self, initiator, target_name, id_method=ID_METHOD_IQN):
        ...
        try:
            ...
            err_msg = _('Could not associate initiator %(initiator)s to iSCSI target: %(target_name)s.')
    ...
    def create_snapshot(self, wtd_name, snapshot_name):
        ...
        try:
            ...
            err_msg = _('Failed to create snapshot. WT Disk name: %(wtd_name)s Snapshot name: %(snapshot_name)s')
            raise exceptions.ISCSITargetWMIException(err_msg % dict(wtd_name=wtd_name, snapshot_name=snapshot_name), wmi_exc=wmi_exc)
    ...
    def export_snapshot(self, snapshot_name, dest_path):
        ...
        try:
            ...
            err_msg = _('Failed to export snapshot %(snapshot_name)s to %(dest_path)s.')
            raise exceptions.ISCSITargetWMIException(err_msg % dict(snapshot_name=snapshot_name, dest_path=dest_path), wmi_exc=wmi_exc)

    def delete_snapshot(self, snapshot_name):
        """Driver entry point for deleting a snapshot."""
        try:
            wt_snapshot = self._get_wt_snapshot(snapshot_name, fail_if_not_found=False)
            if wt_snapshot:
                wt_snapshot.Delete_()
        except exceptions.x_wmi as wmi_exc:
            err_msg = _('Failed delete snapshot %s.')
            raise exceptions.ISCSITargetWMIException(err_msg % snapshot_name, wmi_exc=wmi_exc)