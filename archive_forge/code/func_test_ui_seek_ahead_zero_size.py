...
def test_ui_seek_ahead_zero_size(self):
    ...
    for i in range(100):
        current_time_ms += 200
        PutToQueueWithTimeout(status_queue, FileMessage(StorageUrlFromString('gs://foo%s' % i), StorageUrlFromString('bar%s' % i), current_time_ms, message_type=FileMessage.FILE_DOWNLOAD))
    for i in range(100):
        current_time_ms += 200
        PutToQueueWithTimeout(status_queue, FileMessage(StorageUrlFromString('gs://foo%s' % i), StorageUrlFromString('bar%s' % i), current_time_ms, finished=True, message_type=FileMessage.FILE_DOWNLOAD))
    PutToQueueWithTimeout(status_queue, ProducerThreadMessage(100, 0, current_time_ms, finished=True))
