...
class VmwareRestClient(object):
    ...
    def check_required_library(self):
        ...
        if not HAS_PYVMOMI:
            self.module.fail_json(msg=missing_required_lib('PyVmomi'), exception=PYVMOMI_IMP_ERR)
        if not HAS_VSPHERE:
            self.module.fail_json(msg=missing_required_lib('vSphere Automation SDK', url='https://code.vmware.com/web/sdk/7.0/vsphere-automation-python'), exception=VSPHERE_IMP_ERR)
    ...
    def get_datastore_by_name(self, datacenter_name, datastore_name):
        ...
        filter_spec = Datastore.FilterSpec(datacenters=set([datacenter]), names=names)
    ...
    def get_cluster_by_name(self, datacenter_name, cluster_name):
        ...
        filter_spec = Cluster.FilterSpec(datacenters=set([datacenter]), names=names)
    ...
    def get_host_by_name(self, datacenter_name, host_name):
        ...
        filter_spec = Host.FilterSpec(datacenters=set([datacenter]), names=names)
