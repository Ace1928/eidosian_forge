  ...
  *{command}* allows users to connect to, and interact with, a VM's
  ...
  """
    ...
    """
        ...
        parser.add_argument('--dry-run', action='store_true', help='If provided, the ssh command is printed to standard out rather than being executed.')
        parser.add_argument('user_host', completer=completers.InstancesCompleter, metavar='[USER@]INSTANCE', help="        Specifies the user/instance for the serial port connection.\n\n        ``USER'' specifies the username to authenticate as. If omitted,\n        the current OS user is selected.\n        ")
        parser.add_argument('--port', default=1, help='        The number of the requested serial port. Can be 1-4, default is 1.\n\n        Instances can support up to four serial ports. By default, this\n        command will connect to the first serial port. Setting this flag\n        will connect to the requested serial port.\n        ')
        parser.add_argument('--extra-args', type=arg_parsers.ArgDict(min_length=1), default={}, metavar='KEY=VALUE', help='        Optional arguments can be passed to the serial port connection by\n        passing key-value pairs to this flag, such as max-connections=N or\n        replay-lines=N. See {0} for additional options.\n        '.format(SERIAL_PORT_HELP))
        parser.add_argument('--location', help='        If provided, the region in which the serial console connection will\n        occur. Must be the region of the VM to connect to.\n        ')
        flags.AddZoneFlag(parser, resource_type='instance', operation_type='connect to')
    ...
    def Run(self, args):
        ...
        if not remote:
            raise ssh_utils.ArgumentError('Expected argument of the form [USER@]INSTANCE. Received [{0}].'.format(args.user_host))
        if not remote.user:
            remote.user = ssh.GetDefaultSshUsername()
        public_key = ssh_helper.keys.GetPublicKey().ToEntry(include_comment=True)
        instance_ref = instance_flags.SSH_INSTANCE_RESOLVER.ResolveResources([remote.host], compute_scope.ScopeEnum.ZONE, args.zone, holder.resources, scope_lister=instance_flags.GetInstanceZoneScopeLister(client))[0]
        ...
        if int(http_response[0]['status']) == httplib.OK:
            ...
            log.warning('Unable to download and update Host Key for [{0}] from [{1}]. Attempting to connect using existing Host Key in [{2}]. If the connection fails, please try again to update the Host Key.'.format(gateway, hostkey_url, known_hosts.file_path))
        else:
            log.warning('Unable to download Host Key for [{0}] from [{1}]. No Host Key found in known_hosts file [{2}]. gcloud does not have a fallback Host Key and will therefore terminate the connection attempt. If the problem persists, try updating gcloud and connecting again.'.format(gateway, hostkey_url, known_hosts.file_path))
            return
        ...
        oslogin_state = ssh.GetOsloginState(instance, project, remote.user, public_key, expiration_micros, self.ReleaseTrack(), messages=holder.client.messages)
        remote.user = oslogin_state.user
        port = 'port={0}'.format(args.port)
        constructed_username_list = [instance_ref.project, instance_ref.zone, instance_ref.Name(), remote.user, port]
        if not args.dry_run:
        ...
        cmd = ssh.SSHCommand(serial_remote, identity_file=identity_file, port=CONNECTION_PORT, options=options)
        if args.dry_run:
            log.out.Print(' '.join(cmd.Build(ssh_helper.env)))
            return
        if not oslogin_state.oslogin_enabled:
            ssh_helper.EnsureSSHKeyExists(client, remote.user, instance, project, expiration)
