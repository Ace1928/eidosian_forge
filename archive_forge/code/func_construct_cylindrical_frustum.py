...
def construct_cylindrical_frustum():
    ...
    vertex_data = GeomVertexData('cylindrical_frustum_vertices_and_colors', vertex_format, Geom.UHStatic)
    ...
    heights = np.array([0, 1], dtype=np.float32) * h
    ...
    triangle_indices = np.array([[i, (i + 1) % num_segments, i + num_segments] if i % 2 == 0 else [(i + 1) % num_segments, i + num_segments, (i + 1) % num_segments + num_segments] for i in range(num_segments)] + [[i, i + num_segments, (i + 1) % num_segments + num_segments] if i % 2 == 0 else [i + num_segments, (i + 1) % num_segments + num_segments, (i + 1) % num_segments] for i in range(num_segments)], dtype=np.int32)
