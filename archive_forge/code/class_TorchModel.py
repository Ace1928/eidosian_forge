...
class TorchModel(nn.Module, Model, abc.ABC):
    ...
    def __init__(self, config: ModelConfig):
        ...
        if not is_input_decorated(self.forward):
            raise ValueError(f"`{type(self).__name__}.forward()` not decorated with input specification. Decorate it with @check_input_specs() to define a specification and resolve this Error. If you don't want to check anything, you can use an empty spec.")
        if is_output_decorated(self.forward):
            if log_once('torch_model_forward_output_decorated'):
                logger.warning(f"`{type(self).__name__}.forward()` decorated with output specification. This is not recommended for torch models that are used with torch.compile() because it breaks torch dynamo's graph. This can lead lead to slower execution.Remove @check_output_specs() from the forward() method to resolve this.")

    @check_input_specs('input_specs')
    def forward(self, inputs: Union[dict, TensorType], **kwargs) -> Union[dict, TensorType]:
