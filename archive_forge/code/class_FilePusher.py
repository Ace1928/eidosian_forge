...
class FilePusher:
    ...
    def __init__(self, api: 'internal_api.Api', file_stream: 'file_stream.FileStreamApi', settings: Optional['SettingsStatic']=None) -> None:
        self._api = api
    ...
    def print_status(self, prefix: bool=True) -> None:
        ...
        while True:
            if not self.is_alive():
                stop = True
            summary = self._stats.summary()
            line = ' {:.2f}MB of {:.2f}MB uploaded ({:.2f}MB deduped)\r'.format(summary.uploaded_bytes / 1048576.0, summary.total_bytes / 1048576.0, summary.deduped_bytes / 1048576.0)
        ...
        if dedupe_fraction > 0.01:
            wandb.termlog('W&B sync reduced upload amount by %.1f%%             ' % (dedupe_fraction * 100), prefix=prefix)
