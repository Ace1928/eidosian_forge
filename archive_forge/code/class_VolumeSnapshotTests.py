...
class VolumeSnapshotTests(common.BaseVolumeTests):
    ...
    def setUpClass(cls):
        ...
        cmd_output = cls.openstack('volume create ' + '--size 1 ' + cls.VOLLY, parse_output=True)
    ...
    def test_volume_snapshot_list(self):
        ...
        self.assertEqual(name1, cmd_output['name'])
        self.assertEqual(self.VOLUME_ID, cmd_output['volume_id'])
        ...
        self.assertEqual(1, cmd_output['size'])
        self.wait_for_status('volume snapshot', name2, 'available')
        raw_output = self.openstack('volume snapshot set ' + '--state error_deleting ' + name2)
        self.assertOutput('', raw_output)
        cmd_output = self.openstack('volume snapshot list ' + '--long ' + '--status error_deleting', parse_output=True)
        names = [x['Name'] for x in cmd_output]
        self.assertNotIn(name1, names)
        self.assertIn(name2, names)
        raw_output = self.openstack('volume snapshot set ' + '--state error ' + name2)
        self.assertOutput('', raw_output)
        cmd_output = self.openstack('volume snapshot list ' + '--long ' + '--status error', parse_output=True)
        names = [x['Name'] for x in cmd_output]
        self.assertNotIn(name1, names)
        self.assertIn(name2, names)
        cmd_output = self.openstack('volume snapshot list ' + '--volume ' + self.VOLLY, parse_output=True)
        names = [x['Name'] for x in cmd_output]
        self.assertIn(name1, names)
        self.assertIn(name2, names)
        cmd_output = self.openstack('volume snapshot list ' + '--name ' + name1, parse_output=True)
        names = [x['Name'] for x in cmd_output]
        self.assertIn(name1, names)
