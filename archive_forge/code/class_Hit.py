...
class Hit(_BaseSearchObject):
    ...
    def __init__(self, hsps=(), id=None, query_id=None):
        ...
        for attr in ('query_id', 'query_description', 'hit_id', 'hit_description'):
            if len({getattr(hsp, attr) for hsp in hsps}) > 1:
                raise ValueError('Hit object can not contain HSPs with more than one %s.' % attr)
        self._items = []
        for hsp in hsps:
    ...
    def __str__(self):
        ...
        if not self.hsps:
            ...
            lines.append(' HSPs: %s  %s  %s  %s  %s  %s' % ('-' * 4, '-' * 8, '-' * 9, '-' * 6, '-' * 15, '-' * 21))
            pattern = '%11s  %8s  %9s  %6s  %15s  %21s'
            lines.append(pattern % ('#', 'E-value', 'Bit score', 'Span', 'Query range', 'Hit range'))
            ...
            for idx, hsp in enumerate(self.hsps):
                ...
                lines.append(pattern % (idx, evalue, bitscore, aln_span, query_range, hit_range))
        return '\n'.join(lines)
    ...
    description = optionalcascade('_description', 'hit_description', 'Hit description')
    query_description = optionalcascade('_query_description', 'query_description', 'Description of the query that produced the hit')
    id = optionalcascade('_id', 'hit_id', 'Hit ID string.')
    query_id = optionalcascade('_query_id', 'query_id', 'ID string of the query that produced the hit')
