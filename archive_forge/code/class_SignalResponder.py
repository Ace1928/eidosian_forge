...
class SignalResponder(stack_user.StackUser):
    ...
    def _get_ec2_signed_url(self, signal_type=SIGNAL, never_expire=False):
        ...
        if not access_key or not secret_key:
            ...
            secret_key = self.data().get('secret_key')
            if not access_key or not secret_key:
                LOG.warning('Cannot generate signed url, unable to create keypair')
                return
        ...
        params = {'SignatureMethod': 'HmacSHA256', 'SignatureVersion': '2', 'AWSAccessKeyId': access_key}
        if not never_expire:
            params['Timestamp'] = timeutils.utcnow().strftime('%Y-%m-%dT%H:%M:%SZ')
        request = {'host': host_url.netloc.lower(), 'verb': SIGNAL_VERB[signal_type], 'path': unquoted_path, 'params': params}
