...
class GoogleDriveLoader(BaseLoader, BaseModel):
    ...
    def validate_inputs(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if file_types:
            if values.get('document_ids') or values.get('file_ids'):
                raise ValueError('file_types can only be given when folder_id is given, (not when document_ids or file_ids are given).')
            type_mapping = {'document': 'application/vnd.google-apps.document', 'sheet': 'application/vnd.google-apps.spreadsheet', 'pdf': 'application/pdf'}
            ...
            for file_type in file_types:
                if file_type not in allowed_types:
                    raise ValueError(f'Given file type {file_type} is not supported. Supported values are: {short_names}; and their full-form names: {full_names}')
    ...
    def _load_credentials(self) -> Any:
        ...
        try:
            ...
            raise ImportError('You must run `pip install --upgrade google-api-python-client google-auth-httplib2 google-auth-oauthlib` to use the Google Drive loader.')
        creds = None
