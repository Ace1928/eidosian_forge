...
def visit_ilike_op_binary(self, binary, operator, **kw):
    ...
    return '%s ILIKE %s' % (self.process(binary.left, **kw), self.process(binary.right, **kw)) + (' ESCAPE ' + self.render_literal_value(escape, sqltypes.STRINGTYPE) if escape is not None else '')