...
class Teradata(Dialect):
    ...
    class Parser(parser.Parser):
        ...
        def _parse_update(self) -> exp.Update:
            return self.expression(exp.Update, **{'this': self._parse_table(alias_tokens=self.UPDATE_ALIAS_TOKENS), 'from': self._parse_from(joins=True), 'expressions': self._match(TokenType.SET) and self._parse_csv(self._parse_equality), 'where': self._parse_where()})
    ...
    class Generator(generator.Generator):
        ...
        TYPE_MAPPING = {**generator.Generator.TYPE_MAPPING, exp.DataType.Type.GEOMETRY: 'ST_GEOMETRY', exp.DataType.Type.DOUBLE: 'DOUBLE PRECISION'}
        PROPERTIES_LOCATION = {**generator.Generator.PROPERTIES_LOCATION, exp.OnCommitProperty: exp.Properties.Location.POST_INDEX, exp.PartitionedByProperty: exp.Properties.Location.POST_EXPRESSION, exp.StabilityProperty: exp.Properties.Location.POST_CREATE}
        TRANSFORMS = {**generator.Generator.TRANSFORMS, exp.ArgMax: rename_func('MAX_BY'), exp.ArgMin: rename_func('MIN_BY'), exp.ArraySize: rename_func('CARDINALITY'), exp.Max: max_or_greatest, exp.Min: min_or_least, exp.Pow: lambda self, e: self.binary(e, '**'), exp.Select: transforms.preprocess([transforms.eliminate_distinct_on, transforms.eliminate_semi_and_anti_joins]), exp.StrToDate: lambda self, e: f'CAST({self.sql(e, 'this')} AS DATE FORMAT {self.format_time(e)})', exp.ToChar: lambda self, e: self.function_fallback_sql(e), exp.ToNumber: to_number_with_nls_param, exp.Use: lambda self, e: f'DATABASE {self.sql(e, 'this')}', exp.CurrentTimestamp: lambda *_: 'CURRENT_TIMESTAMP', exp.DateAdd: _date_add_sql('+'), exp.DateSub: _date_add_sql('-'), exp.Quarter: lambda self, e: self.sql(exp.Extract(this='QUARTER', expression=e.this))}
        ...
        def createable_sql(self, expression: exp.Create, locations: t.DefaultDict) -> str:
            ...
            if kind == 'TABLE' and locations.get(exp.Properties.Location.POST_NAME):
                ...
                this_properties = self.properties(exp.Properties(expressions=locations[exp.Properties.Location.POST_NAME]), wrapped=False, prefix=',')
