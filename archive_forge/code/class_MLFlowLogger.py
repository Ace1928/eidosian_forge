...
class MLFlowLogger(Logger):
    ...
    def __init__(self, experiment_name: str='lightning_logs', run_name: Optional[str]=None, tracking_uri: Optional[str]=os.getenv('MLFLOW_TRACKING_URI'), tags: Optional[Dict[str, Any]]=None, save_dir: Optional[str]='./mlruns', log_model: Literal[True, False, 'all']=False, prefix: str='', artifact_location: Optional[str]=None, run_id: Optional[str]=None):
    ...
    def experiment(self) -> 'MlflowClient':
        """Actual MLflow object. To use MLflow features in your :class:`~pytorch_lightning.core.LightningModule` do the
        following.
        ...
        if self._experiment_id is None:
            ...
            if expt is not None:
                ...
                self._experiment_id = self._mlflow_client.create_experiment(name=self._experiment_name, artifact_location=self._artifact_location)
        if self._run_id is None:
            if self._run_name is not None:
                self.tags = self.tags or {}
                from mlflow.utils.mlflow_tags import MLFLOW_RUN_NAME
                if MLFLOW_RUN_NAME in self.tags:
                    log.warning(f'The tag {MLFLOW_RUN_NAME} is found in tags. The value will be overridden by {self._run_name}.')
    ...
    def log_metrics(self, metrics: Mapping[str, float], step: Optional[int]=None) -> None:
        ...
        for k, v in metrics.items():
            ...
            if k != new_k:
                rank_zero_warn(f"MLFlow only allows '_', '/', '.' and ' ' special characters in metric name. Replacing {k} with {new_k}.", category=RuntimeWarning)
                k = new_k
    ...
    def _scan_and_log_checkpoints(self, checkpoint_callback: ModelCheckpoint) -> None:
        ...
        for t, p, s, tag in checkpoints:
            metadata = {'score': s.item() if isinstance(s, Tensor) else s, 'original_filename': Path(p).name, 'Checkpoint': {k: getattr(checkpoint_callback, k) for k in ['monitor', 'mode', 'save_last', 'save_top_k', 'save_weights_only', '_every_n_train_steps', '_every_n_val_epochs'] if hasattr(checkpoint_callback, k)}}
