...
def plot_rank(axes, length_plotters, rows, cols, figsize, plotters, bins, kind, colors, ref_line, labels, labeller, ref_line_kwargs, bar_kwargs, vlines_kwargs, marker_vlines_kwargs, backend_kwargs, show):
    """Bokeh rank plot."""
    ...
    if axes is None:
        axes = create_axes_grid(length_plotters, rows, cols, figsize=figsize, sharex=True, sharey=True, backend_kwargs=backend_kwargs)
    else:
        axes = np.atleast_2d(axes)
    for ax, (var_name, selection, isel, var_data) in zip((item for item in axes.flatten() if item is not None), plotters):
        ...
        if kind == 'bars':
            for idx, counts in enumerate(all_counts):
                ...
                ax.vbar(x=bin_ary, top=y_ticks[-1] + counts, bottom=y_ticks[-1], fill_color=colors[idx], **bar_kwargs)
                if ref_line:
            ...
            for idx, counts in enumerate(all_counts):
                ax.circle(bin_ary, counts, fill_color=colors[idx], line_color=colors[idx], **marker_vlines_kwargs)
        ...
        if labels:
            ...
            ax.xaxis.major_label_overrides = dict(zip(map(str, y_ticks), map(str, range(len(y_ticks)))))
        else:
