...
def plot_ts(x_plotters, y_plotters, y_mean_plotters, y_hat_plotters, y_holdout_plotters, x_holdout_plotters, y_forecasts_plotters, y_forecasts_mean_plotters, num_samples, backend_kwargs, y_kwargs, y_hat_plot_kwargs, y_mean_plot_kwargs, vline_kwargs, length_plotters, rows, cols, textsize, figsize, legend, axes, show):
    ...
    for i, ax_i in enumerate(np.ravel(axes)[:length_plotters]):
        ...
        if y_forecasts_plotters is not None:
            ...
            *_, x_forecasts_plotters_i = x_holdout_plotters[i]
            for j in range(num_samples):
                ax_i.plot(x_forecasts_plotters_i, y_forecasts_plotters_i[..., j], **y_hat_plot_kwargs)
            *_, x_forecasts_mean_plotters_i = x_holdout_plotters[i]
            *_, y_forecasts_mean_plotters_i = y_forecasts_mean_plotters[i]
            ax_i.plot(x_forecasts_mean_plotters_i, y_forecasts_mean_plotters_i, **y_mean_plot_kwargs)
            ax_i.axvline(x_plotters_i[-1], **vline_kwargs)
        if legend:
