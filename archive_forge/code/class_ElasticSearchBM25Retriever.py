...
class ElasticSearchBM25Retriever(BaseRetriever):
    ...
    def create(cls, elasticsearch_url: str, index_name: str, k1: float=2.0, b: float=0.75) -> ElasticSearchBM25Retriever:
        ...
        settings = {'analysis': {'analyzer': {'default': {'type': 'standard'}}}, 'similarity': {'custom_bm25': {'type': 'BM25', 'k1': k1, 'b': b}}}
        mappings = {'properties': {'content': {'type': 'text', 'similarity': 'custom_bm25'}}}
        es.indices.create(index=index_name, mappings=mappings, settings=settings)
        return cls(client=es, index_name=index_name)

    def add_texts(self, texts: Iterable[str], refresh_indices: bool=True) -> List[str]:
        ...
        for i, text in enumerate(texts):
            _id = str(uuid.uuid4())
            request = {'_op_type': 'index', '_index': self.index_name, 'content': text, '_id': _id}
            ids.append(_id)
