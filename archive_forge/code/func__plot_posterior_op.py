...
def _plot_posterior_op(idx, values, var_name, selection, ax, bw, circular, linewidth, bins, kind, point_estimate, hdi_prob, multimodal, skipna, ref_val, rope, ref_val_color, rope_color, ax_labelsize, round_to: Optional[int]=None, **kwargs):
    ...
    def display_point_estimate(max_data):
        ...
        point_text = '{point_estimate}={point_value:.{sig_figs}g}'.format(point_estimate=point_estimate, point_value=point_value, sig_figs=sig_figs)
    ...
    if kind == 'kde' and values.dtype.kind == 'f':
        ...
        plot_kde(values, bw=bw, is_circular=circular, fill_kwargs={'fill_alpha': kwargs.pop('fill_alpha', 0)}, plot_kwargs=kwargs, ax=ax, rug=False, backend='bokeh', backend_kwargs={}, show=False)
        max_data = values.max()
        ...
        max_data = hist.max()
        ax.quad(top=hist, bottom=0, left=edges[:-1], right=edges[1:], fill_alpha=0.35, line_alpha=0.35)
