...
def _PrintWarningsForUnpersistedCert(cls, args):
    ...
    if unused_args:
        ...
        log.warning('{names} {verb} specified but will not be used since the issuing CA pool is in the DevOps tier, which does not expose certificate lifecycle.'.format(names=names, verb=verb))