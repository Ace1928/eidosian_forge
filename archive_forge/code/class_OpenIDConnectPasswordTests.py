...
class OpenIDConnectPasswordTests(OpenIDConnectBaseTests, utils.TestCase):
    ...
    def test_options(self):
        ...
        self.assertTrue(set(['username', 'password', 'openid-scope']).issubset(set([o.name for o in options])))
    ...
    def test_basic(self):
        ...
        oidc = self.create(username=username, password=password, identity_provider=identity_provider, protocol=protocol, access_token_endpoint=access_token_endpoint, client_id=client_id, client_secret=client_secret, scope=scope)
