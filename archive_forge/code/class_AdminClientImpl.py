...
class AdminClientImpl(AdminClientInterface):
    ...
    def create_subscription(self, subscription: Subscription, target: Union[BacklogLocation, PublishTime, EventTime]=BacklogLocation.END, starting_offset: Optional[BacklogLocation]=None) -> Subscription:
        if starting_offset:
        ...
        requires_update = requires_seek and subscription.export_config and (subscription.export_config.desired_state == ExportConfig.State.ACTIVE)
        if requires_update:
        ...
        response = self._underlying.create_subscription(request=CreateSubscriptionRequest(parent=str(path.to_location_path()), subscription=subscription, subscription_id=path.name, skip_backlog=skip_backlog))
        if requires_seek:
            self.seek_subscription(subscription_path=path, target=target)
        if requires_update:
            response = self.update_subscription(subscription=Subscription(name=response.name, export_config=ExportConfig(desired_state=ExportConfig.State.ACTIVE)), update_mask=FieldMask(paths=['export_config.desired_state']))
        return response
    ...
    def list_reservations(self, location_path: LocationPath) -> List[Reservation]:
        return [x for x in self._underlying.list_reservations(parent=str(location_path))]

    def update_reservation(self, reservation: Reservation, update_mask: FieldMask) -> Reservation:
        return self._underlying.update_reservation(reservation=reservation, update_mask=update_mask)
    ...
    def list_reservation_topics(self, reservation_path: ReservationPath) -> List[TopicPath]:
        subscription_strings = [x for x in self._underlying.list_reservation_topics(name=str(reservation_path))]
