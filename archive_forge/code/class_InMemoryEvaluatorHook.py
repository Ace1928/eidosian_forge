...
class InMemoryEvaluatorHook(tf.compat.v1.train.SessionRunHook):
  ...
  * It doesn't support custom saver logic (such as ExponentialMovingAverage
  ...
  """
    ...
    def __init__(self, estimator, input_fn, steps=None, hooks=None, name=None, every_n_iter=100):
    ...
    """
        ...
        if estimator.config.num_ps_replicas > 0 or estimator.config.num_worker_replicas > 1:
            raise ValueError('InMemoryEvaluator supports only single machine (aka Local) setting.')
        self._estimator = estimator
        self._input_fn = input_fn
        self._steps = steps
        self._name = name
        self._every_n_iter = every_n_iter
        self._eval_dir = os.path.join(self._estimator.model_dir, 'eval' if not name else 'eval_' + name)
        self._graph = None
    ...
    def begin(self):
        ...
        with self._graph.as_default():
            self._scaffold, self._update_op, self._eval_dict, self._all_hooks = self._estimator._evaluate_build_graph(self._input_fn, self._hooks, checkpoint_path=None)
            ...
            self._var_name_to_eval_var = {v.name: v for v in tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES)}
            self._var_name_to_placeholder = {v.name: tf.compat.v1.placeholder(v.dtype) for v in tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES)}
