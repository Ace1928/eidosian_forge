...
class Radar(Figure):
    ...
    band_type = Enum(['circle', 'polygon'], default_value='circle', allow_none=True).tag(sync=True)
    ...
    def __init__(self, **kwargs):
        ...
        self.loops = Lines(scales=self.scales, display_legend=True, colors=self.colors, stroke_width=2, fill='inside' if self.fill else 'none', marker='circle', marker_size=50)
        self.band_labels = Label(scales=self.scales, default_size=12, font_weight='normal', apply_clip=False, colors=['#ccc'], align='middle')
        self.spoke_labels = Label(scales=self.scales, default_size=14, font_weight='bold', apply_clip=False, colors=['#ccc'], align='middle')
        self.marks = [self.spokes, self.bands, self.loops, self.band_labels, self.spoke_labels]
    ...
    def update_data(self, *args):
        ...
        spoke_data_x, spoke_data_y = (np.sin(self.spoke_data_t), np.cos(self.spoke_data_t))
        with self.spokes.hold_sync():
            self.spokes.x = np.column_stack([self.scaled_band_data[1] * spoke_data_x, spoke_data_x])
            self.spokes.y = np.column_stack([self.scaled_band_data[1] * spoke_data_y, spoke_data_y])
        scaled_data = (self.data.values - self.data_range[0]) / (self.data_range[1] - self.data_range[0])
