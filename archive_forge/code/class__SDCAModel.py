...
class _SDCAModel(object):
  ...
  """
    ...
    def __init__(self, examples, variables, options):
        ...
        supported_losses = ('logistic_loss', 'squared_loss', 'hinge_loss', 'smooth_hinge_loss', 'poisson_loss')
        if options['loss_type'] not in supported_losses:
            raise ValueError('Unsupported loss_type: ', options['loss_type'])
        self._assert_specified(['example_labels', 'example_weights', 'example_ids', 'sparse_features', 'dense_features'], examples)
        self._assert_list(['sparse_features', 'dense_features'], examples)
        self._assert_specified(['sparse_features_weights', 'dense_features_weights'], variables)
        self._assert_list(['sparse_features_weights', 'dense_features_weights'], variables)
        self._assert_specified(['loss_type', 'symmetric_l2_regularization', 'symmetric_l1_regularization'], options)
        ...
        if options['symmetric_l2_regularization'] <= 1.0:
            tf.compat.v1.logging.warn('symmetric_l2_regularization for SDCA should typically be larger than for online optimization methods. Recommended value is of the order of the average L2 norm of the training examples.')
        ...
        self._create_slots()
        self._hashtable = _ShardedMutableDenseHashTable(key_dtype=tf.dtypes.int64, value_dtype=tf.dtypes.float32, num_shards=self._num_table_shards(), default_value=[0.0, 0.0, 0.0, 0.0], empty_key=[0, 0], deleted_key=[1, 1])
            ...
            for name in ['sparse_features_weights', 'dense_features_weights']:
                for var in self._variables[name]:
                    for v in self._var_to_list(var):
                        weights = internal_convert_to_tensor(v)
                        with tf.compat.v1.device(weights.device):
                            sums.append(tf.math.reduce_sum(tf.math.square(tf.cast(weights, tf.dtypes.float64))))
    ...
    def _convert_n_to_tensor(self, input_list, as_ref=False):
        ...
        for x in input_list:
            ...
            output_list.append(internal_convert_to_tensor(tensor_to_convert, as_ref=as_ref))
        return output_list
    ...
    """
        with name_scope(name, 'sdca/minimize'):
            ...
            example_ids_hashed = tf.compat.v1.train.sdca_fprint(internal_convert_to_tensor(self._examples['example_ids']))
            ...
            gather_ids_by_var = {}
            for v_num, (w, i) in enumerate(zip(self._slots['unshrunk_sparse_features_weights'], sparse_feature_indices)):
                sparse_idx = tf.cast(tf.unique(tf.cast(i, tf.dtypes.int32))[0], tf.dtypes.int64)
                ...
                if isinstance(w, list) or isinstance(w, var_ops.PartitionedVariable):
                    ...
                    dim_0_size = self._get_first_dimension_size_statically(w, num_partitions)
                    if tf.compat.dimension_value(dim_0_size):
                        num_total_ids = tf.constant(tf.compat.dimension_value(dim_0_size), flat_ids.dtype)
                    else:
                        dim_0_sizes = []
                        for p in range(num_partitions):
                        ...
                        num_total_ids = tf.math.reduce_sum(tf.cast(tf.stack(dim_0_sizes), flat_ids.dtype))
                    ids_per_partition = num_total_ids // num_partitions
                    extras = num_total_ids % num_partitions
                    p_assignments = tf.math.maximum(flat_ids // (ids_per_partition + 1), (flat_ids - extras) // ids_per_partition)
                    new_ids = tf.where(p_assignments < extras, flat_ids % (ids_per_partition + 1), (flat_ids - extras) % ids_per_partition)
                    p_assignments = tf.cast(p_assignments, tf.dtypes.int32)
                    gather_ids = tf.dynamic_partition(new_ids, p_assignments, num_partitions)
