...
class TestOrchestrationStackTemplate(TestOrchestrationProxy):
    ...
    def test_get_stack_template_with_stack_identity(self, mock_find):
        ...
        self._verify('openstack.proxy.Proxy._get', self.proxy.get_stack_template, method_args=['IDENTITY'], expected_args=[stack_template.StackTemplate], expected_kwargs={'requires_id': False, 'stack_name': stack_name, 'stack_id': stack_id})
    ...
    def test_get_stack_template_with_stack_object(self):
        ...
        stack_name = 'test_stack'
        stk = stack.Stack(id=stack_id, name=stack_name)
        self._verify('openstack.proxy.Proxy._get', self.proxy.get_stack_template, method_args=[stk], expected_args=[stack_template.StackTemplate], expected_kwargs={'requires_id': False, 'stack_name': stack_name, 'stack_id': stack_id})