...
@deprecated(since='0.0.21', removal='0.2.0', alternative_import='langchain_astradb.AstraDBVectorStore')
class AstraDB(VectorStore):
    ...
    def __init__(self, *, embedding: Embeddings, collection_name: str, token: Optional[str]=None, api_endpoint: Optional[str]=None, astra_db_client: Optional[LibAstraDB]=None, async_astra_db_client: Optional[AsyncAstraDB]=None, namespace: Optional[str]=None, metric: Optional[str]=None, batch_size: Optional[int]=None, bulk_insert_batch_concurrency: Optional[int]=None, bulk_insert_overwrite_concurrency: Optional[int]=None, bulk_delete_concurrency: Optional[int]=None, setup_mode: SetupMode=SetupMode.SYNC, pre_delete_collection: bool=False) -> None:
        ...
        self.bulk_insert_overwrite_concurrency: int = bulk_insert_overwrite_concurrency or DEFAULT_BULK_INSERT_OVERWRITE_CONCURRENCY
        ...
        self.astra_env = _AstraDBCollectionEnvironment(collection_name=collection_name, token=token, api_endpoint=api_endpoint, astra_db_client=astra_db_client, async_astra_db_client=async_astra_db_client, namespace=namespace, setup_mode=setup_mode, pre_delete_collection=pre_delete_collection, embedding_dimension=embedding_dimension, metric=metric)
    ...
    def delete(self, ids: Optional[List[str]]=None, concurrency: Optional[int]=None, **kwargs: Any) -> Optional[bool]:
        ...
        """
        if kwargs:
            warnings.warn(f"Method 'delete' of AstraDB vector store invoked with unsupported arguments ({', '.join(sorted(kwargs.keys()))}), which will be ignored.")
        if ids is None:
    ...
    async def adelete(self, ids: Optional[List[str]]=None, concurrency: Optional[int]=None, **kwargs: Any) -> Optional[bool]:
        """Delete by vector ids.
        ...
        """
        if kwargs:
            warnings.warn(f"Method 'adelete' of AstraDB vector store invoked with unsupported arguments ({', '.join(sorted(kwargs.keys()))}), which will be ignored.")
        if ids is None:
            raise ValueError('No ids provided to delete.')
    ...
    async def adelete_collection(self) -> None:
        ...
        """
        await self.astra_env.aensure_db_setup()
        await self.async_astra_db.delete_collection(collection_name=self.collection_name)
    ...
    def add_texts(self, texts: Iterable[str], metadatas: Optional[List[dict]]=None, ids: Optional[List[str]]=None, *, batch_size: Optional[int]=None, batch_concurrency: Optional[int]=None, overwrite_concurrency: Optional[int]=None, **kwargs: Any) -> List[str]:
        ...
        """
        if kwargs:
            warnings.warn(f"Method 'add_texts' of AstraDB vector store invoked with unsupported arguments ({', '.join(sorted(kwargs.keys()))}), which will be ignored.")
        self.astra_env.ensure_db_setup()
        ...
        with ThreadPoolExecutor(max_workers=_b_max_workers) as tpe:
            all_ids_nested = tpe.map(_handle_batch, batch_iterate(batch_size or self.batch_size, documents_to_insert))
        return [iid for id_list in all_ids_nested for iid in id_list]

    async def aadd_texts(self, texts: Iterable[str], metadatas: Optional[List[dict]]=None, ids: Optional[List[str]]=None, *, batch_size: Optional[int]=None, batch_concurrency: Optional[int]=None, overwrite_concurrency: Optional[int]=None, **kwargs: Any) -> List[str]:
        """Run texts through the embeddings and add them to the vectorstore.
        ...
        """
        if kwargs:
            warnings.warn(f"Method 'aadd_texts' of AstraDB vector store invoked with unsupported arguments ({', '.join(sorted(kwargs.keys()))}), which will be ignored.")
        await self.astra_env.aensure_db_setup()
        ...
        async def _handle_batch(document_batch: List[DocDict]) -> List[str]:
            im_result = await self.async_collection.insert_many(documents=document_batch, options={'ordered': False}, partial_failures_allowed=True)
            batch_inserted, missing_from_batch = self._get_missing_from_batch(document_batch, im_result)

            async def _handle_missing_document(missing_document: DocDict) -> str:
                replacement_result = await self.async_collection.find_one_and_replace(filter={'_id': missing_document['_id']}, replacement=missing_document)
                return replacement_result['data']['document']['_id']
            _u_max_workers = overwrite_concurrency or self.bulk_insert_overwrite_concurrency
            batch_replaced = await gather_with_concurrency(_u_max_workers, *[_handle_missing_document(doc) for doc in missing_from_batch])
            return batch_inserted + batch_replaced
        _b_max_workers = batch_concurrency or self.bulk_insert_batch_concurrency
        all_ids_nested = await gather_with_concurrency(_b_max_workers, *[_handle_batch(batch) for batch in batch_iterate(batch_size or self.batch_size, documents_to_insert)])
        return [iid for id_list in all_ids_nested for iid in id_list]
    ...
    async def asimilarity_search_with_score_id_by_vector(self, embedding: List[float], k: int=4, filter: Optional[Dict[str, Any]]=None) -> List[Tuple[Document, float, str]]:
        """Return docs most similar to embedding vector with score and id.
        ...
        await self.astra_env.aensure_db_setup()
        metadata_parameter = self._filter_to_metadata(filter)
        return [(Document(page_content=hit['content'], metadata=hit['metadata']), hit['$similarity'], hit['_id']) async for hit in self.async_collection.paginated_find(filter=metadata_parameter, sort={'$vector': embedding}, options={'limit': k, 'includeSimilarity': True}, projection={'_id': 1, 'content': 1, 'metadata': 1})]
    ...
    async def asimilarity_search_with_score_id(self, query: str, k: int=4, filter: Optional[Dict[str, Any]]=None) -> List[Tuple[Document, float, str]]:
        """Return docs most similar to the query with score and id.
        ...
        return await self.asimilarity_search_with_score_id_by_vector(embedding=embedding_vector, k=k, filter=filter)

    def similarity_search_with_score_by_vector(self, embedding: List[float], k: int=4, filter: Optional[Dict[str, Any]]=None) -> List[Tuple[Document, float]]:
        ...
        """
        return [(doc, score) for doc, score, doc_id in self.similarity_search_with_score_id_by_vector(embedding=embedding, k=k, filter=filter)]
    ...
    async def asimilarity_search_with_score_by_vector(self, embedding: List[float], k: int=4, filter: Optional[Dict[str, Any]]=None) -> List[Tuple[Document, float]]:
        ...
        Returns:
            The list of (Document, score), the most similar to the query vector.
        """
        return [(doc, score) for doc, score, doc_id in await self.asimilarity_search_with_score_id_by_vector(embedding=embedding, k=k, filter=filter)]

    def similarity_search(self, query: str, k: int=4, filter: Optional[Dict[str, Any]]=None, **kwargs: Any) -> List[Document]:
    ...
    async def asimilarity_search(self, query: str, k: int=4, filter: Optional[Dict[str, Any]]=None, **kwargs: Any) -> List[Document]:
        """Return docs most similar to query.
        ...
        """
        embedding_vector = await self.embedding.aembed_query(query)
        return await self.asimilarity_search_by_vector(embedding_vector, k, filter=filter)

    def similarity_search_by_vector(self, embedding: List[float], k: int=4, filter: Optional[Dict[str, Any]]=None, **kwargs: Any) -> List[Document]:
    ...
    async def asimilarity_search_by_vector(self, embedding: List[float], k: int=4, filter: Optional[Dict[str, Any]]=None, **kwargs: Any) -> List[Document]:
        """Return docs most similar to embedding vector.
        ...
        Returns:
            The list of Documents most similar to the query vector.
        """
        return [doc for doc, _ in await self.asimilarity_search_with_score_by_vector(embedding, k, filter=filter)]
    ...
    def similarity_search_with_score(self, query: str, k: int=4, filter: Optional[Dict[str, Any]]=None) -> List[Tuple[Document, float]]:
        ...
        """
        embedding_vector = self.embedding.embed_query(query)
        return self.similarity_search_with_score_by_vector(embedding_vector, k, filter=filter)

    async def asimilarity_search_with_score(self, query: str, k: int=4, filter: Optional[Dict[str, Any]]=None) -> List[Tuple[Document, float]]:
        """Return docs most similar to query with score.
        ...
        """
        embedding_vector = await self.embedding.aembed_query(query)
        return await self.asimilarity_search_with_score_by_vector(embedding_vector, k, filter=filter)
    ...
    def max_marginal_relevance_search_by_vector(self, embedding: List[float], k: int=4, fetch_k: int=20, lambda_mult: float=0.5, filter: Optional[Dict[str, Any]]=None, **kwargs: Any) -> List[Document]:
        ...
        self.astra_env.ensure_db_setup()
        metadata_parameter = self._filter_to_metadata(filter)
        prefetch_hits = list(self.collection.paginated_find(filter=metadata_parameter, sort={'$vector': embedding}, options={'limit': fetch_k, 'includeSimilarity': True}, projection={'_id': 1, 'content': 1, 'metadata': 1, '$vector': 1}))
        return self._get_mmr_hits(embedding, k, lambda_mult, prefetch_hits)

    async def amax_marginal_relevance_search_by_vector(self, embedding: List[float], k: int=4, fetch_k: int=20, lambda_mult: float=0.5, filter: Optional[Dict[str, Any]]=None, **kwargs: Any) -> List[Document]:
        """Return docs selected using the maximal marginal relevance.
        ...
        await self.astra_env.aensure_db_setup()
        metadata_parameter = self._filter_to_metadata(filter)
        prefetch_hits = [hit async for hit in self.async_collection.paginated_find(filter=metadata_parameter, sort={'$vector': embedding}, options={'limit': fetch_k, 'includeSimilarity': True}, projection={'_id': 1, 'content': 1, 'metadata': 1, '$vector': 1})]
        return self._get_mmr_hits(embedding, k, lambda_mult, prefetch_hits)

    def max_marginal_relevance_search(self, query: str, k: int=4, fetch_k: int=20, lambda_mult: float=0.5, filter: Optional[Dict[str, Any]]=None, **kwargs: Any) -> List[Document]:
        """Return docs selected using the maximal marginal relevance.
        ...
        """
        embedding_vector = self.embedding.embed_query(query)
        return self.max_marginal_relevance_search_by_vector(embedding_vector, k, fetch_k, lambda_mult=lambda_mult, filter=filter)

    async def amax_marginal_relevance_search(self, query: str, k: int=4, fetch_k: int=20, lambda_mult: float=0.5, filter: Optional[Dict[str, Any]]=None, **kwargs: Any) -> List[Document]:
        """Return docs selected using the maximal marginal relevance.
        ...
        """
        embedding_vector = await self.embedding.aembed_query(query)
        return await self.amax_marginal_relevance_search_by_vector(embedding_vector, k, fetch_k, lambda_mult=lambda_mult, filter=filter)
    ...
    @classmethod
    def from_texts(cls: Type[ADBVST], texts: List[str], embedding: Embeddings, metadatas: Optional[List[dict]]=None, ids: Optional[List[str]]=None, **kwargs: Any) -> ADBVST:
        ...
        astra_db_store.add_texts(texts=texts, metadatas=metadatas, ids=ids, batch_size=kwargs.get('batch_size'), batch_concurrency=kwargs.get('batch_concurrency'), overwrite_concurrency=kwargs.get('overwrite_concurrency'))
        return astra_db_store

    @classmethod
    async def afrom_texts(cls: Type[ADBVST], texts: List[str], embedding: Embeddings, metadatas: Optional[List[dict]]=None, ids: Optional[List[str]]=None, **kwargs: Any) -> ADBVST:
        """Create an Astra DB vectorstore from raw texts.
        ...
        """
        astra_db_store = AstraDB._from_kwargs(embedding, **kwargs)
        await astra_db_store.aadd_texts(texts=texts, metadatas=metadatas, ids=ids, batch_size=kwargs.get('batch_size'), batch_concurrency=kwargs.get('batch_concurrency'), overwrite_concurrency=kwargs.get('overwrite_concurrency'))
        return astra_db_store

    @classmethod
    def from_documents(cls: Type[ADBVST], documents: List[Document], embedding: Embeddings, **kwargs: Any) -> ADBVST:
