...
@TransformationFactory.register('contrib.remove_zero_terms', doc='Remove terms 0 * var in constraints')
class RemoveZeroTerms(IsomorphicTransformation):
    ...
    CONFIG.declare('constraints_modified', ConfigValue(default={}, description='A dictionary that maps the constraints modified during the transformation to a tuple: (original_expr, modified_expr)'))
    ...
    def _apply_to(self, model, **kwargs):
        ...
        for constr in m.component_data_objects(ctype=Constraint, active=True, descend_into=True):
            ...
            nonzero_vars_indx = [i for i, _ in enumerate(repn.linear_vars) if not repn.linear_coefs[i] == 0]
            const = repn.constant
            constr_body = quicksum((repn.linear_coefs[i] * repn.linear_vars[i] for i in nonzero_vars_indx)) + const
            if constr.equality:
