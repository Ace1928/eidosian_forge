...
``$ASE_LAMMPSRUN_COMMAND``)
    ...
    ase_parameters: Dict[str, Any] = dict(specorder=None, always_triclinic=False, keep_alive=True, keep_tmp_files=False, no_data_file=False, tmp_dir=None, files=[], verbose=False, write_velocities=False, binary_dump=True, lammps_options='-echo log -screen none -log /dev/stdout', trajectory_out=None)
    lammps_parameters = dict(boundary=None, units='metal', atom_style='atomic', special_bonds=None, pair_style='lj/cut 2.5', pair_coeff=['* * 1 1'], masses=None, pair_modify=None, thermo_args=['step', 'temp', 'press', 'cpu', 'pxx', 'pyy', 'pzz', 'pxy', 'pxz', 'pyz', 'ke', 'pe', 'etotal', 'vol', 'lx', 'ly', 'lz', 'atoms'], dump_properties=['id', 'type', 'x', 'y', 'z', 'vx', 'vy', 'vz', 'fx', 'fy', 'fz'], dump_period=1)
    default_parameters = dict(ase_parameters, **lammps_parameters)
    legacy_parameters = ['specorder', 'dump_period', 'always_triclinic', 'keep_alive', 'keep_tmp_files', 'tmp_dir', 'parameters', 'no_data_file', 'files', 'write_velocities', 'trajectory_out']
    ...
    def __init__(self, label='lammps', **kwargs):
        ...
        if self.parameters.tmp_dir is None:
            ...
            if not os.path.isdir(self.parameters.tmp_dir):
                os.mkdir(self.parameters.tmp_dir, 493)
    ...
    def __setattr__(self, key, value):
        ...
        if key in self.legacy_parameters and key != 'parameters':
            ...
            warnings.warn(self.legacy_warn_string.format('{} for {}'.format(self.legacy_parameters_map[key], key)))
    ...
    def run(self, set_atoms=False):
        ...
        if all(pbc):
            ...
            warnings.warn('semi-periodic ASE cell detected - translation ' + 'to proper LAMMPS input cell might fail')
            cell = self.atoms.get_cell()
        ...
        lammps_trj_fd = NamedTemporaryFile(prefix='trj_' + label, suffix='.bin' if self.parameters.binary_dump else '', dir=tempdir, delete=not self.parameters.keep_tmp_files)
        lammps_trj = lammps_trj_fd.name
        if self.parameters.no_data_file:
            lammps_data = None
        else:
            lammps_data_fd = NamedTemporaryFile(prefix='data_' + label, dir=tempdir, delete=not self.parameters.keep_tmp_files, mode='w', encoding='ascii')
            write_lammps_data(lammps_data_fd, self.atoms, specorder=self.parameters.specorder, force_skew=self.parameters.always_triclinic, velocities=self.parameters.write_velocities, prismobj=self.prism, units=self.parameters.units, atom_style=self.parameters.atom_style)
            lammps_data = lammps_data_fd.name
            lammps_data_fd.flush()
        if not self._lmp_alive():
            command = self.get_lammps_command()
            self._lmp_handle = Popen(shlex.split(command, posix=os.name == 'posix'), stdin=PIPE, stdout=PIPE)
        ...
        if self.parameters.keep_tmp_files:
            ...
            fd = lmp_handle.stdin
        write_lammps_in(lammps_in=fd, parameters=self.parameters, atoms=self.atoms, prismobj=self.prism, lammps_trj=lammps_trj, lammps_data=lammps_data)
        if self.parameters.keep_tmp_files:
            lammps_in_fd.close()
        thr_read_log.join()
        if self.parameters.keep_tmp_files:
        ...
        if exitcode and exitcode != 0:
            raise RuntimeError('LAMMPS exited in {} with exit code: {}.'.format(tempdir, exitcode))
        ...
        trj_atoms = read_lammps_dump(infileobj=lammps_trj, order=False, index=-1, prismobj=self.prism, specorder=self.parameters.specorder)
        if set_atoms:
