    ...
    if self.external_id is not None:
        try:
            ...
            if self._default_client_plugin().is_not_found(ex):
                error_message = _('Invalid external resource: Resource %(external_id)s (%(type)s) can not be found.') % {'external_id': self.external_id, 'type': self.type()}
                raise exception.StackValidationFailed(message='%s' % error_message)
            raise