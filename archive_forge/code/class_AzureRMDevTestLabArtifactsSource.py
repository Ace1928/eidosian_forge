...
class AzureRMDevTestLabArtifactsSource(AzureRMModuleBase):
    ...
    def __init__(self):
        ...
        required_if = [('state', 'present', ['source_type', 'uri', 'security_token'])]
        super(AzureRMDevTestLabArtifactsSource, self).__init__(derived_arg_spec=self.module_arg_spec, supports_check_mode=True, supports_tags=True, required_if=required_if)
    ...
    def exec_module(self, **kwargs):
        ...
        if self.state == 'present':
            self.results.update({'id': response.get('id', None), 'is_enabled': response.get('status', None).lower() == 'enabled'})
        return self.results
