...
def GetCredentialsFromFlags() -> GoogleAuthCredentialsUnionType:
  ...
  """
    if bq_auth_flags.APPLICATION_DEFAULT_CREDENTIAL_FILE.value:
        raise app.UsageError('The --application_default_credential_file flag is being deprecated. For now, this flag can still be used by forcing the legacy authentication library with --nouse_google_auth.')
    if bq_auth_flags.SERVICE_ACCOUNT_PRIVATE_KEY_PASSWORD.default != bq_auth_flags.SERVICE_ACCOUNT_PRIVATE_KEY_PASSWORD.value:
    ...
    if bq_auth_flags.OAUTH_ACCESS_TOKEN.value:
        ...
        return google_oauth2.Credentials(token=bq_auth_flags.OAUTH_ACCESS_TOKEN.value, quota_project_id=bq_utils.GetResolvedQuotaProjectID(bq_auth_flags.QUOTA_PROJECT_ID.value, bq_flags.PROJECT_ID.value))
    else:
    ...
    if bq_auth_flags.USE_GCE_SERVICE_ACCOUNT.value:
        ...
        return compute_engine.Credentials(quota_project_id=bq_utils.GetResolvedQuotaProjectID(bq_auth_flags.QUOTA_PROJECT_ID.value, fallback_project_id=None))
    else:
