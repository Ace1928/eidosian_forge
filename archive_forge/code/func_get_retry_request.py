...
def get_retry_request(request: Request, *, spider: Spider, reason: Union[str, Exception, Type[Exception]]='unspecified', max_retry_times: Optional[int]=None, priority_adjust: Optional[int]=None, logger: Logger=retry_logger, stats_base_key: str='retry'):
    ...
    *stats_base_key* is a string to be used as the base key for the
    ...
    if retry_times <= max_retry_times:
        logger.debug('Retrying %(request)s (failed %(retry_times)d times): %(reason)s', {'request': request, 'retry_times': retry_times, 'reason': reason}, extra={'spider': spider})
    ...
    logger.error('Gave up retrying %(request)s (failed %(retry_times)d times): %(reason)s', {'request': request, 'retry_times': retry_times, 'reason': reason}, extra={'spider': spider})
    return None