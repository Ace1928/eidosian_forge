...
class LibvirtNodeDriver(NodeDriver):
    ...
    def __init__(self, uri, key=None, secret=None):
        ...
        if uri is not None and '+tcp' in self._uri:
            if key is None and secret is None:
                raise RuntimeError('The remote Libvirt instance requires ' + "authentication, please set 'key' and " + "'secret' parameters")
            auth = [[libvirt.VIR_CRED_AUTHNAME, libvirt.VIR_CRED_PASSPHRASE], self._cred_callback, None]
    ...
    def _to_node(self, domain):
        ...
        node = Node(id=domain.ID(), name=domain.name(), state=state, public_ips=public_ips, private_ips=private_ips, driver=self, extra=extra)
        node._uuid = domain.UUIDString()
        return node
