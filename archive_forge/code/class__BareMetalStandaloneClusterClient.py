...
class _BareMetalStandaloneClusterClient(client.ClientBase):
    ...
    def _sr_iov_config(self, args: parser_extensions.Namespace):
        ...
        kwargs = {'enabled': self._sr_iov_config_enabled(args)}
    ...
    def _island_mode_cidr_config(self, args: parser_extensions.Namespace):
        ...
        kwargs = {'serviceAddressCidrBlocks': getattr(args, 'island_mode_service_address_cidr_blocks', [])}
    ...
    def _network_config(self, args: parser_extensions.Namespace):
        ...
        kwargs = {'islandModeCidr': self._island_mode_cidr_config(args), 'srIovConfig': self._sr_iov_config(args)}
        if any(kwargs.values()):
    ...
    def _address_pools_from_file(self, args: parser_extensions.Namespace):
        ...
        if not args.metal_lb_address_pools_from_file:
            return []
        address_pools = args.metal_lb_address_pools_from_file.get('addressPools', [])
        if not address_pools:
            self._raise_bad_argument_exception_error('--metal_lb_address_pools_from_file', 'addressPools', 'Metal LB address pools file')
        address_pool_messages = [self._metal_lb_address_pool(address_pool) for address_pool in address_pools]
    ...
    def _metal_lb_node_config(self, metal_lb_node_config):
        ...
        if not node_ip:
            self._raise_bad_argument_exception_error('--metal_lb_load_balancer_node_configs_from_file', 'nodeIp', 'Metal LB Node configs file')
    ...
    def _metal_lb_node_configs_from_file(self, args: parser_extensions.Namespace):
        ...
        if not metal_lb_node_configs:
            self._raise_bad_argument_exception_error('--metal_lb_load_balancer_node_configs_from_file', 'nodeConfigs', 'Metal LB Node configs file')
    ...
    def node_config(self, node_config_args):
        """Constructs proto message BareMetalNodeConfig."""
        kwargs = {'nodeIp': node_config_args.get('node-ip', ''), 'labels': self.parse_node_labels(node_config_args)}
    ...
    def _metal_lb_load_balancer_node_pool_config(self, args: parser_extensions.Namespace):
        ...
        kwargs = {'nodeConfigs': metal_lb_node_configs, 'labels': self._metal_lb_labels(args), 'taints': self._metal_lb_node_taints(args)}
        if any(kwargs.values()):
    ...
    def _metal_lb_config(self, args: parser_extensions.Namespace):
        ...
        kwargs = {'addressPools': address_pools, 'loadBalancerNodePoolConfig': self._metal_lb_node_pool_config(args)}
        if any(kwargs.values()):
    ...
    def _bgp_lb_address_pool(self, address_pool):
        ...
        addresses = address_pool.get('addresses', [])
        if not addresses:
            self._raise_bad_argument_exception_error('--bgp_lb_address_pools_from_file', 'addresses', 'BGP LB address pools file')
        pool = address_pool.get('pool', None)
        if not pool:
            self._raise_bad_argument_exception_error('--bgp_lb_address_pools_from_file', 'pool', 'BGP LB address pools file')
        kwargs = {'addresses': addresses, 'avoidBuggyIps': address_pool.get('avoidBuggyIPs', None), 'manualAssign': address_pool.get('manualAssign', None), 'pool': pool}
        return messages.BareMetalStandaloneLoadBalancerAddressPool(**kwargs)
    ...
    def _bgp_lb_node_configs_from_flag(self, args: parser_extensions.Namespace):
        """Constructs proto message field node_configs."""
        node_config_flag_value = getattr(args, 'bgp_lb_load_balancer_node_configs', []) if args.bgp_lb_load_balancer_node_configs else []
        return [self.node_config(node_config) for node_config in node_config_flag_value]
    ...
    def _bgp_lb_node_pool_config(self, args: parser_extensions.Namespace):
        """Constructs proto message BareMetalStandaloneLoadBalancerNodePoolConfig."""
        kwargs = {'nodePoolConfig': self._bgp_lb_load_balancer_node_pool_config(args)}
        if any(kwargs.values()):
            return messages.BareMetalStandaloneLoadBalancerNodePoolConfig(**kwargs)
        return None
    ...
    def _peer_configs(self, peer_config):
        ...
        if not asn:
            self._raise_bad_argument_exception_error('--bgp_lb_peer_configs_from_file', 'asn', 'BGP LB peer configs file')
        ip_address = peer_config.get('ipAddress', None)
        if not ip_address:
            self._raise_bad_argument_exception_error('--bgp_lb_peer_configs_from_file', 'ipAddress', 'BGP LB peer configs file')
        kwargs = {'asn': asn, 'ipAddress': ip_address, 'controlPlaneNodes': peer_config.get('controlPlaneNodes', [])}
    ...
    def _bgp_lb_config(self, args: parser_extensions.Namespace):
        ...
        kwargs = {'addressPools': address_pools, 'asn': getattr(args, 'bgp_lb_asn', None), 'loadBalancerNodePoolConfig': self._bgp_lb_node_pool_config(args), 'bgpPeerConfigs': peer_configs}
        if any(kwargs.values()):
    ...
    def _load_balancer_config(self, args: parser_extensions.Namespace):
        ...
        kwargs = {'metalLbConfig': self._metal_lb_config(args), 'bgpLbConfig': self._bgp_lb_config(args)}
        if any(kwargs.values()):
    ...
    def _control_plane_node_config(self, control_plane_node_config):
        ...
        if not node_ip:
            self._raise_bad_argument_exception_error('--control_plane_node_configs_from_file', 'nodeIP', 'Control Plane Node configs file')
    ...
    def _control_plane_node_configs_from_file(self, args: parser_extensions.Namespace):
        ...
        if not control_plane_node_configs:
            self._raise_bad_argument_exception_error('--control_plane_node_configs_from_file', 'nodeConfigs', 'Control Plane Node configs file')
    ...
    def _raise_bad_argument_exception_error(self, flag, field, file):
        raise exceptions.BadArgumentException(flag, 'Missing field [' + field + '] in ' + file)
    ...
    def _node_pool_config(self, args: parser_extensions.Namespace):
        ...
        kwargs = {'nodeConfigs': node_configs, 'labels': self._control_plane_node_labels(args), 'taints': self._control_plane_node_taints(args)}
    ...
    def _control_plane_node_pool_config(self, args: parser_extensions.Namespace):
        ...
        kwargs = {'nodePoolConfig': self._node_pool_config(args)}
        return self._set_config_if_exists(messages.BareMetalStandaloneControlPlaneNodePoolConfig, kwargs)
    ...
    def _api_server_args(self, args: parser_extensions.Namespace):
        ...
        if api_server_args_flag_value:
            for key, val in api_server_args_flag_value.items():
                api_server_args.append(messages.BareMetalStandaloneApiServerArgument(argument=key, value=val))
    ...
    def _control_plane_config(self, args: parser_extensions.Namespace):
        ...
        kwargs = {'controlPlaneNodePoolConfig': self._control_plane_node_pool_config(args), 'apiServerArgs': self._api_server_args(args)}
        return self._set_config_if_exists(messages.BareMetalStandaloneControlPlaneConfig, kwargs)
    ...
    def _cluster_operations_config(self, args: parser_extensions.Namespace):
        ...
        kwargs = {'enableApplicationLogs': getattr(args, 'enable_application_logs', None)}
        return self._set_config_if_exists(messages.BareMetalStandaloneClusterOperationsConfig, kwargs)
    ...
    def _maintenance_config(self, args: parser_extensions.Namespace):
        ...
        return self._set_config_if_exists(messages.BareMetalStandaloneMaintenanceConfig, kwargs)
    ...
    def _authorization(self, args: parser_extensions.Namespace):
        ...
        kwargs = {'adminUsers': self._cluster_users(args)}
    ...
    def _security_config(self, args: parser_extensions.Namespace):
        ...
        kwargs = {'authorization': self._authorization(args)}
        return self._set_config_if_exists(messages.BareMetalStandaloneSecurityConfig, kwargs)
    ...
    def _node_access_config(self, args: parser_extensions.Namespace):
        ...
        kwargs = {'loginUser': getattr(args, 'login_user', 'root')}
        return self._set_config_if_exists(messages.BareMetalStandaloneNodeAccessConfig, kwargs)
    ...
    def _remove_annotations(self, args: parser_extensions.Namespace):
        ...
        curr_annotations = self._get_current_annotations(args)
        updated_annotations = {key: value for key, value in curr_annotations.items() if key not in args.remove_annotations}
    ...
    def _dict_to_annotations_message(self, annotations):
        ...
        for key, value in annotations.items():
            additional_property_messages.append(messages.BareMetalStandaloneCluster.AnnotationsValue.AdditionalProperty(key=key, value=value))
        annotation_value_message = messages.BareMetalStandaloneCluster.AnnotationsValue(additionalProperties=additional_property_messages)
