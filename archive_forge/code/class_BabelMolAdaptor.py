...
class BabelMolAdaptor:
    ...
    def confab_conformers(self, forcefield: str='mmff94', freeze_atoms: list[int] | None=None, rmsd_cutoff: float=0.5, energy_cutoff: float=50.0, conf_cutoff: int=100000, verbose: bool=False) -> list[Molecule]:
        """
        ...
        ff = openbabel.OBForceField.FindType(forcefield)
        if ff == 0:
            print(f"Could not find forcefield={forcefield!r} in openbabel, the forcefield will be reset as default 'mmff94'")
            ff = openbabel.OBForceField.FindType('mmff94')
        if freeze_atoms:
    ...
    @classmethod
    def from_file(cls, filename: str, file_format: str='xyz', return_all_molecules: bool=False) -> Self | list[Self]:
        """
