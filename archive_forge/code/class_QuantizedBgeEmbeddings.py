...
class QuantizedBgeEmbeddings(BaseModel, Embeddings):
    ...
    def __init__(self, model_name: str, *, max_seq_len: int=512, pooling_strategy: str='mean', query_instruction: Optional[str]=None, document_instruction: Optional[str]=None, padding: bool=True, model_kwargs: Optional[Dict]=None, encode_kwargs: Optional[Dict]=None, onnx_file_name: Optional[str]='int8-model.onnx', **kwargs: Any) -> None:
        super().__init__(**kwargs)
        if importlib.util.find_spec('intel_extension_for_transformers') is None:
            raise ImportError('Could not import intel_extension_for_transformers python package. Please install it with `pip install -U intel-extension-for-transformers`.')
    ...
    def load_model(self) -> None:
        ...
        self.transformer_tokenizer = AutoTokenizer.from_pretrained(self.model_name_or_path)
    ...
    def _embed(self, inputs: Any) -> Any:
        ...
        last_hidden_state = torch.tensor(last_hidden_state).reshape(inputs['input_ids'].shape[0], inputs['input_ids'].shape[1], self.hidden_size)
