...
class SetupSurfaceSlab:
    ...
    def __init__(self, gui):
        ...
        win.add([pybutton(_('Creating a surface.'), self.make), ui.Button(_('Apply'), self.apply), ui.Button(_('OK'), self.ok)])
    ...
    def make(self, *args):
        ...
        if struct == _('FCC(211)') and (not x % 3 == 0 and ortho):
            self.x_warn.text = _('Please enter a value divisible by 3 for orthogonal cell')
            return
        ...
        for surface in surfaces:
            if surface[0] == struct:
                ...
                label = ngettext('{symbol} {surf} surface with one atom.{vacuum}', '{symbol} {surf} surface with {natoms} atoms.{vacuum}', natoms).format(symbol=symbol, surf=surface[3].__name__, natoms=natoms, vacuum=vacuumtext)
                self.description.text = label
                return py_template.format(func=surface[3].__name__, a=a, c=c_py, symbol=symbol, size=size, ortho=ortho, vacuum=vacuum)
    ...
    def apply(self, *args):
        ...
        if self.atoms is not None:
            ...
            ui.error(_('No valid atoms.'), _('You have not (yet) specified a consistent set of parameters.'))
            return False
