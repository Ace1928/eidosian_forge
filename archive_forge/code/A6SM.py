...
# Constants
VALIDATION_RULES_FILE = pathlib.Path(
...
async def import_module_from_path(name: str, path: str) -> Module:
    """
    Asynchronously imports a module from a given file path and returns it, utilizing the 'import_from_path'
    function from a utility module for enhanced flexibility and error handling.

    Args:
        name (str): The name to assign to the module.
        path (str): The file path of the module to import.
...
async def load_modules_from_config(config_path: pathlib.Path) -> ModulesDict:
    """
    Asynchronously loads modules based on a configuration file specifying module names and paths, using the
    'import_module_from_path' function for each module import. This approach allows for dynamic loading of modules
    at runtime, enhancing the flexibility and extensibility of the application.
    ...
    modules = {}
    load_tasks = [import_module_from_path(name, path) for name, path in config.items()]
    loaded_modules = await asyncio.gather(*load_tasks, return_exceptions=True)

    for module, (name, _) in zip(loaded_modules, config.items()):
...
globals().update(validation_modules)
...
def load_validation_rules_from_modules(module_paths: List[str]) -> ValidationRules:
    ...
    for path in module_paths:
        ...
        try:
            ...
            logging.error(
                f"Module {module_name} not found in globals: {e}", exc_info=True
            )
