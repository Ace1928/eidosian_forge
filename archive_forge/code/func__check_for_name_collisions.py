...
def _check_for_name_collisions(children: Iterable[str], variables: Iterable[Hashable]) -> None:
    colliding_names = set(children).intersection(set(variables))
    if colliding_names:
        raise KeyError(f'Some names would collide between variables and children: {list(colliding_names)}')