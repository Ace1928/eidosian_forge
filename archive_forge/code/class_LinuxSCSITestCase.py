...
class LinuxSCSITestCase(base.TestCase):
    ...
    def test_get_name_from_path(self, realpath_mock):
        ...
        disk_path = '/dev/disk/by-path/ip-10.10.220.253:3260-iscsi-iqn.2000-05.com.3pardata:21810002ac00383d-lun-0'
        name = self.linuxscsi.get_name_from_path(disk_path)
        self.assertEqual(device_name, name)
        disk_path = '/dev/disk/by-path/pci-0000:00:00.0-ip-10.9.8.7:3260-iscsi-iqn.2000-05.com.openstack:2180002ac00383d-lun-0'
        name = self.linuxscsi.get_name_from_path(disk_path)
        self.assertEqual(device_name, name)
    ...
    def test_remove_scsi_device(self, exists_mock):
        ...
        self.linuxscsi.remove_scsi_device('/dev/sdc')
        expected_commands = ['blockdev --flushbufs /dev/sdc', 'tee -a /sys/block/sdc/device/delete']
        self.assertEqual(expected_commands, self.cmds)
    ...
    @ddt.data({'do_raise': False, 'force': False}, {'do_raise': True, 'force': True})
    @ddt.unpack
    ...
    def test_remove_connection_multipath_complete(self, remove_mock, wait_mock, find_dm_mock, get_dm_name_mock, flush_mp_mock, is_mp_running_mock, mp_del_path_mock, remove_link_mock, do_raise, force):
        if do_raise:
        ...
        self.assertEqual(get_dm_name_mock.return_value if do_raise else None, mp_name)
    ...
    @mock.patch.object(linuxscsi.LinuxSCSI, '_remove_scsi_symlinks')
    @mock.patch.object(linuxscsi.LinuxSCSI, 'multipath_del_path')
    @mock.patch.object(linuxscsi.LinuxSCSI, 'is_multipath_running', return_value=False)
    @mock.patch.object(linuxscsi.LinuxSCSI, 'find_sysfs_multipath_dm')
    @mock.patch.object(linuxscsi.LinuxSCSI, 'wait_for_volumes_removal')
    @mock.patch.object(linuxscsi.LinuxSCSI, 'remove_scsi_device')
    def test_remove_connection_singlepath_used(self, remove_mock, wait_mock, find_dm_mock, is_mp_running_mock, mp_del_path_mock, remove_link_mock):
        find_dm_mock.return_value = None
        devices_names = ('sda', 'sdb')
        exc = exception.ExceptionChainer()
        with mock.patch('os.path.realpath', side_effect=self.realpath):
            self.linuxscsi.remove_connection(devices_names, force=mock.sentinel.Force, exc=exc, path_used='/dev/sdb', was_multipath=False)
        find_dm_mock.assert_called_once_with(devices_names)
        is_mp_running_mock.assert_called_once()
        mp_del_path_mock.assert_not_called()
        remove_mock.assert_has_calls([mock.call('/dev/sda', mock.sentinel.Force, exc, False), mock.call('/dev/sdb', mock.sentinel.Force, exc, True)])
        wait_mock.assert_called_once_with(devices_names)
        remove_link_mock.assert_called_once_with(devices_names)
    ...
    def test_extend_volume_with_mpath(self, mock_realpath, mock_device_info, mock_device_size, mock_scsi_wwn, mock_find_mpath_path, mock_valid_dev):
        ...
        expected_cmds = ['tee -a /sys/bus/scsi/drivers/sd/0:0:0:1/rescan', 'tee -a /sys/bus/scsi/drivers/sd/1:0:0:1/rescan', 'multipathd reconfigure', 'multipathd resize map %s' % dm_path]
        self.assertEqual(expected_cmds, self.cmds)
    ...
    @mock.patch.object(linuxscsi, 'LOG')
    @mock.patch('os_brick.privileged.rootwrap.execute')
    def test_is_multipath_running_failure_not_enforcing_multipath(self, mock_exec, mock_log):
        mock_exec.side_effect = putils.ProcessExecutionError()
        res = linuxscsi.LinuxSCSI.is_multipath_running(False, None, mock_exec)
        mock_exec.assert_called_once_with('multipathd', 'show', 'status', run_as_root=True, root_helper=None)
    ...
    @mock.patch.object(linuxscsi, 'LOG')
    @mock.patch('os_brick.privileged.rootwrap.execute')
    def test_is_multipath_running_failure_not_enforcing_exit_code_0(self, mock_exec, mock_log):
        mock_exec.return_value = ('error receiving packet', '')
        res = linuxscsi.LinuxSCSI.is_multipath_running(False, None, mock_exec)
        mock_exec.assert_called_once_with('multipathd', 'show', 'status', run_as_root=True, root_helper=None)
        self.assertFalse(res)
    ...
    def test_get_device_info(self):
        ...
        with mock.patch.object(self.linuxscsi, '_execute') as exec_mock:
            ...
            self.assertEqual(info, {'channel': '1', 'device': '/dev/adevice', 'host': '1', 'id': '0', 'lun': '0'})
    ...
    @mock.patch('os.path.realpath', side_effect=('/dev/sde', '/dev/dm-5', '/dev/dm-6'))
    ...
    def test_get_sysfs_wwn_dm_link(self, get_wwid_mock, glob_mock, realpath_mock, listdir_mock):
        glob_mock.return_value = ['/dev/disk/by-id/scsi-wwid1', '/dev/disk/by-id/scsi-another-dm', '/dev/disk/by-id/scsi-our-dm']
        devices = ['sdc', 'sdd']
        res = self.linuxscsi.get_sysfs_wwn(devices)
        self.assertEqual('our-dm', res)
        glob_mock.assert_called_once_with('/dev/disk/by-id/scsi-*')
        listdir_mock.assert_has_calls([mock.call('/sys/class/block/dm-5/slaves'), mock.call('/sys/class/block/dm-6/slaves')])
        get_wwid_mock.assert_called_once_with(devices)
    ...
    def test_device_name_by_hctl(self, glob_mock):
        ...
        glob_mock.assert_called_once_with('/sys/class/scsi_host/host3/device/session1/target3:4:5/3:4:5:2/block/*')
    ...
    def test_scsi_iscsi(self, echo_mock):
        ...
        echo_mock.assert_called_once_with('/sys/class/scsi_host/hosthost/scan', 'channel target lun')
    ...
    def test_multipath_del_map_retries_remains(self, name_mock, exec_mock):
        mpath_name = '3600d0230000000000e13955cc3757800'
        name_mock.return_value = mpath_name
        self.assertRaises(exception.BrickException, self.linuxscsi.multipath_del_map, 'dm-7')
        self.assertEqual(3, exec_mock.call_count)
        exec_mock.assert_has_calls([mock.call('multipathd', 'del', 'map', mpath_name, run_as_root=True, timeout=5, root_helper=self.linuxscsi._root_helper)] * 3)
