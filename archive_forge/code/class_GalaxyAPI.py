...
class GalaxyAPI:
    ...
    def __init__(self, galaxy, name, url, username=None, password=None, token=None, validate_certs=True, available_api_versions=None, clear_response_cache=False, no_cache=True, priority=float('inf'), timeout=60):
        self.galaxy = galaxy
        self.name = name
        ...
        makedirs_safe(b_cache_dir, mode=448)
    ...
    def __repr__(self):
        ...
        return '<{instance!s} "{name!s}" @ {url!s} with priority {priority!s}>'.format(instance=self, name=self.name, priority=self._priority, url=self.api_server)
    ...
    def _add_auth_token(self, headers, url, token_type=None, required=False):
        ...
        if not self.token and required:
            raise AnsibleError('No access token or username set. A token can be set with --api-key or at {0}.'.format(to_native(C.GALAXY_TOKEN_PATH)))
        if self.token:
    ...
    def get_collection_version_metadata(self, namespace, name, version):
        ...
        return CollectionVersionMetadata(data['namespace']['name'], data['collection']['name'], data['version'], data['download_url'], data['artifact']['sha256'], data['metadata']['dependencies'], data['href'], signatures)
    ...
    def get_collection_versions(self, namespace, name):
        ...
        while True:
            ...
            data = self._call_galaxy(to_native(next_link, errors='surrogate_or_strict'), error_context_msg=error_context_msg, cache=True, cache_key=cache_key)
        self._set_cache()
        return versions
