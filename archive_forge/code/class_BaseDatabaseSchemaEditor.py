...
class BaseDatabaseSchemaEditor:
    ...
    def _delete_composed_index(self, model, fields, constraint_kwargs, sql):
        ...
        if constraint_kwargs.get('unique') is True and constraint_names and self.connection.features.allows_multiple_constraints_on_same_fields:
    ...
    def add_field(self, model, field):
        ...
        if field.remote_field and self.connection.features.supports_foreign_keys and field.db_constraint:
            ...
            if self.sql_create_column_inline_fk:
                ...
                definition += ' ' + self.sql_create_column_inline_fk % {'name': self._fk_constraint_name(model, field, constraint_suffix), 'namespace': '%s.' % self.quote_name(namespace) if namespace else '', 'column': self.quote_name(field.column), 'to_table': self.quote_name(to_table), 'to_column': self.quote_name(to_column), 'deferrable': self.connection.ops.deferrable_sql()}
            else:
                self.deferred_sql.append(self._create_fk_sql(model, field, constraint_suffix))
        sql = self.sql_create_column % {'table': self.quote_name(model._meta.db_table), 'column': self.quote_name(field.column), 'definition': definition}
        self.execute(sql, params or None)
        if field.db_default is NOT_PROVIDED and (not self.skip_default_on_alter(field)) and (self.effective_default(field) is not None):
            changes_sql, params = self._alter_column_default_sql(model, None, field, drop=True)
            sql = self.sql_alter_column % {'table': self.quote_name(model._meta.db_table), 'changes': changes_sql}
            self.execute(sql, params)
        if field.db_comment and self.connection.features.supports_comments and (not self.connection.features.supports_comments_inline):
    ...
    def _alter_field(self, model, old_field, new_field, old_type, new_type, old_db_params, new_db_params, strict=False):
        ...
        fks_dropped = set()
        if self.connection.features.supports_foreign_keys and old_field.remote_field and old_field.db_constraint and self._field_should_be_altered(old_field, new_field, ignore={'db_comment'}):
        ...
        drop_foreign_keys = self.connection.features.supports_foreign_keys and (old_field.primary_key and new_field.primary_key or (old_field.unique and new_field.unique)) and (old_type != new_type or old_collation != new_collation)
        ...
        if old_type != new_type or old_type_suffix != new_type_suffix or old_collation != new_collation or (self.connection.features.supports_comments and old_field.db_comment != new_field.db_comment):
        ...
        if new_field.db_default is not NOT_PROVIDED:
            if old_field.db_default is NOT_PROVIDED or new_field.db_default != old_field.db_default:
        ...
        if old_field.null and (not new_field.null) and (new_field.db_default is NOT_PROVIDED):
        ...
        four_way_default_alteration = (new_field.has_default() or new_field.db_default is not NOT_PROVIDED) and (old_field.null and (not new_field.null))
        if actions or null_actions:
            ...
            if four_way_default_alteration:
                ...
                self.execute(self.sql_update_with_default % {'table': self.quote_name(model._meta.db_table), 'column': self.quote_name(new_field.column), 'default': default_sql}, params)
        ...
        for old_rel, new_rel in rels_to_update:
            ...
            fragment, other_actions = self._alter_column_type_sql(new_rel.related_model, old_rel.field, new_rel.field, rel_type, old_rel_collation, rel_collation)
    ...
    def _alter_column_comment_sql(self, model, new_field, new_type, new_db_comment):
        return (self.sql_alter_column_comment % {'table': self.quote_name(model._meta.db_table), 'column': self.quote_name(new_field.column), 'comment': self._comment_sql(new_db_comment)}, [])
    ...
    def _delete_index_sql(self, model, name, sql=None):
        return Statement(sql or self.sql_delete_index, table=Table(model._meta.db_table, self.quote_name), name=self.quote_name(name))
