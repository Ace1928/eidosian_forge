...
class UIParser(object):
    ...
    def __init__(self, qtcore_module, qtgui_module, qtwidgets_module, creatorPolicy):
        ...
        self.wprops = Properties(self.factory, qtcore_module, qtgui_module, qtwidgets_module)
    ...
    def addHeader(self, elem):
        ...
        if isinstance(w, QtWidgets.QTreeWidget):
            ...
            if len(elem) != 0:
                if elem.tag == 'column':
                    item = self.createWidgetItem('QTableWidgetItem', elem, w.horizontalHeaderItem, self.column_counter)
                    w.setHorizontalHeaderItem(self.column_counter, item)
                    self.column_counter += 1
                elif elem.tag == 'row':
                    item = self.createWidgetItem('QTableWidgetItem', elem, w.verticalHeaderItem, self.row_counter)
    ...
    def createActionGroup(self, elem):
        ...
        self.traverseWidgetTree(elem)
        self.currentActionGroup = None
    widgetTreeItemHandlers = {'widget': createWidget, 'addaction': addAction, 'layout': createLayout, 'spacer': createSpacer, 'item': handleItem, 'action': createAction, 'actiongroup': createActionGroup, 'column': addHeader, 'row': addHeader, 'zorder': setZOrder}
    ...
    def parse(self, filename, resource_suffix):
        ...
        branchHandlers = (('layoutdefault', self.readDefaults), ('class', self.classname), ('buttongroups', self.buttonGroups), ('customwidgets', self.customWidgets), ('widget', self.createUserInterface), ('connections', self.createConnections), ('tabstops', self.setTaborder), ('resources', self.readResources))
