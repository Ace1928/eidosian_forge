...
def _get_unnested_name_scope(self):
    if _is_name_scope_on_model_declaration_enabled:
        with _name_scope_unnester(self._name_scope_on_declaration) as relative_name_scope_on_declaration:
            relative_name_scope = filter(None, [tf.get_current_name_scope(), relative_name_scope_on_declaration])
