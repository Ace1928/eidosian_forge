...
def cossin(X, p=None, q=None, separate=False, swap_sign=False, compute_u=True, compute_vh=True):
    ...
    if p or q:
        ...
        if not np.equal(*X.shape):
            raise ValueError(f'Cosine Sine decomposition only supports square matrices, got {X.shape}')
        m = X.shape[0]
        if p >= m or p <= 0:
        ...
        if len(X) != 4:
            raise ValueError(f'When p and q are None, exactly four arrays should be in X, got {len(X)}')
