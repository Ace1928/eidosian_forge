...
def test_blending(self):
    ...
    model.c1 = Constraint(expr=0.1 * model.x1 + 0.2 * model.x2 >= 8.0, doc='Protein Requirement')
    model.c2 = Constraint(expr=0.08 * model.x1 + 0.1 * model.x2 >= 6.0, doc='Fat Requirement')
    ...
    with open(join(currdir, 'blend.out'), 'r') as out, open(join(currdir, 'blend.txt'), 'r') as txt:
        self.assertStructuredAlmostEqual(json.load(txt), json.load(out), abstol=0.01, allow_second_superset=True)