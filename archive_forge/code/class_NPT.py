...
class NPT(MolecularDynamics):
    ...
    def __init__(self, atoms, timestep, temperature=None, externalstress=None, ttime=None, pfactor=None, *, temperature_K=None, mask=None, trajectory=None, logfile=None, loginterval=1, append_trajectory=False):
        ...
        * The ttime and pfactor are quite critical[4], too small values may
        ...
        """
        MolecularDynamics.__init__(self, atoms, timestep, trajectory, logfile, loginterval, append_trajectory=append_trajectory)
    ...
    def set_mask(self, mask):
        ...
        if mask.shape != (3,) and mask.shape != (3, 3):
            raise RuntimeError('The mask has the wrong shape ' + '(must be a 3-vector or 3x3 matrix)')
        else:
    ...
    def run(self, steps):
        ...
        if not self.initialized:
            self.initialize()
        elif self.have_the_atoms_been_changed():
    ...
    def step(self):
        ...
        if self.pfactor_given is None:
            ...
            deltaeta = -2 * dt * (self.pfact * linalg.det(self.h) * (stress - self.externalstress))
        ...
        deltazeta = 2 * dt * self.tfact * (self.atoms.get_kinetic_energy() - self.desiredEkin)
        ...
        self.atoms.set_momenta(np.dot(self.q_future - self.q_past, self.h / (2 * dt)) * self._getmasses())
    ...
    def get_data(self):
        ...
        return {'eta': self.eta, 'eta_past': self.eta_past, 'zeta': self.zeta, 'zeta_past': self.zeta_past, 'zeta_integrated': self.zeta_integrated, 'h': self.h, 'h_past': self.h_past, 'timeelapsed': self.timeelapsed}
