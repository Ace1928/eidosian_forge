...
class DeleteCachedImage(command.Command):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument('images', metavar='<image>', nargs='+', help=_('Image(s) to delete (name or ID)'))
        return parser
    ...
    def take_action(self, parsed_args):
        ...
        for image in parsed_args.images:
            try:
                ...
                failures += 1
                msg = _("Failed to delete image with name or ID '%(image)s': %(e)s")
                LOG.error(msg, {'image': image, 'e': e})
        if failures > 0:
            total = len(parsed_args.images)
            msg = _('Failed to delete %(failures)s of %(total)s images.') % {'failures': failures, 'total': total}
