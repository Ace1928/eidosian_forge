...
class Multinomial(distribution.Distribution):
  ...
  * `N!` denotes `N` factorial.
  ...
  """
    ...
    def __init__(self, total_count, logits=None, probs=None, validate_args=False, allow_nan_stats=True, name='Multinomial'):
    ...
    """
        ...
        with ops.name_scope(name, values=[total_count, logits, probs]) as name:
            ...
            self._logits, self._probs = distribution_util.get_logits_and_probs(logits=logits, probs=probs, multidimensional=True, validate_args=validate_args, name=name)
            self._mean_val = self._total_count[..., array_ops.newaxis] * self._probs
        super(Multinomial, self).__init__(dtype=self._probs.dtype, reparameterization_type=distribution.NOT_REPARAMETERIZED, validate_args=validate_args, allow_nan_stats=allow_nan_stats, parameters=parameters, graph_parents=[self._total_count, self._logits, self._probs], name=name)
    ...
    def _sample_n(self, n, seed=None):
        ...
        n_draws = array_ops.ones_like(self.logits[..., 0], dtype=n_draws.dtype) * n_draws
        logits = array_ops.ones_like(n_draws[..., array_ops.newaxis], dtype=self.logits.dtype) * self.logits
        flat_logits = array_ops.reshape(logits, [-1, k])
        flat_ndraws = n * array_ops.reshape(n_draws, [-1])
        ...
        def _sample_single(args):
            ...
            return x
        x = map_fn.map_fn(_sample_single, [flat_logits, flat_ndraws], dtype=self.dtype)
    ...
    def _covariance(self):
        ...
        return array_ops.matrix_set_diag(-math_ops.matmul(self._mean_val[..., array_ops.newaxis], p[..., array_ops.newaxis, :]), self._variance())
    ...
    def _maybe_assert_valid_sample(self, counts):
        ...
        return control_flow_ops.with_dependencies([check_ops.assert_equal(self.total_count, math_ops.reduce_sum(counts, -1), message='counts must sum to `self.total_count`')], counts)