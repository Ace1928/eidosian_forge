...
class IOProc(SSHSession):
    ...
    def connect(self):
        stdoutdata = check_output(NETCONF_SHELL, shell=True, stdin=PIPE, stderr=STDOUT).decode(encoding='utf8')
        if 'error: Restricted user session' in stdoutdata:
            obj = re.search('<error-message>\\n?(.*)\\n?</error-message>', stdoutdata, re.M)
            if obj:
        ...
        self._channel = Popen([NETCONF_SHELL], stdin=PIPE, stdout=PIPE, stderr=STDOUT)
