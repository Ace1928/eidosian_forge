...
class UserAgentConfig(object):
    ...
    def __init__(self, platform, app_version, mcc, mnc, os_version, manufacturer, device, os_build_number, phone_id, locale_lang, locale_country):
        """
        :param platform:
        :type platform: int
        :param app_version:
    ...
    def __str__(self):
        return self.STR_TEMPLATE.format(platform=self.platform, app_version=self.app_version, mcc=self.mcc, mnc=self.mnc, os_version=self.os_version, manufacturer=self.manufacturer, device=self.device, os_build_number=self.os_build_number, phone_id=self.phone_id, locale_lang=self.locale_lang, locale_country=self.locale_country)
