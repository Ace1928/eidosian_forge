...
class DockerCluster:
    ...
    def __init__(self, config: Optional[Dict[str, Any]]=None):
        self._base_config_file = os.path.join(os.path.dirname(__file__), 'example_docker.yaml')
        self._tempdir = None
        ...
        self._monitor_script = os.path.join(os.path.dirname(__file__), 'docker_monitor.py')
    ...
    def client_port(self):
        return self._cluster_config.get('provider', {}).get('host_client_port', FAKE_DOCKER_DEFAULT_CLIENT_PORT)
    ...
    def _start_monitor(self):
        self._monitor_process = subprocess.Popen(['python', self._monitor_script, self.config_file])
        time.sleep(2)
    ...
    def start(self):
        ...
        """
        self._start_monitor()
        subprocess.check_output(f'RAY_FAKE_CLUSTER=1 ray up -y {self.config_file}', shell=True)
    ...
    def stop(self):
        ...
        if ray.is_initialized:
            ray.shutdown()
        subprocess.check_output(f'RAY_FAKE_CLUSTER=1 ray down -y {self.config_file}', shell=True)
        self._stop_monitor()
    ...
    def _get_node(self, node_id: Optional[str]=None, num: Optional[int]=None, rand: Optional[str]=None) -> str:
        self._update_nodes()
        if node_id:
            assert not num and (not rand), 'Only provide either `node_id`, `num`, or `random`.'
        elif num:
            assert not node_id and (not rand), 'Only provide either `node_id`, `num`, or `random`.'
            base = 'fffffffffffffffffffffffffffffffffffffffffffffffffff'
            node_id = base + str(num).zfill(5)
        elif rand:
            assert not node_id and (not num), 'Only provide either `node_id`, `num`, or `random`.'
            assert rand in ['worker', 'any'], "`random` must be one of ['worker', 'any']"
            choices = list(self._nodes.keys())
            if rand == 'worker':
                choices.remove('fffffffffffffffffffffffffffffffffffffffffffffffffff00000')
