...
class TestCurveTwinAxes(LoggingComparisonTestCase, TestBokehPlot):
    ...
    def test_multi_y_lims_right_axis(self):
        overlay = (Curve(range(10), vdims=['A']) * Curve(range(10), vdims=['B']).opts(ylim=(-10, 20))).opts(multi_y=True)
        plot = bokeh_renderer.get_plot(overlay)
        y_range = plot.handles['y_range']
        self.assertEqual(y_range.start, 0)
        self.assertEqual(y_range.end, 9)
    ...
    def test_multi_invert_left_axis(self):
        overlay = (Curve(range(10), vdims=['A']).opts(invert_yaxis=True) * Curve(range(10), vdims=['B'])).opts(multi_y=True)
        plot = bokeh_renderer.get_plot(overlay)
        y_range = plot.handles['y_range']
    ...
    def test_multi_invert_right_axis(self):
        overlay = (Curve(range(10), vdims=['A']) * Curve(range(10), vdims=['B']).opts(invert_yaxis=True)).opts(multi_y=True)
        plot = bokeh_renderer.get_plot(overlay)
        y_range = plot.handles['y_range']
        self.assertEqual(y_range.start, 0)
        self.assertEqual(y_range.end, 9)
    ...
    def test_only_x_axis_labels(self):
        overlay = (Curve(range(10), vdims=['A']) * Curve(range(10), vdims=['B'])).opts(multi_y=True, labelled=['x'])
        plot = bokeh_renderer.get_plot(overlay)
        assert plot.state.xaxis[0].axis_label == 'x'
    ...
    def test_none_x_axis_labels(self):
        overlay = (Curve(range(10), vdims=['A']) * Curve(range(10), vdims=['B'])).opts(multi_y=True, labelled=['y'])
        plot = bokeh_renderer.get_plot(overlay)
