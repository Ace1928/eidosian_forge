...
def write_conn(self):
    if self._write_conn is None:
        if self.mode.appending:
            self._write_conn = self.ftp.transfercmd(str('APPE ') + _encode(self.path, self.ftp.encoding))
        else:
            self._write_conn = self.ftp.transfercmd(str('STOR ') + _encode(self.path, self.ftp.encoding), self.pos)
