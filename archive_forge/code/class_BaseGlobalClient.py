...
class BaseGlobalClient(abc.ABC):
    ...
    serialization_compression_level: Optional[int] = 19
    data_expiration: Optional[int] = None
    data_shared_global: Optional[bool] = False
    cache_expiration: Optional[int] = 60 * 60 * 24 * 3
    ...
    def data(self) -> 'PersistentDict':
        ...
        if self._data is None:
            serializer_kwargs = {'compression': self.serialization_compression, 'compression_level': self.serialization_compression_level, 'raise_errors': True, **self.pdict_get_extra_serialization_kwargs('data')}
            base_key = f'{self.settings.ctx.module_name}.{self.name}.data' if self.data_shared_global else f'{self.settings.ctx.module_name}.{self.settings.app_env.name}.{self.name}.data'
            self._data = self.settings.ctx.get_persistent_dict(base_key=base_key, expiration=self.data_expiration, serializer=self.serialization, serializer_kwargs=serializer_kwargs)
        return self._data
    ...
    def cache(self) -> 'PersistentDict':
        ...
        if self._cache is None:
            serializer_kwargs = {'compression': self.serialization_compression, 'compression_level': self.serialization_compression_level, 'raise_errors': True, **self.pdict_get_extra_serialization_kwargs('cache')}
            base_key = f'{self.settings.ctx.module_name}.{self.name}.cache' if self.cache_shared_global else f'{self.settings.ctx.module_name}.{self.settings.app_env.name}.{self.name}.cache'
            self._cache = self.settings.ctx.get_persistent_dict(base_key=base_key, expiration=self.cache_expiration, serializer=self.serialization, serializer_kwargs=serializer_kwargs)
        return self._cache
    ...
    def catch(self, exc: Type['Exception'], msg: Optional[str]=None, fatal: Optional[bool]=False):
        ...
        try:
            ...
            if fatal:
                raise exc(msg, error=e) from e

    def run(self, data: 'BaseSchema', cachable: Optional[bool]=True, **kwargs) -> 'ResultSchema':
        """
        Runs the client
        """
        raise NotImplementedError('run must be implemented by the Client')

    async def arun(self, data: 'BaseSchema', cachable: Optional[bool]=True, **kwargs) -> 'ResultSchema':
        """
        Runs the client
        """
        raise NotImplementedError('arun must be implemented by the Client')

    def __call__(self, data: 'BaseSchema', cachable: Optional[bool]=True, is_async: Optional[bool]=True, **kwargs) -> 'ResultSchema':
        """
        Runs the client
        """
    ...
    def _get_client(self, name: str, state: Optional[Literal['global', 'local']]=None, kind: Optional[str]='client', include_kind: Optional[bool]=None, **kwargs) -> 'ClientTypes':
        """
        Retrieves a client
        """
