...
class PortPairGroupTest(common.HeatTestCase):
    ...
    def setUp(self):
        ...
        self.patchobject(self.test_client_plugin, 'resolve_ext_resource').return_value = 'port1'

    def test_resource_mapping(self):
        mapping = port_pair_group.resource_mapping()
        self.assertEqual(port_pair_group.PortPairGroup, mapping['OS::Neutron::PortPairGroup'])
    ...
    def test_resource_handle_create(self):
        ...
        self.assertEqual('test_port_pair_group', self.test_resource.properties.get(port_pair_group.PortPairGroup.NAME))
        self.assertEqual('desc', self.test_resource.properties.get(port_pair_group.PortPairGroup.DESCRIPTION))
        self.assertEqual(['port1'], self.test_resource.properties.get(port_pair_group.PortPairGroup.PORT_PAIRS))
        self.test_resource.data_set = mock.Mock()
        self.test_resource.handle_create()
        mock_ppg_create.assert_called_once_with('port_pair_group', {'name': 'test_port_pair_group', 'description': 'desc', 'port_pairs': ['port1']})
    ...
    def test_resource_handle_delete(self):
        ...
        mock_ppg_delete.assert_called_once_with('port_pair_group', self.test_resource.resource_id)
    ...
    def test_resource_handle_update(self):
        ...
        self.test_resource.resource_id = '477e8273-60a7-4c41-b683-fdb0bc7cd151'
        prop_diff = {'name': 'name-updated', 'description': 'description-updated'}
        self.test_resource.handle_update(json_snippet=None, tmpl_diff=None, prop_diff=prop_diff)
        mock_ppg_patch.assert_called_once_with('port_pair_group', {'name': 'name-updated', 'description': 'description-updated'}, self.test_resource.resource_id)
