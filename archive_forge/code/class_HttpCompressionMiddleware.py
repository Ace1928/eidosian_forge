...
class HttpCompressionMiddleware:
    ...
    def from_crawler(cls, crawler):
        ...
        try:
            ...
            warnings.warn("HttpCompressionMiddleware subclasses must either modify their '__init__' method to support a 'crawler' parameter or reimplement their 'from_crawler' method.", ScrapyDeprecationWarning)
            mw = cls()
    ...
    def process_response(self, request, response, spider):
        ...
        if isinstance(response, Response):
            ...
            if content_encoding:
                ...
                try:
                    decoded_body = self._decode(response.body, encoding.lower(), max_size)
                except _DecompressionMaxSizeExceeded:
                    raise IgnoreRequest(f'Ignored response {response} because its body ({len(response.body)} B) exceeded DOWNLOAD_MAXSIZE ({max_size} B) during decompression.')
                if len(response.body) < warn_size <= len(decoded_body):
                    logger.warning(f'{response} body size after decompression ({len(decoded_body)} B) is larger than the download warning size ({warn_size} B).')
                if self.stats:
