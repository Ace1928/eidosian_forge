...
class Register(base.CreateCommand):
    ...
    def Run(self, args):
        ...
        with endpoint_util.GkemulticloudEndpointOverride(location):
            ...
            with kube_util.KubernetesClient(kubeconfig=attached_flags.GetKubeconfig(args), context=attached_flags.GetContext(args), enable_workload_identity=True) as kube_client:
                ...
                try:
                    if not flags.GetValidateOnly(args):
                        ...
                        retryer = retry.Retryer(max_retrials=constants.ATTACHED_INSTALL_AGENT_VERIFY_RETRIES)
                        retryer.RetryOnException(cluster_util.verify_install_agent_deployed, args=(kube_client,), sleep_ms=constants.ATTACHED_INSTALL_AGENT_VERIFY_WAIT_MS)
