...
class ImageApiMixin:
    ...
    def images(self, name=None, quiet=False, all=False, filters=None):
        ...
        Raises:
            :py:class:`docker.errors.APIError`
                If the server returns an error.
        """
        params = {'only_ids': 1 if quiet else 0, 'all': 1 if all else 0}
        if name:
    ...
    def import_image(self, src=None, repository=None, tag=None, image=None, changes=None, stream_src=False):
        ...
        params = _import_image_params(repository, tag, image, src=src if isinstance(src, str) else None, changes=changes)
