...
class TensorflowPrinter(ArrayPrinter, AbstractPythonCodePrinter):
    ...
    def _print_Derivative(self, expr):
        ...
        def unfold(expr, args):
            ...
            return '%s(%s, %s)[0]' % (self._module_format('tensorflow.gradients'), unfold(expr, args[:-1]), self._print(args[-1]))
    ...
    def _print_Piecewise(self, expr):
        ...
        if len(expr.args) == 1:
            return '{}({}, {}, {})'.format(self._module_format(tensorflow_piecewise), self._print(cond), self._print(e), 0)
        return '{}({}, {}, {})'.format(self._module_format(tensorflow_piecewise), self._print(cond), self._print(e), self._print(Piecewise(*expr.args[1:])))
    ...
    def _print_MatMul(self, expr):
        ...
        if args:
            return '%s*%s' % (self.parenthesize(Mul.fromiter(args), PRECEDENCE['Mul']), self._expand_fold_binary_op('tensorflow.linalg.matmul', mat_args))
        else:
