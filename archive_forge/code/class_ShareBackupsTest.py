...
class ShareBackupsTest(utils.TestCase):
    ...
    def setUp(self):
        ...
        self.manager = share_backups.ShareBackupManager(fakes.FakeClient(api_version=microversion))
    ...
    def test_get(self):
        with mock.patch.object(self.manager, '_get', mock.Mock()):
            ...
            self.manager._get.assert_called_once_with(share_backups.RESOURCE_PATH % FAKE_BACKUP, share_backups.RESOURCE_NAME)

    def test_restore(self):
        with mock.patch.object(self.manager, '_action', mock.Mock()):
            self.manager.restore(FAKE_BACKUP)
            self.manager._action.assert_called_once_with('restore', FAKE_BACKUP)

    def test_list(self):
        with mock.patch.object(self.manager, '_list', mock.Mock()):
            self.manager.list()
            self.manager._list.assert_called_once_with(share_backups.RESOURCES_PATH + '/detail', share_backups.RESOURCES_NAME)
    ...
    def test_reset_state(self):
        with mock.patch.object(self.manager, '_action', mock.Mock()):
            ...
            self.manager._action.assert_called_once_with('reset_status', FAKE_BACKUP, {'status': 'fake_status'})
    ...
    def test_update(self):
        ...
        with mock.patch.object(self.manager, '_update', mock.Mock()):
            ...
            self.manager._update.assert_called_once_with(share_backups.RESOURCE_PATH % backup.id, {'share_backup': data})