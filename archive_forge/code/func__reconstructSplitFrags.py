...
def _reconstructSplitFrags(f):
    return (_SHYWordHS if isinstance(f, _HSFrag) else _SHYWord) if isinstance(f, _SHYWord) else (_SplitFragLL if isinstance(f, _SplitFragLL) else _HSFrag) if isinstance(f, _HSFrag) else list