...
class AdaptiveOptimizer:
    ...
    **Example**
    ...
    def step_and_cost(self, circuit, operator_pool, drain_pool=False, params_zero=True):
        ...
        if drain_pool:
            operator_pool = [gate for gate in operator_pool if all((gate.name != operation.name or gate.wires != operation.wires for operation in circuit.tape.operations))]
        ...
        for _ in range(self.param_steps):
            params, _ = optimizer.step_and_cost(qnode, params, gates=selected_gates, initial_circuit=circuit.func)
