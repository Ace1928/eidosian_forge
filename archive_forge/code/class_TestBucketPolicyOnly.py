...
class TestBucketPolicyOnly(testcase.GsUtilIntegrationTestCase):
    ...
    def _AssertEnabled(self, bucket_uri, value):
        stdout = self.RunGsUtil(self._get_bpo_cmd + [suri(bucket_uri)], return_stdout=True)
        bucket_policy_only_re = re.compile('^\\s*Enabled:\\s+(?P<enabled_val>.+)$', re.MULTILINE)
    ...
    def testTooFewArgumentsFails(self):
        ...
        stderr = self.RunGsUtil(self._set_bpo_cmd, return_stderr=True, expected_status=1)
        self.assertIn('command requires at least', stderr)
        stderr = self.RunGsUtil(self._get_bpo_cmd, return_stderr=True, expected_status=1)
        self.assertIn('command requires at least', stderr)
        stderr = self.RunGsUtil(['bucketpolicyonly'], return_stderr=True, expected_status=1)
        self.assertIn('command requires at least', stderr)