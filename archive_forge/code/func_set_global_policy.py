...
def set_global_policy(policy):
  ...
  """
    ...
    if not base_layer_utils.v2_dtype_behavior_enabled():
        raise ValueError('The global policy can only be set in TensorFlow 2 or if V2 dtype behavior has been set. To enable V2 dtype behavior, call "tf.compat.v1.keras.layers.enable_v2_dtype_behavior()"')
    if policy is not None and (not isinstance(policy, Policy)):
        policy = Policy(policy)
    is_mixed_policy = policy is not None and policy.compute_dtype != policy.variable_dtype
    if is_mixed_policy:
        _check_if_mixed_precision_graph_rewrite_is_enabled(policy)
    if policy is not None and policy.compute_dtype is not None and (not dtypes.as_dtype(policy.compute_dtype).is_floating):
        raise ValueError('set_global_policy can only be used to set the global policy to floating-point policies, such as "float32" and "mixed_float16", but got policy: %s' % (policy.name,))
