...
class AutoStructify(transforms.Transform):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.config.get('enable_auto_doc_ref', False):
            self.reporter.warning('AutoStructify option "enable_auto_doc_ref" is deprecated')
    default_priority = 1
    suffix_set = set(['md', 'rst'])
    default_config = {'enable_auto_doc_ref': False, 'auto_toc_maxdepth': 1, 'auto_toc_tree_section': None, 'enable_auto_toc_tree': True, 'enable_eval_rst': True, 'enable_math': True, 'enable_inline_math': True, 'commonmark_suffixes': ['.md'], 'url_resolver': lambda x: x, 'known_url_schemes': None}
    ...
    def parse_ref(self, ref):
        ...
        if len(suffix) == 2 and suffix[1] in AutoStructify.suffix_set and (os.path.exists(abspath) and abspath.startswith(self.root_dir)):
            if os.path.sep != '/':
