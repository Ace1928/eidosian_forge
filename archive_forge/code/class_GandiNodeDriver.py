...
class GandiNodeDriver(BaseGandiDriver, NodeDriver):
    ...
    def _to_node(self, vm):
        return Node(id=vm['id'], name=vm['hostname'], state=NODE_STATE_MAP.get(vm['state'], NodeState.UNKNOWN), public_ips=vm.get('ips', []), private_ips=[], driver=self, extra={'ai_active': vm.get('ai_active'), 'datacenter_id': vm.get('datacenter_id'), 'description': vm.get('description')})
    ...
    def _to_volume(self, disk):
        ...
        return StorageVolume(id=disk['id'], name=disk['name'], size=int(disk['size']), driver=self, extra=extra)
    ...
    def create_node(self, name, size, image, location=None, login=None, password=None, inet_family=4, keypairs=None):
        """
        ...
        if not login and (not keypairs):
            raise GandiException(1020, 'Login and password or ssh keypair must be defined for node creation')
        ...
        vm_spec = {'datacenter_id': dc_id, 'hostname': name, 'memory': int(size.ram), 'cores': cores, 'bandwidth': int(size.bandwidth), 'ip_version': inet_family}
        if login and password:
            vm_spec.update({'login': login, 'password': password})
        if keypair_ids:
            vm_spec['keys'] = keypair_ids
        op_disk, op_iface, op_vm = self.connection.request('hosting.vm.create_from', vm_spec, disk_spec, src_disk_id).object
    ...
    def _to_size(self, id, size):
        return NodeSize(id=id, name='%s cores' % id, ram=size['memory'], disk=size['disk'], bandwidth=size['bandwidth'], price=self._get_size_price(size_id='1') * id, driver=self.connection.driver)
    ...
    def create_volume(self, size, name, location=None, snapshot=None):
        ...
        :rtype:   :class:`StorageVolume`
        """
        disk_param = {'name': name, 'size': int(size), 'datacenter_id': int(location.id)}
        if snapshot:
    ...
    def _to_key_pair(self, data):
        key_pair = KeyPair(name=data['name'], fingerprint=data['fingerprint'], public_key=data.get('value', None), private_key=data.get('privatekey', None), driver=self, extra={'id': data['id']})
        return key_pair
