...
class SingleStoreDB(VectorStore):
    ...
    def _get_connection(self: SingleStoreDB) -> Any:
        try:
            ...
            raise ImportError('Could not import singlestoredb python package. Please install it with `pip install singlestoredb`.')
        ...
        try:
            ...
            try:
                if self.use_vector_index:
                    ...
                    cur.execute('CREATE TABLE IF NOT EXISTS {}\n                        ({} TEXT CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci,\n                        {} VECTOR({}, F32) NOT NULL, {} JSON,\n                        VECTOR INDEX {} ({}) {});'.format(self.table_name, self.content_field, self.vector_field, self.vector_size, self.metadata_field, self.vector_index_name, self.vector_field, index_options))
                else:
                    cur.execute('CREATE TABLE IF NOT EXISTS {}\n                        ({} TEXT CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci,\n                        {} BLOB, {} JSON);'.format(self.table_name, self.content_field, self.vector_field, self.metadata_field))
            finally:
                cur.close()
        finally:
            conn.close()
    ...
    def add_texts(self, texts: Iterable[str], metadatas: Optional[List[dict]]=None, embeddings: Optional[List[List[float]]]=None, **kwargs: Any) -> List[str]:
        ...
        try:
            ...
            try:
                for i, text in enumerate(texts):
                    ...
                    cur.execute('INSERT INTO {} VALUES (%s, JSON_ARRAY_PACK(%s), %s)'.format(self.table_name), (text, '[{}]'.format(','.join(map(str, embedding))), json.dumps(metadata)))
                if self.use_vector_index:
