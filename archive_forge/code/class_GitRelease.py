...
class GitRelease(CompletableGithubObject):
    ...
    def upload_asset_from_memory(self, file_like: BinaryIO, file_size: int, name: str, content_type: Opt[str]=NotSet, label: str='') -> github.GitReleaseAsset.GitReleaseAsset:
        ...
        resp_headers, data = self._requester.requestMemoryBlobAndCheck('POST', self.upload_url.split('{?')[0], parameters=post_parameters, headers=headers, file_like=file_like)
        return github.GitReleaseAsset.GitReleaseAsset(self._requester, resp_headers, data, completed=True)
    ...
    def get_assets(self) -> PaginatedList[github.GitReleaseAsset.GitReleaseAsset]:
        ...
        """
        return github.PaginatedList.PaginatedList(github.GitReleaseAsset.GitReleaseAsset, self._requester, f'{self.url}/assets', None)
    ...
    def _useAttributes(self, attributes: dict[str, Any]) -> None:
        ...
        if 'assets' in attributes:
            self._assets = self._makeListOfClassesAttribute(github.GitReleaseAsset.GitReleaseAsset, attributes['assets'])