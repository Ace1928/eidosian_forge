...
class RcodeZeroDNSDriver(DNSDriver):
    ...
    def create_zone(self, domain, type='master', ttl=None, extra={}):
        ...
        try:
            ...
            if e.code == httplib.UNPROCESSABLE_ENTITY and e.message.find("Zone '%s' already configured for your account" % domain):
                raise ZoneAlreadyExistsError(zone_id=zone_id, driver=self, value=e.message)
            raise e
        return Zone(id=zone_id, domain=domain, type=type.lower(), ttl=None, driver=self, extra=extra)
    ...
    def update_zone(self, zone, domain, type=None, ttl=None, extra=None):
        ...
        try:
            ...
            if e.code == httplib.UNPROCESSABLE_ENTITY and e.message.startswith("Domain '%s' update failed" % domain):
    ...
    def delete_record(self, record):
        ...
        payload = self._to_patchrequest(record.zone.id, None, record.name, record.type, record.data, record.extra, 'delete')
        try:
            self.connection.request(action=action, data=json.dumps(payload), method='PATCH')
        except BaseHTTPError as e:
            if e.code == httplib.UNPROCESSABLE_ENTITY and e.message.startswith('Could not find domain'):
    ...
    def get_record(self, zone_id, record_id):
        ...
        :rtype: :class:`Record`
        """
        records = self.list_records(Zone(id=zone_id, domain=zone_id, type=None, ttl=None, driver=self, extra=None))
    ...
    def update_record(self, record, name, type, data, extra=None):
        ...
        """
        action = '{}/zones/{}/rrsets'.format(self.api_root, record.zone.id)
        payload = self._to_patchrequest(record.zone.id, record, name, type, data, record.extra, 'update')
        try:
            self.connection.request(action=action, data=json.dumps(payload), method='PATCH')
        except BaseHTTPError as e:
            if e.code == httplib.UNPROCESSABLE_ENTITY and e.message.startswith('Could not find domain'):
        ...
        if not (extra is None or extra.get('ttl', None) is None):
            ...
            ttl = record.ttl
        return Record(id=hashlib.md5(str(name + ' ' + data).encode('utf-8')).hexdigest(), name=name, data=data, type=type, zone=record.zone, driver=self, ttl=ttl, extra=extra)

    def _to_zone(self, item):
        extra = {}
        for e in ['dnssec_status', 'dnssec_status_detail', 'dnssec_ksk_status', 'dnssec_ksk_status_detail', 'dnssec_ds', 'dnssec_dnskey', 'dnssec_safe_to_unsign', 'dnssec', 'masters', 'serial', 'created', 'last_check']:
            if e in item:
                extra[e] = item[e]
        return Zone(id=item['domain'], domain=item['domain'], type=item['type'].lower(), ttl=None, driver=self, extra=extra)
    ...
    def _to_records(self, items, zone):
        ...
        for item in items:
            for record in item['records']:
                ...
                records.append(Record(id=hashlib.md5(str(recordname + ' ' + record['content']).encode('utf-8')).hexdigest(), name=recordname, data=record['content'], type=item['type'], zone=zone, driver=self, ttl=item['ttl'], extra=extra))
        return records
