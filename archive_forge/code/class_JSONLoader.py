...
class JSONLoader(BaseLoader):
    ...
    def __init__(self, file_path: Union[str, Path], jq_schema: str, content_key: Optional[str]=None, is_content_key_jq_parsable: Optional[bool]=False, metadata_func: Optional[Callable[[Dict, Dict], Dict]]=None, text_content: bool=True, json_lines: bool=False):
    ...
    def _get_text(self, sample: Any) -> str:
        ...
        if self._text_content and (not isinstance(content, str)):
            raise ValueError(f'Expected page_content is string, got {type(content)} instead.                     Set `text_content=False` if the desired input for                     `page_content` is not a string')
