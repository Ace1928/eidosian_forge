...
class LLMChainFilter(BaseDocumentCompressor):
    ...
    def compress_documents(self, documents: Sequence[Document], query: str, callbacks: Optional[Callbacks]=None) -> Sequence[Document]:
        ...
        for doc in documents:
            ...
            include_doc = self.llm_chain.predict_and_parse(**_input, callbacks=callbacks)
            if include_doc:
                filtered_docs.append(doc)
        return filtered_docs

    @classmethod
    def from_llm(cls, llm: BaseLanguageModel, prompt: Optional[BasePromptTemplate]=None, **kwargs: Any) -> 'LLMChainFilter':
