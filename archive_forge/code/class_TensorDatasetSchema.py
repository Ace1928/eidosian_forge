...
class TensorDatasetSchema:
    ...
    def __init__(self, features: Schema, targets: Schema=None):
        if not isinstance(features, Schema):
            raise MlflowException(f"features must be mlflow.types.Schema, got '{type(features)}'", INVALID_PARAMETER_VALUE)
        if targets is not None and (not isinstance(targets, Schema)):
            raise MlflowException(f"targets must be either None or mlflow.types.Schema, got '{type(features)}'", INVALID_PARAMETER_VALUE)
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        """
        return {'mlflow_tensorspec': {'features': self.features.to_json(), 'targets': self.targets.to_json() if self.targets is not None else None}}
    ...
    def from_dict(cls, schema_dict: Dict[str, Any]):
        ...
        if 'mlflow_tensorspec' not in schema_dict:
            raise MlflowException("TensorDatasetSchema dictionary is missing expected key 'mlflow_tensorspec'", INVALID_PARAMETER_VALUE)
    ...
    def __eq__(self, other) -> bool:
        return isinstance(other, TensorDatasetSchema) and self.features == other.features and (self.targets == other.targets)
