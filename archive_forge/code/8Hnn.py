...
@StandardDecorator(validation_rules={"x": lambda x: x > 0}, log_level=logging.INFO)
...
@StandardDecorator(
    cache_results=True, cache_maxsize=100, enable_performance_logging=True
)
...
class ExampleClass:
    ...
    @StandardDecorator(log_level=logging.DEBUG)
    ...
    @StandardDecorator(retries=3, delay=2, retry_exceptions=(Exception,))
    ...
    @StandardDecorator()
    def example_function(param1: int, param2: Optional[str] = None) -> bool:
        """
        ...
        if param2 is not None and not isinstance(param2, str):
            ...
            raise ValueError("param2 cannot be 'error'")
        elif param2 == "warning":
            logging.warning("param2 is set to 'warning'")
        else:
            logging.info("param2 is set to: %s", param2)
        return True
...
if __name__ == "__main__":
    ...
    # Test examples
    try:
        print(f"Sync Example Result: {sync_example(10)}")
    except Exception as e:
        print(f"Sync Example Failed: {e}")

    asyncio.run(async_example(5))

    example_class_instance = ExampleClass(param="Example parameter")
    try:
        example_class_instance.example_method(value=-1)
    except ValueError as e:
        print(f"Example Method Failed: {e}")

    print(
        f"Example Function Result: {example_class_instance.example_function(10, 'example')}"
    )

    # Advanced examples
    print(f"Advanced Data Processing Result: {advanced_data_processing([1, 2, 3])}")
    print(f"User Authentication Result: {user_authentication('admin', 'admin')}")
# Additional code and functionalities can be added below following the same standards and conventions.
