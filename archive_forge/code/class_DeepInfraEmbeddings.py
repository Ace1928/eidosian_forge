...
class DeepInfraEmbeddings(BaseModel, Embeddings):
    ...
    def _embed(self, input: List[str]) -> List[List[float]]:
        ...
        headers = {'Authorization': f'bearer {self.deepinfra_api_token}', 'Content-Type': 'application/json'}
        try:
            res = requests.post(f'https://api.deepinfra.com/v1/inference/{self.model_id}', headers=headers, json={'inputs': input, 'normalize': self.normalize, **_model_kwargs})
