...
class WebSocketCommonProtocol(asyncio.Protocol):
    ...
    * on the server side, when the connection handler terminates.
    ...
    def __init__(self, *, logger: Optional[LoggerLike]=None, ping_interval: Optional[float]=20, ping_timeout: Optional[float]=20, close_timeout: Optional[float]=None, max_size: Optional[int]=2 ** 20, max_queue: Optional[int]=2 ** 5, read_limit: int=2 ** 16, write_limit: int=2 ** 16, host: Optional[str]=None, port: Optional[int]=None, secure: Optional[bool]=None, legacy_recv: bool=False, loop: Optional[asyncio.AbstractEventLoop]=None, timeout: Optional[float]=None) -> None:
        if legacy_recv:
    ...
    async def recv(self) -> Data:
        ...
        while len(self.messages) <= 0:
            ...
            try:
                await asyncio.wait([pop_message_waiter, self.transfer_data_task], return_when=asyncio.FIRST_COMPLETED)
            finally:
    ...
    async def send(self, message: Union[Data, Iterable[Data], AsyncIterable[Data]]) -> None:
        ...
        if isinstance(message, (str, bytes, bytearray, memoryview)):
            ...
            aiter_message = cast(Callable[[AsyncIterable[Data]], AsyncIterator[Data]], type(message).__aiter__)(message)
            try:
                fragment = await cast(Callable[[AsyncIterator[Data]], Awaitable[Data]], type(aiter_message).__anext__)(aiter_message)
    ...
    async def read_frame(self, max_size: Optional[int]) -> Frame:
        ...
        """
        frame = await Frame.read(self.reader.readexactly, mask=not self.is_client, max_size=max_size, extensions=self.extensions)
        if self.debug:
    ...
    async def write_close_frame(self, close: Close, data: Optional[bytes]=None) -> None:
        """
            ...
            while True:
                ...
                if self.ping_timeout is not None:
                    try:
                        ...
                        self.fail_connection(CloseCode.INTERNAL_ERROR, 'keepalive ping timeout')
                        break
        except ConnectionClosed:
            pass
        except Exception:
