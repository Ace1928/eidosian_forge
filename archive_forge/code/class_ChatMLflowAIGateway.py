...
class ChatMLflowAIGateway(BaseChatModel):
    ...
    def __init__(self, **kwargs: Any):
        warnings.warn('`ChatMLflowAIGateway` is deprecated. Use `ChatMlflow` or `ChatDatabricks` instead.', DeprecationWarning)
        try:
            import mlflow.gateway
        except ImportError as e:
            raise ImportError('Could not import `mlflow.gateway` module. Please install it with `pip install mlflow[gateway]`.') from e
    ...
    def _default_params(self) -> Dict[str, Any]:
        params: Dict[str, Any] = {'gateway_uri': self.gateway_uri, 'route': self.route, **(self.params.dict() if self.params else {})}
        return params
    ...
    def _convert_message_to_dict(message: BaseMessage) -> dict:
        ...
        if message.additional_kwargs:
            logger.warning('Additional message arguments are unsupported by MLflow AI Gateway  and will be ignored: %s', message.additional_kwargs)
        return message_dict
