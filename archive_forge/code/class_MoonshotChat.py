...
class MoonshotChat(MoonshotCommon, ChatOpenAI):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        values['moonshot_api_key'] = get_from_dict_or_env(values, 'moonshot_api_key', 'MOONSHOT_API_KEY')
        try:
            import openai
        except ImportError:
            raise ImportError('Could not import openai python package. Please install it with `pip install openai`.')
        client_params = {'api_key': values['moonshot_api_key'], 'base_url': values['base_url'] if 'base_url' in values else MOONSHOT_SERVICE_URL_BASE}
