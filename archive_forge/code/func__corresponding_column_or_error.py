...
def _corresponding_column_or_error(fromclause, column, require_embedded=False):
    ...
    if c is None:
        raise exc.InvalidRequestError("Given column '%s', attached to table '%s', failed to locate a corresponding column from table '%s'" % (column, getattr(column, 'table', None), fromclause.description))
    return c