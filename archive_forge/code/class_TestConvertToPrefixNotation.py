...
class TestConvertToPrefixNotation(unittest.TestCase):
    ...
    def test_linear_expression(self):
        ...
        expected = [(LinearExpression, 5), 3, (MonomialTermExpression, 2), 1, m.x[1], (MonomialTermExpression, 2), 2, m.x[2], (MonomialTermExpression, 2), 3, m.x[3], (MonomialTermExpression, 2), 4, m.x[4]]
    ...
    def test_multiple(self):
        ...
        m.y = pe.Var()
        e = m.x ** 2 + m.x * m.y / 3 + 4
        expected = [(SumExpression, 3), (PowExpression, 2), m.x, 2, (DivisionExpression, 2), (ProductExpression, 2), m.x, m.y, 3, 4]
        pn = convert_expression_to_prefix_notation(e)
        self.assertEqual(pn, expected)
    ...
    def test_external_function(self):
        ...
        expected = [(ProductExpression, 2), 2, (ExternalFunctionExpression, 2, m.hypot), m.x, (ProductExpression, 2), m.x, m.y]
        pn = convert_expression_to_prefix_notation(e)
        self.assertEqual(expected, pn)
    ...
    def test_expr_if(self):
        ...
        pn = convert_expression_to_prefix_notation(e)
        expected = [(Expr_ifExpression, 3), (InequalityExpression, 2), m.x, 0, (EqualityExpression, 2), (LinearExpression, 2), (MonomialTermExpression, 2), 1, m.y, (MonomialTermExpression, 2), 1, m.x, 0, (EqualityExpression, 2), (LinearExpression, 2), (MonomialTermExpression, 2), 1, m.y, (MonomialTermExpression, 2), -1, m.x, 0]
        self.assertEqual(pn, expected)
