...
class DepthwiseConv(Conv):
    ...
    def __init__(self, rank, kernel_size, strides=1, padding='valid', depth_multiplier=1, data_format=None, dilation_rate=1, activation=None, use_bias=True, depthwise_initializer='glorot_uniform', bias_initializer='zeros', depthwise_regularizer=None, bias_regularizer=None, activity_regularizer=None, depthwise_constraint=None, bias_constraint=None, **kwargs):
        super().__init__(rank, filters=None, kernel_size=kernel_size, strides=strides, padding=padding, data_format=data_format, dilation_rate=dilation_rate, activation=activation, use_bias=use_bias, bias_regularizer=bias_regularizer, activity_regularizer=activity_regularizer, bias_constraint=bias_constraint, **kwargs)
    ...
    def build(self, input_shape):
        if len(input_shape) != self.rank + 2:
            raise ValueError(f'Inputs to `DepthwiseConv` should have rank {self.rank + 2}. Received input_shape={input_shape}.')
        input_shape = tf.TensorShape(input_shape)
        channel_axis = self._get_channel_axis()
        if input_shape.dims[channel_axis].value is None:
            raise ValueError(f'The channel dimension of the inputs to `DepthwiseConv` should be defined. The input_shape received is {input_shape}, where axis {channel_axis} (0-based) is the channel dimension, which found to be `None`.')
