...
def ValidateResource(resource_ref):
    ...
    k8s_resource_name_regex = re.compile('(?=^[a-z0-9-]{1,63}$)(?!^\\-.*)(?!.*\\-$)')
    if not k8s_resource_name_regex.match(resource_ref.Name()):
        raise serverless_exceptions.ArgumentError('Invalid resource name [{}]. The name must use only lowercase alphanumeric characters and dashes, cannot begin or end with a dash, and cannot be longer than 63 characters.'.format(resource_ref.Name()))