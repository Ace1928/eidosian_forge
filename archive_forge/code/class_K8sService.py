...
class K8sService:
    ...
    def find_resource(self, kind: str, api_version: str, fail: bool=False) -> Optional[Resource]:
        try:
            return self.client.resource(kind, api_version)
        except (ResourceNotFoundError, ResourceNotUniqueError):
            if fail:
                raise CoreException('Failed to find exact match for %s.%s by [kind, name, singularName, shortNames]' % (api_version, kind))

    def wait(self, resource: Resource, instance: Dict) -> Tuple[bool, Optional[Dict], int]:
        wait_sleep = self.module.params.get('wait_sleep')
        wait_timeout = self.module.params.get('wait_timeout')
        wait_condition = None
        if self.module.params.get('wait_condition') and self.module.params['wait_condition'].get('type'):
        ...
        waiter = get_waiter(self.client, resource, condition=wait_condition, state=state)
        return waiter.wait(timeout=wait_timeout, sleep=wait_sleep, name=instance['metadata'].get('name'), namespace=instance['metadata'].get('namespace'), label_selectors=label_selectors)
    ...
    def find(self, kind: str, api_version: str, name: str=None, namespace: Optional[str]=None, label_selectors: Optional[List[str]]=None, field_selectors: Optional[List[str]]=None, wait: Optional[bool]=False, wait_sleep: Optional[int]=5, wait_timeout: Optional[int]=120, state: Optional[str]='present', condition: Optional[Dict]=None) -> Dict:
        ...
        try:
            success, resources, duration = waiter.wait(timeout=wait_timeout, sleep=wait_sleep, name=name, namespace=namespace, label_selectors=label_selectors, field_selectors=field_selectors)
        except BadRequestError:
            return result
        except CoreException as e:
            raise e
        except Exception as e:
            raise CoreException("Exception '{0}' raised while trying to get resource using (name={1}, namespace={2}, label_selectors={3}, field_selectors={4})".format(e, name, namespace, label_selectors, field_selectors))
        if not resources or (resources['kind'].endswith('List') and (not resources.get('items'))):
            return result
        ...
        for instance in instances:
            ...
            success, res, duration = waiter.wait(timeout=wait_timeout, sleep=wait_sleep, name=name, namespace=namespace)
            if not success:
                raise CoreException('Failed to gather information about %s(s) even after waiting for %s seconds' % (res.get('kind'), duration))
    ...
    def create(self, resource: Resource, definition: Dict) -> Dict:
        ...
        if self._client_side_dry_run:
            ...
            try:
                k8s_obj = self.client.create(resource, definition, namespace=namespace).to_dict()
            except ConflictError:
                self.module.warn('{0} was not found, but creating it returned a 409 Conflict error. This can happen                             if the resource you are creating does not directly create a resource of the same kind.'.format(name))
                return dict()
            except Exception as e:
                reason = e.body if hasattr(e, 'body') else e
                msg = 'Failed to create object: {0}'.format(reason)
    ...
    def apply(self, resource: Resource, definition: Dict, existing: Optional[ResourceInstance]=None) -> Dict:
        namespace = definition['metadata'].get('namespace')
        ...
        if self._client_side_dry_run:
            ...
            try:
                ...
                k8s_obj = self.client.apply(resource, definition, namespace=namespace, **params).to_dict()
            except Exception as e:
                reason = e.body if hasattr(e, 'body') else e
    ...
    def replace(self, resource: Resource, definition: Dict, existing: ResourceInstance) -> Dict:
        append_hash = self.module.params.get('append_hash', False)
        name = definition['metadata'].get('name')
        namespace = definition['metadata'].get('namespace')
        if self._client_side_dry_run:
            ...
            try:
                k8s_obj = self.client.replace(resource, definition, name=name, namespace=namespace, append_hash=append_hash).to_dict()
            except Exception as e:
                reason = e.body if hasattr(e, 'body') else e
    ...
    def update(self, resource: Resource, definition: Dict, existing: ResourceInstance) -> Dict:
        name = definition['metadata'].get('name')
        namespace = definition['metadata'].get('namespace')
        if self._client_side_dry_run:
            ...
            for merge_type in self.module.params.get('merge_type') or ['strategic-merge', 'merge']:
                try:
                    k8s_obj = self.patch_resource(resource, definition, name, namespace, merge_type=merge_type)
                    exception = None
                except CoreException as e:
                    exception = e
                    continue
    ...
    def delete(self, resource: Resource, definition: Dict, existing: Optional[ResourceInstance]=None) -> Dict:
