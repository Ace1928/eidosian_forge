...
class WhisperMessage(CiphertextMessage):
    ...
    def __init__(self, messageVersion=None, macKey=None, ECPublicKey_senderRatchetKey=None, counter=None, previousCounter=None, ciphertext=None, senderIdentityKey=None, receiverIdentityKey=None, serialized=None):
        self.serialized = ''
        if serialized:
            try:
                assert type(serialized) in (str, bytes), 'Expected serialized %s, got %s' % (str, type(serialized))
                messageParts = ByteUtil.split(serialized, 1, len(serialized) - 1 - WhisperMessage.MAC_LENGTH, WhisperMessage.MAC_LENGTH)
            ...
            mac = self.getMac(messageVersion, senderIdentityKey, receiverIdentityKey, macKey, ByteUtil.combine(version, message))
