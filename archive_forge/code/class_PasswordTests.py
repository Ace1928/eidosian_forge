...
class PasswordTests(utils.GenericPluginTestCase):
    ...
    def test_default_domain_id_with_v3(self):
        ...
        p = super(PasswordTests, self).test_default_domain_id_with_v3(default_domain_id=default_domain_id)
    ...
    def test_default_domain_name_with_v3(self):
        ...
        p = super(PasswordTests, self).test_default_domain_name_with_v3(default_domain_name=default_domain_name)
    ...
    def test_password_cache_id(self):
        ...
        default_domain_id = uuid.uuid4().hex
        a = password.Password(self.TEST_URL, username=username, password=the_password, project_name=project_name, default_domain_id=default_domain_id)
        b = password.Password(self.TEST_URL, username=username, password=the_password, project_name=project_name, default_domain_id=default_domain_id)
        a_id = a.get_cache_id()
        b_id = b.get_cache_id()
        self.assertEqual(a_id, b_id)
        c = password.Password(self.TEST_URL, username=username, password=uuid.uuid4().hex, project_name=project_name, default_domain_id=default_domain_id)
        c_id = c.get_cache_id()
