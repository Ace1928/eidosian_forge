...
def xarray_pipeline(xr_ds, schema, canvas, glyph, summary, *, antialias=False):
    ...
    if not xr_ds.chunks:
        return glyph_dispatch(glyph, xr_ds, schema, canvas, summary, antialias=antialias, cuda=cuda)
    else:
        from datashader.data_libraries.dask_xarray import dask_xarray_pipeline
        return dask_xarray_pipeline(glyph, xr_ds, schema, canvas, summary, antialias=antialias, cuda=cuda)