...
class MyDevicePortal(object):
    ...
    def get_object_by_name(self, name):
        result = None
        gen_items_responses = self.ise.exec(family='my_device_portal', function='get_my_device_portal_generator')
        try:
    ...
    def get_object_by_id(self, id):
        try:
            result = self.ise.exec(family='my_device_portal', function='get_my_device_portal_by_id', handle_func_exception=False, params={'id': id}).response['MyDevicePortal']
        except (TypeError, AttributeError) as e:
            self.ise.fail_json(msg='An error occured when executing operation. Check the configuration of your API Settings and API Gateway settings on your ISE server. This collection assumes that the API Gateway, the ERS APIs and OpenAPIs are enabled. You may want to enable the (ise_debug: True) argument. The error was: {error}'.format(error=e))
        except Exception:
            result = None
    ...
    def create(self):
        result = self.ise.exec(family='my_device_portal', function='create_my_device_portal', params=self.new_object).response
        return result
    ...
    def update(self):
        ...
        result = self.ise.exec(family='my_device_portal', function='update_my_device_portal_by_id', params=self.new_object).response
        return result
    ...
    def delete(self):
        ...
        if not id:
            id_ = self.get_object_by_name(name).get('id')
            self.new_object.update(dict(id=id_))
        result = self.ise.exec(family='my_device_portal', function='delete_my_device_portal_by_id', params=self.new_object).response
        return result