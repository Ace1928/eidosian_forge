    ...
    **Example**
    ...
    def __init__(self, *params, control_wires=None, wires=None, control_values=None):
        ...
        if Wires.shared_wires([wires, control_wires]):
            raise ValueError('The control wires must be different from the wires specified to apply the unitary on.')
        self._hyperparameters = {'u_wires': wires, 'control_wires': control_wires, 'control_values': control_values}
    ...
    def compute_matrix(U, control_wires, u_wires, control_values=None):
        ...
        **Example**
        ...
        if not (len(shape) in {2, 3} and shape[-2:] == (target_dim, target_dim)):
            raise ValueError(f'Input unitary must be of shape {(target_dim, target_dim)} or (batch_size, {target_dim}, {target_dim}).')
    ...
    def pow(self, z):
        if isinstance(z, int):
            return [ControlledQutritUnitary(qml.math.linalg.matrix_power(self.data[0], z), control_wires=self.control_wires, wires=self.hyperparameters['u_wires'], control_values=self.hyperparameters['control_values'])]
        return super().pow(z)

    def adjoint(self):
        return ControlledQutritUnitary(qml.math.conj(qml.math.moveaxis(self.data[0], -2, -1)), control_wires=self.control_wires, wires=self.hyperparameters['u_wires'], control_values=self.hyperparameters['control_values'])
    ...
    def _controlled(self, wire):
        ...
        return ControlledQutritUnitary(*self.parameters, control_wires=ctrl_wires, wires=self.hyperparameters['u_wires'], control_values=values)