...
class CVNeuralNetLayers(Operation):
    ...
    def __init__(self, theta_1, phi_1, varphi_1, r, phi_r, theta_2, phi_2, varphi_2, a, phi_a, k, wires, id=None):
        n_wires = len(wires)
        ...
        super().__init__(theta_1, phi_1, varphi_1, r, phi_r, theta_2, phi_2, varphi_2, a, phi_a, k, wires=wires, id=id)
    ...
    def compute_decomposition(theta_1, phi_1, varphi_1, r, phi_r, theta_2, phi_2, varphi_2, a, phi_a, k, wires):
        ...
        **Example**
        ...
        for m in range(n_layers):
            op_list.append(qml.Interferometer(theta=theta_1[m], phi=phi_1[m], varphi=varphi_1[m], wires=wires))
            for i in range(len(wires)):
                op_list.append(qml.Squeezing(r[m, i], phi_r[m, i], wires=wires[i]))
            op_list.append(qml.Interferometer(theta=theta_2[m], phi=phi_2[m], varphi=varphi_2[m], wires=wires))
            for i in range(len(wires)):
    ...
    def shape(n_layers, n_wires):
        ...
        """
        n_if = n_wires * (n_wires - 1) // 2
        shapes = [(n_layers, n_if)] * 2 + [(n_layers, n_wires)] * 3 + [(n_layers, n_if)] * 2 + [(n_layers, n_wires)] * 4
        return shapes