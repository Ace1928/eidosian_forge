...
class PWInput:
    ...
    def __init__(self, structure, pseudo=None, control=None, system=None, electrons=None, ions=None, cell=None, kpoints_mode='automatic', kpoints_grid=(1, 1, 1), kpoints_shift=(0, 0, 0)):
        """
    ...
    def as_dict(self):
        ...
        Returns:
            dict
        """
        return {'structure': self.structure.as_dict(), 'pseudo': self.pseudo, 'sections': self.sections, 'kpoints_mode': self.kpoints_mode, 'kpoints_grid': self.kpoints_grid, 'kpoints_shift': self.kpoints_shift}
    ...
    def from_dict(cls, dct: dict) -> Self:
        ...
        Returns:
            PWInput object
        """
        return cls(structure=Structure.from_dict(dct['structure']), pseudo=dct['pseudo'], control=dct['sections']['control'], system=dct['sections']['system'], electrons=dct['sections']['electrons'], ions=dct['sections']['ions'], cell=dct['sections']['cell'], kpoints_mode=dct['kpoints_mode'], kpoints_grid=dct['kpoints_grid'], kpoints_shift=dct['kpoints_shift'])
    ...
    def from_str(cls, string: str) -> Self:
        ...
        def input_mode(line):
            ...
            if line == '/':
                return None
            return mode
        sections: dict[str, dict] = {'control': {}, 'system': {}, 'electrons': {}, 'ions': {}, 'cell': {}}
        pseudo = {}
        lattice = []
        species = []
        coords = []
        ...
        for line in lines:
            ...
            if mode is None:
                ...
                m_l = re.match('(-?\\d+\\.?\\d*)\\s+(-?\\d+\\.?\\d*)\\s+(-?\\d+\\.?\\d*)', line)
                m_p = re.match('(\\w+)\\s+(-?\\d+\\.\\d*)\\s+(-?\\d+\\.?\\d*)\\s+(-?\\d+\\.?\\d*)', line)
                if m_l:
        ...
        structure = Structure(Lattice(lattice), species, coords, coords_are_cartesian=coords_are_cartesian, site_properties=site_properties)
        return cls(structure=structure, control=sections['control'], pseudo=pseudo, system=sections['system'], electrons=sections['electrons'], ions=sections['ions'], cell=sections['cell'], kpoints_mode=kpoints_mode, kpoints_grid=kpoints_grid, kpoints_shift=kpoints_shift)
    ...
    def proc_val(key, val):
        ...
        """
        float_keys = ('etot_conv_thr', 'forc_conv_thr', 'conv_thr', 'Hubbard_U', 'Hubbard_J0', 'degauss', 'starting_magnetization')
        int_keys = ('nstep', 'iprint', 'nberrycyc', 'gdir', 'nppstr', 'ibrav', 'nat', 'ntyp', 'nbnd', 'nr1', 'nr2', 'nr3', 'nr1s', 'nr2s', 'nr3s', 'nspin', 'nqx1', 'nqx2', 'nqx3', 'lda_plus_u_kind', 'edir', 'report', 'esm_nfit', 'space_group', 'origin_choice', 'electron_maxstep', 'mixing_ndim', 'mixing_fixed_ns', 'ortho_para', 'diago_cg_maxiter', 'diago_david_ndim', 'nraise', 'bfgs_ndim', 'if_pos', 'nks', 'nk1', 'nk2', 'nk3', 'sk1', 'sk2', 'sk3', 'nconstr')
        bool_keys = ('wf_collect', 'tstress', 'tprnfor', 'lkpoint_dir', 'tefield', 'dipfield', 'lelfield', 'lorbm', 'lberry', 'lfcpopt', 'monopole', 'nosym', 'nosym_evc', 'noinv', 'no_t_rev', 'force_symmorphic', 'use_all_frac', 'one_atom_occupations', 'starting_spin_angle', 'noncolin', 'x_gamma_extrapolation', 'lda_plus_u', 'lspinorb', 'london', 'ts_vdw_isolated', 'xdm', 'uniqueb', 'rhombohedral', 'realxz', 'block', 'scf_must_converge', 'adaptive_thr', 'diago_full_acc', 'tqr', 'remove_rigid_rot', 'refold_pos')
        ...
        try:
            ...
            if key in float_keys:
                return float(re.search('^-?\\d*\\.?\\d*d?-?\\d*', val.lower()).group(0).replace('d', 'e'))
            if key in int_keys:
