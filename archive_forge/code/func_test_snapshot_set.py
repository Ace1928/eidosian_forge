...
def test_snapshot_set(self):
    ...
    name = uuid.uuid4().hex
    new_name = name + '_'
    cmd_output = self.openstack('volume snapshot create ' + '--volume ' + self.VOLLY + ' --description aaaa ' + name, parse_output=True)
    ...
    self.assertEqual(name, cmd_output['display_name'])
    self.assertEqual(1, cmd_output['size'])
    self.assertEqual('aaaa', cmd_output['display_description'])
    self.wait_for_status('volume snapshot', name, 'available')
    raw_output = self.openstack('volume snapshot set ' + '--name ' + new_name + ' --description bbbb ' + '--property Alpha=a ' + '--property Beta=b ' + name)
    self.assertOutput('', raw_output)
    cmd_output = self.openstack('volume snapshot show ' + new_name, parse_output=True)
    self.assertEqual(new_name, cmd_output['display_name'])
    self.assertEqual(1, cmd_output['size'])
    self.assertEqual('bbbb', cmd_output['display_description'])
    self.assertEqual({'Alpha': 'a', 'Beta': 'b'}, cmd_output['properties'])
