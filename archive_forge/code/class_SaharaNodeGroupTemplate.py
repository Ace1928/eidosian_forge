...
class SaharaNodeGroupTemplate(resource.Resource):
    ...
    def translation_rules(self, props):
        ...
        return [translation.TranslationRule(props, translation.TranslationRule.RESOLVE, [self.FLAVOR], client_plugin=self.client_plugin('nova'), finder='find_flavor_by_name_or_id'), translation.TranslationRule(props, translation.TranslationRule.RESOLVE, [self.FLOATING_IP_POOL], client_plugin=neutron_client_plugin, finder='find_resourceid_by_name_or_id', entity=neutron_client_plugin.RES_TYPE_NETWORK)]
    ...
    def handle_create(self):
        ...
        LOG.info("Node Group Template '%s' has been created", node_group_template.name)
    ...
    def validate(self):
        ...
        if pool:
            if self.is_using_neutron():
                neutron_client_plugin = self.client_plugin('neutron')
                try:
                    neutron_client_plugin.find_resourceid_by_name_or_id(neutron_client_plugin.RES_TYPE_NETWORK, pool)
                except Exception as ex:
                    if neutron_client_plugin.is_not_found(ex) or neutron_client_plugin.is_no_unique(ex):
                ...
                try:
                    ...
                    if self.client_plugin('nova').is_not_found(ex):
                        ...
                        raise exception.StackValidationFailed(message=err_msg)
                    raise
        self.client_plugin().validate_hadoop_version(self.properties[self.PLUGIN_NAME], self.properties[self.HADOOP_VERSION])
        plugin = self.client().plugins.get_version_details(self.properties[self.PLUGIN_NAME], self.properties[self.HADOOP_VERSION])
        allowed_processes = [item for sublist in list(plugin.node_processes.values()) for item in sublist]
        ...
        if unsupported_processes:
            msg = _("Plugin %(plugin)s doesn't support the following node processes: %(unsupported)s. Allowed processes are: %(allowed)s") % {'plugin': self.properties[self.PLUGIN_NAME], 'unsupported': ', '.join(unsupported_processes), 'allowed': ', '.join(allowed_processes)}
            raise exception.StackValidationFailed(path=[self.stack.t.RESOURCES, self.name, self.stack.t.get_section_name(rsrc_defn.PROPERTIES)], message=msg)

    def parse_live_resource_data(self, resource_properties, resource_data):
        result = super(SaharaNodeGroupTemplate, self).parse_live_resource_data(resource_properties, resource_data)
