...
class NetAppOntapNetPort:
    ...
    def set_playbook_zapi_key_map(self):
        self.na_helper.zapi_string_keys = {'duplex_admin': 'administrative-duplex', 'speed_admin': 'administrative-speed', 'flowcontrol_admin': 'administrative-flowcontrol', 'ipspace': 'ipspace'}
        self.na_helper.zapi_bool_keys = {'up_admin': 'is-administrative-up', 'autonegotiate_admin': 'is-administrative-auto-negotiate'}
        self.na_helper.zapi_int_keys = {'mtu': 'mtu'}
    ...
    def get_net_port_rest(self, port):
        ...
        query = {'name': port, 'node.name': self.parameters['node']}
        fields = 'name,node,uuid,enabled'
        record, error = rest_generic.get_one_record(self.rest_api, api, query, fields)
        if error:
            self.module.fail_json(msg=error)
        if record:
            current = {'name': record['name'], 'node': record['node']['name'], 'uuid': record['uuid'], 'up_admin': record['enabled']}
            return current
        return None
    ...
    def apply(self):
        ...
        if missing_ports:
            ...
            if len(missing_ports) > 1:
                plural = 's'
            self.module.fail_json(changed=self.na_helper.changed, modify=modified, msg='Error: port%s: %s not found on node: %s%s' % (plural, ', '.join(missing_ports), self.parameters['node'], suffix))
