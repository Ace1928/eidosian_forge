...
class DocReaderModel(object):
    ...
    def __init__(self, opt, word_dict, feature_dict, state_dict=None):
        ...
        if opt['optimizer'] == 'sgd':
            self.optimizer = optim.SGD(parameters, opt['learning_rate'], momentum=opt['momentum'], weight_decay=opt['weight_decay'])
    ...
    def predict(self, ex):
        self.network.eval()
        if self.opt['cuda']:
            inputs = [Variable(e.cuda(non_blocking=True), volatile=True) for e in ex[:5]]
        else:
    ...
    def reset_parameters(self):
        if self.opt['tune_partial'] > 0:
            ...
            if offset < self.network.embedding.weight.data.size(0):
                self.network.embedding.weight.data[offset:] = self.network.fixed_embedding

    def save(self, filename):
        params = {'state_dict': {'network': self.network.state_dict()}, 'feature_dict': self.feature_dict, 'config': self.opt}
        try:
