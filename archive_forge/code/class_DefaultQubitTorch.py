...
class DefaultQubitTorch(DefaultQubitLegacy):
    ...
    * Only exact expectation values, variances, and probabilities are differentiable.
    ...
    _tensordot = staticmethod(lambda a, b, axes: torch.tensordot(a, b, axes if isinstance(axes, int) else tuple(map(list, axes))))
    ...
    def execute(self, circuit, **kwargs):
        ...
        if not self._torch_device_specified:
            ...
            if self._state.device != self._torch_device:
                self._state = self._state.to(self._torch_device)
        elif par_torch_device is not None:
            ...
            if params_cuda_device != specified_device_cuda:
                warnings.warn(f'Torch device {self._torch_device} specified upon PennyLane device creation does not match the Torch device of the gate parameters; {self._torch_device} will be used.')
