...
class ModuleHelper(DeprecateAttrsMixin, VarsMixin, DependencyMixin, ModuleHelperBase):
    ...
    def __init__(self, module=None):
        ...
        for name, value in self.module.params.items():
            self.vars.set(name, value, diff=name in self.diff_params, output=name in self.output_params, change=None if not self.change_params else name in self.change_params, fact=name in self.facts_params)
