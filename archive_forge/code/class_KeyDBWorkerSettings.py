...
class KeyDBWorkerSettings(BaseSettings):
    ...
    job_serializer: Optional[SerializerType] = SerializerType.dill
    job_prefix: Optional[str] = 'job'
    job_timeout: Optional[int] = 120
    ...
    function_tracker_ttl: Optional[int] = 60 * 60 * 24 * 7
    ...
    def timers(self) -> Dict[str, int]:
        ...
        """
        return {'schedule': self.schedule_timer, 'stats': self.stats_timer, 'sweep': self.sweep_timer, 'abort': self.abort_timer, 'heartbeat': self.heartbeat_timer, 'broadcast': self.broadcast_timer}

    def get_functions(self, verbose: Optional[bool]=None) -> List[Callable]:
        """
    ...
    async def run_dependencies(self, ctx: Dict, verbose: Optional[bool]=None) -> Dict[str, Any]:
        """
        ...
        for name, dep in self.tasks.dependencies.items():
            func, kwargs = dep
            if verbose:
                logger.info(f'[dependency] setting ctx[{name}]: result of {func.__name__}')
    ...
    async def run_context(self, ctx: Dict, verbose: Optional[bool]=None) -> Dict[str, Any]:
        """
        ...
        for name, dep in self.tasks.context_funcs.items():
            func, kwargs = dep
            if verbose:
                logger.info(f'[context] setting ctx[{name}]: result of {func.__name__}')
            if kwargs.get('_set_ctx'):
                ctx = await func(ctx, **kwargs) if is_coro_func(func) else func(ctx, **kwargs)
            else:
                ctx[name] = await func(**kwargs) if is_coro_func(func) else func(**kwargs)
    ...
    async def run_startup_funcs(self, ctx: Dict, verbose: Optional[bool]=True) -> Dict[str, Any]:
        """
        ...
        for name, dep in self.tasks.startup_funcs.items():
            func, kwargs = dep
            if verbose:
                logger.info(f'[startup] Setting ctx[{name}] = result of: {func.__name__}')
            if kwargs.get('_set_ctx'):
                ctx = await func(ctx, **kwargs) if is_coro_func(func) else func(ctx, **kwargs)
            else:
                ctx[name] = await func(**kwargs) if is_coro_func(func) else func(**kwargs)
    ...
    async def run_shutdown_funcs(self, ctx: Dict, verbose: Optional[bool]=None) -> Dict[str, Any]:
        """
        ...
        for name, dep in self.tasks.shutdown_funcs.items():
            func, kwargs = dep
            if verbose:
                logger.info(f'[shutdown] task: {name} = {func.__name__}')
            await func(ctx=ctx, **kwargs) if is_coro_func(func) else func(ctx=ctx, **kwargs)
        return ctx

    async def get_startup_context(self, ctx: Dict[str, Any], verbose: Optional[bool]=None) -> Dict[str, Any]:
        """
        ...
        for name, obj in self.tasks.context.items():
            if name not in ctx:
                ctx[name] = obj
                if verbose:
                    logger.info(f'[context] setting ctx[{name}]: {obj}')
    ...
    def add_context(self, obj: Optional[Any]=None, name: Optional[str]=None, verbose: Optional[bool]=None, silenced: Optional[bool]=None, _fx: Optional[Callable]=None, disabled: Optional[bool]=False, **kwargs):
        """
        ...
        if obj is not None:
            ...
            if callable(obj):
                ...
                if verbose:
                    logger.info(f'Registered context function {name}: {obj}')
            else:
                self.tasks.context[name] = obj
                if verbose:
                    logger.info(f'Registered context {name}: {obj}')
            if silenced is True:
        ...
        if _fx is not None:
            ...
            if verbose:
                logger.info(f'Registered context function {name}: {_fx}')
            if silenced is True:
                self.add_function_to_silenced(name)
            return
        ...
        def wrapper(func: Callable):
            ...
            if verbose:
                logger.info(f'Registered context function {func_name}: {func}')
            if silenced is True:
                self.add_function_to_silenced(func_name)
            return func
        return wrapper

    def add_dependency(self, obj: Optional[Any]=None, name: Optional[str]=None, verbose: Optional[bool]=None, _fx: Optional[Callable]=None, silenced: Optional[bool]=None, disabled: Optional[bool]=False, **kwargs):
        """
        ...
        if obj is not None:
            ...
            if verbose:
                logger.info(f'Registered dependency {name}: {obj}')
            if silenced is True:
                self.add_function_to_silenced(name)
            return
        if _fx is not None:
            ...
            if verbose:
                logger.info(f'Registered dependency {name}: {_fx}')
            if silenced is True:
                self.add_function_to_silenced(name)
            return
        ...
        def wrapper(func: Callable):
            ...
            if verbose:
                logger.info(f'Registered depency{func_name}: {func}')
            if silenced is True:
                self.add_function_to_silenced(func_name)
            return func
        return wrapper
    ...
    def on_shutdown(self, name: Optional[str]=None, verbose: Optional[bool]=None, _fx: Optional[Callable]=None, silenced: Optional[bool]=None, disabled: Optional[bool]=False, **kwargs):
        """
        ...
        if _fx is not None:
            ...
            if verbose:
                logger.info(f'Registered shutdown function {name}: {_fx}')
            if silenced is True:
                self.add_function_to_silenced(name)
            return
        ...
        def decorator(func: Callable):
            ...
            if verbose:
                logger.info(f'Registered shutdown function {func_name}: {func}')
            if silenced is True:
                self.add_function_to_silenced(name)
            return func
        return decorator

    def add_function(self, name: Optional[str]=None, _fx: Optional[Callable]=None, verbose: Optional[bool]=None, silenced: Optional[bool]=None, silenced_stages: Optional[List[str]]=None, disabled: Optional[bool]=False, **kwargs):
        """
    ...
    def has_silenced_functions(self) -> bool:
        ...
        """
        if self.tasks.silenced_functions:
            return True
    ...
    def set_queue_func(self, queue_func: Union[Callable, 'TaskQueue']):
        """
    ...
    def add(self, task: TaskType=TaskType.default, name: Optional[str]=None, obj: Optional[Any]=None, verbose: Optional[bool]=None, disabled: Optional[bool]=False, **kwargs):
        """
        Add a function to the worker queue.
