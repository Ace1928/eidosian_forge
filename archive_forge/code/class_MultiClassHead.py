...
class MultiClassHead(base_head.Head):
  ...
  """
    ...
    def __init__(self, n_classes, weight_column=None, label_vocabulary=None, loss_reduction=tf.losses.Reduction.SUM_OVER_BATCH_SIZE, loss_fn=None, name=None):
        ...
        if label_vocabulary is not None and len(label_vocabulary) != n_classes:
            raise ValueError('"label_vocabulary" does not have "n_classes" items. len(label_vocabulary)={}, n_classes={}, label_vocabulary={}'.format(len(label_vocabulary), n_classes, label_vocabulary))
    ...
    def _unweighted_loss_and_weights(self, logits, label_ids, features):
        """Computes loss spec."""
        if self._loss_fn:
            unweighted_loss = base_head.call_loss_fn(loss_fn=self._loss_fn, labels=label_ids, logits=logits, features=features, expected_loss_dim=1)
        else:
    ...
    def loss(self, labels, logits, features=None, mode=None, regularization_losses=None):
        ...
        with ops.name_scope('losses', values=(logits, labels, regularization_losses, features)):
            ...
            unweighted_loss, weights = self._unweighted_loss_and_weights(logits, label_ids, features)
    ...
    """
        ...
        valid_keys = [pred_keys.LOGITS, pred_keys.PROBABILITIES, pred_keys.CLASS_IDS, pred_keys.CLASSES, pred_keys.ALL_CLASS_IDS, pred_keys.ALL_CLASSES]
        if keys:
        ...
        with ops.name_scope('predictions', values=(logits,)):
            ...
            if pred_keys.ALL_CLASS_IDS in keys:
                predictions[pred_keys.ALL_CLASS_IDS] = base_head.all_class_ids(logits, n_classes=self._n_classes)
            if pred_keys.ALL_CLASSES in keys:
                predictions[pred_keys.ALL_CLASSES] = base_head.all_classes(logits, n_classes=self._n_classes, label_vocabulary=self._label_vocabulary)
            return predictions
