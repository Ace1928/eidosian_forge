...
class InteriorPointMA27Interface(MA27, IPLinearSolverInterface):
    ...
    def __init__(self, cntl_options=None, icntl_options=None, iw_factor=1.2, a_factor=2):
        super(InteriorPointMA27Interface, self).__init__(cntl_options=cntl_options, icntl_options=icntl_options, iw_factor=iw_factor, a_factor=a_factor)
    ...
    def do_symbolic_factorization(self, matrix: Union[spmatrix, BlockMatrix], raise_on_error: bool=True) -> LinearSolverResults:
        self._num_status = None
        return super(InteriorPointMA27Interface, self).do_symbolic_factorization(matrix=matrix, raise_on_error=raise_on_error)

    def do_numeric_factorization(self, matrix: Union[spmatrix, BlockMatrix], raise_on_error: bool=True) -> LinearSolverResults:
        res = super(InteriorPointMA27Interface, self).do_numeric_factorization(matrix=matrix, raise_on_error=raise_on_error)
