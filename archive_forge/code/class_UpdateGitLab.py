...
class UpdateGitLab(base.UpdateCommand):
    ...
    """
        ...
        flag_config.add_argument('--repository', help='Repository resource (2nd gen) to use, in the format "projects/*/locations/*/connections/*/repositories/*".')
    ...
    """
        ...
        if args.branch_pattern:
            ...
            trigger.repositoryEventConfig.push = messages.PushFilter(branch=args.branch_pattern)
            update_mask.append('repository_event_config.pull_request')
        if args.tag_pattern:
            if trigger.repositoryEventConfig is None:
                trigger.repositoryEventConfig = messages.RepositoryEventConfig()
            trigger.repositoryEventConfig.push = messages.PushFilter(tag=args.tag_pattern)
            update_mask.append('repository_event_config.pull_request')
        if args.pull_request_pattern:
            pull_request_pattern = args.pull_request_pattern
            if trigger.repositoryEventConfig is None:
                trigger.repositoryEventConfig = messages.RepositoryEventConfig()
            trigger.repositoryEventConfig.pullRequest = messages.PullRequestFilter(branch=pull_request_pattern)
        ...
        if args.comment_control:
            ...
            trigger.repositoryEventConfig.pullRequest.commentControl = messages.PullRequestFilter.CommentControlValueValuesEnum(args.comment_control)
            update_mask.append('repository_event_config.push')
    ...
    """
        ...
        update_mask = list(set(map(lambda m: sub if m.startswith(sub) else m, update_mask)))
        update_mask.sort()
