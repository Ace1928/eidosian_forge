...
class GlanceManager(base.Manager):
    ...
    def find_image(self, name_or_id):
        ...
        with self.alternate_service_type('image', allowed_types=('image',)):
            if uuidutils.is_uuid_like(name_or_id):
                ...
                matches = self._list('/v2/images?name=%s' % name_or_id, 'images')
                num_matches = len(matches)
                if num_matches == 0:
                    msg = 'No %s matching %s.' % (self.resource_class.__name__, name_or_id)
                    raise exceptions.NotFound(404, msg)
                elif num_matches > 1:
                    msg = _("Multiple %(class)s matches found for '%(name)s', use an ID to be more specific.") % {'class': self.resource_class.__name__.lower(), 'name': name_or_id}
    ...
    def find_images(self, names_or_ids):
        ...
        """
        with self.alternate_service_type('image', allowed_types=('image',)):
            matches = self._list('/v2/images?id=in:%s' % ','.join(names_or_ids), 'images')
            matches.extend(self._list('/v2/images?names=in:%s' % ','.join(names_or_ids), 'images'))
            missed = set(names_or_ids) - set((m.name for m in matches)) - set((m.id for m in matches))
            if missed:
                msg = _('Unable to find image(s): %(images)s') % {'images': ','.join(missed)}
                raise exceptions.NotFound(404, msg)
