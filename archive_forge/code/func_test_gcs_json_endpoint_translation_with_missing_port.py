...
def test_gcs_json_endpoint_translation_with_missing_port(self):
    with _mock_boto_config({'Credentials': {'gs_json_host': 'foo_host', 'json_api_version': 'v2'}}):
        flags, env_vars = self._fake_command._translate_boto_config()
        self.assertEqual(flags, [])
        self.assertEqual(env_vars, {'CLOUDSDK_API_ENDPOINT_OVERRIDES_STORAGE': 'https://foo_host/storage/v2'})