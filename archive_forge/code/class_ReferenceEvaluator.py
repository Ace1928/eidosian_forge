...
class ReferenceEvaluator:
    ...
    def __init__(self, proto: Any, opsets: Optional[Dict[str, int]]=None, functions: Optional[List[Union['ReferenceEvaluator', FunctionProto]]]=None, verbose: int=0, new_ops: Optional[List[OpRun]]=None, optimized: bool=True):
        if optimized:
            if new_ops is None:
        ...
        if isinstance(proto, ModelProto):
            ...
            self.onnx_graph_ = None
            self.opsets_ = {proto.domain: 1 if proto.domain != '' else onnx_opset_version()}
        else:
        ...
        if functions is not None:
            for f in functions:
                if isinstance(f, FunctionProto):
                    self.functions_[f.domain, f.name] = self.__class__(f, verbose=verbose, functions=list(self.functions_.values()))
        ...
        if new_ops is not None:
            for cl in new_ops:
                if not hasattr(cl, 'op_domain'):
                    raise AttributeError(f"Class {cl} must define attribute 'op_domain'.")
    ...
    def _init(self) -> None:
        ...
        for init in self.inits_:
            self.rt_inits_[init.name] = self.retrieve_external_data(init) if uses_external_data(init) else to_array_extended(init)
        run_params = {'log': lambda pattern, *args: self._log(10, pattern, *args), 'opsets': self.opsets, 'verbose': self.verbose, 'new_ops': self.new_ops_, 'existing_functions': self.functions_.copy(), 'evaluator_cls': self.__class__}
        ...
        for node in self.nodes_:
            try:
                ...
                if self.all_types_:
                    ...
                    if None in it:
                        cl = lambda *args, parent=self: OpFunctionContextDependant(*args, parent=parent)
                    else:
                        cl = self._load_impl(node, it)
                else:
                    raise RuntimeContextError(f'No implementation was found for node type {node.op_type!r} from domain {node.domain!r}. If this node has a context dependent implementation, you should run function infer_shapes before calling ReferenceEvaluator.') from e
            try:
                inst = cl(node, run_params)
            except TypeError as e:
                raise TypeError(f'Unable to instantiate class {cl!r} with run_params={run_params} and node={node}.') from e
    ...
    def _load_impl(self, node: NodeProto, input_types: Optional[TypeProto]=None) -> Any:
        ...
        if node.domain not in self.opsets:
            raise RuntimeError(f'Domain {node.domain!r} (node type: {node.op_type!r}) is not specified. Known opsets: {self.opsets!r}.')
        ...
        if node.domain == '':
            from onnx.reference.ops import load_op
            try:
                return load_op(node.domain, node.op_type, version, expand=expand, evaluator_cls=self.__class__)
            except RuntimeContextError:
                if input_types is None:
                    raise
                return load_op(node.domain, node.op_type, version, node=node, input_types=input_types, expand=expand, evaluator_cls=self.__class__)
        if expand:
            raise NotImplementedError(f'Expanding an operator with its function definition is only implemented for the main opset. Remove operator {node.domain},{node.op_type} from the list of inlined operator.')
        if node.domain == 'ai.onnx.preview.training':
            from onnx.reference.ops.aionnx_preview_training import load_op as load_op_pt
            return load_op_pt(node.domain, node.op_type, version, evaluator_cls=self.__class__)
        if node.domain == 'experimental':
            from onnx.reference.ops.experimental import load_op as load_op_exp
            return load_op_exp(node.domain, node.op_type, version, evaluator_cls=self.__class__)
        if node.domain == 'ai.onnx.ml':
            from onnx.reference.ops.aionnxml import load_op as load_op_ml
            return load_op_ml(node.domain, node.op_type, version, evaluator_cls=self.__class__)
        if key in self.functions_:
            from onnx.reference.ops import load_op
            impl = self.functions_[key]
            return load_op(node.domain, node.op_type, version, custom=impl, evaluator_cls=self.__class__)
        raise NotImplementedError(f'Node type {node.op_type!r} from domain {node.domain!r} is unknown, known functions: {sorted(self.functions_)}.')

    def run(self, output_names, feed_inputs: Dict[str, Any], attributes: Optional[Dict[str, Any]]=None):
        ...
        for node in self.rt_nodes_:
            ...
            for i in node.input:
                if i not in results:
                    raise RuntimeError(f'Unable to find input {i!r} in known results {sorted(results)}, self.rt_inits_ has {sorted(self.rt_inits_)}, feed_inputs has {sorted(feed_inputs)}.')
        ...
        for name in output_names:
            if name not in results:
                raise RuntimeError(f'Unable to find output name {name!r} in {sorted(results)}, proto is\n{self.proto_}')
