...
class MakoPlugin(CacheImpl):
    ...
    def __init__(self, cache):
        ...
        try:
            ...
            raise KeyError("'cache_regions' argument is required on the Mako Lookup or Template object for usage with the dogpile.cache plugin.")

    def _get_region(self, **kw):
        try:
            region = kw['region']
        except KeyError:
            raise KeyError("'cache_region' argument must be specified with 'cache=True'within templates for usage with the dogpile.cache plugin.")
        try:
