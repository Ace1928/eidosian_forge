...
class _BareMetalAdminClusterClient(client.ClientBase):
    ...
    def _annotations(self, args: parser_extensions.Namespace):
        ...
        for key, value in annotations.items():
            additional_property_messages.append(messages.BareMetalAdminCluster.AnnotationsValue.AdditionalProperty(key=key, value=value))
        return messages.BareMetalAdminCluster.AnnotationsValue(additionalProperties=additional_property_messages)
    ...
    def _network_config(self, args: parser_extensions.Namespace):
        ...
        kwargs = {'islandModeCidr': self._island_mode_cidr_config(args)}
        if any(kwargs.values()):
    ...
    def _vip_config(self, args: parser_extensions.Namespace):
        ...
        kwargs = {'controlPlaneVip': getattr(args, 'control_plane_vip', None)}
        if any(kwargs.values()):
    ...
    def _load_balancer_config(self, args: parser_extensions.Namespace):
        ...
        kwargs = {'manualLbConfig': self._manual_lb_config(args), 'portConfig': self._port_config(args), 'vipConfig': self._vip_config(args)}
        if any(kwargs.values()):
    ...
    def _node_config(self, node_config_args):
        """Constructs proto message BareMetalNodeConfig."""
        kwargs = {'nodeIp': node_config_args.get('node-ip', ''), 'labels': self._parse_node_labels(node_config_args)}
        if any(kwargs.values()):
            return messages.BareMetalNodeConfig(**kwargs)
        return None
    ...
    def _workload_node_config(self, args: parser_extensions.Namespace):
        ...
        kwargs = {'maxPodsPerNode': getattr(args, 'max_pods_per_node', None)}
        if any(kwargs.values()):
    ...
    def _bare_metal_admin_cluster(self, args: parser_extensions.Namespace):
        ...
        kwargs = {'name': self._admin_cluster_name(args), 'description': getattr(args, 'description', None), 'annotations': self._annotations(args), 'bareMetalVersion': getattr(args, 'version', None), 'networkConfig': self._network_config(args), 'controlPlane': self._control_plane_config(args), 'loadBalancer': self._load_balancer_config(args), 'storage': self._storage_config(args), 'proxy': self._proxy_config(args), 'clusterOperations': self._cluster_operations_config(args), 'maintenanceConfig': self._maintenance_config(args), 'nodeConfig': self._workload_node_config(args), 'nodeAccessConfig': self._node_access_config(args), 'securityConfig': self._security_config(args), 'binaryAuthorization': self._binary_authorization(args)}
        if any(kwargs.values()):
