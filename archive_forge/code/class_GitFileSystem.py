...
class GitFileSystem(AbstractFileSystem):
        ...
        if isinstance(tree, pygit2.Tree):
            ...
            for obj in tree:
                if isinstance(obj, pygit2.Tree):
                    out.append({'type': 'directory', 'name': '/'.join([path, obj.name]).lstrip('/'), 'hex': obj.hex, 'mode': f'{obj.filemode:o}', 'size': 0})
                else:
                    out.append({'type': 'file', 'name': '/'.join([path, obj.name]).lstrip('/'), 'hex': obj.hex, 'mode': f'{obj.filemode:o}', 'size': obj.size})
        else:
            obj = tree
            out = [{'type': 'file', 'name': obj.name, 'hex': obj.hex, 'mode': f'{obj.filemode:o}', 'size': obj.size}]
        if detail:
            return out
    ...
    def _open(self, path, mode='rb', block_size=None, autocommit=True, cache_options=None, ref=None, **kwargs):
