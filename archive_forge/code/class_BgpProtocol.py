...
class BgpProtocol(Protocol, Activity):
    ...
    """
    MESSAGE_MARKER = b'\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff'
    ...
    def __init__(self, socket, signal_bus, is_reactive_conn=False):
        ...
        activity_name = 'BgpProtocol %s, %s, %s' % (is_reactive_conn, self._remotename, self._localname)
    ...
    def is_colliding(self, other_protocol):
        if not isinstance(other_protocol, BgpProtocol):
            raise ValueError('Currently only support comparing with `BgpProtocol`')
        if self._remotename[0] == other_protocol._remotename[0] and self._localname[0] == other_protocol._localname[0]:
            return True
        return False
    ...
    def _check_route_fmly_adv(self, open_msg, route_family):
        ...
        for cap in local_caps:
            if cap.cap_code == BGP_CAP_MULTIPROTOCOL:
                if route_family.afi == cap.afi and route_family.safi == cap.safi:
                    match_found = True
        return match_found
    ...
    def is_mbgp_cap_valid(self, route_family):
        ...
        """
        return self.is_route_family_adv(route_family) and self.is_route_family_adv_recv(route_family)
    ...
    def is_four_octet_as_number_cap_valid(self):
        ...
        return self.cap_four_octet_as_number and self._peer.cap_four_octet_as_number
    ...
    def data_received(self, next_bytes):
        try:
            ...
            LOG.error('BGPExc Exception while receiving data: %s \n Traceback %s \n' % (str(exc), traceback.format_exc()))
            if exc.SEND_ERROR:
    ...
    def send_notification(self, code, subcode):
        ...
        if len(self._localname):
            LOG.error('Sent notification to %r >> %s', self._localname, notification)
        self._socket.close()
    ...
    def send(self, msg):
        if not self.started:
            raise BgpProtocolException('Tried to send message to peer when this protocol instance is not started or is no longer is started state.')
        ...
        *Notification* message we close connection with peer.
        ...
        if msg.type == BGP_MSG_OPEN:
            if self.state == BGP_FSM_OPEN_SENT:
                ...
                if msg.hold_time == 0:
                    LOG.info('The Hold Time sent by the peer is zero, hence not setting any Hold Time and Keep Alive timers.')
                else:
                    self._start_timers(msg.hold_time)
                    self._send_keepalive()
                return
            else:
                LOG.error('Open message received when current state is not OpenSent')
            ...
            LOG.error('Received notification message, hence closing connection %s', msg)
            self._socket.close()
            return
        if msg.type == BGP_MSG_KEEPALIVE or msg.type == BGP_MSG_UPDATE:
            if self._expiry:
                self._expiry.reset()
        if msg.type in (BGP_MSG_UPDATE, BGP_MSG_KEEPALIVE, BGP_MSG_ROUTE_REFRESH):
