...
class CarRacing(gym.Env, EzPickle):
    ...
    """
    metadata = {'render_modes': ['human', 'rgb_array', 'state_pixels'], 'render_fps': FPS}

    def __init__(self, render_mode: Optional[str]=None, verbose: bool=False, lap_complete_percent: float=0.95, domain_randomize: bool=False, continuous: bool=True):
        EzPickle.__init__(self, render_mode, verbose, lap_complete_percent, domain_randomize, continuous)
    ...
    def _create_track(self):
        ...
        well_glued_together = np.sqrt(np.square(first_perp_x * (track[0][2] - track[-1][2])) + np.square(first_perp_y * (track[0][3] - track[-1][3])))
    ...
    def reset(self, *, seed: Optional[int]=None, options: Optional[dict]=None):
        ...
        while True:
            ...
            if success:
                break
            if self.verbose:
                print('retry to generate track (normal if there are not manyinstances of this message)')
        ...
        if action is not None:
            if self.continuous:
                ...
                if not self.action_space.contains(action):
                    raise InvalidAction(f'you passed the invalid action `{action}`. The supported action_space is `{self.action_space}`')
