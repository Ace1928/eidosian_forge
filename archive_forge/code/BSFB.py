...
class ModelShowcase(ShowBase):
    ...
    def setup_lights(self):
        """
        Configure and position various types of lights within the scene to enhance the visual appearance of the 3D models.
        This method meticulously sets up point, ambient, and directional lights, each contributing uniquely to the scene's illumination.
        """
        # Configuration and instantiation of a Point Light
        point_light = PointLight("point_light")
        point_light.setColor(np.array([1.0, 1.0, 1.0, 1.0], dtype=np.float32))
        point_light_node_path = self.render.attachNewNode(point_light)
        point_light_node_path.setPos(np.array([10, 20, 0], dtype=np.float32))
        self.render.setLight(point_light_node_path)

        # Configuration and instantiation of an Ambient Light
        ambient_light = AmbientLight("ambient_light")
        ambient_light.setColor(np.array([0.5, 0.5, 0.5, 1.0], dtype=np.float32))
        ambient_light_node_path = self.render.attachNewNode(ambient_light)
        self.render.setLight(ambient_light_node_path)

        # Configuration and instantiation of a Directional Light
        directional_light = DirectionalLight("directional_light")
        directional_light.setColor(np.array([0.8, 0.8, 0.8, 1.0], dtype=np.float32))
        directional_light_node_path = self.render.attachNewNode(directional_light)
        directional_light_node_path.setHpr(np.array([0, -60, 0], dtype=np.float32))
