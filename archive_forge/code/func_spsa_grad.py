...
@partial(transform, expand_transform=_expand_transform_spsa, classical_cotransform=_contract_qjac_with_cjac, final_transform=True)
def spsa_grad(tape: qml.tape.QuantumTape, argnum=None, h=1e-05, approx_order=2, n=1, strategy='center', f0=None, validate_params=True, num_directions=1, sampler=_rademacher_sampler, sampler_rng=None) -> (Sequence[qml.tape.QuantumTape], Callable):
    ...
    **Example**
    ...
    diff_methods = find_and_validate_gradient_methods(tape, 'numeric', trainable_params) if validate_params else {idx: 'F' for idx in trainable_params}
    ...
    indices = [i for i in range(num_trainable_params) if i in diff_methods and diff_methods[i] != '0']
    ...
    for idx_rep in range(num_directions):
        ...
        inv_direction = qml.math.divide(1, direction, where=list(direction != 0), out=qml.math.zeros_like(direction))
    ...
    def _single_shot_batch_result(results):
        ...
        return tuple(grads)
    processing_fn = partial(_processing_fn, shots=tape.shots, single_shot_batch_fn=_single_shot_batch_result)
