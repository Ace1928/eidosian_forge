...
class STSSocketmapResponder:
    ...
    def __init__(self, cfg, loop, cache):
        ...
        self._default_zone = ZoneEntry(cfg['default_zone']['strict_testing'], STSResolver(loop=loop, timeout=cfg['default_zone']['timeout']), cfg['default_zone']['require_sni'])
        self._zones = dict(((k, ZoneEntry(zone['strict_testing'], STSResolver(loop=loop, timeout=zone['timeout']), zone['require_sni'])) for k, zone in cfg['zones'].items()))
        self._cache = cache
        self._children = set()
        self._server = None
        ...
        if self._unix:
            ...
            if self._reuse_port:
                if sys.platform in ('win32', 'cygwin'):
                    opts = {'host': self._host, 'port': self._port, 'reuse_address': True}
                elif os.name == 'posix':
                    if sys.platform.startswith('freebsd'):
                        sockopts = [(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1), (socket.SOL_SOCKET, 65536, 1)]
                        sock = await create_custom_socket(self._host, self._port, options=sockopts)
                        opts = {'sock': sock}
                    else:
                        opts = {'host': self._host, 'port': self._port, 'reuse_address': True, 'reuse_port': True}
    ...
    async def stop(self):
        ...
        while True:
            self._logger.warning('Awaiting %d client handlers to finish...', len(self._children))
            ...
            try:
                ...
                self._logger.warning('Shutdown timeout expired. Remaining handlers terminated.')
                try:
                    await remaining
    ...
    async def process_request(self, raw_req):
        ...
        if self.is_stale(cached):
            ...
            if status is STSFetchResult.NOT_CHANGED:
                ...
                have_policy = False
            elif cached.pol_body['max_age'] + cached.ts < ts:
                have_policy = False
        else:
