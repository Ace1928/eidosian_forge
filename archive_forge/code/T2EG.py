...
Module Structure:
        indedatabase.py - KeyDB ASynchronous Sparse Typed TTL Cache System and Database Config/Setup/Management and Session Management:
            - KeyDB Configuration:
        ...
        indeutils.py - Data handling logic
            - Data Input:
                input_validate - Validation - Validate The Input Data for Type, Syntax, Expected Values, and Expected Data Types and all requirements, packages, modules, classes, functions, methods, objects, arguments, keyword arguments, logic, sequence, operations, intermediate products and outputs. Maximum possible verbosity for anything decorated and all context etc. captured (providing not captured prior).
                    input_inspect - Create unique specific generic identifiers for all information and keep all actual types/values/parameters verbatim and all other information (metadata) to store in the metadata cache/file.
                create_hash - Hashing - Create a unique hash using all of the metadata to verify integrity and uniqueness and ensure no redundancy or duplication.
                categorize - Categorization - Split the data into its appropriate categories for the different caches.
                encode - Encoding - Reversibly encode the data to convert it into a bytes format for storage.
            indebitmatrix.py - Bit Matrix Standardisation - Convert from bytes into bit_matrix for data format standardisation.
                compress - Compression - Compress the data using the most efficient compression algorithm available.
                encrypt - Encryption - encrypt the data using the most secure encryption algorithm available.
            ...
            indecache.py - Storage (Cache Logic) - Retrieve the data from the appropriate cache, if not in cache load from blob into cache and then retrieve.
                decrypt - Decryption - Decrypt the data using the most secure decryption algorithm available.
                decompress - Decompression - Decompress the data using the most efficient decompression algorithm available.
            indebitmatrix.py - Bit Matrix Standardisation - Convert from bit_matrix into bytes for data format standardisation.
                decode - decoding - Reversibly decode the data to convert it back into its original form.
                consolidate - Category Consolidation - Consolidate the data from its different categories.
                check_hash - Hash Verification - Verify the hash to ensure integrity and uniqueness.
                retrieval_validate - Validation - Validate the data against the expected schema to ensure integrity and adherence to the expected structure.
