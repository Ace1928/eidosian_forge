...
class DefaultQubitLegacy(QubitDevice):
    ...
    version = __version__
    author = 'Xanadu Inc.'
    operations = {'GlobalPhase', 'Identity', 'Snapshot', 'BasisState', 'StatePrep', 'QubitStateVector', 'QubitUnitary', 'ControlledQubitUnitary', 'BlockEncode', 'MultiControlledX', 'IntegerComparator', 'DiagonalQubitUnitary', 'PauliX', 'X', 'PauliY', 'Y', 'PauliZ', 'Z', 'MultiRZ', 'Hadamard', 'S', 'Adjoint(S)', 'T', 'Adjoint(T)', 'SX', 'Adjoint(SX)', 'CNOT', 'SWAP', 'ISWAP', 'PSWAP', 'Adjoint(ISWAP)', 'SISWAP', 'Adjoint(SISWAP)', 'SQISW', 'CSWAP', 'Toffoli', 'CCZ', 'CY', 'CZ', 'CH', 'PhaseShift', 'PCPhase', 'ControlledPhaseShift', 'CPhaseShift00', 'CPhaseShift01', 'CPhaseShift10', 'CPhase', 'RX', 'RY', 'RZ', 'Rot', 'CRX', 'CRY', 'CRZ', 'CRot', 'IsingXX', 'IsingYY', 'IsingZZ', 'IsingXY', 'SingleExcitation', 'SingleExcitationPlus', 'SingleExcitationMinus', 'DoubleExcitation', 'DoubleExcitationPlus', 'DoubleExcitationMinus', 'QubitCarry', 'QubitSum', 'OrbitalRotation', 'QFT', 'ECR'}
    observables = {'PauliX', 'X', 'PauliY', 'Y', 'PauliZ', 'Z', 'Hadamard', 'Hermitian', 'Identity', 'Projector', 'SparseHamiltonian', 'Hamiltonian', 'Sum', 'SProd', 'Prod', 'Exp', 'Evolution'}
    ...
    def __init__(self, wires, *, r_dtype=np.float64, c_dtype=np.complex128, shots=None, analytic=None):
        ...
        self._apply_ops = {'PauliX': self._apply_x, 'PauliY': self._apply_y, 'PauliZ': self._apply_z, 'Hadamard': self._apply_hadamard, 'S': self._apply_s, 'T': self._apply_t, 'SX': self._apply_sx, 'CNOT': self._apply_cnot, 'SWAP': self._apply_swap, 'CZ': self._apply_cz, 'Toffoli': self._apply_toffoli}
    ...
    def _apply_parametrized_evolution(self, state: TensorLike, operation: ParametrizedEvolution):
        ...
        """
        raise NotImplementedError(f'The device {self.short_name} cannot execute a ParametrizedEvolution operation. Please use the jax interface.')
    ...
    def _apply_operation(self, state, operation):
        ...
        if str(operation.name) in self._apply_ops:
    ...
    def expval(self, observable, shot_range=None, bin_size=None):
        ...
        if isinstance(observable, Sum) and (not self.shots):
            return measure(ExpectationMP(observable.map_wires(self.wire_map)), self._pre_rotated_state, is_state_batched)
        ...
        backprop_mode = (not isinstance(self.state, np.ndarray) or any((not isinstance(d, (float, np.ndarray)) for d in observable.data))) and observable.name == 'Hamiltonian'
        if backprop_mode:
            if is_state_batched:
                raise NotImplementedError('Expectation values of Hamiltonians for interface!=None are not supported together with parameter broadcasting yet')
            ...
            for op, coeff in zip(observable.ops, observable.data):
                ...
                product = self._gather(self._conj(self.state), coo.row) * Hmat * self._gather(self.state, coo.col)
            ...
            if is_state_batched:
                res = qml.math.array([csr_matrix.dot(csr_matrix(self._conj(_state)), csr_matrix.dot(Hmat, csr_matrix(_state[..., None]))).toarray()[0] for _state in state])
            else:
                res = csr_matrix.dot(csr_matrix(self._conj(state)), csr_matrix.dot(Hmat, csr_matrix(state[..., None]))).toarray()[0]
    ...
    def capabilities(cls):
        ...
        capabilities.update(model='qubit', supports_inverse_operations=True, supports_analytic_computation=True, supports_broadcasting=True, returns_state=True, passthru_devices={'tf': 'default.qubit.tf', 'torch': 'default.qubit.torch', 'autograd': 'default.qubit.autograd', 'jax': 'default.qubit.jax'})
        return capabilities
    ...
    def _apply_unitary(self, state, mat, wires):
        ...
        if mat_batch_size:
            ...
            if state_batch_size:
                raise NotImplementedError(f'Applying a broadcasted unitary to an already broadcasted state via _apply_unitary is not supported. Broadcasting sizes are ({mat_batch_size}, {state_batch_size}).')
        if state_batch_size:
    ...
    def _apply_unitary_einsum(self, state, mat, wires):
        ...
        new_state_indices = functools.reduce(lambda old_string, idx_pair: old_string.replace(idx_pair[0], idx_pair[1]), zip(affected_indices, new_indices), state_indices)
    ...
    def analytic_probability(self, wires=None):
        ...
        flat_state = self._reshape(self._state, (batch_size, dim) if batch_size is not None else (dim,))
    ...
    def _get_diagonalizing_gates(self, circuit: qml.tape.QuantumTape) -> List[Operation]:
        meas_filtered = [m for m in circuit.measurements if m.obs is None or not isinstance(m.obs, qml.Hamiltonian)]
