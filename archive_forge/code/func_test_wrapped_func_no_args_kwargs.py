...
def test_wrapped_func_no_args_kwargs():
    ...
    def f(x, y, **kwargs):
        assert not any((isinstance(value, uncert_core.UFloat) for value in [x, y] + list(kwargs.values())))
    ...
    assert ufloats_close(f_auto_unc(x, y, **kwargs), f_wrapped(x, y, **kwargs))
    assert ufloats_close(f_auto_unc(y=y, x=x, **kwargs), f_wrapped(y=y, x=x, **kwargs))
    f_wrapped = uncert_core.wrap(f, [lambda x, y, **kwargs: 2, lambda x, y, **kwargs: math.cos(y)], {'z:': lambda x, y, **kwargs: 3})
    assert ufloats_close(f_auto_unc(x, y, **kwargs), f_wrapped(x, y, **kwargs))
    assert ufloats_close(f_auto_unc(y=y, x=x, **kwargs), f_wrapped(y=y, x=x, **kwargs))
