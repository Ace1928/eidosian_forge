...
class Test_dual_power(_conic_tester_base, unittest.TestCase):
    _object_factory = lambda self: dual_power(r1=variable(lb=0), r2=variable(lb=0), x=[variable(), variable()], alpha=parameter(value=0.4))
    ...
    def test_expression(self):
        ...
        c.x[0].value = 1.2
        c.x[1].value = -5.3
        val = round((1.2 ** 2 + (-5.3) ** 2) ** 0.5 - (2.7 / 0.4) ** 0.4 * (3.7 / 0.6) ** 0.6, 9)
