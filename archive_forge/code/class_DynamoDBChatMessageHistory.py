...
class DynamoDBChatMessageHistory(BaseChatMessageHistory):
    ...
    def __init__(self, table_name: str, session_id: str, endpoint_url: Optional[str]=None, primary_key_name: str='SessionId', key: Optional[Dict[str, str]]=None, boto3_session: Optional[Session]=None, kms_key_id: Optional[str]=None, ttl: Optional[int]=None, ttl_key_name: str='expireAt', history_size: Optional[int]=None):
        if boto3_session:
        ...
        if kms_key_id:
            try:
                ...
                raise ImportError('Unable to import dynamodb_encryption_sdk, please install with `pip install dynamodb-encryption-sdk`.') from e
            actions = AttributeActions(default_action=CryptoAction.DO_NOTHING, attribute_actions={'History': CryptoAction.ENCRYPT_AND_SIGN})
            aws_kms_cmp = AwsKmsCryptographicMaterialsProvider(key_id=kms_key_id)
            self.table = EncryptedTable(table=self.table, materials_provider=aws_kms_cmp, attribute_actions=actions, auto_refresh_table_indexes=False)
    ...
    def messages(self) -> List[BaseMessage]:
        ...
        try:
            ...
            raise ImportError('Unable to import botocore, please install with `pip install botocore`.') from e
        response = None
        try:
    ...
    def add_message(self, message: BaseMessage) -> None:
        ...
        try:
            if self.ttl:
                ...
                self.table.put_item(Item={**self.key, 'History': messages, self.ttl_key_name: expireAt})
            else:
