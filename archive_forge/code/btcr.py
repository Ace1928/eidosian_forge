...
FieldDefinition = Tuple[
    Type[Any], Union[Any, Tuple[()], None]
]  # Defines the structure of a single field definition
FieldsDictType = Dict[str, FieldDefinition]  # Maps field names to their definitions
ModelType = TypeVar(
...
class UniversalModel(BaseModel):
    ...
    @classmethod
    def create_dynamic_model(cls, name: str, fields: FieldsDictType):
        """
        ...
        existing_fields = set(cls.__fields__.keys())
        ...
        # Prepare fields correctly for Pydantic's create_model function
        model_fields = {
            field: (
        ...
        new_model = create_model(
            name, __base__=cls, **model_fields
        )  # Dynamically creating the model
        return new_model
