...
class SQLiteSavepointTest(db_test_base._DbTestCase):
    ...
    def setUp(self):
        ...
        meta = MetaData()
        self.test_table = Table('test_table', meta, Column('id', Integer, primary_key=True), Column('data', String(10)))
    ...
    def test_plain_transaction(self):
        ...
        self.assertEqual([(1, 'data 1')], conn.execute(self.test_table.select().order_by(self.test_table.c.id)).fetchall())
        trans.rollback()
        with self.engine.connect() as conn:
            self.assertEqual(0, conn.scalar(sqlalchemy.select(sqlalchemy.func.count(self.test_table.c.id)).select_from(self.test_table)))
