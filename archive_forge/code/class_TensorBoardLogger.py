...
class TensorBoardLogger(Logger):
    ...
    def __init__(self, root_dir: _PATH, name: Optional[str]='lightning_logs', version: Optional[Union[int, str]]=None, default_hp_metric: bool=True, prefix: str='', sub_dir: Optional[_PATH]=None, **kwargs: Any):
        if not _TENSORBOARD_AVAILABLE and (not _TENSORBOARDX_AVAILABLE):
            raise ModuleNotFoundError(f'Neither `tensorboard` nor `tensorboardX` is available. Try `pip install`ing either.\n{str(_TENSORBOARDX_AVAILABLE)}\n{str(_TENSORBOARD_AVAILABLE)}')
        super().__init__()
        ...
        for k, v in metrics.items():
            ...
            if isinstance(v, dict):
                ...
                try:
                    ...
                    raise ValueError(f'\n you tried to log {v} which is currently not supported. Try a dict or a scalar/tensor.') from ex
    ...
    def log_graph(self, model: Module, input_array: Optional[Tensor]=None) -> None:
        ...
        if input_array is None:
            rank_zero_warn('Could not log computational graph to TensorBoard: The `model.example_input_array` attribute is not set or `input_array` was not given.')
        elif not isinstance(input_array, (Tensor, tuple)):
            rank_zero_warn(f"Could not log computational graph to TensorBoard: The `input_array` or `model.example_input_array` has type {type(input_array)} which can't be traced by TensorBoard. Make the input array a tuple representing the positional arguments to the model's `forward()` implementation.")
        elif callable(getattr(model, '_on_before_batch_transfer', None)) and callable(getattr(model, '_apply_batch_transfer_handler', None)):
