...
def is_potential_secret_option(option_name):
    if not NO_LOG_REGEX.search(option_name):
        return False
    if option_name.endswith(('_count', '_type', '_alg', '_algorithm', '_timeout', '_name', '_comment', '_bits', '_id', '_identifier', '_period', '_file', '_filename')):
        return False
    if any((part in option_name for part in ('publickey', 'public_key', 'keyusage', 'key_usage', 'keyserver', 'key_server', 'keysize', 'key_size', 'keyservice', 'key_service', 'pub_key', 'pubkey', 'keyboard', 'secretary'))):
        return False
    return True