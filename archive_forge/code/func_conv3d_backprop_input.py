...
def conv3d_backprop_input(input: _atypes.TensorFuzzingAnnotation[TV_Conv3DBackpropInput_T], filter: _atypes.TensorFuzzingAnnotation[TV_Conv3DBackpropInput_T], out_backprop: _atypes.TensorFuzzingAnnotation[TV_Conv3DBackpropInput_T], strides, padding: str, dilations=[1, 1, 1, 1, 1], name=None) -> _atypes.TensorFuzzingAnnotation[TV_Conv3DBackpropInput_T]:
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'Conv3DBackpropInput', name, input, filter, out_backprop, 'strides', strides, 'padding', padding, 'dilations', dilations)
            return _result
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('Conv3DBackpropInput', input=input, filter=filter, out_backprop=out_backprop, strides=strides, padding=padding, dilations=dilations, name=name)
    _result = _outputs[:]
