...
class WBArtifactHandler(StorageHandler):
    ...
    def load_path(self, manifest_entry: ArtifactManifestEntry, local: bool=False) -> Union[URIStr, FilePathStr]:
        ...
        dep_artifact = wandb.Artifact._from_id(hex_to_b64_id(artifact_id), self.client.client)
    ...
    def store_path(self, artifact: 'Artifact', path: Union[URIStr, FilePathStr], name: Optional[StrPath]=None, checksum: bool=True, max_objects: Optional[int]=None) -> Sequence[ArtifactManifestEntry]:
        ...
        while iter_path is not None and urlparse(iter_path).scheme == self._scheme:
            ...
            target_artifact = wandb.Artifact._from_id(hex_to_b64_id(artifact_id), self.client.client)
        ...
        path = URIStr('{}://{}/{}'.format(self._scheme, b64_to_hex_id(B64MD5(target_artifact.id)), artifact_file_path))
        assert entry is not None
