...
class GraphEstimator(object, metaclass=abc.ABCMeta):
    ...
    def graph(self, G):
        ...
        if G is None:
            self._reset_graph()
    n_pca = attribute('n_pca', default=100, on_set=partial(utils.check_if_not, None, utils.check_positive, utils.check_int))
    ...
    distance = attribute('distance', default='euclidean', on_set=partial(utils.check_in, ['euclidean', 'precomputed', 'cosine', 'correlation', 'cityblock', 'l1', 'l2', 'manhattan', 'braycurtis', 'canberra', 'chebyshev', 'dice', 'hamming', 'jaccard', 'kulsinski', 'mahalanobis', 'matching', 'minkowski', 'rogerstanimoto', 'russellrao', 'seuclidean', 'sokalmichener', 'sokalsneath', 'sqeuclidean', 'yule', 'precomputed_affinity', 'precomputed_distance']))
    n_svd = attribute('n_svd', default=100, on_set=partial(utils.check_if_not, None, utils.check_positive, utils.check_int))
    n_jobs = attribute('n_jobs', on_set=partial(utils.check_if_not, None, utils.check_int))
    verbose = attribute('verbose', default=0)
    thresh = attribute('thresh', default=0.0001, on_set=partial(utils.check_if_not, 0, utils.check_positive))
    ...
    def __init__(self, knn=5, decay=40, n_pca=100, n_landmark=None, random_state=None, distance='euclidean', n_svd=100, n_jobs=1, verbose=1, thresh=0.0001, **kwargs):
        if verbose is True:
            verbose = 1
    ...
    def fit(self, X, **kwargs):
        ...
        if precomputed is None:
            _logger.log_info('Building graph on {} samples and {} features.'.format(X.shape[0], X.shape[1]))
        else:
            _logger.log_info('Building graph on precomputed {} matrix with {} samples.'.format(precomputed, X.shape[0]))
        ...
        if self.graph is None:
            with _logger.log_task('graph and diffusion operator'):
                self.graph = api.Graph(X, n_pca=n_pca, precomputed=precomputed, n_landmark=n_landmark, random_state=self.random_state, knn=self.knn, decay=self.decay, distance=self.distance, n_svd=self._parse_n_svd(self.X, self.n_svd), n_jobs=self.n_jobs, thresh=self.thresh, verbose=self.verbose, **self.kwargs, **kwargs)
        return self