...
class OneCommonTeacher(FixedDialogTeacher):
    ...
    def __init__(self, opt, shared=None):
        ...
        build(opt)
        data_path = os.path.join(opt['datapath'], 'onecommon', 'onecommon-1.0', 'aaai2020', 'experiments', 'data', 'onecommon', self.datatype + '_reference_0.txt')
    ...
    def observe(self, observation):
        if 'metrics' not in observation:
            observation['metrics'] = {}
        if 'output' in observation and observation['output'] is not None and ('output' in self.expected) and (self.expected['output'] is not None):
        ...
        if 'referents' in observation and observation['referents'] is not None and (len(observation['referents']) > 0) and ('referents' in self.expected) and (self.expected['referents'] is not None) and (len(self.expected['referents']) > 0):
    ...
    def _setup_data(self, data_path):
        ...
        for data in raw_data:
            ...
            self.episodes.append({'context': context, 'dialogue': dialogue, 'referents': referents, 'output': output})
