...
class AbstractGetTests:
    ...
    def test_get_file_to_file_in_new_directory(self, fs, fs_join, fs_bulk_operations_scenario_0, local_fs, local_join, local_target):
        source = fs_bulk_operations_scenario_0
        target = local_target
        local_fs.mkdir(target)
        fs.get(fs_join(source, 'subdir', 'subfile1'), local_join(target, 'newdir', 'newfile'))
        assert local_fs.isdir(local_join(target, 'newdir'))
