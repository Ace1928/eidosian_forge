...
class CrossEncoderRankerAgent(TorchRankerAgent):
    ...
    def build_model(self):
        return BertWrapper(BertModel.from_pretrained(self.pretrained_path), 1, add_transformer_layer=self.opt['add_transformer_layer'], layer_pulled=self.opt['pull_from_layer'], aggregation=self.opt['bert_aggregation'])

    def init_optim(self, params, optim_states=None, saved_optim_type=None):
        self.optimizer = get_bert_optimizer([self.model], self.opt['type_optimization'], self.opt['learningrate'], fp16=self.opt.get('fp16'))
    ...
    def score_candidates(self, batch, cand_vecs, cand_encs=None):
        ...
        all_tokens, all_segments = concat_without_padding(tokens_context, tokens_cands, self.use_cuda, self.NULL_IDX)
    ...
    def _set_text_vec(self, *args, **kwargs):
        ...
        if obs is not None and 'text_vec' in obs and ('added_start_end_tokens' not in obs):
            obs.force_set('text_vec', surround(obs['text_vec'], self.START_IDX, self.END_IDX))
