...
def test_preconlbfgs():
    ...
    assert np.abs(np.array(energies) - -0.022726045433998365).max() < 0.0001
    cu0 = bulk('Cu') * (2, 2, 2)
    cu0.rattle(0.01)
    ...
    for precon in [None, Exp(mu=1.0)]:
        ...
        opt.run(fmax=0.001)
        assert abs(cu.get_distance(0, 1) / a0 - 1.2) < 0.001
        assert np.all(abs(cu.positions[2] - cu0.positions[2]) < 0.001)
        assert np.all(abs(cu.positions[3] - cu0.positions[3]) < 0.001)