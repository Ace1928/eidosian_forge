...
class DeepLake(VectorStore):
    ...
    def __init__(self, dataset_path: str=_LANGCHAIN_DEFAULT_DEEPLAKE_PATH, token: Optional[str]=None, embedding: Optional[Embeddings]=None, embedding_function: Optional[Embeddings]=None, read_only: bool=False, ingestion_batch_size: int=1024, num_workers: int=0, verbose: bool=True, exec_option: Optional[str]=None, runtime: Optional[Dict]=None, index_params: Optional[Dict[str, Union[int, str]]]=None, **kwargs: Any) -> None:
        ...
        if _DEEPLAKE_INSTALLED is False:
            raise ImportError('Could not import deeplake python package. Please install it with `pip install deeplake[enterprise]`.')
        if runtime == {'tensor_db': True} and version_compare(deeplake.__version__, '3.6.7') == -1:
            raise ImportError(f'To use tensor_db option you need to update deeplake to `3.6.7` or higher. Currently installed deeplake version is {deeplake.__version__}. ')
        self.dataset_path = dataset_path
        if embedding_function:
            logger.warning('Using embedding function is deprecated and will be removed in the future. Please use embedding instead.')
        self.vectorstore = DeepLakeVectorStore(path=self.dataset_path, embedding_function=embedding_function or embedding, read_only=read_only, token=token, exec_option=exec_option, verbose=verbose, runtime=runtime, index_params=index_params, **kwargs)
    ...
    def add_texts(self, texts: Iterable[str], metadatas: Optional[List[dict]]=None, ids: Optional[List[str]]=None, **kwargs: Any) -> List[str]:
        ...
        try:
            return self.vectorstore.add(text=texts, metadata=metadatas, embedding_data=texts, embedding_tensor='embedding', embedding_function=self._embedding_function.embed_documents, return_ids=True, **kwargs)
        except SampleExtendError as e:
            if "Failed to append a sample to the tensor 'metadata'" in str(e):
                msg = "**Hint: You might be using invalid type of argument in document loader (e.g. 'pathlib.PosixPath' instead of 'str')"
    ...
    def _search(self, query: Optional[str]=None, embedding: Optional[Union[List[float], np.ndarray]]=None, embedding_function: Optional[Callable]=None, k: int=4, distance_metric: Optional[str]=None, use_maximal_marginal_relevance: bool=False, fetch_k: Optional[int]=20, filter: Optional[Union[Dict, Callable]]=None, return_score: bool=False, exec_option: Optional[str]=None, deep_memory: bool=False, **kwargs: Any) -> Any[List[Document], List[Tuple[Document, float]]]:
        ...
        if kwargs.get('tql'):
            return self._search_tql(tql=kwargs['tql'], exec_option=exec_option, return_score=return_score, embedding=embedding, embedding_function=embedding_function, distance_metric=distance_metric, use_maximal_marginal_relevance=use_maximal_marginal_relevance, filter=filter)
        ...
        result = self.vectorstore.search(embedding=embedding, k=fetch_k if use_maximal_marginal_relevance else k, distance_metric=distance_metric, filter=filter, exec_option=exec_option, return_tensors=['embedding', 'metadata', 'text', self._id_tensor_name], deep_memory=deep_memory)
        scores = result['score']
        ...
        if use_maximal_marginal_relevance:
            ...
            indices = maximal_marginal_relevance(embedding, embeddings, k=min(k, len(texts)), lambda_mult=lambda_mult)
    ...
    def similarity_search(self, query: str, k: int=4, **kwargs: Any) -> List[Document]:
        ...
        """
        return self._search(query=query, k=k, use_maximal_marginal_relevance=False, return_score=False, **kwargs)

    def similarity_search_by_vector(self, embedding: Union[List[float], np.ndarray], k: int=4, **kwargs: Any) -> List[Document]:
        """
        ...
        Returns:
            List[Document]: List of Documents most similar to the query vector.
        """
        return self._search(embedding=embedding, k=k, use_maximal_marginal_relevance=False, return_score=False, **kwargs)
    ...
    def max_marginal_relevance_search_by_vector(self, embedding: List[float], k: int=4, fetch_k: int=20, lambda_mult: float=0.5, exec_option: Optional[str]=None, **kwargs: Any) -> List[Document]:
        """
        ...
        """
        return self._search(embedding=embedding, k=k, fetch_k=fetch_k, use_maximal_marginal_relevance=True, lambda_mult=lambda_mult, exec_option=exec_option, **kwargs)

    def max_marginal_relevance_search(self, query: str, k: int=4, fetch_k: int=20, lambda_mult: float=0.5, exec_option: Optional[str]=None, **kwargs: Any) -> List[Document]:
        ...
        return self._search(query=query, k=k, fetch_k=fetch_k, use_maximal_marginal_relevance=True, lambda_mult=lambda_mult, exec_option=exec_option, embedding_function=embedding_function, **kwargs)

    @classmethod
    def from_texts(cls, texts: List[str], embedding: Optional[Embeddings]=None, metadatas: Optional[List[dict]]=None, ids: Optional[List[str]]=None, dataset_path: str=_LANGCHAIN_DEFAULT_DEEPLAKE_PATH, **kwargs: Any) -> DeepLake:
    ...
    def _validate_kwargs(cls, kwargs, method_name):
        if kwargs:
            ...
            if unsupported_items:
                raise TypeError(f'`{unsupported_items}` are not a valid argument to {method_name} method')
