...
def test_contour_order(glyph0, glyph1):
    ...
    if not done:
        ...
        matching_control, matching_cost_control, identity_cost_control = matching_for_vectors(m0Control, m1Control)
        done = matching_cost_control == identity_cost_control
    if not done:
        m0Green = glyph0.greenVectors
        m1Green = glyph1.greenVectors
        matching_green, matching_cost_green, identity_cost_green = matching_for_vectors(m0Green, m1Green)
        done = matching_cost_green == identity_cost_green
    if not done:
        if not done:
            m1ControlReversed = [(-m[0],) + m[1:] for m in m1Control]
            matching_control_reversed, matching_cost_control_reversed, identity_cost_control_reversed = matching_for_vectors(m0Control, m1ControlReversed)
    ...
    log.debug('test-contour-order: tolerance %g', this_tolerance)
