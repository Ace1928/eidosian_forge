...
class OSCClientTestBase(base.ClientTestBase):
    ...
    def check_object_deleted(self, object_name, object_id, timeout=CONF.build_timeout, interval=CONF.build_interval):
    ...
    def openstack(self, action, flags='', params='', fail_ok=False, merge_stderr=False, client=None):
        """Executes openstack command for given action"""
        if '--os-share-api-version' not in flags:
            flags = flags + '--os-share-api-version %s' % CONF.max_api_microversion
        if client is None:
            client = self.admin_client
        return client.openstack(action, flags=flags, params=params, fail_ok=fail_ok, merge_stderr=merge_stderr)
    ...
    def create_share(self, share_protocol=None, size=None, name=None, snapshot_id=None, properties=None, share_network=None, description=None, public=False, share_type=None, availability_zone=None, share_group=None, add_cleanup=True, client=None, wait=None, wait_for_status='available'):
        name = name or data_utils.rand_name('autotest_share_name')
        share_type = share_type or 'dhss_false'
        cmd = 'create %(protocol)s %(size)s %(name)s %(desc)s %(public)s %(stype)s' % {'protocol': share_protocol or 'NFS', 'size': size or '1', 'name': '--name %s' % name, 'desc': '--description %s' % description, 'public': '--public %s' % public, 'stype': '--share-type %s' % share_type}
        if snapshot_id:
            cmd = cmd + ' --snapshot-id %s' % snapshot_id
        if properties:
            for key, value in properties.items():
                cmd = cmd + ' --property %(key)s=%(value)s' % {'key': key, 'value': value}
        if share_network:
        ...
        self._wait_for_object_status('share', share_object['id'], wait_for_status)
        if add_cleanup:
            self.addCleanup(self.openstack, 'share delete %s --wait' % share_object['id'])
        return share_object
    ...
    def create_share_access_rule(self, share, access_type, access_to, properties=None, access_level=None, wait=False, lock_visibility=False, lock_deletion=False, lock_reason=None, add_cleanup=False):
    ...
    def create_snapshot(self, share, name=None, description=None, wait=True, force=None, add_cleanup=True, client=None):
        ...
        if add_cleanup:
            self.addCleanup(self.openstack, f'share snapshot delete {snapshot_object['id']} --wait')
    ...
    def create_share_network(self, neutron_net_id=None, neutron_subnet_id=None, name=None, description=None, availability_zone=None, add_cleanup=True):
        ...
        self._wait_for_object_status('share network', share_network_obj['id'], 'active')
        if add_cleanup:
            self.addCleanup(self.openstack, f'share network delete {share_network_obj['id']}')
    ...
    def create_resource_lock(self, resource_id, resource_type='share', resource_action='delete', lock_reason=None, add_cleanup=True, client=None):
        ...
        if add_cleanup:
            self.addCleanup(self.openstack, 'share lock delete %s' % lock['id'], client=client)
        return lock

    def create_backup(self, share_id, name=None, description=None, backup_options=None, add_cleanup=True):
        ...
        self._wait_for_object_status('share backup', backup_object['id'], 'available')
        if add_cleanup:
            self.addCleanup(self.openstack, f'share backup delete {backup_object['id']} --wait')
