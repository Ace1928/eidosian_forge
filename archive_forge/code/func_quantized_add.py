...
def quantized_add(x: _atypes.TensorFuzzingAnnotation[TV_QuantizedAdd_T1], y: _atypes.TensorFuzzingAnnotation[TV_QuantizedAdd_T2], min_x: _atypes.TensorFuzzingAnnotation[_atypes.Float32], max_x: _atypes.TensorFuzzingAnnotation[_atypes.Float32], min_y: _atypes.TensorFuzzingAnnotation[_atypes.Float32], max_y: _atypes.TensorFuzzingAnnotation[_atypes.Float32], Toutput: TV_QuantizedAdd_Toutput=_dtypes.qint32, name=None):
  ...
  """
    ...
    if tld.is_eager:
        try:
            _result = pywrap_tfe.TFE_Py_FastPathExecute(_ctx, 'QuantizedAdd', name, x, y, min_x, max_x, min_y, max_y, 'Toutput', Toutput)
    ...
    _, _, _op, _outputs = _op_def_library._apply_op_helper('QuantizedAdd', x=x, y=y, min_x=min_x, max_x=max_x, min_y=min_y, max_y=max_y, Toutput=Toutput, name=name)
    _result = _outputs[:]
