...
class VoilaTreeHandler(BaseVoilaHandler):
    ...
    async def get(self, path=''):
        ...
        if dir_exists:
            ...
            self.write(self.render_template('tree.html', page_title=page_title, notebook_path=path, breadcrumbs=breadcrumbs, contents=contents, terminals_available=False, server_root=get_server_root_dir(self.settings), query=self.request.query))
        elif file_exists:
            model = await ensure_async(cm.get(path, content=False))
            service = 'notebooks' if model['type'] == 'notebook' else 'files'
            url = url_path_join(self.base_url, service, url_escape(path))
