...
class MongoDocumentManager(RecordManager):
    ...
    def __init__(self, namespace: str, *, mongodb_url: str, db_name: str, collection_name: str='documentMetadata') -> None:
    ...
    def update(self, keys: Sequence[str], *, group_ids: Optional[Sequence[Optional[str]]]=None, time_at_least: Optional[float]=None) -> None:
        ...
        for key, group_id in zip(keys, group_ids):
            self.sync_collection.find_one_and_update({'namespace': self.namespace, 'key': key}, {'$set': {'group_id': group_id, 'updated_at': self.get_time()}}, upsert=True)
    ...
    async def aupdate(self, keys: Sequence[str], *, group_ids: Optional[Sequence[Optional[str]]]=None, time_at_least: Optional[float]=None) -> None:
        ...
        for key, group_id in zip(keys, group_ids):
            await self.async_collection.find_one_and_update({'namespace': self.namespace, 'key': key}, {'$set': {'group_id': group_id, 'updated_at': update_time}}, upsert=True)
    ...
    def exists(self, keys: Sequence[str]) -> List[bool]:
        ...
        existing_keys = {doc['key'] for doc in self.sync_collection.find({'namespace': self.namespace, 'key': {'$in': keys}}, {'key': 1})}
    ...
    def list_keys(self, *, before: Optional[float]=None, after: Optional[float]=None, group_ids: Optional[Sequence[str]]=None, limit: Optional[int]=None) -> List[str]:
        ...
        cursor = self.sync_collection.find(query, {'key': 1}).limit(limit) if limit else self.sync_collection.find(query, {'key': 1})
    ...
    async def alist_keys(self, *, before: Optional[float]=None, after: Optional[float]=None, group_ids: Optional[Sequence[str]]=None, limit: Optional[int]=None) -> List[str]:
        ...
        if group_ids:
            query['group_id'] = {'$in': group_ids}
        cursor = self.async_collection.find(query, {'key': 1}).limit(limit) if limit else self.async_collection.find(query, {'key': 1})
        return [doc['key'] async for doc in cursor]

    def delete_keys(self, keys: Sequence[str]) -> None:
        """Delete documents from the MongoDB collection."""
        self.sync_collection.delete_many({'namespace': self.namespace, 'key': {'$in': keys}})

    async def adelete_keys(self, keys: Sequence[str]) -> None:
        """Asynchronously delete documents from the MongoDB collection."""
        await self.async_collection.delete_many({'namespace': self.namespace, 'key': {'$in': keys}})