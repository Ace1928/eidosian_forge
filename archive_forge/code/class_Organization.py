...
class Organization(CompletableGithubObject):
    ...
    def create_fork(self, repo: Repository, name: Opt[str]=NotSet, default_branch_only: Opt[bool]=NotSet) -> Repository:
        """
        :calls: `POST /repos/{owner}/{repo}/forks <https://docs.github.com/en/rest/reference/repos#forks>`_
        """
        assert isinstance(repo, github.Repository.Repository), repo
        return repo.create_fork(self, name=name, default_branch_only=default_branch_only)
    ...
    def create_repo(self, name: str, description: Opt[str]=NotSet, homepage: Opt[str]=NotSet, private: Opt[bool]=NotSet, visibility: Opt[str]=NotSet, has_issues: Opt[bool]=NotSet, has_wiki: Opt[bool]=NotSet, has_downloads: Opt[bool]=NotSet, has_projects: Opt[bool]=NotSet, team_id: Opt[int]=NotSet, auto_init: Opt[bool]=NotSet, license_template: Opt[str]=NotSet, gitignore_template: Opt[str]=NotSet, allow_squash_merge: Opt[bool]=NotSet, allow_merge_commit: Opt[bool]=NotSet, allow_rebase_merge: Opt[bool]=NotSet, delete_branch_on_merge: Opt[bool]=NotSet, allow_update_branch: Opt[bool]=NotSet, is_template: Opt[bool]=NotSet, allow_auto_merge: Opt[bool]=NotSet, use_squash_pr_title_as_default: Opt[bool]=NotSet, squash_merge_commit_title: Opt[str]=NotSet, squash_merge_commit_message: Opt[str]=NotSet, merge_commit_title: Opt[str]=NotSet, merge_commit_message: Opt[str]=NotSet, custom_properties: Opt[dict[str, Any]]=NotSet) -> github.Repository.Repository:
        ...
        assert squash_merge_commit_message in ['PR_BODY', 'COMMIT_MESSAGES', 'BLANK', NotSet], squash_merge_commit_message
        ...
        post_parameters = NotSet.remove_unset_items({'name': name, 'description': description, 'homepage': homepage, 'private': private, 'visibility': visibility, 'has_issues': has_issues, 'has_wiki': has_wiki, 'has_downloads': has_downloads, 'has_projects': has_projects, 'team_id': team_id, 'auto_init': auto_init, 'license_template': license_template, 'gitignore_template': gitignore_template, 'allow_squash_merge': allow_squash_merge, 'allow_merge_commit': allow_merge_commit, 'allow_rebase_merge': allow_rebase_merge, 'delete_branch_on_merge': delete_branch_on_merge, 'allow_update_branch': allow_update_branch, 'is_template': is_template, 'allow_auto_merge': allow_auto_merge, 'use_squash_pr_title_as_default': use_squash_pr_title_as_default, 'squash_merge_commit_title': squash_merge_commit_title, 'squash_merge_commit_message': squash_merge_commit_message, 'merge_commit_title': merge_commit_title, 'merge_commit_message': merge_commit_message, 'custom_properties': custom_properties})
        headers, data = self._requester.requestJsonAndCheck('POST', f'{self.url}/repos', input=post_parameters, headers={'Accept': Consts.repoVisibilityPreview})
        return github.Repository.Repository(self._requester, headers, data, completed=True)
    ...
    def create_team(self, name: str, repo_names: Opt[list[Repository]]=NotSet, permission: Opt[str]=NotSet, privacy: Opt[str]=NotSet, description: Opt[str]=NotSet, parent_team_id: Opt[int]=NotSet, maintainers: Opt[list[int]]=NotSet, notification_setting: Opt[str]=NotSet) -> Team:
        """
        ...
        post_parameters: dict[str, Any] = NotSet.remove_unset_items({'name': name, 'permission': permission, 'privacy': privacy, 'description': description, 'parent_team_id': parent_team_id, 'maintainers': maintainers, 'notification_setting': notification_setting})
    ...
    def create_variable(self, variable_name: str, value: str, visibility: str='all', selected_repositories: github.GithubObject.Opt[list[github.Repository.Repository]]=NotSet) -> github.OrganizationVariable.OrganizationVariable:
        """
        ...
        if visibility == 'selected':
            ...
            assert selected_repositories is NotSet
        post_parameters: dict[str, Any] = {'name': variable_name, 'value': value, 'visibility': visibility}
        if is_defined(selected_repositories):
        ...
        return github.OrganizationVariable.OrganizationVariable(requester=self._requester, headers={}, attributes={'name': variable_name, 'visibility': visibility, 'value': value, 'selected_repositories_url': f'{self.url}/actions/variables/{urllib.parse.quote(variable_name)}/repositories', 'url': self.url}, completed=False)
    ...
    def get_variables(self) -> PaginatedList[OrganizationVariable]:
        ...
        """
        return PaginatedList(github.OrganizationVariable.OrganizationVariable, self._requester, f'{self.url}/actions/variables', None, list_item='variables')
    ...
    def get_variable(self, variable_name: str) -> OrganizationVariable:
        ...
        """
        assert isinstance(variable_name, str), variable_name
        return github.OrganizationVariable.OrganizationVariable(requester=self._requester, headers={}, attributes={'url': f'{self.url}/actions/variables/{urllib.parse.quote(variable_name)}'}, completed=False)
    ...
    def edit(self, billing_email: Opt[str]=NotSet, blog: Opt[str]=NotSet, company: Opt[str]=NotSet, description: Opt[str]=NotSet, email: Opt[str]=NotSet, location: Opt[str]=NotSet, name: Opt[str]=NotSet) -> None:
        """
        ...
        post_parameters = NotSet.remove_unset_items({'billing_email': billing_email, 'blog': blog, 'company': company, 'description': description, 'email': email, 'location': location, 'name': name})
        headers, data = self._requester.requestJsonAndCheck('PATCH', self.url, input=post_parameters)
        self._useAttributes(data)

    def edit_hook(self, id: int, name: str, config: dict[str, str], events: Opt[list[str]]=NotSet, active: Opt[bool]=NotSet) -> Hook:
        """
    ...
    def get_issues(self, filter: Opt[str]=NotSet, state: Opt[str]=NotSet, labels: Opt[list[Label]]=NotSet, sort: Opt[str]=NotSet, direction: Opt[str]=NotSet, since: Opt[datetime]=NotSet) -> PaginatedList[Issue]:
        """
    ...
    def get_members(self, filter_: Opt[str]=NotSet, role: Opt[str]=NotSet) -> PaginatedList[NamedUser]:
        """
        ...
        return PaginatedList(github.NamedUser.NamedUser, self._requester, f'{self.url}/members', url_parameters)
    ...
    def get_repo(self, name: str) -> Repository:
        ...
        headers, data = self._requester.requestJsonAndCheck('GET', f'/repos/{self.login}/{name}', headers={'Accept': Consts.repoVisibilityPreview})
        return github.Repository.Repository(self._requester, headers, data, completed=True)

    def get_repos(self, type: Opt[str]=NotSet, sort: Opt[str]=NotSet, direction: Opt[str]=NotSet) -> PaginatedList[Repository]:
        """
        ...
        return PaginatedList(github.Repository.Repository, self._requester, f'{self.url}/repos', url_parameters, headers={'Accept': Consts.repoVisibilityPreview})
    ...
    def invitations(self) -> PaginatedList[NamedUser]:
        ...
        """
        return PaginatedList(github.NamedUser.NamedUser, self._requester, f'{self.url}/invitations', None, headers={'Accept': Consts.mediaTypeOrganizationInvitationPreview})
    ...
    def has_in_public_members(self, public_member: NamedUser) -> bool:
        ...
        """
        assert isinstance(public_member, github.NamedUser.NamedUser), public_member
        status, headers, data = self._requester.requestJson('GET', f'{self.url}/public_members/{public_member._identity}')
        return status == 204
    ...
    def create_migration(self, repos: list[str], lock_repositories: Opt[bool]=NotSet, exclude_attachments: Opt[bool]=NotSet) -> Migration:
        """
        ...
        post_parameters = NotSet.remove_unset_items({'repositories': repos, 'lock_repositories': lock_repositories, 'exclude_attachments': exclude_attachments})
        headers, data = self._requester.requestJsonAndCheck('POST', f'/orgs/{self.login}/migrations', input=post_parameters, headers={'Accept': Consts.mediaTypeMigrationPreview})
    ...
    def get_migrations(self) -> PaginatedList[Migration]:
        ...
        """
        return PaginatedList(github.Migration.Migration, self._requester, f'/orgs/{self.login}/migrations', None, headers={'Accept': Consts.mediaTypeMigrationPreview})
    ...
    def get_installations(self) -> PaginatedList[Installation]:
        ...
        """
        return PaginatedList(github.Installation.Installation, self._requester, f'{self.url}/installations', None, None, list_item='installations')
    ...
    def _useAttributes(self, attributes: dict[str, Any]) -> None:
        ...
        if 'default_repository_permission' in attributes:
        ...
        if 'public_members_url' in attributes:
        ...
        if 'two_factor_requirement_enabled' in attributes:
