...
class UniqueVariableNamesRule(ASTValidationRule):
    ...
    def enter_operation_definition(self, node: OperationDefinitionNode, *_args: Any) -> None:
        ...
        seen_variable_definitions = group_by(variable_definitions, attrgetter('variable.name.value'))
        for variable_name, variable_nodes in seen_variable_definitions.items():
            if len(variable_nodes) > 1:
                self.report_error(GraphQLError(f"There can be only one variable named '${variable_name}'.", [node.variable.name for node in variable_nodes]))