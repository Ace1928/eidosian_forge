...
class SyncAsyncLauncher:
    ...
    def __init__(self) -> None:
        ...
        self.log_performance: Callable[[FuncType, float, float], None] = lambda _, __, ___: None
        self.result: Callable[[], Any] = lambda: None

    async def _setup_logging_and_validation(self, func: FuncType, args: ArgsType, kwargs: KwargsType) -> None:
        """
        ...
        if asyncio.get_event_loop().is_running():
            self.logger = await asyncio.run_coroutine_threadsafe(alogging.AsyncLogger('SyncAsyncLauncher'), asyncio.get_event_loop())
            self.validate_result = await asyncio.run_coroutine_threadsafe(validate._validate_func_signature(func, args, kwargs), asyncio.get_event_loop())
            self.start_time = await asyncio.run_coroutine_threadsafe(alogging.start_time, asyncio.get_event_loop())
            self.end_time = await asyncio.run_coroutine_threadsafe(alogging.end_time, asyncio.get_event_loop())
            self.log_performance = await asyncio.run_coroutine_threadsafe(alogging.log_performance, asyncio.get_event_loop())
            self.result = await asyncio.run_coroutine_threadsafe(wrapper_logic.wrapper_logic(func, True, args, kwargs), asyncio.get_event_loop())
        else:
    ...
    async def async_wrapper(self, func: FuncType, *args: ArgsType, **kwargs: KwargsType) -> Any:
        """
        ...
        self.logger.info(f'Async call to {func.__name__} with args: {args} and kwargs: {kwargs}')
    ...
    async def async_wrapper(self, func: FuncType, *args: ArgsType, **kwargs: KwargsType) -> Any:
        ...
        """
        asyncio.run_coroutine_threadsafe(self._setup_logging_and_validation(func, args, kwargs), asyncio.get_event_loop())
        self.logger.info(f'Sync call to {func.__name__} with args: {args} and kwargs: {kwargs}')
        asyncio.run_coroutine_threadsafe(self.validate_result(), asyncio.get_event_loop())
        try:
            start_time: float = asyncio.run_coroutine_threadsafe(self.start_time(), asyncio.get_event_loop()).result()
            result: Any = asyncio.run_coroutine_threadsafe(func(*args, **kwargs), asyncio.get_event_loop()).result()
            end_time: float = asyncio.run_coroutine_threadsafe(self.end_time(), asyncio.get_event_loop()).result()
            asyncio.run_coroutine_threadsafe(self.log_performance(func, start_time, end_time), asyncio.get_event_loop())
            return result
        except Exception as e:
    ...
    async def async_sync_wrapper(self, func: FuncType) -> Union[FuncType, Callable[..., Any]]:
        """
    ...
    async def _cancel_outstanding_tasks(self) -> None:
        ...
        tasks: List[asyncio.Task] = [task for task in asyncio.all_tasks(loop) if task is not asyncio.current_task()]
        for task in tasks:
            task.cancel()
