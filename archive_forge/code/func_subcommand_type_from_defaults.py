...
def subcommand_type_from_defaults(defaults: Mapping[str, T], descriptions: Mapping[str, str]={}, *, prefix_names: bool=True) -> TypeForm[T]:
    ...
    """
    return Union.__getitem__(tuple((Annotated.__class_getitem__((type(v), subcommand(k, default=v, description=descriptions.get(k, ''), prefix_name=prefix_names))) for k, v in defaults.items())))