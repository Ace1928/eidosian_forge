...
class LobsterNeighbors(NearNeighbors):
    ...
    def __init__(self, structure: Structure, filename_icohp: str | None='ICOHPLIST.lobster', obj_icohp: Icohplist | None=None, are_coops: bool=False, are_cobis: bool=False, valences: list[float] | None=None, limits: tuple[float, float] | None=None, additional_condition: int=0, only_bonds_to: list[str] | None=None, perc_strength_icohp: float=0.15, noise_cutoff: float=0.1, valences_from_charges: bool=False, filename_charge: str | None=None, obj_charge: Charge | None=None, which_charge: str='Mulliken', adapt_extremum_to_add_cond: bool=False, add_additional_data_sg: bool=False, filename_blist_sg1: str | None=None, filename_blist_sg2: str | None=None, id_blist_sg1: str='ICOOP', id_blist_sg2: str='ICOBI') -> None:
        ...
        if add_additional_data_sg:
            ...
            self.bonding_list_1 = Icohplist(filename=self.filename_blist_sg1, are_coops=are_coops_id1, are_cobis=are_cobis_id1)
            if self.id_blist_sg2.lower() == 'icoop':
                ...
                are_coops_id2 = False
                are_cobis_id2 = True
            self.bonding_list_2 = Icohplist(filename=self.filename_blist_sg2, are_coops=are_coops_id2, are_cobis=are_cobis_id2)
        if additional_condition not in range(7):
            raise ValueError(f'Unexpected additional_condition={additional_condition!r}, must be one of {list(range(7))}')
            ...
            if valences_from_charges and filename_charge is not None:
                ...
                try:
                    ...
                    if additional_condition in [1, 3, 5, 6]:
                        raise ValueError('Valences cannot be assigned, additional_conditions 1, 3, 5 and 6 will not work')
        else:
        ...
        self._evaluate_ce(lowerlimit=self.lowerlimit, upperlimit=self.upperlimit, only_bonds_to=only_bonds_to, additional_condition=self.additional_condition, perc_strength_icohp=perc_strength_icohp, adapt_extremum_to_add_cond=adapt_extremum_to_add_cond)
