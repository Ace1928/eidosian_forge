...
def test_server_and_custom_headers(responder, read_method, parquet_engine):
    ...
    if parquet_engine is None:
        df_http = read_method(f'http://localhost:{responder}', storage_options={'User-Agent': custom_user_agent})
    else:
        df_http = read_method(f'http://localhost:{responder}', storage_options={'User-Agent': custom_user_agent}, engine=parquet_engine)
