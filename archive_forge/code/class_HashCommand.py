...
class HashCommand(Command):
    ...
    help_spec = Command.HelpSpec(help_name='hash', help_name_aliases=['checksum'], help_type='command_help', help_one_line_summary='Calculate file hashes', help_text=_DETAILED_HELP_TEXT, subcommand_help_text={})

    def get_gcloud_storage_args(self):
        gcloud_storage_map = GcloudStorageMap(gcloud_command=['storage', 'hash', _GCLOUD_FORMAT_STRING], flag_map={'-h': GcloudStorageFlag('--hex'), '-c': None, '-m': None})
        ...
        if sub_opts:
            for o, unused_a in sub_opts:
                if o == '-c':
                    ...
                    cloud_format_func = lambda digest: hashing_helper.Base64ToHexHash(digest).decode('ascii')
                elif o == '-m':
                    calc_md5 = True
                    found_hash_option = True
    ...
    def RunCommand(self):
        ...
        calc_crc32c, calc_md5, format_func, cloud_format_func, output_format = self._ParseOpts(self.sub_opts, self.logger)
        matched_one = False
        for url_str in self.args:
            for file_ref in self.WildcardIterator(url_str).IterObjects(bucket_listing_fields=['crc32c', 'customerEncryption', 'md5Hash', 'size']):
                matched_one = True
                url = StorageUrlFromString(url_str)
                file_name = file_ref.storage_url.object_name
                if StorageUrlFromString(url_str).IsFileUrl():
                    file_size = os.path.getsize(file_name)
                    self.gsutil_api.status_queue.put(FileMessage(url, None, time.time(), size=file_size, finished=False, message_type=FileMessage.FILE_HASH))
                    callback_processor = ProgressCallbackWithTimeout(file_size, FileProgressCallbackHandler(self.gsutil_api.status_queue, src_url=StorageUrlFromString(url_str), operation_name='Hashing').call)
                    hash_dict = self._GetHashClassesFromArgs(calc_crc32c, calc_md5)
                    with open(file_name, 'rb') as fp:
                        hashing_helper.CalculateHashesFromContents(fp, hash_dict, callback_processor=callback_processor)
                    self.gsutil_api.status_queue.put(FileMessage(url, None, time.time(), size=file_size, finished=True, message_type=FileMessage.FILE_HASH))
                else:
                    hash_dict = {}
                ...
                for name, digest in six.iteritems(hash_dict):
                    print('\tHash (%s):\t\t%s' % (name, format_func(digest) if url.IsFileUrl() else cloud_format_func(digest)))
        if not matched_one:
