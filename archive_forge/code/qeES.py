...
class UnicodeCharacterGeneratorGUI(tk.Tk):
    ...
    def generate_characters(self):
        ...
        This method retrieves the user-selected options from the GUI, such as font type, character size, custom Unicode ranges,
        output destination, output file, and batch size. It then configures the character text widget font based on the selected
        font type and size.

        The custom Unicode ranges provided by the user are processed and stored in the `custom_ranges` list. Each range is
        represented as a tuple of start and end values in hexadecimal format.

        The `generate_utf_characters` function is called with the `custom_ranges` to generate the UTF characters based on the
        provided ranges.

        The `generate_color_spectrum` method is called to generate a color spectrum for coloring the characters.

        Depending on the selected output destination, the generated characters are either displayed in the GUI's text widget or
        saved to a file. If the output destination is set to "console", the characters are displayed in the text widget using
        parallel processing to speed up the printing process. The characters are printed in batches, and each character is assigned
        a color from the generated color spectrum. The text widget is cleared before printing the characters.

        If the output destination is set to a file, the generated characters are saved to the specified output file. A message box
        is displayed to inform the user that the characters have been saved successfully.
        ...
        # Configure the character text widget font based on the selected font type and size
        self.character_text.configure(state="normal")
        self.character_text.configure(font=(font_type, character_size))

        # Process custom Unicode ranges provided by the user
        custom_ranges = []
        ...
        # Generate the color spectrum for coloring the characters
        colors = self.generate_color_spectrum()

        # Print or save the characters based on the selected output option
        if output_destination == "console":
            # Clear the text widget before printing the characters
            ...
            with multiprocessing.Pool() as pool:
                ...
                for batch in batches:
                    ...
                    for char, color in zip(batch, itertools.cycle(colors)):
                        # Insert the character with a unique color tag
                        self.character_text.insert("end", f"{char}", "color")
                        # Configure the color tag with the corresponding color
                        self.character_text.tag_config(
                            f"color_{color}", foreground=color
                        )
                        # Apply the color tag to the inserted character
                        self.character_text.tag_add(
                            f"color_{color}", "end-2c", "end-1c"
                        )
                    # Insert a newline after each batch
                    self.character_text.insert("end", "\n")
        else:
            # Save characters to the specified output file
            with open(output_file, "w", encoding="utf-8") as file:
                for char in characters:
                    file.write(f"{char}\n")
            # Display a message box to inform the user that the characters have been saved
