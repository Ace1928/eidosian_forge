...
class Trait(resource.Resource):
    ...
    _query_mapping = resource.QueryParameters('name', 'associated', include_pagination_defaults=False)
    ...
    def list(cls, session, paginated=True, base_path=None, allow_unknown_params=False, *, microversion=None, **params):
        ...
        for trait_name in data['traits']:
            trait = {'name': trait_name, **uri_params}
            value = cls.existing(microversion=microversion, connection=session._get_connection(), **trait)
