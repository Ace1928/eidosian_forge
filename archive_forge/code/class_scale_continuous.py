...
class scale_continuous(scale):
    ...
    def limits(self):
        ...
        elif self._limits is not None and (not self.range.is_empty()) and (len(self._limits) == len(self.range.range)):
            l1, l2 = self._limits
    ...
    def map(self, x: FloatArrayLike, limits: Optional[ScaleContinuousLimits]=None) -> FloatArrayLike:
        if limits is None:
            limits = self.limits
