...
class VMwareAPISession(object):
    ...
    def __init__(self, host, server_username, server_password, api_retry_count, task_poll_interval, scheme='https', create_session=True, wsdl_loc=None, pbm_wsdl_loc=None, port=443, cacert=None, insecure=True, pool_size=10, connection_timeout=None, op_id_prefix='oslo.vmware'):
    ...
    def _create_session(self):
        ...
        if self._session_id and self.is_current_session_active():
            LOG.debug('Current session: %s is active.', _trunc_id(self._session_id))
            return
        session_manager = self.vim.service_content.sessionManager
        LOG.debug('Logging into host: %s.', self._host)
        session = self.vim.Login(session_manager, userName=self._server_username, password=self._server_password, locale='en')
        self._session_id = session.key
        self._session_username = session.userName
        LOG.info('Successfully established new session; session ID is %s.', _trunc_id(self._session_id))
    ...
    def logout(self):
        ...
        if self._session_id:
            LOG.info('Logging out and terminating the current session with ID = %s.', _trunc_id(self._session_id))
            try:
                self.vim.Logout(self.vim.service_content.sessionManager)
                self._session_id = None
            except Exception:
                LOG.exception('Error occurred while logging out and terminating the current session with ID = %s.', _trunc_id(self._session_id))
        else:
    ...
    def _poll_task(self, task, ctx):
        ...
        if ctx is not None:
            ctx.update_store()
        try:
            task_info = self.invoke_api(vim_util, 'get_object_property', self.vim, task, 'info', skip_op_id=True)
        except exceptions.VimException:
            with excutils.save_and_reraise_exception():
                LOG.exception('Error occurred while reading info of task: %s.', task)
        else:
            ...
            if task_info.state in ['queued', 'running']:
                if hasattr(task_info, 'progress'):
                    LOG.debug('Task: %(task)s progress is %(progress)s%%.', {'task': task_detail, 'progress': task_info.progress})
                ...
                def get_completed_task():
                    ...
                    return task_detail
                LOG.debug('Task: %s completed successfully.', get_completed_task())
    ...
    def _get_error_message(self, lease):
        ...
        try:
            ...
            LOG.warning('Error occurred while reading error message for lease: %s.', lease, exc_info=True)
            return 'Unknown'