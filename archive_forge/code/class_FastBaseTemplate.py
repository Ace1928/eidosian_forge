...
class FastBaseTemplate(BasicTemplate):
    ...
    design = param.ClassSelector(class_=Design, default=Fast, is_instance=False, instantiate=False, doc='\n        A Design applies a specific design system to a template.')
    _css = [_ROOT / 'fast.css']
    ...
    def __init__(self, **params):
        ...
        self.param.update({p: v for p, v in self._design.theme.style.param.values().items() if p != 'name' and p in self.param and (p not in params)})
    ...
    def _update_vars(self):
        ...
        style.param.update({p: getattr(self, p) for p in style.param if p != 'name' and p in self.param})
