...
class GradientLLM(BaseLLM):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        if values['gradient_access_token'] is None or len(values['gradient_access_token']) < 10:
            raise ValueError('env variable `GRADIENT_ACCESS_TOKEN` must be set')
        if values['gradient_workspace_id'] is None or len(values['gradient_access_token']) < 3:
        ...
        try:
            ...
            logging.warning('DeprecationWarning: `GradientLLM` will use `pip install gradientai` in future releases of langchain.')
        except Exception:
            pass
        return values
    ...
    def _identifying_params(self) -> Mapping[str, Any]:
        ...
        return {**{'gradient_api_url': self.gradient_api_url}, **{'model_kwargs': _model_kwargs}}
    ...
    def _call(self, prompt: str, stop: Optional[List[str]]=None, run_manager: Optional[CallbackManagerForLLMRun]=None, **kwargs: Any) -> str:
        ...
        try:
            ...
            if response.status_code != 200:
                raise Exception(f'Gradient returned an unexpected response with status {response.status_code}: {response.text}')
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(**self._kwargs_post_request(prompt=prompt, kwargs=kwargs)) as response:
                    if response.status != 200:
                        raise Exception(f'Gradient returned an unexpected response with status {response.status}: {response.text}')
    ...
    async def _agenerate(self, prompts: List[str], stop: Optional[List[str]]=None, run_manager: Optional[AsyncCallbackManagerForLLMRun]=None, **kwargs: Any) -> LLMResult:
        """Run the LLM on the given prompt and input."""
        generations = []
        for generation in asyncio.gather(([self._acall(prompt, stop=stop, run_manager=run_manager, **kwargs)] for prompt in prompts)):
