...
def hue_to_rgb(hue):
    """
    Convert a hue value (in degrees) to an RGB color.
    This function assumes full saturation and value (brightness).
    """
    hue = hue % 360  # Ensure hue is within 0-360 degrees
    c = 1.0  # Saturation and value are both 1.0
    x = 1 - abs((hue / 60.0 % 2) - 1)
    m = 0.0  # No need to adjust for different ranges of RGB
    ...
    r, g, b = (r + m), (g + m), (b + m)  # Add m to match the formula (r, g, b) = (r+m, g+m, b+m)
    return r, g, b

def from_hpr_to_vbase4(hue, pitch, roll):
    """
    Custom method to convert HPR (Hue, Pitch, Roll) to a VBase4 object.
    This function uses the hue to determine color and ignores pitch and roll.
    """
...
# Example usage:
hue = 270  # A hue value somewhere on the color wheel
color_vector = from_hpr_to_vbase4(hue, 0, 0)
print(color_vector)
