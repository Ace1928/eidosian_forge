...
class HandshakePattern(object):
    REGEX_PATTERN_NAME_MODIFIERS = '([A-Z1-9]{1,4})([a-z0-9+]+)*'
    TEMPLATE_REPR = '{name}:\n{patterns}'
    TEMPLATE_REPR_PATTERNS_WITH_PRE = '{pre_patterns}\n  ...\n{message_patterns}'
    TEMPLATE_REPR_MESSAGE_SEND = '  -> {tokens}'
    TEMPLATE_REPR_MESSAGE_RECV = '  <- {tokens}'

    def __init__(self, name, message_patterns, initiator_pre_messages=None, responder_pre_message_pattern=None, interpret_as_bob=False):
        """
        :param name:
        :type name: str
        ...
        self._initiator_pre_message_pattern = initiator_pre_messages or tuple()
        self._responder_pre_message_pattern = responder_pre_message_pattern or tuple()
        self._interpret_as_bob = interpret_as_bob
