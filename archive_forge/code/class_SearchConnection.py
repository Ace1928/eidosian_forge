...
class SearchConnection(object):
    ...
    def build_query(self, q=None, bq=None, rank=None, return_fields=None, size=10, start=0, facet=None, facet_constraints=None, facet_sort=None, facet_top_n=None, t=None):
    ...
    def __call__(self, query):
        ...
        if 'messages' in data and 'error' in data:
            for m in data['messages']:
                if m['severity'] == 'fatal':
                    raise SearchServiceException('Error processing search %s => %s' % (params, m['message']), query)
        elif 'error' in data:
