...
def SetAdminUsers(messages, args, request):
  ...
  """
    ...
    if properties.VALUES.auth.credential_file_override.Get() is not None:
        raise gcloud_exceptions.RequiredArgumentException('--admin-users', 'Required if auth/credential_file_override is defined.')
    ...
    if service_account_override is not None:
        ...
        if default_account is None:
            raise gcloud_exceptions.RequiredArgumentException('--admin-users', 'Required if no account is active and --impersonate-service-account is undefined.')
