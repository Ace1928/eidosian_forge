...
class InMemoryVectorStore(VectorStore):
    ...
    def add_texts(self, texts: Iterable[str], metadatas: Optional[List[dict]]=None, **kwargs: Any) -> List[str]:
        ...
        for i, text in enumerate(texts):
            ...
            ids.append(doc_id)
            self.store[doc_id] = {'id': doc_id, 'vector': vectors[i], 'text': text, 'metadata': metadatas[i] if metadatas else {}}
        return ids
    ...
    def similarity_search_with_score_by_vector(self, embedding: List[float], k: int=4) -> List[Tuple[Document, float]]:
        ...
        for doc in self.store.values():
            ...
            docs_with_similarity.append((Document(page_content=doc['text'], metadata=doc['metadata']), similarity))
    ...
    def max_marginal_relevance_search_by_vector(self, embedding: List[float], k: int=4, fetch_k: int=20, lambda_mult: float=0.5, **kwargs: Any) -> List[Document]:
        ...
        mmr_chosen_indices = maximal_marginal_relevance(np.array(embedding, dtype=np.float32), [doc['vector'] for doc, _ in prefetch_hits], k=k, lambda_mult=lambda_mult)
        return [Document(page_content=prefetch_hits[idx][0]['text'], metadata=prefetch_hits[idx][0]['metadata']) for idx in mmr_chosen_indices]
    ...
    def max_marginal_relevance_search(self, query: str, k: int=4, fetch_k: int=20, lambda_mult: float=0.5, **kwargs: Any) -> List[Document]:
        ...
        return self.max_marginal_relevance_search_by_vector(embedding_vector, k, fetch_k, lambda_mult=lambda_mult)

    @classmethod
    def from_texts(cls, texts: List[str], embedding: Embeddings, metadatas: Optional[List[dict]]=None, **kwargs: Any) -> 'InMemoryVectorStore':
        store = cls(embedding=embedding)
        store.add_texts(texts=texts, metadatas=metadatas)
        return store

    @classmethod
    async def afrom_texts(cls, texts: List[str], embedding: Embeddings, metadatas: Optional[List[dict]]=None, **kwargs: Any) -> 'InMemoryVectorStore':
