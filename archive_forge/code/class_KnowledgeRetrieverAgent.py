...
class KnowledgeRetrieverAgent(Agent):
    ...
    def add_cmdline_args(argparser):
        ...
        parser.add_argument('--num-retrieved', type=int, default=7, help='how many passages to retrieve for each category')
        parser.add_argument('--add-token-knowledge', type='bool', default=False, help='Add knowledge token to retrieved knowledge')
    ...
    def __init__(self, opt, shared=None):
        ...
        self.model_path = os.path.join(opt['datapath'], 'models', 'wizard_of_wikipedia', 'full_dialogue_retrieval_model')
        if not shared:
    ...
    def _get_checked_sentence(self, knowledge_no_title):
        selector_obs = self._format_selector_observation(knowledge_no_title, episode_done=self.observation.get('episode_done', False))
    ...
    def act(self):
        ...
        if self.next_wizard is not None:
            self.maintain_retriever_history({'text': self.next_wizard}, actor_id='wizard')
        return act
