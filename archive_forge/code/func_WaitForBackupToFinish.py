...
def WaitForBackupToFinish(backup, max_wait_ms=1800000, exponential_sleep_multiplier=1.4, jitter_ms=1000, wait_ceiling_ms=180000, status_update=_BackupStatusUpdate, sleep_ms=2000, client=None):
    ...
    try:
        result = retryer.RetryOnResult(func=backup_poller.Poll, args=(backup,), should_retry_if=backup_poller.IsNotDone, sleep_ms=sleep_ms)
        log.Print('Backup completed. Backup state: {0}'.format(result.state))
        return result
    except retry.WaitException:
        raise WaitForCompletionTimeoutError('Timeout waiting for backup to complete. Backup is not completed, use "gcloud container backup-restore backups describe" command to check backup status.')