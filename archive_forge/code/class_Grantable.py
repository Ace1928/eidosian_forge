...
class Grantable(ABC):
        ...
        for grant in grants:
            for grantee in grant.to:
                ...
                if invalid_privileges:
                    ...
                    raise InvalidPrivilegeError(f'Cannot grant the following privileges for database entity of type {self.__class__.__name__}: {formatted_invalid_privileges}. Valid privileges for this entity include: {formatted_valid_privileges}.')
                else:
    ...
    def _safe_grant(self, grantee: Role, privileges: set[Privilege]) -> None:
        ...
        """
        if not self._exists():
            raise EntityExistsError(f'There is no {self.__class__.__name__} with the name {self.name}. The {self.__class__.__name__} must exist to grant privileges.')
        else:
