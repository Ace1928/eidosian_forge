...
def _PerformParallelUploadFileToObject(cls, args, thread_state=None):
  ...
  """
    ...
    with fp:
        preconditions = None
        dst_object_metadata = apitools_messages.Object(name=args.dst_url.object_name, bucket=args.dst_url.bucket_name, contentType=args.content_type, storageClass=args.storage_class)
        orig_prefer_api = gsutil_api.prefer_api
        try:
            if global_copy_helper_opts.canned_acl:
                gsutil_api.prefer_api = ApiSelector.XML
            ret = _UploadFileToObject(args.src_url, fp, args.file_length, args.dst_url, dst_object_metadata, preconditions, gsutil_api, cls.logger, cls, _ParallelCopyExceptionHandler, gzip_exts=None, allow_splitting=False, is_component=True, gzip_encoded=args.gzip_encoded)
        finally:
            if global_copy_helper_opts.canned_acl:
                gsutil_api.prefer_api = orig_prefer_api
    component = ret[2]
    WriteComponentToParallelUploadTrackerFile(args.tracker_file, args.tracker_file_lock, component, cls.logger, encryption_key_sha256=args.encryption_key_sha256)
    return ret