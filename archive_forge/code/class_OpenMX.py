...
class OpenMX(FileIOCalculator):
    ...
    """
    implemented_properties = ['free_energy', 'energy', 'energies', 'forces', 'stress', 'dipole', 'chemical_potential', 'magmom', 'magmoms', 'eigenvalues']
    ...
    def __init__(self, restart=None, ignore_bad_restart_file=FileIOCalculator._deprecated, label='./openmx', atoms=None, command=None, mpi=None, pbs=None, **kwargs):
        self.initialize_pbs(pbs)
        self.initialize_mpi(mpi)
        self.initialize_output_setting(**kwargs)
        FileIOCalculator.__init__(self, restart, ignore_bad_restart_file, label, atoms, command, **kwargs)
    ...
    def clean(self, prefix='test', queue_num=None):
        ...
        files = [fileName + '.cif', fileName + '.dden.cube', fileName + '.ene', fileName + '.md', fileName + '.md2', fileName + '.tden.cube', fileName + '.sden.cube', fileName + '.v0.cube', fileName + '.v1.cube', fileName + '.vhart.cube', fileName + '.den0.cube', fileName + '.bulk.xyz', fileName + '.den1.cube', fileName + '.xyz', pbs_Name + '.o' + str(queue_num), pbs_Name + '.e' + str(queue_num)]
        for f in files:
            try:
    ...
    def calculate(self, atoms=None, properties=None, system_changes=all_changes):
        ...
        if self.parameters.data_path is None:
            if 'OPENMX_DFT_DATA_PATH' not in os.environ:
                warnings.warn('Please either set OPENMX_DFT_DATA_PATH as anenviroment variable or specify "data_path" asa keyword argument')
        self.prind('Start Calculation')
        if properties is None:
            properties = self.implemented_properties
        try:
            Calculator.calculate(self, atoms, properties, system_changes)
            self.write_input(atoms=self.atoms, parameters=self.parameters, properties=properties, system_changes=system_changes)
    ...
    def write_input(self, atoms=None, parameters=None, properties=[], system_changes=[]):
        ...
        write_openmx(label=self.label, atoms=atoms, parameters=self.parameters, properties=properties, system_changes=system_changes)
    ...
    def set(self, **kwargs):
            ...
            if key in ['dat_arguments'] and isinstance(value, dict):
                ...
                for subkey in kwargs[key]:
                    if subkey not in default_dict:
                        ...
                        raise TypeError('Unknown subkeyword "{0}" of keyword "{1}".  Must be one of: {2}'.format(subkey, key, allowed))
        changed_parameters = {}
        for key, value in kwargs.items():
        ...
        if value is not None and (not (isinstance(value, (float, int)) and value > 0)):
            mess = "'%s' must be a positive number(in eV),                 got '%s'" % ('energy_cutoff', value)
    ...
    def get_command(self, processes, threads, runfile=None, outfile=None):
        ...
        if processes is None:
            ...
            if command is None:
                warnings.warn('Either specify OPENMX_COMMAND as an environment                variable or specify processes as a keyword argument')
        else:
            threads_string = ' -nt ' + str(threads)
            if threads is None:
                threads_string = ''
            command += 'mpirun -np ' + str(processes) + ' ' + self.command + ' %s ' + threads_string + ' |tee %s'
        if runfile is None:
        ...
        try:
            ...
            raise ValueError("The 'OPENMX_COMMAND' environment must " + 'be a format string' + ' with four string arguments.\n' + "Example : 'mpirun -np 4 openmx ./%s -nt 2 > ./%s'.\n" + "Got '%s'" % command)
        return command
    ...
    def get_bz_k_points(self):
        ...
        for i in range(n1):
            for j in range(n2):
                for k in range(n3):
                    bz_k_points.append((0.5 * float(2 * i - n1 + 1) / n1, 0.5 * float(2 * j - n2 + 1) / n2, 0.5 * float(2 * k - n3 + 1) / n3))
    ...
    def get_lattice_type(self):
        ...
        if abc.ptp() < 0.01 * min_lv:
            ...
            if abs(abc[0] - abc[1]).min() < 0.01 * min_lv:
                ...
                return 'orthorhombic'
        elif abs(abc[0] - abc[1]) < 0.01 * min_lv and abs(angles[2] - 120) < 1 and (abs(angles[:2] - 90).max() < 1):
            return 'hexagonal'
        else:
            return 'not special'
    ...
    def get_number_of_spins(self):
        try:
            ...
            elif self['scf_spinpolarization'] == 'on':
                return 2
            elif self['scf_spinpolarization'] == 'nc' or np.any(self['initial_magnetic_moments_euler_angles']) is not None:
                return 1
        except KeyError:
            return 1
