...
def _CreateParams(self, client, resource_manager_tags):
    resource_manager_tags_map = resource_manager_tags_utils.GetResourceManagerTags(resource_manager_tags)
    params = client.messages.InstanceGroupManagerParams
    additional_properties = [params.ResourceManagerTagsValue.AdditionalProperty(key=key, value=value) for key, value in sorted(six.iteritems(resource_manager_tags_map))]
    return params(resourceManagerTags=params.ResourceManagerTagsValue(additionalProperties=additional_properties))