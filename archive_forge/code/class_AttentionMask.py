...
class AttentionMask:
    ...
    def to(self, device: Optional[torch.device]=None, dtype: Optional[torch.dtype]=None) -> 'AttentionMask':
        assert device is None or isinstance(device, torch.device)
        assert dtype is None or isinstance(dtype, torch.dtype)
        assert device is not None or dtype is not None
        if (device and device == self.device or not device) and (dtype and dtype == self.dtype or not dtype):
            return self
