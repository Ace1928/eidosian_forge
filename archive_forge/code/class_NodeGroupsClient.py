...
class NodeGroupsClient(object):
    ...
    def SetNodeTemplate(self, node_group_ref, node_template):
        ...
        node_template_ref = util.ParseNodeTemplate(self.resources, node_template, project=node_group_ref.project, region=compute_util.ZoneNameToRegionName(node_group_ref.zone))
        set_request = self.messages.NodeGroupsSetNodeTemplateRequest(nodeTemplate=node_template_ref.RelativeName())
        request = self.messages.ComputeNodeGroupsSetNodeTemplateRequest(nodeGroupsSetNodeTemplateRequest=set_request, nodeGroup=node_group_ref.Name(), project=node_group_ref.project, zone=node_group_ref.zone)
        return self._service.SetNodeTemplate(request)

    def AddNodes(self, node_group_ref, additional_node_count):
        request = self.messages.ComputeNodeGroupsAddNodesRequest(nodeGroupsAddNodesRequest=self.messages.NodeGroupsAddNodesRequest(additionalNodeCount=additional_node_count), nodeGroup=node_group_ref.Name(), project=node_group_ref.project, zone=node_group_ref.zone)
        return self._service.AddNodes(request)

    def DeleteNodes(self, node_group_ref, nodes):
        request = self.messages.ComputeNodeGroupsDeleteNodesRequest(nodeGroupsDeleteNodesRequest=self.messages.NodeGroupsDeleteNodesRequest(nodes=nodes), nodeGroup=node_group_ref.Name(), project=node_group_ref.project, zone=node_group_ref.zone)
    ...
    def Patch(self, node_group_ref, args):
        ...
        set_request = self.messages.NodeGroup(autoscalingPolicy=autoscaling_policy_ref)
    ...
    def Update(self, node_group_ref, node_template=None, additional_node_count=None, delete_nodes=None, autoscaling_policy_args=None, share_setting_args=None):
        ...
        result = None
        result = self._WaitForResult(operation_poller, set_node_template_ref, 'Setting node template on [{0}] to [{1}].'.format(node_group_name, node_template)) or result
        result = self._WaitForResult(operation_poller, add_nodes_ref, 'Adding [{0}] nodes to [{1}].'.format(additional_node_count, node_group_name)) or result
        deleted_nodes_str = ','.join(map(str, delete_nodes or []))
        result = self._WaitForResult(operation_poller, delete_nodes_ref, 'Deleting nodes [{0}] in [{1}].'.format(deleted_nodes_str, node_group_name)) or result
        autoscaling_policy_str_list = []
        if autoscaling_policy_args:
            if autoscaling_policy_args.autoscaler_mode:
                mode_str = 'autoscaler-mode={0}'.format(autoscaling_policy_args.autoscaler_mode)
        ...
        result = self._WaitForResult(operation_poller, autoscaling_policy_ref, 'Updating autoscaling policy on [{0}] to [{1}].'.format(node_group_name, autoscaling_policy_str)) or result
        ...
        if share_setting_args:
            ...
            if share_setting_args.share_with:
                with_str = 'share-with={0}'.format(','.join(share_setting_args.share_with))
                share_setting_str_list.append(with_str)
        share_setting_str = ','.join(share_setting_str_list)
        result = self._WaitForResult(operation_poller, share_settings_ref, 'Updating share setting on [{0}] to [{1}].'.format(node_group_name, share_setting_str)) or result
        return result