...
class SslutilsTestCase(base.ServiceBaseTestCase):
    ...
    def test_is_enabled_no_ssl_cert_file_fails(self, exists_mock):
        ...
        self.conf.set_default('cert_file', '/no/such/file', group=sslutils.config_section)
    ...
    def test_is_enabled_no_ssl_key_file_fails(self, exists_mock):
        ...
        self.conf.set_default('cert_file', self.cert_file_name, group=sslutils.config_section)
        self.conf.set_default('key_file', '/no/such/file', group=sslutils.config_section)
        self.assertRaises(RuntimeError, sslutils.is_enabled, self.conf)
    ...
    def test_wrap_ca_file(self):
        self.conf.set_default('ca_file', self.ca_file_name, group=sslutils.config_section)
        ssl_kwargs = {'ca_certs': self.conf.ssl.ca_file, 'cert_reqs': ssl.CERT_REQUIRED}
