...
def ExpansionGatewayDeploymentExists(self):
    ...
    _, err = self._RunKubectl(['get', 'deploy', _EXPANSION_GATEWAY_NAME, '-n', 'istio-system'], None)
    if err:
        if 'NotFound' in err:
            return False
        raise exceptions.Error('Error retrieving the expansion gateway deployment: {}'.format(err))
    return True