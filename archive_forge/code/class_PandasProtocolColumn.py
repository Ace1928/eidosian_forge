...
class PandasProtocolColumn(ProtocolColumn):
    ...
    def dtype(self) -> Tuple[DTypeKind, int, str, str]:
        ...
        if isinstance(dtype, pandas.CategoricalDtype):
            ...
            _, bitwidth, c_arrow_dtype_f_str, _ = self._dtype_from_primitive_pandas_dtype(codes.dtype)
            dtype_cache = (DTypeKind.CATEGORICAL, bitwidth, c_arrow_dtype_f_str, '=')
    ...
    def get_chunks(self, n_chunks: Optional[int]=None) -> Iterable['PandasProtocolColumn']:
        ...
        if n_chunks is None or n_chunks == cur_n_chunks:
            ...
            for i in range(len(cum_row_lengths) - 1):
                yield PandasProtocolColumn(self._col.take_2d_labels_or_positional(row_positions=range(cum_row_lengths[i], cum_row_lengths[i + 1]), col_positions=None), allow_copy=self._col._allow_copy)
            return
        ...
        new_partitions = self._col._partition_mgr_cls.map_axis_partitions(0, self._col._partitions, lambda df: df, keep_partitioning=False, lengths=new_lengths)
        new_df = self._col.__constructor__(new_partitions, self._col.index, self._col.columns, new_lengths, self._col.column_widths)
        cum_row_lengths = np.cumsum([0] + new_df.row_lengths)
        for i in range(len(cum_row_lengths) - 1):
            yield PandasProtocolColumn(new_df.take_2d_labels_or_positional(row_positions=range(cum_row_lengths[i], cum_row_lengths[i + 1]), col_positions=None), allow_copy=self._allow_copy)
    ...
    def _get_data_buffer(self) -> Tuple[PandasProtocolBuffer, Any]:
        """
        ...
        if dtype[0] in (DTypeKind.INT, DTypeKind.UINT, DTypeKind.FLOAT, DTypeKind.BOOL):
            buffer = PandasProtocolBuffer(self._col.to_numpy().flatten(), allow_copy=self._allow_copy)
    ...
    def _get_offsets_buffer(self) -> Tuple[PandasProtocolBuffer, Any]:
        ...
        if self.dtype[0] == DTypeKind.STRING:
            ...
            raise NoOffsetsBuffer('This column has a fixed-length dtype so does not have an offsets buffer')
