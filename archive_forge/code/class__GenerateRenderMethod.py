...
class _GenerateRenderMethod:
    ...
    def visitControlLine(self, node):
        if node.isend:
            ...
            if not children or (all((isinstance(c, (parsetree.Comment, parsetree.ControlLine)) for c in children)) and all((node.is_ternary(c.keyword) or c.isend for c in children if isinstance(c, parsetree.ControlLine)))):
                self.printer.writeline('pass')
    ...
    def visitTextTag(self, node):
        ...
        for n in node.nodes:
            n.accept_visitor(self)
        if filtered:
            self.printer.writelines('finally:', '__M_buf, __M_writer = context._pop_buffer_and_writer()', '__M_writer(%s)' % self.create_filter_callable(node.filter_args.args, '__M_buf.getvalue()', False), None)
    ...
    def visitBlockTag(self, node):
        if node.is_anonymous:
            ...
            nameargs = node.get_argument_expressions(as_call=True)
            nameargs += ['**pageargs']
            self.printer.writeline("if 'parent' not in context._data or not hasattr(context._data['parent'], '%s'):" % node.funcname)
    ...
    def visitCallTag(self, node):
        ...
        self.printer.writelines("context.caller_stack.nextcaller = runtime.Namespace('caller', context, callables=ccall(__M_caller))", 'try:')
        self.printer.start_source(node.lineno)
        self.printer.writelines('__M_writer(%s)' % self.create_filter_callable([], node.expression, True), 'finally:', 'context.caller_stack.nextcaller = None', None)