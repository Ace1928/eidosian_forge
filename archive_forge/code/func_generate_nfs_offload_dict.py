...
def generate_nfs_offload_dict(module, array):
    ...
    if AC_REQUIRED_API_VERSION in api_version:
        ...
        for target in range(0, len(offload)):
            ...
            offload_info[offloadt] = {'status': offload[target]['status'], 'mount_point': offload[target]['mount_point'], 'protocol': offload[target]['protocol'], 'mount_options': offload[target]['mount_options'], 'address': offload[target]['address']}
