...
class NucliaUnderstandingAPI(BaseTool):
    ...
    description: str = 'A wrapper around Nuclia Understanding API endpoints. Useful for when you need to extract text from any kind of files. '
    ...
    def _run(self, action: str, id: str, path: Optional[str], text: Optional[str], run_manager: Optional[CallbackManagerForToolRun]=None) -> str:
        """Use the tool."""
        if action == 'push':
    ...
    async def _arun(self, action: str, id: str, path: Optional[str]=None, text: Optional[str]=None, run_manager: Optional[AsyncCallbackManagerForToolRun]=None) -> str:
    ...
    def _pushField(self, id: str, field: Any) -> str:
        ...
        response = requests.post(self._config['BACKEND'] + '/processing/push', headers={'content-type': 'application/json', 'x-stf-nuakey': 'Bearer ' + self._config['NUA_KEY']}, json=field)
        if response.status_code != 200:
