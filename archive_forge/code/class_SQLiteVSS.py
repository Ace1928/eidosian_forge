...
class SQLiteVSS(VectorStore):
    ...
    def __init__(self, table: str, connection: Optional[sqlite3.Connection], embedding: Embeddings, db_file: str='vss.db'):
        """Initialize with sqlite client with vss extension."""
        try:
            import sqlite_vss
        except ImportError:
            raise ImportError('Could not import sqlite-vss python package. Please install it with `pip install sqlite-vss`.')
        if not connection:
    ...
    def add_texts(self, texts: Iterable[str], metadatas: Optional[List[dict]]=None, **kwargs: Any) -> List[str]:
        ...
        data_input = [(text, json.dumps(metadata), json.dumps(embed)) for text, metadata, embed in zip(texts, metadatas, embeds)]
        self._connection.executemany(f'INSERT INTO {self._table}(text, metadata, text_embedding) VALUES (?,?,?)', data_input)
        self._connection.commit()
    ...
    @classmethod
    def from_texts(cls: Type[SQLiteVSS], texts: List[str], embedding: Embeddings, metadatas: Optional[List[dict]]=None, table: str='langchain', db_file: str='vss.db', **kwargs: Any) -> SQLiteVSS:
