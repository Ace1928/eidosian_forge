...
def collective_permute_eager_fallback(input: _atypes.TensorFuzzingAnnotation[TV_CollectivePermute_T], source_target_pairs: _atypes.TensorFuzzingAnnotation[_atypes.Int32], name, ctx) -> _atypes.TensorFuzzingAnnotation[TV_CollectivePermute_T]:
    ...
    _attrs = ('T', _attr_T)
    _result = _execute.execute(b'CollectivePermute', 1, inputs=_inputs_flat, attrs=_attrs, ctx=ctx, name=name)
    if _execute.must_record_gradient():
        _execute.record_gradient('CollectivePermute', _inputs_flat, _attrs, _result)
    _result, = _result
    return _result