...
def _create_multipart_upload_artifact(artifact_path):
    ...
    request_message = _get_request_message(CreateMultipartUpload(), schema={'path': [_assert_required, _assert_string], 'num_parts': [_assert_intlike]})
    path = request_message.path
    num_parts = request_message.num_parts
    artifact_repo = _get_artifact_repo_mlflow_artifacts()
    _validate_support_multipart_upload(artifact_repo)
    create_response = artifact_repo.create_multipart_upload(path, num_parts, artifact_path)
