...
class LimitTestCase(common.IdentityTests):
    ...
    def test_limit_create_with_options(self):
        ...
        params = {'region_id': region_id, 'registered_limit_id': registered_limit_id}
        ...
        params = {'project_id': project_id, 'service_id': service_id, 'resource_name': resource_name, 'resource_limit': 15, 'region_id': region_id, 'description': description}
        raw_output = self.openstack('limit create --project %(project_id)s --service %(service_id)s --resource-limit %(resource_limit)s --region %(region_id)s --description %(description)s %(resource_name)s' % params, cloud=SYSTEM_CLOUD)
        items = self.parse_show(raw_output)
        limit_id = self._extract_value_from_items('id', items)
        self.addCleanup(self.openstack, 'limit delete %s' % limit_id, cloud=SYSTEM_CLOUD)
        self.assert_show_fields(items, self.LIMIT_FIELDS)
    ...
    def test_limit_show(self):
        ...
        raw_output = self.openstack('limit show %s' % limit_id, cloud=SYSTEM_CLOUD)
        items = self.parse_show(raw_output)
        self.assert_show_fields(items, self.LIMIT_FIELDS)

    def test_limit_set_description(self):
        limit_id = self._create_dummy_limit()
        params = {'description': data_utils.arbitrary_string(), 'limit_id': limit_id}
        raw_output = self.openstack('limit set --description %(description)s %(limit_id)s' % params, cloud=SYSTEM_CLOUD)
        items = self.parse_show(raw_output)
        self.assert_show_fields(items, self.LIMIT_FIELDS)
    ...
    def test_limit_set_resource_limit(self):
        ...
        raw_output = self.openstack('limit set --resource-limit %(resource_limit)s %(limit_id)s' % params, cloud=SYSTEM_CLOUD)
        items = self.parse_show(raw_output)
        self.assert_show_fields(items, self.LIMIT_FIELDS)
    ...
    def test_limit_delete(self):
        ...
        raw_output = self.openstack('limit delete %s' % limit_id, cloud=SYSTEM_CLOUD)
