...
class ScalarsPlugin(base_plugin.TBPlugin):
    ...
    def __init__(self, context):
        ...
        self._version_checker = plugin_util._MetadataVersionChecker(data_kind='scalar', latest_known_version=0)

    def get_plugin_apps(self):
        return {'/scalars': self.scalars_route, '/scalars_multirun': self.scalars_multirun_route, '/tags': self.tags_route}
    ...
    def scalars_impl(self, ctx, tag, run, experiment, output_format):
        ...
        all_scalars = self._data_provider.read_scalars(ctx, experiment_id=experiment, plugin_name=metadata.PLUGIN_NAME, downsample=self._downsample_to, run_tag_filter=provider.RunTagFilter(runs=[run], tags=[tag]))
    ...
    def scalars_route(self, request):
        ...
        if tag is None or run is None:
            raise errors.InvalidArgumentError('Both run and tag must be specified: tag=%r, run=%r' % (tag, run))
        ctx = plugin_util.context(request.environ)
        experiment = plugin_util.experiment_id(request.environ)
        output_format = request.args.get('format')
        body, mime_type = self.scalars_impl(ctx, tag, run, experiment, output_format)
