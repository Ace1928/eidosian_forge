...
class FileContentsTests(SynchronousTestCase):
    ...
    def test_matches(self, contents: str, encoding: str) -> None:
        ...
        assert_that(fileContents(equal_to(contents)).matches(p, description), equal_to(True))
        assert_that(str(description), equal_to(''))

    @given(just("some text, it doesn't matter what"), sampled_from(['ascii', 'latin-1', 'utf-8']))
    def test_mismatches(self, contents: str, encoding: str) -> None:
        ...
        p.setContent(contents.encode(encoding))
        description = StringDescription()
        assert_that(fileContents(not_(anything())).matches(p, description), equal_to(False))
    ...
    def test_ioerror(self) -> None:
        ...
        assert_that(str(description), AllOf(contains_string('fileContents'), contains_string('FileNotFoundError'), contains_string(repr(p.path))))