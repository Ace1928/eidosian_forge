...
@cite_gibbs
@due.dcite(Doi('10.1103/PhysRevB.90.174107'), description='High-throughput computational screening of thermal conductivity, Debye temperature, and Grüneisen parameter using a quasiharmonic Debye model', path='pymatgen.analysis.quasiharmonic')
class QuasiHarmonicDebyeApprox:
    """Quasi-harmonic approximation."""

    def __init__(self, energies, volumes, structure, t_min=300.0, t_step=100, t_max=300.0, eos='vinet', pressure=0.0, poisson=0.25, use_mie_gruneisen=False, anharmonic_contribution=False) -> None:
        """
        Args:
        ...
        self.avg_mass = physical_constants['atomic mass constant'][0] * self.mass / self.natoms
        self.kb = physical_constants['Boltzmann constant in eV/K'][0]
        self.hbar = physical_constants['Planck constant over 2 pi in eV s'][0]
        self.gpa_to_ev_ang = 1 / 160.21766208
        self.gibbs_free_energy: list[float] = []
        self.temperatures: list[float] = []
        self.optimum_volumes: list[float] = []
        logger.info('Fitting E and V')
        self.eos = EOS(eos)
        self.ev_eos_fit = self.eos.fit(volumes, energies)
        self.bulk_modulus = self.ev_eos_fit.b0_GPa
    ...
    def optimize_gibbs_free_energy(self):
        ...
        Note: The data points for which the equation of state fitting fails
            are skipped.
        """
        temperatures = np.linspace(self.temperature_min, self.temperature_max, int(np.ceil((self.temperature_max - self.temperature_min) / self.temperature_step) + 1))
        for temp in temperatures:
            try:
                ...
                if len(temperatures) <= 1:
                    raise
                logger.info(f'EOS fitting failed, skipping temp={temp!r}')
    ...
    def optimizer(self, temperature):
        ...
        for idx, v in enumerate(self.volumes):
            G_V.append(self.energies[idx] + self.pressure * v * self.gpa_to_ev_ang + self.vibrational_free_energy(temperature, v))
    ...
    def debye_temperature(self, volume: float) -> float:
        ...
        debye = 2.9772e-11 * (volume / self.natoms) ** (-1.0 / 6.0) * f_sigma * np.sqrt(self.bulk_modulus / self.avg_mass)
        if self.anharmonic_contribution:
            gamma = self.gruneisen_parameter(0, self.ev_eos_fit.v0)
