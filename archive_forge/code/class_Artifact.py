...
class Artifact:
    ...
    def __init__(self, name: str, type: str, description: Optional[str]=None, metadata: Optional[Dict[str, Any]]=None, incremental: bool=False, use_as: Optional[str]=None) -> None:
        if not re.match('^[a-zA-Z0-9_\\-.]+$', name):
            raise ValueError(f'Artifact name may only contain alphanumeric characters, dashes, underscores, and dots. Invalid name: {name}')
        if type == 'job' or type.startswith('wandb-'):
            raise ValueError("Artifact types 'job' and 'wandb-*' are reserved for internal use. Please use a different type.")
        if incremental:
        ...
        self._added_objs: Dict[int, Tuple[data_types.WBValue, ArtifactManifestEntry]] = {}
        ...
        self._manifest: Optional[ArtifactManifest] = ArtifactManifestV1(self._storage_policy)
    ...
    def _from_id(cls, artifact_id: str, client: RetryingClient) -> Optional['Artifact']:
        ...
        response = client.execute(query, variable_values={'id': artifact_id})
    ...
    @classmethod
    def _from_name(cls, entity: str, project: str, name: str, client: RetryingClient) -> 'Artifact':
        query = gql('\n            query ArtifactByName(\n                $entityName: String!,\n                $projectName: String!,\n                $name: String!\n            ) {\n                project(name: $projectName, entityName: $entityName) {\n                    artifact(name: $name) {\n                        ...ArtifactFragment\n                    }\n                }\n            }\n            ' + cls._get_gql_artifact_fragment())
        response = client.execute(query, variable_values={'entityName': entity, 'projectName': project, 'name': name})
        attrs = response.get('project', {}).get('artifact')
        if attrs is None:
            raise ValueError(f'Unable to fetch artifact with name {entity}/{project}/{name}')
        return cls._from_attrs(entity, project, name, attrs, client)

    @classmethod
    def _from_attrs(cls, entity: str, project: str, name: str, attrs: Dict[str, Any], client: RetryingClient) -> 'Artifact':
        ...
        artifact._name = name
        aliases = [alias['alias'] for alias in attrs['aliases'] if alias['artifactCollection']['project']['entityName'] == entity and alias['artifactCollection']['project']['name'] == project and (alias['artifactCollection']['name'] == name.split(':')[0])]
        version_aliases = [alias for alias in aliases if util.alias_is_version_index(alias)]
        ...
        artifact._source_name = '{}:v{}'.format(attrs['artifactSequence']['name'], attrs['versionIndex'])
        ...
        artifact.metadata = cls._normalize_metadata(json.loads(attrs['metadata'] or '{}'))
        artifact._ttl_duration_seconds = artifact._ttl_duration_seconds_from_gql(attrs.get('ttlDurationSeconds'))
        artifact._ttl_is_inherited = True if attrs.get('ttlIsInherited') is None else attrs['ttlIsInherited']
        artifact._aliases = [alias for alias in aliases if not util.alias_is_version_index(alias)]
    ...
    def collection(self) -> ArtifactCollection:
        ...
        return ArtifactCollection(self._client, self.entity, self.project, base_name, self.type)
    ...
    def source_collection(self) -> ArtifactCollection:
        ...
        return ArtifactCollection(self._client, self.source_entity, self.source_project, base_name, self.type)
    ...
    def manifest(self) -> ArtifactManifest:
        ...
        if self._manifest is None:
            ...
            response = self._client.execute(query, variable_values={'entityName': self._entity, 'projectName': self._project, 'name': self._name})
    ...
    def save(self, project: Optional[str]=None, settings: Optional['wandb.wandb_sdk.wandb_settings.Settings']=None) -> None:
        ...
        if wandb.run is None:
            ...
            with wandb.init(entity=self._source_entity, project=project or self._source_project, job_type='auto', settings=settings) as run:
                if self._incremental:
    ...
    def _set_save_future(self, save_future: 'MessageFuture', client: RetryingClient) -> None:
    ...
    def wait(self, timeout: Optional[int]=None) -> 'Artifact':
        ...
        if self.is_draft():
            ...
            if not result:
                raise WaitTimeoutError('Artifact upload wait timed out, failed to fetch Artifact response')
    ...
    def _update(self) -> None:
        ...
        if response.get('AddAliasesInputInfoType'):
            ...
            if len(aliases_to_add) > 0:
                ...
                assert self._client is not None
                self._client.execute(add_mutation, variable_values={'artifactID': self.id, 'aliases': [{'entityName': self._entity, 'projectName': self._project, 'artifactCollectionName': self._name.split(':')[0], 'alias': alias} for alias in aliases_to_add]})
            if len(aliases_to_delete) > 0:
                delete_mutation = gql('\n                    mutation deleteAliases(\n                        $artifactID: ID!,\n                        $aliases: [ArtifactCollectionAliasInput!]!,\n                    ) {\n                        deleteAliases(\n                            input: {artifactID: $artifactID, aliases: $aliases}\n                        ) {\n                            success\n                        }\n                    }\n                    ')
                assert self._client is not None
                self._client.execute(delete_mutation, variable_values={'artifactID': self.id, 'aliases': [{'entityName': self._entity, 'projectName': self._project, 'artifactCollectionName': self._name.split(':')[0], 'alias': alias} for alias in aliases_to_delete]})
            self._saved_aliases = copy(self._aliases)
        else:
            aliases = [{'artifactCollectionName': self._name.split(':')[0], 'alias': alias} for alias in self._aliases]
        ...
        if 'ttlIsInherited' in fields:
            mutation_template = mutation_template.replace('_TTL_DURATION_SECONDS_TYPE_', '$ttlDurationSeconds: Int64,').replace('_TTL_DURATION_SECONDS_VALUE_', 'ttlDurationSeconds: $ttlDurationSeconds,').replace('_TTL_DURATION_SECONDS_FIELDS_', 'ttlDurationSeconds ttlIsInherited')
        else:
            if self._ttl_changed:
                termwarn('Server not compatible with setting Artifact TTLs, please upgrade the server to use Artifact TTL')
            mutation_template = mutation_template.replace('_TTL_DURATION_SECONDS_TYPE_', '').replace('_TTL_DURATION_SECONDS_VALUE_', '').replace('_TTL_DURATION_SECONDS_FIELDS_', '')
        mutation = gql(mutation_template)
        assert self._client is not None
        ttl_duration_input = self._ttl_duration_seconds_to_gql()
        response = self._client.execute(mutation, variable_values={'artifactID': self.id, 'description': self.description, 'metadata': util.json_dumps_safer(self.metadata), 'ttlDurationSeconds': ttl_duration_input, 'aliases': aliases})
    ...
    def add_file(self, local_path: str, name: Optional[str]=None, is_tmp: Optional[bool]=False) -> ArtifactManifestEntry:
    ...
    def add(self, obj: data_types.WBValue, name: StrPath) -> ArtifactManifestEntry:
        ...
        if not any((isinstance(obj, t) for t in allowed_types)):
            raise ValueError('Found object of type {}, expected one of {}.'.format(obj.__class__, allowed_types))
        obj_id = id(obj)
    ...
    def _add_local_file(self, name: StrPath, path: StrPath, digest: Optional[B64MD5]=None) -> ArtifactManifestEntry:
        with tempfile.NamedTemporaryFile(dir=get_staging_dir(), delete=False) as f:
            staging_path = f.name
            shutil.copyfile(path, staging_path)
            os.chmod(staging_path, 256)
        entry = ArtifactManifestEntry(path=name, digest=digest or md5_file_b64(staging_path), size=os.path.getsize(staging_path), local_path=staging_path)
        self.manifest.add_entry(entry)
    ...
    def get_path(self, name: StrPath) -> ArtifactManifestEntry:
        ...
        termwarn('Artifact.get_path(name) is deprecated, use Artifact.get_entry(name) instead.')
    ...
    def download(self, root: Optional[str]=None, allow_missing_references: bool=False, skip_cache: Optional[bool]=None, path_prefix: Optional[StrPath]=None) -> FilePathStr:
        ...
        if get_core_path() != '':
            return self._download_using_core(root=root, allow_missing_references=allow_missing_references)
        return self._download(root=root, allow_missing_references=allow_missing_references, skip_cache=skip_cache, path_prefix=path_prefix)
    ...
    @classmethod
    def should_download_entry(cls, entry: ArtifactManifestEntry, prefix: Optional[StrPath]) -> bool:
        if prefix is None:
            return True
        return cls.path_contains_dir_prefix(entry.path, prefix)

    def _download_using_core(self, root: str, allow_missing_references: bool=False) -> FilePathStr:
        import pathlib
        ...
        assert backend.interface
        handle = backend.interface.deliver_download_artifact(self.id, root, allow_missing_references)
        self._download(root=root, allow_missing_references=allow_missing_references)
    ...
    def _download(self, root: str, allow_missing_references: bool=False, skip_cache: Optional[bool]=None, path_prefix: Optional[StrPath]=None) -> FilePathStr:
        ...
        if nfiles > 5000 or size > 50 * 1024 * 1024:
            log = True
            termlog('Downloading large artifact {}, {:.2f}MB. {} files... '.format(self.name, size / (1024 * 1024), nfiles))
            start_time = datetime.now()
        download_logger = ArtifactDownloadLogger(nfiles=nfiles)

        def _download_entry(entry: ArtifactManifestEntry, api_key: Optional[str], cookies: Optional[Dict], headers: Optional[Dict]) -> None:
        ...
        download_entry = partial(_download_entry, api_key=_thread_local_api_settings.api_key, cookies=_thread_local_api_settings.cookies, headers=_thread_local_api_settings.headers)
        ...
        if log:
            ...
            termlog(f'Done. {hours}:{minutes}:{seconds:.1f}', prefix=False)
        return FilePathStr(root)

    @retry.retriable(retry_timedelta=timedelta(minutes=3), retryable_exceptions=requests.RequestException)
    def _fetch_file_urls(self, cursor: Optional[str], per_page: Optional[int]=5000) -> Any:
        query = gql('\n            query ArtifactFileURLs($id: ID!, $cursor: String, $perPage: Int) {\n                artifact(id: $id) {\n                    files(after: $cursor, first: $perPage) {\n                        pageInfo {\n                            hasNextPage\n                            endCursor\n                        }\n                        edges {\n                            node {\n                                name\n                                directUrl\n                            }\n                        }\n                    }\n                }\n            }\n            ')
        assert self._client is not None
        response = self._client.execute(query, variable_values={'id': self.id, 'cursor': cursor, 'perPage': per_page}, timeout=60)
    ...
    def files(self, names: Optional[List[str]]=None, per_page: int=50) -> ArtifactFiles:
    ...
    def _delete(self, delete_aliases: bool=False) -> None:
        ...
        assert self._client is not None
        self._client.execute(mutation, variable_values={'artifactID': self.id, 'deleteAliases': delete_aliases})
    ...
    def link(self, target_path: str, aliases: Optional[List[str]]=None) -> None:
        ...
        """
        if wandb.run is None:
            with wandb.init(entity=self._source_entity, project=self._source_project, job_type='auto', settings=wandb.Settings(silent='true')) as run:
    ...
    def used_by(self) -> List[Run]:
        ...
        assert self._client is not None
        response = self._client.execute(query, variable_values={'id': self.id})
        return [Run(self._client, edge['node']['project']['entityName'], edge['node']['project']['name'], edge['node']['name']) for edge in response.get('artifact', {}).get('usedBy', {}).get('edges', [])]
    ...
    def logged_by(self) -> Optional[Run]:
        ...
        if creator.get('name') is None:
            return None
        return Run(self._client, creator['project']['entityName'], creator['project']['name'], creator['name'])
    ...
    @staticmethod
    def _expected_type(entity_name: str, project_name: str, name: str, client: RetryingClient) -> Optional[str]:
        """Returns the expected type for a given artifact name and project."""
        query = gql('\n            query ArtifactType(\n                $entityName: String,\n                $projectName: String,\n                $name: String!\n            ) {\n                project(name: $projectName, entityName: $entityName) {\n                    artifact(name: $name) {\n                        artifactType {\n                            name\n                        }\n                    }\n                }\n            }\n            ')
        if ':' not in name:
            name += ':latest'
        response = client.execute(query, variable_values={'entityName': entity_name, 'projectName': project_name, 'name': name})
        return (((response.get('project') or {}).get('artifact') or {}).get('artifactType') or {}).get('name')
    ...
    def _load_manifest(self, url: str) -> None:
        with requests.get(url) as request:
            ...
            self._manifest = ArtifactManifest.from_manifest_json(json.loads(util.ensure_text(request.content)))
    ...
    def _get_gql_artifact_fragment() -> str:
        ...
        if 'ttlIsInherited' not in fields:
            return fragment.replace('ttlDurationSeconds', '').replace('ttlIsInherited', '')
        return fragment
    ...
    def _ttl_duration_seconds_from_gql(self, gql_ttl_duration_seconds: Optional[int]) -> Optional[int]:
