...
class AzureNodeDriverTests(LibcloudTestCase):
    ...
    def setUp(self):
        ...
        self.driver = Azure(self.TENANT_ID, self.SUBSCRIPTION_ID, self.APPLICATION_ID, self.APPLICATION_PASS)
    ...
    def test_locations_returned_successfully(self):
        ...
        self.assertEqual([loc.name for loc in locations], ['East US', 'East US 2', 'West US', 'Central US', 'South Central US', 'North Europe', 'West Europe', 'East Asia', 'Southeast Asia', 'Japan East', 'Japan West'])

    def test_sizes_returned_successfully(self):
        location = self.driver.list_locations()[0]
        sizes = self.driver.list_sizes(location=location)
        self.assertEqual([size.name for size in sizes], ['Standard_A0', 'Standard_A1', 'Standard_A2'])

    def test_ex_get_ratecard(self):
        ratecard = self.driver.ex_get_ratecard('0026P')
        self.assertEqual(set(ratecard.keys()), {'Currency', 'Locale', 'IsTaxIncluded', 'OfferTerms', 'Meters'})
    ...
    def test_create_node(self):
        ...
        node = self.driver.create_node('test-node-1', size, image, auth, location=location, ex_resource_group='000000', ex_storage_account='000000', ex_user_name='any_user', ex_network='000000', ex_subnet='000000', ex_use_managed_disks=True)
    ...
    def test_create_node_ex_os_disk_delete(self):
        ...
        image = AzureImage('1', '1', 'ubuntu', 'pub', location.id, self.driver)
        auth = NodeAuthPassword('any_password')
        node = self.driver.create_node('test-node-1', size, image, auth, location=location, ex_resource_group='000000', ex_storage_account='000000', ex_user_name='any_user', ex_network='000000', ex_subnet='000000', ex_use_managed_disks=True, ex_os_disk_delete=True)
        hardware_profile = node.extra['properties']['hardwareProfile']
        os_profile = node.extra['properties']['osProfile']
        storage_profile = node.extra['properties']['storageProfile']
        self.assertEqual(node.name, 'test-node-1')
    ...
    def test_create_node_compute_gallery_image(self):
        ...
        self.assertTrue('diskSizeGB' not in storage_profile['osDisk'])
        self.assertTrue('deleteOption' not in storage_profile['osDisk'])
        self.assertTrue(storage_profile['imageReference'], {'id': '/subscriptions/sub-id00/resourceGroups/resource-group00/providers/Microsoft.Compute/galleries/gallery00/images/image00'})
    ...
    def test_destroy_node(self, time_sleep_mock):
        ...
        AzureMockHttp.responses = [lambda f: (httplib.OK, None, {}, 'OK'), lambda f: error(BaseHTTPError, code=404, message='Not found')]
    ...
    def test_destroy_node__retry(self, time_sleep_mock):
        ...
        def error(e, **kwargs):
            raise e(**kwargs)
        node = self.driver.list_nodes()[0]
        AzureMockHttp.responses = [lambda f: error(BaseHTTPError, code=202, message='Deleting'), lambda f: (httplib.OK, None, {}, 'OK'), lambda f: (httplib.OK, None, {}, 'OK'), lambda f: (httplib.OK, None, {}, 'OK'), lambda f: error(BaseHTTPError, code=404, message='Not found')]
        ret = self.driver.destroy_node(node)
        self.assertTrue(ret)
        self.assertEqual(4, time_sleep_mock.call_count)
    ...
    def test_destroy_node__async(self, time_sleep_mock):
        ...
        def error(e, **kwargs):
            raise e(**kwargs)
        node = self.driver.list_nodes()[0]
        AzureMockHttp.responses = [lambda f: error(BaseHTTPError, code=202, message='Deleting'), lambda f: error(BaseHTTPError, code=404, message='Not found')]
        ret = self.driver.destroy_node(node)
        self.assertTrue(ret)
    ...
    def test_destroy_node__failed(self):
        ...
        def error(e, **kwargs):
            raise e(**kwargs)
        node = self.driver.list_nodes()[0]
        AzureMockHttp.responses = [lambda f: error(BaseHTTPError, code=403, message='Forbidden')]
        with self.assertRaises(BaseHTTPError):
            self.driver.destroy_node(node)
    ...
    def test_create_volume(self):
        ...
        volume = self.driver.create_volume(2, 'test-disk-1', location, ex_resource_group='000000', ex_tags={'description': 'MyVolume'}, ex_zones=['1', '2', '3'], ex_iops=12345, ex_throughput=6789)
    ...
    def test_create_volume_snapshot(self):
        location = self.driver.list_locations()[-1]
        volume = self.driver.list_volumes()[0]
        snap = self.driver.create_volume_snapshot(volume, 'test-snap-1', location=location, ex_resource_group='000000')
