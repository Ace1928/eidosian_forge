...
class BedrockBase(BaseModel, ABC):
    ...
    guardrails: Optional[Mapping[str, Any]] = {'id': None, 'version': None, 'trace': False}
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        try:
            ...
            values['region_name'] = get_from_dict_or_env(values, 'region_name', 'AWS_DEFAULT_REGION', default=session.region_name)
            client_params = {}
            ...
            raise ValueError(f'Could not load credentials to authenticate with AWS client. Please check that credentials in the specified profile name are valid. Bedrock error: {e}') from e
        return values
    ...
    async def _aprepare_input_and_invoke_stream(self, prompt: str, stop: Optional[List[str]]=None, run_manager: Optional[AsyncCallbackManagerForLLMRun]=None, **kwargs: Any) -> AsyncIterator[GenerationChunk]:
        _model_kwargs = self.model_kwargs or {}
        provider = self._get_provider()
        if stop:
            if provider not in self.provider_stop_sequence_key_name_map:
        ...
        body = json.dumps(input_body)
        response = await asyncio.get_running_loop().run_in_executor(None, lambda: self.client.invoke_model_with_response_stream(body=body, modelId=self.model_id, accept='application/json', contentType='application/json'))
