...
class CellularTypeData:
    ...
    def __init__(self,
        ...
        try:
            ...
            self.max_energy_storage = np.clip(np.random.uniform(150.0, 300.0, n_particles), 150.0, 1000.0)  # Energy storage
            self.sensory_sensitivity = np.clip(np.random.uniform(0.5, 1.5, n_particles), 0.1, 2.0)  # Sensory sensitivity
            self.short_term_memory = np.zeros(n_particles, dtype=np.float64)  # Short-term memory
            self.long_term_memory = np.zeros(n_particles, dtype=np.float64)  # Long-term memory
            self.memory_transfer_rate = np.clip(np.random.uniform(0.1, 0.9, n_particles), 0.01, 0.99)  # Memory transfer rate
            self.communication_range = np.clip(np.random.uniform(50.0, 200.0, n_particles), 20.0, 500.0)  # Communication range
            self.socialization_tendency = np.clip(np.random.uniform(0.0, 1.0, n_particles), 0.0, 1.0)  # Socialization tendency
            self.colony_building_skill = np.clip(np.random.uniform(0.0, 1.0, n_particles), 0.0, 1.0)  # Colony building skill
            self.cultural_influence = np.clip(np.random.uniform(0.0, 1.0, n_particles), 0.0, 1.0)  # Cultural influence
...
class CellularTypeManager:
    ...
    def _validate_type_data(self, data: CellularTypeData) -> bool:
        ...
        try:
            ...
            return all(hasattr(data, attr) and getattr(data, attr) is not None for attr in required_attrs)
        except Exception:
            return False
    ...
    def draw_cellular_type(self, ct: 'CellularTypeData') -> None:
        ...
        try:
            ...
            for i in range(0, len(alive_indices), self._particle_batch_size):
                ...
                for idx, (pos, energy, speed) in enumerate(zip(positions, energies, speed_factors)):
