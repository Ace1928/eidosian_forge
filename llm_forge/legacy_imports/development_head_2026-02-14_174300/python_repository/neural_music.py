...
# =============================================================================
# CUSTOM THREAD AND MEMORY SETTINGS FOR FINE-GRAINED CONTROL
# =============================================================================
# These settings can help maximize resource usage across CPU cores
# and enable GPU memory growth if available.

# Allow TensorFlow to grow GPU memory usage as needed (so it doesn't grab all memory at once).
...
# Optionally, set the number of parallel CPU threads for ops:
# Increase these if you have many CPU cores and want heavier CPU utilization.
# Example: set them to the number of logical cores or higher to push CPU usage further.
tf.config.threading.set_intra_op_parallelism_threads(0)  # 0 => Let TF decide
tf.config.threading.set_inter_op_parallelism_threads(0)  # 0 => Let TF decide
...
def download_dataset():
    """
    Downloads and extracts the MAESTRO dataset if not already present, into data/maestro-v2.0.0.
    """
...
def list_midi_files():
    """
    Lists all MIDI files under the downloaded maestro directory.
    """
...
def parse_midi_file(midi_file):
    """
    Parses a MIDI file into a PrettyMIDI object, returns None on failure.
    """
    try:
...
def extract_notes(midi_file):
    """
    Extracts notes (pitch, step, duration, velocity, etc.) from a single MIDI file.
...
def notes_to_midi(notes_df, out_file, instrument_name='Acoustic Grand Piano', velocity=100):
    """
    Converts a DataFrame of notes back into a MIDI file, saved to out_file.
    """
...
def plot_piano_roll(notes, count=None, title_suffix=''):
    """
    Plots the piano roll (time vs pitch) for the given notes DataFrame.
    """
...
def plot_distributions(notes, drop_percentile=2.5, title_suffix=''):
    """
    Plots distributions for pitch, step, and duration from a notes DataFrame.
    """
...
def create_sequences(dataset, seq_length, vocab_size):
    """
    Creates input-label sequences for model training from a tf.data.Dataset containing [pitch, step, duration].
    """
...
def mse_with_positive_pressure(y_true, y_pred):
    """
    Custom MSE loss that encourages step/duration predictions to remain >= 0.
    """
...
def advanced_build_model(seq_length, learning_rate, scale_factor):
    """
    Builds and compiles a more complex RNN model with multiple LSTM and Dense layers.
    Specifically designed to push hardware resources to their limit while staying robust.
    """
    inputs = tf.keras.Input(shape=(seq_length, 3), name='input_notes')
    x = inputs
    for i in range(3):  # Add 3 stacked LSTM layers
        units = 128 * scale_factor
...
def train_model(model, train_ds, epochs, checkpoint_dir, steps_per_epoch):
    """
    Trains the RNN model, resuming from any available checkpoints,
    and saves full model each epoch for continuity and interrupt-resilience.
    Provides a robust approach to memory usage by chunking data and caching to disk.
    """
    ...
    # BackupAndRestore ensures the model can be recovered after a crash/interrupt
    backup_restore_cb = tf.keras.callbacks.BackupAndRestore(
        backup_dir=str(checkpoint_dir / 'backup')
    )

    callbacks = [
        # Saves the entire model (not just weights) every epoch, ensuring continuity
...
def generate_next_note(notes, model, temperature):
    """
    Generates one note (pitch, step, duration) from the input sequence, given a trained model.
    """
    ...
    # Adjust pitch logits by temperature
    ...
    # Clamp step & duration to non-negative
    step = max(0, step)
...
def generate_music(model, initial_notes, num_predictions, temperature, out_file, instrument_name):
    """
    Generates a sequence of notes from a trained model and writes to a MIDI file.
    """
    generated_notes = []
    ...
    for _ in range(num_predictions):
        pitch, step_val, dur = generate_next_note(input_notes, model, temperature)
        start = prev_start + step_val
        end = start + dur
...
def visualize_generated_notes(generated_df):
    """
    Quickly visualizes newly generated notes with a piano roll and distributions.
    """
...
def automatic_setup():
    """
    Ensures data is downloaded, notes extracted, dataset is prepared,
    and a model is trained if needed. Uses advanced_build_model for the best performance.
    Dynamically offloads to disk as needed for memory-limited systems.
    """
    download_dataset()

    # Prepare the CSV with extracted notes if not present
    ...
    # For extremely large datasets, rely on caching to disk (offload as needed)
    buffer_size = int(min(n_notes - CONFIG['DEFAULT_SEQ_LENGTH'], 200000) * CONFIG['BUFFER_MULTIPLIER'])

    train_ds = (
        seq_ds
        .shuffle(buffer_size)
        .take(buffer_size)  # limit processing to avoid potential OOM
        .cache(str(pathlib.Path(CONFIG['CACHE_DIR']) / 'train_ds_cache'))  # cache to disk
        .repeat()
    ...
    # Build and train the advanced model
    model = advanced_build_model(
        CONFIG['DEFAULT_SEQ_LENGTH'],
        CONFIG['LEARNING_RATE'],
        CONFIG['ADVANCED_MODEL_SCALE']
    )
...
def main():
    """
    Main entry point: automatically sets up data, trains the model if needed,
    then generates multiple tracks of music. 
    Intended to push CPU, RAM, and disk usage to their limits or as close as possible
    on any machine, for maximum throughput and scale.
    """
    automatic_setup()
    ...
    # Load the first found model (could be improved to select among multiple if needed)
    ...
    # Scale the initial notes (pitch dimension gets divided by VOCAB_SIZE)
