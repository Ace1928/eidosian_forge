...
from eidos_config import DEFAULT_LLM_BACKEND, DEFAULT_LLM_MODEL, DEFAULT_SENTENCE_TRANSFORMER_MODEL, DEFAULT_TEMPERATURE
...
class EidosChatSession:
    ...
    def __init__(
        ...
        self.identity_system = identity_system or EidosMemorySystem(
            ...
            llm_controller=LLMController(
                backend=DEFAULT_LLM_BACKEND, model=DEFAULT_LLM_MODEL, chat_mode=True  # <-- ensures plain text usage
            ),
        )
        self.message_system = message_system or EidosMemorySystem(
            model_name=DEFAULT_SENTENCE_TRANSFORMER_MODEL, llm_backend=DEFAULT_LLM_BACKEND, llm_model=DEFAULT_LLM_MODEL
        )
    ...
    def handle_user_input(
        ...
        if is_key:
            ...
            key_event_id = self.key_events_system.create_unit(key_info, tags=["key_event", f"reason_{reason}"])
        ...
        combined_prompt = self.prompt_templates.format(
            "chat", identity_context=identity_text, conversation_history=conversation_history, user_input=user_input
        )
        ...
        ai_unit_id = self.message_system.create_unit(
            reply, tags=["ai_reply"], timestamp=datetime.fromtimestamp(ai_msg.timestamp).strftime("%Y%m%d%H%M")
        )
        ...
        if is_key:
            ...
            key_event_id = self.key_events_system.create_unit(
                key_info, tags=["key_event", "ai_generated", f"reason_{reason}"]
            )
            self.total_key_events += 1
    ...
    def _generate_ai_reply(self, prompt: str, temperature: float = DEFAULT_TEMPERATURE) -> str:
        ...
        try:
            response = llm_controller.get_completion(prompt, response_format=None, temperature=temperature)
    ...
    def _generate_streaming_reply(
        self, prompt: str, callback: StreamCallback, temperature: float = DEFAULT_TEMPERATURE
    ) -> str:
        """
    ...
    def summarize_conversation(self) -> str:
        ...
        messages_text = "\n".join(
            [f"{msg.role.capitalize()}: {msg.content}" for msg in self.active_conversation.messages]
        )
...
Summary:"""
        ...
        try:
            summary = self.identity_system.llm_controller.get_completion(
                prompt, response_format=None, temperature=0.3  # Lower temperature for more deterministic summaries
            )
