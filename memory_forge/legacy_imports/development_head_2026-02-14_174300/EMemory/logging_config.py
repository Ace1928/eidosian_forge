...
class PerformanceTracker:
    ...
    def __init__(self):
        self.timers = {}
        self.async_timers = {}
        self.stats = {"calls": {}, "total_time": {}, "max_time": {}, "min_time": {}}
    ...
    def get_stats(self, name: Optional[str] = None) -> Dict[str, Any]:
        ...
        if name:
            if name not in self.stats["calls"]:
                return {"calls": 0, "total_time": 0.0, "avg_time": 0.0, "max_time": 0.0, "min_time": 0.0}
...
def configure_logging(
    ...
    **kwargs,
    ...
    if COLORLOG_AVAILABLE and config.enable_colors:
        ...
        console_handler = RichHandler(console=console, show_time=True, show_path=True, enable_link_path=True)
        console_handler.setFormatter(logging.Formatter("%(message)s"))
        handlers.append(console_handler)
    else:
        # Standard console handler with emojis
        console_handler = logging.StreamHandler()
        emoji_formatter = EmojiLogFormatter(add_emojis_to_format(config.console_format), datefmt=config.date_format)
    ...
    if config.enable_file:
        ...
        try:
            ...
            file_handler = RotatingFileHandler(log_file, maxBytes=config.max_size, backupCount=config.backup_count)
        except Exception as e:
    ...
    if config.enable_json:
        ...
        try:
            ...
            json_handler = RotatingFileHandler(json_log_file, maxBytes=config.max_size, backupCount=config.backup_count)
        except Exception as e:
            logger = logging.getLogger(__name__)
    ...
    logging.info(
        f"Logging initialized on {platform_info['system']} {platform_info['release']} "
