...
class RecursiveCognition:
    ...
    def __init__(
        ...
        self.llm_controller = llm_controller or LLMController(
            backend="ollama", chat_mode=True  # Default to local execution  # Use chat mode for natural responses
        )
...
"""
    ...
    async def process_async(
        self, prompt: str, stream_callback: Optional[Callable[[str, bool], None]] = None, temperature: float = 0.7
    ) -> str:
        """
        ...
        try:
            ...
            if stream_callback:
                ...
                full_response = await asyncio.to_thread(
                    self.llm_controller.get_completion, prompt=prompt, temperature=temperature
                )
    ...
    def process_sync(self, prompt: str, temperature: float = 0.7) -> str:
        ...
        try:
            ...
            response = self.llm_controller.get_completion(prompt=prompt, temperature=temperature)
    ...
    async def _stream_response_async(
        self, prompt: str, stream_callback: Callable[[str, bool], None], temperature: float = 0.7
    ) -> str:
        """
