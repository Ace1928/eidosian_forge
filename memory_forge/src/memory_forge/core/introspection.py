...
class MemoryIntrospector:
    ...
    def get_organization_recommendations(self) -> List[str]:
        ...
        if stats.total_memories > 100:
            recommendations.append("Consider implementing periodic memory consolidation to merge related memories.")

        if stats.avg_importance > 0.7:
            recommendations.append("Many high-importance memories. Consider reviewing importance scores for accuracy.")

        if stats.by_type.get("episodic", 0) > 50 and stats.by_type.get("semantic", 0) < 10:
            recommendations.append("Heavy episodic vs semantic imbalance. Extract key facts into semantic memories.")

        if not stats.by_namespace.get("code"):
            recommendations.append("No code-related memories. Consider storing lessons from coding sessions.")
