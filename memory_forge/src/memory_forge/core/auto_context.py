...
class AutoContextEngine:
    ...
    def suggest_context(
        ...
        priority_namespaces = self.NAMESPACE_PRIORITIES.get(context_type, self.NAMESPACE_PRIORITIES["default"])
    ...
    def _score_memory(
        ...
        final_score = (
            sum(scores.get(key, 0) * weight for key, weight in self.WEIGHTS.items()) + namespace_bonus + tier_bonus
        )
    ...
    def format_suggestions(
        ...
        for i, sugg in enumerate(suggestions, 1):
            content_preview = (
                sugg.memory.content[:100] + "..." if len(sugg.memory.content) > 100 else sugg.memory.content
            )

            if format_type == "brief":
                lines.append(
                    f"  {i}. [{sugg.source_tier.value}] " f"(score:{sugg.relevance_score:.2f}) {content_preview}"
                )
            else:  # detailed
