...
class AffectModule:
    ...
    def tick(self, ctx: TickContext) -> None:
        ...
        emit_delta = clamp01(ctx.config.get("affect_emit_delta_threshold"), default=0.04)
        ...
        drives = intero_state.get("drives") if isinstance(intero_state.get("drives"), Mapping) else {}
        ...
        prev = state.get("modulators") if isinstance(state.get("modulators"), Mapping) else {}
        prev_mod = {str(k): clamp01(v, default=0.5) for k, v in prev.items()}

        targets = {
            "arousal": clamp01((0.55 * threat) + (0.45 * curiosity), default=0.5),
            "valence": clamp01((0.55 * energy) + (0.2 * curiosity) - (0.45 * threat), default=0.5),
            "stability": clamp01(1.0 - (0.7 * threat) - (0.2 * novelty_hunger), default=0.5),
        ...
        for key, target in targets.items():
            ...
            value = clamp01((1.0 - alpha) * prev_value + (alpha * target), default=prev_value)
        ...
        payload = normalize_workspace_payload(payload, fallback_kind="AFFECT", source_module=self.name)
        ctx.broadcast(
            self.name,
            payload,
