# `agent_forge/src/agent_forge/consciousness/bench`

<!-- EIDOS:DOCSYS:START -->
- Contract: `eidos.directory_doc.v1`
- Generated: `2026-03-13T00:19:21Z`
- Path: `agent_forge/src/agent_forge/consciousness/bench`

## What It Is

Managed directory documentation for `agent_forge/src/agent_forge/consciousness/bench`, generated from tracked files and directory structure.

## Why It Exists

This directory exists to hold the `forge` surface for `agent_forge` and keep that responsibility separate from adjacent forge concerns.

## How It Works

- Tracked files in scope: `8`
- Child directories: `0`
- Tests detected: `False`
- Python modules: `ablations, golden, red_team, reporting, scoring, tasks, trials`

## Contents

- No managed child directories detected.

## Prominent Files

- [`__init__.py`](./__init__.py)
- [`ablations.py`](./ablations.py)
- [`golden.py`](./golden.py)
- [`red_team.py`](./red_team.py)
- [`reporting.py`](./reporting.py)
- [`scoring.py`](./scoring.py)
- [`tasks.py`](./tasks.py)
- [`trials.py`](./trials.py)

## Strengths

- The directory exposes importable Python modules rather than only opaque assets.

## Weaknesses / Risks

- No directly associated test coverage was detected under the tracked file set for this directory.

## Next Steps

- Add focused tests or point this directory explicitly at its validating test surface.
- Keep this README synchronized with code and test changes through the managed documentation toolchain.

## References

- Parent README: [`agent_forge/src/agent_forge/consciousness/README.md`](../README.md)
- Forge README: [`agent_forge/README.md`](../../../../README.md)

## Accuracy Contract

- This README is generated from tracked repository structure and conservative code scanning.
- Behavior claims should be backed by tests, routes, or directly linked source files.
<!-- EIDOS:DOCSYS:END -->
