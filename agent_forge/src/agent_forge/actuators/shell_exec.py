...
from agent_forge.core.state import add_run  # DB write
...
@eidosian()
def run_step(
    base_dir: str, step_id: str, cmd: Sequence[str], *, cwd: str, budget_s: float, template: str | None = None
) -> dict:
    ...
    start = _now()
    try:
        rc, out, err = run_sandboxed(cmd, cwd=cwd, timeout_s=budget_s, cpu_quota_s=budget_s, mem_mb=1024)
        end = _now()
        r = add_run(base_dir, step_id, start, end, rc, len(out) + len(err), "")
        try:
            write_run_artifacts(
                base_dir, r.id, out, err, {"cmd": list(cmd), "cwd": str(cwd), "rc": int(rc), "status": "ok"}
            )
        except Exception:
            pass
        ...
        try:
            write_run_artifacts(base_dir, r.id, b"", b"", {"cmd": list(cmd), "cwd": str(cwd), "status": "timeout"})
        except Exception:
            pass
        ...
        try:
            write_run_artifacts(
                base_dir, r.id, b"", b"", {"cmd": list(cmd), "cwd": str(cwd), "status": "error", "error": str(e)}
            )
        except Exception:
            pass
