...
def summarize_import_status(status: dict[str, Any] | None) -> dict[str, Any]:
    payload = status if isinstance(status, dict) else {}
    gemini = payload.get('gemini') if isinstance(payload.get('gemini'), dict) else {}
    ...
    codex_event_total = _safe_int(codex.get('imported_event_total'))
    codex_last_imported_count = _safe_int(codex.get('last_imported_count'))
    imported_records = gemini_records + (codex_event_total or codex_thread_count)
    return {
        'last_sync_at': payload.get('last_sync_at'),
        'gemini_records': gemini_records,
        'codex_records': codex_event_total or codex_thread_count,
        'codex_thread_count': codex_thread_count,
        'codex_last_imported_count': codex_last_imported_count,
...
def import_gemini_journal(
    *,
    ...
    status['gemini']['imported_record_count'] = len(status['gemini']['imported_ids'])
    status['gemini']['last_imported_count'] = imported
...
def import_codex_rollouts(
    *,
    ...
    status['codex']['imported_thread_count'] = len(imported_threads)
    status['codex']['last_imported_count'] = imported
    status['codex']['imported_event_total'] = _safe_int(status['codex'].get('imported_event_total')) + imported
